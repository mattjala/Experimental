...
public class H5Ex_D_Transform {
  ...
  private static void writeData() {
    ...
    // Create the dataset transfer property list and define the transform
    // expression.
    try {
    ...
    // Create the dataset using the default properties. Unfortunately we must
    // save as a native type or the transform operation will fail.
    try {
      if ((file_id >= 0) && (filespace_id >= 0))
        dataset_id =
            H5.H5Dcreate(file_id, DATASET, HDF5Constants.H5T_NATIVE_INT,
    ...
    try {
      if ((dataset_id >= 0) && (dxpl_id >= 0))
        H5.H5Dwrite(dataset_id, HDF5Constants.H5T_NATIVE_INT,
                    HDF5Constants.H5S_ALL, HDF5Constants.H5S_ALL, dxpl_id,
                    dset_data);
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  private static void readData() {
    ...
    try {
      file_id = H5.H5Fopen(FILE, HDF5Constants.H5F_ACC_RDONLY,
    ...
    System.out.println("Data as written with transform  '" + TRANSFORM +
                       "' and read with transform  '" + RTRANSFORM + "'");
    for (int i = 0; i < DIM_X; i++) {
      System.out.print(" [");
      for (int j = 0; j < DIM_Y; j++)
        System.out.print(" " + dset_data[i][j] + " ");
