...
public class H5Ex_D_Nbit {
  ...
  enum H5Z_filter {
    ...
    private static final Map<Integer, H5Z_filter> lookup =
        new HashMap<Integer, H5Z_filter>();
  ...
  private static boolean checkNbitFilter() {
    ...
    try {
      ...
      if (((filter_info & HDF5Constants.H5Z_FILTER_CONFIG_ENCODE_ENABLED) ==
           0) ||
          ((filter_info & HDF5Constants.H5Z_FILTER_CONFIG_DECODE_ENABLED) ==
           0)) {
        System.out.println(
            "N-Bit filter not available for encoding and decoding.");
        return false;
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private static void writeData() throws Exception {
    ...
    try {
      ...
      // uncompressed size of 32 bits, but will have a size of 16 bits after
      // being packed by the N-Bit filter.
      dtype_id = H5.H5Tcopy(HDF5Constants.H5T_STD_I32LE);
      H5.H5Tset_precision(dtype_id, 16);
      H5.H5Tset_offset(dtype_id, 5);

      // Create the dataset creation property list, add the N-Bit filter and set
      // the chunk size.
  ...
  public static void main(String[] args) {
    /*
     * Check if N-Bit compression is available and can be used for both
     * compression and decompression. Normally we do not perform error checking
     * in these examples for the sake of clarity, but in this case we will make
     * an exception because this filter is an optional part of the hdf5 library.
     */
    try {
