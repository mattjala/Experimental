...
public class H5Ex_D_Alloc {
  ...
  enum H5D_space_status {
    ...
    private static final Map<Integer, H5D_space_status> lookup =
        new HashMap<Integer, H5D_space_status>();
  ...
  private static void allocation() {
    ...
    try {
      if ((file_id >= 0) && (filespace_id >= 0))
        dataset_id1 =
            H5.H5Dcreate(file_id, DATASETNAME1, HDF5Constants.H5T_NATIVE_INT,
                         filespace_id, HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
    } catch (Exception e) {
      e.printStackTrace();
    }
    ...
    try {
      if ((file_id >= 0) && (filespace_id >= 0) && (dcpl_id >= 0))
        dataset_id2 = H5.H5Dcreate(
            file_id, DATASETNAME2, HDF5Constants.H5T_NATIVE_INT, filespace_id,
    ...
    String the_space = " ";
    if (H5D_space_status.get(space_status) !=
        H5D_space_status.H5D_SPACE_STATUS_ALLOCATED)
      the_space += "not ";
    System.out.println("Space for " + DATASETNAME1 + " has" + the_space +
                       "been allocated.");
    System.out.println("Storage size for " + DATASETNAME1 +
                       " is: " + storage_size + " bytes.");
    ...
    System.out.println("Space for " + DATASETNAME2 + " has" + the_space +
                       "been allocated.");
    System.out.println("Storage size for " + DATASETNAME2 +
                       " is: " + storage_size + " bytes.");
    System.out.println();
