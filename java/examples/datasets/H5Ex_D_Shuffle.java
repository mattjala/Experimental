...
public class H5Ex_D_Shuffle {
    ...
    try {
      ...
      if (((filter_info & HDF5Constants.H5Z_FILTER_CONFIG_ENCODE_ENABLED) ==
          ...
          ((filter_info & HDF5Constants.H5Z_FILTER_CONFIG_DECODE_ENABLED) ==
           0)) {
        System.out.println(
            "Shuffle filter not available for encoding and decoding.");
        return false;
      }
    } catch (Exception e) {
      e.printStackTrace();
    ...
    try {
      if ((file_id >= 0) && (filespace_id >= 0) && (dcpl_id >= 0))
        dataset_id = H5.H5Dcreate(
            file_id, DATASETNAME, HDF5Constants.H5T_STD_I32LE, filespace_id,
    ...
    try {
      if (dcpl_id >= 0) {
        ...
        for (int indx = 0; indx < nfilters; indx++) {
          ...
          int filter_type = -1;
          filter_type =
              H5.H5Pget_filter(dcpl_id, indx, flags, cd_nelmts, cd_values, 120,
  ...
  public static void main(String[] args) {
    ...
    if (H5Ex_D_Shuffle.checkGzipFilter() &&
        H5Ex_D_Shuffle.checkShuffleFilter()) {
