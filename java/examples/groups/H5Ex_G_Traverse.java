...
public class H5Ex_G_Traverse {
  ...
  private static void OpenGroup() {
    ...
    try {
      ...
      H5.H5Literate(file_id, HDF5Constants.H5_INDEX_NAME,
...
class H5L_iter_callbackT implements H5L_iterate_t {
  public int callback(long group, String name, H5L_info_t info,
    ...
    int spaces =
        2 * (od.recurs + 1); // Number of white spaces to prepend to output.
        ...
        if (group_check(od, infobuf.token)) {
          ...
          return_val =
              H5.H5Literate_by_name(group, name, HDF5Constants.H5_INDEX_NAME,
                                    HDF5Constants.H5_ITER_NATIVE, 0L, iter_cb2,
                                    nextod, HDF5Constants.H5P_DEFAULT);
        }
        for (int i = 0; i < spaces; i++)
          System.out.print(" ");
        System.out.println("}");
  ...
  public boolean group_check(opdata od, H5O_token_t target_token) {
    if (od.obj_token.equals(target_token))
      ...
      return group_check(od.prev,
                         target_token); // Recursively examine the next node
  }
}
