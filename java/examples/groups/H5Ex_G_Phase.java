...
public class H5Ex_G_Phase {
  ...
  enum H5G_storage {
    ...
    private static final Map<Integer, H5G_storage> lookup =
        new HashMap<Integer, H5G_storage>();
  ...
  private static void CreateGroup() {
    ...
    // Set file access property list to allow the latest file format.This will
    // allow the library to create new format groups.
    try {
    ...
    try {
      if (fapl_id >= 0)
        file_id = H5.H5Fcreate(FILE, HDF5Constants.H5F_ACC_TRUNC,
                               HDF5Constants.H5P_DEFAULT, fapl_id);
    } catch (Exception e) {
      e.printStackTrace();
    }
    ...
    try {
      if ((file_id >= 0) && (gcpl_id >= 0))
        group_id = H5.H5Gcreate(file_id, name, HDF5Constants.H5P_DEFAULT,
                                gcpl_id, HDF5Constants.H5P_DEFAULT);
    } catch (Exception e) {
      e.printStackTrace();
    }

    // Add subgroups to "group" one at a time, print the storage type for
    // "group" after each subgroup is created.
    for (i = 1; i <= MAX_GROUPS; i++) {
      ...
      try {
        if (group_id >= 0) {
          ...
          System.out.print(ginfo.nlinks + " Group" +
                           (ginfo.nlinks == 1 ? " " : "s") +
                           ": Storage type is ");
          switch (H5G_storage.get(ginfo.storage_type)) {
          case H5G_STORAGE_TYPE_COMPACT:
            System.out.println(
                "H5G_STORAGE_TYPE_COMPACT"); // New compact format
            break;
          case H5G_STORAGE_TYPE_DENSE:
            System.out.println(
                "H5G_STORAGE_TYPE_DENSE"); // New dense (indexed) format
            break;
          case H5G_STORAGE_TYPE_SYMBOL_TABLE:
            System.out.println(
                "H5G_STORAGE_TYPE_SYMBOL_TABLE"); // Original format
            break;
    ...
    // Delete subgroups one at a time, print the storage type for "group" after
    // each subgroup is deleted.
