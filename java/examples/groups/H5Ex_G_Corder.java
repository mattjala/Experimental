...
public class H5Ex_G_Corder {
  ...
  private static void CreateGroup() throws Exception {
    ...
    try {
      ...
      file_id =
          H5.H5Fcreate(FILE, HDF5Constants.H5F_ACC_TRUNC,
                       HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);

      // Create group creation property list and enable link creation order
      // tracking.
      gcpl_id = H5.H5Pcreate(HDF5Constants.H5P_GROUP_CREATE);
      status = H5.H5Pset_link_creation_order(
          gcpl_id, HDF5Constants.H5P_CRT_ORDER_TRACKED +
                       HDF5Constants.H5P_CRT_ORDER_INDEXED);

      // Create primary group using the property list.
      if (status >= 0)
        group_id =
            H5.H5Gcreate(file_id, "index_group", HDF5Constants.H5P_DEFAULT,
                         gcpl_id, HDF5Constants.H5P_DEFAULT);

      try {
        /*
         * Create subgroups in the primary group. These will be tracked by
         * creation order. Note that these groups do not have to have the
         * creation order tracking property set.
         */
        subgroup_id =
            H5.H5Gcreate(group_id, "H", HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
        status = H5.H5Gclose(subgroup_id);
        subgroup_id =
            H5.H5Gcreate(group_id, "D", HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
        status = H5.H5Gclose(subgroup_id);
        subgroup_id =
            H5.H5Gcreate(group_id, "F", HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
        status = H5.H5Gclose(subgroup_id);
        subgroup_id =
            H5.H5Gcreate(group_id, "5", HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
        status = H5.H5Gclose(subgroup_id);
        ...
        for (i = 0; i < ginfo.nlinks; i++) {
          ...
          name = H5.H5Lget_name_by_idx(
              group_id, ".", HDF5Constants.H5_INDEX_NAME,
        ...
        for (i = 0; i < ginfo.nlinks; i++) {
          // Retrieve the name of the ith link in a group
          name = H5.H5Lget_name_by_idx(
              group_id, ".", HDF5Constants.H5_INDEX_CRT_ORDER,
              HDF5Constants.H5_ITER_INC, i, HDF5Constants.H5P_DEFAULT);
          System.out.println("Index " + i + ": " + name);
        }
      } catch (Exception e) {
