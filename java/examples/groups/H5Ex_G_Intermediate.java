...
public class H5Ex_G_Intermediate {
  ...
  private void CreateGroup() throws Exception {
    ...
    try {
      ...
      // Create group_id creation property list and set it to allow creation of
      // intermediate group_ids.
      gcpl_id = H5.H5Pcreate(HDF5Constants.H5P_LINK_CREATE);
      H5.H5Pset_create_intermediate_group(gcpl_id, true);

      /*
       * Create the group_id /G1/G2/G3. Note that /G1 and /G1/G2 do not exist
       * yet. This call would cause an error if we did not use the previously
       * created property list.
       */
      group_id =
          H5.H5Gcreate(file_id, "/G1/G2/G3", gcpl_id, HDF5Constants.H5P_DEFAULT,
                       HDF5Constants.H5P_DEFAULT);
      // Print all the objects in the file_ids to show that intermediate
      // group_ids have been created.
      ...
      H5.H5Ovisit(file_id, HDF5Constants.H5_INDEX_NAME,
                  HDF5Constants.H5_ITER_NATIVE, iter_cb, iter_data);
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
  ...
  private class H5O_iter_callback implements H5O_iterate_t {
    public int callback(long group, String name, H5O_info_t info,
      ...
      // Check if the current object is the root group, and if not print the
      // full path name and type.
