...
public class H5Ex_T_ObjectReferenceAttribute {
  ...
  enum H5G_obj {
    ...
    H5G_TYPE(HDF5Constants
                 .H5O_TYPE_NAMED_DATATYPE); /* Object is a named data type */
    private static final Map<Integer, H5G_obj> lookup =
        new HashMap<Integer, H5G_obj>();
  ...
  private static void CreateDataset() {
    ...
    try {
      ...
      if ((file_id >= 0) && (dataspace_id >= 0)) {
        dataset_id =
            H5.H5Dcreate(file_id, DATASETNAME2, HDF5Constants.H5T_STD_I32LE,
    ...
    try {
      if (file_id >= 0)
        group_id =
            H5.H5Gcreate(file_id, GROUPNAME, HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
      if (group_id >= 0)
    ...
    try {
      if (file_id >= 0) {
        try {
          dset_data[0] = H5.H5Rcreate_object(file_id, GROUPNAME,
        ...
        try {
          dset_data[1] = H5.H5Rcreate_object(file_id, DATASETNAME2,
                                             HDF5Constants.H5P_DEFAULT);
        } catch (Throwable err) {
          err.printStackTrace();
        }
      ...
      try {
        ...
        if (dataspace_id >= 0) {
          dataset_id = H5.H5Dcreate(
              file_id, DATASETNAME, HDF5Constants.H5T_STD_I32LE, dataspace_id,
              HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
              HDF5Constants.H5P_DEFAULT);
          H5.H5Sclose(dataspace_id);
          dataspace_id = HDF5Constants.H5I_INVALID_HID;
      ...
      try {
        if ((dataset_id >= 0) && (dataspace_id >= 0))
          attribute_id =
              H5.H5Acreate(dataset_id, ATTRIBUTENAME, HDF5Constants.H5T_STD_REF,
                           dataspace_id, HDF5Constants.H5P_DEFAULT,
                           HDF5Constants.H5P_DEFAULT);
      } catch (Exception e) {
        e.printStackTrace();
        ...
        try {
          ...
          try {
            ...
            for (int indx = 0; indx < dims[0]; indx++) {
              ...
              try {
                object_id = H5.H5Ropen_object(dset_data[indx],
                                              HDF5Constants.H5P_DEFAULT,
                                              HDF5Constants.H5P_DEFAULT);
                try {
                  object_type = H5.H5Rget_obj_type3(dset_data[indx],
