...
public class H5Ex_T_Compound {
  ...
  static class Sensor_Datatype {
    ...
    static String[] memberNames = {"Serial number", "Location",
    ...
    static int[] memberStorage = {INTEGERSIZE, MAXSTRINGSIZE, DOUBLESIZE,
                                  DOUBLESIZE};
  ...
  static class Sensor {
    ...
    Sensor(int serial_no, String location, double temperature,
           double pressure) {
    ...
    void readBuffer(ByteBuffer databuf, int dbposition) {
      ...
      stringbuf.limit(dbposition + Sensor_Datatype.getOffset(1) +
                      MAXSTRINGSIZE);
  ...
  private static void CreateDataset() {
    ...
    // Initialize data.
    object_data[0] = (ArrayList) new Sensor(
                         1153, new String("Exterior (static)"), 53.23, 24.57)
                         .get();
    object_data[1] =
        (ArrayList) new Sensor(1184, new String("Intake"), 55.12, 22.95).get();
    object_data[2] = (ArrayList) new Sensor(1027, new String("Intake manifold"),
                                            103.55, 31.23)
                         .get();
    object_data[3] = (ArrayList) new Sensor(
                         1313, new String("Exhaust manifold"), 1252.89, 84.11)
                         .get();
    ...
    try {
      if ((file_id >= 0) && (dataspace_id >= 0) && (filetype_id >= 0))
        dataset_id =
            H5.H5Dcreate(file_id, DATASETNAME, filetype_id, dataspace_id,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
    ...
    try {
      if ((dataset_id >= 0) && (memtype_id >= 0))
        H5.H5DwriteVL(dataset_id, memtype_id, HDF5Constants.H5S_ALL,
  ...
  private static void ReadDataset() {
    ...
    try {
      if (file_id >= 0)
        dataset_id =
            H5.H5Dopen(file_id, DATASETNAME, HDF5Constants.H5P_DEFAULT);
    } catch (Exception e) {
      e.printStackTrace();
    }
    ...
    try {
      if ((dataset_id >= 0) && (memtype_id >= 0))
        H5.H5DreadVL(dataset_id, memtype_id, HDF5Constants.H5S_ALL,
                     HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT,
                     (Object[])object_data);
