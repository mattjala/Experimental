...
public class H5Ex_T_Commit {
  ...
  enum H5T_class {
    ...
    private static final Map<Long, H5T_class> lookup =
        new HashMap<Long, H5T_class>();
  ...
  private static class Sensor_Datatype {
    ...
    String[] memberNames = {"Serial number", "Location", "Temperature (F)",
                            "Pressure (inHg)"};
  ...
  private static void ReadDataType() {
    ...
    try {
      if (file_id >= 0)
        filetype_id =
            H5.H5Topen(file_id, DATATYPENAME, HDF5Constants.H5P_DEFAULT);
    } catch (Exception e) {
      e.printStackTrace();
    }
