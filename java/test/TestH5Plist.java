...
public class TestH5Plist {
  ...
  private static final char[] prop3_def = {
      'T', 'e', 'n', ' ', 'c',
  ...
  public void createPropClass() throws NullPointerException, HDF5Exception {
    ...
    try {
      plist_class_id =
          H5.H5Pcreate_class_nocb(HDF5Constants.H5P_ROOT, CLASS1_NAME);
    } catch (Throwable err) {
      err.printStackTrace();
  ...
  public void testH5P_genprop_basic_class() {
    ...
    try {
      ...
      try {
        cid1 =
            H5.H5Pcreate_class_nocb(HDF5Constants.H5P_FILE_CREATE, CLASS2_NAME);
      } catch (Throwable err) {
        err.printStackTrace();
  ...
  public void testH5P_genprop_basic_class_prop() {
    ...
    assertTrue("H5Pget_size " + PROP1_NAME + " size: " + size,
               size == PROP1_SIZE);
    ...
    assertTrue("H5Pget_size " + PROP2_NAME + " size: " + size,
               size == PROP2_SIZE);
    ...
    try {
      byte[] prop_value =
          new String(prop3_def).getBytes(StandardCharsets.UTF_8);
    ...
    assertTrue("H5Pget_size " + PROP3_NAME + " size: " + size,
               size == PROP3_SIZE);
  ...
  // Test basic generic property list code. Tests iterating over properties in a
  // generic class.
  @Test
  public void testH5P_genprop_class_iter() {
    ...
    assertTrue(
        "H5Piterate " + (((H5P_iter_data)iter_data).iterdata.get(0)).iter_count,
  ...
  public void testH5P_genprop_basic_list_prop() {
    ...
    try {
      ...
      try {
        ...
        H5.H5Pregister2_nocb(plist_class_id, PROP2_NAME, PROP2_SIZE,
                             prop_value);
      } catch (Throwable err) {
        err.printStackTrace();
        fail("H5Pregister2 plist_class_id: " + PROP2_NAME + err);
  ...
  //    // Test basic generic property list code. Tests callbacks for property
  //    lists in a generic class.
  //    @Test
  ...
  //            public int callback(long list_id, H5P_cls_create_func_t
  //            cls_data) {
  //                System.err.println("H5P_cls_create_callback enter");
  //                cdata cd =
  //                ((H5P_cls_create_data)cls_create_data).clsdata.get(0);
  //                cd.cls_count++;
  //                cd.cls_id = list_id;
  //                return 0;
  //            }
  //        }
  //        H5P_cls_create_func_cb cls_create_cb = new
  //        H5P_cls_create_callback();
  //
  ...
  //            public int callback(long list_id1, long list_id2,
  //            H5P_cls_copy_func_t cls_data) {
  //                cdata cd =
  //                ((H5P_cls_copy_data)cls_copy_data).clsdata.get(0);
  //                cd.cls_count++;
  //                cd.cls_id = list_id1;
  //                return 0;
  //            }
  ...
  //            {
  //                cdata cd =
  //                ((H5P_cls_close_data)cls_close_data).clsdata.get(0);
  //                cd.cls_count++;
  //                cd.cls_id = list_id;
  //                return 0;
  //            }
  ...
  //
  //        long    cid1 = HDF5Constants.H5I_INVALID_HID;        // Generic
  //        Property class ID long    cid2 = HDF5Constants.H5I_INVALID_HID; //
  //        Generic Property class ID long    lid1 =
  //        HDF5Constants.H5I_INVALID_HID;        // Generic Property list ID
  //        long    lid2 = HDF5Constants.H5I_INVALID_HID;        // Generic
  //        Property list ID long    lid3 = HDF5Constants.H5I_INVALID_HID; //
  //        Generic Property list ID long    nprops = -1;    // Number of
  //        properties in class
  //
  //        try {
  //            // Create a new generic class, derived from the root of the
  //            class hierarchy try {
  //                cid1 = H5.H5Pcreate_class(HDF5Constants.H5P_ROOT,
  //                CLASS1_NAME, cls_create_cb, cls_create_data, cls_copy_cb,
  //                cls_copy_data, cls_close_cb, cls_close_data);
  //            }
  //            catch (Throwable err) {
  //                err.printStackTrace();
  //                fail("H5Pcreate_class cid1: " + err);
  ...
  //                byte[] prop_value = HDFNativeData.intToByte(prop1_def);
  //
  //                H5.H5Pregister2(cid1, PROP1_NAME, PROP1_SIZE, prop_value,
  ...
  //                byte[] prop_value = HDFNativeData.floatToByte(prop2_def);
  //
  //                H5.H5Pregister2(cid1, PROP2_NAME, PROP2_SIZE, prop_value,
  //                null, null, null, null, null, null, null);
  //            }
  //            catch (Throwable err) {
  //                err.printStackTrace();
  ...
  //            try {
  //                byte[] prop_value = new
  //                String(prop3_def).getBytes(StandardCharsets.UTF_8);
  //
  //                H5.H5Pregister2(cid1, PROP3_NAME, PROP3_SIZE, prop_value,
  //                null, null, null, null, null, null, null);
  //            }
  //            catch (Throwable err) {
  //                err.printStackTrace();
  ...
  //            assertFalse("H5Pcreate
  //            ",((H5P_cls_create_data)cls_create_data).clsdata.isEmpty());
  //            assertTrue("H5Pcreate
  //            "+((H5P_cls_create_data)cls_create_data).clsdata.get(0).cls_id
  //            ,((H5P_cls_create_data)cls_create_data).clsdata.get(0).cls_id ==
  //            lid1); assertTrue("H5Pcreate
  ...
  //            assertFalse("H5Pcopy
  //            ",((H5P_cls_copy_data)cls_copy_data).clsdata.isEmpty());
  //            assertTrue("H5Pcopy
  //            "+((H5P_cls_copy_data)cls_copy_data).clsdata.get(0).cls_id
  //            ,((H5P_cls_copy_data)cls_copy_data).clsdata.get(0).cls_id ==
  //            lid3); assertTrue("H5Pcopy
  ...
  //            assertFalse("H5Pclose
  //            ",((H5P_cls_close_data)cls_close_data).clsdata.isEmpty());
  //            assertTrue("H5Pclose
  //            "+((H5P_cls_close_data)cls_close_data).clsdata.get(0).cls_id
  //            ,((H5P_cls_close_data)cls_copy_data).clsdata.get(0).cls_id ==
  //            lid1); assertTrue("H5Pclose
