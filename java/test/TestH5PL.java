...
public class TestH5PL {
  ...
  public void TestH5PLpaths() {
    try {
      ...
      nPaths = H5.H5PLsize();
      nTruePaths++;
      assertTrue("# paths should be " + nTruePaths + " but was " + nPaths,
                 nTruePaths == nPaths);
      ...
      // The (index+1) path should move down to fill the space when the path is
      // removed.
      index = nStartPaths;
  ...
  public void TestH5PLdlopen() {
    ...
    try {
      ...
      try {
        ...
        if (dcpl_id >= 0)
          H5.H5Pset_filter(dcpl_id, H5Z_FILTER_DYNLIB4,
                           HDF5Constants.H5Z_FLAG_MANDATORY, 4, cd_values);
      } catch (Exception e) {
        e.printStackTrace();
      ...
      try {
        if ((file_id >= 0) && (filespace_id >= 0) && (dcpl_id >= 0))
          dataset_id = H5.H5Dcreate(
              file_id, DATASETNAME, HDF5Constants.H5T_NATIVE_INT, filespace_id,
