...
public class TestH5D {
  ...
  private final void _createPDataset(long fid, long dsid, String name,
                                     long dcpl_val) {
    ...
    try {
      H5did0 = H5.H5Dcreate(fid, name, HDF5Constants.H5T_STD_I32BE, dsid,
                            HDF5Constants.H5P_DEFAULT, H5dcpl_id,
  ...
  private final void _createChunkDataset(long fid, long dsid, String name,
                                         long dapl) {
    ...
    try {
      H5did = H5.H5Dcreate(fid, name, HDF5Constants.H5T_STD_I32BE, dsid,
  ...
  private final void _createDataset(long fid, long dsid, String name,
                                    long dapl) {
    try {
      H5did = H5.H5Dcreate(fid, name, HDF5Constants.H5T_STD_I32BE, dsid,
  ...
  private final void _createVLStrDataset(String name, long dapl) {
    ...
    try {
      ...
      assertTrue("TestH5D._createVLStrDataset.H5Tis_variable_str",
                 H5.H5Tis_variable_str(H5dtid));
    } catch (Throwable err) {
      err.printStackTrace();
    ...
    try {
      H5did =
          H5.H5Dcreate(H5fid, name, H5dtid, H5dsid, HDF5Constants.H5P_DEFAULT,
  ...
  private final void _openH5file(String filename, String dsetname, long dapl) {
    try {
      H5fid = H5.H5Fopen(filename, HDF5Constants.H5F_ACC_RDONLY,
                         HDF5Constants.H5P_DEFAULT);
    } catch (Throwable err) {
      err.printStackTrace();
  ...
  public void testH5Dcreate() {
    ...
    try {
      dataset_id =
          H5.H5Dcreate(H5fid, "dset", HDF5Constants.H5T_STD_I32BE, H5dsid,
                       HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
                       HDF5Constants.H5P_DEFAULT);
    } catch (Exception err) {
      err.printStackTrace();
  ...
  public void testH5Dget_storage_size() {
    ...
    assertTrue("testH5Dget_storage_size: " + storage_size,
               storage_size == DIM_X * DIM_Y * 4);
  }
  ...
  public void testH5Dget_access_plist() {
    ...
    try {
      H5.H5Pget_cache(H5faplid, mdc_nelmts1, rdcc_nelmts1, rdcc_nbytes1,
                      rdcc_w01);
    } catch (Exception err) {
      err.printStackTrace();
    ...
    assertTrue("testH5Dget_access_plist: ",
               rdcc_nelmts2 == rdcc_nelmts2 && rdcc_nbytes2 == rdcc_nbytes2);
  }
  ...
  public void testH5Dget_space_status() {
    ...
    assertTrue("testH5Dget_space_status0 - H5.H5Dget_space_status: ",
               space_status0 ==
                   H5D_space_status.H5D_SPACE_STATUS_ALLOCATED.getCode());
  ...
  public void testH5Dfill_null() {
    ...
    try {
      H5.H5Dfill(null, HDF5Constants.H5T_NATIVE_UINT, buf_array,
                 HDF5Constants.H5T_NATIVE_UINT, H5dsid);
    } catch (Exception err) {
      err.printStackTrace();
      fail("H5.H5Dfill: " + err);
    }
  ...
  public void testH5Dfill() {
    ...
    // Fill selection in memory
    try {
      H5.H5Dfill(fill_value, HDF5Constants.H5T_NATIVE_UINT, buf_array,
                 HDF5Constants.H5T_NATIVE_UINT, H5dsid);
    } catch (Exception err) {
      err.printStackTrace();
      fail("H5.H5Dfill: " + err);
  ...
  public void testH5Diterate() {
    ...
    class H5D_iter_callback implements H5D_iterate_cb {
      public int callback(byte[] elem_buf, long elem_id, int ndim, long[] point,
    ...
    for (int indx = 0; indx < DIM_X; indx++)
      for (int jndx = 0; jndx < DIM_Y; jndx++) {
        ((H5D_iter_data)iter_data).fill_coords[2 * (indx * DIM_Y + jndx)] =
            indx;
        ((H5D_iter_data)iter_data).fill_coords[2 * (indx * DIM_Y + jndx) + 1] =
            jndx;
      } /* end for */
    ...
    int op_status = -1;
    try {
      op_status = H5.H5Diterate(buf_array, HDF5Constants.H5T_NATIVE_UINT,
                                H5dsid, iter_cb, iter_data);
    } catch (Throwable err) {
      err.printStackTrace();
      fail("H5.H5Diterate: " + err);
    }
  ...
  public void testH5Diterate_write() {
    ...
    for (int indx = 0; indx < DIM_X; indx++)
      for (int jndx = 0; jndx < DIM_Y; jndx++)
        assertTrue("H5.H5Diterate: [" + indx + "," + jndx + "] " +
  ...
  public void testH5Dvlen_get_buf_size() {
    String[] str_data = {"Parting", "is such", "sweet",    "sorrow.", "Testing",
                         "one",     "two",     "three.",   "Dog,",    "man's",
                         "best",    "friend.", "Diamonds", "are",     "a",
                         "girls!",  "S A",     "T U R",    "D A Y",   "night",
                         "That's",  "all",     "folks",    "!!!"};
    long vl_size = -1; /* Number of bytes used */
    long str_data_bytes = 0;
    for (int idx = 0; idx < str_data.length; idx++)
      str_data_bytes +=
          str_data[idx].length() + 1; // Account for terminating null

    _createVLStrDataset("dset", HDF5Constants.H5P_DEFAULT);

    try {
      if ((H5did >= 0) && (H5dtid >= 0))
        H5.H5DwriteVL(H5did, H5dtid, HDF5Constants.H5S_ALL,
                      HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT,
                      str_data);
    } catch (Exception e) {
      e.printStackTrace();
    ...
    assertTrue("H5Dvlen_get_buf_size " + vl_size + " == " + str_data_bytes,
               vl_size == str_data_bytes);
  }
  ...
  public void testH5Dvlen_string_buffer() throws Throwable {
    ...
    try {
      H5.H5Tset_size(H5dtid, HDF5Constants.H5T_VARIABLE);
      assertTrue("testH5Dvlen_string_buffer.H5Tis_variable_str",
                 H5.H5Tis_variable_str(H5dtid));
    } catch (Throwable err) {
      err.printStackTrace();
    ...
    try {
      ...
      assertTrue("testH5Dvlen_string_buffer.H5Tvlen_create: ",
                 dtype_str_id >= 0);
    } catch (Exception err) {
      if (dtype_str_id > 0)
        try {
    ...
    try {
      ...
      dset_str_id =
          H5.H5Dcreate(H5fid, dset_str_name, dtype_str_id, dspace_id,
                       HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
                       HDF5Constants.H5P_DEFAULT);
      assertTrue("testH5Dvlen_string_buffer: ", dset_str_id >= 0);

      H5.H5DwriteVL(dset_str_id, dtype_str_id, HDF5Constants.H5S_ALL,
                    HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT,
                    vl_str_data);
    } catch (Exception err) {
      if (dset_str_id > 0)
    ...
    try {
      H5.H5DreadVL(dset_str_id, dtype_str_id, HDF5Constants.H5S_ALL,
                   HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT,
                   vl_readbuf);
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  public void testH5Dvlen_write_read() {
    String[] str_wdata = {
        "Parting",  "is such", "sweet",  "sorrow.", "Testing", "one",
        "two",      "three.",  "Dog,",   "man's",   "best",    "friend.",
        "Diamonds", "are",     "a",      "girls!",  "S A",     "T U R",
        "D A Y",    "night",   "That's", "all",     "folks",   "!!!"};
    ...
    try {
      if ((H5did >= 0) && (H5dtid >= 0))
        H5.H5DreadVL(H5did, H5dtid, HDF5Constants.H5S_ALL,
                     HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT,
                     str_rdata);
    } catch (Exception e) {
      e.printStackTrace();
    ...
    for (int v = 0; v < DIM_X * DIM_Y; v++)
      assertTrue("testH5Dvlen_write_read " + str_wdata[v] +
                     " == " + str_rdata[v],
  ...
  public void testH5DVLwr() {
    ...
    try {
      ...
      try {
        ...
        assertTrue(dspace_id > 0);
        dset_int_id =
            H5.H5Dcreate(H5fid, dset_int_name, dtype_int_id, dspace_id,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT);
      ...
      try {
        ...
        assertTrue(dspace_id > 0);
        dset_dbl_id =
            H5.H5Dcreate(H5fid, dset_dbl_name, dtype_dbl_id, dspace_id,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT);
  ...
  public void testH5DVLwrVL() {
    ...
    try {
      ...
      try {
        ...
        assertTrue(dspace_id > 0);
        dset_int_id =
            H5.H5Dcreate(H5fid, dset_int_name, base_dtype_int_id, dspace_id,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT);
        assertTrue("testH5DVLwrVL: ", dset_int_id >= 0);

        H5.H5DwriteVL(dset_int_id, base_dtype_int_id, HDF5Constants.H5S_ALL,
                      HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT,
                      base_vl_int_data);
      } catch (Exception err) {
        if (dset_int_id > 0)
      ...
      try {
        H5.H5DreadVL(dset_int_id, base_dtype_int_id, HDF5Constants.H5S_ALL,
                     HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT,
                     base_vl_readbuf);
      } catch (Exception ex) {
        ex.printStackTrace();
  ...
  public void testH5DArray_string_buffer() throws Throwable {
    ...
    try {
      ...
      assertTrue("testH5DArray_string_buffer.H5Tarray_create: ",
                 dtype_str_id >= 0);
    } catch (Exception err) {
      if (dtype_str_id > 0)
        try {
  ...
  public void testH5DArrayenum_rw() {
    ...
    try {
      ...
      // Query member number and member index by member name, for enumeration
      // type.
      assertTrue("Can't get member number",
                 H5.H5Tget_nmembers(dtype_enum_id) == 5);
      ...
      assertTrue("Incorrect name for enum member",
                 enum_name.compareTo("YELLOW") == 0);

      ArrayList[] arr_enum_data = new ArrayList[4];
      try {
        // Write Integer data
        arr_enum_data[0] =
            new ArrayList<Integer>(Arrays.asList(10, 11, 12, 13));
        arr_enum_data[1] =
            new ArrayList<Integer>(Arrays.asList(11, 12, 13, 14));
        arr_enum_data[2] =
            new ArrayList<Integer>(Arrays.asList(12, 13, 14, 10));
        arr_enum_data[3] =
            new ArrayList<Integer>(Arrays.asList(13, 14, 10, 11));
        Class dataClass = arr_enum_data.getClass();
        assertTrue("testH5DArrayenum_wr.getClass: " + dataClass,
                   dataClass.isArray());

        try {
          dtype_arr_enum_id =
              H5.H5Tarray_create(HDF5Constants.H5T_STD_U32LE, 1, dims);
          assertTrue("testH5DArrayenum_wr.H5Tarray_create: ",
                     dtype_arr_enum_id >= 0);
        } catch (Exception err) {
        ...
        assertTrue(dspace_id > 0);
        dset_enum_id =
            H5.H5Dcreate(H5fid, dset_enum_name, dtype_arr_enum_id, dspace_id,
                         HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT,
                         HDF5Constants.H5P_DEFAULT);
