...
public class TestH5T {
  ...
  public void testH5Tenum_functions() {
    ...
    try {
      ...
      assertTrue("Can't get member number",
                 H5.H5Tget_nmembers(filetype_id) == 5);
      assertTrue("Can't get correct index number",
                 H5.H5Tget_member_index(filetype_id, "ORANGE") == 3);

      // Commit enumeration datatype and close it */
      H5.H5Tcommit(H5fid, enum_type, filetype_id, HDF5Constants.H5P_DEFAULT,
                   HDF5Constants.H5P_DEFAULT, HDF5Constants.H5P_DEFAULT);
  ...
  public void testH5Topaque_functions() {
    ...
    try {
      ...
      assertTrue("Incorrect tag for opaque type",
                 opaque_name.compareTo("opaque type") == 0);
    } catch (Throwable err) {
      err.printStackTrace();
  ...
  public void testH5Tvlen_create() {
    ...
    try {
      ...
      assertTrue("testH5Tvlen_create:H5Tget_class",
                 vlclass == HDF5Constants.H5T_VLEN);
      assertFalse("testH5Tis_variable_str:H5Tget_class",
                  vlclass == HDF5Constants.H5T_STRING);
    } catch (Throwable err) {
      err.printStackTrace();
  ...
  public void testH5Tis_variable_str() {
    ...
    try {
      ...
      int vlclass = H5.H5Tget_class(filetype_id);
      assertTrue("testH5Tis_variable_str:H5Tget_class",
                 vlclass == HDF5Constants.H5T_STRING);
      assertFalse("testH5Tvlen_create:H5Tget_class",
                  vlclass == HDF5Constants.H5T_VLEN);

      assertTrue("testH5Tis_variable_str:H5Tis_variable_str",
                 H5.H5Tis_variable_str(filetype_id));
  ...
  public void testH5Tcompound_functions() {
    ...
    try {
      ...
      // type.
      assertTrue("Can't get member number",
                 H5.H5Tget_nmembers(filetype_id) == 2);
      assertTrue("Can't get correct index number",
                 H5.H5Tget_member_index(filetype_id, "Lat") == 1);

      // We started to support this function for compound type in 1.8.6 release.
      int order = H5.H5Tget_order(filetype_id);
      assertFalse("Can't get order for compound type.",
                  order == HDF5Constants.H5T_ORDER_ERROR);
      ...
      assertTrue("Can't get correct class",
                 H5.H5Tdetect_class(filetype_id, HDF5Constants.H5T_FLOAT));
      // Make certain that an incorrect class is not detected
      assertFalse("Can get incorrect class",
                  H5.H5Tdetect_class(filetype_id, HDF5Constants.H5T_TIME));

      // Query member name by member index
      String index_name = H5.H5Tget_member_name(filetype_id, 0);
      assertTrue("Incorrect name for member index",
                 index_name.compareTo("Lon") == 0);
