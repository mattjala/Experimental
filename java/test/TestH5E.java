...
public class TestH5E {
  ...
  public void H5Eget_stack_class() {
    ...
    try {
      hdf_java_classid =
          H5.H5Eregister_class("HDF-Java-Error", "hdf-java", "2.5");
  ...
  public void testH5Epush() {
    ...
    try {
      try {
        maj_err_id = H5.H5Ecreate_msg(hdf_java_classid, HDF5Constants.H5E_MAJOR,
                                      "Error in Test");
        assertFalse("testH5Epush: H5.H5Ecreate_msg_major: " + maj_err_id,
                    maj_err_id < 0);
      } catch (Throwable err) {
        err.printStackTrace();
      ...
      try {
        min_err_id = H5.H5Ecreate_msg(hdf_java_classid, HDF5Constants.H5E_MINOR,
      ...
      assertFalse("testH5Epush: H5.H5Ecreate_stack: " + estack_id,
                  estack_id < 0);
      ...
      H5.H5Epush(estack_id, "TestH5E.java", err_func, 354, hdf_java_classid,
                 maj_err_id, min_err_id, err_msg);
  ...
  public void testH5Ewalk() {
    ...
    try {
      H5.H5Ewalk2(current_stackid, HDF5Constants.H5E_WALK_UPWARD, walk_cb,
                  walk_data);
    } catch (Throwable err) {
      err.printStackTrace();
      fail("testH5Ewalk:H5Ewalk2 " + err);
    }
    assertFalse("testH5Ewalk:H5Ewalk2 ",
                ((H5E_walk_data)walk_data).walkdata.isEmpty());
    assertTrue("testH5Ewalk:H5Ewalk2 " +
                   ((H5E_walk_data)walk_data).walkdata.size(),
