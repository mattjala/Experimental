...
public class TestH5VL {
  ...
  public void testH5VLnative_init() {
    try {
      ...
      is_registered = H5.H5VLis_connector_registered_by_name(
          HDF5Constants.H5VL_NATIVE_NAME);
      assertTrue("H5.H5VLis_connector_registered_by_name H5VL_NATIVE_NAME",
                 is_registered);

      is_registered = H5.H5VLis_connector_registered_by_name("FAKE_VOL_NAME");
      assertFalse("H5.H5VLis_connector_registered_by_name FAKE_VOL_NAME",
                  is_registered);

      is_registered = H5.H5VLis_connector_registered_by_value(
          HDF5Constants.H5VL_NATIVE_VALUE);
      assertTrue("H5.H5VLis_connector_registered_by_value H5VL_NATIVE_VALUE",
                 is_registered);
    } catch (Throwable err) {
      err.printStackTrace();
  ...
  public void testH5VLget_connector_id_by_name() {
    try {
      long native_id =
          H5.H5VLget_connector_id_by_name(HDF5Constants.H5VL_NATIVE_NAME);
  ...
  public void testH5VLget_connector_id_by_value() {
    try {
      long native_id =
          H5.H5VLget_connector_id_by_value(HDF5Constants.H5VL_NATIVE_VALUE);
      assertTrue("H5.H5VLget_connector_id_by_value H5VL_NATIVE_VALUE",
                 native_id >= 0);
      assertEquals(HDF5Constants.H5VL_NATIVE, native_id);
    } catch (Throwable err) {
      err.printStackTrace();
    ...
    try {
      ...
      if (connector == null)
        assertTrue("H5.H5VLget_connector_name H5VL_NATIVE",
                   native_name.compareToIgnoreCase(
                       HDF5Constants.H5VL_NATIVE_NAME) == 0);
    } catch (Throwable err) {
      err.printStackTrace();
