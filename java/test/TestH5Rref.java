...
public class TestH5Rref {
  ...
  public void testH5Rget_object() {
    ...
    try {
      ...
      assertTrue("testH5Rget_object: H5Tget_class",
    ...
    try {
      H5.H5Dread(H5did, HDF5Constants.H5T_STD_REF, HDF5Constants.H5S_ALL,
                 HDF5Constants.H5S_ALL, HDF5Constants.H5P_DEFAULT, refbuf);
    } catch (Exception err) {
      err.printStackTrace();
    ...
    for (int i = 0; i < ndims; i++) {
      try {
        ...
        assertTrue("testH5Rget_object: H5Rget_type[" + i + "]=" + ret_val,
                   ret_val == ref_type);
        if (!byteArrayCheck(refbuf[i])) {
          try {
            obj_type =
                H5.H5Rget_obj_type3(refbuf[i], HDF5Constants.H5P_DEFAULT);
  ...
  public void testH5Rget_obj_type3() {
    ...
    for (int i = 0; i < ndims; i++) {
      try {
        ret_val = H5.H5Rget_type(refbuf[i]);
        assertTrue("testH5Rget_obj_type3: H5Rget_type[" + i + "]=" + ret_val,
                   ret_val == ref_type);
        if (!byteArrayCheck(refbuf[i])) {
          try {
            obj_type =
  ...
  public void testH5Rget_region_dataset() {
    ...
    for (int i = 0; i < ndims; i++) {
      try {
        ...
        assertTrue("testH5Rget_region_dataset: H5Rget_type[" + i +
                       "]=" + ret_val,
                   ret_val == ref_type);
        try {
          loc_id = H5.H5Ropen_object(refbuf[i], HDF5Constants.H5P_DEFAULT,
                                     HDF5Constants.H5P_DEFAULT);
          assertTrue(loc_id >= 0);
          try {
            ...
            try {
              ...
              if (region_type == HDF5Constants.H5S_SEL_POINTS) {
                ...
                long coord[][] = {{6, 9}, {2, 2}, {8, 4}, {1, 6}, {2, 8},
                                  {3, 2}, {0, 4}, {9, 0}, {7, 1}, {3, 3}};
                try {
                  H5.H5Sget_select_elem_pointlist(loc_sid, 0, reg_npoints,
                                                  getcoord);
                  assertTrue("H5.H5Sget_select_elem_pointlist",
                             coord[0][0] == getcoord[0]);
                  assertTrue("H5.H5Sget_select_elem_pointlist",
                             coord[0][1] == getcoord[1]);
                  assertTrue("H5.H5Sget_select_elem_pointlist",
                             coord[1][0] == getcoord[2]);
                  assertTrue("H5.H5Sget_select_elem_pointlist",
                             coord[1][1] == getcoord[3]);
                  assertTrue("H5.H5Sget_select_elem_pointlist",
                             coord[2][0] == getcoord[4]);
                  assertTrue("H5.H5Sget_select_elem_pointlist",
                             coord[2][1] == getcoord[5]);
                ...
                long block[] = {8, 8};
                try {
                  H5.H5Sget_select_hyper_blocklist(loc_sid, 0, reg_nblocks,
                                                   getblocks);
              ...
              err2.printStackTrace();
              assertTrue("testH5Rget_region_dataset: H5Sget_select_type: " +
                             err2,
                         i > 1);
            }
  ...
  public void testH5Rget_region_attribute() {
    ...
    for (int i = 0; i < ndims; i++) {
      try {
        ret_val = H5.H5Rget_type(refbuf[i]);
        assertTrue("testH5Rget_region_attribute: H5Rget_type[" + i +
                       "]=" + ret_val,
                   ret_val == ref_type);
        try {
          loc_id = H5.H5Ropen_attr(refbuf[i], HDF5Constants.H5P_DEFAULT,
                                   HDF5Constants.H5P_DEFAULT);
          assertTrue(loc_id >= 0);
          if (!byteArrayCheck(refbuf[i])) {
            try {
