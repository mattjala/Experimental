 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1userblock(JNIEnv *env,
                                                             jclass clss,
                                                             jlong plist,
                                                             jlong size) {
  long sz = (long)size;
 ...
 * Signature: (J[J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1userblock(JNIEnv *env,
                                                             jclass clss,
                                                             jlong plist,
                                                             jlongArray size) {
  jboolean isCopy;
  ...
  PIN_LONG_ARRAY(ENVONLY, size, theArray, &isCopy,
                 "H5Pget_userblock: size not pinned");
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1sizes(
  ...
  if ((retVal = H5Pset_sizes((hid_t)plist, (size_t)sizeof_addr,
 ...
 * Signature: (J[J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1sizes(JNIEnv *env,
                                                         jclass clss,
                                                         jlong plist,
                                                         jlongArray size) {
  jboolean isCopy;
  ...
  if (arrLen < 2)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pget_sizes: size input array < 2 elements");

  PIN_LONG_ARRAY(ENVONLY, size, theArray, &isCopy,
                 "H5Pget_sizes: size not pinned");
 ...
 * Signature: (JII)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1sym_1k(JNIEnv *env,
                                                          jclass clss,
                                                          jlong plist, jint ik,
                                                          jint lk) {
  herr_t retVal = FAIL;
 ...
 * Signature: (J[I)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1sym_1k(JNIEnv *env,
                                                          jclass clss,
                                                          jlong plist,
                                                          jintArray size) {
  jboolean isCopy;
  ...
  PIN_INT_ARRAY(ENVONLY, size, theArray, &isCopy,
                "H5Pget_sym_k: size not pinned");

  if ((status = H5Pget_sym_k((hid_t)plist, (unsigned *)&(theArray[0]),
 ...
 * Signature: (JI)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1istore_1k(JNIEnv *env,
                                                             jclass clss,
                                                             jlong plist,
                                                             jint ik) {
  herr_t retVal = FAIL;
 ...
 * Signature: (J[I)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1istore_1k(JNIEnv *env,
                                                             jclass clss,
                                                             jlong plist,
                                                             jintArray ik) {
  jboolean isCopy;
  ...
  PIN_INT_ARRAY(ENVONLY, ik, theArray, &isCopy,
                "H5Pget_store_k: size not pinned");
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1file_1space_1page_1size(
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Pget_1file_1space_1page_1size(
 ...
 * Signature: (JIZJ)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1file_1space_1strategy(
    JNIEnv *env, jclass clss, jlong fcpl_id, jint strategy, jboolean persist,
    jlong threshold) {
  UNUSED(clss);

  if (H5Pset_file_space_strategy((hid_t)fcpl_id,
                                 (H5F_fspace_strategy_t)strategy, (bool)persist,
                                 (hsize_t)threshold) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J[Z[J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1file_1space_1strategy(
    JNIEnv *env, jclass clss, jlong fcpl_id, jbooleanArray persist,
    jlongArray threshold) {
  H5F_fspace_strategy_t thestrategy =
      H5F_FSPACE_STRATEGY_FSM_AGGR; /* Library default */
  jboolean isCopy;
  ...
  if (persist)
    PIN_BOOL_ARRAY(ENVONLY, persist, persistArray, &isCopy,
                   "H5Pget_file_space: persist not pinned");
  if (threshold)
    PIN_LONG_ARRAY(ENVONLY, threshold, thresholdArray, &isCopy,
                   "H5Pget_file_space: threshold not pinned");

  if ((status = H5Pget_file_space_strategy((hid_t)fcpl_id, &thestrategy,
                                           (bool *)persistArray,
  ...
  if (thresholdArray)
    UNPIN_LONG_ARRAY(ENVONLY, threshold, thresholdArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (persistArray)
    UNPIN_BOOL_ARRAY(ENVONLY, persist, persistArray,
                     (status < 0) ? JNI_ABORT : 0);
...
Java_hdf_hdf5lib_H5_H5Pget_1file_1space_1strategy_1persist(JNIEnv *env,
                                                           jclass clss,
                                                           jlong fcpl_id) {
  bool persist = false;
  herr_t status = FAIL;
  ...
  if ((status = H5Pget_file_space_strategy((hid_t)fcpl_id, NULL, &persist,
                                           NULL)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
Java_hdf_hdf5lib_H5_H5Pget_1file_1space_1strategy_1threshold(JNIEnv *env,
                                                             jclass clss,
                                                             jlong fcpl_id) {
  hsize_t threshold = 0;
  herr_t status = FAIL;
  ...
  if ((status = H5Pget_file_space_strategy((hid_t)fcpl_id, NULL, NULL,
                                           &threshold)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1shared_1mesg_1nindexes(
  ...
  if (nindexes > H5O_SHMESG_MAX_NINDEXES)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pset_shared_mesg_nindexes: number of indexes is "
                          "greater than H5O_SHMESG_MAX_NINDEXES");

  if ((retVal = H5Pset_shared_mesg_nindexes((hid_t)plist_id,
                                            (unsigned)nindexes)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1shared_1mesg_1index(
  ...
  if ((unsigned)mesg_type_flags > H5O_SHMESG_ALL_FLAG)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY,
        "H5Pset_shared_mesg_index: unrecognized flags in mesg_type_flags");
  ...
  if ((unsigned)index_num >= nindexes)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY,
        "H5Pset_shared_mesg_index: index_num is too large; no such index");

  if ((retVal = H5Pset_shared_mesg_index((hid_t)fcpl_id, (unsigned)index_num,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1shared_1mesg_1index(
  ...
  if (NULL == mesg_info)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_shared_mesg_index: mesg_info is NULL");
  ...
  if ((unsigned)index_num >= nindexes)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY,
        "H5Pget_shared_mesg_index: index_num is too large; no such index");

  PIN_INT_ARRAY(ENVONLY, mesg_info, theArray, &isCopy,
                "H5Pget_shared_mesg_index: input not pinned");

  if ((retVal = H5Pget_shared_mesg_index((hid_t)fcpl_id, (unsigned)index_num,
                                         (unsigned *)&(theArray[0]),
                                         (unsigned *)&(theArray[1]))) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1shared_1mesg_1phase_1change(
  ...
  if (max_list + 1 < min_btree)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pset_shared_mesg_phase_change: minimum B-tree "
                          "value is greater than maximum list value");
  if (max_list > H5O_SHMESG_MAX_LIST_SIZE)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pset_shared_mesg_phase_change: max list value is "
                          "larger than H5O_SHMESG_MAX_LIST_SIZE");
  if (min_btree > H5O_SHMESG_MAX_LIST_SIZE)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pset_shared_mesg_phase_change: min btree value is "
                          "larger than H5O_SHMESG_MAX_LIST_SIZE");

  if ((retVal = H5Pset_shared_mesg_phase_change(
           (hid_t)fcpl_id, (unsigned)max_list, (unsigned)min_btree)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1shared_1mesg_1phase_1change(
  ...
  if (NULL == size)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_shared_mesg_phase_change: size is NULL");

  PIN_INT_ARRAY(ENVONLY, size, theArray, &isCopy,
                "H5Pget_shared_mesg_phase_change: input not pinned");

  if ((retVal = H5Pget_shared_mesg_phase_change(
           (hid_t)fcpl_id, (unsigned *)&(theArray[0]),
           (unsigned *)&(theArray[1]))) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
