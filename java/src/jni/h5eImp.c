 ...
 * Signature: (J)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Eauto_1is_1v2(JNIEnv *env,
                                                               jclass clss,
                                                               jlong stk_id) {
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Eregister_1class(
    JNIEnv *env, jclass clss, jstring cls_name, jstring lib_name,
    jstring version) {
  ...
  if (NULL == version)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Eregister_class: version string is NULL");

  PIN_JAVA_STRING(ENVONLY, cls_name, the_cls_name, NULL,
                  "H5Eregister_class: class name not pinned");
  PIN_JAVA_STRING(ENVONLY, lib_name, the_lib_name, NULL,
                  "H5Eregister_class: lib name not pinned");
  PIN_JAVA_STRING(ENVONLY, version, the_version, NULL,
                  "H5Eregister_class: version string not pinned");

  if ((ret_val = H5Eregister_class(the_cls_name, the_lib_name, the_version)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Eunregister_1class(JNIEnv *env,
                                                                jclass clss,
                                                                jlong cls_id) {
  UNUSED(clss);
  ...
  if (cls_id < 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Eunregister_class: invalid error class ID");
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Eclose_1msg(JNIEnv *env,
                                                         jclass clss,
                                                         jlong err_id) {
  UNUSED(clss);
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Ecreate_1msg(
  ...
  if (NULL == err_msg)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Ecreate_msg: error message string is NULL");

  PIN_JAVA_STRING(ENVONLY, err_msg, the_err_msg, NULL,
                  "H5Ecreate_msg: error message string not pinned");
 ...
 * Signature: ()J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Ecreate_1stack(JNIEnv *env,
                                                             jclass clss) {
  hid_t ret_val = H5I_INVALID_HID;
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Eclose_1stack(JNIEnv *env,
                                                           jclass clss,
                                                           jlong stk_id) {
  UNUSED(clss);
 ...
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Eprint2(JNIEnv *env, jclass clss,
                                                     jlong stk_id,
                                                     jobject stream_obj) {
  herr_t ret_val = FAIL;
...
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Eget_1class_1name(
  ...
  if ((H5Eget_class_name((hid_t)cls_id, (char *)namePtr,
                         (size_t)buf_size + 1)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  namePtr[buf_size] = '\0';
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Eset_1current_1stack(
  ...
  if (stk_id < 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Eset_current_stack: invalid error stack ID");
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Epop(JNIEnv *env, jclass clss,
                                                  jlong stk_id, jlong count) {
  UNUSED(clss);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Epush2(
  ...
  PIN_JAVA_STRING(ENVONLY, filename, fName, NULL,
                  "H5Epush2: filename not pinned");
  PIN_JAVA_STRING(ENVONLY, funcname, fncName, NULL,
                  "H5Epush2: function name not pinned");
  PIN_JAVA_STRING(ENVONLY, err_desc, errMsg, NULL,
                  "H5Epush2: error message not pinned");

  if ((ret_val = H5Epush2((hid_t)stk_id, fName, fncName, (unsigned)linenumber,
                          (hid_t)class_id, (hid_t)major_id, (hid_t)minor_id,
                          errMsg)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Eclear2(JNIEnv *env, jclass clss,
                                                     jlong stk_id) {
  UNUSED(clss);
...
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Eget_1msg(
  ...
  PIN_INT_ARRAY(ENVONLY, error_msg_type_list, theArray, NULL,
                "H5Eget_msg: error_msg_type_list not pinned");

  if ((H5Eget_msg((hid_t)msg_id, &error_msg_type, (char *)namePtr,
                  (size_t)buf_size + 1)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  namePtr[buf_size] = '\0';
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Eget_1num(JNIEnv *env,
                                                        jclass clss,
                                                        jlong stk_id) {
  ssize_t ret_val = -1;
...
static herr_t H5E_walk_cb(int nindx, const H5E_error2_t *info, void *cb_data) {
  ...
  if (JVMPTR->AttachCurrentThread(JVMPAR, (void **)&cbenv, NULL) < 0) {
    ...
    H5_JNI_FATAL_ERROR(CBENVONLY,
                       "H5E_walk_cb: failed to attach current thread to JVM");
  }
  ...
  if (NULL ==
      (mid = CBENVPTR->GetMethodID(CBENVONLY, cbcls, "callback",
                                   "(ILhdf/hdf5lib/structs/H5E_error2_t;Lhdf/"
                                   "hdf5lib/callbacks/H5E_walk_t;)I")))
    CHECK_JNI_EXCEPTION(CBENVONLY, JNI_FALSE);
  ...
  CALL_CONSTRUCTOR(
      CBENVONLY, "hdf/hdf5lib/structs/H5E_error2_t",
      "(JJJILjava/lang/String;Ljava/lang/String;Ljava/lang/String;)V", args,
      cb_info_t);

  status = CBENVPTR->CallIntMethod(CBENVONLY, visit_callback, mid, nindx,
                                   cb_info_t, op_data);
  CHECK_JNI_EXCEPTION(CBENVONLY, JNI_FALSE);
 ...
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Ewalk2(JNIEnv *env, jclass clss,
                                                    jlong stk_id,
                                                    jlong direction,
                                                    jobject callback_op,
                                                    jobject op_data) {
  ...
  if (H5Ewalk2(stk_id, (H5E_direction_t)direction, (H5E_walk2_t)H5E_walk_cb,
               (void *)&wrapper) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
