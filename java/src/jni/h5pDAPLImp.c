...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1chunk_1cache(
    JNIEnv *env, jclass clss, jlong dapl, jlong rdcc_nslots, jlong rdcc_nbytes,
    jdouble rdcc_w0) {
  UNUSED(clss);

  if (H5Pset_chunk_cache((hid_t)dapl, (size_t)rdcc_nslots, (size_t)rdcc_nbytes,
                         (double)rdcc_w0) < 0)
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pget_1chunk_1cache(
  ...
  if (NULL != rdcc_w0)
    PIN_DOUBLE_ARRAY(ENVONLY, rdcc_w0, w0Array, &isCopy,
                     "H5Pget_chunk_cache: rdcc_w0 array not pinned");
  if (NULL != rdcc_nslots)
...

  {
    ...
    if ((status = H5Pget_chunk_cache((hid_t)dapl, &rdcc_nslots_t, &nbytes_t,
                                     (double *)w0Array)) < 0)
      H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (rdcc_nslotsArray)
    UNPIN_LONG_ARRAY(ENVONLY, rdcc_nslots, rdcc_nslotsArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (w0Array)
 ...
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1efile_1prefix(
  ...
  if (NULL == prefix)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pset_efile_prefix: external file prefix is NULL");
 ...
 */
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Pget_1efile_1prefix(
  ...
  if (NULL == (pre = (char *)malloc(sizeof(char) * (size_t)prefix_size + 1)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Pget_efile_prefix: memory allocation failed");

  if (H5Pget_efile_prefix((hid_t)dapl_id, (char *)pre,
  ...
  if (NULL == (str = ENVPTR->NewStringUTF(ENVONLY, pre))) {
    ...
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Pget_efile_prefix: out of memory - unable to "
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1append_1flush(
    JNIEnv *env, jclass clss, jlong plist_id, jint ndims, jlongArray boundary,
  ...
  if ((status = H5Pset_append_flush(
           (hid_t)plist_id, (unsigned)ndims, (const hsize_t *)boundary,
 ...
 * Signature: (JI)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1virtual_1view(JNIEnv *env,
                                                                 jclass clss,
                                                                 jlong dapl_id,
                                                                 jint view) {
  UNUSED(clss);
...
static herr_t H5D_append_cb(hid_t dataset_id, hsize_t *cur_dims,
                            void *cb_data) {
  ...
  if (JVMPTR->AttachCurrentThread(JVMPAR, (void **)&cbenv, NULL) < 0) {
    ...
    H5_JNI_FATAL_ERROR(CBENVONLY,
                       "H5D_append_cb: failed to attach current thread to JVM");
  }
  ...
  if (NULL != cur_dims) {
    ...
    CBENVPTR->SetLongArrayRegion(CBENVONLY, cur_dimsArray, 0, 2,
                                 (const jlong *)cur_dims);
    CHECK_JNI_EXCEPTION(CBENVONLY, JNI_FALSE);

    status = CBENVPTR->CallIntMethod(CBENVONLY, visit_callback, mid, dataset_id,
                                     cur_dims, op_data);
    CHECK_JNI_EXCEPTION(CBENVONLY, JNI_FALSE);
  }
