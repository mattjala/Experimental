 ...
 * Signature: (I)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Screate(JNIEnv *env,
                                                        jclass clss,
                                                        jint type) {
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Screate_1simple(
  ...
  if (NULL != maxdims) {
    if ((mrank = ENVPTR->GetArrayLength(ENVONLY, maxdims)) < 0) {
      ...
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Screate_simple: maxdims array length < 0");
    }

    if (mrank != rank)
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Screate_simple: maxdims rank is invalid");
  }

  PIN_LONG_ARRAY(ENVONLY, dims, dimsP, &isCopy,
                 "H5Screate_simple: dims not pinned");

  if (NULL == (sa = lp = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Screate_simple: failed to allocate dims buffer");
  ...
  if (NULL == maxdims) {
    ...
    PIN_LONG_ARRAY(ENVONLY, maxdims, maxdimsP, &isCopy,
                   "H5Screate_simple: maxdims not pinned");

    if (NULL == (msa = lp = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Screate_simple: failed to allocate maxdims buffer");
  ...
  if ((retVal = H5Screate_simple(rank, (const hsize_t *)sa,
                                 (const hsize_t *)msa)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Scopy(JNIEnv *env, jclass clss,
                                                      jlong space_id) {
  hid_t retVal = H5I_INVALID_HID;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1elements(
    JNIEnv *env, jclass clss, jlong space_id, jint op, jint num_elemn,
    jlongArray coords) {
  jboolean isCopy;
  hssize_t *sa = NULL;
  jlong *P = NULL;
  ...
  PIN_LONG_ARRAY(ENVONLY, coords, P, &isCopy,
                 "H5Sselect_elements: coords not pinned");

  if (NULL ==
      (sa = (hssize_t *)malloc((size_t)num_elemn * 2 * sizeof(hssize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Sselect_elements: failed to allocate coordinate buffer");

  for (i = 0; i < (num_elemn * 2); i++) {
    sa[i] = P[i];
  } /* end for */

  if ((status = H5Sselect_elements(space_id, (H5S_seloper_t)op, num_elemn,
                                   (const hssize_t **)&sa)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1elements(
  ...
  PIN_BYTE_ARRAY(ENVONLY, coords, P, &isCopy,
                 "H5Sselect_elements: coords not pinned");

  if ((size = ENVPTR->GetArrayLength(ENVONLY, coords)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sselect_elements: coords array length < 0");
  }
  ...
  if ((status = H5Sselect_elements(space_id, (H5S_seloper_t)op,
                                   (size_t)num_elemn, (const hsize_t *)llp)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1all(JNIEnv *env,
                                                          jclass clss,
                                                          jlong space_id) {
  herr_t retVal = FAIL;
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1none(JNIEnv *env,
                                                           jclass clss,
                                                           jlong space_id) {
  herr_t retVal = FAIL;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sget_1simple_1extent_1dims(
    JNIEnv *env, jclass clss, jlong space_id, jlongArray dims,
    jlongArray maxdims) {
  jboolean isCopy;
  hsize_t *sa = NULL;
  hsize_t *msa = NULL;
  ...
  if (NULL == dims) {
    ...
    PIN_LONG_ARRAY(ENVONLY, dims, dimsP, &isCopy,
                   "H5Sget_simple_extent_dims: dims not pinned");

    if ((rank = (int)ENVPTR->GetArrayLength(ENVONLY, dims)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sget_simple_extent_dims: dims array length < 0");
    }

    if (NULL == (sa = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY,
          "H5Sget_simple_extent_dims: failed to allocate dimension buffer");
  }
  ...
  if (NULL == maxdims) {
    ...
    PIN_LONG_ARRAY(ENVONLY, maxdims, maxdimsP, &isCopy,
                   "H5Sget_simple_extent_dims: maxdims not pinned");

    if ((mrank = (int)ENVPTR->GetArrayLength(ENVONLY, maxdims)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY, "H5Sget_simple_extent_dims: maxdims array length < 0");
    }
    ...
    if (rank < 0)
      ...
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY, "H5Sget_simple_extent_dims: maxdims rank not same as dims");

    if (NULL == (msa = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(ENVONLY, "H5Sget_simple_extent_dims: failed to "
                                      "allocate maximum dimension buffer");
  }

  if ((status = H5Sget_simple_extent_dims(space_id, (hsize_t *)sa,
                                          (hsize_t *)msa)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sset_1extent_1simple(
    JNIEnv *env, jclass clss, jlong space_id, jint rank, jlongArray dims,
    jlongArray maxdims) {
  jboolean isCopy;
  hsize_t *sa = NULL;
  hsize_t *msa = NULL;
  ...
  if ((drank = ENVPTR->GetArrayLength(ENVONLY, dims)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sset_extent_simple: dims array length < 0");
  }

  if (drank != rank)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sset_extent_simple: dims rank is invalid");

  if (NULL != maxdims) {
    if ((mrank = ENVPTR->GetArrayLength(ENVONLY, maxdims)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sset_extent_simple: maxdims array length < 0");
    }

    if (mrank != rank)
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sset_extent_simple: maxdims rank is invalid");
  }

  PIN_LONG_ARRAY(ENVONLY, dims, dimsP, &isCopy,
                 "H5Sset_extent_simple: dims not pinned");

  if (NULL == (sa = lp = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Sset_extent_simple: failed to allocate dimension buffer");
  ...
  if (NULL == maxdims) {
    ...
    PIN_LONG_ARRAY(ENVONLY, maxdims, maxdimsP, &isCopy,
                   "H5Sset_extent_simple: maxdims not pinned");
  ...
  if ((status = H5Sset_extent_simple(space_id, rank, (hsize_t *)sa,
                                     (hsize_t *)msa)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Sis_1simple(JNIEnv *env,
                                                             jclass clss,
                                                             jlong space_id) {
  htri_t bval = JNI_FALSE;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Soffset_1simple(
  ...
  if (NULL != offset) {
    PIN_BYTE_ARRAY(ENVONLY, offset, P, &isCopy,
                   "H5Soffset_simple: offset not pinned");

    if ((i = ENVPTR->GetArrayLength(ENVONLY, offset)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Soffset_simple: offset array length < 0");
    }
    ...
    if (NULL == (sa = lp = (hssize_t *)malloc((size_t)rank * sizeof(hssize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Soffset_simple: failed to allocate offset buffer");
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sextent_1copy(JNIEnv *env,
                                                           jclass clss,
                                                           jlong space_id,
                                                           jlong src_id) {
  herr_t retVal = FAIL;
 ...
 * Signature: (JJ)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Sextent_1equal(JNIEnv *env,
                                                                jclass clss,
                                                                jlong space_id,
                                                                jlong src_id) {
  htri_t bval = JNI_FALSE;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1hyperslab(
    JNIEnv *env, jclass clss, jlong space_id, jint op, jlongArray start,
  ...
  if ((start_rank = ENVPTR->GetArrayLength(ENVONLY, start)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sselect_hyperslab: start array length < 0");
  }
  if ((count_rank = ENVPTR->GetArrayLength(ENVONLY, count)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sselect_hyperslab: count array length < 0");
  }

  if (start_rank != count_rank)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Sselect_hyperslab: count and start have different rank!");

  PIN_LONG_ARRAY(ENVONLY, start, startP, &isCopy,
                 "H5Sselect_hyperslab: start not pinned");

  if (NULL ==
      (strt = lp = (hsize_t *)malloc((size_t)start_rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Sselect_hyperslab: failed to allocate start buffer");
  ...
  PIN_LONG_ARRAY(ENVONLY, count, countP, &isCopy,
                 "H5Sselect_hyperslab: count not pinned");

  if (NULL ==
      (cnt = lp = (hsize_t *)malloc((size_t)count_rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Sselect_hyperslab: failed to allocate count buffer");
  ...
  if (NULL == stride) {
    ...
    if ((stride_rank = ENVPTR->GetArrayLength(ENVONLY, stride)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sselect_hyperslab: stride array length < 0");
    }

    if (stride_rank != start_rank)
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY,
          "H5Sselect_hyperslab: stride and start have different rank!");

    PIN_LONG_ARRAY(ENVONLY, stride, strideP, &isCopy,
                   "H5Sselect_hyperslab: stride not pinned");

    if (NULL ==
        (strd = lp = (hsize_t *)malloc((size_t)stride_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Sselect_hyperslab: failed to allocate stride buffer");
  ...
  if (NULL == block) {
    ...
    if ((block_rank = ENVPTR->GetArrayLength(ENVONLY, block)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sselect_hyperslab: block array length < 0");
    }

    if (block_rank != start_rank)
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY, "H5Sselect_hyperslab: block and start have different rank!");

    PIN_LONG_ARRAY(ENVONLY, block, blockP, &isCopy,
                   "H5Sselect_hyperslab: block not pinned");

    if (NULL ==
        (blk = lp = (hsize_t *)malloc((size_t)block_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Sselect_hyperslab: failed to allocate block buffer");
  ...
  if ((status =
           H5Sselect_hyperslab(space_id, (H5S_seloper_t)op,
                               (const hsize_t *)strt, (const hsize_t *)strd,
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5__1H5Sclose(JNIEnv *env, jclass clss,
                                                      jlong space_id) {
  herr_t retVal = FAIL;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sget_1select_1hyper_1blocklist(
    JNIEnv *env, jclass clss, jlong spaceid, jlong startblock, jlong numblocks,
    jlongArray buf) {
  jboolean isCopy;
  hsize_t *ba = NULL;
  size_t i, buf_size;
  ...
  if (NULL == buf)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Sget_select_hyper_blocklist: buf is NULL");
  if (numblocks < 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sget_select_hyper_blocklist: numblocks < 0");
  ...
  if ((buflen = ENVPTR->GetArrayLength(ENVONLY, buf)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Sget_select_hyper_blocklist: buf array length < 0");
  }

  if (buflen < (numblocks * rank))
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Sget_select_hyper_blocklist: buffer input array too small");

  PIN_LONG_ARRAY(ENVONLY, buf, bufP, &isCopy,
                 "H5Sget_select_hyper_blocklist: buffer not pinned");
  ...
  if ((status = H5Sget_select_hyper_blocklist(
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sget_1select_1elem_1pointlist(
    JNIEnv *env, jclass clss, jlong spaceid, jlong startpoint, jlong numpoints,
    jlongArray buf) {
  jboolean isCopy;
  hsize_t *ba = NULL;
  jlong *bufP = NULL;
  ...
  if (NULL == buf)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Sget_select_elem_pointlist: buffer is NULL");
  ...
  if ((buflen = ENVPTR->GetArrayLength(ENVONLY, buf)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sget_select_elem_pointlist: buf array length < 0");
  }

  if (buflen < (numpoints * rank))
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Sget_select_elem_pointlist: buffer input array too small");

  PIN_LONG_ARRAY(ENVONLY, buf, bufP, &isCopy,
                 "H5Sget_select_elem_pointlist: buffer not pinned");

  if (NULL == (ba = (hsize_t *)malloc(((size_t)numpoints * (size_t)rank) *
                                      sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "H5Sget_select_elem_pointlist: failed to allocate point list buffer");

  if ((status =
           H5Sget_select_elem_pointlist((hid_t)spaceid, (hsize_t)startpoint,
                                        (hsize_t)numpoints, (hsize_t *)ba)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Sget_1select_1bounds(
  ...
  PIN_LONG_ARRAY(ENVONLY, start, startP, &isCopy,
                 "H5Sget_select_bounds: start not pinned");

  if ((rank = ENVPTR->GetArrayLength(ENVONLY, start)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sget_select_bounds: start array length < 0");
  }

  if (NULL == (strt = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Sget_select_bounds: failed to allocate start buffer");

  PIN_LONG_ARRAY(ENVONLY, end, endP, &isCopy,
                 "H5Sget_select_bounds: end not pinned");

  if (NULL == (en = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Sget_select_bounds: failed to allocate end buffer");

  if ((status = H5Sget_select_bounds((hid_t)spaceid, (hsize_t *)strt,
                                     (hsize_t *)en)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)[B
 */
JNIEXPORT jbyteArray JNICALL Java_hdf_hdf5lib_H5_H5Sencode(JNIEnv *env,
                                                           jclass clss,
                                                           jlong obj_id) {
  ...
  if (NULL == (bufPtr = (unsigned char *)calloc((size_t)1, buf_size)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Sencode: failed to allocate encoding buffer");
  ...
  ENVPTR->SetByteArrayRegion(ENVONLY, returnedArray, 0, (jsize)buf_size,
                             (jbyte *)bufPtr);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
 ...
 * Signature: ([B)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Sdecode(JNIEnv *env, jclass clss,
                                                      jbyteArray buf) {
  jboolean isCopy;
  jbyte *bufP = NULL;
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Sget_1regular_1hyperslab(
    JNIEnv *env, jclass clss, jlong space_id, jlongArray start,
    jlongArray stride, jlongArray count, jlongArray block) {
  jboolean isCopy;
  hsize_t *strt = NULL, *strd = NULL, *cnt = NULL, *blk = NULL;
  jlong *startP = NULL, *strideP = NULL, *countP = NULL, *blockP = NULL;
  ...
  if (space_id < 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sget_regular_hyperslab: invalid dataspace ID");
  ...
  if (NULL == start) {
    ...
    if ((start_rank = ENVPTR->GetArrayLength(ENVONLY, start)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sget_regular_hyperslab: start array length < 0");
    }
    ...
    PIN_LONG_ARRAY(ENVONLY, start, startP, &isCopy,
                   "H5Sget_regular_hyperslab: start not pinned");

    if (NULL ==
        (strt = (hsize_t *)malloc((size_t)start_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Sget_regular_hyperslab: failed to allocate start buffer");
  }
  ...
  if (NULL == stride) {
    ...
    if ((stride_rank = ENVPTR->GetArrayLength(ENVONLY, stride)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY, "H5Sget_regular_hyperslab: stride array length < 0");
    }

    if (stride_rank != rank)
      H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Sget_regular_hyperslab: stride rank "
                                     "doesn't match dataspace rank!");

    PIN_LONG_ARRAY(ENVONLY, stride, strideP, &isCopy,
                   "H5Sget_regular_hyperslab: stride not pinned");

    if (NULL ==
        (strd = (hsize_t *)malloc((size_t)stride_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY,
          "H5Sget_regular_hyperslab: failed to allocate stride buffer");
  }
  ...
  if (NULL == count) {
    ...
    if ((count_rank = ENVPTR->GetArrayLength(ENVONLY, count)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sget_regular_hyperslab: count array length < 0");
    }
    ...
    PIN_LONG_ARRAY(ENVONLY, count, countP, &isCopy,
                   "H5Sget_regular_hyperslab: count not pinned");

    if (NULL == (cnt = (hsize_t *)malloc((size_t)count_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Sget_regular_hyperslab: failed to allocate count buffer");
  }
  ...
  if (NULL == block) {
    ...
    if ((block_rank = ENVPTR->GetArrayLength(ENVONLY, block)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sget_regular_hyperslab: block array length < 0");
    }
    ...
    PIN_LONG_ARRAY(ENVONLY, block, blockP, &isCopy,
                   "H5Sget_regular_hyperslab: block not pinned");

    if (NULL == (blk = (hsize_t *)malloc((size_t)block_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Sget_regular_hyperslab: failed to allocate block buffer");
  }

  if ((status =
           H5Sget_regular_hyperslab(space_id, (hsize_t *)strt, (hsize_t *)strd,
                                    (hsize_t *)cnt, (hsize_t *)blk)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1copy(JNIEnv *env,
                                                           jclass clss,
                                                           jlong dst_id,
                                                           jlong src_id) {
  herr_t status = FAIL;
 ...
 * Signature: (JJ)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1shape_1same(
    JNIEnv *env, jclass clss, jlong space1_id, jlong space2_id) {
  htri_t bval = JNI_FALSE;
 ...
 * Signature: (J[J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1adjust(
    JNIEnv *env, jclass clss, jlong space_id, jlongArray offset) {
  jboolean isCopy;
  hssize_t *offst = NULL;
  jlong *offsetP = NULL;
  jsize offset_rank = -1;
  ...
  if (NULL == offset) {
    ...
    if (offset_rank != rank)
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY,
          "H5Sselect_adjust: offset rank doesn't match dataspace rank!");

    PIN_LONG_ARRAY(ENVONLY, offset, offsetP, &isCopy,
                   "H5Sselect_adjust: offset not pinned");

    if (NULL ==
        (offst = (hssize_t *)malloc((size_t)offset_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Sselect_adjust: failed to allocate offset buffer");
  }
 ...
 * Signature: (J[J[J)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1intersect_1block(
    JNIEnv *env, jclass clss, jlong space_id, jlongArray start,
    jlongArray end) {
  htri_t bval = JNI_FALSE;
  jboolean isCopy;
  ...
  if (space_id < 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Sselect_intersect_block: invalid dataspace ID");
  ...
  if (NULL == start) {
    ...
    if ((start_rank = ENVPTR->GetArrayLength(ENVONLY, start)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sselect_intersect_block: start length < 0");
    }

    if (start_rank != rank)
      H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Sselect_intersect_block: start rank "
                                     "doesn't match dataspace rank!");

    PIN_LONG_ARRAY(ENVONLY, start, startP, &isCopy,
                   "H5Sselect_intersect_block: start not pinned");

    if (NULL ==
        (strt = (hsize_t *)malloc((size_t)start_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY,
          "H5Sselect_intersect_block: failed to allocate start buffer");
  }
  ...
  if (NULL == end) {
    ...
    if ((end_rank = ENVPTR->GetArrayLength(ENVONLY, end)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Sselect_intersect_block: end array length < 0");
    }
    ...
    PIN_LONG_ARRAY(ENVONLY, end, endP, &isCopy,
                   "H5Sselect_intersect_block: end not pinned");

    if (NULL == (nd = (hsize_t *)malloc((size_t)end_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Sselect_intersect_block: failed to allocate end buffer");
  }

  if ((bval = H5Sselect_intersect_block(space_id, (hsize_t *)strt,
                                        (hsize_t *)nd)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (JJJ)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Sselect_1project_1intersection(
  ...
  if ((sid = H5Sselect_project_intersection(src_space_id, dst_space_id,
                                            src_intersect_space_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Scombine_1hyperslab(
    JNIEnv *env, jclass clss, jlong space_id, jint op, jlongArray start,
    jlongArray stride, jlongArray count, jlongArray block) {
  jboolean isCopy;
  hsize_t *strt = NULL, *strd = NULL, *cnt = NULL, *blk = NULL;
  ...
  if ((start_rank = ENVPTR->GetArrayLength(ENVONLY, start)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Scombine_hyperslab: start array length < 0");
  }
  if ((count_rank = ENVPTR->GetArrayLength(ENVONLY, count)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Scombine_hyperslab: count array length < 0");
  }

  if (start_rank != count_rank)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Scombine_hyperslab: count and start have different rank!");

  PIN_LONG_ARRAY(ENVONLY, start, startP, &isCopy,
                 "H5Scombine_hyperslab: start not pinned");

  if (NULL ==
      (strt = lp = (hsize_t *)malloc((size_t)start_rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Scombine_hyperslab: failed to allocate start buffer");
  ...
  PIN_LONG_ARRAY(ENVONLY, count, countP, &isCopy,
                 "H5Scombine_hyperslab: count not pinned");

  if (NULL ==
      (cnt = lp = (hsize_t *)malloc((size_t)count_rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Scombine_hyperslab: failed to allocate count buffer");
  ...
  if (NULL == stride) {
    ...
    if ((stride_rank = ENVPTR->GetArrayLength(ENVONLY, stride)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Scombine_hyperslab: stride array length < 0");
    }

    if (stride_rank != start_rank)
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY,
          "H5Scombine_hyperslab: stride and start have different rank!");

    PIN_LONG_ARRAY(ENVONLY, stride, strideP, &isCopy,
                   "H5Scombine_hyperslab: stride not pinned");

    if (NULL ==
        (strd = lp = (hsize_t *)malloc((size_t)stride_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Scombine_hyperslab: failed to allocate stride buffer");
  ...
  if (NULL == block) {
    ...
    if ((block_rank = ENVPTR->GetArrayLength(ENVONLY, block)) < 0) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "H5Scombine_hyperslab: block array length < 0");
    }

    if (block_rank != start_rank)
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY,
          "H5Scombine_hyperslab: block and start have different rank!");

    PIN_LONG_ARRAY(ENVONLY, block, blockP, &isCopy,
                   "H5Scombine_hyperslab: block not pinned");

    if (NULL ==
        (blk = lp = (hsize_t *)malloc((size_t)block_rank * sizeof(hsize_t))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Scombine_hyperslab: failed to allocate block buffer");
  ...
  if ((sid = H5Scombine_hyperslab(space_id, (H5S_seloper_t)op,
                                  (const hsize_t *)strt, (const hsize_t *)strd,
 ...
 * Signature: (JIJ)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Smodify_1select(
    JNIEnv *env, jclass clss, jlong space1_id, jint op, jlong space2_id) {
  herr_t status = FAIL;

  UNUSED(clss);

  if ((status = H5Smodify_select((hid_t)space1_id, (H5S_seloper_t)op,
                                 (hid_t)space2_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (JIJ)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Scombine_1select(
    JNIEnv *env, jclass clss, jlong space1_id, jint op, jlong space2_id) {
  hid_t sid = H5I_INVALID_HID;

  UNUSED(clss);

  if ((sid = H5Scombine_select((hid_t)space1_id, (H5S_seloper_t)op,
                               (hid_t)space2_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
