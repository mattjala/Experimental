 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5open(JNIEnv *env, jclass clss) {
  herr_t retVal = FAIL;
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5close(JNIEnv *env, jclass clss) {
  herr_t retVal = FAIL;
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5dont_1atexit(JNIEnv *env,
                                                          jclass clss) {
  herr_t retVal = FAIL;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5get_1libversion(
  ...
  if ((status = H5get_libversion((unsigned *)&(libversionArray[0]),
                                 (unsigned *)&(libversionArray[1]),
                                 (unsigned *)&(libversionArray[2]))) < 0)
    H5_LIBRARY_ERROR(ENVONLY);

done:
  if (libversionArray)
    UNPIN_INT_ARRAY(ENVONLY, libversion, libversionArray,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5check_1version(
  ...
  return (jint)H5check_version((unsigned)majnum, (unsigned)minnum,
                               (unsigned)relnum);
 ...
 *
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5garbage_1collect(JNIEnv *env,
                                                              jclass clss) {
  herr_t retVal = FAIL;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5set_1free_1list_1limits(
  ...
  if ((retVal = H5set_free_list_limits(
