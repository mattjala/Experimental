 ...
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5VLregister_1connector_1by_1name(
  ...
  if (NULL == connector_name)
    H5_NULL_ARGUMENT_ERROR(
        ENVONLY, "H5VLregister_connector_by_name: VOL connector name is NULL");

  PIN_JAVA_STRING(
      ENVONLY, connector_name, volName, NULL,
 ...
 * Signature: (IJ)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5VLregister_1connector_1by_1value(
    JNIEnv *env, jclass clss, jint connector_value, jlong vipl_id) {
  hid_t status = H5I_INVALID_HID;

  UNUSED(clss);

  if ((status = H5VLregister_connector_by_value(
           (H5VL_class_value_t)connector_value, (hid_t)vipl_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
Java_hdf_hdf5lib_H5_H5VLis_1connector_1registered_1by_1name(
  ...
  if (NULL == connector_name)
    H5_NULL_ARGUMENT_ERROR(
        ENVONLY,
        "H5VLis_connector_registered_by_name: VOL connector name is NULL");
...
Java_hdf_hdf5lib_H5_H5VLis_1connector_1registered_1by_1value(
  ...
  if (connector_value < 0)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY,
        "H5VLis_connector_registered_by_value: VOL connector value < 0");

  if ((bval = H5VLis_connector_registered_by_value(
           (H5VL_class_value_t)connector_value)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5VLget_1connector_1id(
 ...
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5VLget_1connector_1id_1by_1name(
    JNIEnv *env, jclass clss, jobject connector_name) {
  const char *volName = NULL;
  hid_t status = H5I_INVALID_HID;
  ...
  if (NULL == connector_name)
    H5_NULL_ARGUMENT_ERROR(
        ENVONLY, "H5VLget_connector_id_by_name: VOL connector name is NULL");
 ...
 * Signature: (I)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5VLget_1connector_1id_1by_1value(
    JNIEnv *env, jclass clss, jint connector_value) {
  hid_t status = H5I_INVALID_HID;
  ...
  if (connector_value < 0)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5VLget_connector_id_by_value: VOL connector value < 0");

  if ((status = H5VLget_connector_id_by_value(
           (H5VL_class_value_t)connector_value)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5VLget_1connector_1name(
  ...
  if (buf_size > 0) {
    if (NULL == (volName = (char *)malloc(sizeof(char) * (size_t)buf_size + 1)))
      H5_OUT_OF_MEMORY_ERROR(ENVONLY, "H5VLget_connector_name: failed to "
                                      "allocated VOL connector name buffer");

    if ((status = H5VLget_connector_name((hid_t)object_id, volName,
                                         (size_t)buf_size + 1)) < 0)
      H5_LIBRARY_ERROR(ENVONLY);
    volName[buf_size] = '\0';
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5VLclose(JNIEnv *env, jclass clss,
                                                     jlong connector_id) {
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5VLunregister_1connector(
