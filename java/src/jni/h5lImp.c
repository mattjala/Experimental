...
static herr_t H5L_iterate_cb(hid_t g_id, const char *name,
                             const H5L_info2_t *info, void *cb_data);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Lcopy(
  ...
  PIN_JAVA_STRING(ENVONLY, cur_name, lCurName, NULL,
                  "H5Lcopy: src name not pinned");
  PIN_JAVA_STRING(ENVONLY, dst_name, lDstName, NULL,
                  "H5Lcopy: dest name not pinned");

  if ((status = H5Lcopy((hid_t)cur_loc_id, lCurName, (hid_t)dst_loc_id,
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Lcreate_1external(
  ...
  PIN_JAVA_STRING(ENVONLY, file_name, lFileName, NULL,
                  "H5Lcreate_external: file name not pinned");
  PIN_JAVA_STRING(ENVONLY, cur_name, lCurName, NULL,
                  "H5Lcreate_external: object name not pinned");
  PIN_JAVA_STRING(ENVONLY, dst_name, lDstName, NULL,
                  "H5Lcreate_external: link name not pinned");

  if ((status =
           H5Lcreate_external(lFileName, lCurName, (hid_t)dst_loc_id, lDstName,
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Lcreate_1hard(
  ...
  PIN_JAVA_STRING(ENVONLY, cur_name, lCurName, NULL,
                  "H5Lcreate_hard: object name not pinned");
  PIN_JAVA_STRING(ENVONLY, dst_name, lDstName, NULL,
                  "H5Lcreate_hard: link name not pinned");

  if ((status = H5Lcreate_hard((hid_t)cur_loc_id, lCurName, (hid_t)dst_loc_id,
                               lDstName, (hid_t)create_id, (hid_t)access_id)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Lcreate_1soft(
  ...
  PIN_JAVA_STRING(ENVONLY, cur_name, linkTarget, NULL,
                  "H5Lcreate_soft: link target not pinned");
  PIN_JAVA_STRING(ENVONLY, dst_name, linkName, NULL,
                  "H5Lcreate_soft: link name not pinned");

  if ((status = H5Lcreate_soft(linkTarget, (hid_t)dst_loc_id, linkName,
                               (hid_t)create_id, (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Ldelete(JNIEnv *env, jclass clss,
  ...
  PIN_JAVA_STRING(ENVONLY, name, linkName, NULL,
                  "H5Ldelete: link name not pinned");
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Ldelete_1by_1idx(
  ...
  PIN_JAVA_STRING(ENVONLY, name, groupName, NULL,
                  "H5Ldelete_by_idx: group name not pinned");

  if ((status =
           H5Ldelete_by_idx((hid_t)loc_id, groupName, (H5_index_t)index_field,
                            (H5_iter_order_t)order, n, (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Lexists(
  ...
  PIN_JAVA_STRING(ENVONLY, name, linkName, NULL,
                  "H5Lexists: link name not pinned");
...
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Lget_1info(
  ...
  PIN_JAVA_STRING(ENVONLY, name, linkName, NULL,
                  "H5Lget_info: link name not pinned");

  if ((status = H5Lget_info2((hid_t)loc_id, linkName, &infobuf,
                             (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);

  /*
   * Set the last argument based upon whether this is a hard link (which returns
   * object tokens) or a soft/user-defined link (which returns the link's value
   * size). Then, call the appropriate constructor to instantiate an H5L_info_t
   * object.
   */
  if (infobuf.type == H5L_TYPE_HARD) {
    ...
    CALL_CONSTRUCTOR(ENVONLY, "hdf/hdf5lib/structs/H5L_info_t", "(IZJIJ)V",
                     args, ret_obj);
  }
...
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Lget_1info_1by_1idx(
  ...
  PIN_JAVA_STRING(ENVONLY, name, groupName, NULL,
                  "H5Lget_info_by_idx: group name not pinned");

  if ((status = H5Lget_info_by_idx2((hid_t)loc_id, groupName,
                                    (H5_index_t)index_field,
                                    (H5_iter_order_t)order, (hsize_t)link_n,
                                    &infobuf, (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Lget_1name_1by_1idx(
    JNIEnv *env, jclass clss, jlong loc_id, jstring name, jint index_field,
    jint order, jlong link_n, jlong access_id) {
  const char *groupName = NULL;
  jstring str = NULL;
  ...
  PIN_JAVA_STRING(ENVONLY, name, groupName, NULL,
                  "H5Lget_name_by_idx: group name not pinned");

  /* Get the length of the link name */
  if ((status_size =
           H5Lget_name_by_idx((hid_t)loc_id, groupName, (H5_index_t)index_field,
                              (H5_iter_order_t)order, (hsize_t)link_n,
                              (char *)NULL, (size_t)0, H5P_DEFAULT)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);

  /* add extra space for the null terminator */
  if (NULL ==
      (linkName = (char *)malloc(sizeof(char) * (size_t)status_size + 1)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Lget_name_by_idx: failed to allocate buffer for link name");

  if ((H5Lget_name_by_idx((hid_t)loc_id, groupName, (H5_index_t)index_field,
                          (H5_iter_order_t)order, (hsize_t)link_n,
                          (char *)linkName, (size_t)status_size + 1,
                          (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Lget_1value(
  ...
  PIN_JAVA_STRING(ENVONLY, name, linkName, NULL,
                  "H5Lget_value: link name not pinned");

  /* Get the length of the link value */
  if ((status = H5Lget_info2((hid_t)loc_id, linkName, &infobuf, H5P_DEFAULT)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (NULL ==
      (linkValue = (char *)malloc(sizeof(char) * infobuf.u.val_size + 1)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Lget_val: failed to allocate buffer for link value");

  if ((status = H5Lget_val((hid_t)loc_id, linkName, (void *)linkValue,
                           infobuf.u.val_size + 1, (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  case H5L_TYPE_EXTERNAL: {
    if ((status =
             H5Lunpack_elink_val(linkValue, (size_t)infobuf.u.val_size,
                                 (unsigned *)NULL, &file_name, &obj_name)) < 0)
      H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Lget_1value_1by_1idx(
    JNIEnv *env, jclass clss, jlong loc_id, jstring name, jint index_field,
    jint order, jlong link_n, jobjectArray link_value, jlong access_id) {
  H5L_info2_t infobuf = {H5L_TYPE_ERROR, false, -1, H5T_CSET_ERROR, {{{0}}}};
  const char *file_name = NULL;
  const char *obj_name = NULL;
  ...
  PIN_JAVA_STRING(ENVONLY, name, grpName, NULL,
                  "H5Lget_val_by_idx: group name not pinned");

  /* Get the length of the link value */
  if ((status =
           H5Lget_info_by_idx2((hid_t)loc_id, grpName, (H5_index_t)index_field,
                               (H5_iter_order_t)order, (hsize_t)link_n,
                               &infobuf, (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (H5L_TYPE_HARD == infobuf.type)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Lget_val_by_idx: hard links are unsupported");
  ...
  if (NULL == (linkValue = (void *)malloc(infobuf.u.val_size + 1)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Lget_val_by_idx: failed to allocate buffer for link value");

  if ((status = H5Lget_val_by_idx(
           (hid_t)loc_id, grpName, (H5_index_t)index_field,
           (H5_iter_order_t)order, (hsize_t)link_n, (void *)linkValue,
           infobuf.u.val_size + 1, (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  case H5L_TYPE_EXTERNAL: {
    if ((status = H5Lunpack_elink_val(
             (char *)linkValue, (size_t)infobuf.u.val_size, (unsigned *)NULL,
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Lmove(
  ...
  PIN_JAVA_STRING(ENVONLY, cur_name, lCurName, NULL,
                  "H5Lmove: src name not pinned");
  PIN_JAVA_STRING(ENVONLY, dst_name, lDstName, NULL,
                  "H5Lmove: dest name not pinned");

  if ((status = H5Lmove((hid_t)cur_loc_id, lCurName, (hid_t)dst_loc_id,
                        lDstName, (hid_t)create_id, (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
static herr_t H5L_iterate_cb(hid_t g_id, const char *name,
  ...
  if (JVMPTR->AttachCurrentThread(JVMPAR, (void **)&cbenv, NULL) < 0) {
    ...
    H5_JNI_FATAL_ERROR(
        CBENVONLY, "H5L_iterate_cb: failed to attach current thread to JVM");
  }
  ...
  if (info->type == H5L_TYPE_HARD) {
    ...
    CALL_CONSTRUCTOR(CBENVONLY, "hdf/hdf5lib/structs/H5L_info_t",
                     "(IZJILhdf/hdf5lib/structs/H5O_token_t;)V", args,
                     cb_info_t);
  } else {
    args[0].i = info->type;
    args[1].z = info->corder_valid;
    ...
    CALL_CONSTRUCTOR(CBENVONLY, "hdf/hdf5lib/structs/H5L_info_t", "(IZJIJ)V",
                     args, cb_info_t);
  }
 ...
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Lvisit(JNIEnv *env, jclass clss,
                                                    jlong grp_id, jint idx_type,
                                                    jint order,
                                                    jobject callback_op,
                                                    jobject op_data) {
  ...
  if ((status = H5Lvisit2((hid_t)grp_id, (H5_index_t)idx_type,
                          (H5_iter_order_t)order, H5L_iterate_cb,
                          (void *)&wrapper)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Lvisit_1by_1name(
  ...
  PIN_JAVA_STRING(ENVONLY, name, grpName, NULL,
                  "H5Lvisit_by_name: group name not pinned");

  if ((status = H5Lvisit_by_name2((hid_t)grp_id, grpName, (H5_index_t)idx_type,
                                  (H5_iter_order_t)order, H5L_iterate_cb,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Literate(
  ...
  if ((status = H5Literate2((hid_t)grp_id, (H5_index_t)idx_type,
                            (H5_iter_order_t)order, &start_idx, H5L_iterate_cb,
                            (void *)&wrapper)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Literate_1by_1name(
  ...
  PIN_JAVA_STRING(ENVONLY, name, groupName, NULL,
                  "H5Literate_by_name: group name not pinned");

  if ((status = H5Literate_by_name2(
           (hid_t)grp_id, groupName, (H5_index_t)idx_type,
           (H5_iter_order_t)order, &start_idx, H5L_iterate_cb, (void *)&wrapper,
           (hid_t)access_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Lis_1registered(JNIEnv *env,
                                                             jclass clss,
                                                             jint link_cls_id) {
  htri_t ret_val = FAIL;
 ...
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Lunregister(JNIEnv *env,
                                                         jclass clss,
                                                         jint link_cls_id) {
  UNUSED(clss);
