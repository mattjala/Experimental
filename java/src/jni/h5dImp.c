...
static herr_t H5DreadVL_asstr(JNIEnv *env, hid_t did, hid_t tid, hid_t mem_sid,
                              hid_t file_sid, hid_t xfer_plist_id,
                              jobjectArray buf);
static herr_t H5DreadVL_str(JNIEnv *env, hid_t did, hid_t tid, hid_t mem_sid,
                            hid_t file_sid, hid_t xfer_plist_id,
                            jobjectArray buf);
static herr_t H5DwriteVL_asstr(JNIEnv *env, hid_t did, hid_t tid, hid_t mem_sid,
                               hid_t file_sid, hid_t xfer_plist_id,
                               jobjectArray buf);
static herr_t H5DwriteVL_str(JNIEnv *env, hid_t did, hid_t tid, hid_t mem_sid,
                             hid_t file_sid, hid_t xfer_plist_id,
                             jobjectArray buf);
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Dcreate(
  ...
  PIN_JAVA_STRING(ENVONLY, name, dsetName, NULL,
                  "H5Dcreate: dataset name not pinned");

  if ((dset_id =
           H5Dcreate2((hid_t)loc_id, dsetName, (hid_t)type_id, (hid_t)space_id,
                      H5P_DEFAULT, (hid_t)create_plist_id, H5P_DEFAULT)) < 0)
 ...
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Dopen(JNIEnv *env, jclass clss,
                                                      jlong loc_id,
                                                      jstring name) {
  const char *dsetName = NULL;
  hid_t dset_id = H5I_INVALID_HID;
  ...
  PIN_JAVA_STRING(ENVONLY, name, dsetName, NULL,
                  "H5Dopen: dataset name not pinned");
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Dget_1space(JNIEnv *env,
                                                            jclass clss,
                                                            jlong dataset_id) {
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Dget_1type(JNIEnv *env,
                                                           jclass clss,
                                                           jlong dataset_id) {
  hid_t retVal = H5I_INVALID_HID;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread(
    JNIEnv *env, jclass clss, jlong dataset_id, jlong mem_type_id,
    jlong mem_space_id, jlong file_space_id, jlong xfer_plist_id,
    jbyteArray buf, jboolean isCriticalPinning) {
  ...
  if (vl_data_class) {
    ...
    if (NULL == (readBuf = calloc((size_t)vl_array_len, typeSize)))
      H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                             "H5Dread: failed to allocate raw VL read buffer");
  } else {
    if (isCriticalPinning) {
      ...
      PIN_BYTE_ARRAY(ENVONLY, buf, readBuf, &readBufIsCopy,
                     "H5Dread: read buffer not pinned");
    }
  }

  if ((status = H5Dread((hid_t)dataset_id, (hid_t)mem_type_id,
  ...
  if (vl_data_class) {
    ...
    if (NULL == (writeBuf = calloc((size_t)vl_array_len, typeSize)))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5Dwrite: failed to allocate raw VL write buffer");
  } else {
    if (isCriticalPinning) {
      ...
      PIN_BYTE_ARRAY(ENVONLY, buf, writeBuf, &writeBufIsCopy,
                     "H5Dwrite: write buffer not pinned");
    }
  }
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5__1H5Dclose(JNIEnv *env, jclass clss,
                                                      jlong dataset_id) {
  herr_t retVal = FAIL;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dvlen_1reclaim(
  ...
  PIN_BYTE_ARRAY(ENVONLY, buf, vlenBuf, &vlenBufIsCopy,
                 "H5Dvlen_reclaim: buffer not pinned");

  if ((status = H5Dvlen_reclaim((hid_t)type_id, (hid_t)space_id,
                                (hid_t)xfer_plist_id, vlenBuf)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
#endif
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1short(
  ...
  if (isCriticalPinning) {
    ...
    PIN_SHORT_ARRAY(ENVONLY, buf, readBuf, &readBufIsCopy,
                    "H5Dread_short: read buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dwrite_1short(
  ...
  if (isCriticalPinning) {
    ...
    PIN_SHORT_ARRAY(ENVONLY, buf, writeBuf, &writeBufIsCopy,
                    "H5Dwrite_short: write buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1int(
  ...
  if (isCriticalPinning) {
    ...
    PIN_INT_ARRAY(ENVONLY, buf, readBuf, &readBufIsCopy,
                  "H5Dread_int: read buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dwrite_1int(
  ...
  if (isCriticalPinning) {
    ...
    PIN_INT_ARRAY(ENVONLY, buf, writeBuf, &writeBufIsCopy,
                  "H5Dwrite_int: write buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1long(
  ...
  if (isCriticalPinning) {
    ...
    PIN_LONG_ARRAY(ENVONLY, buf, readBuf, &readBufIsCopy,
                   "H5Dread_long: read buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dwrite_1long(
  ...
  if (isCriticalPinning) {
    ...
    PIN_LONG_ARRAY(ENVONLY, buf, writeBuf, &writeBufIsCopy,
                   "H5Dwrite_long: write buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1float(
  ...
  if (isCriticalPinning) {
    ...
    PIN_FLOAT_ARRAY(ENVONLY, buf, readBuf, &readBufIsCopy,
                    "H5Dread_float: read buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dwrite_1float(
  ...
  if (isCriticalPinning) {
    ...
    PIN_FLOAT_ARRAY(ENVONLY, buf, writeBuf, &writeBufIsCopy,
                    "H5Dwrite_float: write buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1double(
  ...
  if (isCriticalPinning) {
    ...
    PIN_DOUBLE_ARRAY(ENVONLY, buf, readBuf, &readBufIsCopy,
                     "H5Dread_double: read buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dwrite_1double(
  ...
  if (isCriticalPinning) {
    ...
    PIN_DOUBLE_ARRAY(ENVONLY, buf, writeBuf, &writeBufIsCopy,
                     "H5Dwrite_double: write buffer not pinned");
  }
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1string(
  ...
  for (i = 0, pos = 0; i < n; i++) {
    ...
    if (NULL == (jstr = ENVPTR->NewStringUTF(ENVONLY, cstr))) {
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                             "H5Dread_string: out of memory - unable to "
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dwrite_1string(
  ...
  if (NULL == (c_buf = (char *)malloc((size_t)n * str_len)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Dwrite_string: memory allocation failed");
  ...
  for (i = 0; i < (size_t)n; i++) {
    ...
    PIN_JAVA_STRING(ENVONLY, obj, utf8, NULL,
                    "H5Dwrite_string: string not pinned");
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5DreadVL(
  ...
  if (NULL == (readBuf = calloc((size_t)vl_array_len, typeSize)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5DreadVL: failed to allocate raw VL read buffer");
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5DwriteVL(
  ...
  if (NULL == (writeBuf = calloc((size_t)vl_array_len, typeSize)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5DwriteVL: failed to allocate raw VL write buffer");
...
static herr_t H5DreadVL_asstr(JNIEnv *env, hid_t did, hid_t tid, hid_t mem_sid,
  ...
  if (NULL == (readBuf = calloc((size_t)n, typeSize)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5DreadVL_asstr: failed to allocate read buffer");

  if ((status = H5Dread(did, tid, mem_sid, file_sid, xfer_plist_id, readBuf)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (!h5str.s)
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5DreadVL_asstr: failed to allocate buffer");
  ...
  for (i = 0; i < (size_t)n; i++) {
    ...
    if (!h5str_sprintf(ENVONLY, &h5str, did, tid,
 ...
 */
static herr_t H5DwriteVL_str(JNIEnv *env, hid_t dataset_id, hid_t mem_type_id,
                             hid_t mem_space_id, hid_t file_space_id,
  ...
  if (NULL == (writeBuf = (char **)calloc((size_t)size + 1, sizeof(char *))))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "H5DwriteVL_str: failed to allocate "
  ...
  for (i = 0; i < size; ++i) {
    ...
    PIN_JAVA_STRING(ENVONLY, obj, utf8, NULL,
                    "H5DwriteVL_str: string not pinned");

    if (NULL == (writeBuf[i] = (char *)malloc((size_t)length + 1)))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "H5DwriteVL_str: failed to allocate string buffer");
  ...
  if ((status = H5Dwrite(dataset_id, mem_type_id, mem_space_id, file_space_id,
                         xfer_plist_id, writeBuf)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
static herr_t H5DwriteVL_asstr(JNIEnv *env, hid_t did, hid_t tid, hid_t mem_sid,
  ...
  for (i = 0; i < (size_t)n; ++i) {
    ...
    PIN_JAVA_STRING(ENVONLY, jstr, utf8, NULL,
                    "H5DwriteVL_asstr: failed to pin string buffer");
    ...
    if (!h5str_convert(ENVONLY, &utf8_copy, did, tid,
  ...
  if ((status =
           H5Dwrite(did, tid, mem_sid, file_sid, xfer_plist_id, writeBuf)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dread_1reg_1ref(
  ...
  if (NULL ==
    ...
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Dread_reg_ref: failed to allocate read buffer");

  if ((status =
           H5Dread((hid_t)dataset_id, (hid_t)mem_type_id, (hid_t)mem_space_id,
                   (hid_t)file_space_id, xfer_plist_id, ref_data)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (!h5str.s)
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Dread_reg_ref: failed to allocate buffer");

  for (i = 0; i < n; i++) {
    h5str.s[0] = '\0';

    if (!h5str_sprintf(ENVONLY, &h5str, (hid_t)dataset_id, (hid_t)mem_type_id,
                       (void *)&ref_data[i], 0))
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Dcreate2(
  ...
  PIN_JAVA_STRING(ENVONLY, name, dsetName, NULL,
                  "H5Dcreate2: dataset name not pinned");

  if ((dset_id = H5Dcreate2((hid_t)loc_id, dsetName, (hid_t)type_id,
                            (hid_t)space_id, (hid_t)link_plist_id,
                            (hid_t)create_plist_id, (hid_t)access_plist_id)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Dopen2(JNIEnv *env, jclass clss,
                                                       jlong loc_id,
                                                       jstring name,
                                                       jlong access_plist) {
  const char *dsetName = NULL;
  ...
  PIN_JAVA_STRING(ENVONLY, name, dsetName, NULL,
                  "H5Dopen2: dataset name not pinned");
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Dcreate_1anon(
    JNIEnv *env, jclass clss, jlong loc_id, jlong type_id, jlong space_id,
    jlong dcpl_id, jlong dapl_id) {
  hid_t dset_id = H5I_INVALID_HID;

  UNUSED(clss);

  if ((dset_id = H5Dcreate_anon((hid_t)loc_id, (hid_t)type_id, (hid_t)space_id,
 ...
 * Signature: (J)I;
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dget_1space_1status(JNIEnv *env,
                                                                 jclass clss,
                                                                 jlong loc_id) {
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Dget_1offset(JNIEnv *env,
                                                           jclass clss,
                                                           jlong loc_id) {
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Dvlen_1get_1buf_1size(
  ...
  if (H5Dvlen_get_buf_size((hid_t)dataset_id, (hid_t)type_id, (hid_t)space_id,
                           &sz) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Dfill(
    JNIEnv *env, jclass clss, jbyteArray fill, jlong fill_type_id,
    jbyteArray buf, jlong buf_type_id, jlong space_id) {
  jboolean isCopy1;
  jboolean isCopy2;
  herr_t status = FAIL;
  jbyte *fillP = NULL;
  ...
  if (fill) {
    PIN_BYTE_ARRAY(ENVONLY, fill, fillP, &isCopy1,
                   "H5Dfill: fill buffer not pinned");
  }

  if ((status = H5Dfill((const void *)fillP, (hid_t)fill_type_id, (void *)buffP,
                        (hid_t)buf_type_id, (hid_t)space_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J[J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Dset_1extent(JNIEnv *env,
                                                          jclass clss,
                                                          jlong loc_id,
                                                          jlongArray buf) {
  jboolean isCopy;
  ...
  PIN_LONG_ARRAY(ENVONLY, buf, dimsBuf, &isCopy,
                 "H5Dset_extent: buffer not pinned");

  if (NULL == (dims = (hsize_t *)malloc((size_t)rank * sizeof(hsize_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Dset_extent: failed to allocate dataset dimension buffer");
...
static herr_t H5D_iterate_cb(void *elem, hid_t elem_id, unsigned ndim,
                             const hsize_t *point, void *cb_data) {
  ...
  if (JVMPTR->AttachCurrentThread(JVMPAR, (void **)&cbenv, NULL) < 0) {
    ...
    H5_JNI_FATAL_ERROR(
        CBENVONLY, "H5D_iterate_cb: failed to attach current thread to JVM");
  }
  ...
  CBENVPTR->SetLongArrayRegion(CBENVONLY, pointArray, 0, 2,
                               (const jlong *)point);
  CHECK_JNI_EXCEPTION(CBENVONLY, JNI_FALSE);

  status =
      CBENVPTR->CallIntMethod(CBENVONLY, visit_callback, mid, (void *)elemArray,
                              elem_id, ndim, pointArray, op_data);
  CHECK_JNI_EXCEPTION(CBENVONLY, JNI_FALSE);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Diterate(
  ...
  PIN_BYTE_ARRAY(ENVONLY, buf, iterBuf, &isCopy,
                 "H5Diterate: buffer not pinned");

  if ((status = H5Diterate((void *)iterBuf, (hid_t)buf_type, (hid_t)space,
                           (H5D_operator_t)H5D_iterate_cb, (void *)&wrapper)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Dflush(JNIEnv *env, jclass clss,
                                                    jlong loc_id) {
  UNUSED(clss);
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Drefresh(JNIEnv *env, jclass clss,
                                                      jlong loc_id) {
  UNUSED(clss);
