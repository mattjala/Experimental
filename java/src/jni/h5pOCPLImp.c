...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1create_1intermediate_1group(
  ...
  if ((retVal = H5Pset_create_intermediate_group(
           (hid_t)lcpl_id, (unsigned)crt_intermed_group)) < 0)
...
Java_hdf_hdf5lib_H5_H5Pget_1create_1intermediate_1group(JNIEnv *env,
                                                        jclass clss,
                                                        jlong lcpl_id) {
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1attr_1phase_1change(
  ...
  if ((retVal = H5Pset_attr_phase_change((hid_t)ocpl_id, (unsigned)max_compact,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1attr_1phase_1change(
  ...
  if (NULL == attributes)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_attr_phase_change: attributes is NULL");

  PIN_INT_ARRAY(ENVONLY, attributes, theArray, &isCopy,
                "H5Pget_attr_phase_change: input not pinned");

  if ((retVal =
           H5Pget_attr_phase_change((hid_t)ocpl_id, (unsigned *)&(theArray[0]),
                                    (unsigned *)&(theArray[1]))) < 0)
    H5_LIBRARY_ERROR(ENVONLY);

done:
  if (theArray)
    UNPIN_INT_ARRAY(ENVONLY, attributes, theArray,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1attr_1creation_1order(
  ...
  if ((retVal = H5Pset_attr_creation_order((hid_t)ocpl_id,
