 ...
 */
JNIEXPORT jbyteArray JNICALL Java_hdf_hdf5lib_H5_H5Rcreate_1object(
  ...
  PIN_JAVA_STRING(ENVONLY, name, refName, NULL,
                  "H5Rcreate_object: reference name not pinned");

  if (NULL == (refBuf = calloc(1, H5R_REF_BUF_SIZE)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Rcreate_object: failed to allocate reference buffer");

  H5R_ref_t loc_ref;
  if ((status =
           H5Rcreate_object((hid_t)loc_id, refName, (hid_t)aid, &loc_ref)) < 0)
  ...
  ENVPTR->SetByteArrayRegion(ENVONLY, ref, 0, (jsize)H5R_REF_BUF_SIZE,
                             (jbyte *)refBuf);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
...
Java_hdf_hdf5lib_H5_H5Rcreate_1region(JNIEnv *env, jclass clss, jlong loc_id,
                                      jstring name, jlong space_id, jlong aid) {
  const char *refName = NULL;
  jbyteArray ref = NULL;
  jbyte *refBuf = NULL;
  herr_t status = FAIL;
  ...
  PIN_JAVA_STRING(ENVONLY, name, refName, NULL,
                  "H5Rcreate_region: reference name not pinned");

  if (NULL == (refBuf = calloc(1, H5R_REF_BUF_SIZE)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Rcreate_region: failed to allocate reference buffer");

  H5R_ref_t loc_ref;
  if ((status = H5Rcreate_region((hid_t)loc_id, refName, space_id, (hid_t)aid,
                                 &loc_ref)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT jbyteArray JNICALL Java_hdf_hdf5lib_H5_H5Rcreate_1attr(
  ...
  PIN_JAVA_STRING(ENVONLY, attr_name, attrName, NULL,
                  "H5Rcreate_attr: attribute name not pinned");

  PIN_JAVA_STRING(ENVONLY, name, refName, NULL,
                  "H5Rcreate_attr: reference name not pinned");

  if (NULL == (refBuf = calloc(1, H5R_REF_BUF_SIZE)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Rcreate_attr: failed to allocate reference buffer");

  H5R_ref_t loc_ref;
  if ((status = H5Rcreate_attr((hid_t)loc_id, refName, attrName, (hid_t)aid,
                               &loc_ref)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: ([B)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Rdestroy(JNIEnv *env, jclass clss,
                                                      jbyteArray ref) {
  jboolean isCopy;
  jbyte *refBuf = NULL;
  jsize refBufLen;
  ...
  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rdestroy: reference buffer not pinned");
 ...
 * Signature: ([B)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Rget_1type(JNIEnv *env,
                                                        jclass clss,
                                                        jbyteArray ref) {
  jboolean isCopy;
  jbyte *refBuf = NULL;
  ...
  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_type: reference buffer not pinned");
 ...
 * Signature: ([B[B)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Requal(JNIEnv *env,
                                                        jclass clss,
                                                        jbyteArray ref1,
                                                        jbyteArray ref2) {
  jboolean isCopy;
  ...
  PIN_BYTE_ARRAY(ENVONLY, ref1, refBuf1, &isCopy,
                 "H5Requal: reference1 buffer not pinned");
  ...
  PIN_BYTE_ARRAY(ENVONLY, ref2, refBuf2, &isCopy,
                 "H5Requal: reference2 buffer not pinned");
 ...
 * Signature: ([B)[B
 */
JNIEXPORT jbyteArray JNICALL Java_hdf_hdf5lib_H5_H5Rcopy(JNIEnv *env,
                                                         jclass clss,
                                                         jbyteArray src_ref) {
  jboolean isCopy;
  ...
  PIN_BYTE_ARRAY(ENVONLY, src_ref, src_refBuf, &isCopy,
                 "H5Rcopy: src reference buffer not pinned");

  if (NULL == (dst_refBuf = calloc(1, H5R_REF_BUF_SIZE)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY, "H5Rcreate_attr: failed to allocate dst reference buffer");
  ...
  if (NULL ==
      (dst_ref = ENVPTR->NewByteArray(ENVONLY, (jsize)H5R_REF_BUF_SIZE)))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  ENVPTR->SetByteArrayRegion(ENVONLY, dst_ref, 0, (jsize)H5R_REF_BUF_SIZE,
                             (jbyte *)dst_refBuf);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

done:
  if (src_refBuf)
    UNPIN_BYTE_ARRAY(ENVONLY, src_ref, src_refBuf,
 ...
 * Signature: ([BJJ)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Ropen_1object(
  ...
  if ((refBufLen = ENVPTR->GetArrayLength(ENVONLY, ref)) < 0) {
    ...
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Ropen_object: reference array length < 0");
  }

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Ropen_object: reference buffer not pinned");
 ...
 * Signature: ([BJJ)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Ropen_1region(
    JNIEnv *env, jclass clss, jbyteArray ref, jlong rapl_id, jlong oapl_id) {
  jboolean isCopy;
  jbyte *refBuf = NULL;
  jsize refBufLen;
  ...
  if ((refBufLen = ENVPTR->GetArrayLength(ENVONLY, ref)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Ropen_region: reference array length < 0");
  }

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Ropen_region: reference buffer not pinned");
 ...
 * Signature: ([BJJ)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Ropen_1attr(
  ...
  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Ropen_attr: reference buffer not pinned");
 ...
 * Signature: ([BJ)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Rget_1obj_1type3(JNIEnv *env,
                                                              jclass clss,
                                                              jbyteArray ref,
                                                              jlong rapl_id) {
  ...
  if (NULL == ref)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Rget_obj_type3: reference buffer is NULL");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_obj_type3: reference buffer not pinned");
 ...
 */
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Rget_1file_1name(
  ...
  if (NULL == ref)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Rget_file_name: reference buffer is NULL");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_file_name: reference buffer not pinned");
  ...
  if ((check_size = H5Rget_file_name(&loc_ref, namePtr, (size_t)buf_size + 1)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
  namePtr[buf_size] = '\0';
 ...
 */
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Rget_1obj_1name(JNIEnv *env,
                                                                jclass clss,
                                                                jbyteArray ref,
                                                                jlong rapl_id) {
  jboolean isCopy;
  ...
  if (NULL == ref)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Rget_obj_name: reference buffer is NULL");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_obj_name: reference buffer not pinned");
  ...
  if ((check_size = H5Rget_obj_name(&loc_ref, (hid_t)rapl_id, namePtr,
                                    (size_t)buf_size + 1)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  namePtr[buf_size] = '\0';
 ...
 * Signature: ([B)Ljava/lang/String;
 */
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Rget_1attr_1name(
    JNIEnv *env, jclass clss, jbyteArray ref) {
  jboolean isCopy;
  jbyte *refBuf = NULL;
  jstring str = NULL;
  ...
  if (NULL == ref)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Rget_attr_name: reference buffer is NULL");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_attr_name: reference buffer not pinned");
  ...
  if ((check_size = H5Rget_attr_name(&loc_ref, namePtr, (size_t)buf_size + 1)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
  namePtr[buf_size] = '\0';
 ...
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Rcreate(JNIEnv *env, jclass clss,
                                                     jbyteArray ref,
                                                     jlong loc_id, jstring name,
                                                     jint ref_type,
                                                     jlong space_id) {
  const char *refName = NULL;
  jboolean isCopy;
  ...
  if ((H5R_OBJECT == ref_type) && (refBufLen != H5R_OBJ_REF_BUF_SIZE))
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY,
        "H5Rcreate: reference input array length != H5R_OBJ_REF_BUF_SIZE");
  else if ((H5R_DATASET_REGION == ref_type) &&
           (refBufLen != H5R_DSET_REG_REF_BUF_SIZE))
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Rcreate: region reference input array "
                                   "length != H5R_DSET_REG_REF_BUF_SIZE");
  else if ((H5R_OBJECT != ref_type) && (H5R_DATASET_REGION != ref_type))
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Rcreate: unknown reference type");

  PIN_JAVA_STRING(ENVONLY, name, refName, NULL,
                  "H5Rcreate: reference name not pinned");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rcreate: reference buffer not pinned");

  if ((status = H5Rcreate(refBuf, (hid_t)loc_id, refName, (H5R_type_t)ref_type,
                          (hid_t)space_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Rdereference(
    JNIEnv *env, jclass clss, jlong dataset, jlong access_list, jint ref_type,
    jbyteArray ref) {
  jboolean isCopy;
  jbyte *refBuf = NULL;
  jsize refBufLen;
  ...
  if ((H5R_OBJECT == ref_type) && (refBufLen != H5R_OBJ_REF_BUF_SIZE))
    H5_BAD_ARGUMENT_ERROR(
        ...
        "H5Rdereference: reference input array length != H5R_OBJ_REF_BUF_SIZE");
           ...
           (refBufLen != H5R_DSET_REG_REF_BUF_SIZE))
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Rdereference: region reference input "
                                   "array length != H5R_DSET_REG_REF_BUF_SIZE");
  else if ((H5R_OBJECT != ref_type) && (H5R_DATASET_REGION != ref_type))
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Rdereference: unknown reference type");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rderefernce: reference buffer not pinned");

  if ((retVal = H5Rdereference2((hid_t)dataset, (hid_t)access_list,
                                (H5R_type_t)ref_type, refBuf)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Rget_1region(
  ...
  if (refBufLen != H5R_DSET_REG_REF_BUF_SIZE)
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Rget_region: region reference input "
                                   "array length != H5R_DSET_REG_REF_BUF_SIZE");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_region: reference buffer not pinned");

  if ((retVal = H5Rget_region((hid_t)dataset, (H5R_type_t)ref_type, refBuf)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Rget_1obj_1type(
  ...
  if (NULL == ref)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Rget_obj_type: reference buffer is NULL");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_obj_type: reference buffer not pinned");

  if ((retVal = H5Rget_obj_type2((hid_t)loc_id, (H5R_type_t)ref_type, refBuf,
                                 &object_info)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Rget_1obj_1type2(
  ...
  if (NULL == ref_obj)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Rget_obj_type: reference object is NULL");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_obj_type: reference buffer not pinned");

  PIN_INT_ARRAY(ENVONLY, ref_obj, ref_objP, &isCopy2,
                "H5Rget_obj_type: reference object array not pinned");

  if ((status = H5Rget_obj_type2((hid_t)loc_id, (H5R_type_t)ref_type, refBuf,
                                 (H5O_type_t *)ref_objP)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Rget_1name(
  ...
  if ((H5R_OBJECT == ref_type) && (refBufLen != H5R_OBJ_REF_BUF_SIZE))
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY,
        "H5Rget_name: reference input array length != H5R_OBJ_REF_BUF_SIZE");
  else if ((H5R_DATASET_REGION == ref_type) &&
           (refBufLen != H5R_DSET_REG_REF_BUF_SIZE))
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Rget_name: region reference input array "
                                   "length != H5R_DSET_REG_REF_BUF_SIZE");
  else if ((H5R_OBJECT != ref_type) && (H5R_DATASET_REGION != ref_type))
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Rget_name: unknown reference type");

  PIN_BYTE_ARRAY(ENVONLY, ref, refBuf, &isCopy,
                 "H5Rget_name: reference buffer not pinned");

  if (NULL == (aName = malloc(sizeof(char) * (size_t)size + 1)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "H5Rget_name: failed to allocate referenced object name buffer");

  if ((ret_val = (jlong)H5Rget_name((hid_t)loc_id, (H5R_type_t)ref_type, refBuf,
                                    aName, (size_t)size + 1)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  aName[(size_t)size] = '\0';
 ...
 */
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Rget_1name_1string(
    JNIEnv *env, jclass clss, jlong loc_id, jint ref_type, jbyteArray ref) {
  jboolean isCopy;
  jstring str = NULL;
  jsize refBufLen;
  ...
  /* Get the length of the name */
  if ((buf_size = H5Rget_name((hid_t)loc_id, (H5R_type_t)ref_type, refBuf, NULL,
                              0)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
