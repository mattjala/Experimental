 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5__1H5Gclose(JNIEnv *env, jclass clss,
                                                      jlong group_id) {
  herr_t retVal = FAIL;
 ...
 *   public H5G_STORAGE_TYPE  storage_type; // Type of storage for links in
 * group public long     nlinks;       // Number of links in group public long
 * max_corder;   // Current max. creation order value for group public int
...
jobject create_H5G_info_t(JNIEnv *env, H5G_info_t group_info) {
  ...
  if (NULL ==
      (cls = ENVPTR->FindClass(ENVONLY, "hdf/hdf5lib/structs/H5G_info_t")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL == (obj = ENVPTR->AllocObject(ENVONLY, cls)))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL == (fid_storage_type =
                   ENVPTR->GetFieldID(ENVONLY, cls, "storage_type", "I")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (NULL ==
      (fid_max_corder = ENVPTR->GetFieldID(ENVONLY, cls, "max_corder", "J")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  ENVPTR->SetLongField(ENVONLY, obj, fid_max_corder,
                       (jlong)group_info.max_corder);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Gcreate2(
    JNIEnv *env, jclass clss, jlong loc_id, jstring name, jlong link_plist_id,
  ...
  PIN_JAVA_STRING(ENVONLY, name, grpName, NULL,
                  "H5Gcreate2: group name not pinned");

  if ((group_id = H5Gcreate2((hid_t)loc_id, grpName, (hid_t)link_plist_id,
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Gcreate_1anon(
  ...
  if ((group_id =
           H5Gcreate_anon((hid_t)loc_id, (hid_t)gcpl_id, (hid_t)gapl_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Gopen2(JNIEnv *env, jclass clss,
                                                       jlong loc_id,
                                                       jstring name,
  ...
  PIN_JAVA_STRING(ENVONLY, name, grpName, NULL,
                  "H5Gopen2: group name not pinned");
 ...
 */
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Gget_1info(JNIEnv *env,
                                                           jclass clss,
                                                           jlong loc_id) {
  ...
  if (NULL == (obj = create_H5G_info_t(env, group_info)))
    H5_JNI_FATAL_ERROR(ENVONLY,
                       "H5Gget_info: unable to create H5G_info_t object");
...
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Gget_1info_1by_1name(
  ...
  PIN_JAVA_STRING(ENVONLY, name, grpName, NULL,
                  "H5Gget_info_by_name: group name not pinned");

  if ((ret_val = H5Gget_info_by_name((hid_t)loc_id, grpName, &group_info,
                                     (hid_t)lapl_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (NULL == (obj = create_H5G_info_t(env, group_info))) {
    ...
    H5_JNI_FATAL_ERROR(
        ENVONLY, "H5Gget_info_by_name: unable to create H5G_info_t object");
  }
...
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Gget_1info_1by_1idx(
  ...
  PIN_JAVA_STRING(ENVONLY, name, grpName, NULL,
                  "H5Gget_info_by_idx: group name not pinned");

  if ((ret_val = H5Gget_info_by_idx((hid_t)loc_id, grpName, cindex_type, corder,
                                    (hsize_t)n, &group_info, (hid_t)lapl_id)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (NULL == (obj = create_H5G_info_t(env, group_info))) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_JNI_FATAL_ERROR(
        ENVONLY, "H5Gget_info_by_idx: unable to create H5G_info_t object");
  }
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Gflush(JNIEnv *env, jclass clss,
                                                    jlong loc_id) {
  UNUSED(clss);
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Grefresh(JNIEnv *env, jclass clss,
                                                      jlong loc_id) {
  UNUSED(clss);
