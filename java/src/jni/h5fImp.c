 ...
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Fopen(JNIEnv *env, jclass clss,
...
Java_hdf_hdf5lib_H5__1H5Fcreate(JNIEnv *env, jclass clss, jstring name,
                                jint flags, jlong create_id, jlong access_id) {
  const char *fileName = NULL;
  hid_t status = H5I_INVALID_HID;
  ...
  PIN_JAVA_STRING(ENVONLY, name, fileName, NULL,
                  "H5Fcreate: file name not pinned");
 ...
 * Signature: (JI)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Fflush(JNIEnv *env, jclass clss,
                                                    jlong object_id,
                                                    jint scope) {
  herr_t retVal = FAIL;
 ...
 */
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Fget_1name(JNIEnv *env,
                                                           jclass clss,
                                                           jlong file_id) {
  jstring str = NULL;
  ssize_t buf_size;
 ...
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Fis_1hdf5(JNIEnv *env,
                                                           jclass clss,
                                                           jstring name) {
  ...
  PIN_JAVA_STRING(ENVONLY, name, fileName, NULL,
                  "H5Fis_hdf5: file name not pinned");
 ...
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Fis_1accessible(
    JNIEnv *env, jclass clss, jstring name, jlong fapl_id) {
  const char *fileName = NULL;
  htri_t bval = JNI_FALSE;
  ...
  PIN_JAVA_STRING(ENVONLY, name, fileName, NULL,
                  "H5Fis_accessible: file name not pinned");
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Fget_1intent(JNIEnv *env,
                                                          jclass clss,
                                                          jlong file_id) {
  unsigned intent = 0;
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Fget_1fileno(JNIEnv *env,
                                                           jclass clss,
                                                           jlong file_id) {
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5__1H5Fclose(JNIEnv *env, jclass clss,
                                                      jlong file_id) {
  herr_t status = FAIL;
 ...
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Fmount(JNIEnv *env, jclass clss,
  ...
  PIN_JAVA_STRING(ENVONLY, name, fileName, NULL,
                  "H5Fmount: file name not pinned");

  if ((status = H5Fmount((hid_t)loc_id, fileName, (hid_t)child_id,
                         (hid_t)plist_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Funmount(JNIEnv *env, jclass clss,
                                                      jlong loc_id,
                                                      jstring name) {
  const char *fileName = NULL;
  herr_t status = FAIL;
  ...
  PIN_JAVA_STRING(ENVONLY, name, fileName, NULL,
                  "H5Funmount: file name not pinned");
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Fget_1freespace(JNIEnv *env,
                                                              jclass clss,
                                                              jlong file_id) {
  hssize_t ret_val = -1;
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5__1H5Freopen(JNIEnv *env,
                                                        jclass clss,
                                                        jlong file_id) {
  hid_t retVal = H5I_INVALID_HID;
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Fget_1obj_1ids(
  ...
  PIN_LONG_ARRAY(ENVONLY, obj_id_list, obj_id_listP, &isCopy,
                 "H5Fget_obj_ids: obj_id_list not pinned");
  ...
  if ((ret_val = H5Fget_obj_ids((hid_t)file_id, (unsigned int)types,
                                (size_t)maxObjs, id_list)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (id_list)
    free(id_list);
  if (obj_id_listP)
    UNPIN_LONG_ARRAY(ENVONLY, obj_id_list, obj_id_listP,
                     (ret_val < 0) ? JNI_ABORT : 0);
 ...
 * Signature: (JI)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Fget_1obj_1count(JNIEnv *env,
                                                               jclass clss,
                                                               jlong file_id,
                                                               jint types) {
  ssize_t ret_val = -1;
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Fget_1filesize(JNIEnv *env,
                                                             jclass clss,
                                                             jlong file_id) {
  hsize_t size = 0;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Fget_1mdc_1size(
  ...
  if (NULL == metadata_cache)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Fget_mdc_size: metadata cache array is NULL");

  if ((size = ENVPTR->GetArrayLength(ENVONLY, metadata_cache)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Fget_mdc_size: metadata_cache length < 0");
  }

  if (size < 3)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Fget_mdc_size: length of metadata_cache array < 3");

  if (H5Fget_mdc_size((hid_t)file_id, &max_size, &min_clean_size, &cur_size,
                      &cur_num_entries) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 */
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Fget_1info(JNIEnv *env,
                                                           jclass clss,
                                                           jlong obj_id) {
  H5F_info2_t finfo;
  jobject ihinfobuf;
  ...
  CALL_CONSTRUCTOR(ENVONLY, "hdf/hdf5lib/structs/H5F_info2_t",
                   "(IJJIJJIJLhdf/hdf5lib/structs/H5_ih_info_t;)V", args,
                   ret_obj);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Fget_1mdc_1logging_1status(
  ...
  if (NULL == mdc_logging_status)
    H5_NULL_ARGUMENT_ERROR(
        ENVONLY, "H5Fget_mdc_logging_status: mdc_logging_status is NULL");

  if ((size = ENVPTR->GetArrayLength(ENVONLY, mdc_logging_status)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Fget_mdc_logging_status: mdc_logging_status length < 0");
  }

  if (size < 2)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Fget_mdc_logging_status: length of mdc_logging_status < 2");

  if (H5Fget_mdc_logging_status((hid_t)file_id, &is_enabled,
                                &is_currently_logging) < 0)
    H5_LIBRARY_ERROR(ENVONLY);

  PIN_BOOL_ARRAY(
      ENVONLY, mdc_logging_status, mdc_logging_status_ptr, &isCopy,
 ...
 * Signature: (JZ)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Fset_1dset_1no_1attrs_1hint(
  ...
  if ((retVal = H5Fset_dset_no_attrs_hint((hid_t)file_id, (bool)minimize_val)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Fget_1dset_1no_1attrs_1hint(
    JNIEnv *env, jclass clss, jlong file_id) {
 ...
 * Signature: (JII)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Fset_1libver_1bounds(
    JNIEnv *env, jclass clss, jlong file_id, jint low, jint high) {
  herr_t retVal = FAIL;

  UNUSED(clss);

  if ((retVal = H5Fset_libver_bounds((hid_t)file_id, (H5F_libver_t)low,
                                     (H5F_libver_t)high)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
