...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1local_1heap_1size_1hint(
  ...
  if ((retVal =
           H5Pset_local_heap_size_hint((hid_t)gcpl_id, (size_t)size_hint)) < 0)
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1link_1creation_1order(
  ...
  if ((retVal = H5Pset_link_creation_order((hid_t)gcpl_id,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1est_1link_1info(
    JNIEnv *env, jclass clss, jlong gcpl_id, jint est_num_entries,
    jint est_name_len) {
  herr_t retVal = FAIL;
  ...
  if ((est_num_entries > 65535) || (est_name_len > 65535))
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Pset_est_link_info: est. name length or "
                                   "number of entries must be < 65536");

  if ((retVal = H5Pset_est_link_info((hid_t)gcpl_id, (unsigned)est_num_entries,
                                     (unsigned)est_name_len)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1est_1link_1info(
  ...
  PIN_INT_ARRAY(ENVONLY, link_info, theArray, &isCopy,
                "H5Pget_est_link_info: input not pinned");
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1link_1phase_1change(
  ...
  if (max_compact < min_dense)
    H5_BAD_ARGUMENT_ERROR(ENVONLY, "H5Pset_link_phase_change: max compact "
                                   "value must be >= min dense value");
  if (max_compact > 65535)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Pset_link_phase_change: max compact value must be < 65536");
  if (min_dense > 65535)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Pset_link_phase_change: min dense value must be < 65536");

  if ((retVal = H5Pset_link_phase_change((hid_t)gcpl_id, (unsigned)max_compact,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1link_1phase_1change(
  ...
  PIN_INT_ARRAY(ENVONLY, links, theArray, &isCopy,
                "H5Pget_link_phase_change: input not pinned");

  if ((retVal =
           H5Pget_link_phase_change((hid_t)gcpl_id, (unsigned *)&(theArray[0]),
                                    (unsigned *)&(theArray[1]))) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
