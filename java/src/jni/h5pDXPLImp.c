...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1buffer(
    JNIEnv *env, jclass clss, jlong plist, jlong size, jbyteArray tconv,
    jbyteArray bkg) {
#ifdef notdef
  jboolean isCopy;
  jbyte *tconvP = NULL;
...
#ifdef notdef
  if (NULL != tconv)
    PIN_BYTE_ARRAY(ENVONLY, tconv, tconvP, &isCopy,
                   "H5Pset_buffer: tconv not pinned");
  if (NULL != bkg)
    PIN_BYTE_ARRAY(ENVONLY, bkg, bkgP, &isCopy,
                   "H5Pset_buffer: bkg not pinned");
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1buffer(
  ...
  PIN_BYTE_ARRAY(ENVONLY, tconv, tconvP, &isCopy,
                 "H5Pget_buffer: tconv not pinned");
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1buffer_1size(JNIEnv *env,
                                                                jclass clss,
                                                                jlong plist,
                                                                jlong size) {
  UNUSED(clss);
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Pget_1buffer_1size(JNIEnv *env,
                                                                 jclass clss,
                                                                 jlong plist) {
  size_t size = 0;
 ...
 * Signature: (JZ)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1preserve(JNIEnv *env,
                                                            jclass clss,
                                                            jlong plist,
                                                            jboolean status) {
  bool st = JNI_FALSE;
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1preserve(JNIEnv *env,
                                                            jclass clss,
                                                            jlong plist) {
  herr_t retVal = FAIL;
 ...
 * Signature: (JI)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1edc_1check(JNIEnv *env,
                                                              jclass clss,
                                                              jlong plist,
                                                              jint check) {
  herr_t retVal = FAIL;
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1edc_1check(JNIEnv *env,
                                                              jclass clss,
                                                              jlong plist) {
  H5Z_EDC_t retVal = -1;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1data_1transform(
  ...
  if (NULL == expression)
    H5_NULL_ARGUMENT_ERROR(
        ENVONLY, "H5Pset_data_transform: data transform expression is NULL");

  PIN_JAVA_STRING(
      ENVONLY, expression, express, NULL,
...
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Pget_1data_1transform(
    JNIEnv *env, jclass clss, jlong plist_id, jobjectArray expression,
    jlong size) {
  jstring str = NULL;
  char *express = NULL;
  ...
  if ((express_size = H5Pget_data_transform((hid_t)plist_id, (char *)NULL,
                                            (size_t)size)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);

  if (NULL ==
      (express = (char *)malloc(sizeof(char) * (size_t)express_size + 1)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Pget_data_transform: memory allocation failed");

  if (H5Pget_data_transform((hid_t)plist_id, express,
                            (size_t)express_size + 1) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (NULL == (str = ENVPTR->NewStringUTF(ENVONLY, express))) {
    ...
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Pget_data_transform: out of memory - unable to "
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1hyper_1vector_1size(
  ...
  if ((retVal = H5Pset_hyper_vector_size((hid_t)plist, (size_t)vector_size)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1hyper_1vector_1size(
  ...
  if (NULL == vector_size)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_hyper_vector_size: vector_size is NULL");
  ...
  if (theArray)
    UNPIN_LONG_ARRAY(ENVONLY, vector_size, theArray,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1btree_1ratios(
    JNIEnv *env, jclass clss, jlong plist_id, jdouble left, jdouble middle,
    jdouble right) {
  herr_t status = FAIL;

  UNUSED(clss);

  if ((status = H5Pset_btree_ratios((hid_t)plist_id, (double)left,
                                    (double)middle, (double)right)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1btree_1ratios(
  ...
  if (NULL == left)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_btree_ratios: left input array is NULL");
  if (NULL == middle)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_btree_ratios: middle input array is NULL");
  if (NULL == right)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_btree_ratios: right input array is NULL");

  PIN_DOUBLE_ARRAY(ENVONLY, left, leftP, &isCopy,
                   "H5Pget_btree_ratios: left array not pinned");
  PIN_DOUBLE_ARRAY(ENVONLY, middle, middleP, &isCopy,
                   "H5Pget_btree_ratios: middle array not pinned");
  PIN_DOUBLE_ARRAY(ENVONLY, right, rightP, &isCopy,
                   "H5Pget_btree_ratios: right array not pinned");
