 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_hdf_hdf5lib_H5_H5Pget_1driver(JNIEnv *env,
                                                           jclass clss,
                                                           jlong plist) {
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fclose_1degree(
  ...
  if ((retVal = H5Pset_fclose_degree((hid_t)plist,
                                     (H5F_close_degree_t)fc_degree)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1fclose_1degree(JNIEnv *env,
                                                                  jclass clss,
                                                                  jlong plist) {
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1core(
    JNIEnv *env, jclass clss, jlong fapl_id, jlong increment,
    jboolean backing_store) {
  herr_t retVal = FAIL;

  UNUSED(clss);

  if ((retVal = H5Pset_fapl_core((hid_t)fapl_id, (size_t)increment,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1fapl_1core(
    JNIEnv *env, jclass clss, jlong fapl_id, jlongArray increment,
  ...
  PIN_LONG_ARRAY(ENVONLY, increment, incArray, &isCopy,
                 "H5Pget_fapl_core: incArray not pinned");
  PIN_BOOL_ARRAY(ENVONLY, backing_store, backArray, &isCopy,
                 "H5Pget_fapl_core: backArray not pinned");

  {
    ...
    if ((status = H5Pget_fapl_core((hid_t)fapl_id, &inc_t, (bool *)backArray)) <
        0)
      H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (backArray)
    UNPIN_BOOL_ARRAY(ENVONLY, backing_store, backArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (incArray)
    UNPIN_LONG_ARRAY(ENVONLY, increment, incArray,
                     (status < 0) ? JNI_ABORT : 0);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1direct(
    JNIEnv *env, jclass clss, jlong fapl_id, jlong alignment, jlong block_size,
    jlong cbuf_size) {
  herr_t retVal = FAIL;
  ...
  if ((retVal = H5Pset_fapl_direct((hid_t)fapl_id, (size_t)alignment,
                                   (size_t)block_size, (size_t)cbuf_size)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1fapl_1direct(
  ...
  if (NULL == info)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_fapl_direct: info input array is NULL");
  ...
  PIN_LONG_ARRAY(ENVONLY, info, theArray, &isCopy,
                 "H5Pget_fapl_direct: info not pinned");

  if ((retVal = H5Pget_fapl_direct((hid_t)fapl_id, &alignment, &block_size,
                                   &cbuf_size)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1family(
  ...
  if ((retVal = H5Pset_fapl_family((hid_t)plist, (hsize_t)ms,
                                   (hid_t)memb_plist)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1fapl_1family(
  ...
  PIN_LONG_ARRAY(ENVONLY, memb_size, sizeArray, &isCopy,
                 "H5Pget_family: sizeArray not pinned");
  ...
  PIN_LONG_ARRAY(ENVONLY, memb_plist, plistArray, &isCopy,
                 "H5Pget_family: plistArray not pinned");
  ...
  if (plistArray)
    UNPIN_LONG_ARRAY(ENVONLY, memb_plist, plistArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (sa)
    free(sa);
  if (sizeArray)
    UNPIN_LONG_ARRAY(ENVONLY, memb_size, sizeArray,
                     (status < 0) ? JNI_ABORT : 0);
 ...
 */
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Pget_1fapl_1hdfs(
    JNIEnv *env, jclass clss, jlong fapl_id) {
  ...
  if (NULL ==
      (j_namenode_name = ENVPTR->NewStringUTF(ENVONLY, fa.namenode_name))) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
  ...
  if (NULL == (j_kerb_cache_path =
                   ENVPTR->NewStringUTF(ENVONLY, fa.kerberos_ticket_cache))) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "H5Pget_fapl_hdfs: out of memory - can't "
                                    "create kerberos_ticket_cache string");
  }
  ...
  CALL_CONSTRUCTOR(
      ...
      "(Ljava/lang/String;ILjava/lang/String;Ljava/lang/String;I)V", args,
      ret_obj);
#else
  UNUSED(fapl_id);
 ...
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1hdfs(
  ...
  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "namenode_name",
                                        "Ljava/lang/String;")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL ==
      (j_str = (jstring)ENVPTR->GetObjectField(ENVONLY, fapl_config, fid)))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "user_name",
                                        "Ljava/lang/String;")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (j_str) {
    PIN_JAVA_STRING(ENVONLY, j_str, str, NULL,
                    "H5FDset_fapl_hdfs: fapl_config user_name not pinned");
  ...
  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "kerberos_ticket_cache",
                                        "Ljava/lang/String;")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (j_str) {
    PIN_JAVA_STRING(
        ENVONLY, j_str, str, NULL,
        "H5FDset_fapl_hdfs: fapl_config kerberos_ticket_cache not pinned");

    strncpy(instance.kerberos_ticket_cache, str,
            H5FD__HDFS_KERB_CACHE_PATH_SPACE + 1);
    instance.kerberos_ticket_cache[H5FD__HDFS_KERB_CACHE_PATH_SPACE] = '\0';

    UNPIN_JAVA_STRING(ENVONLY, j_str, str);
    str = NULL;
  } else
    memset(instance.kerberos_ticket_cache, 0,
           H5FD__HDFS_KERB_CACHE_PATH_SPACE + 1);

  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "stream_buffer_size", "I")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1log(
    JNIEnv *env, jclass clss, jlong fapl_id, jstring logfile, jlong flags,
    jlong buf_size) {
  ...
  PIN_JAVA_STRING(ENVONLY, logfile, pLogfile, NULL,
                  "H5Pset_fapl_log: log file name not pinned");

  if ((retVal = H5Pset_fapl_log((hid_t)fapl_id, pLogfile,
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1multi(
  ...
  if (memb_map)
    PIN_INT_ARRAY(ENVONLY, memb_map, themapArray, &isCopy,
                  "H5Pset_fapl_multi: memb_map not pinned");
  if (memb_fapl)
  ...
  if (memb_name) {
    for (i = 0; i < H5FD_MEM_NTYPES; i++) {
      ...
      if (NULL == (obj = (jstring)ENVPTR->GetObjectArrayElement(
                       ENVONLY, (jobjectArray)memb_name, (jsize)i))) {
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      ...
      PIN_JAVA_STRING(ENVONLY, obj, utf8, NULL,
                      "H5Pset_fapl_multi: string not pinned");

      str_len = strlen(utf8);

      if (NULL == (member_name[i] = (char *)malloc(str_len + 1)))
        H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                               "H5Pset_fapl_multi: memory allocation failed");
  ...
  if ((status = H5Pset_fapl_multi((hid_t)tid, (const H5FD_mem_t *)themapArray,
                                  (const hid_t *)thefaplArray, mName,
                                  (const haddr_t *)theaddrArray, (bool)relax)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (memb_name && mName) {
    ...
    for (i = 0; i < H5FD_MEM_NTYPES; i++) {
      ...
      if (NULL == (rstring = ENVPTR->NewStringUTF(ENVONLY, member_name[i]))) {
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
        H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                               "H5Pset_fapl_multi: out of memory - unable to "
                               "construct string from UTF characters");
      }

      if (NULL ==
          (o = ENVPTR->GetObjectArrayElement(ENVONLY, memb_name, (jsize)i))) {
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      ...
      if (JNI_TRUE == (bb = ENVPTR->IsInstanceOf(ENVONLY, o, Sjc))) {
        ENVPTR->SetObjectArrayElement(ENVONLY, memb_name, (jsize)i,
                                      (jobject)rstring);
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      }
  ...
  if (utf8)
    UNPIN_JAVA_STRING(ENVONLY, obj, utf8);
  if (theaddrArray)
    UNPIN_LONG_ARRAY(ENVONLY, memb_addr, theaddrArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (thefaplArray)
    UNPIN_LONG_ARRAY(ENVONLY, memb_fapl, thefaplArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (themapArray)
    UNPIN_INT_ARRAY(ENVONLY, memb_map, themapArray,
                    (status < 0) ? JNI_ABORT : 0);
...
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Pget_1fapl_1multi(
  ...
  if (memb_map)
    PIN_INT_ARRAY(ENVONLY, memb_map, themapArray, &isCopy,
                  "H5Pget_fapl_multi: memb_map not pinned");
  if (memb_fapl)
    PIN_LONG_ARRAY(ENVONLY, memb_fapl, thefaplArray, &isCopy,
  ...
  if (memb_name)
    if (NULL == (mName = (char **)calloc(H5FD_MEM_NTYPES, sizeof(*mName))))
      H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                             "H5Pget_fapl_multi: memory allocation failed");

  if ((status = H5Pget_fapl_multi((hid_t)tid, (H5FD_mem_t *)themapArray,
                                  (hid_t *)thefaplArray, mName,
    ...
    for (i = 0; i < H5FD_MEM_NTYPES; i++) {
      if (mName[i]) {
        if (NULL == (str = ENVPTR->NewStringUTF(ENVONLY, mName[i]))) {
          CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
          H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                                 "H5Pget_fapl_multi: out of memory - unable to "
                                 "construct string from UTF characters");
        }
 ...
 */
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Pget_1fapl_1ros3(
    JNIEnv *env, jclass clss, jlong fapl_id) {
 ...
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1ros3(
    JNIEnv *env, jclass clss, jlong fapl_id, jobject fapl_config) {
#ifdef H5_HAVE_ROS3_VFD
  ...
  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "aws_region",
                                        "Ljava/lang/String;")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (j_str) {
    PIN_JAVA_STRING(ENVONLY, j_str, str, NULL,
                    "H5Pset_fapl_ros3: fapl_config aws_region not pinned");
  ...
  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "secret_id",
                                        "Ljava/lang/String;")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (j_str) {
    PIN_JAVA_STRING(ENVONLY, j_str, str, NULL,
                    "H5Pset_fapl_ros3: fapl_config secret_id not pinned");
  ...
  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "secret_key",
                                        "Ljava/lang/String;")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (j_str) {
    PIN_JAVA_STRING(ENVONLY, j_str, str, NULL,
                    "H5Pset_fapl_ros3: fapl_config secret_key not pinned");
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1split(
    JNIEnv *env, jclass clss, jlong fapl_id, jstring metaext, jlong meta_pl_id,
  ...
  PIN_JAVA_STRING(ENVONLY, metaext, mstr, NULL,
                  "H5Pset_fapl_split: metaext not pinned");
  PIN_JAVA_STRING(ENVONLY, rawext, rstr, NULL,
                  "H5Pset_fapl_split: rawext not pinned");

  if ((retVal = H5Pset_fapl_split((hid_t)fapl_id, mstr, (hid_t)meta_pl_id, rstr,
                                  (hid_t)raw_pl_id)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1sec2(JNIEnv *env,
                                                              jclass clss,
                                                              jlong fapl_id) {
  herr_t retVal = FAIL;
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1fapl_1stdio(JNIEnv *env,
                                                               jclass clss,
                                                               jlong fapl_id) {
  herr_t retVal = FAIL;
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1alignment(
  ...
  if ((retVal = H5Pset_alignment((hid_t)plist, (hsize_t)thr, (hsize_t)align)) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1alignment(
  ...
  if (NULL == alignment)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_alignment: input alignment is NULL");

  if ((arrLen = ENVPTR->GetArrayLength(ENVONLY, alignment)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pget_alignment: alignment array length < 0");
  }
  if (arrLen < 2)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pget_alignment: alignment input array < 2");

  PIN_LONG_ARRAY(ENVONLY, alignment, theArray, &isCopy,
                 "H5Pget_alignment: input array not pinned");
  ...
  if (theArray)
    UNPIN_LONG_ARRAY(ENVONLY, alignment, theArray,
                     (status < 0) ? JNI_ABORT : 0);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1cache(
  ...
  if (NULL != rdcc_w0)
    PIN_DOUBLE_ARRAY(ENVONLY, rdcc_w0, w0Array, &isCopy,
                     "H5Pget_cache: rdcc_w0 array not pinned");
  if (NULL != rdcc_nelmts)
  ...
  if (NULL == nbytesArray)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "nbytesArray should not be NULL after pinning");
  ...
  if (nbytesArray)
    UNPIN_LONG_ARRAY(ENVONLY, rdcc_nbytes, nbytesArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (rdcc_nelmtsArray)
    UNPIN_LONG_ARRAY(ENVONLY, rdcc_nelmts, rdcc_nelmtsArray,
                     (status < 0) ? JNI_ABORT : 0);
  if (w0Array)
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1evict_1on_1close(
  ...
  if ((retVal =
           H5Pset_evict_on_close((hid_t)fapl_id, (bool)evict_on_close_val)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1file_1locking(
    JNIEnv *env, jclass clss, jlong fapl_id, jboolean use_file_locking,
  ...
  if (H5Pset_file_locking((hid_t)fapl_id, use_file_locking_val,
                          ignore_when_disabled_val) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 * Signature: (J)Z
 */
JNIEXPORT jboolean JNICALL Java_hdf_hdf5lib_H5_H5Pget_1use_1file_1locking(
    JNIEnv *env, jclass clss, jlong fapl_id) {
  bool use_file_locking_val = true;
  bool unused = true;
  jboolean bval = JNI_FALSE;
...
Java_hdf_hdf5lib_H5_H5Pget_1ignore_1disabled_1file_1locking(JNIEnv *env,
                                                            jclass clss,
                                                            jlong fapl_id) {
  bool ignore_when_disabled_val = true;
  bool unused = true;
  ...
  if (H5Pget_file_locking((hid_t)fapl_id, &unused, &ignore_when_disabled_val) <
      0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1metadata_1read_1attempts(
  ...
  if (attempts <= 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pset_metadata_read_attempts: attempts <= 0");
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1mdc_1config(
  ...
  cacheinfo.rpt_fcn_enabled =
      ENVPTR->GetBooleanField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "open_trace_file", "Z")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.open_trace_file =
      ENVPTR->GetBooleanField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  cacheinfo.close_trace_file =
      ENVPTR->GetBooleanField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "trace_file_name",
                                        "Ljava/lang/String;")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL ==
      (j_str = (jstring)ENVPTR->GetObjectField(ENVONLY, cache_config, fid)))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (j_str) {
    PIN_JAVA_STRING(ENVONLY, j_str, str, NULL,
                    "H5Pset_mdc_config: cache_config not pinned");
  ...
  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "evictions_enabled", "Z")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.evictions_enabled =
      ENVPTR->GetBooleanField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  cacheinfo.initial_size =
      (size_t)ENVPTR->GetLongField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "min_clean_fraction", "D")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.min_clean_fraction =
      ENVPTR->GetDoubleField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  cacheinfo.epoch_length =
      (long int)ENVPTR->GetLongField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL == (fid = ENVPTR->GetFieldID(ENVONLY, cls, "incr_mode", "I")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  incr_mode = ENVPTR->GetIntField(ENVONLY, cache_config,
                                  fid); /*(enum H5C_cache_incr_mode) */
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  cacheinfo.incr_mode = (enum H5C_cache_incr_mode)incr_mode;

  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "lower_hr_threshold", "D")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.lower_hr_threshold =
      ENVPTR->GetDoubleField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "apply_max_increment", "Z")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.apply_max_increment =
      ENVPTR->GetBooleanField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  flash_incr_mode = ENVPTR->GetIntField(
      ENVONLY, cache_config, fid); /*(enum H5C_cache_flash_incr_mode) */
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  decr_mode = ENVPTR->GetIntField(ENVONLY, cache_config,
                                  fid); /*(enum H5C_cache_decr_mode) */
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  cacheinfo.decr_mode = (enum H5C_cache_decr_mode)decr_mode;

  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "upper_hr_threshold", "D")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.upper_hr_threshold =
      ENVPTR->GetDoubleField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "apply_max_decrement", "Z")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.apply_max_decrement =
      ENVPTR->GetBooleanField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  cacheinfo.max_decrement =
      (size_t)ENVPTR->GetLongField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "epochs_before_eviction", "I")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.epochs_before_eviction =
      ENVPTR->GetIntField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "apply_empty_reserve", "Z")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.apply_empty_reserve =
      ENVPTR->GetBooleanField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "dirty_bytes_threshold", "J")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.dirty_bytes_threshold =
      (size_t)ENVPTR->GetLongField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  if (NULL ==
      (fid = ENVPTR->GetFieldID(ENVONLY, cls, "metadata_write_strategy", "I")))
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

  cacheinfo.metadata_write_strategy =
      ENVPTR->GetIntField(ENVONLY, cache_config, fid);
  CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
 ...
 */
JNIEXPORT jobject JNICALL Java_hdf_hdf5lib_H5_H5Pget_1mdc_1config(JNIEnv *env,
                                                                  jclass clss,
                                                                  jlong plist) {
  H5AC_cache_config_t cacheinfo;
  jstring j_str = NULL;
  ...
  if (NULL ==
      (j_str = ENVPTR->NewStringUTF(ENVONLY, cacheinfo.trace_file_name))) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "H5Pget_mdc_config: out of memory - unable "
                                    "to construct string from UTF characters");
  }
  ...
  CALL_CONSTRUCTOR(ENVONLY, "hdf/hdf5lib/structs/H5AC_cache_config_t",
                   "(IZZZLjava/lang/String;ZZJDJJJIDDZJIDDIDDZJIZDJI)V", args,
                   ret_obj);
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5Pset_1mdc_1log_1options(
    JNIEnv *env, jclass clss, jlong fapl_id, jboolean is_enabled,
  ...
  if (NULL == location)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pset_mdc_log_options: location string is NULL");

  PIN_JAVA_STRING(ENVONLY, location, locStr, NULL,
                  "H5Pset_mdc_log_options: location string not pinned");

  if ((retVal = H5Pset_mdc_log_options((hid_t)fapl_id, (bool)is_enabled, locStr,
                                       (bool)start_on_access)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jstring JNICALL Java_hdf_hdf5lib_H5_H5Pget_1mdc_1log_1options(
  ...
  if (NULL == mdc_log_options)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_mdc_log_options: mdc_log_options is NULL");

  if ((arrLen = ENVPTR->GetArrayLength(ENVONLY, mdc_log_options)) < 0) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Pget_mdc_log_options: mdc_log_options array length < 0");
  }
  if (arrLen < 2)
    H5_BAD_ARGUMENT_ERROR(
        ENVONLY, "H5Pget_mdc_log_options: length of mdc_log_options < 2");
  ...
  if (H5Pget_mdc_log_options((hid_t)fapl_id, &is_enabled, NULL, &location_size,
                             &start_on_access) < 0)
    H5_LIBRARY_ERROR(ENVONLY);

  if (!location_size)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "H5Pget_mdc_log_options: location_size is 0");

  location_size++; /* add extra space for the null terminator */
  if (NULL == (lname = (char *)malloc(sizeof(char) * location_size)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Pget_mdc_log_options: memory allocation failed");

  if ((status = H5Pget_mdc_log_options((hid_t)fapl_id, &is_enabled, lname,
                                       &location_size, &start_on_access)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (NULL == (str = ENVPTR->NewStringUTF(ENVONLY, lname))) {
    CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5Pget_mdc_log_options: out of memory - unable to "
                           "construct string from UTF characters");
  }
  ...
  if (mdc_log_options_ptr)
    UNPIN_BOOL_ARRAY(ENVONLY, mdc_log_options, mdc_log_options_ptr,
                     (status < 0) ? JNI_ABORT : 0);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pset_1libver_1bounds(
  ...
  if ((retVal = H5Pset_libver_bounds((hid_t)fapl_id, (H5F_libver_t)low,
                                     (H5F_libver_t)high)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Pget_1libver_1bounds(
  ...
  if (NULL == libver)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5Pget_libver_bounds: libversion bounds is NULL");

  PIN_INT_ARRAY(ENVONLY, libver, theArray, &isCopy,
                "H5Pget_libver_bounds: input not pinned");

  if ((retVal =
           H5Pget_libver_bounds((hid_t)fapl_id, (H5F_libver_t *)&(theArray[0]),
