  ...
  {                                                                            \
    ...
    if (NULL == (jc = ENVPTR->FindClass(ENVONLY, (className))))                \
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);                                 \
                                                                               \
    if (NULL == (jm = ENVPTR->GetMethodID(ENVONLY, jc, "<init>",               \
                                          "(Ljava/lang/String;)V"))) {         \
      ...
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);                                 \
                                                                               ...
                                                                               \
    if (NULL ==                                                                \
        (ex = ENVPTR->NewObjectA(ENVONLY, jc, jm, (jvalue *)(args)))) {        \
...
static jboolean H5JNIErrorClass(JNIEnv *env, const char *message,
                                const char *className);

/* get the major and minor error numbers on the top of the error stack */
static herr_t walk_error_callback(unsigned n, const H5E_error2_t *err_desc,
                                  void *_err_nums) {
...
Java_hdf_hdf5lib_exceptions_HDF5LibraryException_printStackTrace0(
  ...
  if (NULL == file_name) {
    ...
    PIN_JAVA_STRING(ENVONLY, file_name, file, NULL,
                    "printStackTrace0: file name not pinned");
...
Java_hdf_hdf5lib_exceptions_HDF5LibraryException__1getMajorErrorNumber(
  ...
  if (H5Ewalk2(H5E_DEFAULT, H5E_WALK_DOWNWARD, walk_error_callback, &err_nums) <
      0)
    return -1;
 ...
 *  exception.
 */
jboolean h5outOfMemory(JNIEnv *env, const char *message) {
 ...
 *  exception.
 */
jboolean h5assertion(JNIEnv *env, const char *message) {
 ...
 *  exception.
 */
jboolean h5JNIFatalError(JNIEnv *env, const char *message) {
 ...
 *  exception.
 */
jboolean h5nullArgument(JNIEnv *env, const char *message) {
 ...
 *  exception.
 */
jboolean h5badArgument(JNIEnv *env, const char *message) {
 ...
 *  exception.
 */
jboolean h5unimplemented(JNIEnv *env, const char *message) {
 ...
 *  exception.
 */
jboolean h5raiseException(JNIEnv *env, const char *message,
...
jboolean h5libraryError(JNIEnv *env) {
  ...
  if ((stk_id = H5Eget_current_stack()) >= 0)
    ...
    if (H5Ewalk2(stk_id, H5E_WALK_DOWNWARD, walk_error_callback,
                 &exceptionNumbers) < 0)
      goto done;
  ...
  if (msg_size > 0) {
    if (NULL == (msg_str = (char *)calloc((size_t)msg_size + 1, sizeof(char))))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY,
          "h5libraryerror: failed to allocate buffer for error message");

    if ((msg_size = H5Eget_msg(min_num, &error_msg_type, msg_str,
                               (size_t)msg_size + 1)) < 0)
      goto done;
    msg_str[msg_size] = '\0';
