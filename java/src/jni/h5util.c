...
int H5TOOLS_TEXT_BLOCK =
    16; /* Number of elements on a line in a text export file */
...
int h5str_old_region_dataset(JNIEnv *env, h5str_t *out_str, hid_t container,
                             void *ref_buf, int expand_data);
int h5str_region_dataset(JNIEnv *env, h5str_t *out_str, H5R_ref_t *ref_vp,
                         int expand_data);

static int h5str_dump_region_blocks(JNIEnv *env, h5str_t *str, hid_t region,
                                    hid_t region_obj, int expand_data);
static int h5str_dump_region_points(JNIEnv *env, h5str_t *str, hid_t region,
                                    hid_t region_obj, int expand_data);
static int h5str_dump_region_attribute(JNIEnv *env, h5str_t *str,
                                       hid_t region_id);
...
static int h5str_dump_simple_data(JNIEnv *env, FILE *stream, hid_t container,
                                  hid_t type, void *_mem, hsize_t nelmts);
static int h5str_render_bin_output(FILE *stream, hid_t container, hid_t tid,
                                   void *_mem, hsize_t block_nelmts);
static int render_bin_output_region_data_blocks(FILE *stream, hid_t region_id,
                                                hid_t container, int ndims,
                                                hid_t type_id, hssize_t nblocks,
                                                hsize_t *ptdata);
static int render_bin_output_region_blocks(FILE *stream, hid_t region_space,
                                           hid_t region_id, hid_t container);
static int render_bin_output_region_data_points(
...
static int render_bin_output_region_points(FILE *stream, hid_t region_space,
                                           hid_t region_id, hid_t container);
jobject translate_atomic_rbuf(JNIEnv *env, jlong mem_type_id,
                              H5T_class_t type_class, void *raw_buf);
void translate_atomic_wbuf(JNIEnv *env, jobject in_obj, jlong mem_type_id,
                           H5T_class_t type_class, void *raw_buf);
...
char *h5str_append(h5str_t *str, const char *cstr) {
  ...
  while (len >=
         str->max) /* not enough to hold the new string, double the space */
  {
 ...
 *        FAILURE: 0
 */
size_t h5str_convert(JNIEnv *env, char **in_str, hid_t container, hid_t tid,
                     void *out_buf, size_t out_buf_offset) {
  unsigned char *ucptr = NULL;
  char fmt_llong[8], fmt_ullong[8];
  H5T_class_t tclass = H5T_NO_CLASS;
  const char delimiter[] =
      " ," H5_COMPOUND_BEGIN_INDICATOR H5_COMPOUND_END_INDICATOR
  ...
  case H5T_FLOAT: {
    ...
    default:
      H5_BAD_ARGUMENT_ERROR(ENVONLY, "h5str_convert: floating-point datatype "
                                     "size didn't match any of expected sizes");
      break;
    }
  ...
  case H5T_INTEGER: {
    ...
    default:
      H5_BAD_ARGUMENT_ERROR(ENVONLY, "h5str_convert: integer datatype size "
                                     "didn't match any of expected sizes");
      break;
    }
  ...
  case H5T_COMPOUND: {
    ...
    for (i = 0; i < (unsigned)n; i++) {
      ...
      if (!h5str_convert(ENVONLY, &this_str, container, mtid, out_buf,
                         member_offset)) {
  ...
  case H5T_ARRAY: {
    ...
    if (NULL == (cptr = (char *)calloc((size_t)total_elmts, baseTypeSize)))
      H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                             "h5str_convert: failed to allocate array buffer");
  ...
  case H5T_VLEN: {
    ...
    if (NULL == (vl_buf->p = malloc(baseTypeSize)))
      H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                             "h5str_convert: failed to allocate vlen buffer");
    vl_buf->len = 1;
    ...
    for (i = 0; cur_char != ')' && cur_char != '\0'; i++) {
      if (i >= vl_buf->len) {
        ...
        if (NULL == (tmp_realloc = (char *)realloc(
                         vl_buf->p, vl_buf->len * 2 * baseTypeSize)))
          H5_OUT_OF_MEMORY_ERROR(
              ENVONLY, "h5str_convert: failed to reallocate vlen buffer");
        vl_buf->len *= 2;
        vl_buf->p = tmp_realloc;
      }

      if (!(h5str_convert(ENVONLY, &this_str, container, mtid, vl_buf->p,
                          i * baseTypeSize))) {
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
        vl_buf->len = i;
        goto done;
 ...
 *-------------------------------------------------------------------------
 */
int h5str_sprint_old_reference(JNIEnv *env, h5str_t *out_str, hid_t region_obj,
                               void *ref_buf) {
  char ref_name[1024];
  int ret_value = FAIL;

  if ((H5Rget_name(region_obj, H5R_DATASET_REGION, ref_buf, (char *)ref_name,
                   1024)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
 ...
 *-------------------------------------------------------------------------
 */
int h5str_sprint_reference(JNIEnv *env, h5str_t *out_str, void *ref_p) {
  ssize_t buf_size;
  char *ref_name = NULL;
  ...
  if (buf_size > 0) {
    ref_name = (char *)malloc(sizeof(char) * (size_t)buf_size + 1);
    if (H5Rget_obj_name(ref_vp, H5P_DEFAULT, ref_name, (size_t)buf_size + 1) >=
        0) {
      ref_name[buf_size] = '\0';
      if (!h5str_append(out_str, ref_name))
        H5_ASSERTION_ERROR(ENVONLY, "Unable to append string.");
...
int h5str_old_region_dataset(JNIEnv *env, h5str_t *out_str, hid_t container,
  ...
  if ((region_obj = H5Rdereference2(container, H5P_DEFAULT, H5R_DATASET_REGION,
                                    ref_buf)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if ((region_type = H5Sget_select_type(region_sid)) > H5S_SEL_ERROR) {
    if (H5S_SEL_POINTS == region_type) {
      if (h5str_dump_region_points(ENVONLY, out_str, region_sid, region_obj,
                                   expand_data) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    } else if (H5S_SEL_HYPERSLABS == region_type) {
      if (h5str_dump_region_blocks(ENVONLY, out_str, region_sid, region_obj,
                                   expand_data) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    }
  }
...
int h5str_region_dataset(JNIEnv *env, h5str_t *out_str, H5R_ref_t *ref_vp,
  ...
  if ((region_type = H5Sget_select_type(new_obj_sid)) > H5S_SEL_ERROR) {
    if (H5S_SEL_POINTS == region_type) {
      if (h5str_dump_region_points(ENVONLY, out_str, new_obj_sid, new_obj_id,
                                   expand_data) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    } else if (H5S_SEL_HYPERSLABS == region_type) {
      if (h5str_dump_region_blocks(ENVONLY, out_str, new_obj_sid, new_obj_id,
                                   expand_data) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    }
  }
 ...
 *        FAILURE: 0
 */
size_t h5str_sprintf(JNIEnv *env, h5str_t *out_str, hid_t container, hid_t tid,
                     void *in_buf, int expand_data) {
  ...
  case H5T_FLOAT: {
    ...
    default:
      H5_BAD_ARGUMENT_ERROR(ENVONLY, "h5str_sprintf: floating-point datatype "
                                     "size didn't match any of expected sizes");
      break;
    }
  ...
  case H5T_STRING: {
    ...
    if (is_variable) {
      /* cp_vp is the pointer into the struct where a `char*' is stored. So we
       * have
  ...
  case H5T_INTEGER: {
    ...
    default:
      H5_BAD_ARGUMENT_ERROR(ENVONLY, "h5str_sprintf: integer datatype size "
                                     "didn't match any of expected sizes");
      break;
    }
  ...
  case H5T_COMPOUND: {
    ...
    for (i = 0; i < (unsigned)n; i++) {
      ...
      if (!h5str_sprintf(ENVONLY, out_str, container, mtid, &cptr[offset],
                         expand_data))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    ...
    if (H5Tenum_nameof(tid, cptr, enum_name, sizeof enum_name) >= 0) {
      ...
      if (1 == typeSize) {
        ...
        for (i = 0; i < typeSize; i++)
          if (snprintf(this_str, this_len, "%s%02x", i ? ":" : "", ucptr[i]) <
              0)
            H5_JNI_FATAL_ERROR(ENVONLY, "h5str_sprintf: snprintf failure");
      }
    }
  ...
  case H5T_REFERENCE: {
    if (H5R_DSET_REG_REF_BUF_SIZE == typeSize) {
      if (h5str_old_region_dataset(ENVONLY, out_str, container, cptr,
                                   expand_data) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      ...
      if (!h5str_is_zero(&loc_ref, H5Tget_size(H5T_STD_REF))) {
        ...
        case H5R_OBJECT1:
          if (H5Rget_obj_type3(&loc_ref, H5P_DEFAULT, &obj_type) >= 0) {
            switch (obj_type) {
            case H5O_TYPE_DATASET:
              if (h5str_region_dataset(ENVONLY, out_str, &loc_ref,
                                       expand_data) < 0)
                CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
              break;
            ...
            default: {
              ...
              H5O_info2_t oi;
              char *obj_tok_str = NULL;
              if ((new_obj_id = H5Ropen_object(&loc_ref, H5P_DEFAULT,
                                               H5P_DEFAULT)) >= 0) {
              ...
              case H5O_TYPE_NAMED_DATATYPE:
                if (snprintf(this_str, this_len, "%s ", H5_TOOLS_DATATYPE) < 0)
                  H5_JNI_FATAL_ERROR(ENVONLY,
                                     "h5str_sprintf: snprintf failure");
                break;
              ...
              /* Print OID */
              {
                ...
                if (NULL == (this_str = malloc(that_len)))
                  H5_OUT_OF_MEMORY_ERROR(
                      ENVONLY,
                      "h5str_sprintf: failed to allocate string buffer");
                if (snprintf(this_str, that_len, "%lu:%s", oi.fileno,
                             token_str) < 0)
                  H5_JNI_FATAL_ERROR(ENVONLY,
                                     "h5str_sprintf: snprintf failure");
        ...
        case H5R_DATASET_REGION2:
          ...
          break;
        case H5R_ATTR:
          if ((new_obj_id = H5Ropen_attr(&loc_ref, H5P_DEFAULT, H5P_DEFAULT)) >=
              0) {
  ...
  case H5T_ARRAY: {
    ...
    for (i = 0; i < total_elmts; i++) {
      if (!h5str_sprintf(ENVONLY, out_str, container, mtid,
                         &(cptr[i * baseSize]), expand_data))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  ...
  case H5T_VLEN: {
    ...
    for (i = 0; i < (unsigned)vl_buf->len; i++) {
      if (!h5str_sprintf(ENVONLY, out_str, container, mtid,
                         &(((char *)vl_buf->p)[i * baseSize]), expand_data))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
 ...
 *
 * Description:
 *      This is a special case subfunction to print the data in a region
 *reference of type blocks.
 *
 * Return:
 *      The function returns FAIL if there was an error, otherwise SUCCEED
 *-------------------------------------------------------------------------
 */
static int h5str_print_region_data_blocks(JNIEnv *env, hid_t region_id,
                                          h5str_t *str, int ndims,
                                          hid_t type_id, hssize_t nblocks,
                                          hsize_t *ptdata) {
  unsigned indx;
  hsize_t *dims1 = NULL;
  ...
  if (nblocks < 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "h5str_print_region_data_blocks: nblocks < 0");
  ...
  if (NULL == (dims1 = (hsize_t *)malloc(sizeof(hsize_t) * (size_t)ndims)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "h5str_print_region_data_blocks: failed to "
                                    "allocate dimension array buffer");
  ...
  if (NULL == (region_buf = malloc(type_size * (size_t)numelem)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "h5str_print_region_data_blocks: failed to allocate region buffer");
  ...
  if (NULL == (start = (hsize_t *)malloc(sizeof(hsize_t) * (size_t)ndims)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "h5str_print_region_data_blocks: failed to "
                                    "allocate hyperslab start buffer");

  if (NULL == (count = (hsize_t *)malloc(sizeof(hsize_t) * (size_t)ndims)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "h5str_print_region_data_blocks: failed to "
                                    "allocate hyperslab count buffer");
  ...
  for (blkndx = 0; blkndx < (hsize_t)nblocks; blkndx++) {
    ...
    if (H5Dread(region_id, type_id, mem_space, sid1, H5P_DEFAULT, region_buf) <
        0)
      H5_LIBRARY_ERROR(ENVONLY);
    ...
    for (numindex = 0; numindex < numelem; numindex++) {
      if (!h5str_sprintf(ENVONLY, str, region_id, type_id,
                         ((char *)region_buf + numindex * type_size), 1))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);

      if (numindex + 1 < numelem)
        if (!h5str_append(str, ", "))
          H5_ASSERTION_ERROR(ENVONLY, "Unable to append string.");
    } /* end for (jndx = 0; jndx < numelem; jndx++, region_elmtno++,
         ctx.cur_elmt++) */
...
int h5str_dump_region_blocks(JNIEnv *env, h5str_t *str, hid_t region_space,
                             hid_t region_id, int expand_data) {
  hssize_t nblocks;
  hsize_t alloc_size;
  hsize_t *ptdata = NULL;
  ...
  alloc_size =
      (hsize_t)nblocks * (hsize_t)ndims * 2 * (hsize_t)sizeof(ptdata[0]);
  if (NULL == (ptdata = (hsize_t *)malloc((size_t)alloc_size)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "h5str_dump_region_blocks: failed to allocate region block buffer");

  if (H5Sget_select_hyper_blocklist(region_space, (hsize_t)0, (hsize_t)nblocks,
                                    ptdata) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (expand_data) {
    ...
    if (h5str_print_region_data_blocks(ENVONLY, region_id, str, ndims, type_id,
                                       nblocks, ptdata) < 0)
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  } else {
    ...
    for (i = 0; i < nblocks; i++) {
      ...
      for (j = 0; j < ndims; j++) {
        ...
        if (snprintf(tmp_str, sizeof(tmp_str), "%s%lu", j ? "," : "(",
          ...
          H5_JNI_FATAL_ERROR(ENVONLY,
                             "h5str_dump_region_blocks: snprintf failure");
 ...
 * Description:
 *      This is a special case subfunction to print the data in a region
 *reference of type points.
 *
 * Return:
 *      The function returns FAIL on error, otherwise SUCCEED
 *-------------------------------------------------------------------------
 */
static int h5str_print_region_data_points(JNIEnv *env, hid_t region_space,
                                          hid_t region_id, h5str_t *str,
                                          int ndims, hid_t type_id,
                                          hssize_t npoints, hsize_t *ptdata) {
  hsize_t *dims1 = NULL;
  hsize_t total_size[H5S_MAX_RANK];
  size_t jndx;
  size_t type_size;
  ...
  if (npoints < 0)
    H5_BAD_ARGUMENT_ERROR(ENVONLY,
                          "h5str_print_region_data_points: npoints < 0");

  /* Allocate space for the dimension array */
  if (NULL == (dims1 = (hsize_t *)malloc(sizeof(hsize_t) * (size_t)ndims)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "h5str_print_region_data_points: failed to "
                                    "allocate dimension array buffer");
  ...
  if (NULL == (region_buf = malloc(type_size * (size_t)npoints)))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "h5str_print_region_data_points: failed to allocate region buffer");

  if (H5Dread(region_id, type_id, mem_space, region_space, H5P_DEFAULT,
              region_buf) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  for (jndx = 0; jndx < (size_t)npoints; jndx++) {
    ...
    if (!h5str_sprintf(ENVONLY, str, region_id, type_id,
                       ((char *)region_buf + jndx * type_size), 1))
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
...
int h5str_dump_region_points(JNIEnv *env, h5str_t *str, hid_t region_space,
                             hid_t region_id, int expand_data) {
  hsize_t alloc_size;
  hssize_t npoints = -1;
  hsize_t *ptdata = NULL;
  ...
  if (NULL == (ptdata = (hsize_t *)malloc((size_t)alloc_size)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY, "h5str_dump_region_points: failed to "
                                    "allocate region point data buffer");

  if (H5Sget_select_elem_pointlist(region_space, (hsize_t)0, (hsize_t)npoints,
                                   ptdata) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if (expand_data) {
    ...
    if (h5str_print_region_data_points(ENVONLY, region_space, region_id, str,
                                       ndims, type_id, npoints, ptdata) < 0)
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
  } else {
    ...
    for (i = 0; i < npoints; i++) {
      ...
      for (j = 0; j < ndims; j++) {
        ...
        if (snprintf(tmp_str, sizeof(tmp_str), "%s%lu", j ? "," : "(",
          ...
          H5_JNI_FATAL_ERROR(ENVONLY,
                             "h5str_dump_region_points: snprintf failure");
 ...
 *-------------------------------------------------------------------------
 */
static hid_t h5str_get_little_endian_type(hid_t tid) {
  H5T_class_t type_class;
  H5T_sign_t sign;
  size_t size;
  hid_t p_type = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
static hid_t h5str_get_big_endian_type(hid_t tid) {
  H5T_class_t type_class;
  H5T_sign_t sign;
  size_t size;
  hid_t p_type = H5I_INVALID_HID;
...
static int h5str_render_bin_output(FILE *stream, hid_t container, hid_t tid,
  ...
  case H5T_COMPOUND: {
    ...
    for (block_index = 0; block_index < block_nelmts; block_index++) {
      ...
      for (j = 0; j < (unsigned)nmembs; j++) {
        ...
        if (h5str_render_bin_output(stream, container, memb, mem + offset, 1) <
            0) {
          H5Tclose(memb);
          ret_value = FAIL;
          break;
  ...
  case H5T_VLEN: {
    ...
    for (block_index = 0; block_index < block_nelmts; block_index++) {
      ...
      memcpy(&vl_elem, ((unsigned char *)_mem) + block_index * size,
             sizeof(hvl_t));
      ...
      /* dump the array element */
      if (h5str_render_bin_output(stream, container, memb,
                                  ((char *)(vl_elem.p)), nelmts) < 0) {
        ret_value = FAIL;
        break;
      }
    }
  ...
  case H5T_REFERENCE: {
    ...
    for (block_index = 0; block_index < block_nelmts; block_index++) {
      ...
      memcpy(&loc_ref, ((unsigned char *)_mem) + block_index * size,
             sizeof(H5R_ref_t));

      if ((region_id = H5Ropen_object(&loc_ref, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        continue;
      if ((region_space = H5Ropen_region(&loc_ref, H5P_DEFAULT, H5P_DEFAULT)) >=
          0) {
        if (!h5str_is_zero(&loc_ref, H5Tget_size(H5T_STD_REF))) {
...
static int render_bin_output_region_data_blocks(FILE *stream, hid_t region_id,
  ...
  for (blkndx = 0; blkndx < nblocks; blkndx++) {
    ...
    if (H5Sselect_hyperslab(sid1, H5S_SELECT_SET, start, NULL, count, NULL) <
        0) {
      ret_value = FAIL;
      goto done;
    }
    ...
    if (h5str_render_bin_output(stream, container, type_id, (char *)region_buf,
                                numelem) < 0) {
      ret_value = FAIL;
      goto done;
    }
 ...
 *
 * Description:
 *      This is a special case subfunction to dump a region reference using
 *blocks.
 *
 * Return:
...
static int render_bin_output_region_blocks(FILE *stream, hid_t region_space,
  ...
  if (nblocks > 0) {
    ...
    if (H5Sget_select_hyper_blocklist(region_space, (hsize_t)0,
                                      (hsize_t)nblocks, ptdata) < 0) {
      ret_value = FAIL;
      goto done;
    }
 ...
 *
 * Return:
 *      The function returns False if the last dimension has been reached,
 *otherwise True
 *
 *-------------------------------------------------------------------------
 */
static int render_bin_output_region_points(FILE *stream, hid_t region_space,
  ...
  if (npoints > 0) {
    ...
    if (render_bin_output_region_data_points(stream, region_space, region_id,
                                             container, ndims, type_id, npoints,
                                             ptdata) < 0) {
      ret_value = FAIL;
      goto done;
    }
  ...
  if (NULL != (buf = (unsigned char *)malloc((size_t)alloc_size))) {
    ...
    if (H5Aread(region_id, type_id, buf) >= 0) {
      ...
      for (i = 0; i < p_nelmts; i++) {
        ...
        if (!(bytes_in =
                  h5str_sprintf(ENVONLY, str, region_id, type_id, memref, 1)))
          CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
...
int h5str_dump_simple_dset(JNIEnv *env, FILE *stream, hid_t dset,
                           int binary_order) {
  ...
  if (H5Tequal(f_type, H5T_STD_REF_DSETREG)) {
    ...
    if (NULL ==
        (sm_buf = (unsigned char *)calloc(
             MAX(sizeof(unsigned), sizeof(H5R_ref_t)), (size_t)p_nelmts)))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "h5str_dump_simple_mem: failed to allocate sm_buf");
    ...
    if (binary_order == 99) {
      if (h5str_dump_simple_data(ENVONLY, stream, dset, H5T_STD_REF, sm_buf,
                                 (size_t)p_nelmts) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    } else {
      if (h5str_render_bin_output(stream, dset, H5T_STD_REF, sm_buf,
                                  (size_t)p_nelmts) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    }
  } else {
    ...
    if (p_nelmts > 0) {
      ...
      if (sndims > 0) {
        for (i = (size_t)sndims; i > 0; --i) {
          ...
          if (size == 0) /* datum size > H5TOOLS_BUFSIZE */
            size = 1;
          sm_size[i - 1] =
              (((total_size[i - 1]) < (size)) ? (total_size[i - 1]) : (size));
      ...
      if (sm_nbytes > 0) {
        if (NULL == (sm_buf = (unsigned char *)malloc((size_t)sm_nbytes)))
          H5_OUT_OF_MEMORY_ERROR(
              ENVONLY, "h5str_dump_simple_dset: failed to allocate sm_buf");
        ...
        for (elmtno = 0; elmtno < (hsize_t)p_nelmts; elmtno += hs_nelmts) {
          ...
          if (sndims > 0) {
            ...
            if (H5Sselect_hyperslab(f_space, H5S_SELECT_SET, hs_offset, NULL,
                                    hs_size, NULL) < 0)
              H5_LIBRARY_ERROR(ENVONLY);

            if (H5Sselect_hyperslab(sm_space, H5S_SELECT_SET, zero, NULL,
                                    &hs_nelmts, NULL) < 0)
              H5_LIBRARY_ERROR(ENVONLY);
          } else {
          ...
          if (binary_order == 99) {
            if (h5str_dump_simple_data(ENVONLY, stream, dset, p_type, sm_buf,
                                       hs_nelmts) < 0)
              CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
          } else {
            if (h5str_render_bin_output(stream, dset, p_type, sm_buf,
                                        hs_nelmts) < 0)
              CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
          }
...
int h5str_dump_simple_mem(JNIEnv *env, FILE *stream, hid_t attr_id,
                          int binary_order) {
  hid_t f_space = H5I_INVALID_HID; /* file data space */
  hsize_t alloc_size;
  int sndims;                       /* rank of dataspace */
  ...
  if (H5Tequal(f_type, H5T_STD_REF_DSETREG)) {
    ...
    if (binary_order == 99) {
      if (h5str_dump_simple_data(ENVONLY, stream, attr_id, H5T_STD_REF, sm_buf,
                                 (size_t)p_nelmts) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    } else {
      if (h5str_render_bin_output(stream, attr_id, H5T_STD_REF, sm_buf,
                                  (size_t)p_nelmts) < 0)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    }
  } else {
    ...
    if (p_nelmts > 0) {
      ...
      if (binary_order == 99) {
        if (h5str_dump_simple_data(ENVONLY, stream, attr_id, p_type, sm_buf,
                                   (size_t)p_nelmts) < 0)
          CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      } else {
        if (h5str_render_bin_output(stream, attr_id, p_type, sm_buf,
                                    (size_t)p_nelmts) < 0)
          CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      }
...
static int h5str_dump_simple_data(JNIEnv *env, FILE *stream, hid_t container,
  ...
  for (i = 0, line_count = 0; i < nelmts; i++, line_count++) {
    ...
    if (!buffer.s)
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "h5str_dump_simple_data: failed to allocate buffer");

    if (!(bytes_in =
              h5str_sprintf(ENVONLY, &buffer, container, type, memref, 1)))
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    ...
    if ((i > 0) && (bytes_in > 0)) {
      ...
      if (line_count >= H5TOOLS_TEXT_BLOCK) {
        ...
        if (fprintf(stream, "\n") < 0)
          H5_JNI_FATAL_ERROR(ENVONLY,
                             "h5str_dump_simple_data: fprintf failure");
      }
    }
 ...
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5AreadComplex(JNIEnv *env,
                                                          jclass clss,
                                                          jlong attr_id,
                                                          jlong mem_type_id,
                                                          jobjectArray buf) {
  ...
  size = (((H5Tget_size(mem_type_id)) > (H5Tget_size(p_type)))
  ...
  if (NULL == (readBuf = (char *)malloc((size_t)n * size)))
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5AreadComplex: failed to allocate read buffer");
  ...
  if (!h5str.s)
    H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                           "H5AreadComplex: failed to allocate string buffer");

  for (i = 0; i < (size_t)n; i++) {
    h5str.s[0] = '\0';

    if (!h5str_sprintf(ENVONLY, &h5str, attr_id, mem_type_id,
                       readBuf + (i * size), 0))
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Acopy(JNIEnv *env, jclass clss,
                                                   jlong src_id, jlong dst_id) {
  hssize_t npoints;
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Dcopy(JNIEnv *env, jclass clss,
                                                   jlong src_id, jlong dst_id) {
  hssize_t npoints;
  ...
  if ((retVal =
           H5Dread((hid_t)src_id, tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
  ...
  if ((retVal = H5Dwrite((hid_t)dst_id, tid, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                         buf)) < 0)
    H5_LIBRARY_ERROR(ENVONLY);
...
#ifdef __cplusplus
herr_t obj_info_all(hid_t g_id, const char *name, const H5L_info2_t *linfo,
                    void *op_data);
herr_t obj_info_max(hid_t g_id, const char *name, const H5L_info2_t *linfo,
                    void *op_data);
int H5Gget_obj_info_max(hid_t, char **, int *, int *, H5O_token_t *, long);
int H5Gget_obj_info_full(hid_t loc_id, char **objname, int *otype, int *ltype,
                         unsigned long *fno, H5O_token_t *obj_token,
                         int indexType, int indexOrder);
#else
static herr_t obj_info_all(hid_t g_id, const char *name,
                           const H5L_info2_t *linfo, void *op_data);
static herr_t obj_info_max(hid_t g_id, const char *name,
                           const H5L_info2_t *linfo, void *op_data);
static int H5Gget_obj_info_max(hid_t, char **, int *, int *, H5O_token_t *,
                               long);
static int H5Gget_obj_info_full(hid_t loc_id, char **objname, int *otype,
                                int *ltype, unsigned long *fno,
                                H5O_token_t *obj_token, int indexType,
                                int indexOrder);
#endif
 ...
 * for 'n' and if it exceeds the number of links in the group, we will most
 * likely end up overwriting memory heap-tracking info.
 */
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Gget_1obj_1info_1full(
  ...
  PIN_INT_ARRAY(ENVONLY, oType, otarr, &isCopy,
                "H5Gget_obj_info_full: oType not pinned");
  PIN_INT_ARRAY(ENVONLY, lType, ltarr, &isCopy,
                "H5Gget_obj_info_full: lType not pinned");
  PIN_LONG_ARRAY(ENVONLY, fNo, fnoP, &isCopy,
                 "H5Gget_obj_info_full: fNo not pinned");

  if (NULL == (oName = (char **)calloc((size_t)n, sizeof(*oName))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "H5Gget_obj_info_full: failed to allocate buffer for object name");

  if (NULL == (tokens = (H5O_token_t *)calloc((size_t)n, sizeof(H5O_token_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "H5Gget_obj_info_full: failed to allocate buffer for object tokens");

  if (NULL ==
      (fnos = (unsigned long *)calloc((size_t)n, sizeof(unsigned long))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "H5Gget_obj_info_full: failed to allocate buffer for file number info");

  if (group_name) {
    PIN_JAVA_STRING(ENVONLY, group_name, gName, &isCopy,
                    "H5Gget_obj_info_full: group_name not pinned");

    if ((gid = H5Gopen2((hid_t)loc_id, gName, H5P_DEFAULT)) < 0)
      H5_LIBRARY_ERROR(ENVONLY);
  }

  if ((ret_val = H5Gget_obj_info_full(gid, oName, (int *)otarr, (int *)ltarr,
                                      fnos, tokens, indexType, indexOrder)) < 0)
    H5_JNI_FATAL_ERROR(ENVONLY,
...
JNIEXPORT jint JNICALL Java_hdf_hdf5lib_H5_H5Gget_1obj_1info_1max(
    ...
    jintArray oType, jintArray lType, jobjectArray oToken, jlong maxnum,
    jint n) {
  H5O_token_t *tokens = NULL;
  jboolean isCopy;
  jstring str;
  ...
  PIN_INT_ARRAY(ENVONLY, oType, otarr, &isCopy,
                "H5Gget_obj_info_max: oType not pinned");
  PIN_INT_ARRAY(ENVONLY, lType, ltarr, &isCopy,
                "H5Gget_obj_info_max: lType not pinned");

  if (NULL == (oName = (char **)calloc((size_t)n, sizeof(*oName))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "H5Gget_obj_info_max: failed to allocate buffer for object name");

  if (NULL == (tokens = (H5O_token_t *)calloc((size_t)n, sizeof(H5O_token_t))))
    H5_OUT_OF_MEMORY_ERROR(
        ENVONLY,
        "H5Gget_obj_info_max: failed to allocate buffer for object tokens");

  if ((ret_val = H5Gget_obj_info_max((hid_t)loc_id, oName, (int *)otarr,
                                     (int *)ltarr, tokens, maxnum)) < 0)
    H5_JNI_FATAL_ERROR(ENVONLY,
...
int H5Gget_obj_info_full(hid_t loc_id, char **objname, int *otype, int *ltype,
  ...
  if (H5Literate2(loc_id, (H5_index_t)indexType, (H5_iter_order_t)indexOrder,
                  NULL, obj_info_all, (void *)&info) < 0) {
    /*
    ...
    if (H5Literate2(loc_id, H5_INDEX_NAME, H5_ITER_INC, NULL, obj_info_all,
                    (void *)&info) < 0)
      return -1;
  }
...
int H5Gget_obj_info_max(hid_t loc_id, char **objname, int *otype, int *ltype,
                        H5O_token_t *obj_token, long maxnum) {
  info_all_t info;
  ...
  if (H5Lvisit2(loc_id, H5_INDEX_NAME, H5_ITER_NATIVE, obj_info_max,
                (void *)&info) < 0)
    return -1;
...
herr_t obj_info_all(hid_t loc_id, const char *name, const H5L_info2_t *info,
                    void *op_data) {
  ...
  if (object_exists) {
    if (H5Oget_info_by_name3(loc_id, name, &object_info, H5O_INFO_ALL,
                             H5P_DEFAULT) < 0)
      goto done;
    ...
    memcpy(&datainfo->obj_token[datainfo->count], &object_info.token,
           sizeof(object_info.token));
  }
...
herr_t obj_info_max(hid_t loc_id, const char *name, const H5L_info2_t *info,
                    void *op_data) {
  info_all_t *datainfo = (info_all_t *)op_data;
  H5O_info2_t object_info;
...
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5export_1dataset(
    JNIEnv *env, jclass clss, jstring file_export_name, jlong file_id,
  ...
  if (NULL == file_export_name)
    H5_NULL_ARGUMENT_ERROR(ENVONLY,
                           "H5export_dataset: file_export_name is NULL");

  if (NULL == object_path)
    H5_NULL_ARGUMENT_ERROR(ENVONLY, "H5export_dataset: object_path is NULL");

  PIN_JAVA_STRING(ENVONLY, object_path, object_name, &isCopy,
  ...
  if ((ret_val = h5str_dump_simple_dset(ENVONLY, stream, dataset_id,
                                        binary_order)) < 0)
 ...
 * Signature: (Ljava/lang/String;JLjava/lang/String;I)V
 */
JNIEXPORT void JNICALL Java_hdf_hdf5lib_H5_H5export_1attribute(
    JNIEnv *env, jclass clss, jstring file_export_name, jlong dset_id,
  ...
  if ((ret_val =
           h5str_dump_simple_mem(ENVONLY, stream, attr_id, binary_order)) < 0)
    H5_ASSERTION_ERROR(ENVONLY, "h5str_dump_simple_dset failed");
...
jobject translate_atomic_rbuf(JNIEnv *env, jlong mem_type_id,
  ...
  jmethodID arrListMethod =
      ENVPTR->GetMethodID(ENVONLY, arrCList, "<init>", "(I)V");
  ...
  /*jobjectArray
   ...
   ENVPTR->GetStaticMethodID(ENVONLY, cBool, "valueOf",
   "(Z)Ljava/lang/Boolean;");
   */
  jmethodID byteValueMid = ENVPTR->GetStaticMethodID(ENVONLY, cByte, "valueOf",
                                                     "(B)Ljava/lang/Byte;");
  jmethodID shortValueMid = ENVPTR->GetStaticMethodID(
      ENVONLY, cShort, "valueOf", "(S)Ljava/lang/Short;");
  jmethodID intValueMid = ENVPTR->GetStaticMethodID(ENVONLY, cInt, "valueOf",
                                                    "(I)Ljava/lang/Integer;");
  jmethodID longValueMid = ENVPTR->GetStaticMethodID(ENVONLY, cLong, "valueOf",
                                                     "(J)Ljava/lang/Long;");
  jmethodID floatValueMid = ENVPTR->GetStaticMethodID(
      ENVONLY, cFloat, "valueOf", "(F)Ljava/lang/Float;");
  jmethodID doubleValueMid = ENVPTR->GetStaticMethodID(
      ENVONLY, cDouble, "valueOf", "(D)Ljava/lang/Double;");
  ...
  case H5T_VLEN: {
    ...
    if (vl_elem.len != (size_t)nelmts)
      H5_JNI_FATAL_ERROR(
          ENVONLY, "translate_atomic_rbuf: overflow of number of VL elements");

    if (nelmts < 0)
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "translate_atomic_rbuf: number of VL elements < 0");

    /* The list we're going to return: */
    if (NULL == (jList = (jobjectArray)ENVPTR->NewObject(ENVONLY, arrCList,
                                                         arrListMethod, 0)))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY,
  ...
  case H5T_ARRAY: {
    ...
    if (NULL == (objBuf = malloc(typeSize)))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "translate_atomic_rbuf: failed to allocate buffer");
  ...
  case H5T_INTEGER: {
    ...
    case sizeof(jbyte): {
      ...
      if (NULL == (jobj = ENVPTR->CallStaticObjectMethod(
                       ENVONLY, cByte, byteValueMid, byteValue)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      break;
    }
    case sizeof(jshort): {
      ...
      if (NULL == (jobj = ENVPTR->CallStaticObjectMethod(
                       ENVONLY, cShort, shortValueMid, shortValue)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      break;
    }
    case sizeof(jint): {
      ...
      if (NULL == (jobj = ENVPTR->CallStaticObjectMethod(
                       ENVONLY, cInt, intValueMid, intValue)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      break;
    }
    case sizeof(jlong): {
      ...
      if (NULL == (jobj = ENVPTR->CallStaticObjectMethod(
                       ENVONLY, cLong, longValueMid, longValue)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      break;
    }
    }
  ...
  case H5T_FLOAT: {
    ...
    case sizeof(jfloat): {
      ...
      if (NULL == (jobj = ENVPTR->CallStaticObjectMethod(
                       ENVONLY, cFloat, floatValueMid, (double)floatValue)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      break;
    }
    case sizeof(jdouble): {
      ...
      if (NULL == (jobj = ENVPTR->CallStaticObjectMethod(
                       ENVONLY, cDouble, doubleValueMid, doubleValue)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      break;
    }
    }
  ...
  case H5T_REFERENCE: {
    ...
    if (typeSize != (size_t)byteArraySize)
      H5_JNI_FATAL_ERROR(ENVONLY,
                         "translate_atomic_rbuf: overflow of byteArraySize");
  ...
  case H5T_STRING: {
    ...
    if (is_variable) {
      ...
      if (NULL == (jobj = ENVPTR->NewStringUTF(ENVONLY, *var_str_buf))) {
        ...
        H5_OUT_OF_MEMORY_ERROR(
            ENVONLY, "translate_atomic_rbuf: out of memory - unable to "
...
void translate_atomic_wbuf(JNIEnv *env, jobject in_obj, jlong mem_type_id,
  ...
  jmethodID mToArray = ENVPTR->GetMethodID(ENVONLY, arrCList, "toArray",
                                           "()[Ljava/lang/Object;");
  ...
  jmethodID byteValueMid =
      ENVPTR->GetMethodID(ENVONLY, cByte, "byteValue", "()B");
  jmethodID shortValueMid =
      ENVPTR->GetMethodID(ENVONLY, cShort, "shortValue", "()S");
  jmethodID intValueMid = ENVPTR->GetMethodID(ENVONLY, cInt, "intValue", "()I");
  jmethodID longValueMid =
      ENVPTR->GetMethodID(ENVONLY, cLong, "longValue", "()J");
  jmethodID floatValueMid =
      ENVPTR->GetMethodID(ENVONLY, cFloat, "floatValue", "()F");
  jmethodID doubleValueMid =
      ENVPTR->GetMethodID(ENVONLY, cDouble, "doubleValue", "()D");
  ...
  case H5T_VLEN: {
    ...
    if (jnelmts < 0)
      H5_BAD_ARGUMENT_ERROR(ENVONLY,
                            "translate_atomic_wbuf: number of VL elements < 0");

    vl_elem.len = (size_t)jnelmts;

    if (NULL == (vl_elem.p = malloc((size_t)jnelmts * vlSize)))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "translate_atomic_wbuf: failed to allocate vlen ptr buffer");

    translate_wbuf(ENVONLY, (jobjectArray)in_obj, memb, vlClass, (jsize)jnelmts,
                   vl_elem.p);
  ...
  case H5T_COMPOUND: {
    ...
    if (mToArray == NULL)
      CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
    jobjectArray array =
        (jobjectArray)ENVPTR->CallObjectMethod(ENVONLY, in_obj, mToArray);
    jsize jnelmts = ENVPTR->GetArrayLength(ENVONLY, array);

    if (jnelmts != nmembs)
      H5_BAD_ARGUMENT_ERROR(ENVONLY, "translate_atomic_wbuf: number of "
                                     "elements not equal to number of members");
  ...
  case H5T_ARRAY: {
    ...
    if (jnelmts < 0)
      H5_BAD_ARGUMENT_ERROR(
          ENVONLY, "translate_atomic_wbuf: number of array elements < 0");

    if (NULL == (objBuf = malloc((size_t)jnelmts * vlSize)))
      H5_OUT_OF_MEMORY_ERROR(
          ENVONLY, "translate_atomic_wbuf: failed to allocate buffer");
  ...
  case H5T_INTEGER: {
    ...
    case sizeof(jshort): {
      jshort shortValue =
          ENVPTR->CallShortMethod(ENVONLY, in_obj, shortValueMid);
  ...
  case H5T_FLOAT: {
    ...
    case sizeof(jfloat): {
      jfloat floatValue =
          ENVPTR->CallFloatMethod(ENVONLY, in_obj, floatValueMid);
      memcpy(char_buf, ((char *)&floatValue), typeSize);
      break;
    }
    case sizeof(jdouble): {
      jdouble doubleValue =
          ENVPTR->CallDoubleMethod(ENVONLY, in_obj, doubleValueMid);
  ...
  case H5T_STRING: {
    ...
    if (NULL != in_obj) {
      PIN_JAVA_STRING(ENVONLY, in_obj, utf8, NULL,
                      "translate_atomic_wbuf jobj not pinned");
...
void translate_rbuf(JNIEnv *env, jobjectArray ret_buf, jlong mem_type_id,
                    H5T_class_t type_class, jsize count, void *raw_buf) {
  hid_t memb = H5I_INVALID_HID;
  int ret_buflen = -1;
  ...
  jmethodID arrListMethod =
      ENVPTR->GetMethodID(ENVONLY, arrCList, "<init>", "(I)V");
  jmethodID arrAddMethod =
      ENVPTR->GetMethodID(ENVONLY, arrCList, "add", "(Ljava/lang/Object;)Z");
  ...
  case H5T_VLEN: {
    ...
    for (i = 0; i < (size_t)count; i++) {
      ...
      if (vl_elem.len != (size_t)nelmts)
        H5_JNI_FATAL_ERROR(ENVONLY,
                           "translate_rbuf: overflow of number of VL elements");

      if (nelmts < 0)
        H5_BAD_ARGUMENT_ERROR(ENVONLY,
                              "translate_rbuf: number of VL elements < 0");

      /* The list we're going to return: */
      if (i < (size_t)ret_buflen) {
        jList = ENVPTR->GetObjectArrayElement(ENVONLY, (jobjectArray)ret_buf,
                                              (jsize)i);
      }
      if (NULL == jList) {
        ...
        if (NULL == (jList = (jobjectArray)ENVPTR->NewObject(ENVONLY, arrCList,
                                                             arrListMethod, 0)))
          H5_OUT_OF_MEMORY_ERROR(
              ENVONLY, "translate_rbuf: failed to allocate list read buffer");
      }

      translate_rbuf(ENVONLY, jList, memb, vlClass, (jsize)nelmts, vl_elem.p);
      if (found_jList == JNI_FALSE) {
        jboolean addResult = ENVPTR->CallBooleanMethod(
            ENVONLY, ret_buf, arrAddMethod, (jobject)jList);
        if (!addResult)
          H5_JNI_FATAL_ERROR(ENVONLY, "translate_rbuf: cannot add VL element");
      } else {
        ENVPTR->SetObjectArrayElement(ENVONLY, ret_buf, (jsize)i,
                                      (jobject)jList);
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
      }
  ...
  case H5T_COMPOUND: {
    ...
    for (i = 0; i < (size_t)count; i++) {
      ...
      for (x = 0; x < (size_t)nmembs; x++) {
        ...
        if (jobj) {
          if (found_jList == JNI_FALSE)
            ENVPTR->CallBooleanMethod(ENVONLY, jList, arrAddMethod,
                                      (jobject)jobj);
          else
            ENVPTR->SetObjectArrayElement(ENVONLY, jList, (jsize)i,
                                          (jobject)jobj);
          CHECK_JNI_EXCEPTION(ENVONLY, JNI_TRUE);
  ...
  case H5T_ARRAY: {
    ...
    if (NULL == (objBuf = malloc(typeSize)))
      H5_OUT_OF_MEMORY_ERROR(ENVONLY,
                             "translate_rbuf: failed to allocate buffer");
    ...
    for (i = 0; i < (size_t)count; i++) {
      ...
      /* The list we're going to return: */
      if (i < (size_t)ret_buflen) {
        if (NULL == (jList = ENVPTR->GetObjectArrayElement(
                         ENVONLY, (jobjectArray)ret_buf, (jsize)i)))
          found_jList = JNI_FALSE;
      }
      if (NULL == jList) {
        if (NULL == (jList = (jobjectArray)ENVPTR->NewObject(ENVONLY, arrCList,
  ...
  case H5T_STRING: {
    /* Convert each element to a list */
    for (i = 0; i < (size_t)count; i++) {
      jobj = translate_atomic_rbuf(ENVONLY, mem_type_id, type_class,
                                   char_buf + i * typeSize);
      if (jobj) {
        if (ret_buflen == 0)
          ENVPTR->CallBooleanMethod(ENVONLY, ret_buf, arrAddMethod,
                                    (jobject)jobj);
        else
          ENVPTR->SetObjectArrayElement(ENVONLY, ret_buf, (jsize)i,
                                        (jobject)jobj);
...
void translate_wbuf(JNIEnv *env, jobjectArray in_buf, jlong mem_type_id,
                    H5T_class_t type_class, jsize count, void *raw_buf) {
  hid_t memb = H5I_INVALID_HID;
  jobjectArray jList = NULL;
  jobject jobj = NULL;
  ...
  case H5T_VLEN: {
    ...
    for (i = 0; i < (size_t)count; i++) {
      ...
      if (NULL == (jList = ENVPTR->GetObjectArrayElement(
                       ENVONLY, (jobjectArray)in_buf, (jsize)i)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      ...
      if (mToArray == NULL)
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
      jobjectArray array =
          (jobjectArray)ENVPTR->CallObjectMethod(ENVONLY, jList, mToArray);
      jsize jnelmts = ENVPTR->GetArrayLength(ENVONLY, array);

      if (jnelmts < 0)
        H5_BAD_ARGUMENT_ERROR(ENVONLY,
                              "translate_wbuf: number of VL elements < 0");

      vl_elem.len = (size_t)jnelmts;

      if (NULL == (vl_elem.p = malloc((size_t)jnelmts * vlSize)))
        H5_OUT_OF_MEMORY_ERROR(
            ENVONLY, "translate_wbuf: failed to allocate vlen ptr buffer");
  ...
  case H5T_COMPOUND: {
    ...
    for (i = 0; i < (size_t)count; i++) {
      ...
      if (jnelmts != nmembs)
        H5_BAD_ARGUMENT_ERROR(ENVONLY, "translate_wbuf: number of elements not "
                                       "equal to number of members");
      ...
      for (x = 0; x < (size_t)nmembs; x++) {
        ...
        jobject arr_obj =
            ENVPTR->GetObjectArrayElement(ENVONLY, array, (jsize)x);
        translate_atomic_wbuf(ENVONLY, arr_obj, memb, memb_vlClass,
                              char_buf + i * typeSize + memb_offset);
        ENVPTR->DeleteLocalRef(ENVONLY, arr_obj);
        H5Tclose(memb);
  ...
  case H5T_ARRAY: {
    ...
    for (i = 0; i < (size_t)count; i++) {
      ...
      if (jnelmts < 0)
        H5_BAD_ARGUMENT_ERROR(ENVONLY,
                              "translate_wbuf: number of array elements < 0");
  ...
  case H5T_STRING: {
    /* Convert each list to an array element */
    for (i = 0; i < (size_t)count; i++) {
      if (NULL == (jobj = ENVPTR->GetObjectArrayElement(
                       ENVONLY, (jobjectArray)in_buf, (jsize)i)))
        CHECK_JNI_EXCEPTION(ENVONLY, JNI_FALSE);
