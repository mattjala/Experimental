 ...
 * HDF5 entry point, generally with the arguments and return codes analogous to
 the C interface.
 * <p>
 ...
 * rule is that
 * all HDF functions that return SUCCEED/FAIL are declared <i>boolean</i> in the
 Java version, rather than
 * <i>int</i> as
 * in the C. Functions that return a value or else FAIL are declared the
 equivalent to the C function.
 * However, in most
 * cases the Java method will raise an exception instead of returning an error
 code.
 * @see @ref ERRORS.
 * <p>
 * Java does not support pass by reference of arguments, so arguments that are
 returned through <b>OUT</b>
 * parameters
 * must be wrapped in an object or array. The Java API for HDF consistently
 ...
 * All the routines where this convention is used will have specific
 documentation of the details, given
 * below.
 * <p>
 * <b>Arrays</b>
 * <p>
 * HDF5 needs to read and write multi-dimensional arrays of any number type (and
 records). The HDF5 API
 * describes the
 * layout of the source and destination, and the data for the array passed as a
 ...
 * where ``void *'' means that the data may be any valid numeric type, and is a
 contiguous block of bytes that
 * is the
 * data for a multi-dimensional array. The other parameters describe the
 dimensions, rank, and datatype of the
 * array on
 ...
 * multidimensional
 * arrays are definitely <i>not</i> laid out contiguously in memory. It would be
 infeasible to declare a
 * separate
 * routine for every combination of number type and dimensionality. For that
 reason, the
 *  <a href="./HDFArray.html"><b>HDFArray</b></a> class is used to discover the
 type, shape, and
 * size of the
 * data array at run time, and to convert to and from a contiguous array of
 bytes in synchronized static
 * native C order.
 * <p>
 * The upshot is that any Java array of numbers (either primitive or sub-classes
 of type <b>Number</b>) can be
 * passed as
 * an ``Object'', and the Java API will translate to and from the appropriate
 packed array of bytes needed by
 * the C
 * library. So the function above would be declared:
 *
 * <pre>
 * public synchronized static native int H5Dread(long fid, long filetype, long
 memtype, long memspace, Object
 * data);
 * </pre>
 ...
 * The HDF5 API defines a set of constants and enumerated values. Most of these
 values are available to Java
 * programs
 * via the class <a href="./HDF5Constants.html"> <b>HDF5Constants</b></a>. For
 example, the
 * parameters for
 * the h5open() call include two numeric values,
 <b><i>HDFConstants.H5F_ACC_RDWR</i></b> and
 * <b><i>HDF5Constants.H5P_DEFAULT</i></b>. As would be expected, these numbers
 correspond to the C constants
 * <b><i>H5F_ACC_RDWR</i></b> and <b><i>H5P_DEFAULT</i></b>.
 * <p>
 * The HDF5 API defines a set of values that describe number types and sizes,
 such as "H5T_NATIVE_INT" and
 * "hsize_t".
 * These values are determined at run time by the HDF5 C library. To support
 these parameters, the Java class
 * <a href="./HDF5CDataTypes.html"> <b>HDF5CDataTypes</b></a> looks up the
 ...
 * <b><i>HDF5CDataTypes</i></b> may have different values on different
 platforms.
 * <p>
 * <b>Error handling and Exceptions</b>
 * <p>
 * The HDF5 error API (H5E) manages the behavior of the error stack in the HDF5
 library. This API is
 * available from the
 * JHI5. Errors are converted into Java exceptions. This is totally different
 from the C interface, but is
 * very natural
 * for Java programming.
 * <p>
 * The exceptions of the JHI5 are organized as sub-classes of the class
 * <a href="./HDF5Exception.html"> <b>HDF5Exception</b></a>. There are two
 subclasses
 * of
 ...
 * while sub-classes of the latter represent errors in the JHI5 wrapper and
 support code.
 * <p>
 * The super-class <b><i>HDF5LibraryException</i></b> implements the method
 * '<b><i>printStackTrace()</i></b>', which prints out the HDF5 error stack, as
 described
