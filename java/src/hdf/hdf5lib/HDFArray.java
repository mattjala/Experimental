 ...
 * containing scientific data. <p> The methods support the conversion of an
 * array to and from Java to a one-dimensional array of bytes suitable for I/O
 * by the C library. <p> This class heavily uses the
 * @ref HDFNATIVE
...
public class HDFArray {
   ...
   * numbers or sub-classes of Number. <p> The input is analysed to determine
   * the number of dimensions and size of each dimension, as well as the type of
   * the elements. <p> The description is saved in private variables, and used
   * to convert data.
   *
   * @param anArray
   *                The array object.
   ...
   *     sufficient to hold the data of the
  ...
  public byte[] byteify() throws HDF5JavaException {
    ...
    if (_theArray == null) {
      /* exception: not an array */
      HDF5JavaException ex =
          new HDF5JavaException("HDFArray: byteify not an array?: ");
      throw(ex);
    }
      ...
      if (ArrayDescriptor.NT == 'B') {
        ...
        try {
          ...
          if (ArrayDescriptor.NT == 'I') {
            therow = HDFNativeData.intToByte(0, ArrayDescriptor.dimlen[1],
                                             (int[])_theArray);
          } else if (ArrayDescriptor.NT == 'S') {
            therow = HDFNativeData.shortToByte(0, ArrayDescriptor.dimlen[1],
                                               (short[])_theArray);
          } else if (ArrayDescriptor.NT == 'F') {
            therow = HDFNativeData.floatToByte(0, ArrayDescriptor.dimlen[1],
                                               (float[])_theArray);
          } else if (ArrayDescriptor.NT == 'J') {
            therow = HDFNativeData.longToByte(0, ArrayDescriptor.dimlen[1],
                                              (long[])_theArray);
          } else if (ArrayDescriptor.NT == 'D') {
            therow = HDFNativeData.doubleToByte(0, ArrayDescriptor.dimlen[1],
                                                (double[])_theArray);
    ...
    while (n < ArrayDescriptor.totalSize) {
      ...
      try {
        if (ArrayDescriptor.NT == 'J') {
          arow = HDFNativeData.longToByte(
              0, ArrayDescriptor.dimlen[ArrayDescriptor.dims],
              (long[])ArrayDescriptor.objs[ArrayDescriptor.dims - 1]);
        } else if (ArrayDescriptor.NT == 'I') {
          arow = HDFNativeData.intToByte(
              0, ArrayDescriptor.dimlen[ArrayDescriptor.dims],
              (int[])ArrayDescriptor.objs[ArrayDescriptor.dims - 1]);
        } else if (ArrayDescriptor.NT == 'S') {
          arow = HDFNativeData.shortToByte(
              0, ArrayDescriptor.dimlen[ArrayDescriptor.dims],
          ...
          /* 32 bit float */
          arow = HDFNativeData.floatToByte(
              0, ArrayDescriptor.dimlen[ArrayDescriptor.dims],
              (float[])ArrayDescriptor.objs[ArrayDescriptor.dims - 1]);
        } else if (ArrayDescriptor.NT == 'D') {
          /* 64 bit float */
          arow = HDFNativeData.doubleToByte(
              0, ArrayDescriptor.dimlen[ArrayDescriptor.dims],
              (double[])ArrayDescriptor.objs[ArrayDescriptor.dims - 1]);
        } else if (ArrayDescriptor.NT == 'L') {
          if (ArrayDescriptor.className.equals("java.lang.Byte")) {
            arow = ByteObjToByte(
                (Byte[])ArrayDescriptor.objs[ArrayDescriptor.dims - 1]);
          } else if (ArrayDescriptor.className.equals("java.lang.Integer")) {
            arow = IntegerToByte(
                (Integer[])ArrayDescriptor.objs[ArrayDescriptor.dims - 1]);
          } else if (ArrayDescriptor.className.equals("java.lang.Short")) {
            arow = ShortToByte(
                (Short[])ArrayDescriptor.objs[ArrayDescriptor.dims - 1]);
          } else if (ArrayDescriptor.className.equals("java.lang.Float")) {
  ...
  /**
   * Given a one-dimensional array of bytes representing numbers, convert it to
   * a java array of the shape and size passed to the constructor.
   *
   * @param bytes
   ...
   * objects.
   * @exception
   *              hdf.hdf5lib.exceptions.HDF5JavaException the object not an
   * array or other internal error.
   */
  public Object arrayify(byte[] bytes) throws HDF5JavaException {
    ...
    default:
      HDF5JavaException ex = new HDF5JavaException(
          "HDFArray: unknown or unsupported type: " + ArrayDescriptor.NT);
      throw(ex);
    ...
    for (i = 0; i <= ArrayDescriptor.dims - 2; i++) {
      if (ArrayDescriptor.currentindex[i] != ArrayDescriptor.dimlen[i] - 1) {
        throw new java.lang.InternalError(new String(
            ...
            i + "] = " + ArrayDescriptor.currentindex[i] + " (should be " +
            (ArrayDescriptor.dimlen[i] - 1) + "?"));
      }
    }
 ...
 * arbitrary array. <p> We use java.lang.reflection here.
 */
class ArrayDescriptor {
  ...
  public ArrayDescriptor(Object anArray) throws HDF5JavaException {
    ...
    if (NT == 'B') {
      ...
      throw new HDF5JavaException(
          new String("ArrayDesciptor: Error:  array is not numeric (type is " +
                     css + ") ?"));
    }
  ...
  public void dumpInfo() {
    ...
    for (i = 0; i <= dims; i++) {
      ...
      String ss = tc.toString();
      System.out.println(i + ":  start " + dimstart[i] + ": len " + dimlen[i] +
                         " current " + currentindex[i] + " bytetoindex " +
                         bytetoindex[i] + " object " + objs[i] + " otype " +
                         ss);
    }
  }
}
