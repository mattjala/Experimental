 ...
 * the arguments and return codes analogous to the C interface. <p> For details
 * of the HDF5 library, @see @ref RM <hr> <p> <b>Mapping of arguments for
 * Java</b>
 *
 * <p>
 * In general, arguments to the HDF Java API are straightforward translations
 * from the 'C' API described in the @ref RM.
 *
 * <table border=1>
 ...
 * as above. The exception is for arrays, which are discussed below. <p> The
 * <i>return value</i> of Java methods is also the analogous type, as above. A
 * major exception to that rule is that all HDF Java functions will raise an
 * exception upon failure in the Java version, rather than just return
 * <i>int</i> as in the C. Functions that return a value are declared equivalent
 * to the C function. However, in most cases the Java method will raise an
 * exception instead of returning an error code.
 * @see @ref ERRORS.
 * <p>
 ...
 * The Java API for HDF consistently wraps arguments in arrays. Where possible
 * the Java function may return the OUT parameter as an object or basic type.
 * <p>
 ...
 * documentation of the details, given below. <p> <b>@ref HDFARRAY</b> <p> HDF5
 * needs to read and write multi-dimensional arrays of any number type (and
 ...
 * other parameters describe the dimensions, rank, and datatype of the array
 * ondisk (source) and in memory (destination). <p> For Java, this ``ANY'' is a
 * problem, as the type of data must always be declared. Furthermore,
 * multidimensional arrays are definitely <i>not</i> laid out contiguously in
 * memory. It would be infeasible to declare a separate routine for every
 * combination of number type and dimensionality. For that reason, the
 * @ref HDFARRAY <b>HDFArray</b> class is used to discover the type, shape, and
 * size of the data array at run time, and to convert to and from a contiguous
 * array of bytes in synchronized static native C order. <p> The upshot is that
 * any Java array of numbers (either primitive or sub-classes of type
 * <b>Number</b>) can be passed as an ``Object'', and the Java API will
 ...
 * such as float[][], or int[][][], or Double[][]. <p> <b>@ref HDF5CONST</b> <p>
 * The HDF5 API defines a set of constants and enumerated values. Most of these
 * values are available to Java programs via the class @ref HDF5CONST
 * <b>HDF5Constants</b></a>. For example, the parameters for the h5open() call
 ...
 * <b><i>#H5P_DEFAULT</i></b>. <p> The HDF5 API defines a set of values that
 * describe number types and sizes, such as "H5T_NATIVE_INT" and "hsize_t".
 * These values are determined at run time by the HDF5 C library. To support
 * these parameters, the Java HDFConstants class looks up the values when
 * initiated. The values can be accessed as public variables of the Java class,
 * such as:
 *
 * @code
 ...
 * difference is that the <b><i>HDFConstants</i></b> may have different values
 * on different platforms. <p> <b>@ref ERRORS</b> <p> The HDF5 error API (@ref
 * H5E) manages the behavior of the error stack in the HDF5 library. This API is
 * omitted from the JHI5. Errors are converted into Java exceptions. This is
 * totally different from the C interface, but is very natural for Java
 * programming. <p> The exceptions of the JHI5 are organized as sub-classes of
 * the class
 ...
 * sub-classes of the former represent errors from the HDF5 C library, while
 * sub-classes of the latter represent errors in the JHI5 wrapper and support
 * code. <p> The super-class <b><i>HDF5LibraryException</i></b> implements the
 * method '<b><i>printStackTrace()</i></b>', which prints out the HDF5 error
 * stack, as described in the HDF5 C API <i><b>@ref H5Eprint()</b>.</i> This may
 * be used by Java exception handlers to print out the HDF5 error stack. <hr>
 *
...
public class H5 implements java.io.Serializable {
   ...
   * System.loadLibrary()
   */
  public final static String H5_LIBRARY_NAME_PROPERTY_KEY =
      "hdf.hdf5lib.H5.loadLibraryName";
  ...
  public static void loadH5Lib() {
    ...
    if ((majnum != null) && (minnum != null) && (relnum != null)) {
      H5.H5check_version(majnum.intValue(), minnum.intValue(),
                         relnum.intValue());
    }
  }
   ...
   *do not match), this function causes the application to abort (i.e., crash)
   *
   * See C API function: @ref herr_t H5check_version(unsigned majnum, unsigned
   *minnum, unsigned relnum)
   **/
  public synchronized static native int H5check_version(int majnum, int minnum,
                                                        int relnum);
   ...
   *been unloaded). <p> This is called by the static initializer, so this should
   ...
   * of the HDF5 C library on stdout. This behavior may be disabled by calling
   * H5error_off().
   *
   * @return a non-negative value if successful
   */
   ...
   * @ingroup JH5
   *
   * Turn on error handling. By default, the C library prints the error stack of
   * the HDF5 C library on stdout. This behavior may be re-enabled by calling
   * H5error_on().
   */
   ...
   *      of -1 for a limit means no limit of that type.  These limits are
   * global for the entire library.  Each "global" limit only applies to free
   * lists of that type, so if an application sets a limit of 1 MB on each of
   * the global lists, up to 3 MB of total storage might be allocated (1MB on
  ...
  H5set_free_list_limits(int reg_global_lim, int reg_list_lim,
  ...
  H5export_dataset(String file_export_name, long file_id, String object_path,
   ...
   **/
  public synchronized static native void
  H5export_attribute(String file_export_name, long dataset_id,
   ...
   * @ingroup JH5
   *
   * H5is_library_threadsafe Checks to see if the library was built with
   ...
   *intended usage of a primary data object. A primary data object may be a
   *dataset, group, or committed datatype.
   *
   * @see H5A, C-API
   *
   ...
   *            Name is null.
   **/
  public static long H5Acreate(long loc_id, String attr_name, long type_id,
                               long space_id, long acpl_id, long aapl_id)
      throws HDF5LibraryException, NullPointerException {
    long id =
        _H5Acreate2(loc_id, attr_name, type_id, space_id, acpl_id, aapl_id);
    if (id > 0) {
   ...
   **/
  private synchronized static native long
  _H5Acreate2(long loc_id, String attr_name, long type_id, long space_id,
   ...
   *            IN: Location or object identifier; may be dataset or group
   * @param obj_name
   *            IN: Name, relative to loc_id, of object that attribute is to be
   *attached to
   * @param attr_name
   *            IN: Attribute name
   * @param type_id
   ...
   *            IN: Attribute dataspace identifier
   * @param acpl_id
   *            IN: Attribute creation property list identifier (currently not
   *used).
   * @param aapl_id
   *            IN: Attribute access property list identifier (currently not
   *used).
   * @param lapl_id
   ...
   *            name is null.
   **/
  public static long H5Acreate_by_name(long loc_id, String obj_name,
                                       String attr_name, long type_id,
                                       long space_id, long acpl_id,
                                       long aapl_id, long lapl_id)
      throws HDF5LibraryException, NullPointerException {
    long id = _H5Acreate_by_name(loc_id, obj_name, attr_name, type_id, space_id,
  ...
  private synchronized static native long
  _H5Acreate_by_name(long loc_id, String obj_name, String attr_name,
   ...
   * @ingroup JH5A
   *
   * H5Adelete_by_idx removes an attribute, specified by its location in an
   *index, from an object.
   *
   * @param loc_id
   *            IN: Location or object identifier; may be dataset or group
   * @param obj_name
   *            IN: Name of object, relative to location, from which attribute
   *is to be removed
   * @param idx_type
   *            IN: Type of index
   * @param order
   ...
   **/
  public synchronized static native void
  H5Adelete_by_idx(long loc_id, String obj_name, int idx_type, int order,
                   long n, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5A
   *
   * H5Adelete_by_name removes the attribute attr_name from an object specified
   *by location and name, loc_id and obj_name, respectively.
   *
   * @param loc_id
   *            IN: Location or object identifier; may be dataset or group
   * @param obj_name
   ...
   **/
  public synchronized static native int
  H5Adelete_by_name(long loc_id, String obj_name, String attr_name,
                    long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            attr_name is null.
   **/
  public synchronized static native boolean H5Aexists(long obj_id,
                                                      String attr_name)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @param obj_name
   *            IN: Name, relative to loc_id, of object that attribute is
   *attached to.
   * @param attr_name
   ...
   *            name is null.
   **/
  public synchronized static native boolean H5Aexists_by_name(long loc_id,
                                                              String obj_name,
                                                              String attr_name,
                                                              long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            obj_name is null.
   **/
  public synchronized static native H5A_info_t H5Aget_info_by_idx(
      long loc_id, String obj_name, int idx_type, int order, long n,
   ...
   *            obj_name is null.
   **/
  public synchronized static native H5A_info_t H5Aget_info_by_name(
      long loc_id, String obj_name, String attr_name, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5A
   *
   * H5Aget_name_by_idx retrieves the name of an attribute that is attached to
   *an object, which is specified by its location and name, loc_id and obj_name,
   *respectively.
   *
   * @param attr_id
   *            IN: Attribute identifier
   ...
   *            obj_name is null.
   **/
  public synchronized static native String H5Aget_name_by_idx(
      long attr_id, String obj_name, int idx_type, int order, long n,
      long lapl_id) throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5A
   *
   * H5Aopen opens an existing attribute, attr_name, that is attached to an
   *object specified an object identifier, object_id.
   *
   * @param obj_id
  ...
  private synchronized static native long
  _H5Aopen(long obj_id, String attr_name, long aapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            Name is null.
   **/
  public static long H5Aopen_by_idx(long loc_id, String obj_name, int idx_type,
                                    int order, long n, long aapl_id,
                                    long lapl_id)
      throws HDF5LibraryException, NullPointerException {
    long id =
        _H5Aopen_by_idx(loc_id, obj_name, idx_type, order, n, aapl_id, lapl_id);
    if (id > 0) {
  ...
  private synchronized static native long
  _H5Aopen_by_idx(long loc_id, String obj_name, int idx_type, int order, long n,
                  long aapl_id, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5A
   *
   * H5Aopen_by_name Opens an attribute for an object by object name and
   *attribute name
   *
   * @param loc_id
   ...
   *            obj_name is null.
   **/
  public static long H5Aopen_by_name(long loc_id, String obj_name,
                                     String attr_name, long aapl_id,
                                     long lapl_id)
      throws HDF5LibraryException, NullPointerException {
  ...
  private synchronized static native long
  _H5Aopen_by_name(long loc_id, String obj_name, String attr_name, long aapl_id,
                   long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            Error from the HDF5 Library.
   * @exception NullPointerException
   *            data buffer is null. See public synchronized static native int
   *H5Aread( )
   **/
  public synchronized static int H5Aread(long attr_id, long mem_type_id,
                                         Object obj, boolean isCriticalPinning)
      throws HDF5Exception, HDF5LibraryException, NullPointerException {
    int status = -1;
    boolean is1D = false;
    ...
    if (is1D && (dname == 'B')) {
      ...
      status =
          H5Aread_short(attr_id, mem_type_id, (short[])obj, isCriticalPinning);
      ...
      status =
          H5Aread_long(attr_id, mem_type_id, (long[])obj, isCriticalPinning);
    } else if (is1D && (dname == 'F')) {
      log.trace("H5Aread_dname_F");
      status =
          H5Aread_float(attr_id, mem_type_id, (float[])obj, isCriticalPinning);
    } else if (is1D && (dname == 'D')) {
      log.trace("H5Aread_dname_D");
      status = H5Aread_double(attr_id, mem_type_id, (double[])obj,
                              isCriticalPinning);
   ...
   * H5Aread reads an attribute, specified with attr_id. The attribute's memory
   *datatype is specified with mem_type_id. The entire attribute is read into
   *buffer of double from the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to read.
   * @param mem_type_id
   ...
   *            data buffer is null.
   **/
  public synchronized static int H5Aread_double(long attr_id, long mem_type_id,
                                                double[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   *            data buffer is null.
   **/
  public synchronized static int H5Aread_float(long attr_id, long mem_type_id,
                                               float[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   *            data buffer is null.
   **/
  public synchronized static native int H5Aread_int(long attr_id,
                                                    long mem_type_id, int[] buf,
                                                    boolean isCriticalPinning)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            data buffer is null.
   **/
  public synchronized static int H5Aread_int(long attr_id, long mem_type_id,
                                             int[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   *            data buffer is null.
   **/
  public synchronized static int H5Aread_long(long attr_id, long mem_type_id,
                                              long[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   * H5Aread reads an attribute, specified with attr_id. The attribute's memory
   *datatype is specified with mem_type_id. The entire attribute is read into
   *buffer of String from the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to read.
   * @param mem_type_id
   ...
   *            data buffer is null.
   **/
  public synchronized static int H5Aread_short(long attr_id, long mem_type_id,
                                               short[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   * H5Aread reads an attribute, specified with attr_id. The attribute's memory
   *datatype is specified with mem_type_id. The entire attribute is read into
   *buffer of variable-lenght from the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to read.
   * @param mem_type_id
   ...
   * H5Aread reads an attribute, specified with attr_id. The attribute's memory
   *datatype is specified with mem_type_id. The entire attribute is read into
   *buffer of variable-lenght strings from the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to read.
   * @param mem_type_id
   *            IN: Identifier of the attribute datatype (in memory).
   * @param buf
   *            Buffer of variable-lenght strings to store data read from the
   *file.
   *
   * @return a non-negative value if successful
   *
   ...
   *object specified by loc_id and obj_name. The attribute named old_attr_name
   *is renamed new_attr_name.
   *
   * @param loc_id
   *            IN: Location or object identifier; may be dataset or group
   * @param obj_name
   *            IN: Name of object, relative to location, whose attribute is to
   *be renamed
   * @param old_attr_name
   *            IN: Prior attribute name
   * @param new_attr_name
   ...
   **/
  public synchronized static native int
  H5Arename_by_name(long loc_id, String obj_name, String old_attr_name,
   ...
   *            data is null.
   **/
  public synchronized static native int H5Awrite(long attr_id, long mem_type_id,
                                                 byte[] buf,
                                                 boolean isCriticalPinning)
      throws HDF5LibraryException, NullPointerException;
  ...
  public synchronized static int H5Awrite(long attr_id, long mem_type_id,
    ...
    if (is1D && (dname == 'B')) {
      ...
      status =
          H5Awrite_short(attr_id, mem_type_id, (short[])obj, isCriticalPinning);
    } else if (is1D && (dname == 'I')) {
      status =
          H5Awrite_int(attr_id, mem_type_id, (int[])obj, isCriticalPinning);
    } else if (is1D && (dname == 'J')) {
      status =
          H5Awrite_long(attr_id, mem_type_id, (long[])obj, isCriticalPinning);
    } else if (is1D && (dname == 'F')) {
      status =
          H5Awrite_float(attr_id, mem_type_id, (float[])obj, isCriticalPinning);
    } else if (is1D && (dname == 'D')) {
      status = H5Awrite_double(attr_id, mem_type_id, (double[])obj,
                               isCriticalPinning);
    } else if (is1D && (dataClass.getComponentType() == String.class)) {
   ...
   * H5Awrite writes an attribute, specified with attr_id. The attribute's
   *memory datatype is specified with mem_type_id. The entire attribute is
   *written from buffer of double to the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to write.
   * @param mem_type_id
   ...
   *            data is null.
   **/
  public synchronized static int H5Awrite_double(long attr_id, long mem_type_id,
                                                 double[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   * H5Awrite writes an attribute, specified with attr_id. The attribute's
   *memory datatype is specified with mem_type_id. The entire attribute is
   *written from buffer of float to the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to write.
   * @param mem_type_id
   ...
   *            data is null.
   **/
  public synchronized static int H5Awrite_float(long attr_id, long mem_type_id,
                                                float[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   * H5Awrite writes an attribute, specified with attr_id. The attribute's
   *memory datatype is specified with mem_type_id. The entire attribute is
   *written from buffer of int to the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to write.
   * @param mem_type_id
   ...
   *            data is null.
   **/
  public synchronized static int H5Awrite_int(long attr_id, long mem_type_id,
                                              int[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   * H5Awrite writes an attribute, specified with attr_id. The attribute's
   *memory datatype is specified with mem_type_id. The entire attribute is
   *written from buffer of long to the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to write.
   * @param mem_type_id
   ...
   *            data is null.
   **/
  public synchronized static int H5Awrite_long(long attr_id, long mem_type_id,
                                               long[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   * H5Awrite writes an attribute, specified with attr_id. The attribute's
   *memory datatype is specified with mem_type_id. The entire attribute is
   *written from buffer of short to the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to write.
   * @param mem_type_id
   ...
   *            data is null.
   **/
  public synchronized static int H5Awrite_short(long attr_id, long mem_type_id,
                                                short[] buf)
      throws HDF5LibraryException, NullPointerException {
   ...
   * H5Awrite writes an attribute, specified with attr_id. The attribute's
   *memory datatype is specified with mem_type_id. The entire attribute is
   *written from buffer of string to the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to write.
   * @param mem_type_id
   ...
   * H5Awrite writes an attribute, specified with attr_id. The attribute's
   *memory datatype is specified with mem_type_id. The entire attribute is
   *written from buffer of variable-lenght to the file.
   *
   * @param attr_id
   *            IN: Identifier of an attribute to write.
   * @param mem_type_id
   *            IN: Identifier of the attribute datatype (in memory).
   * @param buf
   *            IN: Buffer of variable-lenght with data to be written to the
   *file.
   *
   * @return a non-negative value if successful
   *
   ...
   * @ingroup JH5A
   *
   * H5Awrite_VLStrings writes a variable length String dataset, specified by
   *its identifier attr_id, from the application memory buffer buffer of
   *variable-lenght strings into the file.
   *
   ...
   * @param buf
   *            Buffer of variable-lenght strings with data to be written to the
   *file.
   *
   * @return a non-negative value if successful
   *
   ...
   * @ingroup JH5A
   *
   * H5Aget_create_plist retrieves a copy of the attribute creation property
   *list identifier.
   *
   * @param attr_id
   *            IN: Identifier of an attribute.
  ...
  private synchronized static native long _H5Aget_create_plist(long attr_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5A
   *
   * H5Aiterate2 iterates over the attributes attached to a dataset, named
   *datatype, or group, as specified by obj_id. For each attribute,
   *user-provided data, op_data, with additional information as defined below,
   *is passed to a user-defined function, op, which operates on that attribute.
   *
   * @param loc_id
   *            IN: Identifier for object to which attributes are attached; may
   *be group, dataset, or named datatype.
   * @param idx_type
   *            IN: The type of index specified by idx_type can be one of the
   *following: H5_INDEX_NAME             An alphanumeric index by attribute
   *name. H5_INDEX_CRT_ORDER        An index by creation order.
   * @param order
   *            IN: The order in which the index is to be traversed, as
   *specified by order, can be one of the following: H5_ITER_INC     Iteration
   *is from beginning to end, i.e., a top-down iteration incrementing the index
   *position at each step. H5_ITER_DEC     Iteration starts at the end of the
   *index, i.e., a bottom-up iteration decrementing the index position at each
   *step. H5_ITER_NATIVE  HDF5 iterates in the fastest-available order. No
   *information is provided as to the order, but HDF5 ensures that each element
   *in the index will be visited if the iteration completes successfully.
   * @param idx
   *            IN/OUT: Initial and returned offset within index.
   * @param op
   *            IN: Callback function to operate on each value.
   * @param op_data
   *            IN/OUT: Pointer to any user-efined data for use by operator
   *function.
   *
   * @return returns the return value of the first operator that returns a
   *positive value, or zero if all members were processed with no operator
   *returning non-zero.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   **/
  public synchronized static native int
  H5Aiterate(long loc_id, int idx_type, int order, long idx, H5A_iterate_cb op,
   ...
   * @ingroup JH5A
   *
   * H5Aiterate_by_name iterates over the attributes attached to the dataset or
   *group specified with loc_id and obj_name. For each attribute, user-provided
   *data, op_data, with additional information as defined below, is passed to a
   *user-defined function, op, which operates on that attribute.
   *
   * @param loc_id
   *            IN: Identifier for object to which attributes are attached; may
   *be group, dataset, or named datatype.
   ...
   **/
  public synchronized static native int
  H5Aiterate_by_name(long loc_id, String obj_name, int idx_type, int order,
                     long idx, H5A_iterate_cb op, H5A_iterate_t op_data,
                     long lapl_id)
      throws HDF5LibraryException, NullPointerException;
  ...
  public static long H5Dcreate(long loc_id, String name, long type_id,
                               long space_id, long lcpl_id, long dcpl_id,
                               long dapl_id)
      throws HDF5LibraryException, NullPointerException {
    long id =
        _H5Dcreate2(loc_id, name, type_id, space_id, lcpl_id, dcpl_id, dapl_id);
    if (id > 0) {
   ...
   **/
  private synchronized static native long
  _H5Dcreate2(long loc_id, String name, long type_id, long space_id,
  ...
  private synchronized static native long
  _H5Dcreate_anon(long loc_id, long type_id, long space_id, long dcpl_id,
   ...
   * @param buf
   *            IN/OUT: Pointer to the memory buffer containing the selection to
   *be filled.
   * @param buf_type
   *            IN: Datatype of dataspace elements to be filled.
   * @param space_id
   *            IN: Dataspace describing memory buffer and containing the
   ...
   *
   * @param buf
   *            IN/OUT: Pointer to the memory containing the elements to iterate
   *over.
   * @param buf_type
   ...
   **/
  public synchronized static native int
  H5Diterate(byte[] buf, long buf_type, long space_id, H5D_iterate_cb op,
   ...
   **/
  private synchronized static native long _H5Dopen2(long loc_id, String name,
                                                    long dapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   **/
  public synchronized static native int
  H5Dread(long dataset_id, long mem_type_id, long mem_space_id,
  ...
  public synchronized static int H5Dread(long dataset_id, long mem_type_id,
    ...
    if (is1D && (dname == 'B')) {
      ...
      status = H5Dread(dataset_id, mem_type_id, mem_space_id, file_space_id,
                       xfer_plist_id, buf, isCriticalPinning);
      if (status >= 0) {
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of type double.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dread_double(long dataset_id, long mem_type_id, long mem_space_id,
  ...
  H5Dread_double(long dataset_id, long mem_type_id, long mem_space_id,
                 long file_space_id, long xfer_plist_id, double[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dread_double(dataset_id, mem_type_id, mem_space_id, file_space_id,
                          xfer_plist_id, buf, true);
  }
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of float.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dread_float(long dataset_id, long mem_type_id, long mem_space_id,
  ...
  H5Dread_float(long dataset_id, long mem_type_id, long mem_space_id,
                long file_space_id, long xfer_plist_id, float[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dread_float(dataset_id, mem_type_id, mem_space_id, file_space_id,
                         xfer_plist_id, buf, true);
  }
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of int.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dread_int(long dataset_id, long mem_type_id, long mem_space_id,
  ...
  H5Dread_int(long dataset_id, long mem_type_id, long mem_space_id,
              long file_space_id, long xfer_plist_id, int[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dread_int(dataset_id, mem_type_id, mem_space_id, file_space_id,
                       xfer_plist_id, buf, true);
  }
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of long.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dread_long(long dataset_id, long mem_type_id, long mem_space_id,
  ...
  H5Dread_long(long dataset_id, long mem_type_id, long mem_space_id,
               long file_space_id, long xfer_plist_id, long[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dread_long(dataset_id, mem_type_id, mem_space_id, file_space_id,
                        xfer_plist_id, buf, true);
  }
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of string.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dread_reg_ref(long dataset_id, long mem_type_id, long mem_space_id,
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of short.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dread_short(long dataset_id, long mem_type_id, long mem_space_id,
  ...
  H5Dread_short(long dataset_id, long mem_type_id, long mem_space_id,
                long file_space_id, long xfer_plist_id, short[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dread_short(dataset_id, mem_type_id, mem_space_id, file_space_id,
                         xfer_plist_id, buf, true);
  }
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of variable-lenght.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5DreadVL(long dataset_id, long mem_type_id, long mem_space_id,
   ...
   **/
  public synchronized static native int
  H5Dread_string(long dataset_id, long mem_type_id, long mem_space_id,
                 long file_space_id, long xfer_plist_id, String[] buf)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   * H5Dread reads a (partial) dataset, specified by its identifier dataset_id,
   *from the file into the application memory buffer of variable-lenght strings.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dread_VLStrings(long dataset_id, long mem_type_id, long mem_space_id,
                    long file_space_id, long xfer_plist_id, Object[] buf)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @param size
   *            IN: Array containing the new magnitude of each dimension of the
   *dataset.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *            size is null.
   **/
  public synchronized static native void H5Dset_extent(long dset_id,
                                                       long size[])
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5D
   *
   * H5Dvlen_get_buf_size determines the number of bytes required to store the
   *VL data from the dataset, using the space_id for the selection in the
   *dataset on disk and the type_id for the memory representation of the VL data
   *in memory.
   *
   * @param dset_id
   ...
   *
   * @return the size in bytes of the memory buffer required to store the VL
   *data.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *dataset_id, from the application memory buffer into the file.
   *
   * @param dataset_id
   *            Identifier of the dataset read from.
   * @param mem_type_id
   ...
   **/
  public synchronized static native int
  H5Dwrite(long dataset_id, long mem_type_id, long mem_space_id,
  ...
  public synchronized static int H5Dwrite(long dataset_id, long mem_type_id,
    ...
    if (is1D && (dname == 'B')) {
      ...
      log.trace("H5Dwrite_string type");
      status = H5Dwrite_string(dataset_id, mem_type_id, mem_space_id,
                               file_space_id, xfer_plist_id, (String[])obj);
    } else if (H5.H5Tget_class(mem_type_id) == HDF5Constants.H5T_VLEN) {
      log.trace("H5DwriteVL type");
      status = H5DwriteVL(dataset_id, mem_type_id, mem_space_id, file_space_id,
                          xfer_plist_id, (Object[])obj);
    } else {
      HDFArray theArray = new HDFArray(obj);
      byte[] buf = theArray.byteify();
   ...
   **/
  public synchronized static native int
  H5Dwrite_double(long dataset_id, long mem_type_id, long mem_space_id,
                  long file_space_id, long xfer_plist_id, double[] buf,
                  boolean isCriticalPinning)
      throws HDF5LibraryException, NullPointerException;
  ...
  H5Dwrite_double(long dataset_id, long mem_type_id, long mem_space_id,
                  long file_space_id, long xfer_plist_id, double[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dwrite_double(dataset_id, mem_type_id, mem_space_id, file_space_id,
                           xfer_plist_id, buf, true);
  }
   ...
   **/
  public synchronized static native int
  H5Dwrite_float(long dataset_id, long mem_type_id, long mem_space_id,
                 long file_space_id, long xfer_plist_id, float[] buf,
                 boolean isCriticalPinning)
      throws HDF5LibraryException, NullPointerException;
  ...
  H5Dwrite_float(long dataset_id, long mem_type_id, long mem_space_id,
                 long file_space_id, long xfer_plist_id, float[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dwrite_float(dataset_id, mem_type_id, mem_space_id, file_space_id,
                          xfer_plist_id, buf, true);
  }
   ...
   **/
  public synchronized static native int
  H5Dwrite_int(long dataset_id, long mem_type_id, long mem_space_id,
               long file_space_id, long xfer_plist_id, int[] buf,
               boolean isCriticalPinning)
      throws HDF5LibraryException, NullPointerException;
  ...
  H5Dwrite_int(long dataset_id, long mem_type_id, long mem_space_id,
               long file_space_id, long xfer_plist_id, int[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dwrite_int(dataset_id, mem_type_id, mem_space_id, file_space_id,
                        xfer_plist_id, buf, true);
  }
   ...
   **/
  public synchronized static native int
  H5Dwrite_long(long dataset_id, long mem_type_id, long mem_space_id,
                long file_space_id, long xfer_plist_id, long[] buf,
                boolean isCriticalPinning)
      throws HDF5LibraryException, NullPointerException;
  ...
  H5Dwrite_long(long dataset_id, long mem_type_id, long mem_space_id,
                long file_space_id, long xfer_plist_id, long[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dwrite_long(dataset_id, mem_type_id, mem_space_id, file_space_id,
                         xfer_plist_id, buf, true);
  }
   ...
   **/
  public synchronized static native int
  H5Dwrite_short(long dataset_id, long mem_type_id, long mem_space_id,
                 long file_space_id, long xfer_plist_id, short[] buf,
                 boolean isCriticalPinning)
      throws HDF5LibraryException, NullPointerException;
  ...
  H5Dwrite_short(long dataset_id, long mem_type_id, long mem_space_id,
                 long file_space_id, long xfer_plist_id, short[] buf)
      throws HDF5LibraryException, NullPointerException {
    return H5Dwrite_short(dataset_id, mem_type_id, mem_space_id, file_space_id,
                          xfer_plist_id, buf, true);
  }
   ...
   **/
  public synchronized static native int
  H5Dwrite_string(long dataset_id, long mem_type_id, long mem_space_id,
                  long file_space_id, long xfer_plist_id, String[] buf)
      throws HDF5LibraryException, NullPointerException;
   ...
   **/
  public synchronized static native int
  H5DwriteVL(long dataset_id, long mem_type_id, long mem_space_id,
             long file_space_id, long xfer_plist_id, Object[] buf)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5D
   *
   * H5Dwrite_VLStrings writes a (partial) variable length String dataset,
   *specified by its identifier dataset_id, from the application memory buffer
   *buf into the file.
   *
   * ---- contributed by Rosetta Biosoftware
   *
  ...
  public synchronized static native int
  H5Dwrite_VLStrings(long dataset_id, long mem_type_id, long mem_space_id,
                     long file_space_id, long xfer_plist_id, Object[] buf)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5D
   *
   * H5Dflush causes all buffers associated with a dataset to be immediately
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Dflush(long dset_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5D
   *
   * H5Drefresh causes all buffers associated with a dataset to be cleared and
   *immediately re-loaded with updated contents from disk. This function
   *essentially closes the dataset, evicts all metadata associated with it from
   *the cache, and then re-opens the dataset. The reopened dataset is
   *automatically re-registered with the same ID.
   *
   * @param dset_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Drefresh(long dset_id)
      throws HDF5LibraryException;

  // /////// unimplemented ////////
  // herr_t H5Ddebug(hid_t dset_id);
  // herr_t H5Dget_chunk_storage_size(hid_t dset_id, const hsize_t *offset,
  // hsize_t *chunk_bytes); herr_t H5Dformat_convert(hid_t dset_id); herr_t
   ...
   *if there are problems initializing the library. <p> This may be used by
   *exception handlers to assure that the error condition in the HDF5 library
   *has been reset.
   *
   ...
   *
   * @param stack_id
   *            IN: Error stack identifier.If the identifier is H5E_DEFAULT, the
   *current error stack will be printed.
   * @param stream
   ...
   * @ingroup JH5E
   *
   * H5Epop deletes the number of error records specified in count from the top
   *of the error stack specified by estack_id (including major, minor messages
   *and description).
   *
   * @param stack_id
   *            IN: Error stack identifier.
   ...
   * @ingroup JH5E
   *
   * H5Epush pushes a new error record onto the error stack specified by
   *estack_id.
   *
   * @param stack_id
   *            IN: Error stack identifier.
   ...
   * @ingroup JH5E
   *
   * H5Epush2 pushes a new error record onto the error stack specified by
   *estack_id.
   *
   * @param stack_id
   *            IN: Error stack identifier.
   ...
   **/
  public synchronized static native void
  H5Epush2(long stack_id, String file, String func, int line, long cls_id,
           long maj_id, long min_id, String msg)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            IN: Name of the error class.
   * @param lib_name
   *            IN: Name of the client library or application to which the error
   *class belongs.
   * @param version
   *            IN: Version of the client library or application to which the
   *error class belongs.
   ...
   * @ingroup JH5E
   *
   * H5Ewalk walks the error stack specified by estack_id for the current thread
   *and calls the function specified in func for each error along the way.
   *
   * @param stack_id
   *            IN: Error stack identifier.
   * @param direction
   ...
   *            func is null.
   **/
  public static void H5Ewalk(long stack_id, long direction, H5E_walk_cb func,
   ...
   * @ingroup JH5E
   *
   * H5Ewalk2 walks the error stack specified by estack_id for the current
   *thread and calls the function specified in func for each error along the
   *way.
   *
   * @param stack_id
   *            IN: Error stack identifier.
   ...
   *            func is null.
   **/
  public synchronized static native void H5Ewalk2(long stack_id, long direction,
                                                  H5E_walk_cb func,
                                                  H5E_walk_t client_data)
      throws HDF5LibraryException, NullPointerException;
  ...
  // int H5Eget_auto(long estack_id, H5E_auto2_t func, PointerByReference
  // client_data);
  // {
  //         return H5Eget_auto2(estack_id, func, client_data);
  // }
  // int H5Eget_auto2(long estack_id, H5E_auto2_t func, PointerByReference
  // client_data);
  ...
  // public static void H5Epush(long err_stack, String file, String func, int
  // line,
  //             long cls_id, long maj_id, long min_id, String msg, ...)
  // {
  ...
  //         ...);
  // }
  // public synchronized static native void H5Epush2(long err_stack, String
  ...
  // H5_DLL herr_t H5ESwait(hid_t es_id, uint64_t timeout, size_t
  // *num_in_progress, hbool_t *err_occurred); H5_DLL herr_t H5EScancel(hid_t
  // es_id, size_t *num_not_canceled, hbool_t *err_occurred); H5_DLL herr_t
  // H5ESget_count(hid_t es_id, size_t *count); H5_DLL herr_t
  // H5ESget_op_counter(hid_t es_id, uint64_t *counter); H5_DLL herr_t
  // H5ESget_err_status(hid_t es_id, hbool_t *err_occurred); H5_DLL herr_t
  // H5ESget_err_count(hid_t es_id, size_t *num_errs); H5_DLL herr_t
  // H5ESget_err_info(hid_t es_id, size_t num_err_info, H5ES_err_info_t
  // err_info[],
  //                                size_t *err_cleared);
  // H5_DLL herr_t H5ESfree_err_info(size_t num_err_info, H5ES_err_info_t
  // err_info[]); H5_DLL herr_t H5ESregister_insert_func(hid_t es_id,
  // H5ES_event_insert_func_t func, void *ctx); H5_DLL herr_t
  // H5ESregister_complete_func(hid_t es_id, H5ES_event_complete_func_t func,
  // void *ctx); H5_DLL herr_t H5ESclose(hid_t es_id);
  //
   ...
   *            <li>
   *            @ref H5F_ACC_TRUNC Truncate file, if it already exists, erasing
   *all data previously stored in the file.</li> <li>
   *            @ref H5F_ACC_EXCL Fail if file already exists.</li>
   *            <li>
   *            @ref H5P_DEFAULT Apply default file access and creation
   *properties.</li>
   *            </ul>
   *
   * @param create_id
   *            File creation property list identifier, used when modifying
   *default file meta-data. Use H5P_DEFAULT for default access properties.
   * @param access_id
   *            File access property list identifier. If parallel file access is
   *desired, this is a collective call according to the communicator stored in
   *the access_id (not supported in Java). Use H5P_DEFAULT for default access
   *properties.
   *
   * @return a file identifier if successful
   *
  ...
  private synchronized static native long
  _H5Fcreate(String name, int flags, long create_id, long access_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *(memory) cache. <p> After this call completes, the file (or object) is in a
   *consistent state and all data written to date is assured to be permanent.
   *
   * @param object_id
   *            Identifier of object used to identify the file. <b>object_id</b>
   *can be any object associated with the file, including the file itself, a
   *dataset, a group, an attribute, or a named data type.
   * @param scope
   *            specifies the scope of the flushing action, in the case that the
   *HDF5 file is not a single physical file. <p> Valid values are: <UL> <LI>
   *H5F_SCOPE_GLOBAL Flushes the entire virtual file.</LI> <LI> H5F_SCOPE_LOCAL
   ...
   * @ingroup JH5F
   *
   * H5Fget_freespace returns the amount of space that is unused by any objects
   *in the file.
   *
   * @param file_id
   *            IN: File identifier for a currently-open HDF5 file
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native long H5Fget_fileno(long file_id)
      throws HDF5LibraryException;
   ...
   *size</li> <li>metadata_cache[1] = min_clean_size_ptr // current cache
   *minimum clean size</li> <li>metadata_cache[2] = cur_size_ptr // current
   *cache size</li>
   *            </ul>
   *
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native long H5Fget_obj_count(long file_id,
                                                          int types)
      throws HDF5LibraryException;
   ...
   *            name is null.
   **/
  public synchronized static native boolean H5Fis_accessible(String name,
                                                             long fapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @param name
   *            The name of the group onto which the file specified by child_id
   *is to be mounted.
   * @param child_id
   ...
   * @ingroup JH5F
   *
   * Given a mount point, H5Funmount disassociates the mount point's file from
   ...
   **/
  public synchronized static native void
  H5Freset_mdc_hit_rate_stats(long file_id) throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native H5F_info2_t H5Fget_info(long obj_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5F
   *
   * H5Fclear_elink_file_cache evicts all the cached child files in the
   *specified file's external file cache, causing them to be closed if there is
   *nothing else holding them open.
   *
   * @param file_id
   *            IN: Identifier of the target file.
   *
   ...
   * @ingroup JH5F
   *
   * H5Fstart_swmr_write will activate SWMR writing mode for a file associated
   *with file_id. This routine will prepare and ensure the file is safe for SWMR
   *writing.
   *
   * @param file_id
   *            IN: Identifier of the target file.
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Fstart_swmr_write(long file_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5F
   *
   * H5Fstart_mdc_logging starts logging metadata cache events if logging was
   *previously enabled.
   *
   * @param file_id
   *            IN: Identifier of the target file.
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Fstart_mdc_logging(long file_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5F
   *
   * H5Fstop_mdc_logging stops logging metadata cache events if logging was
   *previously enabled and is currently ongoing.
   *
   * @param file_id
   *            IN: Identifier of the target file.
   *
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Fstop_mdc_logging(long file_id)
      throws HDF5LibraryException;
   ...
   *             mdc_logging_status[1] = is_currently_logging, whether events
   ...
   * @ingroup JH5F
   *
   * H5Fget_dset_no_attrs_hint gets the file-level setting to create minimized
   *dataset object headers.
   *
   * @param file_id
   *            IN: Identifier of the target file.
   *
   * @return true if the file-level is set to create minimized dataset object
   *headers, false if not.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   **/
  public synchronized static native boolean
  H5Fget_dset_no_attrs_hint(long file_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5F
   *
   * H5Fset_dset_no_attrs_hint sets the file-level setting to create minimized
   *dataset object headers.
   *
   * @param file_id
   *            IN: Identifier of the target file.
   ...
   * @ingroup JH5F
   *
   * H5Fset_libver_bounds sets a different low and high bounds while a file is
   *open.
   *
   * @param file_id
   *            IN: Identifier of the target file.
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Fset_libver_bounds(long file_id,
                                                              int low, int high)
      throws HDF5LibraryException;
  ...
  // herr_t H5Fget_metadata_read_retry_info(hid_t file_id, H5F_retry_info_t
  // *info); ssize_t H5Fget_free_sections(hid_t file_id, H5F_mem_t type, size_t
  // nsects, H5F_sect_info_t *sect_info/*out*/);
  ...
  //     unsigned hits[2], unsigned misses[2], unsigned evictions[2], unsigned
  //     bypasses[2]);
  //  herr_t H5Fget_mdc_image_info(hid_t file_id, haddr_t *image_addr, hsize_t
  //  *image_size);
  ...
  // H5AC_cache_config_t in which the current metadata cache configuration is to
  // be reported.
  // *
  // * @return none
  // *
  ...
  //  H5FD_t *H5FDopen(const char *name, unsigned flags, hid_t fapl_id, haddr_t
  //  maxaddr); herr_t H5FDclose(H5FD_t *file); int H5FDcmp(const H5FD_t *f1,
  //  const H5FD_t *f2); int H5FDquery(const H5FD_t *f, unsigned long *flags);
  //  haddr_t H5FDalloc(H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id, hsize_t
  //  size); herr_t H5FDfree(H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id,
  //  haddr_t addr, hsize_t size); haddr_t H5FDget_eoa(H5FD_t *file, H5FD_mem_t
  //  type); herr_t H5FDset_eoa(H5FD_t *file, H5FD_mem_t type, haddr_t eoa);
  //  haddr_t H5FDget_eof(H5FD_t *file, H5FD_mem_t type);
  //  herr_t H5FDget_vfd_handle(H5FD_t *file, hid_t fapl, void**file_handle);
  //  herr_t H5FDread(H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id, haddr_t
  //  addr, size_t size, void *buf/*out*/); herr_t H5FDwrite(H5FD_t *file,
  //  H5FD_mem_t type, hid_t dxpl_id, haddr_t addr, size_t size, const void
  //  *buf); herr_t H5FDflush(H5FD_t *file, hid_t dxpl_id, hbool_t closing);
  //  herr_t H5FDtruncate(H5FD_t *file, hid_t dxpl_id, hbool_t closing); herr_t
  //  H5FDlock(H5FD_t *file, hbool_t rw); herr_t H5FDunlock(H5FD_t *file);
   ...
   *properties have been implemented at this time; use H5P_DEFAULT.)
   *
   ...
   *            name is null.
   **/
  public static long H5Gcreate(long loc_id, String name, long lcpl_id,
                               long gcpl_id, long gapl_id)
      throws HDF5LibraryException, NullPointerException {
  ...
  private synchronized static native long
  _H5Gcreate2(long loc_id, String name, long lcpl_id, long gcpl_id,
  ...
  private synchronized static native long
  _H5Gcreate_anon(long loc_id, long gcpl_id, long gapl_id)
      throws HDF5LibraryException;
   ...
   *            name is null.
   **/
  public synchronized static native H5G_info_t
   ...
   */
  public synchronized static int
  H5Gget_obj_info_all(long loc_id, String name, String[] objNames,
    ...
    return H5Gget_obj_info_all(loc_id, name, objNames, objTypes, null, null,
                               tokens, HDF5Constants.H5_INDEX_NAME);
  }
  ...
  H5Gget_obj_info_all(long loc_id, String name, String[] objNames,
                      ...
                      int indx_type)
      throws HDF5LibraryException, NullPointerException {
    return H5Gget_obj_info_full(loc_id, name, objNames, objTypes, ltype, null,
                                tokens, indx_type, -1);
  }
  ...
  H5Gget_obj_info_all(long loc_id, String name, String[] objNames,
      ...
      throws HDF5LibraryException, NullPointerException {
    return H5Gget_obj_info_full(loc_id, name, objNames, objTypes, ltype, fno,
                                tokens, indx_type, -1);
  }
   ...
   */
  public synchronized static int
  H5Gget_obj_info_full(long loc_id, String name, String[] objNames,
                       int[] objTypes, int[] ltype, long[] fno,
    ...
    int status =
        H5Gget_obj_info_full(loc_id, name, objNames, objTypes, ltype, fno,
                             tokens, objNames.length, indx_type, indx_order);
    for (int indx = 0; indx < objNames.length; indx++)
      log.trace("H5Gget_obj_info_full: objNames={}", objNames[indx]);
    return status;
  }

  private synchronized static native int H5Gget_obj_info_full(
      long loc_id, String name, String[] objNames, int[] objTypes, int[] ltype,
      long[] fno, H5O_token_t[] tokens, int n, int indx_type, int indx_order)
      throws HDF5LibraryException, NullPointerException;

  /**
   * @ingroup JH5G
   *
   * H5Gget_obj_info_idx report the name and type of object with index 'idx' in
   * a Group. The 'idx' corresponds to the index maintained by H5Giterate. Each
   * link is returned, so objects with multiple links will be counted once for
   * each link.
   *
   * @param loc_id
   *            IN: file or group ID.
  ...
  public synchronized static int H5Gget_obj_info_idx(long loc_id, String name,
                                                     int idx, String[] oname,
                                                     int[] type)
      throws HDF5LibraryException, NullPointerException {
    String n[] = new String[1];
    n[0] = new String("");
    oname[0] = H5Lget_name_by_idx(loc_id, name, HDF5Constants.H5_INDEX_NAME,
                                  HDF5Constants.H5_ITER_INC, idx,
                                  HDF5Constants.H5P_DEFAULT);
    H5L_info_t info = H5Lget_info_by_idx(
        loc_id, name, HDF5Constants.H5_INDEX_NAME, HDF5Constants.H5_ITER_INC,
   ...
   * located in the file or group specified by loc_id up to maximum specified by
   * objMax.
   *
   * @param loc_id
   *            IN: File or group identifier
   ...
   */
  public synchronized static int
  H5Gget_obj_info_max(long loc_id, String[] objNames, int[] objTypes,
    ...
    return H5Gget_obj_info_max(loc_id, objNames, objTypes, lnkTypes, objToken,
                               objMax, objNames.length);
  }

  private synchronized static native int
  H5Gget_obj_info_max(long loc_id, String[] oname, int[] otype, int[] ltype,
   ...
   * @ingroup JH5G
   *
   * H5Gflush causes all buffers associated with a group to be immediately
   *flushed to disk without removing the data from the cache.
   *
   * @param group_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Gflush(long group_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5G
   *
   * H5Grefresh causes all buffers associated with a group to be cleared and
   *immediately re-loaded with updated contents from disk. This function
   *essentially closes the group, evicts all metadata associated with it from
   *the cache, and then re-opens the group. The reopened group is automatically
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Grefresh(long group_id)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5I
   *
   * H5Iget_name_long retrieves the name of an object specified by the
   *identifier, obj_id.
   * @deprecated
   *
   * @param obj_id
   ...
   * @ingroup JH5I
   *
   * H5Iget_name retrieves the name of an object specified by the identifier,
   *obj_id.
   *
   * @param obj_id
   *            IN: Identifier of the object.
   ...
   * @ingroup JH5I
   *
   * H5Iget_ref obtains the number of references outstanding specified by the
   *identifier, obj_id.
   *
   * @param obj_id
   *            IN: Identifier of the object.
   ...
   * @ingroup JH5I
   *
   * H5Idec_ref decrements the reference count specified by the identifier,
   *obj_id. If the reference count for an ID reaches zero, the object will be
   *closed.
   *
   * @param obj_id
   *            IN: Identifier of the object.
   ...
   * @ingroup JH5I
   *
   * H5Iinc_ref increments the reference count specified by the identifier,
   *obj_id.
   *
   * @param obj_id
   *            IN: Identifier of the object.
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Iget_type(long obj_id)
      throws HDF5LibraryException;
   ...
   *
   * @param type_id
   *            IN: The identifier of the type whose reference count is to be
   *retrieved
   *
   ...
   * @ingroup JH5I
   *
   * H5Idec_type_ref decrements the reference count on an identifier type. The
   *reference count is used by the library to indicate when an identifier type
   *can be destroyed. If the reference count reaches zero, this function will
   *destroy it.
   *
   * @param type_id
   *            IN: The identifier of the type whose reference count is to be
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Idec_type_ref(long type_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5I
   *
   * H5Iinc_type_ref increments the reference count on an ID type. The reference
   *count is used by the library to indicate when an ID type can be destroyed.
   *
   * @param type_id
   *            IN: The identifier of the type whose reference count is to be
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Iinc_type_ref(long type_id)
      throws HDF5LibraryException;
   ...
   *
   * @param type_id
   *            IN: Identifier for the identifier type whose member count will
   *be retrieved
   *
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native boolean H5Itype_exists(int type_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5I
   *
   * H5Iclear_type deletes all identifiers of the type identified by the
   *argument type.
   *
   * @param type_id
   *            IN: Identifier of identifier type which is to be cleared of
   *identifiers
   * @param force
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Iclear_type(int type_id,
                                                       boolean force)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5I
   *
   * H5Idestroy_type deletes an entire identifier type. All identifiers of this
   *type are destroyed and no new identifiers of this type can be registered.
   *
   * @param type_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Idestroy_type(int type_id)
      throws HDF5LibraryException;
   ...
   *            name is null.
   **/
  public synchronized static native void H5Lcopy(long src_loc, String src_name,
                                                 long dst_loc, String dst_name,
   ...
   **/
  public synchronized static native void
  H5Lcreate_external(String file_name, String obj_name, long link_loc_id,
   ...
   **/
  public synchronized static native void
  H5Lcreate_hard(long cur_loc, String cur_name, long dst_loc, String dst_name,
                 long lcpl_id, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   **/
  public synchronized static native void
  H5Lcreate_soft(String link_target, long link_loc_id, String link_name,
                 long lcpl_id, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            name is null.
   **/
  public synchronized static native void H5Ldelete(long loc_id, String name,
                                                   long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   * @param loc_id
   *            IN: File or group identifier specifying location of subject
   *group
   * @param group_name
   ...
   **/
  public synchronized static native void
  H5Ldelete_by_idx(long loc_id, String group_name, int idx_type, int order,
                   long n, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            name is null.
   **/
  public synchronized static native boolean H5Lexists(long loc_id, String name,
                                                      long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            name is null.
   **/
  public synchronized static native H5L_info_t H5Lget_info(long loc_id,
                                                           String name,
                                                           long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5L
   *
   * H5Lget_info_by_idx opens a named datatype at the location specified by
   *loc_id and return an identifier for the datatype.
   *
   * @param loc_id
   *            IN: File or group identifier specifying location of subject
   *group
   ...
   *            group_name is null.
   **/
  public synchronized static native H5L_info_t H5Lget_info_by_idx(
      long loc_id, String group_name, int idx_type, int order, long n,
      long lapl_id) throws HDF5LibraryException, NullPointerException;
   ...
   *            group_name is null.
   **/
  public synchronized static native String H5Lget_name_by_idx(
      long loc_id, String group_name, int idx_type, int order, long n,
      long lapl_id) throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5L
   *
   * H5Lget_value returns the link value of a symbolic link. Note that this
   *function is a combination of H5Lget_info(), H5Lget_val() and for external
   ...
   * @param link_value
   *            OUT: Path of the symbolic link, or the file_name and path of an
   *external file.
   * @param lapl_id
   *            IN: Link access property list identifier
   *
   ...
   * @ingroup JH5L
   *
   * H5Lget_value_by_idx retrieves value of the nth link in a group, according
   *to the order within an index. Note that this function is a combination of
   *H5Lget_info(), H5Lget_val() and for external links, H5Lunpack_elink_val.
   *
   * @param loc_id
   *            IN: File or group identifier specifying location of subject
   *group
   ...
   **/
  public synchronized static native int
  H5Lget_value_by_idx(long loc_id, String group_name, int idx_type, int order,
                      long n, String[] link_value, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @param op
   *            IN: Callback function passing data regarding the link to the
   *calling application
   * @param op_data
   *            IN: User-defined pointer to data required by the application for
   ...
   **/
  public synchronized static native int
  H5Literate(long grp_id, int idx_type, int order, long idx, H5L_iterate_t op,
   ...
   **/
  public synchronized static native int
  H5Literate_by_name(long grp_id, String group_name, int idx_type, int order,
                     long idx, H5L_iterate_t op, H5L_iterate_opdata_t op_data,
                     long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            IN: New link name.
   * @param lcpl_id
   *            IN: Link creation property list identifier to be associated with
   *the new link.
   * @param lapl_id
   *            IN: Link access property list identifier to be associated with
   *the new link.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *            name is null.
   **/
  public synchronized static native void H5Lmove(long src_loc, String src_name,
                                                 long dst_loc, String dst_name,
                                                 long lcpl_id, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Lvisit(long grp_id, int idx_type,
   ...
   **/
  public synchronized static native int
  H5Lvisit_by_name(long loc_id, String group_name, int idx_type, int order,
                   H5L_iterate_t op, H5L_iterate_opdata_t op_data, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *registered, either by the HDF5 Library or by the user through the use of
   *H5Lregister.
   *
   * @param link_cls_id
   *            IN: User-defined link class identifier
   *
   * @return Returns a positive value if the link class has been registered and
   *zero if it is unregistered. Otherwise returns a negative value; this may
   *mean that the identifier is not a valid user-defined class identifier.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Lis_registered(int link_cls_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5L
   *
   * H5Lunregister unregisters a class of user-defined links, preventing them
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Lunregister(int link_cls_id)
      throws HDF5LibraryException;

  // /////// unimplemented ////////
  // herr_t H5Lcreate_ud(hid_t link_loc_id, const char *link_name,
  //         H5L_type_t link_type, const void *udata, size_t udata_size, hid_t
  ...
  //         unsigned *flags, const char **filename/*out*/, const char
  //         **obj_path /*out*/);
   ...
   *
   * @param src_loc_id
   *            IN: Object identifier indicating the location of the source
   *object to be copied
   * @param src_name
   ...
   **/
  public synchronized static native void
  H5Ocopy(long src_loc_id, String src_name, long dst_loc_id, String dst_name,
          long ocpypl_id, long lcpl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   **/
  @Deprecated
  public synchronized static native void H5Oset_comment(long obj_id,
                                                        String comment)
      throws HDF5LibraryException;
   ...
   * @param name
   *            IN: Relative name of the object whose comment is to be set or
   *reset.
   * @param lapl_id
   *            IN: Link access property list identifier.
   *
   ...
   *            name is null.
   **/
  public synchronized static native String H5Oget_comment_by_name(long loc_id,
                                                                  String name,
                                                                  long lapl_id)
      throws HDF5LibraryException, IllegalArgumentException,
   ...
   * @ingroup JH5O
   *
   * H5Oget_info retrieves the metadata for an object specified by an
   *identifier.
   *
   * @param loc_id
   ...
   *            name is null.
   **/
  public static H5O_info_t H5Oget_info(long loc_id)
      throws HDF5LibraryException, NullPointerException {
   ...
   *            IN: File or group identifier
   * @param group_name
   *            IN: Name of group, relative to loc_id, in which object is
   *located
   * @param idx_type
   *            IN: Type of index by which objects are ordered
   * @param order
   ...
   *            IN: Object to open
   * @param lapl_id
   *            IN: Access property list identifier for the link pointing to the
   *object (Not currently used; pass as H5P_DEFAULT.)
   *
   * @return object information
   *
   * @exception HDF5LibraryException
   ...
   *            name is null.
   **/
  public static H5O_info_t H5Oget_info_by_idx(long loc_id, String group_name,
                                              int idx_type, int order, long n,
                                              long lapl_id)
      throws HDF5LibraryException, NullPointerException {
    return H5Oget_info_by_idx(loc_id, group_name, idx_type, order, n,
                              HDF5Constants.H5O_INFO_ALL, lapl_id);
  }
   ...
   *            name is null.
   **/
  public synchronized static native H5O_info_t
  H5Oget_info_by_idx(long loc_id, String group_name, int idx_type, int order,
                     long n, int fields, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            name is null.
   **/
  public static H5O_info_t H5Oget_info_by_name(long loc_id, String name,
                                               long lapl_id)
      throws HDF5LibraryException, NullPointerException {
    return H5Oget_info_by_name(loc_id, name, HDF5Constants.H5O_INFO_ALL,
                               lapl_id);
  }
   ...
   * @param new_loc_id
   *            IN: File or group identifier specifying location at which object
   *is to be linked.
   * @param new_name
   ...
   *            name is null.
   **/
  public synchronized static native void H5Olink(long obj_id, long new_loc_id,
                                                 String new_name, long lcpl_id,
                                                 long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5O
   *
   * H5Oopen opens a group, dataset, or named datatype specified by a location
   *and a path name.
   *
   * @param loc_id
   *            IN: File or group identifier
   ...
   *
   * @param obj_id
   *            IN: Identifier of the object at which the recursive iteration
   *begins.
   * @param idx_type
   *            IN: Type of index
   * @param order
   *            IN: Order of iteration within index
   * @param op
   *            IN: Callback function passing data regarding the object to the
   *calling application
   * @param op_data
   *            IN: User-defined pointer to data required by the application for
   *its processing of the object
   *
   * @return returns the return value of the first operator that returns a
   *positive value, or zero if all members were processed with no operator
   *returning non-zero.
   ...
   *            name is null.
   **/
  public static int H5Ovisit(long obj_id, int idx_type, int order,
                             H5O_iterate_t op, H5O_iterate_opdata_t op_data)
      throws HDF5LibraryException, NullPointerException {
   ...
   **/
  public synchronized static native int
  H5Ovisit(long obj_id, int idx_type, int order, H5O_iterate_t op,
   ...
   *            name is null.
   **/
  public static int H5Ovisit_by_name(long loc_id, String obj_name, int idx_type,
                                     int order, H5O_iterate_t op,
                                     H5O_iterate_opdata_t op_data, long lapl_id)
      throws HDF5LibraryException, NullPointerException {
    return H5Ovisit_by_name(loc_id, obj_name, idx_type, order, op, op_data,
                            HDF5Constants.H5O_INFO_ALL, lapl_id);
  }
   ...
   **/
  public synchronized static native int
  H5Ovisit_by_name(long loc_id, String obj_name, int idx_type, int order,
                   H5O_iterate_t op, H5O_iterate_opdata_t op_data, int fields,
                   long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5O
   *
   * H5Oexists_by_name is used by an application to check that an existing link
   *resolves to an object. Primarily, it is designed to check for dangling soft,
   ...
   *            name is null.
   **/
  public synchronized static native boolean H5Oexists_by_name(long loc_id,
                                                              String obj_name,
                                                              long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5O
   *
   * H5Oopen_by_token opens a group, dataset, or named datatype using its object
   *token within an HDF5 file.
   *
   * @param loc_id
   *            IN: File or group identifier
   * @param token
   ...
   *            Error from the HDF5 Library.
   **/
  public static long H5Oopen_by_token(long loc_id, H5O_token_t token)
      throws HDF5LibraryException {
  ...
  private synchronized static native long _H5Oopen_by_token(long loc_id,
                                                            H5O_token_t token)
      throws HDF5LibraryException, NullPointerException;
  ...
  private synchronized static native long
  _H5Oopen_by_idx(long loc_id, String group_name, int idx_type, int order,
                  long n, long lapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5O
   *
   * H5Oflush causes all buffers associated with an object to be immediately
   *flushed to disk without removing the data from the cache. object_id can be
   *any named object associated with an HDF5 file including a dataset, a group,
   *or a committed datatype.
   *
   * @param object_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Oflush(long object_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5O
   *
   * H5Orefresh causes all buffers associated with an object to be cleared and
   *immediately re-loaded with updated contents from disk. This function
   *essentially closes the object, evicts all metadata associated with it from
   *the cache, and then re-opens the object. The reopened object is
   *automatically re-registered with the same ID. object_id can be any named
   *object associated with an HDF5 file including a dataset, a group, or a
   *committed datatype.
   *
   * @param object_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Orefresh(long object_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5O
   *
   * H5Odisable_mdc_flushes corks an object, keeping dirty entries associated
   *with the object in the metadata cache.
   *
   * @param object_id
   ...
   * @ingroup JH5O
   *
   * H5Oenable_mdc_flushes uncorks an object, keeping dirty entries associated
   *with the object in the metadata cache.
   *
   * @param object_id
   ...
   * @ingroup JH5O
   *
   * H5Oget_native_info retrieves the native HDF5-specific metadata for an HDF5
   *object specified by an identifier. Native HDF5-specific metadata includes
   *things like object header information and object storage layout information.
   *
   * @param loc_id
   *            IN: Identifier for target object
   *
   ...
   *            name is null.
   **/
  public synchronized static native H5O_native_info_t
   ...
   * @ingroup JH5O
   *
   * H5Oget_native_info_by_idx retrieves the native HDF5-specific metadata for
   *an HDF5 object, identifying the object by an index position. Native
   *HDF5-specific metadata includes things like object header information and
   ...
   *            name is null.
   **/
  public static H5O_native_info_t
  H5Oget_native_info_by_idx(long loc_id, String group_name, int idx_type,
                            int order, long n, long lapl_id)
      throws HDF5LibraryException, NullPointerException {
    return H5Oget_native_info_by_idx(loc_id, group_name, idx_type, order, n,
                                     HDF5Constants.H5O_NATIVE_INFO_ALL,
                                     lapl_id);
  }
  ...
  public synchronized static native H5O_native_info_t
  H5Oget_native_info_by_idx(long loc_id, String group_name, int idx_type,
                            int order, long n, int fields, long lapl_id)
      throws HDF5LibraryException, NullPointerException;

  /**
   * @ingroup JH5O
   *
   * H5Oget_native_info_by_name retrieves the native HDF5-specific metadata for
   *an HDF5 object, identifying the object by location and relative name. Native
   *HDF5-specific metadata includes things like object header information and
   *object storage layout information.
   *
   * @param loc_id
  ...
  H5Oget_native_info_by_name(long loc_id, String name, long lapl_id)
      throws HDF5LibraryException, NullPointerException {
    return H5Oget_native_info_by_name(
        loc_id, name, HDF5Constants.H5O_NATIVE_INFO_ALL, lapl_id);
  }
  ...
  // H5O_token_t *token2,
  //            int *cmp_value);
  // herr_t H5Otoken_to_str(hid_t loc_id, const H5O_token_t *token, char
  // **token_str); herr_t H5Otoken_from_str(hid_t loc_id, const char *token_str,
  // H5O_token_t *token);
   ...
   * @return a true value if the property exists in the property object; false
   *     if the property does not exist;
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   */
  public synchronized static native boolean H5Pexist(long plid, String name)
      throws HDF5LibraryException;
   ...
   * @return a property list class if successful. Otherwise returns H5P_ROOT
   *(-1).
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
  ...
  private synchronized static native int _H5Pclose_class(long plid)
      throws HDF5LibraryException;
  ...
  // void *copy_data); typedef herr_t (*H5P_cls_close_func_t)(hid_t prop_id,
  // void *close_data);

  // Define property list callback function pointer types
  // typedef herr_t (*H5P_prp_cb1_t)(const char *name, size_t size, void
  // *value); typedef herr_t (*H5P_prp_cb2_t)(hid_t prop_id, const char *name,
  // size_t size, void *value); typedef H5P_prp_cb1_t H5P_prp_create_func_t;
  // typedef H5P_prp_cb2_t H5P_prp_set_func_t;
  // typedef H5P_prp_cb2_t H5P_prp_get_func_t;
  // typedef herr_t (*H5P_prp_encode_func_t)(const void *value, void **buf,
  // size_t *size); typedef herr_t (*H5P_prp_decode_func_t)(const void **buf,
  // void *value); typedef H5P_prp_cb2_t H5P_prp_delete_func_t; typedef
  // H5P_prp_cb1_t H5P_prp_copy_func_t; typedef int
  // (*H5P_prp_compare_func_t)(const void *value1, const void *value2, size_t
  // size); typedef H5P_prp_cb1_t H5P_prp_close_func_t;

  // Define property list iteration function type
  // typedef herr_t (*H5P_iterate_t)(hid_t id, const char *name, void
  // *iter_data);

  /**
   * @ingroup JH5P
   *
   * H5Pcreate_class_nocb creates an new property class with no callback
   *functions.
   *
   * @param parent_class
  ...
  private synchronized static native long
  _H5Pcreate_class_nocb(long parent_class, String name)
      throws HDF5LibraryException;

  //    public static long H5Pcreate_class(long parent_class, String name,
  //    H5P_cls_create_func_cb create_op, H5P_cls_create_func_t create_data,
  //             H5P_cls_copy_func_cb copy_op, H5P_cls_copy_func_t copy_data,
  //             H5P_cls_close_func_cb close_op, H5P_cls_close_func_t
  //             close_data) throws HDF5LibraryException {
  //        long id = _H5Pcreate_class(parent_class, name, create_op,
  //        create_data, copy_op, copy_data, close_op, close_data);
  //          if (id > 0) {
  ...
  //    }
  //
  //    private synchronized static native long _H5Pcreate_class(long
  //    parent_class, String name, H5P_cls_create_func_cb create_op,
  //    H5P_cls_create_func_t create_data,
  //            H5P_cls_copy_func_cb copy_op, H5P_cls_copy_func_t copy_data,
  //            H5P_cls_close_func_cb close_op, H5P_cls_close_func_t close_data)
  //            throws HDF5LibraryException;
  ...
  //    public synchronized static native void H5Pregister2(long plist_class,
  //    String name, long size, byte[] def_value, H5P_prp_create_func_cb
  //    prp_create, H5P_prp_set_func_cb prp_set,
  //          H5P_prp_get_func_cb prp_get, H5P_prp_delete_func_cb prp_delete,
  //          H5P_prp_copy_func_cb prp_copy, H5P_prp_compare_func_cb prp_cmp,
  ...
  // public synchronized static native void H5Pinsert2(long plist, String name,
  // long size,  byte[] value, H5P_prp_set_func_cb prp_set, H5P_prp_get_func_cb
  // prp_get,
  //      H5P_prp_delete_func_cb prp_delete, H5P_prp_copy_func_cb prp_copy,
  //      H5P_prp_compare_func_cb prp_cmp, H5P_prp_close_func_cb prp_close)
  //      throws HDF5LibraryException;
   ...
   *
   * <pre>
   *      attributes[0] =  The maximum number of attributes to be stored in
   *compact storage attributes[1] =  The minimum number of attributes to be
   ...
   * @ingroup JH5P
   *
   * H5Pset_attr_phase_change sets threshold values for attribute storage on an
   *object. These thresholds determine the point at which attribute storage
   *changes from compact storage (i.e., storage in the object header) to dense
   *storage (i.e., storage in a heap and indexed with a B-tree).
   *
   * @param ocpl_id
   *            IN: : Object (dataset or group) creation property list
   *identifier
   * @param max_compact
   ...
   *
   * @return Flags specifying whether to track and index attribute creation
   *order
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *            IN: Flags specifying whether to track and index attribute
   *creation order
   *
   * @return Returns a non-negative value if successful; otherwise returns a
   *negative value.
   ...
   * @ingroup JH5P
   *
   * H5Pset_obj_track_times sets a property in the object creation property
   *list, ocpl_id, that governs the recording of times associated with an
   *object.
   *
   * @param ocpl_id
   *            IN: Object creation property list identifier
   *
   * @param track_times
   *            IN: TRUE or FALSE, specifying whether object times are to be
   *tracked
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   * @ingroup JH5P
   *
   * H5Pmodify_filter modifies the specified FILTER in the transient or
   *permanent output filter pipeline depending on whether PLIST is a dataset
   *creation or dataset transfer property list.  The FLAGS argument specifies
   *certain general properties of the filter and is documented below. The
   *CD_VALUES is an array of CD_NELMTS integers which are auxiliary data for the
   *filter.  The integer values will be stored in the dataset object header as
   *part of the filter information. <p> The FLAGS argument is a bit vector of
   *the following fields: <p> H5Z_FLAG_OPTIONAL(0x0001) If this bit is set then
   *the filter is optional.  If the filter fails during an H5Dwrite() operation
   *then the filter is just excluded from the pipeline for the chunk for which
   *it failed; the filter will not participate in the pipeline during an
   *H5Dread() of the chunk.  If this bit is clear and the filter fails then the
   *entire I/O operation fails. If this bit is set but encoding is disabled for
   *a filter, attempting to write will generate an error. <p> Note:        This
   *function currently supports only the permanent filter pipeline.  That is,
   *PLIST_ID must be a dataset creation property list.
   *
   * @param plist
   ...
   **/
  public synchronized static native int
  H5Pmodify_filter(long plist, long filter, int flags, long cd_nelmts,
                   int[] cd_values)
      throws HDF5LibraryException, NullPointerException;
   ...
   *number, in a filter pipeline, specified by the property list with which it
   *is associated.
   *
   * @param plist
   *            IN: Property list identifier.
   * @param filter_number
   *            IN: Sequence number within the filter pipeline of the filter for
   ...
   *
   **/
  public static int H5Pget_filter(long plist, int filter_number, int[] flags,
                                  long[] cd_nelmts, int[] cd_values,
   ...
   *flags, int[] cd_nelmts, int[] cd_values, int namelen, String[] name, int[]
   *filter_config)
   *
   **/
  private synchronized static native int
  H5Pget_filter2(long plist, int filter_number, int[] flags, long[] cd_nelmts,
                 int[] cd_values, long namelen, String[] name,
                 int[] filter_config)
      throws ArrayIndexOutOfBoundsException, ArrayStoreException,
             HDF5LibraryException, NullPointerException;
   ...
   *
   **/
  public static int H5Pget_filter_by_id(long plist_id, long filter_id,
                                        int[] flags, long[] cd_nelmts,
                                        int[] cd_values, long namelen,
   ...
   *filter id, in a filter pipeline, specified by the property list with which
   *it is associated.
   *
   * @param plist_id
   *            IN: Property list identifier.
   ...
   **/
  public synchronized static native int
  H5Pget_filter_by_id2(long plist_id, long filter_id, int[] flags,
   ...
   *                      in the dataset creation property list are available
   *currently.
   *
   * @param dcpl_id
   *            IN: Property list identifier.
   ...
   * @ingroup JH5P
   *
   * H5Pget_shared_mesg_nindexes retrieves number of shared object header
   ...
   **/
  public synchronized static native int
  H5Pget_shared_mesg_nindexes(long fcpl_id) throws HDF5LibraryException;
   ...
   **/
  public synchronized static native int
  H5Pset_shared_mesg_index(long fcpl_id, int index_num, int mesg_type_flags,
                           int min_mesg_size)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_shared_mesg_phase_change retrieves shared object header message
   *phase change information.
   *
   * @param fcpl_id
   *            IN: : File creation property list identifier
   * @param size
   *            The threshold values for storage of shared object header message
   *indexes in a file.
   *
   *            <pre>
   *      size[0] =  Threshold above which storage of a shared object header
   *message index shifts from list to B-tree size[1] =  Threshold below which
   *storage of a shared object header message index reverts to list format
   * </pre>
   *
   * @return Returns a non-negative value if successful; otherwise returns a
   *negative value.
   ...
   *            IN: File creation property list identifier
   * @param max_list
   *            IN: Threshold above which storage of a shared object header
   *message index shifts from list to B-tree
   * @param min_btree
   *            IN: Threshold below which storage of a shared object header
   ...
   * @ingroup JH5P
   *
   * H5Pset_file_space_strategy sets the file space management strategy for the
   *file associated with fcpl_id to strategy. There are four strategies that
   *applications can select and they are described in the Parameters section.
   *
   * @param fcpl_id
   *            IN: File creation property list identifier
   * @param strategy
   ...
   *                        Mechanisms: free-space managers, aggregators, and
   *virtual file drivers This is the library default when not set.
   *                H5F_FSPACE_STRATEGY_PAGE
   *                        Mechanisms: free-space managers with embedded paged
   *aggregation and virtual file drivers H5F_FSPACE_STRATEGY_AGGR Mechanisms:
   *aggregators and virtual file drivers H5F_FSPACE_STRATEGY_NONE Mechanisms:
   *virtual file drivers
   * @param persist
   *            IN: True to persist free-space.
   * @param threshold
   *            IN: The free-space section threshold. The library default is 1,
   *which is to track all free-space sections. Passing a value of zero (0)
   *indicates that the value of threshold is not to be modified.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   * @exception IllegalArgumentException
   ...
   **/
  public synchronized static native void
  H5Pset_file_space_strategy(long fcpl_id, int strategy, boolean persist,
                             long threshold)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_file_space_strategy provides the means for applications to manage
   *the HDF5 file's file space strategy for their specific needs.
   *
   * @param fcpl_id
   *            IN: File creation property list identifier
   * @param persist
   *            IN/OUT: The current free-space persistence. NULL, persist not
   *queried.
   * @param threshold
   *            IN/OUT: The current free-space section threshold. NULL,
   *threshold not queried.
   *
   ...
   * @ingroup JH5P
   *
   * H5Pget_file_space_strategy_persist provides the means for applications to
   *manage the HDF5 file's file space strategy for their specific needs.
   *
   * @param fcpl_id
   *            IN: File creation property list identifier
   *
   ...
   *
   **/
  public synchronized static native boolean H5Pget_file_space_strategy_persist(
      long fcpl_id) throws HDF5LibraryException, IllegalArgumentException;

  /**
   * @ingroup JH5P
   *
   * H5Pget_file_space_strategy_threshold provides the means for applications to
   *manage the HDF5 file's file space strategy for their specific needs.
   *
   * @param fcpl_id
   *            IN: File creation property list identifier
   ...
   * @ingroup JH5P
   *
   * H5Pset_file_space_page_size retrieves the file space page size for
   *aggregating small metadata or raw data.
   *
   * @param fcpl_id
   *            IN: File creation property list identifier
   * @param page_size
   ...
   * @ingroup JH5P
   *
   * H5Pget_file_space_page_size Sets the file space page size for paged
   *aggregation.
   *
   * @param fcpl_id
   *            IN: File creation property list identifier
   ...
   * associated with the file access property list or data transfer property
   * list plid.
   *
   * @param plid
   ...
   *            Error from the HDF5 Library.
   */
  public synchronized static native long H5Pget_driver(long plid)
      throws HDF5LibraryException;
   ...
   * @param rdcc_nelmts
   *            IN/OUT: Number of elements (objects) in the raw data chunk
   *cache.
   * @param rdcc_nbytes
   ...
   **/
  public synchronized static native int
  H5Pget_cache(long plist, int[] mdc_nelmts, long[] rdcc_nelmts,
   ...
   **/
  public synchronized static native int
  H5Pset_cache(long plist, int mdc_nelmts, long rdcc_nelmts, long rdcc_nbytes,
               double rdcc_w0) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pget_mdc_config gets the initial metadata cache configuration contained
   *in a file access property list. This configuration is used when the file is
   *opened.
   *
   * @param plist_id
   *            IN: Identifier of the file access property list.
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native H5AC_cache_config_t
  H5Pget_mdc_config(long plist_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pset_mdc_config sets the initial metadata cache configuration contained
   *in a file access property list and loads it into the instance of
   ...
   *            IN: Identifier of the file access property list.
   * @param config_ptr
   *            IN: H5AC_cache_config_t, the initial metadata cache
   *configuration.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native boolean H5Pget_gc_references(long fapl_id)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_fclose_degree returns the degree for the file close behavior for a
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Pset_fclose_degree(long fapl_id,
                                                             int degree)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   **/
  public synchronized static native void H5Pset_meta_block_size(long fapl_id,
                                                                long size)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_sieve_buf_size retrieves the current settings for the data sieve
   *buffer size property from a file access property list.
   *
   * @param fapl_id
   *            IN: Identifier for property list to query.
   *
   ...
   * @ingroup JH5P
   *
   * H5Pset_sieve_buf_size Sets the maximum size of the data seive buffer used
   *for file drivers which are capable of using data sieving.  The data sieve
   ...
   *data reads and writes will occur in at least 64KB blocks. Setting the value
   *to 0 with this function will turn off the data sieving
   *
   * @param fapl_id
   *            IN: Identifier of property list to modify.
   * @param size
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Pset_sieve_buf_size(long fapl_id,
                                                               long size)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_small_data_block_size retrieves the size of a block of small data in
   ...
   **/
  public synchronized static native long
  H5Pget_small_data_block_size(long plist) throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Pset_small_data_block_size(long plist,
                                                                     long size)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_libver_bounds retrieves the lower and upper bounds on the HDF5
   *Library versions that indirectly determine the object formats versions used
   *when creating objects in the file.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   * @param libver
   *            The earliest/latest version of the library that will be used for
   *writing objects.
   *
   *            <pre>
   *      libver[0] =  The earliest version of the library that will be used for
   *writing objects libver[1] =  The latest version of the library that will be
   ...
   *
   **/
  public synchronized static native int H5Pget_libver_bounds(long fapl_id,
                                                             int[] libver)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   **/
  public synchronized static native int H5Pset_libver_bounds(long fapl_id,
                                                             int low, int high)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   **/
  public synchronized static native int
  H5Pget_elink_file_cache_size(long fapl_id) throws HDF5LibraryException;
   ...
   * @param location
   *            IN: Location of log in UTF-8/ASCII (file path/name) (On Windows,
   *this must be ASCII).
   * @param start_on_access
   *            IN: Whether the logging begins as soon as the file is opened or
   *created.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   **/
  public synchronized static native void
  H5Pset_mdc_log_options(long fapl_id, boolean is_enabled, String location,
   ...
   *             mdc_logging_options[1] = start_on_access, whether the logging
   *begins as soon as the file is opened or created
   *
   * @return the location of log in UTF-8/ASCII (file path/name) (On Windows,
   *this must be ASCII).
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   *
   **/
  public synchronized static native String H5Pget_mdc_log_options(
      long fapl_id, boolean[] mdc_log_options) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pget_metadata_read_attempts retrieves the number of read attempts that is
   *set in the file access property list plist_id.
   *
   * @param plist_id
   *            IN: File access property list identifier
   *
   ...
   **/
  public synchronized static native long
  H5Pget_metadata_read_attempts(long plist_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pset_metadata_read_attempts sets the number of reads that the library
   *will try when reading checksummed metadata in an HDF5 file opened with SWMR
   *access. When reading such metadata, the library will compare the checksum
   *computed for the metadata just read with the checksum stored within the
   *piece of checksum. When performing SWMR operations on a file, the checksum
   *check might fail when the library reads data on a system that is not atomic.
   *To remedy such situations, the library will repeatedly read the piece of
   *metadata until the check passes or finally fails the read when the allowed
   ...
   * @ingroup JH5P
   *
   * H5Pget_evict_on_close retrieves the file access property list setting that
   *determines whether an HDF5 object will be evicted from the library's
   *metadata cache when it is closed.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   *
   ...
   * @ingroup JH5P
   *
   * H5Pset_evict_on_close controls the library's behavior of evicting metadata
   *associated with a closed object.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   ...
   **/
  public synchronized static native boolean
  H5Pget_use_file_locking(long fapl_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pget_use_file_locking retrieves whether we ignore file locks when they
   *are disabled.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   ...
   **/
  public synchronized static native boolean
  H5Pget_ignore_disabled_file_locking(long fapl_id) throws HDF5LibraryException;
   ...
   *            IN: Whether the library will use file locking when opening files
   *(mainly for SWMR semantics).
   *
   * @param ignore_when_disabled
   *            IN: Whether file locking will be ignored when disabled on a file
   *system (useful for Lustre).
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   *
   **/
  public synchronized static native void
  H5Pset_file_locking(long fapl_id, boolean use_file_locking,
                      boolean ignore_when_disabled) throws HDF5LibraryException;

  //  /////  unimplemented /////
  // herr_t H5Pset_vol(hid_t plist_id, hid_t new_vol_id, const void
  // *new_vol_info); herr_t H5Pget_vol_id(hid_t plist_id, hid_t *vol_id); herr_t
   ...
   *            max_ndims &lt;=0
   **/
  public synchronized static native int H5Pget_chunk(long plist, int max_ndims,
                                                     long[] dims)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   * virtual dataspace identifier vspace_id to the elements of the source
   *dataset described by the source dataset dataspace identifier src_space_id.
   *The source dataset is identified by the name of the file where it is
   *located, src_file_name, and the name of the dataset, src_dset_name.
   *
   * @param dcpl_id
   *            IN: The identifier of the dataset creation property list that
   ...
   *located. The file might not exist yet. The name can be specified using a
   *C-style printf statement.
   * @param src_dset_name
   *            IN: The path to the HDF5 dataset in the file specified by
   *src_file_name. The dataset might not exist yet. The dataset name can be
   *specified using a C-style printf statement.
   * @param src_space_id
   *            IN: The source dataset dataspace identifier with a selection
   ...
   **/
  public synchronized static native void
  H5Pset_virtual(long dcpl_id, long vspace_id, String src_file_name,
   ...
   * @ingroup JH5P
   *
   * H5Pget_virtual_count gets the number of mappings for a virtual dataset that
   *has the creation property list specified by dcpl_id.
   *
   * @param dcpl_id
   *            IN: The identifier of the virtual dataset creation property
   *list.
   *
   ...
   * @ingroup JH5P
   *
   * H5Pget_virtual_vspace takes the dataset creation property list for the
   *virtual dataset, dcpl_id, and the mapping index, index, and returns a
   *dataspace identifier for the selection within the virtual dataset used in
   *the mapping.
   *
   * @param dcpl_id
   *            IN: The identifier of the virtual dataset creation property
   ...
   *            An id is &lt;=0
   **/
  public synchronized static native long H5Pget_virtual_vspace(long dcpl_id,
                                                               long index)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_virtual_srcspace takes the dataset creation property list for the
   *virtual dataset, dcpl_id, and the mapping index, index, and returns a
   *dataspace identifier for the selection within the source dataset used in the
   *mapping.
   *
   * @param dcpl_id
   *            IN: The identifier of the virtual dataset creation property
   ...
   *            An id is &lt;=0
   **/
  public synchronized static native long H5Pget_virtual_srcspace(long dcpl_id,
                                                                 long index)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_virtual_filename takes the dataset creation property list for the
   *virtual dataset, dcpl_id, the mapping index, index, the size of the filename
   *for a source dataset, size, and retrieves the name of the file for a source
   ...
   *            An id is &lt;=0
   **/
  public synchronized static native String H5Pget_virtual_filename(long dcpl_id,
                                                                   long index)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_virtual_dsetname takes the dataset creation property list for the
   *virtual dataset, dcpl_id, the mapping index, index, the size of the dataset
   *name for a source dataset, size, and retrieves the name of the source
   *dataset used in the mapping.
   *
   * @param dcpl_id
   *            IN: The identifier of the virtual dataset creation property
   ...
   *            An id is &lt;=0
   **/
  public synchronized static native String H5Pget_virtual_dsetname(long dcpl_id,
                                                                   long index)
      throws HDF5LibraryException, IllegalArgumentException;

  //    /////  unimplemented /////
  //    /**
  //     * H5Pget_vds_file_cache_size retrieves the size of the vds link open
  //     file cache.
  //     *
  //     * @param fapl_id
  //     *            IN: File access property list identifier
  ...
  //     *
  //     **/
  //    public synchronized static native int H5Pget_vds_file_cache_size(long
  //    fapl_id) throws HDF5LibraryException;
  //
  //    /**
  //     * H5Pset_vds_file_cache_size sets the number of files that can be held
  //     open in an vds link open
  //     * file cache.
  //     *
  //     * @param fapl_id
  //     *            IN: File access property list identifier
  ...
  //     *
  //     **/
  //    public synchronized static native void H5Pset_vds_file_cache_size(long
  //    fapl_id, int efc_size)
  //            throws HDF5LibraryException;
   ...
   **/
  public synchronized static native int
  H5Pget_external(long plist, int idx, long name_size, String[] name,
                  long[] size)
      throws ArrayIndexOutOfBoundsException, ArrayStoreException,
             HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   *            name is null.
   **/
  public synchronized static native int H5Pset_external(long plist, String name,
   ...
   * @ingroup JH5P
   *
   * H5Pset_fill_value checks if the fill value is defined for a dataset
   ...
   * @ingroup JH5P
   *
   * H5Pset_chunk_opts Sets the edge chunk option in a dataset creation property
   *list.
   *
   * @param dcpl_id
   *            IN: Dataset creation property list identifier
   ...
   *                H5D_CHUNK_DONT_FILTER_PARTIAL_CHUNKS - filters are not
   *applied to partial edge chunks. 0 - Disables option; partial edge chunks
   *will be compressed.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library
   **/
  public synchronized static native void H5Pset_chunk_opts(long dcpl_id,
                                                           int opts)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_chunk_opts retrieves the edge chunk option setting stored in the
   ...
   *
 */
  public synchronized static native int H5Pget_chunk_opts(long dcpl_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pget_dset_no_attrs_hint accesses the flag for whether or not datasets
   *created by the given dcpl will be created with a "minimized" object header.
   *
   * @param dcpl_id
   *            IN: Dataset creation property list
   *
   * @return true if the given dcpl is set to create minimized dataset object
   *headers, false if not.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   **/
  public synchronized static native boolean
  H5Pget_dset_no_attrs_hint(long dcpl_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pset_dset_no_attrs_hint sets the dcpl to minimize (or explicitly to not
   *minimized) dataset object headers upon creation.
   *
   * @param dcpl_id
   *            IN: Dataset creation property list
   *
   ...
   **/
  public synchronized static native void
  H5Pget_chunk_cache(long dapl_id, long[] rdcc_nslots, long[] rdcc_nbytes,
                     double[] rdcc_w0)
      throws HDF5LibraryException, NullPointerException;
   ...
   **/
  public synchronized static native void
  H5Pset_chunk_cache(long dapl_id, long rdcc_nslots, long rdcc_nbytes,
                     double rdcc_w0) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pset_virtual_view takes the access property list for the virtual dataset,
   *dapl_id, and the flag, view, and sets the VDS view according to the flag
   *value.
   *
   * @param dapl_id
   *            IN: Dataset access property list identifier for the virtual
   *dataset
   * @param view
   *            IN: Flag specifying the extent of the data to be included in the
   *view.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library
   **/
  public synchronized static native void H5Pset_virtual_view(long dapl_id,
                                                             int view)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_virtual_view takes the virtual dataset access property list,
   dapl_id, and retrieves the flag,
   ...
   *
 */
  public synchronized static native int H5Pget_virtual_view(long dapl_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pset_virtual_printf_gap sets the access property list for the virtual
   *dataset, dapl_id, to instruct the library to stop looking for the mapped
   *data stored in the files and/or datasets with the printf-style names after
   *not finding gap_size files and/or datasets. The found source files and
   * datasets will determine the extent of the unlimited virtual dataset with
   *the printf-style mappings.
   *
   * @param dapl_id
   *            IN: Dataset access property list identifier for the virtual
   *dataset
   * @param gap_size
   *            IN: Maximum number of files and/or datasets allowed to be
   *missing for determining the extent of an unlimited virtual dataset with
   ...
   * @ingroup JH5P
   *
   * H5Pget_virtual_printf_gap returns the maximum number of missing
   printf-style files and/or datasets for
   * determining the extent of an unlimited virtual dataaset, gap_size, using
   the access property list for
   ...
   * @return Maximum number of files and/or datasets allowed to be missing for
   determining
   *            the extent of an unlimited virtual dataset with printf-style
   mappings.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library
 ...
 */
   ...
   *
   **/
  public synchronized static native void H5Pset_virtual_prefix(long dapl_id,
                                                               String prefix)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   **/
  public synchronized static native void H5Pset_efile_prefix(long dapl_id,
                                                             String prefix)
      throws HDF5LibraryException, NullPointerException;

  // public synchronized static native void H5Pset_append_flush(long plist_id,
  // int ndims, long[] boundary, H5D_append_cb func, H5D_append_t udata) throws
  // HDF5LibraryException;

  // public synchronized static native void H5Pget_append_flush(long plist_id,
  // int dims, long[] boundary, H5D_append_cb func, H5D_append_t udata) throws
  // HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_data_transform retrieves the data transform expression previously
   *set in the dataset transfer property list plist_id by H5Pset_data_transform.
   *
   * @param plist_id
   ...
   *
   **/
  public synchronized static native int H5Pset_data_transform(long plist_id,
                                                              String expression)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_buffer gets type conversion and background buffers. Returns buffer
   ...
   *            plist is invalid.
   **/
  public synchronized static native int H5Pget_buffer(long plist, byte[] tconv,
                                                      byte[] bkg)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_buffer_size gets type conversion and background buffer size, in
   *bytes, if successful; otherwise 0 on failure.
   *
   * @param plist
   *            Identifier for the dataset transfer property list.
   *
   ...
   *pointers to application-allocated buffers. If the buffer size is smaller
   *than the entire amount of data being transferred between the application and
   *the file, and a type conversion buffer or background buffer is required,
   *then strip mining will be used.
   *
   ...
   *data elements. When strip mining a 100x200x300x150 hyperslab of a simple
   *data space, the buffer must be large enough to hold 1x200x300x150 data
   *elements.
   *
   * @param plist
   *            Identifier for the dataset transfer property list.
   ...
   *            plist is invalid.
   **/
  public synchronized static native void H5Pset_buffer_size(long plist,
                                                            long size)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   **/
  public synchronized static native int
  H5Pget_btree_ratios(long plist_id, double[] left, double[] middle,
                      double[] right)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_hyper_vector_size reads values previously set with
   ...
   * @ingroup JH5P
   *
   * H5Pget_create_intermediate_group determines whether property is set to
   ...
   **/
  public synchronized static native boolean
  H5Pget_create_intermediate_group(long lcpl_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pset_create_intermediate_group specifies in property list whether to
   *create missing intermediate groups
   *
   * @param lcpl_id
   *            IN: Link creation property list identifier
   * @param crt_intermed_group
   *            IN: Flag specifying whether to create intermediate groups upon
   ...
   * @ingroup JH5P
   *
   * H5Pget_local_heap_size_hint Retrieves the anticipated size of the local
   *heap for original-style groups.
   *
   * @param gcpl_id
   ...
   **/
  public synchronized static native long
  H5Pget_local_heap_size_hint(long gcpl_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pset_local_heap_size_hint Specifies the anticipated maximum size of a
   *local heap.
   *
   * @param gcpl_id
   *            IN: Group creation property list identifier
   ...
   * @ingroup JH5P
   *
   * H5Pget_link_phase_change Queries the settings for conversion between
   *compact and dense groups.
   *
   * @param gcpl_id
   *            IN: Group creation property list identifier
   * @param links
   *            The max. no. of compact links &amp; the min. no. of dense links,
   ...
   *
   **/
  public synchronized static native int H5Pget_link_phase_change(long gcpl_id,
                                                                 int[] links)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_est_link_info Queries data required to estimate required local heap
   *or object header size.
   *
   * @param gcpl_id
   *            IN: Group creation property list identifier
   * @param link_info
   *            Estimated number of links to be inserted into group And the
   ...
   *
   **/
  public synchronized static native int H5Pget_est_link_info(long gcpl_id,
                                                             int[] link_info)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5P
   *
   * H5Pset_est_link_info Sets estimated number of links and length of link
   *names in a group.
   *
   * @param gcpl_id
   *            IN: Group creation property list identifier
   ...
   *gcpl_id, and returns a flag indicating whether link creation order is
   *tracked and/or indexed in a group.
   *
   * @param gcpl_id
   *            IN: Group creation property list identifier
   *
   ...
   *
   **/
  public synchronized static native void H5Pset_char_encoding(long plist_id,
                                                              int encoding)
      throws HDF5LibraryException;
   ...
   *traversals allowed, nlinks, before the library assumes it has found a cycle
   *and aborts the traversal. This value is set in the link access property list
   *lapl_id.
   *
   * @param lapl_id
   *            IN: File access property list identifier
   ...
   *
   **/
  public synchronized static native long H5Pget_elink_prefix(long lapl_id,
                                                             String[] prefix)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   **/
  public synchronized static native int H5Pset_elink_prefix(long lapl_id,
                                                            String prefix)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   **/
  public synchronized static native int H5Pset_elink_fapl(long lapl_id,
                                                          long fapl_id)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pget_elink_acc_flags retrieves the external link traversal file access
   *flag from the specified link access property list.
   *
   * @param lapl_id
   *            IN: Link access property list identifier
   *
   ...
   * @ingroup JH5P
   *
   * H5Pset_elink_acc_flags Sets the external link traversal file access flag in
   ...
   *
   **/
  public synchronized static native int H5Pset_elink_acc_flags(long lapl_id,
                                                               int flags)
      throws HDF5LibraryException, IllegalArgumentException;
   ...
   *
   **/
  public synchronized static native void H5Pset_copy_object(long ocp_plist_id,
                                                            int copy_options)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5P
   *
   * H5Pset_fapl_core modifies the file access property list to use the
   *H5FD_CORE driver.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   ...
   *                 info[1] = backing_store - write to file name on flush
   *setting
   *
   * @return a non-negative value if successful; otherwise returns a negative
   *value.
   ...
   *
   **/
  public synchronized static native int H5Pget_fapl_direct(long fapl_id,
                                                           long[] info)
      throws HDF5LibraryException;
   ...
   **/
  public synchronized static native int
  H5Pset_fapl_direct(long fapl_id, long alignment, long block_size,
                     long cbuf_size) throws HDF5LibraryException;

  /**
   * @ingroup JH5P
   *
   * H5Pget_fapl_family Returns information about the family file access
   *property list.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   * @param memb_size
   *            OUT: the size in bytes of each file member (used only when
   *creating a new file)
   * @param memb_fapl_id
   *            OUT: the file access property list to be used for each family
   *member
   *
   * @return a non-negative value if successful; otherwise returns a negative
   *value.
   ...
   *            IN: File access property list identifier
   * @param memb_size
   *            IN: the size in bytes of each file member (used only when
   *creating a new file)
   * @param memb_fapl_id
   *            IN: the file access property list to be used for each family
   *member
   *
   * @return a non-negative value if successful; otherwise returns a negative
   *value.
   ...
   * @ingroup JH5P
   *
   * H5Pset_fapl_hdfs Modify the file access property list to use the H5FD_HDFS
   *driver.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   ...
   *
   **/
  public synchronized static native H5FD_hdfs_fapl_t H5Pget_fapl_hdfs(
      long fapl_id) throws HDF5LibraryException, NullPointerException;
   ...
   * @param memb_addr
   *            IN: The offsets within the virtual address space, from 0 (zero)
   *to HADDR_MAX, at which each type of data storage begins.
   *
   * @return a boolean value; Allows read-only access to incomplete file sets
   *when TRUE.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *
   **/
  public synchronized static native boolean H5Pget_fapl_multi(
      long fapl_id, int[] memb_map, long[] memb_fapl, String[] memb_name,
      long[] memb_addr) throws HDF5LibraryException, NullPointerException;
   ...
   **/
  public synchronized static native void
  H5Pset_fapl_multi(long fapl_id, int[] memb_map, long[] memb_fapl,
                    String[] memb_name, long[] memb_addr, boolean relax)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            IN: File access property list identifier.
   * @param logfile
   *            IN: logfile is the name of the file in which the logging entries
   *are to be recorded.
   * @param flags
   ...
   * @ingroup JH5P
   *
   * H5Pset_fapl_split Sets up use of the split I/O driver. Makes the multi
   *driver act like the old split driver which stored meta data in one file and
   *raw data in another file
   *
   * @param fapl_id
   *            IN: File access property list identifier
   * @param meta_ext
   ...
   **/
  public synchronized static native void
  H5Pset_fapl_split(long fapl_id, String meta_ext, long meta_plist_id,
                    String raw_ext, long raw_plist_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5P
   *
   * H5Pset_fapl_ros3 Modify the file access property list to use the H5FD_ROS3
   *driver.
   *
   * @param fapl_id
   *            IN: File access property list identifier
   ...
   *
   **/
  public synchronized static native H5FD_ros3_fapl_t H5Pget_fapl_ros3(
      long fapl_id) throws HDF5LibraryException, NullPointerException;
  ...
  // *new_driver_info) const void *H5Pget_driver_info(hid_t plist_id) herr_t
  // H5Pget_multi_type(hid_t fapl_id, H5FD_mem_t *type) herr_t
  // H5Pset_multi_type(hid_t fapl_id, H5FD_mem_t type) herr_t
  ...
  // H5FD_file_image_callbacks_t *callbacks_ptr); herr_t
  // H5Pset_file_image_callbacks(hid_t fapl_id, H5FD_file_image_callbacks_t
  // *callbacks_ptr); herr_t H5Pset_core_write_tracking(hid_t fapl_id, hbool_t
  // is_enabled, size_t page_size); herr_t H5Pget_core_write_tracking(hid_t
  // fapl_id, hbool_t *is_enabled, size_t *page_size); #ifdef H5_HAVE_PARALLEL
  // herr_t H5Pset_all_coll_metadata_ops(hid_t accpl_id, hbool_t is_collective);
  //  herr_t H5Pget_all_coll_metadata_ops(hid_t plist_id, hbool_t
  //  *is_collective);
  ...
  // #endif /* H5_HAVE_PARALLEL */
  //  herr_t H5Pset_mdc_image_config(hid_t plist_id, H5AC_cache_image_config_t
  //  *config_ptr); herr_t H5Pget_mdc_image_config(hid_t plist_id,
  //  H5AC_cache_image_config_t *config_ptr /*out*/); herr_t
  //  H5Pset_page_buffer_size(hid_t plist_id, size_t buf_size, unsigned
  //  min_meta_per, unsigned min_raw_per);
  // herr_t H5Pget_page_buffer_size(hid_t fapl_id, size_t *buf_size, unsigned
  // *min_meta_perc, unsigned *min_raw_perc); herr_t H5Pset_object_flush_cb
  // (hid_t fapl_id, H5F_flush_cb_t func, void *user_data); herr_t
  // H5Pget_object_flush_cb (hid_t fapl_id, H5F_flush_cb_t *func, void
  // **user_data);
  ...
  // boundary[], H5D_append_cb_t func, void *user_data); herr_t
  // H5Pget_append_flush(hid_t dapl_id, int ndims, hsize_t boundary[],
  ...
  // *op_data) herr_t H5Pget_vlen_mem_manager(hid_t plist, H5MM_allocate_t
  // *alloc, void **alloc_info, H5MM_free_t *free, void
  // **free_info )
  // herr_t H5Pset_vlen_mem_manager(hid_t plist, H5MM_allocate_t alloc, void
  // *alloc_info, H5MM_free_t free, void *free_info ) herr_t
  // H5Pget_type_conv_cb(hid_t plist, H5T_conv_except_func_t *func, void
  // **op_data) herr_t H5Pset_type_conv_cb( hid_t plist, H5T_conv_except_func_t
  // func, void *op_data) #ifdef H5_HAVE_PARALLEL
  //  herr_t H5Pget_mpio_actual_chunk_opt_mode(hid_t plist_id,
  //  H5D_mpio_actual_chunk_opt_mode_t *actual_chunk_opt_mode); herr_t
  //  H5Pget_mpio_actual_io_mode(hid_t plist_id, H5D_mpio_actual_io_mode_t
  //  *actual_io_mode); herr_t H5Pget_mpio_no_collective_cause(hid_t plist_id,
  //  uint32_t *local_no_collective_cause, uint32_t
  ...
  // **op_data ) herr_t H5Pset_elink_cb( hid_t lapl_id, H5L_elink_traverse_t
  // func, void *op_data )
  ...
  // void **op_data); herr_t H5Pset_mcdt_search_cb(hid_t plist_id,
  // H5O_mcdt_search_cb_t func, void *op_data);
   ...
   * @ingroup JH5PL
   *
   * H5PLset_loading_state uses one argument to enable or disable individual
   *plugins. The plugin_flags parameter is an encoded integer in which each bit
   *controls a specific plugin or class of plugins. A plugin bit set to 0 (zero)
   *prevents the use of the dynamic plugin corresponding to that bit position. A
   *plugin bit set to 1 (one) allows the use of that dynamic plugin. All dynamic
   *plugins can be enabled by setting plugin_flags to a negative value. A value
   *of 0 (zero) will disable all dynamic plugins.
   *
   *        H5PLset_loading_state inspects the HDF5_PLUGIN_PRELOAD environment
   *variable every time it is called. If the environment variable is set to the
   *special :: string, all dynamic plugins will be disabled.
   *
   * @param plugin_flags
   *            IN: The list of dynamic plugin types to enable or disable.
   *                A plugin bit set to 0 (zero) prevents use of that dynamic
   *plugin. A plugin bit set to 1 (one) enables use of that dynamic plugin.
   *                Setting plugin_flags to a negative value enables all dynamic
   *plugins. Setting plugin_flags to 0 (zero) disables all dynamic plugins.
   *
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   * @ingroup JH5PL
   *
   * H5PLget_loading_state retrieves the state of the dynamic plugins flag,
   *plugin_flags..
   *
   * @return the list of dynamic plugin types that are enabled or disabled.
   *             A plugin bit set to 0 (zero) indicates that that dynamic plugin
   *is disabled. A plugin bit set to 1 (one) indicates that that dynamic plugin
   *is enabled. If the value of plugin_flags is negative, all dynamic plugins
   *are enabled. If the value of plugin_flags is 0 (zero), all dynamic plugins
   *are disabled.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5PLget_loading_state()
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5PLappend(String plugin_path)
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5PLprepend(String plugin_path)
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5PLreplace(String plugin_path,
                                                     int index)
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5PLinsert(String plugin_path,
                                                    int index)
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5PLremove(int index)
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native String H5PLget(int index)
      throws HDF5LibraryException;
  ...
  private synchronized static native int
  H5Rcreate(byte[] ref, long loc_id, String name, int ref_type, long space_id)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   *
   * @param loc_id
   *            IN: Location identifier used to locate the object being pointed
   *to.
   * @param name
   ...
   **/
  public static long H5Rdereference(long dataset, long access_list,
  ...
  private synchronized static native long
  _H5Rdereference(long dataset, long access_list, int ref_type, byte[] ref)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   *
   * @param loc_id
   *            IN: Identifier for the dataset containing the reference or for
   *the group that dataset is in.
   * @param ref_type
   *            IN: Type of reference.
   * @param ref
   *            IN: An object or dataset region reference.
   * @param name
   *            OUT: A name associated with the referenced object or dataset
   *region.
   * @param size
   *            IN: The size of the name buffer.
   *
   * @return Returns the length of the name if successful, returning 0 (zero) if
   *no name is associated with the identifier. Otherwise returns a negative
   *value.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *            IN: An object or dataset region reference.
   *
   * @return Returns the name if successful, returning null if no name is
   ...
   *            Argument is illegal.
   **/
  public synchronized static native String H5Rget_name_string(long loc_id,
                                                              int ref_type,
                                                              byte[] ref)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   * @ingroup JH5R
   *
   * H5Rget_obj_type2 Retrieves the type of object that an object reference
   *points to.
   *
   * @see public static int H5Rget_obj_type(int loc_id, int ref_type, byte
   *ref[])
   **/
  private synchronized static native int
  H5Rget_obj_type2(long loc_id, int ref_type, byte ref[], int[] obj_type)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
  ...
  private synchronized static native long
  _H5Rget_region(long loc_id, int ref_type, byte[] ref)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
  ...
  public synchronized static native
      byte[] H5Rcreate_object(long loc_id, String name, long access_id)
          throws HDF5LibraryException, NullPointerException,
                 IllegalArgumentException;
   ...
   * @ingroup JH5R
   *
   * H5Rcreate_region creates the reference, pointing to the region represented
   *by space id within the object named name located at loc id.
   *
   * @param loc_id
   *            IN: Location identifier used to locate the object being pointed
   *to.
   ...
   *            IN: Name of object at location loc_id.
   * @param space_id
   *            IN: Identifies the dataset region that a dataset region
   *reference points to.
   * @param access_id
   *            IN: Object access identifier to the object being pointed to.
   *
   ...
   **/
  public synchronized static native
      byte[] H5Rcreate_region(long loc_id, String name, long space_id,
                              long access_id)
          throws HDF5LibraryException, NullPointerException,
                 IllegalArgumentException;
   ...
   * @ingroup JH5R
   *
   * H5Rcreate_attr creates the reference, pointing to the attribute named attr
   *name and attached to the object named name located at loc id.
   *
   * @param loc_id
   *            IN: Location identifier used to locate the object being pointed
   *to.
   ...
   **/
  public synchronized static native
      byte[] H5Rcreate_attr(long loc_id, String name, String attr_name,
                            long access_id)
          throws HDF5LibraryException, NullPointerException,
                 IllegalArgumentException;
   ...
   *
   * @param ref_ptr
   *            IN: Reference to an object, region or attribute attached to an
   *object.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *object.
   *
   * @return a valid reference type if successful; otherwise returns H5R
   *UNKNOWN.
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   * @ingroup JH5R
   *
   * H5Requal determines whether two references point to the same object, region
   *or attribute.
   *
   * @param ref1_ptr
   *            IN: Reference to an object, region or attribute attached to an
   ...
   *            an input array is invalid.
   **/
  public synchronized static native boolean H5Requal(byte[] ref1_ptr,
                                                     byte[] ref2_ptr)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   *function such as H5Oclose or H5Dclose for datasets.
   *
   * @param ref_ptr
   *            IN: Reference to an object, region or attribute attached to an
   *object.
   * @param rapl_id
   *            IN: A reference access property list identifier for the
   *reference. The access property list can be used to access external files
   *that the reference points to (through a file access property list).
   * @param oapl_id
   *            IN: An object access property list identifier for the reference.
   *The access property property list must be of the same type as the object
   *being referenced, that is a group or dataset property list.
   *
   * @return a valid identifier if successful
   *
   * @exception HDF5LibraryException
  ...
  private synchronized static native long
  _H5Ropen_object(byte[] ref_ptr, long rapl_id, long oapl_id)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   * @ingroup JH5R
   *
   * H5Ropen region creates a copy of the dataspace of the dataset pointed to by
   *a region reference, ref ptr, and defines a selection matching the selection
   *pointed to by ref ptr within the dataspace copy. Use H5Sclose to release the
   *dataspace identifier returned by this function when the identifier is no
   *longer needed.
   *
   * @param ref_ptr
   *            IN: Reference to an object, region or attribute attached to an
  ...
  private synchronized static native long
  _H5Ropen_region(byte[] ref_ptr, long rapl_id, long oapl_id)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   * @ingroup JH5R
   *
   * H5Ropen_attr opens the attribute attached to the object and returns an
   *identifier. The attribute opened with this function should be closed with
   *H5Aclose when it is no longer needed so that resource leaks will not
   *develop.
   *
   * @param ref_ptr
   *            IN: Reference to an object, region or attribute attached to an
   ...
   *that the reference points to (through a file access property list).
   * @param aapl_id
   *            IN: An attribute access property list identifier for the
   *reference. The access property property list must be of the same type as the
   *object being referenced, that is a group or dataset property list.
   *
  ...
  private synchronized static native long
  _H5Ropen_attr(byte[] ref_ptr, long rapl_id, long aapl_id)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   *            array is invalid.
   **/
  public synchronized static native int H5Rget_obj_type3(byte[] ref_ptr,
                                                         long rapl_id)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   * @ingroup JH5R
   *
   * H5Rget_file_name retrieves the file name for the object, region or
   *attribute reference pointed to.
   *
   * @param ref_ptr
   *            IN: Reference to an object, region or attribute attached to an
   *object.
   ...
   * @ingroup JH5R
   *
   * H5Rget_obj_name retrieves the object name for the object, region or
   *attribute reference pointed to.
   *
   * @param ref_ptr
   *            IN: Reference to an object, region or attribute attached to an
   ...
   *            array is invalid.
   **/
  public synchronized static native String H5Rget_obj_name(byte[] ref_ptr,
                                                           long rapl_id)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   * @ingroup JH5R
   *
   * H5Rget_attr_name retrieves the attribute name for the object, region or
   *attribute reference pointed to.
   *
   * @param ref_ptr
   *            IN: Reference to an object, region or attribute attached to an
  ...
  private synchronized static native long
  _H5Screate_simple(int rank, long[] dims, long[] maxdims)
      throws HDF5Exception, NullPointerException;
   ...
   **/
  public synchronized static native long
  H5Sset_extent_simple(long space_id, int rank, long[] current_size,
                       long[] maximum_size)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static long H5Sset_extent_simple(long space_id, int rank,
                                                       byte[] current_size,
                                                       byte[] maximum_size)
      throws HDF5LibraryException, NullPointerException {
   ...
   **/
  public synchronized static native long
  H5Sget_simple_extent_npoints(long space_id) throws HDF5LibraryException;
   ...
   **/
  public synchronized static native int
  H5Sget_simple_extent_ndims(long space_id) throws HDF5LibraryException;
   ...
   *
   * @param space_id
   *            The identifier for the dataspace from which the extent is to be
   *removed.
   *
   * @return a non-negative value if successful
   *
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native boolean H5Sextent_equal(
      long first_space_id, long second_space_id) throws HDF5LibraryException;
   ...
   * @ingroup JH5S
   *
   * H5Sget_select_type retrieves the type of selection currently defined for
   ...
   * @ingroup JH5S
   *
   * H5Sselect_copy copies all the selection information (including offset) from
   *the source dataspace to the destination dataspace.
   *
   * @param dst_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Sselect_copy(long dst_id,
                                                        long src_id)
      throws HDF5LibraryException;
   ...
   *
   * @param space_id
   *            The identifier for the dataspace in which the selection is being
   *reset.
   *
   ...
   *            offset is null.
   **/
  public synchronized static native void H5Sselect_adjust(long space_id,
                                                          long[][] offset)
      throws HDF5LibraryException, NullPointerException;
   ...
   *<p> The start and end buffers must be large enough to hold the dataspace
   *rank number of coordinates.
   *
   * @param space_id
   *            Identifier of dataspace to release.
   * @param start
   ...
   * @ingroup JH5S
   *
   * H5Sselect_shape_same checks to see if the current selection in the
   *dataspaces are the same dimensionality and shape. This is primarily used for
   *reading the entire selection in one swoop.
   *
   * @param   space1_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native boolean H5Sselect_shape_same(long space1_id,
                                                                 long space2_id)
      throws HDF5LibraryException;
   ...
   *
   * @return a TRUE  if the current selection in the dataspace intersects with
   *the block given FALSE otherwise
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   * @exception NullPointerException
   ...
   **/
  private synchronized static native int
  H5Sselect_elements(long space_id, int op, int num_elements, byte[] coord)
      throws HDF5LibraryException, NullPointerException;
   ...
   **/
  public synchronized static native long
  H5Sget_select_elem_npoints(long spaceid) throws HDF5LibraryException;
   ...
   **/
  public synchronized static native int
  H5Sget_select_elem_pointlist(long spaceid, long startpoint, long numpoints,
                               long[] buf)
      throws HDF5LibraryException, NullPointerException;
   ...
   *selected region for the dataspace specified by space_id. The start, stride,
   *count, and block arrays must be the same size as the rank of the dataspace.
   *
   * @param space_id
   ...
   **/
  public synchronized static int
  H5Sselect_hyperslab(long space_id, int op, byte[] start, byte[] stride,
    ...
    return H5Sselect_hyperslab(space_id, op, lastart, lastride, lacount,
                               lablock);
  }
   ...
   **/
  public synchronized static native int
  H5Sselect_hyperslab(long space_id, int op, long[] start, long[] stride,
   ...
   * @ingroup JH5S
   *
   * H5Scombine_hyperslab combines a hyperslab selection with the current
   *selection for a dataspace, creating a new dataspace to return the generated
   *selection. If the current selection is not a hyperslab, it is freed and the
   *hyperslab parameters passed in are combined with the H5S_SEL_ALL hyperslab
   *(ie. a selection composing the entire current extent).  If STRIDE or BLOCK
   *is NULL, they are assumed to be set to all '1'.
   *
   * @param space_id
   ...
   **/
  public synchronized static native long
  H5Scombine_hyperslab(long space_id, int op, long[] start, long[] stride,
                       long[] count, long[] block)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   * @ingroup JH5S
   *
   * H5Smodify_select refine an existing hyperslab selection with an operation,
   *using a second hyperslab. The first selection is modified to contain the
   *result of space1 operated on by space2.
   *
   * @param space1_id
   *            ID of the destination dataspace
   * @param op
   ...
   * @ingroup JH5S
   *
   * H5Scombine_select combines two existing hyperslab selections with an
   *operation, returning a new dataspace with the resulting selection.  The
   *dataspace extent from space1 is copied for the dataspace extent of the newly
   *created dataspace.
   *
   * @param space1_id
   ...
   * @ingroup JH5S
   *
   * H5Sis_regular_hyperslab retrieves a regular hyperslab selection for the
   ...
   **/
  public synchronized static native boolean
  H5Sis_regular_hyperslab(long space_id) throws HDF5LibraryException;

  /**
   * @ingroup JH5S
   *
   * H5Sget_regular_hyperslab determines if a hyperslab selection is regular for
   *the dataspace specified by space_id. The start, stride, count, and block
   *arrays must be the same size as the rank of the dataspace.
   *
   * @param space_id
   *            IN: Identifier of dataspace selection to modify
   * @param start
   ...
   **/
  public synchronized static native void
  H5Sget_regular_hyperslab(long space_id, long[] start, long[] stride,
                           long[] count, long[] block)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   **/
  public synchronized static native long
  H5Sget_select_hyper_nblocks(long spaceid) throws HDF5LibraryException;

  /**
   * @ingroup JH5S
   *
   * H5Sget_select_hyper_blocklist returns an array of hyperslab blocks. The
   *block coordinates have the same dimensionality (rank) as the dataspace they
   ...
   **/
  public synchronized static native int
  H5Sget_select_hyper_blocklist(long spaceid, long startblock, long numblocks,
                                long[] buf)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5S
   *
   * H5Sselect_project_intersection projects the intersection of the selections
   *of src_space_id and src_intersect_space_id within the selection of
   *src_space_id as a selection within the selection of dst_space_id.
   *
   * @param src_space_id
   *            Selection that is mapped to dst_space_id, and intersected with
   *src_intersect_space_id
   * @param dst_space_id
   *            Selection that is mapped to src_space_id
   * @param src_intersect_space_id
   *            Selection whose intersection with src_space_id is projected to
   *dst_space_id to obtain the result
   *
   ...
   *dst_space on success
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   **/
  public synchronized static native long
  H5Sselect_project_intersection(long src_space_id, long dst_space_id,
  ...
  // public synchronized static native     H5Ssel_iter_create(hid_t spaceid,
  // size_t elmt_size, unsigned flags); public synchronized static native
  // H5Ssel_iter_get_seq_list(hid_t sel_iter_id, size_t maxseq, size_t maxbytes,
  // size_t *nseq,
  //                                        size_t *nbytes, hsize_t *off, size_t
  //                                        *len);
  // public synchronized static native    H5Ssel_iter_reset(hid_t sel_iter_id,
  // hid_t space_id); public synchronized static native H5Ssel_iter_close(hid_t
  // sel_iter_id);
  ...
  private synchronized static native long
  _H5Tarray_create2(long base_id, int ndims, long[] dim)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            name is null.
   **/
  public synchronized static native void H5Tcommit(long loc_id, String name,
                                                   long type_id, long lcpl_id,
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Tcompiler_conv(long src_id,
                                                          long dst_id)
      throws HDF5LibraryException;
   ...
   **/
  public synchronized static native void
  H5Tconvert(long src_id, long dst_id, long nelmts, byte[] buf,
             byte[] background, long plist_id)
      throws HDF5LibraryException, NullPointerException;

  // int H5Tconvert(int src_id, int dst_id, long nelmts, Pointer buf, Pointer
  // background, int plist_id);
   ...
   *identifier, a predefined datatype (defined in H5Tpublic.h), or a dataset
   *Identifier.
   *
   * @return a datatype identifier if successful
   *
   ...
   * @ingroup JH5T
   *
   * H5Tcreate creates a new datatype of the specified class with the specified
   *number of bytes.
   *
   * @param tclass
   ...
   *
   * @return true if the datatype specified in dtype_id contains any datatypes
   *of the datatype class
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   **/
  public synchronized static native boolean H5Tdetect_class(long type_id,
                                                            int cls)
      throws HDF5LibraryException;
   ...
   *            IN: Identifier of the object to be encoded.
   * @param buf
   *            OUT: Buffer for the object to be encoded into. If the provided
   ...
   *            buf is null.
   **/
  public synchronized static native int H5Tencode(long obj_id, byte[] buf,
                                                  long nalloc)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            name is null.
   **/
  public synchronized static native void H5Tenum_insert(long type, String name,
                                                        byte[] value)
      throws HDF5LibraryException, NullPointerException;
  ...
  private synchronized static native int
  H5Tenum_insert_int(long type, String name, int[] value)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            value is null.
   **/
  public synchronized static native String H5Tenum_nameof(long type,
                                                          byte[] value,
                                                          long size)
      throws HDF5LibraryException, NullPointerException;

  // int H5Tenum_nameof(int type, Pointer value, Buffer name/* out */, long
  // size);
   ...
   *            name is null.
   **/
  public static int H5Tenum_nameof(long type, int[] value, String[] name,
                                   int size)
      throws HDF5LibraryException, NullPointerException {
  ...
  private synchronized static native int
  H5Tenum_nameof_int(long type, int[] value, String[] name, int size)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Tenum_valueof(long type, String name,
                                                         byte[] value)
      throws HDF5LibraryException, NullPointerException;
  ...
  private synchronized static native int
  H5Tenum_valueof_int(long type, String name, int[] value)
      throws HDF5LibraryException, NullPointerException;
   ...
   * @ingroup JH5T
   *
   * H5Tget_array_dims returns the sizes of the dimensions of the specified
   ...
   * @ingroup JH5T
   *
   * H5Tget_array_dims2 returns the sizes of the dimensions of the specified
   *array datatype object.
   *
   * @param type_id
   *            IN: Datatype identifier of array object.
   ...
   *            dims is null.
   **/
  public synchronized static native int H5Tget_array_dims2(long type_id,
                                                           long[] dims)
      throws HDF5LibraryException, NullPointerException;
   ...
   *            IN: Identifier of datatype to query.
   *
   * @return datatype class identifier if successful; otherwise
   *H5T_NO_CLASS(-1).
   *
   * @exception HDF5LibraryException
   *            Error from the HDF5 Library.
   ...
   *bits.</li> <li>fields[1] = epos OUT: location to return exponent
   *bit-position.</li> <li>fields[2] = esize OUT: location to return size of
   *exponent in bits.</li> <li>fields[3] = mpos OUT: location to return mantissa
   *bit-position.</li> <li>fields[4] = msize OUT: location to return size of
   *mantissa in bits.</li>
   *            </ul>
   *
   * @exception HDF5LibraryException
   ...
   **/
  public synchronized static native void H5Tget_fields(long type_id,
                                                       long[] fields)
      throws HDF5LibraryException, NullPointerException,
             IllegalArgumentException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Tset_fields(long type_id, long spos,
                                                       long epos, long esize,
   ...
   *            Error from the HDF5 Library.
   **/
  public static int H5Tset_fields(long type_id, int spos, int epos, int esize,
                                  int mpos, int msize)
      throws HDF5LibraryException {
    H5Tset_fields(type_id, (long)spos, (long)epos, (long)esize, (long)mpos,
                  (long)msize);
    return 0;
  }
   ...
   *then H5Tset_inpad will be filled according to the value of the padding value
   *property inpad.
   *
   * @param type_id
   *            IN: Identifier of datatype to modify.
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native int H5Tget_member_index(long type_id,
                                                            String field_name)
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native String H5Tget_member_name(long type_id,
                                                              int field_idx)
      throws HDF5LibraryException;
   ...
   *compound datatype. This is the byte offset in the HDF5 file/library, NOT the
  ...
  private synchronized static native int
  H5Tget_member_value_int(long type_id, int membno, int[] value)
      throws HDF5LibraryException, NullPointerException;
   ...
   *
   * @param type_id
   *            IN: Identifier of datatype to query. Direction of search is
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Tset_offset(long type_id,
                                                       long offset)
      throws HDF5LibraryException;
   ...
   *            IN: Identifier of datatype to query.
   * @param pad
   *            OUT: locations to return least-significant and most-significant
   *bit padding type.
   *
   *            <pre>
   ...
   *            Error from the HDF5 Library.
   **/
  public static int H5Tset_precision(long type_id, int precision)
      throws HDF5LibraryException {
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Tset_precision(long type_id,
                                                          long precision)
      throws HDF5LibraryException;
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native long H5Tget_size(long type_id)
      throws HDF5LibraryException;
   ...
   * @ingroup JH5T
   *
   * H5Tis_variable_str determines whether the datatype identified in type_id is
   ...
   * @param name
   *            IN: A datatype name, defined within the file or group identified
   *by loc_id.
   * @param tapl_id
   *            IN: Datatype access property list.
   *
  ...
  private synchronized static native long _H5Topen2(long loc_id, String name,
                                                    long tapl_id)
      throws HDF5LibraryException, NullPointerException;
   ...
   *it more efficient (space-wise) to store that data. <P> <b>WARNING:</b> This
   *call only affects the C-data, even if it succeeds, there may be no visible
   ...
   * @ingroup JH5T
   *
   * H5Tflush causes all buffers associated with a committed datatype to be
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Tflush(long dtype_id)
      throws HDF5LibraryException;

  /**
   * @ingroup JH5T
   *
   * H5Trefresh causes all buffers associated with a committed datatype to be
   *cleared and immediately re-loaded with updated contents from disk. This
   *function essentially closes the datatype, evicts all metadata associated
   *with it from the cache, and then re-opens the datatype. The reopened
   *datatype is automatically re-registered with the same ID.
   *
   * @param dtype_id
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native void H5Trefresh(long dtype_id)
      throws HDF5LibraryException;
  ...
  // public synchronized static native int H5Tregister(H5T_pers_t pers, String
  // name, int src_id, int dst_id, H5T_conv_t func) throws HDF5LibraryException,
  // NullPointerException;

  // public synchronized static native int H5Tunregister(H5T_pers_t pers, String
  // name, int src_id, int dst_id, H5T_conv_t func) throws HDF5LibraryException,
  // NullPointerException;
   ...
   * @ingroup JH5VL
   *
   * H5VLregister_connector_by_name registers a new VOL connector as a member of
   *the virtual object layer class.
   *
   ...
   * @ingroup JH5VL
   *
   * H5VLregister_connector_by_value registers a new VOL connector as a member
   *of the virtual object layer class.
   *
   ...
   * @ingroup JH5VL
   *
   * H5VLis_connector_registered_by_name tests whether a VOL class has been
   *registered.
   *
   * @param name
   *            IN: name of the connector.
   ...
   **/
  public synchronized static native boolean
  H5VLis_connector_registered_by_name(String name) throws HDF5LibraryException;
  /**
   * @ingroup JH5VL
   *
   * H5VLis_connector_registered_by_value tests whether a VOL class has been
   *registered.
   *
   * @param connector_value
   *            IN: value of the connector.
   ...
   * @ingroup JH5VL
   *
   * H5VLget_connector_id retrieves the ID for a registered VOL connector for a
   *given object.
   *
   * @param object_id
   *            IN: Identifier of the object.
   ...
   * @ingroup JH5VL
   *
   * H5VLget_connector_id_by_name retrieves the ID for a registered VOL
   *connector.
   *
   * @param name
   *            IN: name of the connector.
   ...
   **/
  public synchronized static native long
  H5VLget_connector_id_by_name(String name) throws HDF5LibraryException;
  /**
   * @ingroup JH5VL
   *
   * H5VLget_connector_id_by_value retrieves the ID for a registered VOL
   *connector.
   *
   * @param connector_value
   *            IN: value of the connector.
   ...
   * @ingroup JH5VL
   *
   * H5VLget_connector_name returns the connector name for the VOL associated
   *with the object or file ID.
   *
   * @param object_id
   *            IN: Identifier of the object.
   *
   ...
   *            Error from the HDF5 Library.
   **/
  public synchronized static native String
  H5VLget_connector_name(long object_id) throws HDF5LibraryException;
  /**
   * @ingroup JH5VL
   *
   ...
   **/
  public synchronized static native void
  H5VLunregister_connector(long connector_id) throws HDF5LibraryException;
