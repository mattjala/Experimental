...
static int test_parallel_read(MPI_Comm comm, int mpi_rank, int mpi_size,
                              int group);
...
static int generate_test_file(MPI_Comm comm, int mpi_rank, int group_id) {
  ...
  if (pass) {
    if ((file_id = H5Fcreate(data_filename, H5F_ACC_TRUNC, fctmpl, fapl_id)) <
        0) {
      pass = FALSE;
  ...
  if (pass) {
    ...
    if ((H5Sselect_hyperslab(filespace, H5S_SELECT_SET, &offset, NULL, &count,
                             NULL)) < 0) {
      pass = FALSE;
  ...
  if (pass) {
    if ((dset_id = H5Dcreate2(file_id, "dataset0", H5T_NATIVE_FLOAT, filespace,
  ...
  if (pass) {
    if ((H5Dwrite(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, dxpl_id,
                  data_slice)) < 0) {
      pass = FALSE;
  ...
  if (pass) {
    ...
    if ((dset_id_ch =
             H5Dcreate2(file_id, "dataset0_chunked", H5T_NATIVE_FLOAT,
                        filespace, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
      pass = FALSE;
      failure_mssg = "H5Dcreate2() failed.\n";
    }
  }
  ...
  if (pass) {
    if ((H5Dwrite(dset_id_ch, H5T_NATIVE_FLOAT, memspace, filespace, dxpl_id,
                  data_slice)) < 0) {
      pass = FALSE;
      failure_mssg = "H5Dwrite() failed.\n";
    }
  ...
  if (MPI_Allreduce(&local_failure, &global_failures, 1, MPI_INT, MPI_SUM,
...
static int test_parallel_read(MPI_Comm comm, int mpi_rank, int mpi_size,
  ...
  if (pass) {
    if ((H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT,
                 data_slice)) < 0) {
      pass = FALSE;
  ...
  if (group_size <= LIMIT_NPROC) {
    ...
    if ((data_slice = (float *)malloc((size_t)dset_size * filetype_size)) ==
        NULL) {
      pass = FALSE;
      failure_mssg = "malloc of data_slice failed.\n";
    }
    ...
    if (pass) {
      if ((H5Dread(dset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, dxpl_id,
                   data_slice)) < 0) {
        pass = FALSE;
        failure_mssg = "H5Dread() failed\n";
      }
    ...
    if (pass) {
      ...
      if (pass) {
        if (prop_value != TRUE) {
          pass = FALSE;
          failure_mssg =
              "rank 0 Bcast optimization was mistakenly not performed\n";
        }
      }
    }
#endif /* H5_HAVE_INSTRUMENTED_LIBRARY */
    ...
    if (pass) {
      if ((H5Dread(dset_id_ch, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, dxpl_id,
                   data_slice)) < 0) {
        pass = FALSE;
        failure_mssg = "H5Dread() failed\n";
      }
    ...
    if (pass) {
      ...
      if (pass) {
        if (prop_value == TRUE) {
          pass = FALSE;
          failure_mssg = "rank 0 Bcast optimization was mistakenly performed "
                         "for chunked dataset\n";
        }
      }
    }
    ...
    if ((data_slice = (float *)malloc((size_t)(dset_size * 2) *
                                      filetype_size)) == NULL) {
      pass = FALSE;
      failure_mssg = "malloc of data_slice failed.\n";
    }
    ...
    if (pass) {
      ...
      if ((H5Sselect_hyperslab(memspace, H5S_SELECT_SET, &offset, NULL, &offset,
                               NULL)) < 0) {
        pass = FALSE;
        failure_mssg = "H5Sselect_hyperslab() failed\n";
      }
    ...
    /* read this processes section of the data */
    if (pass) {
      if ((H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, H5S_ALL, dxpl_id,
                   data_slice)) < 0) {
        pass = FALSE;
        failure_mssg = "H5Dread() failed\n";
      }
...
int main(int argc, char **argv) {
  ...
  if ((MPI_Comm_split(MPI_COMM_WORLD, which_group, 0, &group_comm)) !=
      MPI_SUCCESS) {
