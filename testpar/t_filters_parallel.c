...
const char *filterNames[] = {"Deflate", "Shuffle", "Fletcher32",
                             "SZIP",    "Nbit",    "ScaleOffset"};

/* Function pointer typedef for test functions */
typedef void (*test_func)(const char *parent_group, H5Z_filter_t filter_id,
...
static herr_t set_dcpl_filter(hid_t dcpl_id, H5Z_filter_t filter_id,
                              filter_options_t *filter_options);
static herr_t verify_space_alloc_status(hid_t dset_id, hid_t dcpl_id,
                                        num_chunks_written_t chunks_written);

#ifdef H5_HAVE_PARALLEL_FILTERED_WRITES
/* Tests for writing data in parallel */
static void test_write_one_chunk_filtered_dataset(const char *parent_group,
                                                  H5Z_filter_t filter_id,
                                                  hid_t fapl_id, hid_t dcpl_id,
                                                  hid_t dxpl_id);
static void test_write_filtered_dataset_no_overlap(const char *parent_group,
                                                   H5Z_filter_t filter_id,
                                                   hid_t fapl_id, hid_t dcpl_id,
                                                   hid_t dxpl_id);
static void test_write_filtered_dataset_no_overlap_partial(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_overlap(const char *parent_group,
                                                H5Z_filter_t filter_id,
                                                hid_t fapl_id, hid_t dcpl_id,
                                                hid_t dxpl_id);
static void test_write_filtered_dataset_single_unlim_dim_no_overlap(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_single_unlim_dim_overlap(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_multi_unlim_dim_no_overlap(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_multi_unlim_dim_overlap(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_single_no_selection(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_all_no_selection(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_point_selection(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_filtered_dataset_interleaved_write(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_transformed_filtered_dataset_no_overlap(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_3d_filtered_dataset_no_overlap_separate_pages(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_3d_filtered_dataset_no_overlap_same_pages(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_3d_filtered_dataset_overlap(const char *parent_group,
                                                   H5Z_filter_t filter_id,
                                                   hid_t fapl_id, hid_t dcpl_id,
                                                   hid_t dxpl_id);
static void test_write_cmpd_filtered_dataset_no_conversion_unshared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_cmpd_filtered_dataset_no_conversion_shared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_cmpd_filtered_dataset_type_conversion_unshared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_write_cmpd_filtered_dataset_type_conversion_shared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
#endif

/* Tests for reading data in parallel */
static void test_read_one_chunk_filtered_dataset(const char *parent_group,
                                                 H5Z_filter_t filter_id,
                                                 hid_t fapl_id, hid_t dcpl_id,
                                                 hid_t dxpl_id);
static void test_read_filtered_dataset_no_overlap(const char *parent_group,
                                                  H5Z_filter_t filter_id,
                                                  hid_t fapl_id, hid_t dcpl_id,
                                                  hid_t dxpl_id);
static void test_read_filtered_dataset_overlap(const char *parent_group,
                                               H5Z_filter_t filter_id,
                                               hid_t fapl_id, hid_t dcpl_id,
                                               hid_t dxpl_id);
static void test_read_filtered_dataset_single_no_selection(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_filtered_dataset_all_no_selection(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_filtered_dataset_point_selection(const char *parent_group,
                                                       H5Z_filter_t filter_id,
                                                       hid_t fapl_id,
                                                       hid_t dcpl_id,
                                                       hid_t dxpl_id);
...
static void test_read_transformed_filtered_dataset_no_overlap(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_3d_filtered_dataset_no_overlap_separate_pages(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_3d_filtered_dataset_no_overlap_same_pages(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_3d_filtered_dataset_overlap(const char *parent_group,
                                                  H5Z_filter_t filter_id,
                                                  hid_t fapl_id, hid_t dcpl_id,
                                                  hid_t dxpl_id);
static void test_read_cmpd_filtered_dataset_no_conversion_unshared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_cmpd_filtered_dataset_no_conversion_shared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_cmpd_filtered_dataset_type_conversion_unshared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
static void test_read_cmpd_filtered_dataset_type_conversion_shared(
    const char *parent_group, H5Z_filter_t filter_id, hid_t fapl_id,
    hid_t dcpl_id, hid_t dxpl_id);
 ...
 */
static void test_write_serial_read_parallel(const char *parent_group,
                                            H5Z_filter_t filter_id,
                                            hid_t fapl_id, hid_t dcpl_id,
                                            hid_t dxpl_id);

#ifdef H5_HAVE_PARALLEL_FILTERED_WRITES
static void test_write_parallel_read_serial(const char *parent_group,
                                            H5Z_filter_t filter_id,
                                            hid_t fapl_id, hid_t dcpl_id,
                                            hid_t dxpl_id);

/* Other miscellaneous tests */
static void test_shrinking_growing_chunks(const char *parent_group,
                                          H5Z_filter_t filter_id, hid_t fapl_id,
                                          hid_t dcpl_id, hid_t dxpl_id);
static void test_edge_chunks_no_overlap(const char *parent_group,
                                        H5Z_filter_t filter_id, hid_t fapl_id,
                                        hid_t dcpl_id, hid_t dxpl_id);
static void test_edge_chunks_overlap(const char *parent_group,
                                     H5Z_filter_t filter_id, hid_t fapl_id,
                                     hid_t dcpl_id, hid_t dxpl_id);
static void test_edge_chunks_partial_write(const char *parent_group,
                                           H5Z_filter_t filter_id,
                                           hid_t fapl_id, hid_t dcpl_id,
                                           hid_t dxpl_id);
static void test_fill_values(const char *parent_group, H5Z_filter_t filter_id,
                             hid_t fapl_id, hid_t dcpl_id, hid_t dxpl_id);
static void test_fill_value_undefined(const char *parent_group,
                                      H5Z_filter_t filter_id, hid_t fapl_id,
                                      hid_t dcpl_id, hid_t dxpl_id);
static void test_fill_time_never(const char *parent_group,
                                 H5Z_filter_t filter_id, hid_t fapl_id,
                                 hid_t dcpl_id, hid_t dxpl_id);
#endif
...
static herr_t set_dcpl_filter(hid_t dcpl_id, H5Z_filter_t filter_id,
  ...
  case H5Z_FILTER_SZIP: {
    ...
    VRFY(H5Pget_chunk(dcpl_id, H5S_MAX_RANK, chunk_dims) >= 0,
         "H5Pget_chunk succeeded");
  ...
  default: {
    ...
    return H5Pset_filter(dcpl_id, filter_id, filter_options->flags,
                         filter_options->cd_nelmts, filter_options->cd_values);
  }
  }
}
  ...
  if (nfilters > 0) {
    ...
    case H5D_ALLOC_TIME_LATE:
      ...
      if (chunks_written == SOME_CHUNKS_WRITTEN ||
          chunks_written == ALL_CHUNKS_WRITTEN)
        VRFY((space_status == H5D_SPACE_STATUS_ALLOCATED) ||
...
static void test_write_one_chunk_filtered_dataset(const char *parent_group,
  ...
  sel_dims[0] =
      (hsize_t)WRITE_ONE_CHUNK_FILTERED_DATASET_NROWS / (hsize_t)mpi_size;
  sel_dims[1] = (hsize_t)WRITE_ONE_CHUNK_FILTERED_DATASET_NCOLS;

  filespace = H5Screate_simple(WRITE_ONE_CHUNK_FILTERED_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace =
      H5Screate_simple(WRITE_ONE_CHUNK_FILTERED_DATASET_DIMS, sel_dims, NULL);
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_ONE_CHUNK_FILTERED_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_ONE_CHUNK_FILTERED_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  ...
  if (VERBOSE_MED) {
    printf("Process %d is writing with count[ %" PRIuHSIZE ", %" PRIuHSIZE
           " ], stride[ %" PRIuHSIZE ", %" PRIuHSIZE " ], start[ %" PRIuHSIZE
           ", %" PRIuHSIZE " ], block size[ %" PRIuHSIZE ", %" PRIuHSIZE " ]\n",
           mpi_rank, count[0], count[1], stride[0], stride[1], start[0],
           start[1], block[0], block[1]);
    fflush(stdout);
  }
  ...
  VRFY((H5Sselect_hyperslab(filespace, H5S_SELECT_SET, start, stride, count,
                            block) >= 0),
  ...
  VRFY((H5Dwrite(dset_id, HDF5_DATATYPE_NAME, memspace, filespace, dxpl_id,
                 data) >= 0),
  ...
  dset_id =
      H5Dopen2(group_id, WRITE_ONE_CHUNK_FILTERED_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");

  VRFY((H5Dread(dset_id, HDF5_DATATYPE_NAME, H5S_ALL, H5S_ALL, dxpl_id,
                read_buf) >= 0),
...
static void test_write_filtered_dataset_no_overlap(const char *parent_group,
  ...
  filespace = H5Screate_simple(WRITE_UNSHARED_FILTERED_CHUNKS_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(WRITE_UNSHARED_FILTERED_CHUNKS_DATASET_DIMS,
                              sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_UNSHARED_FILTERED_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_UNSHARED_FILTERED_CHUNKS_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
   ...
   */
  count[0] = 1;
  count[1] = (hsize_t)WRITE_UNSHARED_FILTERED_CHUNKS_NCOLS /
             (hsize_t)WRITE_UNSHARED_FILTERED_CHUNKS_CH_NCOLS;
  ...
  start[0] = ((hsize_t)mpi_rank *
              (hsize_t)WRITE_UNSHARED_FILTERED_CHUNKS_CH_NROWS * count[0]);
  start[1] = 0;
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++)
    correct_buf[i] =
        (C_DATATYPE)((i %
                      (dataset_dims[0] / (hsize_t)mpi_size * dataset_dims[1])) +
                     (i /
                      (dataset_dims[0] / (hsize_t)mpi_size * dataset_dims[1])));
  ...
  dset_id = H5Dopen2(group_id, WRITE_UNSHARED_FILTERED_CHUNKS_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_no_overlap_partial(
  ...
  filespace = H5Screate_simple(
      WRITE_UNSHARED_FILTERED_CHUNKS_PARTIAL_DATASET_DIMS, dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id,
                     WRITE_UNSHARED_FILTERED_CHUNKS_PARTIAL_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_UNSHARED_FILTERED_CHUNKS_PARTIAL_DATASET_NAME,
  ...
  start[0] =
      ((hsize_t)mpi_rank *
       (hsize_t)WRITE_UNSHARED_FILTERED_CHUNKS_PARTIAL_CH_NROWS * count[0]);
  start[1] = 0;
  ...
  for (i = 0; i < (size_t)mpi_size; i++) {
    size_t rank_n_elems =
        (size_t)(mpi_size * (WRITE_UNSHARED_FILTERED_CHUNKS_PARTIAL_CH_NROWS *
                             WRITE_UNSHARED_FILTERED_CHUNKS_PARTIAL_CH_NCOLS));
    size_t data_idx = i;
  ...
  VRFY((H5Dwrite(dset_id, HDF5_DATATYPE_NAME, H5S_BLOCK, filespace, dxpl_id,
                 data) >= 0),
       "Dataset write succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, WRITE_UNSHARED_FILTERED_CHUNKS_PARTIAL_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_overlap(const char *parent_group,
  ...
  sel_dims[1] = (hsize_t)WRITE_SHARED_FILTERED_CHUNKS_CH_NCOLS *
                (hsize_t)DIM1_SCALE_FACTOR;

  filespace = H5Screate_simple(WRITE_SHARED_FILTERED_CHUNKS_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(WRITE_SHARED_FILTERED_CHUNKS_DATASET_DIMS,
                              sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_SHARED_FILTERED_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_SHARED_FILTERED_CHUNKS_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
   ...
   * it to the hyperslab in the file
   */
  count[0] = (hsize_t)WRITE_SHARED_FILTERED_CHUNKS_NROWS /
             (hsize_t)WRITE_SHARED_FILTERED_CHUNKS_CH_NROWS;
  count[1] = (hsize_t)WRITE_SHARED_FILTERED_CHUNKS_NCOLS /
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++)
    correct_buf[i] =
        (C_DATATYPE)((dataset_dims[1] *
                      (i / ((hsize_t)mpi_size * dataset_dims[1]))) +
                     (i % dataset_dims[1]) +
                     (((i % ((hsize_t)mpi_size * dataset_dims[1])) /
                       dataset_dims[1]) %
                      dataset_dims[1]));
  ...
  dset_id = H5Dopen2(group_id, WRITE_SHARED_FILTERED_CHUNKS_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_single_unlim_dim_no_overlap(
  ...
  if (MAINPROCESS)
    HDputs("Testing write to unshared filtered chunks w/ single unlimited "
           "dimension");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_UNSHARED_ONE_UNLIM_DIM_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_UNSHARED_ONE_UNLIM_DIM_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  for (i = 0; i < (size_t)WRITE_UNSHARED_ONE_UNLIM_DIM_NLOOPS; i++) {
     ...
     */
    count[0] = 1;
    count[1] = (hsize_t)WRITE_UNSHARED_ONE_UNLIM_DIM_NCOLS /
    ...
    dset_id = H5Dopen2(group_id, WRITE_UNSHARED_ONE_UNLIM_DIM_DATASET_NAME,
                       H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset open succeeded");

    memset(read_buf, 255, data_size);

    VRFY((H5Dread(dset_id, HDF5_DATATYPE_NAME, H5S_BLOCK, filespace, dxpl_id,
                  read_buf) >= 0),
         "Dataset read succeeded");

    /* Verify the correct data was written */
    VRFY((0 == memcmp(read_buf, data, data_size)),
         "Data verification succeeded");
    ...
    if (i < (size_t)WRITE_UNSHARED_ONE_UNLIM_DIM_NLOOPS - 1) {
      ...
      VRFY(H5Dset_extent(dset_id, dataset_dims) >= 0,
           "H5Dset_extent succeeded");
...
static void test_write_filtered_dataset_single_unlim_dim_overlap(
  ...
  if (MAINPROCESS)
    HDputs("Testing write to shared filtered chunks w/ single unlimited "
           "dimension");
  ...
  sel_dims[0] = (hsize_t)DIM0_SCALE_FACTOR;
  sel_dims[1] =
      (hsize_t)WRITE_SHARED_ONE_UNLIM_DIM_CH_NCOLS * (hsize_t)DIM1_SCALE_FACTOR;

  filespace = H5Screate_simple(WRITE_SHARED_ONE_UNLIM_DIM_DATASET_DIMS,
                               dataset_dims, max_dims);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_SHARED_ONE_UNLIM_DIM_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_SHARED_ONE_UNLIM_DIM_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  for (i = 0; i < (size_t)WRITE_SHARED_ONE_UNLIM_DIM_NLOOPS; i++) {
     ...
     * it to the hyperslab in the file
     */
    count[0] = (hsize_t)WRITE_SHARED_ONE_UNLIM_DIM_NROWS /
               (hsize_t)WRITE_SHARED_ONE_UNLIM_DIM_CH_NROWS;
    count[1] = (hsize_t)WRITE_SHARED_ONE_UNLIM_DIM_NCOLS /
               (hsize_t)WRITE_SHARED_ONE_UNLIM_DIM_CH_NCOLS;
    ...
    dset_id = H5Dopen2(group_id, WRITE_SHARED_ONE_UNLIM_DIM_DATASET_NAME,
                       H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_multi_unlim_dim_no_overlap(
  ...
  if (MAINPROCESS)
    HDputs("Testing write to unshared filtered chunks w/ two unlimited "
           "dimensions");
  ...
  filespace = H5Screate_simple(WRITE_UNSHARED_TWO_UNLIM_DIM_DATASET_DIMS,
                               dataset_dims, max_dims);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_UNSHARED_TWO_UNLIM_DIM_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_UNSHARED_TWO_UNLIM_DIM_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  for (i = 0; i < (size_t)WRITE_UNSHARED_TWO_UNLIM_DIM_NLOOPS; i++) {
    ...
    dset_id = H5Dopen2(group_id, WRITE_UNSHARED_TWO_UNLIM_DIM_DATASET_NAME,
                       H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_multi_unlim_dim_overlap(
  ...
  sel_dims[0] = (hsize_t)DIM0_SCALE_FACTOR;
  sel_dims[1] =
      (hsize_t)WRITE_SHARED_TWO_UNLIM_DIM_CH_NCOLS * (hsize_t)DIM1_SCALE_FACTOR;

  filespace = H5Screate_simple(WRITE_SHARED_TWO_UNLIM_DIM_DATASET_DIMS,
                               dataset_dims, max_dims);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_SHARED_TWO_UNLIM_DIM_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_SHARED_TWO_UNLIM_DIM_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  for (i = 0; i < (size_t)WRITE_SHARED_TWO_UNLIM_DIM_NLOOPS; i++) {
    ...
    dset_id = H5Dopen2(group_id, WRITE_SHARED_TWO_UNLIM_DIM_DATASET_NAME,
                       H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_single_no_selection(
  ...
  if (MAINPROCESS)
    HDputs("Testing write to filtered chunks with a single process having no "
           "selection");
  ...
  filespace =
      H5Screate_simple(WRITE_SINGLE_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS,
                       dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(
      WRITE_SINGLE_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id,
                     WRITE_SINGLE_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_SINGLE_NO_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  start[0] = (hsize_t)mpi_rank *
             (hsize_t)WRITE_SINGLE_NO_SELECTION_FILTERED_CHUNKS_CH_NROWS *
             count[0];
  start[1] = 0;
  ...
  /* Compute the correct offset into the buffer for the process having no
   * selection and clear it */
  segment_length = dataset_dims[0] * dataset_dims[1] / (hsize_t)mpi_size;
  memset(correct_buf +
             ((size_t)WRITE_SINGLE_NO_SELECTION_FILTERED_CHUNKS_NO_SELECT_PROC *
              segment_length),
  ...
  /* Verify space allocation status - data should only have been written if MPI
   * size > 1 */
  verify_space_alloc_status(
      dset_id, plist_id,
...
static void test_write_filtered_dataset_all_no_selection(
  ...
  if (MAINPROCESS)
    HDputs("Testing write to filtered chunks with all processes having no "
           "selection");
  ...
  filespace = H5Screate_simple(
      WRITE_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(
      WRITE_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id,
                     WRITE_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, WRITE_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_point_selection(
  ...
  sel_dims[0] =
      (hsize_t)WRITE_POINT_SELECTION_FILTERED_CHUNKS_NROWS / (hsize_t)mpi_size;
  sel_dims[1] = (hsize_t)WRITE_POINT_SELECTION_FILTERED_CHUNKS_NCOLS;

  filespace = H5Screate_simple(
      WRITE_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(
      WRITE_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id,
                     WRITE_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_POINT_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  num_points = (hsize_t)WRITE_POINT_SELECTION_FILTERED_CHUNKS_NROWS *
               (hsize_t)WRITE_POINT_SELECTION_FILTERED_CHUNKS_NCOLS /
               (hsize_t)mpi_size;
  ...
  for (i = 0; i < num_points; i++)
    for (j = 0; j < WRITE_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS; j++)
      coords[(i * WRITE_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS) + j] =
          (j > 0)
              ? (i % (hsize_t)WRITE_POINT_SELECTION_FILTERED_CHUNKS_NCOLS)
              : ((hsize_t)mpi_rank +
                 ((hsize_t)mpi_size *
                  (i / (hsize_t)WRITE_POINT_SELECTION_FILTERED_CHUNKS_NCOLS)));

  VRFY((H5Sselect_elements(filespace, H5S_SELECT_SET, (hsize_t)num_points,
  ...
  dset_id =
      H5Dopen2(group_id, WRITE_POINT_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_filtered_dataset_interleaved_write(
  ...
  filespace = H5Screate_simple(INTERLEAVED_WRITE_FILTERED_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace =
      H5Screate_simple(INTERLEAVED_WRITE_FILTERED_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, INTERLEAVED_WRITE_FILTERED_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, INTERLEAVED_WRITE_FILTERED_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
   ...
   * it to the hyperslab in the file
   */
  count[0] = (hsize_t)(INTERLEAVED_WRITE_FILTERED_DATASET_NROWS /
                       INTERLEAVED_WRITE_FILTERED_DATASET_CH_NROWS);
  count[1] = (hsize_t)(INTERLEAVED_WRITE_FILTERED_DATASET_NCOLS /
                       INTERLEAVED_WRITE_FILTERED_DATASET_CH_NCOLS);
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++)
    ...
    correct_buf[i] =
        (C_DATATYPE)((i % (hsize_t)INTERLEAVED_WRITE_FILTERED_DATASET_NCOLS)
                     ...
                     + ((i %
                         (hsize_t)(mpi_size *
                                   INTERLEAVED_WRITE_FILTERED_DATASET_NCOLS)) /
                        (hsize_t)INTERLEAVED_WRITE_FILTERED_DATASET_NCOLS)

                     /* Add the amount that gets added when a rank moves down to
                        its next section vertically in the dataset */
                     + ((hsize_t)INTERLEAVED_WRITE_FILTERED_DATASET_NCOLS *
                        (i /
                         (hsize_t)(mpi_size *
                                   INTERLEAVED_WRITE_FILTERED_DATASET_NCOLS))));
  ...
  dset_id =
      H5Dopen2(group_id, INTERLEAVED_WRITE_FILTERED_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_transformed_filtered_dataset_no_overlap(
  ...
  filespace =
      H5Screate_simple(WRITE_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_DIMS,
                       dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(
      WRITE_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id,
                     WRITE_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_NAME,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  start[0] =
      ((hsize_t)mpi_rank *
       (hsize_t)WRITE_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_CH_NROWS * count[0]);
  start[1] = 0;
  ...
  VRFY((H5Pset_data_transform(plist_id, "x") >= 0),
       "Set data transform expression succeeded");

  VRFY((H5Dwrite(dset_id, HDF5_DATATYPE_NAME, memspace, filespace, plist_id,
                 data) >= 0),
       "Dataset write succeeded");
  ...
  dset_id = H5Dopen2(group_id,
                     WRITE_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");

  VRFY((H5Dread(dset_id, HDF5_DATATYPE_NAME, H5S_ALL, H5S_ALL, plist_id,
                read_buf) >= 0),
       "Dataset read succeeded");
...
static void test_write_3d_filtered_dataset_no_overlap_separate_pages(
  ...
  if (MAINPROCESS)
    HDputs("Testing write to unshared filtered chunks on separate pages in 3D "
           "dataset");
  ...
  filespace =
      H5Screate_simple(WRITE_UNSHARED_FILTERED_CHUNKS_3D_SEP_PAGE_DATASET_DIMS,
                       dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(
      WRITE_UNSHARED_FILTERED_CHUNKS_3D_SEP_PAGE_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id,
                     WRITE_UNSHARED_FILTERED_CHUNKS_3D_SEP_PAGE_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_UNSHARED_FILTERED_CHUNKS_3D_SEP_PAGE_DATASET_NAME,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  if (VERBOSE_MED) {
    printf(
        "Process %d is writing with count[ %" PRIuHSIZE ", %" PRIuHSIZE
        ", %" PRIuHSIZE " ], stride[ %" PRIuHSIZE ", %" PRIuHSIZE
        ", %" PRIuHSIZE " ], start[ %" PRIuHSIZE ", %" PRIuHSIZE ", %" PRIuHSIZE
        " ], block size[ %" PRIuHSIZE ", %" PRIuHSIZE ", %" PRIuHSIZE " ]\n",
        mpi_rank, count[0], count[1], count[2], stride[0], stride[1], stride[2],
  ...
  correct_buf_size = dataset_dims[0] * dataset_dims[1] * dataset_dims[2] *
                     sizeof(*correct_buf);
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++)
    correct_buf[i] =
        (C_DATATYPE)((i % (hsize_t)mpi_size) + (i / (hsize_t)mpi_size));
...
static void test_write_3d_filtered_dataset_no_overlap_same_pages(
  ...
  if (MAINPROCESS)
    HDputs("Testing write to unshared filtered chunks on the same pages in 3D "
           "dataset");
  ...
  filespace =
      H5Screate_simple(WRITE_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_DIMS,
                       dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(
      WRITE_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id,
                     WRITE_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_NAME,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  block[2] = 1;
  start[0] = ((hsize_t)mpi_rank *
              (hsize_t)WRITE_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_CH_NROWS *
              count[0]);
  start[1] = 0;
  start[2] = 0;
  ...
  dset_id = H5Dopen2(group_id,
                     WRITE_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_3d_filtered_dataset_overlap(const char *parent_group,
  ...
  filespace = H5Screate_simple(WRITE_SHARED_FILTERED_CHUNKS_3D_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace = H5Screate_simple(WRITE_SHARED_FILTERED_CHUNKS_3D_DATASET_DIMS,
                              sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_SHARED_FILTERED_CHUNKS_3D_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_SHARED_FILTERED_CHUNKS_3D_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  count[0] = (hsize_t)(WRITE_SHARED_FILTERED_CHUNKS_3D_NROWS /
                       WRITE_SHARED_FILTERED_CHUNKS_3D_CH_NROWS);
  count[1] = (hsize_t)(WRITE_SHARED_FILTERED_CHUNKS_3D_NCOLS /
                       WRITE_SHARED_FILTERED_CHUNKS_3D_CH_NCOLS);
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++)
    ...
    correct_buf[i] =
        (C_DATATYPE)((i % (hsize_t)(WRITE_SHARED_FILTERED_CHUNKS_3D_DEPTH *
                                    WRITE_SHARED_FILTERED_CHUNKS_3D_NCOLS))

                     /* Add the Row Index */
                     + ((i % (hsize_t)(mpi_size *
                                       WRITE_SHARED_FILTERED_CHUNKS_3D_DEPTH *
                                       WRITE_SHARED_FILTERED_CHUNKS_3D_NCOLS)) /
  ...
  dset_id = H5Dopen2(group_id, WRITE_SHARED_FILTERED_CHUNKS_3D_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_cmpd_filtered_dataset_no_conversion_unshared(
  ...
  hsize_t dataset_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t sel_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t
      start[WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t stride
      [WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t
      count[WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t
      block[WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  size_t i, correct_buf_size;
  hid_t file_id = H5I_INVALID_HID, dset_id = H5I_INVALID_HID,
        plist_id = H5I_INVALID_HID, memtype = H5I_INVALID_HID;
  hid_t group_id = H5I_INVALID_HID;
  hid_t filespace = H5I_INVALID_HID, memspace = H5I_INVALID_HID;

  if (MAINPROCESS)
    HDputs("Testing write to unshared filtered chunks in Compound Datatype "
           "dataset without Datatype "
           "conversion");
  ...
  dataset_dims[1] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_NCOLS;
  chunk_dims[0] =
      WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_CH_NROWS;
  chunk_dims[1] =
      WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_CH_NCOLS;
  sel_dims[0] = WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_CH_NROWS;
  sel_dims[1] = (hsize_t)
      WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_ENTRIES_PER_PROC;
  ...
  VRFY((H5Tinsert(memtype, "ShortData", HOFFSET(COMPOUND_C_DATATYPE, field1),
                  H5T_NATIVE_SHORT) >= 0),
       "Datatype insertion succeeded");
  VRFY((H5Tinsert(memtype, "IntData", HOFFSET(COMPOUND_C_DATATYPE, field2),
                  H5T_NATIVE_INT) >= 0),
       "Datatype insertion succeeded");
  VRFY((H5Tinsert(memtype, "LongData", HOFFSET(COMPOUND_C_DATATYPE, field3),
                  H5T_NATIVE_LONG) >= 0),
       "Datatype insertion succeeded");

  dset_id = H5Dcreate2(
      group_id,
      WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_NAME,
  ...
  correct_buf_size =
      dataset_dims[0] * dataset_dims[1] * sizeof(COMPOUND_C_DATATYPE);
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++) {
    correct_buf[i].field1 =
        (short)((i % dataset_dims[1]) + (i / dataset_dims[1]));

    correct_buf[i].field2 =
        (int)((i % dataset_dims[1]) + (i / dataset_dims[1]));

    correct_buf[i].field3 =
        (long)((i % dataset_dims[1]) + (i / dataset_dims[1]));
  }

  VRFY((H5Dwrite(dset_id, memtype, memspace, filespace, dxpl_id, data) >= 0),
       "Dataset write succeeded");
  ...
  VRFY((H5Dread(dset_id, memtype, H5S_ALL, H5S_ALL, dxpl_id, read_buf) >= 0),
       "Dataset read succeeded");
...
static void test_write_cmpd_filtered_dataset_no_conversion_shared(
  ...
  COMPOUND_C_DATATYPE *correct_buf = NULL;
  hsize_t dataset_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t sel_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      start[WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      stride[WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      count[WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      block[WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  size_t i, correct_buf_size;
  hid_t file_id = H5I_INVALID_HID, dset_id = H5I_INVALID_HID,
        plist_id = H5I_INVALID_HID, memtype = H5I_INVALID_HID;
  hid_t group_id = H5I_INVALID_HID;
  ...
  if (MAINPROCESS)
    HDputs("Testing write to shared filtered chunks in Compound Datatype "
           "dataset without Datatype "
           "conversion");
  ...
  /* Create the dataspace for the dataset */
  dataset_dims[0] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_NROWS;
  dataset_dims[1] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_NCOLS;
  chunk_dims[0] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_CH_NROWS;
  chunk_dims[1] = WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_CH_NCOLS;
  sel_dims[0] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_CH_NROWS /
      (hsize_t)mpi_size;
  sel_dims[1] = (hsize_t)
      WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_ENTRIES_PER_PROC;
  ...
  dset_id = H5Dcreate2(
      group_id,
      WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_NAME, memtype,
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++) {
    ...
    correct_buf[i].field2 =
        (int)((dataset_dims[1] * (i / ((hsize_t)mpi_size * dataset_dims[1]))) +
              (i % dataset_dims[1]) +
              (((i % ((hsize_t)mpi_size * dataset_dims[1])) / dataset_dims[1]) %
               dataset_dims[1]));

    correct_buf[i].field3 =
        (long)((dataset_dims[1] * (i / ((hsize_t)mpi_size * dataset_dims[1]))) +
               (i % dataset_dims[1]) +
               (((i % ((hsize_t)mpi_size * dataset_dims[1])) /
                 dataset_dims[1]) %
                dataset_dims[1]));
  ...
  dset_id =
      H5Dopen2(group_id,
               WRITE_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_cmpd_filtered_dataset_type_conversion_unshared(
  ...
  COMPOUND_C_DATATYPE *correct_buf = NULL;
  hsize_t dataset_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t sel_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_DATASET_DIMS];
  ...
  size_t i, correct_buf_size;
  hid_t file_id = H5I_INVALID_HID, dset_id = H5I_INVALID_HID,
        plist_id = H5I_INVALID_HID, filetype = H5I_INVALID_HID,
  ...
  /* Create the dataspace for the dataset */
  dataset_dims[0] =
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_NROWS;
  dataset_dims[1] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_NCOLS;
  chunk_dims[0] =
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_CH_NROWS;
  chunk_dims[1] =
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_CH_NCOLS;
  sel_dims[0] =
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_CH_NROWS;
  sel_dims[1] = (hsize_t)
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_ENTRIES_PER_PROC;
  ...
  dset_id = H5Dcreate2(
      group_id,
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_DATASET_NAME,
  ...
  /* Ensure that this test currently fails since type conversions break
   * collective mode */
  H5E_BEGIN_TRY {
    VRFY((H5Dwrite(dset_id, memtype, memspace, filespace, dxpl_id, data) < 0),
         "Dataset write succeeded");
...
static void test_write_cmpd_filtered_dataset_type_conversion_shared(
  ...
  COMPOUND_C_DATATYPE *correct_buf = NULL;
  hsize_t dataset_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t sel_dims
      [WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      start[WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t stride
      [WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      count[WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      block[WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  size_t i, correct_buf_size;
  hid_t file_id = H5I_INVALID_HID, dset_id = H5I_INVALID_HID,
        plist_id = H5I_INVALID_HID;
  ...
  if (MAINPROCESS)
    HDputs("Testing write to shared filtered chunks in Compound Datatype "
           "dataset with Datatype conversion");
  ...
  /* Create the dataspace for the dataset */
  dataset_dims[0] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_NROWS;
  dataset_dims[1] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_NCOLS;
  chunk_dims[0] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_CH_NROWS;
  chunk_dims[1] =
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_CH_NCOLS;
  sel_dims[0] =
      (hsize_t)WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_CH_NROWS /
      (hsize_t)mpi_size;
  sel_dims[1] = (hsize_t)
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_ENTRIES_PER_PROC;
  ...
  dset_id = H5Dcreate2(
      group_id,
      WRITE_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_NAME,
      filetype, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
...
static void test_read_one_chunk_filtered_dataset(const char *parent_group,
  ...
  if (MAINPROCESS) {
    ...
    VRFY((H5Pset_libver_bounds(plist_id, H5F_LIBVER_LATEST,
                               H5F_LIBVER_LATEST) >= 0),
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(READ_ONE_CHUNK_FILTERED_DATASET_DIMS,
                                 dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id, READ_ONE_CHUNK_FILTERED_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(group_id, READ_ONE_CHUNK_FILTERED_DATASET_NAME,
                         HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                         H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
    ...
    VRFY((H5Dwrite(dset_id, HDF5_DATATYPE_NAME, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   correct_buf) >= 0),
         "Dataset write succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, READ_ONE_CHUNK_FILTERED_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");

  sel_dims[0] =
      (hsize_t)READ_ONE_CHUNK_FILTERED_DATASET_NROWS / (hsize_t)mpi_size;
  sel_dims[1] = (hsize_t)READ_ONE_CHUNK_FILTERED_DATASET_NCOLS;

  /* Setup one-dimensional memory dataspace for reading the dataset data into a
   * contiguous buffer */
  ...
  if (VERBOSE_MED) {
    printf("Process %d is reading with count[ %" PRIuHSIZE ", %" PRIuHSIZE
           " ], stride[ %" PRIuHSIZE ", %" PRIuHSIZE " ], start[ %" PRIuHSIZE
           ", %" PRIuHSIZE " ], block size[ %" PRIuHSIZE ", %" PRIuHSIZE " ]\n",
           mpi_rank, count[0], count[1], stride[0], stride[1], start[0],
           start[1], block[0], block[1]);
  ...
  VRFY((H5Dread(dset_id, HDF5_DATATYPE_NAME, memspace, filespace, dxpl_id,
                read_buf) >= 0),
       "Dataset read succeeded");
  ...
  VRFY((MPI_SUCCESS == MPI_Allgatherv(read_buf, (int)flat_dims[0],
                                      C_DATATYPE_MPI, global_buf, recvcounts,
...
static void test_read_filtered_dataset_no_overlap(const char *parent_group,
  ...
  correct_buf_size = (hsize_t)READ_UNSHARED_FILTERED_CHUNKS_NROWS *
                     (hsize_t)READ_UNSHARED_FILTERED_CHUNKS_NCOLS *
                     sizeof(*correct_buf);
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(READ_UNSHARED_FILTERED_CHUNKS_DATASET_DIMS,
                                 dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id, READ_UNSHARED_FILTERED_CHUNKS_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(group_id, READ_UNSHARED_FILTERED_CHUNKS_DATASET_NAME,
                         HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                         H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id = H5Dopen2(group_id, READ_UNSHARED_FILTERED_CHUNKS_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
   ...
   */
  count[0] = 1;
  count[1] = (hsize_t)READ_UNSHARED_FILTERED_CHUNKS_NCOLS /
             (hsize_t)READ_UNSHARED_FILTERED_CHUNKS_CH_NCOLS;
  ...
  start[0] = ((hsize_t)mpi_rank *
              (hsize_t)READ_UNSHARED_FILTERED_CHUNKS_CH_NROWS * count[0]);
  start[1] = 0;
...
static void test_read_filtered_dataset_overlap(const char *parent_group,
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(READ_SHARED_FILTERED_CHUNKS_DATASET_DIMS,
                                 dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id, READ_SHARED_FILTERED_CHUNKS_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(group_id, READ_SHARED_FILTERED_CHUNKS_DATASET_NAME,
                         HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                         H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, READ_SHARED_FILTERED_CHUNKS_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");

  sel_dims[0] = (hsize_t)DIM0_SCALE_FACTOR;
  sel_dims[1] = (hsize_t)READ_SHARED_FILTERED_CHUNKS_CH_NCOLS *
                (hsize_t)DIM1_SCALE_FACTOR;
   ...
   * reads it to the selection in memory
   */
  count[0] = (hsize_t)READ_SHARED_FILTERED_CHUNKS_NROWS /
             (hsize_t)READ_SHARED_FILTERED_CHUNKS_CH_NROWS;
  count[1] = (hsize_t)READ_SHARED_FILTERED_CHUNKS_NCOLS /
             (hsize_t)READ_SHARED_FILTERED_CHUNKS_CH_NCOLS;
   ...
   */
  {
    ...
    for (; loop_count; loop_count--) {
      VRFY((MPI_SUCCESS ==
            MPI_Allgatherv(
                &read_buf[(count[0] - loop_count) * dataset_dims[1]],
...
static void test_read_filtered_dataset_single_no_selection(
  ...
  if (MAINPROCESS)
    HDputs("Testing read from filtered chunks with a single process having no "
           "selection");
  ...
  segment_length = dataset_dims[0] * dataset_dims[1] / (hsize_t)mpi_size;
  memset(correct_buf +
             ((size_t)READ_SINGLE_NO_SELECTION_FILTERED_CHUNKS_NO_SELECT_PROC *
              segment_length),
  ...
  if (MAINPROCESS) {
    ...
    VRFY((H5Pset_chunk(plist_id,
                       READ_SINGLE_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
  ...
  dset_id =
      H5Dopen2(group_id, READ_SINGLE_NO_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
  ...
  start[0] = (hsize_t)mpi_rank *
             (hsize_t)READ_SINGLE_NO_SELECTION_FILTERED_CHUNKS_CH_NROWS *
             count[0];
  start[1] = 0;
  ...
  for (i = 0; i < (size_t)mpi_size; i++)
    displs[i] =
        (int)(i * (size_t)(READ_SINGLE_NO_SELECTION_FILTERED_CHUNKS_CH_NROWS *
                           READ_SINGLE_NO_SELECTION_FILTERED_CHUNKS_NCOLS));

  if (mpi_rank == READ_SINGLE_NO_SELECTION_FILTERED_CHUNKS_NO_SELECT_PROC)
    VRFY((MPI_SUCCESS == MPI_Allgatherv(read_buf, 0, C_DATATYPE_MPI, global_buf,
                                        recvcounts, displs, C_DATATYPE_MPI,
                                        comm)),
         "MPI_Allgatherv succeeded");
  else
    VRFY((MPI_SUCCESS == MPI_Allgatherv(read_buf, (int)flat_dims[0],
...
static void test_read_filtered_dataset_all_no_selection(
  ...
  if (MAINPROCESS)
    HDputs("Testing read from filtered chunks with all processes having no "
           "selection");
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(
        READ_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id,
                       READ_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(
        group_id, READ_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
        HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, READ_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
  ...
  memspace = H5Screate_simple(
      READ_ALL_NO_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
...
static void test_read_filtered_dataset_point_selection(const char *parent_group,
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(
        READ_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS, dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id,
                       READ_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(
        group_id, READ_POINT_SELECTION_FILTERED_CHUNKS_DATASET_NAME,
        HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id = H5Dopen2(
      group_id, READ_POINT_SELECTION_FILTERED_CHUNKS_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");

  sel_dims[0] =
      (hsize_t)READ_POINT_SELECTION_FILTERED_CHUNKS_NROWS / (hsize_t)mpi_size;
  ...
  num_points = (hsize_t)READ_POINT_SELECTION_FILTERED_CHUNKS_NROWS *
               (hsize_t)READ_POINT_SELECTION_FILTERED_CHUNKS_NCOLS /
               (hsize_t)mpi_size;
  coords = (hsize_t *)calloc(1, 2 * num_points * sizeof(*coords));
  VRFY((NULL != coords), "Coords calloc succeeded");
  ...
  for (i = 0; i < num_points; i++)
    for (j = 0; j < READ_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS; j++)
      coords[(i * READ_POINT_SELECTION_FILTERED_CHUNKS_DATASET_DIMS) + j] =
          (j > 0)
              ? (i % (hsize_t)READ_POINT_SELECTION_FILTERED_CHUNKS_NCOLS)
              : ((hsize_t)mpi_rank +
                 ((hsize_t)mpi_size *
                  (i / (hsize_t)READ_POINT_SELECTION_FILTERED_CHUNKS_NCOLS)));
   ...
   */
  {
    ...
    for (; cur_loop_count; cur_loop_count--) {
      VRFY((MPI_SUCCESS ==
            MPI_Allgatherv(&read_buf[(original_loop_count - cur_loop_count) *
                                     dataset_dims[1]],
                           recvcounts[mpi_rank], C_DATATYPE_MPI,
                           &global_buf[(original_loop_count - cur_loop_count) *
                                       total_recvcounts],
                           recvcounts, displs, C_DATATYPE_MPI, comm)),
           "MPI_Allgatherv succeeded");
    }
...
static void test_read_filtered_dataset_interleaved_read(
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++)
    ...
    correct_buf[i] =
        (C_DATATYPE)((i % (hsize_t)INTERLEAVED_READ_FILTERED_DATASET_NCOLS)
                     ...
                     +
                     ((i % (hsize_t)(mpi_size *
                                     INTERLEAVED_READ_FILTERED_DATASET_NCOLS)) /
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(INTERLEAVED_READ_FILTERED_DATASET_DIMS,
                                 dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id, INTERLEAVED_READ_FILTERED_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(group_id, INTERLEAVED_READ_FILTERED_DATASET_NAME,
                         HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                         H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, INTERLEAVED_READ_FILTERED_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
   ...
   * reads it to the selection in memory
   */
  count[0] = (hsize_t)(INTERLEAVED_READ_FILTERED_DATASET_NROWS /
                       INTERLEAVED_READ_FILTERED_DATASET_CH_NROWS);
  count[1] = (hsize_t)(INTERLEAVED_READ_FILTERED_DATASET_NCOLS /
                       INTERLEAVED_READ_FILTERED_DATASET_CH_NCOLS);
...
static void test_read_3d_filtered_dataset_no_overlap_separate_pages(
  ...
  if (MAINPROCESS)
    HDputs("Testing read from unshared filtered chunks on separate pages in 3D "
           "dataset");
  ...
  if (MAINPROCESS) {
    ...
    VRFY((H5Pset_chunk(plist_id,
                       READ_UNSHARED_FILTERED_CHUNKS_3D_SEP_PAGE_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(
        group_id, READ_UNSHARED_FILTERED_CHUNKS_3D_SEP_PAGE_DATASET_NAME,
        HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, READ_UNSHARED_FILTERED_CHUNKS_3D_SEP_PAGE_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
  ...
  /*
   * Due to the nature of 3-dimensional reading, create an MPI vector type that
   * allows each rank to write to the nth position of the global data buffer,
   * where n is the rank number.
   */
  VRFY((MPI_SUCCESS == MPI_Type_vector((int)flat_dims[0], 1, mpi_size,
                                       C_DATATYPE_MPI, &vector_type)),
       "MPI_Type_vector succeeded");
  VRFY((MPI_SUCCESS == MPI_Type_commit(&vector_type)),
       "MPI_Type_commit succeeded");

  /*
   * Resize the type to allow interleaving,
   * so make it only one MPI_LONG wide
   */
  VRFY((MPI_SUCCESS == MPI_Type_create_resized(vector_type, 0, sizeof(long),
                                               &resized_vector_type)),
  ...
  VRFY(
      (MPI_SUCCESS == MPI_Allgather(read_buf, (int)flat_dims[0], C_DATATYPE_MPI,
                                    global_buf, 1, resized_vector_type, comm)),
  ...
  VRFY((MPI_SUCCESS == MPI_Type_free(&resized_vector_type)),
       "MPI_Type_free succeeded");
...
static void test_read_transformed_filtered_dataset_no_overlap(
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace =
        H5Screate_simple(READ_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_DIMS,
                         dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id,
                       READ_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(
        group_id, READ_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_NAME,
        HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
    ...
    VRFY((H5Dwrite(dset_id, HDF5_DATATYPE_NAME, H5S_ALL, H5S_ALL, plist_id,
                   correct_buf) >= 0),
         "Dataset write succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, READ_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_DATASET_NAME,
               H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
  ...
  start[0] =
      ((hsize_t)mpi_rank *
       (hsize_t)READ_UNSHARED_TRANSFORMED_FILTERED_CHUNKS_CH_NROWS * count[0]);
  start[1] = 0;
  ...
  VRFY((H5Dread(dset_id, HDF5_DATATYPE_NAME, memspace, filespace, plist_id,
                read_buf) >= 0),
       "Dataset read succeeded");
...
static void test_read_3d_filtered_dataset_no_overlap_same_pages(
  ...
  if (MAINPROCESS)
    HDputs("Testing read from unshared filtered chunks on the same pages in 3D "
           "dataset");
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(
        READ_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_DIMS, dataset_dims,
        NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");

    /* Create chunked dataset */
    chunk_dims[0] =
        (hsize_t)READ_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_CH_NROWS;
    chunk_dims[1] =
    ...
    VRFY((H5Pset_chunk(plist_id,
                       READ_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(
        group_id, READ_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_DATASET_NAME,
        HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  start[0] =
      ((hsize_t)mpi_rank *
       (hsize_t)READ_UNSHARED_FILTERED_CHUNKS_3D_SAME_PAGE_CH_NROWS * count[0]);
  start[1] = 0;
  start[2] = 0;
...
static void test_read_3d_filtered_dataset_overlap(const char *parent_group,
  ...
  for (i = 0; i < correct_buf_size / sizeof(*correct_buf); i++)
    /* Add the Column Index */
    correct_buf[i] =
        (C_DATATYPE)((i % (hsize_t)(READ_SHARED_FILTERED_CHUNKS_3D_DEPTH *
                                    READ_SHARED_FILTERED_CHUNKS_3D_NCOLS))

                     /* Add the Row Index */
                     + ((i % (hsize_t)(mpi_size *
                                       READ_SHARED_FILTERED_CHUNKS_3D_DEPTH *
                                       READ_SHARED_FILTERED_CHUNKS_3D_NCOLS)) /
  ...
  if (MAINPROCESS) {
    ...
    /* Create the dataspace for the dataset */
    filespace = H5Screate_simple(READ_SHARED_FILTERED_CHUNKS_3D_DATASET_DIMS,
                                 dataset_dims, NULL);
    VRFY((filespace >= 0), "File dataspace creation succeeded");
    ...
    VRFY((H5Pset_chunk(plist_id, READ_SHARED_FILTERED_CHUNKS_3D_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(group_id, READ_SHARED_FILTERED_CHUNKS_3D_DATASET_NAME,
                         HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                         H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id = H5Dopen2(group_id, READ_SHARED_FILTERED_CHUNKS_3D_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
   ...
   * reads it to the selection in memory
   */
  count[0] = (hsize_t)(READ_SHARED_FILTERED_CHUNKS_3D_NROWS /
                       READ_SHARED_FILTERED_CHUNKS_3D_CH_NROWS);
  count[1] = (hsize_t)(READ_SHARED_FILTERED_CHUNKS_3D_NCOLS /
                       READ_SHARED_FILTERED_CHUNKS_3D_CH_NCOLS);
  ...
  {
    size_t run_length = (size_t)(READ_SHARED_FILTERED_CHUNKS_3D_NCOLS *
                                 READ_SHARED_FILTERED_CHUNKS_3D_DEPTH);
    size_t num_blocks =
        (size_t)(READ_SHARED_FILTERED_CHUNKS_3D_NROWS / mpi_size);

    /*
     * Due to the nature of 3-dimensional reading, create an MPI vector type
     * that allows each rank to write to the nth position of the global data
     * buffer, where n is the rank number.
     */
    VRFY((MPI_SUCCESS == MPI_Type_vector((int)num_blocks, (int)run_length,
                                         (int)(mpi_size * (int)run_length),
                                         C_DATATYPE_MPI, &vector_type)),
         "MPI_Type_vector succeeded");
    VRFY((MPI_SUCCESS == MPI_Type_commit(&vector_type)),
         "MPI_Type_commit succeeded");
     ...
     */
    VRFY((MPI_SUCCESS ==
          MPI_Type_create_resized(vector_type, 0,
                                  (MPI_Aint)(run_length * sizeof(long)),
                                  &resized_vector_type)),
         "MPI_Type_create_resized");
    VRFY((MPI_SUCCESS == MPI_Type_commit(&resized_vector_type)),
         "MPI_Type_commit succeeded");
...
static void test_read_cmpd_filtered_dataset_no_conversion_unshared(
  ...
  hsize_t dataset_dims
      [READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t sel_dims
      [READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t
      start[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t
      stride[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t
      count[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t
      block[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t flat_dims[1];
  size_t i, read_buf_size, correct_buf_size;
  hid_t file_id = H5I_INVALID_HID, dset_id = H5I_INVALID_HID,
        plist_id = H5I_INVALID_HID, memtype = H5I_INVALID_HID;
  ...
  if (MAINPROCESS)
    HDputs("Testing read from unshared filtered chunks in Compound Datatype "
           "dataset without Datatype "
           "conversion");
  ...
  dataset_dims[0] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_NROWS;
  dataset_dims[1] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_NCOLS;
  ...
  if (MAINPROCESS) {
    ...
    /* Create chunked dataset */
    chunk_dims[0] =
        READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_CH_NROWS;
    chunk_dims[1] =
        READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_CH_NCOLS;
    ...
    dset_id = H5Dcreate2(
        group_id,
        READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_DATASET_NAME,
        memtype, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
    ...
    VRFY((H5Dwrite(dset_id, memtype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   correct_buf) >= 0),
         "Dataset write succeeded");
  ...
  sel_dims[0] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_CH_NROWS;
  sel_dims[1] = (hsize_t)
      READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_UNSHARED_ENTRIES_PER_PROC;
  ...
  VRFY((H5Dread(dset_id, memtype, memspace, filespace, dxpl_id, read_buf) >= 0),
       "Dataset read succeeded");
  ...
  VRFY((MPI_SUCCESS ==
        MPI_Allgatherv(
            read_buf, (int)(flat_dims[0] * sizeof(COMPOUND_C_DATATYPE)),
...
static void test_read_cmpd_filtered_dataset_no_conversion_shared(
  ...
  COMPOUND_C_DATATYPE *global_buf = NULL;
  hsize_t dataset_dims
      [READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      sel_dims[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      start[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      stride[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      count[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      block[READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t flat_dims[1];
  size_t i, read_buf_size, correct_buf_size;
  hid_t file_id = H5I_INVALID_HID, dset_id = H5I_INVALID_HID,
        plist_id = H5I_INVALID_HID, memtype = H5I_INVALID_HID;
  ...
  if (MAINPROCESS)
    HDputs("Testing read from shared filtered chunks in Compound Datatype "
           "dataset without Datatype "
           "conversion");
  ...
  dataset_dims[0] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_NROWS;
  dataset_dims[1] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_NCOLS;
  ...
  if (MAINPROCESS) {
    ...
    /* Create chunked dataset */
    chunk_dims[0] =
        (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_CH_NROWS;
    chunk_dims[1] =
        (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_CH_NCOLS;
  ...
  dset_id = H5Dopen2(
      group_id, READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_DATASET_NAME,
      H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
  ...
  sel_dims[0] =
      ...
      (hsize_t)mpi_size;
  sel_dims[1] = (hsize_t)
      READ_COMPOUND_FILTERED_CHUNKS_NO_CONVERSION_SHARED_ENTRIES_PER_PROC;
...
static void test_read_cmpd_filtered_dataset_type_conversion_unshared(
  ...
  COMPOUND_C_DATATYPE *global_buf = NULL;
  hsize_t dataset_dims
      [READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_DATASET_DIMS];
  hsize_t sel_dims
      [READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_DATASET_DIMS];
  ...
  dataset_dims[0] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_NROWS;
  dataset_dims[1] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_NCOLS;
  ...
  if (MAINPROCESS) {
    ...
    /* Create chunked dataset */
    chunk_dims[0] =
        READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_CH_NROWS;
    chunk_dims[1] =
        READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_CH_NCOLS;
  ...
  sel_dims[0] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_CH_NROWS;
  sel_dims[1] = (hsize_t)
      READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_UNSHARED_ENTRIES_PER_PROC;
...
static void test_read_cmpd_filtered_dataset_type_conversion_shared(
  ...
  COMPOUND_C_DATATYPE *global_buf = NULL;
  hsize_t dataset_dims
      [READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t chunk_dims
      [READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t sel_dims
      [READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      start[READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      stride[READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      count[READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t
      block[READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_DATASET_DIMS];
  hsize_t flat_dims[1];
  size_t i, read_buf_size, correct_buf_size;
  hid_t file_id = H5I_INVALID_HID, dset_id = H5I_INVALID_HID,
        plist_id = H5I_INVALID_HID;
  ...
  dataset_dims[0] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_NROWS;
  dataset_dims[1] =
      (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_NCOLS;
  ...
  if (MAINPROCESS) {
    ...
    /* Create chunked dataset */
    chunk_dims[0] =
        (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_CH_NROWS;
    chunk_dims[1] =
        (hsize_t)READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_CH_NCOLS;
  ...
  sel_dims[0] =
      ...
      (hsize_t)mpi_size;
  sel_dims[1] = (hsize_t)
      READ_COMPOUND_FILTERED_CHUNKS_TYPE_CONVERSION_SHARED_ENTRIES_PER_PROC;
...
static void test_write_serial_read_parallel(const char *parent_group,
  ...
  if (MAINPROCESS) {
    ...
    VRFY((H5Pset_chunk(plist_id, WRITE_SERIAL_READ_PARALLEL_DATASET_DIMS,
                       chunk_dims) >= 0),
         "Chunk size set");
    ...
    dset_id = H5Dcreate2(group_id, WRITE_SERIAL_READ_PARALLEL_DATASET_NAME,
                         HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                         H5P_DEFAULT);
    VRFY((dset_id >= 0), "Dataset creation succeeded");
    ...
    data_size =
        dataset_dims[0] * dataset_dims[1] * dataset_dims[2] * sizeof(*data);
  ...
  /* All ranks open the file and verify their "portion" of the dataset is
   * correct */
  file_id = H5Fopen(filenames[0], H5F_ACC_RDWR, fapl_id);
  VRFY((file_id >= 0), "Test file open succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, WRITE_SERIAL_READ_PARALLEL_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_write_parallel_read_serial(const char *parent_group,
  ...
  filespace = H5Screate_simple(WRITE_PARALLEL_READ_SERIAL_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace =
      H5Screate_simple(WRITE_PARALLEL_READ_SERIAL_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_PARALLEL_READ_SERIAL_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_PARALLEL_READ_SERIAL_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
   ...
   */
  count[0] = 1;
  count[1] = (hsize_t)WRITE_PARALLEL_READ_SERIAL_NCOLS /
             (hsize_t)WRITE_PARALLEL_READ_SERIAL_CH_NCOLS;
  count[2] = (hsize_t)mpi_size;
  ...
  offset[0] = ((hsize_t)mpi_rank *
               (hsize_t)WRITE_PARALLEL_READ_SERIAL_CH_NROWS * count[0]);
  offset[1] = 0;
  offset[2] = 0;
  ...
  if (VERBOSE_MED) {
    printf("Process %d is writing with count[ %" PRIuHSIZE ", %" PRIuHSIZE
           ", %" PRIuHSIZE " ], stride[ %" PRIuHSIZE ", %" PRIuHSIZE
           ", %" PRIuHSIZE " ], offset[ %" PRIuHSIZE ", %" PRIuHSIZE
           ", %" PRIuHSIZE " ], block size[ %" PRIuHSIZE ", %" PRIuHSIZE
           ", %" PRIuHSIZE " ]\n",
           mpi_rank, count[0], count[1], count[2], stride[0], stride[1],
           stride[2], offset[0], offset[1], offset[2], block[0], block[1],
           block[2]);
    fflush(stdout);
  }
  ...
  VRFY((H5Sselect_hyperslab(filespace, H5S_SELECT_SET, offset, stride, count,
                            block) >= 0),
       "Hyperslab selection succeeded");
  ...
  if (MAINPROCESS) {
    ...
    VRFY((H5Dread(dset_id, HDF5_DATATYPE_NAME, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                  read_buf) >= 0),
         "Dataset read succeeded");
...
static void test_shrinking_growing_chunks(const char *parent_group,
  ...
  filespace = H5Screate_simple(SHRINKING_GROWING_CHUNKS_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");

  memspace =
      H5Screate_simple(SHRINKING_GROWING_CHUNKS_DATASET_DIMS, sel_dims, NULL);
  VRFY((memspace >= 0), "Memory dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, SHRINKING_GROWING_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, SHRINKING_GROWING_CHUNKS_DATASET_NAME,
                       H5T_NATIVE_DOUBLE, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
   ...
   */
  count[0] = 1;
  count[1] = (hsize_t)SHRINKING_GROWING_CHUNKS_NCOLS /
             (hsize_t)SHRINKING_GROWING_CHUNKS_CH_NCOLS;
  ...
  start[0] = ((hsize_t)mpi_rank * (hsize_t)SHRINKING_GROWING_CHUNKS_CH_NROWS *
              count[0]);
  start[1] = 0;
  ...
  for (i = 0; i < SHRINKING_GROWING_CHUNKS_NLOOPS; i++) {
    ...
    VRFY((H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, memspace, filespace, dxpl_id,
                   data) >= 0),
         "Dataset write succeeded");
    ...
    VRFY((H5Dread(dset_id, H5T_NATIVE_DOUBLE, memspace, filespace, dxpl_id,
                  read_buf) >= 0),
         "Dataset read succeeded");
...
static void test_edge_chunks_no_overlap(const char *parent_group,
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_UNSHARED_FILTERED_EDGE_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_UNSHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  start[0] = ((hsize_t)mpi_rank *
              (hsize_t)WRITE_UNSHARED_FILTERED_EDGE_CHUNKS_CH_NROWS);
  start[1] = (hsize_t)(WRITE_UNSHARED_FILTERED_EDGE_CHUNKS_NCOLS -
  ...
  /* Verify space allocation status */
  verify_space_alloc_status(dset_id, plist_id,
                            (mpi_size > 1) ? SOME_CHUNKS_WRITTEN
                                           : ALL_CHUNKS_WRITTEN);

  VRFY((H5Dclose(dset_id) >= 0), "Dataset close succeeded");

  /* Verify the correct data was written */
  dset_id = H5Dopen2(group_id, WRITE_UNSHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_UNSHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME2,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  /* Verify the correct data was written */
  dset_id = H5Dopen2(
      group_id, WRITE_UNSHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME2, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_edge_chunks_overlap(const char *parent_group,
  ...
  filespace = H5Screate_simple(WRITE_SHARED_FILTERED_EDGE_CHUNKS_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, WRITE_SHARED_FILTERED_EDGE_CHUNKS_DATASET_DIMS,
                     chunk_dims) >= 0),
       "Chunk size set");
  ...
  dset_id = H5Dcreate2(group_id, WRITE_SHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
   ...
   * it to the hyperslab in the file
   */
  count[0] = (hsize_t)(WRITE_SHARED_FILTERED_EDGE_CHUNKS_NROWS /
                       WRITE_SHARED_FILTERED_EDGE_CHUNKS_CH_NROWS);
  count[1] = 1;
  ...
  start[0] = (hsize_t)mpi_rank;
  start[1] = (hsize_t)(WRITE_SHARED_FILTERED_EDGE_CHUNKS_NCOLS -
                       WRITE_SHARED_FILTERED_EDGE_CHUNKS_CH_NCOLS);
  ...
  /* Verify the correct data was written */
  dset_id = H5Dopen2(group_id, WRITE_SHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
  ...
  dset_id = H5Dcreate2(
      group_id, WRITE_SHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME2,
      HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  /* Verify the correct data was written */
  dset_id = H5Dopen2(group_id, WRITE_SHARED_FILTERED_EDGE_CHUNKS_DATASET_NAME2,
                     H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
...
static void test_fill_values(const char *parent_group, H5Z_filter_t filter_id,
  ...
  filespace =
      H5Screate_simple(FILL_VALUES_TEST_DATASET_DIMS, dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_fill_value(plist_id, HDF5_DATATYPE_NAME, &fill_value) >= 0),
       "Fill Value set");

  dset_id =
      H5Dcreate2(group_id, FILL_VALUES_TEST_DATASET_NAME, HDF5_DATATYPE_NAME,
                 filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  VRFY((0 == memcmp(read_buf, correct_buf, read_buf_size)),
       "Data verification succeeded");
  ...
  VRFY((MPI_SUCCESS == MPI_Allgatherv(data, recvcounts[mpi_rank],
                                      C_DATATYPE_MPI, correct_buf, recvcounts,
                                      displs, C_DATATYPE_MPI, comm)),
       "MPI_Allgatherv succeeded");
   ...
   */
  count[0] =
      (hsize_t)FILL_VALUES_TEST_NROWS / (hsize_t)FILL_VALUES_TEST_CH_NROWS;
  count[1] =
      (hsize_t)FILL_VALUES_TEST_NCOLS / (hsize_t)FILL_VALUES_TEST_CH_NCOLS;
  stride[0] = (hsize_t)FILL_VALUES_TEST_CH_NROWS;
  stride[1] = (hsize_t)FILL_VALUES_TEST_CH_NCOLS;
  ...
  for (i = 0; i < read_buf_size / sizeof(*read_buf); i++)
    VRFY((read_buf[i] != FILL_VALUES_TEST_FILL_VAL),
         "Data verification succeeded");
  ...
  VRFY((H5Pset_fill_time(plist_id, H5D_FILL_TIME_ALLOC) >= 0),
       "H5Pset_fill_time succeeded");

  dset_id =
      H5Dcreate2(group_id, FILL_VALUES_TEST_DATASET_NAME2, HDF5_DATATYPE_NAME,
                 filespace, H5P_DEFAULT, plist_id, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
...
static void test_fill_value_undefined(const char *parent_group,
  ...
  sel_dims[0] = (hsize_t)DIM0_SCALE_FACTOR;
  sel_dims[1] =
      (hsize_t)FILL_VALUE_UNDEFINED_TEST_CH_NCOLS * (hsize_t)DIM1_SCALE_FACTOR;

  filespace = H5Screate_simple(FILL_VALUE_UNDEFINED_TEST_DATASET_DIMS,
                               dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_fill_value(plist_id, HDF5_DATATYPE_NAME, NULL) >= 0),
       "Fill Value set");

  dset_id = H5Dcreate2(group_id, FILL_VALUE_UNDEFINED_TEST_DATASET_NAME,
                       HDF5_DATATYPE_NAME, filespace, H5P_DEFAULT, plist_id,
                       H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset creation succeeded");
  ...
  dset_id =
      H5Dopen2(group_id, FILL_VALUE_UNDEFINED_TEST_DATASET_NAME, H5P_DEFAULT);
  VRFY((dset_id >= 0), "Dataset open succeeded");
   ...
   * it to the hyperslab in the file
   */
  count[0] = (hsize_t)FILL_VALUE_UNDEFINED_TEST_NROWS /
             (hsize_t)FILL_VALUE_UNDEFINED_TEST_CH_NROWS;
  count[1] = (hsize_t)FILL_VALUE_UNDEFINED_TEST_NCOLS /
             (hsize_t)FILL_VALUE_UNDEFINED_TEST_CH_NCOLS;
  stride[0] = (hsize_t)FILL_VALUE_UNDEFINED_TEST_CH_NROWS;
  stride[1] = (hsize_t)FILL_VALUE_UNDEFINED_TEST_CH_NCOLS;
...
static void test_fill_time_never(const char *parent_group,
  ...
  sel_dims[0] = (hsize_t)DIM0_SCALE_FACTOR;
  sel_dims[1] =
      (hsize_t)FILL_TIME_NEVER_TEST_CH_NCOLS * (hsize_t)DIM1_SCALE_FACTOR;

  filespace =
      H5Screate_simple(FILL_TIME_NEVER_TEST_DATASET_DIMS, dataset_dims, NULL);
  VRFY((filespace >= 0), "File dataspace creation succeeded");
  ...
  VRFY((H5Pset_chunk(plist_id, FILL_TIME_NEVER_TEST_DATASET_DIMS, chunk_dims) >=
        0),
       "Chunk size set");
   ...
   */
  VRFY((0 != memcmp(read_buf, fill_buf, read_buf_size)),
       "Data verification succeeded");
  ...
  VRFY((MPI_SUCCESS == MPI_Allgatherv(data, recvcounts[mpi_rank],
                                      C_DATATYPE_MPI, fill_buf, recvcounts,
                                      displs, C_DATATYPE_MPI, comm)),
       "MPI_Allgatherv succeeded");
   ...
   * it to the hyperslab in the file
   */
  count[0] = (hsize_t)FILL_TIME_NEVER_TEST_NROWS /
             (hsize_t)FILL_TIME_NEVER_TEST_CH_NROWS;
  count[1] = (hsize_t)FILL_TIME_NEVER_TEST_NCOLS /
             (hsize_t)FILL_TIME_NEVER_TEST_CH_NCOLS;
  stride[0] = (hsize_t)FILL_TIME_NEVER_TEST_CH_NROWS;
  stride[1] = (hsize_t)FILL_TIME_NEVER_TEST_CH_NCOLS;
  ...
  for (i = 0; i < read_buf_size / sizeof(*read_buf); i++)
    VRFY((read_buf[i] != FILL_TIME_NEVER_TEST_FILL_VAL),
         "Data verification succeeded");
...
int main(int argc, char **argv) {
  ...
  VRFY((H5Pset_all_coll_metadata_ops(fapl_id, TRUE) >= 0),
       "H5Pset_all_coll_metadata_ops succeeded");
  VRFY((H5Pset_coll_metadata_write(fapl_id, TRUE) >= 0),
       "H5Pset_coll_metadata_write succeeded");

  VRFY((H5Pset_libver_bounds(fapl_id, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) >=
        0),
       "Set libver bounds succeeded");
  ...
  VRFY((H5Pset_file_space_strategy(fcpl_id, H5F_FSPACE_STRATEGY_PAGE, TRUE,
                                   1) >= 0),
       "H5Pset_file_space_strategy succeeded");

  VRFY((h5_fixname(FILENAME[0], fapl_id, filenames[0], sizeof(filenames[0])) !=
        NULL),
  ...
  for (cur_filter_idx = 0; cur_filter_idx < num_filters; cur_filter_idx++) {
    ...
    for (chunk_opt = H5FD_MPIO_CHUNK_ONE_IO;
         chunk_opt <= H5FD_MPIO_CHUNK_MULTI_IO; chunk_opt++) {
      H5D_alloc_time_t space_alloc_time;

      /* Run tests with all available space allocation times */
      for (space_alloc_time = H5D_ALLOC_TIME_EARLY;
           space_alloc_time <= H5D_ALLOC_TIME_INCR; space_alloc_time++) {
        ...
        if (MAINPROCESS)
          printf("== Running tests with filter '%s' using '%s' and '%s' "
                 "allocation time ==\n\n",
        ...
        if (0 == (filter_config & H5Z_FILTER_CONFIG_ENCODE_ENABLED)) {
          if (MAINPROCESS)
            printf(" ** SKIPPED tests with filter '%s' - filter not "
                   "encode-enabled **\n\n",
                   filterNames[cur_filter_idx]);
          continue;
        }
        ...
        HDsnprintf(group_name, sizeof(group_name), "%s_%s_%s",
                   filterNames[cur_filter_idx],
                   H5FD_MPIO_CHUNK_ONE_IO == chunk_opt ? "linked-chunk-io"
                                                       : "multi-chunk-io",
                   alloc_time);
