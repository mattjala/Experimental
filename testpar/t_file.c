...
static int create_file(const char *filename, hid_t fcpl, hid_t fapl,
                       int metadata_write_strategy);
static int open_file(const char *filename, hid_t fapl,
                     int metadata_write_strategy, hsize_t page_size,
...
void test_split_comm_access(void) {
  ...
  if (is_old) {
    ...
    H5E_BEGIN_TRY { ret = H5Fdelete(filename, acc_tpl); }
    H5E_END_TRY
...
void test_page_buffer_access(void) {
  ...
  ret =
      H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, 1, (hsize_t)0);
  VRFY((ret == 0), "");
  ...
  /* This should fail because collective metadata writes are not supported with
   * page buffering */
  ...
  ret = create_file(filename, fcpl, fapl,
                    H5AC_METADATA_WRITE_STRATEGY__DISTRIBUTED);
  VRFY((ret == 0), "");
  ret = open_file(filename, fapl, H5AC_METADATA_WRITE_STRATEGY__DISTRIBUTED,
                  sizeof(int) * 100, sizeof(int) * 100000);
  VRFY((ret == 0), "");

  ret = create_file(filename, fcpl, fapl,
                    H5AC_METADATA_WRITE_STRATEGY__PROCESS_0_ONLY);
  VRFY((ret == 0), "");
  ret = open_file(filename, fapl, H5AC_METADATA_WRITE_STRATEGY__PROCESS_0_ONLY,
                  sizeof(int) * 100, sizeof(int) * 100000);
  VRFY((ret == 0), "");
  ...
  if (MAINPROCESS) {
    ...
    ret = H5F_block_write(f, H5FD_MEM_SUPER, meta_addr,
                          sizeof(int) * (size_t)num_elements, data);
    VRFY((ret == 0), "");
    ret = H5F_block_write(f, H5FD_MEM_SUPER, meta_addr,
                          sizeof(int) * (size_t)num_elements, data);
    ret = H5F_block_write(f, H5FD_MEM_DRAW, raw_addr,
                          sizeof(int) * (size_t)num_elements, data);
    VRFY((ret == 0), "");

    VRFY((H5SL_count(f->shared->page_buf->slist_ptr) == page_count),
         "Wrong number of pages in PB");
    ...
    for (i = 0; i < 50; i++)
      data[i] = i + 50;
    ret = H5F_block_write(f, H5FD_MEM_DRAW, raw_addr + (sizeof(int) * 50),
                          sizeof(int) * 50, data);
    VRFY((ret == 0), "");
    ret = H5F_block_write(f, H5FD_MEM_SUPER, meta_addr + (sizeof(int) * 50),
                          sizeof(int) * 50, data);
    VRFY((ret == 0), "");
    VRFY((H5SL_count(f->shared->page_buf->slist_ptr) == page_count),
         "Wrong number of pages in PB");
    ...
    for (i = 0; i < 100; i++)
      data[i] = i + 100;
    ret = H5F_block_write(f, H5FD_MEM_DRAW, raw_addr + (sizeof(int) * 100),
                          sizeof(int) * 100, data);
    VRFY((ret == 0), "");
    ret = H5F_block_write(f, H5FD_MEM_SUPER, meta_addr + (sizeof(int) * 100),
                          sizeof(int) * 100, data);
    VRFY((ret == 0), "");
    VRFY((H5SL_count(f->shared->page_buf->slist_ptr) == page_count),
         "Wrong number of pages in PB");
...
static int create_file(const char *filename, hid_t fcpl, hid_t fapl,
  ...
  for (k = 0; k < NUM_DSETS; k++) {
    ...
    dset_id = H5Dcreate2(grp_id, dset_name, H5T_NATIVE_INT, sid, H5P_DEFAULT,
    ...
    ret = H5Dwrite(dset_id, H5T_NATIVE_INT, mem_dataspace, sid, H5P_DEFAULT,
                   data_array);
    VRFY((ret == 0), "");
...
static int open_file(const char *filename, hid_t fapl,
  ...
  for (k = 0; k < NUM_DSETS; k++) {
    ...
    VRFY(dims[0] == (hsize_t)(ROW_FACTOR * mpi_size),
         "Wrong dataset dimensions");
    VRFY(dims[1] == (hsize_t)(COL_FACTOR * mpi_size),
         "Wrong dataset dimensions");
  ...
  grp_id2 = H5Gcreate2(file_id, "GROUP/GROUP2", H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
  VRFY((grp_id2 >= 0), "");
...
void test_delete(void) {
  ...
  H5E_BEGIN_TRY { is_accessible = H5Fis_accessible(filename, fapl_id); }
  H5E_END_TRY
