...
static int test_encode_decode(hid_t orig_pl, int mpi_rank, int recv_proc) {
  ...
  if (mpi_rank == 0) {
    ...
    MPI_Isend(sbuf, send_size, MPI_BYTE, recv_proc, 124, MPI_COMM_WORLD,
              &req[1]);
  } /* end if */
...
void test_plist_ed(void) {
  ...
  ret = H5Pset_attr_creation_order(
      ocpl, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED));
...
void external_links(void) {
  ...
  if (mpi_rank == 0) {
    ...
    ret = H5Lcreate_external(filename_ext, group_path, group, link_name,
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (doIO) {
      ...
      HDsnprintf(link_path, sizeof(link_path), "%s%s%s", group_path, "/",
                 link_name);
