...
static void hs_dr_pio_test__setup(const int test_num, const int edge_size,
                                  const int checker_edge_size,
                                  const int chunk_edge_size,
                                  const int small_rank, const int large_rank,
                                  const hbool_t use_collective_io,
                                  const hid_t dset_type, const int express_test,
  ...
  tv_ptr->stride_ptr =
      &(tv_ptr->stride[PAR_SS_DR_MAX_RANK - tv_ptr->large_rank]);
  ...
  tv_ptr->small_ds_buf_0 =
      (uint32_t *)malloc(sizeof(uint32_t) * tv_ptr->small_ds_size);
  ...
  tv_ptr->small_ds_slice_buf =
      (uint32_t *)malloc(sizeof(uint32_t) * tv_ptr->small_ds_slice_size);
  VRFY((tv_ptr->small_ds_slice_buf != NULL),
       "malloc of small_ds_slice_buf succeeded");

  tv_ptr->large_ds_buf_0 =
      (uint32_t *)malloc(sizeof(uint32_t) * tv_ptr->large_ds_size);
  ...
  tv_ptr->large_ds_slice_buf =
      (uint32_t *)malloc(sizeof(uint32_t) * tv_ptr->large_ds_slice_size);
  VRFY((tv_ptr->large_ds_slice_buf != NULL),
       "malloc of large_ds_slice_buf succeeded");
  ...
  memset(tv_ptr->small_ds_slice_buf, 0,
         sizeof(uint32_t) * tv_ptr->small_ds_slice_size);
  ...
  memset(tv_ptr->large_ds_slice_buf, 0,
         sizeof(uint32_t) * tv_ptr->large_ds_slice_size);
  ...
  tv_ptr->dims[1] = tv_ptr->dims[2] = tv_ptr->dims[3] = tv_ptr->dims[4] =
      (hsize_t)(tv_ptr->edge_size);
  ...
  if (tv_ptr->chunk_edge_size > 0) {
    ...
    tv_ptr->chunk_dims[1] = tv_ptr->chunk_dims[2] = tv_ptr->chunk_dims[3] =
    ...
    ret =
        H5Pset_chunk(small_ds_dcpl_id, tv_ptr->small_rank, tv_ptr->chunk_dims);
    ...
    ret =
        H5Pset_chunk(large_ds_dcpl_id, tv_ptr->large_rank, tv_ptr->chunk_dims);
    VRFY((ret != FAIL), "H5Pset_chunk() large_ds_dcpl_id succeeded");
  }

  /* create the small dataset */
  tv_ptr->small_dataset = H5Dcreate2(
      tv_ptr->fid, "small_dataset", tv_ptr->dset_type,
      tv_ptr->file_small_ds_sid_0, H5P_DEFAULT, small_ds_dcpl_id, H5P_DEFAULT);
  VRFY((ret != FAIL), "H5Dcreate2() small_dataset succeeded");

  /* create the large dataset */
  tv_ptr->large_dataset = H5Dcreate2(
      tv_ptr->fid, "large_dataset", tv_ptr->dset_type,
      tv_ptr->file_large_ds_sid_0, H5P_DEFAULT, large_ds_dcpl_id, H5P_DEFAULT);
  ...
  ret = H5Sselect_hyperslab(tv_ptr->mem_small_ds_sid, H5S_SELECT_SET,
                            tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                            tv_ptr->block);
  VRFY((ret >= 0), "H5Sselect_hyperslab(mem_small_ds_sid, set) succeeded");

  ret = H5Sselect_hyperslab(tv_ptr->file_small_ds_sid_0, H5S_SELECT_SET,
                            tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                            tv_ptr->block);
  ...
  if (MAINPROCESS) { /* add an additional slice to the selections */
    ...
    ret = H5Sselect_hyperslab(tv_ptr->mem_small_ds_sid, H5S_SELECT_OR,
                              tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                              tv_ptr->block);
    VRFY((ret >= 0), "H5Sselect_hyperslab(mem_small_ds_sid, or) succeeded");

    ret = H5Sselect_hyperslab(tv_ptr->file_small_ds_sid_0, H5S_SELECT_OR,
                              tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                              tv_ptr->block);
  ...
  ret = H5Dwrite(tv_ptr->small_dataset, tv_ptr->dset_type,
                 tv_ptr->mem_small_ds_sid, tv_ptr->file_small_ds_sid_0,
                 tv_ptr->xfer_plist, tv_ptr->small_ds_buf_0);
   ...
   */
  ret = H5Dread(tv_ptr->small_dataset, H5T_NATIVE_UINT32,
                tv_ptr->full_mem_small_ds_sid, tv_ptr->full_file_small_ds_sid,
                tv_ptr->xfer_plist, tv_ptr->small_ds_buf_1);
  ...
  ret = H5Sselect_hyperslab(tv_ptr->mem_large_ds_sid, H5S_SELECT_SET,
                            tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                            tv_ptr->block);
  VRFY((ret >= 0), "H5Sselect_hyperslab(mem_large_ds_sid, set) succeeded");

  ret = H5Sselect_hyperslab(tv_ptr->file_large_ds_sid_0, H5S_SELECT_SET,
                            tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                            tv_ptr->block);
  ...
  ret = H5Sselect_hyperslab(tv_ptr->mem_large_ds_process_slice_sid,
                            H5S_SELECT_SET, tv_ptr->start, tv_ptr->stride,
                            tv_ptr->count, tv_ptr->block);
  VRFY((ret >= 0),
       "H5Sselect_hyperslab(mem_large_ds_process_slice_sid, set) succeeded");

  ret = H5Sselect_hyperslab(tv_ptr->file_large_ds_process_slice_sid,
                            H5S_SELECT_SET, tv_ptr->start, tv_ptr->stride,
                            tv_ptr->count, tv_ptr->block);
  VRFY((ret >= 0),
       "H5Sselect_hyperslab(file_large_ds_process_slice_sid, set) succeeded");

  if (MAINPROCESS) { /* add an additional slice to the selections */

    tv_ptr->start[0] = (hsize_t)(tv_ptr->mpi_size);

    ret = H5Sselect_hyperslab(tv_ptr->mem_large_ds_sid, H5S_SELECT_OR,
                              tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                              tv_ptr->block);
    VRFY((ret >= 0), "H5Sselect_hyperslab(mem_large_ds_sid, or) succeeded");

    ret = H5Sselect_hyperslab(tv_ptr->file_large_ds_sid_0, H5S_SELECT_OR,
                              tv_ptr->start, tv_ptr->stride, tv_ptr->count,
                              tv_ptr->block);
   ...
   * data set.
   */
  ret = H5Dread(tv_ptr->large_dataset, H5T_NATIVE_UINT32,
                tv_ptr->full_mem_large_ds_sid, tv_ptr->full_file_large_ds_sid,
                tv_ptr->xfer_plist, tv_ptr->large_ds_buf_1);
    ...
    do {
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
          ...
          ret = H5Sselect_hyperslab(tv_ptr->file_large_ds_sid_0, H5S_SELECT_SET,
                                    tv_ptr->start_ptr, tv_ptr->stride_ptr,
                                    tv_ptr->count_ptr, tv_ptr->block_ptr);
          VRFY((ret != FAIL),
           ...
           */
          check = H5Sselect_shape_same(tv_ptr->small_ds_slice_sid,
                                       tv_ptr->file_large_ds_sid_0);
          ...
          fprintf(stdout, "%s:%d: start = %d %d %d %d %d.\n", fcnName,
                  (int)(tv_ptr->mpi_rank), (int)(tv_ptr->start[0]),
                  (int)(tv_ptr->start[1]), (int)(tv_ptr->start[2]),
...
#endif /* CONTIG_HS_DR_PIO_TEST__D2M_L2S__DEBUG */
          ret = H5Dread(tv_ptr->large_dataset, H5T_NATIVE_UINT32,
                        tv_ptr->small_ds_slice_sid, tv_ptr->file_large_ds_sid_0,
                        tv_ptr->xfer_plist, tv_ptr->small_ds_slice_buf);
          ...
          expected_value =
              (uint32_t)((i * tv_ptr->edge_size * tv_ptr->edge_size *
      ...
      } while ((tv_ptr->large_rank > 2) && ((tv_ptr->small_rank - 1) <= 1) &&
               (l < tv_ptr->edge_size));
      k++;
    } while ((tv_ptr->large_rank > 3) && ((tv_ptr->small_rank - 1) <= 2) &&
             (k < tv_ptr->edge_size));
    j++;
  } while ((tv_ptr->large_rank > 4) && ((tv_ptr->small_rank - 1) <= 3) &&
           (j < tv_ptr->edge_size));
...
#if CONTIG_HS_DR_PIO_TEST__D2M_S2L__DEBUG
  fprintf(stdout,
          "%s reading slices of on disk small data set into slices of big data "
          "set.\n",
          fcnName);
#endif /* CONTIG_HS_DR_PIO_TEST__D2M_S2L__DEBUG */
    ...
    do {
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
           ...
           * selections as having the same shape.
           */
          check = H5Sselect_shape_same(tv_ptr->file_small_ds_sid_0,
                                       tv_ptr->mem_large_ds_sid);
          VRFY((check == TRUE), "H5Sselect_shape_same passed");
...
#endif /* CONTIG_HS_DR_PIO_TEST__D2M_S2L__DEBUG */
          ret = H5Dread(tv_ptr->small_dataset, H5T_NATIVE_UINT32,
                        tv_ptr->mem_large_ds_sid, tv_ptr->file_small_ds_sid_0,
                        tv_ptr->xfer_plist, tv_ptr->large_ds_buf_1);
           ...
           */
          ptr_1 = tv_ptr->large_ds_buf_1;
          expected_value = (uint32_t)((size_t)(tv_ptr->mpi_rank) *
                                      tv_ptr->small_ds_slice_size);
          start_index = (size_t)((i * tv_ptr->edge_size * tv_ptr->edge_size *
                                  tv_ptr->edge_size * tv_ptr->edge_size) +
                                 (j * tv_ptr->edge_size * tv_ptr->edge_size *
                                  tv_ptr->edge_size) +
                                 (k * tv_ptr->edge_size * tv_ptr->edge_size) +
    ...
    do {
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
          ...
          ret = H5Dwrite(tv_ptr->small_dataset, H5T_NATIVE_UINT32,
                         tv_ptr->mem_small_ds_sid, tv_ptr->file_small_ds_sid_0,
                         tv_ptr->xfer_plist, tv_ptr->small_ds_buf_2);
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
          ...
          fprintf(stdout, "%s:%d: skipping test with start = %d %d %d %d %d.\n",
                  fcnName, (int)(tv_ptr->mpi_rank), (int)(tv_ptr->start[0]),
                  (int)(tv_ptr->start[1]), (int)(tv_ptr->start[2]),
                  (int)(tv_ptr->start[3]), (int)(tv_ptr->start[4]));
          fprintf(stdout, "%s:%d: mem/file extent dims = %d/%d.\n", fcnName,
                  tv_ptr->mpi_rank,
           ...
           */
          ret = H5Dwrite(tv_ptr->large_dataset, H5T_NATIVE_UINT32,
                         tv_ptr->large_ds_slice_sid,
                         tv_ptr->file_large_ds_process_slice_sid,
                         tv_ptr->xfer_plist, tv_ptr->large_ds_buf_2);
           ...
           */
          check = H5Sselect_shape_same(tv_ptr->mem_small_ds_sid,
                                       tv_ptr->file_large_ds_sid_0);
          VRFY((check == TRUE), "H5Sselect_shape_same passed");
...
#endif /* CONTIG_HS_DR_PIO_TEST__M2D_S2L__DEBUG */
          ret = H5Dwrite(tv_ptr->large_dataset, H5T_NATIVE_UINT32,
                         tv_ptr->mem_small_ds_sid, tv_ptr->file_large_ds_sid_0,
                         tv_ptr->xfer_plist, tv_ptr->small_ds_buf_0);
          VRFY((ret != FAIL),
...
static void contig_hs_dr_pio_test__run_test(
    const int test_num, const int edge_size, const int chunk_edge_size,
    ...
    int64_t *tests_skipped_ptr, int mpi_rank) {
  ...
  if (MAINPROCESS)
    printf("\r - running test #%lld: small rank = %d, large rank = %d",
           (long long)(test_num + 1), small_rank, large_rank);

  hs_dr_pio_test__setup(test_num, edge_size, -1, chunk_edge_size, small_rank,
                        large_rank, use_collective_io, dset_type, express_test,
                        tv_ptr);
  ...
  if (MAINPROCESS) {
    fprintf(stdout,
            "test %d: Subtests complete -- tests run/skipped/total = "
            "%lld/%lld/%lld.\n",
...
static void contig_hs_dr_pio_test(ShapeSameTestMethods sstest_type) {
  int express_test;
  ...
  mpi_result = MPI_Allreduce((void *)&local_express_test, (void *)&express_test,
  ...
  for (large_rank = 3; large_rank <= PAR_SS_DR_MAX_RANK; large_rank++) {
    ...
    for (small_rank = 2; small_rank < large_rank; small_rank++) {
      ...
      case IND_CONTIG:
        ...
        contig_hs_dr_pio_test__run_test(
            test_num, edge_size, chunk_edge_size, small_rank, large_rank, FALSE,
            dset_type, express_test, &skips, max_skips, &total_tests,
            &tests_run, &tests_skipped, mpi_rank);
        test_num++;
        break;
      ...
      case COL_CONTIG:
        ...
        contig_hs_dr_pio_test__run_test(
            test_num, edge_size, chunk_edge_size, small_rank, large_rank, TRUE,
            dset_type, express_test, &skips, max_skips, &total_tests,
            &tests_run, &tests_skipped, mpi_rank);
        test_num++;
        break;
...
static void
ckrbrd_hs_dr_pio_test__slct_ckrbrd(const int mpi_rank, const hid_t tgt_sid,
                                   const int tgt_rank, const int edge_size,
                                   const int checker_edge_size,
  ...
  offset_count =
      (hsize_t)((edge_size - checker_edge_size) / (checker_edge_size * 2));
            ...
            if (((i + j + k + l + m) % 2) == 0) {
              ...
              if (first_selection) {
                ...
                ret = H5Sselect_hyperslab(
                    tgt_sid, H5S_SELECT_SET, &(start[n_cube_offset]),
                ...
                ret = H5Sselect_hyperslab(
                    tgt_sid, H5S_SELECT_OR, &(start[n_cube_offset]),
                    &(stride[n_cube_offset]), &(count[n_cube_offset]),
                    &(block[n_cube_offset]));
  ...
  fprintf(stdout, "%s%d: H5Sget_select_npoints(tgt_sid) = %d.\n", fcnName,
          mpi_rank, (int)H5Sget_select_npoints(tgt_sid));
#endif /* CKRBRD_HS_DR_PIO_TEST__SELECT_CHECKER_BOARD__DEBUG */
  ...
  ret =
      H5Sselect_hyperslab(tgt_sid, H5S_SELECT_AND, start, stride, count, block);
...
static hbool_t ckrbrd_hs_dr_pio_test__verify_data(
    uint32_t *buf_ptr, const int rank, const int edge_size,
  ...
  fprintf(stdout, "%s first_expected_val = %d.\n", fcnName,
          (int)first_expected_val);
  fprintf(stdout, "%s starts_in_checker = %d.\n", fcnName,
          (int)buf_starts_in_checker);
}
#endif
...
ckrbrd_hs_dr_pio_test__d2m_l2s(struct hs_dr_pio_test_vars_t *tv_ptr) {
  ...
  ckrbrd_hs_dr_pio_test__slct_ckrbrd(
      tv_ptr->mpi_rank, tv_ptr->small_ds_slice_sid, tv_ptr->small_rank - 1,
...
#if CHECKER_BOARD_HS_DR_PIO_TEST__D2M_L2S__DEBUG
  fprintf(stdout, "%s:%d: initial small_ds_slice_buf = ", fcnName,
          tv_ptr->mpi_rank);
    ...
    do {
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
          ...
          ckrbrd_hs_dr_pio_test__slct_ckrbrd(
              tv_ptr->mpi_rank, tv_ptr->file_large_ds_sid_0, tv_ptr->large_rank,
              tv_ptr->edge_size, tv_ptr->checker_edge_size,
              tv_ptr->small_rank - 1, tv_ptr->start);
          ...
          data_ok = ckrbrd_hs_dr_pio_test__verify_data(
              tv_ptr->small_ds_slice_buf, tv_ptr->small_rank - 1,
              tv_ptr->edge_size, tv_ptr->checker_edge_size, expected_value,
              (hbool_t)TRUE);
...
ckrbrd_hs_dr_pio_test__d2m_s2l(struct hs_dr_pio_test_vars_t *tv_ptr) {
  ...
  ckrbrd_hs_dr_pio_test__slct_ckrbrd(
      tv_ptr->mpi_rank, tv_ptr->file_small_ds_sid_0, tv_ptr->small_rank,
      tv_ptr->edge_size, tv_ptr->checker_edge_size, tv_ptr->small_rank - 1,
      sel_start);
    ...
    do {
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
          ...
          ckrbrd_hs_dr_pio_test__slct_ckrbrd(
              tv_ptr->mpi_rank, tv_ptr->mem_large_ds_sid, tv_ptr->large_rank,
              tv_ptr->edge_size, tv_ptr->checker_edge_size,
              tv_ptr->small_rank - 1, tv_ptr->start);
...
#if CHECKER_BOARD_HS_DR_PIO_TEST__D2M_S2L__DEBUG
          {
            ...
            fprintf(stdout, "%s:%d: expected_value = %d.\n", fcnName,
                    tv_ptr->mpi_rank, expected_value);
            fprintf(stdout, "%s:%d: start/stop index = %d/%d.\n", fcnName,
                    tv_ptr->mpi_rank, start_index, stop_index);
            n = 0;
          ...
          data_ok = ckrbrd_hs_dr_pio_test__verify_data(
              ptr_1, tv_ptr->small_rank - 1, tv_ptr->edge_size,
...
ckrbrd_hs_dr_pio_test__m2d_l2s(struct hs_dr_pio_test_vars_t *tv_ptr) {
  ...
  ckrbrd_hs_dr_pio_test__slct_ckrbrd(
      tv_ptr->mpi_rank, tv_ptr->file_small_ds_sid_1, tv_ptr->small_rank,
      tv_ptr->edge_size, tv_ptr->checker_edge_size, tv_ptr->small_rank - 1,
      sel_start);
...
#if CHECKER_BOARD_HS_DR_PIO_TEST__M2D_L2S__DEBUG
  fprintf(stdout,
          "%s writing checker boards selections of slices from big ds to "
          "slices of small ds on disk.\n",
          fcnName);
#endif /* CHECKER_BOARD_HS_DR_PIO_TEST__M2D_L2S__DEBUG */
    ...
    do {
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
           ...
           */
          check = H5Sselect_shape_same(tv_ptr->file_small_ds_sid_1,
                                       tv_ptr->mem_large_ds_sid);
          VRFY((check == TRUE), "H5Sselect_shape_same passed.");
...
#endif /* CHECKER_BOARD_HS_DR_PIO_TEST__M2D_L2S__DEBUG */
          ret = H5Dwrite(tv_ptr->small_dataset, H5T_NATIVE_UINT32,
                         tv_ptr->mem_large_ds_sid, tv_ptr->file_small_ds_sid_1,
                         tv_ptr->xfer_plist, tv_ptr->large_ds_buf_0);
          VRFY((ret >= 0), "H5Dwrite() slice to large ds succeeded.");
          ...
          data_ok &= ckrbrd_hs_dr_pio_test__verify_data(
              tv_ptr->small_ds_buf_1 + start_index, tv_ptr->small_rank - 1,
              tv_ptr->edge_size, tv_ptr->checker_edge_size, expected_value,
              (hbool_t)TRUE);
...
ckrbrd_hs_dr_pio_test__m2d_s2l(struct hs_dr_pio_test_vars_t *tv_ptr) {
  ...
  ret = H5Sselect_hyperslab(tv_ptr->mem_large_ds_sid, H5S_SELECT_SET,
                            ...
                            tv_ptr->block);
  VRFY((ret >= 0),
       "H5Sselect_hyperslab(tv_ptr->mem_large_ds_sid, set) succeeded");
  ...
  ckrbrd_hs_dr_pio_test__slct_ckrbrd(tv_ptr->mpi_rank, tv_ptr->mem_small_ds_sid,
                                     tv_ptr->small_rank, tv_ptr->edge_size,
                                     tv_ptr->checker_edge_size,
                                     tv_ptr->small_rank - 1, sel_start);
...
#if CHECKER_BOARD_HS_DR_PIO_TEST__M2D_S2L__DEBUG
  fprintf(stdout,
          "%s writing process checkerboard selections of slices of small ds to "
          "process slices of large "
          "ds on disk.\n",
          fcnName);
    ...
    do {
      ...
      do {
        if ((tv_ptr->skips)++ < tv_ptr->max_skips) { /* skip the test */
          ...
          ckrbrd_hs_dr_pio_test__slct_ckrbrd(
              tv_ptr->mpi_rank, tv_ptr->file_large_ds_sid_1, tv_ptr->large_rank,
              tv_ptr->edge_size, tv_ptr->checker_edge_size,
              tv_ptr->small_rank - 1, tv_ptr->start);
...
#endif /* CHECKER_BOARD_HS_DR_PIO_TEST__M2D_S2L__DEBUG */
          ret = H5Dwrite(tv_ptr->large_dataset, H5T_NATIVE_UINT32,
                         tv_ptr->mem_small_ds_sid, tv_ptr->file_large_ds_sid_1,
                         tv_ptr->xfer_plist, tv_ptr->small_ds_buf_0);
          VRFY((ret != FAIL),
               "H5Dwrite of small ds slice to large ds succeeded");
          ...
          data_ok &= ckrbrd_hs_dr_pio_test__verify_data(
              tv_ptr->large_ds_buf_1 + start_index, tv_ptr->small_rank - 1,
              tv_ptr->edge_size, tv_ptr->checker_edge_size, expected_value,
              (hbool_t)TRUE);
...
static void ckrbrd_hs_dr_pio_test__run_test(
    const int test_num, const int edge_size, const int checker_edge_size,
...

{
  ...
  hs_dr_pio_test__setup(test_num, edge_size, checker_edge_size, chunk_edge_size,
                        small_rank, large_rank, use_collective_io, dset_type,
                        express_test, tv_ptr);
...
static void ckrbrd_hs_dr_pio_test(ShapeSameTestMethods sstest_type) {
  int express_test;
  int local_express_test;
  int mpi_size = -1;
  int mpi_rank = -1;
  ...
  for (large_rank = 3; large_rank <= PAR_SS_DR_MAX_RANK; large_rank++) {
    ...
    for (small_rank = 2; small_rank < large_rank; small_rank++) {
      ...
      case IND_CONTIG:
        ...
        chunk_edge_size = 0;
        ckrbrd_hs_dr_pio_test__run_test(
            test_num, edge_size, checker_edge_size, chunk_edge_size, small_rank,
            large_rank, FALSE, dset_type, express_test, &skips, max_skips,
            &total_tests, &tests_run, &tests_skipped, mpi_rank);
        test_num++;
        break;
        /* end of case IND_CONTIG */
      ...
      case COL_CONTIG:
        ...
        ckrbrd_hs_dr_pio_test__run_test(
            test_num, edge_size, checker_edge_size, chunk_edge_size, small_rank,
            large_rank, TRUE, dset_type, express_test, &skips, max_skips,
            &total_tests, &tests_run, &tests_skipped, mpi_rank);
        test_num++;
        break;
        /* end of case COL_CONTIG */
      ...
      if ((MAINPROCESS) && (tests_skipped > 0)) {
        fprintf(stdout,
                "     run/skipped/total = %" PRId64 "/%" PRId64 "/%" PRId64
                ".\n",
...
/* Shape Same test using contiguous hyperslab using independent IO on contiguous
 * datasets */
static void sscontig1(void) { contig_hs_dr_pio_test(IND_CONTIG); }

/* Shape Same test using contiguous hyperslab using collective IO on contiguous
 * datasets */
static void sscontig2(void) { contig_hs_dr_pio_test(COL_CONTIG); }

/* Shape Same test using contiguous hyperslab using independent IO on chunked
 * datasets */
static void sscontig3(void) { contig_hs_dr_pio_test(IND_CHUNKED); }

/* Shape Same test using contiguous hyperslab using collective IO on chunked
 * datasets */
static void sscontig4(void) { contig_hs_dr_pio_test(COL_CHUNKED); }

/* Shape Same test using checker hyperslab using independent IO on contiguous
 * datasets */
static void sschecker1(void) { ckrbrd_hs_dr_pio_test(IND_CONTIG); }

/* Shape Same test using checker hyperslab using collective IO on contiguous
 * datasets */
static void sschecker2(void) { ckrbrd_hs_dr_pio_test(COL_CONTIG); }

/* Shape Same test using checker hyperslab using independent IO on chunked
 * datasets */
static void sschecker3(void) { ckrbrd_hs_dr_pio_test(IND_CHUNKED); }

/* Shape Same test using checker hyperslab using collective IO on chunked
 * datasets */
static void sschecker4(void) { ckrbrd_hs_dr_pio_test(COL_CHUNKED); }
...
int main(int argc, char **argv) {
  ...
  if (H5Pget_vol_cap_flags(fapl, &vol_cap_flags_g) < 0) {
    if (MAINPROCESS)
      printf("Failed to get the capability flag of the VOL connector being "
             "used\n");
  ...
  /* Make sure the connector supports the API functions being tested.  This test
   * only uses a few API functions, such as H5Fcreate/close/delete,
   * H5Dcreate/write/read/close,
   */
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC)) {
    if (MAINPROCESS)
      printf("API functions for basic file and dataset aren't supported with "
             "this connector\n");
