 ...
 *-------------------------------------------------------------------------
 */
static void filter_read_internal(const char *filename, hid_t dcpl,
                                 hsize_t *dset_size) {
  ...
  if (MAINPROCESS) {
    ...
    file = H5Fcreate(h5_rmprefix(filename), H5F_ACC_TRUNC, H5P_DEFAULT,
                     H5P_DEFAULT);
    VRFY(file >= 0, "H5Fcreate");

    /* Create the dataset */
    dataset = H5Dcreate2(file, name, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT);
    VRFY(dataset >= 0, "H5Dcreate2");

    hrc = H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   points);
    VRFY(hrc >= 0, "H5Dwrite");
#if 0
  ...
  hrc =
      H5Sselect_hyperslab(sid, H5S_SELECT_SET, hs_offset, NULL, hs_size, NULL);
...
void test_filter_read(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      puts("SKIPPED");
      printf("    API functions for basic file, dataset or filter aren't "
