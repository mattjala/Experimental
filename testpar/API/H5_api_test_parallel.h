...
/*
 * Macros to surround an action that will be performed non-collectively. Once
 * the operation has completed, a consensus will be formed by all ranks on
...
#define END_INDEPENDENT_OP(op_name)                                            \
                                                                               ...
                                                                               \
  op_##op_name##_end                                                           \
      : if (MPI_SUCCESS != MPI_Allreduce(MPI_IN_PLACE, &ind_op_failed, 1,      \
                                         MPI_C_BOOL, MPI_LOR,                  \
                                         MPI_COMM_WORLD)) {                    \
    if (MAINPROCESS)                                                           \
      printf("    failed to collect consensus about whether non-collective "   \
  ...
  if (ind_op_failed) {                                                         \
    if (MAINPROCESS)                                                           \
      printf("    failure detected during non-collective operation - all "     \
             "other ranks will now fail "                                      \
             "too\n");                                                         \
    goto error;                                                                \
  }                                                                            \
  }
