...
const char *FILENAME[3] = {"bigio_test.h5", "single_rank_independent_io.h5",
                           NULL};
 ...
 */
static void set_coords(hsize_t start[], hsize_t count[], hsize_t stride[],
 ...
 */
static void fill_datasets(hsize_t start[], hsize_t block[],
                          B_DATATYPE *dataset) {
...
void point_set(hsize_t start[], hsize_t count[], hsize_t stride[],
  ...
  if (VERBOSE_MED) {
    printf("start[]=(%" PRIuHSIZE ", %" PRIuHSIZE "), "
           ...
           block[0], block[1], block[0] * block[1] * count[0] * count[1]);
    k = 0;
 ...
 */
static void dataset_print(hsize_t start[], hsize_t block[],
                          B_DATATYPE *dataset) {
  B_DATATYPE *dataptr = dataset;
  hsize_t i, j;
 ...
 * Print the content of the dataset.
 */
static int verify_data(hsize_t start[], hsize_t count[], hsize_t stride[],
                       hsize_t block[], B_DATATYPE *dataset,
                       B_DATATYPE *original) {
  hsize_t i, j;
  int vrfyerrs;
...
static void ccslab_set(int mpi_rank, int mpi_size, hsize_t start[],
  ...
  case BYROW_SELECTUNBALANCE:
    ...
    if ((mpi_rank * 3) < (mpi_size * 2))
      ...
      start[0] = 1 + space_dim1 * (hsize_t)mpi_size / 2 +
                 (hsize_t)(mpi_rank - 2 * mpi_size / 3);
    break;
...
static void dataset_big_write(void) {
  ...
  dataset = H5Dcreate2(fid, DATASET1, H5T_NATIVE_LLONG, sid, H5P_DEFAULT,
  ...
  sid = H5Screate_simple(RANK, dims, NULL);
  VRFY_G((sid >= 0), "H5Screate_simple succeeded");
  dataset = H5Dcreate2(fid, DATASET2, H5T_NATIVE_LLONG, sid, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  VRFY_G((dataset >= 0), "H5Dcreate2 succeeded");
  H5Sclose(sid);
  ...
  sid = H5Screate_simple(RANK, dims, NULL);
  VRFY_G((sid >= 0), "H5Screate_simple succeeded");
  dataset = H5Dcreate2(fid, DATASET3, H5T_NATIVE_LLONG, sid, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  VRFY_G((dataset >= 0), "H5Dcreate2 succeeded");
  H5Sclose(sid);
  ...
  sid = H5Screate_simple(RANK, dims, NULL);
  VRFY_G((sid >= 0), "H5Screate_simple succeeded");
  dataset = H5Dcreate2(fid, DATASET4, H5T_NATIVE_LLONG, sid, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  VRFY_G((dataset >= 0), "H5Dcreate2 succeeded");
  H5Sclose(sid);
...
static void single_rank_independent_io(void) {
  ...
  if (MAIN_PROCESS) {
    ...
    /* Write data */
    ret = H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_BLOCK, fspace_id, H5P_DEFAULT,
                   data);
    ...
    /* Read data back */
    ret = H5Dread(dset_id, H5T_NATIVE_INT, H5S_BLOCK, fspace_id, H5P_DEFAULT,
                  data);
...
void coll_chunk3(void) {
  ...
  coll_chunktest(filename, mpi_size_g, BYROW_CONT, API_NONE, HYPER, HYPER,
                 OUT_OF_ORDER);
  coll_chunktest(filename, mpi_size_g, BYROW_CONT, API_NONE, HYPER, POINT,
                 OUT_OF_ORDER);
  coll_chunktest(filename, mpi_size_g, BYROW_CONT, API_NONE, POINT, ALL,
                 OUT_OF_ORDER);
  coll_chunktest(filename, mpi_size_g, BYROW_CONT, API_NONE, POINT, POINT,
                 OUT_OF_ORDER);
  coll_chunktest(filename, mpi_size_g, BYROW_CONT, API_NONE, POINT, HYPER,
                 OUT_OF_ORDER);
...
static void coll_chunktest(const char *filename, int chunk_factor,
  ...
  ccslab_set(mpi_rank_g, mpi_size_g, start, count, stride, block,
             select_factor);
  ...
  if (facc_type == FACC_MPIO) {
    ...
    case API_MULTI_IND:
      status = H5Pget(xfer_plist, H5D_XFER_COLL_CHUNK_MULTI_RATIO_IND_NAME,
                      &prop_value);
      VRFY_G((status >= 0), "testing property list get succeeded");
      VRFY_G((prop_value == 0), "API to set MULTI-CHUNK IO transferring to "
...
int main(int argc, char **argv) {
   ...
   * only uses a few API functions, such as H5Fcreate/open/close/delete,
   * H5Dcreate/write/read/close, and H5Dget_space. */
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) ||
      !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_MORE)) {
    if (MAIN_PROCESS)
      printf("API functions for basic file, dataset basic or more aren't "
