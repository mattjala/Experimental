...
void file_image_daisy_chain_test(void) {
  ...
  if (mpi_rank == 0) {
    ...
    dset_id = H5Dcreate2(file_id, "v", H5T_NATIVE_INT, space_id, H5P_DEFAULT,
    ...
    mpi_result = MPI_Ssend((void *)image_ptr, (int)image_len, MPI_BYTE, 1, 0,
                           MPI_COMM_WORLD);
    ...
    mpi_result = MPI_Recv((void *)(&image_len), (int)sizeof(ssize_t), MPI_BYTE,
                          mpi_size - 1, 0, MPI_COMM_WORLD, &rcvstat);
    VRFY((mpi_result == MPI_SUCCESS), "received image len from process n-1");

    image_ptr = (void *)malloc((size_t)image_len);
    VRFY(image_ptr != NULL, "allocated file image receive buffer.");

    mpi_result = MPI_Recv((void *)image_ptr, (int)image_len, MPI_BYTE,
                          mpi_size - 1, 0, MPI_COMM_WORLD, &rcvstat);
    ...
    err = H5Dread(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                  (void *)vector_ptr);
    ...
    mpi_result = MPI_Recv((void *)(&image_len), (int)sizeof(ssize_t), MPI_BYTE,
                          mpi_rank - 1, 0, MPI_COMM_WORLD, &rcvstat);
    ...
    mpi_result = MPI_Ssend((void *)image_ptr, (int)image_len, MPI_BYTE,
                           (mpi_rank + 1) % mpi_size, 0, MPI_COMM_WORLD);
    VRFY((mpi_result == MPI_SUCCESS),
