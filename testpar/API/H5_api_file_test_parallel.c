...
static int test_create_file(void) {
  ...
  if ((file_id = H5Fcreate(FILE_CREATE_TEST_FILENAME, H5F_ACC_TRUNC,
...
static int test_open_file(void) {
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fopen_rdonly) {
      ...
      if ((file_id = H5Fopen(H5_api_test_parallel_filename, H5F_ACC_RDONLY,
                             fapl_id)) < 0) {
        H5_FAILED();
...
static int test_split_comm_file_access(void) {
  ...
  if (is_old) {
    ...
    if ((file_id = H5Fcreate(SPLIT_FILE_COMM_TEST_FILE_NAME, H5F_ACC_TRUNC,
                             H5P_DEFAULT, fapl_id)) < 0) {
      H5_FAILED();
...
access_end:
  ...
  if (MPI_SUCCESS != MPI_Allreduce(MPI_IN_PLACE, &err_occurred, 1, MPI_INT,
  ...
  if (err_occurred) {
    H5_FAILED();
    printf("    an error occurred on only some ranks during split-communicator "
           "file access! - "
