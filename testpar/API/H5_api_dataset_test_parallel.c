...
#define DATASET_WRITE_DATA_VERIFY_TEST_GROUP_NAME                              \
  "dataset_write_data_verification_test"
#define DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME1                              \
  "dataset_write_data_verification_all"
#define DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME2                              \
  "dataset_write_data_verification_hyperslab"
#define DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME3                              \
  "dataset_write_data_verification_points"
static int test_write_dataset_data_verification(void) {
  ...
  if ((file_id =
           H5Fopen(H5_api_test_parallel_filename, H5F_ACC_RDWR, fapl_id)) < 0) {
    H5_FAILED();
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_DATA_VERIFY_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(DATASET_WRITE_DATA_VERIFY_TEST_SPACE_RANK,
                                    dims, NULL)) < 0)
    TEST_ERROR;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Dwrite_all_read) {
      ...
      if ((dset_id =
               H5Dopen2(group_id, DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME1,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED();
      ...
      /*
       * Write data to dataset on rank 0 only. All ranks will read the data
       * back.
       */
      if (MAINPROCESS) {
        for (i = 0, data_size = 1;
             i < DATASET_WRITE_DATA_VERIFY_TEST_SPACE_RANK; i++)
          data_size *= dims[i];
        data_size *= DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE;

        if (NULL != (write_buf = malloc(data_size))) {
          for (i = 0; i < data_size / DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE;
               i++)
            ((int *)write_buf)[i] = (int)i;

          if (H5Dwrite(dset_id, DATASET_WRITE_DATA_VERIFY_TEST_DSET_DTYPE,
      ...
      if (MPI_SUCCESS != MPI_Allreduce(MPI_IN_PLACE, &op_failed, 1, MPI_C_BOOL,
      ...
      if ((file_id = H5Fopen(H5_api_test_parallel_filename, H5F_ACC_RDWR,
                             fapl_id)) < 0) {
        H5_FAILED();
      ...
      if ((group_id = H5Gopen2(container_group,
                               DATASET_WRITE_DATA_VERIFY_TEST_GROUP_NAME,
                               H5P_DEFAULT)) < 0) {
        H5_FAILED();
      ...
      if (NULL ==
          (read_buf = malloc((hsize_t)space_npoints *
                             DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE))) {
        H5_FAILED();
        printf("    couldn't allocate buffer for dataset read\n");
        PART_ERROR(H5Dwrite_all_read);
      }

      if (H5Dread(dset_id, DATASET_WRITE_DATA_VERIFY_TEST_DSET_DTYPE, H5S_ALL,
    ...
    PART_BEGIN(H5Dwrite_hyperslab_read) {
      ...
      for (i = 1, data_size = 1; i < DATASET_WRITE_DATA_VERIFY_TEST_SPACE_RANK;
           i++)
        data_size *= dims[i];
      data_size *= DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE;
      ...
      if ((dset_id =
               H5Dopen2(group_id, DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME2,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't open dataset '%s'\n",
               DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME2);
      ...
      for (i = 0; i < (size_t)mpi_size; i++) {
        ...
        for (j = 0; j < data_size / DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE;
             j++) {
          if (((int *)read_buf)
                  [j + (i * (data_size /
                             DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE))] !=
    ...
    PART_BEGIN(H5Dwrite_point_sel_read) {
      ...
      /* Use different data than the previous test to ensure that the data
       * actually changed. */
      for (i = 0; i < data_size / DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE;
           i++)
        ((int *)write_buf)[i] = mpi_size - mpi_rank;

      if (NULL == (points = malloc(
                       DATASET_WRITE_DATA_VERIFY_TEST_SPACE_RANK *
                       (data_size / DATASET_WRITE_DATA_VERIFY_TEST_DTYPE_SIZE) *
                       sizeof(hsize_t)))) {
        H5_FAILED();
      ...
      if ((dset_id =
               H5Dopen2(group_id, DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME3,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't open dataset '%s'\n",
               DATASET_WRITE_DATA_VERIFY_TEST_DSET_NAME3);
...
#define DATASET_INDEPENDENT_WRITE_TEST_GROUP_NAME                              \
  "independent_dataset_write_test"
...
static int test_write_dataset_independent(void) {
   ...
   */
  if (generate_random_parallel_dimensions(
          DATASET_INDEPENDENT_WRITE_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(DATASET_INDEPENDENT_WRITE_TEST_SPACE_RANK,
                                    dims, NULL)) < 0)
    TEST_ERROR;

  /* create a dataset collectively */
  if ((dset_id1 =
           H5Dcreate2(group_id, DATASET_INDEPENDENT_WRITE_TEST_DSET_NAME1,
                      DATASET_INDEPENDENT_WRITE_TEST_DSET_DTYPE, fspace_id,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
  ...
  if ((dset_id2 =
           H5Dcreate2(group_id, DATASET_INDEPENDENT_WRITE_TEST_DSET_NAME2,
                      DATASET_INDEPENDENT_WRITE_TEST_DSET_DTYPE, fspace_id,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
  ...
  BEGIN_INDEPENDENT_OP(dset_write) {
    if (mpi_rank % 2 == 0) {
      if (H5Dwrite(dset_id1, DATASET_INDEPENDENT_WRITE_TEST_DSET_DTYPE,
                   mspace_id, fspace_id, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    even ranks failed to write to dataset 1\n");
        INDEPENDENT_OP_ERROR(dset_write);
      }
      if (H5Dwrite(dset_id2, DATASET_INDEPENDENT_WRITE_TEST_DSET_DTYPE,
                   mspace_id, fspace_id, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
  ...
  if ((group_id =
           H5Gopen2(container_group, DATASET_INDEPENDENT_WRITE_TEST_GROUP_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open container sub-group '%s'\n",
           DATASET_INDEPENDENT_WRITE_TEST_GROUP_NAME);
  ...
  if ((dset_id1 = H5Dopen2(group_id, DATASET_INDEPENDENT_WRITE_TEST_DSET_NAME1,
                           H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
  ...
  if ((dset_id2 = H5Dopen2(group_id, DATASET_INDEPENDENT_WRITE_TEST_DSET_NAME2,
                           H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
  ...
  if (NULL == (read_buf = malloc((hsize_t)space_npoints *
                                 DATASET_INDEPENDENT_WRITE_TEST_DTYPE_SIZE))) {
    H5_FAILED();
    printf("    couldn't allocate buffer for dataset read\n");
    goto error;
  }

  if (H5Dread(dset_id1, DATASET_INDEPENDENT_WRITE_TEST_DSET_DTYPE, H5S_ALL,
              H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_INDEPENDENT_WRITE_TEST_DSET_NAME1);
  ...
  for (i = 0; i < (size_t)mpi_size; i++) {
    ...
    for (j = 0; j < data_size / DATASET_INDEPENDENT_WRITE_TEST_DTYPE_SIZE;
         j++) {
      if (((int *)
               read_buf)[j +
                         (i * (data_size /
  ...
  if (H5Dread(dset_id2, DATASET_INDEPENDENT_WRITE_TEST_DSET_DTYPE, H5S_ALL,
              H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_INDEPENDENT_WRITE_TEST_DSET_NAME2);
...
static int test_write_dataset_one_proc_0_selection(void) {
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_ONE_PROC_0_SEL_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_ONE_PROC_0_SEL_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id =
           H5Dcreate2(group_id, DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_NAME,
                      DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_DTYPE, fspace_id,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't create dataset '%s'\n",
  ...
  for (i = 1, data_size = 1; i < DATASET_WRITE_ONE_PROC_0_SEL_TEST_SPACE_RANK;
       i++)
    data_size *= dims[i];
  ...
  BEGIN_INDEPENDENT_OP(write_buf_alloc) {
    if (!MAINPROCESS) {
      ...
      for (i = 0; i < data_size / DATASET_WRITE_ONE_PROC_0_SEL_TEST_DTYPE_SIZE;
           i++)
        ((int *)write_buf)[i] = mpi_rank;
    }
  }
  ...
  {
    hsize_t mdims[] = {data_size /
                       DATASET_WRITE_ONE_PROC_0_SEL_TEST_DTYPE_SIZE};
  ...
  BEGIN_INDEPENDENT_OP(dset_write) {
    if (H5Dwrite(dset_id, DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_DTYPE,
                 mspace_id, fspace_id, H5P_DEFAULT, write_buf) < 0) {
      H5_FAILED();
      printf("    couldn't write to dataset '%s'\n",
             DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_NAME);
  ...
  if ((dset_id = H5Dopen2(group_id, DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_NAME,
                          H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_NAME);
  ...
  if (H5Dread(dset_id, DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_DTYPE, H5S_ALL,
              H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_WRITE_ONE_PROC_0_SEL_TEST_DSET_NAME);
  ...
  for (i = 0; i < (size_t)mpi_size; i++) {
    ...
    if (i != 0) {
      for (j = 0; j < data_size / DATASET_WRITE_ONE_PROC_0_SEL_TEST_DTYPE_SIZE;
           j++) {
        if (((int *)read_buf)
                [j + (i * (data_size /
                           DATASET_WRITE_ONE_PROC_0_SEL_TEST_DTYPE_SIZE))] !=
...
#define DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_GROUP_NAME                        \
  "one_rank_none_sel_write_test"
...
static int test_write_dataset_one_proc_none_selection(void) {
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id =
           H5Dcreate2(group_id, DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DSET_NAME,
  ...
  for (i = 1, data_size = 1;
       i < DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_SPACE_RANK; i++)
    data_size *= dims[i];
  ...
  BEGIN_INDEPENDENT_OP(write_buf_alloc) {
    if (!MAINPROCESS) {
      ...
      for (i = 0;
           i < data_size / DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DTYPE_SIZE; i++)
        ((int *)write_buf)[i] = mpi_rank;
    }
  }
  END_INDEPENDENT_OP(write_buf_alloc);
  ...
  {
    hsize_t mdims[] = {data_size /
                       DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DTYPE_SIZE};
  ...
  BEGIN_INDEPENDENT_OP(dset_write) {
    if (H5Dwrite(dset_id, DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DSET_DTYPE,
                 mspace_id, fspace_id, H5P_DEFAULT, write_buf) < 0) {
      H5_FAILED();
      printf("    couldn't write to dataset '%s'\n",
             DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DSET_NAME);
  ...
  if (H5Dread(dset_id, DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DSET_DTYPE, H5S_ALL,
              H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DSET_NAME);
  ...
  for (i = 0; i < (size_t)mpi_size; i++) {
    ...
    if (i != 0) {
      for (j = 0;
           j < data_size / DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DTYPE_SIZE;
           j++) {
        if (((int *)read_buf)
                [j + (i * (data_size /
                           DATASET_WRITE_ONE_PROC_NONE_SEL_TEST_DTYPE_SIZE))] !=
...
#define DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_GROUP_NAME                         \
  "one_rank_all_sel_write_test"
...
static int test_write_dataset_one_proc_all_selection(void) {
  ...
  TESTING("write to dataset with one rank using all selection; others none "
          "selection");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id =
           H5Dcreate2(group_id, DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_NAME,
                      DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_DTYPE, fspace_id,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't create dataset '%s'\n",
  ...
  for (i = 0, data_size = 1; i < DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_SPACE_RANK;
       i++)
    data_size *= dims[i];
  ...
  BEGIN_INDEPENDENT_OP(write_buf_alloc) {
    if (MAINPROCESS) {
      ...
      for (i = 0;
           i < data_size / DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DTYPE_SIZE; i++)
        ((int *)write_buf)[i] = (int)i;
    }
  }
  END_INDEPENDENT_OP(write_buf_alloc);
  ...
  {
    hsize_t mdims[] = {data_size /
                       DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DTYPE_SIZE};
  ...
  BEGIN_INDEPENDENT_OP(dset_write) {
    if (H5Dwrite(dset_id, DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_DTYPE,
                 mspace_id, fspace_id, H5P_DEFAULT, write_buf) < 0) {
      H5_FAILED();
      printf("    couldn't write to dataset '%s'\n",
             DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_NAME);
  ...
  if (H5Dread(dset_id, DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_DTYPE, H5S_ALL,
              H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DSET_NAME);
  ...
  for (i = 0; i < data_size / DATASET_WRITE_ONE_PROC_ALL_SEL_TEST_DTYPE_SIZE;
       i++) {
    if (((int *)read_buf)[i] != (int)i) {
      H5_FAILED();
      printf("    data verification failed\n");
...
#define DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_GROUP_NAME                       \
  "hyper_sel_file_all_sel_mem_write_test"
#define DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DSET_NAME                        \
  "hyper_sel_file_all_sel_mem_dset"
static int test_write_dataset_hyper_file_all_mem(void) {
#ifdef BROKEN
  hssize_t space_npoints;
  hsize_t *dims = NULL;
  ...
  TESTING("write to dataset with hyperslab sel. for file space; all sel. for "
          "memory");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id =
           H5Dcreate2(group_id, DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DSET_NAME,
  ...
  for (i = 1, data_size = 1;
       i < DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_SPACE_RANK; i++)
    data_size *= dims[i];
  ...
  for (i = 0; i < data_size / DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DTYPE_SIZE;
       i++)
    ((int *)write_buf)[i] = mpi_rank;
  ...
  if (H5Dwrite(dset_id, DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DSET_DTYPE,
               H5S_ALL, fspace_id, H5P_DEFAULT, write_buf) < 0) {
    H5_FAILED();
    printf("    couldn't write to dataset '%s'\n",
           DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DSET_NAME);
    goto error;
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DSET_NAME);
  ...
  if (NULL ==
      (read_buf = malloc((hsize_t)space_npoints *
                         DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DTYPE_SIZE))) {
    H5_FAILED();
    printf("    couldn't allocate buffer for dataset read\n");
    goto error;
  }

  if (H5Dread(dset_id, DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DSET_DTYPE,
  ...
  for (i = 0; i < (size_t)mpi_size; i++) {
    ...
    for (j = 0;
         j < data_size / DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DTYPE_SIZE;
         j++) {
      if (((int *)read_buf)
              [j + (i * (data_size /
                         DATASET_WRITE_HYPER_FILE_ALL_MEM_TEST_DTYPE_SIZE))] !=
...
#define DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_GROUP_NAME                       \
  "all_sel_file_hyper_sel_mem_write_test"
#define DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_NAME                        \
  "all_sel_file_hyper_sel_mem_dset"
static int test_write_dataset_all_file_hyper_mem(void) {
  hssize_t space_npoints;
  hsize_t *dims = NULL;
  size_t i, data_size;
  ...
  TESTING("write to dataset with all sel. for file space; hyperslab sel. for "
          "memory");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id =
           H5Dcreate2(group_id, DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_NAME,
  ...
  for (i = 0, data_size = 1;
       i < DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_SPACE_RANK; i++)
    data_size *= dims[i];
  ...
  BEGIN_INDEPENDENT_OP(write_buf_alloc) {
    if (MAINPROCESS) {
      /*
       * Allocate twice the amount of memory needed and leave "holes" in the
       * memory buffer in order to prove that the mapping from hyperslab
       * selection <-> all selection works correctly.
       */
      ...
      for (i = 0; i < 2 * (data_size /
                           DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DTYPE_SIZE);
           i++) {
  ...
  /*
   * Only have rank 0 perform the dataset write, as writing the entire dataset
   * on all ranks might be stressful on system resources. There's also no
   * guarantee as to what the outcome would be, since the writes would be
  ...
  BEGIN_INDEPENDENT_OP(dset_write) {
    if (MAINPROCESS) {
      hsize_t start[1] = {0};
      hsize_t stride[1] = {2};
      hsize_t count[1] = {data_size /
                          DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DTYPE_SIZE};
      hsize_t block[1] = {1};
      hsize_t mdims[] = {
          2 * (data_size / DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DTYPE_SIZE)};
      ...
      if (H5Sselect_hyperslab(mspace_id, H5S_SELECT_SET, start, stride, count,
                              block) < 0) {
        H5_FAILED();
        printf("    couldn't select hyperslab for dataset write\n");
        INDEPENDENT_OP_ERROR(dset_write);
      ...
      if (H5Dwrite(dset_id, DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_NAME);
        INDEPENDENT_OP_ERROR(dset_write);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  if (H5Dread(dset_id, DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_DTYPE,
              H5S_ALL, H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  for (i = 0; i < data_size / DATASET_WRITE_ALL_FILE_HYPER_MEM_TEST_DTYPE_SIZE;
       i++) {
    if (((int *)read_buf)[i] != (int)i) {
      H5_FAILED();
      printf("    data verification failed\n");
...
#define DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_GROUP_NAME                       \
  "all_sel_file_point_sel_mem_write_test"
#define DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_NAME                        \
  "all_sel_file_point_sel_mem_dset"
static int test_write_dataset_all_file_point_mem(void) {
  hssize_t space_npoints;
  hsize_t *points = NULL;
  hsize_t *dims = NULL;
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id =
           H5Dcreate2(group_id, DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_NAME,
  ...
  for (i = 0, data_size = 1;
       i < DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_SPACE_RANK; i++)
    data_size *= dims[i];
  ...
  BEGIN_INDEPENDENT_OP(write_buf_alloc) {
    if (MAINPROCESS) {
      /*
       * Allocate twice the amount of memory needed and leave "holes" in the
       * memory buffer in order to prove that the mapping from point selection
       * <-> all selection works correctly.
       */
      if (NULL == (write_buf = malloc(2 * data_size))) {
        H5_FAILED();
        printf("    couldn't allocate buffer for dataset write\n");
      ...
      for (i = 0; i < 2 * (data_size /
                           DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DTYPE_SIZE);
           i++) {
        /* Write actual data to even indices */
        if (i % 2 == 0)
          ((int *)write_buf)[i] = (int)((i / 2) + (i % 2));
  ...
  BEGIN_INDEPENDENT_OP(dset_write) {
    if (MAINPROCESS) {
      hsize_t mdims[] = {
          2 * (data_size / DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DTYPE_SIZE)};
      int j;
      ...
      if (H5Sselect_elements(
              mspace_id, H5S_SELECT_SET,
              data_size / DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DTYPE_SIZE,
              points) < 0) {
        H5_FAILED();
      ...
      if (H5Dwrite(dset_id, DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  if (H5Dread(dset_id, DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_DTYPE,
              H5S_ALL, H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  for (i = 0; i < data_size / DATASET_WRITE_ALL_FILE_POINT_MEM_TEST_DTYPE_SIZE;
       i++) {
    if (((int *)read_buf)[i] != (int)i) {
      H5_FAILED();
      printf("    data verification failed\n");
...
#define DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_GROUP_NAME                     \
  "hyper_sel_file_point_sel_mem_write_test"
#define DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_NAME                      \
  "hyper_sel_file_point_sel_mem_dset"
static int test_write_dataset_hyper_file_point_mem(void) {
  hssize_t space_npoints;
  hsize_t *dims = NULL;
  hsize_t *points = NULL;
  ...
  TESTING("write to dataset with hyperslab sel. for file space; point sel. for "
          "memory");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id = H5Dcreate2(
           group_id, DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_NAME,
           DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_DTYPE, fspace_id,
           H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't create dataset '%s'\n",
  ...
  for (i = 1, data_size = 1;
       i < DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_SPACE_RANK; i++)
    data_size *= dims[i];
   ...
   * buffer in order to prove that the mapping from point selection <->
   * hyperslab selection works correctly.
   */
  if (NULL == (write_buf = malloc(2 * data_size))) {
    H5_FAILED();
    printf("    couldn't allocate buffer for dataset write\n");
  ...
  for (i = 0;
       i < 2 * (data_size / DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DTYPE_SIZE);
       i++) {
    /* Write actual data to even indices */
    if (i % 2 == 0)
      ((int *)write_buf)[i] = mpi_rank;
  ...
  {
    hsize_t mdims[] = {
        2 * (data_size / DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DTYPE_SIZE)};
    int j;
  ...
  if (H5Dwrite(dset_id, DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_DTYPE,
               mspace_id, fspace_id, H5P_DEFAULT, write_buf) < 0) {
    H5_FAILED();
    printf("    couldn't write to dataset '%s'\n",
           DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  if (NULL ==
      (read_buf = malloc((hsize_t)space_npoints *
                         DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DTYPE_SIZE))) {
    H5_FAILED();
    printf("    couldn't allocate buffer for dataset read\n");
    goto error;
  }

  if (H5Dread(dset_id, DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_DTYPE,
              H5S_ALL, H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  for (i = 0; i < (size_t)mpi_size; i++) {
    ...
    for (j = 0;
         j < data_size / DATASET_WRITE_HYPER_FILE_POINT_MEM_TEST_DTYPE_SIZE;
         j++) {
      if (((int *)read_buf)
              [j +
               (i * (data_size /
...
#define DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_GROUP_NAME                     \
  "point_sel_file_hyper_sel_mem_write_test"
#define DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_NAME                      \
  "point_sel_file_hyper_sel_mem_dset"
static int test_write_dataset_point_file_hyper_mem(void) {
  hssize_t space_npoints;
  hsize_t *dims = NULL;
  hsize_t *points = NULL;
  ...
  TESTING("write to dataset with point sel. for file space; hyperslab sel. for "
          "memory");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  if ((fspace_id = H5Screate_simple(
           DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;

  if ((dset_id = H5Dcreate2(
           group_id, DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_NAME,
           DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_DTYPE, fspace_id,
           H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't create dataset '%s'\n",
  ...
  for (i = 1, data_size = 1;
       i < DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK; i++)
    data_size *= dims[i];
  data_size *= DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE;

  /*
   * Allocate twice the amount of memory needed and leave "holes" in the memory
   * buffer in order to prove that the mapping from hyperslab selection <->
   * point selection works correctly.
   */
  if (NULL == (write_buf = malloc(2 * data_size))) {
    H5_FAILED();
    printf("    couldn't allocate buffer for dataset write\n");
  ...
  for (i = 0;
       i < 2 * (data_size / DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE);
       i++) {
    /* Write actual data to even indices */
    if (i % 2 == 0)
      ((int *)write_buf)[i] = mpi_rank;
  ...
  if (NULL ==
      (points = malloc(
           (data_size / DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE) *
           DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK *
           sizeof(hsize_t)))) {
    H5_FAILED();
    printf("    couldn't allocate buffer for point selection\n");
    goto error;
  ...
  for (i = 0;
       i < data_size / DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE;
       i++) {
    size_t j;

    for (j = 0; j < DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK; j++) {
      size_t idx =
          (i * (size_t)DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK) + j;

      if (j == 0)
        points[idx] = (hsize_t)mpi_rank;
      else if (j !=
               (size_t)DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK - 1)
        points[idx] = i / dims[j + 1];
      else
        points[idx] = i % dims[j];
    }
  ...
  if (H5Sselect_elements(fspace_id, H5S_SELECT_SET,
                         data_size /
                             DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE,
                         points) < 0) {
    H5_FAILED();
    printf("    couldn't set point selection for dataset write\n");
    goto error;
...

  {
    ...
    hsize_t block[1] = {1};
    hsize_t mdims[] = {
        2 * (data_size / DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE)};
  ...
  if (H5Dwrite(dset_id, DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_DTYPE,
               mspace_id, fspace_id, H5P_DEFAULT, write_buf) < 0) {
    H5_FAILED();
    printf("    couldn't write to dataset '%s'\n",
           DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  if (H5Dread(dset_id, DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_DTYPE,
              H5S_ALL, H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  for (i = 0; i < (size_t)mpi_size; i++) {
    ...
    for (j = 0;
         j < data_size / DATASET_WRITE_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE;
         j++) {
      if (((int *)read_buf)
              [j +
               (i * (data_size /
...
static int test_read_dataset_one_proc_0_selection(void) {
  ...
  if (generate_random_parallel_dimensions(
          DATASET_READ_ONE_PROC_0_SEL_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      if ((fspace_id = H5Screate_simple(
               DATASET_READ_ONE_PROC_0_SEL_TEST_SPACE_RANK, dims, NULL)) < 0) {
        H5_FAILED();
      ...
      if ((dset_id = H5Dcreate2(
               group_id, DATASET_READ_ONE_PROC_0_SEL_TEST_DSET_NAME,
               DATASET_READ_ONE_PROC_0_SEL_TEST_DSET_DTYPE, fspace_id,
               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't create dataset '%s'\n",
      ...
      for (i = 0, data_size = 1;
           i < DATASET_READ_ONE_PROC_0_SEL_TEST_SPACE_RANK; i++)
        data_size *= dims[i];
      ...
      for (i = 0; i < (size_t)mpi_size; i++) {
        ...
        size_t elem_per_proc =
            (data_size / DATASET_READ_ONE_PROC_0_SEL_TEST_DTYPE_SIZE) / dims[0];
      ...
      {
        hsize_t mdims[] = {data_size /
                           DATASET_READ_ONE_PROC_0_SEL_TEST_DTYPE_SIZE};
      ...
      if (H5Dwrite(dset_id, DATASET_READ_ONE_PROC_0_SEL_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_READ_ONE_PROC_0_SEL_TEST_DSET_NAME);
  ...
  if ((dset_id = H5Dopen2(group_id, DATASET_READ_ONE_PROC_0_SEL_TEST_DSET_NAME,
                          H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_READ_ONE_PROC_0_SEL_TEST_DSET_NAME);
  ...
  BEGIN_INDEPENDENT_OP(read_buf_alloc) {
    if (!MAINPROCESS) {
      read_buf_size = ((size_t)(space_npoints / mpi_size) *
                       DATASET_READ_ONE_PROC_0_SEL_TEST_DTYPE_SIZE);
  ...
  BEGIN_INDEPENDENT_OP(dset_read) {
    if (H5Dread(dset_id, DATASET_READ_ONE_PROC_0_SEL_TEST_DSET_DTYPE, mspace_id,
...
#define DATASET_READ_ONE_PROC_NONE_SEL_TEST_GROUP_NAME                         \
  "one_rank_none_sel_read_test"
...
static int test_read_dataset_one_proc_none_selection(void) {
  ...
  if (generate_random_parallel_dimensions(
          DATASET_READ_ONE_PROC_NONE_SEL_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      if ((fspace_id =
               H5Screate_simple(DATASET_READ_ONE_PROC_NONE_SEL_TEST_SPACE_RANK,
                                dims, NULL)) < 0) {
        H5_FAILED();
        printf("    failed to create file dataspace for dataset\n");
        INDEPENDENT_OP_ERROR(dset_create);
      ...
      if ((dset_id = H5Dcreate2(
               group_id, DATASET_READ_ONE_PROC_NONE_SEL_TEST_DSET_NAME,
               DATASET_READ_ONE_PROC_NONE_SEL_TEST_DSET_DTYPE, fspace_id,
               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't create dataset '%s'\n",
      ...
      for (i = 0, data_size = 1;
           i < DATASET_READ_ONE_PROC_NONE_SEL_TEST_SPACE_RANK; i++)
        data_size *= dims[i];
      ...
      {
        hsize_t mdims[] = {data_size /
                           DATASET_READ_ONE_PROC_NONE_SEL_TEST_DTYPE_SIZE};
      ...
      if (H5Dwrite(dset_id, DATASET_READ_ONE_PROC_NONE_SEL_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_READ_ONE_PROC_NONE_SEL_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_READ_ONE_PROC_NONE_SEL_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_READ_ONE_PROC_NONE_SEL_TEST_DSET_NAME);
  ...
  BEGIN_INDEPENDENT_OP(dset_read) {
    if (H5Dread(dset_id, DATASET_READ_ONE_PROC_NONE_SEL_TEST_DSET_DTYPE,
...
#define DATASET_READ_ONE_PROC_ALL_SEL_TEST_GROUP_NAME                          \
  "one_rank_all_sel_read_test"
...
static int test_read_dataset_one_proc_all_selection(void) {
  ...
  TESTING("read from dataset with one rank using all selection; others none "
          "selection");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_READ_ONE_PROC_ALL_SEL_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      if ((fspace_id =
               H5Screate_simple(DATASET_READ_ONE_PROC_ALL_SEL_TEST_SPACE_RANK,
                                dims, NULL)) < 0) {
        H5_FAILED();
        printf("    failed to create file dataspace for dataset\n");
        INDEPENDENT_OP_ERROR(dset_create);
      ...
      if ((dset_id = H5Dcreate2(
               group_id, DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_NAME,
               DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_DTYPE, fspace_id,
               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't create dataset '%s'\n",
      ...
      for (i = 0, data_size = 1;
           i < DATASET_READ_ONE_PROC_ALL_SEL_TEST_SPACE_RANK; i++)
        data_size *= dims[i];
      ...
      {
        hsize_t mdims[] = {data_size /
                           DATASET_READ_ONE_PROC_ALL_SEL_TEST_DTYPE_SIZE};
      ...
      if (H5Dwrite(dset_id, DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_NAME);
  ...
  BEGIN_INDEPENDENT_OP(read_buf_alloc) {
    if (MAINPROCESS) {
      read_buf_size =
          (size_t)space_npoints * DATASET_READ_ONE_PROC_ALL_SEL_TEST_DTYPE_SIZE;
  ...
  BEGIN_INDEPENDENT_OP(dset_read) {
    if (H5Dread(dset_id, DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_DTYPE,
                mspace_id, fspace_id, H5P_DEFAULT, read_buf) < 0) {
      H5_FAILED();
      printf("    couldn't read from dataset '%s'\n",
             DATASET_READ_ONE_PROC_ALL_SEL_TEST_DSET_NAME);
 ...
 */
static int test_read_dataset_hyper_file_all_mem(void) {
  TESTING("read from dataset with hyperslab sel. for file space; all sel. for "
          "memory");
...
#define DATASET_READ_ALL_FILE_HYPER_MEM_TEST_GROUP_NAME                        \
  "all_sel_file_hyper_sel_mem_read_test"
#define DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_NAME                         \
  "all_sel_file_hyper_sel_mem_dset"
static int test_read_dataset_all_file_hyper_mem(void) {
  hssize_t space_npoints;
  hsize_t *dims = NULL;
  ...
  TESTING("read from dataset with all sel. for file space; hyperslab sel. for "
          "memory");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_READ_ALL_FILE_HYPER_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      if ((group_id = H5Gcreate2(
               container_group, DATASET_READ_ALL_FILE_HYPER_MEM_TEST_GROUP_NAME,
               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't create container sub-group '%s'\n",
      ...
      if ((fspace_id =
               H5Screate_simple(DATASET_READ_ALL_FILE_HYPER_MEM_TEST_SPACE_RANK,
                                dims, NULL)) < 0) {
        H5_FAILED();
        printf("    failed to create file dataspace for dataset\n");
        INDEPENDENT_OP_ERROR(dset_create);
      ...
      if ((dset_id = H5Dcreate2(
               group_id, DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_NAME,
               DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_DTYPE, fspace_id,
               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't create dataset '%s'\n",
      ...
      for (i = 0, data_size = 1;
           i < DATASET_READ_ALL_FILE_HYPER_MEM_TEST_SPACE_RANK; i++)
        data_size *= dims[i];
      ...
      {
        hsize_t mdims[] = {data_size /
                           DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DTYPE_SIZE};
      ...
      if (H5Dwrite(dset_id, DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  /*
   * Only have rank 0 perform the dataset read, as reading the entire dataset on
   * all ranks might be stressful on system resources.
   */
  BEGIN_INDEPENDENT_OP(dset_read) {
    if (MAINPROCESS) {
      hsize_t start[1] = {0};
      ...
      /*
       * Allocate twice the amount of memory needed and leave "holes" in the
       * memory buffer in order to prove that the mapping from all selection <->
       * hyperslab selection works correctly.
       */
      read_buf_size = (size_t)(2 * space_npoints) *
                      DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DTYPE_SIZE;
      ...
      if (H5Dread(dset_id, DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_DTYPE,
                  mspace_id, H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
        H5_FAILED();
        printf("    couldn't read from dataset '%s'\n",
               DATASET_READ_ALL_FILE_HYPER_MEM_TEST_DSET_NAME);
        INDEPENDENT_OP_ERROR(dset_read);
...
#define DATASET_READ_ALL_FILE_POINT_MEM_TEST_GROUP_NAME                        \
  "all_sel_file_point_sel_mem_read_test"
#define DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_NAME                         \
  "all_sel_file_point_sel_mem_dset"
static int test_read_dataset_all_file_point_mem(void) {
  hssize_t space_npoints;
  hsize_t *points = NULL;
  ...
  if (generate_random_parallel_dimensions(
          DATASET_READ_ALL_FILE_POINT_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      if ((group_id = H5Gcreate2(
               container_group, DATASET_READ_ALL_FILE_POINT_MEM_TEST_GROUP_NAME,
               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't create container sub-group '%s'\n",
      ...
      if ((fspace_id =
               H5Screate_simple(DATASET_READ_ALL_FILE_POINT_MEM_TEST_SPACE_RANK,
                                dims, NULL)) < 0) {
        H5_FAILED();
        printf("    failed to create file dataspace for dataset\n");
        INDEPENDENT_OP_ERROR(dset_create);
      ...
      if ((dset_id = H5Dcreate2(
               group_id, DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_NAME,
               DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_DTYPE, fspace_id,
               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        printf("    couldn't create dataset '%s'\n",
      ...
      for (i = 0, data_size = 1;
           i < DATASET_READ_ALL_FILE_POINT_MEM_TEST_SPACE_RANK; i++)
        data_size *= dims[i];
      ...
      {
        hsize_t mdims[] = {data_size /
                           DATASET_READ_ALL_FILE_POINT_MEM_TEST_DTYPE_SIZE};
      ...
      if (H5Dwrite(dset_id, DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  BEGIN_INDEPENDENT_OP(dset_read) {
    if (MAINPROCESS) {
      ...
      if (H5Sselect_elements(mspace_id, H5S_SELECT_SET, (size_t)space_npoints,
                             points) < 0) {
        H5_FAILED();
      ...
      if (H5Dread(dset_id, DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_DTYPE,
                  mspace_id, H5S_ALL, H5P_DEFAULT, read_buf) < 0) {
        H5_FAILED();
        printf("    couldn't read from dataset '%s'\n",
               DATASET_READ_ALL_FILE_POINT_MEM_TEST_DSET_NAME);
...
#define DATASET_READ_HYPER_FILE_POINT_MEM_TEST_GROUP_NAME                      \
  "hyper_sel_file_point_sel_mem_read_test"
#define DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_NAME                       \
  "hyper_sel_file_point_sel_mem_dset"
static int test_read_dataset_hyper_file_point_mem(void) {
  hssize_t space_npoints;
  hsize_t *dims = NULL;
  ...
  TESTING("read from dataset with hyperslab sel. for file space; point sel. "
          "for memory");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_READ_HYPER_FILE_POINT_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      if ((fspace_id = H5Screate_simple(
               DATASET_READ_HYPER_FILE_POINT_MEM_TEST_SPACE_RANK, dims, NULL)) <
          0) {
        H5_FAILED();
        printf("    failed to create file dataspace for dataset\n");
        INDEPENDENT_OP_ERROR(dset_create);
      ...
      if ((dset_id = H5Dcreate2(
               group_id, DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_NAME,
      ...
      for (i = 0, data_size = 1;
           i < DATASET_READ_HYPER_FILE_POINT_MEM_TEST_SPACE_RANK; i++)
        data_size *= dims[i];
      ...
      {
        hsize_t mdims[] = {data_size /
                           DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DTYPE_SIZE};
      ...
      if (H5Dwrite(dset_id, DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_NAME);
   ...
   * point selection works correctly.
   */
  read_buf_size = (2 * (size_t)(space_npoints / mpi_size) *
                   DATASET_READ_ONE_PROC_NONE_SEL_TEST_DTYPE_SIZE);
  if (NULL == (read_buf = calloc(1, read_buf_size))) {
    H5_FAILED();
    printf("    couldn't allocate buffer for dataset read\n");
...

  {
    ...
    if (H5Sselect_elements(mspace_id, H5S_SELECT_SET,
                           (size_t)(space_npoints / mpi_size), points) < 0) {
      H5_FAILED();
      printf("    couldn't set point selection for dataset read\n");
      goto error;
    }
  ...
  if (H5Dread(dset_id, DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_DTYPE,
              mspace_id, fspace_id, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_READ_HYPER_FILE_POINT_MEM_TEST_DSET_NAME);
...
#define DATASET_READ_POINT_FILE_HYPER_MEM_TEST_GROUP_NAME                      \
  "point_sel_file_hyper_sel_mem_read_test"
#define DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_NAME                       \
  "point_sel_file_hyper_sel_mem_dset"
static int test_read_dataset_point_file_hyper_mem(void) {
  hssize_t space_npoints;
  hsize_t *dims = NULL;
  ...
  TESTING("read from dataset with point sel. for file space; hyperslab sel. "
          "for memory");
  ...
  if (generate_random_parallel_dimensions(
          DATASET_READ_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      if ((fspace_id = H5Screate_simple(
               DATASET_READ_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK, dims, NULL)) <
          0) {
        H5_FAILED();
        printf("    failed to create file dataspace for dataset\n");
        INDEPENDENT_OP_ERROR(dset_create);
      ...
      if ((dset_id = H5Dcreate2(
               group_id, DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_NAME,
      ...
      for (i = 0, data_size = 1;
           i < DATASET_READ_POINT_FILE_HYPER_MEM_TEST_SPACE_RANK; i++)
        data_size *= dims[i];
      ...
      {
        hsize_t mdims[] = {data_size /
                           DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DTYPE_SIZE};
      ...
      if (H5Dwrite(dset_id, DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_DTYPE,
                   mspace_id, H5S_ALL, H5P_DEFAULT, write_buf) < 0) {
        H5_FAILED();
        printf("    couldn't write to dataset '%s'\n",
               DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  if ((dset_id =
           H5Dopen2(group_id, DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    couldn't open dataset '%s'\n",
           DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_NAME);
  ...
  if (H5Dread(dset_id, DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_DTYPE,
              mspace_id, fspace_id, H5P_DEFAULT, read_buf) < 0) {
    H5_FAILED();
    printf("    couldn't read from dataset '%s'\n",
           DATASET_READ_POINT_FILE_HYPER_MEM_TEST_DSET_NAME);
...
#define DATASET_MULTI_CHUNK_WRITE_SAME_SPACE_READ_TEST_DSET_DTYPESIZE          \
  sizeof(int)
...
#define DATASET_MULTI_CHUNK_WRITE_SAME_SPACE_READ_TEST_DSET_NAME               \
  "multi_chunk_dataset"
static int test_write_multi_chunk_dataset_same_shape_read(void) {
  hsize_t *dims = NULL;
  ...
  void *write_buf = NULL;
  int read_buf
      [1][DATASET_MULTI_CHUNK_WRITE_SAME_SPACE_READ_TEST_FIXED_CHUNK_DIMSIZE];
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    SKIPPED();
    printf("    API functions for basic file, group, dataset, or getting "
  ...
  for (i = 0;
    ...
    if (i == 0) {
      ...
      chunk_dims[i] =
          DATASET_MULTI_CHUNK_WRITE_SAME_SPACE_READ_TEST_FIXED_CHUNK_DIMSIZE;
    }
  }
  ...
  /*
   * Each rank reads their respective chunks in the dataset, checking the data
   * for each one.
   */
  if (MAINPROCESS)
    printf("\n");
  for (i = 0, n_chunks_per_rank = (data_size / (size_t)mpi_size) / chunk_size;
       i < n_chunks_per_rank; i++) {
    size_t j, k;
    ...
    for (j = 0; j < chunk_dims[0]; j++) {
      for (k = 0; k < chunk_dims[1]; k++) {
        size_t val =
            ((j * chunk_dims[0]) + k + i) +
            ((hsize_t)mpi_rank *
             n_chunks_per_rank); /* Additional value offset for each rank */
...
#define DATASET_MULTI_CHUNK_WRITE_DIFF_SPACE_READ_TEST_DSET_DTYPESIZE          \
  sizeof(int)
...
static int test_write_multi_chunk_dataset_diff_shape_read(void) {
  ...
  for (i = 0;
    ...
    if (i == 0) {
      ...
      chunk_dims[i] =
          DATASET_MULTI_CHUNK_WRITE_DIFF_SPACE_READ_TEST_FIXED_CHUNK_DIMSIZE;
    }
  }
  ...
  for (i = 0, n_chunks_per_rank = (data_size / (size_t)mpi_size) / chunk_size;
    ...
    for (j = 0;
         j < DATASET_MULTI_CHUNK_WRITE_DIFF_SPACE_READ_TEST_READ_BUF_DIMSIZE;
         j++)
      for (k = 0;
           k < DATASET_MULTI_CHUNK_WRITE_DIFF_SPACE_READ_TEST_READ_BUF_DIMSIZE;
           k++)
        read_buf[j][k] = 0;
...
#define DATASET_MULTI_CHUNK_OVERWRITE_SAME_SPACE_READ_TEST_DSET_DTYPESIZE      \
  sizeof(int)
#define DATASET_MULTI_CHUNK_OVERWRITE_SAME_SPACE_READ_TEST_DSET_DTYPE          \
  H5T_NATIVE_INT
#define DATASET_MULTI_CHUNK_OVERWRITE_SAME_SPACE_READ_TEST_GROUP_NAME          \
  "multi_chunk_dataset_same_space_overwrite_test"
#define DATASET_MULTI_CHUNK_OVERWRITE_SAME_SPACE_READ_TEST_DSET_NAME           \
  "multi_chunk_dataset"
...
static int test_overwrite_multi_chunk_dataset_same_shape_read(void) {
  ...
  int read_buf
      [1]
      [DATASET_MULTI_CHUNK_OVERWRITE_SAME_SPACE_READ_TEST_FIXED_CHUNK_DIMSIZE];
  ...
  for (i = 0;
    ...
    if (i == 0) {
      ...
      dims[i] =
          DATASET_MULTI_CHUNK_OVERWRITE_SAME_SPACE_READ_TEST_FIXED_DIMSIZE;
      chunk_dims[i] =
          DATASET_MULTI_CHUNK_OVERWRITE_SAME_SPACE_READ_TEST_FIXED_CHUNK_DIMSIZE;
    }
  }
  ...
  BEGIN_INDEPENDENT_OP(dset_create) {
    if (MAINPROCESS) {
      ...
      /* Set dataset space allocation time to Early to ensure all chunk-related
...
#define DATASET_MULTI_CHUNK_OVERWRITE_DIFF_SPACE_READ_TEST_DSET_DTYPESIZE      \
  sizeof(int)
#define DATASET_MULTI_CHUNK_OVERWRITE_DIFF_SPACE_READ_TEST_DSET_DTYPE          \
  H5T_NATIVE_INT
...
static int test_overwrite_multi_chunk_dataset_diff_shape_read(void) {
  ...
  for (i = 0;
    ...
    if (i == 0) {
      ...
      dims[i] =
          DATASET_MULTI_CHUNK_OVERWRITE_DIFF_SPACE_READ_TEST_FIXED_DIMSIZE;
      chunk_dims[i] =
          DATASET_MULTI_CHUNK_OVERWRITE_DIFF_SPACE_READ_TEST_FIXED_CHUNK_DIMSIZE;
    }
  }
