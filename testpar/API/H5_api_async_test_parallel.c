...
static int test_one_dataset_io(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      SKIPPED();
      printf("    API functions for basic file, dataset, or flush aren't "
  ...
  if ((fapl_id = create_mpi_fapl(MPI_COMM_WORLD, MPI_INFO_NULL,
                                 coll_metadata_read)) < 0)
    TEST_ERROR;

  /* Create dataspace */
  if (generate_random_parallel_dimensions(ONE_DATASET_IO_TEST_SPACE_RANK,
                                          &dims) < 0)
    TEST_ERROR;

  if ((space_id =
           H5Screate_simple(ONE_DATASET_IO_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;
  ...
  if ((file_id = H5Fcreate_async(PAR_ASYNC_API_TEST_FILE, H5F_ACC_TRUNC,
                                 H5P_DEFAULT, fapl_id, es_id)) < 0)
    TEST_ERROR;
  ...
  if ((dset_id =
           H5Dcreate_async(file_id, "dset", H5T_NATIVE_INT, space_id,
  ...
  if (H5Sselect_hyperslab(space_id, H5S_SELECT_SET, start, stride, count,
                          block) < 0) {
    H5_FAILED();
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(single_dset_eswait) {
      ...
      if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, mspace_id, space_id,
                         H5P_DEFAULT, write_buf, es_id) < 0)
        PART_TEST_ERROR(single_dset_eswait);

      /* Wait for the event stack to complete */
      if (H5ESwait(es_id, H5_API_TEST_WAIT_FOREVER, &num_in_progress,
                   &op_failed) < 0)
        PART_TEST_ERROR(single_dset_eswait);
      if (op_failed)
        PART_TEST_ERROR(single_dset_eswait);
      ...
      if (H5Dread_async(dset_id, H5T_NATIVE_INT, mspace_id, space_id,
                        H5P_DEFAULT, read_buf, es_id) < 0)
        PART_TEST_ERROR(single_dset_eswait);
    ...
    PART_BEGIN(single_dset_fclose) {
      ...
      if ((file_id = H5Fopen_async(PAR_ASYNC_API_TEST_FILE, H5F_ACC_RDONLY,
                                   fapl_id, es_id)) < 0)
        PART_TEST_ERROR(single_dset_fclose);
  ...
  /* Wait for the event stack to complete */
  if (H5ESwait(es_id, H5_API_TEST_WAIT_FOREVER, &num_in_progress, &op_failed) <
      0)
    TEST_ERROR;
  if (op_failed)
    TEST_ERROR;
...
static int test_multi_dataset_io(void) {
  ...
  hid_t dset_id[MULTI_DATASET_IO_TEST_NDSETS] = {
      H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID,
      H5I_INVALID_HID};
  hid_t space_id = H5I_INVALID_HID;
  hid_t mspace_id = H5I_INVALID_HID;
  hid_t es_id = H5I_INVALID_HID;
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(MULTI_DATASET_IO_TEST_SPACE_RANK,
                                          &dims) < 0)
    TEST_ERROR;

  /* Create dataspace */
  if ((space_id =
           H5Screate_simple(MULTI_DATASET_IO_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(multi_dset_open) {
      ...
      for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
        ...
        /* Create the dataset asynchronously */
        if ((dset_id[i] = H5Dcreate_async(file_id, dset_name, H5T_NATIVE_INT,
        ...
        buf_start_idx =
            i * (data_size / MULTI_DATASET_IO_TEST_NDSETS / sizeof(int));
        buf_end_idx = buf_start_idx +
                      (data_size / MULTI_DATASET_IO_TEST_NDSETS / sizeof(int));
        for (j = buf_start_idx; j < buf_end_idx; j++)
          ((int *)write_buf)[j] = mpi_rank;

        /* Write the dataset asynchronously */
        if (H5Dwrite_async(dset_id[i], H5T_NATIVE_INT, mspace_id, space_id,
                           H5P_DEFAULT, &write_buf[buf_start_idx], es_id) < 0)
          PART_TEST_ERROR(multi_dset_open);
      } /* end for */
      ...
      for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
        ...
        /* Read the dataset asynchronously */
        if (H5Dread_async(dset_id[i], H5T_NATIVE_INT, mspace_id, space_id,
                          H5P_DEFAULT, &read_buf[buf_start_idx], es_id) < 0)
          PART_TEST_ERROR(multi_dset_open);
      } /* end for */
    ...
    PART_BEGIN(multi_dset_close) {
      ...
      for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
        ...
        if ((dset_id[0] =
                 H5Dopen_async(file_id, dset_name, H5P_DEFAULT, es_id)) < 0)
        ...
        /* Write the dataset asynchronously */
        if (H5Dwrite_async(dset_id[0], H5T_NATIVE_INT, mspace_id, space_id,
                           H5P_DEFAULT, &write_buf[buf_start_idx], es_id) < 0)
          PART_TEST_ERROR(multi_dset_close);
      ...
      for (i = 0; i < MULTI_DATASET_IO_TEST_NDSETS; i++) {
        ...
        buf_start_idx =
            i * (data_size / MULTI_DATASET_IO_TEST_NDSETS / sizeof(int));
        if (H5Dread_async(dset_id[0], H5T_NATIVE_INT, mspace_id, space_id,
                          H5P_DEFAULT, &read_buf[buf_start_idx], es_id) < 0)
          PART_TEST_ERROR(multi_dset_close);
...
static int test_multi_file_dataset_io(void) {
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(MULTI_FILE_DATASET_IO_TEST_SPACE_RANK,
                                          &dims) < 0)
    TEST_ERROR;

  /* Create dataspace */
  if ((space_id = H5Screate_simple(MULTI_FILE_DATASET_IO_TEST_SPACE_RANK, dims,
                                   NULL)) < 0)
    TEST_ERROR;
  ...
  /* Setup memory space for write_buf */
  {
    hsize_t mdims[] = {data_size / MULTI_FILE_DATASET_IO_TEST_NFILES /
                       sizeof(int)};
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(multi_file_dset_open) {
      ...
      for (i = 0; i < MULTI_FILE_DATASET_IO_TEST_NFILES; i++) {
        ...
        /* Create file asynchronously */
        if ((file_id[i] = H5Fcreate_async(file_name, H5F_ACC_TRUNC, H5P_DEFAULT,
                                          fapl_id, es_id)) < 0)
        ...
        /* Create the dataset asynchronously */
        if ((dset_id[i] = H5Dcreate_async(file_id[i], "dset", H5T_NATIVE_INT,
                                          space_id, H5P_DEFAULT, H5P_DEFAULT,
                                          H5P_DEFAULT, es_id)) < 0)
          PART_TEST_ERROR(multi_file_dset_open);
         ...
         * each dset since we can't overwrite the buffers until I/O is done. */
        buf_start_idx =
            i * (data_size / MULTI_FILE_DATASET_IO_TEST_NFILES / sizeof(int));
        buf_end_idx =
            buf_start_idx +
            (data_size / MULTI_FILE_DATASET_IO_TEST_NFILES / sizeof(int));
        for (j = buf_start_idx; j < buf_end_idx; j++)
          ((int *)write_buf)[j] = mpi_rank;
    ...
    PART_BEGIN(multi_file_dset_dclose) {
      ...
      for (i = 0; i < MULTI_FILE_DATASET_IO_TEST_NFILES; i++) {
        ...
        /* Open the dataset asynchronously */
        if ((dset_id[0] =
                 H5Dopen_async(file_id[i], "dset", H5P_DEFAULT, es_id)) < 0)
    ...
    PART_BEGIN(multi_file_dset_fclose) {
      ...
      for (i = 0; i < MULTI_FILE_DATASET_IO_TEST_NFILES; i++) {
        ...
        if ((file_id[0] =
                 H5Fopen_async(file_name, H5F_ACC_RDWR, fapl_id, es_id)) < 0)
          PART_TEST_ERROR(multi_file_dset_fclose);

        /* Open the dataset asynchronously */
        if ((dset_id[0] =
                 H5Dopen_async(file_id[0], "dset", H5P_DEFAULT, es_id)) < 0)
          PART_TEST_ERROR(multi_file_dset_fclose);
      ...
      for (i = 0; i < MULTI_FILE_DATASET_IO_TEST_NFILES; i++) {
        ...
        /* Open the file asynchronously */
        if ((file_id[0] =
                 H5Fopen_async(file_name, H5F_ACC_RDONLY, fapl_id, es_id)) < 0)
          PART_TEST_ERROR(multi_file_dset_fclose);
...
static int test_multi_file_grp_dset_io(void) {
  ...
  if (generate_random_parallel_dimensions(
          MULTI_FILE_GRP_DSET_IO_TEST_SPACE_RANK, &dims) < 0)
    TEST_ERROR;

  /* Create dataspace */
  if ((space_id = H5Screate_simple(MULTI_FILE_GRP_DSET_IO_TEST_SPACE_RANK, dims,
                                   NULL)) < 0)
    TEST_ERROR;
  ...
  /* Setup memory space for write_buf */
  {
    hsize_t mdims[] = {data_size / MULTI_FILE_GRP_DSET_IO_TEST_NFILES /
                       sizeof(int)};
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(multi_file_grp_dset_no_kick) {
      ...
      for (i = 0; i < MULTI_FILE_GRP_DSET_IO_TEST_NFILES; i++) {
        ...
        /* Create file asynchronously */
        if ((file_id = H5Fcreate_async(file_name, H5F_ACC_TRUNC, H5P_DEFAULT,
                                       fapl_id, es_id)) < 0)
        ...
        if ((grp_id = H5Gcreate_async(file_id, "grp", H5P_DEFAULT, H5P_DEFAULT,
                                      H5P_DEFAULT, es_id)) < 0)
          PART_TEST_ERROR(multi_file_grp_dset_no_kick);
         ...
         * each dset since we can't overwrite the buffers until I/O is done. */
        buf_start_idx =
            i * (data_size / MULTI_FILE_GRP_DSET_IO_TEST_NFILES / sizeof(int));
        buf_end_idx =
            buf_start_idx +
            (data_size / MULTI_FILE_GRP_DSET_IO_TEST_NFILES / sizeof(int));
        for (j = buf_start_idx; j < buf_end_idx; j++)
          ((int *)write_buf)[j] = mpi_rank;
...
static int test_set_extent(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      SKIPPED();
      printf("    API functions for basic file, dataset, dataset more, or "
             "flush aren't supported "
             "with this connector\n");
    }
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(SET_EXTENT_TEST_SPACE_RANK, &dims) <
      0)
    TEST_ERROR;

  if (NULL ==
      (maxdims = malloc(SET_EXTENT_TEST_SPACE_RANK * sizeof(hsize_t)))) {
    H5_FAILED();
  ...
  for (i = 0; i < SET_EXTENT_TEST_SPACE_RANK; i++) {
    maxdims[i] =
        (i == 0) ? dims[i] + (hsize_t)(SET_EXTENT_TEST_NUM_EXTENDS * mpi_size)
                 : dims[i];
    cdims[i] = (dims[i] == 1) ? 1 : dims[i] / 2;
  }

  /* Create file dataspace */
  if ((space_id = H5Screate_simple(SET_EXTENT_TEST_SPACE_RANK, dims, maxdims)) <
      0)
    TEST_ERROR;
  ...
  for (i = 0; i < SET_EXTENT_TEST_NUM_EXTENDS; i++) {
    ...
    if (i) {
      ...
      if (H5Sset_extent_simple(space_id, SET_EXTENT_TEST_SPACE_RANK, dims,
                               maxdims) < 0)
        TEST_ERROR;
      ...
      {
        hsize_t mdims[] = {
            (i + 1) * (data_size / SET_EXTENT_TEST_NUM_EXTENDS / sizeof(int))};
...
static int test_attribute_exists(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      SKIPPED();
      printf("    API functions for basic file, dataset, dataset more, "
             "attribute, or flush aren't "
             "supported with this connector\n");
    }
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(ATTRIBUTE_EXISTS_TEST_SPACE_RANK,
                                          &dims) < 0)
    TEST_ERROR;

  /* Create dataspace */
  if ((space_id =
           H5Screate_simple(ATTRIBUTE_EXISTS_TEST_SPACE_RANK, dims, NULL)) < 0)
    TEST_ERROR;
  ...
  if ((file_id = H5Fopen_async(PAR_ASYNC_API_TEST_FILE, H5F_ACC_RDWR, fapl_id,
                               es_id)) < 0)
    TEST_ERROR;
  ...
  /* Create the dataset asynchronously */
  if ((dset_id = H5Dcreate_async(file_id, "attr_exists_dset", H5T_NATIVE_INT,
                                 space_id, H5P_DEFAULT, H5P_DEFAULT,
                                 H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;
   ...
   * Skip this function because it isn't supported for the native vol in
   * parallel.
   */
  if (!is_native_vol && H5Oflush_async(dset_id, es_id) < 0)
    TEST_ERROR;
  ...
  if (!exists2)
    FAIL_PUTS_ERROR(
        "    H5Aexists returned FALSE for an attribute that should exist");
...
static int test_attribute_io(void) {
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(ATTRIBUTE_IO_TEST_SPACE_RANK, &dims) <
      0)
    TEST_ERROR;

  /* Create dataspace */
  if ((space_id = H5Screate_simple(ATTRIBUTE_IO_TEST_SPACE_RANK, dims, NULL)) <
      0)
    TEST_ERROR;
  ...
  /* Create the dataset asynchronously */
  if ((dset_id =
           H5Dcreate_async(file_id, "attr_dset", H5T_NATIVE_INT, space_id,
                           H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;
...
static int test_attribute_io_tconv(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      SKIPPED();
      printf("    API functions for basic file, attribute, or flush aren't "
             "supported with this "
             "connector\n");
    }
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(ATTRIBUTE_IO_TCONV_TEST_SPACE_RANK,
                                          &dims) < 0)
    TEST_ERROR;

  /* Create dataspace */
  if ((space_id = H5Screate_simple(ATTRIBUTE_IO_TCONV_TEST_SPACE_RANK, dims,
                                   NULL)) < 0)
    TEST_ERROR;
  ...
  if ((attr_id = H5Acreate_by_name_async(file_id, "attr_dset", "attr_tconv",
                                         H5T_STD_U16BE, space_id, H5P_DEFAULT,
                                         H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;
...
static int test_attribute_io_compound(void) {
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(ATTRIBUTE_IO_COMPOUND_TEST_SPACE_RANK,
                                          &dims) < 0)
    TEST_ERROR;
  ...
  if ((mtype_id = H5Tcreate(H5T_COMPOUND, sizeof(tattr_cmpd_t))) < 0)
    TEST_ERROR;
  if (H5Tinsert(mtype_id, "a_name", HOFFSET(tattr_cmpd_t, a), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(mtype_id, "b_name", HOFFSET(tattr_cmpd_t, b), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;

  if ((mtypea_id = H5Tcreate(H5T_COMPOUND, sizeof(tattr_cmpd_t))) < 0)
    TEST_ERROR;
  if (H5Tinsert(mtypea_id, "a_name", HOFFSET(tattr_cmpd_t, a), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;

  if ((mtypeb_id = H5Tcreate(H5T_COMPOUND, sizeof(tattr_cmpd_t))) < 0)
    TEST_ERROR;
  if (H5Tinsert(mtypeb_id, "b_name", HOFFSET(tattr_cmpd_t, b), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  ...
  /* Create dataspace */
  if ((space_id = H5Screate_simple(ATTRIBUTE_IO_COMPOUND_TEST_SPACE_RANK, dims,
                                   NULL)) < 0)
    TEST_ERROR;
...
static int test_group(void) {
  ...
  if ((parent_group_id = H5Gcreate_async(file_id, "group_parent", H5P_DEFAULT,
                                         gcpl_id, H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;
  ...
  if ((group_id =
           H5Gopen_async(parent_group_id, "group1", H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;
  ...
  if (vol_cap_flags_g & H5VL_CAP_FLAG_CREATION_ORDER) {
    ...
    if (H5Gget_info_by_idx_async(parent_group_id, ".", H5_INDEX_CRT_ORDER,
                                 H5_ITER_INC, 1, &info2, H5P_DEFAULT,
                                 es_id) < 0)
      TEST_ERROR;
  }
...
static int test_link(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      SKIPPED();
      printf("    API functions for basic file, link, hard link, soft link, "
             "flush, or creation order "
  ...
  /* Create the parent group asynchronously */
  if ((parent_group_id = H5Gcreate_async(file_id, "link_parent", H5P_DEFAULT,
                                         gcpl_id, H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;
   ...
   * Skip this function for the native vol because it isn't supported in
   * parallel.
   */
  ...
  if (H5Lcreate_hard_async(parent_group_id, "group", parent_group_id,
                           "hard_link", H5P_DEFAULT, H5P_DEFAULT, es_id) < 0)
    TEST_ERROR;

  /* Flush the parent group asynchronously.  This will effectively work as a
   * barrier, guaranteeing the soft link create takes place after the hard
   * link create. Skip this function for the native vol because it isn't
  ...
  if (H5Lexists_async(parent_group_id, "hard_link", &existsh1, H5P_DEFAULT,
                      es_id) < 0)
    TEST_ERROR;

  /* Check if soft link exists */
  if (H5Lexists_async(parent_group_id, "soft_link", &existss1, H5P_DEFAULT,
                      es_id) < 0)
    TEST_ERROR;
  ...
  /* Check if hard link exists */
  if (H5Lexists_async(parent_group_id, "hard_link", &existsh2, H5P_DEFAULT,
                      es_id) < 0)
    TEST_ERROR;

  /* Check if soft link exists */
  if (H5Lexists_async(parent_group_id, "soft_link", &existss2, H5P_DEFAULT,
                      es_id) < 0)
    TEST_ERROR;
  ...
  /* Check if hard link exists */
  if (H5Lexists_async(parent_group_id, "hard_link", &existsh3, H5P_DEFAULT,
                      es_id) < 0)
    TEST_ERROR;

  /* Check if soft link exists */
  if (H5Lexists_async(parent_group_id, "soft_link", &existss3, H5P_DEFAULT,
                      es_id) < 0)
    TEST_ERROR;
  ...
  if (!existsh1)
    FAIL_PUTS_ERROR(
        "    link exists returned FALSE for link that should exist");
  if (!existss1)
    FAIL_PUTS_ERROR(
        "    link exists returned FALSE for link that should exist");
  if (!existsh2)
  ...
  if (existss2)
    FAIL_PUTS_ERROR(
        "    link exists returned TRUE for link that should not exist");
  if (existsh3)
    FAIL_PUTS_ERROR(
        "    link exists returned TRUE for link that should not exist");
  if (existsh3)
...
static int test_ocopy_orefresh(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      SKIPPED();
      printf("    API functions for basic file, group, dataset, object more, "
             "flush, or refresh "
             "aren't supported with this connector\n");
    }
  ...
  /* Create dataspace */
  if (generate_random_parallel_dimensions(OCOPY_REFRESH_TEST_SPACE_RANK,
                                          &dims) < 0)
    TEST_ERROR;

  /* Create dataspace */
  if ((space_id = H5Screate_simple(OCOPY_REFRESH_TEST_SPACE_RANK, dims, NULL)) <
      0)
    TEST_ERROR;
  ...
  /* Create the parent group asynchronously */
  if ((parent_group_id = H5Gcreate_async(file_id, "ocopy_parent", H5P_DEFAULT,
                                         H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;

  /* Create dataset asynchronously. */
  if ((dset_id =
           H5Dcreate_async(parent_group_id, "dset", H5T_NATIVE_INT, space_id,
                           H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id)) < 0)
    TEST_ERROR;
  if (H5Dclose_async(dset_id, es_id) < 0)
    TEST_ERROR;
