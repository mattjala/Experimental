...
int generate_random_parallel_dimensions(int space_rank, hsize_t **dims_out) {
   ...
   */
  if (MPI_SUCCESS !=
      MPI_Bcast(dims, space_rank, MPI_UNSIGNED_LONG_LONG, 0, MPI_COMM_WORLD))
    goto error;
...
int main(int argc, char **argv) {
  ...
  if (provided < required) {
    if (MAINPROCESS)
      printf("** INFO: couldn't initialize with MPI_THREAD_MULTIPLE threading "
             "support **\n");
  }
  ...
  HDsnprintf(H5_api_test_parallel_filename, H5_API_TEST_FILENAME_MAX_LENGTH,
             "%s%s", test_path_prefix, PARALLEL_TEST_FILE_NAME);
  ...
  if (NULL == (vol_connector_string = HDgetenv(HDF5_VOL_CONNECTOR))) {
    ...
    BEGIN_INDEPENDENT_OP(copy_connector_string) {
      if (NULL ==
          (vol_connector_string_copy = HDstrdup(vol_connector_string))) {
        if (MAINPROCESS)
  ...
  if (MAINPROCESS) {
    printf("Running parallel API tests with VOL connector '%s' and info string "
           "'%s'\n\n",
  ...
  BEGIN_INDEPENDENT_OP(check_vol_register) {
    ...
    if (0 != HDstrcmp(vol_connector_name, "native")) {
      ...
      if ((is_registered =
               H5VLis_connector_registered_by_name(vol_connector_name)) < 0) {
        if (MAINPROCESS)
          fprintf(stderr,
                  "Unable to determine if VOL connector is registered\n");
      ...
      if (!is_registered) {
        ...
        if ((registered_con_id =
                 H5VLget_connector_id_by_name(vol_connector_name)) < 0) {
          if (MAINPROCESS)
            fprintf(stderr,
                    "Couldn't retrieve ID of registered VOL connector\n");
          INDEPENDENT_OP_ERROR(check_vol_register);
        }
  ...
  BEGIN_INDEPENDENT_OP(create_test_container) {
    if (MAINPROCESS) {
      if (create_test_container(H5_api_test_parallel_filename,
                                vol_cap_flags_g) < 0) {
  ...
  if (n_tests_run_g > 0) {
    if (MAINPROCESS)
      printf("The below statistics are minimum values due to the possibility "
             "of some ranks failing a "
             "test while others pass:\n");

    if (MPI_SUCCESS != MPI_Allreduce(MPI_IN_PLACE, &n_tests_passed_g, 1,
                                     MPI_UNSIGNED_LONG_LONG, MPI_MIN,
                                     MPI_COMM_WORLD)) {
      if (MAINPROCESS)
        printf("    failed to collect consensus about the minimum number of "
               "tests that passed -- "
               "reporting rank 0's (possibly inaccurate) value\n");
    }

    if (MAINPROCESS)
      printf("%s%zu/%zu (%.2f%%) API tests passed across all ranks with VOL "
             "connector '%s'\n",
             n_tests_passed_g > 0 ? "At least " : "", n_tests_passed_g,
             n_tests_run_g,
             ((double)n_tests_passed_g / (double)n_tests_run_g * 100.0),
             vol_connector_name);

    if (MPI_SUCCESS != MPI_Allreduce(MPI_IN_PLACE, &n_tests_failed_g, 1,
                                     MPI_UNSIGNED_LONG_LONG, MPI_MIN,
                                     MPI_COMM_WORLD)) {
      if (MAINPROCESS)
        printf("    failed to collect consensus about the minimum number of "
    ...
    if (MAINPROCESS) {
      printf("%s%zu/%zu (%.2f%%) API tests did not pass across all ranks with "
             "VOL connector '%s'\n",
             n_tests_failed_g > 0 ? "At least " : "", n_tests_failed_g,
             n_tests_run_g,
             ((double)n_tests_failed_g / (double)n_tests_run_g * 100.0),
             vol_connector_name);

      printf(
          "%zu/%zu (%.2f%%) API tests were skipped with VOL connector '%s'\n",
