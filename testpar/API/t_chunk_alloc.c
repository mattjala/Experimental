 ...
 */
static void create_chunked_dataset(const char *filename, int chunk_factor,
                                   write_type write_pattern) {
  ...
  if (MAINPROCESS) {
    ...
    dataset = H5Dcreate2(file_id, DSET_NAME, H5T_NATIVE_UCHAR, dataspace,
    ...
    if (write_pattern == sec_last) {
      ...
      hrc = H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, offset, stride,
                                count, block);
      VRFY((hrc >= 0), "");

      /* Write sec_last chunk */
      hrc = H5Dwrite(dataset, H5T_NATIVE_UCHAR, memspace, dataspace,
                     H5P_DEFAULT, buffer);
      VRFY((hrc >= 0), "H5Dwrite");
    } /* end if */
 ...
 */
static void parallel_access_dataset(const char *filename, int chunk_factor,
                                    access_type action, hid_t *file_id,
                                    hid_t *dataset) {
  /* HDF5 gubbins */
 ...
 *    interleaved pattern.
 */
static void verify_data(const char *filename, int chunk_factor,
                        write_type write_pattern, int vclose, hid_t *file_id,
                        hid_t *dataset) {
  /* HDF5 gubbins */
  ...
  for (i = 0; i < nchunks; i++) {
    ...
    hrc = H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, offset, stride, count,
                              block);
    VRFY((hrc >= 0), "");

    /* Read the chunk */
    hrc = H5Dread(*dataset, H5T_NATIVE_UCHAR, memspace, dataspace, H5P_DEFAULT,
                  buffer);
    VRFY((hrc >= 0), "H5Dread");
...
void test_chunk_alloc(void) {
  ...
  parallel_access_dataset(filename, CHUNK_FACTOR, open_only, &file_id,
                          &dataset);
  ...
  parallel_access_dataset(filename, CHUNK_FACTOR, extend_only, &file_id,
                          &dataset);
  /* reopen dataset in parallel, read and verify the data */
  verify_data(filename, CHUNK_FACTOR, none, CLOSE, &file_id, &dataset);
  ...
  /* Reopen dataset in parallel, read and verify the data. The file and dataset
   * are not closed*/
  ...
  parallel_access_dataset(filename, CHUNK_FACTOR, write_all, &file_id,
                          &dataset);
  /* reopen dataset in parallel, read and verify the data */
