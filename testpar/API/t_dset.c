...
static void slab_set(int mpi_rank, int mpi_size, hsize_t start[],
  ...
  case ZCOL:
    ...
    stride[0] = block[0];
    stride[1] =
        (hsize_t)(mpi_rank ? block[1] : 1); /* avoid setting stride to 0 */
    count[0] = 1;
    count[1] = 1;
    start[0] = 0;
...
void dataset_writeInd(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      puts("SKIPPED");
      printf("    API functions for basic file, basic dataset, or more aren't "
             "supported with this "
             "connector\n");
      fflush(stdout);
    }
...
void dataset_readAll(void) {
  ...
  num_points = (size_t)dim1;
  coords =
      (hsize_t *)malloc((size_t)dim0 * (size_t)dim1 * RANK * sizeof(hsize_t));
  VRFY((coords != NULL), "coords malloc succeeded");
...
void extend_readInd(void) {
  ...
  dims[0]++;
  H5E_BEGIN_TRY { ret = H5Dset_extent(dataset1, dims); }
  H5E_END_TRY
...
void compress_readAll(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      puts("SKIPPED");
      printf("    API functions for basic file or dataset aren't supported "
             "with this connector\n");
      fflush(stdout);
    }
 ...
 *                  Performs collective and then independent I/O with hthe same
...
static void test_no_collective_cause_mode(int selection_mode) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      puts("SKIPPED");
      printf("    API functions for basic file, dataset, or dataset more "
...
void dataset_atomicity(void) {
  ...
  if (MAINPROCESS) {
    ...
    /* should fail */
    H5E_BEGIN_TRY { ret = H5Fset_mpi_atomicity(fid, TRUE); }
    H5E_END_TRY
