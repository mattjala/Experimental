...
void zero_dim_dset(void) {
  ...
  dsid = H5Dcreate2(fid, "dset", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
...
void multiple_dset_write(void) {
  ...
  ret = H5Sselect_hyperslab(filespace, H5S_SELECT_SET, chunk_origin, chunk_dims,
                            count, chunk_dims);
  ...
  for (n = 0; n < ndatasets; n++) {
    ...
    dataset = H5Dcreate2(iof, dname, H5T_NATIVE_DOUBLE, filespace, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT);
...
void compact_dataset(void) {
  ...
  ret = H5Pinsert2(dxpl, H5D_XFER_COLL_RANK0_BCAST_NAME,
                   H5D_XFER_COLL_RANK0_BCAST_SIZE, &prop_value, NULL, NULL,
                   NULL, NULL, NULL, NULL);
...
void null_dataset(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      puts("SKIPPED");
      printf("    API functions for basic file, dataset, or attribute aren't "
             "supported with this "
             "connector\n");
      fflush(stdout);
    }
  ...
  /* Create a compact dataset */
  dataset = H5Dcreate2(iof, dname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
  ...
  attr = H5Acreate2(dataset, attr_name, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                    H5P_DEFAULT);
...
void big_dataset(void) {
  ...
  dataset = H5Dcreate2(iof, dname, H5T_NATIVE_UCHAR, filespace, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset >= 0), "H5Dcreate2 succeeded");
...
void dataset_fillvalue(void) {
  ...
  dataset = H5Dcreate2(iof, dname, H5T_NATIVE_INT, filespace, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset >= 0), "H5Dcreate2 succeeded");
      ...
      for (j = 0; j < (int)dset_dims[1]; j++)
        for (k = 0; k < (int)dset_dims[2]; k++)
          for (l = 0; l < (int)dset_dims[3]; l++, trdata++)
            if (*trdata != 0)
              if (err_num++ < MAX_ERR_REPORT || VERBOSE_MED)
                printf("Rank %d: Dataset Verify failed at [%d][%d][%d][%d]: "
                       "expect 0, got %d\n",
  ...
  ret = H5Sselect_hyperslab(filespace, H5S_SELECT_SET, req_start, NULL,
                            req_count, NULL);
  VRFY((ret >= 0), "H5Sselect_hyperslab succeeded on memory dataspace");
  ret = H5Sselect_hyperslab(memspace, H5S_SELECT_SET, req_start, NULL,
                            req_count, NULL);
  VRFY((ret >= 0), "H5Sselect_hyperslab succeeded on memory dataspace");
        ...
        for (k = 0; k < (int)dset_dims[2]; k++)
          for (l = 0; l < (int)dset_dims[3]; l++, twdata++, trdata++)
            if (i < mpi_size) {
              if (*twdata != *trdata)
                if (err_num++ < MAX_ERR_REPORT || VERBOSE_MED)
                  printf("Dataset Verify failed at [%d][%d][%d][%d]: expect "
                         "%d, got %d\n",
              ...
              if (*trdata != 0)
                if (err_num++ < MAX_ERR_REPORT || VERBOSE_MED)
                  printf("Dataset Verify failed at [%d][%d][%d][%d]: expect 0, "
                         "got %d\n",
                         i, j, k, l, *trdata);
            } /* end else */
    if (err_num > MAX_ERR_REPORT && !VERBOSE_MED)
...
void collective_group_write(void) {
  ...
  ret1 = H5Sselect_hyperslab(memspace, H5S_SELECT_SET, chunk_origin, chunk_dims,
                             count, chunk_dims);
  filespace = H5Screate_simple(DIM, file_dims, NULL);
  ...
  for (m = 0; m < ngroups; m++) {
    ...
    did = H5Dcreate2(gid, dname, H5T_NATIVE_INT, filespace, H5P_DEFAULT, dcpl,
                     H5P_DEFAULT);
    VRFY((did > 0), dname);
...
void multiple_group_write(void) {
  ...
  memspace = H5Screate_simple(DIM, file_dims, NULL);
  VRFY((memspace >= 0), "memspace");
  ret = H5Sselect_hyperslab(memspace, H5S_SELECT_SET, chunk_origin, chunk_dims,
                            count, chunk_dims);
 ...
 */
static void create_group_recursive(hid_t memspace, hid_t filespace, hid_t gid,
                                   int counter) {
  hid_t child_gid;
  int mpi_rank;
  char gname[64];
...
void multiple_group_read(void) {
  ...
  memspace = H5Screate_simple(DIM, file_dims, NULL);
  H5Sselect_hyperslab(memspace, H5S_SELECT_SET, chunk_origin, chunk_dims, count,
                      chunk_dims);
  filespace = H5Screate_simple(DIM, file_dims, NULL);
 ...
 * checks the data.
 */
static void recursive_read_group(hid_t memspace, hid_t filespace, hid_t gid,
                                 int counter) {
  hid_t child_gid;
...
static void write_attribute(hid_t obj_id, int this_type, int num) {
  ...
  if (this_type == is_group) {
    ...
    aid = H5Acreate2(obj_id, attr_name, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                     H5P_DEFAULT);
      ...
      if (*indata != *outdata)
        if (err_num++ < MAX_ERR_REPORT || VERBOSE_MED)
          printf("Dataset Verify failed at [%lu][%lu](row %lu, col%lu): expect "
                 ...
                 (unsigned long)j, *outdata, *indata);
    }
  if (err_num > MAX_ERR_REPORT && !VERBOSE_MED)
    printf("[more errors ...]\n");
  if (err_num)
...
static void get_slab(hsize_t chunk_origin[], hsize_t chunk_dims[],
                     hsize_t count[], hsize_t file_dims[], int size) {
  int mpi_rank, mpi_size;
...
void io_mode_confusion(void) {
  ...
  dset_id = H5Dcreate2(file_id, dataset_name, H5T_NATIVE_INT, filespace,
 ...
 */
const char *dataset_name[NUM_DATA_SETS] = {"dataset_0", "dataset_1",
                                           "dataset_2", "dataset_3"};
const char *att_name[NUM_DATA_SETS] = {"attribute_0", "attribute_1",
                                       "attribute_2", "attribute_3"};
const char *lg_att_name[NUM_DATA_SETS] = {
    "large_attribute_0", "large_attribute_1", "large_attribute_2",
    "large_attribute_3"};
...
void rr_obj_hdr_flush_confusion(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    if (MAINPROCESS) {
      puts("SKIPPED");
      printf("    API functions for basic file, dataset, attribute, dataset "
             "more, attribute more, or "
...
void rr_obj_hdr_flush_confusion_writer(MPI_Comm comm) {
  ...
  for (i = 0; i < NUM_DATA_SETS; i++) {
    ...
    dataset[i] =
        H5Dcreate2(file_id, dataset_name[i], H5T_NATIVE_DOUBLE, disk_space[i],
                   H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  for (i = 0; i < NUM_DATA_SETS; i++) {
    err = H5Sselect_hyperslab(disk_space[i], H5S_SELECT_SET, disk_start, NULL,
                              disk_count, NULL);
    ...
    err = H5Sselect_hyperslab(mem_space[i], H5S_SELECT_SET, mem_start, NULL,
                              mem_count, NULL);
    VRFY((err >= 0), "H5Sselect_hyperslab(2) failed.\n");
    err = H5Dwrite(dataset[i], H5T_NATIVE_DOUBLE, mem_space[i], disk_space[i],
                   dxpl_id, data);
    ...
    if (steps > 0) {
      ...
      if (steps >= 1) {
        ...
        for (i = 0; i < NUM_DATA_SETS; i++) {
          ...
          mem_space[i] = H5Screate_simple(1, mem_size, NULL);
          VRFY((mem_space[i] >= 0), "H5Screate_simple(2) failed.\n");
          err = H5Sselect_hyperslab(mem_space[i], H5S_SELECT_SET, mem_start,
                                    NULL, mem_count, NULL);
          VRFY((err >= 0), "H5Sselect_hyperslab(2) failed.\n");
          ...
          if (tri_err == 0) {
            ...
            for (j = 0; j < LOCAL_DATA_SIZE; j++)
              if (!H5_DBL_ABS_EQUAL(att_read[j], att[j])) {
                fprintf(stdout,
                        "%0d:%s: Mismatched attribute data read in Dataset %d, "
                        "at position "
                        "%d: expect %f, got %f.\n",
          ...
          if (tri_err == 0) {
            ...
            for (j = 0; j < LARGE_ATTR_SIZE; j++)
              if (!H5_DBL_ABS_EQUAL(lg_att_read[j], lg_att[j])) {
                fprintf(stdout,
                        "%0d:%s: Mismatched large attribute data read in "
                        "Dataset %d, at "
...
void chunk_align_bug_1(void) {
  ...
  dset_id = H5Dcreate2(file_id, "dset", H5T_NATIVE_CHAR, space_id, H5P_DEFAULT,
                       dcpl_id, H5P_DEFAULT);
