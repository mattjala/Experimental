...
typedef enum {
  ...
  TEST_TYPE_CONV_SEL_EMPTY,    /* some processes have null/empty selections and
...
typedef enum {
  ...
  DSET_WITH_CONV_AND_NO_BKG, /* Dataset with type conversion but no background
                                buffer */
 ...
 */
static void set_dxpl(hid_t dxpl, H5D_selection_io_mode_t select_io_mode,
                     H5FD_mpio_xfer_t mpio_type,
...
static void check_io_mode(hid_t dxpl, unsigned chunked) {
  ...
  if (chunked) {
    ...
    nerrors++;
    if (MAINPROCESS)
      printf("\n     Failed: Incorrect I/O mode (expected contiguous, returned "
             "%u)",
             (unsigned)actual_io_mode);
  }
 ...
 */
static void test_no_type_conv(hid_t fid, unsigned chunked, unsigned dtrans,
                              unsigned mwbuf) {
  int i;
  ...
  HDsnprintf(dset_name, sizeof(dset_name), "no_tconv_%s_%s_%s",
             chunked ? "chunked" : "contig", dtrans ? "xform" : "noxform",
             mwbuf ? "mwbuf" : "nomwbuf");
  ...
  set_dxpl(dxpl, H5D_SELECTION_IO_MODE_ON, H5FD_MPIO_COLLECTIVE,
 ...
 */
static void test_no_size_change_no_bkg(hid_t fid, unsigned chunked,
                                       unsigned mwbuf) {
  int i;
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  ...
  if ((wbuf = (char *)malloc((size_t)(4 * DSET_SELECT_DIM))) == NULL)
    P_TEST_ERROR;
  if (mwbuf &&
      (wbuf_bak = (char *)malloc((size_t)(4 * DSET_SELECT_DIM))) == NULL)
    P_TEST_ERROR;
  ...
  /* Generate dataset name */
  HDsnprintf(dset_name, sizeof(dset_name), "no_size_change_%s_%s",
             chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");

  /* Create 1d dataset */
  if ((did = H5Dcreate2(fid, dset_name, H5T_STD_I32BE, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    P_TEST_ERROR;
  ...
  /* Verify data read */
  for (i = 0; i < (int)block[0]; i++) {
    if (rbuf[4 * i + 0] != wbuf[4 * i + 0] ||
        rbuf[4 * i + 1] != wbuf[4 * i + 1] ||
        rbuf[4 * i + 2] != wbuf[4 * i + 2] ||
        rbuf[4 * i + 3] != wbuf[4 * i + 3]) {
      nerrors++;
  ...
  /* Verify data read */
  for (i = 0; i < (int)block[0]; i++) {
    if (rbuf[4 * i + 0] != wbuf[4 * i + 3] ||
        rbuf[4 * i + 1] != wbuf[4 * i + 2] ||
        rbuf[4 * i + 2] != wbuf[4 * i + 1] ||
        rbuf[4 * i + 3] != wbuf[4 * i + 0]) {
      nerrors++;
      printf("\n     Error in data verification at index %d\n",
             i + (int)start[0]);
      break;
 ...
 */
static void test_larger_mem_type_no_bkg(hid_t fid, unsigned chunked,
                                        unsigned dtrans, unsigned mwbuf) {
  int i;
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  hid_t dcpl = H5I_INVALID_HID;
  ...
  /* Generate dataset name */
  HDsnprintf(dset_name, sizeof(dset_name), "larger_no_bkg_%s_%s_%s",
             chunked ? "chunked" : "contig", dtrans ? "xform" : "noxform",
             mwbuf ? "mwbuf" : "nomwbuf");
  ...
  /* Read data from the dataset (if dtrans, without data transform set in dxpl)
   */
  if (H5Dread(did, H5T_NATIVE_LLONG, mspace_id, fspace_id, ntrans_dxpl, rbuf) <
      0)
    P_TEST_ERROR;
 ...
 */
static void test_smaller_mem_type_no_bkg(hid_t fid, unsigned chunked,
                                         unsigned dtrans, unsigned mwbuf) {
  int i;
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  ...
  /* Generate dataset name */
  HDsnprintf(dset_name, sizeof(dset_name), "smaller_no_bkg_%s_%s_%s",
             chunked ? "chunked" : "contig", dtrans ? "xform" : "noxform",
             mwbuf ? "mwbuf" : "nomwbuf");
  ...
  /* Read data from the dataset (if dtrans, without data transform set in dxpl)
   */
  if (H5Dread(did, H5T_NATIVE_SHORT, mspace_id, fspace_id, ntrans_dxpl, rbuf) <
      0)
    P_TEST_ERROR;
 ...
 *      Verify values read
 *  (b) Update all fields of the compound type in memory write buffer with new
 * unique values Write some but not all all compound fields to disk Read the
 * entire compound type Verify the fields have the correct (old or new) values
 *  (c) Update all fields of the compound type in memory read buffer with new
 * unique values Read some but not all the compound fields to memory Verify the
 * fields have the correct (old, middle  or new) values (d) Set up a different
 * compound type which has:
 ...
 */
static void test_cmpd_with_bkg(hid_t fid, unsigned chunked, unsigned mwbuf) {
  int i;
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  hid_t dcpl = H5I_INVALID_HID;
  ...
  if (NULL == (s1_wbuf = (s1_t *)malloc(sizeof(s1_t) * DSET_SELECT_DIM)))
    P_TEST_ERROR;
  if (mwbuf &&
      NULL == (s1_wbuf_bak = (s1_t *)malloc(sizeof(s1_t) * DSET_SELECT_DIM)))
    P_TEST_ERROR;
  ...
  if (NULL == (s2_wbuf = (s2_t *)malloc(sizeof(s2_t) * DSET_SELECT_DIM)))
    P_TEST_ERROR;
  if (mwbuf &&
      NULL == (s2_wbuf_bak = (s2_t *)malloc(sizeof(s2_t) * DSET_SELECT_DIM)))
    P_TEST_ERROR;
  ...
  /* Generate dataset name */
  HDsnprintf(dset_name, sizeof(dset_name), "cmpd_with_bkg_%s_%s",
             chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");

  /* Create 1d dataset */
  if ((did = H5Dcreate2(fid, dset_name, s1_tid, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    P_TEST_ERROR;
  ...
  for (i = 0; i < (int)block[0]; i++)
    if (s1_wbuf[i].a != s1_rbuf[i].a || s1_wbuf[i].b != s1_rbuf[i].b ||
      ...
      printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
             i + (int)start[0], s1_wbuf[i].a, s1_rbuf[i].a, s1_wbuf[i].b,
             s1_rbuf[i].b, s1_wbuf[i].c, s1_rbuf[i].c, s1_wbuf[i].d,
             s1_rbuf[i].d);
      break;
    }
  ...
  for (i = 0; i < (int)block[0]; i++)
    if (s1_rbuf[i].a != s1_wbuf[i].a ||
        s1_rbuf[i].b != (4 * (i + (int)start[0]) + 1) ||
        s1_rbuf[i].c != s1_wbuf[i].c ||
        s1_rbuf[i].d != (4 * (i + (int)start[0]) + 3)) {
      nerrors++;
      printf("\n     Error in 2nd data verification:\n");
      printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
             i + (int)start[0], s1_wbuf[i].a, s1_rbuf[i].a,
             (4 * (i + (int)start[0]) + 1), s1_rbuf[i].b, s1_wbuf[i].c,
             s1_rbuf[i].c, (4 * (i + (int)start[0]) + 3), s1_rbuf[i].d);
      break;
    }
  ...
  for (i = 0; i < (int)block[0]; i++)
    if (s2_wbuf[i].a != s2_rbuf[i].a || s2_wbuf[i].b != s2_rbuf[i].b ||
      ...
      printf("     At index %d: %d/%d, %lld/%lld, %d/%d, %d/%d\n",
             i + (int)start[0], s2_wbuf[i].a, s2_rbuf[i].a, s2_wbuf[i].b,
             s2_rbuf[i].b, s2_wbuf[i].c, s2_rbuf[i].c, s2_wbuf[i].d,
             s2_rbuf[i].d);
      break;
    }
...
/*
 *  Case 6: Type conversions + some processes have null/empty selections in
 * datasets
 */
static void test_type_conv_sel_empty(hid_t fid, unsigned chunked,
                                     unsigned dtrans, unsigned mwbuf) {
  int i;
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  ...
  /* Generate dataset name */
  HDsnprintf(dset_name, sizeof(dset_name), "tconv_sel_empty_%s_%s_%s",
             chunked ? "chunked" : "contig", dtrans ? "xform" : "noxform",
             mwbuf ? "mwbuf" : "nomwbuf");
  ...
  if (H5Dread(did, H5T_NATIVE_LONG, mspace_id, fspace_id, ntrans_dxpl, lrbuf) <
      0)
    P_TEST_ERROR;
  ...
  /* If dtrans, without data transform set in dxpl */
  if (H5Dread(did, H5T_NATIVE_SHORT, mspace_id, fspace_id, ntrans_dxpl, srbuf) <
      0)
    P_TEST_ERROR;
  ...
  /* If dtrans, without data transform set in dxpl */
  if (H5Dread(did, H5T_NATIVE_LLONG, mspace_id, fspace_id, ntrans_dxpl,
              llrbuf) < 0)
    P_TEST_ERROR;
 ...
 *  --Datasets with/without type conversion+smaller/larger mem type+no
 * background buffer
 *
 ...
 */
static void test_multi_dsets_no_bkg(hid_t fid, unsigned chunked,
                                    unsigned dtrans, unsigned mwbuf) {
  size_t ndsets;
  int i, j;
  hid_t dcpl = H5I_INVALID_HID;
  ...
  for (i = 0; i < (int)ndsets; i++) {
    ...
    if ((dset_dids[i] =
             H5Dcreate2(fid, dset_names[i],
                        ((HDrandom() % 2) ? H5T_NATIVE_LONG : H5T_NATIVE_INT),
  ...
  for (i = 0; i < (int)ndsets; i++) {
    ...
    if (H5Sselect_hyperslab(file_sids[i], H5S_SELECT_SET, start, stride, count,
                            block) < 0)
      P_TEST_ERROR;
  }
  ...
  /* Write data to the dataset with/without data transform */
  if (H5Dwrite_multi(ndsets, dset_dids, mem_tids, mem_sids, file_sids, dxpl,
                     wbufs) < 0)
    P_TEST_ERROR;
  ...
  /* Read data from the dataset (if dtrans, without data transform set in dxpl)
   */
  if (H5Dread_multi(ndsets, dset_dids, mem_tids, mem_sids, file_sids,
                    ntrans_dxpl, rbufs) < 0)
    P_TEST_ERROR;
  ...
  if (dtrans) {
    ...
    /* Read the data from the dataset with data transform set in dxpl */
    if (H5Dread_multi(ndsets, dset_dids, mem_tids, mem_sids, file_sids, dxpl,
                      rbufs) < 0)
      P_TEST_ERROR;
    ...
    for (i = 0; i < (int)ndsets; i++)
      for (j = 0; j < (int)block[0]; j++)
        if (rbufi[i][j] != (2 * trans_wbufi[i][j])) {
          nerrors++;
          printf("\n     Error in 1st (with dtrans) data verification for dset "
                 "%d:\n",
                 i);
          printf("     At index %d: %d, %d\n", j + (int)start[0],
  ...
  /* Copy wbuf if the library will be modifying it */
  if (mwbuf)
    memcpy(total_lwbuf_bak, total_lwbuf,
           ndsets * DSET_SELECT_DIM * sizeof(long));
  ...
  /* Restore wbuf from backup if the library modified it */
  if (mwbuf)
    memcpy(total_lwbuf, total_lwbuf_bak,
           ndsets * DSET_SELECT_DIM * sizeof(long));
  ...
  for (i = 0; i < (int)ndsets; i++) {
    for (j = 0; j < (int)block[0]; j++) {
      if (lrbufi[i][j] != (dtrans ? (2 * trans_lwbufi[i][j]) : lwbufi[i][j])) {
        ...
        printf("     At index %d: %ld/%ld\n", j + (int)start[0],
               (dtrans ? (2 * trans_lwbufi[i][j]) : lwbufi[i][j]),
               lrbufi[i][j]);
        break;
      }
    }
 ...
 *       --Verify values read
 *   Case (b) Update all fields of the compound type in memory write buffer with
 * new unique values dset0:
 ...
 *   Case (c) Update all fields of the compound type in memory read buffer with
 * new unique values Randomized <mm> dataset:
 *       --Read some but not all the compound fields to memory
 *       --Verify the fields have the correct (old(a) or new) values
 *       dset0:
 *       --Untouched
 *       --Read and verify the fields have the correct (old(a) or middle(b))
 * values Remaining datasets:
 *       --Untouched
 *       --Read and verify the fields have the correct old(a) values
 ...
 *      --Verify values read
 */
static void test_multi_dsets_cmpd_with_bkg(hid_t fid, unsigned chunked,
                                           unsigned mwbuf) {
  size_t ndsets;
  int i, j, mm;
  hid_t dcpl = H5I_INVALID_HID;
  ...
  for (i = 0; i < (int)ndsets; i++) {
    ...
    /* Generate dataset name */
    HDsnprintf(dset_names[i], sizeof(dset_names[i]), "multi_cmpd_dset%d_%s_%s",
               i, chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
  ...
  for (i = 0; i < (int)ndsets; i++)
    for (j = 0; j < (int)block[0]; j++) {
      if (wbufi[i][j].a != rbufi[i][j].a || wbufi[i][j].b != rbufi[i][j].b ||
        ...
        printf("\n     Error in 1st data verification for dset %d:\n", i);
        printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
               j + (int)start[0], wbufi[i][j].a, rbufi[i][j].a, wbufi[i][j].b,
               rbufi[i][j].b, wbufi[i][j].c, rbufi[i][j].c, wbufi[i][j].d,
               rbufi[i][j].d);
    ...
    if (i == 0) { /* dset0 */
      for (j = 0; j < (int)block[0]; j++)
        if (wbufi[i][j].a != rbufi[i][j].a ||
          ...
          printf("\n     Error in 2nd data verification for dset %d:\n", i);
          printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
                 j + (int)start[0], wbufi[i][j].a, rbufi[i][j].a,
                 (4 * (j + (int)start[0]) + 1), rbufi[i][j].b, wbufi[i][j].c,
      ...
      for (j = 0; j < (int)block[0]; j++)
        if ((4 * (j + (int)start[0])) != rbufi[i][j].a ||
          ...
          printf("\n     Error in 2nd data verification for dset %d:\n", i);
          printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
                 j + (int)start[0], (4 * (j + (int)start[0])), rbufi[i][j].a,
                 (4 * (j + (int)start[0]) + 1), rbufi[i][j].b,
                 (4 * (j + (int)start[0]) + 2), rbufi[i][j].c,
                 (4 * (j + (int)start[0]) + 3), rbufi[i][j].d);
          break;
  ...
  if (H5Sselect_all(file_sids[mm]) < 0)
    P_TEST_ERROR;
  if (H5Sselect_hyperslab(file_sids[mm], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    P_TEST_ERROR;
  ...
  for (j = 0; j < (int)block[0]; j++)
    if (wbufi[0][j].a != rbufi[0][j].a ||
      ...
      printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
             j + (int)start[0], wbufi[0][j].a, rbufi[0][j].a,
             (4 * (j + (int)start[0]) + 1), rbufi[0][j].b, wbufi[0][j].c,
  ...
  for (j = 0; j < (int)block[0]; j++)
    if (rbufi[mm][j].a != ((4 * (j + (int)start[0])) + (2 * DSET_SELECT_DIM)) ||
      ...
      printf("     At index %d: %d/%d, %d/%d, %d/%d, %d/%d\n",
             ...
             rbufi[mm][j].a, ((4 * (j + (int)start[0])) + 1), rbufi[mm][j].b,
  ...
  for (i = 0; i < (int)ndsets; i++) {
    for (j = 0; j < (int)block[0]; j++)
      if (s2_rbufi[i][j].a != s2_wbufi[i][j].a ||
        ...
        printf("\n     Error in 3rd data verification for dset %d:\n", i);
        printf("     At index %d: %d/%d, %lld/%lld, %d/%d, %d/%d\n",
               j + (int)start[0], s2_wbufi[i][j].a, s2_rbufi[i][j].a,
               s2_wbufi[i][j].b, s2_rbufi[i][j].b, s2_wbufi[i][j].c,
               s2_rbufi[i][j].c, s2_wbufi[i][j].d, s2_rbufi[i][j].d);
        break;
      }
  }
 ...
 */
static void test_multi_dsets_size_change_no_bkg(hid_t fid, unsigned chunked,
                                                unsigned mwbuf) {
  size_t ndsets;
  int i, j;
  hid_t dcpl = H5I_INVALID_HID;
  ...
  for (i = 0; i < (int)ndsets; i++) {
    ...
    /* Generate dataset name */
    HDsnprintf(dset_names[i], sizeof(dset_names[i]), "multi_size_dset%d_%s_%s",
               i, chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");

    /* Create ith dataset */
    if ((dset_dids[i] =
             H5Dcreate2(fid, dset_names[i], H5T_STD_I32BE, file_sids[i],
                        H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
      P_TEST_ERROR;
  }
 ...
 */
static void test_multi_dsets_conv_sel_empty(hid_t fid, unsigned chunked,
                                            unsigned dtrans, unsigned mwbuf) {
  size_t ndsets;
  int i, j;
  hid_t dcpl = H5I_INVALID_HID;
  ...
  for (i = 0; i < (int)ndsets; i++) {
    ...
    /* Generate dataset name */
    HDsnprintf(dset_names[i], sizeof(dset_names[i]),
               "multi_sel_dset%d_%s_%s_%s", i, chunked ? "chunked" : "contig",
               dtrans ? "xform" : "noxform", mwbuf ? "mwbuf" : "nomwbuf");

    if (i == 0) {
      if ((dset_dids[i] =
               H5Dcreate2(fid, dset_names[i], H5T_NATIVE_INT, file_sids[i],
                          H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
        P_TEST_ERROR;
    } else {
      if ((dset_dids[i] = H5Dcreate2(
               fid, dset_names[i],
  ...
  if (NULL == (total_trans_wbuf = (int *)malloc(buf_size)))
    P_TEST_ERROR;
  if (NULL ==
      (total_lrbuf = (long *)malloc(ndsets * DSET_SELECT_DIM * sizeof(long))))
    P_TEST_ERROR;
  ...
  if (MAINPROCESS) {
    if (H5Sselect_hyperslab(file_sids[0], H5S_SELECT_SET, start, stride, count,
                            block) < 0)
      P_TEST_ERROR;
  } else {
  ...
  /* need to make memory space to match for process 0 */
  if (MAINPROCESS) {
    if (H5Sselect_hyperslab(mem_sids[mm], H5S_SELECT_SET, start, stride, count,
                            block) < 0)
      P_TEST_ERROR;
  } else {
    if (H5Sselect_all(mem_sids[mm]) < 0)
  ...
  if (MAINPROCESS) {
    ...
    for (j = 0; j < save_block0; j++)
      if (l_rbufi[0][j] !=
          (dtrans ? (long)trans_wbufi[0][j] : (long)wbufi[0][j])) {
        nerrors++;
  ...
  if (mpi_rank) {
    ...
    for (j = 0; j < (int)block[0]; j++)
      if (l_rbufi[mm][j] !=
          (long)(dtrans ? trans_wbufi[mm][j] : wbufi[mm][j])) {
        nerrors++;
    ...
    for (j = 0; j < (int)dims[0]; j++)
      if (l_rbufi[ll][j] !=
          (long)(dtrans ? trans_wbufi[ll][j] : wbufi[ll][j])) {
        nerrors++;
        printf("     Verify dset %d at index %d: %ld, %ld\n", ll, j,
               (long)(dtrans ? trans_wbufi[ll][j] : wbufi[ll][j]),
               l_rbufi[ll][j]);
 ...
 */
static void test_multi_dsets_all(int niter, hid_t fid, unsigned chunked,
                                 unsigned mwbuf) {
  size_t ndsets;
  int i, j, mm;
  int s, n;
  ...
  /* for n niter to ensure that all randomized dset_types with multi_dset_type_t
   * will be covered */
  for (n = 0; n < niter; n++) {
    ...
    for (i = 0; i < (int)ndsets; i++) {
      ...
      if (mm == 0) {
        ...
        HDsnprintf(dset_names[i], sizeof(dset_names[i]),
                   "multi_all_nconv_dset%d_%s_%s", i,
                   chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
        if ((dset_dids[i] =
                 H5Dcreate2(fid, dset_names[i], H5T_NATIVE_INT, file_sids[i],
                            H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
        ...
        HDsnprintf(dset_names[i], sizeof(dset_names[i]),
                   "multi_all_conv_nbkg_dset%d_%s_%s", i,
                   chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
        if ((dset_dids[i] =
                 H5Dcreate2(fid, dset_names[i], H5T_NATIVE_LONG, file_sids[i],
                            H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
          P_TEST_ERROR;
      } else {
        dset_types[i] = DSET_WITH_CONV_AND_BKG;
        HDsnprintf(dset_names[i], sizeof(dset_names[i]),
                   "multi_all_conv_bkg_dset%d_%s_%s", i,
                   chunked ? "chunked" : "contig", mwbuf ? "mwbuf" : "nomwbuf");
        if ((dset_dids[i] = H5Dcreate2(fid, dset_names[i], s1_tid, file_sids[i],
                                       H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
          P_TEST_ERROR;
      ...
      if (H5Sselect_hyperslab(file_sids[i], H5S_SELECT_SET, start, stride,
                              count, block) < 0)
        P_TEST_ERROR;
    ...
    if (NULL == (ul_total_wbuf2 = (unsigned long *)malloc(buf_size)))
      P_TEST_ERROR;
    if (mwbuf &&
        NULL == (ul_total_wbuf2_bak = (unsigned long *)malloc(buf_size)))
      P_TEST_ERROR;
    buf_size = ndsets * DSET_SELECT_DIM * sizeof(long);
    ...
    for (s = SETTING_A; s <= SETTING_B; s++) {
      ...
      for (i = 0; i < (int)ndsets; i++) {
        ...
        case DSET_WITH_CONV_AND_BKG:
          ...
          if (s == SETTING_A) {
            ...
            for (j = 0; j < (int)block[0]; j++) {
              s4_wbufi3[i][j].b =
                  DSET_SELECT_DIM + (unsigned int)(j + (int)start[0]);
              s4_wbufi3[i][j].d =
                  DSET_SELECT_DIM + (unsigned int)(j + (int)start[0]);
            }
            mem_tids[i] = s4_tid;
      ...
      if (mwbuf) {
        memcpy(total_wbuf1_bak, total_wbuf1,
               ndsets * DSET_SELECT_DIM * sizeof(int));
        memcpy(ul_total_wbuf2_bak, ul_total_wbuf2,
               ndsets * DSET_SELECT_DIM * sizeof(unsigned long));
        memcpy(l_total_wbuf2_bak, l_total_wbuf2,
               ndsets * DSET_SELECT_DIM * sizeof(long));
        memcpy(s1_total_wbuf3_bak, s1_total_wbuf3,
               ndsets * DSET_SELECT_DIM * sizeof(s1_t));
        memcpy(s4_total_wbuf3_bak, s4_total_wbuf3,
               ndsets * DSET_SELECT_DIM * sizeof(s4_t));
      }
      ...
      if (mwbuf) {
        memcpy(total_wbuf1, total_wbuf1_bak,
               ndsets * DSET_SELECT_DIM * sizeof(int));
        memcpy(ul_total_wbuf2, ul_total_wbuf2_bak,
               ndsets * DSET_SELECT_DIM * sizeof(unsigned long));
        memcpy(l_total_wbuf2, l_total_wbuf2_bak,
               ndsets * DSET_SELECT_DIM * sizeof(long));
        memcpy(s1_total_wbuf3, s1_total_wbuf3_bak,
               ndsets * DSET_SELECT_DIM * sizeof(s1_t));
        memcpy(s4_total_wbuf3, s4_total_wbuf3_bak,
               ndsets * DSET_SELECT_DIM * sizeof(s4_t));
      }

      if (H5Dread_multi(ndsets, dset_dids, r_mem_tids, mem_sids, file_sids,
                        dxpl, rbufs) < 0)
        P_TEST_ERROR;
 ...
 *        as needed.
 */
static void test_no_selection_io_cause_mode(const char *filename, hid_t fapl,
                                            uint32_t test_mode) {
  hid_t dcpl = H5I_INVALID_HID;
  hid_t dxpl = H5I_INVALID_HID;
  ...
  if (test_mode & TEST_NOT_CONTIGUOUS_OR_CHUNKED_DATASET) {
    if (H5Pset_layout(dcpl, H5D_COMPACT) < 0)
      P_TEST_ERROR;
    no_selection_io_cause_write_expected |=
        H5D_SEL_IO_NOT_CONTIGUOUS_OR_CHUNKED_DATASET;
    no_selection_io_cause_read_expected |=
        H5D_SEL_IO_NOT_CONTIGUOUS_OR_CHUNKED_DATASET;
  }
  ...
  if (test_mode & TEST_DATATYPE_CONVERSION) {
     ...
     * H5FD__read/write_from_selection() triggering
     * H5D_SEL_IO_NO_VECTOR_OR_SELECTION_IO_CB.
     */
    no_selection_io_cause_read_expected |=
        H5D_SEL_IO_NO_VECTOR_OR_SELECTION_IO_CB;

    /* Exception case: This will turn off selection I/O landing at
     * H5FD__mpio_write() */
  ...
  if ((did = H5Dcreate2(fid, "no_selection_io_cause", H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    P_TEST_ERROR;
...
static void test_get_no_selection_io_cause(const char *filename, hid_t fapl) {
  ...
  test_no_selection_io_cause_mode(filename, fapl,
                                  TEST_NOT_CONTIGUOUS_OR_CHUNKED_DATASET);
  test_no_selection_io_cause_mode(filename, fapl, TEST_DATATYPE_CONVERSION);
  test_no_selection_io_cause_mode(
      filename, fapl, TEST_DATATYPE_CONVERSION | TEST_TCONV_BUF_TOO_SMALL);
  test_no_selection_io_cause_mode(filename, fapl,
                                  TEST_DATATYPE_CONVERSION |
                                      TEST_TCONV_BUF_TOO_SMALL |
                                      TEST_IN_PLACE_TCONV);
 ...
 *
 * Optimized bufs (bufs[1] is NULL) is used when passing as a parameter to the
 * mpio driver for selection I/O.  When computing mpi_bufs_base in that routine,
 * it is not accounted for and therefore causing segmentation fault when running
 * the test.
 *
 * Fix:
...
static void test_bug_optimized_bufs(const char *filename, hid_t fapl) {
  ...
  if ((did = H5Dcreate2(fid, "bug_optimized_bufs", H5T_NATIVE_INT, fspace_id,
                        H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    P_TEST_ERROR;
 ...
 *
 * The file is created with userblock.  Before passing down to the mpio driver
 * for selection I/O, the parameter offsets[] is added by base_addr (size of the
 * uesrblock). For the independent case in the mpio driver for selection I/O,
 * the intermediate routine for the API H5FDread/write_vector_from_selection()
 * is called. The parameter offsets[] is passed as is to the intermediate
 * routine which will be added again by base_addr causing incorrect data
 * retrieval.
 *
 * Fix:
 * The parameter offsets[] needs to be adjusted by the base_addr addition before
 * calling the intermediate routine.
 */
static void test_bug_base_addr(const char *filename, hid_t fapl) {
  ...
  if ((did = H5Dcreate2(fid, "bug_base_addr", H5T_NATIVE_INT, sid, H5P_DEFAULT,
 ...
 * The library eventually calls H5FD__mpio_write() performing scalar calls for
 * the writes
 *
 * For process 0:
 * The library internal calls H5D__write():
 ...
 *
 * Fix:
 * Create wrapper functions for the API H5FDread/write_vector_from_selection()
 * and they will be called by H5FD__mpio_read/write_selection() for independent
 * access.
 *
 */
static void test_bug_api_library(const char *filename, hid_t fapl) {
  ...
  if ((did = H5Dcreate2(fid, "bug_coll_to_ind", H5T_NATIVE_INT, sid,
...
static void test_bugs_select_on(const char *filename, hid_t fapl) {
  if (MAINPROCESS) {
    printf("\n");
    TESTING("to verify bugs exposed when H5D_SELECTION_IO_MODE_ON is set as "
            "library default");
  }
    ...
    for (dtrans = FALSE; dtrans <= TRUE; dtrans++) {
      ...
      for (mwbuf = FALSE; mwbuf <= TRUE; mwbuf++) {
        ...
        for (test_select_config = (int)TEST_NO_TYPE_CONV;
             test_select_config < (int)TEST_SELECT_NTESTS;
             test_select_config++) {
          ...
          case TEST_NO_SIZE_CHANGE_NO_BKG: /* case 2 */
            ...
            /* Data transforms does not apply to the dataset datatype for this
             * test */
            if (dtrans) {
              if (MAINPROCESS)
                SKIPPED();
          ...
          case TEST_MULTI_CONV_SIZE_CHANGE: /* case 9 */
            if (MAINPROCESS)
              TESTING_2(
                  "multi-datasets: type conv + size change + no bkg buffer");
          ...
          case TEST_MULTI_ALL: /* case 11 */
            if (MAINPROCESS)
              TESTING_2(
                  "multi-datasets: no conv + conv without bkg + conv with bkg");
