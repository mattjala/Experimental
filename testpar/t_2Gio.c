...
// static int enable_error_stack = 0;   /* enable error stack; disable=0
// enable=1 */ static const char *TestProgName = NULL; static void
// (*TestPrivateUsage)(void) = NULL; static int (*TestPrivateParser)(int ac,
// char *av[]) = NULL;
 ...
 */
static void slab_set(int mpi_rank, int mpi_size, hsize_t start[],
  ...
  if (VERBOSE_MED) {
    printf("start[]=(%lu,%lu), count[]=(%lu,%lu), stride[]=(%lu,%lu), "
           "block[]=(%lu,%lu), total "
           "datapoints=%lu\n",
 ...
 */
void point_set(hsize_t start[], hsize_t count[], hsize_t stride[],
...
int dataset_vrfy(hsize_t start[], hsize_t count[], hsize_t stride[],
  ...
  if (VERBOSE_MED) {
    ...
    printf(
        "start(%lu, %lu), count(%lu, %lu), stride(%lu, %lu), block(%lu, %lu)\n",
        (unsigned long)start[0], (unsigned long)start[1],
        (unsigned long)count[0], (unsigned long)count[1],
        (unsigned long)stride[0], (unsigned long)stride[1],
        (unsigned long)block[0], (unsigned long)block[1]);
    ...
    for (j = 0; j < block[1]; j++) {
      if (*dataset != *original) {
        if (vrfyerrs++ < MAX_ERR_REPORT || VERBOSE_MED) {
          printf("Dataset Verify failed at [%lu][%lu](row %lu, col %lu): "
                 "expect %d, got %d\n",
...
static int MpioTest2G(MPI_Comm comm) {
  ...
  H5_CHECKED_ASSIGN(slice_per_process, size_t,
                    (shape[0] + (hsize_t)mpi_size - 1) / (hsize_t)mpi_size,
                    hsize_t);
  ...
  status = H5Sselect_hyperslab(filedataspace, H5S_SELECT_SET, h5_offsets, NULL,
                               h5_counts, NULL);
  ...
  status = H5Dwrite(dset_id, H5T_NATIVE_INT, memorydataspace, filedataspace,
                    plist_id, data);
...
void dataset_writeInd(void) {
  ...
  dataset1 = H5Dcreate2(fid, DATASETNAME1, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset1 >= 0), "H5Dcreate2 succeeded");

  /* create another dataset collectively */
  dataset2 = H5Dcreate2(fid, DATASETNAME2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
...
void dataset_readInd(void) {
  ...
  /* allocate memory for data buffer */
  data_array1 =
      (DATATYPE *)malloc((size_t)dim0 * (size_t)dim1 * sizeof(DATATYPE));
  VRFY((data_array1 != NULL), "data_array1 malloc succeeded");
  data_origin1 =
      (DATATYPE *)malloc((size_t)dim0 * (size_t)dim1 * sizeof(DATATYPE));
...
void dataset_writeAll(void) {
  ...
  dataset2 = H5Dcreate2(fid, DATASETNAME2, datatype, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset2 >= 0), "H5Dcreate2 2 succeeded");

  /* create a third dataset collectively */
  dataset3 = H5Dcreate2(fid, DATASETNAME3, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset3 >= 0), "H5Dcreate2 succeeded");

  dataset5 = H5Dcreate2(fid, DATASETNAME7, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset5 >= 0), "H5Dcreate2 succeeded");
  dataset6 = H5Dcreate2(fid, DATASETNAME8, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset6 >= 0), "H5Dcreate2 succeeded");
  dataset7 = H5Dcreate2(fid, DATASETNAME9, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  ...
  dataset4 = H5Dcreate2(fid, DATASETNAME4, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* Dataset3: each process takes a block of rows, except process zero uses
   * "none" selection. */
  slab_set(mpi_rank, mpi_size, start, count, stride, block, BYROW);
  ...
  /* Dataset4: each process writes no data, except process zero uses "all"
   * selection. */
  ...
  ret = H5Sselect_hyperslab(mem_dataspace, H5S_SELECT_SET, start, stride, count,
                            block);
  VRFY((ret >= 0), "H5Sset_hyperslab succeeded");
...
void dataset_readAll(void) {
  ...
  /* set up the coords array selection */
  num_points = (size_t)dim1;
  coords = (hsize_t *)malloc((size_t)dim0 * (size_t)dim1 * MAX_RANK *
                             sizeof(hsize_t));
  VRFY((coords != NULL), "coords malloc succeeded");
  ...
  start[0] = (hsize_t)dim0 / (hsize_t)mpi_size * (hsize_t)mpi_rank;
  start[1] = 0;
  ret = dataset_vrfy(start, count, stride, block,
                     data_array1 + (dim0 / mpi_size * dim1 * mpi_rank),
                     data_origin1);
  if (ret)
    nerrors++;
...
void extend_writeInd(void) {
  ...
  hsize_t max_dims[MAX_RANK] = {H5S_UNLIMITED,
                                H5S_UNLIMITED}; /* dataset maximum dim sizes */
  DATATYPE *data_array1 = NULL;                 /* data buffer */
   ...
   */
  {
    ...
    ret = H5Pget_cache(acc_tpl, &mdc_nelmts, &rdcc_nelmts, &rdcc_nbytes,
                       &rdcc_w0);
...
void extend_writeInd2(void) {
  ...
  /* create an extendible dataset collectively */
  dataset = H5Dcreate2(fid, DATASETNAME1, H5T_NATIVE_INT, fs, H5P_DEFAULT, dcpl,
                       H5P_DEFAULT);
  ...
  ret = H5Sselect_hyperslab(fs, H5S_SELECT_SET, &orig_size, NULL, &one,
                            &orig_size);
  VRFY((ret >= 0), "H5Sselect_hyperslab succeeded");
  ret = H5Dwrite(dataset, H5T_NATIVE_INT, ms, fs, H5P_DEFAULT, written);
  VRFY((ret >= 0), "H5Dwrite succeeded");
...
void compress_readAll(void) {
  ...
  unsigned disable_partial_chunk_filters; /* Whether filters are disabled on
                                             partial chunks */
  ...
  for (disable_partial_chunk_filters = 0; disable_partial_chunk_filters <= 1;
    ...
    if (mpi_rank == 0) {
      ...
      /* Create the file */
      fid = H5Fcreate(h5_rmprefix(filename), H5F_ACC_TRUNC, H5P_DEFAULT,
                      H5P_DEFAULT);
      VRFY((fid > 0), "H5Fcreate succeeded");
      ...
      /* Create dataset */
      dataset = H5Dcreate2(fid, "compressed_data", H5T_NATIVE_INT, dataspace,
                           H5P_DEFAULT, dcpl, H5P_DEFAULT);
      VRFY((dataset > 0), "H5Dcreate2 succeeded");

      /* Write compressed data */
      ret = H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     data_orig);
      VRFY((ret >= 0), "H5Dwrite succeeded");
    ...
    if (dataset > 0) {
      ...
      ret = H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, xfer_plist,
                    data_read);
      VRFY((ret >= 0), "H5Dread succeeded");
      ...
      for (u = 0; u < dim; u++)
        if (data_orig[u] != data_read[u]) {
          printf("Line #%d: written!=retrieved: data_orig[%u]=%d, "
                 "data_read[%u]=%d\n",
                 __LINE__, (unsigned)u, data_orig[u], (unsigned)u,
                 data_read[u]);
          nerrors++;
        }

#ifdef H5_HAVE_PARALLEL_FILTERED_WRITES
      ret = H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, xfer_plist,
                     data_read);
      VRFY((ret >= 0), "H5Dwrite succeeded");
#endif
 ...
 *
 * Input:   selection_mode: changes the way processes select data from the
 * space, as well as some dxpl flags to get collective I/O to break in different
 * ways.
 *
 *          The relevant I/O function and expected response for each mode:
 *              TEST_ACTUAL_IO_MULTI_CHUNK_IND:
 *                  H5D_mpi_chunk_collective_io, each process reports
 * independent I/O
 *
 *              TEST_ACTUAL_IO_MULTI_CHUNK_COL:
 *                  H5D_mpi_chunk_collective_io, each process reports collective
 * I/O
 *
 ...
 *                  H5D_mpi_chunk_collective_io, processes disagree. The root
 * reports collective, the rest report independent I/O
 *
 *              TEST_ACTUAL_IO_DIRECT_MULTI_CHUNK_IND:
 *                  Same test TEST_ACTUAL_IO_MULTI_CHUNK_IND.
 *                  Set directly go to multi-chunk-io without num threshold
 * calc. TEST_ACTUAL_IO_DIRECT_MULTI_CHUNK_COL: Same test
 * TEST_ACTUAL_IO_MULTI_CHUNK_COL. Set directly go to multi-chunk-io without num
 * threshold calc.
 *
 *              TEST_ACTUAL_IO_LINK_CHUNK:
 *                  H5D_link_chunk_collective_io, processes report linked chunk
 * I/O
 *
 ...
 *                  Simple independent I/O. This tests that the defaults are
 * properly set.
 *
 *              TEST_ACTUAL_IO_RESET:
 *                  Performs collective and then independent I/O with the same
 * dxpl to make sure the peroperty is correctly reset to the default on each
 * use. Specifically, this test runs
 * TEST_ACTUAL_IO_MULTI_CHUNK_NO_OPT_MIX_DISAGREE (The most complex case that
 * works on all builds) and then performs an independent read and write with the
 * same dxpls.
 *
 ...
 *          path way to multi-chunk-io by H5FD_MPIO_CHUNK_MULTI_IO instead of
 * num-threshold.
 */
static void test_actual_io_mode(int selection_mode) {
  H5D_mpio_actual_chunk_opt_mode_t actual_chunk_opt_mode_write =
      H5D_MPIO_NO_CHUNK_OPTIMIZATION;
  H5D_mpio_actual_chunk_opt_mode_t actual_chunk_opt_mode_read =
      H5D_MPIO_NO_CHUNK_OPTIMIZATION;
  H5D_mpio_actual_chunk_opt_mode_t actual_chunk_opt_mode_expected =
      H5D_MPIO_NO_CHUNK_OPTIMIZATION;
   ...
   */
  multi_chunk_io = (selection_mode == TEST_ACTUAL_IO_MULTI_CHUNK_IND ||
  ...
  /* Create the dataset */
  dataset = H5Dcreate2(fid, "actual_io", data_type, sid, H5P_DEFAULT, dcpl,
                       H5P_DEFAULT);
  ...
  /* RESET tests that the properties are properly reset to defaults each time
   * I/O is performed. To achieve this, we have RESET perform collective I/O
   * (which would change the values from the defaults) followed by independent
   * I/O (which should report the default values). RESET doesn't need to have a
   * unique selection, so we reuse MULTI_CHUMK_MIX_DISAGREE, which was chosen
   * because it is a complex case that works on all builds. The independent
   * section of RESET can be found at the end of this function.
   */
  ...
  ret = H5Sselect_hyperslab(file_space, H5S_SELECT_SET, start, stride, count,
                            block);
  VRFY((ret >= 0), "H5Sset_hyperslab succeeded");

  /* Create a memory dataspace mirroring the dataset and select the same
   * hyperslab as in the file space.
   */
  mem_space = H5Screate_simple(MAX_RANK, dims, NULL);
  VRFY((mem_space >= 0), "mem_space created");

  ret = H5Sselect_hyperslab(mem_space, H5S_SELECT_SET, start, stride, count,
                            block);
  VRFY((ret >= 0), "H5Sset_hyperslab succeeded");
  ...
  ret = H5Pget_mpio_actual_chunk_opt_mode(dxpl_write,
  ...
  ret =
      H5Pget_mpio_actual_chunk_opt_mode(dxpl_read, &actual_chunk_opt_mode_read);
  VRFY((ret >= 0), "retrieving actual chunk opt mode succeeded");
  ...
  if (actual_chunk_opt_mode_expected != (H5D_mpio_actual_chunk_opt_mode_t)-1 &&
    ...
    HDsnprintf(message, sizeof(message),
               "Actual Chunk Opt Mode has the correct value for %s.\n",
               test_name);
    VRFY((actual_chunk_opt_mode_write == actual_chunk_opt_mode_expected),
         message);
    HDsnprintf(message, sizeof(message),
               "Actual IO Mode has the correct value for %s.\n", test_name);
  ...
  /* To test that the property is successfully reset to the default, we perform
   * some independent I/O after the collective I/O
   */
  if (selection_mode == TEST_ACTUAL_IO_RESET) {
    if (mpi_rank == 0) {
      ...
      VRFY(actual_chunk_opt_mode_write == H5D_MPIO_NO_CHUNK_OPTIMIZATION,
           "actual_chunk_opt_mode has correct value for reset write "
           "(independent)");
      ...
      ret = H5Pget_mpio_actual_io_mode(dxpl_read, &actual_io_mode_read);
      VRFY((ret >= 0), "retrieving actual io mode succeeded");
      ret = H5Pget_mpio_actual_chunk_opt_mode(dxpl_read,
                                              &actual_chunk_opt_mode_read);
      VRFY((ret >= 0), "retrieving actual chunk opt mode succeeded");

      VRFY(actual_chunk_opt_mode_read == H5D_MPIO_NO_CHUNK_OPTIMIZATION,
           "actual_chunk_opt_mode has correct value for reset read "
           "(independent)");
 ...
 *    H5Pget_mpio_no_collective_cause properties in the DXPL have the correct
 * values.
 *
 * Input:
 ...
 *         Test for Data Type Conversion as the cause of breaking collective
 * I/O.
 *
 *       TEST_DATA_TRANSFORMS:
 *         Test for Data Transform feature as the cause of breaking collective
 * I/O.
 *
 ...
 *         Test for Externl-File storage as the cause of breaking collective
 * I/O.
 */
#define FILE_EXTERNAL "nocolcause_extern.data"
static void test_no_collective_cause_mode(int selection_mode) {
  ...
  if (selection_mode & TEST_NOT_SIMPLE_OR_SCALAR_DATASPACES) {
    ...
    /* Create the basic Space */
    /* if this is a compact dataset, create a small dataspace that does not
     * exceed 64K */
    if (selection_mode & TEST_NOT_CONTIGUOUS_OR_CHUNKED_DATASET_COMPACT) {
  ...
  /* Create the dataset */
  dataset = H5Dcreate2(fid, "nocolcause", data_type, sid, H5P_DEFAULT, dcpl,
                       H5P_DEFAULT);
  VRFY((dataset >= 0), "H5Dcreate2() dataset succeeded");
  ...
  if (selection_mode & TEST_DATATYPE_CONVERSION) {
    ...
    /* Disable selection I/O since datatype conversion is supported in
    ...
    no_collective_cause_local_expected |=
        H5D_MPIO_DATATYPE_CONVERSION | H5D_MPIO_NO_SELECTION_IO;
  ...
  if (selection_mode & TEST_DATA_TRANSFORMS) {
    ...
    /* Disable selection I/O since data transforms are supported in collective
     * with selection I/O */
    ret = H5Pset_selection_io(dxpl_write, H5D_SELECTION_IO_MODE_OFF);
    VRFY((ret >= 0), "H5Pset_selection_io succeeded");
  ...
  ret = H5Pget_mpio_no_collective_cause(dxpl_write,
                                        &no_collective_cause_local_write,
  ...
  /* Get the cause of broken collective I/O */
  ret = H5Pget_mpio_no_collective_cause(dxpl_read,
                                        &no_collective_cause_local_read,
  ...
  VRFY((no_collective_cause_local_read == no_collective_cause_local_write),
       "reading and writing are the same for local cause of Broken Collective "
       "I/O");
  VRFY((no_collective_cause_global_read == no_collective_cause_global_write),
       "reading and writing are the same for global cause of Broken Collective "
       "I/O");
  ...
  HDsnprintf(
      ...
      test_name);
  VRFY((no_collective_cause_local_write == no_collective_cause_local_expected),
       message);
  memset(message, 0, sizeof(message));
  HDsnprintf(
      message, sizeof(message),
      ...
      test_name);
  VRFY(
      (no_collective_cause_global_write == no_collective_cause_global_expected),
      message);
...
void dataset_atomicity(void) {
  ...
  /* create datasets */
  dataset1 = H5Dcreate2(fid, DATASETNAME5, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset1 >= 0), "H5Dcreate2 succeeded");

  dataset2 = H5Dcreate2(fid, DATASETNAME6, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  VRFY((dataset2 >= 0), "H5Dcreate2 succeeded");

  /* initialize datasets to 0s */
  if (0 == mpi_rank) {
    ret = H5Dwrite(dataset1, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   write_buf);
    VRFY((ret >= 0), "H5Dwrite dataset1 succeeded");

    ret = H5Dwrite(dataset2, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   write_buf);
    VRFY((ret >= 0), "H5Dwrite dataset2 succeeded");
  }
  ...
  /* The other processes read the entire dataset */
    ...
    ret = H5Dread(dataset1, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                  read_buf);
    VRFY((ret >= 0), "H5Dwrite() dataset multichunk write succeeded");
  }
  ...
  if (0 != mpi_rank) {
    ...
    for (i = 1; i < buf_size; i++) {
      if (read_buf[i] != compare) {
        printf("Atomicity Test Failed Process %d: read_buf[%d] is %d, should "
               "be %d\n",
    ...
    for (i = 0; i < dim0; i++) {
      ...
      for (j = 0; j < dim1; j++) {
        if ((hsize_t)j >= (hsize_t)mpi_rank * (block[1] + 1)) {
          H5_CHECKED_ASSIGN(k, int,
                            (hsize_t)dim1 - (hsize_t)mpi_rank * (block[1] + 1) +
                                (hsize_t)k,
                            hsize_t);
          break;
        }
...
void test_dense_attr(void) {
  ...
  atid = H5Acreate2(gid, "bar", H5T_STD_U64LE, atFileSpace, H5P_DEFAULT,
                    H5P_DEFAULT);
...
int main(int argc, char **argv) {
  ...
  if (mpi_size > 2) {
    ...
    if (mpi_rank >= 2)
      rank_color = 1;
    if (MPI_Comm_split(test_comm, rank_color, mpi_rank, &test_comm) !=
        MPI_SUCCESS) {
  ...
  if (mpi_rank < 2) {
    ...
    AddTest("idsetw", dataset_writeInd, NULL, "dataset independent write",
            PARATESTFILE);

    AddTest("idsetr", dataset_readInd, NULL, "dataset independent read",
            PARATESTFILE);

    AddTest("cdsetw", dataset_writeAll, NULL, "dataset collective write",
            PARATESTFILE);

    AddTest("cdsetr", dataset_readAll, NULL, "dataset collective read",
            PARATESTFILE);
