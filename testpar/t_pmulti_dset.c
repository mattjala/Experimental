...
#define MDSET_ALL_FLAGS                                                        \
  (MDSET_FLAG_CHUNK | MDSET_FLAG_MLAYOUT | MDSET_FLAG_SHAPESAME |              \
   MDSET_FLAG_MDSET | MDSET_FLAG_COLLECTIVE | MDSET_FLAG_COLLECTIVE_OPT |      \
   MDSET_FLAG_TCONV | MDSET_FLAG_FILTER)
...
static void test_pmdset(size_t niter, unsigned flags) {
  ...
  op_data_incr =
      (unsigned)max_dsets * MAX_DSET_X * MAX_DSET_Y * (unsigned)mpi_size;
  ...
  if (NULL == (efbuf = (unsigned *)malloc(buf_size)))
    T_PMD_ERROR;
  if (NULL == (dset_usage = (unsigned char *)malloc(max_dsets * MAX_DSET_X *
                                                    MAX_DSET_Y)))
    T_PMD_ERROR;
  ...
  for (i = 0; i < max_dsets; i++)
    for (j = 0; j < MAX_DSET_X; j++) {
      ...
      dset_usagei[i][j] =
          dset_usage + (i * MAX_DSET_X * MAX_DSET_Y) + (j * MAX_DSET_Y);
    } /* end for */
  ...
  if (flags & MDSET_FLAG_COLLECTIVE) {
    ...
    if (flags & MDSET_FLAG_COLLECTIVE_OPT) {
      if (H5Pset_dxpl_mpio_collective_opt(dxpl_id, H5FD_MPIO_COLLECTIVE_IO) < 0)
        T_PMD_ERROR;
    } else if (H5Pset_dxpl_mpio_collective_opt(dxpl_id,
                                               H5FD_MPIO_INDIVIDUAL_IO) < 0)
      T_PMD_ERROR;
  } /* end if */
    ...
    for (j = 0; j < max_dsets; j++)
      for (k = 0; k < MAX_DSET_X; k++)
        for (l = 0; l < MAX_DSET_Y; l++)
          wbufi[j][k][l] =
              (unsigned)(((unsigned)mpi_rank * max_dsets * MAX_DSET_X *
                          MAX_DSET_Y) +
    ...
    for (j = 0; j < OPS_PER_FILE; j++) {
      ...
      for (k = 0; k < ndsets; k++) {
        ...
        for (l = 0; l < (size_t)mpi_size; l++) {
          ...
          rank_data_diff =
              (int)((unsigned)max_dsets * MAX_DSET_X * MAX_DSET_Y) *
              ((int)l - (int)mpi_rank);
          ...
          if (HDrandom() % 2) {
            ...
            size_t max_hs_x =
                (MAX_HS_X <= dset_dims[k][0])
                    ? MAX_HS_X
                    : dset_dims[k]
                               [0]; /* Determine maximum hyperslab size in X */
            size_t max_hs_y =
                (MAX_HS_Y <= dset_dims[k][1])
                    ? MAX_HS_Y
                    : dset_dims[k]
                               [1]; /* Determine maximum hyperslab size in Y */
            ...
            for (m = 0; m < nhs; m++) {
              ...
              for (n = 0; overlap && (n < MAX_SEL_RETRIES); n++) {
                ...
                start[m][0] = (count[m][0] == dset_dims[k][0])
                                  ? 0
                                  : (hsize_t)HDrandom() %
                                        (dset_dims[k][0] - count[m][0] + 1);
                start[m][1] = (count[m][1] == dset_dims[k][1])
                                  ? 0
                                  : (hsize_t)HDrandom() %
                                        (dset_dims[k][1] - count[m][1] + 1);

                /* If writing, check for overlap with other processes */
                overlap = FALSE;
                if (!do_read)
                  for (o = start[m][0];
                       (o < (start[m][0] + count[m][0])) && !overlap; o++)
                    for (p = start[m][1];
                         (p < (start[m][1] + count[m][1])) && !overlap; p++)
              ...
              if (l == (size_t)mpi_rank) {
                if (H5Sselect_hyperslab(mem_space_ids[k], H5S_SELECT_OR,
                                        start[m], NULL, count[m], NULL) < 0)
                  T_PMD_ERROR;
                if (H5Sselect_hyperslab(file_space_ids[k], H5S_SELECT_OR,
                                        start[m], NULL, count[m], NULL) < 0)
                  T_PMD_ERROR;
              } /* end if */
              ...
              if (do_read) {
                ...
                for (n = start[m][0]; n < (start[m][0] + count[m][0]); n++)
                  for (o = start[m][1]; o < (start[m][1] + count[m][1]); o++)
                    efbufi[k][n][o] =
                        (unsigned)((int)wbufi[k][n][o] + rank_data_diff);
            } /* end for */
            ...
            /* Reset dataset usage array if reading, since in this case we don't
             * care about overlapping selections between processes */
            if (do_read)
              memset(dset_usage, 0, max_dsets * MAX_DSET_X * MAX_DSET_Y);
            ...
            for (m = 0; m < npoints; m++) {
              ...
              for (n = 0; overlap && (n < MAX_SEL_RETRIES); n++) {
                /* Generate point */
                points[2 * m] =
                    (unsigned)((hsize_t)HDrandom() % dset_dims[k][0]);
                points[(2 * m) + 1] =
                    (unsigned)((hsize_t)HDrandom() % dset_dims[k][1]);

                /* Check for overlap with other processes (write) or this
                 * process (always) */
                overlap = FALSE;
              ...
              /* Update dataset usage array after each point to prevent the same
               * point being selected twice by a single process, since this is
               * not supported by MPI */
              dset_usagei[k][points[2 * m]][points[(2 * m) + 1]] =
                  (unsigned char)1;
            } /* end for */
            ...
            if ((l == (size_t)mpi_rank) && (npoints > 0))
              if (H5Sselect_elements(file_space_ids[k], H5S_SELECT_APPEND,
                                     npoints, points) < 0)
                T_PMD_ERROR;
            ...
            if (do_read) {
              ...
              for (m = 0; m < npoints; m++)
                efbufi[k][points[2 * m]][points[(2 * m) + 1]] =
                    (unsigned)((int)wbufi[k][points[2 * m]]
                                         [points[(2 * m) + 1]] +
                               rank_data_diff);
            ...
            if ((l == (size_t)mpi_rank) && (npoints > 0)) {
              ...
              /* Select elements */
              if (H5Sselect_elements(mem_space_ids[k], H5S_SELECT_APPEND,
                                     npoints, points) < 0)
                T_PMD_ERROR;
            } /* end if */
          }   /* end else */
      ...
      if (do_read) {
        if (flags & MDSET_FLAG_MDSET) {
          ...
          if (H5Dread_multi(ndsets, dset_ids, mem_type_ids, mem_space_ids,
                            file_space_ids, dxpl_id, rbufs) < 0)
            T_PMD_ERROR;
        } /* end if */
        else
          /* Read */
        ...
        if (flags & MDSET_FLAG_MDSET) {
          ...
          if (H5Dwrite_multi(ndsets, dset_ids, mem_type_ids, mem_space_ids,
                             file_space_ids, dxpl_id, wbufs) < 0)
            T_PMD_ERROR;
        } /* end if */
        else
          /* Write */
