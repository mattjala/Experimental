...
/*
   This program will test irregular hyperslab selections with collective write
   and read. The way to test whether collective write and read works is to use
   independent IO output to verify the collective output.
 ...
 *-------------------------------------------------------------------------
 */
void coll_irregular_cont_write(void) { coll_write_test(0); }
 ...
 *-------------------------------------------------------------------------
 */
void coll_irregular_cont_read(void) { coll_read_test(); }
 ...
 *-------------------------------------------------------------------------
 */
void coll_irregular_simple_chunk_write(void) { coll_write_test(1); }
 ...
 *-------------------------------------------------------------------------
 */
void coll_irregular_simple_chunk_read(void) { coll_read_test(); }

/*-------------------------------------------------------------------------
 * Function:    coll_irregular_complex_chunk_write
 *
 * Purpose:    Wrapper to test the collectively irregular hyperslab write in
 *chunk storage(4 chunks)
 *
 * Return:    Success:    0
 *
 *        Failure:    -1
 ...
 *-------------------------------------------------------------------------
 */
void coll_irregular_complex_chunk_write(void) { coll_write_test(4); }
 ...
 *-------------------------------------------------------------------------
 */
void coll_irregular_complex_chunk_read(void) { coll_read_test(); }
...
void coll_write_test(int chunk_factor) {
  ...
  matrix_out = (int *)malloc(sizeof(int) * (size_t)mdim[0] * (size_t)mdim[1] *
                             (size_t)mpi_size);
  matrix_out1 = (int *)malloc(sizeof(int) * (size_t)mdim[0] * (size_t)mdim[1] *
                              (size_t)mpi_size);
   ...
   */
  datasetc = H5Dcreate2(file, "collect_write", H5T_NATIVE_INT, fspaceid,
                        H5P_DEFAULT, dcrt_plist, H5P_DEFAULT);
  VRFY((datasetc >= 0), "dataset created succeeded");

  dataseti = H5Dcreate2(file, "independ_write", H5T_NATIVE_INT, fspaceid,
                        H5P_DEFAULT, dcrt_plist, H5P_DEFAULT);
  ...
  ret = H5Sselect_hyperslab(fspaceid, H5S_SELECT_SET, start, stride, count,
                            block);
  ...
  ret =
      H5Sselect_hyperslab(fspaceid, H5S_SELECT_OR, start, stride, count, block);
  VRFY((ret >= 0), "hyperslab selection succeeded");
  ...
  ret = H5Sselect_hyperslab(mspaceid1, H5S_SELECT_SET, start, stride, count,
                            block);
  VRFY((ret >= 0), "hyperslab selection succeeded");

  /* independent write */
  ret = H5Dwrite(dataseti, H5T_NATIVE_INT, mspaceid1, fspaceid, H5P_DEFAULT,
                 vector);
  ...
  /* collective write */
  ret = H5Dwrite(datasetc, H5T_NATIVE_INT, mspaceid1, fspaceid, dxfer_plist,
                 vector);
  ...
  ***/
  ...
  ret = H5Sselect_hyperslab(fspaceid1, H5S_SELECT_SET, start, stride, count,
                            block);
  VRFY((ret >= 0), "hyperslab selection succeeded");
  ...
  ret = H5Sselect_hyperslab(fspaceid1, H5S_SELECT_OR, start, stride, count,
                            block);
  VRFY((ret >= 0), "hyperslab selection succeeded");
  ...
  ret = H5Sselect_hyperslab(mspaceid, H5S_SELECT_SET, start, stride, count,
                            block);
  VRFY((ret >= 0), "hyperslab selection succeeded");
  ...
  ret =
      H5Sselect_hyperslab(mspaceid, H5S_SELECT_OR, start, stride, count, block);
  VRFY((ret >= 0), "hyperslab selection succeeded");
  ...
  memset(matrix_out, 0,
         sizeof(int) * (size_t)MSPACE_DIM1 * (size_t)MSPACE_DIM2 *
             (size_t)mpi_size);
  memset(matrix_out1, 0,
         sizeof(int) * (size_t)MSPACE_DIM1 * (size_t)MSPACE_DIM2 *
             (size_t)mpi_size);
  /*
  ...
  ret = H5Dread(datasetc, H5T_NATIVE_INT, mspaceid, fspaceid, H5P_DEFAULT,
                matrix_out);
  VRFY((ret >= 0), "H5D independent read succeed");

  ret = H5Dread(dataseti, H5T_NATIVE_INT, mspaceid, fspaceid, H5P_DEFAULT,
                matrix_out1);
  VRFY((ret >= 0), "H5D independent read succeed");
...
static void coll_read_test(void) {
  ...
  mdim[0] = MSPACE_DIM1;
  mdim[1] = (hsize_t)(MSPACE_DIM2 * mpi_size);
  matrix_out = (int *)malloc(sizeof(int) * (size_t)MSPACE_DIM1 *
                             (size_t)MSPACE_DIM2 * (size_t)mpi_size);
  matrix_out1 = (int *)malloc(sizeof(int) * (size_t)MSPACE_DIM1 *
                              (size_t)MSPACE_DIM2 * (size_t)mpi_size);
  ...
  /* Collective read */
  ret = H5Dread(dataseti, H5T_NATIVE_INT, mspaceid, fspaceid1, dxfer_plist,
                matrix_out);
  ...
  /* Independent read */
  ret = H5Dread(dataseti, H5T_NATIVE_INT, mspaceid, fspaceid1, H5P_DEFAULT,
                matrix_out1);
  VRFY((ret >= 0), "H5D independent read succeed");
...
static void lower_dim_size_comp_test__select_checker_board(
    const int mpi_rank, const hid_t tgt_sid, const int tgt_rank,
  ...
  offset_count = (hsize_t)((dims[sel_offset] - (hsize_t)checker_edge_size) /
  ...
  if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
    fprintf(stdout, "%s:%d: base_count/offset_count = %d/%d.\n", fcnName,
            mpi_rank, base_count, offset_count);
  }
#endif /* LOWER_DIM_SIZE_COMP_TEST__SELECT_CHECKER_BOARD__DEBUG */
            ...
            if (((i + j + k + l + m) % 2) == 0) {
              ...
              if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
                ...
                fprintf(stdout, "%s:%d: stride = %d %d %d %d %d.\n", fcnName,
                        mpi_rank, (int)stride[0], (int)stride[1],
              ...
              if (first_selection) {
                ...
                ret = H5Sselect_hyperslab(
                    tgt_sid, H5S_SELECT_SET, &(start[ds_offset]),
                ...
                ret = H5Sselect_hyperslab(
                    tgt_sid, H5S_SELECT_OR, &(start[ds_offset]),
                    &(stride[ds_offset]), &(count[ds_offset]),
                    &(block[ds_offset]));
...
static hbool_t lower_dim_size_comp_test__verify_data(
    uint32_t *buf_ptr,
...
static void lower_dim_size_comp_test__run_test(const int chunk_edge_size,
  ...
  if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
    fprintf(stdout, "%s:%d: small ds size / slice size = %d / %d.\n", fcnName,
            mpi_rank, (int)small_ds_size, (int)small_ds_slice_size);
    fprintf(stdout, "%s:%d: large ds size / slice size = %d / %d.\n", fcnName,
            mpi_rank, (int)large_ds_size, (int)large_ds_slice_size);
  }
#endif /* LOWER_DIM_SIZE_COMP_TEST__RUN_TEST__DEBUG */
  ...
  if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
    fprintf(stdout, "%s:%d: small_dims[] = %d %d %d %d %d\n", fcnName, mpi_rank,
            (int)small_dims[0], (int)small_dims[1], (int)small_dims[2],
            (int)small_dims[3], (int)small_dims[4]);
    fprintf(stdout, "%s:%d: large_dims[] = %d %d %d %d %d\n", fcnName, mpi_rank,
            (int)large_dims[0], (int)large_dims[1], (int)large_dims[2],
            (int)large_dims[3], (int)large_dims[4]);
  }
#endif
  ...
  if (chunk_edge_size > 0) {
    ...
    if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
      fprintf(stdout, "%s:%d: small chunk dims[] = %d %d %d %d %d\n", fcnName,
              mpi_rank, (int)small_chunk_dims[0], (int)small_chunk_dims[1],
              (int)small_chunk_dims[2], (int)small_chunk_dims[3],
    ...
    large_chunk_dims[1] = large_chunk_dims[2] = large_chunk_dims[3] =
        large_chunk_dims[4] = (hsize_t)chunk_edge_size;

#if LOWER_DIM_SIZE_COMP_TEST__RUN_TEST__DEBUG
    if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
      fprintf(stdout, "%s:%d: large chunk dims[] = %d %d %d %d %d\n", fcnName,
              mpi_rank, (int)large_chunk_dims[0], (int)large_chunk_dims[1],
              (int)large_chunk_dims[2], (int)large_chunk_dims[3],
  ...
  small_dataset = H5Dcreate2(fid, "small_dataset", dset_type, file_small_ds_sid,
                             H5P_DEFAULT, small_ds_dcpl_id, H5P_DEFAULT);
  VRFY((ret >= 0), "H5Dcreate2() small_dataset succeeded");

  /* create the large dataset */
  large_dataset = H5Dcreate2(fid, "large_dataset", dset_type, file_large_ds_sid,
                             H5P_DEFAULT, large_ds_dcpl_id, H5P_DEFAULT);
  VRFY((ret >= 0), "H5Dcreate2() large_dataset succeeded");

#if LOWER_DIM_SIZE_COMP_TEST__RUN_TEST__DEBUG
  if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
    fprintf(stdout, "%s:%d: small/large ds id = %d / %d.\n", fcnName, mpi_rank,
            (int)small_dataset, (int)large_dataset);
  }
#endif /* LOWER_DIM_SIZE_COMP_TEST__RUN_TEST__DEBUG */
  ...
  ret = H5Sselect_hyperslab(mem_small_ds_sid, H5S_SELECT_SET, start, stride,
                            count, block);
  VRFY((ret >= 0), "H5Sselect_hyperslab(mem_small_ds_sid, set) succeeded");

  ret = H5Sselect_hyperslab(file_small_ds_sid, H5S_SELECT_SET, start, stride,
                            count, block);
  ...
  if (MAINPROCESS) { /* add an additional slice to the selections */
    ...
    ret = H5Sselect_hyperslab(mem_small_ds_sid, H5S_SELECT_OR, start, stride,
                              count, block);
    VRFY((ret >= 0), "H5Sselect_hyperslab(mem_small_ds_sid, or) succeeded");

    ret = H5Sselect_hyperslab(file_small_ds_sid, H5S_SELECT_OR, start, stride,
                              count, block);
   ...
   */
  ret = H5Dread(small_dataset, H5T_NATIVE_UINT32, full_mem_small_ds_sid,
                full_file_small_ds_sid, xfer_plist, small_ds_buf_1);
  ...
  ret = H5Sselect_hyperslab(mem_large_ds_sid, H5S_SELECT_SET, start, stride,
                            count, block);
  VRFY((ret >= 0), "H5Sselect_hyperslab(mem_large_ds_sid, set) succeeded");

  ret = H5Sselect_hyperslab(file_large_ds_sid, H5S_SELECT_SET, start, stride,
                            count, block);
  ...
  if (MAINPROCESS) { /* add an additional slice to the selections */
    ...
    ret = H5Sselect_hyperslab(mem_large_ds_sid, H5S_SELECT_OR, start, stride,
                              count, block);
    VRFY((ret >= 0), "H5Sselect_hyperslab(mem_large_ds_sid, or) succeeded");

    ret = H5Sselect_hyperslab(file_large_ds_sid, H5S_SELECT_OR, start, stride,
                              count, block);
  ...
  ret = H5Sselect_hyperslab(mem_large_ds_sid, H5S_SELECT_AND, start, stride,
                            count, block);
  VRFY((ret != FAIL), "H5Sselect_hyperslab(mem_large_ds_sid, and) succeeded");

  ret = H5Sselect_hyperslab(file_large_ds_sid, H5S_SELECT_AND, start, stride,
                            count, block);
  ...
  if (mpi_rank == LOWER_DIM_SIZE_COMP_TEST_DEBUG_TARGET_RANK) {
    ...
    fprintf(stdout, "%s:%d: mem_large_ds_sid dims[%d] = %d %d %d %d %d\n",
            fcnName, mpi_rank, rank, (int)dims[0], (int)dims[1], (int)dims[2],
   ...
   * data set.
   */
  ret = H5Dread(large_dataset, H5T_NATIVE_UINT32, full_mem_large_ds_sid,
                full_file_large_ds_sid, xfer_plist, large_ds_buf_1);
  ...
  small_sel_start[1] = small_sel_start[2] = small_sel_start[3] =
      small_sel_start[4] = 0;

  lower_dim_size_comp_test__select_checker_board(
      mpi_rank, file_small_ds_sid,
      /* tgt_rank          = */ 5, small_dims,
      /* checker_edge_size = */ 3,
      /* sel_rank */ 2, small_sel_start);

  expected_value =
      (uint32_t)((small_sel_start[0] * small_dims[1] * small_dims[2] *
  ...
  ret = H5Dread(small_dataset, H5T_NATIVE_UINT32, mem_large_ds_sid,
                file_small_ds_sid, xfer_plist, large_ds_buf_1);
  ...
  start_index =
      (int)((large_sel_start[0] * large_dims[1] * large_dims[2] *
  ...
  data_ok = lower_dim_size_comp_test__verify_data(
      ptr_1,
#if LOWER_DIM_SIZE_COMP_TEST__VERIFY_DATA__DEBUG
      mpi_rank,
#endif /* LOWER_DIM_SIZE_COMP_TEST__VERIFY_DATA__DEBUG */
  ...
  ret = H5Dread(large_dataset, H5T_NATIVE_UINT32, mem_small_ds_sid,
                file_large_ds_sid, xfer_plist, small_ds_buf_1);
  ...
  expected_value =
      (uint32_t)((large_sel_start[0] * large_dims[1] * large_dims[2] *
                  large_dims[3] * large_dims[4]) +
                 (large_sel_start[1] * large_dims[2] * large_dims[3] *
                  large_dims[4]) +
                 (large_sel_start[2] * large_dims[3] * large_dims[4]) +
...
void lower_dim_size_comp_test(void) {
  ...
  for (use_collective_io = 0; use_collective_io <= 1; use_collective_io++) {
    chunk_edge_size = 0;
    lower_dim_size_comp_test__run_test(
        chunk_edge_size, (hbool_t)use_collective_io, H5T_NATIVE_UINT);
...
void link_chunk_collective_io_test(void) {
  ...
  dims[0] = ((hsize_t)mpi_size) *
            ((hsize_t)(LINK_CHUNK_COLLECTIVE_IO_TEST_CHUNK_SIZE));
  ...
  /* create the data set */
  dset_id = H5Dcreate2(file_id, "dataset", H5T_NATIVE_DOUBLE, file_ds_sid,
                       H5P_DEFAULT, ds_dcpl_id, H5P_DEFAULT);
  ...
  /* setup local data */
  expected_value =
      (double)(LINK_CHUNK_COLLECTIVE_IO_TEST_CHUNK_SIZE) * (double)(mpi_rank);
  ...
  ret = H5Sselect_hyperslab(file_ds_sid, H5S_SELECT_SET, start, stride, count,
                            block);
  ...
  /* write the data set */
  ret = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, write_mem_ds_sid, file_ds_sid,
                 xfer_plist, local_data_written);
  ...
  ret = H5Dread(dset_id, H5T_NATIVE_DOUBLE, read_mem_ds_sid, file_ds_sid,
                xfer_plist, local_data_read);
