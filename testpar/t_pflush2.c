 ...
 *              that a file can be read after a parallel application crashes as
 * long as the file was flushed first.  We simulate a crash by calling _exit(0)
 * since this doesn't flush HDF5 caches but still exits with success.
 */
    ...
    for (j = 0; j < dims[1]; j++) {
      ...
      if (data_g[(i * 100) + j] != val) {
        ...
        printf("    data_g[%lu][%lu] = %d\n", (unsigned long)i,
               (unsigned long)j, data_g[(i * 100) + j]);
        printf("    should be %d\n", val);
      }
    }
  }
