...
hbool_t disp_failure_mssgs =
    TRUE; /* global force display of failure messages */
...
static void setup_vfd_test_file(int file_name_id, char *file_name, int mpi_size,
                                H5FD_mpio_xfer_t xfer_mode,
                                H5FD_mpio_collective_opt_t coll_opt_mode,
                                const char *vfd_name, haddr_t eoa,
                                H5FD_t **lf_ptr, hid_t *fapl_id_ptr,
                                hid_t *dxpl_id_ptr);
static void takedown_vfd_test_file(int mpi_rank, char *filename,
                                   H5FD_t **lf_ptr, hid_t *fapl_id_ptr,
                                   hid_t *dxpl_id_ptr);

/* test functions */
static unsigned vector_read_test_1(int file_name_id, int mpi_rank, int mpi_size,
                                   H5FD_mpio_xfer_t xfer_mode,
                                   H5FD_mpio_collective_opt_t coll_opt_mode,
                                   const char *vfd_name);
static unsigned vector_read_test_2(int file_name_id, int mpi_rank, int mpi_size,
                                   H5FD_mpio_xfer_t xfer_mode,
                                   H5FD_mpio_collective_opt_t coll_opt_mode,
                                   const char *vfd_name);
static unsigned vector_read_test_3(int file_name_id, int mpi_rank, int mpi_size,
                                   H5FD_mpio_xfer_t xfer_mode,
                                   H5FD_mpio_collective_opt_t coll_opt_mode,
                                   const char *vfd_name);
static unsigned vector_read_test_4(int file_name_id, int mpi_rank, int mpi_size,
                                   H5FD_mpio_xfer_t xfer_mode,
                                   H5FD_mpio_collective_opt_t coll_opt_mode,
                                   const char *vfd_name);
static unsigned vector_read_test_5(int file_name_id, int mpi_rank, int mpi_size,
                                   H5FD_mpio_xfer_t xfer_mode,
                                   H5FD_mpio_collective_opt_t coll_opt_mode,
                                   const char *vfd_name);

static unsigned vector_write_test_1(int file_name_id, int mpi_rank,
                                    int mpi_size, H5FD_mpio_xfer_t xfer_mode,
                                    H5FD_mpio_collective_opt_t coll_opt_mode,
                                    const char *vfd_name);
static unsigned vector_write_test_2(int file_name_id, int mpi_rank,
                                    int mpi_size, H5FD_mpio_xfer_t xfer_mode,
                                    H5FD_mpio_collective_opt_t coll_opt_mode,
                                    const char *vfd_name);
static unsigned vector_write_test_3(int file_name_id, int mpi_rank,
                                    int mpi_size, H5FD_mpio_xfer_t xfer_mode,
                                    H5FD_mpio_collective_opt_t coll_opt_mode,
                                    const char *vfd_name);
static unsigned vector_write_test_4(int file_name_id, int mpi_rank,
                                    int mpi_size, H5FD_mpio_xfer_t xfer_mode,
                                    H5FD_mpio_collective_opt_t coll_opt_mode,
                                    const char *vfd_name);
static unsigned vector_write_test_5(int file_name_id, int mpi_rank,
                                    int mpi_size, H5FD_mpio_xfer_t xfer_mode,
                                    H5FD_mpio_collective_opt_t coll_opt_mode,
                                    const char *vfd_name);
static unsigned vector_write_test_6(int file_name_id, int mpi_rank,
                                    int mpi_size, H5FD_mpio_xfer_t xfer_mode,
                                    H5FD_mpio_collective_opt_t coll_opt_mode,
                                    const char *vfd_name);
static unsigned vector_write_test_7(int file_name_id, int mpi_rank,
                                    int mpi_size, H5FD_mpio_xfer_t xfer_mode,
                                    H5FD_mpio_collective_opt_t coll_opt_mode,
                                    const char *vfd_name);
/*
 * Tests for selection I/O:
 * They are derived from test_selection_io() in test/vfd.c and modified for
 * parallel testing.
 */
...
#define CHECK_PASSED()                                                         \
  do {                                                                         \
    int err_result = (nerrors > curr_nerrors);                                 \
                                                                               \
    MPI_Allreduce(MPI_IN_PLACE, &err_result, 1, MPI_INT, MPI_MAX,              \
                  MPI_COMM_WORLD);                                             \
                                                                               \
    if (MAINPROCESS) {                                                         \
      if (err_result == 0)                                                     \
...
static herr_t test_selection_io_read_verify(
    hid_t dxpl, int mpi_rank, hsize_t start[], hsize_t block[], H5FD_t *lf,
...
static herr_t test_selection_io_write(hid_t dxpl, H5FD_t *lf, H5FD_mem_t type,
...
static void test_selection_io_real(int mpi_rank, int mpi_size, H5FD_t *lf,
                                   hid_t dxpl);
static void test_selection_io_types_1d(int mpi_rank, int mpi_size, H5FD_t *lf,
                                       hid_t dxpl, H5FD_mem_t type,
                                       haddr_t addrs[], size_t element_sizes[],
                                       hid_t mem_spaces[], hid_t file_spaces[],
                                       hsize_t dims1[]);
static void test_selection_io_types_2d(int mpi_rank, int mpi_size, H5FD_t *lf,
                                       hid_t dxpl, H5FD_mem_t type,
                                       haddr_t addrs[], size_t element_sizes[],
                                       hid_t mem_spaces[], hid_t file_spaces[],
                                       hsize_t dims2[]);
static void test_selection_io_types_1d_2d(
    int mpi_rank, int mpi_size, H5FD_t *lf, hid_t dxpl, H5FD_mem_t type,
    haddr_t addrs[], size_t element_sizes[], hid_t mem_spaces[],
    hid_t file_spaces[], hsize_t dims1[], hsize_t dims2[]);
static void test_selection_io_types_shorten(
    int mpi_rank, int mpi_size, H5FD_t *lf, hid_t dxpl, H5FD_mem_t type,
    haddr_t addrs[], size_t element_sizes[], hid_t mem_spaces[],
    hid_t file_spaces[], hsize_t dims1[], hsize_t dims2[]);
...
static unsigned alloc_and_init_file_images(int mpi_size) {
  ...
  if (pass) {
    ...
    if ((!increasing_fi_buf) || (!decreasing_fi_buf) || (!negative_fi_buf) ||
        (!zero_fi_buf) || (!read_fi_buf)) {
...
static void setup_vfd_test_file(int file_name_id, char *file_name, int mpi_size,
  ...
  /* setup the file name -- do this now, since setting up the ioc faple requires
   * it.  This will probably change */
  if (pass) {

    if (h5_fixname(FILENAMES[file_name_id], H5P_DEFAULT, filename,
  ...
  if (pass) {
    ...
    eoa =
        (haddr_t)mpi_size * (haddr_t)INTS_PER_RANK * (haddr_t)(sizeof(int32_t));
...
static unsigned vector_read_test_1(int file_name_id, int mpi_rank, int mpi_size,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 1 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 1 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 1 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  /* 1) Open the test file with the specified VFD, set the eoa, and setup the
   * dxpl */
  if (pass) {
    ...
    setup_vfd_test_file(file_name_id, filename, mpi_size, xfer_mode,
                        coll_opt_mode, vfd_name, eoa, &lf, &fapl_id, &dxpl_id);
  }
  ...
  if (pass) {
    ...
    size_t image_size =
        (size_t)mpi_size * (size_t)INTS_PER_RANK * sizeof(int32_t);

    if (mpi_rank == 0) {

      if (H5FDwrite(lf, H5FD_MEM_DRAW, H5P_DEFAULT, (haddr_t)0, image_size,
  ...
  if (pass) {
    ...
    addrs[0] =
        (haddr_t)mpi_rank * (haddr_t)INTS_PER_RANK * (haddr_t)(sizeof(int32_t));
    ...
    for (i = 0; i < mpi_size * INTS_PER_RANK; i++) {
      ...
      if ((i < mpi_rank * INTS_PER_RANK) ||
          (i >= (mpi_rank + 1) * INTS_PER_RANK)) {
...
static unsigned vector_read_test_2(int file_name_id, int mpi_rank, int mpi_size,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 2 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 2 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 2 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    if (mpi_rank % 2 == 0) {
      ...
      count = 1;
      types[0] = H5FD_MEM_DRAW;
      addrs[0] = (haddr_t)mpi_rank * (haddr_t)INTS_PER_RANK *
                 (haddr_t)(sizeof(int32_t));
      sizes[0] = (size_t)INTS_PER_RANK * sizeof(int32_t);
      bufs[0] = (void *)(&(read_fi_buf[mpi_rank * INTS_PER_RANK]));
    } else {
...
static unsigned vector_read_test_3(int file_name_id, int mpi_rank, int mpi_size,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 3 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 3 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 3 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    addrs[1] =
        base_addr + ((haddr_t)(INTS_PER_RANK / 4) * (haddr_t)(sizeof(int32_t)));
    sizes[1] = (size_t)(INTS_PER_RANK / 4) * sizeof(int32_t);

    types[2] = H5FD_MEM_DRAW;
    addrs[2] =
        base_addr + ((haddr_t)(INTS_PER_RANK / 2) * (haddr_t)(sizeof(int32_t)));
    sizes[2] = (size_t)(INTS_PER_RANK / 4) * sizeof(int32_t);

    types[3] = H5FD_MEM_DRAW;
    addrs[3] = base_addr +
               ((haddr_t)(3 * INTS_PER_RANK / 4) * (haddr_t)(sizeof(int32_t)));
  ...
  if (pass) {
    ...
    for (i = 0; ((pass) && (i < INTS_PER_RANK / 4)); i++) {
      ...
      if (((mpi_rank % 2 == 0) &&
           (buf_0[i] != negative_fi_buf[base_index + i])) ||
          ((mpi_rank % 2 == 1) &&
           (buf_3[i] != negative_fi_buf[base_index + i]))) {
    ...
    for (i = 0; ((pass) && (i < INTS_PER_RANK / 4)); i++) {
      ...
      if (((mpi_rank % 2 == 0) &&
           (buf_1[i] != negative_fi_buf[base_index + i])) ||
          ((mpi_rank % 2 == 1) &&
           (buf_2[i] != negative_fi_buf[base_index + i]))) {
...
static unsigned vector_read_test_4(int file_name_id, int mpi_rank, int mpi_size,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 4 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 4 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 4 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    if ((mpi_rank % 4) == 0) {
      ...
      types[0] = H5FD_MEM_DRAW;
      addrs[0] =
          base_addr + (haddr_t)((size_t)(INTS_PER_RANK / 2) * sizeof(int32_t));
      sizes[0] = (size_t)(INTS_PER_RANK / 4) * sizeof(int32_t);
      bufs[0] = (void *)(&(read_fi_buf[base_index + (INTS_PER_RANK / 2)]));

      types[1] = H5FD_MEM_DRAW;
      addrs[1] =
          base_addr + (haddr_t)((size_t)(INTS_PER_RANK / 4) * sizeof(int32_t));
      sizes[1] = (size_t)(INTS_PER_RANK / 8) * sizeof(int32_t);
      bufs[1] = (void *)(&(read_fi_buf[base_index + (INTS_PER_RANK / 4)]));

      types[2] = H5FD_MEM_DRAW;
      addrs[2] =
          base_addr + (haddr_t)((size_t)(INTS_PER_RANK / 16) * sizeof(int32_t));
      ...
      types[1] = H5FD_MEM_DRAW;
      addrs[1] = base_addr +
                 (haddr_t)((size_t)((INTS_PER_RANK / 2) + 1) * sizeof(int32_t));
...
static unsigned vector_read_test_5(int file_name_id, int mpi_rank, int mpi_size,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 5 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 5 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector read test 5 -- %s / col op / col I/O",
                 vfd_name);
    }
...
static unsigned vector_write_test_1(int file_name_id, int mpi_rank,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 1 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 1 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 1 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    if (H5FDread(lf, H5FD_MEM_DRAW, H5P_DEFAULT, (haddr_t)0, image_size,
                 (void *)read_fi_buf) < 0) {
...
static unsigned vector_write_test_2(int file_name_id, int mpi_rank,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 2 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 2 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 2 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    for (i = 0; ((pass) && (i < mpi_size)); i++) {
      ...
      if (i % 2 == 1) { /* odd block */
        ...
        for (j = i * INTS_PER_RANK; ((pass) && (j < (i + 1) * INTS_PER_RANK));
             j++) {
...
static unsigned vector_write_test_3(int file_name_id, int mpi_rank,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 3 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 3 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 3 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    bufs[1] = (const void *)(&(
        decreasing_fi_buf[(mpi_rank * INTS_PER_RANK) + (INTS_PER_RANK / 4)]));

    types[2] = H5FD_MEM_DRAW;
    addrs[2] = addrs[1] + (haddr_t)(bytes_per_write);
    sizes[2] = bytes_per_write;
    bufs[2] = (const void *)(&(
        negative_fi_buf[(mpi_rank * INTS_PER_RANK) + (INTS_PER_RANK / 2)]));

    types[3] = H5FD_MEM_DRAW;
    addrs[3] = addrs[2] + (haddr_t)(bytes_per_write);
    sizes[3] = bytes_per_write;
    bufs[3] = (const void *)(&(
        zero_fi_buf[(mpi_rank * INTS_PER_RANK) + (3 * (INTS_PER_RANK / 4))]));
...
static unsigned vector_write_test_4(int file_name_id, int mpi_rank,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 4 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 4 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 4 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    sizes[1] = bytes_per_write;
    bufs[1] = (const void *)(&(
        negative_fi_buf[(mpi_rank * INTS_PER_RANK) + (INTS_PER_RANK / 4)]));

    types[2] = H5FD_MEM_DRAW;
    addrs[2] = addrs[1] + (haddr_t)(bytes_per_write);
    sizes[2] = bytes_per_write;
    bufs[2] = (const void *)(&(
        decreasing_fi_buf[(mpi_rank * INTS_PER_RANK) + (INTS_PER_RANK / 2)]));

    types[3] = H5FD_MEM_DRAW;
    addrs[3] = addrs[2] + (haddr_t)(bytes_per_write);
    sizes[3] = bytes_per_write;
    bufs[3] = (const void *)(&(increasing_fi_buf[(mpi_rank * INTS_PER_RANK) +
                                                 (3 * (INTS_PER_RANK / 4))]));
 ...
 *                       INTS_PER_RANK / 2 + 1 and running for (INTS_PER_RANK /
 *2)
 *                       - 2 entries
 *
...
static unsigned vector_write_test_5(int file_name_id, int mpi_rank,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 5 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 5 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 5 -- %s / col op / col I/O",
                 vfd_name);
    }
  ...
  if (pass) {
    ...
    if ((mpi_rank % 4) == 0) {
      ...
      sizes[0] = (size_t)(INTS_PER_RANK / 4) * sizeof(int32_t);
      bufs[0] =
          (const void *)(&(negative_fi_buf[base_index + (INTS_PER_RANK / 2)]));
      ...
      sizes[1] = (size_t)(INTS_PER_RANK / 8) * sizeof(int32_t);
      bufs[1] = (const void *)(&(
          decreasing_fi_buf[base_index + (INTS_PER_RANK / 4)]));
      ...
      sizes[2] = (size_t)(INTS_PER_RANK / 16) * sizeof(int32_t);
      bufs[2] = (const void *)(&(
          increasing_fi_buf[base_index + (INTS_PER_RANK / 16)]));
    } else if ((mpi_rank % 4) == 1) {
      ...
      sizes[1] = (size_t)((INTS_PER_RANK / 2) - 2) * sizeof(int32_t);
      bufs[1] = (const void *)(&(
          decreasing_fi_buf[base_index + (INTS_PER_RANK / 2) + 1]));
    } else if ((mpi_rank % 4) == 2) {
          ...
          if (((INTS_PER_RANK / 2) <= k) && (k < (3 * (INTS_PER_RANK / 4)))) {
            ...
            if (read_fi_buf[j] != negative_fi_buf[j]) {
              ...
              printf("\nread_fi_buf[%d] = %d, %d expected.\n", j,
                     read_fi_buf[j], negative_fi_buf[j]);
            }
          } else if (((INTS_PER_RANK / 4) <= k) &&
                     (k < (3 * (INTS_PER_RANK / 8)))) {
            ...
            if (read_fi_buf[j] != decreasing_fi_buf[j]) {
              ...
              printf("\nread_fi_buf[%d] = %d, %d expected.\n", j,
                     read_fi_buf[j], decreasing_fi_buf[j]);
            }
            ...
            if (read_fi_buf[j] != increasing_fi_buf[j]) {
              ...
              printf("\nread_fi_buf[%d] = %d, %d expected.\n", j,
                     read_fi_buf[j], increasing_fi_buf[j]);
            }
          } else {
...
static unsigned vector_write_test_6(int file_name_id, int mpi_rank,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 6 -- %s / independent", vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 6 -- %s / col op / ind I/O",
                 vfd_name);
    } else {
      ...
      HDsnprintf(test_title, sizeof(test_title),
                 "parallel vector write test 6 -- %s / col op / col I/O",
                 vfd_name);
    }
...
static unsigned vector_write_test_7(int file_name_id, int mpi_rank,
  ...
  if (mpi_rank == 0) {
    ...
    if (xfer_mode == H5FD_MPIO_INDEPENDENT) {
      ...
      HDsprintf(test_title, "parallel vector write test 7 -- %s / independent",
                vfd_name);
    } else if (coll_opt_mode == H5FD_MPIO_INDIVIDUAL_IO) {

      HDsprintf(test_title,
                "parallel vector write test 7 -- %s / col op / ind I/O",
                vfd_name);
    } else {
      ...
      HDsprintf(test_title,
                "parallel vector write test 7 -- %s / col op / col I/O",
                vfd_name);
    }
  ...
  if (pass) {
    ...
    for (i = 0; i < (int)count; i++) {
      ...
      bufs[i] = (void *)(&(
          increasing_fi_buf[base_index + (i * (INTS_PER_RANK / 8))]));
    }
...
static void test_vector_io(int mpi_rank, int mpi_size) {
  ...
  if (!pass) {
    ...
    printf("\nAllocation and initialize of file image buffers failed.  Test "
           "aborted.\n");
    nerrors += (int)nerrs;
    return;
  }
  ...
  nerrs += vector_read_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_read_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_read_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_read_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_read_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_read_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_write_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_1(0, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_write_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_2(1, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_write_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_3(2, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_write_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_4(3, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_write_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_5(4, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_write_test_6(5, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_6(5, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_6(5, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, "mpio");

  nerrs += vector_write_test_7(6, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_7(6, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, "mpio");
  nerrs += vector_write_test_7(6, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, "mpio");
  ...
  nerrs += vector_read_test_1(7, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_1(7, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_1(7, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_read_test_2(8, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_2(8, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_2(8, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_read_test_3(9, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_3(9, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_3(9, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_read_test_4(10, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_4(10, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_4(10, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_read_test_5(11, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_5(11, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_read_test_5(11, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                              H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_write_test_1(7, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_1(7, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_1(7, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_write_test_2(8, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_2(8, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_2(8, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_write_test_3(9, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_3(9, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_3(9, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_write_test_4(10, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_4(10, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_4(10, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_write_test_5(11, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_5(11, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_5(11, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_write_test_6(12, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_6(12, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_6(12, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);

  nerrs += vector_write_test_7(13, mpi_rank, mpi_size, H5FD_MPIO_INDEPENDENT,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_7(13, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_INDIVIDUAL_IO, H5FD_SUBFILING_NAME);
  nerrs += vector_write_test_7(13, mpi_rank, mpi_size, H5FD_MPIO_COLLECTIVE,
                               H5FD_MPIO_COLLECTIVE_IO, H5FD_SUBFILING_NAME);
#endif
...
static herr_t test_selection_io_read_verify(
  ...
  /* Issue read call */
  if (H5FDread_selection(lf, type, dxpl, count, mem_spaces, file_spaces,
                         offsets, element_sizes, (void **)rbufs) < 0)
    goto error;

  /* Verify result */
  for (i = 0; i < (int)rbufcount; i++) {
    hsize_t endblock =
        MIN((start[i] + block[i]), (hsize_t)(sel_dim0 * sel_dim1));
    for (j = (int)start[i]; j < (int)endblock; j++)
      if (rbufs[i][j] != erb[i][j]) {
        H5_FAILED();
        AT();
        printf("data read from file does not match expected values at mapping "
               "array location %d: %d\n",
               i, mpi_rank);
...
static herr_t test_selection_io_write(hid_t dxpl, H5FD_t *lf, H5FD_mem_t type,
  ...
  /* Issue write call */
  if (H5FDwrite_selection(lf, type, dxpl, count, mem_spaces, file_spaces,
...
/*
 * Perform the following tests that use shortened arrays for wbuf and element
 * sizes
 * --Test 1: Strided <> Strided 1D and 2D I/O for both file and memory spaces
 * --Reset selections
 * --Test 2: Strided <> Strided 2D I/O, 2 different selections in the same
 * memory buffer
 * --Reset selections
 */
static void test_selection_io_types_shorten(
  ...
  for (shorten_element_sizes = 0; shorten_element_sizes <= 1;
       shorten_element_sizes++) {
    /*
    ...
    if (H5Sselect_hyperslab(mem_spaces[0], H5S_SELECT_SET, start, stride, count,
                            block) < 0)
      P_TEST_ERROR;
    ...
    if (H5Sselect_hyperslab(file_spaces[0], H5S_SELECT_SET, start, stride,
                            count, block) < 0)
      P_TEST_ERROR;
    ...
    start[1] = 1;
    if (H5Sselect_hyperslab(file_spaces[1], H5S_SELECT_SET, start, stride,
                            count, block) < 0)
      P_TEST_ERROR;
    ...
    start[0] = 1 + ((hsize_t)mpi_rank * stride[0] * count[0]);
    start[1] = 0;
    if (H5Sselect_hyperslab(mem_spaces[1], H5S_SELECT_SET, start, stride, count,
                            block) < 0)
      P_TEST_ERROR;
    ...
    /* Issue write call */
    if (test_selection_io_write(dxpl, lf, type, 2, mem_spaces, file_spaces,
                                addrs, element_sizes, (int **)wbufs) < 0)
      P_TEST_ERROR;
    ...
    /* Read and verify */
    if (test_selection_io_read_verify(
            dxpl, mpi_rank, verify_start, verify_block, lf, type, 2, mem_spaces,
    ...
    /* Update file bufs - need to reuse 1D array so data stays consistent, so
     * use math to find 1D index into 2D array */
...
static void test_selection_io_types_1d_2d(
  ...
  start[0] = (hsize_t)mpi_rank * count[0];
  start[1] = 1;
  if (H5Sselect_hyperslab(file_spaces[1], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    P_TEST_ERROR;
  ...
  /* Issue write call */
  if (test_selection_io_write(dxpl, lf, type, 1, &mem_spaces[0],
                              &file_spaces[1], &addrs[1], element_sizes,
                              (int **)&wbufs[0]) < 0)
    P_TEST_ERROR;
  ...
  if (test_selection_io_read_verify(dxpl, mpi_rank, verify_start, verify_block,
                                    lf, type, 1, &mem_spaces[0],
                                    &file_spaces[1], &addrs[1], element_sizes,
                                    1, (int **)&erbufs[0], FALSE) < 0)
    P_TEST_ERROR;
  ...
  stride[0] = 2;
  start[0] = (hsize_t)mpi_rank * stride[0] * count[0];
  if (H5Sselect_hyperslab(file_spaces[0], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    P_TEST_ERROR;
  ...
  /* Issue write call */
  if (test_selection_io_write(dxpl, lf, type, 1, &mem_spaces[1],
                              &file_spaces[0], &addrs[0], element_sizes,
                              (int **)&wbufs[1]) < 0)
    P_TEST_ERROR;
  ...
  verify_block[0] = (count[0] * count[1] * stride[0]);
  if (test_selection_io_read_verify(dxpl, mpi_rank, verify_start, verify_block,
                                    lf, type, 1, &mem_spaces[1],
                                    &file_spaces[0], &addrs[0], element_sizes,
                                    1, (int **)&erbufs[1], FALSE) < 0)
    P_TEST_ERROR;
...
static void test_selection_io_types_2d(int mpi_rank, int mpi_size, H5FD_t *lf,
  ...
  start[1] = 0;
  if (H5Sselect_hyperslab(file_spaces[1], H5S_SELECT_SET, start, NULL, count,
                          NULL) < 0)
    P_TEST_ERROR;
  ...
  for (i = 0; i < sel_dim0 / 2; i++)
    for (j = 0; j < sel_dim1; j++)
      erbuf2[(((2 * i) + 1) * sel_dim1) + j] =
          wbuf2[(((2 * i) + 1) * sel_dim1) + j];
  ...
  if (H5Sselect_hyperslab(mem_spaces[1], H5S_SELECT_SET, start, NULL, count,
                          NULL) < 0)
    P_TEST_ERROR;
...
static void test_selection_io_types_1d(int mpi_rank, int mpi_size, H5FD_t *lf,
  ...
  count[0] = (hsize_t)(((sel_dim0 * sel_dim1) / 2) / mpi_size);
  start[0] = 1 + ((hsize_t)mpi_rank * count[0]);
  if (H5Sselect_hyperslab(file_spaces[0], H5S_SELECT_SET, start, NULL, count,
                          NULL) < 0)
    P_TEST_ERROR;
  ...
  /* Strided selection in file */
  block[0] = 1;
  count[0] = (hsize_t)(((sel_dim0 * sel_dim1) / 2) /
                       mpi_size); /* count is this value from twice above */
  ...
  /* Contiguous selection in memory */
  start[0] = 1 + ((hsize_t)mpi_rank * count[0]);
  if (H5Sselect_hyperslab(mem_spaces[0], H5S_SELECT_SET, start, NULL, count,
                          NULL) < 0)
    P_TEST_ERROR;
 ...
 *  test_selection_io_types_shorten()
 *      --Selection I/O tests that use shortened arrays for wbuf and element
 * sizes
 */
static void test_selection_io_real(int mpi_rank, int mpi_size, H5FD_t *lf,
                                   hid_t dxpl) {
  hid_t mem_spaces[2] = {H5I_INVALID_HID,
                         H5I_INVALID_HID}; /* memory dataspaces vector */
  hid_t file_spaces[2] = {H5I_INVALID_HID,
                          H5I_INVALID_HID}; /* file dataspaces vector */
  ...
  for (type = 1; type < H5FD_MEM_NTYPES; type++) {
    ...
    addrs[0] = H5FDalloc(lf, type, H5P_DEFAULT,
                         (sizeof(int) * (hsize_t)sel_dim0 * (hsize_t)sel_dim1));
    addrs[1] = H5FDalloc(lf, type, H5P_DEFAULT,
                         (sizeof(int) * (hsize_t)sel_dim0 * (hsize_t)sel_dim1));

    test_selection_io_types_1d(mpi_rank, mpi_size, lf, dxpl, type, addrs,
                               element_sizes, mem_spaces, file_spaces, dims1);
    test_selection_io_types_2d(mpi_rank, mpi_size, lf, dxpl, type, addrs,
                               element_sizes, mem_spaces, file_spaces, dims2);
    test_selection_io_types_1d_2d(mpi_rank, mpi_size, lf, dxpl, type, addrs,
                                  element_sizes, mem_spaces, file_spaces, dims1,
                                  dims2);
    test_selection_io_types_shorten(mpi_rank, mpi_size, lf, dxpl, type, addrs,
                                    element_sizes, mem_spaces, file_spaces,
                                    dims1, dims2);
...
static void test_selection_io(int mpi_rank, int mpi_size) {
  ...
  hid_t col_xfer_dxpl =
      H5I_INVALID_HID; /* dxpl: collective access with collective I/O */
  hid_t ind_io_dxpl =
      H5I_INVALID_HID; /* dxpl: collective access with individual I/O */
  ...
  /* Set dxpl for collective access which will have H5FD_MPIO_COLLECTIVE_IO as
   * default */
  ...
  for (collective = 0; collective < iotypes; collective++) {
    ...
    if (MAINPROCESS) {
      if (collective) {
        if (collective == 1)
          printf(
              "     Testing with Collective access: collective I/O         ");
        else
          printf(
              "     Testing with Collective_access: Individual I/O         ");
      } else
