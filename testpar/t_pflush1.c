...
static hid_t create_test_file(char *name, size_t name_length, hid_t fapl_id) {
  ...
  /* Create some groups */
  if ((top_level_gid = H5Gcreate2(fid, "some_groups", H5P_DEFAULT, H5P_DEFAULT,
                                  H5P_DEFAULT)) < 0)
    goto error;
  for (i = 0; i < N_GROUPS; i++) {
    HDsnprintf(name, name_length, "grp%02u", (unsigned)i);
    if ((gid = H5Gcreate2(top_level_gid, name, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto error;
    if (H5Gclose(gid) < 0)
      goto error;
...
int main(int argc, char *argv[]) {
   ...
   * Then the _exit is still needed to stop at_exit from happening in some
   * systems. Note that MPIO VFD returns the address of the file-handle in the
   * VFD struct because MPI_File_close wants to modify the file-handle variable.
   */
