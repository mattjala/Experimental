...
#define CHECK_PASSED()                                                         \
  do {                                                                         \
    int err_result = (nerrors > curr_nerrors);                                 \
                                                                               \
    mpi_code_g =                                                               \
        MPI_Allreduce(MPI_IN_PLACE, &err_result, 1, MPI_INT, MPI_MAX, comm_g); \
...
/* Utility functions */
static hid_t create_subfiling_ioc_fapl(MPI_Comm comm, MPI_Info info,
                                       hbool_t custom_config,
                                       H5FD_subfiling_params_t *custom_cfg,
...
static void test_create_and_close(void) {
  ...
  H5E_BEGIN_TRY { H5Fdelete(SUBF_FILENAME, fapl_id); }
  H5E_END_TRY
...
static void test_ioc_only_fail(void) {
  ...
  VRFY((H5Pset_mpi_params(fapl_id, comm_g, info_g) >= 0),
       "H5Pset_mpi_params succeeded");
...
static void test_config_file(void) {
   ...
   */
  if (mpi_rank == 0) {
    stripe_size = (rand() % (H5FD_SUBFILING_DEFAULT_STRIPE_SIZE -
                             SUBFILING_MIN_STRIPE_SIZE + 1)) +
  ...
  if (MAINPROCESS) {
    ...
    VRFY((fread(config_buf, (size_t)config_file_len, 1, config_file) == 1),
         "fread succeeded");
    ...
    VRFY((HDsscanf(substr, "stripe_size=%" PRId64, &read_stripe_size) == 1),
         "HDsscanf succeeded");
    VRFY((read_stripe_size == cfg.stripe_size),
         "Stripe size comparison succeeded");

    /* Check the aggregator_count field in the configuration file */
    substr = HDstrstr(config_buf, "aggregator_count");
    VRFY(substr, "HDstrstr succeeded");

    VRFY((HDsscanf(substr, "aggregator_count=%d", &read_aggr_count) == 1),
         "HDsscanf succeeded");
    if (cfg.stripe_count < num_iocs_g)
      VRFY((read_aggr_count == cfg.stripe_count),
           "Aggregator count comparison succeeded");
    else
      VRFY((read_aggr_count == num_iocs_g),
           "Aggregator count comparison succeeded");
    ...
    VRFY((HDsscanf(substr, "subfile_count=%d", &read_stripe_count) == 1),
         "HDsscanf succeeded");
    VRFY((read_stripe_count == cfg.stripe_count),
         "Stripe count comparison succeeded");
    ...
    VRFY((H5_dirname(resolved_path, &subfile_dir) >= 0),
         "H5_dirname succeeded");
    ...
    HDsnprintf(scan_format, sizeof(scan_format), "hdf5_file=%%%zus",
               (size_t)(PATH_MAX - 1));
    ...
    HDsnprintf(scan_format, sizeof(scan_format), "subfile_dir=%%%zus",
               (size_t)(PATH_MAX - 1));
    VRFY((HDsscanf(substr, scan_format, tmp_buf) == 1), "HDsscanf succeeded");
    ...
    for (size_t i = 0; i < (size_t)cfg.stripe_count; i++) {
      HDsnprintf(subfile_name, PATH_MAX, H5FD_SUBFILING_FILENAME_TEMPLATE,
                 SUBF_FILENAME, (uint64_t)file_info.st_ino, num_digits,
                 (int)i + 1, cfg.stripe_count);
    ...
    HDsnprintf(subfile_name, PATH_MAX, H5FD_SUBFILING_FILENAME_TEMPLATE,
               SUBF_FILENAME, (uint64_t)file_info.st_ino, num_digits,
               (int)cfg.stripe_count + 1, cfg.stripe_count);
    substr = HDstrstr(config_buf, subfile_name);
...
static void test_stripe_sizes(void) {
  ...
  for (size_t i = 0; i < SUBF_NITER; i++) {
    ...
    if (MAINPROCESS) {
      ...
      fapl_id =
          create_subfiling_ioc_fapl(MPI_COMM_SELF, MPI_INFO_NULL, TRUE, &cfg,
                                    H5FD_IOC_DEFAULT_THREAD_POOL_SIZE);
      VRFY((fapl_id >= 0), "FAPL creation succeeded");
      ...
      VRFY((H5FDset_eoa(file_ptr, H5FD_MEM_DEFAULT, file_end_addr + nbytes) >=
            0),
      ...
      write_status = H5FDwrite(file_ptr, H5FD_MEM_DRAW, dxpl_id, write_addr,
                               nbytes, c_write_buf);
      ...
      for (int j = 0; j < num_subfiles; j++) {
        ...
        HDsnprintf(tmp_filename, PATH_MAX, H5FD_SUBFILING_FILENAME_TEMPLATE,
                   SUBF_FILENAME, (uint64_t)file_info.st_ino, num_digits, j + 1,
                   num_subfiles);
        ...
        VRFY((subfile_size >= cfg.stripe_size),
             "File size verification succeeded");
      }
      ...
      write_addr = file_end_addr;
      write_status = H5FDwrite_vector(file_ptr, dxpl_id, 1, &write_type,
      ...
      for (int j = 0; j < num_subfiles; j++) {
        ...
        VRFY((subfile_size >= 2 * cfg.stripe_size),
             "File size verification succeeded");
      }
    ...
    /* Set EOA for following write call */
    VRFY((H5FDset_eoa(file_ptr, H5FD_MEM_DEFAULT,
                      file_end_addr + ((size_t)mpi_size * nbytes)) >= 0),
         "H5FDset_eoa succeeded");
    ...
    if (MAINPROCESS) {
      ...
      for (int j = 0; j < num_subfiles; j++) {
        ...
        VRFY((subfile_size >= (mpi_size * cfg.stripe_size)),
             "File size verification succeeded");
      }
    ...
    if (MAINPROCESS) {
      ...
      for (int j = 0; j < num_subfiles; j++) {
        ...
        VRFY((subfile_size >= (2 * mpi_size * cfg.stripe_size)),
             "File size verification succeeded");
      }
...
static void test_selection_strategies(void) {
  ...
  for (H5FD_subfiling_ioc_select_t strategy = 0;
       strategy < ioc_selection_options; strategy++) {
     ...
     */
    if (strategy == SELECT_IOC_ONE_PER_NODE ||
        strategy == SELECT_IOC_WITH_CONFIG)
      continue;

    /* Test with 1 MPI rank and then all MPI ranks */
    for (size_t num_ranks_choice = 0; num_ranks_choice < NUM_RANKS_CHOICES;
         num_ranks_choice++) {
       ...
       */
      for (size_t criteria_format_choice = 0;
           criteria_format_choice < NUM_CRITERIA_FORMATS;
           criteria_format_choice++) {
        MPI_Comm file_comm = comm_g;
        char criteria_buf[256];
        char
            sel_criteria[128]; /* Use char buffer for criteria as we may support
        ...
        VRFY(HDsetenv(H5FD_SUBFILING_IOC_SELECTION_CRITERIA, criteria_buf, 1) >=
                 0,
             "HDsetenv succeeded");
        ...
        if ((num_active_ranks == mpi_size) || (mpi_rank == 0)) {
          ...
          fapl_id = create_subfiling_ioc_fapl(
              file_comm, info_g, TRUE, &cfg, H5FD_IOC_DEFAULT_THREAD_POOL_SIZE);
          VRFY((fapl_id >= 0), "FAPL creation succeeded");

          file_id =
              H5Fcreate(SUBF_FILENAME, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_id);
          VRFY((file_id >= 0), "H5Fcreate succeeded");
          ...
          VRFY(HDunsetenv(H5FD_SUBFILING_IOC_SELECTION_CRITERIA) >= 0,
               "HDunsetenv succeeded");
        }
...
static void test_read_different_stripe_size(void) {
  ...
  VRFY((H5Sselect_hyperslab(fspace_id, H5S_SELECT_SET, start, NULL, count,
                            NULL) >= 0),
  ...
  VRFY((H5Dwrite(dset_id, SUBF_HDF5_TYPE, H5S_BLOCK, fspace_id, dxpl_id, buf) >=
        0),
  ...
  VRFY((H5Dread(dset_id, SUBF_HDF5_TYPE, H5S_BLOCK, fspace_id, dxpl_id, buf) >=
        0),
  ...
  for (size_t i = 0; i < count[0]; i++) {
    ...
    VRFY((buf_value == (SUBF_C_TYPE)((size_t)mpi_rank + i)),
...
static void test_subfiling_precreate_rank_0(void) {
  ...
  target_size = (stripe_size_g > 0) ? (size_t)stripe_size_g
  ...
  if (MAINPROCESS) {
    ...
    cfg.ioc_selection = SELECT_IOC_ONE_PER_NODE;
    cfg.stripe_size = (stripe_size_g > 0) ? stripe_size_g
                                          : H5FD_SUBFILING_DEFAULT_STRIPE_SIZE;
    cfg.stripe_count = num_iocs_g;
    ...
    for (size_t i = 0; i < dset_dims[0]; i++)
      ((SUBF_C_TYPE *)buf)[i] =
          (SUBF_C_TYPE)((i / n_elements_per_rank) + (i % n_elements_per_rank));

    VRFY((H5Dwrite(dset_id, SUBF_HDF5_TYPE, H5S_BLOCK, fspace_id, dxpl_id,
                   buf) >= 0),
         "Dataset write succeeded");
    ...
    for (int i = 0; i < num_subfiles; i++) {
      ...
      HDsnprintf(tmp_filename, PATH_MAX, H5FD_SUBFILING_FILENAME_TEMPLATE,
                 SUBF_FILENAME, (uint64_t)file_info.st_ino, num_digits, i + 1,
                 num_subfiles);
...
static void test_subfiling_write_many_read_one(void) {
  ...
  if (MAINPROCESS) {
    fapl_id =
        create_subfiling_ioc_fapl(MPI_COMM_SELF, MPI_INFO_NULL, FALSE, NULL, 0);
    VRFY((fapl_id >= 0), "FAPL creation succeeded");
    ...
    VRFY((H5Dread(dset_id, SUBF_HDF5_TYPE, H5S_BLOCK, H5S_ALL, dxpl_id, buf) >=
          0),
         "Dataset read succeeded");
    ...
    for (size_t i = 0; i < (size_t)mpi_size; i++) {
      for (size_t j = 0; j < count[0]; j++) {
        ...
        VRFY((buf_value == (SUBF_C_TYPE)(j + i)),
             "data verification succeeded");
      }
    }
...
static void test_subfiling_write_many_read_few(void) {
  ...
  if (reading_file) {
    fapl_id =
        create_subfiling_ioc_fapl(sub_comm, MPI_INFO_NULL, FALSE, NULL, 0);
    VRFY((fapl_id >= 0), "FAPL creation succeeded");
...
static void test_subfiling_h5fuse(void) {
  ...
  if (MAINPROCESS) {
    ...
    if (pid == 0) {
      ...
      HDsnprintf(tmp_filename, PATH_MAX,
                 "%s/" H5FD_SUBFILING_CONFIG_FILENAME_TEMPLATE, config_dir,
    ...
    VRFY((HDstat(SUBF_FILENAME, &file_info) >= 0), "HDstat succeeded");
    VRFY(((size_t)file_info.st_size >= target_size),
         "File size verification succeeded");
  ...
  if (MAINPROCESS) {
    ...
    /* Generate name for configuration file */
    HDsnprintf(filename_buf, PATH_MAX,
               "%s/" H5FD_SUBFILING_CONFIG_FILENAME_TEMPLATE, config_dir,
               SUBF_FILENAME, file_inode);
    ...
    for (int i = 0; i < num_subfiles; i++) {
      ...
      HDsnprintf(filename_buf, PATH_MAX, H5FD_SUBFILING_FILENAME_TEMPLATE,
                 SUBF_FILENAME, file_inode, num_digits, i + 1, num_subfiles);
...
int main(int argc, char **argv) {
  ...
  HDcompile_assert(SUBFILING_MIN_STRIPE_SIZE <=
                   H5FD_SUBFILING_DEFAULT_STRIPE_SIZE);
  ...
  if (MPI_SUCCESS !=
      (mpi_code_g = MPI_Comm_split_type(comm_g, MPI_COMM_TYPE_SHARED, mpi_rank,
  ...
  if (MPI_SUCCESS !=
      (mpi_code_g = MPI_Comm_size(node_local_comm, &node_local_size))) {
    if (MAINPROCESS)
      printf("MPI_Comm_size failed with error code %d\n", mpi_code_g);
    nerrors++;
    goto exit;
  ...
  if (MPI_SUCCESS !=
      (mpi_code_g = MPI_Comm_rank(node_local_comm, &node_local_rank))) {
    if (MAINPROCESS)
      printf("MPI_Comm_rank failed with error code %d\n", mpi_code_g);
    nerrors++;
    goto exit;
  ...
  if (MPI_SUCCESS != (mpi_code_g = MPI_Allreduce(MPI_IN_PLACE, &num_nodes_g, 1,
                                                 MPI_INT, MPI_SUM, comm_g))) {
    if (MAINPROCESS)
   ...
   */
  if (MPI_SUCCESS != (mpi_code_g = MPI_Comm_split(comm_g, node_local_rank,
                                                  mpi_rank, &ioc_comm))) {
    if (MAINPROCESS)
  ...
  if (mpi_size > 1) {
    if (MPI_SUCCESS !=
        (mpi_code_g = MPI_Bcast(&seed, 1, MPI_UNSIGNED, 0, comm_g))) {
      if (MAINPROCESS)
  ...
  if (!config_dir) {
    if (MAINPROCESS)
      printf("couldn't allocate space for subfiling config file directory "
             "buffer\n");
    nerrors++;
    goto exit;
  }
   ...
   */
  num_iocs_g =
      (ioc_per_node_g > 0) ? (int)ioc_per_node_g * num_nodes_g : num_nodes_g;
  ...
  if (!(env_value = HDgetenv(H5FD_SUBFILING_CONFIG_FILE_PREFIX))) {
    ...
    if (rand_value == 0) {
      ...
      if (MAINPROCESS) {
        if ((HDmkdir(SUBFILING_CONFIG_FILE_DIR, (mode_t)0755) < 0) &&
            (errno != EEXIST)) {
      ...
      if (HDsetenv(H5FD_SUBFILING_CONFIG_FILE_PREFIX, SUBFILING_CONFIG_FILE_DIR,
                   1) < 0) {
        if (MAINPROCESS)
          printf("HDsetenv failed\n");
        nerrors++;
