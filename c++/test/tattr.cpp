...
static void test_attr_basic_write(FileAccPropList &fapl) {
  ...
  try {
    ...
    // Create a dataset
    DataSet dataset =
        fid1.createDataSet(DSET1_NAME, PredType::NATIVE_UCHAR, ds_space);

    // Create dataspace for attribute
    DataSpace att_space(ATTR1_RANK, dims2);

    // Create a file attribute
    Attribute file_attr2 =
        fid1.createAttribute(FATTR1_NAME, PredType::NATIVE_INT, att_space);
    ...
    Attribute ds_attr1 =
        dataset.createAttribute(ATTR1_NAME, PredType::NATIVE_INT, att_space);
    ...
    Attribute ds_attr2 =
        dataset.createAttribute(ATTR2_NAME, PredType::NATIVE_INT, att_space);
    Attribute ds_attr3 =
        dataset.createAttribute(ATTR3_NAME, PredType::NATIVE_INT, att_space);
    ...
    Attribute gr_attr =
        group.createAttribute(ATTR2_NAME, PredType::NATIVE_INT, sid3);

    // Check storage size for attribute
    hsize_t attr_size = gr_attr.getStorageSize();
    verify_val(static_cast<long>(attr_size),
               static_cast<long>(ATTR2_DIM1 * ATTR2_DIM2 * sizeof(int)),
    ...
    issue_fail_msg("test_attr_basic_write()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
  ...
  try {
    ...
    if (!attr_exists)
      throw InvalidActionException("H5File::attrExists",
                                   "Attribute should exist but does not");
    ...
    verify_val(static_cast<size_t>(name_size), FATTR1_NAME.length(),
               "Attribute::getName", __LINE__, __FILE__);
    verify_val(const_cast<const char *>(fattr1_name), FATTR1_NAME,
               "Attribute::getName", __LINE__, __FILE__);
    delete[] fattr1_name;
    ...
    name_size = fattr1.getName(fattr1_name, buf_size + 1);
    CHECK(name_size, FAIL, "Attribute::getName", __LINE__, __FILE__);
    verify_val(static_cast<size_t>(name_size), FATTR1_NAME.size(),
               "Attribute::getName", __LINE__, __FILE__);
    verify_val(const_cast<const char *>(fattr1_name),
               const_cast<const char *>(short_name), "Attribute::getName",
               __LINE__, __FILE__);
    delete[] fattr1_name;
    ...
    name_size = fattr1.getName(fattr1_name, buf_size + 1);
    CHECK(name_size, FAIL, "Attribute::getName", __LINE__, __FILE__);
    verify_val(fattr1_name, FATTR1_NAME, "Attribute::getName", __LINE__,
               __FILE__);
    delete[] fattr1_name;
    ...
    verify_val(fattr1_name3, FATTR1_NAME, "Attribute::getName", __LINE__,
               __FILE__);
    ...
    verify_val(dattr_name2, ATTR1_NAME, "Attribute::getName", __LINE__,
               __FILE__);
    ...
    issue_fail_msg("test_attr_getname()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
} // test_attr_getname()
...
static void test_attr_rename(FileAccPropList &fapl) {
  ...
  try {
    ...
    verify_val(fattr_name, FATTR_TMP_NAME, "Attribute::getName", __LINE__,
               __FILE__);
    ...
    verify_val(attr_name, ATTR_TMP_NAME, "Attribute::getName", __LINE__,
               __FILE__);
    ...
    verify_val(attr2_name, ATTR2_NAME, "Attribute::getName", __LINE__,
               __FILE__);
...
static void test_attr_basic_read(FileAccPropList &fapl) {
  ...
  try {
    ...
    verify_val(static_cast<long>(oinfo.num_attrs), 3, "DataSet::getObjinfo",
               __LINE__, __FILE__);
    ...
    verify_val(static_cast<long>(oinfo.num_attrs), 1, "Group::getObjinfo",
               __LINE__, __FILE__);
    ...
    for (i = 0; i < ATTR2_DIM1; i++)
      for (j = 0; j < ATTR2_DIM2; j++)
        if (attr_data2[i][j] != read_data2[i][j]) {
          TestErrPrintf("%d: attribute data different: attr_data2[%" PRIuHSIZE
                        "][%" PRIuHSIZE "]=%d, "
                        "read_data2[%" PRIuHSIZE "][%" PRIuHSIZE "]=%d\n",
                        __LINE__, i, j, attr_data2[i][j], i, j,
                        read_data2[i][j]);
        }
    PASSED();
  } // end try block
    ...
    issue_fail_msg("test_attr_basic_read()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_attr_compound_write(FileAccPropList &fapl) {
  ...
  try {
    ...
    // Create a dataset
    DataSet dataset =
        fid1.createDataSet(DSET1_NAME, PredType::NATIVE_UCHAR, sid1);

    // Create the attribute datatype.
    CompType comp_type(sizeof(struct attr4_struct));

    attr4_field1_off = HOFFSET(struct attr4_struct, i);
    comp_type.insertMember(ATTR4_FIELDNAME1, attr4_field1_off,
                           PredType::NATIVE_INT);

    attr4_field2_off = HOFFSET(struct attr4_struct, d);
    comp_type.insertMember(ATTR4_FIELDNAME2, attr4_field2_off,
                           PredType::NATIVE_DOUBLE);

    attr4_field3_off = HOFFSET(struct attr4_struct, c);
    comp_type.insertMember(ATTR4_FIELDNAME3, attr4_field3_off,
    ...
    try {
      Attribute invalid_attr =
          dataset.createAttribute(ATTR4_NAME, comp_type, sid2);
    } catch (AttributeIException &E) // catching invalid creating attribute
    {
    } // do nothing, exception expected
    ...
    issue_fail_msg("test_attr_compound_write()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_attr_compound_read(FileAccPropList &fapl) {
  ...
  try {
    ...
    memset(&oinfo, 0, sizeof(oinfo));
    dataset.getObjinfo(oinfo, H5O_INFO_NUM_ATTRS);
    verify_val(static_cast<long>(oinfo.num_attrs), 1, "DataSet::getObjinfo",
               __LINE__, __FILE__);
    ...
    verify_val(rank, ATTR4_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);

    // Get the dims of the dataspace and verify them
    int ndims = space.getSimpleExtentDims(dims);
    verify_val(ndims, ATTR4_RANK, "DataSpace::getSimpleExtentDims", __LINE__,
               __FILE__);
    ...
    verify_val(static_cast<long>(type_class), static_cast<long>(H5T_COMPOUND),
               "Attribute::getTypeClass", __LINE__, __FILE__);
    ...
    for (int j = 0; j < fields; j++) {
      ...
      if (!((fieldname == ATTR4_FIELDNAME1) ||
            (fieldname == ATTR4_FIELDNAME2) || (fieldname == ATTR4_FIELDNAME3)))
        TestErrPrintf("%d:invalid field name for field #%d: %s\n", __LINE__, j,
                      fieldname.c_str());
    }

    offset = datatype.getMemberOffset(0);
    verify_val(offset, attr4_field1_off, "DataType::getMemberOffset", __LINE__,
               __FILE__);

    offset = datatype.getMemberOffset(1);
    verify_val(offset, attr4_field2_off, "DataType::getMemberOffset", __LINE__,
               __FILE__);

    offset = datatype.getMemberOffset(2);
    verify_val(offset, attr4_field3_off, "DataType::getMemberOffset", __LINE__,
               __FILE__);
    ...
    verify_val(static_cast<long>(type_class), static_cast<long>(H5T_INTEGER),
               "DataType::getMemberClass", __LINE__, __FILE__);
    ...
    verify_val(static_cast<long>(order),
               static_cast<long>(PredType::NATIVE_INT.getOrder()),
               "DataType::getOrder", __LINE__, __FILE__);

    // Get and verify the size of this member's type
    size = i_type.getSize();
    verify_val(size, PredType::NATIVE_INT.getSize(), "DataType::getSize",
               __LINE__, __FILE__);

    // Get and verify class, order, and size of the second member's type
    type_class = datatype.getMemberClass(1);
    verify_val(static_cast<long>(type_class), static_cast<long>(H5T_FLOAT),
               "DataType::getMemberClass", __LINE__, __FILE__);
    FloatType f_type = datatype.getMemberFloatType(1);
    order = f_type.getOrder();
    verify_val(static_cast<long>(order),
               static_cast<long>(PredType::NATIVE_DOUBLE.getOrder()),
               "DataType::getOrder", __LINE__, __FILE__);
    size = f_type.getSize();
    verify_val(size, PredType::NATIVE_DOUBLE.getSize(), "DataType::getSize",
               __LINE__, __FILE__);
    ...
    verify_val(static_cast<long>(order),
               static_cast<long>(PredType::NATIVE_SCHAR.getOrder()),
               "DataType::getOrder", __LINE__, __FILE__);
    size = s_type.getSize();
    verify_val(size, PredType::NATIVE_SCHAR.getSize(), "DataType::getSize",
               __LINE__, __FILE__);
    ...
    for (ii = 0; ii < ATTR4_DIM1; ii++)
      for (jj = 0; jj < ATTR4_DIM2; jj++)
        if (memcmp(&attr_data4[ii][jj], &read_data4[ii][jj],
                   sizeof(struct attr4_struct)) != 0) {
          TestErrPrintf("%d:attribute data different: attr_data4[%" PRIuHSIZE
                        "][%" PRIuHSIZE "].i=%d, "
                        "read_data4[%" PRIuHSIZE "][%" PRIuHSIZE "].i=%d\n",
                        __LINE__, ii, jj, attr_data4[ii][jj].i, ii, jj,
                        read_data4[ii][jj].i);
          TestErrPrintf("%d:attribute data different: attr_data4[%" PRIuHSIZE
                        "][%" PRIuHSIZE "].d=%f, "
                        "read_data4[%" PRIuHSIZE "][%" PRIuHSIZE "].d=%f\n",
                        __LINE__, ii, jj, attr_data4[ii][jj].d, ii, jj,
                        read_data4[ii][jj].d);
          TestErrPrintf("%d:attribute data different: attr_data4[%" PRIuHSIZE
                        "][%" PRIuHSIZE "].c=%c, "
                        "read_data4[%" PRIuHSIZE "][%" PRIuHSIZE "].c=%c\n",
                        __LINE__, ii, jj, attr_data4[ii][jj].c, ii, jj,
                        read_data4[ii][jj].c);
        } /* end if */
  ...
  try {
    ...
    issue_fail_msg("test_attr_compound_read()", __LINE__, __FILE__,
                   "Unable to truncate file, possibly because some objects are "
                   "left opened");
  }
...
static void test_attr_scalar_write(FileAccPropList &fapl) {
  ...
  try {
    ...
    // Create an attribute for the dataset
    Attribute ds_attr =
        dataset.createAttribute(ATTR5_NAME, PredType::NATIVE_FLOAT, att_space);
    ...
    // invalid action exception.
    try {
      Attribute invalid_attr = dataset.createAttribute(
          ATTR5_NAME, PredType::NATIVE_FLOAT, att_space);
    ...
    issue_fail_msg("test_attr_scalar_write()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
  ...
  try {
    ...
    if (abs(read_data2 - attr_data5) > FLT_EPSILON)
      TestErrPrintf("%d: attribute data different: read_data2=%f, "
                    "attr_data5=%f\n",
                    __LINE__, static_cast<double>(read_data2),
    ...
    issue_fail_msg("test_attr_scalar_read()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_attr_mult_write(FileAccPropList &fapl) {
  ...
  try {
    ...
    Attribute ds_attr2 =
        dataset.createAttribute(ATTR2_NAME, PredType::NATIVE_INT, att2_space);
    ...
    Attribute ds_attr3 = dataset.createAttribute(
        ATTR3_NAME, PredType::NATIVE_DOUBLE, att3_space);
    ...
    issue_fail_msg("test_attr_mult_write()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_attr_mult_read(FileAccPropList &fapl) {
  ...
  try {
    ...
    // Get the rank of the dataspace and verify it
    int rank = space.getSimpleExtentNdims();
    verify_val(rank, ATTR1_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);
    ...
    if (dims[0] != ATTR1_DIM1)
      TestErrPrintf(
          "%d:attribute dimensions different: dims[0]=%d, should be %" PRIuHSIZE
          "\n",
    ...
    verify_val(rank, ATTR2_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);
    ...
    // Get the rank of the dataspace and verify it
    rank = space.getSimpleExtentNdims();
    verify_val(rank, ATTR3_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);

    // Get the dims of the dataspace and verify them
    (void)space.getSimpleExtentDims(dims);
    verify_val(static_cast<long>(dims[0]), static_cast<long>(ATTR3_DIM1),
               "attribute dimensions", __FILE__, __LINE__);
      ...
      for (j = 0; j < ATTR3_DIM2; j++)
        for (k = 0; k < ATTR3_DIM3; k++)
          if (abs(attr_data3[i][j][k] - read_data3[i][j][k]) > DBL_EPSILON)
            TestErrPrintf("%d: attribute data different: attr_data3[%" PRIuHSIZE
                          "][%" PRIuHSIZE "][%" PRIuHSIZE "]=%f, "
                          "read_data3[%" PRIuHSIZE "][%" PRIuHSIZE
                          "][%" PRIuHSIZE "]=%f\n",
    ...
    issue_fail_msg("test_attr_mult_read()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
} // test_attr_mult_read()
...
static void test_attr_delete(FileAccPropList &fapl) {
  ...
  try {
    ...
    verify_val(attr_name, FATTR1_NAME, "Attribute::getName", __LINE__,
               __FILE__);
    fattr.close();
    ...
    try {
      ...
      // continuation here, that means no exception has been thrown
      throw InvalidActionException(
          "DataSet::removeAttr", "Attempting to remove non-existing attribute");
    ...
    try {
      ...
      // continuation here, that means no exception has been thrown
      throw InvalidActionException(
          "DataSet::removeAttr", "Attempting to delete non-existing attribute");
    } catch (AttributeIException &E) // catching invalid removing attribute
    {
    } // do nothing, exception expected
...
static void test_attr_dtype_shared(FileAccPropList &fapl) {
  ...
  try {
    ...
    { // First enclosed block
      ...
      verify_val(static_cast<int>(statbuf.nlink), 1, "DataType::getObjinfo",
                 __LINE__, __FILE__);
#endif
      ...
      // Check reference count on named datatype
      fid1.getObjinfo(TYPE1_NAME, statbuf);
      verify_val(static_cast<int>(statbuf.nlink), 2, "H5File::getObjinfo",
                 __LINE__, __FILE__);
#endif
      ...
      // Check reference count on named datatype
      fid1.getObjinfo(TYPE1_NAME, statbuf);
      verify_val(static_cast<int>(statbuf.nlink), 3, "DataSet::getObjinfo",
                 __LINE__, __FILE__);
#endif
      ...
      // Check reference count on named datatype
      fid1.getObjinfo(TYPE1_NAME, statbuf);
      verify_val(static_cast<int>(statbuf.nlink), 2,
                 "DataSet::getObjinfo after DataSet::removeAttr", __LINE__,
                 __FILE__);
#endif
      ...
      // Check reference count on named datatype
      fid1.getObjinfo(TYPE1_NAME, statbuf);
      verify_val(static_cast<int>(statbuf.nlink), 3, "DataSet::createAttribute",
                 __LINE__, __FILE__);
#endif
    ...
    { // Second enclosed block...
      ...
      // Check reference count on named datatype
      fid1.getObjinfo(TYPE1_NAME, statbuf);
      verify_val(static_cast<int>(statbuf.nlink), 3, "DataSet::openAttribute",
                 __LINE__, __FILE__);
#endif
    ...
    // Check reference count on named datatype
    fid1.getObjinfo(TYPE1_NAME, statbuf);
    verify_val(static_cast<int>(statbuf.nlink), 1, "H5File::unlink", __LINE__,
               __FILE__);
#endif
    ...
    verify_val(static_cast<long>(filesize), static_cast<long>(empty_filesize),
               "Checking file size", __LINE__, __FILE__);

    PASSED();
  } // end try block

  catch (Exception &E) {
    issue_fail_msg("test_attr_dtype_shared()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_string_attr(FileAccPropList &fapl) {
  ...
  try {
    ...
    Attribute gr_flattr1 =
        root.createAttribute(ATTR1_FL_STR_NAME, fls_type, att_space);
    ...
    // Create an attribute for the root group.
    Attribute gr_flattr2 =
        root.createAttribute(ATTR2_FL_STR_NAME, fls_type, att_space);
    ...
    if (read_flstr1 != ATTRSTR_DATA)
      TestErrPrintf(
          "Line %d: Attribute data different: ATTRSTR_DATA=%s,read_flstr1=%s\n",
    ...
    if (read_flstr2 != ATTRSTR_DATA)
      TestErrPrintf(
          "Line %d: Attribute data different: ATTRSTR_DATA=%s,read_flstr2=%s\n",
    ...
    // Create an attribute for the root group.
    Attribute gr_vlattr =
        root.createAttribute(ATTR_VL_STR_NAME, vls_type, att_space);
    ...
    if (read_str != ATTRSTR_DATA)
      TestErrPrintf(
          "Line %d: Attribute data different: ATTRSTR_DATA=%s,read_str=%s\n",
...
static void test_attr_dense_create(FileCreatPropList &fcpl,
  ...
  try {
    ...
    // Create a dataset
    DataSet dataset =
        fid1.createDataSet(DSET1_NAME, PredType::NATIVE_UCHAR, ds_space, dcpl);

    unsigned max_compact = 0, min_dense = 0;

    // Retrieve limits for compact/dense attribute storage
    dcpl.getAttrPhaseChange(max_compact, min_dense);
    verify_val(max_compact, MAX_COMPACT_DEF,
               "DSetCreatPropList::getAttrPhaseChange", __LINE__, __FILE__);
    verify_val(min_dense, MIN_DENSE_DEF,
               "DSetCreatPropList::getAttrPhaseChange", __LINE__, __FILE__);
    ...
    dcpl.getAttrPhaseChange(max_compact, min_dense);
    verify_val(max_compact, static_cast<unsigned>(7),
               "DSetCreatPropList::getAttrPhaseChange", __LINE__, __FILE__);
    verify_val(min_dense, static_cast<unsigned>(5),
               "DSetCreatPropList::getAttrPhaseChange", __LINE__, __FILE__);
    ...
    for (attr_num = 0; attr_num < max_compact; attr_num++) {
      ...
      Attribute attr =
          dataset.createAttribute(attr_name, PredType::NATIVE_UINT, ds_space);
    ...
    issue_fail_msg("test_attr_dense_create()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_attr_corder_create_basic(FileCreatPropList &fcpl,
  ...
  try {
    ...
    verify_val(static_cast<long>(crt_order_flags), 0,
    ...
    try {
      ...
      // continuation here, that means no exception has been thrown
      throw InvalidActionException(
          "DSetCreatPropList::getAttrCrtOrder",
    ...
    verify_val(
        crt_order_flags,
        static_cast<unsigned>(H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED),
        "DSetCreatPropList::getAttrCrtOrder", __LINE__, __FILE__);
    ...
    issue_fail_msg("test_attr_corder_create_basic()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
extern "C" void test_attr() {
  ...
  try {
    ...
    for (new_format = FALSE; new_format <= TRUE; new_format++) {
      ...
      test_attr_compound_write(
          curr_fapl); // Test complex datatype H5A writing code
      test_attr_compound_read(
          curr_fapl); // Test complex datatype H5A reading code

      test_attr_scalar_write(
          curr_fapl); // Test scalar dataspace H5A writing code
      test_attr_scalar_read(
          curr_fapl); // Test scalar dataspace H5A reading code
      ...
      test_attr_dtype_shared(
          curr_fapl); // Test using shared datatypes in attributes
