...
static void test_array_compound_array() {
  ...
  for (idxi = 0; idxi < SPACE1_DIM1; idxi++)
    for (idxj = 0; idxj < ARRAY1_DIM1; idxj++) {
      ...
      for (idxk = 0; idxk < ARRAY1_DIM1; idxk++) {
        float temp = static_cast<float>(idxi) * 10.0F +
                     static_cast<float>(idxj) * 2.5F + static_cast<float>(idxk);
  ...
  try {
    ...
    verify_val(type_name, ARRAYTYPE_NAME,
               "DataType::getObjName tests constructor", __LINE__, __FILE__);
    named_type.close();
    ...
    verify_val(mclass == H5T_ARRAY, true, "f2_type.getClass", __LINE__,
               __FILE__);
    ...
    verify_val(ndims, ARRAY1_RANK, "atype_check.getArrayNDims", __LINE__,
               __FILE__);
    ...
    verify_val(ndims, ARRAY1_RANK, "new_arrtype.getArrayNDims", __LINE__,
               __FILE__);
    ...
    verify_val(mclass == H5T_COMPOUND, true, "atype_check.getClass", __LINE__,
               __FILE__);
    ...
    verify_val(ndims, ARRAY1_RANK, "f2_atype_check.getArrayNDims", __LINE__,
               __FILE__);
    ...
    for (idxi = 0; idxi < SPACE1_DIM1; idxi++) {
      for (idxj = 0; idxj < ARRAY1_DIM1; idxj++) {
        if (wdata[idxi][idxj].i != rdata[idxi][idxj].i) {
          TestErrPrintf(
              "Array data information doesn't match!, wdata[%" PRIuHSIZE
              "][%" PRIuHSIZE "].i=%d, "
    ...
    issue_fail_msg("test_array_compound_array", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_array_assignment() {
  ...
  try {
    ...
    issue_fail_msg("test_array_assignment", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
 ...
 *-------------------------------------------------------------------------
 */
extern "C" void cleanup_array() { HDremove(FILENAME.c_str()); } // cleanup_array
