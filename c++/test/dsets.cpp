...
static herr_t test_create(H5File &file) {
  ...
  try {
    ...
    dataset = new DataSet(
        file.createDataSet(DSET_DEFAULT_NAME, PredType::NATIVE_DOUBLE, space));
    ...
    try {
      dataset = new DataSet(file.createDataSet(DSET_DEFAULT_NAME,
    ...
    verify_val(ds_name, DSET_DEFAULT_NAME_PATH, "DataSet::getObjName", __LINE__,
               __FILE__);

    // Get and verify the comment from this dataset, using
    // H5std_string getComment(const H5std_string& name, <buf_size=0, by
    // default>)
    H5std_string comment = file.getComment(DSET_DEFAULT_NAME);
    verify_val(comment, "This is a dataset", "DataSet::getComment", __LINE__,
               __FILE__);
    ...
    try {
      ...
      // continuation here, that means no exception has been thrown
      throw InvalidActionException("H5File::openDataSet",
                                   "Attempted to open a non-existent dataset");
...
static herr_t test_simple_io(H5File &file) {
  ...
  try {
    ...
    // Create the dataset
    DataSet dataset(
        file.createDataSet(DSET_SIMPLE_IO_NAME, PredType::NATIVE_INT, space));
    ...
    for (i = 0; i < 100; i++)
      for (j = 0; j < 200; j++) {
        int status =
            check_values(static_cast<hsize_t>(i), static_cast<hsize_t>(j),
                         points[i][j], check[i][j]);
        if (status == -1)
...
static herr_t test_datasize(FileAccPropList &fapl) {
  ...
  try {
    ...
    if (n_dims < 0) {
      throw Exception("test_compression",
                      "DataSpace::getSimpleExtentDims() failed");
    }
    ...
    if (ds_size != expected_size) {
      ...
      throw Exception("test_compression",
                      "Failed in testing DataSet::getInMemDataSize()");
    }
...
static herr_t test_tconv(H5File &file) {
  ...
  try {
    ...
    // Create the data set
    DataSet dataset(
        file.createDataSet(DSET_TCONV_NAME, PredType::STD_I32LE, space));
    ...
    for (int i = 0; i < 1000000; i++) {
      if (in[4 * i + 0] != out[4 * i + 3] || in[4 * i + 1] != out[4 * i + 2] ||
        ...
        throw Exception("DataSet::read",
                        "Read with byte order conversion failed");
      }
    }
...
static herr_t test_compression(H5File &file) {
  ...
  try {
    ...
    dataset->write(static_cast<void *>(points), PredType::NATIVE_INT, space1,
                   space1, xfer);
    dataset->read(static_cast<void *>(check), PredType::NATIVE_INT, space1,
                  space1, xfer);
    ...
    for (i = 0; i < hs_size[0]; i++) {
      for (j = 0; j < hs_size[1]; j++) {
        if (points[hs_offset[0] + i][hs_offset[1] + j] !=
          ...
          cerr << "    At index "
               << static_cast<unsigned long>((hs_offset[0] + i)) << ","
               << static_cast<unsigned long>((hs_offset[1] + j)) << endl;

          cerr << "    At original: "
               << static_cast<int>(points[hs_offset[0] + i][hs_offset[1] + j])
               << endl;
          cerr << "    At returned: "
               << static_cast<int>(check[hs_offset[0] + i][hs_offset[1] + j])
               << endl;
    ...
    dataset = new DataSet(file.createDataSet(
        DSET_BOGUS_NAME, PredType::NATIVE_INT, space2, dscreatplist));
...
static herr_t test_multiopen(H5File &file) {
  ...
  try {
    ...
    DataSet dset1 =
        file.createDataSet("multiopen", PredType::NATIVE_INT, *space, dcpl);
  ...
  try {
    ...
    try { // block of bitfield_1
      ...
      for (i = 0; i < sizeof buf; i++)
        buf[i] =
            static_cast<unsigned char>(0xff) ^ static_cast<unsigned char>(i);
    ...
    // catch exceptions thrown in try block of bitfield_1
      ...
      cerr << " FAILED" << endl;
      cerr << "    <<<  "
           << "bitfield_1: " << E.getFuncName() << " - " << E.getDetailMsg()
           << "  >>>" << endl
           << endl;
      delete dset;
      return -1;
    ...
    // catch exceptions thrown in try block of bitfield_2
      ...
      cerr << " FAILED" << endl;
      cerr << "    <<<  "
           << "bitfield_2: " << E.getFuncName() << " - " << E.getDetailMsg()
           << "  >>>" << endl
           << endl;
      delete dset;
    ...
    // catch exceptions thrown in try block of opaque_1
      ...
      cerr << " FAILED" << endl;
      cerr << "    <<<  "
           << "opaque_1: " << E.getFuncName() << " - " << E.getDetailMsg()
           << "  >>>" << endl
           << endl;
      delete dset;
      delete optype;
    ...
    // catch exceptions thrown in try block of opaque_2
      ...
      cerr << " FAILED" << endl;
      cerr << "    <<<  "
           << "opaque_2: " << E.getFuncName() << " - " << E.getDetailMsg()
           << "  >>>" << endl
           << endl;
      delete dset;
      delete optype;
...
static herr_t test_getnativeinfo(H5File &file) {
  ...
  try {
    ...
    verify_val(static_cast<long>(ninfo.hdr.nchunks), 1,
               "DataSet::getNativeObjinfo", __LINE__, __FILE__);
    dataset.close();
...
static herr_t test_chunk_cache(const FileAccPropList &fapl) {
  ...
  try {
    ...
    verify_val(nslots_1, nslots_4, "DSetAccPropList::getChunkCache", __LINE__,
               __FILE__);
    verify_val(nbytes_1, nbytes_4, "DSetAccPropList::getChunkCache", __LINE__,
               __FILE__);
    ...
    verify_val(static_cast<long>(nlinks), 134, "DSetAccPropList::getNumLinks",
               __LINE__, __FILE__);
    ...
    H5File file(FILE_ACCPLIST, H5F_ACC_TRUNC, FileCreatPropList::DEFAULT,
                fapl_local);
    ...
    DataSet dataset(file.createDataSet(DSET_CHUNKED_NAME, PredType::NATIVE_INT,
                                       mspace, dcpl, dapl));
    ...
    verify_val(nslots_2, nslots_4, "DSetCreatPropList::getChunkCache", __LINE__,
               __FILE__);
    verify_val(nbytes_2, nbytes_4, "DSetCreatPropList::getChunkCache", __LINE__,
               __FILE__);
    verify_val(H5_DBL_ABS_EQUAL(w0_2, w0_4), 1,
               "DSetCreatPropList::getChunkCache", __LINE__, __FILE__);
    ...
    verify_val(nslots_3, nslots_4, "DSetCreatPropList::getLayout", __LINE__,
               __FILE__);
    verify_val(nbytes_3, nbytes_4, "DSetCreatPropList::getLayout", __LINE__,
               __FILE__);
    verify_val(H5_DBL_ABS_EQUAL(w0_3, w0_4), 1, "DSetCreatPropList::getLayout",
               __LINE__, __FILE__);
...
static herr_t test_virtual() {
  ...
  try {
    ...
    verify_val(static_cast<long>(layout), static_cast<long>(H5D_VIRTUAL),
               "DSetCreatPropList::getLayout", __LINE__, __FILE__);
...
static herr_t test_operator(H5File &file) {
  ...
  try {
    ...
    // We're not sure what they are, so we won't check.
    DataSet dataset =
        file.createDataSet(DSET_OPERATOR, PredType::NATIVE_DOUBLE, space);
