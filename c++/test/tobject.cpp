...
const H5std_string
    DSET_IN_GRP1_2_PATH("/Top Group/Sub-Group 1.2/Dataset_in_Group_1.2");
...
static void test_get_objname() {
  ...
  try {
    ...
    if (name_len > 4) {
      ...
      verify_val(const_cast<const char *>(grp1_name), "/Top",
                 "Group::getObjName", __LINE__, __FILE__);
      delete[] grp1_name;
    }
    ...
    DataSet dsinfile =
        file.createDataSet(DSET_IN_FILE, PredType::NATIVE_DOUBLE, space);

    // Create a dataset in the group
    DataSet dsingrp =
        grp1.createDataSet(DSET_IN_GRP1, PredType::NATIVE_INT, space);
    ...
    verify_val(ds_name, DSET_IN_FILE_PATH, "DataSet::getObjName", __LINE__,
               __FILE__);

    name_len = dsingrp.getObjName(ds_name); // default len
    verify_val(ds_name, DSET_IN_GRP1_PATH, "DataSet::getObjName", __LINE__,
               __FILE__);
    ...
    verify_val(ds_name, DSET_IN_GRP1_2_PATH, "DataSet::getObjName", __LINE__,
               __FILE__);
...
static void test_existance() {
  ...
  try {
    ...
    verify_val(exists, TRUE, "Group::nameExists DSET_IN_GRP1", __LINE__,
               __FILE__);
    // Deprecated
    ...
    verify_val(exists, TRUE, "Group::nameExists DSET_IN_GRP1_2", __LINE__,
               __FILE__);
    // Deprecated
    exists = grp1_2.exists(DSET_IN_GRP1_2);
    verify_val(exists, TRUE, "Group::exists DSET_IN_GRP1_2", __LINE__,
               __FILE__);
    ...
    verify_val(exists, TRUE,
               "Group::nameExists given dataset with full path name", __LINE__,
               __FILE__);

    exists = grp1_2.nameExists(DSET_IN_GRP1);
    verify_val(exists, FALSE, "Group::nameExists DSET_IN_GRP1", __LINE__,
               __FILE__);
    // Deprecated
    exists = dset1.exists("/Top Group/Dataset_in_Group_1");
    verify_val(exists, TRUE, "Group::exists given dataset with full path name",
               __LINE__, __FILE__);
...
static void test_get_objname_ontypes() {
  ...
  try {
    ...
    verify_val(inttype_name, "/INT type of STD_B8LE", "DataType::getObjName",
               __LINE__, __FILE__);
    ...
    verify_val(std_b8le_name, "/INT type of STD_B8LE", "DataType::getObjName",
               __LINE__, __FILE__);
    ...
    verify_val(type_name, "/STD_B8LE", "DataType::getObjName", __LINE__,
               __FILE__);
    ...
    verify_val(type_name, "/copy of STD_B8LE", "DataType::getObjName", __LINE__,
               __FILE__);
    ...
    verify_val(name_len,
               static_cast<ssize_t>(HDstrlen("/typetests/IntType NATIVE_INT")),
               "DataType::getObjName", __LINE__, __FILE__);
    verify_val(type_name, "/typetests/IntType NATIVE_INT",
               "DataType::getObjName", __LINE__, __FILE__);
...
static void test_get_objtype() {
  ...
  try {
    ...
    verify_val(static_cast<long>(objtype), static_cast<long>(H5O_TYPE_DATASET),
               "DataSet::childObjType", __LINE__, __FILE__);

    // Get and verify object type with
    // H5O_type_t childObjType(const char* objname)
    objtype = grp1.childObjType(GROUP1_1.c_str());
    verify_val(static_cast<long>(objtype), static_cast<long>(H5O_TYPE_GROUP),
               "DataSet::childObjType", __LINE__, __FILE__);
    ...
    verify_val(static_cast<long>(objtype),
...
static void test_open_object_header() {
  ...
  try {
    ...
    verify_val(static_cast<long>(id_type), static_cast<long>(H5I_GROUP),
               "H5Iget_type for group ID", __LINE__, __FILE__);
    id_type = IdComponent::getHDFObjType(obj_dtype);
    verify_val(static_cast<long>(id_type), static_cast<long>(H5I_DATATYPE),
               "H5Iget_type for datatype ID", __LINE__, __FILE__);
    id_type = IdComponent::getHDFObjType(obj_dset);
    verify_val(static_cast<long>(id_type), static_cast<long>(H5I_DATASET),
               "H5Iget_type for dataset ID", __LINE__, __FILE__);

    /* Do something more complex with each of the IDs to make sure */

    Group grp2(obj_grp);
    hsize_t num_objs = grp2.getNumObjs();
    verify_val(static_cast<long>(num_objs), 1, "H5Gget_info", __LINE__,
               __FILE__);
    ...
    verify_val(static_cast<long>(type_class), static_cast<long>(H5T_INTEGER),
               "H5Tget_class", __LINE__, __FILE__);
    dtype.close();
...
static void test_getobjectinfo_same_file() {
  ...
  try {
    ...
    verify_val(oinfo1.fileno, oinfo2.fileno, "file number from getObjinfo",
               __LINE__, __FILE__);
    ...
    verify_val(oinfo1.fileno, oinfo2.fileno, "file number from getObjectInfo",
               __LINE__, __FILE__);
  ...
  // catch all other exceptions
    ...
    cerr << " in Exception " << E.getCFuncName()
         << "detail: " << E.getCDetailMsg() << endl;
    issue_fail_msg("test_getobjectinfo_same_file()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_intermediate_groups() {
  ...
  try {
    ...
    verify_val(crt_int_grps, true,
               "LinkCreatPropList::getCreateIntermediateGroup", __LINE__,
               __FILE__);
  ...
  // catch all other exceptions
    ...
    cerr << " in Exception " << E.getCFuncName()
         << "detail: " << E.getCDetailMsg() << endl;
    issue_fail_msg("test_intermediate_groups()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
