...
static void test_vlstring_dataset() {
  ...
  try {
    ...
    if (HDstrcmp(string_ds_check, DSET1_DATA.c_str()) != 0)
      TestErrPrintf("Line %d: Attribute data different: "
                    "DSET1_DATA=%s,string_ds_check=%s\n",
    ...
    if (read_str != DSET1_DATA)
      TestErrPrintf(
          "Line %d: Attribute data different: DSET1_DATA=%s,read_str=%s\n",
    ...
    if (HDstrcmp(string_ds_check, dynstring_ds_write) != 0)
      TestErrPrintf("VL string datasets don't match!, dynstring_ds_write=%s, "
                    "string_ds_check=%s\n",
  ...
  // Catch all exceptions.
    ...
    issue_fail_msg("test_vlstring_dataset()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_vlstring_array_dataset() {
  ...
  // Catch all exceptions.
    ...
    issue_fail_msg("test_vlstring_array_dataset()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_vlstrings_special() {
  ...
  try {
    ...
    for (ii = 0; ii < SPACE1_DIM1; ii++) {
      ...
      if (wlen != rlen) {
        TestErrPrintf("VL data lengths don't match!, strlen(wdata[%d])=%u, "
                      "strlen(rdata[%d])=%u\n",
                      static_cast<int>(ii), static_cast<unsigned>(wlen),
                      static_cast<int>(ii), static_cast<unsigned>(rlen));
        continue;
      }
  ...
  // Catch all exceptions.
    ...
    issue_fail_msg("test_vlstrings_special()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_vlstring_type() {
  ...
  try {
    ...
    verify_val(static_cast<long>(pad), static_cast<long>(H5T_STR_NULLPAD),
               "StrType::getStrpad", __LINE__, __FILE__);
    ...
    verify_val(static_cast<long>(type_class), static_cast<long>(H5T_STRING),
               "DataType::getClass", __LINE__, __FILE__);
    bool is_variable_str = vlst.isVariableStr();
    verify_val(is_variable_str, true, "DataType::isVariableStr", __LINE__,
               __FILE__);

    // Check default character set and padding.
    H5T_cset_t cset = vlst.getCset();
    verify_val(static_cast<long>(cset), static_cast<long>(H5T_CSET_ASCII),
               "StrType::getCset", __LINE__, __FILE__);
    pad = vlst.getStrpad();
    verify_val(static_cast<long>(pad), static_cast<long>(H5T_STR_NULLPAD),
               "StrType::getStrpad", __LINE__, __FILE__);
  ...
  // Catch all exceptions.
    ...
    issue_fail_msg("test_vlstring_type()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_compact_vlstring() {
  ...
  try {
    ...
    for (i = 0; i < SPACE1_DIM1; i++) {
      if (HDstrlen(wdata[i]) != strlen(rdata[i])) {
        TestErrPrintf("VL data length don't match!, strlen(wdata[%d])=%d, "
                      "strlen(rdata[%d])=%d\n",
                      static_cast<int>(i), static_cast<int>(HDstrlen(wdata[i])),
                      static_cast<int>(i),
  ...
  // Catch all exceptions.
    ...
    issue_fail_msg("test_compact_vlstrings()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_vlstring_attribute() {
  ...
  try {
    ...
    if (HDstrcmp(string_att_check, string_att_write) != 0)
      TestErrPrintf("VL string attributes don't match!, string_att_write=%s, "
                    "string_att_check=%s\n",
  ...
  // Catch all exceptions.
    ...
    issue_fail_msg("test_vlstring_attribute()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_vlstring_array_attribute() {
  const char *string_att_array[SPACE1_DIM1] = {
      "Line 1", "Line 2", "Line 3", "Line 4"}; // Information to write
  ...
  // Catch all exceptions.
    ...
    issue_fail_msg("test_vlstring_array_attribute()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
 ...
 *-------------------------------------------------------------------------
 */
extern "C" void cleanup_vlstrings() {
