...
static void test_h5s_basic() {
  ...
  try {
    ...
    verify_val(rank, SPACE1_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);
    ...
    verify_val(ndims, SPACE1_RANK, "DataSpace::getSimpleExtentDims", __LINE__,
               __FILE__);
    ...
    verify_val(static_cast<long>(n),
               SPACE2_DIM1 * SPACE2_DIM2 * SPACE2_DIM3 * SPACE2_DIM4,
               "DataSpace::getSimpleExtentNpoints", __LINE__, __FILE__);

    // Get the logical rank of dataspace sid2 and verify it
    rank = sid2.getSimpleExtentNdims();
    verify_val(rank, SPACE2_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);
    ...
    verify_val(memcmp(tdims, dims2, SPACE2_RANK * sizeof(unsigned)), 0,
               "DataSpace::getSimpleExtentDims", __LINE__, __FILE__);
    verify_val(memcmp(tmax, max2, SPACE2_RANK * sizeof(unsigned)), 0,
               "DataSpace::getSimpleExtentDims", __LINE__, __FILE__);
    ...
    try {
      ...
      // Should FAIL but didn't, so throw an invalid action exception
      throw InvalidActionException(
          "H5File::openDataSet", "Opening a dataset with higher dimensionality "
                                 "than what the library can handle");
...
static void test_h5s_scalar_write() {
  ...
  try {
    ...
    hssize_t n; // Number of dataspace elements
    n = sid1.getSimpleExtentNpoints();
    verify_val(static_cast<long>(n), 1, "DataSpace::getSimpleExtentNpoints",
               __LINE__, __FILE__);

    int rank; // Logical rank of dataspace
    rank = sid1.getSimpleExtentNdims();
    verify_val(rank, SPACE3_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);
    ...
    DataSet dataset =
        fid1.createDataSet("Dataset1", PredType::NATIVE_UINT, sid1);
    dataset.write(&space3_data, PredType::NATIVE_UINT);

    PASSED();
  } // end of try block
  catch (Exception &E) {
    issue_fail_msg("test_h5s_scalar_write()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_h5s_scalar_read() {
  ...
  try {
    ...
    verify_val(ndims, SPACE3_RANK, "DataSpace::getSimpleExtentNdims", __LINE__,
               __FILE__);
    ...
    issue_fail_msg("test_h5s_scalar_read()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_h5s_null() {
  ...
  try {
    ...
    hssize_t n; // Number of dataspace elements
    n = sid1.getSimpleExtentNpoints();
    verify_val(static_cast<long>(n), 0, "DataSpace::getSimpleExtentNpoints",
               __LINE__, __FILE__);
...
static void test_h5s_compound_scalar_write() {
  ...
  try {
    ...
    // all the exceptions caused by negative returned values by C APIs
    issue_fail_msg("test_h5s_compound_scalar_write()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
...
static void test_h5s_compound_scalar_read() {
  ...
  try {
    ...
    if (memcmp(&space4_data, &rdata, sizeof(struct space4_struct)) != 0) {
      ...
      cerr << "scalar data different: space4_data.u=" << space4_data.u
           << ", read_data4.u=" << rdata.u << endl;
      cerr << "scalar data different: space4_data.f=" << space4_data.f
           << ", read_data4.f=" << rdata.f << endl;
      TestErrPrintf(
    ...
    // all the exceptions caused by negative returned values by C APIs
    issue_fail_msg("test_h5s_compound_scalar_read()", __LINE__, __FILE__,
                   E.getCDetailMsg());
  }
