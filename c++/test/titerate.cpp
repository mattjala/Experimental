...
const H5std_string
    DSET_IN_GRP1_2_PATH("/Top Group/Sub-Group 1.2/Dataset in Group 1.2");
...
static void printelems(const Group &group, const H5std_string &dsname,
                       const H5std_string &atname);
 ...
 */
static int iter_strcmp(const void *s1, const void *s2) {
  return (HDstrcmp(*reinterpret_cast<const char *const *>(s1),
                   *reinterpret_cast<const char *const *>(s2)));
}
...
static void test_iter_group(FileAccPropList &fapl) {
  ...
  try {
    ...
    idx = 0;
    ret = H5Literate2(file.getId(), H5_INDEX_NAME, H5_ITER_INC, &idx, liter_cb,
                      &info);
    ...
    check_values(lnames[NDATASETS], "HDstrdup returns NULL", __LINE__,
                 __FILE__);
    ...
    verify_val(static_cast<long>(nobjs), NDATASETS + 2, "H5Gget_info", __LINE__,
               __FILE__);
    ...
    for (hsize_t i = 0; i < nobjs; i++) {
      ...
      // H5_ITER_INC, (hsize_t)i, dataset_name, (size_t)NAMELEN, H5P_DEFAULT);

      // oinfo = root_group.childObjType((hsize_t)i, H5_INDEX_NAME, H5_ITER_INC,
      // "."); ret = H5Oget_info_by_idx(root_group, ".", H5_INDEX_NAME,
      // H5_ITER_INC, (hsize_t)i, &oinfo, H5P_DEFAULT);
    }
    ...
    try {
      ...
      throw InvalidActionException("Group::getObjnameByIdx",
                                   "Attempt to iterate with invalid index");
    ...
    try {
      ...
      // Should FAIL but didn't, so throw an invalid action exception
      throw InvalidActionException("Group::getObjnameByIdx",
                                   "Attempt to iterate with negative index");
...
static void test_HDFFV_9920() {
  ...
  try {
    ...
    DataSet fds =
        file.createDataSet(FDATASET_NAME, PredType::STD_I32BE, dspace);
    DataSet gds = gr1.createDataSet(GDATASET_NAME, PredType::STD_I32BE, dspace);

    // Create a file attribute and a group attribute.
    Attribute fa1 =
        file.createAttribute(FATTR_NAME, PredType::STD_I32BE, dspace);
    Attribute ga1 =
        gr1.createAttribute(GATTR_NAME, PredType::STD_I32BE, dspace);
