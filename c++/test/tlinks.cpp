...
static const char *FILENAME[] = {"link0", "link1.h5", "link2.h5", "visit",
                                 NULL};
...
static void test_basic_links(hid_t fapl_id, hbool_t new_format) {
  ...
  try {
    ...
    // Create a hard link
    if (H5Lcreate_hard(file_id, "dset1", H5L_SAME_LOC, "grp1/hard1",
                       H5P_DEFAULT, H5P_DEFAULT) < 0)
      throw Exception("test_basic_links", "H5Lcreate_hard failed");

    // Create a symbolic link
    if (H5Lcreate_soft("/dset1", file_id, "grp1/soft", H5P_DEFAULT,
                       H5P_DEFAULT) < 0)
      throw Exception("test_basic_links", "H5Lcreate_soft failed");

    // Create a symbolic link to something that doesn't exist
    if (H5Lcreate_soft("foobar", file_id, "grp1/dangle", H5P_DEFAULT,
                       H5P_DEFAULT) < 0)
      throw Exception("test_basic_links", "H5Lcreate_soft failed");

    // Create a recursive symbolic link
    if (H5Lcreate_soft("/grp1/recursive", file_id, "/grp1/recursive",
                       H5P_DEFAULT, H5P_DEFAULT) < 0)
      throw Exception("test_basic_links", "H5Lcreate_soft failed");
    ...
    verify_val(softlink_val, "/dset1", "H5File::getLinkval grp1/soft", __LINE__,
               __FILE__);

    H5std_string dngllink_val = file.getLinkval("grp1/dangle");
    verify_val(dngllink_val, "foobar", "H5File::getLinkval grp1/dangle",
               __LINE__, __FILE__);

    H5std_string reclink_val = file.getLinkval("grp1/recursive");
    verify_val(reclink_val, "/grp1/recursive",
               "H5File::getLinkval grp1/recursive", __LINE__, __FILE__);
  ...
  try {
    ...
    if (file.nameExists("grp1/soft", LinkAccPropList::DEFAULT) != TRUE)
      throw InvalidActionException("H5File::nameExists",
                                   "grp1/soft doesn't exist");
    // Deprecated
...
static void test_lcpl(hid_t fapl_id, hbool_t new_format) {
  H5L_info2_t linfo;
  char filename[1024];
  hsize_t dims[2];
  ...
  try {
    ...
    if (linfo.cset != H5T_CSET_ASCII)
      throw InvalidActionException("H5Lget_info",
                                   "Character encoding is not default");
 ...
 *-------------------------------------------------------------------------
 */
static void test_move(hid_t fapl_id, hbool_t new_format) {
  char filename[1024];
  ...
  try {
    ...
    file_a.moveLink("/First_group/group_newer_name", grp_2,
                    "/Second_group/group_newest_name");
...
static void test_copy(hid_t fapl_id, hbool_t new_format) {
  ...
  try {
    ...
    if (file_b.exists("soft_new_name") != TRUE)
      throw InvalidActionException("H5File::exists",
                                   "soft_new_name doesn't exist");
    ...
    file_a.copyLink("/First_group/group_newer_name", grp_2,
                    "/Second_group/group_newest_name");
...
static void test_num_links(hid_t fapl_id, hbool_t new_format) {
  ...
  try {
    ...
    verify_val(read_nlinks, nlinks, "LinkAccPropList::setNumLinks", __LINE__,
               __FILE__);
...
// Operator function
static int visit_obj_cb(H5Object &obj, const H5std_string name,
...
static void test_visit(hid_t fapl_id, hbool_t new_format) {
  ...
  try {
    ...
    H5File *file =
        new H5File(filename, H5F_ACC_TRUNC, FileCreatPropList::DEFAULT, fapl);
    ...
    DataSet *dataset = new DataSet(file->createDataSet(
        DSET1_NAME, PredType::NATIVE_INT, *dataspace, ds_creatplist));
    ...
    dataset = new DataSet(
        file->createDataSet(DSET2_NAME, PredType::NATIVE_FLOAT, *dataspace));
    ...
    file->visit(H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata,
                H5O_INFO_BASIC);
    ...
    group->visit(H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata,
                 H5O_INFO_BASIC);
...
extern "C" void test_links() {
  ...
  try {
    ...
    /* Set the "use the latest version of the format" bounds for creating
       objects in the file */
    if (H5Pset_libver_bounds(fapl2_id, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) <
        0)
