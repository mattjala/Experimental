...
static void test_compound_1() {
  ...
  try {
    ...
    complex_type.insertMember("real", HOFFSET(complex_t, re),
                              PredType::NATIVE_DOUBLE);
    complex_type.insertMember("imaginary", HOFFSET(complex_t, im),
                              PredType::NATIVE_DOUBLE);
    PASSED();
  } // end of try block
...
static void test_compound_2() {
  ...
  try {
    ...
    buf = static_cast<unsigned char *>(
        malloc(nelmts * MAX(sizeof(src_typ_t), sizeof(dst_typ_t))));
    ...
    for (i = 0; i < nelmts; i++) {
      ...
      if (s_ptr->a != d_ptr->a || s_ptr->b != d_ptr->b ||
        ...
        H5_FAILED();
        cerr << "    i=" << i << endl;
        cerr << "    src={a=" << s_ptr->a << ", b=" << s_ptr->b << "c=["
             << s_ptr->c[0] << "," << s_ptr->c[1] << "," << s_ptr->c[2] << ","
             << s_ptr->c[3] << ", d=" << s_ptr->d << ", e=" << s_ptr->e << "}"
             << endl;
        cerr << "    dst={a=" << s_ptr->a << ", b=" << s_ptr->b << "c=["
             << s_ptr->c[0] << "," << s_ptr->c[1] << "," << s_ptr->c[2] << ","
             << s_ptr->c[3] << ", d=" << s_ptr->d << ", e=" << s_ptr->e << "}"
             << endl;
      }
    ...
    for (i = 0; i < nelmts; i++) {
      ...
      if (s_ptr->a != d_ptr->a || s_ptr->c[0] != d_ptr->c[0] ||
        ...
        cerr << "    src={a=" << s_ptr->a << ", b=" << s_ptr->b << ", c=["
             ...
             << s_ptr->c[3] << "], d=" << s_ptr->d << ", e=" << s_ptr->e << "}"
             << endl;
        cerr << "    dst={a=" << d_ptr->a << ", c=[" << d_ptr->c[0] << ","
             << d_ptr->c[1] << "," << d_ptr->c[2] << "," << d_ptr->c[3]
             << "], e=" << d_ptr->e << "}" << endl;
      }
    }
    ...
    for (i = 0; i < nelmts; i++) {
      ...
      if (s_ptr->a != d_ptr->a || s_ptr->b != d_ptr->b ||
        ...
        cerr << "    src={a=" << s_ptr->a << ", b=" << s_ptr->b << "c=["
             ...
             << s_ptr->c[3] << ", d=" << s_ptr->d << ", e=" << s_ptr->e << "}"
             << endl;
        cerr << "    dst={a=" << d_ptr->a << ", b=" << d_ptr->b << "c=["
             << d_ptr->c[0] << "," << d_ptr->c[1] << "," << d_ptr->c[2] << ","
             << d_ptr->c[3] << ", d=" << d_ptr->d << ", e=" << d_ptr->e << "}"
             << endl;
      } // if
    }   // for
...
static void test_compound_5() {
  ...
  try {
    ...
    src_type.insertMember("tdim", HOFFSET(src_typ_t, tdim),
                          PredType::NATIVE_SHORT);
    src_type.insertMember("coll_ids", HOFFSET(src_typ_t, coll_ids),
                          short_array);

    CompType dst_type(sizeof(dst_typ_t));
    dst_type.insertMember("name", HOFFSET(dst_typ_t, name), strg);
    dst_type.insertMember("tdim", HOFFSET(dst_typ_t, tdim),
                          PredType::NATIVE_SHORT);
...
static void test_compound_7() {
  ...
  try {
    ...
    verify_val(type_size, sizeof(s1_typ_t), "DataType::getSize", __LINE__,
               __FILE__);
    ...
    verify_val_noteq(type_size, sizeof(s2_typ_t), "DataType::getSize", __LINE__,
                     __FILE__);
...
static void test_compound_set_size() {
  ...
  try {
    ...
    verify_val(static_cast<long>(new_size), 33, "DataType::getSize", __LINE__,
               __FILE__);
    ...
    verify_val(static_cast<long>(new_size), 32, "DataType::getSize", __LINE__,
               __FILE__);
