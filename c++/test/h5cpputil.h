...
void check_values(const char *value, const char *msg, int line,
...
public:
  InvalidActionException(const H5std_string &func_name,
...
public:
  TestFailedException(const H5std_string &func_name,
                      const H5std_string &message = DEFAULT_MSG);
  TestFailedException();
...
// Verifies
void verify_val(const char *x, const char *value, const char *where, int line,
                const char *file_name);

template <class Type1, class Type2>
void verify_val(Type1 x, Type2 value, const char *where, int line,
                const char *file_name) {
  if (GetTestVerbosity() >= VERBO_HI) {
    cerr << endl;
    cerr << "   Call to routine: " << where << " at line " << line << " in "
         << file_name << " had value " << x << endl;
  }
  if (x != value) {
    cerr << endl;
    cerr << "*** UNEXPECTED VALUE from " << where << " should be " << value
         << ", but is " << x << " at line " << line << " in " << file_name
         << endl;
    IncTestNumErrs();
...
template <class Type1, class Type2>
void verify_val(Type1 x, Type2 value, const char *msg, const char *file_name,
                int line) {
  if (x != value) {
    cerr << endl;
    cerr << "*** UNEXPECTED VALUE: " << file_name << ":line " << line << ": "
         << msg << " different: " << x << ", should be " << value << endl;
    IncTestNumErrs();
...
template <class Type1, class Type2>
void verify_val_noteq(Type1 x, Type2 value, const char *where, int line,
                      const char *file_name) {
  if (GetTestVerbosity() >= VERBO_HI) {
    cerr << endl;
    cerr << "   Call to routine: " << where << " at line " << line << " in "
  ...
  if (x == value) {
    cerr << endl;
    cerr << "*** UNEXPECTED VALUE from " << where << " should not be " << value
...
template <class Type1, class Type2>
void CHECK(Type1 x, Type2 value, const char *msg, int line,
           const char *file_name) {
  if (x == value) {
    cerr << endl;
...
template <class Type1, class Type2>
void verify_val(Type1 x, Type2 value, float epsilon, const char *msg, int line,
                const char *file_name) {
  if (x == value) {
    cerr << endl;
