...
Attribute::Attribute(const Attribute &original)
    : AbstractDs(), H5Location(), id(original.id) {
...
Attribute::Attribute(const hid_t existing_id)
    : AbstractDs(), H5Location(), id(existing_id) {
  incRefCount(); // increment number of references to this id
}
...
///\exception   H5::AttributeIException
//--------------------------------------------------------------------------
void Attribute::write(const DataType &mem_type,
...
ssize_t Attribute::getName(char *attr_name, size_t buf_size) const {
  ...
  if (name_size < 0) {
    ...
    throw AttributeIException("Attribute::getName",
                              "Attribute must have a name, name length is 0");
  }
...
//              library.
//--------------------------------------------------------------------------
hid_t Attribute::getId() const { return (id); }
...
//              Attribute::read
//--------------------------------------------------------------------------
void Attribute::p_read_fixed_len(const DataType &mem_type,
...
//              Attribute::read. -BMR
//--------------------------------------------------------------------------
void Attribute::p_read_variable_len(const DataType &mem_type,
                                    H5std_string &strg) const {
