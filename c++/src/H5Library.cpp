...
//--------------------------------------------------------------------------
void H5Library::dontAtExit() { (void)H5dont_atexit(); }
...
///\exception   H5::LibraryIException
//--------------------------------------------------------------------------
void H5Library::getLibVersion(unsigned &majnum, unsigned &minnum,
                              unsigned &relnum) {
  herr_t ret_value = H5get_libversion(&majnum, &minnum, &relnum);
  if (ret_value < 0) {
    throw LibraryIException("H5Library::getLibVersion",
...
//--------------------------------------------------------------------------
void H5Library::checkVersion(unsigned majnum, unsigned minnum,
                             unsigned relnum) {
  herr_t ret_value = H5check_version(majnum, minnum, relnum);
  if (ret_value < 0) {
    throw LibraryIException("H5Library::checkVersion",
                            "H5check_version failed");
  }
}
...
void H5Library::garbageCollect() {
  ...
  if (ret_value < 0) {
    throw LibraryIException("H5Library::garbageCollect",
                            "H5garbage_collect failed");
  }
}
...
void H5Library::initH5cpp() {
  ...
  if (ret_value != 0)
    throw LibraryIException("H5Library::initH5cpp",
                            "Registering termH5cpp failed");

  ret_value = std::atexit(PredType::deleteConstants);
  if (ret_value != 0)
    throw LibraryIException("H5Library::initH5cpp",
                            "Registering PredType::deleteConstants failed");

  ret_value = std::atexit(PropList::deleteConstants);
  if (ret_value != 0)
    throw LibraryIException("H5Library::initH5cpp",
                            "Registering PropList::deleteConstants failed");
  ...
  if (ret_value != 0)
    throw LibraryIException("H5Library::initH5cpp",
                            "Registering DataSpace::deleteConstants failed");
}
...
///\param       reg_global_lim - IN: Limit on all "regular" free list memory
///used \param       reg_list_lim   - IN: Limit on memory used in each "regular"
///free list \param       arr_global_lim - IN: Limit on all "array" free list
///memory used \param       arr_list_lim   - IN: Limit on memory used in each
///"array" free list \param       blk_global_lim - IN: Limit on all "block" free
///list memory used \param       blk_list_lim   - IN: Limit on memory used in
///each "block" free list \exception   H5::LibraryIException \par Description
...
void H5Library::setFreeListLimits(int reg_global_lim, int reg_list_lim,
  ...
  herr_t ret_value =
      H5set_free_list_limits(reg_global_lim, reg_list_lim, arr_global_lim,
                             arr_list_lim, blk_global_lim, blk_list_lim);
  if (ret_value < 0) {
    throw LibraryIException("H5Library::setFreeListLimits",
                            "H5set_free_list_limits failed");
  }
}
