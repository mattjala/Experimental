...
//              Otherwise, just simply return the object pointer
//              PREDTYPE_CONST_.
//
//              Note that, there is a similar function to getPredTypes() in
//              other classes, that have global constants, is called
//              getConstant().
//
//--------------------------------------------------------------------------
PredType *PredType::getPredTypes() {
  ...
  if (PREDTYPE_CONST_ == 0)
    ...
    throw H5::DataTypeIException("PredType::getPredTypes",
                                 "PredType::getPredTypes is being invoked on "
                                 "an allocated PREDTYPE_CONST_");
...
//  PredType constants.  Note that, there is a similar function to
//  getPredTypes() in other classes, that have global constants, is called
//  getConstant().
...
September 2015:
        ...
        2015, it was recognized that the order in which the global constants
were created and destroyed was actually undefined, thus can be different between
different compilers.  This resulted in failure when compilers destroy PredType
constants before others because when PredType::AtExit was destroyed, the C
library was closed, so when the constants of other classes such as PropList or
DataSpace were being deleted, the C library would not be available.
        ...
        ----------
                ...
                // Creates a dynamic DataSpace object representing the C
constant
                // - in "H5DataSpace.cpp"
        ...
        4.  This section shows the differences between the old and new methods
            ...
            FileCreatPropList, FileAccPropList, LinkAccPropList,
LinkCreatPropList, ObjCreatPropList for those classes.  The HDF5 C constant
"H5P_DEFAULT" can be replaced by any of these respectively: H5P_DATASET_ACCESS,
            H5P_DATASET_CREATE, H5P_DATASET_XFER, H5P_FILE_CREATE,
H5P_FILE_ACCESS, H5P_LINK_ACCESS, H5P_LINK_CREATE, and H5P_OBJECT_CREATE.
        ...
        ----------
            ...
            The same functions are added to the subclasses of PropList instead
of using PropList's because of the class types and in favor of clarity.
