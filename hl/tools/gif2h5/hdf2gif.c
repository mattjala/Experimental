...
int main(int argc, char **argv) {
  ...
  Background = 0;
  {
    ...
    if (H5IMget_image_info(fid, image_name, &width, &height, &planes, interlace,
                           &npals) < 0) {
    ...
    if (npals) {
      if (H5IMget_palette_info(fid, image_name, 0, pal_dims) < 0) {
        fprintf(stderr,
                "Unable to get information of the palette. Aborting.\n");
        goto out;
      }
    ...
    i = hdfWriteGIF(fpGif, Image, 0, RHeight, RWidth, r1, g1, b1, pc2nc, 256, 8,
                    BitsPerPixel);
    fputc(0x00, fpGif);
    free(Image);
  }
