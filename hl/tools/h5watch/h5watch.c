 ...
 *  This tool uses private routines H5LD_construct_vector()and
 * H5LD_clean_vector() This tool uses H5LD_memb_t data structure declared in
 * H5LDprivate.h
 */

static const char *progname = "h5watch"; /* tool name */
static char *g_list_of_fields =
    NULL;                         /* command line input for "list_of_fields" */
static char *g_dup_fields = NULL; /* copy of "list_of_fields" */
static H5LD_memb_t **g_listv = NULL; /* vector info for "list_of_fields" */

static bool g_monitor_size_only =
    false; /* monitor changes in dataset dimension sizes */
static unsigned g_polling_interval =
    1; /* polling interval to check appended data */
...
static unsigned g_retry = DEFAULT_RETRY; /* # of times to try opening the file
                                            if somehow file is unstable */
static bool g_display_hex =
    false; /* display data in hexadecimal format : LATER */
static bool g_user_interrupt =
    false; /* Flag to indicate that user interrupted execution */

static herr_t doprint(hid_t did, const hsize_t *start, const hsize_t *block,
                      int rank);
 ...
 *          Call the tools library routine h5tools_dump_dset() to do the
 *printing. (This routine is mostly copied from dump_dataset_values() in
 *tools/h5ls/h5ls.c and modified accordingly).
 *
 ...
 */
static herr_t slicendump(hid_t did, hsize_t *prev_dims, hsize_t *cur_dims,
                         hsize_t *start, hsize_t *block, int rank,
                         int subrank) {
  int i;                      /* Local index variable */
  ...
  if ((subrank - 1) > 0) {
    ...
    for (i = 0; i < (hssize_t)MIN(prev_dims[ind], cur_dims[ind]); i++) {
      start[ind] = (hsize_t)i;
      if ((ret_value = slicendump(did, prev_dims, cur_dims, start, block, rank,
                                  subrank - 1)) < 0)
        goto done;
    } /* end for */
  }   /* end if */
 ...
 *              For dataset with increase in at least one of its dimension
 *sizes: it will print the new appended data to the dataset
 *
...
static herr_t monitor_dataset(hid_t fid, char *dsetname) {
  ...
  while (!g_user_interrupt) {
    ...
    if (i != ndims) {
      ...
      for (u = 0; u < ndims; u++) {
        fprintf(stdout, "dimension %d: %" PRIuHSIZE "->%" PRIuHSIZE "", u,
                prev_dims[u], cur_dims[u]);
      ...
      if (!g_monitor_size_only) {
        ...
        for (u = 0; u < ndims; u++) {
          ...
          if (cur_dims[u] > prev_dims[u]) {
            ...
            if ((ret_value = slicendump(did, prev_dims, cur_dims, start, block,
                                        ndims, ndims)) < 0)
              goto done;
            break;
          }
...
static herr_t process_cmpd_fields(hid_t fid, char *dsetname) {
  ...
  if (((dtid = H5Dget_type(did)) < 0) ||
      (tid = H5Tget_native_type(dtid, H5T_DIR_DEFAULT)) < 0) {
  ...
  /* Allocate memory for a list vector of H5LD_memb_t structures to store
   * "g_list_of_fields" info */
...
static herr_t check_dataset(hid_t fid, char *dsetname) {
  ...
  bool unlim_max_dims =
      false; /* whether dataset has unlimited or max. dimension setting */
  void *edata;
  H5E_auto2_t func;
  H5D_layout_t layout;
  herr_t ret_value = SUCCEED; /* Return value */
...
static void usage(const char *prog) {
  ...
  fprintf(stdout, "        --dim             Monitor changes in size of "
                  "dataset dimensions only.\n");
  fprintf(
      stdout,
      "        --width=N         Set the number of columns to N for output.\n");
  fprintf(stdout, "                              A value of 0 sets the number "
                  "of columns to the\n");
  fprintf(stdout, "                              maximum (65535). The default "
                  "width is 80 columns.\n");
  fprintf(stdout, "        --polling=N       Set the polling interval to N (in "
                  "seconds) when the\n");
  fprintf(stdout, "                              dataset will be checked for "
                  "appended data.  The default\n");
  fprintf(stdout, "                              polling interval is 1.\n");
  fprintf(stdout, "        --fields=<list_of_fields>\n");
  fprintf(stdout, "                              Display data for the fields "
                  "specified in <list_of_fields>\n");
  fprintf(stdout, "                              for a compound data type.  "
                  "<list_of_fields> can be\n");
  fprintf(stdout, "                              specified as follows:\n");
  fprintf(stdout, "                                   1) A comma-separated "
                  "list of field names in a\n");
  fprintf(stdout, "                                   compound data type.  "
                  "\",\" is the separator\n");
  fprintf(stdout, "                                   for field names while "
                  "\".\" is the separator\n");
  fprintf(stdout, "                                   for a nested field.\n");
  fprintf(stdout, "                                   2) A single field name "
                  "in a compound data type.\n");
  fprintf(stdout, "                                   Can use this option "
                  "multiple times.\n");
  fprintf(stdout, "                              Note that backslash is the "
                  "escape character to avoid\n");
  fprintf(stdout, "                              characters in field names "
                  "that conflict with the tool's\n");
  fprintf(stdout, "                              separators.\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "     OBJECT is specified as "
                  "[<filename>/<path_to_dataset>/<dsetname>]\n");
  fprintf(stdout, "        <filename>            Name of the HDF5 file.  It "
                  "may be preceded by path\n");
  fprintf(stdout, "                              separated by slashes to the "
                  "specified HDF5 file.\n");
  fprintf(stdout, "        <path_to_dataset>     Path separated by slashes to "
                  "the specified dataset\n");
  fprintf(stdout, "        <dsetname>            Name of the dataset\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "     User can end the h5watch process by ctrl-C (SIGINT) or "
  ...
  while ((opt = H5_get_option(argc, argv, s_opts, l_opts)) != EOF) {
    ...
    case 'f': /* --fields=<list_of_fields> */
      if (g_list_of_fields == NULL) {
        ...
        if ((g_list_of_fields = (char *)realloc(g_list_of_fields,
                                                strlen(g_list_of_fields) +
                                                    strlen(str) + 2)) == NULL) {
          error_msg("memory allocation failed (file %s:line %d)\n", __FILE__,
                    __LINE__);
          leave(EXIT_FAILURE);
...
int main(int argc, char *argv[]) {
  ...
  do {
    while (fname && *fname) {
      fid = h5tools_fopen(fname, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, fapl,
                          false, drivername, sizeof drivername);
