...
static int read_palette(const char *file_name, rgb_t *palette,
                        size_t palette_size);
...
static int test_simple(void) {
  ...
  /* Read image */
  if (H5IMget_image_info(fid, IMAGE1_NAME, &width, &height, &planes, interlace,
                         &npals) < 0)
    goto out;
  ...
  /* Write image */
  if (H5IMmake_image_24bit(fid, IMAGE2_NAME, width, height, "INTERLACE_PIXEL",
                           buf2))
    goto out;

  /* Read image */
  if (H5IMget_image_info(fid, IMAGE2_NAME, &width, &height, &planes, interlace,
                         &npals) < 0)
    goto out;
...
static int test_data(void) {
  ...
  /* make image */
  if ((H5IMmake_image_24bit(fid, IMAGE3_NAME, width, height, "INTERLACE_PLANE",
                            image_data)) < 0)
    goto out;
...
The example generates 3 HDF5 images, one that generates an image from all the
values, another that generates an image from the land values and another that
generates an image from the sea values. For the example we used data from MODB,
the Mediterranean Oceanic Data Base http://modb.oce.ulg.ac.be/
...
static int test_generate(void) {
  ...
  !The second line contains the exclusion value, the minimum and the maximum
  value of this ! file. These numbers are stored with the FORTRAN format E12.5.
  ! The remaining lines contains the data of the array, with 5 numbers per line
  ! (except the last line for each I-line).
  ! The array is stored in horizontal slices from sea surface to sea bottom and
  from ! north to south. So the indexes go from :
  !
  !   DO K = KMAX to 1
  !       DO J = JMAX to 1
  !           DO I = 1 to IMAX
  ...
  */
  ...
  if (NULL == data)
    goto out;
  image_data =
      (unsigned char *)malloc((size_t)n_elements * sizeof(unsigned char));
  if (NULL == image_data)
    goto out;
  ...
  /* Make the image */
  if ((H5IMmake_image_8bit(fid, "All data", (hsize_t)imax, (hsize_t)jmax,
                           image_data)) < 0)
    goto out;
  ...
  /* make the image */
  if ((H5IMmake_image_8bit(fid, "Land data", (hsize_t)imax, (hsize_t)jmax,
                           image_data)) < 0)
    goto out;
  ...
  for (i = 0; i < n_elements; i++) {
    if (data[i] > 0.0F)
      ...
      image_data[i] =
          (unsigned char)((255.0F * (data[i] - xmin)) / (xmax - xmin));
    }
  }

  /* make the image */
  if ((H5IMmake_image_8bit(fid, "Sea data", (hsize_t)imax, (hsize_t)jmax,
                           image_data)) < 0)
    goto out;
...
static int read_palette(const char *fname, rgb_t *palette,
                        size_t palette_size) {
  FILE *file;
  char buffer[80];
  unsigned u;
