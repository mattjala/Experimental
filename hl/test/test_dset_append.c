...
static int test_dataset_append_notset(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DNAME_NOTSET, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static herr_t append_func(hid_t H5_ATTR_UNUSED dset_id,
 ...
 *
 * Purpose:     Verify that the object flush property and the append flush
 *property are working properly when appending rows and columns to a dataset
...
static int test_dataset_append_rows_columns(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DNAME_UNLIM, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, dapl)) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Verify that the object flush property and the append flush
 *property are working properly when appending rows to a dataset with one
 *extendible dimension (row).
 *
 * Return:      Success:    0
 *              Failure:    1
 *
...
static int test_dataset_append_rows(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DNAME_ROW, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        dapl)) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Verify that the object flush property and the append flush
 *property are working properly when appending columns to a dataset with one
 *extendible dimension (column).
 *
 * Return:      Success:    0
 *              Failure:    1
 *
...
static int test_dataset_append_columns(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DNAME_COLUMN, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, dapl)) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Verify that the object flush property and the append flush
 *property are working properly when appending rows and columns to an extendible
 *dataset.
 *
 *              A BUG occurs:
 ...
 *              The data is incorrect when the dataset is closed, opened again,
 *and read at this point; NOTE: the problem does not occur when H5Dflush() is
 *not performed for each row/column.
 *
 * Return:      Success:    0
 *              Failure:    1
 *
...
static int test_dataset_append_BUG1(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DBUGNAME1, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        dapl)) < 0)
    TEST_ERROR;
...
static int test_dataset_append_BUG2(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DBUGNAME2, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        dapl)) < 0)
    TEST_ERROR;
 ...
 * Purpose:     Verify that the object flush property and the append flush
 *property are working properly when appending rows and columns to an extendible
 *dataset where the append size is less than the boundary size.
 *
 * Return:      Success:    0
 *              Failure:    1
 *
...
static int test_dataset_append_less(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DNAME_LESS, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        dapl)) < 0)
    TEST_ERROR;
 ...
 * Purpose:     Verify that the object flush property and the append flush
 *property are working properly when appending rows and columns to an extendible
 *dataset where row: the append size is 3 times of the boundary size the append
 *callback/flush is performed on the 1st boundary hit column: the boundary is
 *greater than the append size the boundary is not hit at all
 *
 * Return:      Success:    0
 *              Failure:    1
 *
...
static int test_dataset_append_vary(hid_t fid) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DNAME_VARY, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        dapl)) < 0)
    TEST_ERROR;
...
int main(void) {
  ...
  /*
   * The following tests illustrate the scenarios when H5DOappend does not work
   * with extensible array indexing:
   *        - when the dataset has 1 unlimited dimension and the other dimension
   * is fixed but extendible
   *        - the dataset expands along 1 dimension and then expands along the
   * other dimension
   */
  flush_ct = 0; /* Reset flush counter */
