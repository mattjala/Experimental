...
static particle_t testPart[NRECORDS] = {
    {"zero", 0, 0, 0.0F, 0.0},    {"one", 10, 10, 1.0F, 10.0},
...
static hid_t make_particle_type(void) {
  ...
  if (H5Tinsert(type_id, "Lat", HOFFSET(particle_t, lati), H5T_NATIVE_INT) < 0)
    return FAIL;
  if (H5Tinsert(type_id, "Long", HOFFSET(particle_t, longi), H5T_NATIVE_INT) <
      0)
    return FAIL;
  if (H5Tinsert(type_id, "Pressure", HOFFSET(particle_t, pressure),
                H5T_NATIVE_FLOAT) < 0)
    return FAIL;
  if (H5Tinsert(type_id, "Temperature", HOFFSET(particle_t, temperature),
                H5T_NATIVE_DOUBLE) < 0)
    return FAIL;
...
static int create_hl_table(hid_t fid) {
  ...
  size_t part_offset[NFIELDS] = {
      HOFFSET(particle_t, name), HOFFSET(particle_t, lati),
      HOFFSET(particle_t, longi), HOFFSET(particle_t, pressure),
      HOFFSET(particle_t, temperature)};

  /* Define field information */
  const char *field_names[NFIELDS] = {"Name", "Latitude", "Longitude",
  ...
  status = H5TBmake_table("Table Title", fid, H5TB_TABLE_NAME, (hsize_t)NFIELDS,
                          (hsize_t)NRECORDS, sizeof(particle_t), field_names,
                          part_offset, field_type, chunk_size, fill_data,
                          compress, testPart);
...
static int test_compress(void) {
  ...
  /* Create a file. */
  if ((fid1 = H5Fcreate(TEST_COMPRESS_FILE, H5F_ACC_TRUNC, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  table =
      H5PTcreate_fl(fid1, "Compressed Test Dataset", part_t, (hsize_t)80, 8);
  if (H5PTis_valid(table) < 0)
    TEST_ERROR;
...
static int test_rw_nonnative_dt(hid_t fid) {
  ...
  /* This table's "packets" will be simple integers and it will use no
   * compression */
  if (H5Tget_order(H5T_NATIVE_INT) == H5T_ORDER_LE) {
    ptable = H5PTcreate(fid, "Packet Test Dataset, Non-native", H5T_STD_I32BE,
                        (hsize_t)100, H5P_DEFAULT);
  } else {
    ptable = H5PTcreate(fid, "Packet Test Dataset, Non-native", H5T_STD_I32LE,
                        (hsize_t)100, H5P_DEFAULT);
  }
