...
static int verify_ptlengthtype(hid_t fid, const char *table_name,
                               herr_t checked_value);

/* Adds an attribute to the named packet table */
static int adding_attribute(hid_t fid, const char *table_name,
                            const char *attr_name);

/* Verifies that the named attribute was written and is read correctly */
static int verify_attribute(hid_t fid, const char *table_name,
                            const char *attr_name);
...
static int test_VLof_atomic(void) {
  ...
  snprintf(msg, sizeof(msg),
           "The number of packets in the packet table must be %u\n", NRECORDS);
  ...
  for (uu = 0; uu < NRECORDS; uu++)
    for (vv = 0; vv < (uu + 1); vv++) {
      if (((unsigned int *)readBuf[uu].p)[vv] !=
...
static int test_VLof_comptype(void) {
  ...
  for (uu = 0; uu < NRECORDS; uu++) {
    if (writeBuf[uu].len != readBuf[uu].len) {
      fprintf(stderr,
              "%d: VL data length don't match!, writeBuf[%u].len=%d, "
              "readBuf[%u].len=%d\n",
    ...
    for (vv = 0; vv < (uu + 1); vv++) {
      if (((unsigned int *)writeBuf[uu].p)[vv] !=
          ((unsigned int *)readBuf[uu].p)[vv]) {
        fprintf(stderr,
                "VL data values don't match!, writeBuf[uu].p[%d]=%d, "
                "readBuf[uu].p[%d]=%d\n",
...
static int test_compound_VL_VLtype(void) {
  ...
  for (uu = 0; uu < NRECORDS; uu++) {
    ...
    for (t1 = (hvl_t *)((writeBuf[uu].v).p), vv = 0; vv < (uu + L1_INCM);
         vv++, t1++) {
  ...
  for (uu = 0; uu < NRECORDS; uu++) {
    if (writeBuf[uu].u != readBuf[uu].u) {
      fprintf(stderr,
              "Integer components don't match!, writeBuf[%u].u=%u, "
              "readBuf[%u].u=%u\n",
    ...
    if (!H5_FLT_ABS_EQUAL(writeBuf[uu].f, readBuf[uu].f)) {
      fprintf(stderr,
              "Float components don't match!, writeBuf[%u].f=%f, "
              "readBuf[%u].f=%f\n",
    ...
    if (writeBuf[uu].v.len != readBuf[uu].v.len) {
      fprintf(stderr,
              "%d: VL data length don't match!, writeBuf[%d].v.len=%zu, "
              "readBuf[%d].v.len=%zu\n",
              __LINE__, uu, writeBuf[uu].v.len, uu, readBuf[uu].v.len);
      continue;
    } /* end if */

    for (t1 = (hvl_t *)(writeBuf[uu].v.p), t2 = (hvl_t *)(readBuf[uu].v.p),
        vv = 0;
         (size_t)vv < readBuf[uu].v.len; vv++, t1++, t2++) {
      if (t1->len != t2->len) {
        fprintf(stderr,
                "%d: VL data length don't match!, uu=%u, vv=%u, t1->len=%zu, "
                "t2->len=%zu\n",
      ...
      for (ww = 0; (size_t)ww < t2->len; ww++) {
        if (((unsigned int *)t1->p)[ww] != ((unsigned int *)t2->p)[ww]) {
          fprintf(stderr,
                  "VL data values don't match!, t1->p[%u]=%u, t2->p[%u]=%u\n",
                  ww, ((unsigned int *)t1->p)[ww], ww,
                  ((unsigned int *)t2->p)[ww]);
          continue;
        } /* end if */
      }   /* end for */
    }     /* end for */
...
static int test_H5PTis_varlen(void) {
  ...
  /* Create a new table */
  ptable =
      H5PTcreate(fid, PT_FIXED_LEN, H5T_STD_I32BE, (hsize_t)100, H5P_DEFAULT);
  ...
  if (ret < 0)
    goto error;
  ret =
      verify_ptlengthtype(fid, PT_COMP_VLEN, 0); /* compound of vlen, no vlen */
  if (ret < 0)
    goto error;
...
#define ATTR_RANK 1
#define ATTR_DIM 3
static int attr_data[ATTR_DIM] = {256, 11945,
                                  -22107}; /* values to be written to attr */
...
static int adding_attribute(hid_t fid, const char *table_name,
  ...
  attr_id = H5Acreate2(dset_id, attr_name, H5T_NATIVE_INT, space_id,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t verify_attribute(hid_t fid, const char *table_name,
                               const char *attr_name) {
  hid_t ptable = H5I_INVALID_HID;  /* Packet table identifier */
  hid_t attr_id = H5I_INVALID_HID; /* Attribute identifier */
  ...
  for (ii = 0; ii < ATTR_DIM; ii++)
    if (attr_data[ii] != read_data[ii])
      TestErrPrintf(
          "%d: attribute data different: attr_data[%d]=%d, read_data[%d]=%d\n",
 ...
 *-------------------------------------------------------------------------
 */
static herr_t verify_accessors(hid_t fid, const char *table_name,
                               bool uses_vlen_type) {
  hid_t ptable = H5I_INVALID_HID;   /* Packet table identifier */
  hid_t dset_id = H5I_INVALID_HID;  /* Dataset associated with the pt */
  ...
  *buf = '\0';
  if ((name_size = H5Iget_name(dset_id, (char *)buf, NAME_BUF_SIZE)) < 0)
    goto error;
  VERIFY(strcmp(buf, table_name),
         "Names of dataset and packet table don't match");
...
static int testfl_VLof_comptype(void) {
  ...
  for (uu = 0; uu < NRECORDS; uu++) {
    if (writeBuf[uu].len != readBuf[uu].len) {
      fprintf(stderr,
              "%d: VL data length don't match!, writeBuf[%u].len=%zu, "
              "readBuf[%u].len=%zu\n",
    ...
    for (vv = 0; vv < (uu + 1); vv++) {
      if (((unsigned int *)writeBuf[uu].p)[vv] !=
          ((unsigned int *)readBuf[uu].p)[vv]) {
        fprintf(stderr,
                "VL data values don't match!, writeBuf[uu].p[%u]=%u, "
                "readBuf[uu].p[%u]=%u\n",
                vv, ((unsigned int *)writeBuf[uu].p)[vv], vv,
                ((unsigned int *)readBuf[uu].p)[vv]);
        continue;
      } /* write value != read value */
    }
...
static int testfl_compound_VL_VLtype(void) {
  ...
  for (uu = 0; uu < NRECORDS; uu++) {
    ...
    if (writeBuf[uu].v.len != readBuf[uu].v.len) {
      fprintf(stderr,
              "%d: VL data length don't match!, writeBuf[%u].v.len=%zu, "
              "readBuf[%u].v.len=%zu\n",
              __LINE__, uu, writeBuf[uu].v.len, uu, readBuf[uu].v.len);
      continue;
    } /* end if */
