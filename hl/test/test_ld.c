 ...
 */
#define TWO_NTESTS 9
static int two_tests[TWO_NTESTS][2] = {{2, 2},  {2, -1},  {2, 0},
                                       {-1, 2}, {-1, -1}, {-1, 0},
                                       {0, 2},  {0, -1},  {0, 0}};
...
#define TWO_CMPD_VALID1                                                        \
  2 /* DSET_CMPD_TWO with VALID_FIELDS1 or VALID_ESC_FIELDS1 */
#define TWO_CMPD_VALID2                                                        \
  3 /* DSET_CMPD_TWO with VALID_FIELDS2 or VALID_ESC_FIELDS2 */
...
static herr_t test_LD_elmts_one(const char *file, const char *dname,
                                const char *fields);
static herr_t test_LD_elmts_two(const char *file, const char *dname,
                                const char *fields);

static herr_t verify_elmts_two(int type, const hsize_t *ext_dims,
                               const hsize_t *prev_dims, void *_ldbuf,
                               void *_buf);
...
/* NOTE:
 * This will fail on heiwa and amani when VALID_FIELDS1 is
 * "field1,field3,field4" because of alignment problems: amani and heiwa - 8
 * byte alignment jam - 4 byte alignment This will need to be fixed in the
 * library for H5Tget_native_type().
 */
...
static test_valid_fields1
    *vbuf1; /* buffer for storing retrieved elements (FIELDS1) */
static test_valid_fields2
    *vbuf2; /* buffer for storing retrieved elements (FIELDS2) */
...
static herr_t test_LD_dims_params(const char *file) {
  ...
  hsize_t
      one_cur_dims[1]; /* current dimension sizes for 1-dimensonal dataset */
  hsize_t
      two_cur_dims[2]; /* current dimension sizes for 2-dimensional dataset */
  hid_t invalid_id = -1;
...
static herr_t test_LD_dims(const char *file) {
  ...
  hid_t did = -1; /* dataset identifier */
  hsize_t
      one_prev_dims[1]; /* original dimension sizes for 1-dimensonal dataset */
  hsize_t
      one_cur_dims[1]; /* current dimension sizes for 1-dimensonal dataset */
  hsize_t
      one_ext_dims[1]; /* extended dimension sizes for 1-dimensonal dataset */
  hsize_t
      two_prev_dims[2]; /* original dimension sizes for 2-dimensional dataset */
  hsize_t
      two_cur_dims[2]; /* current dimension sizes for 2-dimensional dataset */
  hsize_t
      two_ext_dims[2]; /* extended dimension sizes for 2-dimensional dataset*/
 ...
 *      4. INVALID_ESC_FIELDS1:
 *"field2\\..\\,k.a,field2\\..\\\\c,field4\\,.k\\,"
 *      5. INVALID_ESC_FIELDS2:
 *"field2\\..\\,b.a,field2\\..\\\\c,field4\\,.b\\,."
 *       6. INVALID_ESC_FIELDS3:
 *"field2\\..\\,,b.a,field2\\..\\\\c,field4\\,.b\\,"
 *
 **********************************************************************************
 */
static int test_LD_size(const char *file) {
  ...
  /* Obtain size for VALID_ESC_FIELDS1:
   * "field\\,1,field2\\..\\.a,field\\\\3,field4\\," */
...
static int test_LD_elmts_invalid(const char *file) {
  ...
  /* Verify failure from case #4: FIELDS is nonNULL but the dataset is not
   * compound datatype */
  ...
  ret = H5LDget_dset_elmts(did, prev_dims, cur_dims,
                           "field2.k.a,field2.c,field4.k", tbuf);
  VERIFY_EQUAL(ret, FAIL)
 ...
 **************************************************************************************
 * Testing for the High Level public routine: H5LDget_dset_elmts()
 *     Verify elements retrieved via H5LDget_dset_elmts() are correct as
 *expected when the dataset's dimension sizes are changed according to
 *one_tests[]:
 *
...
static herr_t test_LD_elmts_one(const char *file, const char *dname,
  ...
  for (i = 0; i < ONE_NTESTS; i++) {
    ...
    if (one_tests[i] > 0) {
      if (!strcmp(dname, DSET_CMPD) || !strcmp(dname, DSET_CMPD_ESC)) {
        if (fields) {
          if (!strcmp(fields, VALID_FIELDS1) ||
              !strcmp(fields, VALID_ESC_FIELDS1)) {
          ...
          else if (!strcmp(fields, VALID_FIELDS2) ||
                   !strcmp(fields, VALID_ESC_FIELDS2)) {
            /* Retrieve the elmemts in BUF */
      ...
      /* Verify failure when changes between prev_dims and ext_dims are
       * same/decrease */
 ...
 * Testing for the High Level public routine: H5LDget_dset_elmts()
 *     Verify elements retrieved via H5LDget_dset_elmts() are correct as
 *expected when the datset's dimension sizes are changed accordingly to
 *two_tests[]:
 *
      ...
      if (!strcmp(dname, DSET_CMPD_TWO)) {
        if (fields) {
          if (!strcmp(fields, VALID_FIELDS1) ||
            ...
            if (H5LDget_dset_elmts(did, prev_dims, ext_dims, fields, vbuf1) < 0)
              TEST_ERROR;
            if (verify_elmts_two(TWO_CMPD_VALID1, ext_dims, prev_dims, vbuf1,
                                 cbuf) < 0)
              TEST_ERROR;
          } /* end if */
          else if (!strcmp(fields, VALID_FIELDS2) ||
            ...
            if (H5LDget_dset_elmts(did, prev_dims, ext_dims, fields, vbuf2) < 0)
              TEST_ERROR;
            if (verify_elmts_two(TWO_CMPD_VALID2, ext_dims, prev_dims, vbuf2,
                                 cbuf) < 0)
              TEST_ERROR;
          } /* end else-if */
          else
          ...
          if (H5LDget_dset_elmts(did, prev_dims, ext_dims, fields, ccbuf) < 0)
            TEST_ERROR;
          if (verify_elmts_two(TWO_CMPD_NULL, ext_dims, prev_dims, ccbuf,
                               cbuf) < 0)
            TEST_ERROR;
        }    /* end else */
      }      /* end if */
...
int main(void) {
  ...
  /* Set up temporary buffers for tests: test_LD_elmts_one() &
   * test_LD_elmts_two() */
  ...
  if (NULL == (vbuf1 = (test_valid_fields1 *)malloc(sizeof(test_valid_fields1) *
                                                    TEST_BUF_SIZE)))
    FAIL_STACK_ERROR;
  if (NULL == (vbuf2 = (test_valid_fields2 *)malloc(sizeof(test_valid_fields2) *
                                                    TEST_BUF_SIZE)))
    FAIL_STACK_ERROR;
