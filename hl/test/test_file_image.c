...
/* For superblock version 0, 1: the offset to "file consistency flags" is 20
 * with size of 4 bytes */
/* The file consistency flags is the "status_flags" field in H5F_super_t */
/* Note: the offset and size will be different when using superblock version 2
 * for the test file */
 ...
 *-------------------------------------------------------------------------
 */
static int test_file_image(size_t open_images, size_t nflags,
  ...
  if (NULL ==
      (input_flags = (unsigned *)malloc(sizeof(unsigned) * open_images)))
  ...
  for (i = 0; i < open_images; i++) {
    ...
    /* create file */
    if ((file_id[i] = H5Fcreate(filename[i], H5F_ACC_TRUNC, H5P_DEFAULT,
                                H5P_DEFAULT)) < 0)
    ...
    if ((dset_id[i] = H5Dcreate2(file_id[i], DSET_NAME, H5T_NATIVE_INT,
    ...
    if (i == 1) {
      if (H5Dwrite(dset_id[i], H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   data2) < 0)
    ...
    /* buffers for the rest of the file images are filled with data from the
       respective files */
    else {
      if ((buf_size[i] = H5Fget_file_image(file_id[i], buf_ptr[i],
                                           (size_t)buf_size[i])) < 0)
        FAIL_PUTS_ERROR("H5Fget_file_image() failed");
    } /* end else */
  ...
  for (i = 0; i < open_images; i++) {
    ...
    if (i == 2) {
      H5E_BEGIN_TRY {
        ...
        file_id[i] = H5LTopen_file_image(buf_ptr[i], (size_t)buf_size[i],
                                         input_flags[i]);
      }
      H5E_END_TRY
    ...
    /* open rest of valid file images */
      ...
      if ((file_id[i] = H5LTopen_file_image(buf_ptr[i], (size_t)buf_size[i],
                                            input_flags[i])) < 0)
      ...
      if (input_flags[i] & H5LT_FILE_IMAGE_DONT_COPY)
        ...
        VERIFY(*core_buf_ptr_ptr != buf_ptr[i],
               "vfd buffer and user buffer should be different");

      /*
       *  When the vfd and user buffers are different and
       * H5LT_FILE_IMAGE_OPEN_RW is enabled, status_flags in the superblock
       * needs to be cleared in the vfd buffer for the comparison to proceed as
       * expected.  The user buffer as returned from H5Fget_file_image() has
       * already cleared status_flags.  The superblock's status_flags is used
       * for the implementation of file locking.
       */
      if (input_flags[i] & H5LT_FILE_IMAGE_OPEN_RW &&
          !(input_flags[i] & H5LT_FILE_IMAGE_DONT_COPY)) {
        ...
        /* Clear status_flags in the superblock for the vfd buffer: file locking
         * is using status_flags
         */
  ...
  for (i = 0; i < open_images; i++) {
    ...
    /* read dataset */
    if (H5Dread(dset_id[i], H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                data3) < 0)
      ...
      for (j = 0; j < n_values / nrow; j++)
        for (k = 0; k < nrow; k++)
          if (data3[j * nrow + k] == data1[j * nrow + k])
            FAIL_PUTS_ERROR("comparison of image values with original data "
                            "should have failed");
    } /* end if */
    /* verify contents for the rest of the file images */
      ...
      for (j = 0; j < n_values / nrow; j++)
        for (k = 0; k < nrow; k++)
          if (data3[j * nrow + k] != data1[j * nrow + k])
            FAIL_PUTS_ERROR(
                "comparison of image values with original data failed");
    } /* end else */
  ...
  for (i = 0; i < open_images; i++) {
    ...
    if (!(input_flags[i] & H5LT_FILE_IMAGE_OPEN_RW)) {
      ...
      H5E_BEGIN_TRY {
        status1 = H5Dwrite(dset_id[i], H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                           H5P_DEFAULT, data1);
      }
      H5E_END_TRY
 ...
 */
  ...
  for (i = 0; i < open_images; i++) {
    /* if opening the file image failed, continue next iteration */
    if ((dset_id[i] < 0) || (file_id[i] < 0) ||
        (!(input_flags[i] & H5LT_FILE_IMAGE_OPEN_RW)))
      continue;
...
int main(void) {
  ...
  flags[7] = H5LT_FILE_IMAGE_OPEN_RW | H5LT_FILE_IMAGE_DONT_COPY |
             H5LT_FILE_IMAGE_DONT_RELEASE;

  /* Test file image operations. The flag combinations are assigned to file
   * images in round-robin fashion */
