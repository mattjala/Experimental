...
static int cmp_par(hsize_t i, hsize_t j, particle_t *rbuf, particle_t *wbuf) {
  if ((strcmp(rbuf[i].name, wbuf[j].name) != 0) ||
      rbuf[i].lati != wbuf[j].lati || rbuf[i].longi != wbuf[j].longi ||
    ...
    fprintf(stderr, "%s %ld %f %f %d\n", rbuf[i].name, rbuf[i].longi,
            (double)rbuf[i].pressure, rbuf[i].temperature, rbuf[i].lati);
    fprintf(stderr, "%s %ld %f %f %d\n", wbuf[j].name, wbuf[j].longi,
            (double)wbuf[j].pressure, wbuf[j].temperature, wbuf[j].lati);
    return -1;
  }
  return 0;
}
...
static int test_table(hid_t fid, int do_write) {
  ...
  particle_t abuf[2] = {{"eight", 80, 8.0, 80.0, 80},
                        {"nine", 90, 9.0, 90.0, 90}};
  ...
  float pressure_in[NRECORDS] = {0.0F, 1.0F, 2.0F, 3.0F,
                                 4.0F, 5.0F, 6.0F, 7.0F};
  ...
  size_t field_offset_pos[2] = {HOFFSET(position_t, longi),
                                HOFFSET(position_t, lati)};
  size_t field_offset_namepre[2] = {HOFFSET(namepressure_t, name),
                                    HOFFSET(namepressure_t, pressure)};
  size_t field_sizes_pos[2] = {sizeof(position_in[0].longi),
                               sizeof(position_in[0].lati)};
  size_t field_sizes_namepre[2] = {sizeof(namepre_in[0].name),
  ...
  size_t dst_offset2[NFIELDS + 1] = {
      HOFFSET(particle2_t, name),     HOFFSET(particle2_t, longi),
      HOFFSET(particle2_t, pressure), HOFFSET(particle2_t, temperature),
      HOFFSET(particle2_t, lati),     HOFFSET(particle2_t, new_field)};
  size_t dst_sizes2[NFIELDS + 1] = {
      sizeof(rbuf2[0].name),     sizeof(rbuf2[0].longi),
  ...
  size_t dst_offset3[NFIELDS - 1] = {
      HOFFSET(particle3_t, name), HOFFSET(particle3_t, longi),
      HOFFSET(particle3_t, temperature), HOFFSET(particle3_t, lati)};

  size_t dst_sizes3[NFIELDS - 1] = {
      sizeof(rbuf3[0].name), sizeof(rbuf3[0].longi),
  ...
  size_t tbl_offset[NFIELDS + 1] = {
      HOFFSET(particle4_t, state), HOFFSET(particle4_t, posx),
  ...
  size_t field_offset[NFIELDS] = {
      HOFFSET(particle_t, name), HOFFSET(particle_t, longi),
      HOFFSET(particle_t, pressure), HOFFSET(particle_t, temperature),
      HOFFSET(particle_t, lati)};
  size_t field_size[NFIELDS] = {
      sizeof(rbuf[0].name), sizeof(rbuf[0].longi), sizeof(rbuf[0].pressure),
  ...
  particle4_t fill_data[1] = {{9999999,
                               -9999999,
                               999999,
                               {999, 999, 999},
                               {999, 999, 999},
  ...
  const char *field_names[NFIELDS] = {"Name", "Longitude", "Pressure",
  ...
  if (do_write) {
    ...
    if (H5TBmake_table(TITLE, fid, "table1", FIELDS, RECORDS, type_size_mem,
                       field_names, field_offset, field_type, chunk_size, fill,
                       compress, wbuf) < 0)
      goto out;
    PASSED();
  }
  ...
  if (H5TBread_table(fid, "table1", type_size_mem, field_offset, field_size,
                     rbuf) < 0)
    goto out;
  ...
  if (do_write) {
    ...
    wstart = 0;
    wrecords = 8;
    if (H5TBwrite_records(fid, "table2", wstart, wrecords, type_size_mem,
                          field_offset, field_size, wbuf) < 0)
      goto out;

    /* read it back */
    if (H5TBread_table(fid, "table2", type_size_mem, field_offset, field_size,
                       rbuf) < 0)
      goto out;
  ...
  rstart = 0;
  rrecords = 8;
  if (H5TBread_records(fid, tname, rstart, rrecords, type_size_mem,
  ...
  if (do_write) {
     ...
     */
    arecords = 2;
    if (H5TBappend_records(fid, "table2", arecords, type_size_mem, field_offset,
                           field_size, abuf) < 0)
      return 1;
    ...
    if (H5TBread_records(fid, "table2", rstart, rrecords, type_size_mem,
                         field_offset, field_size, rbuf) < 0)
      return 1;
  ...
  if (do_write) {
    ...
    istart = 1;
    irecords = 2;
    if (H5TBinsert_record(fid, "table2", istart, irecords, type_size_mem,
                          field_offset, field_size, ibuf) < 0)
      return 1;
  ...
  if (do_write) {
    ...
    rstart = 0;
    if (H5TBread_records(fid, "table3", rstart, rrecords, type_size_mem,
                         field_offset, field_size, rbuf) < 0)
      goto out;
    ...
    /* Read complete table */
    if (H5TBread_table(fid, "table3", type_size_mem, field_offset, field_size,
                       rbuf) < 0)
      goto out;
  ...
  if (do_write) {
    ...
    /* Make the table */
    if (H5TBmake_table("Table Title", fid, "table", NFIELDS + 1,
                       (hsize_t)NRECORDS, tbl_size, field_names4, tbl_offset,
                       field_type4, chunk_size, fill_data, compress,
                       p_data) < 0)
      goto out;
    /* Delete records */
    start = 3;
  ...
  if (do_write) {
    ...
    nrecords = 2;
    start2 = 6;
    if (H5TBadd_records_from(fid, "table4", start1, nrecords, "table5",
                             start2) < 0)
      goto out;

    /* read final table */
    if (H5TBread_table(fid, "table5", type_size_mem, field_offset, field_size,
                       rbuf) < 0)
      goto out;
  ...
  if (do_write) {
    ...
    /* read merged table */
    if (H5TBread_table(fid, "table8", type_size_mem, field_offset, field_size,
                       rbufc) < 0)
      goto out;
    ...
    fid1 = H5Fcreate("combine_tables1.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                     H5P_DEFAULT);
    fid2 = H5Fcreate("combine_tables2.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                     H5P_DEFAULT);
    ...
    /* read merged table */
    if (H5TBread_table(fid1, "table3", type_size_mem, field_offset, field_size,
                       rbufc) < 0)
      goto out;
  ...
  if (do_write) {
    ...
    if (H5TBmake_table(TITLE, fid, "table9", FIELDS, RECORDS, type_size_mem,
                       field_names, field_offset, field_type, chunk_size, fill1,
                       compress, 0) < 0)
      goto out;
    ...
    if (H5TBwrite_fields_name(fid, "table9", "Pressure", start, nrecords,
                              sizeof(float), 0, field_sizes_pre,
                              pressure_in) < 0)
      goto out;
    ...
    start = 2;
    nrecords = 3;
    if (H5TBwrite_fields_name(fid, "table9", "Latitude,Longitude", start,
                              nrecords, sizeof(position_t), field_offset_pos,
                              field_sizes_pos, position_in) < 0)
      goto out;

    /* read back the all table */
    start = 0;
    nrecords = NRECORDS;
    if (H5TBread_table(fid, "table9", type_size_mem, field_offset, field_size,
                       rbuf) < 0)
      goto out;
      ...
      for (i = 0; i < NRECORDS; i++) {
        if (i >= 2 && i <= 4) {
          if (rbuf[i].lati != position_in[i - NRECORDS_ADD + 1].lati ||
              ...
              !H5_FLT_ABS_EQUAL(rbuf[i].pressure,
                                pressure_in[i - NRECORDS_ADD + 1])) {
            fprintf(stderr, "%ld %f %d\n", rbuf[i].longi,
                    (double)rbuf[i].pressure, rbuf[i].lati);
            fprintf(stderr, "%ld %f %d\n", position_in[i].longi,
                    (double)pressure_in[i], position_in[i].lati);
            goto out;
          }
        }
      }
  ...
  if (do_write) {
    ...
    start = 0;
    nrecords = NRECORDS_ADD;
    if (H5TBwrite_fields_name(fid, "table10", "Latitude,Longitude", start,
                              nrecords, sizeof(position_t), field_offset_pos,
                              field_sizes_pos, position_in) < 0)
      goto out;
  } /*write*/
  ...
  start = 0;
  nrecords = NRECORDS_ADD;
  if (H5TBread_fields_name(fid, "table10", "Latitude,Longitude", start,
                           nrecords, sizeof(position_t), field_offset_pos,
                           field_sizes_pos, position_out) < 0)
    goto out;
  ...
  if (do_write) {
    ...
    if (H5TBwrite_fields_name(fid, "table10", "Name,Pressure", start, nrecords,
                              sizeof(namepressure_t), field_offset_namepre,
                              field_sizes_namepre, namepre_in) < 0)
      goto out;
  } /*write*/
  ...
  start = 0;
  nrecords = NRECORDS;
  if (H5TBread_fields_name(fid, "table10", "Name,Pressure", start, nrecords,
                           sizeof(namepressure_t), field_offset_namepre,
  ...
  for (i = 0; i < 3; i++) {
    ...
    if ((strcmp(namepre_out[i].name, namepre_in[iistart + i].name) != 0) ||
        !H5_FLT_ABS_EQUAL(namepre_out[i].pressure,
                          namepre_in[iistart + i].pressure)) {
      goto out;
    }
  }
  ...
  if (do_write) {
    ...
    start = 2;
    nrecords = NRECORDS_ADD;
    if (H5TBwrite_fields_index(fid, "table11", nfields, field_index_pre, start,
                               nrecords, sizeof(float), 0, field_sizes_pre,
                               pressure_in) < 0)
      goto out;
    ...
    start = 2;
    nrecords = NRECORDS_ADD;
    if (H5TBwrite_fields_index(fid, "table11", nfields, field_index_pos, start,
                               nrecords, sizeof(position_t), field_offset_pos,
                               field_sizes_pos, position_in) < 0)
      goto out;
    ...
    start = 0;
    nrecords = NRECORDS;
    if (H5TBread_fields_index(fid, "table11", nfields, field_index, start,
                              nrecords, type_size_mem, field_offset, field_size,
                              rbuf) < 0)
      goto out;
  ...
  if (do_write) {
    ...
    start = 0;
    nrecords = NRECORDS;
    if (H5TBwrite_fields_index(fid, "table12", nfields, field_index_pre, start,
                               nrecords, sizeof(float), 0, field_sizes_pre,
                               pressure_in) < 0)
      goto out;
  }
  ...
  start = 0;
  nrecords = NRECORDS;
  if (H5TBread_fields_index(fid, "table12", nfields, field_index_pre, start,
                            nrecords, sizeof(float), 0, field_sizes_pre,
                            pressure_out) < 0)
    goto out;
  ...
  if (do_write) {
    ...
    start = 0;
    nrecords = NRECORDS_ADD;
    if (H5TBwrite_fields_index(fid, "table12", nfields, field_index_pos, start,
                               nrecords, sizeof(position_t), field_offset_pos,
                               field_sizes_pos, position_in) < 0)
      goto out;
  } /*write*/
  ...
  start = 0;
  nrecords = NRECORDS_ADD;
  if (H5TBread_fields_index(fid, "table12", nfields, field_index_pos, start,
                            nrecords, sizeof(position_t), field_offset_pos,
                            field_sizes_pos, position_out) < 0)
    goto out;
  ...
  if (do_write) {
    ...
    start = 0;
    nrecords = NRECORDS;
    if (H5TBwrite_fields_index(fid, "table12", nfields, field_index_namepre,
                               start, nrecords, sizeof(namepressure_t),
                               field_offset_namepre, field_sizes_namepre,
                               namepre_in) < 0)
      goto out;
  }
  ...
  start = 0;
  nrecords = NRECORDS;
  if (H5TBread_fields_index(fid, "table12", nfields, field_index_namepre, start,
                            nrecords, sizeof(namepressure_t),
                            field_offset_namepre, field_sizes_namepre,
                            namepre_out) < 0)
    goto out;
  ...
  if (do_write) {
    ...
    for (i = 0; i < NRECORDS; i++) {
      if ((strcmp(rbuf2[i].name, wbuf[i].name) != 0) ||
          rbuf2[i].lati != wbuf[i].lati || rbuf2[i].longi != wbuf[i].longi ||
  ...
  if (do_write) {
    ...
    /* read the table */
    if (H5TBread_table(fid, "table14", dst_size3, dst_offset3, dst_sizes3,
                       rbuf3) < 0)
      goto out;
    ...
    for (i = 0; i < NRECORDS; i++) {
      if ((strcmp(rbuf3[i].name, wbuf[i].name) != 0) ||
          rbuf3[i].lati != wbuf[i].lati || rbuf3[i].longi != wbuf[i].longi ||
  ...
  /* Get field info */
  if (H5TBget_field_info(fid, "table1", names_out, sizes_out, offset_out,
                         &size_out) < 0)
    goto out;
...
int main(void) {
  ...
  puts("Testing table with file creation mode (read/write in native "
       "architecture):");
