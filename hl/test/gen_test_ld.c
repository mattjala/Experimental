 ...
 *
 *  The following datasets are chunked, H5D_ALLOC_TIME_INCR, max. dimensional
 *setting: DSET_ONE: one-dimensional dataset DSET_TWO: two-dimensional dataset
 *      DSET_CMPD: one-dimensional dataset with compound type
 *      DSET_CMPD_ESC: one-dimensional dataset with compound type and member
 *names with escape/separator characters DSET_CMPD_TWO: two-dimensional dataset
 *with compound type
 *
 ...
 **************************************************************************************
 */
static int generate_dset(hid_t fid, const char *dname, int ndims, hsize_t *dims,
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, dname, dtid, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    goto done;
...
int main(void) {
  ...
  set_t
      two_cbuf[TWO_DIMS0 * TWO_DIMS1]; /* Buffer for data with compound type */
  ...
  if (generate_dset(fid, DSET_ONE, 1, cur_dims, max_dims, H5T_NATIVE_INT,
                    one_data) < 0)
    goto done;
  if (generate_dset(fid, DSET_NONE, 1, cur_dims, NULL, H5T_NATIVE_INT,
                    one_data) < 0)
    goto done;
  if (generate_dset(fid, DSET_NOMAX, 1, cur_dims, NULL, H5T_NATIVE_INT,
                    one_data) < 0)
    goto done;
  if (generate_dset(fid, DSET_ALLOC_LATE, 1, cur_dims, max_dims, H5T_NATIVE_INT,
                    one_data) < 0)
    goto done;
  if (generate_dset(fid, DSET_ALLOC_EARLY, 1, cur_dims, max_dims,
                    H5T_NATIVE_INT, one_data) < 0)
    goto done;
  ...
  /* Generate DSET_TWO */
  if (generate_dset(fid, DSET_TWO, 2, cur2_dims, max2_dims, H5T_NATIVE_INT,
                    two_data) < 0)
    goto done;
  ...
  if (H5Tinsert(set_tid, "field2", HOFFSET(set_t, field2), sub2_tid) < 0)
    goto done;
  if (H5Tinsert(set_tid, "field3", HOFFSET(set_t, field3), H5T_NATIVE_DOUBLE) <
      0)
    goto done;
  ...
  if ((esc_set_tid = H5Tcreate(H5T_COMPOUND, sizeof(set_t))) < 0)
    goto done;
  if (H5Tinsert(esc_set_tid, "field,1", HOFFSET(set_t, field1),
                H5T_NATIVE_INT) < 0)
    goto done;
  if (H5Tinsert(esc_set_tid, "field2.", HOFFSET(set_t, field2), esc_sub2_tid) <
      0)
    goto done;
  if (H5Tinsert(esc_set_tid, "field\\3", HOFFSET(set_t, field3),
                H5T_NATIVE_DOUBLE) < 0)
    goto done;
  if (H5Tinsert(esc_set_tid, "field4,", HOFFSET(set_t, field4), esc_sub4_tid) <
      0)
    goto done;

  /* Generate DSET_CMPD, DSET_CMPD_ESC */
  if (generate_dset(fid, DSET_CMPD, 1, cur_dims, max_dims, set_tid, one_cbuf) <
      0)
    goto done;
  if (generate_dset(fid, DSET_CMPD_ESC, 1, cur_dims, max_dims, esc_set_tid,
                    one_cbuf) < 0)
    goto done;
  ...
  if (generate_dset(fid, DSET_CMPD_TWO, 2, cur2_dims, max2_dims, set_tid,
                    two_cbuf) < 0)
    goto done;
  ...
  if ((null_did = H5Dcreate2(fid, DSET_NULL, H5T_NATIVE_UINT, null_sid,
  ...
  if ((scalar_did = H5Dcreate2(fid, DSET_SCALAR, H5T_NATIVE_INT, scalar_sid,
                               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto done;
