...
int main(void) {
  ...
  size_t dst_offset[NFIELDS] = {
      HOFFSET(Particle, name), HOFFSET(Particle, lati),
      HOFFSET(Particle, longi), HOFFSET(Particle, pressure),
      HOFFSET(Particle, temperature)};
  size_t dst_sizes[NFIELDS] = {sizeof(dst_buf[0].name), sizeof(dst_buf[0].lati),
  ...
  float pressure_in[NRECORDS_ADD] = /* Define new values for the field
                                       "Pressure"  */
      {0.0F, 1.0F, 2.0F};
  Position position_in[NRECORDS_ADD] = {
      /* Define new values for "Latitude,Longitude"  */
      {0, 0},
      {10, 10},
      {20, 20}};
  NamePressure
      namepre_in[NRECORDS_ADD] = /* Define new values for "Name,Pressure"  */
      {
          {"zero", 0.0F},
          {"one", 1.0F},
          {"two", 2.0F},
  ...
  file_id =
      H5Fcreate("ex_table_04.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);

  /* Make the table */
  H5TBmake_table("Table Title", file_id, TABLE_NAME, NFIELDS, NRECORDS,
                 dst_size, field_names, dst_offset, field_type, chunk_size,
  ...
  start = 2;
  nrecords = NRECORDS_ADD;
  H5TBwrite_fields_name(file_id, TABLE_NAME, "Latitude,Longitude", start,
                        nrecords, sizeof(Position), field_offset_pos,
                        field_sizes_pos, position_in);
  ...
  for (i = 0; i < NRECORDS; i++) {
    printf("%-5s %-5d %-5d %-5f %-5f", dst_buf[i].name, dst_buf[i].lati,
           dst_buf[i].longi, dst_buf[i].pressure, dst_buf[i].temperature);
    printf("\n");
  }
