...
int TestCompress() {
  ...
  try {
    ...
    FL_PacketTable wrapper(fileID, COMPRESS_PT, H5T_NATIVE_CHAR, 100,
                           dscreatplist.getId());
    ...
    char filter_name[8];
    dcpl.getFilterById(H5Z_FILTER_DEFLATE, flags, cd_nelemts, NULL, 8,
                       filter_name, config);
...
int TestErrors() {
  ...
  if (error >= 0)
    goto error;
  error = wrapper.GetPackets(static_cast<unsigned>(-60),
                             static_cast<unsigned>(-62), records);
  if (error >= 0)
    goto error;
...
int TestHDFFV_9758() {
  ...
  { // so ptable will go out of scope before PASSED
    ...
    if (count != NUM_PACKETS) {
      std::cerr << "Number of packets in packet table should be " << NUM_PACKETS
                << " but is " << count << endl;
    }
