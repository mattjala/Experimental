...
int_f h5ltmake_dataset_c(hid_t_f *loc_id, size_t_f *namelen, _fcd name,
  ...
  ret = H5LTmake_dataset((hid_t)*loc_id, c_name, (int)*rank, c_dims,
                         (hid_t)*type_id, buf);
  if (ret < 0)
    goto done;
...
int_f h5ltset_attribute_c(hid_t_f *loc_id, size_t_f *namelen, _fcd dsetname,
  ...
  if (strncmp(dtype, "I", 1) == 0) {
    if ((size_t)*sizeof_val == sizeof(int))
      ret = H5LT_set_attribute_numerical(c_loc_id, c_name, c_attrname, c_size,
                                         H5T_NATIVE_INT, (const int *)buf);
    else if ((size_t)*sizeof_val == sizeof(long))
      ret = H5LT_set_attribute_numerical(c_loc_id, c_name, c_attrname, c_size,
                                         H5T_NATIVE_LONG, (const long *)buf);
    ...
    if ((size_t)*sizeof_val == sizeof(float))
      ret = H5LT_set_attribute_numerical(c_loc_id, c_name, c_attrname, c_size,
                                         H5T_NATIVE_FLOAT, (const float *)buf);
    else if ((size_t)*sizeof_val == sizeof(double))
      ret =
          H5LT_set_attribute_numerical(c_loc_id, c_name, c_attrname, c_size,
                                       H5T_NATIVE_DOUBLE, (const double *)buf);
...
int_f h5ltget_attribute_c(hid_t_f *loc_id, size_t_f *namelen, _fcd dsetname,
  ...
  if (strncmp(dtype, "I", 1) == 0) {
    if ((size_t)*sizeof_val == sizeof(int))
      ret =
          H5LTget_attribute(c_loc_id, c_name, c_attrname, H5T_NATIVE_INT, buf);
    else if ((size_t)*sizeof_val == sizeof(long))
      ret =
          H5LTget_attribute(c_loc_id, c_name, c_attrname, H5T_NATIVE_LONG, buf);
#if H5_SIZEOF_LONG != H5_SIZEOF_LONG_LONG
    else if ((size_t)*sizeof_val == sizeof(long long))
      ret = H5LTget_attribute(c_loc_id, c_name, c_attrname, H5T_NATIVE_LLONG,
                              buf);
#endif /* H5_SIZEOF_LONG != H5_SIZEOF_LONG_LONG */
    else
      goto done;
    ...
    if ((size_t)*sizeof_val == sizeof(float))
      ret = H5LTget_attribute(c_loc_id, c_name, c_attrname, H5T_NATIVE_FLOAT,
                              buf);
    else if ((size_t)*sizeof_val == sizeof(double))
      ret = H5LTget_attribute(c_loc_id, c_name, c_attrname, H5T_NATIVE_DOUBLE,
                              buf);
    else if ((size_t)*sizeof_val == sizeof(long double))
      ret = H5LTget_attribute(c_loc_id, c_name, c_attrname, H5T_NATIVE_LDOUBLE,
                              buf);
    else
      goto done;
  }
...
int_f h5ltget_dataset_info_c(hid_t_f *loc_id, size_t_f *namelen, _fcd name,
  ...
  ret = H5LTget_dataset_info(c_loc_id, c_name, c_dims, &c_classtype,
                             &c_type_size);
  if (ret < 0)
    goto done;
...
int_f h5ltget_attribute_info_c(hid_t_f *loc_id, size_t_f *namelen, _fcd name,
  ...
  ret = H5LTget_attribute_info(c_loc_id, c_name, c_attrname, c_dims,
                               &c_classtype, &c_type_size);
  if (ret < 0)
    goto done;
