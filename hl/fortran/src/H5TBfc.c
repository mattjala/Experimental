    ...
    char *field_names)                               /* field_names */
{
   ...
   */
  if (NULL ==
      (c_field_names = (char **)calloc((size_t)num_elem, sizeof(char *))))
    HGOTO_DONE(FAIL);

  /* copy data to long C string */
  if (NULL == (tmp = (char *)HD5f2cstring(
                   field_names,
                   (size_t) * (max_char_size_field_names) * (size_t)num_elem)))
    HGOTO_DONE(FAIL);
  /*
  ...
  tmp_p = tmp;
  for (i = 0; i < num_elem; i++) {
    if (NULL == (c_field_names[i] =
                     (char *)malloc((size_t)char_len_field_names[i] + 1)))
      HGOTO_DONE(FAIL);
   ...
   */
  if (H5TBmake_table(
          c_name1, (hid_t)*loc_id, c_name, c_nfields, (hsize_t)*nrecords,
          (size_t)*type_size, (const char **)c_field_names, c_field_offset,
          c_field_types, (hsize_t)*chunk_size, NULL, *compress, NULL) < 0)
    HGOTO_DONE(FAIL);
    ...
    char *field_names, void *data)       /* field_names */
{
  ...
  if (H5TBmake_table(
          c_name1, (hid_t)*loc_id, c_name, c_nfields, (hsize_t)*nrecords,
          (size_t)*type_size, (const char **)c_field_names, c_field_offset,
          c_field_types, (hsize_t)*chunk_size, fill_data, *compress, data) < 0)
    HGOTO_DONE(FAIL);
...
int_f h5tbread_table_c(hid_t_f *loc_id, _fcd name, size_t_f *namelen,
  ...
  if (NULL ==
      (c_dst_offset = (size_t *)malloc(sizeof(size_t) * (size_t)c_nfields)))
    HGOTO_DONE(FAIL);
  if (NULL ==
      (c_dst_sizes = (size_t *)malloc(sizeof(size_t) * (size_t)c_nfields)))
    HGOTO_DONE(FAIL);
...
int_f h5tbwrite_field_name_c(hid_t_f *loc_id, size_t_f *namelen, _fcd name,
  ...
  if (H5TBwrite_fields_name((hid_t)*loc_id, c_name, c_name1, (hsize_t)*start,
                            (hsize_t)*nrecords, c_type_size[0], 0, c_type_size,
                            buf) < 0)
    HGOTO_DONE(FAIL);
...
int_f h5tbwrite_field_index_c(hid_t_f *loc_id, size_t_f *namelen, _fcd name,
  ...
  if (H5TBwrite_fields_index((hid_t)*loc_id, c_name, (hsize_t)1, &c_field_index,
                             (hsize_t)*start, (hsize_t)*nrecords, c_type_size,
                             0, &c_type_size, buf) < 0)
    HGOTO_DONE(FAIL);
...
int_f h5tbread_field_index_c(hid_t_f *loc_id, size_t_f *namelen, _fcd name,
  ...
  if (H5TBread_fields_index((hid_t)*loc_id, c_name, (hsize_t)1, &c_field_index,
                            (hsize_t)*start, (hsize_t)*nrecords, c_type_size, 0,
                            &c_type_size, buf) < 0)
    HGOTO_DONE(FAIL);
...

{
  ...
  if (NULL ==
    ...
    HGOTO_DONE(FAIL);
  if (NULL ==
      (c_field_names = (char **)calloc((size_t)c_nfields, sizeof(char *))))
    HGOTO_DONE(FAIL);

  for (i = 0; i < c_nfields; i++)
    if (NULL ==
        (c_field_names[i] = (char *)malloc(sizeof(char) * HLTB_MAX_FIELD_LEN)))
      HGOTO_DONE(FAIL);
