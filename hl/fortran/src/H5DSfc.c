...
int_f h5dsset_scale_c(hid_t_f *dsid, _fcd dimname, size_t_f *dimnamelen) {
  ...
  if (*dimnamelen != 0)
    if (NULL ==
        (c_dimname = (char *)HD5f2cstring(dimname, (size_t)*dimnamelen)))
      HGOTO_DONE(FAIL);
...
int_f h5dsis_attached_c(hid_t_f *did, hid_t_f *dsid, int_f *idx,
  ...
  if ((c_is_attached =
           H5DSis_attached((hid_t)*did, (hid_t)*dsid, (unsigned)*idx)) < 0)
    HGOTO_DONE(FAIL);
 ...
 *-------------------------------------------------------------------------
 */
int_f h5dsset_label_c(hid_t_f *did, int_f *idx, _fcd label,
                      size_t_f *labellen) {
  char *c_label = NULL;
  int_f ret_value = 0;
...
int_f h5dsget_label_c(hid_t_f *did, int_f *idx, _fcd label, size_t_f *size) {
  ...
  if ((size_c = H5DSget_label((hid_t)*did, (unsigned)*idx, c_label,
                              (size_t)*size + 1)) < 0)
    HGOTO_DONE(FAIL);
  ...
  *size = (size_t_f)
      size_c; /* Don't subtract '1'  because H5DSget_label doesn't include the
 ...
 *-------------------------------------------------------------------------
 */
int_f h5dsget_scale_name_c(hid_t_f *did, _fcd name, size_t_f *size) {
  ...
  if ((size_c = H5DSget_scale_name((hid_t)*did, c_scale_name,
                                   (size_t)*size + 1)) < 0)
    HGOTO_DONE(FAIL);
...
int_f h5dsget_num_scales_c(hid_t_f *did, int_f *idx, int_f *num_scales) {
  ...
  if ((*num_scales = (int_f)H5DSget_num_scales((hid_t)*did, (unsigned)*idx)) <
      0)
    HGOTO_DONE(FAIL);
