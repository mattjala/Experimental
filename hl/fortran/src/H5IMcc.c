 ...
 *-------------------------------------------------------------------------
 */
herr_t H5IM_get_palette(hid_t loc_id, const char *image_name, int pal_number,
                        hid_t tid, void *pal_data);
...
herr_t H5IMmake_image_8bitf(hid_t loc_id, const char *dset_name, hsize_t width,
                            hsize_t height, int_f *buf) {
  ...
  if ((did = H5Dcreate2(loc_id, dset_name, H5T_NATIVE_UINT8, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    return -1;

  /* write with memory type H5T_NATIVE_INT */
  /* Use long type if Fortran integer is 8 bytes and C long long is also 8
   * bytes*/
  /* Fail if otherwise */
  if (buf) {
    if (sizeof(int_f) == sizeof(int)) {
      ...
      if (H5Dwrite(did, H5T_NATIVE_LONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) <
          0)
        return -1;
    } else if (sizeof(int_f) == sizeof(long long)) {
      if (H5Dwrite(did, H5T_NATIVE_LLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) <
          0)
        return -1;
    } else
      return -1;
  ...
  if (H5LTset_attribute_string(loc_id, dset_name, "IMAGE_VERSION",
                               IMAGE_VERSION) < 0)
    return -1;

  /* attach the IMAGE_SUBCLASS attribute */
  if (H5LTset_attribute_string(loc_id, dset_name, "IMAGE_SUBCLASS",
                               "IMAGE_INDEXED") < 0)
    return -1;
...
herr_t H5IMmake_image_24bitf(hid_t loc_id, const char *dset_name, hsize_t width,
  ...
  /* create the dataset as H5T_NATIVE_UCHAR */
  if ((did = H5Dcreate2(loc_id, dset_name, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    return -1;
  ...
  /* Attach the INTERLACE_MODE attribute. This attributes is only for true color
   * images */
  if (H5LTset_attribute_string(loc_id, dset_name, "INTERLACE_MODE", interlace) <
      0)
    return -1;
...
herr_t H5IMmake_palettef(hid_t loc_id, const char *pal_name,
                         const hsize_t *pal_dims, int_f *pal_data)

{
  ...
  /* create the dataset as H5T_NATIVE_UCHAR */
  if ((did = H5Dcreate2(loc_id, pal_name, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    return -1;
  ...
  if (pal_data) {
    if (sizeof(int_f) == sizeof(int)) {
      ...
      if (H5Dwrite(did, H5T_NATIVE_LONG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   pal_data) < 0)
        return -1;
    } else if (sizeof(int_f) == sizeof(long long)) {
      if (H5Dwrite(did, H5T_NATIVE_LLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   pal_data) < 0)
        return -1;
    } else
      return -1;
...
herr_t H5IMget_palettef(hid_t loc_id, const char *image_name, int pal_number,
                        int_f *pal_data) {
  if (sizeof(int_f) == sizeof(int))
    return H5IM_get_palette(loc_id, image_name, pal_number, H5T_NATIVE_INT,
                            pal_data);
  else if (sizeof(int_f) == sizeof(long))
    return H5IM_get_palette(loc_id, image_name, pal_number, H5T_NATIVE_LONG,
                            pal_data);
  else if (sizeof(int_f) == sizeof(long long))
    return H5IM_get_palette(loc_id, image_name, pal_number, H5T_NATIVE_LLONG,
                            pal_data);
  else
    return -1;
}
...
herr_t H5IM_get_palette(hid_t loc_id, const char *image_name, int pal_number,
  ...
  if (has_pal == 1) {
    ...
    if (attr_class == H5T_REFERENCE) {
      ...
      /* Get the palette id */
      if ((pal_id = H5Rdereference2(image_id, H5P_DEFAULT, H5R_OBJECT,
                                    &refbuf[pal_number])) < 0)
        goto out;
