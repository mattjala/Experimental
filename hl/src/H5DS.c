 ...
 *-------------------------------------------------------------------------
 */
herr_t H5DSwith_new_ref(hid_t obj_id, hbool_t *with_new_ref) {
...
herr_t H5DSattach_scale(hid_t did, hid_t dsid, unsigned int idx) {
  ...
  ds_list_t *dsbuf =
      NULL; /* array of attribute data in the DS pointing to the dataset */
  ds_list_t *dsbuf_w = NULL; /* array of "REFERENCE_LIST" attribute data to
                                write when adding new reference to a dataset */
  ...
  if (has_dimlist == 0) {
    ...
    if ((aid = H5Acreate2(did, DIMENSION_LIST, tid, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto out;
   ...
   */
    ...
    for (i = 0; i < (int)buf[idx].len; i++) {
      ...
      if (is_new_ref) {
        ...
        /* get the scale id for this REF */
        if ((dsid_j = H5Rdereference2(did, H5P_DEFAULT, H5R_OBJECT, &ref_j)) <
            0)
          goto out;
      }
  ...
  if (has_reflist == 0) {
    ...
    if (is_new_ref) {
      ...
      if (H5Tinsert(tid, "dataset", HOFFSET(nds_list_t, ref), H5T_STD_REF) < 0)
        goto out;
      if (H5Tinsert(tid, "dimension", HOFFSET(nds_list_t, dim_idx),
                    H5T_NATIVE_UINT) < 0)
        goto out;
    } else {
      if ((tid = H5Tcreate(H5T_COMPOUND, sizeof(ds_list_t))) < 0)
        goto out;
      if (H5Tinsert(tid, "dataset", HOFFSET(ds_list_t, ref), H5T_STD_REF_OBJ) <
          0)
        goto out;
      if (H5Tinsert(tid, "dimension", HOFFSET(ds_list_t, dim_idx),
                    H5T_NATIVE_UINT) < 0)
        goto out;
    }

    /* create the attribute */
    if ((aid = H5Acreate2(dsid, REFERENCE_LIST, tid, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto out;
   ...
   */
    ...
    /* Recreate the references we read from the existing "REFERENCE_LIST"
     * attribute */
    ...
    /* the attribute must be deleted, in order to the new one can reflect the
     * changes*/
    ...
    if ((aid = H5Acreate2(dsid, REFERENCE_LIST, tid, sid_w, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto out;
 ...
 * Note that a scale may be associated with more than dimension of the same
 *dataset. If so, the detach operation only deletes one of the associations, for
 *DID.
 *
 *-------------------------------------------------------------------------
 */
herr_t H5DSdetach_scale(hid_t did, hid_t dsid, unsigned int idx) {
  ...
  nds_list_t *ndsbuf =
      NULL; /* array of attribute data in the DS pointing to the dataset */
  nds_list_t *ndsbuf_w = NULL; /* array of attribute data in the DS pointing to
                                  the dataset to write*/
  ds_list_t *dsbuf =
      NULL; /* array of attribute data in the DS pointing to the dataset */
  ds_list_t *dsbuf_w = NULL; /* array of attribute data in the DS pointing to
                                the dataset to write*/
  hsize_t dims[1];           /* dimension of the "REFERENCE_LIST" array */
  H5R_ref_t nref;
        ...
        if (!token_cmp) {
          ...
          if (j < len - 1) {
            if (is_new_ref) {
              ...
              ((hobj_ref_t *)buf[idx].p)[j] =
                  ((hobj_ref_t *)buf[idx].p)[len - 1];
            }
          }
          len = --buf[idx].len;
          if (len == 0) {
 ...
 *  dimension IDX of dataset DID by checking if DID has a pointer in the
 *REFERENCE_LIST attribute and DSID (scale ) has a pointer in the DIMENSION_LIST
 *attribute
 *
 * Return:
 ...
 *
 * Purpose: H5DSiterate_scales iterates over the scales attached to dimension
 *DIM of dataset DID. For each scale in the list, the visitor_data and some
 ...
 *  int *DS_IDX;             IN/OUT: on input the dimension scale index to start
 *iterating, on output the next index to visit. If NULL, start at the first
 *position. H5DS_iterate_t VISITOR;  IN: the visitor function void
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5DSiterate_scales(hid_t did, unsigned int dim, int *ds_idx,
    ...
    if (buf[dim].len > 0) {
      ...
      for (i = j_idx; i < nscales; i++) {
        if (is_new_ref) {
          ...
          H5E_BEGIN_TRY {
            /* get the DS id */
            if ((scale_id = H5Ropen_object(&nref, H5P_DEFAULT, H5P_DEFAULT)) <
                0)
              goto out;
          }
          H5E_END_TRY
          ...
          H5E_BEGIN_TRY {
            /* get the DS id */
            if ((scale_id =
                     H5Rdereference2(did, H5P_DEFAULT, H5R_OBJECT, &ref)) < 0)
              goto out;
          }
          H5E_END_TRY
        }
...
herr_t H5DSset_label(hid_t did, unsigned int idx, const char *label) {
  ...
  if (has_labels == 0) {
    ...
    /* create the attribute */
    if ((aid = H5Acreate2(did, DIMENSION_LABELS, tid, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto out;
...
htri_t H5DSis_scale(hid_t did) {
  ...
  if (attr_class == 0) {
    ...
    /* compare strings */
    if (strncmp(buf, DIMENSION_SCALE_CLASS,
                MIN(strlen(DIMENSION_SCALE_CLASS), strlen(buf))) == 0)
      is_ds = 1;
...
static herr_t H5DS_is_reserved(hid_t did, bool *is_reserved) {
  ...
  if (strncmp(buf, IMAGE_CLASS, MIN(strlen(IMAGE_CLASS), strlen(buf))) == 0 ||
      strncmp(buf, PALETTE_CLASS, MIN(strlen(PALETTE_CLASS), strlen(buf))) ==
          0 ||
