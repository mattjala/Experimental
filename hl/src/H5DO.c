 ...
 *
 * Note:
 * 	This routine is copied from the fast forward feature branch:
 *features/hdf5_ff src/H5FF.c:H5DOappend() with the following modifications: 1)
 *Remove and replace macro calls such as FUNC_ENTER_API, H5TRACE, HGOTO_ERROR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5DOappend(hid_t dset_id, hid_t dxpl_id, unsigned axis, size_t extension,
  ...
  hsize_t stride[H5S_MAX_RANK]; /* H5Sselect_hyperslab: # of elements to move
                                   when selecting */
  hsize_t
      block[H5S_MAX_RANK]; /* H5Sselect_hyperslab: # of elements in a block */

  hsize_t *boundary = NULL; /* Boundary set in append flush property */
  H5D_append_cb_t
      append_cb;           /* Callback function set in append flush property */
  ...
  if (FAIL == H5Sselect_hyperslab(new_space_id, H5S_SELECT_SET, start, stride,
                                  count, block))
    goto done;
