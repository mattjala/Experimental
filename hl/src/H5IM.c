...
herr_t H5IMmake_image_8bit(hid_t loc_id, const char *dset_name, hsize_t width,
                           hsize_t height, const unsigned char *buf) {
  ...
  /* Make the dataset */
  if (H5LTmake_dataset(loc_id, dset_name, IMAGE8_RANK, dims, H5T_NATIVE_UCHAR,
                       buf) < 0)
    return -1;
...
herr_t H5IMmake_image_24bit(hid_t loc_id, const char *dset_name, hsize_t width,
  ...
  /* Make the dataset */
  if (H5LTmake_dataset(loc_id, dset_name, IMAGE24_RANK, dims, H5T_NATIVE_UCHAR,
                       buf) < 0)
    return -1;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t find_palette(H5_ATTR_UNUSED hid_t loc_id, const char *name,
                           H5_ATTR_UNUSED const H5A_info_t *ainfo,
...
herr_t H5IM_find_palette(hid_t loc_id) {
  return H5Aiterate2(loc_id, H5_INDEX_NAME, H5_ITER_INC, NULL, find_palette,
                     NULL);
}
...
herr_t H5IMget_image_info(hid_t loc_id, const char *dset_name, hsize_t *width,
                          hsize_t *height, hsize_t *planes, char *interlace,
                          hssize_t *npals) {
  hid_t did = -1;
  hid_t sid = -1;
  hsize_t dims[IMAGE24_RANK];
...
herr_t H5IMmake_palette(hid_t loc_id, const char *pal_name,
...

{
  ...
  if (H5LTmake_dataset(loc_id, pal_name, 2, pal_dims, H5T_NATIVE_UCHAR,
                       pal_data) < 0)
    return -1;
...
herr_t H5IMlink_palette(hid_t loc_id, const char *image_name,
                        const char *pal_name)

{
  ...
  if (ok_pal == 0) {
    ...
    if ((aid = H5Acreate2(did, "PALETTE", atid, asid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto out;
...
herr_t H5IMunlink_palette(hid_t loc_id, const char *image_name,
                          const char *pal_name) {
  hid_t did;
  hid_t atid;
  hid_t aid;
...
herr_t H5IMget_npalettes(hid_t loc_id, const char *image_name,
                         hssize_t *npals) {
  hid_t did;
  hid_t atid;
  hid_t aid;
...
herr_t H5IMget_palette_info(hid_t loc_id, const char *image_name,
  ...
  if (has_pal == 1) {
    ...
    if ((pal_id = H5Rdereference2(did, H5P_DEFAULT, H5R_OBJECT,
                                  &refbuf[pal_number])) < 0)
      goto out;
...
herr_t H5IMget_palette(hid_t loc_id, const char *image_name, int pal_number,
  ...
  if (has_pal == 1) {
    ...
    if (H5Dread(pal_id, H5Dget_type(pal_id), H5S_ALL, H5S_ALL, H5P_DEFAULT,
                pal_data) < 0)
      goto out;
...
herr_t H5IMis_image(hid_t loc_id, const char *dset_name) {
  ...
  if (has_class == 0) {
    ...
    if (strncmp(attr_data, IMAGE_CLASS,
                MIN(strlen(IMAGE_CLASS), strlen(attr_data))) == 0)
      ret = 1;
    else
      ret = 0;
...
herr_t H5IMis_palette(hid_t loc_id, const char *dset_name) {
  ...
  if (has_class == 0) {
    ...
    if (strncmp(attr_data, PALETTE_CLASS,
                MIN(strlen(PALETTE_CLASS), strlen(attr_data))) == 0)
      ret = 1;
    else
      ret = 0;
