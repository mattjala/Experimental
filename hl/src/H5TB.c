...
static herr_t H5TB_attach_attributes(const char *table_title, hid_t loc_id,
                                     const char *dset_name, hsize_t nfields,
                                     hid_t tid);

static hid_t H5TB_create_type(hid_t loc_id, const char *dset_name,
                              size_t type_size, const size_t *field_offset,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBmake_table(const char *table_title, hid_t loc_id,
  ...
  /* insert fields. */
  for (i = 0; i < nfields; i++)
    if (H5Tinsert(mem_type_id, field_names[i], field_offset[i],
                  field_types[i]) < 0)
      goto out;
  ...
  */
  ...
  /* create the dataset. */
  if ((did = H5Dcreate2(loc_id, dset_name, mem_type_id, sid, H5P_DEFAULT,
                        plist_id, H5P_DEFAULT)) < 0)
    goto out;
  ...
  if (fill_data) {
    ...
    for (i = 0; i < nfields; i++) {
      ...
      if ((attr_id = H5Acreate2(did, attr_name, field_types[i], sid,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBappend_records(hid_t loc_id, const char *dset_name, hsize_t nrecords,
                          size_t type_size, const size_t *field_offset,
  ...
  if ((mem_type_id = H5TB_create_type(loc_id, dset_name, type_size,
                                      field_offset, field_sizes, tid)) < 0)
    goto out;

  /* append the records */
  if ((H5TB_common_append_records(did, mem_type_id, (size_t)nrecords,
                                  nrecords_orig, buf)) < 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBwrite_records(hid_t loc_id, const char *dset_name, hsize_t start,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBwrite_fields_name(hid_t loc_id, const char *dset_name,
                             const char *field_names, hsize_t start,
                             hsize_t nrecords, size_t type_size,
  ...
  for (i = 0; i < nfields; i++) {
    ...
    if (H5TB_find_field(member_name, field_names)) {
      ...
      if (field_offset) {
        if (H5Tinsert(write_type_id, member_name, field_offset[j], nmtype_id) <
            0)
          goto out;
      } /* end if */
      /* only one field */
  ...
  offset[0] = start;
  count[0] = nrecords;
  if (H5Sselect_hyperslab(file_space_id, H5S_SELECT_SET, offset, NULL, count,
                          NULL) < 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBwrite_fields_index(hid_t loc_id, const char *dset_name,
                              hsize_t nfields, const int *field_index,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBread_table(hid_t loc_id, const char *dset_name, size_t type_size,
                      const size_t *field_offset, const size_t *field_sizes,
                      void *dst_buf) {
  hid_t did = H5I_INVALID_HID;
  ...
  if ((mem_type_id = H5TB_create_type(loc_id, dset_name, type_size,
                                      field_offset, field_sizes, ftype_id)) < 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBread_records(hid_t loc_id, const char *dset_name, hsize_t start,
                        hsize_t nrecords, size_t type_size,
                        const size_t *field_offset, const size_t *field_sizes,
                        void *buf) {
  hid_t did = H5I_INVALID_HID;
  ...
  /* read the records */
  if ((H5TB_common_read_records(did, mem_type_id, start, (size_t)nrecords,
                                nrecords_orig, buf)) < 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBread_fields_name(hid_t loc_id, const char *dset_name,
                            const char *field_names, hsize_t start,
                            hsize_t nrecords, size_t type_size,
                            const size_t *field_offset,
  ...
  /* check to make sure field was found, no reason to continue if it does not
   * exist */
  if (j == 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBread_fields_index(hid_t loc_id, const char *dset_name,
                             hsize_t nfields, const int *field_index,
                             hsize_t start, hsize_t nrecords, size_t type_size,
                             const size_t *field_offset,
                             const size_t *field_sizes, void *buf) {
 ...
 *
 * Purpose: Delete records from middle of table ("pulling up" all the records
 *after it)
 *
 * Return: Success: 0, Failure: -1
 *
 *-------------------------------------------------------------------------
 */
herr_t H5TBdelete_record(hid_t loc_id, const char *dset_name, hsize_t start,
                         hsize_t nrecords) {
  hid_t did = H5I_INVALID_HID;
  hid_t tid = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  ...
  if (read_nrecords) {
    ...
    /* create the memory data type. */
    if ((mem_type_id = H5TB_create_type(loc_id, dset_name, src_size, src_offset,
                                        src_sizes, tid)) < 0)
      goto out;
 ...
 *
 * Purpose: Inserts records into middle of table ("pushing down" all the records
 *after it)
 *
 * Return: Success: 0, Failure: -1
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBinsert_record(hid_t loc_id, const char *dset_name, hsize_t start,
                         hsize_t nrecords, size_t type_size,
                         const size_t *field_offset, const size_t *field_sizes,
                         void *buf) {
  hid_t did = H5I_INVALID_HID;
  ...
  /* read the records after the inserted one(s) */
  if (H5TBread_records(loc_id, dset_name, start, read_nrecords, type_size,
                       field_offset, field_sizes, tmp_buf) < 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBadd_records_from(hid_t loc_id, const char *dset_name1,
                            hsize_t start1, hsize_t nrecords,
  ...
  /* get field info */
  if (H5TBget_field_info(loc_id, dset_name1, NULL, src_sizes, src_offset,
                         &src_size) < 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBcombine_tables(hid_t loc_id1, const char *dset_name1, hid_t loc_id2,
  ...
  /* get field info */
  if (H5TBget_field_info(loc_id1, dset_name1, NULL, src_sizes, src_offset,
                         &src_size) < 0)
    goto out;
  ...
  /*-------------------------------------------------------------------------
   * here we do not clone the file space from the 1st dataset, because we want
   *to create an empty table. Instead we create a new dataspace with zero
   *records and expandable.
   *-------------------------------------------------------------------------
   */
  dims[0] = 0;
  ...
  /* create the dataset */
  if ((did_3 = H5Dcreate2(loc_id1, dset_name3, tid_3, sid_3, H5P_DEFAULT, pid_3,
                          H5P_DEFAULT)) < 0)
    goto out;
   ...
   *-------------------------------------------------------------------------
   */
  if (H5TB_attach_attributes("Merge table", loc_id1, dset_name3, nfields,
                             tid_3) < 0)
    goto out;
  ...
  if (has_fill) {
    ...
    for (i = 0; i < nfields; i++) {
      ...
      if ((attr_id = H5Acreate2(did_3, attr_name, member_type_id, sid,
                                H5P_DEFAULT, H5P_DEFAULT)) < 0)
        goto out;
  ...
  if (H5TBappend_records(loc_id1, dset_name3, nrecords, src_size, src_offset,
                         src_sizes, tmp_buf) < 0)
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBinsert_field(hid_t loc_id, const char *dset_name,
                        const char *field_name, hid_t field_type,
                        hsize_t position, const void *fill_data,
                        const void *buf) {
  ...
  /* create the dataset. */
  if ((did_2 = H5Dcreate2(loc_id, "new", tid_2, sid_2, H5P_DEFAULT, pid_2,
                          H5P_DEFAULT)) < 0)
    goto out;
   ...
   *-------------------------------------------------------------------------
   */
  if (H5Lmove(loc_id, "new", H5L_SAME_LOC, dset_name, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    goto out;
  ...
  /* set the attributes */
  if (H5TB_attach_attributes(table_title, loc_id, dset_name, (hsize_t)nfields,
                             tid_3) < 0)
    goto out;
  ...
  for (i = 0; i < (nfields - 1); i++) {
    ...
    if ((attr_id = H5Acreate2(did_3, attr_name, member_type_id, sid_3,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0)
      goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBdelete_field(hid_t loc_id, const char *dset_name,
  ...
  for (i = 0; i < nfields; i++) {
    ...
    if (!H5TB_find_field(member_name, field_name)) {
      ...
      /* get it */
      if (has_fill > 0)
        if (H5LT_get_attribute_disk(did_1, attr_name,
                                    tmp_fill_buf + curr_offset) < 0)
          goto out;
  ...
  /*-------------------------------------------------------------------------
   * we have to read field by field of the old dataset and save it into the new
   *one
   *-------------------------------------------------------------------------
   */
  for (i = 0; i < nfields; i++) {
    ...
    if (!H5TB_find_field(member_name, field_name)) {
      ...
      /* read */
      if (H5Dread(did_1, read_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, tmp_buf) <
          0)
        goto out;
      ...
      /* write */
      if (H5Dwrite(did_2, write_type_id, H5S_ALL, H5S_ALL, preserve_id,
                   tmp_buf) < 0)
        goto out;
  ...
  /* set the attributes */
  if (H5TB_attach_attributes(table_title, loc_id, dset_name, nfields, tid_3) <
      0)
    goto out;
  ...
  if (has_fill > 0) {
    ...
    for (i = 0; i < nfields; i++) {
      ...
      if ((attr_id = H5Acreate2(did_3, attr_name, member_type_id, sid_1,
                                H5P_DEFAULT, H5P_DEFAULT)) < 0)
        goto out;
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5TBAget_fill(hid_t loc_id, const char *dset_name, hid_t dset_id,
  ...
  for (i = 0; i < nfields; i++) {
    ...
    /* get it */
    if (has_fill > 0)
      if (H5LT_get_attribute_disk(dset_id, attr_name, dst_buf + src_offset[i]) <
          0)
        goto out;
  }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBget_table_info(hid_t loc_id, const char *dset_name, hsize_t *nfields,
                          hsize_t *nrecords) {
  hid_t tid = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  hid_t did = H5I_INVALID_HID;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5TBget_field_info(hid_t loc_id, const char *dset_name,
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE static bool H5TB_find_field(const char *field,
  ...
  while ((end = strstr(start, ",")) != 0) {
    ...
    if (strncmp(start, field, (size_t)count) == 0 &&
        (size_t)count == strlen(field))
      return true;
    start = end + 1;
  } /* end while */
 ...
 *          Used by H5TBcombine_tables; not used by H5TBmake_table, which does
 *not read the fill value attributes from an existing table
 *
 * Return: Success: 0, Failure: -1
 *
 * Comments:
