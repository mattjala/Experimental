...
typedef struct {
  ...
  hsize_t current_index; /* The index of the packet that get_next_packet will
                            read next */
...
hid_t H5PTcreate(hid_t loc_id, const char *dset_name, hid_t dtype_id,
  ...
  if (H5PT_ptable_id_type < 0)
    if ((H5PT_ptable_id_type = H5Iregister_type((size_t)H5PT_HASH_TABLE_SIZE, 0,
                                                (H5I_free_t)H5PT_free_id)) < 0)
      goto error;
  ...
  /* Create the dataset. */
  if ((dset_id = H5Dcreate2(loc_id, dset_name, dtype_id, space_id, H5P_DEFAULT,
                            plistcopy_id, H5P_DEFAULT)) < 0)
    goto error;
...
herr_t H5PTclose(hid_t table_id) {
  ...
  if ((table = (htbl_t *)H5Iremove_verify(table_id, H5PT_ptable_id_type)) ==
      NULL)
    goto error;
...
herr_t H5PTappend(hid_t table_id, size_t nrecords, const void *data) {
  ...
  if ((table = (htbl_t *)H5Iobject_verify(table_id, H5PT_ptable_id_type)) ==
      NULL)
    goto error;
  ...
  if ((H5TB_common_append_records(table->dset_id, table->type_id, nrecords,
                                  table->size, data)) < 0)
    goto error;
...
herr_t H5PTget_next(hid_t table_id, size_t nrecords, void *data) {
  ...
  if ((H5TB_common_read_records(table->dset_id, table->type_id,
                                table->current_index, nrecords, table->size,
                                data)) < 0)
    goto error;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PTread_packets(hid_t table_id, hsize_t start, size_t nrecords,
                        void *data) {
  htbl_t *table;
  ...
  if (H5TB_common_read_records(table->dset_id, table->type_id, start, nrecords,
                               table->size, data) < 0)
    goto error;
...
herr_t H5PTcreate_index(hid_t table_id) {
  ...
  /* find the table struct from its ID */
  if ((table = (htbl_t *)(htbl_t *)H5Iobject_verify(
           table_id, H5PT_ptable_id_type)) == NULL)
    return FAIL;
