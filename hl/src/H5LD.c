 ...
 *-------------------------------------------------------------------------
 */
int H5LD_construct_vector(char *fields, H5LD_memb_t *listv[] /*OUT*/,
                          hid_t par_tid) {
  ...
  while (!end_of_fields) {
    H5LD_memb_t *memb =
        NULL;              /* Pointer to structure for storing a field's info */
    ...
    /* Continue processing till: not valid or comma encountered or "fields"
     * ended */
...
static herr_t H5LD_get_dset_elmts(hid_t did, const hsize_t *prev_dims,
  ...
  hssize_t
      snum_elmts;    /* Number of dataset elements in the selection (signed) */
  ...
  /* Verify that cur_dims must have one dimension whose size is greater than
   * prev_dims */
  ...
  if (ctr == 1) { /* changes for only one dimension */
    ...
    /* Make the selection in the dataset based on "cur_dims" and "prev_dims" */
    if (H5Sselect_hyperslab(sid, H5S_SELECT_SET, start, NULL, cur_dims, NULL) <
        0)
      goto done;
    if (H5Sselect_hyperslab(sid, H5S_SELECT_NOTB, start, NULL, prev_dims,
                            NULL) < 0)
      goto done;
  } /* end else */
  ...
  if (fields == NULL) { /* nothing in "fields" */
    ...
    unsigned char *buf_p =
        (unsigned char *)buf; /* Pointer to the destination buffer */
    ...
    if (NULL ==
        (sav_buf = tmp_buf = (char *)calloc((size_t)num_elmts, tot_tsize)))
      goto done;
