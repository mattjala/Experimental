...
static void *image_malloc(size_t size, H5FD_file_image_op_t file_image_op,
                          void *udata);
static void *image_memcpy(void *dest, const void *src, size_t size,
                          H5FD_file_image_op_t file_image_op, void *udata);
static void *image_realloc(void *ptr, size_t size,
                           H5FD_file_image_op_t file_image_op, void *udata);
static herr_t image_free(void *ptr, H5FD_file_image_op_t file_image_op,
                         void *udata);
...
static void *image_malloc(size_t size, H5FD_file_image_op_t file_image_op,
  ...
  /* the app buffer is "copied" to only one FAPL. Afterwards, FAPLs can be
   * "copied" */
 ...
 */
static void *image_memcpy(void *dest, const void *src, size_t size,
                          H5FD_file_image_op_t file_image_op, void *_udata) {
  H5LT_file_image_ud_t *udata = (H5LT_file_image_ud_t *)_udata;
...
static herr_t image_free(void *ptr, H5FD_file_image_op_t file_image_op,
  ...
  case H5FD_FILE_IMAGE_OP_PROPERTY_LIST_CLOSE:
    ...
    /* release the shared buffer only if indicated by the respective flag and
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5LT_get_attribute_mem(hid_t loc_id, const char *obj_name,
...
static herr_t H5LT_make_dataset_numerical(hid_t loc_id, const char *dset_name,
...
herr_t H5LTmake_dataset(hid_t loc_id, const char *dset_name, int rank,
  ...
  return (
      H5LT_make_dataset_numerical(loc_id, dset_name, rank, dims, tid, data));
}
...
herr_t H5LTmake_dataset_char(hid_t loc_id, const char *dset_name, int rank,
                             const hsize_t *dims, const char *data) {
  return (H5LT_make_dataset_numerical(loc_id, dset_name, rank, dims,
...
herr_t H5LTmake_dataset_short(hid_t loc_id, const char *dset_name, int rank,
                              const hsize_t *dims, const short *data) {
  return (H5LT_make_dataset_numerical(loc_id, dset_name, rank, dims,
...
herr_t H5LTmake_dataset_int(hid_t loc_id, const char *dset_name, int rank,
                            const hsize_t *dims, const int *data) {
  return (H5LT_make_dataset_numerical(loc_id, dset_name, rank, dims,
                                      H5T_NATIVE_INT, data));
}
...
herr_t H5LTmake_dataset_long(hid_t loc_id, const char *dset_name, int rank,
                             const hsize_t *dims, const long *data) {
  return (H5LT_make_dataset_numerical(loc_id, dset_name, rank, dims,
...
hid_t H5LTopen_file_image(void *buf_ptr, size_t buf_size, unsigned flags) {
  ...
  H5FD_file_image_callbacks_t callbacks = {
      &image_malloc, &image_memcpy, &image_realloc, &image_free,
  ...
  if (flags & H5LT_FILE_IMAGE_DONT_COPY) {
    ...
    if (NULL ==
        (udata = (H5LT_file_image_ud_t *)malloc(sizeof(H5LT_file_image_ud_t))))
      goto out;

    /* Initialize udata with info about app buffer containing file image  and
     * flags */
  ...
  snprintf(file_name, (sizeof(file_name) - 1), "file_image_%ld",
           file_name_counter++);
...
static herr_t H5LT_read_dataset_numerical(hid_t loc_id, const char *dset_name,
                                          hid_t tid, void *data) {
  hid_t did;
...
herr_t H5LTread_dataset(hid_t loc_id, const char *dset_name, hid_t tid,
                        void *data) {
...
herr_t H5LTread_dataset_char(hid_t loc_id, const char *dset_name, char *data) {
  return (
      H5LT_read_dataset_numerical(loc_id, dset_name, H5T_NATIVE_CHAR, data));
}
...
herr_t H5LTread_dataset_short(hid_t loc_id, const char *dset_name,
                              short *data) {
  return (
      H5LT_read_dataset_numerical(loc_id, dset_name, H5T_NATIVE_SHORT, data));
}
...
herr_t H5LTread_dataset_long(hid_t loc_id, const char *dset_name, long *data) {
  return (
      H5LT_read_dataset_numerical(loc_id, dset_name, H5T_NATIVE_LONG, data));
}
...
herr_t H5LTread_dataset_float(hid_t loc_id, const char *dset_name,
                              float *data) {
  return (
      H5LT_read_dataset_numerical(loc_id, dset_name, H5T_NATIVE_FLOAT, data));
}
...
herr_t H5LTread_dataset_double(hid_t loc_id, const char *dset_name,
                               double *data) {
  return (
      H5LT_read_dataset_numerical(loc_id, dset_name, H5T_NATIVE_DOUBLE, data));
}
...
static herr_t find_dataset(H5_ATTR_UNUSED hid_t loc_id, const char *name,
                           H5_ATTR_UNUSED const H5L_info2_t *linfo,
                           void *op_data) {
  /* Define a default zero value for return. This will cause the iterator to
   * continue if the dataset is not found yet.
   */
  int ret = 0;
  ...
  /* Define a positive value for return value if the dataset was found. This
...
herr_t H5LTfind_dataset(hid_t loc_id, const char *dset_name) {
  return H5Literate2(loc_id, H5_INDEX_NAME, H5_ITER_INC, 0, find_dataset,
                     (void *)dset_name);
}
...
herr_t H5LTset_attribute_string(hid_t loc_id, const char *obj_name,
  ...
  if ((attr_id = H5Acreate2(obj_id, attr_name, attr_type, attr_space_id,
 ...
 *
 * Purpose: Private function used by H5LTset_attribute_int and
...
herr_t H5LT_set_attribute_numerical(hid_t loc_id, const char *obj_name,
                                    const char *attr_name, size_t size,
                                    hid_t tid, const void *data) {
  ...
  /* Create the attribute. */
  if ((attr_id = H5Acreate2(obj_id, attr_name, tid, sid, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    goto out;
...
herr_t H5LTset_attribute_uchar(hid_t loc_id, const char *obj_name,
                               const char *attr_name, const unsigned char *data,
                               size_t size) {
...
herr_t H5LTset_attribute_short(hid_t loc_id, const char *obj_name,
                               const char *attr_name, const short *data,
                               size_t size) {
...
herr_t H5LTset_attribute_int(hid_t loc_id, const char *obj_name,
                             const char *attr_name, const int *data,
                             size_t size) {
...
herr_t H5LTset_attribute_uint(hid_t loc_id, const char *obj_name,
                              const char *attr_name, const unsigned int *data,
                              size_t size) {
...
herr_t H5LTset_attribute_long(hid_t loc_id, const char *obj_name,
                              const char *attr_name, const long *data,
                              size_t size) {
...
herr_t H5LTset_attribute_long_long(hid_t loc_id, const char *obj_name,
                                   const char *attr_name, const long long *data,
                                   size_t size) {
...
herr_t H5LTset_attribute_ulong(hid_t loc_id, const char *obj_name,
                               const char *attr_name, const unsigned long *data,
                               size_t size) {
...
herr_t H5LTset_attribute_float(hid_t loc_id, const char *obj_name,
                               const char *attr_name, const float *data,
                               size_t size) {
...
herr_t H5LTset_attribute_double(hid_t loc_id, const char *obj_name,
                                const char *attr_name, const double *data,
                                size_t size) {
 ...
 *-------------------------------------------------------------------------
 */
static char *realloc_and_append(bool _no_user_buf, size_t *len, char *buf,
  ...
  if (_no_user_buf) {
    ...
    /* If the buffer isn't big enough, reallocate it.  Otherwise, go to do
     * strcat. */
    if (str_to_add &&
        ((ssize_t)(*len - (strlen(buf) + strlen(str_to_add) + 1)) < LIMIT)) {
      *len +=
          ((strlen(buf) + strlen(str_to_add) + 1) / INCREMENT + 1) * INCREMENT;
 ...
 *
 *-----------------------------------------------------------------------*/
static char *print_enum(hid_t type, char *str, size_t *str_len, bool no_ubuf,
                        size_t indt) {
  ...
  for (i = 0; i < nmembs; i++) {
    if ((name[i] = H5Tget_member_name(type, (unsigned)i)) == NULL)
      goto out;
    if (H5Tget_member_value(type, (unsigned)i, value + (size_t)i * super_size) <
        0)
      goto out;
  }
  ...
  for (i = 0; i < nmembs; i++) {
    ...
    if (H5T_SGN_NONE == H5Tget_sign(native))
      snprintf(tmp_str, TMP_LEN, "%u",
               *((unsigned int *)((void *)(value + (size_t)i * dst_size))));
    else
      snprintf(tmp_str, TMP_LEN, "%d",
               *((int *)((void *)(value + (size_t)i * dst_size))));
    if (!(str = realloc_and_append(no_ubuf, str_len, str, tmp_str)))
      goto out;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5LTdtype_to_text(hid_t dtype, char *str, H5LT_lang_t lang_type,
                         size_t *len) {
  ...
  if (len && !str) {
    ...
    text_str[0] = '\0';
    if (!(text_str =
              H5LT_dtype_to_text(dtype, text_str, lang_type, &str_len, 1)))
      goto out;
...
herr_t H5LTget_attribute_char(hid_t loc_id, const char *obj_name,
                              const char *attr_name, char *data) {
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_CHAR,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_uchar(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_UCHAR,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_short(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_SHORT,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_ushort(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_USHORT,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_int(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_INT,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_uint(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_UINT,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_long(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_LONG,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_long_long(hid_t loc_id, const char *obj_name,
                                   const char *attr_name, long long *data) {
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_LLONG,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_ulong(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_ULONG,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_ullong(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_ULLONG,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_float(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_FLOAT,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute_double(hid_t loc_id, const char *obj_name,
  ...
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, H5T_NATIVE_DOUBLE,
                             data) < 0)
    return -1;
...
herr_t H5LTget_attribute(hid_t loc_id, const char *obj_name,
                         const char *attr_name, hid_t mem_type_id, void *data) {
  /* Get the attribute */
  if (H5LT_get_attribute_mem(loc_id, obj_name, attr_name, mem_type_id, data) <
      0)
    return -1;
...
herr_t H5LT_get_attribute_disk(hid_t loc_id, const char *attr_name,
                               void *attr_out) {
  /* identifiers */
  hid_t attr_id;
  hid_t attr_type;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5LT_set_attribute_string(hid_t dset_id, const char *name,
                                 const char *buf) {
  hid_t tid;
  hid_t sid = -1;
  hid_t aid = -1;
...
htri_t H5LTpath_valid(hid_t loc_id, const char *path,
  ...
  if ((link_exists = H5Lexists(loc_id, tmp_path, H5P_DEFAULT)) < 0) {
    ...
    /* Determine if link resolves to an actual object for check_object_valid
     * true */
