 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__hdr_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
  ...
  if (cls->crt_dbg_ctx)
    ...
    if (NULL == (dbg_ctx = cls->crt_dbg_ctx(f, obj_addr)))
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTGET, FAIL,
                  "unable to create fixed array debugging context");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__iblock_debug(H5F_t *f, haddr_t H5_ATTR_UNUSED addr, FILE *stream,
                          int indent, int fwidth, const H5EA_class_t *cls,
  ...
  if (cls->crt_dbg_ctx)
    ...
    if (NULL == (dbg_ctx = cls->crt_dbg_ctx(f, obj_addr)))
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTGET, FAIL,
                  "unable to create extensible array debugging context");

  /* Load the extensible array header */
  if (NULL ==
      (hdr = H5EA__hdr_protect(f, hdr_addr, dbg_ctx, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, FAIL,
                "unable to load extensible array header");
  ...
  if (hdr->cparam.idx_blk_elmts > 0) {
    ...
    for (u = 0; u < hdr->cparam.idx_blk_elmts; u++) {
      ...
      if ((hdr->cparam.cls->debug)(
              stream, (indent + 3), MAX(0, (fwidth - 3)), (hsize_t)u,
              ((uint8_t *)iblock->elmts) +
                  (hdr->cparam.cls->nat_elmt_size * u)) < 0)
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTGET, FAIL,
                    "can't get element for debugging");
    } /* end for */
  }   /* end if */
  ...
  if (iblock->ndblk_addrs > 0) {
    ...
    for (u = 0; u < iblock->ndblk_addrs; u++) {
      ...
      fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", (indent + 3), "",
              MAX(0, (fwidth - 3)), temp_str, iblock->dblk_addrs[u]);
    } /* end for */
  }   /* end if */
  ...
  if (iblock->nsblk_addrs > 0) {
    ...
    for (u = 0; u < iblock->nsblk_addrs; u++) {
      ...
      HDsnprintf(temp_str, sizeof(temp_str), "Address #%u:", u);
      fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", (indent + 3), "",
              MAX(0, (fwidth - 3)), temp_str, iblock->sblk_addrs[u]);
    } /* end for */
  }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__sblock_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                          int fwidth, const H5EA_class_t *cls, haddr_t hdr_addr,
                          unsigned sblk_idx, haddr_t obj_addr) {
  /* Local variables */
  H5EA_hdr_t *hdr = NULL;       /* Shared extensible array header */
  ...
  /* Protect super block */
  /* (Note: setting parent of super block to 'hdr' for this operation should be
   * OK -QAK) */
  if (NULL == (sblock = H5EA__sblock_protect(hdr, (H5EA_iblock_t *)hdr, addr,
                                             sblk_idx, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(
        H5E_EARRAY, H5E_CANTPROTECT, FAIL,
  ...
  if (sblock->ndblks > 0) {
    ...
    for (u = 0; u < sblock->ndblks; u++) {
      ...
      HDsnprintf(temp_str, sizeof(temp_str), "Address #%u:", u);
      fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", (indent + 3), "",
              MAX(0, (fwidth - 3)), temp_str, sblock->dblk_addrs[u]);
    } /* end for */
  }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__dblock_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                          int fwidth, const H5EA_class_t *cls, haddr_t hdr_addr,
                          size_t dblk_nelmts, haddr_t obj_addr) {
  /* Local variables */
  H5EA_hdr_t *hdr = NULL;       /* Shared extensible array header */
  ...
  /* Protect data block */
  /* (Note: setting parent of data block to 'hdr' for this operation should be
   * OK -QAK) */
  if (NULL == (dblock = H5EA__dblock_protect(hdr, hdr, addr, dblk_nelmts,
                                             H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(
        H5E_EARRAY, H5E_CANTPROTECT, FAIL,
        "unable to protect extensible array data block, address = %" PRIuHADDR,
        addr);
