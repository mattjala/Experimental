 ...
 *-------------------------------------------------------------------------
 */
H5EA_sblock_t *H5EA__sblock_alloc(H5EA_hdr_t *hdr, H5EA_iblock_t *parent,
                                  unsigned sblk_idx) {
  ...
  if (sblock->dblk_nelmts > hdr->dblk_page_nelmts) {
    ...
    assert((sblock->dblk_npages * hdr->dblk_page_nelmts) ==
           sblock->dblk_nelmts);
    ...
    if (NULL == (sblock->page_init = H5FL_BLK_CALLOC(
                     page_init, sblock->ndblks * sblock->dblk_page_init_size)))
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL,
                  "memory allocation failed for super block page init bitmask");

    /* Compute data block page size */
    sblock->dblk_page_size =
        (hdr->dblk_page_nelmts * hdr->cparam.raw_elmt_size) +
        H5EA_SIZEOF_CHKSUM;
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5EA__sblock_create(H5EA_hdr_t *hdr, H5EA_iblock_t *parent,
  ...
  if (HADDR_UNDEF == (sblock_addr = H5MF_alloc(hdr->f, H5FD_MEM_EARRAY_SBLOCK,
                                               (hsize_t)sblock->size)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF,
  ...
  H5VM_array_fill(sblock->dblk_addrs, &tmp_addr, sizeof(haddr_t),
                  sblock->ndblks);

  /* Cache the new extensible array super block */
  if (H5AC_insert_entry(hdr->f, H5AC_EARRAY_SBLOCK, sblock_addr, sblock,
 ...
 *
 * Purpose:	Convenience wrapper around protecting extensible array super
 *block
 *
 ...
 *-------------------------------------------------------------------------
 */
H5EA_sblock_t *H5EA__sblock_protect(H5EA_hdr_t *hdr, H5EA_iblock_t *parent,
  ...
  if (NULL == (sblock = (H5EA_sblock_t *)H5AC_protect(
                   hdr->f, H5AC_EARRAY_SBLOCK, sblk_addr, &udata, flags)))
    HGOTO_ERROR(
 ...
 *
 * Purpose:	Convenience wrapper around unprotecting extensible array super
 *block
 *
...
herr_t H5EA__sblock_unprotect(H5EA_sblock_t *sblock, unsigned cache_flags) {
  ...
  if (H5AC_unprotect(sblock->hdr->f, H5AC_EARRAY_SBLOCK, sblock->addr, sblock,
                     cache_flags) < 0)
    HGOTO_ERROR(
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__sblock_delete(H5EA_hdr_t *hdr, H5EA_iblock_t *parent,
                           haddr_t sblk_addr, unsigned sblk_idx) {
  H5EA_sblock_t *sblock = NULL; /* Pointer to super block */
  ...
  /* Protect super block */
  if (NULL == (sblock = H5EA__sblock_protect(hdr, parent, sblk_addr, sblk_idx,
                                             H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(
  ...
  for (u = 0; u < sblock->ndblks; u++) {
    ...
    if (H5_addr_defined(sblock->dblk_addrs[u])) {
      /* Delete data block */
      if (H5EA__dblock_delete(hdr, sblock, sblock->dblk_addrs[u],
                              sblock->dblk_nelmts) < 0)
  ...
  if (sblock &&
      H5EA__sblock_unprotect(sblock, H5AC__DIRTIED_FLAG | H5AC__DELETED_FLAG |
