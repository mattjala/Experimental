...
/* Data exchange structure to use when building table of compact attributes for
 * an object */
typedef struct {
  ...
  hbool_t
      bogus_crt_idx; /* Whether bogus creation index values need to be set */
} H5A_compact_bt_ud_t;

/* Data exchange structure to use when building table of dense attributes for an
 * object */
typedef struct {
  H5A_attr_table_t *atable; /* Pointer to attribute table to build */
  size_t curr_attr;         /* Current attribute to operate on */
...
static herr_t H5A__compact_build_table_cb(H5O_t *oh,
                                          H5O_mesg_t *mesg /*in,out*/,
                                          unsigned sequence,
...
static herr_t H5A__attr_sort_table(H5A_attr_table_t *atable,
 ...
 *-------------------------------------------------------------------------
 */
H5A_t *H5A__create(const H5G_loc_t *loc, const char *attr_name,
  ...
  if (!(H5S_has_extent(space)))
    HGOTO_ERROR(H5E_ATTR, H5E_BADVALUE, NULL,
  ...
  if (NULL == (attr = H5FL_CALLOC(H5A_t)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, NULL,
                "memory allocation failed for attribute info");

  if (NULL == (attr->shared = H5FL_CALLOC(H5A_shared_t)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, NULL,
                "can't allocate shared attr structure");

  /* If the creation property list is H5P_ATTRIBUTE_CREATE_DEFAULT, use the
  ...
  if (acpl_id == H5P_ATTRIBUTE_CREATE_DEFAULT)
    ...
    if (H5P_get(ac_plist, H5P_STRCRT_CHAR_ENCODING_NAME,
                &(attr->shared->encoding)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get character encoding flag");
  } /* end else */
  ...
  if (H5T_set_loc(attr->shared->dt, H5F_VOL_OBJ(loc->oloc->file),
                  H5T_LOC_DISK) < 0)
  ...
  if (H5S_set_version(loc->oloc->file, attr->shared->ds) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, NULL,
                "can't set version of dataspace");
   ...
   * SOHM table
   */
  if (H5SM_try_share(attr->oloc.file, NULL, 0, H5O_DTYPE_ID, attr->shared->dt,
                     NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_BADMESG, NULL, "trying to share datatype failed");
  if (H5SM_try_share(attr->oloc.file, NULL, 0, H5O_SDSPACE_ID, attr->shared->ds,
                     NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_BADMESG, NULL,
                "trying to share dataspace failed");
  ...
  if (H5T_is_named(attr->shared->dt))
    ...
    if (H5T_link(attr->shared->dt, 1) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_LINKCOUNT, NULL,
                  "unable to adjust shared datatype link count");
   ...
   */
  attr->shared->dt_size =
      H5O_msg_raw_size(attr->oloc.file, H5O_DTYPE_ID, FALSE, attr->shared->dt);
  attr->shared->ds_size = H5O_msg_raw_size(attr->oloc.file, H5O_SDSPACE_ID,
                                           FALSE, attr->shared->ds);
  ...
  if (H5A__set_version(attr->oloc.file, attr) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, NULL,
                "unable to update attribute version");

  /* Insert the attribute into the object header */
  if (H5O__attr_create(&(attr->oloc), attr) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTINSERT, NULL,
 ...
 *-------------------------------------------------------------------------
 */
H5A_t *H5A__create_by_name(const H5G_loc_t *loc, const char *obj_name,
                           const char *attr_name, const H5T_t *type,
...
H5A_t *H5A__open(const H5G_loc_t *loc, const char *attr_name) {
  ...
  if (NULL == (attr = H5O__attr_open_by_name(loc->oloc, attr_name)))
    HGOTO_ERROR(
        H5E_ATTR, H5E_CANTOPENOBJ, NULL,
        "unable to load attribute info from object header for attribute: '%s'",
        attr_name);
 ...
 *-------------------------------------------------------------------------
 */
H5A_t *H5A__open_by_idx(const H5G_loc_t *loc, const char *obj_name,
 ...
 *-------------------------------------------------------------------------
 */
H5A_t *H5A__open_by_name(const H5G_loc_t *loc, const char *obj_name,
...
herr_t H5A__read(const H5A_t *attr, const H5T_t *mem_type, void *buf) {
    ...
    if (attr->obj_opened && !attr->shared->data)
      ...
      if (!H5T_path_noop(tpath)) {
        ...
        if ((src_id = H5I_register(H5I_DATATYPE,
                                   H5T_copy(attr->shared->dt, H5T_COPY_ALL),
                                   FALSE)) < 0 ||
            (dst_id = H5I_register(
                 H5I_DATATYPE, H5T_copy(mem_type, H5T_COPY_ALL), FALSE)) < 0)
        ...
        if (need_bkg) {
          ...
          if (NULL == (bkg_buf = H5FL_BLK_CALLOC(attr_buf, buf_size)))
            HGOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL,
        ...
        if (H5T_convert(tpath, src_id, dst_id, nelmts, (size_t)0, (size_t)0,
                        tconv_buf, bkg_buf) < 0)
  ...
  if (src_id >= 0 && H5I_dec_ref(src_id) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CANTDEC, FAIL,
...
herr_t H5A__write(H5A_t *attr, const H5T_t *mem_type, const void *buf) {
  ...
  /* If there's actually data elements for the attribute, make a copy of the
   * data passed in */
  if (nelmts > 0) {
    /* Get the memory and file datatype sizes */
    ...
    if (!H5T_path_noop(tpath)) {
      ...
      if (need_bkg) {
        /* Use the existing attribute data buffer, if present, as the background
         * buffer, otherwise allocate one.  Note we don't need to track which
         * one it is since both use the "attr_buf" free list block. */
        if (attr->shared->data) {
          bkg_buf = attr->shared->data;
          attr->shared->data = NULL;

          /* Clear background buffer if it's not supposed to be initialized with
           * file contents */
    ...
    /* No type conversion necessary */
      ...
      if (attr->shared->data == NULL)
        if (NULL == (attr->shared->data =
                         H5FL_BLK_MALLOC(attr_buf, dst_type_size * nelmts)))
...
--------------------------------------------------------------------------*/
herr_t H5A__get_name(H5A_t *attr, size_t buf_size, char *buf,
...
hid_t H5A_get_space(H5A_t *attr) {
  ...
  if (H5I_INVALID_HID == ret_value && ds && H5S_close(ds) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, H5I_INVALID_HID,
...
hid_t H5A__get_create_plist(H5A_t *attr) {
  ...
  if (NULL ==
      (plist = (H5P_genplist_t *)H5I_object(H5P_LST_ATTRIBUTE_CREATE_ID_g)))
  ...
  if ((new_plist_id = H5P_copy_plist(plist, TRUE)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
                "unable to copy attribute creation properties");
  if (NULL == (new_plist = (H5P_genplist_t *)H5I_object(new_plist_id)))
    HGOTO_ERROR(H5E_PLIST, H5E_BADTYPE, FAIL, "can't get property list");

  /* Set the character encoding on the new property list */
  if (H5P_set(new_plist, H5P_STRCRT_CHAR_ENCODING_NAME,
              &(attr->shared->encoding)) < 0)
...
herr_t H5A__shared_free(H5A_t *attr) {
  ...
  if (attr->shared->dt) {
    if (H5T_close_real(attr->shared->dt) < 0)
      HDONE_ERROR(H5E_ATTR, H5E_CANTRELEASE, FAIL,
...
herr_t H5A__close(H5A_t *attr) {
  ...
  if (attr->obj_opened && (H5O_close(&(attr->oloc), NULL) < 0))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTRELEASE, FAIL,
                "can't release object header info");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__exists_by_name(H5G_loc_t loc, const char *obj_name,
                           const char *attr_name, hbool_t *attr_exists) {
  H5G_loc_t obj_loc;          /* Location used to open group */
  H5G_name_t obj_path;        /* Opened object group hier. path */
  H5O_loc_t obj_oloc;         /* Opened object object location */
  hbool_t loc_found = FALSE;  /* Entry at 'obj_name' found */
  ...
  if (H5O__attr_exists(obj_loc.oloc, attr_name, attr_exists) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                "unable to determine if attribute exists");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5A__compact_build_table_cb(H5O_t H5_ATTR_UNUSED *oh,
                                          H5O_mesg_t *mesg /*in,out*/,
                                          unsigned sequence,
                                          unsigned H5_ATTR_UNUSED *oh_modified,
                                          void *_udata /*in,out*/) {
  H5A_compact_bt_ud_t *udata =
      (H5A_compact_bt_ud_t *)_udata; /* Operator user data */
  ...
  if (udata->curr_attr == udata->atable->nattrs) {
    ...
    if (NULL == (new_table = (H5A_t **)H5FL_SEQ_REALLOC(
                     H5A_t_ptr, udata->atable->attrs, new_table_size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, H5_ITER_ERROR,
                  "unable to extend attribute table");
  ...
  if (NULL == (udata->atable->attrs[udata->curr_attr] =
                   H5A__copy(NULL, (const H5A_t *)mesg->native)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__compact_build_table(H5F_t *f, H5O_t *oh, H5_index_t idx_type,
  ...
  udata.curr_attr = 0;
  udata.bogus_crt_idx =
      (hbool_t)((oh->version == H5O_VERSION_1 ||
  ...
  if (atable->nattrs > 0) {
    ...
    if (H5A__attr_sort_table(atable, idx_type, order) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTSORT, FAIL,
                  "error sorting attribute table");
  } /* end if */
...
static herr_t H5A__dense_build_table_cb(const H5A_t *attr, void *_udata) {
  H5A_dense_bt_ud_t *udata =
      (H5A_dense_bt_ud_t *)_udata; /* 'User data' passed in */
  herr_t ret_value = H5_ITER_CONT; /* Return value */
  ...
  if (NULL == (udata->atable->attrs[udata->curr_attr] = H5FL_CALLOC(H5A_t)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, H5_ITER_ERROR,
                "can't allocate attribute");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__dense_build_table(H5F_t *f, const H5O_ainfo_t *ainfo,
                              H5_index_t idx_type, H5_iter_order_t order,
                              H5A_attr_table_t *atable) {
  ...
  if (H5B2_get_nrec(bt2_name, &nrec) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                "can't retrieve # of records in index");
  ...
  if (atable->nattrs > 0) {
    ...
    if ((atable->attrs =
             (H5A_t **)H5FL_SEQ_CALLOC(H5A_t_ptr, atable->nattrs)) == NULL)
  ...
  if (bt2_name && H5B2_close(bt2_name) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, FAIL,
...
static herr_t H5A__attr_sort_table(H5A_attr_table_t *atable,
  ...
  if (idx_type == H5_INDEX_NAME) {
    if (order == H5_ITER_INC)
      qsort(atable->attrs, atable->nattrs, sizeof(H5A_t *),
            H5A__attr_cmp_name_inc);
    ...
    if (order == H5_ITER_INC)
      qsort(atable->attrs, atable->nattrs, sizeof(H5A_t *),
            H5A__attr_cmp_corder_inc);
    else if (order == H5_ITER_DEC)
      qsort(atable->attrs, atable->nattrs, sizeof(H5A_t *),
            H5A__attr_cmp_corder_dec);
    else
      assert(order == H5_ITER_NATIVE);
  } /* end else */
 ...
 *
 * Purpose:     Iterate over table containing a list of attributes for an
 *object, making appropriate callbacks
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
herr_t H5A__attr_iterate_table(const H5A_attr_table_t *atable, hsize_t skip,
                               hsize_t *last_attr, hid_t loc_id,
  ...
  for (; u < atable->nattrs && !ret_value; u++) {
    ...
    case H5A_ATTR_OP_APP2: {
      ...
      if (H5A__get_info(atable->attrs[u], &ainfo) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, H5_ITER_ERROR,
                    "unable to get attribute info");

      /* Make the application callback */
      ret_value = (attr_op->u.app_op2)(
          loc_id, ((atable->attrs[u])->shared)->name, &ainfo, op_data);
      break;
    }
    ...
    case H5A_ATTR_OP_APP:
      /* Make the application callback */
      ret_value = (attr_op->u.app_op)(
          loc_id, ((atable->attrs[u])->shared)->name, op_data);
      break;
  ...
  if (atable->nattrs > 0) {
    ...
    for (u = 0; u < atable->nattrs; u++)
      if (atable->attrs[u] && H5A__close(atable->attrs[u]) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTFREE, FAIL,
                    "unable to release attribute");
  } /* end if */
  else
...
herr_t H5A__set_version(const H5F_t *f, H5A_t *attr) {
  hbool_t type_shared, space_shared; /* Flags to indicate that shared messages
                                        are used for this attribute */
  ...
  if (attr->shared->encoding != H5T_CSET_ASCII)
    version = H5O_ATTR_VERSION_3; /* Write version which includes the character
                                     encoding */
  else if (type_shared || space_shared)
    version = H5O_ATTR_VERSION_2; /* Write out version with flag for indicating
                                     shared datatype or dataspace */
  else
    version = H5O_ATTR_VERSION_1; /* Write out basic version */

  /* Upgrade to the version indicated by the file's low bound if higher */
  version =
      (uint8_t)MAX(version, (uint8_t)H5O_attr_ver_bounds[H5F_LOW_BOUND(f)]);

  /* Version bounds check */
  if (version > H5O_attr_ver_bounds[H5F_HIGH_BOUND(f)])
    HGOTO_ERROR(H5E_ATTR, H5E_BADRANGE, FAIL,
                "attribute version out of bounds");
 ...
 *-------------------------------------------------------------------------
 */
H5A_t *H5A__attr_copy_file(const H5A_t *attr_src, H5F_t *file_dst,
  ...
  *attr_dst = *attr_src;
  ...
  if (H5T_set_loc(attr_dst->shared->dt, H5F_VOL_OBJ(file_dst), H5T_LOC_DISK) <
      0)
  ...
  if (!H5T_is_named(attr_src->shared->dt)) {
    ...
    if (H5O_msg_reset_share(H5O_DTYPE_ID, attr_dst->shared->dt) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, NULL,
                  "unable to reset datatype sharing");
  }

  /* Copy the dataspace for the attribute. Make sure the maximal dimension is
   * also copied. Otherwise the comparison in the test may complain about it.
   * SLU 2011/4/12 */
  ...
  if (H5O_msg_reset_share(H5O_SDSPACE_ID, attr_dst->shared->ds) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, NULL,
                "unable to reset dataspace sharing");
   ...
   */
  if (H5SM_try_share(file_dst, NULL, H5SM_DEFER, H5O_DTYPE_ID,
                     attr_dst->shared->dt, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, NULL,
                "can't share attribute datatype");
  if (H5SM_try_share(file_dst, NULL, H5SM_DEFER, H5O_SDSPACE_ID,
                     attr_dst->shared->ds, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, NULL,
                "can't share attribute dataspace");

  /* Compute the sizes of the datatype and dataspace. This is their raw
   * size unless they're shared.
   */
  attr_dst->shared->dt_size =
      H5O_msg_raw_size(file_dst, H5O_DTYPE_ID, FALSE, attr_dst->shared->dt);
  assert(attr_dst->shared->dt_size > 0);
  attr_dst->shared->ds_size =
      H5O_msg_raw_size(file_dst, H5O_SDSPACE_ID, FALSE, attr_dst->shared->ds);
  ...
  if (attr_src->shared->data) {
    if (NULL == (attr_dst->shared->data =
                     H5FL_BLK_MALLOC(attr_buf, attr_dst->shared->data_size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
    ...
    if (H5T_detect_class(attr_src->shared->dt, H5T_VLEN, FALSE) > 0) {
      ...
      if ((tid_src = H5I_register(H5I_DATATYPE, attr_src->shared->dt, FALSE)) <
          0)
      ...
      if ((tid_dst = H5I_register(H5I_DATATYPE, attr_dst->shared->dt, FALSE)) <
          0)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, NULL,
      ...
      if ((buf_sid = H5I_register(H5I_DATASPACE, buf_space, FALSE)) < 0) {
        H5S_close(buf_space);
        HGOTO_ERROR(H5E_ID, H5E_CANTREGISTER, NULL,
                    "unable to register dataspace ID");
      } /* end if */
      ...
      if (NULL == (reclaim_buf = H5FL_BLK_MALLOC(attr_buf, buf_size)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                    "memory allocation NULLed for raw data chunk");
      ...
      if (H5T_convert(tpath_src_mem, tid_src, tid_mem, nelmts, (size_t)0,
                      (size_t)0, buf, bkg_buf) < 0)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, NULL,
                    "datatype conversion NULLed");
      ...
      if (H5T_convert(tpath_mem_dst, tid_mem, tid_dst, nelmts, (size_t)0,
                      (size_t)0, buf, bkg_buf) < 0)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, NULL,
                    "datatype conversion NULLed");
  ...
  if (buf_sid > 0 && H5I_dec_ref(buf_sid) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CANTFREE, NULL,
...
herr_t H5A__attr_post_copy_file(const H5O_loc_t *src_oloc,
                                const H5A_t *attr_src, H5O_loc_t *dst_oloc,
  ...
  if (H5T_is_named(attr_src->shared->dt)) {
    ...
    if (H5O_copy_header_map(src_oloc_dt, dst_oloc_dt, cpy_info, FALSE, NULL,
                            NULL) < 0)
   ...
   */
  if (H5SM_try_share(file_dst, NULL, H5SM_WAS_DEFERRED, H5O_DTYPE_ID,
                     attr_dst->shared->dt, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, FAIL,
                "can't share attribute datatype");
  if (H5SM_try_share(file_dst, NULL, H5SM_WAS_DEFERRED, H5O_SDSPACE_ID,
                     attr_dst->shared->ds, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, FAIL,
                "can't share attribute dataspace");
   ...
   */
  if ((NULL != attr_dst->shared->data) &&
      (H5T_get_class(attr_dst->shared->dt, FALSE) == H5T_REFERENCE)) {
    ...
    if (cpy_info->expand_ref) {
      /* Copy objects referenced in source buffer to destination file and set
       * destination elements */
      if (H5O_copy_expand_ref(file_src, H5I_INVALID_HID, attr_src->shared->dt,
                              attr_src->shared->data,
                              attr_src->shared->data_size, file_dst,
                              attr_dst->shared->data, cpy_info) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTCOPY, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5A__dense_post_copy_file_cb(const H5A_t *attr_src,
                                           void *_udata) {
  ...
  if (NULL ==
      (attr_dst = H5A__attr_copy_file(attr_src, udata->file,
                                      udata->recompute_size, udata->cpy_info)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCOPY, H5_ITER_ERROR, "can't copy attribute");

  if (H5A__attr_post_copy_file(udata->oloc_src, attr_src, udata->oloc_dst,
                               attr_dst, udata->cpy_info) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCOPY, H5_ITER_ERROR, "can't copy attribute");

  /* Reset shared location information */
  if (H5O_msg_reset_share(H5O_ATTR_ID, attr_dst) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
                "unable to reset attribute sharing");
  ...
  if (H5A__dense_insert(udata->file, udata->ainfo, attr_dst) < 0)
    HGOTO_ERROR_TAG(H5E_OHDR, H5E_CANTINSERT, H5_ITER_ERROR,
                    "unable to add to dense storage");
  ...
  if (attr_dst && H5A__close(attr_dst) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, FAIL,
                "can't close destination attribute");
...
herr_t H5A__dense_post_copy_file_all(const H5O_loc_t *src_oloc,
  ...
  if (H5A__dense_iterate(src_oloc->file, (hid_t)0, ainfo_src, H5_INDEX_NAME,
                         H5_ITER_NATIVE, (hsize_t)0, NULL, &attr_op,
                         &udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__rename_by_name(H5G_loc_t loc, const char *obj_name,
...
static herr_t H5A__iterate_common(hid_t loc_id, H5_index_t idx_type,
  ...
  if ((ret_value = H5O__attr_iterate(loc_id, idx_type, order, start_idx,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__iterate(const H5G_loc_t *loc, const char *obj_name,
  ...
  if ((ret_value = H5A__iterate_common(obj_loc_id, idx_type, order, idx,
                                       &attr_op, op_data)) < 0)
    HERROR(H5E_ATTR, H5E_BADITER, "error iterating over attributes");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__iterate_old(hid_t loc_id, unsigned *attr_num, H5A_operator1_t op,
                        void *op_data) {
  H5A_attr_iter_op_t attr_op; /* Attribute operator */
  ...
  /* Call internal attribute iteration routine */
  if ((ret_value = H5A__iterate_common(loc_id, H5_INDEX_CRT_ORDER, H5_ITER_INC,
                                       &idx, &attr_op, op_data)) < 0)
    HERROR(H5E_ATTR, H5E_BADITER, "error iterating over attributes");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__delete_by_name(const H5G_loc_t *loc, const char *obj_name,
                           const char *attr_name) {
  H5G_loc_t obj_loc;          /* Location used to open group */
  H5G_name_t obj_path;        /* Opened object group hier. path */
  H5O_loc_t obj_oloc;         /* Opened object object location */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__delete_by_idx(const H5G_loc_t *loc, const char *obj_name,
                          H5_index_t idx_type, H5_iter_order_t order,
                          hsize_t n) {
  H5G_loc_t obj_loc;          /* Location used to open group */
  H5G_name_t obj_path;        /* Opened object group hier. path */
