...
typedef struct H5D_earray_it_ud_t {
  H5D_chunk_common_ud_t
      common; /* Common info for Fixed Array user data (must be first) */
...
static int H5D__earray_idx_iterate_cb(hsize_t idx, const void *_elmt,
                                      void *_udata);
static int H5D__earray_idx_delete_cb(const H5D_chunk_rec_t *chunk_rec,
                                     void *_udata);
...
static herr_t H5D__earray_filt_encode(void *raw, const void *elmt,
                                      size_t nelmts, void *ctx);
static herr_t H5D__earray_filt_decode(const void *raw, void *elmt,
                                      size_t nelmts, void *ctx);
...
static herr_t H5D__earray_idx_size(const H5D_chk_idx_info_t *idx_info,
                                   hsize_t *size);
static herr_t H5D__earray_idx_reset(H5O_storage_chunk_t *storage,
                                    hbool_t reset_addr);
...
static void *H5D__earray_crt_context(void *_udata) {
  ...
  H5D_earray_ctx_ud_t *udata = (H5D_earray_ctx_ud_t *)
      _udata;             /* User data for extensible array context */
   ...
   */
  ctx->chunk_size_len =
      1 + ((H5VM_log2_gen((uint64_t)udata->chunk_size) + 8) / 8);
...
static herr_t H5D__earray_dst_context(void *_ctx) {
  H5D_earray_ctx_t *ctx =
      (H5D_earray_ctx_t *)_ctx; /* Extensible array callback context */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__earray_encode(void *raw, const void *_elmt, size_t nelmts,
                                 void *_ctx) {
  H5D_earray_ctx_t *ctx =
      (H5D_earray_ctx_t *)_ctx; /* Extensible array callback context */
  const haddr_t *elmt =
      (const haddr_t *)_elmt; /* Convenience pointer to native elements */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__earray_decode(const void *_raw, void *_elmt, size_t nelmts,
                                 void *_ctx) {
  H5D_earray_ctx_t *ctx =
      (H5D_earray_ctx_t *)_ctx;     /* Extensible array callback context */
  haddr_t *elmt = (haddr_t *)_elmt; /* Convenience pointer to native elements */
  const uint8_t *raw =
      (const uint8_t *)_raw; /* Convenience pointer to raw elements */
 ...
 */
static herr_t H5D__earray_filt_fill(void *nat_blk, size_t nelmts) {
  H5D_earray_filt_elmt_t fill_val =
      H5D_EARRAY_FILT_FILL; /* Value to fill elements with */
  ...
  H5VM_array_fill(nat_blk, &fill_val, H5EA_CLS_FILT_CHUNK->nat_elmt_size,
                  nelmts);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__earray_filt_encode(void *_raw, const void *_elmt,
                                      size_t nelmts, void *_ctx) {
  H5D_earray_ctx_t *ctx =
      (H5D_earray_ctx_t *)_ctx;   /* Extensible array callback context */
  uint8_t *raw = (uint8_t *)_raw; /* Convenience pointer to raw elements */
  const H5D_earray_filt_elmt_t *elmt = (const H5D_earray_filt_elmt_t *)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__earray_filt_decode(const void *_raw, void *_elmt,
                                      size_t nelmts, void *_ctx) {
  H5D_earray_ctx_t *ctx =
      (H5D_earray_ctx_t *)_ctx; /* Extensible array callback context */
  H5D_earray_filt_elmt_t *elmt = (H5D_earray_filt_elmt_t *)
      _elmt; /* Convenience pointer to native elements */
  const uint8_t *raw =
      (const uint8_t *)_raw; /* Convenience pointer to raw elements */
...
static herr_t H5D__earray_filt_debug(FILE *stream, int indent, int fwidth,
  ...
  fprintf(stream, "%*s%-*s {%" PRIuHADDR ", %u, %0x}\n", indent, "", fwidth,
          temp_str, elmt->addr, elmt->nbytes, elmt->filter_mask);
...
static void *H5D__earray_crt_dbg_context(H5F_t *f, haddr_t obj_addr) {
  ...
  hbool_t obj_opened =
      FALSE;           /* Flag to indicate that the object header was opened */
  ...
  if (H5O_close(&obj_loc, NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, NULL,
                "can't close object header");
...
static herr_t H5D__earray_dst_dbg_context(void *_dbg_ctx) {
  H5D_earray_ctx_ud_t *dbg_ctx = (H5D_earray_ctx_ud_t *)
      _dbg_ctx; /* Context for extensible array callback */
...
static herr_t H5D__earray_idx_depend(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (NULL == (oh = H5O_protect(&oloc, H5AC__READ_ONLY_FLAG, TRUE)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTPROTECT, FAIL,
                "unable to protect object header");

  /* Retrieve the dataset's object header proxy */
  if (NULL == (oh_proxy = H5O_get_proxy(oh)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "unable to get dataset object header proxy");

  /* Make the extensible array a child flush dependency of the dataset's object
   * header */
  ...
  if (oh && H5O_unprotect(&oloc, oh, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTUNPROTECT, FAIL,
...
static herr_t H5D__earray_idx_open(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (NULL == (idx_info->storage->u.earray.ea =
                   H5EA_open(idx_info->f, idx_info->storage->idx_addr, &udata)))
...
static herr_t H5D__earray_idx_init(const H5D_chk_idx_info_t *idx_info,
  ...
  for (u = 0; u < ndims; u++) {
    ...
    if (H5S_UNLIMITED == max_dims[u]) {
      ...
      if (unlim_dim >= 0)
        HGOTO_ERROR(H5E_DATASET, H5E_ALREADYINIT, FAIL,
                    "already found unlimited dimension");
  ...
  if (unlim_dim < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_UNINITIALIZED, FAIL,
                "didn't find unlimited dimension");
...
static herr_t H5D__earray_idx_create(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (idx_info->pline->nused > 0) {
    ...
    cparam.raw_elmt_size =
        (uint8_t)(H5F_SIZEOF_ADDR(idx_info->f) + chunk_size_len + 4);
  } /* end if */
  else {
  ...
  cparam.data_blk_min_elmts =
      idx_info->layout->u.earray.cparam.data_blk_min_elmts;
  assert(cparam.data_blk_min_elmts > 0);
  cparam.max_dblk_page_nelmts_bits =
      idx_info->layout->u.earray.cparam.max_dblk_page_nelmts_bits;
  ...
  if (NULL == (idx_info->storage->u.earray.ea =
                   H5EA_create(idx_info->f, &cparam, &udata)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "can't create extensible array");

  /* Get the address of the extensible array in file */
  if (H5EA_get_addr(idx_info->storage->u.earray.ea,
                    &(idx_info->storage->idx_addr)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't query extensible array address");
...
static herr_t H5D__earray_idx_insert(const H5D_chk_idx_info_t *idx_info,
  ...
  if (!H5_addr_defined(udata->chunk_block.offset))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "The chunk should have allocated already");
  if (udata->chunk_idx != (udata->chunk_idx & 0xffffffff)) /* negative value */
    HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL,
                "chunk index must be less than 2^32");
  ...
  if (idx_info->pline->nused > 0) {
    ...
    H5_CHECKED_ASSIGN(elmt.nbytes, uint32_t, udata->chunk_block.length,
                      hsize_t);
...
static herr_t H5D__earray_idx_get_addr(const H5D_chk_idx_info_t *idx_info,
  ...
  if (idx_info->layout->u.earray.unlim_dim > 0) {
    ...
    H5VM_swizzle_coords(hsize_t, swizzled_coords,
                        idx_info->layout->u.earray.unlim_dim);

    /* Calculate the index of this chunk */
    idx = H5VM_chunk_index(ndims, swizzled_coords,
                           idx_info->layout->u.earray.swizzled_dim,
                           idx_info->layout->u.earray.swizzled_max_down_chunks);
  } /* end if */
  else {
    /* Calculate the index of this chunk */
    idx = H5VM_array_offset_pre((idx_info->layout->ndims - 1),
                                idx_info->layout->max_down_chunks,
                                udata->common.scaled);
  } /* end else */
...
static herr_t H5D__earray_idx_resize(H5O_layout_chunk_t *layout) {
  ...
  if (layout->u.earray.unlim_dim > 0) {
    hsize_t swizzled_chunks[H5O_LAYOUT_NDIMS]; /* Swizzled form of # of chunks
                                                  in each dimension */
    hsize_t
        swizzled_max_chunks[H5O_LAYOUT_NDIMS]; /* Swizzled form of max # of
                                                  chunks in each dimension */

    /* Get the swizzled chunk dimensions */
    H5MM_memcpy(layout->u.earray.swizzled_dim, layout->dim,
                (layout->ndims - 1) * sizeof(layout->dim[0]));
    H5VM_swizzle_coords(uint32_t, layout->u.earray.swizzled_dim,
                        layout->u.earray.unlim_dim);

    /* Get the swizzled number of chunks in each dimension */
    H5MM_memcpy(swizzled_chunks, layout->chunks,
                (layout->ndims - 1) * sizeof(swizzled_chunks[0]));
    H5VM_swizzle_coords(hsize_t, swizzled_chunks, layout->u.earray.unlim_dim);

    /* Get the swizzled "down" sizes for each dimension */
    H5VM_array_down((layout->ndims - 1), swizzled_chunks,
                    layout->u.earray.swizzled_down_chunks);
    ...
    H5VM_swizzle_coords(hsize_t, swizzled_max_chunks,
                        layout->u.earray.unlim_dim);

    /* Get the swizzled max "down" sizes for each dimension */
    H5VM_array_down((layout->ndims - 1), swizzled_max_chunks,
                    layout->u.earray.swizzled_max_down_chunks);
  }
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__earray_idx_iterate_cb(hsize_t H5_ATTR_UNUSED idx,
                                      const void *_elmt, void *_udata) {
  ...
  if (udata->filtered) {
    const H5D_earray_filt_elmt_t *filt_elmt =
        (const H5D_earray_filt_elmt_t *)_elmt;
  ...
  if (H5_addr_defined(udata->chunk_rec.chunk_addr))
    if ((ret_value = (udata->cb)(&udata->chunk_rec, udata->udata)) < 0)
      HERROR(H5E_DATASET, H5E_CALLBACK,
  ...
  while (curr_dim >= 0) {
    ...
    if (udata->chunk_rec.scaled[curr_dim] >=
        udata->common.layout->max_chunks[curr_dim]) {
...
static int H5D__earray_idx_iterate(const H5D_chk_idx_info_t *idx_info,
  ...
  if (H5EA_get_stats(ea, &ea_stat) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't query extensible array statistics");
  ...
  if (ea_stat.stored.max_idx_set > 0) {
    ...
    if ((ret_value = H5EA_iterate(ea, H5D__earray_idx_iterate_cb, &udata)) < 0)
      HERROR(H5E_DATASET, H5E_BADITER,
             "unable to iterate over fixed array chunk index");
  } /* end if */
...
static herr_t H5D__earray_idx_remove(const H5D_chk_idx_info_t *idx_info,
  ...
  if (NULL == idx_info->storage->u.earray.ea) {
    ...
    if (H5EA_patch_file(idx_info->storage->u.earray.ea, idx_info->f) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL,
                  "can't patch earray file pointer");
  ...
  if (idx_info->layout->u.earray.unlim_dim > 0) {
    ...
    /* Calculate the index of this chunk */
    idx =
        H5VM_array_offset_pre((idx_info->layout->ndims - 1),
                              idx_info->layout->max_down_chunks, udata->scaled);
  } /* end else */
  ...
  if (idx_info->pline->nused > 0) {
    ...
    if (!(H5F_INTENT(idx_info->f) & H5F_ACC_SWMR_WRITE)) {
      ...
      if (H5MF_xfree(idx_info->f, H5FD_MEM_DRAW, elmt.addr,
                     (hsize_t)elmt.nbytes) < 0)
    ...
    if (!(H5F_INTENT(idx_info->f) & H5F_ACC_SWMR_WRITE)) {
      H5_CHECK_OVERFLOW(idx_info->layout->size, /*From: */ uint32_t,
                        /*To: */ hsize_t);
      if (H5MF_xfree(idx_info->f, H5FD_MEM_DRAW, addr,
                     (hsize_t)idx_info->layout->size) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL, "unable to free chunk");
    } /* end if */
...
static int H5D__earray_idx_delete_cb(const H5D_chunk_rec_t *chunk_rec,
  ...
  if (H5MF_xfree(f, H5FD_MEM_DRAW, chunk_rec->chunk_addr,
                 (hsize_t)chunk_rec->nbytes) < 0)
...
static herr_t H5D__earray_idx_delete(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (H5_addr_defined(idx_info->storage->idx_addr)) {
    H5D_earray_ctx_ud_t
        ctx_udata; /* User data for extensible array open call */

    /* Iterate over the chunk addresses in the extensible array, deleting each
     * chunk */
    if (H5D__earray_idx_iterate(idx_info, H5D__earray_idx_delete_cb,
                                idx_info->f) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_BADITER, FAIL,
                  "unable to iterate over chunk addresses");

    /* Close extensible array */
    if (H5EA_close(idx_info->storage->u.earray.ea) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL,
                  "unable to close extensible array");
    ...
    if (H5EA_delete(idx_info->f, idx_info->storage->idx_addr, &ctx_udata) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTDELETE, FAIL,
                  "unable to delete chunk extensible array");
...
H5D__earray_idx_copy_setup(const H5D_chk_idx_info_t *idx_info_src,
  ...
  /* Create the extensible array that describes chunked storage in the dest.
   * file */
