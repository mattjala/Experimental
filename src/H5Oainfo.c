...
static void *H5O__ainfo_decode(H5F_t *f, H5O_t *open_oh, unsigned mesg_flags,
                               ...
                               const uint8_t *p);
static herr_t H5O__ainfo_encode(H5F_t *f, hbool_t disable_shared, uint8_t *p,
                                const void *_mesg);
static void *H5O__ainfo_copy(const void *_mesg, void *_dest);
static size_t H5O__ainfo_size(const H5F_t *f, hbool_t disable_shared,
                              const void *_mesg);
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__ainfo_decode(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh,
  ...
  /* Set the number of attributes on the object to an invalid value, so we query
   * it later */
  ...
  /* Address of v2 B-tree to index names of attributes (names are always
   * indexed) */
  if (H5_IS_BUFFER_OVERFLOW(p, sizeof_addr, p_end))
    HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                "ran off end of input buffer while decoding");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__ainfo_encode(H5F_t *f, hbool_t H5_ATTR_UNUSED disable_shared,
  ...
  *p++ = H5O_AINFO_VERSION;
  ...
  flags = (unsigned char)(flags |
                          (ainfo->index_corder ? H5O_AINFO_INDEX_CORDER : 0));
  *p++ = flags;
  ...
  /* Address of v2 B-tree to index creation order of attributes, if they are
   * indexed */
  if (ainfo->index_corder)
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__ainfo_size(const H5F_t *f,
  ...
  ret_value =
      (size_t)(1   /* Version */
               + 1 /* Index flags */
               + (ainfo->track_corder ? 2
                                      : 0) /* Curr. max. creation order value */
               +
               H5F_SIZEOF_ADDR(
                   f) /* Address of fractal heap to store "dense" attributes */
               + H5F_SIZEOF_ADDR(f) /* Address of v2 B-tree for indexing names
                                       of attributes */
               + (ainfo->index_corder
                      ? H5F_SIZEOF_ADDR(f)
                      : 0)); /* Address of v2 B-tree for indexing creation order
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__ainfo_delete(H5F_t *f, H5O_t H5_ATTR_NDEBUG_UNUSED *open_oh,
                                void *_mesg) {
  ...
  if (H5_addr_defined(ainfo->fheap_addr))
    ...
    if (H5A__dense_delete(f, ainfo) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTFREE, FAIL,
                  "unable to free dense attribute storage");
  ...
  *ainfo_dst = *ainfo_src;
  ...
  if (H5_addr_defined(ainfo_src->fheap_addr)) {
    ...
    if (H5A__dense_create(file_dst, ainfo_dst) < 0)
      HGOTO_ERROR_TAG(H5E_OHDR, H5E_CANTINIT, NULL,
...
static herr_t H5O__ainfo_post_copy_file(const H5O_loc_t *src_oloc,
  ...
  if (H5_addr_defined(ainfo_src->fheap_addr))
    if (H5A__dense_post_copy_file_all(src_oloc, ainfo_src, dst_oloc,
                                      (H5O_ainfo_t *)mesg_dst, cpy_info) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__ainfo_debug(H5F_t H5_ATTR_UNUSED *f, const void *_mesg,
  ...
  fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
          ...
          ainfo->track_corder ? "TRUE" : "FALSE");
  fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
          "Index creation order of attributes:",
          ainfo->index_corder ? "TRUE" : "FALSE");
  ...
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "'Dense' attribute storage name index v2 B-tree address:",
          ainfo->name_bt2_addr);
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "'Dense' attribute storage creation order index v2 B-tree address:",
          ainfo->corder_bt2_addr);
