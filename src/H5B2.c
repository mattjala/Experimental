...
H5B2_t *H5B2_create(H5F_t *f, const H5B2_create_t *cparam, void *ctx_udata) {
  ...
  if (NULL == (bt2 = H5FL_MALLOC(H5B2_t)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, NULL,
                "memory allocation failed for v2 B-tree info");

  /* Look up the B-tree header */
  if (NULL ==
      (hdr = H5B2__hdr_protect(f, hdr_addr, ctx_udata, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, NULL,
                "unable to protect v2 B-tree header");
  ...
  if (hdr && H5B2__hdr_unprotect(hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, NULL,
                "unable to release v2 B-tree header");
  if (!ret_value && bt2)
    if (H5B2_close(bt2) < 0)
      HDONE_ERROR(H5E_BTREE, H5E_CANTCLOSEOBJ, NULL,
                  "unable to close v2 B-tree");
...
H5B2_t *H5B2_open(H5F_t *f, haddr_t addr, void *ctx_udata) {
  ...
  /* Look up the B-tree header */
  if (NULL ==
      (hdr = H5B2__hdr_protect(f, addr, ctx_udata, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, NULL,
                "unable to protect v2 B-tree header");

  /* Check for pending heap deletion */
  if (hdr->pending_delete)
    HGOTO_ERROR(H5E_BTREE, H5E_CANTOPENOBJ, NULL,
                "can't open v2 B-tree pending deletion");
...
herr_t H5B2_insert(H5B2_t *bt2, void *udata) {
  ...
  if (H5B2__insert(hdr, udata) < 0)
    HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, FAIL,
                "unable to insert record into B-tree");
...
herr_t H5B2_update(H5B2_t *bt2, void *udata, H5B2_modify_t op, void *op_data) {
  ...
  H5B2_update_status_t status =
      H5B2_UPDATE_UNKNOWN;    /* Whether the record was inserted/modified */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (hdr->depth > 0) {
    if (H5B2__update_internal(hdr, hdr->depth, NULL, &hdr->root, &status,
                              H5B2_POS_ROOT, hdr, udata, op, op_data) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL,
                  "unable to update record in B-tree internal node");
  } /* end if */
  else {
    if (H5B2__update_leaf(hdr, &hdr->root, &status, H5B2_POS_ROOT, hdr, udata,
                          op, op_data) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTUPDATE, FAIL,
                  "unable to update record in B-tree leaf node");
  } /* end else */
  ...
  else if (H5B2_UPDATE_SHADOW_DONE == status ||
           H5B2_UPDATE_INSERT_DONE == status) {
...
herr_t H5B2_iterate(H5B2_t *bt2, H5B2_operator_t op, void *op_data) {
  ...
  if (hdr->root.node_nrec > 0)
    ...
    if ((ret_value = H5B2__iterate_node(hdr, hdr->depth, &hdr->root, hdr, op,
                                        op_data)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2_find(H5B2_t *bt2, void *udata, hbool_t *found, H5B2_found_t op,
                 void *op_data) {
  H5B2_hdr_t *hdr;               /* Pointer to the B-tree header */
  ...
  while (depth > 0) {
    ...
    if (H5B2__locate_record(hdr->cls, internal->nrec, hdr->nat_off,
      ...
      H5AC_unprotect(hdr->f, H5AC_BT2_INT, curr_node_ptr.addr, internal,
                     H5AC__NO_FLAGS_SET);
      HGOTO_ERROR(H5E_BTREE, H5E_CANTCOMPARE, FAIL,
                  "can't compare btree2 records");
    } /* end if */
...

  {
    ...
    if (H5B2__locate_record(hdr->cls, leaf->nrec, hdr->nat_off,
      ...
      /* Unlock current node before failing */
      H5AC_unprotect(hdr->f, H5AC_BT2_LEAF, curr_node_ptr.addr, leaf,
                     H5AC__NO_FLAGS_SET);
      HGOTO_ERROR(H5E_BTREE, H5E_CANTCOMPARE, FAIL,
                  "can't compare btree2 records");
    } /* end if */
      ...
      if (H5B2_POS_MIDDLE != curr_pos) {
        if (idx == 0) {
          if (H5B2_POS_LEFT == curr_pos || H5B2_POS_ROOT == curr_pos) {
            if (hdr->min_native_rec == NULL)
              if (NULL ==
                  (hdr->min_native_rec = H5MM_malloc(hdr->cls->nrec_size)))
                HGOTO_ERROR(
        ...
        if (idx == (unsigned)(leaf->nrec - 1)) {
          if (H5B2_POS_RIGHT == curr_pos || H5B2_POS_ROOT == curr_pos) {
            if (hdr->max_native_rec == NULL)
              if (NULL ==
                  (hdr->max_native_rec = H5MM_malloc(hdr->cls->nrec_size)))
                HGOTO_ERROR(
                    H5E_BTREE, H5E_CANTALLOC, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2_index(H5B2_t *bt2, H5_iter_order_t order, hsize_t idx,
  ...
  if (idx >= curr_node_ptr.all_nrec)
    HGOTO_ERROR(H5E_BTREE, H5E_NOTFOUND, FAIL,
                "B-tree doesn't have that many records");
  ...
  /* Check for reverse indexing and map requested index to appropriate forward
   * index */
...
herr_t H5B2_remove(H5B2_t *bt2, void *udata, H5B2_remove_t op, void *op_data) {
  ...
  if (hdr->depth > 0) {
    hbool_t depth_decreased =
        FALSE; /* Flag to indicate whether the depth of the B-tree decreased */

    if (H5B2__remove_internal(hdr, &depth_decreased, NULL, NULL, hdr->depth,
                              &(hdr->cache_info), NULL, H5B2_POS_ROOT,
                              &hdr->root, udata, op, op_data) < 0)
    ...
    if (H5B2__remove_leaf(hdr, &hdr->root, H5B2_POS_ROOT, hdr, udata, op,
                          op_data) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTDELETE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2_remove_by_idx(H5B2_t *bt2, H5_iter_order_t order, hsize_t idx,
  ...
  if (hdr->depth > 0) {
    ...
    if (H5B2__remove_internal_by_idx(
            hdr, &depth_decreased, NULL, NULL, hdr->depth, &(hdr->cache_info),
    ...
    if (H5B2__remove_leaf_by_idx(hdr, &hdr->root, H5B2_POS_ROOT, hdr,
                                 (unsigned)idx, op, op_data) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTDELETE, FAIL,
                  "unable to remove record from B-tree leaf node");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2_neighbor(H5B2_t *bt2, H5B2_compare_t range, void *udata,
                     H5B2_found_t op, void *op_data) {
  H5B2_hdr_t *hdr;            /* Pointer to the B-tree header */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (hdr->depth > 0) {
    if (H5B2__neighbor_internal(hdr, hdr->depth, &hdr->root, NULL, range, hdr,
                                udata, op, op_data) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_NOTFOUND, FAIL,
                  "unable to find neighbor record in B-tree internal node");
  } /* end if */
  else {
    if (H5B2__neighbor_leaf(hdr, &hdr->root, NULL, range, hdr, udata, op,
                            op_data) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_NOTFOUND, FAIL,
...
herr_t H5B2_close(H5B2_t *bt2) {
  ...
  hbool_t pending_delete =
      FALSE;                  /* Whether the v2 B-tree is pending deletion */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Decrement file reference & check if this is the last open v2 B-tree using
   * the shared B-tree header */
  ...
  if (pending_delete) {
...
#ifndef NDEBUG
    {
      ...
      if (H5AC_get_entry_status(bt2->f, bt2_addr, &hdr_status) < 0)
        HGOTO_ERROR(H5E_BTREE, H5E_CANTGET, FAIL,
                    "unable to check metadata cache status for v2 B-tree "
                    "header, address = %llu",
     ...
     * be in the cache) */
    if (NULL ==
        (hdr = H5B2__hdr_protect(bt2->f, bt2_addr, NULL, H5AC__NO_FLAGS_SET)))
      HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL,
                  "unable to protect v2 B-tree header");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2_delete(H5F_t *f, haddr_t addr, void *ctx_udata, H5B2_remove_t op,
                   void *op_data) {
  H5B2_hdr_t *hdr = NULL;     /* Pointer to the B-tree header */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (NULL == hdr->parent) {
    ...
    if (H5AC_proxy_entry_add_child(parent, hdr->f, hdr->top_proxy) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTSET, FAIL,
                  "unable to add v2 B-tree as child of proxy");
    hdr->parent = parent;
  } /* end if */
