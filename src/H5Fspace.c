 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5F__alloc(H5F_t *f, H5F_mem_t type, hsize_t size, haddr_t *frag_addr,
                   hsize_t *frag_size) {
  ...
  if (!H5_addr_defined(ret_value))
    HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, HADDR_UNDEF,
                "file driver 'alloc' request failed");

  /* Mark EOA dirty */
  if (H5F_eoa_dirty(f) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTMARKDIRTY, HADDR_UNDEF,
                "unable to mark EOA as dirty");
...
herr_t H5F__free(H5F_t *f, H5FD_mem_t type, haddr_t addr, hsize_t size) {
  ...
  if (H5FD_free(f->shared->lf, type, f, addr, size) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTFREE, FAIL,
                "file driver 'free' request failed");
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5F__try_extend(H5F_t *f, H5FD_mem_t type, haddr_t blk_end,
  ...
  if ((ret_value = H5FD_try_extend(f->shared->lf, type, f, blk_end,
                                   extra_requested)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTEXTEND, FAIL,
                "driver try extend request failed");
