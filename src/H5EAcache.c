...
static herr_t H5EA__cache_hdr_get_initial_load_size(void *udata,
                                                    size_t *image_len);
static htri_t H5EA__cache_hdr_verify_chksum(const void *image_ptr, size_t len,
                                            void *udata_ptr);
...
static herr_t H5EA__cache_iblock_get_initial_load_size(void *udata,
                                                       size_t *image_len);
static htri_t H5EA__cache_iblock_verify_chksum(const void *image_ptr,
...
static herr_t H5EA__cache_iblock_image_len(const void *thing,
                                           size_t *image_len);
static herr_t H5EA__cache_iblock_serialize(const H5F_t *f, void *image,
                                           size_t len, void *thing);
static herr_t H5EA__cache_iblock_notify(H5AC_notify_action_t action,
                                        void *thing);
static herr_t H5EA__cache_iblock_free_icr(void *thing);

static herr_t H5EA__cache_sblock_get_initial_load_size(void *udata,
                                                       size_t *image_len);
static htri_t H5EA__cache_sblock_verify_chksum(const void *image_ptr,
                                               size_t len, void *udata_ptr);
...
static herr_t H5EA__cache_sblock_image_len(const void *thing,
                                           size_t *image_len);
static herr_t H5EA__cache_sblock_serialize(const H5F_t *f, void *image,
                                           size_t len, void *thing);
static herr_t H5EA__cache_sblock_notify(H5AC_notify_action_t action,
                                        void *thing);
static herr_t H5EA__cache_sblock_free_icr(void *thing);

static herr_t H5EA__cache_dblock_get_initial_load_size(void *udata,
                                                       size_t *image_len);
static htri_t H5EA__cache_dblock_verify_chksum(const void *image_ptr,
                                               size_t len, void *udata_ptr);
...
static herr_t H5EA__cache_dblock_image_len(const void *thing,
                                           size_t *image_len);
static herr_t H5EA__cache_dblock_serialize(const H5F_t *f, void *image,
                                           size_t len, void *thing);
static herr_t H5EA__cache_dblock_notify(H5AC_notify_action_t action,
                                        void *thing);
...
static herr_t H5EA__cache_dblk_page_get_initial_load_size(void *udata,
                                                          size_t *image_len);
static htri_t H5EA__cache_dblk_page_verify_chksum(const void *image_ptr,
                                                  size_t len, void *udata_ptr);
...
static herr_t H5EA__cache_dblk_page_image_len(const void *thing,
                                              size_t *image_len);
static herr_t H5EA__cache_dblk_page_serialize(const H5F_t *f, void *image,
                                              size_t len, void *thing);
static herr_t H5EA__cache_dblk_page_notify(H5AC_notify_action_t action,
                                           void *thing);
...
const H5AC_class_t H5AC_EARRAY_IBLOCK[1] = {{
    ...
    H5AC__CLASS_NO_FLAGS_SET,       /* Client class behavior flags */
    H5EA__cache_iblock_get_initial_load_size, /* 'get_initial_load_size'
                                                 callback */
    NULL,                             /* 'get_final_load_size' callback */
...
const H5AC_class_t H5AC_EARRAY_SBLOCK[1] = {{
    ...
    H5AC__CLASS_NO_FLAGS_SET,       /* Client class behavior flags */
    H5EA__cache_sblock_get_initial_load_size, /* 'get_initial_load_size'
                                                 callback */
    NULL,                             /* 'get_final_load_size' callback */
...
const H5AC_class_t H5AC_EARRAY_DBLOCK[1] = {{
    ...
    H5AC__CLASS_NO_FLAGS_SET,      /* Client class behavior flags */
    H5EA__cache_dblock_get_initial_load_size, /* 'get_initial_load_size'
                                                 callback */
    NULL,                             /* 'get_final_load_size' callback */
...
const H5AC_class_t H5AC_EARRAY_DBLK_PAGE[1] = {{
    ...
    H5AC__CLASS_NO_FLAGS_SET,           /* Client class behavior flags */
    H5EA__cache_dblk_page_get_initial_load_size, /* 'get_initial_load_size'
                                                    callback */
    NULL,                                /* 'get_final_load_size' callback */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_hdr_get_initial_load_size(void *_udata,
                                                    size_t *image_len) {
  H5EA_hdr_cache_ud_t *udata =
      (H5EA_hdr_cache_ud_t *)_udata; /* User data for callback */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5EA__cache_hdr_verify_chksum(const void *_image, size_t len,
                                            void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5EA__cache_hdr_deserialize(const void *_image, size_t len,
                                         void *_udata,
  ...
  if (memcmp(image, H5EA_HDR_MAGIC, (size_t)H5_SIZEOF_MAGIC) != 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, NULL,
                "wrong extensible array header signature");
  image += H5_SIZEOF_MAGIC;

  /* Version */
  if (*image++ != H5EA_HDR_VERSION)
    HGOTO_ERROR(H5E_EARRAY, H5E_VERSION, NULL,
                "wrong extensible array header version");

  /* Extensible array class */
  id = (H5EA_cls_id_t)*image++;
  if (id >= H5EA_NUM_CLS_ID)
    HGOTO_ERROR(H5E_EARRAY, H5E_BADTYPE, NULL,
                "incorrect extensible array class");
  ...
  hdr->cparam.max_nelmts_bits =
      *image++; /* Log2(Max. # of elements in array) - i.e. # of bits needed to
                   store max. # of elements */
  hdr->cparam.idx_blk_elmts =
      *image++; /* # of elements to store in index block */
  hdr->cparam.data_blk_min_elmts =
      *image++; /* Min. # of elements per data block */
  hdr->cparam.sup_blk_min_data_ptrs =
      *image++; /* Min. # of data block pointers for a super block */
  hdr->cparam.max_dblk_page_nelmts_bits =
      *image++; /* Log2(Max. # of elements in data block page) - i.e. # of bits
  ...
  H5F_DECODE_LENGTH(
      udata->f, image,
      hdr->stats.stored.nsuper_blks); /* Number of super blocks created */
  H5F_DECODE_LENGTH(
      udata->f, image,
      hdr->stats.stored.super_blk_size); /* Size of super blocks created */
  H5F_DECODE_LENGTH(
      udata->f, image,
      hdr->stats.stored.ndata_blks); /* Number of data blocks created */
  H5F_DECODE_LENGTH(
      udata->f, image,
      hdr->stats.stored.data_blk_size); /* Size of data blocks created */
  H5F_DECODE_LENGTH(udata->f, image,
                    hdr->stats.stored.max_idx_set); /* Max. index set (+1) */
  H5F_DECODE_LENGTH(
      udata->f, image,
      hdr->stats.stored.nelmts); /* Number of elements 'realized' */

  /* Internal information */
  H5F_addr_decode(udata->f, &image,
                  &hdr->idx_blk_addr); /* Address of index block */
  ...
  assert((size_t)(image - (const uint8_t *)_image) ==
         (len - H5EA_SIZEOF_CHKSUM));
  ...
  if (!ret_value)
    if (hdr && H5EA__hdr_dest(hdr) < 0)
      HDONE_ERROR(H5E_EARRAY, H5E_CANTFREE, NULL,
...
static herr_t H5EA__cache_hdr_image_len(const void *_thing, size_t *image_len) {
  const H5EA_hdr_t *hdr =
      (const H5EA_hdr_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_hdr_serialize(const H5F_t *f, void *_image,
                                        size_t H5_ATTR_UNUSED len,
                                        void *_thing) {
  H5EA_hdr_t *hdr =
      (H5EA_hdr_t *)_thing; /* Pointer to the extensible array header */
  ...
  *image++ =
      hdr->cparam
          .max_nelmts_bits; /* Log2(Max. # of elements in array) - i.e. # of
                               bits needed to store    max. # of elements */
  *image++ =
      hdr->cparam.idx_blk_elmts; /* # of elements to store in index block */
  *image++ =
      hdr->cparam.data_blk_min_elmts; /* Min. # of elements per data block */
  *image++ = hdr->cparam.sup_blk_min_data_ptrs; /* Min. # of data block pointers
                                                   for a super block */
  *image++ =
      hdr->cparam.max_dblk_page_nelmts_bits; /* Log2(Max. # of elements in data
                                                block page) - i.e. # of bits
                                                needed to store max. # of
  ...
  H5F_ENCODE_LENGTH(
      f, image,
      hdr->stats.stored.data_blk_size); /* Size of data blocks created */
  H5F_ENCODE_LENGTH(f, image,
                    hdr->stats.stored.max_idx_set); /* Max. index set (+1) */
  H5F_ENCODE_LENGTH(
      f, image, hdr->stats.stored.nelmts); /* Number of elements 'realized' */
  ...
  metadata_chksum =
      H5_checksum_metadata(_image, (size_t)(image - (uint8_t *)_image), 0);
  ...
  if (hdr->swmr_write) {
    ...
    case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
      ...
      if (hdr->parent) {
        ...
        if (H5AC_proxy_entry_remove_child((H5AC_proxy_entry_t *)hdr->parent,
          ...
          HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
                      "unable to destroy flush dependency between extensible "
                      "array and proxy");
        hdr->parent = NULL;
      } /* end if */
      ...
      if (hdr->top_proxy) {
        ...
        /* Don't reset hdr->top_proxy here, it's destroyed when the header is
         * freed -QAK */
      } /* end if */
      break;
...
static herr_t H5EA__cache_hdr_free_icr(void *thing) {
  ...
  if (H5EA__hdr_dest((H5EA_hdr_t *)thing) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTFREE, FAIL,
                "can't free extensible array header");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_iblock_get_initial_load_size(void *_udata,
                                                       size_t *image_len) {
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5EA__cache_iblock_verify_chksum(const void *_image, size_t len,
                                               void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5EA__cache_iblock_deserialize(const void *_image, size_t len,
                                            void *_udata,
                                            hbool_t H5_ATTR_UNUSED *dirty) {
  ...
  if (memcmp(image, H5EA_IBLOCK_MAGIC, (size_t)H5_SIZEOF_MAGIC) != 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, NULL,
                "wrong extensible array index block signature");
  image += H5_SIZEOF_MAGIC;

  /* Version */
  if (*image++ != H5EA_IBLOCK_VERSION)
    HGOTO_ERROR(H5E_EARRAY, H5E_VERSION, NULL,
                "wrong extensible array index block version");
  ...
  /* Address of header for array that owns this block (just for file integrity
   * checks) */
  H5F_addr_decode(hdr->f, &image, &arr_addr);
  if (H5_addr_ne(arr_addr, hdr->addr))
    HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, NULL,
                "wrong extensible array header address");
  ...
  if (hdr->cparam.idx_blk_elmts > 0) {
    ...
    if ((hdr->cparam.cls->decode)(image, iblock->elmts,
                                  (size_t)hdr->cparam.idx_blk_elmts,
                                  hdr->cb_ctx) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTDECODE, NULL,
                  "can't decode extensible array index elements");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_iblock_image_len(const void *_thing,
                                           size_t *image_len) {
  const H5EA_iblock_t *iblock =
      (const H5EA_iblock_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_iblock_serialize(const H5F_t *f, void *_image,
                                           size_t H5_ATTR_UNUSED len,
                                           void *_thing) {
  H5EA_iblock_t *iblock =
      (H5EA_iblock_t *)_thing;        /* Pointer to the object to serialize */
  uint8_t *image = (uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t metadata_chksum;           /* Computed metadata checksum value */
  herr_t ret_value = SUCCEED;
  ...
  *image++ = (uint8_t)iblock->hdr->cparam.cls->id;
  ...
  if (iblock->hdr->cparam.idx_blk_elmts > 0) {
    ...
    if ((iblock->hdr->cparam.cls->encode)(
            image, iblock->elmts, (size_t)iblock->hdr->cparam.idx_blk_elmts,
            iblock->hdr->cb_ctx) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTENCODE, FAIL,
                  "can't encode extensible array index elements");
    image +=
        (iblock->hdr->cparam.idx_blk_elmts * iblock->hdr->cparam.raw_elmt_size);
  } /* end if */
...
static herr_t H5EA__cache_iblock_notify(H5AC_notify_action_t action,
  ...
  case H5AC_NOTIFY_ACTION_AFTER_LOAD:
    ...
    if (H5EA__create_flush_depend((H5AC_info_t *)iblock->hdr,
                                  (H5AC_info_t *)iblock) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTDEPEND, FAIL,
                  "unable to create flush dependency between index block and "
                  "header, address = %llu",
  ...
  case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
    ...
    if (H5EA__destroy_flush_depend((H5AC_info_t *)iblock->hdr,
                                   (H5AC_info_t *)iblock) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
...
static herr_t H5EA__cache_iblock_free_icr(void *thing) {
  ...
  if (H5EA__iblock_dest((H5EA_iblock_t *)thing) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTFREE, FAIL,
                "can't free extensible array index block");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_sblock_get_initial_load_size(void *_udata,
                                                       size_t *image_len) {
  H5EA_sblock_cache_ud_t *udata =
      (H5EA_sblock_cache_ud_t *)_udata; /* User data */
  ...
  if (sblock.dblk_nelmts > udata->hdr->dblk_page_nelmts) {
    ...
    assert((sblock.dblk_npages * udata->hdr->dblk_page_nelmts) ==
           sblock.dblk_nelmts);
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5EA__cache_sblock_verify_chksum(const void *_image, size_t len,
                                               void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5EA__cache_sblock_deserialize(const void *_image, size_t len,
                                            void *_udata,
                                            hbool_t H5_ATTR_UNUSED *dirty) {
  ...
  if (NULL ==
      (sblock = H5EA__sblock_alloc(udata->hdr, udata->parent, udata->sblk_idx)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL,
  ...
  if (memcmp(image, H5EA_SBLOCK_MAGIC, (size_t)H5_SIZEOF_MAGIC) != 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, NULL,
                "wrong extensible array super block signature");
  image += H5_SIZEOF_MAGIC;

  /* Version */
  if (*image++ != H5EA_SBLOCK_VERSION)
    HGOTO_ERROR(H5E_EARRAY, H5E_VERSION, NULL,
                "wrong extensible array super block version");
  ...
  if (sblock->dblk_npages > 0) {
    size_t tot_page_init_size =
        sblock->ndblks * sblock->dblk_page_init_size; /* Compute total size of
                                                         'page init' buffer */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_sblock_image_len(const void *_thing,
                                           size_t *image_len) {
  const H5EA_sblock_t *sblock =
      (const H5EA_sblock_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_sblock_serialize(const H5F_t *f, void *_image,
                                           size_t H5_ATTR_UNUSED len,
                                           void *_thing) {
  H5EA_sblock_t *sblock =
      (H5EA_sblock_t *)_thing;        /* Pointer to the object to serialize */
  uint8_t *image = (uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t metadata_chksum;           /* Computed metadata checksum value */
  size_t u;                           /* Local index variable */
...
static herr_t H5EA__cache_sblock_notify(H5AC_notify_action_t action,
  ...
  case H5AC_NOTIFY_ACTION_AFTER_LOAD:
    ...
    if (H5EA__create_flush_depend((H5AC_info_t *)sblock->parent,
                                  (H5AC_info_t *)sblock) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTDEPEND, FAIL,
                  "unable to create flush dependency between super block and "
                  "index block, address = %llu",
  ...
  case H5AC_NOTIFY_ACTION_AFTER_FLUSH:
    ...
    if (sblock->has_hdr_depend) {
      if (H5EA__destroy_flush_depend((H5AC_info_t *)sblock->hdr,
                                     (H5AC_info_t *)sblock) < 0)
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
                    "unable to destroy flush dependency between super block "
                    "and header, address = %llu",
                    (unsigned long long)sblock->addr);
  ...
  case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
    ...
    if (H5EA__destroy_flush_depend((H5AC_info_t *)sblock->parent,
                                   (H5AC_info_t *)sblock) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
...
static herr_t H5EA__cache_sblock_free_icr(void *thing) {
  ...
  if (H5EA__sblock_dest((H5EA_sblock_t *)thing) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTFREE, FAIL,
                "can't free extensible array super block");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_dblock_get_initial_load_size(void *_udata,
                                                       size_t *image_len) {
  H5EA_dblock_cache_ud_t *udata =
      (H5EA_dblock_cache_ud_t *)_udata; /* User data */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5EA__cache_dblock_verify_chksum(const void *_image, size_t len,
                                               void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5EA__cache_dblock_deserialize(const void *_image,
  ...
  if (NULL ==
      (dblock = H5EA__dblock_alloc(udata->hdr, udata->parent, udata->nelmts)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL,
  ...
  if (memcmp(image, H5EA_DBLOCK_MAGIC, (size_t)H5_SIZEOF_MAGIC) != 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, NULL,
                "wrong extensible array data block signature");
  image += H5_SIZEOF_MAGIC;

  /* Version */
  if (*image++ != H5EA_DBLOCK_VERSION)
    HGOTO_ERROR(H5E_EARRAY, H5E_VERSION, NULL,
                "wrong extensible array data block version");
  ...
  if (!dblock->npages) {
    ...
    /* Convert from raw elements on disk into native elements in memory */
    if ((udata->hdr->cparam.cls->decode)(image, dblock->elmts, udata->nelmts,
                                         udata->hdr->cb_ctx) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTDECODE, NULL,
                  "can't decode extensible array data elements");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_dblock_image_len(const void *_thing,
                                           size_t *image_len) {
  const H5EA_dblock_t *dblock =
      (const H5EA_dblock_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_dblock_serialize(const H5F_t *f, void *_image,
                                           size_t H5_ATTR_UNUSED len,
                                           void *_thing) {
  H5EA_dblock_t *dblock =
      (H5EA_dblock_t *)_thing;        /* Pointer to the object to serialize */
  uint8_t *image = (uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t metadata_chksum;           /* Computed metadata checksum value */
  herr_t ret_value = SUCCEED;
  ...
  *image++ = (uint8_t)dblock->hdr->cparam.cls->id;
  ...
  if (!dblock->npages) {
    ...
    /* Convert from native elements in memory into raw elements on disk */
    if ((dblock->hdr->cparam.cls->encode)(image, dblock->elmts, dblock->nelmts,
                                          dblock->hdr->cb_ctx) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTENCODE, FAIL,
                  "can't encode extensible array data elements");
...
static herr_t H5EA__cache_dblock_notify(H5AC_notify_action_t action,
  ...
  case H5AC_NOTIFY_ACTION_AFTER_LOAD:
    ...
    if (H5EA__create_flush_depend((H5AC_info_t *)dblock->parent,
                                  (H5AC_info_t *)dblock) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTDEPEND, FAIL,
                  "unable to create flush dependency between data block and "
                  "parent, address = %llu",
  ...
  case H5AC_NOTIFY_ACTION_AFTER_FLUSH:
    ...
    if (dblock->has_hdr_depend) {
      if (H5EA__destroy_flush_depend((H5AC_info_t *)dblock->hdr,
                                     (H5AC_info_t *)dblock) < 0)
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
                    "unable to destroy flush dependency between direct block "
                    "and header, address = %llu",
                    (unsigned long long)dblock->addr);
  ...
  case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
    ...
    if (H5EA__destroy_flush_depend((H5AC_info_t *)dblock->parent,
                                   (H5AC_info_t *)dblock) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
...
static herr_t H5EA__cache_dblock_free_icr(void *thing) {
  ...
  if (H5EA__dblock_dest((H5EA_dblock_t *)thing) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTFREE, FAIL,
                "can't free extensible array data block");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_dblock_fsf_size(const void *_thing,
                                          hsize_t *fsf_size) {
  const H5EA_dblock_t *dblock =
      (const H5EA_dblock_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_dblk_page_get_initial_load_size(void *_udata,
                                                          size_t *image_len) {
  H5EA_dblk_page_cache_ud_t *udata =
      (H5EA_dblk_page_cache_ud_t *)_udata; /* User data */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5EA__cache_dblk_page_verify_chksum(const void *_image,
                                                  size_t len,
                                                  void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5EA__cache_dblk_page_deserialize(const void *_image, size_t len,
                                               void *_udata,
                                               hbool_t H5_ATTR_UNUSED *dirty) {
  H5EA_dblk_page_t *dblk_page = NULL; /* Data block page info */
  H5EA_dblk_page_cache_ud_t *udata = (H5EA_dblk_page_cache_ud_t *)
      _udata; /* User data for loading data block page */
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
  void *ret_value = NULL;
  ...
  /* Convert from raw elements on disk into native elements in memory */
  if ((udata->hdr->cparam.cls->decode)(image, dblk_page->elmts,
                                       udata->hdr->dblk_page_nelmts,
                                       udata->hdr->cb_ctx) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTDECODE, NULL,
                "can't decode extensible array data elements");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__cache_dblk_page_image_len(const void *_thing,
                                              size_t *image_len) {
  const H5EA_dblk_page_t *dblk_page =
      (const H5EA_dblk_page_t *)_thing; /* Pointer to the object */
 ...
 */
static herr_t
H5EA__cache_dblk_page_serialize(const H5F_t H5_ATTR_NDEBUG_UNUSED *f,
                                void *_image, size_t H5_ATTR_UNUSED len,
                                void *_thing) {
  H5EA_dblk_page_t *dblk_page =
      (H5EA_dblk_page_t *)_thing;     /* Pointer to the object to serialize */
  uint8_t *image = (uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t metadata_chksum;           /* Computed metadata checksum value */
  herr_t ret_value = SUCCEED;
  ...
  /* Convert from native elements in memory into raw elements on disk */
  if ((dblk_page->hdr->cparam.cls->encode)(image, dblk_page->elmts,
                                           dblk_page->hdr->dblk_page_nelmts,
                                           dblk_page->hdr->cb_ctx) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTENCODE, FAIL,
                "can't encode extensible array data elements");
  image +=
      (dblk_page->hdr->dblk_page_nelmts * dblk_page->hdr->cparam.raw_elmt_size);
...
static herr_t H5EA__cache_dblk_page_notify(H5AC_notify_action_t action,
                                           void *_thing) {
  H5EA_dblk_page_t *dblk_page =
      (H5EA_dblk_page_t *)_thing; /* Pointer to the object */
  herr_t ret_value = SUCCEED;
  ...
  case H5AC_NOTIFY_ACTION_AFTER_LOAD:
    /* Create flush dependency on parent */
    if (H5EA__create_flush_depend((H5AC_info_t *)dblk_page->parent,
                                  (H5AC_info_t *)dblk_page) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTDEPEND, FAIL,
                  "unable to create flush dependency between data block page "
                  "and parent, address = %llu",
  ...
  case H5AC_NOTIFY_ACTION_AFTER_FLUSH:
    ...
    if (dblk_page->has_hdr_depend) {
      if (H5EA__destroy_flush_depend((H5AC_info_t *)dblk_page->hdr,
                                     (H5AC_info_t *)dblk_page) < 0)
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
  ...
  case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
    /* Destroy flush dependency on parent */
    if (H5EA__destroy_flush_depend((H5AC_info_t *)dblk_page->parent,
                                   (H5AC_info_t *)dblk_page) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNDEPEND, FAIL,
...
static herr_t H5EA__cache_dblk_page_free_icr(void *thing) {
  ...
  if (H5EA__dblk_page_dest((H5EA_dblk_page_t *)thing) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTFREE, FAIL,
                "can't free extensible array data block page");
