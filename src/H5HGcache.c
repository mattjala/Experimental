...
static herr_t H5HG__cache_heap_get_initial_load_size(void *udata,
                                                     size_t *image_len);
 ...
 */
static herr_t
H5HG__cache_heap_get_initial_load_size(void H5_ATTR_UNUSED *_udata,
                                       size_t *image_len) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HG__cache_heap_get_final_load_size(const void *image,
                                                   size_t image_len,
                                                   void *udata,
                                                   size_t *actual_len) {
  H5HG_heap_t heap;
  ...
  heap.size = 0;
  if (H5HG__hdr_deserialize(&heap, (const uint8_t *)image, image_len,
                            (const H5F_t *)udata) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTDECODE, FAIL,
                "can't decode global heap prefix");
 ...
 *-------------------------------------------------------------------------
 */
static void *H5HG__cache_heap_deserialize(const void *_image, size_t len,
                                          void *_udata,
  ...
  if (H5HG__hdr_deserialize(heap, (const uint8_t *)heap->chunk, len, f) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTDECODE, NULL,
                "can't decode global heap header");
  ...
  while (p < (heap->chunk + heap->size)) {
    ...
    if ((p + H5HG_SIZEOF_OBJHDR(f)) > (heap->chunk + heap->size)) {
      ...
      if (idx >= heap->nalloc) {
        ...
        if (NULL ==
            (new_obj = H5FL_SEQ_REALLOC(H5HG_obj_t, heap->obj, new_alloc)))
  ...
  if (FALSE == H5HG_ISALIGNED(heap->obj[0].size))
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, NULL,
                "decoded global heap is not aligned");
  ...
  if (max_idx >= heap->nused)
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, NULL,
                "bad `next unused` heap index value");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HG__cache_heap_image_len(const void *_thing,
                                         size_t *image_len) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HG__cache_heap_serialize(const H5F_t H5_ATTR_NDEBUG_UNUSED *f,
