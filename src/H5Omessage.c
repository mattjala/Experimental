...
static herr_t H5O__msg_remove_cb(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                 unsigned sequence, unsigned *oh_modified,
                                 void *_udata /*in,out*/);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_create(const H5O_loc_t *loc, unsigned type_id,
  ...
  if (H5O_msg_append_oh(loc->file, oh, type_id, mesg_flags, update_flags,
                        mesg) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, FAIL,
                "unable to append to object header");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_append_oh(H5F_t *f, H5O_t *oh, unsigned type_id,
                         unsigned mesg_flags, unsigned update_flags,
                         void *mesg) {
  ...
  assert(oh);
  assert(H5O_ATTR_ID !=
         type_id); /* Attributes are modified in another routine */
  assert(type_id < NELMTS(H5O_msg_class_g));
  type =
      H5O_msg_class_g[type_id]; /* map the type ID to the actual type object */
  assert(type);
  assert(0 == (mesg_flags & ~H5O_MSG_FLAG_BITS));
  assert(mesg);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__msg_append_real(H5F_t *f, H5O_t *oh, const H5O_msg_class_t *type,
                            unsigned mesg_flags, unsigned update_flags,
                            void *mesg) {
 ...
 *              data.  This is useful when several calls to H5O_msg_write will
 *be made in a sequence.
 *
 * Return:	Success:	Non-negative
 *
 *		Failure:	Negative
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_write(const H5O_loc_t *loc, unsigned type_id,
                     unsigned mesg_flags, unsigned update_flags, void *mesg) {
  ...
  if (H5O__msg_write_real(loc->file, oh, type, mesg_flags, update_flags, mesg) <
      0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_write_oh(H5F_t *f, H5O_t *oh, unsigned type_id,
                        unsigned mesg_flags, unsigned update_flags,
                        void *mesg) {
  const H5O_msg_class_t *type; /* Actual H5O class type for the ID */
  herr_t ret_value = SUCCEED;  /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__msg_write_real(H5F_t *f, H5O_t *oh, const H5O_msg_class_t *type,
                           unsigned mesg_flags, unsigned update_flags,
                           void *mesg) {
  ...
  /* This message is shared, but it's being modified. */
     ...
     */
    if ((status =
             H5SM_try_share(f, ((mesg_flags & H5O_MSG_FLAG_SHARED) ? NULL : oh),
                            0, idx_msg->type->id, mesg, &mesg_flags)) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_BADMESG, FAIL,
                  "error while trying to share message");
    if (status == FALSE && (mesg_flags & H5O_MSG_FLAG_SHARED))
      HGOTO_ERROR(H5E_OHDR, H5E_BADMESG, FAIL,
                  "message changed sharing status");
  } /* end if */
...
htri_t H5O_msg_exists(const H5O_loc_t *loc, unsigned type_id) {
  ...
  if ((ret_value = H5O_msg_exists_oh(oh, type_id)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_READERROR, FAIL,
                "unable to verify object header message");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_remove(const H5O_loc_t *loc, unsigned type_id, int sequence,
                      hbool_t adj_link) {
  ...
  if ((ret_value = H5O__msg_remove_real(loc->file, oh, type, sequence, NULL,
                                        NULL, adj_link)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, FAIL,
                "unable to remove object header message");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_remove_op(const H5O_loc_t *loc, unsigned type_id, int sequence,
  ...
  /* Call the "real" remove routine */
  if ((ret_value = H5O__msg_remove_real(loc->file, oh, type, sequence, op,
                                        op_data, adj_link)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, FAIL,
                "unable to remove object header message");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__msg_remove_real(H5F_t *f, H5O_t *oh, const H5O_msg_class_t *type,
                            int sequence, H5O_operator_t app_op, void *op_data,
                            hbool_t adj_link) {
 ...
 *  passed in to H5O_msg_iterate ('op_data').  The return values from an
 *operator are: A. Zero causes the iterator to continue, returning zero when all
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_iterate(const H5O_loc_t *loc, unsigned type_id,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__msg_iterate_real(H5F_t *f, H5O_t *oh, const H5O_msg_class_t *type,
                             const H5O_mesg_operator_t *op, void *op_data) {
  H5O_mesg_t *idx_msg; /* Pointer to current message */
  unsigned idx;        /* Absolute index of current message in all messages */
  unsigned
      sequence; /* Relative index of current message for messages of type */
  unsigned oh_modified =
      0; /* Whether the callback modified the object header */
  herr_t ret_value = H5_ITER_CONT; /* Return value */
  ...
  for (sequence = 0, idx = 0, idx_msg = &oh->mesg[0];
       idx < oh->nmesgs && !ret_value; idx++, idx_msg++) {
    if (type == idx_msg->type) {
      ...
      if (op->op_type == H5O_MESG_OP_LIB)
        ret_value =
            (op->u.lib_op)(oh, idx_msg, sequence, &oh_modified, op_data);
      else
  ...
  if (oh_modified) {
    ...
    if (H5O_touch_oh(f, oh, FALSE) < 0)
      HDONE_ERROR(H5E_OHDR, H5E_CANTUPDATE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
size_t H5O_msg_raw_size(const H5F_t *f, unsigned type_id,
  ...
  if (0 == (ret_value = (type->raw_size)(f, disable_shared, mesg)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOUNT, 0,
                "unable to determine size of message");
 ...
 *-------------------------------------------------------------------------
 */
size_t H5O_msg_size_f(const H5F_t *f, hid_t ocpl_id, unsigned type_id,
 ...
 *-------------------------------------------------------------------------
 */
size_t H5O_msg_size_oh(const H5F_t *f, const H5O_t *oh, unsigned type_id,
                       const void *mesg, size_t extra_raw) {
  const H5O_msg_class_t *type; /* Actual H5O class type for the ID */
  size_t ret_value = 0;        /* Return value */
...
htri_t H5O_msg_is_shared(unsigned type_id, const void *mesg) {
  ...
  {
    assert(type_id < NELMTS(H5O_msg_class_g));
    type = H5O_msg_class_g[type_id]; /* map the type ID to the actual type
                                        object */
    assert(type);
    assert(mesg);

    /* If messages in a class aren't shareable, then obviously this message
     * isn't shared! :-) */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_set_share(unsigned type_id, const H5O_shared_t *share,
                         void *mesg) {
  const H5O_msg_class_t *type; /* Actual H5O class type for the ID */
  herr_t ret_value = SUCCEED;  /* Return value */
  ...
  if (type->set_share) {
    if ((type->set_share)(mesg, share) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                  "unable to set shared message information");
  } /* end if */
  else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_get_crt_index(unsigned type_id, const void *mesg,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_encode(H5F_t *f, unsigned type_id, hbool_t disable_shared,
 ...
 *-------------------------------------------------------------------------
 */
void *H5O_msg_decode(H5F_t *f, H5O_t *open_oh, unsigned type_id,
  ...
  /* decode */
  if (NULL ==
      (ret_value = (type->decode)(f, open_oh, 0, &ioflags, buf_size, buf)))
 ...
 *
 * Purpose:     Copies a message to file.  If MESG is the null pointer then a
 *null pointer is returned with no error.
 *
 ...
 *-------------------------------------------------------------------------
 */
void *H5O__msg_copy_file(const H5O_msg_class_t *type, H5F_t *file_src,
   ...
   */
  if (NULL == (ret_value = (type->copy_file)(file_src, native_src, file_dst,
                                             recompute_size, mesg_flags,
                                             cpy_info, udata)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, NULL,
                "unable to copy object header message to file");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__msg_alloc(H5F_t *f, H5O_t *oh, const H5O_msg_class_t *type,
  ...
  if ((shared_mesg = H5O_msg_is_shared(type->id, native)) < 0)
    ...
    if (type->link && (type->link)(f, oh, native) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_LINKCOUNT, FAIL,
                  "unable to adjust shared message ref count");
...
static herr_t H5O__copy_mesg(H5F_t *f, H5O_t *oh, size_t idx,
  ...
  if (update_flags & H5O_UPDATE_TIME)
    if (H5O_touch_oh(f, oh, FALSE) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTUPDATE, FAIL,
                  "unable to update time on object");
 ...
 *              Delete an object header message from a file.  This frees the
 *file space used for anything referred to in the object header message.
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
  ...
  *p++ = mesg->flags;
  ...
  if (mesg->native && mesg->type != H5O_MSG_UNKNOWN) {
    ...
    assert(mesg->raw + mesg->raw_size <=
           oh->chunk[mesg->chunkno].image +
               (oh->chunk[mesg->chunkno].size - H5O_SIZEOF_CHKSUM_OH(oh)));
#ifndef NDEBUG
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_msg_get_flags(const H5O_loc_t *loc, unsigned type_id,
                         uint8_t *flags) {
  H5O_t *oh = NULL;            /* Object header to use */
  const H5O_msg_class_t *type; /* Actual H5O class type for the ID */
  H5O_mesg_t *idx_msg;         /* Pointer to message to modify */
