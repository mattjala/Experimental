...
static herr_t H5O__sdspace_pre_copy_file(H5F_t *file_src, const void *mesg_src,
                                         hbool_t *deleted,
...
--------------------------------------------------------------------------*/
static void *H5O__sdspace_decode(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh,
  ...
  if (NULL == (sdim = H5FL_CALLOC(H5S_extent_t)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, NULL,
                "dataspace structure allocation failed");
  ...
  if (version < H5O_SDSPACE_VERSION_1 || version > H5O_SDSPACE_VERSION_2)
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, NULL,
                "wrong version number in dataspace message");
  ...
  if (sdim->rank > 0) {
    ...
    if (NULL ==
        (sdim->size = (hsize_t *)H5FL_ARR_MALLOC(hsize_t, (size_t)sdim->rank)))
    ...
    if (flags & H5S_VALID_MAX) {
      if (NULL ==
          (sdim->max = (hsize_t *)H5FL_ARR_MALLOC(hsize_t, (size_t)sdim->rank)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                    "memory allocation failed");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    void *H5O__sdspace_copy(_mesg, _dest)
        const void *_mesg;	IN: Pointer to the source extent dimensionality
struct
        const void *_dest;	IN: Pointer to the destination extent
dimensionality struct RETURNS Pointer to DEST on success, NULL on failure
 DESCRIPTION
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    void *H5O__sdspace_size(f, mesg)
        H5F_t *f;	  IN: pointer to the HDF5 file struct
        const void *mesg;	IN: Pointer to the source extent dimensionality
struct RETURNS Size of message on success, zero on failure DESCRIPTION This
function returns the size of the raw simple dimensionality message on success.
(Not counting the message type or size fields, only the data portion of the
message).  It doesn't take into account alignment.
...
static size_t H5O__sdspace_size(const H5F_t *f, const void *_mesg) {
  ...
  ret_value = (size_t)(1 + /* Version */
                       ...
                       ((space->version > H5O_SDSPACE_VERSION_1)
                            ? 0
                            : 4)); /* Eliminated/reserved */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__sdspace_pre_copy_file(H5F_t H5_ATTR_UNUSED *file_src,
                                         const void *mesg_src,
                                         hbool_t H5_ATTR_UNUSED *deleted,
                                         const H5O_copy_t *cpy_info,
                                         void *_udata) {
  const H5S_extent_t *src_space_extent =
      (const H5S_extent_t *)mesg_src; /* Source dataspace extent */
  ...
  if (src_space_extent->version >
      H5O_sdspace_ver_bounds[H5F_HIGH_BOUND(cpy_info->file_dst)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "dataspace message version out of bounds");
  ...
  if (udata) {
    ...
    if (NULL == (udata->src_space_extent = H5FL_CALLOC(H5S_extent_t)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_NOSPACE, FAIL,
                  "dataspace extent allocation failed");

    /* Create a copy of the dataspace extent */
    if (H5S__extent_copy_real(udata->src_space_extent, src_space_extent, TRUE) <
        0)
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    void *H5O__sdspace_debug(f, mesg, stream, indent, fwidth)
        H5F_t *f;	        IN: pointer to the HDF5 file struct
        const void *mesg;	IN: Pointer to the source extent dimensionality
struct FILE *stream;		IN: Pointer to the stream for output data int
indent;		IN: Amount to indent information by
 ...
 DESCRIPTION
    ...
    parameter.
--------------------------------------------------------------------------*/
static herr_t H5O__sdspace_debug(H5F_t H5_ATTR_UNUSED *f, const void *mesg,
