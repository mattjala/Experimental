 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_link_create(H5VL_link_create_args_t *args, void *obj,
                                const H5VL_loc_params_t *loc_params,
                                hid_t lcpl_id, hid_t H5_ATTR_UNUSED lapl_id,
  ...
  case H5VL_LINK_CREATE_HARD: {
    ...
    if (NULL != cur_obj &&
        H5G_loc_real(cur_obj, cur_params->obj_type, &cur_loc) < 0)
    ...
    if (H5VL_OBJECT_BY_NAME == cur_params->type) {
      ...
      /* Create the link */
      if (H5L__create_hard(cur_loc_p, cur_params->loc_data.loc_by_name.name,
      ...
      /* Link to the object */
      if (H5L_link(&link_loc, loc_params->loc_data.loc_by_name.name, &cur_loc,
                   lcpl_id) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "unable to create link");
    } /* end else */
  ...
  case H5VL_LINK_CREATE_SOFT: {
    ...
    if (H5G_loc_real(obj, loc_params->obj_type, &link_loc) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a file or file object");
    if (H5L__create_soft(args->args.soft.target, &link_loc,
  ...
  case H5VL_LINK_CREATE_UD: {
    ...
    if (H5G_loc_real(obj, loc_params->obj_type, &link_loc) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a file or file object");
    if (H5L__create_ud(&link_loc, loc_params->loc_data.loc_by_name.name,
                       args->args.ud.buf, args->args.ud.buf_size,
                       args->args.ud.type, lcpl_id) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "unable to create link");
...
herr_t H5VL__native_link_copy(void *src_obj,
  ...
  if (NULL != src_obj &&
      H5G_loc_real(src_obj, loc_params1->obj_type, &src_loc) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a file or file object");
  if (NULL != dst_obj &&
      H5G_loc_real(dst_obj, loc_params2->obj_type, &dst_loc) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a file or file object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_link_get(void *obj, const H5VL_loc_params_t *loc_params,
  ...
  case H5VL_LINK_GET_INFO: {
    ...
    if (loc_params->type == H5VL_OBJECT_BY_NAME) {
      if (H5L_get_info(&loc, loc_params->loc_data.loc_by_name.name,
  ...
  case H5VL_LINK_GET_NAME: {
    if (H5L__get_name_by_idx(
            &loc, loc_params->loc_data.loc_by_idx.name,
            loc_params->loc_data.loc_by_idx.idx_type,
            loc_params->loc_data.loc_by_idx.order,
            loc_params->loc_data.loc_by_idx.n, args->args.get_name.name,
            args->args.get_name.name_size, args->args.get_name.name_len) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_NOTFOUND, FAIL, "unable to get link info");
  ...
  case H5VL_LINK_GET_VAL: {
    /* Get the link information */
    if (loc_params->type == H5VL_OBJECT_BY_NAME) {
      if (H5L__get_val(&loc, loc_params->loc_data.loc_by_name.name,
  ...
  default:
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "can't get this type of information from link");
  } /* end switch */
...
herr_t H5VL__native_link_specific(void *obj,
  ...
  case H5VL_LINK_EXISTS: {
    ...
    if (H5G_loc_real(obj, loc_params->obj_type, &loc) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a file or file object");
    if (H5L__exists(&loc, loc_params->loc_data.loc_by_name.name,
  ...
  case H5VL_LINK_ITER: {
    ...
    if (loc_params->type == H5VL_OBJECT_BY_SELF) {
      if (iter_args->recursive) {
        /* H5Lvisit */
        if ((ret_value =
                 H5G_visit(&loc, ".", iter_args->idx_type, iter_args->order,
                           iter_args->op, iter_args->op_data)) < 0)
          HGOTO_ERROR(H5E_LINK, H5E_BADITER, FAIL, "link visitation failed");
      } /* end if */
      else {
        /* H5Literate */
        if ((ret_value = H5L_iterate(&loc, ".", iter_args->idx_type,
                                     iter_args->order, iter_args->idx_p,
                                     iter_args->op, iter_args->op_data)) < 0)
      ...
      if (iter_args->recursive) {
        ...
        if ((ret_value = H5G_visit(&loc, loc_params->loc_data.loc_by_name.name,
                                   iter_args->idx_type, iter_args->order,
                                   iter_args->op, iter_args->op_data)) < 0)
          HGOTO_ERROR(H5E_LINK, H5E_BADITER, FAIL, "link visitation failed");
      } /* end if */
      else {
        ...
        if ((ret_value = H5L_iterate(
                 &loc, loc_params->loc_data.loc_by_name.name,
      ...
      HGOTO_ERROR(H5E_LINK, H5E_UNSUPPORTED, FAIL,
                  "unknown link iterate params");
  ...
  case H5VL_LINK_DELETE: {
    ...
    if (loc_params->type == H5VL_OBJECT_BY_NAME) { /* H5Ldelete */
      ...
      if (H5L__delete_by_idx(&loc, loc_params->loc_data.loc_by_idx.name,
                             loc_params->loc_data.loc_by_idx.idx_type,
                             loc_params->loc_data.loc_by_idx.order,
