...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5Pregister1(class, name, size, default, prp_create, prp_set,
...
copy callback H5P_prp_close_func_t prp_close; IN: Function pointer to property
close callback RETURNS Returns non-negative on success, negative on failure.
 DESCRIPTION
...
created. (The 'default' value passed to H5Pregister1) The 'create' routine may
...
herr_t H5Pregister1(hid_t cls_id, const char *name, size_t size,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "i*sz*xPCPSPGPDPOPL", cls_id, name, size, def_value,
            prp_create, prp_set, prp_get, prp_delete, prp_copy, prp_close);
...
herr_t H5Pinsert1(hid_t plist_id, const char *name, size_t size, void *value,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE9("e", "i*sz*xPSPGPDPOPL", plist_id, name, size, value, prp_set,
  ...
  if ((ret_value = H5P_insert(plist, name, size, value, prp_set, prp_get, NULL,
                              NULL, prp_delete, prp_copy, NULL, prp_close)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
...
buffer USAGE herr_t H5Pencode1(plist_id, buf, nalloc) hid_t plist_id; IN:
Identifier to property list to encode void *buf:              OUT: buffer to
gold the encoded plist size_t *nalloc;         IN/OUT: size of buffer needed to
encode plist RETURNS Returns non-negative on success, negative on failure.
 DESCRIPTION
...
herr_t H5Pset_file_space(hid_t plist_id, H5F_file_space_type_t strategy,
  ...
  hbool_t new_persist =
      H5F_FREE_SPACE_PERSIST_DEF; /* Persisting free-space or not */
  hsize_t new_threshold =
      H5F_FREE_SPACE_THRESHOLD_DEF; /* Free-space section threshold */
  ...
  if (H5Pset_file_space_strategy(plist_id, new_strategy, new_persist,
                                 new_threshold) < 0)
...
herr_t H5Pget_file_space(hid_t plist_id,
                         H5F_file_space_type_t *strategy /*out*/,
  ...
  if (H5Pget_file_space_strategy(plist_id, &new_strategy, &new_persist,
                                 &new_threshold) < 0)
