    ...
    if (H5T_SORT_VALUE != dt->shared->u.enumer.sorted) {
      ...
      for (i = (nmembs - 1), swapped = TRUE; i > 0 && swapped; --i) {
        for (j = 0, swapped = FALSE; j < i; j++) {
          if (memcmp((uint8_t *)dt->shared->u.enumer.value + (j * size),
                     (uint8_t *)dt->shared->u.enumer.value + ((j + 1) * size),
                     size) > 0) {
            /* Swap names */
            ...
            /* Swap values */
            H5MM_memcpy(
                tbuf, (uint8_t *)dt->shared->u.enumer.value + (j * size), size);
            H5MM_memcpy(
                (uint8_t *)dt->shared->u.enumer.value + (j * size),
                (uint8_t *)dt->shared->u.enumer.value + ((j + 1) * size), size);
            H5MM_memcpy((uint8_t *)dt->shared->u.enumer.value +
                            ((j + 1) * size),
                        tbuf, size);
      ...
      for (i = 0; i < (nmembs - 1); i++)
        assert(memcmp((uint8_t *)dt->shared->u.enumer.value + (i * size),
                      (uint8_t *)dt->shared->u.enumer.value + ((i + 1) * size),
                      size) < 0);
#endif
    } /* end if */
  }   /* end else */
...
herr_t H5T__sort_name(const H5T_t *dt, int *map) {
  ...
  if (H5T_COMPOUND == dt->shared->type) {
    ...
    if (H5T_SORT_NAME != dt->shared->u.enumer.sorted) {
      ...
      /* I never trust a sort :-) -RPM */
      for (i = 0; i < nmembs - 1; i++)
        assert(HDstrcmp(dt->shared->u.enumer.name[i],
                        dt->shared->u.enumer.name[i + 1]) < 0);
#endif
    }
  }
