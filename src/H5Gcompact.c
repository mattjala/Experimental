...
/* PRIVATE PROTOTYPES */
static herr_t H5G__compact_build_table_cb(const void *_mesg, unsigned idx,
                                          void *_udata);
...
static herr_t H5G__compact_lookup_cb(const void *_mesg,
                                     unsigned H5_ATTR_UNUSED idx, void *_udata);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__compact_build_table_cb(const void *_mesg,
  ...
  if (NULL ==
      H5O_msg_copy(H5O_LINK_ID, lnk, &(udata->ltable->lnks[udata->curr_lnk])))
...
static herr_t H5G__compact_build_table(const H5O_loc_t *oloc,
  ...
  if (ltable->nlinks > 0) {
    ...
    if ((ltable->lnks = (H5O_link_t *)H5MM_calloc(sizeof(H5O_link_t) *
                                                  ltable->nlinks)) == NULL)
    ...
    if (H5O_msg_iterate(oloc, H5O_LINK_ID, &op, &udata) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL,
                  "error iterating over link messages");
...
herr_t H5G__compact_get_name_by_idx(const H5O_loc_t *oloc,
                                    ...
                                    hsize_t idx, char *name, size_t name_size,
                                    size_t *name_len) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__compact_remove_common_cb(const void *_mesg,
                                            unsigned H5_ATTR_UNUSED idx,
                                            void *_udata) {
  const H5O_link_t *lnk = (const H5O_link_t *)_mesg; /* Pointer to link */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__compact_remove(const H5O_loc_t *oloc, H5RS_str_t *grp_full_path_r,
                           const char *name) {
  ...
  if (H5O_msg_remove_op(oloc, H5O_LINK_ID, H5O_FIRST,
                        H5G__compact_remove_common_cb, &udata, TRUE) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__compact_iterate(const H5O_loc_t *oloc, const H5O_linfo_t *linfo,
                            H5_index_t idx_type, H5_iter_order_t order,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__compact_lookup(const H5O_loc_t *oloc, const char *name,
...
herr_t H5G__compact_lookup_by_idx(const H5O_loc_t *oloc,
                                  const H5O_linfo_t *linfo, H5_index_t idx_type,
