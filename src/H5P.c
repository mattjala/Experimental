...
hid_t H5Pcopy(hid_t id) {
  ...
  if (H5I_GENPROP_LST != H5I_get_type(id) &&
      H5I_GENPROP_CLS != H5I_get_type(id))
 ...
 USAGE
    hid_t H5Pcreate_class(parent, name, cls_create, create_data,
        ...
        void *close_data;   IN: Pointer to user data to pass along to class
                                    close callback.
 RETURNS
    Returns a valid property list class ID on success, H5I_INVALID_HID on
failure. DESCRIPTION Allocates memory and attaches a class to the property list
class hierarchy. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS EXAMPLES REVISION
LOG
--------------------------------------------------------------------------*/
hid_t H5Pcreate_class(hid_t parent, const char *name,
  ...
  if ((create_data != NULL && cls_create == NULL) ||
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "data specified, but no callback provided");
  ...
  if (parent == H5P_DEFAULT)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                "can't retrieve parent class");

  /* Create the new property list class */
  if (NULL == (pclass = H5P__create_class(par_class, name, H5P_TYPE_USER,
                                          cls_create, create_data, cls_copy,
...
hid_t H5Pcreate(hid_t cls_id) {
  ...
  /* Check arguments. */
  if (NULL ==
      (pclass = (H5P_genclass_t *)H5I_object_verify(cls_id, H5I_GENPROP_CLS)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                "not a property list class");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5Pregister2(class, name, size, default, prp_create, prp_set,
prp_get, prp_close) hid_t class;            IN: Property list class to close
...
callback H5P_prp_delete_func_t prp_delete; IN: Function pointer to property
...
herr_t H5Pregister2(hid_t cls_id, const char *name, size_t size,
  ...
  H5TRACE11("e", "i*sz*xPCPSPGPDPOPMPL", cls_id, name, size, def_value,
            prp_create, prp_set, prp_get, prp_delete, prp_copy, prp_cmp,
            prp_close);
  ...
  if (size > 0 && def_value == NULL)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "properties >0 size must have default");

  /* Create the new property list class */
  orig_pclass = pclass;
  if ((ret_value = H5P__register(&pclass, name, size, def_value, prp_create,
                                 prp_set, prp_get, NULL, NULL, prp_delete,
                                 prp_copy, prp_cmp, prp_close)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                "unable to register property in class");
...
herr_t H5Pinsert2(hid_t plist_id, const char *name, size_t size, void *value,
  ...
  /* Create the new property list class */
  if ((ret_value = H5P_insert(plist, name, size, value, prp_set, prp_get, NULL,
                              NULL, prp_delete, prp_copy, prp_cmp, prp_close)) <
      0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                "unable to register property in plist");
...
htri_t H5Pexist(hid_t id, const char *name) {
  ...
  if (H5I_GENPROP_LST == H5I_get_type(id)) {
    ...
    if ((ret_value = H5P_exist_plist(plist, name)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                  "property does not exist in list");
  } /* end if */
...
/*--------------------------------------------------------------------------
 NAME
    H5Pencode2
 PURPOSE
    Routine to convert the property values in a property list into a binary
buffer. The encoding of property values will be done according to the file
format setting in fapl_id. USAGE herr_t H5Pencode(plist_id, buf, nalloc,
fapl_id) hid_t plist_id;         IN: Identifier to property list to encode void
*buf:              OUT: buffer to gold the encoded plist size_t *nalloc; IN/OUT:
size of buffer needed to encode plist hid_t fapl_id;          IN: File access
property list ID RETURNS Returns non-negative on success, negative on failure.
 DESCRIPTION
...
herr_t H5Pencode2(hid_t plist_id, void *buf, size_t *nalloc, hid_t fapl_id) {
  ...
  if ((ret_value = H5P__encode(plist, TRUE, buf, nalloc)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTENCODE, FAIL,
                "unable to encode property list");
...
/*--------------------------------------------------------------------------
 ...
 DESCRIPTION
     ...
     contain the values for the corresponding properties of the plist. The
decode callback of a certain property decodes its value from the buffer and sets
it in the property list. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS Properties
in the property list that are not encoded in the serialized form retain their
default value. EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
hid_t H5Pdecode(const void *buf) {
  ...
  if ((ret_value = H5P__decode(buf)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTDECODE, H5I_INVALID_HID,
                "unable to decode property list");
...
/*--------------------------------------------------------------------------
 ...
 GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
    Change the name of this function to H5Pget_class (and remove old
H5Pget_class) EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
hid_t H5Pget_class(hid_t plist_id) {
  ...
  if (H5P__access_class(pclass, H5P_MOD_INC_REF) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, H5I_INVALID_HID,
                "Can't increment class ID ref count");
...
herr_t H5Pget_nprops(hid_t id, size_t *nprops /*out*/) {
  ...
  if (nprops == NULL)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid property nprops pointer");
  ...
  if (H5I_GENPROP_LST == H5I_get_type(id)) {
    ...
    if (H5P__get_nprops_plist(plist, nprops) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                  "unable to query # of properties in plist");
  } /* end if */
  else if (H5I_GENPROP_CLS == H5I_get_type(id)) {
    if (NULL == (pclass = (H5P_genclass_t *)H5I_object(id)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a property class");
    if (H5P_get_nprops_pclass(pclass, nprops, FALSE) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                  "unable to query # of properties in pclass");
  } /* end if */
  else
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a property object");
...
/*--------------------------------------------------------------------------
 NAME
    H5Pequal
 PURPOSE
    Routine to query whether two property lists or two property classes are
equal USAGE htri_t H5Pequal(id1, id2) hid_t id1;         IN: Property list or
class ID to compare hid_t id2;         IN: Property list or class ID to compare
 RETURNS
...
htri_t H5Pequal(hid_t id1, hid_t id2) {
  ...
  if ((H5I_GENPROP_LST != H5I_get_type(id1) &&
       H5I_GENPROP_CLS != H5I_get_type(id1)) ||
      (H5I_GENPROP_LST != H5I_get_type(id2) &&
       H5I_GENPROP_CLS != H5I_get_type(id2)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not property objects");
  if (H5I_get_type(id1) != H5I_get_type(id2))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "not the same kind of property objects");
  ...
  if (H5I_GENPROP_LST == H5I_get_type(id1)) {
    ...
    if (H5P__cmp_plist((const H5P_genplist_t *)obj1,
                       (const H5P_genplist_t *)obj2, &cmp_ret) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTCOMPARE, FAIL,
                  "can't compare property lists");
  ...
  /* Must be property classes */
    ...
    if (H5P__cmp_class((const H5P_genclass_t *)obj1,
                       (const H5P_genclass_t *)obj2) == 0)
      ret_value = TRUE;
  } /* end else */
...
int H5Piterate(hid_t id, int *idx, H5P_iterate_t iter_func, void *iter_data) {
  ...
  if (H5I_GENPROP_LST == H5I_get_type(id)) {
    ...
    if ((ret_value = H5P__iterate_plist((H5P_genplist_t *)obj, TRUE,
                                        (idx ? idx : &fake_idx),
  ...
  if (H5I_GENPROP_LST == src_id_type) {
    if (H5P__copy_prop_plist(dst_id, src_id, name) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL,
                  "can't copy property between lists");
  } /* end if */
  /* Must be property classes */
  else {
    if (H5P__copy_prop_pclass(dst_id, src_id, name) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL,
                  "can't copy property between classes");
  } /* end else */
...
herr_t H5Punregister(hid_t pclass_id, const char *name) {
  ...
  if ((ret_value = H5P__unregister(pclass, name)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                "unable to remove property from class");
