...
static hid_t H5A__create_common(H5VL_object_t *vol_obj,
                                ...
                                void **token_ptr);
static hid_t H5A__create_api_common(hid_t loc_id, const char *attr_name,
                                    hid_t type_id, hid_t space_id,
                                    hid_t acpl_id, hid_t aapl_id,
                                    void **token_ptr,
                                    H5VL_object_t **_vol_obj_ptr);
static hid_t H5A__create_by_name_api_common(hid_t loc_id, const char *obj_name,
                                            const char *attr_name,
                                            hid_t type_id, hid_t space_id,
                                            hid_t acpl_id, hid_t aapl_id,
                                            hid_t lapl_id, void **token_ptr,
                                            H5VL_object_t **_vol_obj_ptr);
static hid_t H5A__open_common(H5VL_object_t *vol_obj,
                              H5VL_loc_params_t *loc_params,
                              const char *attr_name, hid_t aapl_id,
                              void **token_ptr);
static hid_t H5A__open_api_common(hid_t loc_id, const char *attr_name,
...
static hid_t H5A__open_by_idx_api_common(hid_t loc_id, const char *obj_name,
                                         ...
                                         void **token_ptr,
                                         H5VL_object_t **_vol_obj_ptr);
static herr_t H5A__write_api_common(hid_t attr_id, hid_t type_id,
                                    const void *buf, void **token_ptr,
                                    H5VL_object_t **_vol_obj_ptr);
static herr_t H5A__read_api_common(hid_t attr_id, hid_t dtype_id, void *buf,
                                   void **token_ptr,
                                   H5VL_object_t **_vol_obj_ptr);
static herr_t H5A__rename_common(H5VL_object_t *vol_obj,
                                 H5VL_loc_params_t *loc_params,
                                 const char *old_name, const char *new_name,
                                 void **token_ptr);
static herr_t H5A__rename_api_common(hid_t loc_id, const char *old_name,
                                     const char *new_name, void **token_ptr,
                                     H5VL_object_t **_vol_obj_ptr);
static herr_t H5A__rename_by_name_api_common(hid_t loc_id, const char *obj_name,
                                             const char *old_attr_name,
                                             const char *new_attr_name,
                                             hid_t lapl_id, void **token_ptr,
                                             H5VL_object_t **_vol_obj_ptr);
static herr_t H5A__exists_common(H5VL_object_t *vol_obj,
                                 H5VL_loc_params_t *loc_params,
                                 const char *attr_name, hbool_t *attr_exists,
                                 void **token_ptr);
static herr_t H5A__exists_api_common(hid_t obj_id, const char *attr_name,
                                     hbool_t *attr_exists, void **token_ptr,
                                     H5VL_object_t **_vol_obj_ptr);
static herr_t H5A__exists_by_name_api_common(hid_t obj_id, const char *obj_name,
                                             const char *attr_name,
                                             hbool_t *attr_exists,
                                             hid_t lapl_id, void **token_ptr,
                                             H5VL_object_t **_vol_obj_ptr);
...
static hid_t H5A__create_common(H5VL_object_t *vol_obj,
  ...
  if (NULL == (attr = H5VL_attr_create(vol_obj, loc_params, attr_name, type_id,
  ...
  if ((ret_value = H5VL_register(H5I_ATTR, attr, vol_obj->connector, TRUE)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTREGISTER, H5I_INVALID_HID,
  ...
  if (H5I_INVALID_HID == ret_value)
    if (attr &&
        H5VL_attr_close(vol_obj, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, H5I_INVALID_HID,
                  "can't close attribute");
...
static hid_t H5A__create_api_common(hid_t loc_id, const char *attr_name,
  ...
  H5VL_object_t **vol_obj_ptr =
      (_vol_obj_ptr ? _vol_obj_ptr
                    : &tmp_vol_obj); /* Ptr to object ptr for loc_id */
  ...
  if (!attr_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "attr_name parameter cannot be NULL");
  if (!*attr_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "attr_name parameter cannot be an empty string");

  /* Set up object access arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_AACC, TRUE, &aapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");
  ...
  /* Create the attribute */
  if ((ret_value =
           H5A__create_common(*vol_obj_ptr, &loc_params, attr_name, type_id,
                              space_id, acpl_id, aapl_id, token_ptr)) < 0)
 ...
 *
 * Parameters:
 *              hid_t loc_id;           IN: Object (dataset or group) to be
 *attached to const char *attr_name;  IN: Name of attribute to locate and open
 *              hid_t type_id;          IN: ID of datatype for attribute
 *              hid_t space_id;         IN: ID of dataspace for attribute
 *              hid_t acpl_id;          IN: ID of creation property list
 *(currently not used) hid_t aapl_id;          IN: Attribute access property
 *list
 *
...
hid_t H5Acreate2(hid_t loc_id, const char *attr_name, hid_t type_id,
  ...
  H5TRACE6("i", "i*siiii", loc_id, attr_name, type_id, space_id, acpl_id,
           aapl_id);

  /* Create the attribute synchronously */
  if ((ret_value = H5A__create_api_common(loc_id, attr_name, type_id, space_id,
                                          acpl_id, aapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to synchronously create attribute");
...
hid_t H5Acreate_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE10("i", "*s*sIui*siiiii", app_file, app_func, app_line, loc_id,
            attr_name, type_id, space_id, acpl_id, aapl_id, es_id);
  ...
  if ((ret_value =
           H5A__create_api_common(loc_id, attr_name, type_id, space_id, acpl_id,
                                  aapl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to asynchronously create attribute");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      if (H5I_dec_app_ref(ret_value) < 0)
        HDONE_ERROR(H5E_ATTR, H5E_CANTDEC, H5I_INVALID_HID,
                    "can't decrement count on attribute ID");
      HGOTO_ERROR(H5E_ATTR, H5E_CANTINSERT, H5I_INVALID_HID,
...
static hid_t H5A__create_by_name_api_common(hid_t loc_id, const char *obj_name,
  ...
  /* Set up object access arguments */
  if (H5VL_setup_name_args(loc_id, obj_name, TRUE, lapl_id, vol_obj_ptr,
                           &loc_params) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");
  ...
  if (H5CX_set_apl(&aapl_id, H5P_CLS_AACC, loc_id, TRUE) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, H5I_INVALID_HID,
                "can't set attribute access property list info");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hid_t H5Acreate_by_name(loc_id, obj_name, attr_name, type_id, space_id,
acpl_id, aapl_id, lapl_id) hid_t loc_id;       IN: Object (dataset or group) to
be attached to const char *obj_name;   IN: Name of object relative to location
        const char *attr_name;  IN: Name of attribute to locate and open
        hid_t type_id;          IN: ID of datatype for attribute
        hid_t space_id;         IN: ID of dataspace for attribute
        hid_t acpl_id;          IN: ID of creation property list (currently not
used) hid_t aapl_id;          IN: Attribute access property list hid_t lapl_id;
IN: Link access property list RETURNS Non-negative on success/H5I_INVALID_HID on
failure
...
hid_t H5Acreate_by_name(hid_t loc_id, const char *obj_name,
  ...
  /* Create the attribute synchronously */
  if ((ret_value = H5A__create_by_name_api_common(
...
hid_t H5Acreate_by_name_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE12("i", "*s*sIui*s*siiiiii", app_file, app_func, app_line, loc_id,
...
static hid_t H5A__open_common(H5VL_object_t *vol_obj,
  ...
  /* Open the attribute */
  if (NULL == (attr = H5VL_attr_open(vol_obj, loc_params, attr_name, aapl_id,
                                     H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open attribute: '%s'", attr_name);
...
static hid_t H5A__open_api_common(hid_t loc_id, const char *attr_name,
  ...
  if (!*attr_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "name parameter cannot be an empty string");

  /* Set up object access arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_AACC, FALSE, &aapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");

  /* Open the attribute */
  if ((ret_value = H5A__open_common(*vol_obj_ptr, &loc_params, attr_name,
                                    aapl_id, token_ptr)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open attribute: '%s'", attr_name);
...
hid_t H5Aopen(hid_t loc_id, const char *attr_name, hid_t aapl_id) {
  ...
  if ((ret_value =
           H5A__open_api_common(loc_id, attr_name, aapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to synchronously open attribute");
...
hid_t H5Aopen_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE7("i", "*s*sIui*sii", app_file, app_func, app_line, loc_id, attr_name,
           aapl_id, es_id);
  ...
  if ((ret_value = H5A__open_api_common(loc_id, attr_name, aapl_id, token_ptr,
                                        &vol_obj)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to asynchronously open attribute");
...
static hid_t H5A__open_by_name_api_common(hid_t loc_id, const char *obj_name,
  ...
  /* obj_name is verified in H5VL_setup_name_args() */
  /* Set up object access arguments */
  if (H5VL_setup_name_args(loc_id, obj_name, FALSE, lapl_id, vol_obj_ptr,
                           &loc_params) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");
...
hid_t H5Aopen_by_name(hid_t loc_id, const char *obj_name, const char *attr_name,
  ...
  if ((ret_value = H5A__open_by_name_api_common(
           loc_id, obj_name, attr_name, aapl_id, lapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to synchronously open attribute");
...
hid_t H5Aopen_by_name_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE9("i", "*s*sIui*s*siii", app_file, app_func, app_line, loc_id,
           obj_name, attr_name, aapl_id, lapl_id, es_id);
  ...
  /* Open the attribute by name asynchronously */
  if ((ret_value =
           H5A__open_by_name_api_common(loc_id, obj_name, attr_name, aapl_id,
                                        lapl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to asynchronously open attribute");
...
static hid_t H5A__open_by_idx_api_common(hid_t loc_id, const char *obj_name,
  ...
  if (order <= H5_ITER_UNKNOWN || order >= H5_ITER_N)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "invalid iteration order specified");

  /* Set up object access arguments */
  if (H5VL_setup_idx_args(loc_id, obj_name, idx_type, order, n, FALSE, lapl_id,
                          vol_obj_ptr, &loc_params) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");
  ...
  /* Open the attribute */
  if ((ret_value = H5A__open_common(*vol_obj_ptr, &loc_params, NULL, aapl_id,
                                    token_ptr)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
...
hid_t H5Aopen_by_idx(hid_t loc_id, const char *obj_name, H5_index_t idx_type,
  ...
  if ((ret_value =
           H5A__open_by_idx_api_common(loc_id, obj_name, idx_type, order, n,
...
hid_t H5Aopen_by_idx_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE11("i", "*s*sIui*sIiIohiii", app_file, app_func, app_line, loc_id,
            obj_name, idx_type, order, n, aapl_id, lapl_id, es_id);
  ...
  if ((ret_value = H5A__open_by_idx_api_common(loc_id, obj_name, idx_type,
                                               order, n, aapl_id, lapl_id,
                                               token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to asynchronously open attribute");
...
static herr_t H5A__write_api_common(hid_t attr_id, hid_t type_id,
  ...
  if (H5VL_setup_args(attr_id, H5I_ATTR, vol_obj_ptr) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                "can't get VOL object for attribute");

  /* Write the attribute data */
  if (H5VL_attr_write(*vol_obj_ptr, type_id, buf, H5P_DATASET_XFER_DEFAULT,
                      token_ptr) < 0)
...
herr_t H5Awrite_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "*s*sIuii*xi", app_file, app_func, app_line, attr_id, dtype_id,
           buf, es_id);
  ...
  if (H5A__write_api_common(attr_id, dtype_id, buf, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_WRITEERROR, FAIL,
                "can't asynchronously write data");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      /* clang-format on */
      HGOTO_ERROR(H5E_ATTR, H5E_CANTINSERT, FAIL,
                  "can't insert token into event set");
...
static herr_t H5A__read_api_common(hid_t attr_id, hid_t dtype_id, void *buf,
  ...
  if (NULL ==
      (*vol_obj_ptr = (H5VL_object_t *)H5I_object_verify(attr_id, H5I_ATTR)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not an attribute");

  /* Read the attribute data */
  if (H5VL_attr_read(*vol_obj_ptr, dtype_id, buf, H5P_DATASET_XFER_DEFAULT,
                     token_ptr) < 0)
...
herr_t H5Aread_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "*s*sIuiixi", app_file, app_func, app_line, attr_id, dtype_id,
           buf, es_id);
  ...
  if (H5A__read_api_common(attr_id, dtype_id, buf, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_READERROR, FAIL,
                "can't asynchronously read data");
...
hid_t H5Aget_space(hid_t attr_id) {
  ...
  /* Get the dataspace */
  if (H5VL_attr_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    H5_REQUEST_NULL) < 0)
...
ssize_t H5Aget_name(hid_t attr_id, size_t buf_size, char *buf /*out*/) {
  ...
  if (!buf && buf_size)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, (-1),
                "buf cannot be NULL if buf_size is non-zero");
...
ssize_t H5Aget_name_by_idx(hid_t loc_id, const char *obj_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE8("Zs", "i*sIiIohxzi", loc_id, obj_name, idx_type, order, n, name,
           size, lapl_id);
  ...
  if (!name && size)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "name cannot be NULL if size is non-zero");
  if (idx_type <= H5_INDEX_UNKNOWN || idx_type >= H5_INDEX_N)
  ...
  if (H5CX_set_apl(&lapl_id, H5P_CLS_LACC, loc_id, FALSE) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, FAIL,
                "can't set access property list info");
...
herr_t H5Aget_info(hid_t attr_id, H5A_info_t *ainfo /*out*/) {
  ...
  if (!ainfo)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "attribute_info parameter cannot be NULL");
...
herr_t H5Aget_info_by_idx(hid_t loc_id, const char *obj_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "i*sIiIohxi", loc_id, obj_name, idx_type, order, n, ainfo,
           lapl_id);
...
static herr_t H5A__rename_common(H5VL_object_t *vol_obj,
  ...
  if (HDstrcmp(old_name, new_name) != 0) {
    ...
    if (H5VL_attr_specific(vol_obj, loc_params, &vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, token_ptr) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTRENAME, FAIL,
                  "can't rename attribute from '%s' to '%s'", old_name,
                  new_name);
  }
...
static herr_t H5A__rename_api_common(hid_t loc_id, const char *old_name,
  ...
  if (!old_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "old attribute name cannot be NULL");
  if (!*old_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "old attribute name cannot be an empty string");
  if (!new_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "new attribute name cannot be NULL");
  if (!*new_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "new attribute name cannot be an empty string");
  ...
  /* Rename the attribute */
  if (H5A__rename_common(*vol_obj_ptr, &loc_params, old_name, new_name,
                         token_ptr) < 0)
...
herr_t H5Arename(hid_t loc_id, const char *old_name, const char *new_name) {
  ...
  if (H5A__rename_api_common(loc_id, old_name, new_name, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTRENAME, FAIL,
                "can't synchronously rename attribute");
...
herr_t H5Arename_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "*s*sIui*s*si", app_file, app_func, app_line, loc_id, old_name,
           new_name, es_id);
  ...
  if (H5A__rename_api_common(loc_id, old_name, new_name, token_ptr, &vol_obj) <
      0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTRENAME, FAIL,
                "can't asynchronously rename attribute");
...
herr_t H5Arename_by_name(hid_t loc_id, const char *obj_name,
  ...
  /* Synchronously rename the attribute */
  if (H5A__rename_by_name_api_common(loc_id, obj_name, old_attr_name,
                                     new_attr_name, lapl_id, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTRENAME, FAIL,
                "can't synchronously rename attribute");
...
herr_t H5Arename_by_name_async(const char *app_file, const char *app_func,
                               unsigned app_line, hid_t loc_id,
                               const char *obj_name, const char *old_attr_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE9("e", "*s*sIui*s*s*sii", app_file, app_func, app_line, loc_id,
           obj_name, old_attr_name, new_attr_name, lapl_id, es_id);
  ...
  /* Asynchronously rename the attribute */
  if (H5A__rename_by_name_api_common(loc_id, obj_name, old_attr_name,
                                     new_attr_name, lapl_id, token_ptr,
                                     &vol_obj) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTRENAME, FAIL,
                "can't synchronously rename attribute");
...
herr_t H5Aiterate2(hid_t loc_id, H5_index_t idx_type, H5_iter_order_t order,
                   hsize_t *idx /*in,out */, H5A_operator2_t op,
                   void *op_data) {
  H5VL_object_t *vol_obj = NULL; /* Object for loc_id */
  H5VL_loc_params_t loc_params;  /* Location parameters for object access */
  H5VL_attr_specific_args_t vol_cb_args; /* Arguments to VOL callback */
  ...
  if ((ret_value =
           H5VL_attr_specific(vol_obj, &loc_params, &vol_cb_args,
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Calls a user's function for each attribute on an object
 USAGE
    herr_t H5Aiterate2(loc_id, obj_name, idx_type, order, idx, op, op_data,
lapl_id) hid_t loc_id;           IN: Base location for object const char
*obj_name;   IN: Name of object relative to location H5_index_t idx_type;    IN:
Type of index to use H5_iter_order_t order;  IN: Order to iterate over index
        hsize_t *idx;           IN/OUT: Starting (IN) & Ending (OUT) attribute
                                    in index & order
        H5A_operator2_t op;     IN: User's function to pass each attribute to
        void *op_data;          IN/OUT: User's data to pass through to iterator
 ...
 DESCRIPTION
    ...
    over ('loc_id'), the name of the current attribute about the object
    ('attr_name'), the attribute's "info" struct ('ainfo') and the pointer to
    the operator data passed in to H5Aiterate_by_name ('op_data').  The return
values from an operator are: A. Zero causes the iterator to continue, returning
zero when all attributes have been processed. B. Positive causes the iterator to
immediately return that positive value, indicating short-circuit success.  The
iterator can be restarted at the next attribute. C. Negative causes the iterator
to immediately return that value, indicating failure.  The iterator can be
restarted at the next attribute.
--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5Adelete(loc_id, name)
        hid_t loc_id;       IN: Object (dataset or group) to have attribute
deleted from const char *name;   IN: Name of attribute to delete RETURNS
    Non-negative on success/Negative on failure
 DESCRIPTION
...
herr_t H5Adelete(hid_t loc_id, const char *name) {
  ...
  if (H5CX_set_loc(loc_id) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, FAIL,
                "can't set collective metadata read");
...
herr_t H5Aclose(hid_t attr_id) {
  ...
  if (H5I_dec_app_ref(attr_id) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTDEC, FAIL,
                "decrementing attribute ID failed");
...
herr_t H5Aclose_async(const char *app_file, const char *app_func,
                      unsigned app_line, hid_t attr_id, hid_t es_id) {
  H5VL_object_t *vol_obj = NULL; /* Object for loc_id */
...
static herr_t H5A__exists_api_common(hid_t obj_id, const char *attr_name,
  ...
  if (NULL == attr_exists)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid pointer for attribute existence");
  ...
  /* Check if the attribute exists */
  if (H5A__exists_common(*vol_obj_ptr, &loc_params, attr_name, attr_exists,
                         token_ptr) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                "unable to determine if attribute exists");
...
htri_t H5Aexists(hid_t obj_id, const char *attr_name) {
  ...
  if (H5A__exists_api_common(obj_id, attr_name, &exists, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                "can't synchronously check if attribute exists");
...
herr_t H5Aexists_async(const char *app_file, const char *app_func,
                       ...
                       hbool_t *attr_exists, hid_t es_id) {
  H5VL_object_t *vol_obj = NULL; /* Object for loc_id */
  void *token = NULL;            /* Request token for async operation        */
  void **token_ptr =
      H5_REQUEST_NULL; /* Pointer to request token for async operation        */
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "*s*sIui*s*bi", app_file, app_func, app_line, obj_id, attr_name,
           attr_exists, es_id);
  ...
  if (H5A__exists_api_common(obj_id, attr_name, attr_exists, token_ptr,
                             &vol_obj) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                "can't asynchronously check if attribute exists");
 ...
 *      Non-negative on success/Negative on failure
 *--------------------------------------------------------------------------*/
static herr_t H5A__exists_by_name_api_common(hid_t loc_id, const char *obj_name,
                                             const char *attr_name,
                                             hbool_t *attr_exists,
                                             hid_t lapl_id, void **token_ptr,
                                             H5VL_object_t **_vol_obj_ptr) {
...
htri_t H5Aexists_by_name(hid_t loc_id, const char *obj_name,
  ...
  /* Synchronously check if an attribute exists */
  exists = FALSE;
  if (H5A__exists_by_name_api_common(loc_id, obj_name, attr_name, &exists,
                                     lapl_id, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                "can't synchronously determine if attribute exists by name");
...
herr_t H5Aexists_by_name_async(const char *app_file, const char *app_func,
                               unsigned app_line, hid_t loc_id,
                               const char *obj_name, const char *attr_name,
                               hbool_t *attr_exists, hid_t lapl_id,
                               hid_t es_id) {
  H5VL_object_t *vol_obj = NULL; /* Object for loc_id */
  void *token = NULL;            /* Request token for async operation        */
  void **token_ptr =
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE9("e", "*s*sIui*s*s*bii", app_file, app_func, app_line, loc_id,
           obj_name, attr_name, attr_exists, lapl_id, es_id);
  ...
  /* Asynchronously check if an attribute exists */
  if (H5A__exists_by_name_api_common(loc_id, obj_name, attr_name, attr_exists,
                                     lapl_id, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
