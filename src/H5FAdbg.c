 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FA__hdr_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
  ...
  if (dbg_ctx && cls->dst_dbg_ctx(dbg_ctx) < 0)
    HDONE_ERROR(H5E_FARRAY, H5E_CANTRELEASE, FAIL,
                "unable to release fixed array debugging context");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FA__dblock_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                          int fwidth, const H5FA_class_t *cls, haddr_t hdr_addr,
                          haddr_t obj_addr) {
  /* Local variables */
  H5FA_hdr_t *hdr = NULL;       /* Shared fixed array header */
  ...
  /* Load the fixed array header */
  if (NULL ==
      (hdr = H5FA__hdr_protect(f, hdr_addr, dbg_ctx, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTPROTECT, FAIL,
                "unable to load fixed array header");
  ...
  fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
          "Array class ID:", hdr->cparam.cls->name);
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "Address of Data Block:", dblock->addr);
  fprintf(stream, "%*s%-*s %" PRIuHSIZE "\n", indent, "", fwidth,
  ...
  if (dblock->npages) {      /* paging */
    ...
    for (page_idx = 0; page_idx < dblock->npages; page_idx++) {
      if (!H5VM_bit_get(dblock->dblk_page_init, page_idx)) {
        ...
        hsize_t
            nelmts_left; /* Remaining elements in the last data block page */
