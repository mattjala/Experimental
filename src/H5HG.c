...
static size_t H5HG__alloc(H5F_t *f, H5HG_heap_t *heap, size_t size,
                          unsigned *heap_flags_ptr);
...
H5HG_heap_t *H5HG__protect(H5F_t *f, haddr_t addr, unsigned flags) {
  ...
  if (NULL ==
      (heap = (H5HG_heap_t *)H5AC_protect(f, H5AC_GHEAP, addr, f, flags)))
...
herr_t H5HG_extend(H5F_t *f, haddr_t addr, size_t need) {
  ...
  if (NULL == (new_chunk = H5FL_BLK_REALLOC(gheap_chunk, heap->chunk,
                                            (heap->size + need))))
  ...
  if (H5AC_resize_entry(heap, heap->size) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTRESIZE, FAIL,
                "unable to resize global heap in cache");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HG_insert(H5F_t *f, size_t size, const void *obj,
                   H5HG_t *hobj /*out*/) {
  ...
  if (0 == (idx = H5HG__alloc(f, heap, size, &heap_flags)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL,
                "unable to allocate global heap object");
 ...
 *-------------------------------------------------------------------------
 */
void *H5HG_read(H5F_t *f, H5HG_t *hobj, void *object /*out*/,
                size_t *buf_size) {
  ...
  if (heap &&
      H5AC_unprotect(f, H5AC_GHEAP, hobj->addr, heap, H5AC__NO_FLAGS_SET) < 0)
...
herr_t H5HG_remove(H5F_t *f, H5HG_t *hobj) {
  ...
  /* When the application selects the same location to rewrite the VL element by
   * using H5Sselect_elements, it can happen that the entry has been removed by
   * first rewrite.  Here we simply skip the removal of the entry and let the
   * second rewrite happen (see HDFFV-10635).  In the future, it'd be nice to
   * handle this situation in H5T_conv_vlen in H5Tconv.c instead of this level
   * (HDFFV-10648). */
  if (heap->obj[hobj->idx].nrefs == 0 && heap->obj[hobj->idx].size == 0 &&
      !heap->obj[hobj->idx].begin)
  ...
  if (NULL == heap->obj[0].begin) {
    ...
    heap->obj[0].size += need;
  memmove(obj_start, obj_start + need,
  ...
  if ((heap->obj[0].size + H5HG_SIZEOF_HDR(f)) == heap->size) {
     ...
     */
    flags |= H5AC__DELETED_FLAG |
             H5AC__FREE_FILE_SPACE_FLAG; /* Indicate that the object was
                                            deleted, for the unprotect call */
  }                                      /* end if */
  else {
    /*
...
herr_t H5HG__free(H5HG_heap_t *heap) {
  ...
  if (H5F_cwfs_remove_heap(heap->shared, heap) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTREMOVE, FAIL,
                "can't remove heap from file's CWFS");
