...
typedef struct H5FD_stdio_t {
  ...
  unsigned
      write_access; /* Flag to indicate the file was opened with write access */
...
static H5FD_t *H5FD_stdio_open(const char *name, unsigned flags, hid_t fapl_id,
                               haddr_t maxaddr);
...
static haddr_t H5FD_stdio_alloc(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                                hsize_t size);
...
static herr_t H5FD_stdio_get_handle(H5FD_t *_file, hid_t fapl,
                                    void **file_handle);
static herr_t H5FD_stdio_read(H5FD_t *lf, H5FD_mem_t type, hid_t fapl_id,
                              haddr_t addr, size_t size, void *buf);
static herr_t H5FD_stdio_write(H5FD_t *lf, H5FD_mem_t type, hid_t fapl_id,
                               haddr_t addr, size_t size, const void *buf);
static herr_t H5FD_stdio_flush(H5FD_t *_file, hid_t dxpl_id, hbool_t closing);
static herr_t H5FD_stdio_truncate(H5FD_t *_file, hid_t dxpl_id,
                                  hbool_t closing);
...
hid_t H5FD_stdio_init(void) {
  ...
  if (lock_env_var && !strcmp(lock_env_var, "BEST_EFFORT"))
    ignore_disabled_file_locks_s = 1; /* Override: Ignore disabled locks */
           ...
           (!strcmp(lock_env_var, "TRUE") || !strcmp(lock_env_var, "1")))
    ignore_disabled_file_locks_s =
        0; /* Override: Don't ignore disabled locks */
  else
    ignore_disabled_file_locks_s =
        -1; /* Environment variable not set, or not set correctly */
...
herr_t H5Pset_fapl_stdio(hid_t fapl_id) {
  ...
  if (0 == H5Pisa_class(fapl_id, H5P_FILE_ACCESS))
    H5Epush_ret(func, H5E_ERR_CLS, H5E_PLIST, H5E_BADTYPE,
...
static H5FD_t *H5FD_stdio_open(const char *name, unsigned flags, hid_t fapl_id,
  ...
  static const char *func =
      "H5FD_stdio_open"; /* Function Name for error reporting */
#ifdef H5_HAVE_WIN32_API
  ...
  if (ADDR_OVERFLOW(maxaddr))
    H5Epush_ret(func, H5E_ERR_CLS, H5E_ARGS, H5E_OVERFLOW, "maxaddr too large",
                NULL);
  ...
  if (!f)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_IO, H5E_CANTOPENFILE, "fopen failed",
                NULL);

  /* Build the return value */
  if (NULL ==
      (file = (H5FD_stdio_t *)calloc((size_t)1, sizeof(H5FD_stdio_t)))) {
    fclose(f);
  ...
  if (ignore_disabled_file_locks_s != -1)
    ...
    if (H5Pget_file_locking(fapl_id, &unused,
  ...
  if (file->fd < 0) {
    ...
    H5Epush_ret(func, H5E_ERR_CLS, H5E_FILE, H5E_CANTOPENFILE,
                "unable to get file descriptor", NULL);
  } /* end if */
  ...
  if (INVALID_HANDLE_VALUE == file->hFile) {
    ...
    fclose(f);
    H5Epush_ret(func, H5E_ERR_CLS, H5E_FILE, H5E_CANTOPENFILE,
                "unable to get Windows file handle", NULL);
  } /* end if */
  ...
  if (fstat(file->fd, &sb) < 0) {
    free(file);
    fclose(f);
    H5Epush_ret(func, H5E_ERR_CLS, H5E_FILE, H5E_BADFILE,
                "unable to fstat file", NULL);
  } /* end if */
...
static herr_t H5FD_stdio_close(H5FD_t *_file) {
  ...
  static const char *func =
      "H5FD_stdio_close"; /* Function Name for error reporting */
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD_stdio_alloc(H5FD_t *_file, H5FD_mem_t /*UNUSED*/ type,
                                hid_t /*UNUSED*/ dxpl_id, hsize_t size) {
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file;
  haddr_t addr;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_set_eoa(H5FD_t *_file, H5FD_mem_t /*UNUSED*/ type,
                                 haddr_t addr) {
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_get_handle(H5FD_t *_file, hid_t /*UNUSED*/ fapl,
                                    void **file_handle) {
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file;
  static const char *func =
      "H5FD_stdio_get_handle"; /* Function Name for error reporting */
  ...
  *file_handle = &(file->fp);
  if (*file_handle == NULL)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_IO, H5E_WRITEERROR, "get handle failed",
                -1);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_read(H5FD_t *_file, H5FD_mem_t /*UNUSED*/ type,
                              hid_t /*UNUSED*/ dxpl_id, haddr_t addr,
                              size_t size, void /*OUT*/ *buf) {
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file;
  static const char *func =
      "H5FD_stdio_read"; /* Function Name for error reporting */
  ...
  /* Check for overflow */
  if (HADDR_UNDEF == addr)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_IO, H5E_OVERFLOW,
                "file address overflowed", -1);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_write(H5FD_t *_file, H5FD_mem_t /*UNUSED*/ type,
                               hid_t /*UNUSED*/ dxpl_id, haddr_t addr,
                               size_t size, const void *buf) {
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file;
  static const char *func =
      "H5FD_stdio_write"; /* Function Name for error reporting */
  ...
  while (size > 0) {
    ...
    if (bytes_wrote != bytes_in ||
        (0 == bytes_wrote && ferror(file->fp))) { /* error */
      file->op = H5FD_STDIO_OP_UNKNOWN;
      file->pos = HADDR_UNDEF;
      H5Epush_ret(func, H5E_ERR_CLS, H5E_IO, H5E_WRITEERROR, "fwrite failed",
                  -1);
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_flush(H5FD_t *_file, hid_t /*UNUSED*/ dxpl_id,
                               hbool_t closing) {
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file;
  static const char *func =
      "H5FD_stdio_flush"; /* Function Name for error reporting */
  ...
  if (file->write_access) {
    if (!closing) {
      if (fflush(file->fp) < 0)
        H5Epush_ret(func, H5E_ERR_CLS, H5E_IO, H5E_WRITEERROR, "fflush failed",
                    -1);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_truncate(H5FD_t *_file, hid_t /*UNUSED*/ dxpl_id,
                                  hbool_t /*UNUSED*/ closing) {
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file;
  static const char *func =
      "H5FD_stdio_truncate"; /* Function Name for error reporting */
  ...
  if (file->write_access) {
    ...
    if (file->eoa != file->eof) {
      ...
      /* Reset seek offset to beginning of file, so that file isn't re-extended
       * later */
      rewind(file->fp);
      ...
      if (INVALID_SET_FILE_POINTER == dwPtrLow) {
        ...
        if (dwError != NO_ERROR)
          H5Epush_ret(func, H5E_ERR_CLS, H5E_FILE, H5E_FILEOPEN,
                      "unable to set file pointer", -1);
      }
...
static herr_t H5FD_stdio_lock(H5FD_t *_file, hbool_t rw) {
  ...
  static const char *func =
      "H5FD_stdio_lock"; /* Function Name for error reporting    */
...
static herr_t H5FD_stdio_unlock(H5FD_t *_file) {
  ...
  H5FD_stdio_t *file = (H5FD_stdio_t *)_file; /* VFD file struct */
  static const char *func =
      "H5FD_stdio_unlock"; /* Function Name for error reporting    */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_stdio_delete(const char *filename,
                                hid_t /*UNUSED*/ fapl_id) {
  static const char *func =
      "H5FD_stdio_delete"; /* Function Name for error reporting    */
  ...
  if (remove(filename) < 0)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_VFL, H5E_CANTDELETEFILE,
                "can't delete file)", -1);
