...
static herr_t H5T__ref_mem_isnull(const H5VL_object_t *src_file,
                                  const void *src_buf, hbool_t *isnull);
static herr_t H5T__ref_mem_setnull(H5VL_object_t *dst_file, void *dst_buf,
                                   void *bg_buf);
static size_t H5T__ref_mem_getsize(H5VL_object_t *src_file, const void *src_buf,
                                   size_t src_size, H5VL_object_t *dst_file,
                                   hbool_t *dst_copy);
static herr_t H5T__ref_mem_read(H5VL_object_t *src_file, const void *src_buf,
                                size_t src_size, H5VL_object_t *dst_file,
                                void *dst_buf, size_t dst_size);
static herr_t H5T__ref_mem_write(H5VL_object_t *src_file, const void *src_buf,
                                 size_t src_size, H5R_type_t src_type,
                                 H5VL_object_t *dst_file, void *dst_buf,
                                 size_t dst_size, void *bg_buf);

static herr_t H5T__ref_disk_isnull(const H5VL_object_t *src_file,
                                   const void *src_buf, hbool_t *isnull);
static herr_t H5T__ref_disk_setnull(H5VL_object_t *dst_file, void *dst_buf,
                                    void *bg_buf);
static size_t H5T__ref_disk_getsize(H5VL_object_t *src_file,
                                    const void *src_buf, size_t src_size,
                                    H5VL_object_t *dst_file, hbool_t *dst_copy);
static herr_t H5T__ref_disk_read(H5VL_object_t *src_file, const void *src_buf,
                                 size_t src_size, H5VL_object_t *dst_file,
                                 void *dst_buf, size_t dst_size);
static herr_t H5T__ref_disk_write(H5VL_object_t *src_file, const void *src_buf,
                                  size_t src_size, H5R_type_t src_type,
                                  H5VL_object_t *dst_file, void *dst_buf,
                                  size_t dst_size, void *bg_buf);
...
static herr_t H5T__ref_dsetreg_disk_isnull(const H5VL_object_t *src_file,
                                           const void *src_buf,
                                           hbool_t *isnull);
static size_t H5T__ref_dsetreg_disk_getsize(H5VL_object_t *src_file,
                                            const void *src_buf,
                                            size_t src_size,
                                            H5VL_object_t *dst_file,
                                            hbool_t *dst_copy);
...
htri_t H5T__ref_set_loc(H5T_t *dt, H5VL_object_t *file, H5T_loc_t loc) {
  ...
  if (loc == dt->shared->u.atomic.u.r.loc &&
      file == dt->shared->u.atomic.u.r.file)
    HGOTO_DONE(FALSE);
  ...
  case H5T_LOC_MEMORY: /* Memory based reference datatype */
    ...
    if (dt->shared->owned_vol_obj) {
      if (H5VL_free_object(dt->shared->owned_vol_obj) < 0)
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCLOSEOBJ, FAIL,
  ...
  case H5T_LOC_DISK: /* Disk based reference datatype */
    ...
    if (dt->shared->u.atomic.u.r.rtype == H5R_OBJECT1) {
      ...
      {
        hbool_t is_native =
            FALSE; /* Whether the file is using the native VOL connector */
      ...
      /* Size on disk, memory size is different */
      dt->shared->size = MAX(sizeof(uint32_t) + H5R_ENCODE_HEADER_SIZE +
                                 cont_info.blob_id_size,
                             ref_encode_size);
      dt->shared->u.atomic.prec = 8 * dt->shared->size;
  ...
  default:
    HGOTO_ERROR(H5E_DATATYPE, H5E_BADRANGE, FAIL,
                "invalid reference datatype location");
  } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5T__ref_mem_isnull(const H5VL_object_t H5_ATTR_UNUSED *src_file,
                                  const void *src_buf, hbool_t *isnull) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5T__ref_mem_setnull(H5VL_object_t H5_ATTR_UNUSED *dst_file,
                                   void *dst_buf, H5_ATTR_UNUSED void *bg_buf) {
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5T__ref_mem_getsize(H5VL_object_t H5_ATTR_UNUSED *src_file,
                                   const void *src_buf,
  ...
  char *file_name_buf_dyn = NULL; /* Pointer to dynamically allocated buffer for
                                     file name, if static buffer is too small */
  ...
  if (NULL != dst_file) {
    hbool_t files_equal =
        TRUE; /* Whether src & dst references are in same file */
    ...
    if (H5VL_file_is_same(vol_obj, dst_file, &files_equal) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCOMPARE, 0,
                  "can't check if files are equal");
  ...
  if (flags || !src_ref->encode_size) {
    ...
    if (src_ref->type == (int8_t)H5R_DATASET_REGION2) {
      hbool_t is_native =
          FALSE; /* Whether the dest. file is using the native VOL connector */
    ...
    /* Get file name */
    if (H5VL_file_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT, NULL) <
        0)
    ...
    if (file_name_len >= sizeof(file_name_buf_static)) {
      ...
      if (NULL == (file_name_buf_dyn = H5MM_malloc(file_name_len + 1)))
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTALLOC, 0,
                    "can't allocate space for file name");
    ...
    if (H5R__encode(file_name, src_ref, NULL, &ret_value, flags) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTENCODE, 0,
...
static herr_t H5T__ref_mem_read(H5VL_object_t H5_ATTR_UNUSED *src_file,
  ...
  /* Encode reference */
  if (H5R__encode(file_name, src_ref, (unsigned char *)dst_buf, &dst_size,
                  flags) < 0)
...
static herr_t H5T__ref_mem_write(H5VL_object_t *src_file, const void *src_buf,
  ...
  if ((src_type == H5R_OBJECT1) || (src_type == H5R_DATASET_REGION1)) {
    hbool_t is_native =
        FALSE; /* Whether the src file is using the native VOL connector */
  ...
  case H5R_OBJECT1: {
    ...
    if (H5R__create_object((const H5O_token_t *)src_buf, token_size, &tmp_ref) <
        0)
  ...
  case H5R_DATASET_REGION1: {
    const struct H5Tref_dsetreg *src_reg =
        (const struct H5Tref_dsetreg *)src_buf;
    size_t token_size = H5F_SIZEOF_ADDR(src_f);

    if (H5R__create_region(&src_reg->token, token_size, src_reg->space,
                           &tmp_ref) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCREATE, FAIL,
  ...
  if ((file_id != H5I_INVALID_HID) && (H5I_dec_ref(file_id) < 0))
    HDONE_ERROR(H5E_REFERENCE, H5E_CANTDEC, FAIL,
                "unable to decrement refcount on location id");
  FUNC_LEAVE_NOAPI(ret_value)
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5T__ref_disk_getsize(H5VL_object_t H5_ATTR_UNUSED *src_file,
                                    const void *src_buf, size_t src_size,
                                    H5VL_object_t H5_ATTR_UNUSED *dst_file,
                                    hbool_t *dst_copy) {
  const uint8_t *p = (const uint8_t *)src_buf;
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5T__ref_dsetreg_disk_getsize(
    H5VL_object_t H5_ATTR_UNUSED *src_file, const void H5_ATTR_UNUSED *src_buf,
...
static herr_t H5T__ref_dsetreg_disk_read(H5VL_object_t *src_file,
  ...
  if (H5R__decode_token_region_compat(
          src_f, (const unsigned char *)src_buf, &src_size, &dst_reg->token,
