...
#define H5D_CRT_ALLOC_TIME_STATE_NAME                                          \
  "alloc_time_state"                     /* Space allocation time state */
#define H5D_CRT_EXT_FILE_LIST_NAME "efl" /* External file list */
#define H5D_CRT_MIN_DSET_HDR_SIZE_NAME                                         \
  "dset_oh_minimize" /* Minimize object header */

/* ========  Dataset access property names ======== */
#define H5D_ACS_DATA_CACHE_NUM_SLOTS_NAME                                      \
  "rdcc_nslots" /* Size of raw data chunk cache(slots) */
#define H5D_ACS_DATA_CACHE_BYTE_SIZE_NAME                                      \
  "rdcc_nbytes" /* Size of raw data chunk cache(bytes) */
#define H5D_ACS_PREEMPT_READ_CHUNKS_NAME                                       \
...
#define H5D_ACS_EFILE_PREFIX_NAME                                              \
  "external file prefix" /* External file prefix */
...
#define H5D_XFER_BKGR_BUF_TYPE_NAME "bkgr_buf_type" /* Background buffer type  \
                                                     */
#define H5D_XFER_BTREE_SPLIT_RATIO_NAME                                        \
  "btree_split_ratio"                         /* B-tree node splitting ratio */
#define H5D_XFER_VLEN_ALLOC_NAME "vlen_alloc" /* Vlen allocation function */
#define H5D_XFER_VLEN_ALLOC_INFO_NAME                                          \
  "vlen_alloc_info"                                   /* Vlen allocation info */
...
#define H5D_XFER_MPIO_COLLECTIVE_OPT_NAME                                      \
  "mpio_collective_opt" /* Optimization of MPI-IO transfer mode */
...
#define H5D_MPIO_LOCAL_NO_COLLECTIVE_CAUSE_NAME                                \
  "local_no_collective_cause" /* cause of broken collective I/O in each        \
                                 process */
#define H5D_MPIO_GLOBAL_NO_COLLECTIVE_CAUSE_NAME                               \
  "global_no_collective_cause" /* cause of broken collective I/O in all        \
                                  processes */
#define H5D_XFER_EDC_NAME "err_detect"      /* EDC */
#define H5D_XFER_FILTER_CB_NAME "filter_cb" /* Filter callback function */
#define H5D_XFER_CONV_CB_NAME                                                  \
  "type_conv_cb" /* Type conversion callback function */
#define H5D_XFER_XFORM_NAME "data_transform" /* Data transform */
#define H5D_XFER_DSET_IO_SEL_NAME                                              \
  "dset_io_selection" /* Dataset I/O selection */
#define H5D_XFER_SELECTION_IO_MODE_NAME                                        \
  "selection_io_mode" /* Selection I/O mode */
#define H5D_XFER_NO_SELECTION_IO_CAUSE_NAME                                    \
  "no_selection_io_cause" /* Cause for no selection I/O */
#define H5D_XFER_MODIFY_WRITE_BUF_NAME                                         \
  "modify_write_buf" /* Modify write buffers */
...
typedef struct H5D_copy_file_ud_t {
  ...
  struct H5S_extent_t
      *src_space_extent; /* Copy of dataspace extent for dataset */
...
typedef struct H5D_append_flush_t {
  ...
  hsize_t
      boundary[H5S_MAX_RANK]; /* The dimension sizes for determining boundary */
...
H5_DLL herr_t H5D_virtual_parse_source_name(
    const char *source_name, H5O_storage_virtual_name_seg_t **parsed_name,
    size_t *static_strlen, size_t *nsubs);
H5_DLL herr_t
H5D_virtual_free_parsed_name(H5O_storage_virtual_name_seg_t *name_seg);

/* Functions that operate on indexed storage */
H5_DLL herr_t H5D_btree_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
