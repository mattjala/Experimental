...
/* Declare a free list to manage the H5HF_indirect_filt_ent_t sequence
 * information */
...
static herr_t H5HF__iblock_pin(H5HF_indirect_t *iblock) {
  ...
  if (H5AC_pin_protected_entry(iblock) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTPIN, FAIL,
                "unable to pin fractal heap indirect block");
  ...
  if (iblock->parent) {
    ...
    assert(iblock->par_entry >= (iblock->hdr->man_dtable.max_direct_rows *
...
static herr_t H5HF__iblock_unpin(H5HF_indirect_t *iblock) {
  ...
  if (H5AC_unpin_entry(iblock) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPIN, FAIL,
                "unable to unpin fractal heap indirect block");
    ...
    if (iblock->parent) {
      ...
      if (iblock->block_off == 0) {
        ...
        iblock->hdr->root_iblock_flags &=
            (unsigned)(~(H5HF_ROOT_IBLOCK_PINNED));
      } /* end if */
    }   /* end else */
...
herr_t H5HF__iblock_dirty(H5HF_indirect_t *iblock) {
  ...
  if (H5AC_mark_entry_dirty(iblock) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTMARKDIRTY, FAIL,
                "unable to mark fractal heap indirect block as dirty");
...
herr_t H5HF__man_iblock_root_create(H5HF_hdr_t *hdr, size_t min_dblock_size) {
  ...
  hbool_t
      have_direct_block; /* Flag to indicate a direct block already exists */
  ...
  if (hdr->man_dtable.cparam.start_root_rows == 0)
    ...
    unsigned
        rows_needed; /* Number of rows needed to get to direct block size */
    unsigned block_row_off; /* Row offset from larger block sizes */

    nrows = hdr->man_dtable.cparam.start_root_rows;

    block_row_off =
        H5VM_log2_of2((uint32_t)min_dblock_size) -
        H5VM_log2_of2((uint32_t)hdr->man_dtable.cparam.start_block_size);
    if (block_row_off > 0)
      block_row_off++; /* Account for the pair of initial rows of the initial
                          block size */
  ...
  if (H5HF__man_iblock_create(hdr, NULL, 0, nrows,
                              hdr->man_dtable.max_root_rows, &iblock_addr) < 0)
  ...
  if (NULL == (iblock = H5HF__man_iblock_protect(hdr, iblock_addr, nrows, NULL,
                                                 0, FALSE, H5AC__NO_FLAGS_SET,
                                                 &did_protect)))
  ...
  if (have_direct_block) {
    ...
    /* Scan free space sections to set any 'parent' pointers to new indirect
     * block */
    if (H5HF__space_create_root(hdr, iblock) < 0)
      HGOTO_ERROR(
          H5E_HEAP, H5E_CANTSET, FAIL,
          "can't set free space section info to new root indirect block");

    /* Unlock first (previously the root) direct block */
    if (H5AC_unprotect(hdr->f, H5AC_FHEAP_DBLOCK, hdr->man_dtable.table_addr,
                       dblock, H5AC__NO_FLAGS_SET) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                  "unable to release fractal heap direct block");
    dblock = NULL;
  } /* end if */

  /* Start iterator at correct location */
  if (H5HF__hdr_start_iter(
          hdr, iblock,
          (hsize_t)(have_direct_block ? hdr->man_dtable.cparam.start_block_size
                                      : 0),
          have_direct_block) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't initialize block iterator");

  /* Check for skipping over direct blocks, in order to get to large enough
   * block */
  if (min_dblock_size > hdr->man_dtable.cparam.start_block_size)
    ...
    if (H5HF__hdr_skip_blocks(hdr, iblock, have_direct_block,
                              ((nrows - 1) * hdr->man_dtable.cparam.width) -
                                  have_direct_block) < 0)
  ...
  if (H5HF__iblock_dirty(iblock) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTDIRTY, FAIL,
                "can't mark indirect block as dirty");
  ...
  /* Compute free space in direct blocks referenced from entries in root
   * indirect block */
  acc_dblock_free = 0;
  for (u = 0; u < nrows; u++)
    acc_dblock_free +=
        hdr->man_dtable.row_tot_dblock_free[u] * hdr->man_dtable.cparam.width;
  ...
  if (H5HF__hdr_adjust_heap(hdr, hdr->man_dtable.row_block_off[nrows],
                            (hssize_t)acc_dblock_free) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTEXTEND, FAIL,
                "can't increase space to cover root direct block");
...
herr_t H5HF__man_iblock_root_double(H5HF_hdr_t *hdr, size_t min_dblock_size) {
  ...
  hsize_t acc_dblock_free; /* Accumulated free space in direct blocks */
  hsize_t next_size; /* The previous value of the "next size" for the new block
                        iterator */
  hsize_t old_iblock_size; /* Old size of indirect block */
  unsigned next_row;       /* The next row to allocate block in */
  unsigned next_entry;     /* The previous value of the "next entry" for the new
                              block iterator */
  unsigned new_next_entry =
      0; /* The new value of the "next entry" for the new block iterator */
  ...
  if (H5HF__man_iter_curr(&hdr->next_block, &next_row, NULL, &next_entry,
                          &iblock) < 0)
  ...
  if (iblock->nrows < hdr->man_dtable.max_direct_rows &&
    ...
    /* Make certain we allocate at least the required row for the block
     * requested */
  ...
  if (!H5F_IS_TMP_ADDR(hdr->f, iblock->addr))
    ...
    if (H5MF_xfree(hdr->f, H5FD_MEM_FHEAP_IBLOCK, iblock->addr,
  ...
  if (H5F_USE_TMP_SPACE(hdr->f)) {
    if (HADDR_UNDEF ==
        (new_addr = H5MF_alloc_tmp(hdr->f, (hsize_t)iblock->size)))
      HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, FAIL,
                  "file allocation failed for fractal heap indirect block");
  } /* end if */
  else {
    if (HADDR_UNDEF == (new_addr = H5MF_alloc(hdr->f, H5FD_MEM_FHEAP_IBLOCK,
                                              (hsize_t)iblock->size)))
      HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, FAIL,
                  "file allocation failed for fractal heap indirect block");
  } /* end else */
  ...
  if (old_iblock_size != iblock->size) {
    if (H5AC_resize_entry(iblock, (size_t)iblock->size) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTRESIZE, FAIL,
                  "unable to resize fractal heap indirect block");
  } /* end if */
  ...
  if (H5_addr_ne(iblock->addr, new_addr)) {
    if (H5AC_move_entry(hdr->f, H5AC_FHEAP_IBLOCK, iblock->addr, new_addr) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTMOVE, FAIL,
                  "unable to move fractal heap root indirect block");
    iblock->addr = new_addr;
  } /* end if */

  /* Re-allocate child block entry array */
  if (NULL == (iblock->ents = H5FL_SEQ_REALLOC(
                   H5HF_indirect_ent_t, iblock->ents,
  ...
  if (skip_direct_rows)
    /* Add skipped blocks to heap's free space */
    if (H5HF__hdr_skip_blocks(hdr, iblock, next_entry,
  ...
  acc_dblock_free = 0;
  for (u = (old_nrows * hdr->man_dtable.cparam.width);
       u < (iblock->nrows * hdr->man_dtable.cparam.width); u++) {
    unsigned row =
        (unsigned)(u /
                   hdr->man_dtable.cparam.width); /* Row for current entry */
  ...
  if (hdr->filter_len > 0 && old_nrows < hdr->man_dtable.max_direct_rows) {
    ...
    if (NULL == (iblock->filt_ents = H5FL_SEQ_REALLOC(
                     H5HF_indirect_filt_ent_t, iblock->filt_ents,
    ...
    for (u = (old_nrows * hdr->man_dtable.cparam.width);
         u < (dir_rows * hdr->man_dtable.cparam.width); u++) {
  ...
  if (iblock->nrows > hdr->man_dtable.max_direct_rows) {
    ...
    unsigned old_indir_rows; /* Previous number of indirect rows in this
                                indirect block */
    ...
    if (NULL == (iblock->child_iblocks = H5FL_SEQ_REALLOC(
                     H5HF_indirect_ptr_t, iblock->child_iblocks,
  ...
  /* Extend heap to cover new root indirect block */
  if (H5HF__hdr_adjust_heap(hdr,
                            2 * hdr->man_dtable.row_block_off[new_nrows - 1],
                            (hssize_t)acc_dblock_free) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTEXTEND, FAIL,
                "can't increase space to cover root direct block");
...
static herr_t H5HF__man_iblock_root_halve(H5HF_indirect_t *iblock) {
  ...
  if (H5_addr_ne(iblock->addr, new_addr)) {
    if (H5AC_move_entry(hdr->f, H5AC_FHEAP_IBLOCK, iblock->addr, new_addr) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTSPLIT, FAIL,
                  "unable to move fractal heap root indirect block");
    iblock->addr = new_addr;
  } /* end if */
  ...
  if (old_nrows > hdr->man_dtable.max_direct_rows) {
    ...
    if (iblock->nrows > hdr->man_dtable.max_direct_rows) {
      ...
      iblock->child_iblocks = (H5HF_indirect_ptr_t *)H5FL_SEQ_FREE(
          H5HF_indirect_ptr_t, iblock->child_iblocks);
  } /* end if */
  ...
  if (H5HF__hdr_adjust_heap(hdr,
    ...
    HGOTO_ERROR(H5E_HEAP, H5E_CANTSHRINK, FAIL,
                "can't reduce space to cover root direct block");
...
static herr_t H5HF__man_iblock_root_revert(H5HF_indirect_t *root_iblock) {
  ...
  if (NULL ==
      (dblock = H5HF__man_dblock_protect(hdr, dblock_addr, dblock_size,
                                         root_iblock, 0, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                "unable to protect fractal heap direct block");
  ...
  if (H5HF__man_iblock_detach(dblock->parent, 0) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTATTACH, FAIL,
                "can't detach direct block from parent indirect block");
  dblock->parent = NULL;
  dblock->par_entry = 0;
  ...
  if (H5HF__hdr_adjust_heap(
          hdr, (hsize_t)hdr->man_dtable.cparam.start_block_size,
  ...
  if (H5HF__space_revert_root(hdr) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTRESET, FAIL,
                "can't reset free space section info");

done:
  if (dblock && H5AC_unprotect(hdr->f, H5AC_FHEAP_DBLOCK, dblock_addr, dblock,
...
herr_t H5HF__man_iblock_alloc_row(H5HF_hdr_t *hdr,
  ...
  H5HF_free_section_t *old_sec_node =
      *sec_node;         /* Pointer to old indirect section node */
  unsigned dblock_entry; /* Entry for direct block */
  hbool_t iblock_held =
      FALSE;                  /* Flag to indicate that indirect block is held */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (old_sec_node->sect_info.state == H5FS_SECT_SERIALIZED ||
      ...
      (TRUE ==
       old_sec_node->u.row.under->u.indirect.u.iblock->removed_from_cache))
  ...
  if (NULL == (iblock = H5HF__sect_row_get_iblock(old_sec_node)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                "can't retrieve indirect block for row section");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_create(H5HF_hdr_t *hdr, H5HF_indirect_t *par_iblock,
                               unsigned par_entry, unsigned nrows,
  ...
  if (NULL == (iblock->ents = H5FL_SEQ_MALLOC(
                   H5HF_indirect_ent_t,
                   (size_t)(iblock->nrows * hdr->man_dtable.cparam.width))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
  ...
  if (hdr->filter_len > 0) {
    ...
    if (NULL == (iblock->filt_ents = H5FL_SEQ_CALLOC(
  ...
  if (H5F_USE_TMP_SPACE(hdr->f)) {
    if (HADDR_UNDEF ==
        (*addr_p = H5MF_alloc_tmp(hdr->f, (hsize_t)iblock->size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "file allocation failed for fractal heap indirect block");
  } /* end if */
  else {
    if (HADDR_UNDEF == (*addr_p = H5MF_alloc(hdr->f, H5FD_MEM_FHEAP_IBLOCK,
                                             (hsize_t)iblock->size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "file allocation failed for fractal heap indirect block");
  } /* end else */
  ...
  if (iblock->parent) {
    ...
    iblock->block_off +=
        hdr->man_dtable.row_block_off[par_entry / hdr->man_dtable.cparam.width];
    iblock->block_off +=
        hdr->man_dtable
            .row_block_size[par_entry / hdr->man_dtable.cparam.width] *
  ...
  if (H5AC_insert_entry(hdr->f, H5AC_FHEAP_IBLOCK, *addr_p, iblock,
                        H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't add fractal heap indirect block to cache");
 ...
 *-------------------------------------------------------------------------
 */
H5HF_indirect_t *H5HF__man_iblock_protect(H5HF_hdr_t *hdr, haddr_t iblock_addr,
                                          unsigned iblock_nrows,
                                          H5HF_indirect_t *par_iblock,
                                          unsigned par_entry,
                                          hbool_t must_protect, unsigned flags,
                                          hbool_t *did_protect) {
  H5HF_parent_t par_info;         /* Parent info for loading block */
  H5HF_indirect_t *iblock = NULL; /* Indirect block from cache */
  hbool_t should_protect =
      FALSE; /* Whether we should protect the indirect block or not */
  ...
  if (!must_protect) {
    ...
    if (par_iblock) {
      ...
      assert(par_iblock->child_iblocks);
      assert(par_entry >=
             (hdr->man_dtable.max_direct_rows * hdr->man_dtable.cparam.width));
  ...
  if (must_protect || should_protect) {
    ...
    if (NULL ==
        (iblock = (H5HF_indirect_t *)H5AC_protect(
             hdr->f, H5AC_FHEAP_IBLOCK, iblock_addr, &cache_udata, flags)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_unprotect(H5HF_indirect_t *iblock, unsigned cache_flags,
                                  hbool_t did_protect) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (did_protect) {
    /* Check for root indirect block */
    if (iblock->block_off == 0) {
      /* Sanity check - shouldn't be recursively unprotecting root indirect
       * block */
      ...
      iblock->hdr->root_iblock_flags &=
          (unsigned)(~(H5HF_ROOT_IBLOCK_PROTECTED));
    } /* end if */

    /* Unprotect the indirect block */
    if (H5AC_unprotect(iblock->hdr->f, H5AC_FHEAP_IBLOCK, iblock->addr, iblock,
                       cache_flags) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                  "unable to release fractal heap indirect block");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_attach(H5HF_indirect_t *iblock, unsigned entry,
                               haddr_t child_addr) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (iblock->hdr->filter_len > 0) {
    ...
    if (row < iblock->hdr->man_dtable.max_direct_rows)
      iblock->filt_ents[entry].size =
          iblock->hdr->man_dtable.row_block_size[row];
  } /* end if */
...
herr_t H5HF__man_iblock_detach(H5HF_indirect_t *iblock, unsigned entry) {
  ...
  H5HF_indirect_t *del_iblock =
      NULL;     /* Pointer to protected indirect block, when deleting */
  unsigned row; /* Row for entry */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (row >= hdr->man_dtable.max_direct_rows) {
    ...
    indir_idx = entry - (hdr->man_dtable.max_direct_rows *
                         hdr->man_dtable.cparam.width);
  ...
  if (iblock->block_off == 0) {
    ...
    if (!iblock->removed_from_cache) {
      ...
      if (iblock->nchildren > 0 &&
          hdr->man_dtable.cparam.start_root_rows != 0 &&
          entry > iblock->max_child) {
        unsigned max_child_row; /* Row for max. child entry */

        /* Compute information needed for determining whether to reduce size of
         * root indirect block */
        max_child_row = iblock->max_child / hdr->man_dtable.cparam.width;
  ...
  if (!iblock->removed_from_cache) {
    ...
    if (iblock->nchildren == 0) {
      hbool_t did_protect =
          FALSE; /* Whether the indirect block was protected */
      ...
      if (NULL ==
          (del_iblock = H5HF__man_iblock_protect(
               hdr, iblock->addr, iblock->nrows, iblock->parent,
               iblock->par_entry, TRUE, H5AC__NO_FLAGS_SET, &did_protect)))
        HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                    "unable to protect fractal heap indirect block");
      ...
      if (iblock->parent) {
        ...
        if (H5HF__man_iblock_detach(iblock->parent, iblock->par_entry) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTATTACH, FAIL,
                      "can't detach from parent indirect block");
        iblock->parent = NULL;
        iblock->par_entry = 0;
      } /* end if */
    }   /* end if */
  ...
  /* Decrement the reference count on this indirect block if we're not deleting
   * it */
  ...
  if (del_iblock) {
    ...
    hbool_t took_ownership =
        FALSE; /* Flag to indicate that block ownership has transitioned */
    ...
    if (took_ownership) {
      ...
      if (!H5F_IS_TMP_ADDR(hdr->f, del_iblock->addr))
        if (H5MF_xfree(hdr->f, H5FD_MEM_FHEAP_IBLOCK, del_iblock->addr,
                       (hsize_t)del_iblock->size) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                      "unable to free fractal heap indirect block file space");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_entry_addr(H5HF_indirect_t *iblock, unsigned entry,
                                   haddr_t *child_addr) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_delete(H5HF_hdr_t *hdr, haddr_t iblock_addr,
                               unsigned iblock_nrows,
                               H5HF_indirect_t *par_iblock,
                               unsigned par_entry) {
  H5HF_indirect_t *iblock; /* Pointer to indirect block */
  ...
  unsigned cache_flags =
      H5AC__NO_FLAGS_SET; /* Flags for unprotecting indirect block */
  hbool_t did_protect;    /* Whether we protected the indirect block or not */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Lock indirect block */
  if (NULL == (iblock = H5HF__man_iblock_protect(
                   hdr, iblock_addr, iblock_nrows, par_iblock, par_entry, TRUE,
    ...
    for (col = 0; col < hdr->man_dtable.cparam.width; col++, entry++) {
      ...
      if (H5_addr_defined(iblock->ents[entry].addr)) {
        ...
        if (row < hdr->man_dtable.max_direct_rows) {
          ...
          if (H5HF__man_dblock_delete(hdr->f, iblock->ents[entry].addr,
                                      dblock_size) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
          ...
          child_nrows =
              H5HF__dtable_size_to_rows(&hdr->man_dtable, row_block_size);

          /* Delete child indirect block */
          if (H5HF__man_iblock_delete(hdr, iblock->ents[entry].addr,
                                      child_nrows, iblock, entry) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
...
#ifndef NDEBUG
  {
    unsigned iblock_status =
        0; /* Indirect block's status in the metadata cache */
  ...
  /* Unprotect the indirect block, with appropriate flags */
  if (iblock &&
      H5HF__man_iblock_unprotect(iblock, cache_flags, did_protect) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                "unable to release fractal heap indirect block");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_size(H5F_t *f, H5HF_hdr_t *hdr, haddr_t iblock_addr,
                             unsigned nrows, H5HF_indirect_t *par_iblock,
                             unsigned par_entry, hsize_t *heap_size) {
  H5HF_indirect_t *iblock = NULL; /* Pointer to indirect block */
  hbool_t did_protect; /* Whether we protected the indirect block or not */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Protect the indirect block */
  if (NULL == (iblock = H5HF__man_iblock_protect(
                   hdr, iblock_addr, nrows, par_iblock, par_entry, FALSE,
                   H5AC__READ_ONLY_FLAG, &did_protect)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTLOAD, FAIL,
                "unable to load fractal heap indirect block");
  ...
  *heap_size += iblock->size;
  ...
  if (iblock->nrows > hdr->man_dtable.max_direct_rows) {
    unsigned
        first_row_bits; /* Number of bits used bit addresses in first row */
    unsigned
        num_indirect_rows; /* Number of rows of blocks in each indirect block */
    unsigned entry;        /* Current entry in row */
    size_t u;              /* Local index variable */
    ...
    num_indirect_rows =
        (H5VM_log2_gen(
             hdr->man_dtable.row_block_size[hdr->man_dtable.max_direct_rows]) -
         first_row_bits) +
        1;
    for (u = hdr->man_dtable.max_direct_rows; u < iblock->nrows;
         u++, num_indirect_rows++) {
      size_t v; /* Local index variable */

      for (v = 0; v < hdr->man_dtable.cparam.width; v++, entry++)
        if (H5_addr_defined(iblock->ents[entry].addr))
          if (H5HF__man_iblock_size(f, hdr, iblock->ents[entry].addr,
                                    num_indirect_rows, iblock, entry,
                                    heap_size) < 0)
            HGOTO_ERROR(
                H5E_HEAP, H5E_CANTLOAD, FAIL,
  ...
  if (iblock &&
      H5HF__man_iblock_unprotect(iblock, H5AC__NO_FLAGS_SET, did_protect) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                "unable to release fractal heap indirect block");
  iblock = NULL;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iblock_parent_info(const H5HF_hdr_t *hdr, hsize_t block_off,
  ...
  if (H5HF__dtable_lookup(&hdr->man_dtable, block_off, &row, &col) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTCOMPUTE, FAIL,
                "can't compute row & column of block");
  ...
  while (row >= hdr->man_dtable.max_direct_rows) {
    ...
    /* Compute the new parent indirect block's offset in the heap's address
     * space */
    ...
    if (H5HF__dtable_lookup(&hdr->man_dtable, (block_off - par_block_off), &row,
                            &col) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCOMPUTE, FAIL,
                  "can't compute row & column of block");
  } /* end while */
