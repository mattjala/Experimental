 ...
 *      directly in an 8 byte integer in memory, think carefully before changing
 * it. -QAK)
 */
...
#define H5O_MSG_FLAG_BITS                                                      \
  (H5O_MSG_FLAG_CONSTANT | H5O_MSG_FLAG_SHARED | H5O_MSG_FLAG_DONTSHARE |      \
   ...
   H5O_MSG_FLAG_MARK_IF_UNKNOWN | H5O_MSG_FLAG_WAS_UNKNOWN |                   \
   H5O_MSG_FLAG_SHAREABLE | H5O_MSG_FLAG_FAIL_IF_UNKNOWN_ALWAYS)
...
#define H5O_CRT_ATTR_MAX_COMPACT_NAME                                          \
  "max compact attr" /* Max. # of attributes to store compactly */
#define H5O_CRT_ATTR_MIN_DENSE_NAME                                            \
  "min dense attr" /* Min. # of attributes to store densely */
...
#define H5O_CRT_PIPELINE_DEF                                                   \
  {                                                                            \
    {0, NULL, H5O_NULL_ID, {{0, HADDR_UNDEF}}}, H5O_PLINE_VERSION_1, 0, 0,     \
        NULL                                                                   \
  }
#ifdef H5O_ENABLE_BOGUS
#define H5O_BOGUS_MSG_FLAGS_NAME                                               \
  "bogus msg flags" /* Flags for 'bogus' message */
...
#define H5O_BAD_MESG_COUNT_NAME                                                \
  "bad message count" /* Flag setting bad message count */
...
#define H5O_CPY_MERGE_COMM_DT_LIST_NAME                                        \
  "merge committed dtype list" /* List of datatype paths to search in the dest \
                                  file for merging */
#define H5O_CPY_MCDT_SEARCH_CB_NAME                                            \
  "committed dtype list search" /* Callback function when the search for a     \
                                   matching committed datatype is complete */

/* If the module using this macro is allowed access to the private variables,
...
/* Structure for callback property before searching the global list of committed
...
typedef struct H5O_copy_t {
  ...
  H5O_copy_dtype_merge_list_t
      *dst_dt_suggestion_list; /* Suggestions for merging committed datatypes */
  ...
  hbool_t dst_dt_list_complete; /* Whether the destination datatype list is
                                   complete (i.e. not only populated with
                                   "suggestions" from
  ...
  H5O_mcdt_search_cb_t
      mcdt_cb;   /* The callback to invoke before searching the global list of
                    committed   datatypes at destination */
...
#define H5O_SHARE_TYPE_COMMITTED                                               \
  2 /* Message is stored in another object header */
#define H5O_SHARE_TYPE_HERE                                                    \
  3 /* Message is stored in this object header, but is shareable */

/* Detect messages that aren't stored in message's object header */
#define H5O_IS_STORED_SHARED(T)                                                \
  ((((T) == H5O_SHARE_TYPE_SOHM) || ((T) == H5O_SHARE_TYPE_COMMITTED))         \
       ? TRUE                                                                  \
       : FALSE)
...
typedef struct H5O_linfo_t {
  ...
  haddr_t corder_bt2_addr; /* Address of v2 B-tree for indexing creation order
                              values of links */
...
typedef struct H5O_storage_chunk_t {
  ...
  union {
    ...
    H5O_storage_chunk_earray_t
        earray; /* Information for extensible array index   */
    H5O_storage_chunk_farray_t farray; /* Information for fixed array index   */
    H5O_storage_chunk_single_filt_t
        single; /* Information for single chunk w/ filters index */
  } u;
} H5O_storage_chunk_t;
...
typedef struct H5O_storage_virtual_srcdset_t {
  /* Stored */
  struct H5S_t *virtual_select; /* Selection in the virtual dataset that is
                                   mapped to source selection */
  ...
  hbool_t
      dset_exists; /* Whether the dataset exists (was opened successfully) */

  /* Temporary - only used during I/O operation, NULL at all other times */
  struct H5S_t
      *projected_mem_space; /* Selection within mem_space for this mapping */
...
typedef struct H5O_storage_virtual_ent_t {
  /* Stored */
  H5O_storage_virtual_srcdset_t
      source_dset;             /* Information about the source dataset */
  ...
  /* Not stored */
  H5O_storage_virtual_srcdset_t
      *sub_dset;          /* Array of sub-source dataset info structs */
  size_t sub_dset_nalloc; /* Number of slots allocated in sub_dset */
  size_t sub_dset_nused; /* Number of slots "used" in sub_dset - essentially the
                            farthest sub dataset in the extent */
  size_t sub_dset_io_start; /* First element in sub_dset involved in current I/O
                               op.  Field has no meaning and may be
                               uninitialized at all other times */
  size_t sub_dset_io_end;   /* First element in sub_dset outside of current I/O
                               op.  Field has no meaning and
                               may be uninitialized at all other times */
  H5O_storage_virtual_name_seg_t
      *parsed_source_file_name; /* Parsed version of source_dset.file_name */
  size_t psfn_static_strlen; /* Length of parsed_source_file_name without block
                                number substitutions */
  size_t psfn_nsubs;         /* Number of block number substitutions in
                                parsed_source_file_name */
  H5O_storage_virtual_name_seg_t
      *parsed_source_dset_name; /* Parsed version of source_dset.dset_name */
  size_t psdn_static_strlen; /* Length of parsed_source_dset_name without block
                                number substitutions */
  size_t psdn_nsubs;         /* Number of block number substitutions in
                                parsed_source_dset_name */
  int unlim_dim_source;      /* Unlimited dimension in source_select */
  int unlim_dim_virtual;     /* Unlimited dimension in virtual_select */
  hsize_t unlim_extent_source;  /* Extent of unlimited dimension in source dset
                                   last time virtual_select was
                                   patched to match selection */
  hsize_t unlim_extent_virtual; /* Extent of unlimited dimension in virtual dset
                                   last time source_select was
                                   patched to match selection */
  hsize_t
      clip_size_virtual;    /* Size selection would be clipped to in virtual
                               selection, ignoring other    mappings, when source
                               extent == unlim_extent_source */
  hsize_t clip_size_source; /* Size selection would be clipped to in source
                               selection when virtual extent ==
                               unlim_extent_virtual */
  H5O_virtual_space_status_t
      source_space_status; /* Extent patching status of source_select */
  H5O_virtual_space_status_t
      virtual_space_status; /* Extent patching status of virtual_select */
} H5O_storage_virtual_ent_t;

typedef struct H5O_storage_virtual_t {
  /* Stored in message */
  H5HG_t serial_list_hobjid; /* Global heap ID for the list of virtual mapping
                                entries stored on disk */

  /* Stored in heap */
  size_t list_nused; /* Number of array elements used in list    */
  H5O_storage_virtual_ent_t
      *list; /* Array of virtual dataset mapping entries */

  /* Not stored */
  size_t list_nalloc;             /* Number of slots allocated          */
  hsize_t min_dims[H5S_MAX_RANK]; /* Minimum extent of VDS (maximum of all
                                     non-unlimited selection bounds) */
  H5D_vds_view_t view; /* Method for calculating the extent of the virtual
                          dataset with unlimited selections */
  hsize_t printf_gap;  /* Maximum number of sequential missing source datasets
                          before terminating the search  for more */
...
typedef struct H5O_layout_chunk_farray_t {
  ...
  struct {
    uint8_t
        max_dblk_page_nelmts_bits; /* Log2(Max. # of elements in a data block
                                      page) - i.e. # of bits needed to store
                                      max. # of elements in a data block page */
  } cparam;
...
typedef struct H5O_layout_chunk_earray_t {
  ...
  struct {
    uint8_t max_nelmts_bits;    /* Log2(Max. # of elements in array) - i.e. # of
    ...
    uint8_t sup_blk_min_data_ptrs; /* Min. # of data block pointers for a super
                                      block */
    uint8_t
        max_dblk_page_nelmts_bits; /* Log2(Max. # of elements in data block
                                      page) - i.e. # of bits needed to store
  ...
  hsize_t
      swizzled_down_chunks[H5O_LAYOUT_NDIMS]; /* swizzled "down" size of number
                                                 of chunks in each dimension */
  hsize_t
      swizzled_max_down_chunks[H5O_LAYOUT_NDIMS]; /* swizzled max "down" size of
...
typedef struct H5O_layout_chunk_t {
  ...
  unsigned enc_bytes_per_dim;       /* Encoded # of bytes for storing each chunk
                                       dimension */
  ...
  hsize_t max_chunks[H5O_LAYOUT_NDIMS];  /* # of chunks in each dataset's max.
                                            dimension */
  hsize_t down_chunks[H5O_LAYOUT_NDIMS]; /* "down" size of number of chunks in
                                            each dimension */
  hsize_t max_down_chunks[H5O_LAYOUT_NDIMS]; /* "down" size of number of chunks
                                                in each max dim */
  union {
...
typedef struct H5O_layout_t {
  ...
  const struct H5D_layout_ops_t
      *ops; /* Pointer to data layout I/O operations */
  union {
...
typedef struct H5O_ginfo_t {
  ...
  /* (storage management info) */
  hbool_t store_link_phase_change; /* Whether to store the link phase change
                                      values */
...
typedef struct H5O_ainfo_t {
  ...
  haddr_t corder_bt2_addr; /* Address of v2 B-tree for indexing creation order
                              values of "dense" attributes */

  /* Storage management info */
  hsize_t nattrs; /* Number of attributes on the object */
  haddr_t
      fheap_addr; /* Address of fractal heap for storing "dense" attributes */
  haddr_t name_bt2_addr; /* Address of v2 B-tree for indexing names of "dense"
                            attributes */
} H5O_ainfo_t;
...
/* To indicate an invalid version for a message that does not exist yet for the
 * release */
...
typedef struct H5O_fsinfo_t {
  ...
  size_t
      pgend_meta_thres; /* For paged aggregation: page end metadata threshold */
  haddr_t eoa_pre_fsm_fsalloc; /* For paged aggregation: the eoa before
                                  free-space headers & sinfo */
  haddr_t fs_addr[H5F_MEM_PAGE_NTYPES -
                  1]; /* 13 addresses of free-space managers */
...
H5_DLL herr_t H5O_apply_ohdr(H5F_t *f, H5O_t *oh, hid_t ocpl_id,
                             size_t size_hint, size_t initial_rc,
                             H5O_loc_t *loc_out);
H5_DLL herr_t H5O_open(H5O_loc_t *loc);
H5_DLL void *H5O_open_by_loc(const H5G_loc_t *obj_loc,
                             H5I_type_t *opened_type /*out*/);
...
H5_DLL void *H5O_open_name(const H5G_loc_t *loc, const char *name,
                           H5I_type_t *opened_type /*out*/);
H5_DLL herr_t H5O_get_nlinks(const H5O_loc_t *loc, hsize_t *nlinks);
H5_DLL void *H5O_obj_create(H5F_t *f, H5O_type_t obj_type, void *crt_info,
                            H5G_loc_t *obj_loc);
...
H5_DLL herr_t H5O_get_rc_and_type(const H5O_loc_t *oloc, unsigned *rc,
                                  H5O_type_t *otype);
H5_DLL H5AC_proxy_entry_t *H5O_get_proxy(const H5O_t *oh);

/* Object header message routines */
H5_DLL herr_t H5O_msg_create(const H5O_loc_t *loc, unsigned type_id,
...
H5_DLL herr_t H5O_msg_remove(const H5O_loc_t *loc, unsigned type_id,
                             int sequence, hbool_t adj_link);
H5_DLL herr_t H5O_msg_remove_op(const H5O_loc_t *loc, unsigned type_id,
                                int sequence, H5O_operator_t op, void *op_data,
                                hbool_t adj_link);
...
H5_DLL herr_t H5O_msg_delete(H5F_t *f, H5O_t *open_oh, unsigned type_id,
                             void *mesg);
H5_DLL herr_t H5O_msg_get_flags(const H5O_loc_t *loc, unsigned type_id,
                                uint8_t *flags);
...
H5_DLL herr_t H5O_refresh_metadata_reopen(hid_t oid, hid_t apl_id,
                                          H5G_loc_t *obj_loc,
                                          H5VL_t *vol_driver,
                                          hbool_t start_swmr);
...
H5_DLL herr_t H5O_copy_header_map(const H5O_loc_t *oloc_src,
                                  H5O_loc_t *oloc_dst /*out */,
                                  H5O_copy_t *cpy_info, hbool_t inc_depth,
                                  H5O_type_t *obj_type, void **udata);
H5_DLL herr_t H5O_copy_expand_ref(H5F_t *file_src, hid_t tid_src,
                                  const H5T_t *dt_src, void *buf_src,
                                  size_t nbytes_src, H5F_t *file_dst,
                                  void *buf_dst, H5O_copy_t *cpy_info);

/* Debugging routines */
H5_DLL herr_t H5O_debug_id(unsigned type_id, H5F_t *f, const void *mesg,
...
H5_DLL herr_t H5O_loc_copy(H5O_loc_t *dst, H5O_loc_t *src,
...
H5_DLL herr_t H5O_fsinfo_set_version(H5F_libver_t low, H5F_libver_t high,
                                     H5O_fsinfo_t *fsinfo);
...
H5_DLL herr_t H5O_fill_convert(H5O_fill_t *fill, H5T_t *type,
