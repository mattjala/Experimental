 ...
 *-------------------------------------------------------------------------
 */
hsize_t H5VM_hyper_stride(unsigned n, const hsize_t *size,
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5VM_hyper_eq(unsigned n, const hsize_t *offset1, const hsize_t *size1,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VM_hyper_fill(unsigned n, const hsize_t *_size,
                       const hsize_t *total_size, const hsize_t *offset,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VM_hyper_copy(unsigned n, const hsize_t *_size,
                       const hsize_t *dst_size, const hsize_t *dst_offset,
                       void *_dst, const hsize_t *src_size,
  ...
  /* in-line version of two calls to H5VM_hyper_stride() */
  {
    ...
    default:
      ...
      for (ii = (int)(n - 2), dst_acc = 1, src_acc = 1; ii >= 0; --ii) {
        ...
        dst_stride[ii] =
            dst_acc * (dst_size[ii + 1] - size[ii + 1]); /*overflow checked*/
        src_stride[ii] =
            src_acc * (src_size[ii + 1] - size[ii + 1]); /*overflow checked*/
  ...
  ret_value = H5VM_stride_copy(n, elmt_size, size, dst_stride, dst + dst_start,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VM_stride_fill(unsigned n, hsize_t elmt_size, const hsize_t *size,
  ...
  for (i = 0; i < nelmts; i++) {
    ...
    memset(dst, (int)fill_value,
           (size_t)elmt_size); /*lint !e671 The elmt_size will be OK */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VM_stride_copy(unsigned n, hsize_t elmt_size, const hsize_t *size,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VM_stride_copy_s(unsigned n, hsize_t elmt_size, const hsize_t *size,
 ...
 *
 *		        The dimensionality of the whole array, and the offset is
 *N. The whole array dimensions are TOTAL_SIZE and the coordinate is at offset
 *OFFSET.
 *
 ...
 *-------------------------------------------------------------------------
 */
hsize_t H5VM_array_offset_pre(unsigned n, const hsize_t *acc,
 ...
 *-------------------------------------------------------------------------
 */
hsize_t H5VM_array_offset(unsigned n, const hsize_t *total_size,
                          const hsize_t *offset) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VM_array_calc_pre(hsize_t offset, unsigned n, const hsize_t *down,
                           hsize_t *coords) {
  unsigned u; /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VM_array_calc(hsize_t offset, unsigned n, const hsize_t *total_size,
                       hsize_t *coords) {
 ...
 *-------------------------------------------------------------------------
 */
hsize_t H5VM_chunk_index(unsigned ndims, const hsize_t *coord,
                         const uint32_t *chunk, const hsize_t *down_nchunks) {
  hsize_t scaled_coord[H5VM_HYPER_NDIMS]; /* Scaled, coordinates, in terms of
                                             chunks */
  ...
  chunk_idx =
      H5VM_chunk_index_scaled(ndims, coord, chunk, down_nchunks, scaled_coord);
 ...
 *-------------------------------------------------------------------------
 */
void H5VM_chunk_scaled(unsigned ndims, const hsize_t *coord,
 ...
 *-------------------------------------------------------------------------
 */
hsize_t H5VM_chunk_index_scaled(unsigned ndims, const hsize_t *coord,
  ...
  chunk_idx = H5VM_array_offset_pre(
      ndims, down_nchunks,
 ...
 *
 * Return:      Non-negative # of bytes operated on, on success/Negative on
 *failure
 *
 *-------------------------------------------------------------------------
 */
ssize_t H5VM_opvv(size_t dst_max_nseq, size_t *dst_curr_seq,
  ...
  hsize_t *max_dst_off_ptr, *max_src_off_ptr; /* Pointers to max. source and
                                                 destination offset locations */
  hsize_t *dst_off_ptr,
      *src_off_ptr; /* Pointers to source and destination offset arrays */
  size_t *dst_len_ptr,
      *src_len_ptr; /* Pointers to source and destination length arrays */
  hsize_t tmp_dst_off,
      tmp_src_off; /* Temporary source and destination offset values */
  size_t tmp_dst_len,
      tmp_src_len;       /* Temporary source and destination length values */
  ...
  src_smaller:
    ...
    do {
      ...
      if ((*op)(tmp_dst_off, tmp_src_off, tmp_src_len, op_data) < 0)
        HGOTO_ERROR(H5E_INTERNAL, H5E_CANTOPERATE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5VM_memcpyvv(void *_dst, size_t dst_max_nseq, size_t *dst_curr_seq,
                      size_t dst_len_arr[], hsize_t dst_off_arr[],
                      const void *_src, size_t src_max_nseq,
                      size_t *src_curr_seq, size_t src_len_arr[],
                      hsize_t src_off_arr[]) {
