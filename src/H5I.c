 ...
 *HASH_SIZE is the minimum hash table size to use for the type. FREE_FUNC is
 ...
 *-------------------------------------------------------------------------
 */
H5I_type_t H5Iregister_type(size_t H5_ATTR_DEBUG_API_USED hash_size,
                            unsigned reserved, H5I_free_t free_func) {
  ...
  if ((atomic_load(&(H5I_mt_g.next_type)) < H5I_MAX_NUM_TYPES) &&
      ((new_type = atomic_fetch_add(&(H5I_mt_g.next_type), 1)) <
       H5I_MAX_NUM_TYPES)) {

    result = atomic_compare_exchange_strong(
        &(H5I_mt_g.type_info_allocation_table[new_type]), &expected, TRUE);
    assert(result);
    ...
    /* H5I_mt_g.next_type is now greater than or equal to H5I_MAX_NUM_TYPES.
     * Thus we must scan H5I_mt_g.type_info_allocation_table[] for an
     * un-allocated id.
     */
    new_type = H5I_NTYPES;
    ...
    do {
      ...
      if (atomic_compare_exchange_strong(
              &(H5I_mt_g.type_info_allocation_table[new_type]), &expected,
              TRUE)) {
        done = TRUE;
    ...
    if (!done) {
      ...
      HGOTO_ERROR(H5E_ID, H5E_NOSPACE, H5I_BADID,
                  "Maximum number of ID types exceeded");
    }
  }
...
htri_t H5Itype_exists(H5I_type_t type) {
  ...
  if (H5I_IS_LIB_TYPE(type))
    HGOTO_ERROR(H5E_ID, H5E_BADGROUP, FAIL,
...
herr_t H5Inmembers(H5I_type_t type, hsize_t *num_members) {
  ...
  if (num_members) {
    ...
    if ((members = H5I_nmembers_internal(type)) < 0)
      ...
      HGOTO_ERROR(H5E_ID, H5E_CANTCOUNT, FAIL,
                  "can't compute number of members");

    /* if the type is deleted between the time that we call H5I_nmembers(). and
     * the time that that function looks up its address, H5I_nmembers() will
     * return zero.  if members == 0, check to see if
     * H5I_type_info_array_g[type] is still non-NULL.  If it isn't, flag a
     * "supplied type does not exist" error.
     *
     * Note that this check is subject to its own race conditions if we ever
     * start re-using type IDs.
     */
    if ((0 == members) &&
        (NULL == atomic_load(&(H5I_mt_g.type_info_array[type]))))
...
hid_t H5Iregister(H5I_type_t type, const void *object) {
  ...
  if (H5I_IS_LIB_TYPE(type))
    HGOTO_ERROR(H5E_ID, H5E_BADGROUP, H5I_INVALID_HID,
                "cannot call public function on library type");
...
hid_t H5Iregister_future(H5I_type_t type, const void *object,
  ...
  /* Check arguments */
  if (NULL == realize_cb)
    HGOTO_ERROR(H5E_ID, H5E_BADVALUE, H5I_INVALID_HID,
                "NULL pointer for realize_cb not allowed");
  if (NULL == discard_cb)
    HGOTO_ERROR(H5E_ID, H5E_BADVALUE, H5I_INVALID_HID,
                "NULL pointer for realize_cb not allowed");

  /* Register the future object */
  if ((ret_value = H5I__register(type, object, TRUE, realize_cb, discard_cb)) <
      0)
    HGOTO_ERROR(H5E_ID, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register object");
...
void *H5Iobject_verify(hid_t id, H5I_type_t type) {
  ...
  if (H5I_IS_LIB_TYPE(type))
    ...
    HGOTO_ERROR(H5E_ID, H5E_BADGROUP, NULL,
                "cannot call public function on library type");
...
H5I_type_t H5Iget_type(hid_t id) {
  ...
  if (ret_value <= H5I_BADID ||
      (int)ret_value >= atomic_load(&(H5I_mt_g.next_type)) ||
...
H5I_type_t H5Iget_type(hid_t id) {
  ...
  if (ret_value <= H5I_BADID || (int)ret_value >= H5I_next_type_g ||
      NULL == H5I_object(id))
    HGOTO_DONE(H5I_BADID);
 ...
 *              type of the ID and the type type are the same.  Public interface
 *to H5I__remove_verify.
 *
...
int H5Iinc_type_ref(H5I_type_t type) {
  ...
  if (H5I_IS_LIB_TYPE(type))
    HGOTO_ERROR(H5E_ID, H5E_BADGROUP, (-1),
                "cannot call public function on library type");
...
static int H5I__search_cb(void *obj, hid_t id, void *_udata) {
  H5I_search_ud_t *udata =
      (H5I_search_ud_t *)_udata; /* User data for callback */
 ...
 *-------------------------------------------------------------------------
 */
static int H5I__iterate_pub_cb(void H5_ATTR_UNUSED *obj, hid_t id,
                               void *_udata) {
  H5I_iterate_pub_ud_t *udata =
      (H5I_iterate_pub_ud_t *)_udata; /* User data for callback */
...
herr_t H5Iiterate(H5I_type_t type, H5I_iterate_func_t op, void *op_data) {
   ...
   */
#ifdef H5_HAVE_MULTITHREAD
  if ((ret_value = H5I_iterate_internal(type, H5I__iterate_pub_cb, &int_udata,
                                        TRUE)) < 0)
    HGOTO_ERROR(H5E_ID, H5E_BADITER, FAIL, "can't iterate over ids");
#else  /* H5_HAVE_MULTITHREAD */
  if ((ret_value = H5I_iterate(type, H5I__iterate_pub_cb, &int_udata, TRUE)) <
      0)
    HGOTO_ERROR(H5E_ID, H5E_BADITER, FAIL, "can't iterate over ids");
#endif /* H5_HAVE_MULTITHREAD */
...
hid_t H5Iget_file_id(hid_t obj_id) {
  ...
  if (H5I_FILE == type || H5I_DATATYPE == type || H5I_GROUP == type ||
    ...
    if (NULL == (vol_obj = H5VL_vol_object(obj_id)))
      HGOTO_ERROR(H5E_ID, H5E_BADTYPE, H5I_INVALID_HID,
                  "invalid location identifier");

    /* Get the file ID */
    if ((ret_value = H5F_get_file_id(vol_obj, type, TRUE)) < 0)
      HGOTO_ERROR(H5E_ID, H5E_CANTGET, H5I_INVALID_HID,
                  "can't retrieve file ID");
  } else
    HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, H5I_INVALID_HID,
                "not an ID of a file object");
