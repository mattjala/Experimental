 ...
 *-------------------------------------------------------------------------
 */
H5FS_t *H5FS_create(H5F_t *f, haddr_t *fs_addr, const H5FS_create_t *fs_create,
                    uint16_t nclasses, const H5FS_section_class_t *classes[],
                    void *cls_init_udata, hsize_t alignment,
                    hsize_t threshold) {
  ...
  if (NULL == (fspace = H5FS__new(f, nclasses, classes, cls_init_udata)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "memory allocation failed for free space free list");
  ...
  if (fs_addr) {
    ...
    if (HADDR_UNDEF == (fspace->addr = H5MF_alloc(f, H5FD_MEM_FSPACE_HDR,
                                                  (hsize_t)fspace->hdr_size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
    ...
    if (H5AC_insert_entry(f, H5AC_FSPACE_HDR, fspace->addr, fspace,
      ...
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTINIT, NULL,
  ...
  /* Set the reference count to 1, since we inserted the entry in the cache
   * pinned */
  fspace->rc = 1;
 ...
 *-------------------------------------------------------------------------
 */
H5FS_t *H5FS_open(H5F_t *f, haddr_t fs_addr, uint16_t nclasses,
...
#ifdef H5FS_DEBUG
  fprintf(stderr,
          "%s: Opening free space manager, fs_addr = %" PRIuHADDR
          ", nclasses = %Zu\n",
  ...
  if (NULL ==
      (fspace = (H5FS_t *)H5AC_protect(f, H5AC_FSPACE_HDR, fs_addr,
                                       &cache_udata, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTPROTECT, NULL,
                "unable to load free space header");
#ifdef H5FS_DEBUG
  ...
  if (H5AC_unprotect(f, H5AC_FSPACE_HDR, fs_addr, fspace, H5AC__NO_FLAGS_SET) <
      0)
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTUNPROTECT, NULL,
                "unable to release free space header");
...
herr_t H5FS_delete(H5F_t *f, haddr_t fs_addr) {
...
#ifdef H5FS_DEBUG
  {
    unsigned fspace_status =
        0; /* Free space section info's status in the metadata cache */
    ...
    if (fspace_status) {
      ...
      if (fspace_status & H5AC_ES__IS_FLUSH_DEP_PARENT) {
        fprintf(stderr, "%sH5AC_ES__IS_FLUSH_DEP_PARENT",
                (printed ? " | " : ""));
        printed = TRUE;
      } /* end if */
      if (fspace_status & H5AC_ES__IS_FLUSH_DEP_CHILD) {
        fprintf(stderr, "%sH5AC_ES__IS_FLUSH_DEP_CHILD",
                (printed ? " | " : ""));
        printed = TRUE;
      } /* end if */
    }   /* end if */
  ...
  if (NULL ==
    ...
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTPROTECT, FAIL,
                "unable to protect free space header");
  ...
  if (fspace->serial_sect_count > 0) {
    ...
    if (sinfo_status & H5AC_ES__IN_CACHE) {
      ...
      /* (Free file space) */
      {
        ...
        if (H5AC_expunge_entry(f, H5AC_FSPACE_SINFO, fspace->sect_addr,
                               cache_flags) < 0)
      ...
      if (!H5F_IS_TMP_ADDR(f, fspace->sect_addr))
        if (H5MF_xfree(f, H5FD_MEM_FSPACE_SINFO, fspace->sect_addr,
                       fspace->alloc_sect_size) < 0)
          HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL,
                      "unable to release free space sections");
    } /* end else */
  }   /* end if */

done:
  if (fspace &&
      H5AC_unprotect(f, H5AC_FSPACE_HDR, fs_addr, fspace,
                     H5AC__DELETED_FLAG | H5AC__FREE_FILE_SPACE_FLAG) < 0)
    HDONE_ERROR(H5E_FSPACE, H5E_CANTUNPROTECT, FAIL,
                "unable to release free space header");
...
herr_t H5FS_close(H5F_t *f, H5FS_t *fspace) {
...
#ifdef H5FS_DEBUG
  fprintf(stderr,
          "%s: Entering, fspace = %p, fspace->addr = %" PRIuHADDR
          ", fspace->sinfo = %p\n",
  ...
  if (fspace->sinfo) {
    ...
    fprintf(stderr,
            ...
            __func__, fspace->tot_sect_count, fspace->serial_sect_count,
            fspace->sect_addr, fspace->rc);
    fprintf(stderr,
    ...
    if (fspace->serial_sect_count > 0 && H5_addr_defined(fspace->addr)) {
      ...
      if (fspace->sinfo->dirty) {
        ...
        if (!H5_addr_defined(fspace->sect_addr)) {
          ...
          if (H5F_USE_TMP_SPACE(f)) {
            if (HADDR_UNDEF ==
                (fspace->sect_addr = H5MF_alloc_tmp(f, fspace->sect_size)))
            ...
            if (HADDR_UNDEF ==
                (fspace->sect_addr =
                     H5MF_alloc(f, H5FD_MEM_FSPACE_SINFO, fspace->sect_size)))
              HGOTO_ERROR(H5E_FSPACE, H5E_NOSPACE, FAIL,
                          "file allocation failed for free space sections");
          } /* end if */
      ...
      if (H5AC_insert_entry(f, H5AC_FSPACE_SINFO, fspace->sect_addr,
      ...
      if (H5_addr_defined(fspace->sect_addr)) {
        ...
        if (fspace->client == H5FS_CLIENT_FILE_ID) {
          ...
          fprintf(stderr, "%s: Section info is for file free space\n",
                  __func__);
#endif /* H5FS_DEBUG */
          /* Try to shrink the file or absorb the section info into a block
           * aggregator */
          if (H5F_IS_TMP_ADDR(f, fspace->sect_addr)) {
#ifdef H5FS_DEBUG
            fprintf(stderr,
                    "%s: Section info in temp. address space went 'go away'\n",
                    __func__);
#endif /* H5FS_DEBUG */
            ...
            if ((status = H5MF_try_shrink(f, H5FD_MEM_FSPACE_SINFO,
                                          fspace->sect_addr,
                                          fspace->alloc_sect_size)) < 0)
...
#ifdef H5FS_DEBUG
              fprintf(stderr,
                      "%s: Section info can't 'go away', header will own it\n",
                      __func__);
#endif        /* H5FS_DEBUG */
            } /* end if */
            else {
          ...
          hsize_t old_alloc_sect_size =
              fspace
                  ->alloc_sect_size; /* Previous size of section info in file */
      ...
      if (H5FS__sinfo_dest(fspace->sinfo) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTCLOSEOBJ, FAIL,
                    "unable to destroy free space section info");
    } /* end else */
  ...
  if (H5FS__decr(fspace) < 0)
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTDEC, FAIL,
                "unable to decrement ref. count on free space header");
 ...
 *-------------------------------------------------------------------------
 */
H5FS_t *H5FS__new(const H5F_t *f, uint16_t nclasses,
                  const H5FS_section_class_t *classes[], void *cls_init_udata) {
  ...
  if (nclasses > 0) {
    if (NULL ==
        (fspace->sect_cls = H5FL_SEQ_MALLOC(H5FS_section_class_t, nclasses)))
      HGOTO_ERROR(
          H5E_RESOURCE, H5E_NOSPACE, NULL,
    ...
    for (u = 0; u < nclasses; u++) {
      /* Make certain that section class type can be used as an array index into
       * this array */
      assert(u == classes[u]->type);

      /* Copy the class information into the free space manager */
      H5MM_memcpy(&fspace->sect_cls[u], classes[u],
                  sizeof(H5FS_section_class_t));

      /* Call the class initialization routine, if there is one */
      if (fspace->sect_cls[u].init_cls)
        if ((fspace->sect_cls[u].init_cls)(&fspace->sect_cls[u],
                                           cls_init_udata) < 0)
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, NULL,
                      "unable to initialize section class");
  ...
  if (!ret_value)
    if (fspace) {
       ...
       */
      if (fspace->sect_cls)
        fspace->sect_cls = (H5FS_section_class_t *)H5FL_SEQ_FREE(
            H5FS_section_class_t, fspace->sect_cls);
...
herr_t H5FS_size(const H5FS_t *fspace, hsize_t *meta_size) {
  ...
  *meta_size += fspace->hdr_size +
                (fspace->sinfo ? fspace->sect_size : fspace->alloc_sect_size);
...
herr_t H5FS__incr(H5FS_t *fspace) {
...
#ifdef H5FS_DEBUG
  fprintf(stderr,
          "%s: Entering, fpace->addr = %" PRIuHADDR ", fspace->rc = %u\n",
  ...
  if (fspace->rc == 0 && H5_addr_defined(fspace->addr))
    if (H5AC_pin_protected_entry(fspace) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTPIN, FAIL,
                  "unable to pin free space header");
  ...
  if (fspace->rc == 0) {
    if (H5_addr_defined(fspace->addr)) {
      if (H5AC_unpin_entry(fspace) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTUNPIN, FAIL,
                    "unable to unpin free space header");
    } /* end if */
    else {
      if (H5FS__hdr_dest(fspace) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTCLOSEOBJ, FAIL,
...
herr_t H5FS_alloc_hdr(H5F_t *f, H5FS_t *fspace, haddr_t *fs_addr) {
  ...
  if (!H5_addr_defined(fspace->addr)) {
    ...
    if (HADDR_UNDEF ==
        (fspace->addr =
             H5MF_alloc(f, H5FD_MEM_FSPACE_HDR, (hsize_t)H5FS_HEADER_SIZE(f))))
...
herr_t H5FS_alloc_sect(H5F_t *f, H5FS_t *fspace) {
  ...
  if (!H5_addr_defined(fspace->sect_addr) && fspace->sinfo &&
      fspace->serial_sect_count > 0) {
    if (HADDR_UNDEF == (fspace->sect_addr = H5MF_alloc(f, H5FD_MEM_FSPACE_SINFO,
                                                       fspace->sect_size)))
      HGOTO_ERROR(H5E_FSPACE, H5E_NOSPACE, FAIL,
...
herr_t H5FS_free(H5F_t *f, H5FS_t *fspace, hbool_t free_file_space) {
  ...
  if (H5_addr_defined(fspace->sect_addr)) {
    ...
    if (sinfo_status & H5AC_ES__IN_CACHE || !fspace->sinfo) {
      ...
      if (NULL == (fspace->sinfo = (H5FS_sinfo_t *)H5AC_protect(
        ...
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTPROTECT, FAIL,
                    "unable to protect free space section info");
  ...
  if (H5_addr_defined(fspace->addr)) {
    ...
    if (hdr_status & H5AC_ES__IN_CACHE) {
      ...
      cache_udata.cls_init_udata = NULL;
      if (NULL ==
          (fspace = (H5FS_t *)H5AC_protect(f, H5AC_FSPACE_HDR, fspace->addr,
                                           &cache_udata, H5AC__READ_ONLY_FLAG)))
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTPROTECT, FAIL,
                    "unable to protect free space section info");
      ...
      if (H5AC_unprotect(f, H5AC_FSPACE_HDR, fspace->addr, fspace,
                         cache_flags) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTUNPROTECT, FAIL,
                    "unable to release free space section info");
    } /* end if */
    ...
    if (free_file_space && H5MF_xfree(f, H5FD_MEM_FSPACE_HDR, saved_addr,
                                      (hsize_t)H5FS_HEADER_SIZE(f)) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL,
                  "unable to free free space header");
  } /* end if */
    ...
    if (fspace->sect_cls[u].term_cls)
      if ((fspace->sect_cls[u].term_cls)(&fspace->sect_cls[u]) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTRELEASE, FAIL,
                    "unable to finalize section class");
  } /* end for */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__sinfo_free_sect_cb(void *_sect, void H5_ATTR_UNUSED *key,
                                       void *op_data) {
  H5FS_section_info_t *sect =
      (H5FS_section_info_t *)_sect; /* Section to free */
  const H5FS_sinfo_t *sinfo =
      (const H5FS_sinfo_t *)op_data; /* Free space manager for section */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__sinfo_free_node_cb(void *item, void H5_ATTR_UNUSED *key,
                                       void *op_data) {
  H5FS_node_t *fspace_node =
      (H5FS_node_t *)item; /* Temporary pointer to free space list node */
...
herr_t H5FS__sinfo_dest(H5FS_sinfo_t *sinfo) {
  ...
  if (sinfo->merge_list)
    if (H5SL_close(sinfo->merge_list) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTCLOSEOBJ, FAIL,
                  "can't destroy section merging skip list");
...
void H5FS__assert(const H5FS_t *fspace) {
  ...
  assert(fspace->tot_sect_count ==
         (fspace->serial_sect_count + fspace->ghost_sect_count));
