...
int HDrand(void) { return rand_r(&g_seed); }

void HDsrand(unsigned int seed) { g_seed = seed; }
...
time_t H5_make_time(struct tm *tm) {
  ...
  if ((time_t)-1 == (the_time = HDmktime(tm)))
    HGOTO_ERROR(H5E_INTERNAL, H5E_CANTCONVERT, FAIL,
...
int Wflock(int fd, int operation) {
  ...
  if (operation & LOCK_UN) {
    if (0 == UnlockFileEx(hFile, dwReserved, nNumberOfBytesToLockLow,
      ...
      /* Attempting to unlock an already unlocked file will fail and this can
       * happen in H5Fstart_swmr_write(). For now, just ignore the "error"
       * (error code: 0x9e / 158).
       */
      if (GetLastError() != 158)
        return -1;
 ...
 *-------------------------------------------------------------------------
 */
wchar_t *H5_get_utf16_str(const char *s) {
  ...
  if (NULL ==
      (ret_s = (wchar_t *)H5MM_calloc(sizeof(wchar_t) * (size_t)nwchars)))
    goto error;
 ...
 *-------------------------------------------------------------------------
 */
int Wopen_utf8(const char *path, int oflag, ...) {
 ...
 *-------------------------------------------------------------------------
 */
int Wremove_utf8(const char *path) {
  wchar_t *wpath = NULL; /* UTF-16 version of the path */
  int ret = -1;
 ...
 *
 * Purpose:     To build the path for later searching of target file for
 *external links and external files.  This path can be either:
  ...
  if (H5_CHECK_ABSOLUTE(name)) {
    ...
    if (retcwd != NULL) {
      ...
      if (NULL == (full_path = (char *)H5MM_malloc(path_len)))
        HGOTO_ERROR(H5E_INTERNAL, H5E_NOSPACE, FAIL,
      ...
      HDstrncat(full_path, new_name,
                path_len - (cwdlen + 1) - HDstrlen(H5_DIR_SEPS));
    } /* end if */
  }   /* end else */
 ...
 *--------------------------------------------------------------------------
 */
herr_t H5_combine_path(const char *path1, const char *path2,
                       char **full_name /*out*/) {
  ...
  if (path1 == NULL || *path1 == '\0' || H5_CHECK_ABSOLUTE(path2)) {
    ...
    if (H5_CHECK_ABSOLUTE(path1) || H5_CHECK_ABS_DRIVE(path1)) {
      ...
      if (NULL == (*full_name = (char *)H5MM_malloc(path2_len + 3)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                    "unable to allocate path2 buffer");
     ...
     */
    if (NULL ==
        (*full_name = (char *)H5MM_malloc(
             path1_len + path2_len + 2 +
...
herr_t H5_expand_windows_env_vars(char **env_var) {
  ...
  if (NULL ==
      (temp_buf = (char *)H5MM_calloc((size_t)H5_WIN32_ENV_VAR_BUFFER_SIZE)))
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, FAIL,
                "can't allocate memory for expanded path");

  /* Expand the environment variable string */
  if ((n_chars = ExpandEnvironmentStringsA(*env_var, temp_buf,
...
char *H5_strndup(const char *s, size_t n) {
  ...
  if (NULL == (ret_value = H5MM_malloc(len + 1)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                "can't allocate buffer for string");
...
herr_t H5_dirname(const char *path, char **dirname) {
  ...
  if (NULL == out)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "can't allocate buffer for dirname");
...
herr_t H5_basename(const char *path, char **basename) {
  ...
  if (NULL == out)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "can't allocate buffer for basename");
 ...
 *-------------------------------------------------------------------------
 */
int H5_get_option(int argc, const char *const *argv, const char *opts,
                  const struct h5_long_options *l_opts) {
  static int sp = 1; /* character index in current token */
  int optchar = '?'; /* option character passed back to user */

  if (sp == 1) {
    /* check for more flag-like tokens */
    if (H5_optind >= argc || argv[H5_optind][0] != '-' ||
        argv[H5_optind][1] == '\0') {
      return EOF;
