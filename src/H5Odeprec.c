...
static herr_t H5O__iterate1_adapter(hid_t obj_id, const char *name,
                                    const H5O_info2_t *oinfo2, void *op_data);
static herr_t H5O__get_info_old(H5VL_object_t *vol_obj,
...
static herr_t H5O__iterate1_adapter(hid_t obj_id, const char *name,
  ...
  dm_fields =
      shim_data->fields & (H5O_INFO_BASIC | H5O_INFO_TIME | H5O_INFO_NUM_ATTRS);
  if (dm_fields) {
  ...
  if (nat_fields) {
    ...
    if (NULL == (vol_obj = H5VL_vol_object(obj_id)))
      HGOTO_ERROR(H5E_OHDR, H5E_BADTYPE, H5_ITER_ERROR,
    ...
    if (H5VL_object_optional(vol_obj, &loc_params, &vol_cb_args,
      ...
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                  "can't get native info for object");
    ...
    if (shim_data->fields & H5O_INFO_META_SIZE) {
      H5MM_memcpy(&(oinfo.meta_size.obj), &(nat_info.meta_size.obj),
                  sizeof(H5_ih_info_t));
      H5MM_memcpy(&(oinfo.meta_size.attr), &(nat_info.meta_size.attr),
                  sizeof(H5_ih_info_t));
    }
  }
  ...
  ret_value =
      (shim_data->real_op)(obj_id, name, &oinfo, shim_data->real_op_data);
  ...
  if (dm_fields) {
    ...
    if (fields & H5O_INFO_BASIC) {
      ...
      if (NULL == (vol_obj_data = H5VL_object_data(vol_obj)))
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                    "can't get underlying VOL object");
      ...
      /* Deserialize VOL object token into object address */
      if (H5VL_native_token_to_addr(vol_obj_data, loc_params->obj_type,
                                    dm_info.token, &oinfo->addr) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTUNSERIALIZE, FAIL,
                    "can't deserialize object token into address");
    } /* end if */
...
hid_t H5Oopen_by_addr(hid_t loc_id, haddr_t addr) {
  ...
  if (is_native_vol_obj) {
    /* This is a native-specific routine that requires serialization of the
     * token */
    ...
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, H5I_INVALID_HID,
                "H5Oopen_by_addr is only meant to be used with the native VOL "
                "connector");
...
herr_t H5Oget_info1(hid_t loc_id, H5O_info1_t *oinfo /*out*/) {
  ...
  if (H5O__get_info_old(vol_obj, &loc_params, oinfo, H5O_INFO_ALL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't get deprecated info for object");
...
herr_t H5Oget_info_by_name1(hid_t loc_id, const char *name,
                            H5O_info1_t *oinfo /*out*/, hid_t lapl_id) {
  H5VL_object_t *vol_obj = NULL; /* object of loc_id */
  H5VL_loc_params_t loc_params;
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5Oget_info_by_idx1(hid_t loc_id, const char *group_name,
                           ...
                           hsize_t n, H5O_info1_t *oinfo /*out*/,
                           hid_t lapl_id) {
  H5VL_object_t *vol_obj = NULL; /* object of loc_id */
  H5VL_loc_params_t loc_params;
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 * Purpose:     Retrieve information about an object.
 *
 *              NOTE: Add a parameter "fields" to indicate selection of object
 *info.
 *
 * Return:      SUCCEED/FAIL
 *
...
herr_t H5Oget_info2(hid_t loc_id, H5O_info1_t *oinfo /*out*/, unsigned fields) {
  ...
  if (!is_native_vol_obj)
    HGOTO_ERROR(
        H5E_OHDR, H5E_BADVALUE, H5I_INVALID_HID,
...
herr_t H5Oget_info_by_name2(hid_t loc_id, const char *name,
                            H5O_info1_t *oinfo /*out*/, unsigned fields,
                            hid_t lapl_id) {
  H5VL_object_t *vol_obj; /* Object of loc_id */
  H5VL_loc_params_t loc_params;
  hbool_t is_native_vol_obj;
  ...
  if (!is_native_vol_obj)
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, H5I_INVALID_HID,
                "H5Oget_info_by_name2 is only meant to be used with the native "
                "VOL connector");
...
herr_t H5Oget_info_by_idx2(hid_t loc_id, const char *group_name,
  ...
  if (!is_native_vol_obj)
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, H5I_INVALID_HID,
                "H5Oget_info_by_idx2 is only meant to be used with the native "
                "VOL connector");
...
herr_t H5Ovisit1(hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
  ...
  H5O_visit1_adapter_t
      shim_data;    /* Adapter for passing app callback & user data */
...
herr_t H5Ovisit_by_name2(hid_t loc_id, const char *obj_name,
  ...
  if (!is_native_vol_obj)
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, H5I_INVALID_HID,
                "H5Ovisit_by_name2 is only meant to be used with the native "
                "VOL connector");
