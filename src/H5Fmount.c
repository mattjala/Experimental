...
/* PRIVATE PROTOTYPES */
static void H5F__mount_count_ids_recurse(H5F_t *f, unsigned *nopen_files,
                                         unsigned *nopen_objs);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_mount(const H5G_loc_t *loc, const char *name, H5F_t *child,
  ...
  if (mp_loc.oloc->holding_file != FALSE)
    HGOTO_ERROR(H5E_FILE, H5E_MOUNT, FAIL,
                "mount path cannot contain links to external files");
  ...
  /* Make certain that the parent & child files have the same "file close
   * degree" */
  if (parent->shared->fc_degree != child->shared->fc_degree)
    HGOTO_ERROR(H5E_FILE, H5E_MOUNT, FAIL,
                "mounted file has different file close degree than parent");
  ...
  if (parent->shared->mtab.nmounts >= parent->shared->mtab.nalloc) {
    ...
    H5F_mount_t *x = (H5F_mount_t *)H5MM_realloc(
        parent->shared->mtab.child, n * sizeof(parent->shared->mtab.child[0]));
  ...
  if (H5G_mount(parent->shared->mtab.child[md].group) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTCLOSEOBJ, FAIL,
                "unable to set group mounted flag");
  ...
  if (H5G_name_replace(NULL, H5G_NAME_MOUNT, mp_loc.oloc->file,
                       mp_loc.path->full_path_r, root_loc.oloc->file,
                       root_loc.path->full_path_r) < 0)
...
static void H5F__mount_count_ids_recurse(H5F_t *f, unsigned *nopen_files,
  ...
  *nopen_objs += (f->nopen_objs - f->nmounts);
  ...
  /* Iterate over files mounted in this file and add in their open ID counts
   * also */
  for (u = 0; u < f->shared->mtab.nmounts; u++) {
    ...
    if (f->shared->mtab.child[u].file->parent == f) {
      ...
      H5F__mount_count_ids_recurse(f->shared->mtab.child[u].file, nopen_files,
                                   nopen_objs);
    }
  }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__mount_count_ids(H5F_t *f, unsigned *nopen_files,
                            unsigned *nopen_objs) {
  FUNC_ENTER_PACKAGE_NOERR
...
static herr_t H5F__flush_mounts_recurse(H5F_t *f) {
  ...
  if (nerrors)
    HGOTO_ERROR(H5E_FILE, H5E_CANTFLUSH, FAIL,
                "unable to flush file's child mounts");
    ...
    if (0 == cmp) {
      ...
      if (H5O_loc_free(oloc) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTFREE, FAIL,
                    "unable to free object location");
