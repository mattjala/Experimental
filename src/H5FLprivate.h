...
#define H5FL_TRACK_PARAMS                                                      \
  , const char *call_file, const char *call_func, int call_line
...
#define H5FL_DEFINE_COMMON(t)                                                  \
  H5FL_reg_head_t H5FL_REG_NAME(t) = {0, 0, 0, #t, sizeof(t), NULL}
...
#define H5FL_BLK_DEFINE_COMMON(t)                                              \
  H5FL_blk_head_t H5FL_BLK_NAME(t) = {0, 0, 0, 0, #t "_blk", NULL}
...
#define H5FL_BLK_REALLOC(t, blk, new_size)                                     \
  (uint8_t *)H5FL_blk_realloc(&(H5FL_BLK_NAME(t)), blk,                        \
                              new_size H5FL_TRACK_INFO)

/* Check if there is a free block available to re-use */
#define H5FL_BLK_AVAIL(t, size)                                                \
...
/* Common macro for H5FL_ARR_DEFINE & H5FL_ARR_DEFINE_STATIC (and H5FL_BARR
 * variants) */
#define H5FL_ARR_DEFINE_COMMON(b, t, m)                                        \
  H5FL_arr_head_t H5FL_ARR_NAME(t) = {0,     0, 0,         #t "_arr",          \
                                      m + 1, b, sizeof(t), NULL}
...
#define H5FL_BARR_DEFINE_STATIC(b, t, m)                                       \
  static H5FL_ARR_DEFINE_COMMON(sizeof(b), t, m)

/* Allocate an array of type 't' */
#define H5FL_ARR_MALLOC(t, elem)                                               \
  H5FL_arr_malloc(&(H5FL_ARR_NAME(t)), elem H5FL_TRACK_INFO)

/* Allocate an array of type 't' and clear it to all zeros */
#define H5FL_ARR_CALLOC(t, elem)                                               \
  H5FL_arr_calloc(&(H5FL_ARR_NAME(t)), elem H5FL_TRACK_INFO)
...
#define H5FL_BARR_DEFINE(b, t, m)                                              \
  H5_DLL H5FL_ARR_DEFINE_COMMON(t, m) = sizeof(b)
...
#define H5FL_BARR_DEFINE_STATIC(b, t, m)                                       \
  static H5FL_ARR_DEFINE_COMMON(t, m) = sizeof(b)
#define H5FL_ARR_MALLOC(t, elem)                                               \
  H5MM_malloc(H5FL_ARR_NAME(t) + ((elem) * sizeof(t)))
#define H5FL_ARR_CALLOC(t, elem)                                               \
  H5MM_calloc(H5FL_ARR_NAME(t) + ((elem) * sizeof(t)))
#define H5FL_ARR_FREE(t, obj) (t *)H5MM_xfree(obj)
#define H5FL_ARR_REALLOC(t, obj, new_elem)                                     \
  H5MM_realloc(obj, H5FL_ARR_NAME(t) + ((new_elem) * sizeof(t)))
...
#define H5FL_SEQ_REALLOC(t, obj, new_elem)                                     \
  (t *)H5MM_realloc(obj, (new_elem) * sizeof(t))
...
H5_DLL void *H5FL_seq_realloc(H5FL_seq_head_t *head, void *obj,
                              size_t new_elem H5FL_TRACK_PARAMS);

/* Factory free lists */
H5_DLL H5FL_fac_head_t *H5FL_fac_init(size_t size);
H5_DLL void *
H5FL_fac_malloc(H5FL_fac_head_t *head H5FL_TRACK_PARAMS) H5_ATTR_MALLOC;
H5_DLL void *
H5FL_fac_calloc(H5FL_fac_head_t *head H5FL_TRACK_PARAMS) H5_ATTR_MALLOC;
