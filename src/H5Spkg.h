...
#define H5S_SELECT_INFO_ENC_SIZE_BITS                                          \
  (H5S_SELECT_INFO_ENC_SIZE_2 | H5S_SELECT_INFO_ENC_SIZE_4 |                   \
   H5S_SELECT_INFO_ENC_SIZE_8)

#define H5S_UINT16_MAX                                                         \
  0x0000FFFF /* 2^16 - 1 = 65,535                            */
#define H5S_UINT32_MAX                                                         \
  0xFFFFFFFF /* 2^32 - 1 = 4,294,967,295                     */
#define H5S_UINT64_MAX                                                         \
  ((hsize_t)(-1L)) /* 2^64 - 1 = 18,446,744,073,709,551,615 */
...
#define H5S_SEL_ITER_API_CALL                                                  \
  0x1000 /* Selection iterator created from API call */
...
struct H5S_extent_t {
  ...
  unsigned
      version; /* Version of object header message to encode this object with */
...
struct H5S_pnt_list_t {
  /* The following two fields defines the bounding box of the whole set of
   * points, relative to the offset */
  hsize_t low_bounds[H5S_MAX_RANK];  /* The smallest element selected in each
                                        dimension */
  hsize_t high_bounds[H5S_MAX_RANK]; /* The largest element selected in each
                                        dimension */
  ...
  H5S_pnt_node_t
      *last_idx_pnt; /* Point after the last returned from
                      * H5S__get_select_elem_pointlist(). If we ever add a way
                      * to remove points or add points in the middle of the
                      * pointlist we will need to invalidate these fields. */
};
...
struct H5S_hyper_span_t {
  hsize_t low,
      high; /* Low & high bounds of elements selected for span, inclusive */
...
typedef struct H5S_hyper_op_info_t {
  ...
  union {
    struct H5S_hyper_span_info_t
        *copied;     /* Pointer to already copied span tree */
...
/* Information about a list of hyperslab spans in one dimension (typedef'd in
 * H5Sprivate.h) */
struct H5S_hyper_span_info_t {
  ...
  struct H5S_hyper_span_t *head; /* Pointer to the first span of list of spans
                                    in the current dimension */
  struct H5S_hyper_span_t *tail; /* Pointer to the last span of list of spans in
...
typedef enum {
  H5S_DIMINFO_VALID_IMPOSSIBLE, /* 0: diminfo is not valid and can never be
                                 * valid with the current selection
                                 */
  H5S_DIMINFO_VALID_NO,         /* 1: diminfo is not valid but may or may not be
...
typedef struct {
   ...
   */
  H5S_hyper_dim_t
      app[H5S_MAX_RANK]; /* Application-set per-dim selection info */
...
typedef struct {
  ...
  hsize_t num_elem_non_unlim; /* # of elements in a "slice" excluding the
                                 unlimited dimension */
  H5S_hyper_span_info_t
      *span_lst; /* List of hyperslab span information of all dimensions */
} H5S_hyper_sel_t;

/* Selection information methods */
/* Method to copy a selection */
typedef herr_t (*H5S_sel_copy_func_t)(H5S_t *dst, const H5S_t *src,
...
/* Method to store current selection in "serialized" form (a byte sequence
 * suitable for storing on disk) */
typedef herr_t (*H5S_sel_serialize_func_t)(H5S_t *space, uint8_t **p);
/* Method to create selection from "serialized" form (a byte sequence suitable
 * for storing on disk) */
typedef herr_t (*H5S_sel_deserialize_func_t)(H5S_t **space, const uint8_t **p,
                                             const size_t p_size, hbool_t skip);
/* Method to determine smallest n-D bounding box containing the current
 * selection */
typedef herr_t (*H5S_sel_bounds_func_t)(const H5S_t *space, hsize_t *start,
                                        hsize_t *end);
/* Method to determine linear offset of initial element in selection within
 * dataspace */
...
/* Method to get the number of elements in a slice through the unlimited
 * dimension */
typedef herr_t (*H5S_sel_num_elem_non_unlim_func_t)(
    const H5S_t *space, hsize_t *num_elem_non_unlim);
...
typedef htri_t (*H5S_sel_intersect_block_func_t)(H5S_t *space,
                                                 const hsize_t *start,
                                                 const hsize_t *end);
...
typedef herr_t (*H5S_sel_project_simple)(const H5S_t *space, H5S_t *new_space,
                                         hsize_t *offset);
/* Method to initialize iterator for current selection */
typedef herr_t (*H5S_sel_iter_init_func_t)(H5S_t *space,
...
typedef struct {
  ...
  H5S_sel_is_valid_func_t is_valid; /* Method to determine if current selection
                                       is valid for dataspace */
  H5S_sel_serial_size_func_t
      serial_size; /* Method to determine number of bytes required to store
                      current selection */
  H5S_sel_serialize_func_t
      serialize; /* Method to store current selection in "serialized" form (a
                    byte sequence suitable for storing on disk) */
  H5S_sel_deserialize_func_t
      deserialize; /* Method to store create selection from "serialized" form (a
                      byte sequence suitable for storing on disk) */
  H5S_sel_bounds_func_t bounds; /* Method to determine to smallest n-D bounding
                                   box containing the current selection */
  H5S_sel_offset_func_t offset; /* Method to determine linear offset of initial
                                   element in selection within dataspace */
  H5S_sel_unlim_dim_func_t unlim_dim; /* Method to get unlimited dimension of
                                         selection (or -1 for none) */
  H5S_sel_num_elem_non_unlim_func_t
      num_elem_non_unlim; /* Method to get the number of elements in a slice
                             through the unlimited dimension */
  H5S_sel_is_contiguous_func_t is_contiguous; /* Method to determine if current
                                                 selection is contiguous */
  H5S_sel_is_single_func_t is_single;         /* Method to determine if current
  ...
  H5S_sel_shape_same_func_t
      shape_same; /* Method to determine if two dataspaces' selections are the
                     same shape */
  H5S_sel_intersect_block_func_t
      intersect_block; /* Method to determine if a dataspaces' selection
                          intersects a block */
  H5S_sel_adjust_u_func_t
      adjust_u; /* Method to adjust a selection by an offset */
  H5S_sel_adjust_s_func_t
      adjust_s; /* Method to adjust a selection by an offset (signed) */
...
typedef struct {
  ...
  hbool_t offset_changed; /* Indicate that the offset for the selection has been
                             changed */
  ...
  union {
    H5S_pnt_list_t
        *pnt_lst; /* Info about list of selected points (order is important) */
...
typedef htri_t (*H5S_sel_iter_has_next_block_func_t)(
    const H5S_sel_iter_t *iter);
...
typedef struct H5S_sel_iter_class_t {
  ...
  H5S_sel_iter_coords_func_t
      iter_coords; /* Method to retrieve the current coordinates of iterator for
                      current selection */
  H5S_sel_iter_block_func_t iter_block; /* Method to retrieve the current block
                                           of iterator for current selection */
  H5S_sel_iter_nelmts_func_t
      iter_nelmts; /* Method to determine number of elements left in iterator
                      for current selection */
  H5S_sel_iter_has_next_block_func_t
      iter_has_next_block; /* Method to query if there is another block left in
                              the selection */
  H5S_sel_iter_next_func_t iter_next; /* Method to move selection iterator to
                                         the next element in the selection */
  H5S_sel_iter_next_block_func_t
      iter_next_block; /* Method to move selection iterator to the next block in
                          the selection */
  H5S_sel_iter_get_seq_list_func_t
      iter_get_seq_list; /* Method to retrieve a list of offset/length sequences
                            for selection iterator */
...
H5_DLL herr_t H5S__modify_select(H5S_t *space1, H5S_seloper_t op,
                                 H5S_t *space2);
H5_DLL herr_t H5S__hyper_project_intersection(H5S_t *src_space,
                                              H5S_t *dst_space,
