...
static herr_t H5F__super_ext_create(H5F_t *f, H5O_loc_t *ext_ptr) {
  ...
  /* Check for older version of superblock format that can't support superblock
   * extensions */
  if (f->shared->sblock->super_vers < HDF5_SUPERBLOCK_VERSION_2)
    HGOTO_ERROR(
        H5E_FILE, H5E_CANTCREATE, FAIL,
    ...
    if (H5O_create(f, (size_t)0, (size_t)1, H5P_GROUP_CREATE_DEFAULT, ext_ptr) <
        0)
...
herr_t H5F__super_ext_close(H5F_t *f, H5O_loc_t *ext_ptr, hbool_t was_created) {
  ...
  if (was_created) {
    ...
    if (H5O_link(ext_ptr, 1) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_LINKCOUNT, FAIL,
    ...
    if (H5O_dec_rc_by_loc(ext_ptr) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTDEC, FAIL,
                  "unable to decrement refcount on superblock extension");
  } /* end if */
  ...
  if (sblock->super_vers >= HDF5_SUPERBLOCK_VERSION_2) {
    if (H5_addr_defined(sblock->ext_addr)) {
      ...
      if (!H5F_HAS_FEATURE(f, H5FD_FEAT_IGNORE_DRVRINFO)) {
        ...
        H5_CHECKED_ASSIGN(driver_size, size_t, H5FD_sb_size(f->shared->lf),
                          hsize_t);
        ...
        if (driver_size > 0) {
          ...
          uint8_t dbuf[H5F_MAX_DRVINFOBLOCK_SIZE]; /* Driver info block encoding
                                                      buffer */
          ...
          drvinfo.buf = dbuf;
          if (H5F__super_ext_write_msg(f, H5O_DRVINFO_ID, &drvinfo, FALSE,
                                       H5O_MSG_NO_FLAGS_SET) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__super_read(H5F_t *f, H5P_genplist_t *fa_plist,
  ...
  unsigned sblock_flags =
      H5AC__NO_FLAGS_SET; /* flags used in superblock unprotect call      */
  ...
  if (H5F_HAS_FEATURE(f, H5FD_FEAT_HAS_MPI))
    if ((mpi_size = H5F_mpi_get_size(f)) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "can't retrieve MPI communicator size");
  ...
  if (mpi_size > 1) {
    ...
    if (MPI_SUCCESS != (mpi_result = MPI_Bcast(&super_addr, sizeof(super_addr),
                                               MPI_BYTE, 0, this_comm)))
    ...
    if (H5FD_locate_signature(file, &super_addr) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_NOTHDF5, FAIL,
                  "unable to locate file signature");
  ...
  if (H5_addr_gt(super_addr, 0)) {
    ...
    if (H5F__set_base_addr(f, super_addr) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                  "failed to set base address for file driver");
  } /* end if */
  ...
  if (H5F__set_eoa(f, H5FD_MEM_SUPER,
                   (haddr_t)(H5F_SUPERBLOCK_FIXED_SIZE +
                             H5F_SUPERBLOCK_MINIMAL_VARLEN_SIZE)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
  ...
  if (NULL == (sblock = (H5F_super_t *)H5AC_protect(
                   f, H5AC_SUPERBLOCK, (haddr_t)0, &udata, rw_flags)))
   ...
   *
   * For details, please see RFC:Setting Bounds for Object Creation in
   * HDF5 1.10.0.
   */
  ...
  if (H5F_INTENT(f) & H5F_ACC_SWMR_WRITE) {
    if (sblock->super_vers < HDF5_SUPERBLOCK_VERSION_3)
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                  "superblock version for SWMR is less than 3");
  }
  ...
  if (sblock->super_vers > HDF5_superblock_ver_bounds[f->shared->high_bound])
    HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                "superblock version exceeds high bound");
  ...
  if (((rw_flags & H5AC__READ_ONLY_FLAG) == 0) && udata.ignore_drvrinfo &&
      udata.drvrinfo_removed) {
  ...
  if (H5P_set(c_plist, H5F_CRT_SUPER_VERS_NAME, &sblock->super_vers) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                "unable to set superblock version");
  ...
  if (sblock->super_vers < HDF5_SUPERBLOCK_VERSION_2) {
    ...
    H5MM_memcpy(sblock->btree_k, udata.btree_k,
                sizeof(unsigned) * (size_t)H5B_NUM_BTREE_ID);
  } /* end if */
  else {
  ...
  if (H5P_exist_plist(fa_plist, H5F_ACS_SKIP_EOF_CHECK_NAME) > 0)
    if (H5P_get(fa_plist, H5F_ACS_SKIP_EOF_CHECK_NAME, &skip_eof_check) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get skip EOF check value");
  ...
  if (!skip_eof_check && initial_read) {
    ...
    if ((eof + sblock->base_addr) < udata.stored_eof)
      HGOTO_ERROR(H5E_FILE, H5E_TRUNCATED, FAIL,
                  "truncated file: eof = %llu, sblock->base_addr = %llu, "
                  "stored_eof = %llu",
  ...
  if (H5_addr_defined(sblock->driver_addr)) {
    ...
    H5F_drvrinfo_cache_ud_t
        drvrinfo_udata; /* User data for metadata callbacks */
    unsigned drvinfo_flags =
        H5AC__NO_FLAGS_SET; /* Flags used in driver info block unprotect call */
     ...
     */
    if (H5FD_set_eoa(f->shared->lf, H5FD_MEM_SUPER,
                     sblock->driver_addr + H5F_DRVINFOBLOCK_HDR_SIZE) <
    ...
    if (NULL ==
        (drvinfo = (H5O_drvinfo_t *)H5AC_protect(
             f, H5AC_DRVRINFO, sblock->driver_addr, &drvrinfo_udata, rw_flags)))
      HGOTO_ERROR(H5E_FILE, H5E_CANTPROTECT, FAIL,
                  "unable to load driver info block");
    ...
    if (H5AC_unprotect(f, H5AC_DRVRINFO, sblock->driver_addr, drvinfo,
                       drvinfo_flags) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTUNPROTECT, FAIL,
                  "unable to release driver info block");
  ...
  if (H5F__set_eoa(f, H5FD_MEM_DEFAULT, udata.stored_eof - sblock->base_addr) <
      0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
  ...
  if (H5_addr_defined(sblock->ext_addr)) {
    ...
    H5O_btreek_t
        btreek; /* v1 B-tree 'K' value message from superblock extension */
    ...
    if (sblock->super_vers < HDF5_SUPERBLOCK_VERSION_2)
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                  "invalid superblock - extension message should not be "
    ...
    if (H5_addr_gt(sblock->ext_addr, udata.stored_eof)) {
       ...
       */
      if (H5F__set_eoa(f, H5FD_MEM_OHDR, (haddr_t)(sblock->ext_addr + 1024)) <
          0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                    "unable to set end-of-address marker for file");
    } /* end if */
    ...
    if (status) {
      ...
      if (!udata.ignore_drvrinfo) {
        ...
        if (H5FD_sb_load(f->shared->lf, drvinfo.name, drvinfo.buf) < 0) {
          ...
          HGOTO_ERROR(H5E_FILE, H5E_CANTDECODE, FAIL,
    ...
    if (H5SM_get_info(&ext_loc, c_plist) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "unable to read SOHM table information");
    ...
    if (status) {
      ...
      if (NULL == H5O_msg_read(&ext_loc, H5O_BTREEK_ID, &btreek))
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                    "v1 B-tree 'K' info message not present");
    ...
    if (status) {
      ...
      if (!(flags & H5O_MSG_FLAG_WAS_UNKNOWN)) {
        H5O_fsinfo_t
            fsinfo; /* File space info message from superblock extension */
        ...
        if (H5P_exist_plist(fa_plist, H5F_ACS_NULL_FSM_ADDR_NAME) > 0)
          if (H5P_get(fa_plist, H5F_ACS_NULL_FSM_ADDR_NAME,
                      &f->shared->null_fsm_addr) < 0)
            HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
        ...
        if (NULL == H5O_msg_read(&ext_loc, H5O_FSINFO_ID, &fsinfo))
          HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                      "unable to get free-space manager info message");
        ...
        if (f->shared->fs_strategy != fsinfo.strategy) {
          ...
          if (H5P_set(c_plist, H5F_CRT_FILE_SPACE_STRATEGY_NAME,
                      &fsinfo.strategy) < 0)
            HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                        "unable to set file space strategy");
        } /* end if */
        if (f->shared->fs_persist != fsinfo.persist) {
          f->shared->fs_persist = fsinfo.persist;

          /* Set non-default strategy in the property list */
          if (H5P_set(c_plist, H5F_CRT_FREE_SPACE_PERSIST_NAME,
                      &fsinfo.persist) < 0)
            HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                        "unable to set file space strategy");
        } /* end if */
        if (f->shared->fs_threshold != fsinfo.threshold) {
          ...
          if (H5P_set(c_plist, H5F_CRT_FREE_SPACE_THRESHOLD_NAME,
                      &fsinfo.threshold) < 0)
            HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                        "unable to set file space strategy");
        } /* end if */
        ...
        if (f->shared->fs_page_size != fsinfo.page_size) {
          ...
          if (H5P_set(c_plist, H5F_CRT_FILE_SPACE_PAGE_SIZE_NAME,
                      &fsinfo.page_size) < 0)
            HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                        "unable to set file space page size");
        } /* end if */
         ...
         *  The above two conditions are enabled when the tool h5clear
         * --increment option is used.
         */
         ...
         * && (H5F_INTENT(f) & H5F_ACC_RDWR)) f->shared->first_alloc_dealloc =
         * TRUE;
         *
         * Because:
         * --there is no more f->shared->eoa_pre_fsm_fsalloc and
         *   f->shared->first_alloc_dealloc
         * --the check for null_fsm_addr is directly done in
         * H5MF_settle_meta_data_fsm() and H5MF_settle_raw_data_fsm()
         */
        ...
        /* For fsinfo.mapped: remove the FSINFO message from the superblock
           extension and write a new message to the extension */
        /* For f->shared->null_fsm_addr: just update FSINFO message in the
         * superblock extension */
        if (((fsinfo.mapped || f->shared->null_fsm_addr) &&
             (rw_flags & H5AC__READ_ONLY_FLAG) == 0)) {
    ...
    if (status) {
      ...
      if (NULL == H5O_msg_read(&ext_loc, H5O_MDCI_MSG_ID, &mdci_msg))
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                    "unable to get metadata cache image message");

      /* Indicate to the cache that there's an image to load on first protect
       * call */
      if (H5AC_load_cache_image_on_next_protect(f, mdci_msg.addr, mdci_msg.size,
                                                rw) < 0)
  ...
  if (H5F__set_paged_aggr(f, (hbool_t)H5F_PAGED_AGGR(f)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                "failed to set paged_aggr status for file driver");
  ...
  if (sblock &&
      H5AC_unprotect(f, H5AC_SUPERBLOCK, (haddr_t)0, sblock, sblock_flags) < 0)
    HDONE_ERROR(H5E_FILE, H5E_CANTUNPROTECT, FAIL,
                "unable to close superblock");
  ...
  if (ret_value < 0) {
    ...
    if (f->shared->drvinfo) {
      if (H5AC_unpin_entry(f->shared->drvinfo) < 0)
        HDONE_ERROR(H5E_FILE, H5E_CANTUNPIN, FAIL,
                    "unable to unpin driver info");

      /* Evict the driver info block from the cache */
      if (sblock) {
        if (H5AC_expunge_entry(f, H5AC_DRVRINFO, sblock->driver_addr,
                               H5AC__NO_FLAGS_SET) < 0)
          HDONE_ERROR(H5E_FILE, H5E_CANTEXPUNGE, FAIL,
                      "unable to expunge driver info block");
        f->shared->drvinfo = NULL;
      }
    } /* end if */
    ...
    if (sblock) {
      ...
      if (H5AC_expunge_entry(f, H5AC_SUPERBLOCK, (haddr_t)0,
                             H5AC__NO_FLAGS_SET) < 0)
        HDONE_ERROR(H5E_FILE, H5E_CANTEXPUNGE, FAIL,
                    "unable to expunge superblock");
      f->shared->sblock = NULL;
    } /* end if */
  }   /* end if */
...
herr_t H5F__super_init(H5F_t *f) {
  ...
  hbool_t sblock_in_cache = FALSE;  /* Whether the superblock has been inserted
                                       into the metadata cache */
  H5O_drvinfo_t *drvinfo = NULL;    /* Driver info */
  hbool_t drvinfo_in_cache = FALSE; /* Whether the driver info block has been
                                       inserted into the metadata cache */
  ...
  size_t driver_size;      /* Size of driver info block (bytes)          */
  unsigned super_vers =
      HDF5_SUPERBLOCK_VERSION_DEF; /* Superblock version for file */
  ...
  hbool_t non_default_fs_settings =
      FALSE; /* Whether the file has non-default free-space settings */
  herr_t ret_value = SUCCEED; /* Return Value                              */
  ...
  /* Finalize superblock version to that allowed by the file's low bound if
   * higher */
  super_vers =
      MAX(super_vers, HDF5_superblock_ver_bounds[f->shared->low_bound]);

  /* Version bounds check */
  if (super_vers > HDF5_superblock_ver_bounds[f->shared->high_bound])
    HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                "superblock version out of bounds");
  ...
  if (userblock_size > 0) {
    ...
    hsize_t alignment =
        H5F_PAGED_AGGR(f) ? f->shared->fs_page_size : f->shared->alignment;

    if (userblock_size < alignment)
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                  "userblock size must be > file object alignment");
    if (0 != (userblock_size % alignment))
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                  "userblock size must be an integral multiple of file object "
                  "alignment");
  } /* end if */
  ...
  if (H5F__set_eoa(f, H5FD_MEM_SUPER, userblock_size) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
                "unable to set EOA value for userblock");
  ...
  /* Save a local copy of the superblock version number, size of addresses &
   * offsets */
  ...
  if (HADDR_UNDEF == H5MF_alloc(f, H5FD_MEM_SUPER, superblock_size))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "file allocation failed for superblock");
  ...
  /* Files with non-default free space settings always need the superblock
     extension */
  else if (non_default_fs_settings) {
    assert(super_vers >= HDF5_SUPERBLOCK_VERSION_2);
    need_ext = TRUE;
  ...
  if (need_ext) {
     ...
     */
    if (f->shared->sohm_nindexes > 0) {
      /* Initialize the shared message code & write the SOHM message to the
       * extension */
      if (H5SM_init(f, plist, &ext_loc) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
                    "unable to create SOHM table");
    }
    ...
    if (sblock->btree_k[H5B_SNODE_ID] != HDF5_BTREE_SNODE_IK_DEF ||
        ...
        sblock->sym_leaf_k != H5F_CRT_SYM_LEAF_DEF) {
      H5O_btreek_t
          btreek; /* v1 B-tree 'K' value message for superblock extension */
      ...
      if (H5O_msg_create(&ext_loc, H5O_BTREEK_ID,
    ...
    if (driver_size > 0) {
      ...
      info.buf = dbuf;
      if (H5O_msg_create(&ext_loc, H5O_DRVINFO_ID, H5O_MSG_FLAG_DONTSHARE,
                         H5O_UPDATE_TIME, &info) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
                    "unable to update driver info header message");
    ...
    if (non_default_fs_settings) {
      ...
      /* Write free-space manager info message to superblock extension object
       * header if needed */
      ...
      if (H5O_fsinfo_set_version(H5F_LOW_BOUND(f), H5F_HIGH_BOUND(f), &fsinfo) <
          0)
      ...
      if (H5O_msg_create(&ext_loc, H5O_FSINFO_ID,
                         H5O_MSG_FLAG_DONTSHARE | H5O_MSG_FLAG_MARK_IF_UNKNOWN,
                         H5O_UPDATE_TIME, &fsinfo) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
                    "unable to update free-space info header message");
    } /* end if */
  }   /* end if */
  else {
    ...
    if (driver_size > 0) {
      ...
      if (NULL ==
          (drvinfo = (H5O_drvinfo_t *)H5MM_calloc(sizeof(H5O_drvinfo_t))))
       ...
       */
      H5_CHECKED_ASSIGN(drvinfo->len, size_t, H5FD_sb_size(f->shared->lf),
                        hsize_t);
      ...
      if (H5AC_insert_entry(f, H5AC_DRVRINFO, sblock->driver_addr, drvinfo,
                            H5AC__PIN_ENTRY_FLAG | H5AC__FLUSH_LAST_FLAG |
                                H5AC__FLUSH_COLLECTIVELY_FLAG) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTINS, FAIL,
                    "can't add driver info block to cache");
...
herr_t H5F_eoa_dirty(H5F_t *f) {
  ...
  if (f->shared->drvinfo) {
    if (H5AC_mark_entry_dirty(f->shared->drvinfo) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTMARKDIRTY, FAIL,
                  "unable to mark drvinfo as dirty");
  } /* end if */
  /* If the driver info is stored as a message, update that instead */
    ...
    if (H5F__update_super_ext_driver_msg(f) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTMARKDIRTY, FAIL,
                  "unable to mark drvinfo message as dirty");
  }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__super_ext_write_msg(H5F_t *f, unsigned id, void *mesg,
  ...
  if (H5_addr_defined(f->shared->sblock->ext_addr)) {
    ...
    if (H5F__super_ext_create(f, &ext_loc) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTCREATE, FAIL,
                  "unable to create file's superblock extension");
    ext_created = TRUE;
  } /* end else */
  ...
  if (may_create) {
    ...
    if (H5O_msg_create(&ext_loc, id, (mesg_flags | H5O_MSG_FLAG_DONTSHARE),
                       H5O_UPDATE_TIME, mesg) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "unable to create the message in object header");
  } /* end if */
  else {
    if (!status)
    ...
    if (H5O_msg_write(&ext_loc, id, (mesg_flags | H5O_MSG_FLAG_DONTSHARE),
                      H5O_UPDATE_TIME, mesg) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "unable to write the message in object header");
  } /* end else */
  ...
  if ((status = H5O_msg_exists(&ext_loc, id)) < 0)
    ...
    if (H5O_msg_remove(&ext_loc, id, H5O_ALL, TRUE) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, FAIL,
                  "unable to delete free-space manager info message");
