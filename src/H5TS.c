...
static herr_t H5TS__mutex_acquire(H5TS_mutex_t *mutex, unsigned int lock_count,
                                  bool *acquired);
...
herr_t H5TSmutex_acquire(unsigned int lock_count, bool *acquired){
    FUNC_ENTER_API_NAMECHECK_ONLY
        ...
        FUNC_LEAVE_API_NAMECHECK_ONLY(
            H5TS__mutex_acquire(&H5_g.init_lock, lock_count, acquired))}
...
herr_t H5TS_mutex_lock(H5TS_mutex_t *mutex) {
  ...
  EnterCriticalSection(&mutex->CriticalSection);
#else  /* H5_HAVE_WIN_THREADS */
  /* Acquire the "attempt" lock, increment the attempt lock count, release the
   * lock */
 ...
 *--------------------------------------------------------------------------
 */
herr_t H5TS_have_mutex(H5TS_mutex_t *mutex, bool *have_mutex_ptr) {
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5TS__mutex_unlock(H5TS_mutex_t *mutex,
...
herr_t H5TS_cancel_count_inc(void) {
  ...
  cancel_counter =
      (H5TS_cancel_t *)H5TS_get_thread_local_value(H5TS_cancel_key_s);
 ...
 *--------------------------------------------------------------------------
 */
H5TS_thread_t H5TS_create_thread(H5TS_thread_cb_t func, H5TS_attr_t *attr,
                                 void *udata) {
   ...
   */
  ret_value =
      CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)func, udata, 0, NULL);
