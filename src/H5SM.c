...
static herr_t H5SM__read_mesg_fh_cb(const void *obj, size_t obj_len,
                                    void *_udata);
static herr_t H5SM__read_mesg(H5F_t *f, const H5SM_sohm_t *mesg, H5HF_t *fheap,
                              H5O_t *open_oh, size_t *encoding_size /*out*/,
...
herr_t H5SM_init(H5F_t *f, H5P_genplist_t *fc_plist, const H5O_loc_t *ext_loc) {
  ...
  unsigned index_type_flags[H5O_SHMESG_MAX_NINDEXES]; /* Messages types stored
                                                         in each index */
  unsigned minsizes[H5O_SHMESG_MAX_NINDEXES]; /* Message size sharing threshold
                                                 for each index */
  ...
  if (NULL == (table = H5FL_CALLOC(H5SM_master_table_t)))
    HGOTO_ERROR(H5E_SOHM, H5E_CANTALLOC, FAIL,
                "memory allocation failed for SOHM table");
  ...
  if (H5P_get(fc_plist, H5F_CRT_SHMSG_INDEX_MINSIZE_NAME, &minsizes) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTGET, FAIL,
                "can't get SOHM message min sizes");

  /* Verify that values are valid */
  if (table->num_indexes > H5O_SHMESG_MAX_NINDEXES)
    HGOTO_ERROR(H5E_SOHM, H5E_BADRANGE, FAIL,
                "number of indexes in property list is too large");
  ...
  for (x = 0; x < table->num_indexes; ++x) {
    if (index_type_flags[x] & type_flags_used)
      HGOTO_ERROR(H5E_SOHM, H5E_BADVALUE, FAIL,
                  "the same shared message type flag is assigned to more than "
                  "one index");
  ...
  assert(table->num_indexes > 0 &&
         table->num_indexes <= H5O_SHMESG_MAX_NINDEXES);

  /* Allocate the SOHM indexes as an array. */
  if (NULL == (table->indexes = (H5SM_index_header_t *)H5FL_ARR_MALLOC(
                   H5SM_index_header_t, (size_t)table->num_indexes)))
    HGOTO_ERROR(H5E_SOHM, H5E_NOSPACE, FAIL,
  ...
  if (HADDR_UNDEF == (table_addr = H5MF_alloc(f, H5FD_MEM_SOHM_TABLE,
                                              (hsize_t)table->table_size)))
    HGOTO_ERROR(H5E_SOHM, H5E_NOSPACE, FAIL,
                "file allocation failed for SOHM table");

  /* Cache the new table */
  if (H5AC_insert_entry(f, H5AC_SOHM_TABLE, table_addr, table,
  ...
  if (H5O_msg_create(ext_loc, H5O_SHMESG_ID,
                     H5O_MSG_FLAG_CONSTANT | H5O_MSG_FLAG_DONTSHARE,
                     H5O_UPDATE_TIME, &sohm_table) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTINIT, FAIL,
                "unable to update SOHM header message");
  ...
  if (ret_value < 0) {
    if (table_addr != HADDR_UNDEF)
      H5MF_xfree(f, H5FD_MEM_SOHM_TABLE, table_addr,
                 (hsize_t)table->table_size);
    if (table != NULL)
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5SM__get_index(const H5SM_master_table_t *table, unsigned type_id) {
  size_t x;
  unsigned type_flag;
...
htri_t H5SM_type_shared(H5F_t *f, unsigned type_id) {
  ...
  if (H5_addr_defined(H5F_SOHM_ADDR(f))) {
    ...
    if (NULL == (table = (H5SM_master_table_t *)H5AC_protect(
                     f, H5AC_SOHM_TABLE, H5F_SOHM_ADDR(f), &cache_udata,
                     H5AC__READ_ONLY_FLAG)))
      HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL,
                  "unable to load SOHM master table");
  } /* end if */
  else
  ...
  if (table && H5AC_unprotect(f, H5AC_SOHM_TABLE, H5F_SOHM_ADDR(f), table,
                              H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to close SOHM master table");
...
herr_t H5SM_get_fheap_addr(H5F_t *f, unsigned type_id, haddr_t *fheap_addr) {
  ...
  if ((index_num = H5SM__get_index(table, type_id)) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL,
                "unable to find correct SOHM index");
...
static herr_t H5SM__create_index(H5F_t *f, H5SM_index_header_t *header) {
  ...
  if (header->list_max > 0) {
    ...
    if (HADDR_UNDEF == (list_addr = H5SM__create_list(f, header)))
      HGOTO_ERROR(H5E_SOHM, H5E_CANTCREATE, FAIL,
                  "list creation failed for SOHM index");
  ...
  /* index is a B-tree */
    ...
    if (NULL == (bt2 = H5B2_create(f, &bt2_cparam, f)))
      HGOTO_ERROR(H5E_SOHM, H5E_CANTCREATE, FAIL,
                  "B-tree creation failed for SOHM index");

    /* Retrieve the v2 B-tree's address in the file */
    if (H5B2_get_addr(bt2, &tree_addr) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTGET, FAIL,
                  "can't get v2 B-tree address for SOHM index");
  ...
  /* Create a heap to hold the shared messages that the list or B-tree will
   * index */
  ...
  if (H5HF_get_heap_addr(fheap, &(header->heap_addr)) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTGETSIZE, FAIL,
                "can't get fractal heap address");
...
#ifndef NDEBUG
  {
    ...
    if (H5HF_get_id_len(fheap, &fheap_id_len) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTGETSIZE, FAIL,
                  "can't get fractal heap ID length");
  ...
  if (bt2 && H5B2_close(bt2) < 0)
    HDONE_ERROR(H5E_SOHM, H5E_CANTCLOSEOBJ, FAIL,
                "can't close v2 B-tree for SOHM index");
...
static herr_t H5SM__delete_index(H5F_t *f, H5SM_index_header_t *header,
  ...
  if (header->index_type == H5SM_LIST) {
    ...
    if (index_status & H5AC_ES__IN_CACHE) {
      ...
      if (H5AC_expunge_entry(f, H5AC_SOHM_LIST, header->index_addr,
                             H5AC__FREE_FILE_SPACE_FLAG) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_CANTREMOVE, FAIL,
                    "unable to remove list index from cache");
    } /* end if */
  }   /* end if */
  else {
  ...
  if (delete_heap == TRUE) {
    if (H5HF_delete(f, header->heap_addr) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTDELETE, FAIL,
                  "unable to delete fractal heap");
...
static haddr_t H5SM__create_list(H5F_t *f, H5SM_index_header_t *header) {
  ...
  if (NULL == (list = H5FL_CALLOC(H5SM_list_t)))
    HGOTO_ERROR(H5E_SOHM, H5E_NOSPACE, HADDR_UNDEF,
                "file allocation failed for SOHM list");
  if (NULL == (list->messages =
                   (H5SM_sohm_t *)H5FL_ARR_CALLOC(H5SM_sohm_t, num_entries)))
    HGOTO_ERROR(H5E_SOHM, H5E_NOSPACE, HADDR_UNDEF,
                "file allocation failed for SOHM list");
  ...
  if (HADDR_UNDEF ==
      (addr = H5MF_alloc(f, H5FD_MEM_SOHM_INDEX, (hsize_t)header->list_size)))
    HGOTO_ERROR(H5E_SOHM, H5E_NOSPACE, HADDR_UNDEF,
                "file allocation failed for SOHM list");

  /* Put the list into the cache */
  if (H5AC_insert_entry(f, H5AC_SOHM_LIST, addr, list, H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTINS, HADDR_UNDEF,
                "can't add SOHM list to cache");
...
static herr_t H5SM__convert_list_to_btree(H5F_t *f, H5SM_index_header_t *header,
  ...
  for (x = 0; x < header->list_max; x++) {
    if (list->messages[x].location != H5SM_NO_LOC) {
      ...
      if (H5SM__read_mesg(f, &(key.message), fheap, open_oh, &key.encoding_size,
                          &encoding_buf) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_CANTLOAD, FAIL,
                    "Couldn't read SOHM message in list");
  ...
  if (H5AC_unprotect(f, H5AC_SOHM_LIST, header->index_addr, list,
    ...
    HGOTO_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to release SOHM list");
  *_list = list = NULL;
   ...
   */
  num_messages =
      header->num_messages; /* preserve this across the index deletion */
...
static herr_t H5SM__convert_btree_to_list(H5F_t *f,
  ...
  if (HADDR_UNDEF == (header->index_addr = H5SM__create_list(f, header)))
    HGOTO_ERROR(H5E_SOHM, H5E_CANTINIT, FAIL,
                "unable to create shared message list");
  ...
  if (NULL ==
      (list = (H5SM_list_t *)H5AC_protect(f, H5AC_SOHM_LIST, header->index_addr,
                                          &cache_udata, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL,
                "unable to load SOHM list index");
  ...
  if (list && H5AC_unprotect(f, H5AC_SOHM_LIST, header->index_addr, list,
    ...
    HDONE_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to unprotect SOHM index");
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5SM__can_share_common(const H5F_t *f, unsigned type_id,
                                     const void *mesg) {
  ...
  if ((ret_value = H5O_msg_can_share(type_id, mesg)) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_BADTYPE, FAIL,
                "can_share callback returned error");
  if (ret_value == FALSE)
    HGOTO_DONE(FALSE);
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5SM_can_share(H5F_t *f, H5SM_master_table_t *table,
                      ssize_t *sohm_index_num, unsigned type_id,
                      const void *mesg) {
  size_t mesg_size;
  ...
  if ((tri_ret = H5SM__can_share_common(f, type_id, mesg)) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_BADTYPE, FAIL,
                "'trivial' sharing checks returned error");
  if (tri_ret == FALSE)
    HGOTO_DONE(FALSE);
  ...
  /* At this point, the message will be shared, set the index number if
   * requested. */
  if (sohm_index_num)
  ...
  if (my_table && my_table != table &&
      H5AC_unprotect(f, H5AC_SOHM_TABLE, H5F_SOHM_ADDR(f), my_table,
                     H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to close SOHM master table");
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5SM_try_share(H5F_t *f, H5O_t *open_oh, unsigned defer_flags,
  ...
  if ((tri_ret = H5SM_can_share(f, table, &index_num, type_id, mesg)) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_BADTYPE, FAIL,
                "'complex' sharing checks returned error");
  if (tri_ret == FALSE)
    HGOTO_DONE(FALSE);
  ...
  if (H5SM__write_mesg(f, open_oh, &(table->indexes[index_num]),
                       (defer_flags & H5SM_DEFER) != 0, type_id, mesg,
                       &cache_flags) < 0)
...
static herr_t H5SM__incr_ref(void *record, void *_op_data, hbool_t *changed) {
  ...
  if (message->location == H5SM_IN_OH) {
    ...
    if (H5HF_insert(op_data->key->fheap, op_data->key->encoding_size,
...
static herr_t H5SM__write_mesg(H5F_t *f, H5O_t *open_oh,
  ...
  if (NULL == (encoding_buf = H5MM_malloc(buf_size)))
    HGOTO_ERROR(H5E_SOHM, H5E_NOSPACE, FAIL,
                "can't allocate buffer for encoding");
  if (H5O_msg_encode(f, type_id, TRUE, (unsigned char *)encoding_buf, mesg) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTENCODE, FAIL,
                "can't encode message to be shared");
  ...
  if (header->index_type == H5SM_LIST) {
    ...
    if (NULL == (list = (H5SM_list_t *)H5AC_protect(
                     f, H5AC_SOHM_LIST, header->index_addr, &cache_udata,
    ...
    if (H5SM__find_in_list(list, &key, &empty_pos, &list_pos) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTINSERT, FAIL,
                  "unable to search for message in list");
    ...
    if (defer) {
      ...
      if (list_pos != SIZE_MAX) {
        ...
        if (list->messages[list_pos].location == H5SM_IN_OH) {
          /* Put the message in the heap and record its new heap ID */
          if (H5HF_insert(fheap, key.encoding_size, key.encoding,
                          &shared.u.heap_id) < 0)
            HGOTO_ERROR(H5E_SOHM, H5E_CANTINSERT, FAIL,
                        "unable to insert message into fractal heap");
  ...
  /* Index is a B-tree */
    ...
    if (defer) {
      ...
      if (H5B2_find(bt2, &key, &found, NULL, NULL) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_NOTFOUND, FAIL,
                    "can't search for message in index");
    } /* end if */
    else {
  ...
  if (found) {
    ...
    htri_t share_in_ohdr; /* Whether the new message can be shared in another
                             object's header */
    ...
    if ((share_in_ohdr = H5O_msg_can_share_in_ohdr(type_id)) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_BADTYPE, FAIL,
                  "'share in ohdr' check returned error");
    ...
    if (share_in_ohdr && open_oh) {
      ...
      if (H5O_msg_get_crt_index(type_id, mesg, &shared.u.loc.index) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_CANTGET, FAIL,
                    "unable to retrieve creation index");
    ...
    if (!defer) {
      ...
      /* Check whether the list has grown enough that it needs to become a
       * B-tree */
      if (header->index_type == H5SM_LIST &&
          header->num_messages >= header->list_max)
        if (H5SM__convert_list_to_btree(f, header, &list, fheap, open_oh) < 0)
          HGOTO_ERROR(H5E_SOHM, H5E_CANTDELETE, FAIL,
                      "unable to convert list to B-tree");
      ...
      if (header->index_type == H5SM_LIST) {
        ...
        if (empty_pos == SIZE_MAX) {
          ...
          if (pos == SIZE_MAX || empty_pos == SIZE_MAX)
            HGOTO_ERROR(H5E_SOHM, H5E_CANTINSERT, FAIL,
                        "unable to find empty entry in list");
        }
  ...
  if (H5O_msg_set_share(type_id, &shared, mesg) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_BADMESG, FAIL,
  ...
  /* If we got a list out of the cache, release it (it is always dirty after
   * writing a message) */
  if (list &&
      H5AC_unprotect(f, H5AC_SOHM_LIST, header->index_addr, list,
                     defer ? H5AC__NO_FLAGS_SET : H5AC__DIRTIED_FLAG) < 0)
    HDONE_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to close SOHM index");
...
herr_t H5SM_delete(H5F_t *f, H5O_t *open_oh, H5O_shared_t *sh_mesg) {
   ...
   */
  if (H5SM__delete_from_index(f, open_oh, &(table->indexes[index_num]), sh_mesg,
                              &cache_flags, &mesg_size, &mesg_buf) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTDELETE, FAIL,
                "unable to delete message from SOHM index");

  /* Release the master SOHM table */
  if (H5AC_unprotect(f, H5AC_SOHM_TABLE, H5F_SOHM_ADDR(f), table, cache_flags) <
      0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to close SOHM master table");
  table = NULL;
   ...
   */
  if (mesg_buf) {
    if (NULL == (native_mesg = H5O_msg_decode(f, open_oh, type_id, mesg_size,
                                              (const unsigned char *)mesg_buf)))
      HGOTO_ERROR(H5E_SOHM, H5E_CANTDECODE, FAIL,
  ...
  for (x = 0; x < list->header->list_max; x++) {
    if (list->messages[x].location != H5SM_NO_LOC) {
      ...
      if (H5SM__message_compare(key, &(list->messages[x]), &cmp) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_CANTCOMPARE, FAIL,
                    "can't compare message records");
...
static herr_t H5SM__delete_from_index(H5F_t *f, H5O_t *open_oh,
  ...
  /* Get the encoded message */
  if (H5SM__read_mesg(f, &key.message, fheap, open_oh, &buf_size,
                      &encoding_buf) < 0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTOPENOBJ, FAIL, "unable to open fractal heap");
  ...
  if (header->index_type == H5SM_LIST) {
    H5SM_list_cache_ud_t
        cache_udata; /* User-data for metadata cache callback */
   ...
   */
  if (message_ptr->location == H5SM_IN_OH ||
      message_ptr->u.heap_loc.ref_count == 0) {
    /* Save the location */
    ...
    *cache_flags |= H5AC__DIRTIED_FLAG;
    ...
    if (old_loc == H5SM_IN_HEAP)
      if (H5HF_remove(fheap, &(message_ptr->u.heap_loc.fheap_id)) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_CANTREMOVE, FAIL,
                    "unable to remove message from heap");
    ...
    *mesg_size = buf_size;
    ...
    if (header->num_messages == 0) {
      ...
      if (H5HF_close(fheap) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_CANTCLOSEOBJ, FAIL,
                    "can't close fractal heap");
      fheap = NULL;
    ...
    else if (header->index_type == H5SM_BTREE &&
             header->num_messages < header->btree_min) {
      ...
      if (H5SM__convert_btree_to_list(f, header) < 0)
        HGOTO_ERROR(H5E_SOHM, H5E_CANTINIT, FAIL,
                    "unable to convert btree to list");
    } /* end if */
  }   /* end if */
...
herr_t H5SM_get_info(const H5O_loc_t *ext_loc, H5P_genplist_t *fc_plist) {
  ...
  H5AC_ring_t orig_ring = H5AC_RING_INV; /* Original ring value */
  unsigned
      tmp_sohm_nindexes; /* Number of shared messages indexes in the table */
  ...
  if (status) {
    H5SM_table_cache_ud_t cache_udata; /* User-data for callback */
    unsigned
        index_flags[H5O_SHMESG_MAX_NINDEXES]; /* Message flags for each index */
    unsigned minsizes[H5O_SHMESG_MAX_NINDEXES]; /* Minimum message size for each
                                                   index */
    ...
    if (NULL == H5O_msg_read(ext_loc, H5O_SHMESG_ID, &sohm_table))
      HGOTO_ERROR(H5E_SOHM, H5E_CANTGET, FAIL,
                  "shared message info message not present");
    ...
    assert(H5_addr_defined(H5F_SOHM_ADDR(f)));
    assert(H5F_SOHM_NINDEXES(f) > 0 &&
           H5F_SOHM_NINDEXES(f) <= H5O_SHMESG_MAX_NINDEXES);
    ...
    if (H5P_set(fc_plist, H5F_CRT_SHMSG_NINDEXES_NAME, &tmp_sohm_nindexes) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTSET, FAIL,
                  "can't set number of SOHM indexes");
    if (H5P_set(fc_plist, H5F_CRT_SHMSG_INDEX_TYPES_NAME, index_flags) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTSET, FAIL,
                  "can't set type flags for indexes");
    ...
    if (H5P_set(fc_plist, H5F_CRT_SHMSG_LIST_MAX_NAME, &sohm_l2b) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTGET, FAIL,
                  "can't set SOHM cutoff in property list");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5SM_reconstitute(H5O_shared_t *sh_mesg, H5F_t *f, unsigned msg_type_id,
...
static herr_t H5SM__get_refcount_bt2_cb(const void *_record, void *_op_data) {
  const H5SM_sohm_t *record =
      (const H5SM_sohm_t *)_record; /* v2 B-tree record for message */
  H5SM_sohm_t *op_data =
      (H5SM_sohm_t *)_op_data; /* "op data" from v2 B-tree find */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5SM_get_refcount(H5F_t *f, unsigned type_id,
  ...
  H5SM_list_t *list =
      NULL; /* SOHM index list for message type (if in list form) */
  ...
  /* Look up the master SOHM table */
  if (NULL == (table = (H5SM_master_table_t *)H5AC_protect(
                   f, H5AC_SOHM_TABLE, H5F_SOHM_ADDR(f), &tbl_cache_udata,
                   H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL,
                "unable to load SOHM master table");
  ...
  key.message.u.heap_loc.ref_count =
      0; /* Ref count isn't needed to find message */

  /* Get the encoded message */
  if (H5SM__read_mesg(f, &key.message, fheap, NULL, &buf_size, &encoding_buf) <
      0)
    HGOTO_ERROR(H5E_SOHM, H5E_CANTOPENOBJ, FAIL, "unable to open fractal heap");
  ...
  if (header->index_type == H5SM_LIST) {
    H5SM_list_cache_ud_t
        lst_cache_udata; /* User-data for metadata cache callback */
    size_t list_pos;     /* Position of the message in the list */
    ...
    /* If the index is stored as a list, get it from the cache */
    if (NULL == (list = (H5SM_list_t *)H5AC_protect(
                     f, H5AC_SOHM_LIST, header->index_addr, &lst_cache_udata,
                     H5AC__READ_ONLY_FLAG)))
      HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL, "unable to load SOHM index");
    ...
    msg_exists = FALSE;
    if (H5B2_find(bt2, &key, &msg_exists, H5SM__get_refcount_bt2_cb, &message) <
        0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTGET, FAIL,
                  "error finding message in index");
    if (!msg_exists)
      HGOTO_ERROR(H5E_SOHM, H5E_NOTFOUND, FAIL, "message not in index");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5SM__read_iter_op(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                 unsigned sequence,
  ...
  if (sequence == udata->idx) {
    ...
    if (NULL == (udata->encoding_buf = H5MM_malloc(udata->buf_size)))
      HGOTO_ERROR(H5E_SOHM, H5E_NOSPACE, H5_ITER_ERROR,
...
static herr_t H5SM__read_mesg(H5F_t *f, const H5SM_sohm_t *mesg, H5HF_t *fheap,
  ...
  if (mesg->location == H5SM_IN_OH) {
    ...
    type = H5O_msg_class_g[mesg->msg_type_id]; /* map the type ID to the actual
                                                  type object */
    assert(type);

    /* Reset object location for operation */
    if (H5O_loc_reset(&oloc) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTRESET, FAIL,
                  "unable to initialize location");

    if (NULL == open_oh ||
        mesg->u.mesg_loc.oh_addr != H5O_OH_GET_ADDR(open_oh)) {
    ...
    if ((ret_value = H5O__msg_iterate_real(f, oh, type, &op, &udata)) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_BADITER, FAIL,
                  "unable to iterate over object header messages");
  } /* end if */
  else {
    assert(mesg->location == H5SM_IN_HEAP);

    /* Copy the message from the heap */
    if (H5HF_op(fheap, &(mesg->u.heap_loc.fheap_id), H5SM__read_mesg_fh_cb,
                &udata) < 0)
      HGOTO_ERROR(H5E_SOHM, H5E_CANTLOAD, FAIL,
                  "can't read message from fractal heap.");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5SM_table_debug(H5F_t *f, haddr_t table_addr, FILE *stream, int indent,
  ...
  /* Look up the master SOHM table */
  if (NULL ==
      (table = (H5SM_master_table_t *)H5AC_protect(
           f, H5AC_SOHM_TABLE, table_addr, &cache_udata, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL,
                "unable to load SOHM master table");
  ...
  for (x = 0; x < num_indexes; ++x) {
    ...
    fprintf(stream, "%*s%-*s %s\n", indent + 3, "", fwidth, "SOHM Index Type:",
            (table->indexes[x].index_type == H5SM_LIST
                 ? "List"
                 : (table->indexes[x].index_type == H5SM_BTREE ? "B-Tree"
                                                               : "Unknown")));
  ...
  if (table && H5AC_unprotect(f, H5AC_SOHM_TABLE, table_addr, table,
                              H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to close SOHM master table");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5SM_list_debug(H5F_t *f, haddr_t list_addr, FILE *stream, int indent,
                       int fwidth, haddr_t table_addr) {
  H5SM_master_table_t *table = NULL; /* SOHM master table */
  H5SM_list_t *list =
      NULL; /* SOHM index list for message type (if in list form) */
  H5SM_list_cache_ud_t
      lst_cache_udata; /* List user-data for metadata cache callback */
  H5SM_table_cache_ud_t
      tbl_cache_udata;        /* Table user-data for metadata cache callback */
  ...
  /* Look up the master SOHM table */
  if (NULL == (table = (H5SM_master_table_t *)H5AC_protect(
                   f, H5AC_SOHM_TABLE, table_addr, &tbl_cache_udata,
                   H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL,
                "unable to load SOHM master table");
  ...
  if (NULL == (list = (H5SM_list_t *)H5AC_protect(f, H5AC_SOHM_LIST, list_addr,
                                                  &lst_cache_udata,
                                                  H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_SOHM, H5E_CANTPROTECT, FAIL, "unable to load SOHM index");
  ...
  if (list && H5AC_unprotect(f, H5AC_SOHM_LIST, list_addr, list,
                             H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_SOHM, H5E_CANTUNPROTECT, FAIL,
                "unable to close SOHM index");
  if (table && H5AC_unprotect(f, H5AC_SOHM_TABLE, table_addr, table,
 ...
 *			   (include btree storage used by huge objects in fractal
 *heap)
    ...
    if (table->indexes[u].index_type == H5SM_BTREE) {
      if (H5_addr_defined(table->indexes[u].index_addr)) {
        ...
        if (H5B2_size(bt2, &(ih_info->index_size)) < 0)
          HGOTO_ERROR(H5E_SOHM, H5E_CANTGET, FAIL,
                      "can't retrieve B-tree storage info");
