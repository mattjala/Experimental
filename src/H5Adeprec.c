...
used) RETURNS Non-negative on success/H5I_INVALID_HID on failure
...
--------------------------------------------------------------------------*/
hid_t H5Acreate1(hid_t loc_id, const char *name, hid_t type_id, hid_t space_id,
                 hid_t acpl_id) {
  ...
  if (NULL ==
      (attr = H5VL_attr_create(vol_obj, &loc_params, name, type_id, space_id,
                               acpl_id, H5P_ATTRIBUTE_ACCESS_DEFAULT,
...
hid_t H5Aopen_name(hid_t loc_id, const char *name) {
  ...
  if (NULL == (attr = H5VL_attr_open(
                   vol_obj, &loc_params, name, H5P_ATTRIBUTE_ACCESS_DEFAULT,
                   H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)))
...
hid_t H5Aopen_idx(hid_t loc_id, unsigned idx) {
  ...
  /* Open the attribute */
  if (NULL == (attr = H5VL_attr_open(
                   vol_obj, &loc_params, NULL, H5P_ATTRIBUTE_ACCESS_DEFAULT,
                   H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open attribute");
...
int H5Aget_num_attrs(hid_t loc_id) {
  ...
  if (H5VL_object_get(vol_obj, &loc_params, &vol_cb_args,
    ...
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, (-1),
                "unable to get attribute count for object");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5Aiterate1(loc_id, attr_num, op, data)
        ...
        unsigned *attr_num; IN/OUT: Starting (IN) & Ending (OUT) attribute
number H5A_operator1_t op;  IN: User's function to pass each attribute to void
*op_data;      IN/OUT: User's data to pass through to iterator operator function
 RETURNS
...
--------------------------------------------------------------------------*/
herr_t H5Aiterate1(hid_t loc_id, unsigned *attr_num /*in,out*/,
                   H5A_operator1_t op, void *op_data) {
  H5VL_object_t *vol_obj = NULL;    /* Object of loc_id */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_attr_optional_args_t
      attr_opt_args; /* Arguments for optional operation */
  ...
  /* Get the location object */
  if (NULL == (vol_obj = H5VL_vol_object(loc_id)))
    HGOTO_ERROR(H5E_ATTR, H5E_BADTYPE, H5_ITER_ERROR,
                "invalid location identifier");
  ...
  if ((ret_value =
           H5VL_attr_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                              H5_REQUEST_NULL)) < 0)
