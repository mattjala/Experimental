...
static herr_t H5FA__cache_hdr_get_initial_load_size(void *udata,
                                                    size_t *image_len);
static htri_t H5FA__cache_hdr_verify_chksum(const void *image_ptr, size_t len,
                                            void *udata_ptr);
...
static herr_t H5FA__cache_dblock_get_initial_load_size(void *udata,
                                                       size_t *image_len);
static htri_t H5FA__cache_dblock_verify_chksum(const void *image_ptr,
...
static herr_t H5FA__cache_dblock_image_len(const void *thing,
                                           size_t *image_len);
static herr_t H5FA__cache_dblock_serialize(const H5F_t *f, void *image,
                                           size_t len, void *thing);
static herr_t H5FA__cache_dblock_notify(H5AC_notify_action_t action,
                                        void *thing);
...
static herr_t H5FA__cache_dblk_page_get_initial_load_size(void *udata,
                                                          size_t *image_len);
static htri_t H5FA__cache_dblk_page_verify_chksum(const void *image_ptr,
                                                  size_t len, void *udata_ptr);
...
static herr_t H5FA__cache_dblk_page_image_len(const void *thing,
                                              size_t *image_len);
static herr_t H5FA__cache_dblk_page_serialize(const H5F_t *f, void *image,
                                              size_t len, void *thing);
static herr_t H5FA__cache_dblk_page_notify(H5AC_notify_action_t action,
                                           void *thing);
...
const H5AC_class_t H5AC_FARRAY_DBLOCK[1] = {{
    ...
    H5AC__CLASS_NO_FLAGS_SET, /* Client class behavior flags */
    H5FA__cache_dblock_get_initial_load_size, /* 'get_initial_load_size'
                                                 callback */
    NULL,                             /* 'get_final_load_size' callback */
...
const H5AC_class_t H5AC_FARRAY_DBLK_PAGE[1] = {{
    ...
    H5AC__CLASS_NO_FLAGS_SET,      /* Client class behavior flags */
    H5FA__cache_dblk_page_get_initial_load_size, /* 'get_initial_load_size'
                                                    callback */
    NULL,                                /* 'get_final_load_size' callback */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_hdr_get_initial_load_size(void *_udata,
                                                    size_t *image_len) {
  H5FA_hdr_cache_ud_t *udata =
      (H5FA_hdr_cache_ud_t *)_udata; /* User data for callback */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5FA__cache_hdr_verify_chksum(const void *_image, size_t len,
 ...
 *-------------------------------------------------------------------------
 */
static void *H5FA__cache_hdr_deserialize(const void *_image,
  ...
  if (memcmp(image, H5FA_HDR_MAGIC, (size_t)H5_SIZEOF_MAGIC) != 0)
    HGOTO_ERROR(H5E_FARRAY, H5E_BADVALUE, NULL,
                "wrong fixed array header signature");
  image += H5_SIZEOF_MAGIC;

  /* Version */
  if (*image++ != H5FA_HDR_VERSION)
    HGOTO_ERROR(H5E_FARRAY, H5E_VERSION, NULL,
                "wrong fixed array header version");
  ...
  if (H5_addr_defined(hdr->dblk_addr)) {
    ...
    if (hdr->cparam.nelmts > dblk_page_nelmts) {
      dblock.npages = (size_t)(((hdr->cparam.nelmts + dblk_page_nelmts) - 1) /
                               dblk_page_nelmts);
  ...
  if (H5FA__hdr_init(hdr, udata->ctx_udata) < 0)
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTINIT, NULL,
                "initialization failed for fixed array header");
  assert(hdr->size == len);
  ...
  if (!ret_value)
    if (hdr && H5FA__hdr_dest(hdr) < 0)
      HDONE_ERROR(H5E_FARRAY, H5E_CANTFREE, NULL,
                  "unable to destroy fixed array header");
...
static herr_t H5FA__cache_hdr_image_len(const void *_thing, size_t *image_len) {
  const H5FA_hdr_t *hdr =
      (const H5FA_hdr_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_hdr_serialize(const H5F_t *f, void *_image,
                                        size_t H5_ATTR_UNUSED len,
                                        void *_thing) {
  H5FA_hdr_t *hdr =
      (H5FA_hdr_t *)_thing;           /* Pointer to the fixed array header */
  ...
  *image++ =
  ...
  /* Array statistics */
  H5F_ENCODE_LENGTH(
      f, image, hdr->stats.nelmts); /* Number of elements for the fixed array */

  /* Internal information */
  H5F_addr_encode(f, &image,
                  hdr->dblk_addr); /* Address of fixed array data block */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_hdr_notify(H5AC_notify_action_t action,
                                     void *_thing) {
  ...
  if (hdr->swmr_write) {
      ...
      if (hdr->parent) {
        ...
        if (H5AC_proxy_entry_remove_child((H5AC_proxy_entry_t *)hdr->parent,
          ...
          HGOTO_ERROR(H5E_FARRAY, H5E_CANTUNDEPEND, FAIL,
                      "unable to destroy flush dependency between fixed array "
                      "and proxy");
        hdr->parent = NULL;
      } /* end if */
    ...
    default:
#ifdef NDEBUG
      HGOTO_ERROR(H5E_FARRAY, H5E_BADVALUE, FAIL,
...
static herr_t H5FA__cache_hdr_free_icr(void *thing) {
  ...
  if (H5FA__hdr_dest((H5FA_hdr_t *)thing) < 0)
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTFREE, FAIL,
                "can't free fixed array header");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_dblock_get_initial_load_size(void *_udata,
                                                       size_t *image_len) {
  H5FA_dblock_cache_ud_t *udata =
      (H5FA_dblock_cache_ud_t *)_udata; /* User data */
  H5FA_dblock_t dblock;                 /* Fake data block for computing size */
  size_t dblk_page_nelmts;              /* # of elements per data block page */
  ...
  if (udata->hdr->cparam.nelmts > dblk_page_nelmts) {
    dblock.npages =
        (size_t)(((udata->hdr->cparam.nelmts + dblk_page_nelmts) - 1) /
                 dblk_page_nelmts);
    dblock.dblk_page_init_size = (dblock.npages + 7) / 8;
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5FA__cache_dblock_verify_chksum(const void *_image, size_t len,
                                               void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5FA__cache_dblock_deserialize(const void *_image,
                                            size_t H5_ATTR_NDEBUG_UNUSED len,
                                            void *_udata,
                                            hbool_t H5_ATTR_UNUSED *dirty) {
  H5FA_dblock_t *dblock = NULL; /* Data block info */
  H5FA_dblock_cache_ud_t *udata =
      (H5FA_dblock_cache_ud_t *)_udata; /* User data for loading data block */
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
  ...
  if (memcmp(image, H5FA_DBLOCK_MAGIC, (size_t)H5_SIZEOF_MAGIC) != 0)
    HGOTO_ERROR(H5E_FARRAY, H5E_BADVALUE, NULL,
                "wrong fixed array data block signature");
  image += H5_SIZEOF_MAGIC;

  /* Version */
  if (*image++ != H5FA_DBLOCK_VERSION)
    HGOTO_ERROR(H5E_FARRAY, H5E_VERSION, NULL,
                "wrong fixed array data block version");
  ...
  if (H5_addr_ne(arr_addr, udata->hdr->addr))
    HGOTO_ERROR(H5E_FARRAY, H5E_BADVALUE, NULL,
                "wrong fixed array header address");
  ...
  if (!dblock->npages) {
    ...
    if ((udata->hdr->cparam.cls->decode)(image, dblock->elmts,
                                         (size_t)udata->hdr->cparam.nelmts,
                                         udata->hdr->cb_ctx) < 0)
      HGOTO_ERROR(H5E_FARRAY, H5E_CANTDECODE, NULL,
                  "can't decode fixed array data elements");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_dblock_image_len(const void *_thing,
                                           size_t *image_len) {
  const H5FA_dblock_t *dblock =
      (const H5FA_dblock_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_dblock_serialize(const H5F_t *f, void *_image,
                                           size_t H5_ATTR_UNUSED len,
                                           void *_thing) {
  H5FA_dblock_t *dblock =
      (H5FA_dblock_t *)_thing;        /* Pointer to the object to serialize */
  uint8_t *image = (uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t metadata_chksum;           /* Computed metadata checksum value */
  herr_t ret_value = SUCCEED;
  ...
  *image++ = (uint8_t)dblock->hdr->cparam.cls->id;
  ...
  if (!dblock->npages) {
    ...
    H5_CHECK_OVERFLOW(dblock->hdr->cparam.nelmts, /* From: */ hsize_t,
                      /* To: */ size_t);
    if ((dblock->hdr->cparam.cls->encode)(image, dblock->elmts,
                                          (size_t)dblock->hdr->cparam.nelmts,
                                          dblock->hdr->cb_ctx) < 0)
      HGOTO_ERROR(H5E_FARRAY, H5E_CANTENCODE, FAIL,
                  "can't encode fixed array data elements");
...
static herr_t H5FA__cache_dblock_notify(H5AC_notify_action_t action,
  ...
  if (dblock->hdr->swmr_write) {
    ...
    case H5AC_NOTIFY_ACTION_AFTER_LOAD:
      ...
      if (H5FA__create_flush_depend((H5AC_info_t *)dblock->hdr,
                                    (H5AC_info_t *)dblock) < 0)
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTDEPEND, FAIL,
    ...
    case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
      ...
      if (H5FA__destroy_flush_depend((H5AC_info_t *)dblock->hdr,
                                     (H5AC_info_t *)dblock) < 0)
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTUNDEPEND, FAIL,
...
static herr_t H5FA__cache_dblock_free_icr(void *_thing) {
  ...
  if (H5FA__dblock_dest(dblock) < 0)
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTFREE, FAIL,
                "can't free fixed array data block");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_dblock_fsf_size(const void *_thing,
                                          hsize_t *fsf_size) {
  const H5FA_dblock_t *dblock =
      (const H5FA_dblock_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_dblk_page_get_initial_load_size(void *_udata,
                                                          size_t *image_len) {
  H5FA_dblk_page_cache_ud_t *udata =
      (H5FA_dblk_page_cache_ud_t *)_udata; /* User data */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5FA__cache_dblk_page_verify_chksum(const void *_image,
                                                  size_t len,
                                                  void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5FA__cache_dblk_page_deserialize(const void *_image, size_t len,
                                               void *_udata,
                                               hbool_t H5_ATTR_UNUSED *dirty) {
  H5FA_dblk_page_t *dblk_page = NULL; /* Data block page info */
  H5FA_dblk_page_cache_ud_t *udata = (H5FA_dblk_page_cache_ud_t *)
  ...
  /* Convert from raw elements on disk into native elements in memory */
  if ((udata->hdr->cparam.cls->decode)(image, dblk_page->elmts, udata->nelmts,
                                       udata->hdr->cb_ctx) < 0)
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTDECODE, NULL,
                "can't decode fixed array data elements");
  ...
  if (!ret_value)
    if (dblk_page && H5FA__dblk_page_dest(dblk_page) < 0)
      HDONE_ERROR(H5E_FARRAY, H5E_CANTFREE, NULL,
                  "unable to destroy fixed array data block page");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FA__cache_dblk_page_image_len(const void *_thing,
                                              size_t *image_len) {
  const H5FA_dblk_page_t *dblk_page =
      (const H5FA_dblk_page_t *)_thing; /* Pointer to the object */
 ...
 */
static herr_t
H5FA__cache_dblk_page_serialize(const H5F_t H5_ATTR_NDEBUG_UNUSED *f,
                                void *_image, size_t H5_ATTR_UNUSED len,
                                void *_thing) {
  H5FA_dblk_page_t *dblk_page =
      (H5FA_dblk_page_t *)_thing;     /* Pointer to the object to serialize */
  uint8_t *image = (uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t metadata_chksum;           /* Computed metadata checksum value */
  herr_t ret_value = SUCCEED;
...
static herr_t H5FA__cache_dblk_page_notify(H5AC_notify_action_t action,
                                           void *_thing) {
  H5FA_dblk_page_t *dblk_page =
      (H5FA_dblk_page_t *)_thing; /* Pointer to the object */
  herr_t ret_value = SUCCEED;
...
static herr_t H5FA__cache_dblk_page_free_icr(void *thing) {
  ...
  if (H5FA__dblk_page_dest((H5FA_dblk_page_t *)thing) < 0)
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTFREE, FAIL,
                "can't free fixed array data block page");
