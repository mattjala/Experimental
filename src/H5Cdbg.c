...
herr_t H5C_dump_cache(H5C_t *cache_ptr, const char *cache_name) {
  ...
  while (entry_ptr != NULL) {
    ...
    fprintf(stdout, "  %2d %-32s ", (int)(entry_ptr->type->id),
            (entry_ptr->type->name));
  ...
  if (cache_ptr->slist_len > 0) {
    ...
    while (entry_ptr != NULL) {
      fprintf(stdout, "%s%d       0x%016llx  %4lld    %d/%d       %d    %s\n",
              cache_ptr->prefix, i, (long long)(entry_ptr->addr),
              (long long)(entry_ptr->size), (int)(entry_ptr->is_protected),
              (int)(entry_ptr->is_pinned), (int)(entry_ptr->is_dirty),
              entry_ptr->type->name);
...
herr_t H5C_stats(H5C_t *cache_ptr, const char *cache_name,
  ...
  fprintf(stdout,
          ...
          cache_ptr->prefix, (long)(cache_ptr->slist_size),
  ...
  fprintf(stdout, "%s  Total write / read (max) protects  = %ld / %ld (%ld)\n",
          cache_ptr->prefix, (long)total_write_protects,
          (long)total_read_protects, (long)max_read_protects);
  ...
  fprintf(stdout, "%s  MSIC: Average/max entries skipped  = %lf / %ld\n",
          cache_ptr->prefix, (double)average_entries_skipped_per_calls_to_msic,
  ...
  fprintf(stdout, "%s  MSIC: Average/max entries scanned  = %lf / %ld\n",
          cache_ptr->prefix, (double)average_entries_scanned_per_calls_to_msic,
  ...
  fprintf(stdout, "%s  slist/LRU/index scan restarts   = %lld / %lld / %lld.\n",
          cache_ptr->prefix, (long long)(cache_ptr->slist_scan_restarts),
  ...
  fprintf(
      ...
      "\n",
      cache_ptr->prefix, cache_ptr->images_created, cache_ptr->images_read,
      cache_ptr->images_loaded, cache_ptr->last_image_size);

  fprintf(stdout, "%s  prefetches / dirty prefetches      = %lld / %lld\n",
          cache_ptr->prefix, (long long)(cache_ptr->prefetches),
  ...
  if (cache_ptr->prefetches > 0)
    prefetch_use_rate = 100.0 * ((double)(cache_ptr->prefetch_hits)) /
  ...
  fprintf(stdout, "%s  aggregate max / min accesses       = %d / %d\n",
          cache_ptr->prefix, (int)aggregate_max_accesses,
          (int)aggregate_min_accesses);

  fprintf(stdout, "%s  aggregate max_clears / max_flushes = %d / %d\n",
          cache_ptr->prefix, (int)aggregate_max_clears,
  ...
  if (display_detailed_stats) {
    for (i = 0; i <= cache_ptr->max_type_id; i++) {
      ...
      fprintf(stdout, "%s    hits / misses / hit_rate       = %ld / %ld / %f\n",
              cache_ptr->prefix, (long)(cache_ptr->hits[i]),
              (long)(cache_ptr->misses[i]), hit_rate);
 ...
 */
#ifndef NDEBUG
herr_t H5C_flush_dependency_exists(H5C_t *cache_ptr, haddr_t parent_addr,
  ...
  if (parent_ptr && child_ptr) {
    if (child_ptr->flush_dep_nparents > 0) {
      ...
      assert(child_ptr->flush_dep_parent_nalloc >=
...
herr_t H5C_validate_index_list(H5C_t *cache_ptr) {
  ...
  if (((cache_ptr->il_head == NULL) || (cache_ptr->il_tail == NULL)) &&
    ...
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "Index list pointer validation failed");
  ...
  while (entry_ptr != NULL) {
    if ((entry_ptr != cache_ptr->il_head) &&
      ...
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                  "Index list pointers for entry are invalid");
  ...
  if ((cache_ptr->index_len != len) || (cache_ptr->il_len != len) ||
    ...
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "Index, clean and dirty sizes for cache are invalid");
 ...
 */
#ifndef NDEBUG
herr_t H5C_get_entry_ptr_from_addr(H5C_t *cache_ptr, haddr_t addr,
                                   void **entry_ptr_ptr) {
  H5C_cache_entry_t *entry_ptr = NULL;
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 */
#ifndef NDEBUG
herr_t H5C_verify_entry_type(H5C_t *cache_ptr, haddr_t addr,
...
void H5C_def_auto_resize_rpt_fcn(H5C_t *cache_ptr,
  ...
  case increase:
    ...
    fprintf(stdout, "%scache size increased from (%zu/%zu) to (%zu/%zu).\n",
            ...
            new_max_cache_size, new_min_clean_size);
    break;
  ...
  case flash_increase:
    ...
    fprintf(stdout, "%sflash cache resize(%d) -- size threshold = %zu.\n",
            cache_ptr->prefix, (int)(cache_ptr->resize_ctl.flash_incr_mode),
  ...
  case decrease:
    ...
    case H5C_decr__age_out_with_threshold:
      ...
      fprintf(stdout,
              "%sAuto cache resize -- decrease by ageout with threshold. HR = "
              "%lf > %6.5lf\n",
              cache_ptr->prefix, hit_rate,
              cache_ptr->resize_ctl.upper_hr_threshold);
      break;
  ...
  case increase_disabled:
    fprintf(stdout, "%sAuto cache resize -- increase disabled -- HR = %lf.",
            cache_ptr->prefix, hit_rate);
    break;
...
herr_t H5C__validate_lru_list(H5C_t *cache_ptr) {
  ...
  while (entry_ptr != NULL) {
    if ((entry_ptr != cache_ptr->LRU_head_ptr) &&
      ...
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                  "entry has bad prev/next pointers");
    ...
    if (entry_ptr->is_pinned || entry_ptr->pinned_from_client ||
        entry_ptr->pinned_from_cache)
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                  "invalid entry 'pin origin' fields");
  ...
  if ((cache_ptr->LRU_list_len != (uint32_t)len) ||
      (cache_ptr->LRU_list_size != size))
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "LRU list length/size check failed");
...
herr_t H5C__validate_pinned_entry_list(H5C_t *cache_ptr) {
  ...
  if (((cache_ptr->pel_head_ptr == NULL) ||
    ...
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "pinned list head/tail check failed");
  ...
  if ((cache_ptr->pel_len >= 1) && ((cache_ptr->pel_head_ptr == NULL) ||
  ...
  while (entry_ptr != NULL) {
    ...
    if (!entry_ptr->is_pinned)
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                  "pinned list contains unpinned entry");
  ...
  if ((cache_ptr->pel_len != (uint32_t)len) || (cache_ptr->pel_size != size))
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "pinned list length/size check failed");
...
herr_t H5C__validate_protected_entry_list(H5C_t *cache_ptr) {
  ...
  if (((cache_ptr->pl_head_ptr == NULL) || (cache_ptr->pl_tail_ptr == NULL)) &&
    ...
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "protected list head/tail check failed");
  ...
  if ((cache_ptr->pl_len == 1) &&
    ...
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "protected list sanity check failed");

  if ((cache_ptr->pl_len >= 1) && ((cache_ptr->pl_head_ptr == NULL) ||
  ...
  while (entry_ptr != NULL) {
    ...
    if (!entry_ptr->is_protected)
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                  "protected list contains unprotected entry");

    if (entry_ptr->is_read_only && (entry_ptr->ro_ref_count <= 0))
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                  "read-only entry has non-positive ref count");
  ...
  if ((cache_ptr->pl_len != (uint32_t)len) || (cache_ptr->pl_size != size))
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "protected list length/size check failed");
...
hbool_t H5C__entry_in_skip_list(H5C_t *cache_ptr,
...
#endif /* H5C_COLLECT_CACHE_STATS */
{
  ...
  fprintf(stdout,
          ...
          hit_rate, (long long)(cache_ptr->prefetches),
          (long long)(cache_ptr->dirty_prefetches),
