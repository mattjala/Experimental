...
#define H5D_FARRAY_CREATE_PARAM_SIZE                                           \
  1 /* Size of the creation parameters in bytes */
#define H5D_FARRAY_MAX_DBLK_PAGE_NELMTS_BITS                                   \
  10 /* i.e. 1024 elements per data block page */

/* Extensible array creation values */
#define H5D_EARRAY_CREATE_PARAM_SIZE                                           \
  5 /* Size of the creation parameters in bytes */
...
#define H5D_BT2_CREATE_PARAM_SIZE                                              \
  6 /* Size of the creation parameters in bytes */
...
/* Macro to check if in-place type conversion will be used for a piece and add
 * it to the global type conversion size if it won't be used */
  ...
  {                                                                            \
    ...
    if ((IO_INFO)->may_use_in_place_tconv) {                                   \
      ...
      size_t file_type_size = ((IO_INFO)->op_type == H5D_IO_OP_READ)           \
                                  ...
                                  : (DINFO)->type_info.dst_type_size;          \
                                                                               \
      /* Make sure the memory type is not smaller than the file type,          \
       * otherwise the memory buffer                                           \
      ...
      if (mem_type_size >= file_type_size) {                                   \
        ...
        if (H5S_select_contig_block((PIECE_INFO)->mspace, &is_contig,          \
                                    &sel_off, NULL) < 0)                       \
          HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,                         \
                      "can't check if dataspace is contiguous");               \
                                                                               \
        /* If the first sequence includes all the elements selected in this    \
         * piece, it it contiguous                                             \
         */                                                                    \
        if (is_contig) {                                                       \
          ...
          (PIECE_INFO)->buf_off = (size_t)sel_off * mem_type_size;             \
                                                                               ...
                                                                               \
    /* If we're not using in-place type conversion, add this piece to global   \
     * type conversion buffer size.  This will only be used if we must                                      \
     * allocate a type conversion buffer for the entire I/O. */                \
    if (!(PIECE_INFO)->in_place_tconv)                                         \
      (IO_INFO)->tconv_buf_size +=                                             \
          (PIECE_INFO)->piece_points * MAX((DINFO)->type_info.src_type_size,   \
                                           (DINFO)->type_info.dst_type_size);  \
  }
...
typedef struct H5D_type_info_t {
  ...
  const H5T_subset_info_t *cmpd_subset; /* Info related to the compound subset
...
typedef herr_t (*H5D_layout_init_func_t)(H5F_t *f, const H5D_t *dset,
                                         hid_t dapl_id);
typedef hbool_t (*H5D_layout_is_space_alloc_func_t)(
    const H5O_storage_t *storage);
typedef hbool_t (*H5D_layout_is_data_cached_func_t)(
...
typedef ssize_t (*H5D_layout_readvv_func_t)(
    const struct H5D_io_info_t *io_info,
    const struct H5D_dset_io_info_t *dset_info, size_t dset_max_nseq,
    size_t *dset_curr_seq, size_t dset_len_arr[], hsize_t dset_offset_arr[],
    size_t mem_max_nseq, size_t *mem_curr_seq, size_t mem_len_arr[],
    hsize_t mem_offset_arr[]);
typedef ssize_t (*H5D_layout_writevv_func_t)(
    const struct H5D_io_info_t *io_info,
    const struct H5D_dset_io_info_t *dset_info, size_t dset_max_nseq,
    size_t *dset_curr_seq, size_t dset_len_arr[], hsize_t dset_offset_arr[],
    size_t mem_max_nseq, size_t *mem_curr_seq, size_t mem_len_arr[],
...
typedef herr_t (*H5D_layout_io_term_func_t)(struct H5D_io_info_t *io_info,
                                            struct H5D_dset_io_info_t *di);
typedef herr_t (*H5D_layout_dest_func_t)(H5D_t *dataset);

/* Typedef for grouping layout I/O routines */
typedef struct H5D_layout_ops_t {
  H5D_layout_construct_func_t
      construct;               /* Layout constructor for new datasets */
  ...
  H5D_layout_is_data_cached_func_t
      is_data_cached; /* Query routine to determine if any raw data is cached.
                         If routine is not present then the layout type never
                         caches raw data. */
  H5D_layout_io_init_func_t io_init; /* I/O initialization routine */
  H5D_layout_mdio_init_func_t
      mdio_init; /* Multi Dataset I/O initialization routine - called after all
  ...
  H5D_layout_writevv_func_t
      writevv; /* Low-level I/O routine for writing data */
  H5D_layout_flush_func_t
      flush; /* Low-level I/O routine for flushing raw data */
...
typedef herr_t (*H5D_io_single_read_func_t)(
    const struct H5D_io_info_t *io_info,
    const struct H5D_dset_io_info_t *dset_info);
typedef herr_t (*H5D_io_single_write_func_t)(
    const struct H5D_io_info_t *io_info,
    const struct H5D_dset_io_info_t *dset_info);

typedef herr_t (*H5D_io_single_read_md_func_t)(
    const struct H5D_io_info_t *io_info, hsize_t nelmts, H5S_t *file_space,
    H5S_t *mem_space);
typedef herr_t (*H5D_io_single_write_md_func_t)(
    const struct H5D_io_info_t *io_info, hsize_t nelmts, H5S_t *file_space,
    H5S_t *mem_space);
...
typedef struct H5D_io_ops_t {
  H5D_layout_read_func_t
      multi_read; /* High-level I/O routine for reading data */
  H5D_layout_write_func_t
      multi_write; /* High-level I/O routine for writing data */
  H5D_io_single_read_func_t
      single_read; /* I/O routine for reading single block */
  H5D_io_single_write_func_t
      single_write; /* I/O routine for writing single block */
} H5D_io_ops_t;

/* Typedef for raw data I/O framework info (multi-dataset I/O) */
typedef struct H5D_md_io_ops_t {
  H5D_layout_read_md_func_t multi_read_md; /* High-level I/O routine for reading
                                              data for multi-dset */
  H5D_layout_write_md_func_t multi_write_md; /* High-level I/O routine for
                                                writing data for multi-dset */
  H5D_io_single_read_md_func_t
      single_read_md; /* I/O routine for reading single block for multi-dset */
  H5D_io_single_write_md_func_t
      single_write_md; /* I/O routine for writing single block for multi-dset */
} H5D_md_io_ops_t;
...
typedef struct H5D_piece_info_t {
  ...
  hsize_t scaled[H5O_LAYOUT_NDIMS]; /* Scaled coordinates of chunk (in file
                                       dataset's dataspace) */
  H5S_t *fspace;          /* Dataspace describing chunk & selection in it */
  unsigned fspace_shared; /* Indicate that the file space for a chunk is shared
                             and shouldn't be freed */
  H5S_t *mspace; /* Dataspace describing selection in memory corresponding to
                    this chunk */
  unsigned mspace_shared; /* Indicate that the memory space for a chunk is
                             shared and shouldn't be freed */
...
typedef struct H5D_dset_io_info_t {
  ...
  H5D_layout_ops_t
      layout_ops; /* Dataset layout I/O operation function pointers */
...
typedef struct H5D_io_info_t {
  ...
  H5D_piece_info_t *
      *sel_pieces;       /* Array of info struct for all pieces in I/O */
  ...
  hbool_t tconv_buf_allocated; /* Whether the type conversion buffer was
                                  allocated */
  size_t tconv_buf_size;       /* Size of type conversion buffer */
  uint8_t *bkg_buf;            /* Background buffer */
  hbool_t bkg_buf_allocated;   /* Whether the background buffer was allocated */
  size_t bkg_buf_size;         /* Size of background buffer */
  size_t max_tconv_type_size;  /* Largest of all source and destination type
                                  sizes involved in type  conversion */
  hbool_t must_fill_bkg; /* Whether any datasets need a background buffer filled
                            with destination contents */
  hbool_t may_use_in_place_tconv; /* Whether datasets in this I/O could
                                     potentially use in-place type
                                     conversion if the type sizes are compatible
                                     with it */
#ifdef H5_HAVE_PARALLEL
  H5D_mpio_actual_io_mode_t
      actual_io_mode; /* Actual type of collective or independent I/O */
#endif                /* H5_HAVE_PARALLEL */
...
typedef hbool_t (*H5D_chunk_is_space_alloc_func_t)(
...
typedef herr_t (*H5D_chunk_copy_setup_func_t)(
    const H5D_chk_idx_info_t *idx_info_src,
    const H5D_chk_idx_info_t *idx_info_dst);
typedef herr_t (*H5D_chunk_copy_shutdown_func_t)(
    H5O_storage_chunk_t *storage_src, H5O_storage_chunk_t *storage_dst);
typedef herr_t (*H5D_chunk_size_func_t)(const H5D_chk_idx_info_t *idx_info,
                                        hsize_t *idx_size);
typedef herr_t (*H5D_chunk_reset_func_t)(H5O_storage_chunk_t *storage,
                                         hbool_t reset_addr);
...
typedef struct H5D_chunk_ops_t {
  ...
  H5D_chunk_copy_setup_func_t copy_setup; /* Routine to perform any necessary
                                             setup for copying chunks */
  H5D_chunk_copy_shutdown_func_t
      copy_shutdown; /* Routine to perform any necessary shutdown for copying
                        chunks */
  ...
  H5D_chunk_dest_func_t
      dest; /* Routine to destroy indexing information in memory */
} H5D_chunk_ops_t;
...
typedef struct H5D_chunk_map_t {
  ...
  H5SL_t *dset_sel_pieces; /* Skip list containing information for each chunk
                              selected */
...
typedef struct H5D_rdcc_t {
  ...
  struct H5D_rdcc_ent_t
      *tmp_head; /* Head of temporary doubly linked list.  Chunks on this list
                    are not in the hash table (slot).  The head entry is a
                    sentinel (does not refer to an actual chunk). */
  size_t nbytes_used;           /* Current cached raw data in bytes */
  int nused;                    /* Number of chunk slots in use        */
  H5D_chunk_cached_t last;      /* Cached copy of last chunk information */
  struct H5D_rdcc_ent_t **slot; /* Chunk slots, each points to a chunk*/
  H5SL_t *
      sel_chunks; /* Skip list containing information for each chunk selected */
  ...
  hsize_t scaled_power2up[H5S_MAX_RANK]; /* The scaled dim sizes, rounded up to
                                            next power of 2 */
  unsigned scaled_encode_bits[H5S_MAX_RANK]; /* The number of bits needed to
                                                encode the scaled dim sizes */
} H5D_rdcc_t;
...
typedef struct H5D_rdcdc_t {
  ...
  size_t
      sieve_buf_size; /* Size of the data sieve buffer allocated (in bytes) */
  hbool_t
      sieve_dirty; /* Flag to indicate that the data sieve buffer is dirty */
} H5D_rdcdc_t;
...
struct H5D_shared_t {
  ...
  hsize_t curr_power2up[H5S_MAX_RANK]; /* The curr. dim sizes, rounded up to
                                          next power of 2 */
  ...
  struct {
    ...
    H5SL_t *sel_pieces; /* Skip list containing information for each piece
                           selected */
  } cache;
...
typedef struct H5D_fill_buf_info_t {
  H5MM_allocate_t
      fill_alloc_func;        /* Routine to call for allocating fill buffer */
  ...
  H5T_path_t *fill_to_mem_tpath; /* Datatype conversion path for converting the
                                    fill value to the memory buffer */
  H5T_path_t *mem_to_dset_tpath; /* Datatype conversion path for converting the
                                    memory buffer to the dataset elements */
  ...
  size_t mem_elmt_size,
      file_elmt_size;         /* Size of element in memory and on disk */
...
/* The v2 B-tree class for indexing chunked datasets with >1 unlimited
 * dimensions */
...
H5_DLL H5D_t *H5D__create_named(const H5G_loc_t *loc, const char *name,
                                hid_t type_id, const H5S_t *space,
...
H5_DLL herr_t H5D__get_num_chunks(const H5D_t *dset, const H5S_t *space,
                                  hsize_t *nchunks);
H5_DLL herr_t H5D__get_chunk_info(const H5D_t *dset, const H5S_t *space,
                                  hsize_t chk_idx, hsize_t *coord,
                                  unsigned *filter_mask, haddr_t *offset,
                                  hsize_t *size);
H5_DLL herr_t H5D__get_chunk_info_by_coord(const H5D_t *dset,
                                           const hsize_t *coord,
                                           unsigned *filter_mask, haddr_t *addr,
                                           hsize_t *size);
H5_DLL herr_t H5D__chunk_iter(H5D_t *dset, H5D_chunk_iter_op_t cb,
                              void *op_data);
H5_DLL haddr_t H5D__get_offset(const H5D_t *dset);
H5_DLL herr_t H5D__vlen_get_buf_size(H5D_t *dset, hid_t type_id, hid_t space_id,
                                     hsize_t *size);
H5_DLL herr_t H5D__vlen_get_buf_size_gen(H5VL_object_t *vol_obj, hid_t type_id,
...
H5_DLL herr_t H5D_select_io_mem(void *dst_buf, H5S_t *dst_space,
                                const void *src_buf, H5S_t *src_space,
...
H5_DLL herr_t H5D__layout_oh_write(const H5D_t *dataset, H5O_t *oh,
...
H5_DLL herr_t H5D__contig_copy(H5F_t *f_src,
                               ...
                               H5F_t *f_dst, H5O_storage_contig_t *storage_dst,
                               H5T_t *src_dtype, H5O_copy_t *cpy_info);
...
H5_DLL void *H5D__chunk_mem_realloc(void *chk, size_t size,
                                    const H5O_pline_t *pline);
H5_DLL herr_t H5D__chunk_update_old_edge_chunks(H5D_t *dset, hsize_t old_dim[]);
H5_DLL hbool_t H5D__chunk_is_partial_edge_chunk(unsigned dset_ndims,
                                                const uint32_t *chunk_dims,
                                                const hsize_t *chunk_scaled,
...
H5_DLL herr_t H5D__chunk_get_offset_copy(const H5D_t *dset,
                                         const hsize_t *offset,
                                         hsize_t *offset_copy);
H5_DLL herr_t H5D__chunk_direct_write(H5D_t *dset, uint32_t filters,
                                      hsize_t *offset, uint32_t data_size,
                                      const void *buf);
...
H5_DLL herr_t H5D__virtual_hold_source_dset_files(
    const H5D_t *dset, H5D_virtual_held_file_t **head);
...
H5_DLL herr_t H5D__fill(const void *fill, const H5T_t *fill_type, void *buf,
                        const H5T_t *buf_type, H5S_t *space);
H5_DLL herr_t H5D__fill_init(H5D_fill_buf_info_t *fb_info,
                             void *caller_fill_buf, H5MM_allocate_t alloc_func,
                             void *alloc_info, H5MM_free_t free_func,
                             void *free_info, const H5O_fill_t *fill,
                             const H5T_t *dset_type, hid_t dset_type_id,
...
H5_DLL herr_t H5D__mpio_select_read(const H5D_io_info_t *io_info,
                                    hsize_t nelmts, H5S_t *file_space,
                                    H5S_t *mem_space);
...
H5_DLL herr_t H5D__mpio_get_no_coll_cause_strings(char *local_cause,
...
H5_DLL herr_t H5D__current_cache_size_test(hid_t did, size_t *nbytes_used,
                                           int *nused);
#endif /* H5D_TESTING */
