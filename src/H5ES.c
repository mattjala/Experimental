...
hid_t H5EScreate(void) {
  ...
  if (NULL == (es = H5ES__create()))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTCREATE, H5I_INVALID_HID,
                "can't create event set");

  /* Register the new event set to get an ID for it */
  if ((ret_value = H5I_register(H5I_EVENTSET, es, TRUE)) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTREGISTER, H5I_INVALID_HID,
                "can't register event set");
 ...
 *              authors and is _not_ designed for general-purpose application
 *use.
 *
 * Return:      SUCCEED / FAIL
 *
...
herr_t H5ESinsert_request(hid_t es_id, hid_t connector_id, void *request) {
  ...
  if (H5ES__insert_request(es, connector, request) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTINSERT, FAIL,
                "can't insert request into event set");
...
herr_t H5ESget_requests(hid_t es_id, H5_iter_order_t order,
                        hid_t *connector_ids, void **requests, size_t array_len,
                        size_t *count /*out*/) {
  H5ES_t *es;                 /* Event set */
  herr_t ret_value = SUCCEED; /* Return value */

  FUNC_ENTER_API(FAIL)
  H5TRACE6("e", "iIo*i**xzx", es_id, order, connector_ids, requests, array_len,
           count);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ESwait(hid_t es_id, uint64_t timeout, size_t *num_in_progress /*out*/,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EScancel(hid_t es_id, size_t *num_not_canceled /*out*/,
                  hbool_t *op_failed /*out*/) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5ES_NONE != es_id) {
    ...
    if (NULL == num_not_canceled)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "NULL num_not_canceled pointer");
    if (NULL == op_failed)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "NULL op_failed pointer");

    /* Cancel operations */
    if (H5ES__cancel(es, num_not_canceled, op_failed) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTCANCEL, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ESget_err_status(hid_t es_id, hbool_t *err_status /*out*/) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5ES_NONE != es_id) {
    ...
    if (H5ES__get_err_info(es, num_err_info, err_info, num_cleared) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTGET, FAIL,
                  "can't retrieve error info for failed operation(s)");
  } /* end if */
...
herr_t H5ESfree_err_info(size_t num_err_info, H5ES_err_info_t err_info[]) {
  ...
  for (u = 0; u < num_err_info; u++) {
    ...
    if (H5I_dec_app_ref(err_info[u].err_stack_id) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTDEC, FAIL,
                  "can't close error stack for err_info #%zu", u);
  } /* end for */
...
herr_t H5ESregister_insert_func(hid_t es_id, H5ES_event_insert_func_t func,
  ...
  if (H5ES_NONE != es_id) {
    ...
    if (NULL == func)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "NULL function callback pointer");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ESregister_complete_func(hid_t es_id, H5ES_event_complete_func_t func,
                                  void *ctx) {
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5ESclose(hid_t es_id) {
  ...
  if (H5ES_NONE != es_id) {
    ...
    if (H5I_dec_app_ref(es_id) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTDEC, FAIL,
                  "unable to decrement ref count on event set");
  } /* end if */
