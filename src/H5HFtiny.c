...
#define H5HF_TINY_LEN_SHORT                                                    \
  16 /* Max. length able to be encoded in first heap ID byte */
#define H5HF_TINY_MASK_SHORT                                                   \
  0x0F /* Mask for length in first heap ID byte                */
#define H5HF_TINY_MASK_EXT                                                     \
  0x0FFF /* Mask for length in two heap ID bytes                 */
#define H5HF_TINY_MASK_EXT_1                                                   \
  0x0F00 /* Mask for length in first byte of two heap ID bytes   */
#define H5HF_TINY_MASK_EXT_2                                                   \
  0x00FF /* Mask for length in second byte of two heap ID bytes  */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__tiny_insert(H5HF_hdr_t *hdr, size_t obj_size, const void *obj,
                         void *_id) {
  ...
  memset(
      id + obj_size, 0,
      (hdr->id_len - ((size_t)1 + (size_t)hdr->tiny_len_extended + obj_size)));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__tiny_get_obj_len(H5HF_hdr_t *hdr, const uint8_t *id,
                              size_t *obj_len_p) {
  size_t enc_obj_size; /* Encoded object size */
  ...
  if (!hdr->tiny_len_extended)
     ...
     */
    enc_obj_size =
        (size_t) * (id + 1) | ((size_t)(*id & H5HF_TINY_MASK_EXT_1) << 8);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__tiny_op(H5HF_hdr_t *hdr, const uint8_t *id, H5HF_operator_t op,
                     void *op_data) {
  herr_t ret_value = SUCCEED; /* Return value */
