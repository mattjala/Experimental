...
/* This message derives from H5O message class, for old fill value before
 * version 1.5 */
...
/* This message derives from H5O message class, for new fill value after
 * version 1.4  */
...
#define H5O_FILL_FLAGS_ALL                                                     \
  (H5O_FILL_MASK_ALLOC_TIME |                                                  \
   (H5O_FILL_MASK_FILL_TIME << H5O_FILL_SHIFT_FILL_TIME) |                     \
...
static void *H5O__fill_new_decode(H5F_t H5_ATTR_UNUSED *f,
  ...
  fill->version = *p++;
  if (fill->version < H5O_FILL_VERSION_1 ||
  ...
  if (fill->version < H5O_FILL_VERSION_3) {
    ...
    if (flags & (unsigned)~H5O_FILL_FLAGS_ALL)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, NULL,
                  "unknown flag for fill value message");

    /* Space allocation time */
    fill->alloc_time = (H5D_alloc_time_t)((flags >> H5O_FILL_SHIFT_ALLOC_TIME) &
                                          H5O_FILL_MASK_ALLOC_TIME);

    /* Fill value write time */
    fill->fill_time = (H5D_fill_time_t)((flags >> H5O_FILL_SHIFT_FILL_TIME) &
    ...
    if (flags & H5O_FILL_FLAG_UNDEFINED_VALUE) {
      ...
      if (flags & H5O_FILL_FLAG_HAVE_VALUE)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, NULL,
                    "have value and undefined value flags both set");
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__fill_old_decode(H5F_t *f, H5O_t *open_oh,
                                  unsigned H5_ATTR_UNUSED mesg_flags,
                                  unsigned H5_ATTR_UNUSED *ioflags,
                                  size_t p_size, const uint8_t *p) {
  ...
  if (fill->size > 0) {
    ...
    /* Ensure that fill size doesn't exceed buffer size, due to possible data
     * corruption */
    if (H5_IS_BUFFER_OVERFLOW(p, fill->size, p_end))
      HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                  "ran off end of input buffer while decoding");
    ...
    if (exists) {
      if (NULL ==
          (dt = (H5T_t *)H5O_msg_read_oh(f, open_oh, H5O_DTYPE_ID, NULL)))
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__fill_new_encode(H5F_t H5_ATTR_UNUSED *f, uint8_t *p,
                                   const void *_fill) {
  ...
  *p++ = (uint8_t)fill->version;
  ...
  if (fill->version < H5O_FILL_VERSION_3) {
    ...
    *p++ = (uint8_t)fill->fill_defined;
    ...
    flags = (uint8_t)(flags | ((H5O_FILL_MASK_FILL_TIME & fill->fill_time)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__fill_old_encode(H5F_t H5_ATTR_UNUSED *f, uint8_t *p,
                                   const void *_fill) {
  const H5O_fill_t *fill = (const H5O_fill_t *)_fill;
    ...
    if (src->type) {
      ...
      if (!H5T_path_noop(tpath)) {
        ...
        dst_id = H5I_register(H5I_DATATYPE,
                              H5T_copy(dst->type, H5T_COPY_TRANSIENT), FALSE);
        if (dst_id < 0)
        ...
        if (H5T_convert(tpath, src_id, dst_id, (size_t)1, (size_t)0, (size_t)0,
          ...
          HGOTO_ERROR(H5E_OHDR, H5E_CANTCONVERT, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__fill_new_size(const H5F_t H5_ATTR_UNUSED *f,
                                 const void *_fill) {
  const H5O_fill_t *fill = (const H5O_fill_t *)_fill;
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__fill_old_size(const H5F_t H5_ATTR_UNUSED *f,
                                 const void *_fill) {
  const H5O_fill_t *fill = (const H5O_fill_t *)_fill;
...
herr_t H5O_fill_reset_dyn(H5O_fill_t *fill) {
  hid_t fill_type_id = -1;    /* Datatype ID for fill value datatype when
                                 reclaiming VL fill values */
  ...
  if (fill->buf) {
    if (fill->type && H5T_detect_class(fill->type, H5T_VLEN, FALSE) > 0) {
      ...
      if ((fill_type_id = H5I_register(H5I_DATATYPE, fill_type, FALSE)) < 0) {
        ...
        HGOTO_ERROR(H5E_OHDR, H5E_CANTREGISTER, FAIL,
                    "unable to register fill value datatype");
      } /* end if */
      ...
      if (H5T_reclaim(fill_type_id, fill_space, fill->buf) < 0) {
        ...
        HGOTO_ERROR(H5E_OHDR, H5E_BADITER, FAIL,
                    "unable to reclaim variable-length fill value data");
      } /* end if */
  ...
  if (fill_type_id > 0 && H5I_dec_ref(fill_type_id) < 0)
    HDONE_ERROR(H5E_OHDR, H5E_CANTDEC, FAIL,
                "unable to decrement ref count for temp ID");
...
static herr_t H5O__fill_pre_copy_file(H5F_t H5_ATTR_UNUSED *file_src,
  ...
  const H5O_fill_t *fill_src =
      (const H5O_fill_t *)mesg_src; /* Source fill value */
  herr_t ret_value = SUCCEED;       /* Return value */
  ...
  if (fill_src->version >
      H5O_fill_ver_bounds[H5F_HIGH_BOUND(cpy_info->file_dst)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "fill value message version out of bounds");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__fill_debug(H5F_t H5_ATTR_UNUSED *f, const void *_fill,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_fill_convert(H5O_fill_t *fill, H5T_t *dset_type,
  ...
  if (!H5T_path_noop(tpath)) {
    ...
    if ((src_id = H5I_register(H5I_DATATYPE, H5T_copy(fill->type, H5T_COPY_ALL),
                               FALSE)) < 0 ||
        (dst_id = H5I_register(H5I_DATATYPE, H5T_copy(dset_type, H5T_COPY_ALL),
                               FALSE)) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
    ...
    if (NULL ==
        (buf = H5MM_malloc(MAX(fill_type_size, H5T_get_size(dset_type)))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "memory allocation failed for type conversion");
    H5MM_memcpy(buf, fill->buf, fill_type_size);

    /* Use CALLOC here to clear the buffer in case later the library thinks
     * there's data in the background. */
    if (H5T_path_bkg(tpath) &&
        NULL == (bkg = H5MM_calloc(H5T_get_size(dset_type))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "memory allocation failed for type conversion");
...
herr_t H5O_fill_set_version(H5F_t *f, H5O_fill_t *fill) {
  ...
  if (version > H5O_fill_ver_bounds[H5F_HIGH_BOUND(f)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "Filter pipeline version out of bounds");
