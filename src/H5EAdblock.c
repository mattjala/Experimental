 ...
 *-------------------------------------------------------------------------
 */
H5EA_dblock_t *H5EA__dblock_alloc(H5EA_hdr_t *hdr, void *parent,
                                  size_t nelmts) {
 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5EA__dblock_create(H5EA_hdr_t *hdr, void *parent,
                            hbool_t *stats_changed, hsize_t dblk_off,
                            size_t nelmts) {
  H5EA_dblock_t *dblock = NULL; /* Extensible array data block */
  ...
  if (HADDR_UNDEF == (dblock_addr = H5MF_alloc(hdr->f, H5FD_MEM_EARRAY_DBLOCK,
                                               (hsize_t)dblock->size)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF,
  ...
  if (!dblock->npages)
    ...
    if ((hdr->cparam.cls->fill)(dblock->elmts, (size_t)dblock->nelmts) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, HADDR_UNDEF,
                  "can't set extensible array data block elements to class's "
                  "fill value");

  /* Cache the new extensible array data block */
  if (H5AC_insert_entry(hdr->f, H5AC_EARRAY_DBLOCK, dblock_addr, dblock,
  ...
  if (!H5_addr_defined(ret_value))
    if (dblock) {
      ...
      if (H5_addr_defined(dblock->addr) &&
          H5MF_xfree(hdr->f, H5FD_MEM_EARRAY_DBLOCK, dblock->addr,
                     (hsize_t)dblock->size) < 0)
...
unsigned H5EA__dblock_sblk_idx(const H5EA_hdr_t *hdr, hsize_t idx) {
  ...
  sblk_idx =
      H5VM_log2_gen((uint64_t)((idx / hdr->cparam.data_blk_min_elmts) + 1));
 ...
 *-------------------------------------------------------------------------
 */
H5EA_dblock_t *H5EA__dblock_protect(H5EA_hdr_t *hdr, void *parent,
  ...
  if (NULL == (dblock = (H5EA_dblock_t *)H5AC_protect(
                   hdr->f, H5AC_EARRAY_DBLOCK, dblk_addr, &udata, flags)))
  ...
  if (!ret_value) {
    ...
    if (dblock && H5AC_unprotect(hdr->f, H5AC_EARRAY_DBLOCK, dblock->addr,
 ...
 *
 * Purpose:	Convenience wrapper around unprotecting extensible array data
 *block
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__dblock_unprotect(H5EA_dblock_t *dblock, unsigned cache_flags) {
  ...
  if (H5AC_unprotect(dblock->hdr->f, H5AC_EARRAY_DBLOCK, dblock->addr, dblock,
                     cache_flags) < 0)
    HGOTO_ERROR(
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__dblock_delete(H5EA_hdr_t *hdr, void *parent, haddr_t dblk_addr,
                           size_t dblk_nelmts) {
  ...
  /* Protect data block */
  if (NULL == (dblock = H5EA__dblock_protect(hdr, parent, dblk_addr,
                                             dblk_nelmts, H5AC__NO_FLAGS_SET)))
  ...
  if (dblk_nelmts > hdr->dblk_page_nelmts) {
    size_t npages =
        dblk_nelmts / hdr->dblk_page_nelmts; /* Number of pages in data block */
    ...
    for (u = 0; u < npages; u++) {
      ...
      if (H5AC_expunge_entry(hdr->f, H5AC_EARRAY_DBLK_PAGE, dblk_page_addr,
                             H5AC__NO_FLAGS_SET) < 0)
        HGOTO_ERROR(
            H5E_EARRAY, H5E_CANTEXPUNGE, FAIL,
  ...
  if (dblock &&
      H5EA__dblock_unprotect(dblock, H5AC__DIRTIED_FLAG | H5AC__DELETED_FLAG |
