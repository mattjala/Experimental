...
static herr_t H5AC__receive_haddr_list(MPI_Comm mpi_comm,
                                       unsigned *num_entries_ptr,
                                       haddr_t **haddr_buf_ptr_ptr);
...
static herr_t H5AC__tidy_cache_0_lists(H5AC_t *cache_ptr,
...
herr_t H5AC_add_candidate(H5AC_t *cache_ptr, haddr_t addr) {
  ...
  aux_ptr = (H5AC_aux_t *)H5C_get_aux_ptr(cache_ptr);
  assert(aux_ptr != NULL);
  assert(aux_ptr->metadata_write_strategy ==
...
static herr_t H5AC__broadcast_candidate_list(H5AC_t *cache_ptr,
  ...
  if (MPI_SUCCESS != (mpi_result = MPI_Bcast(&num_entries, 1, MPI_UNSIGNED, 0,
                                             aux_ptr->mpi_comm)))
  ...
  if (num_entries > 0) {
     ...
     */
    if (H5AC__copy_candidate_list_to_buffer(cache_ptr, &chk_num_entries,
                                            &haddr_buf_ptr) < 0) {
    ...
    if (MPI_SUCCESS !=
        (mpi_result = MPI_Bcast((void *)haddr_buf_ptr, (int)buf_size, MPI_BYTE,
                                0, aux_ptr->mpi_comm)))
      HMPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_result)
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5AC__broadcast_clean_list_cb(void *_item,
                                            void H5_ATTR_UNUSED *_key,
                                            void *_udata) {
  H5AC_slist_entry_t *slist_entry_ptr =
      (H5AC_slist_entry_t *)_item; /* Address of item */
  H5AC_addr_list_ud_t *udata =
      (H5AC_addr_list_ud_t *)_udata; /* Context for callback */
  haddr_t addr;
   ...
   * if it exists.
   */
  if (NULL != (slist_entry_ptr = (H5AC_slist_entry_t *)H5SL_remove(
                   udata->aux_ptr->d_slist_ptr, (void *)(&addr))))
    slist_entry_ptr = H5FL_FREE(H5AC_slist_entry_t, slist_entry_ptr);
...
static herr_t H5AC__broadcast_clean_list(H5AC_t *cache_ptr) {
  ...
  if (num_entries > 0) {
    ...
    if (NULL == (addr_buf_ptr = (haddr_t *)H5MM_malloc(buf_size))) {
      /* Push an error, but still participate in following MPI_Bcast */
      HDONE_ERROR(H5E_CACHE, H5E_CANTALLOC, FAIL,
      ...
      /* Free all the clean list entries, building the address list in the
       * callback */
      /* (Callback also removes the matching entries from the dirtied list) */
      if (H5SL_free(aux_ptr->c_slist_ptr, H5AC__broadcast_clean_list_cb,
                    &udata) < 0) {
        /* Push an error, but still participate in following MPI_Bcast */
        HDONE_ERROR(H5E_CACHE, H5E_CANTFREE, FAIL,
                    "Can't build address list for clean entries");
      }
    }

    /* Now broadcast the list of cleaned entries */
    if (MPI_SUCCESS !=
        (mpi_result = MPI_Bcast((void *)addr_buf_ptr, (int)buf_size, MPI_BYTE,
                                0, aux_ptr->mpi_comm)))
      HMPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_result)
  } /* end if */
 ...
 */
static herr_t
H5AC__construct_candidate_list(H5AC_t *cache_ptr,
                               H5AC_aux_t H5_ATTR_NDEBUG_UNUSED *aux_ptr,
                               int sync_point_op) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  assert(aux_ptr->metadata_write_strategy ==
         H5AC_METADATA_WRITE_STRATEGY__DISTRIBUTED);
  assert((sync_point_op == H5AC_SYNC_POINT_OP__FLUSH_CACHE) ||
         (aux_ptr->mpi_rank == 0));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5AC__copy_candidate_list_to_buffer_cb(void *_item,
                                                     void H5_ATTR_UNUSED *_key,
                                                     void *_udata) {
  H5AC_slist_entry_t *slist_entry_ptr =
      (H5AC_slist_entry_t *)_item; /* Address of item */
...
static herr_t H5AC__copy_candidate_list_to_buffer(const H5AC_t *cache_ptr,
  ...
  /* Free all the candidate list entries, building the address list in the
   * callback */
  if (H5SL_free(aux_ptr->candidate_slist_ptr,
                H5AC__copy_candidate_list_to_buffer_cb, &udata) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTFREE, FAIL,
                "Can't build address list for candidate entries");
...
herr_t H5AC__log_deleted_entry(const H5AC_info_t *entry_ptr) {
  ...
  if (NULL != (slist_entry_ptr = (H5AC_slist_entry_t *)H5SL_remove(
                   aux_ptr->d_slist_ptr, (void *)(&addr))))
    slist_entry_ptr = H5FL_FREE(H5AC_slist_entry_t, slist_entry_ptr);

  /* if the entry appears in the cleaned entry slist, remove it. */
  if (NULL != (slist_entry_ptr = (H5AC_slist_entry_t *)H5SL_remove(
                   aux_ptr->c_slist_ptr, (void *)(&addr))))
    slist_entry_ptr = H5FL_FREE(H5AC_slist_entry_t, slist_entry_ptr);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC__log_flushed_entry(H5C_t *cache_ptr, haddr_t addr,
...
herr_t H5AC__log_inserted_entry(const H5AC_info_t *entry_ptr) {
  ...
  if (aux_ptr->mpi_rank == 0) {
    ...
    if (NULL !=
        H5SL_search(aux_ptr->d_slist_ptr, (const void *)(&entry_ptr->addr)))
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
    ...
    if (NULL !=
        H5SL_search(aux_ptr->c_slist_ptr, (const void *)(&entry_ptr->addr)))
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC__log_moved_entry(const H5F_t *f, haddr_t old_addr,
                             haddr_t new_addr) {
  H5AC_t *cache_ptr;
  H5AC_aux_t *aux_ptr;
  ...
  if (H5C_get_entry_status(f, old_addr, &entry_size, &entry_in_cache,
                           &entry_dirty, NULL, NULL, NULL, NULL, NULL,
                           NULL) < 0)
  ...
  if (aux_ptr->mpi_rank == 0) {
     ...
     */
    if (NULL != (slist_entry_ptr = (H5AC_slist_entry_t *)H5SL_remove(
                     aux_ptr->c_slist_ptr, (void *)(&old_addr))))
      slist_entry_ptr = H5FL_FREE(H5AC_slist_entry_t, slist_entry_ptr);
     ...
     */
    if (NULL != (slist_entry_ptr = (H5AC_slist_entry_t *)H5SL_remove(
                     aux_ptr->d_slist_ptr, (void *)(&old_addr))))
...
static herr_t H5AC__propagate_and_apply_candidate_list(H5F_t *f) {
  ...
  if (aux_ptr->mpi_rank == 0) {
    if (H5AC__broadcast_candidate_list(cache_ptr, &num_candidates,
                                       &candidates_list_ptr) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
    ...
    if (H5AC__receive_candidate_list(cache_ptr, &num_candidates,
                                     &candidates_list_ptr) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
  ...
  if (num_candidates > 0) {
    ...
    result = H5C_apply_candidate_list(f, cache_ptr, num_candidates,
                                      candidates_list_ptr, aux_ptr->mpi_rank,
                                      aux_ptr->mpi_size);
     ...
     */
    if (aux_ptr->mpi_rank == 0)
      if (H5AC__tidy_cache_0_lists(cache_ptr, num_candidates,
                                   candidates_list_ptr) < 0)
        HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
...
static herr_t H5AC__receive_haddr_list(MPI_Comm mpi_comm,
   ...
   */
  if (MPI_SUCCESS !=
      (mpi_result = MPI_Bcast(&num_entries, 1, MPI_UNSIGNED, 0, mpi_comm)))
    HMPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_result)
...
static herr_t H5AC__receive_and_apply_clean_list(H5F_t *f) {
  ...
  if (H5AC__receive_haddr_list(aux_ptr->mpi_comm, &num_entries,
                               &haddr_buf_ptr) < 0)
...
static herr_t H5AC__receive_candidate_list(const H5AC_t *cache_ptr,
  ...
  if (H5AC__receive_haddr_list(aux_ptr->mpi_comm, num_entries_ptr,
                               haddr_buf_ptr_ptr) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTGET, FAIL, "can't receive clean list");
...
static herr_t H5AC__rsp__dist_md_write__flush(H5F_t *f) {
  ...
  if (H5SL_count(aux_ptr->candidate_slist_ptr) > 0) {
     ...
     */
    if (H5AC__copy_candidate_list_to_buffer(cache_ptr, &num_entries,
                                            &haddr_buf_ptr) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                  "Can't construct candidate buffer.");
    ...
    /* Apply the candidate list */
    result = H5C_apply_candidate_list(f, cache_ptr, num_entries, haddr_buf_ptr,
...
static herr_t H5AC__rsp__dist_md_write__flush_to_min_clean(H5F_t *f) {
  ...
  if (evictions_enabled) {
    ...
    if (aux_ptr->mpi_rank == 0) {
      /* If constructing candidate list fails, push an error but still
       * participate in collective operations during following candidate list
       * propagation
       */
      if (H5AC__construct_candidate_list(
              cache_ptr, aux_ptr, H5AC_SYNC_POINT_OP__FLUSH_TO_MIN_CLEAN) < 0)
        HDONE_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                    "Can't construct candidate list.");
    }
...
herr_t H5AC__run_sync_point(H5F_t *f, int sync_point_op) {
  ...
  fprintf(stdout, "%d:%s...:%u: (u/uu/i/iu/m/mu) = %zu/%u/%zu/%u/%zu/%u\n",
          aux_ptr->mpi_rank, __func__ aux_ptr->dirty_bytes_propagations,
  ...
  case H5AC_METADATA_WRITE_STRATEGY__PROCESS_0_ONLY:
    ...
    case H5AC_SYNC_POINT_OP__FLUSH_CACHE:
      if (H5AC__rsp__p0_only__flush(f) < 0)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTGET, FAIL,
                    "H5AC__rsp__p0_only__flush() failed.");
      break;
...
static herr_t H5AC__tidy_cache_0_lists(H5AC_t *cache_ptr,
  ...
  for (u = 0; u < num_candidates; u++) {
     ...
     */
    if (NULL != (d_slist_entry_ptr = (H5AC_slist_entry_t *)H5SL_remove(
                     aux_ptr->d_slist_ptr, (void *)&addr)))
      d_slist_entry_ptr = H5FL_FREE(H5AC_slist_entry_t, d_slist_entry_ptr);
    if (NULL != (c_slist_entry_ptr = (H5AC_slist_entry_t *)H5SL_remove(
                     aux_ptr->c_slist_ptr, (void *)&addr)))
