 ...
 *
 * Note:	    The actual name of this routine can be different in each
 *source file that this header file is included in, and must be defined prior to
 ...
 *-------------------------------------------------------------------------
 */
static inline void *H5O_SHARED_DECODE(H5F_t *f, H5O_t *open_oh,
...
#error "Need to define H5O_SHARED_DECODE_REAL macro!"
  ...
  if (mesg_flags & H5O_MSG_FLAG_SHARED) {
    ...
    if (NULL == (ret_value = H5O__shared_decode(f, open_oh, ioflags, p,
                                                H5O_SHARED_TYPE)))
    ...
    *ioflags &= ~H5O_DECODEIO_DIRTY;
    ...
    if (NULL == (ret_value = H5O_SHARED_DECODE_REAL(f, open_oh, mesg_flags,
                                                    ioflags, p_size, p)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTDECODE, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static inline herr_t H5O_SHARED_ENCODE(H5F_t *f, hbool_t disable_shared,
...
#error "Need to define H5O_SHARED_ENCODE_REAL macro!"
  ...
  /* Sanity check */
  assert(sh_mesg->type == H5O_SHARE_TYPE_UNSHARED ||
 ...
 *-------------------------------------------------------------------------
 */
static inline size_t H5O_SHARED_SIZE(const H5F_t *f, hbool_t disable_shared,
                                     const void *_mesg) {
  const H5O_shared_t *sh_mesg = (const H5O_shared_t *)
      _mesg; /* Pointer to shared message portion of actual message */
 ...
 *-------------------------------------------------------------------------
 */
static inline void *H5O_SHARED_COPY_FILE(H5F_t *file_src, void *_native_src,
                                         H5F_t *file_dst,
...
H5O_SHARED_POST_COPY_FILE(const H5O_loc_t H5_ATTR_NDEBUG_UNUSED *oloc_src,
                          const void *mesg_src, H5O_loc_t *oloc_dst,
                          void *mesg_dst, unsigned *mesg_flags,
                          H5O_copy_t *cpy_info) {
...
#error "Need to define H5O_SHARED_POST_COPY_FILE macro!"
  ...
  if (H5O__shared_post_copy_file(oloc_dst->file, H5O_SHARED_TYPE, shared_src,
                                 shared_dst, mesg_flags, cpy_info) < 0)
  ...
  if (H5O_SHARED_POST_COPY_FILE_UPD(oloc_src, mesg_src, oloc_dst, mesg_dst,
                                    cpy_info) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL,
                "unable to update native message");
 ...
 *-------------------------------------------------------------------------
 */
static inline herr_t H5O_SHARED_DEBUG(H5F_t *f, const void *_mesg, FILE *stream,
