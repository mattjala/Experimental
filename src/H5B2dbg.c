 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__hdr_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                       int fwidth,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Size of node:", (unsigned)hdr->node_size);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Split percent:", hdr->split_percent);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Merge percent:", hdr->merge_percent);
  ...
  for (u = 0; u < (unsigned)(hdr->depth + 1); u++) {
    ...
    fprintf(stream, "%*s%-*s (%u/%u/%u)\n", indent + 3, "", MAX(0, fwidth - 3),
            temp_str, hdr->node_info[u].max_nrec, hdr->node_info[u].split_nrec,
            hdr->node_info[u].merge_nrec);
  } /* end for */

done:
  if (hdr && H5B2__hdr_unprotect(hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_PROTECT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__int_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                       int fwidth, const H5B2_class_t *type, haddr_t hdr_addr,
                       unsigned nrec, unsigned depth,
                       haddr_t H5_ATTR_NDEBUG_UNUSED obj_addr) {
  ...
  if (NULL == (hdr = H5B2__hdr_protect(f, hdr_addr, f, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTLOAD, FAIL,
                "unable to load v2 B-tree header");
  ...
  if (NULL ==
      (internal = H5B2__protect_internal(hdr, NULL, &node_ptr, (uint16_t)depth,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Number of records in node:", internal->nrec);

  /* Print all node pointers and records */
  for (u = 0; u < internal->nrec; u++) {
    /* Print node pointer */
    HDsnprintf(temp_str, sizeof(temp_str), "Node pointer #%u: (all/node/addr)",
               u);
    fprintf(stream, "%*s%-*s (%" PRIuHSIZE "/%u/%" PRIuHADDR ")\n", indent + 3,
            "", MAX(0, fwidth - 3), temp_str, internal->node_ptrs[u].all_nrec,
            internal->node_ptrs[u].node_nrec, internal->node_ptrs[u].addr);
    ...
    (void)(type->debug)(stream, indent + 6, MAX(0, fwidth - 6),
                        H5B2_INT_NREC(internal, hdr, u), hdr->cb_ctx);
  } /* end for */
  ...
  if (hdr && H5B2__hdr_unprotect(hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_PROTECT, FAIL,
                "unable to release v2 B-tree header");
  if (internal &&
      H5AC_unprotect(f, H5AC_BT2_INT, addr, internal, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_PROTECT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__leaf_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                        int fwidth, const H5B2_class_t *type, haddr_t hdr_addr,
                        unsigned nrec, haddr_t H5_ATTR_NDEBUG_UNUSED obj_addr) {
  H5B2_hdr_t *hdr = NULL;     /* B-tree header */
  ...
  node_ptr.addr = addr;
  H5_CHECKED_ASSIGN(node_ptr.node_nrec, uint16_t, nrec, unsigned);
  if (NULL == (leaf = H5B2__protect_leaf(hdr, NULL, &node_ptr, FALSE,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Number of records in node:", leaf->nrec);
  ...
  for (u = 0; u < leaf->nrec; u++) {
    ...
    (void)(type->debug)(stream, indent + 6, MAX(0, fwidth - 6),
                        H5B2_LEAF_NREC(leaf, hdr, u), hdr->cb_ctx);
  } /* end for */

done:
  if (hdr && H5B2__hdr_unprotect(hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_PROTECT, FAIL,
                "unable to release B-tree header");
  if (leaf &&
      H5AC_unprotect(f, H5AC_BT2_LEAF, addr, leaf, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_PROTECT, FAIL,
