...
char *H5Eget_major(H5E_major_t maj) {
  ...
  if (type != H5E_MAJOR)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTGET, NULL,
                "Error message isn't a major one");
...
char *H5Eget_minor(H5E_minor_t min) {
  ...
  if (type != H5E_MINOR)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTGET, NULL,
                "Error message isn't a minor one");
...
herr_t H5Eprint1(FILE *stream) {
  ...
  if (NULL ==
      (estack = H5E__get_my_stack())) /*lint !e506 !e774 Make lint 'constant
                                         value Boolean' in non-threaded case */
