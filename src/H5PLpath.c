...
static herr_t
H5PL__path_table_iterate_process_path(const char *plugin_path,
                                      H5PL_iterate_type_t iter_type,
                                      H5PL_iterate_t iter_op, void *op_data);
static herr_t
H5PL__find_plugin_in_path(const H5PL_search_params_t *search_params,
                          hbool_t *found, const char *dir,
...
static herr_t H5PL__insert_at(const char *path, unsigned int idx) {
  ...
  if (NULL == (path_copy = H5MM_strdup(path)))
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, FAIL,
                "can't make internal copy of path");

#ifdef H5_HAVE_WIN32_API
  /* Clean up Microsoft Windows environment variables in the path string */
  if (H5_expand_windows_env_vars(&path_copy))
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTCONVERT, FAIL,
                "can't expand environment variable string");
#endif /* H5_HAVE_WIN32_API */

  /* If the table entry is in use, make some space */
  if (H5PL_paths_g[idx])
    if (H5PL__make_space_at(idx) < 0)
      HGOTO_ERROR(H5E_PLUGIN, H5E_NOSPACE, FAIL,
                  "unable to make space in the table for the new entry");
...
static herr_t H5PL__replace_at(const char *path, unsigned int idx) {
  ...
  if (!H5PL_paths_g[idx])
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTFREE, FAIL,
                "path entry at index %u in the table is NULL", idx);
...
herr_t H5PL__create_path_table(void) {
  ...
  if (NULL == (H5PL_paths_g = (char **)H5MM_calloc(
                   (size_t)H5PL_path_capacity_g * sizeof(char *))))
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, FAIL,
                "can't allocate memory for path table");
  ...
  if (NULL == paths)
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, FAIL,
                "can't allocate memory for path copy");
  ...
  while (next_path) {
    ...
    if (H5PL__append_path(next_path) < 0)
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, FAIL, "can't insert path: %s",
                  next_path);
...
static herr_t H5PL__expand_path_table(void) {
  ...
  /* Resize the array */
  if (NULL ==
      (H5PL_paths_g = (char **)H5MM_realloc(
           H5PL_paths_g, (size_t)H5PL_path_capacity_g * sizeof(char *))))
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, FAIL,
                "allocating additional memory for path table failed");

  /* Initialize the new memory */
  memset(H5PL_paths_g + H5PL_num_paths_g, 0,
         (size_t)H5PL_PATH_CAPACITY_ADD * sizeof(char *));
...
herr_t H5PL__append_path(const char *path) {
  ...
  if (H5PL__insert_at(path, H5PL_num_paths_g) < 0)
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTINSERT, FAIL,
                "unable to append search path");
...
herr_t H5PL__remove_path(unsigned int idx) {
  ...
  if (!H5PL_paths_g[idx])
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTDELETE, FAIL,
                "search path at index %u is NULL", idx);
...
const char *H5PL__get_path(unsigned int idx) {
  ...
  if (idx >= H5PL_num_paths_g)
    HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, NULL,
                "path index %u is out of range in table", idx);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PL__path_table_iterate(H5PL_iterate_type_t iter_type,
                                H5PL_iterate_t iter_op, void *op_data) {
  unsigned int u;
  ...
  for (u = 0; (u < H5PL_num_paths_g) && (ret_value == H5_ITER_CONT); u++) {
    if ((ret_value = H5PL__path_table_iterate_process_path(
             H5PL_paths_g[u], iter_type, iter_op, op_data)) < 0)
      HGOTO_ERROR(H5E_PLUGIN, H5E_BADITER, H5_ITER_ERROR,
                  "can't iterate over plugins in plugin path '%s'",
                  H5PL_paths_g[u]);
  }
...
H5PL__path_table_iterate_process_path(const char *plugin_path,
  ...
  while (NULL != (dp = HDreaddir(dirp))) {
    ...
    if (!HDstrncmp(dp->d_name, "cyg", (size_t)3) &&
      ...
      len = HDstrlen(plugin_path) + HDstrlen(H5PL_PATH_SEPARATOR) +
            HDstrlen(dp->d_name) + 1 /*\0*/ +
            4; /* Extra "+4" to quiet GCC warning - 2019/07/05, QAK */

      if (NULL == (path = (char *)H5MM_calloc(len)))
        HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, H5_ITER_ERROR,
      ...
      if (HDstat(path, &my_stat) == -1)
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, H5_ITER_ERROR,
                    "can't stat file %s -- error was: %s", path,
                    HDstrerror(errno));
      ...
      plugin_info = NULL;
      plugin_loaded = FALSE;
      if (H5PL__open(path, H5PL_TYPE_NONE, NULL, &plugin_loaded, &plugin_type,
                     &plugin_info) < 0)
        HGOTO_ERROR(H5E_PLUGIN, H5E_CANTGET, H5_ITER_ERROR,
                    "failed to open plugin '%s'", path);

      /* Determine if we should process this plugin */
      plugin_matches =
          (iter_type == H5PL_ITER_TYPE_ALL) ||
          ((iter_type == H5PL_ITER_TYPE_FILTER) &&
           (plugin_type == H5PL_TYPE_FILTER)) ||
          ((iter_type == H5PL_ITER_TYPE_VOL) &&
           (plugin_type == H5PL_TYPE_VOL)) ||
          ((iter_type == H5PL_ITER_TYPE_VFD) && (plugin_type == H5PL_TYPE_VFD));

      /* If the plugin was successfully loaded, call supplied callback function
       * on plugin */
      if (plugin_loaded && plugin_matches &&
          (ret_value = iter_op(plugin_type, plugin_info, op_data)))
        break;
  ...
  if (ret_value < 0)
    HERROR(H5E_PLUGIN, H5E_CALLBACK,
           "callback operator function returned failure");

done:
  if (dirp)
    if (HDclosedir(dirp) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CLOSEERROR, H5_ITER_ERROR,
                  "can't close directory: %s", HDstrerror(errno));
...
H5PL__path_table_iterate_process_path(const char *plugin_path,
  ...
  do {
    /* Ignore '.' and '..' */
    if (HDstrcmp(fdFile.cFileName, ".") != 0 &&
        HDstrcmp(fdFile.cFileName, "..") != 0) {
      hbool_t plugin_matches;
      size_t len;

      /* Allocate & initialize the path name */
      len = HDstrlen(plugin_path) + HDstrlen(H5PL_PATH_SEPARATOR) +
            HDstrlen(fdFile.cFileName) + 1;
 ...
 */
herr_t
H5PL__find_plugin_in_path_table(const H5PL_search_params_t *search_params,
                                hbool_t *found, const void **plugin_info) {
  ...
  *plugin_info = NULL;
  ...
  for (u = 0; u < H5PL_num_paths_g; u++) {
    ...
    if (H5PL__find_plugin_in_path(search_params, found, H5PL_paths_g[u],
                                  plugin_info) < 0)
    ...
    if (*found) {
      if (!plugin_info)
        HGOTO_ERROR(H5E_PLUGIN, H5E_BADVALUE, FAIL,
                    "plugin info should not be NULL");
      break;
    }
  }
...
H5PL__find_plugin_in_path(const H5PL_search_params_t *search_params,
  ...
  *found = FALSE;
  ...
  if (!(dirp = HDopendir(dir)))
    HGOTO_ERROR(H5E_PLUGIN, H5E_OPENERROR, FAIL,
                "can't open directory (%s). Please verify its existence", dir);
  ...
  while (NULL != (dp = HDreaddir(dirp))) {
    ...
    if (!HDstrncmp(dp->d_name, "cyg", (size_t)3) &&
      ...
      /* attempt to open the dynamic library */
      if (H5PL__open(path, search_params->type, search_params->key, found, NULL,
                     plugin_info) < 0)
        HGOTO_ERROR(H5E_PLUGIN, H5E_CANTGET, FAIL,
                    "search in directory failed");
      if (*found)
        HGOTO_DONE(SUCCEED);
  ...
  if (dirp)
    if (HDclosedir(dirp) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CLOSEERROR, FAIL, "can't close directory: %s",
                  HDstrerror(errno));
