 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_create(H5HF_hdr_t *hdr, H5HF_indirect_t *par_iblock,
  ...
  if (par_iblock) {
    unsigned par_row =
        par_entry / hdr->man_dtable.cparam.width; /* Row for block */
    ...
    dblock->block_off += hdr->man_dtable.row_block_size[par_row] *
                         (par_entry % hdr->man_dtable.cparam.width);
    H5_CHECKED_ASSIGN(dblock->size, size_t,
                      hdr->man_dtable.row_block_size[par_row], hsize_t);
  } /* end if */
  else {
  ...
  if (H5F_USE_TMP_SPACE(hdr->f)) {
    if (HADDR_UNDEF ==
        (dblock_addr = H5MF_alloc_tmp(hdr->f, (hsize_t)dblock->size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "file allocation failed for fractal heap direct block");
  } /* end if */
  else {
    if (HADDR_UNDEF == (dblock_addr = H5MF_alloc(hdr->f, H5FD_MEM_FHEAP_DBLOCK,
                                                 (hsize_t)dblock->size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "file allocation failed for fractal heap direct block");
  } /* end else */
  ...
  if (dblock->parent) {
    if (H5HF__man_iblock_attach(dblock->parent, par_entry, dblock_addr) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTATTACH, FAIL,
                  "can't attach direct block to parent indirect block");
  ...
  if (NULL == (sec_node = H5HF__sect_single_new(
                   (dblock->block_off + H5HF_MAN_ABS_DIRECT_OVERHEAD(hdr)),
                   free_space, dblock->parent, dblock->par_entry)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't create section for new direct block's free space");

  /* Check what to do with section node */
  if (ret_sec_node)
    /* Pass back the pointer to the section instead of adding it to the free
     * list */
    *ret_sec_node = sec_node;
    ...
    if (H5HF__space_add(hdr, sec_node, 0) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                  "can't add direct block free space to global list");
  } /* end else */

  /* Cache the new fractal heap direct block */
  if (H5AC_insert_entry(hdr->f, H5AC_FHEAP_DBLOCK, dblock_addr, dblock,
                        H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't add fractal heap direct block to cache");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_destroy(H5HF_hdr_t *hdr, H5HF_direct_t *dblock,
  ...
  /* If the dblock is in real file space, also tell the cache to free its file
   * space */
  ...
  if (H5AC_unprotect(hdr->f, H5AC_FHEAP_DBLOCK, dblock_addr, dblock,
                     cache_flags) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_new(H5HF_hdr_t *hdr, size_t request,
                            H5HF_free_section_t **ret_sec_node) {
   ...
   */
    ...
    /* Update iterator to reflect any previous increments as well as allow for
     * requested direct block size
     */
    if (H5HF__hdr_update_iter(hdr, min_dblock_size) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTUPDATE, FAIL,
                  "unable to update block iterator");
    ...
    H5_CHECKED_ASSIGN(next_size, size_t,
                      hdr->man_dtable.row_block_size[next_row], hsize_t);

    /* Check for skipping over blocks */
    if (min_dblock_size > next_size) {
      fprintf(stderr,
              "%s: Skipping direct block sizes not supported, min_dblock_size "
              "= %zu, next_size = %zu\n",
              __func__, min_dblock_size, next_size);
      HGOTO_ERROR(H5E_HEAP, H5E_UNSUPPORTED, FAIL,
                  "skipping direct block sizes not supported yet");
    } /* end if */
    ...
    if (H5HF__man_dblock_create(hdr, iblock, next_entry, &dblock_addr,
                                ret_sec_node) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL,
                  "can't allocate fractal heap direct block");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
H5HF_direct_t *H5HF__man_dblock_protect(H5HF_hdr_t *hdr, haddr_t dblock_addr,
                                        size_t dblock_size,
  ...
  H5HF_dblock_cache_ud_t
      udata; /* parent and other info for deserializing direct block */
  ...
  if (NULL == (dblock = (H5HF_direct_t *)H5AC_protect(
                   hdr->f, H5AC_FHEAP_DBLOCK, dblock_addr, &udata, flags)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_locate(H5HF_hdr_t *hdr, hsize_t obj_off,
                               H5HF_indirect_t **ret_iblock,
                               unsigned *ret_entry, hbool_t *ret_did_protect,
                               unsigned flags) {
  ...
  assert(
      hdr->man_dtable
          .curr_root_rows); /* Only works for heaps with indirect root block */
  assert(ret_iblock);
  ...
  if (H5HF__dtable_lookup(&hdr->man_dtable, obj_off, &row, &col) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTCOMPUTE, FAIL,
                "can't compute row & column of object");
  ...
  if (NULL == (iblock = H5HF__man_iblock_protect(
                   hdr, iblock_addr, hdr->man_dtable.curr_root_rows, NULL, 0,
  ...
  while (row >= hdr->man_dtable.max_direct_rows) {
    ...
    hbool_t
        new_did_protect; /* Whether we protected the indirect block or not */
    unsigned nrows;      /* Number of rows in new indirect block */
    unsigned cache_flags =
        H5AC__NO_FLAGS_SET; /* Flags for unprotecting parent indirect block */

    /* Compute # of rows in child indirect block */
    nrows = (H5VM_log2_gen(hdr->man_dtable.row_block_size[row]) -
             hdr->man_dtable.first_row_bits) +
            1;
    ...
    if (!H5_addr_defined(iblock_addr)) {
      if (H5HF__man_iblock_create(hdr, iblock, entry, nrows, nrows,
                                  &iblock_addr) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL,
    ...
    if (NULL == (new_iblock = H5HF__man_iblock_protect(
                     hdr, iblock_addr, nrows, iblock, entry, FALSE, flags,
                     &new_did_protect)))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                  "unable to protect fractal heap indirect block");
    ...
    if (H5HF__dtable_lookup(&hdr->man_dtable, (obj_off - iblock->block_off),
                            &row, &col) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCOMPUTE, FAIL,
                  "can't compute row & column of object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_dblock_delete(H5F_t *f, haddr_t dblock_addr,
                               hsize_t dblock_size) {
  ...
  if (dblock_status & H5AC_ES__IN_CACHE) {
    ...
    if (H5AC_expunge_entry(f, H5AC_FHEAP_DBLOCK, dblock_addr,
                           H5AC__NO_FLAGS_SET) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTREMOVE, FAIL,
                  "unable to remove direct block from cache");
  } /* end if */
  ...
  if (!H5F_IS_TMP_ADDR(f, dblock_addr))
    ...
    if (H5MF_xfree(f, H5FD_MEM_FHEAP_DBLOCK, dblock_addr, dblock_size) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                  "unable to free fractal heap direct block file space");
