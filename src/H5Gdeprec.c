...
hid_t H5Gcreate1(hid_t loc_id, const char *name, size_t size_hint) {
  ...
  if (size_hint > UINT32_MAX)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "size_hint cannot be larger than UINT32_MAX");
  ...
  if (size_hint > 0) {
    ...
    if (NULL ==
        (gc_plist = (H5P_genplist_t *)H5I_object(H5P_GROUP_CREATE_DEFAULT)))
    ...
    if (H5P_get(gc_plist, H5G_CRT_GROUP_INFO_NAME, &ginfo) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, H5I_INVALID_HID,
                  "can't get group info");
  ...
  if (NULL == (grp = H5VL_group_create(
                   ...
                   tmp_gcpl, H5P_GROUP_ACCESS_DEFAULT, H5P_DATASET_XFER_DEFAULT,
                   H5_REQUEST_NULL)))
  ...
  if (H5I_INVALID_HID != tmp_gcpl && tmp_gcpl != H5P_GROUP_CREATE_DEFAULT)
    if (H5I_dec_ref(tmp_gcpl) < 0)
      HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, H5I_INVALID_HID,
                  "unable to release property list");
...
hid_t H5Gopen1(hid_t loc_id, const char *name) {
  ...
  /* Open the group */
  if (NULL == (grp = H5VL_group_open(
                   vol_obj, &loc_params, name, H5P_GROUP_ACCESS_DEFAULT,
...
herr_t H5Glink(hid_t cur_loc_id, H5G_link_t type, const char *cur_name,
  ...
  if (type == H5L_TYPE_HARD) {
    ...
    vol_cb_args.args.hard.curr_loc_params.loc_data.loc_by_name.lapl_id =
        H5P_LINK_ACCESS_DEFAULT;

    /* Create the link through the VOL */
    if (H5VL_link_create(&vol_cb_args, &tmp_vol_obj, &new_loc_params,
    ...
    /* Create the link through the VOL */
    if (H5VL_link_create(&vol_cb_args, vol_obj, &loc_params,
                         H5P_LINK_CREATE_DEFAULT, H5P_LINK_ACCESS_DEFAULT,
                         H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL, "unable to create link");
  } /* end else-if */
...
herr_t H5Glink2(hid_t cur_loc_id, const char *cur_name, H5G_link_t type,
  ...
  if (type == H5L_TYPE_HARD) {
    ...
    /* Create the link through the VOL */
    if (H5VL_link_create(&vol_cb_args, vol_obj2, &new_loc_params,
                         H5P_LINK_CREATE_DEFAULT, H5P_LINK_ACCESS_DEFAULT,
                         H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL, "unable to create link");
  } /* end if */
...
herr_t H5Gmove(hid_t src_loc_id, const char *src_name, const char *dst_name) {
  ...
  /* Move the link */
  if (H5VL_link_move(vol_obj, &loc_params1, NULL, &loc_params2,
                     H5P_LINK_CREATE_DEFAULT, H5P_LINK_ACCESS_DEFAULT,
                     H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
...
herr_t H5Gmove2(hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
  ...
  /* Move the link */
  if (H5VL_link_move(vol_obj1, &loc_params1, vol_obj2, &loc_params2,
                     H5P_LINK_CREATE_DEFAULT, H5P_LINK_ACCESS_DEFAULT,
                     H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
...
herr_t H5Gunlink(hid_t loc_id, const char *name) {
  ...
  /* Delete the link */
  if (H5VL_link_specific(vol_obj, &loc_params, &vol_cb_args,
                         H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
...
int H5Gget_comment(hid_t loc_id, const char *name, size_t bufsize,
  ...
  /* Set up collective metadata if appropriate */
  if (H5CX_set_loc(loc_id) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTSET, -1,
                "can't set collective metadata read info");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Giterate(hid_t loc_id, const char *name, int *idx_p, H5G_iterate_t op,
                  void *op_data) {
  H5VL_object_t *vol_obj;           /* Object of loc_id */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_group_optional_args_t
      grp_opt_args;     /* Arguments for optional operation */
  ...
  grp_opt_args.iterate_old.loc_params.loc_data.loc_by_name.lapl_id =
      H5P_LINK_ACCESS_DEFAULT;
  ...
  if ((ret_value =
           H5VL_group_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                               H5_REQUEST_NULL)) < 0)
...
herr_t H5Gget_num_objs(hid_t loc_id, hsize_t *num_objs /*out*/) {
  ...
  if (H5VL_setup_self_args(loc_id, &vol_obj,
...
herr_t H5Gget_objinfo(hid_t loc_id, const char *name, hbool_t follow_link,
  ...
  grp_opt_args.get_objinfo.loc_params.loc_data.loc_by_name.lapl_id =
      H5P_LINK_ACCESS_DEFAULT;
  ...
  if (H5VL_group_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                          H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL, "can't get info for object: '%s'",
                name);
...
static herr_t H5G__get_objinfo_cb(H5G_loc_t H5_ATTR_UNUSED *grp_loc /*in*/,
  ...
  if (udata->statbuf) {
    ...
    if (H5F_get_fileno((obj_loc ? obj_loc : grp_loc)->oloc->file,
                       &statbuf->fileno[0]) < 0)
    ...
    if (udata->follow_link || !lnk || (lnk->type == H5L_TYPE_HARD)) {
      ...
      assert(obj_loc);
      if (H5O_get_info(obj_loc->oloc, &dm_info,
                       H5O_INFO_BASIC | H5O_INFO_TIME) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                    "unable to get data model object info");
      if (H5O_get_native_info(obj_loc->oloc, &nat_info, H5O_INFO_HDR) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                    "unable to get native object info");

      /* Get mapped object type */
      statbuf->type = H5G_map_obj_type(dm_info.type);

      /* Get object number (i.e. address) for object */
      if (H5VL_native_token_to_addr(obj_loc->oloc->file, H5I_FILE,
                                    dm_info.token, &obj_addr) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__get_objinfo(const H5G_loc_t *loc, const char *name,
  ...
  if (H5G_traverse(loc, name,
                   (unsigned)(follow_link
                                  ? H5G_TARGET_NORMAL
                                  : (H5G_TARGET_SLINK | H5G_TARGET_UDLINK)),
 ...
 *              Otherwise `size' is ignored and the function does not store the
 *name, just returning the number of characters required to store the name. If
 *an error occurs then the buffer pointed to by `name' (NULL or non-NULL) is
 *unchanged and the function returns a negative value. If a zero is returned for
 *the name's length, then there is no name associated with the ID.
 *
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Gget_objname_by_idx(hid_t loc_id, hsize_t idx, char *name /*out*/,
                              size_t size) {
  H5VL_object_t *vol_obj;           /* Object of loc_id */
  H5VL_link_get_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_loc_params_t loc_params;
...
H5G_obj_t H5Gget_objtype_by_idx(hid_t loc_id, hsize_t idx) {
  ...
  /* Get the location object */
  if (NULL == (vol_obj = H5VL_vol_object(loc_id)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5G_UNKNOWN,
                "invalid location identifier");
  ...
  if (H5G_UNKNOWN == (ret_value = H5G_map_obj_type(oinfo.type)))
    HGOTO_ERROR(H5E_SYM, H5E_BADTYPE, H5G_UNKNOWN,
                "can't determine object type");
