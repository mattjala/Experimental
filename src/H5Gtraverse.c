...
static herr_t H5G__traverse_ud(const H5G_loc_t *grp_loc, const H5O_link_t *lnk,
                               H5G_loc_t *obj_loc /*in,out*/, unsigned target,
                               hbool_t *obj_exists);
static herr_t H5G__traverse_slink(const H5G_loc_t *grp_loc,
                                  const H5O_link_t *lnk,
                                  H5G_loc_t *obj_loc /*in,out*/,
                                  unsigned target, hbool_t *obj_exists);
static herr_t H5G__traverse_real(const H5G_loc_t *loc, const char *name,
                                 unsigned target, H5G_traverse_t op,
                                 void *op_data);
...
static herr_t H5G__traverse_slink_cb(H5G_loc_t H5_ATTR_UNUSED *grp_loc,
  ...
  H5G_trav_slink_t *udata =
      (H5G_trav_slink_t *)_udata; /* User data passed in */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__traverse_ud(const H5G_loc_t *grp_loc /*in,out*/,
                               const H5O_link_t *lnk,
                               H5G_loc_t *obj_loc /*in,out*/, unsigned target,
                               hbool_t *obj_exists) {
  ...
  if (NULL == (link_class = H5L_find_class(lnk->type)))
    HGOTO_ERROR(H5E_SYM, H5E_NOTREGISTERED, FAIL,
                "unable to get UD link class");
  ...
  if (link_class->version == H5L_LINK_CLASS_T_VERS_0)
    ...
    cb_return = (link_class->trav_func)(lnk->name, cur_grp, lnk->u.ud.udata,
                                        lnk->u.ud.size, H5CX_get_lapl(),
                                        H5CX_get_dxpl());
#else  /* H5_NO_DEPRECATED_SYMBOLS */
  cb_return =
      (link_class->trav_func)(lnk->name, cur_grp, lnk->u.ud.udata,
                              lnk->u.ud.size, H5CX_get_lapl(), H5CX_get_dxpl());
  ...
  if (cb_return < 0) {
    ...
    /* else, we really needed to open the object */
      ...
      HGOTO_ERROR(H5E_SYM, H5E_BADID, FAIL,
                  "traversal callback returned invalid ID");
  } /* end if */
  ...
  if (cur_grp > 0 && H5I_dec_ref(cur_grp) < 0)
    HDONE_ERROR(H5E_SYM, H5E_CANTRELEASE, FAIL,
                "unable to close ID for current location");
...
static herr_t H5G__traverse_slink(const H5G_loc_t *grp_loc,
  ...
  hbool_t tmp_obj_path_set =
      FALSE; /* Flag to indicate that tmp object path is initialized */
  ...
  hbool_t tmp_grp_loc_set =
      FALSE; /* Flag to indicate that tmp group location is initialized */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Traverse the link */
  if (H5G__traverse_real(&tmp_grp_loc, lnk->u.soft.name, target,
                         H5G__traverse_slink_cb, &udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__traverse_special(const H5G_loc_t *grp_loc, const H5O_link_t *lnk,
                             unsigned target, hbool_t last_comp,
  ...
  if (H5L_TYPE_SOFT == lnk->type &&
    ...
    if (H5CX_set_nlinks(nlinks) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_CANTSET, FAIL,
                  "can't update # of soft / UD links to traverse");

    /* Traverse soft link */
    if (H5G__traverse_slink(grp_loc, lnk, obj_loc, (target & H5G_TARGET_EXISTS),
                            obj_exists) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_TRAVERSE, FAIL,
  ...
  if (lnk->type >= H5L_TYPE_UD_MIN &&
    ...
    if (H5G__traverse_ud(grp_loc, lnk, obj_loc, (target & H5G_TARGET_EXISTS),
                         obj_exists) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_TRAVERSE, FAIL,
                  "user-defined link traversal failed");
  } /* end if */
   ...
   *  statement here. -QAK)
   */
  if (H5_addr_defined(obj_loc->oloc->addr) &&
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__traverse_real(const H5G_loc_t *_loc, const char *name,
                                 unsigned target, H5G_traverse_t op,
                                 void *op_data) {
  ...
  hbool_t link_valid =
      FALSE; /* Flag to indicate that the link information is valid */
  hbool_t obj_loc_valid =
      FALSE; /* Flag to indicate that the object location is valid */
  H5G_own_loc_t own_loc = H5G_OWN_NONE; /* Enum to indicate whether callback
                                           took ownership of locations*/
  hbool_t group_copy =
      FALSE;           /* Flag to indicate that the group entry is copied */
  char comp_buf[1024]; /* Temporary buffer for path components */
  char *comp;          /* Pointer to buffer for path components */
  H5WB_t *wb = NULL;   /* Wrapped buffer for temporary buffer */
  hbool_t last_comp = FALSE;  /* Flag to indicate that a component is the last
  ...
  while ((name = H5G__component(name, &nchars)) && *name) {
    ...
    /* If the lookup was OK, build object location and traverse special links,
     * etc. */
    if (lookup_status) {
      ...
      if (H5G__link_to_loc(&grp_loc, &lnk, &obj_loc) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL,
                    "cannot initialize object location");
      obj_loc_valid = TRUE;
      ...
      if (H5G__traverse_special(&grp_loc, &lnk, target, last_comp, &obj_loc,
                                &obj_exists) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_TRAVERSE, FAIL,
                    "special link traversal failed");
    } /* end if */
    ...
    if (!lookup_status) {
      ...
      if (target & H5G_CRT_INTMD_GROUP) {
        const H5O_ginfo_t def_ginfo =
            H5G_CRT_GROUP_INFO_DEF; /* Default group info settings */
        const H5O_linfo_t def_linfo =
            H5G_CRT_LINK_INFO_DEF; /* Default link info settings */
        const H5O_pline_t def_pline =
            H5O_CRT_PIPELINE_DEF; /* Default filter pipeline settings */
        ...
        if (exists) {
          ...
          if (NULL == H5O_msg_read(grp_loc.oloc, H5O_GINFO_ID, &par_ginfo))
            HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                        "group info message not present");
        ...
        if (exists) {
          ...
          if (NULL == H5O_msg_read(grp_loc.oloc, H5O_PLINE_ID, &par_pline))
            HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                        "filter pipeline message not present");
        ...
        /* XXX: Should we allow user to control the group creation params here?
         * -QAK */
        ...
        if (H5G__obj_create_real(grp_oloc.file, ginfo, linfo, pline, &gcrt_info,
                                 obj_loc.oloc /*out*/) < 0)
          HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL,
                      "unable to create group entry");

        /* Insert new group into current group's symbol table */
        if (H5G__loc_insert(&grp_loc, comp, &obj_loc, H5O_TYPE_GROUP,
                            &gcrt_info) < 0)
          HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                      "unable to insert intermediate group");
        ...
        if (ginfo != &def_ginfo)
          if (H5O_msg_reset(H5O_GINFO_ID, (void *)ginfo) < 0)
            HGOTO_ERROR(H5E_SYM, H5E_CANTRELEASE, FAIL,
                        "unable to reset group info message");
        if (linfo != &def_linfo)
          if (H5O_msg_reset(H5O_LINFO_ID, (void *)linfo) < 0)
            HGOTO_ERROR(H5E_SYM, H5E_CANTRELEASE, FAIL,
                        "unable to reset link info message");
        if (pline != &def_pline)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_traverse(const H5G_loc_t *loc, const char *name, unsigned target,
                    H5G_traverse_t op, void *op_data) {
  size_t orig_nlinks;         /* Original value for # of soft / UD links able to
                                 traverse */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5G__traverse_real(loc, name, target, op, op_data) < 0)
    HGOTO_ERROR_TAG(H5E_SYM, H5E_NOTFOUND, FAIL,
