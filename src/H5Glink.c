...
static int H5G__link_cmp_name_inc(const void *lnk1, const void *lnk2) {
  ...
  FUNC_LEAVE_NOAPI(HDstrcmp(((const H5O_link_t *)lnk1)->name,
                            ((const H5O_link_t *)lnk2)->name))
...
static int H5G__link_cmp_name_dec(const void *lnk1, const void *lnk2) {
  ...
  FUNC_LEAVE_NOAPI(HDstrcmp(((const H5O_link_t *)lnk2)->name,
                            ((const H5O_link_t *)lnk1)->name))
...
static int H5G__link_cmp_corder_inc(const void *lnk1, const void *lnk2) {
  ...
  if (((const H5O_link_t *)lnk1)->corder < ((const H5O_link_t *)lnk2)->corder)
    ret_value = -1;
  else if (((const H5O_link_t *)lnk1)->corder >
           ((const H5O_link_t *)lnk2)->corder)
    ret_value = 1;
  else
    ret_value = 0;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__ent_to_link(H5O_link_t *lnk, const H5HL_t *heap,
  ...
  if (ent->type == H5G_CACHED_SLINK) {
    ...
    if ((s = (const char *)H5HL_offset_into(
             heap, ent->cache.slink.lval_offset)) == NULL)
      HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                  "unable to get symbolic link name");

    /* Copy the link value */
    if ((lnk->u.soft.name = H5MM_xstrdup(s)) == NULL)
      HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                  "unable to duplicate symbolic link name");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_link_to_info(const H5O_loc_t *link_loc, const H5O_link_t *lnk,
                        H5L_info2_t *info) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (info) {
    ...
    case H5L_TYPE_HARD:
      ...
      if (H5VL_native_addr_to_token(link_loc->file, H5I_FILE, lnk->u.hard.addr,
                                    &info->u.token) < 0)
    ...
    case H5L_TYPE_SOFT:
      info->u.val_size =
          HDstrlen(lnk->u.soft.name) + 1; /*count the null terminator*/
      break;
    ...
    default: {
      ...
      if (link_class != NULL && link_class->query_func != NULL) {
        ...
        /* Call the link's query routine to retrieve the user-defined link's
         * value size */
        /* (in case the query routine packs/unpacks the link value in some way
         * that changes its size) */
        if ((cb_ret =
                 (link_class->query_func)(lnk->name, lnk->u.ud.udata,
                                          lnk->u.ud.size, NULL, (size_t)0)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__link_to_loc(const H5G_loc_t *grp_loc, const H5O_link_t *lnk,
                        H5G_loc_t *obj_loc) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__link_sort_table(H5G_link_table_t *ltable, H5_index_t idx_type,
  ...
  if (idx_type == H5_INDEX_NAME) {
    if (order == H5_ITER_INC)
      qsort(ltable->lnks, ltable->nlinks, sizeof(H5O_link_t),
            H5G__link_cmp_name_inc);
    else if (order == H5_ITER_DEC)
      qsort(ltable->lnks, ltable->nlinks, sizeof(H5O_link_t),
            H5G__link_cmp_name_dec);
    else
    ...
    if (order == H5_ITER_INC)
      qsort(ltable->lnks, ltable->nlinks, sizeof(H5O_link_t),
            H5G__link_cmp_corder_inc);
    else if (order == H5_ITER_DEC)
      qsort(ltable->lnks, ltable->nlinks, sizeof(H5O_link_t),
            H5G__link_cmp_corder_dec);
    else
      assert(order == H5_ITER_NATIVE);
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__link_iterate_table(const H5G_link_table_t *ltable, hsize_t skip,
                               hsize_t *last_lnk, const H5G_lib_iterate_t op,
                               void *op_data) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__link_name_replace(H5F_t *file, H5RS_str_t *grp_full_path_r,
  ...
  if (grp_full_path_r) {
    ...
    if (H5G_name_replace(lnk, H5G_NAME_DELETE, file, obj_path_r, NULL, NULL) <
        0)
