...
typedef struct H5ES_get_requests_ctx_t {
  hid_t *connector_ids; /* Output buffer for list of connector IDs that match
                           the above requests */
...
typedef struct H5ES_wait_ctx_t {
  ...
  size_t
      *num_in_progress; /* Count of # of operations that have not completed */
...
typedef struct H5ES_gei_ctx_t {
  ...
  H5ES_err_info_t
      *curr_err_info; /* Pointer to current element in err_info[] array */
} H5ES_gei_ctx_t;
...
static herr_t H5ES__insert(H5ES_t *es, H5VL_t *connector, void *request_token,
                           const char *app_file, const char *app_func,
                           unsigned app_line, const char *caller,
                           const char *api_args);
static int H5ES__get_requests_cb(H5ES_event_t *ev, void *_ctx);
static herr_t H5ES__handle_fail(H5ES_t *es, H5ES_event_t *ev);
static herr_t H5ES__op_complete(H5ES_t *es, H5ES_event_t *ev,
                                H5VL_request_status_t ev_status);
...
herr_t H5ES_init(void) {
  ...
  if (H5I_register_type(H5I_EVENTSET_CLS) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTINIT, FAIL,
...
static herr_t H5ES__close_cb(void *_es, void H5_ATTR_UNUSED **rt) {
  ...
  if (H5ES__close(es) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CLOSEERROR, FAIL,
                "unable to close event set");
...
H5ES_t *H5ES__create(void) {
  ...
  if (NULL == (es = H5FL_CALLOC(H5ES_t)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTALLOC, NULL,
                "can't allocate event set object");
  ...
  if (!ret_value)
    if (es && H5ES__close(es) < 0)
      HDONE_ERROR(H5E_EVENTSET, H5E_CANTRELEASE, NULL,
                  "unable to free event set");
...
static herr_t H5ES__insert(H5ES_t *es, H5VL_t *connector, void *request_token,
  ...
  hbool_t ev_inserted =
      FALSE; /* Flag to indicate that event is in active list */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (NULL == (ev = H5ES__event_new(connector, request_token)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTCREATE, FAIL,
                "can't create event object");

  /* Copy the app source information */
  /* The 'app_func' & 'app_file' strings are statically allocated (by the
  ...
  if (api_args && NULL == (ev->op_info.api_args = H5MM_xstrdup(api_args)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTALLOC, FAIL,
                "can't copy API routine arguments");
  ...
  if (es->ins_func)
    if ((es->ins_func)(&ev->op_info, es->ins_ctx) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CALLBACK, FAIL,
                  "'insert' callback for event set failed");
  ...
  if (ret_value < 0)
    if (ev) {
      ...
      if (H5ES__event_free(ev) < 0)
        HDONE_ERROR(H5E_EVENTSET, H5E_CANTRELEASE, FAIL,
                    "unable to release event");
    }
...
herr_t H5ES_insert(hid_t es_id, H5VL_t *connector, void *token,
  ...
  H5RS_str_t *rs =
      NULL; /* Ref-counted string to compose formatted argument string in */
  ...
  if (es->err_occurred)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTINSERT, FAIL,
                "event set has failed operations");
  ...
  if (NULL == (rs = H5RS_create(NULL)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTALLOC, FAIL,
                "can't allocate ref-counted string");

  /* Copy the string for the API routine's arguments */
  /* (skip the six characters from the app's file, function and line #
   * arguments) */
  assert(0 == HDstrncmp(caller_args, "*s*sIu", 6));
  if (H5_trace_args(rs, caller_args + 6, ap) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTSET, FAIL,
                "can't create formatted API arguments");
  if (NULL == (api_args = H5RS_get_str(rs)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTGET, FAIL,
                "can't get pointer to formatted API arguments");

  /* Insert the operation into the event set */
  if (H5ES__insert(es, connector, token, app_file, app_func, app_line, caller,
                   api_args) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTINSERT, FAIL,
                "event set has failed operations");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ES__insert_request(H5ES_t *es, H5VL_t *connector, void *token) {
  herr_t ret_value = SUCCEED; /* Return value */
...
static int H5ES__get_requests_cb(H5ES_event_t *ev, void *_ctx) {
  H5ES_get_requests_ctx_t *ctx =
      (H5ES_get_requests_ctx_t *)_ctx; /* Callback context */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ES__get_requests(H5ES_t *es, H5_iter_order_t order,
...
static herr_t H5ES__op_complete(H5ES_t *es, H5ES_event_t *ev,
  ...
  assert(es);
  assert(ev);
  assert(H5VL_REQUEST_STATUS_SUCCEED == ev_status ||
         H5VL_REQUEST_STATUS_FAIL == ev_status ||
  ...
  if (H5VL_REQUEST_STATUS_SUCCEED == ev_status ||
    ...
    if (es->comp_func) {
      ...
      if ((es->comp_func)(&ev->op_info, op_status, H5I_INVALID_HID,
                          es->comp_ctx) < 0)
        HGOTO_ERROR(H5E_EVENTSET, H5E_CALLBACK, FAIL,
                    "'complete' callback for event set failed");
    } /* end if */

    /* Event success or cancellation */
    if (H5ES__event_completed(ev, &es->active) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTRELEASE, FAIL,
                  "unable to release completed event");
  } /* end if */
    ...
    if (es->comp_func) {
      ...
      if (H5VL_request_specific(ev->request, &vol_cb_args) < 0)
        HGOTO_ERROR(H5E_EVENTSET, H5E_CANTGET, FAIL,
                    "unable to retrieve error stack for operation");

      /* Set values */
      err_stack_id = vol_cb_args.args.get_err_stack.err_stack_id;

      if ((es->comp_func)(&ev->op_info, H5ES_STATUS_FAIL, err_stack_id,
                          es->comp_ctx) < 0)
        HGOTO_ERROR(H5E_EVENTSET, H5E_CALLBACK, FAIL,
                    "'complete' callback for event set failed");
    } /* end if */
...
static int H5ES__wait_cb(H5ES_event_t *ev, void *_ctx) {
  ...
  H5VL_request_status_t ev_status =
      H5VL_REQUEST_STATUS_SUCCEED; /* Status from event's operation */
  uint64_t start_time = 0,
           elapsed_time =
               0; /* Start and elapsed times for waiting on an operation */
  int ret_value = H5_ITER_CONT; /* Return value */
  ...
  if (H5VL_request_wait(ev->request, ctx->timeout, &ev_status) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTWAIT, H5_ITER_ERROR,
                "unable to test operation");
  if (ctx->timeout != H5ES_WAIT_NONE && ctx->timeout != H5ES_WAIT_FOREVER)
  ...
  if (ev_status == H5VL_REQUEST_STATUS_FAIL) {
    ...
    if (H5ES__op_complete(ctx->es, ev, ev_status) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTRELEASE, H5_ITER_ERROR,
                  "unable to release completed event");
    ...
    *ctx->op_failed = TRUE;
    ...
    ret_value = H5_ITER_STOP;
           ...
           ev_status == H5VL_REQUEST_STATUS_CANCELED) {
    /* Handle event completion */
    if (H5ES__op_complete(ctx->es, ev, ev_status) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTRELEASE, H5_ITER_ERROR,
                  "unable to release completed event");
    ...
    /* Should never get a status of 'can't cancel' back from test / wait
     * operation */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ES__wait(H5ES_t *es, uint64_t timeout, size_t *num_in_progress,
                  hbool_t *op_failed) {
...
static int H5ES__cancel_cb(H5ES_event_t *ev, void *_ctx) {
  ...
  if (H5VL_request_cancel(ev->request, &ev_status) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTCANCEL, H5_ITER_ERROR,
  ...
  if (ev_status == H5VL_REQUEST_STATUS_FAIL) {
    /* Handle event completion */
    if (H5ES__op_complete(ctx->es, ev, ev_status) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTSET, H5_ITER_ERROR,
                  "unable to handle failed event");
    ...
    *ctx->op_failed = TRUE;
    ...
    if (H5ES__op_complete(ctx->es, ev, ev_status) < 0)
      HGOTO_ERROR(H5E_EVENTSET, H5E_CANTRELEASE, H5_ITER_ERROR,
           ...
           ev_status == H5VL_REQUEST_STATUS_IN_PROGRESS) {
    /* Increment "not canceled" counter */
    (*ctx->num_not_canceled)++;
  } /* end else-if */
  else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ES__cancel(H5ES_t *es, size_t *num_not_canceled, hbool_t *op_failed) {
  ...
  *op_failed = FALSE;
  ...
  if (H5ES__list_iterate(&es->active, H5_ITER_NATIVE, H5ES__cancel_cb, &ctx) <
      0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_BADITER, FAIL, "iteration failed");
...
static int H5ES__get_err_info_cb(H5ES_event_t *ev, void *_ctx) {
  ...
  /* The 'app_func_name', 'app_file_name', and 'api_name' strings are statically
   * allocated (by the compiler) so there's no need to duplicate them
   * internally, but they are duplicated here, when they are given back to the
   * user.
   */
  if (NULL ==
      (ctx->curr_err_info->api_name = H5MM_xstrdup(ev->op_info.api_name)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTALLOC, H5_ITER_ERROR,
                "can't copy HDF5 API routine name");
  if (NULL ==
      (ctx->curr_err_info->api_args = H5MM_xstrdup(ev->op_info.api_args)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTALLOC, H5_ITER_ERROR,
                "can't copy HDF5 API routine arguments");
  if (NULL == (ctx->curr_err_info->app_file_name =
                   H5MM_xstrdup(ev->op_info.app_file_name)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTALLOC, H5_ITER_ERROR,
                "can't copy HDF5 application file name");
  if (NULL == (ctx->curr_err_info->app_func_name =
                   H5MM_xstrdup(ev->op_info.app_func_name)))
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTALLOC, H5_ITER_ERROR,
                "can't copy HDF5 application function name");
  ...
  if (H5VL_request_specific(ev->request, &vol_cb_args) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTGET, H5_ITER_ERROR,
                "unable to retrieve error stack for operation");

  /* Set value */
  ctx->curr_err_info->err_stack_id =
      vol_cb_args.args.get_err_stack.err_stack_id;
  ...
  if (H5ES__event_free(ev) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTRELEASE, H5_ITER_ERROR,
                "unable to release failed event");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5ES__get_err_info(H5ES_t *es, size_t num_err_info,
                          H5ES_err_info_t err_info[], size_t *num_cleared) {
  ...
  if (H5ES__list_iterate(&es->failed, H5_ITER_NATIVE, H5ES__get_err_info_cb,
                         &ctx) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_BADITER, FAIL, "iteration failed");
...
herr_t H5ES__close(H5ES_t *es) {
  ...
  if (H5ES__list_count(&es->active) > 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_CANTCLOSEOBJ, FAIL,
                ...
                "(i.e. wait on event set first)");

  /* Iterate over the failed events in the set, releasing them */
  if (H5ES__list_iterate(&es->failed, H5_ITER_NATIVE, H5ES__close_failed_cb,
                         (void *)es) < 0)
    HGOTO_ERROR(H5E_EVENTSET, H5E_BADITER, FAIL, "iteration failed");
