...
static herr_t H5O__refresh_metadata_close(H5O_loc_t *oloc, H5G_loc_t *obj_loc,
                                          hid_t oid);
...
herr_t H5O_flush(H5O_loc_t *oloc, hid_t obj_id) {
  ...
  if (H5O_flush_common(oloc, obj_id) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFLUSH, FAIL,
                "unable to flush object and object flush callback");
...
herr_t H5O_flush_common(H5O_loc_t *oloc, hid_t obj_id) {
  ...
  if (H5O__oh_tag(oloc, &tag) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFLUSH, FAIL,
                "unable to flush object metadata");
  ...
  if (H5F_object_flush_cb(oloc->file, obj_id) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFLUSH, FAIL,
                "unable to do object flush callback");
...
static herr_t H5O__oh_tag(const H5O_loc_t *oloc, haddr_t *tag) {
  ...
  if (NULL == (oh = H5O_protect(oloc, H5AC__READ_ONLY_FLAG, FALSE)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                "unable to protect object's object header");
  ...
  if (HADDR_UNDEF == (*tag = H5O_OH_GET_ADDR(oh)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "unable to get address of object header");
 ...
 *	 	H5Fstart_swmr_write() no longer calls the 1st routine.
 *(12/24/15)
 *
 * Return:    	Non-negative on success, negative on failure
 *
...
herr_t H5O_refresh_metadata(H5O_loc_t *oloc, hid_t oid) {
  ...
  hbool_t objs_incr =
      FALSE; /* Whether the object count in the file was incremented */
  H5F_t *file = NULL;
  herr_t ret_value = SUCCEED; /* Return value */

  FUNC_ENTER_NOAPI(FAIL)

  /* If the file is opened with write access, no need to perform refresh
   * actions. */
  if (!(H5F_INTENT(oloc->file) & H5F_ACC_RDWR)) {
    ...
    if (H5O_refresh_metadata_reopen(oid, H5P_DEFAULT, &obj_loc, connector,
                                    FALSE) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, FAIL, "unable to refresh object");
    ...
    if (H5I_get_type(oid) == H5I_DATATYPE)
      if (H5T_restore_refresh_state(oid, &cached_H5O_shared) < 0)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTOPENOBJ, FAIL,
                    "unable to restore datatype state");
 ...
 *
 * Purpose:     This is the first part of the original routine
 *H5O_refresh_metadata(). (1) Save object location information. (2) Handle
 *multiple dataset opens (3) Get object cork status (4) Close the object (5)
 *Flush and evict object metadata (6) Re-cork the object if needed
 *
 * Return:  Success:    Non-negative
 *          Failure:    Negative
 *
  ...
  if (H5I_get_type(oid) == H5I_DATASET)
    if (H5D_mult_refresh_close(oid) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, FAIL,
                  "unable to prepare refresh for dataset");

  /* Retrieve tag for object */
  if (H5O__oh_tag(oloc, &tag) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFLUSH, FAIL,
                "unable to get object header address");
 ...
 *
 * Purpose:     This is the second part of the original routine
 *H5O_refresh_metadata(). (1) Re-open object with the saved object location
 *information. (2) Re-register object ID with the re-opened object.
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
herr_t H5O_refresh_metadata_reopen(hid_t oid, hid_t apl_id, H5G_loc_t *obj_loc,
  ...
  case H5I_DATASET:
    ...
    /* Re-open the dataset */
    if (NULL == (object = H5D_open(obj_loc, apl_id == H5P_DEFAULT
                                                ? H5P_DATASET_ACCESS_DEFAULT
                                                : apl_id)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL, "unable to open dataset");
    if (!start_swmr) /* No need to handle multiple opens when
                        H5Fstart_swmr_write() */
      if (H5D_mult_refresh_reopen((H5D_t *)object) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, FAIL,
                    "unable to finish refresh for dataset");
    break;
  ...
  if ((H5VL_register_using_existing_id(type, object, vol_connector, TRUE,
                                       oid)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTREGISTER, FAIL,
                "unable to re-register object ID after refresh");
