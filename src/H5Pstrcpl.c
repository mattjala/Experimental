...
static herr_t H5P__strcrt_char_encoding_enc(const void *value, void **_pp,
                                            size_t *size);
...
static herr_t H5P__strcrt_reg_prop(H5P_genclass_t *pclass) {
  ...
  if (H5P__register_real(
          pclass, H5P_STRCRT_CHAR_ENCODING_NAME, H5P_STRCRT_CHAR_ENCODING_SIZE,
          &H5P_def_char_encoding_g, NULL, NULL, NULL,
          H5P_STRCRT_CHAR_ENCODING_ENC, H5P_STRCRT_CHAR_ENCODING_DEC, NULL,
          NULL, NULL, NULL) < 0)
...
herr_t H5Pset_char_encoding(hid_t plist_id, H5T_cset_t encoding) {
  ...
  if (encoding <= H5T_CSET_ERROR || encoding >= H5T_NCSET)
    HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL,
                "character encoding is not valid");
...
static herr_t H5P__strcrt_char_encoding_enc(const void *value, void **_pp,
                                            size_t *size) {
  const H5T_cset_t *encoding =
      (const H5T_cset_t *)value; /* Create local alias for values */
