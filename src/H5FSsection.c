...
/* User data for skip list iterator callback for iterating over section size
 * nodes */
typedef struct {
...
/* Local Prototypes */
/********************/
static herr_t H5FS__sect_increase(H5FS_t *fspace,
                                  const H5FS_section_class_t *cls,
                                  unsigned flags);
static herr_t H5FS__sect_decrease(H5FS_t *fspace,
                                  const H5FS_section_class_t *cls);
static herr_t H5FS__size_node_decr(H5FS_sinfo_t *sinfo, unsigned bin,
                                   H5FS_node_t *fspace_node,
                                   const H5FS_section_class_t *cls);
static herr_t H5FS__sect_unlink_size(H5FS_sinfo_t *sinfo,
                                     const H5FS_section_class_t *cls,
...
static herr_t H5FS__sect_link_size(H5FS_sinfo_t *sinfo,
                                   const H5FS_section_class_t *cls,
                                   H5FS_section_info_t *sect);
static herr_t H5FS__sect_link_rest(H5FS_t *fspace,
                                   const H5FS_section_class_t *cls,
...
static herr_t H5FS__sect_merge(H5FS_t *fspace, H5FS_section_info_t **sect,
                               void *op_data);
static htri_t H5FS__sect_find_node(H5FS_t *fspace, hsize_t request,
...
H5FS_sinfo_t *H5FS__sinfo_new(H5F_t *f, H5FS_t *fspace) {
  ...
  if (H5FS__incr(fspace) < 0)
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTINC, NULL,
                "unable to increment ref. count on free space header");
  sinfo->fspace = fspace;
  ...
  if (fspace->sinfo) {
    ...
    if (fspace->sinfo_protected && accmode != fspace->sinfo_accmode) {
      ...
      if (0 == (accmode & (unsigned)(~H5AC__READ_ONLY_FLAG))) {
        ...
        if (H5AC_unprotect(f, H5AC_FSPACE_SINFO, fspace->sect_addr,
        ...
        if (NULL == (fspace->sinfo = (H5FS_sinfo_t *)H5AC_protect(
                         f, H5AC_FSPACE_SINFO, fspace->sect_addr, &cache_udata,
                         H5AC__NO_FLAGS_SET)))
          HGOTO_ERROR(H5E_FSPACE, H5E_CANTPROTECT, FAIL,
                      "unable to load free space sections");
    ...
    if (H5_addr_defined(fspace->sect_addr)) {
      ...
      cache_udata.fspace = fspace;
      if (NULL ==
          (fspace->sinfo = (H5FS_sinfo_t *)H5AC_protect(
               f, H5AC_FSPACE_SINFO, fspace->sect_addr, &cache_udata, accmode)))
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTPROTECT, FAIL,
                    "unable to load free space sections");
 ...
 *
 *              Add the fix in this routine to resolve the potential infinite
 *loop problem when allocating file space for the meta data of the
 ...
 *              to shrink:
 *              --if the current allocated section info size in
 *fspace->sect_size is larger than the previous section info size in
 *fpsace->alloc_sect_size, release the section info
 *              --Otherwise, set the fspace->sect_size to be the same as
 *                fpsace->alloc_sect_size.  This means fspace->sect_size may be
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__sinfo_unlock(H5F_t *f, H5FS_t *fspace, hbool_t modified) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  fprintf(stderr,
          "%s: fspace->sinfo_lock_count = %u, fspace->sinfo_modified = %d, "
          "fspace->sinfo_protected = %d\n",
          __func__, fspace->sinfo_lock_count, fspace->sinfo_modified,
  ...
  if (modified) {
    ...
    if (fspace->sinfo_protected &&
        (0 != ((fspace->sinfo_accmode) & H5AC__READ_ONLY_FLAG)))
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTDIRTY, FAIL,
                  "attempt to modify read-only section info");
  ...
  if (fspace->sinfo_lock_count == 0) {
    hbool_t release_sinfo_space = FALSE; /* Flag to indicate section info space
                                            in file should be released */
    hbool_t closing_or_flushing =
        f->shared->closing; /* Is closing or flushing in progress */

    /* Check whether cache-flush is in progress if closing is not. */
    if (!closing_or_flushing && H5AC_get_cache_flush_in_progress(
                                    f->shared->cache, &closing_or_flushing) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL, "Can't get flush_in_progress");

    /* Check if we actually protected the section info */
    if (fspace->sinfo_protected) {
      unsigned cache_flags =
          H5AC__NO_FLAGS_SET; /* Flags for unprotecting heap */
      ...
      if (fspace->sinfo_modified) {
        ...
        /* On file close or flushing, does not allow section info to shrink in
         * size */
      ...
      if (fspace->sinfo_modified) {
        ...
        if (H5_addr_defined(fspace->sect_addr)) {
          /* Set flag to release section info space in file */
          /* On file close or flushing, only need to release section info with
             size bigger than previous section */
          if (closing_or_flushing) {
            if (fspace->sect_size > fspace->alloc_sect_size)
              release_sinfo_space = TRUE;
            else
    ...
    if (release_sinfo_space) {
      haddr_t old_sect_addr =
          fspace->sect_addr; /* Previous location of section info in file */
      ...
      if (!H5F_IS_TMP_ADDR(f, old_sect_addr))
        if (H5MF_xfree(f, H5FD_MEM_FSPACE_SINFO, old_sect_addr,
                       old_alloc_sect_size) < 0)
          HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL,
                      "unable to free free space sections");
    } /* end if */
  }   /* end if */
...
static herr_t H5FS__sect_serialize_size(H5FS_t *fspace) {
  ...
  if (fspace->serial_sect_count > 0) {
    ...
    sect_buf_size += fspace->sinfo->serial_size_count *
                     H5VM_limit_enc_size((uint64_t)fspace->serial_sect_count);

    /* Size for each differently sized serializable section */
    sect_buf_size +=
        fspace->sinfo->serial_size_count * fspace->sinfo->sect_len_size;
...
static herr_t H5FS__size_node_decr(H5FS_sinfo_t *sinfo, unsigned bin,
  ...
  if (cls->flags & H5FS_CLS_GHOST_OBJ) {
    ...
    /* If the node has no more ghost sections, decrement number of ghost section
     * sizes managed */
    ...
    /* If the node has no more serializable sections, decrement number of
     * serializable section sizes managed */
  ...
  if (H5SL_count(fspace_node->sect_list) == 0) {
    ...
    tmp_fspace_node = (H5FS_node_t *)H5SL_remove(sinfo->bins[bin].bin_list,
                                                 &fspace_node->sect_size);
    ...
    if (H5SL_close(fspace_node->sect_list) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTCLOSEOBJ, FAIL,
                  "can't destroy size tracking node's skip list");
...
static herr_t H5FS__sect_unlink_size(H5FS_sinfo_t *sinfo,
  ...
  if ((fspace_node = (H5FS_node_t *)H5SL_search(sinfo->bins[bin].bin_list,
                                                &sect->size)) == NULL)
    HGOTO_ERROR(H5E_FSPACE, H5E_NOTFOUND, FAIL, "can't find section size node");

  /* Remove the section's node from the list */
  tmp_sect_node =
      (H5FS_section_info_t *)H5SL_remove(fspace_node->sect_list, &sect->addr);
  if (tmp_sect_node == NULL || tmp_sect_node != sect)
    HGOTO_ERROR(H5E_FSPACE, H5E_NOTFOUND, FAIL,
                "can't find section node on size list");
...
static herr_t H5FS__sect_unlink_rest(H5FS_t *fspace,
  ...
  if (!(cls->flags & H5FS_CLS_SEPAR_OBJ)) {
    ...
    tmp_sect_node = (H5FS_section_info_t *)H5SL_remove(
        fspace->sinfo->merge_list, &sect->addr);
    if (tmp_sect_node == NULL || tmp_sect_node != sect)
      HGOTO_ERROR(H5E_FSPACE, H5E_NOTFOUND, FAIL,
                  "can't find section node on size list");
  } /* end if */

  /* Update section info & check if we need less room for the serialized free
   * space sections */
  if (H5FS__sect_decrease(fspace, cls) < 0)
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTINSERT, FAIL,
                "can't increase free space section size on disk");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__sect_remove_real(H5FS_t *fspace,
                                     H5FS_section_info_t *sect) {
...
herr_t H5FS_sect_remove(H5F_t *f, H5FS_t *fspace, H5FS_section_info_t *sect) {
  ...
  if (sinfo_valid && H5FS__sinfo_unlock(f, fspace, TRUE) < 0)
    HDONE_ERROR(H5E_FSPACE, H5E_CANTRELEASE, FAIL,
                "can't release section info");
...
static herr_t H5FS__sect_link_size(H5FS_sinfo_t *sinfo,
                                   ...
                                   H5FS_section_info_t *sect) {
  H5FS_node_t *fspace_node =
      NULL; /* Pointer to free space node of the correct size */
  hbool_t fspace_node_alloc =
      FALSE;                  /* Whether the free space node was allocated */
  unsigned bin;               /* Bin to put the free space section in */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (sinfo->bins[bin].bin_list == NULL) {
    if (NULL ==
        (sinfo->bins[bin].bin_list = H5SL_create(H5SL_TYPE_HSIZE, NULL)))
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTCREATE, FAIL,
                  "can't create skip list for free space nodes");
  } /* end if */
  else
    /* Check for node list of the correct size already */
    fspace_node =
        (H5FS_node_t *)H5SL_search(sinfo->bins[bin].bin_list, &sect->size);
  ...
  if (fspace_node == NULL) {
    ...
    if (NULL == (fspace_node = H5FL_MALLOC(H5FS_node_t)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "memory allocation failed for free space node");
    ...
    if (H5SL_insert(sinfo->bins[bin].bin_list, fspace_node,
                    &fspace_node->sect_size) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTINSERT, FAIL,
                  "can't insert free space node into skip list");
    fspace_node_alloc = FALSE; /* (owned by the bin skip list now, don't need to
                                  free on error) */
...
static herr_t H5FS__sect_link_rest(H5FS_t *fspace,
  ...
  if (!(cls->flags & H5FS_CLS_SEPAR_OBJ)) {
    if (fspace->sinfo->merge_list == NULL)
      if (NULL ==
          (fspace->sinfo->merge_list = H5SL_create(H5SL_TYPE_HADDR, NULL)))
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTCREATE, FAIL,
  ...
  /* Update section info & check if we need more room for the serialized free
   * space sections */
...
static herr_t H5FS__sect_link(H5FS_t *fspace, H5FS_section_info_t *sect,
  ...
  if (H5FS__sect_link_size(fspace->sinfo, cls, sect) < 0)
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTINSERT, FAIL,
                "can't add section to size tracking data structures");
...
static herr_t H5FS__sect_merge(H5FS_t *fspace, H5FS_section_info_t **sect,
  ...
  hbool_t remove_sect =
      FALSE;     /* Whether a section should be removed before shrinking */
  ...
  if (fspace->sinfo->merge_list) {
    do {
      H5SL_node_t *
          less_sect_node; /* Skip list node for section less than new section */
      H5SL_node_t *greater_sect_node =
          NULL; /* Skip list node for section greater than new section */
      H5FS_section_info_t *tmp_sect;      /* Temporary free space section */
      H5FS_section_class_t *tmp_sect_cls; /* Temporary section's class */
      hbool_t greater_sect_node_valid =
          FALSE; /* Indicate if 'greater than' section node is valid */
      ...
      if (less_sect_node) {
         ...
         */
        if ((!(tmp_sect_cls->flags & H5FS_CLS_MERGE_SYM) ||
             (tmp_sect->type == (*sect)->type)) &&
            tmp_sect_cls->can_merge) {
          /* Determine if the sections can merge */
          if ((status = (*tmp_sect_cls->can_merge)(tmp_sect, *sect, op_data)) <
              0)
            HGOTO_ERROR(H5E_FSPACE, H5E_CANTMERGE, FAIL,
                        "can't check for merging sections");
          if (status > 0) {
            /* Sanity check */
      ...
      /* Look for section after new (or merged) section, if not already
       * determined */
      if (!greater_sect_node_valid)
        greater_sect_node =
            H5SL_above(fspace->sinfo->merge_list, &(*sect)->addr);
      ...
      if (greater_sect_node) {
         ...
         *  then check for 'can merge' callback
         */
        if ((!(sect_cls->flags & H5FS_CLS_MERGE_SYM) ||
             ((*sect)->type == tmp_sect->type)) &&
            sect_cls->can_merge) {
          ...
          if (status > 0) {
            ...
            /* It's possible that the merge caused the section to be deleted
             * (particularly in the paged allocation case) */
            if (*sect == NULL)
              HGOTO_DONE(ret_value);
  ...
  do {
    ...
    /* Check for (possibly merged) section able to shrink the size of the
     * container */
    sect_cls = &fspace->sect_cls[(*sect)->type];
    if (sect_cls->can_shrink) {
      if ((status = (*sect_cls->can_shrink)(*sect, op_data)) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTSHRINK, FAIL,
                    "can't check for shrinking container");
      if (status > 0) {
        ...
        /* (callback can indicate that it has discarded the section by setting
         * *sect to NULL) */
        assert(sect_cls->shrink);
        if ((*sect_cls->shrink)(sect, op_data) < 0)
          HGOTO_ERROR(H5E_FSPACE, H5E_CANTINSERT, FAIL,
                      "can't shrink free space container");

        /* If this section was shrunk away, we may need to shrink another
         * section */
        if (*sect == NULL) {
          ...
          if (fspace->sinfo->merge_list) {
            ...
            if (NULL != (last_node = H5SL_last(fspace->sinfo->merge_list))) {
              ...
              *sect = (H5FS_section_info_t *)H5SL_item(last_node);
              ...
              /* Indicate that this section needs to be removed if it causes a
               * shrink */
              remove_sect = TRUE;
            } /* end if */
          }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_sect_add(H5F_t *f, H5FS_t *fspace, H5FS_section_info_t *sect,
  ...
  fprintf(stderr, "%s: *sect = {%" PRIuHADDR ", %" PRIuHSIZE ", %u, %s}\n",
          ...
          (sect->state == H5FS_SECT_LIVE ? "H5FS_SECT_LIVE"
 ...
 *
 * Purpose:     Try to extend a block using space from a section on the free
 *list
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
htri_t H5FS_sect_try_extend(H5F_t *f, H5FS_t *fspace, haddr_t addr,
  ...
  if (fspace->tot_sect_count > 0) {
    ...
    */
    ...
    if ((sect = (H5FS_section_info_t *)H5SL_greater(fspace->sinfo->merge_list,
                                                    &addr))) {
      ...
      if (sect->size >= extra_requested && (addr + size) == sect->addr) {
        ...
        if (sect->size > extra_requested) {
          ...
          /* Re-adding the section could cause it to disappear (particularly
           * when paging) */
          if (sect) {
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5FS_sect_try_merge(H5F_t *f, H5FS_t *fspace, H5FS_section_info_t *sect,
                           unsigned flags, void *op_data) {
  hbool_t sinfo_valid = FALSE;    /* Whether the section info is valid */
  hbool_t sinfo_modified = FALSE; /* Whether the section info was modified */
  ...
  do {
    ...
    if (fspace->sinfo->bins[bin].bin_list) {
      ...
      if (!alignment) { /* no alignment */
        /* Find the first free space section that is large enough to fulfill
         * request */
        /* (Since the bins use skip lists to track the sizes of the
         * address-ordered lists, this is actually a "best fit" algorithm)
         */
        /* Look for large enough free space section in this bin */
        if ((fspace_node = (H5FS_node_t *)H5SL_greater(
                 fspace->sinfo->bins[bin].bin_list, &request))) {
          /* Take first node off of the list (ie. node w/lowest address) */
          if (NULL == (*node = (H5FS_section_info_t *)H5SL_remove_first(
                           fspace_node->sect_list)))
            HGOTO_ERROR(H5E_FSPACE, H5E_CANTREMOVE, FAIL,
                        "can't remove free space node from skip list");
        ...
        while (curr_size_node != NULL) {
          ...
          /* Get the Skip list which holds  pointers to actual free list
           * sections */
          curr_sect_node =
              (H5SL_node_t *)H5SL_first(curr_fspace_node->sect_list);
            ...
            if ((curr_sect->size >= (request + frag_size)) && (cls->split)) {
              ...
              if (NULL == (*node = (H5FS_section_info_t *)H5SL_remove(
                ...
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTREMOVE, FAIL,
                            "can't remove free space node from skip list");
              /* Decrement # of sections in section size node */
              if (H5FS__size_node_decr(fspace->sinfo, bin, curr_fspace_node,
                                       cls) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTREMOVE, FAIL,
                            "can't remove free space size node from skip list");
              ...
              if (H5FS__sect_unlink_rest(fspace, cls, *node) < 0)
                HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL,
                            "can't remove section from non-size tracking data "
                            "structures");

              /*
               * The split() callback splits NODE into 2 sections:
               *  split_sect is the unused fragment for aligning NODE
               *  NODE's addr & size are updated to point to the remaining
               * aligned section split_sect is re-added to free-space
               */
              if (mis_align) {
 ...
 *
 * Purpose:     Locate a section of free space (in existing free space list)
 *that is large enough to fulfill request.
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
htri_t H5FS_sect_find(H5F_t *f, H5FS_t *fspace, hsize_t request,
                      H5FS_section_info_t **node) {
  hbool_t sinfo_valid = FALSE;    /* Whether the section info is valid */
  hbool_t sinfo_modified = FALSE; /* Whether the section info was modified */
  htri_t ret_value = FALSE;       /* Return value */
  ...
  if (fspace->tot_sect_count > 0) {
    ...
    if ((ret_value = H5FS__sect_find_node(fspace, request, node)) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTFREE, FAIL,
                  "can't remove section from bins");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__iterate_sect_cb(void *_item, void H5_ATTR_UNUSED *key,
                                    void *_udata) {
  H5FS_section_info_t *sect_info =
      (H5FS_section_info_t *)_item; /* Free space section to work on */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__iterate_node_cb(void *_item, void H5_ATTR_UNUSED *key,
                                    void *_udata) {
  H5FS_node_t *fspace_node =
      (H5FS_node_t *)_item; /* Free space size node to work on */
  H5FS_iter_ud_t *udata = (H5FS_iter_ud_t *)_udata; /* Callback info */
  herr_t ret_value = SUCCEED;                       /* Return value */
  ...
  if (H5SL_iterate(fspace_node->sect_list, H5FS__iterate_sect_cb, udata) < 0)
    HGOTO_ERROR(H5E_FSPACE, H5E_BADITER, FAIL,
                "can't iterate over section nodes");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_sect_iterate(H5F_t *f, H5FS_t *fspace, H5FS_operator_t op,
                         void *op_data) {
  ...
  if (fspace->tot_sect_count) {
    ...
    for (bin = 0; bin < fspace->sinfo->nbins; bin++) {
      ...
      if (fspace->sinfo->bins[bin].bin_list) {
        ...
        if (H5SL_iterate(fspace->sinfo->bins[bin].bin_list,
                         H5FS__iterate_node_cb, &udata) < 0)
          HGOTO_ERROR(H5E_FSPACE, H5E_BADITER, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_sect_stats(const H5FS_t *fspace, hsize_t *tot_space,
                       hsize_t *nsects) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_sect_change_class(H5F_t *f, H5FS_t *fspace,
  ...
  /* Check if the section's class change will affect the # of serializable or
   * ghost sections */
  if ((old_cls->flags & H5FS_CLS_GHOST_OBJ) !=
      (new_cls->flags & H5FS_CLS_GHOST_OBJ)) {
    H5FS_node_t *fspace_node; /* Free list size node */
    unsigned bin;             /* Bin to put the free space section in */
    ...
    fspace_node = (H5FS_node_t *)H5SL_search(fspace->sinfo->bins[bin].bin_list,
                                             &sect->size);
    assert(fspace_node);
  ...
  if ((old_cls->flags & H5FS_CLS_SEPAR_OBJ) !=
      (new_cls->flags & H5FS_CLS_SEPAR_OBJ)) {
    hbool_t to_mergable; /* Flag if the section is changing to a mergeable
                            section */
...
void H5FS__sect_assert(const H5FS_t *fspace) {
  ...
  if (fspace->sinfo->bins) {
    ...
    hsize_t acc_serial_sect_count; /* Accumulated serializable section count
                                      from bins */
    hsize_t
        acc_ghost_sect_count; /* Accumulated ghost section count from bins */
    size_t
        acc_tot_size_count; /* Accumulated total section size count from bins */
    size_t acc_serial_size_count; /* Accumulated serializable section size count
                                     from bins */
    size_t acc_ghost_size_count;  /* Accumulated ghost section size count from
                                     bins */
    ...
    for (u = 0; u < fspace->sinfo->nbins; u++) {
      ...
      if (fspace->sinfo->bins[u].bin_list) {
        H5SL_node_t
            *curr_size_node;     /* Current section size node in skip list */
        ...
        while (curr_size_node != NULL) {
          ...
          size_t
              size_serial_count;   /* # of serializable sections of this size */
        ...
        assert(fspace->sinfo->bins[u].tot_sect_count ==
               (bin_serial_count + bin_ghost_count));
  ...
  if (fspace->sinfo->merge_list)
    assert(fspace->tot_sect_count ==
           (separate_obj + H5SL_count(fspace->sinfo->merge_list)));
 ...
 *
 *              The previous hack in this routine to avoid the potential
 *infinite loops by allocating file space directly from the end of the file is
 *removed.  The allocation can now be done via the usual file space allocation
 *call H5MF_alloc().
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_vfd_alloc_hdr_and_section_info_if_needed(H5F_t *f, H5FS_t *fspace,
                                                     haddr_t *fs_addr_ptr) {
  ...
  if (fspace->serial_sect_count > 0 && fspace->sinfo) {
    ...
    if (!H5_addr_defined(fspace->addr)) {
      ...
      if (H5F_IS_TMP_ADDR(f, (eoa + fspace->sect_size)))
        HGOTO_ERROR(
            H5E_RESOURCE, H5E_BADRANGE, FAIL,
      ...
      if (HADDR_UNDEF ==
          (fspace->addr = H5MF_alloc(f, H5FD_MEM_FSPACE_HDR, hdr_alloc_size)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                    "file allocation failed for free space header");

      /* Cache the new free space header (pinned) */
      if (H5AC_insert_entry(f, H5AC_FSPACE_HDR, fspace->addr, fspace,
                            H5AC__PIN_ENTRY_FLAG) < 0)
    ...
    if (!H5_addr_defined(fspace->sect_addr)) {
      ...
      if (HADDR_UNDEF ==
          (sect_addr = H5MF_alloc(f, H5FD_MEM_FSPACE_SINFO, sinfo_alloc_size)))
        HGOTO_ERROR(H5E_FSPACE, H5E_NOSPACE, FAIL,
                    "file allocation failed for section info");
      ...
      if (fspace->sect_size > sinfo_alloc_size) {
         ...
         *                                        JRM -- 11/4/16
         */
        if (H5AC_insert_entry(f, H5AC_FSPACE_SINFO, sect_addr, fspace->sinfo,
