...
herr_t H5HL_create(H5F_t *f, size_t size_hint, haddr_t *addr_p /*out*/) {
  ...
  if (NULL == (heap = H5HL__new(H5F_SIZEOF_SIZE(f), H5F_SIZEOF_ADDR(f),
                                H5HL_SIZEOF_HDR(f))))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL,
                "can't allocate new heap struct");

  /* Allocate file space */
  total_size = heap->prfx_size + size_hint;
  if (HADDR_UNDEF ==
      (heap->prfx_addr = H5MF_alloc(f, H5FD_MEM_LHEAP, total_size)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL,
  ...
  /* Add to cache */
  if (FAIL == H5AC_insert_entry(f, H5AC_LHEAP_PRFX, heap->prfx_addr, prfx,
                                H5AC__NO_FLAGS_SET))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "unable to cache local heap prefix");
    ...
    if (prfx) {
      ...
      if (heap) {
        if (H5_addr_defined(heap->prfx_addr))
          if (FAIL ==
              H5MF_xfree(f, H5FD_MEM_LHEAP, heap->prfx_addr, total_size))
            HDONE_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                        "can't release heap data?");
...
static herr_t H5HL__minimize_heap_space(H5F_t *f, H5HL_t *heap) {
  ...
  if (heap->freelist) {
    ...
    if (last_fl) {
       ...
       */
      if (last_fl->size >= (heap->dblk_size / 2) &&
          heap->dblk_size > H5HL_MIN_HEAP) {
...
H5HL_t *H5HL_protect(H5F_t *f, haddr_t addr, unsigned flags) {
  H5HL_cache_prfx_ud_t
      prfx_udata; /* User data for protecting local heap prefix       */
  H5HL_prfx_t *prfx = NULL; /* Local heap prefix */
  H5HL_dblk_t *dblk = NULL; /* Local heap data block */
  H5HL_t *heap = NULL; /* Heap data structure                              */
  unsigned prfx_cache_flags =
      H5AC__NO_FLAGS_SET; /* Cache flags for unprotecting prefix entry        */
  unsigned dblk_cache_flags =
      H5AC__NO_FLAGS_SET; /* Cache flags for unprotecting data block entry    */
  ...
  if (NULL == (prfx = (H5HL_prfx_t *)H5AC_protect(f, H5AC_LHEAP_PRFX, addr,
                                                  &prfx_udata, flags)))
  ...
  if (heap->prots == 0) {
    ...
    if (heap->single_cache_obj)
      ...
      if (NULL == (dblk = (H5HL_dblk_t *)H5AC_protect(
                       f, H5AC_LHEAP_DBLK, heap->dblk_addr, heap, flags)))
  ...
  if (prfx && heap &&
      H5AC_unprotect(f, H5AC_LHEAP_PRFX, heap->prfx_addr, prfx,
                     prfx_cache_flags) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, NULL,
                "unable to release local heap prefix");

  /* Release the data block from the cache, now pinned */
  if (dblk && heap &&
      H5AC_unprotect(f, H5AC_LHEAP_DBLK, heap->dblk_addr, dblk,
                     dblk_cache_flags) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, NULL,
                "unable to release local heap data block");
...
void *H5HL_offset_into(const H5HL_t *heap, size_t offset) {
  ...
  if (offset >= heap->dblk_size)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, NULL,
                "unable to offset into local heap data block");
    ...
    if (heap->single_cache_obj) {
      ...
      if (FAIL == H5AC_unpin_entry(heap->prfx))
        HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPIN, FAIL,
                    "unable to unpin local heap data block");
    } else {
      /* Sanity check */
      assert(heap->dblk);
  ...
  if (!heap->single_cache_obj) {
    ...
    if (FAIL == H5AC_mark_entry_dirty(heap->dblk))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTMARKDIRTY, FAIL,
                  "unable to mark heap data block as dirty");
  }

  /* Mark heap prefix as dirty */
  if (FAIL == H5AC_mark_entry_dirty(heap->prfx))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTMARKDIRTY, FAIL,
                "unable to mark heap prefix as dirty");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HL_insert(H5F_t *f, H5HL_t *heap, size_t buf_size, const void *buf,
                   size_t *offset_out) {
  ...
  if (found == FALSE) {
    ...
    size_t
        new_dblk_size; /* Final size of space allocated for heap data block */
    size_t old_dblk_size; /* Previous size of space allocated for heap data
                             block */
    htri_t was_extended;  /* Whether the local heap's data segment on disk was
                             extended */
    ...
    if (was_extended == TRUE) {
      ...
      if (heap->single_cache_obj) {
        ...
        if (FAIL == H5AC_resize_entry(
                        heap->prfx, (size_t)(heap->prfx_size + new_dblk_size)))
          HGOTO_ERROR(H5E_HEAP, H5E_CANTRESIZE, FAIL,
                      "unable to resize heap prefix in cache");
      } else {
        /* Resize 'standalone' data block */
        if (FAIL == H5AC_resize_entry(heap->dblk, (size_t)new_dblk_size))
          HGOTO_ERROR(H5E_HEAP, H5E_CANTRESIZE, FAIL,
                      "unable to resize heap data block in cache");
      }
...
#endif
    if (NULL == (heap->dblk_image = H5FL_BLK_REALLOC(
                     lheap_chunk, heap->dblk_image, heap->dblk_size)))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL, "memory allocation failed");

    /* Clear new section so junk doesn't appear in the file */
    /* (Avoid clearing section which will be overwritten with newly inserted
     * data) */
    memset(heap->dblk_image + offset + buf_size, 0,
           (new_dblk_size - (offset + buf_size)));
  }
...
herr_t H5HL_delete(H5F_t *f, haddr_t addr) {
  ...
  /* Protect the local heap prefix */
  if (NULL == (prfx = (H5HL_prfx_t *)H5AC_protect(
                   f, H5AC_LHEAP_PRFX, addr, &prfx_udata, H5AC__NO_FLAGS_SET)))
  ...
  if (!heap->single_cache_obj)
    /* Protect the local heap data block */
    if (NULL ==
        (dblk = (H5HL_dblk_t *)H5AC_protect(f, H5AC_LHEAP_DBLK, heap->dblk_addr,
                                            heap, H5AC__NO_FLAGS_SET)))
  ...
  if (dblk && heap &&
      H5AC_unprotect(f, H5AC_LHEAP_DBLK, heap->dblk_addr, dblk, cache_flags) <
          0)
  ...
  if (prfx && heap &&
      H5AC_unprotect(f, H5AC_LHEAP_PRFX, heap->prfx_addr, prfx, cache_flags) <
          0)
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                "unable to release local heap prefix");
...
herr_t H5HL_get_size(H5F_t *f, haddr_t addr, size_t *size) {
  ...
  /* Protect the local heap prefix */
  if (NULL ==
      (prfx = (H5HL_prfx_t *)H5AC_protect(f, H5AC_LHEAP_PRFX, addr, &prfx_udata,
  ...
  *size = heap->dblk_size;
  ...
  if (prfx && FAIL == H5AC_unprotect(f, H5AC_LHEAP_PRFX, heap->prfx_addr, prfx,
                                     H5AC__NO_FLAGS_SET))
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                "unable to release local heap prefix");
