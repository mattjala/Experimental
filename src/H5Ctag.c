...
/* Typedef for tagged entry iterator callback context - expunge tag type
 * metadata */
typedef struct {
  H5F_t *f;       /* File pointer for evicting entry */
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE hbool_t H5C_get_ignore_tags(const H5C_t *cache) {
  FUNC_ENTER_NOAPI_NOERR
 ...
 *
 * Purpose:     Tags an entry with the provided tag (contained in the API
 *context). If sanity checking is enabled, this function will perform validation
 *that a proper tag is contained within the provided data access property list
  ...
  if (NULL == tag_info) {
    ...
    if (NULL == (tag_info = H5FL_CALLOC(H5C_tag_info_t)))
      HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, FAIL,
                  "can't allocate tag info for cache entry");
...
herr_t H5C__untag_entry(H5C_t *cache, H5C_cache_entry_t *entry) {
  ...
  if (NULL != (tag_info = entry->tag_info)) {
    ...
    /* Remove the tag info from the tag list, if there's no more entries with
     * this tag */
...
static herr_t H5C__iter_tagged_entries_real(H5C_t *cache, haddr_t tag,
  ...
  if (tag_info) {
    ...
    H5C_cache_entry_t
        *next_entry; /* Pointer to next entry in hash bucket chain */
    ...
    while (entry) {
      ...
      if ((cb)(entry, cb_ctx) != H5_ITER_CONT)
        HGOTO_ERROR(H5E_CACHE, H5E_BADITER, FAIL,
                    "tagged entry iteration callback failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__iter_tagged_entries(H5C_t *cache, haddr_t tag, hbool_t match_global,
                                H5C_tag_iter_cb_t cb, void *cb_ctx) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5C__iter_tagged_entries_real(cache, tag, cb, cb_ctx) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_BADITER, FAIL,
                "iteration of tagged entries failed");
  ...
  if (match_global) {
    ...
    if (H5C__iter_tagged_entries_real(cache, H5AC__GLOBALHEAP_TAG, cb, cb_ctx) <
        0)
      HGOTO_ERROR(H5E_CACHE, H5E_BADITER, FAIL,
                  "iteration of tagged entries failed");
  } /* end if */
...
static int H5C__evict_tagged_entries_cb(H5C_cache_entry_t *entry, void *_ctx) {
  H5C_tag_iter_evict_ctx_t *ctx =
      (H5C_tag_iter_evict_ctx_t *)_ctx; /* Get pointer to iterator context */
  ...
  if (entry->is_protected)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTFLUSH, H5_ITER_ERROR,
                "Cannot evict protected entry");
  else if (entry->is_dirty)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTFLUSH, H5_ITER_ERROR,
                "Cannot evict dirty entry");
...
herr_t H5C_evict_tagged_entries(H5F_t *f, haddr_t tag, hbool_t match_global) {
  ...
  do {
    ...
    if (H5C__iter_tagged_entries(cache, tag, match_global,
                                 H5C__evict_tagged_entries_cb, &ctx) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_BADITER, FAIL,
                  "Iteration of tagged entries failed");
 ...
 *-------------------------------------------------------------------------
 */
static int H5C__mark_tagged_entries_cb(H5C_cache_entry_t *entry,
                                       void H5_ATTR_UNUSED *_ctx) {
  /* Function enter macro */
  FUNC_ENTER_PACKAGE_NOERR
...
static herr_t H5C__mark_tagged_entries(H5C_t *cache, haddr_t tag) {
  ...
  if (H5C__iter_tagged_entries(cache, tag, TRUE, H5C__mark_tagged_entries_cb,
                               NULL) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_BADITER, FAIL,
                "Iteration of tagged entries failed");
...
herr_t H5C_verify_tag(int id, haddr_t tag) {
  ...
  if (tag == H5AC__IGNORE_TAG)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
                "cannot ignore a tag while doing verification.");
    ...
    if ((id == H5AC_SUPERBLOCK_ID) || (id == H5AC_DRVRINFO_ID)) {
      if (tag != H5AC__SUPERBLOCK_TAG)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
                    "superblock not tagged with H5AC__SUPERBLOCK_TAG");
    } /* end if */
    else {
      if (tag == H5AC__SUPERBLOCK_TAG)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
    ...
    if (tag == H5AC__FREESPACE_TAG &&
        ((id != H5AC_FSPACE_HDR_ID) && (id != H5AC_FSPACE_SINFO_ID)))
      HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
                  "H5AC__FREESPACE_TAG applied to non-freespace entry");

    /* SOHM */
    if ((id == H5AC_SOHM_TABLE_ID) || (id == H5AC_SOHM_LIST_ID))
      if (tag != H5AC__SOHM_TAG)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
                    "sohm entry not tagged with H5AC__SOHM_TAG");

    /* Global Heap */
    if (id == H5AC_GHEAP_ID) {
      if (tag != H5AC__GLOBALHEAP_TAG)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
                    "global heap not tagged with H5AC__GLOBALHEAP_TAG");
    } /* end if */
    else {
      if (tag == H5AC__GLOBALHEAP_TAG)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static int H5C__expunge_tag_type_metadata_cb(H5C_cache_entry_t *entry,
                                             void *_ctx) {
  H5C_tag_iter_ettm_ctx_t *ctx =
      (H5C_tag_iter_ettm_ctx_t *)_ctx; /* Get pointer to iterator context */
  int ret_value = H5_ITER_CONT;        /* Return value */
  ...
  if (entry->type->id == ctx->type_id)
    if (H5C_expunge_entry(ctx->f, entry->type, entry->addr, ctx->flags) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTEXPUNGE, H5_ITER_ERROR,
                  "can't expunge entry");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_expunge_tag_type_metadata(H5F_t *f, haddr_t tag, int type_id,
                                     unsigned flags) {
  H5C_t *cache;                /* Pointer to cache structure */
  ...
  /* Iterate through hash table entries, expunge those with specified tag and
   * type id */
  if (H5C__iter_tagged_entries(cache, tag, FALSE,
                               H5C__expunge_tag_type_metadata_cb, &ctx) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_BADITER, FAIL,
                "Iteration of tagged entries failed");
...
herr_t H5C_get_tag(const void *thing, haddr_t *tag) {
  const H5C_cache_entry_t *entry =
      (const H5C_cache_entry_t *)thing; /* Pointer to cache entry */
