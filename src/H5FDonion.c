...
static herr_t H5FD__onion_open_rw(H5FD_onion_t *, unsigned int, haddr_t,
                                  bool new_open);
static herr_t H5FD__onion_sb_encode(H5FD_t *_file, char *name /*out*/,
                                    unsigned char *buf /*out*/);
static herr_t H5FD__onion_sb_decode(H5FD_t *_file, const char *name,
...
static herr_t H5FD__get_onion_revision_count(H5FD_t *file,
...
herr_t H5Pset_fapl_onion(hid_t fapl_id, const H5FD_onion_fapl_info_t *fa) {
  ...
  if (H5P_DEFAULT == fa->backing_fapl_id) {
    if (NULL == (backing_fapl = H5P_object_verify(H5P_FILE_ACCESS_DEFAULT,
                                                  H5P_FILE_ACCESS)))
      HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "invalid backing fapl id");
  } else {
    if (NULL == (backing_fapl =
                     H5P_object_verify(fa->backing_fapl_id, H5P_FILE_ACCESS)))
      HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "invalid backing fapl id");
  }
  ...
  if (backing_vfd_id != H5FD_SEC2)
    HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                "Onion VFD only supports sec2 backing store");
...
static herr_t H5FD__onion_commit_new_revision_record(H5FD_onion_t *file) {
  ...
  HDstrftime(rec->time_of_creation, sizeof(rec->time_of_creation),
             "%Y%m%dT%H%M%SZ", info);
  ...
  if (NULL == (buf = H5MM_malloc(H5FD_ONION_ENCODED_SIZE_REVISION_RECORD +
                                 (size_t)rec->comment_size +
                                 (H5FD_ONION_ENCODED_SIZE_INDEX_ENTRY *
                                  rec->archival_index.n_entries))))
  ...
  if (TRUE == file->align_history_on_pages)
    file->onion_eof = (file->onion_eof + (file->header.page_size - 1)) &
  ...
  if (history->n_revisions == 0) {
    ...
    if (NULL ==
        (history->record_locs = H5MM_calloc(sizeof(H5FD_onion_record_loc_t))))
      HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL,
    ...
    history->record_locs[0].checksum =
    ...
    if (NULL == (new_list = H5MM_calloc((history->n_revisions + 1) *
                                        sizeof(H5FD_onion_record_loc_t))))
      HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL,
                  "unable to resize record pointer list");
    memcpy(new_list, history->record_locs,
           sizeof(H5FD_onion_record_loc_t) * history->n_revisions);
  ...
  if (H5FD_ONION_STORE_TARGET_ONION == file->fa.store_target) {
    ...
    if (file->is_open_rw) {
      ...
      if (H5FD__onion_write_final_history(file) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                    "Can't write history to backing store");
  ...
  if (file->original_file)
    if (H5FD_close(file->original_file) < 0)
      HDONE_ERROR(H5E_VFL, H5E_CANTRELEASE, FAIL,
                  "can't close backing canon file");
  if (file->onion_file)
 ...
 *-----------------------------------------------------------------------------
 */
static haddr_t H5FD__onion_get_eoa(const H5FD_t *_file,
 ...
 *-----------------------------------------------------------------------------
 */
static haddr_t H5FD__onion_get_eof(const H5FD_t *_file,
                                   H5FD_mem_t H5_ATTR_UNUSED type) {
  const H5FD_onion_t *file = (const H5FD_onion_t *)_file;
 ...
 *      craeated and given initial population on the backing store, and the
 *Onion virtual file handle is set; open effects a write-mode open.
 *
 *      Cannot create 'template' history and proceed with normal write-mode
 *open, as this would in effect create an empty first revision, making the
 *history unintuitive. (create file -> initialize and commit empty first
 *revision (revision 0); any data written to file during the 'create' open, as
 *seen by the user, would be in the second revision (revision 1).)
 *
 * Return:      SUCCEED/FAIL
 *-----------------------------------------------------------------------------
 */
static herr_t H5FD__onion_create_truncate_onion(
    H5FD_onion_t *file, const char *filename, const char *name_onion,
  ...
  if (NULL == (file->original_file =
                   H5FD_open(filename, flags, backing_fapl_id, maxaddr)))
    HGOTO_ERROR(H5E_VFL, H5E_CANTOPENFILE, FAIL,
                "cannot open the backing file");

  if (NULL == (file->onion_file =
                   H5FD_open(name_onion, flags, backing_fapl_id, maxaddr)))
    HGOTO_ERROR(H5E_VFL, H5E_CANTOPENFILE, FAIL,
                "cannot open the backing onion file");

  if (NULL == (file->recovery_file = H5FD_open(recovery_file_nameery, flags,
  ...
  if (H5FD_write(file->recovery_file, H5FD_MEM_DRAW, 0, size, buf) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                "cannot write history to the backing recovery file");
  ...
  if (TRUE == file->align_history_on_pages)
    file->onion_eof =
        (file->onion_eof + (hdr->page_size - 1)) & (~(hdr->page_size - 1));

  rec->archival_index.list = NULL;

  if (NULL ==
      (file->rev_index = H5FD__onion_revision_index_init(file->fa.page_size)))
  ...
  if (FAIL == ret_value)
    HDremove(
        recovery_file_nameery); /* destroy new temp file, if 'twas created */
...
static herr_t H5FD__onion_parse_config_str(const char *config_str,
                                           H5FD_onion_fapl_info_t *fa) {
   ...
   * (h5repack, h5diff, h5dump).  Otherwise, the string should have curly
   * brackets, e.g. {revision_num: 2; page_size: 4;}
   */
  if (config_str[0] != '{')
    ...
    if (NULL == (config_str_copy = H5MM_strdup(config_str)))
      HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL,
                  "can't duplicate configure string");
    ...
    if (!HDstrcmp(config_str_copy, ""))
      HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                  "configure string can't be empty");
    ...
    do {
      if (token1 && token2) {
        if (!HDstrcmp(token1, "version")) {
          ...
          HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                      "unknown token in the configure string: %s", token1);
      }
  ...
  if (H5P_DEFAULT == fa->backing_fapl_id ||
      H5I_INVALID_HID == fa->backing_fapl_id) {
    H5P_genclass_t *pclass; /* Property list class to modify */

    if (NULL == (pclass = (H5P_genclass_t *)H5I_object_verify(H5P_FILE_ACCESS,
                                                              H5I_GENPROP_CLS)))
      HGOTO_ERROR(H5E_VFL, H5E_BADTYPE, FAIL, "not a property list class");

    /* Create the new property list */
    if ((fa->backing_fapl_id = H5P_create_id(pclass, TRUE)) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTCREATE, FAIL,
 ...
 *-----------------------------------------------------------------------------
 */
static H5FD_t *H5FD__onion_open(const char *filename, unsigned flags,
  ...
  if (NULL == fa) {
    if (NULL == (config_str = H5P_peek_driver_config_str(plist)))
      HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL,
                  "missing VFL driver configure string");
    ...
    if (NULL == (new_fa = H5MM_calloc(sizeof(H5FD_onion_fapl_info_t))))
      HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL,
                  "can't allocate memory for onion fapl info struct");
    if (H5FD__onion_parse_config_str(config_str, new_fa) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL,
                  "failed to parse configure string");
  ...
  if (NULL ==
      (name_onion = H5MM_malloc(sizeof(char) * (HDstrlen(filename) + 7))))
    HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL,
                "unable to allocate onion name string");
  HDsnprintf(name_onion, HDstrlen(filename) + 7, "%s.onion", filename);

  if (NULL == (recovery_file_nameery =
                   H5MM_malloc(sizeof(char) * (HDstrlen(name_onion) + 10))))
    HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL,
                "unable to allocate recovery name string");
  HDsnprintf(recovery_file_nameery, HDstrlen(name_onion) + 10, "%s.recovery",
             name_onion);
  file->recovery_file_name = recovery_file_nameery;

  if (NULL == (file->recovery_file_name =
                   H5MM_malloc(sizeof(char) * (HDstrlen(name_onion) + 10))))
    HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL,
                "unable to allocate recovery name string");
  HDsnprintf(file->recovery_file_name, HDstrlen(name_onion) + 10, "%s.recovery",
             name_onion);
  ...
  file->curr_rev_record.archival_index.version =
  ...
  file->curr_rev_record.archival_index.page_size_log2 =
  ...
  if ((H5F_ACC_CREAT | H5F_ACC_TRUNC) & flags) {
    ...
    if (NULL == file->onion_file) {
      if (H5F_ACC_RDWR & flags) {
        ...
        if (HADDR_UNDEF ==
            (canon_eof = H5FD_get_eof(file->original_file, H5FD_MEM_DEFAULT))) {
          HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, NULL,
                      "cannot get size of canonical file");
        }
        ...
        backing_fapl_id =
            H5FD__onion_get_legit_fapl_id(file->fa.backing_fapl_id);
        ...
        if ((file->onion_file = H5FD_open(
                 name_onion, (H5F_ACC_RDWR | H5F_ACC_CREAT | H5F_ACC_TRUNC),
        ...
        /* Write history header with "no" history */
        hdr->history_size =
            H5FD_ONION_ENCODED_SIZE_HISTORY; /* record for later use */
        hdr->history_addr = H5FD_ONION_ENCODED_SIZE_HEADER +
                            1; /* TODO: comment these 2 or do some other way */
        ...
        size =
            H5FD__onion_history_encode(history, hist_buf, &history->checksum);
        file->header.history_size = size; /* record for later use */
        if (H5FD_ONION_ENCODED_SIZE_HISTORY != size) {
          HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL, "can't encode history");
        }
        if (H5FD_set_eoa(file->onion_file, H5FD_MEM_DRAW,
                         saved_size + size + 1) < 0)
          HGOTO_ERROR(H5E_VFL, H5E_CANTSET, NULL, "can't extend EOA");

        if (H5FD_write(file->onion_file, H5FD_MEM_DRAW, 0, saved_size,
                       head_buf) < 0) {
        ...
        if (TRUE == file->align_history_on_pages)
          file->onion_eof = (file->onion_eof + (hdr->page_size - 1)) &
                            (~(hdr->page_size - 1));
         ...
         */
        if (H5FD_write(file->onion_file, H5FD_MEM_DRAW, saved_size + 1, size,
                       hist_buf) < 0) {
          HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, NULL,
    ...
    file->align_history_on_pages =
        (file->header.flags & H5FD_ONION_HEADER_FLAG_PAGE_ALIGNMENT) ? TRUE
                                                                     : FALSE;
    ...
    if (H5FD_ONION_HEADER_FLAG_WRITE_LOCK & file->header.flags) {
      ...
      if (H5FD__onion_ingest_history(&file->history, file->onion_file,
                                     file->header.history_addr,
                                     file->header.history_size) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTDECODE, NULL,
                    "can't get history from backing store");
      ...
      if (fa->revision_num == 0) {
        file->curr_rev_record.logical_eof = canon_eof;
                 ...
                 H5FD__onion_ingest_revision_record(
                     ...
                     MIN(fa->revision_num - 1,
                         (file->history.n_revisions - 1))) < 0) {
        HGOTO_ERROR(H5E_VFL, H5E_CANTDECODE, NULL,
  ...
  if ((H5F_ACC_RDWR | H5F_ACC_CREAT | H5F_ACC_TRUNC) & flags) {
     ...
     * don't have one).
     */
    if (NULL == (file->curr_rev_record.comment = H5MM_strndup(
                     fa->comment, H5FD_ONION_FAPL_INFO_COMMENT_MAX_LEN)))
      HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL,
    ...
    if (file->original_file)
      if (H5FD_close(file->original_file) < 0)
        HDONE_ERROR(H5E_VFL, H5E_CANTRELEASE, NULL,
                    "can't destroy backing canon");
    if (file->onion_file)
      if (H5FD_close(file->onion_file) < 0)
        HDONE_ERROR(H5E_VFL, H5E_CANTRELEASE, NULL,
                    "can't destroy backing onion");
    if (file->recovery_file)
      if (H5FD_close(file->recovery_file) < 0)
        HDONE_ERROR(H5E_VFL, H5E_CANTRELEASE, NULL,
                    "can't destroy backing recov");
 ...
 *-----------------------------------------------------------------------------
 */
static herr_t H5FD__onion_open_rw(H5FD_onion_t *file, unsigned int flags,
                                  haddr_t maxaddr, bool new_open) {
  unsigned char *buf = NULL;
  size_t size = 0;
  uint32_t checksum = 0;
  herr_t ret_value = SUCCEED;
  ...
  if (NULL ==
      (file->recovery_file = H5FD_open(file->recovery_file_name,
                                       (flags | H5F_ACC_CREAT | H5F_ACC_TRUNC),
                                       file->fa.backing_fapl_id, maxaddr)))
    HGOTO_ERROR(H5E_VFL, H5E_CANTOPENFILE, FAIL,
                "unable to create recovery file");

  if (0 == (size = H5FD__onion_write_history(&file->history,
                                             file->recovery_file, 0, 0)))
    HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                "can't write history to recovery file");
  if (size != file->header.history_size)
    HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                "written history differed from expected size");
  ...
  if (NULL ==
      (file->rev_index = H5FD__onion_revision_index_init(file->fa.page_size)))
    HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL, "can't initialize revision index");
  file->curr_rev_record.parent_revision_num =
      file->curr_rev_record.revision_num;
  if (!new_open)
 ...
 *-----------------------------------------------------------------------------
 */
static herr_t H5FD__onion_read(H5FD_t *_file, H5FD_mem_t type,
                               hid_t H5_ATTR_UNUSED dxpl_id, haddr_t offset,
                               size_t len, void *_buf_out) {
  H5FD_onion_t *file = (H5FD_onion_t *)_file;
  uint64_t page_0 = 0;
  size_t n_pages = 0;
  ...
  for (size_t i = 0; i < n_pages; i++) {
    ...
    if (0 == i) {
      ...
      /* Check if we need to add an additional page to make up for the
       * page_gap_head */
      if (page_gap_head > 0 &&
          (page_gap_head + (bytes_to_read % page_size) > page_size ||
           bytes_to_read % page_size == 0)) {
        n_pages++;
      }
    }
    ...
    if (TRUE == file->is_open_rw && file->fa.revision_num != 0 &&
      ...
      if (H5FD_read(file->onion_file, H5FD_MEM_DRAW,
                    entry_out->phys_addr + page_gap_head, page_readsize,
                    buf_out) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
               ...
               H5FD__onion_archival_index_find(
                   &file->curr_rev_record.archival_index, page_i, &entry_out)) {
      ...
      haddr_t addr_start =
          (haddr_t)page_i * (haddr_t)page_size + (haddr_t)page_gap_head;
      haddr_t overlap_size =
          (addr_start > file->origin_eof) ? 0 : file->origin_eof - addr_start;
      haddr_t read_size = MIN(overlap_size, page_readsize);

      /* Get all original bytes in page range */
      if ((read_size > 0) && H5FD_read(file->original_file, type, addr_start,
                                       read_size, buf_out) < 0) {
        HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
 ...
 *-----------------------------------------------------------------------------
 */
static herr_t H5FD__onion_set_eoa(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
                                  haddr_t addr) {
  H5FD_onion_t *file = (H5FD_onion_t *)_file;
 ...
 *-----------------------------------------------------------------------------
 */
static herr_t H5FD__onion_write(H5FD_t *_file, H5FD_mem_t type,
                                hid_t H5_ATTR_UNUSED dxpl_id, haddr_t offset,
                                size_t len, const void *_buf) {
  H5FD_onion_t *file = (H5FD_onion_t *)_file;
  uint64_t page_0 = 0;
  size_t n_pages = 0;
  ...
  for (size_t i = 0; i < n_pages; i++) {
    ...
    if (0 == i) {
       ...
       */
      if (page_gap_head > 0 &&
          (page_gap_head + (bytes_to_write % page_size) > page_size ||
    ...
    if (H5FD__onion_revision_index_find(file->rev_index, page_i, &entry_out)) {
      if (page_gap_head | page_gap_tail) {
        ...
        if (H5FD_read(file->onion_file, H5FD_MEM_DRAW, entry_out->phys_addr,
      ...
      if (H5FD_write(file->onion_file, H5FD_MEM_DRAW, entry_out->phys_addr,
    ...
    if (page_gap_head || page_gap_tail) {
      ...
      if (H5FD__onion_archival_index_find(&file->curr_rev_record.archival_index,
                                          page_i, &entry_out)) {
        /* Page exists in archival index */
         ...
         */
        for (size_t j = MAX(read_size, page_size - page_gap_tail);
             j < page_size; j++)
          page_buf[j] = 0;
    ...
    if (H5FD_set_eoa(file->onion_file, H5FD_MEM_DRAW,
                     file->onion_eof + page_size) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL,
                  "can't modify EOA for new page amendment");

    if (H5FD_write(file->onion_file, H5FD_MEM_DRAW, file->onion_eof, page_size,
                   write_buf) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                  "write amended page data to backing file");
...
herr_t H5FDonion_get_revision_count(const char *filename, hid_t fapl_id,
  ...
  if (NULL ==
      (file = H5FD_open(filename, H5F_ACC_RDONLY, fapl_id, HADDR_UNDEF)))
    HGOTO_ERROR(H5E_VFL, H5E_CANTOPENFILE, FAIL,
                "unable to open file with onion driver");

  /* Call the private function */
  if (H5FD__get_onion_revision_count(file, revision_count) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                "failed to get the number of revisions");
...
herr_t H5FD__onion_write_final_history(H5FD_onion_t *file) {
  ...
  if (0 == (size = H5FD__onion_write_history(&(file->history), file->onion_file,
                                             file->onion_eof, file->onion_eof)))
