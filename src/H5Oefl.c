...
static void *H5O__efl_decode(H5F_t *f, H5O_t *open_oh, unsigned mesg_flags,
                             ...
                             const uint8_t *p);
static herr_t H5O__efl_encode(H5F_t *f, hbool_t disable_shared, uint8_t *p,
                              const void *_mesg);
static void *H5O__efl_copy(const void *_mesg, void *_dest);
static size_t H5O__efl_size(const H5F_t *f, hbool_t disable_shared,
                            const void *_mesg);
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__efl_decode(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh,
  ...
  if (mesg->nalloc <= 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, NULL,
                "bad number of allocated slots when parsing efl msg");
  ...
  if (mesg->nused > mesg->nalloc)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, NULL,
                "bad number of in-use slots when parsing efl msg");
  ...
  if (H5_addr_defined(mesg->heap_addr) == FALSE)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, NULL,
                "bad local heap address when parsing efl msg");

  /* Decode the file list */
  mesg->slot =
      (H5O_efl_entry_t *)H5MM_calloc(mesg->nalloc * sizeof(H5O_efl_entry_t));
  if (NULL == mesg->slot)
    HGOTO_ERROR(H5E_OHDR, H5E_NOSPACE, NULL, "memory allocation failed");

  if (NULL == (heap = H5HL_protect(f, mesg->heap_addr, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, NULL,
                "unable to protect local heap");
  ...
  if (s == NULL)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                "could not obtain pointer into local heap");
  if (*s != '\0')
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                "entry at offset 0 in local heap not an empty string");
#endif
  ...
  for (size_t u = 0; u < mesg->nused; u++) {
    ...
    if ((s = (const char *)H5HL_offset_into(heap, mesg->slot[u].name_offset)) ==
        NULL)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                  "unable to get external file name");
    if (*s == '\0')
  ...
  if (H5HL_unprotect(heap) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, NULL,
                "unable to unprotect local heap");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__efl_encode(H5F_t *f, hbool_t H5_ATTR_UNUSED disable_shared,
...
static void *H5O__efl_copy(const void *_mesg, void *_dest) {
  ...
  size_t u; /* Local index variable */
  hbool_t slot_allocated =
      FALSE; /* Flag to indicate that dynamic allocation has begun */
  void *ret_value = NULL; /* Return value */
  ...
  *dest = *mesg;
  ...
  if (dest->nalloc > 0) {
    if (NULL == (dest->slot = (H5O_efl_entry_t *)H5MM_calloc(
                     dest->nalloc * sizeof(H5O_efl_entry_t))))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, NULL,
                  "can't allocate efl message slots");
    slot_allocated = TRUE;
    for (u = 0; u < mesg->nused; u++) {
      dest->slot[u] = mesg->slot[u];
      if (NULL == (dest->slot[u].name = H5MM_xstrdup(mesg->slot[u].name)))
        HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, NULL,
                    "can't allocate efl message slot name");
    } /* end for */
  }   /* end if */
  ...
  if (NULL == ret_value) {
    if (slot_allocated) {
      for (u = 0; u < dest->nused; u++)
        if (dest->slot[u].name != NULL &&
            dest->slot[u].name != mesg->slot[u].name)
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__efl_size(const H5F_t *f,
...
hsize_t H5O_efl_total_size(H5O_efl_t *efl) {
  ...
  if (efl->nused > 0 && H5O_EFL_UNLIMITED == efl->slot[efl->nused - 1].size)
    ...
    for (u = 0; u < efl->nused; u++, ret_value = tmp) {
      ...
      if (tmp <= ret_value)
        HGOTO_ERROR(H5E_EFL, H5E_OVERFLOW, 0,
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__efl_copy_file(H5F_t H5_ATTR_UNUSED *file_src, void *mesg_src,
                                H5F_t *file_dst,
  ...
  if (NULL ==
      (heap = H5HL_protect(file_dst, efl_dst->heap_addr, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_EFL, H5E_PROTECT, NULL,
                "unable to protect EFL file name heap");

  /* Insert "empty" name first */
  if (H5HL_insert(file_dst, heap, (size_t)1, "", &name_offset) < 0)
    HGOTO_ERROR(H5E_EFL, H5E_CANTINSERT, NULL,
                "can't insert file name into heap");
  ...
  for (idx = 0; idx < efl_src->nused; idx++) {
    ...
    if (H5HL_insert(file_dst, heap, HDstrlen(efl_dst->slot[idx].name) + 1,
  ...
  if (heap && H5HL_unprotect(heap) < 0)
    HDONE_ERROR(H5E_EFL, H5E_PROTECT, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__efl_debug(H5F_t H5_ATTR_UNUSED *f, const void *_mesg,
  ...
  fprintf(stream, "%*s%-*s %zu/%zu\n", indent, "", fwidth,
          "Slots used/allocated:", mesg->nused, mesg->nalloc);
