 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_file_create(const char *name, unsigned flags, hid_t fcpl_id,
                               hid_t fapl_id, hid_t H5_ATTR_UNUSED dxpl_id,
 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_file_open(const char *name, unsigned flags, hid_t fapl_id,
  ...
  /* Open the file */
  if (NULL ==
      (new_file = H5F_open(name, flags, H5P_FILE_CREATE_DEFAULT, fapl_id)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_file_get(void *obj, H5VL_file_get_args_t *args,
                             hid_t H5_ATTR_UNUSED dxpl_id,
                             void H5_ATTR_UNUSED **req) {
  ...
  case H5VL_FILE_GET_FAPL: {
    if ((args->args.get_fapl.fapl_id =
             H5F_get_access_plist((H5F_t *)obj, TRUE)) < 0)
  ...
  case H5VL_FILE_GET_OBJ_IDS: {
    ...
    if (H5F_get_obj_ids((H5F_t *)obj, file_args->types, file_args->max_objs,
                        file_args->oid_list, TRUE, file_args->count) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_file_specific(void *obj, H5VL_file_specific_args_t *args,
                                  hid_t H5_ATTR_UNUSED dxpl_id,
                                  void H5_ATTR_UNUSED **req) {
  herr_t ret_value = SUCCEED; /* Return value */
      ...
      if (H5F_SCOPE_GLOBAL == args->args.flush.scope) {
        ...
        if (H5F_flush_mounts(f) < 0)
          HGOTO_ERROR(H5E_FILE, H5E_CANTFLUSH, FAIL,
                      "unable to flush mounted file hierarchy");
      } /* end if */
      else {
  ...
  case H5VL_FILE_IS_ACCESSIBLE: {
    ...
    if ((result = H5F__is_hdf5(args->args.is_accessible.filename,
                               args->args.is_accessible.fapl_id)) < 0)
  ...
  case H5VL_FILE_IS_EQUAL: {
    if (!obj || !args->args.is_equal.obj2)
      ...
      *args->args.is_equal.same_file =
          (((H5F_t *)obj)->shared ==
           ((H5F_t *)args->args.is_equal.obj2)->shared);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_file_optional(void *obj, H5VL_optional_args_t *args,
                                  hid_t H5_ATTR_UNUSED dxpl_id,
                                  void H5_ATTR_UNUSED **req) {
  ...
  case H5VL_NATIVE_FILE_GET_FILE_IMAGE: {
    ...
    /* Get file image */
    if (H5F__get_file_image(f, gfi_args->buf, gfi_args->buf_size,
                            gfi_args->image_len) < 0)
  ...
  case H5VL_NATIVE_FILE_GET_FREE_SPACE: {
    ...
    if (H5MF_get_freespace(f, gfs_args->size, NULL) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "unable to check free space for file");
  ...
  case H5VL_NATIVE_FILE_GET_FREE_SECTIONS: {
    H5VL_native_file_get_free_sections_t *gfs_args =
        &opt_args->get_free_sections;

    /* Go get the free-space section information in the file */
    if (H5MF_get_free_sections(f, gfs_args->type, gfs_args->nsects,
                               gfs_args->sect_info, gfs_args->sect_count) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "unable to check free space for file");
  ...
  case H5VL_NATIVE_FILE_GET_MDC_CONF: {
    ...
    if (H5AC_get_cache_auto_resize_config(f->shared->cache,
                                          opt_args->get_mdc_config.config) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "can't get metadata cache configuration");
  ...
  case H5VL_NATIVE_FILE_GET_MDC_HR: {
    ...
    if (H5AC_get_cache_hit_rate(f->shared->cache,
                                opt_args->get_mdc_hit_rate.hit_rate) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "can't get metadata cache hit rate");
  ...
  case H5VL_NATIVE_FILE_GET_MDC_SIZE: {
    ...
    /* Get the size data */
    if (H5AC_get_cache_size(f->shared->cache, gms_args->max_size,
                            gms_args->min_clean_size, gms_args->cur_size,
                            gms_args->cur_num_entries) < 0)
  ...
  case H5VL_NATIVE_FILE_SET_MDC_CONFIG: {
    ...
    if (H5AC_set_cache_auto_resize_config(f->shared->cache,
                                          opt_args->set_mdc_config.config) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                  "can't set metadata cache configuration");
  ...
  case H5VL_NATIVE_FILE_GET_METADATA_READ_RETRY_INFO: {
    if (H5F_get_metadata_read_retry_info(
            f, opt_args->get_metadata_read_retry_info.info) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
  ...
  case H5VL_NATIVE_FILE_GET_MDC_LOGGING_STATUS: {
    H5VL_native_file_get_mdc_logging_status_t *gmls_args =
        &opt_args->get_mdc_logging_status;
  ...
  case H5VL_NATIVE_FILE_RESET_PAGE_BUFFERING_STATS: {
    ...
    if (NULL == f->shared->page_buf)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "page buffering not enabled on file");
  ...
  case H5VL_NATIVE_FILE_GET_PAGE_BUFFERING_STATS: {
    H5VL_native_file_get_page_buffering_stats_t *gpbs_args =
        &opt_args->get_page_buffering_stats;
    ...
    /* Get the statistics */
    if (H5PB_get_stats(f->shared->page_buf, gpbs_args->accesses,
                       gpbs_args->hits, gpbs_args->misses, gpbs_args->evictions,
                       gpbs_args->bypasses) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
  ...
  case H5VL_NATIVE_FILE_GET_MDC_IMAGE_INFO: {
    H5VL_native_file_get_mdc_image_info_t *gmii_args =
        &opt_args->get_mdc_image_info;

    /* Go get the address and size of the cache image */
    if (H5AC_get_mdc_image_info(f->shared->cache, gmii_args->addr,
                                gmii_args->len) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
  ...
  case H5VL_NATIVE_FILE_INCR_FILESIZE: {
    ...
    /* This public routine will work only for drivers with this feature
     * enabled.*/
    /* We might introduce a new feature flag in the future */
    if (!H5F_HAS_FEATURE(f, H5FD_FEAT_SUPPORTS_SWMR_IO))
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
    ...
    if (H5F__set_eoa(f, H5FD_MEM_DEFAULT,
                     max_eof_eoa + opt_args->increment_filesize.increment) < 0)
  ...
  case H5VL_NATIVE_FILE_SET_LIBVER_BOUNDS: {
    H5VL_native_file_set_libver_bounds_t *slb_args =
        &opt_args->set_libver_bounds;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_file_close(void *file, hid_t H5_ATTR_UNUSED dxpl_id,
                               void H5_ATTR_UNUSED **req) {
  int nref;
  H5F_t *f = (H5F_t *)file;
