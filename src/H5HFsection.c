...
static herr_t H5HF__sect_single_full_dblock(H5HF_hdr_t *hdr,
                                            H5HF_free_section_t *sect);
...
H5HF__sect_single_deserialize(const H5FS_section_class_t *cls,
...
static htri_t H5HF__sect_single_can_merge(const H5FS_section_info_t *sect1,
                                          const H5FS_section_info_t *sect2,
                                          void *udata);
...
H5HF__sect_indirect_new(H5HF_hdr_t *hdr, haddr_t sect_off, hsize_t sect_size,
                        ...
                        unsigned row, unsigned col, unsigned nentries);
static herr_t H5HF__sect_indirect_init_rows(
    H5HF_hdr_t *hdr, H5HF_free_section_t *sect, hbool_t first_child,
...
static herr_t H5HF__sect_indirect_merge_row(H5HF_hdr_t *hdr,
                                            H5HF_free_section_t *sect1,
                                            H5HF_free_section_t *sect2);
static herr_t H5HF__sect_indirect_build_parent(H5HF_hdr_t *hdr,
                                               H5HF_free_section_t *sect);
...
static herr_t H5HF__sect_indirect_init_cls(H5FS_section_class_t *cls,
                                           void *udata);
...
H5FS_section_class_t H5HF_FSPACE_SECT_CLS_NORMAL_ROW[1] = {{
    ...
    0,                           /* Extra serialized size        */
    H5FS_CLS_MERGE_SYM | H5FS_CLS_SEPAR_OBJ |
...
static herr_t H5FS__sect_init_cls(H5FS_section_class_t *cls, H5HF_hdr_t *hdr) {
   ...
   */
  if (NULL == (cls_prvt = (H5HF_sect_private_t *)H5MM_malloc(
                   sizeof(H5HF_sect_private_t))))
 ...
 *-------------------------------------------------------------------------
 */
H5HF_free_section_t *H5HF__sect_single_new(hsize_t sect_off, size_t sect_size,
                                           H5HF_indirect_t *parent,
                                           unsigned par_entry) {
  ...
  if (NULL == (sect = H5FS__sect_node_new(H5HF_FSPACE_SECT_SINGLE, sect_off,
                                          (hsize_t)sect_size, H5FS_SECT_LIVE)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "memory allocation failed for single section");
...
static herr_t H5HF__sect_single_locate_parent(H5HF_hdr_t *hdr, hbool_t refresh,
  ...
  if (H5HF__man_dblock_locate(hdr, sect->sect_info.addr, &sec_iblock,
                              &sec_entry, &did_protect,
                              H5AC__READ_ONLY_FLAG) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTCOMPUTE, FAIL,
                "can't compute row & column of section");
  ...
  if (H5HF__man_iblock_unprotect(sec_iblock, H5AC__NO_FLAGS_SET, did_protect) <
      0)
  ...
  if (hdr->man_dtable.curr_root_rows == 0) {
    ...
    if (H5HF__sect_single_locate_parent(hdr, FALSE, sect) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                  "can't get section's parent info");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__sect_single_dblock_info(H5HF_hdr_t *hdr,
                                     const H5HF_free_section_t *sect,
                                     haddr_t *dblock_addr,
                                     size_t *dblock_size) {
  ...
  if (hdr->man_dtable.curr_root_rows == 0) {
    ...
    *dblock_size = hdr->man_dtable.row_block_size[sect->u.single.par_entry /
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__sect_single_reduce(H5HF_hdr_t *hdr, H5HF_free_section_t *sect,
                                size_t amt) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (sect->sect_info.size == amt) {
    ...
    if (H5HF__sect_single_free((H5FS_section_info_t *)sect) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTRELEASE, FAIL,
                  "can't free single section node");
  } /* end if */
  else {
    ...
    if (H5HF__space_add(hdr, sect, 0) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                  "can't re-add single section to free space manager");
  } /* end else */
...
static herr_t H5HF__sect_single_full_dblock(H5HF_hdr_t *hdr,
  ...
  if (H5HF__sect_single_dblock_info(hdr, sect, &dblock_addr, &dblock_size) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                "can't retrieve direct block information");
  ...
  if ((dblock_size - dblock_overhead) == sect->sect_info.size &&
    ...
    hbool_t parent_removed; /* Whether the direct block parent was removed from
                               the file */

    if (NULL == (dblock = H5HF__man_dblock_protect(
                     hdr, dblock_addr, dblock_size, sect->u.single.parent,
                     sect->u.single.par_entry, H5AC__NO_FLAGS_SET)))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                  "unable to load fractal heap direct block");
    assert(
        H5_addr_eq(dblock->block_off + dblock_overhead, sect->sect_info.addr));

    /* Convert 'single' section into 'row' section */
    if (H5HF__sect_row_from_single(hdr, sect, dblock) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCONVERT, FAIL,
                  "can't convert single section into row section");

    /* Destroy direct block */
    if (H5HF__man_dblock_destroy(hdr, dblock, dblock_addr, &parent_removed) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTRELEASE, FAIL,
                  "can't release direct block");
    dblock = NULL;
    ...
    if (parent_removed && H5FS_SECT_LIVE == sect->u.row.under->sect_info.state)
      if (H5HF__sect_row_parent_removed(sect) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTUPDATE, FAIL,
                    "can't update section info");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__sect_single_add(H5FS_section_info_t **_sect,
                                    unsigned *flags, void *_udata) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (!(*flags & H5FS_ADD_DESERIALIZING)) {
    H5HF_free_section_t **sect =
        (H5HF_free_section_t **)_sect; /* Fractal heap free section */
    H5HF_sect_add_ud_t *udata =
        (H5HF_sect_add_ud_t *)_udata; /* User callback data */
    ...
    if (H5HF__sect_single_full_dblock(hdr, (*sect)) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCONVERT, FAIL,
                  "can't check/convert single section");
...
H5HF__sect_single_deserialize(const H5FS_section_class_t H5_ATTR_UNUSED *cls,
  ...
  /* Create free list section node */
  if (NULL ==
      (new_sect = H5FS__sect_node_new(H5HF_FSPACE_SECT_SINGLE, sect_addr,
                                      sect_size, H5FS_SECT_SERIALIZED)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "allocation failed for direct block free list section");
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5HF__sect_single_can_merge(const H5FS_section_info_t *_sect1,
                                          const H5FS_section_info_t *_sect2,
                                          void H5_ATTR_UNUSED *_udata) {
  const H5HF_free_section_t *sect1 =
      (const H5HF_free_section_t *)_sect1; /* Fractal heap free section */
  const H5HF_free_section_t *sect2 =
      (const H5HF_free_section_t *)_sect2; /* Fractal heap free section */
  ...
  assert(sect1);
  assert(sect2);
  assert(sect1->sect_info.type ==
         sect2->sect_info.type); /* Checks "MERGE_SYM" flag */
   ...
   */
  if (H5_addr_eq(sect1->sect_info.addr + sect1->sect_info.size,
                 sect2->sect_info.addr))
    HGOTO_DONE(TRUE);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__sect_single_merge(H5FS_section_info_t **_sect1,
                                      H5FS_section_info_t *_sect2,
                                      void *_udata) {
  H5HF_free_section_t **sect1 =
      (H5HF_free_section_t **)_sect1; /* Fractal heap free section */
  H5HF_free_section_t *sect2 =
      (H5HF_free_section_t *)_sect2; /* Fractal heap free section */
  H5HF_sect_add_ud_t *udata =
      (H5HF_sect_add_ud_t *)_udata; /* User callback data */
  H5HF_hdr_t *hdr = udata->hdr;     /* Fractal heap header */
  herr_t ret_value = SUCCEED;       /* Return value */
  ...
  assert(H5_addr_eq((*sect1)->sect_info.addr + (*sect1)->sect_info.size,
                    sect2->sect_info.addr));
  ...
  if ((*sect1)->sect_info.state != H5FS_SECT_LIVE)
    if (H5HF__sect_single_revive(hdr, (*sect1)) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                  "can't revive single free section");
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5HF__sect_single_can_shrink(const H5FS_section_info_t *_sect,
                                           void *_udata) {
  const H5HF_free_section_t *sect =
      (const H5HF_free_section_t *)_sect; /* Fractal heap free section */
  H5HF_sect_add_ud_t *udata =
      (H5HF_sect_add_ud_t *)_udata; /* User callback data */
  H5HF_hdr_t *hdr = udata->hdr;     /* Fractal heap header */
  htri_t ret_value = FALSE;         /* Return value */
...
static herr_t H5HF__sect_single_shrink(H5FS_section_info_t **_sect,
  ...
  /* Retrieve direct block address from section */
  if (H5HF__sect_single_dblock_info(hdr, (*sect), &dblock_addr, &dblock_size) <
      0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                "can't retrieve direct block information");
  ...
  if (NULL == (dblock = H5HF__man_dblock_protect(
    ...
    HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                "unable to load fractal heap direct block");
  assert(H5_addr_eq(dblock->block_off + dblock_size,
...
herr_t H5HF__sect_single_free(H5FS_section_info_t *_sect) {
  H5HF_free_section_t *sect =
      (H5HF_free_section_t *)_sect; /* Pointer to section to free */
 ...
 */
static herr_t
H5HF__sect_single_valid(const H5FS_section_class_t H5_ATTR_UNUSED *cls,
                        const H5FS_section_info_t *_sect) {
  const H5HF_free_section_t *sect =
      (const H5HF_free_section_t *)_sect; /* Pointer to section to check */
  ...
  if (sect->sect_info.state == H5FS_SECT_LIVE) {
    /* Check if this section is not in a direct block that is the root direct
     * block */
    /* (not enough information to check on a single section in a root direct
     * block) */
    if (sect->u.single.parent != NULL) {
      H5HF_indirect_t
          *iblock; /* Indirect block that section's direct block resides in */
      haddr_t dblock_addr; /* Direct block address */
      size_t dblock_size;  /* Direct block size */
      unsigned dblock_status =
          0; /* Direct block's status in the metadata cache */
      size_t H5_ATTR_NDEBUG_UNUSED
          dblock_overhead;                 /* Direct block's overhead */
      ...
      /* Retrieve direct block address from section */
      status = H5HF__sect_single_dblock_info(iblock->hdr,
                                             (const H5HF_free_section_t *)sect,
                                             &dblock_addr, &dblock_size);
      assert(status >= 0);
      assert(
          H5_addr_eq(iblock->ents[sect->u.single.par_entry].addr, dblock_addr));
      ...
      status =
          H5AC_get_entry_status(iblock->hdr->f, dblock_addr, &dblock_status);
      assert(status >= 0);
      ...
      if (!(dblock_status & H5AC_ES__IS_PROTECTED)) {
        ...
        /* Protect the direct block for the section */
        dblock = H5HF__man_dblock_protect(iblock->hdr, dblock_addr, dblock_size,
                                          iblock, sect->u.single.par_entry,
        ...
        status = H5AC_unprotect(iblock->hdr->f, H5AC_FHEAP_DBLOCK, dblock_addr,
                                dblock, H5AC__NO_FLAGS_SET);
        assert(status >= 0);
      } /* end if */
    }   /* end if */
  }     /* end if */
...
H5HF__sect_row_create(haddr_t sect_off, hsize_t sect_size, hbool_t is_first,
  ...
  if (NULL == (sect = H5FS__sect_node_new(
                   (unsigned)(is_first ? H5HF_FSPACE_SECT_FIRST_ROW
                                       : H5HF_FSPACE_SECT_NORMAL_ROW),
                   sect_off, sect_size, under_sect->sect_info.state)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "memory allocation failed for row section");
...
static herr_t H5HF__sect_row_from_single(H5HF_hdr_t *hdr,
  ...
  if (NULL == (sect->u.row.under =
                   H5HF__sect_indirect_for_row(hdr, dblock->parent, sect)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTCREATE, FAIL,
                "serializing row section not supported yet");
...
herr_t H5HF__sect_row_revive(H5HF_hdr_t *hdr, H5HF_free_section_t *sect) {
  ...
  if (H5HF__sect_indirect_revive_row(hdr, sect->u.row.under) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTREVIVE, FAIL,
                "can't revive indirect section");
  assert(sect->sect_info.state == H5FS_SECT_LIVE);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__sect_row_reduce(H5HF_hdr_t *hdr, H5HF_free_section_t *sect,
                             unsigned *entry_p) {
  ...
  /* Forward row section to indirect routines, to handle reducing underlying
   * indirect section */
  ...
  if (H5HF__sect_indirect_reduce_row(hdr, sect, &alloc_from_start) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTSHRINK, FAIL,
                "can't reduce underlying section");
  ...
  *entry_p = (sect->u.row.row * hdr->man_dtable.cparam.width) + sect->u.row.col;
  ...
  if (sect->u.row.num_entries == 1) {
    ...
    if (H5HF__space_add(hdr, sect, 0) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                  "can't re-add indirect section to free space manager");
  } /* end else */
...
static herr_t H5HF__sect_row_first(H5HF_hdr_t *hdr, H5HF_free_section_t *sect) {
  ...
  if (sect->u.row.checked_out)
    ...
    if (H5HF__space_sect_change_class(hdr, sect, H5HF_FSPACE_SECT_FIRST_ROW) <
        0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTSET, FAIL,
                  "can't set row section to be first row");
...
static herr_t H5HF__sect_row_parent_removed(H5HF_free_section_t *sect) {
  ...
  /* Get a copy of the indirect block's offset before decrementing refcount on
   * it */
  tmp_iblock_off = sect->u.row.under->u.indirect.u.iblock->block_off;

  /* Decrement the refcount on the indirect block, since serialized sections
  ...
  for (u = 0; u < sect->u.row.under->u.indirect.dir_nrows; u++)
    sect->u.row.under->u.indirect.dir_rows[u]->sect_info.state =
...
static herr_t H5HF__sect_row_init_cls(H5FS_section_class_t *cls, void *_udata) {
  ...
  if (H5FS__sect_init_cls(cls, hdr) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't initialize common section class");
...
static herr_t H5HF__sect_row_term_cls(H5FS_section_class_t *cls) {
  ...
  if (H5FS__sect_term_cls(cls) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTRELEASE, FAIL,
                "can't terminate common section class");
...
H5HF__sect_row_deserialize(const H5FS_section_class_t *cls, const uint8_t *buf,
  ...
  hdr = ((H5HF_sect_private_t *)(cls->cls_private))->hdr;
  if (NULL == (ret_value = H5HF__sect_indirect_deserialize(
                   hdr, buf, sect_addr, sect_size, des_flags)))
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5HF__sect_row_can_merge(const H5FS_section_info_t *_sect1,
                                       const H5FS_section_info_t *_sect2,
                                       void H5_ATTR_UNUSED *_udata) {
  const H5HF_free_section_t *sect1 =
      (const H5HF_free_section_t *)_sect1; /* Fractal heap free section */
  const H5HF_free_section_t *sect2 =
      (const H5HF_free_section_t *)_sect2; /* Fractal heap free section */
  H5HF_free_section_t *top_indir_sect1,
      *top_indir_sect2;     /* Top indirect section for each row */
  htri_t ret_value = FALSE; /* Return value */
  ...
  if (top_indir_sect1 != top_indir_sect2)
    if (H5HF__sect_indirect_iblock_off(sect1->u.row.under) ==
      ...
      /* Check if second section adjoins first section */
      if (H5_addr_eq((top_indir_sect1->sect_info.addr +
                      top_indir_sect1->u.indirect.span_size),
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__sect_row_merge(H5FS_section_info_t **_sect1,
  ...
  if (sect2->sect_info.addr >= hdr->man_iter_off) {
    ...
    if (H5HF__sect_indirect_shrink(hdr, top_indir_sect) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTSHRINK, FAIL,
                  "can't shrink underlying indirect section");
  } /* end if */
  else
    /* Merge rows' underlying indirect sections together */
    if (H5HF__sect_indirect_merge_row(hdr, (*sect1), sect2) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTMERGE, FAIL,
                  "can't merge underlying indirect sections");
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5HF__sect_row_can_shrink(const H5FS_section_info_t *_sect,
                                        void H5_ATTR_UNUSED *_udata) {
  const H5HF_free_section_t *sect =
      (const H5HF_free_section_t *)_sect; /* Fractal heap free section */
  H5HF_sect_add_ud_t *udata =
...
static herr_t H5HF__sect_row_valid(const H5FS_section_class_t *cls,
  ...
  const H5HF_free_section_t *sect =
      (const H5HF_free_section_t *)_sect; /* Pointer to section to check */
  const H5HF_free_section_t
      *indir_sect; /* Pointer to underlying indirect section */
  unsigned H5_ATTR_NDEBUG_UNUSED
      indir_idx; /* Index of row in underlying indirect section's row array */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__sect_row_debug(const H5FS_section_info_t *_sect,
                                   FILE *stream, int indent, int fwidth) {
  const H5HF_free_section_t *sect =
      (const H5HF_free_section_t *)_sect; /* Section to dump info */
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Column:", sect->u.row.col);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Number of entries:", sect->u.row.num_entries);

  /* If this is a first row section display information about underlying
   * indirect section */
  if (sect->sect_info.type == H5HF_FSPACE_SECT_FIRST_ROW) {
    ...
    H5HF__sect_indirect_debug(sect->u.row.under, stream, indent + 3,
                              MAX(0, fwidth - 3));
  } /* end if */
...
static hsize_t H5HF__sect_indirect_iblock_off(const H5HF_free_section_t *sect) {
  ...
  ret_value = sect->sect_info.state == H5FS_SECT_LIVE
                  ? sect->u.indirect.u.iblock->block_off
...
H5HF__sect_indirect_new(H5HF_hdr_t *hdr, haddr_t sect_off, hsize_t sect_size,
  ...
  /* Create free space section node */
  if (NULL == (sect = H5FS__sect_node_new(
                   H5HF_FSPACE_SECT_INDIRECT, sect_off, sect_size,
                   (iblock ? H5FS_SECT_LIVE : H5FS_SECT_SERIALIZED))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "memory allocation failed for indirect section");

  /* Set the 'indirect' specific fields */
  if (iblock) {
    sect->u.indirect.u.iblock = iblock;
    sect->u.indirect.iblock_entries =
        hdr->man_dtable.cparam.width * sect->u.indirect.u.iblock->max_rows;
  ...
  sect->u.indirect.span_size =
      H5HF__dtable_span_size(&hdr->man_dtable, row, col, nentries);
...
H5HF__sect_indirect_for_row(H5HF_hdr_t *hdr, H5HF_indirect_t *iblock,
  ...
  /* Create free space section node */
  if (NULL == (sect = H5HF__sect_indirect_new(
                   hdr, row_sect->sect_info.addr, row_sect->sect_info.size,
  ...
  if (NULL == (sect->u.indirect.dir_rows = (H5HF_free_section_t **)H5MM_malloc(
                   sizeof(H5HF_free_section_t *))))
    HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, NULL,
                "allocation failed for row section pointer array");
  ...
  if (!ret_value && sect)
    if (H5HF__sect_indirect_free(sect) < 0)
      HDONE_ERROR(H5E_HEAP, H5E_CANTRELEASE, NULL,
                  "can't free indirect section node");
...
static herr_t H5HF__sect_indirect_init_rows(
  ...
  unsigned
      curr_indir_entry; /* Current indirect entry within indirect section */
  ...
  if (start_row < hdr->man_dtable.max_direct_rows) {
    ...
    /* Allocate space for the derived row sections */
    if (NULL == (sect->u.indirect.dir_rows = (H5HF_free_section_t **)
                     H5MM_malloc(sizeof(H5HF_free_section_t *) * dir_nrows)))
  ...
  if (end_row >= hdr->man_dtable.max_direct_rows) {
    ...
    sect->u.indirect.indir_nents =
        (indirect_end_entry - indirect_start_entry) + 1;

    /* Allocate space for the child indirect sections */
    if (NULL ==
        (sect->u.indirect.indir_ents = (H5HF_free_section_t **)H5MM_malloc(
  ...
  for (u = start_row; u <= end_row; u++, curr_row++) {
    if (u < hdr->man_dtable.max_direct_rows) {
      H5HF_free_section_t *row_sect =
          NULL; /* 'Row' free space section to add */

      /* Create 'row' free space section node */
      if (NULL ==
          (row_sect = H5HF__sect_row_create(
               curr_off, (hdr->man_dtable.row_block_size[u] - dblock_overhead),
               first_child, u, row_col, row_entries, sect)))
        HGOTO_ERROR(H5E_HEAP, H5E_CANTCREATE, FAIL,
                    "creation failed for child row section");

      /* Add new row section to array for indirect section */
      sect->u.indirect.dir_rows[curr_row] = row_sect;

      /* Check to see if we should grab the first row section instead of adding
       * it immediately */
      if (first_row_sect)
        *first_row_sect = row_sect;
        ...
        if (H5HF__space_add(hdr, row_sect, space_flags) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                      "can't add row section to free space");
      ...
      unsigned child_nrows; /* Number of child rows in indirect blocks for this
                               row */
      unsigned child_nentries; /* Number of child entries in indirect blocks for
                                  this row */
      unsigned v;              /* Local index variable */

      /* Compute info about row's indirect blocks for child section */
      child_nrows = H5HF__dtable_size_to_rows(
          &hdr->man_dtable, hdr->man_dtable.row_block_size[u]);
      ...
      for (v = 0; v < row_entries; v++) {
        hbool_t did_protect =
            FALSE; /* Whether we protected the indirect block or not */

        /* Try to get the child section's indirect block, if it's available */
        if (sect->sect_info.state == H5FS_SECT_LIVE) {
          haddr_t
              child_iblock_addr; /* Child indirect block's address on disk */
          ...
          if (H5_addr_defined(child_iblock_addr)) {
            if (NULL == (child_iblock = H5HF__man_iblock_protect(
                             hdr, child_iblock_addr, child_nrows,
        ...
        /* Create free space section node */
        if (NULL == (child_sect = H5HF__sect_indirect_new(
                         hdr, curr_off, (hsize_t)0, child_iblock, curr_off, 0,
                         0, child_nentries)))
          HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                      "can't create indirect section");

        /* Initialize rows for new indirect section */
        if (H5HF__sect_indirect_init_rows(
                hdr, child_sect, first_child, first_row_sect, space_flags, 0, 0,
                (child_nrows - 1), (hdr->man_dtable.cparam.width - 1)) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                      "can't initialize indirect section");

        /* If we have a valid child indirect block, release it now */
        /* (will be pinned, if rows reference it) */
        if (child_iblock)
          if (H5HF__man_iblock_unprotect(child_iblock, H5AC__NO_FLAGS_SET,
                                         did_protect) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                        "unable to release fractal heap indirect block");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__sect_indirect_add(H5HF_hdr_t *hdr, H5HF_indirect_t *iblock,
                               unsigned start_entry, unsigned nentries) {
  H5HF_free_section_t *sect = NULL; /* 'Indirect' free space section to add */
  H5HF_free_section_t *first_row_sect =
      NULL;                   /* First row section in new indirect section */
  ...
  for (u = 0; u < start_row; u++)
    sect_off +=
        hdr->man_dtable.row_block_size[u] * hdr->man_dtable.cparam.width;
  ...
  if (H5HF__sect_indirect_init_rows(hdr, sect, TRUE, &first_row_sect,
                                    H5FS_ADD_SKIP_VALID, start_row, start_col,
                                    end_row, end_col) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't initialize indirect section");
  ...
  if (ret_value < 0 && sect)
    if (H5HF__sect_indirect_free(sect) < 0)
      HDONE_ERROR(H5E_HEAP, H5E_CANTRELEASE, FAIL,
                  "can't free indirect section node");
...
static herr_t H5HF__sect_indirect_revive_row(H5HF_hdr_t *hdr,
  ...
  if (H5HF__man_dblock_locate(hdr, sect->sect_info.addr, &sec_iblock, NULL,
                              &did_protect, H5AC__READ_ONLY_FLAG) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTCOMPUTE, FAIL,
                "can't compute row & column of section");
  ...
  /* Unlock indirect block */
  if (sec_iblock && H5HF__man_iblock_unprotect(sec_iblock, H5AC__NO_FLAGS_SET,
                                               did_protect) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                "unable to release fractal heap indirect block");
...
static herr_t H5HF__sect_indirect_revive(H5HF_hdr_t *hdr,
  ...
  if (sect->u.indirect.parent &&
      sect->u.indirect.parent->sect_info.state == H5FS_SECT_SERIALIZED)
    if (H5HF__sect_indirect_revive(hdr, sect->u.indirect.parent,
...
static herr_t H5HF__sect_indirect_reduce_row(H5HF_hdr_t *hdr,
  ...
  row_start_entry = (row_sect->u.row.row * hdr->man_dtable.cparam.width) +
                    row_sect->u.row.col;
  ...
  assert(sect->u.indirect.dir_rows[(row_sect->u.row.row - start_row)] ==
         row_sect);
  ...
  if (sect->u.indirect.parent) {
    hbool_t is_first; /* Flag to indicate that this section is the first section
                         in hierarchy */
    ...
    if (H5HF__sect_indirect_reduce(hdr, sect->u.indirect.parent,
                                   sect->u.indirect.par_entry) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTSHRINK, FAIL,
                  "can't reduce parent indirect section");
    sect->u.indirect.parent = NULL;
    sect->u.indirect.par_entry = 0;

    /* If we weren't the first section, set "first row" for this indirect
     * section */
    if (!is_first)
  ...
  if (sect->u.indirect.num_entries > 1) {
    if (row_entry == start_entry) {
      /* Adjust section start */
      sect->sect_info.addr +=
          hdr->man_dtable.row_block_size[sect->u.indirect.row];
      ...
      if (sect->u.indirect.col == hdr->man_dtable.cparam.width) {
        ...
        if (sect->u.indirect.dir_nrows > 0) {
          ...
          sect->u.indirect.dir_rows =
              (H5HF_free_section_t **)H5MM_xfree(sect->u.indirect.dir_rows);

          /* Make new "first row" in indirect section */
          if (row_sect->sect_info.type == H5HF_FSPACE_SECT_FIRST_ROW)
            if (H5HF__sect_indirect_first(hdr, sect->u.indirect.indir_ents[0]) <
                0)
              HGOTO_ERROR(
                  H5E_HEAP, H5E_CANTINIT, FAIL,
      ...
      new_end_row = ((start_entry + sect->u.indirect.num_entries) - 1) /
                    hdr->man_dtable.cparam.width;
      ...
      hsize_t
          iblock_off; /* Section's indirect block's offset in "heap space" */
      unsigned
          peer_nentries; /* Number of entries in new peer indirect section */
      unsigned peer_dir_nrows; /* Number of direct rows in new peer indirect
                                  section */
      unsigned
          new_start_row; /* New starting row for current indirect section */
      unsigned u;        /* Local index variable */
      ...
      if (NULL == (peer_sect = H5HF__sect_indirect_new(
                       hdr, sect->sect_info.addr, sect->sect_info.size, iblock,
      ...
      if (NULL ==
          (peer_sect->u.indirect.dir_rows = (H5HF_free_section_t **)H5MM_malloc(
      ...
      /* Re-target transferred row sections to point to new underlying indirect
       * section */
      ...
      peer_sect->u.indirect.span_size =
          row_sect->sect_info.addr - peer_sect->sect_info.addr;

      /* Update information for current section */
      sect->sect_info.addr = row_sect->sect_info.addr +
                             hdr->man_dtable.row_block_size[new_start_row];
      sect->u.indirect.span_size -=
          peer_sect->u.indirect
              .span_size; /* (span for row section has already been removed) */
      sect->u.indirect.row = new_start_row;
      sect->u.indirect.col = row_sect->u.row.col + 1;
      sect->u.indirect.num_entries -=
          (peer_nentries + 1); /* Transferred entries, plus the entry allocated
                                  out of the row */

      /* Make certain we've tracked the sections' dependents correctly */
      assert(sect->u.indirect.rc ==
             (sect->u.indirect.indir_nents + sect->u.indirect.dir_nrows));
      assert(peer_sect->u.indirect.rc == (peer_sect->u.indirect.indir_nents +
                                          peer_sect->u.indirect.dir_nrows));

      /* Reset the peer_sect variable, to indicate that it has been hooked into
       * the data structures correctly and shouldn't be freed */
      peer_sect = NULL;
    } /* end else */
  }   /* end if */
  else {
  ...
  /* Free allocated peer_sect.  Note that this is necessary for all failures
   * until peer_sect is linked into the main free space structures (via the
   * direct blocks), and the reference count is updated. */
  if (peer_sect) {
...
static herr_t H5HF__sect_indirect_reduce(H5HF_hdr_t *hdr,
  ...
  if (sect->u.indirect.num_entries > 1) {
    /* Check if we have a parent section to be detached from */
    if (sect->u.indirect.parent) {
      hbool_t is_first; /* Flag to indicate that this section is the first
                           section in hierarchy */
    ...
    if (child_entry == start_entry) {
      ...
      if (sect->u.indirect.indir_nents == 0)
        sect->u.indirect.indir_ents =
            (H5HF_free_section_t **)H5MM_xfree(sect->u.indirect.indir_ents);
    } /* end if */
    else {
      H5HF_indirect_t *iblock; /* Pointer to indirect block for this section */
      hsize_t
      ...
      unsigned
          peer_start_col; /* Starting column for new peer indirect section */
      unsigned child_row; /* Row where child entry is located */
      unsigned
          new_nentries; /* New number of entries for current indirect section */
      unsigned u;       /* Local index variable */
      ...
      /* Update the number of entries in current section & calculate it's span
       * size */
      /* (Will use this to compute the section address for the peer section */
      sect->u.indirect.num_entries = new_nentries;
      sect->u.indirect.span_size =
          H5HF__dtable_span_size(&hdr->man_dtable, sect->u.indirect.row,
      ...
      /* Create peer indirect section */
      if (NULL ==
          (peer_sect = H5HF__sect_indirect_new(
               hdr, peer_sect_addr, sect->sect_info.size, iblock, iblock_off,
               peer_start_row, peer_start_col, peer_nentries)))
        HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                    "can't create indirect section");
      ...
      if (NULL == (peer_sect->u.indirect.indir_ents =
                       (H5HF_free_section_t **)H5MM_malloc(
      ...
      H5MM_memcpy(&peer_sect->u.indirect.indir_ents[0],
                  &sect->u.indirect.indir_ents[sect->u.indirect.indir_nents -
                                               peer_nentries],
                  (sizeof(H5HF_free_section_t *) * peer_nentries));
      sect->u.indirect.indir_nents -=
          (peer_nentries + 1); /* Transferred blocks, plus child entry */
      ...
      if (H5HF__sect_indirect_first(hdr, peer_sect->u.indirect.indir_ents[0]) <
          0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
  ...
  if (H5HF__sect_indirect_decr(sect) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTRELEASE, FAIL,
                "can't decrement section's ref. count ");
...
static herr_t H5HF__sect_indirect_first(H5HF_hdr_t *hdr,
  ...
  if (sect->u.indirect.dir_nrows > 0) {
    ...
    if (H5HF__sect_indirect_first(hdr, sect->u.indirect.indir_ents[0]) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTSET, FAIL,
                  "can't set child indirect section to be first row");
  } /* end else */
 ...
 */
static herr_t H5HF__sect_indirect_merge_row(H5HF_hdr_t *hdr,
                                            H5HF_free_section_t *row_sect1,
                                            H5HF_free_section_t *row_sect2) {
  H5HF_free_section_t *sect1,
      *sect2;            /* Indirect sections underlying row sections */
  ...
  if (sect2->u.indirect.dir_nrows > 0) {
    hsize_t sect1_iblock_off,
        sect2_iblock_off; /* Offset of indirect block underlying row section */
    unsigned new_dir_nrows1; /* New value for number of direct rows in first
                                section */
    unsigned src_row2;       /* Source row for copying from second section */
    unsigned
        nrows_moved2; /* Number of rows to move from second section to first */
    ...
    if (sect1_iblock_off == sect2_iblock_off && end_row1 == start_row2) {
      H5HF_free_section_t
          *last_row_sect1; /* Last row in first indirect section */

      /* Locate the last row section in first indirect section, if we don't
       * already have it */
      if (row_sect1->u.row.row != end_row1)
        last_row_sect1 =
            sect1->u.indirect.dir_rows[sect1->u.indirect.dir_nrows - 1];
      else
      ...
      assert((last_row_sect1->u.row.col + last_row_sect1->u.row.num_entries) ==
             row_sect2->u.row.col);
      ...
      new_dir_nrows1 =
          (sect1->u.indirect.dir_nrows + sect2->u.indirect.dir_nrows) - 1;
      ...
      new_dir_nrows1 =
          sect1->u.indirect.dir_nrows + sect2->u.indirect.dir_nrows;
    ...
    if (nrows_moved2 > 0) {
      H5HF_free_section_t *
          *new_dir_rows; /* Pointer to new array of direct row pointers */

      /* Extend the first section's row array */
      if (NULL == (new_dir_rows = (H5HF_free_section_t **)H5MM_realloc(
                       sect1->u.indirect.dir_rows,
                       sizeof(H5HF_free_section_t *) * new_dir_nrows1)))
        HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, FAIL,
                    "allocation failed for row section pointer array");
  ...
  if (sect2->u.indirect.indir_nents > 0) {
    unsigned new_indir_nents1; /* New value for number of indirect entries in
                                  first section */
    ...
    new_indir_nents1 =
        sect1->u.indirect.indir_nents + sect2->u.indirect.indir_nents;
    ...
    if (sect1->u.indirect.indir_ents == NULL) {
      ...
      H5HF_free_section_t *
          *new_indir_ents; /* Pointer to new array of indirect entries */

      /* Extend the first section's entry array */
      if (NULL == (new_indir_ents = (H5HF_free_section_t **)H5MM_realloc(
                       sect1->u.indirect.indir_ents,
                       sizeof(H5HF_free_section_t *) * new_indir_nents1)))
        HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, FAIL,
                    "allocation failed for row section pointer array");
      ...
      H5MM_memcpy(
          &sect1->u.indirect.indir_ents[sect1->u.indirect.indir_nents],
  ...
  if (merged_rows) {
    ...
    if (H5HF__space_add(hdr, row_sect2, H5FS_ADD_SKIP_VALID) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                  "can't re-add second row section to free space");
  } /* end else */
  ...
  if (sect1->u.indirect.iblock_entries == sect1->u.indirect.num_entries) {
    ...
    if (H5HF__sect_indirect_build_parent(hdr, sect1) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCREATE, FAIL,
                  "can't create parent for full indirect section");
  } /* end if */
...
static herr_t H5HF__sect_indirect_build_parent(H5HF_hdr_t *hdr,
  ...
  if (sect->u.indirect.u.iblock->parent) {
    ...
    if (H5HF__man_iblock_parent_info(hdr, sect->sect_info.addr, &par_block_off,
                                     &par_entry) < 0)
  ...
  if (NULL == (par_sect = H5HF__sect_indirect_new(
                   hdr, sect->sect_info.addr, sect->sect_info.size, par_iblock,
                   par_block_off, par_row, par_col, 1)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL, "can't create indirect section");
  ...
  if (NULL ==
      (par_sect->u.indirect.indir_ents =
...
static herr_t H5HF__sect_indirect_serialize(H5HF_hdr_t *hdr,
  ...
  /* Check if this indirect section has a parent & forward if this section is
   * first */
  if (sect->u.indirect.parent) {
    if (sect->sect_info.addr == sect->u.indirect.parent->sect_info.addr)
      if (H5HF__sect_indirect_serialize(hdr, sect->u.indirect.parent, buf) < 0)
        HGOTO_ERROR(
            H5E_HEAP, H5E_CANTSERIALIZE, FAIL,
    ...
    if (sect->sect_info.state == H5FS_SECT_LIVE) {
      assert(sect->u.indirect.u.iblock);
      UINT64ENCODE_VAR(buf, sect->u.indirect.u.iblock->block_off,
                       hdr->heap_off_size);
    } /* end if */
    else
...
H5HF__sect_indirect_deserialize(H5HF_hdr_t *hdr, const uint8_t *buf,
  ...
  /* Initialize rows for new indirect section */
  if (H5HF__sect_indirect_init_rows(
          hdr, new_sect, TRUE, NULL, H5FS_ADD_DESERIALIZING,
...
static herr_t H5HF__sect_indirect_valid(const H5HF_hdr_t *hdr,
  ...
  if (sect->u.indirect.dir_nrows > 0) {
    ...
    for (u = 0; u < dir_nrows; u++) {
      const H5HF_free_section_t H5_ATTR_NDEBUG_UNUSED
          *tmp_row_sect; /* Pointer to row section */
      ...
      if (u > 0) {
        const H5HF_free_section_t H5_ATTR_NDEBUG_UNUSED
            *tmp_row_sect2; /* Pointer to row section */

        tmp_row_sect2 = sect->u.indirect.dir_rows[u - 1];
        assert(tmp_row_sect2->u.row.row < tmp_row_sect->u.row.row);
        assert(H5_addr_lt(tmp_row_sect2->sect_info.addr,
                          tmp_row_sect->sect_info.addr));
  ...
  if (sect->u.indirect.indir_nents > 0) {
    ...
    for (u = 0; u < sect->u.indirect.indir_nents; u++) {
      const H5HF_free_section_t
          *tmp_child_sect; /* Pointer to child indirect section */
      ...
      if (u > 0) {
        ...
        assert(H5_addr_lt(tmp_child_sect2->sect_info.addr,
                          tmp_child_sect->sect_info.addr));
      } /* end if */
...
static herr_t H5HF__sect_indirect_debug(const H5HF_free_section_t *sect,
  ...
  /* Print indirect section information */
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Row:", sect->u.indirect.row);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Column:", sect->u.indirect.col);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Number of entries:", sect->u.indirect.num_entries);
