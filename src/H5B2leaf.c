...
/* Local Prototypes */
/********************/
static herr_t H5B2__shadow_leaf(H5B2_leaf_t *leaf,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__create_leaf(H5B2_hdr_t *hdr, void *parent,
  ...
  if (NULL == (leaf = H5FL_CALLOC(H5B2_leaf_t)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "memory allocation failed for B-tree leaf info");
  ...
  if (NULL == (leaf->leaf_native =
                   (uint8_t *)H5FL_FAC_MALLOC(hdr->node_info[0].nat_rec_fac)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "memory allocation failed for B-tree leaf native keys");
  memset(leaf->leaf_native, 0,
         hdr->cls->nrec_size * hdr->node_info[0].max_nrec);
  ...
  if (HADDR_UNDEF == (node_ptr->addr = H5MF_alloc(hdr->f, H5FD_MEM_BTREE,
    ...
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "file allocation failed for B-tree leaf node");

  /* Cache the new B-tree node */
  if (H5AC_insert_entry(hdr->f, H5AC_BT2_LEAF, node_ptr->addr, leaf,
                        H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_BTREE, H5E_CANTINIT, FAIL,
                "can't add B-tree leaf to cache");
  inserted = TRUE;
  ...
  if (ret_value < 0) {
    if (leaf) {
      ...
      if (H5B2__leaf_free(leaf) < 0)
        HDONE_ERROR(H5E_BTREE, H5E_CANTFREE, FAIL,
                    "unable to release v2 B-tree leaf node");
    } /* end if */
  }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
H5B2_leaf_t *H5B2__protect_leaf(H5B2_hdr_t *hdr, void *parent,
                                H5B2_node_ptr_t *node_ptr, hbool_t shadow,
                                unsigned flags) {
  ...
  if (NULL == (leaf = (H5B2_leaf_t *)H5AC_protect(
                   hdr->f, H5AC_BT2_LEAF, node_ptr->addr, &udata, flags)))
  ...
  if (hdr->top_proxy && NULL == leaf->top_proxy) {
    ...
    if (H5AC_proxy_entry_add_child(hdr->top_proxy, hdr->f, leaf) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTSET, NULL,
                  "unable to add v2 B-tree leaf node as child of proxy");
    leaf->top_proxy = hdr->top_proxy;
  } /* end if */
  ...
  if (!ret_value) {
    ...
    if (leaf) {
      ...
      /* Unprotect leaf node */
      if (H5AC_unprotect(hdr->f, H5AC_BT2_LEAF, node_ptr->addr, leaf,
                         H5AC__NO_FLAGS_SET) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__neighbor_leaf(H5B2_hdr_t *hdr, H5B2_node_ptr_t *curr_node_ptr,
  ...
  if (neighbor_loc) {
    ...
    HGOTO_ERROR(H5E_BTREE, H5E_NOTFOUND, FAIL,
                "unable to find neighbor record in B-tree");

done:
  /* Release the B-tree leaf node */
  if (leaf && H5AC_unprotect(hdr->f, H5AC_BT2_LEAF, curr_node_ptr->addr, leaf,
                             H5AC__NO_FLAGS_SET) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__insert_leaf(H5B2_hdr_t *hdr, H5B2_node_ptr_t *curr_node_ptr,
                         H5B2_nodepos_t curr_pos, void *parent, void *udata) {
  H5B2_leaf_t *leaf; /* Pointer to leaf node */
  unsigned leaf_flags =
      H5AC__NO_FLAGS_SET;     /* Flags for unprotecting the leaf node */
  ...
  if (leaf->nrec == 0)
    ...
    if (idx < leaf->nrec)
      memmove(H5B2_LEAF_NREC(leaf, hdr, idx + 1),
              H5B2_LEAF_NREC(leaf, hdr, idx),
  ...
  if ((hdr->cls->store)(H5B2_LEAF_NREC(leaf, hdr, idx), udata) < 0)
    HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, FAIL,
                "unable to insert record into leaf node");
  ...
  if (H5B2_POS_MIDDLE != curr_pos) {
      ...
      if (H5B2_POS_LEFT == curr_pos || H5B2_POS_ROOT == curr_pos) {
        if (hdr->min_native_rec == NULL)
          if (NULL == (hdr->min_native_rec = H5MM_malloc(hdr->cls->nrec_size)))
            HGOTO_ERROR(
                H5E_BTREE, H5E_CANTALLOC, FAIL,
                "memory allocation failed for v2 B-tree min record info");
        H5MM_memcpy(hdr->min_native_rec, H5B2_LEAF_NREC(leaf, hdr, idx),
                    hdr->cls->nrec_size);
      } /* end if */
    }   /* end if */
    if (idx == (unsigned)(leaf->nrec - 1)) {
      if (H5B2_POS_RIGHT == curr_pos || H5B2_POS_ROOT == curr_pos) {
        ...
        H5MM_memcpy(hdr->max_native_rec, H5B2_LEAF_NREC(leaf, hdr, idx),
                    hdr->cls->nrec_size);
      } /* end if */
    }   /* end if */
  }     /* end if */
    ...
    if (hdr->swmr_write && (leaf_flags & H5AC__DIRTIED_FLAG))
      if (H5B2__shadow_leaf(leaf, curr_node_ptr) < 0)
        HDONE_ERROR(H5E_BTREE, H5E_CANTCOPY, FAIL,
                    "unable to shadow leaf B-tree node");

    /* Unprotect leaf node */
    if (H5AC_unprotect(hdr->f, H5AC_BT2_LEAF, curr_node_ptr->addr, leaf,
                       leaf_flags) < 0)
      HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, FAIL,
                  "unable to release leaf B-tree node");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__update_leaf(H5B2_hdr_t *hdr, H5B2_node_ptr_t *curr_node_ptr,
                         H5B2_update_status_t *status, H5B2_nodepos_t curr_pos,
                         void *parent, void *udata, H5B2_modify_t op,
                         void *op_data) {
  H5B2_leaf_t *leaf; /* Pointer to leaf node */
  unsigned leaf_flags =
      H5AC__NO_FLAGS_SET;     /* Flags for unprotecting the leaf node */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__swap_leaf(H5B2_hdr_t *hdr, uint16_t depth,
  ...
  if (depth > 1) {
    ...
    if (NULL == (child_internal = H5B2__protect_internal(
                     hdr, internal, &internal->node_ptrs[idx],
    ...
    if (NULL == (child_leaf = H5B2__protect_leaf(hdr, internal,
                                                 &internal->node_ptrs[idx],
  ...
  H5MM_memcpy(hdr->page, H5B2_NAT_NREC(child_native, hdr, 0),
              hdr->cls->nrec_size);
  H5MM_memcpy(H5B2_NAT_NREC(child_native, hdr, 0), swap_loc,
              hdr->cls->nrec_size);
  ...
  *internal_flags_ptr |= H5AC__DIRTIED_FLAG;
  ...
  /* Unlock child node */
  if (child && H5AC_unprotect(hdr->f, child_class, child_addr, child,
...
static herr_t H5B2__shadow_leaf(H5B2_leaf_t *leaf,
  ...
  if (leaf->shadow_epoch <= hdr->shadow_epoch) {
    ...
    if (H5AC_move_entry(hdr->f, H5AC_BT2_LEAF, curr_node_ptr->addr,
                        new_node_addr) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__remove_leaf(H5B2_hdr_t *hdr, H5B2_node_ptr_t *curr_node_ptr,
                         H5B2_nodepos_t curr_pos, void *parent, void *udata,
                         H5B2_remove_t op, void *op_data) {
  H5B2_leaf_t *leaf;               /* Pointer to leaf node */
  haddr_t leaf_addr = HADDR_UNDEF; /* Leaf address on disk */
  unsigned leaf_flags =
      H5AC__NO_FLAGS_SET;     /* Flags for unprotecting leaf node */
  unsigned idx = 0;           /* Location of record which matches key */
  int cmp;                    /* Comparison value of records */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (op)
    if ((op)(H5B2_LEAF_NREC(leaf, hdr, idx), op_data) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTDELETE, FAIL,
                  "unable to remove record into leaf node");
    ...
    if (hdr->swmr_write) {
      if (H5B2__shadow_leaf(leaf, curr_node_ptr) < 0)
        HGOTO_ERROR(H5E_BTREE, H5E_CANTCOPY, FAIL,
                    "unable to shadow leaf node");
      leaf_addr = curr_node_ptr->addr;
    } /* end if */

    /* Pack record out of leaf */
    if (idx < leaf->nrec)
      memmove(H5B2_LEAF_NREC(leaf, hdr, idx),
              H5B2_LEAF_NREC(leaf, hdr, (idx + 1)),
              hdr->cls->nrec_size * (leaf->nrec - idx));
  ...
  /* Release the B-tree leaf node */
  if (leaf &&
      H5AC_unprotect(hdr->f, H5AC_BT2_LEAF, leaf_addr, leaf, leaf_flags) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, FAIL,
                "unable to release leaf B-tree node");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B2__remove_leaf_by_idx(H5B2_hdr_t *hdr, H5B2_node_ptr_t *curr_node_ptr,
...
herr_t H5B2__leaf_free(H5B2_leaf_t *leaf) {
  ...
  if (leaf->leaf_native)
    leaf->leaf_native = (uint8_t *)H5FL_FAC_FREE(
        leaf->hdr->node_info[0].nat_rec_fac, leaf->leaf_native);
 ...
 */
H5_ATTR_PURE herr_t
H5B2__assert_leaf2(const H5B2_hdr_t H5_ATTR_NDEBUG_UNUSED *hdr,
                   const H5B2_leaf_t H5_ATTR_NDEBUG_UNUSED *leaf,
