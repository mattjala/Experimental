 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_datatype_open(void *obj, const H5VL_loc_params_t *loc_params,
                                 const char *name, hid_t H5_ATTR_UNUSED tapl_id,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_datatype_get(void *obj, H5VL_datatype_get_args_t *args,
                                 hid_t H5_ATTR_UNUSED dxpl_id,
                                 void H5_ATTR_UNUSED **req) {
  H5T_t *dt = (H5T_t *)obj;
  ...
  case H5VL_DATATYPE_GET_BINARY: {
    if (H5T_encode(dt, args->args.get_binary.buf,
                   &args->args.get_binary.buf_size) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTSERIALIZE, FAIL,
                  "can't serialize datatype");
  ...
  case H5VL_DATATYPE_GET_TCPL: {
    if (H5I_INVALID_HID ==
        (args->args.get_tcpl.tcpl_id = H5T__get_create_plist(dt)))
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, FAIL,
...
herr_t H5VL__native_datatype_specific(void *obj,
  ...
  case H5VL_DATATYPE_FLUSH: {
    /* Currently, H6Oflush causes H5Fclose to trigger an assertion failure in
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_datatype_close(void *dt, hid_t H5_ATTR_UNUSED dxpl_id,
                                   void H5_ATTR_UNUSED **req) {
  herr_t ret_value = SUCCEED; /* Return value */
