...
bool lfht_add(struct lfht_t *lfht_ptr, unsigned long long int id, void *value) {
  ...
  success = lfht_add_internal(lfht_ptr, bucket_head_ptr, id, hash, false, value,
                              NULL);
  ...
  if (index_bits < lfht_ptr->max_index_bits) {
    ...
    if ((atomic_load(&(lfht_ptr->lfsll_log_len)) / curr_buckets_defined) >= 8) {
      ...
      /* attempt to increment lfht_ptr->index_bits and lfht_buckets_defined.
       * Must do this with a compare and exchange, as it is likely that other
       * threads will be attempting to do the same thing at more or less the
       * same time.
       *
      ...
      if (atomic_compare_exchange_strong(&(lfht_ptr->index_bits), &index_bits,
                                         index_bits + 1)) {

        /* set of lfht_ptr->index_bits succeeded -- must update
         * lfht_ptr->buckets_defined as well.  As it is possible that this
         * update could be interleaved with another index_bits increment, this
         * can get somewhat involved.
         *
         * For the first pass, use the computed current and new value for
         * buckets defined in the call to atomic_compare_exchange_strong(). If
         * this succeeds, we are done.
         *
         * Otherwise, load the current values of lfht_ptr->index_bits and
         * lfht_ptr->buckets_defined, and compute what lfht_ptr->buckets_defined
         * should be given the current value of lfht_ptr->index_bits.  If the
         * two values match, or if the actual value is greater than the computed
         * value, we are done.
         *
         * If not, call atomic_compare_exchange_strong() again to correct the
         * value of lfht_ptr->buckets_defined, and repeat as necessary.
         */
...
bool lfht_add_internal(struct lfht_t *lfht_ptr,
                       struct lfht_node_t *bucket_head_ptr,
                       unsigned long long int id, unsigned long long int hash,
  ...
  do {
     ...
     */
    lfht_find_mod_point(lfht_ptr, bucket_head_ptr, &first_node_ptr,
                        &second_node_ptr, &del_completion_cols,
                        &del_completions, &nodes_visited, hash);
    ...
    if (hash == first_node_ptr->hash) {
      ...
      if (atomic_compare_exchange_strong(&(first_node_ptr->next),
                                         &second_node_ptr, new_node_ptr)) {
  ...
  /* if appropriate, attempt to update lfht_ptr->max_lfsll_log_len and
   * lfht_ptr->max_lfsll_phys_len. In the event of a collision, just ignore it
   * and go on, as I don't see any reasonable way to recover.
   */
  if ((lfsll_log_len = atomic_load(&(lfht_ptr->lfsll_log_len))) >
    ...
    atomic_compare_exchange_strong(&(lfht_ptr->max_lfsll_log_len),
                                   &max_lfsll_log_len, lfsll_log_len);
  }
  ...
  if ((lfsll_phys_len = atomic_load(&(lfht_ptr->lfsll_phys_len))) >
    ...
    atomic_compare_exchange_strong(&(lfht_ptr->max_lfsll_phys_len),
                                   &max_lfsll_phys_len, lfsll_phys_len);
  }
  ...
  atomic_fetch_add(&(lfht_ptr->ins_restarts_due_to_del_col),
                   del_completion_cols);
...
void lfht_clear(struct lfht_t *lfht_ptr) {
  ...
  while (node_ptr) {
    ...
    if (discard_ptr->sentinel) {
      ...
      if (((unsigned long long)(node_ptr)) & 0x01ULL) {
        ...
        /* node_ptr is marked -- remove the mark and increment marked nodes
         * visited */
        node_ptr = (struct lfht_node_t *)(((unsigned long long)(node_ptr)) &
                                          (~0x01ULL));
  ...
  assert(atomic_load(&(lfht_ptr->buckets_initialized)) + 1 ==
         sentinel_nodes_discarded);

  assert(atomic_load(&(lfht_ptr->lfsll_phys_len)) ==
         sentinel_nodes_discarded + marked_nodes_discarded +
             unmarked_nodes_discarded);
...
void lfht_create_hash_bucket(struct lfht_t *lfht_ptr,
                             unsigned long long int hash, int index_bits) {
  bool result;
  ...
  if (NULL == atomic_load(&(lfht_ptr->bucket_idx[target_index]))) {
    ...
    /* it is possible that parent_index == target_index -- hence the following
     * check */
    ...
    if (NULL == atomic_load(&(lfht_ptr->bucket_idx[target_index]))) {
      ...
      if (lfht_add_internal(lfht_ptr, bucket_head_ptr, 0ULL, target_hash, true,
                            NULL, &sentinel_ptr)) {
        ...
        /* the attempt to insert the new sentinel node failed -- which means
         * that that the node already exists.  Thus if it hasn't been set
         * already, lfht_ptr->bucket_idx[target_index] will be set to point to
         * the new sentinel shortly.
         */
...
struct lfht_node_t *lfht_create_node(struct lfht_t *lfht_ptr,
                                     unsigned long long int id,
                                     unsigned long long int hash, bool sentinel,
                                     void *value) {
  ...
  while (!fl_search_done) {
    ...
    if ((test_sfirst.ptr == sfirst.ptr) && (test_sfirst.sn == sfirst.sn)) {
      ...
      if (sfirst.ptr == slast.ptr) {
        ...
        if (!atomic_compare_exchange_strong(&(lfht_ptr->fl_stail), &slast,
                                            new_slast)) {
        ...
        if (atomic_load(&(sfirst.ptr->ref_count)) > 0) {
          ...
          result = atomic_compare_exchange_strong(&(fl_node_ptr->snext), &snext,
                                                  new_snext);
          assert(result);
  ...
  if (NULL == fl_node_ptr) {
    ...
    fl_node_ptr =
        (struct lfht_fl_node_t *)malloc(sizeof(struct lfht_fl_node_t));
...
bool lfht_delete(struct lfht_t *lfht_ptr, unsigned long long int id) {
  ...
  do {
    ...
    if (hash == first_node_ptr->hash) {
       ...
       *
       * 2) another thread has either inserted a new node just after
       * *first_node_ptr or physically deleted *second_node_ptr.
       *
       * No worries if the former, but in latter case, we must try again.
       */
      marked_second_node_ptr =
          (struct lfht_node_t *)(((unsigned long long)(second_node_ptr)) |
                                 0x01ULL);
      ...
      if (atomic_compare_exchange_strong(&(first_node_ptr->next),
        ...
        /* decrement the logical lfsll length.  We will decrement the physical
         * list length when the node is physically deleted from the list.
         */
        ...
        /* recall that atomic_compare_exchamge_strong replaces the expected
         * value with the actual value on failure.  If the low order bit is set,
         * we are in case 1) above -- another thread beat us to marking
         * *first_node_ptr as deleted.
         */
  ...
  atomic_fetch_add(&(lfht_ptr->del_restarts_due_to_del_col),
                   (long long)del_completion_cols);
  atomic_fetch_add(&(lfht_ptr->del_deletion_completions),
                   (long long)del_completions);
  atomic_fetch_add(&(lfht_ptr->nodes_visited_during_dels),
...
void lfht_discard_node(struct lfht_t *lfht_ptr, struct lfht_node_t *node_ptr,
  ...
  result = atomic_compare_exchange_strong(&(fl_node_ptr->tag), &in_use_tag,
                                          LFHT_FL_NODE_ON_FL);
  assert(result);
  ...
  while (!done) {
    ...
    if ((test_fl_stail.ptr == fl_stail.ptr) &&
      ...
      if (NULL == fl_snext.ptr) {
        ...
        /* attempt to append fl_node_ptr by setting fl_tail->next to
         * fl_node_ptr. If this succeeds, update stats and attempt to set
         * lfht_ptr->fl_tail to fl_node_ptr as well.  This may or may not
         * succeed, but in either case we are done.
         */
        new_fl_snext.ptr = fl_node_ptr;
        new_fl_snext.sn = fl_snext.sn + 1;
        if (atomic_compare_exchange_strong(&(fl_stail.ptr->snext), &fl_snext,
                                           new_fl_snext)) {
          ...
          if (!atomic_compare_exchange_strong(&(lfht_ptr->fl_stail), &fl_stail,
                                              new_fl_stail)) {
          ...
          if ((fl_len = atomic_load(&(lfht_ptr->fl_len))) >
            ...
            atomic_compare_exchange_strong(&(lfht_ptr->max_fl_len), &max_fl_len,
                                           fl_len);
          }
        ...
        * of lfht_ptr->fl_len appears to be correct.  This issue must be        \
        * resolved,  but since turning off code to discard excess free list                        \
        * entries doesn't  cause any functional problems, I  am bypassing the                            \
        * issue for now.                                                        \
        */
...
void lfht_dump_list(struct lfht_t *lfht_ptr, FILE *file_ptr) {
  ...
  fprintf(file_ptr,
          "\nLFSLL Logical / Physical Length = %lld/%lld, Free List Len = "
          "%lld.\n\n",
          atomic_load(&(lfht_ptr->lfsll_log_len)),
          atomic_load(&(lfht_ptr->lfsll_phys_len)),
  ...
  while (node_ptr) {
    ...
    fprintf(file_ptr,
            "Node num = %lld, marked = %lld, sentinel = %d, id = 0x%lld, hash "
            "= 0x%llx, value = 0x%llx\n",
            node_num++,
...
void lfht_dump_stats(struct lfht_t *lfht_ptr, FILE *file_ptr) {
  ...
  fprintf(file_ptr, "\nFree List:\n");
  fprintf(file_ptr,
          "Max / current FL Length = %lld /%lld, Nodes added / deleted from "
          "free list = %lld / %lld\n",
  ...
  fprintf(file_ptr,
          "FL node ref count inc / decs = %lld / %lld, ref count inc retrys = "
          "%lld.\n",
  ...
  fprintf(file_ptr,
          "Nodes allocated / freed = %lld / %lld, candidate selection for free "
          "retries = %lld\n",
  ...
  fprintf(file_ptr, "\nHash Buckets:\n");
  fprintf(file_ptr,
          "Hash buckets defined / initialized = %lld / %lld, index_bits = %d, "
          "max index_bits = %d\n",
  ...
  fprintf(file_ptr,
          "Index bits incr cols = %lld, buckets defined update cols / retries "
          "= %lld / %lld.\n",
  ...
  fprintf(file_ptr,
          "recursive bucket inits = %lld, sentinels traversed = %lld.\n",
          atomic_load(&(lfht_ptr->recursive_bucket_inits)),
          atomic_load(&(lfht_ptr->sentinels_traversed)));

  fprintf(file_ptr, "\nInsertions:\n");
  fprintf(file_ptr,
          "successful / failed = %lld/%lld, ins / del cols = %lld/%lld\n",
          atomic_load(&(lfht_ptr->insertions)),
          atomic_load(&(lfht_ptr->insertion_failures)),
  ...
  fprintf(file_ptr, "\nDeletions:\n");
  fprintf(file_ptr,
          "attempted / failed = %lld/%lld, starts / start cols = %lld/%lld, "
          "retries = %lld\n",
  ...
  fprintf(
      file_ptr,
      "del completions = %lld, del col restarts = %lld, nodes visited = %lld\n",
  ...
  fprintf(file_ptr, "attempted / successful / failed = %lld/%lld/%lld\n",
          atomic_load(&(lfht_ptr->searches)),
          atomic_load(&(lfht_ptr->successful_searches)),
          atomic_load(&(lfht_ptr->failed_searches)));
  fprintf(file_ptr,
          "marked/unmoard nodes visited in: successful search %lld/%lld, "
          "failed search %lld/%lld\n",
  ...
  if (atomic_load(&(lfht_ptr->value_searches)) > 0LL) {
    ...
    fprintf(file_ptr,
            "marked/unmoard nodes visited in value searches %lld/%lld, "
            "sentinels traversed %lld\n",
  ...
  if (atomic_load(&(lfht_ptr->itter_inits)) > 0LL) {
    ...
    fprintf(file_ptr, "initiated / nexts / completed = %lld/%lld/%lld\n",
            atomic_load(&(lfht_ptr->itter_inits)),
            atomic_load(&(lfht_ptr->itter_nexts)),
            atomic_load(&(lfht_ptr->itter_ends)));
    fprintf(file_ptr,
            "marked/unmoard nodes visited in itterations %lld/%lld, sentinels "
            "traversed %lld\n",
...
bool lfht_find(struct lfht_t *lfht_ptr, unsigned long long int id,
               void **value_ptr)
...
bool lfht_find_id_by_value(struct lfht_t *lfht_ptr,
                           unsigned long long int *id_ptr, void *value)

{
  ...
  while ((node_ptr) && (!success)) {
    ...
    if (((unsigned long long)(next_ptr)) & 0x01ULL) {
      ...
      /* node is marked -- remove the mark and increment marked nodes visited */
      next_ptr =
          (struct lfht_node_t *)(((unsigned long long)(next_ptr)) & (~0x01ULL));
  ...
  /* it is tempting to assert that lfht_ptr->lfsll_log_len == 0 if success is
   * false.
   *
   * However, there are two problems with this.
   *
   * First, lfsll_log_len is updated only after the fact, and thus will be
   * briefly incorrect after each insertion and deletion.
   *
   * Second, the search for the first element will fail if entries are inserted
   * at the front of the lfsll after the scan for the first element has passed.
   *
   * This is OK, as the result will be correct for some ordering of the
   * insertions and the search for the first element.  If the user wishes to
   * avoid this race, it is his responsibility to ensure that the hash table is
   * quiecent during iterations.
   */
...
void lfht_find_mod_point(struct lfht_t *lfht_ptr,
                         ...
                         struct lfht_node_t **second_ptr_ptr, int *cols_ptr,
                         int *dels_ptr, int *nodes_visited_ptr,
                         unsigned long long int hash) {
  bool done = false;
  bool retry = false;
  int cols = 0;
  ...
  do {
    ...
    do {
      ...
      while (((unsigned long long)(third_ptr)) & 0x01ULL) {
         ...
         */
        third_ptr = (struct lfht_node_t *)(((unsigned long long)(third_ptr)) &
                                           ~0x01ULL);

        assert(third_ptr);
        assert(LFHT_VALID_NODE == third_ptr->tag);

        if (!atomic_compare_exchange_strong(&(first_ptr->next), &second_ptr,
                                            third_ptr)) {
          ...
          /* unlink of *second_ptr succeeded.  Decrement the logical list
           * length, increment dels, increment nodes_visited, discard
           * *second_ptr, set second_ptr to third_ptr, and then load third_ptr
           */
 ...
 * lock free hash table.  Note that the itteration will almost certainly not be
 *in id order, and that entries added during the itteration may or may not be
 *included in the itteration.
 *
...
bool lfht_get_first(struct lfht_t *lfht_ptr, unsigned long long int *id_ptr,
                    void **value_ptr) {
  bool success = false;
  bool marked;
  unsigned long long int marked_nodes_visited = 0;
  ...
  atomic_fetch_add(&(lfht_ptr->marked_nodes_visited_in_itters),
                   marked_nodes_visited);
  atomic_fetch_add(&(lfht_ptr->unmarked_nodes_visited_in_itters),
                   unmarked_nodes_visited);
  atomic_fetch_add(&(lfht_ptr->sentinels_traversed), sentinels_traversed);
  atomic_fetch_add(&(lfht_ptr->sentinels_traversed_in_itters),
                   sentinels_traversed);
...
struct lfht_node_t *lfht_get_hash_bucket_sentinel(struct lfht_t *lfht_ptr,
                                                  unsigned long long int hash) {
  int index_bits;
  struct lfht_node_t *sentinel_ptr = NULL;
  ...
  if (NULL == atomic_load(&(lfht_ptr->bucket_idx[hash_index]))) {
    ...
    lfht_create_hash_bucket(lfht_ptr, hash,
 ...
 *
 * Otherwise, return the id and value of the next entry in *id_ptr and
 **value_ptr, and return true.
 *
 *                                                  JRM -- 7/16/23
 *
 * Changes:
...
bool lfht_get_next(struct lfht_t *lfht_ptr, unsigned long long int old_id,
  ...
  while ((node_ptr) && (!success)) {
    ...
    if (((unsigned long long)(next_ptr)) & 0x01ULL) {
      ...
      /* node is marked -- remove the mark from next_ptr, and increment marked
       * nodes visited */
      next_ptr =
          (struct lfht_node_t *)(((unsigned long long)(next_ptr)) & (~0x01ULL));
 ...
 *    set, set the first bit in the hash.  Then examine the LFHT__NUM_HASH_BITS
 *- 1th bit in the id, and if it is set, set the second bit in the hash.  Repeat
 *until the lower LFHT__NUM_HASH_BITS bits of the id have been examined, and the
 *    corresponding bits in the hash have been set where appropriate.  Observe
 *that hash now contains the lower LFHT__NUM_HASH_BITS bits from the id in
 *reverse order.
 *
 *    We must now modify hash so that if it sentinel hash, no id will hash to
 *it, and it will always be the smallest value in its bucket.  Do this by left
...
void lfht_init(struct lfht_t *lfht_ptr) {
   ...
   */
  fl_node_ptr = (struct lfht_fl_node_t *)lfht_create_node(lfht_ptr, 0ULL, 0ULL,
                                                          false, NULL);
  assert(fl_node_ptr);
  assert(LFHT_FL_NODE_IN_USE == fl_node_ptr->tag);
...
bool lfht_swap_value(struct lfht_t *lfht_ptr, unsigned long long int id,
                     void *new_value, void **old_value_ptr)
