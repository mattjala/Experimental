...
static herr_t H5D__compound_opt_read(size_t nelmts, H5S_sel_iter_t *iter,
                                     ...
                                     uint8_t *tconv_buf,
                                     void *user_buf /*out*/);
static herr_t H5D__compound_opt_write(size_t nelmts,
                                      const H5D_type_info_t *type_info,
                                      void *tconv_buf);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__scatter_file(const H5D_io_info_t *_io_info,
                                const H5D_dset_io_info_t *_dset_info,
                                H5S_sel_iter_t *iter, size_t nelmts,
                                const void *_buf) {
  ...
  if (NULL == (len = H5FL_SEQ_MALLOC(size_t, vec_size)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "can't allocate I/O length vector array");
  if (NULL == (off = H5FL_SEQ_MALLOC(hsize_t, vec_size)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "can't allocate I/O offset vector array");

  /* Loop until all elements are written */
  while (nelmts > 0) {
    /* Get list of sequences for selection to write */
    if (H5S_SELECT_ITER_GET_SEQ_LIST(iter, vec_size, nelmts, &nseq, &nelem, off,
                                     len) < 0)
    ...
    if ((*tmp_dset_info.layout_ops.writevv)(
            &tmp_io_info, &tmp_dset_info, nseq, &dset_curr_seq, len, off,
            (size_t)1, &mem_curr_seq, &mem_len, &mem_off) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_WRITEERROR, FAIL, "write error");

    /* Update buffer */
    tmp_dset_info.buf.cvp =
        (const uint8_t *)tmp_dset_info.buf.cvp + orig_mem_len;
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5D__gather_file(const H5D_io_info_t *_io_info,
                               const H5D_dset_io_info_t *_dset_info,
                               H5S_sel_iter_t *iter, size_t nelmts,
                               void *_buf /*out*/) {
  H5D_io_info_t tmp_io_info;        /* Temporary I/O info object */
  ...
  if (dxpl_vec_size > H5D_IO_VECTOR_SIZE)
    ...
    vec_size = H5D_IO_VECTOR_SIZE;
  if (NULL == (len = H5FL_SEQ_MALLOC(size_t, vec_size)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, 0,
                "can't allocate I/O length vector array");
  if (NULL == (off = H5FL_SEQ_MALLOC(hsize_t, vec_size)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, 0,
                "can't allocate I/O offset vector array");
  ...
  while (nelmts > 0) {
    ...
    if (H5S_SELECT_ITER_GET_SEQ_LIST(iter, vec_size, nelmts, &nseq, &nelem, off,
                                     len) < 0)
      HGOTO_ERROR(H5E_INTERNAL, H5E_UNSUPPORTED, 0,
                  "sequence length generation failed");
    ...
    if ((*tmp_dset_info.layout_ops.readvv)(
            &tmp_io_info, &tmp_dset_info, nseq, &dset_curr_seq, len, off,
            (size_t)1, &mem_curr_seq, &mem_len, &mem_off) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__scatter_mem(const void *_tscat_buf, H5S_sel_iter_t *iter,
 ...
 *-------------------------------------------------------------------------
 */
size_t H5D__gather_mem(const void *_buf, H5S_sel_iter_t *iter, size_t nelmts,
                       void *_tgath_buf /*out*/) {
  const uint8_t *buf =
      (const uint8_t *)_buf; /* Get local copies for address arithmetic */
 ...
 *
 * Purpose:	Perform scatter/gather ead from a contiguous [piece of a]
 *dataset.
 *
 * Return:	Non-negative on success/Negative on failure
 *
...
herr_t H5D__scatgath_read(const H5D_io_info_t *io_info,
  ...
  hbool_t mem_iter_init =
      FALSE; /* Memory selection iteration info has been initialized */
  H5S_sel_iter_t *bkg_iter = NULL; /* Background iteration info*/
  hbool_t bkg_iter_init =
      FALSE; /* Background iteration info has been initialized */
  H5S_sel_iter_t *file_iter = NULL; /* File selection iteration info*/
  hbool_t file_iter_init =
      FALSE;           /* File selection iteration info has been initialized */
  ...
  if (NULL == (mem_iter = H5FL_MALLOC(H5S_sel_iter_t)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "can't allocate memory iterator");
  if (NULL == (bkg_iter = H5FL_MALLOC(H5S_sel_iter_t)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "can't allocate background iterator");
  if (NULL == (file_iter = H5FL_MALLOC(H5S_sel_iter_t)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "can't allocate file iterator");

  /* Figure out the strip mine size. */
  if (H5S_select_iter_init(file_iter, dset_info->file_space,
  ...
  /* Start strip mining... */
  for (smine_start = 0; smine_start < dset_info->nelmts;
       smine_start += smine_nelmts) {
    size_t n; /* Elements operated on */

    assert(H5S_SELECT_ITER_NELMTS(file_iter) ==
           (dset_info->nelmts - smine_start));

    /* Determine strip mine size. First check if we're doing in-place type
     * conversion */
    if (in_place_tconv) {
      /* If this is not a selection I/O operation and there is a background
       * buffer, we cannot exceed request_nelmts.  It could be part of a
       * selection I/O operation if this read is used to fill in a nonexistent
       * chunk */
      ...
      tmp_buf = (uint8_t *)buf +
                dset_info->layout_io_info.contig_piece_info->buf_off +
    ...
    /* Fill background buffer here unless we will use H5D__compound_opt_read().
     * Must do this before
     * the read so the read buffer doesn't get wiped out if we're using in-place
     * type conversion */
     ...
     * and no conversion is needed, copy the data directly into user's buffer
     * and bypass the rest of steps.
     */
    if (H5D__SCATGATH_USE_CMPD_OPT_READ(dset_info, in_place_tconv)) {
      if (H5D__compound_opt_read(smine_nelmts, mem_iter, &dset_info->type_info,
                                 tmp_buf, buf /*out*/) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
      ...
      if (H5T_convert(dset_info->type_info.tpath,
                      dset_info->type_info.src_type_id,
                      dset_info->type_info.dst_type_id, smine_nelmts, (size_t)0,
                      (size_t)0, tmp_buf, io_info->bkg_buf) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, FAIL,
                    "datatype conversion failed");

      /* Do the data transform after the conversion (since we're using type
       * mem_type) */
 ...
 *
 * Purpose:	Perform scatter/gather write to a contiguous [piece of a]
 *dataset.
 *
 * Return:	Non-negative on success/Negative on failure
 *
...
herr_t H5D__scatgath_write(const H5D_io_info_t *io_info,
  ...
  file_iter_init = TRUE; /*file selection iteration info has been initialized */
  if (H5S_select_iter_init(mem_iter, dset_info->mem_space,
                           dset_info->type_info.src_type_size, 0) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to initialize memory selection information");
  mem_iter_init = TRUE; /*file selection iteration info has been initialized */
  if (H5S_select_iter_init(bkg_iter, dset_info->file_space,
                           dset_info->type_info.dst_type_size,
                           H5S_SEL_ITER_GET_SEQ_LIST_SORTED) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to initialize background selection information");
  ...
  for (smine_start = 0; smine_start < dset_info->nelmts;
    ...
    if (in_place_tconv) {
      /* If this is not a selection I/O operation and there is a background
       * buffer, we cannot exceed request_nelmts.  It could be part of a
       * selection I/O operation if this is used to write the fill value to a
       * cached chunk that will immediately be evicted. */
      ...
      /* Calculate buffer position in user buffer */
      /* Use "vp" field of union to twiddle away const.  OK because if we're
       * doing this it means the user explicitly allowed us to modify this
       * buffer via H5Pset_modify_write_buf(). */
      tmp_buf = (uint8_t *)dset_info->buf.vp +
                dset_info->layout_io_info.contig_piece_info->buf_off +
                (smine_start * dset_info->type_info.dst_type_size);
    } else {
      /* Do type conversion using intermediate buffer */
    ...
    if (H5D__SCATGATH_USE_CMPD_OPT_WRITE(dset_info, in_place_tconv)) {
      if (H5D__compound_opt_write(smine_nelmts, &dset_info->type_info,
                                  tmp_buf) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                    "datatype conversion failed");
     ...
     */
    if (H5D__scatter_file(io_info, dset_info, file_iter, smine_nelmts,
                          tmp_buf) < 0)
...
herr_t H5D__scatgath_read_select(H5D_io_info_t *io_info) {
  ...
  size_t tconv_bytes_used =
      0; /* Number of bytes used so far in conversion buffer */
  size_t bkg_bytes_used =
      0;    /* Number of bytes used so far in background buffer */
  ...
  if (NULL == (tmp_bufs = H5MM_malloc(io_info->pieces_added * sizeof(void *))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "memory allocation failed for temporary buffer list");
  ...
  /*!FIXME delay doing this until we find the first mem space that is
   * non-contiguous or doesn't start at 0
   */
  if (NULL ==
      (tmp_mem_spaces = H5MM_malloc(io_info->pieces_added * sizeof(H5S_t *))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
  ...
  for (i = 0; i < io_info->pieces_added; i++) {
    ...
    if (dset_info->type_info.is_xform_noop &&
        dset_info->type_info.is_conv_noop) {
      ...
      if (NULL == (tmp_mem_spaces[i] = H5S_create_simple(
                       1, &io_info->sel_pieces[i]->piece_points, NULL))) {
        memset(&tmp_mem_spaces[i], 0,
               (io_info->pieces_added - i) * sizeof(tmp_mem_spaces[0]));
      ...
      if (io_info->sel_pieces[i]->in_place_tconv)
        ...
        tmp_bufs[i] =
            (uint8_t *)(io_info->rbufs[i]) + io_info->sel_pieces[i]->buf_off;
      else {
        ...
        tconv_bytes_used += io_info->sel_pieces[i]->piece_points *
                            MAX(dset_info->type_info.src_type_size,
                                dset_info->type_info.dst_type_size);
        assert(tconv_bytes_used <= io_info->tconv_buf_size);
      }

      /* Fill background buffer here unless we will use
       * H5D__compound_opt_read().  Must do this before the read so the read
       * buffer doesn't get wiped out if we're using in-place type conversion */
      if (!H5D__SCATGATH_USE_CMPD_OPT_READ(
              dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
        ...
        if (dset_info->type_info.need_bkg) {
          ...
          bkg_bytes_used += io_info->sel_pieces[i]->piece_points *
                            dset_info->type_info.dst_type_size;
          ...
          if (H5T_BKG_YES == dset_info->type_info.need_bkg) {
            ...
            if (H5S_select_iter_init(mem_iter, io_info->mem_spaces[i],
              ...
              HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                          "unable to initialize memory selection information");
            mem_iter_init =
                TRUE; /* Memory selection iteration info has been initialized */
  ...
  if (H5F_shared_select_read(io_info->f_sh, H5FD_MEM_DRAW,
                             (uint32_t)io_info->pieces_added, tmp_mem_spaces,
                             io_info->file_spaces, io_info->addrs,
  ...
  /* Perform type conversion and scatter data to memory buffers for datasets
   * that need this */
  for (i = 0; i < io_info->pieces_added; i++) {
    H5D_dset_io_info_t *dset_info = io_info->sel_pieces[i]->dset_info;
    ...
    if (tmp_mem_spaces[i] != io_info->mem_spaces[i]) {
      ...
      /* If the source and destination are compound types and subset of each
       * other and no conversion is needed, copy the data directly into user's
       * buffer and bypass the rest of steps.
       */
      if (H5D__SCATGATH_USE_CMPD_OPT_READ(
              dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
        if (H5D__compound_opt_read((size_t)io_info->sel_pieces[i]->piece_points,
                                   mem_iter, &dset_info->type_info, tmp_bufs[i],
        ...
        if (H5T_convert(dset_info->type_info.tpath,
                        ...
                        (size_t)io_info->sel_pieces[i]->piece_points, (size_t)0,
                        (size_t)0, tmp_bufs[i], tmp_bkg_buf) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, FAIL,
                      "datatype conversion failed");
         ...
         */
        if (!io_info->sel_pieces[i]->in_place_tconv)
          if (H5D__scatter_mem(tmp_bufs[i], mem_iter,
                               (size_t)io_info->sel_pieces[i]->piece_points,
                               io_info->rbufs[i] /*out*/) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "scatter failed");
      }
...
herr_t H5D__scatgath_write_select(H5D_io_info_t *io_info) {
  ...
  size_t bkg_bytes_used =
      0; /* Number of bytes used so far in background buffer */
  H5S_t **bkg_mem_spaces =
      NULL; /* Array of memory spaces for read to background buffer */
  H5S_t **bkg_file_spaces =
      NULL; /* Array of file spaces for read to background buffer */
  haddr_t *bkg_addrs =
      NULL; /* Array of file addresses for read to background buffer */
  size_t *bkg_element_sizes =
      NULL; /* Array of element sizes for read to background buffer */
  void **bkg_bufs =
      NULL; /* Array background buffers for read of existing file contents */
  size_t bkg_pieces =
      0; /* Number of pieces that need to read the background data from disk */
  size_t i;                   /* Local index variable */
  herr_t ret_value = SUCCEED; /* Return value		*/
  ...
  /* Allocate list of buffers (within the tconv buf) */
  if (NULL == (write_bufs = (const void **)H5MM_malloc(io_info->pieces_added *
                                                       sizeof(const void *))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "memory allocation failed for temporary buffer list");
   ...
   */
  if (NULL ==
      (write_mem_spaces = H5MM_malloc(io_info->pieces_added * sizeof(H5S_t *))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "memory allocation failed for temporary memory space list");
  ...
  for (i = 0; i < io_info->pieces_added; i++) {
    ...
    if (dset_info->type_info.is_xform_noop &&
      ...
      /* Create block memory space */
      if (NULL == (write_mem_spaces[i] = H5S_create_simple(
                       1, &io_info->sel_pieces[i]->piece_points, NULL)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, FAIL,
                    "unable to create simple memory dataspace");
      spaces_added++;
      ...
      if (io_info->sel_pieces[i]->in_place_tconv) {
        ...
        /* Use cast to union to twiddle away const.  OK because if we're doing
         * this it means the user explicitly allowed us to modify this buffer
         * via H5Pset_modify_write_buf(). */
        flex_buf.cvp = io_info->wbufs[i];
        tmp_write_buf =
            (uint8_t *)flex_buf.vp + io_info->sel_pieces[i]->buf_off;
      } else {
        /* Set buffer to point into type conversion buffer */
         ...
         * buffer */
        if ((size_t)io_info->sel_pieces[i]->piece_points !=
            H5D__gather_mem(io_info->wbufs[i], mem_iter,
                            (size_t)io_info->sel_pieces[i]->piece_points,
                            tmp_write_buf /*out*/))
          HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL, "mem gather failed");
      }
      ...
      /* If the source and destination are compound types and the destination is
       * a subset of the source and no conversion is needed, copy the data
       * directly into the type conversion buffer and bypass the rest of steps.
       * If the source is a subset of the destination, the optimization is done
       * in conversion function H5T_conv_struct_opt to protect the background
       * data.
       */
      if (H5D__SCATGATH_USE_CMPD_OPT_WRITE(
              dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
        if (H5D__compound_opt_write(
                (size_t)io_info->sel_pieces[i]->piece_points,
        ...
        if (H5T_BKG_YES == dset_info->type_info.need_bkg) {
          ...
          /* Allocate arrays of parameters for selection read to background
           * buffer if necessary */
          if (!bkg_mem_spaces) {
            assert(!bkg_file_spaces && !bkg_addrs && !bkg_element_sizes &&
                   !bkg_bufs);
            if (NULL == (bkg_mem_spaces = H5MM_malloc(io_info->pieces_added *
                                                      sizeof(H5S_t *))))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                          "memory allocation failed for memory space list");
            if (NULL == (bkg_file_spaces = H5MM_malloc(io_info->pieces_added *
                                                       sizeof(H5S_t *))))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                          "memory allocation failed for file space list");
            if (NULL == (bkg_addrs = H5MM_malloc(io_info->pieces_added *
                                                 sizeof(haddr_t))))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                          "memory allocation failed for piece address list");
            if (NULL == (bkg_element_sizes = H5MM_malloc(io_info->pieces_added *
                                                         sizeof(size_t))))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                          "memory allocation failed for element size list");
            if (NULL == (bkg_bufs = H5MM_malloc(io_info->pieces_added *
                                                sizeof(const void *))))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                          "memory allocation failed for write buffer list");
          }

          /* Use same (block) memory space, file space, address, and element
           * size as write operation
           */
          assert(bkg_mem_spaces && bkg_file_spaces && bkg_addrs &&
                 bkg_element_sizes && bkg_bufs);
          ...
          /* Perform type conversion here to avoid second loop if no dsets use
          ...
          if (H5T_convert(dset_info->type_info.tpath,
                          ...
                          dset_info->type_info.dst_type_id,
                          (size_t)io_info->sel_pieces[i]->piece_points,
                          (size_t)0, (size_t)0, tmp_write_buf, tmp_bkg_buf) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, FAIL,
                        "datatype conversion failed");
        }
      }
  ...
  if (io_info->must_fill_bkg) {
    ...
    if (H5F_shared_select_read(
            io_info->f_sh, H5FD_MEM_DRAW, (uint32_t)bkg_pieces, bkg_mem_spaces,
            bkg_file_spaces, bkg_addrs, bkg_element_sizes, bkg_bufs) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                  "selection read to background buffer failed");

    /* Perform type conversion on pieces with background buffers that were just
     * read */
    for (i = 0; i < io_info->pieces_added; i++) {
      H5D_dset_io_info_t *dset_info = io_info->sel_pieces[i]->dset_info;
      ...
      if ((H5T_BKG_YES == dset_info->type_info.need_bkg) &&
          !H5D__SCATGATH_USE_CMPD_OPT_WRITE(
              dset_info, io_info->sel_pieces[i]->in_place_tconv)) {
        /* Non-const write_buf[i].  Use pointer math here to avoid const
         * warnings.  When there's a background buffer write_buf[i] always
         * points inside the non-const tconv buf so this is OK. */
        void *tmp_write_buf = (void *)((uint8_t *)io_info->tconv_buf +
                                       ((const uint8_t *)write_bufs[i] -
                                        (const uint8_t *)io_info->tconv_buf));
        ...
        if (H5T_convert(dset_info->type_info.tpath,
                        ...
                        dset_info->type_info.dst_type_id,
                        (size_t)io_info->sel_pieces[i]->piece_points, (size_t)0,
                        (size_t)0, tmp_write_buf, bkg_bufs[j]) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, FAIL,
                      "datatype conversion failed");
  ...
  if (H5F_shared_select_write(io_info->f_sh, H5FD_MEM_DRAW,
                              (uint32_t)io_info->pieces_added, write_mem_spaces,
                              io_info->file_spaces, io_info->addrs,
  ...
  if (write_mem_spaces) {
    for (i = 0; i < spaces_added; i++) {
      ...
      if (write_mem_spaces[i] != io_info->mem_spaces[i] &&
          H5S_close(write_mem_spaces[i]) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL, "Can't close dataspace");
    }
...
static herr_t H5D__compound_opt_read(size_t nelmts, H5S_sel_iter_t *iter,
  ...
  while (nelmts > 0) {
    ...
    /* Get list of sequences for selection to write */
    if (H5S_SELECT_ITER_GET_SEQ_LIST(iter, vec_size, nelmts, &nseq, &elmtno,
                                     off, len) < 0)
      HGOTO_ERROR(H5E_INTERNAL, H5E_UNSUPPORTED, 0,
                  "sequence length generation failed");
...
static herr_t H5D__compound_opt_write(size_t nelmts,
                                      ...
                                      void *tconv_buf) {
  uint8_t *xsbuf,
      *xdbuf; /* Source & destination pointers into dataset buffer */
  size_t src_stride,
      dst_stride; /* Strides through source & destination datatypes */
  size_t i;       /* Local index variable */
