...
herr_t H5F_cwfs_add(H5F_t *f, H5HG_heap_t *heap) {
  ...
  if (NULL == f->shared->cwfs) {
    if (NULL == (f->shared->cwfs = (H5HG_heap_t **)H5MM_malloc(
                     H5F_NCWFS * sizeof(H5HG_heap_t *))))
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                  "can't allocate CWFS for file");
    ...
    for (i = H5F_NCWFS - 1; i >= 0; --i)
      if (H5HG_FREE_SIZE(f->shared->cwfs[i]) < H5HG_FREE_SIZE(heap)) {
        memmove(f->shared->cwfs + 1, f->shared->cwfs,
                (size_t)i * sizeof(H5HG_heap_t *));
        f->shared->cwfs[0] = heap;
        break;
      } /* end if */
  } else {
...
herr_t H5F_cwfs_find_free_heap(H5F_t *f, size_t need, haddr_t *addr) {
  ...
  hbool_t found =
      FALSE; /* Flag to indicate a heap with enough space was found */
  herr_t ret_value = SUCCEED; /* Return value */
    ...
    for (cwfsno = 0; cwfsno < f->shared->ncwfs; cwfsno++) {
      ...
      if ((H5HG_SIZE(f->shared->cwfs[cwfsno]) + new_need) <= H5HG_MAXSIZE) {
        ...
        was_extended = H5MF_try_extend(
            f, H5FD_MEM_GHEAP, H5HG_ADDR(f->shared->cwfs[cwfsno]),
...
herr_t H5F_cwfs_remove_heap(H5F_shared_t *shared, H5HG_heap_t *heap) {
  ...
  for (u = 0; u < shared->ncwfs; u++) {
    if (shared->cwfs[u] == heap) {
      shared->ncwfs -= 1;
      memmove(shared->cwfs + u, shared->cwfs + u + 1,
              (shared->ncwfs - u) * sizeof(H5HG_heap_t *));
      break;
    } /* end if */
  }   /* end for */
