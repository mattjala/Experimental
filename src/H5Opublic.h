...
#define H5O_COPY_SHALLOW_HIERARCHY_FLAG                                        \
  (0x0001u) /**< Copy only immediate members */
...
#define H5O_COPY_PRESERVE_NULL_FLAG                                            \
  (0x0020u) /**< Copy NULL messages (empty space) */
#define H5O_COPY_MERGE_COMMITTED_DTYPE_FLAG                                    \
  (0x0040u) /**< Merge committed datatypes in dest file */
#define H5O_COPY_ALL                                                           \
  (0x007Fu) /**< All object copying flags (for internal checking) */
...
#define H5O_SHMESG_SDSPACE_FLAG                                                \
  ((unsigned)1 << 0x0001) /**< Simple Dataspace Message.  */
...
#define H5O_SHMESG_PLINE_FLAG                                                  \
  ((unsigned)1 << 0x000b) /**< Filter pipeline message.  */
...
#define H5O_INFO_BASIC                                                         \
  0x0001u /**< Fill in the fileno, addr, type, and rc fields */
#define H5O_INFO_TIME                                                          \
  0x0002u /**< Fill in the atime, mtime, ctime, and btime fields */
...
#define H5O_IS_TOKEN_UNDEF(token)                                              \
  (!memcmp(&(token), &(H5O_TOKEN_UNDEF), sizeof(H5O_token_t)))
...
typedef struct H5O_hdr_info_t {
  ...
  struct {
    uint64_t
        present; /**< Flags to indicate presence of message type in header */
 ...
 * (For H5Oget_native_info(), H5Oget_native_info_by_name(),
 * H5Oget_native_info_by_idx())
 */
typedef struct H5O_native_info_t {
  H5O_hdr_info_t hdr; /**< Object header information */
  struct {
    H5_ih_info_t obj;  /**< v1/v2 B-tree & local/fractal heap for groups, B-tree
 ...
 * \param[in,out] op_data User-defined pointer to data required by the
 * application in processing the object; a pass-through of the \c op_data
 *                        pointer provided with the H5Ovisit3() function call
 * \return \herr_t_iter
 *
 */
typedef herr_t (*H5O_iterate2_t)(hid_t obj, const char *name,
...
typedef enum H5O_mcdt_search_ret_t {
  ...
  H5O_MCDT_SEARCH_CONT,       /**< Continue the global search of all committed
                               * datatypes in the destination file
                               */
  H5O_MCDT_SEARCH_STOP        /**< Stop the search, but continue copying.  The
                                 committed datatype will be copied        but not merged. */
 ...
 *          specified by a location, \p loc_id, and a path name, \p name, in an
 *HDF5 file.
 *
 *          This function opens the object in the same manner as H5Gopen(),
 *H5Topen(), and H5Dopen(). However, H5Oopen() does not require the type of
 *object to be known beforehand. This can be useful with user-defined links, for
 *instance, when only a path may be known.
 *
 *          H5Oopen() cannot be used to open a dataspace, attribute, property
 *list, or file.
 *
 ...
 *attribute. If an attribute is specified for \p loc_id then the object where
 *the attribute is attached will be accessed.
 *
 *          \p name must be the path to that object relative to \p loc_id.
 *
 *          \p lapl_id is the link access property list associated with the link
 *pointing to the object.  If default link access properties are appropriate,
 *this can be passed in as #H5P_DEFAULT.
 *
 ...
 */
#ifndef H5_DOXYGEN
H5_DLL hid_t H5Oopen_async(const char *app_file, const char *app_func,
                           unsigned app_line, hid_t loc_id, const char *name,
                           hid_t lapl_id, hid_t es_id);
#else
H5_DLL hid_t H5Oopen_async(hid_t loc_id, const char *name, hid_t lapl_id,
                           hid_t es_id);
#endif
 ...
 *
 * \fgdta_loc_obj_id{loc_id}
 * \param[in] group_name Name of group, relative to \p loc_id, in which object
 *is located \idx_type \order \param[in] n Object to open \lapl_id
 *
 * \return \hid_tv{object}
 *
 * \details H5Oopen_by_idx() opens the nth object in the group specified by \p
 *loc_id and \p group_name.
 *
 *          \p loc_id specifies a location identifier.
 *          \p group_name specifies the group relative to \p loc_id in which the
 *object can be found. If \p loc_id fully specifies the group in which the
 *object resides, \p group_name can be a dot (.).
 *
 *          The specific object to be opened within the group is specified by
 *the three parameters: \p idx_type, \p order and \p n.
 *
 *          \p idx_type specifies the type of index by which objects are
 *ordered. Valid index types include the following:
 *
 *          \indexes
 *
 *          \p order specifies the order in which the objects are to be
 *referenced for the purposes of this function.  Valid orders include the
 *following:
 *
 *          \orders
 *
 *          Note that for #H5_ITER_NATIVE, rather than implying a particular
 *order, it instructs the HDF5 library to iterate through the objects in the
 *fastest available order, i.e., in a natural order.
 *
 *          \p n specifies the position of the object within the index.  Note
 *that this count is zero-based; 0 (zero) indicates that the function will
 *return the value of the first object; if \p n is 5, the function will return
 *the value of the sixth object; etc.
 *
 *          \p lapl_id specifies the link access property list to be used in
 *accessing the object.
 *
 *          An object opened with this function should be closed when it is no
 *longer needed so that resource leaks will not develop.  H5Oclose() can be used
 *to close groups, datasets, or committed datatypes.
 *
 * \version 1.8.1 Fortran subroutine introduced in this release.
 *
 * \since 1.8.0
 ...
 */
#ifndef H5_DOXYGEN
H5_DLL hid_t H5Oopen_by_idx_async(const char *app_file, const char *app_func,
                                  unsigned app_line, hid_t loc_id,
                                  const char *group_name, H5_index_t idx_type,
                                  H5_iter_order_t order, hsize_t n,
                                  hid_t lapl_id, hid_t es_id);
#else
H5_DLL hid_t H5Oopen_by_idx_async(hid_t loc_id, const char *group_name,
                                  H5_index_t idx_type, H5_iter_order_t order,
 ...
 * \return \herr_t
 *
 * \details H5Oget_info3() specifies an object by its identifier, \p loc_id ,
 *and retrieves the metadata describing that object in \p oinfo.
 *
 *          The \p fields parameter contains flags to determine which fields
 *will be filled in the H5O_info2_t \c struct returned in \p oinfo. These flags
 *are defined in the H5Opublic.h file:
 *
 *          \obj_info_fields
 *
 * \par Example
...
H5_DLL herr_t H5Oget_info_by_name_async(const char *app_file,
                                        const char *app_func, unsigned app_line,
                                        hid_t loc_id, const char *name,
                                        H5O_info2_t *oinfo /*out*/,
                                        unsigned fields, hid_t lapl_id,
                                        hid_t es_id);
#else
H5_DLL herr_t H5Oget_info_by_name_async(hid_t loc_id, const char *name,
                                        H5O_info2_t *oinfo /*out*/,
                                        unsigned fields, hid_t lapl_id,
                                        hid_t es_id);
#endif
 ...
 *          The \p fields parameter contains flags to determine which fields
 *will be filled in the H5O_info2_t \c struct returned in \p oinfo. These flags
 *are defined in the H5Opublic.h file: \obj_info_fields
 *
 *          The link access property list, \c lapl_id, is not currently used;
 *          it should be passed in as #H5P_DEFAULT.
 *
 ...
 * \return \herr_t
 *
 * \details H5Oget_native_info() retrieves the native file format information
 *for an object specified by \p loc_id.
 *
 ...
 *
 */
H5_DLL herr_t H5Oget_native_info(hid_t loc_id, H5O_native_info_t *oinfo,
                                 unsigned fields);
 ...
 *
 */
H5_DLL herr_t H5Oget_native_info_by_name(hid_t loc_id, const char *name,
 ...
 * \return \herr_t
 *
 * \details H5Oget_native_info_by_idx() retrieves the native file format
 *information for an object specified by \p loc_id, group name, \p group_name,
 *the index by which objects in the group are tracked, \p idx_type, the order by
 *which the index is to be traversed, \p order , and an object's position \p n
 *within that index.
 *
 *          The \p fields parameter indicates which fields to fill in
 *H5O_native_info_t. Possible values defined in H5Opublic.h are: \snippet this
 *H5O_native_info_fields_snip
 *
 *          The link access property list, \c lapl_id, is not currently used;
 *          it should be passed in as #H5P_DEFAULT.
 ...
 *                       may be a file, group, dataset, named datatype or
 *attribute identifier. \param[in] new_name Name of link to be created, relative
 *to \p new_loc_id. \lcpl_id \lapl_id
 *
 * \return \herr_t
 *
 * \details H5Olink() creates a new hard link to an object in an HDF5 file.
 *          \p new_loc_id and \p \p new_link_name specify the location and name
 *of the new link, while \p object_id identifies the object that the link points
 *to.
 *
 ...
 * \return \herr_t
 *
 * \details H5Oincr_refcount() increments the hard link reference count for an
 *object. It should be used any time a user-defined link that references an
 *object by address is added. When the link is deleted, H5Odecr_refcount()
 *should be used.
 *
 ...
 * \warning Improper use can lead to inaccessible data, wasted space in the
 *file, or <b><em>file corruption</em></b>.
 *
 * \version 1.8.11 Fortran subroutine introduced in this release.
 *
 * \since 1.8.0
 ...
 * \return \herr_t
 *
 * \details H5Odecr_refcount() decrements the hard link reference count for an
 *object. It should be used any time a user-defined link that references an
 *object by address is deleted. In general, H5Oincr_refcount() will have been
 *used previously, when the link was created.
 *
 *          An object's reference count is the number of hard links in the
 *          file that point to that object. See the “Programming Model”
 *          section of the HDF5 Groups chapter in the <em>\ref UG</em>
 ...
 *              object copy property list \p ocpypl_id with
 *H5Pset_copy_object().
 *
 ...
 */
#ifndef H5_DOXYGEN
H5_DLL herr_t H5Ocopy_async(const char *app_file, const char *app_func,
...
#else
H5_DLL herr_t H5Ocopy_async(hid_t src_loc_id, const char *src_name,
 ...
 *          - An absolute name of the object, starting from \c /, the file's
 *root group
 ...
 *
 *          The H5Ovisit3() \p op_data parameter is a user-defined pointer to
 the data
 *          required to process objects in the course of the iteration. This
 pointer
 ...
 *
 */
H5_DLL herr_t H5Ovisit3(hid_t obj_id, H5_index_t idx_type,
 ...
 *          a file or an object in a file;  if \p loc_id is an attribute
 *identifier, the object where the attribute is attached will be used. \p
 *obj_name specifies either an object in the file (with an absolute name based
 *on the file's root group) or an object name relative to \p loc_id. If \p
 *loc_id fully specifies the object that is to serve as the root of the
 *iteration, \p obj_name should be '\c .' (a dot). (Note that when \p loc_id
 *fully specifies the object that is to serve as the root of the iteration, the
 *user may wish to consider using H5Ovisit3() instead of H5Ovisit_by_name3().)
 *
 *          Two parameters are used to establish the iteration: \p idx_type
 *          and \p order.
 *
 ...
 *
 *          The \p fields parameter contains flags to determine which fields
 *will be retrieved by the \p op callback function. These flags are defined in
 *the H5Opublic.h file: \obj_info_fields
 *
 *          H5Lvisit_by_name2() and H5Ovisit_by_name3() are companion
 *          functions: one for examining and operating on links; the other
 *          for examining and operating on the objects that those links point
 *to. Both functions ensure that by the time the function completes
 *          successfully, every link or object below the specified point
 *          in the file has been presented to the application for whatever
 ...
 */
#ifndef H5_DOXYGEN
H5_DLL herr_t H5Oclose_async(const char *app_file, const char *app_func,
                             unsigned app_line, hid_t object_id, hid_t es_id);
#else
 ...
 * \return \herr_t
 *
 * \details H5Oflush() causes all buffers associated with an object to be
 *immediately flushed to disk without removing the data from the cache.
 *
 *          The object associated with \p object_id can be any named object in
 *an HDF5 file, including a dataset, a group, or a committed datatype.
 *
 * \warning H5Oflush doesn't work correctly with parallel. It causes an
 *assertion failure in metadata cache during H5Fclose().
 *
 ...
 *      H5Pget_object_flush_cb(), H5Pset_append_flush(),
 *H5Pset_object_flush_cb() \see H5Oare_mdc_flushes_disabled(),
 ...
 */
#ifndef H5_DOXYGEN
H5_DLL herr_t H5Oflush_async(const char *app_file, const char *app_func,
                             unsigned app_line, hid_t obj_id, hid_t es_id);
#else
 ...
 * \return \herr_t
 *
 * \details H5Orefresh() causes all buffers associated with an object to be
 ...
 */
#ifndef H5_DOXYGEN
H5_DLL herr_t H5Orefresh_async(const char *app_file, const char *app_func,
                               unsigned app_line, hid_t oid, hid_t es_id);
#else
 ...
 *
 * \param[in] object_id Identifier of the object that will have flushes
 *disabled; may be a group, named datatype, or dataset identifier
 *
 * \return \herr_t
 *
 * \details H5Odisable_mdc_flushes(), H5Oenable_mdc_flushes() and associated
 *flush functions can be used to control the flushing of entries from a file's
 *metadata cache.
 *
 ...
 *          H5Fflush(), H5Dflush(), H5Gflush(), H5Tflush(), and H5Oflush()
 *calls.
 *
 ...
 * \ingroup H5O
 *
 * \brief Enables flushing of dirty metadata entries from a file's metadata
 *cache
 *
 * \param[in] object_id Identifier of the object that will have flushes
 *re-enabled; may be a group, named datatype, or dataset identifier
 *
 * \return \herr_t
 *
 ...
 *
 *          This function allows an object or cache's dirty metadata entries to
 *be flushed from the cache by the usual cache eviction/flush policy.
 *
 ...
 *disabled and \c 0 if it has not had flushes disabled. \return \herr_t
 *
 * \details H5Oare_mdc_flushes_disabled() determines if an HDF5 object (dataset,
 *group, committed datatype) has had flushes of metadata entries disabled.
 *
 *          The H5Oenable_mdc_flushes(), H5Odisable_mdc_flushes() and
 *          associated flush functions can be used to control the flushing of
 *          entries from a file's metadata cache. Metadata cache entries can be
 *controlled at both the individual HDF5 object level (datasets, groups,
 *          committed datatypes) and the entire metadata cache level.
 *
 * \note HDF5 objects include datasets, groups, and committed datatypes.
 *       Only #hid_t identifiers that represent these objects can be passed to
 *the function. \note Passing in a #hid_t identifier that represents any other
 *HDF5 entity is considered an error. \note It is an error to pass an HDF5 file
 *identifier (obtained from H5Fopen() or H5Fcreate()) to this function.
 *
 * \since 1.10.0
 *
 */
H5_DLL herr_t H5Oare_mdc_flushes_disabled(hid_t object_id,
 ...
 *          and \p token2 for the file or group identifier specified by \p
 *loc_id. Both object tokens must be from the same VOL connector class.
 *
 ...
 *
 */
H5_DLL herr_t H5Otoken_cmp(hid_t loc_id, const H5O_token_t *token1,
 ...
 *
 */
H5_DLL herr_t H5Otoken_to_str(hid_t loc_id, const H5O_token_t *token,
                              char **token_str);
...
#ifndef H5O_MODULE
#define H5Oopen_async(...)                                                     \
  H5Oopen_async(__FILE__, __func__, __LINE__, __VA_ARGS__)
#define H5Oopen_by_idx_async(...)                                              \
  H5Oopen_by_idx_async(__FILE__, __func__, __LINE__, __VA_ARGS__)
#define H5Oget_info_by_name_async(...)                                         \
  H5Oget_info_by_name_async(__FILE__, __func__, __LINE__, __VA_ARGS__)
#define H5Oclose_async(...)                                                    \
  H5Oclose_async(__FILE__, __func__, __LINE__, __VA_ARGS__)
#define H5Oflush_async(...)                                                    \
  H5Oflush_async(__FILE__, __func__, __LINE__, __VA_ARGS__)
#define H5Orefresh_async(...)                                                  \
  H5Orefresh_async(__FILE__, __func__, __LINE__, __VA_ARGS__)
#define H5Ocopy_async(...)                                                     \
  H5Ocopy_async(__FILE__, __func__, __LINE__, __VA_ARGS__)
...
#undef H5O_INFO_ALL
#define H5O_INFO_ALL                                                           \
  (H5O_INFO_BASIC | H5O_INFO_TIME | H5O_INFO_NUM_ATTRS | H5O_INFO_HDR |        \
   H5O_INFO_META_SIZE)
 ...
 * \param[out] info Information about that object
 * \param[in,out] op_data User-defined pointer to data required by the
 * application in processing the object \return \herr_t_iter
 *
 */
typedef herr_t (*H5O_iterate1_t)(hid_t obj, const char *name,
 ...
 *
 * \details H5Oopen_by_addr() opens a group, dataset, or committed (named)
 *datatype using its address within an HDF5 file, \p addr. The resulting opened
 *object is identical to an object opened with H5Oopen() and should be closed
 ...
 *
 *          The object's address within the file, \p addr, is the byte offset of
 *the first byte of the object header from the beginning of the HDF5 file space,
 *i.e., from the beginning of the superblock (see the “HDF5 Storage Model”
 *section of the The HDF5 Data Model and File Structure chapter of the <em>HDF5
 *User's Guide</em>.)
 *
 *          \p addr can be obtained via either of two function calls.
 *H5Gget_objinfo() returns the object's address in the \c objno field of the
 *H5G_stat_t \c struct; H5Lget_info() returns the address in the \c address
 *field of the #H5L_info_t \c struct.
 *
 *          The address of the HDF5 file on a physical device has no effect on
 *H5Oopen_by_addr(), nor does the use of any file driver. As stated above, the
 *object address is its offset within the HDF5 file; HDF5's file drivers will
 *transparently map this to an address on a storage device.
 *
 * \warning This function must be used with care!
 * \warning Improper use can lead to inaccessible data, wasted space in the
 *file, or <b><em>file corruption</em></b>. \warning This function is dangerous
 *if called on an invalid address. The risk can be safely overcome by retrieving
 *the object address with H5Gget_objinfo() or H5Lget_info() immediately before
 *calling H5Oopen_by_addr(). The immediacy of the operation can be important; if
 *time has elapsed and the object has been deleted from the file, the address
 *will be invalid, and file corruption can result.
 *
 ...
 *
 * \details H5Oget_info1() specifies an object by its identifier, \p loc_id ,
 *and retrieves the metadata describing that object in \p oinfo.
 *
 * \note If you are iterating through a lot of different objects to
 *       retrieve information via the H5Oget_info() family of routines,
 ...
 *                 Description section. \n
 *                 Clarified the definition and implementation of the time
 *fields.
 *
 * \since 1.8.0
 *
 ...
 *
 * \deprecated As of HDF5-1.12 this function has been deprecated in favor of
 *             the function H5Oget_info_by_name2() or the macro
 *#H5Oget_info_by_name.
 *
 * \details H5Oget_info_by_name1() specifies an object's location and name, \p
 *loc_id and \p name, respectively, and retrieves the metadata describing that
 *object in \p oinfo, an H5O_info1_t \c struct.
 *
 ...
 * \version 1.10.3 Function H5Oget_info_by_name() was copied to
 *H5Oget_info_by_name1() and the macro #H5Oget_info_by_name was created.
 * \version 1.8.8 Fortran 2003 subroutine and \c h5o_info_t derived type
 *introduced in this release.
 *
 * \since 1.8.0
 *
 */
H5_DLL herr_t H5Oget_info_by_name1(hid_t loc_id, const char *name,
 ...
 *
 * \deprecated As of HDF5-1.12 this function has been deprecated in favor of
 *             the function H5Oget_info_by_idx3() or the macro
 *H5Oget_info_by_idx().
 *
 ...
 * \version 1.10.3 Function H5Oget_info_by_idx() was copied to
 *H5Oget_info_by_idx1() and the macro #H5Oget_info_by_idx was created.
 * \version 1.8.11 Fortran subroutine introduced in this release.
 *
 * \since 1.8.0
 *
 ...
 *             the function H5Oget_info3() or the macro H5Oget_info().
 *
 * \details H5Oget_info2() specifies an object by its identifier, \p loc_id ,
 *and retrieves the metadata describing that object in \p oinfo , an H5O_info1_t
 *\c struct. This \c struct type is described in H5Oget_info1().
 *
 *          The \p fields parameter contains flags to determine which fields
 *will be filled in the H5O_info1_t \c struct returned in \p oinfo. These flags
 *are defined in the H5Opublic.h file:
 *
 *          \obj_info_fields
 *
 ...
 *
 * \deprecated As of HDF5-1.12 this function has been deprecated in favor of
 *             the function H5Oget_info_by_name3() or the macro
 *H5Oget_info_by_name().
 *
 * \details H5Oget_info_by_name2() specifies an object's location and name, \p
 *loc_id and \p name, respectively, and retrieves the metadata describing that
 *object in \p oinfo, an H5O_info1_t \c struct.
 *
 *          The \c struct H5O_info1_t is defined in H5Opublic.h and described
 ...
 *
 */
H5_DLL herr_t H5Oget_info_by_name2(hid_t loc_id, const char *name,
 ...
 *          \p n within that index.
 *
 *          \p oinfo, in which the object information is returned, is a \c
 *struct of type H5O_info1_t.  This and other \c struct types used by
 *H5Oget_info_by_idx2() are described in H5Oget_info_by_idx1().
 *
 *          If \p loc_id fully specifies the group in which the object resides,
 ...
 *
 */
H5_DLL herr_t H5Ovisit1(hid_t obj_id, H5_index_t idx_type,
 ...
 *iteration, \p obj_name should be '\c .' (a dot). (Note that when \p loc_id
 *fully specifies the object that is to serve as the root of the iteration, the
 *user may wish to consider using H5Ovisit1() instead of H5Ovisit_by_name1().)
 *
 *          Two parameters are used to establish the iteration: \p idx_type
 *          and \p order.
 *
 ...
 * \version 1.10.3 The H5Ovisit_by_name() function was renamed to
 *H5Ovisit_by_name1(), and the macro #H5Ovisit_by_name was created.
 * \version 1.8.11 Fortran subroutine introduced in this release.
 *
 * \since 1.8.0
 *
 ...
 *
 */
H5_DLL herr_t H5Ovisit2(hid_t obj_id, H5_index_t idx_type,
 ...
 *identifier, the object where the attribute is attached will be used. \p
 *obj_name specifies either an object in the file (with an absolute name based
 *in the file's root group) or an object name relative to \p loc_id. If \p
 *loc_id fully specifies the object that is to serve as the root of the
 *iteration, \p obj_name should be '\c .' (a dot). (Note that when \p loc_id
 *fully specifies the object that is to serve as the root of the iteration, the
 *user may wish to consider using H5Ovisit2() instead of #H5Ovisit_by_name.)
 *
 *          Two parameters are used to establish the iteration: \p idx_type
 *          and \p order.
 *
