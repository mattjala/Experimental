...
#define H5D_CHUNK_GET_NODE_INFO(dinfo, node)                                   \
  (dinfo->layout_io_info.chunk_map->use_single                                 \
       ? dinfo->layout_io_info.chunk_map->single_piece_info                    \
       : (H5D_piece_info_t *)H5SL_item(node))
#define H5D_CHUNK_GET_NEXT_NODE(dinfo, node)                                   \
  (dinfo->layout_io_info.chunk_map->use_single ? (H5SL_node_t *)NULL           \
                                               : H5SL_next(node))
...
/* Sanity check on chunk index types: commonly used by a lot of routines in this
 * file */
...
typedef struct H5D_rdcc_ent_t {
  ...
  struct H5D_rdcc_ent_t
      *tmp_next; /*next item in temporary doubly-linked list */
  struct H5D_rdcc_ent_t
      *tmp_prev; /*previous item in temporary doubly-linked list */
} H5D_rdcc_ent_t;
...
typedef struct H5D_chunk_it_ud1_t {
  H5D_chunk_common_ud_t
      common; /* Common info for B-tree user data (must be first) */
...
/* Callback info for iteration to obtain chunk address and the index of the
 * chunk for all chunks in the B-tree. */
...
typedef struct H5D_chunk_it_ud3_t {
  ...
  H5T_path_t
      *tpath_src_mem; /* Datatype conversion path from source file to memory */
  H5T_path_t
      *tpath_mem_dst; /* Datatype conversion path from memory to dest. file */
...
static herr_t H5D__chunk_io_init_selections(H5D_io_info_t *io_info,
                                            H5D_dset_io_info_t *dinfo);
static herr_t H5D__chunk_mdio_init(H5D_io_info_t *io_info,
                                   H5D_dset_io_info_t *dinfo);
static herr_t H5D__chunk_read(H5D_io_info_t *io_info,
                              H5D_dset_io_info_t *dinfo);
static herr_t H5D__chunk_write(H5D_io_info_t *io_info,
                               H5D_dset_io_info_t *dinfo);
static herr_t H5D__chunk_flush(H5D_t *dset);
static herr_t H5D__chunk_io_term(H5D_io_info_t *io_info,
                                 H5D_dset_io_info_t *di);
static herr_t H5D__chunk_dest(H5D_t *dset);

/* Chunk query operation callbacks */
static int H5D__get_num_chunks_cb(const H5D_chunk_rec_t *chunk_rec,
                                  void *_udata);
static int H5D__get_chunk_info_cb(const H5D_chunk_rec_t *chunk_rec,
                                  void *_udata);
static int H5D__get_chunk_info_by_coord_cb(const H5D_chunk_rec_t *chunk_rec,
                                           void *_udata);
static int H5D__chunk_iter_cb(const H5D_chunk_rec_t *chunk_rec, void *udata);

/* "Nonexistent" layout operation callback */
static ssize_t H5D__nonexistent_readvv(
    const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info,
...
/* Format convert cb */
static int H5D__chunk_format_convert_cb(const H5D_chunk_rec_t *chunk_rec,
                                        void *_udata);

/* Helper routines */
static herr_t H5D__chunk_set_info_real(H5O_layout_chunk_t *layout,
                                       unsigned ndims, const hsize_t *curr_dims,
...
static herr_t H5D__create_piece_map_single(H5D_dset_io_info_t *di,
                                           H5D_io_info_t *io_info);
static herr_t H5D__create_piece_file_map_all(H5D_dset_io_info_t *di,
                                             H5D_io_info_t *io_info);
static herr_t H5D__create_piece_file_map_hyper(H5D_dset_io_info_t *di,
                                               H5D_io_info_t *io_info);
...
static herr_t H5D__piece_file_cb(void *elem, const H5T_t *type, unsigned ndims,
                                 const hsize_t *coords, void *_opdata);
static herr_t H5D__piece_mem_cb(void *elem, const H5T_t *type, unsigned ndims,
                                const hsize_t *coords, void *_opdata);
static herr_t H5D__chunk_may_use_select_io(H5D_io_info_t *io_info,
                                           const H5D_dset_io_info_t *dset_info);
static unsigned H5D__chunk_hash_val(const H5D_shared_t *shared,
...
static herr_t H5D__chunk_unlock(const H5D_io_info_t *io_info,
                                const H5D_dset_io_info_t *dset_info,
                                const H5D_chunk_ud_t *udata, hbool_t dirty,
...
#ifdef H5_HAVE_PARALLEL
static herr_t H5D__chunk_collective_fill(
    const H5D_t *dset, H5D_chunk_coll_fill_info_t *chunk_fill_info,
    const void *fill_buf, const void *partial_chunk_fill_buf);
static int H5D__chunk_cmp_coll_fill_info(const void *_entry1,
                                         const void *_entry2);
#endif /* H5_HAVE_PARALLEL */

/* Debugging helper routine callback */
static int H5D__chunk_dump_index_cb(const H5D_chunk_rec_t *chunk_rec,
                                    void *_udata);
...
static const H5D_layout_ops_t H5D_LOPS_NONEXISTENT[1] = {
    {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, H5D__nonexistent_readvv,
     NULL, NULL, NULL, NULL}};
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_direct_write(H5D_t *dset, uint32_t filters, hsize_t *offset,
  ...
  hbool_t need_insert =
      FALSE; /* Whether the chunk needs to be inserted into the index */
  ...
  assert((H5_addr_defined(udata.chunk_block.offset) &&
         ...
         (!H5_addr_defined(udata.chunk_block.offset) &&
          udata.chunk_block.length == 0));
  ...
  if (0 == idx_info.pline->nused && H5_addr_defined(old_chunk.offset))
    /* If there are no filters and we are overwriting the chunk we can just set
     * values */
    need_insert = FALSE;
  else {
    /* Otherwise, create the chunk it if it doesn't exist, or reallocate the
     * chunk if its size has changed.
     */
    if (H5D__chunk_file_alloc(&idx_info, &old_chunk, &udata.chunk_block,
                              &need_insert, scaled) < 0)
  ...
  if (UINT_MAX != udata.idx_hint) {
    const H5D_rdcc_t *rdcc =
        &(dset->shared->cache.chunk); /*raw data chunk cache */
  ...
  if (H5F_shared_block_write(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW,
                             udata.chunk_block.offset, data_size, buf) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_direct_read(const H5D_t *dset, hsize_t *offset,
  ...
  *filters = 0;
  ...
  if (H5F_shared_block_read(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW,
                            udata.chunk_block.offset, udata.chunk_block.length,
                            buf) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__get_chunk_storage_size(H5D_t *dset, const hsize_t *offset,
  ...
  if (!H5_addr_defined(udata.chunk_block.offset) && UINT_MAX == udata.idx_hint)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "chunk storage is not allocated");
  ...
  if (dset->shared->dcpl_cache.pline.nused > 0) {
    ...
    if (!H5_addr_defined(udata.chunk_block.offset))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "chunk address isn't defined");

    /* Return the chunk size on disk */
    *storage_size = udata.chunk_block.length;
  }
  /* There are no filters registered, return the chunk size from the storage
     layout */
  else
 ...
 *
 * Purpose:     Internal routine to set the information about chunks for a
 *dataset
 *
 * Return:      SUCCEED/FAIL
 *-------------------------------------------------------------------------
...
static herr_t H5D__chunk_set_info_real(H5O_layout_chunk_t *layout,
  ...
  if (ndims == 0)
    HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                "number of dimensions cannot be zero");
  ...
  for (unsigned u = 0; u < ndims; u++) {
    ...
    if (H5S_UNLIMITED == max_dims[u])
      ...
      if (layout->dim[u] == 0)
        HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                    "dimension size must be > 0, dim = %u ", u);

      layout->max_chunks[u] =
          ((max_dims[u] + layout->dim[u]) - 1) / layout->dim[u];
    }
...
herr_t H5D__chunk_set_info(const H5D_t *dset) {
  ...
  if (H5D__chunk_set_info_real(&dset->shared->layout.u.chunk,
                               dset->shared->ndims, dset->shared->curr_dims,
                               dset->shared->max_dims) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "can't set layout's chunk info");

  /* Call the index's "resize" callback */
  if (dset->shared->layout.storage.u.chunk.ops->resize &&
      (dset->shared->layout.storage.u.chunk.ops->resize)(
          &dset->shared->layout.u.chunk) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "unable to resize chunk index information");
...
herr_t H5D__chunk_set_sizes(H5D_t *dset) {
  ...
  unsigned max_enc_bytes_per_dim; /* Max. number of bytes required to encode
                                     this dimension */
  unsigned u;                     /* Iterator */
  herr_t ret_value = SUCCEED;     /* Return value */
  ...
  /* Increment # of chunk dimensions, to account for datatype size as last
   * element */
  ...
  for (u = 0; u < (unsigned)dset->shared->layout.u.chunk.ndims; u++) {
    unsigned enc_bytes_per_dim; /* Number of bytes required to encode this
                                   dimension */

    /* Get encoded size of dim, in bytes */
    enc_bytes_per_dim =
        (H5VM_log2_gen(dset->shared->layout.u.chunk.dim[u]) + 8) / 8;
  ...
  H5_CHECKED_ASSIGN(dset->shared->layout.u.chunk.size, uint32_t, chunk_size,
                    uint64_t);
...
static herr_t H5D__chunk_construct(H5F_t H5_ATTR_UNUSED *f, H5D_t *dset) {
  ...
  assert((unsigned)(dset->shared->layout.u.chunk.ndims) <=
         NELMTS(dset->shared->layout.u.chunk.dim));
  ...
  for (u = 0; u < dset->shared->layout.u.chunk.ndims - 1; u++) {
     ...
     */
    if (dset->shared->curr_dims[u] &&
        dset->shared->max_dims[u] != H5S_UNLIMITED &&
        dset->shared->max_dims[u] < dset->shared->layout.u.chunk.dim[u])
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "chunk size must be <= maximum dimension size for "
  ...
  if (H5D_chunk_idx_reset(&dset->shared->layout.storage.u.chunk, TRUE) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to reset chunked storage index");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__chunk_init(H5F_t *f, const H5D_t *const dset,
                              hid_t dapl_id) {
  H5D_chk_idx_info_t idx_info; /* Chunked index info */
  H5D_rdcc_t *rdcc =
      &(dset->shared->cache
            .chunk);    /* Convenience pointer to dataset's chunk cache */
  ...
  /* Use the properties in dapl_id if they have been set, otherwise use the
   * properties from the file */
  if (H5P_get(dapl, H5D_ACS_DATA_CACHE_NUM_SLOTS_NAME, &rdcc->nslots) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get data cache number of slots");
  ...
  if (dset->shared->ndims > 1) {
    ...
    for (u = 0; u < dset->shared->ndims; u++) {
      ...
      /* Round up to the next integer # of chunks, to accommodate partial chunks
       */
      rdcc->scaled_dims[u] = (dset->shared->curr_dims[u] +
                              dset->shared->layout.u.chunk.dim[u] - 1) /
                             dset->shared->layout.u.chunk.dim[u];

      if (!(scaled_power2up = H5VM_power2up(rdcc->scaled_dims[u])))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                    "unable to get the next power of 2");
  ...
  if (sc->ops->init &&
      (sc->ops->init)(&idx_info, dset->shared->space, dset->oloc.addr) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "can't initialize indexing information");
  idx_init = true;

  /* Set the number of chunks in dataset, etc. */
  if (H5D__chunk_set_info(dset) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to set # of chunks for dataset");
...
static herr_t H5D__chunk_io_init(H5D_io_info_t *io_info,
  ...
  int sm_ndims;     /* The number of dimensions of the memory buffer's dataspace
                       (signed) */
  unsigned u;       /* Local index variable */
  herr_t ret_value = SUCCEED; /* Return value        */
  ...
  if ((sm_ndims = H5S_GET_EXTENT_NDIMS(dinfo->mem_space)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                "unable to get dimension number");
  ...
  if ((file_space_normalized =
           H5S_hyper_normalize_offset(dinfo->file_space, old_offset)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "unable to normalize selection");
  ...
  if (H5D__chunk_io_init_selections(io_info, dinfo) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to create file and memory chunk selections");
  ...
  if (io_info->use_select_io != H5D_SELECTION_IO_MODE_OFF)
    if (H5D__chunk_may_use_select_io(io_info, dinfo) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't check if selection I/O is possible");

  /* Calculate type conversion buffer size if necessary.  Currently only
   * implemented for selection I/O. */
  if (io_info->use_select_io != H5D_SELECTION_IO_MODE_OFF &&
...
static herr_t H5D__chunk_io_init_selections(H5D_io_info_t *io_info,
  ...
  if (dinfo->nelmts == 1
    ...
    if (NULL == dataset->shared->cache.chunk.single_space) {
      ...
      if ((dataset->shared->cache.chunk.single_space =
               H5S_copy(dinfo->file_space, TRUE, FALSE)) == NULL)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                    "unable to copy file space");

      /* Resize chunk's dataspace dimensions to size of chunk */
      if (H5S_set_extent_real(dataset->shared->cache.chunk.single_space,
                              fm->chunk_dim) < 0)
    ...
    if (NULL == dataset->shared->cache.chunk.sel_chunks)
      if (NULL == (dataset->shared->cache.chunk.sel_chunks =
                       H5SL_create(H5SL_TYPE_HSIZE, NULL)))
    ...
    if ((fm->fsel_type = H5S_GET_SELECT_TYPE(dinfo->file_space)) < H5S_SEL_NONE)
      HGOTO_ERROR(H5E_DATASET, H5E_BADSELECT, FAIL,
    ...
    if (sel_hyper_flag) {
      ...
      /* Spaces might not be the same shape, iterate over the file selection
       * directly */
      if (H5S_select_iterate(&bogus, file_type, dinfo->file_space, &iter_op,
                             &io_info_wrap) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                    "unable to create file chunk selections");
    ...
    if (sel_hyper_flag &&
        H5S_SELECT_SHAPE_SAME(dinfo->file_space, dinfo->mem_space) == TRUE) {
      /* Reset chunk template information */
      fm->mchunk_tmpl = NULL;
      ...
      if (H5D__create_piece_mem_map_hyper(dinfo) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                    "unable to create memory chunk selections");
    } /* end if */
    else if (sel_hyper_flag && fm->f_ndims == 1 && fm->m_ndims == 1 &&
             H5S_SELECT_IS_REGULAR(dinfo->mem_space) &&
             H5S_SELECT_IS_SINGLE(dinfo->mem_space)) {
      ...
      if (H5S_select_none(tmp_mspace) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                    "unable to de-select memory space");
      ...
      if (H5S_select_iter_init(&(fm->mem_iter), dinfo->mem_space, elmt_size,
                               0) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
      ...
      /* Spaces aren't the same shape, iterate over the memory selection
       * directly */
      if (H5S_select_iterate(&bogus, file_type, dinfo->file_space, &iter_op,
                             &io_info_wrap) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
  ...
  /* Release the [potentially partially built] chunk mapping information if an
   * error occurs */
  if (ret_value < 0) {
    ...
    if (H5D__chunk_io_term(io_info, dinfo) < 0)
      HDONE_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                  "unable to release chunk mapping");
  } /* end if */
  ...
  if (file_type && (H5T_close_real(file_type) < 0))
    HDONE_ERROR(H5E_DATATYPE, H5E_CANTFREE, FAIL,
                "Can't free temporary datatype");
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5D__free_piece_info(void *item, void H5_ATTR_UNUSED *key,
                            void H5_ATTR_UNUSED *opdata) {
...
static herr_t H5D__create_piece_map_single(H5D_dset_io_info_t *di,
  ...
  hsize_t
      sel_start[H5O_LAYOUT_NDIMS]; /* Offset of low bound of file selection */
  hsize_t
      sel_end[H5O_LAYOUT_NDIMS]; /* Offset of high bound of file selection */
  unsigned u;                    /* Local index variable */
  herr_t ret_value = SUCCEED;    /* Return value */
  ...
  if (H5S_SELECT_BOUNDS(di->file_space, sel_start, sel_end) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                "can't get file selection bound info");
  ...
  /* Calculate the index of this chunk */
  piece_info->index = H5VM_array_offset_pre(
      fm->f_ndims, di->layout->u.chunk.down_chunks, piece_info->scaled);

  /* Copy selection for file's dataspace into chunk dataspace */
  if (H5S_select_copy(fm->single_space, di->file_space, FALSE) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                "unable to copy file selection");
...
static herr_t H5D__create_piece_file_map_all(H5D_dset_io_info_t *di,
  ...
  hsize_t zeros[H5S_MAX_RANK];     /* All zero vector (for start parameter to
                                      setting hyperslab on partial     chunks) */
  hsize_t coords[H5S_MAX_RANK];    /* Current coordinates of chunk */
  hsize_t end[H5S_MAX_RANK];       /* Final coordinates of chunk */
  hsize_t scaled[H5S_MAX_RANK];    /* Scaled coordinates for this chunk */
  hsize_t chunk_index;             /* "Index" of chunk */
  hsize_t curr_partial_clip[H5S_MAX_RANK]; /* Current partial dimension sizes to
                                              clip against */
  hsize_t partial_dim_size[H5S_MAX_RANK];  /* Size of a partial dimension */
  hbool_t is_partial_dim[H5S_MAX_RANK];    /* Whether a dimension is currently a
                                              partial chunk */
  ...
  if (NULL ==
      (tmp_fchunk = H5S_create_simple(fm->f_ndims, fm->chunk_dim, NULL)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, FAIL,
                "unable to create dataspace for chunk");

  /* Iterate through each chunk in the dataset */
  while (sel_points) {
    H5D_piece_info_t
        *new_piece_info;  /* Piece information to insert into skip list */
    ...
    if (NULL == (new_piece_info->fspace = H5S_copy(tmp_fchunk, TRUE, FALSE)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                  "unable to copy chunk dataspace");
    new_piece_info->fspace_shared = FALSE;

    /* If there are partial dimensions for this chunk, set the hyperslab for
     * them */
    if (num_partial_dims > 0)
      if (H5S_select_hyperslab(new_piece_info->fspace, H5S_SELECT_SET, zeros,
                               NULL, curr_partial_clip, NULL) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTSELECT, FAIL,
    ...
    if (H5SL_insert(fm->dset_sel_pieces, new_piece_info,
  ...
  if (tmp_fchunk && H5S_close(tmp_fchunk) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTRELEASE, FAIL,
                "can't release temporary dataspace");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__create_piece_file_map_hyper(H5D_dset_io_info_t *dinfo,
                                               H5D_io_info_t *io_info) {
  H5D_chunk_map_t *fm;      /* Convenience pointer to chunk map */
  H5S_t *tmp_fchunk = NULL; /* Temporary file dataspace */
  hsize_t
  ...
  hsize_t sel_points;            /* Number of elements in file selection */
  hsize_t
      start_coords[H5O_LAYOUT_NDIMS]; /* Starting coordinates of selection */
  hsize_t coords[H5O_LAYOUT_NDIMS];   /* Current coordinates of chunk */
  hsize_t end[H5O_LAYOUT_NDIMS];      /* Final coordinates of chunk */
  hsize_t chunk_index;                /* Index of chunk */
  hsize_t
      start_scaled[H5S_MAX_RANK]; /* Starting scaled coordinates of selection */
  hsize_t scaled[H5S_MAX_RANK];   /* Scaled coordinates for this chunk */
  int curr_dim;                   /* Current dimension to increment */
  unsigned u;                     /* Local index variable */
  herr_t ret_value = SUCCEED;     /* Return value */
  ...
  /* Get bounding box for selection (to reduce the number of chunks to iterate
   * over) */
  ...
  /* Calculate the index of this chunk */
  chunk_index = H5VM_array_offset_pre(
      fm->f_ndims, dinfo->layout->u.chunk.down_chunks, scaled);
  ...
  while (sel_points) {
    ...
    if (TRUE == H5S_SELECT_INTERSECT_BLOCK(dinfo->file_space, coords, end)) {
      H5D_piece_info_t
          *new_piece_info;  /* chunk information to insert into skip list */
      hsize_t chunk_points; /* Number of elements in chunk selection */

      /* Create dataspace for chunk, 'AND'ing the overall selection with
       *  the current chunk.
       */
      if (H5S_combine_hyperslab(dinfo->file_space, H5S_SELECT_AND, coords, NULL,
                                fm->chunk_dim, NULL, &tmp_fchunk) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
      ...
      /* Copy the chunk's scaled coordinates */
      H5MM_memcpy(new_piece_info->scaled, scaled,
                  sizeof(hsize_t) * fm->f_ndims);
      new_piece_info->scaled[fm->f_ndims] = 0;
      ...
      if (H5SL_insert(fm->dset_sel_pieces, new_piece_info,
        ...
        H5D__free_piece_info(new_piece_info, NULL, NULL);
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
                    "can't insert piece into skip list");
      } /* end if */
    ...
    if (coords[curr_dim] > sel_end[curr_dim]) {
      do {
        ...
        coords[curr_dim] =
            start_coords[curr_dim]; /*lint !e771 The start_coords will always be
                                       initialized */
        end[curr_dim] = (coords[curr_dim] + fm->chunk_dim[curr_dim]) - 1;
...
static herr_t H5D__create_piece_mem_map_hyper(const H5D_dset_io_info_t *dinfo) {
  ...
  hsize_t
      file_sel_start[H5S_MAX_RANK]; /* Offset of low bound of file selection */
  hsize_t
      file_sel_end[H5S_MAX_RANK]; /* Offset of high bound of file selection */
  hsize_t
      mem_sel_start[H5S_MAX_RANK]; /* Offset of low bound of file selection */
  hsize_t
      mem_sel_end[H5S_MAX_RANK]; /* Offset of high bound of file selection */
  ...
  if (H5SL_count(fm->dset_sel_pieces) == 1) {
    ...
    while (curr_node) {
      hsize_t coords[H5S_MAX_RANK];        /* Current coordinates of chunk */
      hssize_t piece_adjust[H5S_MAX_RANK]; /* Adjustment to make to a particular
                                              chunk */
      ...
      if ((piece_info->mspace = H5S_copy(dinfo->mem_space, TRUE, FALSE)) ==
          NULL)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                    "unable to copy memory space");

      /* Get the chunk's selection type */
      if ((chunk_sel_type = H5S_GET_SELECT_TYPE(piece_info->fspace)) <
          H5S_SEL_NONE)
        HGOTO_ERROR(H5E_DATASET, H5E_BADSELECT, FAIL,
                    "unable to get type of selection");
      ...
      if (H5S_SEL_ALL == chunk_sel_type) {
        ...
        if (H5S_select_hyperslab(piece_info->mspace, H5S_SELECT_SET, coords,
                                 NULL, fm->chunk_dim, NULL) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTSELECT, FAIL,
 ...
 *
 * Purpose:    Create all chunk selections for 1-dimensional regular memory
 *space that has only one single block in the selection
 *
 * Return:    Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
...
static herr_t H5D__create_piece_mem_map_1d(const H5D_dset_io_info_t *dinfo) {
  ...
  if (H5SL_count(fm->dset_sel_pieces) == 1) {
    ...
    while (curr_node) {
      ...
      if (H5S_select_hyperslab(piece_info->mspace, H5S_SELECT_SET,
                               mem_sel_start, NULL, &tmp_count,
                               &chunk_points) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTSELECT, FAIL,
                    "can't create chunk memory selection");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__piece_file_cb(void H5_ATTR_UNUSED *elem,
                                 const H5T_t H5_ATTR_UNUSED *type,
                                 unsigned ndims, const hsize_t *coords,
                                 void *_opdata) {
  H5D_io_info_wrap_t *opdata = (H5D_io_info_wrap_t *)_opdata;
  H5D_io_info_t *io_info =
      (H5D_io_info_t *)opdata->io_info; /* io info for multi dset */
  H5D_dset_io_info_t *dinfo =
      (H5D_dset_io_info_t *)
          opdata->dinfo;        /* File<->memory piece mapping info */
  H5D_chunk_map_t *fm;          /* Convenience pointer to chunk map */
  H5D_piece_info_t *piece_info; /* Chunk information for current piece */
  hsize_t
      coords_in_chunk[H5O_LAYOUT_NDIMS]; /* Coordinates of element in chunk */
  ...
  /* Calculate the index of this chunk */
  chunk_index =
      H5VM_chunk_index_scaled(ndims, coords, dinfo->layout->u.chunk.dim,
  ...
  if (chunk_index == fm->last_index) {
     ...
     * a new node. */
    if (NULL == (piece_info = (H5D_piece_info_t *)H5SL_search(
                     fm->dset_sel_pieces, &chunk_index))) {
      ...
      if ((fspace = H5S_create_simple(fm->f_ndims, fm->chunk_dim, NULL)) ==
          NULL) {
        piece_info = H5FL_FREE(H5D_piece_info_t, piece_info);
      ...
      /* Insert the new chunk into the skip list */
      if (H5SL_insert(fm->dset_sel_pieces, piece_info, &piece_info->index) <
          0) {
  ...
  for (u = 0; u < fm->f_ndims; u++)
    coords_in_chunk[u] =
        coords[u] - (scaled[u] * dinfo->layout->u.chunk.dim[u]);

  /* Add point to file selection for chunk */
  if (H5S_select_elements(piece_info->fspace, H5S_SELECT_APPEND, (size_t)1,
                          coords_in_chunk) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSELECT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__piece_mem_cb(void H5_ATTR_UNUSED *elem,
                                const H5T_t H5_ATTR_UNUSED *type,
                                unsigned ndims, const hsize_t *coords,
                                void *_opdata) {
  H5D_io_info_wrap_t *opdata = (H5D_io_info_wrap_t *)_opdata;
  H5D_dset_io_info_t *dinfo =
      (H5D_dset_io_info_t *)
          opdata->dinfo;               /* File<->memory chunk mapping info */
  ...
  if (chunk_index == fm->last_index) {
    ...
    if (NULL == piece_info->mspace)
      ...
      if (NULL ==
          (piece_info->mspace = H5S_copy(fm->mchunk_tmpl, FALSE, FALSE)))
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, H5_ITER_ERROR,
                    "unable to copy file space");
  ...
  if (H5S_SELECT_ITER_COORDS(&fm->mem_iter, coords_in_mem) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, H5_ITER_ERROR,
                "unable to get iterator coordinates");

  /* Add point to memory selection for chunk */
  if (fm->msel_type == H5S_SEL_POINTS) {
    if (H5S_select_elements(piece_info->mspace, H5S_SELECT_APPEND, (size_t)1,
                            coords_in_mem) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSELECT, H5_ITER_ERROR,
                  "unable to select element");
  } /* end if */
  else {
    if (H5S_hyper_add_span_element(piece_info->mspace, fm->m_ndims,
                                   coords_in_mem) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSELECT, H5_ITER_ERROR,
                  "unable to select element");
  } /* end else */
...
static herr_t H5D__chunk_mdio_init(H5D_io_info_t *io_info,
  ...
  while (piece_node) {
    /* Get piece info */
    if (NULL == (piece_info = (H5D_piece_info_t *)H5D_CHUNK_GET_NODE_INFO(
                     dinfo, piece_node)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "couldn't get piece info from list");
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5D__chunk_cacheable(const H5D_io_info_t H5_ATTR_PARALLEL_USED *io_info,
                            H5D_dset_io_info_t *dset_info, haddr_t caddr,
                            hbool_t write_op) {
  const H5D_t *dataset = NULL; /* Local pointer to dataset info */
  hbool_t has_filters =
      FALSE;               /* Whether there are filters on the chunk or not */
  ...
  if (has_filters)
    ...
    if (io_info->using_mpi_vfd &&
        (H5F_ACC_RDWR & H5F_INTENT(dataset->oloc.file)))
      ret_value = FALSE;
    else {
#endif /* H5_HAVE_PARALLEL */
      ...
      if ((size_t)dataset->shared->layout.u.chunk.size >
        ...
        if (write_op && !H5_addr_defined(caddr)) {
          const H5O_fill_t *fill =
              &(dataset->shared->dcpl_cache.fill); /* Fill value info */
          H5D_fill_value_t fill_status;            /* Fill value status */

          /* Revtrieve the fill value status */
          if (H5P_is_fill_value_defined(fill, &fill_status) < 0)
            HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                        "can't tell if fill value defined");
 ...
 */
static herr_t
H5D__chunk_may_use_select_io(H5D_io_info_t *io_info,
                             const H5D_dset_io_info_t *dset_info) {
  const H5D_t *dataset = NULL; /* Local pointer to dataset info */
  herr_t ret_value = SUCCEED;  /* Return value */
  ...
  if (dataset->shared->dcpl_cache.pline.nused > 0) {
    ...
    if (H5PB_enabled(io_info->f_sh, H5FD_MEM_DRAW, &page_buf_enabled) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't check if page buffer is enabled");
    if (page_buf_enabled) {
      ...
      if (!(io_info->using_mpi_vfd &&
        ...
        H5_CHECK_OVERFLOW(dataset->shared->layout.u.chunk.size, uint32_t,
                          size_t);
        if ((size_t)dataset->shared->layout.u.chunk.size <=
            dataset->shared->cache.chunk.nbytes_max) {
          io_info->use_select_io = H5D_SELECTION_IO_MODE_OFF;
...
static herr_t H5D__chunk_read(H5D_io_info_t *io_info,
  ...
  H5D_dset_io_info_t
      nonexistent_dset_info;           /* "nonexistent" I/O dset info object */
  ...
  {
    const H5O_fill_t *fill =
        &(dset_info->dset->shared->dcpl_cache.fill); /* Fill value info */
    H5D_fill_value_t fill_status;                    /* Fill value status */
  ...
  if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
    ...
    if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
      ...
      assert(
          sizeof(chunk_mem_spaces_local) / sizeof(chunk_mem_spaces_local[0]) ==
          sizeof(chunk_file_spaces_local) / sizeof(chunk_file_spaces_local[0]));
      assert(sizeof(chunk_mem_spaces_local) /
                 sizeof(chunk_mem_spaces_local[0]) ==
             sizeof(chunk_addrs_local) / sizeof(chunk_addrs_local[0]));
      if (num_chunks > (sizeof(chunk_mem_spaces_local) /
                        sizeof(chunk_mem_spaces_local[0]))) {
        if (NULL ==
            (chunk_mem_spaces = H5MM_malloc(num_chunks * sizeof(H5S_t *))))
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                      "memory allocation failed for memory space list");
        if (NULL ==
            (chunk_file_spaces = H5MM_malloc(num_chunks * sizeof(H5S_t *))))
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
    ...
    while (chunk_node) {
      ...
      if (H5_addr_defined(udata.chunk_block.offset)) {
        /* Add chunk to list for selection I/O, if not performing multi dataset
         * I/O */
        if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
        ...
        /* Set up nonexistent dataset info for (fill value) read from
         * nonexistent chunk */
        ...
        /* Set request_nelmts.  This is not normally set by the upper layers
         * because selection I/O usually does not use strip mining
         * (H5D__scatgath_write), and instead allocates buffers large enough for
         * the entire I/O.  Set request_nelmts to be large enough for all
         * selected elements in this chunk because it must be at least that
         * large */
        nonexistent_dset_info.type_info.request_nelmts =
            nonexistent_dset_info.nelmts;

        /* Perform the actual read operation from the nonexistent chunk
         */
        if ((dset_info->io_ops.single_read)(&nonexistent_io_info,
                                            &nonexistent_dset_info) < 0)
     ...
     * otherwise the higher level will handle it after all datasets have been
     * processed */
    if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
      ...
      if (H5F_shared_select_read(H5F_SHARED(dset_info->dset->oloc.file),
                                 H5FD_MEM_DRAW, (uint32_t)num_chunks,
                                 chunk_mem_spaces, chunk_file_spaces,
                                 chunk_addrs, element_sizes, bufs) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                    "chunk selection read failed");
...
#ifdef H5_HAVE_PARALLEL
    /* Report that collective chunk I/O was used (will only be set on the DXPL
     * if collective I/O was requested) */
    ...
    H5D_storage_t
        ctg_store; /* Chunk storage information as contiguous dataset */
    ...
    H5_CHECKED_ASSIGN(ctg_store.contig.dset_size, hsize_t,
                      dset_info->dset->shared->layout.u.chunk.size, uint32_t);
    ...
    while (chunk_node) {
      ...
      /* Check for non-existent chunk & skip it if appropriate */
      if (H5_addr_defined(udata.chunk_block.offset) ||
          UINT_MAX != udata.idx_hint || !skip_missing_chunks) {
        H5D_io_info_t
            *chk_io_info;   /* Pointer to I/O info object for this chunk */
        ...
        if ((cacheable = H5D__chunk_cacheable(
                 io_info, dset_info, udata.chunk_block.offset, FALSE)) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                      "can't tell if chunk is cacheable");
        if (cacheable) {
          ...
          H5_CHECK_OVERFLOW(chunk_info->piece_points, /*From:*/ size_t,
                            /*To:*/ uint32_t);
          src_accessed_bytes = (uint32_t)chunk_info->piece_points *
                               (uint32_t)dset_info->type_info.src_type_size;

          /* Lock the chunk into the cache */
          if (NULL == (chunk = H5D__chunk_lock(io_info, dset_info, &udata,
                                               FALSE, FALSE)))
        ...
        chk_io_info->dsets_info[0].layout_io_info.contig_piece_info =
            chunk_info;
        ...
        if ((dset_info->io_ops.single_read)(chk_io_info,
                                            &chk_io_info->dsets_info[0]) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "chunked read failed");
...
static herr_t H5D__chunk_write(H5D_io_info_t *io_info,
  ...
  if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
    ...
    while (chunk_node) {
      ...
      /* Determine if we should use the chunk cache */
      if ((cacheable = H5D__chunk_cacheable(
               io_info, dset_info, udata.chunk_block.offset, TRUE)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                    "can't tell if chunk is cacheable");
      if (cacheable) {
        ...
        /* Compute # of bytes accessed in chunk */
        H5_CHECK_OVERFLOW(dset_info->type_info.dst_type_size, /*From:*/ size_t,
                          /*To:*/ uint32_t);
        H5_CHECK_OVERFLOW(chunk_info->piece_points, /*From:*/ size_t,
                          /*To:*/ uint32_t);
        dst_accessed_bytes = (uint32_t)chunk_info->piece_points *
                             (uint32_t)dset_info->type_info.dst_type_size;
        ...
        /* Release the cache lock on the chunk */
        if (H5D__chunk_unlock(io_info, dset_info, &udata, TRUE, chunk,
        ...
        if (!H5_addr_defined(udata.chunk_block.offset)) {
          ...
          /* Set up the size of chunk for user data */
          udata.chunk_block.length =
              dset_info->dset->shared->layout.u.chunk.size;

          /* Allocate the chunk */
          if (H5D__chunk_file_alloc(&idx_info, NULL, &udata.chunk_block,
                                    &need_insert, chunk_info->scaled) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, FAIL,
          ...
          H5D__chunk_cinfo_cache_update(
              &dset_info->dset->shared->cache.chunk.last, &udata);

          /* Insert chunk into index */
          if (need_insert &&
              dset_info->dset->shared->layout.storage.u.chunk.ops->insert)
    ...
    while (chunk_node) {
      ...
      chk_io_info->dsets_info[0].mem_space = chunk_info->mspace;
      chk_io_info->dsets_info[0].nelmts = chunk_info->piece_points;
      if ((dset_info->io_ops.single_write)(chk_io_info,
                                           &chk_io_info->dsets_info[0]) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "chunked write failed");
...
static herr_t H5D__chunk_flush(H5D_t *dset) {
  ...
  unsigned nerrors =
      0; /* Count of any errors encountered when flushing chunks */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__chunk_io_term(H5D_io_info_t H5_ATTR_UNUSED *io_info,
                                 H5D_dset_io_info_t *di) {
  H5D_chunk_map_t *fm;        /* Convenience pointer to chunk map */
  herr_t ret_value = SUCCEED; /*return value        */
  ...
  if (fm->use_single) {
    ...
    if (fm->dset_sel_pieces) {
      if (H5SL_free(fm->dset_sel_pieces, H5D__free_piece_info, NULL) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTNEXT, FAIL,
                    "can't free dataset skip list");
    } /* end if */
  ...
  /* Free chunk map */
  di->layout_io_info.chunk_map =
      H5FL_FREE(H5D_chunk_map_t, di->layout_io_info.chunk_map);
...
static herr_t H5D__chunk_dest(H5D_t *dset) {
  ...
  H5D_rdcc_ent_t *ent = NULL,
                 *next = NULL; /* Pointer to current & next cache entries */
  ...
  if (nerrors)
    HDONE_ERROR(H5E_IO, H5E_CANTFLUSH, FAIL,
                "unable to flush one or more raw data chunks");
...
herr_t H5D_chunk_idx_reset(H5O_storage_chunk_t *storage, hbool_t reset_addr) {
  ...
  if ((storage->ops->reset)(storage, reset_addr) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                "unable to reset chunk index info");
...
static herr_t H5D__chunk_cinfo_cache_update(H5D_chunk_cached_t *last,
  ...
  H5MM_memcpy(last->scaled, udata->common.scaled,
              sizeof(hsize_t) * udata->common.layout->ndims);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_lookup(const H5D_t *dset, const hsize_t *scaled,
                         H5D_chunk_ud_t *udata) {
  ...
  if (found) {
    ...
    if (!H5D__chunk_cinfo_cache_found(&dset->shared->cache.chunk.last, udata)) {
      ...
      if (H5F_HAS_FEATURE(idx_info.f, H5FD_FEAT_HAS_MPI)) {
        ...
        H5F_set_coll_metadata_reads(idx_info.f, &md_reads_file_flag,
      ...
      if ((sc->ops->get_addr)(&idx_info, udata) < 0)
         ...
         *
         * It has been verified that in the serial library, when writing to
         * chunks with the real chunk cache disabled and with filters involved,
         * the functions within this file are correctly called in such a manner
         * that this single chunk cache is always updated correctly. Therefore,
         * this check is not needed for the serial library.
         *
         ...
         * invalidate this single chunk cache. Even if the function were
         * available, this check prevents us from doing the work of going
         * through and caching each chunk in the write operation, when we're
         * only going to invalidate the cache at the end of a parallel write
         * anyway.
         *
         *  - JTH (7/13/2018)
         */
  ...
  if (restore_md_reads_state)
    H5F_set_coll_metadata_reads(dset->oloc.file, &md_reads_file_flag,
                                &md_reads_context_flag);
#endif /* H5_HAVE_PARALLEL */
...
static herr_t H5D__chunk_flush_entry(const H5D_t *dset, H5D_rdcc_ent_t *ent,
  ...
  if (ent->dirty) {
    ...
    if (dset->shared->dcpl_cache.pline.nused &&
      ...
      if (H5Z_pipeline(&(dset->shared->dcpl_cache.pline), 0,
                       &(udata.filter_mask), err_detect, filter_cb, &nbytes,
                       &alloc, &buf) < 0)
    ...
    if (must_alloc) {
       ...
       *  if its size changed.
       */
      if (H5D__chunk_file_alloc(&idx_info, &(ent->chunk_block),
                                &udata.chunk_block, &need_insert,
                                ent->scaled) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, FAIL,
                    "unable to insert/resize chunk on chunk level");
  ...
  if (reset) {
    ...
    if (ent->chunk != NULL)
      ent->chunk = (uint8_t *)H5D__chunk_mem_xfree(
          ent->chunk, ((ent->edge_chunk_state & H5D_RDCC_DISABLE_FILTERS)
                           ? NULL
...
static herr_t H5D__chunk_cache_evict(const H5D_t *dset, H5D_rdcc_ent_t *ent,
  ...
  if (flush) {
    ...
    if (H5D__chunk_flush_entry(dset, ent, TRUE) < 0)
      HDONE_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
...
static herr_t H5D__chunk_cache_prune(const H5D_t *dset, size_t size) {
    ...
    for (i = 0; i < nmeth && (rdcc->nbytes_used + size) > total; i++) {
      if (0 == i && p[0] && !p[0]->locked &&
          ((0 == p[0]->rd_count && 0 == p[0]->wr_count) ||
           (0 == p[0]->rd_count &&
            dset->shared->layout.u.chunk.size == p[0]->wr_count) ||
           (dset->shared->layout.u.chunk.size == p[0]->rd_count &&
            0 == p[0]->wr_count))) {
        /*
  ...
  if (nerrors)
    HGOTO_ERROR(H5E_IO, H5E_CANTFLUSH, FAIL,
...
static void *H5D__chunk_lock(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
  ...
  H5O_pline_t *pline; /* I/O pipeline info - always equal to the pline passed to
                         H5D__chunk_mem_alloc */
  H5O_pline_t
      *old_pline; /* Old pipeline, i.e. pipeline used to read the chunk */
  const H5O_layout_t *layout;  /* Dataset layout */
  const H5O_fill_t *fill;      /* Fill value info */
  H5D_fill_buf_info_t fb_info; /* Dataset's fill buffer info */
  hbool_t fb_info_init =
      FALSE;           /* Whether the fill value buffer has been initialized */
  H5D_rdcc_t *rdcc;    /*raw data chunk cache*/
  H5D_rdcc_ent_t *ent; /*cache entry        */
  size_t chunk_size;   /*size of a chunk    */
  hbool_t disable_filters =
      FALSE;          /* Whether to disable filters (when adding to cache) */
  ...
  if (UINT_MAX != udata->idx_hint) {
    ...
    if (pline->nused) {
      ...
      if (udata->new_unfilt_chunk) {
        ...
        ent->edge_chunk_state &=
            ~(H5D_RDCC_DISABLE_FILTERS | H5D_RDCC_NEWLY_DISABLED_FILTERS);
      } /* end else */
    }   /* end if */
    ...
    if (relax) {
      ...
      if (H5_addr_defined(chunk_addr)) {
        ...
        if (NULL == (chunk = H5D__chunk_mem_alloc(
                         my_chunk_alloc,
                         (udata->new_unfilt_chunk ? old_pline : pline))))
          HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                      "memory allocation failed for raw data chunk");
        if (H5F_shared_block_read(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW,
                                  chunk_addr, my_chunk_alloc, chunk) < 0)
        ...
        if (old_pline && old_pline->nused) {
          ...
          if (H5Z_pipeline(old_pline, H5Z_FLAG_REVERSE, &(udata->filter_mask),
                           err_detect, filter_cb, &my_chunk_alloc, &buf_alloc,
                           &chunk) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTFILTER, NULL,
        ...
        if (fill->fill_time == H5D_FILL_TIME_ALLOC ||
          ...
          if (H5D__fill_init(&fb_info, chunk, NULL, NULL, NULL, NULL,
            ...
            HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                        "can't initialize fill buffer info");
          fb_info_init = TRUE;
      ...
      if (!ent || !ent->locked) {
        ...
        if (NULL == (ent = H5FL_CALLOC(H5D_rdcc_ent_t)))
          HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, NULL,
                      "can't allocate raw data chunk entry");
        ...
        H5MM_memcpy(ent->scaled, udata->common.scaled,
                    sizeof(hsize_t) * layout->u.chunk.ndims);
  ...
  if (fb_info_init && H5D__fill_term(&fb_info) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, NULL,
 ...
 */
static herr_t
H5D__chunk_unlock(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
                  const H5D_dset_io_info_t *dset_info,
  ...
  if (UINT_MAX == udata->idx_hint) {
     ...
     */
    hbool_t is_unfiltered_edge_chunk =
        FALSE; /* Whether the chunk is an unfiltered edge chunk */
    ...
    if (dirty) {
      ...
      H5MM_memcpy(fake_ent.scaled, udata->common.scaled,
                  sizeof(hsize_t) * layout->u.chunk.ndims);
      assert(layout->u.chunk.size > 0);
      ...
      if (chunk)
        chunk = H5D__chunk_mem_xfree(chunk,
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__chunk_allocated_cb(const H5D_chunk_rec_t *chunk_rec,
                                   void *_udata) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_allocate(const H5D_t *dset, hbool_t full_overwrite,
                           const hsize_t old_dim[]) {
  H5D_chk_idx_info_t idx_info; /* Chunked index info */
  const H5D_chunk_ops_t *ops =
      dset->shared->layout.storage.u.chunk.ops; /* Chunk operations */
  hsize_t
      min_unalloc[H5O_LAYOUT_NDIMS]; /* First chunk in each dimension that is
                                        unallocated (in scaled coordinates) */
  hsize_t
      max_unalloc[H5O_LAYOUT_NDIMS]; /* Last chunk in each dimension that is
                                        unallocated (in scaled coordinates) */
  hsize_t scaled[H5O_LAYOUT_NDIMS];  /* Offset of current chunk (in scaled
                                        coordinates) */
  ...
  H5O_pline_t *pline =
      &(dset->shared->dcpl_cache.pline);        /* I/O pipeline info */
  H5O_pline_t def_pline = H5O_CRT_PIPELINE_DEF; /* Default pipeline */
  const H5O_fill_t *fill =
      &(dset->shared->dcpl_cache.fill); /* Fill value info */
...
#ifdef H5_HAVE_PARALLEL
  hbool_t blocks_written =
      FALSE; /* Flag to indicate that chunk was actually written */
  hbool_t using_mpi = FALSE; /* Flag to indicate that the file is being accessed
                                with an MPI-capable file driver */
  H5D_chunk_coll_fill_info_t
      chunk_fill_info; /* chunk address information for doing I/O */
#endif                 /* H5_HAVE_PARALLEL */
  hbool_t carry;       /* Flag to indicate that chunk increment carrys to higher
                          dimension (sorta) */
  unsigned space_ndims;     /* Dataset's space rank */
  const hsize_t *space_dim; /* Dataset's dataspace dimensions */
  const uint32_t *chunk_dim =
      layout->u.chunk.dim;     /* Convenience pointer to chunk dimensions */
  unsigned op_dim;             /* Current operating dimension */
  H5D_fill_buf_info_t fb_info; /* Dataset's fill buffer info */
  hbool_t fb_info_init =
      FALSE; /* Whether the fill value buffer has been initialized */
  hbool_t has_unfilt_edge_chunks =
      FALSE; /* Whether there are partial edge chunks with disabled filters */
  hbool_t
      unfilt_edge_chunk_dim[H5O_LAYOUT_NDIMS]; /* Whether there are unfiltered
                                                  edge chunks at the edge of
                                                  each dimension */
  hsize_t edge_chunk_scaled[H5O_LAYOUT_NDIMS]; /* Offset of the unfiltered edge
                                                  chunks at the edge of each
                                                  dimension */
  unsigned nunfilt_edge_chunk_dims = 0; /* Number of dimensions on an edge */
  H5O_storage_chunk_t *sc =
      &(layout->storage.u.chunk); /* Convenience variable */
  herr_t ret_value = SUCCEED;     /* Return value */
  ...
  /* Check args */
  assert(dset && H5D_CHUNKED == layout->type);
  assert(layout->u.chunk.ndims > 0 &&
         layout->u.chunk.ndims <= H5O_LAYOUT_NDIMS);
  H5D_CHUNK_STORAGE_INDEX_CHK(sc);
  ...
  for (op_dim = 0; op_dim < space_ndims; op_dim++) {
    ...
    if (chunk_dim[op_dim] == 0)
      HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                  "chunk size must be > 0, dim = %u ", op_dim);
    min_unalloc[op_dim] =
        (old_dim[op_dim] + chunk_dim[op_dim] - 1) / chunk_dim[op_dim];
  ...
  if (should_fill) {
    ...
    if (H5D__fill_init(&fb_info, NULL, H5D__chunk_mem_alloc, pline,
                       H5D__chunk_mem_free, pline,
                       &dset->shared->dcpl_cache.fill, dset->shared->type,
    ...
    if (!fb_info.has_vlen_fill_type && pline->nused > 0) {
      ...
      if (has_unfilt_edge_chunks) {
        if (NULL == (unfilt_fill_buf =
                         H5D__chunk_mem_alloc(orig_chunk_size, &def_pline)))
          HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                      "memory allocation failed for raw data chunk");
      ...
      if (H5Z_pipeline(pline, 0, &filter_mask, err_detect, filter_cb,
    ...
    while (!carry) {
      ...
      if (fb_info_init && fb_info.has_vlen_fill_type) {
        ...
        if (fb_info.fill_buf_size < orig_chunk_size) {
          if (NULL == (fb_info.fill_buf = H5D__chunk_mem_realloc(
      ...
      if (should_fill) {
        ...
        if (using_mpi) {
          ...
          chunk_fill_info.chunk_info[chunk_fill_info.num_chunks].addr =
              udata.chunk_block.offset;
          chunk_fill_info.chunk_info[chunk_fill_info.num_chunks].chunk_size =
              chunk_size;
          chunk_fill_info.chunk_info[chunk_fill_info.num_chunks]
              .unfiltered_partial_chunk = (*fill_buf == unfilt_fill_buf);
...
#endif /* H5_HAVE_PARALLEL */
          if (H5F_shared_block_write(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW,
                                     udata.chunk_block.offset, chunk_size,
                                     *fill_buf) < 0)
            HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                        "unable to write raw data to file");
#ifdef H5_HAVE_PARALLEL
        ...
        if (scaled[i] > max_unalloc[i]) {
          ...
          if (unfilt_edge_chunk_dim[i] &&
            ...
            if (should_fill && nunfilt_edge_chunk_dims == 0 &&
              ...
              assert(!H5D__chunk_is_partial_edge_chunk(space_ndims, chunk_dim,
                                                       scaled, space_dim));
              fill_buf = &fb_info.fill_buf;
              chunk_size = orig_chunk_size;
            } /* end if */
  ...
  if (using_mpi && blocks_written)
    if (H5D__chunk_collective_fill(dset, &chunk_fill_info, fb_info.fill_buf,
                                   unfilt_fill_buf) < 0)
      HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                  "unable to write raw data to file");
#endif /* H5_HAVE_PARALLEL */
...
herr_t H5D__chunk_update_old_edge_chunks(H5D_t *dset, hsize_t old_dim[]) {
  hsize_t old_edge_chunk_sc[H5O_LAYOUT_NDIMS]; /* Offset of first previously
                                                  incomplete chunk in each
                                                  dimension */
  hsize_t max_edge_chunk_sc[H5O_LAYOUT_NDIMS]; /* largest offset of chunks that
                                                  might need to be modified in
                                                  each dimension */
  hbool_t new_full_dim[H5O_LAYOUT_NDIMS]; /* Whether the plane of chunks in this
                                             dimension needs to be modified */
  const H5O_layout_t *layout = &(dset->shared->layout); /* Dataset layout */
  ...
  for (op_dim = 0; op_dim < space_ndims; op_dim++) {
    ...
    if ((space_dim[op_dim] / chunk_dim[op_dim]) >=
        (old_edge_chunk_sc[op_dim] + 1))
  ...
  for (op_dim = 0; op_dim < space_ndims; op_dim++) {
    ...
    while (!carry) {
      ...
      assert(H5D__chunk_is_partial_edge_chunk(space_ndims, chunk_dim, chunk_sc,
                                              old_dim) &&
             !H5D__chunk_is_partial_edge_chunk(space_ndims, chunk_dim, chunk_sc,
                                               space_dim));
      ...
      if (H5_addr_defined(chk_udata.chunk_block.offset) ||
        ...
        if (NULL ==
            (chunk = (void *)H5D__chunk_lock(&chk_io_info, &chk_dset_info,
                                             &chk_udata, FALSE, TRUE)))
          HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                      "unable to lock raw data chunk");

        /* Unlock the chunk */
        if (H5D__chunk_unlock(&chk_io_info, &chk_dset_info, &chk_udata, TRUE,
                              chunk, (uint32_t)0) < 0)
          HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                      "unable to unlock raw data chunk");
      } /* end if */
...
static herr_t H5D__chunk_collective_fill(
  ...
  hbool_t have_xfer_mode =
      FALSE; /* Whether the previous xffer mode has been retrieved */
  ...
  if (partial_chunk_fill_buf)
    assert(dset->shared->layout.u.chunk.flags &
           H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS);
  ...
  num_blocks =
      (size_t)(chunk_fill_info->num_chunks /
  ...
  if (num_blocks > 0 || (leftover && leftover > mpi_rank)) {
    ...
    if (NULL == (chunk_disp_array = (MPI_Aint *)H5MM_malloc(
                     (size_t)(blocks + 1) * sizeof(MPI_Aint))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
    ...
    if (partial_chunk_fill_buf) {
      ...
      /* Calculate the displacement between the fill buffer and partial chunk
       * fill buffer */
      if (MPI_SUCCESS != (mpi_code = MPI_Get_address(fill_buf, &fill_buf_addr)))
        HMPI_GOTO_ERROR(FAIL, "MPI_Get_address failed", mpi_code)
      if (MPI_SUCCESS != (mpi_code = MPI_Get_address(partial_chunk_fill_buf,
                                                     &partial_fill_buf_addr)))
        HMPI_GOTO_ERROR(FAIL, "MPI_Get_address failed", mpi_code)

#if H5_CHECK_MPI_VERSION(3, 1)
      partial_fill_buf_disp =
          MPI_Aint_diff(partial_fill_buf_addr, fill_buf_addr);
#else
       ...
       */
      if (NULL == (block_disps = (MPI_Aint *)H5MM_calloc((size_t)(blocks + 1) *
                                                         sizeof(MPI_Aint))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate block displacements buffer");
    }
    ...
    for (i = 1; i < chunk_fill_info->num_chunks; i++) {
      if (chunk_fill_info->chunk_info[i].addr <
          chunk_fill_info->chunk_info[i - 1].addr)
        need_sort = TRUE;

      if (chunk_fill_info->chunk_info[i].chunk_size !=
          chunk_fill_info->chunk_info[i - 1].chunk_size)
    ...
    if (!all_same_block_len)
      if (NULL ==
          (block_lens = (int *)H5MM_malloc((size_t)(blocks + 1) * sizeof(int))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate chunk lengths buffer");
    ...
    for (i = 0; i < (size_t)blocks; i++) {
      ...
      if (!all_same_block_len)
        H5_CHECKED_ASSIGN(block_lens[i], int,
                          chunk_fill_info->chunk_info[idx].chunk_size, size_t);
    ...
    if (leftover && leftover > mpi_rank) {
      chunk_disp_array[blocks] =
          (MPI_Aint)chunk_fill_info->chunk_info[(blocks * mpi_size) + mpi_rank]
              .addr;

      if (!all_same_block_len)
        H5_CHECKED_ASSIGN(
            block_lens[blocks], int,
            chunk_fill_info->chunk_info[(blocks * mpi_size) + mpi_rank]
                .chunk_size,
            size_t);

      if (chunk_fill_info->chunk_info[(blocks * mpi_size) + mpi_rank]
              .unfiltered_partial_chunk) {
        assert(partial_chunk_fill_buf);
    ...
    if (all_same_block_len) {
      ...
      H5_CHECKED_ASSIGN(block_len, int,
                        chunk_fill_info->chunk_info[0].chunk_size, size_t);

      mpi_code = MPI_Type_create_hindexed_block(
          blocks, block_len, chunk_disp_array, MPI_BYTE, &file_type);
      ...
      if (partial_chunk_fill_buf) {
         ...
         * need to be written to using the unfiltered fill buffer. Use an
         * hindexed block type rather than an hvector.
         */
        mpi_code = MPI_Type_create_hindexed_block(
            blocks, block_len, block_disps, MPI_BYTE, &mem_type);
        if (mpi_code != MPI_SUCCESS)
          HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_hindexed_block failed",
                          mpi_code)
      } else {
        mpi_code =
            MPI_Type_create_hvector(blocks, block_len, 0, MPI_BYTE, &mem_type);
        if (mpi_code != MPI_SUCCESS)
      ...
      mpi_code = MPI_Type_create_hindexed(blocks, block_lens, block_disps,
                                          MPI_BYTE, &mem_type);
      if (mpi_code != MPI_SUCCESS)
        HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_hindexed failed", mpi_code)
    }
  ...
  if (H5F_shared_block_write(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW,
                             (haddr_t)0, (blocks) ? (size_t)1 : (size_t)0,
                             fill_buf) < 0)
    HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                "unable to write raw data to file");
...
static herr_t H5D__chunk_prune_fill(H5D_chunk_it_ud1_t *udata,
  ...
  const H5D_t *dset =
      udata->dset_info->dset; /* Local pointer to the dataset info */
  ...
  hbool_t chunk_iter_init =
      FALSE;          /* Whether the chunk iterator has been initialized */
  ...
  if (!udata->fb_info_init) {
    ...
    if (H5D__fill_init(&udata->fb_info, NULL, NULL, NULL, NULL, NULL,
                       &dset->shared->dcpl_cache.fill, dset->shared->type,
                       dset->shared->type_id, (size_t)udata->elmts_per_chunk,
                       chunk_size) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "can't initialize fill buffer info");
  ...
  for (u = 0; u < rank; u++) {
    count[u] =
        MIN(layout->u.chunk.dim[u],
            (udata->space_dim[u] - (scaled[u] * layout->u.chunk.dim[u])));
    assert(count[u] > 0);
  } /* end for */
  ...
  if (H5S_select_hyperslab(udata->chunk_space, H5S_SELECT_NOTB,
                           udata->hyper_start, NULL, count, NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSELECT, FAIL,
                "unable to select hyperslab");

  /* Lock the chunk into the cache, to get a pointer to the chunk buffer */
  if (NULL == (chunk = (void *)H5D__chunk_lock(io_info, udata->dset_info,
                                               &chk_udata, FALSE, FALSE)))
    HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                "unable to lock raw data chunk");
  ...
  if (NULL == (chunk_iter = H5FL_MALLOC(H5S_sel_iter_t)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "can't allocate chunk selection iterator");

  /* Create a selection iterator for scattering the elements to memory buffer */
  if (H5S_select_iter_init(chunk_iter, udata->chunk_space,
                           layout->u.chunk.dim[rank], 0) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to initialize chunk selection information");
  chunk_iter_init = TRUE;

  /* Scatter the data into memory */
  if (H5D__scatter_mem(udata->fb_info.fill_buf, chunk_iter, (size_t)sel_nelmts,
                       chunk /*out*/) < 0)
  ...
  if (H5D__chunk_unlock(io_info, udata->dset_info, &chk_udata, TRUE, chunk,
                        bytes_accessed) < 0)
    HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                "unable to unlock raw data chunk");
  ...
  if (chunk_iter_init && H5S_SELECT_ITER_RELEASE(chunk_iter) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                "Can't release selection iterator");
  if (chunk_iter)
 ...
 *  4. If found then remove it from the B-tree and deallocate file storage for
 *the chunk
 *
 ...
 * (60,80), (80,80), (80,60), (80,40), (80,20), and (80,0), one is actually
 *allocated that needs to be released. To release the chunks, we traverse the
 *B-tree to obtain a list of unused allocated chunks, and then call H5B_remove()
 *for each chunk.
 *
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_prune_by_extent(H5D_t *dset, const hsize_t *old_dim) {
  hsize_t min_mod_chunk_sc[H5O_LAYOUT_NDIMS]; /* Scaled offset of first chunk to
                                                 modify in each dimension */
  hsize_t max_mod_chunk_sc[H5O_LAYOUT_NDIMS]; /* Scaled offset of last chunk to
                                                 modify in each dimension */
  hssize_t max_fill_chunk_sc[H5O_LAYOUT_NDIMS]; /* Scaled offset of last chunk
                                                   that might be filled in each
                                                   dimension */
  hbool_t fill_dim[H5O_LAYOUT_NDIMS]; /* Whether the plane of edge chunks in
                                         this dimension needs to be filled */
  hsize_t min_partial_chunk_sc[H5O_LAYOUT_NDIMS]; /* Offset of first partial (or
                                                     empty) chunk in each
                                                     dimension */
  hbool_t new_unfilt_dim[H5O_LAYOUT_NDIMS]; /* Whether the plane of edge chunks
                                               in this dimension are newly
                                               unfiltered */
  H5D_chk_idx_info_t idx_info;              /* Chunked index info */
  H5D_io_info_t chk_io_info;                /* Chunked I/O info object */
  H5D_dset_io_info_t chk_dset_info;         /* Chunked I/O dset info object */
  ...
  hbool_t udata_init = FALSE; /* Whether the chunk index iterator user data has
                                 been initialized */
  ...
  hbool_t disable_edge_filters =
      FALSE; /* Whether to disable filters on partial edge chunks */
  ...
  /* (also copy the chunk dimensions into 'hsize_t' array for creating
   * dataspace) */
  ...
  for (op_dim = 0; op_dim < (unsigned)space_ndims; op_dim++) {
    ...
    if (0 == space_dim[op_dim])
      ...
      max_fill_chunk_sc[op_dim] = (hssize_t)((
    ...
    if (shrunk_dim[op_dim]) {
      ...
      if ((hssize_t)min_mod_chunk_sc[op_dim] == max_fill_chunk_sc[op_dim]) {
        ...
        if (disable_edge_filters &&
            old_dim[op_dim] >= (min_mod_chunk_sc[op_dim] + 1))
  ...
  for (op_dim = 0; op_dim < (unsigned)space_ndims; op_dim++) {
    hbool_t dims_outside_fill[H5O_LAYOUT_NDIMS]; /* Dimensions in chunk offset
                                                    outside fill dimensions */
    int ndims_outside_fill; /* Number of dimensions in chunk offset outside fill
                               dimensions */
    hbool_t carry; /* Flag to indicate that chunk increment carrys to higher
                      dimension (sorta) */
    ...
    while (!carry) {
      ...
      if (0 == ndims_outside_fill) {
        ...
        assert(H5D__chunk_is_partial_edge_chunk(
            space_ndims, layout->u.chunk.dim, scaled, space_dim));
        ...
        if (UINT_MAX != chk_udata.idx_hint)
          if (H5D__chunk_cache_evict(dset, rdcc->slot[chk_udata.idx_hint],
                                     FALSE) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTREMOVE, FAIL,
                        "unable to evict chunk");
      ...
      for (i = (int)(space_ndims - 1); i >= 0; --i) {
        ...
        if (scaled[i] > max_mod_chunk_sc[i]) {
          ...
          if (!dims_outside_fill[i] &&
              (hssize_t)scaled[i] > max_fill_chunk_sc[i]) {
 ...
 *
 * Purpose:     Callback when obtaining the chunk addresses for all existing
 *chunks
 *
 * Return:    Success:    Non-negative
 *        Failure:    Negative
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__chunk_addrmap_cb(const H5D_chunk_rec_t *chunk_rec,
                                 void *_udata) {
  H5D_chunk_it_ud2_t *udata =
      (H5D_chunk_it_ud2_t *)_udata; /* User data for callback */
  unsigned rank =
      udata->common.layout->ndims - 1; /* # of dimensions of dataset */
  hsize_t chunk_index;

  FUNC_ENTER_PACKAGE_NOERR

  /* Compute the index for this chunk */
  chunk_index = H5VM_array_offset_pre(rank, udata->common.layout->down_chunks,
                                      chunk_rec->scaled);
...
herr_t H5D__chunk_delete(H5F_t *f, H5O_t *oh, H5O_storage_t *storage) {
  ...
  hbool_t layout_read =
      FALSE;         /* Whether the layout message was read from the file */
  H5O_pline_t pline; /* I/O pipeline message */
  hbool_t pline_read =
      FALSE;     /* Whether the I/O pipeline message was read from the file */
  ...
  if ((exists = H5O_msg_exists_oh(oh, H5O_PLINE_ID)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to check for object header message");
  else if (exists) {
    if (NULL == H5O_msg_read_oh(f, oh, H5O_PLINE_ID, &pline))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get I/O pipeline message");
    pline_read = TRUE;
  } /* end else if */
  else
  ...
  if ((storage->u.chunk.ops->idx_delete)(&idx_info) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTDELETE, FAIL,
                "unable to delete chunk index");

done:
  /* Clean up any messages read in */
  if (pline_read)
    if (H5O_msg_reset(H5O_PLINE_ID, &pline) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, FAIL,
                  "unable to reset I/O pipeline message");
  if (layout_read)
    if (H5O_msg_reset(H5O_LAYOUT_ID, &layout) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, FAIL,
                  "unable to reset layout message");
...
static int H5D__chunk_copy_cb(const H5D_chunk_rec_t *chunk_rec, void *_udata) {
  H5D_chunk_it_ud3_t *udata =
      (H5D_chunk_it_ud3_t *)_udata; /* User data for callback */
  ...
  const H5O_pline_t *pline =
      udata->pline; /* I/O pipeline for applying filters */
  ...
  if (pline && pline->nused) {
    ...
    if ((udata->common.layout->flags &
         H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS) &&
        H5D__chunk_is_partial_edge_chunk(udata->dset_ndims,
                                         udata->common.layout->dim,
  ...
  if (udata->do_convert) {
    if (H5T_detect_class(udata->dt_src, H5T_VLEN, FALSE) > 0)
      ...
      HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, H5_ITER_ERROR,
                  "unable to copy dataset elements");
  } /* end if */
  ...
  if (nbytes > buf_size) {
    ...
    if (udata->bkg) {
      ...
      if (!udata->cpy_info->expand_ref)
        memset((uint8_t *)udata->bkg + buf_size, 0,
  ...
  if (udata->chunk_in_cache && udata->chunk) {
    ...
    if (udata->chunk_in_cache) {
      ...
      if (NULL == ent)
        HGOTO_ERROR(H5E_IO, H5E_BADVALUE, H5_ITER_ERROR,
                    "NULL chunk entry pointer");

      assert(H5_addr_defined(chunk_rec->chunk_addr));
      assert(H5_addr_defined(ent->chunk_block.offset));

      H5_CHECKED_ASSIGN(nbytes, size_t, shared_fo->layout.u.chunk.size,
                        uint32_t);
      H5MM_memcpy(buf, ent->chunk, nbytes);
    } else {
      /* read chunk data from the source file */
      if (H5F_block_read(udata->file_src, H5FD_MEM_DRAW, chunk_rec->chunk_addr,
                         nbytes, buf) < 0)
        HGOTO_ERROR(H5E_IO, H5E_READERROR, H5_ITER_ERROR,
                    "unable to read raw data chunk");
    }
  }

  /* Need to uncompress filtered variable-length & reference data elements that
   * are not found in chunk cache
   */
  if (must_filter && (is_vlen || fix_ref) && !udata->chunk_in_cache) {
    unsigned filter_mask = chunk_rec->filter_mask;

    if (H5Z_pipeline(pline, H5Z_FLAG_REVERSE, &filter_mask, H5Z_NO_EDC,
                     filter_cb, &nbytes, &buf_size, &buf) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTFILTER, H5_ITER_ERROR,
                  "data pipeline read failed");
  } /* end if */
  ...
  if (is_vlen) {
    ...
    if (H5T_convert(tpath_src_mem, tid_src, tid_mem, (size_t)udata->nelmts,
                    (size_t)0, (size_t)0, buf, bkg) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, H5_ITER_ERROR,
    ...
    if (H5T_convert(tpath_mem_dst, tid_mem, tid_dst, udata->nelmts, (size_t)0,
                    (size_t)0, buf, bkg) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, H5_ITER_ERROR,
                  "datatype conversion failed");
    ...
    if (H5T_reclaim(tid_mem, buf_space, reclaim_buf) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_BADITER, H5_ITER_ERROR,
    ...
    if (udata->cpy_info->expand_ref) {
      ...
      if (H5O_copy_expand_ref(udata->file_src, udata->tid_src, udata->dt_src,
                              buf, nbytes, udata->idx_info_dst->f, bkg,
                              udata->cpy_info) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, H5_ITER_ERROR,
  ...
  /* Need to compress variable-length or reference data elements or a chunk
   * found in cache before writing to file */
  if (must_filter && (is_vlen || fix_ref || udata->chunk_in_cache)) {
    if (H5Z_pipeline(pline, 0, &(udata_dst.filter_mask), H5Z_NO_EDC, filter_cb,
                     &nbytes, &buf_size, &buf) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTFILTER, H5_ITER_ERROR,
                  "output pipeline failed");
#if H5_SIZEOF_SIZE_T > 4
    /* Check for the chunk expanding too much to encode in a 32-bit value */
    if (nbytes > ((size_t)0xffffffff))
      HGOTO_ERROR(H5E_DATASET, H5E_BADRANGE, H5_ITER_ERROR,
                  "chunk too large for 32-bit length");
#endif /* H5_SIZEOF_SIZE_T > 4 */
  ...
  udata_dst.chunk_idx = H5VM_array_offset_pre(
      udata_dst.common.layout->ndims - 1,
      udata_dst.common.layout->max_down_chunks, udata_dst.common.scaled);

  /* Allocate chunk in the file */
  if (H5D__chunk_file_alloc(udata->idx_info_dst, NULL, &udata_dst.chunk_block,
                            &need_insert, udata_dst.common.scaled) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, FAIL,
                "unable to insert/resize chunk on chunk level");

  /* Write chunk data to destination file */
  assert(H5_addr_defined(udata_dst.chunk_block.offset));
  if (H5F_block_write(udata->idx_info_dst->f, H5FD_MEM_DRAW,
                      udata_dst.chunk_block.offset, nbytes, buf) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, H5_ITER_ERROR,
                "unable to write raw data to file");
  ...
  if (need_insert && udata->idx_info_dst->storage->ops->insert)
    if ((udata->idx_info_dst->storage->ops->insert)(udata->idx_info_dst,
                                                    &udata_dst, NULL) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_copy(H5F_t *f_src, H5O_storage_chunk_t *storage_src,
                       H5O_layout_chunk_t *layout_src, H5F_t *f_dst,
  ...
  hbool_t do_convert =
      FALSE; /* Indicate that type conversions should be performed */
  ...
  /* Initialize layout information */
  {
    ...
    if ((sndims = H5S_extent_get_dims(ds_extent_src, curr_dims, max_dims)) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get dataspace dimensions");
  ...
  if ((tid_src = H5I_register(H5I_DATATYPE, dt_src, FALSE)) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, FAIL,
                "unable to register source file datatype");
  ...
  if (H5T_detect_class(dt_src, H5T_VLEN, FALSE) > 0) {
    ...
    if ((tid_mem = H5I_register(H5I_DATATYPE, dt_mem, FALSE)) < 0) {
      ...
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, FAIL,
                  "unable to register memory datatype");
    } /* end if */
    ...
    if (0 == (max_dt_size = H5T_get_size(dt_src)))
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                  "unable to determine datatype size");
    ...
    if ((sid_buf = H5I_register(H5I_DATASPACE, buf_space, FALSE)) < 0) {
      ...
      HGOTO_ERROR(H5E_ID, H5E_CANTREGISTER, FAIL,
  ...
  if (do_convert) {
    ...
    /* Check for reference datatype and no expanding references & clear
     * background buffer */
  ...
  if ((storage_src->ops->iterate)(&idx_info_src, H5D__chunk_copy_cb, &udata) <
      0)
    HGOTO_ERROR(H5E_DATASET, H5E_BADITER, FAIL,
                "unable to iterate over chunk index to copy data");
      ...
      if (!H5_addr_defined(ent->chunk_block.offset)) {
        ...
        if (H5D__chunk_copy_cb(&chunk_rec, &udata) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                      "unable to copy chunk data in cache");
      }
      next = ent->next;
    } /* end for */
  }
  ...
  if (sid_buf > 0 && H5I_dec_ref(sid_buf) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                "can't decrement temporary dataspace ID");
  if (tid_src > 0 && H5I_dec_ref(tid_src) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                "Can't decrement temporary datatype ID");
  ...
  if (copy_setup_done)
    if (storage_src->ops->copy_shutdown &&
      ...
      HDONE_ERROR(H5E_DATASET, H5E_CANTRELEASE, FAIL,
                  "unable to shut down index copying info");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_bh_info(const H5O_loc_t *loc, H5O_t *oh, H5O_layout_t *layout,
                          hsize_t *index_size) {
  H5D_chk_idx_info_t idx_info; /* Chunked index info */
  ...
  htri_t exists; /* Flag if header message of interest exists */
  hbool_t idx_info_init =
      FALSE; /* Whether the chunk index info has been initialized */
  ...
  if ((exists = H5O_msg_exists_oh(oh, H5O_PLINE_ID)) < 0)
    ...
    if (NULL == H5O_msg_read_oh(loc->file, oh, H5O_PLINE_ID, &pline))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't find I/O pipeline message");
    pline_read = TRUE;
  } /* end else if */
  else
    memset(&pline, 0, sizeof(pline));
  ...
  if (sc->ops->size && (sc->ops->size)(&idx_info, index_size) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "unable to retrieve chunk index info");
...
static int H5D__chunk_dump_index_cb(const H5D_chunk_rec_t *chunk_rec,
  ...
  H5D_chunk_it_ud4_t *udata =
      (H5D_chunk_it_ud4_t *)_udata; /* User data from caller */
  ...
  if (udata->stream) {
    ...
    fprintf(udata->stream, "        0x%08x %8" PRIu32 " %10" PRIuHADDR " [",
  ...
  if (stream) {
    ...
    if ((sc->ops->dump)(sc, stream) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                  "unable to dump chunk index info");
...
herr_t H5D__chunk_stats(const H5D_t *dset, hbool_t headers) {
  ...
  if (headers) {
    ...
    fprintf(H5DEBUG(AC), "   %-18s %8u %8u %7s %8d+%-9ld\n", "raw data chunks",
            rdcc->stats.nhits, rdcc->stats.nmisses, ascii, rdcc->stats.ninits,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__nonexistent_readvv_cb(hsize_t H5_ATTR_UNUSED dst_off,
                                         hsize_t src_off, size_t len,
                                         void *_udata) {
  ...
  if (H5D__fill_init(&fb_info, (udata->rbuf + src_off), NULL, NULL, NULL, NULL,
                     &udata->dset->shared->dcpl_cache.fill,
                     udata->dset->shared->type, udata->dset->shared->type_id,
                     (size_t)0, len) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "can't initialize fill buffer info");
  fb_info_init = TRUE;
  ...
  if (fb_info.has_vlen_fill_type &&
      H5D__fill_refill_vl(&fb_info, fb_info.elmts_per_buf) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, FAIL,
                "can't refill fill value buffer");
...
H5D__nonexistent_readvv(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
                        ...
                        size_t chunk_len_arr[], hsize_t chunk_off_arr[],
                        size_t mem_max_nseq, size_t *mem_curr_seq,
  ...
  if ((ret_value =
           H5VM_opvv(chunk_max_nseq, chunk_curr_seq, chunk_len_arr,
                     chunk_off_arr, mem_max_nseq, mem_curr_seq, mem_len_arr,
                     mem_off_arr, H5D__nonexistent_readvv_cb, &udata)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTOPERATE, FAIL,
                "can't perform vectorized fill value init");
 ...
 *-------------------------------------------------------------------------
 */
hbool_t H5D__chunk_is_partial_edge_chunk(unsigned dset_ndims,
  ...
  *need_insert = FALSE;
  ...
  if (idx_info->pline->nused > 0) {
    ...
    assert(idx_info->storage->idx_type != H5D_CHUNK_IDX_NONE);
    {
      ...
      allow_chunk_size_len =
          1 + ((H5VM_log2_gen((uint64_t)(idx_info->layout->size)) + 8) / 8);
      if (allow_chunk_size_len > 8)
        allow_chunk_size_len = 8;

      /* Compute encoded size of chunk */
      new_chunk_size_len =
          (H5VM_log2_gen((uint64_t)(new_chunk->length)) + 8) / 8;
    ...
    if (old_chunk && H5_addr_defined(old_chunk->offset)) {
      ...
      if (new_chunk->length != old_chunk->length) {
        ...
        if (!(H5F_INTENT(idx_info->f) & H5F_ACC_SWMR_WRITE))
          if (H5MF_xfree(idx_info->f, H5FD_MEM_DRAW, old_chunk->offset,
                         old_chunk->length) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                        "unable to free chunk");
        alloc_chunk = TRUE;
      } /* end if */
      else {
  ...
  if (alloc_chunk) {
    ...
    case H5D_CHUNK_IDX_SINGLE:
      ...
      H5_CHECK_OVERFLOW(new_chunk->length, /*From: */ uint32_t,
                        /*To: */ hsize_t);
      new_chunk->offset =
          H5MF_alloc(idx_info->f, H5FD_MEM_DRAW, (hsize_t)new_chunk->length);
      if (!H5_addr_defined(new_chunk->offset))
...
static int H5D__chunk_format_convert_cb(const H5D_chunk_rec_t *chunk_rec,
  ...
  if (new_idx_info->pline->nused &&
    ...
    if (H5F_block_read(new_idx_info->f, H5FD_MEM_DRAW, chunk_addr, read_size,
                       buf) < 0)
      HGOTO_ERROR(H5E_IO, H5E_READERROR, H5_ITER_ERROR,
                  "unable to read raw data chunk");

    /* Pass the chunk through the pipeline */
    if (H5Z_pipeline(new_idx_info->pline, 0, &filter_mask, H5Z_NO_EDC,
                     filter_cb, &nbytes, &read_size, &buf) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTFILTER, H5_ITER_ERROR,
                  "output pipeline failed");
    ...
    if ((chunk_addr = H5MF_alloc(new_idx_info->f, H5FD_MEM_DRAW,
                                 (hsize_t)nbytes)) == HADDR_UNDEF)
  ...
  if ((new_idx_info->storage->ops->insert)(new_idx_info, &insert_udata, NULL) <
      0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, H5_ITER_ERROR,
                "unable to insert chunk addr into index");
 ...
 *
 * Purpose:     Iterate over the chunks for the current chunk index and insert
 *the the chunk addresses into v1 B-tree chunk index via callback.
 *
 * Return:      Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_format_convert(H5D_t *dset, H5D_chk_idx_info_t *idx_info,
   ...
   * into version 1 B-tree index
   */
  if ((idx_info->storage->ops->iterate)(idx_info, H5D__chunk_format_convert_cb,
                                        &udata) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_BADITER, FAIL,
                "unable to iterate over chunk index to chunk info");
 ...
 */
static int
H5D__chunk_index_empty_cb(const H5D_chunk_rec_t H5_ATTR_UNUSED *chunk_rec,
                          void *_udata) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_index_empty(const H5D_t *dset, hbool_t *empty) {
  H5D_chk_idx_info_t idx_info;   /* Chunked index info */
  H5D_rdcc_ent_t *ent;           /* Cache entry  */
  ...
  *empty = TRUE;
  ...
  if (H5_addr_defined(idx_info.storage->idx_addr)) {
    ...
    if ((dset->shared->layout.storage.u.chunk.ops->iterate)(
            &idx_info, H5D__chunk_index_empty_cb, empty) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "unable to retrieve allocated chunk information from index");
  }
 ...
 */
static int
H5D__get_num_chunks_cb(const H5D_chunk_rec_t H5_ATTR_UNUSED *chunk_rec,
                       void *_udata) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__get_num_chunks(const H5D_t *dset, const H5S_t H5_ATTR_UNUSED *space,
                           hsize_t *nchunks) {
  H5D_chk_idx_info_t idx_info;   /* Chunked index info */
  ...
  if (!H5_addr_defined(idx_info.storage->idx_addr))
    *nchunks = 0;
    ...
    /* Iterate over the allocated chunks */
    if ((dset->shared->layout.storage.u.chunk.ops->iterate)(
            &idx_info, H5D__get_num_chunks_cb, &num_chunks) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "unable to retrieve allocated chunk information from index");
    *nchunks = num_chunks;
  }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__get_chunk_info(const H5D_t *dset, const H5S_t H5_ATTR_UNUSED *space,
                           hsize_t chk_index, hsize_t *offset,
  ...
  if (H5_addr_defined(idx_info.storage->idx_addr)) {
    ...
    /* Iterate over the allocated chunks */
    if ((dset->shared->layout.storage.u.chunk.ops->iterate)(
            &idx_info, H5D__get_chunk_info_cb, &udata) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "unable to retrieve allocated chunk information from index");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__get_chunk_info_by_coord(const H5D_t *dset, const hsize_t *offset,
                                    unsigned *filter_mask, haddr_t *addr,
                                    hsize_t *size) {
  ...
  if (H5_addr_defined(idx_info.storage->idx_addr)) {
    ...
    H5VM_chunk_scaled(dset->shared->ndims, offset, layout->u.chunk.dim,
                      udata.scaled);
    ...
    if ((dset->shared->layout.storage.u.chunk.ops->iterate)(
      ...
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "unable to retrieve information of the chunk by its scaled "
                  "coordinates");
 ...
 *
 * Purpose:     Call the user-defined function with the chunk data. The iterator
 *continues if the user-defined function returns H5_ITER_CONT, and stops if
 *H5_ITER_STOP is returned.
 *
 * Return:      Success:    H5_ITER_CONT or H5_ITER_STOP
 *              Failure:    Negative (H5_ITER_ERROR)
 *
...
static int H5D__chunk_iter_cb(const H5D_chunk_rec_t *chunk_rec, void *udata) {
  ...
  if ((ret_value = (data->op)(offset, (unsigned)chunk_rec->filter_mask,
                              chunk_rec->chunk_addr, (hsize_t)chunk_rec->nbytes,
                              data->op_data)) < 0)
...
herr_t H5D__chunk_iter(H5D_t *dset, H5D_chunk_iter_op_t op, void *op_data) {
  ...
  if (H5_addr_defined(idx_info.storage->idx_addr)) {
    ...
    if ((ret_value = (layout->storage.u.chunk.ops->iterate)(
             &idx_info, H5D__chunk_iter_cb, &ud)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__chunk_get_offset_copy(const H5D_t *dset, const hsize_t *offset,
                                  hsize_t *offset_copy) {
  unsigned u;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  for (u = 0; u < dset->shared->ndims; u++) {
    ...
    if (offset[u] > dset->shared->curr_dims[u])
      HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                  "offset exceeds dimensions of dataset");

    /* Make sure the offset fall right on a chunk's boundary */
    if (offset[u] % dset->shared->layout.u.chunk.dim[u])
      HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                  "offset doesn't fall on chunks's boundary");
