...
static hid_t H5G__open_api_common(hid_t loc_id, const char *name, hid_t gapl_id,
                                  void **token_ptr,
                                  H5VL_object_t **_vol_obj_ptr);
static herr_t H5G__get_info_api_common(hid_t loc_id,
...
static herr_t H5G__get_info_by_name_api_common(hid_t loc_id, const char *name,
                                               ...
                                               H5VL_object_t **_vol_obj_ptr);
static herr_t H5G__get_info_by_idx_api_common(
...
static hid_t H5G__create_api_common(hid_t loc_id, const char *name,
  ...
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_GACC, TRUE, &gapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTSET, H5I_INVALID_HID,
  ...
  if (H5P_DEFAULT == gcpl_id)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                "not a group creation property list");

  /* Set the LCPL for the API context */
  H5CX_set_lcpl(lcpl_id);

  /* Create the group */
  if (NULL == (grp = H5VL_group_create(*vol_obj_ptr, &loc_params, name, lcpl_id,
                                       gcpl_id, gapl_id,
                                       H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, H5I_INVALID_HID,
                "unable to create group");

  /* Get an ID for the group */
  if ((ret_value =
           H5VL_register(H5I_GROUP, grp, (*vol_obj_ptr)->connector, TRUE)) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to get ID for group handle");

done:
  if (H5I_INVALID_HID == ret_value)
    if (grp && H5VL_group_close(*vol_obj_ptr, H5P_DATASET_XFER_DEFAULT,
                                H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, H5I_INVALID_HID,
                  "unable to release group");
 ...
 *                  const char *name; IN: Absolute or relative name of the new
 *group hid_t lcpl_id;	  IN: Property list for link creation hid_t gcpl_id;
 *IN: Property list for group creation hid_t gapl_id;	  IN: Property list for
 *group access
 *
...
hid_t H5Gcreate2(hid_t loc_id, const char *name, hid_t lcpl_id, hid_t gcpl_id,
  ...
  if ((ret_value = H5G__create_api_common(loc_id, name, lcpl_id, gcpl_id,
                                          gapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to synchronously create group");
...
hid_t H5Gcreate_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE9("i", "*s*sIui*siiii", app_file, app_func, app_line, loc_id, name,
  ...
  if ((ret_value = H5G__create_api_common(loc_id, name, lcpl_id, gcpl_id,
                                          gapl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to asynchronously create group");
    ...
    /* clang-format off */
      ...
      if (H5I_dec_app_ref_always_close(ret_value) < 0)
        HDONE_ERROR(H5E_SYM, H5E_CANTDEC, H5I_INVALID_HID,
                    "can't decrement count on group ID");
      HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, H5I_INVALID_HID,
 ...
 *                  hid_t loc_id;	  IN: File or group identifier
 *                  const char *name; IN: Absolute or relative name of the new
 *group hid_t gcpl_id;	  IN: Property list for group creation hid_t gapl_id;
 *IN: Property list for group access
 *
 * Example:     To create missing groups "A" and "B01" along the given path
 *"/A/B01/grp" hid_t create_id = H5Pcreate(H5P_GROUP_CREATE); int   status =
 *H5Pset_create_intermediate_group(create_id, TRUE); hid_t gid =
 *H5Gcreate_anon(file_id, "/A/B01/grp", create_id, H5P_DEFAULT);
 *
 * Return:      Success:    The object ID of a new, empty group open for
 *                          writing. Call H5Gclose() when finished with
 *                          the group.
...
hid_t H5Gcreate_anon(hid_t loc_id, hid_t gcpl_id, hid_t gapl_id) {
  ...
  /* Create the group */
  if (NULL == (grp = H5VL_group_create(
                   vol_obj, &loc_params, NULL, H5P_LINK_CREATE_DEFAULT, gcpl_id,
  ...
  /* Cleanup on failure */
  if (H5I_INVALID_HID == ret_value)
    if (grp && H5VL_group_close(vol_obj, H5P_DATASET_XFER_DEFAULT,
                                H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, H5I_INVALID_HID,
                  "unable to release group");
...
static hid_t H5G__open_api_common(hid_t loc_id, const char *name, hid_t gapl_id,
  ...
  /* Set up object access arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_GACC, FALSE, &gapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");

  if (NULL == (grp = H5VL_group_open(*vol_obj_ptr, &loc_params, name, gapl_id,
                                     H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, H5I_INVALID_HID,
...
hid_t H5Gopen_async(const char *app_file, const char *app_func,
  ...
  if ((ret_value = H5G__open_api_common(loc_id, name, gapl_id, token_ptr,
                                        &vol_obj)) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to asynchronously open group");
...
hid_t H5Gget_create_plist(hid_t group_id) {
  ...
  /* Check args */
  if (NULL ==
      (vol_obj = (H5VL_object_t *)H5I_object_verify(group_id, H5I_GROUP)))
  ...
  if (H5VL_group_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                     H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, H5I_INVALID_HID,
                "can't get group's creation property list");
...
static herr_t H5G__get_info_api_common(hid_t loc_id,
  ...
  if (!group_info)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "group_info parameter cannot be NULL");

  /* Set up VOL callback & object access arguments */
  vol_cb_args.op_type = H5VL_GROUP_GET_INFO;
  if (H5VL_setup_self_args(loc_id, vol_obj_ptr,
                           &vol_cb_args.args.get_info.loc_params) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTSET, FAIL,
                "can't set object access arguments");
  vol_cb_args.args.get_info.ginfo = group_info;

  /* Retrieve group information */
  if (H5VL_group_get(*vol_obj_ptr, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                     token_ptr) < 0)
...
herr_t H5Gget_info(hid_t loc_id, H5G_info_t *group_info /*out*/) {
  ...
  if (H5G__get_info_api_common(loc_id, group_info, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                "unable to synchronously get group info");
...
herr_t H5Gget_info_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE6("e", "*s*sIuixi", app_file, app_func, app_line, loc_id, group_info,
           es_id);
  ...
  if (H5G__get_info_api_common(loc_id, group_info, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                "unable to asynchronously get group info");
...
herr_t H5Gget_info_by_name(hid_t loc_id, const char *name,
                           H5G_info_t *group_info /*out*/, hid_t lapl_id) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Retrieve group information synchronously */
  if (H5G__get_info_by_name_api_common(loc_id, name, group_info, lapl_id, NULL,
                                       NULL) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                "can't synchronously retrieve group info");
...
herr_t H5Gget_info_by_name_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE8("e", "*s*sIui*sxii", app_file, app_func, app_line, loc_id, name,
  ...
  /* Retrieve group information asynchronously */
  if (H5G__get_info_by_name_api_common(loc_id, name, group_info, lapl_id,
                                       token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                "can't asynchronously retrieve group info");
...
herr_t H5Gget_info_by_idx(hid_t loc_id, const char *group_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "i*sIiIohxi", loc_id, group_name, idx_type, order, n,
           group_info, lapl_id);

  /* Retrieve group information synchronously */
  if (H5G__get_info_by_idx_api_common(loc_id, group_name, idx_type, order, n,
                                      group_info, lapl_id, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                "can't synchronously retrieve group info");
...
herr_t H5Gget_info_by_idx_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE11("e", "*s*sIui*sIiIohxii", app_file, app_func, app_line, loc_id,
            group_name, idx_type, order, n, group_info, lapl_id, es_id);
...
herr_t H5Gclose_async(const char *app_file, const char *app_func,
                      unsigned app_line, hid_t group_id, hid_t es_id) {
  H5VL_object_t *vol_obj = NULL; /* Object for loc_id */
  ...
  if (connector && H5VL_conn_dec_rc(connector) < 0)
    HDONE_ERROR(H5E_SYM, H5E_CANTDEC, FAIL,
...
herr_t H5Gflush(hid_t group_id) {
  ...
  if (H5VL_group_specific(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                          H5_REQUEST_NULL) < 0)
