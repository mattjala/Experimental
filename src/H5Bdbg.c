 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent, int fwidth,
  ...
  fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "Tree type ID:",
          ((shared->type->id) == H5B_SNODE_ID
               ? "H5B_SNODE_ID"
               : ((shared->type->id) == H5B_CHUNK_ID ? "H5B_CHUNK_ID"
                                                     : "Unknown!")));
  fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth,
          "Size of node:", shared->sizeof_rnode);
  fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth,
          "Size of raw (disk) key:", shared->sizeof_rkey);
  fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
          "Dirty flag:", bt->cache_info.is_dirty ? "True" : "False");
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth, "Level:", bt->level);
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "Address of left sibling:", bt->left);
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "Address of right sibling:", bt->right);
  fprintf(stream, "%*s%-*s %u (%u)\n", indent, "", fwidth,
          "Number of children (max):", bt->nchildren, shared->two_k);
  ...
  for (u = 0; u < bt->nchildren; u++) {
    ...
    fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent + 3, "",
            MAX(3, fwidth) - 3, "Address:", bt->child[u]);

    /* If there is a key debugging routine, use it to display the left & right
     * keys */
    if (type->debug_key) {
      ...
      (void)(type->debug_key)(stream, indent + 6, MAX(6, fwidth) - 6,
 ...
 */
#ifdef H5B_DEBUG
herr_t H5B__assert(H5F_t *f, haddr_t addr, const H5B_class_t *type,
                   void *udata) {
  H5B_t *bt = NULL;
  H5UC_t *rc_shared;          /* Ref-counted shared info */
  H5B_shared_t *shared;       /* Pointer to shared B-tree info */
  ...
  cache_udata.type = type;
  cache_udata.rc_shared = rc_shared;
  bt = (H5B_t *)H5AC_protect(f, H5AC_BT, addr, &cache_udata,
                             H5AC__READ_ONLY_FLAG);
  assert(bt);
  ...
  for (ncell = 0; cur; ncell++) {
    bt = (H5B_t *)H5AC_protect(f, H5AC_BT, cur->addr, &cache_udata,
                               H5AC__READ_ONLY_FLAG);
    assert(bt);
    ...
    if (cur->level > 0) {
      ...
      for (u = 0; u < bt->nchildren; u++) {
        ...
        cmp = (type->cmp2)(H5B_NKEY(bt, shared, u), udata,
                           H5B_NKEY(bt, shared, u + 1));
        assert(cmp < 0);
      } /* end for */
    }   /* end if */
