...
/* Macro to ease detecting "complex" datatypes (i.e. those with base types or
 * fields) */
#define H5T_IS_COMPLEX(t)                                                      \
  ((t) == H5T_COMPOUND || (t) == H5T_ENUM || (t) == H5T_VLEN ||                \
   (t) == H5T_ARRAY || (t) == H5T_REFERENCE)
...
#define H5T_IS_ATOMIC(dt)                                                      \
  (!(H5T_IS_COMPLEX((dt)->type) || (dt)->type == H5T_OPAQUE))
...
/* This version also encodes the member offset of compound fields more
 * efficiently */
 ...
 * H5T_VISIT_COMPLEX_LAST will mean visiting _only_ "simple" "leafs" in the
 * "tree" */
/* (_not_ setting H5T_VISIT_SIMPLE and setting either H5T_VISIT_COMPLEX_FIRST or
 * H5T_VISIT_COMPLEX_LAST will
 * mean visiting all nodes _except_ "simple" "leafs" in the "tree" */

/* Define an internal macro for converting long long to long double.  Mac
 * OS 10.4 gives some incorrect conversions. */
#if (H5_WANT_DATA_ACCURACY && defined(H5_LLONG_TO_LDOUBLE_CORRECT)) ||         \
    (!H5_WANT_DATA_ACCURACY)
#define H5T_CONV_INTERNAL_LLONG_LDOUBLE 1
#endif

/* Define an internal macro for converting unsigned long long to long double.
 * SGI compilers give some incorrect conversion.  64-bit Solaris does different
 * rounding.   Windows Visual Studio 6 does not support unsigned long long.  For
 * FreeBSD(sleipnir), the last 2 bytes of mantissa are lost when compiler tries
 * to do the conversion.  For Cygwin, compiler doesn't do rounding correctly.
...
/* Define an internal macro for converting long double to long long.  SGI
 * compilers give some incorrect conversions. Mac OS 10.4 gives incorrect
 * conversions. HP-UX 11.00 compiler generates floating exception. The hard
 * conversion on Windows .NET 2003 has a bug and gives wrong exception value. */
#if (H5_WANT_DATA_ACCURACY && defined(H5_LDOUBLE_TO_LLONG_ACCURATE)) ||        \
    (!H5_WANT_DATA_ACCURACY)
#define H5T_CONV_INTERNAL_LDOUBLE_LLONG 1
#endif

/* Define an internal macro for converting long double to unsigned long long.
 * SGI compilers give some incorrect conversions.  Mac OS 10.4 gives incorrect
...
typedef struct H5T_conv_func_t {
  hbool_t
      is_app; /* Whether conversion function is registered from application */
  union {
...
typedef herr_t (*H5T_ref_isnullfunc_t)(const H5VL_object_t *file,
                                       const void *src_buf, hbool_t *isnull);
typedef herr_t (*H5T_ref_setnullfunc_t)(H5VL_object_t *file, void *dst_buf,
                                        void *bg_buf);
typedef size_t (*H5T_ref_getsizefunc_t)(H5VL_object_t *src_file,
                                        const void *src_buf, size_t src_size,
                                        H5VL_object_t *dst_file,
                                        hbool_t *dst_copy);
typedef herr_t (*H5T_ref_readfunc_t)(H5VL_object_t *src_file,
                                     const void *src_buf, size_t src_size,
                                     H5VL_object_t *dst_file, void *dst_buf,
                                     size_t dst_size);
...
typedef herr_t (*H5T_vlen_getlen_func_t)(H5VL_object_t *file,
                                         const void *vl_addr, size_t *len);
typedef void *(*H5T_vlen_getptr_func_t)(void *vl_addr);
typedef herr_t (*H5T_vlen_isnull_func_t)(const H5VL_object_t *file,
                                         void *vl_addr, hbool_t *isnull);
typedef herr_t (*H5T_vlen_setnull_func_t)(H5VL_object_t *file, void *_vl,
                                          void *_bg);
typedef herr_t (*H5T_vlen_read_func_t)(H5VL_object_t *file, void *_vl,
                                       void *buf, size_t len);
typedef herr_t (*H5T_vlen_write_func_t)(
    H5VL_object_t *file, const H5T_vlen_alloc_info_t *vl_alloc_info, void *_vl,
...
typedef struct H5T_vlen_class_t {
  H5T_vlen_getlen_func_t getlen; /* Function to get VL sequence size (in element
                                    units, not bytes) */
...
typedef struct H5T_shared_t {
  ...
  H5VL_object_t
      *owned_vol_obj; /* Vol object owned by this type (free on close) */
  union {
...
struct H5T_t {
  ...
  H5VL_object_t *
      vol_obj; /* pointer to VOL object when working with committed datatypes */
};
...
H5_DLL herr_t H5T__conv_order_opt(hid_t src_id, hid_t dst_id,
...
H5_DLL herr_t H5T__conv_struct(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t bkg_stride, void *_buf, void *bkg);
H5_DLL herr_t H5T__conv_struct_opt(hid_t src_id, hid_t dst_id,
                                   H5T_cdata_t *cdata, size_t nelmts,
                                   size_t buf_stride, size_t bkg_stride,
                                   void *_buf, void *bkg);
H5_DLL herr_t H5T__conv_enum(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t bkg_stride, void *buf, void *bkg);
H5_DLL herr_t H5T__conv_enum_numeric(hid_t src_id, hid_t dst_id,
                                     H5T_cdata_t *cdata, size_t nelmts,
                                     size_t buf_stride, size_t bkg_stride,
                                     void *buf, void *bkg);
...
H5_DLL herr_t H5T__conv_schar_uchar(hid_t src_id, hid_t dst_id,
                                    H5T_cdata_t *cdata, size_t nelmts,
                                    size_t buf_stride, size_t bkg_stride,
                                    void *buf, void *bkg);
H5_DLL herr_t H5T__conv_uchar_schar(hid_t src_id, hid_t dst_id,
                                    H5T_cdata_t *cdata, size_t nelmts,
                                    size_t buf_stride, size_t bkg_stride,
                                    void *buf, void *bkg);
H5_DLL herr_t H5T__conv_schar_short(hid_t src_id, hid_t dst_id,
                                    H5T_cdata_t *cdata, size_t nelmts,
                                    size_t buf_stride, size_t bkg_stride,
                                    void *buf, void *bkg);
H5_DLL herr_t H5T__conv_schar_ushort(hid_t src_id, hid_t dst_id,
                                     H5T_cdata_t *cdata, size_t nelmts,
                                     size_t buf_stride, size_t bkg_stride,
                                     void *buf, void *bkg);
H5_DLL herr_t H5T__conv_uchar_short(hid_t src_id, hid_t dst_id,
                                    H5T_cdata_t *cdata, size_t nelmts,
                                    size_t buf_stride, size_t bkg_stride,
                                    void *buf, void *bkg);
H5_DLL herr_t H5T__conv_uchar_ushort(hid_t src_id, hid_t dst_id,
                                     H5T_cdata_t *cdata, size_t nelmts,
                                     size_t buf_stride, size_t bkg_stride,
                                     void *buf, void *bkg);
...
H5_DLL herr_t H5T__conv_uint_ushort(hid_t src_id, hid_t dst_id,
                                    ...
                                    size_t buf_stride, size_t bkg_stride,
                                    void *buf, void *bkg);
H5_DLL herr_t H5T__conv_int_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                 size_t nelmts, size_t buf_stride,
                                 size_t bkg_stride, void *buf, void *bkg);
H5_DLL herr_t H5T__conv_uint_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                 size_t nelmts, size_t buf_stride,
                                 size_t bkg_stride, void *buf, void *bkg);
H5_DLL herr_t H5T__conv_int_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                 size_t nelmts, size_t buf_stride,
                                 size_t bkg_stride, void *buf, void *bkg);
...
H5_DLL herr_t H5T__conv_ulong_ushort(hid_t src_id, hid_t dst_id,
                                     ...
                                     size_t buf_stride, size_t bkg_stride,
                                     void *buf, void *bkg);
H5_DLL herr_t H5T__conv_long_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                 size_t nelmts, size_t buf_stride,
                                 size_t bkg_stride, void *buf, void *bkg);
...
H5_DLL herr_t H5T__bit_shift(uint8_t *buf, ssize_t shift_dist, size_t offset,
                             size_t size);
...
H5_DLL void H5T__bit_set_d(uint8_t *buf, size_t offset, size_t size,
                           uint64_t val);
H5_DLL ssize_t H5T__bit_find(const uint8_t *buf, size_t offset, size_t size,
...
H5_DLL herr_t H5T__vlen_reclaim(void *elem, const H5T_t *dt,
                                H5T_vlen_alloc_info_t *alloc_info);
H5_DLL htri_t H5T__vlen_set_loc(H5T_t *dt, H5VL_object_t *file, H5T_loc_t loc);

/* Array functions */
H5_DLL H5T_t *H5T__array_create(H5T_t *base, unsigned ndims,
...
H5_DLL herr_t H5T__enum_insert(const H5T_t *dt, const char *name,
                               const void *value);
H5_DLL herr_t H5T__get_member_value(const H5T_t *dt, unsigned membno,
                                    void *value);
