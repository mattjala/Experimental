...
static herr_t H5O__copy_expand_ref_region1(H5O_loc_t *src_oloc,
                                           ...
                                           H5O_copy_t *cpy_info);
static herr_t
H5O__copy_expand_ref_object2(H5O_loc_t *src_oloc, hid_t tid_src,
                             const H5T_t *dt_src, const void *buf_src,
                             size_t nbytes_src, H5O_loc_t *dst_oloc,
                             H5G_loc_t *dst_root_loc, void *buf_dst,
...
static herr_t H5O__copy_obj_by_ref(H5O_loc_t *src_oloc, H5O_loc_t *dst_oloc,
  ...
  if ((ret_value = H5O_copy_header_map(src_oloc, dst_oloc, cpy_info, FALSE,
                                       NULL, NULL)) < 0)
...
static herr_t H5O__copy_expand_ref_object1(H5O_loc_t *src_oloc,
  ...
  for (i = 0; i < ref_count; i++) {
    ...
    if (0 == memcmp(src_buf, zeros, buf_size))
      ...
      if (H5R__decode_token_obj_compat(src_buf, &buf_size, &tmp_token,
                                       token_size) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTDECODE, FAIL,
                    "unable to decode src object address");
      if (H5VL_native_token_to_addr(src_oloc->file, H5I_FILE, tmp_token,
                                    &src_oloc->addr) < 0)
      ...
      if (H5VL_native_addr_to_token(dst_oloc->file, H5I_FILE, dst_oloc->addr,
                                    &tmp_token) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTSERIALIZE, FAIL,
                    "can't serialize address into object token");
      if (H5R__encode_token_obj_compat((const H5O_token_t *)&tmp_token,
                                       token_size, dst_buf, &buf_size) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTDECODE, FAIL,
                    "unable to encode dst object address");
    } /* end else */
  }   /* end for */
...
static herr_t H5O__copy_expand_ref_region1(H5O_loc_t *src_oloc,
  ...
  for (i = 0; i < ref_count; i++) {
    ...
    if (0 == memcmp(src_buf, zeros, buf_size))
      ...
      /* Read from heap */
      if (H5R__decode_heap(src_oloc->file, src_buf, &buf_size, &data,
                           &data_size) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTDECODE, FAIL,
                    "unable to decode dataset region information");
      ...
      /* Attempt to copy object from source to destination file */
      if (H5O__copy_obj_by_ref(src_oloc, dst_oloc, dst_root_loc, cpy_info) <
          0) {
        H5MM_free(data);
        HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL, "unable to copy object");
      } /* end if */
      ...
      /* Write to heap */
      if (H5R__encode_heap(dst_oloc->file, dst_buf, &buf_size, data,
                           (size_t)data_size) < 0) {
        H5MM_free(data);
        HGOTO_ERROR(H5E_OHDR, H5E_CANTENCODE, FAIL,
                    "unable to encode dataset region information");
      }
...
H5O__copy_expand_ref_object2(H5O_loc_t *src_oloc, hid_t tid_src,
  ...
  if ((tid_src == H5I_INVALID_HID) &&
      (tid_src = H5I_register(H5I_DATATYPE, dt_src, FALSE)) < 0)
  ...
  /* Use extra conversion buffer (TODO we should avoid using an extra buffer
   * once the H5Ocopy code has been reworked) */
  ...
  if (H5T_convert(tpath_src_mem, tid_src, tid_mem, ref_count, (size_t)0,
                  (size_t)0, conv_buf, NULL) < 0)
  ...
  for (i = 0; i < ref_count; i++) {
    ...
    if (memcmp(ref, zeros, H5R_REF_BUF_SIZE)) {
      ...
      if (H5VL_native_addr_to_token(dst_oloc->file, H5I_FILE, dst_oloc->addr,
        ...
        HGOTO_ERROR(H5E_OHDR, H5E_CANTSERIALIZE, FAIL,
                    "can't serialize address into object token");
      if (H5R__set_obj_token(ref, (const H5O_token_t *)&tmp_token, token_size) <
          0)
      ...
      if (H5R__set_loc_id(ref, dst_loc_id, TRUE, FALSE) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                    "unable to set destination loc id");
    } /* end if */
  }   /* end for */
  ...
  if (H5T_convert(tpath_mem_dst, tid_mem, tid_dst, ref_count, (size_t)0,
                  (size_t)0, conv_buf, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCONVERT, FAIL, "datatype conversion failed");
  ...
  if (H5T_reclaim(tid_mem, buf_space, reclaim_buf) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_BADITER, FAIL,
                "unable to reclaim reference data");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_copy_expand_ref(H5F_t *file_src, hid_t tid_src, const H5T_t *dt_src,
                           void *buf_src, size_t nbytes_src, H5F_t *file_dst,
                           void *buf_dst, H5O_copy_t *cpy_info) {
  H5O_loc_t dst_oloc; /* Copied object object location */
  H5O_loc_t src_oloc; /* Temporary object location for source object */
  H5G_loc_t
      dst_root_loc; /* The location of root group of the destination file */
  size_t ref_count;
  herr_t ret_value = SUCCEED;
  ...
  case H5R_OBJECT1:
    if (H5O__copy_expand_ref_object1(&src_oloc, buf_src, &dst_oloc,
                                     &dst_root_loc, buf_dst, ref_count,
                                     cpy_info) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL,
                  "unable to expand H5R_OBJECT1 reference");
    break;
  case H5R_DATASET_REGION1:
    if (H5O__copy_expand_ref_region1(&src_oloc, buf_src, &dst_oloc,
                                     &dst_root_loc, buf_dst, ref_count,
                                     cpy_info) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL,
                  "unable to expand H5R_DATASET_REGION1 reference");
    break;
  case H5R_DATASET_REGION2:
  case H5R_ATTR:
  case H5R_OBJECT2:
    if (H5O__copy_expand_ref_object2(&src_oloc, tid_src, dt_src, buf_src,
                                     nbytes_src, &dst_oloc, &dst_root_loc,
