 ...
 *
 *	<sblk idx>  <# of dblks>  <size of dblks>       Range of elements in
 *sblk
 *	==========  ============  =============== ========================= 0 1
 *1 * <dblk min elmts>   0 * <dblk min elmts> <->   1 * <dblk min elmts> - 1 1
 *1         2 * <dblk min elmts>   1 * <dblk min elmts> <->   3 * <dblk min
 *elmts> - 1 2          2         2 * <dblk min elmts>   3 * <dblk min elmts>
 *<->   7 * <dblk min elmts> - 1 3          2         4 * <dblk min elmts>   7 *
 *<dblk min elmts> <->  15 * <dblk min elmts> - 1 4          4         4 * <dblk
 *min elmts>  15 * <dblk min elmts> <->  31 * <dblk min elmts> - 1 5          4
 *8 * <dblk min elmts>  31 * <dblk min elmts> <->  63 * <dblk min elmts> - 1 6
 *8         8 * <dblk min elmts>  63 * <dblk min elmts> <-> 127 * <dblk min
 *elmts> - 1 7          8        16 * <dblk min elmts> 127 * <dblk min elmts>
 *<-> 255 * <dblk min elmts> - 1 .          .         . * <dblk min elmts>   . *
 *<dblk min elmts> <->   . * <dblk min elmts> - 1 .          .         . * <dblk
 *min elmts>   . * <dblk min elmts> <->   . * <dblk min elmts> - 1 .          .
 *. * <dblk min elmts>   . * <dblk min elmts> <->   . * <dblk min elmts> - 1
 *
 *	Therefore:
 *		<sblk idx>(<elmt idx>) = lg2((<elmt idx> / <dblk min elmts>) +
 *1)
 *		<# of dblks>(<sblk idx>) = 2 ^ (<sblk idx> / 2)
 *		<size of dblk>(<sblk idx>) = 2 ^ ((<sblk idx> + 1) / 2)
 *		<total # of sblks>(<max. # of elmts>) = 1 + (lg2(<max. # of
 *elmts>) - lg2(<dblk min_elmts>))
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
...
herr_t H5EA__hdr_init(H5EA_hdr_t *hdr, void *ctx_udata) {
  ...
  hdr->nsblks = 1 + (hdr->cparam.max_nelmts_bits -
                     H5VM_log2_of2(hdr->cparam.data_blk_min_elmts));
  hdr->dblk_page_nelmts = (size_t)1 << hdr->cparam.max_dblk_page_nelmts_bits;
  hdr->arr_off_size =
      (unsigned char)H5EA_SIZEOF_OFFSET_BITS(hdr->cparam.max_nelmts_bits);
  ...
  if (NULL == (hdr->sblk_info = H5FL_SEQ_MALLOC(H5EA_sblk_info_t, hdr->nsblks)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, FAIL,
                "memory allocation failed for super block info array");
  ...
  for (u = 0; u < hdr->nsblks; u++) {
    ...
    hdr->sblk_info[u].dblk_nelmts =
        H5EA_SBLK_DBLK_NELMTS(u, hdr->cparam.data_blk_min_elmts);
    hdr->sblk_info[u].start_idx = start_idx;
    hdr->sblk_info[u].start_dblk = start_dblk;

    /* Advance starting indices for next super block */
    start_idx += (hsize_t)hdr->sblk_info[u].ndblks *
                 (hsize_t)hdr->sblk_info[u].dblk_nelmts;
 ...
 *
 * Return:	Non-NULL pointer to buffer for elements on success/NULL on
 *failure
 *
 *-------------------------------------------------------------------------
 */
void *H5EA__hdr_alloc_elmts(H5EA_hdr_t *hdr, size_t nelmts) {
  ...
  idx = H5VM_log2_of2((uint32_t)nelmts) -
        H5VM_log2_of2((uint32_t)hdr->cparam.data_blk_min_elmts);
  ...
  if (idx >= hdr->elmt_fac.nalloc) {
    ...
    size_t new_nalloc = MAX3(
        1, (idx + 1),
        (2 * hdr->elmt_fac.nalloc)); /* New number of factories allocated */

    /* Re-allocate array of element factories */
    if (NULL == (new_fac = H5FL_SEQ_REALLOC(H5FL_fac_head_ptr_t,
                                            hdr->elmt_fac.fac, new_nalloc)))
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL,
  ...
  if (NULL == hdr->elmt_fac.fac[idx]) {
    if (NULL == (hdr->elmt_fac.fac[idx] = H5FL_fac_init(
                     nelmts * (size_t)hdr->cparam.cls->nat_elmt_size)))
 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5EA__hdr_create(H5F_t *f, const H5EA_create_t *cparam,
                         void *ctx_udata) {
...
#ifndef NDEBUG
  {
    ...
    if (cparam->max_nelmts_bits > H5EA_MAX_NELMTS_IDX_MAX)
      HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF,
                  "max. # of elements bits must be <= %u",
    ...
    if (dblk_page_nelmts < cparam->idx_blk_elmts)
      HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF,
                  "# of elements per data block page must be greater than # of "
                  "elements in index block");

    /* Compute the number of elements in data blocks for first actual super
     * block */
    ...
    if (dblk_page_nelmts < dblk_nelmts)
      HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF,
                  "max. # of elements per data block page bits must be > # of "
                  "elements in "
                  "first data block from super block");

    if (cparam->max_dblk_page_nelmts_bits > cparam->max_nelmts_bits)
      HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, HADDR_UNDEF,
                  "max. # of elements per data block page bits must be <= max. "
                  "# of elements bits");
  }
#endif /* NDEBUG */
  ...
  if (HADDR_UNDEF ==
      (hdr->addr = H5MF_alloc(f, H5FD_MEM_EARRAY_HDR, (hsize_t)hdr->size)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF,
  ...
  if (hdr->swmr_write)
    if (NULL == (hdr->top_proxy = H5AC_proxy_entry_create()))
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTCREATE, HADDR_UNDEF,
                  "can't create extensible array entry proxy");

  /* Cache the new extensible array header */
  if (H5AC_insert_entry(f, H5AC_EARRAY_HDR, hdr->addr, hdr,
                        H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTINSERT, HADDR_UNDEF,
                "can't add extensible array header to cache");
  inserted = TRUE;
  ...
  if (hdr->top_proxy)
    if (H5AC_proxy_entry_add_child(hdr->top_proxy, f, hdr) < 0)
      HGOTO_ERROR(
          H5E_EARRAY, H5E_CANTSET, HADDR_UNDEF,
    ...
    if (hdr) {
      ...
      if (H5_addr_defined(hdr->addr) &&
        ...
        HDONE_ERROR(H5E_EARRAY, H5E_CANTFREE, HADDR_UNDEF,
                    "unable to free extensible array header");
...
herr_t H5EA__hdr_incr(H5EA_hdr_t *hdr) {
  ...
  if (hdr->rc == 0)
    if (H5AC_pin_protected_entry(hdr) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTPIN, FAIL,
                  "unable to pin extensible array header");
...
herr_t H5EA__hdr_decr(H5EA_hdr_t *hdr) {
  ...
  if (hdr->rc == 0) {
    ...
    if (H5AC_unpin_entry(hdr) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTUNPIN, FAIL,
                  "unable to unpin extensible array header");
  }
...
herr_t H5EA__hdr_modified(H5EA_hdr_t *hdr) {
  ...
  if (H5AC_mark_entry_dirty(hdr) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTMARKDIRTY, FAIL,
                "unable to mark extensible array header as dirty");
 ...
 *-------------------------------------------------------------------------
 */
H5EA_hdr_t *H5EA__hdr_protect(H5F_t *f, haddr_t ea_addr, void *ctx_udata,
                              unsigned flags) {
  ...
  /* Protect the header */
  if (NULL == (hdr = (H5EA_hdr_t *)H5AC_protect(f, H5AC_EARRAY_HDR, ea_addr,
                                                &udata, flags)))
  ...
  hdr->f = f; /* (Must be set again here, in case the header was already in the
                 cache -QAK) */
  ...
  if (hdr->swmr_write && NULL == hdr->top_proxy) {
    /* Create 'top' proxy for extensible array entries */
    if (NULL == (hdr->top_proxy = H5AC_proxy_entry_create()))
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTCREATE, NULL,
                  "can't create extensible array entry proxy");

    /* Add header as child of 'top' proxy */
    if (H5AC_proxy_entry_add_child(hdr->top_proxy, f, hdr) < 0)
      HGOTO_ERROR(
          H5E_EARRAY, H5E_CANTSET, NULL,
          "unable to add extensible array entry as child of array proxy");
  } /* end if */
...
herr_t H5EA__hdr_delete(H5EA_hdr_t *hdr) {
  ...
  if (H5AC_get_entry_status(hdr->f, hdr->addr, &hdr_status) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTGET, FAIL,
                "unable to check metadata cache status for array header");
  ...
  if (H5_addr_defined(hdr->idx_blk_addr)) {
    ...
    if (H5EA__iblock_delete(hdr) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTDELETE, FAIL,
                  "unable to delete extensible array index block");
  } /* end if */
...
herr_t H5EA__hdr_dest(H5EA_hdr_t *hdr) {
  ...
  if (hdr->elmt_fac.fac) {
    ...
    /* Free factory array */
    hdr->elmt_fac.fac = (H5FL_fac_head_t **)H5FL_SEQ_FREE(H5FL_fac_head_ptr_t,
                                                          hdr->elmt_fac.fac);
  } /* end if */
  ...
  if (hdr->sblk_info)
    hdr->sblk_info =
        (H5EA_sblk_info_t *)H5FL_SEQ_FREE(H5EA_sblk_info_t, hdr->sblk_info);

  /* Destroy the 'top' proxy */
  if (hdr->top_proxy) {
    if (H5AC_proxy_entry_dest(hdr->top_proxy) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTRELEASE, FAIL,
                  "unable to destroy extensible array 'top' proxy");
    hdr->top_proxy = NULL;
  } /* end if */
