...
#define H5VL_NATIVE_CAP_FLAGS                                                  \
  (H5VL_CAP_FLAG_NATIVE_FILES | H5VL_CAP_FLAG_ATTR_BASIC |                     \
   H5VL_CAP_FLAG_ATTR_MORE | H5VL_CAP_FLAG_DATASET_BASIC |                     \
   H5VL_CAP_FLAG_DATASET_MORE | H5VL_CAP_FLAG_FILE_BASIC |                     \
   H5VL_CAP_FLAG_FILE_MORE | H5VL_CAP_FLAG_GROUP_BASIC |                       \
   H5VL_CAP_FLAG_GROUP_MORE | H5VL_CAP_FLAG_LINK_BASIC |                       \
   H5VL_CAP_FLAG_LINK_MORE | H5VL_CAP_FLAG_OBJECT_BASIC |                      \
   H5VL_CAP_FLAG_OBJECT_MORE | H5VL_CAP_FLAG_REF_BASIC |                       \
   H5VL_CAP_FLAG_REF_MORE | H5VL_CAP_FLAG_OBJ_REF | H5VL_CAP_FLAG_REG_REF |    \
   H5VL_CAP_FLAG_ATTR_REF | H5VL_CAP_FLAG_STORED_DATATYPES |                   \
   H5VL_CAP_FLAG_CREATION_ORDER | H5VL_CAP_FLAG_ITERATE |                      \
   H5VL_CAP_FLAG_STORAGE_SIZE | H5VL_CAP_FLAG_BY_IDX |                         \
   H5VL_CAP_FLAG_GET_PLIST | H5VL_CAP_FLAG_FLUSH_REFRESH |                     \
   H5VL_CAP_FLAG_EXTERNAL_LINKS | H5VL_CAP_FLAG_HARD_LINKS |                   \
   H5VL_CAP_FLAG_SOFT_LINKS | H5VL_CAP_FLAG_UD_LINKS |                         \
   H5VL_CAP_FLAG_TRACK_TIMES | H5VL_CAP_FLAG_MOUNT | H5VL_CAP_FLAG_FILTERS |   \
...
hid_t H5VL_native_register(void) {
  ...
  if (H5I_INVALID_HID == H5VL_NATIVE_ID_g) {
    ...
    if ((H5VL_NATIVE_ID_g = H5VL__register_connector(
             &H5VL_native_cls_g, TRUE, H5P_VOL_INITIALIZE_DEFAULT)) < 0)
      HGOTO_ERROR(H5E_VOL, H5E_CANTINSERT, H5I_INVALID_HID,
                  "can't create ID for native VOL connector");
 ...
 *---------------------------------------------------------------------------
 */
herr_t H5VL__native_introspect_get_cap_flags(const void H5_ATTR_UNUSED *info,
                                             uint64_t *cap_flags) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_get_file_addr_len(void *obj, H5I_type_t obj_type,
                                      size_t *addr_len) {
  ...
  if (H5VL_native_get_file_struct(obj, obj_type, &file) < 0)
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "couldn't get file from VOL object");
...
herr_t H5VLnative_addr_to_token(hid_t loc_id, haddr_t addr,
  ...
  if (H5VL_native_addr_to_token(vol_obj, vol_obj_type, addr, token) < 0)
    HGOTO_ERROR(H5E_VOL, H5E_CANTSERIALIZE, FAIL,
                "couldn't serialize haddr_t into object token");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL_native_addr_to_token(void *obj, H5I_type_t obj_type, haddr_t addr,
                                 H5O_token_t *token) {
  uint8_t *p;
  ...
  if (H5VL__native_get_file_addr_len(obj, obj_type, &addr_len) < 0)
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "couldn't get length of haddr_t from VOL object");
...
herr_t H5VLnative_token_to_addr(hid_t loc_id, H5O_token_t token,
  ...
  if (H5VL_native_token_to_addr(vol_obj, vol_obj_type, token, addr) < 0)
    HGOTO_ERROR(H5E_VOL, H5E_CANTUNSERIALIZE, FAIL,
                "couldn't deserialize object token into haddr_t");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL_native_token_to_addr(void *obj, H5I_type_t obj_type,
