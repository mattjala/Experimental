...
static int H5F__get_all_count_cb(void H5_ATTR_UNUSED *obj_ptr,
                                 hid_t H5_ATTR_UNUSED obj_id, void *key);

/* Callback for getting IDs for open objects in a file */
static int H5F__get_all_ids_cb(void H5_ATTR_UNUSED *obj_ptr, hid_t obj_id,
                               void *key);

/* Helper routines for sync/async API calls */
static herr_t H5F__post_open_api_common(H5VL_object_t *vol_obj,
                                        void **token_ptr);
static hid_t H5F__create_api_common(const char *filename, unsigned flags,
                                    hid_t fcpl_id, hid_t fapl_id,
                                    void **token_ptr);
static hid_t H5F__open_api_common(const char *filename, unsigned flags,
                                  hid_t fapl_id, void **token_ptr);
static hid_t H5F__reopen_api_common(hid_t file_id, void **token_ptr);
static herr_t H5F__flush_api_common(hid_t object_id, H5F_scope_t scope,
                                    void **token_ptr,
...
hid_t H5Fget_create_plist(hid_t file_id) {
  ...
  if (H5VL_file_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTCOPY, H5I_INVALID_HID,
                "unable to retrieve file creation properties");
...
hid_t H5Fget_access_plist(hid_t file_id) {
  ...
  if (H5VL_file_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
    ...
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, H5I_INVALID_HID,
                "can't get file access property list");
...
ssize_t H5Fget_obj_count(hid_t file_id, unsigned types) {
  ...
  if (file_id != (hid_t)H5F_OBJ_ALL) {
    ...
    if (H5VL_file_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                      H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, (-1),
                  "unable to get object count in file(s)");

    /* Set return value */
    ret_value = (ssize_t)count;
  }
  /* If we passed in the 'special' ID, get the count for everything open in the
   * library, iterating over all open files and getting the object count for
   * each.
   */
  else {
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Fget_obj_ids(hid_t file_id, unsigned types, size_t max_objs,
                       hid_t *oid_list /*out*/) {
  ssize_t ret_value = 0; /* Return value */
  ...
  if (file_id != (hid_t)H5F_OBJ_ALL) {
    ...
    if (H5VL_file_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                      H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, (-1),
                  "unable to get object ids in file(s)");
...
herr_t H5Fget_vfd_handle(hid_t file_id, hid_t fapl_id,
  ...
  H5VL_native_file_optional_args_t
      file_opt_args;          /* Arguments for optional operation */
...
htri_t H5Fis_accessible(const char *filename, hid_t fapl_id) {
  ...
  if (H5VL_file_specific(NULL, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_NOTHDF5, FAIL,
                "unable to determine if file is accessible as HDF5");
...
static herr_t H5F__post_open_api_common(H5VL_object_t *vol_obj,
                                        void **token_ptr) {
  uint64_t supported; /* Whether 'post open' operation is supported by VOL
                         connector */
  herr_t ret_value = SUCCEED; /* Return value     */
  ...
  supported = 0;
  if (H5VL_introspect_opt_query(vol_obj, H5VL_SUBCLS_FILE,
                                H5VL_NATIVE_FILE_POST_OPEN, &supported) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                "can't check for 'post open' operation");
...
static hid_t H5F__create_api_common(const char *filename, unsigned flags,
  ...
  if (NULL == (new_file = H5VL_file_create(
                   &connector_prop, filename, flags, fcpl_id, fapl_id,
  ...
  if ((ret_value = H5VL_register_using_vol_id(
           H5I_FILE, new_file, connector_prop.connector_id, TRUE)) < 0)
...
hid_t H5Fcreate(const char *filename, unsigned flags, hid_t fcpl_id,
  ...
  if ((ret_value =
           H5F__create_api_common(filename, flags, fcpl_id, fapl_id, NULL)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to synchronously create file");

  /* Get the file object */
  if (NULL == (vol_obj = H5VL_vol_object(ret_value)))
    HGOTO_ERROR(H5E_FILE, H5E_BADTYPE, H5I_INVALID_HID,
...
hid_t H5Fcreate_async(const char *app_file, const char *app_func,
                      unsigned app_line, const char *filename, unsigned flags,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE8("i", "*s*sIu*sIuiii", app_file, app_func, app_line, filename, flags,
           fcpl_id, fapl_id, es_id);
  ...
  if ((ret_value = H5F__create_api_common(filename, flags, fcpl_id, fapl_id,
                                          token_ptr)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to asynchronously create file");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      if (H5I_dec_app_ref(ret_value) < 0)
        HDONE_ERROR(H5E_FILE, H5E_CANTDEC, H5I_INVALID_HID,
                    "can't decrement count on file ID");
      HGOTO_ERROR(H5E_FILE, H5E_CANTINSERT, H5I_INVALID_HID,
                  "can't insert token into event set");
    } /* end if */

  /* Reset token for 'post open' operation */
  /* (Unnecessary if create operation didn't change it, but not worth checking
   * -QAK) */
  token = NULL;
...
static hid_t H5F__open_api_common(const char *filename, unsigned flags,
  ...
  if (!filename || !*filename)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID, "invalid file name");
  /* Reject undefined flags (~H5F_ACC_PUBLIC_FLAGS) and the H5F_ACC_TRUNC &
   * H5F_ACC_EXCL flags */
  if ((flags & ~H5F_ACC_PUBLIC_FLAGS) || (flags & H5F_ACC_TRUNC) ||
      (flags & H5F_ACC_EXCL))
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "invalid file open flags");
  ...
  if ((flags & H5F_ACC_SWMR_WRITE) && 0 == (flags & H5F_ACC_RDWR))
    HGOTO_ERROR(
        H5E_FILE, H5E_CANTOPENFILE, H5I_INVALID_HID,
  ...
  if (NULL ==
      (new_file = H5VL_file_open(&connector_prop, filename, flags, fapl_id,
                                 H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, H5I_INVALID_HID,
                "unable to open file");
...
hid_t H5Fopen_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE7("i", "*s*sIu*sIuii", app_file, app_func, app_line, filename, flags,
           fapl_id, es_id);
  ...
  if ((ret_value = H5F__open_api_common(filename, flags, fapl_id, token_ptr)) <
      0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, H5I_INVALID_HID,
                "unable to asynchronously open file");
...
static herr_t H5F__flush_api_common(hid_t object_id, H5F_scope_t scope,
  ...
  if (H5I_FILE != obj_type && H5I_GROUP != obj_type &&
      H5I_DATATYPE != obj_type && H5I_DATASET != obj_type &&
      H5I_ATTR != obj_type)
  ...
  /* Flush the object */
  if (H5VL_file_specific(*vol_obj_ptr, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         token_ptr) < 0)
...
herr_t H5Fflush(hid_t object_id, H5F_scope_t scope) {
  ...
  if (H5F__flush_api_common(object_id, scope, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTFLUSH, FAIL,
                "unable to synchronously flush file");
...
herr_t H5Fflush_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE6("e", "*s*sIuiFsi", app_file, app_func, app_line, object_id, scope,
           es_id);
  ...
  if (H5F__flush_api_common(object_id, scope, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTFLUSH, FAIL,
                "unable to asynchronously flush file");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      /* clang-format on */
      HGOTO_ERROR(H5E_FILE, H5E_CANTINSERT, FAIL,
                  "can't insert token into event set");
...
herr_t H5Fdelete(const char *filename, hid_t fapl_id) {
  ...
  /* Get the VOL info from the fapl */
  if (NULL ==
      (plist = (H5P_genplist_t *)H5I_object_verify(fapl_id, H5I_GENPROP_LST)))
  ...
  if (H5VL_file_specific(NULL, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTDELETEFILE, FAIL,
...
herr_t H5Fmount(hid_t loc_id, const char *name, hid_t child_id,
  ...
  int same_connector =
      0; /* Whether parent and child files use the same connector */
  herr_t ret_value = SUCCEED; /* Return value         */
  ...
  if (H5I_FILE != loc_type && H5I_GROUP != loc_type)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "loc_id parameter not a file or group ID");
  if (!name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "name parameter cannot be NULL");
  if (!*name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "name parameter cannot be the empty string");
  if (H5I_FILE != H5I_get_type(child_id))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "child_id parameter not a file ID");
  if (H5P_DEFAULT == plist_id)
    plist_id = H5P_FILE_MOUNT_DEFAULT;
  else if (TRUE != H5P_isa_class(plist_id, H5P_FILE_MOUNT))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "plist_id is not a file mount property list ID");
  ...
  if (H5I_FILE == loc_type) {
    ...
    if (NULL == (grp = H5VL_group_open(vol_obj, &loc_params, "/",
    ...
    if (NULL == (loc_vol_obj = H5VL_create_object(grp, vol_obj->connector)))
      HGOTO_ERROR(H5E_FILE, H5E_CANTOPENOBJ, FAIL,
                  "can't create VOL object for root group");
  } /* end if */
  else {
  ...
  if (H5VL_cmp_connector_cls(&same_connector, loc_vol_obj->connector->cls,
                             child_vol_obj->connector->cls) < 0)
  ...
  vol_cb_args.args.mount.child_file =
      child_vol_obj
          ->data; /* Don't unwrap fully, so each connector can see its object */
   ...
   */
  if (H5VL_group_specific(loc_vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                          H5_REQUEST_NULL) < 0)
  ...
  if (grp) {
    assert(loc_vol_obj);
    if (H5VL_group_close(loc_vol_obj, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
...
static hid_t H5F__reopen_api_common(hid_t file_id, void **token_ptr) {
  ...
  /* Reopen the file */
  if (H5VL_file_specific(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         token_ptr) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, H5I_INVALID_HID,
                "unable to reopen file via the VOL connector");
  ...
  /* Get an ID for the file */
  if ((ret_value =
           H5VL_register(H5I_FILE, reopen_file, vol_obj->connector, TRUE)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register file handle");
...
hid_t H5Freopen(hid_t file_id) {
  ...
  if ((ret_value = H5F__reopen_api_common(file_id, NULL)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, H5I_INVALID_HID,
                "unable to synchronously reopen file");

  /* Get the file object */
  if (NULL == (vol_obj = H5VL_vol_object(ret_value)))
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, H5I_INVALID_HID,
                "can't get handle for re-opened file");
...
hid_t H5Freopen_async(const char *app_file, const char *app_func,
  ...
  if ((ret_value = H5F__reopen_api_common(file_id, token_ptr)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, H5I_INVALID_HID,
                "unable to asynchronously reopen file");
...
herr_t H5Fget_intent(hid_t file_id, unsigned *intent_flags /*out*/) {
  ...
  if (intent_flags) {
    ...
    if (H5VL_file_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                      H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "unable to get file's intent flags");
  } /* end if */
...
herr_t H5Fget_fileno(hid_t file_id, unsigned long *fnumber /*out*/) {
  ...
  if (fnumber) {
    ...
    if (H5VL_file_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                      H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "unable to get file's 'file number'");
  } /* end if */
...
herr_t H5Fget_mdc_config(hid_t file_id, H5AC_cache_config_t *config /*out*/) {
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                "unable to get metadata cache configuration");
...
herr_t H5Fset_mdc_config(hid_t file_id, const H5AC_cache_config_t *config_ptr) {
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                "unable to set metadata cache configuration");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Fget_mdc_size(hid_t file_id, size_t *max_size /*out*/,
                       size_t *min_clean_size /*out*/, size_t *cur_size /*out*/,
                       int *cur_num_entries /*out*/) {
  H5VL_object_t *vol_obj;
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_file_optional_args_t
      file_opt_args;          /* Arguments for optional operation */
...
herr_t H5Fget_info2(hid_t obj_id, H5F_info2_t *finfo /*out*/) {
  ...
  if (!finfo)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "file info pointer can't be NULL");
...
herr_t H5Fget_metadata_read_retry_info(hid_t file_id,
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                "can't get metadata read retry info");
 ...
 *
 * Purpose:     To get free-space section information for free-space manager
 *with TYPE that is associated with file FILE_ID. If SECT_INFO is null, this
 *routine returns the total # of free-space sections.
 *
...
ssize_t H5Fget_free_sections(hid_t file_id, H5F_mem_t type, size_t nsects,
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, (-1),
                "unable to get file free sections");
...
herr_t H5Fclear_elink_file_cache(hid_t file_id) {
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
    ...
    HGOTO_ERROR(H5E_FILE, H5E_CANTRELEASE, FAIL,
                "can't release external file cache");
...
herr_t H5Fstart_swmr_write(hid_t file_id) {
  ...
  if (NULL == (vol_obj = (H5VL_object_t *)H5I_object_verify(file_id, H5I_FILE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "hid_t identifier is not a file ID");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Fget_mdc_logging_status(hid_t file_id, hbool_t *is_enabled /*out*/,
  ...
  file_opt_args.get_mdc_logging_status.is_currently_logging =
...
herr_t H5Fset_libver_bounds(hid_t file_id, H5F_libver_t low,
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                "can't set library version bounds");
...
herr_t H5Fformat_convert(hid_t file_id) {
  ...
  if (NULL == (vol_obj = (H5VL_object_t *)H5I_object_verify(file_id, H5I_FILE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "file_id parameter is not a valid file identifier");
...
herr_t H5Freset_page_buffering_stats(hid_t file_id) {
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                "can't reset stats for page buffering");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Fget_page_buffering_stats(hid_t file_id, unsigned accesses[2] /*out*/,
                                   unsigned hits[2] /*out*/,
  ...
  if (NULL == (vol_obj = (H5VL_object_t *)H5I_object_verify(file_id, H5I_FILE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "not a file ID");
  if (NULL == accesses || NULL == hits || NULL == misses || NULL == evictions ||
      NULL == bypasses)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "NULL input parameters for stats");
  ...
  if (H5VL_file_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                         H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                "can't retrieve stats for page buffering");
 ...
 *
 * Purpose:     Retrieves the image_addr and image_len for the cache image in
 *the file. image_addr:  --base address of the on disk metadata cache image
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Fget_mdc_image_info(hid_t file_id, haddr_t *image_addr /*out*/,
                             hsize_t *image_len /*out*/) {
  H5VL_object_t *vol_obj;           /* File info */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_file_optional_args_t
      file_opt_args;          /* Arguments for optional operation */
 ...
 *
 * Purpose:     Set the EOA for the file to the maximum of (EOA, EOF) +
 *increment
 *
 * Return:      Success:    Non-negative
 *              Failure:    Negative
 ...
 *
 * Purpose:     Get the file-level setting to create minimized dataset object
 *headers. Result is stored at pointer `minimize`.
 *
 * Return:      Success:    Non-negative
 *              Failure:    Negative
 *-------------------------------------------------------------------------
 */
herr_t H5Fget_dset_no_attrs_hint(hid_t file_id, hbool_t *minimize /*out*/) {
  H5VL_object_t *vol_obj;           /* File info */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_file_optional_args_t
      file_opt_args;          /* Arguments for optional operation */
