...
typedef struct H5FD_log_fapl_t {
  ...
  size_t buf_size; /* Size of buffers for track flavor and number of times each
                      byte is accessed */
} H5FD_log_fapl_t;
...
typedef struct H5FD_log_t {
  ...
  unsigned long long
      total_truncate_ops; /* Total number of truncate operations              */
...
static haddr_t H5FD__log_alloc(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                               hsize_t size);
static herr_t H5FD__log_free(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
...
static herr_t H5FD__log_get_handle(H5FD_t *_file, hid_t fapl,
                                   void **file_handle);
static herr_t H5FD__log_read(H5FD_t *_file, H5FD_mem_t type, hid_t fapl_id,
                             haddr_t addr, size_t size, void *buf);
static herr_t H5FD__log_write(H5FD_t *_file, H5FD_mem_t type, hid_t fapl_id,
...
static const H5FD_log_fapl_t H5FD_log_default_config_g = {
    NULL, H5FD_LOG_LOC_IO | H5FD_LOG_ALLOC, 4096};
...
static void *H5FD__log_fapl_copy(const void *_old_fa) {
  ...
  if (NULL ==
      (new_fa = (H5FD_log_fapl_t *)H5MM_calloc(sizeof(H5FD_log_fapl_t))))
    ...
    if (file->fa.flags & H5FD_LOG_FILE_WRITE) {
      ...
      while (addr < file->eoa) {
        if (file->nwrite[addr] != last_val) {
          fprintf(file->logfp,
                  "\tAddr %10" PRIuHADDR "-%10" PRIuHADDR
                  " (%10lu bytes) written to %3d times\n",
                  last_addr, (addr - 1), (unsigned long)(addr - last_addr),
                  (int)last_val);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__log_query(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__log_alloc(H5FD_t *_file, H5FD_mem_t type,
                               hid_t H5_ATTR_UNUSED dxpl_id, hsize_t size) {
  H5FD_log_t *file = (H5FD_log_t *)_file;
  haddr_t addr;
  ...
  if (file->fa.flags != 0) {
    ...
    if (file->fa.flags & H5FD_LOG_ALLOC)
      fprintf(file->logfp,
              "%10" PRIuHADDR "-%10" PRIuHADDR " (%10" PRIuHSIZE
              " bytes) (%s) Allocated\n",
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__log_free(H5FD_t *_file, H5FD_mem_t type,
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__log_get_eoa(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__log_get_eof(const H5FD_t *_file,
                                 H5FD_mem_t H5_ATTR_UNUSED type) {
  const H5FD_log_t *file = (const H5FD_log_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__log_get_handle(H5FD_t *_file, hid_t H5_ATTR_UNUSED fapl,
                                   void **file_handle) {
  H5FD_log_t *file = (H5FD_log_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__log_read(H5FD_t *_file, H5FD_mem_t type,
                             hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr,
  ...
  if (addr != file->pos || OP_READ != file->op) {
    ...
    if (file->fa.flags & H5FD_LOG_LOC_SEEK) {
      fprintf(file->logfp, "Seek: From %10" PRIuHADDR " To %10" PRIuHADDR,
              file->pos, addr);
  ...
  while (size > 0) {
    ...
    if (-1 == bytes_read) { /* error */
      ...
      if (file->fa.flags & H5FD_LOG_LOC_READ)
        fprintf(file->logfp,
                "Error! Reading: %10" PRIuHADDR "-%10" PRIuHADDR
                " (%10zu bytes)\n",
  ...
  if (file->fa.flags & H5FD_LOG_LOC_READ) {
    fprintf(file->logfp,
            "%10" PRIuHADDR "-%10" PRIuHADDR " (%10zu bytes) (%s) Read",
            orig_addr, (orig_addr + orig_size) - 1, orig_size, flavors[type]);

    /* Verify that we are reading in the type of data we allocated in this
     * location */
    if (file->flavor) {
      assert(type == H5FD_MEM_DEFAULT ||
             type == (H5FD_mem_t)file->flavor[orig_addr] ||
             (H5FD_mem_t)file->flavor[orig_addr] == H5FD_MEM_DEFAULT);
      assert(type == H5FD_MEM_DEFAULT ||
             type == (H5FD_mem_t)file->flavor[(orig_addr + orig_size) - 1] ||
             (H5FD_mem_t)file->flavor[(orig_addr + orig_size) - 1] ==
                 H5FD_MEM_DEFAULT);
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__log_write(H5FD_t *_file, H5FD_mem_t type,
                              hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr,
  ...
  /* Verify that we are writing out the type of data we allocated in this
   * location */
  if (file->flavor) {
  ...
  while (size > 0) {
    ...
    if (-1 == bytes_wrote) { /* error */
      ...
      if (file->fa.flags & H5FD_LOG_LOC_WRITE)
        fprintf(file->logfp,
                "Error! Writing: %10" PRIuHADDR "-%10" PRIuHADDR
                " (%10zu bytes)\n",
                orig_addr, (orig_addr + orig_size) - 1, orig_size);
  ...
  if (file->fa.flags & H5FD_LOG_LOC_WRITE) {
    fprintf(file->logfp,
            "%10" PRIuHADDR "-%10" PRIuHADDR " (%10zu bytes) (%s) Written",
            orig_addr, (orig_addr + orig_size) - 1, orig_size, flavors[type]);

    /* Check if this is the first write into a "default" section, grabbed by the
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__log_truncate(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
  ...
  if (!H5_addr_eq(file->eoa, file->eof)) {
    ...
    if (file->fa.flags & H5FD_LOG_TRUNCATE) {
       ...
       * Note that the truncate time is NOT emitted for when just
       * H5FD_LOG_TIME_TRUNCATE is set.
       */
      if (file->fa.flags & H5FD_LOG_TIME_TRUNCATE)
        fprintf(file->logfp, " (%fs @ %f)\n", trunc_times.elapsed,
                trunc_timer.initial.elapsed);
      else
        fprintf(file->logfp, "\n");
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__log_delete(const char *filename,
