...
static void *H5O__btreek_decode(H5F_t *f, H5O_t *open_oh, unsigned mesg_flags,
                                unsigned *ioflags, size_t p_size,
                                const uint8_t *p);
static herr_t H5O__btreek_encode(H5F_t *f, hbool_t disable_shared, uint8_t *p,
                                 const void *_mesg);
static void *H5O__btreek_copy(const void *_mesg, void *_dest);
static size_t H5O__btreek_size(const H5F_t *f, hbool_t disable_shared,
                               const void *_mesg);
...
static void *H5O__btreek_decode(H5F_t H5_ATTR_NDEBUG_UNUSED *f,
  ...
  if (NULL == (mesg = (H5O_btreek_t *)H5MM_calloc(sizeof(H5O_btreek_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "memory allocation failed for v1 B-tree 'K' message");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__btreek_encode(H5F_t H5_ATTR_UNUSED *f,
...
static void *H5O__btreek_copy(const void *_mesg, void *_dest) {
  ...
  if (!dest &&
      NULL == (dest = (H5O_btreek_t *)H5MM_malloc(sizeof(H5O_btreek_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__btreek_size(const H5F_t H5_ATTR_UNUSED *f,
                               hbool_t H5_ATTR_UNUSED disable_shared,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__btreek_debug(H5F_t H5_ATTR_UNUSED *f, const void *_mesg,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Chunked storage internal B-tree 'K' value:",
          mesg->btree_k[H5B_CHUNK_ID]);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Symbol table node internal B-tree 'K' value:",
          mesg->btree_k[H5B_SNODE_ID]);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
