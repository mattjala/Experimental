 ...
 *
 * Return:      Success:        Non-negative, with *pp pointing to the first
 *byte after the last symbol.
 *
 *              Failure:        Negative
 *
 *-------------------------------------------------------------------------
 */
herr_t H5G__ent_decode_vec(const H5F_t *f, const uint8_t **pp,
  ...
  for (u = 0; u < n; u++) {
    if (*pp > p_end)
      HGOTO_ERROR(H5E_SYM, H5E_CANTDECODE, FAIL,
                  "ran off the end of the image buffer");
  ...
  *pp += 4; /*reserved*/
  ...
  default:
    HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                "unknown symbol table entry cache type");
  } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__ent_encode_vec(const H5F_t *f, uint8_t **pp, const H5G_entry_t *ent,
                           unsigned n) {
  unsigned u;                 /* Local index variable */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Return:      Success:        Non-negative, with *pp pointing to the first
 *byte after the symbol table entry.
 *
 *              Failure:        Negative
 *
 *-------------------------------------------------------------------------
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__ent_convert(H5F_t *f, H5HL_t *heap, const char *name,
    ...
    if (obj_type == H5O_TYPE_GROUP) {
      ...
      if (ent->type != H5G_NOTHING_CACHED)
        ...
        if (H5O_loc_reset(&targ_oloc) < 0)
          HGOTO_ERROR(H5E_SYM, H5E_CANTRESET, FAIL,
                      "unable to initialize target location");
        targ_oloc.file = f;
      ...
      if (NULL == (oh = H5O_protect(&targ_oloc, H5AC__READ_ONLY_FLAG, FALSE)))
        HGOTO_ERROR(H5E_SYM, H5E_CANTPROTECT, FAIL,
      ...
      if (stab_exists) {
        ...
        if (NULL == H5O_msg_read_oh(f, oh, H5O_STAB_ID, &stab)) {
          if (H5O_unprotect(&targ_oloc, oh, H5AC__NO_FLAGS_SET) < 0)
            HERROR(H5E_SYM, H5E_CANTUNPROTECT,
      ...
      if (H5O_unprotect(&targ_oloc, oh, H5AC__NO_FLAGS_SET) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTUNPROTECT, FAIL,
                    "unable to release object header");
    } /* end else */
    else
      ent->type = H5G_NOTHING_CACHED;
  ...
  case H5L_TYPE_SOFT: {
    ...
    if (H5HL_insert(f, heap, HDstrlen(lnk->u.soft.name) + 1, lnk->u.soft.name,
                    &lnk_offset) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__ent_debug(const H5G_entry_t *ent, FILE *stream, int indent,
  ...
  case H5G_CACHED_STAB:
    ...
    fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", nested_indent, "",
            nested_fwidth, "B-tree address:", ent->cache.stab.btree_addr);

    fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", nested_indent, "",
            nested_fwidth, "Heap address:", ent->cache.stab.heap_addr);
    break;
