...
htri_t H5O__is_attr_empty_test(hid_t oid) {
  ...
  if (oh->version > H5O_VERSION_1) {
    if (ainfo_exists) {
      ...
      if (H5_addr_defined(ainfo.fheap_addr)) {
        ...
        if (NULL ==
            (bt2_name = H5B2_open(loc->file, ainfo.name_bt2_addr, NULL)))
  ...
  if (bt2_name && H5B2_close(bt2_name) < 0)
    HDONE_ERROR(H5E_OHDR, H5E_CANTCLOSEOBJ, FAIL,
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5O__attr_dense_info_test(oid, name_count, corder_count)
        ...
        hsize_t *corder_count;  OUT: Number of attributes in creation order
index RETURNS Non-negative on success, negative on failure DESCRIPTION
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5O__attr_dense_info_test(hid_t oid, hsize_t *name_count,
  ...
  /* Get the object header */
  if (NULL == (oh = H5O_protect(loc, H5AC__READ_ONLY_FLAG, FALSE)))
    HGOTO_ERROR_TAG(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                    "unable to load object header");
  ...
  if (oh->version > H5O_VERSION_1) {
    /* Check for (& retrieve if available) attribute info */
    if (H5A__get_ainfo(loc->file, oh, &ainfo) < 0)
      HGOTO_ERROR_TAG(H5E_ATTR, H5E_CANTGET, FAIL,
                      "can't check for attribute info message");
  } /* end if */
  ...
  if (H5_addr_defined(ainfo.corder_bt2_addr)) {
    ...
    if (NULL ==
        (bt2_corder = H5B2_open(loc->file, ainfo.corder_bt2_addr, NULL)))
      HGOTO_ERROR_TAG(H5E_OHDR, H5E_CANTOPENOBJ, FAIL,
...
herr_t H5O__check_msg_marked_test(hid_t oid, hbool_t flag_val) {
  ...
  if (idx == oh->nmesgs)
    HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL,
                "'unknown' message type not found");
...
herr_t H5O__expunge_chunks_test(const H5O_loc_t *loc) {
  ...
  if (H5O_unprotect(loc, oh, H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, FAIL,
                "unable to unprotect object header");
  ...
  for (u = nchunks - 1; u < nchunks; u--)
    if (H5AC_expunge_entry(loc->file, (u == 0 ? H5AC_OHDR : H5AC_OHDR_CHK),
                           chk_addr[u], H5AC__NO_FLAGS_SET) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTEXPUNGE, FAIL,
                  "unable to expunge object header chunk");
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5O__msg_get_chunkno_test(hid_t oid, unsigned msg_type,
                                 unsigned *chunk_num) {
  H5O_t *oh = NULL;               /* Object header */
  H5O_loc_t *loc;                 /* Pointer to object's location */
  H5O_mesg_t *idx_msg;            /* Pointer to message */
