...
static herr_t H5FD__read_selection_translate(
    uint32_t skip_vector_cb, H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id,
    uint32_t count, H5S_t **mem_spaces, H5S_t **file_spaces, haddr_t offsets[],
    size_t element_sizes[], void *bufs[] /* out */);
static herr_t H5FD__write_selection_translate(
    uint32_t skip_vector_cb, H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id,
    uint32_t count, H5S_t **mem_spaces, H5S_t **file_spaces, haddr_t offsets[],
...
herr_t H5FD_locate_signature(H5FD_t *file, haddr_t *sig_addr) {
  ...
  for (n = 8; n < maxpow; n++) {
    ...
    if (H5FD_read(file, H5FD_MEM_SUPER, addr, (size_t)H5F_SIGNATURE_LEN, buf) <
        0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_read(H5FD_t *file, H5FD_mem_t type, haddr_t addr, size_t size,
                 void *buf /*out*/) {
  ...
  /* Dispatch to driver */
  if ((file->cls->read)(file, type, dxpl_id, addr + file->base_addr, size,
                        buf) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_write(H5FD_t *file, H5FD_mem_t type, haddr_t addr, size_t size,
                  const void *buf) {
  ...
  /* Dispatch to driver */
  if ((file->cls->write)(file, type, dxpl_id, addr + file->base_addr, size,
                         buf) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_read_vector(H5FD_t *file, uint32_t count, H5FD_mem_t types[],
    ...
    for (i = 0; i < count; i++) {
      ...
      if ((addrs[i] + size) > eoa)
        ...
        HGOTO_ERROR(
            ...
            (int)i, (unsigned long long)(addrs[i]), (int)i,
            (unsigned long long)size, (unsigned long long)eoa);
    }
  }
  ...
  if (file->cls->read_vector) {
    ...
    if ((file->cls->read_vector)(file, dxpl_id, count, types, addrs, sizes,
                                 bufs) < 0)

      HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                  "driver read vector request failed");
  } else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_write_vector(H5FD_t *file, uint32_t count, H5FD_mem_t types[],
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((addrs[i] + size) > eoa)
      ...
      HGOTO_ERROR(H5E_ARGS, H5E_OVERFLOW, FAIL,
                  "addr overflow, addrs[%d] = %llu, sizes[%d] = %llu, \
                        eoa = %llu",
                  (int)i, (unsigned long long)(addrs[i]), (int)i,
                  (unsigned long long)size, (unsigned long long)eoa);
  }
  ...
  if (file->cls->write_vector) {
    ...
    if ((file->cls->write_vector)(file, dxpl_id, count, types, addrs, sizes,
                                  bufs) < 0)

      HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                  "driver write vector request failed");
  } else {
...
static herr_t H5FD__read_selection_translate(
  ...
  if (count > 0) {
    ...
    if (NULL == (file_iter = H5FL_MALLOC(H5S_sel_iter_t)))
      HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL,
                  "couldn't allocate file selection iterator");
    if (NULL == (mem_iter = H5FL_MALLOC(H5S_sel_iter_t)))
      HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL,
                  "couldn't allocate memory selection iterator");
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    if (H5S_select_iter_init(file_iter, file_spaces[i], element_size, 0) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                  "can't initialize sequence list for file space");
    file_iter_init = TRUE;
    if (H5S_select_iter_init(mem_iter, mem_spaces[i], element_size, 0) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                  "can't initialize sequence list for memory space");
    mem_iter_init = TRUE;
    ...
    while (file_seq_i < file_nseq || nelmts > 0) {
      ...
      if (file_seq_i == H5FD_SEQ_LIST_LEN) {
        if (H5S_SELECT_ITER_GET_SEQ_LIST(file_iter, H5FD_SEQ_LIST_LEN, SIZE_MAX,
                                         &file_nseq, &seq_nelem, file_off,
                                         file_len) < 0)
      ...
      if (mem_seq_i == H5FD_SEQ_LIST_LEN) {
        if (H5S_SELECT_ITER_GET_SEQ_LIST(mem_iter, H5FD_SEQ_LIST_LEN, SIZE_MAX,
                                         &mem_nseq, &seq_nelem, mem_off,
                                         mem_len) < 0)
          HGOTO_ERROR(H5E_INTERNAL, H5E_UNSUPPORTED, FAIL,
                      "sequence length generation failed");
        assert(mem_nseq > 0);
      ...
      if (use_vector) {
        ...
        if (vec_arr_nused == vec_arr_nalloc) {
          ...
          if (addrs == addrs_local) {
            ...
            /* Reallocate arrays */
            if (NULL == (tmp_ptr = H5MM_realloc(addrs, vec_arr_nalloc *
                                                           sizeof(*addrs) * 2)))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                          "memory reallocation failed for address list");
            addrs = tmp_ptr;
            if (NULL == (tmp_ptr = H5MM_realloc(sizes, vec_arr_nalloc *
                                                           sizeof(*sizes) * 2)))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                          "memory reallocation failed for size list");
            sizes = tmp_ptr;
            if (NULL == (tmp_ptr = H5MM_realloc(
                             vec_bufs, vec_arr_nalloc * sizeof(*vec_bufs) * 2)))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
        ...
        if ((file->cls->read)(
                file, type, dxpl_id, offsets[i] + file_off[file_seq_i], io_len,
    ...
    if (H5S_SELECT_ITER_RELEASE(file_iter) < 0)
      HGOTO_ERROR(H5E_INTERNAL, H5E_CANTFREE, FAIL,
                  "can't release file selection iterator");
    file_iter_init = FALSE;
    if (H5S_SELECT_ITER_RELEASE(mem_iter) < 0)
      HGOTO_ERROR(H5E_INTERNAL, H5E_CANTFREE, FAIL,
                  "can't release memory selection iterator");
    mem_iter_init = FALSE;
  }
  ...
  if (use_vector) {
    ...
    if ((file->cls->read_vector)(file, dxpl_id, (uint32_t)vec_arr_nused, types,
                                 addrs, sizes, vec_bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                  "driver read vector request failed");
  } else {
    uint32_t no_selection_io_cause;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_read_selection(H5FD_t *file, H5FD_mem_t type, uint32_t count,
  ...
  if (file->cls->read_selection) {
    ...
    if (count > sizeof(mem_space_ids_local) / sizeof(mem_space_ids_local[0])) {
      if (NULL == (mem_space_ids = H5MM_malloc(count * sizeof(hid_t))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "memory allocation failed for dataspace list");
    ...
    for (; num_spaces < count; num_spaces++) {
      if ((mem_space_ids[num_spaces] =
               H5I_register(H5I_DATASPACE, mem_spaces[num_spaces], TRUE)) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL,
                    "unable to register dataspace ID");

      if ((file_space_ids[num_spaces] = H5I_register(
               H5I_DATASPACE, file_spaces[num_spaces], TRUE)) < 0) {
    ...
    if ((file->cls->read_selection)(file, type, dxpl_id, count, mem_space_ids,
                                    file_space_ids, offsets, element_sizes,
                                    bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                  "driver read selection request failed");
  } else
    /* Otherwise, implement the selection read as a sequence of regular
     * or vector read calls.
     */
    if (H5FD__read_selection_translate(SKIP_NO_CB, file, type, dxpl_id, count,
                                       mem_spaces, file_spaces, offsets,
                                       element_sizes, bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                  "translation to vector or scalar read failed");
 ...
 *
 *              --If the underlying VFD supports selection reads and
 *!skip_selection_cb, pass the call through directly.
 *
 *              --If it doesn't, convert the selection reads into a sequence of
 *vector or scalar reads depending on skip_vector_cb.
 *
 * Return:      Success:    SUCCEED
 *                          All reads have completed successfully, and
 *                          the results havce been into the supplied
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_read_selection_id(uint32_t skip_cb, H5FD_t *file, H5FD_mem_t type,
  ...
  if (!skip_selection_cb && file->cls->read_selection) {
    ...
    for (i = 0; i < count; i++) {
      if (NULL == (mem_spaces[i] = (H5S_t *)H5I_object_verify(mem_space_ids[i],
                                                              H5I_DATASPACE)))
        HGOTO_ERROR(H5E_VFL, H5E_BADTYPE, H5I_INVALID_HID,
                    "can't retrieve memory dataspace from ID");
      if (NULL == (file_spaces[i] = (H5S_t *)H5I_object_verify(
                       file_space_ids[i], H5I_DATASPACE)))
        HGOTO_ERROR(H5E_VFL, H5E_BADTYPE, H5I_INVALID_HID,
    ...
    if (H5FD__read_selection_translate(skip_vector_cb, file, type, dxpl_id,
                                       count, mem_spaces, file_spaces, offsets,
                                       element_sizes, bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                  "translation to vector or scalar read failed");
  }
...
static herr_t H5FD__write_selection_translate(
  ...
  if (use_vector) {
    H5_CHECK_OVERFLOW(vec_arr_nused, size_t, uint32_t);
    if ((file->cls->write_vector)(file, dxpl_id, (uint32_t)vec_arr_nused, types,
                                  addrs, sizes, vec_bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                  "driver write vector request failed");
  } else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_write_selection(H5FD_t *file, H5FD_mem_t type, uint32_t count,
                            H5S_t **mem_spaces, H5S_t **file_spaces,
                            haddr_t offsets[], size_t element_sizes[],
                            const void *bufs[]) {
  hbool_t offsets_cooked = FALSE;
  ...
  if (file->cls->write_selection) {
    ...
    if ((file->cls->write_selection)(file, type, dxpl_id, count, mem_space_ids,
                                     file_space_ids, offsets, element_sizes,
                                     bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                  "driver write selection request failed");
  } else
    /* Otherwise, implement the selection write as a sequence of regular
     * or vector write calls.
     */

    if (H5FD__write_selection_translate(SKIP_NO_CB, file, type, dxpl_id, count,
                                        mem_spaces, file_spaces, offsets,
                                        element_sizes, bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                  "translation to vector or scalar write failed");
 ...
 *              skip_selection_cb and skip_vector_cb:
 *
 *              --If the underlying VFD supports selection writes and
 *!skip_selection_cb, pass the call through directly.
 *
 *              --If it doesn't, convert the selection writes into a sequence of
 *vector or scalar reads depending on skip_vector_cb.
 *
 * Return:      Success:    SUCCEED
 *                          All writes have completed successfully.
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_write_selection_id(uint32_t skip_cb, H5FD_t *file, H5FD_mem_t type,
                               uint32_t count, hid_t mem_space_ids[],
                               hid_t file_space_ids[], haddr_t offsets[],
                               size_t element_sizes[], const void *bufs[]) {
  hbool_t offsets_cooked = FALSE;
  ...
  if (!skip_selection_cb && file->cls->write_selection) {
    ...
    if (H5FD__write_selection_translate(skip_vector_cb, file, type, dxpl_id,
                                        count, mem_spaces, file_spaces, offsets,
                                        element_sizes, bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                  "translation to vector or scalar write failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_read_vector_from_selection(H5FD_t *file, H5FD_mem_t type,
                                       uint32_t count, hid_t mem_space_ids[],
  ...
  if (H5FD_read_selection_id(SKIP_SELECTION_CB, file, type, count,
                             mem_space_ids, file_space_ids, offsets,
                             element_sizes, bufs) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_write_vector_from_selection(H5FD_t *file, H5FD_mem_t type,
                                        uint32_t count, hid_t mem_space_ids[],
                                        hid_t file_space_ids[],
                                        haddr_t offsets[],
  ...
  /* Call private function */
  /* (Note compensating for base address addition in internal routine) */
  if (H5FD_write_selection_id(SKIP_SELECTION_CB, file, type, count,
                              mem_space_ids, file_space_ids, offsets,
                              element_sizes, bufs) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_read_from_selection(H5FD_t *file, H5FD_mem_t type, uint32_t count,
  ...
  /* Call private function */
  /* (Note compensating for base address addition in internal routine) */
  if (H5FD_read_selection_id(SKIP_SELECTION_CB | SKIP_VECTOR_CB, file, type,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_write_from_selection(H5FD_t *file, H5FD_mem_t type, uint32_t count,
                                 hid_t mem_space_ids[], hid_t file_space_ids[],
                                 haddr_t offsets[], size_t element_sizes[],
                                 const void *bufs[]) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Call private function */
  /* (Note compensating for base address addition in internal routine) */
  if (H5FD_write_selection_id(SKIP_SELECTION_CB | SKIP_VECTOR_CB, file, type,
                              count, mem_space_ids, file_space_ids, offsets,
                              element_sizes, bufs) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                "file selection write request failed");
...
haddr_t H5FD_get_eof(const H5FD_t *file, H5FD_mem_t type) {
  ...
  if (file->cls->get_eof) {
    if (HADDR_UNDEF == (ret_value = (file->cls->get_eof)(file, type)))
      HGOTO_ERROR(H5E_VFL, H5E_CANTGET, HADDR_UNDEF,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__sort_io_req_real(size_t count, haddr_t *addrs,
                                     hbool_t *was_sorted,
                                     struct H5FD_srt_tmp_t **srt_tmp) {
  size_t i;
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_sort_vector_io_req(hbool_t *vector_was_sorted, uint32_t _count,
                               H5FD_mem_t types[], haddr_t addrs[],
                               size_t sizes[], H5_flexible_const_ptr_t bufs[],
                               H5FD_mem_t **s_types_ptr, haddr_t **s_addrs_ptr,
                               size_t **s_sizes_ptr,
  ...
  if (H5FD__sort_io_req_real(count, addrs, vector_was_sorted, &srt_tmp) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "sorting error in selection offsets");
  ...
  if (*vector_was_sorted) {
    ...
    *s_bufs_ptr = bufs;
    ...
    if ((NULL ==
        ...
        (NULL == (*s_bufs_ptr = (H5_flexible_const_ptr_t *)malloc(
                      count * sizeof(H5_flexible_const_ptr_t))))) {

      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                  "can't alloc sorted vector(s)");
    }
     ...
     */
    for (i = 1; i < count &&
                ((fixed_size_index == count) || (fixed_type_index == count));
         i++) {
 ...
 *file_space_ids, offsets, element_sizes and bufs arrays, storing the base
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_sort_selection_io_req(
    hbool_t *selection_was_sorted, size_t count, hid_t mem_space_ids[],
  ...
  assert((count == 0) ||
         ((s_mem_space_ids_ptr) && (NULL == *s_mem_space_ids_ptr)));
  assert((count == 0) ||
         ((s_file_space_ids_ptr) && (NULL == *s_file_space_ids_ptr)));
  assert((count == 0) || ((s_offsets_ptr) && (NULL == *s_offsets_ptr)));
  assert((count == 0) ||
         ((s_element_sizes_ptr) && (NULL == *s_element_sizes_ptr)));
  assert((count == 0) || ((s_bufs_ptr) && (NULL == *s_bufs_ptr)));
   ...
   */
  if (H5FD__sort_io_req_real(count, offsets, selection_was_sorted, &srt_tmp) <
      0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "sorting error in selection offsets");
  ...
  if (*selection_was_sorted) {
    ...
    *s_bufs_ptr = bufs;
    ...
    if ((NULL ==
      ...
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                  "can't alloc sorted selection(s)");
    }
    ...
    for (i = 0; i < count; i++) {
      ...
      (*s_element_sizes_ptr)[i] =
          element_sizes[MIN(j, fixed_element_sizes_index)];
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_check_plugin_load(const H5FD_class_t *cls, const H5PL_key_t *key,
                              hbool_t *success) {
  herr_t ret_value = SUCCEED; /* Return value */
...
static int H5FD__get_driver_cb(void *obj, hid_t id, void *_op_data) {
  H5FD_get_driver_ud_t *op_data =
      (H5FD_get_driver_ud_t *)_op_data; /* User data for callback */
...
hid_t H5FD_register_driver_by_name(const char *name, hbool_t app_ref) {
  ...
  if ((driver_is_registered =
           H5FD_is_driver_registered_by_name(name, &driver_id)) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_BADITER, H5I_INVALID_HID,
                "can't check if driver is already registered");
  ...
  if (driver_is_registered) {
    ...
    if (H5I_inc_ref(driver_id, app_ref) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTINC, H5I_INVALID_HID,
    ...
    if ((driver_id = H5FD_register(cls, sizeof(*cls), app_ref)) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, H5I_INVALID_HID,
                  "unable to register VFD ID");
  } /* end else */
...
hid_t H5FD_register_driver_by_value(H5FD_class_value_t value, hbool_t app_ref) {
  ...
  /* Check if driver is already registered */
  if ((driver_is_registered =
           H5FD_is_driver_registered_by_value(value, &driver_id)) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_BADITER, H5I_INVALID_HID,
                "can't check if driver is already registered");
...
hid_t H5FD_get_driver_id_by_name(const char *name, hbool_t is_api) {
  ...
  if (H5I_iterate(H5I_VFL, H5FD__get_driver_cb, &op_data, FALSE) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_BADITER, H5I_INVALID_HID,
                "can't iterate over VFL drivers");
