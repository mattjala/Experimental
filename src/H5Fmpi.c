...
MPI_Comm H5F_mpi_get_comm(const H5F_t *f) {
  ...
  if ((ret_value = H5FD_mpi_get_comm(f->shared->lf)) == MPI_COMM_NULL)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, MPI_COMM_NULL,
                "driver get_comm request failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__set_mpi_atomicity(H5F_t *file, hbool_t flag) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__get_mpi_atomicity(const H5F_t *file, hbool_t *flag) {
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Fget_mpi_atomicity(hid_t file_id, hbool_t *flag /*out*/) {
  H5VL_object_t *vol_obj;           /* File info */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_file_optional_args_t
      file_opt_args;          /* Arguments for optional operation */
  ...
  /* otherwise, this is from H5Fopen or H5Fcreate and has to be collective */
    ...
    if (NULL == (driver_class = H5FD_get_class(driver_prop.driver_id)))
      HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                  "can't get driver class structure");
 ...
 *-------------------------------------------------------------------------
 */
hbool_t H5F_get_coll_metadata_reads(const H5F_t *file) {
 ...
 *-------------------------------------------------------------------------
 */
void H5F_set_coll_metadata_reads(H5F_t *file,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_mpi_get_file_block_type(hbool_t commit, MPI_Datatype *new_type,
