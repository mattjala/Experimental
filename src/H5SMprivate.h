 ...
 */
#define H5SM_DEFER                                                             \
  0x01u /* Don't actually write shared message to index, heap; just update     \
           shared info */
#define H5SM_WAS_DEFERRED                                                      \
  0x02u /* Message was previously updated by a call to H5SM_try_share with     \
           H5SM_DEFER */
...
H5_DLL herr_t H5SM_get_fheap_addr(H5F_t *f, unsigned type_id,
                                  haddr_t *fheap_addr);
...
H5_DLL herr_t H5SM_table_debug(H5F_t *f, haddr_t table_addr, FILE *stream,
                               int indent, int fwidth, unsigned table_vers,
