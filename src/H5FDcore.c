...
/* Prototypes */
static herr_t H5FD__core_add_dirty_region(H5FD_core_t *file, haddr_t start,
                                          haddr_t end);
static herr_t H5FD__core_destroy_dirty_list(H5FD_core_t *file);
static herr_t H5FD__core_write_to_bstore(H5FD_core_t *file, haddr_t addr,
                                         size_t size);
...
static herr_t H5FD__core_get_handle(H5FD_t *_file, hid_t fapl,
                                    void **file_handle);
static herr_t H5FD__core_read(H5FD_t *_file, H5FD_mem_t type, hid_t fapl_id,
                              haddr_t addr, size_t size, void *buf);
static herr_t H5FD__core_write(H5FD_t *_file, H5FD_mem_t type, hid_t fapl_id,
                               haddr_t addr, size_t size, const void *buf);
static herr_t H5FD__core_flush(H5FD_t *_file, hid_t dxpl_id, hbool_t closing);
static herr_t H5FD__core_truncate(H5FD_t *_file, hid_t dxpl_id,
                                  hbool_t closing);
...
static const H5FD_core_fapl_t H5FD_core_default_config_g = {
    (size_t)H5_MB, TRUE, H5FD_CORE_WRITE_TRACKING_FLAG,
    H5FD_CORE_WRITE_TRACKING_PAGE_SIZE};
static const H5FD_core_fapl_t H5FD_core_default_paged_config_g = {
    (size_t)H5_MB, TRUE, TRUE, (size_t)4096};
...
static herr_t H5FD__core_add_dirty_region(H5FD_core_t *file, haddr_t start,
  ...
  while (a_item && a_item->start > start) {
    ...
    /* Delete this node */
    a_item =
        (H5FD_core_region_t *)H5SL_remove(file->dirty_list, &a_item->start);
  ...
  if (create_new_node) {
    if (NULL ==
        (item = (H5FD_core_region_t *)H5SL_search(file->dirty_list, &start))) {
      ...
      if (H5SL_insert(file->dirty_list, item, &item->start) < 0)
        HGOTO_ERROR(H5E_SLIST, H5E_CANTINSERT, FAIL,
                    "can't insert new dirty region: (%llu, %llu)\n",
...
static herr_t H5FD__core_destroy_dirty_list(H5FD_core_t *file) {
  ...
  if (file->dirty_list) {
    ...
    while (NULL !=
           (region = (H5FD_core_region_t *)H5SL_remove_first(file->dirty_list)))
      region = H5FL_FREE(H5FD_core_region_t, region);

    if (H5SL_close(file->dirty_list) < 0)
      HGOTO_ERROR(H5E_SLIST, H5E_CLOSEERROR, FAIL,
                  "can't close core vfd dirty list");
  ...
  while (size > 0) {
    ...
    if (-1 == bytes_wrote) { /* error */
      ...
      HGOTO_ERROR(
          ...
          "write to backing store failed: time = %s, filename = '%s', file "
          "descriptor = %d, "
          "errno = %d, error message = '%s', ptr = %p, total write size = "
          "%llu, bytes this "
          "sub-write = %llu, bytes actually written = %llu, offset = %llu",
          HDctime(&mytime), file->name, file->fd, myerrno, HDstrerror(myerrno),
          (void *)ptr, (unsigned long long)size, (unsigned long long)bytes_in,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_core_write_tracking(hid_t plist_id, hbool_t is_enabled,
                                  size_t page_size) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_core_write_tracking(hid_t plist_id, hbool_t *is_enabled /*out*/,
...
static H5FD_t *H5FD__core_open(const char *name, unsigned flags, hid_t fapl_id,
  ...
  if (H5P_peek(plist, H5F_ACS_FILE_IMAGE_INFO_NAME, &file_image_info) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                "can't get initial file image info");

  /* If the file image exists and this is an open, make sure the file doesn't
   * exist */
  ...
  if (!(H5F_ACC_CREAT & flags)) {
     ...
     */
    if (size) {
      /* Allocate memory for the file's data, using the file image callback if
       * available. */
      if (file->fi_callbacks.image_malloc) {
        if (NULL ==
            (file->mem = (unsigned char *)file->fi_callbacks.image_malloc(
      ...
      /* If there is an initial file image, copy it, using the callback if
       * possible */
      if (file_image_info.buffer && file_image_info.size > 0) {
        if (file->fi_callbacks.image_memcpy) {
          if (file->mem != file->fi_callbacks.image_memcpy(
                               file->mem, file_image_info.buffer, size,
                               H5FD_FILE_IMAGE_OP_FILE_OPEN,
                               file->fi_callbacks.udata))
            HGOTO_ERROR(H5E_FILE, H5E_CANTCOPY, NULL,
        ...
        while (size > 0) {
          ...
          if (-1 == bytes_read) { /* error */
            ...
            HGOTO_ERROR(
                ...
                "file read failed: time = %s, filename = '%s', file descriptor "
                "= %d, errno = %d, "
                "error message = '%s', file->mem = %p, total read size = %llu, "
                "bytes this "
                "sub-read = %llu, bytes actually read = %llu, offset = %llu",
                HDctime(&mytime), file->name, file->fd, myerrno,
                HDstrerror(myerrno), (void *)file->mem,
  ...
  if (fa->backing_store) {
    ...
    use_write_tracking =
        (TRUE == fa->write_tracking) /* user asked for write tracking */
        && !(o_flags &
             O_RDONLY) /* file is open for writing (i.e. not read-only) */
    ...
    if (use_write_tracking)
      if (NULL == (file->dirty_list = H5SL_create(H5SL_TYPE_HADDR, NULL)))
        HGOTO_ERROR(H5E_SLIST, H5E_CANTCREATE, NULL,
                    "can't create core vfd dirty region list");
  } /* end if */
...
static herr_t H5FD__core_close(H5FD_t *_file) {
  ...
  if (file->mem) {
    ...
    if (file->fi_callbacks.image_free) {
      if (file->fi_callbacks.image_free(file->mem,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__core_query(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__core_get_eoa(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__core_set_eoa(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
                                 haddr_t addr) {
  H5FD_core_t *file = (H5FD_core_t *)_file;
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__core_get_eof(const H5FD_t *_file,
                                  H5FD_mem_t H5_ATTR_UNUSED type) {
  const H5FD_core_t *file = (const H5FD_core_t *)_file;
...
static herr_t H5FD__core_get_handle(H5FD_t *_file, hid_t fapl,
  ...
  if (H5P_FILE_ACCESS_DEFAULT != fapl && H5P_DEFAULT != fapl) {
    ...
    if (NULL == (plist = (H5P_genplist_t *)H5I_object(fapl)))
      HGOTO_ERROR(H5E_VFL, H5E_BADTYPE, FAIL,
    ...
    if (H5P_exist_plist(plist, H5F_ACS_WANT_POSIX_FD_NAME) > 0) {
      hbool_t want_posix_fd; /* Setting for retrieving file descriptor from core
                                VFD */

      /* Get property */
      if (H5P_get(plist, H5F_ACS_WANT_POSIX_FD_NAME, &want_posix_fd) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                    "can't get property of retrieving file descriptor");

      /* If property is set, pass back the file descriptor instead of the memory
       * address */
      if (want_posix_fd)
        *file_handle = &(file->fd);
      else
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__core_read(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__core_write(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
                               hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr,
  ...
  if (addr + size > file->eof) {
    ...
    H5_CHECKED_ASSIGN(new_eof, size_t,
                      file->increment * ((addr + size) / file->increment),
                      hsize_t);
    ...
    if (file->fi_callbacks.image_realloc) {
      if (NULL == (x = (unsigned char *)file->fi_callbacks.image_realloc(
                       file->mem, new_eof, H5FD_FILE_IMAGE_OP_FILE_RESIZE,
                       file->fi_callbacks.udata)))
        HGOTO_ERROR(
      ...
      if (NULL == (x = (unsigned char *)H5MM_realloc(file->mem, new_eof)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "unable to allocate memory block of %llu bytes",
                    (unsigned long long)new_eof);
    } /* end else */
  ...
  if (file->dirty_list) {
    ...
    if (H5FD__core_add_dirty_region(file, start, end) != SUCCEED)
      HGOTO_ERROR(H5E_VFL, H5E_CANTINSERT, FAIL,
                  "unable to add core VFD dirty region during write call - "
                  "addresses: start=%llu end=%llu",
                  (unsigned long long)start, (unsigned long long)end);
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__core_flush(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
  ...
  if (file->dirty && file->fd >= 0 && file->backing_store) {
    ...
    if (file->dirty_list) {
      ...
      while (NULL != (item = (H5FD_core_region_t *)H5SL_remove_first(
                          file->dirty_list))) {
        ...
        if (item->start < file->eof) {
          ...
          if (H5FD__core_write_to_bstore(file, item->start, size) != SUCCEED)
            HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                        "unable to write to backing store");
        } /* end if */
    ...
    /* Otherwise, write the entire file out at once */
      ...
      if (H5FD__core_write_to_bstore(file, (haddr_t)0, (size_t)file->eof) !=
          SUCCEED)
        HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                    "unable to write to backing store");
    } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__core_truncate(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
                                  hbool_t closing) {
  H5FD_core_t *file = (H5FD_core_t *)_file;
  ...
  if (!closing || file->backing_store) {
    if (closing) /* set eof to eoa */
      ...
      /* Determine new size of memory buffer */
      H5_CHECKED_ASSIGN(new_eof, size_t,
                        file->increment * (file->eoa / file->increment),
                        hsize_t);
    ...
    if (!H5_addr_eq(file->eof, (haddr_t)new_eof)) {
      ...
      if (closing && (file->fd >= 0) && file->backing_store) {
        ...
        /* Extend the file to make sure it's large enough.
         *
         * Since INVALID_SET_FILE_POINTER can technically be a valid return
         * value from SetFilePointer(), we also need to check GetLastError().
         */
        dwPtrLow =
...
static herr_t H5FD__core_delete(const char *filename, hid_t fapl_id) {
  ...
  if (fa->backing_store)
    if (HDremove(filename) < 0)
      HSYS_GOTO_ERROR(H5E_VFL, H5E_CANTDELETEFILE, FAIL,
