 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF_id_print(H5HF_t *fh, const void *_id, FILE *stream, int indent,
                     int fwidth) {
  ...
  if ((id_flags & H5HF_ID_TYPE_MASK) == H5HF_ID_TYPE_MAN) {
    ...
    HGOTO_ERROR(H5E_HEAP, H5E_UNSUPPORTED, FAIL,
                "heap ID type not supported yet");
  } /* end else */
...
static herr_t H5HF__dtable_debug(const H5HF_dtable_t *dtable, FILE *stream,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Doubling table width:", dtable->cparam.width);
 ...
 *-------------------------------------------------------------------------
 */
void H5HF_hdr_print(const H5HF_hdr_t *hdr, hbool_t dump_internal, FILE *stream,
                    int indent, int fwidth) {
  ...
  if (hdr->filter_len > 0) {
    ...
    H5O_debug_id(H5O_PLINE_ID, hdr->f, &(hdr->pline), stream, indent + 3,
                 MAX(0, fwidth - 3));
  } /* end if */
  ...
  if (dump_internal) {
    ...
    if (hdr->root_iblock)
      H5HF_iblock_print(hdr->root_iblock, dump_internal, stream, indent + 3,
                        fwidth);
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF_hdr_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                      int fwidth) {
  ...
  if (hdr &&
      H5AC_unprotect(f, H5AC_FHEAP_HDR, addr, hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_PROTECT, FAIL,
...
static herr_t H5HF_dblock_debug_cb(H5FS_section_info_t *_sect, void *_udata) {
  H5HF_free_section_t *sect =
      (H5HF_free_section_t *)_sect; /* Section to dump info */
  H5HF_debug_iter_ud1_t *udata =
      (H5HF_debug_iter_ud1_t *)_udata; /* User data for callbacks */
  haddr_t sect_start, sect_end; /* Section's beginning and ending offsets */
  haddr_t dblock_start,
      dblock_end; /* Direct block's beginning and ending offsets */
  ...
  if ((sect_start <= dblock_end &&
       sect_end >= dblock_start) || /* section within or overlaps w/beginning of
                                       direct block*/
      (sect_start <= dblock_end &&
       sect_end >= dblock_end)) { /* section overlaps w/end of direct block */
    ...
    HDsnprintf(temp_str, sizeof(temp_str),
               "Section #%u:", (unsigned)udata->sect_count);
    fprintf(udata->stream, "%*s%-*s %8zu, %8zu\n", udata->indent + 3, "",
            MAX(0, udata->fwidth - 9), temp_str, start, len);
    udata->sect_count++;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF_dblock_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                         int fwidth, haddr_t hdr_addr, size_t block_size) {
  H5HF_hdr_t *hdr = NULL;       /* Fractal heap header info */
   ...
   */
  if (NULL == (dblock = H5HF__man_dblock_protect(hdr, addr, block_size, NULL, 0,
  ...
  fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth,
          "Size of block header:", blk_prefix_size);
  ...
  if (H5HF__space_start(hdr, FALSE) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't initialize heap free space");

  /* If there is a free space manager for the heap, check for sections that
   * overlap this block */
  if (hdr->fspace) {
    ...
    if (H5FS_sect_iterate(f, hdr->fspace, H5HF_dblock_debug_cb, &udata) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_BADITER, FAIL,
                  "can't iterate over heap's free space");
  ...
  if (dblock && H5AC_unprotect(f, H5AC_FHEAP_DBLOCK, addr, dblock,
                               H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_PROTECT, FAIL,
                "unable to release fractal heap direct block");
  if (hdr &&
      H5AC_unprotect(f, H5AC_FHEAP_HDR, hdr_addr, hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_PROTECT, FAIL,
                "unable to release fractal heap header");
  H5MM_xfree(marker);
 ...
 *-------------------------------------------------------------------------
 */
void H5HF_iblock_print(const H5HF_indirect_t *iblock, hbool_t dump_internal,
  ...
  fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth,
          "Size of indirect block:", iblock->size);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Current # of rows:", iblock->nrows);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Max. # of rows:", iblock->max_rows);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
  ...
  if (hdr->filter_len > 0)
    fprintf(stream,
            "%*sDirect Block Entries: (address/compressed size/filter mask)\n",
            indent, "");
  else
    fprintf(stream, "%*sDirect Block Entries: (address)\n", indent, "");
  for (u = 0; u < hdr->man_dtable.max_direct_rows && u < iblock->nrows; u++) {
    HDsnprintf(temp_str, sizeof(temp_str), "Row #%u: (block size: %lu)",
               (unsigned)u, (unsigned long)hdr->man_dtable.row_block_size[u]);
    ...
    for (v = 0; v < hdr->man_dtable.cparam.width; v++) {
      ...
      if (hdr->filter_len > 0)
        fprintf(stream, "%*s%-*s %9" PRIuHADDR "/%6zu/%x\n", indent + 6, "",
                MAX(0, fwidth - 6), temp_str, iblock->ents[off].addr,
  ...
  if (iblock->nrows > hdr->man_dtable.max_direct_rows) {
    ...
    for (u = hdr->man_dtable.max_direct_rows; u < iblock->nrows; u++) {
      num_indirect_rows =
          (H5VM_log2_gen(hdr->man_dtable.row_block_size[u]) - first_row_bits) +
          1;
      HDsnprintf(temp_str, sizeof(temp_str), "Row #%u: (# of rows: %u)",
                 (unsigned)u, num_indirect_rows);
      fprintf(stream, "%*s%-*s\n", indent + 3, "", MAX(0, fwidth - 3),
              temp_str);
      for (v = 0; v < hdr->man_dtable.cparam.width; v++) {
        size_t off = (u * hdr->man_dtable.cparam.width) + v;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF_iblock_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                         int fwidth, haddr_t hdr_addr, unsigned nrows) {
  H5HF_hdr_t *hdr = NULL;         /* Fractal heap header info */
   ...
   */
  if (NULL ==
      (iblock = H5HF__man_iblock_protect(hdr, addr, nrows, NULL, 0, FALSE,
 ...
 *
 * Purpose:	Prints debugging info about a free space section for a fractal
 *heap.
 *
 * Return:	Non-negative on success/Negative on failure
 *
...
static herr_t H5HF_sects_debug_cb(H5FS_section_info_t *_sect, void *_udata) {
  ...
  H5HF_debug_iter_ud2_t *udata =
      (H5HF_debug_iter_ud2_t *)_udata; /* User data for callbacks */
  herr_t ret_value = SUCCEED;          /* Return value */
  ...
  fprintf(udata->stream, "%*s%-*s %" PRIuHADDR "\n", udata->indent, "",
          udata->fwidth, "Section address:", sect->sect_info.addr);
  fprintf(udata->stream, "%*s%-*s %" PRIuHSIZE "\n", udata->indent, "",
          udata->fwidth, "Section size:", sect->sect_info.size);
  ...
  if (H5FS_sect_debug(udata->fspace, _sect, udata->stream, udata->indent + 3,
    ...
    HGOTO_ERROR(H5E_HEAP, H5E_BADITER, FAIL,
                "can't dump section's debugging info");
 ...
 *
 * Purpose:	Prints debugging info about free space sections for a fractal
 *heap.
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5HF_sects_debug(H5F_t *f, haddr_t fh_addr, FILE *stream, int indent,
                        int fwidth) {
  H5HF_hdr_t *hdr = NULL;     /* Fractal heap header info */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (hdr &&
      H5AC_unprotect(f, H5AC_FHEAP_HDR, fh_addr, hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_PROTECT, FAIL,
                "unable to release fractal heap header");
