...
static herr_t
H5VL__native_dataset_io_setup(size_t count, void *obj[], hid_t mem_type_id[],
                              hid_t mem_space_id[], hid_t file_space_id[],
                              hid_t dxpl_id, H5_flexible_const_ptr_t buf[],
                              H5D_dset_io_info_t *dinfo);
static herr_t H5VL__native_dataset_io_cleanup(size_t count,
                                              hid_t mem_space_id[],
                                              hid_t file_space_id[],
                                              H5D_dset_io_info_t *dinfo);
  ...
  for (i = 0; i < count; i++) {
    ...
    if (NULL == dinfo[i].dset->oloc.file)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                  "dataset is not associated with a file");
    ...
    if (H5S_ALL == file_space_id[i])
      ...
      HGOTO_ERROR(H5E_DATASET, H5E_BADTYPE, FAIL,
                  "H5S_BLOCK is not allowed for file dataspace");
      ...
      if (NULL == (plist = H5P_object_verify(dxpl_id, H5P_DATASET_XFER)))
        HGOTO_ERROR(H5E_DATASET, H5E_BADID, FAIL,
                    "bad dataset transfer property list");
      ...
      if (H5S_SELECT_COPY(dinfo[i].file_space, space, TRUE) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                    "can't copy dataset I/O selection");
    } /* end else-if */
    else {
      /* Get the dataspace pointer */
      if (NULL == (dinfo[i].file_space = (H5S_t *)H5I_object_verify(
                       file_space_id[i], H5I_DATASPACE)))
        HGOTO_ERROR(H5E_DATASET, H5E_BADTYPE, FAIL,
                    "file_space_id is not a dataspace ID");
    } /* end else */
    ...
    if (H5S_ALL == mem_space_id[i])
      ...
      if (nelmts > 0) {
        ...
        if (NULL == (dinfo[i].mem_space = H5S_create(H5S_NULL)))
          HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, FAIL,
                      "unable to create NULL memory dataspace");
      } /* end else */
    }   /* end if */
    else if (H5S_PLIST == mem_space_id[i])
      HGOTO_ERROR(H5E_DATASET, H5E_BADTYPE, FAIL,
                  "H5S_PLIST is not allowed for memory dataspace");
    else {
      /* Get the dataspace pointer */
      if (NULL == (dinfo[i].mem_space = (H5S_t *)H5I_object_verify(
                       mem_space_id[i], H5I_DATASPACE)))
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                    "mem_space_id is not a dataspace ID");
    } /* end else */
...
static herr_t H5VL__native_dataset_io_cleanup(size_t count,
  ...
  /* Iterate over datasets */
  for (i = 0; i < count; i++) {
    /* Free memory dataspace if it was created.  Use HDONE_ERROR in this
     * function so we always try to free everything we can. */
...
void *H5VL__native_dataset_create(void *obj,
                                  ...
                                  hid_t dapl_id, hid_t H5_ATTR_UNUSED dxpl_id,
  ...
  if (NULL ==
      (space = (const H5S_t *)H5I_object_verify(space_id, H5I_DATASPACE)))
  ...
  if (NULL == name) {
    ...
    if (NULL ==
        (dset = H5D__create(loc.oloc->file, type_id, space, dcpl_id, dapl_id)))
  ...
  if (NULL == name) {
    ...
    if (dset) {
      ...
      if (NULL == (oloc = H5D_oloc(dset)))
        HDONE_ERROR(H5E_DATASET, H5E_CANTGET, NULL,
                    "unable to get object location of dataset");
 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_dataset_open(void *obj, const H5VL_loc_params_t *loc_params,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_read(size_t count, void *obj[], hid_t mem_type_id[],
                                 hid_t mem_space_id[], hid_t file_space_id[],
  ...
  if (count > 1)
    if (NULL == (dinfo = (H5D_dset_io_info_t *)H5MM_malloc(
                     count * sizeof(H5D_dset_io_info_t))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate dset info array buffer");

  /* Get file & memory dataspaces */
  if (H5VL__native_dataset_io_setup(count, obj, mem_type_id, mem_space_id,
                                    file_space_id, dxpl_id,
                                    (H5_flexible_const_ptr_t *)buf, dinfo) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to set up file and memory dataspaces");
  ...
  /* Clean up */
  if (H5VL__native_dataset_io_cleanup(count, mem_space_id, file_space_id,
                                      dinfo) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTRELEASE, FAIL,
                "unable to release dataset info");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_write(size_t count, void *obj[],
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_get(void *obj, H5VL_dataset_get_args_t *args,
                                hid_t H5_ATTR_UNUSED dxpl_id,
                                void H5_ATTR_UNUSED **req) {
  ...
  case H5VL_DATASET_GET_TYPE: {
    if ((args->args.get_type.type_id = H5D__get_type(dset)) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get datatype ID of dataset");
  ...
  case H5VL_DATASET_GET_DCPL: {
    if ((args->args.get_dcpl.dcpl_id = H5D_get_create_plist(dset)) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get creation property list for dataset");
  ...
  case H5VL_DATASET_GET_DAPL: {
    if ((args->args.get_dapl.dapl_id = H5D_get_access_plist(dset)) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get access property list for dataset");
  ...
  case H5VL_DATASET_GET_STORAGE_SIZE: {
    if (H5D__get_storage_size(dset, args->args.get_storage_size.storage_size) <
        0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get size of dataset's storage");
    break;
  }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_optional(void *obj, H5VL_optional_args_t *args,
                                     hid_t dxpl_id, void H5_ATTR_UNUSED **req) {
  H5D_t *dset = (H5D_t *)obj; /* Dataset */
  H5VL_native_dataset_optional_args_t *opt_args =
      args->args;             /* Pointer to native operation's arguments */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  case H5VL_NATIVE_DATASET_GET_CHUNK_INDEX_TYPE: {
    ...
    *opt_args->get_chunk_idx_type.idx_type =
        dset->shared->layout.u.chunk.idx_type;
  ...
  case H5VL_NATIVE_DATASET_GET_CHUNK_STORAGE_SIZE: {
    H5VL_native_dataset_get_chunk_storage_size_t *gcss_args =
        &opt_args->get_chunk_storage_size;
    ...
    if (H5D__get_chunk_storage_size(dset, gcss_args->offset, gcss_args->size) <
        0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
  ...
  case H5VL_NATIVE_DATASET_GET_NUM_CHUNKS: {
    ...
    if (gnc_args->space_id == H5S_ALL)
      ...
      if (NULL == (space = (const H5S_t *)H5I_object_verify(gnc_args->space_id,
                                                            H5I_DATASPACE)))
  ...
  case H5VL_NATIVE_DATASET_GET_CHUNK_INFO_BY_IDX: {
    H5VL_native_dataset_get_chunk_info_by_idx_t *gcibi_args =
        &opt_args->get_chunk_info_by_idx;
    const H5S_t *space;
    ...
    if (gcibi_args->space_id == H5S_ALL)
      ...
      if (NULL == (space = (const H5S_t *)H5I_object_verify(
                       gcibi_args->space_id, H5I_DATASPACE)))
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a valid dataspace ID");
    ...
    /* Call private function */
    if (H5D__get_chunk_info(dset, space, gcibi_args->chk_index,
                            gcibi_args->offset, gcibi_args->filter_mask,
                            gcibi_args->addr, gcibi_args->size) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
  ...
  case H5VL_NATIVE_DATASET_GET_CHUNK_INFO_BY_COORD: {
    H5VL_native_dataset_get_chunk_info_by_coord_t *gcibc_args =
        &opt_args->get_chunk_info_by_coord;
    ...
    /* Call private function */
    if (H5D__get_chunk_info_by_coord(dset, gcibc_args->offset,
                                     gcibc_args->filter_mask, gcibc_args->addr,
                                     gcibc_args->size) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
  ...
  case H5VL_NATIVE_DATASET_CHUNK_READ: {
     ...
     */
    if (H5D__chunk_get_offset_copy(dset, chunk_read_args->offset, offset_copy) <
        0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                  "failure to copy offset array");

    /* Read the raw chunk */
    if (H5D__chunk_direct_read(dset, offset_copy, &chunk_read_args->filters,
  ...
  case H5VL_NATIVE_DATASET_CHUNK_WRITE: {
    H5VL_native_dataset_chunk_write_t *chunk_write_args =
        &opt_args->chunk_write;
    hsize_t offset_copy[H5O_LAYOUT_NDIMS]; /* Internal copy of chunk offset */
     ...
     * (we don't want to mess with the user's buffer).
     */
    if (H5D__chunk_get_offset_copy(dset, chunk_write_args->offset,
                                   offset_copy) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                  "failure to copy offset array");

    /* Write chunk */
    if (H5D__chunk_direct_write(dset, chunk_write_args->filters, offset_copy,
                                chunk_write_args->size,
  ...
  case H5VL_NATIVE_DATASET_GET_VLEN_BUF_SIZE: {
    H5VL_native_dataset_get_vlen_buf_size_t *gvbs_args =
        &opt_args->get_vlen_buf_size;

    if (H5D__vlen_get_buf_size(dset, gvbs_args->type_id, gvbs_args->space_id,
                               gvbs_args->size) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get size of vlen buf needed");
    break;
  }
  ...
  case H5VL_NATIVE_DATASET_CHUNK_ITER: {
    ...
    /* Call private function */
    if ((ret_value = H5D__chunk_iter(dset, opt_args->chunk_iter.op,
                                     opt_args->chunk_iter.op_data)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_dataset_close(void *dset, hid_t H5_ATTR_UNUSED dxpl_id,
                                  void H5_ATTR_UNUSED **req) {
  herr_t ret_value = SUCCEED; /* Return value */
