...
static hid_t H5R__open_object_api_common(H5R_ref_t *ref_ptr, hid_t rapl_id,
                                         hid_t oapl_id, void **token_ptr,
                                         H5VL_object_t **_vol_obj_ptr);
static hid_t H5R__open_region_api_common(H5R_ref_t *ref_ptr, hid_t rapl_id,
                                         hid_t oapl_id, void **token_ptr,
                                         H5VL_object_t **_vol_obj_ptr);
static hid_t H5R__open_attr_api_common(H5R_ref_t *ref_ptr, hid_t rapl_id,
...
herr_t H5Rcreate_object(hid_t loc_id, const char *name, hid_t oapl_id,
  ...
  H5VL_object_specific_args_t
      obj_spec_vol_cb_args;     /* Arguments to VOL callback */
  ...
  if (H5P_DEFAULT == oapl_id)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "oapl_id is not a link access property list ID");
  ...
  /* Get container info */
  if (H5VL_file_get(vol_obj_file, &file_get_vol_cb_args,
  ...
  if (H5VL_object_specific(vol_obj, &loc_params, &obj_spec_vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, FAIL,
                "unable to retrieve object token");

  /* Create the reference (do not pass filename, since file_id is attached) */
  memset(ref_ptr, 0, H5R_REF_BUF_SIZE);
  if (H5R__create_object(&obj_token, cont_info.token_size,
                         (H5R_ref_priv_t *)ref_ptr) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCREATE, FAIL,
                "unable to create object reference");

  /* Attach loc_id to reference and hold reference to it */
  if (H5R__set_loc_id((H5R_ref_priv_t *)ref_ptr, file_id, TRUE, TRUE) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTSET, FAIL,
                "unable to attach location id to reference");

done:
  if (file_id != H5I_INVALID_HID && H5I_dec_ref(file_id) < 0)
    HDONE_ERROR(H5E_REFERENCE, H5E_CANTDEC, FAIL,
                "unable to decrement refcount on file");
...
herr_t H5Rcreate_region(hid_t loc_id, const char *name, hid_t space_id,
  ...
  if ((space_id == H5I_INVALID_HID) || (space_id == H5S_ALL) ||
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "reference region dataspace id must be valid");
  if (NULL ==
      (space = (struct H5S_t *)H5I_object_verify(space_id, H5I_DATASPACE)))
  ...
  /* Create the reference (do not pass filename, since file_id is attached) */
  memset(ref_ptr, 0, H5R_REF_BUF_SIZE);
  if (H5R__create_region((const H5O_token_t *)&obj_token, cont_info.token_size,
                         space, (H5R_ref_priv_t *)ref_ptr) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCREATE, FAIL,
                "unable to create region reference");
...
herr_t H5Rcreate_attr(hid_t loc_id, const char *name, const char *attr_name,
  ...
  /* Create the reference (do not pass filename, since file_id is attached) */
  memset(ref_ptr, 0, H5R_REF_BUF_SIZE);
  if (H5R__create_attr((const H5O_token_t *)&obj_token, cont_info.token_size,
                       attr_name, (H5R_ref_priv_t *)ref_ptr) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCREATE, FAIL,
                "unable to create attribute reference");
...
herr_t H5Rdestroy(H5R_ref_t *ref_ptr) {
  ...
  if (H5R__destroy((H5R_ref_priv_t *)ref_ptr) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTFREE, FAIL,
                "unable to destroy reference");
...
H5R_type_t H5Rget_type(const H5R_ref_t *ref_ptr) {
  ...
  /* Check args */
  if (NULL == ref_ptr)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5R_BADTYPE,
  ...
  if ((ret_value <= H5R_BADTYPE) || (ret_value >= H5R_MAXTYPE))
    HGOTO_ERROR(H5E_REFERENCE, H5E_BADVALUE, H5R_BADTYPE,
...
htri_t H5Requal(const H5R_ref_t *ref1_ptr, const H5R_ref_t *ref2_ptr) {
  ...
  /* Compare references */
  if ((ret_value = H5R__equal((const H5R_ref_priv_t *)ref2_ptr,
                              (const H5R_ref_priv_t *)ref2_ptr)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCOMPARE, FAIL,
                "cannot compare references");
...
herr_t H5Rcopy(const H5R_ref_t *src_ref_ptr, H5R_ref_t *dst_ref_ptr) {
  ...
  /* Copy reference */
  if (H5R__copy((const H5R_ref_priv_t *)src_ref_ptr,
                (H5R_ref_priv_t *)dst_ref_ptr) < 0)
 ...
 *
 * Purpose:     This is the common function for opening an object via a
 *reference.
 *
...
static hid_t H5R__open_object_api_common(H5R_ref_t *ref_ptr, hid_t rapl_id,
  ...
  if (H5I_INVALID_HID ==
      (loc_id = H5R__get_loc_id((const H5R_ref_priv_t *)ref_ptr))) {
    /* Attempt to re-open file and pass rapl_id as a fapl_id */
    if ((loc_id = H5R__reopen_file((H5R_ref_priv_t *)ref_ptr, rapl_id)) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENFILE, H5I_INVALID_HID,
                  "cannot re-open referenced file");
  }
  ...
  if (NULL ==
    ...
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open object by token");

  /* Register object */
  if ((ret_value = H5VL_register(opened_type, opened_obj,
                                 (*vol_obj_ptr)->connector, TRUE)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTREGISTER, H5I_INVALID_HID,
...
hid_t H5Ropen_object(H5R_ref_t *ref_ptr, hid_t rapl_id, hid_t oapl_id) {
  ...
  if ((ret_value = H5R__open_object_api_common(ref_ptr, rapl_id, oapl_id, NULL,
                                               NULL)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open object synchronously");
...
hid_t H5Ropen_object_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE7("i", "*s*sIu*Rriii", app_file, app_func, app_line, ref_ptr, rapl_id,
           oapl_id, es_id);
  ...
  if ((ret_value = H5R__open_object_api_common(ref_ptr, rapl_id, oapl_id,
                                               token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open object asynchronously");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINSERT, H5I_INVALID_HID,
...
static hid_t H5R__open_region_api_common(H5R_ref_t *ref_ptr, hid_t rapl_id,
  ...
  /* Register object */
  if ((opened_obj_id = H5VL_register(opened_type, opened_obj,
                                     (*vol_obj_ptr)->connector, FALSE)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register object handle");
  ...
  if (H5VL_dataset_get(opened_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                       H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get dataspace from dataset");
  ...
  if (H5R__get_region((const H5R_ref_priv_t *)ref_ptr, space) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get selection on dataspace");
  ...
  if ((opened_obj_id != H5I_INVALID_HID) && (H5I_dec_ref(opened_obj_id) < 0))
    HDONE_ERROR(H5E_REFERENCE, H5E_CLOSEERROR, H5I_INVALID_HID,
                "can't close object");
...
hid_t H5Ropen_region(H5R_ref_t *ref_ptr, hid_t rapl_id, hid_t oapl_id) {
  ...
  if ((ret_value = H5R__open_region_api_common(ref_ptr, rapl_id, oapl_id, NULL,
                                               NULL)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open region synchronously");
...
hid_t H5Ropen_region_async(const char *app_file, const char *app_func,
  ...
  if ((ret_value = H5R__open_region_api_common(ref_ptr, rapl_id, oapl_id,
                                               token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open region asynchronously");
 ...
 *
 * Purpose:     This is the common function for opening an attribute via a
 *reference.
 *
 * Return:      Valid ID on success / H5I_INVALID_HID on failure
 *
...
static hid_t H5R__open_attr_api_common(H5R_ref_t *ref_ptr, hid_t rapl_id,
  ...
  /* Open the attribute */
  if (NULL == (opened_attr = H5VL_attr_open(
                   opened_obj, &loc_params,
  ...
  if ((ret_value = H5VL_register(H5I_ATTR, opened_attr,
                                 (*vol_obj_ptr)->connector, TRUE)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register attribute handle");
  ...
  if ((opened_obj_id != H5I_INVALID_HID) && (H5I_dec_ref(opened_obj_id) < 0))
    HDONE_ERROR(H5E_REFERENCE, H5E_CLOSEERROR, H5I_INVALID_HID,
                "can't close object");
  if (H5I_INVALID_HID == ret_value) /* Cleanup on failure */
    if (opened_attr && H5VL_attr_close(*vol_obj_ptr, H5P_DATASET_XFER_DEFAULT,
                                       H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_REFERENCE, H5E_CLOSEERROR, H5I_INVALID_HID,
...
hid_t H5Ropen_attr(H5R_ref_t *ref_ptr, hid_t rapl_id, hid_t aapl_id) {
  ...
  if ((ret_value = H5R__open_attr_api_common(ref_ptr, rapl_id, aapl_id, NULL,
                                             NULL)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_OPENERROR, H5I_INVALID_HID,
                "unable to open attribute synchronously");
...
hid_t H5Ropen_attr_async(const char *app_file, const char *app_func,
  ...
  if ((ret_value = H5R__open_attr_api_common(ref_ptr, rapl_id, aapl_id,
                                             token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_OPENERROR, H5I_INVALID_HID,
                "unable to open attribute asynchronously");
...
herr_t H5Rget_obj_type3(H5R_ref_t *ref_ptr, hid_t rapl_id,
  ...
  if (H5I_INVALID_HID ==
    ...
    /* Attempt to re-open file and pass rapl_id as a fapl_id */
    if ((loc_id = H5R__reopen_file((H5R_ref_priv_t *)ref_ptr, rapl_id)) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENFILE, FAIL,
                  "cannot re-open referenced file");
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_file_name(const H5R_ref_t *ref_ptr, char *buf /*out*/,
                         size_t size) {
  hid_t loc_id;      /* Reference location ID */
  ...
  if (H5I_INVALID_HID ==
    ...
    if ((ret_value = H5R__get_file_name((const H5R_ref_priv_t *)ref_ptr, buf,
                                        size)) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, (-1),
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_obj_name(H5R_ref_t *ref_ptr, hid_t rapl_id, char *buf /*out*/,
                        size_t size) {
  hid_t loc_id;                       /* Reference location ID */
  H5VL_object_t *vol_obj = NULL;      /* Object of loc_id */
  H5VL_object_get_args_t vol_cb_args; /* Arguments to VOL callback */
  ...
  if (H5I_INVALID_HID ==
    ...
    /* Attempt to re-open file and pass rapl_id as a fapl_id */
    if ((loc_id = H5R__reopen_file((H5R_ref_priv_t *)ref_ptr, rapl_id)) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTOPENFILE, (-1),
                  "cannot re-open referenced file");
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_attr_name(const H5R_ref_t *ref_ptr, char *buf /*out*/,
                         size_t size) {
  ssize_t ret_value; /* Return value */
  ...
  /* Get attribute name */
  if ((ret_value =
           H5R__get_attr_name((const H5R_ref_priv_t *)ref_ptr, buf, size)) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, (-1),
                "unable to determine attribute name");
