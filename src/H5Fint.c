...
static herr_t H5F__get_objects(const H5F_t *f, unsigned types, size_t max_index,
                               hid_t *obj_id_list, hbool_t app_ref,
...
static H5F_t *H5F__new(H5F_shared_t *shared, unsigned flags, hid_t fcpl_id,
                       hid_t fapl_id, H5FD_t *lf);
static herr_t H5F__check_if_using_file_locks(H5P_genplist_t *fapl,
                                             hbool_t *use_file_locking);
static herr_t H5F__dest(H5F_t *f, hbool_t flush);
static herr_t H5F__build_actual_name(const H5F_t *f, const H5P_genplist_t *fapl,
                                     const char *name,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__parse_file_lock_env_var(htri_t *use_locks) {
  ...
  if (lock_env_var &&
      (!HDstrcmp(lock_env_var, "FALSE") || !HDstrcmp(lock_env_var, "0")))
    *use_locks = FALSE; /* Override: Never use locks */
  else if (lock_env_var && (!HDstrcmp(lock_env_var, "TRUE") ||
                            !HDstrcmp(lock_env_var, "BEST_EFFORT") ||
...
static herr_t H5F__set_vol_conn(H5F_t *file) {
  ...
  /* Retrieve a copy of the "top-level" connector property, before any
   * pass-through connectors modified or unwrapped it.
   */
  if (H5CX_get_vol_connector_prop(&connector_prop) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                "can't get VOL connector info from API context");
  ...
  if (H5I_inc_ref(connector_prop.connector_id, FALSE) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTINC, FAIL,
                "incrementing VOL connector ID failed");

  conn_id_incr = TRUE;

  /* Retrieve the connector for the ID */
  if (NULL == (file->shared->vol_cls =
                   (H5VL_class_t *)H5I_object(connector_prop.connector_id)))
  ...
  if (ret_value < 0 && conn_id_incr)
    if (H5I_dec_ref(connector_prop.connector_id) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CANTDEC, FAIL,
                  "can't decrement VOL connector ID");
...
hid_t H5F_get_access_plist(H5F_t *f, hbool_t app_ref) {
  ...
  hbool_t driver_prop_copied =
      FALSE; /* Whether the driver property has been set up */
  H5VL_connector_prop_t
      connector_prop; /* Property for VOL connector ID & info */
  unsigned efc_size = 0;
  ...
  if (NULL ==
      (old_plist = (H5P_genplist_t *)H5I_object(H5P_LST_FILE_ACCESS_ID_g)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID, "not a property list");
  if ((ret_value = H5P_copy_plist(old_plist, app_ref)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, H5I_INVALID_HID,
                "can't copy file access property list");
  if (NULL == (new_plist = (H5P_genplist_t *)H5I_object(ret_value)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID, "not a property list");

  /* Copy properties of the file access property list */
  if (H5P_set(new_plist, H5F_ACS_META_CACHE_INIT_CONFIG_NAME,
              &(f->shared->mdc_initCacheCfg)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set initial metadata cache resize config.");
  if (H5P_set(new_plist, H5F_ACS_DATA_CACHE_NUM_SLOTS_NAME,
              &(f->shared->rdcc_nslots)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set data cache number of slots");
  if (H5P_set(new_plist, H5F_ACS_DATA_CACHE_BYTE_SIZE_NAME,
              &(f->shared->rdcc_nbytes)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set data cache byte size");
  if (H5P_set(new_plist, H5F_ACS_PREEMPT_READ_CHUNKS_NAME,
              &(f->shared->rdcc_w0)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
  ...
  if (H5P_set(new_plist, H5F_ACS_GARBG_COLCT_REF_NAME, &(f->shared->gc_ref)) <
      0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set garbage collect reference");
  if (H5P_set(new_plist, H5F_ACS_META_BLOCK_SIZE_NAME,
              &(f->shared->meta_aggr.alloc_size)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set metadata cache size");
  if (H5P_set(new_plist, H5F_ACS_SIEVE_BUF_SIZE_NAME,
              &(f->shared->sieve_buf_size)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't sieve buffer size");
  if (H5P_set(new_plist, H5F_ACS_SDATA_BLOCK_SIZE_NAME,
              &(f->shared->sdata_aggr.alloc_size)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set 'small data' cache size");
  if (H5P_set(new_plist, H5F_ACS_LIBVER_LOW_BOUND_NAME, &f->shared->low_bound) <
      0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set 'low' bound for library format versions");
  if (H5P_set(new_plist, H5F_ACS_LIBVER_HIGH_BOUND_NAME,
              &f->shared->high_bound) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set 'high' bound for library format versions");
  if (H5P_set(new_plist, H5F_ACS_METADATA_READ_ATTEMPTS_NAME,
              &(f->shared->read_attempts)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set 'read attempts ' flag");
  if (H5P_set(new_plist, H5F_ACS_OBJECT_FLUSH_CB_NAME,
              &(f->shared->object_flush)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
  ...
  if (f->shared->page_buf != NULL) {
    if (H5P_set(new_plist, H5F_ACS_PAGE_BUFFER_SIZE_NAME,
                &(f->shared->page_buf->max_size)) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                  "can't set page buffer size");
    ...
    if (H5P_set(new_plist, H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_NAME,
                &(f->shared->page_buf->min_raw_perc)) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                  "can't set minimum raw data fraction of page buffer");
  } /* end if */
#ifdef H5_HAVE_PARALLEL
  if (H5P_set(new_plist, H5_COLL_MD_READ_FLAG_NAME,
              &(f->shared->coll_md_read)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set collective metadata read flag");
  if (H5P_set(new_plist, H5F_ACS_COLL_MD_WRITE_FLAG_NAME,
              &(f->shared->coll_md_write)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set collective metadata read flag");
  if (H5F_HAS_FEATURE(f, H5FD_FEAT_HAS_MPI)) {
    ...
    if (H5P_set(new_plist, H5F_ACS_MPI_PARAMS_COMM_NAME, &mpi_comm) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                  "can't set MPI communicator");

    /* Retrieve and set MPI info object */
    if (H5P_get(old_plist, H5F_ACS_MPI_PARAMS_INFO_NAME, &mpi_info) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, H5I_INVALID_HID,
                  "can't get MPI info object");
    if (H5P_set(new_plist, H5F_ACS_MPI_PARAMS_INFO_NAME, &mpi_info) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                  "can't set MPI info object");
  }
#endif /* H5_HAVE_PARALLEL */
  if (H5P_set(new_plist, H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_NAME,
              &(f->shared->mdc_initCacheImageCfg)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set initial metadata cache resize config.");
  ...
  if (f->shared->fc_degree == H5F_CLOSE_DEFAULT &&
      H5P_set(new_plist, H5F_ACS_CLOSE_DEGREE_NAME,
              &(f->shared->lf->cls->fc_degree)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set file close degree");
  else if (f->shared->fc_degree != H5F_CLOSE_DEFAULT &&
           H5P_set(new_plist, H5F_ACS_CLOSE_DEGREE_NAME,
                   &(f->shared->fc_degree)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set file close degree");

done:
  /* Release the copy of the driver info, if it was set up */
  if (driver_prop_copied &&
      H5FD_free_driver_info(driver_prop.driver_id, driver_prop.driver_info) < 0)
    HDONE_ERROR(H5E_FILE, H5E_CANTCLOSEOBJ, H5I_INVALID_HID,
                "can't close copy of driver info");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_get_obj_count(const H5F_t *f, unsigned types, hbool_t app_ref,
                         size_t *obj_id_count_ptr) {
  ...
  /* Perform the query */
  if ((ret_value =
           H5F__get_objects(f, types, 0, NULL, app_ref, obj_id_count_ptr)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_get_obj_ids(const H5F_t *f, unsigned types, size_t max_objs,
                       hid_t *oid_list, hbool_t app_ref,
                       size_t *obj_id_count_ptr) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Perform the query */
  if ((ret_value = H5F__get_objects(f, types, max_objs, oid_list, app_ref,
                                    obj_id_count_ptr)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_BADITER, FAIL, "H5F__get_objects failed");
 ...
 *---------------------------------------------------------------------------
 */
static herr_t H5F__get_objects(const H5F_t *f, unsigned types, size_t max_nobjs,
                               hid_t *obj_id_list, hbool_t app_ref,
                               size_t *obj_id_count_ptr) {
  ...
  /* If the caller just wants to count the number of objects (OLIST.MAX_NOBJS is
   * zero), or the caller wants to get the list of IDs and the list isn't full,
   * search through dataset IDs to count number of datasets, and put their
   * IDs on the object list */
  if (!olist.max_nobjs ||
      (olist.max_nobjs && olist.list_index < olist.max_nobjs)) {
  ...
  /* If the caller just wants to count the number of objects (OLIST.MAX_NOBJS is
   * zero), or the caller wants to get the list of IDs and the list isn't full,
   * search through datatype IDs to count number of named datatypes, and put
   * their IDs on the object list */
  if (!olist.max_nobjs ||
      (olist.max_nobjs && olist.list_index < olist.max_nobjs)) {
  ...
  if (olist->obj_type == H5I_FILE) {
    if ((olist->file_info.local &&
         (!olist->file_info.ptr.file ||
          (olist->file_info.ptr.file &&
           (H5F_t *)obj_ptr == olist->file_info.ptr.file))) ||
        (!olist->file_info.local &&
         (!olist->file_info.ptr.shared ||
          (olist->file_info.ptr.shared &&
           ((H5F_t *)obj_ptr)->shared == olist->file_info.ptr.shared)))) {
      add_obj = TRUE;
    }    /* end if */
  }      /* end if */
    ...
    default:
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5_ITER_ERROR,
                  "unknown or invalid data object");
    } /* end switch */
    ...
    if ((olist->file_info.local &&
        ...
        (!olist->file_info.local &&
         ((!olist->file_info.ptr.shared && olist->obj_type == H5I_DATATYPE &&
          ...
          (!olist->file_info.ptr.shared && olist->obj_type != H5I_DATATYPE) ||
          (oloc && oloc->file &&
           oloc->file->shared == olist->file_info.ptr.shared)))) {
      add_obj = TRUE;
    } /* end if */
  }   /* end else */
...
static herr_t H5F__build_name(const char *prefix, const char *file_name,
  ...
  /* Allocate a buffer to hold the filename + prefix + possibly the delimiter +
   * terminating null byte */
  if (NULL == (*full_name = (char *)H5MM_malloc(
                   prefix_len + fname_len + 2 +
                   2))) /* Extra "+2" to quiet GCC warning - 2019/07/05, QAK */
    HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                "unable to allocate filename buffer");

  /* Compose the full file name */
  HDsnprintf(*full_name, (prefix_len + fname_len + 2 + 2), "%s%s%s",
             prefix, /* Extra "+2" to quiet GCC warning - 2019/07/05, QAK */
             ((prefix_len == 0 || H5_CHECK_DELIMITER(prefix[prefix_len - 1]))
                  ? ""
                  : H5_DIR_SEPS),
             file_name);
 ...
 *
 * Purpose:     Get the first pathname in the list of pathnames stored in
env_prefix,
 ...
 *-------------------------------------------------------------------------
 */
H5F_t *H5F_prefix_open_file(H5F_t *primary_file, H5F_prefix_open_t prefix_type,
                            const char *prop_prefix, const char *file_name,
                            unsigned file_intent, hid_t fapl_id) {
  ...
  if (H5_CHECK_ABSOLUTE(file_name) || H5_CHECK_ABS_PATH(file_name)) {
    /* Try opening file */
    src_file = H5F__efc_open(efc, file_name, file_intent,
                             H5P_FILE_CREATE_DEFAULT, fapl_id);
    ...
    if (NULL != env_prefix) {
      ...
      while ((tmp_env_prefix) && (*tmp_env_prefix)) {
        ...
        if (out_prefix_name && (*out_prefix_name)) {
          if (H5F__build_name(out_prefix_name, temp_file_name,
                              &full_name /*out*/) < 0) {
          ...
          /* Try opening file */
          src_file = H5F__efc_open(efc, full_name, file_intent,
                                   H5P_FILE_CREATE_DEFAULT, fapl_id);
  ...
  /* Try searching from main file's "extpath": see description in H5F_open() &
   * H5_build_extpath() */
  if (src_file == NULL) {
    char *dspath;
  ...
  if (src_file == NULL) {
    /* Try opening file */
    src_file = H5F__efc_open(efc, temp_file_name, file_intent,
                             H5P_FILE_CREATE_DEFAULT, fapl_id);
  ...
  if (src_file == NULL) {
    ...
    if (NULL == (actual_file_name = H5MM_strdup(H5F_ACTUAL_NAME(primary_file))))
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, NULL,
                  "can't duplicate resolved file name string");
    ...
    if (H5F__build_name((ptr ? actual_file_name : ""), temp_file_name,
                        &full_name /*out*/) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, NULL,
                  "can't prepend prefix to filename");
...
htri_t H5F__is_hdf5(const char *name, hid_t fapl_id) {
  ...
  /* Open the file */
  /* NOTE:    This now uses the fapl_id that was passed in, so
   * H5Fis_accessible() should work with arbitrary VFDs, unlike H5Fis_hdf5().
   */
   ...
   *
   * If the file is open with an exclusive lock on an operating system that
   * enforces mandatory file locks (like Windows), creating a new file handle
   * and attempting to read through it will fail so we have to try this first.
   */
...
static H5F_t *H5F__new(H5F_shared_t *shared, unsigned flags, hid_t fcpl_id,
  ...
  if (shared) {
    ...
    /* This is done to be backward compatible with 1.10 library that has the FSM
     * hack */
    ...
    if (H5P_get(plist, H5F_CRT_SHMSG_NINDEXES_NAME, &f->shared->sohm_nindexes) <
        0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get number of SOHM indexes");
    assert(f->shared->sohm_nindexes < 255);
    if (H5P_get(plist, H5F_CRT_FILE_SPACE_STRATEGY_NAME,
                &f->shared->fs_strategy) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get file space strategy");
    if (H5P_get(plist, H5F_CRT_FREE_SPACE_PERSIST_NAME,
                &f->shared->fs_persist) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get file space persisting status");
    if (H5P_get(plist, H5F_CRT_FREE_SPACE_THRESHOLD_NAME,
                &f->shared->fs_threshold) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get free-space section threshold");
    if (H5P_get(plist, H5F_CRT_FILE_SPACE_PAGE_SIZE_NAME,
                &f->shared->fs_page_size) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get file space page size");
    ...
    if (H5F_HAS_FEATURE(f, H5FD_FEAT_PAGED_AGGR))
      if (f->shared->fs_strategy == H5F_FSPACE_STRATEGY_PAGE ||
          f->shared->fs_persist)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                    "can't open with this strategy or persistent fs");

    /* Get the FAPL values to cache */
    if (NULL == (plist = (H5P_genplist_t *)H5I_object(fapl_id)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "not file access property list");
    if (H5P_get(plist, H5F_ACS_META_CACHE_INIT_CONFIG_NAME,
                &(f->shared->mdc_initCacheCfg)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get initial metadata cache resize config");
    if (H5P_get(plist, H5F_ACS_DATA_CACHE_NUM_SLOTS_NAME,
                &(f->shared->rdcc_nslots)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get data cache number of slots");
    if (H5P_get(plist, H5F_ACS_DATA_CACHE_BYTE_SIZE_NAME,
                &(f->shared->rdcc_nbytes)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get data cache byte size");
    if (H5P_get(plist, H5F_ACS_PREEMPT_READ_CHUNKS_NAME,
                &(f->shared->rdcc_w0)) < 0)
    ...
    if (H5P_get(plist, H5F_ACS_SIEVE_BUF_SIZE_NAME,
                &(f->shared->sieve_buf_size)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL, "can't get sieve buffer size");
    if (H5P_get(plist, H5F_ACS_LIBVER_LOW_BOUND_NAME, &(f->shared->low_bound)) <
        0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get 'low' bound for library format versions");
    if (H5P_get(plist, H5F_ACS_LIBVER_HIGH_BOUND_NAME,
                &(f->shared->high_bound)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get 'high' bound for library format versions");
    if (H5P_get(plist, H5F_ACS_USE_MDC_LOGGING_NAME,
                &(f->shared->use_mdc_logging)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get 'use mdc logging' flag");
    if (H5P_get(plist, H5F_ACS_START_MDC_LOG_ON_ACCESS_NAME,
                &(f->shared->start_mdc_log_on_access)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get 'start mdc log on access' flag");
    if (H5P_get(plist, H5F_ACS_META_BLOCK_SIZE_NAME,
                &(f->shared->meta_aggr.alloc_size)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get metadata cache size");
    f->shared->meta_aggr.feature_flag = H5FD_FEAT_AGGREGATE_METADATA;
    if (H5P_get(plist, H5F_ACS_SDATA_BLOCK_SIZE_NAME,
                &(f->shared->sdata_aggr.alloc_size)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get 'small data' cache size");
    ...
    if (efc_size > 0)
      if (NULL == (f->shared->efc = H5F__efc_create(efc_size)))
        HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                    "can't create external file cache");
#ifdef H5_HAVE_PARALLEL
    if (H5P_get(plist, H5_COLL_MD_READ_FLAG_NAME, &(f->shared->coll_md_read)) <
        0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get collective metadata read flag");
    if (H5P_get(plist, H5F_ACS_COLL_MD_WRITE_FLAG_NAME,
                &(f->shared->coll_md_write)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get collective metadata write flag");
#endif /* H5_HAVE_PARALLEL */
    if (H5P_get(plist, H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_NAME,
                &(f->shared->mdc_initCacheImageCfg)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get initial metadata cache resize config");
    ...
    if (H5FD_get_feature_flags(lf, &f->shared->feature_flags) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, NULL,
                  "can't get feature flags from VFD");
    ...
    if (H5FD_get_fs_type_map(lf, f->shared->fs_type_map) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, NULL,
                  "can't get free space type mapping from VFD");
    if (H5MF_init_merge_flags(f->shared) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                  "problem initializing free space merge flags");
    ...
    /* Retrieve the # of read attempts here so that sohm in superblock will get
     * the correct # of attempts
     */
    if (H5P_get(plist, H5F_ACS_METADATA_READ_ATTEMPTS_NAME,
                &f->shared->read_attempts) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get the # of read attempts");

    /* When opening file with SWMR access, the # of read attempts is
     * H5F_SWMR_METADATA_READ_ATTEMPTS if not set */
    /* When opening file without SWMR access, the # of read attempts is always
     * H5F_METADATA_READ_ATTEMPTS (set or not set) */
    if (H5F_INTENT(f) & (H5F_ACC_SWMR_READ | H5F_ACC_SWMR_WRITE)) {
      ...
      if (H5FD_set_feature_flags(f->shared->lf, f->shared->feature_flags) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTSET, NULL,
                    "can't set feature_flags in VFD");
    } else {
      /* If no value for read attempts has been set, use the default */
      if (!f->shared->read_attempts)
    ...
    if (H5F_set_retries(f) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                  "can't set retries and retries_nbins");

    /* Get the metadata cache log location (if we're logging) */
    {
      char *mdc_log_location =
          NULL; /* location of metadata cache log location */
      ...
      if (mdc_log_location != NULL) {
        ...
        if (NULL == (f->shared->mdc_log_location =
                         (char *)H5MM_calloc((len + 1) * sizeof(char))))
    ...
    if (H5P_get(plist, H5F_ACS_OBJECT_FLUSH_CB_NAME,
                &(f->shared->object_flush)) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, NULL,
                  "can't get object flush cb info");

    /* Get the VOL connector info */
    if (H5F__set_vol_conn(f) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                  "can't cache VOL connector info");

    /* Create a metadata cache with the specified number of elements.
     * The cache might be created with a different number of elements and
     * the access property list should be updated to reflect that.
     */
    if (H5AC_create(f, &(f->shared->mdc_initCacheCfg),
                    &(f->shared->mdc_initCacheImageCfg)) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                  "unable to create metadata cache");

    /* Create the file's "open object" information */
    if (H5FO_create(f) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                  "unable to create open object data structure");

    /* Add new "shared" struct to list of open files */
    if (H5F__sfile_add(f->shared) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                  "unable to append to list of open files");
  } /* end else */
    ...
    if (!shared) {
      ...
      if (f->shared->efc)
        if (H5F__efc_destroy(f->shared->efc) < 0)
          HDONE_ERROR(H5E_FILE, H5E_CANTRELEASE, NULL,
                      "can't destroy external file cache");
...
static herr_t H5F__dest(H5F_t *f, hbool_t flush) {
  ...
  if (1 == f->shared->nrefs) {
    ...
    if ((H5F_ACC_RDWR & H5F_INTENT(f)) && flush)
      if (H5F__flush_phase1(f) < 0)
        ...
        HDONE_ERROR(H5E_FILE, H5E_CANTFLUSH, FAIL,
                    "unable to flush cached data (phase 1)");
    ...
    if ((H5F_ACC_RDWR & H5F_INTENT(f)) && flush)
      if (H5F__flush_phase2(f, TRUE) < 0)
        /* Push error, but keep going */
        HDONE_ERROR(H5E_FILE, H5E_CANTFLUSH, FAIL,
                    "unable to flush cached data (phase 2)");
    ...
    if (f->shared->sblock) {
      ...
      if (H5F_ACC_RDWR & H5F_INTENT(f)) {
        if (H5MF_close(f) < 0)
          /* Push error, but keep going*/
          HDONE_ERROR(H5E_FILE, H5E_CANTRELEASE, FAIL,
                      "can't release file free space info");
        ...
        if (flush) {
          ...
          f->shared->sblock->status_flags &=
              (uint8_t)(~H5F_SUPER_SWMR_WRITE_ACCESS);

          /* Mark EOA info dirty in cache, so change will get encoded */
          if (H5F_eoa_dirty(f) < 0)
            /* Push error, but keep going*/
            HDONE_ERROR(H5E_FILE, H5E_CANTMARKDIRTY, FAIL,
      ...
      if (f->shared->drvinfo)
        if (H5AC_unpin_entry(f->shared->drvinfo) < 0)
          /* Push error, but keep going*/
          HDONE_ERROR(H5E_FSPACE, H5E_CANTUNPIN, FAIL,
                      "unable to unpin drvinfo");

      /* Unpin the superblock, since we're about to destroy the cache */
      if (H5AC_unpin_entry(f->shared->sblock) < 0)
        /* Push error, but keep going*/
        HDONE_ERROR(H5E_FSPACE, H5E_CANTUNPIN, FAIL,
                    "unable to unpin superblock");
    ...
    if (H5PB_dest(f->shared) < 0)
      /* Push error, but keep going*/
      HDONE_ERROR(H5E_FILE, H5E_CANTRELEASE, FAIL,
                  "problems closing page buffer cache");

    /* Clean up the metadata cache log location string */
    if (f->shared->mdc_log_location)
      f->shared->mdc_log_location =
          (char *)H5MM_xfree(f->shared->mdc_log_location);
    ...
    for (actype = 0; actype < (int)H5AC_NTYPES; actype++)
      if (f->shared->retries[actype])
        f->shared->retries[actype] =
            (uint32_t *)H5MM_xfree(f->shared->retries[actype]);
  ...
  if (f->vol_obj) {
    ...
    if (H5CX_get_vol_wrap_ctx((void **)&vol_wrap_ctx) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "can't get VOL object wrap context");
 ...
 *
 * The following two tables show results of file opens for single and concurrent
 *access:
 *
 ...
 *-- W  W  W  W  R  R  R  R                       W  W  W  W  R  R  R  R #2nd
 *open#                                   #2nd open#
 *            -------------------------- --------------------------
 *   -- --  W | s  x  x  s  x  x  f  f |          -- --  W | f  x  x  f  x  x  f
 *f | SR --  W | x  x  x  x  x  x  x  x |          SR --  W | x  x  x  x  x  x
 *x  x | SR SW  W | x  x  x  x  x  x  x  x |          SR SW  W | x  x  x  x  x
 *x  x  x |
 *   -- SW  W | f  x  x  s  x  x  f  f |          -- SW  W | f  x  x  f  x  x  f
 *f |
 *   -- SW  R | x  x  x  x  x  x  x  x |          -- SW  R | x  x  x  x  x  x  x
 *x | SR SW  R | x  x  x  x  x  x  x  x |          SR SW  R | x  x  x  x  x  x
 *x  x | SR --  R | s  x  x  s  x  x  s  f |          SR --  R | f  x  x  s  x
 *x  s  s |
 *   -- --  R | s  x  x  s  x  x  s  s |          -- --  R | f  x  x  f  x  x  s
 *s |
 *            -------------------------- --------------------------
 *
 *      Notations:
 ...
 *
 *        x: the first open or second open itself fails due to invalid flags
 *combination f: the open fails with flags combination from both the first and
 *second opens s: the open succeeds with flags combination from both the first
 *and second opens
 *
 *
 ...
 *-------------------------------------------------------------------------
 */
H5F_t *H5F_open(const char *name, unsigned flags, hid_t fcpl_id,
                hid_t fapl_id) {
  ...
  if (tent_flags != flags) {
    ...
    H5E_BEGIN_TRY { lf = H5FD_open(name, tent_flags, fapl_id, HADDR_UNDEF); }
    H5E_END_TRY
  }
  ...
  if ((tent_flags == flags) || (lf == NULL)) {
    ...
    if (NULL == (lf = H5FD_open(name, tent_flags, fapl_id, HADDR_UNDEF)))
      HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                  "unable to open file: name = '%s', tent_flags = %x", name,
                  tent_flags);
  }
  ...
  if ((shared = H5F__sfile_search(lf)) != NULL) {
    ...
    if (H5FD_close(lf) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                  "unable to close low-level file info");
    ...
    if ((flags & H5F_ACC_SWMR_WRITE) &&
        0 == (shared->flags & H5F_ACC_SWMR_WRITE))
      HGOTO_ERROR(
          H5E_FILE, H5E_CANTOPENFILE, NULL,
          "SWMR write access flag not the same for file that is already open");
    if ((flags & H5F_ACC_SWMR_READ) && !((shared->flags & H5F_ACC_SWMR_WRITE) ||
                                         (shared->flags & H5F_ACC_SWMR_READ) ||
    ...
    if (use_file_locking)
      if (H5FD_lock(lf, (hbool_t)((flags & H5F_ACC_RDWR) ? TRUE : FALSE)) < 0) {
        ...
        if (H5FD_close(lf) < 0)
          HDONE_ERROR(H5E_FILE, H5E_CANTCLOSEFILE, NULL,
                      "unable to close low-level file info");
        HGOTO_ERROR(H5E_FILE, H5E_CANTLOCKFILE, NULL,
    ...
    if (NULL == (file = H5F__new(NULL, flags, fcpl_id, fapl_id, lf))) {
      ...
      if (H5FD_close(lf) < 0)
        HDONE_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                    "unable to close low-level file info");
      HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                  "unable to initialize file structure");
    } /* end if */

    /* Need to set status_flags in the superblock if the driver has a 'lock'
     * method */
    if (drvr->lock)
      set_flag = TRUE;
  } /* end else */
  ...
  if (page_buf_size) {
    ...
    /* Temporary: fail file create when page buffering feature is enabled for
     * parallel */
    HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                "page buffering is disabled for parallel");
#endif /* H5_HAVE_PARALLEL */
    /* Query for other page buffer cache properties */
    if (H5P_get(a_plist, H5F_ACS_PAGE_BUFFER_MIN_META_PERC_NAME,
                &page_buf_min_meta_perc) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, NULL,
                  "can't get minimum metadata fraction of page buffer");
    if (H5P_get(a_plist, H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_NAME,
                &page_buf_min_raw_perc) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, NULL,
                  "can't get minimum raw data fraction of page buffer");
  } /* end if */
   ...
   * empty or not.
   */
  if (0 == (MAX(H5FD_get_eof(lf, H5FD_MEM_SUPER),
                H5FD_get_eoa(lf, H5FD_MEM_SUPER))) &&
      (flags & H5F_ACC_RDWR)) {
    /*
    ...
    if (page_buf_size)
      if (H5PB_create(shared, page_buf_size, page_buf_min_meta_perc,
                      page_buf_min_raw_perc) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                    "unable to create page buffer");

    /* Initialize information about the superblock and allocate space for it */
    /* (Writes superblock extension messages, if there are any) */
    if (H5F__super_init(file) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, NULL,
                  "unable to allocate file superblock");
  ...
  if (shared->nrefs == 1)
    ...
    if (shared->evict_on_close != evict_on_close)
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, NULL,
                  "file evict-on-close value doesn't match");
  } /* end if */

  /* Formulate the absolute path for later search of target file for external
   * links */
  if (shared->nrefs == 1) {
  ...
  if (set_flag) {
    if (H5F_INTENT(file) &
        H5F_ACC_RDWR) { /* Set and check consistency of status_flags */
      ...
      if (file->shared->sblock->super_vers >= HDF5_SUPERBLOCK_VERSION_3) {
        ...
        if (file->shared->sblock->status_flags & H5F_SUPER_WRITE_ACCESS ||
          ...
          HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                      "file is already open for write/SWMR write (may use "
                      "<h5clear file> to clear "
      ...
      if (file->shared->sblock->super_vers >= HDF5_SUPERBLOCK_VERSION_3) {
        if (H5F_INTENT(file) & H5F_ACC_SWMR_READ) {
          if ((file->shared->sblock->status_flags & H5F_SUPER_WRITE_ACCESS &&
               !(file->shared->sblock->status_flags &
        ...
        else if ((file->shared->sblock->status_flags &
                  H5F_SUPER_WRITE_ACCESS) ||
                 (file->shared->sblock->status_flags &
                  H5F_SUPER_SWMR_WRITE_ACCESS))
          HGOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                      "file is already open for write (may use <h5clear file> "
                      "to clear file "
                      "consistency flags)");
      } /* version 3 superblock */
    }   /* end else */
...
herr_t H5F__post_open(H5F_t *f) {
  ...
  if (NULL == (f->vol_obj = H5VL_create_object_using_vol_id(H5I_FILE, f,
                                                            f->shared->vol_id)))
...
static herr_t H5F__flush_phase2(H5F_t *f, hbool_t closing) {
  ...
  if (H5AC_flush(f) < 0)
    /* Push error, but keep going*/
    HDONE_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                "unable to flush metadata cache");
  ...
  /* Flush the entire metadata cache again since the EOA could have changed in
   * the truncate call. */
  if (H5AC_flush(f) < 0)
    /* Push error, but keep going*/
    HDONE_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
  ...
  if (H5F__accum_flush(f->shared) < 0)
    ...
    HDONE_ERROR(H5E_IO, H5E_CANTFLUSH, FAIL,
                "unable to flush metadata accumulator");
...
herr_t H5F__close(H5F_t *f) {
  ...
  if (f->shared->fc_degree == H5F_CLOSE_SEMI) {
    ...
    unsigned nopen_objs =
        0; /* Number of open objects in file/mount hierarchy */

    /* Get the number of open objects and open files on this file/mount
     * hierarchy */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_try_close(H5F_t *f, hbool_t *was_closed /*out*/) {
  unsigned nopen_files = 0; /* Number of open files in file/mount hierarchy */
  ...
  case H5F_CLOSE_SEMI:
    ...
    /* If we've gotten this far (ie. there are no open objects in the file),
     * fall through to flush & close */
    break;
  ...
  case H5F_CLOSE_STRONG:
    ...
    /* If we've gotten this far (ie. there are no open file IDs in the
     * file/mount hierarchy), fall through to flush & close */
    break;
  ...
  /* If the file close degree is "strong", close all the open objects in this
   * file */
  if (f->shared->fc_degree == H5F_CLOSE_STRONG) {
    ...
    if (f->nopen_objs > 0) {
      ...
      if (result < 0)
        HGOTO_ERROR(H5E_INTERNAL, H5E_BADITER, FAIL,
                    "H5F_get_obj_ids failed(2)");
    } /* end if */
  }   /* end if */
 ...
 *		        to the same file as the specified file handle.  Both
 *handles share caches and other information.  The only difference between the
 *handles is that the new handle is not mounted anywhere and no files are
 *mounted on it.
 *
...
H5F_t *H5F__reopen(H5F_t *f) {
   ...
   */
  if (NULL == (ret_value = H5F__new(f->shared, 0, H5P_FILE_CREATE_DEFAULT,
...
hid_t H5F_get_id(H5F_t *file) {
  ...
  if (H5I_find_id(file, H5I_FILE, &ret_value) < 0 ||
    ...
    if (H5I_inc_ref(ret_value, FALSE) < 0)
      HGOTO_ERROR(H5E_ID, H5E_CANTINC, H5I_INVALID_HID,
                  "incrementing file ID failed");
  } /* end else */
 ...
 */
  ...
  if (H5F_HAS_FEATURE(f, H5FD_FEAT_POSIX_COMPAT_HANDLE)) {
    ...
    if (HDlstat(name, &lst) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                  "can't retrieve stat info for file");
    ...
    if (S_IFLNK == (lst.st_mode & S_IFMT)) {
      ...
      if (NULL ==
          (realname = (char *)H5MM_calloc((size_t)PATH_MAX * sizeof(char))))
      ...
      if ((new_fapl_id = H5P_copy_plist(fapl, FALSE)) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTCOPY, FAIL,
      ...
      if (H5F_get_vfd_handle(f, new_fapl_id, (void **)&fd) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                    "can't retrieve POSIX file descriptor");
      ...
      if (st.st_mode != fst.st_mode || st.st_ino != fst.st_ino ||
          st.st_dev != fst.st_dev)
        HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                    "files' st_ino or st_dev fields changed!");

      /* Get the resolved path for the file name */
      if (NULL == HDrealpath(name, realname))
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                    "can't retrieve real path for file");
  ...
  if (new_fapl_id > 0)
    if (H5I_dec_app_ref(new_fapl_id) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CANTCLOSEOBJ, FAIL,
                  "can't close duplicated FAPL");
#ifdef H5_HAVE_SYMLINK
  if (realname)
 ...
 *-------------------------------------------------------------------------
 */
void H5F_addr_encode_len(size_t addr_len, uint8_t **pp /*in,out*/,
                         haddr_t addr) {
 ...
 *-------------------------------------------------------------------------
 */
void H5F_addr_decode_len(size_t addr_len, const uint8_t **pp /*in,out*/,
                         haddr_t *addr_p /*out*/) {
 ...
 *-------------------------------------------------------------------------
 */
void H5F_addr_decode(const H5F_t *f, const uint8_t **pp /*in,out*/,
                     haddr_t *addr_p /*out*/) {
  /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
  FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__get_file_image(H5F_t *file, void *buf_ptr, size_t buf_len,
                           size_t *image_len) {
  ...
  if (buf_ptr != NULL) {
    ...
    if (H5FD_read(fd_ptr, H5FD_MEM_DEFAULT, 0, (size_t)eoa, buf_ptr) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_READERROR, FAIL,
 ...
 *              f->shared->retries_nbins > 0 (calculated based on
 *f->shared->read_attempts)
 *
 * Return:      SUCCEED/FAIL
 *-------------------------------------------------------------------------
 */
herr_t H5F_track_metadata_read_retries(H5F_t *f, unsigned actype,
                                       unsigned retries) {
  unsigned
      log_ind; /* Index to the array of retries based on log10 of retries */
  ...
  if (NULL == f->shared->retries[actype])
    if (NULL == (f->shared->retries[actype] = (uint32_t *)H5MM_calloc(
...
herr_t H5F_object_flush_cb(H5F_t *f, hid_t obj_id) {
  ...
  if (f->shared->object_flush.func &&
    ...
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "object flush callback returns error");
...
herr_t H5F__set_base_addr(const H5F_t *f, haddr_t addr) {
  ...
  if (H5FD_set_base_addr(f->shared->lf, addr) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                "driver set_base_addr request failed");
...
herr_t H5F__set_paged_aggr(const H5F_t *f, hbool_t paged) {
  ...
  if (H5FD_set_paged_aggr(f->shared->lf, paged) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                "driver set paged aggr mode failed");
...
herr_t H5F__get_max_eof_eoa(const H5F_t *f, haddr_t *max_eof_eoa) {
  ...
  if (HADDR_UNDEF == tmp_max)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                "file get eof/eoa requests failed");
...
herr_t H5F_get_metadata_read_retry_info(H5F_t *file, H5F_retry_info_t *info) {
  ...
  /* Map and copy information to info's retries for metadata items with tracking
   * for read retries */
  j = 0;
...
herr_t H5F__start_swmr_write(H5F_t *f) {
  ...
  size_t nt_attr_count =
      0;                 /* # of opened named datatypes  + opened attributes */
  ...
  size_t u;                     /* Local index variable */
  hbool_t setup =
      FALSE; /* Boolean flag to indicate whether SWMR setting is enabled */
  ...
  if (f->shared->sblock->super_vers < HDF5_SUPERBLOCK_VERSION_3)
    HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                "file superblock version - should be at least 3");

  /* Check for correct file format version */
  if ((f->shared->low_bound < H5F_LIBVER_V110) ||
      (f->shared->high_bound < H5F_LIBVER_V110))
    HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                "file format version does not support SWMR - needs to be 1.10 "
                "or greater");

  /* Should not be marked for SWMR writing mode already */
  if (f->shared->sblock->status_flags & H5F_SUPER_SWMR_WRITE_ACCESS)
    HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                "file already in SWMR writing mode");
  ...
  if (ci_load || ci_write)
    HGOTO_ERROR(H5E_FILE, H5E_UNSUPPORTED, FAIL,
                "can't have both SWMR and MDC cache image");
  ...
  if (H5F_get_obj_count(f, H5F_OBJ_DATATYPE | H5F_OBJ_ATTR, FALSE,
                        &nt_attr_count) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_BADITER, FAIL, "H5F_get_obj_count failed");
  if (nt_attr_count > 0)
    HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                "named datatypes and/or attributes opened in the file");

  /* Get the # of opened datasets and groups */
  if (H5F_get_obj_count(f, H5F_OBJ_GROUP | H5F_OBJ_DATASET, FALSE,
                        &grp_dset_count) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_BADITER, FAIL, "H5F_get_obj_count failed");

  if (grp_dset_count > 0) {
    /* Allocate space for group and object locations */
    if ((obj_ids = (hid_t *)H5MM_malloc(grp_dset_count * sizeof(hid_t))) ==
        NULL)
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                  "can't allocate buffer for hid_t");
    if ((obj_glocs = (H5G_loc_t *)H5MM_malloc(grp_dset_count *
                                              sizeof(H5G_loc_t))) == NULL)
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                  "can't allocate buffer for object group locations");
    if ((obj_olocs = (H5O_loc_t *)H5MM_malloc(grp_dset_count *
                                              sizeof(H5O_loc_t))) == NULL)
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                  "can't allocate buffer for object locations");
    if ((obj_paths = (H5G_name_t *)H5MM_malloc(grp_dset_count *
                                               sizeof(H5G_name_t))) == NULL)
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                  "can't allocate buffer for object paths");

    /* Taking a shortcut here to use calloc to initialize obj_apl_ids to all
     * H5P_DEFAULT.  If this changes in the future we'll need to either
     * initialize this array to all H5P_DEFAULT or ensure 0 cannot be a valid
     * value and check for 0 at cleanup. */
    if ((obj_apl_ids = (hid_t *)H5MM_calloc(grp_dset_count * sizeof(hid_t))) ==
        NULL)
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                  "can't allocate buffer for hid_t");
    ...
    if (H5F_get_obj_ids(f, H5F_OBJ_GROUP | H5F_OBJ_DATASET, grp_dset_count,
                        obj_ids, FALSE, &grp_dset_count) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL, "H5F_get_obj_ids failed");

    /* Save the VOL connector and the object wrapping context for the refresh
     * step */
    if (grp_dset_count > 0) {
    ...
    for (u = 0; u < grp_dset_count; u++) {
      ...
      if (H5I_dec_ref(obj_ids[u]) < 0)
        HGOTO_ERROR(H5E_ID, H5E_CANTCLOSEOBJ, FAIL,
                    "decrementing object ID failed");
    } /* end for */
  }   /* end if */
  ...
  if (H5F_USE_FILE_LOCKING(f)) {
    ...
    if (H5FD_unlock(f->shared->lf) < 0) {
      HGOTO_ERROR(H5E_FILE, H5E_CANTUNLOCKFILE, FAIL,
                  "unable to unlock the file");
    }
  ...
  if (H5F_super_dirty(f) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTMARKDIRTY, FAIL,
                "unable to mark superblock as dirty");
  ...
  if (H5F__evict_cache_entries(f) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTFLUSH, FAIL,
                "unable to evict file's cached information");

  /* Refresh (reopen) the objects (groups & datasets) in the file */
  for (u = 0; u < grp_dset_count; u++)
    if (H5O_refresh_metadata_reopen(obj_ids[u], obj_apl_ids[u], &obj_glocs[u],
                                    vol_connector, TRUE) < 0)
  ...
  if (ret_value < 0 && setup) {
    ...
    f->shared->read_attempts = H5F_METADATA_READ_ATTEMPTS;
    if (H5F_set_retries(f) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
                  "can't set retries and retries_nbins");
  ...
  if (H5F_USE_FILE_LOCKING(f))
    if (H5FD_unlock(f->shared->lf) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CANTUNLOCKFILE, FAIL,
                  "unable to unlock the file");
  ...
  if (obj_apl_ids) {
    for (u = 0; u < grp_dset_count; u++)
      if (obj_apl_ids[u] != H5P_DEFAULT && obj_apl_ids[u] >= 0 &&
          H5I_dec_ref(obj_apl_ids[u]) < 0)
        HDONE_ERROR(H5E_ID, H5E_CANTDEC, FAIL,
                    "decrementing property list ID failed");
...
herr_t H5F__format_convert(H5F_t *f) {
  ...
  if (!(f->shared->fs_strategy == H5F_FILE_SPACE_STRATEGY_DEF &&
    ...
    if (H5MF_try_close(f) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTRELEASE, FAIL,
                  "unable to free free-space address");
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5F_get_file_id(H5VL_object_t *vol_obj, H5I_type_t obj_type,
                      hbool_t app_ref) {
  ...
  if (H5I_INVALID_HID == file_id) {
    ...
    if (H5VL_set_vol_wrapper(vol_obj) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTSET, H5I_INVALID_HID,
                  "can't set VOL wrapper info");
    vol_wrapper_set = TRUE;

    if ((file_id = H5VL_wrap_register(H5I_FILE, vol_obj_file, app_ref)) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTREGISTER, H5I_INVALID_HID,
                  "unable to register file handle");
  } /* end if */
  else {
  ...
  if (vol_wrapper_set && H5VL_reset_vol_wrapper() < 0)
    HDONE_ERROR(H5E_FILE, H5E_CANTRESET, H5I_INVALID_HID,
