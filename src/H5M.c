...
static hid_t H5M__create_api_common(hid_t loc_id, const char *name,
                                    hid_t key_type_id, hid_t val_type_id,
                                    hid_t lcpl_id, hid_t mcpl_id, hid_t mapl_id,
                                    void **token_ptr,
                                    H5VL_object_t **_vol_obj_ptr);
static hid_t H5M__open_api_common(hid_t loc_id, const char *name, hid_t mapl_id,
                                  void **token_ptr,
                                  H5VL_object_t **_vol_obj_ptr);
static herr_t H5M__put_api_common(hid_t map_id, hid_t key_mem_type_id,
                                  const void *key, hid_t val_mem_type_id,
                                  const void *value, hid_t dxpl_id,
                                  void **token_ptr,
                                  H5VL_object_t **_vol_obj_ptr);
static herr_t H5M__get_api_common(hid_t map_id, hid_t key_mem_type_id,
                                  const void *key, hid_t val_mem_type_id,
                                  void *value, hid_t dxpl_id, void **token_ptr,
                                  H5VL_object_t **_vol_obj_ptr);
...
static herr_t H5M__close_cb(H5VL_object_t *map_vol_obj, void **request) {
  ...
  /* Close the map */
  if (H5VL_optional(map_vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    request) < 0)
...
static hid_t H5M__create_api_common(hid_t loc_id, const char *name,
  ...
  if (H5P_DEFAULT == mcpl_id)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                "mcpl_id is not a map create property list ID");

  /* Set up VOL callback arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_MACC, TRUE, &mapl_id, vol_obj_ptr,
                          &map_args.create.loc_params) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTSET, H5I_INVALID_HID,
  ...
  /* Create the map */
  if (H5VL_optional(*vol_obj_ptr, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    token_ptr) < 0)
  ...
  if ((ret_value =
           H5VL_register(H5I_MAP, map, (*vol_obj_ptr)->connector, TRUE)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register map handle");
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Mcreate(hid_t loc_id, const char *name, hid_t key_type_id,
  ...
  if ((ret_value =
           H5M__create_api_common(loc_id, name, key_type_id, val_type_id,
                                  lcpl_id, mcpl_id, mapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to create map synchronously");
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Mcreate_async(const char *app_file, const char *app_func,
                      unsigned app_line, hid_t loc_id, const char *name,
                      hid_t key_type_id, hid_t val_type_id, hid_t lcpl_id,
  ...
  H5TRACE11("i", "*s*sIui*siiiiii", app_file, app_func, app_line, loc_id, name,
            key_type_id, val_type_id, lcpl_id, mcpl_id, mapl_id, es_id);
  ...
  if ((ret_value = H5M__create_api_common(loc_id, name, key_type_id,
                                          val_type_id, lcpl_id, mcpl_id,
                                          mapl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to create map asynchronously");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      if (H5I_dec_app_ref_always_close(ret_value) < 0)
        HDONE_ERROR(H5E_MAP, H5E_CANTDEC, H5I_INVALID_HID,
                    "can't decrement count on map ID");
      HGOTO_ERROR(H5E_MAP, H5E_CANTINSERT, H5I_INVALID_HID,
 ...
 *                          be linked into the group hierarchy before being
 *closed or it will be deleted. The dataset should be closed when the caller is
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Mcreate_anon(hid_t loc_id, hid_t key_type_id, hid_t val_type_id,
  ...
  if (H5P_DEFAULT == mcpl_id)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                "not map create property list ID");
  ...
  /* Create the map */
  if (H5VL_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTINIT, H5I_INVALID_HID, "unable to create map");
  map = map_args.create.map;
  ...
  if (H5I_INVALID_HID == ret_value) {
    ...
    if (map && H5VL_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                             H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, H5I_INVALID_HID,
                  "unable to release map");
  } /* end if */
...
static hid_t H5M__open_api_common(hid_t loc_id, const char *name, hid_t mapl_id,
  ...
  /* Set up VOL callback arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_MACC, FALSE, &mapl_id, vol_obj_ptr,
                          &map_args.open.loc_params) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");
  ...
  if (H5VL_optional(*vol_obj_ptr, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    token_ptr) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open map");
...
hid_t H5Mopen(hid_t loc_id, const char *name, hid_t mapl_id) {
  ...
  if ((ret_value = H5M__open_api_common(loc_id, name, mapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to open map synchronously");
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Mopen_async(const char *app_file, const char *app_func,
                    unsigned app_line, hid_t loc_id, const char *name,
  ...
  if ((ret_value = H5M__open_api_common(loc_id, name, mapl_id, token_ptr,
                                        &vol_obj)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to open map asynchronously");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Mclose_async(const char *app_file, const char *app_func,
                      unsigned app_line, hid_t map_id, hid_t es_id) {
  void *token = NULL; /* Request token for async operation            */
  void **token_ptr =
      H5_REQUEST_NULL; /* Pointer to request token for async operation */
  ...
  if (H5I_dec_app_ref_always_close_async(map_id, token_ptr) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTDEC, FAIL,
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      /* clang-format on */
      HGOTO_ERROR(H5E_MAP, H5E_CANTINSERT, FAIL,
                  "can't insert token into event set");

done:
  if (connector && H5VL_conn_dec_rc(connector) < 0)
    HDONE_ERROR(H5E_MAP, H5E_CANTDEC, FAIL,
...
hid_t H5Mget_key_type(hid_t map_id) {
  ...
  if (H5VL_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get key datatype");
...
hid_t H5Mget_create_plist(hid_t map_id) {
  ...
  if (H5VL_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get map creation properties");
...
hid_t H5Mget_access_plist(hid_t map_id) {
  ...
  if (H5VL_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get map access properties");
...
herr_t H5Mget_count(hid_t map_id, hsize_t *count /*out*/, hid_t dxpl_id) {
  ...
  if (H5VL_optional(vol_obj, &vol_cb_args, dxpl_id, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get KV pair count for map");
...
static herr_t H5M__put_api_common(hid_t map_id, hid_t key_mem_type_id,
  ...
  if (val_mem_type_id < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid value memory datatype ID");

  /* Get map pointer */
  if (NULL ==
      (*vol_obj_ptr = (H5VL_object_t *)H5I_object_verify(map_id, H5I_MAP)))
...
herr_t H5Mput(hid_t map_id, hid_t key_mem_type_id, const void *key,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE6("e", "ii*xi*xi", map_id, key_mem_type_id, key, val_mem_type_id,
           value, dxpl_id);

  /* Add key-value pair to the map synchronously */
  if ((ret_value =
           H5M__put_api_common(map_id, key_mem_type_id, key, val_mem_type_id,
                               value, dxpl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTPUT, FAIL,
                "unable to put value to map synchronously");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Mput_async(const char *app_file, const char *app_func,
                    unsigned app_line, hid_t map_id, hid_t key_mem_type_id,
                    const void *key, hid_t val_mem_type_id, const void *value,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIuii*xi*xii", app_file, app_func, app_line, map_id,
            key_mem_type_id, key, val_mem_type_id, value, dxpl_id, es_id);
  ...
  if ((ret_value =
           H5M__put_api_common(map_id, key_mem_type_id, key, val_mem_type_id,
                               value, dxpl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTPUT, FAIL,
                "unable to put value to map asynchronously");
...
herr_t H5Mget(hid_t map_id, hid_t key_mem_type_id, const void *key,
  ...
  if ((ret_value =
           H5M__get_api_common(map_id, key_mem_type_id, key, val_mem_type_id,
                               value, dxpl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTGET, FAIL,
                "unable to get value from map synchronously");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Mget_async(const char *app_file, const char *app_func,
                    unsigned app_line, hid_t map_id, hid_t key_mem_type_id,
                    const void *key, hid_t val_mem_type_id, void *value,
                    hid_t dxpl_id, hid_t es_id) {
  H5VL_object_t *vol_obj = NULL; /* Object for loc_id */
  void *token = NULL;            /* Request token for async operation        */
  void **token_ptr =
  ...
  if ((ret_value =
           H5M__get_api_common(map_id, key_mem_type_id, key, val_mem_type_id,
                               value, dxpl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTGET, FAIL,
                "unable to get value from map asynchronously");
...
herr_t H5Mexists(hid_t map_id, hid_t key_mem_type_id, const void *key,
  ...
  /* Check if key exists */
  if (H5VL_optional(vol_obj, &vol_cb_args, dxpl_id, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_MAP, H5E_CANTGET, ret_value,
                "unable to check if key exists");
...
herr_t H5Miterate(hid_t map_id, hsize_t *idx, hid_t key_mem_type_id,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE6("e", "i*hiMI*xi", map_id, idx, key_mem_type_id, op, op_data,
           dxpl_id);
  ...
  /* Iterate over keys */
  if ((ret_value =
           H5VL_optional(vol_obj, &vol_cb_args, dxpl_id, H5_REQUEST_NULL)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Miterate_by_name(hid_t loc_id, const char *map_name, hsize_t *idx,
                          hid_t key_mem_type_id, H5M_iterate_t op,
  ...
  if (!map_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "map_name parameter cannot be NULL");
  if (!*map_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "map_name parameter cannot be an empty string");
  if (key_mem_type_id < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "invalid key memory datatype ID");
  if (!op)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "no operator specified");
  ...
  map_args.specific.args.iterate.loc_params.loc_data.loc_by_name.name =
      map_name;
  map_args.specific.args.iterate.loc_params.loc_data.loc_by_name.lapl_id =
      lapl_id;
  map_args.specific.args.iterate.idx = (idx ? *idx : 0);
  map_args.specific.args.iterate.key_mem_type_id = key_mem_type_id;
  map_args.specific.args.iterate.op = op;
