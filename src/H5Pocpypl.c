...
/* Definitions for callback function when completing the search for a matching
...
/* Property callbacks */
static herr_t H5P__ocpy_merge_comm_dt_list_set(hid_t prop_id, const char *name,
                                               size_t size, void *value);
static herr_t H5P__ocpy_merge_comm_dt_list_get(hid_t prop_id, const char *name,
                                               size_t size, void *value);
static herr_t H5P__ocpy_merge_comm_dt_list_enc(const void *value, void **_pp,
                                               size_t *size);
static herr_t H5P__ocpy_merge_comm_dt_list_dec(const void **_pp, void *value);
static herr_t H5P__ocpy_merge_comm_dt_list_del(hid_t prop_id, const char *name,
                                               size_t size, void *value);
static herr_t H5P__ocpy_merge_comm_dt_list_copy(const char *name, size_t size,
                                                void *value);
static int H5P__ocpy_merge_comm_dt_list_cmp(const void *value1,
                                            const void *value2, size_t size);
static herr_t H5P__ocpy_merge_comm_dt_list_close(const char *name, size_t size,
                                                 void *value);
...
static const unsigned H5O_def_ocpy_option_g =
    H5O_CPY_OPTION_DEF; /* Default object copy flags */
...
static const H5O_mcdt_cb_info_t H5O_def_mcdt_cb_g =
    H5O_CPY_MCDT_SEARCH_CB_DEF; /* Default callback before searching the global
...
static herr_t H5P__ocpy_reg_prop(H5P_genclass_t *pclass) {
  ...
  if (H5P__register_real(pclass, H5O_CPY_OPTION_NAME, H5O_CPY_OPTION_SIZE,
                         &H5O_def_ocpy_option_g, NULL, NULL, NULL,
                         H5O_CPY_OPTION_ENC, H5O_CPY_OPTION_DEC, NULL, NULL,
                         NULL, NULL) < 0)
  ...
  if (H5P__register_real(
          pclass, H5O_CPY_MERGE_COMM_DT_LIST_NAME,
          H5O_CPY_MERGE_COMM_DT_LIST_SIZE, &H5O_def_merge_comm_dtype_list_g,
          NULL, H5O_CPY_MERGE_COMM_DT_LIST_SET, H5O_CPY_MERGE_COMM_DT_LIST_GET,
          H5O_CPY_MERGE_COMM_DT_LIST_ENC, H5O_CPY_MERGE_COMM_DT_LIST_DEC,
          H5O_CPY_MERGE_COMM_DT_LIST_DEL, H5O_CPY_MERGE_COMM_DT_LIST_COPY,
          H5O_CPY_MERGE_COMM_DT_LIST_CMP, H5O_CPY_MERGE_COMM_DT_LIST_CLOSE) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register property for callback when completing the search for a matching
   * named datatype from the named dtype list */
  /* (Note: this property should not have an encode/decode callback -QAK) */
  if (H5P__register_real(pclass, H5O_CPY_MCDT_SEARCH_CB_NAME,
                         H5O_CPY_MCDT_SEARCH_CB_SIZE, &H5O_def_mcdt_cb_g, NULL,
...
H5P__copy_merge_comm_dt_list(H5O_copy_dtype_merge_list_t **value) {
  const H5O_copy_dtype_merge_list_t
      *src_dt_list; /* Source merge named datatype lists */
  H5O_copy_dtype_merge_list_t *dst_dt_list =
      NULL; /* Destination merge named datatype lists */
  H5O_copy_dtype_merge_list_t
      *dst_dt_list_tail = NULL,
 ...
 *
 * Purpose:     Copies a merge committed datatype list property when it's set
 *for a property list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__ocpy_merge_comm_dt_list_set(hid_t H5_ATTR_UNUSED prop_id,
  ...
  if (H5P__copy_merge_comm_dt_list((H5O_copy_dtype_merge_list_t **)value) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL,
                "can't copy merge committed dtype list");
 ...
 *
 * Purpose:     Copies a merge committed datatype list property when it's
 *retrieved from a property list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 *
 *-------------------------------------------------------------------------
 */
static herr_t H5P__ocpy_merge_comm_dt_list_get(hid_t H5_ATTR_UNUSED prop_id,
                                               const char H5_ATTR_UNUSED *name,
                                               size_t H5_ATTR_UNUSED size,
                                               void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
...
static herr_t H5P__ocpy_merge_comm_dt_list_enc(const void *value, void **_pp,
                                               size_t *size) {
  const H5O_copy_dtype_merge_list_t *const *dt_list_ptr =
      (const H5O_copy_dtype_merge_list_t *const *)value;
  uint8_t **pp = (uint8_t **)_pp;
  const H5O_copy_dtype_merge_list_t
      *dt_list; /* Pointer to merge named datatype list */
...
static herr_t H5P__ocpy_merge_comm_dt_list_dec(const void **_pp, void *_value) {
  H5O_copy_dtype_merge_list_t **dt_list = (H5O_copy_dtype_merge_list_t **)
      _value; /* Pointer to merge named datatype list */
 ...
 *
 * Purpose:     Frees memory used to store the merge committed datatype list
 *property
 *
 * Return:	Success:	Non-negative
 * 		Failure:	Negative
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5P__ocpy_merge_comm_dt_list_del(hid_t H5_ATTR_UNUSED prop_id,
                                               const char H5_ATTR_UNUSED *name,
                                               size_t H5_ATTR_UNUSED size,
                                               void *value) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5P__ocpy_merge_comm_dt_list_copy(const char H5_ATTR_UNUSED *name,
                                                size_t H5_ATTR_UNUSED size,
                                                void *value) {
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__ocpy_merge_comm_dt_list_cmp(const void *_dt_list1,
                                            const void *_dt_list2,
                                            size_t H5_ATTR_UNUSED size) {
  const H5O_copy_dtype_merge_list_t
      *dt_list1 = *(H5O_copy_dtype_merge_list_t *const *)
 ...
 */
static herr_t
H5P__ocpy_merge_comm_dt_list_close(const char H5_ATTR_UNUSED *name,
 ...
 *
 * Purpose:     Set properties when copying an object (group, dataset, and
 *datatype) from one location to another
 *
 * Usage:       H5Pset_copy_group(plist_id, cpy_option)
 *              hid_t plist_id;			IN: Property list to copy object
 *              unsigned cpy_option; 		IN: Options to copy object such
 *as H5O_COPY_SHALLOW_HIERARCHY_FLAG    -- Copy only immediate members
 *                  H5O_COPY_EXPAND_SOFT_LINK_FLAG     -- Expand soft links into
 *new objects/ H5O_COPY_EXPAND_EXT_LINK_FLAG      -- Expand external links into
 *new objects H5O_COPY_EXPAND_REFERENCE_FLAG -- Copy objects that are pointed by
 *references H5O_COPY_WITHOUT_ATTR_FLAG         -- Copy object without copying
 *attributes
 *
 * Return:      Non-negative on success/Negative on failure
 *
...
herr_t H5Padd_merge_committed_dtype_path(hid_t plist_id, const char *path) {
  H5P_genplist_t *plist; /* Property list pointer */
  H5O_copy_dtype_merge_list_t
      *old_list; /* Merge committed dtype list currently present */
  ...
  if (H5P_peek(plist, H5O_CPY_MERGE_COMM_DT_LIST_NAME, &old_list) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get merge named dtype list");
  ...
  if (H5P_poke(plist, H5O_CPY_MERGE_COMM_DT_LIST_NAME, &new_obj) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set merge named dtype list");
...
herr_t H5Pfree_merge_committed_dtype_paths(hid_t plist_id) {
  H5P_genplist_t *plist; /* Property list pointer */
  H5O_copy_dtype_merge_list_t
      *dt_list;               /* Merge committed dtype list currently present */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5P_peek(plist, H5O_CPY_MERGE_COMM_DT_LIST_NAME, &dt_list) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get merge committed dtype list");
  ...
  if (H5P_poke(plist, H5O_CPY_MERGE_COMM_DT_LIST_NAME, &dt_list) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set merge committed dtype list");
 ...
 *
 * Purpose:     Set the callback function when a matching committed datatype is
 *not found from the list of paths stored in the object copy property list.
 ...
 *
 * Usage:       H5Pget_mcdt_search_cb(plist_id, H5O_mcdt_search_cb_t *func, void
 ***op_data) hid_t plist_id;                 IN: Property list to copy object
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_mcdt_search_cb(hid_t plist_id, H5O_mcdt_search_cb_t *func /*out*/,
                             void **op_data /*out*/) {
  H5P_genplist_t *plist;      /* Property list pointer */
  H5O_mcdt_cb_info_t cb_info; /* Callback info struct */
  herr_t ret_value = SUCCEED; /* Return value */
