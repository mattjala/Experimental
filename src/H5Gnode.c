...
static herr_t H5G__node_found(H5F_t *f, haddr_t addr, const void *_lt_key,
...
static herr_t H5G__node_decode_key(const H5B_shared_t *shared,
                                   const uint8_t *raw, void *_key);
static herr_t H5G__node_encode_key(const H5B_shared_t *shared, uint8_t *raw,
                                   const void *_key);
 ...
 *
 * Return:      Success:    Pointer to the raw B-tree page for this file's
 *groups
 *
 *              Failure:	Can't fail
 *
 *-------------------------------------------------------------------------
 */
static H5UC_t *H5G__node_get_shared(const H5F_t *f,
...
static herr_t H5G__node_debug_key(FILE *stream, int indent, int fwidth,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Heap offset:", (unsigned)key->offset);
  ...
  if (udata->heap) {
    ...
    fprintf(stream, "%*s%-*s ", indent, "", fwidth,
            "Cannot get name; heap address not specified\n");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__node_create(H5F_t *f, H5B_ins_t H5_ATTR_UNUSED op,
                               void *_lt_key, void H5_ATTR_UNUSED *_udata,
  ...
  if (HADDR_UNDEF ==
      (*addr_p = H5MF_alloc(f, H5FD_MEM_BTREE, (hsize_t)sym->node_size)))
...
static herr_t H5G__node_cmp2(void *_lt_key, void *_udata, void *_rt_key) {
  ...
  if ((s1 = (const char *)H5HL_offset_into(udata->heap, lt_key->offset)) ==
      NULL)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL, "unable to get key name");
  if ((s2 = (const char *)H5HL_offset_into(udata->heap, rt_key->offset)) ==
      NULL)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL, "unable to get key name");
...
static herr_t H5G__node_cmp3(void *_lt_key, void *_udata, void *_rt_key) {
  ...
  if (HDstrcmp(udata->name, s) <= 0)
    ...
    /* right side */
    if ((s = (const char *)H5HL_offset_into(udata->heap, rt_key->offset)) ==
        NULL)
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL, "unable to get key name");
    if (HDstrcmp(udata->name, s) > 0)
      ret_value = 1;
 ...
 *                          returned through the UDATA pointer, if *FOUND is
 *true. Failure:    Negative if not found.
 *
 *-------------------------------------------------------------------------
 */
static herr_t H5G__node_found(H5F_t *f, haddr_t addr,
   ...
   */
  if (NULL == (sn = (H5G_node_t *)H5AC_protect(f, H5AC_SNODE, addr, f,
  ...
  while (lt < rt && cmp) {
    ...
    if ((s = (const char *)H5HL_offset_into(udata->common.heap,
                                            sn->entry[idx].name_off)) == NULL)
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                  "unable to get symbol table name");
  ...
  if (sn && H5AC_unprotect(f, H5AC_SNODE, addr, sn, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_SYM, H5E_PROTECT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static H5B_ins_t H5G__node_insert(H5F_t *f, haddr_t addr,
                                  void H5_ATTR_UNUSED *_lt_key,
                                  hbool_t H5_ATTR_UNUSED *lt_key_changed,
  ...
  while (lt < rt) {
    ...
    if ((s = (const char *)H5HL_offset_into(udata->common.heap,
                                            sn->entry[idx].name_off)) == NULL)
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, H5B_INS_ERROR,
                  "unable to get symbol table name");
  ...
  if (H5G__ent_convert(f, udata->common.heap, udata->common.name, udata->lnk,
                       udata->obj_type, udata->crt_info, &ent) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTCONVERT, H5B_INS_ERROR,
                "unable to convert link");
  ...
  if (sn->nsyms >= 2 * H5F_SYM_LEAF_K(f)) {
    ...
    /* The right node */
    if (H5G__node_create(f, H5B_INS_FIRST, NULL, NULL, NULL,
                         new_node_p /*out*/) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, H5B_INS_ERROR,
                  "unable to split symbol table node");

    if (NULL == (snrt = (H5G_node_t *)H5AC_protect(f, H5AC_SNODE, *new_node_p,
                                                   f, H5AC__NO_FLAGS_SET)))
      HGOTO_ERROR(H5E_SYM, H5E_CANTLOAD, H5B_INS_ERROR,
                  "unable to split symbol table node");
    ...
    /* The left node */
    memset(sn->entry + H5F_SYM_LEAF_K(f), 0,
           H5F_SYM_LEAF_K(f) * sizeof(H5G_entry_t));
  ...
  if (snrt && H5AC_unprotect(f, H5AC_SNODE, *new_node_p, snrt, snrt_flags) < 0)
    HDONE_ERROR(H5E_SYM, H5E_PROTECT, H5B_INS_ERROR,
                "unable to release symbol table node");
...
static H5B_ins_t H5G__node_remove(
    H5F_t *f, haddr_t addr, void H5_ATTR_NDEBUG_UNUSED *_lt_key /*in,out*/,
  ...
  if (udata->common.name != NULL) {
    ...
    if (NULL == (lnk.name = (char *)H5HL_offset_into(udata->common.heap,
                                                     sn->entry[idx].name_off)))
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, H5B_INS_ERROR,
                  "unable to get link name");
    ...
    if (sn->entry[idx].type == H5G_CACHED_SLINK) {
      ...
      if (NULL ==
          (lnk.u.soft.name = (char *)H5HL_offset_into(
               udata->common.heap, sn->entry[idx].cache.slink.lval_offset)))
        HGOTO_ERROR(H5E_SYM, H5E_CANTGET, H5B_INS_ERROR,
                    "unable to get link name");
    } /* end if */
    else {
    ...
    if (lnk.type == H5L_TYPE_HARD) {
      ...
      if (lnk.u.soft.name) {
        ...
        if (H5HL_remove(f, udata->common.heap,
    ...
    if (H5HL_remove(f, udata->common.heap, sn->entry[idx].name_off,
                    link_name_len) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, H5B_INS_ERROR,
                  "unable to remove link name from local heap");
    ...
    if (1 == sn->nsyms) {
      ...
      sn->nsyms -= 1;
      sn_flags |= H5AC__DIRTIED_FLAG;
      memmove(sn->entry + idx, sn->entry + idx + 1,
  ...
  if (sn && H5AC_unprotect(f, H5AC_SNODE, addr, sn, sn_flags) < 0)
    HDONE_ERROR(H5E_SYM, H5E_CANTUNPROTECT, H5B_INS_ERROR,
                "unable to release symbol table node");
 ...
 *-------------------------------------------------------------------------
 */
int H5G__node_iterate(H5F_t *f, const void H5_ATTR_UNUSED *_lt_key,
                      haddr_t addr, const void H5_ATTR_UNUSED *_rt_key,
                      void *_udata) {
  ...
  if (NULL == (sn = (H5G_node_t *)H5AC_protect(f, H5AC_SNODE, addr, f,
                                               H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_SYM, H5E_CANTLOAD, H5_ITER_ERROR,
                "unable to load symbol table node");

  /*
   * Iterate over the symbol table node entries.
   */
  for (u = 0, ents = sn->entry; u < sn->nsyms && ret_value == H5_ITER_CONT;
       u++) {
    if (udata->skip > 0)
      --udata->skip;
    else {
      ...
      if ((name = (const char *)H5HL_offset_into(udata->heap,
                                                 ents[u].name_off)) == NULL)
        HGOTO_ERROR(H5E_SYM, H5E_CANTGET, H5_ITER_ERROR,
                    "unable to get symbol table node name");
      ...
      if (H5O_msg_reset(H5O_LINK_ID, &lnk) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTFREE, H5_ITER_ERROR,
                    "unable to release link message");
    } /* end else */
  ...
  /* Release resources */
  if (sn && H5AC_unprotect(f, H5AC_SNODE, addr, sn, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_SYM, H5E_PROTECT, H5_ITER_ERROR,
 ...
 *-------------------------------------------------------------------------
 */
int H5G__node_sumup(H5F_t *f, const void H5_ATTR_UNUSED *_lt_key, haddr_t addr,
 ...
 *-------------------------------------------------------------------------
 */
int H5G__node_by_idx(H5F_t *f, const void H5_ATTR_UNUSED *_lt_key, haddr_t addr,
                     const void H5_ATTR_UNUSED *_rt_key, void *_udata) {
   ...
   */
  if (udata->idx >= udata->num_objs &&
      udata->idx < (udata->num_objs + sn->nsyms)) {
...
herr_t H5G__node_init(H5F_t *f) {
  ...
  if (NULL == (shared = H5B_shared_new(f, H5B_SNODE, sizeof_rkey)))
    HGOTO_ERROR(H5E_BTREE, H5E_NOSPACE, FAIL,
  ...
  if (H5F_SET_GRP_BTREE_SHARED(f, H5UC_create(shared, H5B_shared_free)) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "can't create ref-count wrapper for shared B-tree info");
 ...
 *-------------------------------------------------------------------------
 */
int H5G__node_copy(H5F_t *f, const void H5_ATTR_UNUSED *_lt_key, haddr_t addr,
                   const void H5_ATTR_UNUSED *_rt_key, void *_udata) {
  ...
  if (NULL ==
      (heap = H5HL_protect(f, udata->src_heap_addr, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, H5_ITER_ERROR,
                "unable to protect symbol name");

  /* copy object in this node one by one */
  for (i = 0; i < sn->nsyms; i++) {
    H5G_entry_t *src_ent = &(sn->entry[i]); /* Convenience variable to refer to
                                               current source group entry */
    H5O_link_t lnk;                         /* Link to insert */
    const char *name;                       /* Name of source object */
    H5G_entry_t
        tmp_src_ent; /* Temporary copy. Change will not affect the cache */
    ...
    if (H5G_CACHED_SLINK == src_ent->type && cpy_info->expand_soft_link) {
      ...
      if ((link_name = (char *)H5HL_offset_into(
               heap, tmp_src_ent.cache.slink.lval_offset)) == NULL)
      ...
      if (H5G__loc_addr(&grp_loc, link_name, &obj_addr) >= 0) {
        ...
        H5E_clear_stack(
            NULL); /* discard any errors from a dangling soft link */
    ...
    if (H5_addr_defined(src_ent->header)) {
      ...
      if (H5O_copy_header_map(&tmp_src_oloc, &new_dst_oloc, cpy_info, TRUE,
                              &obj_type, (void **)&cpy_udata) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, H5_ITER_ERROR,
      ...
      /* Construct link information for eventual insertion */
      lnk.type = H5L_TYPE_SOFT;
      if ((lnk.u.soft.name = (char *)H5HL_offset_into(
               heap, src_ent->cache.slink.lval_offset)) == NULL)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, H5_ITER_ERROR,
                    "unable to get link name");
    } /* else if */
    else
    ...
    if ((name = (const char *)H5HL_offset_into(heap, src_ent->name_off)) ==
        NULL)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, H5_ITER_ERROR,
                  "unable to get source object name");

    /* Set copied metadata tag */
    H5_BEGIN_TAG(H5AC__COPIED_TAG)

    /* Insert the new object in the destination file's group */
    /* (Don't increment the link count - that's already done above for hard
     * links) */
    if (H5G__stab_insert_real(
            udata->dst_file, udata->dst_stab, name, &lnk, obj_type,
            (obj_type == H5O_TYPE_GROUP ? &gcrt_info : NULL)) < 0)
      HGOTO_ERROR_TAG(H5E_DATATYPE, H5E_CANTINIT, H5_ITER_ERROR,
                      "unable to insert the name");
  ...
  if (heap && H5HL_unprotect(heap) < 0)
    HDONE_ERROR(H5E_SYM, H5E_PROTECT, H5_ITER_ERROR,
                "unable to unprotect symbol name");
 ...
 *-------------------------------------------------------------------------
 */
int H5G__node_build_table(H5F_t *f, const void H5_ATTR_UNUSED *_lt_key,
                          haddr_t addr, const void H5_ATTR_UNUSED *_rt_key,
                          void *_udata) {
  ...
  for (u = 0; u < sn->nsyms; u++) {
    ...
    if ((name = (const char *)H5HL_offset_into(udata->heap,
                                               sn->entry[u].name_off)) == NULL)
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, H5_ITER_ERROR,
                  "unable to get symbol table link name");

    /* Determine the link to operate on in the table */
    linkno = udata->ltable->nlinks++;

    /* Convert the entry to a link */
    if (H5G__ent_to_link(&udata->ltable->lnks[linkno], udata->heap,
                         &sn->entry[u], name) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTCONVERT, H5_ITER_ERROR,
                  "unable to convert symbol table entry to link");
  } /* end for */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__node_iterate_size(H5F_t *f, const void H5_ATTR_UNUSED *_lt_key,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_node_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
  ...
  if (NULL == (sn = (H5G_node_t *)H5AC_protect(f, H5AC_SNODE, addr, f,
    ...
    fprintf(stream, "%*s%-*s %u of %u\n", indent, "", fwidth,
            "Number of Symbols:", sn->nsyms, (unsigned)(2 * H5F_SYM_LEAF_K(f)));
    ...
    for (u = 0; u < sn->nsyms; u++) {
      ...
      if (heap) {
        const char *s =
            (const char *)H5HL_offset_into(heap, sn->entry[u].name_off);
  ...
  if (heap && H5HL_unprotect(heap) < 0)
    HDONE_ERROR(H5E_SYM, H5E_PROTECT, FAIL,
                "unable to unprotect symbol table heap");
