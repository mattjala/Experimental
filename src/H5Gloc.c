...
herr_t H5G_loc_real(void *obj, H5I_type_t type, H5G_loc_t *loc) {
  ...
  case H5I_FILE: {
    ...
    if (H5G_root_loc(f, loc) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                  "unable to create location for file");
    break;
  }
  ...
  case H5I_DATATYPE: {
    ...
    if (NULL == (loc->oloc = H5T_oloc(dt)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "unable to get object location of datatype");
    if (NULL == (loc->path = H5T_nameof(dt)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "unable to get path of datatype");
    break;
  }
  ...
  case H5I_ATTR: {
    ...
    if (NULL == (loc->oloc = H5A_oloc(attr)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "unable to get object location of attribute");
    if (NULL == (loc->path = H5A_nameof(attr)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "unable to get path of attribute");
    break;
  }

  case H5I_DATASPACE:
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "unable to get group location of dataspace");
  ...
  case H5I_GENPROP_LST:
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "unable to get group location of property list");
  ...
  case H5I_VOL:
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "unable to get group location of a virtual object layer (VOL) "
                "connector");
  ...
  case H5I_EVENTSET:
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "unable to get group location of a event set");
...
herr_t H5G_loc(hid_t loc_id, H5G_loc_t *loc) {
  ...
  if (H5G_loc_real(obj, H5I_get_type(loc_id), loc) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "unable to fill in location struct");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_copy(H5G_loc_t *dst, const H5G_loc_t *src,
...
herr_t H5G_loc_free(H5G_loc_t *loc) {
  ...
  if (H5O_loc_free(loc->oloc) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTRELEASE, FAIL,
                "unable to free object header location");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_find(const H5G_loc_t *loc, const char *name,
...
static herr_t H5G__loc_find_by_idx_cb(H5G_loc_t H5_ATTR_UNUSED *grp_loc /*in*/,
  ...
  if (H5G__traverse_special(obj_loc, &fnd_lnk, H5G_TARGET_NORMAL, TRUE,
                            udata->loc, &obj_exists) < 0)
 ...
 *
 * Purpose:	Find a symbol from a location, according to the order in an
 *index
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_find_by_idx(const H5G_loc_t *loc, const char *group_name,
  ...
  /* Traverse group hierarchy to locate object */
  if (H5G_traverse(loc, group_name, H5G_TARGET_NORMAL, H5G__loc_find_by_idx_cb,
                   &udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't find object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__loc_insert(H5G_loc_t *grp_loc, char *name, H5G_loc_t *obj_loc,
  ...
  lnk.corder_valid =
      FALSE; /* Indicate that the creation order isn't valid (yet) */
  lnk.name = name;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_exists(const H5G_loc_t *loc, const char *name, hbool_t *exists) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Traverse group hierarchy to locate object */
  if (H5G_traverse(loc, name, H5G_TARGET_EXISTS, H5G__loc_exists_cb, exists) <
      0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__loc_addr(const H5G_loc_t *loc, const char *name,
                     haddr_t *addr /*out*/) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Purpose:     Retrieve the data model information for an object from a group
 *location and path to that object
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_info(const H5G_loc_t *loc, const char *name,
...
static herr_t H5G__loc_native_info_cb(H5G_loc_t H5_ATTR_UNUSED *grp_loc /*in*/,
  ...
  H5G_loc_native_info_t *udata =
      (H5G_loc_native_info_t *)_udata; /* User data passed in */
  herr_t ret_value = SUCCEED;          /* Return value */
 ...
 *
 * Purpose:	Retrieve the native information for an object from a group
 *location and path to that object
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_native_info(const H5G_loc_t *loc, const char *name,
  ...
  /* Traverse group hierarchy to locate object */
  if (H5G_traverse(loc, name, H5G_TARGET_NORMAL, H5G__loc_native_info_cb,
                   &udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't find object");
  ...
  if (udata->comment && *udata->comment) {
    if (NULL == (comment.s = HDstrdup(udata->comment)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                  "can't copy group comment");
    if (H5O_msg_create(obj_loc->oloc, H5O_NAME_ID, 0, H5O_UPDATE_TIME,
                       &comment) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_set_comment(const H5G_loc_t *loc, const char *name,
                           const char *comment) {
  ...
  /* Traverse group hierarchy to locate object */
  if (H5G_traverse(loc, name, H5G_TARGET_NORMAL, H5G__loc_set_comment_cb,
                   &udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't find object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_loc_get_comment(const H5G_loc_t *loc, const char *name,
  ...
  /* Traverse group hierarchy to locate object */
  if (H5G_traverse(loc, name, H5G_TARGET_NORMAL, H5G__loc_get_comment_cb,
                   &udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't find object");
