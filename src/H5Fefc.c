...
typedef struct H5F_efc_ent_t {
  ...
  unsigned nopen; /* Number of times this file is currently opened by an EFC
                     client */
} H5F_efc_ent_t;
...
struct H5F_efc_t {
  ...
  H5F_efc_ent_t
      *LRU_head; /* Head of LRU list.  This is the least recently used file */
  H5F_efc_ent_t
      *LRU_tail;   /* Tail of LRU list.  This is the most recently used file */
  ...
  H5F_shared_t
      *tmp_next; /* Next file in temporary list used by H5F__efc_try_close() */
};
 ...
 *-------------------------------------------------------------------------
 */
H5F_t *H5F__efc_open(H5F_efc_t *efc, const char *name, unsigned flags,
                     hid_t fcpl_id, hid_t fapl_id) {
  H5F_efc_ent_t *ent = NULL; /* Entry for target file in efc */
  hbool_t open_file =
      FALSE; /* Whether ent->file needs to be closed in case of error */
  ...
  if (ent) {
    ...
    if (H5SL_insert(efc->slist, ent, ent->name) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTINSERT, NULL,
                  "can't insert entry into skip list");
  ...
  if (efc->nfiles > 0) {
    ...
    if (efc->nfiles > 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTFREE, FAIL,
                  "can't destroy EFC after incomplete release");
  } /* end if */
...
herr_t H5F__efc_try_close(H5F_t *f) {
  H5F_shared_t
      *tail; /* Tail of linked list of found files.  Head will be f->shared. */
  H5F_shared_t *uncloseable_head =
      NULL; /* Head of linked list of files found to be uncloseable by the first
               pass */
  H5F_shared_t *uncloseable_tail =
      NULL; /* Tail of linked list of files found to be uncloseable by the first
               pass */
