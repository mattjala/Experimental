...
static void *H5C__load_entry(H5F_t *f,
                             ...
                             const H5C_class_t *type, haddr_t addr,
                             void *udata);
...
static herr_t H5C__deserialize_prefetched_entry(
    H5F_t *f, H5C_t *cache_ptr, H5C_cache_entry_t **entry_ptr_ptr,
...
static herr_t H5C__unpin_entry_from_client(H5C_t *cache_ptr,
  ...
  if (!entry_ptr->pinned_from_client)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTUNPIN, FAIL,
                "entry wasn't pinned by cache client");
...
static herr_t H5C__generate_image(H5F_t *f, H5C_t *cache_ptr,
  ...
  if ((entry_ptr->type->pre_serialize) &&
      ((entry_ptr->type->pre_serialize)(f, (void *)entry_ptr, entry_ptr->addr,
                                        entry_ptr->size, &new_addr, &new_len,
                                        &serialize_flags) < 0))
  ...
  if (serialize_flags != H5C__SERIALIZE_NO_FLAGS_SET) {
    ...
    if (serialize_flags & H5C__SERIALIZE_RESIZED_FLAG) {
      ...
      if (NULL == (entry_ptr->image_ptr = H5MM_realloc(
                       entry_ptr->image_ptr, new_len + H5C_IMAGE_EXTRA_SPACE)))
      ...
      H5C__UPDATE_INDEX_FOR_SIZE_CHANGE(cache_ptr, entry_ptr->size, new_len,
                                        entry_ptr, !entry_ptr->is_dirty, FAIL);
...
#if H5C_DO_MEMORY_SANITY_CHECKS
  assert(0 == memcmp(((uint8_t *)entry_ptr->image_ptr) + entry_ptr->size,
                     H5C_IMAGE_SANITY_VALUE, H5C_IMAGE_EXTRA_SPACE));
#endif /* H5C_DO_MEMORY_SANITY_CHECKS */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__flush_single_entry(H5F_t *f, H5C_cache_entry_t *entry_ptr,
                               unsigned flags) {
    ...
    if ((!suppress_image_entry_writes || !entry_ptr->include_in_image) &&
      ...
      if (cache_ptr->coll_write_list) {
        ...
        if (entry_ptr->prefetched) {
          ...
          mem_type =
              cache_ptr->class_table_ptr[entry_ptr->prefetch_type_id]->mem_type;
        } /* end if */
        else
          mem_type = entry_ptr->type->mem_type;

        if (H5F_block_write(f, mem_type, entry_ptr->addr, entry_ptr->size,
  ...
  if (destroy) {
     ...
     */
    if (entry_ptr->type->notify &&
        (entry_ptr->type->notify)(H5C_NOTIFY_ACTION_BEFORE_EVICT, entry_ptr) <
            0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
  ...
  if (destroy) {
    ...
    if (free_file_space) {
      ...
      if (H5MF_xfree(f, entry_ptr->type->mem_type, entry_ptr->addr, fsf_size) <
          0)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTFREE, FAIL,
  ...
  assert((ret_value != SUCCEED) || (destroy_entry) ||
         (!entry_ptr->flush_in_progress));
  assert((ret_value != SUCCEED) || (destroy_entry) || (take_ownership) ||
         (!entry_ptr->is_dirty));
...
static herr_t H5C__verify_len_eoa(H5F_t *f, const H5C_class_t *type,
                                  haddr_t addr, size_t *len, hbool_t actual) {
  H5FD_mem_t
      cooked_type; /* Modified type, accounting for switching global heaps */
...
void *H5C__load_entry(H5F_t *f,
                      ...
                      const H5C_class_t *type, haddr_t addr, void *udata) {
  hbool_t dirty =
      FALSE; /* Flag indicating whether thing was dirtied during deserialize */
  uint8_t *image = NULL; /* Buffer for disk image                    */
  void *thing = NULL;    /* Pointer to thing loaded                  */
  H5C_cache_entry_t *entry =
      NULL;   /* Alias for thing loaded, as cache entry   */
  ...
  /* Call the get_initial_load_size callback, to retrieve the initial size of
   * image */
  ...
  if (0 == (type->flags & H5C__CLASS_SKIP_READS)) {
    ...
    size_t actual_len =
        len; /* The actual length, after speculative reads have been resolved */
    ...
    do {
      ...
      if ((type->flags & H5C__CLASS_SPECULATIVE_LOAD_FLAG) && len_changed) {
        ...
        if (actual_len != len) {
          ...
          if (NULL == (new_image = H5MM_realloc(
                           image, actual_len + H5C_IMAGE_EXTRA_SPACE)))
            HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, NULL,
                        "image null after H5MM_realloc()");
          image = (uint8_t *)new_image;
#if H5C_DO_MEMORY_SANITY_CHECKS
          ...
          if (actual_len > len) {
            ...
            if (!coll_access || 0 == mpi_rank) {
               ...
               * loaded thing, go get the on-disk image again (the extra
               * portion).
               */
              if (H5F_block_read(f, type->mem_type, addr + len,
                ...
                if (coll_access) {
                  ...
                  HDONE_ERROR(H5E_CACHE, H5E_CANTLOAD, NULL,
                              "can't read image");
                } else
#endif
            ...
            if (coll_access) {
              ...
              if (MPI_SUCCESS != (mpi_code = MPI_Bcast(image + len, buf_size,
                                                       MPI_BYTE, 0, comm)))
                HMPI_GOTO_ERROR(NULL, "MPI_Bcast failed", mpi_code)
            } /* end if */
#endif        /* H5_HAVE_PARALLEL */
...
static herr_t H5C__mark_flush_dep_dirty(H5C_cache_entry_t *entry) {
  ...
  for (u = 0; u < entry->flush_dep_nparents; u++) {
    ...
    /* If the parent has a 'notify' callback, send a 'child entry dirtied'
     * notice */
    if (entry->flush_dep_parent[u]->type->notify &&
        (entry->flush_dep_parent[u]->type->notify)(
            H5C_NOTIFY_ACTION_CHILD_DIRTIED, entry->flush_dep_parent[u]) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
...
static herr_t H5C__mark_flush_dep_clean(H5C_cache_entry_t *entry) {
  ...
  for (i = ((int)entry->flush_dep_nparents) - 1; i >= 0; i--) {
    ...
    /* If the parent has a 'notify' callback, send a 'child entry cleaned'
     * notice */
    if (entry->flush_dep_parent[i]->type->notify &&
        (entry->flush_dep_parent[i]->type->notify)(
            H5C_NOTIFY_ACTION_CHILD_CLEANED, entry->flush_dep_parent[i]) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
...
static herr_t H5C__mark_flush_dep_serialized(H5C_cache_entry_t *entry_ptr) {
  ...
  for (i = ((int)entry_ptr->flush_dep_nparents) - 1; i >= 0; i--) {
    ...
    /* If the parent has a 'notify' callback, send a 'child entry serialized'
     * notice */
    if (entry_ptr->flush_dep_parent[i]->type->notify &&
        (entry_ptr->flush_dep_parent[i]->type->notify)(
...
static herr_t H5C__mark_flush_dep_unserialized(H5C_cache_entry_t *entry_ptr) {
  ...
  for (u = 0; u < entry_ptr->flush_dep_nparents; u++) {
    ...
    /* If the parent has a 'notify' callback, send a 'child entry unserialized'
     * notice */
    if (entry_ptr->flush_dep_parent[u]->type->notify &&
        (entry_ptr->flush_dep_parent[u]->type->notify)(
            H5C_NOTIFY_ACTION_CHILD_UNSERIALIZED,
            entry_ptr->flush_dep_parent[u]) < 0)
      HGOTO_ERROR(
          H5E_CACHE, H5E_CANTNOTIFY, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__serialize_single_entry(H5F_t *f, H5C_t *cache_ptr,
                                   H5C_cache_entry_t *entry_ptr) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (NULL == entry_ptr->image_ptr) {
    ...
    if (NULL == (entry_ptr->image_ptr =
      ...
      HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, FAIL,
                  "memory allocation failed for on disk image buffer");
#if H5C_DO_MEMORY_SANITY_CHECKS
    H5MM_memcpy(((uint8_t *)entry_ptr->image_ptr) + image_size,
                H5C_IMAGE_SANITY_VALUE, H5C_IMAGE_EXTRA_SPACE);
#endif /* H5C_DO_MEMORY_SANITY_CHECKS */
  }    /* end if */
  ...
  if (H5C__generate_image(f, cache_ptr, entry_ptr) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTSERIALIZE, FAIL,
                "Can't generate image for cache entry");
...
static herr_t H5C__deserialize_prefetched_entry(
  ...
  for (i = (int)(pf_entry_ptr->fd_parent_count) - 1; i >= 0; i--) {
    ...
    assert(pf_entry_ptr->flush_dep_parent[i]->addr ==
           pf_entry_ptr->fd_parent_addrs[i]);

    if (H5C_destroy_flush_dependency(pf_entry_ptr->flush_dep_parent[i],
                                     pf_entry_ptr) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTUNDEPEND, FAIL,
  ...
  if (pf_entry_ptr->fd_child_count > 0) {
    if (NULL == (fd_children = (H5C_cache_entry_t **)H5MM_calloc(
                     sizeof(H5C_cache_entry_t **) *
                     (size_t)(pf_entry_ptr->fd_child_count + 1))))
      HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, FAIL,
                  "memory allocation failed for fd child ptr array");

    if (H5C__destroy_pf_entry_child_flush_deps(cache_ptr, pf_entry_ptr,
                                               fd_children) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTUNDEPEND, FAIL,
   ...
   */
  if (NULL ==
      (thing = type->deserialize(pf_entry_ptr->image_ptr, len, udata, &dirty)))
  ...
  if (ds_entry_ptr->type->notify &&
      (ds_entry_ptr->type->notify)(H5C_NOTIFY_ACTION_AFTER_LOAD, ds_entry_ptr) <
          0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_insert_entry(H5F_t *f, const H5C_class_t *type, haddr_t addr,
...
#ifdef H5_HAVE_PARALLEL
  hbool_t coll_access =
      FALSE; /* whether access to the cache entry is done collectively */
#endif       /* H5_HAVE_PARALLEL */
...
herr_t H5C_mark_entry_clean(void *_thing) {
  ...
  if (entry_ptr->is_protected)
    ...
    /* Also reset the 'flush_marker' flag, since the entry shouldn't be flushed
     * now */
    entry_ptr->flush_marker = FALSE;
    ...
    if (was_dirty) {
      ...
      if (entry_ptr->flush_dep_nparents > 0)
        if (H5C__mark_flush_dep_clean(entry_ptr) < 0)
          HGOTO_ERROR(H5E_CACHE, H5E_CANTMARKCLEAN, FAIL,
                      "Can't propagate flush dep clean");
    } /* end if */
  }   /* end if */
  else
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_move_entry(H5C_t *cache_ptr, const H5C_class_t *type,
...
herr_t H5C_resize_entry(void *thing, size_t new_size) {
  ...
  if (!(entry_ptr->is_pinned || entry_ptr->is_protected))
    HGOTO_ERROR(H5E_CACHE, H5E_BADTYPE, FAIL,
                "Entry isn't pinned or protected??");
  ...
  if (entry_ptr->size != new_size) {
    ...
    if (cache_ptr->flash_size_increase_possible) {
      if (new_size > entry_ptr->size) {
        ...
        if (size_increase >= cache_ptr->flash_size_increase_threshold)
          if (H5C__flash_increase_cache_size(cache_ptr, entry_ptr->size,
                                             new_size) < 0)
            HGOTO_ERROR(H5E_CACHE, H5E_CANTRESIZE, FAIL,
                        "flash cache increase failed");
      }
    }
    ...
    if (entry_ptr->is_protected)
      H5C__DLL_UPDATE_FOR_SIZE_CHANGE(cache_ptr->pl_len, cache_ptr->pl_size,
                                      entry_ptr->size, new_size, FAIL)
...
herr_t H5C_pin_protected_entry(void *thing) {
  H5C_t *cache_ptr;
  H5C_cache_entry_t *entry_ptr =
      (H5C_cache_entry_t *)thing; /* Pointer to entry to pin */
  herr_t ret_value = SUCCEED;     /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
void *H5C_protect(H5F_t *f, const H5C_class_t *type, haddr_t addr, void *udata,
                  unsigned flags) {
  H5C_t *cache_ptr;
  H5AC_ring_t ring = H5C_RING_UNDEFINED;
  hbool_t hit;
  ...
  hbool_t was_loaded =
      FALSE; /* Whether the entry was loaded as a result of the protect */
  size_t empty_space;
  void *thing;
  H5C_cache_entry_t *entry_ptr;
  void *ret_value = NULL; /* Return value */
  ...
  if (entry_ptr != NULL) {
    ...
    if (entry_ptr->prefetched) {
       ...
       */
      if (H5C__deserialize_prefetched_entry(f, cache_ptr, &entry_ptr, type,
                                            addr, udata) < 0)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTLOAD, NULL,
    ...
    if (coll_access) {
      if (!entry_ptr->is_dirty && !entry_ptr->coll_access) {
        ...
        if (entry_ptr->image_ptr == NULL) {
          ...
          if (NULL == (entry_ptr->image_ptr = H5MM_malloc(
                           entry_ptr->size + H5C_IMAGE_EXTRA_SPACE)))
            HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, NULL,
                        "memory allocation failed for on disk image buffer");
#if H5C_DO_MEMORY_SANITY_CHECKS
          H5MM_memcpy(((uint8_t *)entry_ptr->image_ptr) + entry_ptr->size,
          ...
          if (0 == mpi_rank && H5C__generate_image(f, cache_ptr, entry_ptr) < 0)
             ...
             */
            HDONE_ERROR(H5E_CACHE, H5E_CANTGET, NULL,
                        "can't generate entry's image");
        } /* end if */
        assert(entry_ptr->image_ptr);
  ...
  if (H5C__validate_protected_entry_list(cache_ptr) < 0 ||
      H5C__validate_pinned_entry_list(cache_ptr) < 0 ||
      H5C__validate_lru_list(cache_ptr) < 0)
    HDONE_ERROR(H5E_CACHE, H5E_SYSTEM, NULL,
                "an extreme sanity check failed on exit");
#endif /* H5C_DO_EXTREME_SANITY_CHECKS */
...
herr_t H5C_unpin_entry(void *_entry_ptr) {
  H5C_t *cache_ptr;
  H5C_cache_entry_t *entry_ptr =
      (H5C_cache_entry_t *)_entry_ptr; /* Pointer to entry to unpin */
  herr_t ret_value = SUCCEED;          /* Return value */
  ...
  if (entry_ptr->ro_ref_count > 1) {
    ...
    if (deleted) {
      ...
      assert((!cache_ptr->slist_enabled) ||
             (((!was_clean) || dirtied) == (entry_ptr->in_slist)));
...
herr_t H5C_unsettle_entry_ring(void *_entry) {
  H5C_cache_entry_t *entry =
      (H5C_cache_entry_t *)_entry; /* Entry whose ring to unsettle */
...
herr_t H5C_create_flush_dependency(void *parent_thing, void *child_thing) {
  H5C_t *cache_ptr;
  H5C_cache_entry_t *parent_entry =
      (H5C_cache_entry_t *)parent_thing; /* Ptr to parent thing's entry */
  H5C_cache_entry_t *child_entry =
      (H5C_cache_entry_t *)child_thing; /* Ptr to child thing's entry */
  herr_t ret_value = SUCCEED;           /* Return value */
  ...
  if (!(parent_entry->is_protected || parent_entry->is_pinned))
    HGOTO_ERROR(H5E_CACHE, H5E_CANTDEPEND, FAIL,
                "Parent entry isn't pinned or protected");
  ...
  if (child_entry->flush_dep_nparents >= child_entry->flush_dep_parent_nalloc) {
    if (child_entry->flush_dep_parent_nalloc == 0) {
      ...
      if (NULL == (child_entry->flush_dep_parent = H5FL_SEQ_MALLOC(
                       H5C_cache_entry_ptr_t, H5C_FLUSH_DEP_PARENT_INIT)))
        HGOTO_ERROR(
      ...
      if (NULL == (child_entry->flush_dep_parent = H5FL_SEQ_REALLOC(
                       H5C_cache_entry_ptr_t, child_entry->flush_dep_parent,
  ...
  if (child_entry->is_dirty) {
    /* Sanity check */
    assert(parent_entry->flush_dep_ndirty_children <
           parent_entry->flush_dep_nchildren);
    ...
    if (parent_entry->type->notify &&
        (parent_entry->type->notify)(H5C_NOTIFY_ACTION_CHILD_DIRTIED,
                                     parent_entry) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
                  "can't notify parent about child entry dirty flag set");
  } /* end if */
  ...
  if (!child_entry->image_up_to_date) {
    assert(parent_entry->flush_dep_nunser_children <
           parent_entry->flush_dep_nchildren);
     ...
     * notice */
    if (parent_entry->type->notify &&
        (parent_entry->type->notify)(H5C_NOTIFY_ACTION_CHILD_UNSERIALIZED,
                                     parent_entry) < 0)
      HGOTO_ERROR(
          H5E_CACHE, H5E_CANTNOTIFY, FAIL,
          "can't notify parent about child entry serialized flag reset");
...
herr_t H5C_destroy_flush_dependency(void *parent_thing, void *child_thing) {
  H5C_t *cache_ptr;
  H5C_cache_entry_t *parent_entry =
      (H5C_cache_entry_t *)parent_thing; /* Ptr to parent entry */
  H5C_cache_entry_t *child_entry =
      (H5C_cache_entry_t *)child_thing; /* Ptr to child entry */
  unsigned u;                           /* Local index variable */
  herr_t ret_value = SUCCEED;           /* Return value */
  ...
  if (0 == parent_entry->flush_dep_nchildren)
    HGOTO_ERROR(
        H5E_CACHE, H5E_CANTUNDEPEND, FAIL,
  ...
  if (u < (child_entry->flush_dep_nparents - 1))
    memmove(&child_entry->flush_dep_parent[u],
            ...
            (child_entry->flush_dep_nparents - u - 1) *
  ...
  if (child_entry->is_dirty) {
     ...
     * notice */
    if (parent_entry->type->notify &&
        (parent_entry->type->notify)(H5C_NOTIFY_ACTION_CHILD_CLEANED,
                                     parent_entry) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
                  "can't notify parent about child entry dirty flag reset");
  } /* end if */
  ...
  if (!child_entry->image_up_to_date) {
     ...
     * notice */
    if (parent_entry->type->notify &&
        (parent_entry->type->notify)(H5C_NOTIFY_ACTION_CHILD_SERIALIZED,
                                     parent_entry) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
                  "can't notify parent about child entry serialized flag set");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_expunge_entry(H5F_t *f, const H5C_class_t *type, haddr_t addr,
                         unsigned flags) {
  H5C_t *cache_ptr;
  H5C_cache_entry_t *entry_ptr = NULL;
  unsigned flush_flags =
...
herr_t H5C_remove_entry(void *_entry) {
  ...
  if (entry->is_dirty)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTREMOVE, FAIL,
                "can't remove dirty entry from cache");
  if (entry->is_protected)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTREMOVE, FAIL,
                "can't remove protected entry from cache");
  if (entry->is_pinned)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTREMOVE, FAIL,
                "can't remove pinned entry from cache");
  ...
  if (entry->type->notify &&
      (entry->type->notify)(H5C_NOTIFY_ACTION_BEFORE_EVICT, entry) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTNOTIFY, FAIL,
                "can't notify client about entry to evict");
