...
static herr_t H5R__encode_obj_token(const H5O_token_t *obj_token,
                                    size_t token_size, unsigned char *buf,
                                    size_t *nalloc);
static herr_t H5R__decode_obj_token(const unsigned char *buf, size_t *nbytes,
                                    H5O_token_t *obj_token,
                                    uint8_t *token_size);
static herr_t H5R__encode_region(H5S_t *space, unsigned char *buf,
                                 size_t *nalloc);
static herr_t H5R__decode_region(const unsigned char *buf, size_t *nbytes,
                                 H5S_t **space_ptr);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__create_object(const H5O_token_t *obj_token, size_t token_size,
                          H5R_ref_priv_t *ref) {
  size_t encode_size;
  ...
  if (H5R__encode(NULL, ref, NULL, &encode_size, 0) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTENCODE, FAIL,
                "unable to determine encoding size");
  ref->encode_size = (uint32_t)encode_size;

  H5R_LOG_DEBUG(
      "Created object reference, %d, filename=%s, obj_addr=%s, encode size=%u",
      (int)sizeof(H5R_ref_priv_t), ref->info.obj.filename,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__create_region(const H5O_token_t *obj_token, size_t token_size,
  ...
  H5R_LOG_DEBUG(
      "Created region reference, %d, filename=%s, obj_addr=%s, encode size=%u",
      (int)sizeof(H5R_ref_priv_t), ref->info.obj.filename,
      H5R__print_token(ref->info.obj.token), ref->encode_size);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__create_attr(const H5O_token_t *obj_token, size_t token_size,
  ...
  if (HDstrlen(attr_name) > H5R_MAX_STRING_LEN)
    HGOTO_ERROR(H5E_REFERENCE, H5E_ARGS, FAIL,
                "attribute name too long (%d > %d)", (int)HDstrlen(attr_name),
                H5R_MAX_STRING_LEN);
  ...
  H5R_LOG_DEBUG("Created attribute reference, %d, filename=%s, obj_addr=%s, "
                "attr name=%s, encode size=%u",
                (int)sizeof(H5R_ref_priv_t), ref->info.obj.filename,
                H5R__print_token(ref->info.obj.token), ref->info.attr.name,
                ref->encode_size);
...
herr_t H5R__destroy(H5R_ref_priv_t *ref) {
  ...
  H5R_LOG_DEBUG(
      "Destroying reference, filename=%s, obj_addr=%s, encode size=%u",
  ...
  case H5R_MAXTYPE:
    ...
    HGOTO_ERROR(H5E_REFERENCE, H5E_UNSUPPORTED, FAIL,
                "internal error (invalid reference type)");
  ...
  if (ref->type && (ref->loc_id != H5I_INVALID_HID)) {
    if (ref->app_ref) {
      if (H5I_dec_app_ref(ref->loc_id) < 0)
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDEC, FAIL,
                    "decrementing location ID failed");
    } else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__set_loc_id(H5R_ref_priv_t *ref, hid_t id, hbool_t inc_ref,
                       hbool_t app_ref) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (inc_ref && H5I_inc_ref(ref->loc_id, app_ref) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTINC, FAIL,
                "incrementing location ID failed");
  ref->app_ref = app_ref;
...
hid_t H5R__reopen_file(H5R_ref_priv_t *ref, hid_t fapl_id) {
  ...
  if (NULL == (new_file = H5VL_file_open(
                   &connector_prop, H5R_REF_FILENAME(ref), H5F_ACC_RDWR,
...
htri_t H5R__equal(const H5R_ref_priv_t *ref1, const H5R_ref_priv_t *ref2) {
  ...
  if (ref1->token_size != ref2->token_size)
    HGOTO_DONE(FALSE);
  if (0 !=
      memcmp(&ref1->info.obj.token, &ref2->info.obj.token, ref1->token_size))
    HGOTO_DONE(FALSE);
  ...
  case H5R_DATASET_REGION2:
    if ((ret_value =
             H5S_extent_equal(ref1->info.reg.space, ref2->info.reg.space)) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCOMPARE, FAIL,
                  "cannot compare dataspace extents");
    break;
  case H5R_ATTR:
...
herr_t H5R__copy(const H5R_ref_priv_t *src_ref, H5R_ref_priv_t *dst_ref) {
  ...
  H5MM_memcpy(&dst_ref->info.obj.token, &src_ref->info.obj.token,
              sizeof(H5O_token_t));
  ...
  case H5R_OBJECT2:
    break;
  case H5R_DATASET_REGION2:
    if (NULL == (dst_ref->info.reg.space =
                     H5S_copy(src_ref->info.reg.space, FALSE, TRUE)))
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTCOPY, FAIL,
  ...
  if (src_ref->loc_id == H5I_INVALID_HID) {
    ...
    if (NULL ==
        (dst_ref->info.obj.filename = HDstrdup(src_ref->info.obj.filename)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__get_obj_token(const H5R_ref_priv_t *ref, H5O_token_t *obj_token,
                          size_t *token_size) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__set_obj_token(H5R_ref_priv_t *ref, const H5O_token_t *obj_token,
                          size_t token_size) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__encode(const char *filename, const H5R_ref_priv_t *ref,
                   unsigned char *buf, size_t *nalloc, unsigned flags) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__decode(const unsigned char *buf, size_t *nbytes,
                   H5R_ref_priv_t *ref) {
  ...
  if (buf_size < H5R_ENCODE_HEADER_SIZE)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, FAIL,
...
static herr_t H5R__decode_obj_token(const unsigned char *buf, size_t *nbytes,
  ...
  *token_size = *p++;
  if (*token_size > sizeof(H5O_token_t))
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, FAIL, "Invalid token size (%u)",
                *token_size);
...
static herr_t H5R__encode_region(H5S_t *space, unsigned char *buf,
  ...
  if ((buf_size = H5S_SELECT_SERIAL_SIZE(space)) < 0)
    HGOTO_ERROR(
        H5E_REFERENCE, H5E_CANTENCODE, FAIL,
...
static herr_t H5R__decode_region(const unsigned char *buf, size_t *nbytes,
  ...
  if (H5S_set_extent_simple(space, rank, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTSET, FAIL,
                "can't set extent rank for selection");

  if (p - 1 > p_end)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, FAIL,
                "Ran off end of buffer while decoding");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__encode_heap(H5F_t *f, unsigned char *buf, size_t *nalloc,
  ...
  if (buf && *nalloc >= buf_size) {
    ...
    if (H5HG_insert(f, data_size, data, &hobjid) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_WRITEERROR, FAIL,
                  "Unable to write reference information");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__decode_heap(H5F_t *f, const unsigned char *buf, size_t *nbytes,
  ...
  if (NULL == (*data_ptr = (unsigned char *)H5HG_read(
                   f, &hobjid, (void *)*data_ptr, data_size)))
    HGOTO_ERROR(H5E_REFERENCE, H5E_READERROR, FAIL,
                "Unable to read reference data");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__decode_token_obj_compat(const unsigned char *buf, size_t *nbytes,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5R__decode_token_region_compat(H5F_t *f, const unsigned char *buf,
                                       size_t *nbytes, H5O_token_t *obj_token,
  ...
  if (space_ptr) {
    ...
    if (p - 1 >= p_end)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, FAIL,
                  "Ran off end of buffer while deserializing");
