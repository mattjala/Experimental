...
/* Local heap prefix */
static herr_t H5HL__cache_prefix_get_initial_load_size(void *udata,
                                                       size_t *image_len);
...
static herr_t H5HL__cache_datablock_get_initial_load_size(void *udata,
                                                          size_t *image_len);
...
static herr_t H5HL__cache_datablock_serialize(const H5F_t *f, void *image,
                                              size_t len, void *thing);
static herr_t H5HL__cache_datablock_notify(H5C_notify_action_t action,
                                           void *_thing);
...
static herr_t H5HL__hdr_deserialize(H5HL_t *heap, const uint8_t *image,
                                    size_t len, H5HL_cache_prfx_ud_t *udata);
...
const H5AC_class_t H5AC_LHEAP_PRFX[1] = {{
    ...
    H5HL__cache_prefix_get_initial_load_size, /* 'get_initial_load_size'
                                                 callback */
...
const H5AC_class_t H5AC_LHEAP_DBLK[1] = {{
    ...
    H5HL__cache_datablock_get_initial_load_size, /* 'get_initial_load_size'
                                                    callback */
 ...
 */
static herr_t
H5HL__cache_prefix_get_initial_load_size(void H5_ATTR_UNUSED *_udata,
                                         size_t *image_len) {
  FUNC_ENTER_PACKAGE_NOERR
...
static herr_t H5HL__cache_prefix_get_final_load_size(const void *_image,
  ...
  H5HL_cache_prfx_ud_t *udata =
      (H5HL_cache_prfx_ud_t *)_udata; /* User data for callback */
  ...
  if (H5HL__hdr_deserialize(&heap, (const uint8_t *)image, image_len, udata) <
      0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTDECODE, FAIL,
                "can't decode local heap header");
 ...
 *-------------------------------------------------------------------------
 */
static void *H5HL__cache_prefix_deserialize(const void *_image, size_t len,
                                            void *_udata,
                                            hbool_t H5_ATTR_UNUSED *dirty) {
  H5HL_t *heap = NULL;      /* Local heap */
  H5HL_prfx_t *prfx = NULL; /* Heap prefix deserialized */
  const uint8_t *image =
      (const uint8_t *)_image;            /* Pointer into decoding buffer */
  const uint8_t *p_end = image + len - 1; /* End of image buffer */
  H5HL_cache_prfx_ud_t *udata =
      (H5HL_cache_prfx_ud_t *)_udata; /* User data for callback */
  ...
  if (NULL == (heap = H5HL__new(udata->sizeof_size, udata->sizeof_addr,
                                udata->sizeof_prfx)))
  ...
  if (heap->dblk_size) {
    ...
    if (H5_addr_eq((heap->prfx_addr + heap->prfx_size), heap->dblk_addr)) {
      ...
      if (NULL ==
          (heap->dblk_image = H5FL_BLK_MALLOC(lheap_chunk, heap->dblk_size)))
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HL__cache_prefix_image_len(const void *_thing,
                                           size_t *image_len) {
  const H5HL_prfx_t *prfx =
      (const H5HL_prfx_t *)_thing; /* Pointer to local heap prefix to query */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HL__cache_prefix_serialize(const H5_ATTR_NDEBUG_UNUSED H5F_t *f,
                                           void *_image,
                                           size_t H5_ATTR_NDEBUG_UNUSED len,
                                           void *_thing) {
  H5HL_prfx_t *prfx =
      (H5HL_prfx_t *)_thing; /* Pointer to local heap prefix to query */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HL__cache_datablock_get_initial_load_size(void *_udata,
                                                          size_t *image_len) {
 ...
 *		heap data block, deserialize it, load its contents into a newly
 *allocated instance of H5HL_dblk_t, and return a pointer to the new instance.
 *
 * Return:      Success:        Pointer to in core representation
 *              Failure:        NULL
 *
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HL__cache_datablock_image_len(const void *_thing,
                                              size_t *image_len) {
  const H5HL_dblk_t *dblk =
      (const H5HL_dblk_t *)_thing; /* Pointer to the local heap data block */
 ...
 */
static herr_t
H5HL__cache_datablock_serialize(const H5F_t H5_ATTR_NDEBUG_UNUSED *f,
                                void *image, size_t H5_ATTR_NDEBUG_UNUSED len,
                                void *_thing) {
  H5HL_t *heap; /* Pointer to the local heap */
  H5HL_dblk_t *dblk =
      (H5HL_dblk_t *)_thing; /* Pointer to the local heap data block */
  ...
  case H5AC_NOTIFY_ACTION_AFTER_LOAD:
    ...
    if (FAIL == H5AC_pin_protected_entry(dblk->heap->prfx))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTPIN, FAIL,
                  "unable to pin local heap prefix");
    break;
  ...
  case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
    ...
    if (FAIL == H5AC_unpin_entry(dblk->heap->prfx))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPIN, FAIL,
                  "unable to unpin local heap prefix");
    break;
 ...
 *
 * Purpose:	Free the in memory representation of the supplied local heap
 *data block.
 *
 * Return:      Success:        SUCCEED
 *              Failure:        FAIL
