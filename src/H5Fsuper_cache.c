 ...
 *
 * Purpose:		Implement file superblock & driver info metadata cache
 *methods
 *
 *-------------------------------------------------------------------------
 */
...
static herr_t H5F__cache_superblock_get_initial_load_size(void *udata,
                                                          size_t *image_len);
static herr_t H5F__cache_superblock_get_final_load_size(const void *image_ptr,
                                                        size_t image_len,
                                                        void *udata,
                                                        size_t *actual_len);
static htri_t H5F__cache_superblock_verify_chksum(const void *image_ptr,
...
static herr_t H5F__cache_superblock_image_len(const void *thing,
                                              size_t *image_len);
static herr_t H5F__cache_superblock_serialize(const H5F_t *f, void *image,
                                              size_t len, void *thing);
static herr_t H5F__cache_superblock_free_icr(void *thing);

static herr_t H5F__cache_drvrinfo_get_initial_load_size(void *udata,
                                                        size_t *image_len);
...
static herr_t H5F__cache_drvrinfo_image_len(const void *thing,
                                            size_t *image_len);
static herr_t H5F__cache_drvrinfo_serialize(const H5F_t *f, void *image,
                                            size_t len, void *thing);
static herr_t H5F__cache_drvrinfo_free_icr(void *thing);

/* Local encode/decode routines */
static herr_t H5F__superblock_prefix_decode(
    H5F_super_t *sblock, const uint8_t **image_ref, size_t len,
    const H5F_superblock_cache_ud_t *udata, hbool_t extend_eoa);
static herr_t H5F__drvrinfo_prefix_decode(H5O_drvinfo_t *drvinfo,
                                          char *drv_name,
                                          const uint8_t **image_ref, size_t len,
                                          H5F_drvrinfo_cache_ud_t *udata,
                                          hbool_t extend_eoa);
...
const H5AC_class_t H5AC_SUPERBLOCK[1] = {{
    ...
    H5F__cache_superblock_get_initial_load_size, /* 'get_initial_load_size'
                                                    callback */
...
const H5AC_class_t H5AC_DRVRINFO[1] = {{
    ...
    H5FD_MEM_SUPER,      /* File space memory type for client */
    H5AC__CLASS_SPECULATIVE_LOAD_FLAG,         /* Client class behavior flags */
    H5F__cache_drvrinfo_get_initial_load_size, /* 'get_initial_load_size'
                                                  callback */
...
static herr_t H5F__superblock_prefix_decode(
    ...
    const H5F_superblock_cache_ud_t *udata, hbool_t extend_eoa) {
  const uint8_t *image =
      (const uint8_t *)*image_ref;      /* Pointer into raw data buffer */
  ...
  if (H5_IS_BUFFER_OVERFLOW(image, H5F_SIGNATURE_LEN, end))
    HGOTO_ERROR(H5E_FILE, H5E_OVERFLOW, FAIL,
  ...
  /* Size check */
  if (((size_t)(image - (const uint8_t *)*image_ref)) !=
  ...
  if (sblock->sizeof_addr != 2 && sblock->sizeof_addr != 4 &&
      sblock->sizeof_addr != 8 && sblock->sizeof_addr != 16 &&
      sblock->sizeof_addr != 32)
    HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL, "bad byte number in an address");
  if (sblock->sizeof_size != 2 && sblock->sizeof_size != 4 &&
      sblock->sizeof_size != 8 && sblock->sizeof_size != 16 &&
      sblock->sizeof_size != 32)
  ...
  if (extend_eoa) {
    ...
    variable_size = (size_t)H5F_SUPERBLOCK_VARLEN_SIZE(
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__drvrinfo_prefix_decode(H5O_drvinfo_t *drvrinfo,
                                          char *drv_name,
                                          const uint8_t **image_ref, size_t len,
                                          H5F_drvrinfo_cache_ud_t *udata,
                                          hbool_t extend_eoa) {
  ...
  /* Extend the EOA if required so that we can read the complete driver info
   * block */
  if (extend_eoa) {
 ...
 */
static herr_t
H5F__cache_superblock_get_initial_load_size(void H5_ATTR_UNUSED *_udata,
                                            size_t *image_len) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__cache_superblock_get_final_load_size(const void *_image,
                                                        size_t image_len,
                                                        void *_udata,
                                                        size_t *actual_len) {
  const uint8_t *image = _image; /* Pointer into raw data buffer */
  H5F_superblock_cache_ud_t *udata =
      (H5F_superblock_cache_ud_t *)_udata; /* User data */
  ...
  if (H5F__superblock_prefix_decode(&sblock, &image, image_len, udata, TRUE) <
      0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTDECODE, FAIL,
                "can't decode file superblock prefix");
  ...
  *actual_len = H5F_SUPERBLOCK_FIXED_SIZE +
                (size_t)H5F_SUPERBLOCK_VARLEN_SIZE(
                    sblock.super_vers, sblock.sizeof_addr, sblock.sizeof_size);
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5F__cache_superblock_verify_chksum(const void *_image,
 ...
 *-------------------------------------------------------------------------
 */
static void *H5F__cache_superblock_deserialize(const void *_image, size_t len,
                                               void *_udata,
  ...
  if (sblock->super_vers < HDF5_SUPERBLOCK_VERSION_2) {
    ...
    if (H5_IS_BUFFER_OVERFLOW(image, 1, end))
      HGOTO_ERROR(H5E_FILE, H5E_OVERFLOW, NULL,
                  "image pointer is out of bounds");
    ...
    if (H5_IS_BUFFER_OVERFLOW(image, 1, end))
      HGOTO_ERROR(H5E_FILE, H5E_OVERFLOW, NULL,
                  "image pointer is out of bounds");
    image++;
    udata->f->shared->sizeof_addr =
        sblock->sizeof_addr; /* Keep a local copy also */
    ...
    if (H5_IS_BUFFER_OVERFLOW(image, 1, end))
      HGOTO_ERROR(H5E_FILE, H5E_OVERFLOW, NULL,
                  "image pointer is out of bounds");
    image++;
    udata->f->shared->sizeof_size =
        sblock->sizeof_size; /* Keep a local copy also */
    ...
    if (H5_IS_BUFFER_OVERFLOW(image, H5F_sizeof_addr(udata->f) * 4, end))
      HGOTO_ERROR(H5E_FILE, H5E_OVERFLOW, NULL,
                  "image pointer is out of bounds");
    H5F_addr_decode(udata->f, (const uint8_t **)&image,
                    &sblock->base_addr /*out*/);
    H5F_addr_decode(udata->f, (const uint8_t **)&image,
                    &sblock->ext_addr /*out*/);
    H5F_addr_decode(udata->f, (const uint8_t **)&image,
                    &udata->stored_eof /*out*/);
    H5F_addr_decode(udata->f, (const uint8_t **)&image,
    ...
    if (sblock->root_ent)
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, NULL,
                  "root entry should not exist yet");
    if (NULL ==
        (sblock->root_ent = (H5G_entry_t *)H5MM_calloc(sizeof(H5G_entry_t))))
      HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, NULL,
                  "can't allocate space for root group symbol table entry");

    /* Decode the root group symbol table entry */
    if (H5G_ent_decode(udata->f, (const uint8_t **)&image, sblock->root_ent,
                       end) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTDECODE, NULL,
    ...
    /* Base, superblock extension, end of file & root group object header
     * addresses */
    H5F_addr_decode(udata->f, (const uint8_t **)&image,
                    &sblock->base_addr /*out*/);
    H5F_addr_decode(udata->f, (const uint8_t **)&image,
  ...
  if (!ret_value && sblock)
    if (H5F__super_free(sblock) < 0)
      HDONE_ERROR(H5E_FILE, H5E_CANTFREE, NULL,
                  "unable to destroy superblock data");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__cache_superblock_image_len(const void *_thing,
                                              size_t *image_len) {
  const H5F_super_t *sblock =
      (const H5F_super_t *)_thing; /* Pointer to the object */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__cache_superblock_serialize(const H5F_t *f, void *_image,
  ...
  if (sblock->super_vers < HDF5_SUPERBLOCK_VERSION_2) {
    ...
    *image++ = 0; /* reserved */
    ...
    if (H5G_ent_encode(f, &image, sblock->root_ent) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTENCODE, FAIL,
                  "can't encode root group symbol table entry");
    ...
    *image++ = sblock->status_flags;
    ...
    if (NULL == (root_oloc = H5G_oloc(f->shared->root_grp)))
      HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
                  "unable to retrieve root group information");
    ...
    chksum = H5_checksum_metadata(
        _image, ((size_t)H5F_SUPERBLOCK_SIZE(sblock) - H5F_SIZEOF_CHKSUM), 0);
    ...
    assert((size_t)(image - (uint8_t *)_image) ==
           (size_t)H5F_SUPERBLOCK_SIZE(sblock));
  }
 ...
 */
static herr_t
H5F__cache_drvrinfo_get_initial_load_size(void H5_ATTR_UNUSED *_udata,
                                          size_t *image_len) {
  FUNC_ENTER_PACKAGE_NOERR
  ...
  /* Set the initial image length size */
  *image_len =
      H5F_DRVINFOBLOCK_HDR_SIZE; /* Fixed size portion of driver info block */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__cache_drvrinfo_get_final_load_size(const void *_image,
                                                      size_t image_len,
                                                      void *_udata,
                                                      size_t *actual_len) {
  const uint8_t *image = _image; /* Pointer into raw data buffer */
  H5F_drvrinfo_cache_ud_t *udata =
      (H5F_drvrinfo_cache_ud_t *)_udata; /* User data */
  ...
  if (H5F__drvrinfo_prefix_decode(&drvrinfo, NULL, &image, image_len, udata,
                                  TRUE) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTDECODE, FAIL,
                "can't decode file driver info prefix");
 ...
 *-------------------------------------------------------------------------
 */
static void *H5F__cache_drvrinfo_deserialize(const void *_image, size_t len,
                                             void *_udata,
                                             hbool_t H5_ATTR_UNUSED *dirty) {
  ...
  /* Deserialize the file driver info's prefix */
  if (H5F__drvrinfo_prefix_decode(drvinfo, drv_name, &image, len, udata,
                                  FALSE) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTDECODE, NULL,
                "can't decode file driver info prefix");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__cache_drvrinfo_image_len(const void *_thing,
                                            size_t *image_len) {
  const H5O_drvinfo_t *drvinfo =
      (const H5O_drvinfo_t *)_thing; /* Pointer to the object */
  ...
  /* Set the image length size */
  *image_len =
      (size_t)(H5F_DRVINFOBLOCK_HDR_SIZE + /* Fixed-size portion of driver info
                                              block */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__cache_drvrinfo_serialize(const H5F_t *f, void *_image,
  ...
  *image++ = 0;                         /* reserved */
  ...
  if (H5FD_sb_encode(f->shared->lf, (char *)image,
                     dbuf + H5F_DRVINFOBLOCK_HDR_SIZE) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
