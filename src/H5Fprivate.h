...
#define H5F_SET_GRP_BTREE_SHARED(F, RC)                                        \
  (((F)->shared->grp_btree_shared = (RC)) ? SUCCEED : FAIL)
...
#define H5F_ENCODE_LENGTH(f, p, l)                                             \
  H5_ENCODE_LENGTH_LEN(p, l, H5F_SIZEOF_SIZE(f))
#define H5F_DECODE_LENGTH(f, p, l)                                             \
  H5_DECODE_LENGTH_LEN(p, l, H5F_SIZEOF_SIZE(f))
...
#define H5F_CRT_USER_BLOCK_NAME                                                \
  "block_size" /* Size of the file user block in bytes */
#define H5F_CRT_SYM_LEAF_NAME                                                  \
  "symbol_leaf" /* 1/2 rank for symbol table leaf nodes */
#define H5F_CRT_SYM_LEAF_DEF 4
#define H5F_CRT_BTREE_RANK_NAME                                                \
  "btree_rank" /* 1/2 rank for btree internal nodes    */
#define H5F_CRT_ADDR_BYTE_NUM_NAME                                             \
  "addr_byte_num" /* Byte number in an address            */
#define H5F_CRT_OBJ_BYTE_NUM_NAME                                              \
  "obj_byte_num" /* Byte number for object size          */
#define H5F_CRT_SUPER_VERS_NAME                                                \
  "super_version" /* Version number of the superblock     */
/* Number of shared object header message indexes */
#define H5F_CRT_SHMSG_NINDEXES_NAME "num_shmsg_indexes"
#define H5F_CRT_SHMSG_INDEX_TYPES_NAME                                         \
  "shmsg_message_types" /* Types of message in each index */
/* Minimum size of messages in each index */
#define H5F_CRT_SHMSG_INDEX_MINSIZE_NAME "shmsg_message_minsize"
#define H5F_CRT_SHMSG_LIST_MAX_NAME                                            \
  "shmsg_list_max" /* Shared message list maximum size */
#define H5F_CRT_SHMSG_BTREE_MIN_NAME                                           \
  "shmsg_btree_min" /* Shared message B-tree minimum size */
#define H5F_CRT_FILE_SPACE_STRATEGY_NAME                                       \
  "file_space_strategy" /* File space handling strategy */
#define H5F_CRT_FREE_SPACE_PERSIST_NAME                                        \
  "free_space_persist" /* Free-space persisting status */
#define H5F_CRT_FREE_SPACE_THRESHOLD_NAME                                      \
  "free_space_threshold" /* Free space section threshold */
#define H5F_CRT_FILE_SPACE_PAGE_SIZE_NAME                                      \
  "file_space_page_size" /* File space page size */

/* ========= File Access properties ============ */
#define H5F_ACS_META_CACHE_INIT_CONFIG_NAME                                    \
  "mdc_initCacheCfg" /* Initial metadata cache resize configuration */
#define H5F_ACS_DATA_CACHE_NUM_SLOTS_NAME                                      \
  "rdcc_nslots" /* Size of raw data chunk cache(slots) */
#define H5F_ACS_DATA_CACHE_BYTE_SIZE_NAME                                      \
  "rdcc_nbytes" /* Size of raw data chunk cache(bytes) */
#define H5F_ACS_PREEMPT_READ_CHUNKS_NAME                                       \
  "rdcc_w0"                                  /* Preemption read chunks first */
...
#define H5F_ACS_META_BLOCK_SIZE_NAME                                           \
  "meta_block_size" /* Minimum metadata allocation block size (when            \
                       aggregating metadata allocations) */
#define H5F_ACS_SIEVE_BUF_SIZE_NAME                                            \
  "sieve_buf_size" /* Maximum sieve buffer size (when data sieving is allowed  \
                      by file driver) */
#define H5F_ACS_SDATA_BLOCK_SIZE_NAME                                          \
  "sdata_block_size" /* Minimum "small data" allocation block size (when       \
...
#define H5F_ACS_FAMILY_OFFSET_NAME                                             \
  "family_offset" /* Offset position in file for family file driver */
#define H5F_ACS_FAMILY_NEWSIZE_NAME                                            \
  "family_newsize" /* New member size of family driver.  (private property     \
                      only used by h5repart) */
#define H5F_ACS_FAMILY_TO_SINGLE_NAME                                          \
  "family_to_single" /* Whether to convert family to a single-file driver.     \
                        (private property only used by h5repart) */
#define H5F_ACS_MULTI_TYPE_NAME                                                \
  "multi_type" /* Data type in multi file driver */
#define H5F_ACS_LIBVER_LOW_BOUND_NAME                                          \
  "libver_low_bound" /* 'low' bound of library format versions */
#define H5F_ACS_LIBVER_HIGH_BOUND_NAME                                         \
  "libver_high_bound" /* 'high' bound of library format versions */
#define H5F_ACS_WANT_POSIX_FD_NAME                                             \
  "want_posix_fd" /* Internal: query the file descriptor from the core VFD,    \
                   * instead of the memory address                             \
                   */
#define H5F_ACS_METADATA_READ_ATTEMPTS_NAME                                    \
  "metadata_read_attempts" /* # of metadata read attempts */
#define H5F_ACS_OBJECT_FLUSH_CB_NAME                                           \
  "object_flush_cb"                      /* Object flush callback */
...
#define H5F_ACS_CLEAR_STATUS_FLAGS_NAME                                        \
  "clear_status_flags" /* Whether to clear superblock status_flags (private    \
                        * property only used by h5clear)                       \
                        */
#define H5F_ACS_NULL_FSM_ADDR_NAME                                             \
  "null_fsm_addr" /* Nullify addresses of free-space managers */
/* Private property used only by h5clear */
#define H5F_ACS_SKIP_EOF_CHECK_NAME "skip_eof_check" /* Skip EOF check */
/* Private property used only by h5clear */
#define H5F_ACS_USE_MDC_LOGGING_NAME                                           \
  "use_mdc_logging" /* Whether to use metadata cache logging */
#define H5F_ACS_MDC_LOG_LOCATION_NAME                                          \
  "mdc_log_location" /* Name of metadata cache log location */
...
#define H5F_ACS_EVICT_ON_CLOSE_FLAG_NAME                                       \
  "evict_on_close_flag" /* Whether or not the metadata cache will evict        \
                           objects on close */
#define H5F_ACS_COLL_MD_WRITE_FLAG_NAME                                        \
  "collective_metadata_write" /* property indicating whether metadata writes   \
                               * are done collectively or not                  \
                               */
#define H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_NAME                              \
  "mdc_initCacheImageCfg" /* Initial metadata cache image creation             \
                             configuration */
#define H5F_ACS_PAGE_BUFFER_SIZE_NAME                                          \
  "page_buffer_size" /* the maximum size for the page buffer cache */
#define H5F_ACS_PAGE_BUFFER_MIN_META_PERC_NAME                                 \
  "page_buffer_min_meta_perc" /* the min metadata percentage for the page      \
                                 buffer cache */
#define H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_NAME                                  \
  "page_buffer_min_raw_perc" /* the min raw data percentage for the page       \
                                buffer cache */
#define H5F_ACS_USE_FILE_LOCKING_NAME                                          \
  "use_file_locking" /* whether or not we use file locks for SWMR control and  \
                      * to prevent multiple writers                            \
                      */
#define H5F_ACS_IGNORE_DISABLED_FILE_LOCKS_NAME                                \
  "ignore_disabled_file_locks" /* whether or not we ignore "locks disabled"    \
                                  errors */
#ifdef H5_HAVE_PARALLEL
#define H5F_ACS_MPI_PARAMS_COMM_NAME                                           \
  "mpi_params_comm" /* the MPI communicator */
#define H5F_ACS_MPI_PARAMS_INFO_NAME "mpi_params_info" /* the MPI info struct  \
                                                        */
#endif                                                 /* H5_HAVE_PARALLEL */

/* ======================== File Mount properties ====================*/
#define H5F_MNT_SYM_LOCAL_NAME                                                 \
  "local" /* Whether absolute symlinks local to file. */
...
#define HDF5_SUPERBLOCK_VERSION_1                                              \
  1 /* Version with non-default B-tree 'K' value */
#define HDF5_SUPERBLOCK_VERSION_2                                              \
  2 /* Revised version with superblock extension and checksum */
#define HDF5_SUPERBLOCK_VERSION_3                                              \
  3 /* With file locking and consistency flags (at least this version for SWMR \
       support) */
#define HDF5_SUPERBLOCK_VERSION_LATEST                                         \
  HDF5_SUPERBLOCK_VERSION_3 /* The maximum super block format    */
...
#define HDF5_BTREE_CHUNK_IK_DEF                                                \
  32 /* Note! this value is assumed                                            \
        ...
        must compensate. -QAK                                                  \
     */
#define HDF5_BTREE_IK_MAX_ENTRIES                                              \
  65536 /* 2^16 - 2 bytes for storing entries (children) */
...
/* For paged aggregation: drop free-space with size <= this threshold for small
 * meta section */
...
/* Default size for meta data aggregation block (can be set via
 * H5Pset_meta_block_size()) */
#define H5F_META_BLOCK_SIZE_DEF 2048
/* Default size for small data aggregation block (can be set via
 * H5Pset_small_data_block_size()) */
#define H5F_SDATA_BLOCK_SIZE_DEF 2048

/* Check for file using paged aggregation */
#define H5F_SHARED_PAGED_AGGR(F_SH)                                            \
  ((F_SH)->fs_strategy == H5F_FSPACE_STRATEGY_PAGE && (F_SH)->fs_page_size)
#define H5F_PAGED_AGGR(F)                                                      \
  (F->shared->fs_strategy == H5F_FSPACE_STRATEGY_PAGE &&                       \
   F->shared->fs_page_size)

/* Metadata read attempt values */
#define H5F_METADATA_READ_ATTEMPTS                                             \
  1 /* Default # of read attempts for non-SWMR access */
#define H5F_SWMR_METADATA_READ_ATTEMPTS                                        \
  100 /* Default # of read attempts for SWMR access */
...
#define H5F_MEM_PAGE_GENERIC                                                   \
  H5F_MEM_PAGE_LARGE_SUPER /* Large-sized generic: meta and raw */
...
H5_DLL hid_t H5F_get_file_id(H5VL_object_t *vol_obj, H5I_type_t obj_type,
                             hbool_t app_ref);
...
H5_DLL herr_t H5F_get_obj_count(const H5F_t *f, unsigned types, hbool_t app_ref,
                                size_t *obj_id_count_ptr);
H5_DLL herr_t H5F_get_obj_ids(const H5F_t *f, unsigned types, size_t max_objs,
...
H5_DLL herr_t H5F_shared_get_file_driver(const H5F_shared_t *f_sh,
                                         H5FD_t **file_handle);
...
H5_DLL herr_t H5F_mount(const struct H5G_loc_t *loc, const char *name,
                        H5F_t *child, hid_t plist_id);
...
H5_DLL herr_t H5F_shared_block_read(H5F_shared_t *f_sh, H5FD_mem_t type,
                                    haddr_t addr, size_t size,
                                    void *buf /*out*/);
H5_DLL herr_t H5F_block_read(H5F_t *f, H5FD_mem_t type, haddr_t addr,
                             size_t size, void *buf /*out*/);
H5_DLL herr_t H5F_shared_block_write(H5F_shared_t *f_sh, H5FD_mem_t type,
                                     haddr_t addr, size_t size,
                                     const void *buf);
...
H5_DLL herr_t H5F_shared_select_read(H5F_shared_t *f_sh, H5FD_mem_t type,
                                     uint32_t count, struct H5S_t **mem_spaces,
                                     struct H5S_t **file_spaces,
                                     haddr_t offsets[], size_t element_sizes[],
                                     void *bufs[] /* out */);
H5_DLL herr_t H5F_shared_select_write(H5F_shared_t *f_sh, H5FD_mem_t type,
                                      uint32_t count, struct H5S_t **mem_spaces,
                                      struct H5S_t **file_spaces,
                                      haddr_t offsets[], size_t element_sizes[],
                                      const void *bufs[]);
...
H5_DLL herr_t H5F_shared_vector_read(H5F_shared_t *f_sh, uint32_t count,
                                     H5FD_mem_t types[], haddr_t addrs[],
                                     size_t sizes[], void *bufs[]);
H5_DLL herr_t H5F_shared_vector_write(H5F_shared_t *f_sh, uint32_t count,
                                      H5FD_mem_t types[], haddr_t addrs[],
...
H5_DLL herr_t H5F_get_checksums(const uint8_t *buf, size_t chk_size,
                                uint32_t *s_chksum, uint32_t *c_chksum);

/* Routine to track the # of retries */
H5_DLL herr_t H5F_track_metadata_read_retries(H5F_t *f, unsigned actype,
                                              unsigned retries);
...
H5_DLL void H5F_addr_decode(const H5F_t *f, const uint8_t **pp,
                            haddr_t *addr_p);
H5_DLL void H5F_addr_decode_len(size_t addr_len, const uint8_t **pp,
                                haddr_t *addr_p);
...
H5_DLL herr_t H5F_mpi_retrieve_comm(hid_t loc_id, hid_t acspl_id,
                                    MPI_Comm *mpi_comm);
H5_DLL herr_t H5F_mpi_get_file_block_type(hbool_t commit,
                                          MPI_Datatype *new_type,
                                          hbool_t *new_type_derived);
H5_DLL hbool_t H5F_get_coll_metadata_reads(const H5F_t *f);
H5_DLL void H5F_set_coll_metadata_reads(H5F_t *f,
                                        H5P_coll_md_read_flag_t *file_flag,
                                        hbool_t *context_flag);
H5_DLL herr_t H5F_shared_get_mpi_file_sync_required(const H5F_shared_t *f_sh,
                                                    hbool_t *flag);
#endif /* H5_HAVE_PARALLEL */
...
H5_DLL H5F_t *H5F_prefix_open_file(H5F_t *primary_file,
                                   ...
                                   const char *file_name, unsigned file_intent,
                                   hid_t fapl_id);
