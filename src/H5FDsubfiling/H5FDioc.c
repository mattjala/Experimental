...
static herr_t H5FD__ioc_write_vector_internal(H5FD_t *_file, uint32_t count,
                                              H5FD_mem_t types[],
                                              haddr_t addrs[], size_t sizes[],
                                              const void *bufs[] /* data_in */);
static herr_t H5FD__ioc_read_vector_internal(H5FD_t *_file, uint32_t count,
                                             haddr_t addrs[], size_t sizes[],
                                             void *bufs[] /* data_out */);
...
hid_t H5FD_ioc_init(void) {
  ...
  if (H5I_VFL != H5I_get_type(H5FD_IOC_g)) {
    ...
    if ((H5FD_IOC_g = H5FD_register(&H5FD_ioc_g, sizeof(H5FD_class_t), FALSE)) <
        0)
      H5_SUBFILING_GOTO_ERROR(H5E_ID, H5E_CANTREGISTER, H5I_INVALID_HID,
                              "can't register IOC VFD");
      ...
      if (mpi_initialized) {
        ...
        if (MPI_SUCCESS != (mpi_code = MPI_Query_thread(&provided)))
          H5_SUBFILING_MPI_GOTO_ERROR(H5I_INVALID_HID,
                                      "MPI_Query_thread failed", mpi_code);
        if (provided != MPI_THREAD_MULTIPLE)
          H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTINIT, H5I_INVALID_HID,
                                  "IOC VFD requires the use of MPI_Init_thread "
    ...
    if (MPI_SUCCESS != (mpi_code = MPI_Comm_get_attr(MPI_COMM_WORLD, MPI_TAG_UB,
                                                     ...
                                                     &key_val_retrieved)))
      H5_SUBFILING_MPI_GOTO_ERROR(H5I_INVALID_HID, "MPI_Comm_get_attr failed",
                                  mpi_code);
  ...
  if (vfd_config == NULL) {
    ...
    if (H5FD__ioc_get_default_config(ioc_conf) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                              "can't get default IOC VFD configuration");
...
static herr_t H5FD__ioc_validate_config(const H5FD_ioc_config_t *fa) {
  ...
  if (fa->version != H5FD_IOC_CURR_FAPL_VERSION)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "Unknown H5FD_ioc_config_t version");

  if (fa->magic != H5FD_IOC_FAPL_MAGIC)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid H5FD_ioc_config_t magic value");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_sb_encode(H5FD_t *_file, char *name,
                                  unsigned char *buf) {
...
static herr_t H5FD__ioc_sb_decode(H5FD_t *_file, const char *name,
  ...
  if (H5FD__ioc_validate_config(&file->fa) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                            "decoded IOC VFD configuration info is invalid");
...
static H5FD_t *H5FD__ioc_open(const char *name, unsigned flags, hid_t fapl_id,
  ...
  const H5FD_ioc_config_t *config_ptr =
  ...
  if (H5_subfiling_get_config_prop(plist_ptr, &file_ptr->subf_config) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                            "can't get subfiling configuration from FAPL");
  if (H5_subfiling_validate_config(&file_ptr->subf_config) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_BADVALUE, NULL,
  ...
  if (H5_subfiling_get_file_id_prop(plist_ptr, &file_ptr->file_id) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                            "can't get stub file ID from FAPL");
  if (file_ptr->file_id == UINT64_MAX) {
    ...
    H5_SUBFILING_DONE_ERROR(
        ...
        "subfiling stub file ID property was missing from FAPL - IOC VFD "
        "wasn't correctly stacked under "
  ...
  if (H5_open_subfiles(file_ptr->file_path, file_ptr->file_id,
                       &file_ptr->subf_config, ioc_flags, file_ptr->comm,
                       &file_ptr->context_id) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                            "unable to open subfiles for file '%s'", name);
...
static herr_t H5FD__ioc_close_int(H5FD_ioc_t *file_ptr) {
  ...
  if (file_ptr->context_id >= 0) {
    subfiling_context_t *sf_context =
        H5_get_subfiling_object(file_ptr->context_id);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_query(const H5FD_t H5_ATTR_UNUSED *_file,
                              unsigned long *flags /* out */) {
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_get_type_map(const H5FD_t H5_ATTR_UNUSED *_file,
                                     H5FD_mem_t H5_ATTR_UNUSED *type_map) {
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__ioc_alloc(H5FD_t H5_ATTR_UNUSED *_file,
                               H5FD_mem_t H5_ATTR_UNUSED type,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_free(H5FD_t H5_ATTR_UNUSED *_file,
                             H5FD_mem_t H5_ATTR_UNUSED type,
                             hid_t H5_ATTR_UNUSED dxpl_id,
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_get_handle(H5FD_t H5_ATTR_UNUSED *_file,
...
static herr_t H5FD__ioc_read(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
  ...
  if (REGION_OVERFLOW(addr, size))
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_OVERFLOW, FAIL,
                            "addr overflow, addr = %" PRIuHADDR, addr);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_write(H5FD_t *_file, H5FD_mem_t type,
                              hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr,
  ...
  ret_value =
      H5FD__ioc_write_vector_internal(_file, 1, &type, &addr, &size, &buf);
...
static herr_t H5FD__ioc_write_vector(H5FD_t *_file, hid_t dxpl_id,
  ...
  ret_value =
      H5FD__ioc_write_vector_internal(_file, count, types, addrs, sizes, bufs);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_flush(H5FD_t H5_ATTR_UNUSED *_file,
                              hid_t H5_ATTR_UNUSED dxpl_id,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_truncate(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
                                 hbool_t H5_ATTR_UNUSED closing) {
  H5FD_ioc_t *file = (H5FD_ioc_t *)_file;
  herr_t ret_value = SUCCEED;
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_lock(H5FD_t H5_ATTR_UNUSED *_file,
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5FD__ioc_unlock(H5FD_t H5_ATTR_UNUSED *_file) {
  herr_t ret_value = SUCCEED;
...
static herr_t H5FD__ioc_del(const char *name, hid_t fapl) {
  ...
  if (mpi_rank == 0) {
    ...
    if (HDstat(name, &st) < 0)
      H5_SUBFILING_SYS_GOTO_ERROR(H5E_FILE, H5E_SYSERRSTR, FAIL,
                                  "HDstat failed");
    ...
    HDsnprintf(tmp_filename, PATH_MAX,
               ...
               prefix_env ? prefix_env : file_dirname, base_filename,
               (uint64_t)st.st_ino);
...
#ifdef H5FD_IOC_DEBUG
        printf(
            "** WARNING: couldn't delete Subfiling configuration file '%s'\n",
            tmp_filename);
#endif
    ...
    if (H5_get_subfiling_config_from_file(config_file, NULL, &read_n_subfiles) <
        0)
      H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_READERROR, FAIL,
                              "can't read subfiling config file");
    ...
    for (int i = 0; i < n_subfiles; i++) {
      /* TODO: No support for subfile directory prefix currently */
      HDsnprintf(tmp_filename, PATH_MAX, "%s/" H5FD_SUBFILING_FILENAME_TEMPLATE,
                 file_dirname, base_filename, (uint64_t)st.st_ino, num_digits,
                 i + 1, n_subfiles);
      ...
      if (HDremove(tmp_filename) < 0) {
        ...
        if (ENOENT != errno)
          H5_SUBFILING_SYS_GOTO_ERROR(H5E_FILE, H5E_CANTDELETEFILE, FAIL,
                                      "can't delete subfile");
      }
    }
...
static herr_t H5FD__ioc_write_vector_internal(H5FD_t *_file, uint32_t count,
  ...
  if (NULL == (sf_context = H5_get_subfiling_object(sf_context_id)))
    H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTGET, FAIL,
                            "can't get subfiling context from ID");
  ...
  if (NULL == (sf_io_reqs = calloc((size_t)count, sizeof(*sf_io_reqs))))
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                            "can't allocate I/O request array");
  if (NULL == (mpi_reqs = malloc(2 * (size_t)count * sizeof(*mpi_reqs))))
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                            "can't allocate MPI request array");
  ...
  for (size_t i = 0; i < (size_t)count; i++) {
    ...
    if (sizes[i] == 0)
      H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                              "invalid size argument of 0");

    H5_CHECK_OVERFLOW(addrs[i], haddr_t, int64_t);
    H5_CHECK_OVERFLOW(sizes[i], size_t, int64_t);
    write_status = ioc__write_independent_async(
        sf_context_id, (int64_t)addrs[i], (int64_t)sizes[i], bufs[i],
        &sf_io_reqs[i]);

    if (write_status < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                              "couldn't queue write operation");
...
static herr_t H5FD__ioc_read_vector_internal(H5FD_t *_file, uint32_t count,
  ...
  for (size_t i = 0; i < (size_t)count; i++) {
    ...
    H5_CHECK_OVERFLOW(sizes[i], size_t, int64_t);
    read_status =
        ioc__read_independent_async(sf_context_id, (int64_t)addrs[i],
                                    (int64_t)sizes[i], bufs[i], &sf_io_reqs[i]);

    if (read_status < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_READERROR, FAIL,
                              "couldn't queue read operation");
