 ...
 * into that subfile
 */
static inline void calculate_target_ioc(int64_t file_offset,
                                        int64_t stripe_size,
                                        int num_io_concentrators,
                                        int num_subfiles, int64_t *target_ioc,
 ...
 *-------------------------------------------------------------------------
 */
herr_t ioc__write_independent_async(int64_t context_id, int64_t offset,
   ...
   */
  calculate_target_ioc(offset, sf_context->sf_stripe_size, num_io_concentrators,
                       num_subfiles, &ioc_start, &ioc_offset, &ioc_subfile_idx);
  ...
  if (MPI_SUCCESS !=
      (mpi_code =
           MPI_Irecv(&data_tag, 1, MPI_INT, io_concentrators[ioc_start],
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Send(msg, 1, H5_subfiling_rpc_msg_type,
                           io_concentrators[ioc_start], WRITE_INDEP,
  ...
  if (NULL == (sf_io_request = malloc(sizeof(io_req_t))))
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_WRITEERROR, FAIL,
                            "couldn't allocate I/O request");
  ...
  if (MPI_SUCCESS !=
      (mpi_code =
           MPI_Irecv(&sf_io_request->io_comp_tag, 1, MPI_INT,
  ...
  if (MPI_SUCCESS != (mpi_code = MPI_Isend(data, (int)elements, MPI_BYTE,
                                           io_concentrators[ioc_start],
                                           data_tag, sf_context->sf_data_comm,
                                           &sf_io_request->io_transfer_req)))
  ...
  if (ret_value < 0) {
    ...
    if (sf_io_request) {
      if (sf_io_request->io_transfer_req != MPI_REQUEST_NULL) {
        if (MPI_SUCCESS != (mpi_code = MPI_Wait(&sf_io_request->io_transfer_req,
                                                MPI_STATUS_IGNORE)))
          H5_SUBFILING_MPI_DONE_ERROR(FAIL, "MPI_Wait failed", mpi_code);
      }
      if (sf_io_request->io_comp_req != MPI_REQUEST_NULL) {
        if (MPI_SUCCESS != (mpi_code = MPI_Wait(&sf_io_request->io_comp_req,
                                                MPI_STATUS_IGNORE)))
          H5_SUBFILING_MPI_DONE_ERROR(FAIL, "MPI_Wait failed", mpi_code);
      }
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t ioc__read_independent_async(int64_t context_id, int64_t offset,
   ...
   */
  if (NULL == (sf_io_request = malloc(sizeof(io_req_t))))
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_READERROR, FAIL,
                            "couldn't allocate I/O request");
  ...
  if (need_data_tag) {
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Send(msg, 1, H5_subfiling_rpc_msg_type,
                             io_concentrators[ioc_start], READ_INDEP,
                             sf_context->sf_msg_comm)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Send failed", mpi_code);
   ...
   */
  if (MPI_SUCCESS != (mpi_code = MPI_Irecv(data, (int)elements, MPI_BYTE,
                                           io_concentrators[ioc_start],
                                           data_tag, sf_context->sf_data_comm,
                                           &sf_io_request->io_transfer_req)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Irecv failed", mpi_code);
 ...
 *-------------------------------------------------------------------------
 */
herr_t ioc__async_completion(MPI_Request *mpi_reqs, size_t num_reqs) {
  herr_t ret_value = SUCCEED;
  int mpi_code;
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Waitall((int)num_reqs, mpi_reqs, MPI_STATUSES_IGNORE)))
