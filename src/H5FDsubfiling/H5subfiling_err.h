  ...
  do {                                                                         \
    ...
    if (err_func) {                                                            \
      if (H5subfiling_err_stack_g >= 0 && H5subfiling_err_class_g >= 0) {      \
        H5Epush2(H5subfiling_err_stack_g, __FILE__, __func__, __LINE__,        \
                 H5subfiling_err_class_g, err_major, err_minor, __VA_ARGS__);  \
      } else {                                                                 \
  ...
  do {                                                                         \
    ...
    if (err_func) {                                                            \
      if ((H5subfiling_err_stack_g >= 0) &&                                    \
          (H5Eget_num(H5subfiling_err_stack_g) > 0)) {                         \
...
#define H5_SUBFILING_SYS_GOTO_ERROR(err_major, err_minor, ret_val, str)        \
  do {                                                                         \
    int myerrno = errno;                                                       \
    H5_SUBFILING_GOTO_ERROR(err_major, err_minor, ret_val,                     \
...
#define H5_SUBFILING_MPI_DONE_ERROR(retcode, str, mpierr)                      \
  do {                                                                         \
    MPI_Error_string(mpierr, H5subfiling_mpi_error_str,                        \
                     &H5subfiling_mpi_error_str_len);                          \
    H5_SUBFILING_DONE_ERROR(H5E_INTERNAL, H5E_MPI, retcode,                    \
                            "%s: MPI error string is '%s'", str,               \
...
#define H5_SUBFILING_MPI_GOTO_ERROR(retcode, str, mpierr)                      \
  do {                                                                         \
    MPI_Error_string(mpierr, H5subfiling_mpi_error_str,                        \
                     &H5subfiling_mpi_error_str_len);                          \
    H5_SUBFILING_GOTO_ERROR(H5E_INTERNAL, H5E_MPI, retcode,                    \
                            "%s: MPI error string is '%s'", str,               \
                            H5subfiling_mpi_error_str);                        \
  } while (0)
