...
typedef struct ioc_data_t {
  ...
  /* sf_io_ops_pending is use to track the number of I/O operations pending so
   * that we can wait until all I/O operations have been serviced before
   * shutting down the worker thread pool. The value of this variable must
   * always be non-negative.
   *
   * Note that this is a convenience variable -- we could use io_queue.q_len
   * instead. However, accessing this field requires locking io_queue.q_mutex.
   */
...
static int ioc_file_queue_write_indep(sf_work_request_t *msg, int ioc_idx,
                                      int source, MPI_Comm comm,
                                      uint32_t counter);
static int ioc_file_queue_read_indep(sf_work_request_t *msg, int ioc_idx,
                                     int source, MPI_Comm comm,
                                     uint32_t counter);

static int ioc_file_write_data(int fd, int64_t file_offset, void *data_buffer,
                               int64_t data_size, int ioc_idx);
static int ioc_file_read_data(int fd, int64_t file_offset, void *data_buffer,
                              int64_t data_size, int ioc_idx);
...
static void ioc_io_queue_complete_entry(ioc_data_t *ioc_data,
                                        ioc_io_queue_entry_t *entry_ptr);
...
static void ioc_io_queue_add_entry(ioc_data_t *ioc_data,
                                   sf_work_request_t *wk_req_ptr);
...
int initialize_ioc_threads(void *_sf_context) {
  ...
  ioc_data->io_queue =
      (ioc_io_queue_t){/* magic               = */ H5FD_IOC__IO_Q_MAGIC,
                       /* q_head              = */ NULL,
                       /* q_tail              = */ NULL,
                       /* num_pending         = */ 0,
                       /* num_in_progress     = */ 0,
  ...
  if (hg_thread_mutex_init(&ioc_data->io_queue.q_mutex) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, (-1),
                            "can't initialize IOC thread queue mutex");
  ...
  if (hg_thread_pool_init(thread_pool_size, &ioc_data->io_thread_pool) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, (-1),
                            "can't initialize IOC worker thread pool");

  /* Create the main IOC thread that will receive and dispatch I/O requests */
  if (hg_thread_create(&ioc_data->ioc_main_thread, ioc_thread_main, ioc_data) <
      0)
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, (-1),
                            "can't create IOC main thread");
...
int finalize_ioc_threads(void *_sf_context) {
  subfiling_context_t *sf_context = _sf_context;
  ioc_data_t *ioc_data = NULL;
  int ret_value = 0;
  ...
  if (ioc_data->io_queue.num_failed > 0)
    H5_SUBFILING_DONE_ERROR(H5E_IO, H5E_CLOSEERROR, -1,
                            "%" PRId32 " I/O requests failed",
...
static int ioc_main(ioc_data_t *ioc_data) {
  ...
  /* tell initialize_ioc_threads() that ioc_main() is ready to enter its main
   * loop */
  ...
  while ((!shutdown_requested) ||
         (0 < atomic_load(&ioc_data->sf_io_ops_pending)) ||
    ...
    if (MPI_SUCCESS !=
        (mpi_code = (MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG,
                                context->sf_msg_comm, &flag, &status))))
    ...
    if (flag) {
      ...
      if ((tag != READ_INDEP) && (tag != WRITE_INDEP) && (tag != TRUNC_OP) &&
          (tag != GET_EOF_OP))
        H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, -1,
                                "invalid work request operation (%d)", tag);
      ...
      if (count < 0)
        H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, -1,
                                "invalid work request message size (%d)",
                                count);

      if ((size_t)count > sizeof(sf_work_request_t))
        H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, -1,
                                "work request message is too large (%d)",
                                count);
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Recv(&wk_req, count, MPI_BYTE, source, tag,
...
static HG_THREAD_RETURN_TYPE handle_work_request(void *arg) {
  ...
  switch (msg->tag) {
  case WRITE_INDEP:
    op_ret = ioc_file_queue_write_indep(msg, msg->ioc_idx, msg->source,
                                        sf_context->sf_data_comm,
                                        q_entry_ptr->counter);
    break;

  case READ_INDEP:
    op_ret = ioc_file_queue_read_indep(msg, msg->ioc_idx, msg->source,
                                       sf_context->sf_data_comm,
                                       q_entry_ptr->counter);
    break;
  ...
  default:
    ...
    H5_subfiling_log(
        file_context_id,
        "%s: IOC %d received unknown message with tag %x from rank %d",
  ...
  if (op_ret < 0) {
    ...
    H5_subfiling_log(file_context_id,
                     "%s: IOC %d request(%s) from rank(%d), (%" PRId64
                     ", %" PRId64 ", %" PRId64 ") FAILED with ret %d",
                     __func__, msg->ioc_idx,
                     translate_opcode((io_op_t)msg->tag), msg->source,
 ...
 *-------------------------------------------------------------------------
 */
void H5FD_ioc_begin_thread_exclusive(void) {
 ...
 *-------------------------------------------------------------------------
 */
void H5FD_ioc_end_thread_exclusive(void) {
  hg_thread_mutex_unlock(&ioc_thread_mutex);
}

static herr_t send_ack_to_client(int ack_val, int dest_rank, int source_rank,
                                 int msg_tag, MPI_Comm comm) {
  int mpi_code;
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Send(&ack_val, 1, MPI_INT, dest_rank, msg_tag, comm)))
...
static herr_t send_nack_to_client(int dest_rank, int source_rank, int msg_tag,
                                  MPI_Comm comm) {
  int nack = 0;
  int mpi_code;
  herr_t ret_value = SUCCEED;
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Send(&nack, 1, MPI_INT, dest_rank, msg_tag, comm)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Send", mpi_code);
...
static int ioc_file_queue_write_indep(sf_work_request_t *msg, int ioc_idx,
  ...
  if (data_size < 0) {
    send_nack = TRUE;
    H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_BADVALUE, -1,
                            "invalid data size for write");
  }
  ...
  sf_eof += (haddr_t)((stripe_id * sf_context->sf_blocksize_per_stripe) +
  ...
  H5_subfiling_log(file_context_id,
                   ...
                   ioc_idx, __func__, source, data_size, file_offset,
                   t_queue_delay);
#endif
  ...
  if (NULL == (recv_buf = malloc((size_t)data_size))) {
    send_nack = TRUE;
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, -1,
                            "couldn't allocate receive buffer for data");
  }
  ...
  if (send_ack_to_client(rcv_tag, source, ioc_idx, WRITE_INDEP_ACK, comm) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_WRITEERROR, -1,
                            "couldn't send ACK to client");

  /* Receive data from client */
  H5_CHECK_OVERFLOW(data_size, int64_t, int);
  if (MPI_SUCCESS != (mpi_code = MPI_Recv(recv_buf, (int)data_size, MPI_BYTE,
                                          source, rcv_tag, comm, &msg_status)))
    H5_SUBFILING_MPI_GOTO_ERROR(-1, "MPI_Recv failed", mpi_code);

  if (MPI_SUCCESS !=
      (mpi_code = MPI_Get_count(&msg_status, MPI_BYTE, &data_bytes_received)))
    H5_SUBFILING_MPI_GOTO_ERROR(-1, "MPI_Get_count failed", mpi_code);

  if (data_bytes_received != data_size)
    H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_WRITEERROR, -1,
                            "message size mismatch -- expected = %" PRId64
                            ", actual = %d",
  ...
  if (sf_fid < 0)
    H5_subfiling_log(
        file_context_id,
        "%s: WARNING: attempt to write data to closed subfile FID %d", __func__,
        sf_fid);
#endif

  if (sf_fid >= 0) {
    /* Actually write data received from client into subfile */
    if ((write_ret = ioc_file_write_data(sf_fid, file_offset, recv_buf,
                                         data_size, ioc_idx)) < 0)
      H5_SUBFILING_GOTO_ERROR(
          H5E_IO, H5E_WRITEERROR, -1,
   ...
   */
  if (MPI_SUCCESS != (mpi_code = MPI_Send(&rcv_tag, 1, MPI_INT, source,
                                          WRITE_DATA_DONE, comm)))
  ...
  if (send_nack) {
    ...
    if (send_nack_to_client(source, ioc_idx, WRITE_INDEP_ACK, comm) < 0)
      H5_SUBFILING_DONE_ERROR(H5E_IO, H5E_WRITEERROR, -1,
                              "couldn't send NACK to client");
  }
...
static int ioc_file_queue_read_indep(sf_work_request_t *msg, int ioc_idx,
  ...
  if (NULL == (send_buf = malloc((size_t)data_size))) {
    ...
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, -1,
                            "couldn't allocate send buffer for data");
  }
  ...
  if (need_data_tag) {
     ...
     */
    assert(H5FD_IOC_tag_ub_val_ptr &&
           (*H5FD_IOC_tag_ub_val_ptr >= IO_TAG_BASE));
    send_tag = (int)(counter % (INT_MAX - IO_TAG_BASE));
    send_tag %= (*H5FD_IOC_tag_ub_val_ptr - IO_TAG_BASE);
    send_tag += IO_TAG_BASE;

    if (send_ack_to_client(send_tag, source, ioc_idx, READ_INDEP_ACK, comm) <
        0) {
      send_empty_buf = FALSE;
  ...
  if (sf_fid < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_BADVALUE, -1,
                            "subfile file descriptor %d is invalid", sf_fid);

  if ((read_ret = ioc_file_read_data(sf_fid, file_offset, send_buf, data_size,
                                     ioc_idx)) < 0) {
    H5_SUBFILING_GOTO_ERROR(
        H5E_IO, H5E_READERROR, read_ret,
        "read function(FID=%d, Source=%d) returned an error (%d)", sf_fid,
        source, read_ret);
  }
  ...
  H5_CHECK_OVERFLOW(data_size, int64_t, int);
  if (MPI_SUCCESS != (mpi_code = MPI_Send(send_buf, (int)data_size, MPI_BYTE,
                                          source, send_tag, comm)))
    H5_SUBFILING_MPI_GOTO_ERROR(-1, "MPI_Send failed", mpi_code);
  ...
  H5_subfiling_log(sf_context->sf_context_id,
                   "[ioc(%d)] MPI_Send to source(%d) completed\n", ioc_idx,
                   source);
#endif
  ...
  if (need_data_tag && send_nack) {
    ...
    if (send_nack_to_client(source, ioc_idx, READ_INDEP_ACK, comm) < 0)
      H5_SUBFILING_DONE_ERROR(H5E_IO, H5E_READERROR, -1,
                              "couldn't send NACK to client");
  }
  if (send_empty_buf) {
    /*
     ...
     */
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Send(NULL, 0, MPI_BYTE, source, send_tag, comm)))
...
static int ioc_file_write_data(int fd, int64_t file_offset, void *data_buffer,
  ...
  while (bytes_remaining) {
    ...
    bytes_written =
        HDpwrite(fd, this_data, (size_t)bytes_remaining, file_offset);
    ...
    if (bytes_written >= 0) {
...
#ifdef H5FD_IOC_DEBUG
      printf(
          "[ioc(%d) %s]: wrote %ld bytes, remaining=%ld, file_offset=%" PRId64
          "\n",
      ...
      H5_SUBFILING_SYS_GOTO_ERROR(H5E_IO, H5E_WRITEERROR, -1,
                                  "HDpwrite failed");
    }
  }
...
static int ioc_file_read_data(int fd, int64_t file_offset, void *data_buffer,
  ...
  while (bytes_remaining) {
    ...
    if (bytes_read > 0) {
      ...
      if (retries == 0) {
#ifdef H5FD_IOC_DEBUG
        printf("[ioc(%d) %s]: TIMEOUT: file_offset=%" PRId64
               ", data_size=%ld\n",
               ioc_idx, __func__, file_offset, data_size);
#endif

        H5_SUBFILING_SYS_GOTO_ERROR(H5E_IO, H5E_READERROR, -1,
                                    "HDpread failed");
      }
...
static int ioc_file_truncate(sf_work_request_t *msg) {
  ...
  if (NULL == (sf_context = H5_get_subfiling_object(file_context_id)))
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTGET, -1,
                            "couldn't retrieve subfiling context");
  ...
  if (HDftruncate(fd, (off_t)length) != 0)
    H5_SUBFILING_SYS_GOTO_ERROR(H5E_FILE, H5E_SEEKERROR, -1,
                                "HDftruncate failed");
  ...
  if (MPI_SUCCESS !=
      (mpi_code =
           MPI_Send(msg->header, 1, H5_subfiling_rpc_msg_type, msg->source,
  ...
  printf("[ioc(%d) %s]: truncated subfile to %lld bytes. ret = %d\n", ioc_idx,
         __func__, (long long)length, errno);
  fflush(stdout);
#endif
...
#ifdef H5_SUBFILING_DEBUG
  H5_subfiling_log(file_context_id, "%s: reporting file EOF as %" PRId64 ".",
                   __func__, eof_req_reply[1]);
#endif

  /* return the subfile EOF to the querying rank */
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Send(eof_req_reply, 1, H5_subfiling_rpc_msg_type, source,
                           GET_EOF_COMPLETED, comm)))
...
static void ioc_io_queue_add_entry(ioc_data_t *ioc_data,
  ...
  memcpy((void *)(&(entry_ptr->wk_req)), (const void *)wk_req_ptr,
  ...
  H5_subfiling_log(wk_req_ptr->context_id,
                   "%s: request %d queued. op = %d, req = (%lld, %lld, %lld), "
  ...
  if (ioc_data->io_queue.q_len != atomic_load(&ioc_data->sf_io_ops_pending)) {
    H5_subfiling_log(wk_req_ptr->context_id,
                     "%s: ioc_data->io_queue->q_len = %d != %d = "
                     "atomic_load(&ioc_data->sf_io_ops_pending).",
                     __func__, ioc_data->io_queue.q_len,
                     atomic_load(&ioc_data->sf_io_ops_pending));
  }
#endif
 ...
 *-------------------------------------------------------------------------
 */
/* TODO: Keep an eye on statistics and optimize this algorithm if necessary.
 * While it is O(N) where N is the number of elements in the I/O Queue if there
 * are are no-overlaps, it can become O(N**2) in the worst case.
 */
static void ioc_io_queue_dispatch_eligible_entries(ioc_data_t *ioc_data,
                                                   hbool_t try_lock) {
  ...
  while ((entry_ptr) && (ioc_data->io_queue.num_pending > 0)) {
    ...
    if (ioc_data->io_queue.q_head->in_progress) {
      if ((ioc_data->io_queue.q_head->wk_req.tag == TRUNC_OP) ||
        ...
        /* we have a truncate or get eof operation in progress -- thus no other
         * operations can be dispatched until the truncate or get eof operation
         * completes.  Just break out of the loop.
         */
    ...
    if (!entry_ptr->in_progress) {
      ...
      assert((scan_ptr == NULL) ||
             (scan_ptr->magic == H5FD_IOC__IO_Q_ENTRY_MAGIC));

      if ((entry_ptr->wk_req.tag == TRUNC_OP) ||
          (entry_ptr->wk_req.tag == GET_EOF_OP)) {

        if (scan_ptr != NULL) {

          /* the TRUNC_OP or GET_EOF_OP is not at the head of the queue, and
           * thus cannot be dispatched.  Further, no operation can be dispatched
           * if a truncate request appears before it in the queue.  Thus we have
           * done all we can and will break out of the loop.
           */
          break;
        }
      }
      ...
      while ((scan_ptr) && (!conflict_detected)) {
        ...
        /* at present, I/O requests are scalar -- i.e. single blocks specified
         * by offset and length. when this changes, this if statement will have
         * to be updated accordingly.
         */
        if (((scan_offset + scan_len) > entry_offset) &&
            ((entry_offset + entry_len) > scan_offset)) {

          /* the two request overlap -- unless they are both reads, we have
           * detected a conflict */

          /* TODO: update this if statement when we add collective I/O */
          if ((entry_ptr->wk_req.tag != READ_INDEP) ||
              (scan_ptr->wk_req.tag != READ_INDEP)) {
...
#ifdef H5FD_IOC_COLLECT_STATS
        if (ioc_data->io_queue.num_in_progress >
            ioc_data->io_queue.max_num_in_progress) {
          ioc_data->io_queue.max_num_in_progress =
              ioc_data->io_queue.num_in_progress;
        }
...
static void ioc_io_queue_complete_entry(ioc_data_t *ioc_data,
  ...
  H5_subfiling_log(
      entry_ptr->wk_req.context_id,
      "%s: request %d completed with ret %d. op = %d, req = (%lld, %lld, "
      "%lld), "
      "q-ed/disp/ops_pend = %d/%d/%d.",
   ...
   */
  if ((entry_ptr->wk_req.tag == GET_EOF_OP) ||
      (entry_ptr->wk_req.tag == TRUNC_OP))
    assert(ioc_data->io_queue.num_in_progress == 0);
#endif
