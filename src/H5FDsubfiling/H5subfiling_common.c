...
static herr_t init_subfiling(const char *base_filename, uint64_t file_id,
                             ...
                             int file_acc_flags, MPI_Comm comm,
                             int64_t *context_id_out);
static herr_t init_app_topology(H5FD_subfiling_params_t *subfiling_config,
                                MPI_Comm comm, MPI_Comm node_comm,
                                sf_topology_t **app_topology_out);
static herr_t get_ioc_selection_criteria_from_env(
    H5FD_subfiling_ioc_select_t *ioc_selection_type, char **ioc_sel_info_str);
static herr_t find_cached_topology_info(MPI_Comm comm,
                                        H5FD_subfiling_params_t *subf_config,
                                        long iocs_per_node,
                                        sf_topology_t **app_topology);
static herr_t init_app_layout(sf_topology_t *app_topology, MPI_Comm comm,
                              MPI_Comm node_comm);
static herr_t gather_topology_info(app_layout_t *app_layout, MPI_Comm comm,
                                   MPI_Comm intra_comm);
...
static herr_t init_subfiling_context(subfiling_context_t *sf_context,
                                     ...
                                     sf_topology_t *app_topology,
                                     MPI_Comm file_comm);
static herr_t open_subfile_with_context(subfiling_context_t *sf_context,
                                        int file_acc_flags);
static herr_t record_fid_to_subfile(uint64_t file_id,
                                    int64_t subfile_context_id,
                                    int *next_index);
...
static herr_t create_config_file(subfiling_context_t *sf_context,
                                 const char *base_filename,
                                 const char *config_dir,
                                 const char *subfile_dir,
                                 hbool_t truncate_if_exists);
static herr_t open_config_file(const char *base_filename,
                               const char *config_dir, uint64_t file_id,
 ...
 *-------------------------------------------------------------------------
 */
int64_t H5_new_subfiling_object_id(sf_obj_type_t obj_type) {
  int64_t index_val = 0;
 ...
 *-------------------------------------------------------------------------
 */
void *H5_get_subfiling_object(int64_t object_id) {
  ...
  if (obj_type == SF_CONTEXT) {
    ...
    if (!sf_context_cache) {
      if (NULL == (sf_context_cache = calloc(DEFAULT_CONTEXT_CACHE_SIZE,
                                             sizeof(*sf_context_cache))))
        H5_SUBFILING_GOTO_ERROR(
            H5E_RESOURCE, H5E_CANTALLOC, NULL,
    ...
    if ((size_t)obj_index >= sf_context_cache_size) {
      ...
      if (NULL == (tmp_realloc = realloc(sf_context_cache,
                                         new_size * sizeof(*sf_context_cache))))
        H5_SUBFILING_GOTO_ERROR(
            H5E_RESOURCE, H5E_CANTALLOC, NULL,
            "couldn't allocate space for subfiling context cache");
      ...
      memset(&sf_context_cache[old_num_entries], 0,
             (sf_context_cache_size - old_num_entries) *
                 sizeof(*sf_context_cache));
    ...
    if (!sf_topology_cache) {
      if (NULL == (sf_topology_cache = calloc(DEFAULT_TOPOLOGY_CACHE_SIZE,
                                              sizeof(*sf_topology_cache))))
        H5_SUBFILING_GOTO_ERROR(
            H5E_RESOURCE, H5E_CANTALLOC, NULL,
    ...
    if ((size_t)obj_index >= sf_topology_cache_size) {
      ...
      if (NULL ==
          (tmp_realloc = realloc(sf_topology_cache,
                                 new_size * sizeof(*sf_topology_cache))))
        H5_SUBFILING_GOTO_ERROR(
            H5E_RESOURCE, H5E_CANTALLOC, NULL,
            "couldn't allocate space for subfiling topology cache");
      ...
      memset(&sf_topology_cache[old_num_entries], 0,
             (sf_topology_cache_size - old_num_entries) *
                 sizeof(*sf_topology_cache));
  ...
  if (obj_type == SF_CONTEXT) {
    ...
    if (H5_free_subfiling_object_int(sf_context) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTFREE, FAIL,
                              "couldn't free subfiling object");
    ...
    if (H5_free_subfiling_topology(sf_topology) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTFREE, FAIL,
                              "couldn't free subfiling topology");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_open_subfiling_stub_file(const char *name, unsigned flags,
                                   MPI_Comm file_comm, H5FD_t **file_ptr,
                                   uint64_t *file_id) {
  ...
  if (!name)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid subfiling stub file name");
  ...
  if (mpi_rank == 0) {
    ...
    if ((fapl_id = H5P_create_id(H5P_CLS_FILE_ACCESS_g, FALSE)) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                              "can't create FAPL for stub file");
    if (NULL == (plist = H5P_object_verify(fapl_id, H5P_FILE_ACCESS)))
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_BADTYPE, FAIL,
    ...
    if (H5P_set_driver(plist, H5FD_MPIO, NULL, NULL) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                              "can't set MPI I/O driver on FAPL");

    if (NULL == (stub_file = H5FD_open(name, flags, fapl_id, HADDR_UNDEF)))
      H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL,
                              "couldn't open HDF5 stub file");
  ...
  if (mpi_size > 1) {
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Bcast(&stub_file_id, 1, MPI_UINT64_T, 0, file_comm)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_open_subfiles(const char *base_filename, uint64_t file_id,
                        H5FD_subfiling_params_t *subfiling_config,
  ...
  if (!base_filename)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL,
                            "invalid subfiling base filename");

  if (!subfiling_config)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL,
                            "invalid subfiling configuration");

  if (!context_id_out)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL,
                            "invalid subfiling context ID pointer");

  /* Initialize new subfiling context ID based on configuration information */
  if (init_subfiling(base_filename, file_id, subfiling_config, file_acc_flags,
                     file_comm, &context_id) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                            "couldn't initialize subfiling context");
  ...
  if (open_subfile_with_context(sf_context, file_acc_flags) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL,
                            "couldn't open subfiling subfiles");
...
#ifdef H5_SUBFILING_DEBUG
  {
    ...
    HDsnprintf(sf_context->sf_logfile_name, PATH_MAX, "%s.log.%d",
               sf_context->h5_filename, mpi_rank);

    if (NULL ==
        (sf_context->sf_logfile = fopen(sf_context->sf_logfile_name, "a")))
  ...
  *context_id_out = context_id;
   ...
   */
  {
    ...
    if (mpi_size > 1) {
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Allreduce(MPI_IN_PLACE, &err_result, 1, MPI_INT,
                                    MPI_MAX, file_comm)))
...
static herr_t init_subfiling(const char *base_filename, uint64_t file_id,
  ...
  if (prefix_env) {
    if (NULL == (new_context->config_file_prefix = HDstrdup(prefix_env)))
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTCOPY, FAIL,
                              "couldn't copy config file prefix string");
  }
  ...
  if (0 == (file_acc_flags & O_CREAT)) {
    ...
    if (mpi_rank == 0) {
      ...
      if (config[0] >= 0) {
         ...
         */
        if (open_config_file(file_basename,
                             prefix_env ? prefix_env : subfile_dir, file_id,
                             "r", &config_file) < 0)
          config[0] = -1;
      }
      ...
      if (config[0] >= 0) {
        if (!config_file)
           ...
           */
          if (H5_get_subfiling_config_from_file(config_file, &config[0],
                                                &config[1]) < 0)
            config[0] = -1;
        }
      }
    ...
    if (mpi_size > 1) {
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Bcast(config, 2, MPI_INT64_T, 0, comm)))
        H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_code);
    }
    ...
    if (config[0] == -1)
      H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL,
                              "lead process couldn't read the number of "
                              "subfiles from subfiling configuration file");
    else {
      if (config[0] > 0)
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_split_type(comm, MPI_COMM_TYPE_SHARED, mpi_rank,
                                      MPI_INFO_NULL, &node_comm)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_split_type failed", mpi_code);

  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_set_errhandler(node_comm, MPI_ERRORS_RETURN)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_set_errhandler failed",
                                mpi_code);
#else
#error "MPI-3 required for MPI_Comm_split_type"
  ...
  if (init_app_topology(subfiling_config, comm, node_comm, &app_topology) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                            "couldn't initialize application topology");

  new_context->sf_context_id = context_id;

  if (init_subfiling_context(new_context, base_filename, file_id,
                             subfiling_config, app_topology, comm) < 0)
    H5_SUBFILING_GOTO_ERROR(
  ...
  *context_id_out = context_id;
  ...
  if (ret_value < 0) {
    ...
    if (H5_mpi_comm_free(&node_comm) < 0)
      H5_SUBFILING_DONE_ERROR(H5E_VFL, H5E_CANTFREE, FAIL,
                              "couldn't free MPI communicator");
...
static herr_t init_app_topology(H5FD_subfiling_params_t *subfiling_config,
  ...
  if (get_ioc_selection_criteria_from_env(&ioc_selection_type, &ioc_sel_str) <
      0)
    ...
    if (ioc_sel_str) {
      ...
      if ((ERANGE == errno) || (ioc_select_val <= 0)) {
        printf("invalid IOC selection strategy string '%s' for strategy "
               "SELECT_IOC_EVERY_NTH_RANK; defaulting to "
  ...
  default:
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                            "invalid IOC selection strategy");
    break;
  }
  ...
  if (find_cached_topology_info(comm, subfiling_config, iocs_per_node,
                                &app_topology) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
  ...
  if (!app_topology) {
    ...
    if (H5_mpi_comm_dup(comm, &app_topology->app_comm) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTCOPY, FAIL,
                              "can't duplicate MPI communicator");

    if (init_app_layout(app_topology, comm, node_comm) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                              "couldn't initialize application layout");
    ...
    if (app_topology->selection_type == SELECT_IOC_ONE_PER_NODE)
      app_topology->n_io_concentrators =
          (int)iocs_per_node * app_topology->app_layout->node_count;
  ...
  if (ret_value < 0) {
    if (app_topology && (topology_id >= 0)) {
      if (H5_free_subfiling_object(topology_id) < 0)
        H5_SUBFILING_DONE_ERROR(H5E_VFL, H5E_CANTFREE, FAIL,
                                "can't free subfiling topology object");
    }
  }
    ...
    if (opt_value) {
      ...
      *opt_value++ = '\0';
      ...
      if ((check_value < 0) || (check_value >= ioc_selection_options))
        H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                                "invalid IOC selection type value %ld "
                                "from " H5FD_SUBFILING_IOC_SELECTION_CRITERIA
                                " environment variable",
                                check_value);
...
static herr_t find_cached_topology_info(MPI_Comm comm,
  ...
  for (size_t i = 0; i < sf_topology_cache_num_entries; i++) {
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Comm_compare(comm, cached_topology->app_comm, &result)))
...
static herr_t init_app_layout(sf_topology_t *app_topology, MPI_Comm comm,
  ...
  if (MPI_SUCCESS != (mpi_code = MPI_Comm_size(comm, &app_layout->world_size)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_size failed", mpi_code);
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_rank(node_comm, &app_layout->node_local_rank)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_rank failed", mpi_code);
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_size(node_comm, &app_layout->node_local_size)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_size failed", mpi_code);

  if (NULL == (app_layout->layout = malloc((size_t)app_layout->world_size *
                                           sizeof(*app_layout->layout))))
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
  ...
  if (gather_topology_info(app_layout, comm, node_comm) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                            "can't gather application topology info");
  ...
  if (NULL ==
      (app_layout->node_ranks = malloc((size_t)app_layout->node_count *
                                       sizeof(*app_layout->node_ranks))))
    H5_SUBFILING_GOTO_ERROR(
   ...
   */
  for (size_t i = 0, node_rank_index = 0; i < (size_t)app_layout->world_size;
       i++) {
    if (app_layout->layout[i].node_local_rank == 0) {
...
static herr_t gather_topology_info(app_layout_t *app_layout, MPI_Comm comm,
   ...
   */
  {
    ...
    if (MPI_SUCCESS != (mpi_code = MPI_Comm_group(intra_comm, &node_group)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_group failed", mpi_code);
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Group_translate_ranks(node_group, 1, &local_lead,
                                              file_group, &lead_rank)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Group_translate_ranks failed",
                                  mpi_code);

    if (MPI_UNDEFINED == lead_rank)
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                              "can't determine lead rank on node");
  ...
  if (sf_world_size > 1) {
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Allgather(&my_layout_info, 4, MPI_INT,
                                  app_layout->layout, 4, MPI_INT, comm)))
    ...
    if (MPI_SUCCESS != (mpi_code = MPI_Comm_split(comm, node_local_rank,
                                                  sf_world_rank, &aggr_comm)))
    ...
    /* Allocate a partial layout info array to aggregate into from node-local
     * ranks */
    if (node_local_rank == 0) {
      if (NULL == (layout_info_partial = malloc((size_t)node_local_size *
                                                sizeof(*layout_info_partial))))
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Gather(&my_layout_info, 4, MPI_INT, layout_info_partial,
                               4, MPI_INT, 0, intra_comm)))
    ...
    if (node_local_rank == 0) {
      ...
      if (NULL ==
          (recv_counts = malloc((size_t)aggr_comm_size * sizeof(*recv_counts))))
        H5_SUBFILING_DONE_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                                "can't allocate receive counts array");
      if (NULL ==
          (recv_displs = malloc((size_t)aggr_comm_size * sizeof(*recv_displs))))
        H5_SUBFILING_DONE_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                                "can't allocate receive displacements array");

      if (MPI_SUCCESS !=
          (mpi_code = MPI_Allgather(&send_size, 1, MPI_INT, recv_counts, 1,
                                    MPI_INT, aggr_comm)))
        H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Allgather failed", mpi_code);
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Allgatherv(layout_info_partial, send_size, MPI_INT,
                                     app_layout->layout, recv_counts,
                                     recv_displs, MPI_INT, aggr_comm)))
     ...
     */
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Bcast(app_layout->layout, 4 * sf_world_size, MPI_INT, 0,
                              intra_comm)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_code);
#endif
  }
...
static int compare_layout_nodelocal(const void *layout1, const void *layout2) {
  ...
  if (l1->node_lead_rank == l2->node_lead_rank) {
    return (l1->node_local_rank > l2->node_local_rank) -
           (l1->node_local_rank < l2->node_local_rank);
  } else
    return (l1->node_lead_rank > l2->node_lead_rank) -
           (l1->node_lead_rank < l2->node_lead_rank);
}
...
static herr_t identify_ioc_ranks(sf_topology_t *app_topology, int rank_stride) {
  ...
  if (NULL == (app_topology->io_concentrators = malloc(
                   (size_t)max_iocs * sizeof(*app_topology->io_concentrators))))
    H5_SUBFILING_GOTO_ERROR(
        H5E_RESOURCE, H5E_CANTALLOC, FAIL,
  ...
  case SELECT_IOC_ONE_PER_NODE: {
    ...
    for (size_t i = 0; i < (size_t)app_layout->node_count; i++) {
      ...
      for (size_t j = 1; j < (size_t)iocs_per_node; j++) {
        ...
        assert(total_ioc_count < app_topology->n_io_concentrators);
        io_concentrators[total_ioc_count] =
            app_layout->layout[node_index++].rank;
...
static herr_t init_subfiling_context(subfiling_context_t *sf_context,
  ...
  if ((env_value = HDgetenv(H5FD_SUBFILING_SUBFILE_PREFIX))) {
    if (NULL == (sf_context->subfile_prefix = HDstrdup(env_value)))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "couldn't copy subfile prefix value");
  }
  ...
  sf_context->sf_blocksize_per_stripe =
      sf_context->sf_stripe_size * sf_context->sf_num_subfiles;
  ...
  if (app_topology->rank_is_ioc) {
    ...
    sf_context->sf_base_addr =
        (int64_t)(app_topology->ioc_idx * sf_context->sf_stripe_size);
    ...
    if (NULL == (sf_context->sf_fids = malloc((size_t)sf_context->sf_num_fids *
                                              sizeof(*sf_context->sf_fids))))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "couldn't allocate subfile IDs array");
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_dup(file_comm, &sf_context->sf_msg_comm)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_dup failed", mpi_code);

  if (MPI_SUCCESS != (mpi_code = MPI_Comm_set_errhandler(
                          sf_context->sf_msg_comm, MPI_ERRORS_RETURN)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_set_errhandler failed",
                                mpi_code);

  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_dup(file_comm, &sf_context->sf_data_comm)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_dup failed", mpi_code);

  if (MPI_SUCCESS != (mpi_code = MPI_Comm_set_errhandler(
                          sf_context->sf_data_comm, MPI_ERRORS_RETURN)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_set_errhandler failed",
                                mpi_code);

  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_dup(file_comm, &sf_context->sf_eof_comm)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_dup failed", mpi_code);

  if (MPI_SUCCESS != (mpi_code = MPI_Comm_set_errhandler(
                          sf_context->sf_eof_comm, MPI_ERRORS_RETURN)))
    H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_set_errhandler failed",
                                mpi_code);
  ...
  if (app_topology->n_io_concentrators > 1) {
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Comm_split(file_comm, app_topology->rank_is_ioc,
                                   mpi_rank, &sf_context->sf_group_comm)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_split failed", mpi_code);

    if (MPI_SUCCESS != (mpi_code = MPI_Comm_rank(sf_context->sf_group_comm,
                                                 &sf_context->sf_group_rank)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_rank failed", mpi_code);

    if (MPI_SUCCESS != (mpi_code = MPI_Comm_size(sf_context->sf_group_comm,
                                                 &sf_context->sf_group_size)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Comm_size failed", mpi_code);
  }
  ...
  if (sf_context->sf_stripe_size <= 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                            "invalid subfiling stripe size (%" PRId64 ")",
                            sf_context->sf_stripe_size);

  if (sf_context->sf_num_subfiles <= 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                            "invalid subfiling stripe count (%d)",
...
static herr_t open_subfile_with_context(subfiling_context_t *sf_context,
   ...
   */
  if (record_fid_to_subfile(sf_context->h5_file_id, sf_context->sf_context_id,
                            NULL) < 0)
    H5_SUBFILING_GOTO_ERROR(
        H5E_VFL, H5E_CANTINIT, FAIL,
...
static herr_t record_fid_to_subfile(uint64_t file_id,
  ...
  if (!sf_open_file_map) {
    if (NULL == (sf_open_file_map = malloc((size_t)DEFAULT_FILE_MAP_ENTRIES *
                                           sizeof(*sf_open_file_map))))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "couldn't allocate open file mapping");
  ...
  if (index == sf_file_map_size) {
    ...
    if (NULL == (tmp_realloc =
                     realloc(sf_open_file_map, ((size_t)(sf_file_map_size * 2) *
                                                sizeof(*sf_open_file_map)))))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
...
static herr_t ioc_open_files(int64_t file_context_id, int file_acc_flags) {
  ...
  assert(sf_context->topology->ioc_idx >=
         0); /* Only IOC ranks should be here */

  /* Get the basename of the full HDF5 filename */
  if (H5_basename(sf_context->h5_filename, &base) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                            "can't get HDF5 file basename");
  ...
  if (sf_context->subfile_prefix) {
    if (NULL == (subfile_dir = H5MM_strdup(sf_context->subfile_prefix)))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "couldn't copy subfile prefix");
  } else {
    if (H5_dirname(sf_context->h5_filename, &subfile_dir) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "couldn't get HDF5 file dirname");
  }
  ...
  for (int i = 0; i < sf_context->sf_num_fids; i++) {
    ...
    subfile_idx = (i * sf_context->topology->n_io_concentrators) +
                  sf_context->topology->ioc_idx + 1;
     ...
     */
    HDsnprintf(filepath, PATH_MAX, "%s/" H5FD_SUBFILING_FILENAME_TEMPLATE,
               subfile_dir, base, sf_context->h5_file_id, num_digits,
               subfile_idx, num_subfiles);

    if ((sf_context->sf_fids[i] = HDopen(filepath, file_acc_flags, mode)) < 0)
      H5_SUBFILING_SYS_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, FAIL,
                                  "failed to open subfile");
  }
  ...
  if ((file_acc_flags & O_CREAT) && (sf_context->topology->ioc_idx == 0)) {
    ...
    if (create_config_file(sf_context, base, config_dir, subfile_dir,
 ...
 *              - the number of I/O concentrators used for I/O to the file's
 *subfiles
 *              - the number of subfiles the logical HDF5 file consists of
 *              - the base HDF5 filename
 *              - the optional directory prefix where the file's subfiles are
 *placed
...
static herr_t create_config_file(subfiling_context_t *sf_context,
  ...
  if (sf_context->h5_file_id == UINT64_MAX)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                            "invalid HDF5 file ID %" PRIu64,
                            sf_context->h5_file_id);
  if (*base_filename == '\0')
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                            "invalid base HDF5 filename '%s'", base_filename);
  if (*config_dir == '\0')
    config_dir = ".";
  if (*subfile_dir == '\0')
    subfile_dir = ".";
  ...
  HDsnprintf(config_filename, PATH_MAX,
             "%s/" H5FD_SUBFILING_CONFIG_FILENAME_TEMPLATE, config_dir,
  ...
  if (!config_file_exists || truncate_if_exists) {
    ...
    if (NULL == (line_buf = malloc(PATH_MAX)))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "couldn't allocate buffer for writing to "
    ...
    for (int k = 0; k < n_subfiles; k++) {
      HDsnprintf(line_buf, PATH_MAX, H5FD_SUBFILING_FILENAME_TEMPLATE "\n",
                 base_filename, sf_context->h5_file_id, num_digits, k + 1,
                 n_subfiles);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_get_subfiling_config_from_file(FILE *config_file,
                                         int64_t *stripe_size,
  ...
  if (NULL == (config_buf = malloc((size_t)config_file_len + 1)))
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                            "couldn't allocate space for reading from "
                            "subfiling configuration file");

  if (fread(config_buf, (size_t)config_file_len, 1, config_file) != 1)
    H5_SUBFILING_SYS_GOTO_ERROR(
        H5E_FILE, H5E_READERROR, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_resolve_pathname(const char *filepath, MPI_Comm comm,
                           char **resolved_filepath) {
  ...
  if (mpi_rank == 0) {
    if (NULL == (resolved_path = HDrealpath(filepath, NULL))) {
      if (ENOENT == errno) {
        ...
        H5_SUBFILING_GOTO_ERROR(
            H5E_VFL, H5E_CANTGET, FAIL,
            "can't resolve subfile path, errno = %d, error message = '%s'",
            errno, HDstrerror(errno));
    }

    if (resolved_path) {
      H5_CHECKED_ASSIGN(path_len, hsize_t, (HDstrlen(resolved_path) + 1),
                        size_t);
    } else
  ...
  if (mpi_size > 1) {
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Bcast(&path_len, 1, HSIZE_AS_MPI_TYPE, 0, comm)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_code);
  }
  ...
  if (mpi_rank != 0) {
    if (NULL == (resolved_path = malloc(path_len)))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "can't allocate file name buffer");
  }
  ...
  if (mpi_size > 1) {
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Bcast(resolved_path, (int)path_len, MPI_CHAR, 0, comm)))
      H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_code);
  }
  ...
  *resolved_filepath = resolved_path;
  ...
  if (ret_value < 0) {
    ...
    if (!bcasted_path && (path_len != HSIZE_UNDEF)) {
      H5_CHECK_OVERFLOW(path_len, hsize_t, int);
      if (MPI_SUCCESS != (mpi_code = MPI_Bcast(resolved_path, (int)path_len,
                                               MPI_CHAR, 0, comm)))
        H5_SUBFILING_MPI_DONE_ERROR(FAIL, "MPI_Bcast failed", mpi_code);
    }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_close_subfiles(int64_t subfiling_context_id, MPI_Comm file_comm) {
  subfiling_context_t *sf_context = NULL;
  ...
  if (mpi_size > 1) {
    ...
    while (!barrier_complete) {
      ...
      if (MPI_SUCCESS != (mpi_code = MPI_Test(&barrier_req, &barrier_complete,
                                              MPI_STATUS_IGNORE)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_subfiling_set_config_prop(H5P_genplist_t *plist_ptr,
  ...
  if (!vfd_config)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid subfiling configuration pointer");

  if ((prop_exists = H5P_exist_plist(plist_ptr, H5FD_SUBFILING_CONFIG_PROP)) <
      0)
    H5_SUBFILING_GOTO_ERROR(
        H5E_PLIST, H5E_CANTGET, FAIL,
  ...
  if (prop_exists) {
    ...
    if (H5P_insert(plist_ptr, H5FD_SUBFILING_CONFIG_PROP,
                   sizeof(H5FD_subfiling_params_t),
                   eliminate_const_warning.ptr_to_data, NULL, NULL, NULL, NULL,
                   NULL, NULL, NULL, NULL) < 0)
      H5_SUBFILING_GOTO_ERROR(
          H5E_PLIST, H5E_CANTREGISTER, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_subfiling_get_config_prop(H5P_genplist_t *plist_ptr,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_subfiling_set_file_id_prop(H5P_genplist_t *plist_ptr,
                                     uint64_t file_id) {
  htri_t prop_exists = FAIL;
  herr_t ret_value = SUCCEED;
  ...
  if ((prop_exists = H5P_exist_plist(plist_ptr, H5FD_SUBFILING_STUB_FILE_ID)) <
      0)
  ...
  if (prop_exists) {
    if (H5P_set(plist_ptr, H5FD_SUBFILING_STUB_FILE_ID, &file_id) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                              "can't set file ID property on FAPL");
  } else {
    if (H5P_insert(plist_ptr, H5FD_SUBFILING_STUB_FILE_ID, sizeof(uint64_t),
                   &file_id, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                   NULL) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5_subfiling_get_file_id_prop(H5P_genplist_t *plist_ptr,
                                     uint64_t *file_id) {
  htri_t prop_exists = FAIL;
  herr_t ret_value = SUCCEED;
  ...
  if (prop_exists) {
    if (H5P_get(plist_ptr, H5FD_SUBFILING_STUB_FILE_ID, file_id) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                              "can't get file ID property from FAPL");
  } else
    *file_id = UINT64_MAX;
 ...
 *-------------------------------------------------------------------------
 */
int64_t H5_subfile_fid_to_context(uint64_t file_id) {
  int64_t ret_value = -1;
...
H5_subfiling_validate_config(const H5FD_subfiling_params_t *subf_config) {
  ...
  if (!subf_config)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "NULL subfiling configuration pointer");
  ...
  if (ioc_sel_type != SELECT_IOC_ONE_PER_NODE &&
      ioc_sel_type != SELECT_IOC_EVERY_NTH_RANK &&
      ioc_sel_type != SELECT_IOC_WITH_CONFIG &&
      ioc_sel_type != SELECT_IOC_TOTAL)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid IOC selection method");

  if (subf_config->stripe_size <= 0)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid stripe size");

  if (subf_config->stripe_count <= 0 &&
      subf_config->stripe_count != H5FD_SUBFILING_DEFAULT_STRIPE_COUNT)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
...
#ifdef H5_SUBFILING_DEBUG
void H5_subfiling_log(int64_t sf_context_id, const char *fmt, ...) {
  subfiling_context_t *sf_context = NULL;
  va_list log_args;
