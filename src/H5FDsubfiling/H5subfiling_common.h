...
typedef struct app_layout_t {
  layout_t
      *layout; /* Array of (rank, node local rank, node local size) values */
...
typedef struct topology {
  ...
  H5FD_subfiling_ioc_select_t
      selection_type; /* Cache our IOC selection criteria   */
} sf_topology_t;
...
H5_DLL herr_t H5_open_subfiling_stub_file(const char *name, unsigned flags,
...
H5_DLL herr_t H5_get_subfiling_config_from_file(FILE *config_file,
                                                int64_t *stripe_size,
                                                int64_t *num_subfiles);
H5_DLL herr_t H5_resolve_pathname(const char *filepath, MPI_Comm comm,
                                  char **resolved_filepath);

H5_DLL herr_t H5_subfiling_set_config_prop(
    H5P_genplist_t *plist_ptr, const H5FD_subfiling_params_t *vfd_config);
H5_DLL herr_t H5_subfiling_get_config_prop(H5P_genplist_t *plist_ptr,
                                           H5FD_subfiling_params_t *vfd_config);
H5_DLL herr_t H5_subfiling_set_file_id_prop(H5P_genplist_t *plist_ptr,
                                            uint64_t file_id);
H5_DLL herr_t H5_subfiling_get_file_id_prop(H5P_genplist_t *plist_ptr,
                                            uint64_t *file_id);
