...
typedef struct H5FD_subfiling_t {
  ...
  uint64_t file_id;
  int64_t context_id; /* The value used to lookup a subfiling context for the
                         file */

  hbool_t
      fail_to_encode; /* Used to check for failures from sb_get_size routine */
...
static herr_t H5FD__subfiling_sb_encode(H5FD_t *_file, char *name,
                                        unsigned char *buf);
static herr_t H5FD__subfiling_sb_decode(H5FD_t *_file, const char *name,
...
static herr_t
H5FD__subfiling_get_default_config(hid_t fapl_id,
...
static herr_t init_indep_io(subfiling_context_t *sf_context,
                            int64_t file_offset, size_t io_nelemts,
                            size_t dtype_extent, size_t max_iovec_len,
                            int64_t *mem_buf_offset,
                            int64_t *target_file_offset, int64_t *io_block_len,
                            int *first_subfile_index, int *n_subfiles_used,
                            int64_t *max_io_req_per_subfile);
static herr_t iovec_fill_first(subfiling_context_t *sf_context,
                               int64_t iovec_depth, int64_t target_datasize,
                               int64_t start_mem_offset,
                               int64_t start_file_offset, int64_t first_io_len,
                               int64_t *mem_offset_out,
                               int64_t *target_file_offset_out,
                               int64_t *io_block_len_out);
static herr_t iovec_fill_last(subfiling_context_t *sf_context,
                              int64_t iovec_depth, int64_t target_datasize,
                              int64_t start_mem_offset,
                              int64_t start_file_offset, int64_t last_io_len,
                              int64_t *mem_offset_out,
                              int64_t *target_file_offset_out,
                              int64_t *io_block_len_out);
static herr_t
iovec_fill_first_last(subfiling_context_t *sf_context, int64_t iovec_depth,
                      int64_t target_datasize, int64_t start_mem_offset,
                      int64_t start_file_offset, int64_t first_io_len,
                      int64_t last_io_len, int64_t *mem_offset_out,
                      int64_t *target_file_offset_out,
                      int64_t *io_block_len_out);
static herr_t
iovec_fill_uniform(subfiling_context_t *sf_context, int64_t iovec_depth,
                   int64_t target_datasize, int64_t start_mem_offset,
                   int64_t start_file_offset, int64_t *mem_offset_out,
...
hid_t H5FD_subfiling_init(void) {
  ...
  if (H5I_VFL != H5I_get_type(H5FD_SUBFILING_g)) {
    ...
    if ((H5FD_SUBFILING_g =
             H5FD_register(&H5FD_subfiling_g, sizeof(H5FD_class_t), FALSE)) < 0)
    ...
    if ((H5subfiling_err_class_g = H5Eregister_class(H5SUBFILING_ERR_CLS_NAME,
                                                     H5SUBFILING_ERR_LIB_NAME,
    ...
    if (MPI_SUCCESS != (mpi_code = MPI_Initialized(&mpi_initialized)))
      H5_SUBFILING_MPI_GOTO_ERROR(H5I_INVALID_HID, "MPI_Initialized failed",
                                  mpi_code);
    if (mpi_initialized) {
      /* If MPI is initialized, validate that it was initialized with
       * MPI_THREAD_MULTIPLE */
      if (MPI_SUCCESS != (mpi_code = MPI_Query_thread(&provided)))
        H5_SUBFILING_MPI_GOTO_ERROR(H5I_INVALID_HID, "MPI_Query_thread failed",
                                    mpi_code);
      if (provided != MPI_THREAD_MULTIPLE)
        H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTINIT, H5I_INVALID_HID,
                                "Subfiling VFD requires the use of "
                                "MPI_Init_thread with MPI_THREAD_MULTIPLE");
    } else {
      int required = MPI_THREAD_MULTIPLE;

      if (MPI_SUCCESS !=
          (mpi_code = MPI_Init_thread(NULL, NULL, required, &provided)))
        H5_SUBFILING_MPI_GOTO_ERROR(H5I_INVALID_HID, "MPI_Init_thread failed",
                                    mpi_code);
     ...
     */
    HDcompile_assert(sizeof(((sf_work_request_t *)NULL)->header) ==
                     3 * sizeof(int64_t));
    if (H5_subfiling_rpc_msg_type == MPI_DATATYPE_NULL) {
      if (MPI_SUCCESS != (mpi_code = MPI_Type_contiguous(
        ...
        H5_SUBFILING_MPI_GOTO_ERROR(H5I_INVALID_HID,
                                    "MPI_Type_contiguous failed", mpi_code);
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Type_commit(&H5_subfiling_rpc_msg_type)))
        H5_SUBFILING_MPI_GOTO_ERROR(H5I_INVALID_HID, "MPI_Type_commit failed",
                                    mpi_code);
    }
  }
...
static herr_t H5FD__subfiling_term(void) {
  ...
  if (H5FD_SUBFILING_g >= 0) {
    ...
    if (H5_subfiling_rpc_msg_type != MPI_DATATYPE_NULL) {
      if (!mpi_finalized) {
        if (MPI_SUCCESS !=
            (mpi_code = MPI_Type_free(&H5_subfiling_rpc_msg_type)))
          H5_SUBFILING_MPI_GOTO_ERROR(FAIL, "MPI_Type_free failed", mpi_code);
      }
#ifdef H5FD_SUBFILING_DEBUG
      else
        printf("** WARNING **: HDF5 is terminating the Subfiling VFD after "
               "MPI_Finalize() was "
    ...
    if (H5subfiling_err_stack_g >= 0) {
      ...
      if (H5Eclose_stack(H5subfiling_err_stack_g) < 0) {
        H5_SUBFILING_DONE_ERROR(H5E_VFL, H5E_CLOSEERROR, FAIL,
...
herr_t H5Pset_fapl_subfiling(hid_t fapl_id,
  ...
  if (H5FD_subfiling_init() < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTINIT, FAIL,
                            "can't initialize subfiling VFD");

  if (NULL == (plist = H5P_object_verify(fapl_id, H5P_FILE_ACCESS)))
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
  ...
  if (H5_mpi_info_free(&info) < 0)
    H5_SUBFILING_DONE_ERROR(H5E_VFL, H5E_CANTFREE, FAIL,
                            "can't free MPI Info object");

  if (subfiling_conf) {
    if (subfiling_conf->ioc_fapl_id >= 0 &&
        H5I_dec_ref(subfiling_conf->ioc_fapl_id) < 0)
      H5_SUBFILING_DONE_ERROR(H5E_PLIST, H5E_CANTDEC, FAIL,
                              "can't close IOC FAPL");
    free(subfiling_conf);
  }
...
herr_t H5Pget_fapl_subfiling(hid_t fapl_id,
  ...
  if (use_default_config) {
    ...
    if (H5FD__copy_plist(config_ptr->ioc_fapl_id, &(config_out->ioc_fapl_id)) <
        0)
...
H5FD__subfiling_get_default_config(hid_t fapl_id,
  ...
  if (ret_value < 0) {
    if (config_out->ioc_fapl_id >= 0 && H5Pclose(config_out->ioc_fapl_id) < 0)
      H5_SUBFILING_DONE_ERROR(H5E_PLIST, H5E_CANTCLOSEOBJ, FAIL,
                              "can't close FAPL");
    config_out->ioc_fapl_id = H5I_INVALID_HID;
  }
...
H5FD__subfiling_validate_config(const H5FD_subfiling_config_t *fa) {
  ...
  if (fa->magic != H5FD_SUBFILING_FAPL_MAGIC)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid H5FD_subfiling_config_t magic value");
  ...
  if (H5_subfiling_validate_config(&fa->shared_cfg) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid subfiling configuration parameters");
...
static herr_t H5FD__subfiling_sb_encode(H5FD_t *_file, char *name,
  ...
  if (file->fail_to_encode)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTENCODE, FAIL,
                            "can't encode subfiling driver info message - "
                            "message was too large or internal error occurred");

  if (NULL == (sf_context = H5_get_subfiling_object(file->context_id)))
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                            "can't get subfiling context object");
...
static herr_t H5FD__subfiling_sb_decode(H5FD_t *_file, const char *name,
  ...
  if (file->fail_to_encode)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTDECODE, FAIL,
                            "can't decode subfiling driver info message - "
                            "message wasn't encoded (or encoded improperly)");
  ...
  if (HDstrncmp(name, "Subfilin", 9))
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                            "invalid driver name in superblock");
  ...
  if (file->fa.shared_cfg.stripe_count != sf_context->sf_num_subfiles)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                            "specified subfiling stripe count (%d) doesn't "
                            "match value stored in file (%" PRId32 ")",
...
static void *H5FD__subfiling_fapl_get(H5FD_t *_file) {
  ...
  if (fa == NULL) {
    H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
...
static int H5FD__copy_plist(hid_t fapl_id, hid_t *id_out_ptr) {
  ...
  if (FALSE == H5P_isa_class(fapl_id, H5P_FILE_ACCESS))
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADTYPE, -1,
                            "not a file access property list");
  ...
  *id_out_ptr = H5P_copy_plist(plist_ptr, FALSE);
  if (H5I_INVALID_HID == *id_out_ptr)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADTYPE, -1,
...
static H5FD_t *H5FD__subfiling_open(const char *name, unsigned flags,
  ...
  if (NULL == (file_ptr = (H5FD_subfiling_t *)H5FL_CALLOC(H5FD_subfiling_t)))
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL,
  ...
  if (NULL == (plist_ptr = (H5P_genplist_t *)H5I_object(fapl_id)))
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                            "not a file access property list");
  ...
  if (H5FD_mpi_self_initialized) {
    ...
    if (file_ptr->comm == MPI_COMM_NULL)
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL,
                              "invalid or unset MPI communicator in FAPL");
  }

  /* Get the MPI rank of this process and the total number of processes */
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_rank(file_ptr->comm, &file_ptr->mpi_rank)))
    H5_SUBFILING_MPI_GOTO_ERROR(NULL, "MPI_Comm_rank failed", mpi_code);
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Comm_size(file_ptr->comm, &file_ptr->mpi_size)))
    H5_SUBFILING_MPI_GOTO_ERROR(NULL, "MPI_Comm_size failed", mpi_code);

  /* Work around an HDF5 metadata cache bug with distributed metadata writes
   * when MPI size == 1 */
  if (file_ptr->mpi_size == 1) {
    H5AC_cache_config_t mdc_config;

    /* Get the current initial metadata cache resize configuration */
    if (H5P_get(plist_ptr, H5F_ACS_META_CACHE_INIT_CONFIG_NAME, &mdc_config) <
        0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                              "can't get metadata cache initial config");
    mdc_config.metadata_write_strategy =
        H5AC_METADATA_WRITE_STRATEGY__PROCESS_0_ONLY;
    if (H5P_set(plist_ptr, H5F_ACS_META_CACHE_INIT_CONFIG_NAME, &mdc_config) <
        0)
      H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTSET, NULL,
  ...
  if (H5FD__copy_plist(config_ptr->ioc_fapl_id, &(file_ptr->fa.ioc_fapl_id)) <
      0) {
    file_ptr->fa.ioc_fapl_id = H5I_INVALID_HID;
  ...
  if (driver->value != H5_VFD_IOC)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                            "unable to open file '%s' - only IOC VFD is "
  ...
  if (H5_resolve_pathname(name, file_ptr->comm, &file_ptr->file_path) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTGET, NULL,
                            "can't resolve filepath");
  if (H5_dirname(file_ptr->file_path, &file_ptr->file_dir) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTGET, NULL,
                            "can't get filepath dirname");

  /*
   * Create/open the HDF5 stub file and get its inode value for
   * the internal mapping from file inode to subfiling context.
   */
  if (H5_open_subfiling_stub_file(file_ptr->file_path, flags, file_ptr->comm,
                                  &file_ptr->stub_file, &file_ptr->file_id) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTOPENFILE, NULL,
                            "can't open HDF5 stub file");

  /* Set stub file ID on IOC fapl so it can reuse on open */
  if (H5_subfiling_set_file_id_prop(plist_ptr, file_ptr->file_id) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_CANTSET, NULL,
                            "can't set stub file ID on FAPL");

  /* Open the HDF5 file's subfiles */
  if (NULL == (file_ptr->sf_file = H5FD_open(
                   name, flags, file_ptr->fa.ioc_fapl_id, HADDR_UNDEF)))
    H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTOPENFILE, NULL,
  ...
  if (driver->value == H5_VFD_IOC) {
     ...
     */
    if (H5_open_subfiles(file_ptr->file_path, file_ptr->file_id,
                         &file_ptr->fa.shared_cfg, ioc_flags, file_ptr->comm,
  ...
  if (file_ptr->mpi_size > 1) {
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Bcast(&sf_eof, 1, MPI_INT64_T, 0, file_ptr->comm)))
  ...
  if (sf_eof < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTGET, NULL,
                            "lead MPI process failed to get file EOF");
  ...
  if (config_ptr == &default_config)
    if (H5I_dec_ref(config_ptr->ioc_fapl_id) < 0)
      H5_SUBFILING_DONE_ERROR(H5E_PLIST, H5E_CANTCLOSEOBJ, NULL,
                              "can't close IOC FAPL");
    ...
    if (file_ptr) {
      ...
      if (file_ptr->comm != MPI_COMM_NULL) {
        if (!bcasted_eof) {
          ...
          if (file_ptr->mpi_size > 1) {
            if (MPI_SUCCESS != (mpi_code = MPI_Bcast(&sf_eof, 1, MPI_INT64_T, 0,
                                                     file_ptr->comm)))
      ...
      if (H5FD__subfiling_close_int(file_ptr) < 0)
        H5_SUBFILING_DONE_ERROR(H5E_FILE, H5E_CLOSEERROR, NULL,
                                "couldn't close file");
    }
  }
...
static herr_t H5FD__subfiling_close_int(H5FD_subfiling_t *file_ptr) {
  ...
  if (file_ptr->sf_file && H5FD_close(file_ptr->sf_file) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTCLOSEFILE, FAIL,
                            "unable to close subfile");
  if (file_ptr->stub_file && H5FD_close(file_ptr->stub_file) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTCLOSEFILE, FAIL,
                            "unable to close HDF5 stub file");

  /* if set, close the copy of the plist for the underlying VFD. */
  if ((file_ptr->fa.ioc_fapl_id >= 0) &&
...
static herr_t H5FD__subfiling_close(H5FD_t *_file) {
  ...
  if (H5FD__subfiling_close_int(file_ptr) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTCLOSEFILE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__subfiling_query(const H5FD_t H5_ATTR_UNUSED *_file,
...
static herr_t H5FD__subfiling_set_eoa(H5FD_t *_file, H5FD_mem_t type,
  ...
  if (file_ptr->mpi_rank == 0) {
    if (H5FD_set_eoa(file_ptr->stub_file, type, addr) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTSET, FAIL,
                              "can't set HDF5 stub file EOA");
  }
...
static herr_t H5FD__subfiling_read(H5FD_t *_file, H5FD_mem_t type,
  ...
  if (!H5_addr_defined(addr))
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "addr undefined, addr = %" PRIuHADDR, addr);
  if (REGION_OVERFLOW(addr, size))
    H5_SUBFILING_GOTO_ERROR(
        H5E_ARGS, H5E_OVERFLOW, FAIL,
        "addr overflow, addr = %" PRIuHADDR ", size = %" PRIuHADDR, addr, size);

  /* Temporarily reject collective I/O until support is implemented (unless
   * types are simple MPI_BYTE) */
  {
   ...
   */
  sf_context =
      (subfiling_context_t *)H5_get_subfiling_object(file_ptr->context_id);
  assert(sf_context);
  ...
  if (num_subfiles <= 0) {
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                            "invalid number of subfiles (%d)", num_subfiles);
     ...
     */
    if (NULL ==
        (source_data_offset = calloc(1, (size_t)num_subfiles * max_depth *
                                            sizeof(*source_data_offset))))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "can't allocate source data offset I/O vector");
    if (NULL == (sf_data_size = calloc(1, (size_t)num_subfiles * max_depth *
                                              sizeof(*sf_data_size))))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                              "can't allocate subfile data size I/O vector");
    if (NULL == (sf_offset = calloc(1, (size_t)num_subfiles * max_depth *
                                           sizeof(*sf_offset))))
      H5_SUBFILING_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
     ...
     */
    status = init_indep_io(
        sf_context,         /* IN: Context used to look up config info */
        ...
        &first_subfile_idx, /* OUT: Subfile index corresponding to starting
                               offset */
        &num_subfiles_used, /* OUT: Number of actual subfiles used */
        &max_io_req_per_subfile); /* OUT: Maximum number of requests to any
                                     subfile */

    if (status < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTINIT, FAIL,
                              "can't initialize IOC transactions");
    ...
    if (max_io_req_per_subfile > 0) {
      ...
      for (int64_t i = 0; i < max_io_req_per_subfile; i++) {
        ...
        for (uint32_t k = 0, vec_idx = 0; k < vector_len; k++) {
          ...
          H5_CHECKED_ASSIGN(io_addrs[vec_idx], haddr_t, sf_offset[idx],
                            int64_t);
          H5_CHECKED_ASSIGN(io_sizes[vec_idx], size_t, sf_data_size[idx],
                            int64_t);
        ...
        if (!rank0_bcast || (file_ptr->mpi_rank == 0)) {
          /* Make vector read call to subfile */
          if (H5FD_read_vector(file_ptr->sf_file, final_vec_len, io_types,
                               io_addrs, io_sizes, io_bufs) < 0)
            H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                                    "read from subfile failed");
        }
      }
      ...
      if (rank0_bcast && (file_ptr->mpi_size > 1)) {
        ...
        if (MPI_SUCCESS !=
            MPI_Bcast(buf, (int)size, MPI_BYTE, 0, file_ptr->comm))
          H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                                  "can't broadcast data from rank 0");
      }
    }
  }
...
static herr_t H5FD__subfiling_write(H5FD_t *_file, H5FD_mem_t type,
  ...
  if (num_subfiles <= 0) {
    ...
    if (H5FD_write_vector(file_ptr->sf_file, 1, &type, &addr, &size, &buf) < 0)
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                              "write to subfile failed");
    ...
    if ((type == H5FD_MEM_SUPER) && (file_ptr->mpi_rank == 0)) {
      if (H5FD_write_vector(file_ptr->stub_file, 1, &type, &addr, &size, &buf) <
          0)
        H5_SUBFILING_GOTO_ERROR(
            H5E_IO, H5E_WRITEERROR, FAIL,
    ...
    if (max_io_req_per_subfile > 0) {
      ...
      for (int64_t i = 0; i < max_io_req_per_subfile; i++) {
        ...
        /* Make vector write call to subfile */
        if (H5FD_write_vector(file_ptr->sf_file, final_vec_len, io_types,
                              io_addrs, io_sizes, io_bufs) < 0)
          H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                                  "write to subfile failed");
         ...
         */
        if (file_ptr->mpi_rank == 0) {
          for (size_t count_idx = 0; count_idx < (size_t)final_vec_len;
               count_idx++) {
            if (io_types[count_idx] == H5FD_MEM_SUPER) {
              if (H5FD_write(file_ptr->stub_file, H5FD_MEM_SUPER,
                             io_addrs[count_idx], io_sizes[count_idx],
                             io_bufs[count_idx]) < 0)
                H5_SUBFILING_GOTO_ERROR(
                    H5E_IO, H5E_WRITEERROR, FAIL,
                    "couldn't write superblock information to stub file");
...
static herr_t H5FD__subfiling_read_vector(H5FD_t *_file, hid_t dxpl_id,
  ...
  if (file_ptr->fa.require_ioc) {
    ...
    for (k = 0; k < (int)count; k++) {
      ...
      if (H5FD__subfiling_read(_file, type, dxpl_id, addrs[k], size, bufs[k]) !=
          SUCCEED)
        H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
...
static herr_t H5FD__subfiling_write_vector(H5FD_t *_file, hid_t dxpl_id,
  ...
  /* Check arguments
   * RAW - Do we really need to check arguments once again?
   * These have already been checked in H5FD__subfiling_write_vector (see
   * below)!
   */
  if (!file_ptr)
    H5_SUBFILING_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
  ...
  if (file_ptr->fa.require_ioc) {
    ...
    for (k = 0; k < (int)count; k++) {
      ...
      if (H5FD__subfiling_write(_file, type, dxpl_id, addrs[k], size,
                                bufs[k]) != SUCCEED)
        H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
...
static herr_t H5FD__subfiling_truncate(H5FD_t *_file,
  ...
  if (!H5_addr_eq(file->eoa, file->last_eoa)) {
    ...
    if (file->mpi_size > 1) {
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Bcast(&sf_eof, 1, MPI_INT64_T, 0, file->comm)))
    ...
    /* truncate subfiles */
    /* This is a hack.  We should be doing the truncate of the subfiles via
     * calls to H5FD_truncate() with the IOC.  However, that system is messed up
     * at present. thus the following hack. JRM -- 12/18/21
     */
    if (H5FD__subfiling__truncate_sub_files(file->context_id, eoa, file->comm) <
        0)
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_CANTUPDATE, FAIL,
                              "subfile truncate request failed");
...
static herr_t H5FD__subfiling_del(const char *name, hid_t fapl) {
  ...
  if (H5FD_SUBFILING != H5P_peek_driver(plist))
    H5_SUBFILING_GOTO_ERROR(H5E_PLIST, H5E_BADVALUE, FAIL,
                            "incorrect driver set on FAPL");
  ...
  if (H5FD_delete(name, subfiling_config->ioc_fapl_id) < 0)
    H5_SUBFILING_GOTO_ERROR(H5E_FILE, H5E_CANTDELETE, FAIL,
...
static herr_t H5FD__subfiling_ctl(H5FD_t *_file, uint64_t op_code,
  ...
  default: /* unknown op code */
    if (flags & H5FD_CTL_FAIL_IF_UNKNOWN_FLAG) {
      H5_SUBFILING_GOTO_ERROR(H5E_VFL, H5E_FCNTL, FAIL,
...
static herr_t init_indep_io(subfiling_context_t *sf_context,
  ...
  *max_io_req_per_subfile = 0;
  ...
  max_iovec_depth =
      ((row_stripe_idx_final - row_stripe_idx_start) / num_subfiles) + 1;
  ...
  *max_io_req_per_subfile = max_iovec_depth;
  ...
  H5_subfiling_log(sf_context->sf_context_id,
                   "%s: FILE OFFSET = %" PRId64
                   ", DATA SIZE = %zu, STRIPE SIZE = %" PRId64,
                   __func__, file_offset, io_nelemts, stripe_size);
  H5_subfiling_log(sf_context->sf_context_id,
                   "%s: FIRST SUBFILE = %" PRId64 ", LAST SUBFILE = %" PRId64
                   ", "
                   "MAX IOVEC DEPTH = %" PRId64 ", START LENGTH = %" PRId64
                   ", FINAL LENGTH = %" PRId64,
                   __func__, first_subfile, last_subfile, max_iovec_depth,
  ...
  for (int i = 0, k = (int)first_subfile; i < num_subfiles; i++) {
    ...
    memset(_target_file_offset, 0,
           (max_iovec_len * sizeof(*_target_file_offset)));
    ...
    if (total_bytes < data_size) {
      ...
      if (!is_first && !is_last) {
        ...
        if (last_subfile < first_subfile) {
           ...
           */
          thin_uniform_section = thin_uniform_section ||
                                 ((last_subfile < k) && (k < first_subfile));
        }
    ...
    if (num_subfiles > 1) {
      ...
      if (is_first) {
        if (is_last) { /* First + Last */
          if (iovec_fill_first_last(sf_context, iovec_depth, subfile_bytes,
                                    mem_offset, curr_file_offset, start_length,
                                    final_length, _mem_buf_offset,
                                    _target_file_offset, _io_block_len) < 0)
            H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTINIT, FAIL,
                                    "can't fill I/O vectors");
        } else { /* First ONLY */
          if (iovec_fill_first(sf_context, iovec_depth, subfile_bytes,
                               mem_offset, curr_file_offset, start_length,
                               _mem_buf_offset, _target_file_offset,
                               _io_block_len) < 0)
            H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTINIT, FAIL,
                                    "can't fill I/O vectors");
        }
        ...
        if (iovec_fill_last(sf_context, iovec_depth, subfile_bytes, mem_offset,
                            curr_file_offset, final_length, _mem_buf_offset,
                            _target_file_offset, _io_block_len) < 0)
          H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTINIT, FAIL,
                                  "can't fill I/O vectors");
        ...
        if (iovec_fill_uniform(sf_context, iovec_depth, subfile_bytes,
                               mem_offset, curr_file_offset, _mem_buf_offset,
                               _target_file_offset, _io_block_len) < 0)
          H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTINIT, FAIL,
                                  "can't fill I/O vectors");
    ...
    if (k == num_subfiles) {
      k = 0;
      offset_in_block = 0;
      curr_max_iovec_depth =
          ((final_stripe_idx - curr_stripe_idx) / num_subfiles) + 1;
...
static herr_t iovec_fill_first(subfiling_context_t *sf_context,
...
#ifdef H5_SUBFILING_DEBUG
  H5_subfiling_log(sf_context->sf_context_id,
                   "%s: start_mem_offset = %" PRId64
                   ", start_file_offset = %" PRId64 ", first_io_len = %" PRId64,
  ...
  H5_subfiling_log(sf_context->sf_context_id,
                   ...
                   __func__, mem_offset_out[0], target_file_offset_out[0],
                   io_block_len_out[0]);
#endif
  ...
  if (first_io_len > 0) {
    ...
    int64_t next_file_offset =
        start_file_offset + (block_size - offset_in_stripe);
...
#ifdef H5_SUBFILING_DEBUG
      H5_subfiling_log(sf_context->sf_context_id,
                       "%s: mem_offset[%" PRId64 "] = %" PRId64
                       ", file_offset[%" PRId64 "] = %" PRId64
                       ", io_block_len[%" PRId64 "] = %" PRId64,
                       __func__, i, mem_offset_out[i], i,
                       target_file_offset_out[i], i, io_block_len_out[i]);
#endif
    ...
    if (total_bytes != target_datasize)
      H5_SUBFILING_GOTO_ERROR(H5E_IO, H5E_CANTINIT, FAIL,
                              "total bytes (%" PRId64
                              ") didn't match target data size (%" PRId64 ")!",
...
static herr_t iovec_fill_last(subfiling_context_t *sf_context,
  ...
  H5_subfiling_log(sf_context->sf_context_id,
                   "%s: start_mem_offset = %" PRId64
                   ", start_file_offset = %" PRId64 ", last_io_len = %" PRId64,
...
#ifdef H5_SUBFILING_DEBUG
    H5_subfiling_log(sf_context->sf_context_id,
                     "%s: mem_offset[0] = %" PRId64
                     ", file_offset[0] = %" PRId64
                     ", io_block_len[0] = %" PRId64,
                     __func__, mem_offset_out[0], target_file_offset_out[0],
                     io_block_len_out[0]);
#endif
...
iovec_fill_uniform(subfiling_context_t *sf_context, int64_t iovec_depth,
  ...
  H5_subfiling_log(sf_context->sf_context_id,
                   "%s: start_mem_offset = %" PRId64
                   ", start_file_offset = %" PRId64 ", segment size = %" PRId64,
...
#ifdef H5_SUBFILING_DEBUG
    H5_subfiling_log(sf_context->sf_context_id, "%s: target_datasize = 0",
                     __func__);
#endif
