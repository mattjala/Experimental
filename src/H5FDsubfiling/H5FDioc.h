...
/**
 * \def H5FD_IOC
 * Macro that returns the identifier for the #H5FD_IOC driver. \hid_t{file
 * driver}
 */
 ...
 * Unique number used to distinguish the #H5FD_IOC driver from other HDF5 file
 * drivers
 */
 ...
 * #H5FD_IOC_THREAD_POOL_SIZE environment variable.
 *
 */
typedef struct H5FD_ioc_config_t {
  ...
  int32_t
      thread_pool_size; /* Number of I/O concentrator worker threads to use */
} H5FD_ioc_config_t;
//! <!-- [H5FD_ioc_config_t_snip] -->
 ...
 * \ingroup FAPL
 *
 * \brief Modifies the specified File Access Property List to use the #H5FD_IOC
 * driver
 *
 * \fapl_id
 * \param[in] vfd_config Pointer to #H5FD_IOC driver configuration structure.
 * May be NULL. \returns \herr_t
 *
 * \details H5Pset_fapl_ioc() modifies the File Access Property List to use the
 *          #H5FD_IOC driver.
 *
 *          The #H5FD_IOC driver is a reference implementation of an "I/O
 * concentrator" file driver that works in conjunction with the #H5FD_SUBFILING
 * driver and provides the I/O backend for servicing I/O requests to subfiles.
 *
 *          Typically, an HDF5 application won't need to call this routine
 * directly. The #H5FD_IOC driver is usually set up as a side effect of an HDF5
 * application using the #H5FD_SUBFILING driver, but this routine is provided in
 * case the application wishes to manually configure the #H5FD_IOC driver.
 *
 ...
 *       If the two drivers differ in configuration settings, application
 ...
 * \ingroup FAPL
 *
 * \brief Queries a File Access Property List for #H5FD_IOC file driver
 * properties
 *
 * \fapl_id
 * \param[out] config_out Pointer to H5FD_ioc_config_t structure through which
 * the #H5FD_IOC file driver properties will be returned.
 *
 * \returns \herr_t
 *
 * \details H5Pget_fapl_ioc() queries the specified File Access Property List
 * for #H5FD_IOC driver properties as set by H5Pset_fapl_ioc(). If the #H5FD_IOC
 *          driver has not been set on the File Access Property List, a default
 *          configuration is returned. An HDF5 application may use this
 * functionality to manually configure the #H5FD_IOC driver by calling
 * H5Pget_fapl_ioc() on a newly-created File Access Property List, adjusting the
 * default values and then calling H5Pset_fapl_ioc() with the configured
