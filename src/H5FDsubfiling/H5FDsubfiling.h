...
/**
 * \def H5FD_SUBFILING
 * Macro that returns the identifier for the #H5FD_SUBFILING driver. \hid_t{file
 * driver}
 */
 ...
 * Unique number used to distinguish the #H5FD_SUBFILING driver from other HDF5
 * file drivers
 */
 ...
 */
#define H5FD_SUBFILING_IOC_SELECTION_CRITERIA                                  \
...
typedef enum {
  ...
  /* NOTE: Add to the Fortran constants (H5f90global.F90)  when adding new
     entries */
} H5FD_subfiling_ioc_select_t;

/**
 * \struct H5FD_subfiling_params_t
 * \brief Subfiling parameter structure that is shared between the
 * #H5FD_SUBFILING and #H5FD_IOC drivers
 *
 ...
 *      current default is to select one MPI rank per node to be an I/O
 * concentrator. Refer to #H5FD_subfiling_ioc_select_t for a description of the
 ...
 *      in a round-robin wrap-around fashion in segments equal to the stripe
 * size.
 *
 *      For example, in an HDF5 file consisting of four subfiles with a 1MiB
 * stripe size, the first and fifth 1MiB of data would reside in the first
 * subfile, the second and sixth 1MiB of data would reside in the second subfile
 * and so on.
 *
 *      This value can also be set or adjusted with the
 * #H5FD_SUBFILING_STRIPE_SIZE environment variable.
 *
 * \var int32_t H5FD_subfiling_params_t::stripe_count
 *      The target number of subfiles to use for the logical HDF5 file. The
 * current default is to use one subfile per node, but it can be useful to set a
 *      different target number of subfiles, especially if the HDF5 application
 * will pre-create the HDF5 file on a single MPI rank. In that particular case,
 * the single rank will need to know how many subfiles the logical HDF5 file
 * will consist of in order to properly pre-create the file.
 *
 *      This value is used in conjunction with the IOC selection method to
 * determine which MPI ranks will be assigned as I/O concentrators.
 * Alternatively, the mapping between MPI ranks and I/O concentrators can be set
 * or adjusted with a combination of the #ioc_selection field and the
 * #H5FD_SUBFILING_IOC_PER_NODE and #H5FD_SUBFILING_IOC_SELECTION_CRITERIA
 * environment variables.
 */
typedef struct H5FD_subfiling_params_t {
  H5FD_subfiling_ioc_select_t
      ioc_selection;    /* Method to select I/O concentrators          */
 ...
 * \brief Configuration structure for H5Pset_fapl_subfiling() /
 ...
 *          a File Access Property List. A pointer to an instance of this
 * structure is a parameter to H5Pset_fapl_subfiling() and
 * H5Pget_fapl_subfiling().
 *
 ...
 *      the #H5FD_IOC driver by calling H5Pset_fapl_ioc(), but future
 * development may allow other file drivers to be used.
 *
 * \var hbool_t H5FD_subfiling_config_t::require_ioc
 *      A boolean flag which indicates whether the #H5FD_SUBFILING driver should
 *      use the #H5FD_IOC driver for its I/O operations. This field should
 * currently always be set to TRUE.
 *
 * \var H5FD_subfiling_params_t H5FD_subfiling_config_t::shared_cfg
 *      A structure which contains the subfiling parameters that are shared
 * between the #H5FD_SUBFILING and #H5FD_IOC drivers. This includes the subfile
 * stripe size, stripe count, IOC selection method, etc.
 *
 */
typedef struct H5FD_subfiling_config_t {
  ...
  hid_t ioc_fapl_id;   /* The FAPL setup with the stacked VFD to use for I/O
                          concentrators */
  hbool_t require_ioc; /* Whether to use the IOC VFD (currently must always be
                          TRUE)       */
  H5FD_subfiling_params_t shared_cfg; /* Subfiling/IOC parameters (stripe size,
                                         stripe count, etc.)       */
 ...
 * \ingroup FAPL
 *
 * \brief Modifies the specified File Access Property List to use the
 * #H5FD_SUBFILING driver
 *
 * \fapl_id
 * \param[in] vfd_config Pointer to #H5FD_SUBFILING driver configuration
 * structure. May be NULL. \returns \herr_t
 *
 * \details H5Pset_fapl_subfiling() modifies the File Access Property List to
 * use the #H5FD_SUBFILING driver.
 *
 *          The #H5FD_SUBFILING driver is an MPI-based file driver that allows
 * an HDF5 application to distribute a logical HDF5 file across a collection of
 * "subfiles" in equal-sized data segment "stripes". I/O to the logical HDF5
 * file is then directed to the appropriate "subfile" according to the
 *          #H5FD_SUBFILING configuration and a system of I/O concentrators,
 * which are MPI ranks operating worker threads.
 *
 *          By allowing a configurable stripe size, number of I/O concentrators
 * and method for selecting MPI ranks as I/O concentrators, the #H5FD_SUBFILING
 *          driver aims to enable an HDF5 application to find a middle ground
 * between the single shared file and file-per-process approaches to parallel
 * file I/O for the particular machine the application is running on. In
 * general, the goal is to avoid some of the complexity of the file-per-process
 * approach while also minimizing the locking issues of the single shared file
 * approach on a parallel file system.
 *
 * \note Since the #H5FD_SUBFILING driver is an MPI-based file driver, the HDF5
 *       application should ensure that H5Pset_mpi_params() is called before
 * this routine so that the appropriate MPI communicator and info objects will
 * be setup for use by the #H5FD_SUBFILING and #H5FD_IOC drivers.
 *
 * \note The current architecture of the #H5FD_SUBFILING driver requires that
 * the HDF5 application must have been initialized with MPI_Init_thread() using
 ...
 *       implementation I/O concentrator VFD will be used with the default
 * settings of one I/O concentrator per node and a stripe size of 32MiB. Refer
 * to the H5FD_subfiling_config_t documentation for information about
 * configuration for the #H5FD_SUBFILING driver.
 *
 * \since 1.14.0
 *
 */
 ...
 * \ingroup FAPL
 *
 * \brief Queries a File Access Property List for #H5FD_SUBFILING file driver
 * properties
 *
 * \fapl_id
 * \param[out] config_out Pointer to H5FD_subfiling_config_t structure through
 * which the #H5FD_SUBFILING file driver properties will be returned.
 *
 * \returns \herr_t
 *
 * \details H5Pget_fapl_subfiling() queries the specified File Access Property
 * List for #H5FD_SUBFILING driver properties as set by H5Pset_fapl_subfiling().
 * If the #H5FD_SUBFILING driver has not been set on the File Access Property
 * List, a default configuration is returned. An HDF5 application may use this
 *          functionality to manually configure the #H5FD_SUBFILING driver by
 * calling H5Pget_fapl_subfiling() on a newly-created File Access Property List,
 * adjusting the default values and then calling H5Pset_fapl_subfiling() with
 * the configured H5FD_subfiling_config_t structure.
 *
 * \note H5Pget_fapl_subfiling() returns the #H5FD_SUBFILING driver properties
 * as they were initially set for the File Access Property List using
 * H5Pset_fapl_subfiling(). Alternatively, the driver properties can be modified
 * at runtime according to values set for the #H5FD_SUBFILING_STRIPE_SIZE,
 * #H5FD_SUBFILING_IOC_PER_NODE and #H5FD_SUBFILING_IOC_SELECTION_CRITERIA
 * environment variables. However, driver properties set through environment
 * variables will not be reflected in what is returned by
 * H5Pget_fapl_subfiling(), so an application may need to check those
 *       environment variables to get accurate values for the #H5FD_SUBFILING
 * driver properties.
 *
 * \since 1.14.0
 *
