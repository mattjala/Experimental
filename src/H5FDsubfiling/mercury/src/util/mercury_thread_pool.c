...
/*---------------------------------------------------------------------------*/
int hg_thread_pool_init(unsigned int thread_count,
                        hg_thread_pool_t **pool_ptr) {
  int ret = HG_UTIL_SUCCESS, rc;
  struct hg_thread_pool_private *priv_pool = NULL;
  unsigned int i;

  HG_UTIL_CHECK_ERROR(pool_ptr == NULL, error, ret, HG_UTIL_FAIL,
                      "NULL pointer");
  ...
  for (i = 0; i < thread_count; i++) {
    rc = hg_thread_create(&priv_pool->threads[i], hg_thread_pool_worker,
                          (void *)priv_pool);
    HG_UTIL_CHECK_ERROR(rc != HG_UTIL_SUCCESS, error, ret, HG_UTIL_FAIL,
