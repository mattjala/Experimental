 ...
 * lock free hash table.  The fields of the node are discussed individually
 *below:
 *
 * tag:		Unsigned integer set to LFHT_VALID_NODE whenever the node is
 *either in the SLL or the free list, and to LFHT_INVALID_NODE just before it is
 *discarded.
 *
 ...
 *              malloc() & calloc(), the least significant few bits (at least
 *three in all cases investigated to date) will be zero.
 *
 *              This fact is used to allow atomic marking of the node for
 *deletion. If the low order bit of the next pointer is 1, the node is logically
 *              deleted from the SLL.  It will be physically deleted from the
 *SLL by a subsequent insert or delete call.  See section 9.8 of "The Art of
 *Multiprocessor Programming" by Herlihy, Luchangco, Shavit, and Spear for
 *further details.
 *
 ...
 *
 *              Note that duplicate hash codes cannot appear in the LFSLL, and
 *that nodes in the LFSLL appear in strictly increasing hash order.
 *
 * sentinel:    Boolean flag that is true if the node is a sentinel node, and
 *              false otherwise.
 *
 * value:       Pointer to whatever structure is used to contain the value
 *associated with the id, or NULL if the node is a sentinel node.
 *
 *              This field is atomic, as we allow the client code to modify it
 *in an existing entry in the hash table.
 *
 ***********************************************************************************/
 ...
 *
 * The lfht_flsptr_t combines a pointer to lfht_fl_node_t with a serial number
 *in a 128 bit package.
 *
 * Unfortunately, this means that operations on atomic instances of this
 *structure may or may not be truly atomic.  Instead, the C11 run time may
 *maintain atomicity with a mutex.  While this may have performance
 *implications, there should be no correctness implications.
 *
 ...
 *              list, re-initialzed, its pointer cast to a pointer to
 *lfht_node_t, and returned to the caller.
 *
 * tag:		Atomic unsigned integer set to LFHT_FL_NODE_IN_USE whenever the
 *              node is in the SLL, to LFHT_FL_NODE_ON_FL when the node is on
 *the free list, and to LFHT_FL_NODE_INVALID just before the instance of
 ...
 *
 * Entries in the hash table are stored in a lock free singly linked list
 *(LFSLL).
 *
 ...
 * node with a reference count (see declarations of lfht_node_t and
 *lfht_fl_node_t above).
 *
 ...
 * the node’s ref_count back to zero.  This has the same net effect, but is not
 *as efficient.  Needless to say, this issue should be revisited for the
 *production version.
 *
 ...
 *
 * lfsll_log_len:  Atomic integer used to maintain a count of the number of
 *nodes in the SLL less the sentry nodes and the regular nodes that have been
 *marked for deletion.
 *
 ...
 *
 * max_lfsll_phys_len: Maximum physical length of the LFSLL.  In the
 *multi-thread case, this number should be viewed as aproximate.
 *
 *
 ...
 *              during successful searches.
 *
 * marked_nodes_visited_in_failed_searches: Number of marked nodes visited
 ...
 *              during successful value swaps.
 *
 * marked_nodes_visited_in_failed_val_swaps: Number of marked nodes visited
 ...
 *               visited during itterations through the entries in the hash
 *table.
 *
