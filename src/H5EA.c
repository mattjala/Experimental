...
static herr_t H5EA__lookup_elmt(const H5EA_t *ea, hsize_t idx,
                                hbool_t will_extend, unsigned thing_acc,
                                void **thing, uint8_t **thing_elmt_buf,
...
static H5EA_t *H5EA__new(H5F_t *f, haddr_t ea_addr, hbool_t from_open,
  ...
  if (NULL == (ea = H5FL_CALLOC(H5EA_t)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, NULL,
                "memory allocation failed for extensible array info");

  /* Lock the array header into memory */
  if (NULL ==
      (hdr = H5EA__hdr_protect(f, ea_addr, ctx_udata, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, NULL,
                "unable to load extensible array header");

  /* Check for pending array deletion */
  if (from_open && hdr->pending_delete)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTOPENOBJ, NULL,
                "can't open extensible array pending deletion");
  ...
  if (H5EA__hdr_incr(ea->hdr) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTINC, NULL,
                "can't increment reference count on shared array header");
  ...
  if (hdr && H5EA__hdr_unprotect(hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, NULL,
                "unable to release extensible array header");
  if (!ret_value)
    if (ea && H5EA_close(ea) < 0)
      HDONE_ERROR(H5E_EARRAY, H5E_CLOSEERROR, NULL,
...
static herr_t H5EA__lookup_elmt(const H5EA_t *ea, hsize_t idx,
  ...
  unsigned iblock_cache_flags =
      H5AC__NO_FLAGS_SET; /* Flags to unprotecting index block */
  unsigned sblock_cache_flags =
      H5AC__NO_FLAGS_SET;        /* Flags to unprotecting super block */
  ...
  *thing_unprot_func = (H5EA__unprotect_func_t)NULL;
  ...
  if (NULL == (iblock = H5EA__iblock_protect(hdr, thing_acc)))
    HGOTO_ERROR(
        H5E_EARRAY, H5E_CANTPROTECT, FAIL,
  ...
  if (idx < hdr->cparam.idx_blk_elmts) {
    ...
    *thing_unprot_func = (H5EA__unprotect_func_t)H5EA__iblock_unprotect;
    ...
    elmt_idx =
        idx - (hdr->cparam.idx_blk_elmts + hdr->sblk_info[sblk_idx].start_idx);
    ...
    if (sblk_idx < iblock->nsblks) {
      ...
      /* Protect data block */
      if (NULL == (dblock = H5EA__dblock_protect(
                       hdr, iblock, iblock->dblk_addrs[dblk_idx],
      ...
      if (will_extend && !dblock->has_hdr_depend) {
        if (H5EA__create_flush_depend((H5AC_info_t *)hdr,
                                      (H5AC_info_t *)dblock) < 0)
          HGOTO_ERROR(H5E_EARRAY, H5E_CANTDEPEND, FAIL,
                      "unable to create flush dependency between data block "
                      "and header, index = %llu",
      ...
      *thing_unprot_func = (H5EA__unprotect_func_t)H5EA__dblock_unprotect;
      ...
      size_t sblk_off; /* Offset of super block in index block array of super
                          blocks */
      ...
      if (!H5_addr_defined(iblock->sblk_addrs[sblk_off])) {
        ...
        if (0 == (thing_acc & H5AC__READ_ONLY_FLAG)) { /* i.e. r/w access */
          ...
          /* Create super block */
          sblk_addr =
              H5EA__sblock_create(hdr, iblock, &stats_changed, sblk_idx);
      ...
      /* Protect super block */
      if (NULL == (sblock = H5EA__sblock_protect(hdr, iblock,
                                                 iblock->sblk_addrs[sblk_off],
                                                 sblk_idx, thing_acc)))
        HGOTO_ERROR(
            H5E_EARRAY, H5E_CANTPROTECT, FAIL,
      ...
      if (!H5_addr_defined(sblock->dblk_addrs[dblk_idx])) {
        ...
        if (0 == (thing_acc & H5AC__READ_ONLY_FLAG)) { /* i.e. r/w access */
          ...
          dblk_off = hdr->sblk_info[sblk_idx].start_idx +
                     (dblk_idx * hdr->sblk_info[sblk_idx].dblk_nelmts);
          dblk_addr = H5EA__dblock_create(hdr, sblock, &stats_changed, dblk_off,
                                          sblock->dblk_nelmts);
          if (!H5_addr_defined(dblk_addr))
            HGOTO_ERROR(H5E_EARRAY, H5E_CANTCREATE, FAIL,
                        "unable to create extensible array data block");
          ...
          /* Create flush dependency on header, if extending the array and one
           * doesn't already exist
           */
          if (will_extend && !sblock->has_hdr_depend) {
            if (H5EA__create_flush_depend((H5AC_info_t *)sblock->hdr,
                                          (H5AC_info_t *)sblock) < 0)
              HGOTO_ERROR(H5E_EARRAY, H5E_CANTDEPEND, FAIL,
                          "unable to create flush dependency between super "
                          "block and header, address "
                          "= %llu",
      ...
      if (sblock->dblk_npages) {
        ...
        if (!H5VM_bit_get(sblock->page_init, page_init_idx)) {
          ...
          if (0 == (thing_acc & H5AC__READ_ONLY_FLAG)) { /* i.e. r/w access */
            ...
            if (H5EA__dblk_page_create(hdr, sblock, dblk_page_addr) < 0)
              HGOTO_ERROR(H5E_EARRAY, H5E_CANTCREATE, FAIL,
                          "unable to create data block page");
        ...
        if (NULL == (dblk_page = H5EA__dblk_page_protect(
                         hdr, sblock, dblk_page_addr, thing_acc)))
          HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, FAIL,
                      "unable to protect extensible array data block page, "
                      "address = %llu",
        ...
        *thing_unprot_func = (H5EA__unprotect_func_t)H5EA__dblk_page_unprotect;
        ...
        /* Protect data block */
        if (NULL == (dblock = H5EA__dblock_protect(
                         hdr, sblock, sblock->dblk_addrs[dblk_idx],
  ...
  /* Release resources */
  if (iblock && *thing != iblock &&
      H5EA__iblock_unprotect(iblock, iblock_cache_flags) < 0)
    HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, FAIL,
                "unable to release extensible array index block");
  /* (Note: super blocks don't contain elements, so don't have a '*thing !=
   * sblock' check) */
  if (sblock && H5EA__sblock_unprotect(sblock, sblock_cache_flags) < 0)
    HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, FAIL,
                "unable to release extensible array super block");
  if (dblock && *thing != dblock &&
      H5EA__dblock_unprotect(dblock, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, FAIL,
                "unable to release extensible array data block");
  if (dblk_page && *thing != dblk_page &&
      H5EA__dblk_page_unprotect(dblk_page, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, FAIL,
...
herr_t H5EA_set(const H5EA_t *ea, hsize_t idx, const void *elmt) {
  H5EA_hdr_t *hdr = ea->hdr; /* Header for EA */
  void *thing = NULL; /* Pointer to the array metadata containing the array
                         index we are interested in */
  uint8_t *
      thing_elmt_buf; /* Pointer to the element buffer for the array metadata */
  hsize_t thing_elmt_idx; /* Index of the element in the element buffer for the
                             array metadata */
  H5EA__unprotect_func_t thing_unprot_func; /* Function pointer for unprotecting
                                               the array metadata */
  hbool_t will_extend; /* Flag indicating if setting the element will extend the
                          array */
  unsigned thing_cache_flags =
      H5AC__NO_FLAGS_SET; /* Flags for unprotecting array metadata */
  herr_t ret_value = SUCCEED;
  ...
  if (H5EA__lookup_elmt(ea, idx, will_extend, H5AC__NO_FLAGS_SET, &thing,
                        &thing_elmt_buf, &thing_elmt_idx,
                        &thing_unprot_func) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, FAIL,
                "unable to protect array metadata");
  ...
  H5MM_memcpy(thing_elmt_buf +
                  (hdr->cparam.cls->nat_elmt_size * thing_elmt_idx),
              elmt, hdr->cparam.cls->nat_elmt_size);
  ...
  if (thing && (thing_unprot_func)(thing, thing_cache_flags) < 0)
    HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, FAIL,
                "unable to release extensible array metadata");
  ...
  if (idx >= hdr->stats.stored.max_idx_set) {
    ...
    if ((hdr->cparam.cls->fill)(elmt, (size_t)1) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, FAIL,
                  "can't set element to class's fill value");
  } /* end if */
  else {
    uint8_t *thing_elmt_buf; /* Pointer to the element buffer for the array
                                metadata */
    hsize_t thing_elmt_idx;  /* Index of the element in the element buffer for
                                the array metadata */
    ...
    /* Look up the array metadata containing the element we want to set */
    if (H5EA__lookup_elmt(ea, idx, FALSE, H5AC__READ_ONLY_FLAG, &thing,
                          &thing_elmt_buf, &thing_elmt_idx,
                          &thing_unprot_func) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, FAIL,
                  "unable to protect array metadata");
...
herr_t H5EA_depend(H5EA_t *ea, H5AC_proxy_entry_t *parent) {
  ...
  if (NULL == hdr->parent) {
    ...
    if (H5AC_proxy_entry_add_child(parent, hdr->f, hdr->top_proxy) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, FAIL,
                  "unable to add extensible array as child of proxy");
    hdr->parent = parent;
  }
...
herr_t H5EA_close(H5EA_t *ea) {
  ...
  if (ea->hdr) {
    /* Decrement file reference & check if this is the last open extensible
     * array using the shared array header */
    ...
    if (pending_delete) {
...
#ifndef NDEBUG
      {
        ...
        if (H5AC_get_entry_status(ea->f, ea_addr, &hdr_status) < 0)
          HGOTO_ERROR(H5E_EARRAY, H5E_CANTGET, FAIL,
                      "unable to check metadata cache status for extensible "
                      "array header");
      ...
      /* Lock the array header into memory */
      /* (OK to pass in NULL for callback context, since we know the header must
       * be in the cache) */
      if (NULL ==
          (hdr = H5EA__hdr_protect(ea->f, ea_addr, NULL, H5AC__NO_FLAGS_SET)))
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTLOAD, FAIL,
                    "unable to load extensible array header");
      ...
      if (H5EA__hdr_delete(hdr) < 0)
        HGOTO_ERROR(H5E_EARRAY, H5E_CANTDELETE, FAIL,
                    "unable to delete extensible array");
    } /* end if */
    else {
      /* Decrement the reference count on the array header */
      /* (don't put in H5EA_hdr_fuse_decr() as the array header may be evicted
...
herr_t H5EA_delete(H5F_t *f, haddr_t ea_addr, void *ctx_udata) {
  ...
  /* Lock the array header into memory */
  if (NULL ==
      (hdr = H5EA__hdr_protect(f, ea_addr, ctx_udata, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTPROTECT, FAIL,
                "unable to protect extensible array header, address = %llu",
...
int H5EA_iterate(H5EA_t *ea, H5EA_operator_t op, void *udata) {
  ...
  if (NULL == (elmt = H5FL_BLK_MALLOC(ea_native_elmt,
                                      ea->hdr->cparam.cls->nat_elmt_size)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, H5_ITER_ERROR,
                "memory allocation failed for extensible array element");

  /* Iterate over all elements in array */
  for (u = 0;
       u < ea->hdr->stats.stored.max_idx_set && ret_value == H5_ITER_CONT;
       u++) {
    /* Get array element */
    if (H5EA_get(ea, u, elmt) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTGET, H5_ITER_ERROR,
                  "unable to delete fixed array");
