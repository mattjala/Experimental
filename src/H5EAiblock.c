...
H5EA_iblock_t *H5EA__iblock_alloc(H5EA_hdr_t *hdr) {
  ...
  if (hdr->cparam.idx_blk_elmts > 0)
    if (NULL == (iblock->elmts =
                     H5FL_BLK_MALLOC(idx_blk_elmt_buf,
  ...
  if (iblock->ndblk_addrs > 0)
    if (NULL ==
        (iblock->dblk_addrs = H5FL_SEQ_MALLOC(haddr_t, iblock->ndblk_addrs)))
      HGOTO_ERROR(
          H5E_EARRAY, H5E_CANTALLOC, NULL,
  ...
  if (iblock->nsblk_addrs > 0)
    if (NULL ==
        (iblock->sblk_addrs = H5FL_SEQ_MALLOC(haddr_t, iblock->nsblk_addrs)))
      HGOTO_ERROR(
          H5E_EARRAY, H5E_CANTALLOC, NULL,
...
haddr_t H5EA__iblock_create(H5EA_hdr_t *hdr, hbool_t *stats_changed) {
  ...
  if (HADDR_UNDEF == (iblock_addr = H5MF_alloc(hdr->f, H5FD_MEM_EARRAY_IBLOCK,
                                               (hsize_t)iblock->size)))
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTALLOC, HADDR_UNDEF,
  ...
  if (hdr->cparam.idx_blk_elmts > 0) {
    ...
    if ((hdr->cparam.cls->fill)(iblock->elmts,
                                (size_t)hdr->cparam.idx_blk_elmts) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, HADDR_UNDEF,
                  "can't set extensible array index block elements to class's "
                  "fill value");
  } /* end if */
  ...
  if (iblock->ndblk_addrs > 0) {
    haddr_t tmp_addr =
        HADDR_UNDEF; /* Address value to fill data block addresses with */

    /* Set all the data block addresses to "undefined" address value */
    H5VM_array_fill(iblock->dblk_addrs, &tmp_addr, sizeof(haddr_t),
                    iblock->ndblk_addrs);
  } /* end if */
  ...
  if (iblock->nsblk_addrs > 0) {
    haddr_t tmp_addr =
        HADDR_UNDEF; /* Address value to fill super block addresses with */

    /* Set all the super block addresses to "undefined" address value */
    H5VM_array_fill(iblock->sblk_addrs, &tmp_addr, sizeof(haddr_t),
                    iblock->nsblk_addrs);
  } /* end if */

  /* Cache the new extensible array index block */
  if (H5AC_insert_entry(hdr->f, H5AC_EARRAY_IBLOCK, iblock_addr, iblock,
 ...
 *
 * Purpose:	Convenience wrapper around protecting extensible array index
 *block
 *
 * Return:	Non-NULL pointer to index block on success/NULL on failure
 *
...
H5EA_iblock_t *H5EA__iblock_protect(H5EA_hdr_t *hdr, unsigned flags) {
  ...
  if (NULL == (iblock = (H5EA_iblock_t *)H5AC_protect(
                   hdr->f, H5AC_EARRAY_IBLOCK, hdr->idx_blk_addr, hdr, flags)))
    HGOTO_ERROR(
        H5E_EARRAY, H5E_CANTPROTECT, NULL,
  ...
  if (!ret_value) {
    ...
    if (iblock && H5AC_unprotect(hdr->f, H5AC_EARRAY_IBLOCK, iblock->addr,
                                 iblock, H5AC__NO_FLAGS_SET) < 0)
      HDONE_ERROR(
          H5E_EARRAY, H5E_CANTUNPROTECT, NULL,
 ...
 *
 * Purpose:	Convenience wrapper around unprotecting extensible array index
 *block
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__iblock_unprotect(H5EA_iblock_t *iblock, unsigned cache_flags) {
  ...
  if (H5AC_unprotect(iblock->hdr->f, H5AC_EARRAY_IBLOCK, iblock->addr, iblock,
                     cache_flags) < 0)
    HGOTO_ERROR(
        H5E_EARRAY, H5E_CANTUNPROTECT, FAIL,
        "unable to unprotect extensible array index block, address = %llu",
        (unsigned long long)iblock->addr);
...
herr_t H5EA__iblock_delete(H5EA_hdr_t *hdr) {
  ...
  if (iblock->nsblk_addrs > 0) {
    ...
    for (u = 0; u < iblock->nsblk_addrs; u++) {
      ...
      if (H5_addr_defined(iblock->sblk_addrs[u])) {
        /* Delete super block */
        if (H5EA__sblock_delete(hdr, iblock, iblock->sblk_addrs[u],
                                (unsigned)(u + iblock->nsblks)) < 0)
          HGOTO_ERROR(H5E_EARRAY, H5E_CANTDELETE, FAIL,
  ...
  if (iblock &&
      H5EA__iblock_unprotect(iblock, H5AC__DIRTIED_FLAG | H5AC__DELETED_FLAG |
