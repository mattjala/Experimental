...
typedef struct H5FD_splitter_fapl_t {
  ...
  char log_file_path[H5FD_SPLITTER_PATH_MAX +
                     1]; /* file to record errors reported by the W/O channel */
...
typedef struct H5FD_splitter_t {
  ...
  unsigned
      version; /* version of the H5FD_splitter_vfd_config_t structure used */
...
static herr_t H5FD__splitter_log_error(const H5FD_splitter_t *file,
                                       const char *atfunc, const char *msg);
...
static herr_t H5FD__splitter_get_default_wo_path(char *new_path,
                                                 size_t new_path_len,
                                                 const char *base_filename);
static hsize_t H5FD__splitter_sb_size(H5FD_t *_file);
static herr_t H5FD__splitter_sb_encode(H5FD_t *_file, char *name /*out*/,
                                       unsigned char *buf /*out*/);
static herr_t H5FD__splitter_sb_decode(H5FD_t *_file, const char *name,
...
static H5FD_t *H5FD__splitter_open(const char *name, unsigned flags,
                                   hid_t fapl_id, haddr_t maxaddr);
static herr_t H5FD__splitter_close(H5FD_t *_file);
static int H5FD__splitter_cmp(const H5FD_t *_f1, const H5FD_t *_f2);
static herr_t H5FD__splitter_query(const H5FD_t *_file,
                                   unsigned long *flags /* out */);
static herr_t H5FD__splitter_get_type_map(const H5FD_t *_file,
                                          H5FD_mem_t *type_map);
static haddr_t H5FD__splitter_alloc(H5FD_t *file, H5FD_mem_t type,
                                    hid_t dxpl_id, hsize_t size);
static herr_t H5FD__splitter_free(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                                  haddr_t addr, hsize_t size);
static haddr_t H5FD__splitter_get_eoa(const H5FD_t *_file,
                                      H5FD_mem_t H5_ATTR_UNUSED type);
static herr_t H5FD__splitter_set_eoa(H5FD_t *_file,
                                     H5FD_mem_t H5_ATTR_UNUSED type,
                                     haddr_t addr);
static haddr_t H5FD__splitter_get_eof(const H5FD_t *_file,
                                      H5FD_mem_t H5_ATTR_UNUSED type);
static herr_t H5FD__splitter_get_handle(H5FD_t *_file,
                                        hid_t H5_ATTR_UNUSED fapl,
                                        void **file_handle);
static herr_t H5FD__splitter_read(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                                  haddr_t addr, size_t size, void *buf);
static herr_t H5FD__splitter_write(H5FD_t *_file, H5FD_mem_t type,
                                   hid_t dxpl_id, haddr_t addr, size_t size,
                                   const void *buf);
static herr_t H5FD__splitter_flush(H5FD_t *_file, hid_t dxpl_id,
                                   hbool_t closing);
static herr_t H5FD__splitter_truncate(H5FD_t *_file, hid_t dxpl_id,
                                      hbool_t closing);
...
static herr_t H5FD__splitter_ctl(H5FD_t *_file, uint64_t op_code,
...
herr_t H5Pget_fapl_splitter(hid_t fapl_id,
                            H5FD_splitter_vfd_config_t *config /*out*/) {
  ...
  HDstrncpy(config->log_file_path, fapl_ptr->log_file_path,
            H5FD_SPLITTER_PATH_MAX + 1);
...
H5FD__splitter_populate_config(H5FD_splitter_vfd_config_t *vfd_config,
  ...
  HDstrncpy(fapl_out->log_file_path, vfd_config->log_file_path,
            H5FD_SPLITTER_PATH_MAX + 1);
  ...
  if (H5P_DEFAULT != vfd_config->rw_fapl_id) {
    ...
    if (H5P_set_driver_by_value(plist, H5_VFD_SEC2, NULL, TRUE) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL,
                  "can't set default driver on R/W channel FAPL");
  }
  if (H5P_DEFAULT != vfd_config->wo_fapl_id) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__splitter_flush(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
                                   hbool_t closing) {
  ...
  if (H5FDflush(file->wo_file, dxpl_id, closing) < 0)
    H5FD_SPLITTER_WO_ERROR(file, __func__, H5E_VFL, H5E_CANTFLUSH, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__splitter_read(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
  ...
  if (H5FDread(file->rw_file, type, dxpl_id, addr, size, buf) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                "Reading from R/W channel failed");
...
static herr_t H5FD__splitter_write(H5FD_t *_file, H5FD_mem_t type,
  ...
  if (H5FDwrite(file->wo_file, type, dxpl_id, addr, size, buf) < 0)
    H5FD_SPLITTER_WO_ERROR(file, __func__, H5E_VFL, H5E_WRITEERROR, FAIL,
 ...
 */
static void *H5FD__splitter_fapl_copy(const void *_old_fa) {
  const H5FD_splitter_fapl_t *old_fa_ptr =
      (const H5FD_splitter_fapl_t *)_old_fa;
  ...
  HDstrncpy(new_fa_ptr->wo_path, old_fa_ptr->wo_path,
            H5FD_SPLITTER_PATH_MAX + 1);
  HDstrncpy(new_fa_ptr->log_file_path, old_fa_ptr->log_file_path,
            H5FD_SPLITTER_PATH_MAX + 1);
 ...
 */
static H5FD_t *H5FD__splitter_open(const char *name, unsigned flags,
                                   hid_t splitter_fapl_id, haddr_t maxaddr) {
  H5FD_splitter_t *file_ptr = NULL; /* Splitter VFD info */
  const H5FD_splitter_fapl_t *fapl_ptr =
  ...
  if (NULL == fapl_ptr) {
    ...
    if (*default_fapl->wo_path == '\0')
      if (H5FD__splitter_get_default_wo_path(
              default_fapl->wo_path, H5FD_SPLITTER_PATH_MAX + 1, name) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, NULL,
  ...
  /* Copy simpler info */
  HDstrncpy(file_ptr->fa.wo_path, fapl_ptr->wo_path,
            H5FD_SPLITTER_PATH_MAX + 1);
  HDstrncpy(file_ptr->fa.log_file_path, fapl_ptr->log_file_path,
            H5FD_SPLITTER_PATH_MAX + 1);
  ...
  file_ptr->wo_file =
      H5FD_open(fapl_ptr->wo_path, flags, fapl_ptr->wo_fapl_id, HADDR_UNDEF);
  if (!file_ptr->wo_file)
    H5FD_SPLITTER_WO_ERROR(file_ptr, __func__, H5E_VFL, H5E_CANTOPENFILE, NULL,
...
static herr_t H5FD__splitter_set_eoa(H5FD_t *_file,
  ...
  if (H5FD_set_eoa(file->wo_file, type, addr) < 0)
    H5FD_SPLITTER_WO_ERROR(file, __func__, H5E_VFL, H5E_CANTSET, FAIL,
...
static herr_t H5FD__splitter_truncate(H5FD_t *_file, hid_t dxpl_id,
  ...
  if (H5FDtruncate(file->wo_file, dxpl_id, closing) < 0)
    H5FD_SPLITTER_WO_ERROR(file, __func__, H5E_VFL, H5E_CANTUPDATE, FAIL,
...
static herr_t H5FD__splitter_lock(H5FD_t *_file, hbool_t rw) {
  ...
  if (file->wo_file != NULL)
    if (H5FD_lock(file->wo_file, rw) < 0)
      H5FD_SPLITTER_WO_ERROR(file, __func__, H5E_VFL, H5E_CANTLOCKFILE, FAIL,
    ...
    if (flags & H5FD_CTL_ROUTE_TO_TERMINAL_VFD_FLAG) {
      ...
      if (flags & H5FD_CTL_FAIL_IF_UNKNOWN_FLAG)
        HGOTO_ERROR(H5E_VFL, H5E_FCNTL, FAIL,
                    "VFD ctl request failed (unknown op code and fail if "
                    "unknown flag is set)");
    }
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__splitter_alloc(H5FD_t *_file, H5FD_mem_t type,
                                    hid_t dxpl_id, hsize_t size) {
  H5FD_splitter_t *file = (H5FD_splitter_t *)_file; /* VFD file struct */
  haddr_t ret_value = HADDR_UNDEF;                  /* Return value */
   ...
   */
  if ((ret_value = H5FDalloc(file->rw_file, type, dxpl_id, size)) ==
      HADDR_UNDEF)
...
static herr_t H5FD__splitter_free(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
  ...
  if (H5FDfree(file->wo_file, type, dxpl_id, addr, size) < 0)
    H5FD_SPLITTER_WO_ERROR(file, __func__, H5E_VFL, H5E_CANTINIT, FAIL,
...
static herr_t H5FD__splitter_delete(const char *filename, hid_t fapl_id) {
  ...
  if (H5P_FILE_ACCESS_DEFAULT == fapl_id) {
    ...
    if (*default_fapl->wo_path == '\0')
      if (H5FD__splitter_get_default_wo_path(
              default_fapl->wo_path, H5FD_SPLITTER_PATH_MAX + 1, filename) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL,
                    "can't generate default filename for W/O channel");
    ...
    if (NULL == (plist = (H5P_genplist_t *)H5I_object(fapl_id)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                  "not a file access property list");
    if (NULL == (fapl_ptr = (const H5FD_splitter_fapl_t *)H5P_peek_driver_info(
                     plist))) {
      if (NULL == (default_fapl = H5FL_CALLOC(H5FD_splitter_fapl_t)))
        HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, FAIL,
                    "unable to allocate file access property list struct");
      ...
      /* If W/O path is not set, use base filename with '_wo' suffix */
      if (*default_fapl->wo_path == '\0')
        if (H5FD__splitter_get_default_wo_path(default_fapl->wo_path,
                                               H5FD_SPLITTER_PATH_MAX + 1,
                                               filename) < 0)
          HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL,
                      "can't generate default filename for W/O channel");
  ...
  if (H5FDdelete(fapl_ptr->wo_path, fapl_ptr->wo_fapl_id) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTDELETEFILE, FAIL,
                "unable to delete W/O channel file");
