...
H5MF__sect_deserialize(const H5FS_section_class_t *cls, const uint8_t *buf,
...
static herr_t H5MF__sect_small_add(H5FS_section_info_t **_sect, unsigned *flags,
                                   void *_udata);
 ...
 *-------------------------------------------------------------------------
 */
H5MF_free_section_t *H5MF__sect_new(unsigned ctype, haddr_t sect_off,
                                    hsize_t sect_size) {
...
herr_t H5MF__sect_free(H5FS_section_info_t *_sect) {
  H5MF_free_section_t *sect =
      (H5MF_free_section_t *)_sect; /* File free section */
...
static herr_t H5MF__sect_valid(const H5FS_section_class_t H5_ATTR_UNUSED *cls,
  ...
  const H5MF_free_section_t *sect =
      (const H5MF_free_section_t *)_sect; /* File free section */
#endif                                    /* NDEBUG */
 ...
 *
 * Purpose:	Split SECT into 2 sections: fragment for alignment & the aligned
 *section SECT's addr and size are updated to point to the aligned section
 *
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5MF__sect_simple_can_merge(const H5FS_section_info_t *_sect1,
                                          const H5FS_section_info_t *_sect2,
                                          void H5_ATTR_UNUSED *_udata) {
  const H5MF_free_section_t *sect1 =
      (const H5MF_free_section_t *)_sect1; /* File free section */
  const H5MF_free_section_t *sect2 =
      (const H5MF_free_section_t *)_sect2; /* File free section */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5MF__sect_simple_merge(H5FS_section_info_t **_sect1,
                                      H5FS_section_info_t *_sect2,
                                      void H5_ATTR_UNUSED *_udata) {
  H5MF_free_section_t **sect1 =
      (H5MF_free_section_t **)_sect1; /* File free section */
  H5MF_free_section_t *sect2 =
      (H5MF_free_section_t *)_sect2; /* File free section */
...
static htri_t H5MF__sect_simple_can_shrink(const H5FS_section_info_t *_sect,
  ...
  /* Check if the section is exactly at the end of the allocated space in the
   * file */
  if (H5_addr_eq(end, eoa)) {
    ...
    /* Shrinking can't occur if the 'eoa_shrink_only' flag is set and we're not
     * shrinking the EOA */
    ...
    /* Check if this section is allowed to merge with small 'raw' aggregation
     * block */
    if (udata->f->shared->fs_aggr_merge[udata->alloc_type] &
      ...
      /* See if section can absorb the aggregator & vice versa */
      if ((status =
               H5MF__aggr_can_absorb(udata->f, &(udata->f->shared->sdata_aggr),
                                     sect, &(udata->shrink))) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTMERGE, FAIL,
                    "error merging section with aggregation block");
      else if (status > 0) {
...
static herr_t H5MF__sect_simple_shrink(H5FS_section_info_t **_sect,
  ...
  if (H5MF_SHRINK_EOA == udata->shrink) {
    ...
    if (H5F__free(udata->f, udata->alloc_type, (*sect)->sect_info.addr,
                  (*sect)->sect_info.size) < 0)
    ...
    if (H5MF__aggr_absorb(udata->f, udata->aggr, *sect,
                          udata->allow_sect_absorb) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTMERGE, FAIL,
 ...
 *
 * Purpose:     Perform actions on a small "meta" action before adding it to the
 *free space manager: 1) Drop the section if it is at page end and its size <=
 *page end threshold 2) Adjust section size to include page end threshold if
...
static herr_t H5MF__sect_small_add(H5FS_section_info_t **_sect, unsigned *flags,
                                   void *_udata) {
  H5MF_free_section_t **sect =
      (H5MF_free_section_t **)_sect; /* Fractal heap free section */
  H5MF_sect_ud_t *udata = (H5MF_sect_ud_t *)_udata; /* User data for callback */
  haddr_t sect_end;
  hsize_t rem, prem;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Adjust the section if it is not at page end but its size + prem is at page
     end */
  else if (prem <= H5F_PGEND_META_THRES(udata->f)) {
    (*sect)->sect_info.size += prem;
#ifdef H5MF_ALLOC_DEBUG_MORE
    fprintf(stderr,
            "%s: section is adjusted {%" PRIuHADDR ", %" PRIuHSIZE "}\n",
            __func__, (*sect)->sect_info.addr, (*sect)->sect_info.size);
#endif /* H5MF_ALLOC_DEBUG_MORE */
  }    /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5MF__sect_small_can_merge(const H5FS_section_info_t *_sect1,
                                         const H5FS_section_info_t *_sect2,
                                         void *_udata) {
  const H5MF_free_section_t *sect1 =
      (const H5MF_free_section_t *)_sect1; /* File free section */
  ...
  if (ret_value > 0)
    ...
    if ((sect1->sect_info.addr / udata->f->shared->fs_page_size) !=
        (((sect2->sect_info.addr + sect2->sect_info.size - 1) /
          udata->f->shared->fs_page_size)))
      ret_value = FALSE;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5MF__sect_small_merge(H5FS_section_info_t **_sect1,
                                     H5FS_section_info_t *_sect2,
                                     void *_udata) {
  H5MF_free_section_t **sect1 =
      (H5MF_free_section_t **)_sect1; /* File free section */
  ...
  if ((*sect1)->sect_info.size == udata->f->shared->fs_page_size) {
    if (H5MF_xfree(udata->f, udata->alloc_type, (*sect1)->sect_info.addr,
                   (*sect1)->sect_info.size) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTFREE, FAIL,
                  "can't free merged section");

    /* Need to free possible metadata page in the PB cache */
    /* This is in response to the data corruption bug from fheap.c with page
     * buffering + page strategy */
    /* Note: Large metadata page bypasses the PB cache */
    /* Note: Update of raw data page (large or small sized) is handled by the PB
     * cache */
    if (udata->f->shared->page_buf != NULL &&
        udata->alloc_type != H5FD_MEM_DRAW)
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5MF__sect_large_can_merge(const H5FS_section_info_t *_sect1,
                                         const H5FS_section_info_t *_sect2,
                                         void H5_ATTR_UNUSED *_udata) {
  const H5MF_free_section_t *sect1 =
      (const H5MF_free_section_t *)_sect1; /* File free section */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5MF__sect_large_merge(H5FS_section_info_t **_sect1,
                                     H5FS_section_info_t *_sect2,
                                     void H5_ATTR_UNUSED *_udata) {
  H5MF_free_section_t **sect1 =
      (H5MF_free_section_t **)_sect1; /* File free section */
...
static htri_t H5MF__sect_large_can_shrink(const H5FS_section_info_t *_sect,
  ...
  /* Retrieve the end of the file's address space */
  if (HADDR_UNDEF ==
      (eoa = H5FD_get_eoa(udata->f->shared->lf, udata->alloc_type)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                "driver get_eoa request failed");
   ...
   * file */
  if (H5_addr_eq(end, eoa) &&
      sect->sect_info.size >= udata->f->shared->fs_page_size) {
    /* Set the shrinking type */
    udata->shrink = H5MF_SHRINK_EOA;
#ifdef H5MF_ALLOC_DEBUG_MORE
    fprintf(stderr,
...
static herr_t H5MF__sect_large_shrink(H5FS_section_info_t **_sect,
  ...
  H5MF_EOA_MISALIGN(udata->f, (*sect)->sect_info.addr,
                    udata->f->shared->fs_page_size, frag_size);

  /* Free full pages from EOA */
  /* Retain partial page in the free-space manager so as to keep EOA at page
   * boundary */
  if (H5F__free(udata->f, udata->alloc_type,
                (*sect)->sect_info.addr + frag_size,
