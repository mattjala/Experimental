...
static htri_t H5G__common_path(const H5RS_str_t *fullpath_r,
...
static herr_t H5G__name_move_path(H5RS_str_t **path_r_ptr,
                                  const char *full_suffix, const char *src_path,
                                  const char *dst_path);
...
char *H5G_normalize(const char *name) {
  ...
  if (NULL == (norm = H5MM_strdup(name)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "memory allocation failed for normalized string");
...
static htri_t H5G__common_path(const H5RS_str_t *fullpath_r,
  ...
  /* If we reached the end of the prefix path to check, it must be a valid
   * prefix */
  if (*prefix == '\0')
    ret_value = TRUE;
...
static H5RS_str_t *H5G__build_fullpath(const char *prefix, const char *name) {
  ...
  if (NULL == (ret_value = H5RS_create(prefix)))
    HGOTO_ERROR(H5E_SYM, H5E_CANTCREATE, NULL,
                "can't create ref-counted string");
  if (prefix[HDstrlen(prefix) - 1] != '/')
    H5RS_aputc(ret_value,
               '/'); /* Add separator, if the prefix doesn't end in one */
...
herr_t H5G_name_set(const H5G_name_t *loc, H5G_name_t *obj, const char *name) {
  ...
  if (loc->full_path_r) {
    ...
    if ((obj->full_path_r =
             H5G_build_fullpath_refstr_str(loc->full_path_r, name)) == NULL)
  ...
  if (loc->user_path_r) {
    ...
    if ((obj->user_path_r =
             H5G_build_fullpath_refstr_str(loc->user_path_r, name)) == NULL)
      HGOTO_ERROR(H5E_SYM, H5E_PATH, FAIL, "can't build user path name");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_name_copy(H5G_name_t *dst, const H5G_name_t *src,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_get_name(const H5G_loc_t *loc, char *name /*out*/, size_t size,
                    size_t *name_len, hbool_t *cached) {
  ...
  if (loc->path->user_path_r != NULL && loc->path->obj_hidden == 0) {
    ...
    if (H5G_get_name_by_addr(loc->oloc->file, loc->oloc, name, size, name_len) <
        0)
 ...
 */
static int H5G__name_replace_cb(void *obj_ptr, hid_t obj_id, void *key) {
  const H5G_names_t *names =
      (const H5G_names_t *)key; /* Get operation's information */
  ...
  hbool_t obj_in_child =
      FALSE; /* Flag to indicate that the object is in the child mount hier. */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (!obj_path->full_path_r)
    HGOTO_DONE(
        SUCCEED); /* No need to look at object, it's path is already invalid */
  ...
  if (H5F_PARENT(oloc->file)) {
    ...
    while (H5F_PARENT(top_obj_file) != NULL) {
      /* Check if object is in child mount hier. (for mount & unmount
       * operations) */
  ...
  /* Check if object is in top of child mount hier. (for mount & unmount
   * operations) */
  if (names->dst_file && H5F_SAME_SHARED(top_obj_file, names->dst_file))
    obj_in_child = TRUE;
  ...
  case H5G_NAME_UNMOUNT:
    if (obj_in_child) {
      ...
      if (obj_path->user_path_r &&
          H5RS_len(rs) < H5RS_len(obj_path->user_path_r)) {
        /* Free user path */
  ...
  case H5G_NAME_DELETE:
    /* Check if the location being unlinked is in the path for the current
     * object */
    if (H5G__common_path(obj_path->full_path_r, names->src_full_path_r)) {
  ...
  case H5G_NAME_MOVE: /* Link move case, check for relative names case */
    ...
    if (H5G__common_path(obj_path->full_path_r, names->src_full_path_r)) {
      ...
      /* Make certain that the source and destination names are full (not
       * relative) paths */
      assert(*src_path == '/');
      assert(*dst_path == '/');
      ...
      if (obj_path->user_path_r)
        if (H5G__name_move_path(&(obj_path->user_path_r), full_suffix, src_path,
                                dst_path) < 0)
          HGOTO_ERROR(H5E_SYM, H5E_PATH, FAIL, "can't build user path name");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_name_replace(const H5O_link_t *lnk, H5G_names_op_t op,
                        H5F_t *src_file, H5RS_str_t *src_full_path_r,
  ...
  if (src_full_path_r) {
    hbool_t search_group =
        FALSE; /* Flag to indicate that groups are to be searched */
    hbool_t search_dataset =
        FALSE; /* Flag to indicate that datasets are to be searched */
    hbool_t search_datatype =
        FALSE; /* Flag to indicate that datatypes are to be searched */
    ...
    if (lnk) {
      ...
      case H5L_TYPE_SOFT:
        /* Symbolic links might resolve to any object, so we need to search all
         * IDs */
    ...
    if (search_group || search_dataset || search_datatype) {
      H5G_names_t
          names; /* Structure to hold operation information for callback */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__get_name_by_addr_cb(hid_t gid, const char *path,
                                       const H5L_info2_t *linfo, void *_udata) {
  H5G_gnba_iter_t *udata =
      (H5G_gnba_iter_t *)_udata;   /* User data for iteration */
  ...
  if (linfo->type == H5L_TYPE_HARD) {
    ...
    if (H5VL_native_token_to_addr(udata->loc->file, H5I_FILE, linfo->u.token,
                                  &link_addr) < 0)
    ...
    if (udata->loc->addr == link_addr) {
      ...
      if (udata->loc->addr == obj_loc.oloc->addr &&
          udata->loc->file == obj_loc.oloc->file) {
        if (NULL == (udata->path = H5MM_strdup(path)))
          HGOTO_ERROR(H5E_SYM, H5E_CANTALLOC, H5_ITER_ERROR,
                      "can't duplicate path string");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_get_name_by_addr(H5F_t *f, const H5O_loc_t *loc, char *name,
                            size_t size, size_t *name_len) {
  ...
  if (root_loc.oloc->addr == loc->addr && root_loc.oloc->file == loc->file) {
    ...
    if ((status = H5G_visit(&root_loc, "/", H5_INDEX_NAME, H5_ITER_NATIVE,
                            H5G__get_name_by_addr_cb, &udata)) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_BADITER, FAIL,
                  "group traversal failed while looking for object name");
    else if (status > 0)
      found_obj = TRUE;
  } /* end else */
