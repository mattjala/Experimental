 ...
 *-------------------------------------------------------------------------
 */
H5G_t *H5G__create_named(const H5G_loc_t *loc, const char *name, hid_t lcpl_id,
                         hid_t gcpl_id) {
...
H5G_t *H5G__create(H5F_t *file, H5G_obj_create_t *gcrt_info) {
  ...
  unsigned oloc_init = 0; /* Flag to indicate that the group object location was
  ...
  if (H5G__obj_create(file, gcrt_info, &(grp->oloc) /*out*/) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, NULL,
                "unable to create group object header");
  ...
  if (H5FO_insert(grp->oloc.file, grp->oloc.addr, grp->shared, TRUE) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, NULL,
                "can't insert group into list of open objects");
  ...
  if (ret_value == NULL) {
    ...
    if (oloc_init) {
      ...
      if (H5O_close(&(grp->oloc), NULL) < 0)
        HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, NULL,
                    "unable to release object header");
      if (H5O_delete(file, grp->oloc.addr) < 0)
        HDONE_ERROR(H5E_SYM, H5E_CANTDELETE, NULL,
...
H5G_t *H5G_open(const H5G_loc_t *loc) {
  ...
  if (NULL == (grp = H5FL_CALLOC(H5G_t)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "can't allocate space for group");
  ...
  if ((shared_fo = (H5G_shared_t *)H5FO_opened(grp->oloc.file,
                                               grp->oloc.addr)) == NULL) {
    ...
    if (H5FO_top_count(grp->oloc.file, grp->oloc.addr) == 0) {
      ...
      if (H5O_open(&(grp->oloc)) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, NULL,
...
herr_t H5G_close(H5G_t *grp) {
  ...
  if (0 == grp->shared->fo_count) {
    ...
    if (H5AC_cork(grp->oloc.file, grp->oloc.addr, H5AC__GET_CORKED, &corked) <
        0)
    ...
    if (corked)
      if (H5AC_cork(grp->oloc.file, grp->oloc.addr, H5AC__UNCORK, NULL) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTUNCORK, FAIL,
    ...
    if (H5FO_delete(grp->oloc.file, grp->oloc.addr) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTRELEASE, FAIL,
                  "can't remove group from list of open objects");
    if (H5O_close(&(grp->oloc), &file_closed) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL, "unable to close");

    /* Evict group metadata if evicting on close */
    if (!file_closed && H5F_SHARED(grp->oloc.file) &&
        H5F_EVICT_ON_CLOSE(grp->oloc.file)) {
...
H5O_loc_t *H5G_oloc(H5G_t *grp){
    ...
    FUNC_ENTER_NOAPI_NOINIT_NOERR
        ...
        FUNC_LEAVE_NOAPI(grp ? &(grp->oloc) : NULL)} /* end H5G_oloc() */
...
H5G_name_t *H5G_nameof(H5G_t *grp){
    ...
    FUNC_ENTER_NOAPI_NOINIT_NOERR
        ...
        FUNC_LEAVE_NOAPI(grp ? &(grp->path) : NULL)} /* end H5G_nameof() */
...
static herr_t H5G__iterate_cb(const H5O_link_t *lnk, void *_udata) {
  H5G_iter_appcall_ud_t *udata =
      (H5G_iter_appcall_ud_t *)_udata; /* User data for callback */
  ...
  case H5G_LINK_OP_OLD:
    ...
    ret_value =
        (udata->lnk_op.op_func.op_old)(udata->gid, lnk->name, udata->op_data);
    break;
  ...
  case H5G_LINK_OP_NEW: {
    ...
    if (H5G_link_to_info(udata->link_loc, lnk, &info) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, H5_ITER_ERROR,
                  "unable to get info for link");

    /* Make the application callback */
    ret_value = (udata->lnk_op.op_func.op_new)(udata->gid, lnk->name, &info,
                                               udata->op_data);
  } break;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_iterate(H5G_loc_t *loc, const char *group_name, H5_index_t idx_type,
  ...
  if ((ret_value = H5G__obj_iterate(&(grp->oloc), idx_type, order, skip,
                                    last_lnk, H5G__iterate_cb, &udata)) < 0)
 ...
 */
static herr_t
H5G__free_visit_visited(void *item, void H5_ATTR_UNUSED *key,
...
static herr_t H5G__visit_cb(const H5O_link_t *lnk, void *_udata) {
  H5G_iter_visit_ud_t *udata =
      (H5G_iter_visit_ud_t *)_udata;          /* User data for callback */
  H5L_info2_t info;                           /* Link info */
  H5G_loc_t obj_loc;                          /* Location of object */
  H5G_name_t obj_path;                        /* Object's group hier. path */
  H5O_loc_t obj_oloc;                         /* Object's object location */
  hbool_t obj_found = FALSE;                  /* Object at 'name' found */
  size_t old_path_len = udata->curr_path_len; /* Length of path before appending
                                                 this link's name */
  ...
  if (len_needed > udata->path_buf_size) {
    ...
    if (NULL == (new_path = H5MM_realloc(udata->path, len_needed)))
      HGOTO_ERROR(H5E_SYM, H5E_NOSPACE, H5_ITER_ERROR,
                  "can't allocate path string");
  ...
  if (ret_value == H5_ITER_CONT && lnk->type == H5L_TYPE_HARD) {
    ...
    if (NULL == H5SL_search(udata->visited, &obj_pos)) {
      ...
      if (otype == H5O_TYPE_GROUP) {
        H5G_loc_t *old_loc =
            udata->curr_loc; /* Pointer to previous group location info */
        ...
        ret_value = H5G__obj_iterate(&obj_oloc, idx_type, udata->order,
                                     (hsize_t)0, NULL, H5G__visit_cb, udata);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_visit(H5G_loc_t *loc, const char *group_name, H5_index_t idx_type,
  ...
  if ((udata.visited = H5SL_create(H5SL_TYPE_OBJ, NULL)) == NULL)
    HGOTO_ERROR(H5E_SYM, H5E_CANTCREATE, FAIL,
  ...
  if ((ret_value = H5G__obj_iterate(&(grp->oloc), idx_type, order, (hsize_t)0,
                                    NULL, H5G__visit_cb, &udata)) < 0)
...
hid_t H5G_get_create_plist(const H5G_t *grp) {
  ...
  if (NULL ==
      (gcpl_plist = (H5P_genplist_t *)H5I_object(H5P_LST_GROUP_CREATE_ID_g)))
  ...
  if (ginfo_exists) {
    ...
    if (NULL == H5O_msg_read(&(grp->oloc), H5O_GINFO_ID, &ginfo))
      HGOTO_ERROR(H5E_SYM, H5E_BADMESG, H5I_INVALID_HID,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__get_info_by_name(const H5G_loc_t *loc, const char *name,
                             H5G_info_t *grp_info) {
  H5G_loc_t grp_loc;          /* Location used to open group */
  H5G_name_t grp_path;        /* Opened object group hier. path */
  H5O_loc_t grp_oloc;         /* Opened object object location */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__get_info_by_idx(const H5G_loc_t *loc, const char *group_name,
  ...
  if (H5G_loc_find_by_idx(loc, group_name, idx_type, order, n,
                          &grp_loc /*out*/) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "group not found");
  loc_found = TRUE;
