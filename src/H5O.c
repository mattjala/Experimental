...
static herr_t H5O__get_info_by_name_api_common(hid_t loc_id, const char *name,
                                               ...
                                               void **token_ptr,
                                               H5VL_object_t **_vol_obj_ptr);
static herr_t H5O__copy_api_common(hid_t src_loc_id, const char *src_name,
                                   hid_t dst_loc_id, const char *dst_name,
                                   hid_t ocpypl_id, hid_t lcpl_id,
                                   void **token_ptr,
                                   H5VL_object_t **_vol_obj_ptr);
static herr_t H5O__flush_api_common(hid_t obj_id, void **token_ptr,
                                    H5VL_object_t **_vol_obj_ptr);
static herr_t H5O__refresh_api_common(hid_t oid, void **token_ptr,
                                      H5VL_object_t **_vol_obj_ptr);
...
static hid_t H5O__open_api_common(hid_t loc_id, const char *name, hid_t lapl_id,
  ...
  if (H5VL_setup_name_args(loc_id, name, FALSE, lapl_id, vol_obj_ptr,
                           &loc_params) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");

  /* Open the object */
  if (NULL ==
      (opened_obj = H5VL_object_open(*vol_obj_ptr, &loc_params, &opened_type,
                                     H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open object");

  /* Get an atom for the object */
  if ((ret_value = H5VL_register(opened_type, opened_obj,
                                 (*vol_obj_ptr)->connector, TRUE)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to atomize object handle");
...
hid_t H5Oopen_async(const char *app_file, const char *app_func,
  ...
  if ((ret_value = H5O__open_api_common(loc_id, name, lapl_id, token_ptr,
                                        &vol_obj)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to asynchronously open object");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      if (H5I_dec_app_ref_always_close(ret_value) < 0)
        HDONE_ERROR(H5E_OHDR, H5E_CANTDEC, H5I_INVALID_HID,
                    "can't decrement count on object ID");
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINSERT, H5I_INVALID_HID,
 ...
 *
 * Purpose:     This is the common function for opening an object within an
 *index
 *
 * Return:      Success:    An open object identifier
 *              Failure:    H5I_INVALID_HID
...
static hid_t H5O__open_by_idx_api_common(hid_t loc_id, const char *group_name,
  ...
  /* Set up object access arguments */
  if (H5VL_setup_idx_args(loc_id, group_name, idx_type, order, n, FALSE,
                          lapl_id, vol_obj_ptr, &loc_params) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");
...
hid_t H5Oopen_by_idx(hid_t loc_id, const char *group_name, H5_index_t idx_type,
  ...
  /* Open the object synchronously */
  if ((ret_value = H5O__open_by_idx_api_common(
           loc_id, group_name, idx_type, order, n, lapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to synchronously open object");
...
hid_t H5Oopen_by_idx_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE10("i", "*s*sIui*sIiIohii", app_file, app_func, app_line, loc_id,
            group_name, idx_type, order, n, lapl_id, es_id);
  ...
  /* Open the object asynchronously */
  if ((ret_value =
           H5O__open_by_idx_api_common(loc_id, group_name, idx_type, order, n,
...
hid_t H5Oopen_by_token(hid_t loc_id, H5O_token_t token) {
  ...
  /* Open the object */
  if (NULL == (opened_obj =
                   H5VL_object_open(vol_obj, &loc_params, &opened_type,
                                    H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open object");

  /* Register the object's ID */
  if ((ret_value = H5VL_register(opened_type, opened_obj, vol_obj->connector,
                                 TRUE)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register object handle");
...
static herr_t H5O__copy_api_common(hid_t src_loc_id, const char *src_name,
  ...
  /* Copy the object */
  if (H5VL_object_copy(vol_obj1, &loc_params1, src_name, *vol_obj_ptr,
                       &loc_params2, dst_name, ocpypl_id, lcpl_id,
 ...
 *                          When this flag is specified, it will copy objects
 *that are pointed by the references and update the values of references in the
 *destination file.  Otherwise (default) the values of references in the
 *destination will set to zero The current implementation does not handle
 *references inside of other datatype structure. For example, if a member of
 *compound datatype is reference, H5Ocopy() will copy that field as it is. It
 *will not set the value to zero as default is used nor copy the object pointed
 *by that field the flag is set 2) Copy object within the same file: This flag
 *does not have any effect to the H5Ocopy(). Datasets or attributes of
 *references are copied as they are, i.e. values of references of the
 *destination object are the same as the values of the source object
 *
 *              OPTIONS THAT MAY APPLY TO COPY IN THE FUTURE.
 *                  H5O_COPY_EXPAND_EXT_LINK_FLAG
 *                      If this flag is specified, it will expand the external
 ...
 *                  Add an attribute to the copied object(s) that say the
 *date/time for the copy or other information about the source file.
 *
 *              The intermediate group creation property should be passed in
 *              using the lcpl instead of the ocpypl.
 *
 * Usage:      H5Ocopy(src_loc_id, src_name, dst_loc_id, dst_name, ocpypl_id,
 *lcpl_id) hid_t src_loc_id         IN: Source file or group identifier. const
 *char *src_name     IN: Name of the source object to be copied hid_t dst_loc_id
 *IN: Destination file or group identifier const char *dst_name     IN: Name of
 *the destination object hid_t ocpypl_id          IN: Properties which apply to
 *the copy hid_t lcpl_id            IN: Properties which apply to the new hard
 *link
 *
 *
 * Return:      SUCCEED/FAIL
...
herr_t H5Ocopy(hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
  ...
  if (H5O__copy_api_common(src_loc_id, src_name, dst_loc_id, dst_name,
                           ocpypl_id, lcpl_id, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL,
                "unable to synchronously copy object");
...
herr_t H5Ocopy_async(const char *app_file, const char *app_func,
                     unsigned app_line, hid_t src_loc_id, const char *src_name,
                     hid_t dst_loc_id, const char *dst_name, hid_t ocpypl_id,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIui*si*siii", app_file, app_func, app_line, src_loc_id,
            src_name, dst_loc_id, dst_name, ocpypl_id, lcpl_id, es_id);
  ...
  if (H5O__copy_api_common(src_loc_id, src_name, dst_loc_id, dst_name,
                           ocpypl_id, lcpl_id, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL,
                "unable to asynchronously copy object");
...
static herr_t H5O__flush_api_common(hid_t obj_id, void **token_ptr,
  ...
  /* Flush the object */
  if (H5VL_object_specific(*vol_obj_ptr, &loc_params, &vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, token_ptr) < 0)
...
herr_t H5Oflush(hid_t obj_id) {
  ...
  if (H5O__flush_api_common(obj_id, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFLUSH, FAIL,
                "unable to synchronously flush object");
...
herr_t H5Oflush_async(const char *app_file, const char *app_func,
  ...
  if (H5O__flush_api_common(obj_id, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFLUSH, FAIL,
                "unable to asynchronously flush object");
...
herr_t H5Orefresh(hid_t oid) {
  ...
  if (H5O__refresh_api_common(oid, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, FAIL,
                "unable to synchronously refresh object");
...
herr_t H5Orefresh_async(const char *app_file, const char *app_func,
  ...
  if (H5O__refresh_api_common(oid, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, FAIL,
                "unable to asynchronously refresh object");
...
/* Avoid compiler warning on 32-bit machines */
  ...
  if (lcpl_id != H5P_DEFAULT &&
      (TRUE != H5P_isa_class(lcpl_id, H5P_LINK_CREATE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
  ...
  if (vol_obj1 && vol_obj2) {
    ...
    if (H5VL_cmp_connector_cls(&same_connector, vol_obj1->connector->cls,
                               vol_obj2->connector->cls) < 0)
      HGOTO_ERROR(H5E_FILE, H5E_CANTCOMPARE, FAIL,
  ...
  if (H5VL_link_create(&vol_cb_args, &tmp_vol_obj, &new_loc_params, lcpl_id,
...
herr_t H5Oincr_refcount(hid_t object_id) {
  ...
  if (H5VL_object_specific(vol_obj, &loc_params, &vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_LINKCOUNT, FAIL,
                "modifying object link count failed");
...
htri_t H5Oexists_by_name(hid_t loc_id, const char *name, hid_t lapl_id) {
  ...
  if (H5VL_object_specific(vol_obj, &loc_params, &vol_cb_args,
    ...
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "unable to determine if '%s' exists", name);
...
herr_t H5Oget_info3(hid_t loc_id, H5O_info2_t *oinfo /*out*/, unsigned fields) {
  ...
  if (H5VL_object_get(vol_obj, &loc_params, &vol_cb_args,
                      H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't get data model info for object");
...
static herr_t H5O__get_info_by_name_api_common(hid_t loc_id, const char *name,
  ...
  /* Retrieve the object's information */
  if (H5VL_object_get(*vol_obj_ptr, &loc_params, &vol_cb_args,
                      H5P_DATASET_XFER_DEFAULT, token_ptr) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't get data model info for object");
...
herr_t H5Oget_info_by_name3(hid_t loc_id, const char *name,
  ...
  if (H5O__get_info_by_name_api_common(loc_id, name, oinfo, fields, lapl_id,
                                       NULL, NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't synchronously retrieve object info");
...
herr_t H5Oget_info_by_name_async(const char *app_file, const char *app_func,
                                 unsigned app_line, hid_t loc_id,
                                 const char *name, H5O_info2_t *oinfo /*out*/,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE9("e", "*s*sIui*sxIuii", app_file, app_func, app_line, loc_id, name,
           oinfo, fields, lapl_id, es_id);
  ...
  if (H5O__get_info_by_name_api_common(loc_id, name, oinfo, fields, lapl_id,
                                       token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't asynchronously retrieve object info");
...
herr_t H5Oget_info_by_idx3(hid_t loc_id, const char *group_name,
                           ...
                           hsize_t n, H5O_info2_t *oinfo /*out*/,
                           unsigned fields, hid_t lapl_id) {
  H5VL_object_t *vol_obj;             /* Object of loc_id */
  H5VL_object_get_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_loc_params_t loc_params;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Oget_native_info(hid_t loc_id, H5O_native_info_t *oinfo /*out*/,
                          unsigned fields) {
  H5VL_object_t *vol_obj;           /* Object of loc_id */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_object_optional_args_t
      obj_opt_args; /* Arguments for optional operation */
  H5VL_loc_params_t loc_params;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Retrieve the object's information */
  if (H5VL_object_optional(vol_obj, &loc_params, &vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't get native file format info for object");
...
herr_t H5Oget_native_info_by_name(hid_t loc_id, const char *name,
  ...
  if (H5VL_object_optional(vol_obj, &loc_params, &vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't get native file format info for object: '%s'", name);
...
herr_t H5Oget_native_info_by_idx(hid_t loc_id, const char *group_name,
                                 H5_index_t idx_type, H5_iter_order_t order,
                                 hsize_t n, H5O_native_info_t *oinfo /*out*/,
                                 unsigned fields, hid_t lapl_id) {
  H5VL_object_t *vol_obj;           /* Object of loc_id */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_object_optional_args_t
...
herr_t H5Oset_comment_by_name(hid_t loc_id, const char *name,
  ...
  if (H5VL_object_optional(vol_obj, &loc_params, &vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                "can't set comment for object: '%s'", name);
...
ssize_t H5Oget_comment_by_name(hid_t loc_id, const char *name,
   ...
   */
  if (H5CX_set_apl(&lapl_id, H5P_CLS_LACC, loc_id, FALSE) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, (-1),
                "can't set access property list info");
  ...
  if (H5VL_object_optional(vol_obj, &loc_params, &vol_cb_args,
                           H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, (-1),
                "can't get comment for object: '%s'", name);
...
herr_t H5Ovisit3(hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
  ...
  /* Visit the objects */
  if ((ret_value =
           H5VL_object_specific(vol_obj, &loc_params, &vol_cb_args,
                                H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)) < 0)
...
herr_t H5Ovisit_by_name3(hid_t loc_id, const char *obj_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE8("e", "i*sIiIoOI*xIui", loc_id, obj_name, idx_type, order, op,
           op_data, fields, lapl_id);

  /* Check args */
  if (!obj_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "obj_name parameter cannot be NULL");
  if (!*obj_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "obj_name parameter cannot be an empty string");
  if (idx_type <= H5_INDEX_UNKNOWN || idx_type >= H5_INDEX_N)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "invalid index type specified");
  if (order <= H5_ITER_UNKNOWN || order >= H5_ITER_N)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
...
herr_t H5Oclose_async(const char *app_file, const char *app_func,
  ...
  if (H5ES_NONE != es_id) {
    ...
    if (NULL == (vol_obj = H5VL_vol_object(object_id)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                  "can't get VOL object for object");
  ...
  if (H5I_dec_app_ref_async(object_id, token_ptr) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCLOSEFILE, FAIL,
  ...
  if (connector && H5VL_conn_dec_rc(connector) < 0)
    HDONE_ERROR(H5E_OHDR, H5E_CANTDEC, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__are_mdc_flushes_disabled(const H5O_loc_t *oloc,
  ...
  if (H5AC_cork(oloc->file, oloc->addr, H5AC__GET_CORKED, are_disabled) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "unable to retrieve object's cork status");
 ...
 *
 * Purpose:     Retrieve the object's "cork" status in the parameter
 *"are_disabled": TRUE if mdc flushes for the object is disabled FALSE if mdc
 *flushes for the object is not disabled
 *
...
herr_t H5Oare_mdc_flushes_disabled(hid_t object_id, hbool_t *are_disabled) {
  ...
  if (!are_disabled)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "unable to get object location from ID");
...
herr_t H5Otoken_cmp(hid_t loc_id, const H5O_token_t *token1,
  ...
  if (H5VL_token_cmp(vol_obj, token1, token2, cmp_value) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOMPARE, FAIL,
...
herr_t H5Otoken_from_str(hid_t loc_id, const char *token_str,
  ...
  if (H5VL_token_from_str(vol_obj, vol_obj_type, token_str, token) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTUNSERIALIZE, FAIL,
                "object token deserialization failed");
