...
#define H5_MULTI_DIAG_OFF(x)                                                   \
  H5_MULTI_DIAG_PRAGMA(push)                                                   \
  H5_MULTI_DIAG_PRAGMA(ignored H5_MULTI_DIAG_JOINSTR("-W", x))
...
#define UNIQUE_MEMBERS_CORE(MAP, ITER, SEEN, LOOPVAR)                          \
  {                                                                            \
    ...
    for (ITER = H5FD_MEM_SUPER; ITER < H5FD_MEM_NTYPES;                        \
         ITER = (H5FD_mem_t)(ITER + 1)) {                                      \
      LOOPVAR = MAP[ITER];                                                     \
...
#define ALL_MEMBERS(LOOPVAR)                                                   \
  {                                                                            \
    H5FD_mem_t LOOPVAR;                                                        \
    for (LOOPVAR = H5FD_MEM_DEFAULT; LOOPVAR < H5FD_MEM_NTYPES;                \
         LOOPVAR = (H5FD_mem_t)(LOOPVAR + 1)) {
...
typedef struct H5FD_multi_t {
  ...
  haddr_t
      memb_eoa[H5FD_MEM_NTYPES]; /*EOA for individual files,
...
/* Private functions */
static herr_t H5FD_split_populate_config(const char *meta_ext,
                                         hid_t meta_plist_id,
                                         const char *raw_ext,
                                         hid_t raw_plist_id, hbool_t relax,
...
static H5FD_t *H5FD_multi_open(const char *name, unsigned flags, hid_t fapl_id,
                               haddr_t maxaddr);
...
static herr_t H5FD_multi_get_handle(H5FD_t *_file, hid_t fapl,
                                    void **file_handle);
static haddr_t H5FD_multi_alloc(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                                hsize_t size);
static herr_t H5FD_multi_free(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                              haddr_t addr, hsize_t size);
static herr_t H5FD_multi_read(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                              haddr_t addr, size_t size, void *_buf /*out*/);
static herr_t H5FD_multi_write(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                               haddr_t addr, size_t size, const void *_buf);
static herr_t H5FD_multi_flush(H5FD_t *_file, hid_t dxpl_id, hbool_t closing);
static herr_t H5FD_multi_truncate(H5FD_t *_file, hid_t dxpl_id,
                                  hbool_t closing);
...
static herr_t H5FD_multi_ctl(H5FD_t *_file, uint64_t op_code, uint64_t flags,
...
herr_t H5Pset_fapl_split(hid_t fapl, const char *meta_ext, hid_t meta_plist_id,
  ...
  static const char *func =
      "H5Pset_fapl_split"; /* Function Name for error reporting */
...
herr_t H5Pset_fapl_multi(hid_t fapl_id, const H5FD_mem_t *memb_map,
  ...
  H5FD_multi_fapl_t fa;
  static const char *func =
      "H5FDset_fapl_multi"; /* Function Name for error reporting */
  ...
  if (H5I_GENPROP_LST != H5Iget_type(fapl_id) ||
    ...
    H5Epush_ret(func, H5E_ERR_CLS, H5E_PLIST, H5E_BADVALUE,
                "not an access list", -1);
  if (H5FD_multi_populate_config(memb_map, memb_fapl, memb_name, memb_addr,
                                 relax, &fa) < 0)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_CANTSET,
                "can't setup driver configuration", -1);
...
herr_t H5Pget_fapl_multi(hid_t fapl_id, H5FD_mem_t *memb_map /*out*/,
  ...
  H5FD_mem_t mt;
  static const char *func =
      "H5FDget_fapl_multi"; /* Function Name for error reporting */
  ...
  if (H5FD_MULTI != H5Pget_driver(fapl_id))
    H5Epush_ret(func, H5E_ERR_CLS, H5E_PLIST, H5E_BADVALUE,
                "incorrect VFL driver", -1);
  H5E_BEGIN_TRY { fa = (const H5FD_multi_fapl_t *)H5Pget_driver_info(fapl_id); }
  H5E_END_TRY
  if (!fa || (H5P_FILE_ACCESS_DEFAULT == fapl_id)) {
    if (H5FD_multi_populate_config(NULL, NULL, NULL, NULL, TRUE, &default_fa) <
        0)
      H5Epush_ret(func, H5E_ERR_CLS, H5E_VFL, H5E_CANTSET,
  ...
  if (memb_fapl) {
    for (mt = H5FD_MEM_DEFAULT; mt < H5FD_MEM_NTYPES;
         mt = (H5FD_mem_t)(mt + 1)) {
...
static herr_t H5FD_split_populate_config(const char *meta_ext,
                                         ...
                                         H5FD_multi_fapl_t *fa_out) {
  static const char *func =
      "H5FD_split_populate_config"; /* Function Name for error reporting */
  static char
      meta_name_g[H5FD_MULT_MAX_FILE_NAME_LEN]; /* Static scratch buffer to
                                                   store metadata member name */
  static char
      raw_name_g[H5FD_MULT_MAX_FILE_NAME_LEN]; /* Static scratch buffer to store
                                                  raw data member name */
  ...
  ALL_MEMBERS(mt) {
    ...
    _memb_map[mt] =
        ((mt == H5FD_MEM_DRAW || mt == H5FD_MEM_GHEAP) ? H5FD_MEM_DRAW
                                                       : H5FD_MEM_SUPER);
  ...
  ALL_MEMBERS(mt) {
    ...
    if (mmt < 0 || mmt >= H5FD_MEM_NTYPES)
      H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADRANGE,
                  "file resource type out of range", -1);

    /*
     * All members of MEMB_FAPL must be either defaults or actual file
     * access property lists.
     */
    if (H5P_DEFAULT != _memb_fapl[mmt] &&
        TRUE != H5Pisa_class(_memb_fapl[mmt], H5P_FILE_ACCESS))
      H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                  "file resource type incorrect", -1);

    /* All names must be defined */
    if (!_memb_name[mmt] || !_memb_name[mmt][0])
      H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                  "file resource type not set", -1);
  }
  END_MEMBERS
...
static herr_t H5FD_multi_populate_config(const H5FD_mem_t *memb_map,
                                         ...
                                         H5FD_multi_fapl_t *fa_out) {
  static const char *func =
      "H5FD_multi_populate_config"; /* Function Name for error reporting */
  static const char *letters = "Xsbrglo";
  static char _memb_name_g[H5FD_MEM_NTYPES][16]; /* Static scratch buffer to
                                                    store member names */
  H5FD_mem_t mt, mmt;
  H5FD_mem_t _memb_map[H5FD_MEM_NTYPES];
  hid_t _memb_fapl[H5FD_MEM_NTYPES];
  ...
  if (!memb_addr) {
    for (mt = H5FD_MEM_DEFAULT; mt < H5FD_MEM_NTYPES; mt = (H5FD_mem_t)(mt + 1))
      _memb_addr[mt] =
          (hsize_t)(mt ? (mt - 1) : 0) * (HADDR_MAX / (H5FD_MEM_NTYPES - 1));
  ...
  for (mt = H5FD_MEM_DEFAULT; mt < H5FD_MEM_NTYPES; mt = (H5FD_mem_t)(mt + 1)) {
    ...
    if (H5P_DEFAULT != memb_fapl[mmt] &&
        TRUE != H5Pisa_class(memb_fapl[mmt], H5P_FILE_ACCESS))
      H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                  "file resource type incorrect", -1);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_multi_sb_encode(H5FD_t *_file, char *name /*out*/,
                                   unsigned char *buf /*out*/) {
  H5FD_multi_t *file = (H5FD_multi_t *)_file;
  haddr_t memb_eoa;
  unsigned char *p;
  ...
  H5FD_mem_t m;
  static const char *func =
      "H5FD_multi_sb_encode"; /* Function Name for error reporting */
  ...
  if (H5Tconvert(H5T_NATIVE_HADDR, H5T_STD_U64LE, nseen * 2, buf + 8, NULL,
                 H5P_DEFAULT) < 0)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_DATATYPE, H5E_CANTCONVERT,
                "can't convert superblock info", -1);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_multi_sb_decode(H5FD_t *_file, const char *name,
                                   const unsigned char *buf) {
  H5FD_multi_t *file = (H5FD_multi_t *)_file;
  ...
  haddr_t *ap;
  static const char *func =
      "H5FD_multi_sb_decode"; /* Function Name for error reporting */
  ...
  if (strcmp(name, "NCSAmult") != 0)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_FILE, H5E_BADVALUE,
                "invalid multi superblock", -1);
  ...
  if (H5Tconvert(H5T_STD_U64LE, H5T_NATIVE_HADDR, nseen * 2, x, NULL,
    ...
    H5Epush_ret(func, H5E_ERR_CLS, H5E_DATATYPE, H5E_CANTCONVERT,
                "can't convert superblock info", -1);
  ap = (haddr_t *)((void *)x); /* Extra (void *) cast to quiet "cast to create
  ...
  if (compute_next(file) < 0)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                "compute_next() failed", -1);

  /* Open all necessary files */
  if (open_members(file) < 0)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                "open_members() failed", -1);
  ...
  UNIQUE_MEMBERS(file->fa.memb_map, mt) {
    ...
    /* Save the individual EOAs in one place for later comparison (in
     * H5FD_multi_set_eoa)
     */
...
static void *H5FD_multi_fapl_copy(const void *_old_fa) {
  ...
  H5FD_multi_fapl_t *new_fa =
      (H5FD_multi_fapl_t *)calloc(1, sizeof(H5FD_multi_fapl_t));
  int nerrors = 0;
  static const char *func =
      "H5FD_multi_fapl_copy"; /* Function Name for error reporting */
  ...
  if (nerrors) {
    ...
    free(new_fa);
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                "can't release object on error", NULL);
  }
  return new_fa;
}
...
static herr_t H5FD_multi_fapl_free(void *_fa) {
  ...
  static const char *func =
      "H5FD_multi_fapl_free"; /* Function Name for error reporting */
  ...
  ALL_MEMBERS(mt) {
    if (fa->memb_fapl[mt] >= 0)
      if (H5Idec_ref(fa->memb_fapl[mt]) < 0)
        H5Epush_ret(func, H5E_ERR_CLS, H5E_FILE, H5E_CANTCLOSEOBJ,
                    "can't close property list", -1);
    if (fa->memb_name[mt])
...
static H5FD_t *H5FD_multi_open(const char *name, unsigned flags, hid_t fapl_id,
  ...
  H5FD_mem_t m;
  static const char *func =
      "H5FD_multi_open"; /* Function Name for error reporting */
  ...
  H5E_END_TRY
  if (!fa || (H5P_FILE_ACCESS_DEFAULT == fapl_id) ||
      (H5FD_MULTI != H5Pget_driver(fapl_id))) {
    ...
    if (env && !strcmp(env, "split")) {
      if (H5Pset_fapl_split(fapl_id, NULL, H5P_DEFAULT, NULL, H5P_DEFAULT) < 0)
        H5Epush_goto(func, H5E_ERR_CLS, H5E_FILE, H5E_CANTSET,
                     "can't set property value", error);
    } else {
  ...
  if (close_fapl >= 0)
    if (H5Pclose(close_fapl) < 0)
      H5Epush_goto(func, H5E_ERR_CLS, H5E_FILE, H5E_CANTCLOSEOBJ,
                   "can't close property list", error);

  /* Compute derived properties and open member files */
  if (compute_next(file) < 0)
    H5Epush_goto(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                 "compute_next() failed", error);
  if (open_members(file) < 0)
    H5Epush_goto(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                 "open_members() failed", error);
...
static herr_t H5FD_multi_close(H5FD_t *_file) {
  ...
  int nerrors = 0;
  static const char *func =
      "H5FD_multi_close"; /* Function Name for error reporting */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_multi_query(const H5FD_t *_f,
  ...
  if (flags) {
    ...
    *flags |= H5FD_FEAT_AGGREGATE_SMALLDATA; /* OK to aggregate "small" raw data
                                                allocations */
    *flags |= H5FD_FEAT_USE_ALLOC_SIZE; /* OK just pass the allocation size to
                                           the alloc callback */
    *flags |= H5FD_FEAT_PAGED_AGGR; /* OK special file space mapping for paged
                                       aggregation */
  }                                 /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD_multi_get_type_map(const H5FD_t *_file,
                                      H5FD_mem_t *type_map) {
...
static haddr_t H5FD_multi_get_eoa(const H5FD_t *_file, H5FD_mem_t type) {
  ...
  haddr_t eoa = 0;
  static const char *func =
      "H5FD_multi_get_eoa"; /* Function Name for error reporting */
  ...
  if (H5FD_MEM_DEFAULT == type) {
    UNIQUE_MEMBERS(file->fa.memb_map, mt) {
      ...
      if (file->memb[mt]) {
        ...
        H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE, "bad eoa",
                    HADDR_UNDEF);
      }
...
static herr_t H5FD_multi_set_eoa(H5FD_t *_file, H5FD_mem_t type, haddr_t eoa) {
  ...
  herr_t status;
  static const char *func =
      "H5FD_multi_set_eoa"; /* Function Name for error reporting */
...
static haddr_t H5FD_multi_get_eof(const H5FD_t *_file, H5FD_mem_t type) {
  ...
  haddr_t eof = 0;
  static const char *func =
      "H5FD_multi_get_eof"; /* Function Name for error reporting */
  ...
  if (H5FD_MEM_DEFAULT == type) {
    UNIQUE_MEMBERS(file->fa.memb_map, mt) {
      ...
      if (file->memb[mt]) {
        ...
        H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE, "bad eof",
                    HADDR_UNDEF);
      }
      if (tmp_eof > eof)
        eof = tmp_eof;
...
static herr_t H5FD_multi_get_handle(H5FD_t *_file, hid_t fapl,
  ...
  H5FD_mem_t type, mmt;
  static const char *func =
      "H5FD_multi_get_handle"; /* Function Name for error reporting */

  /* Get data type for multi driver */
  if (H5Pget_multi_type(fapl, &type) < 0)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                "can't get data type for multi driver", -1);
  if (type < H5FD_MEM_DEFAULT || type >= H5FD_MEM_NTYPES)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                "data type is out of range", -1);
  mmt = file->fa.memb_map[type];
  if (H5FD_MEM_DEFAULT == mmt)
    mmt = type;
...
static haddr_t H5FD_multi_alloc(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
  ...
  haddr_t addr;
  static const char *func =
      "H5FD_multi_alloc"; /* Function Name for error reporting */
  ...
  if (HADDR_UNDEF == (addr = H5FDalloc(file->memb[mmt], mmt, dxpl_id, size)))
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                "member file can't alloc", HADDR_UNDEF);
...
static herr_t H5FD_multi_free(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
  ...
  return H5FDfree(file->memb[mmt], mmt, dxpl_id, addr - file->fa.memb_addr[mmt],
                  size);
}
...
static herr_t H5FD_multi_write(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
  ...
  return H5FDwrite(file->memb[hi], type, dxpl_id, addr - start_addr, size,
                   _buf);
...
static herr_t H5FD_multi_flush(H5FD_t *_file, hid_t dxpl_id, hbool_t closing) {
  ...
  int nerrors = 0;
  static const char *func =
      "H5FD_multi_flush"; /* Function Name for error reporting */
...
static herr_t H5FD_multi_truncate(H5FD_t *_file, hid_t dxpl_id,
  ...
  int nerrors = 0;
  static const char *func =
      "H5FD_multi_truncate"; /* Function Name for error reporting */
  ...
  if (nerrors)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_INTERNAL, H5E_BADVALUE,
                "error truncating member files", -1);
...
static herr_t H5FD_multi_lock(H5FD_t *_file, hbool_t rw) {
  ...
  H5FD_mem_t out_mt = H5FD_MEM_DEFAULT;
  static const char *func =
      "H5FD_multi_unlock"; /* Function Name for error reporting */
  ...
  if (nerrors)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_VFL, H5E_CANTLOCKFILE,
                "error locking member files", -1);
  return 0;
...
static herr_t H5FD_multi_unlock(H5FD_t *_file) {
  ...
  if (nerrors)
    H5Epush_ret(func, H5E_ERR_CLS, H5E_VFL, H5E_CANTUNLOCKFILE,
                "error unlocking member files", -1);
...
static int open_members(H5FD_multi_t *file) {
  ...
  int nchars;
  static const char *func =
      "(H5FD_multi)open_members"; /* Function Name for error reporting */
  ...
  UNIQUE_MEMBERS(file->fa.memb_map, mt) {
    ...
    H5E_BEGIN_TRY {
      file->memb[mt] =
          H5FDopen(tmp, file->flags, file->fa.memb_fapl[mt], HADDR_UNDEF);
    }
    H5E_END_TRY
    if (!file->memb[mt]) {
...
static herr_t H5FD_multi_delete(const char *filename, hid_t fapl_id) {
  ...
  H5FD_multi_fapl_t default_fa;
  static const char *func =
      "H5FD_multi_delete"; /* Function Name for error reporting    */
  ...
  if (!fa) {
    ...
    if (env && !strcmp(env, "split")) {
      if (H5FD_split_populate_config(NULL, H5P_DEFAULT, NULL, H5P_DEFAULT, TRUE,
                                     &default_fa) < 0)
        H5Epush_ret(func, H5E_ERR_CLS, H5E_VFL, H5E_CANTSET,
                    "can't setup driver configuration", -1);
    } else {
      if (H5FD_multi_populate_config(NULL, NULL, NULL, NULL, TRUE,
                                     &default_fa) < 0)
        H5Epush_ret(func, H5E_ERR_CLS, H5E_VFL, H5E_CANTSET,
                    "can't setup driver configuration", -1);
    }
  ...
  UNIQUE_MEMBERS(fa->memb_map, mt) {
    ...
    nchars = snprintf(full_filename, sizeof(full_filename), fa->memb_name[mt],
                      filename);
...
static herr_t H5FD_multi_ctl(H5FD_t *_file, uint64_t op_code, uint64_t flags,
  ...
  H5FD_multi_t *file = (H5FD_multi_t *)_file;
  static const char *func =
      "H5FD_multi_ctl"; /* Function Name for error reporting */
  herr_t ret_value = 0;
  ...
  default:
    if (flags & H5FD_CTL_FAIL_IF_UNKNOWN_FLAG)
      H5Epush_ret(func, H5E_ERR_CLS, H5E_VFL, H5E_FCNTL,
                  "VFD ctl request failed (unknown op code and fail if unknown "
                  "flag is set)",
                  -1);
