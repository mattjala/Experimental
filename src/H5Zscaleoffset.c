...
static htri_t H5Z__can_apply_scaleoffset(hid_t dcpl_id, hid_t type_id,
                                         hid_t space_id);
static enum H5Z_scaleoffset_t H5Z__scaleoffset_get_type(unsigned dtype_class,
                                                        unsigned dtype_size,
                                                        unsigned dtype_sign);
static herr_t
H5Z__scaleoffset_set_parms_fillval(H5P_genplist_t *dcpl_plist, H5T_t *type,
                                   enum H5Z_scaleoffset_t scale_type,
                                   unsigned cd_values[], int need_convert);
static herr_t H5Z__set_local_scaleoffset(hid_t dcpl_id, hid_t type_id,
                                         hid_t space_id);
static size_t H5Z__filter_scaleoffset(unsigned flags, size_t cd_nelmts,
                                      const unsigned cd_values[], size_t nbytes,
                                      size_t *buf_size, void **buf);
static void H5Z__scaleoffset_convert(void *buf, unsigned d_nelmts,
                                     unsigned dtype_size);
...
static void H5Z__scaleoffset_postdecompress_i(void *data, unsigned d_nelmts,
                                              ...
                                              unsigned long long minval);
static herr_t
H5Z__scaleoffset_precompress_fd(void *data, unsigned d_nelmts,
                                enum H5Z_scaleoffset_t type, unsigned filavail,
                                const unsigned cd_values[], uint32_t *minbits,
                                unsigned long long *minval, double D_val);
static herr_t H5Z__scaleoffset_postdecompress_fd(
    void *data, unsigned d_nelmts, enum H5Z_scaleoffset_t type,
...
static void H5Z__scaleoffset_decompress_one_byte(
    unsigned char *data, size_t data_offset, unsigned k, unsigned begin_i,
    const unsigned char *buffer, size_t *j, unsigned *buf_len, parms_atomic p,
    unsigned dtype_len);
static void H5Z__scaleoffset_compress_one_byte(
    const unsigned char *data, size_t data_offset, unsigned k, unsigned begin_i,
    unsigned char *buffer, size_t *j, unsigned *buf_len, parms_atomic p,
    unsigned dtype_len);
static void H5Z__scaleoffset_decompress_one_atomic(unsigned char *data,
                                                   size_t data_offset,
                                                   unsigned char *buffer,
                                                   size_t *j, unsigned *buf_len,
                                                   parms_atomic p);
static void H5Z__scaleoffset_compress_one_atomic(unsigned char *data,
                                                 size_t data_offset,
                                                 unsigned char *buffer,
                                                 size_t *j, unsigned *buf_len,
                                                 parms_atomic p);
...
static void H5Z__scaleoffset_compress(unsigned char *data, unsigned d_nelmts,
...
/* Local macros */
#define H5Z_SCALEOFFSET_TOTAL_NPARMS                                           \
  20 /* Total number of parameters for filter */
#define H5Z_SCALEOFFSET_PARM_SCALETYPE 0 /* "User" parameter for scale type */
#define H5Z_SCALEOFFSET_PARM_SCALEFACTOR                                       \
  1 /* "User" parameter for scale factor */
#define H5Z_SCALEOFFSET_PARM_NELMTS                                            \
  2 /* "Local" parameter for number of elements in the chunk */
...
#define H5Z_SCALEOFFSET_PARM_SIGN                                              \
  5 /* "Local" parameter for integer datatype sign */
#define H5Z_SCALEOFFSET_PARM_ORDER                                             \
  6 /* "Local" parameter for datatype byte order */
#define H5Z_SCALEOFFSET_PARM_FILAVAIL                                          \
  7 /* "Local" parameter for dataset fill value existence */
#define H5Z_SCALEOFFSET_PARM_FILVAL                                            \
  8 /* "Local" parameter for start location to store dataset fill value */
...
#define H5Z_scaleoffset_save_filval(type, cd_values, fill_val)                 \
  {                                                                            \
    ...
    size_t _size_rem =                                                         \
        sizeof(type); /* # of bytes left to copy to cd_values */               \
                                                                               \
...
#define H5Z_scaleoffset_set_filval_1(type, dcpl_plist, dt, cd_values,          \
                                     need_convert)                             \
  do {                                                                         \
    type fill_val;                                                             \
                                                                               \
...
#define H5Z_scaleoffset_set_filval_2(type, dcpl_plist, dt, cd_values,          \
                                     need_convert)                             \
  do {                                                                         \
    type fill_val;                                                             \
                                                                               \
...
#define H5Z_scaleoffset_set_filval_3(type, dcpl_plist, dt, cd_values,          \
                                     need_convert)                             \
  do {                                                                         \
    type fill_val;                                                             \
                                                                               \
    ...
    if (H5P_get_fill_value(dcpl_plist, dt, &fill_val) < 0)                     \
      HGOTO_ERROR(H5E_PLINE, H5E_CANTGET, FAIL, "unable to get fill value");   \
                                                                               \
    /* Store the fill value as the last entry in cd_values[] */                \
    (cd_values)[H5Z_SCALEOFFSET_PARM_FILVAL] =                                 \
        (unsigned)((unsigned char)fill_val);                                   \
  } while (0)

/* Set the fill value parameter in cd_values[] for floating-point type */
#define H5Z_scaleoffset_set_filval_4(type, dcpl_plist, dt, cd_values,          \
                                     need_convert)                             \
  do {                                                                         \
    type fill_val;                                                             \
                                                                               \
  ...
  do {                                                                         \
    if (sizeof(type) <= sizeof(long long))                                     \
      ...
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,                                \
                  "cannot find matched integer datatype");                     \
  } while (0)

/* Find maximum and minimum values of a buffer with fill value defined for
 * integer type */
...
#define H5Z_scaleoffset_max_min_3(i, d_nelmts, buf, filval, max, min, D_val)   \
  {                                                                            \
    i = 0;                                                                     \
    while (i < d_nelmts &&                                                     \
           HDfabs((double)(buf[i] - filval)) < HDpow(10.0, -D_val))            \
      i++;                                                                     \
    if (i < d_nelmts)                                                          \
      min = max = buf[i];                                                      \
    for (; i < d_nelmts; i++) {                                                \
...
#define H5Z_scaleoffset_check_3(i, type, pow_fun, round_fun, max, min,         \
                                minbits, D_val)                                \
  {                                                                            \
...
#define H5Z_scaleoffset_precompress_1(type, data, d_nelmts, filavail,          \
  ...
  do {                                                                         \
    ...
    if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */   \
      H5Z_scaleoffset_get_filval_1(type, cd_values, filval);                   \
      if (*minbits ==                                                          \
          H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max, \
                                           min, and minbits */                 \
        H5Z_scaleoffset_max_min_1(i, d_nelmts, buf, filval, max, min)          \
            H5Z_scaleoffset_check_1(type, max, min, minbits) span =            \
                (type)(max - min + 1);                                         \
      ...
      if (*minbits !=                                                          \
          sizeof(type) * 8) /* change values if minbits != full precision */   \
        for (i = 0; i < d_nelmts; i++)                                         \
          buf[i] = (type)((buf[i] == filval) ? (((type)1 << *minbits) - 1)     \
                                             : (buf[i] - min));                \
...
#define H5Z_scaleoffset_precompress_2(type, data, d_nelmts, filavail,          \
                                      cd_values, minbits, minval)              \
  do {                                                                         \
    ...
    if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */   \
      ...
      if (*minbits !=                                                          \
          sizeof(type) * 8) /* change values if minbits != full precision */   \
        for (i = 0; i < d_nelmts; i++)                                         \
          buf[i] = (type)((buf[i] == filval)                                   \
                              ? (type)(((unsigned type)1 << *minbits) - 1)     \
                              : (buf[i] - min));                               \
    } else { /* fill value undefined */                                        \
      if (*minbits ==                                                          \
          H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max, \
...
/* Modify values of data in precompression if fill value defined for
 * floating-point type */
#define H5Z_scaleoffset_modify_1(i, type, pow_fun, abs_fun, lround_fun,        \
                                 llround_fun, buf, d_nelmts, filval, minbits,  \
                                 min, D_val)                                   \
  {                                                                            \
    if (sizeof(type) == sizeof(int))                                           \
      for (i = 0; i < d_nelmts; i++) {                                         \
        if (abs_fun(buf[i] - filval) < pow_fun((type)10, (type)-D_val))        \
          *(int *)((void *)&buf[i]) =                                          \
              (int)(((unsigned int)1 << *minbits) - 1);                        \
        else                                                                   \
          *(int *)((void *)&buf[i]) =                                          \
              (int)lround_fun(buf[i] * pow_fun((type)10, (type)D_val) -        \
                              min * pow_fun((type)10, (type)D_val));           \
      }                                                                        \
    else if (sizeof(type) == sizeof(long))                                     \
      for (i = 0; i < d_nelmts; i++) {                                         \
        if (abs_fun(buf[i] - filval) < pow_fun((type)10, (type)-D_val))        \
          *(long *)((void *)&buf[i]) =                                         \
              (long)(((unsigned long)1 << *minbits) - 1);                      \
        else                                                                   \
          *(long *)((void *)&buf[i]) =                                         \
              lround_fun(buf[i] * pow_fun((type)10, (type)D_val) -             \
                         min * pow_fun((type)10, (type)D_val));                \
      }                                                                        \
    else if (sizeof(type) == sizeof(long long))                                \
      for (i = 0; i < d_nelmts; i++) {                                         \
...
/* Modify values of data in precompression if fill value undefined for
 * floating-point type */
#define H5Z_scaleoffset_modify_2(i, type, pow_fun, lround_fun, llround_fun,    \
                                 buf, d_nelmts, min, D_val)                    \
  {                                                                            \
    if (sizeof(type) == sizeof(int))                                           \
      for (i = 0; i < d_nelmts; i++)                                           \
        *(int *)((void *)&buf[i]) =                                            \
...
#define H5Z_scaleoffset_precompress_3(                                         \
    type, pow_fun, abs_fun, round_fun, lround_fun, llround_fun, data,          \
  ...
  do {                                                                         \
    ...
    *minval = 0;                                                               \
    if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */   \
      ...
      H5Z_scaleoffset_check_3(i, type, pow_fun, round_fun, max, min, minbits,  \
                              D_val);                                          \
      span = (unsigned long long)(llround_fun(                                 \
                                      max * pow_fun((type)10, (type)D_val) -   \
                                      min * pow_fun((type)10, (type)D_val)) +  \
                                  1);                                          \
      *minbits = H5Z__scaleoffset_log2(span + 1);                              \
      if (*minbits !=                                                          \
          sizeof(type) * 8) /* change values if minbits != full precision */   \
        H5Z_scaleoffset_modify_1(i, type, pow_fun, abs_fun, lround_fun,        \
                                 llround_fun, buf, d_nelmts, filval, minbits,  \
                                 min, D_val);                                  \
    } else { /* fill value undefined */                                        \
      H5Z_scaleoffset_max_min_2(i, d_nelmts, buf, max, min);                   \
      ...
      *minbits = H5Z__scaleoffset_log2(span);                                  \
      if (*minbits !=                                                          \
          sizeof(type) * 8) /* change values if minbits != full precision */   \
        H5Z_scaleoffset_modify_2(i, type, pow_fun, lround_fun, llround_fun,    \
                                 buf, d_nelmts, min, D_val);                   \
    }                                                                          \
...
#define H5Z_scaleoffset_postdecompress_1(type, data, d_nelmts, filavail,       \
                                         cd_values, minbits, minval)           \
  do {                                                                         \
...
#define H5Z_scaleoffset_postdecompress_2(type, data, d_nelmts, filavail,       \
                                         cd_values, minbits, minval)           \
  do {                                                                         \
    type *buf = (type *)data, filval = 0;                                      \
    unsigned i;                                                                \
    ...
    if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */   \
      H5Z_scaleoffset_get_filval_1(type, cd_values, filval);                   \
      for (i = 0; i < d_nelmts; i++)                                           \
        buf[i] = (type)(((unsigned type)buf[i] ==                              \
                         (((unsigned type)1 << minbits) - 1))                  \
                            ? filval                                           \
                            : (buf[i] + minval));                              \
    } else /* fill value undefined */                                          \
      for (i = 0; i < d_nelmts; i++)                                           \
  ...
  do {                                                                         \
    if (sizeof(type) <= sizeof(long long))                                     \
      ...
      if (H5T_native_order_g == H5T_ORDER_LE)                                  \
        ...
        assert(H5T_native_order_g == H5T_ORDER_BE);                            \
        H5MM_memcpy(&min,                                                      \
                    ((char *)&minval) + (sizeof(long long) - sizeof(type)),    \
                    sizeof(type));                                             \
      } /* end else */                                                         \
    else                                                                       \
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,                                \
...
/* Modify values of data in postdecompression if fill value defined for
 * floating-point type */
#define H5Z_scaleoffset_modify_3(i, type, pow_fun, buf, d_nelmts, filval,      \
                                 minbits, min, D_val)                          \
  do {                                                                         \
    if (sizeof(type) == sizeof(int))                                           \
      for (i = 0; i < d_nelmts; i++)                                           \
        buf[i] = (type)((*(int *)((void *)&buf[i]) ==                          \
                         (int)(((unsigned int)1 << minbits) - 1))              \
                            ? filval                                           \
                            : (type)(*(int *)((void *)&buf[i])) /              \
                                      pow_fun((type)10, (type)D_val) +         \
                                  min);                                        \
    else if (sizeof(type) == sizeof(long))                                     \
      for (i = 0; i < d_nelmts; i++)                                           \
        buf[i] = (type)((*(long *)((void *)&buf[i]) ==                         \
                         (long)(((unsigned long)1 << minbits) - 1))            \
                            ? filval                                           \
                            : (type)(*(long *)((void *)&buf[i])) /             \
                                      pow_fun((type)10, (type)D_val) +         \
                                  min);                                        \
    else if (sizeof(type) == sizeof(long long))                                \
      for (i = 0; i < d_nelmts; i++)                                           \
        buf[i] = (type)((*(long long *)((void *)&buf[i]) ==                    \
                         (long long)(((unsigned long long)1 << minbits) - 1))  \
                            ? filval                                           \
...
/* Modify values of data in postdecompression if fill value undefined for
 * floating-point type */
...
#define H5Z_scaleoffset_postdecompress_3(type, pow_fun, data, d_nelmts,        \
  ...
  do {                                                                         \
    ...
    if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */   \
      H5Z_scaleoffset_get_filval_2(type, cd_values, filval);                   \
      H5Z_scaleoffset_modify_3(i, type, pow_fun, buf, d_nelmts, filval,        \
                               minbits, min, D_val);                           \
    } else /* fill value undefined */                                          \
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5Z__can_apply_scaleoffset(hid_t H5_ATTR_UNUSED dcpl_id,
                                         hid_t type_id,
  ...
  if (dtype_class == H5T_INTEGER || dtype_class == H5T_FLOAT) {
    ...
    if (dtype_order != H5T_ORDER_LE && dtype_order != H5T_ORDER_BE)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FALSE,
                  "bad datatype endianness order");
  } else
    HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FALSE,
                "datatype class not supported by scaleoffset");
  ...
  if (dtype_class == H5Z_SCALEOFFSET_CLS_INTEGER) {
    if (dtype_sign == H5Z_SCALEOFFSET_SGN_NONE) { /* unsigned integer */
      if (dtype_size == sizeof(unsigned char))
        ...
        HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, t_bad,
                    "cannot find matched memory datatype");
    }
...
H5Z__scaleoffset_set_parms_fillval(H5P_genplist_t *dcpl_plist, H5T_t *type,
  ...
  if (scale_type == t_uchar)
    ...
    H5Z_scaleoffset_set_filval_1(unsigned long long, dcpl_plist, type,
    ...
    H5Z_scaleoffset_set_filval_4(float, dcpl_plist, type, cd_values,
                                 need_convert);
  else if (scale_type == t_double)
    H5Z_scaleoffset_set_filval_4(double, dcpl_plist, type, cd_values,
                                 need_convert);
...
static herr_t H5Z__set_local_scaleoffset(hid_t dcpl_id, hid_t type_id,
  ...
  size_t cd_nelmts =
      H5Z_SCALEOFFSET_USER_NPARMS; /* Number of filter parameters */
  ...
  if (H5P_get_filter_by_id(dcpl_plist, H5Z_FILTER_SCALEOFFSET, &flags,
                           &cd_nelmts, cd_values, (size_t)0, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTGET, FAIL,
                "can't get scaleoffset parameters");
  ...
  if ((npoints = H5S_GET_EXTENT_NPOINTS(ds)) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTGET, FAIL,
                "unable to get number of points in the dataspace");

  /* Set "local" parameter for this dataset's number of elements */
  H5_CHECKED_ASSIGN(cd_values[H5Z_SCALEOFFSET_PARM_NELMTS], unsigned, npoints,
                    hssize_t);
  ...
  if (status == H5D_FILL_VALUE_UNDEFINED)
    ...
    if ((scale_type = H5Z__scaleoffset_get_type(
             cd_values[H5Z_SCALEOFFSET_PARM_CLASS],
             cd_values[H5Z_SCALEOFFSET_PARM_SIZE],
             cd_values[H5Z_SCALEOFFSET_PARM_SIGN])) == 0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "cannot use C integer datatype for cast");

    /* Get dataset fill value and store in cd_values[] */
    if (H5Z__scaleoffset_set_parms_fillval(dcpl_plist, type, scale_type,
                                           cd_values, need_convert) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTSET, FAIL, "unable to set fill value");
  } /* end else */

  /* Modify the filter's parameters for this dataset */
  if (H5P_modify_filter(dcpl_plist, H5Z_FILTER_SCALEOFFSET, flags,
                        (size_t)H5Z_SCALEOFFSET_TOTAL_NPARMS, cd_values) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTSET, FAIL,
                "can't set local scaleoffset parameters");
...
static size_t H5Z__filter_scaleoffset(unsigned flags, size_t cd_nelmts,
  ...
  enum H5Z_scaleoffset_t
      type;                 /* memory type corresponding to dataset datatype */
  int need_convert = FALSE; /* flag indicating conversion of byte order */
  unsigned char *outbuf = NULL; /* pointer to new output buffer */
  unsigned buf_offset =
      21;         /* buffer offset because of parameters stored in file */
  unsigned i;     /* index */
  ...
  if (cd_nelmts != H5Z_SCALEOFFSET_TOTAL_NPARMS)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, 0,
                "invalid scaleoffset number of parameters");
  ...
  case H5T_ORDER_NONE:
  default:
    HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, 0,
                "bad H5T_NATIVE_INT endianness order");
  } /* end switch */
   ...
   * scale type can be H5Z_SO_FLOAT_DSCALE (0), H5Z_SO_FLOAT_ESCALE (1) or
   * H5Z_SO_INT (other) H5Z_SO_FLOAT_DSCALE : floating-point type,
   * variable-minimum-bits method, scale factor is decimal scale factor
  ...
  if (scale_type ==
      H5Z_SO_FLOAT_DSCALE) { /* floating-point type, variable-minimum-bits */
    D_val = (double)scale_factor;
  } else { /* integer type, or floating-point type with fixed-minimum-bits
              method */
    if (scale_factor > (int)(cd_values[H5Z_SCALEOFFSET_PARM_SIZE] * 8))
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, 0,
                  "minimum number of bits exceeds maximum");
  ...
  if (flags & H5Z_FLAG_REVERSE) {
    ...
    if (minbits != 0)
      H5Z__scaleoffset_decompress(outbuf, d_nelmts,
                                  (unsigned char *)(*buf) + buf_offset, p);
    else {
    ...
    if ((type = H5Z__scaleoffset_get_type(dtype_class, p.size, dtype_sign)) ==
        0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, 0,
                  "cannot use C integer datatype for cast");
  ...
  /* output; compress */
    ...
    if (dtype_class == H5Z_SCALEOFFSET_CLS_INTEGER)
      H5Z__scaleoffset_precompress_i(*buf, d_nelmts, type, filavail, cd_values,
                                     &minbits, &minval);
     ...
     */
    p.minbits = minbits;
    size_out = buf_offset + nbytes * p.minbits / (p.size * 8) +
               1; /* may be 1 larger */

    /* allocate memory space for compressed buffer */
    if (NULL == (outbuf = (unsigned char *)H5MM_malloc(size_out)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, 0,
                  "memory allocation failed for scaleoffset compression");
    ...
    for (i = 0; i < 4; i++)
      ((unsigned char *)outbuf)[i] =
          (unsigned char)((minbits & ((uint32_t)0xff << i * 8)) >> i * 8);

    ((unsigned char *)outbuf)[4] = sizeof(unsigned long long);

    for (i = 0; i < sizeof(unsigned long long); i++)
      ((unsigned char *)outbuf)[5 + i] =
          (unsigned char)((minval & ((unsigned long long)0xff << i * 8)) >>
                          i * 8);
     ...
     */
    if (minbits != 0)
      H5Z__scaleoffset_compress((unsigned char *)*buf, d_nelmts,
                                outbuf + buf_offset, size_out - buf_offset, p);
  }
...
static void H5Z__scaleoffset_precompress_i(void *data, unsigned d_nelmts,
                                           ...
                                           unsigned long long *minval) {
  if (type == t_uchar)
    H5Z_scaleoffset_precompress_1(unsigned char, data, d_nelmts, filavail,
                                  cd_values, minbits, minval);
  else if (type == t_ushort)
    H5Z_scaleoffset_precompress_1(unsigned short, data, d_nelmts, filavail,
                                  cd_values, minbits, minval);
  else if (type == t_uint)
    H5Z_scaleoffset_precompress_1(unsigned int, data, d_nelmts, filavail,
                                  cd_values, minbits, minval);
  else if (type == t_ulong)
    H5Z_scaleoffset_precompress_1(unsigned long, data, d_nelmts, filavail,
                                  cd_values, minbits, minval);
  else if (type == t_ulong_long)
    H5Z_scaleoffset_precompress_1(unsigned long long, data, d_nelmts, filavail,
                                  cd_values, minbits, minval);
        ...
        *minbits = H5Z__scaleoffset_log2((unsigned long long)(span + 1));
        ...
        H5Z_scaleoffset_min_1(
            ...
            for (i = 0; i < d_nelmts; i++) buf[i] =
                (signed char)((buf[i] == filval)
                                  ? (((unsigned char)1 << *minbits) - 1)
                                  : (buf[i] - min));
    } else { /* fill value undefined */
      if (*minbits ==
          H5Z_SO_INT_MINBITS_DEFAULT) { /* minbits not set yet, calculate max,
    ...
    *minval = (unsigned long long)min;
    ...
    H5Z_scaleoffset_precompress_2(short, data, d_nelmts, filavail, cd_values,
                                  minbits, minval);
  else if (type == t_int)
    H5Z_scaleoffset_precompress_2(int, data, d_nelmts, filavail, cd_values,
                                  minbits, minval);
  else if (type == t_long)
    H5Z_scaleoffset_precompress_2(long, data, d_nelmts, filavail, cd_values,
                                  minbits, minval);
  else if (type == t_long_long)
    H5Z_scaleoffset_precompress_2(long long, data, d_nelmts, filavail,
                                  cd_values, minbits, minval);
}
...
static void H5Z__scaleoffset_postdecompress_i(void *data, unsigned d_nelmts,
  ...
  if (type == t_uchar)
    ...
    H5Z_scaleoffset_postdecompress_1(unsigned long long, data, d_nelmts,
                                     filavail, cd_values, minbits, minval);
  else if (type == t_schar) {
    ...
    if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */
      H5Z_scaleoffset_get_filval_1(signed char, cd_values, filval);
      for (i = 0; i < d_nelmts; i++)
        buf[i] = (signed char)((buf[i] == (((unsigned char)1 << minbits) - 1))
                                   ? filval
                                   : (buf[i] + sminval));
    } else /* fill value undefined */
      for (i = 0; i < d_nelmts; i++)
    ...
    H5Z_scaleoffset_postdecompress_2(short, data, d_nelmts, filavail, cd_values,
                                     minbits, sminval);
  else if (type == t_int)
    H5Z_scaleoffset_postdecompress_2(int, data, d_nelmts, filavail, cd_values,
                                     minbits, sminval);
  else if (type == t_long)
    H5Z_scaleoffset_postdecompress_2(long, data, d_nelmts, filavail, cd_values,
                                     minbits, sminval);
  else if (type == t_long_long)
...
H5Z__scaleoffset_precompress_fd(void *data, unsigned d_nelmts,
  ...
  if (type == t_float)
    H5Z_scaleoffset_precompress_3(float, HDpowf, HDfabsf, HDroundf, HDlroundf,
                                  HDllroundf, data, d_nelmts, filavail,
                                  cd_values, minbits, minval, D_val);
  else if (type == t_double)
    H5Z_scaleoffset_precompress_3(double, HDpow, HDfabs, HDround, HDlround,
                                  HDllround, data, d_nelmts, filavail,
                                  cd_values, minbits, minval, D_val);
...
static void H5Z__scaleoffset_decompress_one_byte(
    ...
    const unsigned char *buffer, size_t *j, unsigned *buf_len, parms_atomic p,
    unsigned dtype_len) {
  unsigned dat_len;  /* dat_len is the number of bits to be copied in each data
                        byte */
...
static void H5Z__scaleoffset_decompress_one_atomic(unsigned char *data,
  ...
  if (p.mem_order == H5Z_SCALEOFFSET_ORDER_LE) { /* little endian */
    ...
    for (k = (int)begin_i; k >= 0; k--)
      H5Z__scaleoffset_decompress_one_byte(data, data_offset, (unsigned)k,
                                           begin_i, buffer, j, buf_len, p,
                                           dtype_len);
  } else { /* big endian */
...
static void H5Z__scaleoffset_decompress(unsigned char *data, unsigned d_nelmts,
  ...
  /* decompress */
  for (i = 0; i < d_nelmts; i++)
    H5Z__scaleoffset_decompress_one_atomic(data, i * p.size, buffer, &j,
                                           &buf_len, p);
}
...
static void H5Z__scaleoffset_compress_one_byte(
  ...
  if (*buf_len > dat_len) {
    buffer[*j] |= (unsigned char)((val & ~((unsigned)(~0) << dat_len))
                                  << (*buf_len - dat_len));
    *buf_len -= dat_len;
  } else {
    buffer[*j] |= (unsigned char)((unsigned)(val >> (dat_len - *buf_len)) &
                                  ~((unsigned)(~0) << *buf_len));
    dat_len -= *buf_len;
    H5Z__scaleoffset_next_byte(j, buf_len);
    if (dat_len == 0)
...
static void H5Z__scaleoffset_compress_one_atomic(unsigned char *data,
  ...
  if (p.mem_order == H5Z_SCALEOFFSET_ORDER_LE) { /* little endian */
    ...
    for (k = (int)begin_i; k >= 0; k--)
      H5Z__scaleoffset_compress_one_byte(data, data_offset, (unsigned)k,
                                         begin_i, buffer, j, buf_len, p,
                                         dtype_len);
  } else { /* big endian */
    assert(p.mem_order == H5Z_SCALEOFFSET_ORDER_BE);
...
static void H5Z__scaleoffset_compress(unsigned char *data, unsigned d_nelmts,
  ...
  /* compress */
  for (i = 0; i < d_nelmts; i++)
    H5Z__scaleoffset_compress_one_atomic(data, i * p.size, buffer, &j, &buf_len,
                                         p);
}
