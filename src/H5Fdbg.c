...
herr_t H5F_debug(H5F_t *f, FILE *stream, int indent, int fwidth) {
  ...
  fprintf(stream, "%*s%-*s 0x%08x\n", indent, "", fwidth, "File access flags",
          f->shared->flags);
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Root group symbol table entry version number:",
          (unsigned)HDF5_OBJECTDIR_VERSION);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          ...
          f->shared->sblock->btree_k[H5B_SNODE_ID]);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Indexed storage internal node 1/2 rank:",
          f->shared->sblock->btree_k[H5B_CHUNK_ID]);
  ...
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Shared object header message version number:",
          (unsigned)f->shared->sohm_vers);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
          "Number of shared object header message indexes:",
          (unsigned)f->shared->sohm_nindexes);

  fprintf(
      stream, "%*s%-*s %" PRIuHADDR " (rel)\n", indent, "", fwidth,
  ...
  if (f->shared->root_grp) {
    if (f->shared->sblock
            ->root_ent) /* Use real root group symbol table entry */
