...
static int H5D__farray_idx_iterate_cb(hsize_t idx, const void *_elmt,
                                      void *_udata);
static int H5D__farray_idx_delete_cb(const H5D_chunk_rec_t *chunk_rec,
                                     void *_udata);
...
static herr_t H5D__farray_filt_encode(void *raw, const void *elmt,
                                      size_t nelmts, void *ctx);
static herr_t H5D__farray_filt_decode(const void *raw, void *elmt,
                                      size_t nelmts, void *ctx);
...
static herr_t H5D__farray_idx_size(const H5D_chk_idx_info_t *idx_info,
                                   hsize_t *size);
static herr_t H5D__farray_idx_reset(H5O_storage_chunk_t *storage,
                                    hbool_t reset_addr);
static herr_t H5D__farray_idx_dump(const H5O_storage_chunk_t *storage,
                                   FILE *stream);
...
static void *H5D__farray_crt_context(void *_udata) {
  ...
  H5D_farray_ctx_ud_t *udata =
      (H5D_farray_ctx_ud_t *)_udata; /* User data for fixed array context */
  ...
  if (NULL == (ctx = H5FL_MALLOC(H5D_farray_ctx_t)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, NULL,
                "can't allocate fixed array client callback context");
...
static herr_t H5D__farray_dst_context(void *_ctx) {
  H5D_farray_ctx_t *ctx =
      (H5D_farray_ctx_t *)_ctx; /* Fixed array callback context */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__farray_encode(void *raw, const void *_elmt, size_t nelmts,
                                 void *_ctx) {
  H5D_farray_ctx_t *ctx =
      (H5D_farray_ctx_t *)_ctx; /* Fixed array callback context */
  const haddr_t *elmt =
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__farray_decode(const void *_raw, void *_elmt, size_t nelmts,
                                 void *_ctx) {
  H5D_farray_ctx_t *ctx =
      (H5D_farray_ctx_t *)_ctx;     /* Fixed array callback context */
...
static herr_t H5D__farray_dst_dbg_context(void *_dbg_ctx) {
  H5D_farray_ctx_ud_t *dbg_ctx =
      (H5D_farray_ctx_ud_t *)_dbg_ctx; /* Context for fixed array callback */
 ...
 */
static herr_t H5D__farray_filt_fill(void *nat_blk, size_t nelmts) {
  H5D_farray_filt_elmt_t fill_val =
      H5D_FARRAY_FILT_FILL; /* Value to fill elements with */
  ...
  H5VM_array_fill(nat_blk, &fill_val, H5FA_CLS_FILT_CHUNK->nat_elmt_size,
                  nelmts);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__farray_filt_encode(void *_raw, const void *_elmt,
                                      size_t nelmts, void *_ctx) {
  H5D_farray_ctx_t *ctx =
      (H5D_farray_ctx_t *)_ctx;   /* Fixed array callback context */
  uint8_t *raw = (uint8_t *)_raw; /* Convenience pointer to raw elements */
  const H5D_farray_filt_elmt_t *elmt = (const H5D_farray_filt_elmt_t *)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__farray_filt_decode(const void *_raw, void *_elmt,
                                      size_t nelmts, void *_ctx) {
  H5D_farray_ctx_t *ctx =
      (H5D_farray_ctx_t *)_ctx; /* Fixed array callback context */
  H5D_farray_filt_elmt_t *elmt = (H5D_farray_filt_elmt_t *)
      _elmt; /* Convenience pointer to native elements */
  const uint8_t *raw =
      (const uint8_t *)_raw; /* Convenience pointer to raw elements */
...
static herr_t H5D__farray_idx_depend(const H5D_chk_idx_info_t *idx_info) {
  ...
  /* Make the fixed array a child flush dependency of the dataset's object
   * header proxy */
...
static herr_t H5D__farray_idx_open(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (NULL == (idx_info->storage->u.farray.fa =
                   H5FA_open(idx_info->f, idx_info->storage->idx_addr, &udata)))
...
static herr_t H5D__farray_idx_create(const H5D_chk_idx_info_t *idx_info) {
  ...
  cparam.max_dblk_page_nelmts_bits =
      idx_info->layout->u.farray.cparam.max_dblk_page_nelmts_bits;
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__farray_idx_iterate_cb(hsize_t H5_ATTR_UNUSED idx,
  ...
  if (udata->filtered) {
    const H5D_farray_filt_elmt_t *filt_elmt =
...
static int H5D__farray_idx_iterate(const H5D_chk_idx_info_t *idx_info,
  ...
  if (H5FA_get_stats(fa, &fa_stat) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't query fixed array statistics");
  ...
  if (NULL == idx_info->storage->u.farray.fa) {
    ...
    if (H5FA_patch_file(idx_info->storage->u.farray.fa, idx_info->f) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL,
                  "can't patch fixed array file pointer");
...
static herr_t H5D__farray_idx_delete(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (H5_addr_defined(idx_info->storage->idx_addr)) {
     ...
     */
    if (H5D__farray_idx_iterate(idx_info, H5D__farray_idx_delete_cb,
                                idx_info->f) < 0)
    ...
    if (H5FA_delete(idx_info->f, idx_info->storage->idx_addr, &ctx_udata) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTDELETE, FAIL,
                  "unable to delete chunk fixed array");
