...
#define HDFS_STATS_POW(bin_i, out_ptr)                                         \
  {                                                                            \
    ...
    for (donotshadowindex = 0;                                                 \
         donotshadowindex <                                                    \
         (((bin_i)*HDFS_STATS_INTERVAL) + HDFS_STATS_START_POWER);             \
         donotshadowindex++) {                                                 \
 ...
 *     `H5FD__hdfs_handle_open()` and cleaned up via
...
static hdfs_t *H5FD__hdfs_handle_open(const char *path,
  ...
  handle->file = hdfsOpenFile(handle->filesystem, path, O_RDONLY,
                              stream_buffer_size, 0, 0);
  if (!handle->file)
 ...
 *-------------------------------------------------------------------------
 */
static H5FD_t *H5FD__hdfs_open(const char *path, unsigned flags, hid_t fapl_id,
                               haddr_t maxaddr) {
  ...
  handle = H5FD__hdfs_handle_open(path, fa.namenode_name, fa.namenode_port,
                                  fa.user_name, fa.kerberos_ticket_cache,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__hdfs_query(const H5FD_t H5_ATTR_UNUSED *_file,
                               unsigned long *flags) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__hdfs_get_eoa(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__hdfs_set_eoa(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
                                 haddr_t addr) {
  H5FD_hdfs_t *file = (H5FD_hdfs_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__hdfs_get_eof(const H5FD_t *_file,
                                  H5FD_mem_t H5_ATTR_UNUSED type) {
  const H5FD_hdfs_t *file = (const H5FD_hdfs_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__hdfs_get_handle(H5FD_t *_file, hid_t H5_ATTR_UNUSED fapl,
                                    void **file_handle) {
  H5FD_hdfs_t *file = (H5FD_hdfs_t *)_file;
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__hdfs_read(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__hdfs_truncate(H5FD_t H5_ATTR_UNUSED *_file,
