...
/* Define a code template for comparing scalar keys for the "CMP" in the
 * H5SL_LOCATE macro */
#define H5SL_LOCATE_SCALAR_CMP(SLIST, TYPE, PNODE, PKEY, HASHVAL)              \
  (*(TYPE *)((PNODE)->key) < *(TYPE *)PKEY)

/* Define a code template for comparing string keys for the "CMP" in the
 * H5SL_LOCATE macro */
...
/* Define a code template for comparing H5_obj_t keys for the "CMP" in the
 * H5SL_LOCATE macro */
...
/* Define a code template for comparing generic keys for the "CMP" in the
 * H5SL_LOCATE macro */
#define H5SL_LOCATE_GENERIC_CMP(SLIST, TYPE, PNODE, PKEY, HASHVAL)             \
  ((SLIST)->cmp((TYPE *)((PNODE)->key), (TYPE *)PKEY) < 0)

/* Define a code template for comparing scalar keys for the "EQ" in the
 * H5SL_LOCATE macro */
#define H5SL_LOCATE_SCALAR_EQ(SLIST, TYPE, PNODE, PKEY, HASHVAL)               \
  (*(TYPE *)((PNODE)->key) == *(TYPE *)PKEY)

/* Define a code template for comparing string keys for the "EQ" in the
 * H5SL_LOCATE macro */
#define H5SL_LOCATE_STRING_EQ(SLIST, TYPE, PNODE, PKEY, HASHVAL)               \
  (((PNODE)->hashval == HASHVAL) &&                                            \
   (HDstrcmp((const char *)(PNODE)->key, (const char *)PKEY) == 0))

/* Define a code template for comparing H5_obj_t keys for the "EQ" in the
 * H5SL_LOCATE macro */
#define H5SL_LOCATE_OBJ_EQ(SLIST, TYPE, PNODE, PKEY, HASHVAL)                  \
  ((((TYPE *)((PNODE)->key))->fileno == ((TYPE *)PKEY)->fileno) &&             \
   (((TYPE *)((PNODE)->key))->addr == ((TYPE *)PKEY)->addr))

/* Define a code template for comparing generic keys for the "EQ" in the
 * H5SL_LOCATE macro */
#define H5SL_LOCATE_GENERIC_EQ(SLIST, TYPE, PNODE, PKEY, HASHVAL)              \
  ((SLIST)->cmp((TYPE *)((PNODE)->key), (TYPE *)PKEY) == 0)

/* Define a code template for initializing the hash value for scalar keys for
 * the "HASHINIT" in the H5SL_LOCATE macro */
#define H5SL_LOCATE_SCALAR_HASHINIT(KEY, HASHVAL)

/* Define a code template for initializing the hash value for string keys for
 * the "HASHINIT" in the H5SL_LOCATE macro */
#define H5SL_LOCATE_STRING_HASHINIT(KEY, HASHVAL)                              \
  HASHVAL = H5_hash_string((const char *)KEY);

/* Define a code template for initializing the hash value for H5_obj_t keys for
 * the "HASHINIT" in the H5SL_LOCATE macro */
#define H5SL_LOCATE_OBJ_HASHINIT(KEY, HASHVAL)

/* Define a code template for initializing the hash value for generic keys for
 * the "HASHINIT" in the H5SL_LOCATE macro */
  ...
  {                                                                            \
    ...
    (KEY, HASHVAL) for (_i = (int)SLIST->curr_level; _i >= 0; _i--) {          \
      ...
      while (_count < 3 && X->forward[_i] &&                                   \
             H5_GLUE3(H5SL_LOCATE_, CMP, _CMP)(SLIST, TYPE, X->forward[_i],    \
                                               KEY, HASHVAL)) {                \
        X = X->forward[_i];                                                    \
        _count++;                                                              \
      }                                                                        \
    ...
    X = X->forward[0];                                                         \
    if (X != NULL &&                                                           \
        H5_GLUE3(H5SL_LOCATE_, CMP, _EQ)(SLIST, TYPE, X, KEY, HASHVAL)) {      \
      ...
      if (X->log_nalloc >= H5SL_fac_nused_g) {                                 \
        ...
        if (H5SL_fac_nused_g >= H5SL_fac_nalloc_g) {                           \
          ...
          if (NULL == (H5SL_fac_g = (H5FL_fac_head_t **)H5MM_realloc(          \
            ...
            HGOTO_ERROR(H5E_SLIST, H5E_CANTALLOC, ERR,                         \
                        "memory allocation failed");                           \
        }                                                                      \
                                                                               \
        /* Create the new factory */                                           \
        H5SL_fac_g[H5SL_fac_nused_g] = H5FL_fac_init(                          \
            (((size_t)1) << H5SL_fac_nused_g) * sizeof(H5SL_node_t *));        \
        H5SL_fac_nused_g++;                                                    \
      }                                                                        \
                                                                               \
      /* Allocate space for new forward pointers */                            \
      if (NULL ==                                                              \
          (_tmp = (H5SL_node_t **)H5FL_FAC_MALLOC(H5SL_fac_g[X->log_nalloc]))) \
        HGOTO_ERROR(H5E_SLIST, H5E_CANTALLOC, ERR,                             \
                    "memory allocation failed");                               \
      H5MM_memcpy((void *)_tmp, (const void *)X->forward,                      \
                  (LVL + 1) * sizeof(H5SL_node_t *));                          \
      X->forward = (H5SL_node_t **)H5FL_FAC_FREE(                              \
          H5SL_fac_g[X->log_nalloc - 1], (void *)X->forward);                  \
      X->forward = _tmp;                                                       \
    }                                                                          \
                                                                               \
    X->level++;                                                                \
    ...
    (KEY, HASHVAL) for (_i = (int)SLIST->curr_level; _i >= 0; _i--) {          \
      ...
      for (_count = 0;; _count++) {                                            \
        ...
        if (!_drop && !H5_GLUE3(H5SL_LOCATE_, CMP, _CMP)(                      \
                          SLIST, TYPE, X->forward[_i], KEY, HASHVAL))          \
          _drop = X;                                                           \
                                                                               \
      ...
      assert(!_drop->forward[_i] ||                                            \
             !H5_GLUE3(H5SL_LOCATE_, CMP,                                      \
                       _CMP)(SLIST, TYPE, _drop->forward[_i], KEY, HASHVAL));  \
                                                                               \
      ...
      _next = _drop->forward[_i];                                              \
                                                                               ...
                                                                               \
    if (_next &&                                                               \
        H5_GLUE3(H5SL_LOCATE_, CMP, _EQ)(SLIST, TYPE, _next, KEY, HASHVAL))    \
      HGOTO_ERROR(H5E_SLIST, H5E_CANTINSERT, NULL,                             \
...
#define H5SL_REMOVE(CMP, SLIST, X, TYPE, KEY, HASHVAL)                         \
  {                                                                            \
    ...
    H5SL_node_t *_ldrop =                                                      \
        NULL; /* Low node of gap before the one to drop into */                \
    ...
    (KEY, HASHVAL)                                                             \
        ...
        while (X && (!X->key || H5_GLUE3(H5SL_LOCATE_, CMP, _CMP)(             \
                                    SLIST, TYPE, X, KEY, HASHVAL))) {          \
      _llast = _last;                                                          \
      _last = X;                                                               \
      X = X->forward[_i];                                                      \
    }                                                                          \
    ...
    for (_i--; _i >= 0; _i--) {                                                \
      ...
      for (_count = 0;; _count++) {                                            \
        ...
        if (_drop) {                                                           \
          ...
          /* Check if this node is the start of the next gap */                \
          if (!H5_GLUE3(H5SL_LOCATE_, CMP, _CMP)(SLIST, TYPE, X->forward[_i],  \
                                                 KEY, HASHVAL)) {              \
            _drop = X;                                                         \
...
/* Static functions */
static H5SL_node_t *H5SL__new_node(void *item, const void *key,
                                   uint32_t hashval);
static H5SL_node_t *H5SL__insert_common(H5SL_t *slist, void *item,
                                        const void *key);
static herr_t H5SL__release_common(H5SL_t *slist, H5SL_operator_t op,
                                   void *op_data);
static herr_t H5SL__close_common(H5SL_t *slist, H5SL_operator_t op,
                                 void *op_data);
...
static H5SL_node_t *H5SL__new_node(void *item, const void *key,
  ...
  if (NULL ==
      (ret_value->forward = (H5SL_node_t **)H5FL_FAC_MALLOC(H5SL_fac_g[0]))) {
...
static H5SL_node_t *H5SL__insert_common(H5SL_t *slist, void *item,
  ...
  if (NULL == (x = H5SL__new_node(item, key, hashval)))
    HGOTO_ERROR(H5E_SLIST, H5E_NOSPACE, NULL,
...
static herr_t H5SL__release_common(H5SL_t *slist, H5SL_operator_t op,
  ...
  while (node) {
    ...
    node->forward = (H5SL_node_t **)H5FL_FAC_FREE(H5SL_fac_g[node->log_nalloc],
                                                  node->forward);
  ...
  slist->header->forward = (H5SL_node_t **)H5FL_FAC_FREE(
      H5SL_fac_g[slist->header->log_nalloc], slist->header->forward);
  if (NULL ==
      (slist->header->forward = (H5SL_node_t **)H5FL_FAC_MALLOC(H5SL_fac_g[0])))
...
/*--------------------------------------------------------------------------
 ...
 RETURNS
    Returns pointer to item who key is greater than or equal to 'key' on
success, NULL on failure DESCRIPTION Search for an object in a skip list,
according to it's key, returning the object itself (for an exact match), or the
object with the next lowest key that is greater than 'key' GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 NAME
    H5SL_above
 PURPOSE
    Search for _node_ in a skip list whose object is greater than or equal to
'key' USAGE H5SL_node_t *H5SL_above(slist, key) H5SL_t *slist;          IN/OUT:
Pointer to skip list void *key;              IN: Key for item to search for
...
/*--------------------------------------------------------------------------
 ...
 RETURNS
    Returns pointer to node after slist_node in skip list on success, NULL on
failure. DESCRIPTION Retrieves a pointer to the next node in a skip list, for
iterating over the list. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 ...
 RETURNS
    Returns pointer to node before slist_node in skip list on success, NULL on
failure. DESCRIPTION Retrieves a pointer to the previous node in a skip list,
for iterating over the list. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
H5SL_node_t *H5SL_prev(H5SL_node_t *slist_node) {
   ...
   */
  FUNC_LEAVE_NOAPI(slist_node->backward->key == NULL ? NULL
                                                     : slist_node->backward)
} /* end H5SL_prev() */
