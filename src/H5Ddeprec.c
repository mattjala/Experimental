 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Dcreate1(hid_t loc_id, const char *name, hid_t type_id, hid_t space_id,
                 hid_t dcpl_id) {
  ...
  if (NULL == (dset = H5VL_dataset_create(
                   vol_obj, &loc_params, name, H5P_LINK_CREATE_DEFAULT, type_id,
...
hid_t H5Dopen1(hid_t loc_id, const char *name) {
  ...
  /* Open the dataset */
  if (NULL == (dset = H5VL_dataset_open(
                   vol_obj, &loc_params, name, H5P_DATASET_ACCESS_DEFAULT,
                   H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)))
...
herr_t H5Dextend(hid_t dset_id, const hsize_t size[]) {
  ...
  if (H5VL_dataset_get(vol_obj, &vol_get_cb_args, H5P_DATASET_XFER_DEFAULT,
                       H5_REQUEST_NULL) < 0)
  ...
  if (H5I_INVALID_HID == sid)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "received an invalid dataspace from the dataset");
  if (NULL == (ds = (H5S_t *)H5I_object_verify(sid, H5I_DATASPACE)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "couldn't get dataspace structure from ID");
  ...
  /* Increase size */
  if (H5VL_dataset_specific(vol_obj, &vol_spec_cb_args,
