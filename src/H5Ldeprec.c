 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5L__iterate2_shim(hid_t group_id, const char *name,
                                 const H5L_info2_t *linfo2, void *op_data) {
  ...
  if (linfo2) {
    ...
    if (H5L_TYPE_HARD == linfo2->type) {
      if (H5VLnative_token_to_addr(group_id, linfo2->u.token,
                                   &linfo.u.address) < 0)
  ...
  ret_value =
      shim_data->real_op(group_id, name, &linfo, shim_data->real_op_data);
...
herr_t H5Literate_by_name1(hid_t loc_id, const char *group_name,
  ...
  if (!is_native_vol_obj)
    HGOTO_ERROR(H5E_LINK, H5E_BADVALUE, FAIL,
                "H5Literate_by_name1 is only meant to be used with the native "
                "VOL connector");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Lget_info1(hid_t loc_id, const char *name, H5L_info1_t *linfo /*out*/,
                    hid_t lapl_id) {
  H5VL_object_t *vol_obj = NULL;    /* object of loc_id */
  ...
  if (linfo) {
    ...
    if (H5L_TYPE_HARD == linfo2.type) {
      ...
      if (H5VL_native_token_to_addr(vol_obj_data, loc_params.obj_type,
                                    linfo2.u.token, &linfo->u.address) < 0)
...
herr_t H5Lget_info_by_idx1(hid_t loc_id, const char *group_name,
  ...
  if (!is_native_vol_obj)
    HGOTO_ERROR(H5E_LINK, H5E_BADVALUE, FAIL,
                "H5Lget_info_by_idx1 is only meant to be used with the native "
                "VOL connector");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Lvisit1(hid_t group_id, H5_index_t idx_type, H5_iter_order_t order,
...
herr_t H5Lvisit_by_name1(hid_t loc_id, const char *group_name,
  ...
  if (!is_native_vol_obj)
    HGOTO_ERROR(H5E_LINK, H5E_BADVALUE, FAIL,
                "H5Lvisit_by_name1 is only meant to be used with the native "
                "VOL connector");
