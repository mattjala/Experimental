...
/* Local Prototypes */
/********************/
static herr_t H5C__autoadjust__ageout(H5F_t *f, double hit_rate,
...
static herr_t H5C__flush_invalidate_ring(H5F_t *f, H5C_ring_t ring,
                                         unsigned flags);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__auto_adjust_cache_size(H5F_t *f, hbool_t write_permitted) {
  ...
  if (cache_ptr->resize_ctl.rpt_fcn != NULL)
    (cache_ptr->resize_ctl.rpt_fcn)(
        cache_ptr, H5C__CURR_AUTO_RESIZE_RPT_FCN_VER, hit_rate, status,
...
static herr_t H5C__autoadjust__ageout(H5F_t *f, double hit_rate,
  ...
  if ((cache_ptr->resize_ctl.decr_mode == H5C_decr__age_out) ||
    ...
    if (cache_ptr->max_cache_size > cache_ptr->resize_ctl.min_size) {
      ...
      if (H5C__autoadjust__ageout__evict_aged_out_entries(f, write_permitted) <
          0)
        HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
...
herr_t H5C__autoadjust__ageout__remove_all_markers(H5C_t *cache_ptr) {
  ...
  while (cache_ptr->epoch_markers_active > 0) {
    ...
    assert(cache_ptr->epoch_markers_active ==
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__flash_increase_cache_size(H5C_t *cache_ptr, size_t old_entry_size,
  ...
  if (((cache_ptr->index_size + space_needed) > cache_ptr->max_cache_size) &&
    ...
    case H5C_flash_incr__add_space:
      if (cache_ptr->index_size < cache_ptr->max_cache_size) {
        assert((cache_ptr->max_cache_size - cache_ptr->index_size) <
               space_needed);
        space_needed -= cache_ptr->max_cache_size - cache_ptr->index_size;
      }
      space_needed = (size_t)(((double)space_needed) *
...
static herr_t H5C__flush_invalidate_ring(H5F_t *f, H5C_ring_t ring,
  ...
  while (cache_ptr->index_ring_len[ring] > 0) {
    ...
    if (node_ptr == NULL) {
      assert(cache_ptr->slist_len == (uint32_t)((int32_t)initial_slist_len +
                                                cache_ptr->slist_len_increase));
      assert(cache_ptr->slist_size == (size_t)((ssize_t)initial_slist_size +
    ...
    if ((cur_ring_pel_len > 0) && (cur_ring_pel_len >= old_ring_pel_len)) {
      ...
      HGOTO_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                  "Pinned entry count not decreasing, cur_ring_pel_len = %d, "
                  "old_ring_pel_len = %d, ring = %d",
...
herr_t H5C__flush_ring(H5F_t *f, H5C_ring_t ring, unsigned flags) {
  ...
  while ((cache_ptr->slist_ring_len[ring] > 0) && (protected_entries == 0) &&
    ...
    assert((uint32_t)((int32_t)initial_slist_len +
                      cache_ptr->slist_len_increase) == cache_ptr->slist_len);
    assert((size_t)((ssize_t)initial_slist_size +
...
herr_t H5C__make_space_in_cache(H5F_t *f, size_t space_needed,
  ...
  if (write_permitted) {
    ...
    while (
        (((cache_ptr->index_size + space_needed) > cache_ptr->max_cache_size) ||
    ...
    assert((entries_examined > (2 * initial_list_len)) ||
           ((cache_ptr->pl_size + cache_ptr->pel_size +
             cache_ptr->min_clean_size) > cache_ptr->max_cache_size) ||
