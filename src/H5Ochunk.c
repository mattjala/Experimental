 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__chunk_add(H5F_t *f, H5O_t *oh, unsigned idx,
                      unsigned cont_chunkno) {
  H5O_chunk_proxy_t *chk_proxy =
      NULL; /* Proxy for chunk, to mark it dirty in the cache */
  H5O_chunk_proxy_t *cont_chk_proxy =
      NULL; /* Proxy for chunk containing continuation message that points to
               this chunk, if not chunk 0 */
  ...
  if (H5AC_insert_entry(f, H5AC_OHDR_CHK, oh->chunk[idx].addr, chk_proxy,
                        H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTINSERT, FAIL,
                "unable to cache object header chunk");
...
H5O_chunk_proxy_t *H5O__chunk_protect(H5F_t *f, H5O_t *oh, unsigned idx) {
  ...
  if (0 == idx) {
    ...
    if (NULL == (chk_proxy = (H5O_chunk_proxy_t *)H5AC_protect(
                     f, H5AC_OHDR_CHK, oh->chunk[idx].addr, &chk_udata,
                     H5AC__NO_FLAGS_SET)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__chunk_unprotect(H5F_t *f, H5O_chunk_proxy_t *chk_proxy,
                            hbool_t dirtied) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (0 == chk_proxy->chunkno) {
    ...
    if (H5O__dec_rc(chk_proxy->oh) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTDEC, FAIL,
                  "can't decrement reference count on object header");
    ...
    if (H5AC_unprotect(f, H5AC_OHDR_CHK,
                       chk_proxy->oh->chunk[chk_proxy->chunkno].addr, chk_proxy,
  ...
  if (0 == chk_proxy->chunkno) {
    ...
    if (H5AC_resize_entry(oh, oh->chunk[0].size) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTRESIZE, FAIL,
                  "unable to resize chunk in cache");
  } /* end if */
  else {
...
herr_t H5O__chunk_update_idx(H5F_t *f, H5O_t *oh, unsigned idx) {
  ...
  if (H5AC_unprotect(f, H5AC_OHDR_CHK, oh->chunk[idx].addr, chk_proxy,
...
herr_t H5O__chunk_delete(H5F_t *f, H5O_t *oh, unsigned idx) {
  ...
  /* Release the chunk proxy from the cache, marking it deleted */
  if (chk_proxy && H5AC_unprotect(f, H5AC_OHDR_CHK, oh->chunk[idx].addr,
                                  chk_proxy, cache_flags) < 0)
    HDONE_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, FAIL,
                "unable to release object header chunk");
