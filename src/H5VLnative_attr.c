 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_attr_create(void *obj, const H5VL_loc_params_t *loc_params,
  ...
  if (NULL == (plist = H5P_object_verify(aapl_id, H5P_ATTRIBUTE_ACCESS)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                "AAPL is not an attribute access property list");
  ...
  if (loc_params->type == H5VL_OBJECT_BY_SELF) {
    ...
    /* H5Acreate_by_name */
    if (NULL ==
        (attr = H5A__create_by_name(&loc, loc_params->loc_data.loc_by_name.name,
                                    attr_name, type, space, acpl_id)))
      HGOTO_ERROR(H5E_ATTR, H5E_CANTINIT, NULL, "unable to create attribute");
  } /* end else-if */
  else
    HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, NULL,
                "unknown attribute create parameters");
...
void *H5VL__native_attr_open(void *obj, const H5VL_loc_params_t *loc_params,
  ...
  if (loc_params->type == H5VL_OBJECT_BY_SELF) {
    ...
    if (NULL == (attr = H5A__open_by_name(
                     &loc, loc_params->loc_data.loc_by_name.name, attr_name)))
    ...
    if (NULL ==
        (attr = H5A__open_by_idx(&loc, loc_params->loc_data.loc_by_idx.name,
    ...
    HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, NULL,
                "unknown attribute open parameters");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_read(void *attr, hid_t dtype_id, void *buf,
                              hid_t H5_ATTR_UNUSED dxpl_id,
                              void H5_ATTR_UNUSED **req) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_write(void *attr, hid_t dtype_id, const void *buf,
                               hid_t H5_ATTR_UNUSED dxpl_id,
                               void H5_ATTR_UNUSED **req) {
  H5T_t *mem_type;  /* Memory datatype */
  herr_t ret_value; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_get(void *obj, H5VL_attr_get_args_t *args,
                             hid_t H5_ATTR_UNUSED dxpl_id,
                             void H5_ATTR_UNUSED **req) {
  ...
  case H5VL_ATTR_GET_SPACE: {
    ...
    if ((args->args.get_space.space_id = H5A_get_space(attr)) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_CANTGET, FAIL,
                  "can't get space ID of attribute");
    break;
  }
  ...
  case H5VL_ATTR_GET_TYPE: {
    ...
    if ((args->args.get_type.type_id = H5A__get_type(attr)) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_CANTGET, FAIL,
                  "can't get datatype ID of attribute");
    break;
  }
  ...
  case H5VL_ATTR_GET_ACPL: {
    ...
    if ((args->args.get_acpl.acpl_id = H5A__get_create_plist(attr)) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_CANTGET, FAIL,
                  "can't get creation property list for attr");
  ...
  case H5VL_ATTR_GET_NAME: {
    ...
    if (H5VL_OBJECT_BY_SELF == get_name_args->loc_params.type) {
      if (H5A__get_name((H5A_t *)obj, get_name_args->buf_size,
                        get_name_args->buf, get_name_args->attr_name_len) < 0)
      ...
      if (NULL == (attr = H5A__open_by_idx(
                       &loc, get_name_args->loc_params.loc_data.loc_by_idx.name,
      ...
      *get_name_args->attr_name_len = HDstrlen(attr->shared->name);
      ...
      if (get_name_args->buf) {
        HDstrncpy(
            get_name_args->buf, attr->shared->name,
  ...
  case H5VL_ATTR_GET_INFO: {
    ...
    if (H5VL_OBJECT_BY_SELF == get_info_args->loc_params.type) {
      ...
      /* Open the attribute on the object header */
      if (NULL ==
          (attr = H5A__open_by_name(
               &loc, get_info_args->loc_params.loc_data.loc_by_name.name,
      ...
      /* Open the attribute on the object header */
      if (NULL == (attr = H5A__open_by_idx(
                       &loc, get_info_args->loc_params.loc_data.loc_by_idx.name,
  ...
  default:
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "can't get this type of information from attr");
  } /* end switch */
...
herr_t H5VL__native_attr_specific(void *obj,
  ...
  case H5VL_ATTR_DELETE: {
    if (H5VL_OBJECT_BY_SELF == loc_params->type) {
      ...
      if (H5A__delete_by_name(&loc, loc_params->loc_data.loc_by_name.name,
                              args->args.del.name) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTDELETE, FAIL,
                    "unable to delete attribute");
    } /* end else-if */
    else
      HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, FAIL,
                  "unknown attribute delete location");
    break;
  }
  ...
  case H5VL_ATTR_DELETE_BY_IDX: {
    ...
    if (H5VL_OBJECT_BY_NAME == loc_params->type) {
      /* Delete the attribute from the location */
      if (H5A__delete_by_idx(&loc, loc_params->loc_data.loc_by_name.name,
                             del_by_idx_args->idx_type, del_by_idx_args->order,
                             del_by_idx_args->n) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTDELETE, FAIL,
                    "unable to delete attribute");
    } /* end if */
      ...
      HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, FAIL,
                  "unknown attribute delete_by_idx location");
    break;
  }
  ...
  case H5VL_ATTR_EXISTS: {
    if (loc_params->type == H5VL_OBJECT_BY_SELF) {
      ...
      if (H5O__attr_exists(loc.oloc, args->args.exists.name,
      ...
      /* Check if the attribute exists */
      if (H5A__exists_by_name(loc, loc_params->loc_data.loc_by_name.name,
                              args->args.exists.name,
                              args->args.exists.exists) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                    "unable to determine if attribute exists");
    } /* end else-if */
  ...
  case H5VL_ATTR_ITER: {
    H5VL_attr_iterate_args_t *iter_args =
        &args->args.iterate;            /* Arguments to iterate operation */
    ...
    if ((ret_value = H5A__iterate(&loc, loc_name, iter_args->idx_type,
                                  iter_args->order, iter_args->idx,
  ...
  case H5VL_ATTR_RENAME: {
    if (loc_params->type == H5VL_OBJECT_BY_SELF) { /* H5Arename */
      ...
      if (H5O__attr_rename(loc.oloc, args->args.rename.old_name,
                           args->args.rename.new_name) < 0)
      ...
      if (H5A__rename_by_name(loc, loc_params->loc_data.loc_by_name.name,
                              args->args.rename.old_name,
                              args->args.rename.new_name) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTRENAME, FAIL, "can't rename attribute");
    } /* end else-if */
    else
      HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, FAIL,
                  "unknown attribute rename parameters");
    break;
  }
...
herr_t H5VL__native_attr_optional(void H5_ATTR_UNUSED *obj,
  ...
  case H5VL_NATIVE_ATTR_ITERATE_OLD: {
    ...
    if ((ret_value = H5A__iterate_old(iter_args->loc_id, iter_args->attr_num,
                                      iter_args->op, iter_args->op_data)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_attr_close(void *attr, hid_t H5_ATTR_UNUSED dxpl_id,
                               void H5_ATTR_UNUSED **req) {
  herr_t ret_value = SUCCEED; /* Return value */
