...
--------------------------------------------------------------------------*/
herr_t H5D__fill(const void *fill, const H5T_t *fill_type, void *buf,
                 const H5T_t *buf_type, H5S_t *space) {
  H5S_sel_iter_t *mem_iter = NULL; /* Memory selection iteration info */
  hbool_t mem_iter_init =
      FALSE; /* Whether the memory selection iterator has been initialized */
  ...
  if (fill == NULL) {
    ...
    if (!H5T_path_noop(tpath)) {
      if ((src_id = H5I_register(H5I_DATATYPE,
                                 H5T_copy(fill_type, H5T_COPY_ALL), FALSE)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, FAIL,
                    "unable to register types for conversion");

      if ((dst_id = H5I_register(H5I_DATATYPE, H5T_copy(buf_type, H5T_COPY_ALL),
                                 FALSE)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, FAIL,
                    "unable to register types for conversion");
    } /* end if */
    ...
    if (TRUE == H5T_detect_class(fill_type, H5T_VLEN, FALSE)) {
      ...
      if (NULL ==
          (tmp_buf = H5FL_BLK_MALLOC(type_conv, (size_t)nelmts * buf_size)))
      ...
      if (H5T_convert(tpath, src_id, dst_id, (size_t)nelmts, (size_t)0,
                      (size_t)0, tmp_buf, bkg_buf) < 0)
      ...
      if (NULL == (mem_iter = H5FL_MALLOC(H5S_sel_iter_t)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "can't allocate memory selection iterator");

      /* Create a selection iterator for scattering the elements to memory
       * buffer */
      ...
      if (!H5T_path_noop(tpath)) {
        void *elem_ptr; /* Pointer to element to use for fill value */
        void *bkg_ptr =
            NULL; /* Pointer to background element to use for fill value */
        ...
        if (NULL == (elem_ptr = H5WB_actual(elem_wb, buf_size)))
          HGOTO_ERROR(H5E_DATASET, H5E_NOSPACE, FAIL,
                      "can't get actual buffer");

        /* Copy the user's data into the buffer for conversion */
        H5MM_memcpy(elem_ptr, fill, src_type_size);

        /* If there's no VL type of data, do conversion first then fill the data
         * into the memory buffer. */
        if (H5T_path_bkg(tpath)) {
          /* Wrap the local buffer for background elements */
          if (NULL ==
              (bkg_elem_wb = H5WB_wrap(bkg_elem_buf, sizeof(bkg_elem_buf))))
            HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't wrap buffer");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__fill_init(H5D_fill_buf_info_t *fb_info, void *caller_fill_buf,
                      H5MM_allocate_t alloc_func, void *alloc_info,
                      H5MM_free_t free_func, void *free_info,
                      const H5O_fill_t *fill, const H5T_t *dset_type,
                      hid_t dset_type_id, size_t total_nelmts,
                      size_t max_buf_size) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (fill->buf) {
    ...
    if ((has_vlen_type = H5T_detect_class(dset_type, H5T_VLEN, FALSE)) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                  "unable to detect vlen datatypes?");
    ...
    if (fb_info->has_vlen_fill_type) {
      ...
      if ((fb_info->mem_tid =
               H5I_register(H5I_DATATYPE, fb_info->mem_type, FALSE)) < 0)
      ...
      fb_info->max_elmt_size =
          MAX(fb_info->mem_elmt_size, fb_info->file_elmt_size);

      /* Compute the number of elements that fit within a buffer to write */
      if (total_nelmts > 0)
        fb_info->elmts_per_buf =
            MIN(total_nelmts, MAX(1, (max_buf_size / fb_info->max_elmt_size)));
      else
      ...
      fb_info->fill_buf_size =
          MIN(max_buf_size, (fb_info->elmts_per_buf * fb_info->max_elmt_size));
      ...
      if (caller_fill_buf) {
        ...
        if (alloc_func)
          ...
          fb_info->fill_buf =
              H5FL_BLK_MALLOC(non_zero_fill, fb_info->fill_buf_size);
      ...
      if (NULL == (fb_info->fill_to_mem_tpath =
                       H5T_path_find(dset_type, fb_info->mem_type)))
      ...
      if (H5T_path_bkg(fb_info->fill_to_mem_tpath) ||
        ...
        if (H5T_path_bkg(fb_info->mem_to_dset_tpath))
          fb_info->bkg_buf_size =
              fb_info->elmts_per_buf * fb_info->max_elmt_size;
        else
          fb_info->bkg_buf_size = fb_info->max_elmt_size;

        /* Allocate the background buffer */
        if (NULL == (fb_info->bkg_buf =
                         H5FL_BLK_MALLOC(type_conv, fb_info->bkg_buf_size)))
          HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                      "memory allocation failed");
      } /* end if */
    }   /* end if */
      ...
      /* If fill value is not library default, use it to set the element size */
      assert(fill->size >= 0);
      fb_info->max_elmt_size = fb_info->file_elmt_size =
          fb_info->mem_elmt_size = (size_t)fill->size;
      ...
      H5VM_array_fill(fb_info->fill_buf, fill->buf, fb_info->max_elmt_size,
                      fb_info->elmts_per_buf);
    }    /* end else */
  }      /* end if */
  else { /* Fill the buffer with the default fill value */
    /* Retrieve size of elements */
    fb_info->max_elmt_size = fb_info->file_elmt_size = fb_info->mem_elmt_size =
        H5T_get_size(dset_type);
    ...
    if (caller_fill_buf) {
      ...
      if (alloc_func) {
        ...
        htri_t buf_avail = H5FL_BLK_AVAIL(
            zero_fill,
            fb_info->fill_buf_size); /* Check if there is an already zeroed out
                                        buffer available */
        ...
        if (!buf_avail)
          fb_info->fill_buf =
              H5FL_BLK_CALLOC(zero_fill, fb_info->fill_buf_size);
        else
          fb_info->fill_buf =
              H5FL_BLK_MALLOC(zero_fill, fb_info->fill_buf_size);
      } /* end else */
...
herr_t H5D__fill_refill_vl(H5D_fill_buf_info_t *fb_info, size_t nelmts) {
  ...
  if (H5T_convert(fb_info->fill_to_mem_tpath, fb_info->file_tid,
                  fb_info->mem_tid, (size_t)1, (size_t)0, (size_t)0,
                  fb_info->fill_buf, fb_info->bkg_buf) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, FAIL,
                "data type conversion failed");
  ...
  if (nelmts > 1)
    H5VM_array_fill(
        (void *)((unsigned char *)fb_info->fill_buf + fb_info->mem_elmt_size),
  ...
  /* Make a copy of the fill buffer so we can free dynamic elements after
   * conversion */
  if (fb_info->fill_alloc_func)
    buf = fb_info->fill_alloc_func(fb_info->fill_buf_size,
                                   fb_info->fill_alloc_info);
  else
    buf = H5FL_BLK_MALLOC(non_zero_fill, fb_info->fill_buf_size);
  if (!buf)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "memory allocation failed for temporary fill buffer");

  H5MM_memcpy(buf, fb_info->fill_buf, fb_info->fill_buf_size);

  /* Type convert the dataset buffer, to copy any VL components */
  if (H5T_convert(fb_info->mem_to_dset_tpath, fb_info->mem_tid,
                  fb_info->file_tid, nelmts, (size_t)0, (size_t)0,
                  fb_info->fill_buf, fb_info->bkg_buf) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, FAIL,
                "data type conversion failed");
