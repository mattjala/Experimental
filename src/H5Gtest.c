...
htri_t H5G__is_empty_test(hid_t gid) {
  ...
  htri_t linfo_exists =
      FALSE; /* Indicate that the 'link info' message is present */
  ...
  if (msg_exists > 0) {
    ...
    if (msg_exists > 0)
      HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                  "both symbol table and link messages found");
  ...
  if (linfo_exists > 0) {
    ...
    if (msg_exists > 0)
      HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                  "both symbol table and link info messages found");
  ...
  if (msg_exists > 0) {
    ...
    if (msg_exists > 0)
      HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                  "both symbol table and group info messages found");
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5G__new_dense_info_test(hid_t gid, hsize_t *name_count,
  ...
  if (H5_addr_defined(linfo.corder_bt2_addr)) {
    ...
    if (NULL ==
        (bt2_corder = H5B2_open(grp->oloc.file, linfo.corder_bt2_addr, NULL)))
      HGOTO_ERROR_TAG(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5G__user_path_test(hid_t obj_id, char *user_path, size_t *user_path_len,
                           unsigned *obj_hidden) {
...
herr_t H5G__verify_cached_stab_test(H5O_loc_t *grp_oloc, H5G_entry_t *ent) {
  ...
  if (ent->type != H5G_CACHED_STAB)
    HGOTO_ERROR(H5E_SYM, H5E_BADTYPE, FAIL,
                "symbol table information is not cached");
  ...
  if ((ent->cache.stab.btree_addr != stab.btree_addr) ||
    ...
    HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                "cached stab info does not match object header");
 ...
 *-------------------------------------------------------------------------
 */
static int H5G__verify_cached_stabs_test_cb(H5F_t *f,
  ...
  for (i = 0; i < sn->nsyms; i++) {
    ...
    if (NULL ==
        (targ_oh = H5O_protect(&targ_oloc, H5AC__READ_ONLY_FLAG, FALSE)))
      HGOTO_ERROR(H5E_SYM, H5E_CANTPROTECT, H5_ITER_ERROR,
                  "unable to protect target object header");
    ...
    if (stab_exists) {
      ...
      if (sn->entry[i].type != H5G_CACHED_STAB)
        HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, H5_ITER_ERROR,
                    "STAB message is not cached in group node");
      ...
      HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, H5_ITER_ERROR,
                  "nonexistent STAB message is cached");

    /* Unprotect target object */
    if (H5O_unprotect(&targ_oloc, targ_oh, H5AC__NO_FLAGS_SET) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTUNPROTECT, H5_ITER_ERROR,
  ...
  if (targ_oh) {
    ...
    if (H5O_unprotect(&targ_oloc, targ_oh, H5AC__NO_FLAGS_SET) < 0)
      HDONE_ERROR(H5E_SYM, H5E_CANTUNPROTECT, H5_ITER_ERROR,
                  "unable to release object header");
  } /* end if */
...
herr_t H5G__verify_cached_stabs_test(hid_t gid) {
  ...
  H5G_bt_common_t udata = {
      NULL, NULL}; /* Dummy udata so H5B_iterate doesn't freak out */
