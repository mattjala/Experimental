 ...
 *-------------------------------------------------------------------------
 */
static int H5I__id_dump_cb(void *_item, void H5_ATTR_UNUSED *_key,
                           void *_udata) {
  hbool_t have_global_mutex = TRUE; /* Trivially true in single thread builds */
  hbool_t drop_global_mutex =
      FALSE; /* Trivially false in single thread builds */
  H5I_mt_id_info_kernel_t info_k;
  H5I_mt_id_info_t *id_info_ptr =
      (H5I_mt_id_info_t *)_item;           /* Pointer to the ID node */
...
herr_t H5I_dump_ids_for_type(H5I_type_t type) {
  ...
  if (type_info_ptr) {
    ...
    fprintf(stderr,
            "Global type info/tracking pointer for that type is NULL\n");
  }
