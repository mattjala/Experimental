...
static herr_t H5SM__compare_iter_op(H5O_t *oh, H5O_mesg_t *mesg,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5SM__compare_iter_op(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                    unsigned sequence,
  ...
  if (sequence == udata->idx) {
    ...
    if (aligned_encoded_size > mesg->raw_size)
      ...
      udata->ret =
          memcmp(udata->key->encoding, mesg->raw, udata->key->encoding_size);
    } /* end else */
...
herr_t H5SM__message_compare(const void *rec1, const void *rec2, int *result) {
  ...
  else if (mesg->location == H5SM_IN_OH &&
  ...
  /* If the hash values match, make sure the messages are really the same */
    ...
    if (mesg->location == H5SM_IN_HEAP) {
      ...
      if (H5HF_op(key->fheap, &(mesg->u.heap_loc.fheap_id), H5SM__compare_cb,
                  &udata) < 0)
  ...
  *raw++ = (uint8_t)message->location;
  ...
  if (message->location == H5SM_IN_HEAP) {
    ...
    *raw++ = (uint8_t)message->msg_type_id;
    ...
    H5F_addr_encode_len((size_t)ctx->sizeof_addr, &raw,
                        message->u.mesg_loc.oh_addr);
  } /* end else */
 ...
 *
 * Purpose:	Read an encoded SOHM message from RAW into an H5SM_sohm_t
 *struct.
 *
 * Return:	Non-negative on success
 *              Negative on failure
...
herr_t H5SM__message_decode(const uint8_t *raw, void *_nrecord, void *_ctx) {
  ...
  if (message->location == H5SM_IN_HEAP) {
    ...
    H5F_addr_decode_len((size_t)ctx->sizeof_addr, &raw,
                        &message->u.mesg_loc.oh_addr);
  } /* end else */
