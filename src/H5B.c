...
static H5B_ins_t H5B__insert_helper(H5F_t *f, H5B_ins_ud_t *bt_ud,
                                    const H5B_class_t *type, uint8_t *lt_key,
                                    hbool_t *lt_key_changed, uint8_t *md_key,
                                    void *udata, uint8_t *rt_key,
                                    hbool_t *rt_key_changed,
                                    H5B_ins_ud_t *split_bt_ud /*out*/);
static herr_t H5B__insert_child(H5B_t *bt, unsigned *bt_flags, unsigned idx,
                                haddr_t child, H5B_ins_t anchor,
                                const void *md_key);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_create(H5F_t *f, const H5B_class_t *type, void *udata,
                  haddr_t *addr_p /*out*/) {
  H5B_t *bt = NULL;
  ...
  if (NULL == (bt->rc_shared = (type->get_shared)(f, udata)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTGET, FAIL,
                "can't retrieve B-tree node buffer");
  ...
  if (NULL ==
    ...
    HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, FAIL,
                "memory allocation failed for B-tree root node");
  if (HADDR_UNDEF ==
      (*addr_p = H5MF_alloc(f, H5FD_MEM_BTREE, (hsize_t)shared->sizeof_rnode)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, FAIL,
  ...
  if (ret_value < 0) {
    if (shared && shared->sizeof_rnode > 0) {
      ...
      (void)H5MF_xfree(f, H5FD_MEM_BTREE, *addr_p,
                       (hsize_t)shared->sizeof_rnode);
    } /* end if */
    if (bt)
      /* Destroy B-tree node */
      if (H5B__node_dest(bt) < 0)
        HDONE_ERROR(H5E_BTREE, H5E_CANTFREE, FAIL,
                    "unable to destroy B-tree node");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_find(H5F_t *f, const H5B_class_t *type, haddr_t addr, hbool_t *found,
                void *udata) {
  H5B_t *bt = NULL;
  ...
  if (NULL == (bt = (H5B_t *)H5AC_protect(f, H5AC_BT, addr, &cache_udata,
  ...
  while (lt < rt && cmp) {
    idx = (lt + rt) / 2;
    /* compare */
    if ((cmp = (type->cmp3)(H5B_NKEY(bt, shared, idx), udata,
  ...
  if (cmp)
    *found = FALSE;
    ...
    if (bt->level > 0) {
      ...
      if ((ret_value = (type->found)(
               f, bt->child[idx], H5B_NKEY(bt, shared, idx), found, udata)) < 0)
        HGOTO_ERROR(H5E_BTREE, H5E_NOTFOUND, FAIL,
...
static herr_t H5B__split(H5F_t *f, H5B_ins_ud_t *bt_ud, unsigned idx,
  ...
  if (H5DEBUG(B)) {
    ...
    fprintf(H5DEBUG(B), "H5B__split: %3u {%5.3f,%5.3f,%5.3f} %6s",
            shared->two_k, split_ratios[0], split_ratios[1], split_ratios[2],
            side);
  }
#endif
  ...
  if (NULL == (split_bt_ud->bt =
                   (H5B_t *)H5AC_protect(f, H5AC_BT, split_bt_ud->addr,
  ...
  H5MM_memcpy(split_bt_ud->bt->child, &bt_ud->bt->child[nleft],
  ...
  if (H5_addr_defined(bt_ud->bt->right)) {
    ...
    if (NULL ==
        (tmp_bt = (H5B_t *)H5AC_protect(f, H5AC_BT, bt_ud->bt->right,
                                        &cache_udata, H5AC__NO_FLAGS_SET)))
    ...
    if (H5AC_unprotect(f, H5AC_BT, bt_ud->bt->right, tmp_bt,
  ...
  if (ret_value < 0) {
    if (split_bt_ud->bt &&
        H5AC_unprotect(f, H5AC_BT, split_bt_ud->addr, split_bt_ud->bt,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_insert(H5F_t *f, const H5B_class_t *type, haddr_t addr,
                  void *udata) {
  /*
  ...
  cache_udata.rc_shared = rc_shared;
  bt_ud.addr = addr;
  if (NULL == (bt_ud.bt = (H5B_t *)H5AC_protect(f, H5AC_BT, addr, &cache_udata,
                                                H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, FAIL,
  ...
  if ((int)(my_ins = H5B__insert_helper(
                f, &bt_ud, type, lt_key, &lt_key_changed, md_key, udata, rt_key,
  ...
  if (!rt_key_changed)
    H5MM_memcpy(rt_key,
                H5B_NKEY(split_bt_ud.bt, shared, split_bt_ud.bt->nchildren),
                type->sizeof_nkey);
   ...
   */
  H5_CHECK_OVERFLOW(shared->sizeof_rnode, size_t, hsize_t);
  if (HADDR_UNDEF == (old_root_addr = H5MF_alloc(
                          f, H5FD_MEM_BTREE, (hsize_t)shared->sizeof_rnode)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, FAIL,
  ...
  H5MM_memcpy(H5B_NKEY(new_root_bt, shared, 0), lt_key,
              shared->type->sizeof_nkey);

  new_root_bt->child[1] = split_bt_ud.addr;
  H5MM_memcpy(H5B_NKEY(new_root_bt, shared, 1), md_key,
              shared->type->sizeof_nkey);
  H5MM_memcpy(H5B_NKEY(new_root_bt, shared, 2), rt_key,
              shared->type->sizeof_nkey);

  /* Insert the modified copy of the old root into the file again */
  if (H5AC_insert_entry(f, H5AC_BT, addr, new_root_bt, H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_BTREE, H5E_CANTFLUSH, FAIL,
                "unable to add old B-tree root node to cache");

done:
  if (ret_value < 0)
    if (new_root_bt && H5B__node_dest(new_root_bt) < 0)
      HDONE_ERROR(H5E_BTREE, H5E_CANTRELEASE, FAIL,
                  "unable to free B-tree root node");

  if (bt_ud.bt)
    if (H5AC_unprotect(f, H5AC_BT, bt_ud.addr, bt_ud.bt, bt_ud.cache_flags) < 0)
      HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, FAIL,
                  "unable to unprotect old root");

  if (split_bt_ud.bt)
    if (H5AC_unprotect(f, H5AC_BT, split_bt_ud.addr, split_bt_ud.bt,
                       split_bt_ud.cache_flags) < 0)
      HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, FAIL,
                  "unable to unprotect new child");
...
static herr_t H5B__insert_child(H5B_t *bt, unsigned *bt_flags, unsigned idx,
  ...
  if ((idx + 1) == bt->nchildren) {
    ...
    H5MM_memcpy(
        base + shared->type->sizeof_nkey, base,
    ...
    /* Make room for the new key */
    memmove(base + shared->type->sizeof_nkey, base,
    ...
    /* Make room for the new child address */
    memmove(bt->child + idx + 1, bt->child + idx,
...
static H5B_ins_t H5B__insert_helper(H5F_t *f, H5B_ins_ud_t *bt_ud,
  ...
  H5B_ins_ud_t new_child_bt_ud =
      H5B_INS_UD_T_NULL; /* Newly split child B-tree */
  H5B_ins_t my_ins = H5B_INS_ERROR;
  ...
  *rt_key_changed = FALSE;
  ...
  if (0 == bt->nchildren) {
    ...
    if (type->follow_min) {
      if ((int)(my_ins = (type->insert)(
                    f, bt->child[idx], H5B_NKEY(bt, shared, idx),
    ...
    if (bt->level > 0) {
      ...
      if (NULL == (child_bt_ud.bt =
                       (H5B_t *)H5AC_protect(f, H5AC_BT, child_bt_ud.addr,
                                             &cache_udata, H5AC__NO_FLAGS_SET)))
        HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, H5B_INS_ERROR,
                    "unable to load node");

      if ((int)(my_ins = H5B__insert_helper(
                    f, &child_bt_ud, type, H5B_NKEY(bt, shared, idx),
                    lt_key_changed, md_key, udata,
                    H5B_NKEY(bt, shared, idx + 1), rt_key_changed,
    ...
    assert(idx < bt->nchildren);
    if ((int)(my_ins = (type->insert)(
                  f, bt->child[idx], H5B_NKEY(bt, shared, idx), lt_key_changed,
  ...
  if (H5B_INS_CHANGE == my_ins) {
    ...
    /* Insert the child */
    if (H5B__insert_child(tmp_bt, tmp_bt_flags_ptr, idx, new_child_bt_ud.addr,
                          my_ins, md_key) < 0)
      HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, H5B_INS_ERROR,
                  "can't insert child");
  } /* end else-if */
   ...
   */
  if (split_bt_ud->bt) {
    H5MM_memcpy(md_key, H5B_NKEY(split_bt_ud->bt, shared, 0),
                type->sizeof_nkey);
  ...
  if (child_bt_ud.bt)
    if (H5AC_unprotect(f, H5AC_BT, child_bt_ud.addr, child_bt_ud.bt,
                       child_bt_ud.cache_flags) < 0)
      HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, H5B_INS_ERROR,
                  "unable to unprotect child");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5B__iterate_helper(H5F_t *f, const H5B_class_t *type,
                                  haddr_t addr, H5B_operator_t op,
                                  void *udata) {
  ...
  if (NULL == (bt = (H5B_t *)H5AC_protect(f, H5AC_BT, addr, &cache_udata,
                                          H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, H5_ITER_ERROR,
  ...
  for (u = 0; u < bt->nchildren && ret_value == H5_ITER_CONT; u++) {
    if (bt->level > 0)
      ...
      ret_value = (*op)(f, H5B_NKEY(bt, shared, u), bt->child[u],
  ...
  if (bt && H5AC_unprotect(f, H5AC_BT, addr, bt, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, H5_ITER_ERROR,
                "unable to release B-tree node");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_iterate(H5F_t *f, const H5B_class_t *type, haddr_t addr,
 ...
 *-------------------------------------------------------------------------
 */
static H5B_ins_t H5B__remove_helper(H5F_t *f, haddr_t addr,
                                    const H5B_class_t *type, int level,
                                    uint8_t *lt_key /*out*/,
                                    hbool_t *lt_key_changed /*out*/,
                                    void *udata, uint8_t *rt_key /*out*/,
  ...
  if (bt->level > 0) {
    ...
    if ((int)(ret_value = H5B__remove_helper(
                  f, bt->child[idx], type, level + 1,
                  H5B_NKEY(bt, shared, idx) /*out*/, lt_key_changed /*out*/,
                  udata, H5B_NKEY(bt, shared, idx + 1) /*out*/,
  ...
  if (H5B_INS_REMOVE == ret_value) {
    ...
    if (1 == bt->nchildren) {
      ...
      if (level > 0) {
        ...
        if (H5_addr_defined(bt->left)) {
          if (NULL == (sibling = (H5B_t *)H5AC_protect(f, H5AC_BT, bt->left,
                                                       &cache_udata,
                                                       H5AC__NO_FLAGS_SET)))
            HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, H5B_INS_ERROR,
          ...
          if (H5AC_unprotect(f, H5AC_BT, bt->left, sibling,
                             H5AC__DIRTIED_FLAG) < 0)
        ...
        if (H5_addr_defined(bt->right)) {
          if (NULL == (sibling = (H5B_t *)H5AC_protect(f, H5AC_BT, bt->right,
                                                       &cache_udata,
                                                       H5AC__NO_FLAGS_SET)))
            HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, H5B_INS_ERROR,
          ...
          if (type->critical_key == H5B_RIGHT)
            H5MM_memcpy(H5B_NKEY(sibling, shared, 0), H5B_NKEY(bt, shared, 0),
                        type->sizeof_nkey);

          sibling->left = bt->left;

          if (H5AC_unprotect(f, H5AC_BT, bt->right, sibling,
                             H5AC__DIRTIED_FLAG) < 0)
            HGOTO_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, H5B_INS_ERROR,
                        "unable to release node from tree");
          sibling = NULL; /* Make certain future references will be caught */
        ...
        H5_CHECK_OVERFLOW(shared->sizeof_rnode, size_t, hsize_t);
        if (H5AC_unprotect(f, H5AC_BT, addr, bt,
                           bt_flags | H5AC__DELETED_FLAG) < 0) {
          bt = NULL;
      ...
      if (type->critical_key == H5B_LEFT)
        ...
        memmove(H5B_NKEY(bt, shared, bt->nchildren - 1),
      ...
      memmove(bt->child + idx, bt->child + idx + 1,
  ...
  if (*lt_key_changed && H5_addr_defined(bt->left)) {
    ...
    if (NULL == (sibling = (H5B_t *)H5AC_protect(
                     f, H5AC_BT, bt->left, &cache_udata, H5AC__NO_FLAGS_SET)))
      HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, H5B_INS_ERROR,
                  "unable to protect node");
    ...
    if (NULL == (sibling = (H5B_t *)H5AC_protect(
                     f, H5AC_BT, bt->right, &cache_udata, H5AC__NO_FLAGS_SET)))
      HGOTO_ERROR(H5E_BTREE, H5E_CANTPROTECT, H5B_INS_ERROR,
                  "unable to protect node");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_remove(H5F_t *f, const H5B_class_t *type, haddr_t addr,
                  void *udata) {
  /* These are defined this way to satisfy alignment constraints */
  ...
  /* The actual removal */
  if (H5B_INS_ERROR == H5B__remove_helper(f, addr, type, 0, lt_key,
                                          &lt_key_changed, udata, rt_key,
                                          &rt_key_changed))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTINIT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_delete(H5F_t *f, const H5B_class_t *type, haddr_t addr,
                  void *udata) {
    ...
    for (u = 0; u < bt->nchildren; u++)
      if (H5B_delete(f, type, bt->child[u], udata) < 0)
        HGOTO_ERROR(H5E_BTREE, H5E_CANTLIST, FAIL,
                    "unable to delete B-tree node");

  } /* end if */
  else {
    hbool_t lt_key_changed, rt_key_changed; /* Whether key changed (unused here,
                                               just for callback) */
    ...
    if (type->remove) {
      ...
      for (u = 0; u < bt->nchildren; u++) {
        ...
        if ((type->remove)(f, bt->child[u], H5B_NKEY(bt, shared, u),
          ...
          HGOTO_ERROR(H5E_BTREE, H5E_NOTFOUND, FAIL,
                      "can't remove B-tree node");
      } /* end for */
    }   /* end if */
  }     /* end else */

done:
  if (bt && H5AC_unprotect(f, H5AC_BT, addr, bt,
                           H5AC__DELETED_FLAG | H5AC__FREE_FILE_SPACE_FLAG) < 0)
    HDONE_ERROR(H5E_BTREE, H5E_CANTUNPROTECT, FAIL,
                "unable to release B-tree node in cache");
 ...
 *-------------------------------------------------------------------------
 */
H5B_shared_t *H5B_shared_new(const H5F_t *f, const H5B_class_t *type,
                             size_t sizeof_rkey) {
  ...
  if (NULL == (shared = H5FL_CALLOC(H5B_shared_t)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, NULL,
                "memory allocation failed for shared B-tree info");
  ...
  shared->sizeof_rnode =
      ((size_t)H5B_SIZEOF_HDR(f) +                 /*node header	*/
  ...
  if (NULL == (shared->page = H5FL_BLK_MALLOC(page, shared->sizeof_rnode)))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, NULL,
                "memory allocation failed for B-tree page");
  memset(shared->page, 0, shared->sizeof_rnode);

  if (NULL ==
      (shared->nkey = H5FL_SEQ_MALLOC(size_t, (size_t)(shared->two_k + 1))))
    HGOTO_ERROR(H5E_BTREE, H5E_CANTALLOC, NULL,
                "memory allocation failed for B-tree native keys");
...
static H5B_t *H5B__copy(const H5B_t *old_bt) {
  ...
  if (NULL == (new_node->native =
                   H5FL_BLK_MALLOC(native_block, shared->sizeof_keys)) ||
      NULL ==
  ...
  H5MM_memcpy(new_node->child, old_bt->child,
              (sizeof(haddr_t) * shared->two_k));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5B__get_info_helper(H5F_t *f, const H5B_class_t *type,
                                   haddr_t addr,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5B_get_info(H5F_t *f, const H5B_class_t *type, haddr_t addr,
