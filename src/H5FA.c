...
static H5FA_t *H5FA__new(H5F_t *f, haddr_t fa_addr, hbool_t from_open,
  ...
  if (NULL == (fa = H5FL_CALLOC(H5FA_t)))
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTALLOC, NULL,
                "memory allocation failed for fixed array info");

  /* Lock the array header into memory */
  if (NULL ==
      (hdr = H5FA__hdr_protect(f, fa_addr, ctx_udata, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTPROTECT, NULL,
                "unable to load fixed array header");

  /* Check for pending array deletion */
  if (from_open && hdr->pending_delete)
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTOPENOBJ, NULL,
                "can't open fixed array pending deletion");
  ...
  if (hdr && H5FA__hdr_unprotect(hdr, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_FARRAY, H5E_CANTUNPROTECT, NULL,
                "unable to release fixed array header");
  if (!ret_value)
    if (fa && H5FA_close(fa) < 0)
      HDONE_ERROR(H5E_FARRAY, H5E_CLOSEERROR, NULL,
...
herr_t H5FA_set(const H5FA_t *fa, hsize_t idx, const void *elmt) {
  ...
  H5FA_dblk_page_t *dblk_page =
      NULL; /* Pointer to fixed array Data block page */
  unsigned dblock_cache_flags =
      H5AC__NO_FLAGS_SET; /* Flags to unprotecting fixed array Data block */
  ...
  if (!H5_addr_defined(hdr->dblk_addr)) {
    ...
    if (!H5_addr_defined(hdr->dblk_addr))
      HGOTO_ERROR(H5E_FARRAY, H5E_CANTCREATE, FAIL,
                  "unable to create fixed array data block");
  }
  ...
  /* Protect data block */
  if (NULL ==
      (dblock = H5FA__dblock_protect(hdr, hdr->dblk_addr, H5AC__NO_FLAGS_SET)))
  ...
  if (!dblock->npages) {
    ...
    H5MM_memcpy(((uint8_t *)dblock->elmts) +
                    (hdr->cparam.cls->nat_elmt_size * idx),
    ...
    if (NULL == (dblk_page = H5FA__dblk_page_protect(hdr, dblk_page_addr,
                                                     dblk_page_nelmts,
                                                     H5AC__NO_FLAGS_SET)))
      HGOTO_ERROR(
          H5E_FARRAY, H5E_CANTPROTECT, FAIL,
    ...
    H5MM_memcpy(((uint8_t *)dblk_page->elmts) +
                    (hdr->cparam.cls->nat_elmt_size * elmt_idx),
                elmt, hdr->cparam.cls->nat_elmt_size);
  ...
  if (hdr_dirty)
    if (H5FA__hdr_modified(hdr) < 0)
      HDONE_ERROR(H5E_FARRAY, H5E_CANTMARKDIRTY, FAIL,
                  "unable to mark fixed array header as modified");

  /* Release resources */
  if (dblock && H5FA__dblock_unprotect(dblock, dblock_cache_flags) < 0)
    HDONE_ERROR(H5E_FARRAY, H5E_CANTUNPROTECT, FAIL,
                "unable to release fixed array data block");
  if (dblk_page &&
      H5FA__dblk_page_unprotect(dblk_page, dblk_page_cache_flags) < 0)
    HDONE_ERROR(H5E_FARRAY, H5E_CANTUNPROTECT, FAIL,
...
herr_t H5FA_get(const H5FA_t *fa, hsize_t idx, void *elmt) {
  ...
  if (!H5_addr_defined(hdr->dblk_addr)) {
    ...
    if (NULL == (dblock = H5FA__dblock_protect(hdr, hdr->dblk_addr,
...
herr_t H5FA_close(H5FA_t *fa) {
  ...
  if (fa->hdr) {
    /* Decrement file reference & check if this is the last open fixed array
     * using the shared array header
     */
    ...
    if (pending_delete) {
       ...
       * be in the cache) */
      if (NULL ==
          (hdr = H5FA__hdr_protect(fa->f, fa_addr, NULL, H5AC__NO_FLAGS_SET)))
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTLOAD, FAIL,
                    "unable to load fixed array header");
      ...
      if (H5FA__hdr_delete(hdr) < 0)
        HGOTO_ERROR(H5E_FARRAY, H5E_CANTDELETE, FAIL,
...
herr_t H5FA_delete(H5F_t *f, haddr_t fa_addr, void *ctx_udata) {
  ...
  /* Lock the array header into memory */
  if (NULL ==
      (hdr = H5FA__hdr_protect(f, fa_addr, ctx_udata, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTPROTECT, FAIL,
                "unable to protect fixed array header, address = %llu",
...
int H5FA_iterate(H5FA_t *fa, H5FA_operator_t op, void *udata) {
  ...
  if (NULL == (elmt = H5FL_BLK_MALLOC(fa_native_elmt,
                                      fa->hdr->cparam.cls->nat_elmt_size)))
  ...
  for (u = 0; u < fa->hdr->stats.nelmts && ret_value == H5_ITER_CONT; u++) {
    ...
    if (H5FA_get(fa, u, elmt) < 0)
      HGOTO_ERROR(H5E_FARRAY, H5E_CANTGET, H5_ITER_ERROR,
                  "unable to delete fixed array");
...
herr_t H5FA_depend(H5FA_t *fa, H5AC_proxy_entry_t *parent) {
  ...
  if (NULL == hdr->parent) {
    ...
    if (H5AC_proxy_entry_add_child(parent, hdr->f, hdr->top_proxy) < 0)
      HGOTO_ERROR(H5E_FARRAY, H5E_CANTSET, FAIL,
                  "unable to add fixed array as child of proxy");
    hdr->parent = parent;
  }
