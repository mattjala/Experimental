...
#define H5MF_SECT_CLASS_TYPE(F, S)                                             \
  ((H5F_PAGED_AGGR(F))                                                         \
       ? ((S >= (F)->shared->fs_page_size) ? H5MF_FSPACE_SECT_LARGE            \
...
#define H5MF_SECT_CLS_TYPE(F, S)                                               \
  ((H5F_PAGED_AGGR(F))                                                         \
       ? ((S >= (F)->shared->fs_page_size) ? H5MF_FSPACE_SECT_CLS_LARGE        \
...
typedef struct H5MF_sect_ud_t {
  ...
  hbool_t allow_sect_absorb; /* Whether sections are allowed to absorb a block
                                aggregator */
  hbool_t allow_eoa_shrink_only; /* Whether shrinking eoa is allowed only for
                                    the section */
...
/* H5MF single section inherits serializable properties from
...
H5_DLL htri_t H5MF__find_sect(H5F_t *f, H5FD_mem_t alloc_type, hsize_t size,
                              H5FS_t *fspace, haddr_t *addr);
H5_DLL herr_t H5MF__add_sect(H5F_t *f, H5FD_mem_t alloc_type, H5FS_t *fspace,
...
H5_DLL htri_t H5MF__aggr_try_extend(H5F_t *f, H5F_blk_aggr_t *aggr,
                                    H5FD_mem_t type, haddr_t abs_blk_end,
