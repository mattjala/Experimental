...
#define H5EA_HEADER_SIZE(sizeof_addr, sizeof_size)                             \
  (/* General metadata fields */                                               \
   ...
   + 1 /* Log2(Max. # of elements in data block page) - i.e. # of bits needed  \
...
#define H5EA_SBLOCK_SIZE(s)                                                    \
  (/* General metadata fields */                                               \
   ...
   + ((s)->ndblks *                                                            \
      (s)->hdr->sizeof_addr) /* Data block addresses in super block  */        \
  )
...
/* Compute the first super block index that will hold a certain # of data block
 * pointers */
...
typedef struct H5EA_sblk_info_t {
  ...
  size_t
      dblk_nelmts;   /* Number of elements in each data block for super block */
...
typedef struct H5EA_hdr_t {
  ...
  struct {
    ...
    H5FL_fac_head_t *
        *fac; /* Array of factories for data block element buffers */
  } elmt_fac;
  ...
  hbool_t swmr_write; /* Flag indicating the file is opened with SWMR-write
                         access */
  H5AC_proxy_entry_t
      *top_proxy; /* 'Top' proxy cache entry for all array entries */
...
typedef struct H5EA_iblock_t {
  ...
  size_t nsblks; /* # of super blocks whose data block addresses are in index
                    block */
...
typedef struct H5EA_sblock_t {
  ...
  /* SWMR / Flush dependency information (not stored) */
  hbool_t has_hdr_depend; /* Whether this object has a flush dependency on the
                             header */
  H5AC_proxy_entry_t
      *top_proxy;        /* "Top" proxy cache entry for all array entries */
  ...
  size_t dblk_nelmts; /* # of elements for data blocks reachable through this
                         super block */
  size_t dblk_npages; /* # of pages in each data block */
  size_t
      dblk_page_init_size; /* Size of 'page init' bitmask for each data block */
...
typedef struct H5EA_sblock_cache_ud_t {
  ...
  H5EA_iblock_t
      *parent; /* Pointer to parent object for super block (index block) */
...
typedef struct H5EA_dblock_cache_ud_t {
  H5EA_hdr_t *hdr;   /* Shared extensible array information */
  void *parent;      /* Pointer to parent object for data block (index or super
                        block) */
...
typedef struct H5EA__ctx_cb_t {
  herr_t (*encode)(const void *elmt, size_t nelmts,
...
H5_DLL H5EA_hdr_t *H5EA__hdr_protect(H5F_t *f, haddr_t ea_addr, void *ctx_udata,
                                     unsigned flags);
...
H5_DLL herr_t H5EA__hdr_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                              int fwidth, const H5EA_class_t *cls,
                              haddr_t obj_addr);
H5_DLL herr_t H5EA__iblock_debug(H5F_t *f, haddr_t addr, FILE *stream,
                                 int indent, int fwidth,
                                 const H5EA_class_t *cls, haddr_t hdr_addr,
                                 haddr_t obj_addr);
H5_DLL herr_t H5EA__sblock_debug(H5F_t *f, haddr_t addr, FILE *stream,
                                 int indent, int fwidth,
                                 const H5EA_class_t *cls, haddr_t hdr_addr,
                                 unsigned sblk_idx, haddr_t obj_addr);
H5_DLL herr_t H5EA__dblock_debug(H5F_t *f, haddr_t addr, FILE *stream,
                                 int indent, int fwidth,
                                 const H5EA_class_t *cls, haddr_t hdr_addr,
