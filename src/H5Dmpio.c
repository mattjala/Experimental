...
/* Macros to represent different IO modes(NONE, Independent or collective)for
 * multiple chunk IO case */
#define H5D_CHUNK_IO_MODE_COL 1

/* Macros to represent the regularity of the selection for multiple chunk IO
 * case. */
 ...
 */
#define H5D_CHUNK_REDISTRIBUTE_THRES                                           \
  ((size_t)((25 * H5_MB) / sizeof(H5D_chunk_redistribute_info_t)))
 ...
 * of the fields of one of these structs are initialized at the start of
 * collective filtered I/O in the function
 * H5D__mpio_collective_filtered_chunk_io_setup(). This struct's fields are as
 * follows:
 *
 * index_info - A structure containing the information needed when collectively
 *              re-inserting the chunk into the dataset's chunk index. The
 * structure is distributed to all ranks during the re-insertion operation. Its
 * fields are as follows:
 *
 *     chunk_idx - The index of the chunk in the dataset's chunk index.
 *
 *     filter_mask - A bit-mask that indicates which filters are to be applied
 * to the chunk. Each filter in a chunk's filter pipeline has a bit position
 *                   that can be masked to disable that particular filter for
 * the chunk. This filter mask is saved alongside the chunk in the file.
 *
 *     need_insert - A flag which determines whether or not a chunk needs to be
 * re-inserted into the chunk index after the write operation.
 *
 * chunk_info - A pointer to the chunk's H5D_piece_info_t structure, which
 * contains useful information like the dataspaces containing the selection in
 * the chunk.
 *
 * chunk_current - The address in the file and size of this chunk before the
 * filtering operation. When reading a chunk from the file, this field is used
 * to read the correct amount of bytes. It is also used when redistributing
 *                 shared chunks among MPI ranks and as a parameter to the chunk
 * file space reallocation function.
 *
 * chunk_new - The address in the file and size of this chunk after the
 * filtering operation. This field is relevant when collectively re-allocating
 * space in the file for all of the chunks written to in the I/O operation, as
 *             their sizes may have changed after their data has been filtered.
 *
 * need_read - A flag which determines whether or not a chunk needs to be read
 * from the file. During writes, if a chunk is being fully overwritten (the
 * entire extent is selected in its file dataspace), then it is not necessary to
 ...
 * without trashing the parts of the chunk that are not selected. During reads,
 * this field should generally be true, but may be false if the chunk isn't
 * allocated, for example.
 *
 * skip_filter_pline - A flag which determines whether to skip calls to the
 * filter pipeline for this chunk. This flag is mostly useful for correct
 * handling of partial edge chunks when the "don't filter partial edge chunks"
 * flag is set on the dataset's DCPL.
 *
 * io_size - The total size of I/O to this chunk. This field is an accumulation
 * of the size of I/O to the chunk from each MPI rank which has the chunk
 * selected and is used to determine the value for the previous `full_overwrite`
 * flag.
 *
 * chunk_buf_size - The size in bytes of the data buffer allocated for the chunk
 *
 * orig_owner - The MPI rank which originally had this chunk selected at the
 * beginning of the collective filtered I/O operation. This field is currently
 * used when redistributing shared chunks among MPI ranks.
 *
 * new_owner - The MPI rank which has been selected to perform the modifications
 * to this chunk.
 *
 * num_writers - The total number of MPI ranks writing to this chunk. This field
 * is used when the new owner of a chunk is receiving messages from other MPI
 * ranks that contain their selections in the chunk and the data to update the
 * chunk with. The new owner must know how many MPI ranks it should expect
 * messages from so that it can post an equal number of receive calls.
 *
 * buf - A pointer which serves the dual purpose of holding either the chunk
 * data which is to be written to the file or the chunk data which has been read
 * from the file.
 *
...
/*
 * Top-level structure that contains an array of
 * H5D_filtered_collective_chunk_info_t chunk info structures for collective
 * filtered I/O, as well as other useful information. The struct's fields are as
 * follows:
 *
 * chunk_infos - An array of H5D_filtered_collective_chunk_info_t structures
 * that each contain information about a single chunk when performing collective
 * filtered I/O.
 *
 * chunk_hash_table - A hash table storing H5D_filtered_collective_chunk_info_t
 * structures that is populated when chunk modification data has to be shared
 * between MPI processes during collective filtered I/O. This hash table
 * facilitates quicker and easier lookup of a particular chunk by its "chunk
 * index" value when applying chunk data modification messages from another MPI
 *                    process. Each modification message received from another
 * MPI process will contain the chunk's "chunk index" value that can be used for
 * chunk lookup operations.
 *
 * num_chunks_infos - The number of entries in the `chunk_infos` array.
 *
 * num_chunks_to_read - The number of entries (or chunks) in the `chunk_infos`
 * array that will need to be read in the case of a read operation (which can
 *                      occur during dataset reads or during dataset writes when
 * a chunk has to go through a read - modify - write cycle). The value for this
 * field is based on a chunk's `need_read` field in its particular
 *                      H5D_filtered_collective_chunk_info_t structure, but may
 * be adjusted later depending on file space allocation timing and other
 * factors.
 *
 *                      This field can be helpful to avoid needing to scan
 * through the list of chunk info structures to determine how big of I/O vectors
 * to allocate during read operations, as an example.
 *
 */
...
static herr_t
H5D__multi_chunk_filtered_collective_io(H5D_io_info_t *io_info,
                                        H5D_dset_io_info_t *dset_info,
                                        int mpi_rank, int mpi_size);
static herr_t H5D__link_piece_collective_io(H5D_io_info_t *io_info,
                                            int mpi_rank);
static herr_t
H5D__link_chunk_filtered_collective_io(H5D_io_info_t *io_info,
                                       H5D_dset_io_info_t *dset_info,
                                       int mpi_rank, int mpi_size);
...
static herr_t H5D__final_collective_io(H5D_io_info_t *io_info,
                                       ...
                                       MPI_Datatype mpi_file_type,
                                       MPI_Datatype mpi_buf_type);
static herr_t H5D__obtain_mpio_mode(H5D_io_info_t *io_info,
                                    H5D_dset_io_info_t *di,
                                    uint8_t assign_io_mode[],
                                    haddr_t chunk_addr[], int mpi_rank,
                                    int mpi_size);
static herr_t H5D__mpio_get_sum_chunk(const H5D_io_info_t *io_info,
                                      int *sum_chunkf);
static herr_t H5D__mpio_get_sum_chunk_dset(const H5D_io_info_t *io_info,
                                           const H5D_dset_io_info_t *dset_info,
                                           int *sum_chunkf);
static herr_t H5D__mpio_collective_filtered_chunk_io_setup(
    const H5D_io_info_t *io_info, const H5D_dset_io_info_t *di, int mpi_rank,
    H5D_filtered_collective_io_info_t *chunk_list);
static herr_t H5D__mpio_redistribute_shared_chunks(
    H5D_filtered_collective_io_info_t *chunk_list, const H5D_io_info_t *io_info,
    int mpi_rank, int mpi_size, size_t **rank_chunks_assigned_map);
static herr_t H5D__mpio_redistribute_shared_chunks_int(
    H5D_filtered_collective_io_info_t *chunk_list,
    size_t *num_chunks_assigned_map, hbool_t all_ranks_involved,
    const H5D_io_info_t *io_info, int mpi_rank, int mpi_size);
static herr_t H5D__mpio_share_chunk_modification_data(
    H5D_filtered_collective_io_info_t *chunk_list, H5D_io_info_t *io_info,
...
static herr_t H5D__mpio_collective_filtered_chunk_read(
    H5D_filtered_collective_io_info_t *chunk_list, const H5D_io_info_t *io_info,
    const H5D_dset_io_info_t *di, int mpi_rank);
static herr_t H5D__mpio_collective_filtered_chunk_update(
    H5D_filtered_collective_io_info_t *chunk_list,
...
static herr_t H5D__mpio_collective_filtered_chunk_reallocate(
    H5D_filtered_collective_io_info_t *chunk_list,
    size_t *num_chunks_assigned_map, H5D_io_info_t *io_info,
    H5D_chk_idx_info_t *idx_info, int mpi_rank, int mpi_size);
static herr_t H5D__mpio_collective_filtered_chunk_reinsert(
    H5D_filtered_collective_io_info_t *chunk_list,
    size_t *num_chunks_assigned_map, H5D_io_info_t *io_info,
    H5D_dset_io_info_t *di, H5D_chk_idx_info_t *idx_info, int mpi_rank,
    int mpi_size);
static herr_t H5D__mpio_get_chunk_redistribute_info_types(
    MPI_Datatype *contig_type, hbool_t *contig_type_derived,
    MPI_Datatype *resized_type, hbool_t *resized_type_derived);
static herr_t H5D__mpio_get_chunk_alloc_info_types(
    MPI_Datatype *contig_type, hbool_t *contig_type_derived,
    MPI_Datatype *resized_type, hbool_t *resized_type_derived);
static herr_t H5D__mpio_get_chunk_insert_info_types(
    MPI_Datatype *contig_type, hbool_t *contig_type_derived,
    MPI_Datatype *resized_type, hbool_t *resized_type_derived);
static herr_t H5D__mpio_collective_filtered_vec_io(
    const H5D_filtered_collective_io_info_t *chunk_list, H5F_shared_t *f_sh,
    H5D_io_op_type_t op_type);
static int H5D__cmp_piece_addr(const void *chunk_addr_info1,
                               const void *chunk_addr_info2);
static int H5D__cmp_filtered_collective_io_info_entry(
    const void *filtered_collective_io_info_entry1,
    const void *filtered_collective_io_info_entry2);
static int H5D__cmp_chunk_redistribute_info(const void *entry1,
                                            const void *entry2);
static int H5D__cmp_chunk_redistribute_info_orig_owner(const void *entry1,
                                                       const void *entry2);

#ifdef H5Dmpio_DEBUG
static herr_t H5D__mpio_debug_init(void);
static herr_t H5D__mpio_dump_collective_filtered_chunk_list(
    H5D_filtered_collective_io_info_t *chunk_list, int mpi_rank);
#endif
...
#define H5D_MPIO_DEBUG_THIS_RANK(rank)                                         \
  (H5D_mpio_debug_rank_s[0] < 0 || rank == H5D_mpio_debug_rank_s[0] ||         \
   rank == H5D_mpio_debug_rank_s[1] || rank == H5D_mpio_debug_rank_s[2] ||     \
   rank == H5D_mpio_debug_rank_s[3] || rank == H5D_mpio_debug_rank_s[4] ||     \
   rank == H5D_mpio_debug_rank_s[5] || rank == H5D_mpio_debug_rank_s[6] ||     \
...
#define H5D_MPIO_TIME_STOP(rank)                                               \
  if (time_flag) {                                                             \
    end_time = MPI_Wtime();                                                    \
    H5D_MPIO_DEBUG_VA(rank, "'%s' took %f seconds", op,                        \
                      (end_time - start_time));                                \
  }                                                                            \
  }
...
htri_t H5D__mpio_opt_possible(H5D_io_info_t *io_info) {
  ...
  unsigned local_cause[2] = {
      0, 0}; /* [0] Local reason(s) for breaking collective mode */
             /* [1] Flag if dataset is both: H5S_ALL and small */
  unsigned global_cause[2] = {
      0, 0};               /* Global reason(s) for breaking collective mode */
  htri_t is_vl_storage;    /* Whether the dataset's datatype is stored in a
  ...
  for (i = 0; i < io_info->count; i++) {
     ...
     * If the selection I/O mode is auto (default), disable collective for now
     * and re-enable later if we can */
    if (io_info->use_select_io != H5D_SELECTION_IO_MODE_ON) {
      /* Don't allow collective operations if datatype conversions need to
       * happen */
      if (!type_info->is_conv_noop)
        local_cause[0] |= H5D_MPIO_DATATYPE_CONVERSION;

      /* Don't allow collective operations if data transform operations should
       * occur */
    ...
    if (dset->shared->dcpl_cache.efl.nused > 0)
       ...
       *  storage.  For contiguous storage, mem_space and file_space won't
       * change when it it is doing disk IO.  For chunking storage, mem_space
       * will change for different chunks. So for chunking storage, whether we
       * can use collective IO will defer until each chunk IO is reached.
       */
    ...
    /* Check if we would be able to perform collective if we could use selection
     * I/O.  If so add reasons for not using selection I/O to local_cause[0] */
    if ((io_info->use_select_io == H5D_SELECTION_IO_MODE_OFF) &&
        local_cause[0] &&
        !(local_cause[0] & ~((unsigned)H5D_MPIO_DATATYPE_CONVERSION |
  ...
  if (local_cause[0] & H5D_MPIO_SET_INDEPENDENT)
     ...
     */
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Allreduce(local_cause, global_cause, 2, MPI_UNSIGNED,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__mpio_get_no_coll_cause_strings(char *local_cause,
                                           size_t local_cause_len,
                                           char *global_cause,
  ...
  assert((local_cause && local_cause_len > 0) ||
         (global_cause && global_cause_len > 0));

  /*
   * Use compile-time assertion so this routine is updated
   * when any new "no collective cause" values are added
   */
  HDcompile_assert(H5D_MPIO_NO_COLLECTIVE_MAX_CAUSE ==
                   (H5D_mpio_no_collective_cause_t)0x200);
  ...
  for (int bit_pos = 0; bit_pos < nbits; bit_pos++) {
    ...
    case H5D_MPIO_NO_SELECTION_IO:
      cause_str =
          "collective I/O may be supported by selection or vector I/O but that "
          "feature was "
...
herr_t H5D__mpio_select_read(const H5D_io_info_t *io_info,
                             hsize_t mpi_buf_count,
                             H5S_t H5_ATTR_UNUSED *file_space,
  ...
  if (H5F_shared_block_read(io_info->f_sh, H5FD_MEM_DRAW, io_info->store_faddr,
                            (size_t)mpi_buf_count, rbuf) < 0)
    HGOTO_ERROR(H5E_IO, H5E_READERROR, FAIL,
                "can't finish collective parallel read");
...
herr_t H5D__mpio_select_write(const H5D_io_info_t *io_info,
  ...
  /*OKAY: CAST DISCARDS CONST QUALIFIER*/
  H5_CHECK_OVERFLOW(mpi_buf_count, hsize_t, size_t);
  if (H5F_shared_block_write(io_info->f_sh, H5FD_MEM_DRAW, io_info->store_faddr,
                             (size_t)mpi_buf_count, wbuf) < 0)
    HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                "can't finish collective parallel write");
...
static herr_t H5D__mpio_get_sum_chunk(const H5D_io_info_t *io_info,
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Allreduce(&num_chunkf, sum_chunkf, 1, MPI_INT, MPI_SUM,
                                io_info->comm)))
    HMPI_GOTO_ERROR(FAIL, "MPI_Allreduce failed", mpi_code)
...
static herr_t H5D__mpio_get_sum_chunk_dset(const H5D_io_info_t *io_info,
  ...
  num_chunkf = 0;
  ori_num_chunkf =
      H5SL_count(dset_info->layout_io_info.chunk_map->dset_sel_pieces);
  H5_CHECKED_ASSIGN(num_chunkf, int, ori_num_chunkf, size_t);
 ...
 *                    a) One collective IO defined by one MPI derived datatype
 *to link through all chunks or    b) multiple chunk IOs,to do MPI-IO for each
 *chunk, the IO mode may be adjusted due to the selection pattern for each
 *chunk. For option a)
 *                      1. Sort the chunk address, obtain chunk info according
 *to the sorted chunk address
 ...
 *                      5. Do IO
 *              For option b)
 *                      1. Use MPI_gather and MPI_Bcast to obtain information of
 **collective/independent/none* IO mode for each chunk of the selection
 *                      2. Depending on whether the IO mode is collective or
 *independent or none, Create either MPI derived datatype for each chunk to do
 *collective IO or just do independent IO or independent IO with file set view
 *                      3. Set up collective IO property list for collective
 *mode
 *                      4. DO IO
 *
 * Return:      Non-negative on success/Negative on failure
...
static herr_t H5D__piece_io(H5D_io_info_t *io_info) {
  ...
  unsigned one_link_chunk_io_threshold; /* Threshold to use single collective
                                           I/O for all chunks */
  int sum_chunk = -1;
  int mpi_rank;
  int mpi_size;
  ...
  if (!H5D_mpio_debug_inited && H5D__mpio_debug_init() < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "can't initialize H5Dmpio debugging");
  ...
  if (log_file_flag) {
    ...
    if (NULL == (debug_log_file = fopen(debug_log_filename, "a")))
      HGOTO_ERROR(H5E_IO, H5E_OPENERROR, FAIL,
                  "couldn't open debugging log file");
  ...
  /* Check the optional property list for the collective chunk IO optimization
   * option. Only set here if it's a static option, if it needs to be calculated
   * using the
   * number of chunks per process delay that calculation until later. */
  if (H5CX_get_mpio_chunk_opt_mode(&chunk_opt_mode) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "couldn't get chunk optimization option");
  ...
  if (io_info->count > 1 &&
      (io_option == H5D_ONE_LINK_CHUNK_IO || recalc_io_option)) {
    ...
    if (recalc_io_option && use_multi_dset) {
      ...
      if (one_link_chunk_io_threshold > 0) {
        ...
        if (H5D__mpio_get_sum_chunk(io_info, &sum_chunk) < 0)
          HGOTO_ERROR(
              H5E_DATASPACE, H5E_CANTSWAP, FAIL,
              "unable to obtain the total chunk number of all processes");

        /* If the average number of chunk per process is less than the
         * threshold, we will do multi chunk IO.  If this threshold is not
         * exceeded for all datasets, no need to check it again for each
         * individual dataset. */
        if ((unsigned)sum_chunk / (unsigned)mpi_size <
    ...
    for (i = 0; i < io_info->count; i++) {
      if (io_info->dsets_info[i].layout->type == H5D_CONTIGUOUS) {
        ...
        if (H5D__inter_collective_io(io_info, &io_info->dsets_info[i],
          ...
          HGOTO_ERROR(H5E_IO, H5E_CANTGET, FAIL,
                      "couldn't finish shared collective MPI-IO");

        /* Set the actual I/O mode property. internal_collective_io will not
         * break to independent I/O, so we set it here.
         */
        ...
        if (recalc_io_option) {
          ...
          if (one_link_chunk_io_threshold == 0) {
            ...
            /* Get number of chunks for all processes */
            if (H5D__mpio_get_sum_chunk_dset(io_info, &io_info->dsets_info[i],
                                             &sum_chunk) < 0)
              HGOTO_ERROR(
                  H5E_DATASPACE, H5E_CANTSWAP, FAIL,
                  "unable to obtain the total chunk number of all processes");
            ...
            /* If the average number of chunk per process is greater than a
             * threshold, we will do one link chunked IO. */
            if ((unsigned)sum_chunk / (unsigned)mpi_size >=
                one_link_chunk_io_threshold)
              io_option = H5D_ONE_LINK_CHUNK_IO_MORE_OPT;
            else
        ...
        case H5D_ONE_LINK_CHUNK_IO_MORE_OPT:
          ...
          if (io_info->dsets_info[i].dset->shared->dcpl_cache.pline.nused > 0) {
            if (H5D__link_chunk_filtered_collective_io(
                    io_info, &io_info->dsets_info[i], mpi_rank, mpi_size) < 0)
              HGOTO_ERROR(H5E_IO, H5E_CANTGET, FAIL,
                          "couldn't finish filtered linked chunk MPI-IO");
          } /* end if */
          else {
            /* If there is more than one dataset we cannot make the multi
             * dataset call here, fall back to multi chunk */
            if (io_info->count > 1) {
              io_option = H5D_MULTI_CHUNK_IO_MORE_OPT;
              recalc_io_option = TRUE;

              if (H5D__multi_chunk_collective_io(
                      io_info, &io_info->dsets_info[i], mpi_rank, mpi_size) < 0)
                HGOTO_ERROR(H5E_IO, H5E_CANTGET, FAIL,
        ...
        default:                 /* multiple chunk IO via threshold */
          /* Check if there are any filters in the pipeline */
          if (io_info->dsets_info[i].dset->shared->dcpl_cache.pline.nused > 0) {
            if (H5D__multi_chunk_filtered_collective_io(
                    io_info, &io_info->dsets_info[i], mpi_rank, mpi_size) < 0)
              HGOTO_ERROR(
                  H5E_IO, H5E_CANTGET, FAIL,
                  "couldn't finish optimized multiple filtered chunk MPI-IO");
          } /* end if */
          else {
            /* Perform unfiltered multi chunk collective IO */
            if (H5D__multi_chunk_collective_io(io_info, &io_info->dsets_info[i],
                                               mpi_rank, mpi_size) < 0)
              HGOTO_ERROR(H5E_IO, H5E_CANTGET, FAIL,
                          "couldn't finish optimized multiple chunk MPI-IO");
          }
  ...
  if (debug_log_file) {
    ...
    if (EOF == fclose(debug_log_file))
      HDONE_ERROR(H5E_IO, H5E_CLOSEERROR, FAIL,
                  "couldn't close debugging log file");
    debug_stream = H5DEBUG(D);
  }
#endif
...
#else
H5D__link_piece_collective_io(H5D_io_info_t *io_info,
                              int H5_ATTR_UNUSED mpi_rank)
#endif
{
  MPI_Datatype chunk_final_mtype; /* Final memory MPI datatype for all chunks
                                     with selection */
  hbool_t chunk_final_mtype_is_derived = FALSE;
  MPI_Datatype chunk_final_ftype; /* Final file MPI datatype for all chunks with
                                     selection */
  ...
  int *chunk_mpi_file_counts =
      NULL; /* Count of MPI file datatype for each chunk */
  int *chunk_mpi_mem_counts =
      NULL;     /* Count of MPI memory datatype for each chunk */
  int mpi_code; /* MPI return code */
  ...
  /* Code block for actual actions (Build a MPI Type, IO) */
  {
    ...
    if (num_chunk) {
      ...
      if (need_sort)
        qsort(io_info->sel_pieces, io_info->pieces_added,
              sizeof(io_info->sel_pieces[0]), H5D__cmp_piece_addr);

      /* Allocate chunking information */
      if (NULL == (chunk_mtype = (MPI_Datatype *)H5MM_malloc(
                       num_chunk * sizeof(MPI_Datatype))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate chunk memory datatype buffer");
      if (NULL == (chunk_ftype = (MPI_Datatype *)H5MM_malloc(
                       num_chunk * sizeof(MPI_Datatype))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate chunk file datatype buffer");
      if (NULL == (chunk_file_disp_array =
                       (MPI_Aint *)H5MM_malloc(num_chunk * sizeof(MPI_Aint))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate chunk file displacement buffer");
      if (NULL == (chunk_mem_disp_array =
                       (MPI_Aint *)H5MM_calloc(num_chunk * sizeof(MPI_Aint))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate chunk memory displacement buffer");
      if (NULL ==
          (chunk_mpi_mem_counts = (int *)H5MM_calloc(num_chunk * sizeof(int))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate chunk memory counts buffer");
      if (NULL ==
          (chunk_mpi_file_counts = (int *)H5MM_calloc(num_chunk * sizeof(int))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "couldn't allocate chunk file counts buffer");
      if (NULL == (chunk_mbt_is_derived_array =
                       (hbool_t *)H5MM_calloc(num_chunk * sizeof(hbool_t))))
        HGOTO_ERROR(
            H5E_DATASET, H5E_CANTALLOC, FAIL,
            "couldn't allocate chunk memory is derived datatype flags buffer");
      if (NULL == (chunk_mft_is_derived_array =
                       (hbool_t *)H5MM_calloc(num_chunk * sizeof(hbool_t))))
        HGOTO_ERROR(
            H5E_DATASET, H5E_CANTALLOC, FAIL,
      ...
      for (i = 0; i < num_chunk; i++) {
        hsize_t *permute_map =
            NULL; /* array that holds the mapping from the old,
        ...
        /* NOTE: The permute_map array can be allocated within
         * H5S_mpio_space_type and will be fed into the next call to
         * H5S_mpio_space_type where it will be freed.
         */
        ...
        chunk_file_disp_array[i] =
            (MPI_Aint)piece_info->faddr - (MPI_Aint)ctg_store.contig.dset_addr;

        if (io_info->op_type == H5D_IO_OP_WRITE) {
          chunk_mem_disp_array[i] = (MPI_Aint)piece_info->dset_info->buf.cvp -
                                    (MPI_Aint)base_buf_addr.cvp;
        } else if (io_info->op_type == H5D_IO_OP_READ) {
          chunk_mem_disp_array[i] = (MPI_Aint)piece_info->dset_info->buf.vp -
                                    (MPI_Aint)base_buf_addr.vp;
        }
      } /* end for */
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Type_create_struct(
               (int)num_chunk, chunk_mpi_file_counts, chunk_file_disp_array,
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Type_create_struct(
               (int)num_chunk, chunk_mpi_mem_counts, chunk_mem_disp_array,
    ...
    if (H5D__final_collective_io(io_info, mpi_buf_count, chunk_final_ftype,
                                 chunk_final_mtype) < 0)
  ...
  H5D_MPIO_DEBUG_VA(
      mpi_rank,
      "before freeing memory inside H5D_link_collective_io ret_value = %d",
      ret_value);
#endif
  ...
  if (chunk_final_mtype_is_derived &&
      MPI_SUCCESS != (mpi_code = MPI_Type_free(&chunk_final_mtype)))
    HMPI_DONE_ERROR(FAIL, "MPI_Type_free failed", mpi_code)
  if (chunk_final_ftype_is_derived &&
      MPI_SUCCESS != (mpi_code = MPI_Type_free(&chunk_final_ftype)))
    HMPI_DONE_ERROR(FAIL, "MPI_Type_free failed", mpi_code)
...
H5D__link_chunk_filtered_collective_io(H5D_io_info_t *io_info,
  ...
  H5D_MPIO_DEBUG_VA(
      mpi_rank, "Performing Linked-chunk I/O (%s) with MPI Comm size of %d",
  ...
  /* Set the actual-io-mode property.
   * Link chunk filtered I/O does not break to independent, so can set right
   * away
   */
  ...
  if (H5D__mpio_collective_filtered_chunk_io_setup(io_info, dset_info, mpi_rank,
                                                   &chunk_list) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "couldn't construct filtered I/O info list");

  if (io_info->op_type == H5D_IO_OP_READ) { /* Filtered collective read */
    if (H5D__mpio_collective_filtered_chunk_read(&chunk_list, io_info,
                                                 dset_info, mpi_rank) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                  "couldn't read filtered chunks");
    ...
    if (mpi_size > 1) {
      ...
      if (H5D__mpio_redistribute_shared_chunks(&chunk_list, io_info, mpi_rank,
                                               mpi_size,
                                               &rank_chunks_assigned_map) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                    "unable to redistribute shared chunks");

      /* Send any chunk modification messages for chunks this rank no longer
       * owns */
      if (H5D__mpio_share_chunk_modification_data(
              &chunk_list, io_info, dset_info, mpi_rank, mpi_size,
     ...
     * must participate.
     */
    if (H5D__mpio_collective_filtered_chunk_update(&chunk_list, chunk_msg_bufs,
                                                   chunk_msg_bufs_len, io_info,
                                                   dset_info, mpi_rank) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                  "couldn't update modified chunks");

    /* Free up resources used by chunk hash table now that we're done updating
     * chunks */
    HASH_CLEAR(hh, chunk_list.chunk_hash_table);

    /* All ranks now collectively re-allocate file space for all chunks */
    if (H5D__mpio_collective_filtered_chunk_reallocate(
            &chunk_list, rank_chunks_assigned_map, io_info, &index_info,
            mpi_rank, mpi_size) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                  "couldn't collectively re-allocate file space for chunks");

    /* Perform vector I/O on chunks */
    if (H5D__mpio_collective_filtered_vec_io(&chunk_list, io_info->f_sh,
                                             io_info->op_type) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
     ...
     */
    if (H5D__mpio_collective_filtered_chunk_reinsert(
            &chunk_list, rank_chunks_assigned_map, io_info, dset_info,
 ...
 *
 * Purpose:     To do IO per chunk according to IO
 *mode(collective/independent/none)
 *
 *              1. Use MPI_gather and MPI_Bcast to obtain IO mode in each
 *chunk(collective/independent/none)
 *              2. Depending on whether the IO mode is collective or independent
 *or none, Create either MPI derived datatype for each chunk or just do
 *independent IO
...
static herr_t H5D__multi_chunk_collective_io(H5D_io_info_t *io_info,
  ...
  H5FD_mpio_collective_opt_t last_coll_opt_mode =
      H5FD_MPIO_COLLECTIVE_IO; /* Last parallel transfer with independent IO or
                                * collective IO with this mode
                                */
  H5FD_mpio_collective_opt_t orig_coll_opt_mode =
      H5FD_MPIO_COLLECTIVE_IO;    /* Original parallel transfer property on
                                     entering this function */
  size_t total_chunk;             /* Total # of chunks in dataset */
  size_t num_chunk;               /* Number of chunks for this process */
  H5SL_node_t *piece_node = NULL; /* Current node in chunk skip list */
  H5D_piece_info_t *next_chunk_info =
      NULL; /* Chunk info for next selected chunk */
  ...
  if (H5CX_get_mpio_coll_opt(&orig_coll_opt_mode) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't get MPI-I/O collective_op property");

  /* Set the actual chunk opt mode property */
  H5CX_set_mpio_actual_chunk_opt(H5D_MPIO_MULTI_CHUNK);

  /* Retrieve total # of chunks in dataset */
  H5_CHECKED_ASSIGN(total_chunk, size_t, dset_info->layout->u.chunk.nchunks,
                    hsize_t);
  ...
  if (H5D__obtain_mpio_mode(io_info, dset_info, chunk_io_option, chunk_addr,
                            mpi_rank, mpi_size) < 0)
  ...
  if (num_chunk) {
    ...
    if (NULL == (piece_node = H5SL_first(
                     dset_info->layout_io_info.chunk_map->dset_sel_pieces)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "couldn't get piece node from skip list");
    if (NULL == (next_chunk_info = (H5D_piece_info_t *)H5SL_item(piece_node)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "couldn't get piece info from skip list");
  }
  ...
  for (u = 0; u < total_chunk; u++) {
    ...
    H5S_t *mspace; /* Dataspace describing selection in memory corresponding to
                      this chunk */

#ifdef H5Dmpio_DEBUG
    H5D_MPIO_DEBUG_VA(mpi_rank, "mpi_rank = %d, chunk index = %zu", mpi_rank,
                      u);
#endif
    ...
    if (num_chunk && next_chunk_info->index == u) {
      ...
      /* Advance next chunk to next node in skip list, if there are more chunks
       * selected */
      if (num_chunk) {
        if (NULL == (piece_node = H5SL_next(piece_node)))
          HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                      "chunk skip list terminated early");
        if (NULL ==
            (next_chunk_info = (H5D_piece_info_t *)H5SL_item(piece_node)))
          HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                      "couldn't get piece info from skip list");
      }
    ...
    if (chunk_io_option[u] == H5D_CHUNK_IO_MODE_COL) {
      ...
      H5D_MPIO_DEBUG_VA(
          mpi_rank,
          "inside collective chunk IO mpi_rank = %d, chunk index = %zu",
          mpi_rank, u);
#endif
      ...
      if (chunk_info) {
         ...
         *
         * Note: H5D_MPIO_COLLECTIVE_MULTI | H5D_MPIO_INDEPENDENT =
         * H5D_MPIO_MIXED to ease switching between to mixed I/O without
         * checking the current value of the property. You can see the
         * definition in H5Ppublic.h
         */
        actual_io_mode = (H5D_mpio_actual_io_mode_t)(actual_io_mode |
      ...
      if (last_coll_opt_mode != H5FD_MPIO_COLLECTIVE_IO) {
        if (H5CX_set_mpio_coll_opt(H5FD_MPIO_COLLECTIVE_IO) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                      "can't switch to collective I/O");
...
#ifdef H5Dmpio_DEBUG
      H5D_MPIO_DEBUG_VA(
          mpi_rank, "inside independent IO mpi_rank = %d, chunk index = %zu",
          mpi_rank, u);
#endif
      ...
      if (last_coll_opt_mode != H5FD_MPIO_INDIVIDUAL_IO) {
        if (H5CX_set_mpio_coll_opt(H5FD_MPIO_INDIVIDUAL_IO) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                      "can't switch to individual I/O");
  ...
  if (H5CX_set_mpio_coll_opt(orig_coll_opt_mode) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "can't reset MPI-I/O collective_op property");
...
H5D__multi_chunk_filtered_collective_io(H5D_io_info_t *io_info,
  ...
  H5D_MPIO_TRACE_ENTER(mpi_rank);
  H5D_MPIO_DEBUG_VA(
      mpi_rank, "Performing Multi-chunk I/O (%s) with MPI Comm size of %d",
      io_info->op_type == H5D_IO_OP_WRITE ? "write" : "read", mpi_size);
  ...
  if (MPI_SUCCESS != (mpi_code = MPI_Allreduce(
                          &chunk_list.num_chunk_infos, &max_num_chunks, 1,
  ...
  if (io_info->op_type == H5D_IO_OP_READ) { /* Filtered collective read */
    for (size_t i = 0; i < max_num_chunks; i++) {
      ...
      if (have_chunk_to_process) {
        ...
        single_chunk_list.num_chunks_to_read =
            chunk_list.chunk_infos[i].need_read ? 1 : 0;
      } else {
      ...
      if (H5D__mpio_collective_filtered_chunk_read(&single_chunk_list, io_info,
                                                   dset_info, mpi_rank) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                    "couldn't read filtered chunks");
     ...
     * collective re-allocation and re-insertion of chunks modified by other
     * ranks.
     */
    for (size_t i = 0; i < max_num_chunks; i++) {
      H5D_filtered_collective_io_info_t single_chunk_list = {0};
      ...
      /* Check if this rank has a chunk to work on for this iteration */
      have_chunk_to_process = (i < chunk_list.num_chunk_infos) &&
                              (mpi_rank == chunk_list.chunk_infos[i].new_owner);
      ...
      if (H5D__mpio_collective_filtered_chunk_update(
              &single_chunk_list, chunk_msg_bufs, chunk_msg_bufs_len, io_info,
              dset_info, mpi_rank) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                    "couldn't update modified chunks");
      ...
      /* All ranks now collectively re-allocate file space for all chunks */
      if (H5D__mpio_collective_filtered_chunk_reallocate(
              &single_chunk_list, NULL, io_info, &index_info, mpi_rank,
              mpi_size) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                    "couldn't collectively re-allocate file space for chunks");

      /* Perform vector I/O on chunks */
      if (H5D__mpio_collective_filtered_vec_io(
              &single_chunk_list, io_info->f_sh, io_info->op_type) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                    "couldn't perform vector I/O on filtered chunks");
       ...
       */
      if (H5D__mpio_collective_filtered_chunk_reinsert(
              &single_chunk_list, NULL, io_info, dset_info, &index_info,
              mpi_rank, mpi_size) < 0)
        HGOTO_ERROR(
            H5E_DATASET, H5E_WRITEERROR, FAIL,
            "couldn't collectively re-insert modified chunks into chunk index");
 ...
 *
 * Purpose:     Routine for the shared part of collective IO between multiple
 *chunk collective IO and contiguous collective IO
 *
 * Return:      Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
...
static herr_t H5D__inter_collective_io(H5D_io_info_t *io_info,
  ...
  if ((file_space != NULL) && (mem_space != NULL)) {
     ...
     */
    if (H5S_mpio_space_type(file_space, di->type_info.src_type_size,
                            &mpi_file_type, &mpi_file_count,
    ...
    if (is_permuted)
      assert(permute_map);
    if (H5S_mpio_space_type(mem_space, di->type_info.src_type_size,
                            &mpi_buf_type, &mpi_buf_count,
                            ...
                            &is_permuted /* IN */) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
                  "couldn't create MPI buffer type");
    /* Sanity check */
    if (is_permuted)
      assert(!permute_map);
  } /* end if */
  else {
    /* For non-selection, participate with a none MPI derived datatype, the
     * count is 0.  */
  ...
  /* Perform final collective I/O operation */
  if (H5D__final_collective_io(io_info, (hsize_t)mpi_buf_count, mpi_file_type,
                               mpi_buf_type) < 0)
  ...
  /* Free the MPI buf and file types, if they were derived */
  if (mbt_is_derived &&
      MPI_SUCCESS != (mpi_code = MPI_Type_free(&mpi_buf_type)))
    HMPI_DONE_ERROR(FAIL, "MPI_Type_free failed", mpi_code)
  if (mft_is_derived &&
      MPI_SUCCESS != (mpi_code = MPI_Type_free(&mpi_file_type)))
    HMPI_DONE_ERROR(FAIL, "MPI_Type_free failed", mpi_code)

#ifdef H5Dmpio_DEBUG
  H5D_MPIO_TIME_STOP(mpi_rank);
  H5D_MPIO_DEBUG_VA(
      mpi_rank, "before leaving inter_collective_io ret_value = %d", ret_value);
  H5D_MPIO_TRACE_EXIT(mpi_rank);
#endif
 ...
 *
 * Purpose:     Routine for the common part of collective IO with different
 *storages.
 *
 * Return:      Non-negative on success/Negative on failure
 *
...
static herr_t H5D__final_collective_io(H5D_io_info_t *io_info,
  ...
  if (H5CX_set_mpi_coll_datatypes(mpi_buf_type, mpi_file_type) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "can't set MPI-I/O collective I/O datatypes");

  if (io_info->op_type == H5D_IO_OP_WRITE) {
    if ((io_info->md_io_ops.single_write_md)(io_info, mpi_buf_count, NULL,
                                             NULL) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL, "optimized write failed");
  } /* end if */
  else {
    if ((io_info->md_io_ops.single_read_md)(io_info, mpi_buf_count, NULL,
                                            NULL) < 0)
...
#ifdef H5Dmpio_DEBUG
  H5D_MPIO_TIME_STOP(mpi_rank);
  H5D_MPIO_DEBUG_VA(mpi_rank, "ret_value before leaving final_collective_io=%d",
                    ret_value);
  H5D_MPIO_TRACE_EXIT(mpi_rank);
#endif
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__cmp_piece_addr(const void *piece_info1,
                               const void *piece_info2) {
  haddr_t addr1;
  haddr_t addr2;
...
static int H5D__cmp_filtered_collective_io_info_entry(
  ...
  entry1 = (const H5D_filtered_collective_chunk_info_t *)
      filtered_collective_io_info_entry1;
  entry2 = (const H5D_filtered_collective_chunk_info_t *)
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__cmp_chunk_redistribute_info(const void *_entry1,
                                            const void *_entry2) {
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__cmp_chunk_redistribute_info_orig_owner(const void *_entry1,
                                                       const void *_entry2) {
  const H5D_chunk_redistribute_info_t *entry1;
  const H5D_chunk_redistribute_info_t *entry2;
  int owner1 = -1;
 ...
 *
 * Purpose:     Routine to obtain each io mode(collective,independent or none)
 *for each chunk; Each chunk address is also obtained.
 *
 * Description:
 *
 *              1) Each process provides two piece of information for all chunks
 *having selection a) chunk index b) whether this chunk is regular(for MPI
 *derived datatype not working case)
 *
 ...
 *                 b) With the consideration of the user option, calculate IO
 *mode for each chunk c) Build MPI derived datatype to combine "chunk address"
 *and "assign_io" information in order to do MPI Bcast only once d) MPI Bcast
 *the IO mode and chunk address information for each chunk. 4) Each process then
 *retrieves IO mode and chunk address information to assign_io_mode and
 *chunk_addr.
 *
 * Parameters:
 *
 ...
 *              Output: uint8_t assign_io_mode[], : IO mode, collective,
 *independent or none haddr_t chunk_addr[],     : chunk address array for each
 *chunk
 *
 * Return:      Non-negative on success/Negative on failure
 *
...
static herr_t H5D__obtain_mpio_mode(H5D_io_info_t *io_info,
  ...
  if (H5CX_get_mpio_chunk_opt_ratio(&percent_nproc_per_chunk) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "couldn't get percent nproc per chunk");
  ...
  threshold_nproc_per_chunk =
      (unsigned)mpi_size * percent_nproc_per_chunk / 100;

  /* Allocate memory */
  if (NULL == (io_mode_info = (uint8_t *)H5MM_calloc(total_chunks)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate I/O mode info buffer");
  if (NULL ==
      (mergebuf = (uint8_t *)H5MM_malloc((sizeof(haddr_t) + 1) * total_chunks)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate mergebuf buffer");
  tempbuf = mergebuf + total_chunks;
  if (mpi_rank == root)
    if (NULL == (recv_io_mode_info =
                     (uint8_t *)H5MM_malloc(total_chunks * (size_t)mpi_size)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate recv I/O mode info buffer");

  /* Obtain the regularity and selection information for all chunks in this
   * process. */
  chunk_node = H5SL_first(di->layout_io_info.chunk_map->dset_sel_pieces);
  while (chunk_node) {
    chunk_info = (H5D_piece_info_t *)H5SL_item(chunk_node);

    io_mode_info[chunk_info->index] =
        H5D_CHUNK_SELECT_REG; /* this chunk is selected and is "regular" */
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Gather(io_mode_info, (int)total_chunks, MPI_BYTE,
                             recv_io_mode_info, (int)total_chunks, MPI_BYTE,
                             root, comm)))
  ...
  if (mpi_rank == root) {
    ...
    if (H5F_get_coll_metadata_reads(di->dset->oloc.file)) {
      ...
      H5F_set_coll_metadata_reads(di->dset->oloc.file, &md_reads_file_flag,
    ...
    if (NULL == (nproc_per_chunk =
                     (unsigned *)H5MM_calloc(total_chunks * sizeof(unsigned))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate nproc_per_chunk buffer");
    ...
    /* checking for number of process per chunk and regularity of the
     * selection*/
    for (nproc = 0; nproc < (size_t)mpi_size; nproc++) {
      uint8_t *tmp_recv_io_mode_info =
          recv_io_mode_info + (nproc * total_chunks);

      /* Calculate the number of process per chunk and adding irregular
       * selection option */
  ...
  if (MPI_SUCCESS != (mpi_code = MPI_Bcast(
                          mergebuf, (int)((sizeof(haddr_t) + 1) * total_chunks),
                          MPI_BYTE, root, comm)))
...
static herr_t H5D__mpio_collective_filtered_chunk_io_setup(
  ...
  if ((num_chunks_selected =
           H5SL_count(di->layout_io_info.chunk_map->dset_sel_pieces))) {
    H5D_piece_info_t *chunk_info;
    H5SL_node_t *chunk_node;
    ...
    filter_partial_edge_chunks =
        !(di->dset->shared->layout.u.chunk.flags &
          H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS);

    if (NULL == (local_info_array = H5MM_malloc(num_chunks_selected *
                                                sizeof(*local_info_array))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate local io info array buffer");
    ...
    for (size_t i = 0; chunk_node; i++) {
      ...
      local_info_array[i].io_size =
          (size_t)select_npoints * di->type_info.dst_type_size;

      /*
       * Determine whether this chunk will need to be read from the file. If
       * this is a read operation, the chunk will be read. If this is a write
       * operation, we generally need to read a filtered chunk from the file
       * before modifying it, unless the chunk is being fully overwritten.
       *
       * TODO: Currently the full overwrite status of a chunk is only obtained
       * on a per-rank basis. This means that if the total selection in the
       * chunk, as determined by the combination of selections of all of the
       * ranks interested in the chunk, covers the entire chunk, the performance
       * optimization of not reading the chunk from the file is still valid, but
       * is not applied in the current implementation.
       *
       * To implement this case, a few approaches were considered:
       *
       *  - Keep a running total (distributed to each rank) of the number of
       * chunk elements selected during chunk redistribution and compare that to
       * the total number of elements in the chunk once redistribution is
       * finished
       *
       *  - Process all incoming chunk messages before doing I/O (these are
       * currently processed AFTER doing I/O), combine the owning rank's
       * selection in a chunk with the selections received from other ranks and
       * check to see whether that combined selection covers the entire chunk
       *
       * The first approach will be dangerous if the application performs an
       * overlapping write to a chunk, as the number of selected elements can
       * equal or exceed the number of elements in the chunk without the whole
       * chunk selection being covered. While it might be considered erroneous
       * for an application to do an overlapping write, we don't explicitly
       * disallow it.
       *
       * The second approach contains a bit of complexity in that part of the
       * chunk messages will be needed before doing I/O and part will be needed
       * after doing I/O. Since modification data from chunk messages can't be
       * applied until after any I/O is performed (otherwise, we'll overwrite
       * any applied modification data), chunk messages are currently entirely
       * processed after I/O. However, in order to determine if a chunk is being
       * fully overwritten, we need the dataspace portion of the chunk messages
       * before doing I/O. The naive way to do this is to process chunk messages
       * twice, using just the relevant information from the message before and
       * after I/O. The better way would be to avoid processing chunk messages
       * twice by extracting (and keeping around) the dataspace portion of the
       * message before I/O and processing the rest of the chunk message after
       * I/O. Note that the dataspace portion of each chunk message is used to
       * correctly apply chunk modification data from the message, so must be
       * kept around both before and after I/O in this case.
       */
      if (io_info->op_type == H5D_IO_OP_READ)
        ...
        local_info_array[i].need_read =
            local_info_array[i].io_size <
            (size_t)di->dset->shared->layout.u.chunk.size;
      }
      ...
      if (!filter_partial_edge_chunks) {
        ...
        if (H5D__chunk_is_partial_edge_chunk(
                di->dset->shared->ndims, di->dset->shared->layout.u.chunk.dim,
                chunk_info->scaled, di->dset->shared->curr_dims))
      ...
      if (i) {
        ...
        haddr_t prev_chunk_offset =
            local_info_array[i - 1].chunk_current.offset;

        if (!H5_addr_defined(prev_chunk_offset) ||
            !H5_addr_defined(curr_chunk_offset) ||
     ...
     */
    if (need_sort)
      qsort(local_info_array, num_chunks_selected,
            sizeof(H5D_filtered_collective_chunk_info_t),
...
static herr_t H5D__mpio_redistribute_shared_chunks(
  ...
  assert(io_info);
  assert(mpi_size >
         1); /* No chunk sharing is possible for MPI Comm size of 1 */
   ...
   * MPI communication.
   */
  if (NULL == (num_chunks_map =
                   H5MM_malloc((size_t)mpi_size * sizeof(*num_chunks_map))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "couldn't allocate assigned chunks array");

  /* Perform initial Allgather to determine the collective chunk list size */
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Allgather(&chunk_list->num_chunk_infos, 1,
                                H5_SIZE_T_AS_MPI_TYPE, num_chunks_map, 1,
                                H5_SIZE_T_AS_MPI_TYPE, io_info->comm)))
   ...
   */
  redistribute_on_all_ranks =
      coll_chunk_list_size < H5D_CHUNK_REDISTRIBUTE_THRES;

  if (H5D__mpio_redistribute_shared_chunks_int(chunk_list, num_chunks_map,
                                               redistribute_on_all_ranks,
                                               io_info, mpi_rank, mpi_size) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTREDISTRIBUTE, FAIL,
                "can't redistribute shared chunks");
  ...
  if (rank_chunks_assigned_map) {
    ...
    if (!redistribute_on_all_ranks) {
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Bcast(num_chunks_map, mpi_size, H5_SIZE_T_AS_MPI_TYPE,
                                0, io_info->comm)))
        HMPI_GOTO_ERROR(FAIL, "couldn't broadcast chunk mapping to other ranks",
                        mpi_code)
    }
...
static herr_t H5D__mpio_redistribute_shared_chunks_int(
   ...
   */
  H5_CHECKED_ASSIGN(num_chunks_int, int, num_chunks_assigned_map[mpi_rank],
                    size_t);
  ...
  if (all_ranks_involved || (mpi_rank == 0)) {
     ...
     * (also in rank order).
     */
    if (NULL == (counts_disps_array = H5MM_malloc(
                     2 * (size_t)mpi_size * sizeof(*counts_disps_array)))) {
      ...
      for (i = 0; i < (size_t)mpi_size; i++)
        H5_CHECKED_ASSIGN(counts_ptr[i], int, num_chunks_assigned_map[i],
                          size_t);
  ...
  if (H5D__mpio_get_chunk_redistribute_info_types(
          &packed_type, &packed_type_derived, &struct_type,
          &struct_type_derived) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't create derived datatypes for chunk redistribution info");

  /* Perform gather operation */
  if (H5_mpio_gatherv_alloc(chunk_list->chunk_infos, num_chunks_int,
                            struct_type, counts_ptr, displacements_ptr,
                            packed_type, all_ranks_involved, 0, io_info->comm,
                            mpi_rank, mpi_size, &coll_chunk_list,
  ...
  if (all_ranks_involved || (mpi_rank == 0)) {
    ...
    memset(num_chunks_assigned_map, 0,
           (size_t)mpi_size * sizeof(*num_chunks_assigned_map));

    /* Sort collective chunk list according to chunk index */
    qsort(coll_chunk_list, coll_chunk_list_num_entries,
    ...
    for (i = 0; i < coll_chunk_list_num_entries;) {
      ...
      /* We should never have more writers to a chunk than the number of MPI
       * ranks */
      ...
      for (; set_begin_index < i; set_begin_index++) {
        ...
        entry = &(
            (H5D_chunk_redistribute_info_t *)coll_chunk_list)[set_begin_index];

        entry->new_owner = new_chunk_owner;
        entry->num_writers = num_writers;
      }

      /* Update the number of chunks assigned to the MPI rank that now owns this
       * chunk */
      num_chunks_assigned_map[new_chunk_owner]++;
    }
  ...
  if (all_ranks_involved) {
    ...
    for (i = 0; i < coll_chunk_list_num_entries; i++)
      if (mpi_rank ==
          ((H5D_chunk_redistribute_info_t *)coll_chunk_list)[i].orig_owner)
        break;
    ...
    for (size_t j = 0; j < (size_t)num_chunks_int; j++) {
       ...
       * needs to be updated
       */
      if (chunk_list->chunk_infos[j].need_read &&
          (chunk_list->chunk_infos[j].new_owner != mpi_rank)) {
     ...
     */
    if (MPI_SUCCESS != (mpi_code = MPI_Scatterv(
                            coll_chunk_list, counts_ptr, displacements_ptr,
                            packed_type, chunk_list->chunk_infos,
                            num_chunks_int, struct_type, 0, io_info->comm)))
      HMPI_GOTO_ERROR(FAIL, "unable to scatter shared chunks info buffer",
                      mpi_code)
    ...
    for (i = 0; i < chunk_list->num_chunk_infos; i++) {
      if ((chunk_list->chunk_infos[i].new_owner != mpi_rank) &&
          chunk_list->chunk_infos[i].need_read) {
...
static herr_t H5D__mpio_share_chunk_modification_data(
  ...
  if (chunk_list->num_chunk_infos > 0) {
    ...
    if (NULL == (mem_iter = H5FL_MALLOC(H5S_sel_iter_t)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate dataspace selection iterator");
    ...
    if (NULL == (msg_send_bufs =
                     H5MM_malloc(send_bufs_nalloc * sizeof(*msg_send_bufs))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate chunk modification message buffer array");

    if (NULL == (send_requests =
                     H5MM_malloc(send_bufs_nalloc * sizeof(*send_requests))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate send requests array");
  }
  ...
  for (i = 0, last_assigned_idx = 0; i < chunk_list->num_chunk_infos; i++) {
    ...
    if (mpi_rank == chunk_entry->new_owner) {
      ...
      if (H5S_encode(chunk_info->fspace, &mod_data_p, &space_size) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                    "unable to get encoded dataspace size");
      ...
      if (NULL ==
          (msg_send_bufs[num_send_requests] = H5MM_malloc(mod_data_size)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
      ...
      if (0 == H5D__gather_mem(dset_info->buf.cvp, mem_iter,
                               (size_t)iter_nelmts, mod_data_p))
        HGOTO_ERROR(H5E_IO, H5E_CANTGATHER, FAIL,
                    "couldn't gather from write buffer");
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Issend(
               msg_send_bufs[num_send_requests], (int)mod_data_size, MPI_BYTE,
      ...
      if (num_send_requests == send_bufs_nalloc) {
        ...
        if (NULL ==
            (tmp_alloc = H5MM_realloc(
                 msg_send_bufs, send_bufs_nalloc * sizeof(*msg_send_bufs))))
          HGOTO_ERROR(
              H5E_DATASET, H5E_CANTALLOC, FAIL,
        ...
        if (NULL ==
            (tmp_alloc = H5MM_realloc(
                 send_requests, send_bufs_nalloc * sizeof(*send_requests))))
          HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                      "couldn't resize send requests array");
        send_requests = tmp_alloc;
      }

      if (H5S_SELECT_ITER_RELEASE(mem_iter) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                    "couldn't release memory selection iterator");
      mem_iter_init = FALSE;
    }
  }

  /* Check if the number of send or receive requests will overflow an int (MPI
   * requirement) */
  ...
  if (num_msgs_incoming) {
    if (NULL == (msg_recv_bufs =
                     H5MM_malloc(num_msgs_incoming * sizeof(*msg_recv_bufs))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate chunk modification message buffer array");

    if (NULL == (recv_requests =
                     H5MM_malloc(num_msgs_incoming * sizeof(*recv_requests))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate receive requests array");
  }
  ...
  do {
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Iprobe(MPI_ANY_SOURCE, H5D_CHUNK_MOD_DATA_TAG,
                               io_info->comm, &msg_flag, &status)))
    ...
    if (msg_flag) {
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Get_elements_x(&status, MPI_BYTE, &msg_size)))
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Get_elements(&status, MPI_BYTE, &msg_size)))
        HMPI_GOTO_ERROR(FAIL, "MPI_Get_elements failed", mpi_code)
#endif

      if (msg_size <= 0)
        HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                    "invalid chunk modification message size");

      assert((num_recv_requests + 1) <= num_msgs_incoming);
      if (NULL == (msg_recv_bufs[num_recv_requests] =
                       H5MM_malloc((size_t)msg_size * sizeof(unsigned char))))
        HGOTO_ERROR(
            H5E_DATASET, H5E_CANTALLOC, FAIL,
      ...
      if (MPI_SUCCESS !=
          (mpi_code =
               MPI_Irecv(msg_recv_bufs[num_recv_requests], (int)msg_size,
    ...
    if (ibarrier_posted) {
      ...
      if (MPI_SUCCESS != (mpi_code = MPI_Test(&ibarrier, &ibarrier_completed,
                                              MPI_STATUS_IGNORE)))
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Testall((int)num_send_requests, send_requests,
  ...
  H5_GCC_DIAG_OFF("stringop-overflow")
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Waitall((int)num_recv_requests, recv_requests,
                              MPI_STATUSES_IGNORE)))
  ...
  *chunk_msg_bufs_len = (int)num_recv_requests;
  ...
  if (mem_iter) {
    if (mem_iter_init && H5S_SELECT_ITER_RELEASE(mem_iter) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                  "couldn't release dataspace selection iterator");
  ...
  HERROR(H5E_DATASET, H5E_WRITEERROR,
         "unable to send chunk modification data between MPI ranks - MPI "
         "version < 3 (MPI_Ibarrier missing)")
...
static herr_t H5D__mpio_collective_filtered_chunk_read(
  ...
  hsize_t iter_nelmts; /* Number of points to iterate over for the chunk IO
                          operation */
  ...
  if (chunk_list->num_chunk_infos) {
    ...
    if (H5CX_get_err_detect(&err_detect) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get error detection info");
    ...
    should_fill =
        (di->dset->shared->dcpl_cache.fill.fill_time == H5D_FILL_TIME_ALLOC) ||
        ((di->dset->shared->dcpl_cache.fill.fill_time == H5D_FILL_TIME_IFSET) &&
         di->dset->shared->dcpl_cache.fill.fill_defined);
  }

  /*
   * Allocate memory buffers for all chunks being read. Chunk data buffers are
   * of the largest size between the chunk's current filtered size and the
   * chunk's true size, as calculated by the number of elements in the chunk's
   * file space extent multiplied by the datatype size. This tries to ensure
   * that:
   *
   *  * If we're reading the chunk and the filter normally reduces the chunk
   * size, the unfiltering operation won't need to grow the buffer.
  ...
  for (size_t i = 0; i < chunk_list->num_chunk_infos; i++) {
    ...
    chunk_entry->chunk_buf_size =
        MAX(chunk_entry->chunk_current.length, file_chunk_size);
    ...
    if (NULL == (chunk_entry->buf = H5MM_malloc(chunk_entry->chunk_buf_size))) {
      ...
      HDONE_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate chunk data buffer");
      break;
    }
    ...
    if (H5_addr_defined(chunk_entry->chunk_current.offset)) {
      ...
      if (should_fill) {
        ...
        if (!fb_info_init) {
          ...
          assert(di->dset->shared->ndims ==
                 di->dset->shared->layout.u.chunk.ndims - 1);
          for (size_t j = 0; j < di->dset->shared->layout.u.chunk.ndims - 1;
               j++)
        ...
        if (H5D__fill(fb_info.fill_buf, di->dset->shared->type,
                      chunk_entry->buf, di->type_info.mem_type, fill_space) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
  ...
  if (di->dset->shared->dcpl_cache.fill.alloc_time == H5D_ALLOC_TIME_INCR)
    if (H5D__chunk_index_empty(di->dset, &index_empty) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "couldn't determine if chunk index is empty");
  ...
  for (size_t i = 0; i < chunk_list->num_chunk_infos; i++) {
    ...
    if (chunk_entry->need_read && !chunk_entry->skip_filter_pline) {
      if (H5Z_pipeline(&di->dset->shared->dcpl_cache.pline, H5Z_FLAG_REVERSE,
                       &(chunk_entry->index_info.filter_mask), err_detect,
                       filter_cb, (size_t *)&chunk_entry->chunk_new.length,
                       &chunk_entry->chunk_buf_size, &chunk_entry->buf) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTFILTER, FAIL,
                    "couldn't unfilter chunk for modifying");
    }

    /* Scatter the chunk data to the read buffer */
    iter_nelmts = H5S_GET_SELECT_NPOINTS(chunk_info->fspace);

    if (H5D_select_io_mem(di->buf.vp, chunk_info->mspace, chunk_entry->buf,
                          chunk_info->fspace, di->type_info.src_type_size,
                          (size_t)iter_nelmts) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                  "couldn't copy chunk data to read buffer");
  }
  ...
  if (fb_info_init && H5D__fill_term(&fb_info) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                "Can't release fill buffer info");
...
static herr_t H5D__mpio_collective_filtered_chunk_update(
  ...
  H5S_sel_iter_t *sel_iter =
      NULL;             /* Dataspace selection iterator for H5D__scatter_mem */
  H5Z_EDC_t err_detect; /* Error detection info */
  H5Z_cb_t filter_cb;   /* I/O filter callback function */
  hsize_t file_chunk_size = 0;
  hsize_t iter_nelmts; /* Number of points to iterate over for the chunk IO
  ...
  assert(chunk_list);
  assert((chunk_msg_bufs && chunk_list->chunk_hash_table) ||
         0 == chunk_msg_bufs_len);
  assert(io_info);
  assert(di);
   ...
   * size, as calculated by the number of elements in the chunk's file space
   * extent multiplied by the datatype size. This tries to ensure that:
   *
    ...
    if (chunk_entry->need_read) {
      ...
      if (H5_addr_defined(chunk_entry->chunk_current.offset)) {
        ...
        if (should_fill) {
          ...
          assert(fb_info.fill_buf);
          if (H5D__fill(fb_info.fill_buf, di->dset->shared->type,
                        chunk_entry->buf, type_info->mem_type, fill_space) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                        "couldn't fill chunk buffer with fill value");
        }
      }
  ...
  for (i = 0; i < chunk_list->num_chunk_infos; i++) {
    ...
    if (H5D_select_io_mem(chunk_entry->buf, chunk_info->fspace, di->buf.cvp,
                          chunk_info->mspace, type_info->dst_type_size,
                          (size_t)iter_nelmts) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                  "couldn't copy chunk data to write buffer");
  }

  /* Allocate iterator for memory selection */
  if (NULL == (sel_iter = H5FL_MALLOC(H5S_sel_iter_t)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate memory iterator");
  ...
  for (i = 0; i < (size_t)chunk_msg_bufs_len; i++) {
    ...
    if (msg_ptr) {
      ...
      HASH_FIND(hh, chunk_list->chunk_hash_table, &chunk_idx, sizeof(hsize_t),
                chunk_entry);
      if (chunk_entry == NULL)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTFIND, FAIL,
                    "unable to find chunk entry");
      if (mpi_rank != chunk_entry->new_owner)
        HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                    "chunk owner set to incorrect MPI rank");
      ...
      if (!chunk_entry->buf)
        ...
        if (H5D__scatter_mem(msg_ptr, sel_iter, (size_t)iter_nelmts,
                             chunk_entry->buf) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                      "couldn't scatter to write buffer");

        if (H5S_SELECT_ITER_RELEASE(sel_iter) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                      "couldn't release selection iterator");
        sel_iter_init = FALSE;

        if (dataspace) {
          if (H5S_close(dataspace) < 0)
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTFREE, FAIL,
                        "can't close dataspace");
          dataspace = NULL;
        }
  ...
  for (i = 0; i < chunk_list->num_chunk_infos; i++) {
    if (!chunk_list->chunk_infos[i].skip_filter_pline) {
      if (H5Z_pipeline(&di->dset->shared->dcpl_cache.pline, 0,
                       &(chunk_list->chunk_infos[i].index_info.filter_mask),
                       err_detect, filter_cb,
                       (size_t *)&chunk_list->chunk_infos[i].chunk_new.length,
                       &chunk_list->chunk_infos[i].chunk_buf_size,
                       &chunk_list->chunk_infos[i].buf) < 0)
    ...
    if (chunk_list->chunk_infos[i].chunk_new.length > ((size_t)0xffffffff))
      HGOTO_ERROR(H5E_DATASET, H5E_BADRANGE, FAIL,
                  "chunk too large for 32-bit length");
#endif
  }
...
static herr_t H5D__mpio_collective_filtered_chunk_reallocate(
  ...
  if (H5D__mpio_get_chunk_alloc_info_types(&recv_type, &recv_type_derived,
                                           &send_type, &send_type_derived) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
  ...
  if (num_chunks_assigned_map) {
    ...
    if (H5_mpio_gatherv_alloc(chunk_list->chunk_infos,
                              (int)chunk_list->num_chunk_infos, send_type,
                              counts_ptr, displacements_ptr, recv_type, TRUE, 0,
                              io_info->comm, mpi_rank, mpi_size,
    ...
    if (H5_mpio_gatherv_alloc_simple(
            chunk_list->chunk_infos, (int)chunk_list->num_chunk_infos,
            send_type, recv_type, TRUE, 0, io_info->comm, mpi_rank, mpi_size,
            &gathered_array, &collective_num_entries) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGATHER, FAIL,
                  "can't gather chunk file space info to/from ranks");
  }
  ...
  for (i = 0, num_local_chunks_processed = 0; i < collective_num_entries; i++) {
    ...
    if (H5D__chunk_file_alloc(idx_info, &coll_entry->chunk_current,
    ...
    if (update_local_chunk) {
      ...
      if (num_local_chunks_processed) {
        ...
        haddr_t prev_chunk_offset =
            chunk_list->chunk_infos[num_local_chunks_processed - 1]
                .chunk_new.offset;

        assert(H5_addr_defined(prev_chunk_offset) &&
               H5_addr_defined(curr_chunk_offset));
...
static herr_t H5D__mpio_collective_filtered_chunk_reinsert(
...
#ifdef H5Dmpio_DEBUG
  H5D_MPIO_TRACE_ENTER(mpi_rank);
  H5D_MPIO_TIME_START(mpi_rank,
                      "Reinsertion of modified chunks into chunk index");
#endif
  ...
  if (H5D__mpio_get_chunk_insert_info_types(&recv_type, &recv_type_derived,
                                            &send_type, &send_type_derived) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
  ...
  for (i = 0; i < collective_num_entries; i++) {
    H5D_chunk_insert_info_t *coll_entry =
        &((H5D_chunk_insert_info_t *)gathered_array)[i];
    ...
    if (idx_info->layout->idx_type == H5D_CHUNK_IDX_EARRAY &&
      ...
      H5VM_array_calc_pre(chunk_ud.chunk_idx, di->dset->shared->ndims,
                          idx_info->layout->u.earray.swizzled_down_chunks,
                          scaled_coords);

      H5VM_unswizzle_coords(hsize_t, scaled_coords,
                            idx_info->layout->u.earray.unlim_dim);
    } else {
      H5VM_array_calc_pre(chunk_ud.chunk_idx, di->dset->shared->ndims,
                          di->dset->shared->layout.u.chunk.down_chunks,
                          scaled_coords);
    }
    ...
    for (size_t dbg_idx = 0; dbg_idx < chunk_list->num_chunk_infos; dbg_idx++) {
      if (coll_entry->index_info.chunk_idx ==
          chunk_list->chunk_infos[dbg_idx].index_info.chunk_idx) {
        hbool_t coords_match = !memcmp(
            scaled_coords, chunk_list->chunk_infos[dbg_idx].chunk_info->scaled,
...
static herr_t H5D__mpio_get_chunk_redistribute_info_types(
  ...
  *resized_type_derived = FALSE;
  ...
  if (H5F_mpi_get_file_block_type(FALSE, &chunk_block_type,
                                  &chunk_block_type_derived) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't create derived type for chunk file description");
  ...
  types[4] = MPI_INT;
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Type_create_struct(field_count, block_lengths,
                                         displacements, types, contig_type)))
    HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_struct failed", mpi_code)
  *contig_type_derived = TRUE;
   ...
   * new_owner and num_writers fields from a
   * H5D_filtered_collective_chunk_info_t structure
   */
  block_lengths[0] = 1;
  block_lengths[1] = 1;
  block_lengths[2] = 1;
  ...
  block_lengths[4] = 1;
  displacements[0] =
      offsetof(H5D_filtered_collective_chunk_info_t, chunk_current);
  displacements[1] =
      offsetof(H5D_filtered_collective_chunk_info_t, index_info.chunk_idx);
  ...
  displacements[4] =
      offsetof(H5D_filtered_collective_chunk_info_t, num_writers);
  types[0] = chunk_block_type;
  types[1] = HSIZE_AS_MPI_TYPE;
  types[2] = MPI_INT;
  types[3] = MPI_INT;
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Type_create_struct(field_count, block_lengths,
                                         displacements, types, &struct_type)))
    HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_struct failed", mpi_code)
  struct_type_derived = TRUE;

  if (MPI_SUCCESS !=
      (mpi_code = MPI_Type_create_resized(
           struct_type, 0, sizeof(H5D_filtered_collective_chunk_info_t),
           resized_type)))
...
static herr_t H5D__mpio_get_chunk_insert_info_types(
  ...
  *resized_type_derived = FALSE;
  ...
  contig_type_extent =
      (MPI_Aint)(sizeof(H5F_block_t) + sizeof(H5D_chunk_index_info_t));

  if (MPI_SUCCESS != (mpi_code = MPI_Type_create_resized(
                          struct_type, 0, contig_type_extent, contig_type)))
    HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_resized failed", mpi_code)
  *contig_type_derived = TRUE;
  ...
  displacements[1] =
      offsetof(H5D_filtered_collective_chunk_info_t, index_info.chunk_idx);
  displacements[2] =
      offsetof(H5D_filtered_collective_chunk_info_t, index_info.filter_mask);
  displacements[3] =
      offsetof(H5D_filtered_collective_chunk_info_t, index_info.need_insert);
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Type_create_struct(field_count, block_lengths,
                                         displacements, types, &struct_type)))
    HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_struct failed", mpi_code)
...
static herr_t H5D__mpio_collective_filtered_vec_io(
  ...
  if (iovec_count > 0) {
    if (chunk_list->num_chunk_infos > UINT32_MAX)
      HGOTO_ERROR(
          H5E_INTERNAL, H5E_BADRANGE, FAIL,
    ...
    for (size_t i = 0, vec_idx = 0; i < chunk_list->num_chunk_infos; i++) {
      ...
      chunk_block = (op_type == H5D_IO_OP_READ)
                        ? &chunk_list->chunk_infos[i].chunk_current
  ...
  if (op_type == H5D_IO_OP_WRITE) {
    if (H5F_shared_vector_write(f_sh, iovec_count, io_types, io_addrs, io_sizes,
                                io_wbufs) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                  "vector write call failed");
  } else {
    if (H5F_shared_vector_read(f_sh, iovec_count, io_types, io_addrs, io_sizes,
                               io_rbufs) < 0)
...
static herr_t H5D__mpio_dump_collective_filtered_chunk_list(
  ...
  for (i = 0; i < chunk_list->num_chunk_infos; i++) {
    ...
    chunk_rank =
        (unsigned)H5S_GET_EXTENT_NDIMS(chunk_entry->chunk_info->fspace);
    ...
    H5D_MPIO_DEBUG_VA(mpi_rank,
                      "     Chunk Current Info: { Offset: %" PRIuHADDR
                      ", Length: %" PRIuHADDR " }",
                      chunk_entry->chunk_current.offset,
                      chunk_entry->chunk_current.length);
    H5D_MPIO_DEBUG_VA(mpi_rank,
                      "     Chunk New Info: { Offset: %" PRIuHADDR
                      ", Length: %" PRIuHADDR " }",
    ...
    H5D_MPIO_DEBUG_VA(mpi_rank,
                      "     Chunk Scaled Coords (4-d): { %" PRIuHSIZE
    ...
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Chunk Index: %" PRIuHSIZE,
                      chunk_entry->index_info.chunk_idx);
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Filter Mask: %u",
    ...
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Chunk Info Ptr: %p",
                      (void *)chunk_entry->chunk_info);
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Need Read: %s",
                      chunk_entry->need_read ? "YES" : "NO");
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Chunk I/O Size: %zu",
                      chunk_entry->io_size);
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Chunk Buffer Size: %zu",
                      chunk_entry->chunk_buf_size);
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Original Owner: %d",
                      chunk_entry->orig_owner);
    H5D_MPIO_DEBUG_VA(mpi_rank, "     New Owner: %d", chunk_entry->new_owner);
    H5D_MPIO_DEBUG_VA(mpi_rank, "     # of Writers: %d",
                      chunk_entry->num_writers);
    H5D_MPIO_DEBUG_VA(mpi_rank, "     Chunk Data Buffer Ptr: %p",
                      (void *)chunk_entry->buf);
