    ...
    if (!((HDR)->checked_filters)) {                                           \
      if ((HDR)->pline.nused)                                                  \
        if (H5Z_can_apply_direct(&((HDR)->pline)) < 0)                         \
          HGOTO_ERROR(H5E_ARGS, H5E_CANTINIT, FAIL,                            \
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_insert(H5HF_hdr_t *hdr, size_t obj_size, const void *obj,
                        void *_id) {
  ...
  if ((node_found = H5HF__space_find(hdr, (hsize_t)obj_size, &sec_node)) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL,
                "can't locate free space in fractal heap");

  /* If we didn't find a node, go create a direct block big enough to hold the
   * requested block */
  if (!node_found)
    ...
    if (H5HF__man_dblock_new(hdr, obj_size, &sec_node) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCREATE, FAIL,
                  "can't create fractal heap direct block");
  ...
  if (H5HF__sect_single_dblock_info(hdr, sec_node, &dblock_addr, &dblock_size) <
      0)
  ...
  if (NULL == (dblock = H5HF__man_dblock_protect(
                   hdr, dblock_addr, dblock_size, sec_node->u.single.parent,
  ...
  H5_CHECK_OVERFLOW((sec_node->sect_info.addr - dblock->block_off), hsize_t,
                    size_t);
  ...
  if (H5HF__sect_single_reduce(hdr, sec_node, obj_size) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTSHRINK, FAIL,
                "can't reduce single section node");
  sec_node = NULL;
  ...
  if (H5HF__hdr_adj_free(hdr, -(ssize_t)obj_size) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTDEC, FAIL,
                "can't adjust free space for heap");
  ...
  if (ret_value < 0)
    if (sec_node && H5HF__sect_single_free((H5FS_section_info_t *)sec_node) < 0)
      HDONE_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                  "unable to release section node");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_get_obj_len(H5HF_hdr_t *hdr, const uint8_t *id,
                             size_t *obj_len_p) {
 ...
 *-------------------------------------------------------------------------
 */
void H5HF__man_get_obj_off(const H5HF_hdr_t *hdr, const uint8_t *id,
                           hsize_t *obj_off_p) {
  FUNC_ENTER_PACKAGE_NOERR
...
static herr_t H5HF__man_op_real(H5HF_hdr_t *hdr, const uint8_t *id,
  ...
  if (obj_off > hdr->man_size)
    HGOTO_ERROR(H5E_HEAP, H5E_BADRANGE, FAIL,
                "fractal heap object offset too large");
  if (obj_len == 0)
    HGOTO_ERROR(H5E_HEAP, H5E_BADRANGE, FAIL,
                "invalid fractal heap object size");
  if (obj_len > hdr->man_dtable.cparam.max_direct_size)
    HGOTO_ERROR(H5E_HEAP, H5E_BADRANGE, FAIL,
                "fractal heap object size too large for direct block");
  if (obj_len > hdr->max_man_size)
    HGOTO_ERROR(H5E_HEAP, H5E_BADRANGE, FAIL,
                "fractal heap object should be standalone");
  ...
  if (hdr->man_dtable.curr_root_rows == 0) {
    ...
    /* Lock direct block */
    if (NULL ==
        (dblock = H5HF__man_dblock_protect(hdr, dblock_addr, dblock_size, NULL,
                                           0, dblock_access_flags)))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL,
    ...
    if (H5HF__man_dblock_locate(hdr, obj_off, &iblock, &entry, &did_protect,
    ...
    H5_CHECK_OVERFLOW(
        (hdr->man_dtable.row_block_size[entry / hdr->man_dtable.cparam.width]),
        hsize_t, size_t);
    dblock_size = (size_t)hdr->man_dtable
                      .row_block_size[entry / hdr->man_dtable.cparam.width];

    /* Check for offset of invalid direct block */
    if (!H5_addr_defined(dblock_addr)) {
      /* Unlock indirect block */
      if (H5HF__man_iblock_unprotect(iblock, H5AC__NO_FLAGS_SET, did_protect) <
          0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                    "unable to release fractal heap indirect block");

      HGOTO_ERROR(H5E_HEAP, H5E_BADRANGE, FAIL,
                  "fractal heap ID not in allocated direct block");
    } /* end if */

    /* Lock direct block */
    if (NULL == (dblock = H5HF__man_dblock_protect(hdr, dblock_addr,
                                                   dblock_size, iblock, entry,
                                                   dblock_access_flags))) {
      /* Unlock indirect block */
      if (H5HF__man_iblock_unprotect(iblock, H5AC__NO_FLAGS_SET, did_protect) <
          0)
  ...
  if (op(p, obj_len, op_data) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTOPERATE, FAIL,
                "application's callback failed");

done:
  /* Unlock direct block */
  if (dblock && H5AC_unprotect(hdr->f, H5AC_FHEAP_DBLOCK, dblock_addr, dblock,
                               dblock_cache_flags) < 0)
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                "unable to release fractal heap direct block");
...
herr_t H5HF__man_read(H5HF_hdr_t *hdr, const uint8_t *id, void *obj) {
  ...
  if (H5HF__man_op_real(hdr, id, H5HF__op_read, obj, 0) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTOPERATE, FAIL,
                "unable to operate on heap object");
...
herr_t H5HF__man_write(H5HF_hdr_t *hdr, const uint8_t *id, const void *obj) {
  ...
  if (H5HF__man_op_real(hdr, id, H5HF__op_write, (void *)obj, H5HF_OP_MODIFY) <
      0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTOPERATE, FAIL,
                "unable to operate on heap object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_op(H5HF_hdr_t *hdr, const uint8_t *id, H5HF_operator_t op,
                    void *op_data) {
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5HF__man_remove(H5HF_hdr_t *hdr, const uint8_t *id) {
  H5HF_free_section_t *sec_node =
      NULL;                       /* Pointer to free space section for block */
  ...
  size_t obj_len;           /* Object's length in heap */
  size_t dblock_size;       /* Direct block size */
  hsize_t dblock_block_off; /* Offset of the direct block within the heap's
                               address space */
  ...
  if (hdr->man_dtable.curr_root_rows == 0) {
    ...
    /* Look up indirect block containing direct block */
    if (H5HF__man_dblock_locate(hdr, obj_off, &iblock, &dblock_entry,
                                &did_protect, H5AC__NO_FLAGS_SET) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCOMPUTE, FAIL,
                  "can't compute row & column of section");
    ...
    H5_CHECK_OVERFLOW(
        (hdr->man_dtable
             .row_block_size[dblock_entry / hdr->man_dtable.cparam.width]),
        hsize_t, size_t);
    dblock_size =
        (size_t)(hdr->man_dtable.row_block_size[dblock_entry /
    ...
    dblock_block_off +=
        hdr->man_dtable
            .row_block_off[dblock_entry / hdr->man_dtable.cparam.width];
    dblock_block_off +=
        hdr->man_dtable
            .row_block_size[dblock_entry / hdr->man_dtable.cparam.width] *
  ...
  if (NULL == (sec_node = H5HF__sect_single_new(obj_off, obj_len, iblock,
                                                dblock_entry)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't create section for direct block's free space");
