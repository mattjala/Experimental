...
struct H5P_genclass_t {
  ...
  unsigned plists;  /* Number of property lists that have been created since the
                       last modification to the class */
  ...
  hbool_t deleted;    /* Whether this class has been deleted and is waiting for
                         dependent classes & proplists    to close */
...
struct H5P_genplist_t {
  ...
  size_t nprops;  /* Number of properties in class */
  hbool_t class_init; /* Whether the class initialization callback finished
                         successfully */
...
H5_DLL H5P_genclass_t *
H5P__create_class(H5P_genclass_t *par_class, const char *name,
                  H5P_plist_type_t type, H5P_cls_create_func_t cls_create,
...
H5_DLL herr_t H5P__register_real(
...
H5_DLL int H5P__iterate_plist(const H5P_genplist_t *plist,
                              hbool_t iter_all_prop, int *idx,
                              H5P_iterate_int_t iter_func, void *iter_data);
H5_DLL int H5P__iterate_pclass(const H5P_genclass_t *pclass, int *idx,
                               H5P_iterate_int_t iter_func, void *iter_data);
H5_DLL herr_t H5P__copy_prop_plist(hid_t dst_id, hid_t src_id,
                                   const char *name);
H5_DLL herr_t H5P__copy_prop_pclass(hid_t dst_id, hid_t src_id,
                                    const char *name);
...
H5_DLL herr_t H5P__encode_coll_md_read_flag_t(const void *value, void **_pp,
                                              size_t *size);
