...
static herr_t H5O__add_gap(H5F_t *f, H5O_t *oh, unsigned chunkno,
                           hbool_t *chk_dirtied, size_t idx,
                           uint8_t *new_gap_loc, size_t new_gap_size);
static herr_t H5O__eliminate_gap(H5O_t *oh, hbool_t *chk_dirtied,
                                 H5O_mesg_t *mesg, uint8_t *new_gap_loc,
                                 size_t new_gap_size);
static herr_t H5O__alloc_null(H5F_t *f, H5O_t *oh, size_t null_idx,
                              const H5O_msg_class_t *new_type, void *new_native,
                              size_t new_size);
static htri_t H5O__alloc_extend_chunk(H5F_t *f, H5O_t *oh, unsigned chunkno,
                                      size_t size, size_t *msg_idx);
static herr_t H5O__alloc_find_best_nonnull(const H5F_t *f, const H5O_t *oh,
                                           size_t *size,
                                           H5O_msg_alloc_info_t *found_msg);
static herr_t H5O__alloc_new_chunk(H5F_t *f, H5O_t *oh, size_t size,
                                   size_t *new_idx);
static herr_t H5O__alloc_find_best_null(const H5O_t *oh, size_t size,
                                        size_t *mesg_idx);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__add_gap(H5F_t H5_ATTR_NDEBUG_UNUSED *f, H5O_t *oh,
                           unsigned chunkno, hbool_t *chk_dirtied, size_t idx,
                           uint8_t *new_gap_loc, size_t new_gap_size) {
...
#ifndef NDEBUG
  if (chunkno > 0) {
    unsigned chk_proxy_status =
        0; /* Object header chunk proxy entry cache status */

    /* Check the object header chunk proxy's status in the metadata cache */
    if (H5AC_get_entry_status(f, oh->chunk[chunkno].addr, &chk_proxy_status) <
        0)
  ...
  for (u = 0; u < oh->nmesgs && !merged_with_null; u++) {
    ...
    if (H5O_NULL_ID == oh->mesg[u].type->id && oh->mesg[u].chunkno == chunkno &&
      ...
      if (H5O__eliminate_gap(oh, chk_dirtied, &oh->mesg[u], new_gap_loc,
                             new_gap_size) < 0)
  ...
  if (!merged_with_null) {
    ...
    memmove(new_gap_loc, new_gap_loc + new_gap_size,
            (size_t)((oh->chunk[chunkno].image +
                      (oh->chunk[chunkno].size - H5O_SIZEOF_CHKSUM_OH(oh))) -
 ...
 */
static herr_t H5O__eliminate_gap(H5O_t *oh, hbool_t *chk_dirtied,
                                 H5O_mesg_t *mesg, uint8_t *gap_loc,
                                 size_t gap_size) {
  uint8_t *move_start, *move_end; /* Pointers to area of messages to move */
  hbool_t null_before_gap; /* Flag whether the null message is before the gap or
                              not */
  ...
  if (move_end > move_start) {
    ...
    if (null_before_gap)
      /* Slide messages down */
      memmove(move_start + gap_size, move_start,
              (size_t)(move_end - move_start));
    else {
      /* Slide messages up */
      memmove(move_start - gap_size, move_start,
              (size_t)(move_end - move_start));
    ...
    memmove(move_start - gap_size, move_start,
            mesg->raw_size + (size_t)H5O_SIZEOF_MSGHDR_OH(oh));
...
static herr_t H5O__alloc_null(H5F_t *f, H5O_t *oh, size_t null_idx,
  ...
  if (alloc_msg->raw_size > new_size) {
    ...
    if ((alloc_msg->raw_size - new_size) < (size_t)H5O_SIZEOF_MSGHDR_OH(oh)) {
      size_t gap_size =
          alloc_msg->raw_size - new_size; /* Size of gap produced */
      ...
      size_t new_mesg_size =
          new_size + (size_t)H5O_SIZEOF_MSGHDR_OH(
                         oh); /* Total size of newly allocated message */
      H5O_mesg_t *null_msg;   /* Pointer to new null message */
      ...
      if (oh->nmesgs >= oh->alloc_nmesgs) {
        ...
        /* "Retarget" 'alloc_msg' pointer into newly re-allocated array of
         * messages */
        alloc_msg = &oh->mesg[null_idx];
      } /* end if */
      ...
      if (oh->chunk[null_msg->chunkno].gap > 0) {
        ...
        if (H5O__eliminate_gap(
                oh, &chk_dirtied, null_msg,
                ((oh->chunk[null_chunkno].image +
                  oh->chunk[null_chunkno].size) -
...
herr_t H5O__alloc_msgs(H5O_t *oh, size_t min_alloc) {
  ...
  na =
      oh->alloc_nmesgs + MAX(oh->alloc_nmesgs, min_alloc); /* At least double */
...
static htri_t H5O__alloc_extend_chunk(H5F_t *f, H5O_t *oh, unsigned chunkno,
  ...
  hbool_t adjust_size_flags =
      FALSE;                  /* Whether to adjust the chunk #0 size flags */
  ...
  if (extended_msg) {
    ...
    delta = (aligned_size + (size_t)H5O_SIZEOF_MSGHDR_OH(oh)) -
            oh->chunk[chunkno].gap;
  delta = H5O_ALIGN_OH(oh, delta);

  /* Check for changing the chunk #0 data size enough to need adjusting the
   * flags */
  if (oh->version > H5O_VERSION_1 && chunkno == 0) {
    uint64_t chunk0_size; /* Size of chunk 0's data */
    size_t orig_prfx_size = (size_t)1
                            << (oh->flags &
                                H5O_HDR_CHUNK0_SIZE); /* Original prefix size */
  ...
  was_extended = H5MF_try_extend(f, H5FD_MEM_OHDR, oh->chunk[chunkno].addr,
                                 ...
                                 (hsize_t)(delta + extra_prfx_size));
  if (was_extended < 0) /* error */
    HGOTO_ERROR(H5E_OHDR, H5E_CANTEXTEND, FAIL,
                "can't tell if we can extend chunk");
  ...
  if (adjust_size_flags) {
    ...
    if (H5AC_mark_entry_dirty(oh) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTMARKDIRTY, FAIL,
                  "unable to mark object header as dirty");
  } /* end if */
  ...
  /* Create new null message for end of chunk */
    ...
    oh->mesg[extend_msg].raw_size =
        (delta + oh->chunk[chunkno].gap) - (size_t)H5O_SIZEOF_MSGHDR_OH(oh);
  ...
  oh->chunk[chunkno].image =
      H5FL_BLK_REALLOC(chunk_image, old_image, oh->chunk[chunkno].size);
  if (NULL == oh->chunk[chunkno].image)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, FAIL,
                "can't reallocate extended object header chunk");
  oh->chunk[chunkno].gap = 0;

  /* Wipe new space for chunk */
  memset(oh->chunk[chunkno].image + old_size, 0,
  ...
  if (adjust_size_flags)
    memmove(oh->chunk[0].image + H5O_SIZEOF_HDR(oh) - H5O_SIZEOF_CHKSUM_OH(oh),
            oh->chunk[0].image + H5O_SIZEOF_HDR(oh) - H5O_SIZEOF_CHKSUM_OH(oh) -
                extra_prfx_size,
            old_size - (size_t)H5O_SIZEOF_HDR(oh) + extra_prfx_size);

  /* Spin through existing messages, adjusting them */
  for (u = 0; u < oh->nmesgs; u++) {
    /* Adjust raw addresses for messages in this chunk to reflect new 'image'
     * address */
    if (oh->mesg[u].chunkno == chunkno)
      oh->mesg[u].raw = oh->chunk[chunkno].image + extra_prfx_size +
                        (oh->mesg[u].raw - old_image);

    /* Find continuation message which points to this chunk and adjust chunk's
     * size */
    ...
    if (chunkno > 0 && (H5O_CONT_ID == oh->mesg[u].type->id) &&
      ...
      H5O_chunk_proxy_t *chk_proxy2 =
          NULL;                     /* Chunk that continuation message is in */
      hbool_t chk_dirtied2 = FALSE; /* Flag for unprotecting chunk */
      unsigned cont_chunkno =
          oh->mesg[u].chunkno; /* Chunk # for continuation message */
  ...
  if (H5O__chunk_resize(oh, chk_proxy) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTRESIZE, FAIL,
                "unable to resize object header chunk");
...
static herr_t H5O__alloc_find_best_nonnull(const H5F_t *f, const H5O_t *oh,
   ...
   */
  cont_size =
      H5O_ALIGN_OH(oh, (size_t)(H5F_SIZEOF_ADDR(f) + H5F_SIZEOF_SIZE(f)));
  multi_size = 0;
  for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
    /* Don't consider continuation messages (for now) */
    if (H5O_CONT_ID != curr_msg->type->id) {
      unsigned msg_chunkno =
          curr_msg->chunkno; /* Chunk that the message is in */
      uint8_t *end_chunk_data =
          (oh->chunk[msg_chunkno].image + oh->chunk[msg_chunkno].size) -
          (H5O_SIZEOF_CHKSUM_OH(oh) +
           oh->chunk[msg_chunkno].gap); /* End of message data in chunk */
      uint8_t *end_msg =
          curr_msg->raw + curr_msg->raw_size; /* End of current message */
      ...
      size_t total_size;       /* Total size of available space "around" current
                                  message */
      ...
      if (end_msg == end_chunk_data)
        ...
        for (v = 0, tmp_msg = &oh->mesg[0]; v < oh->nmesgs; v++, tmp_msg++) {
          ...
          /* XXX: Should also check for NULL message in front of current
           * message... */
      ...
      if (total_size >= cont_size) {
        hbool_t better = FALSE; /* Whether the current message is better than a
                                   previous one */
  ...
  if (found_msg->msgno < 0)
    ...
    *size +=
        (size_t)H5O_SIZEOF_MSGHDR_OH(oh) + oh->mesg[found_msg->msgno].raw_size;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__alloc_chunk(H5F_t *f, H5O_t *oh, size_t size, size_t found_null,
  ...
  if (oh->nchunks >= oh->alloc_nchunks) {
    size_t na = MAX(H5O_NCHUNKS,
                    oh->alloc_nchunks * 2); /* Double # of chunks allocated */
    H5O_chunk_t *x;

    if (NULL == (x = H5FL_SEQ_REALLOC(H5O_chunk_t, oh->chunk, na)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, FAIL,
                  "can't allocate larger chunk array, na = %zu", na);
    oh->alloc_nchunks = na;
    oh->chunk = x;
  } /* end if */
  ...
  oh->chunk[chunkno].gap = 0;
  if (NULL ==
      (oh->chunk[chunkno].image = p = H5FL_BLK_CALLOC(chunk_image, size)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, FAIL,
                "can't allocate image for chunk, size = %zu", size);
  ...
  /* Check if we need to move multiple messages, in order to make room for the
   * new message */
  cont_size =
      H5O_ALIGN_OH(oh, (size_t)(H5F_SIZEOF_ADDR(f) + H5F_SIZEOF_SIZE(f)));
  if (found_null >= oh->nmesgs) {
    if (found_msg->msgno < 0) {
      ...
      for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++)
        if (curr_msg->chunkno == chunkno - 1) {
          if (curr_msg->type->id == H5O_NULL_ID) {
            ...
            if (u < oh->nmesgs - 1)
              memmove(curr_msg, curr_msg + 1,
                      ((oh->nmesgs - 1) - u) * sizeof(H5O_mesg_t));
            oh->nmesgs--;
          } /* end if */
          else {
      ...
      null_msg->raw_size = oh->chunk[chunkno - 1].size -
                           ((chunkno == 1) ? (size_t)H5O_SIZEOF_HDR(oh)
                                           : (size_t)H5O_SIZEOF_CHKHDR_OH(oh)) -
      ...
      /* Protect chunk */
      if (NULL == (chk_proxy = H5O__chunk_protect(
                       f, oh, oh->mesg[found_msg->msgno].chunkno)))
        HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                    "unable to load object header chunk");
      ...
      H5MM_memcpy(p, oh->mesg[found_msg->msgno].raw - H5O_SIZEOF_MSGHDR_OH(oh),
                  oh->mesg[found_msg->msgno].raw_size +
                      (size_t)H5O_SIZEOF_MSGHDR_OH(oh));
      ...
      /* Account for copied message in new chunk */
      p += (size_t)H5O_SIZEOF_MSGHDR_OH(oh) +
           oh->mesg[found_msg->msgno].raw_size;
      size -= (size_t)H5O_SIZEOF_MSGHDR_OH(oh) +
              oh->mesg[found_msg->msgno].raw_size;

      /* Add any available space after the message to move to the new null
       * message */
      if (found_msg->gap_size > 0) {
        ...
        H5O_mesg_t *old_null_msg =
            &oh->mesg[found_msg->null_msgno]; /* Pointer to NULL message to
                                                 eliminate */
        ...
        if (found_msg->null_msgno < (oh->nmesgs - 1))
          memmove(old_null_msg, old_null_msg + 1,
                  ((oh->nmesgs - 1) - found_msg->null_msgno) *
                      sizeof(H5O_mesg_t));
  ...
  oh->mesg[idx].raw_size =
      size - (size_t)(H5O_SIZEOF_CHKHDR_OH(oh) + H5O_SIZEOF_MSGHDR_OH(oh));
...
static herr_t H5O__alloc_new_chunk(H5F_t *f, H5O_t *oh, size_t size,
  ...
  if (H5O__alloc_find_best_null(oh, cont_size, &idx) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL,
                "error while locating best null header message");

  /* If we couldn't find a null message, locate the best message to move to new
   * chunk */
  if (idx >= oh->nmesgs) {
    found_msg.msgno = -1;
    if (H5O__alloc_find_best_nonnull(f, oh, &size, &found_msg) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL,
                  "error while locating best non-null header message");
  } /* end if */

  /* Allocate and initialize new chunk in the file */
  if (H5O__alloc_chunk(f, oh, size, idx, &found_msg, new_idx) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, FAIL,
                "can't allocate new object header chunk");
...
static herr_t H5O__alloc_find_best_null(const H5O_t *oh, size_t size,
  ...
  /* Find the smallest null message that could hold the new object header
   * message */
  found_null = -1;
    ...
    if (H5O_NULL_ID == oh->mesg[idx].type->id) {
      ...
      if (oh->mesg[idx].raw_size == size) {
        ...
        if (found_null < 0)
          ...
          /* If we've got more than one exact fit, choose the one in the
           * earliest chunk */
          if (oh->mesg[idx].chunkno < oh->mesg[found_null].chunkno) {
            found_null = (ssize_t)idx;

            /* If we found an exact fit in object header chunk #0, we can get
             * out */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__alloc(H5F_t *f, H5O_t *oh, const H5O_msg_class_t *type,
  ...
  if (0 == raw_size)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
                "can't compute object header message size");
  ...
  if (idx >= oh->nmesgs) {
    ...
    for (chunkno = 0; chunkno < oh->nchunks; chunkno++) {
      ...
      if ((tri_result =
               H5O__alloc_extend_chunk(f, oh, chunkno, raw_size, &idx)) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTEXTEND, FAIL,
                    "can't extend existing chunk");
      if (tri_result == TRUE)
        break;
    } /* end for */
    ...
    if (idx >= oh->nmesgs)
      if (H5O__alloc_new_chunk(f, oh, raw_size, &idx) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOSPACE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__release_mesg(H5F_t *f, H5O_t *oh, H5O_mesg_t *mesg,
                         hbool_t adj_link) {
  H5O_chunk_proxy_t *chk_proxy = NULL; /* Chunk that message is in */
  hbool_t chk_dirtied = FALSE;         /* Flag for unprotecting chunk */
  herr_t ret_value = SUCCEED;          /* Return value */
  ...
  if (NULL == (chk_proxy = H5O__chunk_protect(f, oh, mesg->chunkno)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                "unable to protect object header chunk");
...
static htri_t H5O__move_cont(H5F_t *f, H5O_t *oh, unsigned cont_u) {
  ...
  if (deleted_chunkno == (oh->nchunks - 1)) {
    size_t nonnull_size;  /* Total size of nonnull messages in the chunk pointed
                             to by cont message */
    ...
    for (v = 0, curr_msg = &oh->mesg[0]; v < oh->nmesgs; v++, curr_msg++) {
      if (curr_msg->chunkno == deleted_chunkno) {
        /* Find size of all non-null messages in the chunk pointed to by the
         * continuation message */
        if (curr_msg->type->id != H5O_NULL_ID) {
          assert(curr_msg->type->id != H5O_CONT_ID);
     ...
     */
    if (nonnull_size && nonnull_size <= (gap_size + cont_msg->raw_size +
                                         (size_t)H5O_SIZEOF_MSGHDR_OH(oh))) {
      uint8_t *move_start, *move_end; /* Pointers to area of messages to move */
      ...
      if (H5O__release_mesg(f, oh, cont_msg, FALSE) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, FAIL,
                    "unable to convert into null message");
      ...
      for (v = 0, curr_msg = &oh->mesg[0]; v < oh->nmesgs; v++, curr_msg++)
        ...
        if (curr_msg->chunkno == deleted_chunkno) {
          ...
          if (curr_msg->type->id != H5O_NULL_ID) {
            ...
            H5MM_memcpy(move_start, curr_msg->raw - H5O_SIZEOF_MSGHDR_OH(oh),
                        move_size);
      ...
      if (H5O__chunk_delete(f, oh, deleted_chunkno) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, FAIL,
                    "unable to remove chunk from cache");
      ...
      if (gap_size >= (size_t)H5O_SIZEOF_MSGHDR_OH(oh)) {
        ...
        if (gap_size > 0) {
          ...
          if (H5O__add_gap(f, oh, cont_chunkno, &chk_dirtied, cont_u,
                           move_start, gap_size) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTINSERT, FAIL,
                        "can't insert gap in chunk");
        } /* end if */
      ...
      for (v = oh->nmesgs - 1, curr_msg = &oh->mesg[v]; v < oh->nmesgs;
           v--, curr_msg--)
        /* Look for messages in chunk to delete */
        if (curr_msg->chunkno == deleted_chunkno) {
      ...
      oh->chunk[deleted_chunkno].image =
          H5FL_BLK_FREE(chunk_image, oh->chunk[deleted_chunkno].image);
      oh->nchunks--;
    } /* end if */
    else
      ret_value = FALSE;
...
static htri_t H5O__move_msgs_forward(H5F_t *f, H5O_t *oh) {
  ...
  H5O_chunk_proxy_t *cont_targ_chk_proxy =
      NULL; /* Chunk that continuation message points to */
    ...
    for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
      if (H5O_NULL_ID == curr_msg->type->id) {
        ...
        if ((curr_msg->raw + curr_msg->raw_size) !=
            ((chunk->image + chunk->size) -
             (H5O_SIZEOF_CHKSUM_OH(oh) + chunk->gap))) {
          H5O_mesg_t
              *nonnull_msg; /* Pointer to current message to operate on */
          unsigned v;       /* Local index variable */

          /* Loop over messages again, looking for the message in the chunk
           * after the null message
           */
          for (v = 0, nonnull_msg = &oh->mesg[0]; v < oh->nmesgs;
               v++, nonnull_msg++) {
            ...
            if ((curr_msg->chunkno == nonnull_msg->chunkno) &&
              ...
              /* Don't swap messages if the second message is also a null
               * message */
              /* (We'll merge them together later, in another routine) */
              if (H5O_NULL_ID != nonnull_msg->type->id) {
                /* Protect chunk */
                if (NULL == (null_chk_proxy =
                                 H5O__chunk_protect(f, oh, curr_msg->chunkno)))
                  HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                              "unable to load object header chunk");
                ...
                curr_msg->raw = nonnull_msg->raw + nonnull_msg->raw_size +
                                H5O_SIZEOF_MSGHDR_OH(oh);
        ...
        /* Check if messages in chunk pointed to can replace continuation
         * message */
        if (H5O_CONT_ID == curr_msg->type->id) {
          ...
          if ((status = H5O__move_cont(f, oh, u)) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, FAIL,
                   ...
                   0) { /* Message(s) got moved into "continuation" message */
            packed_msg = TRUE;
            break;
          } /* end else-if */
        }   /* end if */

        /* Loop over messages again, looking for large enough null message in
         * earlier chunk */
        for (v = 0, null_msg = &oh->mesg[0]; v < oh->nmesgs; v++, null_msg++) {
          if (H5O_NULL_ID == null_msg->type->id &&
            ...
            /* Protect chunks */
            if (NULL ==
                (null_chk_proxy = H5O__chunk_protect(f, oh, null_msg->chunkno)))
              HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                          "unable to load object header chunk");
            if (NULL ==
                (curr_chk_proxy = H5O__chunk_protect(f, oh, curr_msg->chunkno)))
              HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                          "unable to load object header chunk");
            ...
            if (oh->swmr_write && (H5O_CONT_ID == curr_msg->type->id)) {
              void *null_chk_mdc_obj; /* The metadata cache object for the
                                         null_msg chunk */
              ...
              if (NULL ==
                  (cont_targ_chk_proxy = H5O__chunk_protect(
              ...
              if (cont_targ_chk_proxy->fd_parent) {
                ...
                assert((void *)curr_chk_proxy ==
                       cont_targ_chk_proxy->fd_parent);

                if (H5AC_destroy_flush_dependency(curr_chk_proxy,
                                                  cont_targ_chk_proxy) < 0)
              ...
              if (0 != null_msg->chunkno) {
                 ...
                 * message chunk */
                if (H5AC_create_flush_dependency(null_chk_mdc_obj,
                                                 cont_targ_chk_proxy) < 0)
            ...
            if (curr_msg->raw_size == null_msg->raw_size) {
              ...
              if (oh->chunk[old_chunkno].gap > 0) {
                ...
                if (H5O__eliminate_gap(oh, &null_chk_dirtied, null_msg,
                                       ((oh->chunk[old_chunkno].image +
                                         oh->chunk[old_chunkno].size) -
                                        (H5O_SIZEOF_CHKSUM_OH(oh) +
                                         oh->chunk[old_chunkno].gap)),
              ...
              if ((null_msg->raw_size - curr_msg->raw_size) <
                  (size_t)H5O_SIZEOF_MSGHDR_OH(oh)) {
                size_t gap_size = null_msg->raw_size -
                                  curr_msg->raw_size; /* Size of gap produced */
                ...
                if (H5O__add_gap(f, oh, null_msg->chunkno, &null_chk_dirtied, v,
                                 null_msg->raw + null_msg->raw_size,
                                 gap_size) < 0)
                  HGOTO_ERROR(H5E_OHDR, H5E_CANTINSERT, FAIL,
                              "can't insert gap in chunk");

                /* Re-use message # for new null message taking place of
                 * non-null message */
                new_null_msg = v;
              } /* end if */
              else {
                ...
                /* Create new null message for previous location of non-null
                 * message */
                if (oh->nmesgs >= oh->alloc_nmesgs) {
                  if (H5O__alloc_msgs(oh, (size_t)1) < 0)
                    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  ...
                  /* "Retarget" 'curr_msg' pointer into newly re-allocated array
                   * of messages
                   */
                  curr_msg = &oh->mesg[u];
                } /* end if */
              ...
              /* Initialize new null message to take over non-null message's
               * location */
              ...
              if (oh->chunk[old_chunkno].gap > 0) {
                /* Eliminate the gap in the chunk */
                if (H5O__eliminate_gap(oh, &curr_chk_dirtied,
  ...
  if (ret_value < 0) {
    if (null_chk_proxy &&
        H5O__chunk_unprotect(f, null_chk_proxy, null_chk_dirtied) < 0)
      HDONE_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, FAIL,
                  "unable to unprotect null object header chunk");
    if (curr_chk_proxy &&
        H5O__chunk_unprotect(f, curr_chk_proxy, curr_chk_dirtied) < 0)
      HDONE_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, FAIL,
                  "unable to unprotect current object header chunk");
    if (cont_targ_chk_proxy &&
        H5O__chunk_unprotect(f, cont_targ_chk_proxy, FALSE) < 0)
      HDONE_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, FAIL,
                  "unable to unprotect continuation message target object "
                  "header chunk");
  } /* end if */
  else
  ...
  do {
    ...
    for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
      if (H5O_NULL_ID == curr_msg->type->id) {
        ...
        for (v = 0, curr_msg2 = &oh->mesg[0]; v < oh->nmesgs;
             v++, curr_msg2++) {
          if (u != v && H5O_NULL_ID == curr_msg2->type->id &&
            ...
            if ((curr_msg->raw + curr_msg->raw_size) ==
              ...
              adj_raw = 0;
              adj_raw_size =
                  (size_t)H5O_SIZEOF_MSGHDR_OH(oh) + curr_msg2->raw_size;
            ...
            /* Check for second message before first message */
              ...
              /* Adjust first message address and extend length to cover second
               * message */
              adj_raw = -((ssize_t)((size_t)H5O_SIZEOF_MSGHDR_OH(oh) +
                                    curr_msg2->raw_size));
              adj_raw_size =
                  (size_t)H5O_SIZEOF_MSGHDR_OH(oh) + curr_msg2->raw_size;
            ...
            if (merged_msg) {
              ...
              /* The merge null message might span the entire chunk: scan for
               * empty chunk to remove */
              if ((result = H5O__remove_empty_chunks(f, oh)) < 0)
                HGOTO_ERROR(H5E_OHDR, H5E_CANTPACK, FAIL,
                            "can't remove empty chunk");
              else if (result > 0)
                /* Get out of loop */
                break;
      ...
      if (H5O_NULL_ID == null_msg->type->id && null_msg->chunkno > 0 &&
        ...
        for (v = 0, cont_msg = &oh->mesg[0]; v < oh->nmesgs; v++, cont_msg++) {
          if (H5O_CONT_ID == cont_msg->type->id) {
            ...
            /* (should only occur when the continuation message is first
             * decoded) */
            if (0 == ((H5O_cont_t *)(cont_msg->native))->chunkno) {
              ...
              for (w = 0; w < oh->nchunks; w++)
                if (oh->chunk[w].addr ==
                    ((H5O_cont_t *)(cont_msg->native))->addr) {
            ...
            if (oh->chunk[null_msg->chunkno].addr ==
                ((H5O_cont_t *)(cont_msg->native))->addr)
              break;
          } /* end if */
        }   /* end for */
        /* Must be a continuation message that points to chunk containing null
         * message */
        assert(v < oh->nmesgs);
        assert(cont_msg);
        assert(((H5O_cont_t *)(cont_msg->native))->chunkno ==
               null_msg->chunkno);
        ...
        if (null_msg->chunkno < (oh->nchunks - 1)) {
          memmove(
              &oh->chunk[null_msg->chunkno], &oh->chunk[null_msg->chunkno + 1],
          ...
          for (u = null_msg->chunkno; u < (oh->nchunks - 1); u++) {
            unsigned chk_proxy_status =
                0; /* Metadata cache status of chunk proxy for chunk */

            /* Check the chunk proxy's status in the metadata cache */
            if (H5AC_get_entry_status(f, oh->chunk[u].addr, &chk_proxy_status) <
                0)
              HGOTO_ERROR(
                  H5E_OHDR, H5E_CANTGET, FAIL,
        ...
        /*
         * Delete null message (in empty chunk that was be freed) from list of
         * messages
         */
        ...
        for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
          ...
          if (H5O_CONT_ID == curr_msg->type->id) {
            ...
            if (0 == ((H5O_cont_t *)(curr_msg->native))->chunkno) {
              ...
              for (w = 0; w < oh->nchunks; w++)
                if (oh->chunk[w].addr ==
                    ((H5O_cont_t *)(curr_msg->native))->addr) {
...
herr_t H5O__condense_header(H5F_t *f, H5O_t *oh) {
  ...
  do {
    ...
    if (result < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTPACK, FAIL,
                  "can't move header messages forward");
    if (result > 0)
      rescan_header = TRUE;
    ...
    if (result < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTPACK, FAIL,
                  "can't pack null header messages");
    if (result > 0)
      rescan_header = TRUE;
...
static herr_t H5O__alloc_shrink_chunk(H5F_t *f, H5O_t *oh, unsigned chunkno) {
  ...
  H5O_chunk_proxy_t *chk_proxy =
      NULL;             /* Metadata cache proxy for chunk to shrink */
  H5O_mesg_t *curr_msg; /* Current message to examine */
  uint8_t *old_image =
      chunk->image;              /* Old address of chunk's image in memory */
  ...
  size_t min_chunk_size =
      H5O_ALIGN_OH(oh, H5O_MIN_SIZE);              /* Minimum chunk size */
   ...
   * end.
   */
  for (u = oh->nmesgs - 1, curr_msg = &oh->mesg[u]; u < oh->nmesgs;
       u--, curr_msg--) {
    if ((H5O_NULL_ID == curr_msg->type->id) && (chunkno == curr_msg->chunkno)) {
      size_t shrink_size = curr_msg->raw_size +
                           sizeof_msghdr; /* Amount to shrink the chunk by */
      ...
      if (curr_msg->raw + curr_msg->raw_size <
        ...
        for (v = 0, curr_msg2 = &oh->mesg[0]; v < oh->nmesgs; v++, curr_msg2++)
          if ((chunkno == curr_msg2->chunkno) &&
              (curr_msg2->raw > curr_msg->raw))
  ...
  total_msg_size = new_size - (size_t)(chunkno == 0 ? H5O_SIZEOF_HDR(oh)
                                                    : H5O_SIZEOF_CHKHDR_OH(oh));
  if (total_msg_size < min_chunk_size) {
    ...
    curr_msg->raw_size =
        MAX(H5O_ALIGN_OH(oh, min_chunk_size - total_msg_size), sizeof_msghdr) -
        sizeof_msghdr;
    curr_msg->chunkno = chunkno;
   ...
   * flags */
  if (oh->version > H5O_VERSION_1 && chunkno == 0) {
    uint64_t chunk0_newsize =
        new_size - (size_t)H5O_SIZEOF_HDR(oh); /* New size of chunk 0's data */
    size_t orig_prfx_size = (size_t)1
                            << (oh->flags &
                                H5O_HDR_CHUNK0_SIZE); /* Original prefix size */
  ...
  for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
    if (adjust_size_flags || (chunk->image != old_image))
      ...
      if (curr_msg->chunkno == chunkno)
        curr_msg->raw =
            chunk->image - less_prfx_size + (curr_msg->raw - old_image);
    ...
    if (chunkno > 0 && (H5O_CONT_ID == curr_msg->type->id) &&
      ...
      /* Protect chunk */
      if (NULL ==
          (cont_chk_proxy = H5O__chunk_protect(f, oh, curr_msg->chunkno)))
        HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, FAIL,
                    "unable to protect object header chunk");
  ...
  if (H5MF_xfree(f, H5FD_MEM_OHDR, chunk->addr + new_size,
                 (hsize_t)(old_size - new_size)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFREE, FAIL,
                "unable to shrink object header chunk");
