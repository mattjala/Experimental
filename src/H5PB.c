...
static htri_t H5PB__make_space(H5F_shared_t *f_sh, H5PB_t *page_buf,
 ...
 *              What follows is my best understanding of Mohamad's intent.
 *
 *              Retrieve statistics collected about page accesses for the page
 *buffer layer.
 *              --accesses: the number of metadata and raw data accesses to the
 *page buffer layer
 *              --hits: the number of metadata and raw data hits in the page
 *buffer layer
 *              --misses: the number of metadata and raw data misses in the page
 *buffer layer
 *              --evictions: the number of metadata and raw data evictions from
 *the page buffer layer
 *              --bypasses: the number of metadata and raw data accesses that
 *bypass the page buffer layer
 *
 * Return:      Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5PB_get_stats(const H5PB_t *page_buf, unsigned accesses[2],
...
herr_t H5PB_print_stats(const H5PB_t *page_buf) {
  ...
  printf("\t Bypasses: %u\n", page_buf->bypasses[0]);
  printf("\t Hit Rate = %f%%\n",
         ((double)page_buf->hits[0] /
          (page_buf->accesses[0] - page_buf->bypasses[0])) *
             100);
  printf("*****************\n\n");
  ...
  printf("\t Bypasses: %u\n", page_buf->bypasses[1]);
  printf("\t Hit Rate = %f%%\n",
         ((double)page_buf->hits[1] /
          (page_buf->accesses[1] - page_buf->bypasses[0])) *
             100);
  printf("*****************\n\n");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_create(H5F_shared_t *f_sh, size_t size,
  ...
  page_buf->min_meta_count =
      (unsigned)((size * page_buf_min_meta_perc) / (f_sh->fs_page_size * 100));
  page_buf->min_raw_count =
      (unsigned)((size * page_buf_min_raw_perc) / (f_sh->fs_page_size * 100));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PB__flush_cb(void *item, void H5_ATTR_UNUSED *key,
                             void *_op_data) {
  H5PB_entry_t *page_entry =
      (H5PB_entry_t *)item; /* Pointer to page entry node */
...
herr_t H5PB_flush(H5F_shared_t *f_sh) {
  ...
  /* Flush all the entries in the PB skiplist, if we have write access on the
   * file */
  if (f_sh->page_buf && (H5F_ACC_RDWR & H5F_SHARED_INTENT(f_sh))) {
    ...
    if (H5SL_iterate(page_buf->slist_ptr, H5PB__flush_cb, f_sh))
      HGOTO_ERROR(H5E_PAGEBUF, H5E_BADITER, FAIL,
                  "can't flush page buffer skip list");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5PB__dest_cb(void *item, void H5_ATTR_UNUSED *key,
                            void *_op_data) {
  H5PB_entry_t *page_entry =
      (H5PB_entry_t *)item; /* Pointer to page entry node */
  ...
  if (op_data->actual_slist) {
    ...
    page_entry->page_buf_ptr =
        H5FL_FAC_FREE(op_data->page_buf->page_fac, page_entry->page_buf_ptr);
  } /* end if */
...
herr_t H5PB_dest(H5F_shared_t *f_sh) {
  ...
  if (f_sh->page_buf) {
    ...
    if (H5SL_destroy(page_buf->slist_ptr, H5PB__dest_cb, &op_data))
      HGOTO_ERROR(H5E_PAGEBUF, H5E_CANTCLOSEOBJ, FAIL,
                  "can't destroy page buffer skip list");
    ...
    if (H5FL_fac_term(page_buf->page_fac) < 0)
      HGOTO_ERROR(H5E_PAGEBUF, H5E_CANTRELEASE, FAIL,
                  "can't destroy page buffer page factory");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_add_new_page(H5F_shared_t *f_sh, H5FD_mem_t type,
                         haddr_t page_addr) {
  ...
  if (NULL == H5SL_search(page_buf->mf_slist_ptr, &(page_addr))) {
    ...
    if (H5SL_insert(page_buf->mf_slist_ptr, page_entry, &(page_entry->addr)) <
        0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_update_entry(H5PB_t *page_buf, haddr_t addr, size_t size,
                         const void *buf) {
  ...
  page_entry =
      (H5PB_entry_t *)H5SL_search(page_buf->slist_ptr, (void *)(&page_addr));
  if (page_entry) {
    haddr_t offset;
...
herr_t H5PB_remove_entry(const H5F_shared_t *f_sh, haddr_t addr) {
  H5PB_t *page_buf; /* Page buffer to operate on */
  H5PB_entry_t *page_entry =
      NULL;                   /* Pointer to the page entry being searched */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  page_entry =
      (H5PB_entry_t *)H5SL_search(page_buf->slist_ptr, (void *)(&addr));
  ...
  if (page_entry) {
    ...
    if (NULL == H5SL_remove(page_buf->slist_ptr, &(page_entry->addr)))
      HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
                  "Page Entry is not in skip list");
    ...
    page_entry->page_buf_ptr =
        H5FL_FAC_FREE(page_buf->page_fac, page_entry->page_buf_ptr);
    page_entry = H5FL_FREE(H5PB_entry_t, page_entry);
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_read(H5F_shared_t *f_sh, H5FD_mem_t type, haddr_t addr, size_t size,
                 void *buf /*out*/) {
  H5PB_t *page_buf;         /* Page buffering info for this file */
  H5PB_entry_t *page_entry; /* Pointer to the corresponding page entry */
  H5FD_t *file;             /* File driver pointer */
  haddr_t first_page_addr,
      last_page_addr; /* Addresses of the first and last pages covered by I/O */
  haddr_t offset;
  ...
  if (NULL == page_buf || size >= page_buf->page_size ||
     ...
     */
    if (NULL == page_buf ||
        (size >= page_buf->page_size && H5FD_MEM_DRAW != type) ||
        (bypass_pb && H5FD_MEM_DRAW == type))
      HGOTO_DONE(SUCCEED);
  } /* end if */
  ...
  if (H5FD_MEM_DRAW == type) {
    last_page_addr =
        ((addr + size - 1) / page_buf->page_size) * page_buf->page_size;
  ...
  if (H5FD_MEM_DRAW == type && size >= page_buf->page_size) {
      ...
      if (node) {
        ...
        if (page_entry->is_dirty) {
          ...
          /* copy the entire fully accessed pages */
            ...
            H5MM_memcpy((uint8_t *)buf + (i * page_buf->page_size),
                        page_entry->page_buf_ptr, page_buf->page_size);
          } /* end else */
        }   /* end if */
        node = H5SL_next(node);
      } /* end if */
    ...
    for (i = 0; i < num_touched_pages; i++) {
      ...
      /* Calculate the aligned address of the page to search for it in the skip
       * list */
      ...
      if (1 == num_touched_pages)
        ...
        access_size =
            (0 == i ? (size_t)((first_page_addr + page_buf->page_size) - addr)
                    : (size - access_size));

      /* Lookup the page in the skip list */
      page_entry = (H5PB_entry_t *)H5SL_search(page_buf->slist_ptr,
                                               (void *)(&search_addr));
      ...
      if (page_entry) {
        ...
        H5MM_memcpy((uint8_t *)buf + buf_offset,
                    (uint8_t *)page_entry->page_buf_ptr + offset, access_size);
      ...
      /* if not found */
        ...
        if ((H5SL_count(page_buf->slist_ptr) * page_buf->page_size) >=
            page_buf->max_size) {
          ...
          if ((can_make_space = H5PB__make_space(f_sh, page_buf, type)) < 0)
            HGOTO_ERROR(H5E_PAGEBUF, H5E_NOSPACE, FAIL,
                        "make space in Page buffer Failed");

          /* if make_space returns 0, then we can't use the page
             buffer for this I/O and we need to bypass */
          if (0 == can_make_space) {
            /* make space can't return FALSE on second touched page since the
             * first is of the same type */
            assert(0 == i);
        ...
        /* Read page through the VFD layer, but make sure we don't read past the
         * EOA. */
        ...
        buf_offset = (0 == i ? 0 : size - access_size);
        H5MM_memcpy((uint8_t *)buf + buf_offset,
                    (uint8_t *)new_page_buf + offset, access_size);
        ...
        if (H5PB__insert_entry(page_buf, page_entry) < 0)
          HGOTO_ERROR(H5E_PAGEBUF, H5E_CANTSET, FAIL,
                      "error inserting new page in page buffer");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_write(H5F_shared_t *f_sh, H5FD_mem_t type, haddr_t addr,
  ...
  if (NULL == page_buf || size >= page_buf->page_size || bypass_pb) {
    ...
    if (bypass_pb) {
      if (H5PB_update_entry(page_buf, addr, size, buf) > 0)
        HGOTO_ERROR(H5E_PAGEBUF, H5E_CANTUPDATE, FAIL,
  ...
  /* Check if existing pages for raw data need to be updated since raw data
  ...
  if (H5FD_MEM_DRAW == type && size >= page_buf->page_size) {
    ...
    for (i = 0; i < num_touched_pages; i++) {
      ...
      /* Discard all fully written pages from the page buffer */
        ...
        page_entry = (H5PB_entry_t *)H5SL_remove(page_buf->slist_ptr,
                                                 (void *)(&search_addr));
        if (page_entry) {
          /* Remove from LRU list */
          H5PB__REMOVE_LRU(page_buf, page_entry)
          ...
          if (H5F_MEM_PAGE_DRAW == page_entry->type ||
              H5F_MEM_PAGE_GHEAP == page_entry->type)
            page_buf->raw_count--;
          else
            page_buf->meta_count--;
    ...
    for (i = 0; i < num_touched_pages; i++) {
      ...
      if (page_entry) {
        ...
        H5MM_memcpy((uint8_t *)page_entry->page_buf_ptr + offset,
                    (const uint8_t *)buf + buf_offset, access_size);
      ...
      /* If not found */
        ...
        if (H5F_ACC_RDWR & H5F_SHARED_INTENT(f_sh))
           ...
           */
          page_entry = (H5PB_entry_t *)H5SL_remove(page_buf->mf_slist_ptr,
                                                   (void *)(&search_addr));
        ...
        /* If found, then just update the buffer pointer to the newly allocate
         * buffer */
        if (page_entry) {
        ...
        /* Otherwise read page through the VFD layer, but make sure we don't
           read past the EOA. */
        else {
        ...
        /* Copy the requested data from the page into the input buffer */
        H5MM_memcpy((uint8_t *)new_page_buf + offset,
                    (const uint8_t *)buf + buf_offset, access_size);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PB_enabled(H5F_shared_t *f_sh, H5FD_mem_t type, hbool_t *enabled) {
  H5PB_t *page_buf;           /* Page buffering info for this file */
  hbool_t bypass_pb = FALSE;  /* Whether to bypass page buffering */
  herr_t ret_value = SUCCEED; /* Return value */
...
static herr_t H5PB__insert_entry(H5PB_t *page_buf, H5PB_entry_t *page_entry) {
  ...
  if (H5SL_insert(page_buf->slist_ptr, page_entry, &(page_entry->addr)) < 0)
    HGOTO_ERROR(H5E_PAGEBUF, H5E_CANTINSERT, FAIL,
                "can't insert entry in skip list");
  assert(H5SL_count(page_buf->slist_ptr) * page_buf->page_size <=
         page_buf->max_size);
...
static htri_t H5PB__make_space(H5F_shared_t *f_sh, H5PB_t *page_buf,
  ...
  if (H5FD_MEM_DRAW == inserted_type) {
    ...
    while (1) {
      if (page_entry->prev &&
          (H5F_MEM_PAGE_DRAW == page_entry->type ||
           H5F_MEM_PAGE_GHEAP == page_entry->type) &&
          page_buf->min_raw_count >= page_buf->raw_count)
        page_entry = page_entry->prev;
      else
  ...
  if (NULL == H5SL_remove(page_buf->slist_ptr, &(page_entry->addr)))
    HGOTO_ERROR(H5E_PAGEBUF, H5E_BADVALUE, FAIL,
                "Tail Page Entry is not in skip list");
  ...
  /* Update statistics */
  if (page_entry->type == H5F_MEM_PAGE_DRAW ||
      H5F_MEM_PAGE_GHEAP == page_entry->type)
    page_buf->evictions[1]++;
  else
    page_buf->evictions[0]++;
...
static herr_t H5PB__write_entry(H5F_shared_t *f_sh, H5PB_entry_t *page_entry) {
  ...
  /* Retrieve the 'eoa' for the file */
  if (HADDR_UNDEF ==
      (eoa = H5F_shared_get_eoa(f_sh, (H5FD_mem_t)page_entry->type)))
    HGOTO_ERROR(H5E_PAGEBUF, H5E_CANTGET, FAIL,
                "driver get_eoa request failed");
  ...
  if (page_entry->addr <= eoa) {
    ...
    if (H5FD_write(file, (H5FD_mem_t)page_entry->type, page_entry->addr,
