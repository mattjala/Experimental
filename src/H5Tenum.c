...
static char *H5T__enum_nameof(const H5T_t *dt, const void *value,
...
hid_t H5Tenum_create(hid_t parent_id) {
  ...
  if ((ret_value = H5I_register(H5I_DATATYPE, dt, TRUE)) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register data type ID");
...
herr_t H5T__enum_insert(const H5T_t *dt, const char *name, const void *value) {
  ...
  for (i = 0; i < dt->shared->u.enumer.nmembs; i++) {
    ...
    if (!memcmp((uint8_t *)dt->shared->u.enumer.value + (i * dt->shared->size),
                value, dt->shared->size))
  ...
  if (dt->shared->u.enumer.nmembs >= dt->shared->u.enumer.nalloc) {
    ...
    if (NULL == (names = (char **)H5MM_realloc(dt->shared->u.enumer.name,
                                               n * sizeof(char *))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
    dt->shared->u.enumer.name = names;

    if (NULL == (values = (uint8_t *)H5MM_realloc(dt->shared->u.enumer.value,
                                                  n * dt->shared->size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  ...
  H5MM_memcpy((uint8_t *)dt->shared->u.enumer.value + (i * dt->shared->size),
              value, dt->shared->size);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__get_member_value(const H5T_t *dt, unsigned membno,
                             void *value /*out*/) {
  ...
  H5MM_memcpy(value,
              (uint8_t *)dt->shared->u.enumer.value +
                  (membno * dt->shared->size),
              dt->shared->size);
...
static char *H5T__enum_nameof(const H5T_t *dt, const void *value,
  ...
  while (lt < rt) {
    md = (lt + rt) / 2;
    cmp = memcmp(value,
                 (uint8_t *)copied_dt->shared->u.enumer.value +
                     (md * copied_dt->shared->size),
