 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iter_start_offset(H5HF_hdr_t *hdr, H5HF_block_iter_t *biter,
                                   hsize_t offset) {
  ...
  H5HF_indirect_t
      *iblock_parent;        /* Parent indirect block of location context */
  ...
  hbool_t root_block =
      TRUE; /* Flag to indicate the current block is the root indirect block */
  ...
  */
  do {
    ...
    /* Walk down the rows in the doubling table until we've found the correct
     * row for the next block */
      ...
      if ((offset >= hdr->man_dtable.row_block_off[row]) &&
          (offset < hdr->man_dtable.row_block_off[row] +
                        (hdr->man_dtable.cparam.width *
                         hdr->man_dtable.row_block_size[row])))
        break;
    ...
    /* Compute column */
    H5_CHECK_OVERFLOW((curr_offset / hdr->man_dtable.row_block_size[row]),
                      hsize_t, unsigned);
    ...
    if (root_block) {
      ...
      iblock_nrows =
          (H5VM_log2_gen(child_size) - hdr->man_dtable.first_row_bits) + 1;
    } /* end else */

    /* Load indirect block for this context location */
    if (NULL ==
        (iblock = H5HF__man_iblock_protect(
             hdr, iblock_addr, iblock_nrows, iblock_parent, iblock_par_entry,
             FALSE, H5AC__NO_FLAGS_SET, &did_protect)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iter_set_entry(const H5HF_hdr_t *hdr, H5HF_block_iter_t *biter,
                                unsigned entry) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iter_start_entry(H5HF_hdr_t *hdr, H5HF_block_iter_t *biter,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iter_next(H5HF_hdr_t *hdr, H5HF_block_iter_t *biter,
                           unsigned nentries) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__man_iter_curr(H5HF_block_iter_t *biter, unsigned *row,
 ...
 *-------------------------------------------------------------------------
 */
hbool_t H5HF__man_iter_ready(H5HF_block_iter_t *biter) {
  FUNC_ENTER_PACKAGE_NOERR
