...
static herr_t H5Z__set_local_shuffle(hid_t dcpl_id, hid_t type_id,
                                     hid_t space_id);
...
static herr_t H5Z__set_local_shuffle(hid_t dcpl_id, hid_t type_id,
  ...
  if (H5P_get_filter_by_id(dcpl_plist, H5Z_FILTER_SHUFFLE, &flags, &cd_nelmts,
  ...
  if (H5P_modify_filter(dcpl_plist, H5Z_FILTER_SHUFFLE, flags,
                        (size_t)H5Z_SHUFFLE_TOTAL_NPARMS, cd_values) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTSET, FAIL,
                "can't set local shuffle parameters");
...
static size_t H5Z__filter_shuffle(unsigned flags, size_t cd_nelmts,
  ...
  if (cd_nelmts != H5Z_SHUFFLE_TOTAL_NPARMS ||
      cd_values[H5Z_SHUFFLE_PARM_SIZE] == 0)
  ...
  if (bytesoftype > 1 && numofelements > 1) {
    ...
    if (NULL == (dest = H5MM_malloc(nbytes)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, 0,
                  "memory allocation failed for shuffle buffer");
