...
int H5FD_mpi_get_rank(H5FD_t *file) {
  ...
  uint64_t flags =
      H5FD_CTL_FAIL_IF_UNKNOWN_FLAG | H5FD_CTL_ROUTE_TO_TERMINAL_VFD_FLAG;
  int rank = -1;
  ...
  /* Dispatch to driver */
  if ((cls->ctl)(file, H5FD_CTL_GET_MPI_RANK_OPCODE, flags, NULL, &rank_ptr) <
      0)
...
int H5FD_mpi_get_size(H5FD_t *file) {
  ...
  /* Dispatch to driver */
  if ((cls->ctl)(file, H5FD_CTL_GET_MPI_SIZE_OPCODE, flags, NULL, &size_ptr) <
      0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL, "driver get_size request failed");

  if (0 >= size)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                "driver get_size request returned bad value");
...
MPI_Comm H5FD_mpi_get_comm(H5FD_t *file) {
  ...
  /* Dispatch to driver */
  if ((cls->ctl)(file, H5FD_CTL_GET_MPI_COMMUNICATOR_OPCODE, flags, NULL,
                 &comm_ptr) < 0)
  ...
  if (comm == MPI_COMM_NULL)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, MPI_COMM_NULL,
                "driver get_comm request failed -- bad comm");
...
herr_t H5FD_mpi_get_file_sync_required(H5FD_t *file,
  ...
  /* Dispatch to driver */
  if ((cls->ctl)(file, H5FD_CTL_GET_MPI_FILE_SYNC_OPCODE, flags, NULL,
                 file_sync_required_ptr) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                "driver get_mpi_file_synce request failed");
...
herr_t H5FD_mpio_wait_for_left_neighbor(H5FD_t *_file) {
  ...
  if (file->mpi_rank != 0) {
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Recv(&msgbuf, 1, MPI_CHAR, file->mpi_rank - 1,
                             MPI_ANY_TAG, file->comm, &rcvstat)))
