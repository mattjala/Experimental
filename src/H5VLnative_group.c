 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_group_create(void *obj, const H5VL_loc_params_t *loc_params,
  ...
  if (name == NULL) {
    ...
    if (grp) {
      ...
      if (NULL == (oloc = H5G_oloc(grp)))
        HDONE_ERROR(H5E_SYM, H5E_CANTGET, NULL,
 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_group_open(void *obj, const H5VL_loc_params_t *loc_params,
                              const char *name, hid_t H5_ATTR_UNUSED gapl_id,
                              hid_t H5_ATTR_UNUSED dxpl_id,
                              void H5_ATTR_UNUSED **req) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_group_get(void *obj, H5VL_group_get_args_t *args,
                              hid_t H5_ATTR_UNUSED dxpl_id,
                              void H5_ATTR_UNUSED **req) {
  ...
  case H5VL_GROUP_GET_GCPL: {
    if ((args->args.get_gcpl.gcpl_id = H5G_get_create_plist((H5G_t *)obj)) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                  "can't get creation property list for group");
  ...
  default:
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "can't get this type of information from group");
  } /* end switch */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_group_specific(void *obj, H5VL_group_specific_args_t *args,
                                   hid_t H5_ATTR_UNUSED dxpl_id,
                                   void H5_ATTR_UNUSED **req) {
  H5G_t *grp = (H5G_t *)obj;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  case H5VL_GROUP_FLUSH: {
    ...
    if (H5F_HAS_FEATURE(grp->oloc.file, H5FD_FEAT_HAS_MPI))
      HGOTO_ERROR(H5E_SYM, H5E_UNSUPPORTED, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_group_optional(void H5_ATTR_UNUSED *obj,
  ...
  case H5VL_NATIVE_GROUP_ITERATE_OLD: {
    ...
    if ((ret_value = H5G_iterate(
             &grp_loc, iter_args->loc_params.loc_data.loc_by_name.name,
  ...
  case H5VL_NATIVE_GROUP_GET_OBJINFO: {
    ...
    if (H5G__get_objinfo(&grp_loc,
                         goi_args->loc_params.loc_data.loc_by_name.name,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_group_close(void *grp, hid_t H5_ATTR_UNUSED dxpl_id,
                                void H5_ATTR_UNUSED **req) {
  herr_t ret_value = SUCCEED; /* Return value */
