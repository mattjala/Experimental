...
static herr_t H5D__none_idx_size(const H5D_chk_idx_info_t *idx_info,
                                 hsize_t *size);
static herr_t H5D__none_idx_reset(H5O_storage_chunk_t *storage,
                                  hbool_t reset_addr);
static herr_t H5D__none_idx_dump(const H5O_storage_chunk_t *storage,
                                 FILE *stream);
...
static herr_t H5D__none_idx_create(const H5D_chk_idx_info_t *idx_info) {
  ...
  assert(idx_info->pline->nused ==
         0); /* Shouldn't have filter defined on entering here */
  assert(idx_info->layout);
  assert(idx_info->storage);
  assert(idx_info->layout->max_nchunks);
  assert(!H5_addr_defined(
      idx_info->storage->idx_addr)); /* address of data shouldn't be defined */
...
static herr_t H5D__none_idx_get_addr(const H5D_chk_idx_info_t *idx_info,
  ...
  udata->chunk_idx = H5VM_array_offset_pre((idx_info->layout->ndims - 1),
                                           idx_info->layout->max_down_chunks,
                                           udata->common.scaled);

  /* Calculate the address of the chunk */
  udata->chunk_block.offset =
      idx_info->storage->idx_addr + udata->chunk_idx * idx_info->layout->size;
...
static int H5D__none_idx_iterate(const H5D_chk_idx_info_t *idx_info,
  ...
  for (u = 0; u < idx_info->layout->nchunks && ret_value == H5_ITER_CONT; u++) {
    ...
    chunk_rec.chunk_addr =
        idx_info->storage->idx_addr + idx * idx_info->layout->size;
...
static herr_t H5D__none_idx_delete(const H5D_chk_idx_info_t *idx_info) {
  ...
  assert(idx_info->pline);
  assert(!idx_info->pline
              ->nused); /* Shouldn't have filter defined on entering here */
  assert(idx_info->layout);
  assert(idx_info->storage);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__none_idx_copy_setup(
 ...
 */
static herr_t
H5D__none_idx_size(const H5D_chk_idx_info_t H5_ATTR_UNUSED *idx_info,
                   hsize_t *index_size) {
  FUNC_ENTER_PACKAGE_NOERR
