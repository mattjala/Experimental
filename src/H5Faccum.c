...
#define H5F_ACCUM_MAX_SIZE                                                     \
  (1024 * 1024) /* Max. accum. buf size (max. I/Os will be 1/2 this size) */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__accum_read(H5F_shared_t *f_sh, H5FD_mem_t map_type, haddr_t addr,
  ...
  if ((f_sh->feature_flags & H5FD_FEAT_ACCUMULATE_METADATA) &&
    ...
    if (size < H5F_ACCUM_MAX_SIZE) {
      ...
      if (H5_addr_defined(accum->loc) &&
        ...
        new_size =
            (size_t)(MAX((addr + size), (accum->loc + accum->size)) - new_addr);

        /* Check if we need more buffer space */
        if (new_size > accum->alloc_size) {
          size_t new_alloc_size; /* New size of accumulator */

          /* Adjust the buffer size to be a power of 2 that is large enough to
           * hold data */
          new_alloc_size = (size_t)1
                           << (1 + H5VM_log2_gen((uint64_t)(new_size - 1)));

          /* Reallocate the metadata accumulator buffer */
          if (NULL == (accum->buf = H5FL_BLK_REALLOC(meta_accum, accum->buf,
                                                     new_alloc_size)))
          ...
          /* Clear the memory */
          memset(accum->buf + accum->size, 0,
                 (accum->alloc_size - accum->size));
        } /* end if */

        /* Read the part before the metadata accumulator */
        if (addr < accum->loc) {
          /* Set the amount to read */
          H5_CHECKED_ASSIGN(amount_before, size_t, (accum->loc - addr),
                            hsize_t);
        ...
        if ((addr + size) > (accum->loc + accum->size)) {
          ...
          /* Set the amount to read */
          H5_CHECKED_ASSIGN(amount_after, size_t,
                            ((addr + size) - (accum->loc + accum->size)),
                            hsize_t);

          /* Dispatch to driver */
          if (H5FD_read(file, map_type, (accum->loc + accum->size),
                        amount_after,
      ...
      /* Current read doesn't overlap with metadata accumulator, read it from
         file */
      else {
        /* Dispatch to driver */
       ...
       */
      if (accum->dirty &&
          H5_addr_overlap(addr, size, accum->loc + accum->dirty_off,
                          accum->dirty_len)) {
        haddr_t dirty_loc =
            accum->loc +
            accum->dirty_off; /* File offset of dirty information */
        ...
        H5MM_memcpy((unsigned char *)buf + buf_off,
                    (unsigned char *)accum->buf + accum->dirty_off + dirty_off,
                    overlap_size);
      } /* end if */
    }   /* end else */
  }     /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5F__accum_adjust(H5F_meta_accum_t *accum, H5FD_t *file,
  ...
  if ((size + accum->size) > accum->alloc_size) {
    ...
    /* Adjust the buffer size to be a power of 2 that is large enough to hold
     * data */
    new_size =
        (size_t)1 << (1 + H5VM_log2_gen((uint64_t)((size + accum->size) - 1)));
    ...
    if (new_size > H5F_ACCUM_MAX_SIZE) {
      ...
      if (size > (H5F_ACCUM_MAX_SIZE / 2)) {
        ...
        if (H5F_ACCUM_PREPEND == adjust) {
          ...
          /* Check if we can slide the dirty region down, to accommodate the
           * request */
          if (accum->dirty && (adjust_size <= H5F_ACCUM_MAX_SIZE)) {
            if ((ssize_t)(H5F_ACCUM_MAX_SIZE -
                          (accum->dirty_off + adjust_size)) >=
                (ssize_t)(2 * size))
      ...
      if (accum->dirty) {
        ...
        if (H5F_ACCUM_PREPEND == adjust) {
          /* Check if the dirty region overlaps the region to eliminate from the
           * accumulator */
          if ((accum->size - shrink_size) <
              (accum->dirty_off + accum->dirty_len)) {
            /* Write out the dirty region from the metadata accumulator, with
             * dispatch to driver
             */
            if (H5FD_write(file, H5FD_MEM_DEFAULT,
    ...
    if (new_size > accum->alloc_size) {
      ...
      /* Reallocate the metadata accumulator buffer */
      if (NULL ==
          (new_buf = H5FL_BLK_REALLOC(meta_accum, accum->buf, new_size)))
      ...
      /* Clear the memory */
      memset(accum->buf + accum->size, 0,
             (accum->alloc_size - (accum->size + size)));
    } /* end if */
  }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__accum_write(H5F_shared_t *f_sh, H5FD_mem_t map_type, haddr_t addr,
  ...
  if ((f_sh->feature_flags & H5FD_FEAT_ACCUMULATE_METADATA) &&
    ...
    if (size < H5F_ACCUM_MAX_SIZE) {
      ...
      if (accum->size > 0) {
        /* Check if the new metadata adjoins the beginning of the current
         * accumulator */
        if (H5_addr_defined(accum->loc) && (addr + size) == accum->loc) {
          /* Check if we need to adjust accumulator size */
          if (H5F__accum_adjust(accum, file, H5F_ACCUM_PREPEND, size) < 0)
            HGOTO_ERROR(H5E_IO, H5E_CANTRESIZE, FAIL,
                        "can't adjust metadata accumulator");
        ...
        /* Check if the piece of metadata being written overlaps the metadata
           accumulator */
        else if (H5_addr_defined(accum->loc) &&
                 H5_addr_overlap(addr, size, accum->loc, accum->size)) {
          size_t add_size; /* New size of the accumulator buffer */

          /* Check if the new metadata is entirely within the current
           * accumulator */
          if (addr >= accum->loc &&
              (addr + size) <= (accum->loc + accum->size)) {
            size_t dirty_off = (size_t)(addr - accum->loc);

            /* Copy the new metadata to the proper location within the
             * accumulator */
            ...
            if (accum->dirty) {
              ...
              if (dirty_off <= accum->dirty_off) {
                if ((dirty_off + size) <= (accum->dirty_off + accum->dirty_len))
                  accum->dirty_len =
                      (accum->dirty_off + accum->dirty_len) - dirty_off;
                else
                  accum->dirty_len = size;
          ...
          /* Check if the new metadata overlaps the beginning of the current
             accumulator */
          else if (addr < accum->loc &&
                   (addr + size) <= (accum->loc + accum->size)) {
            size_t old_offset; /* Offset of old data within the accumulator
                                  buffer */

            /* Calculate the amount we will need to add to the accumulator size,
            ...
            H5_CHECKED_ASSIGN(old_offset, size_t, (addr + size) - accum->loc,
                              hsize_t);

            /* Move the existing metadata to the proper location */
            memmove(accum->buf + size, accum->buf + old_offset,
                    (accum->size - old_offset));
            ...
            /* Adjust the dirty region and mark accumulator dirty */
            if (accum->dirty) {
              size_t curr_dirty_end =
                  add_size + accum->dirty_off + accum->dirty_len;
          ...
          /* Check if the new metadata overlaps the end of the current
             accumulator */
          else if (addr >= accum->loc &&
                   (addr + size) > (accum->loc + accum->size)) {
            ...
            /* Calculate the amount we will need to add to the accumulator size,
             * based on the amount of overlap */
            H5_CHECKED_ASSIGN(add_size, size_t,
                              (addr + size) - (accum->loc + accum->size),
                              hsize_t);
          ...
          /* New metadata overlaps both ends of the current accumulator */
            ...
            if (size > accum->alloc_size) {
              ...
              /* Adjust the buffer size to be a power of 2 that is large enough
               * to hold data */
              new_alloc_size = (size_t)1
                               << (1 + H5VM_log2_gen((uint64_t)(size - 1)));
        ...
        /* New piece of metadata doesn't adjoin or overlap the existing
           accumulator */
        else {
          /* Write out the existing metadata accumulator, with dispatch to
           * driver */
          if (accum->dirty) {
            if (H5FD_write(file, H5FD_MEM_DEFAULT,
                           accum->loc + accum->dirty_off, accum->dirty_len,
          ...
          if (size > accum->alloc_size) {
            ...
            memset(accum->buf + clear_size, 0,
                   (accum->alloc_size - clear_size));
          } /* end if */
          else {
            ...
            if (size < (accum->alloc_size / H5F_ACCUM_THROTTLE) &&
              ...
              size_t tmp_size =
                  (accum->alloc_size /
                   H5F_ACCUM_THROTTLE); /* New size of accumulator buffer */
      ...
      /* No metadata in the accumulator, grab this piece and keep it */
        ...
        if (size > accum->alloc_size) {
          ...
          /* Reallocate the metadata accumulator buffer */
          if (NULL ==
              (accum->buf = H5FL_BLK_REALLOC(meta_accum, accum->buf, new_size)))
            HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                        "unable to allocate metadata accumulator buffer");
        ...
        if (H5_addr_le(addr, accum->loc)) {
          ...
          if (H5_addr_le(addr + size, accum->loc + accum->size)) {
            ...
            if (accum->dirty) {
              haddr_t dirty_start =
                  accum->loc +
                  accum->dirty_off; /* File address of start of dirty region */
              haddr_t dirty_end =
                  dirty_start +
                  accum->dirty_len; /* File address of end of dirty region */
            ...
            if (H5F__accum_reset(f_sh, FALSE) < 0)
              HGOTO_ERROR(H5E_IO, H5E_CANTRESET, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F__accum_free(H5F_shared_t *f_sh, H5FD_mem_t H5_ATTR_UNUSED type,
  ...
  if ((f_sh->feature_flags & H5FD_FEAT_ACCUMULATE_METADATA) &&
    ...
    assert(
        H5FD_MEM_GHEAP !=
        type); /* (global heap data is being treated as raw data currently) */
    ...
    if (H5_addr_le(addr, accum->loc)) {
      ...
      /* Block to free must end within the accumulator */
        ...
        H5_CHECKED_ASSIGN(overlap_size, size_t, (addr + size) - accum->loc,
                          haddr_t);
        ...
        if (accum->dirty) {
          ...
          if (overlap_size < accum->dirty_off)
            ...
            if (overlap_size < (accum->dirty_off + accum->dirty_len)) {
              accum->dirty_len =
                  (accum->dirty_off + accum->dirty_len) - overlap_size;
              accum->dirty_off = 0;
            } /* end if */
    ...
    /* Block to free must start within the accumulator */
      ...
      H5_CHECKED_ASSIGN(overlap_size, size_t, (accum->loc + accum->size) - addr,
                        haddr_t);
      ...
      if (accum->dirty && H5_addr_lt(addr, dirty_end)) {
        ...
        if (H5_addr_lt(addr, dirty_start)) {
          ...
          if (H5_addr_le(tail_addr, dirty_start)) {
            ...
            if (H5FD_write(file, H5FD_MEM_DEFAULT, dirty_start,
          ...
          /* Check for unfreed dirty region to write */
            ...
            if (H5FD_write(file, H5FD_MEM_DEFAULT, dirty_start + dirty_delta,
                           write_size,
...
herr_t H5F__accum_flush(H5F_shared_t *f_sh) {
  ...
  if ((f_sh->feature_flags & H5FD_FEAT_ACCUMULATE_METADATA) &&
    ...
    if (H5FD_write(
            file, H5FD_MEM_DEFAULT, f_sh->accum.loc + f_sh->accum.dirty_off,
