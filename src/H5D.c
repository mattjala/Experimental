...
static hid_t H5D__open_api_common(hid_t loc_id, const char *name, hid_t dapl_id,
                                  void **token_ptr,
                                  H5VL_object_t **_vol_obj_ptr);
static hid_t H5D__get_space_api_common(hid_t dset_id, void **token_ptr,
                                       H5VL_object_t **_vol_obj_ptr);
static herr_t H5D__read_api_common(size_t count, hid_t dset_id[],
                                   hid_t mem_type_id[], hid_t mem_space_id[],
                                   hid_t file_space_id[], hid_t dxpl_id,
                                   void *buf[], void **token_ptr,
                                   H5VL_object_t **_vol_obj_ptr);
static herr_t H5D__write_api_common(size_t count, hid_t dset_id[],
                                    hid_t mem_type_id[], hid_t mem_space_id[],
                                    hid_t file_space_id[], hid_t dxpl_id,
...
static hid_t H5D__create_api_common(hid_t loc_id, const char *name,
  ...
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_DACC, TRUE, &dapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, H5I_INVALID_HID,
  ...
  /* Create the dataset */
  if (NULL == (dset = H5VL_dataset_create(
                   *vol_obj_ptr, &loc_params, name, lcpl_id, type_id, space_id,
                   dcpl_id, dapl_id, H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to create dataset");

  /* Get an ID for the dataset */
  if ((ret_value = H5VL_register(H5I_DATASET, dset, (*vol_obj_ptr)->connector,
                                 TRUE)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register dataset");

done:
  if (H5I_INVALID_HID == ret_value)
    if (dset && H5VL_dataset_close(*vol_obj_ptr, H5P_DATASET_XFER_DEFAULT,
                                   H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, H5I_INVALID_HID,
                  "unable to release dataset");
...
hid_t H5Dcreate2(hid_t loc_id, const char *name, hid_t type_id, hid_t space_id,
  ...
  if ((ret_value =
           H5D__create_api_common(loc_id, name, type_id, space_id, lcpl_id,
                                  dcpl_id, dapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to synchronously create dataset");
...
hid_t H5Dcreate_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE11("i", "*s*sIui*siiiiii", app_file, app_func, app_line, loc_id, name,
            type_id, space_id, lcpl_id, dcpl_id, dapl_id, es_id);
  ...
  if ((ret_value =
           H5D__create_api_common(loc_id, name, type_id, space_id, lcpl_id,
                                  dcpl_id, dapl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, H5I_INVALID_HID,
                "unable to asynchronously create dataset");
    ...
    /* clang-format off */
      ...
      if (H5I_dec_app_ref_always_close(ret_value) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CANTDEC, H5I_INVALID_HID,
                    "can't decrement count on dataset ID");
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, H5I_INVALID_HID,
...
hid_t H5Dcreate_anon(hid_t loc_id, hid_t type_id, hid_t space_id, hid_t dcpl_id,
  ...
  /* Create the dataset */
  if (NULL == (dset = H5VL_dataset_create(
                   vol_obj, &loc_params, NULL, H5P_LINK_CREATE_DEFAULT, type_id,
  ...
  /* Get an ID for the dataset */
  if ((ret_value = H5VL_register(H5I_DATASET, dset, vol_obj->connector, TRUE)) <
      0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register dataset");
  ...
  /* Cleanup on failure */
  if (H5I_INVALID_HID == ret_value)
    if (dset && H5VL_dataset_close(vol_obj, H5P_DATASET_XFER_DEFAULT,
                                   H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, H5I_INVALID_HID,
                  "unable to release dataset");
...
static hid_t H5D__open_api_common(hid_t loc_id, const char *name, hid_t dapl_id,
  ...
  /* Set up object access arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_DACC, FALSE, &dapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");
  ...
  /* Open the dataset */
  if (NULL ==
      (dset = H5VL_dataset_open(*vol_obj_ptr, &loc_params, name, dapl_id,
                                H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, H5I_INVALID_HID,
...
hid_t H5Dopen2(hid_t loc_id, const char *name, hid_t dapl_id) {
  ...
  if ((ret_value = H5D__open_api_common(loc_id, name, dapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to synchronously open dataset");
...
hid_t H5Dopen_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(H5I_INVALID_HID)
  H5TRACE7("i", "*s*sIui*sii", app_file, app_func, app_line, loc_id, name,
           dapl_id, es_id);
  ...
  if ((ret_value = H5D__open_api_common(loc_id, name, dapl_id, token_ptr,
                                        &vol_obj)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to asynchronously open dataset");
...
static hid_t H5D__get_space_api_common(hid_t dset_id, void **token_ptr,
  ...
  /* Check args */
  if (NULL ==
      (*vol_obj_ptr = (H5VL_object_t *)H5I_object_verify(dset_id, H5I_DATASET)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
  ...
  /* Get the dataspace */
  if (H5VL_dataset_get(*vol_obj_ptr, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                       token_ptr) < 0)
...
hid_t H5Dget_space_async(const char *app_file, const char *app_func,
  ...
  if ((ret_value = H5D__get_space_api_common(dset_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, H5I_INVALID_HID,
                "unable to asynchronously get dataspace");
...
herr_t H5Dget_space_status(hid_t dset_id,
                           H5D_space_status_t *allocation /*out*/) {
  ...
  /* Check args */
  if (NULL ==
      (vol_obj = (H5VL_object_t *)H5I_object_verify(dset_id, H5I_DATASET)))
...
hid_t H5Dget_create_plist(hid_t dset_id) {
  ...
  if (H5VL_dataset_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                       H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get dataset creation properties");
 ...
 *be those used by the dataset.  If the properties in the file access property
 *list were used to determine the dataset's chunk cache configuration, then
 *those properties will be present in the returned dataset access property list.
 *If the dataset does not use a chunked layout, then the chunk cache properties
 *will be set to the default.  The chunk cache properties in the returned list
 *              are considered to be “set”, and any use of this list will
 *override the corresponding properties in the file's file access property list.
 *
 *              All link access properties in the returned list will be set to
 *the default values.
 *
...
hid_t H5Dget_access_plist(hid_t dset_id) {
  ...
  if (H5VL_dataset_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                       H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, H5I_INVALID_HID,
                "unable to get dataset access properties");
...
haddr_t H5Dget_offset(hid_t dset_id) {
  ...
  H5VL_native_dataset_optional_args_t
      dset_opt_args;                 /* Arguments for optional operation */
  ...
  /* Get the offset */
  if (H5VL_dataset_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                            H5_REQUEST_NULL) < 0)
...
static herr_t H5D__read_api_common(size_t count, hid_t dset_id[],
  ...
  if (NULL == (*vol_obj_ptr =
                   (H5VL_object_t *)H5I_object_verify(dset_id[0], H5I_DATASET)))
  ...
  for (i = 1; i < count; i++) {
    /* Get the object */
    if (NULL == (tmp_vol_obj = (H5VL_object_t *)H5I_object_verify(dset_id[i],
                                                                  H5I_DATASET)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "dset_id is not a dataset ID");
  ...
  /* Read the data */
  if (H5VL_dataset_read_direct(count, obj, connector, mem_type_id, mem_space_id,
                               file_space_id, dxpl_id, buf, token_ptr) < 0)
...
herr_t H5Dread(hid_t dset_id, hid_t mem_type_id, hid_t mem_space_id,
  ...
  /* Read the data */
  if (H5D__read_api_common(1, &dset_id, &mem_type_id, &mem_space_id,
                           &file_space_id, dxpl_id, &buf, NULL, NULL) < 0)
...
herr_t H5Dread_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIuiiiiixi", app_file, app_func, app_line, dset_id,
  ...
  /* Read the data */
  if (H5D__read_api_common(1, &dset_id, &mem_type_id, &mem_space_id,
                           &file_space_id, dxpl_id, &buf, token_ptr,
                           &vol_obj) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
...
herr_t H5Dread_multi(size_t count, hid_t dset_id[], hid_t mem_type_id[],
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "z*i*i*i*iix", count, dset_id, mem_type_id, mem_space_id,
  ...
  /* Read the data */
  if (H5D__read_api_common(count, dset_id, mem_type_id, mem_space_id,
                           file_space_id, dxpl_id, buf, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                "can't synchronously read data");
...
herr_t H5Dread_multi_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE11("e", "*s*sIuz*i*i*i*iixi", app_file, app_func, app_line, count,
...
herr_t H5Dread_chunk(hid_t dset_id, hid_t dxpl_id, const hsize_t *offset,
  ...
  if (H5P_DEFAULT == dxpl_id)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "dxpl_id is not a dataset transfer property list ID");
  ...
  /* Read the raw chunk */
  if (H5VL_dataset_optional(vol_obj, &vol_cb_args, dxpl_id, H5_REQUEST_NULL) <
      0)
    HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                "can't read unprocessed chunk data");
...
static herr_t H5D__write_api_common(size_t count, hid_t dset_id[],
  ...
  /* Write the data */
  if (H5VL_dataset_write_direct(count, obj, connector, mem_type_id,
                                mem_space_id, file_space_id, dxpl_id, buf,
                                token_ptr) < 0)
...
herr_t H5Dwrite(hid_t dset_id, hid_t mem_type_id, hid_t mem_space_id,
  ...
  /* Write the data */
  if (H5D__write_api_common(1, &dset_id, &mem_type_id, &mem_space_id,
                            &file_space_id, dxpl_id, &buf, NULL, NULL) < 0)
...
herr_t H5Dwrite_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIuiiiii*xi", app_file, app_func, app_line, dset_id,
            mem_type_id, mem_space_id, file_space_id, dxpl_id, buf, es_id);
...
herr_t H5Dwrite_multi(size_t count, hid_t dset_id[], hid_t mem_type_id[],
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "z*i*i*i*ii**x", count, dset_id, mem_type_id, mem_space_id,
           file_space_id, dxpl_id, buf);
  ...
  /* Write the data */
  if (H5D__write_api_common(count, dset_id, mem_type_id, mem_space_id,
                            file_space_id, dxpl_id, buf, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                "can't synchronously write data");
...
herr_t H5Dwrite_multi_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE11("e", "*s*sIuz*i*i*i*ii**xi", app_file, app_func, app_line, count,
            dset_id, mem_type_id, mem_space_id, file_space_id, dxpl_id, buf,
            es_id);
...
herr_t H5Dwrite_chunk(hid_t dset_id, hid_t dxpl_id, uint32_t filters,
  ...
  if (data_size != (size_t)data_size_32)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid data_size - chunks cannot be > 4 GiB");
  ...
  if (H5VL_dataset_optional(vol_obj, &vol_cb_args, dxpl_id, H5_REQUEST_NULL) <
      0)
    HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                "can't write unprocessed chunk data");
...
herr_t H5Dscatter(H5D_scatter_func_t op, void *op_data, hid_t type_id,
                  hid_t dst_space_id, void *dst_buf /*out*/) {
  H5T_t *type;                 /* Datatype */
  ...
  if (H5S_select_iter_init(iter, dst_space, type_size, 0) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to initialize selection iterator information");
  iter_init = TRUE;
  ...
  while (nelmts > 0) {
    ...
    if (op(&src_buf, &src_buf_nbytes, op_data) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CALLBACK, FAIL,
                  "callback operator returned failure");
    ...
    if (!src_buf)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "callback did not return a buffer");
    if (src_buf_nbytes == 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "callback returned a buffer size of 0");
    if (src_buf_nbytes % type_size)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "buffer size is not a multiple of datatype size");
    if (nelmts_scatter > (size_t)nelmts)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "callback returned more elements than in selection");
...
herr_t H5Dgather(hid_t src_space_id, const void *src_buf, hid_t type_id,
                 size_t dst_buf_size, void *dst_buf /*out*/,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "i*xizxDg*x", src_space_id, src_buf, type_id, dst_buf_size,
           dst_buf, op, op_data);
  ...
  if (((size_t)nelmts > dst_buf_nelmts) && (op == NULL))
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "no callback supplied and destination buffer too small");
  ...
  while (nelmts > 0) {
    ...
    if (0 == (nelmts_gathered = H5D__gather_mem(
                  src_buf, iter, MIN(dst_buf_nelmts, (size_t)nelmts), dst_buf)))
...
herr_t H5Dfill(const void *fill, hid_t fill_type_id, void *buf,
  ...
  if (NULL ==
      (fill_type = (H5T_t *)H5I_object_verify(fill_type_id, H5I_DATATYPE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, 0, "not a datatype");
  if (NULL ==
      (buf_type = (H5T_t *)H5I_object_verify(buf_type_id, H5I_DATATYPE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, 0, "not a datatype");
 ...
 *          void *elem;         IN/OUT: Pointer to the element in memory
 *containing the current point. hid_t type_id;      IN: Datatype ID for the
 *elements stored in ELEM. unsigned ndim;       IN: Number of dimensions for
 *POINT array const hsize_t *point; IN: Array containing the location of the
 *element within the original dataspace. void *operator_data;    IN/OUT: Pointer
...
herr_t H5Dvlen_get_buf_size(hid_t dataset_id, hid_t type_id, hid_t space_id,
                            hsize_t *size /*out*/) {
  H5VL_object_t *vol_obj; /* Dataset for this operation */
  uint64_t supported; /* Whether 'get vlen buf size' operation is supported by
                         VOL connector */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  supported = 0;
  if (H5VL_introspect_opt_query(vol_obj, H5VL_SUBCLS_DATASET,
...
static herr_t H5D__set_extent_api_common(hid_t dset_id, const hsize_t size[],
  ...
  if (H5CX_set_loc(dset_id) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "can't set collective metadata read info");
  ...
  /* Set the extent */
  if (H5VL_dataset_specific(*vol_obj_ptr, &vol_cb_args,
...
herr_t H5Dset_extent(hid_t dset_id, const hsize_t size[]) {
  ...
  if ((ret_value = H5D__set_extent_api_common(dset_id, size, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "unable to synchronously change a dataset's dimensions");
...
herr_t H5Dset_extent_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE6("e", "*s*sIui*hi", app_file, app_func, app_line, dset_id, size,
           es_id);
  ...
  if (H5D__set_extent_api_common(dset_id, size, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "unable to asynchronously change a dataset's dimensions");
...
herr_t H5Dflush(hid_t dset_id) {
  ...
  if (NULL ==
      (vol_obj = (H5VL_object_t *)H5I_object_verify(dset_id, H5I_DATASET)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "dset_id parameter is not a valid dataset identifier");
   ...
   */
  if (H5VL_dataset_specific(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                            H5_REQUEST_NULL) < 0)
...
herr_t H5Dformat_convert(hid_t dset_id) {
  ...
  if (H5VL_dataset_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                            H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_INTERNAL, FAIL,
                "can't convert dataset format");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Dget_chunk_index_type(hid_t dset_id,
                               H5D_chunk_index_t *idx_type /*out*/) {
  H5VL_object_t *vol_obj;           /* Dataset for this operation   */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_dataset_optional_args_t
      dset_opt_args;          /* Arguments for optional operation */
  ...
  if (NULL == idx_type)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "idx_type parameter cannot be NULL");
...
herr_t H5Dget_chunk_storage_size(hid_t dset_id, const hsize_t *offset,
  ...
  if (NULL == chunk_nbytes)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "chunk_nbytes parameter cannot be NULL");
  ...
  if (H5VL_dataset_optional(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                            H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't get storage size of chunk");
 ...
 *              unsigned *filter_mask   OUT: Mask for identifying the filters in
 *use haddr_t *addr           OUT: Address of the chunk hsize_t *size OUT: Size
 *of the chunk
 *
 * Return:      Non-negative on success, negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5Dget_chunk_info(hid_t dset_id, hid_t fspace_id, hsize_t chk_index,
                         hsize_t *offset /*out*/, unsigned *filter_mask /*out*/,
 ...
 *
 * Purpose:     Iterates over all chunks in dataset with given callback and user
 *data.
 *
 * Parameters:
 *              hid_t dset_id;          IN: Chunked dataset ID
 *              hid_t dxpl_id;          IN: Dataset transfer property list ID
 *              H5D_chunk_iter_op_t cb  IN: User callback function, called for
 *every chunk. void *op_data           IN/OUT: Optional user data passed on to
 *user callback.
 *
 * Return:      Non-negative on success, negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5Dchunk_iter(hid_t dset_id, hid_t dxpl_id, H5D_chunk_iter_op_t op,
                     void *op_data) {
  H5VL_object_t *vol_obj = NULL;    /* Dataset for this operation */
  H5VL_optional_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_native_dataset_optional_args_t
  ...
  if (NULL == op)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid callback to chunk iteration");
  ...
  if ((ret_value = H5VL_dataset_optional(vol_obj, &vol_cb_args, dxpl_id,
                                         H5_REQUEST_NULL)) < 0)
