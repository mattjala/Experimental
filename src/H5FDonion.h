...
typedef enum H5FD_onion_target_file_constant_t {
  H5FD_ONION_STORE_TARGET_ONION, /**<
                                  * Onion history is stored in a single,
                                  * separate "onion file". Shares filename and
                                  * path as hdf5 file (if any), with only a
...
typedef struct H5FD_onion_fapl_info_t {
  uint8_t version;       /**<
                          ...
                          * Must equal H5FD_ONION_FAPL_VERSION_CURR to be considered
                          * valid.
                          */
  hid_t backing_fapl_id; /**<
                          * Backing or 'child' FAPL ID to handle I/O with the
                          * underlying backing store. It must use the same
                          * backing driver as the original file.
                          */
  uint32_t page_size;    /**<
                          * page_size:   Size of the amended data pages. If opening
                          * an existing file,    must equal the existing page size or
                          * zero. If creating a new    file or an initial revision of
                          * an existing file, must be a    power of 2.
                          *
                          */
  H5FD_onion_target_file_constant_t
      store_target;         /**<
  ...
  uint64_t revision_num;    /**<
                             * Which revision to open. Valid values are 0 (the
                             * original file) or the    revision number of an existing
                             * revision.    H5FD_ONION_FAPL_INFO_REVISION_ID_LATEST
                             * refers to the most    recently-created revision in the
                             * history.
                             */
  uint8_t force_write_open; /**<
                             *              Flag to ignore the write-lock flag
                             * in the onion data and attempt to open the file
                             * write-only anyway. This may be relevant if, for
                             * example, the library crashed while the file was
                             * open in write mode and the write-lock flag was
                             * not cleared. Must equal
                             * H5FD_ONION_FAPL_FLAG_FORCE_OPEN to enable.
                             *
                             */
  uint8_t creation_flags;   /**<
                             * Flag used only when instantiating an onion file.
                             * If the relevant bit is set to a nonzero value, its
 ...
 * \since 1.14.0
 */
H5_DLL herr_t H5FDonion_get_revision_count(const char *filename, hid_t fapl_id,
