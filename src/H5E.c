...
static H5E_cls_t *H5E__register_class(const char *cls_name,
                                      const char *lib_name,
                                      const char *version);
static herr_t H5E__unregister_class(H5E_cls_t *cls, void **request);
static ssize_t H5E__get_class_name(const H5E_cls_t *cls, char *name,
                                   size_t size);
static int H5E__close_msg_cb(void *obj_ptr, hid_t obj_id, void *udata);
static herr_t H5E__close_msg(H5E_msg_t *err, void **request);
static H5E_msg_t *H5E__create_msg(H5E_cls_t *cls, H5E_type_t msg_type,
                                  const char *msg);
...
static const H5I_class_t H5I_ERRCLS_CLS[1] = {{
    ...
    (H5I_free_t)H5E__unregister_class /* Callback routine for closing objects of
                                         this class */
}};
...
static const H5I_class_t H5I_ERRSTK_CLS[1] = {{
    ...
    0,                           /* Class flags */
    0,                           /* # of reserved IDs for class */
    (H5I_free_t)H5E__close_stack /* Callback routine for closing objects of this
                                    class */
}};
...
herr_t H5E_init(void) {
  ...
  HDsnprintf(lib_vers, sizeof(lib_vers), "%u.%u.%u%s", H5_VERS_MAJOR,
             H5_VERS_MINOR, H5_VERS_RELEASE,
             (HDstrlen(H5_VERS_SUBRELEASE) > 0 ? "-" H5_VERS_SUBRELEASE : ""));
  if (NULL ==
      (cls = H5E__register_class(H5E_CLS_NAME, H5E_CLS_LIB_NAME, lib_vers)))
  ...
  if ((H5E_ERR_CLS_g = H5I_register(H5I_ERROR_CLASS, cls, FALSE)) < 0)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTREGISTER, FAIL,
                "can't register error class");
...
hid_t H5Eregister_class(const char *cls_name, const char *lib_name,
  ...
  if (NULL == (cls = H5E__register_class(cls_name, lib_name, version)))
    HGOTO_ERROR(H5E_ERROR, H5E_CANTCREATE, H5I_INVALID_HID,
                "can't create error class");

  /* Register the new error class to get an ID for it */
  if ((ret_value = H5I_register(H5I_ERROR_CLASS, cls, TRUE)) < 0)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTREGISTER, H5I_INVALID_HID,
                "can't register error class");
...
static H5E_cls_t *H5E__register_class(const char *cls_name,
  ...
  if (!ret_value)
    if (cls && H5E__free_class(cls) < 0)
      HDONE_ERROR(H5E_ERROR, H5E_CANTRELEASE, NULL,
                  "unable to free error class");
...
herr_t H5Eunregister_class(hid_t class_id) {
  ...
  if (H5I_dec_app_ref(class_id) < 0)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTDEC, FAIL,
                "unable to decrement ref count on error class");
...
static herr_t H5E__unregister_class(H5E_cls_t *cls,
  ...
  if (H5I_get_first(H5I_ERROR_MSG, &target_id, &object, FALSE) < 0)
    ...
    HGOTO_ERROR(H5E_ERROR, H5E_BADITER, FAIL,
                "unable to get first error message");
  ...
  while (target_id != 0) {
    ...
    if (err_msg->cls == cls) {
      ...
      if (H5E__close_msg(err_msg, NULL) < 0)
        ...
        HGOTO_ERROR(H5E_ERROR, H5E_CANTCLOSEOBJ, H5_ITER_ERROR,
                    "unable to close error message");

      if (NULL == H5I_remove(target_id))

        HGOTO_ERROR(H5E_ERROR, H5E_CANTREMOVE, H5_ITER_ERROR,
    ...
    if (H5I_get_next(H5I_ERROR_MSG, target_id, &target_id, &object, FALSE) < 0)
      ...
      HGOTO_ERROR(H5E_ERROR, H5E_BADITER, FAIL,
                  "unable to get next error message");
  }
...
static int H5E__close_msg_cb(void *obj_ptr, hid_t obj_id, void *udata) {
  ...
  /* H5E__close_msg() checks to see if it has the global mutex in the
   * multi-thread case, so no need to test here in the multi-thread case.
   */
...
herr_t H5Eclose_msg(hid_t err_id) {
  ...
  if (H5I_dec_app_ref(err_id) < 0)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTDEC, FAIL,
                "unable to decrement ref count on error message");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5E__close_msg(H5E_msg_t *err, void H5_ATTR_UNUSED **request) {
#ifdef H5_HAVE_MULTITHREAD
  bool have_global_mutex;
#endif /* H5_HAVE_MULTITHREAD */
...
static H5E_msg_t *H5E__create_msg(H5E_cls_t *cls, H5E_type_t msg_type,
  ...
  /* Don't need global lock here, since all activites in this function are
   * thread local */
  ...
  if (!ret_value)
    if (msg && H5E__close_msg(msg, NULL) < 0)
      HDONE_ERROR(H5E_ERROR, H5E_CANTCLOSEOBJ, NULL,
                  "unable to close error message");
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Eget_msg(hid_t msg_id, H5E_type_t *type /*out*/,
                   char *msg_str /*out*/, size_t size) {
...
static H5E_t *H5E__get_current_stack(void) {
  ...
  if (NULL ==
      (current_stack =
           H5E__get_my_stack())) /*lint !e506 !e774 Make lint 'constant value
                                    Boolean' in non-threaded case */
  ...
  for (u = 0; u < current_stack->nused; u++) {
    H5E_error2_t *current_error,
        *new_error; /* Pointers to errors on each stack */
    ...
    if (H5I_inc_ref(current_error->cls_id, FALSE) < 0)
      HGOTO_ERROR(H5E_ERROR, H5E_CANTINC, NULL,
                  "unable to increment ref count on error class");
    new_error->cls_id = current_error->cls_id;
    if (H5I_inc_ref(current_error->maj_num, FALSE) < 0)
      HGOTO_ERROR(H5E_ERROR, H5E_CANTINC, NULL,
                  "unable to increment ref count on error message");
...
herr_t H5Eset_current_stack(hid_t err_stack) {
  ...
  if (err_stack != H5E_DEFAULT) {
    if (NULL ==
        (estack = (H5E_t *)H5I_object_verify(err_stack, H5I_ERROR_STACK)))
    ...
    if (H5I_dec_app_ref(err_stack) < 0)
      HGOTO_ERROR(H5E_ERROR, H5E_CANTDEC, FAIL,
                  "unable to decrement ref count on error stack");
  } /* end if */
...
static herr_t H5E__set_current_stack(H5E_t *estack) {
  ...
  /* Don't need to verify that we have the global mutex in the multi-thread
   * case, as the only place this function is called is H5Eset_current_stack()
   */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5E__close_stack(H5E_t *estack, void H5_ATTR_UNUSED **request) {
  FUNC_ENTER_PACKAGE_NOERR
  ...
  if (error_stack_id == H5E_DEFAULT) {
    if (NULL ==
        (estack =
             H5E__get_my_stack())) /*lint !e506 !e774 Make lint 'constant value
                                      Boolean' in non-threaded case */
      HGOTO_ERROR(H5E_ERROR, H5E_CANTGET, (-1),
                  "can't get current error stack");
  } /* end if */
  else {
    ...
    if (NULL ==
        (estack = (H5E_t *)H5I_object_verify(error_stack_id, H5I_ERROR_STACK)))
 ...
 *-------------------------------------------------------------------------
 */
static ssize_t H5E__get_num(const H5E_t *estack) {
  FUNC_ENTER_PACKAGE_NOERR
  ...
  if (err_stack == H5E_DEFAULT) {
    if (NULL ==
        (estack =
             H5E__get_my_stack())) /*lint !e506 !e774 Make lint 'constant value
                                      Boolean' in non-threaded case */
      HGOTO_ERROR(H5E_ERROR, H5E_CANTGET, FAIL,
                  "can't get current error stack");
  } /* end if */
  else {
    /* Only clear the error stack if it's not the default stack */
...
herr_t H5Epush2(hid_t err_stack, const char *file, const char *func,
   ...
   *      the H5E_printf_stack() routine - correct errors and make changes in
   * both places. -QAK
   */
  ...
  if (H5E__push_stack(estack, file, func, line, cls_id, maj_id, min_id, tmp) <
      0)
...
static herr_t H5E__print2(hid_t err_stack, FILE *stream) {
  ...
  if (err_stack == H5E_DEFAULT) {
    ...
    if (!have_global_mutex)
      ...
      HGOTO_ERROR(H5E_LIB, H5E_SYSTEM, FAIL,
                  "Don't have global mutex on entry.");
...
herr_t H5Ewalk2(hid_t err_stack, H5E_direction_t direction,
                H5E_walk2_t stack_func, void *client_data) {
  H5E_t *estack;              /* Error stack to operate on */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Eget_auto2(hid_t estack_id, H5E_auto2_t *func /*out*/,
  ...
  if (estack_id == H5E_DEFAULT) {
    ...
    if (NULL ==
        (estack = (H5E_t *)H5I_object_verify(estack_id, H5I_ERROR_STACK)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a error stack ID");
  } /* end else */
...
#ifndef H5_NO_DEPRECATED_SYMBOLS
  /* Fail if the printing function isn't the default(user-set) and set through
   * H5Eset_auto1 */
  if (!op.is_default && op.vers == 1)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTGET, FAIL,
                "wrong API function, H5Eset_auto1 has been called");
#endif /* H5_NO_DEPRECATED_SYMBOLS */
...
herr_t H5Eappend_stack(hid_t dst_stack_id, hid_t src_stack_id,
  ...
  /* Check args */
  if (NULL ==
      (dst_stack = (H5E_t *)H5I_object_verify(dst_stack_id, H5I_ERROR_STACK)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "dst_stack_id not a error stack ID");
  if (NULL ==
      (src_stack = (H5E_t *)H5I_object_verify(src_stack_id, H5I_ERROR_STACK)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "src_stack_id not a error stack ID");
  ...
  if (close_source_stack)
    ...
    if (H5I_dec_app_ref(src_stack_id) < 0)
      HGOTO_ERROR(H5E_ERROR, H5E_CANTDEC, FAIL,
                  "unable to decrement ref count on source error stack");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5E__append_stack(H5E_t *dst_stack, const H5E_t *src_stack) {
  unsigned u;                 /* Local index variable */
  herr_t ret_value = SUCCEED; /* Return value */
