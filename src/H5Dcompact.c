...
static herr_t H5D__compact_iovv_memmanage_cb(hsize_t dst_off, hsize_t src_off,
...
static ssize_t H5D__compact_writevv(
    const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info,
    size_t dset_max_nseq, size_t *dset_curr_seq, size_t dset_size_arr[],
    hsize_t dset_offset_arr[], size_t mem_max_nseq, size_t *mem_curr_seq,
    size_t mem_size_arr[], hsize_t mem_offset_arr[]);
...
herr_t H5D__compact_fill(const H5D_t *dset) {
  ...
  if (H5D__fill_init(&fb_info, dset->shared->layout.storage.u.compact.buf, NULL,
                     NULL, NULL, NULL, &dset->shared->dcpl_cache.fill,
...
static herr_t H5D__compact_construct(H5F_t *f, H5D_t *dset) {
  ...
  for (u = 0; u < dset->shared->ndims; u++)
    if (dset->shared->max_dims[u] > dset->shared->curr_dims[u])
      HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                  "extendible compact dataset not allowed");
  ...
  H5_CHECKED_ASSIGN(dset->shared->layout.storage.u.compact.size, size_t,
                    tmp_size, hssize_t);

  /* Verify data size is smaller than maximum header message size
   * (64KB) minus other layout message fields.
   */
  max_comp_data_size = H5O_MESG_MAX_SIZE -
                       H5D__layout_meta_size(f, &(dset->shared->layout), FALSE);
...
static herr_t H5D__compact_io_init(H5D_io_info_t *io_info,
  ...
  dinfo->store->compact.dirty =
      &dinfo->dset->shared->layout.storage.u.compact.dirty;
...
static herr_t H5D__compact_iovv_memmanage_cb(hsize_t dst_off, hsize_t src_off,
  ...
  H5D_compact_iovv_memmanage_ud_t *udata =
      (H5D_compact_iovv_memmanage_ud_t *)_udata;
  ...
  op_flags =
      H5FD_CTL_ROUTE_TO_TERMINAL_VFD_FLAG | H5FD_CTL_FAIL_IF_UNKNOWN_FLAG;
...
static ssize_t H5D__compact_readvv(
  ...
  if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_MEMMANAGE)) {
    ...
    if ((ret_value = H5VM_opvv(mem_max_nseq, mem_curr_seq, mem_size_arr,
                               mem_offset_arr, dset_max_nseq, dset_curr_seq,
                               dset_size_arr, dset_offset_arr,
                               H5D__compact_iovv_memmanage_cb, &udata)) < 0)
      HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL, "vectorized memcpy failed");
  } else {
    /* Use the vectorized memory copy routine to do actual work */
    if ((ret_value = H5VM_memcpyvv(
             dset_info->buf.vp, mem_max_nseq, mem_curr_seq, mem_size_arr,
...
static ssize_t H5D__compact_writevv(
  ...
  if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_MEMMANAGE)) {
    ...
    /* Request that file driver does the memory copy */
    if ((ret_value = H5VM_opvv(dset_max_nseq, dset_curr_seq, dset_size_arr,
                               dset_offset_arr, mem_max_nseq, mem_curr_seq,
                               mem_size_arr, mem_offset_arr,
                               H5D__compact_iovv_memmanage_cb, &udata)) < 0)
      HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL, "vectorized memcpy failed");
  } else {
    /* Use the vectorized memory copy routine to do actual work */
    if ((ret_value = H5VM_memcpyvv(
             dset_info->store->compact.buf, dset_max_nseq, dset_curr_seq,
...
static herr_t H5D__compact_flush(H5D_t *dset) {
  ...
  if (dset->shared->layout.storage.u.compact.dirty) {
    ...
    if (H5O_msg_write(&(dset->oloc), H5O_LAYOUT_ID, 0, H5O_UPDATE_TIME,
                      &(dset->shared->layout)) < 0) {
...
static herr_t H5D__compact_dest(H5D_t *dset) {
  ...
  dset->shared->layout.storage.u.compact.buf =
      H5MM_xfree(dset->shared->layout.storage.u.compact.buf);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__compact_copy(H5F_t *f_src, H5O_storage_compact_t *_storage_src,
                         H5F_t *f_dst, H5O_storage_compact_t *storage_dst,
  ...
  H5O_storage_compact_t *storage_src =
      _storage_src;           /* Pointer to storage_src */
  herr_t ret_value = SUCCEED; /* Return value */
    ...
    if (f_src != f_dst) {
      ...
      if (cpy_info->expand_ref) {
        ...
        if (H5O_copy_expand_ref(f_src, tid_src, dt_src, storage_src->buf,
                                storage_src->size, f_dst, storage_dst->buf,
                                cpy_info) < 0)
