...
/* Definition for dropping free-space to the floor when reading in the
 * superblock */
...
#define H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_SIZE                              \
  sizeof(H5AC_cache_image_config_t)
#define H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_DEF                               \
  H5AC__DEFAULT_CACHE_IMAGE_CONFIG
#define H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_ENC                               \
  H5P__facc_cache_image_config_enc
#define H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_DEC                               \
  H5P__facc_cache_image_config_dec
#define H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_CMP                               \
  H5P__facc_cache_image_config_cmp
...
static herr_t H5P__facc_file_driver_create(const char *name, size_t size,
                                           void *value);
static herr_t H5P__facc_file_driver_set(hid_t prop_id, const char *name,
                                        size_t size, void *value);
static herr_t H5P__facc_file_driver_get(hid_t prop_id, const char *name,
                                        size_t size, void *value);
static herr_t H5P__facc_file_driver_del(hid_t prop_id, const char *name,
                                        size_t size, void *value);
static herr_t H5P__facc_file_driver_copy(const char *name, size_t size,
                                         void *value);
static int H5P__facc_file_driver_cmp(const void *value1, const void *value2,
                                     size_t size);
static herr_t H5P__facc_file_driver_close(const char *name, size_t size,
                                          void *value);
...
static herr_t H5P__facc_file_image_info_set(hid_t prop_id, const char *name,
                                            size_t size, void *value);
static herr_t H5P__facc_file_image_info_get(hid_t prop_id, const char *name,
                                            size_t size, void *value);
static herr_t H5P__facc_file_image_info_del(hid_t prop_id, const char *name,
                                            size_t size, void *value);
static herr_t H5P__facc_file_image_info_copy(const char *name, size_t size,
                                             void *value);
static int H5P__facc_file_image_info_cmp(const void *value1, const void *value2,
                                         size_t size);
static herr_t H5P__facc_file_image_info_close(const char *name, size_t size,
                                              void *value);

/* encode & decode callbacks */
static herr_t H5P__facc_cache_config_enc(const void *value, void **_pp,
                                         size_t *size);
static herr_t H5P__facc_cache_config_dec(const void **_pp, void *value);
static int H5P__facc_cache_config_cmp(const void *value1, const void *value2,
                                      size_t size);
static herr_t H5P__facc_fclose_degree_enc(const void *value, void **_pp,
                                          size_t *size);
static herr_t H5P__facc_fclose_degree_dec(const void **pp, void *value);
static herr_t H5P__facc_multi_type_enc(const void *value, void **_pp,
                                       size_t *size);
static herr_t H5P__facc_multi_type_dec(const void **_pp, void *value);
static herr_t H5P__facc_libver_type_enc(const void *value, void **_pp,
                                        size_t *size);
static herr_t H5P__facc_libver_type_dec(const void **_pp, void *value);

/* Metadata cache log location property callbacks */
static herr_t H5P__facc_mdc_log_location_enc(const void *value, void **_pp,
                                             size_t *size);
static herr_t H5P__facc_mdc_log_location_dec(const void **_pp, void *value);
static herr_t H5P__facc_mdc_log_location_del(hid_t prop_id, const char *name,
                                             size_t size, void *value);
static herr_t H5P__facc_mdc_log_location_copy(const char *name, size_t size,
                                              void *value);
static int H5P__facc_mdc_log_location_cmp(const void *value1,
                                          const void *value2, size_t size);
static herr_t H5P__facc_mdc_log_location_close(const char *name, size_t size,
                                               void *value);

/* Metadata cache image property callbacks */
static int H5P__facc_cache_image_config_cmp(const void *_config1,
                                            const void *_config2,
                                            size_t H5_ATTR_UNUSED size);
static herr_t H5P__facc_cache_image_config_enc(const void *value, void **_pp,
                                               size_t *size);
...
static herr_t H5P__facc_vol_set(hid_t prop_id, const char *name, size_t size,
                                void *value);
static herr_t H5P__facc_vol_get(hid_t prop_id, const char *name, size_t size,
                                void *value);
static herr_t H5P__facc_vol_del(hid_t prop_id, const char *name, size_t size,
                                void *value);
static herr_t H5P__facc_vol_copy(const char *name, size_t size, void *value);
static int H5P__facc_vol_cmp(const void *value1, const void *value2,
                             size_t size);
static herr_t H5P__facc_vol_close(const char *name, size_t size, void *value);

#ifdef H5_HAVE_PARALLEL
/* MPI communicator callbacks */
static herr_t H5P__facc_mpi_comm_set(hid_t prop_id, const char *name,
                                     size_t size, void *value);
static herr_t H5P__facc_mpi_comm_get(hid_t prop_id, const char *name,
                                     size_t size, void *value);
static herr_t H5P__facc_mpi_comm_del(hid_t prop_id, const char *name,
                                     size_t size, void *value);
static herr_t H5P__facc_mpi_comm_copy(const char *name, size_t size,
                                      void *value);
static int H5P__facc_mpi_comm_cmp(const void *value1, const void *value2,
                                  size_t size);
static herr_t H5P__facc_mpi_comm_close(const char *name, size_t size,
                                       void *value);

/* MPI info callbacks */
static herr_t H5P__facc_mpi_info_set(hid_t prop_id, const char *name,
                                     size_t size, void *value);
static herr_t H5P__facc_mpi_info_get(hid_t prop_id, const char *name,
                                     size_t size, void *value);
static herr_t H5P__facc_mpi_info_del(hid_t prop_id, const char *name,
                                     size_t size, void *value);
static herr_t H5P__facc_mpi_info_copy(const char *name, size_t size,
                                      void *value);
static int H5P__facc_mpi_info_cmp(const void *value1, const void *value2,
                                  size_t size);
static herr_t H5P__facc_mpi_info_close(const char *name, size_t size,
                                       void *value);
#endif /* H5_HAVE_PARALLEL */

/* Internal routines */
static herr_t H5P__facc_set_def_driver_check_predefined(const char *driver_name,
                                                        hid_t *driver_id);
...
static const hsize_t H5F_def_alignment_g =
    H5F_ACS_ALIGN_DEF; /* Default allocation alignment value */
...
static const unsigned H5F_def_gc_ref_g =
    H5F_ACS_GARBG_COLCT_REF_DEF; /* Default garbage collection for references
                                    setting */
static const H5F_close_degree_t H5F_def_close_degree_g =
    H5F_CLOSE_DEGREE_DEF; /* Default file close degree */
static const hsize_t H5F_def_family_offset_g =
    H5F_ACS_FAMILY_OFFSET_DEF; /* Default offset for family VFD */
...
static const H5F_libver_t H5F_def_libver_low_bound_g =
    H5F_ACS_LIBVER_LOW_BOUND_DEF; /* Default setting for "low" bound of format
                                     version */
static const H5F_libver_t H5F_def_libver_high_bound_g =
    H5F_ACS_LIBVER_HIGH_BOUND_DEF; /* Default setting for "high" bound of format
                                      version */

static const hbool_t H5F_def_want_posix_fd_g =
    H5F_ACS_WANT_POSIX_FD_DEF; /* Default setting for retrieving 'handle' from
                                  core VFD */
static const unsigned H5F_def_efc_size_g =
    H5F_ACS_EFC_SIZE_DEF; /* Default external file cache size */
...
static const unsigned H5F_def_metadata_read_attempts_g =
    H5F_ACS_METADATA_READ_ATTEMPTS_DEF; /* Default setting for the # of metadata
                                           read attempts */
...
static const hbool_t H5F_def_clear_status_flags_g =
    H5F_ACS_CLEAR_STATUS_FLAGS_DEF; /* Default to clear the superblock
                                       status_flags */
...
static const hbool_t H5F_def_null_fsm_addr_g =
    H5F_ACS_NULL_FSM_ADDR_DEF; /* Default setting for dropping free-space to the
                                  floor */

static const hbool_t H5F_def_use_mdc_logging_g =
    H5F_ACS_USE_MDC_LOGGING_DEF; /* Default metadata cache logging flag */
static const char *H5F_def_mdc_log_location_g =
    H5F_ACS_MDC_LOG_LOCATION_DEF; /* Default mdc log location */
...
static const H5P_coll_md_read_flag_t H5F_def_coll_md_read_flag_g =
    H5F_ACS_COLL_MD_READ_FLAG_DEF; /* Default setting for the collective
                                      metedata read flag */
static const hbool_t H5F_def_coll_md_write_flag_g =
    H5F_ACS_COLL_MD_WRITE_FLAG_DEF; /* Default setting for the collective
                                       metedata write flag */
static const MPI_Comm H5F_def_mpi_params_comm_g =
    H5F_ACS_MPI_PARAMS_COMM_DEF; /* Default MPI communicator */
static const MPI_Info H5F_def_mpi_params_info_g =
    H5F_ACS_MPI_PARAMS_INFO_DEF; /* Default MPI info struct */
#endif                           /* H5_HAVE_PARALLEL */
static const H5AC_cache_image_config_t H5F_def_mdc_initCacheImageCfg_g =
    H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_DEF; /* Default metadata cache image
                                                 settings */
static const size_t H5F_def_page_buf_size_g =
    H5F_ACS_PAGE_BUFFER_SIZE_DEF; /* Default page buffer size */
static const unsigned H5F_def_page_buf_min_meta_perc_g =
    H5F_ACS_PAGE_BUFFER_MIN_META_PERC_DEF; /* Default page buffer minimum
                                              metadata size */
static const unsigned H5F_def_page_buf_min_raw_perc_g =
    H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_DEF; /* Default page buffer minimum raw
                                             data size */
...
static const hbool_t H5F_def_ignore_disabled_file_locks_g =
    H5F_ACS_IGNORE_DISABLED_FILE_LOCKS_DEF; /* Default ignore disabled file
                                               locks flag */
...
static herr_t H5P__facc_reg_prop(H5P_genclass_t *pclass) {
  const H5FD_driver_prop_t def_driver_prop =
      H5F_ACS_FILE_DRV_DEF; /* Default VFL driver ID & info (initialized from a
                               variable) */
  const H5VL_connector_prop_t def_vol_prop =
      H5F_ACS_VOL_CONN_DEF; /* Default VOL connector ID & info (initialized from
                               a variable) */
  ...
  if (H5P__register_real(pclass, H5F_ACS_META_CACHE_INIT_CONFIG_NAME,
                         H5F_ACS_META_CACHE_INIT_CONFIG_SIZE,
                         &H5F_def_mdc_initCacheCfg_g, NULL, NULL, NULL,
                         H5F_ACS_META_CACHE_INIT_CONFIG_ENC,
                         H5F_ACS_META_CACHE_INIT_CONFIG_DEC, NULL, NULL,
                         H5F_ACS_META_CACHE_INIT_CONFIG_CMP, NULL) < 0)
  ...
  if (H5P__register_real(
          ...
          H5F_ACS_DATA_CACHE_NUM_SLOTS_SIZE, &H5F_def_rdcc_nslots_g, NULL, NULL,
          NULL, H5F_ACS_DATA_CACHE_NUM_SLOTS_ENC,
  ...
  if (H5P__register_real(
          ...
          H5F_ACS_DATA_CACHE_BYTE_SIZE_SIZE, &H5F_def_rdcc_nbytes_g, NULL, NULL,
          NULL, H5F_ACS_DATA_CACHE_BYTE_SIZE_ENC,
          H5F_ACS_DATA_CACHE_BYTE_SIZE_DEC, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the preemption for reading chunks */
  if (H5P__register_real(pclass, H5F_ACS_PREEMPT_READ_CHUNKS_NAME,
                         H5F_ACS_PREEMPT_READ_CHUNKS_SIZE, &H5F_def_rdcc_w0_g,
                         NULL, NULL, NULL, H5F_ACS_PREEMPT_READ_CHUNKS_ENC,
                         H5F_ACS_PREEMPT_READ_CHUNKS_DEC, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the threshold for alignment */
  if (H5P__register_real(pclass, H5F_ACS_ALIGN_THRHD_NAME,
                         H5F_ACS_ALIGN_THRHD_SIZE, &H5F_def_threshold_g, NULL,
                         NULL, NULL, H5F_ACS_ALIGN_THRHD_ENC,
                         H5F_ACS_ALIGN_THRHD_DEC, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the alignment */
  if (H5P__register_real(pclass, H5F_ACS_ALIGN_NAME, H5F_ACS_ALIGN_SIZE,
                         &H5F_def_alignment_g, NULL, NULL, NULL,
                         H5F_ACS_ALIGN_ENC, H5F_ACS_ALIGN_DEC, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          ...
          &H5F_def_meta_block_size_g, NULL, NULL, NULL,
          H5F_ACS_META_BLOCK_SIZE_ENC, H5F_ACS_META_BLOCK_SIZE_DEC, NULL, NULL,
          NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the maximum sieve buffer size */
  if (H5P__register_real(pclass, H5F_ACS_SIEVE_BUF_SIZE_NAME,
                         H5F_ACS_SIEVE_BUF_SIZE_SIZE, &H5F_def_sieve_buf_size_g,
                         NULL, NULL, NULL, H5F_ACS_SIEVE_BUF_SIZE_ENC,
                         H5F_ACS_SIEVE_BUF_SIZE_DEC, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          ...
          &H5F_def_sdata_block_size_g, NULL, NULL, NULL,
          H5F_ACS_SDATA_BLOCK_SIZE_ENC, H5F_ACS_SDATA_BLOCK_SIZE_DEC, NULL,
          NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(pclass, H5F_ACS_FILE_DRV_NAME, H5F_ACS_FILE_DRV_SIZE,
                         &def_driver_prop, H5F_ACS_FILE_DRV_CRT,
                         H5F_ACS_FILE_DRV_SET, H5F_ACS_FILE_DRV_GET, NULL, NULL,
                         H5F_ACS_FILE_DRV_DEL, H5F_ACS_FILE_DRV_COPY,
                         H5F_ACS_FILE_DRV_CMP, H5F_ACS_FILE_DRV_CLOSE) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the file close degree */
  if (H5P__register_real(pclass, H5F_ACS_CLOSE_DEGREE_NAME,
                         H5F_CLOSE_DEGREE_SIZE, &H5F_def_close_degree_g, NULL,
                         NULL, NULL, H5F_CLOSE_DEGREE_ENC, H5F_CLOSE_DEGREE_DEC,
                         NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the offset of family driver info */
  if (H5P__register_real(pclass, H5F_ACS_FAMILY_OFFSET_NAME,
                         H5F_ACS_FAMILY_OFFSET_SIZE, &H5F_def_family_offset_g,
                         NULL, NULL, NULL, H5F_ACS_FAMILY_OFFSET_ENC,
                         H5F_ACS_FAMILY_OFFSET_DEC, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the private property of new family file size. It's used by
   * h5repart only. */
  /* (Note: this property should not have an encode/decode callback -QAK) */
  if (H5P__register_real(pclass, H5F_ACS_FAMILY_NEWSIZE_NAME,
                         H5F_ACS_FAMILY_NEWSIZE_SIZE, &H5F_def_family_newsize_g,
                         NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the private property of whether convert family to a single-file
   * driver. It's used by h5repart only. */
  /* (Note: this property should not have an encode/decode callback -QAK) */
  if (H5P__register_real(pclass, H5F_ACS_FAMILY_TO_SINGLE_NAME,
                         H5F_ACS_FAMILY_TO_SINGLE_SIZE,
                         &H5F_def_family_to_single_g, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the data type of multi driver info */
  if (H5P__register_real(pclass, H5F_ACS_MULTI_TYPE_NAME,
                         H5F_ACS_MULTI_TYPE_SIZE, &H5F_def_mem_type_g, NULL,
                         NULL, NULL, H5F_ACS_MULTI_TYPE_ENC,
                         H5F_ACS_MULTI_TYPE_DEC, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          ...
          &H5F_def_libver_low_bound_g, NULL, NULL, NULL,
          H5F_ACS_LIBVER_LOW_BOUND_ENC, H5F_ACS_LIBVER_LOW_BOUND_DEC, NULL,
          NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          pclass, H5F_ACS_LIBVER_HIGH_BOUND_NAME,
          H5F_ACS_LIBVER_HIGH_BOUND_SIZE, &H5F_def_libver_high_bound_g, NULL,
          NULL, NULL, H5F_ACS_LIBVER_HIGH_BOUND_ENC,
          H5F_ACS_LIBVER_HIGH_BOUND_DEC, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the private property of whether to retrieve the file descriptor
   * from the core VFD */
  /* (used internally to the library only) */
  /* (Note: this property should not have an encode/decode callback -QAK) */
  if (H5P__register_real(pclass, H5F_ACS_WANT_POSIX_FD_NAME,
                         H5F_ACS_WANT_POSIX_FD_SIZE, &H5F_def_want_posix_fd_g,
                         NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(pclass, H5F_ACS_EFC_SIZE_NAME, H5F_ACS_EFC_SIZE_SIZE,
                         &H5F_def_efc_size_g, NULL, NULL, NULL,
                         H5F_ACS_EFC_SIZE_ENC, H5F_ACS_EFC_SIZE_DEC, NULL, NULL,
                         NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  /* (Note: this property should not have an encode/decode callback -QAK) */
  if (H5P__register_real(
          pclass, H5F_ACS_FILE_IMAGE_INFO_NAME, H5F_ACS_FILE_IMAGE_INFO_SIZE,
  ...
  /* (Note: this property should not have an encode/decode callback -QAK) */
  if (H5P__register_real(pclass, H5F_ACS_OBJECT_FLUSH_CB_NAME,
                         H5F_ACS_OBJECT_FLUSH_CB_SIZE,
                         &H5F_def_object_flush_cb_g, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the private property of whether to clear the superblock
   * status_flags. It's used by h5clear only. */
  if (H5P__register_real(pclass, H5F_ACS_CLEAR_STATUS_FLAGS_NAME,
                         H5F_ACS_CLEAR_STATUS_FLAGS_SIZE,
                         &H5F_def_clear_status_flags_g, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the private property of whether to skip EOF check. It's used by
   * h5clear only. */
  if (H5P__register_real(pclass, H5F_ACS_SKIP_EOF_CHECK_NAME,
                         H5F_ACS_SKIP_EOF_CHECK_SIZE, &H5F_def_skip_eof_check_g,
                         NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the private property of whether to drop free-space to the floor.
   * It's used by h5clear only. */
  if (H5P__register_real(pclass, H5F_ACS_NULL_FSM_ADDR_NAME,
                         H5F_ACS_NULL_FSM_ADDR_SIZE, &H5F_def_null_fsm_addr_g,
                         NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          pclass, H5F_ACS_USE_MDC_LOGGING_NAME, H5F_ACS_USE_MDC_LOGGING_SIZE,
          &H5F_def_use_mdc_logging_g, NULL, NULL, NULL,
          H5F_ACS_USE_MDC_LOGGING_ENC, H5F_ACS_USE_MDC_LOGGING_DEC, NULL, NULL,
          NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          pclass, H5F_ACS_MDC_LOG_LOCATION_NAME, H5F_ACS_MDC_LOG_LOCATION_SIZE,
          &H5F_def_mdc_log_location_g, NULL, NULL, NULL,
          H5F_ACS_MDC_LOG_LOCATION_ENC, H5F_ACS_MDC_LOG_LOCATION_DEC,
          H5F_ACS_MDC_LOG_LOCATION_DEL, H5F_ACS_MDC_LOG_LOCATION_COPY,
          H5F_ACS_MDC_LOG_LOCATION_CMP, H5F_ACS_MDC_LOG_LOCATION_CLOSE) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the flag that indicates whether mdc logging starts on file access.
   */
  if (H5P__register_real(pclass, H5F_ACS_START_MDC_LOG_ON_ACCESS_NAME,
                         H5F_ACS_START_MDC_LOG_ON_ACCESS_SIZE,
                         &H5F_def_start_mdc_log_on_access_g, NULL, NULL, NULL,
                         H5F_ACS_START_MDC_LOG_ON_ACCESS_ENC,
                         H5F_ACS_START_MDC_LOG_ON_ACCESS_DEC, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          pclass, H5F_ACS_EVICT_ON_CLOSE_FLAG_NAME,
          H5F_ACS_EVICT_ON_CLOSE_FLAG_SIZE, &H5F_def_evict_on_close_flag_g,
          NULL, NULL, NULL, H5F_ACS_EVICT_ON_CLOSE_FLAG_ENC,
  ...
  if (H5P__register_real(
          pclass, H5_COLL_MD_READ_FLAG_NAME, H5F_ACS_COLL_MD_READ_FLAG_SIZE,
          &H5F_def_coll_md_read_flag_g, NULL, NULL, NULL,
          H5F_ACS_COLL_MD_READ_FLAG_ENC, H5F_ACS_COLL_MD_READ_FLAG_DEC, NULL,
          NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(
          pclass, H5F_ACS_COLL_MD_WRITE_FLAG_NAME,
          H5F_ACS_COLL_MD_WRITE_FLAG_SIZE, &H5F_def_coll_md_write_flag_g, NULL,
          NULL, NULL, H5F_ACS_COLL_MD_WRITE_FLAG_ENC,
  ...
  if (H5P__register_real(
          pclass, H5F_ACS_MPI_PARAMS_COMM_NAME, H5F_ACS_MPI_PARAMS_COMM_SIZE,
  ...
  if (H5P__register_real(pclass, H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_NAME,
                         ...
                         &H5F_def_mdc_initCacheImageCfg_g, NULL, NULL, NULL,
  ...
  if (H5P__register_real(
          pclass, H5F_ACS_PAGE_BUFFER_SIZE_NAME, H5F_ACS_PAGE_BUFFER_SIZE_SIZE,
          &H5F_def_page_buf_size_g, NULL, NULL, NULL,
          H5F_ACS_PAGE_BUFFER_SIZE_ENC, H5F_ACS_PAGE_BUFFER_SIZE_DEC, NULL,
          NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(pclass, H5F_ACS_PAGE_BUFFER_MIN_META_PERC_NAME,
                         ...
                         &H5F_def_page_buf_min_meta_perc_g, NULL, NULL, NULL,
                         H5F_ACS_PAGE_BUFFER_MIN_META_PERC_ENC,
                         H5F_ACS_PAGE_BUFFER_MIN_META_PERC_DEC, NULL, NULL,
                         NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(pclass, H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_NAME,
                         ...
                         &H5F_def_page_buf_min_raw_perc_g, NULL, NULL, NULL,
                         H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_ENC,
                         H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_DEC, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  /* (Note: this property should not have an encode/decode callback -QAK) */
  if (H5P__register_real(pclass, H5F_ACS_VOL_CONN_NAME, H5F_ACS_VOL_CONN_SIZE,
                         &def_vol_prop, H5F_ACS_VOL_CONN_CRT,
                         H5F_ACS_VOL_CONN_SET, H5F_ACS_VOL_CONN_GET, NULL, NULL,
                         H5F_ACS_VOL_CONN_DEL, H5F_ACS_VOL_CONN_COPY,
                         H5F_ACS_VOL_CONN_CMP, H5F_ACS_VOL_CONN_CLOSE) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the use file locking flag */
  if (H5P__register_real(
          pclass, H5F_ACS_USE_FILE_LOCKING_NAME, H5F_ACS_USE_FILE_LOCKING_SIZE,
          &H5F_def_use_file_locking_g, NULL, NULL, NULL,
          H5F_ACS_USE_FILE_LOCKING_ENC, H5F_ACS_USE_FILE_LOCKING_DEC, NULL,
          NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
  ...
  if (H5P__register_real(pclass, H5F_ACS_IGNORE_DISABLED_FILE_LOCKS_NAME,
                         ...
                         &H5F_def_ignore_disabled_file_locks_g, NULL, NULL,
                         NULL, H5F_ACS_IGNORE_DISABLED_FILE_LOCKS_ENC,
                         H5F_ACS_IGNORE_DISABLED_FILE_LOCKS_DEC, NULL, NULL,
                         NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
...
herr_t H5P__facc_set_def_driver(void) {
  ...
  if (driver_env_var && *driver_env_var) {
    ...
    if ((driver_is_registered =
             H5FD_is_driver_registered_by_name(driver_env_var, &driver_id)) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                  "can't check if VFL driver is already registered");
    if (driver_is_registered) {
      assert(driver_id >= 0);

      if (H5I_inc_ref(driver_id, TRUE) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTINC, FAIL,
                    "unable to increment ref count on VFD");
      driver_ref_inc = TRUE;
    } /* end else-if */
    else {
      /* Check for VFL drivers that ship with the library */
      if (H5P__facc_set_def_driver_check_predefined(driver_env_var,
                                                    &driver_id) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                    "can't check for predefined VFL driver name");
      else if (driver_id > 0) {
        if (H5I_inc_ref(driver_id, TRUE) < 0)
          HGOTO_ERROR(H5E_VFL, H5E_CANTINC, FAIL,
                      "can't increment VFL driver refcount");
        driver_ref_inc = TRUE;
      } else {
        /* Register the VFL driver */
        if ((driver_id = H5FD_register_driver_by_name(driver_env_var, TRUE)) <
            0)
          HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL,
                      "can't register VFL driver");
        driver_ref_inc = TRUE;
      } /* end else */
    }   /* end else */
    ...
    if (H5P__class_set(def_fapclass, H5F_ACS_FILE_DRV_NAME, &driver_prop) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL,
                  "can't set default VFL driver for default file access "
                  "property list class");

    /* Get default file access plist */
    if (NULL ==
        (def_fapl = (H5P_genplist_t *)H5I_object(H5P_FILE_ACCESS_DEFAULT)))
      HGOTO_ERROR(H5E_VFL, H5E_BADID, FAIL,
                  "can't find object for default fapl ID");
    ...
    if (H5P_set_driver(def_fapl, driver_prop.driver_id, driver_prop.driver_info,
      ...
      HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL,
                  "can't set default VFL driver for default FAPL");
  }
  ...
  if (ret_value < 0) {
    if (driver_id >= 0 && driver_ref_inc && H5I_dec_app_ref(driver_id) < 0)
      HDONE_ERROR(H5E_PLIST, H5E_CANTDEC, FAIL,
                  "unable to unregister VFL driver");
  } /* end if */
  ...
  if (!HDstrcmp(driver_name, "sec2")) {
    if ((*driver_id = H5FD_SEC2) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize sec2 VFD");
  } else if (!HDstrcmp(driver_name, "core") ||
             !HDstrcmp(driver_name, "core_paged")) {
    if ((*driver_id = H5FD_CORE) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize core VFD");
  } else if (!HDstrcmp(driver_name, "log")) {
    if ((*driver_id = H5FD_LOG) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize log VFD");
  } else if (!HDstrcmp(driver_name, "family")) {
    if ((*driver_id = H5FD_FAMILY) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize family VFD");
  } else if (!HDstrcmp(driver_name, "multi") ||
             !HDstrcmp(driver_name, "split")) {
    if ((*driver_id = H5FD_MULTI) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize multi VFD");
  } else if (!HDstrcmp(driver_name, "stdio")) {
    if ((*driver_id = H5FD_STDIO) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize stdio VFD");
  } else if (!HDstrcmp(driver_name, "splitter")) {
    if ((*driver_id = H5FD_SPLITTER) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize splitter VFD");
  } else if (!HDstrcmp(driver_name, "mpio")) {
#ifdef H5_HAVE_PARALLEL
    if ((*driver_id = H5FD_MPIO) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize MPI I/O VFD");
#else
    ...
    if ((*driver_id = H5FD_DIRECT) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize Direct I/O VFD");
#else
    ...
    if ((*driver_id = H5FD_MIRROR) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize mirror VFD");
#else
    ...
    if ((*driver_id = H5FD_SUBFILING) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize Subfiling VFD");
#else
    ...
    if ((*driver_id = H5FD_WINDOWS) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, FAIL,
                  "couldn't initialize Windows VFD");
#else
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5P_set_driver(H5P_genplist_t *plist, hid_t new_driver_id,
  ...
  if (TRUE == H5P_isa_class(plist->plist_id, H5P_FILE_ACCESS)) {
    H5FD_driver_prop_t
        driver_prop; /* Property for driver ID, info & config. string */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_driver(hid_t plist_id, hid_t new_driver_id,
                     const void *new_driver_info) {
  H5P_genplist_t *plist;      /* Property list pointer */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5P_set_driver_by_name(H5P_genplist_t *plist, const char *driver_name,
  ...
  if (ret_value < 0) {
    if (new_driver_id >= 0 && H5I_dec_app_ref(new_driver_id) < 0)
      HDONE_ERROR(H5E_PLIST, H5E_CANTDEC, FAIL,
                  "can't decrement count on VFD ID");
  }
...
herr_t H5Pset_driver_by_name(hid_t plist_id, const char *driver_name,
  ...
  if (!driver_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "driver_name parameter cannot be NULL");
  if (!*driver_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "driver_name parameter cannot be an empty string");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5P_set_driver_by_value(H5P_genplist_t *plist,
  ...
  /* Register the driver */
  if ((new_driver_id = H5FD_register_driver_by_value(driver_value, app_ref)) <
      0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL, "unable to register VFD");
...
herr_t H5Pset_driver_by_value(hid_t plist_id, H5FD_class_value_t driver_value,
  ...
  if (driver_value < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "negative VFD value is disallowed");
...
hid_t H5P_peek_driver(H5P_genplist_t *plist) {
  ...
  if (TRUE == H5P_isa_class(plist->plist_id, H5P_FILE_ACCESS)) {
    H5FD_driver_prop_t
        driver_prop; /* Property for driver ID, info & configuration string */
  ...
  if (TRUE == H5P_isa_class(plist->plist_id, H5P_FILE_ACCESS)) {
    ...
    if (H5P_peek(plist, H5F_ACS_FILE_DRV_NAME, &driver_prop) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                  "can't get driver configuration string");
...
ssize_t H5Pget_driver_config_str(hid_t fapl_id, char *config_buf,
  ...
  if (!config_buf && buf_size)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, (-1),
                "config_buf cannot be NULL if buf_size is non-zero");
...
static herr_t H5P__file_driver_copy(void *value) {
  ...
  if (value) {
    H5FD_driver_prop_t *info =
        (H5FD_driver_prop_t *)value; /* Driver ID & info struct */
    ...
    if (info->driver_id > 0) {
      ...
      if (info->driver_config_str) {
        ...
        if (NULL == (new_config_str = H5MM_strdup(info->driver_config_str)))
          HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL,
                      "driver configuration string copy failed");
    ...
    if (info->driver_id > 0) {
      ...
      if (info->driver_info)
        if (H5FD_free_driver_info(info->driver_id, info->driver_info) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTFREE, FAIL,
                      "driver info free request failed");
      ...
      if (H5I_dec_ref(info->driver_id) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTDEC, FAIL,
                    "can't decrement reference count for driver ID");
    }
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_driver_create(const char H5_ATTR_UNUSED *name,
                                           size_t H5_ATTR_UNUSED size,
                                           void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_driver_set(hid_t H5_ATTR_UNUSED prop_id,
 ...
 *
 * Purpose:     Copies a file driver property when it's retrieved from a
 *property list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_driver_get(hid_t H5_ATTR_UNUSED prop_id,
                                        const char H5_ATTR_UNUSED *name,
                                        size_t H5_ATTR_UNUSED size,
                                        void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_driver_del(hid_t H5_ATTR_UNUSED prop_id,
                                        const char H5_ATTR_UNUSED *name,
                                        size_t H5_ATTR_UNUSED size,
                                        void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_driver_copy(const char H5_ATTR_UNUSED *name,
                                         size_t H5_ATTR_UNUSED size,
                                         void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Purpose:        Callback routine which is called whenever the file driver ID
 *& info property in the file access property list is compared.
 *
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__facc_file_driver_cmp(const void *_info1, const void *_info2,
                                     size_t H5_ATTR_UNUSED size) {
  const H5FD_driver_prop_t *info1 = (const H5FD_driver_prop_t *)
  ...
  if (info1->driver_info) {
    ...
    if (0 != (cmp_value = memcmp(info1->driver_info, info2->driver_info,
                                 cls1->fapl_size)))
      HGOTO_DONE(cmp_value);
  } /* end if */
  ...
  if (info1->driver_config_str) {
    if (0 != (cmp_value =
                  HDstrcmp(info1->driver_config_str, info2->driver_config_str)))
      HGOTO_DONE(cmp_value);
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_driver_close(const char H5_ATTR_UNUSED *name,
                                          size_t H5_ATTR_UNUSED size,
                                          void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5Pset_family_offset(hid_t fapl_id, hsize_t offset) {
  ...
  if (H5P_DEFAULT == fapl_id)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "can't modify default property list");
  if (NULL == (plist = H5P_object_verify(fapl_id, H5P_FILE_ACCESS)))
    HGOTO_ERROR(H5E_ID, H5E_BADID, FAIL, "can't find object for ID");
  ...
  if (H5P_set(plist, H5F_ACS_FAMILY_OFFSET_NAME, &offset) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set offset for family file");
...
herr_t H5Pset_multi_type(hid_t fapl_id, H5FD_mem_t type) {
  ...
  if (H5P_set(plist, H5F_ACS_MULTI_TYPE_NAME, &type) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set type for multi driver");
...
herr_t H5Pget_multi_type(hid_t fapl_id, H5FD_mem_t *type /*out*/) {
  ...
  if (type) {
    if (H5P_get(plist, H5F_ACS_MULTI_TYPE_NAME, type) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get type for multi driver");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_cache(hid_t plist_id, int H5_ATTR_UNUSED mdc_nelmts,
  ...
  if (H5P_set(plist, H5F_ACS_DATA_CACHE_NUM_SLOTS_NAME, &rdcc_nslots) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set data cache number of slots");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_cache(hid_t plist_id, int *mdc_nelmts,
                    size_t *rdcc_nslots /*out*/, size_t *rdcc_nbytes /*out*/,
                    double *rdcc_w0 /*out*/) {
  H5P_genplist_t *plist;      /* Property list pointer */
  herr_t ret_value = SUCCEED; /* return value */
...
herr_t H5Pset_mdc_image_config(hid_t plist_id,
  ...
  if (H5AC_validate_cache_image_config(config_ptr) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid metadata cache image configuration");
  ...
  if (H5P_set(plist, H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_NAME, config_ptr) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set metadata cache image initial config");
...
herr_t H5Pget_mdc_image_config(hid_t plist_id,
                               H5AC_cache_image_config_t *config /*out*/) {
  H5P_genplist_t *plist;      /* Property list pointer */
  herr_t ret_value = SUCCEED; /* return value */
  ...
  if (H5P_get(plist, H5F_ACS_META_CACHE_INIT_IMAGE_CONFIG_NAME, config) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get metadata cache initial image config");
 ...
 *data reads and writes will occur in at least 64KB blocks. Setting the value to
 *0 with this API function will turn off the data sieving, even if the VFL
 *driver attempts to use that strategy.
 *
 * Return:    Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
...
herr_t H5Pset_small_data_block_size(hid_t plist_id, hsize_t size) {
  ...
  if (H5P_set(plist, H5F_ACS_SDATA_BLOCK_SIZE_NAME, &size) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set 'small data' block size");
...
herr_t H5Pget_small_data_block_size(hid_t plist_id, hsize_t *size /*out*/) {
  ...
  if (size) {
    if (H5P_get(plist, H5F_ACS_SDATA_BLOCK_SIZE_NAME, size) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get 'small data' block size");
  } /* end if */
 ...
 *
 *      The library supports five pairs of (low, high) combinations via
 *H5Pset_libver_bounds(): 1) H5F_LIBVER_EARLIEST, H5F_LIBVER_V18 2)
 *H5F_LIBVER_EARLIEST, H5F_LIBVER_LATEST 4) H5F_LIBVER_V18, H5F_LIBVER_V18 4)
 *H5F_LIBVER_V18, H5F_LIBVER_LATEST 5) H5F_LIBVER_LATEST, H5F_LIBVER_LATEST See
 *detailed description in the RFC: Setting Bounds for Object Creation in
 *HDF5 1.10.0.
 *
 * Return:    Non-negative on success/Negative on failure
 *
...
herr_t H5Pset_libver_bounds(hid_t plist_id, H5F_libver_t low,
  ...
  /* (earliest, earliest), (latest, earliest), (v18, earliest) are not valid
   * combinations */
  if (high == H5F_LIBVER_EARLIEST)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "Invalid (low,high) combination of library version bound");
 ...
 *
 * Purpose:     Returns the current settings for the library version format
 *bounds from a file access property list.
 *
 * Return:      Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
...
herr_t H5Pget_libver_bounds(hid_t plist_id, H5F_libver_t *low /*out*/,
  ...
  if (low)
    if (H5P_get(plist, H5F_ACS_LIBVER_LOW_BOUND_NAME, low) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get low bound for library format versions");

  if (high)
    if (H5P_get(plist, H5F_ACS_LIBVER_HIGH_BOUND_NAME, high) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get high bound for library format versions");
...
herr_t H5Pset_elink_file_cache_size(hid_t plist_id, unsigned efc_size) {
  ...
  if (H5P_set(plist, H5F_ACS_EFC_SIZE_NAME, &efc_size) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set elink file cache size");
...
herr_t H5Pget_elink_file_cache_size(hid_t plist_id,
  ...
  if (efc_size)
    if (H5P_get(plist, H5F_ACS_EFC_SIZE_NAME, efc_size) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get elink file cache size");
...
herr_t H5Pset_file_image(hid_t fapl_id, void *buf_ptr, size_t buf_len) {
  ...
  if (!(((buf_ptr == NULL) && (buf_len == 0)) ||
        ((buf_ptr != NULL) && (buf_len > 0))))
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "inconsistent buf_ptr and buf_len");
  ...
  if (buf_ptr) {
    ...
    if (image_info.callbacks.image_malloc) {
      if (NULL == (image_info.buffer = image_info.callbacks.image_malloc(
                       buf_len, H5FD_FILE_IMAGE_OP_PROPERTY_LIST_SET,
                       image_info.callbacks.udata)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                    "image malloc callback failed");
    } /* end if */
    ...
    if (image_info.callbacks.image_memcpy) {
      if (image_info.buffer != image_info.callbacks.image_memcpy(
                                   image_info.buffer, buf_ptr, buf_len,
                                   H5FD_FILE_IMAGE_OP_PROPERTY_LIST_SET,
                                   image_info.callbacks.udata))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTCOPY, FAIL,
                    "image_memcpy callback failed");
    } /* end if */
    else
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_file_image(hid_t fapl_id, void **buf /*out*/,
                         size_t *buf_len /*out*/) {
  H5P_genplist_t *fapl;              /* Property list pointer */
  H5FD_file_image_info_t image_info; /* File image info */
  herr_t ret_value = SUCCEED;        /* Return value */
  ...
  if (buf != NULL) {
    ...
    if (image_info.buffer != NULL) {
      /* Allocate memory */
      if (image_info.callbacks.image_malloc) {
        if (NULL == (copy_ptr = image_info.callbacks.image_malloc(
...
herr_t H5Pset_file_image_callbacks(hid_t fapl_id,
  ...
  if (info.buffer != NULL || info.size > 0)
    HGOTO_ERROR(H5E_PLIST, H5E_SETDISALLOWED, FAIL,
                "setting callbacks when an image is already set is forbidden. "
  ...
  if (callbacks_ptr->udata)
    if (callbacks_ptr->udata_copy == NULL || callbacks_ptr->udata_free == NULL)
      HGOTO_ERROR(H5E_PLIST, H5E_SETDISALLOWED, FAIL,
                  "udata callbacks must be set if udata is set");
  ...
  if (callbacks_ptr->udata) {
    ...
    if ((info.callbacks.udata =
             callbacks_ptr->udata_copy(callbacks_ptr->udata)) == NULL)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't copy the supplied udata");
  } /* end if */
...
H5Pget_file_image_callbacks(hid_t fapl_id,
                            H5FD_file_image_callbacks_t *callbacks /*out*/) {
  H5P_genplist_t *fapl;        /* Property list pointer */
  H5FD_file_image_info_t info; /* File image info */
  herr_t ret_value = SUCCEED;  /* Return value */
  ...
  *callbacks = info.callbacks;
  ...
  if (info.callbacks.udata != NULL) {
    ...
    if ((callbacks->udata = info.callbacks.udata_copy(info.callbacks.udata)) ==
        0)
  ...
  if (value) {
    ...
    if (info->buffer != NULL && info->size > 0) {
      if (info->callbacks.image_free) {
        if ((*info->callbacks.image_free)(
                info->buffer, H5FD_FILE_IMAGE_OP_PROPERTY_LIST_CLOSE,
...
static int H5P__facc_cache_image_config_cmp(const void *_config1,
                                            const void *_config2,
                                            size_t H5_ATTR_UNUSED size) {
  const H5AC_cache_image_config_t *config1 = (const H5AC_cache_image_config_t *)
      _config1; /* Create local aliases for values */
  const H5AC_cache_image_config_t *config2 = (const H5AC_cache_image_config_t *)
      _config2;      /* Create local aliases for values */
...
static herr_t H5P__facc_cache_image_config_enc(const void *value, void **_pp,
                                               size_t *size) {
  const H5AC_cache_image_config_t *config = (const H5AC_cache_image_config_t *)
      value; /* Create local aliases for value */
...
static herr_t H5P__facc_cache_image_config_dec(const void **_pp, void *_value) {
  ...
  H5MM_memcpy(config, &H5F_def_mdc_initCacheImageCfg_g,
              sizeof(H5AC_cache_image_config_t));
  ...
  if (enc_size != sizeof(unsigned))
    HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, FAIL,
                "unsigned value can't be decoded");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_image_info_set(hid_t H5_ATTR_UNUSED prop_id,
                                            const char H5_ATTR_UNUSED *name,
                                            size_t H5_ATTR_UNUSED size,
                                            void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Purpose:     Copies a file image property when it's retrieved from a property
 *list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_image_info_get(hid_t H5_ATTR_UNUSED prop_id,
                                            const char H5_ATTR_UNUSED *name,
                                            size_t H5_ATTR_UNUSED size,
                                            void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_image_info_del(hid_t H5_ATTR_UNUSED prop_id,
                                            const char H5_ATTR_UNUSED *name,
                                            size_t H5_ATTR_UNUSED size,
                                            void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5P__file_image_info_free(value) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTRELEASE, FAIL,
                "can't release file image info");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_image_info_copy(const char H5_ATTR_UNUSED *name,
                                             size_t H5_ATTR_UNUSED size,
                                             void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__facc_file_image_info_cmp(const void *_info1, const void *_info2,
                                         size_t H5_ATTR_UNUSED size) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_file_image_info_close(const char H5_ATTR_UNUSED *name,
                                              size_t H5_ATTR_UNUSED size,
                                              void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__facc_cache_config_cmp(const void *_config1,
                                      const void *_config2,
                                      size_t H5_ATTR_UNUSED size) {
  const H5AC_cache_config_t *config1 = (const H5AC_cache_config_t *)
      _config1; /* Create local aliases for values */
  const H5AC_cache_config_t *config2 = (const H5AC_cache_config_t *)
      _config2;      /* Create local aliases for values */
  int ret_value = 0; /* Return value */
...
static herr_t H5P__facc_fclose_degree_dec(const void **_pp, void *_value) {
  H5F_close_degree_t *fclose_degree =
      (H5F_close_degree_t *)_value; /* File close degree */
  const uint8_t **pp = (const uint8_t **)_pp;
...
static herr_t H5P__facc_multi_type_enc(const void *value, void **_pp,
  ...
  const H5FD_mem_t *type =
      (const H5FD_mem_t *)value; /* Create local alias for values */
  uint8_t **pp = (uint8_t **)_pp;
...
static herr_t H5P__facc_libver_type_enc(const void *value, void **_pp,
                                        size_t *size) {
  const H5F_libver_t *type =
      (const H5F_libver_t *)value; /* Create local alias for values */
  uint8_t **pp = (uint8_t **)_pp;
...
herr_t H5Pset_metadata_read_attempts(hid_t plist_id, unsigned attempts) {
  ...
  if (H5P_set(plist, H5F_ACS_METADATA_READ_ATTEMPTS_NAME, &attempts) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set # of metadata read attempts");
 ...
 *
 * Purpose:    Returns the # of metadata read attempts set in the file access
 *property list.
 *
 * Return:    Non-negative on success/Negative on failure
 *
...
herr_t H5Pget_metadata_read_attempts(hid_t plist_id,
  ...
  if (attempts) {
    ...
    if (H5P_get(plist, H5F_ACS_METADATA_READ_ATTEMPTS_NAME, attempts) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get the number of metadata read attempts");
...
herr_t H5Pset_object_flush_cb(hid_t plist_id, H5F_flush_cb_t func,
  ...
  if (!func && udata)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "callback is NULL while user data is not");
  ...
  if (H5P_set(plist, H5F_ACS_OBJECT_FLUSH_CB_NAME, &flush_info) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set object flush callback");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_object_flush_cb(hid_t plist_id, H5F_flush_cb_t *func /*out*/,
                              void **udata /*out*/) {
  H5P_genplist_t *plist; /* Property list pointer */
  H5F_object_flush_t flush_info;
  herr_t ret_value = SUCCEED; /* return value */
  ...
  if (H5P_get(plist, H5F_ACS_OBJECT_FLUSH_CB_NAME, &flush_info) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get object flush callback");
...
herr_t H5Pset_mdc_log_options(hid_t plist_id, hbool_t is_enabled,
                              const char *location, hbool_t start_on_access) {
  H5P_genplist_t *plist;      /* Property list pointer */
  ...
  if (H5P_set(plist, H5F_ACS_START_MDC_LOG_ON_ACCESS_NAME, &start_on_access) <
      0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_mdc_log_options(hid_t plist_id, hbool_t *is_enabled /*out*/,
                              char *location /*out*/,
                              size_t *location_size /*out*/,
                              hbool_t *start_on_access /*out*/) {
  H5P_genplist_t *plist;      /* Property list pointer */
  ...
  if (start_on_access)
    if (H5P_get(plist, H5F_ACS_START_MDC_LOG_ON_ACCESS_NAME, start_on_access) <
        0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get start_on_access flag");
  ...
  if (0 != len) {
    ...
    if (NULL == (*log_location = (char *)H5MM_malloc(len + 1)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, FAIL,
                  "memory allocation failed for prefix");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mdc_log_location_del(hid_t H5_ATTR_UNUSED prop_id,
                                             const char H5_ATTR_UNUSED *name,
                                             size_t H5_ATTR_UNUSED size,
                                             void *value) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mdc_log_location_copy(const char H5_ATTR_UNUSED *name,
                                              size_t H5_ATTR_UNUSED size,
                                              void *value) {
  FUNC_ENTER_PACKAGE_NOERR
...
H5P__facc_mdc_log_location_cmp(const void *value1, const void *value2,
                               size_t H5_ATTR_UNUSED size) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mdc_log_location_close(const char H5_ATTR_UNUSED *name,
                                               size_t H5_ATTR_UNUSED size,
                                               void *value) {
  FUNC_ENTER_PACKAGE_NOERR
...
herr_t H5Pset_evict_on_close(hid_t fapl_id,
  ...
  if (TRUE != H5P_isa_class(fapl_id, H5P_FILE_ACCESS))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                "property list is not a file access plist");
  ...
  if (H5P_set(plist, H5F_ACS_EVICT_ON_CLOSE_FLAG_NAME, &evict_on_close) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set evict on close property");
#else
  HGOTO_ERROR(H5E_PLIST, H5E_UNSUPPORTED, FAIL,
...
herr_t H5Pget_evict_on_close(hid_t fapl_id, hbool_t *evict_on_close /*out*/) {
  ...
  if (TRUE != H5P_isa_class(fapl_id, H5P_FILE_ACCESS))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                "property list is not an access plist");
  ...
  if (H5P_get(plist, H5F_ACS_EVICT_ON_CLOSE_FLAG_NAME, evict_on_close) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get evict on close property");
...
herr_t H5Pset_file_locking(hid_t fapl_id, hbool_t use_file_locking,
  ...
  if (H5P_set(plist, H5F_ACS_USE_FILE_LOCKING_NAME, &use_file_locking) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set use file locking property");
  if (H5P_set(plist, H5F_ACS_IGNORE_DISABLED_FILE_LOCKS_NAME,
              &ignore_when_disabled) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
...
herr_t H5Pget_file_locking(hid_t fapl_id, hbool_t *use_file_locking /*out*/,
  ...
  if (H5P_get(plist, H5F_ACS_USE_FILE_LOCKING_NAME, use_file_locking) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get use file locking property");
 ...
 *
 * Purpose:        Generic encoding callback routine for 'coll_md_read_flag'
 *properties.
 *
 * Return:       Success:    Non-negative
 *           Failure:    Negative
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5P__encode_coll_md_read_flag_t(const void *value, void **_pp,
                                       size_t *size) {
  const H5P_coll_md_read_flag_t *coll_md_read_flag =
      (const H5P_coll_md_read_flag_t *)value;
  uint8_t **pp = (uint8_t **)_pp;
 ...
 *
 * Purpose:        Generic decoding callback routine for 'coll_md_read_flag'
 *properties.
 *
 * Return:       Success:    Non-negative
 *           Failure:    Negative
...
herr_t H5P__decode_coll_md_read_flag_t(const void **_pp, void *_value) {
  H5P_coll_md_read_flag_t *coll_md_read_flag =
      (H5P_coll_md_read_flag_t *)_value; /* File close degree */
  const uint8_t **pp = (const uint8_t **)_pp;
...
herr_t H5Pset_all_coll_metadata_ops(hid_t plist_id, hbool_t is_collective) {
  ...
  if (H5P_set(plist, H5_COLL_MD_READ_FLAG_NAME, &coll_meta_read) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set collective metadata read flag");
...
herr_t H5Pget_all_coll_metadata_ops(hid_t plist_id,
  ...
  /* Get value */
  if (is_collective) {
    H5P_coll_md_read_flag_t internal_flag; /* property setting. we need to
    ...
    if (H5P_get(plist, H5_COLL_MD_READ_FLAG_NAME, &internal_flag) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get core collective metadata read flag");
...
herr_t H5Pset_coll_metadata_write(hid_t plist_id, hbool_t is_collective) {
  ...
  if (H5P_set(plist, H5F_ACS_COLL_MD_WRITE_FLAG_NAME, &is_collective) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set collective metadata write flag");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_mpi_params(hid_t plist_id, MPI_Comm *comm /*out*/,
                         MPI_Info *info /*out*/) {
  H5P_genplist_t *plist;      /* Property list pointer */
  herr_t ret_value = SUCCEED; /* return value */
  ...
  if (H5P_get(plist, H5F_ACS_MPI_PARAMS_COMM_NAME, comm) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get MPI communicator from plist");
 ...
 *
 * Purpose:     Copies an MPI comminicator property when it's set for a property
 *list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_comm_set(hid_t H5_ATTR_UNUSED prop_id,
                                     const char H5_ATTR_UNUSED *name,
 ...
 *
 * Purpose:     Copies an MPI comminicator property when it's retrieved from a
 *property list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_comm_get(hid_t H5_ATTR_UNUSED prop_id,
                                     const char H5_ATTR_UNUSED *name,
                                     size_t H5_ATTR_UNUSED size, void *value) {
  MPI_Comm *comm = (MPI_Comm *)value;
  MPI_Comm comm_tmp = MPI_COMM_NULL;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_comm_del(hid_t H5_ATTR_UNUSED prop_id,
                                     const char H5_ATTR_UNUSED *name,
                                     size_t H5_ATTR_UNUSED size, void *value) {
  MPI_Comm *comm = (MPI_Comm *)value;
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_comm_copy(const char H5_ATTR_UNUSED *name,
                                      size_t H5_ATTR_UNUSED size, void *value) {
  MPI_Comm *comm = (MPI_Comm *)value;
  MPI_Comm comm_tmp = MPI_COMM_NULL;
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__facc_mpi_comm_cmp(const void *_comm1, const void *_comm2,
                                  size_t H5_ATTR_UNUSED size) {
  ...
  if (H5_mpi_comm_cmp(*comm1, *comm2, &ret_value) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, 0,
                "unable to compare MPI communicator");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_comm_close(const char H5_ATTR_UNUSED *name,
                                       size_t H5_ATTR_UNUSED size,
                                       void *value) {
  MPI_Comm *comm = (MPI_Comm *)value;
  herr_t ret_value = SUCCEED;
 ...
 *
 * Purpose:     Copies an MPI info object property when it's set for a property
 *list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_info_set(hid_t H5_ATTR_UNUSED prop_id,
                                     const char H5_ATTR_UNUSED *name,
                                     size_t H5_ATTR_UNUSED size, void *value) {
  ...
  if (H5_mpi_info_dup(*info, &info_tmp) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL,
                "unable to duplicate MPI info object");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_info_get(hid_t H5_ATTR_UNUSED prop_id,
                                     const char H5_ATTR_UNUSED *name,
                                     size_t H5_ATTR_UNUSED size, void *value) {
  MPI_Info *info = (MPI_Info *)value;
  MPI_Info info_tmp = MPI_INFO_NULL;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_info_del(hid_t H5_ATTR_UNUSED prop_id,
                                     const char H5_ATTR_UNUSED *name,
                                     size_t H5_ATTR_UNUSED size, void *value) {
  MPI_Info *info = (MPI_Info *)value;
  herr_t ret_value = SUCCEED;
  ...
  if (H5_mpi_info_free(info) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTFREE, FAIL,
                "unable to free MPI info object");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_info_copy(const char H5_ATTR_UNUSED *name,
                                      size_t H5_ATTR_UNUSED size, void *value) {
  MPI_Info *info = (MPI_Info *)value;
  MPI_Info info_tmp = MPI_INFO_NULL;
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__facc_mpi_info_cmp(const void *_info1, const void *_info2,
                                  size_t H5_ATTR_UNUSED size) {
  ...
  if (H5_mpi_info_cmp(*info1, *info2, &ret_value) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, 0,
                "unable to compare MPI info objects");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_mpi_info_close(const char H5_ATTR_UNUSED *name,
                                       size_t H5_ATTR_UNUSED size,
                                       void *value) {
  MPI_Info *info = (MPI_Info *)value;
  herr_t ret_value = SUCCEED;
...
herr_t H5Pget_coll_metadata_write(hid_t plist_id,
  ...
  if (H5P_get(plist, H5F_ACS_COLL_MD_WRITE_FLAG_NAME, is_collective) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get collective metadata write flag");
...
herr_t H5Pset_page_buffer_size(hid_t plist_id, size_t buf_size,
  ...
  if (min_meta_perc + min_raw_perc > 100)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "Sum of minimum metadata and raw data fractions can't be "
                "bigger than 100");
  ...
  if (H5P_set(plist, H5F_ACS_PAGE_BUFFER_MIN_META_PERC_NAME, &min_meta_perc) <
      0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set percentage of min metadata entries");
  if (H5P_set(plist, H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_NAME, &min_raw_perc) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set percentage of min raw data entries");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_page_buffer_size(hid_t plist_id, size_t *buf_size /*out*/,
                               unsigned *min_meta_perc /*out*/,
  ...
  if (min_meta_perc)
    if (H5P_get(plist, H5F_ACS_PAGE_BUFFER_MIN_META_PERC_NAME, min_meta_perc) <
        0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get page buffer minimum metadata percent");
  if (min_raw_perc)
    if (H5P_get(plist, H5F_ACS_PAGE_BUFFER_MIN_RAW_PERC_NAME, min_raw_perc) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get page buffer minimum raw data percent");
...
herr_t H5P_set_vol(H5P_genplist_t *plist, hid_t vol_id, const void *vol_info) {
  ...
  if (TRUE == H5P_isa_class(plist->plist_id, H5P_FILE_ACCESS)) {
    ...
    if (H5P_set(plist, H5F_ACS_VOL_CONN_NAME, &vol_prop) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't set VOL connector ID & info");
  } /* end if */
  else
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a file access property list");
 ...
 *		the reference count on the previous VOL will _NOT_ be
 *decremented. The reference count on the new VOL will _NOT_ be incremented.
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
...
herr_t H5P_reset_vol_class(const H5P_genclass_t *pclass,
  ...
  if (H5P__class_get(pclass, H5F_ACS_VOL_CONN_NAME, &old_vol_prop) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get VOL connector ID & info");
...
herr_t H5Pget_vol_info(hid_t plist_id, void **vol_info /*out*/) {
  ...
  if (TRUE == H5P_isa_class(plist->plist_id, H5P_FILE_ACCESS)) {
    ...
    if (H5P_peek(plist, H5F_ACS_VOL_CONN_NAME, &connector_prop) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get VOL connector property");
    ...
    if (connector_prop.connector_info) {
      ...
      if (NULL ==
          (connector = (H5VL_class_t *)H5I_object(connector_prop.connector_id)))
 ...
 *              retrieve the capability flags for the VOL connector stack, as
 *will be used by a file open or create operation that uses this FAPL.
 *
 *              Current capability flags are:
 *                  H5VL_CAP_FLAG_THREADSAFE   - Connector is threadsafe
 *                  H5VL_CAP_FLAG_ASYNC        - Connector performs operations
 *asynchronously H5VL_CAP_FLAG_NATIVE_FILES - Connector produces native file
 *format
 *
 * Note:        This routine supports the use of the HDF5_VOL_CONNECTOR
 *environment environment variable to override the VOL connector set
 *programmatically for the FAPL (with H5Pset_vol).
 *
 * Note:        The H5VL_CAP_FLAG_ASYNC flag can be checked to see if
 *asynchronous operations are supported by the VOL connector stack.
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 *
...
herr_t H5Pget_vol_cap_flags(hid_t plist_id, uint64_t *cap_flags) {
  ...
  if (cap_flags) {
    ...
    if (TRUE == H5P_isa_class(plist_id, H5P_FILE_ACCESS)) {
      ...
      /* Get property list for ID */
      if (NULL == (plist = (H5P_genplist_t *)H5I_object_verify(
                       plist_id, H5I_GENPROP_LST)))
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a property list");
      ...
      if (H5VL_get_cap_flags(&connector_prop, cap_flags) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                    "can't get VOL connector capability flags");
    } else
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_vol_create(const char H5_ATTR_UNUSED *name,
                                   size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Purpose:     Copies a VOL connector property when it's set for a property
 *list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_vol_set(hid_t H5_ATTR_UNUSED prop_id,
                                const char H5_ATTR_UNUSED *name,
                                size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Purpose:     Copies a VOL connector property when it's retrieved from a
 *property list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_vol_get(hid_t H5_ATTR_UNUSED prop_id,
                                const char H5_ATTR_UNUSED *name,
                                size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_vol_del(hid_t H5_ATTR_UNUSED prop_id,
                                const char H5_ATTR_UNUSED *name,
                                size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_vol_copy(const char H5_ATTR_UNUSED *name,
                                 size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__facc_vol_cmp(const void *_info1, const void *_info2,
                             size_t H5_ATTR_UNUSED size) {
  const H5VL_connector_prop_t *info1 = (const H5VL_connector_prop_t *)
      _info1; /* Create local aliases for values */
  ...
  status = H5VL_cmp_connector_info(cls1, &cmp_value, info1->connector_info,
                                   info2->connector_info);
  assert(status >= 0);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__facc_vol_close(const char H5_ATTR_UNUSED *name,
                                  size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
