 ...
 *-------------------------------------------------------------------------
 */
static size_t H5Z__filter_fletcher32(unsigned flags,
  ...
  if (flags & H5Z_FLAG_REVERSE) { /* Read */
    ...
    if (!(flags & H5Z_FLAG_SKIP_EDC)) {
      ...
      if (stored_fletcher != fletcher && stored_fletcher != reversed_fletcher)
        HGOTO_ERROR(H5E_STORAGE, H5E_READERROR, 0,
                    "data error detected by Fletcher32 checksum");
    }

    /* Set return values */
    /* (Re-use the input buffer, just note that the size is smaller by the size
     * of the checksum) */
