...
static herr_t H5O__pline_pre_copy_file(H5F_t *file_src, const void *mesg_src,
                                       hbool_t *deleted,
...
static void *H5O__pline_decode(H5F_t H5_ATTR_UNUSED *f,
  ...
  if (NULL == (pline->filter = (H5Z_filter_info_t *)H5MM_calloc(
                   pline->nalloc * sizeof(pline->filter[0]))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
  ...
  for (i = 0, filter = &pline->filter[0]; i < pline->nused; i++, filter++) {
    ...
    if (filter->cd_nelmts) {
      ...
      if (filter->cd_nelmts > H5Z_COMMON_CD_VALUES) {
        filter->cd_values =
            (unsigned *)H5MM_malloc(filter->cd_nelmts * sizeof(unsigned));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__pline_encode(H5F_t H5_ATTR_UNUSED *f, uint8_t *p /*out*/,
                                const void *mesg) {
  const H5O_pline_t *pline =
      (const H5O_pline_t *)mesg;   /* Pipeline message to encode */
  ...
  *p++ = (uint8_t)(pline->nused);
  ...
  for (i = 0, filter = &pline->filter[0]; i < pline->nused; i++, filter++) {
    ...
    if (pline->version > H5O_PLINE_VERSION_1 &&
      ...
      /* Filter name length */
      UINT16ENCODE(p, pline->version == H5O_PLINE_VERSION_1
                          ? H5O_ALIGN_OLD(name_length)
                          : name_length);
    } /* end else */
...
static void *H5O__pline_copy(const void *_src, void *_dst /*out*/) {
  const H5O_pline_t *src =
      (const H5O_pline_t *)_src;          /* Source pipeline message */
  ...
  *dst = *src;
  ...
  if (dst->nalloc) {
    ...
    if (NULL == (dst->filter = (H5Z_filter_info_t *)H5MM_calloc(
                     dst->nalloc * sizeof(dst->filter[0]))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
    ...
    for (i = 0; i < src->nused; i++) {
      ...
      if (src->filter[i].name) {
        size_t namelen; /* Length of source filter name, including null
                           terminator  */
      ...
      if (src->filter[i].cd_nelmts > 0) {
        /* Allocate space for the client data elements, or use the internal
         * buffer */
...
static size_t H5O__pline_size(const H5F_t H5_ATTR_UNUSED *f, const void *mesg) {
  ...
  for (i = 0; i < pline->nused; i++) {
    ...
    if (pline->version > H5O_PLINE_VERSION_1 &&
        pline->filter[i].id < H5Z_FILTER_RESERVED)
      name_len = 0;
    else {
      H5Z_class2_t *cls; /* Filter class */
    ...
    ret_value += 2 + /*filter identification number    */
                 (size_t)((pline->version == H5O_PLINE_VERSION_1 ||
                           pline->filter[i].id >= H5Z_FILTER_RESERVED)
                              ? 2
                              : 0) + /*name length            */
                 2 +                 /*flags                */
  ...
  if (pline->filter) {
    ...
    for (i = 0; i < pline->nused; i++) {
      ...
      if (pline->filter[i].cd_values != pline->filter[i]._cd_values)
        pline->filter[i].cd_values =
            (unsigned *)H5MM_xfree(pline->filter[i].cd_values);
    } /* end for */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__pline_pre_copy_file(H5F_t H5_ATTR_UNUSED *file_src,
                                       const void *mesg_src,
                                       hbool_t H5_ATTR_UNUSED *deleted,
                                       const H5O_copy_t *cpy_info,
                                       void *_udata) {
  const H5O_pline_t *pline_src =
      (const H5O_pline_t *)mesg_src; /* Source pline */
  H5O_copy_file_ud_common_t *udata =
      (H5O_copy_file_ud_common_t *)_udata; /* Object copying user data */
  ...
  if (pline_src->version >
      H5O_pline_ver_bounds[H5F_HIGH_BOUND(cpy_info->file_dst)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "pline message version out of bounds");
  ...
  if (udata)
    if (NULL ==
        (udata->src_pline = (H5O_pline_t *)H5O__pline_copy(pline_src, NULL)))
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__pline_debug(H5F_t H5_ATTR_UNUSED *f, const void *mesg,
  ...
  fprintf(stream, "%*s%-*s %zu/%zu\n", indent, "", fwidth,
          "Number of filters:", pline->nused, pline->nalloc);
  ...
  for (size_t i = 0; i < pline->nused; i++) {
    ...
    for (size_t j = 0; j < pline->filter[i].cd_nelmts; j++) {
      ...
      HDsnprintf(field_name, sizeof(field_name), "CD value %lu",
                 (unsigned long)j);
      fprintf(stream, "%*s%-*s %u\n", indent + 6, "", MAX(0, fwidth - 6),
              field_name, pline->filter[i].cd_values[j]);
    }
  }
