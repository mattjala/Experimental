...
#define H5FS_HEADER_SIZE(f)                                                    \
  (/* General metadata fields */                                               \
   ...
   + (unsigned)H5F_SIZEOF_ADDR(                                                \
         f) /* Address of serialized free space sections */                    \
   + (unsigned)H5F_SIZEOF_SIZE(                                                \
         f) /* Size of serialized free space sections used */                  \
   + (unsigned)H5F_SIZEOF_SIZE(                                                \
...
typedef struct H5FS_node_t {
  ...
  H5SL_t *sect_list;   /* Skip list to hold pointers to actual free list section
                          node */
} H5FS_node_t;
...
typedef struct H5FS_sinfo_t {
  ...
  size_t serial_size_count; /* Total number of differently sized serializable
                               sections */
  size_t ghost_size_count;  /* Total number of differently sized un-serializable
                               sections */
  unsigned sect_prefix_size; /* Size of the section serialization prefix (in
                                bytes) */
...
struct H5FS_t {
  ...
  hbool_t
      sinfo_protected; /* Whether the section info was protected when locked */
  ...
  H5FS_section_class_t
      *sect_cls; /* Array of section classes for this free list */
};
