...
static herr_t H5L__create_hard_api_common(hid_t cur_loc_id,
                                          ...
                                          hid_t lapl_id, void **token_ptr,
                                          H5VL_object_t **_vol_obj_ptr);
static herr_t H5L__delete_api_common(hid_t loc_id, const char *name,
                                     hid_t lapl_id, void **token_ptr,
                                     H5VL_object_t **_vol_obj_ptr);
static herr_t H5L__delete_by_idx_api_common(hid_t loc_id,
                                            ...
                                            hid_t lapl_id, void **token_ptr,
                                            H5VL_object_t **_vol_obj_ptr);
static herr_t H5L__exists_api_common(hid_t loc_id, const char *name,
                                     hbool_t *exists, hid_t lapl_id,
                                     void **token_ptr,
                                     H5VL_object_t **_vol_obj_ptr);
static herr_t H5L__iterate_api_common(hid_t group_id, H5_index_t idx_type,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Lmove(hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
  ...
  if (src_loc_id == H5L_SAME_LOC && dst_loc_id == H5L_SAME_LOC)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "source and destination should not both be H5L_SAME_LOC");
  ...
  /* Move the link */
  if (H5VL_link_move(&tmp_vol_obj, &loc_params1, vol_obj2, &loc_params2,
                     lcpl_id, lapl_id, H5P_DATASET_XFER_DEFAULT,
                     H5_REQUEST_NULL) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Lcopy(hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
               const char *dst_name, hid_t lcpl_id, hid_t lapl_id) {
  H5VL_object_t *vol_obj1 = NULL; /* Object of src_id */
  H5VL_loc_params_t loc_params1;
  H5VL_object_t *vol_obj2 = NULL; /* Object of dst_id */
  ...
  /* Copy the link */
  if (H5VL_link_copy(&tmp_vol_obj, &loc_params1, vol_obj2, &loc_params2,
                     lcpl_id, lapl_id, H5P_DATASET_XFER_DEFAULT,
                     H5_REQUEST_NULL) < 0)
...
static herr_t H5L__create_soft_api_common(const char *link_target,
  ...
  if (link_loc_id == H5L_SAME_LOC)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "link location id should not be H5L_SAME_LOC");
  if (!link_target)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "link_target parameter cannot be NULL");
  if (!*link_target)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "link_target parameter cannot be an empty string");
  if (lcpl_id != H5P_DEFAULT &&
      (TRUE != H5P_isa_class(lcpl_id, H5P_LINK_CREATE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "not a link creation property list");
  ...
  if (H5VL_setup_name_args(link_loc_id, link_name, TRUE, lapl_id, vol_obj_ptr,
                           &loc_params) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTSET, FAIL,
  ...
  /* Create the link */
  if (H5VL_link_create(&vol_cb_args, *vol_obj_ptr, &loc_params, lcpl_id,
...
herr_t H5Lcreate_soft(const char *link_target, hid_t link_loc_id,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE5("e", "*si*sii", link_target, link_loc_id, link_name, lcpl_id,
           lapl_id);

  /* Creates a soft link synchronously */
  if (H5L__create_soft_api_common(link_target, link_loc_id, link_name, lcpl_id,
                                  lapl_id, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTCREATE, FAIL,
                "unable to synchronously create soft link");
...
herr_t H5Lcreate_soft_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE9("e", "*s*sIu*si*siii", app_file, app_func, app_line, link_target,
           link_loc_id, link_name, lcpl_id, lapl_id, es_id);
  ...
  if (H5L__create_soft_api_common(link_target, link_loc_id, link_name, lcpl_id,
    ...
    HGOTO_ERROR(H5E_LINK, H5E_CANTCREATE, FAIL,
                "unable to asynchronously create soft link");
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      /* clang-format on */
      HGOTO_ERROR(H5E_LINK, H5E_CANTINSERT, FAIL,
...
static herr_t H5L__create_hard_api_common(hid_t cur_loc_id,
  ...
  H5VL_object_t **tmp_vol_obj_ptr_ptr =
      (_vol_obj_ptr ? _vol_obj_ptr
                    : &tmp_vol_obj_ptr); /* Ptr to ptr to temporary object */
  H5VL_link_create_args_t vol_cb_args;   /* Arguments to VOL callback */
  H5VL_loc_params_t
      link_loc_params; /* Location parameters for link_loc_id object access */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (cur_loc_id == H5L_SAME_LOC && link_loc_id == H5L_SAME_LOC)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "source and destination should not be both H5L_SAME_LOC");
  if (!cur_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "cur_name parameter cannot be NULL");
  if (!*cur_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "cur_name parameter cannot be an empty string");
  if (!link_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "new_name parameter cannot be NULL");
  if (!*link_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "new_name parameter cannot be an empty string");
  if (lcpl_id != H5P_DEFAULT &&
      (TRUE != H5P_isa_class(lcpl_id, H5P_LINK_CREATE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "not a link creation property list");
  ...
  /* Create the link */
  if (H5VL_link_create(&vol_cb_args, *tmp_vol_obj_ptr_ptr, &link_loc_params,
                       lcpl_id, lapl_id, H5P_DATASET_XFER_DEFAULT,
                       token_ptr) < 0)
...
herr_t H5Lcreate_hard(hid_t cur_loc_id, const char *cur_name, hid_t new_loc_id,
  ...
  if (H5L__create_hard_api_common(cur_loc_id, cur_name, new_loc_id, new_name,
    ...
    HGOTO_ERROR(H5E_LINK, H5E_CANTCREATE, FAIL,
                "unable to synchronously create hard link");
...
herr_t H5Lcreate_hard_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIui*si*siii", app_file, app_func, app_line, cur_loc_id,
            cur_name, new_loc_id, new_name, lcpl_id, lapl_id, es_id);
  ...
  if (H5L__create_hard_api_common(cur_loc_id, cur_name, new_loc_id, new_name,
                                  lcpl_id, lapl_id, token_ptr,
                                  &vol_obj_ptr) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTCREATE, FAIL,
                "unable to asynchronously create hard link");
...
herr_t H5Lcreate_external(const char *file_name, const char *obj_name,
  ...
  /* Combine the filename and link name into a single buffer to give to the UD
   * link */
  ...
  if (NULL == (ext_link_buf = H5MM_malloc(buf_size)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "unable to allocate udata buffer");

  /* Encode the external link information */
  p = (uint8_t *)ext_link_buf;
  *p++ = (H5L_EXT_VERSION << 4) |
         H5L_EXT_FLAGS_ALL; /* External link version & flags */
  HDstrncpy((char *)p, file_name,
            buf_size - 1); /* Name of file containing external link's object */
  p += file_name_len;
  HDstrncpy((char *)p, norm_obj_name,
            buf_size - (file_name_len + 1)); /* External link's object */
...
herr_t H5Lcreate_ud(hid_t link_loc_id, const char *link_name,
  ...
  if (!udata && udata_size)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "udata cannot be NULL if udata_size is non-zero");
...
static herr_t H5L__delete_api_common(hid_t loc_id, const char *name,
  ...
  /* Set up object access arguments */
  if (H5VL_setup_name_args(loc_id, name, TRUE, lapl_id, vol_obj_ptr,
                           &loc_params) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTSET, FAIL,
                "can't set object access arguments");
  ...
  /* Delete link */
  if (H5VL_link_specific(*vol_obj_ptr, &loc_params, &vol_cb_args,
...
herr_t H5Ldelete_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "*s*sIui*sii", app_file, app_func, app_line, loc_id, name,
           lapl_id, es_id);
  ...
  if (H5L__delete_api_common(loc_id, name, lapl_id, token_ptr, &vol_obj) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTDELETE, FAIL,
                "unable to asynchronously delete link");
...
static herr_t H5L__delete_by_idx_api_common(hid_t loc_id,
  ...
  /* Set up object access arguments */
  if (H5VL_setup_idx_args(loc_id, group_name, idx_type, order, n, TRUE, lapl_id,
...
herr_t H5Ldelete_by_idx(hid_t loc_id, const char *group_name,
  ...
  /* Delete a link synchronously */
  if (H5L__delete_by_idx_api_common(loc_id, group_name, idx_type, order, n,
                                    lapl_id, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTDELETE, FAIL,
                "unable to synchronously delete link");
...
herr_t H5Ldelete_by_idx_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIui*sIiIohii", app_file, app_func, app_line, loc_id,
...
herr_t H5Lget_val(hid_t loc_id, const char *name, void *buf /*out*/,
  ...
  /* Get the link value */
  if (H5VL_link_get(vol_obj, &loc_params, &vol_cb_args,
                    H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
...
herr_t H5Lget_val_by_idx(hid_t loc_id, const char *group_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE8("e", "i*sIiIohxzi", loc_id, group_name, idx_type, order, n, buf,
           size, lapl_id);
...
static herr_t H5L__exists_api_common(hid_t loc_id, const char *name,
  ...
  if (NULL == exists)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid pointer for link existence");
...
herr_t H5Lexists_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE8("e", "*s*sIui*s*bii", app_file, app_func, app_line, loc_id, name,
  ...
  if (H5L__exists_api_common(loc_id, name, exists, lapl_id, token_ptr,
                             &vol_obj) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                "unable to asynchronously check link existence");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Lget_info2(hid_t loc_id, const char *name, H5L_info2_t *linfo /*out*/,
                    hid_t lapl_id) {
  H5VL_object_t *vol_obj = NULL;    /* object of loc_id */
  H5VL_link_get_args_t vol_cb_args; /* Arguments to VOL callback */
  H5VL_loc_params_t loc_params;     /* Location parameters for object access */
...
herr_t H5Lregister(const H5L_class_t *cls) {
   ...
   * (in src/H5Gtraverse.c), so it's allowed to to pass through here. - QAK,
   * 2018/02/06
   */
  if (cls->version > H5L_LINK_CLASS_T_VERS)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid H5L_class_t version number");
#ifdef H5_NO_DEPRECATED_SYMBOLS
  if (cls->version < H5L_LINK_CLASS_T_VERS)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "deprecated H5L_class_t version number (%d) and library built "
                "without deprecated symbol support",
                cls->version);
#endif /* H5_NO_DEPRECATED_SYMBOLS */

  if (cls->id < H5L_TYPE_UD_MIN || cls->id > H5L_TYPE_MAX)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid link identification number");
  if (cls->trav_func == NULL)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "no traversal function specified");

  /* Do it */
  if (H5L_register(cls) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_NOTREGISTERED, FAIL,
                "unable to register link type");
...
herr_t H5Lunregister(H5L_type_t id) {
  ...
  if (H5L_unregister(id) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_NOTREGISTERED, FAIL,
                "unable to unregister link type");
...
htri_t H5Lis_registered(H5L_type_t id) {
  ...
  if (H5L_is_registered(id, &is_registered) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_BADTYPE, FAIL,
                "could not determine registration status of UD link type");
...
ssize_t H5Lget_name_by_idx(hid_t loc_id, const char *group_name,
  ...
  FUNC_ENTER_API((-1))
  H5TRACE8("Zs", "i*sIiIohxzi", loc_id, group_name, idx_type, order, n, name,
           size, lapl_id);
   ...
   */
  if (H5CX_set_apl(&lapl_id, H5P_CLS_LACC, loc_id, TRUE) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTSET, (-1),
                "can't set access property list info");
...
static herr_t H5L__iterate_api_common(hid_t group_id, H5_index_t idx_type,
  ...
  if ((ret_value = H5VL_link_specific(*vol_obj_ptr, &loc_params, &vol_cb_args,
                                      H5P_DATASET_XFER_DEFAULT, token_ptr)) < 0)
...
herr_t H5Literate2(hid_t group_id, H5_index_t idx_type, H5_iter_order_t order,
  ...
  if ((ret_value = H5L__iterate_api_common(group_id, idx_type, order, idx_p, op,
                                           op_data, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_BADITER, FAIL,
                "synchronous link iteration failed");
...
herr_t H5Literate_async(const char *app_file, const char *app_func,
                        unsigned app_line, hid_t group_id, H5_index_t idx_type,
                        H5_iter_order_t order, hsize_t *idx_p,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIuiIiIo*hLI*xi", app_file, app_func, app_line, group_id,
            idx_type, order, idx_p, op, op_data, es_id);
  ...
  if ((ret_value = H5L__iterate_api_common(group_id, idx_type, order, idx_p, op,
                                           op_data, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_BADITER, FAIL,
                "asynchronous link iteration failed");
...
herr_t H5Literate_by_name2(hid_t loc_id, const char *group_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE8("e", "i*sIiIo*hLI*xi", loc_id, group_name, idx_type, order, idx_p,
           op, op_data, lapl_id);

  /* Check arguments */
  if (!group_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "group_name parameter cannot be NULL");
  if (!*group_name)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "group_name parameter cannot be an empty string");
  if (idx_type <= H5_INDEX_UNKNOWN || idx_type >= H5_INDEX_N)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "invalid index type specified");
  if (order <= H5_ITER_UNKNOWN || order >= H5_ITER_N)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
  ...
  /* Iterate over the links */
  if ((ret_value =
           H5VL_link_specific(vol_obj, &loc_params, &vol_cb_args,
                              H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_BADITER, FAIL, "link iteration failed");
...
herr_t H5Lvisit_by_name2(hid_t loc_id, const char *group_name,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "i*sIiIoLI*xi", loc_id, group_name, idx_type, order, op,
           op_data, lapl_id);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Lunpack_elink_val(const void *_ext_linkval, size_t link_size,
                           unsigned *flags, const char **filename,
                           const char **obj_path) {
  const uint8_t *ext_linkval =
      (const uint8_t *)_ext_linkval; /* Pointer to the link value */
  ...
  if (ext_linkval[link_size - 1] != '\0')
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "linkval buffer is not NULL-terminated");
  ...
  if ((len + 1) >= (link_size - 1))
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "linkval buffer doesn't contain an object path");
  ...
  if (obj_path)
    *obj_path = ((const char *)ext_linkval + 1) + len +
