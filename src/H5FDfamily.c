...
static herr_t H5FD__family_get_handle(H5FD_t *_file, hid_t fapl,
                                      void **file_handle);
static herr_t H5FD__family_read(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                                haddr_t addr, size_t size, void *_buf /*out*/);
static herr_t H5FD__family_write(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                                 haddr_t addr, size_t size, const void *_buf);
static herr_t H5FD__family_flush(H5FD_t *_file, hid_t dxpl_id, hbool_t closing);
static herr_t H5FD__family_truncate(H5FD_t *_file, hid_t dxpl_id,
                                    hbool_t closing);
...
static herr_t H5FD__family_get_default_config(H5FD_family_fapl_t *fa_out) {
  ...
  if (H5P_set_driver_by_value(plist, H5_VFD_SEC2, NULL, TRUE) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL,
                "can't set default driver on member FAPL");

done:
  if (ret_value < 0 && fa_out->memb_fapl_id >= 0) {
    if (H5I_dec_ref(fa_out->memb_fapl_id) < 0)
      HDONE_ERROR(H5E_VFL, H5E_CANTDEC, FAIL,
                  "can't decrement ref. count on member FAPL ID");
  }
...
H5FD__family_get_default_printf_filename(const char *old_filename) {
  ...
  if (NULL == (tmp_buffer = H5MM_malloc(new_filename_len)))
    HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, NULL,
                "can't allocate new filename buffer");
  ...
  if (H5P_DEFAULT == memb_fapl_id) {
    ...
    if (H5FD__family_get_default_config(&fa) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                  "can't get default driver configuration info");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pget_fapl_family(hid_t fapl_id, hsize_t *msize /*out*/,
...
static void *H5FD__family_fapl_get(H5FD_t *_file) {
  ...
  if (NULL ==
      (fa = (H5FD_family_fapl_t *)H5MM_calloc(sizeof(H5FD_family_fapl_t))))
...
static void *H5FD__family_fapl_copy(const void *_old_fa) {
  ...
  if (NULL ==
      (new_fa = (H5FD_family_fapl_t *)H5MM_malloc(sizeof(H5FD_family_fapl_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
...
#if 1 /* JRM */
    /* failure to increment the ref count on new_fa->memb_fapl_id was always a
     * bug, but it was not exposed until we ceased itterating through index
...
static herr_t H5FD__family_sb_encode(H5FD_t *_file, char *name /*out*/,
   ...
   * member file (see H5FD__family_open).  So it isn't safe to use
   * FILE->MEMB_SIZE. If the file is created with v1.8, the correctness of
   * FILE->PMEM_SIZE is checked in H5FD__family_sb_decode. SLU - 2009/3/21
   */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__family_sb_decode(H5FD_t *_file,
  ...
  if (file->mem_newsize)
    ...
    if (msize != file->pmem_size)
      HGOTO_ERROR(H5E_FILE, H5E_BADVALUE, FAIL,
                  "Family member size should be %lu.  But the size from file "
                  "access property is %lu",
...
static H5FD_t *H5FD__family_open(const char *name, unsigned flags,
  ...
  if (H5P_FILE_ACCESS_DEFAULT == fapl_id) {
    ...
    file->memb_size =
        H5FD_FAM_DEF_MEM_SIZE; /* Actual member size to be updated later */
    file->pmem_size =
        H5FD_FAM_DEF_MEM_SIZE; /* Member size passed in through property */
    ...
    if (NULL == (plist = (H5P_genplist_t *)H5I_object(fapl_id)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "not a file access property list");
    if (NULL ==
        (fa = (const H5FD_family_fapl_t *)H5P_peek_driver_info(plist))) {
      if (H5FD__family_get_default_config(&default_fa) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTGET, NULL,
                    "can't get default family VFD configuration");
      fa = &default_fa;
      default_config = TRUE;
    }
    ...
    if (H5P_exist_plist(plist, H5F_ACS_FAMILY_NEWSIZE_NAME) > 0) {
      ...
      if (H5P_get(plist, H5F_ACS_FAMILY_NEWSIZE_NAME, &file->mem_newsize) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL,
                    "can't get new family member size");
    ...
    file->memb_size =
        fa->memb_size; /* Actual member size to be updated later */
    file->pmem_size =
        fa->memb_size; /* Member size passed in through property */

    if (default_config && H5I_dec_ref(fa->memb_fapl_id) < 0)
      HGOTO_ERROR(H5E_ID, H5E_CANTDEC, NULL,
                  "can't decrement ref. count on member FAPL");
  } /* end else */
  ...
  if (NULL == (memb_name = (char *)H5MM_malloc(H5FD_FAM_MEMB_NAME_BUF_SIZE)))
    HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, NULL,
                "unable to allocate member name");
  if (NULL == (temp = (char *)H5MM_malloc(H5FD_FAM_MEMB_NAME_BUF_SIZE)))
    HGOTO_ERROR(H5E_FILE, H5E_CANTALLOC, NULL,
                "unable to allocate temporary member name");
  ...
  if (!HDstrcmp(memb_name, temp)) {
    if (default_config) {
      ...
      if (NULL == (temp = H5FD__family_get_default_printf_filename(name)))
        HGOTO_ERROR(H5E_VFL, H5E_CANTGET, NULL,
                    "can't get default printf-style filename");
      name = temp;
    } else
  ...
  while (1) {
    ...
    if (file->nmembs >= file->amembs) {
      ...
      assert(n > 0);
      if (NULL ==
          (x = (H5FD_t **)H5MM_realloc(file->memb, n * sizeof(H5FD_t *))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
    ...
    H5E_BEGIN_TRY {
      file->memb[file->nmembs] =
          H5FDopen(memb_name, (0 == file->nmembs ? flags : t_flags),
  ...
  /* If the file is reopened and there's only one member file existing, this
   * file may be smaller than the size specified through H5Pset_fapl_family().
   * Update the actual member size.
   */
  ...
  if (ret_value == NULL && file != NULL) {
    ...
    /* Close as many members as possible. Use private function here to avoid
     * clearing the error stack. We need the error message to indicate wrong
     * member file size. */
    for (u = 0; u < file->nmembs; u++)
      if (file->memb[u])
        if (H5FD_close(file->memb[u]) < 0)
          nerrors++;
    if (nerrors)
      HGOTO_ERROR(H5E_FILE, H5E_CANTCLOSEFILE, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__family_query(const H5FD_t *_file,
                                 unsigned long *flags /* out */) {
  const H5FD_family_t *file =
      (const H5FD_family_t *)_file; /* Family VFD info */
  ...
  if (flags) {
    *flags = 0;
    *flags |=
        H5FD_FEAT_AGGREGATE_METADATA; /* OK to aggregate metadata allocations */
    ...
    *flags |= H5FD_FEAT_AGGREGATE_SMALLDATA; /* OK to aggregate "small" raw data
    ...
    if (file && file->repart_members)
      *flags |= H5FD_FEAT_DIRTY_DRVRINFO_LOAD; /* Mark the superblock dirty when
                                                  it is loaded (so the family
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__family_get_eoa(const H5FD_t *_file,
 ...
 */
H5_GCC_CLANG_DIAG_OFF("format-nonliteral")
static herr_t H5FD__family_set_eoa(H5FD_t *_file, H5FD_mem_t type,
                                   haddr_t abs_eoa) {
  H5FD_family_t *file = (H5FD_family_t *)_file;
  ...
  for (u = 0; addr || u < file->nmembs; u++) {
    ...
    if (addr > (haddr_t)file->memb_size) {
      if (H5FD_set_eoa(file->memb[u], type,
                       ((haddr_t)file->memb_size - file->pub.base_addr)) < 0)
...
static herr_t H5FD__family_get_handle(H5FD_t *_file, hid_t fapl,
  ...
  if (H5P_get(plist, H5F_ACS_FAMILY_OFFSET_NAME, &offset) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get offset for family driver");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__family_flush(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
                                 hbool_t closing) {
  H5FD_family_t *file = (H5FD_family_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__family_truncate(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
                                    hbool_t closing) {
  H5FD_family_t *file = (H5FD_family_t *)_file;
  unsigned u, nerrors = 0;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (u < file->nmembs) {
    ...
    for (v = 0; v < u; v++) {
      if (H5FD_unlock(file->memb[v]) < 0)
        /* Push error, but keep going */
        HDONE_ERROR(H5E_IO, H5E_CANTUNLOCKFILE, FAIL,
                    "unable to unlock member files");
    } /* end for */
  ...
  for (u = 0; u < file->nmembs; u++)
    if (file->memb[u])
      if (H5FD_unlock(file->memb[u]) < 0)
        HGOTO_ERROR(H5E_IO, H5E_CANTUNLOCKFILE, FAIL,
                    "unable to unlock member files");
...
static herr_t H5FD__family_delete(const char *filename, hid_t fapl_id) {
  ...
  while (1) {
    ...
    H5_GCC_CLANG_DIAG_OFF("format-nonliteral")
    HDsnprintf(member_name, H5FD_FAM_MEMB_NAME_BUF_SIZE, filename,
               current_member);
    H5_GCC_CLANG_DIAG_ON("format-nonliteral")
     ...
     */
    H5E_BEGIN_TRY { delete_error = H5FD_delete(member_name, memb_fapl_id); }
    H5E_END_TRY
    if (FAIL == delete_error) {
      if (0 == current_member)
        HGOTO_ERROR(H5E_VFL, H5E_CANTDELETEFILE, FAIL,
                    "unable to delete member file");
      else
        H5E_clear_stack(NULL);
      break;
    }
