...
/* Local Prototypes */
/********************/
static herr_t H5R__encode_token_region_compat(H5F_t *f,
                                              const H5O_token_t *obj_token,
                                              size_t token_size, H5S_t *space,
                                              unsigned char *buf,
                                              size_t *nalloc);
static herr_t H5R__decode_token_compat(H5VL_object_t *vol_obj, H5I_type_t type,
                                       H5R_type_t ref_type,
...
static herr_t H5R__decode_token_compat(H5VL_object_t *vol_obj, H5I_type_t type,
  ...
  /* Get container info */
  if (H5VL_file_get(vol_obj_file, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                    H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, FAIL,
  ...
  if (ref_type == H5R_OBJECT1) {
    ...
    /* Get object address */
    if (H5R__decode_token_obj_compat(buf, &buf_size, obj_token,
                                     cont_info.token_size) < 0)
    ...
    /* Get object address */
    if (H5R__decode_token_region_compat(f, buf, &buf_size, obj_token,
                                        cont_info.token_size, NULL) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, FAIL,
...
H5G_obj_t H5Rget_obj_type1(hid_t id, H5R_type_t ref_type, const void *ref) {
  ...
  /* Get object token */
  if (H5R__decode_token_compat(vol_obj, vol_obj_type, ref_type, buf,
                               &obj_token) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, H5G_UNKNOWN,
                "unable to get object token");
...
hid_t H5Rdereference1(hid_t obj_id, H5R_type_t ref_type, const void *ref) {
  ...
  if (H5R__decode_token_compat(vol_obj, vol_obj_type, ref_type, buf,
                               &obj_token) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, H5I_INVALID_HID,
                "unable to get object token");
...
herr_t H5Rcreate(void *ref, hid_t loc_id, const char *name, H5R_type_t ref_type,
  ...
  if (ref_type == H5R_OBJECT1) {
    ...
    if ((ret_value = H5R__encode_token_obj_compat(
             (const H5O_token_t *)&obj_token, cont_info.token_size, buf,
             &buf_size)) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTENCODE, FAIL,
                  "unable to encode object reference");
  } /* end if */
  else {
    H5F_t *f = NULL;
    ...
    if ((ret_value = H5R__encode_token_region_compat(
             f, (const H5O_token_t *)&obj_token, cont_info.token_size, space,
             buf, &buf_size)) < 0)
      HGOTO_ERROR(H5E_REFERENCE, H5E_CANTENCODE, FAIL,
                  "unable to encode region reference");
  } /* end else */
...
hid_t H5Rget_region(hid_t id, H5R_type_t ref_type, const void *ref) {
  ...
  if (H5R__decode_token_region_compat(f, buf, &buf_size, NULL,
                                      cont_info.token_size, &space) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTGET, H5I_INVALID_HID,
  ...
  if (file_id != H5I_INVALID_HID && H5I_dec_ref(file_id) < 0)
    HDONE_ERROR(H5E_REFERENCE, H5E_CANTDEC, H5I_INVALID_HID,
                "unable to decrement refcount on file");
  FUNC_LEAVE_API(ret_value)
} /* end H5Rget_region1() */
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Rget_name(hid_t id, H5R_type_t ref_type, const void *ref,
  ...
  if (H5R__decode_token_compat(vol_obj, vol_obj_type, ref_type, buf,
                               &obj_token) < 0)
    HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, (-1),
                "unable to get object token");
