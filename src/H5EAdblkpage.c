 ...
 *-------------------------------------------------------------------------
 */
H5EA_dblk_page_t *H5EA__dblk_page_alloc(H5EA_hdr_t *hdr,
  ...
  if (NULL ==
      (dblk_page->elmts = H5EA__hdr_alloc_elmts(hdr, hdr->dblk_page_nelmts)))
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__dblk_page_create(H5EA_hdr_t *hdr, H5EA_sblock_t *parent,
                              haddr_t addr) {
  H5EA_dblk_page_t *dblk_page = NULL; /* Extensible array data block page */
  ...
  if ((hdr->cparam.cls->fill)(dblk_page->elmts, (size_t)hdr->dblk_page_nelmts) <
      0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTSET, FAIL,
                "can't set extensible array data block page elements to "
                "class's fill value");

  /* Cache the new extensible array data block page */
  if (H5AC_insert_entry(hdr->f, H5AC_EARRAY_DBLK_PAGE, dblk_page->addr,
                        dblk_page, H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_EARRAY, H5E_CANTINSERT, FAIL,
                "can't add extensible array data block page to cache");
  inserted = TRUE;
    ...
    if (dblk_page) {
      ...
      if (inserted)
        if (H5AC_remove_entry(dblk_page) < 0)
          HDONE_ERROR(
              H5E_EARRAY, H5E_CANTREMOVE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
H5EA_dblk_page_t *H5EA__dblk_page_protect(H5EA_hdr_t *hdr,
                                          H5EA_sblock_t *parent,
                                          haddr_t dblk_page_addr,
                                          unsigned flags) {
  H5EA_dblk_page_t *dblk_page = NULL; /* Extensible array data block page */
  H5EA_dblk_page_cache_ud_t
      udata; /* Information needed for loading data block page */
  H5EA_dblk_page_t *ret_value = NULL;
  ...
  if (NULL ==
      (dblk_page = (H5EA_dblk_page_t *)H5AC_protect(
           hdr->f, H5AC_EARRAY_DBLK_PAGE, dblk_page_addr, &udata, flags)))
    HGOTO_ERROR(
  ...
  if (!ret_value) {
    ...
    if (dblk_page &&
        H5AC_unprotect(hdr->f, H5AC_EARRAY_DBLK_PAGE, dblk_page->addr,
                       dblk_page, H5AC__NO_FLAGS_SET) < 0)
      HDONE_ERROR(H5E_EARRAY, H5E_CANTUNPROTECT, NULL,
                  "unable to unprotect extensible array data block page, "
                  "address = %llu",
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5EA__dblk_page_unprotect(H5EA_dblk_page_t *dblk_page,
  ...
  if (H5AC_unprotect(dblk_page->hdr->f, H5AC_EARRAY_DBLK_PAGE, dblk_page->addr,
                     dblk_page, cache_flags) < 0)
    HGOTO_ERROR(
...
herr_t H5EA__dblk_page_dest(H5EA_dblk_page_t *dblk_page) {
  ...
  if (dblk_page->hdr) {
    ...
    if (dblk_page->elmts) {
      ...
      if (H5EA__hdr_free_elmts(dblk_page->hdr, dblk_page->hdr->dblk_page_nelmts,
                               dblk_page->elmts) < 0)
        HGOTO_ERROR(
