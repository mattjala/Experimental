 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__print_stats(H5T_path_t H5_ATTR_UNUSED *path,
                        int H5_ATTR_UNUSED *nprint /*in,out*/) {
  ...
  if (H5DEBUG(T) && path->stats.ncalls > 0) {
    ...
    if (nprint && 0 == (*nprint)++) {
      ...
      fprintf(H5DEBUG(T), "   %-16s %10s %10s %8s %8s %8s %10s\n", "Conversion",
              "Elmts", "Calls", "User", "System", "Elapsed", "Bandwidth");
      fprintf(H5DEBUG(T), "   %-16s %10s %10s %8s %8s %8s %10s\n", "----------",
              "-----", "-----", "----", "------", "-------", "---------");
    } /* end if */
    ...
    H5_bandwidth(bandwidth, sizeof(bandwidth), (double)nbytes,
                 path->stats.times.elapsed);
    fprintf(H5DEBUG(T), "   %-16s %10" PRIdHSIZE " %10u %8s %8s %8s %10s\n",
            path->name, path->stats.nelmts, path->stats.ncalls, timestrs.user,
...
herr_t H5T_debug(const H5T_t *dt, FILE *stream) {
  ...
  fprintf(stream, "%s%s {nbytes=%lu", s1, s2,
          (unsigned long)(dt->shared->size));
  ...
  if (H5T_IS_ATOMIC(dt->shared)) {
    ...
    case H5T_FLOAT:
      ...
      if (tmp) {
        ...
        fprintf(stream, " bias=0x%08lx%08lx", (unsigned long)hi,
                (unsigned long)lo);
      } else {
        size_t lo = (size_t)(dt->shared->u.atomic.u.f.ebias & 0xffffffff);
