 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5FD__alloc_real(H5FD_t *file, H5FD_mem_t type, hsize_t size,
  ...
  extra = 0;
  if (!file->paged_aggr && file->alignment > 1 &&
      orig_size >= file->threshold) {
  ...
  /* For the multi/split driver: the size passed down to the alloc callback is
   * the original size from H5FD_alloc() */
  /* For all other drivers: the size passed down to the alloc callback is the
   * size + [possibly] alignment size */
  if (file->cls->alloc) {
    ret_value = (file->cls->alloc)(file, type, H5CX_get_dxpl(),
 ...
 *-------------------------------------------------------------------------
 */
haddr_t H5FD_alloc(H5FD_t *file, H5FD_mem_t type, H5F_t *f, hsize_t size,
                   haddr_t *frag_addr, hsize_t *frag_size) {
  haddr_t ret_value = HADDR_UNDEF; /* Return value */
  ...
  if (!H5_addr_defined(ret_value))
    HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, HADDR_UNDEF,
                "real 'alloc' request failed");

  /* Mark EOA info dirty in cache, so change will get encoded */
  if (H5F_eoa_dirty(f) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTMARKDIRTY, HADDR_UNDEF,
                "unable to mark EOA info as dirty");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD__free_real(H5FD_t *file, H5FD_mem_t type, haddr_t addr,
                       hsize_t size) {
...
#ifdef H5FD_ALLOC_DEBUG
  fprintf(stderr,
          "%s: type = %u, addr = %" PRIuHADDR ", size = %" PRIuHSIZE "\n",
  ...
  if (addr > file->maxaddr || H5_addr_overflow(addr, size) ||
      (addr + size) > file->maxaddr)
    HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                "invalid file free space region to free");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FD_free(H5FD_t *file, H5FD_mem_t type, H5F_t *f, haddr_t addr,
                 hsize_t size) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Mark EOA info dirty in cache, so change will get encoded */
  if (H5F_eoa_dirty(f) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTMARKDIRTY, FAIL,
                "unable to mark EOA info as dirty");
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5FD_try_extend(H5FD_t *file, H5FD_mem_t type, H5F_t *f, haddr_t blk_end,
  ...
  if (H5_addr_eq(blk_end, eoa)) {
    ...
    if (HADDR_UNDEF == H5FD__extend(file, type, extra_requested))
      HGOTO_ERROR(H5E_VFL, H5E_CANTEXTEND, FAIL,
                  "driver extend request failed");
