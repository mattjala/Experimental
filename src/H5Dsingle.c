 ...
 *          This is used when the dataset has only 1 chunk (with or without
 * filter): cur_dims[] is equal to max_dims[] is equal to the chunk dims[]
 *          non-filter chunk record: [address of the chunk]
 *          filtered chunk record: 	[address of the chunk, chunk size,
 * filter mask]
 */
...
static herr_t H5D__single_idx_size(const H5D_chk_idx_info_t *idx_info,
                                   hsize_t *size);
static herr_t H5D__single_idx_reset(H5O_storage_chunk_t *storage,
                                    hbool_t reset_addr);
static herr_t H5D__single_idx_dump(const H5O_storage_chunk_t *storage,
                                   FILE *stream);
...
static herr_t H5D__single_idx_insert(const H5D_chk_idx_info_t *idx_info,
  ...
  if (idx_info->pline->nused > 0) {
    H5_CHECKED_ASSIGN(idx_info->storage->u.single.nbytes, uint32_t,
  ...
  if (dset)
    if (dset->shared->dcpl_cache.fill.alloc_time != H5D_ALLOC_TIME_EARLY ||
      ...
      /* Mark the layout dirty so that the address of the single chunk will be
       * flushed later */
      if (H5D__mark(dset, H5D_MARK_LAYOUT) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                    "unable to mark layout as dirty");
 ...
 */
static herr_t
H5D__single_idx_remove(const H5D_chk_idx_info_t *idx_info,
  ...
  if (H5MF_xfree(idx_info->f, H5FD_MEM_DRAW, idx_info->storage->idx_addr,
                 nbytes) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, H5_ITER_ERROR,
                "unable to free dataset chunks");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__single_idx_copy_setup(
    const H5D_chk_idx_info_t H5_ATTR_NDEBUG_UNUSED *idx_info_src,
    const H5D_chk_idx_info_t *idx_info_dst) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 */
static herr_t
H5D__single_idx_size(const H5D_chk_idx_info_t H5_ATTR_UNUSED *idx_info,
                     hsize_t *index_size) {
  FUNC_ENTER_PACKAGE_NOERR
