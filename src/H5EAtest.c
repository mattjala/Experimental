...
static void *H5EA__test_crt_dbg_context(H5F_t H5_ATTR_UNUSED *f,
                                        haddr_t H5_ATTR_UNUSED obj_addr);
...
static herr_t H5EA__test_dst_context(void *_ctx) {
  H5EA__test_ctx_t *ctx =
      (H5EA__test_ctx_t *)_ctx; /* Callback context to destroy */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__test_encode(void *raw, const void *_elmt, size_t nelmts,
                                void *_ctx) {
  H5EA__test_ctx_t *ctx =
      (H5EA__test_ctx_t *)_ctx; /* Callback context to destroy */
  const uint64_t *elmt =
      (const uint64_t *)_elmt; /* Convenience pointer to native elements */
  ...
  if (ctx->cb) {
    if ((*ctx->cb->encode)(elmt, nelmts, ctx->cb->udata) < 0)
      HGOTO_ERROR(H5E_EARRAY, H5E_BADVALUE, FAIL,
                  "extensible array testing callback action failed");
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5EA__test_decode(const void *_raw, void *_elmt, size_t nelmts,
                                void H5_ATTR_NDEBUG_UNUSED *_ctx) {
#ifndef NDEBUG
  H5EA__test_ctx_t *ctx =
      (H5EA__test_ctx_t *)_ctx; /* Callback context to destroy */
#endif                          /* NDEBUG */
...
static herr_t H5EA__test_debug(FILE *stream, int indent, int fwidth,
  ...
  HDsnprintf(temp_str, sizeof(temp_str),
             "Element #%llu:", (unsigned long long)idx);
...
static herr_t H5EA__test_dst_dbg_context(void *_ctx) {
  H5EA__ctx_cb_t *ctx =
      (H5EA__ctx_cb_t *)_ctx; /* Callback context to destroy */
 ...
 *-------------------------------------------------------------------------
 */
int H5EA__cmp_cparam_test(const H5EA_create_t *cparam1,
  ...
  if (cparam1->max_dblk_page_nelmts_bits < cparam2->max_dblk_page_nelmts_bits)
    HGOTO_DONE(-1);
  else if (cparam1->max_dblk_page_nelmts_bits >
           cparam2->max_dblk_page_nelmts_bits)
    HGOTO_DONE(1);
