...
static herr_t H5C__decode_cache_image_header(const H5F_t *f, H5C_t *cache_ptr,
                                             const uint8_t **buf);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_cache_image_status(H5F_t *f, hbool_t *load_ci_ptr,
...
static herr_t H5C__construct_cache_image_buffer(H5F_t *f, H5C_t *cache_ptr) {
  ...
  if (H5C__encode_cache_image_header(f, cache_ptr, &p) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTENCODE, FAIL,
                "header image construction failed");
  assert((size_t)(p - (uint8_t *)cache_ptr->image_buffer) <
   ...
   */
  {
    ...
    for (u = 0; u < fake_cache_ptr->num_entries_in_image; u++) {
      ...
      assert(cache_ptr->image_entries[u].addr ==
             fake_cache_ptr->image_entries[u].addr);
      assert(cache_ptr->image_entries[u].size ==
             fake_cache_ptr->image_entries[u].size);
      assert(cache_ptr->image_entries[u].type_id ==
             fake_cache_ptr->image_entries[u].type_id);
      assert(cache_ptr->image_entries[u].lru_rank ==
             fake_cache_ptr->image_entries[u].lru_rank);
      assert(cache_ptr->image_entries[u].is_dirty ==
             fake_cache_ptr->image_entries[u].is_dirty);
...
herr_t H5C__generate_cache_image(H5F_t *f, H5C_t *cache_ptr) {
  ...
  if (H5C__construct_cache_image_buffer(f, cache_ptr) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                "Can't create metadata cache image");
  ...
  if (cache_ptr->image_entries != NULL) {
    ...
    for (u = 0; u < cache_ptr->num_entries_in_image; u++) {
      ...
      if (ie_ptr->fd_parent_addrs) {
        ...
        ie_ptr->fd_parent_addrs =
            (haddr_t *)H5MM_xfree(ie_ptr->fd_parent_addrs);
      } /* end if */
      else
        assert(ie_ptr->fd_parent_count == 0);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__get_cache_image_config(const H5C_t *cache_ptr,
...
static herr_t H5C__read_cache_image(H5F_t *f, H5C_t *cache_ptr) {
...
#ifdef H5_HAVE_PARALLEL
  {
    ...
    if (NULL == aux_ptr || aux_ptr->mpi_rank == 0) {
       ...
       * an error here will cause other ranks to hang in the following
       * MPI_Bcast.
       */
      ...
      if (aux_ptr) {
        ...
        if (MPI_SUCCESS !=
            (mpi_result =
                 MPI_Bcast(cache_ptr->image_buffer, (int)cache_ptr->image_len,
...
herr_t H5C__load_cache_image(H5F_t *f) {
  ...
  if (H5_addr_defined(cache_ptr->image_addr)) {
    ...
    if (H5C__reconstruct_cache_contents(f, cache_ptr) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTDECODE, FAIL,
  ...
  if (cache_ptr->delete_image) {
    if (H5F__super_ext_remove_msg(f, H5O_MDCI_MSG_ID) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTREMOVE, FAIL,
                  "can't remove metadata cache image message from superblock "
                  "extension");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_load_cache_image_on_next_protect(H5F_t *f, haddr_t addr, hsize_t len,
                                            hbool_t rw) {
  H5C_t *cache_ptr;
...
static int H5C__image_entry_cmp(const void *_entry1, const void *_entry2) {
  const H5C_image_entry_t *entry1 = (const H5C_image_entry_t *)
      _entry1; /* Pointer to first image entry to compare */
  const H5C_image_entry_t *entry2 = (const H5C_image_entry_t *)
      _entry2;       /* Pointer to second image entry to compare */
  ...
  if (cache_ptr->image_ctl.generate_image) {
    ...
    if (cache_ptr->aux_ptr) { /* we have multiple processes */
      ...
      if (aux_ptr->mpi_rank == 0) {
        ...
        if (MPI_SUCCESS !=
            (mpi_result = MPI_Bcast(&p0_image_len, 1, MPI_UNSIGNED, 0,
                                    aux_ptr->mpi_comm)))
          HMPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_result)
       ...
       */
      if (HADDR_UNDEF ==
          (cache_ptr->image_addr = H5FD_alloc(f->shared->lf, H5FD_MEM_SUPER, f,
      ...
      if (HADDR_UNDEF == (cache_ptr->image_addr =
                              H5FD_alloc(f->shared->lf, H5FD_MEM_SUPER, f,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_set_cache_image_config(const H5F_t *f, H5C_t *cache_ptr,
                                  H5C_cache_image_ctl_t *config_ptr) {
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5C_validate_cache_image_config(H5C_cache_image_ctl_t *ctl_ptr) {
  ...
  if (ctl_ptr->save_resize_status != FALSE)
    HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
                "unexpected value in save_resize_status field");
  ...
  if (ctl_ptr->entry_ageout != H5AC__CACHE_IMAGE__ENTRY_AGEOUT__NONE)
    HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
                "unexpected value in entry_ageout field");
...
static herr_t H5C__decode_cache_image_header(const H5F_t *f, H5C_t *cache_ptr,
  ...
  /* Check signature */
  if (memcmp(p, H5C__MDCI_BLOCK_SIGNATURE,
             (size_t)H5C__MDCI_BLOCK_SIGNATURE_LEN) != 0)
    HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
  ...
  if (have_resize_status)
    HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
                "MDC resize status not yet supported");
  ...
  if (cache_ptr->image_data_len != cache_ptr->image_len)
    HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
                "Bad metadata cache image data length");
...
static herr_t H5C__decode_cache_image_entry(const H5F_t *f,
  ...
  assert((is_fd_parent && fd_child_count > 0) ||
  ...
  if (fd_dirty_child_count > fd_child_count)
    HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
                "invalid dirty flush dependency child count");

  /* Decode dependency parent count */
  UINT16DECODE(p, fd_parent_count);
  assert((is_fd_child && fd_parent_count > 0) ||
         (!is_fd_child && fd_parent_count == 0));
  ...
  if (fd_parent_count > 0) {
    ...
    if (NULL == (fd_parent_addrs = (haddr_t *)H5MM_malloc(
                     (size_t)(fd_parent_count)*H5F_SIZEOF_ADDR(f))))
      HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, FAIL,
    ...
    for (i = 0; i < fd_parent_count; i++) {
      ...
      if (!H5_addr_defined(fd_parent_addrs[i]))
        HGOTO_ERROR(H5E_CACHE, H5E_BADVALUE, FAIL,
                    "invalid flush dependency parent offset");
    } /* end for */
  }   /* end if */
...
#if H5C_DO_MEMORY_SANITY_CHECKS
  H5MM_memcpy(((uint8_t *)image_ptr) + size, H5C_IMAGE_SANITY_VALUE,
...
static herr_t H5C__encode_cache_image_header(const H5F_t *f,
  ...
  /* write signature */
  H5MM_memcpy(p, H5C__MDCI_BLOCK_SIGNATURE,
...
static herr_t H5C__encode_cache_image_entry(H5F_t *f, H5C_t *cache_ptr,
  ...
  *p++ = (uint8_t)(ie_ptr->age);
  ...
  if (ie_ptr->fd_dirty_child_count > H5C__MDCI_MAX_FD_CHILDREN)
    HGOTO_ERROR(H5E_CACHE, H5E_BADRANGE, FAIL,
                "fd_dirty_child_count out of range");
  ...
  while (entry_ptr != NULL) {
    if (!entry_ptr->include_in_image && entry_ptr->flush_dep_nparents > 0) {
      ...
      if (entry_ptr->fd_parent_count == 0) {
        ...
        if (NULL ==
            (entry_ptr->fd_parent_addrs = (haddr_t *)H5MM_calloc(
...
H5C__prep_for_file_close__compute_fd_heights_real(H5C_cache_entry_t *entry_ptr,
  ...
  assert((entry_ptr->image_fd_height == 0) ||
  ...
  if (entry_ptr->flush_dep_nparents > 0) {
    ...
    for (u = 0; u < entry_ptr->fd_parent_count; u++) {
      ...
      if (parent_ptr->include_in_image &&
        ...
        H5C__prep_for_file_close__compute_fd_heights_real(parent_ptr,
                                                          fd_height + 1);
    } /* end for */
  }   /* end if */
...
static herr_t H5C__prep_for_file_close__scan_entries(const H5F_t *f,
      ...
      if (entry_ptr->flush_dep_nparents > 0) {
        ...
        if (entry_ptr->flush_dep_nparents == entry_ptr->fd_parent_count) {
          ...
          entry_ptr->fd_parent_addrs =
              (haddr_t *)H5MM_xfree(entry_ptr->fd_parent_addrs);
        } /* end else-if */
        else {
          assert(entry_ptr->fd_parent_count == 0);
        ...
        if (NULL == entry_ptr->fd_parent_addrs)
          if (NULL ==
              (entry_ptr->fd_parent_addrs = (haddr_t *)H5MM_malloc(
  ...
  if (H5C__prep_for_file_close__compute_fd_heights(cache_ptr) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "computation of flush dependency heights failed?!?");
...
static herr_t H5C__reconstruct_cache_contents(H5F_t *f, H5C_t *cache_ptr) {
  ...
  for (u = 0; u < cache_ptr->num_entries_in_image; u++) {
    ...
    for (v = 0; v < pf_entry_ptr->fd_parent_count; v++) {
      ...
      parent_ptr = NULL;
      H5C__SEARCH_INDEX(cache_ptr, pf_entry_ptr->fd_parent_addrs[v], parent_ptr,
                        FAIL);
      if (parent_ptr == NULL)
  ...
  while (pf_entry_ptr != NULL) {
    ...
    if (pf_entry_ptr->type == H5AC_PREFETCHED_ENTRY) {
      ...
      assert(pf_entry_ptr->fd_dirty_child_count ==
             pf_entry_ptr->flush_dep_ndirty_children);
    } /* end if */
  ...
  if (cache_ptr->index_size >= cache_ptr->max_cache_size) {
    ...
    if (cache_ptr->check_write_permitted &&
        (cache_ptr->check_write_permitted)(f, &write_permitted) < 0)
...
static H5C_cache_entry_t *H5C__reconstruct_cache_entry(const H5F_t *f,
  ...
  assert((in_lru && pf_entry_ptr->lru_rank >= 0) ||
         (!in_lru && pf_entry_ptr->lru_rank == -1));
  ...
  if (pf_entry_ptr->fd_parent_count > 0) {
    ...
    if (NULL ==
        (pf_entry_ptr->fd_parent_addrs = (haddr_t *)H5MM_malloc(
  ...
  /* Allocate buffer for entry image */
  if (NULL == (pf_entry_ptr->image_ptr =
                   H5MM_malloc(pf_entry_ptr->size + H5C_IMAGE_EXTRA_SPACE)))
    HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, NULL,
                "memory allocation failed for on disk image buffer");
#if H5C_DO_MEMORY_SANITY_CHECKS
  H5MM_memcpy(((uint8_t *)pf_entry_ptr->image_ptr) + size,
...
static herr_t H5C__write_cache_image_superblock_msg(H5F_t *f, hbool_t create) {
  ...
  if (H5F__super_ext_write_msg(f, H5O_MDCI_MSG_ID, &mdci_msg, create,
                               mesg_flags) < 0)
    HGOTO_ERROR(
        H5E_CACHE, H5E_WRITEERROR, FAIL,
