...
static herr_t H5MF__sects_debug_cb(H5FS_section_info_t *_sect, void *_udata) {
  H5MF_free_section_t *sect =
      (H5MF_free_section_t *)_sect; /* Section to dump info */
  H5MF_debug_iter_ud_t *udata =
      (H5MF_debug_iter_ud_t *)_udata; /* User data for callbacks */
  ...
  fprintf(udata->stream, "%*s%-*s %s\n", udata->indent, "", udata->fwidth,
          ...
          (sect->sect_info.type == H5MF_FSPACE_SECT_SIMPLE
               ...
               : (sect->sect_info.type == H5MF_FSPACE_SECT_SMALL
                      ? "small"
                      : (sect->sect_info.type == H5MF_FSPACE_SECT_LARGE
                             ? "large"
                             : "unknown"))));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5MF_sects_debug(H5F_t *f, haddr_t fs_addr, FILE *stream, int indent,
                        int fwidth) {
  ...
  for (type = H5F_MEM_PAGE_DEFAULT; type < H5F_MEM_PAGE_NTYPES; type++)
    if (H5_addr_eq(f->shared->fs_addr[type], fs_addr)) {
      ...
      if (f->shared->fs_man[type]) {
        ...
        if (H5FS_sect_iterate(f, f->shared->fs_man[type], H5MF__sects_debug_cb,
                              &udata) < 0)
  ...
  if (H5F_PAGED_AGGR(f)) { /* File space paging */
    ...
    for (ptype = H5F_MEM_PAGE_META; ptype < H5F_MEM_PAGE_NTYPES; ptype++) {
      ...
      if (f->shared->fs_man[ptype]) {
        ...
        /* Iterate over all the free space sections */
        if (H5FS_sect_iterate(f, f->shared->fs_man[ptype], H5MF__sects_debug_cb,
                              &udata) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_BADITER, FAIL,
                      "can't iterate over heap's free space");
      } /* end if */
...
#ifdef H5MF_ALLOC_DEBUG
    fprintf(stderr,
            "%s: ma_addr = %" PRIuHADDR ", ma_size = %" PRIuHSIZE
            ", end of ma = %" PRIuHADDR "\n",
...
#ifdef H5MF_ALLOC_DEBUG
    fprintf(stderr,
            "%s: sda_addr = %" PRIuHADDR ", sda_size = %" PRIuHSIZE
            ", end of sda = %" PRIuHADDR "\n",
            __func__, sda_addr, sda_size, (haddr_t)((sda_addr + sda_size) - 1));
#endif /* H5MF_ALLOC_DEBUG */

    /* Iterate over all the free space types that have managers and dump each
     * free list's space */
    for (atype = H5FD_MEM_DEFAULT; atype < H5FD_MEM_NTYPES; atype++) {
      ...
      if (H5FD_MEM_DEFAULT == f->shared->fs_type_map[atype] ||
          atype == f->shared->fs_type_map[atype]) {
        /* Retrieve the 'eoa' for this file memory type */
        if (HADDR_UNDEF == (eoa = H5F_get_eoa(f, atype)))
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                      "driver get_eoa request failed");
        fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent + 3, "",
                MAX(0, fwidth - 3), "eoa:", eoa);
        ...
        if (f->shared->fs_man[atype]) {
          ...
          /* Iterate over all the free space sections */
          if (H5FS_sect_iterate(f, f->shared->fs_man[atype],
                                H5MF__sects_debug_cb, &udata) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_BADITER, FAIL,
                        "can't iterate over heap's free space");
        }    /* end if */
