...
#define H5FA_DBLOCK_PREFIX_SIZE(d)                                             \
  (/* General metadata fields */                                               \
   ...
   + (d)->hdr->sizeof_addr /* File address of Fixed Array header owning the    \
                              data block */                                    \
                                                                               \
   /* Fixed Array Data Block specific fields */                                \
   + (d)->dblk_page_init_size /* Fixed array data block 'page init' bitmasks   \
                                 (can be 0 if no pages) */                     \
  )
...
typedef struct H5FA_dblock_t {
  ...
  uint8_t *
      dblk_page_init; /* Bitmap of whether a data block page is initialized */
...
H5_DLL haddr_t H5FA__hdr_create(H5F_t *f, const H5FA_create_t *cparam,
                                void *ctx_udata);
...
H5_DLL herr_t H5FA__dblk_page_create(H5FA_hdr_t *hdr, haddr_t addr,
                                     size_t nelmts);
