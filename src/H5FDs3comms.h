...
#define S3COMMS_FORMAT_CREDENTIAL(dest, access, iso8601_date, region, service) \
  HDsnprintf((dest), S3COMMS_MAX_CREDENTIAL_SIZE, "%s/%s/%s/%s/aws4_request",  \
...
H5_DLL herr_t H5FD_s3comms_hrb_node_set(hrb_node_t **L, const char *name,
                                        const char *value);
...
H5_DLL s3r_t *H5FD_s3comms_s3r_open(const char url[], const char region[],
                                    const char id[],
                                    const unsigned char signing_key[],
                                    const char token[]);

H5_DLL herr_t H5FD_s3comms_s3r_read(s3r_t *handle, haddr_t offset, size_t len,
                                    void *dest);
...
H5_DLL herr_t H5FD_s3comms_bytes_to_hex(char *dest, const unsigned char *msg,
                                        size_t msg_len, hbool_t lowercase);

H5_DLL herr_t H5FD_s3comms_free_purl(parsed_url_t *purl);

H5_DLL herr_t H5FD_s3comms_HMAC_SHA256(const unsigned char *key, size_t key_len,
...
H5_DLL herr_t H5FD_s3comms_signing_key(unsigned char *md, const char *secret,
                                       const char *region,
                                       const char *iso8601now);

H5_DLL herr_t H5FD_s3comms_tostringtosign(char *dest, const char *req_str,
                                          const char *now, const char *region);

H5_DLL herr_t H5FD_s3comms_trim(char *dest, char *s, size_t s_len,
                                size_t *n_written);
