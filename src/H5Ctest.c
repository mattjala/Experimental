...
static int H5C__verify_cork_tag_test_cb(H5C_cache_entry_t *entry, void *_ctx) {
  H5C_tag_iter_vct_ctx_t *ctx =
      (H5C_tag_iter_vct_ctx_t *)_ctx; /* Get pointer to iterator context */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__verify_cork_tag_test(hid_t fid, H5O_token_t tag_token,
                                 hbool_t status) {
  ...
  if (H5C__iter_tagged_entries(cache, tag, FALSE, H5C__verify_cork_tag_test_cb,
                               &ctx) < 0)
