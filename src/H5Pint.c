...
typedef herr_t (*H5P_do_plist_op_t)(H5P_genplist_t *plist, const char *name,
                                    H5P_genprop_t *prop, void *udata);
typedef herr_t (*H5P_do_pclass_op_t)(H5P_genplist_t *plist, const char *name,
                                     H5P_genprop_t *prop, void *udata);
...
static H5P_genprop_t *H5P__create_prop(
    const char *name, size_t size, H5P_prop_within_t type, const void *value,
...
static H5P_genprop_t *H5P__dup_prop(H5P_genprop_t *oprop,
                                    H5P_prop_within_t type);
static herr_t H5P__free_prop(H5P_genprop_t *prop);
static int H5P__cmp_prop(const H5P_genprop_t *prop1,
                         const H5P_genprop_t *prop2);
static herr_t H5P__do_prop(H5P_genplist_t *plist, const char *name,
                           H5P_do_plist_op_t plist_op,
                           H5P_do_pclass_op_t pclass_op, void *udata);
static int H5P__open_class_path_cb(void *_obj, hid_t H5_ATTR_UNUSED id,
                                   void *_key);
static H5P_genprop_t *H5P__find_prop_pclass(H5P_genclass_t *pclass,
                                            const char *name);
static herr_t H5P__free_prop_cb(void *item, void H5_ATTR_UNUSED *key,
                                void *op_data);
static herr_t H5P__free_del_name_cb(void *item, void H5_ATTR_UNUSED *key,
...
static const H5I_class_t H5I_GENPROPCLS_CLS[1] = {{
    ...
    (H5I_free_t)H5P__close_class_cb /* Callback routine for closing objects of
                                       this class */
}};
...
static const H5I_class_t H5I_GENPROPLST_CLS[1] = {{
    ...
    0,                             /* Class flags */
    0,                             /* # of reserved IDs for class */
    (H5I_free_t)H5P__close_list_cb /* Callback routine for closing objects of
                                      this class */
}};
...
herr_t H5P_init_phase1(void) {
  ...
  do {
    ...
    for (u = 0; u < NELMTS(init_class); u++) {
      H5P_libclass_t const *lib_class =
          init_class[u]; /* Current class to operate on */
      ...
      if (*lib_class->class_id == (-1) &&
        ...
        /* Sanity check - only the root class is not allowed to have a parent
         * class */
        assert(lib_class->par_pclass || lib_class == H5P_CLS_ROOT);

        /* Allocate the new class */
        if (NULL == (*lib_class->pclass = H5P__create_class(
                         lib_class->par_pclass ? *lib_class->par_pclass : NULL,
                         lib_class->name, lib_class->type,
                         lib_class->create_func, lib_class->create_data,
                         lib_class->copy_func, lib_class->copy_data,
                         lib_class->close_func, lib_class->close_data)))
          HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
                      "class initialization failed");

        /* Call routine to register properties for class */
        if (lib_class->reg_prop_func &&
            (*lib_class->reg_prop_func)(*lib_class->pclass) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                      "can't register properties");

        /* Register the new class */
        if ((*lib_class->class_id =
                 H5I_register(H5I_GENPROP_CLS, *lib_class->pclass, FALSE)) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                      "can't register property list class");

        /* Only register the default property list if it hasn't been created yet
         */
        if (lib_class->def_plist_id && *lib_class->def_plist_id == (-1)) {
          /* Register the default property list for the new class*/
          if ((*lib_class->def_plist_id =
                   H5P_create_id(*lib_class->pclass, FALSE)) < 0)
            HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__close_class_cb(void *_pclass,
                                  void H5_ATTR_UNUSED **request) {
  H5P_genclass_t *pclass =
      (H5P_genclass_t *)_pclass; /* Property list class to close */
  herr_t ret_value = SUCCEED;    /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__do_prop_cb1(H5SL_t *slist, H5P_genprop_t *prop,
                               H5P_prp_cb1_t cb) {
  ...
  if (H5P__add_prop(slist, pcopy) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "Can't insert property into skip list");
...
H5P_genclass_t *H5P__copy_pclass(H5P_genclass_t *pclass) {
  ...
  if (NULL == (new_pclass = H5P__create_class(
                   pclass->parent, pclass->name, pclass->type,
    ...
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCREATE, NULL,
                "unable to create property list class");
  ...
  if (pclass->nprops > 0) {
    ...
    while (curr_node != NULL) {
      /* Make a copy of the class's property */
      if (NULL == (pcopy = H5P__dup_prop((H5P_genprop_t *)H5SL_item(curr_node),
...
hid_t H5P_copy_plist(const H5P_genplist_t *old_plist, hbool_t app_ref) {
  ...
  hbool_t has_parent_class; /* Flag to indicate that this property list's class
                               has a parent */
  ...
  new_plist->nprops = 0; /* Initially the plist has the same number of
                            properties as the class */
  new_plist->class_init =
      FALSE; /* Initially, wait until the class callback finishes to set */
  ...
  if ((seen = H5SL_create(H5SL_TYPE_STR, NULL)) == NULL)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCREATE, H5I_INVALID_HID,
                "can't create skip list for seen properties");
  nseen = 0;

  /* Cycle through the deleted properties & copy them into the new list's
   * deleted section */
  ...
  if (H5SL_count(old_plist->props) > 0) {
    ...
    while (curr_node) {
      ...
      if (new_prop->copy) {
        if ((new_prop->copy)(new_prop->name, new_prop->size, new_prop->value) <
            0) {
      ...
      if (H5P__add_prop(new_plist->props, new_prop) < 0) {
        H5P__free_prop(new_prop);
        HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, H5I_INVALID_HID,
                    "Can't insert property into list");
      } /* end if */
  ...
  /*
   * Check for copying class properties (up through list of parent classes
   * also), initialize each with default value & make property 'copy' callback.
   */
  tclass = old_plist->pclass;
  has_parent_class = (hbool_t)(tclass != NULL && tclass->parent != NULL &&
                               tclass->parent->nprops > 0);
  while (tclass != NULL) {
    if (tclass->nprops > 0) {
      /* Walk through the properties in the old class */
      ...
      while (curr_node != NULL) {
        ...
        if (nseen == 0 || H5SL_search(seen, tmp->name) == NULL) {
          /* Call property copy callback, if it exists */
          if (tmp->copy) {
            /* Call the callback & insert changed value into skip list (if
             * necessary) */
          ...
          /* Add property name to "seen" list, if we have other classes to work
           * on */
          if (has_parent_class) {
  ...
  if (H5P__access_class(new_plist->pclass, H5P_MOD_INC_LST) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, H5I_INVALID_HID,
                "Can't increment class ref count");
  ...
  /* Save the property list ID in the property list struct, for use in the
   * property class's 'close' callback
   */
  new_plist->plist_id = new_plist_id;

  /* Call the class callback (if it exists) now that we have the property list
   * ID (up through chain of parent classes also)
   */
  tclass = new_plist->pclass;
  while (NULL != tclass) {
    if (NULL != tclass->copy_func) {
      if ((tclass->copy_func)(new_plist_id, old_plist->plist_id,
                              old_plist->pclass->copy_data) < 0) {
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    H5P_genprop_t *H5P__dup_prop(oprop)
        ...
        H5P_prop_within_t type; IN: Type of object the property will be inserted
into RETURNS Returns a pointer to the newly created duplicate of a property on
success, NULL on failure. DESCRIPTION Allocates memory and copies property
information into a new property object. GLOBAL VARIABLES COMMENTS, BUGS,
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    H5P_genprop_t *H5P__create_prop(name,size,type,value,prp_create,prp_set,
                                   prp_get,prp_delete,prp_close, prp_encode,
prp_decode) const char *name;       IN: Name of property to register size_t
size;            IN: Size of property in bytes H5P_prop_within_t type; IN: Type
of object the property will be inserted into void *value;            IN: Pointer
to buffer containing value for property H5P_prp_create_func_t prp_create;   IN:
Function pointer to property creation callback H5P_prp_set_func_t prp_set; IN:
Function pointer to property set callback H5P_prp_get_func_t prp_get; IN:
Function pointer to property get callback H5P_prp_encode_func_t prp_encode; IN:
Function pointer to property encode H5P_prp_decode_func_t prp_decode; IN:
Function pointer to property decode H5P_prp_delete_func_t prp_delete; IN:
Function pointer to property delete callback H5P_prp_copy_func_t prp_copy; IN:
Function pointer to property copy callback H5P_prp_compare_func_t prp_cmp; IN:
Function pointer to property compare callback H5P_prp_close_func_t prp_close;
IN: Function pointer to property close callback RETURNS Returns a pointer to the
newly created property on success, NULL on failure. DESCRIPTION Allocates memory
and copies property information into a new property object. GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
H5P_genprop_t *H5P__find_prop_plist(const H5P_genplist_t *plist,
                                    const char *name) {
  H5P_genprop_t *ret_value = NULL; /* Return value */
  ...
  if (H5SL_search(plist->del, name) != NULL) {
    HGOTO_ERROR(H5E_PLIST, H5E_NOTFOUND, NULL,
                "property deleted from skip list");
  } /* end if */
  else {
    /* Get the property data from the skip list */
    if (NULL ==
        (ret_value = (H5P_genprop_t *)H5SL_search(plist->props, name))) {
      H5P_genclass_t *tclass; /* Temporary class pointer */

      /* Couldn't find property in list itself, start searching through class
       * info */
      tclass = plist->pclass;
      while (tclass != NULL) {
        /* Find the property in the class */
        if (NULL !=
            (ret_value = (H5P_genprop_t *)H5SL_search(tclass->props, name)))
      ...
      if (ret_value == NULL)
        HGOTO_ERROR(H5E_PLIST, H5E_NOTFOUND, NULL,
                    "can't find property in skip list");
    } /* end else */
  }   /* end else */
...
static herr_t H5P__free_prop_cb(void *item, void H5_ATTR_UNUSED *key,
                                void *op_data) {
  H5P_genprop_t *tprop =
      (H5P_genprop_t *)item; /* Temporary pointer to property */
  hbool_t make_cb =
      *(hbool_t *)op_data; /* Whether to make property 'close' callback */

  FUNC_ENTER_PACKAGE_NOERR

  assert(tprop);

  /* Call the close callback and ignore the return value, there's nothing we can
   * do about it */
...
static int H5P__open_class_path_cb(void *_obj, hid_t H5_ATTR_UNUSED id,
                                   void *_key) {
  H5P_genclass_t *obj =
      (H5P_genclass_t *)_obj; /* Pointer to the class for this ID */
  H5P_check_class_t *key =
      (H5P_check_class_t *)_key; /* Pointer to key information for comparison */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
H5P_genclass_t *H5P__create_class(H5P_genclass_t *par_class, const char *name,
  ...
  if (NULL == (pclass->name = H5MM_xstrdup(name)))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, NULL,
                "property list class name allocation failed");
  pclass->type = type;
  ...
  if (NULL == (pclass->props = H5SL_create(H5SL_TYPE_STR, NULL)))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCREATE, NULL,
                "can't create skip list for properties");
  ...
  if (par_class != NULL) {
    if (H5P__access_class(par_class, H5P_MOD_INC_CLS) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, NULL,
                  "Can't increment parent class ref count");
  } /* end if */
...
static H5P_genplist_t *H5P__create(H5P_genclass_t *pclass) {
  ...
  /* Set class state */
  plist->pclass = pclass;
  plist->nprops = 0; /* Initially the plist has the same number of properties as
                        the class */
  plist->class_init =
      FALSE; /* Initially, wait until the class callback finishes to set */
  ...
  /*
   * Check if we should copy class properties (up through list of parent classes
   * also), initialize each with default value & make property 'create'
   * callback.
   */
  tclass = pclass;
  while (tclass != NULL) {
 ...
 USAGE
    herr_t H5P__register_real(class, name, size, default, prp_create, prp_set,
        ...
        H5P_prp_create_func_t prp_create;   IN: Function pointer to property
                                    creation callback
        H5P_prp_set_func_t prp_set; IN: Function pointer to property set
callback H5P_prp_get_func_t prp_get; IN: Function pointer to property get
callback H5P_prp_encode_func_t prp_encode; IN: Function pointer to property
encode H5P_prp_decode_func_t prp_decode; IN: Function pointer to property decode
        H5P_prp_delete_func_t prp_delete; IN: Function pointer to property
delete callback H5P_prp_copy_func_t prp_copy; IN: Function pointer to property
copy callback H5P_prp_compare_func_t prp_cmp; IN: Function pointer to property
compare callback H5P_prp_close_func_t prp_close; IN: Function pointer to
property close callback RETURNS Returns non-negative on success, negative on
failure. DESCRIPTION Registers a new property with a property list class.  The
property will exist in all property list objects of that class after this
routine is finished.  The name of the property must not already exist.  The
default property value must be provided and all new property lists created with
this property will have the property value set to the default provided.  Any of
    ...
    where the parameters to the callback function are:
        ...
        void *initial_value; IN/OUT: The initial value for the property being
created. (The 'default' value passed to H5Pregister2) The 'create' routine may
modify the value to be set and those changes will be stored as the initial value
of the property.  If the 'create' routine returns a negative value, the new
property value is not copied into the property and the property list creation
routine returns an error value.
    ...
    property list.  H5P_prp_del_func_t is defined as:
        typedef herr_t (*H5P_prp_del_func_t)(hid_t prop_id, const char *name,
            size_t size, void *value);
    where the parameters to the callback function are:
        hid_t prop_id;      IN: The ID of the property list the property is
deleted from. const char *name;   IN: The name of the property being deleted.
        size_t size;        IN: The size of the property value
    ...
    The 'copy' routine may modify the value to be copied and those changes will
be stored as the value of the property.  If the 'copy' routine returns a
    negative value, the new property value is not copied into the property and
    ...
    where the parameters to the callback function are:
        ...
        size_t *size;       IN/OUT: The size of the buffer to encode the
property. void *value;        IN: The value of the property being encoded. void
*plist;        IN: The property list structure. uint8_t **buf;      OUT: The
buffer that holds the encoded property; The 'encode' routine returns the size
needed to encode the property value if the buffer passed in is NULL or the size
is zero. Otherwise it encodes the property value into binary in buf.
    ...
    where the parameters to the callback function are:
        ...
        uint8_t **buf;      IN: The buffer that holds the binary encoded
property; The 'decode' routine decodes the binary buffer passed in and
transforms it into corresponding property values that are set in the property
list passed in.
...
herr_t H5P__register_real(
  ...
  if (NULL == (new_prop = H5P__create_prop(
                   name, size, H5P_PROP_WITHIN_CLASS, def_value, prp_create,
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Internal routine to register a new property in a property list class.
 USAGE
    herr_t H5P__register(class, name, size, default, prp_create, prp_set,
prp_get, prp_close) H5P_genclass_t **class; IN: Property list class to modify
        const char *name;       IN: Name of property to register
        size_t size;            IN: Size of property in bytes
        void *def_value;        IN: Pointer to buffer containing default value
                                    for property in newly created property lists
...
-QAK
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P__register(H5P_genclass_t **ppclass, const char *name, size_t size,
                     const void *def_value, H5P_prp_create_func_t prp_create,
                     H5P_prp_set_func_t prp_set, H5P_prp_get_func_t prp_get,
  ...
  if (pclass->plists > 0 || pclass->classes > 0) {
    if (NULL ==
        (new_class = H5P__create_class(
             pclass->parent, pclass->name, pclass->type, pclass->create_func,
    ...
    if (pclass->nprops > 0) {
      ...
      while (curr_node != NULL) {
        ...
        /* Make a copy of the class's property */
        if (NULL ==
            (pcopy = H5P__dup_prop((H5P_genprop_t *)H5SL_item(curr_node),
                                   H5P_PROP_WITHIN_CLASS)))
          HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL, "Can't copy property");
  ...
  if (H5P__register_real(pclass, name, size, def_value, prp_create, prp_set,
  ...
  if (ret_value < 0)
    if (new_class && H5P__close_class(new_class) < 0)
      HDONE_ERROR(H5E_PLIST, H5E_CANTRELEASE, FAIL,
                  "unable to close new property class");
...
compare callback H5P_prp_close_func_t prp_close; IN: Function pointer to
property close callback RETURNS Returns non-negative on success, negative on
failure. DESCRIPTION Inserts a temporary property into a property list.  The
property will exist only in this property list object.  The name of the property
must not already exist.  The value must be provided unless the property is zero-
...
herr_t H5P_insert(H5P_genplist_t *plist, const char *name, size_t size,
  ...
  if (NULL != H5SL_search(plist->del, name)) {
    ...
    if (NULL == (temp_name = (char *)H5SL_remove(plist->del, name)))
      HGOTO_ERROR(H5E_PLIST, H5E_CANTDELETE, FAIL,
                  "can't remove property from deleted skip list");
  ...
  /* Create property object from parameters */
  if (NULL == (new_prop = H5P__create_prop(name, size, H5P_PROP_WITHIN_LIST,
                                           value, NULL, prp_set, prp_get,
                                           prp_encode, prp_decode, prp_delete,
                                           prp_copy, prp_cmp, prp_close)))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCREATE, FAIL, "Can't create property");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5P__do_prop(plist, name, cb, udata)
        ...
        H5P_do_plist_op_t plist_op;  IN: Pointer to the callback to invoke when
the property is found in the property list H5P_do_pclass_op_t pclass_op; IN:
Pointer to the callback to invoke when the property is found in the property
class void *udata;            IN: Pointer to the user data for the callback
 RETURNS
    Returns non-negative on success, negative on failure.
 DESCRIPTION
  ...
  if (NULL != (prop = (H5P_genprop_t *)H5SL_search(plist->props, name))) {
    ...
    if ((*plist_op)(plist, name, prop, udata) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTOPERATE, FAIL,
                  "can't operate on property");
  } /* end if */
  else {
    /*
     * Check if we should set class properties (up through list of parent
     * classes also), & make property 'set' callback.
     */
    tclass = plist->pclass;
    while (NULL != tclass) {
      if (tclass->nprops > 0) {
        /* Find the property in the class */
        if (NULL !=
            (prop = (H5P_genprop_t *)H5SL_search(tclass->props, name))) {
     ...
     */
    if (NULL == tclass)
      HGOTO_ERROR(H5E_PLIST, H5E_NOTFOUND, FAIL,
                  "can't find property in skip list");
  } /* end else */
...
/*--------------------------------------------------------------------------
 NAME
    H5P__poke_plist_cb
 PURPOSE
    Internal callback for H5P__do_prop, to overwrite a property's value in a
property list. USAGE herr_t H5P__poke_plist_cb(plist, name, value)
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__poke_plist_cb(H5P_genplist_t H5_ATTR_NDEBUG_UNUSED *plist,
                                 const char H5_ATTR_NDEBUG_UNUSED *name,
                                 H5P_genprop_t *prop, void *_udata) {
  H5P_prop_set_ud_t *udata =
      (H5P_prop_set_ud_t *)_udata; /* User data for callback */
  herr_t ret_value = SUCCEED;      /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Internal callback for H5P__do_prop, to overwrite a property's value in a
property list. USAGE herr_t H5P__poke_pclass_cb(plist, name, value)
        H5P_genplist_t *plist;  IN: Property list to overwrite property in
        const char *name;       IN: Name of property to overwrite
        H5P_genprop_t *prop;    IN: Property to overwrite
        void *udata;            IN: User data for operation
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__poke_pclass_cb(H5P_genplist_t *plist,
                                  const char H5_ATTR_NDEBUG_UNUSED *name,
                                  H5P_genprop_t *prop, void *_udata) {
  H5P_prop_set_ud_t *udata =
      (H5P_prop_set_ud_t *)_udata; /* User data for callback */
...
herr_t H5P_poke(H5P_genplist_t *plist, const char *name, const void *value) {
  ...
  udata.value = value;
  if (H5P__do_prop(plist, name, H5P__poke_plist_cb, H5P__poke_pclass_cb,
                   &udata) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTOPERATE, FAIL,
                "can't operate on plist to overwrite value");
...
/*--------------------------------------------------------------------------
 NAME
    H5P__set_plist_cb
 PURPOSE
    Internal callback for H5P__do_prop, to set a property's value in a property
list. USAGE herr_t H5P__set_plist_cb(plist, name, value) H5P_genplist_t *plist;
IN: Property list to set property in const char *name;       IN: Name of
property to set H5P_genprop_t *prop;    IN: Property to set void *udata; IN:
User data for operation RETURNS Returns non-negative on success, negative on
failure. DESCRIPTION Sets a new value for a property in a property list. GLOBAL
VARIABLES COMMENTS, BUGS, ASSUMPTIONS Called when the property is found in the
property list. EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__set_plist_cb(H5P_genplist_t *plist, const char *name,
                                H5P_genprop_t *prop, void *_udata) {
  H5P_prop_set_ud_t *udata =
      (H5P_prop_set_ud_t *)_udata; /* User data for callback */
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Internal callback for H5P__do_prop, to set a property's value in a property
list. USAGE herr_t H5P__set_pclass_cb(plist, name, value) H5P_genplist_t *plist;
IN: Property list to set property in const char *name;       IN: Name of
property to set H5P_genprop_t *prop;    IN: Property to set void *udata; IN:
User data for operation RETURNS Returns non-negative on success, negative on
failure. DESCRIPTION Sets a new value for a property in a property list. GLOBAL
VARIABLES COMMENTS, BUGS, ASSUMPTIONS Called when the property is found in the
property class. EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__set_pclass_cb(H5P_genplist_t *plist, const char *name,
                                 H5P_genprop_t *prop, void *_udata) {
  H5P_prop_set_ud_t *udata =
      (H5P_prop_set_ud_t *)_udata; /* User data for callback */
  H5P_genprop_t *pcopy = NULL;  /* Copy of property to insert into skip list */
...
herr_t H5P_set(H5P_genplist_t *plist, const char *name, const void *value) {
  ...
  /* Find the property and set the value */
  udata.value = value;
  if (H5P__do_prop(plist, name, H5P__set_plist_cb, H5P__set_pclass_cb, &udata) <
      0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTOPERATE, FAIL,
                "can't operate on plist to set value");
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P__class_get(const H5P_genclass_t *pclass, const char *name,
                      void *value) {
  H5P_genprop_t *prop;        /* Temporary property pointer */
  herr_t ret_value = SUCCEED; /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P__class_set(const H5P_genclass_t *pclass, const char *name,
                      const void *value) {
  H5P_genprop_t *prop;        /* Temporary property pointer */
  herr_t ret_value = SUCCEED; /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P__get_size_plist(const H5P_genplist_t *plist, const char *name,
                           size_t *size) {
  H5P_genprop_t *prop;        /* Temporary property pointer */
  herr_t ret_value = SUCCEED; /* return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P__get_size_pclass(H5P_genclass_t *pclass, const char *name,
                            size_t *size) {
  H5P_genprop_t *prop;        /* Temporary property pointer */
  herr_t ret_value = SUCCEED; /* return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P_get_nprops_pclass(const H5P_genclass_t *pclass, size_t *nprops,
                             hbool_t recurse) {
  herr_t ret_value = SUCCEED; /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
int H5P__cmp_class(const H5P_genclass_t *pclass1,
...
static int H5P__cmp_plist_cb(H5P_genprop_t *prop, void *_udata) {
  H5P_plist_cmp_ud_t *udata =
      (H5P_plist_cmp_ud_t *)_udata; /* Pointer to user data */
  htri_t
      prop2_exist; /* Whether the property exists in the second property list */
  ...
  if ((prop2_exist = H5P_exist_plist(udata->plist2, prop->name)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_NOTFOUND, H5_ITER_ERROR,
                "can't lookup existence of property?");
  if (prop2_exist) {
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P__cmp_plist(const H5P_genplist_t *plist1,
                      const H5P_genplist_t *plist2, int *cmp_ret) {
  ...
  if ((ret_value = H5P__iterate_plist(plist1, TRUE, &idx, H5P__cmp_plist_cb,
                                      &udata)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
htri_t H5P_class_isa(const H5P_genclass_t *pclass1,
                     const H5P_genclass_t *pclass2) {
  htri_t ret_value = FAIL; /* Return value */
...
htri_t H5P_isa_class(hid_t plist_id, hid_t pclass_id) {
  ...
  if (NULL == (pclass = (H5P_genclass_t *)H5I_object_verify(pclass_id,
                                                            H5I_GENPROP_CLS)))
  ...
  if ((ret_value = H5P_class_isa(plist->pclass, pclass)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL,
                "unable to compare property list classes");
...
static int H5P__iterate_plist_cb(void *_item, void *_key, void *_udata) {
  ...
  H5P_iter_plist_ud_t *udata =
      (H5P_iter_plist_ud_t *)_udata; /* Pointer to user data */
  int ret_value = H5_ITER_CONT;      /* Return value */
  ...
  if (H5SL_insert(udata->seen, key, key) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, H5_ITER_ERROR,
                "can't insert property into 'seen' skip list");
 ...
 USAGE
    int H5P__iterate_plist(plist, iter_all_prop, idx, cb_func, iter_data)
        ...
        hbool_t iter_all_prop;      IN: Whether to iterate over all properties
                                        (TRUE), or just non-default (i.e.
changed) properties (FALSE). int *idx;                   IN/OUT: Index of the
property to begin with H5P_iterate_t cb_func;    IN: Function pointer to
function to be called with each property iterated over. void *iter_data; IN/OUT:
Pointer to iteration data from user RETURNS Success: Returns the return value of
the last call to ITER_FUNC if it was non-zero, or zero if all properties have
been processed. Failure: negative value DESCRIPTION This routine iterates over
the properties in the property object specified with PLIST_ID.  For each
property in the object, the ITER_DATA and some additional information, specified
below, are passed to the ITER_FUNC function. The iteration begins with the IDX
property in the object and the next element to be processed by the operator is
returned in IDX.  If IDX is NULL, then the iterator starts at the first
property; since no stopping point is returned in this case, the iterator cannot
be restarted if one of the calls to its operator returns non-zero.

The prototype for H5P_iterate_t is:
    typedef herr_t (*H5P_iterate_t)(hid_t id, const char *name, void
*iter_data); The operation receives the property list or class identifier for
the object being iterated over, ID, the name of the current property within the
object, NAME, and the pointer to the operator data passed in to H5Piterate,
ITER_DATA.
...
iteration, the function's behavior is undefined.
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
int H5P__iterate_plist(const H5P_genplist_t *plist, hbool_t iter_all_prop,
                       int *idx, H5P_iterate_int_t cb_func, void *udata) {
  H5P_genclass_t *tclass;        /* Temporary class pointer */
...
property list class.
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static int H5P__iterate_pclass_cb(void *_item, void H5_ATTR_NDEBUG_UNUSED *_key,
                                  void *_udata) {
  H5P_genprop_t *item = (H5P_genprop_t *)_item; /* Pointer to the property */
  H5P_iter_pclass_ud_t *udata =
      (H5P_iter_pclass_ud_t *)_udata; /* Pointer to user data */
  int ret_value = 0;                  /* Return value */
...
iteration, the function's behavior is undefined.
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
int H5P__iterate_pclass(const H5P_genclass_t *pclass, int *idx,
...
/*--------------------------------------------------------------------------
 NAME
    H5P__peek_cb
 PURPOSE
    Internal callback for H5P__do_prop, to peek at a property's value in a
property list. USAGE herr_t H5P__peek_plist_cb(plist, name, value)
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__peek_cb(H5P_genplist_t H5_ATTR_NDEBUG_UNUSED *plist,
                           const char H5_ATTR_NDEBUG_UNUSED *name,
                           H5P_genprop_t *prop, void *_udata) {
  H5P_prop_get_ud_t *udata =
      (H5P_prop_get_ud_t *)_udata; /* User data for callback */
  herr_t ret_value = SUCCEED;      /* Return value */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5P_peek(plist, name, value)
        H5P_genplist_t *plist;  IN: Property list to check
        const char *name;       IN: Name of property to query
        void *value;            OUT: Pointer to the buffer for the property
value RETURNS Returns non-negative on success, negative on failure. DESCRIPTION
...
herr_t H5P_peek(H5P_genplist_t *plist, const char *name, void *value) {
  ...
  if (H5P__do_prop(plist, name, H5P__peek_cb, H5P__peek_cb, &udata) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTOPERATE, FAIL,
                "can't operate on plist to peek at value");
...
/*--------------------------------------------------------------------------
 NAME
    H5P__get_cb
 PURPOSE
    Internal callback for H5P__do_prop, to get a property's value in a property
list. USAGE herr_t H5P__get_plist_cb(plist, name, value) H5P_genplist_t *plist;
IN: Property list to get property in const char *name;       IN: Name of
property to get H5P_genprop_t *prop;    IN: Property to get void *udata; IN:
User data for operation RETURNS Returns non-negative on success, negative on
failure. DESCRIPTION Gets a new value for a property in a property list. GLOBAL
VARIABLES COMMENTS, BUGS, ASSUMPTIONS Called when the property is found in the
property list and when it's found for the property class. EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__get_cb(H5P_genplist_t *plist, const char *name,
                          H5P_genprop_t *prop, void *_udata) {
  H5P_prop_get_ud_t *udata =
      (H5P_prop_get_ud_t *)_udata; /* User data for callback */
  void *tmp_value = NULL;          /* Temporary value for property */
...
herr_t H5P_get(H5P_genplist_t *plist, const char *name, void *value) {
  ...
  if (H5P__do_prop(plist, name, H5P__get_cb, H5P__get_cb, &udata) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTOPERATE, FAIL,
                "can't operate on plist to get value");
...
/*--------------------------------------------------------------------------
 NAME
    H5P__del_plist_cb
 PURPOSE
    Internal callback for H5P__do_prop, to remove a property's value in a
property list. USAGE herr_t H5P__del_plist_cb(plist, name, value) H5P_genplist_t
*plist;  IN: Property list to remove property from const char *name;       IN:
Name of property to remove H5P_genprop_t *prop;    IN: Property to remove void
*udata;            IN: User data for operation RETURNS Returns non-negative on
success, negative on failure. DESCRIPTION Remove a property in a property list.
Called when the property is found in the property list. GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__del_plist_cb(H5P_genplist_t *plist, const char *name,
                                H5P_genprop_t *prop,
  ...
  if (NULL == H5SL_remove(plist->props, prop->name))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTDELETE, FAIL,
                "can't remove property from skip list");
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Internal callback for H5P__do_prop, to remove a property's value in a
property list. USAGE herr_t H5P__del_pclass_cb(plist, name, value)
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5P__del_pclass_cb(H5P_genplist_t *plist, const char *name,
                                 H5P_genprop_t *prop,
                                 void H5_ATTR_UNUSED *_udata) {
  char *del_name = NULL;      /* Pointer to deleted name */
  void *tmp_value = NULL;     /* Temporary value for property */
...
herr_t H5P_remove(H5P_genplist_t *plist, const char *name) {
  ...
  /* Find the property and get the value */
  if (H5P__do_prop(plist, name, H5P__del_plist_cb, H5P__del_pclass_cb, NULL) <
      0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTOPERATE, FAIL,
                "can't operate on plist to remove value");
...
herr_t H5P__copy_prop_plist(hid_t dst_id, hid_t src_id, const char *name) {
  ...
  if (NULL != H5P__find_prop_plist(dst_plist, name)) {
    /* Delete the property from the destination list, calling the 'close'
  ...
  /* If not, get the information required to do an H5Pinsert2 with the property
    ...
    if (NULL ==
        (new_prop = H5P__create_prop(
             prop->name, prop->size, H5P_PROP_WITHIN_LIST, prop->value,
    ...
    /* Call property creation callback, if it exists */
    if (new_prop->create) {
      if ((new_prop->create)(new_prop->name, new_prop->size, new_prop->value) <
          0)
...
herr_t H5P__copy_prop_pclass(hid_t dst_id, hid_t src_id, const char *name) {
  H5P_genclass_t
      *src_pclass; /* Source property class, containing property to copy */
  ...
  if (NULL == (src_pclass = (H5P_genclass_t *)H5I_object(src_id)))
    HGOTO_ERROR(H5E_PLIST, H5E_NOTFOUND, FAIL,
                "source property class object doesn't exist");
  if (NULL == (dst_pclass = (H5P_genclass_t *)H5I_object(dst_id)))
    HGOTO_ERROR(H5E_PLIST, H5E_NOTFOUND, FAIL,
                "destination property class object doesn't exist");
  ...
  if (H5P__register(&dst_pclass, name, prop->size, prop->value, prop->create,
  ...
  if (dst_pclass != orig_dst_pclass) {
    ...
    if (NULL ==
      ...
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "unable to substitute property class in ID");
...
/*--------------------------------------------------------------------------
 ...
 COMMENTS, BUGS, ASSUMPTIONS
    ...
    here, it must have been checked for and called properly prior to this
routine being called. EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P_close(H5P_genplist_t *plist) {
  ...
  /*
   * Check if we should remove class properties (up through list of parent
   * classes also), initialize each with default value & make property 'remove'
   * callback.
   */
  tclass = plist->pclass;
  has_parent_class = (hbool_t)(tclass != NULL && tclass->parent != NULL &&
  ...
  if (H5P__access_class(plist->pclass, H5P_MOD_DEC_LST) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
                "Can't decrement class ref count");
...
H5P_genclass_t *H5P__open_class_path(const char *path) {
  ...
  H5P_check_class_t check_info; /* Structure to hold the information for
  ...
  while (NULL != (delimit = HDstrchr(curr_name, '/'))) {
    ...
    *delimit = '\0';
     ...
     */
    if (H5I_iterate(H5I_GENPROP_CLS, H5P__open_class_path_cb, &check_info,
                    FALSE) < 0)
...
/*--------------------------------------------------------------------------
 ...
 RETURNS
    Returns non-negative on success, negative on failure.
 DESCRIPTION
    Releases memory and de-attach a class from the property list class
hierarchy. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
...
hid_t H5P__new_plist_of_type(H5P_plist_type_t type) {
  ...
  default:
    HGOTO_ERROR(H5E_PLIST, H5E_BADRANGE, FAIL,
                "invalid property list type: %u\n", (unsigned)type);
  } /* end switch */

  /* Get the class object */
  if (NULL == (pclass = (H5P_genclass_t *)H5I_object(class_id)))
    HGOTO_ERROR(H5E_PLIST, H5E_BADTYPE, H5I_INVALID_HID,
 ...
 *-------------------------------------------------------------------------
 */
int H5P_ignore_cmp(const void H5_ATTR_UNUSED *val1,
