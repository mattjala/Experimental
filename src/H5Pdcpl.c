...
#define H5D_DEF_LAYOUT_CHUNK_INIT                                              \
  {                                                                            \
    H5D_CHUNK_IDX_BTREE, (uint8_t)0, (unsigned)0,                              \
...
#define H5D_DEF_LAYOUT_COMPACT                                                 \
  {                                                                            \
    H5D_COMPACT, H5O_LAYOUT_VERSION_DEFAULT, H5D_LOPS_COMPACT,                 \
        {H5D_DEF_LAYOUT_CHUNK_INIT}, H5D_DEF_STORAGE_COMPACT                   \
  }
#define H5D_DEF_LAYOUT_CONTIG                                                  \
  {                                                                            \
    H5D_CONTIGUOUS, H5O_LAYOUT_VERSION_DEFAULT, H5D_LOPS_CONTIG,               \
        {H5D_DEF_LAYOUT_CHUNK_INIT}, H5D_DEF_STORAGE_CONTIG                    \
  }
#define H5D_DEF_LAYOUT_CHUNK                                                   \
  {                                                                            \
    H5D_CHUNKED, H5O_LAYOUT_VERSION_DEFAULT, H5D_LOPS_CHUNK,                   \
        {H5D_DEF_LAYOUT_CHUNK_INIT}, H5D_DEF_STORAGE_CHUNK                     \
  }
#define H5D_DEF_LAYOUT_VIRTUAL                                                 \
  {                                                                            \
    H5D_VIRTUAL, H5O_LAYOUT_VERSION_4, H5D_LOPS_VIRTUAL,                       \
        {H5D_DEF_LAYOUT_CHUNK_INIT}, H5D_DEF_STORAGE_VIRTUAL                   \
  }
...
#define H5D_CRT_FILL_VALUE_DEF                                                 \
  {                                                                            \
    {0, NULL, H5O_NULL_ID, {{0, HADDR_UNDEF}}}, H5O_FILL_VERSION_2, NULL, 0,   \
        NULL, H5D_ALLOC_TIME_LATE, H5D_FILL_TIME_IFSET, FALSE                  \
  }
...
/* General routines */
static herr_t H5P__set_layout(H5P_genplist_t *plist,
...
/* Property callbacks */
static herr_t H5P__dcrt_layout_set(hid_t prop_id, const char *name, size_t size,
                                   void *value);
static herr_t H5P__dcrt_layout_get(hid_t prop_id, const char *name, size_t size,
                                   void *value);
...
static herr_t H5P__dcrt_layout_del(hid_t prop_id, const char *name, size_t size,
                                   void *value);
static herr_t H5P__dcrt_layout_copy(const char *name, size_t size, void *value);
static int H5P__dcrt_layout_cmp(const void *value1, const void *value2,
                                size_t size);
static herr_t H5P__dcrt_layout_close(const char *name, size_t size,
                                     void *value);
static herr_t H5P__dcrt_fill_value_set(hid_t prop_id, const char *name,
                                       size_t size, void *value);
static herr_t H5P__dcrt_fill_value_get(hid_t prop_id, const char *name,
                                       size_t size, void *value);
static herr_t H5P__dcrt_fill_value_enc(const void *value, void **pp,
                                       size_t *size);
static herr_t H5P__dcrt_fill_value_dec(const void **pp, void *value);
static herr_t H5P__dcrt_fill_value_del(hid_t prop_id, const char *name,
                                       size_t size, void *value);
static herr_t H5P__dcrt_fill_value_copy(const char *name, size_t size,
                                        void *value);
static herr_t H5P__dcrt_fill_value_close(const char *name, size_t size,
                                         void *value);
static herr_t H5P__dcrt_ext_file_list_set(hid_t prop_id, const char *name,
                                          size_t size, void *value);
static herr_t H5P__dcrt_ext_file_list_get(hid_t prop_id, const char *name,
                                          size_t size, void *value);
static herr_t H5P__dcrt_ext_file_list_enc(const void *value, void **pp,
                                          size_t *size);
static herr_t H5P__dcrt_ext_file_list_dec(const void **pp, void *value);
static herr_t H5P__dcrt_ext_file_list_del(hid_t prop_id, const char *name,
                                          size_t size, void *value);
static herr_t H5P__dcrt_ext_file_list_copy(const char *name, size_t size,
                                           void *value);
static int H5P__dcrt_ext_file_list_cmp(const void *value1, const void *value2,
                                       size_t size);
static herr_t H5P__dcrt_ext_file_list_close(const char *name, size_t size,
                                            void *value);
...
static const H5O_layout_t H5D_def_layout_g =
    H5D_CRT_LAYOUT_DEF; /* Default storage layout */
static const H5O_fill_t H5D_def_fill_g =
    H5D_CRT_FILL_VALUE_DEF; /* Default fill value */
static const unsigned H5D_def_alloc_time_state_g =
    H5D_CRT_ALLOC_TIME_STATE_DEF; /* Default allocation time state */
static const H5O_efl_t H5D_def_efl_g =
    H5D_CRT_EXT_FILE_LIST_DEF; /* Default external file list */
static const unsigned H5O_ohdr_min_g =
    H5D_CRT_MIN_DSET_HDR_SIZE_DEF; /* Default object header minimization */
...
static herr_t H5P__dcrt_reg_prop(H5P_genclass_t *pclass) {
  ...
  if (H5P__register_real(
          pclass, H5D_CRT_LAYOUT_NAME, H5D_CRT_LAYOUT_SIZE, &H5D_def_layout_g,
          NULL, H5D_CRT_LAYOUT_SET, H5D_CRT_LAYOUT_GET, H5D_CRT_LAYOUT_ENC,
          H5D_CRT_LAYOUT_DEC, H5D_CRT_LAYOUT_DEL, H5D_CRT_LAYOUT_COPY,
          H5D_CRT_LAYOUT_CMP, H5D_CRT_LAYOUT_CLOSE) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the fill value property */
  if (H5P__register_real(pclass, H5D_CRT_FILL_VALUE_NAME,
                         H5D_CRT_FILL_VALUE_SIZE, &H5D_def_fill_g, NULL,
                         H5D_CRT_FILL_VALUE_SET, H5D_CRT_FILL_VALUE_GET,
                         H5D_CRT_FILL_VALUE_ENC, H5D_CRT_FILL_VALUE_DEC,
  ...
  if (H5P__register_real(
          pclass, H5D_CRT_ALLOC_TIME_STATE_NAME, H5D_CRT_ALLOC_TIME_STATE_SIZE,
          &H5D_def_alloc_time_state_g, NULL, NULL, NULL,
          H5D_CRT_ALLOC_TIME_STATE_ENC, H5D_CRT_ALLOC_TIME_STATE_DEC, NULL,
          NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register the external file list property */
  if (H5P__register_real(pclass, H5D_CRT_EXT_FILE_LIST_NAME,
                         H5D_CRT_EXT_FILE_LIST_SIZE, &H5D_def_efl_g, NULL,
                         H5D_CRT_EXT_FILE_LIST_SET, H5D_CRT_EXT_FILE_LIST_GET,
                         H5D_CRT_EXT_FILE_LIST_ENC, H5D_CRT_EXT_FILE_LIST_DEC,
                         H5D_CRT_EXT_FILE_LIST_DEL, H5D_CRT_EXT_FILE_LIST_COPY,
  ...
  if (H5P__register_real(pclass, H5D_CRT_MIN_DSET_HDR_SIZE_NAME,
                         H5D_CRT_MIN_DSET_HDR_SIZE_SIZE, &H5O_ohdr_min_g, NULL,
                         NULL, NULL, H5D_CRT_MIN_DSET_HDR_SIZE_ENC,
                         H5D_CRT_MIN_DSET_HDR_SIZE_DEC, NULL, NULL, NULL,
                         NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_layout_set(hid_t H5_ATTR_UNUSED prop_id,
                                   const char H5_ATTR_UNUSED *name,
                                   size_t H5_ATTR_UNUSED size, void *value) {
  H5O_layout_t *layout =
      (H5O_layout_t *)value; /* Create local aliases for values */
 ...
 *
 * Purpose:     Copies a layout property when it's retrieved from a property
 *list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_layout_get(hid_t H5_ATTR_UNUSED prop_id,
                                   const char H5_ATTR_UNUSED *name,
                                   size_t H5_ATTR_UNUSED size, void *value) {
  H5O_layout_t *layout =
      (H5O_layout_t *)value; /* Create local aliases for values */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_layout_enc(const void *value, void **_pp,
                                   size_t *size) {
  const H5O_layout_t *layout =
      (const H5O_layout_t *)value; /* Create local aliases for values */
    ...
    if (H5D_CHUNKED == layout->type) {
      ...
      *size += (size_t)8;
      ...
      for (u = 0; u < layout->storage.u.virt.list_nused; u++) {
        /* Source file name */
        tmp_size = HDstrlen(layout->storage.u.virt.list[u].source_file_name) +
                   (size_t)1;
        H5MM_memcpy(*pp, layout->storage.u.virt.list[u].source_file_name,
                    tmp_size);
        *pp += tmp_size;
        *size += tmp_size;

        /* Source dataset name */
        tmp_size = HDstrlen(layout->storage.u.virt.list[u].source_dset_name) +
                   (size_t)1;
        H5MM_memcpy(*pp, layout->storage.u.virt.list[u].source_dset_name,
                    tmp_size);
        *pp += tmp_size;
        *size += tmp_size;
        ...
        tmp_size = (size_t)-1;
        tmp_p = *pp;
        if (H5S_encode(layout->storage.u.virt.list[u].source_select, pp,
                       &tmp_size) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTENCODE, FAIL,
                      "unable to serialize source selection");
        *size += (size_t)(*pp - tmp_p);
        ...
        tmp_p = *pp;
        if (H5S_encode(
                layout->storage.u.virt.list[u].source_dset.virtual_select, pp,
                &tmp_size) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTENCODE, FAIL,
                      "unable to serialize virtual selection");
        *size += (size_t)(*pp - tmp_p);
      } /* end for */
    }   /* end if */
  }     /* end if */
      ...
      *size += (size_t)8;
      ...
      for (u = 0; u < layout->storage.u.virt.list_nused; u++) {
        ...
        *size += tmp_size;
        ...
        tmp_size = (size_t)0;
        tmp_p = NULL;
        if (H5S_encode(layout->storage.u.virt.list[u].source_select, &tmp_p,
                       &tmp_size) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTENCODE, FAIL,
                      "unable to serialize source selection");
        *size += tmp_size;
        ...
        tmp_p = NULL;
        if (H5S_encode(
                layout->storage.u.virt.list[u].source_dset.virtual_select,
                &tmp_p, &tmp_size) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTENCODE, FAIL,
                      "unable to serialize virtual selection");
        *size += tmp_size;
...
static herr_t H5P__dcrt_layout_dec(const void **_pp, void *value) {
  ...
  case H5D_VIRTUAL: {
    ...
    if (nentries == (uint64_t)0)
      ...
      /* Allocate entry list */
      if (NULL ==
          (tmp_layout.storage.u.virt.list =
               (H5O_storage_virtual_ent_t *)H5MM_calloc(
                   (size_t)nentries * sizeof(H5O_storage_virtual_ent_t))))
        HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, FAIL,
                    "unable to allocate heap block");
      ...
      for (u = 0; u < (size_t)nentries; u++) {
        ...
        H5MM_memcpy(tmp_layout.storage.u.virt.list[u].source_file_name, *pp,
                    tmp_size);
        *pp += tmp_size;
        ...
        H5MM_memcpy(tmp_layout.storage.u.virt.list[u].source_dset_name, *pp,
                    tmp_size);
        *pp += tmp_size;

        /* Source selection */
        if (NULL ==
            (tmp_layout.storage.u.virt.list[u].source_select = H5S_decode(pp)))
          HGOTO_ERROR(H5E_PLIST, H5E_CANTDECODE, FAIL,
                      "can't decode source space selection");
        tmp_layout.storage.u.virt.list[u].source_space_status =
        ...
        tmp_layout.storage.u.virt.list[u].virtual_space_status =
            H5O_VIRTUAL_STATUS_USER;
        ...
        if (H5D_virtual_parse_source_name(
          ...
          HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
                      "can't parse source dataset name");
        ...
        if ((tmp_layout.storage.u.virt.list[u].psfn_nsubs == 0) &&
          ...
          if (tmp_layout.storage.u.virt.list[u].parsed_source_file_name)
            tmp_layout.storage.u.virt.list[u].source_dset.file_name =
                tmp_layout.storage.u.virt.list[u]
                    .parsed_source_file_name->name_segment;
          else
            tmp_layout.storage.u.virt.list[u].source_dset.file_name =
                tmp_layout.storage.u.virt.list[u].source_file_name;
          if (tmp_layout.storage.u.virt.list[u].parsed_source_dset_name)
            tmp_layout.storage.u.virt.list[u].source_dset.dset_name =
                tmp_layout.storage.u.virt.list[u]
                    .parsed_source_dset_name->name_segment;
          else
            tmp_layout.storage.u.virt.list[u].source_dset.dset_name =
                tmp_layout.storage.u.virt.list[u].source_dset_name;
        } /* end if */
        ...
        tmp_layout.storage.u.virt.list[u].unlim_dim_virtual =
            H5S_get_select_unlim_dim(
                tmp_layout.storage.u.virt.list[u].source_dset.virtual_select);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_layout_del(hid_t H5_ATTR_UNUSED prop_id,
                                   const char H5_ATTR_UNUSED *name,
                                   size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_layout_copy(const char H5_ATTR_UNUSED *name,
                                    size_t H5_ATTR_UNUSED size, void *value) {
  H5O_layout_t *layout =
      (H5O_layout_t *)value; /* Create local aliases for values */
  H5O_layout_t new_layout;
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__dcrt_layout_cmp(const void *_layout1, const void *_layout2,
                                size_t H5_ATTR_UNUSED size) {
  const H5O_layout_t *layout1 = (const H5O_layout_t *)
                         _layout1, /* Create local aliases for values */
  ...
  case H5D_VIRTUAL: {
    ...
    for (u = 0; u < layout1->storage.u.virt.list_nused; u++) {
       ...
       */
      if ((equal = H5S_extent_equal(
               layout1->storage.u.virt.list[u].source_select,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_layout_close(const char H5_ATTR_UNUSED *name,
                                     size_t H5_ATTR_UNUSED size, void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_fill_value_set(hid_t H5_ATTR_UNUSED prop_id,
                                       const char H5_ATTR_UNUSED *name,
                                       size_t H5_ATTR_UNUSED size,
                                       void *value) {
 ...
 *
 * Purpose:     Copies a fill value property when it's retrieved from a property
 *list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_fill_value_get(hid_t H5_ATTR_UNUSED prop_id,
                                       const char H5_ATTR_UNUSED *name,
                                       size_t H5_ATTR_UNUSED size,
                                       void *value) {
  H5O_fill_t *fill = (H5O_fill_t *)value; /* Create local aliases for values */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_fill_value_enc(const void *value, void **_pp,
                                       size_t *size) {
  const H5O_fill_t *fill =
      (const H5O_fill_t *)value; /* Create local aliases for values */
  ...
  *fill = H5D_def_fill_g;
  ...
  if (fill->size > 0) {
    ...
    if (NULL == (fill->buf = H5MM_malloc((size_t)fill->size)))
      HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, FAIL,
                  "memory allocation failed for fill value buffer");
    ...
    *pp += fill->size;
    ...
    if (NULL == (fill->type = H5T_decode(dt_size, *pp)))
      HGOTO_ERROR(H5E_PLIST, H5E_CANTDECODE, FAIL,
                  "can't decode fill value datatype");
    *pp += dt_size;
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_fill_value_del(hid_t H5_ATTR_UNUSED prop_id,
                                       const char H5_ATTR_UNUSED *name,
                                       size_t H5_ATTR_UNUSED size,
                                       void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5O_msg_reset(H5O_FILL_ID, value) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTRESET, FAIL,
                "can't release fill value message");
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_fill_value_copy(const char H5_ATTR_UNUSED *name,
                                        size_t H5_ATTR_UNUSED size,
                                        void *value) {
  H5O_fill_t *fill = (H5O_fill_t *)value; /* Create local aliases for values */
  H5O_fill_t new_fill;
 ...
 *-------------------------------------------------------------------------
 */
int H5P_fill_value_cmp(const void *_fill1, const void *_fill2,
                       size_t H5_ATTR_UNUSED size) {
  const H5O_fill_t *fill1 = (const H5O_fill_t *)
                       _fill1, /* Create local aliases for values */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_fill_value_close(const char H5_ATTR_UNUSED *name,
                                         size_t H5_ATTR_UNUSED size,
                                         void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Purpose:     Copies an external file list property when it's set for a
 *property list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_ext_file_list_set(hid_t H5_ATTR_UNUSED prop_id,
                                          const char H5_ATTR_UNUSED *name,
                                          size_t H5_ATTR_UNUSED size,
                                          void *value) {
 ...
 *
 * Purpose:     Copies an external file list property when it's retrieved from a
 *property list
 *
 * Return:      Success:        Non-negative
 *              Failure:        Negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_ext_file_list_get(hid_t H5_ATTR_UNUSED prop_id,
                                          const char H5_ATTR_UNUSED *name,
                                          size_t H5_ATTR_UNUSED size,
                                          void *value) {
  H5O_efl_t *efl = (H5O_efl_t *)value; /* Create local aliases for values */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_ext_file_list_enc(const void *value, void **_pp,
                                          size_t *size) {
  const H5O_efl_t *efl =
      (const H5O_efl_t *)value; /* Create local aliases for values */
  ...
  *efl = H5D_def_efl_g;
  ...
  for (u = 0; u < nused; u++) {
    ...
    if (efl->nused >= efl->nalloc) {
      ...
      H5O_efl_entry_t *x = (H5O_efl_entry_t *)H5MM_realloc(
          efl->slot, na * sizeof(H5O_efl_entry_t));
      if (!x)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_ext_file_list_del(hid_t H5_ATTR_UNUSED prop_id,
                                          const char H5_ATTR_UNUSED *name,
                                          size_t H5_ATTR_UNUSED size,
                                          void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5O_msg_reset(H5O_EFL_ID, value) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTRESET, FAIL,
                "can't release external file list message");
 ...
 *--------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_ext_file_list_copy(const char H5_ATTR_UNUSED *name,
                                           size_t H5_ATTR_UNUSED size,
                                           void *value) {
  H5O_efl_t *efl = (H5O_efl_t *)value; /* Create local aliases for values */
  H5O_efl_t new_efl;
 ...
 *-------------------------------------------------------------------------
 */
static int H5P__dcrt_ext_file_list_cmp(const void *_efl1, const void *_efl2,
                                       size_t H5_ATTR_UNUSED size) {
  const H5O_efl_t *efl1 = (const H5O_efl_t *)
                      _efl1, /* Create local aliases for values */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5P__dcrt_ext_file_list_close(const char H5_ATTR_UNUSED *name,
                                            size_t H5_ATTR_UNUSED size,
                                            void *value) {
  herr_t ret_value = SUCCEED; /* Return value */
...
static herr_t H5P__set_layout(H5P_genplist_t *plist,
  ...
  if (H5P_get(plist, H5D_CRT_ALLOC_TIME_STATE_NAME, &alloc_time_state) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get space allocation time state");

  /* If we still have the "default" allocation time, change it according to the
   * new layout */
  if (alloc_time_state) {
    ...
    if (H5P_poke(plist, H5D_CRT_FILL_VALUE_NAME, &fill) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't set space allocation time");
  } /* end if */
...
herr_t H5Pset_layout(hid_t plist_id, H5D_layout_t layout_type) {
  ...
  const H5O_layout_t
      *layout; /* Pointer to default layout information for type specified */
  herr_t ret_value = SUCCEED; /* Return value */
...
H5D_layout_t H5Pget_layout(hid_t plist_id) {
  ...
  /* Get the plist structure */
  if (NULL == (plist = H5P_object_verify(plist_id, H5P_DATASET_CREATE)))
    HGOTO_ERROR(H5E_ID, H5E_BADID, H5D_LAYOUT_ERROR,
...
herr_t H5Pset_chunk(hid_t plist_id, int ndims, const hsize_t dim[/*ndims*/]) {
  ...
  H5MM_memcpy(&chunk_layout, &H5D_def_layout_chunk_g,
  ...
  for (u = 0; u < (unsigned)ndims; u++) {
    ...
    if (dim[u] != (dim[u] & 0xffffffff))
      HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL,
                  "all chunk dimensions must be less than 2^32");
    chunk_nelmts *= dim[u];
    if (chunk_nelmts > (uint64_t)0xffffffff)
      HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL,
                  "number of elements in chunk must be < 4GB");
    chunk_layout.u.chunk.dim[u] =
        (uint32_t)dim[u]; /* Store user's chunk dimensions */
  }                       /* end for */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_virtual(hid_t dcpl_id, hid_t vspace_id, const char *src_file_name,
  ...
  H5O_storage_virtual_ent_t *old_list =
      NULL; /* List pointer previously on property list */
  H5O_storage_virtual_ent_t *ent =
      NULL;                         /* Convenience pointer to new VDS entry */
  ...
  if (!src_dset_name)
    HGOTO_ERROR(H5E_PLIST, H5E_BADRANGE, FAIL,
                "source dataset name not provided");
  if (NULL == (vspace = (H5S_t *)H5I_object_verify(vspace_id, H5I_DATASPACE)))
    HGOTO_ERROR(H5E_PLIST, H5E_BADTYPE, FAIL, "not a dataspace");
  if (NULL ==
      (src_space = (H5S_t *)H5I_object_verify(src_space_id, H5I_DATASPACE)))
    HGOTO_ERROR(H5E_PLIST, H5E_BADTYPE, FAIL, "not a dataspace");

  /* Check selections for validity */
  if (H5D_virtual_check_mapping_pre(vspace, src_space,
                                    H5O_VIRTUAL_STATUS_USER) < 0)
  ...
  if (virtual_layout.storage.u.virt.list_nused ==
      virtual_layout.storage.u.virt.list_nalloc) {
    H5O_storage_virtual_ent_t *x; /* Pointer to the new list */
    size_t new_alloc = MAX(H5D_VIRTUAL_DEF_LIST_SIZE,
                           virtual_layout.storage.u.virt.list_nalloc * 2);

    /* Expand size of entry list */
    if (NULL == (x = (H5O_storage_virtual_ent_t *)H5MM_realloc(
                     virtual_layout.storage.u.virt.list,
                     new_alloc * sizeof(H5O_storage_virtual_ent_t))))
      HGOTO_ERROR(H5E_PLIST, H5E_RESOURCE, FAIL,
                  "can't reallocate virtual dataset mapping list");
    virtual_layout.storage.u.virt.list = x;
    virtual_layout.storage.u.virt.list_nalloc = new_alloc;
  } /* end if */

  /* Add virtual dataset mapping entry */
  ent = &virtual_layout.storage.u.virt
             .list[virtual_layout.storage.u.virt.list_nused];
  memset(
      ent, 0,
      sizeof(H5O_storage_virtual_ent_t)); /* Clear before starting to set up */
  ...
  if (NULL == (ent->source_file_name = H5MM_xstrdup(src_file_name)))
    HGOTO_ERROR(H5E_PLIST, H5E_RESOURCE, FAIL,
                "can't duplicate source file name");
  ...
  /* Update min_dims */
  if (H5D_virtual_update_min_dims(&virtual_layout,
                                  virtual_layout.storage.u.virt.list_nused) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
                "unable to update virtual dataset minimum dimensions");
  ...
  if (ret_value < 0) {
    ...
    if (ent) {
      ...
      if (ent->source_dset.virtual_select &&
          H5S_close(ent->source_dset.virtual_select) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                    "unable to release virtual selection");
      ent->source_dset.virtual_select = NULL;
      if (ent->source_select && H5S_close(ent->source_select) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                    "unable to release source selection");
    ...
    if (free_list)
      virtual_layout.storage.u.virt.list =
          (H5O_storage_virtual_ent_t *)H5MM_xfree(
              virtual_layout.storage.u.virt.list);
  } /* end if */
...
hid_t H5Pget_virtual_vspace(hid_t dcpl_id, size_t idx) {
  ...
  if (NULL == (space = H5S_copy(
                   layout.storage.u.virt.list[idx].source_dset.virtual_select,
                   FALSE, TRUE)))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL,
                "unable to copy virtual selection");
...
hid_t H5Pget_virtual_srcspace(hid_t dcpl_id, size_t idx) {
   ...
   * of selection */
  if ((H5O_VIRTUAL_STATUS_INVALID ==
       layout.storage.u.virt.list[idx].source_space_status) &&
    ...
    if ((rank = H5S_GET_EXTENT_NDIMS(
             layout.storage.u.virt.list[idx].source_select)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get source space rank");

    /* Get bounds of selection */
    if (H5S_SELECT_BOUNDS(layout.storage.u.virt.list[idx].source_select,
    ...
    /* Set extent */
    if (H5S_set_extent_simple(layout.storage.u.virt.list[idx].source_select,
                              (unsigned)rank, bounds_end, NULL) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't set source space extent");
  ...
  if (NULL == (space = H5S_copy(layout.storage.u.virt.list[idx].source_select,
                                FALSE, TRUE)))
    HGOTO_ERROR(H5E_PLIST, H5E_CANTCOPY, FAIL,
                "unable to copy source selection");
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Pget_virtual_filename(hid_t dcpl_id, size_t idx, char *name /*out*/,
                                size_t size) {
  H5P_genplist_t *plist; /* Property list pointer */
  H5O_layout_t layout;   /* Layout information */
  ...
  if (name && (size > 0))
    (void)HDstrncpy(name, layout.storage.u.virt.list[idx].source_file_name,
                    size);
  ret_value =
      (ssize_t)HDstrlen(layout.storage.u.virt.list[idx].source_file_name);
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5Pget_virtual_dsetname(hid_t dcpl_id, size_t idx, char *name /*out*/,
                                size_t size) {
  H5P_genplist_t *plist; /* Property list pointer */
  H5O_layout_t layout;   /* Layout information */
  ssize_t ret_value;     /* Return value */
  ...
  if (name && (size > 0))
    (void)HDstrncpy(name, layout.storage.u.virt.list[idx].source_dset_name,
                    size);
  ret_value =
      (ssize_t)HDstrlen(layout.storage.u.virt.list[idx].source_dset_name);
...
herr_t H5Pset_chunk_opts(hid_t plist_id, unsigned options) {
  ...
  uint8_t layout_flags =
      0; /* "options" translated into layout message flags format */
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5Pset_external(hid_t plist_id, const char *name, off_t offset,
  ...
  if (efl.nused >= efl.nalloc) {
    size_t na = efl.nalloc + H5O_EFL_ALLOC;
    H5O_efl_entry_t *x =
        (H5O_efl_entry_t *)H5MM_realloc(efl.slot, na * sizeof(H5O_efl_entry_t));
...
herr_t H5Pget_external(hid_t plist_id, unsigned idx, size_t name_size,
  ...
  /* XXX: Badness!
   *
   * The offset parameter is of type off_t and the offset field of
   * H5O_efl_entry_t is HDoff_t which is a different type on Windows (off_t is a
   * 32-bit long, HDoff_t is __int64, a 64-bit type).
   *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_szip(hid_t plist_id, unsigned options_mask,
  ...
  options_mask &=
      (unsigned)(~(H5_SZIP_LSB_OPTION_MASK | H5_SZIP_MSB_OPTION_MASK));
  ...
  if (H5Z_append(&pline, H5Z_FILTER_SZIP, H5Z_FLAG_OPTIONAL, (size_t)2,
                 cd_values) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                "unable to add szip filter to pipeline");
...
herr_t H5Pset_shuffle(hid_t plist_id) {
  ...
  if (H5P_peek(plist, H5O_CRT_PIPELINE_NAME, &pline) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get pipeline");
  if (H5Z_append(&pline, H5Z_FILTER_SHUFFLE, H5Z_FLAG_OPTIONAL, (size_t)0,
                 NULL) < 0)
...
herr_t H5Pset_nbit(hid_t plist_id) {
  ...
  if (H5P_peek(plist, H5O_CRT_PIPELINE_NAME, &pline) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get pipeline");
  if (H5Z_append(&pline, H5Z_FILTER_NBIT, H5Z_FLAG_OPTIONAL, (size_t)0, NULL) <
      0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                "unable to add nbit filter to pipeline");
  if (H5P_poke(plist, H5O_CRT_PIPELINE_NAME, &pline) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL, "unable to set pipeline");
 ...
 * Parameters:  scale_factor:
                              ...
                              scaleoffset filter will calculate the
 minimum-bits.
...
herr_t H5Pset_scaleoffset(hid_t plist_id, H5Z_SO_scale_type_t scale_type,
   ...
   * scale_type = 0:     floating-point type, filter uses variable-minimum-bits
   * method, scale_factor is decimal scale factor scale_type = 1: floating-point
   * type, filter uses fixed-minimum-bits method, scale_factor is the fixed
   * minimum number of bits scale type = other: integer type, scale_factor is
   * minimum number of bits if scale_factor = 0, then filter calculates minimum
   * number of bits
   */
  ...
  if (H5P_peek(plist, H5O_CRT_PIPELINE_NAME, &pline) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get pipeline");
  if (H5Z_append(&pline, H5Z_FILTER_SCALEOFFSET, H5Z_FLAG_OPTIONAL, (size_t)2,
                 cd_values) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                "unable to add scaleoffset filter to pipeline");
  if (H5P_poke(plist, H5O_CRT_PIPELINE_NAME, &pline) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL, "unable to set pipeline");
...
herr_t H5Pset_fill_value(hid_t plist_id, hid_t type_id, const void *value) {
  ...
  if (value) {
    ...
    /* If necessary, convert fill value datatypes (which copies VL components,
     * etc.) */
    if (!H5T_path_noop(tpath)) {
      ...
      if (H5T_path_bkg(tpath) &&
          NULL == (bkg_buf = H5FL_BLK_CALLOC(type_conv, (size_t)fill.size)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                    "memory allocation failed");

      /* Convert the fill value */
      if (H5T_convert(tpath, type_id, type_id, (size_t)1, (size_t)0, (size_t)0,
                      fill.buf, bkg_buf) < 0) {
        if (bkg_buf)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5P_get_fill_value(H5P_genplist_t *plist, const H5T_t *type,
                          void *value /*out*/) {
  ...
  if ((src_id = H5I_register(
           H5I_DATATYPE, H5T_copy(fill.type, H5T_COPY_TRANSIENT), FALSE)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
  ...
  if (H5T_get_size(type) >= H5T_get_size(fill.type)) {
    ...
    if (NULL == (buf = H5MM_calloc(H5T_get_size(fill.type))))
      HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, FAIL,
                  "memory allocation failed for type conversion");
    if (H5T_path_bkg(tpath) &&
        NULL == (bkg = H5MM_calloc(H5T_get_size(fill.type))))
      HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, FAIL,
                  "memory allocation failed for type conversion");
  } /* end else */
  ...
  /* Do the conversion */
  if ((dst_id =
           H5I_register(H5I_DATATYPE, H5T_copy(type, H5T_COPY_ALL), FALSE)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
                "unable to copy/register datatype");
  if (H5T_convert(tpath, src_id, dst_id, (size_t)1, (size_t)0, (size_t)0, buf,
                  bkg) < 0)
  ...
  if (src_id >= 0 && H5I_dec_ref(src_id) < 0)
    HDONE_ERROR(H5E_PLIST, H5E_CANTDEC, FAIL,
                "can't decrement ref count of temp ID");
...
herr_t H5P_is_fill_value_defined(const H5O_fill_t *fill,
  ...
  /* Check if the fill value was set by the application */
    ...
    *status = H5D_FILL_VALUE_ERROR;
    HGOTO_ERROR(H5E_PLIST, H5E_BADRANGE, FAIL,
                "invalid combination of fill-value info");
  } /* end else */
...
herr_t H5Pset_alloc_time(hid_t plist_id, H5D_alloc_time_t alloc_time) {
  ...
  if (alloc_time < H5D_ALLOC_TIME_DEFAULT || alloc_time > H5D_ALLOC_TIME_INCR)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid allocation time setting");
  ...
  if (alloc_time == H5D_ALLOC_TIME_DEFAULT) {
    ...
    /* Set the "state" of the allocation time property to indicate the user
     * modified it */
    alloc_time_state = 0;
 ...
 *-----------------------------------------------------------------------------
 */
herr_t H5Pget_dset_no_attrs_hint(hid_t dcpl_id, hbool_t *minimize /*out*/) {
