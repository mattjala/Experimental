...
#define HSYS_DONE_ERROR(majorcode, minorcode, retcode, str)                    \
  {                                                                            \
    int myerrno = errno;                                                       \
    /* Other projects may rely on the description format to get the errno and  \
     * any changes should be considered as an API change                                                   \
     */                                                                        \
...
/* On Windows we also emit the result of GetLastError(). This call returns a
 * DWORD, which is always a 32-bit unsigned type. Note that on Windows, either
 * errno or GetLastError() (but probably not both) will be useful depending on
 * whether a C/POSIX or Win32 call failed. The other value will likely be zero,
...
#define HSYS_DONE_ERROR(majorcode, minorcode, retcode, str)                    \
  {                                                                            \
     ...
     */                                                                        \
    HDONE_ERROR(majorcode, minorcode, retcode,                                 \
                "%s, errno = %d, error message = '%s', Win32 GetLastError() "  \
                "= %" PRIu32 "",                                               \
...
#define HMPI_DONE_ERROR(retcode, str, mpierr)                                  \
  {                                                                            \
    ...
    HDONE_ERROR(H5E_INTERNAL, H5E_MPI, retcode,                                \
                "%s: MPI error string is '%s'", str, H5E_mpi_error_str);       \
  }
#define HMPI_GOTO_ERROR(retcode, str, mpierr)                                  \
  {                                                                            \
    MPI_Error_string(mpierr, H5E_mpi_error_str, &H5E_mpi_error_str_len);       \
    HGOTO_ERROR(H5E_INTERNAL, H5E_MPI, retcode,                                \
                "%s: MPI error string is '%s'", str, H5E_mpi_error_str);       \
  }
...
H5_DLL herr_t H5E_printf_stack(H5E_t *estack, const char *file,
