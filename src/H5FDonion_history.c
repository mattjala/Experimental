 ...
 *-----------------------------------------------------------------------------
 */
herr_t H5FD__onion_ingest_history(H5FD_onion_history_t *history_out,
  ...
  if (H5FD_get_eof(raw_file, H5FD_MEM_DRAW) < (addr + size))
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "header indicates history beyond EOF");
  ...
  if (history_out->n_revisions > 0)
    if (NULL ==
        (history_out->record_locs = H5MM_calloc(
             history_out->n_revisions * sizeof(H5FD_onion_record_loc_t))))
 ...
 *
 * Returns:     Success:    Number of bytes written to destination file (always
 *non-zero) Failure:    0
 *-----------------------------------------------------------------------------
 */
uint64_t H5FD__onion_write_history(H5FD_onion_history_t *history, H5FD_t *file,
  ...
  if (NULL == (buf = H5MM_malloc(H5FD_ONION_ENCODED_SIZE_HISTORY +
                                 (H5FD_ONION_ENCODED_SIZE_RECORD_POINTER *
                                  history->n_revisions))))
    HGOTO_ERROR(H5E_VFL, H5E_CANTALLOC, 0,
                "can't allocate buffer for updated history");
  ...
  if ((size + off_start > filesize_curr) &&
      (H5FD_set_eoa(file, H5FD_MEM_DRAW, off_start + size) < 0))
    HGOTO_ERROR(H5E_VFL, H5E_CANTSET, 0,
                "can't modify EOA for updated history");
  ...
  if (0 == history->n_revisions) {
    ...
    if (history->n_revisions != n_revisions)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, 0,
                  "history argument suggests different revision count than "
                  "encoded buffer");
 ...
 *-----------------------------------------------------------------------------
 */
size_t H5FD__onion_history_encode(H5FD_onion_history_t *history,
                                  unsigned char *buf, uint32_t *checksum) {
