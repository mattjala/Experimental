...
static herr_t H5D__virtual_io_init(H5D_io_info_t *io_info,
                                   H5D_dset_io_info_t *dinfo);
static herr_t H5D__virtual_read(H5D_io_info_t *io_info,
                                H5D_dset_io_info_t *dinfo);
...
static herr_t H5D__virtual_build_source_name(
    char *source_name, const H5O_storage_virtual_name_seg_t *parsed_name,
...
static herr_t H5D__virtual_read_one(H5D_dset_io_info_t *dset_info,
                                    H5O_storage_virtual_srcdset_t *source_dset);
static herr_t
H5D__virtual_write_one(H5D_dset_io_info_t *dset_info,
                       H5O_storage_virtual_srcdset_t *source_dset);
...
herr_t H5D_virtual_check_mapping_pre(const H5S_t *vspace,
  ...
  if (nelmts_vs == H5S_UNLIMITED) {
    ...
    if (nelmts_ss == H5S_UNLIMITED) {
      hsize_t nenu_vs; /* Number of elements in the non-unlimited dimensions of
                          vspace */
      hsize_t nenu_ss; /* Number of elements in the non-unlimited dimensions of
                          src_space */
      ...
      if (nenu_vs != nenu_ss)
        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                    "numbers of elements in the non-unlimited dimensions is "
                    "different for source and "
                    "virtual spaces");
    } /* end if */
    ...
    if (nelmts_vs != nelmts_ss)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "virtual and source space selections have different numbers "
                  "of elements");
  ...
  if ((nelmts_vs == H5S_UNLIMITED) && (nelmts_ss != H5S_UNLIMITED)) {
    ...
    if ((ent->psfn_nsubs == 0) && (ent->psdn_nsubs == 0))
      HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, FAIL,
                  "unlimited virtual selection, limited source selection, and "
                  "no printf specifiers in "
                  "source names");

    /* Make sure virtual space uses hyperslab selection */
    if (H5S_GET_SELECT_TYPE(ent->source_dset.virtual_select) !=
        H5S_SEL_HYPERSLABS)
      HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, FAIL,
    ...
    if (ent->source_space_status != H5O_VIRTUAL_STATUS_INVALID) {
      ...
      if (NULL == (tmp_space = H5S_hyper_get_unlim_block(
                       ent->source_dset.virtual_select, (hsize_t)0)))
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                    "can't get first block in virtual selection");

      /* Check number of points */
      nelmts_vs = (hsize_t)H5S_GET_SELECT_NPOINTS(tmp_space);
      if (nelmts_vs != nelmts_ss)
        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                    "virtual (single block) and source space selections have "
    ...
    if ((ent->psfn_nsubs > 0) || (ent->psdn_nsubs > 0))
      HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, FAIL,
                  "printf specifier(s) in source name(s) without an unlimited "
                  "virtual selection and "
...
herr_t H5D_virtual_update_min_dims(H5O_layout_t *layout, size_t idx) {
  ...
  if (H5S_SEL_ERROR ==
      (sel_type = H5S_GET_SELECT_TYPE(ent->source_dset.virtual_select)))
  ...
  if ((rank = H5S_GET_EXTENT_NDIMS(ent->source_dset.virtual_select)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "unable to get number of dimensions");

  /* Get selection bounds */
  if (H5S_SELECT_BOUNDS(ent->source_dset.virtual_select, bounds_start,
                        bounds_end) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "unable to get selection bounds");
...
herr_t H5D_virtual_check_min_dims(const H5D_t *dset) {
  ...
  for (i = 0; i < rank; i++)
    if (dims[i] < dset->shared->layout.storage.u.virt.min_dims[i])
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "virtual dataset dimensions not large enough to contain all "
...
herr_t H5D__virtual_store_layout(H5F_t *f, H5O_layout_t *layout) {
  ...
  if (virt->list_nused > 0) {
    ...
    if (NULL == (str_size = (size_t *)H5MM_malloc(2 * virt->list_nused *
                                                  sizeof(size_t))))
      HGOTO_ERROR(H5E_OHDR, H5E_RESOURCE, FAIL,
                  "unable to allocate string length array");
    ...
    for (i = 0; i < virt->list_nused; i++) {
      ...
      if ((select_serial_size = H5S_SELECT_SERIAL_SIZE(ent->source_select)) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTENCODE, FAIL,
                    "unable to check dataspace selection size");
      block_size += (size_t)select_serial_size;

      /* Virtual dataset selection */
      if ((select_serial_size =
               H5S_SELECT_SERIAL_SIZE(ent->source_dset.virtual_select)) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTENCODE, FAIL,
                    "unable to check dataspace selection size");
      block_size += (size_t)select_serial_size;
    } /* end for */
    ...
    *heap_block_p++ = (uint8_t)H5O_LAYOUT_VDS_GH_ENC_VERS;
    ...
    for (i = 0; i < virt->list_nused; i++) {
      ...
      /* Source dataset name */
      H5MM_memcpy((char *)heap_block_p, ent->source_dset_name,
                  str_size[(2 * i) + 1]);
      ...
      /* Virtual selection */
      if (H5S_SELECT_SERIALIZE(ent->source_dset.virtual_select, &heap_block_p) <
          0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL,
    ...
    if (H5HG_insert(f, block_size, heap_block, &(virt->serial_list_hobjid)) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINSERT, FAIL,
                  "unable to insert virtual dataset heap block");
  } /* end if */
...
herr_t H5D__virtual_copy_layout(H5O_layout_t *layout) {
  ...
  if (virt->list_nused > 0) {
    ...
    if (NULL ==
        (virt->list = H5MM_calloc(virt->list_nused * sizeof(virt->list[0]))))
    ...
    for (i = 0; i < virt->list_nused; i++) {
      ...
      if (NULL == (ent->source_dset.virtual_select = H5S_copy(
                       orig_list[i].source_dset.virtual_select, FALSE, TRUE)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                    "unable to copy virtual selection");

      /* Copy original source names */
      if (NULL ==
          (ent->source_file_name = H5MM_strdup(orig_list[i].source_file_name)))
        HGOTO_ERROR(H5E_DATASET, H5E_RESOURCE, FAIL,
                    "unable to duplicate source file name");
      if (NULL ==
          (ent->source_dset_name = H5MM_strdup(orig_list[i].source_dset_name)))
        HGOTO_ERROR(H5E_DATASET, H5E_RESOURCE, FAIL,
                    "unable to duplicate source dataset name");
      ...
      if (orig_list[i].unlim_dim_virtual < 0) {
        ...
        ent->source_dset.clipped_virtual_select =
            ent->source_dset.virtual_select;
      } /* end if */

      /* Copy parsed names */
      if (H5D__virtual_copy_parsed_name(&ent->parsed_source_file_name,
                                        orig_list[i].parsed_source_file_name) <
          0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                    "unable to copy parsed source file name");
      ...
      if (H5D__virtual_copy_parsed_name(&ent->parsed_source_dset_name,
          ...
          0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                    "unable to copy parsed source dataset name");
      ...
      if (orig_list[i].source_dset.file_name) {
        if (orig_list[i].source_dset.file_name == orig_list[i].source_file_name)
          ...
          ent->source_dset.file_name =
              ent->parsed_source_file_name->name_segment;
        } /* end if */
        else if (NULL == (ent->source_dset.file_name =
      ...
      if (orig_list[i].source_dset.dset_name) {
        if (orig_list[i].source_dset.dset_name == orig_list[i].source_dset_name)
          ...
          ent->source_dset.dset_name =
              ent->parsed_source_dset_name->name_segment;
        } /* end if */
        else if (NULL == (ent->source_dset.dset_name =
  ...
  if (orig_source_fapl >= 0) {
    if (NULL == (plist = (H5P_genplist_t *)H5I_object_verify(orig_source_fapl,
                                                             H5I_GENPROP_LST)))
  ...
  if (orig_source_dapl >= 0) {
    if (NULL == (plist = (H5P_genplist_t *)H5I_object_verify(orig_source_dapl,
                                                             H5I_GENPROP_LST)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a property list");
  ...
  if (ret_value < 0)
    if (H5D__virtual_reset_layout(layout) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                  "unable to reset virtual layout");
...
herr_t H5D__virtual_reset_layout(H5O_layout_t *layout) {
  ...
  for (i = 0; i < virt->list_nused; i++) {
    ...
    if (H5D__virtual_reset_source_dset(ent, &ent->source_dset) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                  "unable to reset source dataset");
...
herr_t H5D__virtual_copy(H5F_t *f_dst, H5O_layout_t *layout_dst) {
  ...
  if (f_dst == f_src) {
    ...
    if ((heap_rc = H5HG_link(
             f_dst, (H5HG_t *)&(layout_dst->u.virt.serial_list_hobjid), 1)) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTMODIFY, FAIL,
                  "unable to adjust global heap reference count");
  } /* end if */
  else
#endif /* NOT_YET */
  {
...
herr_t H5D__virtual_delete(H5F_t *f, H5O_storage_t *storage) {
  ...
  if (storage->u.virt.serial_list_hobjid.addr != HADDR_UNDEF) {
    ...
    if ((heap_rc = H5HG_link(f, (H5HG_t *)&(storage->u.virt.serial_list_hobjid),
                             -1)) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTMODIFY, FAIL,
                  "unable to adjust global heap reference count");
    if (heap_rc == 0)
...
H5D__virtual_open_source_dset(const H5D_t *vdset,
  ...
  hbool_t src_file_open =
      FALSE; /* Whether we have opened and need to close src_file */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (HDstrcmp(source_dset->file_name, ".") != 0) {
    ...
    src_file = H5F_prefix_open_file(
        vdset->oloc.file, H5F_PREFIX_VDS, vdset->shared->vds_prefix,
  ...
  if (src_file) {
    ...
    if (NULL == (src_root_loc.path = H5G_nameof(H5G_rootof(src_file))))
      HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                  "unable to get path for root group");

    /* Try opening the source dataset */
    source_dset->dset =
        H5D__open_name(&src_root_loc, source_dset->dset_name,
    ...
    if (NULL == source_dset->dset) {
      ...
      if (virtual_ent->source_space_status != H5O_VIRTUAL_STATUS_CORRECT) {
        if (H5S_extent_copy(virtual_ent->source_select,
                            source_dset->dset->shared->space) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                      "can't copy source dataspace extent");
  ...
  if (src_file_open)
    if (H5F_efc_close(vdset->oloc.file, src_file) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTCLOSEFILE, FAIL,
                  "can't close source file");
  ...
  if (source_dset->clipped_virtual_select) {
    if (source_dset->clipped_virtual_select != source_dset->virtual_select)
      if (H5S_close(source_dset->clipped_virtual_select) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                    "unable to release clipped virtual selection");
  ...
  if (source_dset->clipped_source_select) {
    if (source_dset->clipped_source_select != virtual_ent->source_select)
      if (H5S_close(source_dset->clipped_source_select) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                    "unable to release clipped source selection");
  ...
  if (!*buf) {
    ...
    if (NULL == (*buf = (char *)H5MM_malloc(src_len + (size_t)1)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                  "unable to allocate name segment struct");
    ...
    *p = *buf;
    ...
    if ((p_offset + src_len + (size_t)1) > *buf_size) {
      ...
      if (NULL == (tmp_buf = (char *)H5MM_realloc(*buf, tmp_buf_size)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "unable to reallocate name segment buffer");
      *buf = tmp_buf;
...
H5D_virtual_parse_source_name(const char *source_name,
  ...
  while ((pct = HDstrchr(p, '%'))) {
    ...
    if (!*tmp_parsed_name_p)
      if (NULL ==
          (*tmp_parsed_name_p = H5FL_CALLOC(H5O_storage_virtual_name_seg_t)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "unable to allocate name segment struct");
    ...
    if (pct[1] == 'b') {
      ...
      if (pct != p)
        ...
        if (H5D__virtual_str_append(p, (size_t)(pct - p), &name_seg_p,
                                    &(*tmp_parsed_name_p)->name_segment,
                                    &name_seg_size) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                      "unable to append name segment");
  ...
  if (tmp_parsed_name) {
    ...
    if (*p == '\0')
      ...
      /* Append string to name segment */
      if (H5D__virtual_str_append(
              p, tmp_strlen - (size_t)(p - source_name), &name_seg_p,
  ...
  while (p_src) {
    ...
    if (p_src->name_segment) {
      if (NULL == ((*p_dst)->name_segment = H5MM_strdup(p_src->name_segment)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "unable to duplicate name segment");
    } /* end if */
...
static herr_t H5D__virtual_build_source_name(
  ...
  if (nsubs == 0) {
    ...
    if (NULL == (tmp_name = (char *)H5MM_malloc(name_len)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                  "unable to allocate name buffer");
    p = tmp_name;
    ...
    do {
      ...
      if (nsubs_rem > 0) {
        ...
        if (HDsnprintf(p, name_len_rem, "%llu", (long long unsigned)blockno) <
            0)
          HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                      "unable to write block number to string");
...
herr_t H5D__virtual_set_extent_unlim(const H5D_t *dset) {
  ...
  assert((storage->view == H5D_VDS_FIRST_MISSING) ||
         (storage->view == H5D_VDS_LAST_AVAILABLE));
    ...
    if (storage->list[i].unlim_dim_virtual >= 0) {
      ...
      if (storage->list[i].unlim_dim_source >= 0) {
        ...
        if (storage->list[i].source_dset.dset) {
          ...
          if (H5S_get_simple_extent_dims(storage->list[i].source_select,
                                         curr_dims, NULL) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                        "can't get source space dimensions");
          ...
          if (curr_dims[storage->list[i].unlim_dim_source] ==
            ...
            if (storage->view == H5D_VDS_LAST_AVAILABLE) {
              if (clip_size != storage->list[i].clip_size_virtual) {
                ...
                if (NULL ==
                    (storage->list[i].source_dset.clipped_virtual_select =
                         H5S_copy(storage->list[i].source_dset.virtual_select,
                                  FALSE, TRUE)))
                  HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                              "unable to copy virtual selection");
              ...
              if (storage->list[i].source_dset.clipped_source_select) {
                assert(storage->list[i].source_dset.clipped_source_select !=
                       storage->list[i].source_select);
                if (H5S_close(
                        storage->list[i].source_dset.clipped_source_select) < 0)
                  HGOTO_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
              ...
              /* Copy source selection */
              if (NULL ==
                  (storage->list[i].source_dset.clipped_source_select =
        ...
        /* printf mapping */
        hsize_t first_missing = 0; /* First missing dataset in the current block
                                      of missing datasets */
          ...
          if (j >= (hsize_t)storage->list[i].sub_dset_nalloc) {
            if (storage->list[i].sub_dset_nalloc == 0) {
              ...
              if (NULL == (storage->list[i].sub_dset =
                               (H5O_storage_virtual_srcdset_t *)H5MM_calloc(
              ...
              (void)memset(
                  &storage->list[i].sub_dset[storage->list[i].sub_dset_nalloc],
                  0,
          ...
          if (storage->list[i].sub_dset[j].dset_exists)
            ...
            if (!storage->list[i].sub_dset[j].file_name)
              if (H5D__virtual_build_source_name(
                      storage->list[i].source_file_name,
            ...
            if (!storage->list[i].sub_dset[j].dset_name)
              if (H5D__virtual_build_source_name(
                      storage->list[i].source_dset_name,
            ...
            /* Open source dataset */
            if (H5D__virtual_open_source_dset(
                    dset, &storage->list[i], &storage->list[i].sub_dset[j]) < 0)
              HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL,
                          "unable to open source dataset");
        ...
        if ((first_missing == (hsize_t)storage->list[i].sub_dset_nused) &&
          ...
          if (first_missing == 0)
            ...
            if (storage->view == H5D_VDS_LAST_AVAILABLE) {
              ...
              if (H5S_SELECT_BOUNDS(storage->list[i]
                                        .sub_dset[first_missing - (hsize_t)1]
                                        .virtual_select,
               ...
               * dimension */
              clip_size =
                  bounds_end[storage->list[i].unlim_dim_virtual] + (hsize_t)1;
            } /* end if */
            else {
              /* Get bounds from first missing virtual selection
               */
              if (H5S_SELECT_BOUNDS(
                      storage->list[i].sub_dset[first_missing].virtual_select,
                      bounds_start, bounds_end) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                            "unable to get selection bounds");
  ...
  if (changed || (!storage->init && (storage->view == H5D_VDS_FIRST_MISSING))) {
    ...
    for (i = 0; i < storage->list_nused; i++) {
      ...
      if ((storage->list[i].unlim_dim_virtual >= 0) &&
        ...
        if (storage->list[i].unlim_dim_source >= 0) {
          ...
          if (storage->list[i].source_dset.clipped_virtual_select) {
            assert(storage->list[i].source_dset.clipped_virtual_select !=
                   storage->list[i].source_dset.virtual_select);
            if (H5S_close(storage->list[i].source_dset.clipped_virtual_select) <
                0)
              HGOTO_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                          "unable to release clipped virtual dataspace");
          } /* end if */
          ...
          clip_size = H5S_hyper_get_clip_extent(
              storage->list[i].source_select,
          ...
          for (j = 0; j < storage->list[i].sub_dset_nalloc; j++) {
            ...
            if (storage->list[i].sub_dset[j].clipped_source_select !=
              ...
              if (storage->list[i].sub_dset[j].clipped_source_select)
                if (H5S_close(
                        storage->list[i].sub_dset[j].clipped_source_select) < 0)
                  HGOTO_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                              "unable to release clipped source dataspace");
       ...
       * extents */
      if (H5S_set_extent(storage->list[i].source_dset.virtual_select,
                         new_dims) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                    "unable to modify size of dataspace");
      if ((storage->list[i].source_dset.clipped_virtual_select !=
           storage->list[i].source_dset.virtual_select) &&
          storage->list[i].source_dset.clipped_virtual_select)
        if (H5S_set_extent(storage->list[i].source_dset.clipped_virtual_select,
                           new_dims) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                      "unable to modify size of dataspace");
      ...
      for (j = 0; j < storage->list[i].sub_dset_nalloc; j++)
        if (storage->list[i].sub_dset[j].virtual_select) {
          if (H5S_set_extent(storage->list[i].sub_dset[j].virtual_select,
                             new_dims) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                        "unable to modify size of dataspace");
          if ((storage->list[i].sub_dset[j].clipped_virtual_select !=
      ...
      if (storage->list[i].unlim_dim_source >= 0) {
        ...
        if (storage->list[i].source_dset.dset) {
          ...
          if (storage->list[i].source_dset.clipped_source_select) {
            assert(storage->list[i].source_dset.clipped_source_select !=
                   storage->list[i].source_select);
            if (H5S_close(storage->list[i].source_dset.clipped_source_select) <
                0)
              HGOTO_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                          "unable to release clipped source dataspace");
          } /* end if */
        ...
        if (!storage->list[i].sub_dset) {
          ...
          /* Clear new space in sub_dset */
          (void)memset(
              &storage->list[i].sub_dset[storage->list[i].sub_dset_nalloc], 0,
        ...
        for (j = 0; j < sub_dset_max; j++) {
          ...
          if (storage->list[i].sub_dset[j].clipped_virtual_select !=
            ...
            if (storage->list[i].sub_dset[j].clipped_virtual_select)
              if (H5S_close(
                      storage->list[i].sub_dset[j].clipped_virtual_select) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                            "unable to release clipped virtual dataspace");
...
herr_t H5D__virtual_init(H5F_t *f, const H5D_t *dset, hid_t dapl_id) {
  ...
  for (i = 0; i < storage->list_nused; i++) {
    ...
    /* Patch extent */
    if (H5S_extent_copy(storage->list[i].source_dset.virtual_select,
                        dset->shared->space) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                  "can't copy virtual dataspace extent");
    ...
    if (H5S_hyper_normalize_offset(storage->list[i].source_dset.virtual_select,
                                   old_offset) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_BADSELECT, FAIL,
                  "unable to normalize dataspace by offset");
    if (H5S_hyper_normalize_offset(storage->list[i].source_select, old_offset) <
        0)
      HGOTO_ERROR(H5E_DATASET, H5E_BADSELECT, FAIL,
                  "unable to normalize dataspace by offset");
  } /* end for */
  ...
  if (storage->source_fapl <= 0) {
    ...
    H5F_close_degree_t close_degree =
        H5F_CLOSE_WEAK; /* Close degree for source files */
    ...
    if (NULL ==
        (source_fapl = (H5P_genplist_t *)H5I_object(storage->source_fapl)))
      ...
      for (j = storage->list[i].sub_dset_io_start;
        ...
        if (storage->list[i].sub_dset[j].dset &&
          ...
          HGOTO_DONE(TRUE);
             ...
             storage->list[i]
                 .source_dset.dset->shared->layout.ops->is_data_cached &&
             storage->list[i]
                 .source_dset.dset->shared->layout.ops->is_data_cached(
 ...
 */
static herr_t H5D__virtual_io_init(H5D_io_info_t *io_info,
                                   H5D_dset_io_info_t H5_ATTR_UNUSED *dinfo) {
  FUNC_ENTER_PACKAGE_NOERR

  /* Disable selection I/O */
  io_info->use_select_io = H5D_SELECTION_IO_MODE_OFF;
  io_info->no_selection_io_cause |=
...
static herr_t H5D__virtual_pre_io(H5D_dset_io_info_t *dset_info,
  ...
  int rank = 0;
  hbool_t bounds_init =
      FALSE;      /* Whether bounds_start, bounds_end, and rank are valid */
  ...
  if (!storage->init)
    if (H5D__virtual_init_all(dset) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "can't initialize virtual layout");
  ...
  for (i = 0; i < storage->list_nused; i++) {
    ...
    if (storage->list[i].psfn_nsubs || storage->list[i].psdn_nsubs) {
      ...
      storage->list[i].sub_dset_io_start =
          (size_t)H5S_hyper_get_first_inc_block(
              storage->list[i].source_dset.virtual_select,
      ...
      storage->list[i].sub_dset_io_end = (size_t)H5S_hyper_get_first_inc_block(
          storage->list[i].source_dset.virtual_select,
          bounds_end[storage->list[i].unlim_dim_virtual], &partial_block);
      if (partial_block)
      ...
      for (j = storage->list[i].sub_dset_io_start;
        ...
        if (!storage->list[i].sub_dset[j].clipped_virtual_select) {
          ...
          if (storage->list[i].source_space_status ==
            ...
            if (H5S_SELECT_BOUNDS(storage->list[i].sub_dset[j].virtual_select,
                                  tmp_dims, vbounds_end) < 0)
              HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                          "unable to get selection bounds");
            ...
            /* Get current VDS dimensions */
            if (H5S_get_simple_extent_dims(dset->shared->space, tmp_dims,
                                           NULL) < 0)
              HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                          "can't get VDS dimensions");

            /* Copy virtual selection */
            if (NULL ==
                (storage->list[i].sub_dset[j].clipped_virtual_select = H5S_copy(
                     storage->list[i].sub_dset[j].virtual_select, FALSE, TRUE)))
              HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                          "unable to copy virtual selection");

            /* Clip virtual selection to real virtual extent */
            (void)memset(start, 0, sizeof(start));
            if (H5S_select_hyperslab(
                    storage->list[i].sub_dset[j].clipped_virtual_select,
                    H5S_SELECT_AND, start, NULL, tmp_dims, NULL) < 0)
              HGOTO_ERROR(H5E_DATASET, H5E_CANTSELECT, FAIL,
                          "unable to clip hyperslab");
      ...
      if (storage->list[i].source_dset.clipped_virtual_select) {
        ...
        if (select_nelmts > (hssize_t)0) {
          ...
          if (!storage->list[i].source_dset.dset)
            /* Try to open dataset */
            if (H5D__virtual_open_source_dset(
                    dset, &storage->list[i], &storage->list[i].source_dset) < 0)
              HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL,
                          "unable to open source dataset");
 ...
 */
static herr_t
H5D__virtual_read_one(H5D_dset_io_info_t *dset_info,
                      H5O_storage_virtual_srcdset_t *source_dset) {
  H5S_t *projected_src_space =
      NULL; /* File space for selection in a single source dataset */
...

    {
      ...
      if (H5D__read(1, &source_dinfo) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                    "can't read source dataset");
    }

    /* Close projected_src_space */
    if (H5S_close(projected_src_space) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                  "can't close projected source space");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_read(H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
  ...
  if (H5F_HAS_FEATURE(dset_info->dset->oloc.file, H5FD_FEAT_HAS_MPI))
    HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                "parallel reads not supported on virtual datasets");
#endif /* H5_HAVE_PARALLEL */

  /* Prepare for I/O operation */
  if (H5D__virtual_pre_io(dset_info, storage, dset_info->file_space,
                          dset_info->mem_space, &tot_nelmts) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCLIP, FAIL,
                "unable to prepare for I/O operation");
  ...
  if (tot_nelmts < nelmts) {
    ...
    if (H5P_is_fill_value_defined(&dset_info->dset->shared->dcpl_cache.fill,
                                  &fill_status) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
    ...
    if (fill_status != H5D_FILL_VALUE_UNDEFINED) {
      ...
      if (NULL == (fill_space = H5S_copy(dset_info->mem_space, FALSE, TRUE)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                    "unable to copy memory selection");
      ...
      for (i = 0; i < storage->list_nused; i++)
        ...
        if (storage->list[i].psfn_nsubs || storage->list[i].psdn_nsubs) {
          ...
          for (j = storage->list[i].sub_dset_io_start;
               j < storage->list[i].sub_dset_io_end; j++)
            if (storage->list[i].sub_dset[j].projected_mem_space)
              if (H5S_select_subtract(
                      fill_space,
                      storage->list[i].sub_dset[j].projected_mem_space) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTCLIP, FAIL,
                            "unable to clip fill selection");
        } /* end if */
      ...
      if (H5D__fill(dset_info->dset->shared->dcpl_cache.fill.buf,
                    dset_info->dset->shared->type, dset_info->buf.vp,
                    dset_info->type_info.mem_type, fill_space) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__virtual_write(H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
                                 H5D_dset_io_info_t *dset_info) {
  H5O_storage_virtual_t *storage; /* Convenient pointer into layout struct */
  hsize_t tot_nelmts;         /* Total number of elements mapped to mem_space */
  size_t nelmts;              /* Number of elements to process */
  ...
  if (H5F_HAS_FEATURE(dset_info->dset->oloc.file, H5FD_FEAT_HAS_MPI))
    HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                "parallel writes not supported on virtual datasets");
#endif /* H5_HAVE_PARALLEL */
    ...
    if (storage->list[i].psfn_nsubs || storage->list[i].psdn_nsubs) {
      ...
      for (j = storage->list[i].sub_dset_io_start;
        ...
        if (H5D__virtual_write_one(dset_info, &storage->list[i].sub_dset[j]) <
            0)
          HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                      "unable to write to source dataset");
    } /* end if */
    else
      ...
      for (j = 0; j < storage->list[i].sub_dset_nused; j++)
        if (storage->list[i].sub_dset[j].dset)
          ...
          if (H5D__flush_real(storage->list[i].sub_dset[j].dset) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                        "unable to flush source dataset");
    } /* end if */
    else if (storage->list[i].source_dset.dset)
      /* Flush source dataset */
...
herr_t H5D__virtual_hold_source_dset_files(const H5D_t *dset,
  ...
  if (ret_value < 0)
    ...
    if (*head && H5D__virtual_release_source_dset_files(*head) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                  "can't release source datasets' files held open");
    ...
    if (storage->list[i].psfn_nsubs || storage->list[i].psdn_nsubs) {
      ...
      for (j = 0; j < storage->list[i].sub_dset_nused; j++)
        ...
        if (storage->list[i].sub_dset[j].dset)
          ...
          if (H5D__virtual_refresh_source_dset(
                  &storage->list[i].sub_dset[j].dset) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTFLUSH, FAIL,
                        "unable to refresh source dataset");
    } /* end if */
    else
      ...
      if (storage->list[i].source_dset.dset)
        /* Refresh source dataset */
        if (H5D__virtual_refresh_source_dset(
                &storage->list[i].source_dset.dset) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTFLUSH, FAIL,
                      "unable to refresh source dataset");
 ...
 *
 * Purpose:     Release the hold on source files that are open, during a refresh
 *event
 *
 * Return:      Non-negative on success/Negative on failure
 *
...
herr_t H5D__virtual_release_source_dset_files(H5D_virtual_held_file_t *head) {
  ...
  while (head) {
    H5D_virtual_held_file_t *tmp =
        head->next; /* Temporary pointer to next node */
    ...
    if (H5F_try_close(head->file, NULL) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTCLOSEFILE, FAIL,
