...
typedef enum {
  H5Z_XFORM_ERROR,
  H5Z_XFORM_INTEGER, /* this represents an integer type in the data transform
                        expression */
  H5Z_XFORM_FLOAT,   /* this represents a floating point type in the data
...
static H5Z_node *H5Z__parse_expression(H5Z_token *current,
                                       H5Z_datval_ptrs *dat_val_pointers);
static H5Z_node *H5Z__parse_term(H5Z_token *current,
                                 H5Z_datval_ptrs *dat_val_pointers);
static H5Z_node *H5Z__parse_factor(H5Z_token *current,
                                   H5Z_datval_ptrs *dat_val_pointers);
...
static herr_t H5Z__xform_eval_full(H5Z_node *tree, size_t array_size,
                                   hid_t array_type, H5Z_result *res);
static void H5Z__xform_destroy_parse_tree(H5Z_node *tree);
static void *H5Z__xform_parse(const char *expression,
                              H5Z_datval_ptrs *dat_val_pointers);
static void *H5Z__xform_copy_tree(H5Z_node *tree,
                                  H5Z_datval_ptrs *dat_val_pointers,
                                  H5Z_datval_ptrs *new_dat_val_pointers);
static void H5Z__xform_reduce_tree(H5Z_node *tree);

/* PGCC (11.8-0) has trouble with the command *p++ = *p OP tree_val. It
 * increments P first before doing the operation.  So I break down the command
 * into two lines: *p = *p OP tree_val; p++; Actually, the behavior of *p++ = *p
 * OP tree_val is undefined. (SLU - 2012/3/19)
 */
#define H5Z_XFORM_DO_OP1(RESL, RESR, TYPE, OP, SIZE)                           \
  {                                                                            \
    ...
    if (((RESL).type == H5Z_XFORM_SYMBOL) &&                                   \
        ((RESR).type != H5Z_XFORM_SYMBOL)) {                                   \
      TYPE *p;                                                                 \
      double tree_val;                                                         \
                                                                               \
      tree_val =                                                               \
          ((RESR).type == H5Z_XFORM_INTEGER ? (double)(RESR).value.int_val     \
                                            : (RESR).value.float_val);         \
      ...
      if ((RESL).type == H5Z_XFORM_ERROR)                                      \
        ...
        tree_val =                                                             \
            ((RESL).type == H5Z_XFORM_INTEGER ? (double)(RESL).value.int_val   \
    ...
    } else if (((RESL).type == H5Z_XFORM_SYMBOL) &&                            \
               ((RESR).type == H5Z_XFORM_SYMBOL)) {                            \
      ...
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,                                \
                  "Unexpected type conversion operation");                     \
  }
...
#define H5Z_XFORM_DO_OP3(OP)                                                   \
  {                                                                            \
    if ((tree->lchild->type == H5Z_XFORM_INTEGER) &&                           \
      ...
      tree->value.int_val =                                                    \
          tree->lchild->value.int_val OP tree->rchild->value.int_val;          \
      H5MM_xfree(tree->lchild);                                                \
      H5MM_xfree(tree->rchild);                                                \
      tree->lchild = NULL;                                                     \
      tree->rchild = NULL;                                                     \
    } else if (((tree->lchild->type == H5Z_XFORM_FLOAT) ||                     \
                (tree->lchild->type == H5Z_XFORM_INTEGER)) &&                  \
               ((tree->rchild->type == H5Z_XFORM_FLOAT) ||                     \
                (tree->rchild->type == H5Z_XFORM_INTEGER))) {                  \
      tree->type = H5Z_XFORM_FLOAT;                                            \
      tree->value.float_val = ((tree->lchild->type == H5Z_XFORM_FLOAT)         \
                                   ? tree->lchild->value.float_val             \
                                   : (double)tree->lchild->value.int_val)      \
          OP((tree->rchild->type == H5Z_XFORM_FLOAT)                           \
                 ? tree->rchild->value.float_val                               \
  ...
  {                                                                            \
    if ((ret_value = (H5Z_node *)H5MM_malloc(sizeof(H5Z_node))) == NULL)       \
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,                             \
                  "Ran out of memory trying to copy parse tree");              \
    else {                                                                     \
      ret_value->type = (TYPE);                                                \
      if (tree->lchild)                                                        \
        ret_value->lchild = (H5Z_node *)H5Z__xform_copy_tree(                  \
            tree->lchild, dat_val_pointers, new_dat_val_pointers);             \
      else                                                                     \
        ret_value->lchild = NULL;                                              \
      if (tree->rchild)                                                        \
        ret_value->rchild = (H5Z_node *)H5Z__xform_copy_tree(                  \
            tree->rchild, dat_val_pointers, new_dat_val_pointers);             \
      else                                                                     \
...
#define H5Z_XFORM_DO_OP5(TYPE, SIZE)                                           \
  {                                                                            \
    TYPE val =                                                                 \
        ((tree->type == H5Z_XFORM_INTEGER) ? (TYPE)tree->value.int_val         \
                                           : (TYPE)tree->value.float_val);     \
    H5VM_array_fill(array, &val, sizeof(TYPE), (SIZE));                        \
  }

/* The difference of this macro from H5Z_XFORM_DO_OP3 is that it handles the
 * operations when the left operand is empty, like -x or +x. The reason that
 * it's separated from H5Z_XFORM_DO_OP3 is because compilers don't accept
 * operations like *x or /x.  So in H5Z__do_op, these two macros are called in
 * different ways. (SLU 2012/3/20)
 */
#define H5Z_XFORM_DO_OP6(OP)                                                   \
  {                                                                            \
    if (!tree->lchild && (tree->rchild->type == H5Z_XFORM_INTEGER)) {          \
      ...
      H5MM_xfree(tree->rchild);                                                \
      tree->rchild = NULL;                                                     \
    } else if ((tree->lchild->type == H5Z_XFORM_INTEGER) &&                    \
               (tree->rchild->type == H5Z_XFORM_INTEGER)) {                    \
      tree->type = H5Z_XFORM_INTEGER;                                          \
      tree->value.int_val =                                                    \
          tree->lchild->value.int_val OP tree->rchild->value.int_val;          \
 ...
 *
 * Purpose:     Rollback the H5Z_token to the previous H5Z_token retrieved.
 *There should only need to be one level of rollback necessary for our grammar.
 *
...
static H5Z_token *H5Z__get_token(H5Z_token *current) {
  ...
  while (current->tok_begin[0] != '\0') {
    if (isspace(current->tok_begin[0])) {
       ...
       */
      if (current->tok_end[0] == '.' || current->tok_end[0] == 'e' ||
          current->tok_end[0] == 'E') {
    ...
    case H5Z_XFORM_PLUS:
      ...
      if (!new_node->rchild) {
        ...
        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL,
                    "Error parsing data transform expression");
      }
...
static H5Z_node *H5Z__parse_factor(H5Z_token *current,
  ...
  case H5Z_XFORM_SYMBOL:
    ...
    factor->value.dat_val =
        &(dat_val_pointers->ptr_dat_val[dat_val_pointers->num_ptrs]);
  ...
  case H5Z_XFORM_LPAREN:
    ...
    if (current->tok_type != H5Z_XFORM_RPAREN) {
      ...
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL,
                  "Syntax error in data transform expression");
    }
    break;
  ...
  case H5Z_XFORM_PLUS:
    ...
    if (new_node) {
      if (new_node->type != H5Z_XFORM_INTEGER &&
          new_node->type != H5Z_XFORM_FLOAT &&
...
static H5Z_node *H5Z__new_node(H5Z_token_type type) {
  ...
  if (NULL == (ret_value = (H5Z_node *)H5MM_calloc(sizeof(H5Z_node))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "Ran out of memory trying to allocate space for nodes in the "
                "parse tree");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Z_xform_eval(H5Z_data_xform_t *data_xform_prop, void *array,
  ...
  if ((array_type = H5Z__xform_find_type(buf_type)) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "Cannot perform data transform on this type.");

  /* After this point, we're assured that the type of the array is handled by
   * the eval code, so we no longer have to check for valid types
   */
  ...
  /* Otherwise, do the full data transform */
    ...
    /* If it's a quadratic transform, we have no choice but to store multiple
     * copies of the data */
    else {
      for (i = 0; i < data_xform_prop->dat_val_pointers->num_ptrs; i++) {
        if (NULL == (data_xform_prop->dat_val_pointers->ptr_dat_val[i] =
                         (void *)H5MM_malloc(
                             array_size *
                             H5T_get_size((H5T_t *)H5I_object(array_type)))))
          HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                      "Ran out of memory trying to allocate space for data in "
                      "data transform");
    ...
    if (H5Z__xform_eval_full(tree, array_size, array_type, &res) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "error while performing data transform");

    if (data_xform_prop->dat_val_pointers->num_ptrs > 1)
      H5MM_memcpy(array, res.value.dat_val,
                  array_size * H5T_get_size((H5T_t *)H5I_object(array_type)));
  ...
  if (ret_value < 0) {
    /* If we ran out of memory above copying the array for temp storage (which
     * we easily can for polynomial transforms of high order) we free those
     * arrays which we already allocated */
    if (data_xform_prop->dat_val_pointers->num_ptrs > 1)
      for (i = 0; i < data_xform_prop->dat_val_pointers->num_ptrs; i++)
 ...
 *
 * Notes:       In the case of a polynomial data transform (ie, the left and
 *right subtree are both of type H5Z_XFORM_SYMBOL), the convention is that the
 *left hand side will accumulate changes and, at the end, the new data will be
 *copied from the lhs.
 *
 * Return:      Nothing
 *
 *-------------------------------------------------------------------------
 */
static herr_t H5Z__xform_eval_full(H5Z_node *tree, const size_t array_size,
                                   const hid_t array_type, H5Z_result *res) {
  ...
  if (tree->type == H5Z_XFORM_INTEGER) {
    ...
    /*since dat_val stores the address of the array which is really stored in
     * the dat_val_pointers, here we make dat_val store a pointer to the array
     * itself instead of the address of it so that the rest of the code below
     * works normally. */
    res->value.dat_val = *((void **)(tree->value.dat_val));
  } /* end if */
  else {
    if (tree->lchild &&
        H5Z__xform_eval_full(tree->lchild, array_size, array_type, &resl) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "error while performing data transform");
     ...
     * 1.  See if "x" is on left hand side, right hand side, or if both sides
     * are "x"
    ...
    /* So, if the left hand side got its data modified, the result stores a
     * pointers to the left hand side's data, ditto for rhs */
    if (resl.type == H5Z_XFORM_SYMBOL)
...
static void *H5Z__xform_copy_tree(H5Z_node *tree,
  ...
  if (tree->type == H5Z_XFORM_INTEGER) {
    ...
    if ((ret_value = (H5Z_node *)H5MM_malloc(sizeof(H5Z_node))) == NULL)
      ...
      ret_value->value.dat_val =
          &(new_dat_val_pointers->ptr_dat_val[new_dat_val_pointers->num_ptrs]);
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL,
                "Error in parse tree while trying to copy");
...
static hbool_t H5Z__op_is_numbs(H5Z_node *_tree) {
  ...
  if (((_tree->lchild->type == H5Z_XFORM_INTEGER) ||
       (_tree->lchild->type == H5Z_XFORM_FLOAT)) &&
      ((_tree->rchild->type == H5Z_XFORM_INTEGER) ||
       (_tree->rchild->type == H5Z_XFORM_FLOAT)))
    ret_value = TRUE;
...
static hbool_t H5Z__op_is_numbs2(H5Z_node *_tree) {
  ...
  if ((!_tree->lchild && ((_tree->rchild->type == H5Z_XFORM_INTEGER) ||
                          (_tree->rchild->type == H5Z_XFORM_FLOAT))) ||
      ((_tree->lchild && ((_tree->lchild->type == H5Z_XFORM_INTEGER) ||
                          (_tree->lchild->type == H5Z_XFORM_FLOAT))) &&
       (_tree->rchild && ((_tree->rchild->type == H5Z_XFORM_INTEGER) ||
                          (_tree->rchild->type == H5Z_XFORM_FLOAT)))))
    ret_value = TRUE;
...
H5Z_data_xform_t *H5Z_xform_create(const char *expr) {
  ...
  if (NULL == (data_xform_prop =
                   (H5Z_data_xform_t *)H5MM_calloc(sizeof(H5Z_data_xform_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "unable to allocate memory for data transform info");

  if (NULL == (data_xform_prop->dat_val_pointers =
                   (H5Z_datval_ptrs *)H5MM_malloc(sizeof(H5Z_datval_ptrs))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
  ...
  /* Find the number of times "x" is used in this equation, and allocate room
   * for storing that many points A more sophisticated check is needed to
   ...
   * stored */
  if (count > 0)
    if (NULL == (data_xform_prop->dat_val_pointers->ptr_dat_val =
                     (void **)H5MM_calloc(count * sizeof(void *))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                  "unable to allocate memory for pointers in transform array");

  /* Initialize the num_ptrs field, which will be used to keep track of the
   * number of copies of the data we have for polynomial transforms */
  data_xform_prop->dat_val_pointers->num_ptrs = 0;

  /* we generate the parse tree right here and store a pointer to its root in
   * the property. */
  if ((data_xform_prop->parse_root = (H5Z_node *)H5Z__xform_parse(
           expr, data_xform_prop->dat_val_pointers)) == NULL)
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "unable to generate parse tree from expression");

  /* Sanity check
   * count should be the same num_ptrs */
  if (count != data_xform_prop->dat_val_pointers->num_ptrs)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                "error copying the parse tree, did not find correct number of "
                "\"variables\"");
...
herr_t H5Z_xform_copy(H5Z_data_xform_t **data_xform_prop) {
  ...
  if (*data_xform_prop) {
    /* Allocate new node */
    if (NULL == (new_data_xform_prop =
                     (H5Z_data_xform_t *)H5MM_calloc(sizeof(H5Z_data_xform_t))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "unable to allocate memory for data transform info");

    /* Copy string */
    if (NULL == (new_data_xform_prop->xform_exp =
                     (char *)H5MM_xstrdup((*data_xform_prop)->xform_exp)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "unable to allocate memory for data transform expression");

    if (NULL == (new_data_xform_prop->dat_val_pointers =
                     (H5Z_datval_ptrs *)H5MM_malloc(sizeof(H5Z_datval_ptrs))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "unable to allocate memory for data transform array storage");

    /* Find the number of times "x" is used in this equation, and allocate room
     * for storing that many points */
    ...
    /* Zero out num_pointers prior to H5Z_xform_cop_tree call; that call will
     * increment it to the right amount */
    new_data_xform_prop->dat_val_pointers->num_ptrs = 0;

    /* Copy parse tree */
    if ((new_data_xform_prop->parse_root = (H5Z_node *)H5Z__xform_copy_tree(
             (*data_xform_prop)->parse_root,
             (*data_xform_prop)->dat_val_pointers,
    ...
    if (count != new_data_xform_prop->dat_val_pointers->num_ptrs)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                  "error copying the parse tree, did not find correct number "
                  "of \"variables\"");
...
hbool_t H5Z_xform_noop(const H5Z_data_xform_t *data_xform_prop) {
  ...
  if (data_xform_prop) {
    ...
    if ((HDstrlen(data_xform_prop->xform_exp) == 1) &&
        data_xform_prop->dat_val_pointers &&
