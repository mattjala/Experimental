...
#define H5O_MIN_SIZE                                                           \
  22 /* Min. obj header data size (must be big enough for a message prefix and \
...
#define H5O_ALIGN_F(F, X)                                                      \
  H5O_ALIGN_VERS(                                                              \
      MAX(H5O_VERSION_1, (uint8_t)H5O_obj_ver_bounds[H5F_LOW_BOUND(F)]), X)
...
#define H5O_SIZEOF_MSGHDR_OH(O)                                                \
  (unsigned)H5O_SIZEOF_MSGHDR_VERS((O)->version,                               \
                                   (O)->flags &H5O_HDR_ATTR_CRT_ORDER_TRACKED)
#define H5O_SIZEOF_MSGHDR_F(F, C)                                              \
  (unsigned)H5O_SIZEOF_MSGHDR_VERS(                                            \
      MAX((H5F_STORE_MSG_CRT_IDX(F) ? H5O_VERSION_LATEST : H5O_VERSION_1),     \
...
#define H5O_LOAD_NATIVE(F, IOF, OH, MSG, ERR)                                  \
  if (NULL == (MSG)->native) {                                                 \
    ...
    if (NULL ==                                                                \
        ((MSG)->native = (msg_type->decode)((F), (OH), (MSG)->flags, &ioflags, \
    ...
    if (msg_type->set_crt_index) {                                             \
      ...
      if ((msg_type->set_crt_index)((MSG)->native, (MSG)->crt_idx) < 0)        \
        HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, ERR,                                \
                    "unable to set creation index");                           \
    } /* end if */                                                             \
  }   /* end if */
...
struct H5O_msg_class_t {
  ...
  void *(*decode)(H5F_t *, H5O_t *, unsigned, unsigned *, size_t,
                  const uint8_t *);
  herr_t (*encode)(H5F_t *, hbool_t, uint8_t *, const void *);
  void *(*copy)(const void *, void *); /*copy native value         */
  size_t (*raw_size)(const H5F_t *, hbool_t,
                     const void *); /*sizeof encoded message	*/
  herr_t (*reset)(void *);          /*free nested data structs  */
  herr_t (*free)(void *);           /*free main data struct  */
  herr_t (*del)(H5F_t *, H5O_t *,
                void *); /* Delete space in file referenced by this message */
  herr_t (*link)(
      H5F_t *, H5O_t *,
      void *); /* Increment any links in file reference by this message */
  herr_t (*set_share)(void *,
                      const H5O_shared_t *); /* Set shared information */
  htri_t (*can_share)(const void *); /* Is message allowed to be shared? */
  herr_t (*pre_copy_file)(
      H5F_t *, const void *, hbool_t *, const H5O_copy_t *,
      void *); /*"pre copy" action when copying native value to file */
  void *(*copy_file)(H5F_t *, void *, H5F_t *, hbool_t *, unsigned *,
                     H5O_copy_t *, void *); /*copy native value to file */
  herr_t (*post_copy_file)(
      const H5O_loc_t *, const void *, H5O_loc_t *, void *, unsigned *,
      H5O_copy_t *); /*"post copy" action when copying native value to file */
  herr_t (*get_crt_index)(
      const void *, H5O_msg_crt_idx_t *); /* Get message's creation index */
  herr_t (*set_crt_index)(void *,
                          H5O_msg_crt_idx_t); /* Set message's creation index */
...
typedef struct H5O_msg_alloc_info_t {
  ...
  size_t
      gap_size; /* Size of any "gap" in the chunk immediately after message */
  size_t null_size;  /* Size of any null message in the chunk immediately after
                        message */
  size_t total_size; /* Total size of "available" space around message */
  unsigned
      null_msgno; /* Message index of null message immediately after message */
...
struct H5O_t {
  H5AC_info_t
      cache_info; /* Information for metadata cache functions, _must_ be */
  ...
  hbool_t store_bad_mesg_count; /* Flag to indicate that a bad message count
                                   should be stored */
...
typedef struct H5O_obj_class_t {
  ...
  const char *name; /*for debugging		     */
  void *(*get_copy_file_udata)(
      void); /*retrieve user data for 'copy file' operation */
  void (*free_copy_file_udata)(
      void *);                  /*free user data for 'copy file' operation */
  htri_t (*isa)(const H5O_t *); /*if a header matches an object class */
  void *(*open)(const H5G_loc_t *,
                H5I_type_t *); /*open an object of this class */
  void *(*create)(H5F_t *, void *,
                  H5G_loc_t *); /*create an object of this class */
  H5O_loc_t *(*get_oloc)(
      hid_t); /*get the object header location for an object */
  herr_t (*bh_info)(
      const H5O_loc_t *loc, H5O_t *oh,
      H5_ih_info_t *bh_info);     /*get the index & heap info for an object */
  herr_t (*flush)(void *obj_ptr); /*flush an opened object of this class */
} H5O_obj_class_t;

/* Node in skip list to map addresses from one file to another during object
 * header copy */
typedef struct H5O_addr_map_t {
  ...
  hbool_t
      is_locked; /* Indicate that the destination object is locked currently */
...
typedef struct H5O_common_cache_ud_t {
  ...
  H5O_cont_msgs_t
      *cont_msg_info; /* Pointer to continuation messages to work on */
...
typedef struct H5O_cache_ud_t {
  hbool_t
      made_attempt; /* Whether the deserialize routine was already attempted */
...
H5_DLL void *H5O__open_by_addr(const H5G_loc_t *loc, haddr_t addr,
...
H5_DLL void *H5O__open_by_idx(const H5G_loc_t *loc, const char *name,
                              ...
                              hsize_t n, H5I_type_t *opened_type /*out*/);
...
H5_DLL herr_t H5O__visit(H5G_loc_t *loc, const char *obj_name,
                         H5_index_t idx_type, H5_iter_order_t order,
...
H5_DLL herr_t H5O__msg_alloc(H5F_t *f, H5O_t *oh, const H5O_msg_class_t *type,
                             unsigned *mesg_flags, void *mesg,
                             size_t *mesg_idx);
H5_DLL herr_t H5O__msg_append_real(H5F_t *f, H5O_t *oh,
                                   const H5O_msg_class_t *type,
                                   unsigned mesg_flags, unsigned update_flags,
                                   void *mesg);
H5_DLL herr_t H5O__msg_write_real(H5F_t *f, H5O_t *oh,
                                  const H5O_msg_class_t *type,
                                  unsigned mesg_flags, unsigned update_flags,
                                  void *mesg);
...
H5_DLL unsigned H5O__msg_count_real(const H5O_t *oh,
                                    const H5O_msg_class_t *type);
H5_DLL herr_t H5O__msg_remove_real(H5F_t *f, H5O_t *oh,
                                   const H5O_msg_class_t *type, int sequence,
                                   H5O_operator_t op, void *op_data,
                                   hbool_t adj_link);
H5_DLL void *H5O__msg_copy_file(const H5O_msg_class_t *type, H5F_t *file_src,
                                void *mesg_src, H5F_t *file_dst,
                                hbool_t *recompute_size, unsigned *mesg_flags,
                                H5O_copy_t *cpy_info, void *udata);
H5_DLL herr_t H5O__msg_iterate_real(H5F_t *f, H5O_t *oh,
                                    const H5O_msg_class_t *type,
...
H5_DLL herr_t H5O__chunk_add(H5F_t *f, H5O_t *oh, unsigned idx,
...
H5_DLL herr_t H5O__are_mdc_flushes_disabled(const H5O_loc_t *oloc,
...
H5_DLL void *H5O__shared_decode(H5F_t *f, H5O_t *open_oh, unsigned *ioflags,
                                const uint8_t *buf,
                                const H5O_msg_class_t *type);
H5_DLL herr_t H5O__shared_encode(const H5F_t *f, uint8_t *buf /*out*/,
...
H5_DLL herr_t H5O__shared_copy_file(H5F_t *file_src, H5F_t *file_dst,
                                    const H5O_msg_class_t *mesg_type,
                                    const void *_native_src, void *_native_dst,
                                    hbool_t *recompute_size,
                                    unsigned *mesg_flags, H5O_copy_t *cpy_info,
                                    void *udata);
H5_DLL herr_t H5O__shared_post_copy_file(
    H5F_t *f, const H5O_msg_class_t *mesg_type, const H5O_shared_t *shared_src,
 ...
 */
/* Layout/Datatype/Dataspace arrays of versions are in H5Dpkg.h, H5Tpkg.h and
 * H5Spkg.h */
...
H5_DLL herr_t H5O__attr_dense_info_test(hid_t oid, hsize_t *name_count,
...
H5_DLL herr_t H5O__msg_get_chunkno_test(hid_t oid, unsigned msg_type,
                                        unsigned *chunk_num);
...
#endif /* H5O_DEBUG */
H5_DLL herr_t H5O__debug_real(H5F_t *f, H5O_t *oh, haddr_t addr, FILE *stream,
                              int indent, int fwidth);
