...
static herr_t H5HF__huge_op_real(H5HF_hdr_t *hdr, const uint8_t *id,
                                 hbool_t is_read, H5HF_operator_t op,
                                 void *op_data);
    ...
    if (hdr->filter_len > 0) {
      bt2_cparam.rrec_size =
          (uint32_t)((unsigned)hdr->sizeof_addr   /* Address of object */
                     ...
                     + (unsigned)hdr->sizeof_size); /* Size of de-filtered
                                                       object in memory */
    ...
    if (hdr->filter_len > 0) {
      bt2_cparam.rrec_size =
          (uint32_t)((unsigned)hdr->sizeof_addr /* Address of filtered object */
                     ...
                     + (unsigned)4 /* Filter mask for filtered object */
                     + (unsigned)hdr->sizeof_size /* Size of de-filtered object
                                                     in memory */
...
herr_t H5HF__huge_init(H5HF_hdr_t *hdr) {
   ...
   */
  if (hdr->filter_len > 0) {
    if ((hdr->id_len - 1) >= (unsigned)(hdr->sizeof_addr + hdr->sizeof_size +
                                        4 + hdr->sizeof_size)) {
      ...
      hdr->huge_id_size =
          (uint8_t)(hdr->sizeof_addr + hdr->sizeof_size + hdr->sizeof_size);
    } /* end if */
    else
...
static hsize_t H5HF__huge_new_id(H5HF_hdr_t *hdr) {
  ...
  if (hdr->huge_ids_wrapped)
    /* Fail for now - eventually should iterate through v2 B-tree, looking for
     * available ID */
    HGOTO_ERROR(H5E_HEAP, H5E_UNSUPPORTED, 0,
                "wrapping 'huge' object IDs not supported yet");
  else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__huge_insert(H5HF_hdr_t *hdr, size_t obj_size, void *obj,
                         void *_id) {
  ...
  unsigned filter_mask =
      0; /* Filter mask for object (only used for filtered objects) */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Check if the v2 B-tree for tracking 'huge' heap objects has been created
   * yet */
  if (!H5_addr_defined(hdr->huge_bt2_addr)) {
    ...
    if (NULL == hdr->huge_bt2) {
      ...
      if (NULL ==
          (hdr->huge_bt2 = H5B2_open(hdr->f, hdr->huge_bt2_addr, hdr->f)))
        HGOTO_ERROR(
            H5E_HEAP, H5E_CANTOPENOBJ, FAIL,
  ...
  if (hdr->filter_len > 0) {
    ...
    nbytes = write_size;
    if (H5Z_pipeline(&(hdr->pline), 0, &filter_mask, H5Z_NO_EDC, filter_cb,
  ...
  if (HADDR_UNDEF == (obj_addr = H5MF_alloc(hdr->f, H5FD_MEM_FHEAP_HUGE_OBJ,
                                            (hsize_t)write_size)))
    HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, FAIL,
                "file allocation failed for fractal heap huge object");

  /* Write the object's data to disk */
  if (H5F_block_write(hdr->f, H5FD_MEM_FHEAP_HUGE_OBJ, obj_addr, write_size,
                      write_buf) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_WRITEERROR, FAIL,
                "writing 'huge' object to file failed");
  ...
  if (hdr->huge_ids_direct) {
    ...
    H5HF_huge_bt2_filt_indir_rec_t
        filt_indir_rec; /* Record for tracking filtered object */
    H5HF_huge_bt2_indir_rec_t
        indir_rec;  /* Record for tracking non-filtered object */
    ...
    if (0 == (new_id = H5HF__huge_new_id(hdr)))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                  "can't generate new ID for object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__huge_get_obj_len(H5HF_hdr_t *hdr, const uint8_t *id,
                              size_t *obj_len_p) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (hdr->huge_ids_direct) {
    ...
    if (hdr->filter_len > 0) {
      H5HF_huge_bt2_filt_indir_rec_t
          found_rec; /* Record found from tracking object */
      H5HF_huge_bt2_filt_indir_rec_t
          search_rec; /* Record for searching for object */
      ...
      if (H5B2_find(hdr->huge_bt2, &search_rec, &found,
                    H5HF__huge_bt2_filt_indir_found, &found_rec) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTFIND, FAIL,
                    "can't check for object in v2 B-tree");
      if (!found)
        HGOTO_ERROR(H5E_HEAP, H5E_NOTFOUND, FAIL,
                    "can't find object in v2 B-tree");
      ...
      *obj_len_p = (size_t)found_rec.obj_size;
      ...
      /* Look up object in v2 B-tree */
      if (H5B2_find(hdr->huge_bt2, &search_rec, &found,
                    H5HF__huge_bt2_indir_found, &found_rec) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTFIND, FAIL,
                    "can't check for object in v2 B-tree");
      if (!found)
        HGOTO_ERROR(H5E_HEAP, H5E_NOTFOUND, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__huge_get_obj_off(H5HF_hdr_t *hdr, const uint8_t *id,
                              hsize_t *obj_off_p) {
...
static herr_t H5HF__huge_op_real(H5HF_hdr_t *hdr, const uint8_t *id,
  ...
  if (H5F_block_read(hdr->f, H5FD_MEM_FHEAP_HUGE_OBJ, obj_addr,
                     (size_t)obj_size, read_buf) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_READERROR, FAIL,
                "can't read 'huge' object's data from the file");
  ...
  if (hdr->filter_len > 0) {
    ...
    if (H5Z_pipeline(&(hdr->pline), H5Z_FLAG_REVERSE, &filter_mask, H5Z_NO_EDC,
  ...
  if (is_read) {
    ...
    /* (if there's no filters, the object was read directly into the user's
     * buffer) */
    if (hdr->filter_len > 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__huge_op(H5HF_hdr_t *hdr, const uint8_t *id, H5HF_operator_t op,
                     void *op_data) {
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5HF__huge_remove(H5HF_hdr_t *hdr, const uint8_t *id) {
  H5HF_huge_remove_ud_t
      udata;                  /* User callback data for v2 B-tree remove call */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (hdr->huge_ids_direct) {
    if (hdr->filter_len > 0) {
      ...
      if (H5B2_remove(hdr->huge_bt2, &search_rec,
                      H5HF__huge_bt2_filt_dir_remove, &udata) < 0)
      ...
      /* Remove the record for tracking the 'huge' object from the v2 B-tree */
      /* (space in the file for the object is freed in the 'remove' callback) */
      if (H5B2_remove(hdr->huge_bt2, &search_rec, H5HF__huge_bt2_dir_remove,
                      &udata) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTREMOVE, FAIL,
                    "can't remove object from B-tree");
    } /* end else */
    ...
    if (hdr->filter_len > 0) {
      ...
      /* (space in the file for the object is freed in the 'remove' callback) */
      if (H5B2_remove(hdr->huge_bt2, &search_rec,
                      H5HF__huge_bt2_filt_indir_remove, &udata) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTREMOVE, FAIL,
                    "can't remove object from B-tree");
    } /* end if */
    else {
      ...
      /* Remove the record for tracking the 'huge' object from the v2 B-tree */
      /* (space in the file for the object is freed in the 'remove' callback) */
      if (H5B2_remove(hdr->huge_bt2, &search_rec, H5HF__huge_bt2_indir_remove,
                      &udata) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTREMOVE, FAIL,
                    "can't remove object from B-tree");
    } /* end else */
