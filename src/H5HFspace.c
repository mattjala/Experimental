...
herr_t H5HF__space_start(H5HF_hdr_t *hdr, hbool_t may_create) {
  const H5FS_section_class_t *classes[] = {
      /* Free space section classes implemented for fractal heap */
  ...
  if (H5_addr_defined(hdr->fs_addr)) {
    ...
    if (NULL ==
        (hdr->fspace = H5FS_open(hdr->f, hdr->fs_addr, NELMTS(classes), classes,
                                 hdr, (hsize_t)H5HF_FSPACE_THRHD_DEF,
    ...
    if (may_create) {
      ...
      if (NULL == (hdr->fspace = H5FS_create(hdr->f, &hdr->fs_addr, &fs_create,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__space_add(H5HF_hdr_t *hdr, H5HF_free_section_t *node,
                       unsigned flags) {
  ...
  if (H5FS_sect_add(hdr->f, hdr->fspace, (H5FS_section_info_t *)node, flags,
                    &udata) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINSERT, FAIL,
                "can't add section to heap free space");
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5HF__space_find(H5HF_hdr_t *hdr, hsize_t request,
  ...
  if (hdr->fspace)
    if ((node_found = H5FS_sect_find(hdr->f, hdr->fspace, request,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__space_revert_root_cb(H5FS_section_info_t *_sect,
...
herr_t H5HF__space_revert_root(const H5HF_hdr_t *hdr) {
  ...
  if (hdr->fspace)
    /* Iterate over all sections, resetting the parent pointers in 'single'
     * sections */
    if (H5FS_sect_iterate(hdr->f, hdr->fspace, H5HF__space_revert_root_cb,
                          NULL) < 0)
 ...
 *		is changing from having a root direct block to an indirect
 *block.
 *
 * Return:	Success:	non-negative
 *		Failure:	negative
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__space_create_root_cb(H5FS_section_info_t *_sect,
                                         void *_udata) {
  H5HF_free_section_t *sect =
      (H5HF_free_section_t *)_sect; /* Section to dump info */
  H5HF_indirect_t *root_iblock =
      (H5HF_indirect_t *)_udata; /* User data for callback */
  herr_t ret_value = SUCCEED;    /* Return value */
...
herr_t H5HF__space_create_root(const H5HF_hdr_t *hdr,
  ...
  /* Only need to scan the sections if the free space has been initialized */
  if (hdr->fspace)
    /* Iterate over all sections, setting the parent pointers in 'single'
     * sections to the new indirect block */
    if (H5FS_sect_iterate(hdr->f, hdr->fspace, H5HF__space_create_root_cb,
                          root_iblock) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_BADITER, FAIL,
                  "can't iterate over sections to set parent pointers");
  ...
  if (hdr->fspace) {
    if (H5FS_size(hdr->fspace, fs_size) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTGET, FAIL,
                  "can't retrieve FS meta storage info");
  } /* end if */
  else
    *fs_size = 0;
...
herr_t H5HF__space_close(H5HF_hdr_t *hdr) {
  ...
  if (hdr->fspace) {
    ...
    if (H5FS_sect_stats(hdr->fspace, NULL, &nsects) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCOUNT, FAIL,
                  "can't query free space section count");
    ...
    if (!nsects) {
      if (H5FS_delete(hdr->f, hdr->fs_addr) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTDELETE, FAIL,
                    "can't delete free space info");
...
herr_t H5HF__space_delete(H5HF_hdr_t *hdr) {
  ...
  if (H5FS_delete(hdr->f, hdr->fs_addr) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                "can't delete to free space manager");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__space_sect_change_class(H5HF_hdr_t *hdr, H5HF_free_section_t *sect,
                                     uint16_t new_class) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5FS_sect_change_class(hdr->f, hdr->fspace, (H5FS_section_info_t *)sect,
                             new_class) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTMODIFY, FAIL,
                "can't modify class of free space section");
