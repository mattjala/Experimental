 ...
 * H5HF_op function when copying an attribute stored in densely stored
 * attributes. (or the shared message heap)
 */
typedef struct {
  /* downward (internal) */
  H5F_t *f; /* Pointer to file that fractal heap is in */
...
herr_t H5A__dense_create(H5F_t *f, H5O_ainfo_t *ainfo) {
  ...
  if (H5HF_get_heap_addr(fheap, &ainfo->fheap_addr) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGETSIZE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5A__dense_fnd_cb(const H5A_t *attr, hbool_t *took_ownership,
                                void *_user_attr) {
  const H5A_t **user_attr = (const H5A_t **)
      _user_attr;             /* User data from v2 B-tree attribute lookup */
  herr_t ret_value = SUCCEED; /* Return value */
...
H5A_t *H5A__dense_open(H5F_t *f, const H5O_ainfo_t *ainfo, const char *name) {
  ...
  H5HF_t *fheap = NULL;      /* Fractal heap handle */
  H5HF_t *shared_fheap =
      NULL;                /* Fractal heap handle for shared header messages */
  ...
  if ((attr_sharable = H5SM_type_shared(f, H5O_ATTR_ID)) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, NULL,
                "can't determine if attributes are shared");
  ...
  if (attr_sharable) {
    ...
    if (H5SM_get_fheap_addr(f, H5O_ATTR_ID, &shared_fheap_addr) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, NULL,
                  "can't get shared message heap address");
  ...
  if (H5B2_find(bt2_name, &udata, &attr_exists, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_NOTFOUND, NULL,
                "can't search for attribute in name index");
  if (attr_exists == FALSE)
    HGOTO_ERROR(H5E_ATTR, H5E_NOTFOUND, NULL,
                "can't locate attribute in name index");
  ...
  if (fheap && H5HF_close(fheap) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, NULL, "can't close fractal heap");
  if (bt2_name && H5B2_close(bt2_name) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, NULL,
                "can't close v2 B-tree for name index");
...
herr_t H5A__dense_insert(H5F_t *f, const H5O_ainfo_t *ainfo, H5A_t *attr) {
  ...
  if (mesg_flags & H5O_MSG_FLAG_SHARED) {
    ...
    if (H5O_msg_encode(f, H5O_ATTR_ID, FALSE, (unsigned char *)attr_ptr, attr) <
        0)
    ...
    if (H5HF_insert(fheap, attr_size, attr_ptr, &udata.id) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTINSERT, FAIL,
                  "unable to insert attribute into fractal heap");
  } /* end else */
  ...
  udata.common.name_hash =
      H5_checksum_lookup3(attr->shared->name, HDstrlen(attr->shared->name), 0);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5A__dense_write_bt2_cb2(void *_record, void *_op_data,
                                       hbool_t *changed) {
  H5A_dense_bt2_corder_rec_t *record =
      (H5A_dense_bt2_corder_rec_t *)_record; /* Record from B-tree */
  H5O_fheap_id_t *new_heap_id =
      (H5O_fheap_id_t *)_op_data; /* "op data" from v2 B-tree modify */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5A__dense_write_bt2_cb(void *_record, void *_op_data,
                                      hbool_t *changed) {
  H5A_dense_bt2_name_rec_t *record =
      (H5A_dense_bt2_name_rec_t *)_record; /* Record from B-tree */
  H5A_bt2_od_wrt_t *op_data =
      (H5A_bt2_od_wrt_t *)_op_data; /* "op data" from v2 B-tree modify */
  H5B2_t *bt2_corder = NULL; /* v2 B-tree handle for creation order index */
  H5WB_t *wb = NULL;         /* Wrapped buffer for attribute data */
  ...
  if (record->flags & H5O_MSG_FLAG_SHARED) {
    ...
    if (H5_addr_defined(op_data->corder_bt2_addr)) {
      ...
      /* Open the creation order index v2 B-tree */
      if (NULL ==
          (bt2_corder = H5B2_open(op_data->f, op_data->corder_bt2_addr, NULL)))
        HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, FAIL,
                    "unable to open v2 B-tree for creation order index");
      ...
      if (H5B2_modify(bt2_corder, &udata, H5A__dense_write_bt2_cb2,
                      &op_data->attr->sh_loc.u.heap_id) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTINSERT, FAIL,
                    "unable to modify record in v2 B-tree");
    } /* end if */
    ...
    *changed = TRUE;
    ...
    if ((attr_size = H5O_msg_raw_size(op_data->f, H5O_ATTR_ID, FALSE,
                                      op_data->attr)) == 0)
    ...
    if (H5O_msg_encode(op_data->f, H5O_ATTR_ID, FALSE,
                       (unsigned char *)attr_ptr, op_data->attr) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTENCODE, FAIL, "can't encode attribute");
...
/* Sanity check */
    ...
    /* (might be more efficient as fractal heap 'op' callback, but leave that
     * for later -QAK) */
    if (H5HF_write(op_data->fheap, &record->id, changed, attr_ptr) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTUPDATE, FAIL,
                  "unable to update attribute in heap");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5A__dense_copy_fh_cb(const void *obj, size_t obj_len,
                                    void *_udata) {
  H5A_fh_ud_cp_t *udata =
      (H5A_fh_ud_cp_t *)_udata; /* User data for fractal heap 'op' callback */
  herr_t ret_value = SUCCEED;   /* Return value */
   ...
   */
  if (NULL == (udata->attr =
                   (H5A_t *)H5O_msg_decode(udata->f, NULL, H5O_ATTR_ID, obj_len,
  ...
  if (udata->record->flags & H5O_MSG_FLAG_SHARED)
    H5SM_reconstitute(&(udata->attr->sh_loc), udata->f, H5O_ATTR_ID,
                      udata->record->id);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__dense_rename(H5F_t *f, const H5O_ainfo_t *ainfo,
  ...
  if (ainfo->index_corder) {
    ...
    if (NULL == (bt2_corder = H5B2_open(f, ainfo->corder_bt2_addr, NULL)))
      HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, FAIL,
                  "unable to open v2 B-tree for creation index");
    ...
    if (corder_attr_exists) {
      ...
      /* Set up the creation order in user data for the v2 B-tree 'record
       * remove' callback */
  ...
  if ((shared_mesg = H5O_msg_is_shared(H5O_ATTR_ID, attr_copy)) > 0) {
     ...
     *      be removed from the file.  (Essentially a "copy on write"
     * operation).
     *
     *      *ick* -QAK, 2007/01/08
     */
...
static int H5A__dense_iterate_bt2_cb(const void *_record, void *_bt2_udata) {
  ...
  H5A_bt2_ud_it_t *bt2_udata =
      (H5A_bt2_ud_it_t *)_bt2_udata; /* User data for callback */
  ...
  if (bt2_udata->skip > 0)
    ...
    if (H5HF_op(fheap, &record->id, H5A__dense_copy_fh_cb, &fh_udata) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTOPERATE, H5_ITER_ERROR,
    ...
    case H5A_ATTR_OP_APP2: {
      ...
      ret_value = (bt2_udata->attr_op->u.app_op2)(bt2_udata->loc_id,
                                                  fh_udata.attr->shared->name,
    ...
    case H5A_ATTR_OP_LIB:
      ...
      ret_value =
          (bt2_udata->attr_op->u.lib_op)(fh_udata.attr, bt2_udata->op_data);
      break;
 ...
 *
 * Purpose:     Iterate over attributes in dense storage structures for an
 *object
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
herr_t H5A__dense_iterate(H5F_t *f, hid_t loc_id, const H5O_ainfo_t *ainfo,
  ...
  if (order == H5_ITER_NATIVE && H5_addr_defined(bt2_addr)) {
    ...
    if (H5A__dense_build_table(f, ainfo, idx_type, order, &atable) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                  "error building table of attributes");
...
static herr_t H5A__dense_remove_bt2_cb(const void *_record, void *_udata) {
  ...
  H5A_bt2_ud_rm_t *udata =
      (H5A_bt2_ud_rm_t *)_udata; /* User data for callback */
  H5A_t *attr =
      *(H5A_t **)
           udata->common.found_op_data; /* Pointer to attribute to remove */
  H5B2_t *bt2_corder = NULL;  /* v2 B-tree handle for creation order index */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5_addr_defined(udata->corder_bt2_addr)) {
    /* Open the creation order index v2 B-tree */
    if (NULL ==
        (bt2_corder = H5B2_open(udata->common.f, udata->corder_bt2_addr, NULL)))
      HGOTO_ERROR(H5E_ATTR, H5E_CANTOPENOBJ, FAIL,
                  "unable to open v2 B-tree for creation order index");
  ...
  if (record->flags & H5O_MSG_FLAG_SHARED) {
    ...
    if (H5SM_delete(udata->common.f, NULL, &(attr->sh_loc)) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTFREE, FAIL,
                  "unable to delete shared attribute");
  } /* end if */
  else {
    ...
    if (H5HF_remove(udata->common.fheap, &record->id) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTREMOVE, FAIL,
                  "unable to remove attribute from fractal heap");
  } /* end else */
 ...
 *
 * Purpose:     v2 B-tree callback for dense attribute storage record removal by
 *index
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
static herr_t H5A__dense_remove_by_idx_bt2_cb(const void *_record,
                                              void *_bt2_udata) {
  H5HF_t *fheap;      /* Fractal heap handle */
  H5B2_t *bt2 = NULL; /* v2 B-tree handle for index */
  const H5A_dense_bt2_name_rec_t *record =
      (const H5A_dense_bt2_name_rec_t *)_record; /* v2 B-tree record */
  H5A_bt2_ud_rmbi_t *bt2_udata =
      (H5A_bt2_ud_rmbi_t *)_bt2_udata; /* User data for callback */
  H5A_fh_ud_cp_t fh_udata; /* User data for fractal heap 'op' callback */
  ...
  /* Check whether to make a copy of the attribute or just need the shared
   * location info */
  if (H5_addr_defined(bt2_udata->other_bt2_addr) ||
      !(record->flags & H5O_MSG_FLAG_SHARED)) {
    /* Call fractal heap 'op' routine, to make copy of attribute to remove */
    if (H5HF_op(fheap, &record->id, H5A__dense_copy_fh_cb, &fh_udata) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTOPERATE, FAIL,
                  "attribute removal callback failed");
  ...
  if (H5_addr_defined(bt2_udata->other_bt2_addr)) {
    ...
    if (bt2_udata->idx_type == H5_INDEX_NAME) {
      ...
      other_bt2_udata.name_hash =
          H5_checksum_lookup3(fh_udata.attr->shared->name,
                              HDstrlen(fh_udata.attr->shared->name), 0);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__dense_remove_by_idx(H5F_t *f, const H5O_ainfo_t *ainfo,
                                H5_index_t idx_type, H5_iter_order_t order,
                                hsize_t n) {
  H5HF_t *fheap = NULL; /* Fractal heap handle */
  H5HF_t *shared_fheap =
  ...
  if (H5_addr_defined(bt2_addr)) {
    ...
    udata.other_bt2_addr = idx_type == H5_INDEX_NAME ? ainfo->corder_bt2_addr
                                                     : ainfo->name_bt2_addr;

    /* Remove the record from the name index v2 B-tree */
    if (H5B2_remove_by_idx(bt2, order, n, H5A__dense_remove_by_idx_bt2_cb,
                           &udata) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTREMOVE, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5A__dense_exists(H5F_t *f, const H5O_ainfo_t *ainfo, const char *name,
                         hbool_t *attr_exists) {
  H5A_bt2_ud_common_t udata; /* User data for v2 B-tree modify */
  H5HF_t *fheap = NULL;      /* Fractal heap handle */
  H5HF_t *shared_fheap =
...
static herr_t H5A__dense_delete_bt2_cb(const void *_record, void *_bt2_udata) {
  const H5A_dense_bt2_name_rec_t *record =
      (const H5A_dense_bt2_name_rec_t *)_record; /* Record from B-tree */
  H5A_bt2_ud_common_t *bt2_udata =
      (H5A_bt2_ud_common_t *)_bt2_udata; /* User data for callback */
  ...
  if (record->flags & H5O_MSG_FLAG_SHARED) {
    ...
    /* Call fractal heap 'op' routine, to copy the attribute information */
    if (H5HF_op(bt2_udata->fheap, &record->id, H5A__dense_copy_fh_cb,
                &fh_udata) < 0)
...
herr_t H5A__dense_delete(H5F_t *f, H5O_ainfo_t *ainfo) {
  ...
  if (H5B2_delete(f, ainfo->name_bt2_addr, NULL, H5A__dense_delete_bt2_cb,
                  &udata) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTDELETE, FAIL,
