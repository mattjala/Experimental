...
static H5T_t *H5T__get_native_type(H5T_t *dt, H5T_direction_t direction,
                                   size_t *struct_align, size_t *offset,
                                   size_t *comp_size);
...
static H5T_t *H5T__get_native_bitfield(size_t prec, H5T_direction_t direction,
                                       size_t *struct_align, size_t *offset,
                                       size_t *comp_size);
static herr_t H5T__cmp_offset(size_t *comp_size, size_t *offset,
                              size_t elem_size, size_t nelems, size_t align,
                              size_t *struct_align);
...
hid_t H5Tget_native_type(hid_t type_id, H5T_direction_t direction) {
  ...
  if (direction != H5T_DIR_DEFAULT && direction != H5T_DIR_ASCEND &&
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                "not valid direction value");

  /* Get the native type */
  if (NULL ==
      (new_dt = H5T__get_native_type(dt, direction, NULL, NULL, &comp_size)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                "cannot retrieve native type");
 ...
 *-------------------------------------------------------------------------
 */
static H5T_t *H5T__get_native_type(H5T_t *dtype, H5T_direction_t direction,
                                   size_t *struct_align, size_t *offset,
                                   size_t *comp_size) {
  ...
  size_t *memb_offset = NULL;  /* List of member offsets in compound type,
                                  including member size and alignment */
  ...
  case H5T_INTEGER: {
    ...
    if (NULL == (ret_value = H5T__get_native_integer(
                     prec, sign, direction, struct_align, offset, comp_size)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "cannot retrieve integer type");
  } /* end case */
  break;

  case H5T_FLOAT:
    if (NULL == (ret_value = H5T__get_native_float(
                     size, direction, struct_align, offset, comp_size)))
  ...
  case H5T_STRING:
    ...
    if (H5T_IS_VL_STRING(dtype->shared)) {
      ...
      if (H5T__cmp_offset(comp_size, offset, sizeof(char *), (size_t)1,
                          H5T_POINTER_ALIGN_g, struct_align) < 0)
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                    "cannot compute compound offset");
    } /* end if */
    else {
      /* Update size, offset and compound alignment for parent. */
      if (H5T__cmp_offset(comp_size, offset, sizeof(char), size,
                          H5T_NATIVE_SCHAR_ALIGN_g, struct_align) < 0)
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                    "cannot compute compound offset");
    } /* end else */
    break;

  /* The time type will be supported in the future.  Simply return "not
   * supported" message for now.*/
  case H5T_TIME:
  ...
  case H5T_BITFIELD: {
    ...
    if (NULL == (ret_value = H5T__get_native_bitfield(
                     prec, direction, struct_align, offset, comp_size)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "cannot retrieve integer for bitfield type");
  } /* end case */
  break;
  ...
  case H5T_REFERENCE: {
    ...
    if (H5T__cmp_offset(comp_size, offset, ref_size, (size_t)1, align,
                        struct_align) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "cannot compute compound offset");
  } /* end case */
  ...
  case H5T_COMPOUND: {
    ...
    size_t children_st_align = 0; /* The max alignment among compound members.
                                     This'll be the compound alignment */

    if ((snmemb = H5T_get_nmembers(dtype)) <= 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "compound data type doesn't have any member");
    ...
    /* Construct child compound type and retrieve a list of their IDs, offsets,
     * total size, and alignment for compound type. */
    for (u = 0; u < nmemb; u++) {
      if (NULL == (memb_type = H5T_get_member_type(dtype, u)))
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                    "member type retrieval failed");

      if (NULL == (comp_mname[u] = H5T__get_member_name(dtype, u)))
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                    "member type retrieval failed");

      if (NULL == (memb_list[u] = H5T__get_native_type(
                       memb_type, direction, &children_st_align,
                       &(memb_offset[u]), &children_size)))
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                    "member identifier retrieval failed");
    ...
    for (u = 0; u < nmemb; u++)
      if (H5T__insert(new_type, comp_mname[u], memb_offset[u], memb_list[u]) <
          0)
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                    "cannot insert member to compound datatype");
     ...
     */
    if (H5T__cmp_offset(comp_size, offset, children_size, (size_t)1,
                        children_st_align, struct_align) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "cannot compute compound offset");
  ...
  case H5T_ENUM: {
    ...
    /* Don't need to do anything special for alignment, offset since the ENUM
     * type usually is integer.
     */

    /* Retrieve base type for enumerated type */
    if (NULL == (super_type = H5T_get_super(dtype)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "unable to get base type for enumerate type");
    if (NULL == (nat_super_type = H5T__get_native_type(
                     super_type, direction, struct_align, offset, comp_size)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "base native type retrieval failed");
    ...
    if ((nat_super_type_id =
             H5I_register(H5I_DATATYPE, nat_super_type, FALSE)) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "cannot register datatype");
    ...
    if ((snmemb = H5T_get_nmembers(dtype)) <= 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "enumerate data type doesn't have any member");
    H5_CHECKED_ASSIGN(nmemb, unsigned, snmemb, int);
    for (u = 0; u < nmemb; u++) {
      ...
      if (H5T_convert(tpath, super_type_id, nat_super_type_id, (size_t)1,
                      (size_t)0, (size_t)0, memb_value, NULL) < 0)
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL, "cannot get member value");
  ...
  case H5T_ARRAY: {
    ...
    if (NULL == (super_type = H5T_get_super(dtype)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "unable to get parent type for array type");
    if (NULL == (nat_super_type =
                     H5T__get_native_type(super_type, direction, &super_align,
                                          &super_offset, &super_size)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "parent native type retrieval failed");
    ...
    if (NULL ==
        (new_type = H5T__array_create(nat_super_type, array_rank, dims)))
    ...
    if (H5T__cmp_offset(comp_size, offset, super_size, (size_t)nelems,
                        super_align, struct_align) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "cannot compute compound offset");
  ...
  case H5T_VLEN: {
    ...
    if (NULL == (super_type = H5T_get_super(dtype)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "unable to get parent type for VL type");
    /* Don't need alignment, offset information if this VL isn't a field of
     * compound type.  If it is, go to a few steps below to compute the
     * information directly. */
    if (NULL == (nat_super_type = H5T__get_native_type(
                     super_type, direction, NULL, NULL, &super_size)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "parent native type retrieval failed");
    ...
    /* Update size, offset and compound alignment for parent compound type
     * directly. */
    vl_align = H5T_HVL_ALIGN_g;
    vl_size = sizeof(hvl_t);

    if (H5T__cmp_offset(comp_size, offset, vl_size, (size_t)1, vl_align,
                        struct_align) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                  "cannot compute compound offset");
  ...
  default:
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                "data type doesn't match any native type");
  } /* end switch */
  ...
  if (NULL == ret_value) {
    if (new_type)
      if (H5T_close_real(new_type) < 0)
        HDONE_ERROR(H5E_DATATYPE, H5E_CLOSEERROR, NULL,
                    "unable to release datatype");
...
static H5T_t *H5T__get_native_integer(size_t prec, H5T_sign_t sign,
  ...
  if (direction == H5T_DIR_DEFAULT || direction == H5T_DIR_ASCEND) {
    if (prec <= H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_SCHAR_g))) {
      ...
      native_size = sizeof(char);
               ...
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_SHORT_g))) {
      match = H5T_NATIVE_INT_MATCH_SHORT;
      native_size = sizeof(short);
    } else if (prec <=
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_INT_g))) {
      match = H5T_NATIVE_INT_MATCH_INT;
      native_size = sizeof(int);
    } else if (prec <=
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_LONG_g))) {
      match = H5T_NATIVE_INT_MATCH_LONG;
      native_size = sizeof(long);
    } else if (prec <=
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_LLONG_g))) {
      match = H5T_NATIVE_INT_MATCH_LLONG;
      native_size = sizeof(long long);
    } else { /* If no native type matches the querried datatype, simply choose
                the type of biggest size. */
      match = H5T_NATIVE_INT_MATCH_LLONG;
      native_size = sizeof(long long);
    }
    ...
    if (prec > H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_LONG_g))) {
      ...
      native_size = sizeof(int);
               ...
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_SCHAR_g))) {
      match = H5T_NATIVE_INT_MATCH_SHORT;
      native_size = sizeof(short);
    } else {
      match = H5T_NATIVE_INT_MATCH_CHAR;
  ...
  if (H5T__cmp_offset(comp_size, offset, native_size, (size_t)1, align,
                      struct_align) < 0)
...
static H5T_t *H5T__get_native_float(size_t size, H5T_direction_t direction,
  ...
  default:
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, NULL,
                "Unknown native floating-point match");
  } /* end switch */
  ...
  if (direction == H5T_DIR_DEFAULT || direction == H5T_DIR_ASCEND) {
    if (prec <= H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_B8_g))) {
      ...
      align = H5T_NATIVE_UINT8_ALIGN_g;
               ...
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_B16_g))) {
      tid = H5T_NATIVE_B16;
      native_size = 2;
      align = H5T_NATIVE_UINT16_ALIGN_g;
    } else if (prec <=
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_B32_g))) {
      tid = H5T_NATIVE_B32;
      native_size = 4;
      align = H5T_NATIVE_UINT32_ALIGN_g;
    } else if (prec <=
               H5T_get_precision((H5T_t *)H5I_object(H5T_NATIVE_B64_g))) {
      tid = H5T_NATIVE_B64;
      native_size = 8;
