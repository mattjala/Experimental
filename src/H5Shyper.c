...
/* Define alias for hsize_t, for allocating H5S_hyper_span_info_t + bounds
 * objects */
...
typedef struct {
  const H5S_hyper_span_t
      *ds_span[H5S_MAX_RANK];   /* Array of the current spans in the destination
                                   space in each dimension */
  hsize_t ds_low[H5S_MAX_RANK]; /* Array of current low bounds (of iteration)
                                   for each element in ds_span */
  H5S_hyper_span_info_t
      *ps_span_info[H5S_MAX_RANK]; /* Array of span info structs for projected
                                      space during iteration */
  uint32_t
      ps_clean_bitmap; /* Bitmap of whether the nth rank has a clean projected
                          space since the last time it was set to 1 */
  ...
  hbool_t share_selection; /* Whether span trees in dst_space can be shared with
                              proj_space */
...
static herr_t H5S__hyper_span_blocklist(const H5S_hyper_span_info_t *spans,
                                        ...
                                        hsize_t rank, hsize_t *startblock,
...
static herr_t H5S__hyper_append_span(H5S_hyper_span_info_t **span_tree,
                                     unsigned ndims, hsize_t low, hsize_t high,
                                     H5S_hyper_span_info_t *down);
...
static herr_t H5S__hyper_merge_spans(H5S_t *space,
                                     H5S_hyper_span_info_t *new_spans);
...
static herr_t H5S__hyper_add_disjoint_spans(H5S_t *space,
                                            H5S_hyper_span_info_t *new_spans);
...
H5S__set_regular_hyperslab(H5S_t *space, const hsize_t start[],
                           const hsize_t *app_stride, const hsize_t app_count[],
                           const hsize_t *app_block, const hsize_t *opt_stride,
                           const hsize_t opt_count[], const hsize_t *opt_block);
static herr_t H5S__fill_in_select(H5S_t *space1, H5S_seloper_t op,
                                  H5S_t *space2, H5S_t **result);
static H5S_t *H5S__combine_select(H5S_t *space1, H5S_seloper_t op,
                                  H5S_t *space2);
static herr_t H5S__hyper_iter_get_seq_list_gen(H5S_sel_iter_t *iter,
                                               size_t maxseq, size_t maxelem,
                                               size_t *nseq, size_t *nelem,
                                               hsize_t *off, size_t *len);
static herr_t H5S__hyper_iter_get_seq_list_opt(H5S_sel_iter_t *iter,
                                               size_t maxseq, size_t maxelem,
                                               size_t *nseq, size_t *nelem,
                                               hsize_t *off, size_t *len);
static herr_t H5S__hyper_iter_get_seq_list_single(H5S_sel_iter_t *iter,
                                                  size_t maxseq, size_t maxelem,
                                                  size_t *nseq, size_t *nelem,
                                                  hsize_t *off, size_t *len);
static herr_t
...
/* Selection callbacks */
static herr_t H5S__hyper_copy(H5S_t *dst, const H5S_t *src,
...
static herr_t H5S__hyper_deserialize(H5S_t **space, const uint8_t **p,
                                     const size_t p_size, hbool_t skip);
static herr_t H5S__hyper_bounds(const H5S_t *space, hsize_t *start,
                                hsize_t *end);
...
static htri_t H5S__hyper_intersect_block(H5S_t *space, const hsize_t *start,
                                         const hsize_t *end);
...
static herr_t H5S__hyper_project_simple(const H5S_t *space, H5S_t *new_space,
                                        hsize_t *offset);
static herr_t H5S__hyper_iter_init(H5S_t *space, H5S_sel_iter_t *iter);

/* Selection iteration callbacks */
static herr_t H5S__hyper_iter_coords(const H5S_sel_iter_t *iter,
                                     hsize_t *coords);
static herr_t H5S__hyper_iter_block(const H5S_sel_iter_t *iter, hsize_t *start,
                                    hsize_t *end);
...
/* Declare a free list to manage the H5S_hyper_span_info_t + hsize_t array
 * struct */
...
static herr_t H5S__hyper_print_spans_helper(FILE *f,
  ...
  while (span) {
    fprintf(f,
            "%s: %*sdepth=%u, span=%p, (%" PRIuHSIZE ", %" PRIuHSIZE
            "), next=%p\n",
            __func__, depth * 2, "", depth, (void *)span, span->low, span->high,
            (void *)span->next);
    if (span->down) {
      fprintf(f,
              "%s: %*sspans=%p, count=%u, bounds[0]={%" PRIuHSIZE
              ", %" PRIuHSIZE "}, head=%p\n",
              __func__, (depth + 1) * 2, "", (void *)span->down,
              span->down->count, span->down->low_bounds[0],
              span->down->high_bounds[0], (void *)span->down->head);
...
static herr_t H5S__hyper_print_spans(FILE *f,
                                     const H5S_hyper_span_info_t *span_lst) {
  FUNC_ENTER_PACKAGE_NOERR

  if (span_lst != NULL) {
    fprintf(f,
            "%s: spans=%p, count=%u, bounds[0]={%" PRIuHSIZE ", %" PRIuHSIZE
            "}, head=%p\n",
            __func__, (void *)span_lst, span_lst->count,
            span_lst->low_bounds[0], span_lst->high_bounds[0],
            (void *)span_lst->head);
...
static herr_t H5S__hyper_print_diminfo_helper(FILE *f, const char *field,
                                              unsigned ndims,
                                              const H5S_hyper_dim_t *dinfo) {
  ...
  if (dinfo != NULL) {
    ...
    for (u = 0; u < ndims; u++)
      fprintf(f, "%" PRIuHSIZE "%s", dinfo[u].start,
              (u < (ndims - 1) ? ", " : "]\n"));
    fprintf(f, "%s: %s: stride=[", __func__, field);
    for (u = 0; u < ndims; u++)
      fprintf(f, "%" PRIuHSIZE "%s", dinfo[u].stride,
              (u < (ndims - 1) ? ", " : "]\n"));
    fprintf(f, "%s: %s: count=[", __func__, field);
    for (u = 0; u < ndims; u++)
      fprintf(f, "%" PRIuHSIZE "%s", dinfo[u].count,
              (u < (ndims - 1) ? ", " : "]\n"));
    fprintf(f, "%s: %s: block=[", __func__, field);
    for (u = 0; u < ndims; u++)
      fprintf(f, "%" PRIuHSIZE "%s", dinfo[u].block,
              (u < (ndims - 1) ? ", " : "]\n"));
  } /* end if */
  else
    fprintf(f, "%s: %s==NULL\n", __func__, field);
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_print_spans_dfs(f, span_lst, depth)
        ...
        unsigned depth;                           IN: the level of this span
list RETURNS non-negative on success, negative on failure GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__hyper_print_spans_dfs(FILE *f,
                                         const H5S_hyper_span_info_t *span_lst,
  ...
  for (u = 0; u < depth; u++)
    fprintf(f, "\t");
  fprintf(f,
          "DIM[%u]: ref_count=%u, #elems=%u, head=%p, tail=%p, actual_tail=%p, "
          "matched=%d\n",
          depth, span_lst->count, num_elems, (void *)span_lst->head,
          (void *)span_lst->tail, (void *)actual_tail,
  ...
  while (cur_elem) {
    for (u = 0; u < depth; u++)
      fprintf(f, "\t");
    fprintf(f,
            "ELEM[%u]: ptr=%p, low=%" PRIuHSIZE ", high=%" PRIuHSIZE
            ", down=%p\n",
            elem_idx++, (void *)cur_elem, cur_elem->low, cur_elem->high,
            (void *)cur_elem->down);
    if (cur_elem->down)
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Output the span elements for one hyperslab selection space in depth-first
order USAGE herr_t H5S__hyper_print_space_dfs(f, space) FILE *f; IN: the file to
output const H5S_t *space;    IN: the selection space to output RETURNS
...
static herr_t H5S__hyper_print_space_dfs(FILE *f, const H5S_t *space) {
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    for (u = 0; u < dims - 1; u++)
      fprintf(f, "%" PRIuHSIZE ",",
              space->select.sel_info.hslab->span_lst->low_bounds[u]);
    fprintf(f, "%" PRIuHSIZE "]\n",
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    for (u = 0; u < dims - 1; u++)
      fprintf(f, "%" PRIuHSIZE ",",
              space->select.sel_info.hslab->span_lst->high_bounds[u]);
    fprintf(f, "%" PRIuHSIZE "]\n",
...
static herr_t H5S__hyper_iter_init(H5S_t *space, H5S_sel_iter_t *iter) {
  hsize_t *
      slab_size; /* Pointer to the dataspace dimensions to use for calc. slab */
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    const H5S_hyper_dim_t
        *tdiminfo; /* Temporary pointer to diminfo information */
    const hsize_t
        *mem_size; /* Temporary pointer to dataspace extent's dimension sizes */
    ...
    if (cont_dim > 0) {
      hbool_t last_dim_flattened =
          TRUE; /* Flag to indicate that the last dimension was flattened */
      unsigned flat_rank =
          rank - cont_dim; /* Number of dimensions after flattening */
      ...
      for (i = (int)rank - 1, acc = 1; i >= 0; i--) {
        if (tdiminfo[i].block == mem_size[i] && i > 0) {
          ...
          if (last_dim_flattened) {
            ...
            /* Reset the "last dim flattened" flag to avoid flattened any
             * further dimensions */
      ...
      /* Initialize "flattened" iterator offset to initial location and
       * dataspace extent and selection information to correct values */
    ...
    if ((iter->flags & H5S_SEL_ITER_API_CALL) &&
      ...
      /* Copy the span tree */
      if (NULL ==
          (iter->u.hyp.spans = H5S__hyper_copy_span(
               space->select.sel_info.hslab->span_lst, space->extent.rank)))
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL, "can't copy span tree");
    } /* end if */
    else {
      /* Share the source dataspace's span tree by incrementing the reference
       * count on it */
  ...
  if (!iter->u.hyp.diminfo_valid) {
    ...
    for (u = 0; u < rank; u++)
      ...
      iter->u.hyp.loc_off[u] =
          ((hsize_t)((hssize_t)iter->u.hyp.off[u] + iter->sel_off[u])) *
          iter->u.hyp.slab[u];
  } /* end if */
    ...
    if (iter->u.hyp.iter_rank != 0 && iter->u.hyp.iter_rank < iter->rank) {
      ...
      while (u >= 0) {
        if (iter->u.hyp.flattened[u]) {
          ...
          H5VM_array_calc(iter->u.hyp.off[v], (unsigned)((begin - u) + 1),
                          &(iter->dims[u]), &(coords[u]));
...
H5S__hyper_iter_has_next_block(const H5S_sel_iter_t *iter) {
  ...
  if (iter->u.hyp.diminfo_valid) {
    ...
    /* Check if the offset of the iterator is at the last location in all
     * dimensions */
    ...
    for (u = 0; u < iter->rank; u++) {
      ...
      if (tdiminfo[u].count == 1)
        continue;
      if (toff[u] !=
          (tdiminfo[u].start + ((tdiminfo[u].count - 1) * tdiminfo[u].stride)))
        HGOTO_DONE(TRUE);
    } /* end for */
  }   /* end if */
  else {
 ...
 *              of elements to read.  Handles walking off the end in all
 *dimensions.
 *
...
static herr_t H5S__hyper_iter_next(H5S_sel_iter_t *iter, size_t nelem) {
  ...
  if (iter->u.hyp.diminfo_valid) {
    ...
    for (u = 0; u < ndims; u++) {
      if (tdiminfo[u].count == 1) {
        ...
        iter_offset[u] =
            (iter->u.hyp.off[u] - tdiminfo[u].start) % tdiminfo[u].stride;
        iter_count[u] =
            (iter->u.hyp.off[u] - tdiminfo[u].start) / tdiminfo[u].stride;
      } /* end else */
    }   /* end for */

    /* Loop through, advancing the offset & counts, until all the nelements are
     * accounted for */
    while (nelem > 0) {
      ...
      while (temp_dim >= 0) {
        ...
        /* If this block is still in the range of blocks to output for the
         * dimension, break out of loop */
        if (iter_offset[temp_dim] < tdiminfo[temp_dim].block)
          ...
          if (iter_count[temp_dim] < tdiminfo[temp_dim].count)
            ...
            iter_count[temp_dim] =
                0; /* reset back to the beginning of the line */
        }          /* end else */
    ...
    for (u = 0; u < ndims; u++)
      iter->u.hyp.off[u] = tdiminfo[u].start +
                           (tdiminfo[u].stride * iter_count[u]) +
                           iter_offset[u];
  } /* end if */
  /* Must be an irregular hyperslab selection */
    ...
    /* Loop through, advancing the span information, until all the nelements are
     * accounted for */
    while (nelem > 0) {
      ...
      while (curr_dim >= 0) {
        ...
        /* If we walked off that span, advance to the next span */
          ...
          if (curr_span != NULL) {
            ...
            /* If we finished the span list in this dimension, decrement the
             * dimension worked on and loop again */
            curr_dim--;
        } /* end else */
      }   /* end while */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t
H5S__hyper_iter_get_seq_list_gen(iter,maxseq,maxelem,nseq,nelem,off,len)
        ...
        size_t *nelem;          OUT: Actual number of elements in sequences
generated hsize_t *off;           OUT: Array of offsets size_t *len; OUT: Array
of lengths RETURNS Non-negative on success/Negative on failure DESCRIPTION Use
the selection in the dataspace to generate a list of byte offsets and lengths
for the region(s) selected.  Start/Restart from the position in the ITER
parameter.  The number of sequences generated is limited by the MAXSEQ parameter
and the number of sequences actually generated is stored in the NSEQ parameter.
 GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
...
static herr_t H5S__hyper_iter_get_seq_list_gen(H5S_sel_iter_t *iter,
  ...
  size_t span_size =
      0;          /* Number of bytes in current span to actually process */
  ...
  unsigned ndims;       /* Number of dimensions of dataset */
  unsigned
      fast_dim; /* Rank of the fastest changing dimension for the dataspace */
  ...
  if (abs_arr[fast_dim] != curr_span->low) {
    ...
    H5_CHECKED_ASSIGN(span_elmts, size_t,
                      ((curr_span->high - abs_arr[fast_dim]) + 1), hsize_t);
    ...
    if (io_elmts_left > 0) {
      ...
      if (NULL != curr_span) {
        ...
        loc_arr[fast_dim] =
            ((hsize_t)((hssize_t)curr_span->low + sel_off[fast_dim])) *
            slab[fast_dim];
      ...
      while (curr_dim >= 0) {
        ...
        /* If we walked off that span, advance to the next span */
          ...
          if (NULL != curr_span) {
            ...
            loc_arr[curr_dim] =
                ((hsize_t)((hssize_t)curr_span->low + sel_off[curr_dim])) *
                slab[curr_dim];
  ...
  while (io_elmts_left > 0 && curr_seq < maxseq) {
    ...
    if (io_elmts_left == 0 || curr_seq >= maxseq) {
      ...
      if (abs_arr[fast_dim] <= curr_span->high) {
        ...
        /* Update location offset */
        loc_arr[fast_dim] =
            ((hsize_t)((hssize_t)curr_span->low + (hssize_t)span_elmts +
                       sel_off[fast_dim])) *
            slab[fast_dim];
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t
H5S__hyper_iter_get_seq_list_opt(iter,maxseq,maxelem,nseq,nelem,off,len)
        H5S_sel_iter_t *iter;   IN/OUT: Selection iterator describing last
                                    position of interest in selection.
        size_t maxseq;          IN: Maximum number of sequences to generate
        size_t maxelem;         IN: Maximum number of elements to include in the
...
static herr_t H5S__hyper_iter_get_seq_list_opt(H5S_sel_iter_t *iter,
  ...
  size_t fast_dim_buf_off; /* Local copy of amount to move fastest dimension
                              buffer offset */
  size_t
      fast_dim_count; /* Number of blocks left in fastest changing dimension */
  ...
  size_t
      start_io_left; /* The initial number of elements left in I/O operation */
  size_t elem_size;  /* Size of each element iterating over */
  unsigned u;        /* Local index variable */
  ...
  assert(!((iter->u.hyp.off[fast_dim] - tdiminfo[fast_dim].start) %
                   tdiminfo[fast_dim].stride !=
               0 ||
           ((iter->u.hyp.off[fast_dim] != tdiminfo[fast_dim].start) &&
            tdiminfo[fast_dim].count == 1)));
  ...
  H5_CHECKED_ASSIGN(fast_dim_buf_off, size_t, slab[fast_dim] * fast_dim_stride,
                    hsize_t);
  ...
  for (u = 0; u < ndims; u++)
    wrap[u] =
        (mem_size[u] - (tdiminfo[u].stride * tdiminfo[u].count)) * slab[u];
  ...
  if (tmp_count[fast_dim] > 0) {
    ...
    H5_CHECKED_ASSIGN(fast_dim_count, size_t,
                      tdiminfo[fast_dim].count - tmp_count[fast_dim], hsize_t);
    ...
    if (tmp_count[fast_dim] >= tdiminfo[fast_dim].count) {
      ...
      /* Increment information to reflect block just processed */
      offset[fast_dim] =
          fast_dim_offset; /* reset the offset in the fastest dimension */
      ...
      while (temp_dim >= 0) {
        ...
        if (tmp_block[temp_dim] < tdiminfo[temp_dim].block)
          ...
          /* Move to the next block in the current dimension */
          offset[temp_dim] +=
              (tdiminfo[temp_dim].stride - tdiminfo[temp_dim].block);
          loc += skip[temp_dim];
          ...
          if (tmp_count[temp_dim] < tdiminfo[temp_dim].count)
            ...
            offset[temp_dim] = (hsize_t)((hssize_t)tdiminfo[temp_dim].start +
                                         sel_off[temp_dim]);
            loc += wrap[temp_dim];
            tmp_count[temp_dim] =
                0; /* reset back to the beginning of the line */
...
#undef DUFF_GUTS
    ...
    while (temp_dim >= 0) {
      ...
      if (tmp_block[temp_dim] < tdiminfo[temp_dim].block)
        ...
        if (tmp_count[temp_dim] < tdiminfo[temp_dim].count)
          ...
          offset[temp_dim] =
              (hsize_t)((hssize_t)tdiminfo[temp_dim].start + sel_off[temp_dim]);
          loc += wrap[temp_dim];
  ...
  /* Decrement number of elements left */
  H5_CHECK_OVERFLOW(actual_elem * (total_rows * tdiminfo[fast_dim].count),
                    hsize_t, size_t);
  ...
  if (io_left > 0 && curr_seq < maxseq) {
    ...
    offset[fast_dim] +=
        (fast_dim_stride *
         tot_blk_count); /* move the offset in the fastest dimension */
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Create a list of offsets & lengths for a selection
 USAGE
    herr_t H5S__hyper_iter_get_seq_list_single(flags, iter, maxseq, maxelem,
nseq, nelem, off, len) unsigned flags;         IN: Flags for extra information
about operation H5S_sel_iter_t *iter;   IN/OUT: Selection iterator describing
last position of interest in selection. size_t maxseq;          IN: Maximum
number of sequences to generate size_t maxelem;         IN: Maximum number of
elements to include in the generated sequences size_t *nseq;           OUT:
Actual number of sequences generated size_t *nelem;          OUT: Actual number
of elements in sequences generated hsize_t *off;           OUT: Array of offsets
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_iter_get_seq_list(iter,maxseq,maxelem,nseq,nelem,off,len)
        ...
        size_t *nseq;           OUT: Actual number of sequences generated
        size_t *nelem;          OUT: Actual number of elements in sequences
generated hsize_t *off;           OUT: Array of offsets (in bytes) size_t *len;
OUT: Array of lengths (in bytes) RETURNS Non-negative on success/Negative on
failure. DESCRIPTION Use the selection in the dataspace to generate a list of
byte offsets and lengths for the region(s) selected.  Start/Restart from the
position in the ITER parameter.  The number of sequences generated is limited by
the MAXSEQ parameter and the number of sequences actually generated is stored in
the NSEQ parameter. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__hyper_iter_get_seq_list(H5S_sel_iter_t *iter, size_t maxseq,
                                           size_t maxelem, size_t *nseq,
                                           size_t *nelem, hsize_t *off,
                                           size_t *len) {
  ...
  if (iter->u.hyp.diminfo_valid) {
    ...
    if ((iter->u.hyp.off[fast_dim] - tdiminfo[fast_dim].start) %
                tdiminfo[fast_dim].stride !=
            0 ||
        ((iter->u.hyp.off[fast_dim] != tdiminfo[fast_dim].start) &&
         tdiminfo[fast_dim].count == 1)) {
      hsize_t *slab;      /* Hyperslab size */
      hsize_t loc;        /* Coordinate offset */
      size_t leftover;    /* The number of elements left over from the last
                             sequence */
      size_t actual_elem; /* The actual number of elements to count */
      size_t elem_size;   /* Size of each element iterating over */
    ...
    if (single_block)
      ...
      ret_value = H5S__hyper_iter_get_seq_list_single(iter, maxseq, maxelem,
                                                      nseq, nelem, off, len);
    else
      /* Use optimized call to generate sequence list */
      ret_value = H5S__hyper_iter_get_seq_list_opt(iter, maxseq, maxelem, nseq,
                                                   nelem, off, len);
  } /* end if */
  else
    /* Call the general sequence generator routine */
    ret_value = H5S__hyper_iter_get_seq_list_gen(iter, maxseq, maxelem, nseq,
                                                 nelem, off, len);
...
static herr_t H5S__hyper_iter_release(H5S_sel_iter_t *iter) {
  ...
  if (iter->u.hyp.spans != NULL)
    if (H5S__hyper_free_span_info(iter->u.hyp.spans) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTFREE, FAIL,
                  "unable to free span info");
...
static H5S_hyper_span_t *H5S__hyper_new_span(hsize_t low, hsize_t high,
  ...
  if (NULL == (ret_value = H5FL_MALLOC(H5S_hyper_span_t)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, NULL,
...
static H5S_hyper_span_info_t *H5S__hyper_new_span_info(unsigned rank) {
  ...
  if (NULL == (ret_value = (H5S_hyper_span_info_t *)H5FL_ARR_CALLOC(hbounds_t,
                                                                    rank * 2)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, NULL,
                "can't allocate hyperslab span info");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    H5S_hyper_span_info_t * H5S__hyper_copy_span_helper(spans, rank, op_info_i,
op_gen) H5S_hyper_span_info_t *spans;   IN: Span tree to copy unsigned rank; IN:
Rank of span tree unsigned op_info_i;             IN: Index of op info to use
...
H5S__hyper_copy_span_helper(H5S_hyper_span_info_t *spans, unsigned rank,
  ...
  if (spans->op_info[op_info_i].op_gen == op_gen) {
    ...
    H5MM_memcpy(ret_value->low_bounds, spans->low_bounds,
                rank * sizeof(hsize_t));
    H5MM_memcpy(ret_value->high_bounds, spans->high_bounds,
                rank * sizeof(hsize_t));
    ret_value->count = 1;
    ...
    /* Set the 'copied' pointer in the node being copied to the newly allocated
     * node */
    ...
    while (span != NULL) {
      /* Allocate a new node */
      if (NULL ==
          (new_span = H5S__hyper_new_span(span->low, span->high, NULL, NULL)))
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, NULL,
                    "can't allocate hyperslab span");
      ...
      if (span->down != NULL) {
        if (NULL == (new_down = H5S__hyper_copy_span_helper(
                         span->down, rank - 1, op_info_i, op_gen)))
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, NULL,
                      "can't copy hyperslab spans");
...
static H5S_hyper_span_info_t *H5S__hyper_copy_span(H5S_hyper_span_info_t *spans,
  ...
  if (NULL == (ret_value = H5S__hyper_copy_span_helper(spans, rank, 0, op_gen)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, NULL,
                "can't copy hyperslab span tree");
...
H5S__hyper_cmp_spans(const H5S_hyper_span_info_t *span_info1,
  ...
  /* Fall through, with default return value of 'TRUE' if spans were already
   * visited */
...
static herr_t H5S__hyper_free_span(H5S_hyper_span_t *span) {
  ...
  /* Decrement the reference count of the 'down spans', freeing them if
   * appropriate */
  if (span->down != NULL)
...
static herr_t H5S__hyper_copy(H5S_t *dst, const H5S_t *src,
  ...
  if (src_hslab->diminfo_valid == H5S_DIMINFO_VALID_YES)
    H5MM_memcpy(&dst_hslab->diminfo, &src_hslab->diminfo,
                sizeof(H5S_hyper_diminfo_t));
  ...
  if (src->select.sel_info.hslab->span_lst != NULL) {
    if (share_selection) {
      ...
      dst->select.sel_info.hslab->span_lst =
          src->select.sel_info.hslab->span_lst;
      dst->select.sel_info.hslab->span_lst->count++;
    } /* end if */
    else
      /* Copy the hyperslab span information */
      dst->select.sel_info.hslab->span_lst = H5S__hyper_copy_span(
          src->select.sel_info.hslab->span_lst, src->extent.rank);
  } /* end if */
  else
...
--------------------------------------------------------------------------*/
static htri_t H5S__hyper_is_valid(const H5S_t *space) {
  const hsize_t *low_bounds,
      *high_bounds; /* Pointers to the correct pair of low & high bounds */
  unsigned u;       /* Counter */
  ...
  for (u = 0; u < space->extent.rank; u++) {
    ...
    if (((hssize_t)low_bounds[u] + space->select.offset[u]) < 0)
      HGOTO_DONE(FALSE);
    if ((high_bounds[u] + (hsize_t)space->select.offset[u]) >=
        space->extent.size[u])
      HGOTO_DONE(FALSE);
  } /* end for */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static hsize_t H5S__hyper_span_nblocks_helper(H5S_hyper_span_info_t *spans,
                                              unsigned op_info_i,
                                              uint64_t op_gen) {
  ...
  if (spans->op_info[op_info_i].op_gen == op_gen)
    ...
    if (span->down) {
      while (span) {
        ...
        ret_value +=
            H5S__hyper_span_nblocks_helper(span->down, op_info_i, op_gen);
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static hsize_t H5S__get_select_hyper_nblocks(const H5S_t *space,
                                             hbool_t app_ref) {
  hsize_t ret_value = 0; /* Return value */
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    for (ret_value = 1, u = 0; u < space->extent.rank; u++)
      ret_value *=
          (app_ref ? space->select.sel_info.hslab->diminfo.app[u].count
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hssize_t H5S__hyper_get_enc_size_real(max_size, enc_size)
        hsize_t max_size:       IN: The maximum size of the hyperslab selection
info uint8_t *enc_size:      OUT:The encoding size RETURNS The size to encode
hyperslab selection info DESCRIPTION Determine the size by comparing "max_size"
with (2^32 - 1) and (2^16 - 1). GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Determine the version and encoded size to use for encoding hyperslab
selection info USAGE hssize_t H5S__hyper_get_version_enc_size(space,
block_count, version, enc_size) const H5S_t *space:             IN: The
dataspace hsize_t block_count:            IN: The number of blocks in the
selection uint32_t *version:              OUT: The version to use for encoding
 ...
 DESCRIPTION
    ...
    (2) whether the number of blocks or selection high bounds exceeds
H5S_UINT32_MAX or not
    ...
    (a) regular hyperslab
        ...
        (2) Special handling for count/block: need to provide room for
H5S_UNLIMITED (b) irregular hyperslab The maximum size needed to store: (1) the
number of blocks (2) the selection high bounds GLOBAL VARIABLES COMMENTS, BUGS,
ASSUMPTIONS EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__hyper_get_version_enc_size(H5S_t *space, hsize_t block_count,
                                              uint32_t *version,
                                              uint8_t *enc_size) {
  ...
  hbool_t count_up_version =
      FALSE; /* Whether number of blocks exceed H5S_UINT32_MAX */
  hbool_t bound_up_version =
      FALSE;                  /* Whether high bounds exceed H5S_UINT32_MAX */
  ...
  if (space->select.sel_info.hslab->unlim_dim < 0) /* ! H5S_UNLIMITED */
    ...
    if (H5S__hyper_bounds(space, bounds_start, bounds_end) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                  "can't get selection bounds");

  /* Determine whether the number of blocks or the high bounds in the selection
   * exceed (2^32 - 1) */
  ...
  if (H5CX_get_libver_bounds(&low_bound, &high_bound) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't get low/high bounds from API context");

  /* Determine regular hyperslab */
  is_regular = H5S__hyper_is_regular(space);

  if (low_bound >= H5F_LIBVER_V112 ||
    ...
    if (count_up_version || bound_up_version)
      ...
      tmp_version = (is_regular && block_count >= 4)
                        ? H5O_sds_hyper_ver_bounds[low_bound]
                        : H5S_HYPER_VERSION_1;
  } /* end else */
  ...
  if (tmp_version > H5O_sds_hyper_ver_bounds[high_bound]) {
    ...
    if (count_up_version)
      ...
      HGOTO_ERROR(
          H5E_DATASPACE, H5E_BADVALUE, FAIL,
  ...
  *version = tmp_version;
  ...
  default:
    HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
                "unknown hyperslab selection version");
    break;
  }
...
static hssize_t H5S__hyper_serial_size(H5S_t *space) {
  ...
  if (H5S__hyper_get_version_enc_size(space, block_count, &version, &enc_size) <
      0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                "can't determine hyper version & enc_size");
  ...
  if (version == H5S_HYPER_VERSION_3) {
    ...
    if (H5S__hyper_is_regular(space))
      ret_value = (hssize_t)14 + ((hssize_t)4 * (hssize_t)enc_size *
                                  (hssize_t)space->extent.rank);
    else {
      /* Version 3: irregular */
      /* Size required is always:
       * <type (4 bytes)> + <version (4 bytes)> + <flags (1 byte)> +
       ...
       * = 14 bytes + enc_size (block_count) + (2 * enc_size * rank *
       * block_count) bytes
       */
      ret_value = 14 + enc_size;
      H5_CHECK_OVERFLOW(
          ((unsigned)2 * enc_size * space->extent.rank * block_count), hsize_t,
          hssize_t);
      ret_value +=
          (hssize_t)((unsigned)2 * enc_size * space->extent.rank * block_count);
    } /* end else */
  }   /* end if */
     ...
     */
    assert(enc_size == 8);
    ret_value = (hssize_t)17 +
                ((hssize_t)4 * (hssize_t)8 * (hssize_t)space->extent.rank);
  } else {
     ...
     * (2 (starting/ending offset) * <enc_size (4 bytes)> * <rank> * <# of
     * blocks) = = 24 bytes + (2 * 4 * rank * block_count)
     */
    ret_value = 24;
    H5_CHECK_OVERFLOW((8 * space->extent.rank * block_count), hsize_t,
                      hssize_t);
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static void H5S__hyper_serialize_helper(const H5S_hyper_span_info_t *spans,
                                        hsize_t *start, hsize_t *end,
                                        hsize_t rank, uint8_t enc_size,
                                        uint8_t **p) {
  ...
  while (curr != NULL) {
    ...
    if (curr->down != NULL) {
      ...
      H5S__hyper_serialize_helper(curr->down, start, end, rank + 1, enc_size,
                                  &pp);
    } /* end if */
    else {
...
static herr_t H5S__hyper_serialize(H5S_t *space, uint8_t **p) {
  const H5S_hyper_dim_t
      *diminfo; /* Alias for dataspace's diminfo information */
  ...
  UINT32ENCODE(pp, (uint32_t)H5S_GET_SELECT_TYPE(
                       space)); /* Store the type of selection */
  ...
  if (is_regular) {
    ...
    H5S__hyper_serialize_helper(space->select.sel_info.hslab->span_lst, start,
                                end, (hsize_t)0, enc_size, &pp);
  } /* end else */

  /* Encode length */
  if (version <= H5S_HYPER_VERSION_2)
    UINT32ENCODE(lenp,
                 (uint32_t)len); /* Store the length of the extra information */
...
static herr_t H5S__hyper_deserialize(H5S_t **space, const uint8_t **p,
  ...
  herr_t ret_value = FAIL;     /* return value */
  const uint8_t *p_end =
      *p + p_size - 1; /* Pointer to last valid byte in buffer */
  FUNC_ENTER_PACKAGE
  ...
  if (version >= (uint32_t)H5S_HYPER_VERSION_2) {
    ...
    if (flags & ~H5S_SELECT_FLAG_BITS)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTLOAD, FAIL,
                  "unknown flag for selection");
  } else {
    /* Skip over the remainder of the header */
  ...
  if (enc_size & ~H5S_SELECT_INFO_ENC_SIZE_BITS)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTLOAD, FAIL,
                "unknown size of point/offset info for selection");

  /* Decode the rank of the point selection */
  if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, pp, sizeof(uint32_t), p_end))
    HGOTO_ERROR(H5E_DATASPACE, H5E_OVERFLOW, FAIL,
                "buffer overflow while decoding selection rank");
  ...
  if (flags & H5S_HYPER_REGULAR) {
    ...
    if ((ret_value = H5S_select_hyperslab(tmp_space, H5S_SELECT_SET, start,
    ...
    for (u = 0; u < num_elem; u++) {
      ...
      switch (enc_size) {
      case H5S_SELECT_INFO_ENC_SIZE_2:
        if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, pp, rank * 2 * sizeof(uint16_t),
                                        p_end))
          HGOTO_ERROR(H5E_DATASPACE, H5E_OVERFLOW, FAIL,
      ...
      case H5S_SELECT_INFO_ENC_SIZE_4:
        if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, pp, rank * 2 * sizeof(uint32_t),
                                        p_end))
          HGOTO_ERROR(H5E_DATASPACE, H5E_OVERFLOW, FAIL,
                      "buffer overflow while decoding selection coordinates");
      ...
      case H5S_SELECT_INFO_ENC_SIZE_8:
        if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, pp, rank * 2 * sizeof(uint64_t),
                                        p_end))
          HGOTO_ERROR(H5E_DATASPACE, H5E_OVERFLOW, FAIL,
                      "buffer overflow while decoding selection coordinates");
      ...
      for (tblock = block, tstart = start, tend = end, v = 0; v < rank;
           v++, tstart++, tend++, tblock++)
        *tblock = (*tend - *tstart) + 1;

      /* Select or add the hyperslab to the current selection */
      if ((ret_value = H5S_select_hyperslab(
               tmp_space, (u == 0 ? H5S_SELECT_SET : H5S_SELECT_OR), start,
               stride, count, block)) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL, "can't change selection");
    } /* end for */
  }   /* end else */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_span_blocklist(spans, start, end, rank, startblock,
numblocks, buf) H5S_hyper_span_info_t *spans;   IN: Dataspace pointer of
selection to query hsize_t start[];       IN/OUT: Accumulated start points
...
static herr_t H5S__hyper_span_blocklist(const H5S_hyper_span_info_t *spans,
  ...
  while (curr != NULL && *numblocks > 0) {
    ...
    if (curr->down != NULL) {
      ...
      /* Recurse down to the next dimension */
      if (H5S__hyper_span_blocklist(curr->down, start, end, (rank + 1),
                                    startblock, numblocks, buf) < 0)
        HGOTO_ERROR(H5E_INTERNAL, H5E_CANTFREE, FAIL,
                    "failed to release hyperslab spans");
    } /* end if */
    else {
...
static herr_t H5S__get_select_hyper_blocklist(H5S_t *space, hsize_t startblock,
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    ret_value = H5S__hyper_span_blocklist(
        space->select.sel_info.hslab->span_lst, start, end, (hsize_t)0,
...
herr_t H5Sget_select_hyper_blocklist(hid_t spaceid, hsize_t startblock,
  ...
  if (space->select.sel_info.hslab->unlim_dim >= 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
                "cannot get blocklist for unlimited selection");

  /* Go get the correct number of blocks */
  if (numblocks > 0)
    ret_value =
        H5S__get_select_hyper_blocklist(space, startblock, numblocks, buf);
  else
...
static herr_t H5S__hyper_bounds(const H5S_t *space, hsize_t *start,
  ...
  if (space->select.offset_changed) {
    ...
    for (u = 0; u < space->extent.rank; u++) {
      ...
      if (((hssize_t)low_bounds[u] + space->select.offset[u]) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_BADRANGE, FAIL,
                    "offset moves selection out of bounds");
  ...
  *offset = 0;
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    for (i = (int)(rank - 1); i >= 0; i--) {
      hssize_t hyp_offset =
          (hssize_t)diminfo[i].start +
          sel_offset[i]; /* Hyperslab's offset in this dimension */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_num_elem_non_unlim(space,num_elem_non_unlim)
        H5S_t *space;           IN: Dataspace pointer to check
        hsize_t *num_elem_non_unlim; OUT: Number of elements in the
non-unlimited dimensions RETURNS Non-negative on success/Negative on failure
 DESCRIPTION
...
static herr_t H5S__hyper_num_elem_non_unlim(const H5S_t *space,
  ...
  if (space->select.sel_info.hslab->unlim_dim >= 0)
    *num_elem_non_unlim = space->select.sel_info.hslab->num_elem_non_unlim;
    ...
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                "selection has no unlimited dimension");
...
static H5_ATTR_PURE htri_t H5S__hyper_is_contiguous(const H5S_t *space) {
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    /* Initialize flags */
    large_contiguous =
        TRUE; /* assume true and reset if the dimensions don't match */
    ...
    /* If there are multiple spans in the slowest changing dimension, the
     * selection isn't contiguous */
    if (span->next != NULL)
      large_contiguous = FALSE;
    else {
      ...
      if (span->down != NULL) {
        ...
        /* Cycle down the spans until we run out of down spans or find a
         * non-contiguous span */
        while (spans != NULL) {
          span = spans->head;
          ...
          if (span->next != NULL) {
            ...
            /* If this span doesn't cover the entire dimension, then this
             * selection isn't contiguous */
    ...
    if (!large_contiguous) {
      ...
      while (spans != NULL) {
        ...
        if (span->next != NULL) {
           ...
           * selection isn't contiguous
           */
          if (u < (space->extent.rank - 1) &&
              ((span->high - span->low) + 1) != 1) {
            small_contiguous = FALSE;
            break;
          } /* end if */
          else {
...
/*--------------------------------------------------------------------------
 NAME
    H5S__hyper_is_single
 PURPOSE
    Check if a hyperslab selection is a single block within the dataspace
extent. USAGE htri_t H5S__hyper_is_single(space) H5S_t *space;           IN:
Dataspace pointer to check RETURNS TRUE/FALSE/FAIL DESCRIPTION Checks to see if
the current selection in the dataspace is a single block. This is primarily used
for reading the entire selection in one swoop. GLOBAL VARIABLES COMMENTS, BUGS,
ASSUMPTIONS EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hbool_t H5S__hyper_spans_shape_same_helper(span1, span2, offset, rest_zeros)
        H5S_hyper_span_info_t *span_info1;      IN: First span tree to compare
        H5S_hyper_span_info_t *span_info2;      IN: Second span tree to compare
        hssize_t offset[];                      IN: Offset between the span
trees hbool_t rest_zeros[];                   IN: Array of flags which indicate
                                                    the rest of the offset[]
array is zero values. RETURNS TRUE (1) or FALSE (0) on success, can't fail
 DESCRIPTION
...
H5S__hyper_spans_shape_same_helper(const H5S_hyper_span_info_t *span_info1,
                                   const H5S_hyper_span_info_t *span_info2,
                                   hssize_t offset[], hbool_t rest_zeros[]) {
  hbool_t ret_value = TRUE; /* Return value */
   ...
   *      that's worth it. - QAK, 2019/01/23)
   */
  if ((hsize_t)((hssize_t)span_info1->low_bounds[0] + offset[0]) !=
      span_info2->low_bounds[0])
    HGOTO_DONE(FALSE);
  else if ((hsize_t)((hssize_t)span_info1->high_bounds[0] + offset[0]) !=
           span_info2->high_bounds[0])
    HGOTO_DONE(FALSE);
  else {
    const H5S_hyper_span_t *span1;
    const H5S_hyper_span_t *span2;
    ...
    while (1) {
      ...
      if (span1->down != NULL || span2->down != NULL) {
        /* If the rest of the span trees have a zero offset, use the faster
         * comparison routine */
        if (rest_zeros[0]) {
          if (!H5S__hyper_cmp_spans(span1->down, span2->down))
            HGOTO_DONE(FALSE);
          ...
          if (!H5S__hyper_spans_shape_same_helper(span1->down, span2->down,
                                                  &offset[1], &rest_zeros[1]))
            HGOTO_DONE(FALSE);
          else {
            /* Keep going... */
          } /* end else */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static H5_ATTR_PURE hbool_t H5S__hyper_spans_shape_same(
    const H5S_hyper_span_info_t *span_info1,
  ...
  hbool_t rest_zeros[H5S_MAX_RANK]; /* Vector of flags to indicate when
                                       remaining offset is all zero */
  ...
  for (u = 0; u < ndims; u++) {
    ...
    assert((span1->down && span2->down) ||
           (NULL == span1->down && NULL == span2->down));
  ...
  if (zero_offset)
    ...
    ret_value = H5S__hyper_spans_shape_same_helper(span_info1, span_info2,
                                                   offset, rest_zeros);
...
static htri_t H5S__hyper_shape_same(H5S_t *space1, H5S_t *space2) {
  ...
  /* If both aren't regular, use fast irregular comparison */
    ...
    ret_value = H5S__hyper_spans_shape_same(
        spans1, space2->select.sel_info.hslab->span_lst, space2_rank);
  } /* end else */
...
static H5S_hyper_span_t *H5S__hyper_coord_to_span(unsigned rank,
  ...
  H5S_hyper_span_info_t *down =
      NULL; /* Pointer to new span tree for next level down */
  H5S_hyper_span_t *ret_value = NULL; /* Return value */
  ...
  if (rank > 1) {
    ...
    /* Update the tail pointer of the down dimension, and it's a single span
     * element */
    down->tail = down->head;
  } /* end if */
  ...
  if (NULL ==
      (new_span = H5S__hyper_new_span(coords[0], coords[0], down, NULL)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, NULL,
                "can't allocate hyperslab span");
  ...
  if (ret_value == NULL && down != NULL)
    if (H5S__hyper_free_span_info(down) < 0)
      HDONE_ERROR(H5E_DATASPACE, H5E_CANTFREE, NULL,
                  "unable to free span info");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_add_span_element_helper(span_tree, rank, coords,
first_dim_modified) H5S_hyper_span_info_t *span_tree;  IN/OUT: Pointer to span
tree to append to unsigned rank;                  IN: Number of dimensions of
coordinates hsize_t *coords;                IN: Location of element to add to
span tree int *first_dim_modified;        IN: Index of the first dimension
modified RETURNS Non-negative on success, negative on failure DESCRIPTION Add a
single element to an existing span tree. GLOBAL VARIABLES COMMENTS, BUGS,
ASSUMPTIONS Assumes that the element is not already covered by the span tree
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t
H5S__hyper_add_span_element_helper(H5S_hyper_span_info_t *span_tree,
  ...
  if (coords[0] >= tail_span->low && coords[0] <= tail_span->high) {
    H5S_hyper_span_t
        *prev_down_tail_span; /* Pointer to previous down spans' tail pointer */
    hsize_t prev_down_tail_span_high; /* Value of previous down spans' tail's
                                         high value */
    ...
    assert(rank > 1);
    if (H5S__hyper_add_span_element_helper(tail_span->down, rank - 1,
                                           &coords[1], first_dim_modified) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, FAIL,
                  "can't insert coordinate into span tree");

    /* Check & update high bounds for lower dimensions */
    if (*first_dim_modified >= 0) {
      unsigned
          first_dim; /* First dimension modified, relative to this span tree */
      hbool_t first_dim_set =
          FALSE;  /* Whether first dimension modified is set */
      unsigned u; /* Local index variable */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S_hyper_add_span_element(space, span_tree, rank, coords)
        H5S_t *space;           IN/OUT: Pointer to dataspace to add coordinate
to unsigned rank;          IN: Number of dimensions of coordinates hsize_t
*coords;       IN: Location of element to add to span tree RETURNS Non-negative
on success, negative on failure DESCRIPTION Add a single element to an existing
span tree. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS Assumes that the element
is not already in the dataspace's selection
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_hyper_add_span_element(H5S_t *space, unsigned rank,
                                  const hsize_t *coords) {
  ...
  if (NULL == space->select.sel_info.hslab) {
    ...
    if (NULL == (space->select.sel_info.hslab = H5FL_MALLOC(H5S_hyper_sel_t)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate hyperslab selection");
    ...
    if (H5S__hyper_add_span_element_helper(
            space->select.sel_info.hslab->span_lst, rank, coords,
            &first_dim_modified) < 0)
  ...
  if (ret_value < 0)
    if (head)
      if (H5S__hyper_free_span_info(head) < 0)
        HDONE_ERROR(H5E_DATASPACE, H5E_CANTFREE, FAIL,
                    "unable to free span info");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hbool_t H5S__hyper_intersect_block_helper(spans, rank, start, end,
op_info_i, op_gen) H5S_hyper_span_info_t *spans;     IN: First span tree to
operate with unsigned rank;     IN: Number of dimensions for span tree hsize_t
*start;    IN: Starting coordinate for block hsize_t *end;      IN: Ending
coordinate for block unsigned op_info_i;             IN: Index of op info to use
        uint64_t op_gen;   IN: Operation generation
 RETURN
...
--------------------------------------------------------------------------*/
static hbool_t
H5S__hyper_intersect_block_helper(H5S_hyper_span_info_t *spans, unsigned rank,
  ...
  if (spans->op_info[op_info_i].op_gen != op_gen) {
    ...
    while (curr != NULL) {
      ...
      if (curr->high < *start)
        ...
        curr = curr->next;
      /* If this span is past the end of the block, then we're done in this
       * dimension */
      else if (curr->low > *end)
        HGOTO_DONE(FALSE);
      /* block & span overlap */
      else {
        /* If this is the bottom dimension, then the span tree overlaps the
         * block */
        if (curr->down == NULL)
          HGOTO_DONE(TRUE);
        /* Recursively check spans in next dimension down */
           ...
           */
          if (H5S__hyper_intersect_block_helper(curr->down, rank - 1, start + 1,
                                                end + 1, op_info_i, op_gen))
            HGOTO_DONE(TRUE);
...
static htri_t H5S__hyper_intersect_block(H5S_t *space, const hsize_t *start,
  ...
  if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
    ...
    if (single_block)
      ...
      for (u = 0; u < space->extent.rank; u++) {
        ...
        if (start[u] > space->select.sel_info.hslab->diminfo.opt[u].start) {
          hsize_t adj_start; /* Start coord, adjusted for hyperslab selection
                                parameters */
          hsize_t nstride;   /* Number of strides into the selection */

          /* Adjust start coord for selection's 'start' offset */
          adj_start =
              start[u] - space->select.sel_info.hslab->diminfo.opt[u].start;

          /* Compute # of strides into the selection */
          if (space->select.sel_info.hslab->diminfo.opt[u].count > 1)
            nstride =
                adj_start / space->select.sel_info.hslab->diminfo.opt[u].stride;
          else
            nstride = 0;
          ...
          if (adj_start >= space->select.sel_info.hslab->diminfo.opt[u].block) {
            hsize_t adj_end; /* End coord, adjusted for hyperslab selection
                                parameters */

            /* Adjust end coord for selection's 'start' offset */
            adj_end =
                end[u] - space->select.sel_info.hslab->diminfo.opt[u].start;
    ...
    /* Always use op_info[0] since we own this op_info, so there can be no
     * simultaneous operations */
    ret_value = H5S__hyper_intersect_block_helper(
        space->select.sel_info.hslab->span_lst, space->extent.rank, start, end,
        0, op_gen);
  } /* end else */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static void H5S__hyper_adjust_u_helper(H5S_hyper_span_info_t *spans,
                                       unsigned rank, const hsize_t *offset,
                                       unsigned op_info_i, uint64_t op_gen) {
  FUNC_ENTER_PACKAGE_NOERR
  ...
  if (spans->op_info[op_info_i].op_gen != op_gen) {
    ...
    while (span != NULL) {
      ...
      if (span->down != NULL)
        H5S__hyper_adjust_u_helper(span->down, rank - 1, offset + 1, op_info_i,
                                   op_gen);
  ...
  if (non_zero_offset) {
    ...
    if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
      for (u = 0; u < space->extent.rank; u++) {
        ...
        assert(space->select.sel_info.hslab->diminfo.low_bounds[u] >=
               offset[u]);
    ...
    if (space->select.sel_info.hslab->span_lst) {
      ...
      /* Always use op_info[0] since we own this op_info, so there can be no
       * simultaneous operations */
      H5S__hyper_adjust_u_helper(space->select.sel_info.hslab->span_lst,
                                 space->extent.rank, offset, 0, op_gen);
    } /* end if */
  }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__hyper_project_simple_lower(const H5S_t *base_space,
                                              H5S_t *new_space) {
  ...
  curr_dim = 0;
  while (down &&
         curr_dim < (base_space->extent.rank - new_space->extent.rank)) {
    /* Sanity check */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__hyper_project_simple_higher(const H5S_t *base_space,
                                               H5S_t *new_space) {
  ...
  while (curr_dim < delta_rank) {
    ...
    /* Allocate a new span_info node */
    if (NULL ==
        (new_span_info = H5S__hyper_new_span_info(new_space->extent.rank))) {
      if (prev_span)
    ...
    /* Allocate a new node */
    if (NULL ==
        (new_span = H5S__hyper_new_span((hsize_t)0, (hsize_t)0, NULL, NULL))) {
    ...
    for (; u < new_space->extent.rank; u++) {
      new_span_info->low_bounds[u] = base_space->select.sel_info.hslab->span_lst
                                         ->low_bounds[u - delta_rank];
      new_span_info->high_bounds[u] =
          base_space->select.sel_info.hslab->span_lst
              ->high_bounds[u - delta_rank];
    } /* end for */
  ...
  if (ret_value < 0 && new_space->select.sel_info.hslab->span_lst) {
    ...
    new_space->select.sel_info.hslab->span_lst =
        (H5S_hyper_span_info_t *)H5FL_ARR_FREE(
            hbounds_t, new_space->select.sel_info.hslab->span_lst);
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__hyper_project_simple(const H5S_t *base_space,
                                        H5S_t *new_space, hsize_t *offset) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* We are setting a new selection, remove any current selection in new
   * dataspace */
  ...
  if (base_space->select.sel_info.hslab->diminfo_valid ==
    ...
    if (new_space->extent.rank < base_space->extent.rank) {
      ...
      *offset = H5VM_array_offset(base_space->extent.rank,
                                  base_space->extent.size, block);
      ...
      *offset = 0;
      ...
      for (new_space_dim = 0;
           new_space_dim < (new_space->extent.rank - base_space->extent.rank);
           new_space_dim++) {
    ...
    new_space->select.sel_info.hslab->diminfo_valid =
        base_space->select.sel_info.hslab->diminfo_valid;
  } /* end else */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static void H5S__hyper_adjust_s_helper(H5S_hyper_span_info_t *spans,
                                       unsigned rank, const hssize_t *offset,
                                       unsigned op_info_i, uint64_t op_gen) {
  FUNC_ENTER_PACKAGE_NOERR
  ...
  if (spans->op_info[op_info_i].op_gen != op_gen) {
    ...
    for (u = 0; u < rank; u++) {
      ...
      spans->low_bounds[u] =
          (hsize_t)((hssize_t)spans->low_bounds[u] - offset[u]);
      spans->high_bounds[u] =
          (hsize_t)((hssize_t)spans->high_bounds[u] - offset[u]);
    } /* end for */
    ...
    while (span != NULL) {
      ...
      /* Recursively adjust spans in next dimension down */
      if (span->down != NULL)
        H5S__hyper_adjust_s_helper(span->down, rank - 1, offset + 1, op_info_i,
                                   op_gen);
...
static herr_t H5S__hyper_adjust_s(H5S_t *space, const hssize_t *offset) {
  ...
  if (non_zero_offset) {
    ...
    if (space->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
      for (u = 0; u < space->extent.rank; u++) {
        assert((hssize_t)space->select.sel_info.hslab->diminfo.opt[u].start >=
               offset[u]);
        space->select.sel_info.hslab->diminfo.opt[u].start =
            (hsize_t)((hssize_t)space->select.sel_info.hslab->diminfo.opt[u]
                          .start -
                      offset[u]);

        /* Adjust the low & high bounds */
        assert((hssize_t)space->select.sel_info.hslab->diminfo.low_bounds[u] >=
               offset[u]);
        space->select.sel_info.hslab->diminfo.low_bounds[u] =
            (hsize_t)((hssize_t)
                          space->select.sel_info.hslab->diminfo.low_bounds[u] -
    ...
    if (space->select.sel_info.hslab->span_lst) {
      ...
      /* Always use op_info[0] since we own this op_info, so there can be no
       * simultaneous operations */
      H5S__hyper_adjust_s_helper(space->select.sel_info.hslab->span_lst,
                                 space->extent.rank, offset, 0, op_gen);
    } /* end if */
  }
...
herr_t H5S_hyper_denormalize_offset(H5S_t *space, const hssize_t *old_offset) {
  ...
  H5MM_memcpy(space->select.offset, old_offset,
              sizeof(hssize_t) * space->extent.rank);
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_append_span(span_tree, ndims, low, high, down)
        H5S_hyper_span_info_t **span_tree;  IN/OUT: Pointer to span tree to
append to unsigned ndims;                  IN: Number of dimension for span
        hsize_t low, high;               IN: Low and high bounds for new span
node H5S_hyper_span_info_t *down;     IN: Down span tree for new node RETURNS
    Non-negative on success, negative on failure
 DESCRIPTION
...
static herr_t H5S__hyper_append_span(H5S_hyper_span_info_t **span_tree,
  ...
  if (*span_tree == NULL) {
    ...
    if (down) {
      ...
      H5MM_memcpy(&((*span_tree)->low_bounds[1]), down->low_bounds,
                  sizeof(hsize_t) * (ndims - 1));
      H5MM_memcpy(&((*span_tree)->high_bounds[1]), down->high_bounds,
                  sizeof(hsize_t) * (ndims - 1));
    } /* end if */
  }   /* end if */
  /* Merge or append to existing merged spans list */
    ...
    if ((((*span_tree)->tail->high + 1) == low) &&
      ...
      /* (No need to update lower dimensions, since this span shares them with
       * previous span) */
      (*span_tree)->high_bounds[0] = high;
    } /* end if */
    else {
      ...
      if (down) {
        /* Check if the down spans for the new span node are the same as the
         * previous span node */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_clip_spans(span_a, span_b, selector, curr_dim, dim_size,
        ...
        unsigned selector;            IN: The parameter deciding which output is
needed (only considering the last three bits ABC: If A is set, then a_not_b is
needed; If B is set, then a_and_b is needed; If C is set, then b_not_a is
needed;
                                           )
...
static herr_t H5S__hyper_clip_spans(H5S_hyper_span_info_t *a_spans,
  ...
  /* If span 'a' is not defined, but 'b' is, copy 'b' and set the other return
     span trees to empty */
  ...
  /* If span 'b' is not defined, but 'a' is, copy 'a' and set the other return
     span trees to empty */
  ...
  /* If span 'a' and 'b' are both defined, calculate the proper span trees */
    ...
    if (H5S__hyper_cmp_spans(a_spans, b_spans)) {
      ...
      *b_not_a = NULL;
      ...
      hbool_t recover_a,
          recover_b; /* Flags to indicate when to recover temporary spans */
      ...
      while (span_a != NULL && span_b != NULL) {
        H5S_hyper_span_info_t
            *down_a_not_b; /* Temporary pointer to a_not_b span tree of down
                              spans for overlapping nodes */
        H5S_hyper_span_info_t
            *down_a_and_b; /* Temporary pointer to a_and_b span tree of down
                              spans for overlapping nodes */
        H5S_hyper_span_info_t
            *down_b_not_a; /* Temporary pointer to b_and_a span tree of down
                              spans for overlapping nodes */
        ...
        if (span_a->high < span_b->low) {
          ...
          if (need_a_not_b)
            if (H5S__hyper_append_span(a_not_b, ndims, span_a->low,
                                       span_a->high, span_a->down) < 0)
        ...
        /* <-----------------------------------> */
        /*             BBBBBBBBBB                */
        else if (span_a->low < span_b->low && (span_a->high >= span_b->low &&
          ...
          if (need_a_not_b)
            if (H5S__hyper_append_span(a_not_b, ndims, span_a->low,
                                       span_b->low - 1, span_a->down) < 0)
              HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, FAIL,
                          "can't allocate hyperslab span");

          /* Check for overlaps between upper part of span 'a' and lower part of
           * span 'b' */

          /* Make certain both spans either have a down span or both don't have
           * one */
          assert((span_a->down != NULL && span_b->down != NULL) ||
                 (span_a->down == NULL && span_b->down == NULL));

          /* If there are no down spans, just add the overlapping area to the
           * a_and_b list */
          if (span_a->down == NULL) {
            /* Merge/add overlapped part with/to a_and_b list */
            if (need_a_and_b)
              if (H5S__hyper_append_span(a_and_b, ndims, span_b->low,
                                         span_a->high, NULL) < 0)
                HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, FAIL,
                            "can't allocate hyperslab span");
          } /* end if */
          /* If there are down spans, check for the overlap in them and add to
           ...
           */
            ...
            if (H5S__hyper_clip_spans(span_a->down, span_b->down, selector,
                                      ndims - 1, &down_a_not_b, &down_a_and_b,
                                      &down_b_not_a) < 0)
            ...
            if (down_a_not_b) {
              ...
              if (H5S__hyper_append_span(a_not_b, ndims, span_b->low,
                                         span_a->high, down_a_not_b) < 0)
                HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, FAIL,
                            "can't allocate hyperslab span");
            ...
            if (down_b_not_a) {
              ...
              if (H5S__hyper_append_span(b_not_a, ndims, span_b->low,
                                         span_a->high, down_b_not_a) < 0)
                HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, FAIL,
                            "can't allocate hyperslab span");
          ...
          if (span_a->high < span_b->high) {
            ...
            if (NULL ==
                (tmp_span = H5S__hyper_new_span(span_a->high + 1, span_b->high,
                                                span_b->down, span_b->next)))
              HGOTO_ERROR(H5E_DATASPACE, H5E_NOSPACE, FAIL,
                          "can't allocate hyperslab span");
        ...
        /*             BBBBBBBBBB                */
          ...
          if (NULL ==
              (tmp_span = H5S__hyper_new_span(span_b->high + 1, span_a->high,
                                              span_a->down, span_a->next)))
            HGOTO_ERROR(H5E_DATASPACE, H5E_NOSPACE, FAIL,
                        "can't allocate hyperslab span");
        ...
        /*             BBBBBBBBBB                */
           ...
           */
            ...
            if (down_b_not_a) {
              ...
              /* Merge/add overlapped part with/to b_not_a list */
              if (H5S__hyper_append_span(b_not_a, ndims, span_a->low,
                                         span_a->high, down_b_not_a) < 0)
                HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, FAIL,
                            "can't allocate hyperslab span");
        ...
        /*             BBBBBBBBBB                */
          ...
          /* Check for overlaps between lower part of span 'a' and upper part of
           * span 'b' */
 ...
 USAGE
    H5S_hyper_span_info_t *H5S__hyper_merge_spans_helper(a_spans, b_spans)
        ...
        H5S_hyper_span_info_t *b_spans; IN: Second hyperslab spans to merge
                                                together
        unsigned ndims;                 IN: Number of dimensions of this span
tree RETURNS Pointer to span tree containing the merged spans on success, NULL
on failure DESCRIPTION Merge two sets of hyperslab spans together and return the
span tree from the merged set. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS
    Handles merging span trees that overlap.
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
static H5S_hyper_span_info_t *
H5S__hyper_merge_spans_helper(H5S_hyper_span_info_t *a_spans,
                              H5S_hyper_span_info_t *b_spans, unsigned ndims) {
  H5S_hyper_span_info_t *merged_spans =
      NULL;                                /* Pointer to the merged span tree */
  H5S_hyper_span_info_t *ret_value = NULL; /* Return value */

  FUNC_ENTER_PACKAGE

  /* Make certain both 'a' & 'b' spans have down span trees or neither does */
  assert((a_spans != NULL && b_spans != NULL) ||
         (a_spans == NULL && b_spans == NULL));
  ...
  if (H5S__hyper_cmp_spans(a_spans, b_spans)) {
    ...
    while (span_a != NULL && span_b != NULL) {
      ...
      if (span_a->high < span_b->low) {
        ...
        if (H5S__hyper_append_span(&merged_spans, ndims, span_a->low,
                                   span_a->high, span_a->down) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, NULL,
                      "can't allocate hyperslab span");
      ...
      /*             BBBBBBBBBB                */
        ...
        if (H5S__hyper_cmp_spans(span_a->down, span_b->down)) {
          ...
          if (H5S__hyper_append_span(&merged_spans, ndims, span_b->low,
                                     span_a->high, tmp_spans) < 0)
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, NULL,
                        "can't allocate hyperslab span");

          /* Release merged span tree for overlapped section */
          if (H5S__hyper_free_span_info(tmp_spans) < 0)
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTFREE, NULL,
                        "unable to free span info");
        }
...
static herr_t H5S__hyper_merge_spans(H5S_t *space,
  ...
  if (space->select.sel_info.hslab->span_lst == NULL) {
    ...
    if (NULL == (merged_spans = H5S__hyper_merge_spans_helper(
                     space->select.sel_info.hslab->span_lst, new_spans,
                     space->extent.rank)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTMERGE, FAIL,
                  "can't merge hyperslab spans");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hsize_t H5S__hyper_spans_nelem_helper(spans, op_info_i, op_gen)
        const H5S_hyper_span_info_t *spans; IN: Hyperslan span tree to count
elements of unsigned op_info_i;             IN: Index of op info to use uint64_t
op_gen;                IN: Operation generation RETURNS Number of elements in
span tree on success; negative on failure DESCRIPTION Counts the number of
elements described by the spans in a span tree. GLOBAL VARIABLES COMMENTS, BUGS,
ASSUMPTIONS EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
static hsize_t H5S__hyper_spans_nelem_helper(H5S_hyper_span_info_t *spans,
                                             unsigned op_info_i,
  ...
  if (spans->op_info[op_info_i].op_gen == op_gen)
    ...
    if (NULL == span->down) {
      ...
      while (span != NULL) {
        ...
        ret_value += nelmts * H5S__hyper_spans_nelem_helper(span->down,
                                                            op_info_i, op_gen);
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hsize_t H5S__hyper_spans_nelem(spans)
        const H5S_hyper_span_info_t *spans; IN: Hyperslan span tree to count
elements of RETURNS Number of elements in span tree on success; negative on
failure DESCRIPTION Counts the number of elements described by the spans in a
span tree. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
static hsize_t H5S__hyper_spans_nelem(H5S_hyper_span_info_t *spans) {
  uint64_t op_gen;       /* Operation generation value */
  hsize_t ret_value = 0; /* Return value */
...
static H5S_hyper_span_info_t *H5S__hyper_make_spans(unsigned rank,
                                                    ...
                                                    const hsize_t *block) {
  H5S_hyper_span_info_t *down =
      NULL;                      /* Pointer to spans in next dimension down */
  ...
  for (i = (int)(rank - 1); i >= 0; i--) {
    ...
    dim_stride = stride[i];
    for (u = 0; u < count[i];
         u++, curr_low += dim_stride, curr_high += dim_stride) {
...
H5S__hyper_update_diminfo(H5S_t *space, H5S_seloper_t op,
  ...
  if (!((op == H5S_SELECT_OR) || (op == H5S_SELECT_XOR)) ||
    ...
    H5MM_memcpy(tmp_diminfo, space->select.sel_info.hslab->diminfo.opt,
                sizeof(tmp_diminfo));
    ...
    for (curr_dim = 0; curr_dim < space->extent.rank; curr_dim++) {
      ...
      if ((tmp_diminfo[curr_dim].start != new_hyper_diminfo[curr_dim].start) ||
          (tmp_diminfo[curr_dim].stride !=
           new_hyper_diminfo[curr_dim].stride) ||
         ...
         * slabs */
        if ((tmp_diminfo[curr_dim].stride !=
             new_hyper_diminfo[curr_dim].stride) &&
        ...
        if ((tmp_diminfo[curr_dim].count == 1) && (high_count == 1)) {
          ...
          if ((tmp_diminfo[curr_dim].start + tmp_diminfo[curr_dim].block) >
            ...
            if (op == H5S_SELECT_OR)
              ...
              if (tmp_diminfo[curr_dim].block != high_block) {
                space->select.sel_info.hslab->diminfo_valid =
          ...
          else if ((tmp_diminfo[curr_dim].start +
                    tmp_diminfo[curr_dim].block) == high_start)
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_rebuild_helper(space)
        ...
        H5S_hyper_dim_t span_slab_info[]; OUT: Rebuilt section of hyperslab
description RETURNS TRUE/FALSE for hyperslab selection rebuilt DESCRIPTION
    Examine the span tree for a hyperslab selection and rebuild
    the start/stride/count/block information for the selection, if possible.
 GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
 ...
 REVISION LOG
    KY, 2005/9/22
--------------------------------------------------------------------------*/
static hbool_t H5S__hyper_rebuild_helper(const H5S_hyper_span_info_t *spans,
                                         H5S_hyper_dim_t span_slab_info[]) {
  const H5S_hyper_span_t *span;      /* Hyperslab span */
...
void H5S__hyper_rebuild(H5S_t *space) {
  ...
  if (FALSE == H5S__hyper_rebuild_helper(space->select.sel_info.hslab->span_lst,
                                         rebuilt_slab_info))
     ...
     */
    H5MM_memcpy(space->select.sel_info.hslab->diminfo.app, rebuilt_slab_info,
                sizeof(rebuilt_slab_info));
    H5MM_memcpy(space->select.sel_info.hslab->diminfo.opt, rebuilt_slab_info,
                sizeof(rebuilt_slab_info));
    H5MM_memcpy(space->select.sel_info.hslab->diminfo.low_bounds,
                space->select.sel_info.hslab->span_lst->low_bounds,
...
static herr_t H5S__hyper_generate_spans(H5S_t *space) {
  ...
  for (u = 0; u < space->extent.rank; u++) {
    ...
    if (space->select.sel_info.hslab->diminfo.opt[u].count == H5S_UNLIMITED)
      HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
                  "can't generate spans with unlimited count");
    if (space->select.sel_info.hslab->diminfo.opt[u].block == H5S_UNLIMITED)
      HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
                  "can't generate spans with unlimited block");
  ...
  if (H5S__generate_hyperslab(space, H5S_SELECT_SET, tmp_start, tmp_stride,
                              tmp_count, tmp_block) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
                "can't generate hyperslabs");
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static H5_ATTR_PURE hbool_t H5S__check_spans_overlap(
    const H5S_hyper_span_info_t *spans1, const H5S_hyper_span_info_t *spans2) {
  hbool_t ret_value = FALSE; /* Return value */
  ...
  if (H5S_RANGE_OVERLAP(spans1->low_bounds[0], spans1->high_bounds[0],
    ...
    while (span1 && span2) {
      ...
      if (span1->high <= span2->high) {
        /* Advance span1, unless it would be off the list and span2 has more
         * nodes */
        ...
        /* Advance span2, unless it would be off the list and span1 has more
         * nodes */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__fill_in_new_space(space1, op, space2_span_lst, can_own_span2,
        ...
        hbool_t can_own_span2;   IN: Indicates whether the 2nd span list could
be owned by the result. If not, the 2nd span list has to be copied. hbool_t
*span2_owned;  OUT: Indicates if the 2nd span list is actually owned H5S_t
**result;  OUT: The dataspace containing the new selection. It could be same
with the 1st dataspace. RETURNS Non-negative on success, negative on failure
 PROGRAMMER
...
static herr_t H5S__fill_in_new_space(H5S_t *space1, H5S_seloper_t op,
                                     ...
                                     hbool_t *updated_spans, H5S_t **result) {
  H5S_hyper_span_info_t *a_not_b =
      NULL; /* Span tree for hyperslab spans in old span tree and not in new
               span tree */
  H5S_hyper_span_info_t *a_and_b =
      NULL; /* Span tree for hyperslab spans in both old and new span trees */
  H5S_hyper_span_info_t *b_not_a =
      NULL; /* Span tree for hyperslab spans in new span tree and not in old
               span tree */
  ...
  *updated_spans = FALSE;
  ...
  overlapped = H5S__check_spans_overlap(space1->select.sel_info.hslab->span_lst,
                                        space2_span_lst);
  ...
  if (!overlapped) {
    ...
    case H5S_SELECT_XOR:
       ...
       */
      if (is_result_new)
        (*result)->select.sel_info.hslab->span_lst = H5S__hyper_copy_span(
            space1->select.sel_info.hslab->span_lst, space1->extent.rank);
      if (!can_own_span2) {
        b_not_a = H5S__hyper_copy_span(space2_span_lst, space1->extent.rank);
        if (H5S__hyper_add_disjoint_spans(*result, b_not_a) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
                      "can't append hyperslabs");

        /* The new_spans are now owned by 'space', so they should not be
         * released */
        b_not_a = NULL;
      } /* end if */
      else {
    ...
    case H5S_SELECT_NOTA:
      if (!is_result_new) {
        ...
        if (H5S__hyper_free_span_info(space1->select.sel_info.hslab->span_lst) <
            0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTFREE, FAIL,
                      "unable to free span info");
...
static herr_t H5S__generate_hyperslab(H5S_t *space, H5S_seloper_t op,
  ...
  if (NULL == (new_spans = H5S__hyper_make_spans(space->extent.rank, start,
                                                 stride, count, block)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
  ...
  if (op == H5S_SELECT_SET) {
    ...
    if (H5S__fill_in_new_space(space, op, new_spans, TRUE, &new_spans_owned,
                               &updated_spans, &space) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSELECT, FAIL,
                  "can't generate the specified hyperslab");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5S__set_regular_hyperslab(H5S_t *space, const hsize_t start[],
  ...
  if (space->select.sel_info.hslab->unlim_dim >= 0) {
    ...
    for (u = 0; u < space->extent.rank; u++)
      if ((int)u != space->select.sel_info.hslab->unlim_dim)
        space->select.sel_info.hslab->num_elem_non_unlim *=
            (opt_count[u] * opt_block[u]);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__hyper_regular_and_single_block(H5S_t *space,
                                                  const hsize_t start[],
                                                  const hsize_t block[]) {
  ...
  if (single_block) {
    ...
    for (u = 0; u < space->extent.rank; u++) {
      ...
      /* Check for overlap */
      if (!H5S_RANGE_OVERLAP(space->select.sel_info.hslab->diminfo.opt[u].start,
                             select_end, start[u], block_end)) {
        overlap = FALSE;
        break;
      } /* end if */
      ...
      new_start[u] =
          MAX(space->select.sel_info.hslab->diminfo.opt[u].start, start[u]);
    ...
    if (overlap) {
      ...
      if (H5S__set_regular_hyperslab(
              space, new_start, H5S_hyper_ones_g, H5S_hyper_ones_g, new_block,
              H5S_hyper_ones_g, H5S_hyper_ones_g, new_block) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL,
                    "can't set regular hyperslab selection");
    } /* end if */
    else
    ...
    hbool_t
        partial_first_span; /* Whether first span in intersection is partial */
    hbool_t
        partial_last_span; /* Whether last span in intersection is partial */
    ...
    for (u = 0; u < space->extent.rank; u++) {
      ...
      if (space->select.sel_info.hslab->diminfo.opt[u].start >= start[u]) {
        ...
        first_span_end = (first_span_start +
                          space->select.sel_info.hslab->diminfo.opt[u].block) -
                         1;
      ...
      if (select_end < block_end) {
        ...
        last_span_start =
            (select_end - space->select.sel_info.hslab->diminfo.opt[u].block) +
            1;
        ...
        /* Adjust end coord for selection's 'start' offset */
        adj_end =
            block_end - space->select.sel_info.hslab->diminfo.opt[u].start;

        /* Compute # of strides into the selection */
        if (space->select.sel_info.hslab->diminfo.opt[u].count > 1)
          nstride =
              adj_end / space->select.sel_info.hslab->diminfo.opt[u].stride;
        else
          nstride = 0;
    ...
    if (overlap) {
      /* Set selection to regular hyperslab */
      if (H5S__set_regular_hyperslab(space, new_start, stride, new_count,
                                     new_block, stride, new_count,
                                     new_block) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL,
                    "can't set regular hyperslab selection");

      /* If there's a partial first or last span, have to 'AND' against
       * selection */
      if (partial_first_span || partial_last_span) {
        /* Generate span tree for regular selection */
        if (H5S__hyper_generate_spans(space) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_UNINITIALIZED, FAIL,
                      "dataspace does not have span tree");

        /* 'AND' against block */
        if (H5S__generate_hyperslab(space, H5S_SELECT_AND, start,
                                    H5S_hyper_ones_g, H5S_hyper_ones_g,
                                    block) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
                      "can't generate hyperslabs");
      } /* end if */
...
herr_t H5S_select_hyperslab(H5S_t *space, H5S_seloper_t op,
  ...
  hsize_t
      int_stride[H5S_MAX_RANK];    /* Internal storage for stride information */
  ...
  for (u = 0; u < space->extent.rank; u++) {
    ...
    if ((count[u] == H5S_UNLIMITED) || (block[u] == H5S_UNLIMITED)) {
      if (unlim_dim >= 0)
        HGOTO_ERROR(
            H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
  ...
  if ((H5S_GET_SELECT_TYPE(space) == H5S_SEL_HYPERSLABS) &&
    ...
    if (!((op == H5S_SELECT_AND) || (op == H5S_SELECT_NOTA)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
                  "unsupported operation on unlimited selection");
    assert(space->select.sel_info.hslab->diminfo_valid);

    /* Clip unlimited selection to include new selection */
    if (H5S_hyper_clip_unlim(
            space, start[space->select.sel_info.hslab->unlim_dim] +
    ...
    assert((space->select.num_elem > (hsize_t)0) ||
  ...
  case H5S_SEL_NONE: /* No elements selected in dataspace */
    ...
    case H5S_SELECT_XOR:   /* Binary "xor" operation for hyperslabs */
    case H5S_SELECT_NOTA:  /* Binary "B not A" operation for hyperslabs */
      op = H5S_SELECT_SET; /* Maps to "set" operation when applied to "none"
                              selection */
      break;
  ...
  case H5S_SEL_ALL: /* All elements selected in dataspace */
    ...
    case H5S_SELECT_NOTB: /* Binary "A not B" operation for hyperslabs */
      ...
      /* Then allow operation to proceed */
      {
        ...
        if (H5S_select_hyperslab(space, H5S_SELECT_SET, tmp_start, tmp_stride,
                                 tmp_count, tmp_block) < 0)
  ...
  case H5S_SEL_POINTS:        /* Can't combine hyperslab operations and point
  ...
  if (op == H5S_SELECT_SET) {
    /* Set selection to regular hyperslab */
    if (H5S__set_regular_hyperslab(space, start, stride, count, block,
                                   opt_stride, opt_count, opt_block) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL,
                  "can't set regular hyperslab selection");
  } /* end if */
    ...
    if (unlim_dim >= 0) {
      ...
      if (space->select.sel_info.hslab->unlim_dim >= 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
                    "cannot modify unlimited selection with another unlimited "
                    "selection");
       ...
       * existing selection.
       */
      H5S__hyper_get_clip_diminfo(start[unlim_dim], opt_stride[unlim_dim],
                                  &tmp_count, &tmp_block,
    ...
    if (H5S_SELECT_AND == op && single_block &&
      ...
      if (H5S__generate_hyperslab(space, op, start, opt_stride, opt_count,
                                  opt_block) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
                    "can't generate hyperslabs");
    } /* end else */
...
herr_t H5Sselect_hyperslab(hid_t space_id, H5S_seloper_t op,
  ...
  if (H5S_SCALAR == H5S_GET_EXTENT_TYPE(space))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "hyperslab doesn't support H5S_SCALAR space");
  if (H5S_NULL == H5S_GET_EXTENT_TYPE(space))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "hyperslab doesn't support H5S_NULL space");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S_combine_hyperslab(old_space, op, start, stride, count, block,
new_space) H5S_t *old_space;            IN: The old space the selection is
performed on H5S_seloper_t op;            IN: Operation to perform on current
selection const hsize_t start[];       IN: Offset of start of hyperslab const
hsize_t *stride;       IN: Hyperslab stride const hsize_t count[];       IN:
Number of blocks included in hyperslab const hsize_t *block;        IN: Size of
block in hyperslab H5S_t **new_space;           OUT: The new dataspace to store
the selection result RETURNS Non-negative on success/Negative on failure
 DESCRIPTION
    Combines a hyperslab selection with the current selection for a dataspace.
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_combine_hyperslab(const H5S_t *old_space, H5S_seloper_t op,
                             const hsize_t start[], const hsize_t *stride,
                             const hsize_t count[], const hsize_t *block,
                             H5S_t **new_space) {
  unsigned u;                 /* Local index variable */
  ...
  if (H5S_GET_SELECT_TYPE(old_space) == H5S_SEL_HYPERSLABS) {
    ...
    for (u = 0; u < old_space->extent.rank; u++) {
      ...
      new_high_bounds[u] =
          start[u] + stride[u] * (count[u] - 1) + (block[u] - 1);
    } /* end for */
    ...
    if (!overlapped) {
      ...
      case H5S_SELECT_XOR:
        ...
        if (NULL == (new_spans = H5S__hyper_make_spans(
                         old_space->extent.rank, start, stride, count, block)))
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
                      "can't create hyperslab information");
        if (NULL != old_space->select.sel_info.hslab->span_lst)
          (*new_space)->select.sel_info.hslab->span_lst =
              H5S__hyper_copy_span(old_space->select.sel_info.hslab->span_lst,
                                   old_space->extent.rank);
      ...
      case H5S_SELECT_NOTB:
        if (NULL != old_space->select.sel_info.hslab->span_lst) {
          if (NULL == ((*new_space)->select.sel_info.hslab->span_lst =
                           H5S__hyper_copy_span(
                               old_space->select.sel_info.hslab->span_lst,
                               old_space->extent.rank)))
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                        "unable to copy dataspace");
        } /* end if */
      ...
      case H5S_SELECT_NOTA:
        if (H5S__set_regular_hyperslab(*new_space, start, stride, count, block,
                                       stride, count, block) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL,
                      "can't set regular selection");
        break;
  ...
  /* Note: a little overhead in calling the function as some conditions are
   * checked again */
...
static herr_t H5S__fill_in_select(H5S_t *space1, H5S_seloper_t op,
  ...
  if (H5S__fill_in_new_space(space1, op,
                             space2->select.sel_info.hslab->span_lst, FALSE,
                             &span2_owned, &updated_spans, result) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSELECT, FAIL,
                "can't create the specified selection");
  ...
  if (updated_spans) {
    if (space2->select.sel_info.hslab->diminfo_valid == H5S_DIMINFO_VALID_YES) {
      if (H5S__hyper_update_diminfo(
              *result, op, space2->select.sel_info.hslab->diminfo.opt) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOUNT, FAIL,
                    "can't update hyperslab info");
    } /* end if */
    else
...
hid_t H5Scombine_hyperslab(hid_t space_id, H5S_seloper_t op,
  ...
  if (start == NULL || count == NULL)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
                "hyperslab not specified");
  if (!(op >= H5S_SELECT_SET && op <= H5S_SELECT_NOTA))
    HGOTO_ERROR(H5E_ARGS, H5E_UNSUPPORTED, H5I_INVALID_HID,
                "invalid selection operation");

  /* Generate new space, with combination of selections */
  if (H5S_combine_hyperslab(space, op, start, stride, count, block,
                            &new_space) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, H5I_INVALID_HID,
                "unable to set hyperslab selection");

  /* Register */
  if ((ret_value = H5I_register(H5I_DATASPACE, new_space, TRUE)) < 0)
    HGOTO_ERROR(H5E_ID, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register dataspace ID");
...
static H5S_t *H5S__combine_select(H5S_t *space1, H5S_seloper_t op,
  ...
  if (NULL == space1->select.sel_info.hslab->span_lst)
    if (H5S__hyper_generate_spans(space1) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_UNINITIALIZED, NULL,
                  "dataspace does not have span tree");
  ...
  if (NULL == space2->select.sel_info.hslab->span_lst) {
    ...
    if (H5S_combine_hyperslab(space1, op, tmp_start, tmp_stride, tmp_count,
                              tmp_block, &new_space) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, NULL,
                  "unable to set hyperslab selection");
  } /* end if */
  else {
    /* Combine new_space (a copy of space 1) & space2, with the result in
     * new_space */
    if (H5S__fill_in_select(space1, op, space2, &new_space) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCLIP, NULL,
                  "can't clip hyperslab information");
  } /* end else */
...
herr_t H5S__modify_select(H5S_t *space1, H5S_seloper_t op, H5S_t *space2) {
  ...
  if (NULL == space2->select.sel_info.hslab->span_lst) {
    ...
    if (H5S_select_hyperslab(space1, op, tmp_start, tmp_stride, tmp_count,
                             tmp_block) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                  "unable to set hyperslab selection");
  } /* end if */
    ...
    if (H5S__fill_in_select(space1, op, space2, &space1) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCLIP, FAIL,
                  "can't perform operation on two selections");
...
herr_t H5Smodify_select(hid_t space1_id, H5S_seloper_t op, hid_t space2_id) {
  ...
  if (H5S__modify_select(space1, op, space2) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                "unable to modify hyperslab selection");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__hyper_proj_int_build_proj(udata)
        H5S_hyper_project_intersect_ud_t *udata; IN/OUT: Persistent shared data
for iteration RETURNS Non-negative on success/Negative on failure. DESCRIPTION
...
static herr_t
H5S__hyper_proj_int_build_proj(H5S_hyper_project_intersect_ud_t *udata) {
  H5S_hyper_span_info_t *copied_span_info =
      NULL;                   /* Temporary span info pointer */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (udata->skip > 0) {
    ...
    do {
      ...
      if (udata->ds_span[udata->depth]->down) {
        if (udata->ds_low[udata->depth] <= udata->ds_span[udata->depth]->high) {
          ...
          if (udata->skip <
              H5S__hyper_spans_nelem_helper(udata->ds_span[udata->depth]->down,
                                            0, udata->op_gen) *
        ...
        assert(udata->ds_low[udata->depth] <=
               udata->ds_span[udata->depth]->high);
        if (udata->skip < (udata->ds_span[udata->depth]->high -
                           udata->ds_low[udata->depth] + 1)) {
        ...
        /* Skip over this entire span */
        udata->skip -= udata->ds_span[udata->depth]->high -
                       udata->ds_low[udata->depth] + 1;
      } /* end else */
    ...
    } while ((udata->skip > 0) || (udata->ds_low[udata->depth] >
                                   udata->ds_span[udata->depth]->high));
    ...
    while (udata->skip > 0) {
      ...
      udata->depth++;
      udata->ds_span[udata->depth] =
          udata->ds_span[udata->depth - 1]->down->head;
      udata->ds_low[udata->depth] = udata->ds_span[udata->depth]->low;
      if (udata->ds_span[udata->depth]->down) {
        do {
          /* If we will run out of elements to skip in this span,
  ...
  do {
    ...
    if (udata->ds_span[udata->depth]->down) {
      if (udata->ds_low[udata->depth] <= udata->ds_span[udata->depth]->high) {
         ...
         */
        if (udata->nelem <
            H5S__hyper_spans_nelem_helper(udata->ds_span[udata->depth]->down, 0,
                                          udata->op_gen) *
                (udata->ds_span[udata->depth]->high -
                 udata->ds_low[udata->depth] + 1)) {
          if (udata->nelem >=
            ...
            if (udata->share_selection) {
              if (H5S__hyper_append_span(
                      &udata->ps_span_info[udata->depth],
                      udata->ds_rank - udata->depth,
                      udata->ds_low[udata->depth],
                      udata->ds_low[udata->depth] +
                          (udata->nelem / udata->ds_span[udata->depth]
                                              ->down->op_info[0]
                                              .u.nelmts) -
                          1,
               ...
               * afterwards) */
              if (NULL ==
                  (copied_span_info = H5S__hyper_copy_span_helper(
        ...
        udata->nelem -=
            udata->ds_span[udata->depth]->down->op_info[0].u.nelmts *
            (udata->ds_span[udata->depth]->high - udata->ds_low[udata->depth] +
             1);
      } /* end if */
    }   /* end if */
    else {
      ...
      if (udata->nelem < (udata->ds_span[udata->depth]->high -
                          udata->ds_low[udata->depth] + 1)) {
        if (H5S__hyper_append_span(
                &udata->ps_span_info[udata->depth], 1,
                udata->ds_low[udata->depth],
    ...
    if (udata->ds_span[udata->depth]->down) {
      ...
      do {
        ...
        if (H5S__hyper_append_span(&udata->ps_span_info[udata->depth], 1,
          ...
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTAPPEND, FAIL,
                      "can't allocate hyperslab span");
        udata->nelem -= udata->ds_span[udata->depth]->high -
                        udata->ds_low[udata->depth] + 1;
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t
H5S__hyper_proj_int_iterate(ss_span_info,sis_span_info,count,depth,udata) const
H5S_hyper_span_info_t *ss_span_info; IN: Span tree for source selection const
H5S_hyper_span_info_t *sis_span_info; IN: Span tree for source intersect
selection hsize_t count;          IN: Number of times to compute the
intersection of ss_span_info and sis_span_info unsigned depth;         IN: Depth
of iteration (in terms of rank) H5S_hyper_project_intersect_ud_t *udata; IN/OUT:
Persistent shared data for iteration RETURNS Non-negative on success/Negative on
failure. DESCRIPTION Computes the intersection of ss_span_info and sis_span_info
and projects it to the projected space (held in udata).  It accomplishes this by
iterating over both spaces and computing the number of elements to skip (in
...
H5S__hyper_proj_int_iterate(H5S_hyper_span_info_t *ss_span_info,
  ...
  for (u = 0; u < (udata->ss_rank - depth); u++)
    if (!H5S_RANGE_OVERLAP(
            ss_span_info->low_bounds[u], ss_span_info->high_bounds[u],
  ...
  if (check_intersect) {
    ...
    for (u = 0; u < count; u++) {
      ...
      do {
        ...
        if (H5S_RANGE_OVERLAP(ss_low, ss_span->high, sis_low, sis_span->high)) {
          ...
          if (ss_span->down) {
            ...
            if (H5S__hyper_proj_int_iterate(ss_span->down, sis_span->down,
                                            high - low + 1, depth + 1,
                                            udata) < 0)
          ...
          if (ss_span->high < sis_low) {
            ...
            if (ss_span->down)
              ...
              H5S_HYPER_PROJ_INT_ADD_SKIP(udata, ss_span->high - ss_low + 1,
                                          FAIL);
      ...
      if (ss_span && !((depth == 0) && (u == count - 1))) {
        ...
        if (ss_span->down) {
          ...
          ss_span = ss_span->next;
          while (ss_span) {
            H5S_HYPER_PROJ_INT_ADD_SKIP(udata, ss_span->high - ss_span->low + 1,
                                        FAIL);
            ss_span = ss_span->next;
          } /* end while */
        }   /* end else */
    ...
    H5S_HYPER_PROJ_INT_ADD_SKIP(
        udata,
        H5S__hyper_spans_nelem_helper((H5S_hyper_span_info_t *)ss_span_info, 0,
                                      udata->op_gen) *
            count,
        FAIL);
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t
H5S__hyper_project_intersection(src_space,dst_space,src_intersect_space,proj_space,share_selection)
        H5S_t *src_space;       IN: Selection that is mapped to dst_space, and
intersected with src_intersect_space H5S_t *dst_space;       IN: Selection that
is mapped to src_space, and which contains the result H5S_t
*src_intersect_space; IN: Selection whose intersection with src_space is
projected to dst_space to obtain the result H5S_t *proj_space;      OUT: Will
contain the result (intersection of src_intersect_space and src_space projected
from src_space to dst_space) after the operation hbool_t share_selection; IN:
Whether we are allowed to share structures inside dst_space with proj_space
RETURNS Non-negative on success/Negative on failure. DESCRIPTION Projects the
intersection of of the selections of src_space and src_intersect_space within
the selection of src_space as a selection within the selection of dst_space. The
result is placed in the selection of proj_space.  Note src_space, dst_space, and
src_intersect_space do not need to use hyperslab selections, but they cannot use
point selections. The result is always a hyperslab or none selection.  Note also
that proj_space can share some span trees with dst_space, so proj_space must not
be subsequently modified if dst_space must be preserved. GLOBAL VARIABLES
COMMENTS, BUGS, ASSUMPTIONS EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S__hyper_project_intersection(H5S_t *src_space, H5S_t *dst_space,
                                       H5S_t *src_intersect_space,
                                       H5S_t *proj_space,
                                       hbool_t share_selection) {
  ...
  assert(H5S_GET_EXTENT_NDIMS(src_space) ==
         H5S_GET_EXTENT_NDIMS(src_intersect_space));
  assert(H5S_GET_SELECT_NPOINTS(src_space) ==
         H5S_GET_SELECT_NPOINTS(dst_space));
  ...
  if (H5S_GET_SELECT_TYPE(src_space) == H5S_SEL_HYPERSLABS) {
    ...
    if (NULL ==
        (ss_span_info_buf = H5S__hyper_make_spans(
             H5S_GET_EXTENT_NDIMS(src_space), H5S_hyper_zeros_g,
             H5S_hyper_zeros_g, H5S_hyper_ones_g, src_space->extent.size)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                  "can't create span tree for ALL source space");
  ...
  if (H5S_GET_SELECT_TYPE(dst_space) == H5S_SEL_HYPERSLABS) {
    ...
    if (NULL == dst_space->select.sel_info.hslab->span_lst)
      if (H5S__hyper_generate_spans(dst_space) < 0)
        HGOTO_ERROR(
            H5E_DATASPACE, H5E_UNINITIALIZED, FAIL,
    ...
    if (NULL ==
        (ds_span_info_buf = H5S__hyper_make_spans(
             H5S_GET_EXTENT_NDIMS(dst_space), H5S_hyper_zeros_g,
  ...
  if (H5S__hyper_proj_int_iterate(
          ss_span_info, src_intersect_space->select.sel_info.hslab->span_lst, 1,
          0, &udata) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOMPARE, FAIL,
                "selection iteration failed");
  ...
  if (udata.ps_span_info[0]) {
    ...
    if (NULL ==
        (proj_space->select.sel_info.hslab = H5FL_CALLOC(H5S_hyper_sel_t)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate hyperslab info");
    ...
    /* Set the number of elements in current selection */
    proj_space->select.num_elem =
        H5S__hyper_spans_nelem(proj_space->select.sel_info.hslab->span_lst);
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    void H5S__hyper_get_clip_diminfo(start,stride,count,block,clip_size)
        ...
        hsize_t *count;         IN/OUT: Count of hyperslab in unlimited
dimension hsize_t *block;         IN/OUT: Block of hyperslab in unlimited
dimension hsize_t clip_size;      IN: Extent that hyperslab will be clipped to
 RETURNS
    Non-negative on success/Negative on failure.
 DESCRIPTION
...
/*--------------------------------------------------------------------------
 ...
 RETURNS
    Non-negative on success/Negative on failure.
 DESCRIPTION
    This function changes the unlimited selection into a fixed-dimension
selection with the extent of the formerly unlimited dimension specified by
clip_size. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS Note this function does
not take the offset into account. EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_hyper_clip_unlim(H5S_t *space, hsize_t clip_size) {
  ...
  H5S_hyper_dim_t *diminfo =
      NULL; /* Convenience pointer to diminfo.opt in unlimited dimension */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Get initial diminfo */
  H5S__hyper_get_clip_diminfo(diminfo->start, diminfo->stride, &diminfo->count,
                              &diminfo->block, clip_size);
  ...
  /* Check for single block in unlimited dimension */
    ...
    /* Calculate number of elements */
    space->select.num_elem =
        diminfo->count * diminfo->block * hslab->num_elem_non_unlim;
  ...
  if (hslab && (H5S_DIMINFO_VALID_YES == hslab->diminfo_valid))
    hslab->diminfo.high_bounds[orig_unlim_dim] =
        ...
        hslab->diminfo.opt[orig_unlim_dim].stride *
            (hslab->diminfo.opt[orig_unlim_dim].count - 1) +
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hsize_t H5S__hyper_get_clip_extent_real(clip_space,num_slices,incl_trail)
        const H5S_t *clip_space, IN: Space that clip size will be calculated
based on hsize_t num_slizes,     IN: Number of slices clip_space should contain
when clipped hbool_t incl_trail;     IN: Whether to include trailing unselected
space RETURNS Clip extent to match num_slices (never fails) DESCRIPTION
...
static hsize_t H5S__hyper_get_clip_extent_real(const H5S_t *clip_space,
                                               hsize_t num_slices,
                                               hbool_t incl_trail) {
  const H5S_hyper_dim_t *diminfo; /* Convenience pointer to opt_unlim_diminfo in
                                     unlimited dimension */
  hsize_t count;
  hsize_t rem_slices;
  hsize_t ret_value = 0; /* Return value */
  ...
  diminfo = &clip_space->select.sel_info.hslab->diminfo
                 .opt[clip_space->select.sel_info.hslab->unlim_dim];
  ...
  if (num_slices == 0)
    ...
    if (rem_slices > 0)
      ...
      if (incl_trail)
        ...
        ret_value = diminfo->start + ((count - (hsize_t)1) * diminfo->stride) +
                    diminfo->block;
    } /* end else */
  }   /* end else */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hsize_t H5S__hyper_get_clip_extent(clip_space,match_space,incl_trail)
        const H5S_t *clip_space, IN: Space that clip size will be calculated
based on const H5S_t *match_space, IN: Space containing the same number of
elements as clip_space should after clipping hbool_t incl_trail;     IN: Whether
to include trailing unselected space RETURNS Calculated clip extent (never
fails) DESCRIPTION Calculates and returns the extent that clip_space should be
clipped to (via H5S_hyper_clip_unlim) in order for it to contain the same number
of elements as match_space.  If the clipped selection would end immediately
before a section of unselected space (i.e. at the end of a block), then if
incl_trail is TRUE, the returned clip extent is selected to include that
trailing "blank" space, otherwise it is selected to end at the end before the
blank space. GLOBAL VARIABLES COMMENTS, BUGS, ASSUMPTIONS Note this assumes the
...
hsize_t H5S_hyper_get_clip_extent(const H5S_t *clip_space,
  ...
  if (match_space->select.type->type == H5S_SEL_NONE)
    ...
    assert((match_space->select.num_elem %
            clip_space->select.sel_info.hslab->num_elem_non_unlim) == 0);
  } /* end else */

  /* Call "real" get_clip_extent function */
  ret_value =
      H5S__hyper_get_clip_extent_real(clip_space, num_slices, incl_trail);
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hsize_t
H5S__hyper_get_clip_extent_match(clip_space,match_space,match_clip_size,incl_trail)
        const H5S_t *clip_space, IN: Space that clip size will be calculated
based on const H5S_t *match_space, IN: Space that, after being clipped to
match_clip_size, contains the same number of elements as clip_space should after
clipping hsize_t match_clip_size, IN: Extent match_space would be clipped to to
match the number of elements in clip_space hbool_t incl_trail;     IN: Whether
to include trailing unselected space RETURNS Calculated clip extent (never
fails) DESCRIPTION Calculates and returns the extent that clip_space should be
clipped to (via H5S_hyper_clip_unlim) in order for it to contain the same number
of elements as match_space would have after being clipped to match_clip_size. If
the clipped selection would end immediately before a section of unselected space
(i.e. at the end of a block), then if incl_trail is TRUE, the returned clip
extent is selected to include that trailing "blank" space, otherwise it is
selected to end at the end before the blank space. GLOBAL VARIABLES COMMENTS,
BUGS, ASSUMPTIONS Note this assumes the offset has been normalized. EXAMPLES
REVISION LOG
--------------------------------------------------------------------------*/
hsize_t H5S_hyper_get_clip_extent_match(const H5S_t *clip_space,
                                        ...
                                        hbool_t incl_trail) {
  const H5S_hyper_dim_t
      *match_diminfo; /* Convenience pointer to opt_unlim_diminfo in unlimited
                         dimension in match_space */
  ...
  match_diminfo = &match_space->select.sel_info.hslab->diminfo
                       .opt[match_space->select.sel_info.hslab->unlim_dim];

  /* Get initial count and block */
  count = match_diminfo->count;
  block = match_diminfo->block;
  H5S__hyper_get_clip_diminfo(match_diminfo->start, match_diminfo->stride,
                              &count, &block, match_clip_size);
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    H5S_t *H5S_hyper_get_unlim_block(space,block_index)
        ...
        hsize_t block_index,    IN: Index of block to return in unlimited
dimension hbool_t incl_trail;     IN: Whether to include trailing unselected
space RETURNS New space on success/NULL on failure. DESCRIPTION Returns a space
containing only the block_indexth block in the unlimited dimension on space. All
blocks in all other dimensions are preserved. GLOBAL VARIABLES COMMENTS, BUGS,
ASSUMPTIONS Note this assumes the offset has been normalized. EXAMPLES REVISION
LOG
--------------------------------------------------------------------------*/
H5S_t *H5S_hyper_get_unlim_block(const H5S_t *space, hsize_t block_index) {
  ...
  hsize_t
      start[H5S_MAX_RANK]; /* Hyperslab selection info for unlim. selection */
  ...
  for (u = 0; u < space->extent.rank; u++) {
    if ((int)u == hslab->unlim_dim) {
      start[u] = hslab->diminfo.opt[u].start +
                 (block_index * hslab->diminfo.opt[u].stride);
      count[u] = (hsize_t)1;
    } /* end if */
    else {
      start[u] = hslab->diminfo.opt[u].start;
  ...
  if (NULL == (space_out = H5S_create(H5S_SIMPLE)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCREATE, NULL,
                "unable to create output dataspace");
  if (H5S__extent_copy_real(&space_out->extent, &space->extent, TRUE) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, NULL,
                "unable to copy destination space extent");

  /* Select block as defined by start/stride/count/block computed above */
  if (H5S_select_hyperslab(space_out, H5S_SELECT_SET, start, stride, count,
                           block) < 0)
  ...
  if (!ret_value)
    if (space_out && H5S_close(space_out) < 0)
      HDONE_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, NULL,
                  "unable to release dataspace");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hsize_t H5S_hyper_get_first_inc_block(space,clip_size,partial)
        ...
        hbool_t *partial;       OUT: Whether the ret_valueth block (first
incomplete block) is partial RETURNS Index of first incomplete block in
clip_size (never fails). DESCRIPTION Calculates and returns the index (as would
be passed to H5S_hyper_get_unlim_block()) of the first block in the unlimited
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
hsize_t H5S_hyper_get_first_inc_block(const H5S_t *space, hsize_t clip_size,
                                      hbool_t *partial) {
  H5S_hyper_sel_t *hslab; /* Convenience pointer to hyperslab info */
  H5S_hyper_dim_t
      *diminfo; /* Convenience pointer to diminfo in unlimited dimension */
  hsize_t ret_value = 0;
  ...
  if (diminfo->start >= clip_size) {
    ...
    ret_value =
        (clip_size - diminfo->start + diminfo->stride - diminfo->block) /
        diminfo->stride;
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5Sget_regular_hyperslab(hid_t spaceid, hsize_t start[] /*out*/,
                                hsize_t stride[] /*out*/,
                                hsize_t count[] /*out*/,
                                hsize_t block[] /*out*/) {
  H5S_t *space;               /* Dataspace to query */
  ...
  if (TRUE != H5S__hyper_is_regular(space))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "not a regular hyperslab selection");
