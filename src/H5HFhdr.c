...
H5HF_hdr_t *H5HF__hdr_alloc(H5F_t *f) {
  ...
  if (NULL == (hdr = H5FL_CALLOC(H5HF_hdr_t)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, NULL,
                "allocation failed for fractal heap shared header");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__hdr_compute_free_space(H5HF_hdr_t *hdr,
                                           unsigned iblock_row) {
  ...
  while (acc_heap_size < iblock_size) {
    acc_heap_size +=
        hdr->man_dtable.row_block_size[curr_row] * hdr->man_dtable.cparam.width;
...
herr_t H5HF__hdr_finish_init_phase1(H5HF_hdr_t *hdr) {
  ...
  hdr->heap_off_size =
      (uint8_t)H5HF_SIZEOF_OFFSET_BITS(hdr->man_dtable.cparam.max_index);
  if (H5HF__dtable_init(&hdr->man_dtable) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                "can't initialize doubling table info");

  /* Set the size of heap IDs */
  hdr->heap_len_size =
      (uint8_t)MIN(hdr->man_dtable.max_dir_blk_off_size,
...
haddr_t H5HF__hdr_create(H5F_t *f, const H5HF_create_t *cparam) {
  ...
  if (cparam->managed.start_block_size == 0)
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, HADDR_UNDEF,
                "starting block size must be greater than zero");
  if (!POWER_OF_TWO(cparam->managed.start_block_size))
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, HADDR_UNDEF,
                "starting block size not power of two");
  if (cparam->managed.max_direct_size == 0)
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, HADDR_UNDEF,
                "max. direct block size must be greater than zero");
  if (cparam->managed.max_direct_size > H5HF_MAX_DIRECT_SIZE_LIMIT)
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, HADDR_UNDEF,
                "max. direct block size too large");
  if (!POWER_OF_TWO(cparam->managed.max_direct_size))
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, HADDR_UNDEF,
                "max. direct block size not power of two");
  ...
  if (cparam->managed.max_index == 0)
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, HADDR_UNDEF,
                "max. heap size must be greater than zero");
#endif /* NDEBUG */
  ...
  if (NULL == (hdr = H5HF__hdr_alloc(f)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, HADDR_UNDEF,
                "can't allocate space for shared heap info");

#ifndef NDEBUG
  if (cparam->managed.max_index > (unsigned)(8 * hdr->sizeof_size))
    HGOTO_ERROR(H5E_HEAP, H5E_BADVALUE, HADDR_UNDEF,
                "max. heap size too large for file");
#endif /* NDEBUG */

  /* Set the creation parameters for the heap */
  hdr->max_man_size = cparam->max_man_size;
  hdr->checksum_dblocks = cparam->checksum_dblocks;
  H5MM_memcpy(&(hdr->man_dtable.cparam), &(cparam->managed),
              sizeof(H5HF_dtable_cparam_t));
  ...
  /* Set "huge" object tracker v2 B-tree address to indicate that there aren't
   * any yet */
  ...
  if (cparam->pline.nused > 0) {
    ...
    if (H5Z_can_apply_direct(&(cparam->pline)) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, HADDR_UNDEF,
                  "I/O filters can't operate on this heap");
    ...
    if (NULL == H5O_msg_copy(H5O_PLINE_ID, &(cparam->pline), &(hdr->pline)))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTCOPY, HADDR_UNDEF,
    ...
    if (H5O_pline_set_version(hdr->f, &(hdr->pline)) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTSET, HADDR_UNDEF,
                  "can't set version of I/O filter pipeline");

    /* Compute the I/O filters' encoded size */
    if (0 == (hdr->filter_len = (unsigned)H5O_msg_raw_size(
                  hdr->f, H5O_PLINE_ID, FALSE, &(hdr->pline))))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTGETSIZE, HADDR_UNDEF,
                  "can't get I/O filter pipeline size");

    /* Compute size of header on disk */
    hdr->heap_size =
        H5HF_HEADER_SIZE(hdr) /* Base header size */
  ...
  case 0: /* Set the length of heap IDs to just enough to hold the offset &
             length of 'normal' objects in the heap */
    hdr->id_len = (unsigned)1 + hdr->heap_off_size + hdr->heap_len_size;
    break;

  case 1: /* Set the length of heap IDs to just enough to hold the information
             needed to directly access 'huge' objects in the heap */
    if (hdr->filter_len > 0)
  ...
  if (HADDR_UNDEF == (hdr->heap_addr = H5MF_alloc(f, H5FD_MEM_FHEAP_HDR,
                                                  (hsize_t)hdr->heap_size)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, HADDR_UNDEF,
                "file allocation failed for fractal heap header");

  /* Cache the new fractal heap header */
  if (H5AC_insert_entry(f, H5AC_FHEAP_HDR, hdr->heap_addr, hdr,
                        H5AC__NO_FLAGS_SET) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTINSERT, HADDR_UNDEF,
                "can't add fractal heap header to cache");
  ...
  if (!H5_addr_defined(ret_value) && hdr)
    if (H5HF__hdr_free(hdr) < 0)
      HDONE_ERROR(H5E_HEAP, H5E_CANTRELEASE, HADDR_UNDEF,
...
H5HF_hdr_t *H5HF__hdr_protect(H5F_t *f, haddr_t addr, unsigned flags) {
  ...
  if (NULL == (hdr = (H5HF_hdr_t *)H5AC_protect(f, H5AC_FHEAP_HDR, addr,
                                                &cache_udata, flags)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, NULL,
                "unable to protect fractal heap header");
  ...
  if (hdr->rc == 0)
    if (H5AC_pin_protected_entry(hdr) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTPIN, FAIL,
                  "unable to pin fractal heap header");
  ...
  if (hdr->rc == 0) {
    ...
    if (H5AC_unpin_entry(hdr) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPIN, FAIL,
                  "unable to unpin fractal heap header");
  } /* end if */
  ...
  if (hdr->filter_len > 0)
    if (H5AC_resize_entry(hdr, (size_t)hdr->heap_size) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTRESIZE, FAIL,
                  "unable to resize fractal heap header");

  /* Mark header as dirty in cache */
  if (H5AC_mark_entry_dirty(hdr) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTMARKDIRTY, FAIL,
                "unable to mark fractal heap header as dirty");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__hdr_adjust_heap(H5HF_hdr_t *hdr, hsize_t new_size,
                             hssize_t extra_free) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__hdr_start_iter(H5HF_hdr_t *hdr, H5HF_indirect_t *iblock,
                            hsize_t curr_off, unsigned curr_entry) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__hdr_skip_blocks(H5HF_hdr_t *hdr, H5HF_indirect_t *iblock,
  ...
  if (H5HF__hdr_inc_iter(hdr, sect_size, nentries) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTRELEASE, FAIL,
                "can't increase allocated heap size");
  ...
  if (hdr->man_dtable.curr_root_rows == 0) {
    if (H5HF__man_iblock_root_create(hdr, min_dblock_size) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTEXTEND, FAIL,
                  "unable to create root indirect block");
  } /* end if */
  else {
    H5HF_indirect_t *iblock; /* Pointer to indirect block */
    hbool_t walked_up,
        walked_down; /* Condition variables for finding direct block location */
    ...
    min_dblock_row =
        H5HF__dtable_size_to_row(&hdr->man_dtable, min_dblock_size);
    ...
    if (!H5HF__man_iter_ready(&hdr->next_block)) {
      ...
      if (H5HF__man_iter_start_offset(hdr, &hdr->next_block,
                                      hdr->man_iter_off) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                    "unable to set block iterator location");
    } /* end if */
    ...
    if (min_dblock_row > next_row && next_row < iblock->nrows) {
      ...
      unsigned
          skip_entries; /* Number of entries to skip in the current block */
      ...
      if (min_dblock_row >= iblock->nrows)
        skip_entries =
            (iblock->nrows * hdr->man_dtable.cparam.width) - next_entry;
      else
    ...
    do {
      ...
      while (next_row >= iblock->nrows) {
        ...
        if (iblock->parent == NULL) {
          if (H5HF__man_iblock_root_double(hdr, min_dblock_size) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTEXTEND, FAIL,
                        "unable to double root indirect block");
        } /* end if */
        else {
          ...
          if (H5HF__man_iter_next(hdr, &hdr->next_block, 1) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTINC, FAIL,
                        "can't advance fractal heap block location");
        } /* end else */
      ...
      if (next_row >= hdr->man_dtable.max_direct_rows) {
        ...
        child_nrows = H5HF__dtable_size_to_rows(
            &hdr->man_dtable, hdr->man_dtable.row_block_size[next_row]);

        /* Check for skipping over indirect blocks */
        /* (that don't have direct blocks large enough to hold direct block size
         * requested) */
        if (hdr->man_dtable.row_block_size[child_nrows - 1] < min_dblock_size) {
          unsigned child_rows_needed; /* Number of rows needed to hold direct
                                         block */
          ...
          child_rows_needed =
              (H5VM_log2_of2((uint32_t)min_dblock_size) -
               H5VM_log2_of2(
                   (uint32_t)hdr->man_dtable.cparam.start_block_size)) +
              2;
          assert(child_rows_needed > child_nrows);
          child_entry = (next_row + (child_rows_needed - child_nrows)) *
                        hdr->man_dtable.cparam.width;
          ...
          hbool_t
              did_protect; /* Whether we protected the indirect block or not */
          haddr_t new_iblock_addr; /* New indirect block's address */

          /* Allocate new indirect block */
          if (H5HF__man_iblock_create(hdr, iblock, next_entry, child_nrows,
                                      child_nrows, &new_iblock_addr) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTALLOC, FAIL,
                        "can't allocate fractal heap indirect block");

          /* Lock new indirect block */
          if (NULL ==
              (new_iblock = H5HF__man_iblock_protect(
                   hdr, new_iblock_addr, child_nrows, iblock, next_entry, FALSE,
          ...
          if (min_dblock_size > hdr->man_dtable.cparam.start_block_size) {
            unsigned
                new_entry; /* Entry of direct block which is large enough */
          ...
          if (H5HF__man_iblock_unprotect(new_iblock, H5AC__NO_FLAGS_SET,
                                         did_protect) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HF__hdr_inc_iter(H5HF_hdr_t *hdr, hsize_t adv_size,
                          unsigned nentries) {
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5HF__hdr_reverse_iter(H5HF_hdr_t *hdr, haddr_t dblock_addr) {
  ...
  if (!H5HF__man_iter_ready(&hdr->next_block))
    /* Start iterator with previous offset of iterator */
    if (H5HF__man_iter_start_offset(hdr, &hdr->next_block, hdr->man_iter_off) <
        0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTINIT, FAIL,
                  "unable to set block iterator location");

  /* Walk backwards through heap, looking for direct block to place iterator
   * after */

  /* Get information about current iterator location */
  if (H5HF__man_iter_curr(&hdr->next_block, NULL, NULL, &curr_entry, &iblock) <
      0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
   ...
   */
  do {
    int tmp_entry; /* Temp. entry for iterator (use signed value to detect
                      errors) */
    ...
    while (tmp_entry >= 0 &&
           (H5_addr_eq(iblock->ents[tmp_entry].addr, dblock_addr) ||
    ...
    if (tmp_entry < 0) {
      ...
      if (iblock->parent) {
        ...
        /* Get information about current iterator location */
        if (H5HF__man_iter_curr(&hdr->next_block, NULL, NULL, &curr_entry,
                                &iblock) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                      "unable to retrieve current block iterator information");
      ...
      if (row < hdr->man_dtable.max_direct_rows) {
        ...
        /* Set the current location of the iterator to next entry after the
         * existing direct block */
        if (H5HF__man_iter_set_entry(hdr, &hdr->next_block, curr_entry) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTSET, FAIL,
                      "unable to set current block iterator location");

        /* Update iterator offset */
        hdr->man_iter_off = iblock->block_off;
        hdr->man_iter_off +=
            hdr->man_dtable
                .row_block_off[curr_entry / hdr->man_dtable.cparam.width];
        hdr->man_iter_off +=
            hdr->man_dtable
                .row_block_size[curr_entry / hdr->man_dtable.cparam.width] *
        ...
        /* Compute # of rows in next child indirect block to use */
        child_nrows = H5HF__dtable_size_to_rows(
            &hdr->man_dtable, hdr->man_dtable.row_block_size[row]);

        /* Lock child indirect block */
        if (NULL ==
            (child_iblock = H5HF__man_iblock_protect(
                 hdr, iblock->ents[curr_entry].addr, child_nrows, iblock,
...
herr_t H5HF__hdr_free(H5HF_hdr_t *hdr) {
  ...
  if (H5HF__dtable_dest(&hdr->man_dtable) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                "unable to destroy fractal heap doubling table");
...
herr_t H5HF__hdr_delete(H5HF_hdr_t *hdr) {
  unsigned cache_flags =
      H5AC__NO_FLAGS_SET;     /* Flags for unprotecting heap header */
  herr_t ret_value = SUCCEED; /* Return value */
...
#ifndef NDEBUG
  {
    ...
    if (H5AC_get_entry_status(hdr->f, hdr->heap_addr, &hdr_status) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                  "unable to check metadata cache status for heap header");
  ...
  if (H5_addr_defined(hdr->fs_addr))
    ...
    if (H5HF__space_delete(hdr) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                  "unable to release fractal heap free space manager");
  ...
  if (H5_addr_defined(hdr->man_dtable.table_addr)) {
    if (hdr->man_dtable.curr_root_rows == 0) {
      ...
      if (H5HF__man_dblock_delete(hdr->f, hdr->man_dtable.table_addr,
                                  dblock_size) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTFREE, FAIL,
                    "unable to release fractal heap root direct block");
    } /* end if */
    else {
      /* Delete root indirect block */
      if (H5HF__man_iblock_delete(hdr, hdr->man_dtable.table_addr,
  ...
  if (H5AC_unprotect(hdr->f, H5AC_FHEAP_HDR, hdr->heap_addr, hdr, cache_flags) <
      0)
