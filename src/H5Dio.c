...
static herr_t H5D__ioinfo_init(size_t count, H5D_io_op_type_t op_type,
                               H5D_dset_io_info_t *dset_info,
                               H5D_io_info_t *io_info);
static herr_t H5D__dset_ioinfo_init(H5D_t *dset, H5D_dset_io_info_t *dset_info,
                                    H5D_storage_t *store);
static herr_t H5D__typeinfo_init(H5D_io_info_t *io_info,
                                 H5D_dset_io_info_t *dset_info,
                                 hid_t mem_type_id);
...
herr_t H5D__read(size_t count, H5D_dset_io_info_t *dset_info) {
  ...
  H5S_t **orig_mem_space =
      NULL; /* If not NULL, ptr to an array of dataspaces       */
  ...
  H5D_storage_t *store =
      &store_local;      /* Union of EFL and chunk pointer in file space */
  size_t io_op_init = 0; /* Number I/O ops that have been initialized */
  size_t io_skipped = 0; /* Number I/O ops that have been skipped (due to the
                            dataset not being allocated) */
  ...
  if (count > 1)
    if (NULL ==
        (store = (H5D_storage_t *)H5MM_malloc(count * sizeof(H5D_storage_t))))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                  "couldn't allocate dset storage info array buffer");
  ...
  if (!(H5F_HAS_FEATURE(dset_info[0].dset->oloc.file, H5FD_FEAT_HAS_MPI))) {
    ...
    if (io_xfer_mode == H5FD_MPIO_COLLECTIVE)
      HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                  "collective access for MPI-based drivers only");
  }    /* end if */
#endif /*H5_HAVE_PARALLEL*/
  ...
  for (i = 0; i < count; i++) {
    ...
    if (H5D__typeinfo_init(&io_info, &(dset_info[i]),
                           dset_info[i].mem_type_id) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "unable to set up type info");

    /* Make certain that the number of elements in each selection is the same,
     * and cache nelmts in dset_info */
    ...
    if (!(H5S_has_extent(dset_info[i].file_space)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "file dataspace does not have extent set");
    if (!(H5S_has_extent(dset_info[i].mem_space)))
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "memory dataspace does not have extent set");

    /* H5S_select_shape_same() has been modified to accept topologically
     * identical selections with different rank as having the same shape (if the
     * most rapidly changing coordinates match up), but the I/O code still has
     ...
     */
    if (dset_info[i].nelmts > 0 &&
        TRUE == H5S_SELECT_SHAPE_SAME(dset_info[i].mem_space,
                                      dset_info[i].file_space) &&
        H5S_GET_EXTENT_NDIMS(dset_info[i].mem_space) !=
            H5S_GET_EXTENT_NDIMS(dset_info[i].file_space)) {
      ptrdiff_t buf_adj = 0;
      ...
      if (!orig_mem_space) {
        if (count > 1) {
          /* Allocate buffer */
          if (NULL ==
              (orig_mem_space = (H5S_t **)H5MM_calloc(count * sizeof(H5S_t *))))
            HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
      ...
      if (H5S_select_construct_projection(
              orig_mem_space[i], &dset_info[i].mem_space,
      ...
      dset_info[i].buf.vp =
          (void *)(((uint8_t *)dset_info[i].buf.vp) + buf_adj);
    } /* end if */
    ...
    if (dset_info[i].nelmts > 0 &&
        ...
        !(dset_info[i].dset->shared->layout.ops->is_data_cached &&
          (*dset_info[i].dset->shared->layout.ops->is_data_cached)(
              dset_info[i].dset->shared))) {
      H5D_fill_value_t fill_status; /* Whether/How the fill value is defined */

      /* Retrieve dataset's fill-value properties */
      if (H5P_is_fill_value_defined(&dset_info[i].dset->shared->dcpl_cache.fill,
                                    &fill_status) < 0)
      ...
      if (fill_status == H5D_FILL_VALUE_UNDEFINED &&
          (dset_info[i].dset->shared->dcpl_cache.fill.fill_time ==
               H5D_FILL_TIME_ALLOC ||
           dset_info[i].dset->shared->dcpl_cache.fill.fill_time ==
               H5D_FILL_TIME_IFSET))
      ...
      /* If we're never going to fill this dataset, just leave the junk in the
       * user's buffer */
      if (dset_info[i].dset->shared->dcpl_cache.fill.fill_time !=
          H5D_FILL_TIME_NEVER)
        /* Go fill the user's selection with the dataset's fill value */
        if (H5D__fill(dset_info[i].dset->shared->dcpl_cache.fill.buf,
                      dset_info[i].dset->shared->type, dset_info[i].buf.vp,
      ...
      if (H5D__dset_ioinfo_init(dset_info[i].dset, &(dset_info[i]),
                                &(store[i])) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                    "unable to set up I/O operation");

      /* Sanity check that space is allocated, if there are elements */
      if (dset_info[i].nelmts > 0)
        assert((*dset_info[i].dset->shared->layout.ops->is_space_alloc)(
                   &dset_info[i].dset->shared->layout.storage) ||
               (dset_info[i].dset->shared->layout.ops->is_data_cached &&
                (*dset_info[i].dset->shared->layout.ops->is_data_cached)(
                    dset_info[i].dset->shared)) ||
  ...
  if (H5D__typeinfo_init_phase2(&io_info) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to set up type info (second phase)");
  ...
  if (H5D__typeinfo_init_phase3(&io_info) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to set up type info (third phase)");
  ...
  if (io_info.md_io_ops.multi_read_md) {
    ...
    if (io_info.piece_count > 0) {
      ...
      if (NULL == (io_info.sel_pieces = H5MM_malloc(
                       io_info.piece_count * sizeof(io_info.sel_pieces[0]))))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
    ...
    for (i = 0; i < count; i++)
      if (dset_info[i].layout_ops.mdio_init) {
        ...
        if ((dset_info[i].layout_ops.mdio_init)(&io_info, &(dset_info[i])) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                      "can't populate array of selected pieces");
    ...
    if (!H5D_LAYOUT_CB_PERFORM_IO(&io_info) && io_info.piece_count > 0) {
      if (NULL == (io_info.mem_spaces =
                       H5MM_malloc(io_info.piece_count * sizeof(H5S_t *))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "memory allocation failed for memory space list");
      if (NULL == (io_info.file_spaces =
                       H5MM_malloc(io_info.piece_count * sizeof(H5S_t *))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "memory allocation failed for file space list");
      if (NULL ==
          (io_info.addrs = H5MM_malloc(io_info.piece_count * sizeof(haddr_t))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "memory allocation failed for piece address list");
      if (NULL == (io_info.element_sizes =
                       H5MM_malloc(io_info.piece_count * sizeof(size_t))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "memory allocation failed for element size list");
      if (NULL ==
          (io_info.rbufs = H5MM_malloc(io_info.piece_count * sizeof(void *))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
    ...
    /* Make final selection I/O call if the multi_read callbacks did not perform
     * the actual I/O (if using selection I/O and either multi dataset or type
     * conversion) */
    ...
    if (io_info.using_mpi_vfd) {
      ...
      /* Only report the collective I/O mode if we're actually performing
       * collective I/O */
      if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
        H5CX_set_mpio_actual_io_mode(io_info.actual_io_mode);

        /* If we did selection I/O, report that we used "link chunk" mode, since
         * that's the most analogous to what selection I/O does */
  ...
  for (i = 0; i < io_op_init; i++)
    if (!dset_info[i].skip_io && dset_info[i].layout_ops.io_term &&
      ...
      HDONE_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL,
                  "unable to shut down I/O op info");

  /* Shut down datatype info for operation */
  if (H5D__typeinfo_term(&io_info) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL,
                "unable to shut down type info");
...
herr_t H5D__write(size_t count, H5D_dset_io_info_t *dset_info) {
  ...
  for (i = 0; i < count; i++) {
    hbool_t should_alloc_space =
        FALSE; /* Whether or not to initialize dataset's storage */
    haddr_t prev_tag = HADDR_UNDEF;
    ...
    if (!dset_info[i].dset->shared->checked_filters) {
      if (H5Z_can_apply(dset_info[i].dset->shared->dcpl_id,
    ...
    if (H5F_HAS_FEATURE(dset_info[i].dset->oloc.file, H5FD_FEAT_HAS_MPI)) {
      /* If MPI based VFD is used, no VL or region reference datatype support
       * yet. */
      /* This is because they use the global heap in the file and we don't */
      /* support parallel access of that yet */
      if (H5T_is_vl_storage(dset_info[i].type_info.mem_type) > 0)
        HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                    "Parallel IO does not support writing VL or region "
    ...
    if (dset_info[i].nelmts > 0 &&
      ...
      /* Adjust the buffer by the given amount */
      dset_info[i].buf.cvp =
          (const void *)(((const uint8_t *)dset_info[i].buf.cvp) + buf_adj);
    } /* end if */
    ...
    /* Set up I/O operation */
    if (H5D__dset_ioinfo_init(dset_info[i].dset, &(dset_info[i]), &(store[i])) <
        0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "unable to set up I/O operation");
    ...
    if (should_alloc_space) {
      ...
      if (H5T_detect_class(dset_info[i].dset->shared->type, H5T_VLEN, FALSE))
        ...
        full_overwrite =
            (hbool_t)((hsize_t)file_nelmts == dset_info[i].nelmts ? TRUE
                                                                  : FALSE);

      /* Allocate storage */
      if (H5D__alloc_storage(dset_info[i].dset, H5D_ALLOC_WRITE, full_overwrite,
                             NULL) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
  ...
  if (io_info.md_io_ops.multi_write_md) {
    ...
    if (!H5D_LAYOUT_CB_PERFORM_IO(&io_info) && io_info.piece_count > 0) {
      ...
      if (NULL == (io_info.element_sizes =
        ...
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "memory allocation failed for element size list");
      if (NULL == (io_info.wbufs =
                       H5MM_malloc(io_info.piece_count * sizeof(const void *))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
    ...
    /* Make final selection I/O call if the multi_write callbacks did not
     * perform the actual I/O (if using selection I/O and either multi dataset
     * or type conversion) */
    if (!H5D_LAYOUT_CB_PERFORM_IO(&io_info)) {
      /* Check for type conversion */
      if (io_info.max_tconv_type_size > 0) {
        ...
        if (H5D__scatgath_write_select(&io_info) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                      "type conversion selection write failed");
      } else {
        /* Call selection I/O directly */
        H5_CHECK_OVERFLOW(io_info.pieces_added, size_t, uint32_t);
        if (H5F_shared_select_write(
                io_info.f_sh, H5FD_MEM_DRAW, (uint32_t)io_info.pieces_added,
                io_info.mem_spaces, io_info.file_spaces, io_info.addrs,
...
static herr_t H5D__ioinfo_init(size_t count, H5D_io_op_type_t op_type,
  ...
  /* Start with selection I/O mode from property list.  If enabled, layout
   * callback will turn it off if it is not supported by the layout.  Handling
   * of H5D_SELECTION_IO_MODE_AUTO occurs in H5D__ioinfo_adjust.
   */
  ...
  if (count > 0)
    io_info->using_mpi_vfd =
        H5F_HAS_FEATURE(dset_info[0].dset->oloc.file, H5FD_FEAT_HAS_MPI);
#endif /* H5_HAVE_PARALLEL */

  /* Check if we could potentially use in-place type conversion */
  if (op_type == H5D_IO_OP_READ)
    /* Always on for read (modulo other restrictions that are handled in layout
     * callbacks) */
    io_info->may_use_in_place_tconv = TRUE;
  else
    /* Only enable in-place type conversion if we're allowed to modify the write
     * buffer */
 ...
 *
 * Purpose:	Routine for determining correct I/O operations for each I/O
 *action.
 *
 * Return:	Non-negative on success/Negative on failure
 *
...
static herr_t H5D__typeinfo_init(H5D_io_info_t *io_info,
  ...
  if (NULL == (type_info->mem_type =
                   (const H5T_t *)H5I_object_verify(mem_type_id, H5I_DATATYPE)))
  ...
  if (type_info->is_xform_noop && type_info->is_conv_noop) {
    ...
    io_info->max_tconv_type_size =
        MAX3(io_info->max_tconv_type_size, type_info->src_type_size,
             type_info->dst_type_size);

    /* Check if we need a background buffer */
    if ((io_info->op_type == H5D_IO_OP_WRITE) &&
        H5T_detect_class(dset->shared->type, H5T_VLEN, FALSE))
...
static herr_t H5D__typeinfo_init_phase2(H5D_io_info_t *io_info) {
   ...
   * it (it will be turned off later if something else conflicts), otherwise
   * disable it */
  ...
  /* If we're doing type conversion and we might be doing selection I/O, check
   * if the buffers are large enough to handle the whole I/O */
  if (io_info->max_tconv_type_size &&
      io_info->use_select_io != H5D_SELECTION_IO_MODE_OFF) {
    size_t max_temp_buf; /* Maximum temporary buffer size */
    size_t i;            /* Local index variable */

    /* Collective I/O, conversion buffer must be large enough for entire I/O
     * (for now) */

    /* Calculate size of background buffer (tconv buf size was calculated in
     * layout io_init callbacks)
     */
    for (i = 0; i < io_info->count; i++) {
      ...
      if (type_info->need_bkg) {
        /* Add size of this dataset's background buffer to the global background
         * buffer size
         */
        io_info->bkg_buf_size +=
            io_info->dsets_info[i].nelmts * type_info->dst_type_size;

        /* Check if we need to fill the background buffer with the destination
         * contents */
    ...
    if (H5CX_get_max_temp_buf(&max_temp_buf) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't retrieve max. temp. buf size");
  ...
  if (io_info->using_mpi_vfd) {
    ...
    if ((opt = H5D__mpio_opt_possible(io_info)) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_BADRANGE, FAIL,
                  "invalid check for direct IO dataspace ");
    ...
    if (opt == TRUE) {
      ...
      if (io_info->op_type == H5D_IO_OP_WRITE) {
        ...
        if (H5F_shared_get_mpi_file_sync_required(io_info->f_sh,
                                                  &mpi_file_sync_required) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                      "can't get MPI file_sync_required flag");

        if (mpi_file_sync_required)
          HGOTO_ERROR(H5E_DATASET, H5E_NO_INDEPENDENT, FAIL,
                      "Can't perform independent write when MPI_File_sync is "
                      "required by ROMIO driver.");
      }
        ...
        if (i < io_info->count) {
          ...
          if ((comm_size = H5F_shared_mpi_get_size(io_info->f_sh)) < 0)
            HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                        "can't get MPI communicator size");

          if (comm_size > 1) {
            char local_no_coll_cause_string[512];
            char global_no_coll_cause_string[512];

            if (H5D__mpio_get_no_coll_cause_strings(
                    local_no_coll_cause_string, 512,
      ...
      if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
        ...
        if (H5CX_set_io_xfer_mode(H5FD_MPIO_INDEPENDENT) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                      "can't set MPI-I/O transfer mode");
      } /* end if */
    }   /* end else */
  }     /* end if */
...
static herr_t H5D__typeinfo_init_phase3(H5D_io_info_t *io_info) {
  ...
  if (io_info->max_tconv_type_size) {
    ...
    if (H5CX_get_tconv_buf(&tconv_buf) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't retrieve temp. conversion buffer pointer");
    ...
    if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
      /* Selection I/O, conversion buffers must be large enough for entire I/O
       * (for now) */

      /* Allocate global type conversion buffer (if any, could be none if
       * datasets in this I/O have 0 elements selected) */
      /* Allocating large buffers here will blow out all other type conversion
       * buffers on the free list.  Should we change this to a regular malloc?
       * Would require keeping track of which version of free() to call. -NAF */
      if (io_info->tconv_buf_size > 0) {
        if (NULL == (io_info->tconv_buf =
                         H5FL_BLK_MALLOC(type_conv, io_info->tconv_buf_size)))
      ...
      if (io_info->bkg_buf_size > 0) {
        if (NULL == (io_info->bkg_buf =
                         H5FL_BLK_MALLOC(type_conv, io_info->bkg_buf_size)))
          HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                      "memory allocation failed for type conversion");
        io_info->bkg_buf_allocated = TRUE;
      }
    } else {
      /* No selection I/O, only need to make sure it's big enough for one
       * element */
      size_t max_temp_buf; /* Maximum temporary buffer size */
      size_t target_size;  /* Desired buffer size	*/
      size_t i;

      /* Make sure selection I/O is disabled (DEFAULT should have been handled
       * by now) */
      ...
      /* If the buffer is too small to hold even one element (in the dataset
       * with the largest , try to make it bigger */
      if (target_size < io_info->max_tconv_type_size) {
        hbool_t default_buffer_info; /* Whether the buffer information are the
                                        defaults */

        /* Detect if we have all default settings for buffers */
        default_buffer_info =
            (hbool_t)((H5D_TEMP_BUF_SIZE == max_temp_buf) &&
      ...
      if (NULL == (io_info->tconv_buf = (uint8_t *)tconv_buf)) {
        ...
        if (NULL ==
            (io_info->tconv_buf = H5FL_BLK_MALLOC(type_conv, target_size)))
          HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                      "memory allocation failed for type conversion");
        io_info->tconv_buf_allocated = TRUE;
      } /* end if */
      ...
      for (i = 0; i < io_info->count; i++) {
        ...
        type_info->request_nelmts = target_size / MAX(type_info->src_type_size,
                                                      type_info->dst_type_size);

        /* Check if we need a background buffer and one hasn't been allocated
         * yet */
        if (type_info->need_bkg && (NULL == io_info->bkg_buf) &&
            (NULL == (io_info->bkg_buf = (uint8_t *)bkgr_buf))) {
          /* Allocate background buffer with the same size as the type
           * conversion buffer.  We can do this since the number of elements
           * that fit in the type conversion buffer will never be larger than
           * the number that could fit in a background buffer of equal size,
           * since the tconv element size is max(src, dst) and the bkg element
           * size is dst */
          if (NULL ==
              (io_info->bkg_buf = H5FL_BLK_MALLOC(type_conv, target_size)))
            HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
