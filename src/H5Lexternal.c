...
static ssize_t H5L__extern_query(const char *link_name, const void *udata,
                                 size_t udata_size, void *buf /*out*/,
                                 size_t buf_size);
...
static hid_t H5L__extern_traverse(const char H5_ATTR_UNUSED *link_name,
                                  hid_t cur_group, const void *_udata,
                                  size_t H5_ATTR_UNUSED udata_size,
                                  hid_t lapl_id, hid_t H5_ATTR_UNUSED dxpl_id) {
  ...
  const uint8_t *p =
      (const uint8_t *)_udata; /* Pointer into external link buffer */
  ...
  hid_t fapl_id =
      H5I_INVALID_HID;  /* File access property list for external link's file */
  ...
  char local_group_name[H5L_EXT_TRAVERSE_BUF_SIZE]; /* Local buffer to hold
                                                       group name */
  H5P_genplist_t *fa_plist; /* File access property list pointer */
  H5F_close_degree_t fc_degree =
      H5F_CLOSE_WEAK;                /* File close degree for target file */
  ...
  if (((*p >> 4) & 0x0F) > H5L_EXT_VERSION)
    HGOTO_ERROR(H5E_LINK, H5E_CANTDECODE, H5I_INVALID_HID,
                "bad version number for external link");
  if ((*p & 0x0F) & ~H5L_EXT_FLAGS_ALL)
    HGOTO_ERROR(H5E_LINK, H5E_CANTDECODE, H5I_INVALID_HID,
                "bad flags for external link");
  p++;
  ...
  if ((fapl_id == H5P_DEFAULT) &&
      ((fapl_id = H5F_get_access_plist(loc.oloc->file, FALSE)) < 0))
    HGOTO_ERROR(H5E_LINK, H5E_CANTGET, H5I_INVALID_HID,
  ...
  if (cb_info.func) {
    ...
    if (H5G_get_name(&loc, NULL, (size_t)0, &group_name_len, NULL) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_CANTGET, H5I_INVALID_HID,
                  "unable to retrieve length of group name");
    ...
    if (group_name_len > sizeof(local_group_name)) {
      if (NULL == (parent_group_name = (char *)H5MM_malloc(group_name_len)))
        HGOTO_ERROR(
            ...
            "can't allocate buffer to hold group name, group_name_len = %zu",
            group_name_len);
    } /* end if */
    else
  ...
  if (H5P_peek(plist, H5L_ACS_ELINK_PREFIX_NAME, &elink_prefix) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, H5I_INVALID_HID,
                "can't get external link prefix");

  /* Search for the target file */
  if (NULL == (ext_file = H5F_prefix_open_file(loc.oloc->file, H5F_PREFIX_ELINK,
                                               elink_prefix, file_name, intent,
                                               fapl_id)))
    HGOTO_ERROR(H5E_LINK, H5E_CANTOPENFILE, H5I_INVALID_HID,
                "unable to open external file, external link file name = '%s'",
                file_name);

  /* Retrieve the "group location" for the file's root group */
  if (H5G_root_loc(ext_file, &root_loc) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_BADVALUE, H5I_INVALID_HID,
  ...
  if (NULL == (ext_obj = H5O_open_name(&root_loc, obj_name, &opened_type)))
    HGOTO_ERROR(H5E_LINK, H5E_CANTOPENOBJ, H5I_INVALID_HID,
  ...
  if ((ext_obj_id = H5VL_wrap_register(opened_type, ext_obj, TRUE)) < 0)
    HGOTO_ERROR(H5E_ID, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register external link object");

  /* Set return value */
  ret_value = ext_obj_id;

done:
  /* XXX (VOL MERGE): Probably also want to consider closing ext_obj here on
   * failures */
  /* Release resources */
  ...
  if (ext_file && H5F_efc_close(loc.oloc->file, ext_file) < 0)
    HDONE_ERROR(H5E_LINK, H5E_CANTCLOSEFILE, H5I_INVALID_HID,
                "problem closing external file");
 ...
 *-------------------------------------------------------------------------
 */
static ssize_t H5L__extern_query(const char H5_ATTR_UNUSED *link_name,
                                 const void *_udata, size_t udata_size,
                                 void *buf /*out*/, size_t buf_size) {
  const uint8_t *udata =
      (const uint8_t *)_udata; /* Pointer to external link buffer */
