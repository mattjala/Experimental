 ...
 *-------------------------------------------------------------------------
 */
herr_t H5HL_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                  int fwidth) {
  H5HL_t *h = NULL;
  int free_block;
  ...
  if (NULL == (h = (H5HL_t *)H5HL_protect(f, addr, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                "unable to load/protect local heap");

  fprintf(stream, "%*sLocal Heap...\n", indent, "");
  fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth,
          "Header size (in bytes):", h->prfx_size);
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "Address of heap data:", h->dblk_addr);
  fprintf(stream, "%*s%-*s %zu\n", indent, "", fwidth,
          "Data bytes allocated for heap:", h->dblk_size);
  ...
  for (free_block = 0, freelist = h->freelist; freelist;
       freelist = freelist->next, free_block++) {
    char temp_str[32];
  ...
  H5_buffer_dump(stream, indent, h->dblk_image, marker, (size_t)0,
                 h->dblk_size);

done:
  if (h && FAIL == H5HL_unprotect(h))
    HDONE_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                "unable to release/unprotect local heap");
