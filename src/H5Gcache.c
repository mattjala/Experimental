...
static herr_t H5G__cache_node_get_initial_load_size(void *udata,
                                                    size_t *image_len);
...
static herr_t H5G__cache_node_serialize(const H5F_t *f, void *image, size_t len,
                                        void *thing);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__cache_node_get_initial_load_size(void *_udata,
                                                    size_t *image_len) {
 ...
 *              node, allocate an instance of H5G_node_t, load the contents of
 *the image into it, and return a pointer to the instance.
 *
 ...
 *-------------------------------------------------------------------------
 */
static void *H5G__cache_node_deserialize(const void *_image, size_t len,
                                         void *_udata,
                                         hbool_t H5_ATTR_UNUSED *dirty) {
  H5F_t *f = (H5F_t *)_udata; /* User data for callback */
  H5G_node_t *sym = NULL;     /* Symbol table node created */
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer to image to deserialize */
  const uint8_t *image_end =
      image + len - 1; /* Pointer to end of image buffer */
  void *ret_value = NULL;
  ...
  if (H5_IS_BUFFER_OVERFLOW(image, H5_SIZEOF_MAGIC, image_end))
    HGOTO_ERROR(H5E_SYM, H5E_OVERFLOW, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__cache_node_serialize(const H5F_t *f, void *_image,
