 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_create_components(H5F_t *f, H5O_stab_t *stab,
                                   size_t size_hint) {
  ...
  if (NULL == (heap = H5HL_protect(f, stab->heap_addr, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_SYM, H5E_PROTECT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_create(H5O_loc_t *grp_oloc, const H5O_ginfo_t *ginfo,
                        H5O_stab_t *stab) {
    ...
    heap_hint =
        ...
        (ginfo->est_num_entries *
         H5HL_ALIGN(ginfo->est_name_len +
                    1)) + /* estimated size of names for links, aligned for
                             inserting into local heap */
  ...
  if (H5G__stab_create_components(grp_oloc->file, stab, size_hint) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL,
                "can't create symbol table components");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_insert_real(H5F_t *f, const H5O_stab_t *stab, const char *name,
                             H5O_link_t *obj_lnk, H5O_type_t obj_type,
                             const void *crt_info) {
  H5HL_t *heap = NULL;        /* Pointer to local heap */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_insert(const H5O_loc_t *grp_oloc, const char *name,
                        H5O_link_t *obj_lnk, H5O_type_t obj_type,
                        const void *crt_info) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_remove(const H5O_loc_t *loc, H5RS_str_t *grp_full_path_r,
                        const char *name) {
  H5HL_t *heap = NULL;        /* Pointer to local heap */
  H5O_stab_t stab;            /*symbol table message		*/
  ...
  /* Pin the heap down in memory */
  if (NULL ==
      (heap = H5HL_protect(loc->file, stab.heap_addr, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_SYM, H5E_PROTECT, FAIL,
                "unable to protect symbol table heap");
...
herr_t H5G__stab_remove_by_idx(const H5O_loc_t *grp_oloc,
  ...
  /* Pin the heap down in memory */
  if (NULL ==
      (heap = H5HL_protect(grp_oloc->file, stab.heap_addr, H5AC__NO_FLAGS_SET)))
    HGOTO_ERROR(H5E_SYM, H5E_PROTECT, FAIL,
                "unable to protect symbol table heap");
...
herr_t H5G__stab_delete(H5F_t *f, const H5O_stab_t *stab) {
  ...
  if (H5B_delete(f, H5B_SNODE, stab->btree_addr, &udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                "unable to delete symbol table B-tree");
  ...
  if (H5HL_delete(f, stab->heap_addr) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                "unable to delete symbol table heap");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_iterate(const H5O_loc_t *oloc, H5_iter_order_t order,
  ...
  /* Pin the heap down in memory */
  if (NULL ==
      (heap = H5HL_protect(oloc->file, stab.heap_addr, H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_SYM, H5E_PROTECT, FAIL,
                "unable to protect symbol table heap");
  ...
  if (order != H5_ITER_DEC) {
    ...
    if ((ret_value = H5B_iterate(oloc->file, H5B_SNODE, stab.btree_addr,
                                 H5G__node_iterate, &udata)) < 0)
      HERROR(H5E_SYM, H5E_CANTNEXT, "iteration operator failed");

    /* Check for too high of a starting index (ex post facto :-) */
    /* (Skipping exactly as many entries as are in the group is currently an
     * error) */
    ...
    /* Iterate over the group members */
    if (H5B_iterate(oloc->file, H5B_SNODE, stab.btree_addr,
                    H5G__node_build_table, &udata) < 0)
...
herr_t H5G__stab_count(const H5O_loc_t *oloc, hsize_t *num_objs) {
  ...
  *num_objs = 0;
  ...
  /* Iterate over the group members */
  if (H5B_iterate(oloc->file, H5B_SNODE, stab.btree_addr, H5G__node_sumup,
                  num_objs) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_bh_size(H5F_t *f, const H5O_stab_t *stab,
  ...
  if (H5B_get_info(f, H5B_SNODE, stab->btree_addr, &bt_info,
                   H5G__node_iterate_size, &snode_size) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__stab_get_name_by_idx_cb(const H5G_entry_t *ent,
                                           void *_udata) {
  ...
  if ((udata->name = H5MM_strdup(name)) == NULL)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                "unable to duplicate symbol table link name");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_get_name_by_idx(const H5O_loc_t *oloc, H5_iter_order_t order,
  ...
  /* Iterate over the group members */
  if (H5B_iterate(oloc->file, H5B_SNODE, stab.btree_addr, H5G__node_by_idx,
                  &udata) < 0)
...
static herr_t H5G__stab_lookup_cb(const H5G_entry_t *ent, void *_udata) {
  H5G_stab_fnd_ud_t *udata =
      (H5G_stab_fnd_ud_t *)_udata; /* 'User data' passed in */
  herr_t ret_value = SUCCEED;      /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_lookup(const H5O_loc_t *grp_oloc, const char *name,
  ...
  /* Pin the heap down in memory */
  if (NULL == (heap = H5HL_protect(grp_oloc->file, stab.heap_addr,
  ...
  /* Search the B-tree */
  if (H5B_find(grp_oloc->file, H5B_SNODE, stab.btree_addr, found, &bt_udata) <
      0)
...
static herr_t H5G__stab_lookup_by_idx_cb(const H5G_entry_t *ent, void *_udata) {
  ...
  if ((name = (const char *)H5HL_offset_into(udata->heap, ent->name_off)) ==
      NULL)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                "unable to get symbol table link name");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__stab_lookup_by_idx(const H5O_loc_t *grp_oloc, H5_iter_order_t order,
  ...
  if (order == H5_ITER_DEC) {
    ...
    /* Iterate over the symbol table nodes, to count the links */
    if (H5B_iterate(grp_oloc->file, H5B_SNODE, stab.btree_addr, H5G__node_sumup,
                    &nlinks) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL, "iteration operator failed");
  ...
  /* Iterate over the group members */
  if (H5B_iterate(grp_oloc->file, H5B_SNODE, stab.btree_addr, H5G__node_by_idx,
                  &udata) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "iteration operator failed");
...
herr_t H5G__stab_valid(H5O_loc_t *grp_oloc, H5O_stab_t *alt_stab) {
  ...
  if (NULL == H5O_msg_read(grp_oloc, H5O_STAB_ID, &stab))
    HGOTO_ERROR(H5E_SYM, H5E_BADMESG, FAIL,
                "unable to read symbol table message");
  ...
  if (H5B_valid(grp_oloc->file, H5B_SNODE, stab.btree_addr) < 0) {
     ...
     * table message */
    if (!alt_stab ||
        H5B_valid(grp_oloc->file, H5B_SNODE, alt_stab->btree_addr) < 0)
  ...
  if (NULL == (heap = H5HL_protect(grp_oloc->file, stab.heap_addr,
     ...
     * table message */
    if (!alt_stab ||
        NULL == (heap = H5HL_protect(grp_oloc->file, alt_stab->heap_addr,
                                     H5AC__READ_ONLY_FLAG)))
  ...
  if (changed) {
    ...
    if (H5O_msg_write(grp_oloc, H5O_STAB_ID, 0,
                      H5O_UPDATE_TIME | H5O_UPDATE_FORCE, &stab) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL,
                  "unable to correct symbol table message");
  } /* end if */
