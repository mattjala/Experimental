...
/* Node in linked list of MPI data types created during traversal of irregular
 * hyperslab selection */
...
/* List to track MPI data types generated during traversal of irregular
 * hyperslab selection */
...
static herr_t H5S__mpio_all_type(const H5S_t *space, size_t elmt_size,
                                 MPI_Datatype *new_type, int *count,
                                 hbool_t *is_derived_type);
static herr_t H5S__mpio_none_type(MPI_Datatype *new_type, int *count,
                                  hbool_t *is_derived_type);
static herr_t H5S__mpio_create_point_datatype(size_t elmt_size,
                                              hsize_t num_points,
                                              MPI_Aint *disp,
                                              MPI_Datatype *new_type);
static herr_t H5S__mpio_point_type(const H5S_t *space, size_t elmt_size,
                                   MPI_Datatype *new_type, int *count,
                                   hbool_t *is_derived_type, hbool_t do_permute,
                                   hsize_t **permute_map, hbool_t *is_permuted);
static herr_t H5S__mpio_permute_type(H5S_t *space, size_t elmt_size,
                                     hsize_t **permute_map,
                                     MPI_Datatype *new_type, int *count,
                                     hbool_t *is_derived_type);
static herr_t H5S__mpio_reg_hyper_type(H5S_t *space, size_t elmt_size,
                                       MPI_Datatype *new_type, int *count,
                                       hbool_t *is_derived_type);
...
static herr_t H5S__obtain_datatype(H5S_hyper_span_info_t *spans,
                                   const hsize_t *down, size_t elmt_size,
...
static herr_t H5S__mpio_all_type(const H5S_t *space, size_t elmt_size,
  ...
  if ((snelmts = (hssize_t)H5S_GET_EXTENT_NPOINTS(space)) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "src dataspace has invalid selection");
...
static herr_t H5S__mpio_create_point_datatype(size_t elmt_size,
  ...
  hbool_t elmt_type_created =
      FALSE; /* Whether the element MPI datatype was created */
  int *inner_blocks =
      NULL; /* Arrays for MPI datatypes when "large" datatype needed */
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Type_contiguous((int)elmt_size, MPI_BYTE, &elmt_type)))
  ...
  if (bigio_count >= num_points) {
    ...
    if (MPI_SUCCESS != (mpi_code = MPI_Type_create_hindexed_block(
                            (int)num_points, 1, disp, elmt_type, new_type)))
    ...
    if (NULL == (blocks = (int *)H5MM_malloc(sizeof(int) * num_points)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate array of blocks");
    ...
    /* Create an MPI datatype for the whole point selection */
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Type_create_hindexed((int)num_points, blocks, disp,
                                             elmt_type, new_type)))
    ...
    /* use LARGE_DATATYPE::
     * We'll create an hindexed_block type for every 2G point count and then
     * combine those and any remaining points into a single large datatype.
     */
    int total_types, i;
    ...
    leftover =
        (hsize_t)num_points - (hsize_t)num_big_types * (hsize_t)bigio_count;
    ...
    if (NULL == (inner_types = (MPI_Datatype *)H5MM_malloc(
                     (sizeof(MPI_Datatype) * (size_t)total_types))))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate array of blocks");

    if (NULL ==
        (inner_blocks = (int *)H5MM_malloc(sizeof(int) * (size_t)total_types)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate array of blocks");

    if (NULL == (inner_disps = (MPI_Aint *)H5MM_malloc(sizeof(MPI_Aint) *
                                                       (size_t)total_types)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate array of blocks");
...
#else
      if (MPI_SUCCESS != (mpi_code = MPI_Type_create_hindexed(
                              (int)remaining_points, blocks,
                              &disp[num_big_types * bigio_count], elmt_type,
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Type_create_struct(total_types, inner_blocks,
                                           inner_disps, inner_types, new_type)))
 ...
 *              Create a permutation array to handle out-of-order point
 *selections.
 *
 * Return:	Non-negative on success, negative on failure.
 *
...
static herr_t H5S__mpio_point_type(const H5S_t *space, size_t elmt_size,
                                   MPI_Datatype *new_type, int *count,
                                   hbool_t *is_derived_type, hbool_t do_permute,
                                   hsize_t **permute, hbool_t *is_permuted) {
  MPI_Aint *disp = NULL; /* Datatype displacement for each point*/
  H5S_pnt_node_t *curr =
      NULL; /* Current point being operated on in from the selection */
  ...
  if (do_permute)
    if (NULL ==
        (*permute = (hsize_t *)H5MM_malloc(sizeof(hsize_t) * num_points)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate permutation array");
  ...
  for (u = 0; u < num_points; u++) {
    ...
    hsize_t disp_tmp =
        H5VM_array_offset(space->extent.rank, space->extent.size, curr->pnt);
    ...
    /* This is a File Space used to set the file view, so adjust the
     * displacements to have them monotonically non-decreasing. Generate the
     * permutation array by indicating at each point being selected, the
     * position it will shifted in the new displacement. Example: Suppose 4
     * points with corresponding are selected Pt 1: disp=6 ; Pt 2: disp=3 ; Pt
     * 3: disp=0 ; Pt 4: disp=4 The permute map to sort the displacements in
     * order will be: point 1: map[0] = L, indicating that this point is not
     * moved (1st point selected) point 2: map[1] = 0, indicating that this
     * point is moved to the first position, since disp_pt1(6) > disp_pt2(3)
     * point 3: map[2] = 0, move to position 0, bec it has the lowest disp
     * between the points selected so far. point 4: map[3] = 2, move the 2nd
     * position since point 1 has a higher disp, but points 2 and 3 have lower
     * displacements.
     */
    if (do_permute) {
  ...
  if (H5S__mpio_create_point_datatype(elmt_size, num_points, disp, new_type) <
      0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
                "couldn't create an MPI Datatype from point selection");
 ...
 *              selection and so the memory datatype has to be permuted using
 *the permutation map created by the file selection.
 *
 ...
 */
static herr_t H5S__mpio_permute_type(H5S_t *space, size_t elmt_size,
                                     hsize_t **permute, MPI_Datatype *new_type,
  ...
  if (NULL == (off = H5MM_malloc(H5D_IO_VECTOR_SIZE * sizeof(*off))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "can't allocate sequence offsets array");
  if (NULL == (len = H5MM_malloc(H5D_IO_VECTOR_SIZE * sizeof(*len))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "can't allocate sequence lengths array");
  ...
  while (max_elem > 0) {
    ...
    if (H5S_SELECT_ITER_GET_SEQ_LIST(sel_iter, (size_t)H5D_IO_VECTOR_SIZE,
                                     max_elem, &nseq, &nelem, off, len) < 0)
...
static herr_t H5S__mpio_reg_hyper_type(H5S_t *space, size_t elmt_size,
  ...
  if (sel_iter->u.hyp.iter_rank != 0 &&
      sel_iter->u.hyp.iter_rank < space->extent.rank) {
    /* Flattened selection */
    ...
    for (u = 0; u < rank; ++u) {
      ...
      if (H5DEBUG(S)) {
        fprintf(H5DEBUG(S),
                "%s: start=%" PRIdHSIZE "  stride=%" PRIuHSIZE
                "  count=%" PRIuHSIZE "  block=%" PRIuHSIZE
                "  xtent=%" PRIuHSIZE,
                __func__, d[u].start, d[u].strid, d[u].count, d[u].block,
                d[u].xtent);
        if (u == 0)
  ...
  **********************************************************************/
  ...
  if (H5DEBUG(S)) {
    i = ((int)rank) - 1;
    fprintf(H5DEBUG(S),
            " offset[%2d]=%" PRIuHSIZE "; max_xtent[%2d]=%" PRIuHSIZE "\n", i,
 ...
 *******************************************************/
  ...
  /* LARGE_DATATYPE::
   * Check if the number of elements to form the inner type fits into a 32 bit
   * integer. If yes then just create the innertype with MPI_Type_contiguous.
  ...
  if (bigio_count >= elmt_size) {
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Type_contiguous((int)elmt_size, MPI_BYTE, &inner_type)))
      HMPI_GOTO_ERROR(FAIL, "MPI_Type_contiguous failed", mpi_code)
  } else
  ...
  for (i = ((int)rank) - 1; i >= 0; --i) {
    ...
    if (H5DEBUG(S))
      fprintf(H5DEBUG(S),
              ...
              " max_xtent=%" PRIuHSIZE "\n",
              __func__, i, d[i].start, d[i].count, d[i].block, d[i].strid,
              d[i].xtent, max_xtent[i]);
#endif
     ...
     ****************************************/
    if (bigio_count >= d[i].count && bigio_count >= d[i].block &&
        bigio_count >= d[i].strid) {
      /* All the parameters fit into 32 bit integers so create the vector type
       * normally */
       ...
       *   2) a collection of elements where an element is a contiguous
       * block(1). Remember that the input arguments to the MPI-IO functions use
       * integer values to represent element counts.  We ARE allowed however, in
       * the more recent MPI implementations to use constructed datatypes
       * whereby the total number of bytes in a transfer could be :
      ...
      if (bigio_count < d[i].block) {
        if (H5_mpio_create_large_type(d[i].block, 0, inner_type, &block_type) <
            0)
          HGOTO_ERROR(
      ...
      if (bigio_count < d[i].count) {
        if (H5_mpio_create_large_type(d[i].count, stride_in_bytes, block_type,
                                      &outer_type) < 0)
          HGOTO_ERROR(
              H5E_DATASPACE, H5E_BADTYPE, FAIL,
    ...
    start_disp =
        (MPI_Aint)d[i].start * (MPI_Aint)offset[i] * (MPI_Aint)elmt_size;
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Type_get_extent(outer_type, &lb, &extent_len)))
    ...
    if (start_disp > 0 || extent_len < new_extent) {
      ...
      mpi_code = MPI_Type_create_hindexed(1, &block_len, &start_disp,
      ...
      mpi_code =
          MPI_Type_create_resized(interm_type, lb, new_extent, &inner_type);
  ...
  /* At this point inner_type is actually the outermost type, even for 0-trip
   * loop */
  *new_type = inner_type;
  if (MPI_SUCCESS != (mpi_code = MPI_Type_commit(new_type)))
    HMPI_GOTO_ERROR(FAIL, "MPI_Type_commit failed", mpi_code)
  ...
  *is_derived_type = TRUE;
  ...
  if (H5DEBUG(S))
    fprintf(H5DEBUG(S), "Leave %s, count=%d  is_derived_type=%s\n", __func__,
            *count, (*is_derived_type) ? "TRUE" : "FALSE");
#endif
  FUNC_LEAVE_NOAPI(ret_value)
...
static herr_t H5S__mpio_span_hyper_type(const H5S_t *space, size_t elmt_size,
  ...
  hbool_t elmt_type_is_derived =
      FALSE;                  /* Whether the element type has been created */
  ...
  if (H5S__obtain_datatype(space->select.sel_info.hslab->span_lst, down,
                           elmt_size, &elmt_type, &span_type, &type_list, 0,
                           op_gen) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
                "couldn't obtain MPI derived data type");
  ...
  if (H5S__release_datatype(&type_list) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                "couldn't release MPI derived data type");
 ...
 *
 * Purpose:	Release the MPI derived datatypes for span-tree hyperslab
 *selection
 *
 * Return:	Non-negative on success, negative on failure.
 *
...
static herr_t H5S__obtain_datatype(H5S_hyper_span_info_t *spans,
  ...
  hsize_t bigio_count;    /* Transition point to create derived type */
  size_t alloc_count =
      0;                 /* Number of span tree nodes allocated at this level */
  size_t outercount = 0; /* Number of span tree nodes at this level */
  MPI_Datatype *inner_type = NULL;
  hbool_t inner_types_freed =
      FALSE; /* Whether the inner_type MPI datatypes have been freed */
  int *blocklen = NULL;
  MPI_Aint *disp = NULL;
  ...
  if (spans->op_info[op_info_i].op_gen != op_gen) {
    H5S_mpio_mpitype_node_t
        *type_node; /* Pointer to new node in MPI data type list */

    /* Allocate the initial displacement & block length buffers */
    alloc_count = H5S_MPIO_INITIAL_ALLOC_COUNT;
    if (NULL ==
        (disp = (MPI_Aint *)H5MM_malloc(alloc_count * sizeof(MPI_Aint))))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate array of displacements");
    ...
    /* If this is the fastest changing dimension, it is the base case for
     * derived datatype. */
    span = spans->head;
    if (NULL == span->down) {
      hbool_t large_block =
          FALSE; /* Whether the block length is larger than 32 bit integer */
      ...
      while (span) {
        ...
        if (outercount >= alloc_count) {
          ...
          if (NULL == (tmp_disp = (MPI_Aint *)H5MM_realloc(
                           disp, alloc_count * sizeof(MPI_Aint))))
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                        "can't allocate array of displacements");
          disp = tmp_disp;
          if (NULL == (tmp_blocklen = (int *)H5MM_realloc(
                           blocklen, alloc_count * sizeof(int))))
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                        "can't allocate array of block lengths");
        ...
        if (bigio_count < (hsize_t)blocklen[outercount])
          large_block = TRUE; /* at least one block type is large, so set this
                                 flag to true */
      ...
      if (bigio_count >= outercount && large_block == FALSE) {
        if (MPI_SUCCESS != (mpi_code = MPI_Type_create_hindexed(
                                (int)outercount, blocklen, disp, *elmt_type,
        ...
        for (u = 0; u < outercount; u++) {
          ...
          /* create the block type from elmt_type while checking the 32 bit int
           * limit */
          if ((hsize_t)(blocklen[u]) > bigio_count) {
            if (H5_mpio_create_large_type((hsize_t)blocklen[u], 0, *elmt_type,
                                          &temp_type) < 0)
              HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
                          "couldn't create a large element datatype in "
                          "span_hyper selection");
          } /* end if */
          else if (MPI_SUCCESS !=
                   (mpi_code = MPI_Type_contiguous((int)blocklen[u], *elmt_type,
                                                   &temp_type)))
            HMPI_GOTO_ERROR(FAIL, "MPI_Type_contiguous failed", mpi_code)

          /* Combine the current datatype that is created with this current
           * block type */
          if (0 == u) /* first iteration, there is no combined datatype yet */
            ...
            MPI_Datatype dt[2] = {spans->op_info[op_info_i].u.down_type,
                                  temp_type};
      ...
      if (NULL == (inner_type = (MPI_Datatype *)H5MM_malloc(
      ...
      outercount = 0;
      while (span) {
        MPI_Datatype down_type; /* Temporary MPI datatype for a span tree node's
                                   children */
        hsize_t nelmts;         /* # of elements covered by current span */
         ...
         */
        /* First using MPI Type vector to build derived data type for this span
         * only */
        ...
        if (H5S__obtain_datatype(span->down, down + 1, elmt_size, elmt_type,
                                 &down_type, type_list, op_info_i, op_gen) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
                      "couldn't obtain MPI derived data type");
        ...
        H5_CHECK_OVERFLOW(nelmts, hsize_t, int);
        if (MPI_SUCCESS !=
            (mpi_code = MPI_Type_create_hvector(
                 (int)nelmts, 1, stride, down_type, &inner_type[outercount])))
          HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_hvector failed", mpi_code)
      ...
      if (MPI_SUCCESS != (mpi_code = MPI_Type_create_struct(
                              (int)outercount, blocklen, disp, inner_type,
                              &spans->op_info[op_info_i].u.down_type)))
        HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_struct failed", mpi_code)
    ...
    if (NULL == (type_node = H5FL_MALLOC(H5S_mpio_mpitype_node_t)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                  "can't allocate MPI data type list node");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5S_mpio_space_type(H5S_t *space, size_t elmt_size,
                           MPI_Datatype *new_type, int *count,
                           hbool_t *is_derived_type, hbool_t do_permute,
                           hsize_t **permute_map, hbool_t *is_permuted) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  case H5S_SIMPLE:
    ...
    if (TRUE == *is_permuted) {
      ...
      case H5S_SEL_HYPERSLABS:
        ...
        if (H5S__mpio_permute_type(space, elmt_size, permute_map, new_type,
                                   count, is_derived_type) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
    ...
    /* the file space is not permuted, so do a regular selection */
      ...
      case H5S_SEL_ALL:
        if (H5S__mpio_all_type(space, elmt_size, new_type, count,
                               is_derived_type) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
                      "couldn't convert 'all' selection to MPI type");
        break;
      ...
      case H5S_SEL_POINTS:
        if (H5S__mpio_point_type(space, elmt_size, new_type, count,
                                 is_derived_type, do_permute, permute_map,
                                 is_permuted) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
      ...
      case H5S_SEL_HYPERSLABS:
        if ((H5S_SELECT_IS_REGULAR(space) == TRUE)) {
          if (H5S__mpio_reg_hyper_type(space, elmt_size, new_type, count,
                                       is_derived_type) < 0)
            HGOTO_ERROR(
                H5E_DATASPACE, H5E_BADTYPE, FAIL,
