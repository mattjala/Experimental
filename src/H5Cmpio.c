 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_apply_candidate_list(H5F_t *f, H5C_t *cache_ptr,
                                unsigned num_candidates,
                                haddr_t *candidates_list_ptr, int mpi_rank,
                                int mpi_size) {
  ...
  for (u = 0; u < num_candidates; u++)
    HDsprintf(&(tbl_buf[HDstrlen(tbl_buf)]), " 0x%llx",
              (long long)(*(candidates_list_ptr + u)));
  ...
  if (f->shared->coll_md_write) {
    ...
    if (NULL ==
        (cache_ptr->coll_write_list = H5SL_create(H5SL_TYPE_HADDR, NULL)))
      HGOTO_ERROR(H5E_CACHE, H5E_CANTCREATE, FAIL,
                  "can't create skip list for entries");
  } /* end if */

  n = num_candidates / (unsigned)mpi_size;
  m = num_candidates % (unsigned)mpi_size;

  if (NULL == (candidate_assignment_table = (unsigned *)H5MM_malloc(
                   sizeof(unsigned) * (size_t)(mpi_size + 1))))
  ...
  for (u = 0; u <= (unsigned)mpi_size; u++)
    HDsprintf(&(tbl_buf[HDstrlen(tbl_buf)]), " %u",
              candidate_assignment_table[u]);
  HDsprintf(&(tbl_buf[HDstrlen(tbl_buf)]), "\n");
  fprintf(stdout, "%s", tbl_buf);

  fprintf(stdout, "%s:%d: flush entries [%u, %u].\n", __func__, mpi_rank,
          first_entry_to_flush, last_entry_to_flush);
  ...
  fprintf(stdout, "%s:%d: num candidates/to clear/to flush = %u/%u/%u.\n",
          __func__, mpi_rank, num_candidates, total_entries_to_clear,
          total_entries_to_flush);
#endif /* H5C_APPLY_CANDIDATE_LIST__DEBUG */
  ...
  if (f->shared->coll_md_write) {
    ...
    if (H5C__collective_write(f) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_WRITEERROR, FAIL,
                  "can't write metadata collectively");
  } /* end if */

done:
  if (candidate_assignment_table != NULL)
    candidate_assignment_table =
        (unsigned *)H5MM_xfree((void *)candidate_assignment_table);
...
herr_t H5C_construct_candidate_list__clean_cache(H5C_t *cache_ptr) {
   ...
   */
  assert(cache_ptr->dirty_index_size <=
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_mark_entries_as_clean(H5F_t *f, unsigned ce_array_len,
  ...
  assert((entries_cleared == ce_array_len) ||
...
herr_t H5C_clear_coll_entries(H5C_t *cache_ptr, hbool_t partial) {
  ...
  clear_cnt =
      (partial ? cache_ptr->coll_list_len / 2 : cache_ptr->coll_list_len);
...
static herr_t H5C__collective_write(H5F_t *f) {
  ...
  if (H5F_shared_vector_write(H5F_SHARED(f), count32, types, addrs, sizes,
                              bufs) < 0)
  ...
  if (orig_xfer_mode != H5FD_MPIO_COLLECTIVE)
    if (H5CX_set_io_xfer_mode(orig_xfer_mode) < 0)
      HDONE_ERROR(H5E_CACHE, H5E_CANTSET, FAIL,
                  "can't set MPI-I/O transfer mode");
...
H5C__flush_candidate_entries(H5F_t *f,
  ...
  while (ring < H5C_RING_NTYPES) {
    if (H5C__flush_candidates_in_ring(f, ring, entries_to_flush[ring],
                                      entries_to_clear[ring]) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                  "flush candidates in ring failed");
...
static herr_t H5C__flush_candidates_in_ring(H5F_t *f, H5C_ring_t ring,
...
#endif /* H5C_DO_SANITY_CHECKS */
  unsigned clear_flags = H5C__FLUSH_CLEAR_ONLY_FLAG | H5C__GENERATE_IMAGE_FLAG |
   ...
   */
  progress = TRUE;
  while (progress && ((entries_flushed < entries_to_flush) ||
                      (entries_cleared < entries_to_clear))) {
    progress = FALSE;
    entry_ptr = cache_ptr->pel_head_ptr;
    while ((entry_ptr != NULL) && ((entries_flushed < entries_to_flush) ||
                                   (entries_cleared < entries_to_clear))) {
