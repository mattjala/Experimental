...
/* Property callbacks */
static herr_t H5P__gcrt_group_info_enc(const void *value, void **_pp,
                                       size_t *size);
static herr_t H5P__gcrt_group_info_dec(const void **_pp, void *value);
static herr_t H5P__gcrt_link_info_enc(const void *value, void **_pp,
                                      size_t *size);
...
static const H5O_ginfo_t H5G_def_ginfo_g =
    H5G_CRT_GROUP_INFO_DEF; /* Default group info settings */
static const H5O_linfo_t H5G_def_linfo_g =
...
static herr_t H5P__gcrt_reg_prop(H5P_genclass_t *pclass) {
  ...
  if (H5P__register_real(pclass, H5G_CRT_GROUP_INFO_NAME,
                         H5G_CRT_GROUP_INFO_SIZE, &H5G_def_ginfo_g, NULL, NULL,
                         NULL, H5G_CRT_GROUP_INFO_ENC, H5G_CRT_GROUP_INFO_DEC,
                         NULL, NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");

  /* Register link info property */
  if (H5P__register_real(pclass, H5G_CRT_LINK_INFO_NAME, H5G_CRT_LINK_INFO_SIZE,
                         &H5G_def_linfo_g, NULL, NULL, NULL,
                         H5G_CRT_LINK_INFO_ENC, H5G_CRT_LINK_INFO_DEC, NULL,
                         NULL, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL,
                "can't insert property into class");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_link_phase_change(hid_t plist_id, unsigned max_compact,
                                unsigned min_dense) {
  H5P_genplist_t *plist;      /* Property list pointer */
  H5O_ginfo_t ginfo;          /* Group information structure */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (max_compact != H5G_CRT_GINFO_MAX_COMPACT ||
      min_dense != H5G_CRT_GINFO_MIN_DENSE)
 ...
 *              than the MAX_COMPACT # of entries (from
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_est_link_info(hid_t plist_id, unsigned est_num_entries,
  ...
  if (est_num_entries > 65535)
    HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL,
                "est. number of entries must be < 65536");
  if (est_name_len > 65535)
    HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL,
                "est. name length must be < 65536");
  ...
  if (est_num_entries != H5G_CRT_GINFO_EST_NUM_ENTRIES ||
      est_name_len != H5G_CRT_GINFO_EST_NAME_LEN)
...
herr_t H5Pset_link_creation_order(hid_t plist_id, unsigned crt_order_flags) {
  ...
  /* Update fields */
  linfo.track_corder =
      (hbool_t)((crt_order_flags & H5P_CRT_ORDER_TRACKED) ? TRUE : FALSE);
  linfo.index_corder =
      (hbool_t)((crt_order_flags & H5P_CRT_ORDER_INDEXED) ? TRUE : FALSE);
...
static herr_t H5P__gcrt_group_info_enc(const void *value, void **_pp,
  ...
  const H5O_ginfo_t *ginfo =
      (const H5O_ginfo_t *)value; /* Create local aliases for values */
...
static herr_t H5P__gcrt_link_info_enc(const void *value, void **_pp,
  ...
  const H5O_linfo_t *linfo =
      (const H5O_linfo_t *)value; /* Create local aliases for values */
  uint8_t **pp = (uint8_t **)_pp;
