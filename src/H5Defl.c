...
static ssize_t H5D__efl_readvv(const H5D_io_info_t *io_info,
                               ...
                               size_t dset_len_arr[], hsize_t dset_offset_arr[],
...
/* Helper routines */
static herr_t H5D__efl_read(const H5O_efl_t *efl, const H5D_t *dset,
                            haddr_t addr, size_t size, uint8_t *buf);
static herr_t H5D__efl_write(const H5O_efl_t *efl, const H5D_t *dset,
...
static herr_t H5D__efl_io_init(H5D_io_info_t *io_info,
  ...
  H5MM_memcpy(&dinfo->store->efl, &(dinfo->dset->shared->dcpl_cache.efl),
              sizeof(H5O_efl_t));
...
static herr_t H5D__efl_read(const H5O_efl_t *efl, const H5D_t *dset,
  ...
  for (u = 0, cur = 0; u < efl->nused; u++) {
    if (H5O_EFL_UNLIMITED == efl->slot[u].size ||
        addr < cur + efl->slot[u].size) {
      skip = addr - cur;
      break;
    } /* end if */
    cur += efl->slot[u].size;
  ...
  while (size) {
    ...
    if (H5_combine_path(dset->shared->extfile_prefix, efl->slot[u].name,
                        &full_name) < 0)
    ...
    if (HDlseek(fd, (HDoff_t)(efl->slot[u].offset + (HDoff_t)skip), SEEK_SET) <
        0)
...
static herr_t H5D__efl_write(const H5O_efl_t *efl, const H5D_t *dset,
  ...
  while (size) {
    ...
    if (H5_combine_path(dset->shared->extfile_prefix, efl->slot[u].name,
                        &full_name) < 0)
      HGOTO_ERROR(H5E_EFL, H5E_NOSPACE, FAIL, "can't build external file name");
    if ((fd = HDopen(full_name, O_CREAT | O_RDWR, H5_POSIX_CREATE_MODE_RW)) <
        0) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__efl_readvv_cb(hsize_t dst_off, hsize_t src_off, size_t len,
                                 void *_udata) {
  H5D_efl_readvv_ud_t *udata =
      (H5D_efl_readvv_ud_t *)_udata; /* User data for H5VM_opvv() operator */
  herr_t ret_value = SUCCEED;        /* Return value */

  FUNC_ENTER_PACKAGE

  /* Read data */
  if (H5D__efl_read(udata->efl, udata->dset, dst_off, len,
 ...
 */
static ssize_t
H5D__efl_readvv(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
                const H5D_dset_io_info_t *dset_info, size_t dset_max_nseq,
  ...
  if ((ret_value =
    ...
    HGOTO_ERROR(H5E_DATASET, H5E_CANTOPERATE, FAIL,
                "can't perform vectorized EFL read");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__efl_writevv_cb(hsize_t dst_off, hsize_t src_off, size_t len,
                                  void *_udata) {
  H5D_efl_writevv_ud_t *udata =
      (H5D_efl_writevv_ud_t *)_udata; /* User data for H5VM_opvv() operator */
  herr_t ret_value = SUCCEED;         /* Return value */

  FUNC_ENTER_PACKAGE

  /* Write data */
  if (H5D__efl_write(udata->efl, udata->dset, dst_off, len,
 ...
 */
static ssize_t
H5D__efl_writevv(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
                 const H5D_dset_io_info_t *dset_info, size_t dset_max_nseq,
                 size_t *dset_curr_seq, size_t dset_len_arr[],
                 hsize_t dset_off_arr[], size_t mem_max_nseq,
                 size_t *mem_curr_seq, size_t mem_len_arr[],
  ...
  if ((ret_value =
    ...
    HGOTO_ERROR(H5E_DATASET, H5E_CANTOPERATE, FAIL,
                "can't perform vectorized EFL write");
done:
  FUNC_LEAVE_NOAPI(ret_value)
...
herr_t H5D__efl_bh_info(H5F_t *f, H5O_efl_t *efl, hsize_t *heap_size) {
  ...
  if (H5HL_heapsize(f, efl->heap_addr, heap_size) < 0)
    HGOTO_ERROR(H5E_EFL, H5E_CANTINIT, FAIL,
                "unable to retrieve local heap info");
