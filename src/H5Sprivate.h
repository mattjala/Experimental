...
typedef struct {
  ...
  hsize_t off[H5S_MAX_RANK];  /* Offset in span node (used as position for
                                 regular hyperslabs) */
  ...
  H5S_hyper_dim_t
      diminfo[H5S_MAX_RANK];  /* "Flattened" regular selection information */
  ...
  hbool_t
      flattened[H5S_MAX_RANK]; /* Whether this dimension has been flattened */

  /* Irregular hyperslab selection fields */
  hsize_t loc_off[H5S_MAX_RANK]; /* Byte offset in buffer, for each dimension's
                                    current offset */
...
typedef struct H5S_sel_iter_t {
  ...
  hsize_t dims[H5S_MAX_RANK];     /* Dimensions of dataspace the selection is
                                     operating on */
...
#define H5S_SELECT_BOUNDS(S, START, END)                                       \
  ((*(S)->select.type->bounds)(S, START, END))
...
#define H5S_SELECT_PROJECT_SCALAR(S, O)                                        \
  ((*(S)->select.type->project_scalar)(S, O))
#define H5S_SELECT_PROJECT_SIMPLE(S, NS, O)                                    \
  ((*(S)->select.type->project_simple)(S, NS, O))
#define H5S_SELECT_ITER_COORDS(ITER, COORDS)                                   \
  ((*(ITER)->type->iter_coords)(ITER, COORDS))
#define H5S_SELECT_ITER_BLOCK(ITER, START, END)                                \
  ((*(ITER)->type->iter_block)(ITER, START, END))
#define H5S_SELECT_ITER_NELMTS(ITER) ((*(ITER)->type->iter_nelmts)(ITER))
#define H5S_SELECT_ITER_HAS_NEXT_BLOCK(ITER)                                   \
  ((*(ITER)->type->iter_has_next_block)(ITER))
#define H5S_SELECT_ITER_NEXT(ITER, NELEM)                                      \
  ((*(ITER)->type->iter_next)(ITER, NELEM))
#define H5S_SELECT_ITER_NEXT_BLOCK(ITER)                                       \
  ((*(ITER)->type->iter_next_block)(ITER))
#define H5S_SELECT_ITER_GET_SEQ_LIST(ITER, MAXSEQ, MAXBYTES, NSEQ, NBYTES,     \
                                     OFF, LEN)                                 \
  ((*(ITER)->type->iter_get_seq_list)(ITER, MAXSEQ, MAXBYTES, NSEQ, NBYTES,    \
                                      OFF, LEN))
...
#define H5S_SELECT_ITER_HAS_NEXT_BLOCK(ITER)                                   \
  (H5S_select_iter_has_next_block(ITER))
...
/* Handle these callbacks in a special way, since they have prologs that need to
 * be executed */
...
#define H5S_SELECT_INTERSECT_BLOCK(S, START, END)                              \
  (H5S_select_intersect_block(S, START, END))
#define H5S_SELECT_RELEASE(S) (H5S_select_release(S))
#define H5S_SELECT_DESERIALIZE(S, BUF, BUF_SIZE)                               \
  (H5S_select_deserialize(S, BUF, BUF_SIZE))
...
H5_DLL herr_t H5S_write(H5F_t *f, struct H5O_t *oh, unsigned update_flags,
                        H5S_t *ds);
...
H5_DLL int H5S_extent_get_dims(const H5S_extent_t *ext, hsize_t dims[],
                               hsize_t max_dims[]);
...
H5_DLL herr_t H5S_select_iterate(void *buf, const H5T_t *type, H5S_t *space,
                                 const H5S_sel_iter_op_t *op, void *op_data);
H5_DLL herr_t H5S_select_fill(const void *fill, size_t fill_size, H5S_t *space,
                              void *buf);
H5_DLL htri_t H5S_select_valid(const H5S_t *space);
H5_DLL hsize_t H5S_get_select_npoints(const H5S_t *space);
H5_DLL herr_t H5S_get_select_bounds(const H5S_t *space, hsize_t *start,
                                    hsize_t *end);
...
H5_DLL htri_t H5S_select_intersect_block(H5S_t *space, const hsize_t *start,
                                         const hsize_t *end);
...
H5_DLL herr_t H5S_select_project_simple(const H5S_t *space, H5S_t *new_space,
                                        hsize_t *offset);
H5_DLL herr_t H5S_select_project_intersection(H5S_t *src_space,
                                              H5S_t *dst_space,
                                              H5S_t *src_intersect_space,
                                              H5S_t **new_space_ptr,
                                              hbool_t share_space);
H5_DLL herr_t H5S_select_subtract(H5S_t *space, H5S_t *subtract_space);
H5_DLL herr_t H5S_select_contig_block(H5S_t *space, hbool_t *is_contig,
...
H5_DLL herr_t H5S_combine_hyperslab(const H5S_t *old_space, H5S_seloper_t op,
                                    const hsize_t start[],
...
H5_DLL herr_t H5S_select_iter_init(H5S_sel_iter_t *iter, H5S_t *space,
...
H5_DLL herr_t H5S_mpio_space_type(H5S_t *space, size_t elmt_size,
                                  /* out: */ MPI_Datatype *new_type, int *count,
