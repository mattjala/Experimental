...
static void *H5O__layout_decode(H5F_t *f, H5O_t *open_oh, unsigned mesg_flags,
                                ...
                                const uint8_t *p);
static herr_t H5O__layout_encode(H5F_t *f, hbool_t disable_shared, uint8_t *p,
                                 const void *_mesg);
static void *H5O__layout_copy(const void *_mesg, void *_dest);
static size_t H5O__layout_size(const H5F_t *f, hbool_t disable_shared,
                               const void *_mesg);
...
static void *H5O__layout_copy_file(H5F_t *file_src, void *mesg_src,
                                   H5F_t *file_dst, hbool_t *recompute_size,
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__layout_decode(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh,
  ...
  if (mesg->version < H5O_LAYOUT_VERSION_3) {
    ...
    if (H5D_CONTIGUOUS != mesg->type && H5D_CHUNKED != mesg->type &&
        H5D_COMPACT != mesg->type)
      HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, NULL,
                  "bad layout type for layout message");
    ...
    if (mesg->type != H5D_CHUNKED) {
      ...
      for (unsigned u = 0; u < ndims; u++) {
        ...
        if (mesg->u.chunk.dim[u] == 0)
          HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, NULL,
                      "bad chunk dimension value when parsing layout message - "
                      "chunk dimension "
    ...
    if (mesg->type == H5D_COMPACT) {
      ...
      if (mesg->storage.u.compact.size > 0) {
        /* Ensure that size doesn't exceed buffer size, due to possible data
         * corruption */
        if (H5_IS_BUFFER_OVERFLOW(p, mesg->storage.u.compact.size, p_end))
          HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                      "ran off end of input buffer while decoding");

        if (NULL == (mesg->storage.u.compact.buf =
                         H5MM_malloc(mesg->storage.u.compact.size)))
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                      "memory allocation failed for compact data buffer");
        H5MM_memcpy(mesg->storage.u.compact.buf, p,
                    mesg->storage.u.compact.size);
    ...
    case H5D_CHUNKED:
      if (mesg->version < H5O_LAYOUT_VERSION_4) {
        ...
        for (unsigned u = 0; u < mesg->u.chunk.ndims; u++) {
          ...
          /* Just in case that something goes very wrong, such as file
           * corruption. */
          if (mesg->u.chunk.dim[u] == 0)
            HGOTO_ERROR(
        ...
        if (mesg->u.chunk.enc_bytes_per_dim == 0 ||
            mesg->u.chunk.enc_bytes_per_dim > 8)
          HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, NULL,
                      "encoded chunk dimension size is too large");

        if (H5_IS_BUFFER_OVERFLOW(
                p, (mesg->u.chunk.ndims * mesg->u.chunk.enc_bytes_per_dim),
                p_end))
          HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                      "ran off end of input buffer while decoding");
        ...
        /* Chunk dimensions */
        for (unsigned u = 0; u < mesg->u.chunk.ndims; u++) {
          UINT64DECODE_VAR(p, mesg->u.chunk.dim[u],
                           mesg->u.chunk.enc_bytes_per_dim);
    ...
    case H5D_VIRTUAL:
      ...
      if (mesg->version < H5O_LAYOUT_VERSION_4)
        HGOTO_ERROR(H5E_OHDR, H5E_VERSION, NULL,
                    "invalid layout version with virtual layout");
      ...
      if (mesg->storage.u.virt.serial_list_hobjid.addr != HADDR_UNDEF) {
        ...
        if (NULL == (heap_block = (uint8_t *)H5HG_read(
                         f, &(mesg->storage.u.virt.serial_list_hobjid), NULL,
                         &block_size)))
          HGOTO_ERROR(H5E_OHDR, H5E_READERROR, NULL,
                      "Unable to read global heap block");
        ...
        if ((uint8_t)H5O_LAYOUT_VDS_GH_ENC_VERS != heap_vers)
          HGOTO_ERROR(H5E_OHDR, H5E_VERSION, NULL,
                      "bad version # of encoded VDS heap information, expected "
                      "%u, got %u",
                      (unsigned)H5O_LAYOUT_VDS_GH_ENC_VERS,
                      (unsigned)heap_vers);

        /* Number of entries */
        if (H5_IS_BUFFER_OVERFLOW(heap_block_p, H5F_sizeof_size(f),
                                  heap_block_p_end))
          HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                      "ran off end of input buffer while decoding");
        ...
        for (size_t i = 0; i < mesg->storage.u.virt.list_nused; i++) {
          ...
          /* Source file name */
          tmp_size =
              HDstrnlen((const char *)heap_block_p, (size_t)avail_buffer_space);
          ...
          H5MM_memcpy(mesg->storage.u.virt.list[i].source_file_name,
          ...
          H5MM_memcpy(mesg->storage.u.virt.list[i].source_dset_name,
                      heap_block_p, tmp_size);
          heap_block_p += tmp_size;
          ...
          if (H5S_SELECT_DESERIALIZE(
                  &mesg->storage.u.virt.list[i].source_select, &heap_block_p,
        ...
        /* Compute checksum */
        computed_chksum =
            H5_checksum_metadata(heap_block, block_size - (size_t)4, 0);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__layout_encode(H5F_t *f,
  ...
  /* Message version */
  *p++ = (uint8_t)((mesg->version < H5O_LAYOUT_VERSION_3) ? H5O_LAYOUT_VERSION_3
                                                          : mesg->version);
  ...
  *p++ = (uint8_t)mesg->type;
  ...
  case H5D_COMPACT:
    ...
    if (mesg->storage.u.compact.size > 0) {
      if (mesg->storage.u.compact.buf)
        H5MM_memcpy(p, mesg->storage.u.compact.buf,
                    mesg->storage.u.compact.size);
      else
  ...
  case H5D_CHUNKED:
    if (mesg->version < H5O_LAYOUT_VERSION_4) {
      ...
      assert(mesg->u.chunk.ndims > 0 &&
             mesg->u.chunk.ndims <= H5O_LAYOUT_NDIMS);
      ...
      *p++ = (uint8_t)mesg->u.chunk.ndims;
      ...
      /* Encoded # of bytes for each chunk dimension */
      assert(mesg->u.chunk.enc_bytes_per_dim > 0 &&
             mesg->u.chunk.enc_bytes_per_dim <= 8);
      *p++ = (uint8_t)mesg->u.chunk.enc_bytes_per_dim;

      /* Dimension sizes */
      for (u = 0; u < mesg->u.chunk.ndims; u++)
        UINT64ENCODE_VAR(p, mesg->u.chunk.dim[u],
                         mesg->u.chunk.enc_bytes_per_dim);
...
static void *H5O__layout_copy(const void *_mesg, void *_dest) {
  ...
  if (!dest && NULL == (dest = H5FL_MALLOC(H5O_layout_t)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, NULL,
                "layout message allocation failed");
  ...
  *dest = *mesg;
  ...
  case H5D_COMPACT:
    ...
    if (mesg->storage.u.compact.size > 0) {
      ...
      if (NULL == (dest->storage.u.compact.buf =
                       H5MM_malloc(dest->storage.u.compact.size)))
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__layout_size(const H5F_t *f,
                               hbool_t H5_ATTR_UNUSED disable_shared,
                               const void *_mesg) {
  const H5O_layout_t *mesg = (const H5O_layout_t *)_mesg;
...
static herr_t H5O__layout_pre_copy_file(H5F_t H5_ATTR_UNUSED *file_src,
  ...
  const H5O_layout_t *layout_src =
      (const H5O_layout_t *)mesg_src; /* Source layout */
  herr_t ret_value = SUCCEED;         /* Return value */
  ...
  if (layout_src->version >
      H5O_layout_ver_bounds[H5F_HIGH_BOUND(cpy_info->file_dst)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "layout message version out of bounds");
...
static void *H5O__layout_copy_file(H5F_t *file_src, void *mesg_src,
  ...
  case H5D_COMPACT:
    if (layout_src->storage.u.compact.buf) {
      ...
      if (H5D__compact_copy(file_src, &layout_src->storage.u.compact, file_dst,
                            &layout_dst->storage.u.compact, udata->src_dtype,
                            cpy_info) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, NULL,
                    "unable to copy chunked storage");
      copied = TRUE;
    } /* end if */
    break;
  ...
  case H5D_CHUNKED:
    if (H5D__chunk_is_space_alloc(&layout_src->storage) ||
      ...
      if (H5D__chunk_copy(
              file_src, &layout_src->storage.u.chunk, &layout_src->u.chunk,
              file_dst, &layout_dst->storage.u.chunk, udata->src_space_extent,
  ...
  case H5D_VIRTUAL:
    ...
    if (H5D__virtual_copy(file_dst, layout_dst) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, NULL,
                  "unable to copy virtual storage");
    break;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__layout_debug(H5F_t H5_ATTR_UNUSED *f, const void *_mesg,
  ...
  case H5D_VIRTUAL:
    ...
    for (u = 0; u < mesg->storage.u.virt.list_nused; u++) {
      ...
      fprintf(stream, "%*s%-*s %s\n", indent + 3, "", fwidth - 3,
              "Source dataset name:",
              mesg->storage.u.virt.list[u].source_dset_name);
      fprintf(stream, "%*s%-*s %s\n", indent + 3, "", fwidth - 3,
