...
static herr_t H5HF__hdr_prefix_decode(H5HF_hdr_t *hdr,
                                      const uint8_t **image_ref);
static herr_t H5HF__dtable_encode(H5F_t *f, uint8_t **pp,
                                  const H5HF_dtable_t *dtable);
static herr_t H5HF__dtable_decode(H5F_t *f, const uint8_t **pp,
                                  H5HF_dtable_t *dtable);

/* Metadata cache (H5AC) callbacks */
static herr_t H5HF__cache_hdr_get_initial_load_size(void *udata,
                                                    size_t *image_len);
static herr_t H5HF__cache_hdr_get_final_load_size(const void *image_ptr,
                                                  size_t image_len, void *udata,
                                                  size_t *actual_len);
static htri_t H5HF__cache_hdr_verify_chksum(const void *image_ptr, size_t len,
                                            void *udata_ptr);
...
static herr_t H5HF__cache_hdr_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                            size_t len, haddr_t *new_addr,
                                            size_t *new_len, unsigned *flags);
static herr_t H5HF__cache_hdr_serialize(const H5F_t *f, void *image, size_t len,
                                        void *thing);
static herr_t H5HF__cache_hdr_free_icr(void *thing);

static herr_t H5HF__cache_iblock_get_initial_load_size(void *udata,
                                                       size_t *image_len);
static htri_t H5HF__cache_iblock_verify_chksum(const void *image_ptr,
...
static herr_t H5HF__cache_dblock_get_initial_load_size(void *udata,
                                                       size_t *image_len);
static htri_t H5HF__cache_dblock_verify_chksum(const void *image_ptr,
                                               size_t len, void *udata_ptr);
...
static herr_t H5HF__cache_dblock_image_len(const void *thing,
                                           size_t *image_len);
static herr_t H5HF__cache_dblock_pre_serialize(H5F_t *f, void *thing,
                                               haddr_t addr, size_t len,
                                               haddr_t *new_addr,
                                               size_t *new_len,
                                               unsigned *flags);
static herr_t H5HF__cache_dblock_serialize(const H5F_t *f, void *image,
                                           size_t len, void *thing);
static herr_t H5HF__cache_dblock_notify(H5AC_notify_action_t action,
                                        void *thing);
static herr_t H5HF__cache_dblock_free_icr(void *thing);
static herr_t H5HF__cache_dblock_fsf_size(const void *_thing,
                                          hsize_t *fsf_size);

/* Debugging Function Prototypes */
#ifndef NDEBUG
static herr_t H5HF__cache_verify_hdr_descendants_clean(H5F_t *f,
                                                       H5HF_hdr_t *hdr,
                                                       hbool_t *fd_clean,
                                                       hbool_t *clean);
static herr_t H5HF__cache_verify_iblock_descendants_clean(
    H5F_t *f, haddr_t fd_parent_addr, H5HF_indirect_t *iblock,
    unsigned *iblock_status, hbool_t *fd_clean, hbool_t *clean);
static herr_t H5HF__cache_verify_iblocks_dblocks_clean(
    H5F_t *f, haddr_t fd_parent_addr, H5HF_indirect_t *iblock,
    hbool_t *fd_clean, hbool_t *clean, hbool_t *has_dblocks);
static herr_t H5HF__cache_verify_descendant_iblocks_clean(
    H5F_t *f, haddr_t fd_parent_addr, H5HF_indirect_t *iblock,
...
const H5AC_class_t H5AC_FHEAP_IBLOCK[1] = {{
    ...
    H5HF__cache_iblock_get_initial_load_size, /* 'get_initial_load_size'
                                                 callback */
...
const H5AC_class_t H5AC_FHEAP_DBLOCK[1] = {{
    ...
    H5AC__CLASS_NO_FLAGS_SET,    /* Client class behavior flags */
    H5HF__cache_dblock_get_initial_load_size, /* 'get_initial_load_size'
                                                 callback */
    NULL,                             /* 'get_final_load_size' callback */
...
static herr_t H5HF__hdr_prefix_decode(H5HF_hdr_t *hdr,
  ...
  if (*image++ != H5HF_HDR_VERSION)
    HGOTO_ERROR(H5E_HEAP, H5E_VERSION, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_hdr_get_initial_load_size(void *_udata,
                                                    size_t *image_len) {
  H5HF_hdr_cache_ud_t *udata =
      (H5HF_hdr_cache_ud_t *)_udata; /* Pointer to user data */
 ...
 */
static herr_t
H5HF__cache_hdr_get_final_load_size(const void *_image,
                                    size_t H5_ATTR_NDEBUG_UNUSED image_len,
                                    void *_udata, size_t *actual_len) {
  H5HF_hdr_t hdr; /* Temporary fractal heap header */
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into into supplied image */
  H5HF_hdr_cache_ud_t *udata =
      (H5HF_hdr_cache_ud_t *)_udata; /* User data for callback */
  herr_t ret_value = SUCCEED;        /* Return value */
  ...
  if (H5HF__hdr_prefix_decode(&hdr, &image) < 0)
    HGOTO_ERROR(H5E_HEAP, H5E_CANTDECODE, FAIL,
                "can't decode fractal heap header prefix");
  ...
  if (hdr.filter_len > 0)
    ...
    *actual_len +=
        (size_t)(H5F_SIZEOF_SIZE(
                     udata->f) /* Size of size for filtered root direct block */
                 + (unsigned)4 /* Size of filter mask for filtered root direct
                                  block */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5HF__cache_hdr_verify_chksum(const void *_image, size_t len,
 ...
 *		header block, allocate an instance of H5HF_hdr_t, load the
 *contents of the buffer into into the new instance of H5HF_hdr_t, and then
 ...
 *-------------------------------------------------------------------------
 */
static void *H5HF__cache_hdr_deserialize(const void *_image, size_t len,
                                         void *_udata,
  ...
  H5F_DECODE_LENGTH(udata->f, image,
                    hdr->huge_next_id); /* Next ID to use for "huge" object */
  H5F_addr_decode(
      udata->f, &image,
      &hdr->huge_bt2_addr); /* Address of "huge" object tracker B-tree */
  ...
  H5F_addr_decode(udata->f, &image,
                  &hdr->fs_addr); /* Address of free section header */
  ...
  assert((size_t)(image - (const uint8_t *)_image) ==
         (hdr->heap_size - H5HF_SIZEOF_CHKSUM));
  ...
  if (hdr->filter_len > 0) {
    ...
    /* Sanity check */
    assert(len > hdr->heap_size); /* A header with filter info is > than a
                                     standard header */
    ...
    if (NULL == (pline = (H5O_pline_t *)H5O_msg_decode(
                     hdr->f, NULL, H5O_PLINE_ID, len, image)))
      HGOTO_ERROR(H5E_HEAP, H5E_CANTDECODE, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_hdr_pre_serialize(H5F_t *f, void *_thing,
                                            haddr_t addr, size_t len,
...
#ifndef NDEBUG
  {
     ...
     * this value.
     */
    if (H5HF__cache_verify_hdr_descendants_clean(
            (H5F_t *)f, hdr, &fd_children_clean, &descendants_clean) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_hdr_serialize(const H5F_t *f, void *_image,
  ...
  *image++ = H5HF_HDR_VERSION;
  ...
  heap_flags =
      (uint8_t)(heap_flags |
                (hdr->huge_ids_wrapped ? H5HF_HDR_FLAGS_HUGE_ID_WRAPPED : 0));
  heap_flags =
      (uint8_t)(heap_flags |
                (hdr->checksum_dblocks ? H5HF_HDR_FLAGS_CHECKSUM_DBLOCKS : 0));
  *image++ = heap_flags;
  ...
  if (hdr->filter_len > 0) {
    ...
    if (H5O_msg_encode(hdr->f, H5O_PLINE_ID, FALSE, image, &(hdr->pline)) < 0)
      HGOTO_ERROR(H5E_HEAP, H5E_CANTENCODE, FAIL,
                  "can't encode I/O pipeline filters");
    image += hdr->filter_len;
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_iblock_get_initial_load_size(void *_udata,
                                                       size_t *image_len) {
  H5HF_iblock_cache_ud_t *udata =
      (H5HF_iblock_cache_ud_t *)_udata; /* User data for callback */
  ...
  *image_len =
      (size_t)H5HF_MAN_INDIRECT_SIZE(udata->par_info->hdr, *udata->nrows);
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5HF__cache_iblock_verify_chksum(const void *_image, size_t len,
                                               void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
 ...
 *		it.
 *
 *		As best I can tell, the size of the indirect block image is
 *fully know before the image is loaded, so this function should succeed unless
 *the image is corrupt or memory allocation fails.
 *
 * Return:	Success:	Pointer to in core representation
 *		Failure:	NULL
 *
 *-------------------------------------------------------------------------
 */
static void *H5HF__cache_iblock_deserialize(const void *_image,
                                            size_t H5_ATTR_NDEBUG_UNUSED len,
                                            void *_udata,
                                            hbool_t H5_ATTR_UNUSED *dirty) {
  ...
  if (hdr->filter_len > 0) {
    ...
    if (NULL == (iblock->filt_ents = H5FL_SEQ_MALLOC(
  ...
  for (u = 0; u < (iblock->nrows * hdr->man_dtable.cparam.width); u++) {
    ...
    if (hdr->filter_len > 0) {
      ...
      if (u <
         ...
         *  not defined)
         */
        assert((H5_addr_defined(iblock->ents[u].addr) &&
                iblock->filt_ents[u].size) ||
               (!H5_addr_defined(iblock->ents[u].addr) &&
  ...
  /* Sanity check */
  assert(iblock->nchildren); /* indirect blocks w/no children should have been
                                deleted */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_iblock_image_len(const void *_thing,
                                           size_t *image_len) {
  const H5HF_indirect_t *iblock =
      (const H5HF_indirect_t *)_thing; /* Indirect block info */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_iblock_pre_serialize(
    H5F_t *f, void *_thing, haddr_t addr, size_t H5_ATTR_UNUSED len,
...
#ifndef NDEBUG
  {
    ...
    if (H5HF__cache_verify_iblock_descendants_clean(
            (H5F_t *)f, iblock->addr, iblock, &iblock_status,
  ...
  if (H5F_IS_TMP_ADDR(f, addr)) {
    ...
    if (HADDR_UNDEF ==
        (iblock_addr = H5MF_alloc((H5F_t *)f, H5FD_MEM_FHEAP_IBLOCK,
                                  (hsize_t)iblock->size)))
    ...
    if (H5AC_move_entry((H5F_t *)f, H5AC_FHEAP_IBLOCK, iblock->addr,
                        iblock_addr) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_iblock_serialize(const H5F_t *f, void *_image,
                                           size_t H5_ATTR_NDEBUG_UNUSED len,
                                           void *_thing) {
  H5HF_hdr_t *hdr; /* Shared fractal heap information */
  H5HF_indirect_t *iblock = (H5HF_indirect_t *)_thing; /* Indirect block info */
  ...
  *image++ = H5HF_IBLOCK_VERSION;
  ...
  /* Compute checksum */
  metadata_chksum = H5_checksum_metadata(
      (uint8_t *)_image, (size_t)(image - (uint8_t *)_image), 0);
 ...
 *		relationships between iblocks and their parents as indirect
 *blocks are loaded / inserted and evicted from the metadata cache.
 *
...
static herr_t H5HF__cache_iblock_notify(H5AC_notify_action_t action,
  ...
  if (iblock->parent == NULL) {
    ...
    unsigned H5_ATTR_NDEBUG_UNUSED
        indir_idx; /* Index in parent's child iblock pointer array */
...
static herr_t H5HF__cache_iblock_free_icr(void *thing) {
  H5HF_indirect_t *iblock =
      (H5HF_indirect_t *)thing; /* Fractal heap indirect block to free */
  herr_t ret_value = SUCCEED;   /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_dblock_get_initial_load_size(void *_udata,
                                                       size_t *image_len) {
  const H5HF_dblock_cache_ud_t *udata =
      (const H5HF_dblock_cache_ud_t *)_udata; /* User data for callback */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5HF__cache_dblock_verify_chksum(const void *_image, size_t len,
                                               void *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  H5HF_dblock_cache_ud_t *udata =
      (H5HF_dblock_cache_ud_t *)_udata; /* User data for callback */
  ...
  if (hdr->filter_len > 0) {
    size_t nbytes; /* Number of bytes used in buffer, after applying reverse
                      filters */
 ...
 *-------------------------------------------------------------------------
 */
static void *H5HF__cache_dblock_deserialize(const void *_image, size_t len,
                                            void *_udata,
                                            hbool_t H5_ATTR_UNUSED *dirty) {
  H5HF_hdr_t *hdr; /* Shared fractal heap information */
  H5HF_dblock_cache_ud_t *udata =
  ...
  if (hdr->filter_len > 0) {
    ...
    if (NULL ==
        (dblock->blk = H5FL_BLK_MALLOC(direct_block, (size_t)dblock->size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
  ...
  /* Sanity check */
  assert((size_t)(image - dblock->blk) ==
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_dblock_image_len(const void *_thing,
                                           size_t *image_len) {
  const H5HF_direct_t *dblock =
      (const H5HF_direct_t *)_thing; /* Direct block info */
  const H5HF_hdr_t *hdr;             /* Shared fractal heap information */
  size_t size;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_dblock_pre_serialize(H5F_t *f, void *_thing,
                                               haddr_t addr, size_t len,
                                               haddr_t *new_addr,
                                               size_t *new_len,
                                               unsigned *flags) {
  ...
  *image++ = H5HF_DBLOCK_VERSION;
  ...
  if (hdr->filter_len > 0) {
    ...
    /* Push direct block data through I/O filter pipeline */
    nbytes = write_size;
    if (H5Z_pipeline(&(hdr->pline), 0, &filter_mask, H5Z_ENABLE_EDC, filter_cb,
    ...
    if (dblock->parent == NULL) {
      ...
      if (hdr->pline_root_direct_size != write_size || at_tmp_addr) {
        ...
        if (HADDR_UNDEF ==
            (dblock_addr = H5MF_alloc((H5F_t *)f, H5FD_MEM_FHEAP_DBLOCK,
                                      (hsize_t)write_size)))
          HGOTO_ERROR(H5E_HEAP, H5E_NOSPACE, FAIL,
 ...
 *		function has to construct the on disk image to determine its
 *size and address, this function simply copies the image prepared by the
 *pre-serialize function into the supplied image buffer, and discards a buffer
 *if necessary.
 *
 * Return:	Success:	SUCCEED
 *		Failure:	FAIL
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5HF__cache_dblock_serialize(const H5F_t H5_ATTR_NDEBUG_UNUSED *f,
                                           void *image,
                                           size_t H5_ATTR_NDEBUG_UNUSED len,
                                           void *_thing) {
  H5HF_direct_t *dblock = (H5HF_direct_t *)_thing; /* Direct block info */
  ...
  assert(dblock);
  assert(dblock->cache_info.type == H5AC_FHEAP_DBLOCK);
  assert((dblock->blk != dblock->write_buf) ||
...
static herr_t H5HF__cache_dblock_notify(H5AC_notify_action_t action,
  ...
  H5HF_direct_t *dblock =
      (H5HF_direct_t *)_thing; /* Fractal heap direct block */
  herr_t ret_value = SUCCEED;  /* Return value */
...
static herr_t H5HF__cache_dblock_fsf_size(const void *_thing,
                                          hsize_t *fsf_size) {
  const H5HF_direct_t *dblock =
      (const H5HF_direct_t *)_thing; /* Fractal heap direct block */
...
static herr_t H5HF__cache_verify_hdr_descendants_clean(H5F_t *f,
   ...
   */
  if (hdr->root_iblock || ((hdr->man_dtable.curr_root_rows > 0) &&
    ...
    if (!root_iblock_in_cache) { /* we are done */
      ...
      *clean = FALSE;
       ...
       * the root inode.
       */
      if (H5AC_flush_dependency_exists(f, hdr->heap_addr, root_iblock_addr,
                                       &fd_exists) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                    "can't check flush dependency");
      assert(fd_exists);
      ...
      *fd_clean = FALSE;
      ...
      if (root_iblock == NULL) { /* we don't have ptr to root iblock */
        if (0 == (root_iblock_status & H5AC_ES__IS_PROTECTED)) {
          ...
          if (NULL == (root_iblock = (H5HF_indirect_t *)H5AC_protect(
                           f, H5AC_FHEAP_IBLOCK, root_iblock_addr, NULL,
                           H5AC__READ_ONLY_FLAG)))
            HGOTO_ERROR_TAG(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                            "H5AC_protect() failed.");
      ...
      if (H5HF__cache_verify_iblock_descendants_clean(
              f, hdr->heap_addr, root_iblock, &root_iblock_status, fd_clean,
              clean) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_SYSTEM, FAIL,
      ...
      if (unprotect_root_iblock) {
        assert(root_iblock);
        if (H5AC_unprotect(f, H5AC_FHEAP_IBLOCK, root_iblock_addr, root_iblock,
                           H5AC__NO_FLAGS_SET) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                      "H5AC_unprotect() failed.");
      } /* end if */
    }   /* end else */
  }     /* end if */
    ...
    if (root_dblock_status & H5AC_ES__IN_CACHE) {
      if (H5AC_verify_entry_type(f, root_dblock_addr, &H5AC_FHEAP_DBLOCK[0],
                                 &in_cache, &type_ok) < 0)
       ...
       */
      if (H5AC_flush_dependency_exists(f, hdr->heap_addr, root_dblock_addr,
                                       &fd_exists) < 0)
        HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                    "can't check flush dependency");
      if (!fd_exists)
...
static herr_t H5HF__cache_verify_iblock_descendants_clean(
  ...
  if (((has_dblocks || has_iblocks)) &&
      (0 == (*iblock_status & H5AC_ES__IS_FLUSH_DEP_PARENT)))
    HGOTO_ERROR(H5E_HEAP, H5E_SYSTEM, FAIL,
                "iblock has children and is not a flush dep parent.");
  if (((has_dblocks || has_iblocks)) &&
      (0 == (*iblock_status & H5AC_ES__IS_PINNED)))
    HGOTO_ERROR(H5E_HEAP, H5E_SYSTEM, FAIL,
        ...
        *has_dblocks = TRUE;
        ...
        if ((dblock_status & H5AC_ES__IS_DIRTY) &&
          ...
          *clean = FALSE;
          ...
          if (H5AC_flush_dependency_exists(f, fd_parent_addr, dblock_addr,
                                           &fd_exists) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                        "can't check flush dependency");
         ...
         * here.
         */
        if (H5AC_flush_dependency_exists(f, iblock_addr, dblock_addr,
                                         &fd_exists) < 0)
          HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                      "can't check flush dependency");
        ...
        *has_iblocks = TRUE;
        ...
        if ((child_iblock_status & H5AC_ES__IS_DIRTY) &&
          ...
          *clean = FALSE;
          ...
          if (H5AC_flush_dependency_exists(f, fd_parent_addr, child_iblock_addr,
                                           &fd_exists) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                        "can't check flush dependency");
        ...
        if (*fd_clean) {
          ...
          if (0 == (child_iblock_status & H5AC_ES__IS_PINNED)) {
            ...
            if (0 == (child_iblock_status & H5AC_ES__IS_PROTECTED)) {
              ...
              if (NULL == (child_iblock = (H5HF_indirect_t *)H5AC_protect(
                               f, H5AC_FHEAP_IBLOCK, child_iblock_addr, NULL,
                               H5AC__READ_ONLY_FLAG)))
                HGOTO_ERROR_TAG(H5E_HEAP, H5E_CANTPROTECT, FAIL,
                                "H5AC_protect() failed.");
              ...
              if (H5AC_get_entry_ptr_from_addr(f, child_iblock_addr,
                                               (void **)(&child_iblock)) < 0)
                HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                            "H5AC_get_entry_ptr_from_addr() failed.");
              assert(child_iblock);
          ...
          if (H5HF__cache_verify_iblock_descendants_clean(
                  f, fd_parent_addr, child_iblock, &child_iblock_status,
                  fd_clean, clean) < 0)
            HGOTO_ERROR(H5E_HEAP, H5E_SYSTEM, FAIL,
          ...
          if (fd_parent_addr != iblock_addr) {
            if (H5AC_flush_dependency_exists(f, iblock_addr, child_iblock_addr,
                                             &fd_exists) < 0)
              HGOTO_ERROR(H5E_HEAP, H5E_CANTGET, FAIL,
                          "can't check flush dependency");
          ...
          if (unprotect_child_iblock) {
            if (H5AC_unprotect(f, H5AC_FHEAP_IBLOCK, child_iblock_addr,
                               child_iblock, H5AC__NO_FLAGS_SET) < 0)
              HGOTO_ERROR(H5E_HEAP, H5E_CANTUNPROTECT, FAIL,
                          "H5AC_unprotect() failed.");
          } /* end if */
        }   /* end if */
