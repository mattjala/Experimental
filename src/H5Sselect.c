...
herr_t H5S_select_offset(H5S_t *space, const hssize_t *offset) {
  ...
  H5MM_memcpy(space->select.offset, offset,
...
/*--------------------------------------------------------------------------
 ...
 RETURNS
    Non-negative on success/Negative on failure
 DESCRIPTION
        This function creates an offset for the selection within an extent,
allowing the same shaped selection to be moved to different locations within a
...
herr_t H5Soffset_simple(hid_t space_id, const hssize_t *offset) {
  ...
  if (space->extent.rank == 0 || (H5S_GET_EXTENT_TYPE(space) == H5S_SCALAR ||
    ...
    HGOTO_ERROR(H5E_ID, H5E_UNSUPPORTED, FAIL,
                "can't set offset on scalar or null dataspace");
  if (offset == NULL)
...
herr_t H5S_select_copy(H5S_t *dst, const H5S_t *src, hbool_t share_selection) {
  ...
  if ((ret_value = (*src->select.type->copy)(dst, src, share_selection)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                "can't copy selection specific information");
...
herr_t H5S_select_deserialize(H5S_t **space, const uint8_t **p,
  ...
  hbool_t skip = (p_size == SIZE_MAX
                      ? TRUE
                      : FALSE); /* If p_size is unknown, skip buffer checks */
  FUNC_ENTER_NOAPI(FAIL)
  ...
  if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, *p, sizeof(uint32_t), p_end))
    HGOTO_ERROR(H5E_DATASPACE, H5E_OVERFLOW, FAIL,
                "buffer overflow while decoding selection type");
  ...
  case H5S_SEL_ALL: /* Entire extent selected */
    ret_value =
        (*H5S_sel_all->deserialize)(space, p, p_size - sizeof(uint32_t), skip);
    break;

  case H5S_SEL_NONE: /* Nothing selected */
    ret_value =
        (*H5S_sel_none->deserialize)(space, p, p_size - sizeof(uint32_t), skip);
    break;
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5Sget_select_bounds(hid_t spaceid, hsize_t start[] /*out*/,
                            hsize_t end[] /*out*/) {
  H5S_t *space;     /* Dataspace to modify selection of */
  ...
  for (u = 0; u < space->extent.rank; u++)
    if (offset[u] > (hssize_t)low_bounds[u])
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "adjustment would move selection below zero offset");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S_select_project_simple(space, new_space, offset)
        const H5S_t *space;             IN: Pointer to dataspace to project
        H5S_t *new_space;               IN/OUT: Pointer to dataspace projected
onto hsize_t *offset;                IN/OUT: Offset of projected point RETURNS
    Non-negative on success, negative on failure
 DESCRIPTION
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_project_simple(const H5S_t *space, H5S_t *new_space,
                                 hsize_t *offset) {
  herr_t ret_value = FAIL; /* Return value */
...
--------------------------------------------------------------------------*/
herr_t H5S_select_iter_init(H5S_sel_iter_t *sel_iter, H5S_t *space,
  ...
  if (sel_iter->rank > 0) {
    H5MM_memcpy(sel_iter->dims, space->extent.size,
                sizeof(hsize_t) * space->extent.rank);
    H5MM_memcpy(sel_iter->sel_off, space->select.offset,
                sizeof(hsize_t) * space->extent.rank);
  }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5S_select_iter_get_seq_list(H5S_sel_iter_t *iter, size_t maxseq,
  ...
  if ((ret_value = (*iter->type->iter_get_seq_list)(
           iter, maxseq, maxelmts, nseq, nelmts, off, len)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                "unable to get selection sequence list");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S_select_iterate(buf, type, space, operator, operator_data)
        ...
        H5S_t *space;   IN: Dataspace object containing selection to iterate
over H5D_operator_t op; IN: Function pointer to the routine to be called for
each element in BUF iterated over. void *operator_data;    IN/OUT: Pointer to
any user-defined data associated with the operation. RETURNS Returns the return
value of the last operator if it was non-zero, or zero if all elements were
processed. Otherwise returns a negative value. DESCRIPTION Iterates over the
selected elements in a memory buffer, calling the user's callback function for
each element.  The selection in the dataspace is modified so that any elements
already iterated over are removed from the selection if the iteration is
interrupted (by the H5D_operator_t function returning non-zero) in the "middle"
of the iteration and may be re-started by the user where it left off.

    NOTE: Until "subtracting" elements from a selection is implemented,
        the selection is not modified.
--------------------------------------------------------------------------*/
herr_t H5S_select_iterate(void *buf, const H5T_t *type, H5S_t *space,
  ...
  if (NULL == (iter = H5FL_MALLOC(H5S_sel_iter_t)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                "can't allocate selection iterator");
  ...
  if ((nelmts = (hssize_t)H5S_GET_SELECT_NPOINTS(space)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOUNT, FAIL,
                "can't get number of elements selected");
  ...
  if (NULL == (len = H5FL_SEQ_MALLOC(size_t, H5D_IO_VECTOR_SIZE)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                "can't allocate length vector array");
  if (NULL == (off = H5FL_SEQ_MALLOC(hsize_t, H5D_IO_VECTOR_SIZE)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                "can't allocate offset vector array");
  ...
  while (max_elem > 0 && user_ret == 0) {
    ...
    if (H5S_SELECT_ITER_GET_SEQ_LIST(iter, (size_t)H5D_IO_VECTOR_SIZE, max_elem,
                                     &nseq, &nelem, off, len) < 0)
    ...
    for (curr_seq = 0; curr_seq < nseq && user_ret == 0; curr_seq++) {
      ...
      while (curr_len > 0 && user_ret == 0) {
        ...
        case H5S_SEL_ITER_OP_APP:
          ...
          user_ret = (op->u.app_op.op)(loc, op->u.app_op.type_id, ndims, coords,
                                       op_data);
          break;
...
htri_t H5S_select_shape_same(H5S_t *space1, H5S_t *space2) {
  ...
  hbool_t iter_a_init =
      FALSE; /* Selection a iteration info has been initialized */
  hbool_t iter_b_init =
      FALSE;               /* Selection b iteration info has been initialized */
  ...
  if (space1->extent.rank > 0 && space2->extent.rank > 0) {
    ...
    /* Otherwise, iterate through all the blocks in the selection */
      ...
      hsize_t start_a[H5S_MAX_RANK]; /* Start point of selection block in
                                        dataspace a */
      hsize_t start_b[H5S_MAX_RANK]; /* Start point of selection block in
                                        dataspace b */
      hsize_t
          end_a[H5S_MAX_RANK]; /* End point of selection block in dataspace a */
      hsize_t
          end_b[H5S_MAX_RANK]; /* End point of selection block in dataspace b */
      hssize_t offset[H5S_MAX_RANK]; /* Offset of selection b blocks relative to
                                        selection a blocks */
      ...
      while (1) {
        ...
        if (first_block) {
          ...
          while (space_b_dim >= 0) {
            ...
            offset[space_a_dim] =
                (hssize_t)start_b[space_b_dim] - (hssize_t)start_a[space_a_dim];
        ...
        /* Check over the blocks for each selection */
          ...
          while (space_b_dim >= 0) {
            ...
            if ((hsize_t)((hssize_t)start_a[space_a_dim] +
                          offset[space_a_dim]) != start_b[space_b_dim])
              HGOTO_DONE(FALSE);
...
/*--------------------------------------------------------------------------
 ...
 COMMENTS, BUGS, ASSUMPTIONS
    ...
    don't call it directly, use the appropriate macro defined in H5Sprivate.h.
--------------------------------------------------------------------------*/
htri_t H5S_select_intersect_block(H5S_t *space, const hsize_t *start,
                                  const hsize_t *end) {
  htri_t ret_value = TRUE; /* Return value */
  ...
  if (H5S_SEL_NONE != H5S_GET_SELECT_TYPE(space)) {
    ...
    if (H5S_SELECT_BOUNDS(space, low, high) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                  "can't get selection bounds for dataspace");
  ...
  if ((ret_value = (*space->select.type->intersect_block)(space, start, end)) <
      0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOMPARE, FAIL,
                "can't intersect block with selection");
...
htri_t H5Sselect_intersect_block(hid_t space_id, const hsize_t *start,
  ...
  if (NULL == start)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                "block start array pointer is NULL");
  if (NULL == end)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                "block end array pointer is NULL");

  /* Range check start & end values */
  for (u = 0; u < space->extent.rank; u++)
    if (start[u] > end[u])
      HGOTO_ERROR(H5E_DATASPACE, H5E_BADRANGE, FAIL,
                  "block start[%u] (%llu) > end[%u] (%llu)", u,
  ...
  if ((ret_value = H5S_select_intersect_block(space, start, end)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOMPARE, FAIL,
                "can't compare selection and block");
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_construct_projection(H5S_t *base_space, H5S_t **new_space_ptr,
                                       unsigned new_space_rank,
                                       hsize_t element_size,
                                       ptrdiff_t *buf_adj) {
  H5S_t *new_space = NULL; /* New dataspace constructed */
  hsize_t
      base_space_dims[H5S_MAX_RANK]; /* Current dimensions of base dataspace */
  hsize_t base_space_maxdims[H5S_MAX_RANK]; /* Maximum dimensions of base
                                               dataspace */
  int sbase_space_rank;     /* Signed # of dimensions of base dataspace */
  unsigned base_space_rank; /* # of dimensions of base dataspace */
  hsize_t projected_space_element_offset =
      0; /* Offset of selected element in projected buffer */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if ((sbase_space_rank = H5S_get_simple_extent_dims(
           base_space, base_space_dims, base_space_maxdims)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
  ...
  if (new_space_rank == 0) {
    ...
    hsize_t
        new_space_dims[H5S_MAX_RANK]; /* Current dimensions for new dataspace */
    hsize_t new_space_maxdims[H5S_MAX_RANK]; /* Maximum dimensions for new
                                                dataspace */
    ...
    if (new_space_rank > base_space_rank) {
      hsize_t tmp_dim_size =
          1; /* Temporary dimension value, for filling arrays */
      ...
      H5VM_array_fill(new_space_dims, &tmp_dim_size, sizeof(tmp_dim_size),
                      rank_diff);
      H5VM_array_fill(new_space_maxdims, &tmp_dim_size, sizeof(tmp_dim_size),
                      rank_diff);
     ...
     */
    if (NULL == (new_space = H5S_create_simple(new_space_rank, new_space_dims,
                                               new_space_maxdims)))
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCREATE, FAIL,
     ...
     */
    if (H5S_SELECT_PROJECT_SIMPLE(base_space, new_space,
                                  &projected_space_element_offset) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL,
                  "unable to project simple selection");
    ...
    if (H5S_GET_EXTENT_TYPE(base_space) == H5S_SIMPLE &&
        base_space->select.offset_changed) {
      if (new_space_rank > base_space_rank) {
        memset(new_space->select.offset, 0,
               sizeof(new_space->select.offset[0]) * rank_diff);
        H5MM_memcpy(&new_space->select.offset[rank_diff],
                    base_space->select.offset,
                    sizeof(new_space->select.offset[0]) * base_space_rank);
      } /* end if */
      else
        H5MM_memcpy(new_space->select.offset,
                    &base_space->select.offset[rank_diff],
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_fill(const void *fill, size_t fill_size, H5S_t *space,
                       void *_buf) {
  H5S_sel_iter_t *iter = NULL; /* Selection iteration info */
  hbool_t iter_init = FALSE; /* Selection iteration info has been initialized */
  hsize_t *off = NULL;       /* Array to store sequence offsets */
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t
H5S_select_project_intersection(src_space,dst_space,src_intersect_space,proj_space,share_selection)
...
is mapped to src_space H5S_t *src_intersect_space; IN: Selection whose
intersection with src_space is projected to dst_space to obtain the result H5S_t
**new_space_ptr;  OUT: Will contain the result (intersection of
src_intersect_space and src_space projected from src_space to dst_space) after
the operation hbool_t share_selection; IN: Whether we are allowed to share
structures inside dst_space with proj_space
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_project_intersection(H5S_t *src_space, H5S_t *dst_space,
                                       H5S_t *src_intersect_space,
                                       H5S_t **new_space_ptr,
                                       hbool_t share_selection) {
  H5S_t *new_space = NULL; /* New dataspace constructed */
  H5S_t *tmp_src_intersect_space =
      NULL; /* Temporary SIS converted from points->hyperslabs */
  ...
  if (H5S_GET_SELECT_TYPE(src_intersect_space) == H5S_SEL_ALL) {
    ...
    if (H5S_select_copy(new_space, dst_space, FALSE) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                  "can't copy destination space selection");
  } /* end if */
   ...
   * "none" */
    ...
    if (H5S_GET_EXTENT_TYPE(dst_space) == H5S_SCALAR) {
      ...
      if (H5S_SELECT_BOUNDS(src_space, coords_start, coords_end) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                    "can't get source space bounds");
      assert(0 ==
             memcmp(coords_start, coords_end,
                    H5S_GET_EXTENT_NDIMS(src_space) * sizeof(coords_start[0])));

      /* Check for intersection */
      if ((intersect = H5S_SELECT_INTERSECT_BLOCK(
               src_intersect_space, coords_start, coords_end)) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOMPARE, FAIL,
                    "can't check for intersection");
      ...
      if (H5S_GET_SELECT_TYPE(src_intersect_space) == H5S_SEL_POINTS) {
        H5S_pnt_node_t *curr_pnt =
            src_intersect_space->select.sel_info.pnt_lst->head;
        ...
        if (H5S__extent_copy_real(&tmp_src_intersect_space->extent,
                                  &src_intersect_space->extent, FALSE) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL,
                      "unable to copy source intersect space extent");

        /* Iterate over points */
        for (curr_pnt = src_intersect_space->select.sel_info.pnt_lst->head;
             curr_pnt; curr_pnt = curr_pnt->next)
          /* Add point to hyperslab selection */
          if (H5S_hyper_add_span_element(tmp_src_intersect_space,
                                         src_intersect_space->extent.rank,
                                         curr_pnt->pnt) < 0)
      ...
      if ((H5S_GET_SELECT_TYPE(src_space) == H5S_SEL_POINTS) ||
        ...
        if (H5S_select_iter_init(ss_iter, src_space, 1,
                                 H5S_SEL_ITER_SHARE_WITH_DATASPACE) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                      "can't initialize source space selection iterator");
        ss_iter_init = TRUE;
        if (H5S_select_iter_init(ds_iter, dst_space, 1,
                                 H5S_SEL_ITER_SHARE_WITH_DATASPACE) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
        ...
        do {
          ...
          /* Check for intersection */
          if ((intersect = H5S_SELECT_INTERSECT_BLOCK(src_intersect_space,
                                                      coords, coords)) < 0)
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOMPARE, FAIL,
                        "can't check for intersection");
          ...
          if (intersect) {
            ...
            if (H5S_select_elements(new_space, H5S_SELECT_APPEND, 1, coords) <
                0)
              HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSELECT, FAIL,
        ...
        if (H5S__hyper_project_intersection(src_space, dst_space,
                                            src_intersect_space, new_space,
                                            share_selection) < 0)
  ...
  *new_space_ptr = new_space;
  ...
  if (tmp_src_intersect_space && H5S_close(tmp_src_intersect_space) < 0)
    HDONE_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                "unable to release temporary dataspace");
  if (ss_iter_init && H5S_SELECT_ITER_RELEASE(ss_iter) < 0)
    HDONE_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                "unable to release source selection iterator");
  if (ds_iter_init && H5S_SELECT_ITER_RELEASE(ds_iter) < 0)
    HDONE_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                "unable to release destination selection iterator");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    hid_t
H5Sselect_project_intersection(src_space_id,dst_space_d,src_intersect_space_id)
        hid_t src_space_id;         IN: Selection that is mapped to
dst_space_id, and intersected with src_intersect_space_id hid_t dst_space_id;
IN: Selection that is mapped to src_space_id hid_t src_intersect_space_id; IN:
Selection whose intersection with src_space_id is projected to dst_space_id to
obtain the result
...
hid_t H5Sselect_project_intersection(hid_t src_space_id, hid_t dst_space_id,
  ...
  if (NULL ==
    ...
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL, "not a dataspace");
  if (NULL ==
      (dst_space = (H5S_t *)H5I_object_verify(dst_space_id, H5I_DATASPACE)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL, "not a dataspace");
  if (NULL == (src_intersect_space = (H5S_t *)H5I_object_verify(
                   src_intersect_space_id, H5I_DATASPACE)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL, "not a dataspace");
  ...
  if (H5S_GET_SELECT_NPOINTS(src_space) != H5S_GET_SELECT_NPOINTS(dst_space))
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                "number of points selected in source space does not match that "
                "in destination space");

  /* Check numbers of dimensions matches in source and source intersect spaces
   */
  if (H5S_GET_EXTENT_NDIMS(src_space) !=
      H5S_GET_EXTENT_NDIMS(src_intersect_space))
    HGOTO_ERROR(
  ...
  /* Perform operation */
  if (H5S_select_project_intersection(src_space, dst_space, src_intersect_space,
                                      &proj_space, FALSE) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCLIP, FAIL,
                "can't project dataspace intersection");
...
herr_t H5S_select_subtract(H5S_t *space, H5S_t *subtract_space) {
  ...
  if ((space->select.type->type != H5S_SEL_NONE) &&
    ...
    /* If either selection is a point selection, fail currently */
      ...
      HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, FAIL,
                  "point selections not currently supported");
    } /* end if */
    else {
...
hid_t H5Ssel_iter_create(hid_t space_id, size_t elmt_size, unsigned flags) {
  ...
  if (flags != (flags & H5S_SEL_ITER_ALL_PUBLIC_FLAGS))
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, H5I_INVALID_HID,
                "invalid selection iterator flag");

  /* Allocate the iterator */
  if (NULL == (sel_iter = H5FL_MALLOC(H5S_sel_iter_t)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, H5I_INVALID_HID,
                "can't allocate selection iterator");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5Ssel_iter_get_seq_list(sel_iter_id, maxseq, maxbytes, nseq, nbytes,
off, len) hid_t   sel_iter_id;  IN: ID of the dataspace selection iterator to
retrieve sequence from size_t  maxseq;       IN: Max. # of sequences to retrieve
 ...
 DESCRIPTION
    ...
    selections is "in order selected", unless the
H5S_SEL_ITER_GET_SEQ_LIST_SORTED flag is passed to H5Sset_iter_create for a
point selection.
...
herr_t H5Ssel_iter_get_seq_list(hid_t sel_iter_id, size_t maxseq,
                                size_t maxelmts, size_t *nseq /*out*/,
                                size_t *nelmts /*out*/, hsize_t *off /*out*/,
                                size_t *len /*out*/) {
  H5S_sel_iter_t *sel_iter;   /* Dataspace selection iterator to operate on */
  herr_t ret_value = SUCCEED; /* Return value */

  FUNC_ENTER_API(FAIL)
  H5TRACE7("e", "izzxxxx", sel_iter_id, maxseq, maxelmts, nseq, nelmts, off,
           len);

  /* Check args */
  if (NULL == (sel_iter = (H5S_sel_iter_t *)H5I_object_verify(
                   sel_iter_id, H5I_SPACE_SEL_ITER)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADTYPE, FAIL,
                "not a dataspace selection iterator");
  if (NULL == nseq)
  ...
  if (NULL == off)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                "offset array pointer is NULL");
  if (NULL == len)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                "length array pointer is NULL");

  /* Get the sequences of bytes */
  if (maxseq > 0 && maxelmts > 0 && sel_iter->elmt_left > 0) {
    if (H5S_SELECT_ITER_GET_SEQ_LIST(sel_iter, maxseq, maxelmts, nseq, nelmts,
                                     off, len) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                  "sequence length generation failed");
  } /* end if */
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5S_select_contig_block(H5S_t *space, hbool_t *is_contig, hsize_t *off,
                               size_t *len) {
  ...
  if (H5S_SELECT_ITER_GET_SEQ_LIST(iter, (size_t)1, (size_t)-1, &nseq_tmp,
                                   &nelem_tmp, &sel_off, &sel_len) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTNEXT, FAIL,
                "sequence length generation failed");

  /* If the first sequence includes all the elements selected in this piece, it
   * it contiguous */
...
herr_t H5Ssel_iter_reset(hid_t sel_iter_id, hid_t space_id) {
  ...
  if (H5S_select_iter_init(sel_iter, space, sel_iter->elmt_size,
                           sel_iter->flags) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                "unable to re-initialize selection iterator");
 ...
 *
 * Purpose:     Called when the ref count reaches zero on a selection iterator's
 *ID
 *
 * Return:	Non-negative on success / Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5S__sel_iter_close_cb(H5S_sel_iter_t *_sel_iter,
                              void H5_ATTR_UNUSED **request) {
  H5S_sel_iter_t *sel_iter =
      (H5S_sel_iter_t *)_sel_iter; /* The selection iterator to close */
  herr_t ret_value = SUCCEED;      /* Return value */
  ...
  if (H5S_sel_iter_close(sel_iter) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CLOSEERROR, FAIL,
                "unable to close selection iterator");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5Ssel_iter_close(sel_iter_id)
        hid_t   sel_iter_id;  IN: ID of the dataspace selection iterator to
close RETURNS Non-negative on success / Negative on failure DESCRIPTION Close a
dataspace selection iterator, releasing its state. PROGRAMMER GLOBAL VARIABLES
 COMMENTS, BUGS, ASSUMPTIONS
 EXAMPLES
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5Ssel_iter_close(hid_t sel_iter_id) {
  ...
  if (H5I_dec_app_ref(sel_iter_id) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTDEC, FAIL,
                "problem freeing dataspace selection iterator ID");
