 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_create(H5F_t *f, H5O_linfo_t *linfo,
  ...
  if (H5HF_get_id_len(fheap, &fheap_id_len) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTGETSIZE, FAIL,
  ...
  if (bt2_name && H5B2_close(bt2_name) < 0)
    HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, FAIL,
 ...
 *
 * Purpose:	Insert a link into the  dense link storage structures for a
 *group
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_insert(H5F_t *f, const H5O_linfo_t *linfo,
  ...
  if (H5HF_insert(fheap, link_size, link_ptr, udata.id) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                "unable to insert link into fractal heap");
  ...
  udata.common.name_hash =
      H5_checksum_lookup3(lnk->name, HDstrlen(lnk->name), 0);
  ...
  if (H5B2_insert(bt2_name, &udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                "unable to insert record into v2 B-tree");
...
static herr_t H5G__dense_lookup_cb(const void *_lnk, void *_user_lnk) {
  ...
  H5O_link_t *user_lnk =
      (H5O_link_t *)_user_lnk; /* User data from v2 B-tree link lookup */
  herr_t ret_value = SUCCEED;  /* Return value */
  ...
  if (H5O_msg_copy(H5O_LINK_ID, lnk, user_lnk) == NULL)
    HGOTO_ERROR(H5E_SYM, H5E_CANTCOPY, H5_ITER_ERROR,
                "can't copy link message");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_lookup(H5F_t *f, const H5O_linfo_t *linfo, const char *name,
                         hbool_t *found, H5O_link_t *lnk) {
  ...
  if (H5B2_find(bt2_name, &udata, found, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                "unable to locate link in name index");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_lookup_by_idx_fh_cb(const void *obj, size_t obj_len,
                                             void *_udata) {
  H5G_fh_ud_lbi_t *udata =
      (H5G_fh_ud_lbi_t *)_udata; /* User data for fractal heap 'op' callback */
  ...
  if (NULL ==
      (tmp_lnk = (H5O_link_t *)H5O_msg_decode(
           udata->f, NULL, H5O_LINK_ID, obj_len, (const unsigned char *)obj)))
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_lookup_by_idx_bt2_cb(const void *_record,
                                              void *_bt2_udata) {
  const H5G_dense_bt2_name_rec_t *record =
      (const H5G_dense_bt2_name_rec_t *)_record;
  H5G_bt2_ud_lbi_t *bt2_udata =
      (H5G_bt2_ud_lbi_t *)_bt2_udata; /* User data for callback */
  ...
  if (H5HF_op(bt2_udata->fheap, record->id, H5G__dense_lookup_by_idx_fh_cb,
              &fh_udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTOPERATE, H5_ITER_ERROR,
                "link found callback failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_lookup_by_idx(H5F_t *f, const H5O_linfo_t *linfo,
  ...
  if (H5_addr_defined(bt2_addr)) {
    ...
    if (NULL == (bt2 = H5B2_open(f, bt2_addr, NULL)))
      HGOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                  "unable to open v2 B-tree for index");
    ...
    if (H5B2_index(bt2, order, n, H5G__dense_lookup_by_idx_bt2_cb, &udata) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                  "unable to locate link in index");
  }      /* end if */
  ...
  if (bt2 && H5B2_close(bt2) < 0)
    HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, FAIL,
                "can't close v2 B-tree for index");
...
static herr_t H5G__dense_build_table_cb(const H5O_link_t *lnk, void *_udata) {
  H5G_dense_bt_ud_t *udata =
      (H5G_dense_bt_ud_t *)_udata; /* 'User data' passed in */
  ...
  /* Copy link information */
  if (H5O_msg_copy(H5O_LINK_ID, lnk, &(udata->ltable->lnks[udata->curr_lnk])) ==
      NULL)
    HGOTO_ERROR(H5E_SYM, H5E_CANTCOPY, H5_ITER_ERROR,
                "can't copy link message");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_build_table(H5F_t *f, const H5O_linfo_t *linfo,
                              H5_index_t idx_type, H5_iter_order_t order,
  ...
  if (ltable->nlinks > 0) {
    ...
    if ((ltable->lnks = (H5O_link_t *)H5MM_malloc(sizeof(H5O_link_t) *
                                                  ltable->nlinks)) == NULL)
     ...
     * messages */
    if (H5G__dense_iterate(f, linfo, H5_INDEX_NAME, H5_ITER_NATIVE, (hsize_t)0,
                           NULL, H5G__dense_build_table_cb, &udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_iterate_fh_cb(const void *obj, size_t obj_len,
                                       void *_udata) {
  H5G_fh_ud_it_t *udata =
      (H5G_fh_ud_it_t *)_udata; /* User data for fractal heap 'op' callback */
  herr_t ret_value = SUCCEED;   /* Return value */
   ...
   */
  if (NULL ==
      (udata->lnk = (H5O_link_t *)H5O_msg_decode(
           udata->f, NULL, H5O_LINK_ID, obj_len, (const unsigned char *)obj)))
    HGOTO_ERROR(H5E_SYM, H5E_CANTDECODE, FAIL, "can't decode link");
...
static herr_t H5G__dense_iterate_bt2_cb(const void *_record, void *_bt2_udata) {
  const H5G_dense_bt2_name_rec_t *record =
      (const H5G_dense_bt2_name_rec_t *)_record;
  H5G_bt2_ud_it_t *bt2_udata =
      (H5G_bt2_ud_it_t *)_bt2_udata; /* User data for callback */
  herr_t ret_value = H5_ITER_CONT;   /* Return value */
  ...
  if (bt2_udata->skip > 0)
    ...
    /* Call fractal heap 'op' routine, to copy the link information */
    if (H5HF_op(bt2_udata->fheap, record->id, H5G__dense_iterate_fh_cb,
                &fh_udata) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTOPERATE, H5_ITER_ERROR,
                  "heap op callback failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_iterate(H5F_t *f, const H5O_linfo_t *linfo,
                          H5_index_t idx_type, H5_iter_order_t order,
                          hsize_t skip, hsize_t *last_lnk, H5G_lib_iterate_t op,
                          void *op_data) {
  H5HF_t *fheap = NULL;                /* Fractal heap handle */
  H5G_link_table_t ltable = {0, NULL}; /* Table of links */
  H5B2_t *bt2 = NULL;                  /* v2 B-tree handle for index */
  ...
  if (order == H5_ITER_NATIVE) {
    ...
    if ((ret_value =
             H5G__link_iterate_table(&ltable, skip, last_lnk, op, op_data)) < 0)
      HERROR(H5E_SYM, H5E_CANTNEXT, "iteration operator failed");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_get_name_by_idx_fh_cb(const void *obj, size_t obj_len,
                                               void *_udata) {
  H5G_fh_ud_gnbi_t *udata =
      (H5G_fh_ud_gnbi_t *)_udata; /* User data for fractal heap 'op' callback */
  ...
  if (NULL ==
      (lnk = (H5O_link_t *)H5O_msg_decode(udata->f, NULL, H5O_LINK_ID, obj_len,
  ...
  if (udata->name) {
    HDstrncpy(udata->name, lnk->name,
              MIN((udata->name_len + 1), udata->name_size));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_get_name_by_idx_bt2_cb(const void *_record,
                                                void *_bt2_udata) {
  const H5G_dense_bt2_name_rec_t *record =
      (const H5G_dense_bt2_name_rec_t *)_record;
  H5G_bt2_ud_gnbi_t *bt2_udata =
      (H5G_bt2_ud_gnbi_t *)_bt2_udata; /* User data for callback */
  ...
  if (H5HF_op(bt2_udata->fheap, record->id, H5G__dense_get_name_by_idx_fh_cb,
              &fh_udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_get_name_by_idx(H5F_t *f, H5O_linfo_t *linfo,
                                  H5_index_t idx_type, H5_iter_order_t order,
                                  hsize_t n, char *name, size_t name_size,
                                  size_t *name_len) {
  H5HF_t *fheap = NULL;                /* Fractal heap handle */
  H5G_link_table_t ltable = {0, NULL}; /* Table of links */
  H5B2_t *bt2 = NULL;                  /* v2 B-tree handle for index */
  ...
  if (H5_addr_defined(bt2_addr)) {
    ...
    if (H5B2_index(bt2, order, n, H5G__dense_get_name_by_idx_bt2_cb, &udata) <
        0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTLIST, FAIL,
                  "can't locate object in v2 B-tree");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_remove_fh_cb(const void *obj, size_t obj_len,
                                      void *_udata) {
  H5G_fh_ud_rm_t *udata =
      (H5G_fh_ud_rm_t *)_udata; /* User data for fractal heap 'op' callback */
  ...
  if (udata->replace_names)
    if (H5G__link_name_replace(udata->f, udata->grp_full_path_r, lnk) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTRENAME, FAIL,
                  "unable to rename open objects");
...
static herr_t H5G__dense_remove_bt2_cb(const void *_record, void *_bt2_udata) {
  const H5G_dense_bt2_name_rec_t *record =
      (const H5G_dense_bt2_name_rec_t *)_record;
  H5G_bt2_ud_rm_t *bt2_udata =
      (H5G_bt2_ud_rm_t *)_bt2_udata; /* User data for callback */
  ...
  /* Call fractal heap 'op' routine, to perform user callback */
  if (H5HF_op(bt2_udata->common.fheap, record->id, H5G__dense_remove_fh_cb,
              &fh_udata) < 0)
  ...
  if (bt2_udata->rem_from_fheap)
    if (H5HF_remove(bt2_udata->common.fheap, record->id) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTREMOVE, FAIL,
                  "unable to remove link from fractal heap");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_remove(H5F_t *f, const H5O_linfo_t *linfo,
  ...
  if (H5B2_remove(bt2, &udata, H5G__dense_remove_bt2_cb, &udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTREMOVE, FAIL,
                "unable to remove link from name index v2 B-tree");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_remove_by_idx_fh_cb(const void *obj, size_t obj_len,
                                             void *_udata) {
  H5G_fh_ud_rmbi_t *udata =
      (H5G_fh_ud_rmbi_t *)_udata; /* User data for fractal heap 'op' callback */
  herr_t ret_value = SUCCEED;     /* Return value */
 ...
 *
 * Purpose:	v2 B-tree callback for dense link storage record removal by
 *index
 *
 * Return:	Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_remove_by_idx_bt2_cb(const void *_record,
                                              void *_bt2_udata) {
  H5G_bt2_ud_rmbi_t *bt2_udata =
      (H5G_bt2_ud_rmbi_t *)_bt2_udata; /* User data for callback */
  ...
  if (bt2_udata->idx_type == H5_INDEX_NAME) {
    ...
    const H5G_dense_bt2_corder_rec_t *record =
        (const H5G_dense_bt2_corder_rec_t *)_record;
  ...
  /* Call fractal heap 'op' routine, to perform user callback */
  if (H5HF_op(bt2_udata->fheap, heap_id, H5G__dense_remove_by_idx_fh_cb,
              &fh_udata) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTOPERATE, FAIL, "link removal callback failed");
  assert(fh_udata.lnk);

  /* Check for removing the link from the "other" index (creation order, when
   * name used and vice versa) */
  if (H5_addr_defined(bt2_udata->other_bt2_addr)) {
    ...
    if (bt2_udata->idx_type == H5_INDEX_NAME) {
      ...
      other_bt2_udata.name_hash = H5_checksum_lookup3(
          fh_udata.lnk->name, HDstrlen(fh_udata.lnk->name), 0);
    ...
    /* Open the index v2 B-tree */
    if (NULL ==
        (bt2 = H5B2_open(bt2_udata->f, bt2_udata->other_bt2_addr, NULL)))
      HGOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                  "unable to open v2 B-tree for 'other' index");
  ...
  if (H5G__link_name_replace(bt2_udata->f, bt2_udata->grp_full_path_r,
                             fh_udata.lnk) < 0)
  ...
  if (bt2 && H5B2_close(bt2) < 0)
    HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, FAIL,
                "can't close v2 B-tree for 'other' index");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__dense_remove_by_idx(H5F_t *f, const H5O_linfo_t *linfo,
  ...
  if (H5_addr_defined(bt2_addr)) {
    ...
    udata.other_bt2_addr = idx_type == H5_INDEX_NAME ? linfo->corder_bt2_addr
                                                     : linfo->name_bt2_addr;
    udata.grp_full_path_r = grp_full_path_r;

    /* Remove the record from the name index v2 B-tree */
    if (H5B2_remove_by_idx(bt2, order, n, H5G__dense_remove_by_idx_bt2_cb,
                           &udata) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTREMOVE, FAIL,
                  "unable to remove link from indexed v2 B-tree");
  }      /* end if */
  else { /* Otherwise, we need to build a table of the links and sort it */
    /* Build the table of links for this group */
    if (H5G__dense_build_table(f, linfo, idx_type, order, &ltable) < 0)
    ...
    if (H5G__dense_remove(f, linfo, grp_full_path_r, ltable.lnks[n].name) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTREMOVE, FAIL,
                  "unable to remove link from dense storage");
  } /* end else */
...
herr_t H5G__dense_delete(H5F_t *f, H5O_linfo_t *linfo, hbool_t adj_link) {
  ...
  if (adj_link) {
    ...
    udata.common.found_op = NULL;
    udata.common.found_op_data = NULL;
    udata.rem_from_fheap =
        FALSE; /* handled in "bulk" below by deleting entire heap */
    udata.corder_bt2_addr = linfo->corder_bt2_addr;
    udata.grp_full_path_r = NULL;
    udata.replace_names = FALSE;

    /* Delete the name index, adjusting the ref. count on links removed */
    if (H5B2_delete(f, linfo->name_bt2_addr, NULL, H5G__dense_remove_bt2_cb,
                    &udata) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                  "unable to delete v2 B-tree for name index");
  ...
  if (linfo->index_corder) {
    /* Delete the creation order index, without adjusting the ref. count on the
     * links  */
    assert(H5_addr_defined(linfo->corder_bt2_addr));
