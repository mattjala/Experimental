...
/* Selection callbacks */
static herr_t H5S__point_copy(H5S_t *dst, const H5S_t *src,
...
static herr_t H5S__point_bounds(const H5S_t *space, hsize_t *start,
                                hsize_t *end);
...
static htri_t H5S__point_intersect_block(H5S_t *space, const hsize_t *start,
                                         const hsize_t *end);
...
static herr_t H5S__point_iter_coords(const H5S_sel_iter_t *iter,
                                     hsize_t *coords);
...
static herr_t H5S__point_iter_init(H5S_t *space, H5S_sel_iter_t *iter) {
  ...
  if ((iter->flags & H5S_SEL_ITER_API_CALL) &&
    ...
    /* Copy the point list */
    if (NULL == (iter->u.pnt.pnt_lst = H5S__copy_pnt_list(
                     space->select.sel_info.pnt_lst, space->extent.rank)))
...
static herr_t H5S__point_iter_get_seq_list(H5S_sel_iter_t *iter, size_t maxseq,
  ...
  while (NULL != node) {
    ...
    if (curr_seq > 0) {
      /* If a sorted sequence is requested, make certain we don't go backwards
       * in the offset */
      if ((iter->flags & H5S_SEL_ITER_GET_SEQ_LIST_SORTED) &&
          loc < off[curr_seq - 1])
        break;
...
static herr_t H5S__point_add(H5S_t *space, H5S_seloper_t op, size_t num_elem,
                             const hsize_t *coord) {
  H5S_pnt_node_t *top = NULL, *curr = NULL,
                 *new_node = NULL; /* Point selection nodes */
  unsigned u;                      /* Counter */
  herr_t ret_value = SUCCEED;      /* Return value */
  ...
  assert(num_elem > 0);
  assert(coord);
  assert(op == H5S_SELECT_SET || op == H5S_SELECT_APPEND ||
         op == H5S_SELECT_PREPEND);

  for (u = 0; u < num_elem; u++) {
    unsigned dim; /* Counter for dimensions */

    /* Allocate space for the new node */
    if (NULL == (new_node = (H5S_pnt_node_t *)H5FL_ARR_MALLOC(
                     hcoords_t, space->extent.rank)))
    ...
    H5MM_memcpy(new_node->pnt, coord + (u * space->extent.rank),
                (space->extent.rank * sizeof(hsize_t)));
...
herr_t H5S_select_elements(H5S_t *space, H5S_seloper_t op, size_t num_elem,
  ...
  if (H5S_GET_SELECT_TYPE(space) != H5S_SEL_POINTS ||
    ...
    H5VM_array_fill(space->select.sel_info.pnt_lst->low_bounds, &tmp,
                    sizeof(hsize_t), space->extent.rank);
    memset(space->select.sel_info.pnt_lst->high_bounds, 0,
           sizeof(hsize_t) * space->extent.rank);
  }
...
static H5S_pnt_list_t *H5S__copy_pnt_list(const H5S_pnt_list_t *src,
  ...
  if (NULL == (dst = H5FL_MALLOC(H5S_pnt_list_t)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, NULL,
                "can't allocate point list node");
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S__point_copy(dst, src, share_selection)
        ...
        hbool_t share_selection; IN: Whether to share the selection between the
dataspaces RETURNS Non-negative on success/Negative on failure DESCRIPTION
...
static herr_t H5S__point_copy(H5S_t *dst, const H5S_t *src,
  ...
  /* Allocate room for the head of the point list */
  if (NULL == (dst->select.sel_info.pnt_lst = H5S__copy_pnt_list(
                   src->select.sel_info.pnt_lst, src->extent.rank)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCOPY, FAIL, "can't copy point list");
...
static htri_t H5S__point_is_valid(const H5S_t *space) {
  ...
  for (u = 0; u < space->extent.rank; u++) {
    ...
    if ((space->select.sel_info.pnt_lst->high_bounds[u] +
         (hsize_t)space->select.offset[u]) > space->extent.size[u])
      HGOTO_DONE(FALSE);
    if (((hssize_t)space->select.sel_info.pnt_lst->low_bounds[u] +
...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    Determine the version and the size (2, 4 or 8 bytes) to encode point
selection info USAGE hssize_t H5S__point_get_version_enc_size(space, version,
enc_size) const H5S_t *space:         IN: Dataspace ID of selection to query
 ...
 DESCRIPTION
    ...
    (2) whether the number of points or selection high bounds exceeds
...
static herr_t H5S__point_get_version_enc_size(const H5S_t *space,
  ...
  hbool_t count_up_version =
      FALSE; /* Whether number of points exceed H5S_UINT32_MAX */
  ...
  if (tmp_version > H5O_sds_point_ver_bounds[high_bound]) {
    if (count_up_version)
      ...
      HGOTO_ERROR(H5E_DATASPACE, H5E_BADRANGE, FAIL,
                  "Dataspace point selection version out of bounds");
  } /* end if */
...
static hssize_t H5S__point_serial_size(H5S_t *space) {
  ...
  if (H5S__point_get_version_enc_size(space, &version, &enc_size) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                "can't determine version and enc_size");
  ...
  ret_value +=
      (hssize_t)(enc_size * space->extent.rank * space->select.num_elem);
...
static herr_t H5S__point_serialize(H5S_t *space, uint8_t **p) {
  ...
  if (version >= 2) {
    *(pp)++ = enc_size; /* Store size of point info */
    ...
    len += 8;  /* Add in advance # of bytes for num of dimensions and num
                  elements  */
  }
...
static herr_t H5S__point_deserialize(H5S_t **space, const uint8_t **p,
  ...
  if (NULL ==
      (coord = (hsize_t *)H5MM_malloc(num_elem * rank * sizeof(hsize_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
  ...
  if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, pp, coordinate_buffer_requirement,
                                  p_end))
    HGOTO_ERROR(H5E_DATASPACE, H5E_OVERFLOW, FAIL,
  ...
  /* Select points */
  if (H5S_select_elements(tmp_space, H5S_SELECT_SET, num_elem,
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__get_select_elem_pointlist(const H5S_t *space,
                                             hsize_t startpoint,
                                             hsize_t numpoints, hsize_t *buf) {
  const hsize_t endpoint =
      startpoint + numpoints; /* Index of last point in iteration */
  H5S_pnt_node_t *node;       /* Point node */
...
static herr_t H5S__point_bounds(const H5S_t *space, hsize_t *start,
  ...
  for (u = 0; u < space->extent.rank; u++) {
    ...
    start[u] =
        (hsize_t)((hssize_t)space->select.sel_info.pnt_lst->low_bounds[u] +
                  space->select.offset[u]);
    end[u] =
        (hsize_t)((hssize_t)space->select.sel_info.pnt_lst->high_bounds[u] +
                  space->select.offset[u]);
  } /* end for */
...
static herr_t H5S__point_offset(const H5S_t *space, hsize_t *offset) {
  ...
  *offset = 0;
  ...
  for (i = (int)space->extent.rank - 1; i >= 0; i--) {
    hssize_t pnt_offset =
        (hssize_t)pnt[i] + sel_offset[i]; /* Point's offset in this dimension */
...
static htri_t H5S__point_is_contiguous(const H5S_t *space) {
  ...
  if (space->select.num_elem == 1)
    ret_value = TRUE;
  else /* More than one point might be contiguous, but it's complex to check and
...
static htri_t H5S__point_shape_same(H5S_t *space1, H5S_t *space2) {
  ...
  while (space2_dim >= 0) {
    ...
    offset[space1_dim] =
        (hssize_t)pnt2->pnt[space2_dim] - (hssize_t)pnt1->pnt[space1_dim];
  ...
  while (pnt1 && pnt2) {
    ...
    while (space2_dim >= 0) {
      if ((hsize_t)((hssize_t)pnt1->pnt[space1_dim] + offset[space1_dim]) !=
          pnt2->pnt[space2_dim])
        HGOTO_DONE(FALSE);
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
htri_t H5S__point_intersect_block(H5S_t *space, const hsize_t *start,
                                  const hsize_t *end) {
...
static herr_t H5S__point_adjust_s(H5S_t *space, const hssize_t *offset) {
  ...
  if (non_zero_offset) {
    ...
    /* update the bound box of the selection */
    for (u = 0; u < rank; u++) {
      assert((hssize_t)space->select.sel_info.pnt_lst->low_bounds[u] >=
             offset[u]);
...
static herr_t H5S__point_project_scalar(const H5S_t *space, hsize_t *offset) {
  ...
  *offset =
      H5VM_array_offset(space->extent.rank, space->extent.size, node->pnt);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__point_project_simple(const H5S_t *base_space,
  ...
  /* Allocate room for the head of the point list */
  if (NULL ==
      (new_space->select.sel_info.pnt_lst = H5FL_MALLOC(H5S_pnt_list_t)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                "can't allocate point list node");
  ...
  if (new_space->extent.rank < base_space->extent.rank) {
    ...
    H5MM_memcpy(block, base_space->select.sel_info.pnt_lst->head->pnt,
                sizeof(hsize_t) * rank_diff);
    *offset = H5VM_array_offset(base_space->extent.rank,
    ...
    while (base_node) {
      /* Create new point */
      if (NULL == (new_node = (H5S_pnt_node_t *)H5FL_ARR_MALLOC(
                       hcoords_t, new_space->extent.rank)))
        HGOTO_ERROR(H5E_DATASPACE, H5E_CANTALLOC, FAIL,
                    "can't allocate point node");
      new_node->next = NULL;
...
herr_t H5Sselect_elements(hid_t spaceid, H5S_seloper_t op, size_t num_elem,
  ...
  if (H5S_SCALAR == H5S_GET_EXTENT_TYPE(space))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "point doesn't support H5S_SCALAR space");
  if (H5S_NULL == H5S_GET_EXTENT_TYPE(space))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "point doesn't support H5S_NULL space");
  if (coord == NULL || num_elem == 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "elements not specified");
  if (!(op == H5S_SELECT_SET || op == H5S_SELECT_APPEND ||
        op == H5S_SELECT_PREPEND))
