...
static const unsigned H5L_def_intmd_group_g =
    H5L_CRT_INTERMEDIATE_GROUP_DEF; /* Default setting for creating intermediate
                                       groups */
...
static herr_t H5P__lcrt_reg_prop(H5P_genclass_t *pclass) {
  ...
  if (H5P__register_real(
          pclass, H5L_CRT_INTERMEDIATE_GROUP_NAME,
          H5L_CRT_INTERMEDIATE_GROUP_SIZE, &H5L_def_intmd_group_g, NULL, NULL,
          NULL, H5L_CRT_INTERMEDIATE_GROUP_ENC, H5L_CRT_INTERMEDIATE_GROUP_DEC,
...
herr_t H5Pset_create_intermediate_group(hid_t plist_id,
  ...
  if (H5P_set(plist, H5L_CRT_INTERMEDIATE_GROUP_NAME, &crt_intmd_group) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                "can't set intermediate group creation flag");
  ...
  if (crt_intmd_group)
    if (H5P_get(plist, H5L_CRT_INTERMEDIATE_GROUP_NAME, crt_intmd_group) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get intermediate group creation flag");
