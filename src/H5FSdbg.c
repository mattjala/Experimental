 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                  int fwidth) {
   ...
   */
  if (NULL ==
      (fspace = (H5FS_t *)H5AC_protect(f, H5AC_FSPACE_HDR, addr, &cache_udata,
                                       H5AC__READ_ONLY_FLAG)))
    HGOTO_ERROR(H5E_FSPACE, H5E_CANTLOAD, FAIL,
  ...
  fprintf(stream, "%*s%-*s %u%%\n", indent, "", fwidth,
          "Shrink percent:", fspace->shrink_percent);
  fprintf(stream, "%*s%-*s %u%%\n", indent, "", fwidth,
          "Expand percent:", fspace->expand_percent);
  fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
  ...
  if (fspace &&
      H5AC_unprotect(f, H5AC_FSPACE_HDR, addr, fspace, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_FSPACE, H5E_PROTECT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_sect_debug(const H5FS_t *fspace, const H5FS_section_info_t *sect,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FS_sects_debug(H5F_t *f, haddr_t H5_ATTR_UNUSED addr, FILE *stream,
                        int indent, int fwidth, haddr_t fs_addr,
                        haddr_t client_addr) {
  H5FS_t *fspace = NULL;           /* Free space header info */
   ...
   */
  if (H5AC_unprotect(f, H5AC_FSPACE_HDR, fs_addr, fspace, H5AC__DELETED_FLAG) <
      0)
    HDONE_ERROR(H5E_FSPACE, H5E_PROTECT, FAIL,
                "unable to release free space header");
  fspace = NULL;
  ...
  case H5FS_CLIENT_FHEAP_ID:
    if (H5HF_sects_debug(f, client_addr, stream, indent + 3,
                         MAX(0, fwidth - 3)) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_SYSTEM, FAIL,
                  "unable to dump fractal heap free space sections");
    break;

  case H5FS_CLIENT_FILE_ID:
    if (H5MF_sects_debug(f, fs_addr, stream, indent + 3, MAX(0, fwidth - 3)) <
        0)
      HGOTO_ERROR(H5E_FSPACE, H5E_SYSTEM, FAIL,
                  "unable to dump file free space sections");
    break;
