...
/* Local Prototypes */
/********************/
static herr_t H5T__commit_api_common(hid_t loc_id, const char *name,
                                     hid_t type_id, hid_t lcpl_id,
                                     hid_t tcpl_id, hid_t tapl_id,
                                     void **token_ptr,
                                     H5VL_object_t **_vol_obj_ptr);
static hid_t H5T__open_api_common(hid_t loc_id, const char *name, hid_t tapl_id,
                                  void **token_ptr,
                                  H5VL_object_t **_vol_obj_ptr);
...
static herr_t H5T__commit_api_common(hid_t loc_id, const char *name,
  ...
  H5VL_object_t *new_obj =
      NULL; /* VOL object that holds the datatype object and the VOL info */
  ...
  if (H5P_DEFAULT == tcpl_id)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "not datatype creation property list");

  /* Set the LCPL for the API context */
  H5CX_set_lcpl(lcpl_id);

  /* Set up object access arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_TACC, TRUE, &tapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTSET, FAIL,
                "can't set object access arguments");

  /* Commit the type */
  if (NULL == (data = H5VL_datatype_commit(
                   *vol_obj_ptr, &loc_params, name, type_id, lcpl_id, tcpl_id,
  ...
  if (NULL == (new_obj = H5VL_create_object(data, (*vol_obj_ptr)->connector)))
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTALLOC, FAIL,
                "can't create VOL object for committed datatype");

  /* Set the committed type object to the VOL connector pointer in the H5T_t
   * struct */
  dt->vol_obj = new_obj;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Tcommit2(hid_t loc_id, const char *name, hid_t type_id, hid_t lcpl_id,
                  hid_t tcpl_id, hid_t tapl_id) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if ((ret_value = H5T__commit_api_common(loc_id, name, type_id, lcpl_id,
                                          tcpl_id, tapl_id, NULL, NULL)) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTOPENOBJ, FAIL,
                "unable to commit datatype synchronously");
...
herr_t H5Tcommit_async(const char *app_file, const char *app_func,
  ...
  FUNC_ENTER_API(FAIL)
  H5TRACE10("e", "*s*sIui*siiiii", app_file, app_func, app_line, loc_id, name,
  ...
  if ((ret_value =
           H5T__commit_api_common(loc_id, name, type_id, lcpl_id, tcpl_id,
                                  tapl_id, token_ptr, &vol_obj)) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTOPENOBJ, FAIL,
                "unable to commit datatype asynchronously");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__commit_named(const H5G_loc_t *loc, const char *name, H5T_t *dt,
  ...
  if (H5L_link_object(loc, name, &ocrt_info, lcpl_id) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                "unable to create and link to named datatype");
  ...
  if (ret_value < 0 && (NULL != ocrt_info.new_obj)) {
    if (dt->shared->state == H5T_STATE_OPEN &&
        dt->sh_loc.type == H5O_SHARE_TYPE_COMMITTED) {
      ...
      if (H5O_close(&(dt->oloc), NULL) < 0)
        HDONE_ERROR(H5E_DATATYPE, H5E_CLOSEERROR, FAIL,
...
herr_t H5Tcommit_anon(hid_t loc_id, hid_t type_id, hid_t tcpl_id,
  ...
  if (H5P_DEFAULT == tapl_id)
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "not datatype access property list");
  ...
  /* Commit the datatype */
  if (NULL ==
      (dt = H5VL_datatype_commit(vol_obj, &loc_params, NULL, type_id,
                                 H5P_LINK_CREATE_DEFAULT, tcpl_id, tapl_id,
...
herr_t H5T__commit(H5F_t *file, H5T_t *type, hid_t tcpl_id) {
   ...
   */
  if (H5T_STATE_NAMED == type->shared->state ||
      H5T_STATE_OPEN == type->shared->state)
  ...
  if (H5O_create(file, dtype_size, (size_t)1, tcpl_id, &temp_oloc) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                "unable to create datatype object header");
  if (H5O_msg_create(&temp_oloc, H5O_DTYPE_ID,
                     H5O_MSG_FLAG_CONSTANT | H5O_MSG_FLAG_DONTSHARE,
                     H5O_UPDATE_TIME, type) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                "unable to update type header message");

  /* Copy the new object header's location into the datatype, taking ownership
   * of it */
  if (H5O_loc_copy_shallow(&(type->oloc), &temp_oloc) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                "unable to copy datatype location");
  ...
  if (H5FO_top_incr(type->sh_loc.file, type->sh_loc.u.loc.oh_addr) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINC, FAIL,
                "can't incr object ref. count");
  if (H5FO_insert(type->sh_loc.file, type->sh_loc.u.loc.oh_addr, type->shared,
                  TRUE) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINSERT, FAIL,
  ...
  if (H5T_set_loc(type, NULL, H5T_LOC_MEMORY) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                "cannot mark datatype in memory");
  ...
  if (ret_value < 0) {
    ...
    if ((type->shared->state == H5T_STATE_TRANSIENT ||
         type->shared->state == H5T_STATE_RDONLY) &&
...
int H5T_link(const H5T_t *type, int adjust) {
  ...
  if ((ret_value = H5O_link(&type->oloc, adjust)) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_LINKCOUNT, (-1),
                "unable to adjust named datatype link count");
...
static hid_t H5T__open_api_common(hid_t loc_id, const char *name, hid_t tapl_id,
  ...
  /* Set up object access arguments */
  if (H5VL_setup_acc_args(loc_id, H5P_CLS_TACC, FALSE, &tapl_id, vol_obj_ptr,
                          &loc_params) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTSET, H5I_INVALID_HID,
                "can't set object access arguments");

  /* Open the datatype */
  if (NULL == (dt = H5VL_datatype_open(*vol_obj_ptr, &loc_params, name, tapl_id,
                                       H5P_DATASET_XFER_DEFAULT, token_ptr)))
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open named datatype");

  /* Register the type and return the ID */
  if ((ret_value = H5VL_register(H5I_DATATYPE, dt, (*vol_obj_ptr)->connector,
                                 TRUE)) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register named datatype");

done:
  /* Cleanup on error */
  if (H5I_INVALID_HID == ret_value)
    if (dt && H5VL_datatype_close(*vol_obj_ptr, H5P_DATASET_XFER_DEFAULT,
                                  H5_REQUEST_NULL) < 0)
      HDONE_ERROR(H5E_DATATYPE, H5E_CLOSEERROR, H5I_INVALID_HID,
...
hid_t H5Topen_async(const char *app_file, const char *app_func,
  ...
  if ((ret_value = H5T__open_api_common(loc_id, name, tapl_id, token_ptr,
                                        &vol_obj)) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
  ...
  if (NULL != token)
    /* clang-format off */
      ...
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINSERT, H5I_INVALID_HID,
                  "can't insert token into event set");
    } /* end if */
...
hid_t H5Tget_create_plist(hid_t dtype_id) {
  ...
  if (FAIL == (is_named = H5T_is_named(type)))
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, H5I_INVALID_HID,
                "can't check whether datatype is committed");
  ...
  if (FALSE == is_named) {
    ...
    if (NULL == (tcpl_plist = (H5P_genplist_t *)H5I_object(
      ...
      HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, H5I_INVALID_HID,
                  "can't get default creation property list");
  ...
  /* If the datatype is committed, retrieve further information */
    ...
    if (H5VL_datatype_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                          H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, H5I_INVALID_HID,
...
herr_t H5Tflush(hid_t type_id) {
  ...
  if (dt->vol_obj) {
    ...
    if (H5VL_datatype_specific(dt->vol_obj, &vol_cb_args,
                               H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTFLUSH, FAIL,
                  "unable to flush datatype");
  }
...
hid_t H5T__get_create_plist(const H5T_t *type) {
  ...
  /* Copy the default datatype creation property list */
  if (NULL ==
      (tcpl_plist = (H5P_genplist_t *)H5I_object(H5P_LST_DATATYPE_CREATE_ID_g)))
  ...
  if (ret_value < 0)
    if (new_tcpl_id > 0)
      if (H5I_dec_app_ref(new_tcpl_id) < 0)
        HDONE_ERROR(H5E_DATATYPE, H5E_CANTDEC, H5I_INVALID_HID,
...
H5T_t *H5T_open(const H5G_loc_t *loc) {
  ...
  if (NULL == (shared_fo = (H5T_shared_t *)H5FO_opened(loc->oloc->file,
                                                       loc->oloc->addr))) {
    ...
    if (H5FO_insert(dt->sh_loc.file, dt->sh_loc.u.loc.oh_addr, dt->shared,
                    FALSE) < 0)
    ...
    if (NULL == (dt = H5FL_MALLOC(H5T_t)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                  "can't allocate space for datatype");
    dt->vol_obj = NULL;
...
herr_t H5T_update_shared(H5T_t *dt) {
  ...
  H5O_UPDATE_SHARED(&(dt->sh_loc), H5O_SHARE_TYPE_COMMITTED, dt->oloc.file,
                    H5O_DTYPE_ID, 0, dt->oloc.addr)
 ...
 *
 * Purpose:     Create a Library datatype with a connector specific datatype
 *object
 *
...
H5T_t *H5T_construct_datatype(H5VL_object_t *vol_obj) {
  ...
  if (H5VL_datatype_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                        H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, NULL,
                "unable to get datatype serialized size");
  ...
  if (H5VL_datatype_get(vol_obj, &vol_cb_args, H5P_DATASET_XFER_DEFAULT,
                        H5_REQUEST_NULL) < 0)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, NULL,
                "unable to get serialized datatype");
...
herr_t H5T_save_refresh_state(hid_t tid, H5O_shared_t *cached_H5O_shared) {
  H5T_t *dt = NULL; /* High level datatype object that wraps the VOL object */
  H5T_t *vol_dt =
      NULL; /* H5T_t pointer stored in the datatype's vol_obj field */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (NULL == vol_dt)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "tid is not not a named datatype ID");
 ...
 *
 * Purpose:     Invokes an optional VOL connector-specific operation on a named
 *datatype
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T_invoke_vol_optional(H5T_t *dt, H5VL_optional_args_t *args,
  ...
  if (dt->vol_obj)
    if (H5VL_datatype_optional_op(dt->vol_obj, args, dxpl_id, req,
                                  vol_obj_ptr) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTOPERATE, FAIL,
