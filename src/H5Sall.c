...
/* Selection callbacks */
static herr_t H5S__all_copy(H5S_t *dst, const H5S_t *src,
...
static htri_t H5S__all_intersect_block(H5S_t *space, const hsize_t *start,
                                       const hsize_t *end);
...
static herr_t H5S__all_project_simple(const H5S_t *space, H5S_t *new_space,
                                      hsize_t *offset);
...
static herr_t H5S__all_iter_block(const H5S_sel_iter_t *iter, hsize_t *start,
                                  hsize_t *end);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__all_iter_init(H5S_t H5_ATTR_UNUSED *space,
                                 H5S_sel_iter_t *iter) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__all_iter_coords(const H5S_sel_iter_t *iter,
                                   hsize_t *coords) {
  ...
  if (H5VM_array_calc(iter->u.all.elmt_offset, iter->rank, iter->dims, coords) <
      0)
...
and the number of sequences actually generated is stored in the NSEQ parameter.
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__all_iter_get_seq_list(H5S_sel_iter_t *iter,
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__all_copy(H5S_t *dst, const H5S_t H5_ATTR_UNUSED *src,
...
static herr_t H5S__all_deserialize(H5S_t **space, const uint8_t **p,
  ...
  if (version < H5S_ALL_VERSION_1 || version > H5S_ALL_VERSION_LATEST)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                "bad version number for all selection");

  /* Skip over the remainder of the header */
  if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, *p, 8, p_end))
    HGOTO_ERROR(H5E_DATASPACE, H5E_OVERFLOW, FAIL,
                "buffer overflow while decoding header");
  *p += 8;
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__all_bounds(const H5S_t *space, hsize_t *start,
                              hsize_t *end) {
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__all_offset(const H5S_t H5_ATTR_UNUSED *space,
                              hsize_t *offset) {
  FUNC_ENTER_PACKAGE_NOERR
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
htri_t H5S__all_intersect_block(H5S_t H5_ATTR_UNUSED *space,
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__all_adjust_u(H5S_t H5_ATTR_UNUSED *space,
                                const hsize_t H5_ATTR_UNUSED *offset) {
  FUNC_ENTER_PACKAGE_NOERR
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5S__all_adjust_s(H5S_t H5_ATTR_UNUSED *space,
                                const hssize_t H5_ATTR_UNUSED *offset) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__all_project_scalar(const H5S_t H5_ATTR_UNUSED *space,
                                      hsize_t *offset) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5S__all_project_simple(const H5S_t H5_ATTR_UNUSED *base_space,
                                      H5S_t *new_space,
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5S_select_all(dsid)
        ...
        hbool_t rel_prev;      IN: Flag whether to release previous selection or
not RETURNS Non-negative on success/Negative on failure DESCRIPTION This
function selects the entire extent for a dataspace. GLOBAL VARIABLES COMMENTS,
BUGS, ASSUMPTIONS EXAMPLES REVISION LOG
--------------------------------------------------------------------------*/
