 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_shared_block_read(H5F_shared_t *f_sh, H5FD_mem_t type, haddr_t addr,
  ...
  if (H5_addr_le(f_sh->tmp_addr, (addr + size)))
    HGOTO_ERROR(H5E_IO, H5E_BADRANGE, FAIL,
                "attempting I/O in temporary file space");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_block_read(H5F_t *f, H5FD_mem_t type, haddr_t addr, size_t size,
                      void *buf /*out*/) {
  H5FD_mem_t map_type;        /* Mapped memory type */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_shared_block_write(H5F_shared_t *f_sh, H5FD_mem_t type, haddr_t addr,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_block_write(H5F_t *f, H5FD_mem_t type, haddr_t addr, size_t size,
                       const void *buf) {
  H5FD_mem_t map_type;        /* Mapped memory type */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_shared_select_read(H5F_shared_t *f_sh, H5FD_mem_t type,
                              uint32_t count, H5S_t **mem_spaces,
  ...
  if (H5FD_read_selection(f_sh->lf, map_type, count, mem_spaces, file_spaces,
                          offsets, element_sizes, bufs) < 0)
    HGOTO_ERROR(H5E_IO, H5E_READERROR, FAIL,
                "selection read through file driver failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_shared_select_write(H5F_shared_t *f_sh, H5FD_mem_t type,
                               uint32_t count, H5S_t **mem_spaces,
                               H5S_t **file_spaces, haddr_t offsets[],
  ...
  /* Pass down to file driver layer (bypass page buffer for now) */
  if (H5FD_write_selection(f_sh->lf, map_type, count, mem_spaces, file_spaces,
                           offsets, element_sizes, bufs) < 0)
    HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                "selection write through file driver failed");

done:
  FUNC_LEAVE_NOAPI(ret_value)
} /* end H5F_shared_select_write() */

herr_t H5F_shared_vector_read(H5F_shared_t *f_sh, uint32_t count,
  ...
  if (H5FD_read_vector(f_sh->lf, count, types, addrs, sizes, bufs) < 0)
    HGOTO_ERROR(H5E_IO, H5E_READERROR, FAIL,
                "vector read through file driver failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_shared_vector_write(H5F_shared_t *f_sh, uint32_t count,
                               H5FD_mem_t types[], haddr_t addrs[],
  ...
  if (H5FD_write_vector(f_sh->lf, count, types, addrs, sizes, bufs) < 0)
    HGOTO_ERROR(H5E_IO, H5E_WRITEERROR, FAIL,
                "vector write through file driver failed");
...
herr_t H5F__evict_cache_entries(H5F_t *f) {
  ...
  if (H5AC_evict(f) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTEXPUNGE, FAIL,
                "unable to evict all except pinned entries");
...
#ifndef NDEBUG
  {
    ...
    if (H5AC_get_cache_size(f->shared->cache, NULL, NULL, NULL,
                            &cur_num_entries) < 0)
    ...
    if (cur_num_entries != 1)
      HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                  "number of cache entries is not correct");
  }
#endif /* NDEBUG */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_get_checksums(const uint8_t *buf, size_t buf_size,
                         uint32_t *s_chksum /*out*/,
                         uint32_t *c_chksum /*out*/) {
