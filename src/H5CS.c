...
#define H5CS_MIN_NSLOTS                                                        \
  16 /* Minimum number of records in an function stack	*/
...
static H5CS_t *H5CS__get_stack(void) {
  ...
  if (!fstack) {
    ...
    fstack = (H5CS_t *)LocalAlloc(
        LPTR, sizeof(H5CS_t)); /* Win32 has to use LocalAlloc to match the
                                  LocalFree in DllMain */
#else
    fstack =
        (H5CS_t *)malloc(sizeof(H5CS_t)); /* Don't use H5MM_malloc() here, it
...
herr_t H5CS_push(const char *func_name) {
  ...
  if (fstack->nused == fstack->nalloc) {
    ...
    const char **x =
        (const char **)realloc(fstack->rec, na * sizeof(const char *));
...
H5CS_t *H5CS_copy_stack(void) {
  H5CS_t *old_stack =
      H5CS_get_my_stack();  /* Existing function stack for library */
  ...
  if (NULL == (new_stack = calloc(1, sizeof(H5CS_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                "can't allocate function stack");
  if (NULL == (new_stack->rec = calloc(old_stack->nused, sizeof(const char *))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                "can't allocate function stack records");
