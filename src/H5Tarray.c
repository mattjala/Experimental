 ...
 *		The type is an array with NDIMS dimensionality and the size of
 *the array is DIMS. The total member size should be relatively small. Array
 *datatypes are currently limited to H5S_MAX_RANK number of dimensions and must
 *have the number of dimensions set greater than 0. (i.e. 0 > ndims <=
 *H5S_MAX_RANK)  All dimensions sizes must be greater than 0 also.
 *
 ...
 *-------------------------------------------------------------------------
 */
H5T_t *H5T__array_create(H5T_t *base, unsigned ndims,
                         const hsize_t dim[/* ndims */]) {
  unsigned u;              /* Local index variable */
  ...
  for (u = 0, ret_value->shared->u.array.nelem = 1; u < ndims; u++) {
    H5_CHECKED_ASSIGN(ret_value->shared->u.array.dim[u], size_t, dim[u],
                      hsize_t);
  ...
  ret_value->shared->size = ret_value->shared->parent->shared->size *
                            ret_value->shared->u.array.nelem;
 ...
 *-------------------------------------------------------------------------
 */
int H5Tget_array_dims1(hid_t type_id, hsize_t dims[] /*out*/,
                       int H5_ATTR_UNUSED perm[] /*out*/) {
