...
herr_t H5G_mkroot(H5F_t *f, hbool_t create_root) {
  ...
  if (H5G__node_init(f) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL,
                "unable to create group node info");
  ...
  if (create_root) {
    ...
    if (f->shared->sblock->super_vers < HDF5_SUPERBLOCK_VERSION_2) {
      ...
      if (NULL == (f->shared->sblock->root_ent =
                       (H5G_entry_t *)H5MM_calloc(sizeof(H5G_entry_t))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "can't allocate space for symbol table entry");
    ...
    if (f->shared->sblock->root_ent &&
       ...
       * symbol table message with the cached symbol table information */
        ...
        cached_stab.btree_addr =
            f->shared->sblock->root_ent->cache.stab.btree_addr;
        cached_stab.heap_addr =
            f->shared->sblock->root_ent->cache.stab.heap_addr;
  ...
  if ((H5F_INTENT(f) & H5F_ACC_RDWR) && stab_exists != FALSE &&
      f->shared->sblock->root_ent &&
      f->shared->sblock->root_ent->type != H5G_CACHED_STAB) {
    ...
    if (stab_exists == -1 &&
        (stab_exists = H5O_msg_exists(root_loc.oloc, H5O_STAB_ID)) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                  "can't check if symbol table message exists");
  ...
  if (ret_value < 0) {
    if (f->shared->root_grp) {
      ...
      if (f->shared->root_grp->shared)
        f->shared->root_grp->shared =
            H5FL_FREE(H5G_shared_t, f->shared->root_grp->shared);
      f->shared->root_grp = H5FL_FREE(H5G_t, f->shared->root_grp);
    } /* end if */
    if (f->shared->sblock)
      f->shared->sblock->root_ent =
          (H5G_entry_t *)H5MM_xfree(f->shared->sblock->root_ent);
  } /* end if */
