...
typedef struct {
  void *fl_tbuf; /* Ptr to the temporary buffer we are using for fixed-length
                    data */
...
static herr_t H5D__init_space(H5F_t *file, const H5D_t *dset,
                              const H5S_t *space);
static herr_t H5D__update_oh_info(H5F_t *file, H5D_t *dset, hid_t dapl_id);
static herr_t H5D__build_file_prefix(const H5D_t *dset,
                                     H5F_prefix_open_t prefix_type,
                                     char **file_prefix);
static herr_t H5D__open_oid(H5D_t *dataset, hid_t dapl_id);
static herr_t H5D__init_storage(H5D_t *dset, hbool_t full_overwrite,
                                hsize_t old_dim[]);
...
static herr_t H5D__prepare_minimized_oh(H5F_t *file, H5D_t *dset,
                                        H5O_loc_t *oloc);
static size_t H5D__calculate_minimum_header_size(H5F_t *file, H5D_t *dset,
                                                 H5O_t *ohdr);
static void *H5D__vlen_get_buf_size_alloc(size_t size, void *info);
static herr_t H5D__vlen_get_buf_size_cb(void *elem, hid_t type_id,
                                        unsigned ndim, const hsize_t *point,
                                        void *op_data);
static herr_t H5D__vlen_get_buf_size_gen_cb(void *elem, hid_t type_id,
                                            unsigned ndim, const hsize_t *point,
                                            void *op_data);
...
herr_t H5D_init(void) {
   ...
   */
  if (NULL ==
      (def_dcpl = (H5P_genplist_t *)H5I_object(H5P_LST_DATASET_CREATE_ID_g)))
    HGOTO_ERROR(H5E_DATASET, H5E_BADTYPE, FAIL,
                "can't get default dataset creation property list");
  ...
  if (H5P_get(def_dcpl, H5D_CRT_EXT_FILE_LIST_NAME,
              &H5D_def_dset.dcpl_cache.efl) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't retrieve external file list");
  if (H5P_get(def_dcpl, H5D_CRT_FILL_VALUE_NAME,
              &H5D_def_dset.dcpl_cache.fill) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't retrieve fill value");
  if (H5P_get(def_dcpl, H5O_CRT_PIPELINE_NAME, &H5D_def_dset.dcpl_cache.pline) <
      0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't retrieve pipeline filter");

  /* Retrieve the prefixes of VDS and external file from the environment
   * variable */
 ...
 *-------------------------------------------------------------------------
 */
H5D_t *H5D__create_named(const H5G_loc_t *loc, const char *name, hid_t type_id,
                         const H5S_t *space, hid_t lcpl_id, hid_t dcpl_id,
                         hid_t dapl_id) {
...
static H5D_shared_t *H5D__new(hid_t dcpl_id, hid_t dapl_id, hbool_t creating,
  ...
  if (!vl_type && creating && dcpl_id == H5P_DATASET_CREATE_DEFAULT) {
    if (H5I_inc_ref(dcpl_id, FALSE) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINC, NULL,
                  "can't increment default DCPL ID");
  ...
  if (!vl_type && creating && dapl_id == H5P_DATASET_ACCESS_DEFAULT) {
    if (H5I_inc_ref(dapl_id, FALSE) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINC, NULL,
                  "can't increment default DAPL ID");
...
static herr_t H5D__init_type(H5F_t *file, const H5D_t *dset, hid_t type_id,
  ...
  hbool_t use_at_least_v18;   /* Flag indicating to use at least v18 format
                                 versions */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Copy the datatype if it's a custom datatype or if it'll change when its
   * location is changed */
  if (!immutable || relocatable || use_at_least_v18) {
    ...
    /* Convert a datatype (if committed) to a transient type if the committed
     * datatype's file location is different from the file location where the
     * dataset will be created.
     */
    if (H5T_convert_committed_datatype(dset->shared->type, file) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "can't get shared datatype info");
    ...
    if (H5T_set_version(file, dset->shared->type) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                  "can't set version of datatype");

    /* Get a datatype ID for the dataset's datatype */
    if ((dset->shared->type_id =
             H5I_register(H5I_DATATYPE, dset->shared->type, FALSE)) < 0)
  ...
  /* Not a custom datatype, just use it directly */
    ...
    if (H5I_inc_ref(type_id, FALSE) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINC, FAIL,
                  "Can't increment datatype ID");
...
static herr_t H5D__cache_dataspace_info(const H5D_t *dset) {
  ...
  if ((sndims = H5S_get_simple_extent_dims(dset->shared->space,
                                           dset->shared->curr_dims,
                                           dset->shared->max_dims)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't cache dataspace dimensions");
...
static herr_t H5D__init_space(H5F_t *file, const H5D_t *dset,
  ...
  if (H5S_set_version(file, dset->shared->space) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "can't set latest version of datatype");
...
static size_t H5D__calculate_minimum_header_size(H5F_t *file, H5D_t *dset,
  ...
  if (get_value == 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                "Can't get size of datatype message");
  ret_value += get_value;

  /* Shared Dataspace message size */
  get_value =
      H5O_msg_size_oh(file, ohdr, H5O_SDSPACE_ID, dset->shared->space, 0);
  if (get_value == 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                "can't get size of dataspace message");
  ret_value += get_value;

  /* "Layout" message size */
  get_value =
      H5O_msg_size_oh(file, ohdr, H5O_LAYOUT_ID, &dset->shared->layout, 0);
  if (get_value == 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                "can't get size of layout message");
  ret_value += get_value;
  ...
  if (get_value == 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                "can't get size of fill value message");
  ret_value += get_value;
  ...
  if (get_value == 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                "can't get size of continuation message");
  ret_value += get_value;
  ...
  if (fill_prop->buf && !use_at_least_v18) {
    ...
    if (H5O_msg_reset_share(H5O_FILL_ID, &old_fill_prop) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                  "can't reset the copied fill property");
  ...
  if (H5D_CHUNKED == dset->shared->layout.type) {
    ...
    if (pline->nused > 0) {
      ...
      if (get_value == 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                    "can't get size of filter message");
      ret_value += get_value;
    }
  }
  ...
  if (dset->shared->dcpl_cache.efl.nused > 0) {
    get_value = H5O_msg_size_oh(file, ohdr, H5O_EFL_ID,
                                &dset->shared->dcpl_cache.efl, 0);
    if (get_value == 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                  "can't get size of external file link message");
    ret_value += get_value;
  }
  ...
  if (H5O_HDR_STORE_TIMES & H5O_OH_GET_FLAGS(ohdr)) {
    ...
    if (H5O_OH_GET_VERSION(ohdr) == 1) {
      ...
      if (get_value == 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, 0,
                    "can't get size of modification time message");
      ret_value += get_value;
    }
  }
...
static herr_t H5D__prepare_minimized_oh(H5F_t *file, H5D_t *dset,
  ...
  if (ohdr_size == 0)
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL,
                "computed header size is invalid");

  /* Special allocation of space for compact datasets is handled by the call
   * here. */
  if (H5O_apply_ohdr(file, oh, dset->shared->dcpl_id, ohdr_size, (size_t)1,
                     oloc) == FAIL)
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL,
...
static herr_t H5D__update_oh_info(H5F_t *file, H5D_t *dset, hid_t dapl_id) {
  ...
  hbool_t layout_init =
      FALSE; /* Flag to indicate that chunk information was initialized */
  hbool_t use_at_least_v18; /* Flag indicating to use at least v18 format
                               versions */
  hbool_t use_minimized_header =
      FALSE;                  /* Flag to use minimized dataset object headers */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5T_detect_class(type, H5T_VLEN, FALSE)) {
    /* If the default fill value is chosen for variable-length types, always
     * write it */
    if (fill_prop->fill_time == H5D_FILL_TIME_IFSET &&
  ...
  if (fill_prop->fill_defined == FALSE &&
      fill_prop->fill_time == H5D_FILL_TIME_ALLOC)
    HGOTO_ERROR(
        H5E_DATASET, H5E_CANTINIT, FAIL,
  ...
  if (fill_changed) {
    ...
    if (NULL ==
        (dc_plist = (H5P_genplist_t *)H5I_object(dset->shared->dcpl_id)))
  ...
  if (TRUE == use_minimized_header) {
    ...
    if (H5O_create(file, ohdr_size, (size_t)1, dset->shared->dcpl_id,
                   oloc /*out*/) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
  ...
  if (NULL == (oh = H5O_pin(oloc)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTPIN, FAIL,
                "unable to pin dataset object header");

  /* Write the dataspace header message */
  if (H5S_append(file, oh, dset->shared->space) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to update dataspace header message");

  /* Write the datatype header message */
  if (H5O_msg_append_oh(file, oh, H5O_DTYPE_ID, H5O_MSG_FLAG_CONSTANT, 0,
                        type) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to update datatype header message");

  /* Write new fill value message */
  if (H5O_msg_append_oh(file, oh, H5O_FILL_NEW_ID, H5O_MSG_FLAG_CONSTANT, 0,
                        fill_prop) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to update new fill value header message");

  /* If there is valid information for the old fill value struct, add it */
  /* (only if we aren't using v18 format versions and above */
  if (fill_prop->buf && !use_at_least_v18) {
    H5O_fill_t old_fill_prop; /* Copy of fill value property, for writing as
                                 "old" fill value */
    ...
    if (H5O_msg_append_oh(file, oh, H5O_FILL_ID, H5O_MSG_FLAG_CONSTANT, 0,
                          &old_fill_prop) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "unable to update old fill value header message");
  } /* end if */

  /* Update/create the layout (and I/O pipeline & EFL) messages */
  if (H5D__layout_oh_create(file, oh, dset, dapl_id) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to update layout/pline/efl header message");
  ...
  if (oh != NULL)
    if (H5O_unpin(oh) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTUNPIN, FAIL,
                  "unable to unpin dataset object header");

  /* Error cleanup */
  if (ret_value < 0)
    if (layout_init)
      /* Destroy the layout information for the dataset */
      if (dset->shared->layout.ops->dest &&
          (dset->shared->layout.ops->dest)(dset) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CANTRELEASE, FAIL,
                    "unable to destroy layout info");
  ...
  if (H5F_PREFIX_VDS == prefix_type) {
    ...
    if (prefix == NULL || *prefix == '\0') {
      if (H5CX_get_vds_prefix(&prefix) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                    "can't get the prefix for vds file");
    }
    ...
    if (prefix == NULL || *prefix == '\0') {
      if (H5CX_get_ext_file_prefix(&prefix) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                    "can't get the prefix for the external file");
    }
  } else
  ...
  if (prefix == NULL || *prefix == '\0' || HDstrcmp(prefix, ".") == 0) {
    ...
    *file_prefix = NULL;
    ...
    if (HDstrncmp(prefix, "${ORIGIN}", HDstrlen("${ORIGIN}")) == 0) {
      ...
      HDsnprintf(*file_prefix, file_prefix_len, "%s%s", filepath,
                 prefix + HDstrlen("${ORIGIN}"));
    } /* end if */
    else {
 ...
 *-------------------------------------------------------------------------
 */
H5D_t *H5D__create(H5F_t *file, hid_t type_id, const H5S_t *space,
  ...
  hbool_t layout_init =
      FALSE; /* Flag to indicate that chunk information was initialized */
  hbool_t layout_copied =
      FALSE; /* Flag to indicate that layout message was copied */
  hbool_t fill_copied =
      FALSE; /* Flag to indicate that fill-value message was copied */
  hbool_t pline_copied =
      FALSE; /* Flag to indicate that pipeline message was copied */
  hbool_t efl_copied =
      FALSE; /* Flag to indicate that external file list message was copied */
  ...
  if (!H5S_has_extent(space))
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, NULL,
                "dataspace extent has not been set.");
  ...
  if (NULL ==
      (new_dset->shared = H5D__new(dcpl_id, dapl_id, TRUE, has_vl_type)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
  ...
  if (new_dset->shared->dcpl_id != H5P_DATASET_CREATE_DEFAULT) {
    ...
    if ((ignore_filters =
             H5Z_ignore_filters(new_dset->shared->dcpl_id, dt, space)) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_CANTINIT, NULL,
                  "H5Z_has_optional_filter() failed");

    if (FALSE == ignore_filters) {
      /* Check if the filters in the DCPL can be applied to this dataset */
      if (H5Z_can_apply(new_dset->shared->dcpl_id, new_dset->shared->type_id) <
          0)
        HGOTO_ERROR(H5E_ARGS, H5E_CANTINIT, NULL,
                    "I/O filters can't operate on this dataset");

      /* Make the "set local" filter callbacks for this dataset */
      if (H5Z_set_local(new_dset->shared->dcpl_id, new_dset->shared->type_id) <
          0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                    "unable to set local filter parameters");
    } /* ignore_filters */

    /* Get new dataset's property list object */
    if (NULL ==
        (dc_plist = (H5P_genplist_t *)H5I_object(new_dset->shared->dcpl_id)))
    ...
    if (H5P_get(dc_plist, H5D_CRT_FILL_VALUE_NAME, fill) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, NULL,
                  "can't retrieve fill value info");
    fill_copied = TRUE;
  ...
  /*
   * Check if this dataset is going into a parallel file and set space
   * allocation time. If the dataset has filters applied to it, writes to the
   * dataset must be collective, so we don't need to force early space
   * allocation. Otherwise, we force early space allocation to facilitate
   * independent raw data operations.
   */
  ...
  if (H5D__layout_set_io_ops(new_dset) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                "unable to initialize I/O operations");
  ...
  if (new_dset->shared->layout.ops->construct &&
    ...
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                "unable to construct layout information");

  /* Update the dataset's object header info. */
  if (H5D__update_oh_info(file, new_dset, new_dset->shared->dapl_id) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                "can't update the metadata cache");
  ...
  if (H5D__append_flush_setup(new_dset, new_dset->shared->dapl_id) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                "unable to set up flush append property");

  /* Set the external file prefix */
  if (H5D__build_file_prefix(new_dset, H5F_PREFIX_EFILE,
                             &new_dset->shared->extfile_prefix) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                "unable to initialize external file prefix");

  /* Set the VDS file prefix */
  if (H5D__build_file_prefix(new_dset, H5F_PREFIX_VDS,
                             &new_dset->shared->vds_prefix) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, NULL,
                "unable to initialize VDS prefix");

  /* Add the dataset to the list of opened objects in the file */
  if (H5FO_top_incr(new_dset->oloc.file, new_dset->oloc.addr) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINC, NULL, "can't incr object ref. count");
  if (H5FO_insert(new_dset->oloc.file, new_dset->oloc.addr, new_dset->shared,
                  TRUE) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, NULL,
                "can't insert dataset into list of open objects");
  ...
  if (!ret_value && new_dset) {
    if (new_dset->shared) {
      if (layout_init)
        if (new_dset->shared->layout.ops->dest &&
            (new_dset->shared->layout.ops->dest)(new_dset) < 0)
          HDONE_ERROR(H5E_DATASET, H5E_CANTRELEASE, NULL,
                      "unable to destroy layout info");
      if (pline_copied)
        if (H5O_msg_reset(H5O_PLINE_ID, &new_dset->shared->dcpl_cache.pline) <
            0)
          HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, NULL,
                      "unable to reset I/O pipeline info");
      if (layout_copied)
        if (H5O_msg_reset(H5O_LAYOUT_ID, &new_dset->shared->layout) < 0)
          HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, NULL,
                      "unable to reset layout info");
      if (fill_copied)
        if (H5O_msg_reset(H5O_FILL_ID, &new_dset->shared->dcpl_cache.fill) < 0)
          HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, NULL,
                      "unable to reset fill-value info");
      if (efl_copied)
        if (H5O_msg_reset(H5O_EFL_ID, &new_dset->shared->dcpl_cache.efl) < 0)
          HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, NULL,
                      "unable to reset external file list info");
      if (new_dset->shared->space && H5S_close(new_dset->shared->space) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, NULL,
      ...
      if (H5_addr_defined(new_dset->oloc.addr)) {
        ...
        if (file) {
          if (H5O_delete(file, new_dset->oloc.addr) < 0)
            HDONE_ERROR(H5E_DATASET, H5E_CANTDELETE, NULL,
                        "unable to delete object header");
        } /* end if */
      }   /* end if */
      if (new_dset->shared->dcpl_id != 0 &&
          H5I_dec_ref(new_dset->shared->dcpl_id) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CANTDEC, NULL,
                    "unable to decrement ref count on property list");
      if (new_dset->shared->dapl_id != 0 &&
          H5I_dec_ref(new_dset->shared->dapl_id) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CANTDEC, NULL,
                    "unable to decrement ref count on property list");
      new_dset->shared->extfile_prefix =
          (char *)H5MM_xfree(new_dset->shared->extfile_prefix);
      new_dset->shared->vds_prefix =
          (char *)H5MM_xfree(new_dset->shared->vds_prefix);
...
H5D_t *H5D_open(const H5G_loc_t *loc, hid_t dapl_id) {
  ...
  if (NULL == (shared_fo = (H5D_shared_t *)H5FO_opened(dataset->oloc.file,
                                                       dataset->oloc.addr))) {
    ...
    /* Add the dataset to the list of opened objects in the file */
    if (H5FO_insert(dataset->oloc.file, dataset->oloc.addr, dataset->shared,
                    FALSE) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, NULL,
                  "can't insert dataset into list of open objects");
    ...
    if (extfile_prefix && dataset->shared->extfile_prefix) {
      if (HDstrcmp(extfile_prefix, dataset->shared->extfile_prefix) != 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, NULL,
                    "new external file prefix does not match external file "
                    "prefix of already open dataset");
    } else {
      if (extfile_prefix || dataset->shared->extfile_prefix)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, NULL,
                    "new external file prefix does not match external file "
  ...
  if (ret_value == NULL) {
    ...
    if (dataset) {
      if (shared_fo == NULL && dataset->shared) { /* Need to free shared fo */
        dataset->shared->extfile_prefix =
            (char *)H5MM_xfree(dataset->shared->extfile_prefix);
        dataset->shared->vds_prefix =
            (char *)H5MM_xfree(dataset->shared->vds_prefix);
  ...
  if (dapl_id != H5P_DATASET_ACCESS_DEFAULT &&
    ...
    if (H5P_exist_plist(dapl, H5D_ACS_APPEND_FLUSH_NAME) > 0) {
      ...
      if (info.ndims > 0) {
        ...
        /* Get dataset rank */
        if ((rank = H5S_get_simple_extent_dims(dset->shared->space, curr_dims,
                                               max_dims)) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
        ...
        if (u != info.ndims)
          HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                      "boundary dimension is not valid");
        ...
        H5MM_memcpy(dset->shared->append_flush.boundary, info.boundary,
                    sizeof(info.boundary));
      } /* end if */
    }   /* end if */
  }     /* end if */
...
static herr_t H5D__open_oid(H5D_t *dataset, hid_t dapl_id) {
  ...
  if (NULL == (dataset->shared =
                   H5D__new(H5P_DATASET_CREATE_DEFAULT, dapl_id, FALSE, FALSE)))
  ...
  if (NULL == (dataset->shared->type =
                   (H5T_t *)H5O_msg_read(&(dataset->oloc), H5O_DTYPE_ID, NULL)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to load type info from dataset header");

  if (H5T_set_loc(dataset->shared->type, H5F_VOL_OBJ(dataset->oloc.file),
                  H5T_LOC_DISK) < 0)
  ...
  /* Get a datatype ID for the dataset's datatype */
  if ((dataset->shared->type_id =
           H5I_register(H5I_DATATYPE, dataset->shared->type, FALSE)) < 0)
  ...
  if (H5D_COMPACT == dataset->shared->layout.type) {
    ...
    if (dataset->shared->layout.storage.u.compact.size != dset_data_size)
      HGOTO_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL,
                  "bad value from dataset header - size of compact dataset's "
                  "data buffer doesn't match "
  ...
  if (msg_exists) {
    ...
    if (msg_exists) {
      ...
      /* Set the space allocation time appropriately, based on the type of
       * dataset storage */
  ...
  alloc_time_state = 0;
  if ((dataset->shared->layout.type == H5D_COMPACT &&
       fill_prop->alloc_time == H5D_ALLOC_TIME_EARLY) ||
      (dataset->shared->layout.type == H5D_CONTIGUOUS &&
       fill_prop->alloc_time == H5D_ALLOC_TIME_LATE) ||
      (dataset->shared->layout.type == H5D_CHUNKED &&
       fill_prop->alloc_time == H5D_ALLOC_TIME_INCR) ||
      (dataset->shared->layout.type == H5D_VIRTUAL &&
       fill_prop->alloc_time == H5D_ALLOC_TIME_INCR))
    alloc_time_state = 1;

  /* Set revised fill value properties, if they are different from the defaults
   */
  if (H5P_fill_value_cmp(&H5D_def_dset.dcpl_cache.fill, fill_prop,
                         sizeof(H5O_fill_t))) {
    ...
    if (H5P_set(plist, H5D_CRT_ALLOC_TIME_STATE_NAME, &alloc_time_state) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                  "can't set allocation time state");
  } /* end if */
  ...
  if (ret_value < 0) {
    if (H5_addr_defined(dataset->oloc.addr) &&
        H5O_close(&(dataset->oloc), NULL) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                  "unable to release object header");
    if (dataset->shared) {
      if (layout_init)
        if (dataset->shared->layout.ops->dest &&
            (dataset->shared->layout.ops->dest)(dataset) < 0)
          HDONE_ERROR(H5E_DATASET, H5E_CANTRELEASE, FAIL,
                      "unable to destroy layout info");
...
herr_t H5D_close(H5D_t *dataset) {
  ...
  if (dataset->shared->fo_count == 0) {
    ...
    if (H5D__flush_real(dataset) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                  "unable to flush cached dataset info");

    /* Set a flag to indicate the dataset is closing, before we start freeing
     * things */
    ...
    case H5D_CONTIGUOUS:
      ...
      if (dataset->shared->cache.contig.sieve_buf)
        dataset->shared->cache.contig.sieve_buf =
            (unsigned char *)H5FL_BLK_FREE(
                sieve_buf, dataset->shared->cache.contig.sieve_buf);
      break;
    ...
    case H5D_CHUNKED:
      ...
      if (dataset->shared->cache.chunk.single_piece_info) {
        dataset->shared->cache.chunk.single_piece_info = H5FL_FREE(
    ...
    case H5D_VIRTUAL: {
      ...
      for (i = 0; i < dataset->shared->layout.storage.u.virt.list_nused; i++) {
        ...
        if (dataset->shared->layout.storage.u.virt.list[i].source_dset.dset) {
          assert(
              dataset->shared->layout.storage.u.virt.list[i].source_dset.dset !=
              dataset);
          if (H5D_close(dataset->shared->layout.storage.u.virt.list[i]
                            .source_dset.dset) < 0)
            HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
        ...
        /* Close sub datasets */
        for (j = 0;
             j < dataset->shared->layout.storage.u.virt.list[i].sub_dset_nused;
             j++)
          if (dataset->shared->layout.storage.u.virt.list[i].sub_dset[j].dset) {
            assert(dataset->shared->layout.storage.u.virt.list[i]
                       .sub_dset[j]
                       .dset != dataset);
            if (H5D_close(dataset->shared->layout.storage.u.virt.list[i]
                              .sub_dset[j]
              ...
              HDONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL,
                          "unable to close source dataset");
            dataset->shared->layout.storage.u.virt.list[i].sub_dset[j].dset =
                NULL;
          } /* end if */
      }     /* end for */
    }       /* end block */
    ...
    if (dataset->shared->dcpl_id != H5P_DATASET_CREATE_DEFAULT)
      free_failed |=
          (H5O_msg_reset(H5O_PLINE_ID, &dataset->shared->dcpl_cache.pline) <
           0) ||
    ...
    if (H5AC_cork(dataset->oloc.file, dataset->oloc.addr, H5AC__GET_CORKED,
                  &corked) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "unable to retrieve an object's cork status");
    if (corked)
      if (H5AC_cork(dataset->oloc.file, dataset->oloc.addr, H5AC__UNCORK,
                    NULL) < 0)
        HDONE_ERROR(H5E_DATASET, H5E_CANTUNCORK, FAIL,
    ...
    if (!file_closed && H5F_SHARED(dataset->oloc.file) &&
        H5F_EVICT_ON_CLOSE(dataset->oloc.file)) {
      if (H5AC_flush_tagged_metadata(dataset->oloc.file, dataset->oloc.addr) <
          0)
        HDONE_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                    "unable to flush tagged metadata");
      if (H5AC_evict_tagged_metadata(dataset->oloc.file, dataset->oloc.addr,
                                     FALSE) < 0)
        HDONE_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                    "unable to evict tagged metadata");
    } /* end if */
  ...
  if (free_failed)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "couldn't free a component of the dataset, but the dataset was "
                "freed anyway.");
  ...
  if (dataset->shared->fo_count > 1) {
    ...
    if (H5D__cache_dataspace_info(dataset) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                  "can't cache dataspace info");

    /* Release layout info */
    if (H5O_msg_reset(H5O_LAYOUT_ID, &dataset->shared->layout) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTRESET, FAIL,
                  "unable to reset layout info");

    /* Re-load layout message info */
    if (NULL == H5O_msg_read(&(dataset->oloc), H5O_LAYOUT_ID,
                             &(dataset->shared->layout)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__alloc_storage(H5D_t *dset, H5D_time_alloc_t time_alloc,
                          hbool_t full_overwrite, hsize_t old_dim[]) {
  H5F_t *f;             /* The dataset's file pointer */
  H5O_layout_t *layout; /* The dataset's layout information */
  hbool_t must_init_space =
      FALSE; /* Flag to indicate that space should be initialized */
  hbool_t addr_set =
      FALSE; /* Flag to indicate that the dataset's storage address was set */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* If the data is stored in external files, don't set an address for the
   * layout We assume that external storage is already allocated by the caller,
   * or at least will be before I/O is performed.
   */
  if (!(0 == H5S_GET_EXTENT_NPOINTS(dset->shared->space) ||
        dset->shared->dcpl_cache.efl.nused > 0)) {
    /* Get a pointer to the dataset's layout information */
    layout = &(dset->shared->layout);

    switch (layout->type) {
    case H5D_CONTIGUOUS:
      if (!(*dset->shared->layout.ops->is_space_alloc)(
    ...
    case H5D_COMPACT:
      ...
      if (NULL == layout->storage.u.compact.buf) {
        ...
        if (layout->storage.u.compact.size > 0) {
          if (NULL == (layout->storage.u.compact.buf =
                           H5MM_malloc(layout->storage.u.compact.size)))
            HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                        "unable to allocate memory for compact dataset");
          if (!full_overwrite)
            memset(layout->storage.u.compact.buf, 0,
                   layout->storage.u.compact.size);
    ...
    if (must_init_space) {
      if (layout->type == H5D_CHUNKED) {
        ...
        if (H5P_is_fill_value_defined(&dset->shared->dcpl_cache.fill,
                                      &fill_status) < 0)
          HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                      "can't tell if fill value defined");
...
static herr_t H5D__init_storage(H5D_t *dset, hbool_t full_overwrite,
  ...
  case H5D_COMPACT:
    /* If we will be immediately overwriting the values, don't bother to clear
     * them */
    if (!full_overwrite) {
  ...
  case H5D_CONTIGUOUS:
     ...
     * them */
    if ((dset->shared->dcpl_cache.efl.nused == 0 ||
         dset->shared->dcpl_cache.fill.buf) &&
        !full_overwrite)
 ...
 *           buffer for the new data to read in.  All the VL data read in is
 *actually placed in this buffer, overwriting the previous data.  Needless to
 *say, this data is not actually usable.
 *
 * Return:   Non-negative on success, negative on failure
 *
 *-------------------------------------------------------------------------
 */
static void *H5D__vlen_get_buf_size_alloc(size_t size, void *info) {
  H5D_vlen_bufsize_common_t *vlen_bufsize_com =
      (H5D_vlen_bufsize_common_t *)info;
  ...
  if (size > vlen_bufsize_com->vl_tbuf_size) {
    if (NULL == (vlen_bufsize_com->vl_tbuf = H5FL_BLK_REALLOC(
                     vlen_vl_buf, vlen_bufsize_com->vl_tbuf, size)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, NULL,
                  "can't reallocate temporary VL data buffer");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__vlen_get_buf_size_cb(void H5_ATTR_UNUSED *elem,
                                        hid_t type_id,
                                        unsigned H5_ATTR_UNUSED ndim,
                                        const hsize_t *point, void *op_data) {
  H5D_vlen_bufsize_native_t *vlen_bufsize =
      (H5D_vlen_bufsize_native_t *)op_data;
  ...
  if (H5S_select_elements(vlen_bufsize->fspace, H5S_SELECT_SET, (size_t)1,
                          point) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, H5_ITER_ERROR,
                "can't select point");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__vlen_get_buf_size(H5D_t *dset, hid_t type_id, hid_t space_id,
                              hsize_t *size) {
  H5D_vlen_bufsize_native_t vlen_bufsize = {
  ...
  if (NULL == (vlen_bufsize.common.fl_tbuf =
                   H5FL_BLK_MALLOC(vlen_fl_buf, H5T_get_size(type))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "no temporary buffers available");
  if (NULL ==
      (vlen_bufsize.common.vl_tbuf = H5FL_BLK_MALLOC(vlen_vl_buf, (size_t)1)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "no temporary buffers available");
  vlen_bufsize.common.vl_tbuf_size = 1;

  /* Set the memory manager to the special allocation routine */
  if (H5CX_set_vlen_alloc_info(H5D__vlen_get_buf_size_alloc,
                               &vlen_bufsize.common, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "can't set VL data allocation routine");
  ...
  if (vlen_bufsize.common.fl_tbuf != NULL)
    vlen_bufsize.common.fl_tbuf =
        H5FL_BLK_FREE(vlen_fl_buf, vlen_bufsize.common.fl_tbuf);
  if (vlen_bufsize.common.vl_tbuf != NULL)
    vlen_bufsize.common.vl_tbuf =
        H5FL_BLK_FREE(vlen_vl_buf, vlen_bufsize.common.vl_tbuf);
 ...
 *           element from a dataset in memory, creating a selection with just
 *the single element selected to read in the element and using a custom memory
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__vlen_get_buf_size_gen_cb(void H5_ATTR_UNUSED *elem,
                                            hid_t type_id,
                                            unsigned H5_ATTR_UNUSED ndim,
                                            const hsize_t *point,
                                            void *op_data) {
  H5D_vlen_bufsize_generic_t *vlen_bufsize =
      (H5D_vlen_bufsize_generic_t *)op_data;
  ...
  if (NULL == (vlen_bufsize->common.fl_tbuf =
                   H5FL_BLK_REALLOC(vlen_fl_buf, vlen_bufsize->common.fl_tbuf,
                                    H5T_get_size(dt))))
  ...
  /* Read in the point (with the custom VL memory allocator) */
  if (H5VL_dataset_read(1, &vlen_bufsize->dset_vol_obj, &type_id,
                        &vlen_bufsize->mspace_id, &vlen_bufsize->fspace_id,
                        vlen_bufsize->dxpl_id, &vlen_bufsize->common.fl_tbuf,
                        H5_REQUEST_NULL) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__vlen_get_buf_size_gen(H5VL_object_t *vol_obj, hid_t type_id,
                                  hid_t space_id, hsize_t *size) {
  H5D_vlen_bufsize_generic_t vlen_bufsize = {NULL,
                                             H5I_INVALID_HID,
                                             NULL,
                                             H5I_INVALID_HID,
                                             H5I_INVALID_HID,
  ...
  if (NULL ==
      (vlen_bufsize.fspace = (H5S_t *)H5I_object(vlen_bufsize.fspace_id)))
  ...
  if (H5P_set_vlen_mem_manager(dxpl, H5D__vlen_get_buf_size_alloc,
                               &vlen_bufsize.common, NULL, NULL) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "can't set VL data allocation routine on DXPL");
...
static herr_t H5D__check_filters(H5D_t *dataset) {
  ...
  if (!dataset->shared->checked_filters) {
    ...
    if (H5P_is_fill_value_defined(fill, &fill_status) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "Couldn't retrieve fill value from dataset.");
    ...
    if (fill_status == H5D_FILL_VALUE_DEFAULT ||
      ...
      if (fill->fill_time == H5D_FILL_TIME_ALLOC ||
          (fill->fill_time == H5D_FILL_TIME_IFSET &&
           fill_status == H5D_FILL_VALUE_USER_DEFINED)) {
        /* Filters must have encoding enabled. Ensure that all filters can be
         * applied */
        if (H5Z_can_apply(dataset->shared->dcpl_id, dataset->shared->type_id) <
            0)
...
herr_t H5D__set_extent(H5D_t *dset, const hsize_t *size) {
  ...
  /* Check if we are allowed to modify the space; only datasets with chunked and
   * external storage are allowed to be modified */
  ...
  if (H5D_CONTIGUOUS == dset->shared->layout.type &&
      0 == dset->shared->dcpl_cache.efl.nused)
  ...
  H5MM_memcpy(curr_dims, dset->shared->curr_dims,
              H5S_MAX_RANK * sizeof(curr_dims[0]));
  ...
  if (changed) {
    ...
    hbool_t update_chunks =
        FALSE; /* Flag to indicate chunk cache update is needed */
    ...
    for (dim_idx = 0; dim_idx < dset->shared->ndims; dim_idx++) {
      ...
      if (H5D_CHUNKED == dset->shared->layout.type && dset->shared->ndims > 1) {
        ...
        if (scaled != dset->shared->cache.chunk.scaled_dims[dim_idx]) {
          hsize_t
              scaled_power2up; /* Scaled value, rounded to next power of 2 */
          ...
          /* Check if the number of bits required to encode the scaled size
           * value changed */
          if (dset->shared->cache.chunk.scaled_power2up[dim_idx] !=
              scaled_power2up) {
            /* Update the 'power2up' & 'encode_bits' values for the current
             * dimension */
            dset->shared->cache.chunk.scaled_power2up[dim_idx] =
                scaled_power2up;
      ...
      if (update_chunks)
        ...
        if (H5D__chunk_update_cache(dset) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                      "unable to update cached chunk indices");
    } /* end if */
    ...
    if (H5D_VIRTUAL == dset->shared->layout.type) {
      ...
      if (H5D_virtual_check_min_dims(dset) < 0)
        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                    "virtual dataset dimensions not large enough to contain "
                    "all limited dimensions "
                    "in all selections");

      /* Patch the virtual selection dataspaces */
      for (u = 0; u < dset->shared->layout.storage.u.virt.list_nused; u++) {
        /* Patch extent */
        if (H5S_set_extent(dset->shared->layout.storage.u.virt.list[u]
                               .source_dset.virtual_select,
                           size) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                      "unable to modify size of dataspace");
        dset->shared->layout.storage.u.virt.list[u].virtual_space_status =
            H5O_VIRTUAL_STATUS_CORRECT;

        /* Patch sub-source datasets */
        for (v = 0;
             v < dset->shared->layout.storage.u.virt.list[u].sub_dset_nused;
             v++)
          if (H5S_set_extent(dset->shared->layout.storage.u.virt.list[u]
                                 .sub_dset[v]
                                 .virtual_select,
    ...
    if (expand &&
      ...
      if (H5D__alloc_storage(dset, H5D_ALLOC_EXTEND, FALSE, curr_dims) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                    "unable to extend dataset storage");
    ...
    if (H5D_CHUNKED == dset->shared->layout.type) {
      if (shrink &&
          ((*dset->shared->layout.ops->is_space_alloc)(
               &dset->shared->layout.storage) ||
...
herr_t H5D__flush_sieve_buf(H5D_t *dataset) {
  ...
  if (dataset->shared->cache.contig.sieve_buf &&
      dataset->shared->cache.contig.sieve_dirty) {
    assert(dataset->shared->layout.type !=
           H5D_COMPACT); /* We should never have a sieve buffer for compact
                            storage */

    /* Write dirty data sieve buffer to file */
    if (H5F_shared_block_write(H5F_SHARED(dataset->oloc.file), H5FD_MEM_DRAW,
                               dataset->shared->cache.contig.sieve_loc,
...
herr_t H5D__flush_real(H5D_t *dataset) {
  ...
  if (!dataset->shared->closing)
    ...
    if (dataset->shared->layout.ops->flush &&
        (dataset->shared->layout.ops->flush)(dataset) < 0)
...
herr_t H5D__flush(H5D_t *dset, hid_t dset_id) {
  ...
  /* Currently, H5Oflush causes H5Fclose to trigger an assertion failure in
   * metadata cache. Leave this situation for the future solution */
  if (H5F_HAS_FEATURE(dset->oloc.file, H5FD_FEAT_HAS_MPI))
    HGOTO_ERROR(H5E_DATASET, H5E_UNSUPPORTED, FAIL,
                "H5Oflush isn't supported for parallel");
  ...
  if (H5O_flush_common(&dset->oloc, dset_id) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTFLUSH, FAIL,
                "unable to flush dataset and object flush callback");
...
herr_t H5D__format_convert(H5D_t *dataset) {
  ...
  hbool_t init_new_index =
      FALSE; /* Indicate that the new chunk index is initialized */
  hbool_t delete_old_layout =
      FALSE; /* Indicate that the old layout message is deleted */
  hbool_t add_new_layout =
      FALSE; /* Indicate that the new layout message is added */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  case H5D_CHUNKED:
    ...
    if (new_idx_info.storage->ops->init &&
        (new_idx_info.storage->ops->init)(&new_idx_info, dataset->shared->space,
                                          dataset->oloc.addr) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
    ...
    if (H5_addr_defined(idx_info.storage->idx_addr)) {
      ...
      /* Iterate over the chunks in the current index and insert the chunk
       * addresses into the version 1 B-tree chunk index
       */
      if (H5D__chunk_format_convert(dataset, &idx_info, &new_idx_info) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_BADITER, FAIL,
                    "unable to iterate/convert chunk index");
    } /* end if */
    ...
    if (H5O_msg_remove(&dataset->oloc, H5O_LAYOUT_ID, H5O_ALL, FALSE) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                  "unable to delete layout message");

    delete_old_layout = TRUE;

    /* Append the new layout message to the object header */
    if (H5O_msg_create(&dataset->oloc, H5O_LAYOUT_ID, 0, H5O_UPDATE_TIME,
                       newlayout) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "unable to update layout header message");

    add_new_layout = TRUE;

    /* Release the old (current) chunk index */
    if (idx_info.storage->ops->dest &&
        (idx_info.storage->ops->dest)(&idx_info) < 0)
  ...
  case H5D_VIRTUAL:
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "virtual dataset layout not supported");
  ...
  if (ret_value < 0 && dataset->shared->layout.type == H5D_CHUNKED) {
    ...
    if (add_new_layout)
      if (H5O_msg_remove(&dataset->oloc, H5O_LAYOUT_ID, H5O_ALL, FALSE) < 0)
        HDONE_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                    "unable to delete layout message");
    ...
    if (delete_old_layout)
      if (H5O_msg_create(&dataset->oloc, H5O_LAYOUT_ID, 0, H5O_UPDATE_TIME,
        ...
        HDONE_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                    "unable to add layout header message");
    ...
    if (init_new_index) {
      if (H5_addr_defined(new_idx_info.storage->idx_addr)) {
        ...
        if (H5AC_expunge_tag_type_metadata(dataset->oloc.file,
                                           dataset->oloc.addr, H5AC_BT_ID,
                                           H5AC__NO_FLAGS_SET))
          HDONE_ERROR(H5E_DATASET, H5E_CANTEXPUNGE, FAIL,
                      "unable to expunge index metadata");
      } /* end if */

      /* Delete v1 B-tree chunk index */
      if (new_idx_info.storage->ops->dest &&
          (new_idx_info.storage->ops->dest)(&new_idx_info) < 0)
...
herr_t H5D__mark(const H5D_t *dataset, unsigned flags) {
  ...
  if (flags) {
    ...
    if (flags & H5D_MARK_SPACE) {
      if (H5S_write(dataset->oloc.file, oh, update_flags,
                    dataset->shared->space) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__flush_all_cb(void *_dataset, hid_t H5_ATTR_UNUSED id,
                             void *_udata) {
...
hid_t H5D_get_create_plist(const H5D_t *dset) {
  ...
  /* Check args */
  if (NULL ==
      (dcpl_plist = (H5P_genplist_t *)H5I_object(dset->shared->dcpl_id)))
  ...
  if (H5O_get_create_plist(&dset->oloc, new_plist) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "can't get object creation info");
  ...
  case H5D_COMPACT:
    copied_layout.storage.u.compact.buf =
        H5MM_xfree(copied_layout.storage.u.compact.buf);
    memset(&copied_layout.storage.u.compact, 0,
           sizeof(copied_layout.storage.u.compact));
    break;
  ...
  case H5D_CHUNKED:
    ...
    if (copied_layout.storage.u.chunk.ops)
      /* Reset address and pointer of the array struct for the chunked storage
       * index */
  ...
  if (copied_fill.buf != NULL && copied_fill.type == NULL) {
    ...
    if (NULL ==
        (copied_fill.type = H5T_copy(dset->shared->type, H5T_COPY_TRANSIENT)))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
    ...
    if (!H5T_path_noop(tpath)) {
      hid_t dst_id,
          src_id; /* Source & destination datatypes for type conversion */
      uint8_t *bkg_buf = NULL; /* Background conversion buffer */
      size_t bkg_size;         /* Size of background buffer */

      /* Wrap copies of types to convert */
      dst_id = H5I_register(
          H5I_DATATYPE, H5T_copy(copied_fill.type, H5T_COPY_TRANSIENT), FALSE);
      if (dst_id < 0)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                    "unable to copy/register datatype");
      src_id = H5I_register(H5I_DATATYPE,
                            H5T_copy(dset->shared->type, H5T_COPY_ALL), FALSE);
      if (src_id < 0) {
        H5I_dec_ref(dst_id);
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                    "unable to copy/register datatype");
      ...
      bkg_size =
          MAX(H5T_GET_SIZE(copied_fill.type), H5T_GET_SIZE(dset->shared->type));
      if (H5T_path_bkg(tpath) &&
          NULL == (bkg_buf = H5FL_BLK_CALLOC(type_conv, bkg_size))) {
        H5I_dec_ref(src_id);
        H5I_dec_ref(dst_id);
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                    "memory allocation failed");
      ...
      /* Convert fill value */
      if (H5T_convert(tpath, src_id, dst_id, (size_t)1, (size_t)0, (size_t)0,
      ...
      if (H5I_dec_ref(src_id) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTDEC, FAIL,
                    "unable to close temporary object");
  ...
  /* Reset efl name_offset and heap_addr, these are the values when the dataset
   * is created */
  if (copied_efl.slot) {
    unsigned u;
  ...
  /* Set back the (possibly modified) external file list property to property
   * list */
  if (H5P_poke(new_plist, H5D_CRT_EXT_FILE_LIST_NAME, &copied_efl) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                "unable to set external file list");
...
hid_t H5D_get_access_plist(const H5D_t *dset) {
  ...
  H5D_append_flush_t def_append_flush_info = {
      0};                      /* Default append flush property */
  ...
  if ((new_dapl_id = H5P_copy_plist(old_plist, TRUE)) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL,
                "can't copy dataset access property list");
  ...
  if (dset->shared->layout.type == H5D_CHUNKED) {
    if (H5P_set(new_plist, H5D_ACS_DATA_CACHE_NUM_SLOTS_NAME,
                &(dset->shared->cache.chunk.nslots)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't set data cache number of slots");
    if (H5P_set(new_plist, H5D_ACS_DATA_CACHE_BYTE_SIZE_NAME,
                &(dset->shared->cache.chunk.nbytes_max)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't set data cache byte size");
    if (H5P_set(new_plist, H5D_ACS_PREEMPT_READ_CHUNKS_NAME,
                &(dset->shared->cache.chunk.w0)) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't set preempt read chunks");
    if (H5P_set(new_plist, H5D_ACS_APPEND_FLUSH_NAME,
                &dset->shared->append_flush) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL,
                  "can't set append flush property");
  } else {
    ...
    if (H5P_get(def_dapl, H5D_ACS_DATA_CACHE_NUM_SLOTS_NAME,
                &def_chunk_info.nslots) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                  "can't get data number of slots");
    if (H5P_set(new_plist, H5D_ACS_DATA_CACHE_NUM_SLOTS_NAME,
                &def_chunk_info.nslots) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                  "can't set data cache number of slots");
    ...
    if (H5P_get(def_dapl, H5D_ACS_DATA_CACHE_BYTE_SIZE_NAME,
                &def_chunk_info.nbytes_max) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
    ...
    if (H5P_set(new_plist, H5D_ACS_APPEND_FLUSH_NAME, &def_append_flush_info) <
        0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTSET, FAIL,
                  "can't set append flush property");
  } /* end if-else */
  ...
  if (dset->shared->layout.type == H5D_VIRTUAL) {
    if (H5P_set(new_plist, H5D_ACS_VDS_VIEW_NAME,
                &(dset->shared->layout.storage.u.virt.view)) < 0)
    ...
    if (!def_dapl && NULL == (def_dapl = (H5P_genplist_t *)H5I_object(
  ...
  if (H5P_set(new_plist, H5D_ACS_VDS_PREFIX_NAME, &(dset->shared->vds_prefix)) <
      0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL, "can't set vds prefix");

  /* Set the external file prefix option */
  if (H5P_set(new_plist, H5D_ACS_EFILE_PREFIX_NAME,
              &(dset->shared->extfile_prefix)) < 0)
...
hid_t H5D__get_space(const H5D_t *dset) {
  ...
  if (dset->shared->layout.type == H5D_VIRTUAL)
    if (H5D__virtual_set_extent_unlim(dset) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                  "unable to update virtual dataset extent");
...
hid_t H5D__get_type(const H5D_t *dset) {
  ...
  if (H5T_patch_file(dset->shared->type, dset->oloc.file) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to patch datatype's file pointer");
...
herr_t H5D__refresh(H5D_t *dset, hid_t dset_id) {
  ...
  hbool_t virt_dsets_held =
      FALSE; /* Whether virtual datasets' files are held open */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (virt_dsets_held)
    if (H5D__virtual_release_source_dset_files(head) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_CANTDEC, FAIL,
                  "can't release VDS source files held open");
