...
static herr_t
H5AC__ext_config_2_int_config(const H5AC_cache_config_t *ext_conf_ptr,
 ...
 *-------------------------------------------------------------------------
 */
int H5AC_term_package(void){FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC_create(const H5F_t *f, H5AC_cache_config_t *config_ptr,
  ...
  if (H5F_HAS_FEATURE(f, H5FD_FEAT_HAS_MPI)) {
    ...
    if (NULL == (aux_ptr = H5FL_CALLOC(H5AC_aux_t)))
      HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, FAIL,
                  "Can't allocate H5AC auxiliary structure");
     ...
     */
    if (NULL ==
        (aux_ptr->candidate_slist_ptr = H5SL_create(H5SL_TYPE_HADDR, NULL)))
      HGOTO_ERROR(H5E_CACHE, H5E_CANTCREATE, FAIL,
                  "can't create candidate entry list");

    if (aux_ptr->mpi_rank == 0)
      f->shared->cache = H5C_create(
          H5AC__DEFAULT_MAX_CACHE_SIZE, H5AC__DEFAULT_MIN_CLEAN_SIZE,
          (H5AC_NTYPES - 1), H5AC_class_s, H5AC__check_if_write_permitted, TRUE,
          H5AC__log_flushed_entry, (void *)aux_ptr);
    else
      f->shared->cache = H5C_create(
          H5AC__DEFAULT_MAX_CACHE_SIZE, H5AC__DEFAULT_MIN_CLEAN_SIZE,
          (H5AC_NTYPES - 1), H5AC_class_s, H5AC__check_if_write_permitted, TRUE,
     ...
     */
    f->shared->cache =
        H5C_create(H5AC__DEFAULT_MAX_CACHE_SIZE, H5AC__DEFAULT_MIN_CLEAN_SIZE,
                   (H5AC_NTYPES - 1), H5AC_class_s,
  ...
  if (H5F_USE_MDC_LOGGING(f))
    if (H5C_log_set_up(f->shared->cache, H5F_MDC_LOG_LOCATION(f),
                       H5C_LOG_STYLE_JSON, H5F_START_MDC_LOG_ON_ACCESS(f)) < 0)
  ...
  if (log_enabled) {
    ...
    if (H5C_log_tear_down(f->shared->cache) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_LOGGING, FAIL,
                  "metadata cache logging tear-down failed");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC_expunge_entry(H5F_t *f, const H5AC_class_t *type, haddr_t addr,
                          unsigned flags) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* If currently logging, generate a message */
  if (f->shared->cache->log_info->logging)
    if (H5C_log_write_expunge_entry_msg(f->shared->cache, addr, type->id,
                                        ret_value) < 0)
      HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
...
herr_t H5AC_get_entry_status(const H5F_t *f, haddr_t addr, unsigned *status) {
  ...
  hbool_t is_corked;
  hbool_t is_flush_dep_child;  /* Entry @ addr is in the cache and is a flush
                                  dependency child */
  hbool_t is_flush_dep_parent; /* Entry @ addr is in the cache and is a flush
                                  dependency parent */
  hbool_t image_is_up_to_date; /* Entry @ addr is in the cache and has an up to
                                  date image */
  herr_t ret_value = SUCCEED;  /* Return value */
  ...
  if (H5C_get_entry_status(f, addr, NULL, &in_cache, &is_dirty, &is_protected,
                           &is_pinned, &is_corked, &is_flush_dep_parent,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC_insert_entry(H5F_t *f, const H5AC_class_t *type, haddr_t addr,
    ...
    if (NULL != (aux_ptr = (H5AC_aux_t *)H5C_get_aux_ptr(f->shared->cache))) {
      ...
      if (H5AC__log_inserted_entry((H5AC_info_t *)thing) < 0)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTINS, FAIL,
                    "H5AC__log_inserted_entry() failed");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC_load_cache_image_on_next_protect(H5F_t *f, haddr_t addr,
...
herr_t H5AC_mark_entry_dirty(void *thing) {
  ...
  H5C_t *cache_ptr =
      NULL; /* Pointer to the entry's associated metadata cache */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (cache_ptr != NULL && cache_ptr->log_info != NULL)
    if (cache_ptr->log_info->logging)
      if (H5C_log_write_mark_entry_dirty_msg(cache_ptr, entry_ptr, ret_value) <
          0)
        HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
    ...
    if ((!entry_ptr->is_dirty) && (!entry_ptr->is_protected) &&
      ...
      if (H5AC__log_cleaned_entry(entry_ptr) < 0)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTMARKCLEAN, FAIL,
                    "can't log cleaned entry");
  }
#endif /* H5_HAVE_PARALLEL */

  if (H5C_mark_entry_clean(thing) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTMARKCLEAN, FAIL,
                "can't mark pinned or protected entry clean");

done:
  /* If currently logging, generate a message */
  if (cache_ptr != NULL && cache_ptr->log_info != NULL)
    if (cache_ptr->log_info->logging)
      if (H5C_log_write_mark_entry_clean_msg(cache_ptr, entry_ptr, ret_value) <
          0)
        HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
...
herr_t H5AC_mark_entry_unserialized(void *thing) {
  ...
  if (H5C_mark_entry_unserialized(thing) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTMARKUNSERIALIZED, FAIL,
                "can't mark entry unserialized");

done:
  /* If currently logging, generate a message */
  if (cache_ptr != NULL && cache_ptr->log_info != NULL)
    if (cache_ptr->log_info->logging)
      if (H5C_log_write_mark_unserialized_entry_msg(cache_ptr, entry_ptr,
                                                    ret_value) < 0)
        HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
...
herr_t H5AC_mark_entry_serialized(void *thing) {
  ...
  if (H5C_mark_entry_serialized(thing) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTMARKSERIALIZED, FAIL,
                "can't mark entry serialized");

done:
  /* If currently logging, generate a message */
  if (cache_ptr != NULL && cache_ptr->log_info != NULL)
    if (cache_ptr->log_info->logging)
      if (H5C_log_write_mark_serialized_entry_msg(cache_ptr, entry_ptr,
                                                  ret_value) < 0)
        HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC_move_entry(H5F_t *f, const H5AC_class_t *type, haddr_t old_addr,
                       haddr_t new_addr) {
#ifdef H5_HAVE_PARALLEL
  H5AC_aux_t *aux_ptr;
#endif                        /* H5_HAVE_PARALLEL */
  ...
  /* If currently logging, generate a message */
  if (f->shared->cache->log_info->logging)
    if (H5C_log_write_move_entry_msg(f->shared->cache, old_addr, new_addr,
                                     type->id, ret_value) < 0)
      HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
...
void *H5AC_protect(H5F_t *f, const H5AC_class_t *type, haddr_t addr,
  ...
  assert(0 ==
         (flags & (unsigned)(~(H5C__READ_ONLY_FLAG | H5C__FLUSH_LAST_FLAG))));
#endif /* H5_HAVE_PARALLEL */
  ...
  /* If currently logging, generate a message */
  {
    ...
    if (f->shared->cache->log_info->logging)
      if (H5C_log_write_protect_entry_msg(f->shared->cache,
                                          (H5AC_info_t *)thing, type->id, flags,
                                          fake_ret_value) < 0)
...
herr_t H5AC_resize_entry(void *thing, size_t new_size) {
  ...
  if (cache_ptr != NULL && cache_ptr->log_info != NULL)
    if (cache_ptr->log_info->logging)
      if (H5C_log_write_resize_entry_msg(cache_ptr, entry_ptr, new_size,
                                         ret_value) < 0)
        HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
...
herr_t H5AC_destroy_flush_dependency(void *parent_thing, void *child_thing) {
  ...
  if (H5C_destroy_flush_dependency(parent_thing, child_thing) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTUNDEPEND, FAIL,
                "H5C_destroy_flush_dependency() failed");
...
herr_t H5AC_unprotect(H5F_t *f, const H5AC_class_t *type, haddr_t addr,
  ...
  if (NULL != (aux_ptr = (H5AC_aux_t *)H5C_get_aux_ptr(f->shared->cache))) {
    ...
    if (deleted && aux_ptr->mpi_rank == 0) {
      if (H5AC__log_deleted_entry((H5AC_info_t *)thing) < 0) {
         ...
         */
        HDONE_ERROR(H5E_CACHE, H5E_CANTUNPROTECT, FAIL,
                    "H5AC__log_deleted_entry() failed");
      }
    }
  }    /* end if */
#endif /* H5_HAVE_PARALLEL */
  ...
  /* If currently logging, generate a message */
  if (f->shared->cache->log_info->logging)
    if (H5C_log_write_unprotect_entry_msg(f->shared->cache, addr, type->id,
                                          flags, ret_value) < 0)
      HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
...
herr_t H5AC_get_cache_auto_resize_config(const H5AC_t *cache_ptr,
  ...
  if (H5C_get_cache_auto_resize_config((const H5C_t *)cache_ptr,
                                       &internal_config) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "H5C_get_cache_auto_resize_config() failed");
  if (H5C_get_evictions_enabled((const H5C_t *)cache_ptr, &evictions_enabled) <
      0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC_get_cache_size(const H5AC_t *cache_ptr, size_t *max_size_ptr,
  ...
  if (H5C_get_cache_size((const H5C_t *)cache_ptr, max_size_ptr,
                         min_clean_size_ptr, cur_size_ptr,
...
herr_t H5AC_get_cache_flush_in_progress(H5AC_t *cache_ptr,
  ...
  if (H5C_get_cache_flush_in_progress((H5C_t *)cache_ptr,
                                      flush_in_progress_ptr) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "H5C_get_cache_flush_in_progress() failed");
...
H5AC_set_cache_auto_resize_config(H5AC_t *cache_ptr,
  ...
  if (config_ptr->open_trace_file) {
     ...
     */
    if (H5C_log_set_up((H5C_t *)cache_ptr, config_ptr->trace_file_name,
                       H5C_LOG_STYLE_TRACE, TRUE) < 0)
      HGOTO_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "mdc logging setup failed");
  }
  ...
  if (H5AC__ext_config_2_int_config(config_ptr, &internal_config) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_SYSTEM, FAIL,
                "H5AC__ext_config_2_int_config() failed");
  ...
  /* If currently logging, generate a message */
  if (cache_ptr->log_info->logging)
    if (H5C_log_write_set_cache_config_msg(cache_ptr, config_ptr, ret_value) <
        0)
      HDONE_ERROR(H5E_CACHE, H5E_LOGGING, FAIL, "unable to emit log message");
...
herr_t H5AC_validate_config(const H5AC_cache_config_t *config_ptr) {
  ...
  if (H5C_validate_resize_config(&internal_config,
                                 H5C_RESIZE_CFG__VALIDATE_ALL) < 0)
...
static herr_t H5AC__check_if_write_permitted(const H5F_t
  ...
  aux_ptr = (H5AC_aux_t *)H5C_get_aux_ptr(f->shared->cache);
  if (aux_ptr != NULL) {
    if ((aux_ptr->mpi_rank == 0) || (aux_ptr->metadata_write_strategy ==
                                     H5AC_METADATA_WRITE_STRATEGY__DISTRIBUTED))
 ...
 *------------------------------------------------------------------------------
 */
herr_t H5AC_evict_tagged_metadata(H5F_t *f, haddr_t metadata_tag,
 ...
 *------------------------------------------------------------------------------
 */
herr_t H5AC_expunge_tag_type_metadata(H5F_t *f, haddr_t tag, int type_id,
                                      unsigned flags) {
  /* Variable Declarations */
  herr_t ret_value = SUCCEED;
  ...
  if (H5C_expunge_tag_type_metadata(f, tag, type_id, flags) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTFLUSH, FAIL,
                "Cannot expunge tagged type entries");
...
herr_t H5AC_get_tag(const void *thing, haddr_t *tag) {
  ...
  if (H5C_get_tag(thing, tag) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
                "Cannot get tag for metadata cache entry");
...
herr_t H5AC_cork(H5F_t *f, haddr_t obj_addr, unsigned action, hbool_t *corked) {
  ...
  assert(action == H5AC__SET_CORK || action == H5AC__UNCORK ||
         action == H5AC__GET_CORKED);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5AC_get_mdc_image_info(const H5AC_t *cache_ptr, haddr_t *image_addr,
                               hsize_t *image_len) {
  herr_t ret_value = SUCCEED; /* Return value */

  FUNC_ENTER_NOAPI(FAIL)

  if (H5C_get_mdc_image_info((const H5C_t *)cache_ptr, image_addr, image_len) <
      0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTGET, FAIL,
                "can't retrieve cache image info");
