...
#define H5MF_FSPACE_SHRINK                                                     \
  80 /* Percent of "normal" size to shrink serialized free space size */
#define H5MF_FSPACE_EXPAND                                                     \
  120 /* Percent of "normal" size to expand serialized free space size */
...
/* For non-paged aggregation: map allocation request type to tracked free-space
 * type */
...
typedef enum {
  ...
  H5MF_AGGR_MERGE_DICHOTOMY, /* Metadata in one free list and raw data in
                                another */
  H5MF_AGGR_MERGE_TOGETHER   /* Metadata & raw data in one free list */
} H5MF_aggr_merge_t;

/* User data for section info iterator callback for iterating over free space
 * sections */
typedef struct {
...
/* Allocator routines */
static haddr_t H5MF__alloc_pagefs(H5F_t *f, H5FD_mem_t alloc_type,
                                  hsize_t size);
...
/* General routines */
static herr_t H5MF__get_free_sects(H5F_t *f, H5FS_t *fspace,
                                   H5MF_sect_iter_ud_t *sect_udata,
                                   size_t *nums);
static hbool_t H5MF__fsm_type_is_self_referential(H5F_shared_t *f_sh,
                                                  H5F_mem_page_t fsm_type);
static hbool_t H5MF__fsm_is_self_referential(H5F_shared_t *f_sh,
                                             H5FS_t *fspace);
static herr_t
H5MF__continue_alloc_fsm(H5F_shared_t *f_sh, H5FS_t *sm_hdr_fspace,
                         H5FS_t *sm_sinfo_fspace, H5FS_t *lg_hdr_fspace,
                         H5FS_t *lg_sinfo_fspace, hbool_t *continue_alloc_fsm);
...
herr_t H5MF_init_merge_flags(H5F_shared_t *f_sh) {
  ...
  case H5MF_AGGR_MERGE_DICHOTOMY:
    ...
    memset(f_sh->fs_aggr_merge, H5F_FS_MERGE_METADATA,
 ...
 *-------------------------------------------------------------------------
 */
void H5MF__alloc_to_fs_type(H5F_shared_t *f_sh, H5FD_mem_t alloc_type,
  ...
  if (H5F_SHARED_PAGED_AGGR(f_sh)) { /* paged aggregation */
    if (size >= f_sh->fs_page_size) {
      if (H5F_SHARED_HAS_FEATURE(
              f_sh, H5FD_FEAT_PAGED_AGGR)) { /* multi or split driver */
        /* For non-contiguous address space, map to large size free-space
         * manager for each alloc_type
         */
        if (H5FD_MEM_DEFAULT == f_sh->fs_type_map[alloc_type])
          *fs_type = (H5F_mem_page_t)(alloc_type + (H5FD_MEM_NTYPES - 1));
        else
          *fs_type = (H5F_mem_page_t)(f_sh->fs_type_map[alloc_type] +
                                      (H5FD_MEM_NTYPES - 1));
      } /* end if */
      else
        /* For contiguous address space, map to generic large size free-space
         * manager */
...
herr_t H5MF__open_fstype(H5F_t *f, H5F_mem_page_t type) {
  ...
  if (H5F_PAGED_AGGR(f)) {
    alignment = (type == H5F_MEM_PAGE_GENERIC) ? f->shared->fs_page_size
                                               : (hsize_t)H5F_ALIGN_DEF;
  ...
  if (NULL == (f->shared->fs_man[type] =
                   H5FS_open(f, f->shared->fs_addr[type], NELMTS(classes),
                             classes, f, alignment, threshold)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, FAIL,
                "can't initialize free space info");
...
static herr_t H5MF__create_fstype(H5F_t *f, H5F_mem_page_t type) {
  ...
  if (NULL == (f->shared->fs_man[type] =
                   H5FS_create(f, NULL, &fs_create, NELMTS(classes), classes, f,
                               alignment, threshold)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, FAIL,
                "can't initialize free space info");
...
static herr_t H5MF__delete_fstype(H5F_t *f, H5F_mem_page_t type) {
  ...
  if (H5FS_delete(f, tmp_fs_addr) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTFREE, FAIL,
                "can't delete free space manager");

  /* Shift [back] to closed state */
  assert(f->shared->fs_state[type] == H5F_FS_STATE_DELETING);
  f->shared->fs_state[type] = H5F_FS_STATE_CLOSED;

  /* Sanity check that the free space manager for this type wasn't started up
   * again */
  assert(!H5_addr_defined(f->shared->fs_addr[type]));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5MF__add_sect(H5F_t *f, H5FD_mem_t alloc_type, H5FS_t *fspace,
...
#endif /* H5MF_ALLOC_DEBUG_MORE */
  /* Add the section */
  if (H5FS_sect_add(f, fspace, (H5FS_section_info_t *)node,
                    H5FS_ADD_RETURNED_SPACE, &udata) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINSERT, FAIL,
 ...
 *
 * Purpose:	To find a section from the specified free-space manager to
 *fulfill the request. If found, re-add the left-over space back to the manager.
 *
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5MF__find_sect(H5F_t *f, H5FD_mem_t alloc_type, hsize_t size,
                       H5FS_t *fspace, haddr_t *addr) {
  H5AC_ring_t orig_ring = H5AC_RING_INV; /* Original ring value */
  H5AC_ring_t fsm_ring = H5AC_RING_INV;  /* Ring of FSM */
  ...
  if ((ret_value =
           H5FS_sect_find(f, fspace, size, (H5FS_section_info_t **)&node)) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                "error locating free space in file");
...
#ifdef H5MF_ALLOC_DEBUG_MORE
      fprintf(stderr,
              "%s: re-adding node, node->sect_info.size = %" PRIuHSIZE "\n",
...
haddr_t H5MF_alloc(H5F_t *f, H5FD_mem_t alloc_type, hsize_t size) {
  ...
  if (H5F_HAVE_FREE_SPACE_MANAGER(f)) {
    ...
    if (!f->shared->fs_man[fs_type] &&
        H5_addr_defined(f->shared->fs_addr[fs_type])) {
      /* Open the free-space manager */
      if (H5MF__open_fstype(f, fs_type) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTOPENOBJ, HADDR_UNDEF,
                    "can't initialize file free space");
    ...
    if (f->shared->fs_man[fs_type])
      if (H5MF__find_sect(f, alloc_type, size, f->shared->fs_man[fs_type],
                          &ret_value) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, HADDR_UNDEF,
                    "error locating a node");
  } /* end if */
...
#ifdef H5MF_ALLOC_DEBUG
  fprintf(stderr,
          "%s: Leaving: ret_value = %" PRIuHADDR ", size = %" PRIuHSIZE "\n",
 ...
 *
 * Purpose:     Allocate space from either the large or small free-space
 *manager. For "large" request: Allocate request from VFD Determine mis-aligned
 *fragment and return the fragment to the appropriate manager For "small"
 *request: Allocate a page from the large manager Determine whether space is
 *available from a mis-aligned fragment being returned to the manager Return
 *left-over space to the manager after fulfilling request
 *
 * Return:      Success:        The file address of new chunk.
 *              Failure:        HADDR_UNDEF
 *
...
static haddr_t H5MF__alloc_pagefs(H5F_t *f, H5FD_mem_t alloc_type,
  ...
  case H5F_MEM_PAGE_LARGE_OHDR: {
    ...
    /* Allocate from VFD */
    if (HADDR_UNDEF ==
        (ret_value = H5F__alloc(f, alloc_type, size + frag_size, NULL, NULL)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, HADDR_UNDEF,
    ...
    if (frag_size) {
      ...
      if (NULL == (node = H5MF__sect_new(H5MF_FSPACE_SECT_LARGE,
                                         ret_value + size, frag_size)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, HADDR_UNDEF,
  ...
  case H5F_MEM_PAGE_OHDR: {
    ...
    if (NULL ==
        (node = H5MF__sect_new(H5MF_FSPACE_SECT_SMALL, (new_page + size),
    ...
    if (f->shared->page_buf != NULL &&
        H5PB_add_new_page(f->shared, alloc_type, new_page) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINSERT, HADDR_UNDEF,
  ...
  if (node)
    if (H5MF__sect_free((H5FS_section_info_t *)node) < 0)
      HDONE_ERROR(H5E_RESOURCE, H5E_CANTRELEASE, HADDR_UNDEF,
                  "can't free section node");
...
herr_t H5MF_xfree(H5F_t *f, H5FD_mem_t alloc_type, haddr_t addr, hsize_t size) {
...
#ifdef H5MF_ALLOC_DEBUG
  fprintf(stderr,
          "%s: Entering - alloc_type = %u, addr = %" PRIuHADDR
          ", size = %" PRIuHSIZE "\n",
  ...
  if (H5_addr_le(f->shared->tmp_addr, addr))
    HGOTO_ERROR(H5E_RESOURCE, H5E_BADRANGE, FAIL,
                "attempting to free temporary file space");

  /* If it's metadata, check if the space to free intersects with the file's
   * metadata accumulator
   */
  if (H5FD_MEM_DRAW != alloc_type) {
    /* Check if the space to free intersects with the file's metadata
     * accumulator */
    ...
    if (!H5_addr_defined(f->shared->fs_addr[fs_type])) {
      ...
      if ((status = H5MF_try_shrink(f, alloc_type, addr, size)) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTMERGE, FAIL,
                    "can't check for absorbing block");
      else if (status > 0)
        /* Indicate success */
        HGOTO_DONE(SUCCEED);
  ...
  /* If size of the freed section is larger than threshold, add it to the free
   * space manager */
  if (size >= f->shared->fs_threshold) {
    ...
    if (H5MF__add_sect(f, alloc_type, f->shared->fs_man[fs_type], node) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINSERT, FAIL,
                  "can't add section to file free space");
    node = NULL;
    ...
    if ((merged = H5FS_sect_try_merge(f, f->shared->fs_man[fs_type],
                                      (H5FS_section_info_t *)node,
                                      H5FS_ADD_RETURNED_SPACE, &udata)) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINSERT, FAIL,
                  "can't merge section to file free space");
  ...
  if (node)
    if (H5MF__sect_free((H5FS_section_info_t *)node) < 0)
      HDONE_ERROR(H5E_RESOURCE, H5E_CANTRELEASE, FAIL,
                  "can't free simple section node");
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5MF_try_extend(H5F_t *f, H5FD_mem_t alloc_type, haddr_t addr,
  ...
  fprintf(stderr,
          "%s: Entering: alloc_type = %u, addr = %" PRIuHADDR
          ", size = %" PRIuHSIZE ", extra_requested = %" PRIuHSIZE "\n",
   ...
   *   To extend a large block at EOA: calculate in advance mis-aligned fragment
   * so EOA will still end at page boundary
   */
  if (H5F_PAGED_AGGR(f)) {
    if (size < f->shared->fs_page_size) {
      ...
      /*   To extend a large block: calculate in advance the mis-aligned
       * fragment so EOA will end at page boundary if extended */
      if (HADDR_UNDEF == (eoa = H5F_get_eoa(f, alloc_type)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL, "Unable to get eoa");
      assert(!(eoa % f->shared->fs_page_size));

      H5MF_EOA_MISALIGN(f, (eoa + extra_requested), f->shared->fs_page_size,
                        frag_size);
    } /* end else */
  }   /* end if */
  ...
  if (allow_extend) {
    ...
    if ((ret_value = H5F__try_extend(f, map_type, end,
                                     extra_requested + frag_size)) < 0)
    ...
    /*   for paged aggregation, put the fragment into the large-sized free-space
     * manager */
    ...
    if (ret_value == FALSE &&
      ...
      aggr = (map_type == H5FD_MEM_DRAW) ? &(f->shared->sdata_aggr)
                                         : &(f->shared->meta_aggr);
      if ((ret_value = H5MF__aggr_try_extend(f, aggr, map_type, end,
                                             extra_requested)) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTEXTEND, FAIL,
                    "error extending aggregation block");
    ...
    if (ret_value == FALSE &&
      ...
      if (f->shared->fs_man[fs_type]) {
        if ((ret_value = H5FS_sect_try_extend(
                 f, f->shared->fs_man[fs_type], addr, size, extra_requested,
                 H5FS_ADD_RETURNED_SPACE, &udata)) < 0)
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTEXTEND, FAIL,
      ...
      /* For paged aggregation and a metadata block: try to extend into page end
       * threshold */
      if (ret_value == FALSE && H5F_PAGED_AGGR(f) &&
          map_type != H5FD_MEM_DRAW) {
        H5MF_EOA_MISALIGN(f, end, f->shared->fs_page_size, frag_size);

        if (frag_size <= H5F_PGEND_META_THRES(f) &&
            extra_requested <= frag_size)
          ret_value = TRUE;
#ifdef H5MF_ALLOC_DEBUG_MORE
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5MF_try_shrink(H5F_t *f, H5FD_mem_t alloc_type, haddr_t addr,
                       hsize_t size) {
  H5MF_free_section_t *node = NULL;      /* Free space section pointer */
  H5MF_sect_ud_t udata;                  /* User data for callback */
  ...
  udata.f = f;
  udata.alloc_type = alloc_type;
  udata.allow_sect_absorb =
      FALSE; /* Force section to be absorbed into aggregator */
  udata.allow_eoa_shrink_only = FALSE;

  /* Check if the block can shrink the container */
  if (sect_cls->can_shrink) {
    if ((ret_value = (*sect_cls->can_shrink)((const H5FS_section_info_t *)node,
                                             &udata)) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTMERGE, FAIL,
                  "can't check if section can shrink container");
    if (ret_value > 0) {
      assert(sect_cls->shrink);

      if ((*sect_cls->shrink)((H5FS_section_info_t **)&node, &udata) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTSHRINK, FAIL,
...
static herr_t H5MF__close_delete_fstype(H5F_t *f, H5F_mem_page_t type) {
...
#ifdef H5MF_ALLOC_DEBUG_MORE
  fprintf(stderr,
          "%s: Check 1.0 - f->shared->fs_man[%u] = %p, f->shared->fs_addr[%u] "
          "= %" PRIuHADDR "\n",
          __func__, (unsigned)type, (void *)f->shared->fs_man[type],
          (unsigned)type, f->shared->fs_addr[type]);
#endif /* H5MF_ALLOC_DEBUG_MORE */
...
#ifdef H5MF_ALLOC_DEBUG_MORE
  fprintf(stderr,
          "%s: Check 2.0 - f->shared->fs_man[%u] = %p, f->shared->fs_addr[%u] "
          "= %" PRIuHADDR "\n",
          __func__, (unsigned)type, (void *)f->shared->fs_man[type],
          (unsigned)type, f->shared->fs_addr[type]);
#endif /* H5MF_ALLOC_DEBUG_MORE */
  ...
  if (H5_addr_defined(f->shared->fs_addr[type]))
    if (H5MF__delete_fstype(f, type) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTRELEASE, FAIL,
                  "can't delete the free space manager");
...
herr_t H5MF_try_close(H5F_t *f) {
  ...
  H5AC_ring_t needed_ring =
      H5AC_RING_INV;          /* Ring value needed for this iteration */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *
 * Purpose:     Close the free space tracker(s) for a file: non-paged
 *aggregation
 *
 * Return:      SUCCEED/FAIL
 *
    ...
    if (H5F_PAGED_AGGR(f)) {
      ...
      for (ptype = H5F_MEM_PAGE_META; ptype < H5F_MEM_PAGE_NTYPES; ptype++) {
        if (f->shared->fs_man[ptype]) {
          ...
          udata.alloc_type =
              (H5FD_mem_t)((H5FD_mem_t)ptype < H5FD_MEM_NTYPES
                               ? ptype
                               : ((ptype % H5FD_MEM_NTYPES) + 1));

          if ((status = H5FS_sect_try_shrink_eoa(f, f->shared->fs_man[ptype],
                                                 &udata)) < 0)
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTSHRINK, FAIL,
                        "can't check for shrinking eoa");
          else if (status > 0)
            eoa_shrank = TRUE;
        } /* end if */
      }   /* end for */
      ...
      for (type = H5FD_MEM_DEFAULT; type < H5FD_MEM_NTYPES; type++) {
        if (f->shared->fs_man[type]) {
          /* Test to see if we need to switch rings -- do so if required */
          if (H5MF__fsm_type_is_self_referential(f->shared,
                                                 (H5F_mem_page_t)type))
            needed_ring = H5AC_RING_MDFSM;
          else
            needed_ring = H5AC_RING_RDFSM;
          ...
          if ((status = H5FS_sect_try_shrink_eoa(f, f->shared->fs_man[type],
                                                 &udata)) < 0)
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTSHRINK, FAIL,
                        "can't check for shrinking eoa");
          else if (status > 0)
...
herr_t H5MF_get_freespace(H5F_t *f, hsize_t *tot_space, hsize_t *meta_size) {
  ...
  htri_t fs_started[H5F_MEM_PAGE_NTYPES]; /* Indicate whether the free-space
  ...
  if (!H5F_PAGED_AGGR(f)) {
    ...
    if (H5MF__aggr_query(f, &(f->shared->meta_aggr), &ma_addr, &ma_size) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                  "can't query metadata aggregator stats");

    /* Retrieve 'small data' aggregator info, if available */
    if (H5MF__aggr_query(f, &(f->shared->sdata_aggr), &sda_addr, &sda_size) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                  "can't query small data aggregator stats");
  } /* end if */

  /* Iterate over all the free space types that have managers and get each free
   * list's space */
  for (type = start_type; type < end_type; type++) {
    ...
    if (f->shared->fs_man[type]) {
      ...
      hsize_t type_meta_size =
          0; /* Amount of free space metadata for each type */

      /* Retrieve free space size from free space manager */
      if (H5FS_sect_stats(f->shared->fs_man[type], &type_fs_size, NULL) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                    "can't query free space stats");
      if (H5FS_size(f->shared->fs_man[type], &type_meta_size) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                    "can't query free space metadata stats");
  ...
  for (type = start_type; type < end_type; type++) {
    ...
    if (fs_started[type])
      if (H5MF__close_fstype(f, type) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, FAIL,
                    "can't close file free space");
  } /* end for */

  /* Set the value(s) to return */
  /* (The metadata & small data aggregators count as free space now, since they
   * aren't at EOA) */
  if (tot_space)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5MF_get_free_sections(H5F_t *f, H5FD_mem_t type, size_t nsects,
                              H5F_sect_info_t *sect_info, size_t *sect_count) {
  H5AC_ring_t orig_ring = H5AC_RING_INV; /* Original ring value */
  H5AC_ring_t curr_ring = H5AC_RING_INV; /* Current ring value */
  H5AC_ring_t needed_ring =
      H5AC_RING_INV;              /* Ring value needed for this iteration.  */
  ...
  if (type == H5FD_MEM_DEFAULT) {
    ...
    if (H5F_PAGED_AGGR(
            f)) /* set to the corresponding LARGE free-space manager */
  ...
  for (ty = start_type; ty < end_type; ty++) {
    ...
    if (!f->shared->fs_man[ty] && H5_addr_defined(f->shared->fs_addr[ty])) {
      if (H5MF__open_fstype(f, ty) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTRELEASE, FAIL,
                    "can't open the free space manager");
    ...
    if (f->shared->fs_man[ty])
      if (H5MF__get_free_sects(f, f->shared->fs_man[ty], &sect_udata, &nums) <
          0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTRELEASE, FAIL,
    ...
    if (fs_started)
      if (H5MF__close_fstype(f, ty) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTCLOSEOBJ, FAIL,
                    "can't close file free space");
 ...
 *
 * Purpose:	Retrieve section information for the specified free-space
 *manager.
 *
 * Return:      Success:        non-negative
 *              Failure:        negative
...
static herr_t H5MF__get_free_sects(H5F_t *f, H5FS_t *fspace,
  ...
  if (sect_udata->sects && *nums > 0)
    /* Iterate over all the free space sections of this type, adding them to the
     * user's section info */
    if (H5FS_sect_iterate(f, fspace, H5MF__sects_cb, sect_udata) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_BADITER, FAIL,
                  "can't iterate over sections");
        ...
        if (!fsm_visited[fsm_type]) {
          ...
          if (f->shared->fs_man[fsm_type]) {
            ...
            if (H5_addr_defined(fs_stat.addr) ||
                H5_addr_defined(fs_stat.sect_addr)) {
          ...
          if (f->shared->fs_man[fsm_type]) {
            ...
            if (!H5MF__fsm_type_is_self_referential(f->shared, fsm_type)) {
              ...
              if (fs_stat.serial_sect_count > 0) {
                ...
                if (H5FS_stat_info(f, f->shared->fs_man[fsm_type], &fs_stat) <
                    0)
                  HGOTO_ERROR(H5E_RESOURCE, H5E_CANTRELEASE, FAIL,
                              "can't get free-space info");
 ...
 *		   to be oversized.  That is, only allow section info to
 *increase in size, not shrink.  The solution is now implemented.
 *
...
herr_t H5MF_settle_meta_data_fsm(H5F_t *f, hbool_t *fsm_settled) {
  ...
  H5F_mem_page_t lg_fssinfo_fs_type =
      H5F_MEM_PAGE_DEFAULT;       /* large fs sinfo fsm */
  ...
  hbool_t continue_alloc_fsm =
      FALSE; /* Continue allocating addr and sect_addr for FSMs */
  H5AC_ring_t orig_ring = H5AC_RING_INV; /* Original ring value */
  herr_t ret_value = SUCCEED;            /* Return value */
  ...
  if (f->shared->fs_persist && !H5F_NULL_FSM_ADDR(f)) {
    ...
    H5MF__alloc_to_fs_type(f->shared, H5FD_MEM_FSPACE_HDR, (size_t)1,
                           &sm_fshdr_fs_type);
    H5MF__alloc_to_fs_type(f->shared, H5FD_MEM_FSPACE_SINFO, (size_t)1,
                           &sm_fssinfo_fs_type);
    ...
    if (H5F_PAGED_AGGR(f)) {
      H5MF__alloc_to_fs_type(f->shared, H5FD_MEM_FSPACE_HDR,
                             f->shared->fs_page_size + 1, &lg_fshdr_fs_type);
      H5MF__alloc_to_fs_type(f->shared, H5FD_MEM_FSPACE_SINFO,
    ...
    do {
      ...
      if (sm_sinfo_fspace && (sm_sinfo_fspace != sm_hdr_fspace))
        if (H5FS_vfd_alloc_hdr_and_section_info_if_needed(
                f, sm_sinfo_fspace, &(f->shared->fs_addr[sm_fssinfo_fs_type])) <
            0)
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
      ...
      if (H5F_PAGED_AGGR(f)) {
        if (lg_hdr_fspace)
          if (H5FS_vfd_alloc_hdr_and_section_info_if_needed(
                  f, lg_hdr_fspace, &(f->shared->fs_addr[lg_fshdr_fs_type])) <
              0)
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
      ...
      if (H5MF__continue_alloc_fsm(f->shared, sm_hdr_fspace, sm_sinfo_fspace,
                                   lg_hdr_fspace, lg_sinfo_fspace,
                                   &continue_alloc_fsm) < 0)
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "can't vfd allocate lg sinfo FSM file space");
     ...
     */
    if (HADDR_UNDEF ==
        (eoa_fsm_fsalloc = H5FD_get_eoa(f->shared->lf, H5FD_MEM_DEFAULT)))
      HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL, "unable to get file size");
...
H5MF__continue_alloc_fsm(H5F_shared_t *f_sh, H5FS_t *sm_hdr_fspace,
  ...
  /* Check sm_hdr_fspace */
  if (sm_hdr_fspace && sm_hdr_fspace->serial_sect_count > 0 &&
      sm_hdr_fspace->sinfo)
  ...
  if (H5F_SHARED_PAGED_AGGR(f_sh) && !(*continue_alloc_fsm)) {
    /* Check lg_hdr_fspace */
    if (lg_hdr_fspace && lg_hdr_fspace->serial_sect_count > 0 &&
        lg_hdr_fspace->sinfo)
...
static hbool_t H5MF__fsm_type_is_self_referential(H5F_shared_t *f_sh,
  ...
  H5MF__alloc_to_fs_type(f_sh, H5FD_MEM_FSPACE_SINFO, (size_t)1,
                         &sm_fssinfo_fsm);

  if (H5F_SHARED_PAGED_AGGR(f_sh)) {
    H5MF__alloc_to_fs_type(f_sh, H5FD_MEM_FSPACE_HDR, f_sh->fs_page_size + 1,
                           &lg_fshdr_fsm);
    H5MF__alloc_to_fs_type(f_sh, H5FD_MEM_FSPACE_SINFO, f_sh->fs_page_size + 1,
                           &lg_fssinfo_fsm);
