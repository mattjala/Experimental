...
static herr_t H5D__btree_shared_create(const H5F_t *f,
                                       H5O_storage_chunk_t *store,
                                       const H5O_layout_chunk_t *layout);

/* B-tree iterator callbacks */
static int H5D__btree_idx_iterate_cb(H5F_t *f, const void *left_key,
                                     haddr_t addr, const void *right_key,
                                     void *_udata);
...
static htri_t H5D__btree_found(H5F_t *f, haddr_t addr, const void *_lt_key,
                               hbool_t *found, void *_udata);
static H5B_ins_t H5D__btree_insert(H5F_t *f, haddr_t addr, void *_lt_key,
                                   hbool_t *lt_key_changed, void *_md_key,
                                   void *_udata, void *_rt_key,
                                   hbool_t *rt_key_changed,
                                   haddr_t *new_node /*out*/);
static H5B_ins_t H5D__btree_remove(H5F_t *f, haddr_t addr, void *_lt_key,
                                   hbool_t *lt_key_changed, void *_udata,
                                   void *_rt_key, hbool_t *rt_key_changed);
static herr_t H5D__btree_decode_key(const H5B_shared_t *shared,
...
static herr_t H5D__btree_idx_size(const H5D_chk_idx_info_t *idx_info,
                                  hsize_t *size);
static herr_t H5D__btree_idx_reset(H5O_storage_chunk_t *storage,
                                   hbool_t reset_addr);
 ...
 *-------------------------------------------------------------------------
 */
static H5UC_t *H5D__btree_get_shared(const H5F_t H5_ATTR_UNUSED *f,
                                     const void *_udata) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__btree_new_node(H5F_t H5_ATTR_NDEBUG_UNUSED *f, H5B_ins_t op,
  ...
  assert(rt_key);
  assert(udata);
  assert(udata->common.layout->ndims > 0 &&
         udata->common.layout->ndims < H5O_LAYOUT_NDIMS);
  assert(addr_p);
  ...
  *addr_p = udata->chunk_block.offset;
   ...
   */
  H5_CHECKED_ASSIGN(lt_key->nbytes, uint32_t, udata->chunk_block.length,
                    hsize_t);
...
static int H5D__btree_cmp2(void *_lt_key, void *_udata, void *_rt_key) {
  ...
  ret_value =
      H5VM_vector_cmp_u(udata->layout->ndims, lt_key->scaled, rt_key->scaled);
...
static int H5D__btree_cmp3(void *_lt_key, void *_udata, void *_rt_key) {
  ...
  if (udata->layout->ndims == 2) {
    ...
    if (H5VM_vector_ge_u(udata->layout->ndims, udata->scaled, rt_key->scaled))
      ret_value = 1;
    else if (H5VM_vector_lt_u(udata->layout->ndims, udata->scaled,
                              lt_key->scaled))
      ret_value = (-1);
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5D__btree_found(H5F_t H5_ATTR_UNUSED *f, haddr_t addr,
                               const void *_lt_key, hbool_t *found,
                               void *_udata) {
  H5D_chunk_ud_t *udata = (H5D_chunk_ud_t *)_udata;
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t H5D__chunk_disjoint(unsigned n, const hsize_t *scaled1,
                                   const hsize_t *scaled2) {
...
static H5B_ins_t H5D__btree_insert(H5F_t H5_ATTR_NDEBUG_UNUSED *f,
  ...
  } else if (H5VM_vector_eq_u(udata->common.layout->ndims, udata->common.scaled,
                              lt_key->scaled) &&
             lt_key->nbytes > 0) {
    /*
  ...
  } else if (H5D__chunk_disjoint(udata->common.layout->ndims, lt_key->scaled,
                                 udata->common.scaled)) {
    assert(H5D__chunk_disjoint(udata->common.layout->ndims, rt_key->scaled,
                               udata->common.scaled));
    /*
     * Split this node, inserting the new new node to the right of the
     * current node.  The MD_KEY is where the split occurs.
     */
    H5_CHECKED_ASSIGN(md_key->nbytes, uint32_t, udata->chunk_block.length,
                      hsize_t);
 ...
 *-------------------------------------------------------------------------
 */
static H5B_ins_t H5D__btree_remove(H5F_t *f, haddr_t addr,
                                   void *_lt_key /*in,out */,
  ...
  if (H5MF_xfree(f, H5FD_MEM_DRAW, addr, (hsize_t)lt_key->nbytes) < 0)
    HGOTO_ERROR(H5E_STORAGE, H5E_CANTFREE, H5B_INS_ERROR,
...
static herr_t H5D__btree_debug_key(FILE *stream, int indent, int fwidth,
  ...
  fprintf(stream, "%*s%-*s %u bytes\n", indent, "", fwidth,
          "Chunk size:", (unsigned)key->nbytes);
  fprintf(stream, "%*s%-*s 0x%08x\n", indent, "", fwidth,
          "Filter mask:", key->filter_mask);
  fprintf(stream, "%*s%-*s {", indent, "", fwidth, "Logical offset:");
  for (u = 0; u < udata->ndims; u++)
    fprintf(stream, "%s%" PRIuHSIZE, u ? ", " : "",
            (key->scaled[u] * udata->common.layout->dim[u]));
  HDfputs("}\n", stream);
...
static herr_t H5D__btree_shared_create(const H5F_t *f,
  ...
  if (NULL == (my_layout = H5FL_MALLOC(H5O_layout_chunk_t)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "can't allocate chunk layout");
  H5MM_memcpy(my_layout, layout, sizeof(H5O_layout_chunk_t));
  shared->udata = my_layout;

  /* Make shared B-tree info reference counted */
  if (NULL ==
      (store->u.btree.shared = H5UC_create(shared, H5D__btree_shared_free)))
    HGOTO_ERROR(H5E_DATASET, H5E_NOSPACE, FAIL,
...
static herr_t H5D__btree_idx_init(const H5D_chk_idx_info_t *idx_info,
  ...
  if (H5D__btree_shared_create(idx_info->f, idx_info->storage,
                               idx_info->layout) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, FAIL,
                "can't create wrapper for shared B-tree info");
...
static herr_t H5D__btree_idx_create(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (H5B_create(idx_info->f, H5B_BTREE, &udata,
...
static herr_t H5D__btree_idx_insert(const H5D_chk_idx_info_t *idx_info,
   ...
   * its size changed.
   */
  if (H5B_insert(idx_info->f, H5B_BTREE, idx_info->storage->idx_addr, udata) <
      0)
...
static herr_t H5D__btree_idx_get_addr(const H5D_chk_idx_info_t *idx_info,
  ...
  found = FALSE;
  if (H5B_find(idx_info->f, H5B_BTREE, idx_info->storage->idx_addr, &found,
               udata) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTFIND, FAIL,
                "can't check for chunk in B-tree");
 ...
 *-------------------------------------------------------------------------
 */
static int H5D__btree_idx_iterate_cb(H5F_t H5_ATTR_UNUSED *f,
                                     const void *_lt_key, haddr_t addr,
                                     const void H5_ATTR_UNUSED *_rt_key,
                                     void *_udata) {
  H5D_btree_it_ud_t *udata = (H5D_btree_it_ud_t *)_udata; /* User data */
  const H5D_btree_key_t *lt_key =
      (const H5D_btree_key_t *)_lt_key; /* B-tree key for chunk */
  ...
  HDcompile_assert(offsetof(H5D_chunk_rec_t, nbytes) ==
                   offsetof(H5D_btree_key_t, nbytes));
  HDcompile_assert(sizeof(chunk_rec.nbytes) == sizeof(lt_key->nbytes));
  HDcompile_assert(offsetof(H5D_chunk_rec_t, scaled) ==
                   offsetof(H5D_btree_key_t, scaled));
  HDcompile_assert(sizeof(chunk_rec.scaled) == sizeof(lt_key->scaled));
  HDcompile_assert(offsetof(H5D_chunk_rec_t, filter_mask) ==
                   offsetof(H5D_btree_key_t, filter_mask));
  HDcompile_assert(sizeof(chunk_rec.filter_mask) ==
                   sizeof(lt_key->filter_mask));
...
static int H5D__btree_idx_iterate(const H5D_chk_idx_info_t *idx_info,
  ...
  if ((ret_value =
           H5B_iterate(idx_info->f, H5B_BTREE, idx_info->storage->idx_addr,
...
static herr_t H5D__btree_idx_remove(const H5D_chk_idx_info_t *idx_info,
   ...
   */
  if (H5B_remove(idx_info->f, H5B_BTREE, idx_info->storage->idx_addr, udata) <
      0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTDELETE, FAIL,
                "unable to remove chunk entry");
...
static herr_t H5D__btree_idx_delete(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (H5_addr_defined(idx_info->storage->idx_addr)) {
    ...
    if (H5D__btree_shared_create(idx_info->f, &tmp_storage, idx_info->layout) <
        0)
    ...
    if (H5UC_DEC(tmp_storage.u.btree.shared) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                  "unable to decrement ref-counted page");
  } /* end if */
...
H5D__btree_idx_copy_setup(const H5D_chk_idx_info_t *idx_info_src,
  ...
  if (H5D__btree_shared_create(idx_info_src->f, idx_info_src->storage,
                               idx_info_src->layout) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, FAIL,
                "can't create wrapper for source shared B-tree info");
  if (H5D__btree_shared_create(idx_info_dst->f, idx_info_dst->storage,
                               idx_info_dst->layout) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, FAIL,
                "can't create wrapper for destination shared B-tree info");

  /* Create the root of the B-tree that describes chunked storage in the dest.
   * file */
...
static herr_t H5D__btree_idx_size(const H5D_chk_idx_info_t *idx_info,
  ...
  if (H5B_get_info(idx_info->f, H5B_BTREE, idx_info->storage->idx_addr,
                   &bt_info, NULL, &udata) < 0)
...
static herr_t H5D__btree_idx_dest(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (H5UC_DEC(idx_info->storage->u.btree.shared) < 0)
    HGOTO_ERROR(H5E_IO, H5E_CANTFREE, FAIL,
                "unable to decrement ref-counted page");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D_btree_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
  ...
  if (shared_init) {
    ...
    if (NULL == storage.u.btree.shared)
      ...
      HDONE_ERROR(H5E_IO, H5E_CANTFREE, FAIL,
                  "unable to decrement ref-counted shared info");
  } /* end if */
