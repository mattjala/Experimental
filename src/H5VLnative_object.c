 ...
 *-------------------------------------------------------------------------
 */
void *H5VL__native_object_open(void *obj, const H5VL_loc_params_t *loc_params,
  ...
  case H5VL_OBJECT_BY_NAME: {
    /* Open the object */
    if (NULL == (ret_value = H5O_open_name(
                     &loc, loc_params->loc_data.loc_by_name.name, opened_type)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, NULL,
                  "unable to open object by name");
    break;
  }

  case H5VL_OBJECT_BY_IDX: {
    /* Open the object */
    if (NULL == (ret_value = H5O__open_by_idx(
                     &loc, loc_params->loc_data.loc_by_idx.name,
                     loc_params->loc_data.loc_by_idx.idx_type,
                     loc_params->loc_data.loc_by_idx.order,
                     loc_params->loc_data.loc_by_idx.n, opened_type)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, NULL,
                  "unable to open object by index");
    break;
  }
  ...
  case H5VL_OBJECT_BY_TOKEN: {
    ...
    if (NULL == (ret_value = H5O__open_by_addr(&loc, addr, opened_type)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, NULL,
                  "unable to open object by address");
    break;
  }
...
herr_t H5VL__native_object_copy(void *src_obj,
                                ...
                                hid_t lcpl_id, hid_t H5_ATTR_UNUSED dxpl_id,
                                void H5_ATTR_UNUSED **req) {
  ...
  /* Copy the object */
  if ((ret_value = H5O__copy(&src_loc, src_name, &dst_loc, dst_name, ocpypl_id,
                             lcpl_id)) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5VL__native_object_get(void *obj, const H5VL_loc_params_t *loc_params,
  ...
  case H5VL_OBJECT_GET_NAME: {
    if (loc_params->type == H5VL_OBJECT_BY_SELF) {
      ...
      if (H5G_get_name(&loc, args->args.get_name.buf,
                       args->args.get_name.buf_size,
      ...
      /* Decode token */
      if (H5VL_native_token_to_addr(obj_oloc.file, H5I_FILE, token,
                                    &obj_oloc.addr) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTUNSERIALIZE, FAIL,
                    "can't deserialize object token into address");

      /* Retrieve object's name */
      if (H5G_get_name_by_addr(
              loc.oloc->file, &obj_oloc, args->args.get_name.buf,
  ...
  case H5VL_OBJECT_GET_TYPE: {
    if (loc_params->type == H5VL_OBJECT_BY_TOKEN) {
      ...
      if (H5O_get_rc_and_type(&obj_oloc, &rc, args->args.get_type.obj_type) <
              0 ||
          0 == rc)
        HGOTO_ERROR(H5E_REFERENCE, H5E_LINKCOUNT, FAIL,
                    "dereferencing deleted object");
    } else
      HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, FAIL,
                  "unknown get_type parameters");
  ...
  case H5VL_OBJECT_GET_INFO: {
    if (loc_params->type == H5VL_OBJECT_BY_SELF) {
      if (H5G_loc_info(&loc, ".", args->args.get_info.oinfo,
                       args->args.get_info.fields) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL, "object not found");
    } /* end if */
    else if (loc_params->type == H5VL_OBJECT_BY_NAME) {
      if (H5G_loc_info(&loc, loc_params->loc_data.loc_by_name.name,
                       args->args.get_info.oinfo,
                       args->args.get_info.fields) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL, "object not found");
    } /* end else-if */
      ...
      if (H5O_get_info(obj_loc.oloc, args->args.get_info.oinfo,
                       args->args.get_info.fields) < 0) {
  ...
  default:
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "can't get this type of information from object");
  } /* end switch */
...
herr_t H5VL__native_object_specific(void *obj,
  ...
  case H5VL_OBJECT_EXISTS: {
    if (loc_params->type == H5VL_OBJECT_BY_NAME) {
      ...
      if (H5G_loc_exists(&loc, loc_params->loc_data.loc_by_name.name,
                         args->args.exists.exists) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                    "unable to determine if '%s' exists",
                    loc_params->loc_data.loc_by_name.name);
    } /* end if */
    else
      HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, FAIL,
                  "unknown object exists parameters");
  ...
  case H5VL_OBJECT_LOOKUP: {
    if (loc_params->type == H5VL_OBJECT_BY_NAME) {
      ...
      /* Find the object */
      if (H5G_loc_find(&loc, loc_params->loc_data.loc_by_name.name, &obj_loc) <
          0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL, "object not found");

      /* Encode token */
      if (H5VL_native_addr_to_token(loc.oloc->file, H5I_FILE,
                                    obj_loc.oloc->addr,
  ...
  case H5VL_OBJECT_VISIT: {
    ...
    if (loc_params->type == H5VL_OBJECT_BY_SELF) {
      if ((ret_value = H5O__visit(&loc, ".", visit_args->idx_type,
                                  visit_args->order, visit_args->op,
                                  visit_args->op_data, visit_args->fields)) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_BADITER, FAIL, "object visitation failed");
    } /* end if */
    else if (loc_params->type == H5VL_OBJECT_BY_NAME) {
      if ((ret_value = H5O__visit(&loc, loc_params->loc_data.loc_by_name.name,
                                  visit_args->idx_type, visit_args->order,
                                  visit_args->op, visit_args->op_data,
                                  visit_args->fields)) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_BADITER, FAIL, "object visitation failed");
    } /* end else-if */
    else
      HGOTO_ERROR(H5E_VOL, H5E_UNSUPPORTED, FAIL,
                  "unknown object visit params");
  ...
  default:
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "can't recognize this operation type");
  } /* end switch */
...
herr_t H5VL__native_object_optional(void *obj,
  ...
  case H5VL_NATIVE_OBJECT_GET_COMMENT: {
    ...
    if (loc_params->type == H5VL_OBJECT_BY_SELF) { /* H5Oget_comment */
      if (H5G_loc_get_comment(&loc, ".", gc_args->buf, gc_args->buf_size,
                              gc_args->comment_len) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                    "can't get comment for object");
    } /* end if */
    else if (loc_params->type ==
             H5VL_OBJECT_BY_NAME) { /* H5Oget_comment_by_name */
      if (H5G_loc_get_comment(&loc, loc_params->loc_data.loc_by_name.name,
                              gc_args->buf, gc_args->buf_size,
                              gc_args->comment_len) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                    "can't get comment for object");
    } /* end else-if */
    ...
    if (loc_params->type == H5VL_OBJECT_BY_SELF) { /* H5Oset_comment */
      if (H5G_loc_set_comment(&loc, ".", opt_args->set_comment.comment) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL, "object not found");
             ...
             H5VL_OBJECT_BY_NAME) { /* H5Oset_comment_by_name */
  ...
  case H5VL_NATIVE_OBJECT_DISABLE_MDC_FLUSHES: {
    if (H5O__disable_mdc_flushes(loc.oloc) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTCORK, FAIL,
                  "unable to cork the metadata cache");
  ...
  case H5VL_NATIVE_OBJECT_ENABLE_MDC_FLUSHES: {
    if (H5O__enable_mdc_flushes(loc.oloc) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTUNCORK, FAIL,
                  "unable to uncork the metadata cache");
  ...
  case H5VL_NATIVE_OBJECT_ARE_MDC_FLUSHES_DISABLED: {
    if (H5O__are_mdc_flushes_disabled(
            loc.oloc, opt_args->are_mdc_flushes_disabled.flag) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                  "unable to determine metadata cache cork status");
    ...
    if (loc_params->type == H5VL_OBJECT_BY_SELF) {
      if (H5G_loc_native_info(&loc, ".", gni_args->ninfo, gni_args->fields) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL, "object not found");
             ...
             H5VL_OBJECT_BY_NAME) { /* H5Oget_info_by_name */
      if (H5G_loc_native_info(&loc, loc_params->loc_data.loc_by_name.name,
                              gni_args->ninfo, gni_args->fields) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL, "object not found");
    }                                                  /* end else-if */
      ...
      if (H5O_get_native_info(obj_loc.oloc, gni_args->ninfo, gni_args->fields) <
          0) {
        H5G_loc_free(&obj_loc);
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL, "can't retrieve object info");
      } /* end if */
  ...
  default:
    HGOTO_ERROR(H5E_VOL, H5E_CANTGET, FAIL,
                "can't perform this operation on object");
  } /* end switch */
