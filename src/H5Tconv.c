 ...
 *        the destination is narrower than the source.
 *
 * xF:          Integers to float-point(float or double) values where the
 * destination is at least as wide as the source.  This case cannot generate
 *              overflows.
 *
 * Fx:          Float-point(float or double) values to integer where the source
 * is at least as wide as the destination.  Overflow can occur when the source
 * magnitude is too large for the destination.
 *
...
/* Added a condition branch(else if (*(S) == (DT)(D_MAX))) which seems
 * redundant. It handles a special situation when the source is "float" and
 * assigned the value of "INT_MAX".  A compiler may do roundup making this value
 * "INT_MAX+1".  However, when do comparison "if (*(S) > (DT)(D_MAX))", the
 * compiler may consider them equal. In this case, do not return exception but
 * make sure the maximum is assigned to the destination.   SLU - 2005/06/29
 */
#define H5T_CONV_Xx_CORE(STYPE, DTYPE, S, D, ST, DT, D_MIN, D_MAX)             \
  {                                                                            \
    if (*(S) > (ST)(D_MAX)) {                                                  \
      H5T_conv_ret_t except_ret =                                              \
          (cb_struct.func)(H5T_CONV_EXCEPT_RANGE_HI, src_id, dst_id, S, D,     \
                           cb_struct.user_data);                               \
      ...
      H5T_conv_ret_t except_ret =                                              \
          (cb_struct.func)(H5T_CONV_EXCEPT_RANGE_LOW, src_id, dst_id, S, D,    \
                           cb_struct.user_data);                               \
      if (except_ret == H5T_CONV_UNHANDLED)                                    \
        /* Let compiler convert if case is ignored by user handler*/           \
        *(D) = (DT)(D_MIN);                                                    \
...
#define H5T_CONV_uS_CORE_1(S, D, ST, DT, D_MIN, D_MAX)                         \
  if (*(S) > (DT)(D_MAX)) {                                                    \
    H5T_conv_ret_t except_ret = (cb_struct.func)(                              \
        H5T_CONV_EXCEPT_RANGE_HI, src_id, dst_id, S, D, cb_struct.user_data);  \
    if (except_ret == H5T_CONV_UNHANDLED)                                      \
      /* Let compiler convert if case is ignored by user handler */            \
      *(D) = (DT)(D_MAX);                                                      \
    else if (except_ret == H5T_CONV_ABORT)                                     \
...
#define H5T_CONV_uS_CORE(STYPE, DTYPE, S, D, ST, DT, D_MIN, D_MAX)             \
  {                                                                            \
    H5T_CONV_uS_CORE_I(H5T_CONV_uS_EVAL_TYPES(STYPE, DTYPE), S, D, ST, DT,     \
                       D_MIN, D_MAX)                                           \
  }
...
#define H5T_CONV_uS_NOEX_CORE(STYPE, DTYPE, S, D, ST, DT, D_MIN, D_MAX)        \
  {                                                                            \
    H5T_CONV_uS_NOEX_CORE_I(H5T_CONV_uS_EVAL_TYPES(STYPE, DTYPE), S, D, ST,    \
                            DT, D_MIN, D_MAX)                                  \
  }
  ...
  {                                                                            \
    if (sprec > dprec) {                                                       \
      ...
      if ((high_bit_pos - low_bit_pos) >= dprec) {                             \
        H5T_conv_ret_t except_ret =                                            \
            (cb_struct.func)(H5T_CONV_EXCEPT_PRECISION, src_id, dst_id, S, D,  \
                             cb_struct.user_data);                             \
        if (except_ret == H5T_CONV_UNHANDLED)                                  \
          /* Let compiler convert if case is ignored by user handler*/         \
          *(D) = (DT)(*(S));                                                   \
 ...
 * It handles a special situation when the source is "float" and assigned the
 * value of "INT_MAX".  Compilers do roundup making this value "INT_MAX+1". This
 * branch is to check that situation and return exception for some compilers,
 * mainly GCC. The branch if (*(S) > (DT)(D_MAX) || (sprec < dprec && *(S) ==
...
#define H5T_CONV_Fx_CORE(STYPE, DTYPE, S, D, ST, DT, D_MIN, D_MAX)             \
  {                                                                            \
    if (*(S) > (ST)(D_MAX) || (sprec < dprec && *(S) == (ST)(D_MAX))) {        \
      ...
      H5T_conv_ret_t except_ret =                                              \
          (cb_struct.func)(H5T_CONV_EXCEPT_TRUNCATE, src_id, dst_id, S, D,     \
                           cb_struct.user_data);                               \
      if (except_ret == H5T_CONV_UNHANDLED)                                    \
        /* Let compiler convert if case is ignored by user handler*/           \
        *(D) = (DT)(*(S));                                                     \
      ...
      case H5T_CONV_CONV:                                                      \
        ...
        while (nelmts > 0) {                                                   \
          ...
          if (d_stride > s_stride) {                                           \
            ...
            safe = nelmts -                                                    \
                   (((nelmts * (size_t)s_stride) + (size_t)(d_stride - 1)) /   \
                    (size_t)d_stride);                                         \
                                                                               \
            ...
            if (safe < 2) {                                                    \
              src =                                                            \
                  (ST *)(src_buf = (void *)((uint8_t *)buf +                   \
                                            (nelmts - 1) * (size_t)s_stride)); \
              dst =                                                            \
                  (DT *)(dst_buf = (void *)((uint8_t *)buf +                   \
          ...
          if (s_mv && d_mv) {                                                  \
            ...
            s = &src_aligned;                                                  \
            H5T_CONV_LOOP_OUTER(PRE_SALIGN, PRE_DALIGN, POST_SALIGN,           \
                                POST_DALIGN, GUTS, STYPE, DTYPE, s, d, ST, DT, \
                                D_MIN, D_MAX)                                  \
          } else if (s_mv) {                                                   \
            /* Alignment is required only for source */                        \
            s = &src_aligned;                                                  \
            H5T_CONV_LOOP_OUTER(PRE_SALIGN, PRE_DNOALIGN, POST_SALIGN,         \
                                POST_DNOALIGN, GUTS, STYPE, DTYPE, s, dst, ST, \
                                DT, D_MIN, D_MAX)                              \
          } else if (d_mv) {                                                   \
            /* Alignment is required only for destination */                   \
            H5T_CONV_LOOP_OUTER(PRE_SNOALIGN, PRE_DALIGN, POST_SNOALIGN,       \
                                POST_DALIGN, GUTS, STYPE, DTYPE, src, d, ST,   \
                                DT, D_MIN, D_MAX)                              \
          } else {                                                             \
            /* Alignment is not required for both source and destination */    \
            H5T_CONV_LOOP_OUTER(PRE_SNOALIGN, PRE_DNOALIGN, POST_SNOALIGN,     \
                                POST_DNOALIGN, GUTS, STYPE, DTYPE, src, dst,   \
                                ST, DT, D_MIN, D_MAX)                          \
          }                                                                    \
                                                                               \
...
/* Macro defining action on source data which needs to be aligned (before main
 * action) */
#define H5T_CONV_LOOP_PRE_SALIGN(ST)                                           \
  {                                                                            \
    /* The uint8_t * cast is required to avoid tripping over undefined         \
     * behavior.                                                               \
     *                                                                         \
     * The typed pointer arrives via a void pointer, which may have any        \
     * alignment. We then cast it to a pointer to a type that is assumed to be                  \
     * aligned, which is undefined behavior (section 6.3.2.3 paragraph 7 of                         \
     * the C99 standard). In the past this hasn't caused many problems, but in                          \
     * some cases (e.g. converting long doubles on macOS), an optimizing                              \
     * compiler might do the wrong thing (in the macOS case, the conversion                                \
     * uses SSE, which has stricter requirements about alignment).                                                \
     */                                                                        \
    H5MM_memcpy(&src_aligned, (const uint8_t *)src, sizeof(ST));               \
  }

/* Macro defining action on source data which doesn't need to be aligned (before
 * main action) */
#define H5T_CONV_LOOP_PRE_SNOALIGN(ST)                                         \
  {}

/* Macro defining action on destination data which needs to be aligned (before
 * main action) */
#define H5T_CONV_LOOP_PRE_DALIGN(DT)                                           \
  { d = &dst_aligned; }

/* Macro defining action on destination data which doesn't need to be aligned
 * (before main action) */
#define H5T_CONV_LOOP_PRE_DNOALIGN(DT)                                         \
  {}

/* Macro defining action on source data which needs to be aligned (after main
 * action) */
#define H5T_CONV_LOOP_POST_SALIGN(ST)                                          \
  {}

/* Macro defining action on source data which doesn't need to be aligned (after
 * main action) */
#define H5T_CONV_LOOP_POST_SNOALIGN(ST)                                        \
  {}

/* Macro defining action on destination data which needs to be aligned (after
 * main action) */
...
/* The outer wrapper for the type conversion loop, to check for an exception
 * handling routine */
#define H5T_CONV_LOOP_OUTER(PRE_SALIGN_GUTS, PRE_DALIGN_GUTS,                  \
                            POST_SALIGN_GUTS, POST_DALIGN_GUTS, GUTS, STYPE,   \
                            DTYPE, S, D, ST, DT, D_MIN, D_MAX)                 \
  if (cb_struct.func) {                                                        \
    H5T_CONV_LOOP(PRE_SALIGN_GUTS, PRE_DALIGN_GUTS, POST_SALIGN_GUTS,          \
                  POST_DALIGN_GUTS, GUTS, STYPE, DTYPE, S, D, ST, DT, D_MIN,   \
                  D_MAX)                                                       \
  } else {                                                                     \
    H5T_CONV_LOOP(PRE_SALIGN_GUTS, PRE_DALIGN_GUTS, POST_SALIGN_GUTS,          \
                  POST_DALIGN_GUTS, H5_GLUE(GUTS, _NOEX), STYPE, DTYPE, S, D,  \
                  ST, DT, D_MIN, D_MAX)                                        \
  }

/* The inner loop of the type conversion macro, actually converting the elements
 */
#define H5T_CONV_LOOP(PRE_SALIGN_GUTS, PRE_DALIGN_GUTS, POST_SALIGN_GUTS,      \
                      POST_DALIGN_GUTS, GUTS, STYPE, DTYPE, S, D, ST, DT,      \
                      D_MIN, D_MAX)                                            \
...
/* Macro to call the actual "guts" of the type conversion, or call the "no
 * exception" guts */
...
static herr_t H5T__reverse_order(uint8_t *rev, uint8_t *s, size_t size,
                                 H5T_order_t order);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_noop(hid_t H5_ATTR_UNUSED src_id, hid_t H5_ATTR_UNUSED dst_id,
                      H5T_cdata_t *cdata, size_t H5_ATTR_UNUSED nelmts,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_order_opt(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
  ...
  case H5T_CONV_INIT:
    ...
    if (src->shared->size != dst->shared->size ||
        0 != src->shared->u.atomic.offset || 0 != dst->shared->u.atomic.offset)
      HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL,
                  "conversion not supported");
    if ((src->shared->type == H5T_REFERENCE &&
         dst->shared->type != H5T_REFERENCE) ||
        (dst->shared->type == H5T_REFERENCE &&
         src->shared->type != H5T_REFERENCE))
      HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL,
                  "conversion not supported");
    if (src->shared->type != H5T_REFERENCE &&
      ...
      HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL,
                  "conversion not supported");
    if (src->shared->size != 1 && src->shared->size != 2 &&
        src->shared->size != 4 && src->shared->size != 8 &&
        src->shared->size != 16)
      HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL,
                  "conversion not supported");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_order(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                       size_t nelmts, size_t buf_stride,
                       size_t H5_ATTR_UNUSED bkg_stride, void *_buf,
                       void H5_ATTR_UNUSED *background) {
  uint8_t *buf = (uint8_t *)_buf;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_b_b(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                     size_t nelmts, size_t buf_stride,
                     size_t H5_ATTR_UNUSED bkg_stride, void *_buf,
                     void H5_ATTR_UNUSED *background) {
  uint8_t *buf = (uint8_t *)_buf;
    ...
    for (elmtno = 0; elmtno < nelmts; elmtno++) {
      ...
      if (src->shared->u.atomic.prec > dst->shared->u.atomic.prec) {
        ...
        if (cb_struct.func) { /*If user's exception handler is present, use it*/
          H5T__reverse_order(
              src_rev, s, src->shared->size,
              src->shared->u.atomic.order); /*reverse order first*/
          except_ret =
              (cb_struct.func)(H5T_CONV_EXCEPT_RANGE_HI, src_id, dst_id,
                               src_rev, d, cb_struct.user_data);
        } /* end if */

        if (except_ret == H5T_CONV_UNHANDLED) {
          H5T__bit_copy(d, dst->shared->u.atomic.offset, s,
                        src->shared->u.atomic.offset,
        ...
        H5T__bit_copy(d, dst->shared->u.atomic.offset, s,
                      src->shared->u.atomic.offset, src->shared->u.atomic.prec);
        H5T__bit_set(
            d, dst->shared->u.atomic.offset + src->shared->u.atomic.prec,
      ...
      case H5T_PAD_ZERO:
        H5T__bit_set(d, msb_pad_offset, 8 * dst->shared->size - msb_pad_offset,
                     FALSE);
        break;
      ...
      if (buf_stride) {
        sp += direction * (ssize_t)buf_stride; /* Note that cast is checked with
                                                  H5_CHECK_OVERFLOW, above */
        dp += direction * (ssize_t)buf_stride; /* Note that cast is checked with
                                                  H5_CHECK_OVERFLOW, above */
      }                                        /* end if */
      else {
        sp += direction *
              (ssize_t)src->shared->size; /* Note that cast is checked with
                                             H5_CHECK_OVERFLOW, above */
        dp += direction *
              (ssize_t)dst->shared->size; /* Note that cast is checked with
                                             H5_CHECK_OVERFLOW, above */
      }                                   /* end else */
    }                                     /* end for */
 ...
 *-------------------------------------------------------------------------
 */
static H5T_conv_struct_t *H5T__conv_struct_free(H5T_conv_struct_t *priv) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5T__conv_struct_init(H5T_t *src, H5T_t *dst,
                                    H5T_cdata_t *cdata) {
  ...
  if (!priv) {
    ...
    if (NULL == (priv = (H5T_conv_struct_t *)(cdata->priv = H5MM_calloc(sizeof(
                                                  H5T_conv_struct_t)))) ||
        NULL ==
            (priv->src2dst = (int *)H5MM_malloc(src_nmembs * sizeof(int))) ||
        NULL == (priv->src_memb_id =
                     (hid_t *)H5MM_malloc(src_nmembs * sizeof(hid_t))) ||
        NULL == (priv->dst_memb_id =
                     (hid_t *)H5MM_malloc(dst_nmembs * sizeof(hid_t))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
    src2dst = priv->src2dst;
    priv->src_nmembs = src_nmembs;

    /* The flag of special optimization to indicate if source members and
     * destination members are a subset of each other.  Initialize it to FALSE
     */
    ...
    for (i = 0; i < src_nmembs; i++) {
      ...
      if (src2dst[i] >= 0) {
        ...
        type =
            H5T_copy(dst->shared->u.compnd.memb[src2dst[i]].type, H5T_COPY_ALL);
        tid = H5I_register(H5I_DATATYPE, type, FALSE);
        assert(tid >= 0);
  ...
  src2dst = priv->src2dst;
  H5MM_xfree(priv->memb_path);
  if (NULL == (priv->memb_path = (H5T_path_t **)H5MM_malloc(
                   src->shared->u.compnd.nmembs * sizeof(H5T_path_t *))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");

  for (i = 0; i < src_nmembs; i++) {
    if (src2dst[i] >= 0) {
      H5T_path_t *tpath =
          H5T_path_find(src->shared->u.compnd.memb[i].type,
                        dst->shared->u.compnd.memb[src2dst[i]].type);

      if (NULL == (priv->memb_path[i] = tpath)) {
        cdata->priv = H5T__conv_struct_free(priv);
        HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL,
                    "unable to convert member datatype");
      } /* end if */
    }   /* end if */
  }     /* end for */
  ...
  if (src_nmembs < dst_nmembs) {
    ...
    for (i = 0; i < src_nmembs; i++) {
       ...
       */
      if (src2dst[i] != (int)i ||
          (src->shared->u.compnd.memb[i].offset !=
           dst->shared->u.compnd.memb[i].offset) ||
    ...
    if (priv->subset_info.subset == H5T_SUBSET_SRC)
      priv->subset_info.copy_size =
          src->shared->u.compnd.memb[src_nmembs - 1].offset +
    ...
    if (priv->subset_info.subset == H5T_SUBSET_DST)
      priv->subset_info.copy_size =
          dst->shared->u.compnd.memb[dst_nmembs - 1].offset +
          dst->shared->u.compnd.memb[dst_nmembs - 1].size;
  } else /* If the numbers of source and dest members are equal and no
          * conversion is needed, the case should have been handled as noop
          * earlier in H5Dio.c. */
  {
  }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_struct(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_struct_opt(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride, size_t bkg_stride,
                            void *_buf, void *_bkg) {
  uint8_t *buf = (uint8_t *)_buf; /*cast for pointer arithmetic    */
  uint8_t *bkg = (uint8_t *)_bkg; /*background pointer arithmetic    */
  ...
  case H5T_CONV_CONV:
     ...
     * packed tightly based on src->shared->size and dst->shared->size.  Also,
     * if BUF_STRIDE and BKG_STRIDE are both non-zero then place background data
     * into the BKG buffer at multiples of BKG_STRIDE; otherwise assume BKG
     * buffer is the packed destination datatype.
     */
    ...
    if (priv->subset_info.subset == H5T_SUBSET_SRC ||
        priv->subset_info.subset == H5T_SUBSET_DST) {
      /* If the optimization flag is set to indicate source members are a subset
       * and in the top of the destination, simply copy the source members to
       * background buffer.
       */
      xbuf = buf;
      xbkg = bkg;
      ...
      for (u = 0, offset = 0; u < src->shared->u.compnd.nmembs; u++) {
        ...
        if (dst_memb->size <= src_memb->size) {
          ...
          if (H5T_convert(priv->memb_path[u], priv->src_memb_id[u],
                          priv->dst_memb_id[src2dst[u]], nelmts, buf_stride,
                          bkg_stride, xbuf, xbkg) < 0)
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                        "unable to convert compound datatype member");
          for (elmtno = 0; elmtno < nelmts; elmtno++) {
      ...
      for (i = (int)src->shared->u.compnd.nmembs - 1; i >= 0; --i) {
        ...
        if (dst_memb->size > src_memb->size) {
          ...
          xbkg = bkg + dst_memb->offset;
          if (H5T_convert(priv->memb_path[i], priv->src_memb_id[i],
                          priv->dst_memb_id[src2dst[i]], nelmts, buf_stride,
                          bkg_stride, xbuf, xbkg) < 0)
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                        "unable to convert compound datatype member");
          for (elmtno = 0; elmtno < nelmts; elmtno++) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5T__conv_enum_init(H5T_t *src, H5T_t *dst, H5T_cdata_t *cdata) {
  ...
  cdata->need_bkg = H5T_BKG_NO;
  if (NULL ==
      (priv = (H5T_enum_struct_t *)(cdata->priv = H5MM_calloc(sizeof(*priv)))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  ...
  if (NULL == (priv->src2dst = (int *)H5MM_malloc(src->shared->u.enumer.nmembs *
                                                  sizeof(int))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  for (i = 0, j = 0;
    ...
    while (j < dst->shared->u.enumer.nmembs &&
           HDstrcmp(src->shared->u.enumer.name[i],
                    dst->shared->u.enumer.name[j]) != 0)
      j++;
   ...
   * The code such as "n =
   * *((int*)(src->shared->u.enumer.value+i*src->shared->size));" can change the
   * value significantly. i.g. if the source value is big-endian 0x0000000f,
   * executing the casting on little-endian machine will get a big number
   * 0x0f000000. Then it can't meet the condition
   * "if(src->shared->u.enumer.nmembs<2 ||
   * (double)length/src->shared->u.enumer.nmembs<1.2)" Because this is the
   * optimized code, we won't fix it. It should still work in some situations.
   * SLU - 2011/5/24)
   */
  if (1 == src->shared->size || sizeof(short) == src->shared->size ||
      sizeof(int) == src->shared->size) {
    for (i = 0; i < src->shared->u.enumer.nmembs; i++) {
      if (1 == src->shared->size)
        ...
        n = *((short *)((void *)((uint8_t *)src->shared->u.enumer.value +
                                 (i * src->shared->size))));
      else
        n = *((int *)((void *)((uint8_t *)src->shared->u.enumer.value +
                               (i * src->shared->size))));
      if (0 == i) {
        domain[0] = domain[1] = n;
      } else {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_enum(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                      size_t nelmts, size_t buf_stride,
                      size_t H5_ATTR_UNUSED bkg_stride, void *_buf,
  ...
  case H5T_CONV_CONV:
    ...
    /* priv->src2dst map was computed for certain sort keys. Make sure those
     * same sort keys are used here during conversion. See
     * H5T__conv_enum_init(). But we actually don't care about the source type's
     * order when doing the O(1) conversion algorithm, which is turned on by
     * non-zero priv->length */
    H5T__sort_name(dst, NULL);
    if (!priv->length)
      H5T__sort_value(src, NULL);
    ...
    for (i = 0; i < nelmts; i++, s += src_delta, d += dst_delta) {
      if (priv->length) {
        /* Use O(1) lookup */
        /* (The casting won't work when the byte orders are different. i.g. if
         * the source value is big-endian 0x0000000f, the direct casting "n =
         * *((int*)s);" will make it a big number 0x0f000000 on little-endian
         * machine. But we won't fix it because it's an optimization code.
         * Please also see the comment in the H5T__conv_enum_init() function.
         * SLU - 2011/5/24)
         */
        if (1 == src->shared->size)
          n = *((signed char *)s);
        ...
        if (n < 0 || (unsigned)n >= priv->length || priv->src2dst[n] < 0) {
          ...
          if (cb_struct.func)
            except_ret = (cb_struct.func)(H5T_CONV_EXCEPT_RANGE_HI, src_id,
                                          dst_id, s, d, cb_struct.user_data);
        ...
        while (lt < rt) {
          md = (lt + rt) / 2;
          cmp = memcmp(s,
                       (uint8_t *)src->shared->u.enumer.value +
                           (md * src->shared->size),
                       src->shared->size);
          if (cmp < 0)
            rt = md;
        ...
        if (lt >= rt) {
          ...
          if (cb_struct.func)
            except_ret = (cb_struct.func)(H5T_CONV_EXCEPT_RANGE_HI, src_id,
                                          dst_id, src, d, cb_struct.user_data);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_enum_numeric(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t H5_ATTR_UNUSED buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *_buf,
                              void H5_ATTR_UNUSED *bkg) {
  ...
  case H5T_CONV_INIT:
    ...
    if (H5T_ENUM != src->shared->type)
      HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL,
                  "source type is not a H5T_ENUM datatype");
    if (H5T_INTEGER != dst->shared->type && H5T_FLOAT != dst->shared->type)
      HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL,
                  "destination is not an integer type");
  ...
  case H5T_CONV_CONV:
    ...
    if (NULL == (tpath = H5T_path_find(src_parent, dst))) {
      ...
      if ((src_parent_id = H5I_register(
               H5I_DATATYPE, H5T_copy(src_parent, H5T_COPY_ALL), FALSE)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, FAIL,
                    "unable to register types for conversion");

      /* Convert the data */
      if (H5T_convert(tpath, src_parent_id, dst_id, nelmts, buf_stride,
                      bkg_stride, _buf, bkg) < 0)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_vlen(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                      size_t nelmts, size_t buf_stride, size_t bkg_stride,
                      void *buf, void *bkg) {
  ...
  case H5T_CONV_INIT:
    ...
    if (H5T_VLEN != dst->shared->type)
      HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL, "not a H5T_VLEN datatype");
    if (H5T_VLEN_STRING == src->shared->u.vlen.type &&
        H5T_VLEN_STRING == dst->shared->u.vlen.type) {
      if ((H5T_CSET_ASCII == src->shared->u.vlen.cset &&
           H5T_CSET_UTF8 == dst->shared->u.vlen.cset) ||
          (H5T_CSET_ASCII == dst->shared->u.vlen.cset &&
           H5T_CSET_UTF8 == src->shared->u.vlen.cset))
        HGOTO_ERROR(
  ...
  case H5T_CONV_CONV:
    ...
    if (NULL ==
        (tpath = H5T_path_find(src->shared->parent, dst->shared->parent)))
      HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL,
    ...
    if ((parent_is_vlen =
             H5T_detect_class(dst->shared->parent, H5T_VLEN, FALSE)) < 0)
    ...
    if (H5CX_get_vlen_alloc_info(&vl_alloc_info) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, FAIL,
                  "unable to retrieve VL allocation info");
    ...
    while (nelmts > 0) {
      ...
      if (d_stride > s_stride) {
        ...
        /* any source elements at the beginning of the buffer) */
        safe =
            nelmts - (((nelmts * (size_t)s_stride) + ((size_t)d_stride - 1)) /
                      (size_t)d_stride);
      ...
      for (elmtno = 0; elmtno < safe; elmtno++) {
        ...
        if ((*(src->shared->u.vlen.cls->isnull))(src->shared->u.vlen.file, s,
                                                 &is_nil) < 0)
          HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, FAIL,
                      "can't check if VL data is 'nil'");
        else if (is_nil) {
          /* Write "nil" sequence to destination location */
          if ((*(dst->shared->u.vlen.cls->setnull))(dst->shared->u.vlen.file, d,
                                                    b) < 0)
            HGOTO_ERROR(H5E_DATATYPE, H5E_WRITEERROR, FAIL,
                        "can't set VL data to 'nil'");
        } /* end else-if */
        else {
          size_t seq_len; /* The number of elements in the current sequence */

          /* Get length of element sequences */
          if ((*(src->shared->u.vlen.cls->getlen))(src->shared->u.vlen.file, s,
                                                   &seq_len) < 0)
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, FAIL, "bad sequence length");

          /* If we are reading from memory and there is no conversion, just get
           * the pointer to sequence */
          if (write_to_file && noop_conv) {
            ...
            size_t src_size,
                dst_size; /*source & destination total size in bytes*/
            ...
            if (!seq_len && !conv_buf) {
              ...
              /* Only allocate conversion buffer in H5T_VLEN_MIN_CONF_BUF_SIZE
               * increments */
              conv_buf_size =
                  ((MAX(src_size, dst_size) / H5T_VLEN_MIN_CONF_BUF_SIZE) + 1) *
                  H5T_VLEN_MIN_CONF_BUF_SIZE;
              if (NULL == (conv_buf = H5FL_BLK_REALLOC(vlen_seq, conv_buf,
                                                       conv_buf_size)))
                HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                            "memory allocation failed for type conversion");
            ...
            /* Read in VL sequence */
            if ((*(src->shared->u.vlen.cls->read))(src->shared->u.vlen.file, s,
                                                   conv_buf, src_size) < 0)
          ...
          if (!noop_conv) {
            ...
            if (nested) {
              ...
              if ((*(dst->shared->u.vlen.cls->getlen))(dst->shared->u.vlen.file,
                                                       b, &bg_seq_len) < 0)
                HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, FAIL,
                            "bad sequence length");
              ...
              if (bg_seq_len > 0) {
                if (tmp_buf_size <
                    (bg_seq_len * MAX(src_base_size, dst_base_size))) {
                  tmp_buf_size =
                      (bg_seq_len * MAX(src_base_size, dst_base_size));
                  if (NULL == (tmp_buf = H5FL_BLK_REALLOC(vlen_seq, tmp_buf,
                                                          tmp_buf_size)))
                    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                                "memory allocation failed for type conversion");
                  memset(tmp_buf, 0, tmp_buf_size);
                ...
                if ((*(dst->shared->u.vlen.cls->read))(
                        dst->shared->u.vlen.file, b, tmp_buf,
                        bg_seq_len * dst_base_size) < 0)
                  HGOTO_ERROR(H5E_DATATYPE, H5E_READERROR, FAIL,
                              "can't read VL data");
              } /* end if */

              /* If the sequence gets shorter, pad out the original sequence
               * with zeros */
            ...
            /* Convert VL sequence */
            if (H5T_convert(tpath, tsrc_id, tdst_id, seq_len, (size_t)0,
                            (size_t)0, conv_buf, tmp_buf) < 0)
              HGOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, FAIL,
                          "datatype conversion failed");
          } /* end if */

          /* Write sequence to destination location */
          if ((*(dst->shared->u.vlen.cls->write))(
                  dst->shared->u.vlen.file, &vl_alloc_info, d, conv_buf, b,
          ...
          if (!noop_conv) {
             ...
             * level if the
             * length of new data elements is shorter than the old data
             * elements.*/
            if (nested && seq_len < bg_seq_len) {
              ...
              for (u = seq_len; u < bg_seq_len; u++, tmp += dst_base_size) {
                ...
                if ((*(dst->shared->u.vlen.cls->del))(dst->shared->u.vlen.file,
                                                      tmp) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_array(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                       size_t nelmts, size_t buf_stride, size_t bkg_stride,
                       void *_buf, void H5_ATTR_UNUSED *_bkg) {
  ...
  case H5T_CONV_CONV:
    ...
    if (src->shared->size >= dst->shared->size || buf_stride > 0) {
      ...
      sp = (uint8_t *)_buf +
           (nelmts - 1) * (buf_stride ? buf_stride : src->shared->size);
      dp = (uint8_t *)_buf +
           (nelmts - 1) * (buf_stride ? buf_stride : dst->shared->size);
      direction = -1;
    }
    ...
    H5_CHECK_OVERFLOW(src->shared->size, size_t, ssize_t);
    H5_CHECK_OVERFLOW(dst->shared->size, size_t, ssize_t);
    src_delta = (ssize_t)direction *
                (ssize_t)(buf_stride ? buf_stride : src->shared->size);
    dst_delta = (ssize_t)direction *
                (ssize_t)(buf_stride ? buf_stride : dst->shared->size);
    ...
    if (NULL ==
      ...
      if ((tsrc_id = H5I_register(H5I_DATATYPE,
                                  H5T_copy(src->shared->parent, H5T_COPY_ALL),
                                  FALSE)) < 0 ||
          (tdst_id = H5I_register(H5I_DATATYPE,
                                  H5T_copy(dst->shared->parent, H5T_COPY_ALL),
                                  FALSE)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, FAIL,
                    "unable to register types for conversion");
    }
    ...
    if (tpath->cdata.need_bkg) {
      ...
      bkg_buf_size = src->shared->u.array.nelem *
                     MAX(src->shared->size, dst->shared->size);
    ...
    for (elmtno = 0; elmtno < nelmts; elmtno++) {
      ...
      /* Convert array */
      if (H5T_convert(tpath, tsrc_id, tdst_id, src->shared->u.array.nelem,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ref(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                     size_t nelmts, size_t buf_stride, size_t bkg_stride,
                     void *buf, void *bkg) {
  H5T_t *src = NULL;          /* source datatype                      */
  H5T_t *dst = NULL;          /* destination datatype                 */
  ...
  case H5T_CONV_INIT:
    ...
    if (H5T_REFERENCE != dst->shared->type)
      HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL,
                  "not a H5T_REFERENCE datatype");
    /* Only allow for source reference that is not an opaque type, destination
     * must be opaque */
    if (!dst->shared->u.atomic.u.r.opaque)
      HGOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL,
                  "not an H5T_STD_REF datatype");
  ...
  case H5T_CONV_CONV: {
    ...
    while (nelmts > 0) {
      ...
      for (elmtno = 0; elmtno < safe; elmtno++) {
        ...
        if ((*(src->shared->u.atomic.u.r.cls->isnull))(
                src->shared->u.atomic.u.r.file, s, &is_nil) < 0)
          HGOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, FAIL,
                      "can't check if reference data is 'nil'");

        if (is_nil) {
          /* Write "nil" reference to destination location */
          if ((*(dst->shared->u.atomic.u.r.cls->setnull))(
                  dst->shared->u.atomic.u.r.file, d, b) < 0)
            HGOTO_ERROR(H5E_DATATYPE, H5E_WRITEERROR, FAIL,
          ...
          if (dst_copy && (dst->shared->u.atomic.u.r.loc == H5T_LOC_DISK))
            ...
            if (dst->shared->u.atomic.u.r.cls->write(
                    ...
                    dst->shared->u.atomic.u.r.file, d, dst->shared->size,
                    b) < 0)
              HGOTO_ERROR(H5E_DATATYPE, H5E_WRITEERROR, FAIL,
                          "can't write reference data");
          } /* end else */
        }   /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_i_i(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                     size_t nelmts, size_t buf_stride,
  ...
  case H5T_CONV_CONV:
    ...
    for (elmtno = 0; elmtno < nelmts; elmtno++) {
       ...
       */
      sfirst = H5T__bit_find(s, src->shared->u.atomic.offset,
                             src->shared->u.atomic.prec, H5T_BIT_MSB, TRUE);
      first = (size_t)sfirst;
      ...
      if (sfirst < 0) {
         ...
         */
        H5T__bit_set(d, dst->shared->u.atomic.offset,
                     dst->shared->u.atomic.prec, FALSE);
        ...
        if (first + 1 == src->shared->u.atomic.prec) {
          ...
          if (cb_struct
            ...
            H5T__reverse_order(
                ...
                src->shared->u.atomic.order); /*reverse order first*/
            except_ret =
                (cb_struct.func)(H5T_CONV_EXCEPT_RANGE_LOW, src_id, dst_id,
                                 src_rev, d, cb_struct.user_data);
          }
          ...
          H5T__bit_set(
              d, dst->shared->u.atomic.offset + src->shared->u.atomic.prec - 1,
        ...
        if (first + 1 >= dst->shared->u.atomic.prec) {
          ...
          if (except_ret == H5T_CONV_UNHANDLED) {
            H5T__bit_set(d, dst->shared->u.atomic.offset,
                         dst->shared->u.atomic.prec - 1, TRUE);
            H5T__bit_set(
                d,
                (dst->shared->u.atomic.offset + dst->shared->u.atomic.prec - 1),
                (size_t)1, FALSE);
          } else if (except_ret == H5T_CONV_ABORT)
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCONVERT, FAIL,
                        "can't handle conversion exception");
         ...
         */
        ssize_t sfz =
            H5T__bit_find(s, src->shared->u.atomic.offset,
        ...
        if (sfz >= 0 && fz + 1 >= dst->shared->u.atomic.prec) {
          ...
          if (except_ret == H5T_CONV_UNHANDLED) {
            H5T__bit_set(d, dst->shared->u.atomic.offset,
                         dst->shared->u.atomic.prec - 1, FALSE);
            H5T__bit_set(
                d,
                (dst->shared->u.atomic.offset + dst->shared->u.atomic.prec - 1),
                (size_t)1, TRUE);
      ...
      if (dst->shared->u.atomic.offset + dst->shared->u.atomic.prec !=
          8 * dst->shared->size) {
        assert(H5T_PAD_ZERO == dst->shared->u.atomic.msb_pad ||
               H5T_PAD_ONE == dst->shared->u.atomic.msb_pad);
        H5T__bit_set(d,
                     dst->shared->u.atomic.offset + dst->shared->u.atomic.prec,
                     8 * dst->shared->size - (dst->shared->u.atomic.offset +
                                              dst->shared->u.atomic.prec),
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_f_f(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                     size_t nelmts, size_t buf_stride,
                     size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                     void H5_ATTR_UNUSED *bkg) {
  ...
  hbool_t denormalized =
      FALSE; /*is either source or destination denormalized?*/
  H5T_conv_cb_t cb_struct = {NULL, NULL}; /*conversion callback structure */
  H5T_conv_ret_t except_ret;              /*return of callback function   */
  hbool_t reverse;            /*if reverse the order of destination        */
  herr_t ret_value = SUCCEED; /*return value                 */
  ...
  case H5T_CONV_CONV:
    ...
    if (src_p->shared->size == dst_p->shared->size || buf_stride) {
      ...
      double olap_d =
          HDceil((double)(dst_p->shared->size) /
      ...
      double olap_d =
          HDceil((double)(src_p->shared->size) /
    ...
    src_delta = (ssize_t)direction *
                (ssize_t)(buf_stride ? buf_stride : src_p->shared->size);
    dst_delta = (ssize_t)direction *
                (ssize_t)(buf_stride ? buf_stride : dst_p->shared->size);
    ...
    for (elmtno = 0; elmtno < nelmts; elmtno++) {
       ...
       */
      if (H5T__bit_find(s, src.u.f.mpos, src.u.f.msize, H5T_BIT_LSB, TRUE) <
          0) {
        if (H5T__bit_find(s, src.u.f.epos, src.u.f.esize, H5T_BIT_LSB, TRUE) <
            0) {
          /* +0 or -0 */
          ...
          goto padding;
        } else if (H5T__bit_find(s, src.u.f.epos, src.u.f.esize, H5T_BIT_LSB,
                                 FALSE) < 0) {
          /* +Inf or -Inf */
          if (cb_struct
                  .func) { /*If user's exception handler is present, use it*/
            ...
            if (sign)
              except_ret =
                  (cb_struct.func)(H5T_CONV_EXCEPT_NINF, src_id, dst_id,
                                   src_rev, d, cb_struct.user_data);
            else
              except_ret =
                  (cb_struct.func)(H5T_CONV_EXCEPT_PINF, src_id, dst_id,
                                   src_rev, d, cb_struct.user_data);
          }
          ...
          if (except_ret == H5T_CONV_UNHANDLED) {
            ...
            if (H5T_NORM_NONE == dst.u.f.norm)
              H5T__bit_set(d, dst.u.f.mpos + dst.u.f.msize - 1, (size_t)1,
                           TRUE);
          } else if (except_ret == H5T_CONV_HANDLED) {
            /*No need to reverse the order of destination because user handles
             * it*/
            reverse = FALSE;
            goto next;
          } else if (except_ret == H5T_CONV_ABORT)
          ...
          goto padding;
                 ...
                 H5T__bit_find(s, src.u.f.mpos, src.u.f.msize - 1, H5T_BIT_LSB,
                               TRUE) < 0 &&
                 H5T__bit_find(s, src.u.f.epos, src.u.f.esize, H5T_BIT_LSB,
                               FALSE) < 0) {
        /*This is a special case for the source of no implied mantissa bit.
         *If the exponent bits are all 1s and only the 1st bit of mantissa
         *is set to 1.  It's infinity. The Intel-Linux "long double" is this
         *case.*/
        /* +Inf or -Inf */
        if (cb_struct.func) { /*If user's exception handler is present, use it*/
          /*reverse order first*/
          H5T__reverse_order(src_rev, s, src_p->shared->size,
                             src_p->shared->u.atomic.order);
          if (sign)
            except_ret = (cb_struct.func)(H5T_CONV_EXCEPT_NINF, src_id, dst_id,
                                          src_rev, d, cb_struct.user_data);
          else
            except_ret = (cb_struct.func)(H5T_CONV_EXCEPT_PINF, src_id, dst_id,
                                          src_rev, d, cb_struct.user_data);
        }
        ...
        if (cb_struct.func) { /*If user's exception handler is present, use it*/
          ...
          H5T__reverse_order(src_rev, s, src_p->shared->size,
                             src_p->shared->u.atomic.order);
          except_ret = (cb_struct.func)(H5T_CONV_EXCEPT_NAN, src_id, dst_id,
                                        src_rev, d, cb_struct.user_data);
        }
      ...
      if (0 == expo || H5T_NORM_NONE == src.u.f.norm) {
        if ((bitno = H5T__bit_find(s, src.u.f.mpos, src.u.f.msize, H5T_BIT_MSB,
                                   TRUE)) > 0) {
          msize = (size_t)bitno;
        } else if (0 == bitno) {
          msize = 1;
      ...
      if (0 == expo || H5T_NORM_NONE == src.u.f.norm) {
        assert(bitno >= 0);
        expo -=
            (int64_t)((src.u.f.ebias - 1) + (src.u.f.msize - (size_t)bitno));
      } else if (H5T_NORM_IMPLIED == src.u.f.norm) {
      ...
      if (msize > 0 && mrsh <= dst.u.f.msize && mrsh + msize > dst.u.f.msize) {
        ...
        if (H5T__bit_get_d(s, (mpos + (size_t)bitno) - 1, (size_t)1) &&
            !denormalized) {
          /* Don't do rounding if exponent is 111...110 and mantissa is
           * 111...11. To do rounding and increment exponent in this case will
           * create an infinity value.*/
          if ((H5T__bit_find(s, mpos + (size_t)bitno, msize - (size_t)bitno,
                             H5T_BIT_LSB, FALSE) >= 0 ||
               expo < expo_max - 1)) {
            carry = H5T__bit_inc(s, mpos + (size_t)bitno - 1,
                                 1 + msize - (size_t)bitno);
            if (carry)
              implied = 2;
          }
        } else if (H5T__bit_get_d(s, (mpos + (size_t)bitno) - 1, (size_t)1) &&
                   denormalized)
          /* For either source or destination, denormalized value doesn't
           * increment carry.*/
      ...
      if (mrsh > dst.u.f.msize + 1) {
        ...
        H5T__bit_set(d, dst.u.f.mpos, dst.u.f.msize, FALSE);
        H5T__bit_set_d(d, dst.u.f.mpos, MIN(2, dst.u.f.msize),
                       (hsize_t)implied);
      } else {
        if (mrsh > 0) {
        ...
        if (mrsh + msize >= dst.u.f.msize) {
          H5T__bit_copy(d, dst.u.f.mpos, s,
                        (mpos + msize + mrsh - dst.u.f.msize),
                        dst.u.f.msize - mrsh);
        } else {
          H5T__bit_copy(d, dst.u.f.mpos + dst.u.f.msize - (mrsh + msize), s,
                        mpos, msize);
    ...
    padding:
      ...
      if (dst.offset > 0) {
        ...
        H5T__bit_set(d, (size_t)0, dst.offset,
                     (hbool_t)(H5T_PAD_ONE == dst.lsb_pad));
      }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_s_s(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                     size_t nelmts, size_t buf_stride,
                     size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                     void H5_ATTR_UNUSED *bkg) {
  H5T_t *src = NULL;            /*source datatype        */
  ...
  case H5T_CONV_INIT:
    ...
    if (src->shared->u.atomic.u.s.pad < 0 ||
        src->shared->u.atomic.u.s.pad >= H5T_NSTR ||
  ...
  case H5T_CONV_CONV:
    ...
    for (elmtno = 0; elmtno < nelmts; elmtno++) {
      ...
      case H5T_STR_NULLTERM:
        for (nchars = 0; nchars < dst->shared->size &&
                         nchars < src->shared->size && s[nchars];
             nchars++) {
          d[nchars] = s[nchars];
        }
        break;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                          size_t nelmts, size_t buf_stride,
                          size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                          void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                          size_t nelmts, size_t buf_stride,
                          size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                          void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                          size_t nelmts, size_t buf_stride,
                          size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                          void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                          size_t nelmts, size_t buf_stride,
                          size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                          void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                size_t nelmts, size_t buf_stride,
                                size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                                void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                size_t nelmts, size_t buf_stride,
                                size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                                void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_schar_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uchar_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_short_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ushort_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                size_t nelmts, size_t buf_stride,
                                size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                                void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_int_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_uint_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_long_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ulong_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_llong_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg) {
...
herr_t H5T__conv_llong_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_float(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ullong_double(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg) {
...
herr_t H5T__conv_ullong_ldouble(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                size_t nelmts, size_t buf_stride,
                                size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                                void H5_ATTR_UNUSED *bkg) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_schar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_uchar(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_short(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_ushort(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                size_t nelmts, size_t buf_stride,
                                size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                                void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                           size_t nelmts, size_t buf_stride,
                           size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                           void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_int(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_uint(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                            size_t nelmts, size_t buf_stride,
                            size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                            void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_long(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_ldouble_ulong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                             size_t nelmts, size_t buf_stride,
                             size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                             void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_float_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                              size_t nelmts, size_t buf_stride,
                              size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                              void H5_ATTR_UNUSED *bkg){
    H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_double_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg) {
  H5_GCC_CLANG_DIAG_OFF("float-equal")
...
herr_t H5T__conv_ldouble_llong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                               size_t nelmts, size_t buf_stride,
                               size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                               void H5_ATTR_UNUSED *bkg) {
  H5_GCC_CLANG_DIAG_OFF("float-equal")
...
herr_t H5T__conv_ldouble_ullong(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                                size_t nelmts, size_t buf_stride,
                                size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                                void H5_ATTR_UNUSED *bkg) {
  H5_GCC_CLANG_DIAG_OFF("float-equal")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_f_i(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                     size_t nelmts, size_t buf_stride,
                     size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                     void H5_ATTR_UNUSED *bkg) {
  /* Traversal-related variables */
  ...
  case H5T_CONV_CONV:
    ...
    for (elmtno = 0; elmtno < nelmts; elmtno++) {
       ...
       */
      H5T__bit_shift(int_buf, expo - (ssize_t)src.u.f.msize, (size_t)0,
                     buf_size * 8);
       ...
       */
      sfirst =
          H5T__bit_find(int_buf, (size_t)0, 8 * buf_size, H5T_BIT_MSB, TRUE);
      first = (size_t)sfirst;
      ...
      if (sfirst < 0) {
          ...
          if (first >= dst.prec) {
            ...
            if (truncated &&
              ...
              H5T__reverse_order(src_rev, s, src_p->shared->size,
                                 src_p->shared->u.atomic.order);
              except_ret =
                  (cb_struct.func)(H5T_CONV_EXCEPT_TRUNCATE, src_id, dst_id,
                                   src_rev, d, cb_struct.user_data);
            }
        ...
        if (sign) {                           /*source is negative*/
          if (first < dst.prec - 1) {
            ...
            if (except_ret ==
                H5T_CONV_UNHANDLED) { /*If this case ignored by user handler*/
              /*Convert to integer representation.  Equivalent to ~(value -
               * 1).*/
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__conv_i_f(hid_t src_id, hid_t dst_id, H5T_cdata_t *cdata,
                     size_t nelmts, size_t buf_stride,
                     size_t H5_ATTR_UNUSED bkg_stride, void *buf,
                     void H5_ATTR_UNUSED *bkg) {
  /* Traversal-related variables */
  ...
  case H5T_CONV_CONV:
    ...
    for (elmtno = 0; elmtno < nelmts; elmtno++) {
      ...
      /* What is the bit position(starting from 0 as first one) for the most
       * significant bit(MSB) of S which is set?
       */
      if (H5T_SGN_2 == src.u.i.sign) {
        sfirst =
            H5T__bit_find(int_buf, (size_t)0, src.prec - 1, H5T_BIT_MSB, TRUE);
        if (sign && sfirst < 0)
      ...
      if (H5T_SGN_2 == src.u.i.sign && sign) {
        if (!is_max_neg) {
          ...
          sfirst = H5T__bit_find(int_buf, (size_t)0, src.prec - 1, H5T_BIT_MSB,
                                 TRUE);
        } else {
          /* If it's maximal negative number 0x80...000, treat it as if it
           * overflowed (create a carry) to help conversion.  i.e. a character
           * type number 0x80 is treated as 0x100.
           */
        ...
        /* Sign bit has been negated if bit vector isn't 0x80...00.  Set all
         * bits in front of sign bit to 0 in the temporary buffer because
         * they're all negated from the previous step.
         */
      ...
      if (first > dst.u.f.msize) {
        /* If the bit sequence is bigger than the mantissa part, there'll be
         * some precision loss.  Let user's handler deal with the case if it's
         * present
         */
        if (cb_struct.func) {
          H5T__reverse_order(
              src_rev, s, src_p->shared->size,
              src_p->shared->u.atomic.order); /*reverse order first*/
          except_ret =
              (cb_struct.func)(H5T_CONV_EXCEPT_PRECISION, src_id, dst_id,
                               src_rev, d, cb_struct.user_data);
        }
        ...
        /* If user's exception handler does deal with it, we do it by dropping
         * off the extra bits at the end and do rounding.  If we have
         * .50...0(decimal) after radix point, we do roundup when the least
         * significant digit before radix is odd, we do rounddown if it's even.
         */
        ...
        H5T__bit_shift(int_buf, (ssize_t)(dst.u.f.msize - first), (size_t)0,
                       buf_size * 8);
        ...
        /* The bit sequence can fit mantissa part.  Left shift to fit in from
         * high-order of bit position. */
        H5T__bit_shift(int_buf, (ssize_t)(dst.u.f.msize - first), (size_t)0,
                       dst.u.f.msize);
      }
      ...
      if (expo > expo_max) {  /*overflows*/
        if (cb_struct.func) { /*user's exception handler.  Reverse back source
                                 order*/
          H5T__reverse_order(
              src_rev, s, src_p->shared->size,
              src_p->shared->u.atomic.order); /*reverse order first*/
      ...
      if (except_ret == H5T_CONV_UNHANDLED) {
        ...
        H5T__bit_copy(d, dst.u.f.mpos, int_buf, (size_t)0,
                      (buf_size * 8) > dst.u.f.msize ? dst.u.f.msize
                                                     : buf_size * 8);
      }
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T_reclaim_cb(void *elem, const H5T_t *dt, unsigned H5_ATTR_UNUSED ndim,
  ...
  if (dt->shared->type == H5T_REFERENCE) {
    if (H5T__ref_reclaim(elem, dt) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTFREE, FAIL,
                  "can't reclaim ref elements");
  } else {
    assert(op_data);
