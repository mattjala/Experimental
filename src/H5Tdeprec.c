...
herr_t H5Tcommit1(hid_t loc_id, const char *name, hid_t type_id) {
  ...
  if (NULL == (data = H5VL_datatype_commit(
                   vol_obj, &loc_params, name, type_id, H5P_LINK_CREATE_DEFAULT,
...
hid_t H5Topen1(hid_t loc_id, const char *name) {
  ...
  /* Open the datatype */
  if (NULL == (dt = H5VL_datatype_open(
                   vol_obj, &loc_params, name, H5P_DATATYPE_ACCESS_DEFAULT,
                   H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)))
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTOPENOBJ, H5I_INVALID_HID,
                "unable to open named datatype");

  /* Register the type and return the ID */
  if ((ret_value = H5VL_register(H5I_DATATYPE, dt, vol_obj->connector, TRUE)) <
      0)
  ...
  if (H5I_INVALID_HID == ret_value)
    if (dt && H5VL_datatype_close(vol_obj, H5P_DATASET_XFER_DEFAULT,
                                  H5_REQUEST_NULL) < 0)
