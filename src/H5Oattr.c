...
static void *H5O__attr_decode(H5F_t *f, H5O_t *open_oh, unsigned mesg_flags,
...
static herr_t H5O__attr_post_copy_file(const H5O_loc_t *src_oloc,
                                       const void *mesg_src,
                                       H5O_loc_t *dst_oloc, void *mesg_dst,
                                       H5O_copy_t *cpy_info);
static herr_t H5O__attr_get_crt_index(const void *_mesg,
...
static void *H5O__attr_decode(H5F_t *f, H5O_t *open_oh,
  ...
  if (attr->shared->version < H5O_ATTR_VERSION_1 ||
      attr->shared->version > H5O_ATTR_VERSION_LATEST)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTLOAD, NULL,
  ...
  if (attr->shared->version >= H5O_ATTR_VERSION_2) {
    ...
    if (flags & (unsigned)~H5O_ATTR_FLAG_ALL)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTLOAD, NULL,
                  "unknown flag for attribute message");
  } else
   ...
   *
   * NOTE: If the buffer overflow error message changes,
   * test_corrupted_attnamelen() in titerate.c will fail since it looks for it
   * explicitly.
   */
  if (H5_IS_BUFFER_OVERFLOW(p, name_len, p_end))
    HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                "ran off end of input buffer while decoding");
  if (NULL ==
      (attr->shared->name = H5MM_strndup((const char *)p, name_len - 1)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");

  /* Make an attempt to detect corrupted name or name length - HDFFV-10588 */
  if (name_len != (HDstrnlen(attr->shared->name, name_len) + 1))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTDECODE, NULL,
                "attribute name has different length than stored length");
  ...
  if (H5_IS_BUFFER_OVERFLOW(p, attr->shared->dt_size, p_end))
    HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                "ran off end of input buffer while decoding");
  if (NULL ==
      (attr->shared->dt = (H5T_t *)(H5O_MSG_DTYPE->decode)(
           f, open_oh,
           ((flags & H5O_ATTR_FLAG_TYPE_SHARED) ? H5O_MSG_FLAG_SHARED : 0),
  ...
  if ((extent = (H5S_extent_t *)(H5O_MSG_SDSPACE->decode)(
           f, open_oh,
           ((flags & H5O_ATTR_FLAG_SPACE_SHARED) ? H5O_MSG_FLAG_SHARED : 0),
           ioflags, attr->shared->ds_size, p)) == NULL)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTDECODE, NULL,
                "can't decode attribute dataspace");
  ...
  if (H5S_select_all(attr->shared->ds, FALSE) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, NULL,
  ...
  H5_CHECKED_ASSIGN(attr->shared->data_size, size_t, ds_size * (hsize_t)dt_size,
                    hsize_t);
  ...
  if (attr->shared->data_size) {
    ...
    if (NULL == (attr->shared->data =
                     H5FL_BLK_MALLOC(attr_buf, attr->shared->data_size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
    H5MM_memcpy(attr->shared->data, p, attr->shared->data_size);
  }

  /* Increment the reference count for this object header message in
     cache(compact storage) or for the object from dense storage. */
  attr->shared->nrefs++;
    ...
    if (attr) {
      if (attr->shared)
        if (H5A__shared_free(attr) < 0)
          HDONE_ERROR(H5E_ATTR, H5E_CANTRELEASE, NULL,
...
static herr_t H5O__attr_encode(H5F_t *f, uint8_t *p, const void *mesg) {
  ...
  size_t name_len;        /* Attribute name length */
  htri_t is_type_shared;  /* Flag to indicate that a shared datatype is used for
                             this attribute */
  htri_t is_space_shared; /* Flag to indicate that a shared dataspace is used
                             for this attribute */
  unsigned flags = 0;     /* Attribute flags */
  ...
  if ((is_type_shared = H5O_msg_is_shared(H5O_DTYPE_ID, attr->shared->dt)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_BADMESG, FAIL,
                "can't determine if datatype is shared");

  if ((is_space_shared = H5O_msg_is_shared(H5O_SDSPACE_ID, attr->shared->ds)) <
      0)
    HGOTO_ERROR(H5E_OHDR, H5E_BADMESG, FAIL,
                "can't determine if dataspace is shared");
  ...
  *p++ = attr->shared->version;
  ...
  if (attr->shared->version < H5O_ATTR_VERSION_2) {
    memset(p + attr->shared->dt_size, 0,
           H5O_ALIGN_OLD(attr->shared->dt_size) - attr->shared->dt_size);
  ...
  if (attr->shared->version < H5O_ATTR_VERSION_2) {
    memset(p + attr->shared->ds_size, 0,
           H5O_ALIGN_OLD(attr->shared->ds_size) - attr->shared->ds_size);
...
static void *H5O__attr_copy(const void *_src, void *_dst) {
  ...
  /* copy */
  if (NULL ==
      (ret_value = (H5A_t *)H5A__copy((H5A_t *)_dst, (const H5A_t *)_src)))
...
static size_t H5O__attr_size(const H5F_t H5_ATTR_UNUSED *f, const void *_mesg) {
  ...
  if (attr->shared->version == H5O_ATTR_VERSION_1)
    ret_value +=
        H5O_ALIGN_OLD(name_len) +              /*attribute name	*/
...
static herr_t H5O__attr_free(void *mesg) {
  ...
  if (H5A__close(attr) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTCLOSEOBJ, FAIL,
                "unable to close attribute object");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_pre_copy_file(H5F_t H5_ATTR_UNUSED *file_src,
                                      const void *native_src, hbool_t *deleted,
   ...
   * exceed the message version allowed by the destination file's high bound.
   */
  if (attr_src->shared->version >
      H5O_attr_ver_bounds[H5F_HIGH_BOUND(cpy_info->file_dst)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "attribute message version out of bounds");
 ...
 */
static void *
H5O__attr_copy_file(H5F_t *file_src,
  ...
  /* Mark datatype as being on disk now.  This step used to be done in a lower
   * level by H5O_dtype_decode.  But it has been moved up.  Not an ideal place,
   * but no better place than here. */
  if (H5T_set_loc(((H5A_t *)native_src)->shared->dt, H5F_VOL_OBJ(file_src),
                  H5T_LOC_DISK) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTINIT, NULL, "invalid datatype location");

  if (NULL == (ret_value = H5A__attr_copy_file((H5A_t *)native_src, file_dst,
                                               recompute_size, cpy_info)))
...
static herr_t H5O__attr_post_copy_file(const H5O_loc_t *src_oloc,
  ...
  if (H5A__attr_post_copy_file(src_oloc, (const H5A_t *)mesg_src, dst_oloc,
                               (H5A_t *)mesg_dst, cpy_info) < 0)
...
static herr_t H5O__attr_debug(H5F_t *f, const void *_mesg, FILE *stream,
  ...
  if ((H5O_MSG_DTYPE->debug)(f, mesg->shared->dt, stream, indent + 3,
                             MAX(0, fwidth - 3)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, FAIL,
                "unable to display datatype message info");

  fprintf(stream, "%*sDataspace...\n", indent, "");
  fprintf(stream, "%*s%-*s %lu\n", indent + 3, "", MAX(0, fwidth - 3),
          "Encoded Size:", (unsigned long)(mesg->shared->ds_size));
  if (H5S_debug(f, mesg->shared->ds, stream, indent + 3, MAX(0, fwidth - 3)) <
      0)
    HGOTO_ERROR(H5E_OHDR, H5E_WRITEERROR, FAIL,
                "unable to display dataspace message info");
