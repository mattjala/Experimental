...
static herr_t H5O__visit_cb(hid_t group, const char *name,
...
const H5O_token_t H5O_TOKEN_UNDEF_g = {{255, 255, 255, 255, 255, 255, 255, 255,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__set_version(H5F_t *f, H5O_t *oh, uint8_t oh_flags,
  ...
  version =
      (uint8_t)MAX(version, (uint8_t)H5O_obj_ver_bounds[H5F_LOW_BOUND(f)]);

  /* Version bounds check */
  if (version > H5O_obj_ver_bounds[H5F_HIGH_BOUND(f)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "object header version out of bounds");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_create(H5F_t *f, size_t size_hint, size_t initial_rc, hid_t ocpl_id,
                  H5O_loc_t *loc /*out*/) {
  H5O_t *oh = NULL;
 ...
 *-----------------------------------------------------------------------------
 */
herr_t H5O_apply_ohdr(H5F_t *f, H5O_t *oh, hid_t ocpl_id, size_t size_hint,
  ...
  if (0 < H5P_exist_plist(oc_plist, H5O_BAD_MESG_COUNT_NAME))
    ...
    if (H5P_get(oc_plist, H5O_BAD_MESG_COUNT_NAME, &oh->store_bad_mesg_count) <
        0)
...
/* Avoid compiler warning on 32-bit machines */
  ...
  oh->mesg[0].raw = oh->chunk[0].image + H5O_SIZEOF_HDR(oh) -
 ...
 *-------------------------------------------------------------------------
 */
void *H5O_open_name(const H5G_loc_t *loc, const char *name,
 ...
 *-------------------------------------------------------------------------
 */
void *H5O__open_by_idx(const H5G_loc_t *loc, const char *name,
 ...
 *-------------------------------------------------------------------------
 */
void *H5O__open_by_addr(const H5G_loc_t *loc, haddr_t addr,
                        H5I_type_t *opened_type) {
  H5G_loc_t obj_loc;      /* Location used to open group */
  H5G_name_t obj_path;    /* Opened object group hier. path */
  H5O_loc_t obj_oloc;     /* Opened object object location */
  ...
  H5G_name_reset(obj_loc.path); /* objects opened through this routine don't
                                   have a path name */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_close(H5O_loc_t *loc, hbool_t *file_closed /*out*/) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (adjust) {
    if (adjust < 0) {
      ...
      if ((unsigned)(-adjust) > oh->nlink)
        HGOTO_ERROR(H5E_OHDR, H5E_LINKCOUNT, (-1),
      ...
      if (H5AC_mark_entry_dirty(oh) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTMARKDIRTY, (-1),
      ...
      if (oh->nlink == 0) {
        ...
        if (H5FO_opened(f, addr) != NULL) {
          ...
          if (H5FO_mark(f, addr, TRUE) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, (-1),
                        "can't mark object for deletion");
        } /* end if */
        else {
    ...
    if (oh->version > H5O_VERSION_1) {
      ...
      if (oh->has_refcount_msg) {
        ...
        if (oh->nlink <= 1) {
          if (H5O__msg_remove_real(f, oh, H5O_MSG_REFCOUNT, H5O_ALL, NULL, NULL,
                                   TRUE) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTDELETE, (-1),
                        "unable to delete refcount message");
        ...
        /* Update refcount message with new link count */
          ...
          if (H5O__msg_write_real(f, oh, H5O_MSG_REFCOUNT,
                                  H5O_MSG_FLAG_DONTSHARE, 0, &refcount) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTUPDATE, (-1),
                        "unable to update refcount message");
        } /* end else */
      }   /* end if */
      else {
        ...
        if (oh->nlink > 1) {
          ...
          if (H5O__msg_append_real(f, oh, H5O_MSG_REFCOUNT,
                                   H5O_MSG_FLAG_DONTSHARE, 0, &refcount) < 0)
            HGOTO_ERROR(H5E_OHDR, H5E_CANTINSERT, (-1),
                        "unable to create new refcount message");
...
int H5O_link(const H5O_loc_t *loc, int adjust) {
  ...
  if (ret_value >= 0 && deleted && H5O_delete(loc->file, loc->addr) < 0)
    HDONE_ERROR(H5E_OHDR, H5E_CANTDELETE, FAIL,
                "can't delete object from file");
 ...
 *              H5O_msg_append->...->H5O_msg_append->H5O_unprotect sequence of
 *calls during an object's creation.
 *
 ...
 *-------------------------------------------------------------------------
 */
H5O_t *H5O_protect(const H5O_loc_t *loc, unsigned prot_flags,
  ...
  if (NULL == (oh = (H5O_t *)H5AC_protect(loc->file, H5AC_OHDR, loc->addr,
                                          &udata, prot_flags)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTPROTECT, NULL,
  ...
  if (cont_msg_info.nmsgs > 0) {
    ...
    chk_udata.oh = oh;
    chk_udata.chunkno =
        UINT_MAX; /* Set to invalid value, for better error detection */
    ...
    while (curr_msg < cont_msg_info.nmsgs) {
      H5O_chunk_proxy_t
          *chk_proxy; /* Proxy for chunk, to bring it into memory */
#ifndef NDEBUG
      ...
      if (NULL ==
          (chk_proxy = (H5O_chunk_proxy_t *)H5AC_protect(
               loc->file, H5AC_OHDR_CHK, cont_msg_info.msgs[curr_msg].addr,
      ...
      if (H5AC_unprotect(loc->file, H5AC_OHDR_CHK,
                         cont_msg_info.msgs[curr_msg].addr, chk_proxy,
                         H5AC__NO_FLAGS_SET) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, NULL,
                    "unable to release object header chunk");
    ...
    cont_msg_info.msgs =
        (H5O_cont_t *)H5FL_SEQ_FREE(H5O_cont_t, cont_msg_info.msgs);
 ...
 */
    ...
    if (oh->version == H5O_VERSION_1 &&
      ...
      HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, NULL,
                  "corrupt object header - incorrect # of messages");
  ...
  if (pin_all_chunks && oh->nchunks > 1) {
    ...
    for (u = 1; u < oh->nchunks; u++) {
      ...
      if (H5AC_pin_protected_entry(chk_proxy) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTPIN, NULL,
                    "unable to pin object header chunk");
  ...
  if (ret_value == NULL && oh) {
    ...
    if (H5O_unprotect(loc, oh, H5AC__NO_FLAGS_SET) < 0)
      HDONE_ERROR(H5E_OHDR, H5E_CANTUNPROTECT, NULL,
...
H5O_t *H5O_pin(const H5O_loc_t *loc) {
  ...
  if (H5O__inc_rc(oh) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTINC, NULL,
                "unable to increment reference count on object header");
...
herr_t H5O_unpin(H5O_t *oh) {
  ...
  if (H5O__dec_rc(oh) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTDEC, FAIL,
                "unable to decrement reference count on object header");
    ...
    for (u = 1; u < oh->nchunks; u++) {
      if (NULL != oh->chunk[u].chunk_proxy) {
        ...
        if (H5AC_unpin_entry(oh->chunk[u].chunk_proxy) < 0)
          HGOTO_ERROR(H5E_OHDR, H5E_CANTUNPIN, FAIL,
                      "unable to unpin object header chunk");
  ...
  if (oh->flags & H5O_HDR_STORE_TIMES) {
    ...
    if (oh->version == H5O_VERSION_1) {
      ...
      for (idx = 0; idx < oh->nmesgs; idx++)
        if (H5O_MSG_MTIME == oh->mesg[idx].type ||
            H5O_MSG_MTIME_NEW == oh->mesg[idx].type)
          break;
      ...
      if (idx == oh->nmesgs) {
        ...
        /* If we would have to create a new message, but we aren't 'forcing' it,
         * get out now */
        if (!force)
          HGOTO_DONE(SUCCEED); /*nothing to do*/

        /* Allocate space for the modification time message */
        if (H5O__msg_alloc(f, oh, H5O_MSG_MTIME_NEW, &mesg_flags, &now, &idx) <
            0)
      ...
      /* Protect chunk */
      if (NULL ==
          (chk_proxy = H5O__chunk_protect(f, oh, oh->mesg[idx].chunkno)))
...
herr_t H5O_touch(const H5O_loc_t *loc, hbool_t force) {
  ...
  unsigned oh_flags =
      H5AC__NO_FLAGS_SET;     /* Flags for unprotecting object header */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5O_touch_oh(loc->file, oh, force) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                "unable to update object modification time");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_bogus_oh(H5F_t *f, H5O_t *oh, unsigned bogus_id,
                    unsigned mesg_flags) {
  ...
  if (idx == oh->nmesgs) {
    ...
    if (NULL == (bogus = H5MM_malloc(sizeof(H5O_bogus_t))))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
                  "memory allocation failed for 'bogus' message");
    ...
    if (H5O__msg_alloc(f, oh, type, &mesg_flags, bogus, &idx) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
                  "unable to allocate space for 'bogus' message");
...
herr_t H5O_delete(H5F_t *f, haddr_t addr) {
  ...
  oh_flags =
      H5AC__DIRTIED_FLAG | H5AC__DELETED_FLAG | H5AC__FREE_FILE_SPACE_FLAG;
...
H5O_loc_t *H5O_get_loc(hid_t object_id) {
  ...
  case H5I_GROUP:
    if (NULL == (ret_value = H5O_OBJ_GROUP->get_oloc(object_id)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                  "unable to get object location from group ID");
    break;

  case H5I_DATASET:
    if (NULL == (ret_value = H5O_OBJ_DATASET->get_oloc(object_id)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                  "unable to get object location from dataset ID");
    break;

  case H5I_DATATYPE:
    if (NULL == (ret_value = H5O_OBJ_DATATYPE->get_oloc(object_id)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                  "unable to get object location from datatype ID");
    break;

  case H5I_MAP:
    HGOTO_ERROR(H5E_OHDR, H5E_BADTYPE, NULL,
...
herr_t H5O_get_hdr_info(const H5O_loc_t *loc, H5O_hdr_info_t *hdr) {
  ...
  if (H5O__get_hdr_info_real(oh, hdr) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "can't retrieve object header info");
 ...
 *
 * Purpose:    Internal routine to retrieve the object header information for an
 *object
 *
 * Return:    Success:    Non-negative
 *        Failure:    Negative
...
static herr_t H5O__get_hdr_info_real(const H5O_t *oh, H5O_hdr_info_t *hdr) {
  ...
  const H5O_chunk_t
      *curr_chunk; /* Pointer to current message being operated on */
  unsigned u;      /* Local index variable */
   ...
   */
  hdr->space.meta = (hsize_t)H5O_SIZEOF_HDR(oh) +
                    (hsize_t)(H5O_SIZEOF_CHKHDR_OH(oh) * (oh->nchunks - 1));
  hdr->space.mesg = 0;
  hdr->space.free = 0;
  hdr->mesg.present = 0;
  hdr->mesg.shared = 0;
  for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
    ...
    if (H5O_NULL_ID == curr_msg->type->id)
      hdr->space.free +=
          (hsize_t)((size_t)H5O_SIZEOF_MSGHDR_OH(oh) + curr_msg->raw_size);
  ...
  assert(hdr->space.total ==
         (hdr->space.free + hdr->space.meta + hdr->space.mesg));
...
herr_t H5O_get_info(const H5O_loc_t *loc, H5O_info2_t *oinfo, unsigned fields) {
  ...
  if (fields & H5O_INFO_BASIC) {
    ...
    if (H5VL_native_addr_to_token(loc->file, H5I_FILE, loc->addr,
                                  &oinfo->token) < 0)
    ...
    if (oh->version > H5O_VERSION_1) {
      ...
      if ((exists = H5O_msg_exists_oh(oh, H5O_MTIME_ID)) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL,
                    "unable to check for MTIME message");
      if (exists > 0) {
        ...
        if ((exists = H5O_msg_exists_oh(oh, H5O_MTIME_NEW_ID)) < 0)
          HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL,
                      "unable to check for MTIME_NEW message");
        if (exists > 0) {
          /* Get "new style" modification time info */
          if (NULL ==
              H5O_msg_read_oh(loc->file, oh, H5O_MTIME_NEW_ID, &oinfo->ctime))
            HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                        "can't read MTIME_NEW message");
        } /* end if */
        else
          oinfo->ctime = 0;
      } /* end else */
  ...
  if (fields & H5O_INFO_NUM_ATTRS)
    if (H5O__attr_count_real(loc->file, oh, &oinfo->num_attrs) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                  "can't retrieve attribute count");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_get_native_info(const H5O_loc_t *loc, H5O_native_info_t *oinfo,
                           unsigned fields) {
  const H5O_obj_class_t *obj_class; /* Class of object for header */
  H5O_t *oh = NULL;                 /* Object header */
  herr_t ret_value = SUCCEED;       /* Return value */
...
herr_t H5O_get_create_plist(const H5O_loc_t *loc, H5P_genplist_t *oc_plist) {
  ...
  if (oh->version > H5O_VERSION_1) {
    ...
    if (H5P_set(oc_plist, H5O_CRT_ATTR_MIN_DENSE_NAME, &oh->min_dense) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                  "can't set min. # of dense attributes in property list");
 ...
 *-------------------------------------------------------------------------
 */
void *H5O_obj_create(H5F_t *f, H5O_type_t obj_type, void *crt_info,
                     H5G_loc_t *obj_loc) {
  ...
  for (u = 0; u < NELMTS(H5O_obj_class_g); u++) {
    ...
    if (H5O_obj_class_g[u]->type == obj_type) {
      ...
      if (NULL ==
          (ret_value = H5O_obj_class_g[u]->create(f, crt_info, obj_loc)))
        HGOTO_ERROR(H5E_OHDR, H5E_CANTOPENOBJ, NULL, "unable to open object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_get_rc_and_type(const H5O_loc_t *loc, unsigned *rc,
                           H5O_type_t *otype) {
  H5O_t *oh = NULL;           /* Object header */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 */
static herr_t
H5O__free_visit_visited(void *item, void H5_ATTR_UNUSED *key,
                        void H5_ATTR_UNUSED *operator_data /*in,out*/) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__visit_cb(hid_t H5_ATTR_UNUSED group, const char *name,
                            const H5L_info2_t *linfo, void *_udata) {
  H5O_iter_visit_ud_t *udata =
      (H5O_iter_visit_ud_t *)_udata; /* User data for callback */
      ...
      if (ret_value == H5_ITER_CONT) {
        ...
        if (oinfo.rc > 1) {
          ...
          if ((new_node = H5FL_MALLOC(H5_obj_t)) == NULL)
            HGOTO_ERROR(H5E_OHDR, H5E_NOSPACE, H5_ITER_ERROR,
                        "can't allocate object node");
  ...
  if (obj_found && H5G_loc_free(&obj_loc) < 0)
    HDONE_ERROR(H5E_OHDR, H5E_CANTRELEASE, H5_ITER_ERROR,
                "can't free location");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__visit(H5G_loc_t *loc, const char *obj_name, H5_index_t idx_type,
  ...
  if ((obj_id = H5VL_wrap_register(opened_type, obj, TRUE)) < 0)
    HGOTO_ERROR(H5E_ID, H5E_CANTREGISTER, FAIL,
                "unable to register visited object");
  ...
  if (oinfo.type == H5O_TYPE_GROUP) {
    ...
    if ((udata.visited = H5SL_create(H5SL_TYPE_OBJ, NULL)) == NULL)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTCREATE, FAIL,
                  "can't create skip list for visited objects");
    ...
    if (oinfo.rc > 1) {
      ...
      if (H5VL_native_token_to_addr(loc->oloc->file, H5I_FILE, oinfo.token,
                                    &(obj_pos->addr)) < 0)
    ...
    if ((ret_value = H5G_visit(&vis_loc, ".", idx_type, order, H5O__visit_cb,
                               &udata)) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_BADITER, FAIL, "object visitation failed");
  } /* end if */

done:
  /* XXX (VOL MERGE): Probably also want to consider closing obj here on
   * failures */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__free(H5O_t *oh, hbool_t H5_ATTR_NDEBUG_UNUSED force) {
  unsigned u;                 /* Local index variable */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (oh->proxy)
    if (H5AC_proxy_entry_dest(oh->proxy) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTFREE, FAIL,
                  "unable to destroy virtual entry used for proxy");
