...
static herr_t H5O__copy_header_real(const H5O_loc_t *oloc_src,
                                    H5O_loc_t *oloc_dst /*out*/,
                                    H5O_copy_t *cpy_info, H5O_type_t *obj_type,
                                    void **udata);
static herr_t H5O__copy_header(const H5O_loc_t *oloc_src,
                               H5O_loc_t *oloc_dst /*out*/, hid_t ocpypl_id,
                               hid_t lcpl_id);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__copy(const H5G_loc_t *loc, const char *src_name, H5G_loc_t *dst_loc,
  ...
  if (H5L_exists_tolerant(dst_loc, dst_name, &dst_exists) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                "unable to check if destination name exists");
  if (dst_exists)
    HGOTO_ERROR(H5E_OHDR, H5E_EXISTS, FAIL,
  ...
  if (obj_open && H5O_close(&src_oloc, NULL) < 0)
    HDONE_ERROR(H5E_OHDR, H5E_CLOSEERROR, FAIL,
...
static herr_t H5O__copy_header_real(const H5O_loc_t *oloc_src,
  ...
  hbool_t *deleted =
      NULL; /* Array of flags indicating whether messages should be copied */
  hbool_t inserted = FALSE; /* Whether the destination object header has been
  ...
  if (obj_class->get_copy_file_udata &&
    ...
    HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
                "unable to retrieve copy user data");
  ...
  if (cpy_info->merge_comm_dt && obj_class->type == H5O_TYPE_NAMED_DATATYPE) {
    ...
    if (fileno_src == fileno_dst) {
       ...
       * necessary */
      if ((merge = H5O__copy_search_comm_dt(oloc_src->file, oh_src, oloc_dst,
                                            cpy_info)) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                    "can't search for matching committed datatype");
    ...
    if (merge) {
      ...
      addr_map->inc_ref_count =
          0; /* Start with no additional ref counts to add */
      addr_map->obj_class = obj_class;
      addr_map->udata = cpy_udata;

      /* Insert into skip list */
      if (H5SL_insert(cpy_info->map_list, addr_map, &(addr_map->src_obj_pos)) <
          0) {
        addr_map = H5FL_FREE(H5O_addr_map_t, addr_map);
        HGOTO_ERROR(H5E_OHDR, H5E_CANTINSERT, FAIL,
                    "can't insert object into skip list");
      } /* end if */
  ...
  /* Flush any dirty messages in source object header to update the header
   * chunks */
  if (H5O__flush_msgs(oloc_src->file, oh_src) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTFLUSH, FAIL,
                "unable to flush object header messages");
  ...
  if (oh_dst->version > H5O_obj_ver_bounds[H5F_HIGH_BOUND(oloc_dst->file)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "destination object header version out of bounds");
   ...
   */
  if (NULL ==
      (deleted = (hbool_t *)H5MM_malloc(sizeof(hbool_t) * oh_src->nmesgs)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  memset(deleted, FALSE, sizeof(hbool_t) * oh_src->nmesgs);

  /* "pre copy" pass over messages, to gather information for actual message
   * copy operation (for messages which depend on information from other
   * messages) Keep track of how many NULL or deleted messages we find (or
   * create)
   */
  null_msgs = 0;
  for (mesgno = 0; mesgno < oh_src->nmesgs; mesgno++) {
    ...
    assert(
        !mesg_src
             ->dirty); /* Should be cleared by earlier call to flush messages */
  ...
  /* Initialize size of message list.  It may or may not include the NULL
   * messages detected above.
   */
  ...
  if (oh_dst->alloc_nmesgs > 0)
    if (NULL ==
        (oh_dst->mesg = H5FL_SEQ_CALLOC(H5O_mesg_t, oh_dst->alloc_nmesgs)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  ...
  for (mesgno = 0; mesgno < oh_dst->nmesgs; mesgno++) {
    ...
    if (copy_type->copy_file) {
      hbool_t recompute_size; /* Whether copy_file callback created a shared
                                 message */
      ...
      if (NULL ==
          (mesg_dst->native = H5O__msg_copy_file(
               copy_type, oloc_src->file, mesg_src->native, oloc_dst->file,
      ...
      if (recompute_size)
        mesg_dst->raw_size = H5O_ALIGN_OH(
            oh_dst, H5O_msg_raw_size(oloc_dst->file, mesg_dst->type->id, FALSE,
                                     mesg_dst->native));

      /* Mark the message in the destination as dirty, so it'll get encoded when
       * the object header is flushed */
      mesg_dst->dirty = TRUE;
  ...
  if (dst_oh_size < H5O_MIN_SIZE) {
    size_t delta =
        (size_t)(H5O_MIN_SIZE - dst_oh_size); /* Delta in chunk size needed */
  ...
  if (HADDR_UNDEF == (oh_dst->chunk[0].addr = H5MF_alloc(
                          oloc_dst->file, H5FD_MEM_OHDR, (hsize_t)dst_oh_size)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
   ...
   *  (QAK - 2010/08/17)
   */
  if (NULL == (oh_dst->chunk[0].image =
                   H5FL_BLK_CALLOC(chunk_image, (size_t)dst_oh_size)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  ...
  for (mesgno = 0; mesgno < oh_dst->nmesgs; mesgno++) {
    ...
    if (!mesg_dst->dirty)
      ...
      H5MM_memcpy(current_pos, mesg_src->raw - msghdr_size,
  ...
  assert(current_pos + dst_oh_gap + dst_oh_null +
             H5O_SIZEOF_CHKSUM_OH(oh_dst) ==
  ...
  /* If we are merging committed datatypes and this is a committed datatype,
   * insert the copied datatype into the list of committed datatypes in the
   * target file.
   */
  if (cpy_info->merge_comm_dt && obj_class->type == H5O_TYPE_NAMED_DATATYPE)
    if (H5O__copy_insert_comm_dt(oloc_src->file, oh_src, oloc_dst, cpy_info) <
        0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL,
                  "can't insert committed datatype into destination list");
  ...
  /* "post copy" loop over messages, to fix up any messages which require a
   * complete object header for destination object
   */
  null_msgs = 0;
  ...
  /* Increment object header's reference count, if any descendents have created
  ...
  if (H5AC_retag_copied_metadata(oloc_dst->file, oloc_dst->addr) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTTAG, FAIL,
                "unable to re-tag metadata entries");

  /* Set metadata tag for destination object's object header */
  H5_BEGIN_TAG(oloc_dst->addr)

  /* Insert destination object header in cache */
  if (H5AC_insert_entry(oloc_dst->file, H5AC_OHDR, oloc_dst->addr, oh_dst,
...
herr_t H5O_copy_header_map(const H5O_loc_t *oloc_src,
                           H5O_loc_t *oloc_dst /*out*/, H5O_copy_t *cpy_info,
                           hbool_t inc_depth, H5O_type_t *obj_type,
                           void **udata /*out*/) {
  H5O_addr_map_t *addr_map = NULL; /* Address mapping of object copied */
  ...
  if (addr_map == NULL) {
    ...
    /* (Can't do this for all copies, since committed datatypes should always be
     * copied) */
    if (inc_depth)
      cpy_info->curr_depth++;

    /* Copy object referred to */
    if (H5O__copy_header_real(oloc_src, oloc_dst, cpy_info, obj_type, udata) <
        0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL, "unable to copy object");
  ...
  if (inc_link)
    if (H5O_link(oloc_dst, 1) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
                  "unable to increment object link count");
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
static herr_t H5O__copy_free_addrmap_cb(void *_item, void H5_ATTR_UNUSED *key,
...
static herr_t H5O__copy_header(const H5O_loc_t *oloc_src,
  ...
  H5O_copy_dtype_merge_list_t *dt_list =
      NULL;                   /* List of datatype merge suggestions */
  ...
  if (H5P_peek(ocpy_plist, H5O_CPY_MERGE_COMM_DT_LIST_NAME, &dt_list) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                "can't get merge committed datatype list");
  ...
  if ((cpy_option & H5O_COPY_SHALLOW_HIERARCHY_FLAG) > 0) {
    ...
    cpy_info.max_depth =
        -1; /* Current default is for full, recursive hier. copy */
...
static herr_t H5O__copy_obj(H5G_loc_t *src_loc, H5G_loc_t *dst_loc,
  ...
  hbool_t entry_inserted =
      FALSE; /* Flag to indicate that the new entry was inserted into a group */
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_free_comm_dt_cb(void *item, void *_key,
...
static int H5O__copy_comm_dt_cmp(const void *_key1, const void *_key2) {
  const H5O_copy_search_comm_dt_key_t *key1 =
      (const H5O_copy_search_comm_dt_key_t *)_key1;
  const H5O_copy_search_comm_dt_key_t *key2 =
      (const H5O_copy_search_comm_dt_key_t *)_key2;
  int ret_value = 0;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_search_comm_dt_attr_cb(const H5A_t *attr,
                                               void *_udata) {
  ...
  if (H5T_is_named(dt)) {
    ...
    if (NULL == (key->dt = (H5T_t *)H5O_msg_copy(H5O_DTYPE_ID, dt, NULL)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, FAIL,
                  "unable to copy datatype message");
...
H5O__copy_search_comm_dt_check(H5O_loc_t *obj_oloc,
  ...
  if (H5O_attr_iterate_real((hid_t)-1, obj_oloc, H5_INDEX_NAME, H5_ITER_NATIVE,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__copy_search_comm_dt_cb(hid_t H5_ATTR_UNUSED group,
                                          const char *name,
                                          const H5L_info2_t *linfo,
                                          void *_udata) {
  H5O_copy_search_comm_dt_ud_t *udata = (H5O_copy_search_comm_dt_ud_t *)
      _udata;                      /* Skip list of dtypes in dest file */
...
static htri_t H5O__copy_search_comm_dt(H5F_t *file_src, H5O_t *oh_src,
  ...
  if (NULL == (key->dt = (H5T_t *)H5O_msg_read_oh(file_src, oh_src,
                                                  H5O_DTYPE_ID, NULL)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, FAIL, "can't read DTYPE message");
  ...
  if (!cpy_info->dst_dt_list) {
    ...
    if (NULL == (cpy_info->dst_dt_list =
                     H5SL_create(H5SL_TYPE_GENERIC, H5O__copy_comm_dt_cmp)))
      HGOTO_ERROR(H5E_OHDR, H5E_CANTCREATE, FAIL,
                  "can't create skip list for committed datatypes");

    /* Add suggested types to list, if they are present */
    if (cpy_info->dst_dt_suggestion_list) {
      H5O_copy_dtype_merge_list_t *suggestion =
          cpy_info->dst_dt_suggestion_list;
      H5G_loc_t obj_loc;   /* Location of object */
      H5O_loc_t obj_oloc;  /* Object's object location */
      H5G_name_t obj_path; /* Object's group hier. path */
  ...
  if (!cpy_info->dst_dt_list_complete) {
    ...
    if (cpy_info->dst_dt_suggestion_list &&
        NULL !=
            (dst_addr = (haddr_t *)H5SL_search(cpy_info->dst_dt_list, key))) {
      ...
      if (cpy_info->mcdt_cb)
        if ((search_cb_ret = cpy_info->mcdt_cb(cpy_info->mcdt_ud)) ==
      ...
      if (search_cb_ret == H5O_MCDT_SEARCH_CONT) {
        ...
        if (!dst_root_loc.oloc) {
          ...
          if (NULL ==
              (dst_root_loc.oloc = H5G_oloc(H5G_rootof(oloc_dst->file))))
            HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                        "unable to get object location for root group");
          if (NULL ==
              (dst_root_loc.path = H5G_nameof(H5G_rootof(oloc_dst->file))))
            HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                        "unable to get path for root group");
        } /* end if */
        else
         ...
         * list */
        if (H5G_visit(&dst_root_loc, "/", H5_INDEX_NAME, H5_ITER_NATIVE,
        ...
        HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                    "unknown return value for callback");
    } /* end if */
  }   /* end if */
 ...
 *
 * Purpose:     Insert the committed datatype at oloc_dst into the merge
 *committed dt skiplist.  The datatype must not be present already.
 *
 * Return:      Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
