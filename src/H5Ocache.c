...
static herr_t H5O__cache_get_final_load_size(const void *image_ptr,
                                             size_t image_len, void *udata,
                                             size_t *actual_len);
static htri_t H5O__cache_verify_chksum(const void *image_ptr, size_t len,
                                       void *udata_ptr);
static void *H5O__cache_deserialize(const void *image, size_t len, void *udata,
                                    hbool_t *dirty);
...
static herr_t H5O__cache_chk_get_initial_load_size(void *udata,
                                                   size_t *image_len);
static htri_t H5O__cache_chk_verify_chksum(const void *image_ptr, size_t len,
                                           void *udata_ptr);
...
/* Prefix routines */
static herr_t H5O__prefix_deserialize(const uint8_t *image, size_t len,
                                      H5O_cache_ud_t *udata);

/* Chunk routines */
static herr_t H5O__chunk_deserialize(H5O_t *oh, haddr_t addr, size_t chunk_size,
                                     const uint8_t *image, size_t len,
                                     H5O_common_cache_ud_t *udata,
                                     hbool_t *dirty);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_get_initial_load_size(void H5_ATTR_UNUSED *_udata,
                                               size_t *image_len) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_get_final_load_size(const void *image,
                                             size_t image_len, void *_udata,
                                             size_t *actual_len) {
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5O__cache_verify_chksum(const void *_image, size_t len,
                                       void *_udata) {
  const uint8_t *image =
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__cache_deserialize(const void *image, size_t len, void *_udata,
                                    hbool_t *dirty) {
  ...
  if (H5O__chunk_deserialize(oh, udata->common.addr, udata->chunk0_size,
                             (const uint8_t *)image, len, &(udata->common),
                             dirty) < 0)
...
static herr_t H5O__cache_notify(H5AC_notify_action_t action, void *_thing) {
    ...
    if (oh->swmr_write) {
      ...
      if (H5AC_proxy_entry_add_parent(oh->proxy, oh) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                    "can't add object header as parent of proxy");
    }
    break;
  ...
  case H5AC_NOTIFY_ACTION_ENTRY_CLEANED: {
    ...
    /* Mark messages stored with the object header (i.e. messages in chunk 0) as
     * clean */
  ...
  case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
    if (oh->swmr_write) {
      ...
      if (H5AC_proxy_entry_remove_parent(oh->proxy, oh) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                    "can't remove object header as parent of proxy");
    }
    break;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__cache_chk_get_initial_load_size(void *_udata,
                                                   size_t *image_len) {
  const H5O_chk_cache_ud_t *udata =
      (const H5O_chk_cache_ud_t *)_udata; /* User data for callback */
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5O__cache_chk_verify_chksum(const void *_image, size_t len,
                                           void *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  H5O_chk_cache_ud_t *udata =
      (H5O_chk_cache_ud_t *)_udata; /* User data for callback */
  htri_t ret_value = TRUE;
...
static void *H5O__cache_chk_deserialize(const void *image, size_t len,
  ...
  if (udata->decoding) {
    ...
    /* Parse the chunk */
    if (H5O__chunk_deserialize(udata->oh, udata->common.addr, udata->size,
                               (const uint8_t *)image, len, &(udata->common),
                               dirty) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTINIT, NULL,
    ...
    H5_CHECKED_ASSIGN(chk_proxy->chunkno, unsigned, udata->oh->nchunks - 1,
                      size_t);
  } else {
    /* Sanity check */
  ...
  if (NULL == ret_value)
    if (chk_proxy && H5O__chunk_dest(chk_proxy) < 0)
      HDONE_ERROR(H5E_OHDR, H5E_CANTRELEASE, NULL,
                  "unable to destroy object header chunk");
...
static herr_t H5O__cache_chk_image_len(const void *_thing, size_t *image_len) {
  const H5O_chunk_proxy_t *chk_proxy =
      (const H5O_chunk_proxy_t *)_thing; /* Chunk proxy to query */
...
static herr_t H5O__cache_chk_serialize(const H5F_t *f, void *image, size_t len,
  ...
  H5O_chunk_proxy_t *chk_proxy =
      (H5O_chunk_proxy_t *)_thing; /* Object header chunk to serialize */
  herr_t ret_value = SUCCEED;
  ...
  case H5AC_NOTIFY_ACTION_AFTER_LOAD:
    if (chk_proxy->oh->swmr_write) {
      ...
      if (chk_proxy->fd_parent) {
        ...
        assert((((H5C_cache_entry_t *)(chk_proxy->fd_parent))->type->id ==
                H5AC_OHDR_ID) ||
               (((H5C_cache_entry_t *)(chk_proxy->fd_parent))->type->id ==
                H5AC_OHDR_CHK_ID));
...
static herr_t H5O__cache_chk_free_icr(void *_thing) {
  H5O_chunk_proxy_t *chk_proxy =
      (H5O_chunk_proxy_t *)_thing; /* Object header chunk proxy to release */
  herr_t ret_value = SUCCEED;
...
static herr_t H5O__add_cont_msg(H5O_cont_msgs_t *cont_msg_info,
  ...
  if (cont_msg_info->nmsgs >= cont_msg_info->alloc_nmsgs) {
    size_t na = MAX(H5O_NCHUNKS, cont_msg_info->alloc_nmsgs *
                                     2); /* Double # of messages allocated */
    H5O_cont_t *x;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__prefix_deserialize(const uint8_t *_image, size_t len,
                                      H5O_cache_ud_t *udata) {
  const uint8_t *image =
      (const uint8_t *)_image;            /* Pointer into raw data buffer */
  ...
  if (!memcmp(image, H5O_HDR_MAGIC, (size_t)H5_SIZEOF_MAGIC)) {
    ...
    if ((udata->v1_pfx_nmesgs > 0 &&
         udata->chunk0_size < H5O_SIZEOF_MSGHDR_OH(oh)) ||
  ...
  if ((size_t)(image - _image) !=
      (size_t)(H5O_SIZEOF_HDR(oh) - H5O_SIZEOF_CHKSUM_OH(oh)))
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL,
                "bad object header prefix length");
  ...
  if (ret_value < 0 && oh)
    if (H5O__free(oh, FALSE) < 0)
      HDONE_ERROR(H5E_OHDR, H5E_CANTRELEASE, FAIL,
                  "unable to destroy object header data");
...
static herr_t H5O__chunk_deserialize(H5O_t *oh, haddr_t addr, size_t chunk_size,
  ...
  unsigned
      nullcnt; /* Count of null messages (for sanity checking gaps in chunks) */
  hbool_t mesgs_modified = FALSE; /* Whether any messages were modified when the
                                     object header was deserialized */
  herr_t ret_value = SUCCEED;
  ...
  if (NULL == (oh->chunk[chunkno].image =
                   H5FL_BLK_MALLOC(chunk_image, oh->chunk[chunkno].size)))
    HGOTO_ERROR(H5E_OHDR, H5E_CANTALLOC, FAIL, "memory allocation failed");
  ...
  if (len < oh->chunk[chunkno].size)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL,
                "attempted to copy too many disk image bytes into buffer");
  ...
  /* Check for magic # on chunks > 0 in later versions of the format */
    ...
    if (memcmp(chunk_image, H5O_CHK_MAGIC, H5_SIZEOF_MAGIC) != 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTLOAD, FAIL,
                  "wrong object header chunk signature");
    chunk_image += H5_SIZEOF_MAGIC;
  }

  /* Decode messages from this chunk */
  eom_ptr = oh->chunk[chunkno].image +
  ...
  while (chunk_image < eom_ptr) {
    ...
    if ((udata->file_intent & H5F_ACC_RDWR) && H5O_NULL_ID == id &&
        oh->nmesgs > 0 && H5O_NULL_ID == oh->mesg[oh->nmesgs - 1].type->id &&
      ...
      if (H5O_CONT_ID == id) {
        ...
        if (NULL ==
            (cont = (H5O_cont_t *)(H5O_MSG_CONT->decode)(
                 udata->f, NULL, 0, &ioflags, mesg->raw_size, mesg->raw)))
          HGOTO_ERROR(H5E_OHDR, H5E_BADMESG, FAIL,
                      "bad continuation message found");
        H5_CHECKED_ASSIGN(cont->chunkno, unsigned,
                          udata->cont_msg_info->nmsgs + 1,
        ...
        if (H5O__add_cont_msg(udata->cont_msg_info, cont) < 0)
          HGOTO_ERROR(H5E_OHDR, H5E_CANTSET, FAIL,
                      "can't add continuation message");
      }
      /* Check if message is a ref. count message */
        ...
        if (oh->version <= H5O_VERSION_1)
          HGOTO_ERROR(
              H5E_OHDR, H5E_VERSION, FAIL,
              "object header version does not support reference count message");
        refcount = (H5O_refcount_t *)(H5O_MSG_REFCOUNT->decode)(
            udata->f, NULL, 0, &ioflags, mesg->raw_size, mesg->raw);
      ...
      /* Mark the message & chunk as dirty if the message was changed by
       * decoding */
    ...
    if ((eom_ptr - chunk_image) > 0 &&
        (eom_ptr - chunk_image) < H5O_SIZEOF_MSGHDR_OH(oh)) {
  ...
  if (chunk_image != oh->chunk[chunkno].image + oh->chunk[chunkno].size)
    HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                "object header image size mismatch");
  ...
  if (ret_value < 0 && udata->cont_msg_info->msgs) {
    udata->cont_msg_info->msgs =
        H5FL_SEQ_FREE(H5O_cont_t, udata->cont_msg_info->msgs);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__chunk_serialize(const H5F_t *f, H5O_t *oh,
                                   unsigned chunkno) {
  ...
  if (oh->version > H5O_VERSION_1)
    ...
    assert(!memcmp(oh->chunk[chunkno].image,
                   (chunkno == 0 ? H5O_HDR_MAGIC : H5O_CHK_MAGIC),
                   H5_SIZEOF_MAGIC));
  else
  ...
  if (oh->version > H5O_VERSION_1) {
    ...
    metadata_chksum =
        H5_checksum_metadata(oh->chunk[chunkno].image,
                             (oh->chunk[chunkno].size - H5O_SIZEOF_CHKSUM), 0);

    /* Metadata checksum */
    chunk_image = oh->chunk[chunkno].image +
                  (oh->chunk[chunkno].size - H5O_SIZEOF_CHKSUM);
