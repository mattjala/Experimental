...
#define H5B2_LEAF_NREC(l, hdr, idx)                                            \
...
typedef struct {
  ...
  hsize_t
      all_nrec; /* Number of records in node pointed to and all it's children */
} H5B2_node_ptr_t;
...
typedef struct {
  ...
  hsize_t
      cum_max_nrec; /* Cumulative max. # of records below this node's depth */
  uint8_t cum_max_nrec_size; /* Size to store cumulative max. # of records for
                                this node (in bytes) */
...
typedef struct H5B2_hdr_t {
  ...
  uint8_t split_percent; /* Percent full at which to split the node, when
                            inserting */
  uint8_t merge_percent; /* Percent full at which to merge the node, when
                            deleting */
  ...
  uint8_t max_nrec_size; /* Size to store max. # of records in any node (in
                            bytes) */
  ...
  H5AC_proxy_entry_t
      *top_proxy;        /* 'Top' proxy cache entry for all B-tree entries */
...
typedef enum H5B2_update_status_t {
  ...
  H5B2_UPDATE_SHADOW_DONE, /* Update modified existing record and modified node
                              was shadowed */
...
H5_DLL herr_t H5B2__split1(H5B2_hdr_t *hdr, uint16_t depth,
...
H5_DLL herr_t H5B2__redistribute3(H5B2_hdr_t *hdr, uint16_t depth,
                                  H5B2_internal_t *internal,
                                  unsigned *internal_flags_ptr, unsigned idx);
H5_DLL herr_t H5B2__merge2(H5B2_hdr_t *hdr, uint16_t depth,
                           H5B2_node_ptr_t *curr_node_ptr,
                           unsigned *parent_cache_info_flags_ptr,
                           H5B2_internal_t *internal,
                           unsigned *internal_flags_ptr, unsigned idx);
H5_DLL herr_t H5B2__merge3(H5B2_hdr_t *hdr, uint16_t depth,
                           H5B2_node_ptr_t *curr_node_ptr,
                           unsigned *parent_cache_info_flags_ptr,
                           H5B2_internal_t *internal,
                           unsigned *internal_flags_ptr, unsigned idx);
...
H5_DLL haddr_t H5B2__hdr_create(H5F_t *f, const H5B2_create_t *cparam,
                                void *ctx_udata);
...
H5_DLL herr_t H5B2__swap_leaf(H5B2_hdr_t *hdr, uint16_t depth,
                              H5B2_internal_t *internal,
                              unsigned *internal_flags_ptr, unsigned idx,
                              void *swap_loc);
...
H5_DLL herr_t H5B2__remove_internal(H5B2_hdr_t *hdr, hbool_t *depth_decreased,
                                    void *swap_loc, void *swap_parent,
                                    uint16_t depth,
...
H5_DLL herr_t H5B2__remove_internal_by_idx(
...
H5_DLL herr_t H5B2__hdr_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                              int fwidth, const H5B2_class_t *type,
                              haddr_t obj_addr);
...
H5_DLL herr_t H5B2__assert_leaf2(const H5B2_hdr_t *hdr, const H5B2_leaf_t *leaf,
