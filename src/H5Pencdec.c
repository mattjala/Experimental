...
herr_t H5P__encode_size_t(const void *value, void **_pp, size_t *size) {
  uint64_t enc_value =
      (uint64_t) * (const size_t *)value; /* Property value to encode */
  uint8_t **pp = (uint8_t **)_pp;
  unsigned enc_size =
      H5VM_limit_enc_size(enc_value); /* Size of encoded property */
...
herr_t H5P__encode_hsize_t(const void *value, void **_pp, size_t *size) {
  uint64_t enc_value =
      (uint64_t) * (const hsize_t *)value; /* Property value to encode */
  unsigned enc_size =
      H5VM_limit_enc_size(enc_value); /* Size of encoded property */
  uint8_t **pp = (uint8_t **)_pp;
...
static int H5P__encode_cb(H5P_genprop_t *prop, void *_udata) {
  H5P_enc_iter_ud_t *udata =
      (H5P_enc_iter_ud_t *)_udata; /* Pointer to user data */
  ...
  if (prop->encode) {
    ...
    *(udata->enc_size_ptr) += prop_name_len;
    ...
    if ((prop->encode)(prop->value, udata->pp, &prop_value_len) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTENCODE, H5_ITER_ERROR,
...
/*-------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5P__encode(const H5P_genplist_t *plist, hbool_t enc_all_prop, void *buf,
                   size_t *nalloc) {
...
hid_t H5P__decode(const void *buf) {
  ...
  if ((uint8_t)H5P_ENCODE_VERS != vers)
    HGOTO_ERROR(H5E_PLIST, H5E_VERSION, FAIL,
                "bad version # of encoded information, expected %u, got %u",
  ...
  while (p) {
    ...
    if (NULL == (prop = H5P__find_prop_plist(plist, name)))
      HGOTO_ERROR(H5E_PLIST, H5E_NOTFOUND, FAIL, "property doesn't exist: '%s'",
                  name);
    ...
    if (prop->size > value_buf_size) {
      if (NULL == (value_buf = H5MM_realloc(value_buf, prop->size)))
        HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, FAIL,
                    "decoding buffer allocation failed");
