...
static herr_t H5FS__sinfo_serialize_sect_cb(void *_item,
                                            void H5_ATTR_UNUSED *key,
                                            void *_udata);
static herr_t H5FS__sinfo_serialize_node_cb(void *_item,
                                            void H5_ATTR_UNUSED *key,
                                            void *_udata);

/* Metadata cache callbacks */
static herr_t H5FS__cache_hdr_get_initial_load_size(void *udata,
                                                    size_t *image_len);
static htri_t H5FS__cache_hdr_verify_chksum(const void *image_ptr, size_t len,
                                            void *udata_ptr);
...
static herr_t H5FS__cache_hdr_serialize(const H5F_t *f, void *image, size_t len,
                                        void *thing);
...
static herr_t H5FS__cache_sinfo_get_initial_load_size(void *udata,
                                                      size_t *image_len);
static htri_t H5FS__cache_sinfo_verify_chksum(const void *image_ptr, size_t len,
                                              void *udata_ptr);
...
static herr_t H5FS__cache_sinfo_notify(H5AC_notify_action_t action,
                                       void *thing);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__cache_hdr_get_initial_load_size(void *_udata,
                                                    size_t *image_len) {
  H5FS_hdr_cache_ud_t *udata =
      (H5FS_hdr_cache_ud_t *)_udata; /* User-data for metadata cache callback */
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5FS__cache_hdr_verify_chksum(const void *_image, size_t len,
 ...
 *-------------------------------------------------------------------------
 */
static void *H5FS__cache_hdr_deserialize(const void *_image,
                                         size_t H5_ATTR_NDEBUG_UNUSED len,
                                         void *_udata,
                                         hbool_t H5_ATTR_UNUSED *dirty) {
  H5FS_t *fspace = NULL; /* Free space header info */
  H5FS_hdr_cache_ud_t *udata =
      (H5FS_hdr_cache_ud_t *)_udata; /* User data for callback */
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
 ...
 *		since it is possible for the section info to either not be
 *allocated at all, or be allocated in temporary (AKA imaginary) files space, it
 *is possible for the above mentioned fields to contain giberish when the free
 *space manager header is serialized.
 *
 *		This function exists to prevent this problem.  It does so by
 *		forcing allocation of real file space for the section
 *information.
 *
 *		Note that in the Version 2 cache, this problem was dealt with by
 *		simply flushing the section info before flushing the header.
 *This was possible, since the clients handled file I/O directly.  As this
 *responsibility has moved to the cache in Version 3, this solution is no longer
 *directly applicable.
 *
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__cache_hdr_pre_serialize(
  ...
  if (fspace->sinfo) {
    ...
    if (fspace->serial_sect_count > 0 && H5_addr_defined(fspace->addr)) {
      ...
      if (!H5_addr_defined(fspace->sect_addr)) { /* case 1 */
        ...
        if (HADDR_UNDEF ==
            (sect_addr = H5MF_alloc((H5F_t *)f, H5FD_MEM_FSPACE_SINFO,
                                    fspace->sect_size)))
        ...
        if (fspace->sect_size > saved_sect_size) {
          ...
          if (H5MF_xfree(f, H5FD_MEM_FSPACE_SINFO, sect_addr, saved_sect_size) <
              0)
        ...
        /* Get the tag for this free space manager and use it to insert the
         * entry */
        if (H5AC_get_tag((const void *)fspace, &tag) < 0)
          HGOTO_ERROR(H5E_FSPACE, H5E_CANTTAG, FAIL,
                      "can't get tag for metadata cache object");
        H5_BEGIN_TAG(tag)
        if (H5AC_insert_entry((H5F_t *)f, H5AC_FSPACE_SINFO, fspace->sect_addr,
                              fspace->sinfo, H5AC__NO_FLAGS_SET) < 0)
          HGOTO_ERROR_TAG(H5E_FSPACE, H5E_CANTINIT, FAIL,
        ...
        if (HADDR_UNDEF ==
            (new_sect_addr = H5MF_alloc((H5F_t *)f, H5FD_MEM_FSPACE_SINFO,
                                        fspace->sect_size)))
          HGOTO_ERROR(H5E_FSPACE, H5E_NOSPACE, FAIL,
                      "file allocation failed for free space sections");
        ...
        if (H5AC_move_entry((H5F_t *)f, H5AC_FSPACE_SINFO, fspace->sect_addr,
                            new_sect_addr) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__cache_hdr_serialize(const H5F_t *f, void *_image,
                                        size_t H5_ATTR_NDEBUG_UNUSED len,
                                        void *_thing) {
  H5FS_t *fspace = (H5FS_t *)_thing;  /* Pointer to the object */
  ...
  case H5AC_NOTIFY_ACTION_ENTRY_DIRTIED:
    if (H5AC_unsettle_entry_ring(fspace) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTFLUSH, FAIL,
                  "unable to mark FSM ring as unsettled");
    break;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__cache_sinfo_get_initial_load_size(void *_udata,
                                                      size_t *image_len) {
  const H5FS_t *fspace; /* free space manager */
  H5FS_sinfo_cache_ud_t *udata =
      (H5FS_sinfo_cache_ud_t *)_udata; /* User data for callback */
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5FS__cache_sinfo_verify_chksum(const void *_image, size_t len,
                                       void H5_ATTR_UNUSED *_udata) {
  const uint8_t *image =
      (const uint8_t *)_image; /* Pointer into raw data buffer */
  uint32_t stored_chksum;      /* Stored metadata checksum value */
 ...
 */
static void *
H5FS__cache_sinfo_deserialize(const void *_image,
                              size_t H5_ATTR_NDEBUG_UNUSED len, void *_udata,
  ...
  if (fspace->serial_sect_count > 0) {
    ...
    hsize_t H5_ATTR_NDEBUG_UNUSED
        old_serial_sect_count; /* Total serializable section count from header
                                */
    hsize_t H5_ATTR_NDEBUG_UNUSED
        old_ghost_sect_count; /* Total ghost section count from header */
    hsize_t H5_ATTR_NDEBUG_UNUSED
        old_tot_space;      /* Total space managed from header */
    ...
    do {
      ...
      for (u = 0; u < node_count; u++) {
        ...
        haddr_t sect_addr =
            0; /* Address of free space section in the address space */
        ...
        if (NULL == (new_sect = (*fspace->sect_cls[sect_type].deserialize)(
                         &fspace->sect_cls[sect_type], image, sect_addr,
                         sect_size, &des_flags)))
        ...
        if (!(des_flags & H5FS_DESERIALIZE_NO_ADD))
          if (H5FS_sect_add(udata->f, fspace, new_sect, H5FS_ADD_DESERIALIZING,
                            udata) < 0)
      ...
      if (fspace->tot_sect_count == old_tot_sect_count)
        break;
             ...
             (((const uint8_t *)_image + old_sect_size) - H5FS_SIZEOF_CHKSUM));

    /* Sanity check */
    assert((size_t)(image - (const uint8_t *)_image) <=
           (old_sect_size - H5FS_SIZEOF_CHKSUM));
  ...
  assert((image == chksum_image) ||
         ((size_t)((image - (const uint8_t *)_image) +
                   (chksum_image - image)) == old_sect_size));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__cache_sinfo_image_len(const void *_thing,
                                          size_t *image_len) {
  ...
  /* Set the image length size */
  H5_CHECKED_ASSIGN(*image_len, size_t, sinfo->fspace->alloc_sect_size,
                    hsize_t);
 ...
 *		for the section info is located in temporary (AKA imaginary)
 *file space.  If it is, relocate file space for the section info to regular
 *file space.
 *
 * Return:	Success:	SUCCEED
 *		Failure:	FAIL
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__cache_sinfo_pre_serialize(
    H5F_t *f, void *_thing, haddr_t addr, size_t H5_ATTR_NDEBUG_UNUSED len,
  ...
  if (H5F_IS_TMP_ADDR(f, fspace->sect_addr)) {
    ...
    if (HADDR_UNDEF ==
        (sinfo_addr =
             H5MF_alloc((H5F_t *)f, H5FD_MEM_FSPACE_SINFO, fspace->sect_size)))
      HGOTO_ERROR(H5E_FSPACE, H5E_NOSPACE, FAIL,
                  "file allocation failed for free space sections");
    ...
    /* Let the metadata cache know the section info moved */
    if (H5AC_move_entry((H5F_t *)f, H5AC_FSPACE_SINFO, sinfo->fspace->sect_addr,
                        sinfo_addr) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_CANTMOVE, FAIL,
                  "unable to move section info");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FS__cache_sinfo_serialize(const H5F_t *f, void *_image,
                                          size_t len, void *_thing) {
  H5FS_sinfo_t *sinfo = (H5FS_sinfo_t *)_thing; /* Pointer to the object */
  ...
  *image++ = H5FS_SINFO_VERSION;
  ...
  for (bin = 0; bin < sinfo->nbins; bin++)
    ...
    if (sinfo->bins[bin].bin_list)
      ...
      if (H5SL_iterate(sinfo->bins[bin].bin_list, H5FS__sinfo_serialize_node_cb,
                       &udata) < 0)
  ...
  metadata_chksum = H5_checksum_metadata(
      _image, (size_t)(chksum_image - (uint8_t *)_image), 0);
  /* Metadata checksum */
  UINT32ENCODE(chksum_image, metadata_chksum);

  /* Sanity check */
  assert((chksum_image == image) ||
         ((size_t)((image - (uint8_t *)_image) + (chksum_image - image)) ==
          sinfo->fspace->sect_size));
...
herr_t H5FS__cache_sinfo_notify(H5AC_notify_action_t action, void *_thing) {
  ...
  if (sinfo->fspace->swmr_write) {
    ...
    case H5AC_NOTIFY_ACTION_AFTER_LOAD:
      ...
      if (H5FS__create_flush_depend((H5AC_info_t *)sinfo->fspace,
                                    (H5AC_info_t *)sinfo) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTDEPEND, FAIL,
    ...
    case H5AC_NOTIFY_ACTION_BEFORE_EVICT:
      ...
      if (H5FS__destroy_flush_depend((H5AC_info_t *)sinfo->fspace,
                                     (H5AC_info_t *)sinfo) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTUNDEPEND, FAIL,
    ...
    *(*udata->image)++ = (uint8_t)sect->type;
    ...
    if (sect_cls->serialize) {
      if ((*sect_cls->serialize)(sect_cls, sect, *udata->image) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTSERIALIZE, FAIL,
                    "can't synchronize section");
...
static herr_t H5FS__sinfo_serialize_node_cb(void *_item,
  ...
  if (fspace_node->serial_count > 0) {
    ...
    UINT64ENCODE_VAR(*udata->image, fspace_node->serial_count,
                     udata->sect_cnt_size);

    /* The size of the sections for this node */
    UINT64ENCODE_VAR(*udata->image, fspace_node->sect_size,
                     udata->sinfo->sect_len_size);

    /* Iterate through all the sections of this size */
    assert(fspace_node->sect_list);
    if (H5SL_iterate(fspace_node->sect_list, H5FS__sinfo_serialize_sect_cb,
                     udata) < 0)
      HGOTO_ERROR(H5E_FSPACE, H5E_BADITER, FAIL,
