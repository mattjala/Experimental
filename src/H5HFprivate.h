 ...
 *
 * Purpose:		Private header for library accessible fractal heap
 *routines.
 *
 *-------------------------------------------------------------------------
 */
...
typedef struct H5HF_dtable_cparam_t {
  ...
  size_t
      start_block_size; /* Starting block size for table (must be power of 2) */
  size_t
      max_direct_size; /* Maximum size of a direct block (must be power of 2) */
  unsigned max_index;  /* Maximum ID/offset for table (integer log2 of actual
                          value, ie. the # of bits  required) */
  unsigned
      start_root_rows; /* Starting number of rows for root indirect block */
...
typedef struct H5HF_create_t {
  H5HF_dtable_cparam_t
      managed; /* Mapped object doubling-table creation parameters */
  hbool_t
      checksum_dblocks;  /* Whether the direct blocks should be checksummed */
  ...
  /* (0 - make ID just large enough to hold length & offset of object in the
   * heap) */
  /* (1 - make ID just large enough to allow 'huge' objects to be accessed
   * directly) */
  /* (n - make ID 'n' bytes in size) */
  H5O_pline_t pline; /* I/O filter pipeline to apply to direct blocks & 'huge'
                        objects */
} H5HF_create_t;
...
typedef struct H5HF_stat_t {
  ...
  hsize_t
      man_iter_off; /* Offset of "new block" iterator in 'managed' heap space */
...
H5_DLL herr_t H5HF_insert(H5HF_t *fh, size_t size, const void *obj,
                          void *id /*out*/);
H5_DLL herr_t H5HF_get_obj_len(H5HF_t *fh, const void *id,
                               size_t *obj_len_p /*out*/);
H5_DLL herr_t H5HF_get_obj_off(H5HF_t *fh, const void *_id,
...
H5_DLL herr_t H5HF_hdr_debug(H5F_t *f, haddr_t addr, FILE *stream, int indent,
                             int fwidth);
H5_DLL herr_t H5HF_dblock_debug(H5F_t *f, haddr_t addr, FILE *stream,
                                int indent, int fwidth, haddr_t hdr_addr,
                                size_t nrec);
H5_DLL void H5HF_iblock_print(const H5HF_indirect_t *iblock,
                              hbool_t dump_internal, FILE *stream, int indent,
                              int fwidth);
H5_DLL herr_t H5HF_iblock_debug(H5F_t *f, haddr_t addr, FILE *stream,
                                int indent, int fwidth, haddr_t hdr_addr,
                                unsigned nrows);
