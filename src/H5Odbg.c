...
herr_t H5O__assert(const H5O_t *oh) {
  ...
  hdr_size = 0;
  meta_space = (size_t)H5O_SIZEOF_HDR(oh) +
               (size_t)(H5O_SIZEOF_CHKHDR_OH(oh) * (oh->nchunks - 1));
  mesg_space = 0;
  free_space = 0;
  ...
  for (u = 0; u < oh->nchunks; u++) {
    ...
    if (oh->version > H5O_VERSION_1) {
      ...
      assert(!memcmp(oh->chunk[u].image,
                     (u == 0 ? H5O_HDR_MAGIC : H5O_CHK_MAGIC),
                     H5_SIZEOF_MAGIC));
  ...
  for (u = 0, curr_msg = &oh->mesg[0]; u < oh->nmesgs; u++, curr_msg++) {
    uint8_t H5_ATTR_NDEBUG_UNUSED
        *curr_hdr;        /* Start of current message header */
    ...
    if (H5O_NULL_ID == curr_msg->type->id)
      ...
      hbool_t H5_ATTR_NDEBUG_UNUSED found_chunk =
          FALSE; /* Found a chunk that matches */
      ...
      for (v = 0; v < oh->nchunks; v++) {
        if (H5_addr_eq(cont->addr, oh->chunk[v].addr) &&
            cont->size == oh->chunk[v].size) {
    ...
    if (curr_msg->chunkno == 0)
      assert(curr_hdr >= oh->chunk[curr_msg->chunkno].image +
                             (H5O_SIZEOF_HDR(oh) - H5O_SIZEOF_CHKSUM_OH(oh)));
    else
      assert(curr_hdr >=
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_debug_id(unsigned type_id, H5F_t *f, const void *mesg, FILE *stream,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__debug_real(H5F_t *f, H5O_t *oh, haddr_t addr, FILE *stream,
                       int indent, int fwidth) {
  ...
  if (oh->version > H5O_VERSION_1) {
    ...
    fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
            ...
            (oh->flags & H5O_HDR_ATTR_STORE_PHASE_CHANGE) ? "Non-default"
                                                          : "Default");
    fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth, "Timestamps:",
  ...
  fprintf(stream, "%*s%-*s %zu (%zu)\n", indent, "", fwidth,
          "Number of chunks (allocated):", oh->nchunks, oh->alloc_nchunks);
  ...
  for (i = 0, chunk_total = 0; i < oh->nchunks; i++) {
    ...
    fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent + 3, "",
            MAX(0, fwidth - 3), "Address:", oh->chunk[i].addr);
  ...
  /* debug each message */
  if (NULL == (sequence = (unsigned *)H5MM_calloc(NELMTS(H5O_msg_class_g) *
                                                  sizeof(unsigned))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  for (i = 0, mesg_total = 0; i < oh->nmesgs; i++) {
    const H5O_msg_class_t
        *debug_type;  /* Type of message to use for callbacks */
    ...
    fprintf(stream, "%*s%-*s 0x%04x `%s' (%d)\n", indent + 3, "",
            MAX(0, fwidth - 3),
            "Message ID (sequence number):", (unsigned)(oh->mesg[i].type->id),
            oh->mesg[i].type->name, sequence[oh->mesg[i].type->id]++);
    fprintf(stream, "%*s%-*s %s\n", indent + 3, "", MAX(0, fwidth - 3),
    ...
    if (oh->mesg[i].flags) {
      ...
      HDcompile_assert(H5O_MSG_FLAG_BITS ==
                       (H5O_MSG_FLAG_CONSTANT | H5O_MSG_FLAG_SHARED |
    ...
    fprintf(stream, "%*s%-*s (%zu, %zu) bytes\n", indent + 3, "",
            MAX(0, fwidth - 3), "Raw message data (offset, size) in chunk:",
    ...
    if (debug_type->debug && oh->mesg[i].native != NULL)
      (debug_type->debug)(f, oh->mesg[i].native, stream, indent + 6,
                          MAX(0, fwidth - 6));
    else
