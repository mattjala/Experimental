...
static herr_t H5G__obj_compact_to_dense_cb(const void *_mesg, unsigned idx,
                                           void *_udata);
static herr_t H5G__obj_remove_update_linfo(const H5O_loc_t *oloc,
                                           H5O_linfo_t *linfo);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__obj_create(H5F_t *f, H5G_obj_create_t *gcrt_info,
                       H5O_loc_t *oloc /*out*/) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__obj_create_real(H5F_t *f, const H5O_ginfo_t *ginfo,
  ...
  hbool_t use_at_least_v18; /* Flag indicating the new group format should be
                               used */
  ...
  /* Check for using the latest version of the group format which is introduced
   * in v18 */
  /* (add more checks for creating "new format" groups when needed) */
  if ((H5F_LOW_BOUND(f) >= H5F_LIBVER_V18) || linfo->track_corder ||
      (pline && pline->nused))
  ...
  if (linfo->index_corder && !linfo->track_corder)
    HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                "must track creation order to create index for it");
  ...
  if (use_at_least_v18) {
    ...
    lnk.name = &null_char;
    link_size = H5O_msg_size_f(f, gcpl_id, H5O_LINK_ID, &lnk,
                               (size_t)ginfo->est_name_len);
    assert(link_size);
  ...
  if (use_at_least_v18) {
    ...
    if (H5O_msg_create(oloc, H5O_LINFO_ID, 0, H5O_UPDATE_TIME, (void *)linfo) <
        0)
  ...
  if (ret_value) {
    ...
    if (linfo->nlinks == HSIZET_MAX) {
      ...
      if (H5_addr_defined(linfo->fheap_addr)) {
        ...
        if (NULL ==
            (bt2_name = H5B2_open(grp_oloc->file, linfo->name_bt2_addr, NULL)))
        ...
        if (H5O_get_nlinks(grp_oloc, &linfo->nlinks) < 0)
          HGOTO_ERROR(H5E_SYM, H5E_CANTGET, FAIL,
                      "can't retrieve # of links for object");
      } /* end if */
    }   /* end if */
  }     /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__obj_compact_to_dense_cb(const void *_mesg,
                                           unsigned H5_ATTR_UNUSED idx,
                                           void *_udata) {
  const H5O_link_t *lnk = (const H5O_link_t *)_mesg; /* Pointer to link */
  H5G_obj_oh_it_ud1_t *udata =
      (H5G_obj_oh_it_ud1_t *)_udata; /* 'User data' passed in */
  ...
  if (H5G__dense_insert(udata->f, udata->linfo, lnk) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                "unable to insert link into dense storage");
...
static herr_t H5G__obj_stab_to_new_cb(const H5O_link_t *lnk, void *_udata) {
  H5G_obj_stab_it_ud1_t *udata =
      (H5G_obj_stab_it_ud1_t *)_udata; /* 'User data' passed in */
  herr_t ret_value = H5_ITER_CONT;     /* Return value */
  ...
  if (H5G_obj_insert(udata->grp_oloc, lnk->name, (H5O_link_t *)lnk, FALSE,
                     H5O_TYPE_UNKNOWN, NULL) < 0)
    HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, H5_ITER_ERROR,
                "can't insert link into group");
  H5_GCC_CLANG_DIAG_ON("cast-qual")
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_obj_insert(const H5O_loc_t *grp_oloc, const char *name,
                      H5O_link_t *obj_lnk, hbool_t adj_link,
  ...
  hbool_t use_old_format;    /* Whether to use 'old format' (symbol table) for
                                insertions or not */
  hbool_t use_new_dense =
      FALSE; /* Whether to use "dense" form of 'new format' group */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (linfo_exists) {
    ...
    if ((link_msg_size = H5O_msg_raw_size(grp_oloc->file, H5O_LINK_ID, FALSE,
                                          obj_lnk)) == 0)
     ...
     *  header message, convert to using dense link storage instead of link
     * messages)
     */
    if (H5_addr_defined(linfo.fheap_addr))
      use_new_dense = TRUE;
    else if (linfo.nlinks < ginfo.max_compact &&
             link_msg_size < H5O_MESG_MAX_SIZE)
      use_new_dense = FALSE;
    else {
      ...
      if (H5G__dense_create(grp_oloc->file, &linfo, pline) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL,
                    "unable to create 'dense' form of new format group");
      ...
      /* Iterate over the 'link' messages, inserting them into the dense link
       * storage  */
      ...
      if (H5O_msg_remove(grp_oloc, H5O_LINK_ID, H5O_ALL, FALSE) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                    "unable to delete link messages");
    ...
    if (obj_lnk->cset != H5T_CSET_ASCII ||
        obj_lnk->type > H5L_TYPE_BUILTIN_MAX) {
      ...
      /* Insert group info message */
      if (H5O_msg_create(grp_oloc, H5O_GINFO_ID, H5O_MSG_FLAG_CONSTANT,
                         H5O_UPDATE_TIME, &new_ginfo) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL, "can't create message");

      /* Set up user data for iteration */
      udata.grp_oloc = grp_oloc;

      /* Iterate through all links in "old format" group and insert them into
       * new format */
      if (H5G__stab_iterate(grp_oloc, H5_ITER_NATIVE, (hsize_t)0, NULL,
                            H5G__obj_stab_to_new_cb, &udata) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTNEXT, FAIL,
                    "error iterating over old format links");

      /* Remove the symbol table message from the group */
      if (H5O_msg_remove(grp_oloc, H5O_STAB_ID, 0, FALSE) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                    "unable to delete old format link storage");
      ...
      if (H5G_obj_insert(grp_oloc, name, obj_lnk, adj_link, obj_type,
                         crt_info) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                    "unable to insert link into group");
  ...
  if (use_old_format) {
    ...
    if (H5G__stab_insert(grp_oloc, name, obj_lnk, obj_type, crt_info) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                  "unable to insert entry into symbol table");
  } /* end if */
  else {
    if (use_new_dense) {
      ...
      if (H5G__compact_insert(grp_oloc, obj_lnk) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTINSERT, FAIL,
                    "unable to insert link as link message");
    } /* end else */
  }   /* end else */
  ...
  if (!use_old_format) {
    ...
    if (H5O_msg_write(grp_oloc, H5O_LINFO_ID, 0, H5O_UPDATE_TIME, &linfo) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                  "can't update link info message");
  } /* end if */
  ...
  if (adj_link && obj_lnk->type == H5L_TYPE_HARD) {
    ...
    if (H5O_link(&obj_oloc, 1) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_LINKCOUNT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__obj_iterate(const H5O_loc_t *grp_oloc, H5_index_t idx_type,
  ...
  if (linfo_exists) {
    ...
    /* Check for creation order tracking, if creation order index lookup
     * requested */
    if (idx_type == H5_INDEX_CRT_ORDER) {
      /* Check if creation order is tracked */
      if (!linfo.track_corder)
        HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL,
                    "creation order not tracked for links in group");
    } /* end if */

    if (H5_addr_defined(linfo.fheap_addr)) {
      /* Iterate over the links in the group, building a table of the link
       * messages */
      if ((ret_value = H5G__dense_iterate(grp_oloc->file, &linfo, idx_type,
                                          order, skip, last_lnk, op, op_data)) <
          0)
        HGOTO_ERROR(H5E_SYM, H5E_BADITER, FAIL,
                    "can't iterate over dense links");
    } /* end if */
    else {
      /* Get the object's name from the link messages */
      if ((ret_value = H5G__compact_iterate(grp_oloc, &linfo, idx_type, order,
                                            skip, last_lnk, op, op_data)) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_BADITER, FAIL,
                    "can't iterate over compact links");
    } /* end else */
  }   /* end if */
  else {
    ...
    if (idx_type != H5_INDEX_NAME)
      HGOTO_ERROR(H5E_SYM, H5E_BADVALUE, FAIL,
                  "no creation order index to query");

    /* Iterate over symbol table */
    if ((ret_value = H5G__stab_iterate(grp_oloc, order, skip, last_lnk, op,
                                       op_data)) < 0)
      HGOTO_ERROR(H5E_SYM, H5E_BADITER, FAIL,
                  "can't iterate over symbol table");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_obj_get_name_by_idx(const H5O_loc_t *oloc, H5_index_t idx_type,
                               H5_iter_order_t order, hsize_t n, char *name,
  ...
  if (linfo_exists) {
    ...
    if (H5_addr_defined(linfo.fheap_addr)) {
      ...
      if (H5G__dense_get_name_by_idx(oloc->file, &linfo, idx_type, order, n,
                                     name, name_size, name_len) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't locate name");
    } /* end if */
    else {
      /* Get the object's name from the link messages */
      if (H5G__compact_get_name_by_idx(oloc, &linfo, idx_type, order, n, name,
                                       name_size, name_len) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't locate name");
    } /* end else */
  }   /* end if */
  else {
    ...
    if (H5G__stab_get_name_by_idx(oloc, order, n, name, name_size, name_len) <
        0)
      HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't locate name");
  } /* end else */
  ...
  if (H5_addr_defined(linfo->fheap_addr)) {
    ...
    if (linfo->nlinks == 0) {
      ...
      if (H5G__dense_delete(oloc->file, linfo, FALSE) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_CANTDELETE, FAIL,
                    "unable to delete dense link storage");
    } /* end if */
    /* Check for switching back to compact storage */
      ...
      if (linfo->nlinks < ginfo.min_dense) {
        ...
        hbool_t can_convert =
            TRUE; /* Whether converting to link messages is possible */
        size_t u; /* Local index */

        /* Build the table of links for this group */
        if (H5G__dense_build_table(oloc->file, linfo, H5_INDEX_NAME,
                                   H5_ITER_NATIVE, &ltable) < 0)
          HGOTO_ERROR(H5E_SYM, H5E_CANTNEXT, FAIL,
                      "error iterating over links");

        /* Pin the object header */
        if (NULL == (oh = H5O_pin(oloc)))
          HGOTO_ERROR(H5E_SYM, H5E_CANTPIN, FAIL,
                      "unable to pin group object header");
        ...
        for (u = 0; u < linfo->nlinks; u++)
          if (H5O_msg_size_oh(oloc->file, oh, H5O_LINK_ID, &(ltable.lnks[u]),
        ...
        if (can_convert) {
          ...
          for (u = 0; u < linfo->nlinks; u++)
            if (H5O_msg_append_oh(oloc->file, oh, H5O_LINK_ID, 0,
                                  H5O_UPDATE_TIME, &(ltable.lnks[u])) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_obj_remove(const H5O_loc_t *oloc, H5RS_str_t *grp_full_path_r,
                      const char *name) {
  H5O_linfo_t linfo;          /* Link info message            */
  htri_t linfo_exists;        /* Whether the link info message exists */
  hbool_t use_old_format;     /* Whether to use 'old format' (symbol table) for
 ...
 *
 * Purpose:     Remove a link from a group, according to the order within an
 *index.
 *
 * Return:	Success:        Non-negative
 *		Failure:	Negative
...
herr_t H5G_obj_remove_by_idx(const H5O_loc_t *grp_oloc,
  ...
  if (linfo_exists) {
    ...
    if (H5_addr_defined(linfo.fheap_addr)) {
      /* Remove object from the dense link storage */
      if (H5G__dense_remove_by_idx(grp_oloc->file, &linfo, grp_full_path_r,
                                   idx_type, order, n) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't remove object");
    } /* end if */
    else {
      /* Remove object from compact link storage */
      if (H5G__compact_remove_by_idx(grp_oloc, &linfo, grp_full_path_r,
                                     idx_type, order, n) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't remove object");
    } /* end else */
  }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G__obj_lookup(const H5O_loc_t *grp_oloc, const char *name,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5G_obj_lookup_by_idx(const H5O_loc_t *grp_oloc, H5_index_t idx_type,
                             H5_iter_order_t order, hsize_t n,
                             H5O_link_t *lnk) {
  H5O_linfo_t linfo;          /* Link info message */
  htri_t linfo_exists;        /* Whether the link info message exists */
  ...
  if (linfo_exists) {
    ...
    if (H5_addr_defined(linfo.fheap_addr)) {
      ...
      if (H5G__dense_lookup_by_idx(grp_oloc->file, &linfo, idx_type, order, n,
                                   lnk) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't locate object");
    } /* end if */
    else {
      /* Get the link from the link messages */
      if (H5G__compact_lookup_by_idx(grp_oloc, &linfo, idx_type, order, n,
                                     lnk) < 0)
        HGOTO_ERROR(H5E_SYM, H5E_NOTFOUND, FAIL, "can't locate object");
    } /* end else */
  }   /* end if */
