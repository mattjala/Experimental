  ...
  if (shared->type == H5O_SHARE_TYPE_SOHM) {
    ...
    if (H5SM_get_fheap_addr(f, type->id, &fheap_addr) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                  "can't get fheap address for shared messages");
    ...
    if (H5HF_get_obj_len(fheap, &(shared->u.heap_id), &mesg_size) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTGET, NULL,
                  "can't get message size from fractal heap.");
    ...
    /* Decode the message */
    if (NULL == (ret_value = (type->decode)(f, open_oh, 0, ioflags, mesg_size,
                                            mesg_ptr)))
...
static herr_t H5O__shared_link_adj(H5F_t *f, H5O_t *open_oh,
  ...
  if (shared->type == H5O_SHARE_TYPE_COMMITTED) {
    ...
    /*if(shared->file->shared != f->shared)
        HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "interfile hard links are not
       allowed")*/
    ...
    assert(shared->type == H5O_SHARE_TYPE_SOHM ||
    ...
    if (adjust < 0) {
      if (H5SM_delete(f, open_oh, shared) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CANTDEC, FAIL,
                    "unable to delete message from SOHM table");
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
void *H5O__shared_decode(H5F_t *f, H5O_t *open_oh, unsigned *ioflags,
  ...
  if (NULL ==
      (ret_value = H5O__shared_read(f, open_oh, ioflags, &sh_mesg, type)))
    HGOTO_ERROR(H5E_OHDR, H5E_READERROR, NULL,
                "unable to retrieve native message");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__shared_encode(const H5F_t *f, uint8_t *buf /*out*/,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__shared_delete(H5F_t *f, H5O_t *open_oh, const H5O_msg_class_t *type,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__shared_link(H5F_t *f, H5O_t *open_oh, const H5O_msg_class_t *type,
                        H5O_shared_t *sh_mesg) {
  herr_t ret_value = SUCCEED; /* Return value */
...
herr_t H5O__shared_copy_file(H5F_t H5_ATTR_NDEBUG_UNUSED *file_src,
                             H5F_t *file_dst, const H5O_msg_class_t *mesg_type,
  ...
  const H5O_shared_t *shared_src = (const H5O_shared_t *)
      _native_src; /* Alias to shared info in native source */
  H5O_shared_t *shared_dst = (H5O_shared_t *)
      _native_dst; /* Alias to shared info in native destination message */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (shared_src->type != H5O_SHARE_TYPE_COMMITTED) {
    ...
    if (H5SM_try_share(file_dst, NULL, H5SM_DEFER, mesg_type->id, _native_dst,
                       mesg_flags) < 0)
     ...
     */
    H5O_UPDATE_SHARED(shared_dst, H5O_SHARE_TYPE_COMMITTED, file_dst,
                      mesg_type->id, 0, HADDR_UNDEF)
 ...
 *              The function is needed at cases such as copying a shared reg_ref
 *attribute. When a shared reg_ref attribute is copied from one file to another,
 *the values in file need to be replaced. The only way to complish that is to
 *delete the old message and write the new message with the correct values.
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
herr_t H5O__shared_post_copy_file(H5F_t *f, const H5O_msg_class_t *mesg_type,
  ...
  if (shared_src->type == H5O_SHARE_TYPE_COMMITTED) {
    ...
    if (H5O_copy_header_map(&src_oloc, &dst_oloc, cpy_info, FALSE, NULL, NULL) <
        0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL, "unable to copy object");

    /* Set up destination message's shared info */
    H5O_UPDATE_SHARED(shared_dst, H5O_SHARE_TYPE_COMMITTED, f, mesg_type->id, 0,
                      dst_oloc.addr)
  } /* end if */
  else
    /* Share the message */
    if (H5SM_try_share(f, NULL, H5SM_WAS_DEFERRED, mesg_type->id, shared_dst,
                       mesg_flags) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__shared_debug(const H5O_shared_t *mesg, FILE *stream, int indent,
                         int fwidth) {
  FUNC_ENTER_PACKAGE_NOERR
  ...
  case H5O_SHARE_TYPE_UNSHARED:
    fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
            "Shared Message type:", "Unshared");
    break;

  case H5O_SHARE_TYPE_COMMITTED:
    fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
            "Shared Message type:", "Obj Hdr");
  ...
  case H5O_SHARE_TYPE_HERE:
    fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
            "Shared Message type:", "Here");
    break;

  default:
    fprintf(stream, "%*s%-*s %s (%u)\n", indent, "", fwidth,
            "Shared Message type:", "Unknown", (unsigned)mesg->type);
  } /* end switch */
