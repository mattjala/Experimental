...
static void *H5O__dtype_copy_file(H5F_t *file_src,
                                  ...
                                  void *native_src, H5F_t *file_dst,
                                  hbool_t *recompute_size, H5O_copy_t *cpy_info,
                                  void *udata);
...
#define H5O_DTYPE_CHECK_VERSION(DT, VERS, MIN_VERS, IOF, CLASS, ERR)           \
  if (((VERS) < (MIN_VERS)) && !(*(IOF)&H5O_DECODEIO_NOCHANGE))                \
    HGOTO_ERROR(H5E_DATATYPE, H5E_VERSION, ERR,                                \
                "incorrect " CLASS " datatype version");
  ...
  if (((VERS) < (MIN_VERS)) && !(*(IOF)&H5O_DECODEIO_NOCHANGE)) {              \
    ...
    if (H5T__upgrade_version((DT), (VERS)) < 0)                                \
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTSET, FAIL,                             \
                  "can't upgrade " CLASS " encoding version");                 \
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5O__dtype_decode_helper(unsigned *ioflags /*in,out*/,
                                       const uint8_t **pp, H5T_t *dt,
  ...
  if (version < H5O_DTYPE_VERSION_1 || version > H5O_DTYPE_VERSION_LATEST)
    HGOTO_ERROR(H5E_DATATYPE, H5E_CANTLOAD, FAIL,
                "bad version number for datatype message");
    ...
    if (version >= H5O_DTYPE_VERSION_3) {
      ...
      if ((flags & 0x40) && !(flags & 0x1))
        HGOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL,
                    "bad byte order for datatype message");
  ...
  case H5T_OPAQUE: {
    ...
    if (0 != (z & 0x7))
      HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, FAIL,
                  "opaque flag field must be aligned");
  ...
  case H5T_COMPOUND: {
    ...
    if (nmembs == 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_BADVALUE, FAIL,
                  "invalid number of members: %u", nmembs);
    if (NULL == (dt->shared->u.compnd.memb =
                     (H5T_cmemb_t *)H5MM_calloc(nmembs * sizeof(H5T_cmemb_t))))
    ...
    if (dt->shared->u.compnd.memb_size != 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_BADVALUE, FAIL,
                  "member size not initialized to zero");
    ...
    for (dt->shared->u.compnd.nmembs = 0; dt->shared->u.compnd.nmembs < nmembs;
      ...
      if (!skip) {
        ...
        if (actual_name_length == max)
          HGOTO_ERROR(H5E_OHDR, H5E_NOSPACE, FAIL,
                      "field name not null terminated");
      } else {
      ...
      /* Version 3 of the datatype message eliminated the padding to multiple of
       * 8 bytes */
      if (version >= H5O_DTYPE_VERSION_3) {
        /* Advance past name, including null terminator */
        if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, *pp, actual_name_length + 1,
                                        p_end))
          HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                      "ran off end of input buffer while decoding");
        *pp += actual_name_length + 1;
        ...
        if (H5_IS_KNOWN_BUFFER_OVERFLOW(
                skip, *pp, ((actual_name_length + 8) / 8) * 8, p_end))
          HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                      "ran off end of input buffer while decoding");
      ...
      /* (starting with version 3 of the datatype message, use the minimum # of
       * bytes required) */
      if (version >= H5O_DTYPE_VERSION_3) {
        if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, *pp, offset_nbytes, p_end))
          HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                      "ran off end of input buffer while decoding");
        UINT32DECODE_VAR(
            *pp, dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset,
            offset_nbytes);
      } else {
        if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, *pp, 4, p_end))
          HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
      ...
      if ((can_upgrade = H5O__dtype_decode_helper(ioflags, pp, temp_type, skip,
                                                  p_end)) < 0) {
        dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].name =
            H5MM_xfree(
                dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].name);
      ...
      /* Go create the array datatype now, for older versions of the datatype
       * message */
      if (version == H5O_DTYPE_VERSION_1) {
        ...
        if (ndims > 0) {
          ...
          if ((array_dt = H5T__array_create(temp_type, ndims, dim)) == NULL) {
            dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs]
                .name = H5MM_xfree(
                dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].name);
            if (H5T_close_real(temp_type) < 0)
              HDONE_ERROR(H5E_DATATYPE, H5E_CANTRELEASE, FAIL,
      ...
      if (dt->shared->u.compnd.nmembs > 0 &&
        ...
        for (unsigned u = 0; u < dt->shared->u.compnd.nmembs; u++)
          if ((dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset >=
               ...
               dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset <
                   (dt->shared->u.compnd.memb[u].offset +
                    dt->shared->u.compnd.memb[u].size)) ||
              (dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset <
                   dt->shared->u.compnd.memb[u].offset &&
               (dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].offset +
                dt->shared->u.compnd.memb[dt->shared->u.compnd.nmembs].size) >
    ...
    if (version < upgrade_to) {
      ...
      if (H5T__upgrade_version(dt, upgrade_to) < 0)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTSET, FAIL,
                    "can't upgrade compound encoding version");
  ...
  case H5T_REFERENCE:
    ...
    if (dt->shared->u.atomic.u.r.rtype == H5R_OBJECT2 ||
      ...
      if (dt->shared->u.atomic.u.r.version != H5R_ENCODE_VERSION)
        HGOTO_ERROR(H5E_DATATYPE, H5E_CANTDECODE, FAIL,
                    "reference version does not match");
    } else
  ...
  case H5T_ENUM: {
    ...
    if (NULL == (dt->shared->parent = H5T__alloc()))
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                  "can't allocate parent datatype");
    if (H5O__dtype_decode_helper(ioflags, pp, dt->shared->parent, skip, p_end) <
        0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTDECODE, FAIL,
                  "unable to decode parent datatype");
    ...
    for (dt->shared->u.enumer.nmembs = 0; dt->shared->u.enumer.nmembs < nmembs;
      ...
      if (NULL == (dt->shared->u.enumer.name[dt->shared->u.enumer.nmembs] =
                       H5MM_xstrdup((const char *)*pp)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTCOPY, FAIL,
                    "can't duplicate enum name string");
    ...
    if (dt->shared->u.enumer.nmembs != nmembs)
      HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                  "incorrect number of enum members decoded");

    /* Values */
    if (H5_IS_KNOWN_BUFFER_OVERFLOW(
            skip, *pp, nmembs * dt->shared->parent->shared->size, p_end))
      HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                  "ran off end of input buffer while decoding");
  ...
  case H5T_ARRAY:
    ...
    if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, *pp, (dt->shared->u.array.ndims * 4),
                                    p_end))
      HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                  "ran off end of input buffer while decoding");
    ...
    if (version < H5O_DTYPE_VERSION_3) {
      if (H5_IS_KNOWN_BUFFER_OVERFLOW(skip, *pp,
                                      (dt->shared->u.array.ndims * 4), p_end))
        HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, FAIL,
                    "ran off end of input buffer while decoding");
    ...
    if (H5O__dtype_decode_helper(ioflags, pp, dt->shared->parent, skip, p_end) <
        0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTDECODE, FAIL,
                  "unable to decode array parent type");
    ...
    H5O_DTYPE_CHECK_VERSION(dt, version, H5O_DTYPE_VERSION_2, ioflags, "array",
                            FAIL)
  ...
  /* Cleanup on error */
  if (ret_value < 0)
    /* Release (reset) dt but do not free it - leave it as an empty datatype as
     * was the case on function entry */
    if (H5T__free(dt) < 0)
      HDONE_ERROR(H5E_DATATYPE, H5E_CANTRELEASE, FAIL,
                  "can't release datatype info");
...
static herr_t H5O__dtype_encode_helper(uint8_t **pp, const H5T_t *dt) {
  ...
  *pp += 4;
  ...
  case H5T_FLOAT:
    ...
    case H5T_ORDER_VAX: /*turn on 1st and 6th (reserved before adding VAX)
                           bits*/
      flags |= 0x41;
  ...
  case H5T_STRING:
    ...
    flags =
        (unsigned)(flags |
                   ((((unsigned)dt->shared->u.atomic.u.s.cset) & 0x0f) << 4));
    break;
  ...
  case H5T_COMPOUND: {
    ...
    for (i = 0; i < dt->shared->u.compnd.nmembs; i++) {
      ...
      assert(dt->shared->version >=
             dt->shared->u.compnd.memb[i].type->shared->version);

      /* Name */
      HDstrcpy((char *)(*pp), dt->shared->u.compnd.memb[i].name);

      /* Version 3 of the datatype message removed the padding to multiple of 8
       * bytes */
       ...
       * bytes required) */
      if (dt->shared->version >= H5O_DTYPE_VERSION_3)
        UINT32ENCODE_VAR(*pp, (uint32_t)dt->shared->u.compnd.memb[i].offset,
                         offset_nbytes);
      else
  ...
  case H5T_REFERENCE:
    ...
    if (dt->shared->u.atomic.u.r.opaque)
      flags = (unsigned)(flags |
                         (((unsigned)dt->shared->u.atomic.u.r.version & 0x0f)
                          << 4));
    break;
  ...
  case H5T_ENUM:
    ...
    if (H5O__dtype_encode_helper(pp, dt->shared->parent) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTENCODE, FAIL,
                  "unable to encode parent datatype");
  ...
  case H5T_VLEN: /* Variable length datatypes...  */
    ...
    if (dt->shared->u.vlen.type == H5T_VLEN_STRING) {
      flags =
          (unsigned)(flags | (((unsigned)dt->shared->u.vlen.pad & 0x0f) << 4));
      flags =
          (unsigned)(flags | (((unsigned)dt->shared->u.vlen.cset & 0x0f) << 8));
    } /* end if */

    /* Encode base type of VL information */
    if (H5O__dtype_encode_helper(pp, dt->shared->parent) < 0)
      HGOTO_ERROR(H5E_DATATYPE, H5E_CANTENCODE, FAIL,
                  "unable to encode VL parent type");
    break;
  ...
  *hdr++ = (uint8_t)(((unsigned)(dt->shared->type) & 0x0f) |
                     (dt->shared->version << 4));
...
/*--------------------------------------------------------------------------
 ...
 DESCRIPTION
        ...
        into a struct in memory native format.  The struct is allocated within
this function using malloc() and is returned to the caller.
--------------------------------------------------------------------------*/
static void *H5O__dtype_decode(H5F_t H5_ATTR_UNUSED *f,
                               H5O_t H5_ATTR_UNUSED *open_oh,
                               unsigned H5_ATTR_UNUSED mesg_flags,
                               unsigned *ioflags /*in,out*/, size_t p_size,
                               const uint8_t *p) {
  hbool_t skip;
  H5T_t *dt = NULL;
...
--------------------------------------------------------------------------*/
static herr_t H5O__dtype_encode(H5F_t H5_ATTR_UNUSED *f, uint8_t *p,
                                const void *mesg) {
...
static size_t H5O__dtype_size(const H5F_t *f, const void *_mesg) {
  ...
  case H5T_OPAQUE:
    ret_value +=
        (HDstrlen(dt->shared->u.opaque.tag) + 7) & (H5T_OPAQUE_TAG_MAX - 8);
    break;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__dtype_set_share(void *_mesg /*in,out*/,
  ...
  if (H5O_set_shared(&(dt->sh_loc), sh) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_CANTCOPY, FAIL,
                "unable to copy shared message info");
...
static htri_t H5O__dtype_can_share(const void *_mesg) {
  ...
  if ((tri_ret = H5T_is_immutable(mesg)) > 0)
    ...
    HGOTO_ERROR(H5E_OHDR, H5E_BADTYPE, FAIL,
                "can't tell if datatype is immutable");
  ...
  if ((tri_ret = H5T_is_named(mesg)) > 0)
    ...
    HGOTO_ERROR(H5E_OHDR, H5E_BADTYPE, FAIL,
                "can't tell if datatype is shared");
 ...
 */
static herr_t H5O__dtype_pre_copy_file(H5F_t *file_src, const void *mesg_src,
                                       hbool_t H5_ATTR_UNUSED *deleted,
                                       const H5O_copy_t *cpy_info,
                                       void *_udata) {
  const H5T_t *dt_src = (const H5T_t *)mesg_src; /* Source datatype */
  H5D_copy_file_ud_t *udata =
      (H5D_copy_file_ud_t *)_udata; /* Dataset copying user data */
  herr_t ret_value = SUCCEED;       /* Return value */
  ...
  /* Check to ensure that the version of the message to be copied does not
     exceed the message version as indicated by the destination file's high
     bound */
  if (dt_src->shared->version >
      H5O_dtype_ver_bounds[H5F_HIGH_BOUND(cpy_info->file_dst)])
    HGOTO_ERROR(H5E_OHDR, H5E_BADRANGE, FAIL,
                "datatype message version out of bounds");
  ...
  if (udata) {
    ...
    /* Set the location of the source datatype to describe the disk form of the
     * data */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__dtype_shared_post_copy_upd(
    const H5O_loc_t H5_ATTR_UNUSED *src_oloc,
    const void H5_ATTR_UNUSED *mesg_src, H5O_loc_t H5_ATTR_UNUSED *dst_oloc,
    void *mesg_dst, H5O_copy_t H5_ATTR_UNUSED *cpy_info) {
...
/*--------------------------------------------------------------------------
 ...
 DESCRIPTION
    ...
    parameter.
--------------------------------------------------------------------------*/
static herr_t H5O__dtype_debug(H5F_t *f, const void *mesg, FILE *stream,
                               int indent, int fwidth) {
  const H5T_t *dt = (const H5T_t *)mesg;
  const char *s;
  char buf[256];
  ...
  fprintf(stream, "%*s%-*s %lu byte%s\n", indent, "", fwidth,
          "Size:", (unsigned long)(dt->shared->size),
          1 == dt->shared->size ? "" : "s");
  ...
  if (H5T_COMPOUND == dt->shared->type) {
    ...
    for (i = 0; i < dt->shared->u.compnd.nmembs; i++) {
      ...
      H5O__dtype_debug(f, dt->shared->u.compnd.memb[i].type, stream, indent + 3,
                       MAX(0, fwidth - 3));
    } /* end for */
  }   /* end if */
  else if (H5T_ENUM == dt->shared->type) {
    fprintf(stream, "%*s%s\n", indent, "", "Base type:");
    H5O__dtype_debug(f, dt->shared->parent, stream, indent + 3,
                     MAX(0, fwidth - 3));
    fprintf(stream, "%*s%-*s %u\n", indent, "", fwidth,
    ...
    case H5T_CSET_RESERVED_15:
      HDsnprintf(buf, sizeof(buf), "H5T_CSET_RESERVED_%d",
    ...
    case H5T_STR_RESERVED_15:
      HDsnprintf(buf, sizeof(buf), "H5T_STR_RESERVED_%d",
    ...
    if (H5T_FLOAT == dt->shared->type) {
      ...
      default:
        if (dt->shared->u.atomic.u.f.pad < 0)
          HDsnprintf(buf, sizeof(buf), "H5T_PAD_%d",
                     -(dt->shared->u.atomic.u.f.pad));
        else
      ...
      case H5T_NSGN:
      default:
        HDsnprintf(buf, sizeof(buf), "H5T_SGN_%d",
                   (int)(dt->shared->u.atomic.u.i.sign));
        s = buf;
        break;
      } /* end switch */
