...
static void *H5O__link_decode(H5F_t *f, H5O_t *open_oh, unsigned mesg_flags,
                              ...
                              const uint8_t *p);
static herr_t H5O__link_encode(H5F_t *f, hbool_t disable_shared, uint8_t *p,
                               const void *_mesg);
static void *H5O__link_copy(const void *_mesg, void *_dest);
static size_t H5O__link_size(const H5F_t *f, hbool_t disable_shared,
                             const void *_mesg);
...
#define H5O_LINK_STORE_LINK_TYPE                                               \
  0x08 /* Whether to store non-default link type */
#define H5O_LINK_STORE_NAME_CSET                                               \
  0x10 /* Whether to store non-default name character set */
#define H5O_LINK_ALL_FLAGS                                                     \
  (H5O_LINK_NAME_SIZE | H5O_LINK_STORE_CORDER | H5O_LINK_STORE_LINK_TYPE |     \
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__link_decode(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh,
  ...
  default:
    HGOTO_ERROR(H5E_OHDR, H5E_BADVALUE, NULL,
                "no appropriate size for name length");
  }
  if (len == 0)
  ...
  default:
    ...
    if (lnk->type == H5L_TYPE_EXTERNAL && len < 3)
      HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                  "external link information length < 3");
    lnk->u.ud.size = len;
    if (len > 0) {
      if (H5_IS_BUFFER_OVERFLOW(p, len, p_end))
        HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
  ...
  if (!ret_value && lnk) {
    ...
    if (lnk->type >= H5L_TYPE_UD_MIN && lnk->u.ud.size > 0 &&
        lnk->u.ud.udata != NULL)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__link_encode(H5F_t *f, hbool_t H5_ATTR_UNUSED disable_shared,
  ...
  *p++ = H5O_LINK_VERSION;
  ...
  link_flags = (unsigned char)(link_flags |
                               (lnk->corder_valid ? H5O_LINK_STORE_CORDER : 0));
  link_flags = (unsigned char)(link_flags | ((lnk->type != H5L_TYPE_HARD)
                                                 ? H5O_LINK_STORE_LINK_TYPE
                                                 : 0));
  link_flags = (unsigned char)(link_flags | ((lnk->cset != H5T_CSET_ASCII)
                                                 ? H5O_LINK_STORE_NAME_CSET
                                                 : 0));
  *p++ = link_flags;
  ...
  *dest = *lnk;
  ...
  if (lnk->type == H5L_TYPE_SOFT) {
    if (NULL == (dest->u.soft.name = H5MM_xstrdup(lnk->u.soft.name)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                  "can't duplicate soft link value");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__link_size(const H5F_t *f,
...
herr_t H5O_link_delete(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh, void *_mesg) {
  H5O_link_t *lnk = (H5O_link_t *)_mesg;
  hid_t file_id =
      -1; /* ID for the file the link is located in (passed to user callback) */
  ...
  if (lnk->type == H5L_TYPE_HARD) {
    ...
    /* Construct object location for object, in order to decrement it's ref
     * count */
    H5O_loc_reset(&oloc);
    oloc.file = f;
  ...
  /* Perform the "delete" callback when a user-defined link is removed */
    ...
    if (NULL == (link_class = H5L_find_class(lnk->type)))
      HGOTO_ERROR(H5E_OHDR, H5E_NOTREGISTERED, FAIL,
    ...
    if (link_class->del_func) {
      ...
      if ((link_class->del_func)(lnk->name, file_id, lnk->u.ud.udata,
                                 lnk->u.ud.size) < 0)
        HGOTO_ERROR(H5E_OHDR, H5E_CALLBACK, FAIL,
                    "link deletion callback returned failure");
    } /* end if */
  }   /* end if */
...
static herr_t H5O__link_post_copy_file(const H5O_loc_t *src_oloc,
  ...
  if (H5L__link_copy_file(dst_oloc->file, link_src, src_oloc, link_dst,
                          cpy_info) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__link_debug(H5F_t H5_ATTR_UNUSED *f, const void *_mesg,
          ...
          (lnk->type == H5L_TYPE_HARD
               ...
               : (lnk->type == H5L_TYPE_SOFT
                      ...
                      : (lnk->type == H5L_TYPE_EXTERNAL
                             ? "External"
                             : (lnk->type >= H5L_TYPE_UD_MIN ? "User-defined"
                                                             : "Unknown")))));
  ...
  fprintf(stream, "%*s%-*s %s\n", indent, "", fwidth,
          ...
          (lnk->cset == H5T_CSET_ASCII
               ? "ASCII"
               : (lnk->cset == H5T_CSET_UTF8 ? "UTF-8" : "Unknown")));
  fprintf(stream, "%*s%-*s '%s'\n", indent, "", fwidth,
          "Link Name:", lnk->name);
  ...
  default:
    if (lnk->type >= H5L_TYPE_UD_MIN) {
      if (lnk->type == H5L_TYPE_EXTERNAL) {
        const char *objname = (const char *)lnk->u.ud.udata +
                              (HDstrlen((const char *)lnk->u.ud.udata) + 1);
