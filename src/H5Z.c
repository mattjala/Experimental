...
typedef struct H5Z_object_t {
  ...
  hbool_t sanity_checked; /* Whether the sanity check for collectively calling
                             H5Zunregister has been done */
...
static int H5Z__check_unregister_dset_cb(void *obj_ptr, hid_t obj_id,
                                         void *key);
static int H5Z__check_unregister_group_cb(void *obj_ptr, hid_t obj_id,
                                          void *key);
...
herr_t H5Z_init(void) {
  ...
  if (H5Z_register(H5Z_SHUFFLE) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                "unable to register shuffle filter");
  if (H5Z_register(H5Z_FLETCHER32) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                "unable to register fletcher32 filter");
  ...
  if (H5Z_register(H5Z_DEFLATE) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                "unable to register deflate filter");
...
#ifdef H5_HAVE_FILTER_SZIP
  {
    ...
    if (encoder_enabled < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                  "check for szip encoder failed");

    H5Z_SZIP->encoder_present = (unsigned)encoder_enabled;
    if (H5Z_register(H5Z_SZIP) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                  "unable to register szip filter");
  }
      ...
      for (dir = 0; dir < 2; dir++) {
        ...
        if (0 == nprint++) {
          ...
          fprintf(H5DEBUG(Z), "   %-16s %10s %10s %8s %8s %8s %10s\n", "Filter",
                  "Total", "Errors", "User", "System", "Elapsed", "Bandwidth");
          fprintf(H5DEBUG(Z), "   %-16s %10s %10s %8s %8s %8s %10s\n", "------",
                  "-----", "------", "----", "------", "-------", "---------");
        } /* end if */
         ...
         */
        H5_bandwidth(bandwidth, sizeof(bandwidth),
        ...
        fprintf(
            ...
            H5Z_stat_table_g[i].stats[dir].errors, timestrs.user,
            timestrs.system, timestrs.elapsed, bandwidth);
      next:
        free(timestrs.user);
...
herr_t H5Zregister(const void *cls) {
  const H5Z_class2_t *cls_real =
      (const H5Z_class2_t *)cls; /* "Real" class pointer */
  herr_t ret_value = SUCCEED;    /* Return value */
  ...
  if (cls_real->version != H5Z_CLASS_T_VERS) {
    ...
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                "invalid H5Z_class_t version number");
...
herr_t H5Z_register(const H5Z_class2_t *cls) {
  ...
  if (i >= H5Z_table_used_g) {
    if (H5Z_table_used_g >= H5Z_table_alloc_g) {
      ...
      H5Z_class2_t *table =
          (H5Z_class2_t *)H5MM_realloc(H5Z_table_g, n * sizeof(H5Z_class2_t));
#ifdef H5Z_DEBUG
      H5Z_stats_t *stat_table = (H5Z_stats_t *)H5MM_realloc(
          H5Z_stat_table_g, n * sizeof(H5Z_stats_t));
#endif /* H5Z_DEBUG */
      if (!table)
      ...
      if (!stat_table)
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                    "unable to extend filter statistics table");
...
herr_t H5Z__unregister(H5Z_filter_t filter_id) {
  ...
  /* Iterate through all opened datasets, returns a failure if any of them uses
   * the filter */
  if (H5I_iterate(H5I_DATASET, H5Z__check_unregister_dset_cb, &object, FALSE) <
      0)
  ...
  /* Iterate through all opened groups, returns a failure if any of them uses
   * the filter */
  if (H5I_iterate(H5I_GROUP, H5Z__check_unregister_group_cb, &object, FALSE) <
      0)
    HGOTO_ERROR(H5E_FILE, H5E_BADITER, FAIL, "iteration failed");
 ...
 *-------------------------------------------------------------------------
 */
static int H5Z__check_unregister_group_cb(void *obj_ptr,
                                          hid_t H5_ATTR_UNUSED obj_id,
                                          void *key) {
  hid_t ocpl_id = -1;
  ...
  if ((ocpl_id = H5G_get_create_plist((H5G_t *)obj_ptr)) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTGET, FAIL,
                "can't get group creation property list");
 ...
 *-------------------------------------------------------------------------
 */
static int H5Z__check_unregister_dset_cb(void *obj_ptr,
                                         hid_t H5_ATTR_UNUSED obj_id,
                                         void *key) {
  hid_t ocpl_id = -1;
  H5Z_object_t *object = (H5Z_object_t *)key;
 ...
 *-------------------------------------------------------------------------
 */
static int H5Z__flush_file_cb(void *obj_ptr, hid_t H5_ATTR_UNUSED obj_id,
  ...
  if (H5F_ACC_RDWR & H5F_INTENT(f)) {
    ...
    if (H5F_flush_mounts((H5F_t *)obj_ptr) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTFLUSH, FAIL,
                  "unable to flush file hierarchy");
  } /* end if */
...
htri_t H5Zfilter_avail(H5Z_filter_t id) {
  ...
  if ((ret_value = H5Z_filter_avail(id)) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_NOTFOUND, FAIL,
                "unable to check the availability of the filter");
...
htri_t H5Z_filter_avail(H5Z_filter_t id) {
  ...
  key.id = (int)id;
  if (NULL !=
      (filter_info = (const H5Z_class2_t *)H5PL_load(H5PL_TYPE_FILTER, &key))) {
    if (H5Z_register(filter_info) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                  "unable to register loaded filter");
    HGOTO_DONE(TRUE);
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5Z__prelude_callback(const H5O_pline_t *pline, hid_t dcpl_id,
  ...
  for (u = 0; u < pline->nused; u++) {
    ...
    if (NULL == (fclass = H5Z_find(pline->filter[u].id))) {
      ...
      if (pline->filter[u].flags & H5Z_FLAG_OPTIONAL)
        ...
        HGOTO_ERROR(H5E_PLINE, H5E_NOTFOUND, FAIL,
                    "required filter was not located");
    } /* end if */
    else {
      ...
      case H5Z_PRELUDE_CAN_APPLY:
        ...
        if (fclass->can_apply) {
          ...
          if (status == FALSE && !(pline->filter[u].flags & H5Z_FLAG_OPTIONAL))
            HGOTO_ERROR(H5E_PLINE, H5E_CANAPPLY, FAIL,
                        "filter parameters not appropriate");
        } /* end if */
        break;
      ...
      case H5Z_PRELUDE_SET_LOCAL:
        ...
        if (fclass->set_local) {
          ...
          if ((fclass->set_local)(dcpl_id, type_id, space_id) < 0)
            /* Indicate error during filter callback */
            HGOTO_ERROR(H5E_PLINE, H5E_SETLOCAL, FAIL,
                        "error during user callback");
        } /* end if */
        break;
 ...
 */
static herr_t
H5Z__prepare_prelude_callback_dcpl(hid_t dcpl_id, hid_t type_id,
                                   H5Z_prelude_type_t prelude_type) {
  ...
  if (dcpl_id != H5P_DATASET_CREATE_DEFAULT) {
    ...
    if (NULL ==
        (dcpl_layout = (H5O_layout_t *)H5MM_calloc(sizeof(H5O_layout_t))))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                  "unable to allocate dcpl layout buffer");
    ...
    if (H5D_CHUNKED == dcpl_layout->type) {
      ...
      if (dcpl_pline.nused > 0) {
        ...
        if (NULL == (space = H5S_create_simple(dcpl_layout->u.chunk.ndims,
                                               chunk_dims, NULL)))
        ...
        /* Make the callbacks */
        if (H5Z__prelude_callback(&dcpl_pline, dcpl_id, type_id, space_id,
                                  prelude_type) < 0)
...
herr_t H5Z_can_apply(hid_t dcpl_id, hid_t type_id) {
  ...
  if (H5Z__prepare_prelude_callback_dcpl(dcpl_id, type_id,
...
herr_t H5Z_set_local(hid_t dcpl_id, hid_t type_id) {
  ...
  if (H5Z__prepare_prelude_callback_dcpl(dcpl_id, type_id,
    ...
    HGOTO_ERROR(H5E_PLINE, H5E_SETLOCAL, FAIL,
                "local filter parameters not set");
...
herr_t H5Z_can_apply_direct(const H5O_pline_t *pline) {
  ...
  /* Make "can apply" callbacks for filters in pipeline */
  if (H5Z__prelude_callback(pline, (hid_t)-1, (hid_t)-1, (hid_t)-1,
                            H5Z_PRELUDE_CAN_APPLY) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANAPPLY, FAIL, "unable to apply filter");
 ...
 *-------------------------------------------------------------------------
 */
htri_t H5Z_ignore_filters(hid_t dcpl_id, const H5T_t *type,
                          const H5S_t *space) {
  H5P_genplist_t *dc_plist;        /* Dataset creation property list object */
  ...
  bad_for_filters =
      (H5S_NULL == space_class || H5S_SCALAR == space_class ||
       H5T_VLEN == type_class ||
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Z_modify(const H5O_pline_t *pline, H5Z_filter_t filter, unsigned flags,
                  size_t cd_nelmts,
  ...
  if (cd_nelmts > 0) {
    ...
    if (cd_nelmts > H5Z_COMMON_CD_VALUES) {
      pline->filter[idx].cd_values =
          (unsigned *)H5MM_malloc(cd_nelmts * sizeof(unsigned));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Z_append(H5O_pline_t *pline, H5Z_filter_t filter, unsigned flags,
                  size_t cd_nelmts,
                  const unsigned int cd_values[/*cd_nelmts*/]) {
  size_t idx;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (pline->nused >= pline->nalloc) {
    ...
    x.filter = (H5Z_filter_info_t *)H5MM_realloc(
        pline->filter, x.nalloc * sizeof(x.filter[0]));
    if (NULL == x.filter)
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
...
H5Z_class2_t *H5Z_find(H5Z_filter_t id) {
  ...
  if ((idx = H5Z__find_idx(id)) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_NOTFOUND, NULL,
                "required filter %d is not registered", id);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Z_pipeline(const H5O_pline_t *pline, unsigned flags,
  ...
  if (pline && (flags & H5Z_FLAG_REVERSE)) { /* Read */
    for (i = pline->nused; i > 0; --i) {
      ...
      if ((fclass_idx = H5Z__find_idx(pline->filter[idx].id)) < 0) {
        ...
        if (NULL != (filter_info = (const H5Z_class2_t *)H5PL_load(
                         H5PL_TYPE_FILTER, &key))) {
          /* Register the filter we loaded */
          if (H5Z_register(filter_info) < 0)
            HGOTO_ERROR(H5E_PLINE, H5E_CANTINIT, FAIL,
                        "unable to register filter");

          /* Search in the table of registered filters again to find the dynamic
           * filter just loaded and registered */
          if ((fclass_idx = H5Z__find_idx(pline->filter[idx].id)) < 0)
            issue_error = TRUE;
        } else
          issue_error = TRUE;
        ...
        /* Check for error */
        if (issue_error) {
          /* Print out the filter name to give more info.  But the name is
           * optional for the filter */
      ...
      if (0 == new_nbytes) {
        if ((cb_struct.func &&
             (H5Z_CB_FAIL == cb_struct.func(pline->filter[idx].id, *buf,
                                            *buf_size, cb_struct.op_data))) ||
            !cb_struct.func)
          HGOTO_ERROR(H5E_PLINE, H5E_READERROR, FAIL,
...
herr_t H5Zget_filter_info(H5Z_filter_t filter,
                          unsigned *filter_config_flags /*out*/) {
