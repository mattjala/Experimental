...
typedef struct H5D_contig_readvv_sieve_ud_t {
  ...
  const H5D_contig_storage_t
      *store_contig;   /* Contiguous storage info for this I/O operation */
...
static herr_t H5D__contig_io_init(H5D_io_info_t *io_info,
                                  H5D_dset_io_info_t *dinfo);
static herr_t H5D__contig_mdio_init(H5D_io_info_t *io_info,
                                    H5D_dset_io_info_t *dinfo);
static ssize_t H5D__contig_readvv(
    const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dinfo,
...
static ssize_t H5D__contig_writevv(
    const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dinfo,
    size_t dset_max_nseq, size_t *dset_curr_seq, size_t dset_len_arr[],
    hsize_t dset_offset_arr[], size_t mem_max_nseq, size_t *mem_curr_seq,
    size_t mem_len_arr[], hsize_t mem_offset_arr[]);
static herr_t H5D__contig_flush(H5D_t *dset);
static herr_t H5D__contig_io_term(H5D_io_info_t *io_info,
                                  H5D_dset_io_info_t *di);

/* Helper routines */
static herr_t H5D__contig_write_one(H5D_io_info_t *io_info,
                                    H5D_dset_io_info_t *dset_info,
                                    hsize_t offset, size_t size);
static herr_t H5D__contig_may_use_select_io(H5D_io_info_t *io_info,
...
herr_t H5D__contig_alloc(H5F_t *f, H5O_storage_contig_t *storage /*out */) {
  ...
  if (HADDR_UNDEF ==
      (storage->addr = H5MF_alloc(f, H5FD_MEM_DRAW, storage->size)))
...
herr_t H5D__contig_fill(H5D_t *dset) {
  ...
  if (H5F_HAS_FEATURE(dset->oloc.file, H5FD_FEAT_HAS_MPI)) {
    ...
    if (MPI_COMM_NULL == (mpi_comm = H5F_mpi_get_comm(dset->oloc.file)))
      HGOTO_ERROR(H5E_INTERNAL, H5E_MPI, FAIL,
  ...
  if ((snpoints = H5S_GET_EXTENT_NPOINTS(dset->shared->space)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "dataset has negative number of elements");
  ...
  if (H5D__fill_init(&fb_info, NULL, NULL, NULL, NULL, NULL,
                     ...
                     dset->shared->type_id, npoints, max_temp_buf) < 0)
  ...
  while (npoints > 0) {
    size_t curr_points; /* Number of elements to write on this iteration of the
                           loop */
...
herr_t H5D__contig_delete(H5F_t *f, const H5O_storage_t *storage) {
  ...
  if (H5MF_xfree(f, H5FD_MEM_DRAW, storage->u.contig.addr,
                 storage->u.contig.size) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, FAIL,
                "unable to free contiguous storage space");
...
static herr_t H5D__contig_construct(H5F_t *f, H5D_t *dset) {
  ...
  if (0 == (dt_size = H5T_GET_SIZE(dset->shared->type)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "unable to retrieve size of datatype");
  ...
  /* Adjust the sieve buffer size to the smaller one between the dataset size
   * and the buffer size from the file access property. (SLU - 2012/3/30) */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_init(H5F_t H5_ATTR_UNUSED *f, const H5D_t *dset,
...
static herr_t H5D__contig_io_init(H5D_io_info_t *io_info,
  ...
  dinfo->store->contig.dset_addr =
      dataset->shared->layout.storage.u.contig.addr;
  dinfo->store->contig.dset_size =
      dataset->shared->layout.storage.u.contig.size;
  ...
  if (dinfo->nelmts) {
    ...
    /* Calculate type conversion buffer size and check for in-place conversion
     * if necessary.  Currently only implemented for selection I/O. */
  ...
  if (ret_value < 0) {
    if (H5D__contig_io_term(io_info, dinfo) < 0)
      HDONE_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                  "unable to release dataset I/O info");
  } /* end if */

  if (file_space_normalized) {
    /* (Casting away const OK -QAK) */
    if (H5S_hyper_denormalize_offset(dinfo->file_space, old_offset) < 0)
      HDONE_ERROR(H5E_DATASET, H5E_BADSELECT, FAIL,
                  "unable to normalize dataspace by offset");
  } /* end if */
...
static herr_t H5D__contig_mdio_init(H5D_io_info_t *io_info,
  ...
  if (dinfo->layout_io_info.contig_piece_info) {
    ...
    io_info->sel_pieces[io_info->pieces_added] =
        dinfo->layout_io_info.contig_piece_info;
...
static herr_t H5D__contig_may_use_select_io(H5D_io_info_t *io_info,
  ...
  /* None of the reasons this function might disable selection I/O are relevant
   * to parallel, so no need to update no_selection_io_cause since we're only
   * keeping track of the reason for no selection I/O in parallel (for now) */

  /* Don't use selection I/O if it's globally disabled, if it's not a contiguous
   * dataset, or if the sieve buffer exists (write) or is dirty (read) */
  ...
  if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
    ...
    if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
      ...
      if (H5F_shared_select_read(H5F_SHARED(dinfo->dset->oloc.file),
                                 H5FD_MEM_DRAW, dinfo->nelmts > 0 ? 1 : 0,
                                 &dinfo->mem_space, &dinfo->file_space,
                                 &(dinfo->store->contig.dset_addr),
                                 &dst_type_size, &(dinfo->buf.vp)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL,
                    "contiguous selection read failed");
    } else {
      if (dinfo->layout_io_info.contig_piece_info) {
        ...
        io_info->element_sizes[io_info->pieces_added] =
    ...
    if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
      ...
      if (H5F_shared_select_write(H5F_SHARED(dinfo->dset->oloc.file),
        ...
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                    "contiguous selection write failed");
    } else {
      if (dinfo->layout_io_info.contig_piece_info) {
        /* Add to mdset selection I/O arrays */
        assert(io_info->mem_spaces);
...
static herr_t H5D__contig_write_one(H5D_io_info_t *io_info,
  ...
  if (H5D__contig_writevv(io_info, dset_info, (size_t)1, &dset_curr_seq,
                          &dset_len, &dset_off, (size_t)1, &mem_curr_seq,
                          &mem_len, &mem_off) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_readvv_sieve_cb(hsize_t dst_off, hsize_t src_off,
                                          size_t len, void *_udata) {
  H5D_contig_readvv_sieve_ud_t *udata = (H5D_contig_readvv_sieve_ud_t *)
      _udata;                       /* User data for H5VM_opvv() operator */
  H5F_shared_t *f_sh = udata->f_sh; /* Shared file for dataset */
  H5D_rdcdc_t *dset_contig =
      udata->dset_contig; /* Cached information about contiguous data */
  ...
  if (NULL == dset_contig->sieve_buf) {
    ...
    if (len > dset_contig->sieve_buf_size) {
      ...
      if (NULL == (dset_contig->sieve_buf =
                       H5FL_BLK_CALLOC(sieve_buf, dset_contig->sieve_buf_size)))
      ...
      min = MIN3(rel_eoa - dset_contig->sieve_loc, max_data,
                 dset_contig->sieve_buf_size);
    ...
    if (addr >= sieve_start && contig_end < sieve_end) {
      unsigned char *base_sieve_buf =
          dset_contig->sieve_buf + (addr - sieve_start);
    ...
    /* Entire request is not within this data sieve buffer */
      ...
      if (len > dset_contig->sieve_buf_size) {
        ...
        if ((sieve_start >= addr && sieve_start < (contig_end + 1)) ||
          ...
          if (dset_contig->sieve_dirty) {
            /* Write to file */
            if (H5F_shared_block_write(f_sh, H5FD_MEM_DRAW, sieve_start,
                                       sieve_size, dset_contig->sieve_buf) < 0)
              HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL,
                          "block write failed");
      ...
      /* Element size fits within the buffer size */
        ...
        /* Grab the data out of the buffer (must be first piece of data in
         * buffer ) */
        H5MM_memcpy(buf, dset_contig->sieve_buf, len);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_readvv_cb(hsize_t dst_off, hsize_t src_off,
                                    size_t len, void *_udata) {
  H5D_contig_readvv_ud_t *udata =
      (H5D_contig_readvv_ud_t *)_udata; /* User data for H5VM_opvv() operator */
  herr_t ret_value = SUCCEED;           /* Return value */
 ...
 *		The data is contiguous.	 The address is the start of the
 *dataset, relative to the base address for the file and the offsets and
...
static ssize_t H5D__contig_readvv(const H5D_io_info_t *io_info,
                                  const H5D_dset_io_info_t *dset_info,
                                  size_t dset_max_nseq, size_t *dset_curr_seq,
  ...
  if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_DATA_SIEVE)) {
    ...
    if ((ret_value =
             H5VM_opvv(dset_max_nseq, dset_curr_seq, dset_len_arr, dset_off_arr,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_writevv_sieve_cb(hsize_t dst_off, hsize_t src_off,
                                           size_t len, void *_udata) {
  H5D_contig_writevv_sieve_ud_t *udata = (H5D_contig_writevv_sieve_ud_t *)
      _udata;                       /* User data for H5VM_opvv() operator */
  H5F_shared_t *f_sh = udata->f_sh; /* Shared file for dataset */
  H5D_rdcdc_t *dset_contig =
      udata->dset_contig; /* Cached information about contiguous data */
  ...
  if (NULL == dset_contig->sieve_buf) {
    ...
    if (len > dset_contig->sieve_buf_size) {
      ...
      if (dset_contig->sieve_size > len)
        memset(dset_contig->sieve_buf + len, 0,
    ...
    /* Entire request is not within this data sieve buffer */
      ...
      /* Element size fits within the buffer size */
        ...
        /* Check if it is possible to (exactly) prepend or append to existing
         * (dirty) sieve buffer */
        ...
        /* Can't add the new data onto the existing sieve buffer */
          ...
          if (dset_contig->sieve_size > len) {
            /* Read the new sieve buffer */
            if (H5F_shared_block_read(
                    f_sh, H5FD_MEM_DRAW, dset_contig->sieve_loc,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_writevv_cb(hsize_t dst_off, hsize_t src_off,
                                     size_t len, void *_udata) {
  H5D_contig_writevv_ud_t *udata = (H5D_contig_writevv_ud_t *)
      _udata;                 /* User data for H5VM_opvv() operator */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  /* Write data */
  if (H5F_shared_block_write(udata->f_sh, H5FD_MEM_DRAW,
                             (udata->dset_addr + dst_off), len,
...
static ssize_t H5D__contig_writevv(const H5D_io_info_t *io_info,
                                   ...
                                   size_t dset_max_nseq, size_t *dset_curr_seq,
                                   size_t dset_len_arr[],
                                   hsize_t dset_off_arr[], size_t mem_max_nseq,
  ...
  if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_DATA_SIEVE)) {
    H5D_contig_writevv_sieve_ud_t
        udata; /* User data for H5VM_opvv() operator */
    ...
    if ((ret_value =
      ...
      HGOTO_ERROR(H5E_DATASET, H5E_CANTOPERATE, FAIL,
                  "can't perform vectorized sieve buffer write");
  } /* end if */
  else {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__contig_io_term(H5D_io_info_t H5_ATTR_UNUSED *io_info,
                                  H5D_dset_io_info_t *di) {
  herr_t ret_value = SUCCEED; /*return value        */
  ...
  if (di->layout_io_info.contig_piece_info) {
    if (H5D__free_piece_info(di->layout_io_info.contig_piece_info, NULL, NULL) <
        0)
...
herr_t H5D__contig_copy(H5F_t *f_src, const H5O_storage_contig_t *storage_src,
                        ...
                        H5T_t *dt_src, H5O_copy_t *cpy_info) {
  ...
  hbool_t is_vlen =
      FALSE; /* Flag to indicate that VL type conversion should occur */
  hbool_t fix_ref =
      FALSE; /* Flag to indicate that ref values should be fixed */
  H5D_shared_t *shared_fo =
      (H5D_shared_t *)cpy_info
          ->shared_fo; /* Pointer to the shared struct for dataset object */
  ...
  if (H5D__contig_alloc(f_dst, storage_dst) < 0)
    HGOTO_ERROR(H5E_IO, H5E_CANTINIT, FAIL,
                "unable to allocate contiguous storage");
  ...
  if (NULL == (buf = H5FL_BLK_MALLOC(type_conv, buf_size)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "memory allocation failed for copy buffer");

  /* Need extra buffer for datatype conversions, to prevent stranding/leaking
   * memory */
  if (is_vlen || fix_ref) {
  ...
  if (H5F_HAS_FEATURE(f_src, H5FD_FEAT_DATA_SIEVE) && shared_fo &&
      shared_fo->cache.contig.sieve_buf) {
    try_sieve = TRUE;
  ...
  while (total_src_nbytes > 0) {
      ...
      if (is_vlen) {
        ...
        if (H5S_set_extent_real(buf_space, buf_dim) < 0)
          HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL,
                      "unable to change buffer dataspace size");
      } /* end if */
      else
        /* Adjust destination & memory bytes to transfer */
    ...
    /* If the entire copy is within the sieve buffer, copy data from the sieve
     * buffer */
    if (try_sieve && (addr_src >= sieve_start) &&
        ((addr_src + src_nbytes - 1) < sieve_end)) {
      unsigned char *base_sieve_buf =
          shared_fo->cache.contig.sieve_buf + (addr_src - sieve_start);
    ...
    if (is_vlen) {
      ...
      if (cpy_info->expand_ref) {
        ...
        if (H5O_copy_expand_ref(f_src, tid_src, dt_src, buf, buf_size, f_dst,
                                bkg, cpy_info) < 0)
          HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, FAIL,
                      "unable to copy reference attribute");

        /* After fix ref, copy the new reference elements to the buffer to write
         * out */
