...
/*
 *
 * Purpose: v2 B-tree indexing for chunked datasets with > 1 unlimited
 * dimensions. Each dataset chunk in the b-tree is identified by its dimensional
 * offset.
 *
 */
...
typedef struct H5D_bt2_ctx_t {
  uint32_t
      chunk_size; /* Size of chunk (bytes; constant for unfiltered object) */
...
/* Shared v2 B-tree methods for indexing filtered and non-filtered chunked
 * datasets */
...
static herr_t H5D__bt2_unfilt_encode(uint8_t *raw, const void *native,
                                     void *ctx);
static herr_t H5D__bt2_unfilt_decode(const uint8_t *raw, void *native,
                                     void *ctx);
...
static herr_t H5D__bt2_idx_size(const H5D_chk_idx_info_t *idx_info,
                                hsize_t *size);
static herr_t H5D__bt2_idx_reset(H5O_storage_chunk_t *storage,
                                 hbool_t reset_addr);
...
static void *H5D__bt2_crt_context(void *_udata) {
  H5D_bt2_ctx_ud_t *udata =
      (H5D_bt2_ctx_ud_t *)_udata; /* User data for building callback context */
  ...
  /* Determine the size of addresses and set the chunk size and # of dimensions
   * for the dataset */
  ...
  if (NULL ==
      (my_dim = (uint32_t *)H5FL_ARR_MALLOC(uint32_t, H5O_LAYOUT_NDIMS)))
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__bt2_compare(const void *_udata, const void *_rec2,
                               int *result) {
  const H5D_bt2_ud_t *udata = (const H5D_bt2_ud_t *)_udata; /* User data */
  const H5D_chunk_rec_t *rec1 = &(udata->rec); /* The search record */
  const H5D_chunk_rec_t *rec2 =
      (const H5D_chunk_rec_t *)_rec2; /* The native record */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__bt2_unfilt_encode(uint8_t *raw, const void *_record,
                                     void *_ctx) {
  H5D_bt2_ctx_t *ctx = (H5D_bt2_ctx_t *)_ctx; /* Callback context structure */
  const H5D_chunk_rec_t *record =
      (const H5D_chunk_rec_t *)_record; /* The native record */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__bt2_unfilt_decode(const uint8_t *raw, void *_record,
                                     void *_ctx) {
  H5D_bt2_ctx_t *ctx = (H5D_bt2_ctx_t *)_ctx; /* Callback context structure */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__bt2_filt_encode(uint8_t *raw, const void *_record,
                                   void *_ctx) {
  H5D_bt2_ctx_t *ctx = (H5D_bt2_ctx_t *)_ctx; /* Callback context structure */
  const H5D_chunk_rec_t *record =
      (const H5D_chunk_rec_t *)_record; /* The native record */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5D__bt2_filt_decode(const uint8_t *raw, void *_record,
                                   void *_ctx) {
  H5D_bt2_ctx_t *ctx = (H5D_bt2_ctx_t *)_ctx; /* Callback context structure */
  H5D_chunk_rec_t *record = (H5D_chunk_rec_t *)_record; /* The native record */
  unsigned u; /* Local index variable */
...
static herr_t H5D__bt2_filt_debug(FILE *stream, int indent, int fwidth,
  ...
  fprintf(stream, "%*s%-*s %u bytes\n", indent, "", fwidth,
          "Chunk size:", (unsigned)record->nbytes);
  fprintf(stream, "%*s%-*s 0x%08x\n", indent, "", fwidth,
          "Filter mask:", record->filter_mask);
...
static herr_t H5D__bt2_idx_open(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (NULL == (idx_info->storage->u.btree2.bt2 =
                   H5B2_open(idx_info->f, idx_info->storage->idx_addr, &u_ctx)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "can't open v2 B-tree for tracking chunked dataset");
...
static herr_t H5D__bt2_idx_create(const H5D_chk_idx_info_t *idx_info) {
  ...
  bt2_cparam.rrec_size = H5F_SIZEOF_ADDR(idx_info->f) /* Address of chunk */
                         + (idx_info->layout->ndims - 1) *
                               8; /* # of dimensions x 64-bit chunk offsets */
  ...
  if (idx_info->pline->nused > 0) {
    ...
    bt2_cparam.rrec_size +=
        chunk_size_len + 4; /* Size of encoded chunk size & filter mask */
  ...
  if (NULL == (idx_info->storage->u.btree2.bt2 =
                   H5B2_create(idx_info->f, &bt2_cparam, &u_ctx)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, FAIL,
                "can't create v2 B-tree for tracking chunked dataset");

  /* Retrieve the v2 B-tree's address in the file */
  if (H5B2_get_addr(idx_info->storage->u.btree2.bt2,
 ...
 */
static herr_t H5D__bt2_mod_cb(void *_record, void *_op_data, hbool_t *changed) {
  H5D_bt2_ud_t *op_data =
      (H5D_bt2_ud_t *)_op_data; /* User data for v2 B-tree calls */
 ...
 *		A filtered chunk:
 *		  If it was not found, create the chunk and pass chunk address
 *back up If it was found but its size changed, reallocate the chunk and pass
 *chunk address back up If it was found but its size was the same, pass chunk
 *address back up
 *
 * Return:	Non-negative on success/Negative on failure
 *
...
static herr_t H5D__bt2_idx_insert(const H5D_chk_idx_info_t *idx_info,
  ...
  if (NULL == idx_info->storage->u.btree2.bt2) {
    ...
    if (H5B2_patch_file(idx_info->storage->u.btree2.bt2, idx_info->f) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL,
                  "can't patch v2 B-tree file pointer");
  ...
  if (idx_info->pline->nused > 0) { /* filtered chunk */
    H5_CHECKED_ASSIGN(bt2_udata.rec.nbytes, uint32_t, udata->chunk_block.length,
                      hsize_t);
  ...
  if (H5B2_update(bt2, &bt2_udata, H5D__bt2_mod_cb, &bt2_udata) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTUPDATE, FAIL,
                "unable to update record in v2 B-tree");
 ...
 *B-tree. This is the callback for H5B2_find() which is called in
...
static herr_t H5D__bt2_idx_get_addr(const H5D_chk_idx_info_t *idx_info,
  ...
  if (H5B2_find(bt2, &bt2_udata, &found, H5D__bt2_found_cb, &found_rec) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTFIND, FAIL,
                "can't check for chunk in v2 B-tree");
...
static int H5D__bt2_idx_iterate_cb(const void *_record, void *_udata) {
  ...
  const H5D_chunk_rec_t *record =
      (const H5D_chunk_rec_t *)_record; /* Native record */
...
static herr_t H5D__bt2_remove_cb(const void *_record, void *_udata) {
  ...
  if (H5MF_xfree(f, H5FD_MEM_DRAW, record->chunk_addr,
                 (hsize_t)record->nbytes) < 0)
...
static herr_t H5D__bt2_idx_remove(const H5D_chk_idx_info_t *idx_info,
  ...
  if (H5B2_remove(bt2, &bt2_udata,
                  (H5F_INTENT(idx_info->f) & H5F_ACC_SWMR_WRITE)
                      ? NULL
                      : H5D__bt2_remove_cb,
                  idx_info->f) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTREMOVE, FAIL,
                "can't remove object from B-tree");
...
static herr_t H5D__bt2_idx_delete(const H5D_chk_idx_info_t *idx_info) {
  ...
  if (H5_addr_defined(idx_info->storage->idx_addr)) {
    ...
    if (H5B2_delete(idx_info->f, idx_info->storage->idx_addr, &u_ctx, remove_op,
                    idx_info->f) < 0)
...
static herr_t H5D__bt2_idx_size(const H5D_chk_idx_info_t *idx_info,
  ...
  if (bt2_cdset && H5B2_close(bt2_cdset) < 0)
    HDONE_ERROR(H5E_SYM, H5E_CLOSEERROR, FAIL,
                "can't close v2 B-tree for tracking chunked dataset");
