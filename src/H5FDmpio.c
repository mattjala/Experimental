...
/* (Can be changed by setting "HDF5_MPI_OPT_TYPES" environment variable to '0'
 * or '1') */
...
typedef struct H5FD_mpio_t {
  ...
  hbool_t mpi_file_sync_required; /* Whether the ROMIO driver requires
                                     MPI_File_sync after write */
} H5FD_mpio_t;
...
static herr_t H5FD__mpio_get_handle(H5FD_t *_file, hid_t fapl,
                                    void **file_handle);
static herr_t H5FD__mpio_read(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                              haddr_t addr, size_t size, void *buf);
static herr_t H5FD__mpio_write(H5FD_t *_file, H5FD_mem_t type, hid_t dxpl_id,
                               haddr_t addr, size_t size, const void *buf);
static herr_t H5FD__mpio_read_vector(H5FD_t *_file,
                                     hid_t H5_ATTR_UNUSED dxpl_id,
                                     uint32_t count, H5FD_mem_t types[],
                                     haddr_t addrs[], size_t sizes[],
                                     void *bufs[]);
static herr_t H5FD__mpio_write_vector(H5FD_t *_file,
                                      hid_t H5_ATTR_UNUSED dxpl_id,
                                      uint32_t count, H5FD_mem_t types[],
                                      haddr_t addrs[], size_t sizes[],
                                      const void *bufs[]);

static herr_t H5FD__mpio_read_selection(H5FD_t *_file, H5FD_mem_t type,
                                        hid_t H5_ATTR_UNUSED dxpl_id,
                                        size_t count, hid_t mem_space_ids[],
                                        hid_t file_space_ids[],
                                        haddr_t offsets[],
                                        size_t element_sizes[], void *bufs[]);

static herr_t H5FD__mpio_write_selection(
    H5FD_t *_file, H5FD_mem_t type, hid_t H5_ATTR_UNUSED dxpl_id, size_t count,
...
static herr_t H5FD__mpio_truncate(H5FD_t *_file, hid_t dxpl_id,
                                  hbool_t closing);
...
static herr_t H5FD__selection_build_types(
    hbool_t io_op_write, size_t num_pieces, H5_flexible_const_ptr_t mbb,
...
hid_t H5FD_mpio_init(void) {
  ...
  if (H5I_VFL != H5I_get_type(H5FD_MPIO_g)) {
    H5FD_MPIO_g = H5FD_register((const H5FD_class_t *)&H5FD_mpio_g,
    ...
    if (env && !HDstrcmp(env, "mpio")) {
      ...
      if (MPI_SUCCESS != MPI_Initialized(&mpi_initialized))
        HGOTO_ERROR(H5E_VFL, H5E_UNINITIALIZED, H5I_INVALID_HID,
                    "can't check if MPI is initialized");
      if (!mpi_initialized) {
        if (MPI_SUCCESS != MPI_Init(NULL, NULL))
          HGOTO_ERROR(H5E_VFL, H5E_CANTINIT, H5I_INVALID_HID,
                      "can't initialize MPI");
...
herr_t H5Pset_fapl_mpio(hid_t fapl_id, MPI_Comm comm, MPI_Info info) {
  ...
  if (MPI_COMM_NULL == comm)
    HGOTO_ERROR(H5E_PLIST, H5E_BADTYPE, FAIL,
                "MPI_COMM_NULL is not a valid communicator");
  ...
  if (ret_value != SUCCEED) {
    if (comm)
      if (H5_mpi_comm_free(comm) < 0)
        HDONE_ERROR(H5E_PLIST, H5E_CANTFREE, FAIL,
...
herr_t H5Pset_dxpl_mpio_chunk_opt_ratio(hid_t dxpl_id,
  ...
  /* Set the transfer mode */
  if (H5P_set(plist, H5D_XFER_MPIO_CHUNK_OPT_RATIO_NAME,
              &percent_num_proc_per_chunk) < 0)
    HGOTO_ERROR(H5E_PLIST, H5E_CANTSET, FAIL, "unable to set value");
...
herr_t H5FD_set_mpio_atomicity(H5FD_t *_file, hbool_t flag) {
...
#ifdef H5FDmpio_DEBUG
  hbool_t H5FD_mpio_debug_t_flag =
      (H5FD_mpio_debug_flags_s[(int)'t'] && H5FD_MPIO_TRACE_THIS_RANK(file));
#endif
  ...
  /* set atomicity value */
  if (MPI_SUCCESS !=
      (mpi_code = MPI_File_set_atomicity(file->f, (int)(flag != FALSE))))
...
static H5FD_t *H5FD__mpio_open(const char *name, unsigned flags, hid_t fapl_id,
  ...
  hbool_t file_opened =
      FALSE;     /* Flag to indicate that the file was successfully opened */
  ...
  H5FD_mpio_debug_t_flag =
      (H5FD_mpio_debug_flags_s[(int)'t'] &&
       (H5FD_mpio_debug_rank_s < 0 || H5FD_mpio_debug_rank_s == mpi_rank));
  if (H5FD_mpio_debug_t_flag)
    fprintf(stderr,
            "%s: (%d) Entering - name = \"%s\", flags = 0x%x, fapl_id = %d, "
            "maxaddr = %lu\n",
            __func__, mpi_rank, name, flags, (int)fapl_id,
  ...
  if (MPI_INFO_NULL != info) {
    ...
    MPI_Info_get(info, H5F_MPIO_DEBUG_KEY, sizeof(debug_str) - 1, debug_str,
                 &flag);
    if (flag)
  ...
  if (MPI_SUCCESS !=
      (mpi_code = MPI_File_open(comm, name, mpi_amode, info, &fh)))
  ...
  if (info_used != MPI_INFO_NULL) {
    ...
    if (info == MPI_INFO_NULL) /* reuse info created from MPI_File_get_info() */
      ...
      for (i = 0; i < nkeys; i++) {
        ...
        if (MPI_SUCCESS != (mpi_code = MPI_Info_get_valuelen(info_used, key,
                                                             &valuelen, &flag)))
          HMPI_GOTO_ERROR(NULL, "MPI_Info_get_valuelen failed", mpi_code)
        /* retrieve the value of nth hint */
        if (MPI_SUCCESS != (mpi_code = MPI_Info_get(
                                info_used, key, valuelen + 1, value, &flag)))
  ...
  /* Retrieve the flag indicating whether MPI_File_sync is needed after each
   * write */
  if (H5_mpio_get_file_sync_required(fh, &file->mpi_file_sync_required) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, NULL,
                "unable to get mpi_file_sync_required hint");
  ...
  /* Broadcast file size */
  if (MPI_SUCCESS != (mpi_code = MPI_Bcast(&file_size, (int)sizeof(MPI_Offset),
                                           MPI_BYTE, 0, comm)))
  ...
  if (ret_value == NULL) {
    if (file_opened)
      MPI_File_close(&fh);
    if (H5_mpi_comm_free(&comm) < 0)
      HDONE_ERROR(H5E_VFL, H5E_CANTFREE, NULL,
                  "unable to free MPI communicator");
    if (H5_mpi_info_free(&info) < 0)
      HDONE_ERROR(H5E_VFL, H5E_CANTFREE, NULL,
                  "unable to free MPI info object");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_query(const H5FD_t H5_ATTR_UNUSED *_file,
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__mpio_get_eoa(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_set_eoa(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
                                 haddr_t addr) {
  H5FD_mpio_t *file = (H5FD_mpio_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__mpio_get_eof(const H5FD_t *_file,
                                  H5FD_mem_t H5_ATTR_UNUSED type) {
  const H5FD_mpio_t *file = (const H5FD_mpio_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_get_handle(H5FD_t *_file, hid_t H5_ATTR_UNUSED fapl,
                                    void **file_handle) {
  H5FD_mpio_t *file = (H5FD_mpio_t *)_file;
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_read(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
  ...
  MPI_Datatype buf_type =
      MPI_BYTE; /* MPI description of the selection in memory */
  ...
  hbool_t H5FD_mpio_debug_t_flag =
      (H5FD_mpio_debug_flags_s[(int)'t'] && H5FD_MPIO_TRACE_THIS_RANK(file));
  hbool_t H5FD_mpio_debug_r_flag =
      (H5FD_mpio_debug_flags_s[(int)'r'] && H5FD_MPIO_TRACE_THIS_RANK(file));
#endif
  int mpi_code; /* MPI return code */
  herr_t ret_value = SUCCEED;
  ...
  if (type == H5FD_MEM_DRAW) {
    ...
    if (H5CX_get_io_xfer_mode(&xfer_mode) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                  "can't get MPI-I/O transfer mode");
    ...
    if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
       ...
       */
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_set_view(file->f, mpi_off, MPI_BYTE, file_type,
  ...
  if (use_view_this_time) {
    ...
    if (H5FD_mpio_debug_r_flag)
      fprintf(stderr, "%s: (%d) using MPIO collective mode\n", __func__,
              file->mpi_rank);
#endif
    /* Get the collective_opt property to check whether the application wants to
     * do IO individually. */
    ...
    if (coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO) {
      ...
      if (H5CX_get_mpio_rank0_bcast()) {
        ...
        if (file->mpi_rank == 0) {
          ...
          if (MPI_SUCCESS !=
              (mpi_code = MPI_File_read_at(file->f, mpi_off, buf, size_i,
                                           buf_type, &mpi_stat)))
            HMPI_DONE_ERROR(FAIL, "MPI_File_read_at failed", mpi_code)
        }

        if (MPI_SUCCESS !=
            (mpi_code = MPI_Bcast(buf, size_i, buf_type, 0, file->comm)))
          HMPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_code)
      } /* end if */
      else
        /* Perform collective read operation */
        if (MPI_SUCCESS !=
            (mpi_code = MPI_File_read_at_all(file->f, mpi_off, buf, size_i,
                                             buf_type, &mpi_stat)))
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_read_at(file->f, mpi_off, buf, size_i, buf_type,
                                       &mpi_stat)))
     ...
     */
    if (MPI_SUCCESS !=
        (mpi_code = MPI_File_set_view(file->f, (MPI_Offset)0, MPI_BYTE,
                                      MPI_BYTE, H5FD_mpi_native_g, file->info)))
      HMPI_GOTO_ERROR(FAIL, "MPI_File_set_view failed", mpi_code)
  } /* end if */
  else {
    if (size != (hsize_t)size_i) {
      ...
      if (H5_mpio_create_large_type(size, 0, MPI_BYTE, &buf_type) < 0)
        HGOTO_ERROR(H5E_INTERNAL, H5E_CANTGET, FAIL,
                    "can't create MPI-I/O datatype");
...
#if H5_CHECK_MPI_VERSION(3, 0)
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Get_elements_x(&mpi_stat, buf_type, &bytes_read))) {
#else
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Get_elements(&mpi_stat, MPI_BYTE, &bytes_read))) {
#endif
  ...
  if (H5FD_mpio_debug_r_flag)
    fprintf(stderr, "%s: (%d) mpi_off = %ld  bytes_read = %lld  type = %s\n",
            __func__, file->mpi_rank, (long)mpi_off, (long long)bytes_read,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_write(H5FD_t *_file, H5FD_mem_t type,
                               hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr,
  ...
  hbool_t H5FD_mpio_debug_t_flag =
      (H5FD_mpio_debug_flags_s[(int)'t'] && H5FD_MPIO_TRACE_THIS_RANK(file));
  hbool_t H5FD_mpio_debug_w_flag =
      (H5FD_mpio_debug_flags_s[(int)'w'] && H5FD_MPIO_TRACE_THIS_RANK(file));
#endif
  int mpi_code; /* MPI return code */
  herr_t ret_value = SUCCEED;
  ...
  /* Verify that no data is written when between MPI_Barrier()s during file
   * flush */
  ...
  if (use_view_this_time) {
    ...
    if (coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO) {
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_write_at_all(file->f, mpi_off, buf, size_i,
                                            buf_type, &mpi_stat)))
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_write_at(file->f, mpi_off, buf, size_i, buf_type,
                                        &mpi_stat)))
...
#if H5_CHECK_MPI_VERSION(3, 0)
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Get_elements_x(&mpi_stat, buf_type, &bytes_written)))
#else
  if (MPI_SUCCESS !=
      (mpi_code = MPI_Get_elements(&mpi_stat, MPI_BYTE, &bytes_written)))
#endif
    HMPI_GOTO_ERROR(FAIL, "MPI_Get_elements failed", mpi_code)
  ...
  if (H5FD_mpio_debug_w_flag)
    fprintf(stderr, "%s: (%d) mpi_off = %ld  bytes_written = %lld  type = %s\n",
            __func__, file->mpi_rank, (long)mpi_off, (long long)bytes_written,
            H5FD__mem_t_to_str(type));
#endif
...
static herr_t H5FD__mpio_vector_build_types(
  ...
  if (count == 1) {
    ...
    *mpi_bufs_base = bufs[0];
    ...
    /* Setup s_addrs, s_sizes and s_bufs (needed for incomplete read filling
     * code and eof calculation code) */
    *s_addrs = addrs;
    *s_sizes = sizes;
    *s_bufs = bufs;
    ...
    if (sizes[0] > bigio_count) {
      /* We need to work around the integer size limit of 2GB. The input size_t
       * size variable cannot fit into an integer, but we can get around that
       * limitation by creating a different datatype and then setting the
       * integer size (or element count) to 1 when using the derived_type. */
      ...
      *size_i = 1;
             ...
             0) { /* create MPI derived types describing the vector write */

    /* sort the vector I/O request into increasing address order if required
     *
     * If the vector is already sorted, the base addresses of types, addrs,
     * sizes, and bufs will be returned in s_types, s_addrs, s_sizes, and s_bufs
     * respectively.
     *
     * If the vector was not already sorted, new, sorted versions of types,
     * addrs, sizes, and bufs are allocated, populated, and returned in s_types,
     * s_addrs, s_sizes, and s_bufs respectively. In this case, this function
     * must free the memory allocated for the sorted vectors.
     */
    if (H5FD_sort_vector_io_req(vector_was_sorted, count, types, addrs, sizes,
                                bufs, &s_types, s_addrs, s_sizes, s_bufs) < 0)
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "can't sort vector I/O request");

    if ((NULL ==
         (mpi_block_lengths = (int *)malloc((size_t)count * sizeof(int)))) ||
        (NULL == (mpi_displacements =
                      (MPI_Aint *)malloc((size_t)count * sizeof(MPI_Aint)))) ||
        (NULL ==
         (mpi_bufs = (MPI_Aint *)malloc((size_t)count * sizeof(MPI_Aint))))) {

      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                  "can't alloc mpi block lengths / displacement");
    }
    ...
    *mpi_bufs_base = (*s_bufs)[j];
    ...
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Get_address(mpi_bufs_base->cvp, &mpi_bufs_base_Aint)))

      HMPI_GOTO_ERROR(FAIL,
                      "MPI_Get_address for s_bufs[] to mpi_bufs_base failed",
                      mpi_code)
    ...
    *size_i = 1;
    ...
    for (i = 0; i < (int)count; i++) {
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Get_address((*s_bufs)[i].cvp, &(mpi_bufs[i]))))
        HMPI_GOTO_ERROR(FAIL,
                        "MPI_Get_address for s_bufs[] - mpi_bufs_base failed",
                        mpi_code)
      ...
      /* Check for size overflow */
      if (size > bigio_count) {
        /* We need to work around the integer size limit of 2GB. The input
         * size_t size variable cannot fit into an integer, but we can get
         * around that limitation by creating a different datatype and then
         * setting the integer size (or element count) to 1 when using the
         * derived_type. */

        /* Allocate arrays to keep track of types and whether they were created,
         * if necessary */
        if (!sub_types) {
          assert(!sub_types_created);

          if (NULL ==
              (sub_types = malloc((size_t)count * sizeof(MPI_Datatype))))
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                        "can't alloc sub types array");
          if (NULL ==
              (sub_types_created = (uint8_t *)calloc((size_t)count, 1))) {
            H5MM_free(sub_types);
            sub_types = NULL;
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
    ...
    if (sub_types) {
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Type_create_struct((int)count, mpi_block_lengths,
                                             mpi_bufs, sub_types, buf_type)))
        HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_struct for buf_type failed",
                        mpi_code)
    } else if (MPI_SUCCESS != (mpi_code = MPI_Type_create_hindexed(
                                   (int)count, mpi_block_lengths, mpi_bufs,
                                   MPI_BYTE, buf_type)))
    ...
    *buf_type_created = TRUE;
    ...
    if (sub_types) {
      if (MPI_SUCCESS != (mpi_code = MPI_Type_create_struct(
                              (int)count, mpi_block_lengths, mpi_displacements,
                              sub_types, file_type)))
        HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_struct for file_type failed",
                        mpi_code)
    } else if (MPI_SUCCESS != (mpi_code = MPI_Type_create_hindexed(
                                   (int)count, mpi_block_lengths,
                                   mpi_displacements, MPI_BYTE, file_type)))
      HMPI_GOTO_ERROR(FAIL, "MPI_Type_create_hindexed for file_type failed",
                      mpi_code)
...
static herr_t H5FD__mpio_read_vector(H5FD_t *_file,
  ...
  MPI_Datatype file_type =
      MPI_BYTE; /* MPI description of the selection in file */
  ...
  H5FD_mpio_collective_opt_t
      coll_opt_mode; /* whether we are doing collective or independent I/O */
  int size_i;
#if MPI_VERSION >= 3
  MPI_Count bytes_read = 0; /* Number of bytes read in */
  MPI_Count type_size;      /* MPI datatype used for I/O's size */
   ...
   * read, it does mean that all ranks are here, so we can use
   * MPI_File_set_view().
   */
  if (H5CX_get_io_xfer_mode(&xfer_mode) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL, "can't get MPI-I/O transfer mode");
  ...
  if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
    /* Build MPI types, etc. */
    if (H5FD__mpio_vector_build_types(
            count, types, addrs, sizes, (H5_flexible_const_ptr_t *)bufs,
      ...
      HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                  "can't build MPI datatypes for I/O");
    ...
    if (coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO) {
      ...
      if (H5CX_get_mpio_rank0_bcast()) {
        ...
        if (file->mpi_rank == 0)
          if (MPI_SUCCESS !=
              (mpi_code = MPI_File_read_at(file->f, mpi_off, mpi_bufs_base,
                                           size_i, buf_type, &mpi_stat)))
            HMPI_GOTO_ERROR(FAIL, "MPI_File_read_at_all failed", mpi_code)
        if (MPI_SUCCESS != (mpi_code = MPI_Bcast(mpi_bufs_base, size_i,
                                                 buf_type, 0, file->comm)))
          HMPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_code)
      } /* end if */
      else if (MPI_SUCCESS !=
               (mpi_code = MPI_File_read_at_all(file->f, mpi_off, mpi_bufs_base,
                                                size_i, buf_type, &mpi_stat)))
        HMPI_GOTO_ERROR(FAIL, "MPI_File_read_at_all failed", mpi_code)
    } /* end if */
    else if (size_i > 0) {
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_read_at(file->f, mpi_off, mpi_bufs_base, size_i,
                                       buf_type, &mpi_stat)))
    ...
    for (i = 0; i < (int)count; i++) {
      ...
      if (size != (size_t)size_i) {
        /* If HERE, then we need to work around the integer size limit
         * of 2GB. The input size_t size variable cannot fit into an integer,
         * but we can get around that limitation by creating a different
         * datatype and then setting the integer size (or element count) to 1
         * when using the derived_type.
         */
      ...
      if (addrs[i] < max_addr) {
        ...
        /* Issue read */
        if (MPI_SUCCESS !=
            (mpi_code = MPI_File_read_at(file->f, mpi_off, bufs[i], size_i,
                                         buf_type, &mpi_stat)))
...
#if MPI_VERSION >= 3
        if (MPI_SUCCESS !=
            (mpi_code = MPI_Get_elements_x(&mpi_stat, MPI_BYTE, &bytes_read)))
#else
        if (MPI_SUCCESS !=
            (mpi_code = MPI_Get_elements(&mpi_stat, MPI_BYTE, &bytes_read)))
#endif
...
static herr_t H5FD__mpio_write_vector(H5FD_t *_file,
   ...
   *
   * While this doesn't mean that we are actually about to do a collective
   * write, it does mean that all ranks are here, so we can use
   * MPI_File_set_view().
   */
  if (H5CX_get_io_xfer_mode(&xfer_mode) < 0)
    HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL, "can't get MPI-I/O transfer mode");
  ...
  if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
    ...
    if (coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO) {
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_write_at_all(file->f, mpi_off, mpi_bufs_base,
                                            size_i, buf_type, &mpi_stat)))
        HMPI_GOTO_ERROR(FAIL, "MPI_File_write_at_all failed", mpi_code)
      ...
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_write_at(file->f, mpi_off, mpi_bufs_base, size_i,
                                        buf_type, &mpi_stat)))
        HMPI_GOTO_ERROR(FAIL, "MPI_File_write_at failed", mpi_code)
    } /* end else */
    ...
    for (i = 0; i < (int)count; i++) {
      ...
      /* Perform write */
      if (MPI_SUCCESS !=
          (mpi_code = MPI_File_write_at(file->f, mpi_off, bufs[i], size_i,
                                        buf_type, &mpi_stat)))
...
static herr_t H5FD__selection_build_types(
  ...
  int *piece_mpi_file_counts =
      NULL; /* Count of MPI file datatype for each piece */
  int *piece_mpi_mem_counts =
      NULL; /* Count of MPI memory datatype for each piece */
  ...
  if (NULL == (piece_mtype = (MPI_Datatype *)H5MM_malloc(num_pieces *
                                                         sizeof(MPI_Datatype))))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate piece memory datatype buffer");
  if (NULL == (piece_ftype = (MPI_Datatype *)H5MM_malloc(num_pieces *
                                                         sizeof(MPI_Datatype))))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate piece file datatype buffer");
  if (NULL == (piece_file_disp_array =
                   (MPI_Aint *)H5MM_malloc(num_pieces * sizeof(MPI_Aint))))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate piece file displacement buffer");
  if (NULL == (piece_mem_disp_array =
                   (MPI_Aint *)H5MM_calloc(num_pieces * sizeof(MPI_Aint))))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate piece memory displacement buffer");
  if (NULL ==
      (piece_mpi_mem_counts = (int *)H5MM_calloc(num_pieces * sizeof(int))))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate piece memory counts buffer");
  if (NULL ==
      (piece_mpi_file_counts = (int *)H5MM_calloc(num_pieces * sizeof(int))))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                "couldn't allocate piece file counts buffer");
  if (NULL == (piece_mmt_is_derived_array =
                   (hbool_t *)H5MM_calloc(num_pieces * sizeof(hbool_t))))
    HGOTO_ERROR(
        H5E_DATASET, H5E_CANTALLOC, FAIL,
        "couldn't allocate piece memory is derived datatype flags buffer");
  if (NULL == (piece_mft_is_derived_array =
                   (hbool_t *)H5MM_calloc(num_pieces * sizeof(hbool_t))))
    HGOTO_ERROR(
        H5E_DATASET, H5E_CANTALLOC, FAIL,
  ...
  for (i = 0; i < num_pieces; i++) {
    hsize_t *permute_map = NULL; /* array that holds the mapping from the old,
                                        out-of-order displacements to the
                                    in-order displacements of the MPI datatypes
                                    of the point selection of the file space */
  ...
  if (MPI_SUCCESS != (mpi_code = MPI_Type_create_struct(
                          (int)num_pieces, piece_mpi_file_counts,
  ...
  *final_ftype_is_derived = TRUE;
  ...
  if (MPI_SUCCESS != (mpi_code = MPI_Type_create_struct(
                          (int)num_pieces, piece_mpi_mem_counts,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_read_selection(
    H5FD_t *_file, H5FD_mem_t type, hid_t H5_ATTR_UNUSED dxpl_id, size_t count,
    hid_t mem_space_ids[], hid_t file_space_ids[], haddr_t offsets[],
  ...
  MPI_Datatype
      final_mtype; /* Final memory MPI datatype for all pieces with selection */
  hbool_t final_mtype_is_derived = FALSE;

  MPI_Datatype
      final_ftype; /* Final file MPI datatype for all pieces with selection */
  ...
  if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
    ...
    if (count) {
      if (H5FD_sort_selection_io_req(
              &selection_was_sorted, count, mem_space_ids, file_space_ids,
              offsets, element_sizes, (H5_flexible_const_ptr_t *)bufs,
              &s_mem_space_ids, &s_file_space_ids, &s_offsets, &s_element_sizes,
              &s_bufs) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                    "can't sort selection I/O request");
      ...
      for (i = 0; i < count; i++) {
        if (NULL == (s_mem_spaces[i] = (H5S_t *)H5I_object_verify(
                         s_mem_space_ids[i], H5I_DATASPACE)))
          HGOTO_ERROR(H5E_VFL, H5E_BADTYPE, H5I_INVALID_HID,
                      "can't retrieve memory dataspace from ID");
        if (NULL == (s_file_spaces[i] = (H5S_t *)H5I_object_verify(
                         s_file_space_ids[i], H5I_DATASPACE)))
          HGOTO_ERROR(H5E_VFL, H5E_BADTYPE, H5I_INVALID_HID,
      ...
      if (H5FD__selection_build_types(
        ...
        HGOTO_ERROR(H5E_VFL, H5E_CANTGET, FAIL,
                    "couldn't build type for MPI-IO");
     ...
     */
    if (MPI_SUCCESS !=
        (mpi_code = MPI_File_set_view(file->f, mpi_off, MPI_BYTE, final_ftype,
                                      H5FD_mpi_native_g, file->info)))
      HMPI_GOTO_ERROR(FAIL, "MPI_File_set_view failed", mpi_code);
    ...
    if (coll_opt_mode == H5FD_MPIO_COLLECTIVE_IO) {
      ...
      if (H5CX_get_mpio_rank0_bcast()) {
        ...
        if (file->mpi_rank == 0) {
          ...
          if (MPI_SUCCESS !=
              (mpi_code = MPI_File_read_at(file->f, mpi_off, mpi_bufs_base,
                                           size_i, final_mtype, &mpi_stat)))
            HMPI_DONE_ERROR(FAIL, "MPI_File_read_at failed", mpi_code);
        }

        if (MPI_SUCCESS != (mpi_code = MPI_Bcast(mpi_bufs_base, size_i,
                                                 final_mtype, 0, file->comm)))
          HMPI_GOTO_ERROR(FAIL, "MPI_Bcast failed", mpi_code);
      } /* end if */
      else
        /* Perform collective read operation */
    ...
    if (!rank0_bcast || (rank0_bcast && file->mpi_rank == 0)) {
      /* How many bytes were actually read? */
#if H5_CHECK_MPI_VERSION(3, 0)
      if (MPI_SUCCESS != (mpi_code = MPI_Get_elements_x(&mpi_stat, final_mtype,
                                                        &bytes_read))) {
#else
      if (MPI_SUCCESS !=
          (mpi_code = MPI_Get_elements(&mpi_stat, MPI_BYTE, &bytes_read))) {
    ...
    if (_file->base_addr > 0) {
      /* Undo base address addition in internal routines before passing down to
       * the mpio driver */
      for (i = 0; i < count; i++) {
    ...
    if (H5FD_read_from_selection(_file, type, (uint32_t)count, mem_space_ids,
                                 file_space_ids, offsets, element_sizes,
                                 bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_READERROR, FAIL,
                  "read vector from selection failed");
  }

done:
  /* Free the MPI buf and file types, if they were derived */
  if (final_mtype_is_derived &&
      MPI_SUCCESS != (mpi_code = MPI_Type_free(&final_mtype)))
    HMPI_DONE_ERROR(FAIL, "MPI_Type_free failed", mpi_code);
  if (final_ftype_is_derived &&
      MPI_SUCCESS != (mpi_code = MPI_Type_free(&final_ftype)))
    HMPI_DONE_ERROR(FAIL, "MPI_Type_free failed", mpi_code);
...
static herr_t H5FD__mpio_write_selection(
  ...
  MPI_Offset mpi_off;
  MPI_Offset
      save_mpi_off;    /* Use at the end of the routine for setting local_eof */
  MPI_Status mpi_stat; /* Status from I/O operation */
  ...
  if (xfer_mode == H5FD_MPIO_COLLECTIVE) {
...
#if H5_CHECK_MPI_VERSION(3, 0)
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Get_elements_x(&mpi_stat, final_mtype, &bytes_written)))
#else
    if (MPI_SUCCESS !=
        (mpi_code = MPI_Get_elements(&mpi_stat, MPI_BYTE, &bytes_written)))
#endif
    ...
    if (bytes_written &&
        (((haddr_t)bytes_written + (haddr_t)save_mpi_off) > file->local_eof))
    ...
    if (H5FD_write_from_selection(_file, type, (uint32_t)count, mem_space_ids,
                                  file_space_ids, offsets, element_sizes,
                                  bufs) < 0)
      HGOTO_ERROR(H5E_VFL, H5E_WRITEERROR, FAIL,
                  "write vector from selection failed");
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_flush(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
                               hbool_t closing) {
  H5FD_mpio_t *file = (H5FD_mpio_t *)_file;
#ifdef H5FDmpio_DEBUG
  hbool_t H5FD_mpio_debug_t_flag =
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__mpio_truncate(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
  ...
  default: /* unknown op code */
    if (flags & H5FD_CTL_FAIL_IF_UNKNOWN_FLAG) {
      ...
      HGOTO_ERROR(H5E_VFL, H5E_FCNTL, FAIL,
                  "unknown op_code and fail if unknown");
    }
    break;
  }
