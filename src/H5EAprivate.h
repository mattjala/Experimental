...
typedef enum H5EA_cls_id_t {
  H5EA_CLS_CHUNK_ID =
      0, /* Extensible array is for indexing dataset chunks w/o filters */
  H5EA_CLS_FILT_CHUNK_ID, /* Extensible array is for indexing dataset chunks
                             w/filters */

  /* Start real class IDs at 0 -QAK */
  /* (keep these last) */
  H5EA_CLS_TEST_ID, /* Extensible array is for testing (do not use for actual
                       data) */
...
typedef struct H5EA_class_t {
  ...
  herr_t (*fill)(void *nat_blk,
                 size_t nelmts); /* Fill array of elements with encoded form of
                                    "missing element" value */
  herr_t (*encode)(
      void *raw, const void *elmt, size_t nelmts,
      void *ctx); /* Encode elements from native form to disk storage form */
  herr_t (*decode)(
      const void *raw, void *elmt, size_t nelmts,
  ...
  void *(*crt_dbg_ctx)(H5F_t *f,
                       haddr_t obj_addr); /* Create debugging context */
...
typedef struct H5EA_create_t {
  ...
  uint8_t max_nelmts_bits; /* Log2(Max. # of elements in array) - i.e. # of bits
                              needed to store max. # of elements */
  ...
  uint8_t sup_blk_min_data_ptrs; /* Min. # of data block pointers for a super
                                    block */
  uint8_t max_dblk_page_nelmts_bits; /* Log2(Max. # of elements in data block
...
typedef struct H5EA_stat_t {
  ...
  struct {
    ...
    hsize_t
        max_idx_set; /* Highest element index stored (+1 - i.e. if element 0 has
                        been set, this value with be '1', if no elements have
                        been stored, this value will be '0') */
