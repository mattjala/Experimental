...
#define H5L_MIN_TABLE_SIZE                                                     \
  32 /* Minimum size of the user-defined link type table if it is allocated */
...
typedef struct {
  ...
  size_t orig_nlinks; /* The original value for the # of soft / UD links that
                         can be traversed */
} H5L_trav_mv_t;
...
static herr_t H5L__get_val_by_idx_cb(H5G_loc_t *grp_loc /*in*/,
                                     const char *name, const H5O_link_t *lnk,
                                     H5G_loc_t *obj_loc,
                                     void *_udata /*in,out*/,
                                     H5G_own_loc_t *own_loc /*out*/);
...
herr_t H5L_init(void) {
  ...
  if (H5L_register_external() < 0)
    HGOTO_ERROR(H5E_LINK, H5E_NOTREGISTERED, FAIL,
                "unable to register external link class");
...
herr_t H5L_register(const H5L_class_t *cls) {
  ...
  if (i >= H5L_table_used_g) {
    if (H5L_table_used_g >= H5L_table_alloc_g) {
      ...
      H5L_class_t *table =
          (H5L_class_t *)H5MM_realloc(H5L_table_g, (n * sizeof(H5L_class_t)));
      if (!table)
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                    "unable to extend link type table");
      H5L_table_g = table;
      H5L_table_alloc_g = n;
    } /* end if */
...
herr_t H5L_unregister(H5L_type_t id) {
  ...
  if (i >= H5L_table_used_g)
    HGOTO_ERROR(H5E_LINK, H5E_NOTREGISTERED, FAIL,
                "link class is not registered");

  /* Remove filter from table */
  /* Don't worry about shrinking table size (for now) */
  memmove(&H5L_table_g[i], &H5L_table_g[i + 1],
          sizeof(H5L_class_t) * ((H5L_table_used_g - 1) - i));
  H5L_table_used_g--;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L_is_registered(H5L_type_t id, hbool_t *is_registered) {
  size_t i; /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L_link(const H5G_loc_t *new_loc, const char *new_name,
  ...
  /* Create the link */
  if (H5L__create_real(new_loc, new_name, obj_loc->path, obj_loc->oloc->file,
                       &lnk, NULL, lcpl_id) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L_link_object(const H5G_loc_t *new_loc, const char *new_name,
  ...
  /* Create the link */
  if (H5L__create_real(new_loc, new_name, NULL, NULL, &lnk, ocrt_info,
                       lcpl_id) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL,
                "unable to create new link to object");
...
static herr_t H5L__link_cb(H5G_loc_t *grp_loc /*in*/, const char *name,
  ...
  hbool_t temp_loc_init = FALSE; /* Temporary location for UD callback
                                    (temp_loc) has been initialized */
  hbool_t obj_created =
      FALSE; /* Whether an object was created (through a hard link) */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (udata->lnk->type == H5L_TYPE_HARD) {
    ...
    if (udata->ocrt_info) {
      ...
      if (NULL == (udata->ocrt_info->new_obj = H5O_obj_create(
                       grp_loc->oloc->file, udata->ocrt_info->obj_type,
  ...
  udata->lnk->corder = 0; /* Will be re-written during group insertion, if the
                             group is tracking creation order */
  ...
  if (udata->lc_plist) {
    ...
    if (H5CX_get_encoding(&udata->lnk->cset) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                  "can't get 'character set' property");
  } /* end if */
  else
    udata->lnk->cset =
        H5F_DEFAULT_CSET; /* Default character encoding for link */
  ...
  if (H5G_obj_insert(grp_loc->oloc, name, udata->lnk, TRUE,
                     udata->ocrt_info ? udata->ocrt_info->obj_type
                                      : H5O_TYPE_UNKNOWN,
                     udata->ocrt_info ? udata->ocrt_info->crt_info : NULL) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL,
                "unable to create new link for object");
  ...
  if (udata->lnk->type >= H5L_TYPE_UD_MIN) {
    ...
    if (NULL == (link_class = H5L_find_class(udata->lnk->type)))
      HGOTO_ERROR(H5E_LINK, H5E_NOTREGISTERED, FAIL,
                  "unable to get class of UD link");
    ...
    if (link_class->create_func != NULL) {
      ...
      if (H5O_loc_copy_deep(&temp_oloc, grp_loc->oloc) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_CANTCOPY, FAIL,
                    "unable to copy object location");
      ...
      if ((grp_id = H5VL_wrap_register(H5I_GROUP, grp, TRUE)) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_CANTREGISTER, FAIL,
                    "unable to register ID for group");

      /* Make callback */
      if ((link_class->create_func)(name, grp_id, udata->lnk->u.ud.udata,
                                    udata->lnk->u.ud.size, H5P_DEFAULT) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_CALLBACK, FAIL,
                    "link creation callback failed");
    } /* end if */
  }   /* end if */
  ...
  if (obj_created) {
    ...
    if (H5O_dec_rc_by_loc(&oloc) < 0)
      HDONE_ERROR(H5E_LINK, H5E_CANTDEC, FAIL,
...
static herr_t H5L__create_real(const H5G_loc_t *link_loc, const char *link_name,
  ...
  unsigned target_flags =
      H5G_TARGET_NORMAL; /* Flags to pass to group traversal function */
  ...
  if (lcpl_id != H5P_DEFAULT) {
    ...
    if (H5CX_get_intermediate_group(&crt_intmd_group) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                  "can't get 'create intermediate group' property");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__create_hard(H5G_loc_t *cur_loc, const char *cur_name,
                        const H5G_loc_t *link_loc, const char *link_name,
                        hid_t lcpl_id) {
  ...
  if (H5L__create_real(link_loc, link_name, NULL, link_file, &lnk, NULL,
                       lcpl_id) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL,
                "unable to create new link to object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__create_soft(const char *target_path, const H5G_loc_t *link_loc,
  ...
  if (H5L__create_real(link_loc, link_name, NULL, NULL, &lnk, NULL, lcpl_id) <
      0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL,
                "unable to create new link to object");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__create_ud(const H5G_loc_t *link_loc, const char *link_name,
  ...
  if (H5L__find_class_idx(type) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL,
                "link class has not been registered with library");
...
static herr_t H5L__get_val_real(const H5O_link_t *lnk, void *buf, size_t size) {
  ...
  /* Check for user-defined link */
    ...
    if (link_class != NULL && link_class->query_func != NULL) {
      if ((link_class->query_func)(lnk->name, lnk->u.ud.udata, lnk->u.ud.size,
                                   buf, size) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_CALLBACK, FAIL,
                    "query callback returned failure");
    } /* end if */
    else if (buf && size > 0)
      ((char *)buf)[0] = '\0';
  } /* end if */
  else
    HGOTO_ERROR(H5E_LINK, H5E_BADTYPE, FAIL,
                "object is not a symbolic or user-defined link");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__get_val(const H5G_loc_t *loc, const char *name, void *buf /*out*/,
                    size_t size) {
  ...
  if (H5G_traverse(loc, name, H5G_TARGET_SLINK | H5G_TARGET_UDLINK,
                   H5L__get_val_cb, &udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__get_val_by_idx(const H5G_loc_t *loc, const char *name,
  ...
  /* Traverse the group hierarchy to locate the object to get info about */
  if (H5G_traverse(loc, name, H5G_TARGET_SLINK | H5G_TARGET_UDLINK,
                   H5L__get_val_by_idx_cb, &udata) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                "can't get link info for index: %llu", (unsigned long long)n);
...
static herr_t H5L__delete_cb(H5G_loc_t *grp_loc /*in*/, const char *name,
  ...
  if (lnk == NULL)
    HGOTO_ERROR(H5E_LINK, H5E_CANTDELETE, FAIL,
                "callback link pointer is NULL (specified link may be '.' or "
                "not exist)");
...
herr_t H5L__delete(const H5G_loc_t *loc, const char *name) {
  ...
  if (H5G_traverse(loc, norm_name,
                   H5G_TARGET_SLINK | H5G_TARGET_UDLINK | H5G_TARGET_MOUNT,
                   H5L__delete_cb, NULL) < 0)
...
static herr_t H5L__delete_by_idx_cb(H5G_loc_t H5_ATTR_UNUSED *grp_loc /*in*/,
  ...
  if (H5G_obj_remove_by_idx(obj_loc->oloc, obj_loc->path->full_path_r,
                            udata->idx_type, udata->order, udata->n) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_NOTFOUND, FAIL, "link not found");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__delete_by_idx(const H5G_loc_t *loc, const char *name,
                          H5_index_t idx_type, H5_iter_order_t order,
                          hsize_t n) {
...
static herr_t H5L__move_dest_cb(H5G_loc_t *grp_loc /*in*/, const char *name,
  ...
  if (obj_loc != NULL)
    HGOTO_ERROR(H5E_LINK, H5E_NOTFOUND, FAIL,
  ...
  if (udata->lnk->type == H5L_TYPE_HARD)
    ...
    if (!H5F_SAME_SHARED(grp_loc->oloc->file, udata->file))
      HGOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL,
                  "moving a link across files is not allowed");
  ...
  if (udata->lnk->type >= H5L_TYPE_UD_MIN) {
    ...
    if ((!udata->copy && link_class->move_func) ||
        (udata->copy && link_class->copy_func)) {
      H5O_loc_t temp_oloc;
      H5G_name_t temp_path;
      ...
      if ((grp_id = H5VL_wrap_register(H5I_GROUP, grp, TRUE)) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_CANTREGISTER, FAIL,
                    "unable to register group ID");

      if (udata->copy) {
        if ((link_class->copy_func)(udata->lnk->name, grp_id,
                                    udata->lnk->u.ud.udata,
                                    udata->lnk->u.ud.size) < 0)
          HGOTO_ERROR(H5E_LINK, H5E_CALLBACK, FAIL,
                      "UD copy callback returned error");
      } /* end if */
      else {
        if ((link_class->move_func)(udata->lnk->name, grp_id,
                                    udata->lnk->u.ud.udata,
                                    udata->lnk->u.ud.size) < 0)
          HGOTO_ERROR(H5E_LINK, H5E_CALLBACK, FAIL,
                      "UD move callback returned error");
      } /* end else */
    }   /* end if */
  }     /* end if */
...
static herr_t H5L__move_cb(H5G_loc_t *grp_loc /*in*/, const char *name,
  ...
  if (lnk == NULL)
    HGOTO_ERROR(H5E_LINK, H5E_NOTFOUND, FAIL,
                "the name of a link must be supplied to move or copy");

  /* Set up user data for move_dest_cb */
  if (NULL ==
      (udata_out.lnk = (H5O_link_t *)H5O_msg_copy(H5O_LINK_ID, lnk, NULL)))
    HGOTO_ERROR(H5E_LINK, H5E_CANTCOPY, FAIL,
                "unable to copy link to be moved");
  ...
  if (H5CX_set_nlinks(udata->orig_nlinks) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTSET, FAIL,
                "can't reset # of soft / UD links to traverse");

  /* Insert the link into its new location */
  if (H5G_traverse(udata->dst_loc, udata->dst_name, udata->dst_target_flags,
                   H5L__move_dest_cb, &udata_out) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_NOTFOUND, FAIL, "unable to follow symbolic link");

  /* If this is a move and not a copy operation, change the object's name and
   * remove the old link */
  if (!udata->copy) {
    ...
    if (*(udata->dst_name) != '/') {
      ...
      if ((dst_name_r = H5G_build_fullpath_refstr_str(
               udata->dst_loc->path->full_path_r, udata->dst_name)) == NULL)
        HGOTO_ERROR(H5E_LINK, H5E_PATH, FAIL,
                    "can't build destination path name");
    } /* end if */
    else
    ...
    /* Fix names up */
    if (H5G_name_replace(lnk, H5G_NAME_MOVE, obj_loc->oloc->file,
                         obj_loc->path->full_path_r, udata->dst_loc->oloc->file,
                         dst_name_r) < 0) {
    ...
    /* Remove the old link */
    if (H5G_obj_remove(grp_loc->oloc, grp_loc->path->full_path_r, orig_name) <
        0) {
      H5RS_decr(dst_name_r);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__move(const H5G_loc_t *src_loc, const char *src_name,
                 const H5G_loc_t *dst_loc, const char *dst_name,
  ...
  if (lcpl_id != H5P_DEFAULT) {
    ...
    if (H5CX_get_intermediate_group(&crt_intmd_group) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get property value for creating missing groups");
    ...
    if (H5CX_get_encoding(&char_encoding) < 0)
      HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL,
                  "can't get property value for character encoding");
  } /* end if */
  ...
  if (H5CX_get_nlinks(&udata.orig_nlinks) < 0)
    HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                "unable to retrieve # of soft / UD links to traverse");

  /* Do the move */
  if (H5G_traverse(src_loc, src_name,
                   H5G_TARGET_MOUNT | H5G_TARGET_SLINK | H5G_TARGET_UDLINK,
                   H5L__move_cb, &udata) < 0)
...
static herr_t H5L__exists_inter_cb(H5G_loc_t H5_ATTR_UNUSED *grp_loc /*in*/,
  ...
  if (lnk != NULL) {
    ...
    if (udata->sep) {
      ...
      if (H5G_traverse(obj_loc, next, H5G_TARGET_SLINK | H5G_TARGET_UDLINK,
                       cb_func, udata) < 0)
        HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                    "can't determine if link exists");
    } /* end if */
    else
      *udata->exists = TRUE;
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L_exists_tolerant(const H5G_loc_t *loc, const char *name,
                           hbool_t *exists) {
  ...
  if ('\0' == *name_trav)
    *exists = TRUE;
    ...
    if (H5G_traverse(loc, name_trav, H5G_TARGET_SLINK | H5G_TARGET_UDLINK,
                     cb_func, &udata) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_CANTGET, FAIL,
                  "can't determine if link exists");
  }
 ...
 * Purpose:	Returns whether a link exists in a group
 *
 * Note:	Same as H5L_exists_tolerant, except that missing links are
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__exists(const H5G_loc_t *loc, const char *name, hbool_t *exists) {
  H5L_trav_le_t udata;        /* User data for traversal */
  herr_t ret_value = SUCCEED; /* Return value */
...
H5L__get_info_cb(H5G_loc_t *grp_loc /*in*/, const char H5_ATTR_UNUSED *name,
                 const H5O_link_t *lnk, H5G_loc_t H5_ATTR_UNUSED *obj_loc,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L_get_info(const H5G_loc_t *loc, const char *name,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__get_info_by_idx(const H5G_loc_t *loc, const char *name,
                            H5_index_t idx_type, H5_iter_order_t order,
  ...
  /* Traverse the group hierarchy to locate the object to get info about */
  if (H5G_traverse(loc, name, H5G_TARGET_SLINK | H5G_TARGET_UDLINK,
                   H5L__get_info_by_idx_cb, &udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__get_name_by_idx(const H5G_loc_t *loc, const char *group_name,
                            H5_index_t idx_type, H5_iter_order_t order,
  ...
  if (H5G_traverse(loc, group_name, H5G_TARGET_SLINK | H5G_TARGET_UDLINK,
                   H5L__get_name_by_idx_cb, &udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L__link_copy_file(H5F_t *dst_file, const H5O_link_t *_src_lnk,
                           const H5O_loc_t *src_oloc, H5O_link_t *dst_lnk,
                           H5O_copy_t *cpy_info) {
  H5O_link_t tmp_src_lnk; /* Temporary copy of src link, when needed */
  const H5O_link_t *src_lnk = _src_lnk; /* Source link */
  hbool_t dst_lnk_init =
      FALSE; /* Whether the destination link is initialized */
  hbool_t expanded_link_open =
      FALSE;                  /* Whether the target location has been opened */
  ...
  if ((H5L_TYPE_SOFT == src_lnk->type && cpy_info->expand_soft_link) ||
    ...
    if (H5G_loc_exists(&lnk_grp_loc, src_lnk->name, &tar_exists) < 0)
      HGOTO_ERROR(H5E_LINK, H5E_CANTCOPY, FAIL,
                  "unable to check if target object exists");
  ...
  if (H5L_TYPE_HARD == src_lnk->type) {
    ...
    if (H5O_copy_header_map(&tmp_src_oloc, &new_dst_oloc, cpy_info, TRUE, NULL,
                            NULL) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5L_iterate(H5G_loc_t *loc, const char *group_name, H5_index_t idx_type,
  ...
  if ((ret_value = H5G_iterate(loc, group_name, idx_type, order, idx, &last_lnk,
                               &lnk_op, op_data)) < 0)
