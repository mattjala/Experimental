 ...
 *-------------------------------------------------------------------------
 */
H5C_t *H5C_create(size_t max_cache_size, size_t min_clean_size, int max_type_id,
                  const H5C_class_t *const *class_table_ptr,
                  H5C_write_permitted_func_t check_write_permitted,
                  hbool_t write_permitted, H5C_log_flush_func_t log_flush,
                  void *aux_ptr) {
  int i;
  ...
  if (NULL == (cache_ptr->log_info =
                   (H5C_log_info_t *)H5MM_calloc(sizeof(H5C_log_info_t))))
...
#ifdef H5_HAVE_PARALLEL
  if ((H5F_INTENT(f) & H5F_ACC_RDWR) && !image_generated &&
      cache_ptr->aux_ptr != NULL && f->shared->fs_persist) {
...
herr_t H5C_set_cache_auto_resize_config(H5C_t *cache_ptr,
  ...
  if (H5C_validate_resize_config(config_ptr,
                                 H5C_RESIZE_CFG__VALIDATE_INTERACTIONS) < 0)
    HGOTO_ERROR(H5E_ARGS, H5E_BADRANGE, FAIL,
  ...
  cache_ptr->resize_enabled =
      cache_ptr->size_increase_possible || cache_ptr->size_decrease_possible;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_set_slist_enabled(H5C_t *cache_ptr, hbool_t slist_enabled,
                             hbool_t clear_slist) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_validate_resize_config(H5C_auto_size_ctl_t *config_ptr,
                                  unsigned int tests) {
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 * Function:    H5C_cork
 *
 * Purpose:     To cork/uncork/get cork status of an object depending on
 *"action": H5C__SET_CORK: To cork the object Return error if the object is
 *already corked H5C__UNCORK: To uncork the object Return error if the object is
 *not corked H5C__GET_CORKED: To retrieve the cork status of an object in the
 *parameter "corked"
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C_cork(H5C_t *cache_ptr, haddr_t obj_addr, unsigned action,
                hbool_t *corked) {
  ...
  assert(action == H5C__SET_CORK || action == H5C__UNCORK ||
         action == H5C__GET_CORKED);
  ...
  if (H5C__GET_CORKED == action) {
    ...
    if (H5C__SET_CORK == action) {
      ...
      /* Sanity check */
      if (NULL == tag_info)
        HGOTO_ERROR(H5E_CACHE, H5E_CANTUNCORK, FAIL,
                    "tag info pointer is NULL");
