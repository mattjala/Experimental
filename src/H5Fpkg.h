...
#define H5F_FS_MERGE_METADATA                                                  \
  0x01 /* Section can merge with metadata aggregator */
...
#define H5F_SUPERBLOCK_FIXED_SIZE                                              \
  (H5F_SIGNATURE_LEN + 1) /* superblock version */
...
#define H5F_SUPERBLOCK_SIZE(s)                                                 \
  (H5F_SUPERBLOCK_FIXED_SIZE + H5F_SUPERBLOCK_VARLEN_SIZE((s)->super_vers,     \
                                                          (s)->sizeof_addr,    \
                                                          (s)->sizeof_size))

/* For superblock version 0 & 1:
   Offset to the file consistency flags (status_flags) in the superblock
   (excluding H5F_SUPERBLOCK_FIXED_SIZE)
 */
...
#define H5F_SUPER_STATUS_OFF(v)                                                \
  (v >= 2 ? (unsigned)2 : H5F_SUPER_STATUS_OFF_V01)
...
typedef struct H5F_meta_accum_t {
  ...
  size_t alloc_size; /* Size of the accumulated metadata buffer allocated (in
                        bytes) */
...
struct H5F_shared_t {
  ...
  uint8_t sizeof_size; /* Size of offsets in file              */
  haddr_t
      sohm_addr; /* Relative address of shared object header message table */
  ...
  hbool_t evict_on_close; /* If the file's objects should be evicted from the
                             metadata cache on close */
  ...
  const H5VL_class_t
      *vol_cls;   /* Pointer to VOL connector class for the container */
  ...
  hbool_t point_of_no_return; /* Flag to indicate that we can't go back and
                                 delete a freespace header when it's used up */

  H5F_fs_state_t fs_state[H5F_MEM_PAGE_NTYPES]; /* State of free space manager
                                                   for each type */
  haddr_t fs_addr[H5F_MEM_PAGE_NTYPES]; /* Address of free space manager info
                                           for each type */
  H5FS_t *fs_man[H5F_MEM_PAGE_NTYPES]; /* Free space manager for each file space
                                          type */
  ...
  H5FD_mem_t fs_type_map[H5FD_MEM_NTYPES]; /* Mapping of "real" file space type
                                              into tracked type */
  H5F_blk_aggr_t meta_aggr;  /* Metadata aggregation info (if aggregating
                                metadata allocations) */
  H5F_blk_aggr_t sdata_aggr; /* "Small data" aggregation info (if aggregating
                                "small data" allocations) */

  /* Paged aggregation info */
  hsize_t fs_page_size;    /* File space page size */
  size_t pgend_meta_thres; /* Do not track page end meta section <= this
                              threshold */
  ...
  /* Metadata retry info */
  unsigned read_attempts; /* The # of reads to try when reading metadata with
                             checksum */
  unsigned retries_nbins; /* # of bins for each retries[] */
  uint32_t *retries[H5AC_NTYPES]; /* Track # of read retries for metadata items
                                     with checksum */

  /* Object flush info */
  H5F_object_flush_t object_flush; /* Information for object flush callback */
  hbool_t crt_dset_min_ohdr_flag;  /* flag to minimize created dataset object
                                      header */
...
struct H5F_t {
  ...
  H5FO_t *obj_count;      /* # of time each object is opened through top file
                             structure   */
...
H5_DLL herr_t H5F__get_file_image(H5F_t *f, void *buf_ptr, size_t buf_len,
                                  size_t *image_len);
...
H5_DLL herr_t H5F__set_libver_bounds(H5F_t *f, H5F_libver_t low,
                                     H5F_libver_t high);
...
H5_DLL herr_t H5F__mount_count_ids(H5F_t *f, unsigned *nopen_files,
                                   unsigned *nopen_objs);

/* Superblock related routines */
H5_DLL herr_t H5F__super_init(H5F_t *f);
H5_DLL herr_t H5F__super_read(H5F_t *f, H5P_genplist_t *fa_plist,
                              hbool_t initial_read);
H5_DLL herr_t H5F__super_size(H5F_t *f, hsize_t *super_size,
                              hsize_t *super_ext_size);
H5_DLL herr_t H5F__super_free(H5F_super_t *sblock);

/* Superblock extension related routines */
H5_DLL herr_t H5F__super_ext_open(H5F_t *f, haddr_t ext_addr,
                                  H5O_loc_t *ext_ptr);
...
H5_DLL herr_t H5F__super_ext_close(H5F_t *f, H5O_loc_t *ext_ptr,
                                   hbool_t was_created);

/* Metadata accumulator routines */
H5_DLL herr_t H5F__accum_read(H5F_shared_t *f_sh, H5FD_mem_t type, haddr_t addr,
                              size_t size, void *buf);
H5_DLL herr_t H5F__accum_write(H5F_shared_t *f_sh, H5FD_mem_t type,
                               haddr_t addr, size_t size, const void *buf);
H5_DLL herr_t H5F__accum_free(H5F_shared_t *f, H5FD_mem_t type, haddr_t addr,
                              hsize_t size);
...
H5_DLL H5F_t *H5F__efc_open(H5F_efc_t *efc, const char *name, unsigned flags,
...
H5_DLL haddr_t H5F__alloc(H5F_t *f, H5F_mem_t type, hsize_t size,
...
/* Testing functions */
#ifdef H5F_TESTING
H5_DLL herr_t H5F__get_sohm_mesg_count_test(hid_t fid, unsigned type_id,
                                            size_t *mesg_count);
