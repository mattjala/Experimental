...
static const H5I_class_t H5I_SPACE_SEL_ITER_CLS[1] = {{
    ...
    (H5I_free_t)H5S__sel_iter_close_cb /* Callback routine for closing objects
                                          of this class */
}};
...
herr_t H5S_init(void) {
  ...
  if (H5I_register_type(H5I_DATASPACE_CLS) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                "unable to initialize dataspace ID class");
...
H5S_t *H5S_create(H5S_class_t type) {
  ...
  if (H5O_msg_reset_share(H5O_SDSPACE_ID, &(new_ds->extent.sh_loc)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTRESET, NULL,
                "unable to reset shared component info");
...
hid_t H5Screate(H5S_class_t type) {
  ...
  if (type <= H5S_NO_CLASS ||
      type > H5S_NULL) /* don't allow complex dataspace yet */
...
herr_t H5S_close(H5S_t *ds) {
  ...
  if (H5S_SELECT_RELEASE(ds) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                "unable to release dataspace selection");

  /* Release extent */
  if (H5S__extent_release(&ds->extent) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, FAIL,
                "unable to release dataspace extent");
...
hid_t H5Scopy(hid_t space_id) {
  ...
  if (ret_value < 0)
    if (dst && H5S_close(dst) < 0)
      HDONE_ERROR(H5E_DATASPACE, H5E_CANTRELEASE, H5I_INVALID_HID,
                  "unable to release dataspace");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5S__extent_copy_real(H5S_extent_t *dst, const H5S_extent_t *src,
                             hbool_t copy_max) {
  unsigned u;
  herr_t ret_value = SUCCEED; /* Return value */
 ...
 *              H5S_extent_copy and H5S_select_copy.  If the SHARE_SELECTION
 *flag is set, then the selection can be shared between the source and
...
hsize_t H5S_get_npoints_max(const H5S_t *ds) {
  ...
  default:
    ...
    HGOTO_ERROR(H5E_DATASPACE, H5E_UNSUPPORTED, 0,
                "internal error (unknown dataspace class)");
  }
 ...
 *-------------------------------------------------------------------------
 */
int H5Sget_simple_extent_dims(hid_t space_id, hsize_t dims[] /*out*/,
                              hsize_t maxdims[] /*out*/) {
  H5S_t *ds;
  int ret_value = -1;
 ...
 *-------------------------------------------------------------------------
 */
int H5S_extent_get_dims(const H5S_extent_t *ext, hsize_t dims[],
                        hsize_t max_dims[]) {
 ...
 *-------------------------------------------------------------------------
 */
int H5S_get_simple_extent_dims(const H5S_t *ds, hsize_t dims[],
                               hsize_t max_dims[]) {
  int ret_value = -1; /* Return value */
  ...
  if ((ret_value = H5S_extent_get_dims(&ds->extent, dims, max_dims)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, FAIL,
                "can't retrieve dataspace extent dims");
...
herr_t H5S_write(H5F_t *f, H5O_t *oh, unsigned update_flags, H5S_t *ds) {
  ...
  if (H5O_msg_write_oh(f, oh, H5O_SDSPACE_ID, 0, update_flags, &(ds->extent)) <
      0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                "can't update simple dataspace message");
...
herr_t H5S_append(H5F_t *f, H5O_t *oh, H5S_t *ds) {
  ...
  if (H5O_msg_append_oh(f, oh, H5O_SDSPACE_ID, 0, 0, &(ds->extent)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL,
                "can't add simple dataspace message");
...
static htri_t H5S__is_simple(const H5S_t *sdim) {
  ...
  ret_value = (H5S_GET_EXTENT_TYPE(sdim) == H5S_SIMPLE ||
               H5S_GET_EXTENT_TYPE(sdim) == H5S_SCALAR)
                  ? TRUE
                  : FALSE;
...
herr_t H5S_set_extent_simple(H5S_t *space, unsigned rank, const hsize_t *dims,
  ...
  if (H5S__extent_release(&space->extent) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTFREE, FAIL,
                "failed to release previous dataspace extent");
  ...
  if (rank == 0) { /* scalar variable */
    ...
    /* Copy the maximum dimensions if specified. Otherwise, the maximal
     * dimensions are the same as the dimension */
 ...
 *-------------------------------------------------------------------------
 */
hid_t H5Screate_simple(int rank, const hsize_t dims[/*rank*/],
  ...
  for (i = 0; i < rank; i++) {
    if (H5S_UNLIMITED == dims[i])
      HGOTO_ERROR(
          H5E_ARGS, H5E_BADVALUE, H5I_INVALID_HID,
          "current dimension must have a specific size, not H5S_UNLIMITED");
  ...
  if (NULL == (space = H5S_create_simple((unsigned)rank, dims, maxdims)))
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTCREATE, H5I_INVALID_HID,
 ...
 *-------------------------------------------------------------------------
 */
H5S_t *H5S_create_simple(unsigned rank, const hsize_t dims[/*rank*/],
                         const hsize_t maxdims[/*rank*/]) {
  H5S_t *ret_value = NULL; /* Return value */
...
herr_t H5S_encode(H5S_t *obj, unsigned char **p, size_t *nalloc) {
  ...
  if ((sselect_size = H5S_SELECT_SERIAL_SIZE(obj)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_BADSIZE, FAIL,
                "can't find dataspace selection size");
  ...
  if (!*p || *nalloc < (extent_size + select_size + 1 + 1 + 1 + 4))
    ...
    *pp++ = (unsigned char)H5F_SIZEOF_SIZE(f);
    ...
    /* Encode size of extent information. Pointer is actually moved in this
     * macro. */
    UINT32ENCODE(pp, extent_size);

    /* Encode the extent part of dataspace */
    if (H5O_msg_encode(f, H5O_SDSPACE_ID, TRUE, pp, obj) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTENCODE, FAIL,
                  "can't encode extent space");
    pp += extent_size;
...
hid_t H5Sdecode(const void *buf) {
  ...
  if ((ds = H5S_decode((const unsigned char **)&buf)) == NULL)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTDECODE, H5I_INVALID_HID,
                "can't decode object");

  /* Register the type and return the ID */
  if ((ret_value = H5I_register(H5I_DATASPACE, ds, TRUE)) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTREGISTER, H5I_INVALID_HID,
                "unable to register dataspace");
...
H5S_t *H5S_decode(const unsigned char **p) {
  ...
  if (*pp++ != H5S_ENCODE_VERSION)
    HGOTO_ERROR(H5E_DATASPACE, H5E_VERSION, NULL,
                "unknown version of encoded dataspace");
  ...
  if (NULL == (extent = (H5S_extent_t *)H5O_msg_decode(f, NULL, H5O_SDSPACE_ID,
                                                       extent_size, pp)))
  ...
  if (H5S__extent_release(extent) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTDELETE, NULL,
                "can't release previous dataspace");
  extent = H5FL_FREE(H5S_extent_t, extent);

  /* Initialize to "all" selection. Deserialization relies on valid existing
   * selection. */
  if (H5S_select_all(ds, FALSE) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, NULL,
                "unable to set all selection");
   ...
   *  Because size of buffer is unknown, assume arbitrarily large buffer to
   * allow decoding. */
  *p = pp;
  if (H5S_SELECT_DESERIALIZE(&ds, p, SIZE_MAX) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTDECODE, NULL,
                "can't decode space selection");
 ...
 *
 * Purpose:     Internal function for retrieving the type of extent for a
 *dataspace object
 *
...
herr_t H5Sset_extent_none(hid_t space_id) {
  ...
  if (H5S__extent_release(&space->extent) < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTDELETE, FAIL,
                "can't release previous dataspace");
    ...
    if (space->extent.size[u] != size[u]) {
      ...
      if (space->extent.max && H5S_UNLIMITED != space->extent.max[u] &&
        ...
        HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL,
                    "dimension cannot exceed the existing maximal size (new: "
                    "%llu max: %llu)",
  ...
  if (ret_value)
    if (H5S_set_extent_real(space, size) < 0)
      HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSET, FAIL,
                  "failed to change dimension size(s)");
...
H5_ATTR_PURE hbool_t H5S_has_extent(const H5S_t *ds) {
  ...
  if (0 == ds->extent.rank && 0 == ds->extent.nelem &&
      H5S_NULL != ds->extent.type)
    ret_value = FALSE;
  else
    ret_value = TRUE;
...
herr_t H5S_set_extent_real(H5S_t *space, const hsize_t *size) {
  ...
  if (H5O_msg_reset_share(H5O_SDSPACE_ID, space) < 0)
    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTRESET, FAIL,
                "can't stop sharing dataspace");
...
htri_t H5Sextent_equal(hid_t space1_id, hid_t space2_id) {
  ...
  /* check args */
  if (NULL ==
          (ds1 = (const H5S_t *)H5I_object_verify(space1_id, H5I_DATASPACE)) ||
      NULL ==
          (ds2 = (const H5S_t *)H5I_object_verify(space2_id, H5I_DATASPACE)))
    HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a dataspace");
...
herr_t H5S_set_version(H5F_t *f, H5S_t *ds) {
  ...
  if (version > H5O_sdspace_ver_bounds[H5F_HIGH_BOUND(f)])
    HGOTO_ERROR(H5E_DATASET, H5E_BADRANGE, FAIL,
                "Dataspace version out of bounds");
