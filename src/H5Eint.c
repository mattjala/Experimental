 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5E__get_msg(const H5E_msg_t *msg, H5E_type_t *type, char *msg_str,
                     size_t size) {
...
static herr_t H5E__walk1_cb(int n, H5E_error1_t *err_desc, void *client_data) {
  ...
  unsigned have_desc =
      1; /* Flag to indicate whether the error has a "real" description */
  ...
  if (eprint->cls.lib_name == NULL ||
      HDstrcmp(cls_ptr->lib_name, eprint->cls.lib_name) != 0) {
  ...
  fprintf(stream, "%*s#%03d: %s line %u in %s()%s%s\n", H5E_INDENT, "", n,
          ...
          (have_desc ? ": " : ""), (have_desc ? err_desc->desc : ""));
...
static herr_t H5E__walk2_cb(unsigned n, const H5E_error2_t *err_desc,
  ...
  /* Get error class info.  Don't use the class of the major or minor error
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5E__walk(const H5E_t *estack, H5E_direction_t direction,
    ...
    if (op->u.func1) {
      ...
      if (H5E_WALK_UPWARD == direction) {
        for (i = 0; i < (int)estack->nused && ret_value == H5_ITER_CONT; i++) {
          /* Point to each error record on the stack and pass it to callback
           * function.*/
        ...
        for (i = (int)(estack->nused - 1); i >= 0 && ret_value == H5_ITER_CONT;
             i--) {
          /* Point to each error record on the stack and pass it to callback
           * function.*/
          old_err.maj_num = estack->slot[i].maj_num;
          ...
          ret_value = (op->u.func1)((int)(estack->nused - (size_t)(i + 1)),
                                    &old_err, client_data);
        } /* end for */
      }   /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5E__get_auto(const H5E_t *estack, H5E_auto_op_t *op,
                     void **client_data) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5E__set_auto(H5E_t *estack, const H5E_auto_op_t *op,
                     void *client_data) {
  FUNC_ENTER_PACKAGE_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5E_printf_stack(H5E_t *estack, const char *file, const char *func,
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5E__push_stack(H5E_t *estack, const char *file, const char *func,
                       unsigned line, hid_t cls_id, hid_t maj_id, hid_t min_id,
                       const char *desc) {
  herr_t ret_value = SUCCEED; /* Return value */
...
static herr_t H5E__clear_entries(H5E_t *estack, size_t nentries) {
  ...
  for (u = 0; nentries > 0; nentries--, u++) {
     ...
     */
    /* (In reverse order that they were incremented, so that reference counts
     * work well) */
...
herr_t H5E_clear_stack(H5E_t *estack) {
  ...
  if (estack == NULL) {
    ...
    /* Set estack to point to the default (in this case thread local) error
     * stack */
...
herr_t H5E__pop(H5E_t *estack, size_t count) {
  ...
  if (H5E__clear_entries(estack, count) < 0)
    HGOTO_ERROR(H5E_ERROR, H5E_CANTRELEASE, FAIL,
                "can't remove errors from stack");
