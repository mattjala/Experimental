...
#define H5HF_DTABLE_INFO_SIZE(h)                                               \
  ((unsigned)2                  /* Width of table (i.e. # of columns) */       \
   ...
   + (unsigned)2 /* Max. size of heap (log2 of actual value - i.e. the # of    \
                    bits) */                                                   \
...
#define H5HF_MAN_INDIRECT_CHILD_DIR_ENTRY_SIZE(h)                              \
  (((h)->filter_len > 0 ? ((unsigned)(h)->sizeof_addr +                        \
                           (unsigned)(h)->sizeof_size + (unsigned)4)           \
...
#define H5HF_SIZEOF_OFFSET_LEN(l)                                              \
  H5HF_SIZEOF_OFFSET_BITS(H5VM_log2_of2((unsigned)(l)))
...
#define H5HF_ID_TYPE_MAN                                                       \
  0x00 /* "Managed" object - stored in fractal heap blocks */
...
typedef struct H5HF_dtable_t {
  ...
  unsigned
      curr_root_rows; /* Current number of rows in the root indirect block */
  ...
  unsigned start_bits;      /* # of bits for starting block size (i.e.
                               log2(start_block_size)) */
...
typedef struct H5HF_block_loc_t {
  ...
  /* Infrastructure */
  H5HF_indirect_t
      *context; /* Pointer to the indirect block containing the block */
  struct H5HF_block_loc_t
      *up; /* Pointer to next level up in the stack of levels */
} H5HF_block_loc_t;
...
typedef struct H5HF_block_iter_t {
  ...
  H5HF_block_loc_t
      *curr; /* Pointer to the current level information for iterator */
} H5HF_block_iter_t;
...
typedef struct H5HF_free_section_t {
  ...
  union {
    struct {
      ...
      unsigned par_entry; /* Entry of free section's direct block in parent
                             indirect block */
    } single;
    struct {
      ...
      hbool_t checked_out; /* Flag to indicate that a row section is temporarily
                              out of the free space manager */
    } row;
    struct {
      ...
      struct H5HF_free_section_t *
          *indir_ents; /* Array of pointers to outstanding child indirect
                          sections */
    } indirect;
  } u;
} H5HF_free_section_t;
...
struct H5HF_hdr_t {
  ...
  hbool_t checksum_dblocks; /* Should the direct blocks in the heap be
                               checksummed? */
  ...
  hsize_t
      huge_next_id; /* Next ID to use for indirectly tracked 'huge' object */
  haddr_t
      huge_bt2_addr; /* Address of v2 B-tree for tracking "huge" object info */
  ...
  unsigned pline_root_direct_filter_mask; /* I/O filter mask for filtered root
                                             direct block */

  /* Statistics for heap (stored in header) */
  hsize_t man_size; /* Total amount of 'managed' space in heap */
  hsize_t
      man_alloc_size;   /* Total amount of allocated 'managed' space in heap */
  ...
  unsigned root_iblock_flags; /* Flags to indicate whether root indirect block
                                 is pinned/protected */
  ...
  hsize_t
      huge_max_id; /* Max. 'huge' heap ID before rolling 'huge' heap IDs over */
  uint8_t huge_id_size;      /* Size of 'huge' heap IDs (in bytes) */
  hbool_t huge_ids_direct;   /* Flag to indicate that 'huge' object's offset &
                                length are stored directly in   heap ID */
  size_t tiny_max_len;       /* Max. size of tiny objects for this heap */
  hbool_t tiny_len_extended; /* Flag to indicate that 'tiny' object's length is
                                stored in extended form (i.e. w/extra byte) */
...
typedef struct H5HF_indirect_filt_ent_t {
  size_t
      size; /* Size of child direct block, after passing though I/O filters */
...
typedef struct H5HF_direct_t {
  ...
  hsize_t file_size;  /* Size of direct block in file (only valid when block's
                         space is being freed) */
...
typedef struct H5HF_huge_bt2_indir_rec_t {
  ...
  hsize_t id;   /* ID used for object (not used for 'huge' objects directly
                   accessed) */
} H5HF_huge_bt2_indir_rec_t;

/* Typedef for indirectly accessed, filtered 'huge' object's records in the v2
 * B-tree */
typedef struct H5HF_huge_bt2_filt_indir_rec_t {
  ...
  unsigned filter_mask; /* I/O pipeline filter mask for filtered object in the
                           file */
...
/* Typedef for directly accessed, filtered 'huge' object's records in the v2
 * B-tree */
...
/* H5HF single section inherits serializable properties from
 * H5FS_section_class_t */
H5_DLLVAR H5FS_section_class_t H5HF_FSPACE_SECT_CLS_SINGLE[1];

/* H5HF 'first' row section inherits serializable properties from
 * H5FS_section_class_t */
H5_DLLVAR H5FS_section_class_t H5HF_FSPACE_SECT_CLS_FIRST_ROW[1];

/* H5HF 'normal' row section inherits serializable properties from
 * H5FS_section_class_t */
H5_DLLVAR H5FS_section_class_t H5HF_FSPACE_SECT_CLS_NORMAL_ROW[1];

/* H5HF indirect section inherits serializable properties from
 * H5FS_section_class_t */
...
H5_DLL unsigned H5HF__dtable_size_to_row(const H5HF_dtable_t *dtable,
                                         size_t block_size);
H5_DLL unsigned H5HF__dtable_size_to_rows(const H5HF_dtable_t *dtable,
                                          hsize_t size);
H5_DLL hsize_t H5HF__dtable_span_size(const H5HF_dtable_t *dtable,
...
H5_DLL herr_t H5HF__hdr_adjust_heap(H5HF_hdr_t *hdr, hsize_t new_size,
                                    hssize_t extra_free);
H5_DLL herr_t H5HF__hdr_inc_alloc(H5HF_hdr_t *hdr, size_t alloc_size);
H5_DLL herr_t H5HF__hdr_start_iter(H5HF_hdr_t *hdr, H5HF_indirect_t *iblock,
...
H5_DLL herr_t H5HF__hdr_inc_iter(H5HF_hdr_t *hdr, hsize_t adv_size,
                                 unsigned nentries);
...
H5_DLL herr_t H5HF__man_iblock_entry_addr(H5HF_indirect_t *iblock,
                                          unsigned entry, haddr_t *child_addr);
...
H5_DLL herr_t H5HF__man_iblock_size(H5F_t *f, H5HF_hdr_t *hdr,
                                    haddr_t iblock_addr, unsigned nrows,
                                    H5HF_indirect_t *par_iblock,
                                    unsigned par_entry,
...
H5_DLL herr_t H5HF__man_dblock_create(H5HF_hdr_t *hdr,
                                      H5HF_indirect_t *par_iblock,
                                      unsigned par_entry, haddr_t *addr_p,
                                      H5HF_free_section_t **ret_sec_node);
...
H5HF__man_dblock_protect(H5HF_hdr_t *hdr, haddr_t dblock_addr,
                         ...
                         unsigned par_entry, unsigned flags);
H5_DLL herr_t H5HF__man_dblock_locate(H5HF_hdr_t *hdr, hsize_t obj_off,
                                      H5HF_indirect_t **par_iblock,
                                      unsigned *par_entry,
                                      hbool_t *par_did_protect, unsigned flags);
...
H5_DLL herr_t H5HF__man_insert(H5HF_hdr_t *fh, size_t obj_size, const void *obj,
                               void *id);
...
H5_DLL herr_t H5HF__man_write(H5HF_hdr_t *hdr, const uint8_t *id,
                              const void *obj);
H5_DLL herr_t H5HF__man_op(H5HF_hdr_t *hdr, const uint8_t *id,
...
H5_DLL herr_t H5HF__huge_insert(H5HF_hdr_t *hdr, size_t obj_size, void *obj,
                                void *id);
...
H5_DLL herr_t H5HF__tiny_insert(H5HF_hdr_t *hdr, size_t obj_size,
...
H5_DLL herr_t H5HF__man_iter_start_offset(H5HF_hdr_t *hdr,
                                          ...
                                          hsize_t offset);
H5_DLL herr_t H5HF__man_iter_start_entry(H5HF_hdr_t *hdr,
                                         H5HF_block_iter_t *biter,
...
H5_DLL herr_t H5HF__man_iter_down(H5HF_block_iter_t *biter,
                                  H5HF_indirect_t *iblock);
H5_DLL herr_t H5HF__man_iter_reset(H5HF_block_iter_t *biter);
H5_DLL herr_t H5HF__man_iter_curr(H5HF_block_iter_t *biter, unsigned *row,
                                  unsigned *col, unsigned *entry,
...
H5_DLL herr_t H5HF_get_id_off_test(const H5HF_t *fh, const void *id,
                                   hsize_t *obj_off);
