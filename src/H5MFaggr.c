...
static haddr_t H5MF__aggr_alloc(H5F_t *f, H5F_blk_aggr_t *aggr,
                                H5F_blk_aggr_t *other_aggr, H5FD_mem_t type,
                                hsize_t size);
static herr_t H5MF__aggr_reset(H5F_t *f, H5F_blk_aggr_t *aggr);
static htri_t H5MF__aggr_can_shrink_eoa(H5F_t *f, H5FD_mem_t type,
                                        H5F_blk_aggr_t *aggr);
...
haddr_t H5MF_aggr_vfd_alloc(H5F_t *f, H5FD_mem_t alloc_type, hsize_t size) {
  ...
  if (alloc_type != H5FD_MEM_DRAW && alloc_type != H5FD_MEM_GHEAP) {
    ...
    if (HADDR_UNDEF == (ret_value = H5MF__aggr_alloc(f, &(f->shared->meta_aggr),
                                                     &(f->shared->sdata_aggr),
                                                     alloc_type, size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, HADDR_UNDEF,
                  "can't allocate metadata");
  } /* end if */
  else {
    /* Allocate "raw" data: H5FD_MEM_DRAW and H5FD_MEM_GHEAP */
    if (HADDR_UNDEF == (ret_value = H5MF__aggr_alloc(
                            f, &(f->shared->sdata_aggr),
...
static haddr_t H5MF__aggr_alloc(H5F_t *f, H5F_blk_aggr_t *aggr,
  ...
  /*
   * If the aggregation feature is enabled for this file and strategy is not
   * H5F_FILE_SPACE_NONE, allocate "generic" space and sub-allocate out of that,
   * if possible. Otherwise just allocate through H5F__alloc().
   */
   ...
   *
   * When closing the destination file after H5Ocopy, the library flushes the
   * fractal heap direct block via H5HF__cache_dblock_pre_serialize().  While
   * doing so, the cache eventually adjusts/evicts ageout entries and ends up
   * flushing out the same entry that is being serialized (flush_in_progress).
   */
  if ((f->shared->feature_flags & aggr->feature_flag) &&
    ...
    if ((f->shared->feature_flags & aggr->feature_flag) &&
        ...
        !f->shared->closing) {
#endif
      haddr_t aggr_frag_addr =
          HADDR_UNDEF;            /* Address of aggregrator fragment */
      ...
      H5FD_mem_t alloc_type,
          other_alloc_type; /* Current aggregator & 'other' aggregator types */

#ifdef H5MF_AGGR_DEBUG
      fprintf(stderr,
              "%s: aggr = {%" PRIuHADDR ", %" PRIuHSIZE ", %" PRIuHSIZE "}\n",
      ...
      if ((size + aggr_frag_size) > aggr->size) {
        ...
        /* Check if the block asked for is too large for 'normal' aggregator
         * block */
        if (size >= aggr->alloc_size) {
          hsize_t ext_size = size + aggr_frag_size;

          /* Check for overlapping into file's temporary allocation space */
          if (H5_addr_gt((aggr->addr + aggr->size + ext_size),
                         f->shared->tmp_addr))
            HGOTO_ERROR(H5E_RESOURCE, H5E_BADRANGE, HADDR_UNDEF,
                        "'normal' file space allocation request will overlap "
                        "into 'temporary' "
                        "file space");

          if ((aggr->addr > 0) &&
              (extended = H5F__try_extend(
                   f, alloc_type, (aggr->addr + aggr->size), ext_size)) < 0)
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, HADDR_UNDEF,
                        "can't extending space");
          else if (extended) {
            ...
            /* Release "other" aggregator, if it exists, is at the end of the
             * allocated space, has allocated more than one block and the
             * unallocated space is greater than its allocation block size.
             */
            if ((other_aggr->size > 0) &&
                (H5_addr_eq((other_aggr->addr + other_aggr->size), eoa)) &&
                (other_aggr->tot_size > other_aggr->size) &&
                ((other_aggr->tot_size - other_aggr->size) >=
                 other_aggr->alloc_size)) {
            ...
            if (HADDR_UNDEF ==
                (ret_value = H5F__alloc(f, alloc_type, size, &eoa_frag_addr,
                                        &eoa_frag_size)))
              HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, HADDR_UNDEF,
          ...
          if ((aggr->addr > 0) &&
            ...
            /* Allocate space from the VFD (i.e. at the end of the file) */
            if (HADDR_UNDEF ==
                (new_space = H5F__alloc(f, alloc_type, aggr->alloc_size,
        ...
        if (eoa_frag_size)
          if (H5MF_xfree(f, alloc_type, eoa_frag_addr, eoa_frag_size) < 0)
            HGOTO_ERROR(H5E_RESOURCE, H5E_CANTFREE, HADDR_UNDEF,
                        "can't free eoa fragment");

        /* Freeing any possible fragment due to alignment in the block after
         * extension */
      ...
      if (HADDR_UNDEF == (ret_value = H5F__alloc(f, type, size, &eoa_frag_addr,
                                                 &eoa_frag_size)))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, HADDR_UNDEF,
                    "can't allocate file space");
    ...
    if (H5F_ALIGNMENT(f) && size >= H5F_THRESHOLD(f))
      assert(!((ret_value + H5FD_get_base_addr(f->shared->lf)) %
               H5F_ALIGNMENT(f)));
   ...
   *		       A) If the request is below the threshold, extend the block
   *into the aggregator B) If the request is above the threshold, a) extend the
   *aggregator by aggr->alloc_size or the extended amount b) extend the block
   *into the aggregator 2) When the aggregator is not at end of file: Extended
   *the block into the aggregator if it has enough space to satisfy the request
   *
   ...
   *-------------------------------------------------------------------------
   */
  htri_t H5MF__aggr_try_extend(H5F_t * f, H5F_blk_aggr_t * aggr,
                               H5FD_mem_t type, haddr_t blk_end,
    ...
    if (f->shared->feature_flags & aggr->feature_flag) {
      ...
      if (H5_addr_eq(blk_end, aggr->addr)) {
        ...
        if (H5_addr_eq(eoa, aggr->addr + aggr->size)) {
          /* If extra_requested is below percentage threshold, extend block into
           * the aggregator. */
          if (extra_requested <=
              (hsize_t)(EXTEND_THRESHOLD * (float)aggr->size)) {
           ...
           * 1) "bubble" up the aggregator by aggr->alloc_size or
           * extra_requested 2) extend the block into the aggregator
           */
          else {
            hsize_t extra = (extra_requested < aggr->alloc_size)
                                ? aggr->alloc_size
                                : extra_requested;

            if ((ret_value = H5F__try_extend(f, type, (aggr->addr + aggr->size),
                                             extra)) < 0)
          ...
          /* Check if aggregator has enough internal space to satisfy the
           * extension. */
   ...
   *                              FALSE(0) - Section and aggregator can not be
   *absorbed Failure:	FAIL
   *
   *-------------------------------------------------------------------------
   */
  htri_t H5MF__aggr_can_absorb(const H5F_t *f, const H5F_blk_aggr_t *aggr,
    ...
    if (f->shared->feature_flags & aggr->feature_flag) {
      ...
      if (H5_addr_eq((sect->sect_info.addr + sect->sect_info.size),
                     aggr->addr) ||
          H5_addr_eq((aggr->addr + aggr->size), sect->sect_info.addr)) {
#ifdef H5MF_AGGR_DEBUG
        fprintf(stderr,
                "%s: section {%" PRIuHADDR ", %" PRIuHSIZE
                "} adjoins aggr = {%" PRIuHADDR ", %" PRIuHSIZE "}\n",
                "H5MF__aggr_can_absorb", sect->sect_info.addr,
                sect->sect_info.size, aggr->addr, aggr->size);
#endif /* H5MF_AGGR_DEBUG */
        /* Check if aggregator would get too large and should be absorbed into
         * section */
   ...
   *-------------------------------------------------------------------------
   */
  herr_t H5MF__aggr_absorb(const H5F_t H5_ATTR_UNUSED *f, H5F_blk_aggr_t *aggr,
    ...
    if ((aggr->size + sect->sect_info.size) >= aggr->alloc_size &&
      ...
      if (H5_addr_eq((sect->sect_info.addr + sect->sect_info.size),
...
#ifdef H5MF_AGGR_DEBUG
        fprintf(stderr,
                "%s: aggr {%" PRIuHADDR ", %" PRIuHSIZE
                "} adjoins front of section = {%" PRIuHADDR ", %" PRIuHSIZE
                "}\n",
                "H5MF__aggr_absorb", aggr->addr, aggr->size,
        ...
        fprintf(stderr,
                "%s: aggr {%" PRIuHADDR ", %" PRIuHSIZE
                "} adjoins end of section = {%" PRIuHADDR ", %" PRIuHSIZE "}\n",
                "H5MF__aggr_absorb", aggr->addr, aggr->size,
                sect->sect_info.addr, sect->sect_info.size);
#endif /* H5MF_AGGR_DEBUG */
      ...
      if (H5_addr_eq((sect->sect_info.addr + sect->sect_info.size),
        ...
        fprintf(stderr,
                "%s: section {%" PRIuHADDR ", %" PRIuHSIZE
                "} adjoins front of aggr = {%" PRIuHADDR ", %" PRIuHSIZE "}\n",
                "H5MF__aggr_absorb", sect->sect_info.addr, sect->sect_info.size,
                aggr->addr, aggr->size);
#endif /* H5MF_AGGR_DEBUG */
        ...
        fprintf(stderr,
                "%s: section {%" PRIuHADDR ", %" PRIuHSIZE
                "} adjoins end of aggr = {%" PRIuHADDR ", %" PRIuHSIZE "}\n",
                "H5MF__aggr_absorb", sect->sect_info.addr, sect->sect_info.size,
                aggr->addr, aggr->size);
#endif /* H5MF_AGGR_DEBUG */
   ...
   *-------------------------------------------------------------------------
   */
  herr_t H5MF__aggr_query(const H5F_t *f, const H5F_blk_aggr_t *aggr,
...
#ifdef H5MF_AGGR_DEBUG
      fprintf(stderr,
              "%s: tmp_addr = %" PRIuHADDR ", tmp_size = %" PRIuHSIZE "\n",
      ...
      if (tmp_size > 0 && (H5F_INTENT(f) & H5F_ACC_RDWR))
        if (H5MF_xfree(f, alloc_type, tmp_addr, tmp_size) < 0)
          HGOTO_ERROR(H5E_RESOURCE, H5E_CANTFREE, FAIL,
                      "can't release aggregator's free space");
    } /* end if */
   ...
   *
   * Purpose:     Reset a metadata & small block aggregators, returning any
   *space back to file
   *
   * Return:      Success:        Non-negative
   *              Failure:        Negative
  ...
  htri_t H5MF__aggrs_try_shrink_eoa(H5F_t * f) {
    ...
    htri_t
        sda_status; /* Whether the small data aggregator can shrink the EOA */
    ...
    if ((ma_status = H5MF__aggr_can_shrink_eoa(f, H5FD_MEM_DEFAULT,
                                               &(f->shared->meta_aggr))) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                  "can't query metadata aggregator stats");
    if (ma_status > 0)
    ...
    if ((sda_status = H5MF__aggr_can_shrink_eoa(f, H5FD_MEM_DRAW,
                                                &(f->shared->sdata_aggr))) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGET, FAIL,
                  "can't query small data aggregator stats");
    if (sda_status > 0)
