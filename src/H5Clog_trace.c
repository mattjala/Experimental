...
static herr_t H5C__trace_write_expunge_entry_log_msg(void *udata,
                                                     haddr_t address,
                                                     int type_id,
                                                     herr_t fxn_ret_value);
static herr_t H5C__trace_write_flush_cache_log_msg(void *udata,
                                                   herr_t fxn_ret_value);
static herr_t H5C__trace_write_insert_entry_log_msg(void *udata,
                                                    haddr_t address,
                                                    int type_id, unsigned flags,
                                                    size_t size,
                                                    herr_t fxn_ret_value);
static herr_t H5C__trace_write_mark_entry_dirty_log_msg(
    void *udata, const H5C_cache_entry_t *entry, herr_t fxn_ret_value);
static herr_t H5C__trace_write_mark_entry_clean_log_msg(
    void *udata, const H5C_cache_entry_t *entry, herr_t fxn_ret_value);
static herr_t H5C__trace_write_mark_unserialized_entry_log_msg(
    void *udata, const H5C_cache_entry_t *entry, herr_t fxn_ret_value);
static herr_t H5C__trace_write_mark_serialized_entry_log_msg(
    void *udata, const H5C_cache_entry_t *entry, herr_t fxn_ret_value);
static herr_t H5C__trace_write_move_entry_log_msg(void *udata, haddr_t old_addr,
                                                  haddr_t new_addr, int type_id,
                                                  herr_t fxn_ret_value);
static herr_t H5C__trace_write_pin_entry_log_msg(void *udata,
...
H5C__trace_write_create_fd_log_msg(void *udata, const H5C_cache_entry_t *parent,
                                   ...
                                   herr_t fxn_ret_value);
static herr_t H5C__trace_write_protect_entry_log_msg(
    void *udata, const H5C_cache_entry_t *entry, int type_id, unsigned flags,
    herr_t fxn_ret_value);
static herr_t
H5C__trace_write_resize_entry_log_msg(void *udata,
                                      const H5C_cache_entry_t *entry,
                                      size_t new_size, herr_t fxn_ret_value);
static herr_t H5C__trace_write_unpin_entry_log_msg(
    void *udata, const H5C_cache_entry_t *entry, herr_t fxn_ret_value);
static herr_t H5C__trace_write_destroy_fd_log_msg(
    void *udata, const H5C_cache_entry_t *parent,
    const H5C_cache_entry_t *child, herr_t fxn_ret_value);
static herr_t H5C__trace_write_unprotect_entry_log_msg(void *udata,
                                                       haddr_t address,
                                                       int type_id,
                                                       unsigned flags,
                                                       herr_t fxn_ret_value);
static herr_t H5C__trace_write_set_cache_config_log_msg(
    void *udata, const H5AC_cache_config_t *config, herr_t fxn_ret_value);
static herr_t H5C__trace_write_remove_entry_log_msg(
    void *udata, const H5C_cache_entry_t *entry, herr_t fxn_ret_value);
 ...
 */
static const H5C_log_class_t H5C_trace_log_class_g = {
    "trace",
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5C__log_trace_set_up(H5C_log_info_t *log_info,
                             const char log_location[], int mpi_rank) {
  ...
  if (NULL == (trace_udata->message = (char *)H5MM_calloc(
                   H5C_MAX_TRACE_LOG_MSG_SIZE * sizeof(char))))
    HGOTO_ERROR(H5E_CACHE, H5E_CANTALLOC, FAIL, "memory allocation failed");
...
static herr_t H5C__trace_write_expunge_entry_log_msg(void *udata,
  ...
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             "H5AC_expunge_entry 0x%lx %d %d\n", (unsigned long)address,
             type_id, (int)fxn_ret_value);
...
static herr_t H5C__trace_write_insert_entry_log_msg(void *udata,
  ...
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             ...
             type_id, flags, (int)size, (int)fxn_ret_value);
...
static herr_t H5C__trace_write_mark_unserialized_entry_log_msg(
  ...
  /* Create the log message string */
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             "H5AC_mark_entry_unserialized 0x%lx %d\n",
...
static herr_t H5C__trace_write_mark_serialized_entry_log_msg(
  ...
  /* Create the log message string */
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             "H5AC_mark_entry_serialized 0x%lx %d\n",
             (unsigned long)(entry->addr), (int)fxn_ret_value);
...
static herr_t H5C__trace_write_pin_entry_log_msg(void *udata,
  ...
  /* Create the log message string */
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             "H5AC_pin_protected_entry 0x%lx %d\n",
             (unsigned long)(entry->addr), (int)fxn_ret_value);
...
H5C__trace_write_create_fd_log_msg(void *udata, const H5C_cache_entry_t *parent,
  ...
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             ...
             (unsigned long)(parent->addr), (unsigned long)(child->addr),
             (int)fxn_ret_value);
...
static herr_t H5C__trace_write_protect_entry_log_msg(
  ...
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             ...
             type_id, flags, (int)(entry->size), (int)fxn_ret_value);
...
static herr_t H5C__trace_write_set_cache_config_log_msg(
  ...
  /* Create the log message string */
  HDsnprintf(trace_udata->message, H5C_MAX_TRACE_LOG_MSG_SIZE,
             "H5AC_set_cache_auto_resize_config %d %d %d %d \"%s\" %d %d %d %f "
             "%d %d %ld %d %f %f %d %f %f "
             "%d %d %d %f %f %d %d %d %d %f %zu %d %d\n",
             config->version, (int)(config->rpt_fcn_enabled),
             (int)(config->open_trace_file), (int)(config->close_trace_file),
             config->trace_file_name, (int)(config->evictions_enabled),
             (int)(config->set_initial_size), (int)(config->initial_size),
             config->min_clean_fraction, (int)(config->max_size),
             (int)(config->min_size), config->epoch_length,
             (int)(config->incr_mode), config->lower_hr_threshold,
             config->increment, (int)(config->flash_incr_mode),
             config->flash_multiple, config->flash_threshold,
             (int)(config->apply_max_increment), (int)(config->max_increment),
             (int)(config->decr_mode), config->upper_hr_threshold,
             config->decrement, (int)(config->apply_max_decrement),
             (int)(config->max_decrement), config->epochs_before_eviction,
             (int)(config->apply_empty_reserve), config->empty_reserve,
             config->dirty_bytes_threshold, config->metadata_write_strategy,
             (int)fxn_ret_value);
