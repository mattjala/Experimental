...
#define H5CX_RETRIEVE_PLIST(PL, FAILVAL)                                       \
  ...
  if (NULL == (*head)->ctx.PL)                                                 \
    ...
    if (NULL == ((*head)->ctx.PL = (H5P_genplist_t *)H5I_object(               \
                     (*head)->ctx.H5_GLUE(PL, _id))))                          \
      HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, (FAILVAL),                         \
                  "can't get property list");

/* Common macro for the duplicated code to retrieve properties from a property
 * list */
#define H5CX_RETRIEVE_PROP_COMMON(PL, DEF_PL, PROP_NAME, PROP_FIELD)           \
  ...
  if ((*head)->ctx.H5_GLUE(PL, _id) == (DEF_PL))                               \
    H5MM_memcpy(&(*head)->ctx.PROP_FIELD,                                      \
                &H5_GLUE3(H5CX_def_, PL, _cache).PROP_FIELD,                   \
    ...
    if (H5P_get((*head)->ctx.PL, (PROP_NAME), &(*head)->ctx.PROP_FIELD) < 0)   \
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,                              \
                  "can't retrieve value from API context");                    \
  } /* end else */                                                             \
                                                                               \
...
/* Macro for the duplicated code to retrieve possibly set properties from a
 * property list */
#define H5CX_RETRIEVE_PROP_VALID_SET(PL, DEF_PL, PROP_NAME, PROP_FIELD)        \
  /* Check if the value has been retrieved already */                          \
  if (!((*head)->ctx.H5_GLUE(PROP_FIELD, _valid) ||                            \
        (*head)->ctx.H5_GLUE(PROP_FIELD, _set))) {                             \
    H5CX_RETRIEVE_PROP_COMMON(PL, DEF_PL, PROP_NAME, PROP_FIELD)               \
  } /* end if */
...
#define H5CX_TEST_SET_PROP(PROP_NAME, PROP_FIELD)                              \
  {                                                                            \
    htri_t check_prop =                                                        \
        0; /* Whether the property exists in the API context's DXPL */         \
                                                                               \
...
#define H5CX_SET_PROP(PROP_NAME, PROP_FIELD)                                   \
  if ((*head)->ctx.H5_GLUE(PROP_FIELD, _set)) {                                \
    ...
    if (H5P_set((*head)->ctx.dxpl, PROP_NAME, &(*head)->ctx.PROP_FIELD) < 0)   \
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTSET, NULL,                              \
                  "error setting data xfer property");                         \
  } /* end if */
...
typedef struct H5CX_t {
  ...
  hbool_t
      coll_metadata_read; /* Whether to use collective I/O for metadata read */
  ...
  hbool_t rank0_bcast; /* Whether a dataset meets read-with-rank0-and-bcast
                          requirements */
  ...
  hbool_t
      max_temp_buf_valid; /* Whether maximum temporary buffer size is valid */
  ...
  size_t
      vec_size; /* Size of hyperslab vector (H5D_XFER_HYPER_VECTOR_SIZE_NAME) */
  hbool_t vec_size_valid; /* Whether hyperslab vector is valid */
#ifdef H5_HAVE_PARALLEL
  H5FD_mpio_xfer_t io_xfer_mode; /* Parallel transfer mode for this request
                                    (H5D_XFER_IO_XFER_MODE_NAME) */
  hbool_t io_xfer_mode_valid;    /* Whether parallel transfer mode is valid */
  H5FD_mpio_collective_opt_t
      mpio_coll_opt; /* Parallel transfer with independent IO or collective IO
                        with this mode (H5D_XFER_MPIO_COLLECTIVE_OPT_NAME) */
  hbool_t mpio_coll_opt_valid; /* Whether parallel transfer option is valid */
  H5FD_mpio_chunk_opt_t
      mpio_chunk_opt_mode; /* Collective chunk option
                              (H5D_XFER_MPIO_CHUNK_OPT_HARD_NAME) */
  hbool_t
      mpio_chunk_opt_mode_valid; /* Whether collective chunk option is valid */
  unsigned mpio_chunk_opt_num;   /* Collective chunk threshold
                                    (H5D_XFER_MPIO_CHUNK_OPT_NUM_NAME) */
  hbool_t mpio_chunk_opt_num_valid; /* Whether collective chunk threshold is
                                       valid */
  unsigned mpio_chunk_opt_ratio;    /* Collective chunk ratio
                                       (H5D_XFER_MPIO_CHUNK_OPT_RATIO_NAME) */
  hbool_t
      mpio_chunk_opt_ratio_valid; /* Whether collective chunk ratio is valid */
#endif                            /* H5_HAVE_PARALLEL */
  ...
  hbool_t
      modify_write_buf_valid; /* Whether the modify_write_buf field is valid */

  /* Return-only DXPL properties to return to application */
#ifdef H5_HAVE_PARALLEL
  H5D_mpio_actual_chunk_opt_mode_t
      mpio_actual_chunk_opt; /* Chunk optimization mode used for parallel I/O
                                (H5D_MPIO_ACTUAL_CHUNK_OPT_MODE_NAME) */
  hbool_t mpio_actual_chunk_opt_set; /* Whether chunk optimization mode used for
                                        parallel I/O is set */
  H5D_mpio_actual_io_mode_t
      mpio_actual_io_mode;         /* Actual I/O mode used for parallel I/O
                                      (H5D_MPIO_ACTUAL_IO_MODE_NAME) */
  hbool_t mpio_actual_io_mode_set; /* Whether actual I/O mode used for parallel
                                      I/O is set */
  uint32_t
      mpio_local_no_coll_cause; /* Local reason for breaking collective I/O
                                   (H5D_MPIO_LOCAL_NO_COLLECTIVE_CAUSE_NAME) */
  hbool_t mpio_local_no_coll_cause_set;   /* Whether local reason for breaking
                                             collective I/O is set */
  hbool_t mpio_local_no_coll_cause_valid; /* Whether local reason for breaking
                                             collective I/O is valid */
  uint32_t
      mpio_global_no_coll_cause; /* Global reason for breaking collective I/O
                                    (H5D_MPIO_GLOBAL_NO_COLLECTIVE_CAUSE_NAME)
                                  */
  hbool_t mpio_global_no_coll_cause_set;   /* Whether global reason for breaking
                                              collective I/O is set */
  hbool_t mpio_global_no_coll_cause_valid; /* Whether global reason for breaking
                                              collective I/O is valid */
#ifdef H5_HAVE_INSTRUMENTED_LIBRARY
  int mpio_coll_chunk_link_hard; /* Instrumented "collective chunk link hard"
                                    value (H5D_XFER_COLL_CHUNK_LINK_HARD_NAME)
                                  */
  hbool_t mpio_coll_chunk_link_hard_set; /* Whether instrumented "collective
                                            chunk link hard" value is set */
  int mpio_coll_chunk_multi_hard; /* Instrumented "collective chunk multi hard"
                                     value
                                     (H5D_XFER_COLL_CHUNK_MULTI_HARD_NAME) */
  hbool_t mpio_coll_chunk_multi_hard_set; /* Whether instrumented "collective
                                             chunk multi hard" value is set */
  int mpio_coll_chunk_link_num_true; /* Instrumented "collective chunk link num
                                        true" value
                                        (H5D_XFER_COLL_CHUNK_LINK_NUM_TRUE_NAME)
                                      */
  hbool_t
      mpio_coll_chunk_link_num_true_set; /* Whether instrumented "collective
                                            chunk link num true" value is set */
  int mpio_coll_chunk_link_num_false; /* Instrumented "collective chunk link num
                                         false" value
                                         (H5D_XFER_COLL_CHUNK_LINK_NUM_FALSE_NAME)
                                       */
  hbool_t mpio_coll_chunk_link_num_false_set; /* Whether instrumented
                                                 "collective chunk link num
                                                 false" value is set */
  int mpio_coll_chunk_multi_ratio_coll; /* Instrumented "collective chunk multi
                                           ratio coll" value
                                           (H5D_XFER_COLL_CHUNK_MULTI_RATIO_COLL_NAME)
                                         */
  hbool_t mpio_coll_chunk_multi_ratio_coll_set; /* Whether instrumented
                                                   "collective chunk multi ratio
                                                   coll" value is set */
  int mpio_coll_chunk_multi_ratio_ind; /* Instrumented "collective chunk multi
                                          ratio ind" value
                                          (H5D_XFER_COLL_CHUNK_MULTI_RATIO_IND_NAME)
                                        */
  hbool_t mpio_coll_chunk_multi_ratio_ind_set; /* Whether instrumented
                                                  "collective chunk multi ratio
                                                  ind" value is set */
  hbool_t
      mpio_coll_rank0_bcast; /* Instrumented "collective chunk multi ratio ind"
                                value
                                (H5D_XFER_COLL_CHUNK_MULTI_RATIO_IND_NAME) */
  hbool_t mpio_coll_rank0_bcast_set; /* Whether instrumented "collective chunk
                                        multi ratio ind" value is set */
#endif                               /* H5_HAVE_INSTRUMENTED_LIBRARY */
#endif                               /* H5_HAVE_PARALLEL */
  uint32_t no_selection_io_cause;    /* Reason for not performing selection I/O
                                           (H5D_XFER_NO_SELECTION_IO_CAUSE_NAME) */
  hbool_t no_selection_io_cause_set; /* Whether reason for not performing
                                        selection I/O is set */
  hbool_t no_selection_io_cause_valid; /* Whether reason for not performing
                                          selection I/O is valid */
  ...
  hbool_t intermediate_group_valid; /* Whether create intermediate group flag is
                                       valid */

  /* Cached LAPL properties */
  size_t
      nlinks; /* Number of soft / UD links to traverse (H5L_ACS_NLINKS_NAME) */
  hbool_t
      nlinks_valid; /* Whether number of soft / UD links to traverse is valid */

  /* Cached DCPL properties */
  hbool_t do_min_dset_ohdr; /* Whether to minimize dataset object header */
  hbool_t do_min_dset_ohdr_valid; /* Whether minimize dataset object header flag
                                     is valid */
  ...
  hbool_t
      extfile_prefix_valid; /* Whether the prefix for external file is valid */
  ...
  H5VL_connector_prop_t
      vol_connector_prop;           /* Property for VOL connector ID & info */
  hbool_t vol_connector_prop_valid; /* Whether property for VOL connector ID &
                                       info is valid */
  void *vol_wrap_ctx; /* VOL connector's "wrap context" for creating IDs */
  hbool_t vol_wrap_ctx_valid; /* Whether VOL connector's "wrap context" for
                                 creating IDs is valid */
} H5CX_t;
...
typedef struct H5CX_dxpl_cache_t {
  ...
  H5D_selection_io_mode_t
      selection_io_mode; /* Selection I/O mode (H5D_XFER_SELECTION_IO_MODE_NAME)
                          */
  uint32_t
      no_selection_io_cause; /* Reasons for not performing selection I/O
                                      (H5D_XFER_NO_SELECTION_IO_CAUSE_NAME) */
  hbool_t modify_write_buf;  /* Whether the library can modify write buffers */
} H5CX_dxpl_cache_t;
...
/* Define a "default" dataset transfer property list cache structure to use for
 * default DXPLs */
static H5CX_dxpl_cache_t H5CX_def_dxpl_cache;

/* Define a "default" link creation property list cache structure to use for
 * default LCPLs */
static H5CX_lcpl_cache_t H5CX_def_lcpl_cache;

/* Define a "default" link access property list cache structure to use for
 * default LAPLs */
static H5CX_lapl_cache_t H5CX_def_lapl_cache;

/* Define a "default" dataset creation property list cache structure to use for
 * default DCPLs */
static H5CX_dcpl_cache_t H5CX_def_dcpl_cache;

/* Define a "default" dataset access property list cache structure to use for
 * default DAPLs */
static H5CX_dapl_cache_t H5CX_def_dapl_cache;

/* Define a "default" file access property list cache structure to use for
 * default FAPLs */
...
herr_t H5CX_init(void) {
  ...
  if (NULL ==
      (dx_plist = (H5P_genplist_t *)H5I_object(H5P_DATASET_XFER_DEFAULT)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
                "not a dataset transfer property list");

  /* Get B-tree split ratios */
  if (H5P_get(dx_plist, H5D_XFER_BTREE_SPLIT_RATIO_NAME,
              &H5CX_def_dxpl_cache.btree_split_ratio) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve B-tree split ratios");

  /* Get maximum temporary buffer size value */
  if (H5P_get(dx_plist, H5D_XFER_MAX_TEMP_BUF_NAME,
              &H5CX_def_dxpl_cache.max_temp_buf) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve maximum temporary buffer size");

  /* Get temporary buffer pointer */
  if (H5P_get(dx_plist, H5D_XFER_TCONV_BUF_NAME,
              &H5CX_def_dxpl_cache.tconv_buf) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve temporary buffer pointer");

  /* Get background buffer pointer */
  if (H5P_get(dx_plist, H5D_XFER_BKGR_BUF_NAME, &H5CX_def_dxpl_cache.bkgr_buf) <
      0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve background buffer pointer");

  /* Get background buffer type */
  if (H5P_get(dx_plist, H5D_XFER_BKGR_BUF_TYPE_NAME,
              &H5CX_def_dxpl_cache.bkgr_buf_type) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve background buffer type");

  /* Get I/O vector size */
  if (H5P_get(dx_plist, H5D_XFER_HYPER_VECTOR_SIZE_NAME,
              &H5CX_def_dxpl_cache.vec_size) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve I/O vector size");

#ifdef H5_HAVE_PARALLEL
  /* Collect Parallel I/O information for possible later use */
  if (H5P_get(dx_plist, H5D_XFER_IO_XFER_MODE_NAME,
              &H5CX_def_dxpl_cache.io_xfer_mode) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve parallel transfer method");
  if (H5P_get(dx_plist, H5D_XFER_MPIO_COLLECTIVE_OPT_NAME,
              &H5CX_def_dxpl_cache.mpio_coll_opt) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve collective transfer option");
  if (H5P_get(dx_plist, H5D_XFER_MPIO_CHUNK_OPT_HARD_NAME,
              &H5CX_def_dxpl_cache.mpio_chunk_opt_mode) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve chunk optimization option");
  if (H5P_get(dx_plist, H5D_XFER_MPIO_CHUNK_OPT_NUM_NAME,
              &H5CX_def_dxpl_cache.mpio_chunk_opt_num) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve chunk optimization threshold");
  if (H5P_get(dx_plist, H5D_XFER_MPIO_CHUNK_OPT_RATIO_NAME,
              &H5CX_def_dxpl_cache.mpio_chunk_opt_ratio) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve chunk optimization ratio");
  ...
  if (H5P_get(dx_plist, H5D_MPIO_LOCAL_NO_COLLECTIVE_CAUSE_NAME,
    ...
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve local cause for breaking collective I/O");
  ...
  if (H5P_get(dx_plist, H5D_XFER_EDC_NAME, &H5CX_def_dxpl_cache.err_detect) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve error detection info");

  /* Get filter callback function */
  if (H5P_get(dx_plist, H5D_XFER_FILTER_CB_NAME,
              &H5CX_def_dxpl_cache.filter_cb) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve filter callback function");
   ...
   */
  if (H5P_peek(dx_plist, H5D_XFER_XFORM_NAME,
               &H5CX_def_dxpl_cache.data_transform) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve data transform info");

  /* Get VL datatype alloc info */
  if (H5P_get(dx_plist, H5D_XFER_VLEN_ALLOC_NAME,
              &H5CX_def_dxpl_cache.vl_alloc_info.alloc_func) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve VL datatype alloc info");
  if (H5P_get(dx_plist, H5D_XFER_VLEN_ALLOC_INFO_NAME,
              &H5CX_def_dxpl_cache.vl_alloc_info.alloc_info) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve VL datatype alloc info");
  if (H5P_get(dx_plist, H5D_XFER_VLEN_FREE_NAME,
              &H5CX_def_dxpl_cache.vl_alloc_info.free_func) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve VL datatype alloc info");
  if (H5P_get(dx_plist, H5D_XFER_VLEN_FREE_INFO_NAME,
              &H5CX_def_dxpl_cache.vl_alloc_info.free_info) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve VL datatype alloc info");

  /* Get datatype conversion struct */
  if (H5P_get(dx_plist, H5D_XFER_CONV_CB_NAME,
              &H5CX_def_dxpl_cache.dt_conv_cb) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve datatype conversion exception callback");

  /* Get the selection I/O mode */
  if (H5P_get(dx_plist, H5D_XFER_SELECTION_IO_MODE_NAME,
              &H5CX_def_dxpl_cache.selection_io_mode) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve parallel transfer method");
  ...
  if (H5P_get(dx_plist, H5D_XFER_NO_SELECTION_IO_CAUSE_NAME,
    ...
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve cause for no selection I/O");

  /* Get the modify write buffer property */
  if (H5P_get(dx_plist, H5D_XFER_MODIFY_WRITE_BUF_NAME,
              &H5CX_def_dxpl_cache.modify_write_buf) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve modify write buffer property");
  ...
  if (NULL ==
      (lc_plist = (H5P_genplist_t *)H5I_object(H5P_LINK_CREATE_DEFAULT)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
                "not a link creation property list");

  /* Get link name character encoding */
  if (H5P_get(lc_plist, H5P_STRCRT_CHAR_ENCODING_NAME,
              &H5CX_def_lcpl_cache.encoding) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve link name encoding");

  /* Get flag whether to create intermediate groups */
  if (H5P_get(lc_plist, H5L_CRT_INTERMEDIATE_GROUP_NAME,
              &H5CX_def_lcpl_cache.intermediate_group) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve intermediate group creation flag");
  ...
  if (NULL ==
      (la_plist = (H5P_genplist_t *)H5I_object(H5P_LINK_ACCESS_DEFAULT)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
                "not a link access property list");

  /* Get number of soft / UD links to traverse */
  if (H5P_get(la_plist, H5L_ACS_NLINKS_NAME, &H5CX_def_lapl_cache.nlinks) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve number of soft / UD links to traverse");
  ...
  if (NULL ==
      (dc_plist = (H5P_genplist_t *)H5I_object(H5P_DATASET_CREATE_DEFAULT)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
                "not a dataset create property list");

  /* Get flag to indicate whether to minimize dataset object header */
  if (H5P_get(dc_plist, H5D_CRT_MIN_DSET_HDR_SIZE_NAME,
              &H5CX_def_dcpl_cache.do_min_dset_ohdr) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve dataset minimize flag");

  /* Get object header flags */
  if (H5P_get(dc_plist, H5O_CRT_OHDR_FLAGS_NAME,
              &H5CX_def_dcpl_cache.ohdr_flags) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve object header flags");
  ...
  if (NULL ==
      (da_plist = (H5P_genplist_t *)H5I_object(H5P_DATASET_ACCESS_DEFAULT)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
                "not a dataset create property list");

  /* Get the prefix for the external file */
  if (H5P_peek(da_plist, H5D_ACS_EFILE_PREFIX_NAME,
               &H5CX_def_dapl_cache.extfile_prefix) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve prefix for external file");

  /* Get the prefix for the VDS file */
  if (H5P_peek(da_plist, H5D_ACS_VDS_PREFIX_NAME,
               &H5CX_def_dapl_cache.vds_prefix) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve prefix for VDS");
  ...
  if (NULL ==
      (fa_plist = (H5P_genplist_t *)H5I_object(H5P_FILE_ACCESS_DEFAULT)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
                "not a dataset create property list");

  /* Get low_bound */
  if (H5P_get(fa_plist, H5F_ACS_LIBVER_LOW_BOUND_NAME,
              &H5CX_def_fapl_cache.low_bound) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve dataset minimize flag");

  if (H5P_get(fa_plist, H5F_ACS_LIBVER_HIGH_BOUND_NAME,
              &H5CX_def_fapl_cache.high_bound) < 0)
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "Can't retrieve dataset minimize flag");
done:
 ...
 *
 * Return:	Success: Non-NULL pointer to head pointer of API context stack
 *for thread Failure: NULL
 *
 *-------------------------------------------------------------------------
 */
static H5CX_node_t **H5CX__get_context(void) {
...
static void H5CX__push_common(H5CX_node_t *cnode) {
  ...
  /* Sanity check */
  assert(cnode);
  head = H5CX_get_my_context(); /* Get the pointer to the head of the API
...
herr_t H5CX_push(void) {
  ...
  if (NULL == (cnode = H5FL_CALLOC(H5CX_node_t)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTALLOC, FAIL,
                "unable to allocate new struct");
 ...
 *
 * Purpose:     Pushes a context for an API call, without using library
 *routines.
 *
 * Note:	This should only be called in special circumstances, like
 *H5close.
 *
 * Return:      <none>
 *
...
herr_t H5CX_retrieve_state(H5CX_state_t **api_state) {
  ...
  if (NULL == (*api_state = H5FL_CALLOC(H5CX_state_t)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTALLOC, FAIL,
                "unable to allocate new API context state");
  ...
  if (H5P_DATASET_CREATE_DEFAULT != (*head)->ctx.dcpl_id) {
    ...
    /* Copy the DCPL ID */
    if (((*api_state)->dcpl_id =
             H5P_copy_plist((H5P_genplist_t *)(*head)->ctx.dcpl, FALSE)) < 0)
  ...
  if (H5P_DATASET_XFER_DEFAULT != (*head)->ctx.dxpl_id) {
    ...
    /* Copy the DXPL ID */
    if (((*api_state)->dxpl_id =
             H5P_copy_plist((H5P_genplist_t *)(*head)->ctx.dxpl, FALSE)) < 0)
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTCOPY, FAIL, "can't copy property list");
  } /* end if */
  else
  ...
  if (H5P_LINK_ACCESS_DEFAULT != (*head)->ctx.lapl_id) {
    ...
    /* Copy the LAPL ID */
    if (((*api_state)->lapl_id =
             H5P_copy_plist((H5P_genplist_t *)(*head)->ctx.lapl, FALSE)) < 0)
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTCOPY, FAIL, "can't copy property list");
  } /* end if */
  else
  ...
  if (H5P_LINK_CREATE_DEFAULT != (*head)->ctx.lcpl_id) {
    ...
    /* Copy the LCPL ID */
    if (((*api_state)->lcpl_id =
             H5P_copy_plist((H5P_genplist_t *)(*head)->ctx.lcpl, FALSE)) < 0)
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTCOPY, FAIL, "can't copy property list");
  } /* end if */
  else
  ...
  if (NULL != (*api_state)->vol_wrap_ctx) {
    ...
    if (H5VL_inc_vol_wrapper((*api_state)->vol_wrap_ctx) < 0)
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTINC, FAIL,
                  "can't increment refcount on VOL wrapping context");
  } /* end if */

  /* Keep a copy of the VOL connector property, if there is one */
  if ((*head)->ctx.vol_connector_prop_valid &&
      (*head)->ctx.vol_connector_prop.connector_id > 0) {
    /* Get the connector property */
    H5MM_memcpy(&(*api_state)->vol_connector_prop,
                &(*head)->ctx.vol_connector_prop,
    ...
    if ((*api_state)->vol_connector_prop.connector_id) {
      ...
      if ((*api_state)->vol_connector_prop.connector_info) {
        ...
        if (H5I_inc_ref((*api_state)->vol_connector_prop.connector_id, FALSE) <
            0)
          HGOTO_ERROR(H5E_CONTEXT, H5E_CANTINC, FAIL,
                      "incrementing VOL connector ID failed");

        /* Retrieve the connector for the ID */
        if (NULL == (connector = (H5VL_class_t *)H5I_object(
                         (*api_state)->vol_connector_prop.connector_id))) {
          /* If failure occurs between incrementing ref count and attaching it
           * to API state, directly decrement ref count */
        ...
        if (H5VL_copy_connector_info(
                connector, &new_connector_info,
        ...
        HGOTO_ERROR(H5E_CONTEXT, H5E_CANTCOPY, FAIL,
...
herr_t H5CX_restore_state(const H5CX_state_t *api_state) {
  ...
  if (api_state->vol_connector_prop.connector_id) {
    H5MM_memcpy(&(*head)->ctx.vol_connector_prop,
                &api_state->vol_connector_prop, sizeof(H5VL_connector_prop_t));
...
herr_t H5CX_free_state(H5CX_state_t *api_state) {
  ...
  if (api_state->vol_connector_prop.connector_id) {
    ...
    if (H5I_dec_ref(api_state->vol_connector_prop.connector_id) < 0)
      HDONE_ERROR(H5E_CONTEXT, H5E_CANTDEC, FAIL,
 ...
 *
 * Purpose:     Sets the low/high bounds according to "f" for the current API
 *call context. When "f" is NULL, the low/high bounds are set to latest format.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 *-------------------------------------------------------------------------
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5CX_set_apl(hid_t *acspl_id, const H5P_libclass_t *libclass,
                    hid_t
  ...
  /* Set access plist to the default property list of the appropriate class if
   * it's the generic default */
  if (H5P_DEFAULT == *acspl_id)
    *acspl_id = *libclass->def_plist_id;
  else {
    htri_t is_lapl; /* Whether the access property list is (or is derived from)
                       a link access property list */
    htri_t is_dapl; /* Whether the access property list is (or is derived from)
                       a dataset access property list */
    htri_t is_fapl; /* Whether the access property list is (or is derived from)
    ...
    if ((is_dapl = H5P_class_isa(*libclass->pclass, *H5P_CLS_DACC->pclass)) < 0)
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                  "can't check for dataset access class");
    else if (is_dapl)
    ...
    if ((is_fapl = H5P_class_isa(*libclass->pclass, *H5P_CLS_FACC->pclass)) < 0)
      HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                  "can't check for file access class");
    else if (is_fapl)
    ...
    if (!is_collective) {
      ...
      /* If collective metadata read requested, set collective metadata read
       * flag */
...
H5CX_set_vol_connector_prop(const H5VL_connector_prop_t *vol_connector_prop) {
  ...
  /* Set the API context value */
  H5MM_memcpy(&(*head)->ctx.vol_connector_prop, vol_connector_prop,
              sizeof(H5VL_connector_prop_t));
...
herr_t H5CX_get_vol_wrap_ctx(void **vol_wrap_ctx) {
  ...
  /* No error is expected at this point.  But in case an application calls
   * H5VLwrap_register which doesn't reset the API context and there is no
   * context, returns a relevant error here
   */
  if (!head)
  ...
  if (!(*head))
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "unable to get the current API context");
...
herr_t H5CX_get_vol_connector_prop(H5VL_connector_prop_t *vol_connector_prop) {
  ...
  if ((*head)->ctx.vol_connector_prop_valid)
    /* Get the value */
    H5MM_memcpy(vol_connector_prop, &(*head)->ctx.vol_connector_prop,
                sizeof(H5VL_connector_prop_t));
  else
 ...
 *
 * Purpose:     Retrieves the metadata cache ring for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
H5AC_ring_t H5CX_get_ring(void) {
  H5CX_node_t **head = NULL; /* Pointer to head of API context list */
  H5AC_ring_t ring =
      H5AC_RING_INV; /* Current metadata cache ring for entries */
 ...
 *
 * Purpose:     Retrieves the "do collective metadata reads" flag for the
 ...
 *
 * Purpose:     Retrieves the MPI datatypes for collective I/O for the current
 *API call context.
 *
 ...
 *
 * Purpose:     Retrieves the "flushing an MPI-opened file" flag for the current
 *API call context.
 *
 * Return:      TRUE / FALSE on success / <can't fail>
 *
 ...
 *
 * Purpose:     Retrieves if the dataset meets read-with-rank0-and-bcast
 *requirements for the current API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 *-------------------------------------------------------------------------
 ...
 *
 * Purpose:     Retrieves the B-tree split ratios for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_btree_split_ratios(double split_ratio[3]) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_BTREE_SPLIT_RATIO_NAME, btree_split_ratio)

  /* Get the B-tree split ratio values */
  H5MM_memcpy(split_ratio, &(*head)->ctx.btree_split_ratio,
              sizeof((*head)->ctx.btree_split_ratio));
 ...
 *
 * Purpose:     Retrieves the maximum temporary buffer size for the current API
 *call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_max_temp_buf(size_t *max_temp_buf) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_MAX_TEMP_BUF_NAME, max_temp_buf)
 ...
 *
 * Purpose:     Retrieves the temporary buffer pointer for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_tconv_buf(void **tconv_buf) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_TCONV_BUF_NAME, tconv_buf)
 ...
 *
 * Purpose:     Retrieves the background buffer pointer for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Retrieves the background buffer type for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Retrieves the hyperslab vector size for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_vec_size(size_t *vec_size) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_HYPER_VECTOR_SIZE_NAME, vec_size)
 ...
 *
 * Purpose:     Retrieves the parallel transfer mode for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_io_xfer_mode(H5FD_mpio_xfer_t *io_xfer_mode) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_IO_XFER_MODE_NAME, io_xfer_mode)
 ...
 *
 * Purpose:     Retrieves the collective / independent parallel I/O option for
...
herr_t H5CX_get_mpio_coll_opt(H5FD_mpio_collective_opt_t *mpio_coll_opt) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_MPIO_COLLECTIVE_OPT_NAME, mpio_coll_opt)
 ...
 *
 * Purpose:     Retrieves the local cause for breaking collective I/O for the
 *current API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_mpio_local_no_coll_cause(uint32_t *mpio_local_no_coll_cause) {
  ...
  H5CX_RETRIEVE_PROP_VALID_SET(dxpl, H5P_DATASET_XFER_DEFAULT,
                               H5D_MPIO_LOCAL_NO_COLLECTIVE_CAUSE_NAME,
 ...
 *
 * Purpose:     Retrieves the global cause for breaking collective I/O for the
 *current API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Retrieves the collective chunk optimization mode for the current
 *API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Retrieves the collective chunk optimization threshold for the
 *current API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Retrieves the collective chunk optimization ratio for the
 *current API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Retrieves the error detection info for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_err_detect(H5Z_EDC_t *err_detect) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT, H5D_XFER_EDC_NAME,
                           err_detect)
 ...
 *
 * Purpose:     Retrieves the I/O filter callback function for the current API
 *call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_filter_cb(H5Z_cb_t *filter_cb) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_FILTER_CB_NAME, filter_cb)
  ...
  if (!(*head)->ctx.data_transform_valid) {
    ...
    if ((*head)->ctx.dxpl_id == H5P_DATASET_XFER_DEFAULT)
      ...
      if (NULL == (*head)->ctx.dxpl)
        ...
        if (NULL == ((*head)->ctx.dxpl =
                         (H5P_genplist_t *)H5I_object((*head)->ctx.dxpl_id)))
          HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
                      "can't get default dataset transfer property list");

      /* Get data transform info value */
      /* (Note: 'peek', not 'get' - if this turns out to be a problem, we may
       * need to copy it and free this in the H5CX pop routine. -QAK)
       */
      if (H5P_peek((*head)->ctx.dxpl, H5D_XFER_XFORM_NAME,
                   &(*head)->ctx.data_transform) < 0)
        HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                    "Can't retrieve data transform info");
    } /* end else */
 ...
 *
 * Purpose:     Retrieves the VL datatype alloc info for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
  ...
  if (!(*head)->ctx.vl_alloc_info_valid) {
    ...
    if ((*head)->ctx.dxpl_id == H5P_DATASET_XFER_DEFAULT)
      ...
      if (H5P_get((*head)->ctx.dxpl, H5D_XFER_VLEN_ALLOC_NAME,
                  &(*head)->ctx.vl_alloc_info.alloc_func) < 0)
        HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                    "Can't retrieve VL datatype alloc info");
      ...
      if (H5P_get((*head)->ctx.dxpl, H5D_XFER_VLEN_FREE_NAME,
                  &(*head)->ctx.vl_alloc_info.free_func) < 0)
        HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                    "Can't retrieve VL datatype alloc info");
 ...
 *
 * Purpose:     Retrieves the datatype conversion exception callback for the
 *current API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_dt_conv_cb(H5T_conv_cb_t *dt_conv_cb) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_CONV_CB_NAME, dt_conv_cb)
 ...
 *
 * Purpose:     Retrieves the selection I/O mode for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_selection_io_mode(H5D_selection_io_mode_t *selection_io_mode) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_SELECTION_IO_MODE_NAME, selection_io_mode)
 ...
 *
 * Purpose:     Retrieves the modify write buffer property for the current API
 *call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_modify_write_buf(hbool_t *modify_write_buf) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dxpl, H5P_DATASET_XFER_DEFAULT,
                           H5D_XFER_MODIFY_WRITE_BUF_NAME, modify_write_buf)
 ...
 *
 * Purpose:     Retrieves the character encoding for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_encoding(H5T_cset_t *encoding) {
  ...
  H5CX_RETRIEVE_PROP_VALID(lcpl, H5P_LINK_CREATE_DEFAULT,
                           H5P_STRCRT_CHAR_ENCODING_NAME, encoding)
 ...
 *
 * Purpose:     Retrieves the create intermediate group flag for the current API
 *call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_intermediate_group(unsigned *crt_intermed_group) {
  ...
  H5CX_RETRIEVE_PROP_VALID(lcpl, H5P_LINK_CREATE_DEFAULT,
                           H5L_CRT_INTERMEDIATE_GROUP_NAME, intermediate_group)
 ...
 *
 * Purpose:     Retrieves the # of soft / UD links to traverse for the current
 *API call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_nlinks(size_t *nlinks) {
  ...
  H5CX_RETRIEVE_PROP_VALID(lapl, H5P_LINK_ACCESS_DEFAULT, H5L_ACS_NLINKS_NAME,
                           nlinks)
...
herr_t H5CX_get_libver_bounds(H5F_libver_t *low_bound,
  ...
  H5CX_RETRIEVE_PROP_VALID(fapl, H5P_FILE_ACCESS_DEFAULT,
                           H5F_ACS_LIBVER_LOW_BOUND_NAME, low_bound)
  H5CX_RETRIEVE_PROP_VALID(fapl, H5P_FILE_ACCESS_DEFAULT,
                           H5F_ACS_LIBVER_HIGH_BOUND_NAME, high_bound)
...
herr_t H5CX_get_dset_min_ohdr_flag(hbool_t *dset_min_ohdr_flag) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dcpl, H5P_DATASET_CREATE_DEFAULT,
                           H5D_CRT_MIN_DSET_HDR_SIZE_NAME, do_min_dset_ohdr)
  ...
  if (!(*head)->ctx.extfile_prefix_valid) {
    ...
    if ((*head)->ctx.dapl_id == H5P_DATASET_ACCESS_DEFAULT)
      ...
      if (NULL == (*head)->ctx.dapl)
        ...
        if (NULL == ((*head)->ctx.dapl =
                         (H5P_genplist_t *)H5I_object((*head)->ctx.dapl_id)))
          HGOTO_ERROR(H5E_CONTEXT, H5E_BADTYPE, FAIL,
       ...
       * need to copy it and free this in the H5CX pop routine. -QAK)
       */
      if (H5P_peek((*head)->ctx.dapl, H5D_ACS_EFILE_PREFIX_NAME,
                   &(*head)->ctx.extfile_prefix) < 0)
        HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                    "Can't retrieve external file prefix");
    } /* end else */
...
herr_t H5CX_get_vds_prefix(const char **vds_prefix) {
  ...
  if (!(*head)->ctx.vds_prefix_valid) {
    ...
    if ((*head)->ctx.dapl_id == H5P_DATASET_ACCESS_DEFAULT)
       ...
       * need to copy it and free this in the H5CX pop routine. -QAK)
       */
      if (H5P_peek((*head)->ctx.dapl, H5D_ACS_VDS_PREFIX_NAME,
                   &(*head)->ctx.vds_prefix) < 0)
        HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                    "Can't retrieve VDS prefix");
    } /* end else */
 ...
 *
 * Purpose:     Sets the "do collective metadata reads" flag for the current API
 *call context.
 *
 * Return:      <none>
 *
 ...
 *
 * Purpose:     Sets the MPI datatypes for collective I/O for the current API
 *call context.
 *
 * Note:	This is only a shallow copy, the datatypes are not duplicated.
 *
 ...
 *
 * Purpose:     Sets the parallel transfer mode for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Sets the "flushing an MPI-opened file" flag for the current API
 *call context.
 *
 * Return:      <none>
 *
 ...
 *
 * Purpose:     Sets the "dataset meets read-with-rank0-and-bcast requirements"
 *flag for the current API call context.
 *
 * Return:      <none>
 *
 *-------------------------------------------------------------------------
 ...
 *
 * Purpose:     Sets the VL datatype alloc info for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 *-------------------------------------------------------------------------
 */
herr_t H5CX_set_vlen_alloc_info(H5MM_allocate_t alloc_func, void *alloc_info,
 ...
 *
 * Purpose:     Sets the # of soft / UD links to traverse for the current API
 *call context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
 ...
 *
 * Purpose:     Sets the actual chunk optimization used for parallel I/O for the
 *current API call context.
 *
 * Return:      <none>
 *
 *-------------------------------------------------------------------------
 */
void H5CX_set_mpio_actual_chunk_opt(
 ...
 *
 * Purpose:     Sets the actual I/O mode used for parallel I/O for the current
 *API call context.
 *
 * Return:      <none>
 *
 ...
 *
 * Purpose:     Sets the local reason for breaking collective I/O for the
 *current API call context.
 *
 * Return:      <none>
 *
 ...
 *
 * Purpose:     Sets the global reason for breaking collective I/O for the
 *current API call context.
 *
 * Return:      <none>
 *
 ...
 *
 * Purpose:     Sets the instrumented "collective chunk link hard" value for the
 *current API call context.
 *
 ...
 *
 * Purpose:     Sets the instrumented "collective chunk multi hard" value for
 *the current API call context.
 *
 * Note:        Only sets value if property set in DXPL
 *
...
H5CX_test_set_mpio_coll_chunk_multi_hard(int mpio_coll_chunk_multi_hard) {
  ...
  H5CX_TEST_SET_PROP(H5D_XFER_COLL_CHUNK_MULTI_HARD_NAME,
                     mpio_coll_chunk_multi_hard)
 ...
 *
 * Purpose:     Sets the instrumented "collective chunk link num tru" value for
 *the current API call context.
 *
 * Note:        Only sets value if property set in DXPL
 *
...
H5CX_test_set_mpio_coll_chunk_link_num_true(int mpio_coll_chunk_link_num_true) {
  ...
  H5CX_TEST_SET_PROP(H5D_XFER_COLL_CHUNK_LINK_NUM_TRUE_NAME,
                     mpio_coll_chunk_link_num_true)
 ...
 *
 * Purpose:     Sets the instrumented "collective chunk link num false" value
 *for the current API call context.
 *
 * Note:        Only sets value if property set in DXPL
 *
...
herr_t H5CX_test_set_mpio_coll_chunk_link_num_false(
  ...
  H5CX_TEST_SET_PROP(H5D_XFER_COLL_CHUNK_LINK_NUM_FALSE_NAME,
                     mpio_coll_chunk_link_num_false)
 ...
 *
 * Purpose:     Sets the instrumented "collective chunk multi ratio coll" value
 *for the current API call context.
 *
 * Note:        Only sets value if property set in DXPL
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5CX_test_set_mpio_coll_chunk_multi_ratio_coll(
  ...
  H5CX_TEST_SET_PROP(H5D_XFER_COLL_CHUNK_MULTI_RATIO_COLL_NAME,
                     mpio_coll_chunk_multi_ratio_coll)
 ...
 *
 * Purpose:     Sets the instrumented "collective chunk multi ratio ind" value
 *for the current API call context.
 *
 * Note:        Only sets value if property set in DXPL
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5CX_test_set_mpio_coll_chunk_multi_ratio_ind(
  ...
  H5CX_TEST_SET_PROP(H5D_XFER_COLL_CHUNK_MULTI_RATIO_IND_NAME,
                     mpio_coll_chunk_multi_ratio_ind)
 ...
 *
 * Purpose:     Sets the instrumented "read-with-rank0-bcast" flag for the
 *current API call context.
 *
 * Note:        Only sets value if property set in DXPL
 *
 ...
 *
 * Purpose:     Retrieves the object header flags for the current API call
 *context.
 *
 * Return:      Non-negative on success / Negative on failure
 *
...
herr_t H5CX_get_ohdr_flags(uint8_t *ohdr_flags) {
  ...
  H5CX_RETRIEVE_PROP_VALID(dcpl, H5P_DATASET_CREATE_DEFAULT,
                           H5O_CRT_OHDR_FLAGS_NAME, ohdr_flags)
...
static H5CX_node_t *H5CX__pop_common(hbool_t update_dxpl_props) {
  ...
  if (update_dxpl_props) {
    ...
    H5CX_SET_PROP(H5D_MPIO_LOCAL_NO_COLLECTIVE_CAUSE_NAME,
                  mpio_local_no_coll_cause)
    H5CX_SET_PROP(H5D_MPIO_GLOBAL_NO_COLLECTIVE_CAUSE_NAME,
                  mpio_global_no_coll_cause)
#ifdef H5_HAVE_INSTRUMENTED_LIBRARY
    H5CX_SET_PROP(H5D_XFER_COLL_CHUNK_LINK_HARD_NAME, mpio_coll_chunk_link_hard)
    H5CX_SET_PROP(H5D_XFER_COLL_CHUNK_MULTI_HARD_NAME,
                  mpio_coll_chunk_multi_hard)
    H5CX_SET_PROP(H5D_XFER_COLL_CHUNK_LINK_NUM_TRUE_NAME,
                  mpio_coll_chunk_link_num_true)
    H5CX_SET_PROP(H5D_XFER_COLL_CHUNK_LINK_NUM_FALSE_NAME,
                  mpio_coll_chunk_link_num_false)
    H5CX_SET_PROP(H5D_XFER_COLL_CHUNK_MULTI_RATIO_COLL_NAME,
                  mpio_coll_chunk_multi_ratio_coll)
    H5CX_SET_PROP(H5D_XFER_COLL_CHUNK_MULTI_RATIO_IND_NAME,
                  mpio_coll_chunk_multi_ratio_ind)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5CX_pop(hbool_t update_dxpl_props) {
  H5CX_node_t *cnode;         /* Context node */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (NULL == (cnode = H5CX__pop_common(update_dxpl_props)))
    HGOTO_ERROR(H5E_CONTEXT, H5E_CANTGET, FAIL,
                "error getting API context node");
