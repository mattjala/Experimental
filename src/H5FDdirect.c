...
typedef struct H5FD_direct_t {
   ...
   * number (which, both? -rpm) may change when the system is restarted or when
   * the file is opened. After a process opens a file, the identifier is
...
static H5FD_t *H5FD__direct_open(const char *name, unsigned flags,
...
static herr_t H5FD__direct_set_eoa(H5FD_t *_file, H5FD_mem_t type,
                                   haddr_t addr);
static haddr_t H5FD__direct_get_eof(const H5FD_t *_file, H5FD_mem_t type);
static herr_t H5FD__direct_get_handle(H5FD_t *_file, hid_t fapl,
                                      void **file_handle);
static herr_t H5FD__direct_read(H5FD_t *_file, H5FD_mem_t type, hid_t fapl_id,
                                haddr_t addr, size_t size, void *buf);
static herr_t H5FD__direct_write(H5FD_t *_file, H5FD_mem_t type, hid_t fapl_id,
                                 haddr_t addr, size_t size, const void *buf);
static herr_t H5FD__direct_truncate(H5FD_t *_file, hid_t dxpl_id,
                                    hbool_t closing);
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5Pset_fapl_direct(hid_t fapl_id, size_t boundary, size_t block_size,
                          size_t cbuf_size) {
    ...
    if (HDwrite(file->fd, buf1, sizeof(int)) < 0) {
      if (HDwrite(file->fd, buf2, file->fa.fbsize) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_WRITEERROR, NULL,
                    "file system may not support Direct I/O");
      else
    ...
    if (HDread(file->fd, buf1, sizeof(int)) < 0) {
      ...
      if (o_flags & O_RDWR) {
        if (HDlseek(file->fd, (HDoff_t)0, SEEK_SET) < 0)
          HSYS_GOTO_ERROR(H5E_IO, H5E_SEEKERROR, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__direct_query(const H5FD_t H5_ATTR_UNUSED *_f,
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__direct_get_eoa(const H5FD_t *_file,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__direct_set_eoa(H5FD_t *_file,
                                   H5FD_mem_t H5_ATTR_UNUSED type,
                                   haddr_t addr) {
  H5FD_direct_t *file = (H5FD_direct_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static haddr_t H5FD__direct_get_eof(const H5FD_t *_file,
                                    H5FD_mem_t H5_ATTR_UNUSED type) {
  const H5FD_direct_t *file = (const H5FD_direct_t *)_file;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__direct_get_handle(H5FD_t *_file, hid_t H5_ATTR_UNUSED fapl,
                                      void **file_handle) {
  H5FD_direct_t *file = (H5FD_direct_t *)_file;
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__direct_read(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
   ...
   */
  if (!_must_align || ((addr % _fbsize == 0) && (size % _fbsize == 0) &&
                       ((size_t)buf % _boundary == 0))) {
    ...
    /*
     * Read the aligned data in file into aligned buffer first, then copy the
     * data into the final buffer.  If the data size is bigger than maximal copy
     * buffer size, do the reading by segment (the outer while loop).  If not,
     * do one step reading.
     */
    do {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__direct_write(H5FD_t *_file, H5FD_mem_t H5_ATTR_UNUSED type,
                                 hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr,
  ...
  if (!_must_align || ((addr % _fbsize == 0) && (size % _fbsize == 0) &&
    ...
    do {
      ...
      if (copy_offset > 0) {
        ...
        if (HDlseek(file->fd, (HDoff_t)(write_addr + write_size - _fbsize),
                    SEEK_SET) < 0)
          HSYS_GOTO_ERROR(H5E_IO, H5E_SEEKERROR, FAIL,
                          "unable to seek to proper position")
      } /* end if */
      ...
      /* look for the right position and append or copy the data to be written
       * to the aligned buffer. Consider all possible situations here: file
       * address is not aligned on file block size; the end of data address is
       * not aligned; the end of data address is aligned; data size is smaller
       * or bigger than maximal copy size.
       */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__direct_truncate(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id,
  ...
  if (file->eoa != file->eof) {
    ...
    (void)SetFilePointer((HANDLE)filehandle, li.LowPart, &li.HighPart,
                         FILE_BEGIN);
    ...
    /*Even though eof is equal to eoa, file is still truncated because Direct
     *I/O write introduces some extra data for alignment.
     */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5FD__direct_delete(const char *filename,
