...
static herr_t H5G__dense_btree2_corder_compare(const void *rec1,
...
static herr_t H5G__dense_btree2_corder_decode(const uint8_t *raw, void *native,
                                              void *ctx);
static herr_t H5G__dense_btree2_corder_debug(FILE *stream, int indent,
...
static herr_t H5G__dense_btree2_name_compare(const void *rec1, const void *rec2,
                                             int *result);
...
static herr_t H5G__dense_fh_name_cmp(const void *obj, size_t obj_len,
                                     void *op_data);
...
static herr_t H5G__dense_fh_name_cmp(const void *obj, size_t obj_len,
                                     void *_udata) {
  H5G_fh_ud_cmp_t *udata =
      (H5G_fh_ud_cmp_t *)_udata; /* User data for 'op' callback */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_btree2_name_compare(const void *_bt2_udata,
                                             const void *_bt2_rec,
                                             int *result) {
  const H5G_bt2_ud_common_t *bt2_udata =
      (const H5G_bt2_ud_common_t *)_bt2_udata;
  const H5G_dense_bt2_name_rec_t *bt2_rec =
      (const H5G_dense_bt2_name_rec_t *)_bt2_rec;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (bt2_udata->name_hash < bt2_rec->hash)
    ...
    *result = 1;
    ...
    if (H5HF_op(bt2_udata->fheap, bt2_rec->id, H5G__dense_fh_name_cmp,
                &fh_udata) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_btree2_name_encode(uint8_t *raw, const void *_nrecord,
                                            void H5_ATTR_UNUSED *ctx) {
  const H5G_dense_bt2_name_rec_t *nrecord =
      (const H5G_dense_bt2_name_rec_t *)_nrecord;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_btree2_name_decode(const uint8_t *raw, void *_nrecord,
                                            void H5_ATTR_UNUSED *ctx) {
  H5G_dense_bt2_name_rec_t *nrecord = (H5G_dense_bt2_name_rec_t *)_nrecord;
...
static herr_t H5G__dense_btree2_name_debug(FILE *stream, int indent, int fwidth,
  ...
  fprintf(stream, "%*s%-*s {%x, ", indent, "", fwidth,
          "Record:", (unsigned)nrecord->hash);
  for (u = 0; u < H5G_DENSE_FHEAP_ID_LEN; u++)
    fprintf(stderr, "%02x%s", nrecord->id[u],
            (u < (H5G_DENSE_FHEAP_ID_LEN - 1) ? " " : "}\n"));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_btree2_corder_store(void *_nrecord,
                                             const void *_udata) {
  const H5G_bt2_ud_ins_t *udata = (const H5G_bt2_ud_ins_t *)_udata;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_btree2_corder_compare(const void *_bt2_udata,
                                               const void *_bt2_rec,
                                               int *result) {
  const H5G_bt2_ud_common_t *bt2_udata =
      (const H5G_bt2_ud_common_t *)_bt2_udata;
  const H5G_dense_bt2_corder_rec_t *bt2_rec =
      (const H5G_dense_bt2_corder_rec_t *)_bt2_rec;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_btree2_corder_encode(uint8_t *raw,
                                              const void *_nrecord,
                                              void H5_ATTR_UNUSED *ctx) {
  const H5G_dense_bt2_corder_rec_t *nrecord =
      (const H5G_dense_bt2_corder_rec_t *)_nrecord;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5G__dense_btree2_corder_decode(const uint8_t *raw,
                                              void *_nrecord,
                                              void H5_ATTR_UNUSED *ctx) {
  H5G_dense_bt2_corder_rec_t *nrecord = (H5G_dense_bt2_corder_rec_t *)_nrecord;
...
H5G__dense_btree2_corder_debug(FILE *stream, int indent, int fwidth,
  ...
  fprintf(stream, "%*s%-*s {%llu, ", indent, "", fwidth,
          "Record:", (unsigned long long)nrecord->corder);
  for (u = 0; u < H5G_DENSE_FHEAP_ID_LEN; u++)
    fprintf(stderr, "%02x%s", nrecord->id[u],
            (u < (H5G_DENSE_FHEAP_ID_LEN - 1) ? " " : "}\n"));
