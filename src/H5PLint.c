...
herr_t H5PL_init(void) {
  ...
  if (H5PL__create_path_table() < 0)
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTINIT, FAIL,
                "can't create plugin search path table");
...
int H5PL_term_package(void) {
  ...
  if (H5PL__close_path_table() < 0)
    HGOTO_ERROR(H5E_PLUGIN, H5E_CANTFREE, (-1),
                "problem closing search path table");
...
const void *H5PL_load(H5PL_type_t type, const H5PL_key_t *key) {
  ...
  case H5PL_TYPE_VFD:
    if ((H5PL_plugin_control_mask_g & H5PL_VFD_PLUGIN) == 0)
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTLOAD, NULL,
                  "Virtual File Driver (VFD) plugins disabled");
    break;
  ...
  /* If not found, try iterating through the path table to find an appropriate
   * plugin */
  if (!found)
    if (H5PL__find_plugin_in_path_table(&search_params, &found, &plugin_info) <
        0)
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTGET, NULL,
                  "can't find plugin in the paths either set by "
                  "HDF5_PLUGIN_PATH, or default location, "
  ...
  if (found)
    ...
    HGOTO_ERROR(H5E_PLUGIN, H5E_NOTFOUND, NULL,
                "can't find plugin. Check either HDF5_VOL_CONNECTOR, "
                "HDF5_PLUGIN_PATH, default location, "
...
herr_t H5PL__open(const char *path, H5PL_type_t type, const H5PL_key_t *key,
                  hbool_t *success, H5PL_type_t *plugin_type,
  ...
  *plugin_info = NULL;
  ...
  /* There are different reasons why a library can't be open, e.g. wrong
   * architecture. If we can't open the library, just return.
   */
  ...
  /* Return a handle for the function H5PLget_plugin_type in the dynamic
   * library. The plugin library is supposed to define this function.
   */
  if (NULL == (get_plugin_type = (H5PL_get_plugin_type_t)H5PL_GET_LIB_FUNC(
                   handle, "H5PLget_plugin_type")))
    HGOTO_DONE(SUCCEED);

  /* Return a handle for the function H5PLget_plugin_info in the dynamic
   * library. The plugin library is supposed to define this function.
   */
  if (NULL == (get_plugin_info = (H5PL_get_plugin_info_t)H5PL_GET_LIB_FUNC(
                   handle, "H5PLget_plugin_info")))
    HGOTO_DONE(SUCCEED);
  ...
  case H5PL_TYPE_FILTER: {
    ...
    if (NULL == (filter_info = (const H5Z_class2_t *)(*get_plugin_info)()))
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTGET, FAIL,
                  "can't get filter info from plugin");
  ...
  case H5PL_TYPE_VOL: {
    ...
    if (NULL == (cls = (const void *)(*get_plugin_info)()))
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTGET, FAIL,
                  "can't get VOL connector info from plugin");
    ...
    /* Ask VOL interface if this class is the one we are looking for and is
     * compatible, etc */
    if (H5VL_check_plugin_load(cls, key, success) < 0)
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTLOAD, FAIL,
                  "VOL connector compatibility check failed");
  ...
  case H5PL_TYPE_VFD: {
    ...
    if (NULL == (cls = (const void *)(*get_plugin_info)()))
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTGET, FAIL,
                  "can't get VFD info from plugin");
    ...
    /* Ask VFD interface if this class is the one we are looking for and is
     * compatible, etc */
    if (H5FD_check_plugin_load(cls, key, success) < 0)
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTLOAD, FAIL,
                  "VFD compatibility check failed");
  ...
  if (*success)
    if (H5PL__add_plugin(loaded_plugin_type, key, handle))
      HGOTO_ERROR(H5E_PLUGIN, H5E_CANTINSERT, FAIL,
                  "unable to add new plugin to plugin cache");

done:
  if (!(*success) && handle)
    if (H5PL__close(handle) < 0)
      HDONE_ERROR(H5E_PLUGIN, H5E_CLOSEERROR, FAIL,
                  "can't close dynamic library");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5PL_iterate(H5PL_iterate_type_t iter_type, H5PL_iterate_t iter_op,
                    void *op_data) {
