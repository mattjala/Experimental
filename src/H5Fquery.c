 ...
 *           to retrieve the main file's location when searching the target
 *file.
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_shared_get_file_driver(const H5F_shared_t *f_sh,
                                  H5FD_t **file_handle) {
  /* Use FUNC_ENTER_NOAPI_NOINIT_NOERR here to avoid performance issues */
  FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5F_shared_get_mpi_file_sync_required(const H5F_shared_t *f_sh,
                                             hbool_t *flag /*out*/) {
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if ((ret_value = H5FD_mpi_get_file_sync_required(f_sh->lf, flag)) < 0)
    HGOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                "driver get_file_sync_required request failed");
 ...
 *-------------------------------------------------------------------------
 */
hbool_t H5F_get_file_locking(const H5F_t *f) {
  FUNC_ENTER_NOAPI_NOINIT_NOERR
 ...
 *-------------------------------------------------------------------------
 */
hbool_t H5F_has_vector_select_io(const H5F_t *f, hbool_t is_write) {
  ...
  if (is_write)
    ret_value = (f->shared->lf->cls->write_vector != NULL ||
                 f->shared->lf->cls->write_selection != NULL);
  else
    ret_value = (f->shared->lf->cls->read_vector != NULL ||
                 f->shared->lf->cls->read_selection != NULL);
