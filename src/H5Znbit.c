...
static size_t H5Z__filter_nbit(unsigned flags, size_t cd_nelmts,
...
static herr_t H5Z__calc_parms_array(const H5T_t *type,
                                    size_t *cd_values_actual_nparms);
static herr_t H5Z__calc_parms_compound(const H5T_t *type,
                                       size_t *cd_values_actual_nparms);

static herr_t H5Z__set_parms_nooptype(const H5T_t *type,
                                      unsigned *cd_values_index,
                                      unsigned cd_values[]);
static herr_t H5Z__set_parms_atomic(const H5T_t *type,
                                    unsigned *cd_values_index,
                                    unsigned cd_values[],
...
static herr_t H5Z__set_parms_array(const H5T_t *type, unsigned *cd_values_index,
                                   unsigned cd_values[],
                                   hbool_t *need_not_compress);
static herr_t H5Z__set_parms_compound(const H5T_t *type,
                                      unsigned *cd_values_index,
                                      unsigned cd_values[],
                                      hbool_t *need_not_compress);

static void H5Z__nbit_next_byte(size_t *j, size_t *buf_len);
static void H5Z__nbit_decompress_one_byte(
...
static void H5Z__nbit_compress_one_byte(const unsigned char *data,
                                        size_t data_offset, unsigned k,
                                        unsigned begin_i, unsigned end_i,
                                        unsigned char *buffer, size_t *j,
                                        size_t *buf_len, const parms_atomic *p,
                                        size_t datatype_len);
static void H5Z__nbit_decompress_one_nooptype(unsigned char *data,
                                              size_t data_offset,
                                              const unsigned char *buffer,
                                              size_t *j, size_t *buf_len,
                                              unsigned size);
static void H5Z__nbit_decompress_one_atomic(unsigned char *data,
                                            size_t data_offset,
                                            unsigned char *buffer, size_t *j,
                                            size_t *buf_len,
                                            const parms_atomic *p);
static herr_t H5Z__nbit_decompress_one_array(
    unsigned char *data, size_t data_offset, unsigned char *buffer, size_t *j,
...
static void H5Z__nbit_compress_one_nooptype(const unsigned char *data,
                                            size_t data_offset,
                                            unsigned char *buffer, size_t *j,
...
static void H5Z__nbit_compress_one_compound(
    unsigned char *data, size_t data_offset, unsigned char *buffer, size_t *j,
    size_t *buf_len, const unsigned parms[], unsigned *parms_index);
static void H5Z__nbit_compress(unsigned char *data, unsigned d_nelmts,
 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5Z__can_apply_nbit(hid_t H5_ATTR_UNUSED dcpl_id, hid_t type_id,
  ...
  *cd_values_actual_nparms += 1;
  ...
  case H5T_ARRAY:
    if (H5Z__calc_parms_array(dtype_base, cd_values_actual_nparms) == FAIL)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "nbit cannot compute parameters for datatype");
    break;
  ...
  if (dtype_base)
    if (H5T_close_real(dtype_base) < 0)
      HDONE_ERROR(H5E_PLINE, H5E_CLOSEERROR, FAIL,
                  "Unable to close base datatype");
...
static herr_t H5Z__calc_parms_compound(const H5T_t *type,
  ...
  *cd_values_actual_nparms += 1;
  ...
  for (u = 0; u < (unsigned)nmembers; u++) {
    H5T_class_t
        dtype_member_class; /* Compound datatype's member datatype's class */
    ...
    if ((dtype_member_class = H5T_get_class(dtype_member, TRUE)) ==
        H5T_NO_CLASS)
    ...
    *cd_values_actual_nparms += 1;
    ...
    case H5T_COMPOUND:
      if (H5Z__calc_parms_compound(dtype_member, cd_values_actual_nparms) ==
          FAIL)
        HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                    "nbit cannot compute parameters for datatype");
      break;
    ...
    if (H5T_close_real(dtype_member) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_CLOSEERROR, FAIL,
                  "Unable to close member datatype");
    dtype_member = NULL;
  } /* end for */
...
static herr_t H5Z__set_parms_atomic(const H5T_t *type,
  ...
  if (dtype_precision > dtype_size * 8 ||
      (dtype_precision + dtype_offset) > dtype_size * 8)
    HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                "invalid datatype precision/offset");
...
static herr_t H5Z__set_parms_array(const H5T_t *type, unsigned *cd_values_index,
  ...
  htri_t
      is_vlstring; /* flag indicating if datatype is variable-length string */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  case H5T_INTEGER:
  case H5T_FLOAT:
    if (H5Z__set_parms_atomic(dtype_base, cd_values_index, cd_values,
                              need_not_compress) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "nbit cannot set parameters for datatype");
    break;

  case H5T_ARRAY:
    if (H5Z__set_parms_array(dtype_base, cd_values_index, cd_values,
                             need_not_compress) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "nbit cannot set parameters for datatype");
    break;
  ...
  case H5T_COMPOUND:
    if (H5Z__set_parms_compound(dtype_base, cd_values_index, cd_values,
                                need_not_compress) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "nbit cannot set parameters for datatype");
    break;
  ...
  case H5T_VLEN:
    ...
    if (dtype_base_class == H5T_VLEN || is_vlstring)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "datatype not supported by nbit");
...
static herr_t H5Z__set_parms_compound(const H5T_t *type,
  ...
  H5T_class_t
      dtype_member_class;     /* Compound datatype's member datatype's class */
  size_t dtype_member_offset; /* Compound datatype's current member datatype's
                                 offset (in bytes) */
  size_t dtype_next_member_offset; /* Compound datatype's next member datatype's
                                      offset (in bytes) */
  ...
  for (u = 0; u < nmembers; u++) {
    ...
    case H5T_INTEGER:
    case H5T_FLOAT:
      if (H5Z__set_parms_atomic(dtype_member, cd_values_index, cd_values,
                                need_not_compress) < 0)
        HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                    "nbit cannot set parameters for datatype");
      break;
    ...
    case H5T_ARRAY:
      if (H5Z__set_parms_array(dtype_member, cd_values_index, cd_values,
                               need_not_compress) < 0)
        HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                    "nbit cannot set parameters for datatype");
      break;
    ...
    case H5T_COMPOUND:
      if (H5Z__set_parms_compound(dtype_member, cd_values_index, cd_values,
                                  need_not_compress) < 0)
        HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                    "nbit cannot set parameters for datatype");
      break;
    ...
    case H5T_VLEN:
      ...
      /* Because for some no-op datatype (VL datatype and VL string datatype),
       * its size can not be retrieved correctly by using function call
       * H5T_get_size, special handling is needed for getting the size. Here the
       * difference between adjacent member offset is used (if alignment is
       * present, the result can be larger, but it does not affect the nbit
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5Z__set_local_nbit(hid_t dcpl_id, hid_t type_id,
                                  hid_t space_id) {
  ...
  hbool_t need_not_compress;  /* Flag if TRUE indicating no need to do nbit
                                 compression */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (cd_values_actual_nparms > H5Z_NBIT_MAX_NPARMS)
    HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                "datatype needs too many nbit parameters");

  /* Allocate memory space for cd_values[] */
  if (NULL == (cd_values = (unsigned *)H5MM_malloc(cd_values_actual_nparms *
                                                   sizeof(unsigned))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                "memory allocation failed for cd_values[]");
  ...
  if (H5P_get_filter_by_id(dcpl_plist, H5Z_FILTER_NBIT, &flags, &cd_nelmts,
  ...
  case H5T_INTEGER:
  case H5T_FLOAT:
    if (H5Z__set_parms_atomic(type, &cd_values_index, cd_values,
                              &need_not_compress) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "nbit cannot set parameters for datatype");
    break;
  ...
  case H5T_ARRAY:
    if (H5Z__set_parms_array(type, &cd_values_index, cd_values,
                             &need_not_compress) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "nbit cannot set parameters for datatype");
    break;
  ...
  case H5T_COMPOUND:
    if (H5Z__set_parms_compound(type, &cd_values_index, cd_values,
                                &need_not_compress) < 0)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "nbit cannot set parameters for datatype");
    break;
  ...
  if (H5P_modify_filter(dcpl_plist, H5Z_FILTER_NBIT, flags,
                        cd_values_actual_nparms, cd_values) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTSET, FAIL,
                "can't set local nbit parameters");
...
static size_t H5Z__filter_nbit(unsigned flags, size_t cd_nelmts,
  ...
  if (flags & H5Z_FLAG_REVERSE) {
    size_out =
        d_nelmts * (size_t)cd_values[4]; /* cd_values[4] stores datatype size */

    /* allocate memory space for decompressed buffer */
    if (NULL == (outbuf = (unsigned char *)H5MM_malloc(size_out)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, 0,
                  "memory allocation failed for nbit decompression");
  ...
  /* output; compress */
    ...
    if (NULL == (outbuf = (unsigned char *)H5MM_malloc(size_out)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, 0,
                  "memory allocation failed for nbit compression");
...
static void H5Z__nbit_decompress_one_byte(
    ...
    unsigned end_i, const unsigned char *buffer, size_t *j, size_t *buf_len,
    const parms_atomic *p, size_t datatype_len) {
  size_t dat_len; /* dat_len is the number of bits to be copied in each data
                     byte */
  size_t dat_offset;
...
static void H5Z__nbit_decompress_one_nooptype(unsigned char *data,
  ...
  for (i = 0; i < size; i++) {
    ...
    data[data_offset + i] = (unsigned char)((
        (val & ~((unsigned)(~0) << *buf_len)) << (dat_len - *buf_len)));
    dat_len -= *buf_len;
    H5Z__nbit_next_byte(j, buf_len);
    if (dat_len == 0)
      continue;
...
static void H5Z__nbit_decompress_one_atomic(unsigned char *data,
  ...
  if (p->order == H5Z_NBIT_ORDER_LE) { /* little endian */
    ...
    for (k = (int)begin_i; k >= (int)end_i; k--)
      H5Z__nbit_decompress_one_byte(data, data_offset, (unsigned)k, begin_i,
                                    end_i, buffer, j, buf_len, p, datatype_len);
  } else { /* big endian */
    /* Sanity check */
...
static herr_t H5Z__nbit_decompress_one_array(
  ...
  case H5Z_NBIT_ATOMIC:
    ...
    n = total_size / p.size;
    for (i = 0; i < n; i++)
      H5Z__nbit_decompress_one_atomic(data, data_offset + i * (size_t)p.size,
                                      buffer, j, buf_len, &p);
    break;
  ...
  case H5Z_NBIT_ARRAY:
    ...
    n = total_size /
        base_size; /* number of base_type elements inside the array datatype */
    begin_index = *parms_index;
    for (i = 0; i < n; i++) {
      if (H5Z__nbit_decompress_one_array(
  ...
  case H5Z_NBIT_COMPOUND:
    ...
    begin_index = *parms_index;
    for (i = 0; i < n; i++) {
      if (H5Z__nbit_decompress_one_compound(
              data, data_offset + i * (size_t)base_size, buffer, j, buf_len,
              parms, parms_index) < 0)
        HGOTO_ERROR(H5E_PLINE, H5E_CANTFILTER, FAIL,
                    "can't decompress compound");
      *parms_index = begin_index;
    }
    break;
  ...
  case H5Z_NBIT_NOOPTYPE:
    ...
    H5Z__nbit_decompress_one_nooptype(data, data_offset, buffer, j, buf_len,
                                      total_size);
    break;
...
static herr_t H5Z__nbit_decompress_one_compound(
    unsigned char *data, size_t data_offset, unsigned char *buffer, size_t *j,
    size_t *buf_len, const unsigned parms[], unsigned *parms_index) {
  unsigned i, nmembers, member_offset, member_class, member_size, used_size = 0,
                                                                  size;
  parms_atomic p;
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  for (i = 0; i < nmembers; i++) {
    ...
    if (used_size > size)
      HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL,
                  "compound member offset overflowed compound size");
    switch (member_class) {
    case H5Z_NBIT_ATOMIC:
      p.size = member_size;
      ...
      H5Z__nbit_decompress_one_atomic(data, data_offset + member_offset, buffer,
                                      j, buf_len, &p);
      break;
...
static herr_t H5Z__nbit_decompress(unsigned char *data, unsigned d_nelmts,
  ...
  size_t buf_len;
  parms_atomic p;
  unsigned parms_index;       /* index in array parms used by
  ...
  case H5Z_NBIT_ATOMIC:
    ...
    for (i = 0; i < d_nelmts; i++)
      H5Z__nbit_decompress_one_atomic(data, i * (size_t)p.size, buffer, &j,
                                      &buf_len, &p);
    break;
  ...
  case H5Z_NBIT_ARRAY:
    ...
    for (i = 0; i < d_nelmts; i++) {
      if (H5Z__nbit_decompress_one_array(data, i * size, buffer, &j, &buf_len,
                                         parms, &parms_index) < 0)
        HGOTO_ERROR(H5E_PLINE, H5E_CANTFILTER, FAIL, "can't decompress array");
      parms_index = 4;
    }
  ...
  case H5Z_NBIT_COMPOUND:
    ...
    parms_index = 4; /* set the index before goto function call */
    for (i = 0; i < d_nelmts; i++) {
      if (H5Z__nbit_decompress_one_compound(data, i * size, buffer, &j,
...
static void H5Z__nbit_compress_one_atomic(unsigned char *data,
                                          size_t data_offset,
                                          unsigned char *buffer, size_t *j,
                                          size_t *buf_len,
                                          const parms_atomic *p) {
  ...
  if (p->order == H5Z_NBIT_ORDER_LE) { /* little endian */
    ...
    for (k = (int)begin_i; k >= (int)end_i; k--)
      H5Z__nbit_compress_one_byte(data, data_offset, (unsigned)k, begin_i,
                                  end_i, buffer, j, buf_len, p, datatype_len);
  } else { /* big endian */
    /* Sanity check */
    assert(p->order == H5Z_NBIT_ORDER_BE);
...
H5Z__nbit_compress_one_array(unsigned char *data, size_t data_offset,
  ...
  case H5Z_NBIT_ATOMIC:
    ...
    n = total_size / p.size;
    for (i = 0; i < n; i++)
      H5Z__nbit_compress_one_atomic(data, data_offset + i * (size_t)p.size,
                                    buffer, j, buf_len, &p);
    break;
  ...
  case H5Z_NBIT_ARRAY:
    ...
    begin_index = *parms_index;
    for (i = 0; i < n; i++) {
      H5Z__nbit_compress_one_array(data, data_offset + i * (size_t)base_size,
  ...
  case H5Z_NBIT_COMPOUND:
    ...
    begin_index = *parms_index;
    for (i = 0; i < n; i++) {
      H5Z__nbit_compress_one_compound(data, data_offset + i * (size_t)base_size,
                                      buffer, j, buf_len, parms, parms_index);
      *parms_index = begin_index;
    }
    break;
  ...
  case H5Z_NBIT_NOOPTYPE:
    (*parms_index)++; /* skip size of no-op type */
    H5Z__nbit_compress_one_nooptype(data, data_offset, buffer, j, buf_len,
                                    total_size);
    break;
...
static void H5Z__nbit_compress_one_compound(
  ...
  for (i = 0; i < nmembers; i++) {
    ...
    case H5Z_NBIT_ATOMIC:
      ...
      p.precision = parms[(*parms_index)++];
      p.offset = parms[(*parms_index)++];
      H5Z__nbit_compress_one_atomic(data, data_offset + member_offset, buffer,
                                    j, buf_len, &p);
      break;

    case H5Z_NBIT_ARRAY:
      H5Z__nbit_compress_one_array(data, data_offset + member_offset, buffer, j,
                                   buf_len, parms, parms_index);
      break;

    case H5Z_NBIT_COMPOUND:
      H5Z__nbit_compress_one_compound(data, data_offset + member_offset, buffer,
                                      j, buf_len, parms, parms_index);
      break;

    case H5Z_NBIT_NOOPTYPE:
      size = parms[(*parms_index)++];
      H5Z__nbit_compress_one_nooptype(data, data_offset + member_offset, buffer,
                                      j, buf_len, size);
      break;
...
static void H5Z__nbit_compress(unsigned char *data, unsigned d_nelmts,
  ...
  case H5Z_NBIT_ATOMIC:
    ...
    for (i = 0; i < d_nelmts; i++)
      H5Z__nbit_compress_one_atomic(data, i * (size_t)p.size, buffer, &new_size,
                                    &buf_len, &p);
    break;
  ...
  case H5Z_NBIT_ARRAY:
    ...
    parms_index = 4;
    for (i = 0; i < d_nelmts; i++) {
      H5Z__nbit_compress_one_array(data, i * size, buffer, &new_size, &buf_len,
                                   parms, &parms_index);
      parms_index = 4;
    }
    break;
  ...
  case H5Z_NBIT_COMPOUND:
    ...
    parms_index = 4;
    for (i = 0; i < d_nelmts; i++) {
      H5Z__nbit_compress_one_compound(data, i * size, buffer, &new_size,
                                      &buf_len, parms, &parms_index);
      parms_index = 4;
    }
    break;
  ...
  /* Update the size to the new value after compression.  If there are any bits
   * hanging over in the last byte, increment the value by 1. */
