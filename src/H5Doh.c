 ...
 */
static void *H5O__dset_create(H5F_t *f, void *_crt_info, H5G_loc_t *obj_loc) {
  H5D_obj_create_t *crt_info =
      (H5D_obj_create_t *)_crt_info; /* Dataset creation parameters */
  ...
  if (NULL == (dset = H5D__create(f, crt_info->type_id, crt_info->space,
                                  crt_info->dcpl_id, crt_info->dapl_id)))
...
static herr_t H5O__dset_bh_info(const H5O_loc_t *loc, H5O_t *oh,
  ...
  hbool_t efl_read =
      FALSE;     /* Whether the external file list message was read */
  htri_t exists; /* Flag if header message of interest exists */
  ...
  if (layout.type == H5D_CHUNKED &&
    ...
    if (H5HG_get_obj_size(loc->file,
                          &(layout.storage.u.virt.serial_list_hobjid),
                          &virtual_heap_size) < 0)
  ...
  if ((exists = H5O_msg_exists_oh(oh, H5O_EFL_ID)) < 0)
    HGOTO_ERROR(H5E_OHDR, H5E_NOTFOUND, FAIL,
                "unable to check for EFL message");
  ...
  if (layout_read && H5O_msg_reset(H5O_LAYOUT_ID, &layout) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, FAIL,
                "unable to reset data storage layout message");
  if (efl_read && H5O_msg_reset(H5O_EFL_ID, &efl) < 0)
    HDONE_ERROR(H5E_DATASET, H5E_CANTRESET, FAIL,
                "unable to reset external file list message");
