 ...
 */
static size_t H5FL_reg_glb_mem_lim =
    1 * 1024 * 1024; /* Default to 1MB limit on all regular free lists */
static size_t H5FL_reg_lst_mem_lim =
    1 * 65536; /* Default to 64KB limit on each regular free list */
static size_t H5FL_arr_glb_mem_lim =
    4 * 1024 * 1024; /* Default to 4MB limit on all array free lists */
static size_t H5FL_arr_lst_mem_lim =
    4 * 65536; /* Default to 256KB limit on each array free list */
static size_t H5FL_blk_glb_mem_lim =
    16 * 1024 * 1024; /* Default to 16MB limit on all block free lists */
static size_t H5FL_blk_lst_mem_lim =
    1024 * 1024; /* Default to 1024KB (1MB) limit on each block free list */
static size_t H5FL_fac_glb_mem_lim =
    16 * 1024 * 1024; /* Default to 16MB limit on all factory free lists */
...
typedef struct H5FL_reg_gc_node_t {
  H5FL_reg_head_t
      *list; /* Pointer to the head of the list to garbage collect */
  struct H5FL_reg_gc_node_t *next; /* Pointer to the next node in the list of
...
typedef struct H5FL_reg_gc_list_t {
  ...
  struct H5FL_reg_gc_node_t *first; /* Pointer to the first node in the list of
                                       things to garbage collect */
} H5FL_reg_gc_list_t;
...
typedef struct H5FL_gc_arr_node_t {
  H5FL_arr_head_t
      *list; /* Pointer to the head of the list to garbage collect */
  struct H5FL_gc_arr_node_t *next; /* Pointer to the next node in the list of
                                      things to garbage collect */
} H5FL_gc_arr_node_t;
...
typedef struct H5FL_gc_arr_list_t {
  size_t mem_freed;                 /* Amount of free memory on list */
  struct H5FL_gc_arr_node_t *first; /* Pointer to the first node in the list of
                                       things to garbage collect */
} H5FL_gc_arr_list_t;
...
typedef struct H5FL_blk_gc_node_t {
  ...
  struct H5FL_blk_gc_node_t *next; /* Pointer to the next node in the list of
                                      things to garbage collect */
} H5FL_blk_gc_node_t;
...
typedef struct H5FL_blk_gc_list_t {
  size_t mem_freed;                 /* Amount of free memory on list */
  struct H5FL_blk_gc_node_t *first; /* Pointer to the first node in the list of
                                       things to garbage collect */
} H5FL_blk_gc_list_t;
...
struct H5FL_fac_gc_node_t {
  H5FL_fac_head_t
      *list; /* Pointer to the head of the list to garbage collect */
  struct H5FL_fac_gc_node_t *next; /* Pointer to the next node in the list of
                                      things to garbage collect */
};
...
typedef struct H5FL_fac_gc_list_t {
  size_t mem_freed;                 /* Amount of free memory on list */
  struct H5FL_fac_gc_node_t *first; /* Pointer to the first node in the list of
                                       things to garbage collect */
} H5FL_fac_gc_list_t;
...
static H5FL_blk_node_t *H5FL__blk_find_list(H5FL_blk_node_t **head,
                                            size_t size);
static H5FL_blk_node_t *H5FL__blk_create_list(H5FL_blk_node_t **head,
                                              size_t size);
  ...
  if (n > 0 && H5FL_out_head_g) {
    ...
    while (trk != NULL) {
      ...
      fprintf(stderr, "\tPtr: %p, File: %s, Function: %s, Line: %d\n",
              (((unsigned char *)trk) + sizeof(H5FL_track_t)), trk->file,
              trk->func, trk->line);
 ...
 *
 * Purpose:	Attempt to allocate space using malloc.  If malloc fails,
 *garbage collect and try again.  If malloc fails again, then return NULL.
 *
...
static void *H5FL__malloc(size_t mem_size) {
  ...
  if (NULL == (ret_value = H5MM_malloc(mem_size))) {
    ...
    if (H5FL_garbage_coll() < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGC, NULL,
                  "garbage collection failed during allocation");
...
static herr_t H5FL__reg_init(H5FL_reg_head_t *head) {
  H5FL_reg_gc_node_t
      *new_node; /* Pointer to the node for the new list to garbage collect */
  herr_t ret_value = SUCCEED; /* return value*/

  FUNC_ENTER_PACKAGE

  /* Allocate a new garbage collection node */
  if (NULL == (new_node = (H5FL_reg_gc_node_t *)H5MM_malloc(
                   sizeof(H5FL_reg_gc_node_t))))
  ...
  /* Make certain that the space allocated is large enough to store a free list
   * pointer (eventually) */
...
void *H5FL_reg_free(H5FL_reg_head_t *head, void *obj) {
  ...
  if (head->onlist * head->size > H5FL_reg_lst_mem_lim)
    if (H5FL__reg_gc_list(head) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGC, NULL,
                  "garbage collection failed during free");
...
void *H5FL_reg_malloc(H5FL_reg_head_t *head H5FL_TRACK_PARAMS) {
  ...
  /* The 'call_func' & 'call_file' strings are statically allocated (by the
   * compiler) there's no need to duplicate them.
   */
...
static herr_t H5FL__reg_gc_list(H5FL_reg_head_t *head) {
  H5FL_reg_node_t
      *free_list; /* Pointer to nodes in free list being garbage collected */

  FUNC_ENTER_PACKAGE_NOERR

  /* For each free list being garbage collected, walk through the nodes and free
   * them */
  free_list = head->list;
...
static herr_t H5FL__reg_gc(void) {
  H5FL_reg_gc_node_t
      *gc_node; /* Pointer into the list of things to garbage collect */
  herr_t ret_value = SUCCEED; /* return value*/
...
static int H5FL__reg_term(void) {
  H5FL_reg_gc_node_t
      *left; /* pointer to garbage collection lists with work left */

  FUNC_ENTER_PACKAGE_NOERR

  /* Free the nodes on the garbage collection list, keeping nodes with
   * allocations outstanding */
  left = NULL;
  while (H5FL_reg_gc_head.first != NULL) {
    H5FL_reg_gc_node_t
        *tmp; /* Temporary pointer to a garbage collection node */
    ...
    /* No allocations left open for list, get rid of it */
      ...
      /* Reset the "initialized" flag, in case we restart this list somehow (I
       * don't know how..) */
 ...
 *
 * Purpose:	Initialize a priority queue of a certain type.  Right now, this
 *just adds the PQ to the list of things to garbage collect.
 *
 * Return:	Success:	Non-negative
 * 		Failure:	Negative
 *
...
static herr_t H5FL__blk_init(H5FL_blk_head_t *head) {
  ...
  /* Allocate a new garbage collection node */
  if (NULL == (new_node = (H5FL_blk_gc_node_t *)H5MM_malloc(
                   sizeof(H5FL_blk_gc_node_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
...
htri_t H5FL_blk_free_block_avail(H5FL_blk_head_t *head, size_t size) {
  ...
  if ((free_list = H5FL__blk_find_list(&(head->head), size)) != NULL &&
      free_list->list != NULL)
    ret_value = TRUE;
  else
    ret_value = FALSE;
...
void *H5FL_blk_malloc(H5FL_blk_head_t *head, size_t size H5FL_TRACK_PARAMS) {
  ...
  if (!head->init)
    if (H5FL__blk_init(head) < 0)
      HGOTO_ERROR(H5E_RESOURCE, H5E_CANTINIT, NULL,
                  "can't initialize 'block' list");

  /* check if there is a free list for blocks of this size */
  /* and if there are any blocks available on the list */
  if (NULL != (free_list = H5FL__blk_find_list(&(head->head), size)) &&
      NULL != free_list->list) {
  ...
  /* No free list available, or there are no nodes on the list, allocate a new
     node to give to the user */
  else {
    ...
    /* Allocate new node, with room for the page info header and the actual page
     * data */
    if (NULL == (temp = (H5FL_blk_list_t *)H5FL__malloc(
                     sizeof(H5FL_blk_list_t) + H5FL_TRACK_SIZE + size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                  "memory allocation failed for chunk");
...
void *H5FL_blk_free(H5FL_blk_head_t *head, void *block) {
  ...
  /* Get the pointer to the native block info header in front of the native
   * block to free */
  temp = (H5FL_blk_list_t *)((
      void *)((unsigned char *)block -
              (sizeof(H5FL_blk_list_t) +
               H5FL_TRACK_SIZE))); /*lint !e826 Pointer-to-pointer cast is
                                      appropriate here */
 ...
 *
 * Purpose:	Resizes a block.  This does things the straightforward, simple
 *way, not actually using realloc.
 *
 * Return:	Success:	NULL
 *
 *		Failure:	never fails
 ...
 *-------------------------------------------------------------------------
 */
void *H5FL_blk_realloc(H5FL_blk_head_t *head, void *block,
  ...
  if (block != NULL) {
    ...
    if (new_size != temp->size) {
      ...
      if ((ret_value = H5FL_blk_malloc(head, new_size H5FL_TRACK_INFO_INT)) ==
          NULL)
        HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                    "memory allocation failed for block");
...
#ifdef H5FL_TRACK
      {
        unsigned char *block_ptr =
            ((unsigned char *)block) - sizeof(H5FL_track_t);
        H5FL_track_t trk;
        ...
        H5CS_close_stack(trk.stack);
        /* The 'func' & 'file' strings are statically allocated (by the
        ...
        assert(trk.stack);
        /* The 'call_func' & 'call_file' strings are statically allocated (by
         * the compiler) there's no need to duplicate them.
         */
        trk.file = call_file;
        trk.func = call_func;
        trk.line = call_line;
...
static herr_t H5FL__blk_gc_list(H5FL_blk_head_t *head) {
  ...
  while (blk_head != NULL) {
    ...
    H5FL_blk_list_t
        *list; /* The free list of native nodes of a particular size */

    /* Sanity check */
    assert((blk_head->onlist && blk_head->list) ||
           (0 == blk_head->onlist && NULL == blk_head->list));
...
static herr_t H5FL__blk_gc(void) {
  ...
  while (gc_node != NULL) {
    /* For each free list being garbage collected, walk through the nodes and
     * free them */
...
static herr_t H5FL__arr_init(H5FL_arr_head_t *head) {
  ...
  /* Allocate a new garbage collection node */
  if (NULL == (new_node = (H5FL_gc_arr_node_t *)H5MM_malloc(
                   sizeof(H5FL_gc_arr_node_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
  ...
  if (NULL == (head->list_arr = (H5FL_arr_node_t *)H5MM_calloc(
                   (size_t)head->maxelem * sizeof(H5FL_arr_node_t))))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL, "memory allocation failed");
 ...
 *-------------------------------------------------------------------------
 */
void *H5FL_arr_malloc(H5FL_arr_head_t *head, size_t elem H5FL_TRACK_PARAMS) {
  ...
  /* Otherwise allocate a node */
    ...
    if (NULL == (new_obj = (H5FL_arr_list_t *)H5FL__malloc(
                     sizeof(H5FL_arr_list_t) + H5FL_TRACK_SIZE + mem_size)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
 ...
 *-------------------------------------------------------------------------
 */
void *H5FL_arr_calloc(H5FL_arr_head_t *head, size_t elem H5FL_TRACK_PARAMS) {
  void *ret_value = NULL; /* Pointer to the block to return */
 ...
 *-------------------------------------------------------------------------
 */
void *H5FL_arr_realloc(H5FL_arr_head_t *head, void *obj,
...
static herr_t H5FL__arr_gc_list(H5FL_arr_head_t *head) {
  ...
  for (u = 0; u < (unsigned)head->maxelem; u++) {
    if (head->list_arr[u].onlist > 0) {
      H5FL_arr_list_t *arr_free_list; /* Pointer to nodes in free list being
                                         garbage collected */
      ...
      H5FL_arr_gc_head.mem_freed -=
          (head->list_arr[u].onlist * head->list_arr[u].size);
...
static herr_t H5FL__arr_gc(void) {
  H5FL_gc_arr_node_t
      *gc_arr_node; /* Pointer into the list of things to garbage collect */
  herr_t ret_value = SUCCEED; /* return value*/
...
void *H5FL_seq_malloc(H5FL_seq_head_t *head, size_t elem H5FL_TRACK_PARAMS) {
  ...
  /* Use block routine */
  ret_value =
      H5FL_blk_malloc(&(head->queue), head->size * elem H5FL_TRACK_INFO_INT);
...
void *H5FL_seq_calloc(H5FL_seq_head_t *head, size_t elem H5FL_TRACK_PARAMS) {
  ...
  /* Use block routine */
  ret_value =
      H5FL_blk_calloc(&(head->queue), head->size * elem H5FL_TRACK_INFO_INT);
 ...
 *-------------------------------------------------------------------------
 */
void *H5FL_seq_realloc(H5FL_seq_head_t *head, void *obj,
                       size_t new_elem H5FL_TRACK_PARAMS) {
  void *ret_value = NULL; /* Pointer to the block to return */
  ...
  /* Use block routine */
  ret_value = H5FL_blk_realloc(&(head->queue), obj,
                               head->size * new_elem H5FL_TRACK_INFO_INT);
...
H5FL_fac_head_t *H5FL_fac_init(size_t size) {
  H5FL_fac_gc_node_t *new_node =
      NULL; /* Pointer to the node for the new list to garbage collect */
  ...
  if (NULL == (factory = (H5FL_fac_head_t *)H5FL_CALLOC(H5FL_fac_head_t)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL,
                "memory allocation failed for factory object");
  ...
  /* Allocate a new garbage collection node */
  if (NULL ==
      (new_node = (H5FL_fac_gc_node_t *)H5FL_MALLOC(H5FL_fac_gc_node_t)))
    HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, "memory allocation failed");
...
herr_t H5FL_fac_term(H5FL_fac_head_t *factory) {
  ...
  if (factory->prev_gc) {
    ...
    H5FL_fac_gc_head.first =
        H5FL_FREE(H5FL_fac_gc_node_t, H5FL_fac_gc_head.first);
...
herr_t H5FL_garbage_coll(void) {
  ...
  if (H5FL__arr_gc() < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGC, FAIL,
                "can't garbage collect array objects");

  /* Garbage collect free lists for blocks */
  if (H5FL__blk_gc() < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGC, FAIL,
                "can't garbage collect block objects");

  /* Garbage collect the free lists for regular objects */
  if (H5FL__reg_gc() < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGC, FAIL,
                "can't garbage collect regular objects");

  /* Garbage collect the free lists for factory objects */
  if (H5FL__fac_gc() < 0)
    HGOTO_ERROR(H5E_RESOURCE, H5E_CANTGC, FAIL,
                "can't garbage collect factory objects");
 ...
 *
 * Purpose:	Sets limits on the different kinds of free lists.  Setting a
 *value of -1 for a limit means no limit of that type.  These limits are global
 ...
 *  int reg_list_lim;    IN: The limit on memory used in each "regular" free
 *list int arr_global_lim;  IN: The limit on all "array" free list memory used
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5FL_set_free_list_limits(int reg_global_lim, int reg_list_lim,
  ...
  H5FL_reg_glb_mem_lim =
      (reg_global_lim == -1 ? UINT_MAX : (size_t)reg_global_lim);
  ...
  H5FL_arr_glb_mem_lim =
      (arr_global_lim == -1 ? UINT_MAX : (size_t)arr_global_lim);
  ...
  H5FL_blk_glb_mem_lim =
      (blk_global_lim == -1 ? UINT_MAX : (size_t)blk_global_lim);
  ...
  H5FL_fac_glb_mem_lim =
      (fac_global_lim == -1 ? UINT_MAX : (size_t)fac_global_lim);
 ...
 *
 * Parameters:
 *  size_t *reg_size;    OUT: The current size of all "regular" free list memory
 *used size_t *arr_size;    OUT: The current size of all "array" free list
 *memory used size_t *blk_size;    OUT: The current size of all "block" free
 *list memory used size_t *fac_size;    OUT: The current size of all "factory"
 *free list memory used
 *
 * Return:	Success:	non-negative
 *		Failure:	negative
 *
 *-------------------------------------------------------------------------
 */
herr_t H5FL_get_free_list_sizes(size_t *reg_size, size_t *arr_size,
  ...
  if (arr_size) {
    ...
    *arr_size = 0;
    gc_arr_node = H5FL_arr_gc_head.first;
    while (gc_arr_node != NULL) {
      H5FL_arr_head_t *head =
          gc_arr_node->list; /* Head of array list elements */
  ...
  if (fac_size) {
    H5FL_fac_gc_node_t
        *gc_fac_node; /* Pointer into the list of things to garbage collect */

    /* Walk through all the free lists */
    gc_fac_node = H5FL_fac_gc_head.first;
    *fac_size = 0;
    while (gc_fac_node != NULL) {
      H5FL_fac_head_t *fac_head =
          gc_fac_node->list; /* Head node for factory list */
