...
typedef struct H5G_name_t {
  H5RS_str_t *full_path_r; /* Path to object, as seen from root of current file
                              mounting hierarchy */
 ...
 * own_loc should be set to H5G_OWN_OBJ_LOC if this callback takes ownership of
 * obj_loc, H5G_OWN_GRP_LOC if it takes ownership of grp_loc, and H5G_OWN_NONE
 * if obj_loc and grp_loc need to be deleted.
 */
...
H5_DLL herr_t H5G_obj_insert(const struct H5O_loc_t *grp_oloc, const char *name,
                             struct H5O_link_t *obj_lnk, hbool_t adj_link,
                             H5O_type_t obj_type, const void *crt_info);
H5_DLL herr_t H5G_obj_get_name_by_idx(const struct H5O_loc_t *oloc,
                                      H5_index_t idx_type,
...
H5_DLL herr_t H5G_obj_lookup_by_idx(const struct H5O_loc_t *grp_oloc,
                                    H5_index_t idx_type, H5_iter_order_t order,
                                    hsize_t n, struct H5O_link_t *lnk);
...
H5_DLL herr_t H5G_ent_decode(const H5F_t *f, const uint8_t **pp,
                             H5G_entry_t *ent, const uint8_t *p_end);

/*
 * These functions operate on group hierarchy names.
 */
H5_DLL herr_t H5G_name_set(const H5G_name_t *loc, H5G_name_t *obj,
                           const char *name);
...
H5_DLL herr_t H5G_name_copy(H5G_name_t *dst, const H5G_name_t *src,
                            H5_copy_depth_t depth);
H5_DLL herr_t H5G_name_free(H5G_name_t *name);
H5_DLL herr_t H5G_get_name(const H5G_loc_t *loc, char *name /*out*/,
                           size_t size, size_t *name_len, hbool_t *cached);
H5_DLL herr_t H5G_get_name_by_addr(H5F_t *f, const struct H5O_loc_t *loc,
                                   char *name, size_t size, size_t *name_len);
...
H5_DLL herr_t H5G_loc_copy(H5G_loc_t *dst, const H5G_loc_t *src,
                           H5_copy_depth_t depth);
...
H5_DLL herr_t H5G_loc_exists(const H5G_loc_t *loc, const char *name,
                             hbool_t *exists);
...
H5_DLL herr_t H5G_loc_set_comment(const H5G_loc_t *loc, const char *name,
                                  const char *comment);
H5_DLL herr_t H5G_loc_get_comment(const H5G_loc_t *loc, const char *name,
