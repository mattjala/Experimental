...
typedef struct H5FO_open_obj_t {
  ...
  hbool_t
      deleted; /* Flag to indicate that the object was deleted from the file */
} H5FO_open_obj_t;
...
herr_t H5FO_create(const H5F_t *f) {
  ...
  if ((f->shared->open_objs = H5SL_create(H5SL_TYPE_HADDR, NULL)) == NULL)
    HGOTO_ERROR(H5E_FILE, H5E_CANTINIT, FAIL,
                "unable to create open object container");
...
void *H5FO_opened(const H5F_t *f, haddr_t addr) {
  ...
  if (NULL != (open_obj = (H5FO_open_obj_t *)H5SL_search(f->shared->open_objs,
                                                         &addr))) {
...
/*--------------------------------------------------------------------------
 ...
 REVISION LOG
--------------------------------------------------------------------------*/
herr_t H5FO_insert(const H5F_t *f, haddr_t addr, void *obj,
                   hbool_t delete_flag) {
  H5FO_open_obj_t *open_obj;  /* Information about open object */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5SL_insert(f->shared->open_objs, &open_obj->addr, open_obj) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTINSERT, FAIL,
                "can't insert object into container");
...
herr_t H5FO_delete(H5F_t *f, haddr_t addr) {
  ...
  if (NULL ==
      (open_obj = (H5FO_open_obj_t *)H5SL_remove(f->shared->open_objs, &addr)))
    HGOTO_ERROR(H5E_CACHE, H5E_CANTRELEASE, FAIL,
                "can't remove object from container");
...
herr_t H5FO_mark(const H5F_t *f, haddr_t addr, hbool_t deleted) {
  ...
  /* Get the object node from the container */
  if (NULL !=
      (open_obj = (H5FO_open_obj_t *)H5SL_search(f->shared->open_objs, &addr)))
...
herr_t H5FO_dest(const H5F_t *f) {
  ...
  if (H5SL_close(f->shared->open_objs) < 0)
    HGOTO_ERROR(H5E_CACHE, H5E_CANTCLOSEOBJ, FAIL,
                "can't close open object info set");
...
herr_t H5FO_top_incr(const H5F_t *f, haddr_t addr) {
  ...
  /* Get the object node from the container */
  if (NULL !=
      (obj_count = (H5FO_obj_count_t *)H5SL_search(f->obj_count, &addr))) {
    (obj_count->count)++;
  } /* end if */
  else {
    /* Allocate new opened object information structure */
...
herr_t H5FO_top_decr(const H5F_t *f, haddr_t addr) {
  ...
  if (NULL !=
    ...
    if (obj_count->count == 0) {
      /* Remove from container */
      if (NULL ==
          (obj_count = (H5FO_obj_count_t *)H5SL_remove(f->obj_count, &addr)))
        HGOTO_ERROR(H5E_CACHE, H5E_CANTRELEASE, FAIL,
                    "can't remove object from container");
