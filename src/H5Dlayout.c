 ...
 *-------------------------------------------------------------------------
 */
size_t H5D__layout_meta_size(const H5F_t *f, const H5O_layout_t *layout,
  ...
  case H5D_COMPACT:
    ...
    if (include_compact_data)
      ret_value +=
          layout->storage.u.compact.size; /* data for compact dataset */
    break;
  ...
  case H5D_CHUNKED:
    if (layout->version < H5O_LAYOUT_VERSION_4) {
      ...
      assert(layout->u.chunk.enc_bytes_per_dim > 0 &&
             layout->u.chunk.enc_bytes_per_dim <= 8);
      ret_value++;

      /* Dimension sizes */
      ret_value +=
          layout->u.chunk.ndims * (size_t)layout->u.chunk.enc_bytes_per_dim;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__layout_set_latest_indexing(H5O_layout_t *layout, const H5S_t *space,
    ...
    if (ndims > 0) {
      ...
      if (H5S_get_simple_extent_dims(space, cur_dims, max_dims) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                    "can't get dataspace max. dimensions");
      ...
      if (unlim_count) { /* dataset with unlimited dimension(s) must be chunked
                          */
        if (1 ==
            unlim_count) { /* Chunked dataset with only 1 unlimited dimension */
           ...
           */
          layout->u.chunk.u.earray.cparam.max_nelmts_bits =
              H5D_EARRAY_MAX_NELMTS_BITS;
          layout->u.chunk.u.earray.cparam.idx_blk_elmts =
              H5D_EARRAY_IDX_BLK_ELMTS;
          layout->u.chunk.u.earray.cparam.sup_blk_min_data_ptrs =
              H5D_EARRAY_SUP_BLK_MIN_DATA_PTRS;
          layout->u.chunk.u.earray.cparam.data_blk_min_elmts =
              H5D_EARRAY_DATA_BLK_MIN_ELMTS;
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__layout_oh_create(H5F_t *file, H5O_t *oh, H5D_t *dset,
                             hid_t dapl_id) {
  ...
  if (layout->type == H5D_CHUNKED) {
    ...
    if (pline->nused > 0 &&
        H5O_msg_append_oh(file, oh, H5O_PLINE_ID, H5O_MSG_FLAG_CONSTANT, 0,
                          pline) < 0)
  ...
  if (dset->shared->layout.ops->init &&
      (dset->shared->layout.ops->init)(file, dset, dapl_id) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to initialize layout information");
  ...
  if (dset->shared->dcpl_cache.efl.nused > 0) {
    H5O_efl_t *efl =
        &dset->shared->dcpl_cache.efl; /* Dataset's external file list */
    ...
    if (H5HL_unprotect(heap) < 0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTUNPROTECT, FAIL,
                  "unable to unprotect EFL file name heap");
    heap = NULL;

    /* Insert EFL message into dataset object header */
    if (H5O_msg_append_oh(file, oh, H5O_EFL_ID, H5O_MSG_FLAG_CONSTANT, 0, efl) <
        0)
      HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
   ...
   */
  /* (Note: this is relying on H5D__alloc_storage not calling H5O_msg_write
   * during dataset creation) */
  if (fill_prop->alloc_time == H5D_ALLOC_TIME_EARLY &&
      H5D_COMPACT != layout->type && !dset->shared->dcpl_cache.pline.nused &&
  ...
  /* Create layout message */
  if (H5O_msg_append_oh(file, oh, H5O_LAYOUT_ID, layout_mesg_flags, 0, layout) <
      0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__layout_oh_read(H5D_t *dataset, hid_t dapl_id,
                           H5P_genplist_t *plist) {
  htri_t msg_exists; /* Whether a particular type of message exists */
  hbool_t pline_copied =
      FALSE; /* Flag to indicate that dcpl_cache.pline's message was copied */
  hbool_t layout_copied =
      FALSE; /* Flag to indicate that layout message was copied */
  hbool_t efl_copied =
      FALSE; /* Flag to indicate that the EFL message was copied */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (msg_exists) {
    ...
    if (NULL == H5O_msg_read(&(dataset->oloc), H5O_PLINE_ID,
                             &dataset->shared->dcpl_cache.pline))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't retrieve message");
    pline_copied = TRUE;
    /* Set the I/O pipeline info in the property list */
    if (H5P_set(plist, H5O_CRT_PIPELINE_NAME,
                &dataset->shared->dcpl_cache.pline) < 0)
   ...
   */
  if (NULL ==
      H5O_msg_read(&(dataset->oloc), H5O_LAYOUT_ID, &(dataset->shared->layout)))
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
  ...
  if (msg_exists) {
    ...
    if (NULL == H5O_msg_read(&(dataset->oloc), H5O_EFL_ID,
                             &dataset->shared->dcpl_cache.efl))
      HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't retrieve message");
    efl_copied = TRUE;

    /* Set the EFL info in the property list */
    if (H5P_set(plist, H5D_CRT_EXT_FILE_LIST_NAME,
  ...
  if (dataset->shared->layout.ops->init &&
      (dataset->shared->layout.ops->init)(dataset->oloc.file, dataset,
                                          dapl_id) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                "unable to initialize layout information");

  /* Adjust chunk dimensions to omit datatype size (in last dimension) for
   * creation property */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5D__layout_oh_write(const H5D_t *dataset, H5O_t *oh,
  ...
  if ((msg_exists = H5O_msg_exists_oh(oh, H5O_LAYOUT_ID)) < 0)
    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                "unable to check if layout message exists");
  if (msg_exists) {
