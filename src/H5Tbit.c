 ...
 *-------------------------------------------------------------------------
 */
void H5T__bit_copy(uint8_t *dst, size_t dst_offset, const uint8_t *src,
  ...
  while (src_offset && size > 0) {
    ...
    dst[d_idx] =
        (uint8_t)(dst[d_idx] |
                  (((src[s_idx] >> src_offset) & (uint8_t)mask) << dst_offset));
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5T__bit_shift(uint8_t *buf, ssize_t shift_dist, size_t offset,
                      size_t size) {
  ...
  if (shift_dist) {
    ...
    if (abs_shift_dist >= size)
      ...
      if (shift_dist > 0) { /* left shift */
        ...
        H5T__bit_copy(buf, offset + abs_shift_dist, shift_buf, (size_t)0,
                      size - abs_shift_dist);

        /* Zero-set the left part*/
        H5T__bit_set(buf, offset, abs_shift_dist, 0);
      } else { /* right shift */
        H5T__bit_copy(shift_buf, (size_t)0, buf, offset + abs_shift_dist,
                      size - abs_shift_dist);
 ...
 *-------------------------------------------------------------------------
 */
ssize_t H5T__bit_find(const uint8_t *buf, size_t offset, size_t size,
...
hbool_t H5T__bit_dec(uint8_t *buf, size_t start, size_t size) {
  ...
  if ((size + start - 1) / 8 > idx) {
    ...
    /* Example:  a sequence like 11000100 and start = 3.  We subtract 00001000
     * from it and get 10111100.  If a sequence is 00000111, we do right shift
     * for START bits and get 00000000.  So we need to borrow from higher byte
     * when we subtract 00001000.
     */
    if (!(buf[idx] >> pos))
      borrow = 1;
    ...
    /* The last partial byte */
    if (borrow && size > 0) {
      /* Similar to the first byte case, where sequence ends in the same byte as
       * starts */
      tmp = buf[idx];
      buf[idx]--;
    ...
    /* Example: a sequence like 11000100 and pos=3, size=3.  We subtract
     * 00001000 and get 10111100.  A bit is borrowed from 6th
     * bit(buf[idx]>>6=00000010, tmp>>6=00000011, not equal).  We need to put
     * this bit back by increment 1000000.
     */
    tmp = buf[idx];
    buf[idx] = (uint8_t)(buf[idx] - (1 << pos));
...
void H5T__bit_neg(uint8_t *buf, size_t start, size_t size) {
  ...
  if ((size + start - 1) / 8 >
      idx) { /*bit sequence doesn't end in the same byte as starts*/
