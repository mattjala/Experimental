...
static herr_t H5O__attr_to_dense_cb(H5O_t *oh, H5O_mesg_t *mesg,
                                    unsigned H5_ATTR_UNUSED sequence,
...
static herr_t H5O__attr_write_cb(H5O_t *oh, H5O_mesg_t *mesg,
                                 unsigned H5_ATTR_UNUSED sequence,
                                 unsigned *oh_modified, void *_udata);
static herr_t H5O__attr_rename_chk_cb(H5O_t H5_ATTR_UNUSED *oh,
                                      H5O_mesg_t *mesg,
                                      unsigned H5_ATTR_UNUSED sequence,
                                      unsigned H5_ATTR_UNUSED *oh_modified,
                                      void *_udata);
static herr_t H5O__attr_rename_mod_cb(H5O_t *oh, H5O_mesg_t *mesg,
                                      unsigned H5_ATTR_UNUSED sequence,
                                      unsigned *oh_modified, void *_udata);
static herr_t H5O__attr_remove_update(const H5O_loc_t *loc, H5O_t *oh,
                                      H5O_ainfo_t *ainfo);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_to_dense_cb(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                    unsigned H5_ATTR_UNUSED sequence,
...
herr_t H5O__attr_create(const H5O_loc_t *loc, H5A_t *attr) {
  ...
  htri_t shared_mesg; /* Should this message be stored in the Shared Message
                         table? */
  ...
  if (oh->version > H5O_VERSION_1) {
    hbool_t new_ainfo =
        FALSE; /* Flag to indicate that the attribute information is new */
    ...
    if ((ainfo_exists = H5A__get_ainfo(loc->file, oh, &ainfo)) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                  "can't check for attribute info message");
    if (!ainfo_exists) {
      /* Initialize attribute information */
      ainfo.track_corder =
          (hbool_t)((oh->flags & H5O_HDR_ATTR_CRT_ORDER_TRACKED) ? TRUE
                                                                 : FALSE);
      ainfo.index_corder =
          (hbool_t)((oh->flags & H5O_HDR_ATTR_CRT_ORDER_INDEXED) ? TRUE
                                                                 : FALSE);
      ainfo.max_crt_idx = 0;
      ...
      assert(ainfo.track_corder ==
             ((oh->flags & H5O_HDR_ATTR_CRT_ORDER_TRACKED) > 0));
      assert(ainfo.index_corder ==
             ((oh->flags & H5O_HDR_ATTR_CRT_ORDER_INDEXED) > 0));
    } /* end else */
    ...
    if (!H5_addr_defined(ainfo.fheap_addr)) {
      ...
      if ((shareable =
               H5SM_can_share(loc->file, NULL, NULL, H5O_ATTR_ID, attr)) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_BADMESG, FAIL,
                    "can't determine attribute sharing status");
      else if (shareable == FALSE) {
        /* Compute the size needed to encode the attribute */
        raw_size = (H5O_MSG_ATTR->raw_size)(loc->file, FALSE, attr);
      } /* end if */

      /* Check for condititions for switching to "dense" attribute storage are
       * met */
      if (ainfo.nattrs == oh->max_compact ||
          (!shareable && raw_size >= H5O_MESG_MAX_SIZE)) {
    ...
    if (ainfo.track_corder) {
      ...
      if (ainfo.max_crt_idx == H5O_MAX_CRT_ORDER_IDX)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTINC, FAIL,
                    "attribute creation index can't be incremented");

      /* Set the creation order index on the attribute & incr. creation order
       * index */
    ...
    if (new_ainfo) {
      if (H5O__msg_append_real(loc->file, oh, H5O_MSG_AINFO,
                               H5O_MSG_FLAG_DONTSHARE, 0, &ainfo) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTINSERT, FAIL,
                    "unable to create new attribute info message");
    } /* end if */
    /* Otherwise, update existing message */
    else if (H5O__msg_write_real(loc->file, oh, H5O_MSG_AINFO,
                                 H5O_MSG_FLAG_DONTSHARE, 0, &ainfo) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTUPDATE, FAIL,
                  "unable to update attribute info message");
  } /* end if */
  else {
    /* Set "bogus" creation index for attribute */
    attr->shared->crt_idx = H5O_MAX_CRT_ORDER_IDX;
  ...
  if (H5O_touch_oh(loc->file, oh, FALSE) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTUPDATE, FAIL,
                "unable to update time on object");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_open_cb(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                unsigned sequence,
                                unsigned H5_ATTR_UNUSED *oh_modified,
                                void *_udata /*in,out*/) {
  ...
  if (HDstrcmp(((H5A_t *)mesg->native)->shared->name, udata->name) == 0) {
    ...
    if (NULL == (udata->attr = H5A__copy(NULL, (H5A_t *)mesg->native)))
      HGOTO_ERROR(H5E_ATTR, H5E_CANTCOPY, H5_ITER_ERROR,
...
H5A_t *H5O__attr_open_by_name(const H5O_loc_t *loc, const char *name) {
  ...
  if (NULL == (oh = H5O_protect(loc, H5AC__READ_ONLY_FLAG, FALSE)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTPROTECT, NULL,
  ...
  if (oh->version > H5O_VERSION_1) {
    ...
    if (H5A__get_ainfo(loc->file, oh, &ainfo) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, NULL,
                  "can't check for attribute info message");
  } /* end if */
   ...
   */
  if ((found_open_attr = H5O__attr_find_opened_attr(loc, &exist_attr, name)) <
      0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, NULL,
                "failed in finding opened attribute");
  else if (found_open_attr == TRUE) {
    if (NULL == (opened_attr = H5A__copy(NULL, exist_attr)))
      HGOTO_ERROR(H5E_ATTR, H5E_CANTCOPY, NULL,
                  "can't copy existing attribute");
  } /* end else if */
  else {
    ...
    if (H5_addr_defined(ainfo.fheap_addr)) {
      ...
      if (!udata.attr)
        HGOTO_ERROR(H5E_ATTR, H5E_NOTFOUND, NULL,
                    "can't locate attribute: '%s'", name);
    ...
    if (H5T_set_loc(opened_attr->shared->dt, H5F_VOL_OBJ(loc->file),
                    H5T_LOC_DISK) < 0)
  ...
  if (oh && H5O_unprotect(loc, oh, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CANTUNPROTECT, NULL,
 ...
 *-------------------------------------------------------------------------
 */
H5A_t *H5O__attr_open_by_idx(const H5O_loc_t *loc, H5_index_t idx_type,
  ...
  if (H5O_attr_iterate_real((hid_t)-1, loc, idx_type, order, n, NULL, &attr_op,
                            &opened_attr) < 0)
   ...
   */
  if (opened_attr) {
    if ((found_open_attr = H5O__attr_find_opened_attr(
             loc, &exist_attr, opened_attr->shared->name)) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, NULL,
                  "failed in finding opened attribute");
...
static htri_t H5O__attr_find_opened_attr(const H5O_loc_t *loc, H5A_t **attr,
  ...
  if (H5F_get_obj_count(loc->file, H5F_OBJ_ATTR | H5F_OBJ_LOCAL, FALSE,
                        &num_open_attr) < 0)
  ...
  if (num_open_attr) {
    ...
    if (NULL ==
      ...
      HGOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL,
                  "unable to allocate memory for attribute ID list");

    /* Retrieve the IDs of all opened attributes */
    if (H5F_get_obj_ids(loc->file, H5F_OBJ_ATTR | H5F_OBJ_LOCAL, num_open_attr,
                        attr_id_list, FALSE, &check_num_attr) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                  "can't get IDs of opened attributes");
    ...
    for (u = 0; u < num_open_attr; u++) {
      ...
      if (NULL ==
          (*attr = (H5A_t *)H5VL_object_verify(attr_id_list[u], H5I_ATTR)))
      ...
      if (!HDstrcmp(name_to_open, (*attr)->shared->name) &&
          loc->addr == (*attr)->oloc.addr && loc_fnum == attr_fnum) {
        ret_value = TRUE;
        break;
      } /* end if */
    }   /* end for */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__attr_update_shared(H5F_t *f, H5O_t *oh, H5A_t *attr,
  ...
  if ((shared_mesg = H5SM_try_share(f, oh, 0, H5O_ATTR_ID, attr, NULL)) == 0)
    HGOTO_ERROR(H5E_ATTR, H5E_BADMESG, FAIL,
                "attribute changed sharing status");
  else if (shared_mesg < 0)
  ...
  if (attr_rc == 1)
    ...
    if (H5O__attr_link(f, oh, attr) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_LINKCOUNT, FAIL,
                  "unable to adjust attribute link count");

  /* Remove the old attribute from the SOHM storage */
  if (H5SM_delete(f, oh, &sh_mesg) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTFREE, FAIL,
                "unable to delete shared attribute in shared storage");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_write_cb(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                 unsigned H5_ATTR_UNUSED sequence,
                                 unsigned *oh_modified,
                                 void *_udata /*in,out*/) {
  ...
  /* Check for correct attribute message to modify */
  if (0 == HDstrcmp(((H5A_t *)mesg->native)->shared->name,
                    udata->attr->shared->name)) {
    /* Protect chunk */
    if (NULL == (chk_proxy = H5O__chunk_protect(udata->f, oh, mesg->chunkno)))
      HGOTO_ERROR(H5E_ATTR, H5E_CANTPROTECT, H5_ITER_ERROR,
                  "unable to load object header chunk");

    /* Because the attribute structure is shared now. The only situation that
     * requires copying the data is when the metadata cache evicts and reloads
     * this attribute.
     * The shared attribute structure will be different in that situation.
     * SLU-2010/7/29 */
    if (((H5A_t *)mesg->native)->shared != udata->attr->shared) {
      ...
      assert(((H5A_t *)mesg->native)->shared->data !=
             udata->attr->shared->data);

      /* (Needs to occur before updating the shared message, or the hash
       *      value on the old & new messages will be the same) */
      H5MM_memcpy(((H5A_t *)mesg->native)->shared->data,
                  udata->attr->shared->data, udata->attr->shared->data_size);
    } /* end if */
    ...
    if (mesg->flags & H5O_MSG_FLAG_SHARED)
      if (H5O__attr_update_shared(udata->f, oh, udata->attr,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_rename_mod_cb(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                      unsigned H5_ATTR_UNUSED sequence,
                                      unsigned *oh_modified,
                                      void *_udata /*in,out*/) {
  H5O_iter_ren_t *udata = (H5O_iter_ren_t *)_udata; /* Operator user data */
  ...
  if (HDstrcmp(((H5A_t *)mesg->native)->shared->name, udata->old_name) == 0) {
    unsigned old_version =
        ((H5A_t *)mesg->native)
            ->shared->version; /* Old version of the attribute */
    ...
    if (H5A__set_version(udata->f, ((H5A_t *)mesg->native)) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTSET, H5_ITER_ERROR,
    ...
    if (mesg->flags & H5O_MSG_FLAG_SHARED) {
      /* Update the shared attribute in the SOHM storage */
      if (H5O__attr_update_shared(udata->f, oh, (H5A_t *)mesg->native, NULL) <
          0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTUPDATE, H5_ITER_ERROR,
                    "unable to update attribute in shared storage");
    } /* end if */
      ...
      if (HDstrlen(udata->new_name) != HDstrlen(udata->old_name) ||
        ...
        *oh_modified = H5O_MODIFY_CONDENSE;
        ...
        if (H5O__msg_append_real(udata->f, oh, H5O_MSG_ATTR,
                                 (mesg->flags | H5O_MSG_FLAG_DONTSHARE), 0,
                                 attr) < 0)
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__attr_rename(const H5O_loc_t *loc, const char *old_name,
                        const char *new_name) {
  H5O_t *oh = NULL;           /* Pointer to actual object header */
  H5O_ainfo_t ainfo;          /* Attribute information for object */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5_addr_defined(ainfo.fheap_addr)) {
    ...
    if (udata.found)
      HGOTO_ERROR(H5E_ATTR, H5E_EXISTS, FAIL,
    ...
    if (!udata.found)
      HGOTO_ERROR(H5E_ATTR, H5E_NOTFOUND, FAIL,
                  "can't locate attribute with old name");
  } /* end else */
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O_attr_iterate_real(hid_t loc_id, const H5O_loc_t *loc,
  ...
  /* Protect the object header to iterate over */
  if (NULL == (oh = H5O_protect(loc, H5AC__READ_ONLY_FLAG, FALSE)))
    HGOTO_ERROR(H5E_ATTR, H5E_CANTPROTECT, FAIL,
                "unable to load object header");
  ...
  if (H5_addr_defined(ainfo.fheap_addr)) {
    ...
    if (H5O_unprotect(loc, oh, H5AC__NO_FLAGS_SET) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTUNPROTECT, FAIL,
                  "unable to release object header");
    oh = NULL;

    /* Iterate over attributes in dense storage */
    if ((ret_value =
             H5A__dense_iterate(loc->file, loc_id, &ainfo, idx_type, order,
                                skip, last_attr, attr_op, op_data)) < 0)
    ...
    if ((ret_value = H5A__attr_iterate_table(&atable, skip, last_attr, loc_id,
                                             attr_op, op_data)) < 0)
  ...
  /* Release resources */
  if (oh && H5O_unprotect(loc, oh, H5AC__NO_FLAGS_SET) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CANTUNPROTECT, FAIL,
                "unable to release object header");
  if (atable.attrs && H5A__attr_release_table(&atable) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CANTFREE, FAIL,
                "unable to release attribute table");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__attr_iterate(hid_t loc_id, H5_index_t idx_type,
                         H5_iter_order_t order, hsize_t skip,
                         hsize_t *last_attr, const H5A_attr_iter_op_t *attr_op,
                         void *op_data) {
  ...
  /* Iterate over attributes to locate correct one */
  if ((ret_value = H5O_attr_iterate_real(loc_id, loc.oloc, idx_type, order,
                                         skip, last_attr, attr_op, op_data)) <
      0)
    HERROR(H5E_ATTR, H5E_BADITER, "error iterating over attributes");
...
static herr_t H5O__attr_remove_update(const H5O_loc_t *loc, H5O_t *oh,
  ...
  if (H5_addr_defined(ainfo->fheap_addr) && ainfo->nattrs < oh->min_dense) {
    hbool_t can_convert =
        TRUE; /* Whether converting to attribute messages is possible */
    size_t u; /* Local index */

    /* Build the table of attributes for this object */
    if (H5A__dense_build_table(loc->file, ainfo, H5_INDEX_NAME, H5_ITER_NATIVE,
                               &atable) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL,
                  "error building attribute table");
    ...
    for (u = 0; u < ainfo->nattrs; u++)
      if (H5O_msg_size_oh(loc->file, oh, H5O_ATTR_ID, (atable.attrs[u]),
                          (size_t)0) >= H5O_MESG_MAX_SIZE) {
        can_convert = FALSE;
        break;
      } /* end if */
    ...
    if (can_convert) {
      ...
      for (u = 0; u < ainfo->nattrs; u++) {
        htri_t shared_mesg; /* Should this message be stored in the Shared
                               Message table? */

        /* Check if attribute is shared */
        if ((shared_mesg = H5O_msg_is_shared(H5O_ATTR_ID, (atable.attrs[u]))) <
            0)
          HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                      "error determining if message is shared");
        else if (shared_mesg == 0) {
          /* Increment reference count on attribute components */
          /* (so that they aren't deleted when the dense attribute storage is
           * deleted) */
        ...
        /* Find out whether the attribute has been opened */
        if ((found_open_attr = H5O__attr_find_opened_attr(
                 loc, &exist_attr, (atable.attrs[u])->shared->name)) < 0)
          HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                      "failed in finding opened attribute");

        /* If found the attribute is already opened, use the opened message to
           insert. If not, still use the message in the attribute table. */
        if (found_open_attr && exist_attr) {
          if (H5O__msg_append_real(loc->file, oh, H5O_MSG_ATTR, 0, 0,
                                   exist_attr) < 0)
            HGOTO_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't create message");

        } /* end if */
        else if (H5O__msg_append_real(loc->file, oh, H5O_MSG_ATTR, 0, 0,
                                      (atable.attrs[u])) < 0)
          HGOTO_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't create message");
      } /* end for */
      ...
      if (H5A__dense_delete(loc->file, ainfo) < 0)
        HGOTO_ERROR(H5E_ATTR, H5E_CANTDELETE, FAIL,
                    "unable to delete dense attribute storage");
    } /* end if */
  }   /* end if */
   ...
   */
  if (H5O__msg_write_real(loc->file, oh, H5O_MSG_AINFO, H5O_MSG_FLAG_DONTSHARE,
                          0, ainfo) < 0)
    HGOTO_ERROR(H5E_ATTR, H5E_CANTUPDATE, FAIL,
                "unable to update attribute info message");
   ...
   */
  if (ainfo->nattrs == 0) {
    if (H5O__msg_remove_real(loc->file, oh, H5O_MSG_AINFO, H5O_ALL, NULL, NULL,
                             TRUE) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTDELETE, FAIL,
                  "unable to delete attribute info");
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_remove_cb(H5O_t *oh, H5O_mesg_t *mesg /*in,out*/,
                                  unsigned H5_ATTR_UNUSED sequence,
                                  unsigned *oh_modified,
                                  void *_udata /*in,out*/) {
...
herr_t H5O__attr_remove(const H5O_loc_t *loc, const char *name) {
  ...
  H5O_ainfo_t ainfo; /* Attribute information for object */
  htri_t ainfo_exists =
      FALSE; /* Whether the attribute info exists in the file */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5_addr_defined(ainfo.fheap_addr)) {
    ...
    if (H5A__dense_remove(loc->file, &ainfo, name) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTDELETE, FAIL,
                  "unable to delete attribute in dense storage");
  } /* end if */
  else {
  ...
  if (ainfo_exists)
    if (H5O__attr_remove_update(loc, oh, &ainfo) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_CANTUPDATE, FAIL,
                  "unable to update attribute info");
 ...
 *-------------------------------------------------------------------------
 */
herr_t H5O__attr_remove_by_idx(const H5O_loc_t *loc, H5_index_t idx_type,
                               H5_iter_order_t order, hsize_t n) {
  H5O_t *oh = NULL;  /* Pointer to actual object header */
  H5O_ainfo_t ainfo; /* Attribute information for object */
  htri_t ainfo_exists =
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__attr_exists_cb(H5O_t H5_ATTR_UNUSED *oh,
                                  H5O_mesg_t *mesg /*in,out*/,
                                  unsigned H5_ATTR_UNUSED sequence,
                                  unsigned H5_ATTR_UNUSED *oh_modified,
                                  void *_udata /*in,out*/) {
 ...
 *
 * Purpose:     Determine if an attribute with a particular name exists on an
 *object
 *
 * Return:      SUCCEED/FAIL
 *
 *-------------------------------------------------------------------------
 */
herr_t H5O__attr_exists(const H5O_loc_t *loc, const char *name,
                        hbool_t *attr_exists) {
  H5O_t *oh = NULL;           /* Pointer to actual object header */
  H5O_ainfo_t ainfo;          /* Attribute information for object */
  herr_t ret_value = SUCCEED; /* Return value */
  ...
  if (H5_addr_defined(ainfo.fheap_addr)) {
    ...
    if (H5A__dense_exists(loc->file, &ainfo, name, attr_exists) < 0)
      HGOTO_ERROR(H5E_ATTR, H5E_BADITER, FAIL,
                  "error checking for existence of attribute");
  } /* end if */
  else {
 ...
 *
 * Purpose:     For 1.8 attribute, returns storage amount for btree and fractal
 *heap
 *
 * Return:      SUCCEED/FAIL
 *
...
herr_t H5O__attr_bh_info(H5F_t *f, H5O_t *oh, H5_ih_info_t *bh_info) {
  ...
  /* Attributes are only stored in fractal heap & indexed w/v2 B-tree in later
   * versions */
  if (oh->version > H5O_VERSION_1) {
    H5O_ainfo_t ainfo; /* Attribute information for object */
    htri_t ainfo_exists =
  ...
  if (bt2_name && H5B2_close(bt2_name) < 0)
    HDONE_ERROR(H5E_ATTR, H5E_CANTCLOSEOBJ, FAIL,
