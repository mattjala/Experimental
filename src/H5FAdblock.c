...
H5FA_dblock_t *H5FA__dblock_alloc(H5FA_hdr_t *hdr) {
  ...
  if (hdr->cparam.nelmts > dblock->dblk_page_nelmts) {
    ...
    hsize_t npages = ((hdr->cparam.nelmts + dblock->dblk_page_nelmts) - 1) /
    ...
    if (NULL == (dblock->dblk_page_init = H5FL_BLK_CALLOC(
                     fa_page_init, dblock->dblk_page_init_size)))
      HGOTO_ERROR(H5E_FARRAY, H5E_CANTALLOC, NULL,
                  "memory allocation failed for page init bitmask");

    /* Compute data block page size */
    dblock->dblk_page_size =
        (dblock->dblk_page_nelmts * hdr->cparam.raw_elmt_size) +
        H5FA_SIZEOF_CHKSUM;
    ...
    if (0 == hdr->cparam.nelmts % dblock->dblk_page_nelmts)
      ...
      dblock->last_page_nelmts =
          (size_t)(hdr->cparam.nelmts % dblock->dblk_page_nelmts);
  } /* end if */
  else {
    ...
    if (NULL ==
        (dblock->elmts = H5FL_BLK_MALLOC(chunk_elmts, (size_t)dblk_size)))
      HGOTO_ERROR(H5E_FARRAY, H5E_CANTALLOC, NULL,
...
haddr_t H5FA__dblock_create(H5FA_hdr_t *hdr, hbool_t *hdr_dirty) {
  ...
  if (HADDR_UNDEF == (dblock_addr = H5MF_alloc(hdr->f, H5FD_MEM_FARRAY_DBLOCK,
                                               (hsize_t)dblock->size)))
    HGOTO_ERROR(H5E_FARRAY, H5E_CANTALLOC, HADDR_UNDEF,
  ...
  if (!H5_addr_defined(ret_value))
    if (dblock) {
      ...
      if (H5_addr_defined(dblock->addr) &&
          H5MF_xfree(hdr->f, H5FD_MEM_FARRAY_DBLOCK, dblock->addr,
 ...
 *-------------------------------------------------------------------------
 */
H5FA_dblock_t *H5FA__dblock_protect(H5FA_hdr_t *hdr, haddr_t dblk_addr,
                                    unsigned flags) {
  H5FA_dblock_t *dblock = NULL; /* Fixed array data block */
  ...
  if (NULL == (dblock = (H5FA_dblock_t *)H5AC_protect(
                   hdr->f, H5AC_FARRAY_DBLOCK, dblk_addr, &udata, flags)))
  ...
  if (!ret_value)
    ...
    if (dblock && H5AC_unprotect(hdr->f, H5AC_FARRAY_DBLOCK, dblock->addr,
...
herr_t H5FA__dblock_unprotect(H5FA_dblock_t *dblock, unsigned cache_flags) {
  ...
  if (H5AC_unprotect(dblock->hdr->f, H5AC_FARRAY_DBLOCK, dblock->addr, dblock,
                     cache_flags) < 0)
...
herr_t H5FA__dblock_delete(H5FA_hdr_t *hdr, haddr_t dblk_addr) {
  ...
  /* Protect data block */
  if (NULL ==
      (dblock = H5FA__dblock_protect(hdr, dblk_addr, H5AC__NO_FLAGS_SET)))
  ...
  if (dblock->npages) {
    ...
    for (u = 0; u < dblock->npages; u++) {
      ...
      if (H5AC_expunge_entry(hdr->f, H5AC_FARRAY_DBLK_PAGE, dblk_page_addr,
                             H5AC__NO_FLAGS_SET) < 0)
        HGOTO_ERROR(
            H5E_FARRAY, H5E_CANTEXPUNGE, FAIL,
  ...
  if (dblock &&
      H5FA__dblock_unprotect(dblock, H5AC__DIRTIED_FLAG | H5AC__DELETED_FLAG |
  ...
  if (dblock->hdr) {
    ...
    if (dblock->npages) {
      ...
      if (dblock->dblk_page_init)
        dblock->dblk_page_init =
            H5FL_BLK_FREE(fa_page_init, dblock->dblk_page_init);
    } /* end if */
