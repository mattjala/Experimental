 ...
 *-------------------------------------------------------------------------
 */
static htri_t H5Z__can_apply_szip(hid_t H5_ATTR_UNUSED dcpl_id, hid_t type_id,
  ...
  if (dtype_order != H5T_ORDER_LE && dtype_order != H5T_ORDER_BE)
    HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FALSE,
                "invalid datatype endianness order");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5Z__set_local_szip(hid_t dcpl_id, hid_t type_id,
                                  hid_t space_id) {
  ...
  if (H5P_get_filter_by_id(dcpl_plist, H5Z_FILTER_SZIP, &flags, &cd_nelmts,
  ...
  if (scanline < cd_values[H5Z_SZIP_PARM_PPB]) {
    ...
    if (npoints < cd_values[H5Z_SZIP_PARM_PPB])
      HGOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                  "pixels per block greater than total number of elements in "
                  "the chunk");
    scanline = (hsize_t)MIN(
        (cd_values[H5Z_SZIP_PARM_PPB] * SZ_MAX_BLOCKS_PER_SCANLINE), npoints);
  } else {
    if (scanline <= SZ_MAX_PIXELS_PER_SCANLINE)
      scanline =
          MIN((cd_values[H5Z_SZIP_PARM_PPB] * SZ_MAX_BLOCKS_PER_SCANLINE),
              scanline);
    else
  ...
  cd_values[H5Z_SZIP_PARM_MASK] &=
      ~((unsigned)SZ_LSB_OPTION_MASK | (unsigned)SZ_MSB_OPTION_MASK);
  switch (dtype_order) {
  ...
  if (H5P_modify_filter(dcpl_plist, H5Z_FILTER_SZIP, flags,
                        H5Z_SZIP_TOTAL_NPARMS, cd_values) < 0)
    HGOTO_ERROR(H5E_PLINE, H5E_CANTSET, FAIL,
                "can't set local szip parameters");
...
static size_t H5Z__filter_szip(unsigned flags, size_t cd_nelmts,
  ...
  H5_CHECKED_ASSIGN(sz_param.options_mask, int, cd_values[H5Z_SZIP_PARM_MASK],
                    unsigned);
  H5_CHECKED_ASSIGN(sz_param.bits_per_pixel, int, cd_values[H5Z_SZIP_PARM_BPP],
                    unsigned);
  H5_CHECKED_ASSIGN(sz_param.pixels_per_block, int,
                    cd_values[H5Z_SZIP_PARM_PPB], unsigned);
  H5_CHECKED_ASSIGN(sz_param.pixels_per_scanline, int,
                    cd_values[H5Z_SZIP_PARM_PPS], unsigned);

  /* Input; uncompress */
  if (flags & H5Z_FLAG_REVERSE) {
    uint32_t stored_nalloc; /* Number of bytes the compressed block will expand
                               into */
    ...
    if (NULL == (outbuf = (unsigned char *)H5MM_malloc(nalloc)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, 0,
                  "memory allocation failed for szip decompression");

    /* Decompress the buffer */
    size_out = nalloc;
    if (SZ_BufftoBuffDecompress(outbuf, &size_out, newbuf, nbytes - 4,
                                &sz_param) != SZ_OK)
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, 0,
                  "szip_filter: decompression failed");
  ...
  /* Output; compress */
    ...
    /* Allocate space for the compressed buffer & header (assume data won't get
     * bigger) */
    if (NULL == (dst = outbuf = (unsigned char *)H5MM_malloc(nbytes + 4)))
      HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, 0,
                  "unable to allocate szip destination buffer");
