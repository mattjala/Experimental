...
static void *H5O__linfo_decode(H5F_t *f, H5O_t *open_oh, unsigned mesg_flags,
                               ...
                               const uint8_t *p);
static herr_t H5O__linfo_encode(H5F_t *f, hbool_t disable_shared, uint8_t *p,
                                const void *_mesg);
static void *H5O__linfo_copy(const void *_mesg, void *_dest);
static size_t H5O__linfo_size(const H5F_t *f, hbool_t disable_shared,
                              const void *_mesg);
 ...
 *-------------------------------------------------------------------------
 */
static void *H5O__linfo_decode(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh,
  ...
  /* Set the number of links in the group to an invalid value, so we query it
   * later */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__linfo_encode(H5F_t *f, hbool_t H5_ATTR_UNUSED disable_shared,
  ...
  *p++ = H5O_LINFO_VERSION;
  ...
  index_flags = (uint8_t)(index_flags |
                          (linfo->index_corder ? H5O_LINFO_INDEX_CORDER : 0));
  *p++ = index_flags;
 ...
 *-------------------------------------------------------------------------
 */
static size_t H5O__linfo_size(const H5F_t *f,
  ...
  ret_value = 1   /* Version */
              + 1 /* Index flags */
              + (linfo->track_corder ? (size_t)8
                                     : 0) /* Curr. max. creation order value */
              + (size_t)H5F_SIZEOF_ADDR(
                    f) /* Address of fractal heap to store "dense" links */
              + (size_t)H5F_SIZEOF_ADDR(
                    f) /* Address of v2 B-tree for indexing names of links */
              + (linfo->index_corder ? (size_t)H5F_SIZEOF_ADDR(f)
                                     : 0); /* Address of v2 B-tree for indexing
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__linfo_delete(H5F_t *f, H5O_t H5_ATTR_UNUSED *open_oh,
                                void *_mesg) {
  ...
  if (H5_addr_defined(linfo->fheap_addr))
    if (H5G__dense_delete(f, linfo, TRUE) < 0)
      HGOTO_ERROR(H5E_OHDR, H5E_CANTFREE, FAIL,
                  "unable to free dense link storage");
...
static void *H5O__linfo_copy_file(H5F_t H5_ATTR_UNUSED *file_src,
  ...
  if (cpy_info->max_depth >= 0 && cpy_info->curr_depth >= cpy_info->max_depth) {
     ...
     */
    /* (XXX: should probably get the "creation" parameters for the source
     * group's dense link storage components and use those - QAK)
     */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__linfo_post_copy_file_cb(const H5O_link_t *src_lnk,
                                           void *_udata) {
  H5O_linfo_postcopy_ud_t *udata =
      (H5O_linfo_postcopy_ud_t *)_udata; /* 'User data' passed in */
  ...
  if (H5L__link_copy_file(udata->dst_oloc->file, src_lnk, udata->src_oloc,
                          &dst_lnk, udata->cpy_info) < 0)
...
static herr_t H5O__linfo_post_copy_file(const H5O_loc_t *src_oloc,
  ...
  if (H5_addr_defined(linfo_src->fheap_addr)) {
     ...
     * messages */
    if (H5G__dense_iterate(src_oloc->file, linfo_src, H5_INDEX_NAME,
                           H5_ITER_NATIVE, (hsize_t)0, NULL,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5O__linfo_debug(H5F_t H5_ATTR_UNUSED *f, const void *_mesg,
  ...
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "'Dense' link storage name index v2 B-tree address:",
          linfo->name_bt2_addr);
  fprintf(stream, "%*s%-*s %" PRIuHADDR "\n", indent, "", fwidth,
          "'Dense' link storage creation order index v2 B-tree address:",
          linfo->corder_bt2_addr);
