...
static void h5tools_print_virtual_selection(
    hid_t vspace, FILE *stream, const h5tool_format_t *info,
 ...
 *-------------------------------------------------------------------------
 */
int h5tools_dump_simple_data(FILE *stream, const h5tool_format_t *info,
                             h5tools_context_t *ctx, /* in,out */
                             hid_t container, unsigned flags, hsize_t nelmts,
  ...
  if (bin_output && (rawdatastream != NULL)) {
    ...
    for (i = 0; i < nelmts; i++, ctx->cur_elmt++, elmt_counter++) {
      ...
      dimension_break = h5tools_render_element(
          stream, info, ctx, &buffer, &curr_pos, ncols, i, elmt_counter);
...
hbool_t h5tools_dump_region_attribute(
  ...
  h5tool_format_t
      outputformat; /* Use to disable prefix for DATA attribute display */
  ...
  dimension_break =
      h5tools_render_element(stream, &outputformat, ctx, buffer, curr_pos,
  ...
  dimension_break =
      h5tools_render_element(stream, info, ctx, buffer, curr_pos, ncols,
...
static int h5tools_print_region_data_blocks(
  ...
  for (blkndx = 0; blkndx < nblocks; blkndx++) {
    ...
    if (H5Sselect_hyperslab(sid1, H5S_SELECT_SET, start, NULL, count, NULL) >=
        0) {
      if (H5Dread(region_id, type_id, mem_space, sid1, H5P_DEFAULT,
                  region_buf) >= 0) {
        ctx.indent_level++;
        if (H5Sget_simple_extent_dims(mem_space, total_size, NULL) >= 0) {
          ...
          elmtno = 0;
          for (numindex = 0; numindex < numelem;
               numindex++, elmtno++, ctx.cur_elmt++) {
            /* Render the region data element begin */
            h5tools_str_reset(buffer);

            h5tools_str_append(buffer, "%s",
                               numindex ? OPTIONAL_LINE_BREAK "" : "");
            ...
            dimension_break = h5tools_render_region_element(
                stream, info, &ctx, buffer, &curr_pos, ncols, ptdata, numindex,
                elmtno);
            /* Render the region data element end */

            if (FALSE == dimension_break)
              elmtno = 0;
          } /* end for (numindex = 0; numindex < numelem; numindex++, elmtno++,
               ctx.cur_elmt++) */
        } else {
 ...
 *-------------------------------------------------------------------------
 */
hbool_t h5tools_dump_region_data_blocks(
    hid_t region_space, hid_t region_id, FILE *stream,
  ...
  if (H5Sget_select_hyper_blocklist(region_space, (hsize_t)0, nblocks, ptdata) <
      0)
  ...
  if (region_output) {
    ...
    h5tools_print_region_data_blocks(region_id, rawdatastream, info, ctx,
                                     buffer, ncols, ndims, type_id, nblocks,
                                     ptdata);
  }
...
hbool_t h5tools_dump_region_data_points(
  ...
  if (H5Sget_select_elem_pointlist(region_space, (hsize_t)0, npoints, ptdata) <
      0)
  ...
  if (region_output) {
    ...
    h5tools_print_region_data_points(region_space, region_id, rawdatastream,
                                     info, ctx, buffer, ncols, ndims, type_id,
                                     npoints, ptdata);
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t h5tools_print_simple_subset(
    ...
    unsigned int row_dim)      /* index of row_counter dimension */
{
  ...
  /* display loop */
  for (; hyperslab_count > 0;
       temp_start[row_dim] += temp_stride[row_dim], hyperslab_count--) {
    ...
    if (H5Sselect_hyperslab(f_space, H5S_SELECT_SET, temp_start, temp_stride,
                            temp_count, temp_block) < 0)
    ...
    if (sm_nelmts > 0) {
      ...
      if (NULL ==
          (sm_buf = (unsigned char *)malloc((size_t)sm_nelmts * p_type_nbytes)))
      ...
      /* initialize the current stripmine position; this is necessary to print
       * the array indices */
      ctx->sm_pos = 0;
      ...
      if (h5tools_dump_simple_data(stream, info, ctx, dset, flags, sm_nelmts,
                                   p_type, sm_buf) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t h5tools_display_simple_subset(FILE *stream,
                                            const h5tool_format_t *info,
                                            h5tools_context_t *ctx, hid_t dset,
                                            hid_t p_type, hid_t f_space,
                                            hsize_t *total_size) {
  size_t i;              /* counters  */
  hsize_t n;             /* counters  */
  ...
  hsize_t
      temp_start[H5S_MAX_RANK]; /* temporary start inside offset count loop */
  hsize_t max_start[H5S_MAX_RANK]; /* maximum start inside offset count loop */
  hsize_t
      temp_count[H5S_MAX_RANK]; /* temporary count inside offset count loop  */
  ...
  if (ctx->ndims > 2) {
    for (i = 0; i < (size_t)ctx->ndims - 2; i++) {
      max_start[i] =
          temp_start[i] + ctx->sset->count.data[i] * ctx->sset->stride.data[i];
      temp_count[i] = 1;
    }
  }
  ...
  for (n = 0; n < outer_count; n++) {
    /* number of read iterations in inner loop, read by rows, to match 2D
     * display */
    if (ctx->ndims > 1) {
    ...
    h5tools_print_simple_subset(stream, info, ctx, dset, p_type, f_space, count,
                                temp_start, temp_count, temp_block, temp_stride,
                                total_size, row_dim);
    ...
    if (ctx->ndims > 2) {
      ...
      do {
        ...
        temp_start[current_outer_dim] +=
            ctx->sset->stride.data[current_outer_dim];
        if (temp_start[current_outer_dim] >= max_start[current_outer_dim]) {
          temp_start[current_outer_dim] =
              ctx->sset->start.data[current_outer_dim];
 ...
 * slower dimensions above the first 2 are incremented one at a time in the
 *outer loop
 *
 * The element position is obtained from the matrix according to:
 *       Given an index I(z,y,x) its position from the beginning of an array
...
static herr_t h5tools_dump_simple_subset(FILE *stream,
  ...
  h5tools_display_simple_subset(stream, info, ctx, dset, p_type, f_space,
                                total_size);
  ...
  if (NULL != (sm_buf = (unsigned char *)malloc((size_t)sm_nbytes))) {
    ...
    for (elmtno = 0; elmtno < p_nelmts; elmtno += hs_nelmts) {
      ...
      if (H5Dread(dset, p_type, sm_space, f_space, H5P_DEFAULT, sm_buf) >= 0) {
        ...
        if (h5tools_dump_simple_data(stream, info, ctx, dset, flags, hs_nelmts,
                                     p_type, sm_buf) < 0)
...
static int h5tools_dump_simple_mem(FILE *stream, const h5tool_format_t *info,
                                   h5tools_context_t *ctx, hid_t attr_id,
                                   hid_t p_type) {
  hid_t f_space = H5I_INVALID_HID; /* file data space */
  hsize_t alloc_size;
  int sndims;                       /* rank of dataspace */
  ...
  if (NULL != (buf = (unsigned char *)malloc((size_t)alloc_size))) {
    ...
    if (H5Aread(attr_id, p_type, buf) >= 0) {
      if (h5tools_dump_simple_data(stream, info, ctx, attr_id,
                                   START_OF_DATA | END_OF_DATA, p_nelmts,
                                   p_type, buf) < 0)
        H5TOOLS_ERROR((-1), "h5tools_dump_simple_data failed");
 ...
 *-------------------------------------------------------------------------
 */
int h5tools_dump_dset(FILE *stream, const h5tool_format_t *info,
                      h5tools_context_t *ctx, hid_t dset) {
  ...
  if (space_type == H5S_SIMPLE || space_type == H5S_SCALAR) {
    if (!ctx->sset)
      ret_value =
          h5tools_dump_simple_dset(rawdatastream, info, ctx, dset, p_type);
    else
      ret_value =
          h5tools_dump_simple_subset(rawdatastream, info, ctx, dset, p_type);
  } else {
    /* space is H5S_NULL */
    ret_value = SUCCEED;
  }
 ...
 *-------------------------------------------------------------------------
 */
int h5tools_dump_mem(FILE *stream, const h5tool_format_t *info,
                     h5tools_context_t *ctx, hid_t attr_id) {
  hid_t f_space = H5I_INVALID_HID;
  hid_t p_type = H5I_INVALID_HID;
  hid_t f_type = H5I_INVALID_HID;
  ...
  if (H5Sis_simple(f_space) <= 0) {
    ...
    ret_value =
        h5tools_dump_simple_mem(rawattrstream, info, ctx, attr_id, p_type);
  }
done:
  if (f_type > 0)
    H5Tclose(f_type);
 ...
 *-------------------------------------------------------------------------
 */
int h5tools_print_datatype(FILE *stream, h5tools_str_t *buffer,
  ...
  case H5T_INTEGER:
    if (H5Tequal(type, H5T_STD_I8BE) == TRUE)
      ...
      h5tools_str_append(buffer, "%zu-bit%s%s integer %zu-bit precision",
                         8 * H5Tget_size(type), order_s, sign_s,
  ...
  case H5T_OPAQUE:
    h5tools_str_append(buffer, "H5T_OPAQUE %s",
    ...
    ctx->indent_level++;
    {
      ...
      h5tools_render_element(stream, info, ctx, buffer, &curr_pos,
...
int h5tools_print_dataspace(h5tools_str_t *buffer, hid_t space) {
  ...
  case H5S_SIMPLE:
    ...
    h5tools_str_append(buffer, "%s %s { %s %" PRIuHSIZE,
                       h5tools_dump_header_format->dataspacedescriptionbegin,
                       S_SIMPLE, h5tools_dump_header_format->dataspacedimbegin,
                       size[0]);
    ...
    h5tools_str_append(buffer, " %s / ",
                       h5tools_dump_header_format->dataspacedimend);
 ...
 *
 *-----------------------------------------------------------------------*/
int h5tools_print_enum(FILE *stream, h5tools_str_t *buffer,
                       const h5tool_format_t *info, h5tools_context_t *ctx,
                       hid_t type) {
  ...
  if (NULL == (value = (unsigned char *)calloc((size_t)nmembs,
                                               MAX(type_size, dst_size))))
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_dump_dataspace(FILE *stream, const h5tool_format_t *info,
                            h5tools_context_t *ctx, hid_t type) {
  h5tools_str_t buffer;           /* string into which to render   */
  size_t ncols = 80;              /* available output width        */
  hsize_t curr_pos = ctx->sm_pos; /* total data element position   */
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_dump_oid(FILE *stream, const h5tool_format_t *info,
...
static void h5tools_print_virtual_selection(
  ...
  case H5S_SEL_POINTS: /* Sequence of points selected  */
    ...
    h5tools_str_append(buffer, " %s",
                       h5tools_dump_header_format->virtualselectionblockend);
    break;
...
void h5tools_print_fill_value(h5tools_str_t *buffer /*in,out*/,
                              const h5tool_format_t *info,
                              h5tools_context_t *ctx /*in,out*/, hid_t dcpl,
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_dump_dcpl(FILE *stream, const h5tool_format_t *info,
                       h5tools_context_t *ctx, hid_t dcpl_id, hid_t type_id,
                       hid_t dset_id) {
  ...
  case H5D_CHUNKED:
    ...
    if (nfilters) {
      ...
      if (ndims && ok) {
        ...
        h5tools_str_append(&buffer, "SIZE %" PRIuHSIZE " (%.3f:1 COMPRESSION)",
                           storage_size, ratio);
      } else
  ...
  case H5D_CONTIGUOUS: {
    ...
    if (n_external) {
       ...
       * the functionality.
       */
      H5VLquery_optional(dset_id, H5VL_SUBCLS_DATASET,
                         H5VL_NATIVE_DATASET_GET_OFFSET, &supported);
  ...
  case H5D_VIRTUAL: {
    ...
    if (n_vmaps) {
      ...
      for (curr_vmap = 0; curr_vmap < n_vmaps; curr_vmap++) {
        ...
        h5tools_str_reset(&buffer);
        h5tools_str_append(&buffer, "%s %zu %s ", VDS_MAPPING, curr_vmap,
                           BEGIN);
        h5tools_render_element(stream, info, ctx, &buffer, &curr_pos,
                               (size_t)ncols, (hsize_t)0, (hsize_t)0);
        ...
        h5tools_print_virtual_selection(virtual_vspace, stream, info, ctx,
                                        &buffer, &curr_pos, (size_t)ncols);
        ...
        h5tools_str_append(&buffer, "%s",
                           h5tools_dump_header_format->virtualfilenameend);
        h5tools_render_element(stream, info, ctx, &buffer, &curr_pos,
                               (size_t)ncols, (hsize_t)0, (hsize_t)0);
        ...
        h5tools_str_append(&buffer, "%s",
                           h5tools_dump_header_format->virtualdatasetnameend);
        h5tools_render_element(stream, info, ctx, &buffer, &curr_pos,
                               (size_t)ncols, (hsize_t)0, (hsize_t)0);

        h5tools_print_virtual_selection(virtual_srcspace, stream, info, ctx,
                                        &buffer, &curr_pos, (size_t)ncols);
    ...
    if (nfilters) {
      for (i = 0; i < nfilters; i++) {
        ...
        case H5Z_FILTER_DEFLATE:
          h5tools_str_append(&buffer, "%s %s %s %d %s", DEFLATE, BEGIN,
                             DEFLATE_LEVEL, cd_values[0], END);
          h5tools_render_element(stream, info, ctx, &buffer, &curr_pos,
                                 (size_t)ncols, (hsize_t)0, (hsize_t)0);
          break;
        ...
        case H5Z_FILTER_SZIP:
          ...
          h5tools_str_reset(&buffer);
          h5tools_str_append(&buffer, "PIXELS_PER_BLOCK %d",
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_dump_comment(FILE *stream, const h5tool_format_t *info,
  ...
  H5VLquery_optional(obj_id, H5VL_SUBCLS_OBJECT, H5VL_NATIVE_OBJECT_GET_COMMENT,
                     &supported);
  ...
  if (cmt_bufsize > 0) {
    comment = (char *)malloc(
        (size_t)(cmt_bufsize + 1)); /* new_size including null terminator */
    if (comment) {
      cmt_bufsize = H5Oget_comment(obj_id, comment, (size_t)cmt_bufsize);
      if (cmt_bufsize > 0) {
        comment[cmt_bufsize] =
            '\0'; /* necessary because null char is not returned */
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_dump_attribute(FILE *stream, const h5tool_format_t *info,
                            h5tools_context_t *ctx, const char *attr_name,
                            hid_t attr_id) {
  h5tools_str_t buffer;           /* string into which to render   */
  size_t ncols = 80;              /* available output width        */
  hsize_t curr_pos = ctx->sm_pos; /* total data element position   */
  ...
  if (HDstrlen(h5tools_dump_header_format->attributeblockend)) {
    h5tools_str_append(&buffer, "%s",
                       h5tools_dump_header_format->attributeblockend);
...
void h5tools_print_packed_bits(h5tools_str_t *buffer, hid_t type) {
  ...
  if ((packed_bits_size > 0) &&
      (packed_data_offset + packed_data_length) > packed_bits_size) {
    error_msg("Packed Bit offset+length value(%u) too large. Max is %d\n",
              packed_data_offset + packed_data_length, packed_bits_size);
    packed_data_mask = 0;
  }
  h5tools_str_append(buffer, "%s %s=%u %s=%u", PACKED_BITS, PACKED_OFFSET,
                     packed_data_offset, PACKED_LENGTH, packed_data_length);
}
...
void h5tools_dump_subsetting_header(FILE *stream, const h5tool_format_t *info,
  ...
  h5tools_str_reset(&buffer);
  h5tools_str_append(&buffer, "%s %s",
                     h5tools_dump_header_format->subsettingbegin,
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_dump_reference(FILE *stream, const h5tool_format_t *info,
                            h5tools_context_t *ctx, hid_t container,
  ...
  for (i = 0; i < ndims; i++, datactx.cur_elmt++, elmt_counter++) {
    ...
    h5tools_str_sprint(&buffer, info, container, H5T_STD_REF, &ref_buf[i],
                       &datactx);
    h5tools_render_element(stream, info, &datactx, &buffer, &curr_pos,
                           (size_t)ncols, (hsize_t)i, (hsize_t)ndims);
    ...
    case H5R_OBJECT1:
      ...
      if (H5Rget_obj_type3(&ref_buf[i], H5P_DEFAULT, &obj_type) >= 0) {
        ...
        case H5O_TYPE_DATASET:
          if ((new_obj_id = H5Ropen_object(&ref_buf[i], H5P_DEFAULT,
                                           H5P_DEFAULT)) >= 0) {
    ...
    case H5R_DATASET_REGION1:
      ...
      if ((new_obj_id =
               H5Ropen_object(&ref_buf[i], H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
        datactx.indent_level++;
        h5tools_dump_data(stream, info, &datactx, new_obj_id, TRUE);
        // h5tools_dump_dset(stream, info, &datactx, new_obj_id);
        datactx.indent_level--;
    ...
    case H5R_DATASET_REGION2:
      ...
      /* if (new_obj_id < 0) - could mean that no reference was written do not
       * throw failure */
      if ((new_obj_id = H5Ropen_object(&ref_buf[i], H5P_DEFAULT, H5P_DEFAULT)) <
          0)
        H5TOOLS_INFO("H5Ropen_object H5R_DATASET_REGION2 failed");
      else {
        if ((new_obj_sid =
                 H5Ropen_region(&ref_buf[i], H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
    ...
    case H5R_ATTR:
      ...
      if ((new_obj_id = H5Ropen_attr(&ref_buf[i], H5P_DEFAULT, H5P_DEFAULT)) >=
          0) {
        h5tools_dump_region_attribute(new_obj_id, stream, info, &datactx,
                                      &buffer, &curr_pos, (size_t)ncols,
                                      (hsize_t)0, (hsize_t)0);
 ...
 * Function:    dump_data
 *
 * Purpose:     Dump attribute, obj_data is FALSE, or dataset data, obj_data is
 *TRUE
 *
 * Return:      void
 *-------------------------------------------------------------------------
 */
void h5tools_dump_data(FILE *stream, const h5tool_format_t *info,
  ...
  if (ctx->sset && obj_data) {
    ...
    h5tools_dump_subsetting_header(stream, &outputformat, ctx,
  ...
  if (H5Tget_class(f_type) == H5T_REFERENCE) {
    ...
    if (!H5Tequal(f_type, H5T_STD_REF) &&
        !H5Tequal(f_type, H5T_STD_REF_DSETREG) &&
    ...
    init_acc_pos(datactx.ndims, total_size, datactx.acc, datactx.pos,
                 datactx.p_min_idx);

    datactx.need_prefix = TRUE;

    if (NULL !=
        (ref_buf = (H5R_ref_t *)calloc(MAX(sizeof(unsigned), sizeof(H5R_ref_t)),
                                       (size_t)ndims))) {
      if (obj_data) {
        if (H5Dread(obj_id, H5T_STD_REF, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    ref_buf) < 0) {
          free(ref_buf);
      ...
      h5tools_dump_reference(stream, &outputformat, &datactx, obj_id, ref_buf,
                             ndims);
      free(ref_buf);
    }
    ctx->indent_level--;
    ...
    datactx = *ctx;
    string_dataformat = *info;
    if ((datactx.display_char && H5Tget_size(f_type) == 1) &&
        (H5Tget_class(f_type) == H5T_INTEGER)) {
      ...
      datactx.indent_level++;
      datactx.need_prefix = TRUE;
      h5tools_simple_prefix(stream, &string_dataformat, &datactx, (hsize_t)0,
                            0);
    ...
    if (datactx.display_char && H5Tget_size(f_type) == 1 &&
        H5Tget_class(f_type) == H5T_INTEGER) {
      H5TOOLS_DEBUG(
          "Print 1-byte integer data as an ASCII character string eol=%s",
          string_dataformat.line_suf);
      ...
      h5tools_str_reset(&buffer);
      h5tools_str_append(&buffer, "\"");
      h5tools_render_element(stream, &string_dataformat, &datactx, &buffer,
  ...
  if (ctx->sset && obj_data) {
    ...
    if (HDstrlen(h5tools_dump_header_format->subsettingblockend)) {
      h5tools_str_append(&buffer, "%s",
                         h5tools_dump_header_format->subsettingblockend);
      if (HDstrlen(h5tools_dump_header_format->subsettingend))
        h5tools_str_append(&buffer, " ");
    }
    if (HDstrlen(h5tools_dump_header_format->subsettingend))
      h5tools_str_append(&buffer, "%s",
                         h5tools_dump_header_format->subsettingend);
    h5tools_render_element(stream, &outputformat, ctx, &buffer, &curr_pos,
                           (size_t)ncols, (hsize_t)0, (hsize_t)0);
  }
