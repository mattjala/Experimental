...
typedef struct h5tool_format_t {
   ...
   *
   *   listv:    h5watch: vector containing info about the list of compound
...
typedef struct h5tools_context_t {
  ...
  hsize_t size_last_dim; /* the size of the last dimension, needed so we can
                            break after each row */
  unsigned
      indent_level; /* the number of times we need some extra indentation */
  unsigned default_indent_level; /* this is used when the indent level gets
                                    changed */
  hsize_t acc[H5S_MAX_RANK];     /* accumulator position */
  hsize_t pos[H5S_MAX_RANK];     /* matrix position */
  hsize_t sm_pos;                /* current stripmine element position */
  const struct H5LD_memb_t *const
      *cmpd_listv; /* h5watch: vector containing info about the list of compound
                      fields to be printed */
...
H5TOOLS_DLLVAR unsigned
    packed_data_offset; /* offset of packed bits to display */
H5TOOLS_DLLVAR unsigned
    packed_data_length; /* length of packed bits to display */
...
H5TOOLS_DLLVAR H5_iter_order_t
    sort_order; /* sort_order [ascending | descending]   */

/* things to display or which are set via command line parameters */
H5TOOLS_DLLVAR int
    enable_error_stack; /* re-enable error stack; disable=0 enable=1 */
...
H5TOOLS_DLL hid_t h5tools_get_fapl(hid_t prev_fapl_id,
                                   h5tools_vol_info_t *vol_info,
                                   h5tools_vfd_info_t *vfd_info);
H5TOOLS_DLL herr_t h5tools_get_vfd_name(hid_t fid, hid_t fapl_id,
                                        char *drivername,
...
H5TOOLS_DLL hbool_t h5tools_is_obj_same(hid_t loc_id1, const char *name1,
                                        hid_t loc_id2, const char *name2);
H5TOOLS_DLL void init_acc_pos(unsigned ndims, const hsize_t *dims, hsize_t *acc,
                              hsize_t *pos, hsize_t *p_min_idx);
H5TOOLS_DLL hsize_t calc_acc_pos(unsigned ndims, hsize_t elemtno,
                                 const hsize_t *acc, hsize_t *pos);
...
H5TOOLS_DLL void h5tools_region_simple_prefix(FILE *stream,
                                              ...
                                              hsize_t elmtno, hsize_t *ptdata,
                                              int secnum);

H5TOOLS_DLL int render_bin_output(FILE *stream, hid_t container, hid_t tid,
                                  void *_mem, hsize_t nelmts);
H5TOOLS_DLL int render_bin_output_region_data_blocks(
    hid_t region_id, FILE *stream, hid_t container, unsigned ndims,
    hid_t type_id, hsize_t nblocks, const hsize_t *ptdata);
H5TOOLS_DLL hbool_t render_bin_output_region_blocks(hid_t region_space,
                                                    hid_t region_id,
                                                    FILE *stream,
                                                    hid_t container);
H5TOOLS_DLL int render_bin_output_region_data_points(
    hid_t region_space, hid_t region_id, FILE *stream, hid_t container,
...
H5TOOLS_DLL hbool_t h5tools_render_element(
...
H5TOOLS_DLL hbool_t h5tools_render_region_element(
    FILE *stream, const h5tool_format_t *info, h5tools_context_t *ctx,
