 ...
 *
 *------------------------------------------------------------------------*/
static void table_attr_mark_exist(const unsigned *exist, char *name,
                                  table_attrs_t *table) {
  if (table->nattrs == table->size) {
    match_attr_t *new_attrs;

    table->size = MAX(1, table->size * 2);
    new_attrs = (match_attr_t *)realloc(table->attrs,
                                        table->size * sizeof(match_attr_t));
    if (new_attrs)
 ...
 *------------------------------------------------------------------------*/
static herr_t build_match_list_attrs(hid_t loc1_id, hid_t loc2_id,
                                     table_attrs_t **table_out,
                                     diff_opt_t *opts) {
  ...
  while (curr1 < oinfo1.num_attrs && curr2 < oinfo2.num_attrs) {
    ...
    if ((attr1_id = H5Aopen_by_idx(loc1_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                                   (hsize_t)curr1, H5P_DEFAULT, H5P_DEFAULT)) <
        0)
    ...
    if ((attr2_id = H5Aopen_by_idx(loc2_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                                   (hsize_t)curr2, H5P_DEFAULT, H5P_DEFAULT)) <
        0)
  ...
  if (opts->mode_verbose_level >= 1)
    parallel_print(
        "Attributes status:  %zu common, %zu only in obj1, %zu only in obj2\n",
...
hsize_t diff_attr_data(hid_t attr1_id, hid_t attr2_id, const char *name1,
                       const char *name2, const char *path1, const char *path2,
                       diff_opt_t *opts) {
  ...
  if (H5Tget_class(ftype1_id) == H5T_REFERENCE) {
    if ((mtype1_id = H5Tcopy(H5T_STD_REF)) < 0)
      H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                         "H5Tcopy(H5T_STD_REF) first attribute ftype failed");
  } else {
    if ((mtype1_id = H5Tget_native_type(ftype1_id, H5T_DIR_DEFAULT)) < 0)
      H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                         "H5Tget_native_type first attribute ftype failed");
  }
  if (H5Tget_class(ftype2_id) == H5T_REFERENCE) {
    if ((mtype2_id = H5Tcopy(H5T_STD_REF)) < 0)
      H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                         "H5Tcopy(H5T_STD_REF) second attribute ftype failed");
  } else {
    if ((mtype2_id = H5Tget_native_type(ftype2_id, H5T_DIR_DEFAULT)) < 0)
      H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                         "H5Tget_native_type second attribute ftype failed");
  }
  ...
  if ((rank1 = H5Sget_simple_extent_dims(space1_id, dims1, NULL)) < 0)
    H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                       "H5Sget_simple_extent_dims first attribute failed");
  if ((rank2 = H5Sget_simple_extent_dims(space2_id, dims2, NULL)) < 0)
    H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                       "H5Sget_simple_extent_dims second attribute failed");
   ...
   * are always same */
  if (diff_can_type(ftype1_id, ftype2_id, rank1, rank2, dims1, dims2, dims1,
                    dims2, opts, 0) == 1) {
     ...
     *------------------------------------------------------------------
     */
    if (FAIL == match_up_memsize(ftype1_id, ftype2_id, &mtype1_id, &mtype2_id,
                                 &msize1, &msize2))
     ...
     *----------------------------------------------------------------------
     */
    H5TOOLS_DEBUG("array compare %s - %s", opts->obj_name[0],
                  opts->obj_name[1]);

    opts->hs_nelmts = opts->nelmts;
    opts->m_tid = mtype1_id;

    /* initialize the current stripmine position; this is necessary to print the
     * array indices */
  ...
  H5TOOLS_DEBUG(
      "check for comparable TYPE and SPACE complete nfound:%d - errstat:%d",
...
hsize_t diff_attr(hid_t loc1_id, hid_t loc2_id, const char *path1,
  ...
  if (build_match_list_attrs(loc1_id, loc2_id, &match_list_attrs, &attr_opts) <
      0) {
    H5TOOLS_GOTO_ERROR(H5DIFF_ERR, "build_match_list_attrs failed");
  }
  H5TOOLS_DEBUG("check match_list_attrs - opts->contents:%d - errstat:%d",
                attr_opts.contents, attr_opts.err_stat);
  ...
  H5TOOLS_DEBUG("match_list_attrs info - opts->contents:%d",
                attr_opts.contents);

  for (u = 0; u < (unsigned)match_list_attrs->nattrs; u++) {
    H5TOOLS_DEBUG("match_list_attrs loop[%d] - errstat:%d", u,
                  attr_opts.err_stat);
    if ((match_list_attrs->attrs[u].exist[0]) &&
        (match_list_attrs->attrs[u].exist[1])) {
      ...
      nfound = diff_attr_data(attr1_id, attr2_id, name1, name2, path1, path2,
                              &attr_opts);
