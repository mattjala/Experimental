 ...
 *-------------------------------------------------------------------------
 */
static void trav_table_add(trav_table_t *table, const char *objname,
                           const H5O_info2_t *oinfo);

static void trav_table_addlink(trav_table_t *table,
                               const H5O_token_t *obj_token, const char *path);
 ...
 *-------------------------------------------------------------------------
 */
void h5trav_set_index(H5_index_t print_index_by,
 ...
 *-------------------------------------------------------------------------
 */
void h5trav_set_verbose(int print_verbose) { trav_verbosity = print_verbose; }
 ...
 *-------------------------------------------------------------------------
 */
static void trav_token_add(trav_addr_t *visited, H5O_token_t *token,
                           const char *path) {
  ...
  if (visited->nused == visited->nalloc) {
    ...
    visited->objs = (trav_addr_path_t *)realloc(
        visited->objs, visited->nalloc * sizeof(trav_addr_path_t));
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t traverse_cb(hid_t loc_id, const char *path,
  ...
  const char *already_visited =
      NULL; /* Whether the link/object was already visited */

  /* Create the full path name for the link */
  if (udata->is_absolute) {
    size_t base_len = HDstrlen(udata->base_grp_name);
    size_t add_slash =
        base_len ? ((udata->base_grp_name)[base_len - 1] != '/') : 1;
    size_t new_name_len =
        base_len + add_slash + HDstrlen(path) + 1 +
  ...
  if (linfo->type == H5L_TYPE_HARD) {
    ...
    if (H5Oget_info_by_name3(loc_id, path, &oinfo, udata->fields, H5P_DEFAULT) <
        0) {
      if (new_name)
        free(new_name);
      return (H5_ITER_ERROR);
    ...
    if (oinfo.rc > 1)
      if (NULL == (already_visited =
                       trav_token_visited(loc_id, udata->seen, &oinfo.token)))
        trav_token_add(udata->seen, &oinfo.token, full_name);

    /* Make 'visit object' callback */
    if (udata->visitor->visit_obj)
      if ((*udata->visitor->visit_obj)(full_name, &oinfo, already_visited,
    ...
    if (udata->visitor->visit_lnk)
      if ((*udata->visitor->visit_lnk)(full_name, linfo,
                                       udata->visitor->udata) < 0) {
        if (new_name)
          free(new_name);
        return (H5_ITER_ERROR);
...
static int traverse(hid_t file_id, const char *grp_name, hbool_t visit_start,
  ...
  if (oinfo.type == H5O_TYPE_GROUP) {
    ...
    if (recurse) {
      ...
      if (H5Lvisit_by_name2(file_id, grp_name, trav_index_by, trav_index_order,
                            traverse_cb, &udata, H5P_DEFAULT) < 0)
        H5TOOLS_ERROR((-1), "H5Lvisit_by_name failed");
    } /* end if */
    else {
      /* Iterate over links in group */
      if (H5Literate_by_name2(file_id, grp_name, trav_index_by,
                              trav_index_order, NULL, traverse_cb, &udata,
                              H5P_DEFAULT) < 0)
 ...
 *-------------------------------------------------------------------------
 */
void trav_info_add(trav_info_t *info, const char *path,
  ...
  if (info) {
    ...
    if (info->nused == info->nalloc) {
      ...
      info->paths = (trav_path_t *)realloc(info->paths,
                                           info->nalloc * sizeof(trav_path_t));
    } /* end if */
...
void trav_fileinfo_add(trav_info_t *info, hid_t loc_id) {
  ...
  if (info->paths[idx].path && HDstrcmp(info->paths[idx].path, ".") != 0)
    H5Oget_info_by_name3(loc_id, info->paths[idx].path, &oinfo, H5O_INFO_BASIC,
                         H5P_DEFAULT);
  else
 ...
 *-------------------------------------------------------------------------
 */
int trav_info_visit_obj(const char *path, const H5O_info2_t *oinfo,
 ...
 *-------------------------------------------------------------------------
 */
int trav_info_visit_lnk(const char *path, const H5L_info2_t *linfo,
                        void *udata) {
  /* Add the link to the 'info' struct */
  trav_info_add(
      (trav_info_t *)udata, path,
 ...
 *-------------------------------------------------------------------------
 */
static int trav_table_visit_obj(const char *path, const H5O_info2_t *oinfo,
 ...
 *-------------------------------------------------------------------------
 */
static int trav_table_visit_lnk(const char *path,
                                const H5L_info2_t H5_ATTR_UNUSED *linfo,
                                void *udata) {
 ...
 *-------------------------------------------------------------------------
 */
static void trav_table_add(trav_table_t *table, const char *path,
                           const H5O_info2_t *oinfo) {
  size_t new_obj;
  ...
  if (table) {
    if (table->nobjs == table->size) {
      ...
      table->objs =
          (trav_obj_t *)realloc(table->objs, table->size * sizeof(trav_obj_t));
    } /* end if */

    new_obj = table->nobjs++;
    if (oinfo)
      memcpy(&table->objs[new_obj].obj_token, &oinfo->token,
             sizeof(H5O_token_t));
    else
      /* Set token to 'undefined' values */
    ...
    table->objs[new_obj].type =
        oinfo ? (h5trav_type_t)oinfo->type : H5TRAV_TYPE_LINK;
...
static void trav_table_addlink(trav_table_t *table,
  ...
  if (table) {
    for (i = 0; i < table->nobjs; i++) {
      if (H5Otoken_cmp(table->fid, &table->objs[i].obj_token, obj_token,
                       &token_cmp) < 0)
        return;
      if (!token_cmp) {
        size_t n;
        ...
        if (table->objs[i].nlinks == (unsigned)table->objs[i].sizelinks) {
          ...
          table->objs[i].links = (trav_link_t *)realloc(
              table->objs[i].links,
              table->objs[i].sizelinks * sizeof(trav_link_t));
        } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
void trav_table_addflags(const unsigned *flags, char *name, h5trav_type_t type,
                         trav_table_t *table) {
  size_t new_obj;
 ...
 *-------------------------------------------------------------------------
 */
static int trav_print_visit_obj(const char *path, const H5O_info2_t *oinfo,
                                const char *already_visited, void *udata) {
  ...
  if (NULL == already_visited) {
    ...
    if (trav_verbosity > 0)
      H5Aiterate_by_name(print_udata->fid, path, trav_index_by,
                         trav_index_order, NULL, trav_attr, &op_data,
                         H5P_DEFAULT);
  } else
 ...
 *-------------------------------------------------------------------------
 */
static int trav_print_visit_lnk(const char *path, const H5L_info2_t *linfo,
                                void *udata) {
  trav_print_udata_t *print_udata = (trav_print_udata_t *)udata;
  ...
  case H5L_TYPE_SOFT:
    if (linfo->u.val_size > 0) {
      ...
      if (targbuf) {
        if (H5Lget_val(print_udata->fid, path, targbuf, linfo->u.val_size + 1,
                       H5P_DEFAULT) < 0)
          targbuf[0] = 0;
        printf(" %-10s %s -> %s\n", "link", path, targbuf);
        free(targbuf);
 ...
 *-------------------------------------------------------------------------
 */
herr_t symlink_visit_add(symlink_trav_t *visited, H5L_type_t type,
  ...
  if (visited->nused == visited->nalloc) {
    ...
    visited->nalloc = MAX(1, visited->nalloc * 2);
    if (NULL ==
        (tmp_ptr = realloc(visited->objs,
                           visited->nalloc * sizeof(symlink_trav_path_t))))
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE hbool_t symlink_is_visited(symlink_trav_t *visited,
                                        H5L_type_t type, const char *file,
                                        const char *path) {
  size_t u; /* Local index variable */
  ...
  for (u = 0; u < visited->nused; u++) {
    ...
    if ((visited->objs[u].type == type) &&
        !HDstrcmp(visited->objs[u].path, path)) {
