...
static hsize_t character_compare(char *mem1, char *mem2, hsize_t elemtno,
                                 size_t u, diff_opt_t *opts);
static hsize_t character_compare_opt(unsigned char *mem1, unsigned char *mem2,
                                     hsize_t elemtno, diff_opt_t *opts);
...
static hsize_t diff_datum(void *_mem1, void *_mem2, hsize_t elemtno,
                          diff_opt_t *opts, hid_t container1_id,
                          hid_t container2_id, mcomp_t *members);
/* element diffs */
static hsize_t diff_float_element(unsigned char *mem1, unsigned char *mem2,
                                  hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_double_element(unsigned char *mem1, unsigned char *mem2,
                                   hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_ldouble_element(unsigned char *mem1, unsigned char *mem2,
                                    hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_schar_element(unsigned char *mem1, unsigned char *mem2,
                                  hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_uchar_element(unsigned char *mem1, unsigned char *mem2,
                                  hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_short_element(unsigned char *mem1, unsigned char *mem2,
                                  hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_ushort_element(unsigned char *mem1, unsigned char *mem2,
                                   hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_int_element(unsigned char *mem1, unsigned char *mem2,
                                hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_uint_element(unsigned char *mem1, unsigned char *mem2,
                                 hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_long_element(unsigned char *mem1, unsigned char *mem2,
                                 hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_ulong_element(unsigned char *mem1, unsigned char *mem2,
                                  hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_llong_element(unsigned char *mem1, unsigned char *mem2,
                                  hsize_t elem_idx, diff_opt_t *opts);
static hsize_t diff_ullong_element(unsigned char *mem1, unsigned char *mem2,
                                   hsize_t elem_idx, diff_opt_t *opts);
...
hsize_t diff_array(void *_mem1, void *_mem2, diff_opt_t *opts,
  ...
  H5TOOLS_START_DEBUG(" - rank:%d hs_nelmts:%" PRIuHSIZE " errstat:%d",
                      opts->rank, opts->hs_nelmts, opts->err_stat);
   ...
   */
  if (type_class != H5T_REFERENCE && type_class != H5T_COMPOUND &&
      type_class != H5T_STRING && type_class != H5T_VLEN &&
  ...
  case H5T_REFERENCE:
    ...
    for (i = 0; i < opts->hs_nelmts; i++) {
      H5TOOLS_DEBUG("opts->pos[%" PRIuHSIZE "]:%" PRIuHSIZE
                    " - nelmts:%" PRIuHSIZE,
                    i, opts->pos[i], opts->hs_nelmts);
      nfound += diff_datum(mem1 + i * size, mem2 + i * size, i, opts,
                           container1_id, container2_id, &members);
      if (opts->count_bool && nfound >= opts->count)
        break;
    } /* i */
...
static hsize_t diff_datum(void *_mem1, void *_mem2, hsize_t elemtno,
  ...
  H5TOOLS_START_DEBUG("ph:%d elemtno:%" PRIuHSIZE " - errstat:%d",
                      opts->print_header, elemtno, opts->err_stat);
  ...
  case H5T_COMPOUND:
    H5TOOLS_DEBUG("H5T_COMPOUND");
    {
      ...
      for (j = 0; j < nmembs; j++) {
        ...
        nfound += diff_datum(mem1 + offset, mem2 + offset, elemtno, &cmpd_opts,
                             container1_id, container2_id, members->m[j]);
      }
      ...
      H5E_BEGIN_TRY {
        ...
        if (err1 < 0 || err2 < 0) {
          ...
          if (HDstrcmp(enum_name1, enum_name2) != 0) {
            ...
            for (u = 0; u < type_size; u++)
              nfound +=
                  character_compare_opt(mem1 + u, mem2 + u, elemtno, opts);
          }
        }
  ...
  case H5T_ARRAY: {
    ...
    arr_opts = *opts;
    H5TOOLS_DEBUG(
        "Check opts: hs_nelmts:%" PRIuHSIZE " to %" PRIuHSIZE " rank:%d to %d",
    ...
    for (u = 0; u < arr_opts.hs_nelmts; u++) {
      nfound += diff_datum(mem1 + u * size, mem2 + u * size, elemtno, &arr_opts,
                           container1_id, container2_id, members);
    }
      ...
      if (H5Tequal(ref_opts.m_tid, H5T_STD_REF)) {
          ...
          if (H5Rget_obj_type3(ref1_buf, H5P_DEFAULT, &obj1_type) >= 0) {
            if (H5Rget_obj_type3(ref2_buf, H5P_DEFAULT, &obj2_type) >= 0) {
              ...
              if (obj1_type == obj2_type) {
                ...
                case H5O_TYPE_DATASET:
                  if ((obj1_id = H5Ropen_object(ref1_buf, H5P_DEFAULT,
                    ...
                    if ((obj2_id = H5Ropen_object(ref2_buf, H5P_DEFAULT,
                                                  H5P_DEFAULT)) >= 0) {
                      nfound =
                          diff_datasetid(obj1_id, obj2_id, opts->obj_name[0],
                ...
                parallel_print(
                    "Different object types referenced: <%s> and <%s>",
                    opts->obj_name[0], opts->obj_name[1]);
                ref_opts.not_cmp = 1;
                ref_opts.err_stat = H5DIFF_ERR;
              }
        ...
        case H5R_DATASET_REGION1:
          ...
          if ((obj1_id = H5Ropen_object(ref1_buf, H5P_DEFAULT, H5P_DEFAULT)) >=
              0) {
            if ((obj2_id =
                     H5Ropen_object(ref2_buf, H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
              if ((region1_id = H5Ropen_region(ref1_buf, H5P_DEFAULT,
                                               H5P_DEFAULT)) >= 0) {
        ...
        case H5R_DATASET_REGION2:
          ...
          /* if (obj_id < 0) - could mean that no reference was written do not
           * throw failure
           */
          if ((obj1_id = H5Ropen_object(ref1_buf, H5P_DEFAULT, H5P_DEFAULT)) <
              0) {
            ...
            if ((obj2_id =
              ...
              H5TOOLS_INFO(
                  "H5Ropen_object H5R_DATASET_REGION2 object 2 failed");
            }
            if (H5Dclose(obj1_id) < 0) {
              ref_opts.err_stat = H5DIFF_ERR;
              H5TOOLS_INFO("H5Dclose H5R_DATASET_REGION2 failed");
        ...
        case H5R_ATTR: {
          ...
          if ((obj1_id = H5Ropen_attr(ref1_buf, H5P_DEFAULT, H5P_DEFAULT)) >=
              0) {
            if ((obj2_id = H5Ropen_attr(ref2_buf, H5P_DEFAULT, H5P_DEFAULT)) >=
                0) {
              /* get name */
        ...
        if (H5Rdestroy(ref1_buf) < 0)
          H5TOOLS_INFO("H5Rdestroy H5R_OBJECT1 failed");
        H5TOOLS_DEBUG("H5T_REFERENCE - H5T_STD_REF complete nfound:%" PRIuHSIZE
                      " - errstat:%d",
  ...
  case H5T_VLEN: {
    ...
    for (j = 0; j < vl_opts.hs_nelmts; j++)
      nfound +=
          diff_datum(((char *)(((hvl_t *)((void *)mem1))->p)) + j * size,
                     ((char *)(((hvl_t *)((void *)mem2))->p)) + j * size,
                     elemtno, /* Extra (void *) cast to quiet "cast to create
     ...
     */
      ...
      if (type_size != sizeof(unsigned long long))
        H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                           "Type size is not unsigned long long size");
...

{
  ...
  if (nblocks1 > 0) {
    ...
    if ((ptdata1 = (hsize_t *)malloc((size_t)alloc_size)) == NULL) {
      ...
      H5Sget_select_hyper_blocklist(region1_id, (hsize_t)0, (hsize_t)nblocks1,
                                    ptdata1);
      ...
      if ((ptdata2 = (hsize_t *)malloc((size_t)alloc_size)) == NULL) {
        ...
        H5Sget_select_hyper_blocklist(region2_id, (hsize_t)0, (hsize_t)nblocks2,
                                      ptdata2);
  ...
  if (npoints1 > 0) {
    ...
    if ((ptdata1 = (hsize_t *)malloc((size_t)alloc_size)) == NULL) {
      ...
      H5Sget_select_elem_pointlist(region1_id, (hsize_t)0, (hsize_t)npoints1,
                                   ptdata1);
      ...
      if ((ptdata2 = (hsize_t *)malloc((size_t)alloc_size)) == NULL) {
        ...
        H5_CHECK_OVERFLOW(npoints1, hssize_t, hsize_t);
        H5Sget_select_elem_pointlist(region2_id, (hsize_t)0, (hsize_t)npoints2,
                                     ptdata2);
...
static hsize_t diff_float_element(unsigned char *mem1, unsigned char *mem2,
  ...
  H5TOOLS_START_DEBUG("delta_bool:%d - percent_bool:%d", opts->delta_bool,
                      opts->percent_bool);
   ...
   */
    ...
    if ((!isnan1 && !isnan2)) {
      ...
      if (not_comparable && !both_zero) {
        ...
        print_pos(opts, elem_idx, 0);
        if (print_data(opts)) {
          parallel_print(F_FORMAT_P_NOTCOMP, (double)temp1_float,
                         (double)temp2_float,
                         (double)ABS(temp1_float - temp2_float));
        }
        nfound++;
...
static hsize_t diff_schar_element(unsigned char *mem1, unsigned char *mem2,
  ...
  if (opts->delta_bool && !opts->percent_bool) {
    if (ABS(temp1_char - temp2_char) > opts->delta) {
      ...
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(I_FORMAT, temp1_char, temp2_char,
  ...
  /* -d and -p */
    ...
    if (not_comparable && !both_zero) {
      ...
      nfound++;
               ...
               PDIFF(temp1_uchar, temp2_uchar) > opts->delta) {
      opts->print_percentage = 1;
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(I_FORMAT_P, temp1_uchar, temp2_uchar,
  ...
  /* -d and -p */
    ...
    if (not_comparable && !both_zero) {
      ...
      nfound++;
               ...
               PDIFF(temp1_ushort, temp2_ushort) > opts->delta) {
      opts->print_percentage = 1;
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(I_FORMAT_P, temp1_ushort, temp2_ushort,
...
static hsize_t diff_int_element(unsigned char *mem1, unsigned char *mem2,
  ...
  if (opts->delta_bool && !opts->percent_bool) {
    if (ABS(temp1_int - temp2_int) > opts->delta) {
      ...
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(I_FORMAT, temp1_int, temp2_int,
...
static hsize_t diff_uint_element(unsigned char *mem1, unsigned char *mem2,
  ...
  if (opts->delta_bool && !opts->percent_bool) {
    if (PDIFF(temp1_uint, temp2_uint) > opts->delta) {
      ...
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(UI_FORMAT, temp1_uint, temp2_uint,
  ...
  /* !-d and -p */
    ...
    if (not_comparable && !both_zero) {
      ...
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(UI_FORMAT_P, temp1_uint, temp2_uint,
                       PDIFF(temp1_uint, temp2_uint), per);
      }
      nfound++;
    }
  }
...
static hsize_t diff_long_element(unsigned char *mem1, unsigned char *mem2,
  ...
  if (opts->delta_bool && !opts->percent_bool) {
    if (ABS(temp1_long - temp2_long) > (long)opts->delta) {
      ...
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(LI_FORMAT, temp1_long, temp2_long,
  ...
  /* -d and -p */
    ...
    if (not_comparable && !both_zero) {
      ...
      nfound++;
               ...
               ABS(temp1_long - temp2_long) > (long)opts->delta) {
      opts->print_percentage = 1;
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(LI_FORMAT_P, temp1_long, temp2_long,
  ...
  /* -d and -p */
    ...
    if (not_comparable && !both_zero) {
      ...
      nfound++;
               ...
               PDIFF(temp1_ulong, temp2_ulong) > (unsigned long)opts->delta) {
      opts->print_percentage = 1;
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(ULI_FORMAT_P, temp1_ulong, temp2_ulong,
    ...
    if (not_comparable && !both_zero) {
      ...
      if (print_data(opts)) {
        parallel_print(LLI_FORMAT_P, temp1_llong, temp2_llong,
                       ABS(temp1_llong - temp2_llong), per);
      }
      nfound++;
    }
  }
  /* -d and -p */
    ...
    if (not_comparable && !both_zero) {
      ...
      nfound++;
               ...
               ABS(temp1_llong - temp2_llong) > (long long)opts->delta) {
      opts->print_percentage = 1;
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(LLI_FORMAT_P, temp1_llong, temp2_llong,
...
static hsize_t diff_ullong_element(unsigned char *mem1, unsigned char *mem2,
  ...
  /* -d and -p */
    ...
    if (not_comparable && !both_zero) {
      ...
      nfound++;
    } else if (per > opts->percent && PDIFF(temp1_ullong, temp2_ullong) >
                                          (unsigned long long)opts->delta) {
      opts->print_percentage = 1;
      print_pos(opts, elem_idx, 0);
      if (print_data(opts)) {
        parallel_print(ULLI_FORMAT_P, temp1_ullong, temp2_ullong,
...
static int ull2float(unsigned long long ull_value, float *f_value) {
  ...
  if ((buf = (unsigned char *)calloc((size_t)1, MAX(src_size, dst_size))) ==
      NULL)
  ...
  /* do conversion */
  if (H5Tconvert(H5T_NATIVE_ULLONG, H5T_NATIVE_FLOAT, (size_t)1, buf, NULL,
                 dxpl_id) < 0)
 ...
 * Function: print_data
 *
 * Purpose:  print data only in report or verbose modes, and do not print in
 *quiet mode
 *-------------------------------------------------------------------------
 */
static int print_data(diff_opt_t *opts) {
  return ((opts->mode_report || opts->mode_verbose) && !opts->mode_quiet) ? 1
                                                                          : 0;
}
...
static void print_header(diff_opt_t *opts) {
  ...
  if (opts->print_percentage) {
    ...
    parallel_print("%-15s %-15s %-15s %-20s\n", "position", opts->obj_name[0],
                   opts->obj_name[1], "difference");
    parallel_print(
        "------------------------------------------------------------\n");
  }
}
...
static void print_pos(diff_opt_t *opts, hsize_t idx, size_t u) {
  ...
  if (print_data(opts)) {
    ...
    if (opts->rank > 0) {
      parallel_print("[ ");
      H5TOOLS_DEBUG("do calc_acc_pos[%" PRIuHSIZE "] nelmts:%" PRIuHSIZE
                    " - errstat:%d",
                    idx, opts->hs_nelmts, opts->err_stat);
      if (opts->sset[0] != NULL) {
        /* Subsetting is used - calculate total position */
        hsize_t curr_idx =
            0; /* current pos in the selection space for each dimension */
        ...
        if (curr_idx < idx) {
          ...
          for (int i = 0; i < opts->rank; i++) {
            ...
            H5TOOLS_DEBUG("curr_idx=%" PRIuHSIZE ", k0=%" PRIuHSIZE
                          ", k1=%" PRIuHSIZE ", curr_pos=%" PRIuHSIZE,
                          curr_idx, k0, k1, curr_pos);
            if (k0 > 0)
              curr_idx = k0 * total_dim_size;
            H5TOOLS_DEBUG("curr_idx=%" PRIuHSIZE ", tmp=%" PRIuHSIZE, curr_idx,
                          tmp);
          ...
          /* check if there is a final calculation needed for slowest changing
           * dimension */
          if (k0 > 0)
            curr_pos += k0 * stride * prev_total_dim_size;
          H5TOOLS_DEBUG("4:curr_idx=%" PRIuHSIZE ", curr_pos=%" PRIuHSIZE,
                        curr_idx, curr_pos);
        }
      }
      /*
      ...
      for (int i = 0; i < opts->rank; i++) {
        H5TOOLS_DEBUG("pos loop:%d with opts->pos=%" PRIuHSIZE
                      " opts->sm_pos=%" PRIuHSIZE,
                      i, opts->pos[i], opts->sm_pos[i]);
        opts->pos[i] += (unsigned long)opts->sm_pos[i];
        H5TOOLS_DEBUG("pos loop:%d with opts->pos=%" PRIuHSIZE, i,
                      opts->pos[i]);
