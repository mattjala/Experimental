...
#endif /* H5DUMP_DEBUG */
static void add_obj(table_t *table, const H5O_token_t *obj_token,
                    const char *objname, hbool_t recorded);
...
void parallel_print(const char *format, ...) {
  ...
  if (!g_Parallel)
    ...
    if (overflow_file == NULL) /*no overflow has occurred yet */ {
      ...
      if ((bytes_written < 0) ||
          ((unsigned)bytes_written >= (OUTBUFF_SIZE - outBuffOffset))) {
        /* Terminate the outbuff at the end of the previous output */
        outBuff[outBuffOffset] = '\0';

        overflow_file = HDtmpfile();
        if (overflow_file == NULL)
          fprintf(rawerrorstream, "warning: could not create overflow file.  "
...
void parse_hsize_list(const char *h_list, subset_d *d) {
  ...
  for (ptr = h_list;
       i < size_count && ptr && *ptr && *ptr != ';' && *ptr != ']'; ptr++)
    if (isdigit(*ptr)) {
...
struct subset_t *parse_subset_params(const char *dset) {
  ...
  /* if dset name is quoted wait till after second quote to look for subset
   * brackets */
  if (*dset == '"')
 ...
 *****************************************************************************
 */
herr_t parse_tuple(const char *start, int sep, char **cpy_out, unsigned *nelems,
                   char ***ptrs_out) {
  char *elem_ptr = NULL;
  char *dest_ptr = NULL;
...
static void dump_table(hid_t fid, char *tablename, table_t *table) {
  ...
  for (u = 0; u < table->nobjs; u++) {
    ...
    PRINTSTREAM(rawoutstream, "%s %s %d %d\n", obj_tok_str,
                table->objs[u].objname, table->objs[u].displayed,
                table->objs[u].recorded);
...
H5_ATTR_PURE obj_t *search_obj(table_t *table, const H5O_token_t *obj_token) {
  ...
  for (u = 0; u < table->nobjs; u++) {
    if (H5Otoken_cmp(table->fid, &table->objs[u].obj_token, obj_token,
                     &token_cmp) < 0)
      return NULL;
    if (!token_cmp)
      return &(table->objs[u]);
 ...
 *
 * Purpose:     Callback to find objects, committed types and store them in
 *tables
 *
 ...
 *-------------------------------------------------------------------------
 */
static herr_t find_objs_cb(const char *name, const H5O_info2_t *oinfo,
                           const char *already_seen, void *op_data) {
...
herr_t init_objs(hid_t fid, find_objs_t *info, table_t **group_table,
  ...
  if ((ret_value = h5trav_visit(fid, "/", TRUE, TRUE, find_objs_cb, NULL, info,
                                H5O_INFO_BASIC)) < 0)
 ...
 */
static void add_obj(table_t *table, const H5O_token_t *obj_token,
                    const char *objname, hbool_t record) {
  size_t u;

  /* See if we need to make table larger */
  if (table->nobjs == table->size) {
    table->size *= 2;
    table->objs = (struct obj_t *)realloc(table->objs,
                                          table->size * sizeof(table->objs[0]));
  } /* end if */
 ...
 *-------------------------------------------------------------------------*/
int H5tools_get_symlink_info(hid_t file_id, const char *linkpath,
  ...
  if ((link_info->trg_path =
           (char *)calloc(link_info->linfo.u.val_size, sizeof(char))) == NULL) {
    if (link_info->opt.msg_mode == 1)
  ...
  if (get_obj_type) {
    ...
    if (H5Oget_info_by_name3(file_id, linkpath, &trg_oinfo, H5O_INFO_BASIC,
                             lapl) < 0) {
      if (link_info->opt.msg_mode == 1)
 ...
 *
 * Description:
 *      These are utility functions to set/get the program name and operation
 *status.
 *-------------------------------------------------------------------------
 */
void h5tools_setprogname(const char *Progname) { h5tools_progname = Progname; }

void h5tools_setstatus(int D_status) { h5tools_d_status = D_status; }

H5_ATTR_PURE const char *h5tools_getprogname(void) { return h5tools_progname; }

H5_ATTR_PURE int h5tools_getstatus(void) { return h5tools_d_status; }
 ...
 *----------------------------------------------------------------------------
 */
herr_t h5tools_parse_ros3_fapl_tuple(const char *tuple_str, int delim,
 ...
 *----------------------------------------------------------------------------
 */
int h5tools_populate_ros3_fapl(H5FD_ros3_fapl_ext_t *fa, const char **values) {
 ...
 *----------------------------------------------------------------------------
 */
herr_t h5tools_parse_hdfs_fapl_tuple(const char *tuple_str, int delim,
  ...
  if (HDstrncmp(props[0], "", 1)) {
    HDstrncpy(fapl_config_out->namenode_name, (const char *)props[0],
              HDstrlen(props[0]));
  }
  ...
  if (HDstrncmp(props[2], "", 1)) {
    HDstrncpy(fapl_config_out->kerberos_ticket_cache, (const char *)props[2],
              HDstrlen(props[2]));
  }
  if (HDstrncmp(props[3], "", 1)) {
    HDstrncpy(fapl_config_out->user_name, (const char *)props[3],
              HDstrlen(props[3]));
  }
