 ...
 *-------------------------------------------------------------------------
 */
hsize_t diff_dataset(hid_t file1_id, hid_t file2_id, const char *obj1_name,
                     const char *obj2_name, diff_opt_t *opts) {
  int status = -1;
  ...
  if ((status = h5tools_canreadf((opts->mode_verbose ? obj1_name : NULL),
                                 dcpl1) == 1) &&
      (status = h5tools_canreadf((opts->mode_verbose ? obj2_name : NULL),
                                 dcpl2) == 1))
 ...
 *-------------------------------------------------------------------------
 */
hsize_t diff_datasetid(hid_t did1, hid_t did2, const char *obj1_name,
                       const char *obj2_name, diff_opt_t *opts) {
  ...
  if (storage_size1 == 0 || storage_size2 == 0) {
    if (stl1 == H5D_VIRTUAL || stl2 == H5D_VIRTUAL) {
      if ((opts->mode_verbose || opts->mode_list_not_cmp) && obj1_name &&
          obj2_name)
   ...
   *-------------------------------------------------------------------------
   */
  if (diff_can_type(f_tid1, f_tid2, rank1, rank2, dims1, dims2, maxdim1,
                    maxdim2, opts, 0) != 1)
    can_compare = 0;
  ...
  if (H5Tget_class(f_tid2) == H5T_REFERENCE) {
    if ((m_tid2 = H5Tcopy(H5T_STD_REF)) < 0)
      H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                         "H5Tcopy(H5T_STD_REF) second ftype failed");
  } else {
  ...
  if (can_compare) {
    ...
    if (sign1 != sign2) {
      ...
      if ((opts->mode_verbose || opts->mode_list_not_cmp) && obj1_name &&
          obj2_name) {
        parallel_print("Not comparable: <%s> has sign %s ", obj1_name,
                       get_sign(sign1));
    ...
    H5TOOLS_DEBUG("can_compare for sign - can_compare=%d opts->not_cmp=%d",
                  can_compare, opts->not_cmp);
  }
  ...
  if (TRUE == h5tools_detect_vlen(m_tid2))
    vl_data2 = TRUE;
  H5TOOLS_DEBUG("h5tools_detect_vlen %d:%d - errstat:%d", vl_data1, vl_data2,
                opts->err_stat);
  ...
  if (can_compare) { /* it is possible to compare */
    ...
    if (tclass != H5T_ARRAY) {
      ...
      if (FAIL == match_up_memsize(f_tid1, f_tid2, &m_tid1, &m_tid2, &m_size1,
                                   &m_size2))
    ...
    init_acc_pos((unsigned)opts->rank, opts->dims, opts->acc, opts->pos,
                 opts->p_min_idx);
    ...
    if (buf1 != NULL && buf2 != NULL && opts->sset[0] == NULL &&
        opts->sset[1] == NULL) {
      ...
      H5TOOLS_DEBUG("diff_array ret nfound:%d - errstat:%d", nfound,
                    opts->err_stat);
      ...
      H5TOOLS_DEBUG(
          "compare by hyperslabs: opts->nelmts=%ld - opts->m_size=%ld",
          opts->nelmts, opts->m_size);
      if (opts->sset[0] != NULL) {
        ...
        if (!opts->sset[0]->start.data || !opts->sset[0]->stride.data ||
          ...
          if (!opts->sset[0]->start.data) {
            ...
            opts->sset[0]->start.data =
                (hsize_t *)calloc((size_t)rank1, sizeof(hsize_t));
          ...
          for (i = 0; i < rank1; i++) {
            if (opts->sset[0]->count.data[i] > 1) {
              if (opts->sset[0]->stride.data[i] <
                  opts->sset[0]->block.data[i]) {
                H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
        ...
        for (i = 0; i < rank1; i++) {
          ...
          hs_size1[i] = 0;
          H5TOOLS_DEBUG("[%d]hs_offset1:%ld, hs_stride1:%ld, hs_count1:%ld, "
                        "hs_block1:%ld",
                        i, hs_offset1[i], hs_stride1[i], hs_count1[i],
                        hs_block1[i]);
        }
      }
      if (opts->sset[1] != NULL) {
        ...
        if (!opts->sset[1]->start.data || !opts->sset[1]->stride.data ||
          ...
          if (!opts->sset[1]->start.data) {
            ...
            opts->sset[1]->start.data =
                (hsize_t *)calloc((size_t)rank2, sizeof(hsize_t));
          ...
          for (i = 0; i < rank2; i++) {
            if (opts->sset[1]->count.data[i] > 1) {
              if (opts->sset[1]->stride.data[i] <
                  opts->sset[1]->block.data[i]) {
                H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
        ...
        for (i = 0; i < rank2; i++) {
          ...
          hs_size2[i] = 0;
          H5TOOLS_DEBUG("[%d]hs_offset2:%ld, hs_stride2:%ld, hs_count2:%ld, "
                        "hs_block2:%ld",
                        i, hs_offset2[i], hs_stride2[i], hs_count2[i],
                        hs_block2[i]);
        }
      }
      ...
      if (opts->rank > 0) {
        for (i = opts->rank; i > 0; --i) {
          ...
          if (size == 0) /* datum size > H5TOOLS_BUFSIZE */
            size = 1;
          H5TOOLS_DEBUG("opts->dims[%d]: %ld - size: %ld", i - 1,
                        opts->dims[i - 1], size);
          if (opts->sset[1] != NULL) {
          ...
          H5TOOLS_DEBUG("sm_size[%d]: %ld - sm_block:%ld", i - 1,
                        sm_size[i - 1], sm_block[i - 1]);
      ...
      for (elmtno = 0; elmtno < opts->nelmts; elmtno += opts->hs_nelmts) {
        ...
        if (NULL == (sm_buf1 = (unsigned char *)malloc((size_t)sm_nbytes)))
          H5TOOLS_GOTO_ERROR(H5DIFF_ERR,
                             "Could not allocate buffer for strip-mine");
        ...
        if (opts->rank > 0) {
          if (opts->sset[0] != NULL) {
            ...
            for (i = 0; i < rank1; i++) {
              H5TOOLS_DEBUG(
                  "[%d]opts->dims: %ld - hs_offset1: %ld - sm_block: %ld", i,
            ...
            if (H5Sselect_hyperslab(sid1, H5S_SELECT_SET, hs_offset1,
                                    hs_stride1, hs_count1, hs_size1) < 0)
            ...
            for (i = 0, hs_nelmts1 = 1; i < rank1; i++) {
              ...
              H5TOOLS_DEBUG("hs_nelmts1:%ld *= hs_size1[%d]: %ld", hs_nelmts1,
                            i, hs_size1[i]);
            }
            if (H5Sselect_hyperslab(sid1, H5S_SELECT_SET, hs_offset1, NULL,
                                    hs_size1, NULL) < 0)
              H5TOOLS_GOTO_ERROR(H5DIFF_ERR, "H5Sselect_hyperslab sid1 failed");
          }
          ...
          if (H5Sselect_hyperslab(sm_space1, H5S_SELECT_SET, zero, NULL,
                                  &sm_nelmts1, NULL) < 0)
          ...
          if (opts->sset[1] != NULL) {
            ...
            for (i = 0; i < rank2; i++) {
              H5TOOLS_DEBUG(
                  "[%d]opts->dims: %ld - hs_offset2: %ld - sm_block: %ld", i,
            ...
            if (H5Sselect_hyperslab(sid2, H5S_SELECT_SET, hs_offset2,
                                    hs_stride2, hs_count2, hs_size2) < 0)
            ...
            for (i = 0, hs_nelmts2 = 1; i < rank2; i++) {
              ...
              H5TOOLS_DEBUG("hs_nelmts2:%ld *= hs_size2[%d]: %ld", hs_nelmts2,
                            i, hs_size2[i]);
            }
          ...
          if (H5Sselect_hyperslab(sm_space2, H5S_SELECT_SET, zero, NULL,
                                  &sm_nelmts2, NULL) < 0)
            H5TOOLS_GOTO_ERROR(H5DIFF_ERR, "H5Sselect_hyperslab failed");
        } else
          hs_nelmts1 = 1;
        ...
        /* print array indices. get the lower bound of the hyperslab and
           calculate the element position at the start of hyperslab */
        ...
        number of differences found in each hyperslab and pass the position at
        the beginning for printing */
        ...
        for (i = opts->rank, carry = 1; i > 0 && carry; --i) {
          if (opts->sset[0] != NULL) {
            H5TOOLS_DEBUG("[%d]hs_size1:%ld - hs_block1:%ld - hs_stride1:%ld",
                          i - 1, hs_size1[i - 1], hs_block1[i - 1],
                          hs_stride1[i - 1]);
            ...
            hs_offset1[i - 1] += hs_size1[i - 1];
          H5TOOLS_DEBUG("[%d]hs_offset1:%ld - opts->dims:%ld", i - 1,
                        hs_offset1[i - 1], opts->dims[i - 1]);
          ...
          if (opts->sset[1] != NULL) {
            H5TOOLS_DEBUG("[%d]hs_size2:%ld - hs_block2:%ld - hs_stride2:%ld",
                          i - 1, hs_size2[i - 1], hs_block2[i - 1],
                          hs_stride2[i - 1]);
            ...
            hs_offset2[i - 1] += hs_size2[i - 1];
          H5TOOLS_DEBUG("[%d]hs_offset2:%ld - opts->dims:%ld", i - 1,
                        hs_offset2[i - 1], opts->dims[i - 1]);
...
int diff_can_type(hid_t f_tid1, hid_t f_tid2, int rank1, int rank2,
                  hsize_t *dims1, hsize_t *dims2, hsize_t *maxdim1,
                  hsize_t *maxdim2, diff_opt_t *opts, int is_compound) {
  H5T_class_t tclass1;
  H5T_class_t tclass2;
  ...
  H5TOOLS_DEBUG("obj_names: %s - %s", opts->obj_name[0], opts->obj_name[1]);
  if (tclass1 != tclass2) {
    if ((opts->mode_verbose || opts->mode_list_not_cmp) && opts->obj_name[0] &&
        opts->obj_name[1]) {
      if (is_compound) {
        parallel_print(
            "Not comparable: <%s> has a class %s and <%s> has a class %s\n",
            opts->obj_name[0], get_class(tclass1), opts->obj_name[1],
            get_class(tclass2));
      } else {
        parallel_print(
            "Not comparable: <%s> is of class %s and <%s> is of class %s\n",
            opts->obj_name[0], get_class(tclass1), opts->obj_name[1],
            get_class(tclass2));
      }
    }
   ...
   *-------------------------------------------------------------------------
   */
  if ((H5Tequal(f_tid1, f_tid2) == 0) && (opts->mode_verbose) &&
  ...
  if (rank1 != rank2) {
    if ((opts->mode_verbose || opts->mode_list_not_cmp) && opts->obj_name[0] &&
        opts->obj_name[1]) {
      parallel_print("Not comparable: <%s> has rank %d, dimensions ",
                     opts->obj_name[0], rank1);
      print_dimensions(rank1, dims1);
      parallel_print(", max dimensions ");
      print_dimensions(rank1, maxdim1);
      parallel_print("\n");
      parallel_print("and <%s> has rank %d, dimensions ", opts->obj_name[1],
                     rank2);
  ...
  if (tclass1 == H5T_COMPOUND) {
    ...
    if (nmembs1 != nmembs2) {
      if ((opts->mode_verbose || opts->mode_list_not_cmp) &&
          opts->obj_name[0] && opts->obj_name[1]) {
        parallel_print("Not comparable: <%s> has %d members ",
                       opts->obj_name[0], nmembs1);
    ...
    for (j = 0; j < nmembs1; j++) {
      ...
      if (diff_can_type(memb_type1, memb_type2, rank1, rank2, dims1, dims2,
                        maxdim1, maxdim2, opts, 1) != 1) {
        opts->not_cmp = 1;
        H5Tclose(memb_type1);
        H5Tclose(memb_type2);
        H5TOOLS_GOTO_DONE(0);
 ...
 *-------------------------------------------------------------------------
 */
void print_sizes(const char *obj1, const char *obj2, hid_t f_tid1, hid_t f_tid2,
