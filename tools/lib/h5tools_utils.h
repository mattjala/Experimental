...
typedef struct obj_t {
  ...
  hbool_t recorded;  /* Flag for named datatypes to indicate they were found in
                        the group hierarchy */
} obj_t;
...
/*extended configuration struct for holding the configuration data to the
 * #H5FD_ROS3 driver */
...
H5TOOLS_DLLVAR unsigned
    h5tools_nCols; /*max number of columns for outputting  */
...
H5TOOLS_DLL herr_t parse_tuple(const char *start, int sep, char **cpy_out,
                               unsigned *nelems, char ***ptrs_out);
...
#endif /* H5DUMP_DEBUG */
H5TOOLS_DLL herr_t init_objs(hid_t fid, find_objs_t *info,
                             table_t **group_table, table_t **dset_table,
                             table_t **type_table);
...
#ifdef H5_HAVE_ROS3_VFD
H5TOOLS_DLL herr_t h5tools_parse_ros3_fapl_tuple(
    const char *tuple_str, int delim, H5FD_ros3_fapl_ext_t *fapl_config_out);
H5TOOLS_DLL int h5tools_populate_ros3_fapl(H5FD_ros3_fapl_ext_t *fa,
                                           const char **values);
#endif /* H5_HAVE_ROS3_VFD */
#ifdef H5_HAVE_LIBHDFS
H5TOOLS_DLL herr_t h5tools_parse_hdfs_fapl_tuple(
    const char *tuple_str, int delim, H5FD_hdfs_fapl_t *fapl_config_out);
#endif
