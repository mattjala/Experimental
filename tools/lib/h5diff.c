 ...
 *
 * Purpose:  special function that prints any output that has been sent to the
 *manager and is currently sitting in the incoming message queue
 *
 * Return:   none
 *-------------------------------------------------------------------------
 */
...
static void print_incoming_data(void) {
  ...
  do {
    MPI_Iprobe(MPI_ANY_SOURCE, MPI_TAG_PRINT_DATA, MPI_COMM_WORLD,
               &incomingMessage, &Status);
    if (incomingMessage) {
      memset(data, 0, PRINT_DATA_MAX_SIZE + 1);
      MPI_Recv(data, PRINT_DATA_MAX_SIZE, MPI_CHAR, Status.MPI_SOURCE,
               MPI_TAG_PRINT_DATA, MPI_COMM_WORLD, &Status);
...
static int is_valid_options(diff_opt_t *opts) {
  ...
  if (opts->no_dangle_links && !opts->follow_links) {
    parallel_print("Error: --no-dangling-links must be used along with "
 ...
 *           0 - not excluded path
 *------------------------------------------------------------------------*/
static int is_exclude_attr(const char *path, h5trav_type_t type,
                           diff_opt_t *opts) {
 ...
 *           Folk, Michael; Zoellick, Bill. (1992). File Structures.
 *Addison-Wesley. Moved out from diff_match() to make code more flexible.
 *
 * Parameter:
 *           table_out [OUT] : return the list
 *------------------------------------------------------------------------*/
static void build_match_list(const char *objname1, trav_info_t *info1,
                             const char *objname2, trav_info_t *info2,
 ...
 *------------------------------------------------------------------------*/
static herr_t trav_grp_objs(const char *path, const H5O_info2_t *oinfo,
 ...
 *------------------------------------------------------------------------*/
static herr_t trav_grp_symlinks(const char *path, const H5L_info2_t *linfo,
                                void *udata) {
  ...
  switch (linfo->type) {
  case H5L_TYPE_SOFT:
    if ((ret_value = H5tools_get_symlink_info(tinfo->fid, path, &lnk_info,
                                              opts->follow_links)) < 0) {
    ...
    if (symlink_is_visited(&(tinfo->symlink_visited), linfo->type, NULL,
                           lnk_info.trg_path))
      H5TOOLS_GOTO_DONE(SUCCEED);

    /* add this link as visited link */
    if (symlink_visit_add(&(tinfo->symlink_visited), linfo->type, NULL,
                          lnk_info.trg_path) < 0)
      H5TOOLS_GOTO_DONE(SUCCEED);

    if (h5trav_visit(tinfo->fid, path, TRUE, TRUE, trav_grp_objs,
                     trav_grp_symlinks, tinfo, H5O_INFO_BASIC) < 0) {
  ...
  case H5L_TYPE_EXTERNAL:
    ...
    if (H5Lunpack_elink_val(lnk_info.trg_path, linfo->u.val_size, NULL,
                            &ext_fname, &ext_path) < 0)
      H5TOOLS_GOTO_DONE(SUCCEED);

    /* check if already visit the target object */
    if (symlink_is_visited(&(tinfo->symlink_visited), linfo->type, ext_fname,
                           ext_path))
      H5TOOLS_GOTO_DONE(SUCCEED);

    /* add this link as visited link */
    if (symlink_visit_add(&(tinfo->symlink_visited), linfo->type, ext_fname,
                          ext_path) < 0)
      H5TOOLS_GOTO_DONE(SUCCEED);
 ...
 *-------------------------------------------------------------------------
 */
hsize_t h5diff(const char *fname1, const char *fname2, const char *objname1,
  ...
  if (opts->vfd_info[0].u.name) {
    if ((fapl1_id = h5tools_get_fapl(H5P_DEFAULT, NULL, &(opts->vfd_info[0]))) <
        0) {
  ...
  if (opts->custom_vol[0] || opts->custom_vfd[0]) {
    if ((fapl1_id = h5tools_get_fapl(
             fapl1_id, opts->custom_vol[0] ? &(opts->vol_info[0]) : NULL,
  ...
  if ((file1_id = h5tools_fopen(fname1, H5F_ACC_RDONLY, fapl1_id,
                                (fapl1_id != H5P_DEFAULT), NULL, (size_t)0)) <
      0) {
  ...
  if (opts->vfd_info[1].u.name) {
    if ((fapl2_id = h5tools_get_fapl(H5P_DEFAULT, NULL, &(opts->vfd_info[1]))) <
        0) {
  ...
  if (opts->custom_vol[1] || opts->custom_vfd[1]) {
    if ((fapl2_id = h5tools_get_fapl(
             fapl2_id, opts->custom_vol[1] ? &(opts->vol_info[1]) : NULL,
  ...
  if ((file2_id = h5tools_fopen(fname2, H5F_ACC_RDONLY, fapl2_id,
                                (fapl2_id != H5P_DEFAULT), NULL, (size_t)0)) <
      0) {
    parallel_print("h5diff: <%s>: unable to open file\n", fname2);
    H5TOOLS_GOTO_ERROR(H5DIFF_ERR, "<%s>: unable to open file\n", fname2);
  }
  ...
  if (objname1) {
     ...
     */
    H5TOOLS_DEBUG(
        "h5diff check if obj1=%s is root, group, single object or symlink",
        obj1fullname);
    if (!HDstrcmp(obj1fullname, "/")) {
      ...
      if (src_linfo1.type == H5L_TYPE_HARD) {
        ...
        if (H5Oget_info_by_name3(file1_id, obj1fullname, &oinfo1,
        ...
        memcpy(&info1_obj->paths[idx].obj_token, &oinfo1.token,
               sizeof(H5O_token_t));
     ...
     */
    H5TOOLS_DEBUG(
        "h5diff check if obj2=%s is root, group, single object or symlink",
        obj2fullname);
    if (!HDstrcmp(obj2fullname, "/")) {
      ...
      if (src_linfo2.type == H5L_TYPE_HARD) {
        ...
        if (H5Oget_info_by_name3(file2_id, obj2fullname, &oinfo2,
                                 H5O_INFO_BASIC, H5P_DEFAULT) < 0) {
          parallel_print("Error: Could not get file contents\n");
          H5TOOLS_GOTO_ERROR(H5DIFF_ERR, "Error: Could not get file contents");
        }
        ...
        memcpy(&info2_obj->paths[idx].obj_token, &oinfo2.token,
               sizeof(H5O_token_t));
  ...
  if (opts->follow_links) {
    ...
    if (l_ret1 == 0) {
      ...
      if (info1_lp != NULL) {
        ...
        memcpy(&info1_lp->paths[idx].obj_token, &trg_linfo1.obj_token,
               sizeof(H5O_token_t));
    ...
    if (l_ret2 == 0) {
      ...
      if (info2_lp != NULL) {
        ...
        memcpy(&info2_lp->paths[idx].obj_token, &trg_linfo2.obj_token,
               sizeof(H5O_token_t));
  ...
  if (!(opts->mode_verbose || opts->mode_report)) {
    ...
    if (l_ret1 > 0 && l_ret2 > 0)
      if (h5tools_is_obj_same(file1_id, obj1fullname, file2_id, obj2fullname) !=
          0)
        H5TOOLS_GOTO_DONE(H5DIFF_NO_ERR);
  }

  both_objs_grp =
      (obj1type == H5TRAV_TYPE_GROUP && obj2type == H5TRAV_TYPE_GROUP);
  if (both_objs_grp) {
    ...
    if (h5trav_visit(file1_id, obj1fullname, TRUE, TRUE, trav_grp_objs,
                     trav_grp_symlinks, info1_grp, H5O_INFO_BASIC) < 0) {
      parallel_print("Error: Could not get file contents\n");
    ...
    if (h5trav_visit(file2_id, obj2fullname, TRUE, TRUE, trav_grp_objs,
                     trav_grp_symlinks, info2_grp, H5O_INFO_BASIC) < 0) {
      parallel_print("Error: Could not get file contents\n");
      H5TOOLS_GOTO_ERROR(H5DIFF_ERR, "Could not get file contents");
    } /* end if */
    info2_lp = info2_grp;
  ...
  if (g_Parallel) {
    ...
    if ((HDstrlen(fname1) > MAX_FILENAME) ||
      ...
      fprintf(
          stderr,
          "The parallel diff only supports path names up to %d characters\n",
          MAX_FILENAME);
    ...
    for (i = 1; i < g_nTasks; i++)
      MPI_Send(filenames, (MAX_FILENAME * 2), MPI_CHAR, i, MPI_TAG_PARALLEL,
               MPI_COMM_WORLD);
  } /* end if */
#endif
  ...
  /* process the objects */
  build_match_list(obj1fullname, info1_lp, obj2fullname, info2_lp, &match_list,
                   opts);
  ...
  nfound = diff_match(file1_id, obj1fullname, info1_lp, file2_id, obj2fullname,
                      info2_lp, match_list, opts);
 ...
 * Function: diff_match
 *
 * Purpose:  Compare common objects in given groups according to table
 *structure. The table structure has flags which can be used to find common
 *objects and will be compared. Common object means same name (absolute path)
 *objects in both location.
 *
 * Return:   Number of differences found
 *
 *-------------------------------------------------------------------------
...
hsize_t diff_match(hid_t file1_id, const char *grp1, trav_info_t *info1,
  ...
  /*-------------------------------------------------------------------------
   * regarding the return value of h5diff (0, no difference in files, 1
   *difference ) 1) the number of objects in file1 must be the same as in file2
...
#ifdef H5_HAVE_PARALLEL
  {
    ...
    for (i = 0; i < table->nobjs; i++) {
      H5TOOLS_DEBUG("diff for common objects[%d] - errstat:%d", i,
                    opts->err_stat);
      if (table->objs[i].flags[0] && table->objs[i].flags[1]) {
        /* make full path for obj1 */
#ifdef H5_HAVE_ASPRINTF
        /* Use the asprintf() routine, since it does what we're trying to do
         * below */
        if (asprintf(&obj1_fullpath, "%s%s", grp1_path, table->objs[i].name) <
            0) {
          H5TOOLS_ERROR(H5DIFF_ERR, "name buffer allocation failed");
        }
#else  /* H5_HAVE_ASPRINTF */
      if ((obj1_fullpath = (char *)malloc(HDstrlen(grp1_path) +
                                          HDstrlen(table->objs[i].name) + 1)) ==
          NULL) {
        H5TOOLS_ERROR(H5DIFF_ERR, "name buffer allocation failed");
      } else {
        ...
        /* Use the asprintf() routine, since it does what we're trying to do
         * below */
        if (asprintf(&obj2_fullpath, "%s%s", grp2_path, table->objs[i].name) <
            0) {
          H5TOOLS_ERROR(H5DIFF_ERR, "name buffer allocation failed");
        }
#else  /* H5_HAVE_ASPRINTF */
      if ((obj2_fullpath = (char *)malloc(HDstrlen(grp2_path) +
                                          HDstrlen(table->objs[i].name) + 1)) ==
          NULL) {
        H5TOOLS_ERROR(H5DIFF_ERR, "name buffer allocation failed");
      } else {
        ...
        if (!g_Parallel) {
          ...
          nfound += diff(file1_id, obj1_fullpath, file2_id, obj2_fullpath, opts,
                         &argdata);
        } /* end if */
#ifdef H5_HAVE_PARALLEL
        else {
          ...
          if (HDstrlen(obj1_fullpath) > 255 || HDstrlen(obj2_fullpath) > 255) {
            printf("The parallel diff only supports object names up to 255 "
                   "characters\n");
            MPI_Abort(MPI_COMM_WORLD, 0);
          } /* end if */
          ...
          if (busyTasks > 0) {
            ...
            MPI_Iprobe(MPI_ANY_SOURCE, MPI_TAG_DONE, MPI_COMM_WORLD,
                       &incomingMessage, &Status);
            ...
            if (incomingMessage) {
              ...
              MPI_Recv(&nFoundbyWorker, sizeof(nFoundbyWorker), MPI_BYTE,
                       Status.MPI_SOURCE, MPI_TAG_DONE, MPI_COMM_WORLD,
                       &Status);
            ...
            if (!havePrintToken) {
              /* If we don't have the token, someone is probably sending us
               * output */
              print_incoming_data();
              ...
              if (incomingMessage) {
                workerTasks[Status.MPI_SOURCE - 1] = 1;
                MPI_Recv(&nFoundbyWorker, sizeof(nFoundbyWorker), MPI_BYTE,
                         Status.MPI_SOURCE, MPI_TAG_TOK_RETURN, MPI_COMM_WORLD,
                         &Status);
                nfound += nFoundbyWorker.nfound;
                opts->not_cmp = opts->not_cmp | nFoundbyWorker.not_cmp;
                busyTasks--;
            ...
            if (havePrintToken) {
              ...
              MPI_Iprobe(MPI_ANY_SOURCE, MPI_TAG_TOK_REQUEST, MPI_COMM_WORLD,
                         &incomingMessage, &Status);
              if (incomingMessage) {
                MPI_Recv(NULL, 0, MPI_BYTE, Status.MPI_SOURCE,
                         MPI_TAG_TOK_REQUEST, MPI_COMM_WORLD, &Status);
                MPI_Send(NULL, 0, MPI_BYTE, Status.MPI_SOURCE,
                         MPI_TAG_PRINT_TOK, MPI_COMM_WORLD);
                havePrintToken = 0;
              } /* end if */
            }   /* end if */
          }     /* end if */
          ...
          for (n = 1; (n < g_nTasks) && !workerFound; n++) {
            if (workerTasks[n - 1]) {
              ...
              MPI_Send(&args, sizeof(args), MPI_BYTE, n, MPI_TAG_ARGS,
                       MPI_COMM_WORLD);
          ...
          if (!workerFound) {
            ...
            if (!havePrintToken) {
              while (!havePrintToken) {
                ...
                if (incomingMessage) {
                  MPI_Recv(&nFoundbyWorker, sizeof(nFoundbyWorker), MPI_BYTE,
                           MPI_ANY_SOURCE, MPI_TAG_TOK_RETURN, MPI_COMM_WORLD,
                           &Status);
                  havePrintToken = 1;
                  nfound += nFoundbyWorker.nfound;
                  opts->not_cmp = opts->not_cmp | nFoundbyWorker.not_cmp;
                  /* send this task the work unit. */
                  MPI_Send(&args, sizeof(args), MPI_BYTE, Status.MPI_SOURCE,
                           MPI_TAG_ARGS, MPI_COMM_WORLD);
                } /* end if */
              }   /* end while */
            }     /* end if */
            /* if we do have the token, check for task to free up, or wait for a
             * task to request it */
            else {
    ...
    if (g_Parallel) {
      ...
      while (busyTasks > 0) {
        ...
        if (Status.MPI_TAG == MPI_TAG_DONE) {
          ...
          /* someone else must have it...wait for them to return it, then give
           * it to the task that just asked for it. */
          else {
            ...
            opts->not_cmp = opts->not_cmp | nFoundbyWorker.not_cmp;
            busyTasks--;
            MPI_Send(NULL, 0, MPI_BYTE, source, MPI_TAG_PRINT_TOK,
                     MPI_COMM_WORLD);
          } /* end else */
        }   /* end else-if */
 ...
 *-------------------------------------------------------------------------
 */
hsize_t diff(hid_t file1_id, const char *path1, hid_t file2_id,
             const char *path2, diff_opt_t *opts, diff_args_t *argdata) {
  int status = -1;
  ...
  if (argdata->type[0] == H5TRAV_TYPE_LINK ||
    ...
    if ((status = H5tools_get_symlink_info(file1_id, path1, &linkinfo1,
                                           opts->follow_links)) < 0)
    ...
    if ((status = H5tools_get_symlink_info(file2_id, path2, &linkinfo2,
                                           opts->follow_links)) < 0)
      H5TOOLS_GOTO_ERROR(H5DIFF_ERR, "H5tools_get_symlink_info failed");
    /* dangling link */
    if (status == 0) {
  ...
  if (argdata->type[0] != argdata->type[1]) {
    ...
    if (opts->mode_verbose || opts->mode_list_not_cmp) {
      parallel_print(
          "Not comparable: <%s> is of type %s and <%s> is of type %s\n", path1,
  ...
  if (argdata->is_same_trgobj) {
    ...
    if (opts->follow_links || is_hard_link) {
      ...
      if (opts->mode_verbose || opts->mode_report) {
        ...
        default:
          parallel_print(
              "Comparison not supported: <%s> and <%s> are of type %s\n", path1,
  ...
  switch (object_type) {
     ...
     */
  case H5TRAV_TYPE_DATASET:
    H5TOOLS_DEBUG("diff object type H5TRAV_TYPE_DATASET - errstat:%d",
                  opts->err_stat);
  ...
  case H5TRAV_TYPE_UDLINK: {
    ...
    if (linkinfo1.linfo.type == H5L_TYPE_EXTERNAL &&
      ...
      if (linkinfo1.linfo.u.val_size == linkinfo2.linfo.u.val_size) {
        status = memcmp(linkinfo1.trg_path, linkinfo2.trg_path,
                        linkinfo1.linfo.u.val_size);
      } else
        status = 1;
