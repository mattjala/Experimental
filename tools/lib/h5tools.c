...
FILE *rawattrstream =
    NULL; /* should initialize to stdout but gcc moans about it */
FILE *rawdatastream =
    NULL; /* should initialize to stdout but gcc moans about it */
FILE *rawinstream =
    NULL; /* should initialize to stdin but gcc moans about it */
FILE *rawoutstream =
    NULL; /* should initialize to stdout but gcc moans about it */
FILE *rawerrorstream =
    NULL; /* should initialize to stderr but gcc moans about it */
...
H5_iter_order_t sort_order =
    H5_ITER_INC; /* sort_order [ascending | descending] */
 ...
 *           Effect of any h5tools function called before this has been called
 *is undetermined.
 *
 * Return    None
 *-------------------------------------------------------------------------
 ...
 *
 * Purpose:  Close or release resources such as files opened by the library.
 *This should be called after all other h5tools functions have been called.
 ...
 *-------------------------------------------------------------------------
 */
static herr_t h5tools_set_fapl_vfd(hid_t fapl_id,
  ...
  case VFD_BY_NAME:
    ...
    if (!HDstrcmp(vfd_info->u.name, drivernames[SEC2_VFD_IDX])) {
      ...
      /* SPLIT Driver */
      if (H5Pset_fapl_split(fapl_id, "-m.h5", H5P_DEFAULT, "-r.h5",
                            H5P_DEFAULT) < 0)
      ...
      if (H5Pset_fapl_ros3(
              fapl_id, &((const H5FD_ros3_fapl_ext_t *)vfd_info->info)->fa) < 0)
        H5TOOLS_GOTO_ERROR(FAIL, "H5Pset_fapl_ros3() failed");

      if (H5Pset_fapl_ros3_token(
              fapl_id, ((const H5FD_ros3_fapl_ext_t *)vfd_info->info)->token) <
          0)
      ...
      if (mpi_initialized && !mpi_finalized) {
        if (H5Pset_fapl_subfiling(
                fapl_id, (const H5FD_subfiling_config_t *)vfd_info->info) < 0)
      ...
      if (H5Pset_fapl_onion(fapl_id,
                            (const H5FD_onion_fapl_info_t *)vfd_info->info) < 0)
       ...
       */
      if (H5Pset_driver_by_name(fapl_id, vfd_info->u.name,
                                (const char *)vfd_info->info) < 0)
        H5TOOLS_GOTO_ERROR(FAIL, "can't load VFD plugin by driver name '%s'",
                           vfd_info->u.name);
    }
  ...
  case VFD_BY_VALUE:
     ...
     * Currently, driver configuration strings are unsupported.
     */
    if (H5Pset_driver_by_value(fapl_id, vfd_info->u.value,
                               (const char *)vfd_info->info) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t h5tools_set_fapl_vol(hid_t fapl_id,
  ...
  case VOL_BY_NAME:
    ...
    if ((connector_is_registered =
             H5VLis_connector_registered_by_name(vol_info->u.name)) < 0)
    ...
    if (connector_is_registered) {
      ...
      if (!HDstrcmp(vol_info->u.name, H5VL_NATIVE_NAME)) {
         ...
         * connectors.
         */
        if ((connector_id = H5VLregister_connector_by_name(vol_info->u.name,
                                                           H5P_DEFAULT)) < 0)
  ...
  case VOL_BY_VALUE:
    ...
    if ((connector_is_registered =
             H5VLis_connector_registered_by_value(vol_info->u.value)) < 0)
      H5TOOLS_GOTO_ERROR(FAIL, "can't check if VOL connector is registered");
    if (connector_is_registered) {
      ...
      if (vol_info->u.value == H5VL_NATIVE_VALUE) {
         ...
         * connectors.
         */
        if ((connector_id = H5VLregister_connector_by_value(vol_info->u.value,
                                                            H5P_DEFAULT)) < 0)
          H5TOOLS_GOTO_ERROR(FAIL, "can't register VOL connector");
      }
    }
  ...
  if (vol_info->info_string)
    if (H5VLconnector_str_to_info(vol_info->info_string, connector_id,
                                  &connector_info) < 0)
 ...
 *-------------------------------------------------------------------------
 */
hid_t h5tools_get_fapl(hid_t prev_fapl_id, h5tools_vol_info_t *vol_info,
                       h5tools_vfd_info_t *vfd_info) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t h5tools_get_vfd_name(hid_t fid, hid_t fapl_id, char *drivername,
 ...
 *-------------------------------------------------------------------------
 */
hid_t h5tools_fopen(const char *fname, unsigned flags, hid_t fapl_id,
                    hbool_t use_specific_driver, char *drivername,
                    size_t drivername_size) {
  ...
  /*
   * If we failed to open the file and the caller specified
   * 'use_specific_driver' as TRUE, we should return failure now since the file
   * couldn't be opened with the VFL driver/VOL connector that was set on the
   * FAPL by the caller.
   */
  if (use_specific_driver)
    H5TOOLS_GOTO_ERROR(H5I_INVALID_HID,
                       "failed to open file using specified FAPL");
   ...
   * connector being looked at, also try using each of the available VFL
   * drivers.
   */
  for (volnum = 0; volnum < NUM_VOLS; volnum++) {
    ...
    if (NATIVE_VOL_IDX == volnum) {
      ...
      for (drivernum = 0; drivernum < NUM_DRIVERS; drivernum++) {
        ...
        H5E_BEGIN_TRY {
          fid = h5tools_fopen(fname, flags, tmp_fapl_id, TRUE, drivername,
                              drivername_size);
        }
        H5E_END_TRY
      ...
      if ((fid = h5tools_fopen(fname, flags, tmp_fapl_id, TRUE, drivername,
                               drivername_size)) >= 0) {
        used_fapl_id = tmp_fapl_id;
        H5TOOLS_GOTO_DONE(fid);
      } else {
  ...
  if (drivername && drivername_size && ret_value >= 0)
    if (used_fapl_id >= 0 &&
        h5tools_get_vfd_name(ret_value, used_fapl_id, drivername,
                             drivername_size) < 0)
      H5TOOLS_ERROR(H5I_INVALID_HID,
                    "failed to retrieve name of VFD used to open file");
 ...
 *           applicable), calculate the prefix string, and display it at the
 *start of a line.
 *
 * Return:   None
 *-------------------------------------------------------------------------
 */
void h5tools_simple_prefix(FILE *stream, const h5tool_format_t *info,
  ...
  H5TOOLS_DEBUG("before CR elmtno=%ld, ctx->cur_column=%d, info->idx_fmt=%s, "
                "info->line_suf=%s",
                elmtno, ctx->cur_column, info->idx_fmt, info->line_suf);
  if (ctx->cur_column) {
 ...
 * Purpose:  If /ctx->need_prefix/ is set then terminate the current line (if
 *           applicable), calculate the prefix string, and display it at the
 *start of a line. Calls region specific function.
 *
 * Return:   None
 *-------------------------------------------------------------------------
 */
void h5tools_region_simple_prefix(FILE *stream, const h5tool_format_t *info,
                                  h5tools_context_t *ctx, hsize_t elmtno,
                                  hsize_t *ptdata, int secnum) {
  h5tools_str_t prefix;
  h5tools_str_t str; /*temporary for indentation */
  size_t templength = 0;
  unsigned u, indentlevel = 0;
...
hbool_t h5tools_render_element(FILE *stream, const h5tool_format_t *info,
  ...
  H5TOOLS_DEBUG("elmt_counter=%ld - local_elmt_counter=%ld", elmt_counter,
                local_elmt_counter);
  ...
  H5TOOLS_DEBUG("elmt_counter=%ld - ctx->size_last_dim=%ld info->line_suf=%s",
                elmt_counter, ctx->size_last_dim, info->line_suf);
  ...
  for (secnum = 0, multiline = 0;
       (section = HDstrtok(secnum ? NULL : s, OPTIONAL_LINE_BREAK)); secnum++) {
    /*
    ...
    if (ctx->need_prefix) {
      ...
      *curr_pos = ctx->sm_pos + local_elmt_counter;
      H5TOOLS_DEBUG("curr_pos=%ld - ctx->sm_pos=%ld - ctx->ndims=%ld",
                    *curr_pos, ctx->sm_pos, ctx->ndims);
...
hbool_t h5tools_render_region_element(FILE *stream, const h5tool_format_t *info,
                                      h5tools_context_t *ctx,
                                      h5tools_str_t *buffer, hsize_t *curr_pos,
  ...
  for (secnum = 0, multiline = 0;
    ...
    if (ctx->need_prefix) {
      ...
      *curr_pos = ctx->sm_pos + local_elmt_counter;
      ...
      h5tools_region_simple_prefix(stream, info, ctx, local_elmt_counter,
                                   ptdata, secnum);
    } else if ((local_elmt_counter || ctx->continuation) && secnum == 0) {
      PUTSTREAM(OPT(info->elmt_suf2, " "), stream);
      ctx->cur_column += HDstrlen(OPT(info->elmt_suf2, " "));
 ...
 *-------------------------------------------------------------------------
 */
void init_acc_pos(unsigned ndims, const hsize_t *dims, hsize_t *acc,
                  hsize_t *pos, hsize_t *p_min_idx) {
  int i;
  unsigned j;
 ...
 *-------------------------------------------------------------------------
 */
hsize_t calc_acc_pos(unsigned ndims, hsize_t elmtno, const hsize_t *acc,
                     hsize_t *pos) {
  int i;
  ...
  if (ndims > 0) {
    for (i = 0; i < (int)ndims; i++) {
      if (curr_pos > 0) {
        ...
        pos[i] = 0;
      H5TOOLS_DEBUG("curr_pos=%ld - pos[%d]=%ld - acc[%d]=%ld", curr_pos, i,
                    pos[i], i, acc[i]);
    }
  }
 ...
 *-------------------------------------------------------------------------
 */
int render_bin_output(FILE *stream, hid_t container, hid_t tid, void *_mem,
  ...
  case H5T_VLEN: {
    ...
    for (block_index = 0; block_index < block_nelmts; block_index++) {
      ...
      /* dump the array element */
      if (render_bin_output(stream, container, memb,
                            ((char *)(((hvl_t *)((void *)mem))->p)),
                            nelmts) < 0) {
        H5Tclose(memb);
        H5TOOLS_THROW((-1), "render_bin_output failed");
      }
      ...
      if (region_output) {
        ...
        for (block_index = 0; block_index < block_nelmts; block_index++) {
          ...
          if ((region_id = H5Ropen_object(&tref, H5P_DEFAULT, H5P_DEFAULT)) < 0)
            ...
            if ((region_space =
                     H5Ropen_region(&tref, H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
              if (!h5tools_is_zero(&tref, H5Tget_size(H5T_STD_REF))) {

                region_type = H5Sget_select_type(region_space);
                if (region_type == H5S_SEL_POINTS)
                  render_bin_output_region_points(region_space, region_id,
                                                  stream, container);
                else
                  render_bin_output_region_blocks(region_space, region_id,
                                                  stream, container);
              } else {
 ...
 *-------------------------------------------------------------------------
 */
int render_bin_output_region_data_blocks(hid_t region_id, FILE *stream,
                                         hid_t container, unsigned ndims,
                                         hid_t type_id, hsize_t nblocks,
 ...
 *           This is a special case function to dump a region reference using
 *points.
 *
 ...
 *-------------------------------------------------------------------------
 */
hbool_t render_bin_output_region_points(hid_t region_space, hid_t region_id,
                                        FILE *stream, hid_t container) {
  hssize_t snpoints;
  hsize_t npoints;
  int sndims;
  ...
  render_bin_output_region_data_points(region_space, region_id, stream,
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE hbool_t h5tools_is_zero(const void *_mem, size_t size) {
 ...
 *
 * Purpose:  Check if two given object IDs or link names point to the same
 *object.
 *
 * Parameters:
 *           hid_t loc_id1: location of the first object
 *           char *name1:   link name of the first object.
 *                          Use "." or NULL if loc_id1 is the object to be
 *compared. hid_t loc_id2: location of the second object char *name1:   link
 *name of the first object. Use "." or NULL if loc_id2 is the object to be
 *compared.
 *
 ...
 *-------------------------------------------------------------------------
 */
hbool_t h5tools_is_obj_same(hid_t loc_id1, const char *name1, hid_t loc_id2,
                            const char *name2) {
