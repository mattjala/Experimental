...
static void h5tools_print_char(h5tools_str_t *str, const h5tool_format_t *info,
                               char ch);
void h5tools_str_indent(h5tools_str_t *str, const h5tool_format_t *info,
 ...
 *-------------------------------------------------------------------------
 */
H5_ATTR_PURE size_t h5tools_str_len(h5tools_str_t *str) { return str->len; }
...
char *h5tools_str_append(h5tools_str_t *str /*in,out*/, const char *fmt, ...) {
  ...
  while (1) {
    ...
    if ((size_t)nchars >= avail ||
        (0 == nchars && (HDstrcmp(fmt, "%s") != 0))) {
      /* Truncation return value as documented by C99, or zero return value with
       * either of the following conditions, each of which indicates that the
       * proper C99 return value probably should have been positive when the
       * format string is something other than "%s" Allocate at least twice as
       * much space and try again.
       */
 ...
 *-------------------------------------------------------------------------
 */
char *h5tools_str_fmt(h5tools_str_t *str /*in,out*/, size_t start,
                      const char *fmt) {
 ...
 *-------------------------------------------------------------------------
 */
char *h5tools_str_prefix(h5tools_str_t *str /*in,out*/,
                         const h5tool_format_t *info, hsize_t elmtno,
                         h5tools_context_t *ctx) {
  size_t i = 0;
  ...
  if (ctx->ndims > 0) {
    ...
    for (i = 0; i < (size_t)ctx->ndims; i++) {
      ...
      H5_GCC_CLANG_DIAG_OFF("format-nonliteral")
      h5tools_str_append(str, OPT(info->idx_n_fmt, "%" PRIuHSIZE),
                         (hsize_t)ctx->pos[i]);
      H5_GCC_CLANG_DIAG_ON("format-nonliteral")
    }
  } else {
 ...
 *
 * Purpose: Renders the line prefix value into string STR. Region reference
 *specific.
 *
 * Return:  Success:    Pointer to the prefix.
 *          Failure:    NULL
 *-------------------------------------------------------------------------
 */
char *h5tools_str_region_prefix(h5tools_str_t *str /*in,out*/,
                                const h5tool_format_t *info, hsize_t elmtno,
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_str_dump_space_slabs(h5tools_str_t *str, hid_t rspace,
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_str_dump_space_blocks(h5tools_str_t *str, hid_t rspace,
                                   const h5tool_format_t *info) {
  hssize_t snblocks;
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_str_dump_space_points(h5tools_str_t *str, hid_t rspace,
                                   const h5tool_format_t *info) {
  hssize_t snpoints;
 ...
 *-------------------------------------------------------------------------
 */
char *h5tools_str_sprint(h5tools_str_t *str, const h5tool_format_t *info,
  ...
  if (info->raw) {
    ...
    case H5T_STRING: {
      ...
      if (H5Tis_variable_str(type)) {
         ...
         * have to dereference the pointer to get the `char*' to pass to
         * HDstrlen(). */
        s = *(char **)((void *)cp_vp);
        if (s != NULL)
          size = HDstrlen(s);
        ...
        if (info->ascii)
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(unsigned char))
              ...
              tempuchar =
                  (unsigned char)((unsigned long long)(tempuchar >>
                                                       packed_data_offset) &
                                  packed_data_mask);
          }
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(char))
              ...
              tempchar =
                  (signed char)((unsigned long long)(tempchar >>
                                                     packed_data_offset) &
                                packed_data_mask);
          }
        ...
        if (H5T_SGN_NONE == nsign) {
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(unsigned int))
              ...
              tempuint = (unsigned)((tempuint >> packed_data_offset) &
                                    packed_data_mask);
          }
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(int))
              ...
              tempint =
                  (int)((unsigned long long)(tempint >> packed_data_offset) &
                        packed_data_mask);
          }
        ...
        if (H5T_SGN_NONE == nsign) {
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(unsigned short))
              ...
              tempushort =
                  (unsigned short)((unsigned long long)(tempushort >>
                                                        packed_data_offset) &
                                   packed_data_mask);
          }
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(short))
              ...
              tempshort = (short)((unsigned long long)(tempshort >>
                                                       packed_data_offset) &
                                  packed_data_mask);
          }
        ...
        if (H5T_SGN_NONE == nsign) {
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(long))
              ...
              templong =
                  (long)((unsigned long long)(templong >> packed_data_offset) &
                         packed_data_mask);
          }
        ...
        if (H5T_SGN_NONE == nsign) {
          ...
          if (packed_bits_num) {
            if (packed_data_offset >= 8 * sizeof(unsigned long long))
              ...
              tempullong =
                  (tempullong >> packed_data_offset) & packed_data_mask;
          }
          h5tools_str_append(str, OPT(info->fmt_ullong, fmt_ullong),
                             tempullong);
        } else {
          long long templlong;
    ...
    case H5T_COMPOUND:
      ...
      if (ctx->cmpd_listv) {        /* there is <list_of_fields> */
        ...
        const H5LD_memb_t *const
            *listv; /* Vector of information for <list_of_fields> */
        ...
        /*
         * Go through the vector containing info about the comma-separated list
         * of compound fields and then members in each field: put in "{", "}",
         * ",", member name and value accordingly.
         */
        ...
        for (curr_field = 0; listv[curr_field] != NULL; curr_field++) {
          ...
          for (i = 0; listv[curr_field]->names[i] != NULL; i++) {
            h5tools_str_append(str, OPT(info->cmpd_name, ""),
                               listv[curr_field]->names[i]);
            if (i) {
              ctx->indent_level++;
              h5tools_str_append(str, "%s", OPT(info->cmpd_pre, "{"));
            }
    ...
    case H5T_REFERENCE:
      ...
      if (h5tools_str_is_zero(vp, nsize))
        ...
        if (H5Tequal(type, H5T_STD_REF)) {
          ...
          h5tools_str_append(str, H5_TOOLS_DATASET);
          h5tools_str_sprint_old_reference(str, container, H5R_DATASET_REGION,
                                           vp);
    ...
    case H5T_ARRAY: {
      ...
      for (i = 0; i < nelmts; i++) {
        ...
        is_next_arry_elmt =
            0; /* dump all values in the array element, so turn it off */
    ...
    case H5T_VLEN: {
      ...
      for (i = 0; i < nelmts; i++) {
        ...
        h5tools_str_sprint(str, info, container, memb,
                           ((char *)(((hvl_t *)((void *)cp_vp))->p)) + i * size,
                           ctx);
 ...
 *-------------------------------------------------------------------------
 */
void h5tools_str_sprint_old_reference(h5tools_str_t *str, hid_t container,
                                      H5R_type_t ref_type, void *vp) {
  hid_t obj = H5I_INVALID_HID;
...
void h5tools_str_sprint_reference(h5tools_str_t *str, H5R_ref_t *ref_vp) {
  ...
  if (buf_size) {
    ...
    if (H5Rget_obj_name(ref_vp, H5P_DEFAULT, obj_name, (size_t)buf_size + 1) >=
        0) {
      obj_name[buf_size] = '\0';
 ...
 *-------------------------------------------------------------------------
 */
char *h5tools_str_replace(const char *string, const char *substr,
  ...
  while ((tok = HDstrstr(head, substr))) {
    ...
    oldstr = newstr;
    newstr = (char *)malloc(HDstrlen(oldstr) - HDstrlen(substr) +
                            HDstrlen(replacement) + 1);
    ...
    memset(newstr + HDstrlen(oldstr) - HDstrlen(substr) + HDstrlen(replacement),
           0, 1);
