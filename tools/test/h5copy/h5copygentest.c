...
static void gent_simple(hid_t loc_id) {
  ...
  /* create dataset */
  did = H5Dcreate2(loc_id, DATASET_SIMPLE, H5T_NATIVE_INT, sid, H5P_DEFAULT,
...
static void gent_chunked(hid_t loc_id) {
  ...
  /* create dataset */
  did = H5Dcreate2(loc_id, DATASET_CHUNK, H5T_NATIVE_INT, sid, H5P_DEFAULT, pid,
                   H5P_DEFAULT);
...
static void gent_compact(hid_t loc_id) {
  ...
  /* create dataset */
  did = H5Dcreate2(loc_id, DATASET_COMPACT, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   pid, H5P_DEFAULT);
...
static void gent_compound(hid_t loc_id) {
  ...
  /* create dataset */
  did = H5Dcreate2(loc_id, DATASET_COMPOUND, tid_c, sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
...
static void gent_compressed(hid_t loc_id) {
  ...
  /* create dataset */
  did = H5Dcreate2(loc_id, DATASET_COMPRESSED, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   pid, H5P_DEFAULT);
...
static void gent_named_vl(hid_t loc_id) {
  ...
  /* create dataset */
  did = H5Dcreate2(loc_id, DATASET_NAMED_VL, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
...
static void gent_nested_vl(hid_t loc_id) {
  ...
  /* create dataset */
  did = H5Dcreate2(loc_id, DATASET_NESTED_VL, tid2, sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
...
static void gent_att_compound_vlstr(hid_t loc_id) {
  ...
  did = H5Dcreate2(loc_id, DATASET_ATTR, H5T_NATIVE_INT, null_sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
...
static void gent_nested_datasets(hid_t loc_id) {
  ...
  /* Create group in location */
  gid =
      H5Gcreate2(loc_id, GROUP_DATASETS, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
static herr_t gen_obj_ref(hid_t loc_id) {
  ...
  oid = H5Dcreate2(loc_id, OBJ_REF_DS, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
  if (oid < 0) {
  ...
  status =
      H5Dwrite(oid, H5T_STD_REF_OBJ, H5S_ALL, H5S_ALL, H5P_DEFAULT, or_data);
  if (status < 0) {
    fprintf(stderr, "Error: %s %d> H5Dwrite failed.\n", __func__, __LINE__);
    ret = FAIL;
    goto out;
...
static herr_t gen_region_ref(hid_t loc_id) {
  ...
  oid2 = H5Dcreate2(loc_id, REG_REF_DS2, H5T_STD_I8LE, sid, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
  if (oid2 < 0) {
    fprintf(stderr, "Error: %s %d> H5Dcreate2 failed.\n", __func__, __LINE__);
    ret = FAIL;
  ...
  oid1 = H5Dcreate2(loc_id, REG_REF_DS1, H5T_STD_REF_DSETREG, sid, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
  if (oid1 < 0) {
    fprintf(stderr, "Error: %s %d> H5Dcreate2 failed.\n", __func__, __LINE__);
    ret = FAIL;
  ...
  status = H5Dwrite(oid1, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    rr_data);
  if (status < 0) {
    fprintf(stderr, "Error: %s %d> H5Dwrite failed.\n", __func__, __LINE__);
    ret = FAIL;
...
static herr_t gen_extlink_trg(hid_t loc_id) {
   ...
   */
  tid = H5Tcopy(H5T_NATIVE_INT);
  status = H5Tcommit2(loc_id, "datatype", tid, H5P_DEFAULT, H5P_DEFAULT,
                      H5P_DEFAULT);
  if (status < 0) {
...
static herr_t gen_extlink_src(hid_t loc_id) {
   ...
   *------------------------------------------------------------------------*/
  /* link to dataset */
  status = H5Lcreate_external(HDF_EXT_TRG_FILE, "/simple", gid, "extlink_dset",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
  ...
  /* link to group */
  status = H5Lcreate_external(HDF_EXT_TRG_FILE, "/group", gid, "extlink_grp",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s %d> H5Lcreate_external failed.\n", __func__,
            __LINE__);
  ...
  /* link to datatype */
  status = H5Lcreate_external(HDF_EXT_TRG_FILE, "/datatype", gid,
                              "extlink_datatype", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s %d> H5Lcreate_external failed.\n", __func__,
            __LINE__);
    ret = FAIL;
  ...
  status = H5Lcreate_external(HDF_EXT_TRG_FILE, "notyet", gid,
                              "extlink_notyet1", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s %d> H5Lcreate_external failed.\n", __func__,
            __LINE__);
    ret = FAIL;
  ...
  status = H5Lcreate_external("notyet_file.h5", "notyet", gid,
                              "extlink_notyet2", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s %d> H5Lcreate_external failed.\n", __func__,
            __LINE__);
    ret = FAIL;
