 ...
 */
static herr_t gen_newgrat_file(const char *fname) {
  ...
  if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, 1,
                                 (hsize_t)1) < 0)
    goto error;
  ...
  for (i = 1; i <= NUM_GRPS; i++) {
    ...
    if ((gid = H5Gcreate2(fid, name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
        0)
      goto error;
    if (H5Gclose(gid) < 0)
      goto error;
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DATASET_NAME, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 1; i <= NUM_ATTRS; i++) {
    ...
    if ((attr_id =
             H5Acreate2(did, attrname, tid, sid, H5P_DEFAULT, H5P_DEFAULT)) < 0)
      goto error;
...
/*
 * Generate an HDF5 file with groups, datasets, attributes for testing the
 * options: -l N (--links=N): Set the threshold for # of links when printing
 * information for small groups. -m N (--dims=N): Set the threshold for the # of
 * dimension sizes when printing information for small datasets. -a N
 * (--numattrs=N): Set the threshold for the # of attributes when printing
 * information for small # of attributes.
 */
static herr_t gen_threshold_file(const char *fname) {
  hid_t fid = H5I_INVALID_HID;     /* File ID */
  ...
  if ((attr_id = H5Acreate2(fid, "attr", H5T_NATIVE_INT, sid1, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    goto error;
  if (H5Aclose(attr_id) < 0)
    goto error;
  ...
  if ((did = H5Dcreate2(fid, "zero_dset", H5T_NATIVE_UCHAR, sid0, H5P_DEFAULT,
  ...
  for (i = 1; i <= (THRES_NUM + 1); i++) {
    ...
    if ((attr_id = H5Acreate2(did, name, H5T_NATIVE_INT, sid1, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
      goto error;
    if (H5Aclose(attr_id) < 0)
      goto error;
  ...
  if ((did = H5Dcreate2(fid, "scalar_dset", H5T_NATIVE_UCHAR, sid3, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto error;
  if (H5Dclose(did) < 0)
    goto error;
  ...
  if ((did = H5Dcreate2(fid, "null_dset", H5T_NATIVE_UCHAR, sid4, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto error;
  if (H5Dclose(did) < 0)
    goto error;
  ...
  if ((attr_id = H5Acreate2(gid, "ATTR", H5T_NATIVE_INT, sid3, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 1; i <= THRES_NUM; i++) {
    ...
    /* Create the dataset */
    if ((did = H5Dcreate2(gid, name, H5T_NATIVE_UCHAR, sid1, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT)) < 0)
      goto error;
  ...
  for (i = 1; i <= THRES_NUM_25; i++) {
    ...
    if ((attr_id = H5Acreate2(gid, name, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
      goto error;
  ...
  /* Create third group */
  if ((gid = H5Gcreate2(fid, "group3", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    goto error;
 ...
 *
 */
static herr_t gen_idx_file(const char *fname) {
  ...
  if ((did2 = H5Dcreate2(fid, DSET_FILTER, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    goto error;
 ...
 *
 */
static herr_t gen_err_refcount(const char *fname) {
