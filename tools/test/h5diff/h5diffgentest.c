...
static int test_comp_vlen_strings(const char *fname1, const char *grp_name,
                                  int is_file_new);
static int test_attributes_verbose_level(const char *fname1,
                                         const char *fname2);
...
static void write_attr_strings(hid_t loc_id, const char *dset_name, hid_t fid,
                               int make_diffs);
static void write_attr_in(hid_t loc_id, const char *dset_name, hid_t fid,
                          int make_diffs);
static void write_dset_in(hid_t loc_id, const char *dset_name, hid_t fid,
                          int make_diffs);
static void gen_datareg(hid_t fid, int make_diffs);
/* utilities */
static int write_attr(hid_t loc_id, int rank, hsize_t *dims, const char *name,
                      hid_t tid, void *buf);
static herr_t write_dset(hid_t loc_id, int rank, hsize_t *dims,
                         const char *name, hid_t tid, void *buf);
...
int main(void) {
  ...
  test_attributes_verbose_level(ATTR_VERBOSE_LEVEL_FILE1,
                                ATTR_VERBOSE_LEVEL_FILE2);
...
static int test_onion_dset_extension(const char *fname) {
  ...
  hid_t dset = H5I_INVALID_HID;
  hid_t dcpl = H5I_INVALID_HID;
  hsize_t dims[2] = {4, 4}, maxdims[2] = {H5S_UNLIMITED, H5S_UNLIMITED},
          chunk[2] = {4, 4};
...
static int test_basic(const char *fname1, const char *fname2,
   ...
   */
  {
     ...
     */
    float data11[3][2] = {{0.000000f, 0.0000001f},
                          {0.0000001f, 0.00000022f},
                          {0.0000001f, 0.0000001f}};
    float data12[3][2] = {{0.000000f, 0.0000002f},
                          {0.0000003f, 0.0000001f},
                          {0.000000f, 0.0000001f}};
...
static int test_types(const char *fname) {
   ...
   *-------------------------------------------------------------------------
   */
  H5Lcreate_external("filename", "objname", fid1, "ext_link", H5P_DEFAULT,
                     H5P_DEFAULT);
  H5Lregister(UD_link_class);
  H5Lcreate_ud(fid1, "ud_link", (H5L_type_t)MY_LINKCLASS, NULL, (size_t)0,
 ...
 *-------------------------------------------------------------------------
 */
static int test_attributes(const char *file,
...
static int test_attributes_verbose_level(const char *fname1,
  ...
  f1_did = H5Dcreate2(fid1, "dset", H5T_NATIVE_INT, f1_sid, H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
  if (f1_did == FAIL) {
  ...
  status = H5Dwrite(f1_did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    dset_data);
  if (status < 0) {
  ...
  status =
      H5Tcommit2(fid1, "ntype", f1_tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
  ...
  f2_did = H5Dcreate2(fid2, "dset", H5T_NATIVE_INT, f2_sid, H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
  if (f2_did == FAIL) {
    fprintf(stderr, "Error: %s> write_dset failed\n", fname2);
    status = FAIL;
  ...
  status = H5Dwrite(f2_did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    dset_data);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname2);
    status = FAIL;
  ...
  status =
      H5Tcommit2(fid2, "ntype", f2_tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Tcommit2 failed.\n", fname2);
    status = FAIL;
    goto out;
 ...
 *-------------------------------------------------------------------------
 */
static int test_datasets(const char *file,
                         int make_diffs /* flag to modify data buffers */) {
  hid_t fid = H5I_INVALID_HID;
  hid_t did = H5I_INVALID_HID;
  hid_t gid = H5I_INVALID_HID;
 ...
 */
static int
test_special_datasets(const char *file,
                      int make_diffs /* flag to modify data buffers */) {
  hid_t fid = H5I_INVALID_HID;
  hid_t did = H5I_INVALID_HID;
  ...
  did = H5Dcreate2(fid, "dset1", H5T_NATIVE_INT, sid0, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  ...
  did = H5Dcreate2(fid, "dset2", H5T_NATIVE_INT, sid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
...
static int test_link_name(const char *fname1) {
  ...
  gid2 =
      H5Gcreate2(fid1, "group_longname", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  status = H5Lcreate_soft("group_longname", fid1, "link_g2", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
...
static int test_soft_links(const char *fname1) {
   ...
   *------------------------------------------------------------------------*/
  gid1 =
      H5Gcreate2(fid1, "target_group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (gid1 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname1);
    status = FAIL;
    goto out;
   ...
   *------------------------------------------------------------------------*/
  /* file 1 */
  status = H5Lcreate_soft("/target_dset1", fid1, "softlink_dset1_1",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/target_dset1", fid1, "softlink_dset1_2",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/target_dset2", fid1, "softlink_dset2", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/target_group", fid1, "softlink_group1", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/target_group", fid1, "softlink_group2", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/no_obj", fid1, "softlink_noexist", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
...
static int test_linked_softlinks(const char *fname1) {
  ...
  gid2 =
      H5Gcreate2(fid1, "target_group1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (gid2 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  gid3 =
      H5Gcreate2(fid1, "target_group2", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (gid3 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  /*---------
   * file 1 */
  status = H5Lcreate_soft("/target_dset1", fid1, "softlink1_to_dset1",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink1_to_dset1", fid1, "softlink1_to_slink1",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink1_to_slink1", fid1, "softlink1_to_slink2",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/target_dset2", fid1, "softlink2_to_dset2",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink2_to_dset2", fid1, "softlink2_to_slink1",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink2_to_slink1", fid1, "softlink2_to_slink2",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("target_group1", fid1, "softlink3_to_group1",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink3_to_group1", fid1, "softlink3_to_slink1",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink3_to_slink1", fid1, "softlink3_to_slink2",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("target_group2", fid1, "softlink4_to_group2",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink4_to_group2", fid1, "softlink4_to_slink1",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("softlink4_to_slink1", fid1, "softlink4_to_slink2",
                          H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
...
static int test_external_links(const char *fname1, const char *fname2) {
   ...
   * target file */
  gid1 =
      H5Gcreate2(fid2, "target_group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (gid1 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname2);
    status = FAIL;
    goto out;
  ...
  gid2 =
      H5Gcreate2(fid2, "target_group2", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (gid2 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname2);
    status = FAIL;
    goto out;
  ...
  /*--------------*/
  /* source file */
  status = H5Lcreate_external(fname2, "/target_group/x_dset", fid1,
                              "ext_link_dset1", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
  ...
  status = H5Lcreate_external(fname2, "/target_group2/x_dset", fid1,
                              "ext_link_dset2", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  status = H5Lcreate_external(fname2, "/target_group", fid1, "/ext_link_grp1",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname2, "/target_group2", fid1, "/ext_link_grp2",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname2, "no_obj", fid1, "ext_link_noexist1",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external("no_file.h5", "no_obj", fid1, "ext_link_noexist2",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
...
static int test_ext2soft_links(const char *fname1, const char *fname2) {
  ...
  /*---------------
   * target file */
  status = H5Lcreate_soft("/dset1", fid2, "softlink_to_dset1", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/dset2", fid2, "softlink_to_dset2", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
  ...
  /*---------------
   * source file */
  status = H5Lcreate_external(fname2, "/target_group", fid1, "ext_link",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname2, "/softlink_to_dset1", fid1,
                              "ext_link_to_slink1", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  status = H5Lcreate_external(fname2, "/softlink_to_dset2", fid1,
                              "ext_link_to_slink2", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
    goto out;
...
static int gen_dataset_idx(const char *file, int format) {
  ...
  /* Create a 1D dataset */
  sid = H5Screate_simple(1, dims, NULL);
  did = H5Dcreate2(fid, "dset", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  ...
  did2 = H5Dcreate2(fid, "dset_filter", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
...
static int test_dangle_links(const char *fname1, const char *fname2) {
  ...
  /* file 1 */
  status =
      H5Lcreate_soft("no_obj", fid1, "soft_link1", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  status =
      H5Lcreate_soft("/dset1", fid1, "soft_link2", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  status =
      H5Lcreate_soft("no_obj", fid1, "soft_link3", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  status =
      H5Lcreate_soft("no_obj1", fid1, "soft_link4", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  /* file 2 */
  status =
      H5Lcreate_soft("no_obj", fid2, "soft_link1", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
    goto out;
  ...
  status =
      H5Lcreate_soft("no_obj", fid2, "soft_link2", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
    goto out;
  ...
  status =
      H5Lcreate_soft("/dset2", fid2, "soft_link3", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
    goto out;
  ...
  status =
      H5Lcreate_soft("no_obj2", fid2, "soft_link4", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
    goto out;
   ...
   *------------------------------------------------------------------------*/
  /* file1 */
  status = H5Lcreate_external(fname2, "no_obj", fid1, "ext_link1", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname2, "/dset1", fid1, "ext_link2", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname2, "no_obj", fid1, "ext_link3", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external("no_file.h5", "no_obj", fid1, "ext_link4",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  /* file2 */
  status = H5Lcreate_external(fname1, "no_obj", fid2, "ext_link1", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname1, "no_obj", fid2, "ext_link2", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname1, "/dset2", fid2, "ext_link3", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
  ...
  status = H5Lcreate_external("no_file.h5", "no_obj", fid2, "ext_link4",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
...
static int test_group_recurse(const char *fname1, const char *fname2) {
  hid_t fid1 = H5I_INVALID_HID;
  hid_t fid2 = H5I_INVALID_HID;
  hid_t gid1_f1 = H5I_INVALID_HID, gid2_f1 = H5I_INVALID_HID,
        gid3_f1 = H5I_INVALID_HID, gid10_f1 = H5I_INVALID_HID;
  hid_t gid1_f2 = H5I_INVALID_HID, gid2_f2 = H5I_INVALID_HID,
        gid3_f2 = H5I_INVALID_HID, gid11_f2 = H5I_INVALID_HID;
  hsize_t dims2[2] = {2, 4};
  ...
  gid2_f1 =
      H5Gcreate2(fid1, "/grp1/grp2", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (gid2_f1 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  gid3_f1 = H5Gcreate2(fid1, "/grp1/grp2/grp3", H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
  if (gid3_f1 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname1);
    status = FAIL;
  ...
  gid2_f2 =
      H5Gcreate2(fid2, "/grp1/grp2", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  if (gid2_f2 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname2);
    status = FAIL;
    goto out;
  ...
  gid3_f2 = H5Gcreate2(fid2, "/grp1/grp2/grp3", H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
  if (gid3_f2 < 0) {
    fprintf(stderr, "Error: %s> H5Gcreate2 failed.\n", fname2);
    status = FAIL;
  ...
  /* file 1 */
  status =
      H5Lcreate_soft("/grp1", fid1, "slink_grp1", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  status = H5Lcreate_soft("/grp1/grp2", fid1, "slink_grp2", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/grp1/grp2/grp3", fid1, "slink_grp3", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
  ...
  status =
      H5Lcreate_soft("/grp10", fid1, "slink_grp10", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  /* file 2 */
  status =
      H5Lcreate_soft("/grp1", fid2, "slink_grp1", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
    goto out;
  ...
  status = H5Lcreate_soft("/grp1/grp2", fid2, "slink_grp2", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
  ...
  status = H5Lcreate_soft("/grp1/grp2/grp3", fid2, "slink_grp3", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
  ...
  status =
      H5Lcreate_soft("/grp11", fid2, "slink_grp11", H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", fname2);
    status = FAIL;
    goto out;
   ...
   *------------------------------------------------------------------------*/
  /* file1 */
  status = H5Lcreate_external(fname2, "/grp1", fid1, "elink_grp1", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname2, "/grp1/grp2", fid1, "elink_grp2",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname2, "/grp1/grp2/grp3", fid1, "elink_grp3",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  /* file2 */
  status = H5Lcreate_external(fname1, "/grp1", fid2, "elink_grp1", H5P_DEFAULT,
                              H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname1, "/grp1/grp2", fid2, "elink_grp2",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
  ...
  status = H5Lcreate_external(fname1, "/grp1/grp2/grp3", fid2, "elink_grp3",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
   ...
   */
  /* file1 */
  status = H5Lcreate_external(fname2, "/grp11", gid10_f1, "elink_grp_circle",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname1);
    status = FAIL;
  ...
  /* file2 */
  status = H5Lcreate_external(fname1, "/grp10", gid11_f2, "elink_grp_circle",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n", fname2);
    status = FAIL;
...
static int test_group_recurse2(void) {
   ...
   */
  fileid1 =
      H5Fcreate(GRP_RECURSE1_EXT, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
   ...
   */
  dset1 = H5Dcreate2(fileid1, GRP_R_DSETNAME1, datatype, dataspace, H5P_DEFAULT,
                     H5P_DEFAULT, H5P_DEFAULT);
   ...
   * datatype and default dataset creation properties.
   */
  dset1 = H5Dcreate2(grp3, GRP_R_DSETNAME1, datatype, dataspace, H5P_DEFAULT,
                     H5P_DEFAULT, H5P_DEFAULT);
   ...
   * datatype and default dataset creation properties.
   */
  dset2 = H5Dcreate2(grp4, GRP_R_DSETNAME2, datatype, dataspace, H5P_DEFAULT,
                     H5P_DEFAULT, H5P_DEFAULT);
   ...
   */
  /* link to dset1 */
  status = H5Lcreate_soft(GRP_R_DSETNAME1, fileid1, "soft_dset1", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", GRP_RECURSE1_EXT);
    status = FAIL;
  ...
  /* create target file */
  fileid4 =
      H5Fcreate(GRP_RECURSE2_EXT3, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* create target file */
  fileid3 =
      H5Fcreate(GRP_RECURSE2_EXT2, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
   ...
   */
  status = H5Lcreate_external(GRP_RECURSE2_EXT3, "/g4", fileid3, "/g2/g3/g4",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n",
            GRP_RECURSE2_EXT2);
  ...
  /* create target file */
  fileid2 =
      H5Fcreate(GRP_RECURSE2_EXT1, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
   ...
   * dset1
   */
  dset1 = H5Dcreate2(fileid2, GRP_R_DSETNAME1, datatype, dataspace, H5P_DEFAULT,
                     H5P_DEFAULT, H5P_DEFAULT);
   ...
   */
  /* link to dset1 */
  status = H5Lcreate_soft(GRP_R_DSETNAME1, fileid2, "soft_dset1", H5P_DEFAULT,
                          H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_soft failed.\n", GRP_RECURSE2_EXT1);
    status = FAIL;
   ...
   */
  status = H5Lcreate_external(GRP_RECURSE2_EXT2, "/g2", fileid2, "/g1/g2",
                              H5P_DEFAULT, H5P_DEFAULT);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Lcreate_external failed.\n",
            GRP_RECURSE2_EXT1);
...
static int test_comp_vlen_strings(const char *fname1, const char *grp_name,
  ...
  typedef struct comp1_t {
    ...
    char str_array_fixlen[FIXLEN_STR_ARRY_DIM]
                         [FIXLEN_STR_ARRY_SIZE]; /* fixed len string array */
    char str_fixlen_array_again[FIXLEN_STR_ARRY_DIM]
                               [FIXLEN_STR_ARRY_SIZE]; /* fixed len string array
                                                        */
  } comp1_t;
  ...
  tid_vlen_str_array =
      H5Tarray_create2(tid_vlen_str_array_pre, COMP_RANK, dims_vlen_str_array);
   ...
   *------------------------------------------------------------------------*/
  sid_fixlen_str_array =
      H5Screate_simple(STR_RANK, dims_fixlen_str_array, NULL);
  ...
  /* Create the array data type for the string array                */
  tid_fixlen_str_array = H5Tarray_create2(tid_fixlen_str_array_pre, COMP_RANK,
                                          dims_fixlen_str_array);
  ...
  H5Tinsert(tid1_comp, "VLEN_STR2", HOFFSET(comp1_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid1_comp, "FIXLEN_STR1", HOFFSET(comp1_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid1_comp, "FIXLEN_STR2", HOFFSET(comp1_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid1_comp, "VLEN_STR_ARRAY1", HOFFSET(comp1_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid1_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp1_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid1_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp1_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid1_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp1_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 2 */
  H5Tinsert(tid2_comp, "VLEN_STR1", HOFFSET(comp2_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid2_comp, "VLEN_STR2", HOFFSET(comp2_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid2_comp, "FIXLEN_STR1", HOFFSET(comp2_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid2_comp, "FIXLEN_STR2", HOFFSET(comp2_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid2_comp, "VLEN_STR_ARRAY1", HOFFSET(comp2_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid2_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp2_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid2_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp2_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid2_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp2_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 3 */
  H5Tinsert(tid3_comp, "VLEN_STR1", HOFFSET(comp3_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid3_comp, "VLEN_STR2", HOFFSET(comp3_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid3_comp, "FIXLEN_STR1", HOFFSET(comp3_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid3_comp, "FIXLEN_STR2", HOFFSET(comp3_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid3_comp, "VLEN_STR_ARRAY1", HOFFSET(comp3_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid3_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp3_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid3_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp3_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid3_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp3_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 4 */
  H5Tinsert(tid4_comp, "VLEN_STR1", HOFFSET(comp4_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid4_comp, "VLEN_STR2", HOFFSET(comp4_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid4_comp, "FIXLEN_STR1", HOFFSET(comp4_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid4_comp, "FIXLEN_STR2", HOFFSET(comp4_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid4_comp, "VLEN_STR_ARRAY1", HOFFSET(comp4_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid4_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp4_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid4_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp4_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid4_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp4_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 5 */
  H5Tinsert(tid5_comp, "VLEN_STR1", HOFFSET(comp5_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid5_comp, "VLEN_STR2", HOFFSET(comp5_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid5_comp, "FIXLEN_STR1", HOFFSET(comp5_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid5_comp, "FIXLEN_STR2", HOFFSET(comp5_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid5_comp, "VLEN_STR_ARRAY1", HOFFSET(comp5_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid5_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp5_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid5_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp5_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid5_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp5_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 6 */
  H5Tinsert(tid6_comp, "VLEN_STR1", HOFFSET(comp6_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid6_comp, "VLEN_STR2", HOFFSET(comp6_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid6_comp, "FIXLEN_STR1", HOFFSET(comp6_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid6_comp, "FIXLEN_STR2", HOFFSET(comp6_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid6_comp, "VLEN_STR_ARRAY1", HOFFSET(comp6_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid6_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp6_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid6_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp6_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid6_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp6_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 7 */
  H5Tinsert(tid7_comp, "VLEN_STR1", HOFFSET(comp7_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid7_comp, "VLEN_STR2", HOFFSET(comp7_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid7_comp, "FIXLEN_STR1", HOFFSET(comp7_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid7_comp, "FIXLEN_STR2", HOFFSET(comp7_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid7_comp, "VLEN_STR_ARRAY1", HOFFSET(comp7_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid7_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp7_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid7_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp7_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid7_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp7_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 8 */
  H5Tinsert(tid8_comp, "VLEN_STR1", HOFFSET(comp8_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid8_comp, "VLEN_STR2", HOFFSET(comp8_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid8_comp, "FIXLEN_STR1", HOFFSET(comp8_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid8_comp, "FIXLEN_STR2", HOFFSET(comp8_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid8_comp, "VLEN_STR_ARRAY1", HOFFSET(comp8_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid8_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp8_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid8_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp8_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid8_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp8_t, str_fixlen_array_again), tid_fixlen_str_array);

  /* compound 9 */
  H5Tinsert(tid9_comp, "VLEN_STR1", HOFFSET(comp9_t, str_vlen), tid_vlen_str);
  H5Tinsert(tid9_comp, "VLEN_STR2", HOFFSET(comp9_t, str_vlen_repeat),
            tid_vlen_str);
  H5Tinsert(tid9_comp, "FIXLEN_STR1", HOFFSET(comp9_t, str_fixlen),
            tid_fixlen_str);
  H5Tinsert(tid9_comp, "FIXLEN_STR2", HOFFSET(comp9_t, str_fixlen_repeat),
            tid_fixlen_str);
  H5Tinsert(tid9_comp, "VLEN_STR_ARRAY1", HOFFSET(comp9_t, str_array_vlen),
            tid_vlen_str_array);
  H5Tinsert(tid9_comp, "VLEN_STR_ARRAY2",
            HOFFSET(comp9_t, str_vlen_array_again), tid_vlen_str_array);
  H5Tinsert(tid9_comp, "FIXLEN_STR_ARRAY1", HOFFSET(comp9_t, str_array_fixlen),
            tid_fixlen_str_array);
  H5Tinsert(tid9_comp, "FIXLEN_STR_ARRAY2",
            HOFFSET(comp9_t, str_fixlen_array_again), tid_fixlen_str_array);
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset1", tid1_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid1_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp1_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset2", tid2_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid2_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp2_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset3", tid3_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid3_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp3_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset4", tid4_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid4_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp4_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset5", tid5_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid5_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp5_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset6", tid6_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid6_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp6_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset7", tid7_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid7_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp7_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset8", tid8_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  status =
      H5Dwrite(did_comp, tid8_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp8_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
  ...
  did_comp = H5Dcreate2(gid, "Compound_dset9", tid9_comp, sid_comp, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);

  /* obj references */
  status = H5Rcreate(&(comp9_buf.objref1), gid, "Compound_dset2", H5R_OBJECT,
                     (hid_t)-1);
  status = H5Rcreate(&(comp9_buf.objref2), gid, "Compound_dset3", H5R_OBJECT,
                     (hid_t)-1);
  status = H5Rcreate(&(comp9_buf.objref3), gid, "Compound_dset4", H5R_OBJECT,
                     (hid_t)-1);

  status =
      H5Dwrite(did_comp, tid9_comp, H5S_ALL, H5S_ALL, H5P_DEFAULT, &comp9_buf);
  if (status < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", fname1);
    status = FAIL;
    goto out;
...
static void test_comps_array(const char *fname, const char *dset,
   ...
   */
  tid_attr =
      H5Acreate2(fid, attr, tid_cmpd1, sid_dset, H5P_DEFAULT, H5P_DEFAULT);
  assert(tid_attr > 0);
...
static void test_comps_vlen(const char *fname, const char *dset,
  ...
  ret =
      H5Tinsert(tid_cmpd1, "vlen_cmpd1", HOFFSET(cmpd1_t, vl), tid_cmpd1_vlen);
  assert(ret >= 0);
...
static void test_comps_vlen_arry(const char *fname, const char *dset,
  ...
  for (i = 0; i < SDIM_DSET; i++) {
    ...
    for (j = 0; j < (i + 1); j++) {
      ...
      for (k = 0; k < SDIM_CMPD_ARRAY; k++) {
        /* compound 3 data */
        ((cmpd2_t *)(wdata[i].vl.p))[j].cmpd3[k].i3 =
            (int)((float)k * 10.5F) + diff;
        ((cmpd2_t *)(wdata[i].vl.p))[j].cmpd3[k].f3 =
            (float)k * 10.5F + (float)diff;
      } /* end for */
    }   /* end for */
  }     /* end for */
  ...
  ret = H5Tinsert(tid_cmpd2, "array_cmpd2", HOFFSET(cmpd2_t, cmpd3),
                  tid_cmpd2_arry);
  assert(ret >= 0);
...
static void test_data_nocomparables(const char *fname, int make_diffs) {
  ...
  /*  dset1 */
  if ((did1 = H5Dcreate2(gid1, "dset1", tid_dset1, sid1, H5P_DEFAULT,
  ...
  if (H5Dwrite(did1, tid_dset1, H5S_ALL, H5S_ALL, H5P_DEFAULT, dset_data_ptr1) <
      0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", "dset1");
    status = FAIL;
    goto out;
  ...
  /*  dset2 */
  status =
      write_dset(gid1, 1, dims1_1, "dset2", H5T_NATIVE_INT, dset_data_ptr2);
  if (status == FAIL) {
    fprintf(stderr, "Error: %s> write_dset failed\n", fname);
    goto out;
  }
  ...
  /* ---------
   *  dset1 */
  if ((did2 = H5Dcreate2(gid2, "dset1", H5T_NATIVE_INT, sid1, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    fprintf(stderr, "Error: %s> H5Dcreate2 failed.\n", "dset1");
    status = FAIL;
    goto out;
  ...
  if (H5Dwrite(did2, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               dset_data_ptr3) < 0) {
    fprintf(stderr, "Error: %s> H5Dwrite failed.\n", "dset1");
    status = FAIL;
    goto out;
  ...
  write_attr(did2, rank_attr, (hsize_t *)attr3_dim_ptr, "attr3", H5T_NATIVE_INT,
             attr_data_ptr3);
   ...
   * dset2 */
  status =
      write_dset(gid2, 1, dims1_1, "dset2", H5T_NATIVE_INT, dset_data_ptr3);
  if (status == FAIL) {
    fprintf(stderr, "Error: %s> write_dset failed\n", fname);
    goto out;
  }
...
static void test_objs_nocomparables(const char *fname1, const char *fname2) {
   ...
   *------------------------------------------------------------------------*/
  /* parent group */
  if ((topgid1 = H5Gcreate2(fid1, "diffobjtypes", H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    PROGRAM_ERROR;
  ...
  /* group */
  if ((gid1 = H5Gcreate2(topgid1, "obj2", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    PROGRAM_ERROR;
  ...
  if ((tid1 = H5Tcopy(H5T_NATIVE_INT)) < 0)
    PROGRAM_ERROR;
  if (H5Tcommit2(topgid1, "obj3", tid1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    PROGRAM_ERROR;
   ...
   *------------------------------------------------------------------------*/
  /* parent group */
  if ((topgid2 = H5Gcreate2(fid2, "diffobjtypes", H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    PROGRAM_ERROR;

  /* group */
  if ((gid2 = H5Gcreate2(topgid2, "obj1", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    PROGRAM_ERROR;
  ...
  if ((tid2 = H5Tcopy(H5T_NATIVE_INT)) < 0)
    PROGRAM_ERROR;
  if (H5Tcommit2(topgid2, "obj2", tid2, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    PROGRAM_ERROR;
...
static void test_objs_strings(const char *fname1, const char *fname2) {
  ...
  hsize_t dims1[] = {3, 4};
  char string1A[12][3] = {"s1", "s2", "s3", "s4", "s5", "s6",
                          "s",  "s",  "s9", "s0", "s1", "s2"};
  char string1B[12][3] = {"s1", "s2", "s3", "s4", "s",  "s",
                          "s7", "s8", "s9", "s0", "s1", "s2"};
  ...
  hsize_t dims3[] = {27};
  char string3A[27][6] = {"abcd0", "abcd1", "abcd2", "abcd3", "abcd4", "abcd5",
                          "abcd6", "abcd7", "abcd8", "abcd9", "abcd0", "abcd1",
                          "abd2",  "abc3",  "bcd4",  "acd5",  "abcd6", "abcd7",
                          "abcd8", "abcd9", "abcd0", "abcd1", "abcd2", "abcd3",
                          "abc4",  "abc5",  "abc6"};
  char string3B[27][6] = {"abcd0", "abcd1", "abcd2", "abcd3", "abcd4", "abcd5",
                          "abcd6", "abcd7", "abcd8", "abcd9", "abcd0", "abcd1",
                          "abcd2", "abcd3", "abcd4", "abcd5", "abd6",  "abc7",
                          "bcd8",  "acd9",  "abcd0", "abcd1", "abcd2", "abcd3",
                          "abd4",  "abd5",  "abd6"};
  ...
  dataset = H5Dcreate2(fid1, "/string1", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  H5Dwrite(dataset, m_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, string1A);
  H5Dclose(dataset);
  dataset = H5Dcreate2(fid2, "/string1", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  ...
  dataset = H5Dcreate2(fid1, "/string2", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  H5Dwrite(dataset, m_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, string2A);
  H5Dclose(dataset);
  dataset = H5Dcreate2(fid2, "/string2", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  ...
  dataset = H5Dcreate2(fid1, "/string3", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  H5Dwrite(dataset, m_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, string3A);
  H5Dclose(dataset);
  dataset = H5Dcreate2(fid2, "/string3", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  ...
  dataset = H5Dcreate2(fid1, "/string4", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  H5Dwrite(dataset, m_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, string4A);
  H5Dclose(dataset);
  dataset = H5Dcreate2(fid2, "/string4", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  ...
  m_type = mkstr(21, H5T_STR_NULLTERM);
  dataset = H5Dcreate2(fid1, "/string5", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  H5Dwrite(dataset, m_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, string4A);
  H5Dclose(dataset);
  dataset = H5Dcreate2(fid2, "/string5", f_type, space, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  H5Dwrite(dataset, m_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, string4B);
  H5Tclose(m_type);
  H5Tclose(f_type);
...
write_attr_strings(hid_t loc_id, const char *dset_name, hid_t fid,
  ...
  hsize_t dims2[2] = {3, 2};
  char buf12[3][2][STR_SIZE] = {
      {"ab", "cd"}, {"ef", "gh"}, {"ij", "kl"}}; /* string */
  char *buf12a[3][2];                            /* VL string */
  char buf22[3][2] = {{1, 2}, {3, 4}, {5, 6}};   /* bitfield, opaque */
  s_t buf32[6] = {{1, 2.0}, {3, 4.0},  {5, 6.0},
                  {7, 8.0}, {9, 10.0}, {11, 12.0}}; /* compound */
...
static void write_dset_in(hid_t loc_id, const char *dset_name, hid_t fid,
  ...
  /* create a dataset */
  did = H5Dcreate2(loc_id, "scalar", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
...

  {
    ...
    ldid = H5Dcreate2(loc_id, "arrayd", ltid, lsid, H5P_DEFAULT, H5P_DEFAULT,
                      H5P_DEFAULT);
  ...
  sid = H5Screate_simple(2, dims2, NULL);
  did = H5Dcreate2(loc_id, "integer2D", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
...
static void gen_datareg(hid_t fid,
                        int make_diffs /* flag to modify data buffers */) {
  /* data dataset */
  ...
  did1 = H5Dcreate2(fid, "dsetref", H5T_NATIVE_INT, sid1, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* write */
  status =
      H5Dwrite(did2, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT, rbuf);
  assert(status >= 0);
 ...
 *-------------------------------------------------------------------------
 */
static int test_hyperslab(const char *fname,
                          int make_diffs /* flag to modify data buffers */) {
  hid_t did = H5I_INVALID_HID;
  hid_t fid = H5I_INVALID_HID;
  ...
  if ((f_sid = H5Screate_simple(1, dims, NULL)) < 0)
    goto out;
  if ((did = H5Dcreate2(fid, "big", H5T_NATIVE_CHAR, f_sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    goto out;
...
static herr_t write_dset(hid_t loc_id, int rank, hsize_t *dims,
  ...
  /* create the dataset */
  if ((did = H5Dcreate2(loc_id, name, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    PROGRAM_ERROR;
