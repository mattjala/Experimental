 ...
 * Function: gen_non()
 *
 * Create a file with SWMR write+non-latest-format--this will result in v3
 * superblock+latest version support: 1) 1 chunked dataset with extensible array
 * chunk indexing type (without data) 2) 1 chunked dataset with version 2 B-tree
 * chunk indexing type (with data) Re-open the file with write+non-latest-format
 * and create: 3) 1 chunked dataset with version 2 B-tree chunk indexing type
 * (without data) 4) 1 chunked dataset with extensible array indexing type (with
 * data) 5) 1 compact and 1 contiguous datasets
 */
static void gen_non(const char *fname) {
  ...
  if ((fid = H5Fcreate(fname, H5F_ACC_TRUNC | H5F_ACC_SWMR_WRITE, fcpl,
                       H5P_DEFAULT)) < 0)
    goto error;
  ...
  /*
   * Create a chunked dataset with extensible array chunk indexing type (without
   * data)
   */
  ...
  /* Create the dataset */
  if ((did1 = H5Dcreate2(fid, DSET_NDATA_EA, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    goto error;
  ...
  /*
   * Create a chunked dataset with version 2 B-tree chunk indexing type (with
   * data)
   */
  ...
  /* Create the dataset */
  if ((did1 = H5Dcreate2(gid, DSET_BT2, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    goto error;
  ...
  /* Create the dataset */
  if ((did1 = H5Dcreate2(fid, DSET_NDATA_BT2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    goto error;
  ...
  /*
   * Create a dataset with extensible array chunk indexing type (with data) in
   * the group
   */
  ...
  /* Create the dataset */
  if ((did2 = H5Dcreate2(gid, DSET_EA, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    goto error;
  ...
  /* Create the dataset */
  if ((did1 = H5Dcreate2(gid, DSET_COMPACT, H5T_NATIVE_INT, sid, H5P_DEFAULT,
  ...
  /* Create the dataset */
  if ((did2 = H5Dcreate2(fid, DSET_CONTIGUOUS, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto error;
  /* Write to the dataset */
  if (H5Dwrite(did2, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
 ...
 * Function: gen_edge()
 *
 * Create a file with write+latest-format--this will result in v3
 * superblock+latest version support: A dataset: chunked, filtered,
 * H5D_CHUNK_DONT_FILTER_PARTIAL_CHUNKS enabled (i.e. the dataset does not
 * filter partial edge chunks)
 */
static void gen_edge(const char *fname) {
  hid_t fid = H5I_INVALID_HID;  /* file id */
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DSET_EDGE, H5T_NATIVE_FLOAT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    goto error;
 ...
 */
static void gen_err_level(const char *fname) {
  hid_t fid = H5I_INVALID_HID;  /* file ID */
  hid_t fapl = H5I_INVALID_HID; /* file access property list */
  hid_t fcpl = H5I_INVALID_HID; /* file creation property list */
  hid_t sid = H5I_INVALID_HID;  /* dataspace id */
  ...
  hsize_t max_dims[2] = {1,
                         H5S_UNLIMITED}; /* dataset maximum dimension sizes */
  ...
  if (H5Pset_chunk(dcpl, 2, chunk_dims) < 0)
    goto error;
  if ((did = H5Dcreate2(fid, DSET_ERR, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (n = 0; n < NUM; n++) {
    ...
    if ((H5Sselect_hyperslab(fsid, H5S_SELECT_SET, start, NULL, count, NULL)) <
        0)
      goto error;
 ...
 *    2) 2 chunked datasets with extensible array chunk indexing type
 * (with/without data) 3) 2 chunked datasets with version 2 B-tree chunk
 * indexing type (with/without data) 4) 2 chunked datasets with fixed array
 * chunk indexing type (with/without data) 5) 2 chunked datasets with implicit
 * array chunk indexing type (with/without data) It will create the file
 * with/without messages in the superblock extension depending on the parameter
 * "what".
 */
static void gen_ext(const char *fname, unsigned new_format, unsigned what) {
  hid_t fid = H5I_INVALID_HID;  /* file id */
  hid_t fapl = H5I_INVALID_HID; /* file access property list */
  hid_t fcpl = H5I_INVALID_HID; /* file creation property list */
  hid_t gid = H5I_INVALID_HID;  /* group id */
  ...
  case 4:
    H5Pset_istore_k(fcpl, ISTORE_IK);
    H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, TRUE,
                               (hsize_t)1);
    break;
  case 5:
    H5Pset_shared_mesg_nindexes(fcpl, 4);
  ...
  case 6:
    H5Pset_istore_k(fcpl, ISTORE_IK);
    H5Pset_shared_mesg_nindexes(fcpl, 4);
    H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_NONE, FALSE,
                               (hsize_t)1);
    break;
  default:
    break;
  ...
  /* Create the dataset */
  if ((did1 = H5Dcreate2(fid, DSET_CONTIGUOUS, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto error;
  ...
  if ((did1 = H5Dcreate2(gid, DSET_NDATA_EA, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    goto error;

  if ((did2 = H5Dcreate2(fid, DSET_EA, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    goto error;
  ...
  if ((did2 = H5Dcreate2(gid, DSET_BT2, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    goto error;
  ...
  /* Create the datasets */
  if ((did1 = H5Dcreate2(fid, DSET_FA, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    goto error;

  if ((did2 = H5Dcreate2(gid, DSET_NDATA_FA, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    goto error;
  ...
  /* Create the 2 datasets */
  if ((did1 = H5Dcreate2(fid, DSET_NONE, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    goto error;

  if ((did2 = H5Dcreate2(gid, DSET_NDATA_NONE, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    goto error;
...
int main(void) {
  ...
  /* Generate old/new format file with/without messages in the superblock
   * extension */
