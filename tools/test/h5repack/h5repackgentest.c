 ...
 */
static int make_dataset(hid_t file_id, const char *dset_name, hid_t mem_type_id,
                        hid_t space_id, hid_t dcpl_id, void *wdata) {
  hid_t dset_id = H5I_INVALID_HID;
  int ret_value = 0;

  dset_id = H5Dcreate2(file_id, dset_name, mem_type_id, space_id, H5P_DEFAULT,
                       dcpl_id, H5P_DEFAULT);
 ...
 * Returns 0 on success, -1 on failure.
 */
static int set_dcpl_external_list(hid_t dcpl, const char *filename,
                                  unsigned n_elts_per_file,
                                  unsigned n_elts_total, hsize_t elt_size) {
  ...
  for (i = 0; i < n_external_files; i++) {
    if (HDsnprintf(name, MAX_NAME_SIZE, "%s_ex-%u.dat", filename, i) >=
        MAX_NAME_SIZE)
      return -1;
 ...
 * Returns 0 on success, -1 on failure.
 */
static int make_file(const char *basename, struct external_def *ext,
  ...
  if (HDsnprintf(filename, MAX_NAME_SIZE, "%s%s.h5", basename,
                 (NULL != ext) ? "_ex" : "") >= MAX_NAME_SIZE)
    H5REPACKGENTEST_OOPS;
  ...
  if (NULL != ext) {
    ...
    if (set_dcpl_external_list(dcpl_id, basename, ext->n_elts_per_file,
                               ext->n_elts_total, ext->type_size) < 0)
      H5REPACKGENTEST_OOPS;
  }
 ...
 * Returns 0 on success, -1 on failure.
 */
static int generate_int32le_1d(hbool_t external) {
  int32_t wdata[12];
  hsize_t dims[] = {12};
 ...
 * Returns 0 on success, -1 on failure.
 */
static int generate_int32le_2d(hbool_t external) {
  int32_t wdata[64];
  hsize_t dims[] = {8, 8};
  struct external_def *def_ptr = NULL;
 ...
 * Returns 0 on success, -1 on failure.
 */
static int generate_int32le_3d(hbool_t external) {
  hsize_t dims[] = {8, 8, 8};
 ...
 * Returns 0 on success, -1 on failure.
 */
static int generate_uint8be(hbool_t external) {
  hsize_t dims[] = {4, 8, 8};
 ...
 * Returns 0 on success, -1 on failure.
 */
static int generate_f32le(hbool_t external) {
  hsize_t dims[] = {12, 6};
