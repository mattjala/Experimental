...
int main(void) {
  ...
  set =
      H5Dcreate2(fil, setname, cmp, spc, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
out:
  if (error < 0) {
    ...
    printf("%14s (%2d) %6s = %s\n", mname ? mname : "(null)",
           (int)H5Tget_member_offset(fix, 0), string5,
    ...
    printf("Data comparison:\n"
           ...
           (double)fok[0], (double)fptr[0], (double)fok[1], (double)fptr[1]);
    if (mname)
      H5free_memory(mname);
    ...
    printf("%14s (%2d) %6f = %f\n"
           ...
           (double)fnok[0], (double)fptr[0], (double)fnok[1], (double)fptr[1]);
    if (mname)
      H5free_memory(mname);
    ...
    printf("\n"
           ...
           "                    %6f = %f\n",
           (double)fok[0], (double)fptr[0], (double)fok[1], (double)fptr[1],
           (double)fnok[0], (double)fptr[2], (double)fnok[1], (double)fptr[3]);
