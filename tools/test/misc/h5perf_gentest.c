...
herr_t create_perf_test_file(const char *fname, int ngrps, int ndsets,
                             int nattrs, hsize_t nrows, hsize_t dim0,
                             hsize_t chunk, int vlen, int compressed,
                             int latest);
...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-f") == 0)
      ...
      printf("\t-r N:\tnumber of rows in the large compound dataset (default: "
             "%d).\n",
             NROWS);
      ...
      printf("Example:\n");
      printf("\t./a.out -f test.h5 -g 10000 -d 5000 -a 500 -r 10000 -s 200 -c "
             "20 -v 40 -l -z\n\n");
      exit(0);
    }
  }
...
herr_t create_perf_test_file(const char *fname, int ngrps, int ndsets,
  ...
  hsize_t dims[1] = {dim0}, dims2d[2] = {dim0, (dim0 / 4 + 1)},
          dims_array[1] = {FIXED_LEN}, dim1[1] = {2};
  ...
  hid_t types[NTYPES] = {H5T_NATIVE_INT,    H5T_NATIVE_UINT64, H5T_NATIVE_FLOAT,
                         H5T_NATIVE_DOUBLE, tid_str,           tid_enum,
                         tid_array_f,       tid_vlen_i,        tid_vlen_s};
  hsize_t coords[4][2] = {{0, 1}, {3, 5}, {1, 0}, {2, 4}}, start = 0,
          stride = 1, count = 1;
  ...
  did =
      H5Dcreate(fid, name, cmp_tid, sid_large, H5P_DEFAULT, dcpl, H5P_DEFAULT);
  ...
  for (j = 0; j < ndsets; j += 12) {
    ...
    did = H5Dcreate(gid1, name, H5T_STD_I32LE, sid_null, H5P_DEFAULT,
    ...
    did = H5Dcreate(gid1, name, H5T_NATIVE_INT, sid_scalar, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, tid_vlen_s, sid_scalar, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, tid_array_f, sid_1d, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
    ...
    did =
        H5Dcreate(gid1, name, tid_str, sid_1d, H5P_DEFAULT, dcpl, H5P_DEFAULT);
    ...
    did =
        H5Dcreate(gid1, name, cmp_tid, sid_1d, H5P_DEFAULT, dcpl, H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, H5T_NATIVE_DOUBLE, sid_2d, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, H5T_NATIVE_INT, sid_1d, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, tid_vlen_i, sid_1d, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, tid_vlen_s, sid_1d, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, H5T_STD_REF_OBJ, sid_2, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
    ...
    did = H5Dcreate(gid1, name, H5T_STD_REF_DSETREG, sid_2, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
    H5Dwrite(did, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
             buf_reg_ref);
    if (!j)
      add_attrs(did, j);
    H5Dclose(did);
