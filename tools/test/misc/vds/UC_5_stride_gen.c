...
int main(void) {
  ...
  if ((vds_dcplid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    UC_ERROR
  if (H5Pset_fill_value(vds_dcplid, UC_5_VDS_DATATYPE, &UC_5_VDS_FILL_VALUE) <
      0)
    UC_ERROR
  ...
  for (i = 0; i < UC_5_N_SOURCES; i++) {
    ...
    if ((fid = H5Fcreate(UC_5_FILE_NAMES[i], H5F_ACC_TRUNC, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
      UC_ERROR
    if ((src_sid = H5Screate_simple(RANK, UC_5_SOURCE_DIMS,
                                    UC_5_SOURCE_MAX_DIMS)) < 0)
      UC_ERROR
    if ((did = H5Dcreate2(fid, UC_5_SOURCE_DSET_NAME, UC_5_SOURCE_DATATYPE,
    ...
    for (j = 0; j < UC_5_SRC_PLANES; j++) {
      ...
      start[1] = 0;
      start[2] = 0;
      if (H5Sselect_hyperslab(fsid, H5S_SELECT_SET, start, NULL, UC_5_PLANE,
                              NULL) < 0)
        UC_ERROR
  ...
  /* file */
  if ((fid = H5Fcreate(UC_5_VDS_FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT,
                       H5P_DEFAULT)) < 0)
    UC_ERROR
