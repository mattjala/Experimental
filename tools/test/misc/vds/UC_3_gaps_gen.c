...
static herr_t create_3_1_vds(void) {
  ...
  if ((vds_dcplid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    UC_ERROR
  if (H5Pset_fill_value(vds_dcplid, UC_31_VDS_DATATYPE, &UC_3_VDS_FILL_VALUE) <
      0)
    UC_ERROR

  /* Create VDS dataspace */
  if ((vds_sid = H5Screate_simple(RANK, UC_31_VDS_DIMS, UC_31_VDS_MAX_DIMS)) <
      0)
    UC_ERROR
  ...
  for (i = 0; i < UC_1_N_SOURCES; i++) {
    ...
    if (H5Sselect_hyperslab(src_sid, H5S_SELECT_SET, start, NULL,
      ...
      UC_ERROR
    if (H5Sselect_hyperslab(vds_sid, H5S_SELECT_SET, position, NULL,
                            UC_1_MAX_DIMS[i], NULL) < 0)
      UC_ERROR
  ...
  /* file */
  if ((fid = H5Fcreate(UC_31_VDS_FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT,
                       H5P_DEFAULT)) < 0)
    UC_ERROR
...
static herr_t create_3_2_vds(void) {
  ...
  if ((vds_dcplid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    UC_ERROR
  if (H5Pset_fill_value(vds_dcplid, UC_32_VDS_DATATYPE, &UC_3_VDS_FILL_VALUE) <
      0)
    UC_ERROR

  /* Create VDS dataspace */
  if ((vds_sid = H5Screate_simple(RANK, UC_32_VDS_DIMS, UC_32_VDS_MAX_DIMS)) <
      0)
    UC_ERROR
  ...
  for (i = 0; i < UC_2_N_SOURCES; i++) {
    ...
    if (H5Sselect_hyperslab(src_sid, H5S_SELECT_SET, start, NULL,
      ...
      UC_ERROR
    if (H5Sselect_hyperslab(vds_sid, H5S_SELECT_SET, UC_32_POSITIONS[i], NULL,
                            UC_2_MAX_DIMS[i], NULL) < 0)
      UC_ERROR
  ...
  /* file */
  if ((fid = H5Fcreate(UC_32_VDS_FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT,
                       H5P_DEFAULT)) < 0)
    UC_ERROR
