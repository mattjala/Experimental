...
static hsize_t UC_1_VDS_MAX_DIMS[RANK] = {UC_1_N_MAX_PLANES, UC_1_FULL_HEIGHT,
                                          UC_1_WIDTH};

/* Planes */
static hsize_t UC_1_PLANES[UC_1_N_SOURCES][RANK] = {
    {1, UC_1_SM_HEIGHT, UC_1_WIDTH}, {1, UC_1_LG_HEIGHT, UC_1_WIDTH},
    {1, UC_1_SM_HEIGHT, UC_1_WIDTH}, {1, UC_1_LG_HEIGHT, UC_1_WIDTH},
    {1, UC_1_SM_HEIGHT, UC_1_WIDTH}, {1, UC_1_LG_HEIGHT, UC_1_WIDTH}};
...
int main(void) {
  ...
  if ((vds_dcplid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    UC_ERROR
  if (H5Pset_fill_value(vds_dcplid, UC_1_VDS_DATATYPE, &UC_1_VDS_FILL_VALUE) <
      0)
    UC_ERROR
  ...
  for (i = 0; i < UC_1_N_SOURCES; i++) {
    ...
    if ((fid = H5Fcreate(UC_1_FILE_NAMES[i], H5F_ACC_TRUNC, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
      UC_ERROR
    if ((src_sid = H5Screate_simple(RANK, UC_1_DIMS[i], UC_1_MAX_DIMS[i])) < 0)
      UC_ERROR
    if ((did = H5Dcreate2(fid, UC_1_SOURCE_DSET_NAME, UC_1_SOURCE_DATATYPE,
    ...
    for (j = 0; j < n_planes; j++) {
      ...
      start[1] = 0;
      start[2] = 0;
      if (H5Sselect_hyperslab(fsid, H5S_SELECT_SET, start, NULL, UC_1_PLANES[i],
                              NULL) < 0)
        UC_ERROR
  ...
  /* file */
  if ((fid = H5Fcreate(UC_1_VDS_FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT,
                       H5P_DEFAULT)) < 0)
    UC_ERROR
