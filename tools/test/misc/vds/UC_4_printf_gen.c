...
int main(void) {
  ...
  if ((src_sid =
           H5Screate_simple(RANK, UC_4_SOURCE_DIMS, UC_4_SOURCE_MAX_DIMS)) < 0)
    UC_ERROR
  ...
  for (i = 0; i < UC_4_N_SOURCES; i++) {
    ...
    if ((fid = H5Fcreate(UC_4_FILE_NAMES[i], H5F_ACC_TRUNC, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
      UC_ERROR
    if ((did = H5Dcreate2(fid, UC_4_SOURCE_DSET_NAME, UC_4_SOURCE_DATATYPE,
                          src_sid, H5P_DEFAULT, src_dcplid, H5P_DEFAULT)) < 0)
      UC_ERROR
    ...
    for (j = 0; j < UC_4_SRC_PLANES; j++) {
      ...
      start[1] = 0;
      start[2] = 0;
      if (H5Sselect_hyperslab(fsid, H5S_SELECT_SET, start, NULL, UC_4_PLANE,
                              NULL) < 0)
        UC_ERROR
  ...
  /* Create file */
  if ((fid = H5Fcreate(UC_4_VDS_FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT,
                       H5P_DEFAULT)) < 0)
    UC_ERROR
  ...
  if ((vds_dcplid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    UC_ERROR
  if (H5Pset_fill_value(vds_dcplid, UC_4_VDS_DATATYPE, &UC_4_VDS_FILL_VALUE) <
      0)
    UC_ERROR
  ...
  start[1] = 0;
  start[2] = 0;
  if (H5Sselect_hyperslab(vds_sid, H5S_SELECT_SET, start, UC_4_SOURCE_MAX_DIMS,
                          count, UC_4_SOURCE_MAX_DIMS) < 0)
    UC_ERROR
