...
static void usage(void) {
  ...
  fprintf(
      stdout,
      "     -1...-9                    Level of compression, from 1 to 9\n");
  fprintf(stdout, "     -c P, --compressability=P  Percentage of "
                  "compressability of the random\n");
  fprintf(stdout, "                                data you want [default: 0]");
  fprintf(stdout, "     -s S, --file-size=S        Maximum size of "
                  "uncompressed file [default: 64M]\n");
  fprintf(
      stdout,
      "     -B S, --max-buffer_size=S  Maximum size of buffer [default: 1M]\n");
  fprintf(stdout, "     -b S, --min-buffer_size=S  Minimum size of buffer "
                  "[default: 128K]\n");
  fprintf(stdout, "     -p D, --prefix=D           The directory prefix to "
                  "place the file\n");
  fprintf(
      stdout,
      "     -r, --random-test          Use random data to write to the file\n");
  fprintf(stdout, "                                [default: no]\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "  D  - a directory which exists\n");
  fprintf(stdout, "  P  - a number between 0 and 100\n");
  fprintf(stdout, "  S  - is a size specifier, an integer >=0 followed by a "
                  "size indicator:\n");
  fprintf(stdout, "\n");
...
static void do_write_test(unsigned long file_size, unsigned long min_buf_size,
...
int main(int argc, char *argv[]) {
  ...
  if (min_buf_size > max_buf_size)
    error("minimum buffer size (%d) exceeds maximum buffer size (%d)",
...
int main(void) {
  fprintf(stdout,
          "No compression IO performance because zlib was not configured\n");
  return EXIT_SUCCESS;
}
