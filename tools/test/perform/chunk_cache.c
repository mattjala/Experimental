...
/*
 *  Purpose: check the performance of chunk cache in these two cases
 * (HDFFV-10601):
 *          1. partial chunks exist along any dimension.
 *          2. number of slots in chunk cache is smaller than the number of
 * chunks in the fastest-growing dimension.
 */
#include "H5private.h"
#include "h5test.h"
#include "hdf5.h"
...
static size_t counter(unsigned flags, size_t cd_nelmts,
...
static int create_dset1(hid_t file) {
  ...
  /* Set the dummy filter simply for counting the number of bytes being read
   * into the memory */
...
static int check_partial_chunks_perf(hid_t file) {
  ...
  for (i = 0; i < DSET1_DIM1; i++) {
    ...
    if (H5Dread(dataset, H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT,
                rdata) < 0)
      goto error;
  }
  ...
  if ((end_t - start_t) > 0.0)
    printf("1. Partial chunks: total read time is %lf; number of bytes being "
           "read from file is %zu\n",
           (end_t - start_t), nbytes_global);
  else
    printf("1. Partial chunks: no total read time because timer is not "
           "available; number of bytes being "
           "read from file is %zu\n",
           nbytes_global);
...
static int check_hash_value_perf(hid_t file) {
  ...
  if ((end_t - start_t) > 0.0)
    printf("2. Hash value: total read time is %lf; number of bytes being read "
           "from file is %zu\n",
           (end_t - start_t), nbytes_global);
  else
    printf("2. Hash value: no total read time because timer is not available; "
