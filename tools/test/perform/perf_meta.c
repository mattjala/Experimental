...
static const char *FILENAME[] = {"meta_perf_1", "meta_perf_2", "meta_perf_3",
                                 NULL};
...
static herr_t create_attrs_1(void) {
  ...
  for (i = 0; i < NUM_DSETS; i++) {
    ...
    for (j = 0; j < NUM_ATTRS; j++) {
      ...
      if ((attr = H5Acreate2(dataset, attr_name, H5T_NATIVE_DOUBLE, small_space,
  ...
  if (facc_type == FACC_DEFAULT ||
      (facc_type != FACC_DEFAULT && MAINPROCESS)) /* only process 0 reports */
#endif                                            /*H5_HAVE_PARALLEL*/
  {
    ...
    fprintf(stderr,
            "1.  Create %d attributes for each of %d existing datasets\n",
            NUM_ATTRS, NUM_DSETS);
...
static herr_t create_attrs_3(void) {
  ...
  for (i = 0; i < loop_num; i++) {
    for (j = 0; j < NUM_DSETS; j++) {
      ...
      for (k = 0; k < BATCH_ATTRS; k++) {
        HDsnprintf(attr_name, sizeof(attr_name),
                   "some attrs for each dset %d %d", i, k);
        attr_t.start = retrieve_time();
...
#endif /*H5_HAVE_PARALLEL*/
  {
    ...
    /* Print out the performance result */
    fprintf(stderr,
            "3.  Create %d attributes for each of %d existing datasets for %d "
            "times\n",
