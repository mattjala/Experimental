...
int create_file(hid_t fapl_id) {
   ...
   * creation properties.
   */
  if ((dataset = H5Dcreate2(file, DIRECT_UNCOMPRESSED_DSET, H5T_NATIVE_INT,
  ...
  if ((dataset = H5Dcreate2(file, REG_NO_COMPRESS_DSET, H5T_NATIVE_INT,
                            dataspace, H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((dataset = H5Dcreate2(file, DIRECT_COMPRESSED_DSET, H5T_NATIVE_INT,
                            dataspace, H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((dataset = H5Dcreate2(file, REG_COMPRESSED_DSET, H5T_NATIVE_INT,
                            dataspace, H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < NX; i++) {
    p = direct_buf[i] =
        (unsigned int *)malloc(CHUNK_NY * CHUNK_NZ * sizeof(unsigned int));
...
int test_direct_write_uncompressed_data(hid_t fapl_id) {
  ...
  for (i = 0; i < NX; i++) {
    status = H5Dwrite_chunk(dataset, dxpl, filter_mask, offset,
                            CHUNK_NY * CHUNK_NZ * sizeof(unsigned int),
                            direct_buf[i]);
    (offset[0])++;
  }
...
int test_direct_write_compressed_data(hid_t fapl_id) {
   ...
   * dataset, using the direct writing function.     */
  for (i = 0; i < NX; i++) {
    status = H5Dwrite_chunk(dataset, dxpl, filter_mask, offset, data_size[i],
                            outbuf[i]);
    (offset[0])++;
  }
