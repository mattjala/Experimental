...
static size_t counter(unsigned H5_ATTR_UNUSED flags, size_t cd_nelmts,
...
static void create_dataset(void) {
  ...
  /* The dataset */
  dset = H5Dcreate2(file, "dset", H5T_NATIVE_SCHAR, space, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
  assert(dset >= 0);
...
static double test_rowmaj(int op, size_t cache_size, size_t io_size) {
...
#endif
  H5Pset_cache(fapl_g, mdc_nelmts, rdcc_nelmts, cache_size * SQUARE(CH_SIZE),
               w0);
  ...
  for (i = 0; i < CH_SIZE * DS_SIZE; i += io_size) {
    ...
    for (j = 0; j < CH_SIZE * DS_SIZE; j += io_size) {
      ...
      H5Sselect_hyperslab(file_space, H5S_SELECT_SET, hs_offset, NULL, hs_size,
                          NULL);

      if (READ == op) {
        H5Dread(dset, H5T_NATIVE_SCHAR, mem_space, file_space, H5P_DEFAULT,
                buf);
      } else {
        H5Dwrite(dset, H5T_NATIVE_SCHAR, mem_space, file_space, H5P_DEFAULT,
                 buf);
      }
      H5Sclose(mem_space);
    }
 ...
 *-------------------------------------------------------------------------
 */
static double test_diag(int op, size_t cache_size, size_t io_size,
                        size_t offset) {
  hid_t file, dset, mem_space, file_space;
  hsize_t i, hs_size[2];
  hsize_t nio = 0;
...
int main(void) {
  ...
  for (cache_size = RM_CACHE_STRT; cache_size <= RM_CACHE_END;
  ...
  for (cache_size = DIAG_CACHE_STRT; cache_size <= DIAG_CACHE_END;
       cache_size += DIAG_CACHE_DELT) {
    for (io_percent = DIAG_START; io_percent <= DIAG_END;
