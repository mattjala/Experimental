...
static struct h5_long_options l_opts[] = {
    {"help", no_arg, 'h'},    {"version", no_arg, 'V'},
...
static void usage(const char *prog) {
  ...
  fprintf(stdout, "   -n, --noop                Perform all the steps except "
  ...
  fprintf(stdout, "    a. chunked dataset: convert the chunk indexing type to "
                  "version 1 B-tree\n");
  fprintf(
      stdout,
  ...
  fprintf(stdout, "  converting the dataset </group/dataset> in the HDF5 file "
                  "<file_name>.\n");
} /* usage() */
...
static int convert(hid_t fid, const char *dname) {
  ...
  case H5D_CHUNKED:
    ...
    if (idx_type == H5D_CHUNK_IDX_BTREE) {
      if (verbose_g)
        fprintf(stdout, "Dataset's chunk indexing type is already version 1 "
                        "B-tree: no further action\n");
      h5tools_setstatus(EXIT_SUCCESS);
      goto done;
    } else if (verbose_g)
      fprintf(stdout,
  ...
  case H5D_CONTIGUOUS:
    if (verbose_g)
      fprintf(stdout, "Dataset is a contiguous dataset: downgrade layout "
                      "version as needed\n");
    break;
  ...
  case H5D_COMPACT:
    if (verbose_g)
      fprintf(
          stdout,
          "Dataset is a compact dataset: downgrade layout version as needed\n");
    break;
 ...
 *-------------------------------------------------------------------------
 */
static int convert_dsets_cb(const char *path, const H5O_info2_t *oi,
                            const char *already_visited, void *_fid) {
...
int main(int argc, char *argv[]) {
  ...
  /* Open the HDF5 file */
  if ((fid = h5tools_fopen(fname_g, H5F_ACC_RDWR, H5P_DEFAULT, FALSE, NULL,
                           0)) < 0) {
  ...
  if (dset_g) { /* Convert a specified dataset in the file */
    ...
    if (h5trav_visit(fid, "/", TRUE, TRUE, convert_dsets_cb, NULL, &fid,
                     H5O_INFO_BASIC) < 0)
      goto done;
  } /* end else */
