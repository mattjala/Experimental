...
typedef struct {
  ...
  size_t name_start; /* # of leading characters to strip off path names on
                        output */
} iter_t;
...
static void usage(void) {
  ...
  PRINTVALSTREAM(rawoutstream, "   -a, --address   Print raw data address.  If "
                               "dataset is contiguous, address\n");
  PRINTVALSTREAM(rawoutstream, "                   is offset in file of "
                               "beginning of raw data. If chunked,\n");
  PRINTVALSTREAM(rawoutstream, "                   returned list of addresses "
                               "indicates offset of each chunk.\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "                   Prints messages from the "
                               "HDF5 error stack as they occur.\n");
  PRINTVALSTREAM(rawoutstream, "   --follow-symlinks\n");
  PRINTVALSTREAM(rawoutstream, "                   Follow symbolic links (soft "
                               "links and external links)\n");
  PRINTVALSTREAM(rawoutstream,
                 "                   to display target object information.\n");
  PRINTVALSTREAM(rawoutstream, "                   Without this option, h5ls "
                               "identifies a symbolic link\n");
  PRINTVALSTREAM(rawoutstream, "                   as a soft link or external "
                               "link and prints the value\n");
  PRINTVALSTREAM(rawoutstream, "                   assigned to the symbolic "
                               "link; it does not provide any\n");
  PRINTVALSTREAM(rawoutstream, "                   information regarding the "
                               "target object or determine\n");
  PRINTVALSTREAM(rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "                   otherwise, h5ls shows error "
                               "message and returns an exit\n");
  PRINTVALSTREAM(rawoutstream, "                   code of 1.\n");
  PRINTVALSTREAM(rawoutstream, "                   Check for any symbolic "
                               "links (soft links or external links)\n");
  PRINTVALSTREAM(rawoutstream, "                   that do not resolve to an "
                               "existing object (dataset, group,\n");
  PRINTVALSTREAM(rawoutstream, "                   or named datatype).\n");
  PRINTVALSTREAM(rawoutstream, "                   If any dangling link is "
                               "found, this situation is treated\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "   --s3-cred=C     Supply S3 authentication "
                               "information to \"ros3\" vfd.\n");
  PRINTVALSTREAM(rawoutstream,
                 "                   Accepts tuple of "
                 "\"(<aws-region>,<access-id>,<access-key>)\".\n");
  PRINTVALSTREAM(rawoutstream, "                   If absent or C->\"(,,)\", "
  ...
  PRINTVALSTREAM(rawoutstream, "                   Accepts tuple of (<namenode "
                               "name>,<namenode port>,\n");
  PRINTVALSTREAM(rawoutstream, "                   ...<kerberos cache "
                               "path>,<username>,<buffer size>)\n");
  PRINTVALSTREAM(rawoutstream, "                   If absent or A == '(,,,,)', "
                               "all default values are used.\n");
  PRINTVALSTREAM(
      rawoutstream,
      "                   Has no effect if vfd flag is not 'hdfs'.\n");
  PRINTVALSTREAM(rawoutstream, "   --vol-value     Value (ID) of the VOL "
  ...
  PRINTVALSTREAM(rawoutstream, "                   HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vfd-info      VFD-specific info to pass "
                               "to the VFL driver used for\n");
  PRINTVALSTREAM(rawoutstream,
                 "                   opening the HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "  file/OBJECT\n");
  PRINTVALSTREAM(rawoutstream, "    Each object consists of an HDF5 file name "
                               "optionally followed by a\n");
  PRINTVALSTREAM(rawoutstream, "    slash and an object name within the file "
                               "(if no object is specified\n");
  PRINTVALSTREAM(rawoutstream, "    within the file then the contents of the "
                               "root group are displayed).\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "    The following options have been removed in "
                               "HDF5 1.12. Use the indicated\n");
 ...
 *-------------------------------------------------------------------------
 */
static int print_string(h5tools_str_t *buffer, const char *s,
                        hbool_t escape_spaces) {
  int nprint = 0;
 ...
 *-------------------------------------------------------------------------
 */
static int print_obj_name(h5tools_str_t *buffer, const iter_t *iter,
                          const char *oname, const char *s) {
  ...
  if (show_file_name_g)
    HDsnprintf(fullname, sizeof(fullname), "%s/%s", iter->fname,
               oname + iter->name_start);
  else
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_native_type(h5tools_str_t *buffer, hid_t type, int ind) {
  if (!simple_output_g) {
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_ieee_type(h5tools_str_t *buffer, hid_t type, int ind) {
...
static void print_precision(h5tools_str_t *buffer, hid_t type, int ind) {
  ...
  if (8 * H5Tget_size(type) != (prec = H5Tget_precision(type))) {
    ...
    if (plsb_s || pmsb_s) {
      ...
      if (pmsb_s) {
        nbits =
            (8 * H5Tget_size(type)) - ((unsigned)H5Tget_offset(type) + prec);
...
static hbool_t print_float_type(h5tools_str_t *buffer, hid_t type, int ind) {
  ...
  if ((1 + esize + msize) < H5Tget_precision(type)) {
    ...
    h5tools_str_append(buffer, "\n%*s(internal padding bits are %s)", ind, "",
                       pad_s);
  }
  return TRUE;
}
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_cmpd_type(h5tools_str_t *buffer, hid_t type, int ind) {
  ...
  h5tools_str_append(buffer, "\n%*s} %lu byte%s", ind, "", (unsigned long)size,
                     1 == size ? "" : "s");
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_enum_type(h5tools_str_t *buffer, hid_t type, int ind) {
  int nmembs;  /* number of members */
  ...
  if (nmembs > 0) {
    ...
    value = (unsigned char *)calloc((size_t)nmembs,
                                    MAX(H5Tget_size(type), dst_size));
    for (i = 0; i < (unsigned)nmembs; i++) {
    ...
    if (native > 0)
      if (H5Tconvert(super, native, (size_t)nmembs, value, NULL, H5P_DEFAULT) <
          0) {
        /* Release resources */
        for (i = 0; i < (unsigned)nmembs; i++)
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_string_type(h5tools_str_t *buffer, hid_t type,
                                 int H5_ATTR_UNUSED ind) {
  H5T_str_t pad;
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_reference_type(h5tools_str_t *buffer, hid_t type,
                                    int H5_ATTR_UNUSED ind) {
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_opaque_type(h5tools_str_t *buffer, hid_t type, int ind) {
  char *tag;
  size_t size;
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_vlen_type(h5tools_str_t *buffer, hid_t type, int ind) {
  hid_t super;
 ...
 *---------------------------------------------------------------------------
 */
static hbool_t print_array_type(h5tools_str_t *buffer, hid_t type, int ind) {
  hid_t super;
  int ndims, i;
  hsize_t *dims = NULL;
 ...
 *-------------------------------------------------------------------------
 */
static hbool_t print_bitfield_type(h5tools_str_t *buffer, hid_t type, int ind) {
  H5T_order_t order;          /* byte order value */
  const char *order_s = NULL; /* byte order string */
...
static void dump_dataset_values(hid_t dset) {
  ...
  if (hexdump_g) {
  ...
  } else if (string_g && H5Tget_size(f_type) == 1 &&
    ...
    HDsnprintf(string_prefix, sizeof(string_prefix), "%s\"",
               outputformat.line_pre);
  ...
  if (H5Tget_class(f_type) == H5T_REFERENCE) {
    ...
    if (NULL !=
      ...
      if (H5Dread(dset, H5T_STD_REF, H5S_ALL, H5S_ALL, H5P_DEFAULT, ref_buf) <
          0) {
        free(ref_buf);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t list_attr(hid_t obj, const char *attr_name,
    ...
    case H5D_CONTIGUOUS:
      ...
      if ((nf = H5Pget_external_count(dcpl)) > 0) {
        for (i = 0, max_len = 0; i < nf; i++) {
          if (H5Pget_external(dcpl, (unsigned)i, sizeof(f_name), f_name, NULL,
                              NULL) < 0)
            continue;
        ...
        h5tools_str_append(&buffer, "        %4s %10s %10s %10s %s\n", "ID",
                           "DSet-Addr", "File-Addr", "Bytes", "File");
        ...
        for (i = 0, total = 0; i < nf; i++) {
          if (H5Pget_external(dcpl, (unsigned)i, sizeof(f_name), f_name,
                              &f_offset, &f_size) < 0) {
            h5tools_str_append(
                &buffer,
    ...
    if ((nf = H5Pget_nfilters(dcpl)) > 0) {
      for (i = 0; i < nf; i++) {
        ...
        h5tools_str_append(&buffer, "    %-10s %s-%u %s {", s,
                           (f_name[0] ? f_name : "method"), (unsigned)filt_id,
                           ((filt_flags & H5Z_FLAG_OPTIONAL) ? "OPT" : ""));
        for (cd_num = 0; cd_num < cd_nelmts; cd_num++)
          h5tools_str_append(&buffer, "%s%u", (cd_num ? ", " : ""),
                             cd_values[cd_num]);
        h5tools_str_append(&buffer, "}\n");
      } /* end for */
    }   /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t list_obj(const char *name, const H5O_info2_t *oinfo,
                       const char *first_seen, void *_iter) {
    ...
    if (verbose_g > 0) {
      ...
      if (oinfo->mtime > 0) {
        ...
        if (tm) {
          ...
          h5tools_render_element(rawoutstream, info, &ctx, &buffer, &curr_pos,
                                 (size_t)info->line_ncols, (hsize_t)0,
                                 (hsize_t)0);
        } /* end if */
      }   /* end if */

      /* Only emit comments if the VOL connector supports that */
      H5VLquery_optional(obj_id, H5VL_SUBCLS_OBJECT,
                         H5VL_NATIVE_OBJECT_GET_COMMENT, &supported);
      ...
      if (supported & H5VL_OPT_QUERY_SUPPORTED) {
        ...
        /* if the actual length of the comment is longer than cmt_bufsize, then
         * call H5Oget_comment again with the correct value.
         */
        if (cmt_bufsize > 0) {
          comment = (char *)malloc((size_t)cmt_bufsize +
                                   1); /* new_size including null terminator */
          if (comment) {
            ...
            if (cmt_bufsize > 0) {
              ...
              h5tools_render_element(rawoutstream, info, &ctx, &buffer,
                                     &curr_pos, (size_t)info->line_ncols,
                                     (hsize_t)0, (hsize_t)0);
            } /* end if */
            free(comment);
          }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t list_lnk(const char *name, const H5L_info2_t *linfo,
                       void *_iter) {
  char *buf = NULL;
  iter_t *iter = (iter_t *)_iter;
  int ret;
  ...
  case H5L_TYPE_SOFT:
    ret =
        H5tools_get_symlink_info(iter->fid, name, &lnk_info, follow_symlink_g);
  ...
  case H5L_TYPE_EXTERNAL: {
    ...
    if (follow_link) {
      ...
      if (symlink_visit_add(iter->symlink_list, linfo->type, filename, path) <
          0) {
        goto done;
      }
...
static herr_t visit_obj(hid_t file, const char *oname, iter_t *iter) {
  ...
  if (H5Oget_info_by_name3(file, oname, &oi, H5O_INFO_BASIC | H5O_INFO_TIME,
                           H5P_DEFAULT) < 0) {
    if (iter->symlink_target) {
  ...
  if (H5O_TYPE_GROUP == oi.type && !grp_literal_g) {
    /* Get ID for group */
    if (!iter->symlink_target &&
        (iter->gid = H5Gopen2(file, oname, H5P_DEFAULT)) < 0) {
    ...
    h5trav_visit(file, oname, (hbool_t)(display_root_g || iter->symlink_target),
                 recursive_g, list_obj, list_lnk, iter,
 ...
 *              Failure: FALSE (0)
 *-------------------------------------------------------------------------*/
static hbool_t is_valid_args(void) {
  hbool_t ret = TRUE;

  if (recursive_g && grp_literal_g) {
    fprintf(rawerrorstream, "Error: 'recursive' option not compatible with "
                            "'group info' option!\n\n");
    ret = FALSE;
    goto out;
  }
  ...
  if (no_dangling_link_g && !follow_symlink_g) {
    fprintf(rawerrorstream, "Error: --no-dangling-links must be used along "
                            "with --follow-symlinks option!\n\n");
    ret = FALSE;
    goto out;
  }
...
int main(int argc, char *argv[]) {
    ...
    if (!HDstrcmp(argv[argno], "--")) {
      ...
      if (start == NULL) {
        fprintf(rawerrorstream,
                ...
                "    For anonymous access, omit \"--s3-cred\" and use only "
                "\"--vfd=ros3\"\n\n");
        usage();
        leave(EXIT_FAILURE);
      }
      ...
      if (h5tools_parse_ros3_fapl_tuple(start, ',', &ros3_fa) < 0) {
        fprintf(rawerrorstream,
                "Error: failed to parse S3 VFD credential info\n\n");
        usage();
        leave(EXIT_FAILURE);
      }
      ...
      if (h5tools_parse_hdfs_fapl_tuple(start, ',', &hdfs_fa) < 0) {
        fprintf(rawerrorstream,
                "Error: failed to parse HDFS VFD configuration info\n\n");
        usage();
        leave(EXIT_FAILURE);
      }
  ...
  if (custom_vol_fapl || custom_vfd_fapl) {
#ifdef H5_HAVE_ROS3_VFD
    if (custom_vfd_fapl &&
        (0 == HDstrcmp(vfd_info.u.name, drivernames[ROS3_VFD_IDX]))) {
      if (!vfd_info.info)
        vfd_info.info = &ros3_fa;
    }
#endif
#ifdef H5_HAVE_LIBHDFS
    if (custom_vfd_fapl &&
        (0 == HDstrcmp(vfd_info.u.name, drivernames[HDFS_VFD_IDX]))) {
      if (!vfd_info.info)
        vfd_info.info = &hdfs_fa;
    }
#endif
    ...
    if ((fapl_id =
             h5tools_get_fapl(H5P_DEFAULT, custom_vol_fapl ? &vol_info : NULL,
  ...
  while (argno < argc) {
    ...
    while (fname && *fname) {
      file_id = h5tools_fopen(fname, H5F_ACC_RDONLY, fapl_id,
                              (fapl_id != H5P_DEFAULT), drivername,
                              sizeof drivername);
