...
static herr_t do_read(results *res, file_descr *fd, parameters *parms,
                      void *buffer);
static herr_t dset_write(int local_dim, file_descr *fd, parameters *parms,
                         void *buffer);
static herr_t posix_buffer_write(int local_dim, file_descr *fd,
                                 parameters *parms, void *buffer);
static herr_t dset_read(int localrank, file_descr *fd, parameters *parms,
                        void *buffer, const char *buffer2);
static herr_t posix_buffer_read(int local_dim, file_descr *fd,
                                parameters *parms, void *buffer);
  ...
  for (i = 0; i < param.rank; i++) {
    ...
    if ((param.dset_size[i] % param.buf_size[i]) != 0) {
      fprintf(stderr,
              ...
              param.rank, (long long)param.dset_size[i], param.rank,
              param.buf_size[i]);
      GOTOERROR(FAIL);
    }
  }
...
static herr_t do_write(results *res, file_descr *fd, parameters *parms,
  ...
  case HDF5:
    ...
    HDsnprintf(dname, sizeof(dname), "Dataset_%ld",
               (unsigned long)parms->num_bytes);
    h5ds_id = H5Dcreate2(fd->h5fd, dname, ELMT_H5_TYPE, h5dset_space_id,
                         H5P_DEFAULT, h5dcpl, H5P_DEFAULT);
        ...
        if (parms->h5_use_chunks && parms->h5_extendable) {
          ...
          for (k = 0; k < parms->rank; k++) {
            ...
            if (dims[k] <= (hsize_t)h5offset[k]) {
              ...
              hrc = H5Sset_extent_simple(h5dset_space_id, parms->rank, dims,
                                         maxdims);
...
hid_t set_vfd(parameters *param) {
  ...
  if (vfd == sec2) {
    ...
    if (H5Pset_fapl_split(my_fapl, "-m.h5", H5P_DEFAULT, "-r.h5", H5P_DEFAULT) <
        0)
      return -1;
  } else if (vfd == multi) {
    ...
    if (H5Pset_fapl_multi(my_fapl, memb_map, memb_fapl, memb_name, memb_addr,
                          FALSE) < 0) {
      free(sv);
      return -1;
    }
    ...
    /* Linux direct read() and write() system calls.  Set memory boundary, file
     * block size, and copy buffer size to the default values. */
