...
#define MB_PER_SEC(bytes, t)                                                   \
  (H5_DBL_ABS_EQUAL(t, 0.0) ? 0.0                                              \
                            : ((((double)(bytes)) / (double)ONE_MB) / (t)))
...
static void accumulate_minmax_stuff(const minmax *mm, int count,
                                    minmax *total_mm);
...
static int run_test(iotype iot, parameters parms, struct options *opts) {
  ...
  write_sys_mm_table =
      (minmax *)calloc((size_t)parms.num_iters, sizeof(minmax));
  write_mm_table = (minmax *)calloc((size_t)parms.num_iters, sizeof(minmax));
  write_gross_mm_table =
      (minmax *)calloc((size_t)parms.num_iters, sizeof(minmax));
  write_raw_mm_table =
      (minmax *)calloc((size_t)parms.num_iters, sizeof(minmax));
...
static void usage(const char *prog) {
  ...
  printf("     -c SL             Selects chunked storage and defines chunks "
         "dimensions\n");
  printf("                       and sizes\n");
  printf("                       [default: Off]\n");
  ...
  printf("                       [default: 1]\n");
  printf("     -r NL             Dimension access order (see below for "
         "description)\n");
  printf("                       [default: 1,2]\n");
  printf("     -t                Selects extendable dimensions for HDF5 "
         "dataset\n");
  printf("                       [default: Off]\n");
  ...
  printf("                       [default: Off]\n");
  printf(
      "     -x SL             Dimensions and sizes of the transfer buffer\n");
  ...
  printf("\n");
  printf("      The example defines an object (dataset, transfer buffer) with "
         "three\n");
  printf("      dimensions. Be aware that as the number of dimensions "
         "increases, the\n");
  ...
  printf("      Data access starts at the cardinal origin of the dataset using "
         "the\n");
  printf("      transfer buffer. The next access occurs on a dataset region "
         "next to\n");
  printf("      the previous one. For a multidimensional dataset, there are "
         "several\n");
  printf("      directions as to where to proceed. This can be specified in "
         "the dimension\n");
  printf("      access order. For example, -r 1,2 states that the tool should "
         "traverse\n");
