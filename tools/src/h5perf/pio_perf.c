...
#define MB_PER_SEC(bytes, t)                                                   \
  (H5_DBL_ABS_EQUAL((t), 0.0) ? 0.0 : ((((double)bytes) / ONE_MB) / (t)))
...
off_t squareo(off_t x) { return x * x; }
...
static void run_test_loop(struct options *opts) {
  ...
  for (num_procs = opts->max_num_procs; num_procs >= opts->min_num_procs;
       num_procs >>= 1) {
    size_t buf_size;
    ...
    if (doing_pio) {
      ...
      for (buf_size = opts->min_xfer_size; buf_size <= opts->max_xfer_size;
           buf_size <<= 1) {
        ...
        if (parms.dim2d) {
          ...
          if (parms.interleaved)
            output_report(
                "Transfer Buffer Size: %ldx%ld bytes, File size: %.2f MB\n",
          ...
          print_indent(1);
          output_report("  # of files: %ld, # of datasets: %ld, dataset size: "
                        "%.2fx%.2f KB\n",
          ...
          output_report(
              "Transfer Buffer Size: %ld bytes, File size: %.2f MB\n", buf_size,
              ((double)parms.num_dsets * (double)parms.num_bytes) / ONE_MB);

          print_indent(1);
          output_report(
              "  # of files: %ld, # of datasets: %ld, dataset size: %.2f MB\n",
              parms.num_files, parms.num_dsets,
...
static int run_test(iotype iot, parameters parms, struct options *opts) {
  ...
  if (opts->print_raw) {
    ...
    output_results(opts, "Raw Data Write", write_raw_mm_table, parms.num_iters,
                   raw_size);
  } /* end if */
  ...
  output_results(opts, "Write Open-Close", write_gross_mm_table,
                 parms.num_iters, raw_size);

  if (opts->print_times) {
    output_times(opts, "Write File Open", write_open_mm_table, parms.num_iters);
    output_times(opts, "Write File Close", write_close_mm_table,
                 parms.num_iters);
  }
  ...
  if (!parms.h5_write_only) {
    ...
    if (opts->print_raw) {
      ...
      output_results(opts, "Raw Data Read", read_raw_mm_table, parms.num_iters,
                     raw_size);
    } /* end if */
    ...
    if (opts->print_times) {
      ...
      output_times(opts, "Read File Close", read_close_mm_table,
                   parms.num_iters);
    }
...
static int create_comm_world(int num_procs, int *doing_pio) {
  ...
  if (num_procs > nprocs) {
    fprintf(stderr,
            "number of process(%d) must be <= number of processes in "
            "MPI_COMM_WORLD(%d)\n",
            num_procs, nprocs);
    goto error_done;
  }
 ...
 * Return:      Nothing
 */
static void output_results(const struct options *opts, const char *name,
                           minmax *table, int table_size, off_t data_size) {
  minmax total_mm;
...
static void output_times(const struct options *opts, const char *name,
                         minmax *table, int table_size) {
  minmax total_mm;
...
static void report_parameters(struct options *opts) {
  ...
  if (opts->dim2d) {
    ...
    recover_size_and_print(
        (long long)(opts->num_bpp * opts->num_bpp * opts->min_num_procs), ":");
    recover_size_and_print(
        (long long)(opts->num_bpp * opts->num_bpp * opts->max_num_procs), "\n");
    ...
    recover_size_and_print(
        (long long)(squareo(opts->num_bpp * opts->min_num_procs) *
                    opts->num_dsets),
        ":");
    recover_size_and_print(
        (long long)(squareo(opts->num_bpp * opts->max_num_procs) *
                    opts->num_dsets),
        "\n");
    ...
    fprintf(output, "rank %d: File size=", rank);
    recover_size_and_print(
        (long long)(opts->num_bpp * opts->min_num_procs * opts->num_dsets),
        ":");
    recover_size_and_print(
        (long long)(opts->num_bpp * opts->max_num_procs * opts->num_dsets),
        "\n");
...
static struct options *parse_command_line(int argc, const char *const *argv) {
  ...
  cl_opts->print_raw =
      FALSE; /* Printing raw data throughput is off by default */
  cl_opts->h5_alignment = 1; /* No alignment for HDF5 objects by default */
  cl_opts->h5_threshold =
      1; /* No threshold for aligning HDF5 objects by default */
...
static void usage(const char *prog) {
  ...
  if (myrank == 0) {
    ...
    printf("     -a S, --align=S             Alignment of objects in HDF5 file "
           "[default: 1]\n");
    printf("     -A AL, --api=AL             Which APIs to test [default: all "
           "of them]\n");
#if 0
    ...
    printf("                                 [default: half the number of "
           "bytes per process\n");
    printf("                                           per dataset]\n");
    printf("     -c, --chunk                 Create HDF5 datasets using "
           "chunked storage\n");
    printf("                                 [default: contiguous storage]\n");
    printf("     -C, --collective            Use collective I/O for MPI and "
           "HDF5 APIs\n");
    printf("                                 [default: independent I/O)\n");
    printf("     -d N, --num-dsets=N         Number of datasets per file "
           "[default: 1]\n");
    printf("     -D DL, --debug=DL           Indicate the debugging level\n");
    printf("                                 [default: no debugging]\n");
    printf("     -e S, --num-bytes=S         Number of bytes per process per "
           "dataset\n");
    printf("                                 (see below for description)\n");
    printf(
    ...
    printf("     -g, --geometry              Use 2D geometry [default: 1D "
           "geometry]\n");
    printf("     -i N, --num-iterations=N    Number of iterations to perform "
           "[default: 1]\n");
    printf("     -I, --interleaved           Interleaved access pattern\n");
    printf("                                 (see below for example)\n");
    printf("                                 [default: Contiguous access "
           "pattern]\n");
    printf("     -o F, --output=F            Output raw data into file F "
           "[default: none]\n");
    printf("     -p N, --min-num-processes=N Minimum number of processes to "
           "use [default: 1]\n");
    printf("     -P N, --max-num-processes=N Maximum number of processes to "
           "use\n");
    printf("                                 [default: all MPI_COMM_WORLD "
           "processes ]\n");
    printf("     -T S, --threshold=S         Threshold for alignment of "
           "objects in HDF5 file\n");
    printf("                                 [default: 1]\n");
    printf("     -w, --write-only            Perform write tests not the read "
           "tests\n");
    ...
    printf("                                 [default: the number of bytes per "
           "process per\n");
    printf("                                           dataset]\n");
    printf("\n");
    printf("  F  - is a filename.\n");
    printf("  N  - is an integer >=0.\n");
    printf("  S  - is a size specifier, an integer >=0 followed by a size "
           "indicator:\n");
    printf("          K - Kilobyte (%d)\n", ONE_KB);
    printf("          M - Megabyte (%d)\n", ONE_MB);
    printf("          G - Gigabyte (%d)\n", ONE_GB);
    ...
    printf("\n");
    printf("  Dataset size:\n");
    printf("      Depending on the selected geometry, each test dataset is "
           "either a linear\n");
    printf("      array of size bytes-per-process * num-processes, or a square "
           "array of size\n");
    printf("      (bytes-per-process * num-processes) x (bytes-per-process * "
           "num-processes).\n");
    printf("\n");
    printf("  Block size vs. Transfer buffer size:\n");
    printf("      buffer-size controls the size of the memory buffer, which is "
           "broken into\n");
    printf("      blocks and written to the file. Depending on the selected "
           "geometry, each\n");
    printf("      block can be a linear array of size block-size or a square "
           "array of size\n");
    printf("      block-size x block-size. The arrangement in which blocks are "
           "written is\n");
    printf("      determined by the access pattern.\n");
    printf("\n");
    printf("      In 1D geometry, the transfer buffer is a linear array of "
           "size buffer-size.\n");
    printf("      In 2D geometry, it is a rectangular array of size block-size "
           "x buffer-size\n");
    printf("      or buffer-size x block-size if interleaved pattern is "
           "selected.\n");
    printf("\n");
    printf("  Interleaved and Contiguous patterns in 1D geometry:\n");
    printf("      When contiguous access pattern is chosen, the dataset is "
           "evenly divided\n");
    printf("      into num-processes regions and each process writes data to "
           "its own region.\n");
    printf("      When interleaved blocks are written to a dataset, space for "
           "the first\n");
    printf("      block of the first process is allocated in the dataset, then "
           "space is\n");
    printf("      allocated for the first block of the second process, etc. "
           "until space is\n");
    printf("      allocated for the first block of each process, then space is "
           "allocated for\n");
    printf("      the second block of the first process, the second block of "
           "the second\n");
    printf("      process, etc.\n");
    printf("\n");
    printf("      For example, with a 3 process run, 512KB bytes-per-process, "
           "256KB transfer\n");
    printf("        buffer size, and 64KB block size, each process must issue "
           "2 transfer\n");
    printf("        requests to complete access to the dataset.\n");
    printf("          Contiguous blocks of the first transfer request are "
           "written like so:\n");
    printf("              1111----2222----3333----\n");
    printf("          Interleaved blocks of the first transfer request are "
           "written like so:\n");
    printf("              123123123123------------\n");
    printf("        The actual number of I/O operations involved in a transfer "
           "request\n");
    printf("        depends on the access pattern and communication mode.\n");
    printf("        When using independent I/O with interleaved pattern, each "
           "process\n");
    printf("        performs 4 small non-contiguous I/O operations per "
           "transfer request.\n");
    printf("        If collective I/O is turned on, the combined content of "
           "the buffers of\n");
    printf("        the 3 processes will be written using one collective I/O "
           "operation\n");
    printf("        per transfer request.\n");
    printf("\n");
    printf("      For information about access patterns in 2D geometry, please "
           "refer to the\n");
    ...
    printf("  HDF5_NOCLEANUP   Do not remove data files if set [default "
           "remove]\n");
