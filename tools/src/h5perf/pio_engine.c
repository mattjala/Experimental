...
#define H5DCREATE(fd, name, type, space, dcpl)                                 \
  H5Dcreate(fd, name, type, space, dcpl)
...
static herr_t do_write(results *res, file_descr *fd, parameters *parms,
                       long ndsets, off_t nelmts, size_t buf_size,
                       void *buffer);
 ...
 */
static herr_t do_write(results *res, file_descr *fd, parameters *parms,
                       long ndsets, off_t nbytes, size_t buf_size,
                       void *buffer) {
  ...
  size_t nbytes_xfer_advance; /* Number of bytes transferred in a single I/O
                                 operation */
  ...
  MPI_Datatype mpi_partial_buffer_cont; /* MPI derived type for partial 2D
                                           contiguous buffer */
  MPI_Datatype mpi_inter_type; /* MPI derived type for 2D interleaved file  */
  MPI_Datatype mpi_partial_buffer_inter; /* MPI derived type for partial 2D
                                            interleaved buffer */
  ...
  MPI_Datatype
      mpi_chunk_inter_type; /* MPI derived type for 2D chunk interleaved file */
  MPI_Datatype mpi_collective_type; /* Generic MPI derived type for 2D
                                       collective access */
  ...
  /* 2D dataspace */
    ...
    if (!parms->interleaved) {
      bytes_begin[0] =
          (off_t)((double)snbytes * pio_mpi_rank_g / pio_mpi_nprocs_g);
      bytes_begin[1] = 0;
    } /* end if */
    /* Interleaved Pattern: */
    else {
  ...
  bytes_count =
      (off_t)(((double)nbytes * (pio_mpi_rank_g + 1)) / pio_mpi_nprocs_g) -
      (off_t)(((double)nbytes * pio_mpi_rank_g) / pio_mpi_nprocs_g);
  ...
  case MPIO: /* MPI-I/O setup */
    ...
    if (!parms->dim2d) {
      ...
      mrc =
          MPI_Type_vector((int)(buf_size / blk_size), (int)1,
    ...
    /* 2D dataspace */
      ...
      mrc = MPI_Type_vector((int)(buf_size / blk_size), (int)1,
                            (int)((size_t)snbytes / blk_size), mpi_full_chunk,
                            &mpi_chunk_inter_type);
      VRFY((mrc == MPI_SUCCESS), "MPIO_TYPE_CREATE");
  ...
  case PHDF5: /* HDF5 setup */
    ...
    if (!parms->dim2d) {
      if (nbytes > 0) {
        ...
        hrc = H5Sselect_hyperslab(h5dset_space_id, H5S_SELECT_SET, h5start,
                                  h5stride, h5count, h5block);
  ...
  for (ndset = 1; ndset <= ndsets; ++ndset) {
    ...
    case PHDF5:
      ...
      h5ds_id =
          H5DCREATE(fd->h5fd, dname, ELMT_H5_TYPE, h5dset_space_id, h5dcpl);
    ...
    if (!parms->dim2d) {
      ...
      /* Set base file offset for all I/O patterns and POSIX access */
      posix_file_offset =
          dset_offset + bytes_begin[0] * snbytes + bytes_begin[1];

      /* Set base file offset for all I/O patterns and MPI access */
      mpi_file_offset =
          (MPI_Offset)(dset_offset + bytes_begin[0] * snbytes + bytes_begin[1]);
    } /* end else */
    ...
    while (nbytes_xfer < bytes_count) {
        ...
        if (!parms->dim2d) {
          ...
          /* Interleaved access pattern */
            ...
            while (nbytes_toxfer > 0) {
              /* Skip offset over blocks of other processes */
              file_offset =
                  posix_file_offset + (off_t)(nbytes_xfer * pio_mpi_nprocs_g);
        ...
        /* 2D dataspace */
          ...
          if (!parms->h5_use_chunks) {
            ...
            /* Interleaved access pattern */
              ...
              file_offset = posix_file_offset +
                            (off_t)(((((size_t)nbytes_xfer / buf_size) *
                                      (size_t)pio_mpi_nprocs_g) /
                                     (size_t)snbytes) *
                                        (buf_size * (size_t)snbytes) +
                                    (((size_t)nbytes_xfer / buf_size) *
                                     (size_t)pio_mpi_nprocs_g) %
                                        (size_t)snbytes);
          ...
          /* Chunked storage */
            ...
            /*Interleaved access pattern */
              ...
              /* Global offset advance after each I/O operation */
              /* file_offset_advance =
               * (off_t)(snbytes/blk_size*(blk_size*blk_size)); */
          ...
          /* Collective file access */
            ...
            /* Interleaved access pattern */
              ...
              mrc = MPI_File_set_view(fd->mpifd, mpi_offset, mpi_blk_type,
                                      mpi_file_type, "native", h5_io_info_g);
        ...
        /* 2D dataspace */
          ...
          if (!parms->h5_use_chunks) {
            ...
            if (!parms->interleaved) {
              ...
              mpi_offset = mpi_file_offset +
                           (MPI_Offset)((((size_t)nbytes_xfer / blk_size) /
                                         (size_t)snbytes) *
                                        (blk_size * (size_t)snbytes)) +
                           (MPI_Offset)(((size_t)nbytes_xfer / blk_size) %
                                        (size_t)snbytes);
          ...
          /* Chunked storage */
            ...
            /*Interleaved access pattern */
              ...
              /* Global offset advance after each I/O operation */
              /* mpi_offset_advance =
               * (MPI_Offset)(snbytes/blk_size*(blk_size*blk_size)); */
          ...
          if (!parms->collective) {
            ...
            while (nbytes_toxfer > 0) {
              ...
              mrc = MPI_File_write_at(fd->mpifd, mpi_offset, buf_p,
                                      (int)nbytes_xfer_advance, MPI_BYTE,
                                      &mpi_status);
              VRFY((mrc == MPI_SUCCESS), "MPIO_WRITE");
          ...
          /* Common code for collective file access */
            ...
            /* Set the file view */
            mrc =
                MPI_File_set_view(fd->mpifd, mpi_offset, MPI_BYTE,
                                  mpi_collective_type, "native", h5_io_info_g);
            VRFY((mrc == MPI_SUCCESS), "MPIO_VIEW");

            /* Perform write */
            MPI_File_write_at_all(fd->mpifd, 0, buffer,
                                  (int)(buf_size * blk_size), MPI_BYTE,
                                  &mpi_status);
            VRFY((mrc == MPI_SUCCESS), "MPIO_WRITE");
      ...
      case PHDF5:
        ...
        if (!parms->dim2d) {
          ...
          hrc = H5Dwrite(h5ds_id, ELMT_H5_TYPE, h5mem_space_id, h5dset_space_id,
                         h5dxpl, buffer);
          VRFY((hrc >= 0), "H5Dwrite");
        ...
        /* 2D dataspace */
          ...
          if (!parms->interleaved) {
            /* Contiguous pattern */
            h5offset[0] = (hssize_t)(((size_t)nbytes_xfer /
                                      ((size_t)snbytes * blk_size)) *
                                     blk_size);
            h5offset[1] = (hssize_t)(((size_t)nbytes_xfer %
                                      ((size_t)snbytes * blk_size)) /
                                     blk_size);
            ...
            /* Skip offset over blocks of other processes */
            h5offset[0] =
                (hssize_t)((((size_t)nbytes_xfer * (size_t)pio_mpi_nprocs_g) /
  ...
  /* 2D dataspace */
    ...
    /* Interleaved Pattern: */
      ...
      if (!parms->h5_use_chunks || parms->io_type == PHDF5)
        ...
        bytes_begin[1] =
            (off_t)blk_size * (off_t)blk_size * (off_t)pio_mpi_rank_g;
    } /* end else */
  }   /* end else */
    ...
    while (nbytes_xfer < bytes_count) {
      ...
      case POSIXIO:
        ...
        if (!parms->dim2d) {
          ...
          if (!parms->interleaved) {
            ...
            rc =
                ((ssize_t)buf_size == POSIXREAD(fd->posixfd, buffer, buf_size));
            VRFY((rc != 0), "POSIXREAD");
      ...
      case MPIO:
          ...
          if (!parms->collective) {
            ...
            /* Interleaved access pattern */
              ...
              while (nbytes_toxfer > 0) {
                ...
                /* Perform independent read */
                mrc = MPI_File_read_at(fd->mpifd, mpi_offset, buf_p, (int)1,
                                       mpi_blk_type, &mpi_status);
                VRFY((mrc == MPI_SUCCESS), "MPIO_READ");
        ...
        /* 2D dataspace */
          ...
          /* Common code for collective file access */
            ...
            /* Perform read */
            MPI_File_read_at_all(fd->mpifd, 0, buffer,
                                 (int)(buf_size * blk_size), MPI_BYTE,
                                 &mpi_status);
            VRFY((mrc == MPI_SUCCESS), "MPIO_READ");
      ...
      case PHDF5:
        ...
        if (!parms->dim2d) {
          ...
          /* Read the buffer in */
          hrc = H5Dread(h5ds_id, ELMT_H5_TYPE, h5mem_space_id, h5dset_space_id,
                        h5dxpl, buffer);
          VRFY((hrc >= 0), "H5Dread");
...
static herr_t do_fopen(parameters *param, char *fname, file_descr *fd /*out*/,
  ...
  case MPIO:
    if (flags & (PIO_CREATE | PIO_WRITE)) {
      ...
      mrc = MPI_File_open(pio_comm_g, fname, MPI_MODE_RDONLY, h5_io_info_g,
                          &fd->mpifd);
      if (mrc != MPI_SUCCESS) {
        fprintf(stderr, "MPI File Open failed(%s)\n", fname);
        GOTOERROR(FAIL);
