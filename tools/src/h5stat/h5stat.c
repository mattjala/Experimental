...
typedef struct iter_t {
  ...
  unsigned long
      dset_rank_count[H5S_MAX_RANK]; /* Number of datasets of each rank */
  hsize_t max_dset_dims;             /* Maximum dimension size of dataset */
  unsigned long
      *small_dset_dims; /* Size of dimensions of small datasets tracked */
  unsigned long
      dset_layouts[H5D_NLAYOUTS]; /* Type of storage for each dataset */
  unsigned long dset_comptype[H5_NFILTERS_IMPL]; /* Number of currently
                                                    implemented filters */
  unsigned long dset_ntypes; /* Number of diff. dataset datatypes found */
  dtype_info_t
      *dset_type_info;     /* Pointer to dataset datatype information found */
  unsigned dset_dim_nbins; /* Number of bins for dataset dimensions */
  unsigned long
      *dset_dim_bins; /* Pointer to array of bins for dataset dimensions */
  ohdr_info_t dset_ohdr_info; /* Object header information for datasets */
  hsize_t dset_storage_size;  /* Size of raw data for datasets */
  hsize_t dset_external_storage_size; /* Size of raw data for datasets with
                                         external storage */
  ...
  hsize_t SM_index_storage_size;   /* index (btree & list) size for SOHM table
                                      (1.8) */
  ...
  unsigned long
      num_small_sects[SIZE_SMALL_SECTS]; /* Size of small free-space sections */
  unsigned sect_nbins; /* Number of bins for free-space section sizes */
  unsigned long
      *sect_bins; /* Pointer to array of bins for free-space section sizes */
  hsize_t datasets_index_storage_size; /* meta size for chunked dataset's
                                          indexing type */
  hsize_t datasets_heap_storage_size;  /* heap size for dataset with external
                                          storage */
...
static int display_dset_dtype_meta =
    FALSE;                       /* display datasets' datatype information */
...
static int display_summary =
    FALSE; /* display summary of file space information */

static int display_file_metadata =
    FALSE; /* display file space info for file's metadata */
static int display_group_metadata =
    FALSE; /* display file space info for groups' metadata */
static int display_dset_metadata =
    FALSE; /* display file space info for datasets' metadata */
...
/* e.g. "filemetadata" has to precede "file"; "groupmetadata" has to precede
 * "group" etc. */
...
static void usage(const char *prog) {
  ...
  fprintf(stdout, "     --enable-error-stack  Prints messages from the HDF5 "
                  "error stack as they occur\n");
  fprintf(stdout, "                           Optional value 2 also prints "
                  "file open errors\n");
  ...
  fprintf(stdout, "     -F, --filemetadata    Print file space information for "
                  "file's metadata\n");
  fprintf(stdout, "     -g, --group           Print group information\n");
  fprintf(stdout, "     -l N, --links=N       Set the threshold for the # of "
                  "links when printing\n");
  fprintf(stdout, "                           information for small groups.  N "
                  "is an integer greater\n");
  fprintf(stdout,
          "                           than 0.  The default threshold is 10.\n");
  fprintf(stdout, "     -G, --groupmetadata   Print file space information for "
                  "groups' metadata\n");
  fprintf(stdout, "     -d, --dset            Print dataset information\n");
  fprintf(stdout, "     -m N, --dims=N        Set the threshold for the "
                  "dimension sizes when printing\n");
  fprintf(stdout, "                           information for small datasets.  "
                  "N is an integer greater\n");
  fprintf(stdout,
          "                           than 0.  The default threshold is 10.\n");
  fprintf(stdout, "     -D, --dsetmetadata    Print file space information for "
                  "datasets' metadata\n");
  fprintf(stdout,
          "     -T, --dtypemetadata   Print datasets' datatype information\n");
  fprintf(stdout, "     -A, --attribute       Print attribute information\n");
  fprintf(stdout, "     -a N, --numattrs=N    Set the threshold for the # of "
                  "attributes when printing\n");
  fprintf(stdout, "                           information for small # of "
                  "attributes.  N is an integer greater\n");
  fprintf(stdout,
          "                           than 0.  The default threshold is 10.\n");
  ...
  fprintf(stdout, "     --s3-cred=<cred>      Access file on S3, using "
                  "provided credential\n");
  fprintf(stdout, "                           <cred> :: (region,id,key)\n");
  fprintf(stdout, "                           If <cred> == \"(,,)\", no "
                  "authentication is used.\n");
  fprintf(stdout, "     --hdfs-attrs=<attrs>  Access a file on HDFS with given "
                  "configuration\n");
  fprintf(stdout, "                           attributes.\n");
  fprintf(stdout, "                           <attrs> :: (<namenode "
                  "name>,<namenode port>,\n");
  fprintf(stdout, "                                       <kerberos cache "
                  "path>,<username>,\n");
  fprintf(stdout, "                                       <buffer size>)\n");
  fprintf(stdout, "                           If an attribute is empty, a "
 ...
 *-------------------------------------------------------------------------
 */
static herr_t attribute_stats(iter_t *iter, const H5O_info2_t *oi,
                              const H5O_native_info_t *native_oi) {
  ...
  if ((bin + 1) > iter->attr_nbins) {
    iter->attr_bins = (unsigned long *)realloc(
        iter->attr_bins, (bin + 1) * sizeof(unsigned long));
 ...
 *-------------------------------------------------------------------------
 */
static herr_t group_stats(iter_t *iter, const char *name, const H5O_info2_t *oi,
                          const H5O_native_info_t *native_oi) {
  ...
  if ((ret_value = H5Gget_info_by_name(iter->fid, name, &ginfo, H5P_DEFAULT)) <
      0)
  ...
  if ((bin + 1) > iter->group_nbins) {
    ...
    if ((iter->group_bins = (unsigned long *)realloc(
             iter->group_bins, (bin + 1) * sizeof(unsigned long))) == NULL)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t dataset_stats(iter_t *iter, const char *name,
                            const H5O_info2_t *oi,
                            const H5O_native_info_t *native_oi) {
  unsigned bin;               /* "bin" the number of objects falls in */
  ...
  if (ndims == 1) {
    ...
    if ((bin + 1) > iter->dset_dim_nbins) {
      /* Allocate more storage for info about dataset's datatype */
      if ((iter->dset_dim_bins = (unsigned long *)realloc(
               iter->dset_dim_bins, (bin + 1) * sizeof(unsigned long))) == NULL)
        H5TOOLS_GOTO_ERROR(FAIL, "H5Drealloc() failed");
  ...
  if (type_found)
    ...
    if ((iter->dset_type_info = (dtype_info_t *)realloc(
             iter->dset_type_info, iter->dset_ntypes * sizeof(dtype_info_t))) ==
        NULL)
      H5TOOLS_GOTO_ERROR(FAIL, "H5Drealloc() failed");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t datatype_stats(iter_t *iter, const H5O_info2_t *oi,
                             const H5O_native_info_t *native_oi) {
  herr_t ret_value = SUCCEED;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t obj_stats(const char *path, const H5O_info2_t *oi,
                        const char *already_visited, void *_iter) {
  ...
  if (NULL == already_visited) {
    ...
    if (H5Oget_native_info_by_name(iter->fid, path, &native_info,
                                   H5O_NATIVE_INFO_ALL, H5P_DEFAULT) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t lnk_stats(const char H5_ATTR_UNUSED *path, const H5L_info2_t *li,
                        void *_iter) {
  iter_t *iter = (iter_t *)_iter;
...
static herr_t freespace_stats(hid_t fid, iter_t *iter) {
  ...
  if ((nsects = H5Fget_free_sections(fid, H5FD_MEM_DEFAULT, 0, NULL)) < 0)
    ...
    if (NULL == (sect_info = (H5F_sect_info_t *)calloc(
                     (size_t)nsects, sizeof(H5F_sect_info_t))))
      return (FAIL);
    nsects =
        H5Fget_free_sections(fid, H5FD_MEM_DEFAULT, (size_t)nsects, sect_info);
    assert(nsects);
  } /* end else-if */
  ...
  for (u = 0; u < (size_t)nsects; u++) {
    ...
    if (bin >= iter->sect_nbins) {
      ...
      iter->sect_bins = (unsigned long *)realloc(
          iter->sect_bins, (bin + 1) * sizeof(unsigned long));
...
static int parse_command_line(int argc, const char *const *argv,
  ...
  while ((opt = H5_get_option(argc, argv, s_opts, l_opts)) != EOF) {
    ...
    case 'O':
      ...
      if (NULL == (hand = (struct handler_t *)calloc(
                       (size_t)1, sizeof(struct handler_t)))) {
...
static herr_t print_file_metadata(const iter_t *iter) {
  ...
  fprintf(stdout,
          "\t\tDatasets(exclude compact data): %" PRIuHSIZE "/%" PRIuHSIZE "\n",
...
static herr_t print_group_metadata(const iter_t *iter) {
  ...
  fprintf(stdout,
          "\tObject headers (total/unused): %" PRIuHSIZE "/%" PRIuHSIZE "\n",
          iter->group_ohdr_info.total_size, iter->group_ohdr_info.free_size);
...
static herr_t print_storage_summary(const iter_t *iter) {
  ...
  total_meta =
      ...
      iter->group_ohdr_info.total_size + iter->dset_ohdr_info.total_size +
      iter->dtype_ohdr_info.total_size + iter->groups_btree_storage_size +
      iter->groups_heap_storage_size + iter->attrs_btree_storage_size +
      iter->attrs_heap_storage_size + iter->datasets_index_storage_size +
  ...
  fprintf(stdout,
          "  Amount/Percent of tracked free space: %" PRIuHSIZE
          " bytes/%3.1f%%\n",
          iter->free_space, percent);

  if (iter->filesize <
      (total_meta + iter->dset_storage_size + iter->free_space)) {
    unaccount = (total_meta + iter->dset_storage_size + iter->free_space) -
                iter->filesize;
    fprintf(stdout,
            "  ??? File has %" PRIuHSIZE
            " more bytes accounted for than its size! ???\n",
            unaccount);
  } else {
    unaccount = iter->filesize -
                (total_meta + iter->dset_storage_size + iter->free_space);
...
int main(int argc, char *argv[]) {
  ...
  if (fname) {
    ...
    iter.num_small_groups = (unsigned long *)calloc((size_t)sgroups_threshold,
                                                    sizeof(unsigned long));
    iter.num_small_attrs = (unsigned long *)calloc(
        (size_t)(sattrs_threshold + 1), sizeof(unsigned long));
    iter.small_dset_dims = (unsigned long *)calloc((size_t)sdsets_threshold,
                                                   sizeof(unsigned long));

    if (iter.num_small_groups == NULL || iter.num_small_attrs == NULL ||
        iter.small_dset_dims == NULL) {
      error_msg("Unable to allocate memory for tracking small "
    ...
    if (H5Pget_file_space_strategy(fcpl, &iter.fs_strategy, &iter.fs_persist,
                                   &iter.fs_threshold) < 0)
      warn_msg("Unable to retrieve file space information\n");
    assert(iter.fs_strategy >= 0 &&
           iter.fs_strategy < H5F_FSPACE_STRATEGY_NTYPES);
    ...
    if (display_object) {
      ...
      for (u = 0; u < hand->obj_count; u++) {
        if (h5trav_visit(fid, hand->obj[u], TRUE, TRUE, obj_stats, lnk_stats,
                         &iter, H5O_INFO_ALL) < 0) {
      ...
      if (h5trav_visit(fid, "/", TRUE, TRUE, obj_stats, lnk_stats, &iter,
                       H5O_INFO_ALL) < 0) {
