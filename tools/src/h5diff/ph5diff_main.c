 ...
 * Comments:
 *
 * This function drives the diff process and will do a serial or parallel diff
 *depending on the value of the global variable g_Parallel (default is 0), set
 *to 1 when the program is run as "ph5diff"
 *-------------------------------------------------------------------------
 */
...
static void ph5diff_worker(int nID) {
  ...
  while (1) {
    ...
    if (Status.MPI_TAG == MPI_TAG_PARALLEL) {
      ...
      /* Retrieve filenames */
      MPI_Recv(filenames, MAX_FILENAME * 2, MPI_CHAR, 0, MPI_ANY_TAG,
               MPI_COMM_WORLD, &Status);

      /* disable error reporting */
      H5E_BEGIN_TRY {
        /* Open the files */
        if ((file1_id = H5Fopen(filenames[0], H5F_ACC_RDONLY, H5P_DEFAULT)) <
            0) {
          printf("h5diff Task [%d]: <%s>: unable to open file\n", nID,
                 filenames[0]);
          MPI_Abort(MPI_COMM_WORLD, 0);
        }
        if ((file2_id = H5Fopen(filenames[1], H5F_ACC_RDONLY, H5P_DEFAULT)) <
            0) {
          printf("h5diff Task [%d]: <%s>: unable to open file\n", nID,
                 filenames[1]);
          MPI_Abort(MPI_COMM_WORLD, 0);
    ...
    /* Check for work */
      ...
      if (file1_id < 0 || file2_id < 0) {
        printf(
            "ph5diff_worker: ERROR: work received before/without filenames\n");
        break;
      }

      /* Recv parameters for diff from manager task */
      MPI_Recv(&args, sizeof(args), MPI_BYTE, 0, MPI_TAG_ARGS, MPI_COMM_WORLD,
               &Status);

      /* Do the diff */
      diffs.nfound = diff(file1_id, args.name1, file2_id, args.name2,
                          &(args.opts), &(args.argdata));
      ...
      if (outBuffOffset > 0) {
        ...
        /* When get token, send all of our output to the manager task and then
         * return the token */
        for (i = 0; i < outBuffOffset; i += PRINT_DATA_MAX_SIZE)
          MPI_Send(outBuff + i, PRINT_DATA_MAX_SIZE, MPI_BYTE, 0,
                   MPI_TAG_PRINT_DATA, MPI_COMM_WORLD);

        /* An overflow file exists, so we send it's output to the manager too
         * and then delete it */
        if (overflow_file) {
          ...
          while ((tmp = getc(overflow_file)) >= 0) {
            *(out_data + i++) = (char)tmp;
            if (i == PRINT_DATA_MAX_SIZE) {
              MPI_Send(out_data, PRINT_DATA_MAX_SIZE, MPI_BYTE, 0,
                       MPI_TAG_PRINT_DATA, MPI_COMM_WORLD);
              i = 0;
              memset(out_data, 0, PRINT_DATA_MAX_SIZE);
            }
        ...
        MPI_Send(&diffs, sizeof(diffs), MPI_BYTE, 0, MPI_TAG_TOK_RETURN,
                 MPI_COMM_WORLD);
      } else
        MPI_Send(&diffs, sizeof(diffs), MPI_BYTE, 0, MPI_TAG_DONE,
                 MPI_COMM_WORLD);
    }
    /* Check for leaving */
...
void print_manager_output(void) {
  ...
  if ((outBuffOffset > 0) && g_Parallel) {
    ...
    fprintf(stderr,
            "h5diff error: outBuffOffset>0, but we're not in parallel!\n");
  }
}
...
void h5diff_exit(int status) {
  ...
  if (g_Parallel) {
    ...
    MPI_Finalize();
    status = EXIT_SUCCESS; /* Reset exit status, since some mpiexec commands
                              generate output on failure status */
  }
