...
static void check_options(diff_opt_t *opts) {
  ...
  if ((opts->delta_bool + opts->percent_bool + opts->use_system_epsilon) > 1) {
    printf("%s error: -d, -p and --use-system-epsilon options are "
           "mutually-exclusive;\n",
           PROGRAMNAME);
    printf("use no more than one.\n");
    printf("Try '-h' or '--help' option for more information or see the %s "
           "entry in the 'HDF5 "
           "Reference Manual'.\n",
           PROGRAMNAME);
...
void parse_command_line(int argc, const char *const *argv, const char **fname1,
                        const char **fname2, const char **objname1,
                        const char **objname2, diff_opt_t *opts) {
  int i;
  int opt;
  struct exclude_path_list *exclude_head, *exclude_prev, *exclude_node;
  struct exclude_path_list *exclude_attr_head, *exclude_attr_prev,
      *exclude_attr_node;
  ...
  while ((opt = H5_get_option(argc, argv, s_opts, l_opts)) != EOF) {
    ...
    case 'E':
      ...
      if ((exclude_node = (struct exclude_path_list *)malloc(
               sizeof(struct exclude_path_list))) == NULL) {
    ...
    case 'A':
      ...
      /* create linked list of excluding objects */
      if ((exclude_attr_node = (struct exclude_path_list *)malloc(
               sizeof(struct exclude_path_list))) == NULL) {
        printf("Error: lack of memory!\n");
        h5diff_exit(EXIT_FAILURE);
      }
  ...
  if (opts->vfd_info[0].u.name &&
      !HDstrcmp(opts->vfd_info[0].u.name, "onion")) {
  ...
  if (opts->vfd_info[1].u.name &&
      !HDstrcmp(opts->vfd_info[1].u.name, "onion")) {
...
void usage(void) {
  ...
  PRINTVALSTREAM(rawoutstream, "   --enable-error-stack Prints messages from "
                               "the HDF5 error stack as they occur.\n");
  PRINTVALSTREAM(rawoutstream, "                        Optional value 2 also "
                               "prints file open errors.\n");
  ...
  PRINTVALSTREAM(rawoutstream, "         Verbose mode. Print differences "
                               "information and list of objects.\n");
  PRINTVALSTREAM(rawoutstream, "   -vN --verbose=N\n");
  PRINTVALSTREAM(rawoutstream, "         Verbose mode with level. Print "
                               "differences and list of objects.\n");
  PRINTVALSTREAM(rawoutstream,
                 "         Level of detail depends on value of N:\n");
  PRINTVALSTREAM(rawoutstream,
                 "          0 : Identical to '-v' or '--verbose'.\n");
  PRINTVALSTREAM(rawoutstream, "          1 : All level 0 information plus "
                               "one-line attribute status summary.\n");
  PRINTVALSTREAM(rawoutstream, "          2 : All level 1 information plus "
                               "extended attribute status report.\n");
  PRINTVALSTREAM(rawoutstream,
                 "          3 : All level 2 information plus file names.\n");
  PRINTVALSTREAM(rawoutstream, "   -q, --quiet\n");
  PRINTVALSTREAM(rawoutstream, "         Quiet mode. Do not produce output.\n");
  PRINTVALSTREAM(rawoutstream, "   --vol-value-1           Value (ID) of the "
                               "VOL connector to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           first HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vol-name-1            Name of the VOL "
                               "connector to use for opening the first\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vol-info-1            VOL-specific info "
                               "to pass to the VOL connector used for\n");
  PRINTVALSTREAM(
      rawoutstream,
      "                           opening the first HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vol-value-2           Value (ID) of the "
                               "VOL connector to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           second HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vol-name-2            Name of the VOL "
                               "connector to use for opening the second\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vol-info-2            VOL-specific info "
                               "to pass to the VOL connector used for\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream,
                 "                           the VOL named by "
                 "HDF5_VOL_CONNECTOR (or the native VOL connector,\n");
  PRINTVALSTREAM(rawoutstream, "                           if that environment "
                               "variable is unset) will be used\n");
  PRINTVALSTREAM(rawoutstream, "   --vfd-value-1           Value (ID) of the "
                               "VFL driver to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           first HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vfd-name-1            Name of the VFL "
                               "driver to use for opening the first\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vfd-info-1            VFD-specific info "
                               "to pass to the VFL driver used for\n");
  PRINTVALSTREAM(
      rawoutstream,
      "                           opening the first HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vfd-value-2           Value (ID) of the "
                               "VFL driver to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           second HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vfd-name-2            Name of the VFL "
                               "driver to use for opening the second\n");
  PRINTVALSTREAM(rawoutstream,
                 "                           HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --vfd-info-2            VFD-specific info "
                               "to pass to the VFL driver used for\n");
  PRINTVALSTREAM(
      rawoutstream,
      "                           opening the second HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "   --follow-symlinks\n");
  PRINTVALSTREAM(rawoutstream, "         Follow symbolic links (soft links and "
                               "external links and compare the)\n");
  PRINTVALSTREAM(rawoutstream, "         links' target objects.\n");
  PRINTVALSTREAM(rawoutstream, "         If symbolic link(s) with the same "
                               "name exist in the files being\n");
  PRINTVALSTREAM(rawoutstream, "         compared, then determine whether the "
                               "target of each link is an existing\n");
  PRINTVALSTREAM(rawoutstream, "         object (dataset, group, or named "
                               "datatype) or the link is a dangling\n");
  PRINTVALSTREAM(rawoutstream, "         link (a soft or external link "
                               "pointing to a target object that does\n");
  PRINTVALSTREAM(rawoutstream, "         not yet exist).\n");
  PRINTVALSTREAM(rawoutstream, "         - If both symbolic links are dangling "
                               "links, they are treated as being\n");
  PRINTVALSTREAM(
      rawoutstream,
      "           the same; by default, h5diff returns an exit code of 0.\n");
  PRINTVALSTREAM(rawoutstream, "           If, however, --no-dangling-links is "
                               "used with --follow-symlinks,\n");
  PRINTVALSTREAM(rawoutstream, "           this situation is treated as an "
                               "error and h5diff returns an\n");
  PRINTVALSTREAM(rawoutstream, "           exit code of 2.\n");
  PRINTVALSTREAM(rawoutstream, "         - If only one of the two links is a "
                               "dangling link,they are treated as\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "           this situation is treated as an "
                               "error and h5diff returns an\n");
  PRINTVALSTREAM(rawoutstream, "           exit code of 2.\n");
  PRINTVALSTREAM(rawoutstream, "         - If both symbolic links point to "
                               "existing objects, h5diff compares the\n");
  PRINTVALSTREAM(rawoutstream, "           two objects.\n");
  PRINTVALSTREAM(rawoutstream, "         If any symbolic link specified in the "
                               "call to h5diff does not exist,\n");
  PRINTVALSTREAM(
      rawoutstream,
      "         h5diff treats it as an error and returns an exit code of 2.\n");
  PRINTVALSTREAM(rawoutstream, "   --no-dangling-links\n");
  PRINTVALSTREAM(rawoutstream, "         Must be used with --follow-symlinks "
                               "option; otherwise, h5diff shows\n");
  PRINTVALSTREAM(rawoutstream,
                 "         error message and returns an exit code of 2.\n");
  PRINTVALSTREAM(rawoutstream, "         Check for any symbolic links (soft "
                               "links or external links) that do not\n");
  PRINTVALSTREAM(rawoutstream, "         resolve to an existing object "
                               "(dataset, group, or named datatype).\n");
  PRINTVALSTREAM(rawoutstream, "         If any dangling link is found, this "
                               "situation is treated as an error\n");
  PRINTVALSTREAM(rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "         Print difference if (|a-b| > D). D "
                               "must be a positive number. Where a\n");
  PRINTVALSTREAM(rawoutstream, "         is the data point value in file1 and "
                               "b is the data point value in file2.\n");
  PRINTVALSTREAM(rawoutstream,
                 "         Can not use with '-p' or '--use-system-epsilon'.\n");
  PRINTVALSTREAM(rawoutstream, "   -p R, --relative=R\n");
  PRINTVALSTREAM(rawoutstream, "         Print difference if (|(a-b)/b| > R). "
                               "R must be a positive number. Where a\n");
  PRINTVALSTREAM(rawoutstream, "         is the data point value in file1 and "
                               "b is the data point value in file2.\n");
  PRINTVALSTREAM(rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream,
                 "         Print difference if (|a-b| > EPSILON), EPSILON is "
                 "system defined value. Where a\n");
  PRINTVALSTREAM(rawoutstream, "         is the data point value in file1 and "
                               "b is the data point value in file2.\n");
  PRINTVALSTREAM(rawoutstream, "         If the system epsilon is not "
                               "defined,one of the following predefined\n");
  ...
  PRINTVALSTREAM(rawoutstream, "         Exclude the specified path to an "
                               "object when comparing files or groups.\n");
  PRINTVALSTREAM(rawoutstream, "         If a group is excluded, all member "
                               "objects will also be excluded.\n");
  PRINTVALSTREAM(
      rawoutstream,
      "         The specified path is excluded wherever it occurs.\n");
  PRINTVALSTREAM(rawoutstream, "         This flexibility enables the same "
                               "option to exclude either objects that\n");
  PRINTVALSTREAM(rawoutstream, "         exist only in one file or common "
                               "objects that are known to differ.\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "         When comparing files, \"path\" is the "
                               "absolute path to the excluded;\n");
  PRINTVALSTREAM(rawoutstream, "         object; when comparing groups, "
                               "\"path\" is similar to the relative\n");
  PRINTVALSTREAM(rawoutstream, "         path from the group to the excluded "
                               "object. This \"path\" can be\n");
  PRINTVALSTREAM(rawoutstream, "         taken from the first section of the "
                               "output of the --verbose option.\n");
  PRINTVALSTREAM(rawoutstream, "         For example, if you are comparing the "
                               "group /groupA in two files and\n");
  PRINTVALSTREAM(rawoutstream,
                 "         you want to exclude /groupA/groupB/groupC in both "
                 "files, the exclude\n");
  PRINTVALSTREAM(rawoutstream, "         option would read as follows:\n");
  PRINTVALSTREAM(rawoutstream,
                 "           --exclude-path \"/groupB/groupC\"\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "         If there are multiple paths to an "
                               "object, only the specified path(s)\n");
  PRINTVALSTREAM(rawoutstream, "         will be excluded; the comparison will "
                               "include any path not explicitly\n");
  PRINTVALSTREAM(rawoutstream, "         excluded.\n");
  PRINTVALSTREAM(rawoutstream, "         This option can be used repeatedly to "
                               "exclude multiple paths.\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream,
                 "   --exclude-attribute \"path/to/object/with/attribute\"\n");
  PRINTVALSTREAM(rawoutstream,
                 "         Exclude attributes on the specified path to an "
                 "object when comparing files or groups.\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "         If there are multiple paths to an "
                               "object, only the specified path(s)\n");
  ...
  PRINTVALSTREAM(rawoutstream, "  Default mode: print the number of "
                               "differences found and where they occurred\n");
  PRINTVALSTREAM(rawoutstream,
                 "  -r Report mode: print the above plus the differences\n");
  PRINTVALSTREAM(rawoutstream, "  -v Verbose mode: print the above plus a list "
                               "of objects and warnings\n");
  PRINTVALSTREAM(rawoutstream, "  -q Quiet mode: do not print output\n");

  PRINTVALSTREAM(rawoutstream, "\n");

  PRINTVALSTREAM(rawoutstream, " File comparison:\n");
  PRINTVALSTREAM(rawoutstream, "  If no objects [obj1[ obj2]] are specified, "
                               "the h5diff comparison proceeds as\n");
  PRINTVALSTREAM(rawoutstream, "  a comparison of the two files' root groups.  "
                               "That is, h5diff first compares\n");
  PRINTVALSTREAM(rawoutstream, "  the names of root group members, generates a "
                               "report of root group objects\n");
  PRINTVALSTREAM(rawoutstream, "  that appear in only one file or in both "
                               "files, and recursively compares\n");
  ...
  PRINTVALSTREAM(rawoutstream, "      First compares the names of member "
                               "objects (relative path, from the\n");
  PRINTVALSTREAM(rawoutstream, "      specified group) and generates a report "
                               "of objects that appear in only\n");
  PRINTVALSTREAM(rawoutstream, "      one group or in both groups. Common "
                               "objects are then compared recursively.\n");
  PRINTVALSTREAM(rawoutstream, "  2) Datasets\n");
  PRINTVALSTREAM(rawoutstream, "      Array rank and dimensions, datatypes, "
                               "and data values are compared.\n");
  ...
  PRINTVALSTREAM(rawoutstream, "      (The option --follow-symlinks overrides "
  ...
  PRINTVALSTREAM(rawoutstream, "  --no-compact-subset  Disable compact form of "
                               "subsetting and allow the use\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          of \"[\" in dataset names.\n");
  PRINTVALSTREAM(rawoutstream, "  Subsetting is available by using the "
                               "fcompact form of subsetting, as follows:\n");
  PRINTVALSTREAM(rawoutstream,
                 "    obj1 /foo/mydataset[START;STRIDE;COUNT;BLOCK]\n");
  PRINTVALSTREAM(rawoutstream, "  It is not required to use all parameters, "
                               "but until the last parameter value used,\n");
  PRINTVALSTREAM(rawoutstream,
                 "  all of the semicolons (;) are required, even when a "
  ...
  PRINTVALSTREAM(rawoutstream, "  The STRIDE, COUNT, and BLOCK parameters are "
                               "optional and will default to 1 in\n");
  PRINTVALSTREAM(rawoutstream, "  each dimension. START is optional and will "
                               "default to 0 in each dimension.\n");
  PRINTVALSTREAM(rawoutstream, "  Each of START, STRIDE, COUNT, and BLOCK must "
                               "be a comma-separated list of integers with\n");
  PRINTVALSTREAM(rawoutstream,
