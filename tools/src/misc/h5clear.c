 ...
 *      (1) -s, --status:   clear the status_flags field from the file's
 * superblock (2) -m, --image:    remove the metadata cache image from the file
...
static void usage(const char *prog) {
  ...
  fprintf(stdout, "   -s, --status              Clear the status_flags field "
                  "in the file's superblock\n");
  fprintf(stdout, "   -m, --image               Remove the metadata cache "
                  "image from the file\n");
  fprintf(stdout,
          "   --filesize                Print the file's EOA and EOF\n");
  fprintf(stdout, "   --increment=C             Set the file's EOA to the "
                  "maximum of (EOA, EOF) + C for\n");
  fprintf(stdout, "                             the file <file_name>.\n");
  fprintf(stdout, "                             C is >= 0; C is optional and "
                  "will default to 1M when not set.\n");
  fprintf(stdout, "                             This option helps to repair a "
                  "crashed file where the stored EOA\n");
  fprintf(stdout, "                             in the superblock is different "
                  "from the actual EOF.\n");
  fprintf(stdout, "                             The file's EOA and EOF will be "
                  "the same after applying\n");
  fprintf(stdout, "                             this option to the file.\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "Examples of use:\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "h5clear -s file_name\n");
  fprintf(stdout, "  Clear the status_flags field in the superblock of the "
                  "HDF5 file <file_name>.\n");
  fprintf(stdout, "\n");
  ...
  fprintf(stdout, "  Set the EOA to the maximum of (EOA, EOF) + 1M for the "
                  "file <file_name>.\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "h5clear --increment=512 file_name\n");
  fprintf(stdout, "  Set the EOA to the maximum of (EOA, EOF) + 512 for the "
                  "file <file_name>.\n");
} /* usage() */
 ...
 *          (1) -s, --status:   clear the status_flags field from the file's
 *superblock (2) -m, --image:    remove the metadata cache image from the file
 *          (3) --increment=C:  set the file's EOA to the maximum of (EOA, EOF)
 *+ C (4) --filesize:     print the file's EOA and EOF
 *
...
int main(int argc, char *argv[]) {
  ...
  if (!clear_status_flags && !remove_cache_image && !increment_eoa_eof &&
      !print_filesize) {
    usage(h5tools_getprogname());
    h5tools_setstatus(EXIT_FAILURE);
    goto done;
  ...
  if (clear_status_flags) {
    ...
    if (H5Pset(fapl, H5F_ACS_CLEAR_STATUS_FLAGS_NAME, &clear_status_flags) <
        0) {
      error_msg("H5Pset\n");
      h5tools_setstatus(EXIT_FAILURE);
      goto done;
