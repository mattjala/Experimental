...
/* Make these 2 private properties(defined in H5Fprivate.h) available to
 * h5repart. The first one updates the member file size in the superblock.  The
 * second one change file driver from family to a single file driver.
 */
...
static void usage(const char *progname) {
  fprintf(stderr,
          "usage: %s [-v] [-V] [-[b|m] N[g|m|k]] "
          "[-family_to_sec2|-family_to_single] SRC DST\n",
          progname);
  ...
  fprintf(stderr, "   -family_to_single   Change file driver from family to "
                  "the default single-file VFD "
                  "(windows or sec2)\n");
 ...
 *-------------------------------------------------------------------------
 */
static off_t get_size(const char *progname, int *argno, int argc,
                      char *argv[]) {
  off_t retval = -1;
  char *suffix = NULL;
...
int main(int argc, char *argv[]) {
  ...
  if ((dst = HDopen(dst_name, O_RDWR | O_CREAT | O_TRUNC,
  ...
  /* Modify family driver information saved in superblock through private
   * property. These private properties are for this tool only. */
  ...
  if (family_to_single) {
     ...
     * the library to ignore the family driver information saved in the
     * superblock.
     */
    ...
    /* Modify family size saved in superblock through private property. It
     * signals library to save the new member size(specified in command line) in
     * superblock. This private property is for this tool only. */
   ...
   * member size.  If the original file is a family file and the new file is a
   * single file, the property FAMILY_TO_SINGLE will signal the library to
   * switch to default single-file driver when the new file is opened.  If the
   * original file is a single file and the new file can only be a single file,
   * reopen the new file should fail. There's nothing to do in this case.
   */
