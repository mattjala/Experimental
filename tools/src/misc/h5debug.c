...
int main(int argc, char *argv[]) {
  ...
  if (!memcmp(sig, H5F_SIGNATURE, (size_t)H5F_SIGNATURE_LEN)) {
    ...
    if (extra_count == 0 || extra[0] == 0) {
      fprintf(stderr, "\nWarning: Providing the group's local heap address "
                      "will give more information\n");
      fprintf(stderr, "Symbol table node usage:\n");
      fprintf(stderr, "\th5debug <filename> <Symbol table node address> "
                      "<address of local heap>\n\n");
    } /* end if */
    ...
    case H5B_SNODE_ID:
      ...
      if (extra_count == 0 || extra[0] == 0) {
        ...
        fprintf(stderr, "\th5debug <filename> <B-tree node address> <address "
                        "of local heap>\n\n");
        exit_value = 4;
        goto done;
      } /* end if */
    ...
    case H5B_CHUNK_ID:
      /* Check for extra parameters */
      if (extra_count == 0 || extra[0] == 0) {
        fprintf(stderr, "ERROR: Need number of dimensions of chunk in order to "
                        "dump chunk B-tree node\n");
        fprintf(stderr, "B-tree chunked storage node usage:\n");
        fprintf(stderr, "\th5debug <filename> <B-tree node address> <# of "
                        "dimensions> <slowest "
      ...
      if (ndims > 9) {
        fprintf(stderr,
                "ERROR: Only 9 dimensions support currently (fix h5debug)\n");
        fprintf(stderr, "B-tree chunked storage node usage:\n");
        fprintf(stderr, "\th5debug <filename> <B-tree node address> <# of "
                        "dimensions> <slowest "
                        "chunk dim>...<fastest chunk dim>\n");
    ...
    if ((cls == H5D_BT2 || cls == H5D_BT2_FILT) &&
        (extra_count == 0 || extra[0] == 0)) {
      fprintf(stderr, "ERROR: Need v2 B-tree header address and object header "
                      "address containing the "
                      "layout message in order to dump header\n");
      fprintf(stderr, "v2 B-tree hdr usage:\n");
      fprintf(stderr, "\th5debug <filename> <v2 B-tree header address> <object "
                      "header address>\n");
      exit_value = 4;
      goto done;
    } /* end if */
    ...
    if ((cls == H5D_BT2 || cls == H5D_BT2_FILT) &&
        (extra_count == 0 || extra[0] == 0 || extra[1] == 0 || extra[2] == 0 ||
         extra[3] == 0)) {
      fprintf(stderr, "ERROR: Need v2 B-tree header address, the node's number "
                      "of records, depth, and object "
                      "header address containing the layout message in order "
                      "to dump internal node\n");
      fprintf(stderr, "NOTE: Leaf nodes are depth 0, the internal nodes above "
                      "them are depth 1, etc.\n");
      fprintf(stderr, "v2 B-tree internal node usage:\n");
      fprintf(stderr, "\th5debug <filename> <internal node address> <v2 B-tree "
                      "header address> "
      ...
      goto done;
               ...
               extra[2] == 0) {
      fprintf(stderr, "ERROR: Need v2 B-tree header address and the node's "
    ...
    status = H5B2__int_debug(f, addr, stdout, 0, VCOL, cls, extra[0],
                             (unsigned)extra[1], (unsigned)extra[2],
                             (haddr_t)extra[3]);
    ...
    if ((cls == H5D_BT2 || cls == H5D_BT2_FILT) &&
      ...
      fprintf(
          stderr,
          "ERROR: Need v2 B-tree header address, number of records, and object "
          "header "
          "address containing the layout message in order to dump leaf node\n");
      fprintf(stderr, "v2 B-tree leaf node usage:\n");
      fprintf(stderr, "\th5debug <filename> <leaf node address> <v2 B-tree "
                      "header address> <number "
                      "of records> <object header address>\n");
      exit_value = 4;
      goto done;
    } else if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
      fprintf(stderr, "ERROR: Need v2 B-tree header address and number of "
                      "records in order to dump leaf node\n");
      fprintf(stderr, "v2 B-tree leaf node usage:\n");
      fprintf(stderr, "\th5debug <filename> <leaf node address> <v2 B-tree "
                      "header address> <number of records>\n");
      exit_value = 4;
      goto done;
    } /* end if */

    status = H5B2__leaf_debug(f, addr, stdout, 0, VCOL, cls, extra[0],
                              (unsigned)extra[1], (haddr_t)extra[2]);
    ...
    if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
      fprintf(stderr, "ERROR: Need fractal heap header address and size of "
                      "direct block in order to "
                      "dump direct block\n");
      fprintf(stderr, "Fractal heap direct block usage:\n");
      fprintf(stderr, "\th5debug <filename> <direct block address> <heap "
                      "header address> <size of direct block>\n");
      exit_value = 4;
      goto done;
    } /* end if */

    status =
        H5HF_dblock_debug(f, addr, stdout, 0, VCOL, extra[0], (size_t)extra[1]);
    ...
    /* Check for enough valid parameters */
    if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
      fprintf(stderr, "ERROR: Need fractal heap header address and number of "
                      "rows in order to dump "
                      "indirect block\n");
      fprintf(stderr, "Fractal heap indirect block usage:\n");
      fprintf(stderr, "\th5debug <filename> <indirect block address> <heap "
                      "header address> <number of rows>\n");
      exit_value = 4;
      goto done;
    } /* end if */

    status = H5HF_iblock_debug(f, addr, stdout, 0, VCOL, extra[0],
                               (unsigned)extra[1]);
    ...
    /* Check for enough valid parameters */
    if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
      fprintf(stderr, "ERROR: Need free space header address and client "
                      "address in order to dump "
                      "serialized sections\n");
      fprintf(stderr, "Free space serialized sections usage:\n");
      fprintf(stderr, "\th5debug <filename> <serialized sections address> "
                      "<free space header "
    ...
    /* Check for enough valid parameters */
    if (extra_count == 0 || extra[0] == 0) {
      fprintf(stderr, "ERROR: Need shared message header address in order to "
                      "shared message list\n");
      fprintf(stderr, "Shared message list usage:\n");
      fprintf(stderr, "\th5debug <filename> <shared message list address> "
                      "<shared message header address>\n");
      exit_value = 4;
      goto done;
    } /* end if */
    ...
    /* Check for enough valid parameters */
    if (extra_count == 0 || extra[0] == 0) {
      fprintf(stderr, "ERROR: Need object header address containing the layout "
                      "message in order to dump header\n");
      fprintf(stderr, "Extensible array header block usage:\n");
      fprintf(stderr, "\th5debug <filename> <Extensible Array header address> "
    ...
    if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
      fprintf(stderr,
              "ERROR: Need extensible array header address and object header "
              "address "
              "containing the layout message in order to dump index block\n");
      fprintf(stderr, "Extensible array index block usage:\n");
      fprintf(stderr, "\th5debug <filename> <index block address> <array "
                      "header address> <object header address\n");
      exit_value = 4;
      goto done;
    } /* end if */

    status =
        H5EA__iblock_debug(f, addr, stdout, 0, VCOL, cls, extra[0], extra[1]);
    ...
    if (extra_count == 0 || extra[0] == 0 || extra[1] == 0 || extra[2] == 0) {
      fprintf(stderr, "ERROR: Need extensible array header address, super "
                      "block index and object "
                      "header address containing the layout message in order "
                      "to dump super block\n");
      fprintf(stderr, "Extensible array super block usage:\n");
      fprintf(stderr, "\th5debug <filename> <super block address> <array "
                      "header address> <super "
                      "block index> <object header address>\n");
      exit_value = 4;
      goto done;
    } /* end if */

    status = H5EA__sblock_debug(f, addr, stdout, 0, VCOL, cls, extra[0],
                                (unsigned)extra[1], extra[2]);
    ...
    /* Check for enough valid parameters */
    if (extra_count == 0 || extra[0] == 0 || extra[1] == 0 || extra[2] == 0) {
      fprintf(stderr, "ERROR: Need extensible array header address, # of "
                      "elements in data block and object "
                      "header address containing the layout message in order "
                      "to dump data block\n");
      fprintf(stderr, "Extensible array data block usage:\n");
      fprintf(stderr, "\th5debug <filename> <data block address> <array header "
                      "address> <# of "
    ...
    status = H5EA__dblock_debug(f, addr, stdout, 0, VCOL, cls, extra[0],
                                (size_t)extra[1], extra[2]);
    ...
    if (extra_count == 0 || extra[0] == 0) {
      ...
      fprintf(stderr, "\th5debug <filename> <Fixed Array header address> "
                      "<object header address>\n");
      exit_value = 4;
      goto done;
    } /* end if */
    ...
    /* Check for enough valid parameters */
    if (extra_count == 0 || extra[0] == 0 || extra[1] == 0) {
      fprintf(stderr, "ERROR: Need fixed array header address and object "
                      "header address containing "
                      "the layout message in order to dump data block\n");
      fprintf(stderr, "fixed array data block usage:\n");
      fprintf(stderr, "\th5debug <filename> <data block address> <array header "
                      "address> <object header address>\n");
      exit_value = 4;
      goto done;
    } /* end if */

    status =
        H5FA__dblock_debug(f, addr, stdout, 0, VCOL, cls, extra[0], extra[1]);
