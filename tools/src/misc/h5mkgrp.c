...
static struct h5_long_options l_opts[] = {
    {"help", no_arg, 'h'},           {"latest", no_arg, 'l'},
    {"parents", no_arg, 'p'},        {"verbose", no_arg, 'v'},
    {"version", no_arg, 'V'},        {"vol-value", require_arg, '1'},
    {"vol-name", require_arg, '2'},  {"vol-info", require_arg, '3'},
    {"vfd-value", require_arg, '4'}, {"vfd-name", require_arg, '5'},
    {"vfd-info", require_arg, '6'},  {NULL, 0, '\0'}};
...
static void usage(const char *prog) {
  ...
  PRINTVALSTREAM(rawoutstream, "      -l, --latest       Use latest version of "
                               "file format to create groups\n");
  ...
  PRINTVALSTREAM(rawoutstream,
                 "                         the VOL named by HDF5_VOL_CONNECTOR "
                 "(or the native VOL connector,\n");
...
static int parse_command_line(int argc, const char *const *argv,
  ...
  if (custom_vol || custom_vfd) {
    if ((tmp_fapl_id =
             h5tools_get_fapl(options->fapl_id, custom_vol ? &vol_info : NULL,
...
int main(int argc, char *argv[]) {
  ...
  if (params_g.latest) {
    ...
    if (H5Pset_libver_bounds(params_g.fapl_id, H5F_LIBVER_LATEST,
                             H5F_LIBVER_LATEST) < 0) {
      error_msg(
  ...
  fid = h5tools_fopen(params_g.fname, H5F_ACC_RDWR, params_g.fapl_id,
                      (params_g.fapl_id != H5P_DEFAULT), NULL, 0);
  ...
  if (fid < 0)
    fid =
        H5Fcreate(params_g.fname, H5F_ACC_EXCL, H5P_DEFAULT, params_g.fapl_id);
  ...
  for (curr_group = 0; curr_group < params_g.ngroups; curr_group++) {
    ...
    if ((gid = H5Gcreate2(fid, params_g.groups[curr_group], lcpl_id,
