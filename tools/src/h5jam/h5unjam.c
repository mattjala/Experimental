...
static void usage(const char *prog) {
  fflush(stdout);
  fprintf(stdout,
          "usage: %s -i <in_file.h5>  [-o <out_file.h5> ] [-u <out_user_file> "
          "| --delete]\n",
          prog);
  fprintf(stdout, "\n");
  fprintf(stdout, "Splits user file and HDF5 file into two files: user block "
                  "data and HDF5 data.\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "OPTIONS\n");
  fprintf(stdout, "  -i in_file.h5   Specifies the HDF5 as input.  If the "
                  "input HDF5 file\n");
  fprintf(stdout, "                  contains no user block, exit with an "
                  "error message.\n");
  fprintf(
      stdout,
  ...
  fprintf(stdout, "                  Specifies the output file containing the "
                  "data from the\n");
  fprintf(stdout, "                  user block.\n");
  fprintf(stdout, "                  Cannot be used with --delete option.\n");
  fprintf(stdout, "  --delete        Remove the user block from the input HDF5 "
                  "file. The content\n");
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, "  If neither --delete nor -u is specified, the user block "
                  "from the input file\n");
...
int main(int argc, char *argv[]) {
  ...
  if (copy_to_file(rawinstream, rawdatastream, (ssize_t)usize,
                   (ssize_t)(fsize - (ssize_t)usize)) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t copy_to_file(FILE *infid, FILE *ofid, ssize_t _where,
                    ssize_t show_much) {
  ...
  while (how_much > 0) {
    ...
    if (bytes_wrote != bytes_read ||
        (0 == bytes_wrote && ferror(ofid))) { /* error */
      ret_value = -1;
      goto done;
    } /* end if */
  }   /* end while */
