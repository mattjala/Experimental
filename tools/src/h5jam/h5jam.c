...
static struct h5_long_options l_opts[] = {
    {"help", no_arg, 'h'},   {"i", require_arg, 'i'},  {"u", require_arg, 'u'},
...
static void usage(const char *prog) {
  fflush(stdout);
  fprintf(stdout,
          "usage: %s -i <in_file.h5> -u <in_user_file> [-o <out_file.h5>] "
          "[--clobber]\n",
          prog);
  fprintf(stdout, "\n");
  fprintf(stdout, "Adds user block to front of an HDF5 file and creates a new "
                  "concatenated file.\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "OPTIONS\n");
  fprintf(stdout, "  -i in_file.h5    Specifies the input HDF5 file.\n");
  fprintf(stdout, "  -u in_user_file  Specifies the file to be inserted into "
                  "the user block.\n");
  fprintf(stdout,
          "                   Can be any file format except an HDF5 format.\n");
  fprintf(stdout, "  -o out_file.h5   Specifies the output HDF5 file.\n");
  fprintf(stdout, "                   If not specified, the user block will be "
                  "concatenated in\n");
  fprintf(stdout, "                   place to the input HDF5 file.\n");
  fprintf(stdout, "  --clobber        Wipes out any existing user block before "
                  "concatenating\n");
  fprintf(stdout, "                   the given user block.\n");
  fprintf(stdout, "                   The size of the new user block will be "
                  "the larger of;\n");
  fprintf(stdout, "                    - the size of existing user block in "
                  "the input HDF5 file\n");
  fprintf(stdout, "                    - the size of user block required by "
                  "new input user file\n");
  fprintf(stdout,
...
int main(int argc, char *argv[]) {
  ...
  if (testval > 0) {
    error_msg("-u <user_file> cannot be HDF5 file, but it appears to be an "
              "HDF5 file.\n");
    help_ref_msg(stderr);
    h5tools_setstatus(EXIT_FAILURE);
    goto done;
  ...
  if (output_file == NULL) {
    ...
    ofid = HDopen(output_file, O_WRONLY | O_CREAT | O_TRUNC,
  ...
  if (!do_clobber) {
    where =
        copy_some_to_file(h5fid, ofid, (hsize_t)0, (hsize_t)0, (ssize_t)usize);
  }
 ...
 *-------------------------------------------------------------------------
 */
hsize_t copy_some_to_file(int infid, int outfid, hsize_t starting,
                          hsize_t startout, ssize_t limit) {
  char buf[1024];
  h5_stat_t sbuf;
  int res;
  ssize_t tot = 0;
 ...
 *
 * Return:      Success:    the location of the header == the size of the padded
 *user block. Failure:    none
 *-------------------------------------------------------------------------
 */
