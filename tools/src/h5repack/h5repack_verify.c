...
static int verify_filters(hid_t pid, hid_t tid, int nfilters,
...
int h5repack_verify(const char *in_fname, const char *out_fname,
                    pack_opt_t *options) {
  ...
  H5F_fspace_strategy_t in_strategy,
      out_strategy; /* file space handling strategy for in/output file */
  hbool_t in_persist,
      out_persist; /* free-space persist status for in/output file */
  hsize_t in_threshold,
      out_threshold; /* free-space section threshold for in/output file */
  hsize_t in_pagesize,
      out_pagesize; /* file space page size for input/output file */
  int ret_value = 0;
    ...
    for (i = 0; i < travt->nobjs; i++) {
      ...
      if (travt->objs[i].type == H5TRAV_TYPE_DATASET) {
        ...
        if (options->all_filter == 1) {
          if (verify_filters(pid, tid, options->n_filter_g,
                             options->filter_g) <= 0)
            ok = 0;
        }
  ...
  if (H5Pget_file_space_strategy(fcpl_in, &in_strategy, &in_persist,
                                 &in_threshold) < 0)
    H5TOOLS_GOTO_ERROR((-1), "H5Pget_file_space_strategy failed to retrieve "
                             "file space strategy & threshold");

  /* Get file space page size for input file */
  if (H5Pget_file_space_page_size(fcpl_in, &in_pagesize) < 0)
    H5TOOLS_GOTO_ERROR(
        (-1),
        "H5Pget_file_space_page_size failed to retrieve file space page size");
  ...
  if (H5Pget_file_space_strategy(fcpl_out, &out_strategy, &out_persist,
                                 &out_threshold) < 0)
    H5TOOLS_GOTO_ERROR((-1), "H5Pget_file_space_strategy failed to retrieve "
                             "file space strategy & threshold");
  ...
  /*
   * If -S option is set, the file space handling strategy should be set as
   * specified. If -S option is not set, the file space handling strategy should
   * be the same as the input file's strategy.
   */
  if (options->fs_strategy) {
    if (out_strategy != (options->fs_strategy == (H5F_fspace_strategy_t)-1
                             ? 0
                             : options->fs_strategy))
  ...
  /*
   * If the -P option is set, the free-space persist status should be set as
   * specified. If the -P option is not set, the free-space persist status
   * should be the same as the input file's free-space persist status
   */
  if (options->fs_persist) {
    if (out_persist !=
        (hbool_t)(options->fs_persist == (-1) ? FALSE : options->fs_persist))
      H5TOOLS_GOTO_ERROR((-1),
                         "free-space persist status not set as unexpected");
  } else {
  ...
  if (options->fs_threshold) {
    if (out_threshold !=
        (hsize_t)(options->fs_threshold == (-1) ? 0 : options->fs_threshold))
  ...
  /*
   * If the -G option is set, the file space page size should be set as
   * specified. If the -G option is not set, the file space page size should be
   * the same as the input file's file space page size.
   */
  if (options->fs_pagesize) {
    if (out_pagesize !=
        (hsize_t)(options->fs_pagesize == (-1) ? 0 : options->fs_pagesize))
...
int h5repack_cmp_pl(const char *fname1, hid_t fname1_fapl, const char *fname2,
                    hid_t fname2_fapl) {
  ...
  if ((fid1 = h5tools_fopen(fname1, H5F_ACC_RDONLY, fname1_fapl,
                            (fname1_fapl != H5P_DEFAULT), NULL, 0)) < 0)
    H5TOOLS_GOTO_ERROR((-1), "h5tools_fopen failed <%s>: %s", fname1,
                       H5FOPENERROR);
  if ((fid2 = h5tools_fopen(fname2, H5F_ACC_RDONLY, fname2_fapl,
                            (fname2_fapl != H5P_DEFAULT), NULL, 0)) < 0)
    H5TOOLS_GOTO_ERROR((-1), "h5tools_fopen failed <%s>: %s", fname2,
                       H5FOPENERROR);
  ...
  for (i = 0; i < trav->nobjs; i++) {
    if (trav->objs[i].type == H5TRAV_TYPE_GROUP) {
      if ((gid = H5Gopen2(fid1, trav->objs[i].name, H5P_DEFAULT)) < 0)
        H5TOOLS_GOTO_ERROR((-1), "H5Gopen2 failed on first <%s>",
                           trav->objs[i].name);
      ...
      if ((gid = H5Gopen2(fid2, trav->objs[i].name, H5P_DEFAULT)) < 0)
        H5TOOLS_GOTO_ERROR((-1), "H5Gopen2 failed on second <%s>",
                           trav->objs[i].name);
      if ((gcplid = H5Gget_create_plist(gid)) < 0)
        H5TOOLS_GOTO_ERROR((-1), "H5Gget_create_plist failed");
      ...
      if (crt_order_flag1 != crt_order_flag2)
        H5TOOLS_GOTO_ERROR(0, "property lists failed for <%s> are different",
                           trav->objs[i].name);
    } else if (trav->objs[i].type == H5TRAV_TYPE_DATASET) {
      if ((dset1 = H5Dopen2(fid1, trav->objs[i].name, H5P_DEFAULT)) < 0)
        H5TOOLS_GOTO_ERROR((-1), "H5Dopen2 failed on first <%s>",
                           trav->objs[i].name);
      if ((dset2 = H5Dopen2(fid2, trav->objs[i].name, H5P_DEFAULT)) < 0)
        H5TOOLS_GOTO_ERROR((-1), "H5Dopen2 failed on second <%s>",
                           trav->objs[i].name);
...
static int verify_filters(hid_t pid, hid_t tid, int nfilters,
  ...
  for (i = 0; i < nfilters_dcpl; i++) {
    ...
    filtn = H5Pget_filter2(pid, (unsigned)i, &filt_flags, &cd_nelmts, cd_values,
    ...
    case H5Z_FILTER_SHUFFLE:
      ...
      if (cd_nelmts != H5Z_SHUFFLE_TOTAL_NPARMS &&
          filter[i].cd_nelmts != H5Z_SHUFFLE_USER_NPARMS)
        return 0;
    ...
    case H5Z_FILTER_SZIP:
      ...
      if (cd_nelmts != H5Z_SZIP_TOTAL_NPARMS &&
          filter[i].cd_nelmts != H5Z_SZIP_USER_NPARMS)
        return 0;

      /* "User" parameter for pixels-per-block (index 1) */
      if (cd_values[H5Z_SZIP_PARM_PPB] !=
          filter[i].cd_values[H5Z_SZIP_PARM_PPB])
        return 0;
