...
int h5repack_addfilter(const char *str, pack_opt_t *options) {
  obj_list_t *obj_list =
      NULL;             /* one object list for the -f and -l option entry */
...
int h5repack_addlayout(const char *str, pack_opt_t *options) {
  obj_list_t *obj_list =
      NULL;         /*one object list for the -t and -c option entry */
  ...
  if (options->all_layout == 1) {
    error_msg("invalid layout input: 'all' option is present with other "
              "objects <%s>\n",
              str);
    return ret_value;
  }
...
hid_t copy_named_datatype(hid_t type_in, hid_t fidout,
                          named_dt_t **named_dt_head_p, trav_table_t *travt,
                          pack_opt_t *options) {
      ...
      if (travt->objs[i].type == H5TRAV_TYPE_NAMED_DATATYPE) {
        ...
        *named_dt_head_p = dt;
        ...
        if (H5Otoken_cmp(type_in, &oinfo.token, &dt->obj_token, &token_cmp) < 0)
          H5TOOLS_GOTO_ERROR(H5I_INVALID_HID,
                             "failed to compare object tokens");
        if (!token_cmp)
          dt_ret = dt;
...
int copy_attr(hid_t loc_in, hid_t loc_out, named_dt_t **named_dt_head_p,
  ...
  for (u = 0; u < (unsigned)oinfo.num_attrs; u++) {
    ...
    if (!is_ref) {
      ...
      if ((attr_out = H5Acreate2(loc_out, name, wtype_id, space_id, H5P_DEFAULT,
                                 H5P_DEFAULT)) < 0)
      ...
      /* Check if we have VL data and string in the attribute's  datatype that
       * must be reclaimed */
...
static int check_options(pack_opt_t *options) {
  ...
  for (i = 0; i < options->op_tbl->nelems; i++) {
    ...
    if (options->op_tbl->objs[i].chunk.rank > 0) {
      ...
      has_ck = 1;
    } else if (options->op_tbl->objs[i].chunk.rank ==
               -2) { /* TODO: replace 'magic number' */
      if (options->verbose > 0)
        ...
        if (pack.filter[j].filtn >= 0) {
          if (pack.filter[j].filtn > H5Z_FILTER_SCALEOFFSET) {
            printf(" <%s> with %s filter %d\n", name,
                   get_sfilter(pack.filter[j].filtn), pack.filter[j].filtn);
          } else {
  ...
  if (options->all_filter == 1 && has_cp)
    H5TOOLS_GOTO_ERROR((-1), "invalid compression input: 'all' option is "
                             "present with other objects");
  ...
  if (options->grp_compact < 0)
    H5TOOLS_GOTO_ERROR(
        (-1), "invalid maximum number of links to store as header messages");
  if (options->grp_indexed < 0)
    H5TOOLS_GOTO_ERROR(
        (-1), "invalid minimum number of links to store in the indexed format");
  if (options->grp_indexed > options->grp_compact)
    H5TOOLS_GOTO_ERROR(
        (-1), "minimum indexed size is greater than the maximum compact size");
  for (i = 0; i < 8; i++)
  ...
  if (options->ublock_filename != NULL && options->ublock_size == 0) {
    if (options->verbose > 0) {
      printf("Warning: user block size missing for file %s. Assigning a "
...
static int check_objects(const char *fname, pack_opt_t *options) {
  ...
  if ((fid = h5tools_fopen(fname, H5F_ACC_RDONLY, options->fin_fapl,
                           (options->fin_fapl != H5P_DEFAULT), NULL, 0)) < 0)
    H5TOOLS_GOTO_ERROR((-1), "h5tools_fopen failed <%s>: %s", fname,
                       H5FOPENERROR);
  ...
  for (i = 0; i < options->op_tbl->nelems; i++) {
    ...
    for (ifil = 0; ifil < obj.nfilters; ifil++) {
      ...
      case H5Z_FILTER_SZIP: {
        ...
        if (csize < ppb) {
          printf(" <warning: SZIP settings, chunk size is smaller than pixels "
                 "per block>\n");
          H5TOOLS_GOTO_DONE(0);
        }
      } /* end case SZIP */
