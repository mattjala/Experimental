...
static void aux_tblinsert_filter(pack_opttbl_t *table, unsigned int I,
  ...
  if (table->objs[I].nfilters < H5_REPACK_MAX_NFILTERS)
    ...
    H5TOOLS_INFO(
        "cannot insert the filter in this object. Maximum capacity exceeded");
}
...
static int aux_inctable(pack_opttbl_t *table, unsigned n_objs) {
  ...
  table->size += n_objs;
  table->objs =
      (pack_info_t *)realloc(table->objs, table->size * sizeof(pack_info_t));
  if (table->objs == NULL) {
...
int options_table_init(pack_opttbl_t **tbl) {
  ...
  table->nelems = 0;
  if (NULL == (table->objs =
                   (pack_info_t *)malloc(table->size * sizeof(pack_info_t)))) {
    free(table);
    H5TOOLS_GOTO_ERROR((-1), "not enough memory for options table");
  }
...
int options_add_layout(obj_list_t *obj_list, unsigned n_objs, pack_info_t *pack,
  ...
  if (table->nelems > 0) {
    ...
    for (j = 0; j < n_objs; j++) {
      ...
      /* cases where we have an already inserted name but there is a new name
       also example: -f dset1:GZIP=1 -l dset1,dset2:CHUNK=20x20 dset1 is already
       inserted, but dset2 must also be
       */
...
int options_add_filter(obj_list_t *obj_list, unsigned n_objs,
  ...
  if (table->nelems > 0) {
    ...
    for (j = 0; j < n_objs; j++) {
      ...
      /* cases where we have an already inserted name but there is a new name
       also example: -l dset1:CHUNK=20x20 -f dset1,dset2:GZIP=1 dset1 is already
       inserted, but dset2 must also be
       */
      else if (found && HDstrcmp(obj_list[j].obj, table->objs[i].path) != 0) {
        /* keep the grow in a temp var */
