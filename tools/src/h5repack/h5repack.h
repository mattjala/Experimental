...
#define MAX_COMPACT_DSIZE                                                      \
  64512 /* max data size for compact layout. -1k for header size */

/* timing formats */
#define FORMAT_OBJ_TIME " %-27s  %e/%e   %s\n"       /* obj type, name */
#define FORMAT_OBJ_ATTR_TIME "  %-27s  %e/%e   %s\n" /* obj type, name */
#define FORMAT_OBJ_NOTIME                                                      \
  " %-27s                              %s\n" /* obj type, name */
#define FORMAT_OBJ_ATTR_NOTIME                                                 \
  "  %-27s                              %s\n" /* obj type, name */
...
typedef struct {
  ...
  filter_info_t filter_g[H5_REPACK_MAX_NFILTERS]; /*global filter array for the
                                                     ALL case */
  ...
  H5F_libver_t
      low_bound; /* The file's low bound as in H5Fset_libver_bounds() */
  H5F_libver_t
      high_bound;  /* The file's high bound as in H5Fset_libver_bounds() */
  hid_t fin_fapl;  /* FAPL to use for opening the input file */
  hid_t fout_fapl; /* FAPL to use for opening/creating the output file */
  int grp_compact; /* Set the maximum number of links to store as header
                      messages in the group */
  int grp_indexed; /* Set the minimum number of links to store in the indexed
                      format */
  int msg_size[8]; /* Minimum size of shared messages: dataspace,
                      datatype, fill value, filter pipeline, attribute */
  const char *ublock_filename; /* user block file name */
  hsize_t ublock_size;         /* user block size */
  hsize_t meta_block_size;     /* metadata aggregation block size (for
...
int options_add_layout(obj_list_t *obj_list, unsigned n_objs, pack_info_t *pack,
                       pack_opttbl_t *table);
int options_add_filter(obj_list_t *obj_list, unsigned n_objs,
                       filter_info_t filt, pack_opttbl_t *table);
...
obj_list_t *parse_layout(const char *str, unsigned *n_objs,
