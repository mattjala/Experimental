 ...
 *-------------------------------------------------------------------------
 */
static int get_hyperslab(hid_t dcpl_id, int rank_dset,
                         const hsize_t dims_dset[], size_t size_datum,
                         hsize_t dims_hslab[], hsize_t *hslab_nbytes_p);
static void print_dataset_info(hid_t dcpl_id, char *objname, double per, int pr,
                               pack_opt_t *options, double read_time,
                               double write_time);
 ...
 *-------------------------------------------------------------------------
 */
int copy_objects(const char *fnamein, const char *fnameout,
                 pack_opt_t *options) {
  hid_t fidin = H5I_INVALID_HID;
  hid_t fidout = H5I_INVALID_HID;
  hid_t fcpl_in =
      H5I_INVALID_HID; /* file creation property list ID for input file */
  ...
  hsize_t
      set_threshold; /* Free-space section threshold to be set in output file */
  ...
  if ((fidin = h5tools_fopen(fnamein, H5F_ACC_RDONLY, options->fin_fapl,
                             (options->fin_fapl != H5P_DEFAULT), NULL,
                             (size_t)0)) < 0)
    H5TOOLS_GOTO_ERROR((-1), "h5tools_fopen failed <%s>: %s", fnamein,
                       H5FOPENERROR);
  ...
  /* get user block size and file space strategy/persist/threshold */
  {
    if ((fcpl_in = H5Fget_create_plist(fidin)) < 0)
      H5TOOLS_GOTO_ERROR(
          (-1),
          "H5Fget_create_plist failed to retrieve file creation property list");

    if (H5Pget_userblock(fcpl_in, &ub_size) < 0)
      H5TOOLS_GOTO_ERROR((-1),
                         "H5Pget_userblock failed to retrieve userblock size");

    /* If the -S option is not set, get "strategy" from the input file */
    if (H5Pget_file_space_strategy(fcpl_in, &in_strategy, &in_persist,
                                   &in_threshold) < 0)
      H5TOOLS_GOTO_ERROR(
          (-1),
          "H5Pget_file_space_strategy failed to retrieve file space strategy");

    /* If the -G option is not set, get "pagesize" from the input file */
    if (H5Pget_file_space_page_size(fcpl_in, &in_pagesize) < 0)
      H5TOOLS_GOTO_ERROR((-1), "H5Pget_file_space_page_size failed to retrieve "
  ...
  if (options->fout_fapl == H5P_DEFAULT)
    if ((options->fout_fapl = H5Pcreate(H5P_FILE_ACCESS)) < 0)
      H5TOOLS_GOTO_ERROR(
          (-1), "H5Pcreate failed to create file access property list");

  /* It can be default, latest or other settings by users */
  if (H5Pset_libver_bounds(options->fout_fapl, options->low_bound,
                           options->high_bound) < 0)
    H5TOOLS_GOTO_ERROR(
        (-1), "H5Pset_libver_bounds failed to set format version bounds");
  ...
  if (options->low_bound >= H5F_LIBVER_V18 || ub_size > 0) {
    ...
    if ((fcpl = H5Pcreate(H5P_FILE_CREATE)) < 0)
      H5TOOLS_GOTO_ERROR(
          (-1), "H5Pcreate failed to create a file creation property list");

    if (ub_size > 0)
      if (H5Pset_userblock(fcpl, ub_size) < 0)
        H5TOOLS_GOTO_ERROR(
            (-1), "H5Pset_userblock failed to set non-default userblock size");
    ...
    if (options->low_bound >= H5F_LIBVER_V18) {
      ...
      if (H5Pset_link_phase_change(fcpl, (unsigned)options->grp_compact,
        ...
        H5TOOLS_GOTO_ERROR((-1), "H5Pset_link_phase_change failed to adjust "
                                 "group creation parameters for root group");
      ...
      if (nindex > 0) {
        ...
        /* msg_size[0]=dataspace, 1=datatype, 2=file value, 3=filter pipeline,
         * 4=attribute */
  ...
  if (options->alignment > 0) {
    ...
    if (H5Pset_alignment(options->fout_fapl, options->threshold,
                         options->alignment) < 0)
  ...
  if (options->meta_block_size > 0) {
    ...
    if (H5Pset_meta_block_size(options->fout_fapl, options->meta_block_size) <
        0)
      H5TOOLS_GOTO_ERROR(
          (-1), "H5Pset_meta_block_size failed to set metadata block size");
  }
  ...
  if (options->fs_strategy ==
      (H5F_fspace_strategy_t)-1) /* A default strategy is specified by user */
    set_strategy = FS_STRATEGY_DEF;
  else if (options->fs_strategy !=
           (H5F_fspace_strategy_t)0) /* Set strategy as specified by user */
  ...
  if (H5Pset_file_space_strategy(fcpl, set_strategy, set_persist,
                                 set_threshold) < 0)
    H5TOOLS_GOTO_ERROR(
        (-1), "H5Pset_file_space_strategy failed to set file space strategy");

  if (options->fs_pagesize ==
      -1) /* A "0" file space page size is specified by user */
    set_pagesize = (hsize_t)0;
  else if (options->fs_pagesize !=
           0) /* Set file space page size as specified by user */
    set_pagesize = (hsize_t)options->fs_pagesize;

  if (set_pagesize !=
      FS_PAGESIZE_DEF) /* Set non-default file space page size as specified */
    if (H5Pset_file_space_page_size(fcpl, set_pagesize) < 0)
      H5TOOLS_GOTO_ERROR(
          (-1),
          "H5Pset_file_space_page_size failed to set file space page size");
  ...
  if ((fidout = H5Fcreate(fnameout, H5F_ACC_TRUNC, fcpl, options->fout_fapl)) <
      0)
  ...
  if (travt) {
    ...
    if (do_copy_objects(fidin, fidout, travt, options) < 0)
      H5TOOLS_GOTO_ERROR(
          (-1), "do_copy_objects from <%s> could not copy data to <%s>",
          fnamein, fnameout);
    ...
    if (do_copy_refobjs(fidin, fidout, travt, options) < 0)
      H5TOOLS_GOTO_ERROR(
          (-1), "do_copy_refobjs from <%s> could not copy data to <%s>",
          fnamein, fnameout);
  }
...
int get_hyperslab(hid_t dcpl_id, int rank_dset, const hsize_t dims_dset[],
                  size_t size_datum, hsize_t dims_hslab[],
  ...
  hsize_t nchunk_fit; /* number of chunks that fits in hyperslab buffer
                         (H5TOOLS_BUFSIZE) */
  hsize_t ndatum_fit; /* number of datum that fits in hyperslab buffer
                         (H5TOOLS_BUFSIZE) */
  ...
  if (dset_layout == H5D_CHUNKED) {
    ...
    /* 2. if a chunk is bigger than hyperslab buffer */
      ...
      for (k = rank_dset; k > 0; --k) {
        ...
        if (hslab_nbytes <= 0)
          H5TOOLS_GOTO_ERROR((-1),
                             "calculate total size for the hyperslab failed");
      }
    }
  }
 ...
 * Return:   0, ok, -1 no
 *
 *  A threshold of H5TOOLS_MALLOCSIZE (128 MB) is the limit upon which I/O
 *hyperslab is done i.e., if the memory needed to read a dataset is greater than
 *this limit, then hyperslab I/O is done instead of one operation I/O For each
 *dataset, the memory needed is calculated according to
 *
 *  memory needed = number of elements * size of each element
 *
 *  if the memory needed is lower than H5TOOLS_MALLOCSIZE, then the following
 *operations are done
 *
 *  H5Dread( input_dataset1 )
 *  H5Dread( input_dataset2 )
 *
 *  with all elements in the datasets selected. If the memory needed is greater
 *than H5TOOLS_MALLOCSIZE, then the following operations are done instead:
 *
 *  a strip mine is defined for each dimension k (a strip mine is defined as a
 *  hyperslab whose size is memory manageable) according to the formula
 *
 *  (1) strip_mine_size[k ] = MIN(dimension[k ], H5TOOLS_BUFSIZE / size of
 *memory type)
 *
 *  where H5TOOLS_BUFSIZE is a constant currently defined as 1MB. This formula
 *assures that for small datasets (small relative to the H5TOOLS_BUFSIZE
 *constant), the strip mine size k is simply defined as its dimension k, but for
 *larger datasets the hyperslab size is still memory manageable. a cycle is done
 *until the number of elements in the dataset is reached. In each iteration, two
 *parameters are defined for the function H5Sselect_hyperslab, the start and
 *size of each hyperslab, according to
 *
 *  (2) hyperslab_size [k] = MIN(dimension[k] - hyperslab_offset[k],
 *strip_mine_size [k])
 *
 *  where hyperslab_offset [k] is initially set to zero, and later incremented
 *in hyperslab_size[k] offsets. The reason for the operation
 *
 *  dimension[k] - hyperslab_offset[k]
 *
 *  in (2) is that, when using the strip mine size, it assures that the
 *"remaining" part of the dataset that does not fill an entire strip mine is
 *processed.
 *
 ...
 *     Selection would be same as the hyperslab except for the remaining edge
 *portion of the dataset. The code take care of the remaining portion if exist.
 *
 *-------------------------------------------------------------------------
 */
                    ...
                    pack_opt_t *options) /* repack options */
{
  ...
  named_dt_t *named_dt_head =
      NULL;       /* Pointer to the stack of named datatypes copied */
  size_t msize;   /* size of type */
  hsize_t nelmts; /* number of elements in dataset */
  H5D_space_status_t space_status; /* determines whether space has been
    ...
    for (i = 0; i < travt->nobjs; i++) {
      ...
      case H5TRAV_TYPE_GROUP:
        ...
        if (HDstrcmp(travt->objs[i].name, "/") == 0) {
          ...
          if ((grp_out = H5Gcreate2(fidout, travt->objs[i].name, H5P_DEFAULT,
                                    gcpl_out, H5P_DEFAULT)) < 0)
      ...
      case H5TRAV_TYPE_DATASET: {
        ...
        if (options->op_tbl->objs) {
          for (u = 0; u < options->op_tbl->nelems; u++) {
            if (HDstrcmp(travt->objs[i].name, options->op_tbl->objs[u].path) ==
                0)
              for (ifil = 0; ifil < options->op_tbl->objs[ifil].nfilters;
                   ifil++) {
        ...
        if (is_named)
          if ((wtype_id = copy_named_datatype(ftype_id, fidout, &named_dt_head,
                                              travt, options)) < 0)
         ...
         *
         * Note that H5Ocopy is currently unsafe to use for objects that reside
         *in or interact with global heaps, such as variable-length datatypes.
         *This appears to be due to H5Ocopy not correctly translating in the
         *case where these objects move to different global heap addresses in
         *the repacked file.
         *-------------------------------------------------------------------------
         */
        use_h5ocopy =
            !(options->op_tbl->nelems || options->all_filter == 1 ||
              options->all_layout == 1 || is_ref || is_vlen || is_named);

        /*
         * Check if we are using different source and destination VOL
         * connectors. In this case, we currently have to avoid usage of H5Ocopy
         * since it doesn't support this.
         */
        if (use_h5ocopy && (options->fin_fapl != H5P_DEFAULT ||
                            options->fout_fapl != H5P_DEFAULT)) {
          hid_t in_vol_id;
          hid_t out_vol_id;
        ...
        if (!use_h5ocopy) {
           ...
           * Accordingly, there would be no filter or chunk properties to
           * preserve, so create a new DCPL. Otherwise, copy dcpl_in.
           */
          ...
          if (h5tools_canreadf((travt->objs[i].name), dcpl_in) == 1) {
             ...
             * we cannot just copy the buffers, but instead we recreate the
             *reference in a second traversal of the output file
             *-------------------------------------------------------------------------
             */
            if (H5T_REFERENCE != H5Tget_class(wtype_id)) {
              ...
              /* apply the filter */
              if (apply_s)
                if (apply_filters(travt->objs[i].name, rank, dims, msize,
                                  dcpl_out, options, &has_filter) < 0)
              ...
              } /* end if layout change requested for entire file or individual
                   object */

              /*-------------------------------------------------------------------------
               * create the output dataset;
               * disable error checking in case the dataset cannot be created
               *with the modified dcpl; in that case use the original instead
               *-------------------------------------------------------------------------
               */
              dset_out =
              ...
              if (dset_out == H5I_INVALID_HID) {
                ...
                if (options->verbose > 0)
                  printf(" warning: could not create dataset <%s>. Applying "
                         "original "
                         "settings\n",
                         travt->objs[i].name);

                if ((dset_out = H5Dcreate2(fidout, travt->objs[i].name,
                                           wtype_id, f_space_id, H5P_DEFAULT,
               ...
               */
              if (nelmts > 0 &&
                  space_status != H5D_SPACE_STATUS_NOT_ALLOCATED) {
                size_t need = (size_t)(nelmts * msize); /* bytes needed */

                /* have to read the whole dataset if there is only one element
                 * in the dataset */
                ...
                /* Set up collective write if using filters in parallel */
                {
                  ...
                  hbool_t parallel =
                      (H5FD_MPIO == H5Pget_driver(options->fout_fapl));
                ...
                if (buf != NULL) {
                  ...
                  if (H5Dread(dset_in, wtype_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                              buf) < 0)
                  ...
                  if (H5Dwrite(dset_out, wtype_id, H5S_ALL, H5S_ALL, dxpl_id,
                               buf) < 0)
                  ...
                  if (get_hyperslab(dcpl_tmp, rank, dims, p_type_nbytes,
                                    hslab_dims, &hslab_nbytes) < 0)
                    H5TOOLS_GOTO_ERROR((-1), "get_hyperslab failed");

                  hslab_buf = malloc((size_t)hslab_nbytes);
                  if (hslab_buf == NULL)
                    H5TOOLS_GOTO_ERROR((-1),
                                       "can't allocate space for hyperslab");
                  ...
                  for (elmtno = 0; elmtno < p_nelmts;
                       elmtno += hs_select_nelmts) {
                    if (rank > 0) {
              ...
              if (options->verbose > 0) {
                ...
                /* only print the compression ratio if there was a filter
                 * request */
                ...
                if (has_filter && apply_s == 0)
                  printf(" <warning: filter not applied to %s. dataset smaller "
                         "than "
                         "%d bytes>\n",
         ...
         */
          ...
          if (H5Ocopy(fidin, /* Source file or group identifier */
                      travt->objs[i]
                          .name, /* Name of the source object to be copied */
          ...
          if ((dset_in = H5Dopen2(fidin, travt->objs[i].name, H5P_DEFAULT)) < 0)
            H5TOOLS_GOTO_ERROR((-1), "H5Dopen2 failed");
          if ((dset_out = H5Dopen2(fidout, travt->objs[i].name, H5P_DEFAULT)) <
              0)
            H5TOOLS_GOTO_ERROR((-1), "H5Dopen2 failed");
          ...
          if (options->verbose > 0) {
            if (options->verbose == 2)
              printf(FORMAT_OBJ_TIME, "dset", 0.0, write_time,
                     travt->objs[i].name);
            else
      ...
      case H5TRAV_TYPE_NAMED_DATATYPE:
        ...
        if (H5Tclose(type_out) < 0)
          H5TOOLS_GOTO_ERROR((-1), "H5Tclose failed");
        type_out = H5I_INVALID_HID; /* named datatypes stack, named_dt_head,
                                       manages allocation */
      ...
      case H5TRAV_TYPE_UDLINK:
        ...
        /* Check -X option. */
        if (options->merge) {
          if (H5tools_get_symlink_info(fidin, travt->objs[i].name, &linkinfo,
                                       1) == 0) {
            /* dangling link */
            if (options->prune) {
              printf("Pruned %s.\n", travt->objs[i].name);
            ...
            if ((lcpl_id = H5Pcreate(H5P_LINK_CREATE)) < 0) {
              H5TOOLS_GOTO_ERROR((-1),
                                 "H5Pcreate link creation property failed");
            }
            ...
            if (H5Ocopy(fidin, travt->objs[i].name, fidout, travt->objs[i].name,
                        ocpl_id, lcpl_id) < 0)
              H5TOOLS_GOTO_ERROR((-1), "H5Ocopy failed");
...
static void print_dataset_info(hid_t dcpl_id, char *objname, double ratio,
                               int pr, pack_opt_t *options, double read_time,
                               double write_time) {
  char strfilter[255];
  ...
  for (i = 0; i < nfilters; i++) {
    ...
    if ((filtn = H5Pget_filter2(dcpl_id, (unsigned)i, &filt_flags, &cd_nelmts,
                                cd_values, sizeof(f_objname), f_objname,
                                NULL)) < 0) {
...
static int copy_user_block(const char *infile, const char *outfile,
  ...
  /* User block must be any power of 2 equal to 512 or greater (512, 1024, 2048,
   * etc.) */
...
static void print_user_block(const char *filename, hid_t fid) {
  ...
  hid_t fcpl =
      H5I_INVALID_HID; /* file creation property list ID for HDF5 file */
  int i;
  int ret_value = 0;
