  ...
  for (i = (size_t)(end_obj + 1), k = 0, j = 0; i < len; i++, k++) {
    ...
    if (c == '=' || i == len - 1) {
      if (c == '=') {    /*one more parameter */
         ...
         * pixels per block is a even number in 2-32 and coding method is 'EC'
         *or 'NN' example SZIP=8,NN
         *-------------------------------------------------------------------------
         */
         ...
         *   float datatype using E-scaling method, H5Z_SO_FLOAT_ESCALE  (ES) ,
         *not yet implemented for integer datatypes, scale_factor denotes
         *Minimum Bits for float datatypes, scale_factor denotes decimal scale
         *factor examples SOFF=31,IN SOFF=3,DF
         *-------------------------------------------------------------------------
         */
