             ...
             pack_info_t *objout /*OUT*/) /* info about object to filter */
{
  ...
  for (i = 0; i < nfilters; i++) {
    if ((objout->filter[i].filtn = H5Pget_filter2(
             dcpl_id, (unsigned)i, &objout->filter[i].filt_flag,
             &objout->filter[i].cd_nelmts, objout->filter[i].cd_values,
  ...
  if (filtobj.nfilters) {
    for (i = 0; i < filtobj.nfilters; i++) {
      ...
      if (H5Zfilter_avail(filtobj.filter[i].filtn) <= 0)
        H5TOOLS_GOTO_ERROR((-1), "%d filter unavailable",
                           filtobj.filter[i].filtn);
    } /* for */
  }   /* nfilters */
