...
static int copy_refs_attr(hid_t loc_in, hid_t loc_out, trav_table_t *travt,
                          hid_t fidout);
static herr_t update_ref_value(hid_t obj_id, H5R_type_t ref_type, void *ref_in,
                               hid_t fid_out, void *ref_out,
                               trav_table_t *travt);
...
int do_copy_refobjs(hid_t fidin, hid_t fidout, trav_table_t *travt,
      ...
      if (travt->objs[i].nlinks)
        for (j = 0; j < travt->objs[i].nlinks; j++)
          H5Lcreate_hard(fidout, travt->objs[i].name, H5L_SAME_LOC,
                         travt->objs[i].links[j].new_name, H5P_DEFAULT,
                         H5P_DEFAULT);
      break;
      ...
      if (h5tools_canreadf(NULL, dcpl_id) == 1) {
        ...
        if (H5Tequal(mtype_id, H5T_STD_REF_OBJ)) {
          ...
          if (nelmts) {
            ...
            if (H5Dread(dset_in, mtype_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) <
                0)
            ...
            for (u = 0; u < nelmts; u++) {
              H5E_BEGIN_TRY {
                if ((refobj_id = H5Rdereference2(dset_in, H5P_DEFAULT,
                                                 H5R_OBJECT, &buf[u])) < 0)
                  continue;
              }
              H5E_END_TRY
              ...
              if ((refname = MapIdToName(refobj_id, travt)) != NULL) {
                ...
                if (H5Rcreate(&refbuf[u], fidout, refname, H5R_OBJECT,
                              (hid_t)-1) < 0)
           ...
           */
          if ((dset_out =
                   H5Dcreate2(fidout, travt->objs[i].name, mtype_id, space_id,
                              H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0)
            H5TOOLS_GOTO_ERROR((-1), "H5Dcreate2 failed");
          if (nelmts)
            if (H5Dwrite(dset_out, mtype_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                         refbuf) < 0)
         ...
         */
          ...
          hid_t refobj_id = H5I_INVALID_HID;
          hdset_reg_ref_t *refbuf =
              NULL;                    /* input buffer for region references */
          ...
          if (nelmts) {
             ...
             *-------------------------------------------------------------------------
             */
            refbuf = (hdset_reg_ref_t *)calloc(
                sizeof(hdset_reg_ref_t), (size_t)nelmts); /*init to zero */
            if (refbuf == NULL) {
              printf("cannot allocate memory\n");
              H5TOOLS_GOTO_ERROR((-1), "calloc failed");
            }
 ...
 *
 * Modified:
 *           Update values of references(object and region) for the following
 *types: 1) References, 2) ARRAY of reference, 3) VLEN of references. 4)
 *COMPOUND of references. This function does not handle references in other
 *complicated structures, such as references in nested compound datatypes.
 *-------------------------------------------------------------------------
 */
                          ...
                          hid_t fidout) /* for saving references */
{
  ...
  for (u = 0; u < (unsigned)oinfo.num_attrs; u++) {
    ...
    /* open attribute */
    if ((attr_id = H5Aopen_by_idx(loc_in, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                                  (hsize_t)u, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    ...
    if (type_class == H5T_VLEN) {
      ...
      ref_comp_size =
          (size_t *)malloc((size_t)nmembers * sizeof(ref_comp_size));
      ref_comp_field_n = 0;
      ...
      /* This line below needs to be moved in this loop instead of inserting
         outside. Otherwise, ref_comp_field_n may be >0 for the next attribute,
         which may not be the reference type and will be accidentally treated as
         the reference type. It will then cause the H5Acreate2 failed since that
         attribute is already created. KY 2020-02-07
      */
    ...
    if ((attr_out = H5Acreate2(loc_out, name, ftype_id, space_id, H5P_DEFAULT,
                               H5P_DEFAULT)) < 0)
    ...
    if (nelmts > 0) {
      ...
      if ((is_ref || is_ref_array) && (H5R_OBJ_REF_BUF_SIZE == msize)) {
        ...
        for (i = 0; i < (unsigned)nelmts; i++)
          if (update_ref_value(attr_id, H5R_OBJECT, &((hobj_ref_t *)buf)[i],
                               fidout, &((hobj_ref_t *)refbuf)[i], travt) < 0)
            continue;
      } /* H5T_STD_REF_OBJ */
      /* handle region references */
      else if ((is_ref || is_ref_array) &&
               (H5R_DSET_REG_REF_BUF_SIZE == msize)) {
        buf = (hdset_reg_ref_t *)malloc((unsigned)(nelmts * msize));
        ...
        if (H5R_OBJ_REF_BUF_SIZE == msize) {
          ...
          for (i = 0; i < (unsigned)nelmts; i++) {
            ...
            for (j = 0; j < ((hvl_t *)buf)[i].len; j++) {
              if (update_ref_value(attr_id, H5R_OBJECT, &(ptr[j]), fidout,
                                   &ref_out, travt) < 0)
                continue;
          ...
          for (i = 0; i < (unsigned)nelmts; i++) {
            ...
            for (j = 0; j < ((hvl_t *)buf)[i].len; j++) {
              if (update_ref_value(attr_id, H5R_DATASET_REGION, &(ptr[j]),
                                   fidout, &ref_out, travt) < 0)
                continue;
              memcpy(&(ptr[j]), &ref_out, msize);
            }
          } /* for (i=0; i<nelems; i++) */
        ...
        for (i = 0; i < (unsigned)nelmts; i++) {
          for (j = 0; j < (unsigned)ref_comp_field_n; j++) {
            if (ref_comp_size[j] == H5R_OBJ_REF_BUF_SIZE) {
              size_t idx = (i * msize) +
                           H5Tget_member_offset(mtype_id, ref_comp_index[j]);
              hobj_ref_t ref_out;
              ...
              size_t idx =
                  i * msize + H5Tget_member_offset(mtype_id, ref_comp_index[j]);
              hdset_reg_ref_t ref_out;

              if (update_ref_value(attr_id, H5R_DATASET_REGION,
                                   (hdset_reg_ref_t *)(((char *)buf) + idx),
                                   fidout, &ref_out, travt) < 0)
                continue;
              memcpy(((char *)buf) + idx, &ref_out, ref_comp_size[j]);
            } /* else if */
    ...
    if (refbuf == buf)
      refbuf = NULL; /* set it to NULL to avoid double free since buf and refbuf
                        are the same. */
...
static const char *MapIdToName(hid_t refobj_id, trav_table_t *travt) {
  ...
  for (u = 0; u < travt->nobjs; u++) {
    if (travt->objs[u].type == (h5trav_type_t)H5O_TYPE_DATASET ||
      ...
      if (H5Otoken_cmp(refobj_id, &ref_oinfo.token, &travt->objs[u].obj_token,
                       &token_cmp) < 0)
        goto out;
      if (!token_cmp) {
