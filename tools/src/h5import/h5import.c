...
int main(int argc, char *argv[]) {
  ...
  const char *err9 =
      "Cannot specify more than 30 input files in one call to h5import.\n";
...
static int processDataFile(char *infile, struct Input *in, hid_t file_id) {
   ...
   *-------------------------------------------------------------------------
   */
  if (in->inputClass == 4 /* "IN" */ || in->inputClass == 3 /* "FP" */ ||
      in->inputClass == 7 /* "UIN" */) {
  ...
  case 16:
    ...
    case 4: /* IN */
      for (i = 0; i < len; i++, in16++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *in16 = swap_int16(temp16); else
        */
        *in16 = temp16;
#ifdef H5DEBUGIMPORT
        printf("readIntegerData %d (0x%.8X)\n", *in16, temp16);
  ...
  case 32:
    ...
    case 4: /* IN */
      for (i = 0; i < len; i++, in32++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *in32 = swap_int32(temp32); else
        */
        *in32 = temp32;
#ifdef H5DEBUGIMPORT
  ...
  case 64:
    ...
    case 4: /* IN */
      for (i = 0; i < len; i++, in64++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *in64 = swap_int64(temp64); else
        */
        *in64 = temp64;
#ifdef H5DEBUGIMPORT
        printf("readIntegerData %d (0x%.8X)\n", *in64, temp64);
  ...
  case 16:
    ...
    case 7: /* UIN */
      for (i = 0; i < len; i++, in16++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *in16 = swap_uint16(temp16); else
        */
        *in16 = temp16;
#ifdef H5DEBUGIMPORT
  ...
  case 32:
    ...
    case 7: /* UIN */
      for (i = 0; i < len; i++, in32++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *in32 = swap_uint32(temp32); else
        */
        *in32 = temp32;
#ifdef H5DEBUGIMPORT
  ...
  case 64:
    ...
    case 7: /* UIN */
      for (i = 0; i < len; i++, in64++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *in64 = swap_uint64(temp64); else
        */
        *in64 = temp64;
#ifdef H5DEBUGIMPORT
  ...
  case 32:
    ...
    case 3: /* FP */
      ...
      for (i = 0; i < len; i++, bfp32++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *bfp32 = swap_uint32(temp32); else
        */
        *bfp32 = temp32;
#ifdef H5DEBUGIMPORT
  ...
  case 64:
    ...
    case 3: /* FP */
      ...
      for (i = 0; i < len; i++, bfp64++) {
        ...
        /*
                        if (in-> h5dumpInput && (in->inputByteOrder !=
           in->outputByteOrder)) *bfp64 = swap_uint64(temp64); else
        */
        *bfp64 = temp64;
#ifdef H5DEBUGIMPORT
...
static int processStrData(FILE *strm, struct Input *in, hid_t file_id) {
    ...
    if (in->path.count > 1) {
      ...
      while (j < in->path.count - 1) {
        if ((group_id = H5Gopen2(handle, in->path.group[j], H5P_DEFAULT)) < 0) {
          group_id = H5Gcreate2(handle, in->path.group[j++], H5P_DEFAULT,
                                H5P_DEFAULT, H5P_DEFAULT);
          for (; j < in->path.count - 1; j++)
            group_id = H5Gcreate2(group_id, in->path.group[j], H5P_DEFAULT,
                                  H5P_DEFAULT, H5P_DEFAULT);
          handle = group_id;
          break;
        }
  ...
  if ((dset_id = H5Dcreate2(handle, in->path.group[j], type_id, space_id,
  ...
  while (EOF != (c = HDfgetc(strm))) {
    ...
    if (c == 10) { /* eol */
      ...
      if (H5Dwrite(dset_id, type_id, mspace_id, fspace_id, H5P_DEFAULT, &str2) <
          0)
        goto out;
      ...
      if (str3 != NULL) {
        ...
        *str3 = '\0';
        ...
        if (HDstrlen(str2) > 0) {
          ...
          if (H5Dwrite(dset_id, type_id, mspace_id, fspace_id, H5P_DEFAULT,
                       &str2) < 0)
            goto out;
...
static int allocateFloatStorage(struct Input *in) {
  ...
  switch (in->inputSize) {
  case 32:
    if ((in->data = (VOIDP)malloc((size_t)len * sizeof(H5DT_FLOAT32))) ==
        NULL) {
      (void)fprintf(stderr, "%s", err1);
      return (-1);
    }
  ...
  case 64:
    if ((in->data = (VOIDP)malloc((size_t)len * sizeof(H5DT_FLOAT64))) ==
        NULL) {
      (void)fprintf(stderr, "%s", err1);
      return (-1);
    }
...
static int processConfigurationFile(char *infile, struct Input *in) {
  ...
  const char *err1 =
      "Unable to open the configuration file:  %s for reading.\n";
  ...
  const char *err7b =
      "DIMENSION-SIZES cannot appear before RANK is provided.\n";
  ...
  const char *err12b = "CHUNKED-DIMENSION-SIZES cannot appear before "
                       "DIMENSION-SIZES are provided.\n";
  const char *err12c =
      "Error in retrieving the chunked dimension sizes from %s.\n";
  ...
  const char *err14b =
      "Error in retrieving the compression parameter from %s.\n";
  const char *err15a = "EXTERNAL-STORAGE keyword appears twice in %s.\n";
  const char *err15b =
      "Error in retrieving the external storage parameters from %s.\n";
  const char *err16a = "MAXIMUM-DIMENSIONS keyword appears twice in %s.\n";
  const char *err16b =
      "MAXIMUM-DIMENSIONS cannot appear before DIMENSION-SIZES are provided.\n";
  const char *err16c =
      "Error in retrieving the maximum dimension sizes from %s.\n";
      ...
      if (!HDstrcmp("DATASET", key)) { /* PATH */
        ...
        if (!HDstrcmp("SCALAR", temp)) { /* SCALAR */
          ...
          if (!HDstrcmp("(", temp)) { /* start paren */
            ...
            if (HDfscanf(strm, "%254s", temp) !=
                1) { /* Dimension with optional comma */
            ...
            if ((in->sizeOfDimension = (hsize_t *)malloc(
                     (size_t)in->rank * sizeof(hsize_t))) == NULL) {
              goto error;
            }
#ifdef H5DEBUGIMPORT
          ...
          if (!HDstrcmp("/", temp)) { /* / max dims */
            if ((in->maxsizeOfDimension = (hsize_t *)malloc(
                     (size_t)in->rank * sizeof(hsize_t))) == NULL) {
              goto error;
            }
            if (HDfscanf(strm, "%254s", temp) != 1) { /* start paren */
        ...
        if (!HDstrcmp("CHUNKED", temp)) { /* CHUNKED */
          if ((in->sizeOfChunk = (hsize_t *)malloc((size_t)in->rank *
                                                   sizeof(hsize_t))) == NULL) {
      ...
      case 10: /* COMPRESSION-TYPE */
        ...
        if (in->configOptionVector[COMPRESS_PARAM] == 0) {
          if (in->compressionType == 0)
            in->compressionParam =
                6; /* default value if compressionType is GZIP */
        }
        break;
...
static int validateConfigurationParameters(struct Input *in) {
  ...
  const char *err2 = "Cannot specify chunking or compression or extendible "
                     "data sets with the external file option.\n";
  const char *err3 = "Cannot specify the compression or the extendible data "
                     "sets without the chunking option.\n";
  const char *err4a = "OUTPUT-ARCHITECTURE cannot be STD if OUTPUT-CLASS is "
                      "floating point (FP).\n";
  const char *err4b =
      "OUTPUT-ARCHITECTURE cannot be IEEE if OUTPUT-CLASS is integer (IN).\n";
  const char *err5 =
      "For OUTPUT-CLASS FP, valid values for OUTPUT-SIZE are (32, 64) .\n";
...
static int InputClassStrToInt(char *temp) {
  int i;
  char classKeywordTable[8][15] = {"TEXTIN", "TEXTFP", "TEXTFPE", "FP",
                                   "IN",     "STR",    "TEXTUIN", "UIN"};
  for (i = 0; i < 8; i++)
    if (!HDstrcmp(classKeywordTable[i], temp))
      return i;
...
static int getDimensionSizes(struct Input *in, FILE *strm) {
  ...
  const char *err1 = "Unable to allocate dynamic memory.\n";
  const char *err2 = "No. of dimensions for which dimension sizes provided is "
                     "not equal to provided rank.\n";
...
static int getChunkedDimensionSizes(struct Input *in, FILE *strm) {
  ...
  const char *err1 = "Unable to allocate dynamic memory.\n";
  const char *err2 = "No. of dimensions for which chunked dimension sizes "
                     "provided is not equal to provided rank.\n";
  const char *err3 = "The CHUNKED-DIMENSION-SIZES cannot exceed the sizes of "
                     "DIMENSION-SIZES\n";
...
static int getMaximumDimensionSizes(struct Input *in, FILE *strm) {
  ...
  const char *err1 = "Unable to allocate dynamic memory.\n";
  const char *err2 = "No. of dimensions for which maximum dimension sizes "
                     "provided is not equal to provided rank.\n";
  const char *err3 = "The MAXIMUM-DIMENSIONS cannot be less than the sizes of "
                     "DIMENSION-SIZES. Exception: "
...
static int process(struct Options *opt) {
  ...
  const char *err5 = "Error in creating the output data set. Dataset with the "
                     "same name may exist at the specified path\n";
  ...
  H5E_BEGIN_TRY {
    if ((file_id = H5Fopen(opt->outfile, H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
      if ((file_id = H5Fcreate(opt->outfile, H5F_ACC_TRUNC, H5P_DEFAULT,
                               H5P_DEFAULT)) == FAIL) {
    ...
    if (in->inputClass != 5) { /* STR */
      ...
      H5E_BEGIN_TRY {
        ...
        if (in->path.count > 1) {
          ...
          handle = file_id;
          while (j < in->path.count - 1) {
            if ((group_id = H5Gopen2(handle, in->path.group[j], H5P_DEFAULT)) <
                0) {
              group_id = H5Gcreate2(handle, in->path.group[j++], H5P_DEFAULT,
                                    H5P_DEFAULT, H5P_DEFAULT);
              for (; j < in->path.count - 1; j++)
      ...
      /* create dataspace */
      if (in->configOptionVector[EXTEND] == 1)
        dataspace = H5Screate_simple(in->rank, in->sizeOfDimension,
      ...
      H5E_BEGIN_TRY {
        /* create data set */
        if ((dataset = H5Dcreate2(handle, in->path.group[j], outtype, dataspace,
                                  H5P_DEFAULT, proplist, H5P_DEFAULT)) < 0) {
          (void)fprintf(stderr, "%s", err5);
          H5Pclose(proplist);
          H5Sclose(dataspace);
          H5Fclose(file_id);
      ...
      /* write dataset */
      if (H5Dwrite(dataset, intype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   (VOIDP)in->data) < 0) {
        (void)fprintf(stderr, "%s", err6);
        H5Dclose(dataset);
        H5Pclose(proplist);
...
int32_t swap_int32(int32_t val) {
  val =
      (int32_t)(((uint32_t)(val << 8) & 0xFF00FF00) | ((val >> 8) & 0xFF00FF));
...
int64_t swap_int64(int64_t val) {
  ...
  return (int64_t)((uint64_t)(val << 32) |
                   ((uint64_t)(val >> 32) & 0xFFFFFFFFULL));
}
...
void help(char *name) {
  ...
  (void)fprintf(stdout,
                "\t   %s <infile> -c[onfig] <configfile> [<infile> -c[config] "
                "<configfile>...]",
                name);
  ...
  (void)fprintf(
      stdout, "\t  specified by the user. The utility can also accept ASCII\n");
  (void)fprintf(
      stdout,
  ...
  (void)fprintf(
      stdout, "\t  1. ASCII text file with numeric data (floating point or \n");
  ...
  (void)fprintf(
      stdout, "\t  3. Binary file with native integer (signed or unsigned)\n");
  ...
  (void)fprintf(
      stdout, "\t  also provided as an input to the utility. (See Section \n");
  (void)fprintf(
      stdout,
  ...
  (void)fprintf(
      stdout, "\t  specified in this configuration file. A point to note is\n");
  (void)fprintf(
      stdout,
  ...
  (void)fprintf(
      stdout, "\t  The utility extracts the input data from the input file \n");
  (void)fprintf(
      stdout, "\t  according to the specified parameters and saves it into \n");
  ...
  (void)fprintf(
      stdout, "\t    single n-dimensional floating point or integer array \n");
  (void)fprintf(stdout,
  ...
  (void)fprintf(
      stdout, "\t    configuration file and this is done by the -c option.\n");
  (void)fprintf(stdout,
  ...
  (void)fprintf(
      stdout, "\t    <outfile>. The output file may be an existing file or \n");
  (void)fprintf(stdout,
  ...
  (void)fprintf(stdout, "\t  when used with the options '-o outfilename -b' of "
                        "a single dataset (-d) \n");
  (void)fprintf(stdout, "\t  OR organized as \"CONFIG-KEYWORD VALUE\" pairs, "
                        "one pair on each \n");
  ...
  (void)fprintf(
      stdout, "\t      CHUNKED-DIMENSION-SIZES to be specified. A value of \n");
  ...
  (void)fprintf(
      stdout, "\t  (6 X 3 X 5 x 2 x 4)  in a binary file with the rank and \n");
  (void)fprintf(
      stdout,
