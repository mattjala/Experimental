...
static void usage(void) {
  ...
  PRINTVALSTREAM(rawoutstream, "      -p, --parents      No error if existing, "
  ...
  PRINTVALSTREAM(rawoutstream, "      ref         Copy references and any "
                               "referenced objects, i.e., objects\n");
  PRINTVALSTREAM(rawoutstream,
                 "                  that the references point to.\n");
  PRINTVALSTREAM(rawoutstream, "                    Referenced objects are "
                               "copied in addition to the objects\n");
  PRINTVALSTREAM(rawoutstream, "                  specified on the command "
                               "line and reference datasets are\n");
  PRINTVALSTREAM(rawoutstream, "                  populated with correct "
                               "reference values. Copies of referenced\n");
  PRINTVALSTREAM(rawoutstream, "                  datasets outside the copy "
                               "range specified on the command line\n");
  PRINTVALSTREAM(rawoutstream, "                  will normally have a "
                               "different name from the original.\n");
  PRINTVALSTREAM(rawoutstream, "                    (Default:Without this "
                               "option, reference value(s) in any\n");
  PRINTVALSTREAM(rawoutstream, "                  reference datasets are set "
                               "to NULL and referenced objects are\n");
  PRINTVALSTREAM(rawoutstream, "                  not copied unless they are "
                               "otherwise within the copy range\n");
  PRINTVALSTREAM(rawoutstream,
                 "                  specified on the command line.)\n\n");
  PRINTVALSTREAM(
      rawoutstream,
      "      noattr      Copy object without copying attributes\n\n");
  PRINTVALSTREAM(rawoutstream, "      allflags    Switches all flags from the "
                               "default to the non-default setting\n\n");
  PRINTVALSTREAM(
      rawoutstream,
 ...
 *
 * shallow  H5O_COPY_SHALLOW_HIERARCHY_FLAG:  Copy only immediate members for
 *groups soft     H5O_COPY_EXPAND_SOFT_LINK_FLAG:  Expand soft links into new
 *objects ext      H5O_COPY_EXPAND_EXT_LINK_FLAG: Expand external links into new
 *objects ref      H5O_COPY_EXPAND_OBJ_REFERENCE_FLAG: Copy objects that are
 *pointed by references noattr   H5O_COPY_WITHOUT_ATTR_FLAG Copy object without
 *copying attributes allflags Switches all flags from the default to the
 *non-default setting
 *
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = H5_get_option(argc, (const char *const *)argv, s_opts,
                              l_opts)) != EOF) {
    switch ((char)opt) {
    case 'd':
  ...
  fid_src =
      h5tools_fopen(fname_src, H5F_ACC_RDONLY, H5P_DEFAULT, FALSE, NULL, 0);
  ...
  /* (use "EXCL" instead of "TRUNC", so we don't blow away existing non-HDF5
   * file) */
  if (fid_dst < 0)
  ...
  if (parents) {
    ...
    if (H5Pset_create_intermediate_group(lcpl_id, 1) < 0) {
      ...
      H5TOOLS_GOTO_ERROR(EXIT_FAILURE,
  ...
  if (li_ret == 0) {
    /* dangling link */
    if (H5Lcopy(fid_src, oname_src, fid_dst, oname_dst, H5P_DEFAULT,
                H5P_DEFAULT) < 0)
