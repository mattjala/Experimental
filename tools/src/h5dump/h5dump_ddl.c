...
static herr_t dump_all_cb(hid_t group, const char *name,
 ...
 *
 * Purpose:     attribute function callback called by H5Aiterate2, displays the
 *attribute
 *
 ...
 *-------------------------------------------------------------------------
 */
herr_t dump_attr_cb(hid_t oid, const char *attr_name,
...
static herr_t dump_all_cb(hid_t group, const char *name,
  ...
  if (linfo->type == H5L_TYPE_HARD) {
    ...
    /* Stat the object */
    if (H5Oget_info_by_name3(group, name, &oinfo, H5O_INFO_BASIC, H5P_DEFAULT) <
        0) {
      ...
      if (dump_opts.display_data) {
        ...
        if (dump_opts.display_vds_first) {
          if (H5Pset_virtual_view(dapl_id, H5D_VDS_FIRST_MISSING) < 0)
            error_msg(
                "error in setting access property list ID, virtual_view\n");
        }
        if (dump_opts.vds_gap_size > 0) {
          if (H5Pset_virtual_printf_gap(dapl_id,
                                        (hsize_t)dump_opts.vds_gap_size) < 0)
            error_msg("error in setting access property list ID, "
                      "virtual_printf_gap\n");
        }
      }
        ...
        if (oinfo.rc > 1 || hit_elink) {
          ...
          if (found_obj == NULL) {
            ...
            h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                                   &curr_pos, (size_t)outputformat->line_ncols,
                                   (hsize_t)0, (hsize_t)0);
            ...
            if (HDstrlen(h5tools_dump_header_format->datasetend))
              h5tools_str_append(&buffer, "%s",
                                 h5tools_dump_header_format->datasetend);
            h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                                   &curr_pos, (size_t)outputformat->line_ncols,
                                   (hsize_t)0, (hsize_t)0);
            ...
            /* Render the element */
            h5tools_str_reset(&buffer);
            h5tools_str_append(&buffer, "%s \"%s\"", HARDLINK,
                               found_obj->objname);
            h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                                   &curr_pos, (size_t)outputformat->line_ncols,
                                   (hsize_t)0, (hsize_t)0);
    ...
    case H5L_TYPE_SOFT:
      if ((targbuf = (char *)malloc(linfo->u.val_size)) == NULL) {
        ...
        if (H5Lget_val(group, name, targbuf, linfo->u.val_size, H5P_DEFAULT) <
            0) {
        ...
        if (HDstrlen(h5tools_dump_header_format->softlinkend))
          h5tools_str_append(&buffer, "%s",
                             h5tools_dump_header_format->softlinkend);
        h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                               &curr_pos, (size_t)outputformat->line_ncols,
                               (hsize_t)0, (hsize_t)0);
    ...
    case H5L_TYPE_EXTERNAL:
      if ((targbuf = (char *)malloc(linfo->u.val_size)) == NULL) {
        ...
        if (H5Lget_val(group, name, targbuf, linfo->u.val_size, H5P_DEFAULT) <
          ...
          if (H5Lunpack_elink_val(targbuf, linfo->u.val_size, NULL, &filename,
                                  &targname) < 0) {
            indentation(dump_indent);
        ...
        if (HDstrlen(h5tools_dump_header_format->extlinkblockend)) {
          h5tools_str_append(&buffer, "%s",
                             h5tools_dump_header_format->extlinkblockend);
    ...
    default:
      ...
      if (HDstrlen(h5tools_dump_header_format->udlinkblockend)) {
        h5tools_str_append(&buffer, "%s",
                           h5tools_dump_header_format->udlinkblockend);
        if (HDstrlen(h5tools_dump_header_format->udlinkend))
          h5tools_str_append(&buffer, " ");
      }
      if (HDstrlen(h5tools_dump_header_format->udlinkend))
        h5tools_str_append(&buffer, "%s",
                           h5tools_dump_header_format->udlinkend);
      h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                             &curr_pos, (size_t)outputformat->line_ncols,
                             (hsize_t)0, (hsize_t)0);
...
void attr_iteration(hid_t gid, unsigned attr_crt_order_flags) {
  /* attribute iteration: if there is a request to do H5_INDEX_CRT_ORDER and
     tracking order is set in the group for attributes, then, sort by creation
     order, otherwise by name */
  if (dump_opts.include_attrs) {
    if ((sort_by == H5_INDEX_CRT_ORDER) &&
        (attr_crt_order_flags & H5P_CRT_ORDER_TRACKED)) {
      ...
      if (H5Aiterate2(gid, H5_INDEX_NAME, sort_order, NULL, dump_attr_cb,
                      NULL) < 0) {
        error_msg("error getting attribute information\n");
        h5tools_setstatus(EXIT_FAILURE);
      } /* end if */
...
void dump_named_datatype(hid_t tid, const char *name) {
  ...
  h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos,
                         (size_t)outputformat->line_ncols, (hsize_t)0,
                         (hsize_t)0);
...
void dump_group(hid_t gid, const char *name) {
  ...
  if (HDstrlen(h5tools_dump_header_format->groupblockend)) {
    h5tools_str_append(&buffer, "%s",
                       h5tools_dump_header_format->groupblockend);
 ...
 *-------------------------------------------------------------------------
 */
void dump_data(hid_t obj_id, int obj_data, struct subset_t *sset,
               int display_index) {
  h5tools_context_t ctx; /* print context  */
  h5tool_format_t *outputformat = &h5tools_dataformat;
  h5tool_format_t string_dataformat;
...
void dump_fcpl(hid_t fid) {
   ...
   * just return.
   */
  H5VLquery_optional(fid, H5VL_SUBCLS_FILE, H5VL_NATIVE_FILE_GET_INFO,
                     &supported);
...
static herr_t attr_search(hid_t oid, const char *attr_name,
...
static herr_t obj_search(const char *path, const H5O_info2_t *oi,
                         const char H5_ATTR_UNUSED *already_visited,
                         void *_op_data) {
  ...
  H5Aiterate_by_name(handle_data->fid, path, H5_INDEX_NAME, H5_ITER_INC, NULL,
                     attr_search, (void *)&attr_data, H5P_DEFAULT);
...
static herr_t lnk_search(const char *path, const H5L_info2_t *li,
                         void *_op_data) {
  size_t search_len;
  size_t k;
  char *search_name;
 ...
 *-------------------------------------------------------------------------
 */
void handle_paths(hid_t fid, const char *path_name, void H5_ATTR_UNUSED *data,
                  int H5_ATTR_UNUSED pe,
  ...
  if ((gid = H5Gopen2(fid, "/", H5P_DEFAULT)) < 0) {
    ...
    if (h5trav_visit(fid, "/", TRUE, TRUE, obj_search, lnk_search,
                     &handle_udata, H5O_INFO_BASIC) < 0) {
 ...
 *-------------------------------------------------------------------------
 */
void handle_attributes(hid_t fid, const char *attr, void H5_ATTR_UNUSED *data,
                       int H5_ATTR_UNUSED pe,
                       const char H5_ATTR_UNUSED *display_name) {
  ...
  if ((oid = H5Oopen(fid, obj_name, H5P_DEFAULT)) < 0) {
    ...
    if (HDstrlen(h5tools_dump_header_format->attributeend))
      h5tools_str_append(&buffer, "%s",
                         h5tools_dump_header_format->attributeend);
    h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos,
                           (size_t)outputformat->line_ncols, (hsize_t)0,
                           (hsize_t)0);
 ...
 *-------------------------------------------------------------------------
 */
void handle_datasets(hid_t fid, const char *dset, void *data, int pe,
  ...
  if (dump_opts.display_data) {
    ...
    if (dump_opts.vds_gap_size > 0) {
      if (H5Pset_virtual_printf_gap(dapl_id, (hsize_t)dump_opts.vds_gap_size) <
          0)
        error_msg(
  ...
  if (sset) {
    ...
    if (!sset->start.data || !sset->stride.data || !sset->count.data ||
        !sset->block.data) {
  ...
  if (oinfo.rc > 1 || hit_elink) {
    ...
    if (found_obj) {
      if (found_obj->displayed) {
        ...
        indentation(dump_indent);
        end_obj(h5tools_dump_header_format->datasetend,
                h5tools_dump_header_format->datasetblockend);
      } else {
        found_obj->displayed = TRUE;
        dump_indent += COL;
 ...
 *-------------------------------------------------------------------------
 */
void handle_groups(hid_t fid, const char *group, void H5_ATTR_UNUSED *data,
                   int pe, const char *display_name) {
  hid_t gid;
  ...
  if ((gid = H5Gopen2(fid, group, H5P_DEFAULT)) < 0) {
    if (pe) {
      ...
      end_obj(h5tools_dump_header_format->groupend,
              h5tools_dump_header_format->groupblockend);
      h5tools_setstatus(EXIT_FAILURE);
    }
  } else {
 ...
 *-------------------------------------------------------------------------
 */
void handle_links(hid_t fid, const char *links, void H5_ATTR_UNUSED *data,
                  int H5_ATTR_UNUSED pe,
                  const char H5_ATTR_UNUSED *display_name) {
  H5L_info2_t linfo;
  ...
  if (H5Lget_info2(fid, links, &linfo, H5P_DEFAULT) < 0) {
    ...
    case H5L_TYPE_EXTERNAL:
      ...
      if (H5Lget_val(fid, links, buf, linfo.u.val_size, H5P_DEFAULT) >= 0) {
        ...
        if (H5Lunpack_elink_val(buf, linfo.u.val_size, NULL, &elink_file,
                                &elink_path) >= 0) {
          indentation(COL);
          ...
          error_msg(
              "h5dump error: unable to unpack external link value for \"%s\"\n",
              links);
          h5tools_setstatus(EXIT_FAILURE);
        }
      } else {
        error_msg(
            "h5dump error: unable to get external link value for \"%s\"\n",
            links);
        h5tools_setstatus(EXIT_FAILURE);
      }
      end_obj(h5tools_dump_header_format->extlinkend,
              h5tools_dump_header_format->extlinkblockend);
      break;
    ...
    default:
      ...
      end_obj(h5tools_dump_header_format->udlinkend,
              h5tools_dump_header_format->udlinkblockend);
      break;
    } /* end switch */
    free(buf);
 ...
 *-------------------------------------------------------------------------
 */
void handle_datatypes(hid_t fid, const char *type, void H5_ATTR_UNUSED *data,
                      int pe, const char *display_name) {
  hid_t type_id;
 ...
 *-------------------------------------------------------------------------
 */
static int dump_extlink(hid_t group, const char *linkname,
                        const char *objname) {
  hid_t oid;
  H5O_info2_t oi;
