...
typedef struct dump_functions_t {
  ...
  herr_t (*dump_attribute_function)(hid_t, const char *, const H5A_info_t *,
                                    void *);
...
typedef struct h5dump_table_items_t {
  ...
  hid_t
      oid; /* ID of an object in this file, held open so fileno is consistent */
...
dump_opt_t dump_opts = {TRUE,  FALSE, TRUE,  TRUE,  FALSE, FALSE,
...
#define PACKED_BITS_SIZE_MAX                                                   \
  (8 *                                                                         \
   sizeof(long long)) /* Maximum bits size of integer types of packed-bits */
/* mask list for packed bits */
unsigned long long packed_mask[PACKED_BITS_MAX]; /* packed bits are restricted
                                                    to 8*sizeof(llong) bytes */
