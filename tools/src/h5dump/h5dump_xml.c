 ...
 *-------------------------------------------------------------------------
 */
static herr_t xml_dump_all_cb(hid_t group, const char *name,
      ...
      if ((obj = H5Dopen2(group, name, H5P_DEFAULT)) >= 0) {
        if (oinfo.rc > 1 || hit_elink) {
          ...
          if (found_obj == NULL) {
            ...
            char *t_prefix =
                xml_escape_the_name(HDstrcmp(prefix, "") ? prefix : "/");
            ...
            xml_name_to_XID(obj, found_obj->objname, pointerxid,
                            (int)sizeof(pointerxid), 1);
            ...
            h5tools_str_reset(&buffer);
            h5tools_str_append(&buffer,
                               "<%sDatasetPtr OBJ-XID=\"%s\" H5Path=\"%s\"/>",
    ...
    case H5L_TYPE_SOFT:
      if ((targbuf = (char *)malloc(linfo->u.val_size)) == NULL) {
        ...
        if (H5Lget_val(group, name, targbuf, linfo->u.val_size, H5P_DEFAULT) <
          ...
          t_link_path =
              (char *)malloc(HDstrlen(prefix) + linfo->u.val_size + 1);
          if (targbuf[0] == '/')
          ...
          res = xml_name_to_XID(group, t_link_path, targetxid,
                                (int)sizeof(targetxid), 0);
          if (res == 0) {
            /* target obj found */
            ctx.need_prefix = TRUE;
      ...
      if ((targbuf = (char *)malloc(linfo->u.val_size)) == NULL) {
        ...
        if (H5Lget_val(group, name, targbuf, linfo->u.val_size, H5P_DEFAULT) <
          ...
          if (H5Lunpack_elink_val(targbuf, linfo->u.val_size, NULL, &filename,
            ...
            /* Create OBJ-XIDs for the parent and object */
            xml_name_to_XID(group, t_obj_path, linkxid, (int)sizeof(linkxid),
                            1);
            xml_name_to_XID(group, prefix, parentxid, (int)sizeof(parentxid),
                            1);
 ...
 */
int xml_name_to_XID(hid_t loc_id, const char *str, char *outstr, int outlen,
                    int gen) {
  ...
  if (!in_group && H5Tcommitted(type) > 0) {
    ...
    if (found_obj) {
      ...
      if (!found_obj->recorded) {
        ...
        h5tools_str_reset(&buffer);
        h5tools_str_append(&buffer,
                           "<%sNamedDataTypePtr OBJ-XID=\"%s\" H5Path=\"%s\"/>",
      ...
      h5tools_str_reset(&buffer);
      h5tools_str_append(&buffer,
                         "<!-- h5dump error: unknown committed type. -->");
      h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                             &curr_pos, (size_t)outputformat->line_ncols,
                             (hsize_t)0, (hsize_t)0);
      h5tools_setstatus(EXIT_FAILURE);
    ...
    case H5T_FLOAT:
      ...
      h5tools_str_append(&buffer, "\" SignBitLocation=\"%lu\" ",
                         (unsigned long)spos);
      h5tools_str_append(&buffer,
                         "ExponentBits=\"%lu\" ExponentLocation=\"%lu\" ",
                         (unsigned long)esize, (unsigned long)epos);
      h5tools_str_append(&buffer,
                         "MantissaBits=\"%lu\" MantissaLocation=\"%lu\" />",
    ...
    case H5T_OPAQUE:
      ...
      /* Render the element */
      h5tools_str_reset(&buffer);
      h5tools_str_append(&buffer, "<%sOpaqueType Tag=\"%s\" ", xmlnsprefix,
                         mname);
      H5free_memory(mname);
      size = H5Tget_size(type);
      h5tools_str_append(&buffer, "Size=\"%lu\"/>", (unsigned long)size);
    ...
    case H5T_COMPOUND:
      ...
      for (i = 0; i < nmembers; i++) {
        ...
        /* Render the element */
        h5tools_str_reset(&buffer);
        h5tools_str_append(&buffer, "<%sField FieldName=\"%s\">", xmlnsprefix,
                           t_fname);
        h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                               &curr_pos, (size_t)outputformat->line_ncols,
                               (hsize_t)0, (hsize_t)0);
    ...
    case H5T_ENUM:
      ...
      /* Render the element */
      h5tools_str_reset(&buffer);
      h5tools_str_append(&buffer, "<%sEnumType Nelems=\"%d\">", xmlnsprefix,
                         nmembs);
      h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                             &curr_pos, (size_t)outputformat->line_ncols,
                             (hsize_t)0, (hsize_t)0);
    ...
    case H5T_ARRAY:
      ...
      for (i = 0; i < ndims; i++) {
        ...
        /* Render the element */
        h5tools_str_reset(&buffer);
        h5tools_str_append(&buffer, "<%sArrayDimension DimSize=\"%u\"/>",
  ...
  if (H5Tcommitted(type) > 0) {
    ...
    if (found_obj) {
      ...
      if (!found_obj->recorded) {
        ...
        h5tools_str_reset(&buffer);
        h5tools_str_append(
            &buffer, "<%sNamedDataTypePtr OBJ-XID=\"%s\" H5Path=\"%s\" />",
            xmlnsprefix, dtxid, t_objname);
        h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                               &curr_pos, (size_t)outputformat->line_ncols,
                               (hsize_t)0, (hsize_t)0);
...
void xml_dump_dataspace(hid_t space) {
  ...
  case H5S_SIMPLE:
    ...
    /* Render the element */
    h5tools_str_reset(&buffer);
    h5tools_str_append(&buffer, "<%sSimpleDataspace Ndims=\"%d\">", xmlnsprefix,
                       ndims);
    h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos,
                           (size_t)outputformat->line_ncols, (hsize_t)0,
                           (hsize_t)0);
 ...
 *-------------------------------------------------------------------------
 */
void xml_dump_data(hid_t obj_id, int obj_data,
                   struct subset_t H5_ATTR_UNUSED *sset,
                   int H5_ATTR_UNUSED pindex) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t xml_dump_attr(hid_t attr, const char *attr_name,
  ...
  /* Render the element */
  h5tools_str_reset(&buffer);
  h5tools_str_append(&buffer, "<%sAttribute Name=\"%s\">", xmlnsprefix,
                     t_aname);
  h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer, &curr_pos,
                         (size_t)outputformat->line_ncols, (hsize_t)0,
                         (hsize_t)0);
  ...
  if ((attr_id = H5Aopen(attr, attr_name, H5P_DEFAULT)) >= 0) {
    ...
    if (dump_opts.display_attr_data && space_type != H5S_NULL) {
      ...
      case H5T_COMPOUND:
        ...
        h5tools_str_reset(&buffer);
        h5tools_str_append(
            &buffer, "<!-- Note: format of compound data not specified -->");
        h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                               &curr_pos, (size_t)outputformat->line_ncols,
                               (hsize_t)0, (hsize_t)0);
        dump_function_table->dump_data_function(attr_id, ATTRIBUTE_DATA, NULL,
      ...
      default:
        ...
        /* Render the element */
        h5tools_str_reset(&buffer);
        h5tools_str_append(&buffer, "<!-- Unknown datatype: %d -->",
                           H5Tget_class(type));
        h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                               &curr_pos, (size_t)outputformat->line_ncols,
                               (hsize_t)0, (hsize_t)0);
...
void xml_dump_named_datatype(hid_t type, const char *name) {
  ...
  if (HDstrncmp(name, "#", (size_t)1) == 0) {
    ...
    h5tools_str_append(&buffer,
                       ...
                       "Parents=\"%s\" H5ParentPaths=\"%s\">",
                       xmlnsprefix, name, dtxid, parentxid,
    ...
    if (oinfo.rc > 1) {
      ...
      if (found_obj == NULL) {
        ...
        xml_name_to_XID(type, found_obj->objname, pointerxid,
                        (int)sizeof(pointerxid), 1);
  ...
  if (oinfo.rc > 1) {
    ...
    if (found_obj == NULL) {
      ...
      if (found_obj->displayed) {
        ...
        if (isRoot) {
          ...
          t_objname = xml_escape_the_name(
              found_obj->objname); /* point to the NDT by name */
          par_name = xml_escape_the_name(par);
          ...
          h5tools_str_append(&buffer,
                             ...
                             "Parents=\"%s\" H5ParentPaths=\"%s\" />",
                             xmlnsprefix, ptrstr, t_objname, parentxid,
                             par_name);
          h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                                 &curr_pos, (size_t)outputformat->line_ncols,
                                 (hsize_t)0, (hsize_t)0);
        ...
        if ((sort_by == H5_INDEX_CRT_ORDER) &&
          ...
          if (H5Aiterate2(gid, sort_by, sort_order, NULL,
                          dump_function_table->dump_attribute_function,
                          NULL) < 0) {
        ...
        if (isRoot && unamedtype) {
          ...
          for (u = 0; u < type_table->nobjs; u++) {
            if (!type_table->objs[u].recorded) {
              ...
              H5Otoken_to_str(dset, &type_table->objs[u].obj_token,
                              &obj_tok_str);
        ...
        if ((sort_by == H5_INDEX_CRT_ORDER) &&
          ...
          H5Literate2(gid, H5_INDEX_NAME, sort_order, NULL, xml_dump_all_cb,
                      NULL);
    ...
    if (isRoot) {
      ...
      h5tools_str_append(&buffer,
                         "<%sGroup Name=\"%s\" OBJ-XID=\"%s\" H5Path=\"%s\" "
                         "Parents=\"%s\" H5ParentPaths=\"%s\" >",
                         xmlnsprefix, t_name, grpxid, t_tmp, parentxid,
                         par_name);
      free(t_tmp);
      free(par_name);
    }
...
static void check_filters(hid_t dcpl) {
  ...
  if (nfilt <= 0)
    return;
  for (i = 0; i < nfilt; i++) {
    filter = H5Pget_filter2(dcpl, (unsigned)i, &flags, (size_t *)&cd_nelmts,
                            cd_values, (size_t)120, namebuf, NULL);
...
static void xml_dump_fill_value(hid_t dcpl, hid_t type) {
  ...
  if (H5Tget_class(type) == H5T_REFERENCE) {
    ...
    case H5T_ARRAY:
      ...
      h5tools_str_reset(&buffer);
      h5tools_str_append(&buffer,
                         "<!-- Array fill values not yet implemented. -->");
      h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                             &curr_pos, (size_t)outputformat->line_ncols,
                             (hsize_t)0, (hsize_t)0);
    ...
    default:
      ...
      /* Render the element */
      h5tools_str_reset(&buffer);
      h5tools_str_append(&buffer, "<!-- Unknown fill datatype: %d -->",
                         H5Tget_class(type));
      h5tools_render_element(rawoutstream, outputformat, &ctx, &buffer,
                             &curr_pos, (size_t)outputformat->line_ncols,
                             (hsize_t)0, (hsize_t)0);
 ...
 *-------------------------------------------------------------------------
 */
void xml_dump_dataset(hid_t did, const char *name,
                      struct subset_t H5_ATTR_UNUSED *sset) {
  hid_t type;
  hid_t space;
  hid_t dcpl;
  ...
  h5tools_str_reset(&buffer);
  h5tools_str_append(&buffer,
                     "<%sDataset Name=\"%s\" OBJ-XID=\"%s\" H5Path= \"%s\" "
                     "Parents=\"%s\" H5ParentPaths=\"%s\">",
  ...
  if (H5D_CHUNKED == H5Pget_layout(dcpl)) {
    if ((maxdims = H5Sget_simple_extent_ndims(space)) < 0) {
      ...
      for (i = 0; i < ndims; i++) {
        ...
        h5tools_str_reset(&buffer);
        h5tools_str_append(&buffer,
                           "<%sChunkDimension DimSize=\"%" PRIuHSIZE "\" />",
  ...
  if ((sort_by == H5_INDEX_CRT_ORDER) &&
      (attr_crt_order_flags & H5P_CRT_ORDER_TRACKED)) {
    if (H5Aiterate2(did, sort_by, sort_order, NULL,
                    dump_function_table->dump_attribute_function, NULL) < 0) {
      error_msg("error getting attribute information\n");
      h5tools_setstatus(EXIT_FAILURE);
    } /* end if */
    ...
    if (H5Aiterate2(did, H5_INDEX_NAME, sort_order, NULL,
                    dump_function_table->dump_attribute_function, NULL) < 0) {
      error_msg("error getting attribute information\n");
      h5tools_setstatus(EXIT_FAILURE);
    } /* end if */
...
static void xml_print_enum(hid_t type) {
  ...
  value =
      (unsigned char *)calloc((size_t)nmembs, MAX(H5Tget_size(type), dst_size));
