...
static const dump_functions ddl_function_table = {
    dump_group,    dump_named_datatype, dump_dataset, dump_dataspace,
    dump_datatype, dump_attr_cb,        dump_data};

/* XML output */
static const dump_functions xml_function_table = {
    xml_dump_group,     xml_dump_named_datatype, xml_dump_dataset,
    xml_dump_dataspace, xml_dump_datatype,       xml_dump_attr,
    xml_dump_data};
...
static const char *s_opts =
    "a:b*c:d:ef:g:hik:l:m:n*o*pq:rs:t:uvw:xyz:A*BCD:E*F:G:HM:N:O*RS:VX:";
...
static void usage(const char *prog) {
  ...
  PRINTVALSTREAM(rawoutstream, "                          Optional value 2 "
                               "also prints file open errors.\n");
  PRINTVALSTREAM(rawoutstream, "                          Default setting "
                               "disables any error reporting.\n");
  PRINTVALSTREAM(rawoutstream,
                 "--------------- File Options ---------------\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "     -H,   --header       Print the header "
                               "only; no data is displayed\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "                          Use blank(empty) "
                               "filename F to suppress ddl display\n");
  PRINTVALSTREAM(rawoutstream, "     --s3-cred=<cred>     Supply S3 "
                               "authentication information to \"ros3\" vfd.\n");
  PRINTVALSTREAM(rawoutstream, "                          <cred> :: "
                               "\"(<aws-region>,<access-id>,<access-key>)\"\n");
  PRINTVALSTREAM(rawoutstream, "                          If absent or <cred> "
                               "-> \"(,,)\", no authentication.\n");
  PRINTVALSTREAM(rawoutstream, "                          Has no effect if "
                               "filedriver is not \"ros3\".\n");
  PRINTVALSTREAM(rawoutstream, "     --hdfs-attrs=<attrs> Supply configuration "
                               "information for HDFS file access.\n");
  PRINTVALSTREAM(
      rawoutstream,
      "                          For use with \"--filedriver=hdfs\"\n");
  PRINTVALSTREAM(rawoutstream, "                          <attrs> :: "
                               "(<namenode name>,<namenode port>,\n");
  PRINTVALSTREAM(rawoutstream, "                                      "
                               "<kerberos cache path>,<username>,\n");
  PRINTVALSTREAM(rawoutstream,
                 "                                      <buffer size>)\n");
  PRINTVALSTREAM(rawoutstream, "                          Any absent attribute "
                               "will use a default value.\n");
  PRINTVALSTREAM(rawoutstream, "     --vol-value          Value (ID) of the "
                               "VOL connector to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "     --vol-name           Name of the VOL "
                               "connector to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "     --vol-info           VOL-specific info to "
                               "pass to the VOL connector used for\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          opening the HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "                          If none of the above "
                               "options are used to specify a VOL, then\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          the VOL named by "
  ...
  PRINTVALSTREAM(rawoutstream, "     --vfd-value          Value (ID) of the "
                               "VFL driver to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "     --vfd-name           Name of the VFL "
                               "driver to use for opening the\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream, "     --vfd-info           VFD-specific info to "
                               "pass to the VFL driver used for\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          opening the HDF5 file specified\n");
  PRINTVALSTREAM(rawoutstream,
                 "--------------- Object Options ---------------\n");
  ...
  PRINTVALSTREAM(rawoutstream, "                          If an attribute name "
                               "contains a slash (/), escape the\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "     -l P, --soft-link=P  Print the value(s) "
                               "of the specified soft link\n");
  PRINTVALSTREAM(
      rawoutstream,
      "     -t P, --datatype=P   Print the specified named datatype\n");
  PRINTVALSTREAM(rawoutstream,
                 "     -N P, --any_path=P   Print any attribute, dataset, "
                 "group, datatype, or link that matches P\n");
  PRINTVALSTREAM(rawoutstream, "                          P can be the "
                               "absolute path or just a relative path.\n");
  PRINTVALSTREAM(
      rawoutstream,
      "     -A,   --onlyattr     Print the header and value of attributes\n");
  PRINTVALSTREAM(rawoutstream, "                          Optional value 0 "
                               "suppresses printing attributes.\n");
  PRINTVALSTREAM(rawoutstream, "     --vds-view-first-missing Set the VDS "
                               "bounds to first missing mapped elements.\n");
  PRINTVALSTREAM(rawoutstream, "     --vds-gap-size=N     Set the missing file "
                               "gap size, N=non-negative integers\n");
  PRINTVALSTREAM(rawoutstream,
                 "--------------- Object Property Options ---------------\n");
  PRINTVALSTREAM(rawoutstream,
                 "     -i,   --object-ids   Print the object ids\n");
  PRINTVALSTREAM(rawoutstream, "     -p,   --properties   Print dataset "
                               "filters, storage layout and fill value\n");
  PRINTVALSTREAM(rawoutstream, "     -M L, --packedbits=L Print packed bits as "
                               "unsigned integers, using mask\n");
  PRINTVALSTREAM(rawoutstream, "                          format L for an "
                               "integer dataset specified with\n");
  PRINTVALSTREAM(rawoutstream, "                          option -d. L is a "
                               "list of offset,length values,\n");
  PRINTVALSTREAM(rawoutstream, "                          separated by commas. "
                               "Offset is the beginning bit in\n");
  PRINTVALSTREAM(rawoutstream, "                          the data value and "
                               "length is the number of bits of\n");
  ...
  PRINTVALSTREAM(rawoutstream, "     --no-compact-subset  Disable compact form "
                               "of subsetting and allow the use\n");
  PRINTVALSTREAM(rawoutstream,
                 "                          of \"[\" in dataset names.\n");
  PRINTVALSTREAM(rawoutstream, "     -w N, --width=N      Set the number of "
                               "columns of output. A value of 0 (zero)\n");
  PRINTVALSTREAM(rawoutstream, "                          sets the number of "
                               "columns to the maximum (65535).\n");
  PRINTVALSTREAM(rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "                          E.g., to dump a file "
                               "called \"-f\", use h5dump -- -f\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream,
                 "--------------- Subsetting Options ---------------\n");
  PRINTVALSTREAM(rawoutstream, " Subsetting is available by using the "
                               "following options with a dataset\n");
  PRINTVALSTREAM(
      rawoutstream,
      " option. Subsetting is done by selecting a hyperslab from the data.\n");
  PRINTVALSTREAM(rawoutstream, " Thus, the options mirror those for performing "
                               "a hyperslab selection.\n");
  PRINTVALSTREAM(rawoutstream,
                 " One of the START, COUNT, STRIDE, or BLOCK parameters are "
                 "mandatory if you do subsetting.\n");
  ...
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "      -s START,  --start=START    Offset of "
                               "start of subsetting selection\n");
  PRINTVALSTREAM(rawoutstream,
                 "      -S STRIDE, --stride=STRIDE  Hyperslab stride\n");
  PRINTVALSTREAM(rawoutstream, "      -c COUNT,  --count=COUNT    Number of "
                               "blocks to include in selection\n");
  PRINTVALSTREAM(
      rawoutstream,
      "      -k BLOCK,  --block=BLOCK    Size of block in hyperslab\n");
  PRINTVALSTREAM(rawoutstream,
                 "  START, COUNT, STRIDE, and BLOCK - is a list of integers "
                 "the number of which are equal to the\n");
  PRINTVALSTREAM(rawoutstream,
                 "      number of dimensions in the dataspace being queried\n");
  PRINTVALSTREAM(rawoutstream, "      (Alternate compact form of subsetting is "
                               "described in the Reference Manual)\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(
      rawoutstream,
      "--------------- Option Argument Conventions ---------------\n");
  PRINTVALSTREAM(rawoutstream, "  D - is the file driver to use in opening the "
                               "file. Acceptable values\n");
  PRINTVALSTREAM(rawoutstream,
                 "      are \"sec2\", \"family\", \"split\", \"multi\", "
                 "\"direct\", and \"stream\". Without\n");
  PRINTVALSTREAM(rawoutstream, "      the file driver flag, the file will be "
                               "opened with each driver in\n");
  PRINTVALSTREAM(rawoutstream, "      turn and in the order specified above "
                               "until one driver succeeds\n");
  PRINTVALSTREAM(rawoutstream, "      in opening the file.\n");
  PRINTVALSTREAM(rawoutstream, "      See examples below for family, split, "
                               "and multi driver special file name usage.\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  ...
  PRINTVALSTREAM(rawoutstream, "  B - is the form of binary output: NATIVE for "
                               "a memory type, FILE for the\n");
  PRINTVALSTREAM(rawoutstream, "        file type, LE or BE for pre-existing "
                               "little or big endian types.\n");
  PRINTVALSTREAM(rawoutstream, "        Must be used with -o (output file) and "
                               "it is recommended that\n");
  PRINTVALSTREAM(rawoutstream, "        -d (dataset) is used. B is an optional "
                               "argument, defaults to NATIVE\n");
  PRINTVALSTREAM(rawoutstream, "  Q - is the sort index type. It can be "
                               "\"creation_order\" or \"name\" (default)\n");
  PRINTVALSTREAM(rawoutstream, "  Z - is the sort order type. It can be "
  ...
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "     Attribute \"high/low\" of the group "
                               "/bar_none in the file quux.h5\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "      h5dump -d /foo -s \"0,1\" -S \"1,1\" -c "
                               "\"2,3\" -k \"2,2\" quux.h5\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(
      rawoutstream,
  ...
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "  4) Display two packed bits (bits 0-1 and "
                               "bits 4-6) in the dataset /dset\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  ...
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream,
                 "  7) Dataset foo in multi files mf-s.h5, mf-b.h5, mf-r.h5, "
                 "mf-g.h5, mf-l.h5 and mf-o.h5\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "      h5dump -d /foo -f multi mf\n");
  PRINTVALSTREAM(rawoutstream, "\n");
  PRINTVALSTREAM(rawoutstream, "  8) Dataset foo in family files fam00000.h5 "
                               "fam00001.h5 and fam00002.h5\n");
  PRINTVALSTREAM(rawoutstream, "\n");
...
ssize_t table_list_add(hid_t oid, unsigned long file_no) {
  ...
  if (table_list.nused == table_list.nalloc) {
    ...
    if (NULL == (tmp_ptr = (h5dump_table_items_t *)realloc(
                     table_list.tables,
                     table_list.nalloc * sizeof(table_list.tables[0]))))
      return -1;
  ...
  if (init_objs(oid, &info, &table_list.tables[idx].group_table,
                &table_list.tables[idx].dset_table,
 ...
 *this function should be at the start of the list you want to parse.
 *
...
static struct handler_t *parse_command_line(int argc, const char *const *argv) {
  ...
  if ((hand = (struct handler_t *)calloc((size_t)argc,
                                         sizeof(struct handler_t))) == NULL) {
    goto error;
  }
  ...
  parse_start:
    ...
    case 'k': {
      ...
      if (last_dset->subset_info) {
        ...
        last_dset->subset_info = s =
            (struct subset_t *)calloc(1, sizeof(struct subset_t));
      }
...
int main(int argc, char *argv[]) {
  ...
  if (use_custom_vol_g || use_custom_vfd_g) {
    if ((fapl_id = h5tools_get_fapl(
             H5P_DEFAULT, use_custom_vol_g ? &vol_info_g : NULL,
  ...
  while (H5_optind < argc) {
    ...
    /* A short cut to get the revision count of an onion file without opening
     * the file */
    ...
    if (doxml_g) {
      ...
      if (xml_dtd_uri_g == NULL) {
        ...
        if (useschema_g && HDstrcmp(xmlnsprefix, "") != 0) {
          error_msg("Cannot set Schema URL for a qualified namespace--use -X "
                    "or -U option with -D \n");
          h5tools_setstatus(EXIT_FAILURE);
          goto done;
        }
      ...
      if (useschema_g) {
        if (HDstrcmp(xmlnsprefix, "") == 0) {
          ...
          PRINTSTREAM(
              rawoutstream,
              "<%sHDF5-File "
              "xmlns:%s=\"http://hdfgroup.org/HDF5/XML/schema/HDF5-File.xsd\" "
              "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "
              "xsi:schemaLocation=\"http://hdfgroup.org/HDF5/XML/schema/"
              "HDF5-File "
        ...
        PRINTSTREAM(rawoutstream,
                    "<!DOCTYPE HDF5-File PUBLIC \"HDF5-File.dtd\" \"%s\">\n",
                    xml_dtd_uri_g);
    ...
    if (!doxml_g) {
      if (dump_opts.display_fi) {
        PRINTVALSTREAM(rawoutstream, "\n");
        dump_fcontents(fid);
        end_obj(h5tools_dump_header_format->fileend,
                h5tools_dump_header_format->fileblockend);
        PRINTVALSTREAM(rawoutstream, "\n");
        goto done;
      }
