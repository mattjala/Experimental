...
static void extlink_example(void) {
  ...
  source_file_id =
      H5Fcreate(SOURCE_FILE, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
  targ_file_id =
      H5Fcreate(TARGET_FILE, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);

  /* Create a group in the target file for the external link to point to. */
  group_id = H5Gcreate2(targ_file_id, "target_group", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT);
   ...
   */
  H5Lcreate_external(TARGET_FILE, "target_group", source_file_id, "ext_link",
   ...
   */
  group_id = H5Gcreate2(source_file_id, "ext_link/new_group", H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
...
static void extlink_prefix_example(void) {
  ...
  source_file_id =
      H5Fcreate(PREFIX_SOURCE_FILE, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
  red_file_id = H5Fcreate("red/prefix_target.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                          H5P_DEFAULT);
  blue_file_id = H5Fcreate("blue/prefix_target.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                           H5P_DEFAULT);

  /* This test needs a red and a blue directory in the filesystem. If they're
   * not present, trying to create the files above will fail.
   */
  if (red_file_id < 0 || blue_file_id < 0)
    printf("This test requires directories named 'red' and 'blue' to exist. "
   ...
   * name.
   */
  H5Lcreate_external("prefix_target.h5", "/", source_file_id, "ext_link",
                     H5P_DEFAULT, H5P_DEFAULT);
   ...
   */
  group2_id =
      H5Gcreate2(group_id, "pink", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  group_id = H5Gopen2(source_file_id, "ext_link", gapl_id);
  group2_id =
      H5Gcreate2(group_id, "sky blue", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
static void soft_link_example(void) {
  ...
  group_id =
      H5Gcreate2(file_id, TARGET_GROUP, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  H5Gclose(group_id);

  /* This is how we create a normal soft link to the group.
   */
  H5Lcreate_soft(TARGET_GROUP, file_id, SOFT_LINK_NAME, H5P_DEFAULT,
                 H5P_DEFAULT);
   ...
   * as its udata.1
   */
  H5Lcreate_ud(file_id, UD_SOFT_LINK_NAME, (H5L_type_t)UD_SOFT_CLASS,
               TARGET_GROUP, strlen(TARGET_GROUP) + 1, H5P_DEFAULT,
               H5P_DEFAULT);
...
static void hard_link_example(void) {
   ...
   */
  H5Lcreate_hard(file_id, TARGET_GROUP, file_id, HARD_LINK_NAME, H5P_DEFAULT,
                 H5P_DEFAULT);
   ...
   * as its udata.
   */
  H5Lcreate_ud(file_id, UD_HARD_LINK_NAME, (H5L_type_t)UD_HARD_CLASS,
               &(li.u.token), sizeof(H5O_token_t), H5P_DEFAULT, H5P_DEFAULT);
...
static void plist_link_example(void) {
  ...
  file_id = H5Fcreate(HARD_LINK_FILE, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
  group_id =
      H5Gcreate2(file_id, "group_1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  H5Gclose(group_id);
  group_id = H5Gcreate2(file_id, "group_1/group_2", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT);
  H5Gclose(group_id);
   ...
   */
  H5Pinsert2(gapl_id, PLIST_LINK_PROP, sizeof(const char *), &(path), NULL,
