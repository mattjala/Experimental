...
const char *DSETNAME[] = {
    ...
    "dataset25", "dataset26", "dataset27", "dataset28", "dataset29",
    "dataset30", "dataset31", "dataset32", "dataset33", "dataset34",
    "dataset35", "dataset36", "dataset37", "dataset38", "dataset39",
    NULL};
...
int main(void) {
  ...
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 0, H5O_SHMESG_SDSPACE_FLAG | H5O_SHMESG_ATTR_FLAG, 40);
  if (ret < 0)
    goto error;
  ...
  if (ret < 0)
    goto error;
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 0, H5O_SHMESG_DTYPE_FLAG | H5O_SHMESG_SDSPACE_FLAG, 40);
  if (ret < 0)
    goto error;
...
herr_t create_standard_file(const char *filename, hid_t fcpl_id) {
  ...
  for (x = 0; x < NUM_DATASETS; ++x) {
    /* Create a dataset */
    dset_id = H5Dcreate2(file_id, DSETNAME[x], type_id, space_id, H5P_DEFAULT,
