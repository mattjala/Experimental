 ...
 */
void fill_databuf(hsize_t start[], hsize_t count[], hsize_t stride[],
                  C_DATATYPE *data) {
...
static void write_dataset_some_no_sel(hid_t file_id, hid_t dxpl_id) {
  ...
  /* Create the dataset */
  dset_id = H5Dcreate2(file_id, EXAMPLE_DSET_NAME, HDF5_DATATYPE,
                       file_dataspace, H5P_DEFAULT, dcpl_id, H5P_DEFAULT);
  ...
  if (no_selection) {
    ...
    H5Sselect_hyperslab(file_dataspace, H5S_SELECT_SET, start, stride, count,
                        NULL);
  ...
  H5Dwrite(dset_id, HDF5_DATATYPE, no_selection ? H5S_ALL : H5S_BLOCK,
           file_dataspace, dxpl_id, data);
...
int main(int argc, char **argv) {
  ...
  snprintf(filename, PATH_MAX, "%s%s%s", par_prefix ? par_prefix : "",
...
int main(void) {
  printf("HDF5 not configured with parallel support or parallel filtered "
         "writes are disabled!\n");
  return 0;
}
