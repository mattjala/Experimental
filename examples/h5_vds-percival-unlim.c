...
int main(void) {
  ...
  hsize_t vdsdims[3] = {4 * DIM0_1, VDSDIM1, VDSDIM2},
          ...
          dims_max[3] = {DIM0, DIM1, DIM2}, vdsdims_out[3], vdsdims_max_out[3],
  ...
  status =
      H5Sselect_hyperslab(vspace, H5S_SELECT_SET, start, NULL, count, block);
  mem_space = H5Screate_simple(RANK, vdsdims_out, NULL);
  status =
      H5Dread(vdset, H5T_NATIVE_INT, mem_space, vspace, H5P_DEFAULT, rdata);
  printf(" All data: \n");
  ...
  dims[0] = 2 * DIM0_1;
  status =
      H5Sselect_hyperslab(vspace, H5S_SELECT_SET, start, stride, count, block);
  mem_space = H5Screate_simple(RANK, dims, NULL);
  status =
      H5Dread(vdset, H5T_NATIVE_INT, mem_space, vspace, H5P_DEFAULT, a_rdata);
  printf(" All data: \n");
