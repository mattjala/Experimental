...
static void subfiling_write_default(hid_t fapl_id, int mpi_size, int mpi_rank) {
   ...
   *           on this.
   */
  H5Pset_alignment(subfiling_fapl, 0,
                   33554432); /* Align to default 32MiB stripe size */

  /* Parse any parallel prefix and create filename */
  par_prefix = getenv("HDF5_PARAPREFIX");

  snprintf(filename, sizeof(filename), "%s%s%s", par_prefix ? par_prefix : "",
           par_prefix ? "/" : "", EXAMPLE_FILE);
   ...
   * Write to dataset
   */
  H5Dwrite(dset_id, EXAMPLE_DSET_DATATYPE, H5S_BLOCK, filespace, H5P_DEFAULT,
           data);
...
static void subfiling_write_custom(hid_t fapl_id, int mpi_size, int mpi_rank) {
   ...
   */
  H5Pset_alignment(subfiling_fapl, 0,
                   1048576); /* Align to custom 1MiB stripe size */

  /* Parse any parallel prefix and create filename */
  par_prefix = getenv("HDF5_PARAPREFIX");

  snprintf(filename, sizeof(filename), "%s%s%s", par_prefix ? par_prefix : "",
           par_prefix ? "/" : "", EXAMPLE_FILE2);
 ...
 */
static void subfiling_write_precreate(hid_t fapl_id, int mpi_size,
                                      int mpi_rank) {
  EXAMPLE_DSET_C_DATATYPE *data;
  H5FD_subfiling_config_t subf_config;
  hsize_t dset_dims[EXAMPLE_DSET_DIMS];
  ...
  if (mpi_rank == 0) {
     ...
     */
    data =
        malloc(dset_dims[0] * dset_dims[1] * sizeof(EXAMPLE_DSET_C_DATATYPE));
   ...
   */
  H5Dread(dset_id, EXAMPLE_DSET_DATATYPE, H5S_BLOCK, H5S_ALL, H5P_DEFAULT,
          data);
...
int main(void) {
  printf("Example program cannot run - HDF5 must be built with parallel "
         "support and Subfiling VFD support\n");
  return 0;
}
