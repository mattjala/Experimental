...
int main(void) {
  ...
  hsize_t vdsdims[3] = {4 * DIM0_1, VDSDIM1, VDSDIM2},
          ...
          extdims[3] = {0, DIM1,
                        DIM2}, /* Dimensions of the extended source datasets */
      chunk_dims[3] = {DIM0_1, DIM1, DIM2}, dims_max[3] = {DIM0, DIM1, DIM2},
          vdsdims_out[3], vdsdims_max_out[3],
  ...
  for (i = 0; i < PLANE_STRIDE; i++) {
    ...
    dset = H5Dcreate2(file, SRC_DATASET[i], H5T_NATIVE_INT, src_space,
                      H5P_DEFAULT, dcpl, H5P_DEFAULT);
    status =
        H5Dwrite(dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wdata);
   ...
   *
   */
  status = H5Sselect_hyperslab(src_space, H5S_SELECT_SET, start, NULL,
                               src_count, block);
  for (i = 0; i < PLANE_STRIDE; i++) {
    status = H5Sselect_hyperslab(vspace, H5S_SELECT_SET, start, stride, count,
                                 block);
    status =
        H5Pset_virtual(dcpl, vspace, SRC_FILE[i], SRC_DATASET[i], src_space);
    start[0]++;
  }
  ...
  vdset = H5Dcreate2(vfile, DATASET, H5T_NATIVE_INT, vspace, H5P_DEFAULT, dcpl,
                     H5P_DEFAULT);
  ...
  for (i = 0; i < PLANE_STRIDE; i++) {
    ...
    status = H5Sselect_hyperslab(src_space, H5S_SELECT_SET, start, NULL, count,
                                 block);
  ...
  for (i = 0; i < 2; i++) {
    status = H5Pset_virtual_view(dapl, i ? H5D_VDS_LAST_AVAILABLE
                                         : H5D_VDS_FIRST_MISSING);
    vdset = H5Dopen2(vfile, DATASET, dapl);

    /* Let's get space of the VDS and its dimension; we should get 32(or
     * 20)x10x10 */
    vspace = H5Dget_space(vdset);
    H5Sget_simple_extent_dims(vspace, vdsdims_out, vdsdims_max_out);
    printf("VDS dimensions, bounds = H5D_VDS_%s: ",
           i ? "LAST_AVAILABLE" : "FIRST_MISSING");
    for (j = 0; j < RANK; j++)
  ...
  for (i = 0; i < (int)num_map; i++) {
    ...
    if (H5Sget_select_type(src_space) == H5S_SEL_HYPERSLABS) {
      if (H5Sis_regular_hyperslab(src_space)) {
        status = H5Sget_regular_hyperslab(src_space, start_out, stride_out,
                                          count_out, block_out);
        printf("         start  = [%llu, %llu, %llu] \n",
               (unsigned long long)start_out[0],
               (unsigned long long)start_out[1],
