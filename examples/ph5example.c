...
void dataset_fill(hsize_t start[], hsize_t count[], hsize_t stride[],
                  DATATYPE *dataset);
void dataset_print(hsize_t start[], hsize_t count[], hsize_t stride[],
                   DATATYPE *dataset);
int dataset_vrfy(hsize_t start[], hsize_t count[], hsize_t stride[],
                 DATATYPE *dataset, DATATYPE *original);
...
void dataset_fill(hsize_t start[], hsize_t count[], hsize_t stride[],
  ...
  for (i = 0; i < count[0]; i++) {
    for (j = 0; j < count[1]; j++) {
      *dataptr++ =
          (i * stride[0] + start[0]) * 100 + (j * stride[1] + start[1] + 1);
    }
  }
}
      ...
      if (*dataset++ != *original++) {
        ...
        if (nerr <= MAX_ERR_REPORT) {
          printf("Dataset Verify failed at [%lu][%lu](row %lu, col %lu): "
                 ...
                 (unsigned long)(j * stride[1] + start[1]), *(dataset - 1),
                 *(original - 1));
        }
      }
    }
...
void phdf5writeInd(char *filename) {
  ...
  hsize_t dims1[SPACE1_RANK] = {SPACE1_DIM1,
                                SPACE1_DIM2};     /* dataspace dim sizes */
  ...
  dataset1 = H5Dcreate2(fid1, DATASETNAME1, H5T_NATIVE_INT, sid1, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  assert(dataset1 != FAIL);
  MESG("H5Dcreate2 succeed");

  /* create another dataset collectively */
  dataset2 = H5Dcreate2(fid1, DATASETNAME2, H5T_NATIVE_INT, sid1, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
...
void phdf5readInd(char *filename) {
  ...
  ret = dataset_vrfy(start, count, stride, &data_array1[0][0],
                     &data_origin1[0][0]);
  assert(ret != FAIL);
