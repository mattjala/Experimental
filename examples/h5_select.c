...
int main(void) {
   ...
   */
  dataset = H5Dcreate2(file, "Matrix in file", H5T_NATIVE_INT, fid, H5P_DEFAULT,
                       plist, H5P_DEFAULT);
  ...
  /*
   * Select hyperslab.
   * We will use 48 elements of the vector buffer starting at the second
   * element. Selected elements are 1 2 3 . . . 48
   */
  start[0] = 1;
  stride[0] = 1;
  count[0] = 48;
  ...
  ret =
      H5Sselect_elements(fid, H5S_SELECT_SET, NPOINTS, (const hsize_t *)coord);
