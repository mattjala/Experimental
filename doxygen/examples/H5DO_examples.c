...
size_t       buf_size = CHUNK_NX*CHUNK_NY*sizeof(int);
                 ...
                 * properties */
                if ((dset_id = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT,
                 ...
                 */
                ret = compress2(z_dst, &z_dst_nbytes, z_src, z_src_nbytes,
                                aggression);
                ...
                /* Overwrite the first chunk with uncompressed data.  Set the
                 * filter mask to
                 *  * indicate the compression filter is skipped */
                filter_mask = 0x00000001;
                offset[0] = offset[1] = 0;
                if (H5DOwrite_chunk(dset_id, H5P_DEFAULT, filter_mask, offset,
                                    buf_size, direct_buf) < 0)
                  goto error;

                /* Read the entire dataset back for data verification converting
                 * ints to longs */
                if (H5Dread(dataset, H5T_NATIVE_LONG, H5S_ALL, H5S_ALL,
                            H5P_DEFAULT, outbuf_long) < 0)
                  goto error;
...
size_t       buf_size = CHUNK_NX*CHUNK_NY*sizeof(int);
            ...
            int read_dst_buf[CHUNK_NX]
                            [CHUNK_NY]; /* Buffer to hold un-compressed data */
             ...
             * properties */
            if ((dset_id =
                     H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, dataspace,
            ...
            ret =
                H5Dget_chunk_storage_size(dataset, offset, &read_chunk_nbytes);

            readbuf = malloc(read_chunk_nbytes);
            pt_readbuf = (const Bytef *)readbuf;

            /* Use H5DOread_chunk() to read the chunk back */
            if ((status = H5DOread_chunk(dataset, H5P_DEFAULT, offset,
                                         &read_filter_mask, readbuf)) < 0)
              goto error;

            ret = uncompress((Bytef *)read_dst_buf, (uLongf *)&buf_size,
                             pt_readbuf, (uLong)read_chunk_nbytes);
