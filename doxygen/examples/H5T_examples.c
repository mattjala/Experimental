...
int main(void) {
  ...
  //! <!-- [create] -->
  {
    ...
    if ((file = H5Fcreate("t1.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_file;
    }
    // create a compound datatype with room for real and imaginary parts
    if ((dtype = H5Tcreate(H5T_COMPOUND, 2 * sizeof(double))) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_dtype;
    }
    ...
    if (H5Tcommit(file, "pre-complex", dtype, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
      ret_val = EXIT_FAILURE;
    }
  ...
  //! <!-- [read] -->
  {
    ...
    if ((file = H5Fopen("t1.h5", H5F_ACC_RDONLY, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_file;
    }
    // open the datatype object stored in the file
    if ((dtype = H5Topen(file, "pre-complex", H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_dtype;
    }
    ...
    switch (
        H5Tget_class(dtype)) { // this time we are only interested in compounds
    case H5T_COMPOUND:
  ...
  //! <!-- [update] -->
  {
    ...
    if ((file = H5Fopen("t1.h5", H5F_ACC_RDWR, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_file;
    }
    ...
    if (H5Tcommit(file, "complex", clone, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
      ret_val = EXIT_FAILURE;
    }
