...
//! <!-- [iter_cb] -->
herr_t iter_cb(hid_t group, const char *name, const H5L_info_t *info,
               void *op_data) {
...
int main(void) {
  ...
  //! <!-- [create] -->
  {
    ...
    if ((file = H5Fcreate("l1.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
    ...
    if (H5Lcreate_soft("e1 62 80 87 04 09 43 ba 02 d3", file,
                       "/path/to/nowhere", lcpl, H5P_DEFAULT) < 0) {
      ret_val = EXIT_FAILURE;
      goto fail_link;
    }
    // create an external link to nowhere in a non-existent file
    if (H5Lcreate_external("non-existent-file.h5", "???", file, "external",
                           lcpl, H5P_DEFAULT) < 0) {
      ret_val = EXIT_FAILURE;
      goto fail_link;
    }
  ...
  //! <!-- [read] -->
  {
    ...
    if ((file = H5Fopen("l1.h5", H5F_ACC_RDONLY, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_read;
    }

    if (H5Literate(file, H5_INDEX_NAME, H5_ITER_NATIVE, &idx, iter_cb, NULL) <
        0) {
      ret_val = EXIT_FAILURE;
      goto fail_iterate;
    }
  ...
  //! <!-- [update] -->
  {
    ...
    if ((file = H5Fopen("l1.h5", H5F_ACC_RDWR, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_update;
    }
