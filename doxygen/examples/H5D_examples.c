...
//! <!-- [H5Dchunk_iter_cb] -->
int chunk_cb(const hsize_t *offset, unsigned filter_mask, haddr_t addr,
...
int main(void) {
  ...
  //! <!-- [create] -->
  {
    ...
    if ((dset = H5Dcreate2(file, dset_name, H5T_STD_I32LE, fspace, lcpl,
  ...
  //! <!-- [update] -->
  {
    ...
    if (H5Sselect_hyperslab(fspace, H5S_SELECT_SET, (hsize_t[]){1},
                            (hsize_t[]){2}, (hsize_t[]){5}, NULL) < 0) {
      ret_val = EXIT_FAILURE;
      goto fail_update;
    }

    // (implicitly) select and write the first 5 elements of the second column
    // of NEW_ELTS
    if (H5Dwrite(dset, H5T_NATIVE_INT, H5S_ALL, fspace, H5P_DEFAULT, new_elts) <
        0)
      ret_val = EXIT_FAILURE;
