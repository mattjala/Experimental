...
int main(void) {
  ...
  //! <!-- [create] -->
  {
    ...
    if ((dspace = H5Screate_simple(2, (hsize_t[]){8, 8}, NULL)) ==
        H5I_INVALID_HID) {
  ...
  //! <!-- [update] -->
  {
    ...
    if (H5Sselect_hyperslab(dspace, H5S_SELECT_SET, (hsize_t[]){0, 0},
                            (hsize_t[]){2, 2}, (hsize_t[]){4, 4}, NULL) < 0) {
      ret_val = EXIT_FAILURE;
      goto fail_select;
    }
    // select the black fields on odd rows
    // notice the H5S_SELECT_OR operator
    if (H5Sselect_hyperslab(dspace, H5S_SELECT_OR, (hsize_t[]){1, 1},
                            (hsize_t[]){2, 2}, (hsize_t[]){4, 4}, NULL) < 0) {
      ret_val = EXIT_FAILURE;
      goto fail_select;
    }
