...
int main(void) {
  ...
  //! <!-- [create] -->
  {
    __label__ fail_file, fail_fspace, fail_dset, fail_sel, fail_aspace,
        fail_attr, fail_awrite;
    hid_t file, fspace, dset, aspace, attr;
    H5R_ref_t ref;

    if ((file = H5Fcreate("reference.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                          H5P_DEFAULT)) == H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_file;
    }
    // create a region reference which selects all elements of the dataset at
    // "/data"
    if ((fspace = H5Screate_simple(2, (hsize_t[]){10, 20}, NULL)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_fspace;
    }
    if ((dset = H5Dcreate(file, "data", H5T_STD_I32LE, fspace, H5P_DEFAULT,
    ...
    // store the region reference in a scalar attribute of the root group called
    // "region"
    if ((aspace = H5Screate(H5S_SCALAR)) == H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_aspace;
    }
    if ((attr = H5Acreate(file, "region", H5T_STD_REF, aspace, H5P_DEFAULT,
                          H5P_DEFAULT)) == H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_attr;
    }
  ...
  //! <!-- [read] -->
  {
    ...
    if ((file = H5Fopen("reference.h5", H5F_ACC_RDONLY, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_file;
    }
  ...
  //! <!-- [update] -->
  {
    ...
    if ((file = H5Fopen("reference.h5", H5F_ACC_RDWR, H5P_DEFAULT)) ==
        H5I_INVALID_HID) {
      ret_val = EXIT_FAILURE;
      goto fail_file;
    }
