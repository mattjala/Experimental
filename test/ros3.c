...
static int test_fapl_config_validation(void) {
  ...
  for (i = 0; i < ncases; i++) {
    ...
    if (success == SUCCEED) {
      ...
      JSVERIFY(SUCCEED, H5Pget_fapl_ros3(fapl_id, &fa_fetch),
               "unable to get fapl")

      JSVERIFY(H5FD_CURR_ROS3_FAPL_T_VERSION, fa_fetch.version,
               "invalid version number")
      JSVERIFY(config.version, fa_fetch.version, "version number mismatch")
      JSVERIFY(config.authenticate, fa_fetch.authenticate,
 ...
 *-------------------------------------------------------------------------
 */
static int test_ros3_fapl(void) {
  /************************
   * test-local variables *
   ************************/
...
static int test_eof_eoa(void) {
  ...
  fd_shakespeare =
      H5FDopen(url_text_restricted, H5F_ACC_RDONLY, fapl_id, HADDR_UNDEF);
  ...
  JSVERIFY(0, H5FDget_eoa(fd_shakespeare, H5FD_MEM_DEFAULT),
           "EoA should be unset by H5FDopen")

  /* set EoA below EoF
   */
  JSVERIFY(SUCCEED, H5FDset_eoa(fd_shakespeare, H5FD_MEM_DEFAULT, 44442202),
           "unable to set EoA (lower)")
  JSVERIFY(5458199, H5FDget_eof(fd_shakespeare, H5FD_MEM_DEFAULT),
           "EoF changed")
  JSVERIFY(44442202, H5FDget_eoa(fd_shakespeare, H5FD_MEM_DEFAULT),
           "EoA unchanged")

  /* set EoA above EoF
   */
  JSVERIFY(SUCCEED, H5FDset_eoa(fd_shakespeare, H5FD_MEM_DEFAULT, 6789012),
           "unable to set EoA (higher)")
  JSVERIFY(5458199, H5FDget_eof(fd_shakespeare, H5FD_MEM_DEFAULT),
           "EoF changed")
  JSVERIFY(6789012, H5FDget_eoa(fd_shakespeare, H5FD_MEM_DEFAULT),
           "EoA unchanged")
...
static int test_H5FDread_without_eoa_set_fails(void) {
  ...
  file_shakespeare =
      H5FDopen(url_text_restricted, H5F_ACC_RDONLY, fapl_id, MAXADDR);
  ...
  H5E_BEGIN_TRY{/* mute stack trace on expected failure */
                JSVERIFY(FAIL,
                         H5FDread(file_shakespeare, H5FD_MEM_DRAW, H5P_DEFAULT,
                                  1200699, 102, buffer),
...
static int test_read(void) {
  ...
  file_raven =
      H5FDopen(                 /* will open with "authenticating" fapl */
               url_text_public, /* TODO: check return state: anon access of
                                 * restricted says OK? (not NULL)
                                 */
               H5F_ACC_RDONLY, fapl_id,
  ...
  for (test_i = 0; test_i < testcase_count; test_i++) {
    ...
    H5E_BEGIN_TRY {
      open_return = H5FDread(file_raven, H5FD_MEM_DRAW, H5P_DEFAULT, test.addr,
                             test.len, buffer);
    }
    H5E_END_TRY
...
static int test_noops_and_autofails(void) {
   ...
   */
  H5E_BEGIN_TRY{JSVERIFY(
      FAIL, H5FDwrite(file, H5FD_MEM_DRAW, H5P_DEFAULT, 1000, 35, data),
      "write must fail")} H5E_END_TRY

      H5E_BEGIN_TRY{JSVERIFY(FAIL, H5FDtruncate(file, H5P_DEFAULT, FALSE),
                             "truncate must fail")} H5E_END_TRY
...
static int test_H5F_integration(void) {
  ...
  H5E_BEGIN_TRY{
       ...
       */
      H5E_BEGIN_TRY{FAIL_IF(0 <= H5Fcreate(url_missing, H5F_ACC_RDONLY,
                                           H5P_DEFAULT, fapl_id))} H5E_END_TRY
...
int main(void) {
  ...
  if (S3_TEST_MAX_URL_SIZE <
      HDsnprintf(url_text_restricted, (size_t)S3_TEST_MAX_URL_SIZE, "%s/%s",
  ...
  if (S3_TEST_MAX_URL_SIZE <
      HDsnprintf(url_text_public, (size_t)S3_TEST_MAX_URL_SIZE, "%s/%s",
                 (const char *)s3_test_bucket_url,
  ...
  if (S3_TEST_MAX_URL_SIZE <
      HDsnprintf(url_h5_public, (size_t)S3_TEST_MAX_URL_SIZE, "%s/%s",
                 (const char *)s3_test_bucket_url,
  ...
  if (S3_TEST_MAX_URL_SIZE <
      HDsnprintf(url_missing, S3_TEST_MAX_URL_SIZE, "%s/%s",
                 (const char *)s3_test_bucket_url,
