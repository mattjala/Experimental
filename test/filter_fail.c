...
static const char *FILENAME[] = {"filter_fail_with_cache",
                                 "filter_fail_without_cache", NULL};
 ...
 *-------------------------------------------------------------------------
 */
static size_t filter_fail(unsigned int flags, size_t H5_ATTR_UNUSED cd_nelmts,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_filter_write(char *file_name, hid_t my_fapl,
  ...
  /* create a dataset */
  if ((dataset = H5Dcreate2(file, DSET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Write data.  If the chunk cache is enabled, H5Dwrite should succeed.  If it
   * is disabled, H5Dwrite should fail. */
  if (cache_enabled) {
    if (H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, sid, H5P_DEFAULT, points) <
        0)
      TEST_ERROR;
  } else {
    ...
    H5E_BEGIN_TRY {
      ret =
          H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, sid, H5P_DEFAULT, points);
    }
    H5E_END_TRY
    if (ret >= 0) {
      H5_FAILED();
      HDputs("    Data writing is supposed to fail because the chunk can't be "
             "written to file.");
      TEST_ERROR;
    }
  }
  ...
  /* Close dataset.  If the chunk cache is enabled, the flushing of chunks
   * should fail during H5Dclose.  If it is disabled, H5Dwrite should fail but
   * H5Dclose should succeed. */
  if (cache_enabled) {
    H5E_BEGIN_TRY { ret = H5Dclose(dataset); }
    H5E_END_TRY
    if (ret >= 0) {
      H5_FAILED();
      HDputs("    Dataset is supposed to fail because the chunk can't be "
             "flushed to file.");
      TEST_ERROR;
    }
  } else {
...
static herr_t test_filter_read(char *file_name, hid_t my_fapl) {
  ...
  if (H5Sselect_hyperslab(sid, H5S_SELECT_SET, hs_offset, stride, hs_size,
                          NULL) < 0)
    TEST_ERROR;
  ...
  if (H5Sselect_hyperslab(mspace, H5S_SELECT_SET, zero, stride, &nelmts, NULL) <
      0)
    TEST_ERROR;
