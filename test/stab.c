 ...
 *-------------------------------------------------------------------------
 */
static int test_misc(hid_t fcpl, hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1); /* File ID */
  ...
  if ((g1 = H5Gcreate2(fid, "test_1a", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if ((g2 = H5Gcreate2(g1, "sub_1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((g3 = H5Gcreate2(fid, "test_1b", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  if ((g3 = H5Gopen2(fid, "/test_1b", H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_comment_by_name(g3, "././.", comment, sizeof comment,
                             H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_long(hid_t fcpl, hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1); /* File ID */
  hid_t g1 = (-1), g2 = (-1);
  size_t name2Len;
 ...
 *-------------------------------------------------------------------------
 */
static int test_large(hid_t fcpl, hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1);             /* File ID */
   ...
   */
  if ((cwg = H5Gcreate2(fid, "/big", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (new_format)
  ...
  for (i = 0; i < LARGE_NOBJS; i++) {
    ...
    if ((dir = H5Gcreate2(cwg, name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
        0)
      TEST_ERROR;
    if (H5Gclose(dir) < 0)
      TEST_ERROR;
...
static int lifecycle(hid_t fcpl, hid_t fapl2) {
  ...
  /* Set GCPL parameters */
  if (H5Pset_local_heap_size_hint(gcpl,
                                  (size_t)LIFECYCLE_LOCAL_HEAP_SIZE_HINT) < 0)
    TEST_ERROR;
  if (H5Pset_link_phase_change(gcpl, LIFECYCLE_MAX_COMPACT,
                               LIFECYCLE_MIN_DENSE) < 0)
    TEST_ERROR;
  if (H5Pset_est_link_info(gcpl, LIFECYCLE_EST_NUM_ENTRIES,
                           LIFECYCLE_EST_NAME_LEN) < 0)
    TEST_ERROR;

  /* Create group for testing lifecycle */
  if ((gid = H5Gcreate2(fid, LIFECYCLE_TOP_GROUP, H5P_DEFAULT, gcpl,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Use internal testing routine to check that the group has no links or symbol
   * table */
  ...
  /* Create several more bottom groups, to push the top group almost to a symbol
   * table */
  ...
  /* Check that the object header is only one chunk and the space has been
   * allocated correctly */
  ...
  /* Create one more "bottom" group, which should push top group into using a
   * symbol table */
  HDsnprintf(objname, sizeof(objname), LIFECYCLE_BOTTOM_GROUP, u);
  if ((gid2 = H5Gcreate2(gid, objname, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
  ...
  /* Check that the object header is still one chunk and the space has been
   * allocated correctly */
  if (H5Oget_native_info(gid, &ninfo, H5O_NATIVE_INFO_HDR) < 0)
    TEST_ERROR;
  if (ninfo.hdr.space.total != 151)
  ...
  /* Unlink one more object from the group, which should transform back to using
   * links */
  HDsnprintf(objname, sizeof(objname), LIFECYCLE_BOTTOM_GROUP, u);
  if (H5Ldelete(gid, objname, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
...
static int long_compact(hid_t fcpl, hid_t fapl2) {
  ...
  /* Use internal testing routine to check that the group has no links or dense
   * storage */
  if (H5G__is_empty_test(gid) != TRUE)
    TEST_ERROR;
  ...
  /* Check on top group's status */
  /* (Should have dense storage to hold links, since name is too long for object
   * header message) */
  if (H5G__is_empty_test(gid) == TRUE)
    TEST_ERROR;
  if (H5G__has_links_test(gid, NULL) == TRUE)
  ...
  /* Check on top group's status */
  /* (Should still be dense storage to hold links, since name is too long for
...
static int no_compact(hid_t fcpl, hid_t fapl2) {
  ...
  /* Set GCPL parameters */
  if (H5Pset_link_phase_change(gcpl, NO_COMPACT_MAX_COMPACT,
                               NO_COMPACT_MIN_DENSE) < 0)
    TEST_ERROR;
  ...
  if ((gid = H5Gcreate2(fid, NO_COMPACT_TOP_GROUP, H5P_DEFAULT, gcpl,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static int gcpl_on_root(hid_t fapl2) {
  ...
  /* Set GCPL parameters */
  if (H5Pset_link_phase_change(fcpl, GCPL_ON_ROOT_MAX_COMPACT,
                               GCPL_ON_ROOT_MIN_DENSE) < 0)
    TEST_ERROR;
  ...
  /* Create a group and intermediate groups, to check if root group settings are
   * inherited */
  if ((gid2 = H5Gcreate2(gid, GCPL_ON_ROOT_BOTTOM_GROUP, lcpl, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
int main(void) {
  ...
  if (H5Pset_file_space_strategy(fcpl2, H5F_FSPACE_STRATEGY_PAGE, 1,
                                 (hsize_t)1) < 0)
    TEST_ERROR;
  ...
  for (new_format = FALSE; new_format <= TRUE; new_format++) {
    ...
    if (new_format) {
      my_fapl = fapl2;
      my_fcpl =
          fcpl2; /* Set to use paged aggregation and persisting free-space */
    }
