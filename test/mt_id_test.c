 ...
 *
 * Note that id_type_kernel_t is included in this section, as it gathers
 *together all fields of id_type_t that must be handled as a single atomic unit.
 *
 ...
 * Stats:
 *
 * successful_registers: Number of times the type has been registered
 *successfully.
 *
 ...
 *
 * Note that id_object_kernel_t is included in this header comment, as it
 *gathers together all fields of id_object_t that must be handled as a single
 *atomic unit.
 *
 ...
 *              inconsistency in the fields that are modified during the
 *realization of the future ID, and the ID type is not marked as thread safe,
 *              this is an error in the HDF5 code.
 *
 * k            Atomic instance of id_object_kernel_t -- a structure that
 *contains all fields in id_object_t that must be treated as a single atomic
 *              value.  These fields are discussed individually below:
 *
 ...
 *
 * real_id_obj_index: If k.real_id_defined is TRUE, this field contains the
 *index of the instance of id_object_kernel_t in its containing array. Otherwise
 ...
 * The fields of this structure are discussed individually below.
 *
 * Note that id_instance_kernel_t is included in this header comment, as it
 *gathers together all fields of id_instance_t that must be handled as a single
 *atomic unit.
 *
 * tag:         Integer field that must be set to ID_INSTANCE_T__TAG.  This
 *field is used to sanity check pointers.
 *
 * index:       integer field containing the index of this instance of
 *              id_instance_t in its containing array.
 *
 * k            Atomic instance of id_instance_kernel_t -- a structure that
 *contains all fields in id_instance_t that must be treated as a single atomic
 *              value.  These fields are discussed individually below:
 *
 * k.in_progress:  Creation of an id is not atomic.  This field is set
 *              when a thread is about to create an ID.  If successful, it
 ...
 *
 *              When the linked future ID is realized, it discards its own
 *object, and adopts the object from the source real ID.  When this is done, the
 *obj_index field is invalidated by setting it to -1.
 *
 ...
 *
 * type_id:     Atomic instance of H5I_type_t.  This field is a convenience
 *field for use when type_index is non-negative.  In this context, it is
 *              initialized to H5I_BADID, and set to the id of the type
 *              associated with the instance of id_type_t indicated by
 *type_index if it is defined.  Note that that this field being set does not
 ...
 *              atomically when a call to H5Iremove_verify() completes
 *successfully. As a result it is possible for other calls to fail when
 *according to the test bed, the target ID exists and the other call should have
 *succeeded.
 *
 ...
 * instance of id_instance_t is associated.  All statistics fields are
 *initialized to zero, and incremented to reflect the target events.
 *
 ...
 *
 * successful_remove_verifies: Atomic long long integer used to track the number
 *of successful attempts to remove the ID and return a pointer to its associated
 *object.
 *
 ...
 *
 * ds: 	Boolean flag instructing the receiving function to display stats on
 *exit.
 *
...
int register_type(id_type_t *id_type_ptr, hbool_t cs, hbool_t ds,
                  hbool_t rpt_failures, int tid);
void try_register_type(int type_index, hbool_t cs, hbool_t ds,
                       hbool_t rpt_failures, int tid);
int clear_type(id_type_t *id_type_ptr, hbool_t force, hbool_t cs, hbool_t ds,
               hbool_t rpt_failures, int tid);
void try_clear_type(int type_index, hbool_t force, hbool_t cs, hbool_t ds,
                    hbool_t rpt_failures, int tid);
int destroy_type(id_type_t *id_type_ptr, hbool_t cs, hbool_t ds,
                 hbool_t rpt_failures, int tid);
int try_destroy_type(int type_index, hbool_t cs, hbool_t ds,
                     hbool_t rpt_failures, int tid);

int register_id(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
                id_object_t *id_obj_ptr, hbool_t cs, hbool_t ds,
                hbool_t rpt_failures, int tid);
void try_register_id(int id_index, int obj_index, hbool_t cs, hbool_t ds,
                     hbool_t rpt_failures, int tid);
int register_future_id(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
                       id_object_t *id_obj_ptr,
                       H5I_future_realize_func_t realize_cb,
                       H5I_future_discard_func_t discard_cb, hbool_t cs,
                       hbool_t ds, hbool_t rpt_failures, int tid);
int link_real_and_future_ids(id_object_t *future_id_obj_ptr,
                             id_object_t *real_id_obj_ptr,
                             hbool_t rpt_failures);
int object_verify(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
                  id_object_t *id_obj_ptr, hbool_t cs, hbool_t ds,
                  hbool_t rpt_failures, int tid);
int try_object_verify(int id_index, hbool_t cs, hbool_t ds,
                      hbool_t rpt_failures, int tid);
int get_type(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr, hbool_t cs,
             hbool_t ds, hbool_t rpt_failures, int tid);
int remove_verify(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
                  id_object_t *id_obj_ptr, hbool_t cs, hbool_t ds,
                  hbool_t rpt_failures, int tid);
int try_remove_verify(int id_index, hbool_t cs, hbool_t ds,
                      hbool_t rpt_failures, int tid);
int dec_ref(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
            id_object_t *id_obj_ptr, hbool_t cs, hbool_t ds,
            hbool_t rpt_failure, int tid);
int try_dec_ref(int id_index, hbool_t cs, hbool_t ds, hbool_t rpt_failures,
                int tid);
int inc_ref(id_instance_t *id_inst_ptr, hbool_t cs, hbool_t ds,
            hbool_t rpt_failures, int tid);
int try_inc_ref(int id_index, hbool_t cs, hbool_t ds, hbool_t rpt_failures,
                int tid);
int get_ref(id_instance_t *id_inst_ptr, hbool_t cs, hbool_t ds,
            hbool_t rpt_failures, int tid);
int try_get_ref(int id_index, hbool_t cs, hbool_t ds, hbool_t rpt_failures,
                int tid);
int nmembers(id_type_t *id_type_ptr, hbool_t cs, hbool_t ds,
             hbool_t rpt_failures, int tid);
htri_t type_exists(id_type_t *id_type_ptr, hbool_t cs, hbool_t ds,
                   hbool_t rpt_failures, int tid);
int inc_type_ref(id_type_t *id_type_ptr, hbool_t cs, hbool_t ds,
                 hbool_t rpt_failures, int tid);
int dec_type_ref(id_type_t *id_type_ptr, hbool_t cs, hbool_t ds,
                 hbool_t rpt_failures, int tid);
int try_dec_type_ref(id_type_t *id_type_ptr, hbool_t cs, hbool_t ds,
                     hbool_t rpt_failures, int tid);

int create_types(int types_start, int types_count, int types_stride, hbool_t cs,
                 hbool_t ds, hbool_t rpt_failures, int tid);
...
int register_ids(int types_start, int types_count, int types_stride,
                 int ids_start, int ids_count, int ids_stride, hbool_t cs,
                 hbool_t ds, hbool_t rpt_failures, int tid);
int dec_refs(int types_start, int types_count, int types_stride, int ids_start,
             int ids_count, int ids_stride, hbool_t cs, hbool_t ds,
             hbool_t rpt_failures, int tid);
int inc_refs(int ids_start, int ids_count, int ids_stride, hbool_t cs,
             hbool_t ds, hbool_t rpt_failures, int tid);
...
void serial_test_2(int types_start, int types_count, int ids_start,
                   int ids_count);
...
void init_globals(void) {
  ...
  if ((NULL == types_array) || (NULL == objects_array) ||
      (NULL == id_instance_array)) {

    fprintf(
        stderr,
        "init_globals(): One or more array allocations failed -- exiting.\n");
    exit(1);
  }
 ...
 *
 *      In the context of the H5I test code, this means that both the instance
 *of id_object_t pointed to by the obj parameter, and the instance of
 *id_instance_t associated with it must be marked as deleted.
 *
 *      Note that this implementation of the free function is not multi-thread
 *safe -- and must therefore not be used with ID types that are marked as
 *multi-thread safe.
 *
 *                                                        JRM -- 4/16/24
 *
...
herr_t free_func(void *obj, void H5_ATTR_UNUSED **request) {
  ...
  if ((atomic_compare_exchange_strong(&(id_instance_array[id_index].k),
 ...
 *
 *      2) Verify that the supplied future object is marked as being a future
 *object,
 *
 *      3) Verify that a real object for the supplied future object has been
 *created (i.e. that the real_id_defined flag is set), and
 *
 *      4) Verify that the supplied future object has not yet been realized
 *(i.e. that the future_id_realized flag has not been set).
 *
 *      5) Verify that the supplied future object has not yet been discarded
 *(i.e. that the future_id_discarded_flag has not beem set.
 *
 *      If any of these conditions FAIL, return FAIL
 *
 *      If all of these conditions are met, attempt to update the test test
 *framework to reflect the immenant relization of the future ID as follows:
 *
 *      1) Invalidate the obj_index field of the id_instance_t associated with
 *the real id.  Do this with a compare and set, so as to detect the case in
 *which the real ID has been deleted.  If this is the case, return failure.
 *
 *         Note that we are running into a fundamental problem with the future
 *ID mechanism -- since H5I doesn't know anything about the real ID until it
 *         gets a successful return from the realize callback, it can't protect
 *         the real ID from deletion prior to its consumption as part of fhe
 *future ID realization.
 *
 *         Needless to say, a well behaved host application can do this, but we
 *don't want to depend on this.  Thus for now, I am papering over the problem in
 *my test bed, as the effect of using a compare and set here is to cause the
 *         realize callback to fail on subsequent if the first realize call
 *succeeds.
 *
 *      2) Set the old_id_index field of the id_object_t associated with the
 *real_id equal to its id_index field, and then set the id_index field equal to
 *the same field in the future object.
 *
 *      3) Set the obj_index field in the future instance of id_instance_t to
 *the value of the index field of the real_id instance of id_object_t.
 *
 *      4) Invalidate the id_index field of the future id instance of
 *id_object_t.
 *
 *      5) set set the furure_id_realized flag via a call to
 *          atomic_compare_exchange_strong() on &(future_id->k).
 *
 *      If all the above succeed, set *actual_object_id to
 *future_object->real_id, and return SUCCEED.
 *
 *      Otherwise, return FAIL.
 *
 *      Observe that the operations of the realize callback are anything but
 *atomic.  As long as the host type is not listed as mulit-thread safe, this
 *function should be run within the global mutex -- making this point moot.
 *
 *                                                              JRM -- 3/8/24
 *
 * Returs:
...
herr_t realize_cb_0(void *future_object, hid_t *actual_object_id) {
  ...
  if ((NULL == future_id_obj_ptr) ||
      (ID_OBJECT_T__TAG != future_id_obj_ptr->tag) ||
  ...
  if (success) {
    ...
    if ((future_id_obj_k.in_progress) || (!future_id_obj_k.allocated) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr, "realize_cb_0(): future id obj in progress, not "
                        "created, or discarded on entry.\n");
      }
    }
  }
  ...
  if ((success) && (!future_id_obj_k.real_id_defined)) {
    ...
    /* real ID isn't defined yet -- function reports failure, but no error
     * message */
   ...
   *
   * If so, just return failure since this condition will occur if the source
   * real ID is deleted before the future ID is realized.  If so, the realize_cb
   * will succeed the first time around, but the realization will fail when H5I
   * tries to remove the
   */
  ...
  if (success) {
    ...
    /* Initial sanity have passed -- attempt to realize the ID.  Assuming that
     * H5I is maintaining the global lock during the realize_cb call, and there
     * is no attempt to re-use instances of id_instance_t or id_object_t, this
     * operation should succeed.
     */

    /* get indicies and pointers to the real id instance and object, and the
     * future id_instance.
     *
     * Do basic sanity checks, but don't verify that the real ID is still valid.
     * If it has been deleted, this should be caught by the H5I future ID
     * realization code.
     */
    ...
    assert(atomic_load(&(real_id_inst_ptr->obj_index)) ==
           real_id_obj_ptr->index);
    assert(atomic_load(&(real_id_obj_ptr->id_index)) ==
           real_id_inst_ptr->index);

    assert(atomic_load(&(future_id_inst_ptr->obj_index)) ==
           future_id_obj_ptr->index);
    assert(atomic_load(&(future_id_obj_ptr->id_index)) ==
           future_id_inst_ptr->index);

    /* Invalidate the obj_index field of the id_instance_t associated with the
     * real id.  Do this with a compare and set, so as to detect the case in
     * which the real ID has been deleted.  If this is the case, return failure.
     *
     ...
     *
     * Needless to say, a well behaved host application can do this, but we
     * don't want to depend on this.  Thus for now, I am papering over the
     * problem in my test bed, as the effect of using a compare and set here is
     * to cause the realize callback to fail on subsequent if the first realize
     * call succeeds.
     */
    expected_obj_index = real_id_obj_index;

    if (!atomic_compare_exchange_strong(&(real_id_inst_ptr->obj_index),
                                        &expected_obj_index, -1)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "realize_cb_0(): expected real_id_inst_ptr->obj_index = %d, "
                "actual = %d.\n",
  ...
  if (success) {
    ...
    atomic_store(&(real_id_obj_ptr->old_id_index),
                 atomic_load(&(real_id_obj_ptr->id_index)));
    atomic_store(&(real_id_obj_ptr->id_index), future_id_inst_index);

    /* Set the obj_index field in the future instance of id_instance_t to the
     * value of the index field of the real_id instance of id_object_t.
     */
    assert(future_id_obj_ptr->index ==
           atomic_load(&(future_id_inst_ptr->obj_index)));
    atomic_store(&(future_id_inst_ptr->obj_index), real_id_obj_ptr->index);

    /* Invalidate the id_index field of the future id instance of id_object_t,
     * but save the value in the old_id_index field first.
     */
    assert(future_id_inst_ptr->index ==
           atomic_load(&(future_id_obj_ptr->id_index)));
    assert(-1 == atomic_load(&(future_id_obj_ptr->old_id_index)));
    atomic_store(&(future_id_obj_ptr->old_id_index),
                 atomic_load(&(future_id_obj_ptr->id_index)));
    ...
    mod_future_id_obj_k.future_id_discarded =
        future_id_obj_k.future_id_discarded;
    ...
    if (atomic_compare_exchange_strong(&(future_id_obj_ptr->k),
      ...
      *actual_object_id = atomic_load(&(future_id_obj_ptr->real_id));
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "realize_cb_0(): attempt to set ruture_id_realized failed.\n");
      }
    }
  }
 ...
 * discard_cb_0
 *
 *      Test framework implementation of the discard_cb used to test future IDs.
 *Note that this implementation of the discard call back is not multi-thread
 *safe, and depends upon calling function to ensure mutual exclusion on the
 *target future object during execution.
 *
 *      The discard callback in invoked under two circumstances.
 *
 *      The first case is the discard of the object associated with a future ID
 *after it has been replaced with the object associated the source real ID.  In
 *this case, it is necessary to set the discarded and future_id_discarded flags
 *on the target instance of id_object_t, and the realized flag in the associated
 *instance of 1d_instance_t.
 *
 *      The second case is the discard of the object associated with a future ID
 *when that ID is discarded before it can be realized.  Here, it is only
 *necessary to set the discarded flag in the target instance of id_object_t.
 *
 *      The two cases can be distinguished by the value of the
 *future_id_realized flag.
 *
 ...
 *
 *      Then test to see if the supplied future object is maked as being
 *realized (i.e. that the future_id_realized flag is set).
 *
 *      if it isn't, we are dealing with case 2 above.  set the discarded flag
 *via a call to via a call to atomic_compare_exchange_strong() on
 *&(future_id_obj_ptr->k), and return SUCCESS if this operation succeeds, and
 *FAIL otherwise.
 *
 *      If it is, we are dealing with case 1 above.  Proceeds as follows:
 *
 *      3) Verify that the supplied future object has not yet been discarded
 *(i.e. that the future_id_discarded flag has not been set).
 *
 *      4) Verify that the id_index field of the future object is valid.
 *
 *      5) Verify that the instance of id_instance_t referenced by the id_index
 *field is valid, is marked as a future ID, but not yet marked as being
 *realized.
 *
 *      If all of these conditions are met, attempt to set the discarded and
 *future_id_ discarded flags via a call to atomic_compare_exchange_strong() on
 *      &(future_id_obj_ptr->k), and attempt to set the realized flag on the
 *associated instance of id_instance_t to TRUE.
 *
 *      If any of these tests fail, or if any of the flag updates fail, return
 *FALSE.
 *
...
herr_t discard_cb_0(void *future_object) {
  ...
  if (success) {
    ...
    if ((id_obj_k.in_progress) || (!id_obj_k.allocated) ||
        (id_obj_k.discarded)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr, "discard_cb_0(): future id obj in progress, not "
                        "created, or discarded on entry.\n");
      }
    }
  }
  ...
  if (success) {
    ...
    if (!id_obj_k.future) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "discard_cb_0(): future id obj not marked as future.\n");
      }
    }
  }
  ...
  if ((success) && (!id_obj_k.future_id_realized)) {
    ...
    if (!atomic_compare_exchange_strong(&(future_id_obj_ptr->k), &id_obj_k,
                                        mod_id_obj_k)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr, "discard_cb_0(): case 2: attempt to set discarded flag "
                        "on future id objected failed.\n");
      }
    }
  }
  ...
  if ((success) && (id_obj_k.future_id_realized)) {
    ...
    if ((future_id_inst_index < 0) ||
        (future_id_inst_index >= NUM_ID_INSTANCES)) {
  ...
  if ((success) && (id_obj_k.future_id_realized)) {
    ...
    if ((NULL == future_id_inst_ptr) ||
        (ID_INSTANCE_T__TAG != future_id_inst_ptr->tag)) {
  ...
  if ((success) && (id_obj_k.future_id_realized)) {
    ...
    if ((id_inst_k.in_progress) || (!id_inst_k.created) ||
        (id_inst_k.discarded)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr, "discard_cb_0(): future id instance in progress, not "
      ...
      if (rpt_failures) {
        ...
        fprintf(
            stderr,
            "discard_cb_0(): future id instance not marked as future id.\n");
      }
      ...
      if (rpt_failures) {
        ...
        fprintf(
            stderr,
            "discard_cb_0(): future id instance already marked as realized.\n");
      }
    }
  }
  ...
  if ((success) && (id_obj_k.future_id_realized)) {
    ...
    if (!atomic_compare_exchange_strong(&(future_id_obj_ptr->k), &id_obj_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr, "discard_cb_0(): attempt to set flags on future id "
                        "objected failed..\n");
      }
    }
  }
  ...
  if ((success) && (id_obj_k.future_id_realized)) {
    ...
    if (!atomic_compare_exchange_strong(&(future_id_inst_ptr->k), &id_inst_k,
                                        mod_id_inst_k)) {
 ...
 * register_type()
 *
 *    Register a new type, associate it with the supplied instance of id_type_t,
 *and update that structure accordingly.
 *
 ...
 *
 *    If the rpt_failures flag is set, the function will write an error message
 *to stderr if an error is detected.  If such an error message is generated, the
  ...
  if (success) {
    ...
    if ((id_k.in_progress) || (id_k.created)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_type():%d: type at index %d registraion already done "
                "or in progress (1).\n",
      ...
      if (!atomic_compare_exchange_strong(&(id_type_ptr->k), &id_k, mod_id_k)) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "register_type():%d: type at index %d registraion already "
                  "done or in progress (2).\n",
                  tid, id_type_ptr->index);
        }
      }
    }
  ...
  if (success) {
    ...
    if (H5I_BADID == type_id) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_type():%d: call to H5Iregister_type() failed of type "
                "at index %d.\n",
                tid, id_type_ptr->index);
      }
    ...
    if (!atomic_compare_exchange_strong(&(id_type_ptr->k), &id_k, mod_id_k)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_type():%d: update of id_type_ptr->k failed.  index = "
                "%d, id = 0x%llx.\n",
 ...
 * try_register_type()
 *
 *    Attempt to register a new type, associate it with types_array[type_index]
 *and update that instance of id_type_t accordingly.
 *
 *    If successful, increment types_array[type_index].successful_registers.  On
 *failure, increment increment types_array[type_index].failed_registers
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
 ...
 *    If the rpt_failures flag is set, the function will write an error message
 *to stderr if an error is detected.  If such an error message is generated, the
 *triggering thread (given in the tid field) is reported. JRM -- 4/2/24
 *
 * Returns: void
 *
 ***********************************************************************************************/
...
void try_register_type(int type_index, hbool_t cs, hbool_t ds,
  ...
  if ((0 > type_index) || (type_index >= NUM_ID_TYPES)) {
    ...
    if (rpt_failures) {
      ...
      fprintf(stderr,
              "try_register_type():%d: type_index = %d out of reange.\n", tid,
              type_index);
    }
  }
  ...
  if (success) {
    ...
    if ((id_k.in_progress) || (id_k.created)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_type():%d: type at index %d registraion already "
                "done or in progress (1).\n",
                tid, type_index);
      }
      ...
      if (!atomic_compare_exchange_strong(&(types_array[type_index].k), &id_k,
                                          mod_id_k)) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_register_type():%d: type at index %d registraion "
                  "already done or in progress (2).\n",
                  tid, type_index);
        }
      }
    }
  ...
  if (success) {
    ...
    if (H5I_BADID == type_id) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_type():%d: call to H5Iregister_type() failed for "
                "type at index %d.\n",
                tid, type_index);
      }
    } else {
    ...
    if (!atomic_compare_exchange_strong(&(types_array[type_index].k), &id_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_type():%d: update of id_type_ptr->k failed.  "
                "index = %d, id = 0x%llx.\n",
 ...
 * clear_type()
 *
 *    If it is marked as existing, call H5Iclear_type() on the type ID
 *associated with the instance of id_type_t pointed to by id_type_ptr, with the
 *supplied force flag, and update *id_type_ptr accordingly.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
 ...
 *triggering thread (given in the tid field) is reported.
 *
 *    The function returns 0 on success, and 1 if any error (including
 *non-existance of the target type) is detected.
 *
 ***********************************************************************************************/
...
int clear_type(id_type_t *id_type_ptr, hbool_t force, hbool_t cs, hbool_t ds,
  ...
  if (success) {
    ...
    if ((id_type_k.in_progress) || (!id_type_k.created) ||
        (id_type_k.discarded)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "clear_id():%d: target id type either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    ...
    if (H5Iclear_type((H5I_type_t)(id_type_k.type_id), force) != SUCCEED) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "clear_type():%d: H5Iclear_type(0x%llx, %d) reports failure.\n",
 ...
 * try_clear_type()
 *
 *    Attempt to clear the type associated with it with types_array[type_index]
 *and update that instance of id_type_t accordingly.
 *
 *    Call H5Iclear_type() on types_array[type_index].k.type_id with the
 *supplied force flag, and update types_array[type_index] accordingly.
 *
 *    Note that types_array[type_index].k.type_id may not be defined, or the
 *target type may have been deteted when the call is made.  However, the call to
 *H5Iclear_type() is made regardless, and the results recorded.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
  ...
  if (success) {
    ...
    if (H5Iclear_type((H5I_type_t)(id_type_k.type_id), force) != SUCCEED) {
      ...
      if (rpt_failures) {
        ...
        fprintf(
            stderr,
            "try_clear_type():%d: H5Iclear_type(0x%llx, %d) reports failure.\n",
            tid, (unsigned long long)(id_type_k.type_id), (int)force);
      }
    }
  }
 ...
 * destroy_type()()
 *
 *    If it is marked as existing, call H5Idestroy_type() on the type ID
 *associated with the instance of id_type_t pointed to by id_type_ptr, and
  ...
  if (success) {
    ...
    if ((id_type_k.in_progress) || (!id_type_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "destroy_type():%d: target id type either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    ...
    if (destroy_succeeded) { /* set the discarded flag */
      ...
      while (!id_type_k.discarded) {
        ...
        atomic_compare_exchange_strong(&(id_type_ptr->k), &id_type_k,
                                       mod_id_type_k);
      ...
      if (retries > 0) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "dec_ref():%d: %d retries needed  to mark type as discarded "
                  "after H5Idestroy_type(0x%llx) reports success.\n",
 ...
 * try_destroy_type()
 *
 *    Attempt to destroy the type associated with it with
 *types_array[type_index] and update that instance of id_type_t accordingly.
 *
 *    Call H5Idestroy_type() on types_array[type_index].k.type_id, and update
 *    types_array[type_index] accordingly.
 *
 *    Examine the kernel of types_array[type_index] before and after to
 *determine the expected result, and flag an assert if the result is contrary to
 *the expected one. Note that the expected result may not be known -- in which
 *case the test is ambiguous.
 *
 *    Note that types_array[type_index].k.type_id may not be defined, or the
 *target type may have been deteted when the call is made.  However, the call to
 *H5Iclear_type() is made regardless, and the results recorded.
  ...
  if (success) {
    ...
    if (result != SUCCEED) {
      ...
      while (!id_type_k.discarded) {
        ...
        atomic_compare_exchange_strong(&(types_array[type_index].k), &id_type_k,
                                       mod_id_type_k);
      ...
      if (retries > 0) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_destroy_type():%d: %d retries needed to mark type "
                  "(0x%llx) as discarded.\n",
                  tid, retries, (unsigned long long)(id_type_k.type_id));
        }
      }
    ...
    if (((!pre_id_type_k.created) && (!post_id_type_k.created)) ||
      ...
      /* Type did not exists at the time that H5Idestroy_type() was called --
       * thus the call should have failed.
       */
      assert(!success);
      ...
      /* the type existed when H5Idestroy_type() case called -- thus the call
      ...
      /* the type may or may not have existed at the time that H5Idestroy_type()
       * was called -- thus the test result is ambiguous.
       */
      ambiguous_result++;
    }
  }
 ...
 * register_id()
 *
 *    Register a new id, associate it with the supplied instances of
 *id_instance_t and id_object_t, and update those structures accordingly.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
...
int register_id(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
  ...
  if ((id_type_k.in_progress) || (!id_type_k.created) ||
    ...
    if (rpt_failures) {
      ...
      fprintf(stderr,
              "register_id():%d: target id type either in progress, not "
              "created, or discarded on entry.\n",
              tid);
    }
  } else {
  ...
  if (success) {
    if ((id_inst_k.in_progress) || (id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_id():%d: target id inst in progress, not created, or "
                "discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    if ((id_obj_k.in_progress) || (id_obj_k.allocated) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_id():%d: target id obj in progress, not created, or "
                "discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    ...
    if (!atomic_compare_exchange_strong(&(id_inst_ptr->k), &id_inst_k,
                                        mod_id_inst_k)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_id():%d: can't mark target id inst in progress.\n",
                tid);
      }
    } else {
  ...
  if (success) {
    ...
    if (!atomic_compare_exchange_strong(&(id_obj_ptr->k), &id_obj_k,
                                        mod_id_obj_k)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_id():%d: can't mark target id obj in progress.\n",
                tid);
      }
      ...
      if (!atomic_compare_exchange_strong(&(id_inst_ptr->k), &id_inst_k,
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "register_id():%d: can't reset in progress on mark target id "
                  "inst.\n",
                  tid);
        }
      }
  ...
  if (success) {
    ...
    if (!atomic_compare_exchange_strong(&(id_obj_ptr->k), &id_obj_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_id():%d: Can't update id object for id registration "
                "success or failure.\n",
                tid);
      }
    }
    ...
    if (!atomic_compare_exchange_strong(&(id_inst_ptr->k), &id_inst_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_id():%d: Can't update id instande for id "
 ...
 * try_register_id()
 *
 *    Attempt to register a new id of type
 *id_instance_array[id_index].type_index and associated object
 *objects_array[obj_index], and update id_instance_array[id_index] and
...
void try_register_id(int id_index, int obj_index, hbool_t cs, hbool_t ds,
  ...
  if (success) {
    ...
    if ((type_index < 0) || (type_index >= NUM_ID_TYPES)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_id():%d: type_index = %d out of reange.\n", tid,
                type_index);
      }
      assert((0 <= type_index) && (type_index < NUM_ID_TYPES));
    }
  ...
  if (success) {
    if ((id_inst_k.in_progress) || (id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_id():%d: target id inst in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
  ...
  if (success) {
    if ((id_obj_k.in_progress) || (id_obj_k.allocated) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_id():%d: target id obj in progress, not created, "
                "or discarded on entry.\n",
                tid);
      }
  ...
  if (success) {
    ...
    if (!atomic_compare_exchange_strong(&(id_instance_array[id_index].k),
      ...
      if (rpt_failures) {
        ...
        fprintf(
            stderr,
            "try_register_id():%d: can't mark target id inst in progress.\n",
            tid);
      }
      ...
      id_inst_k =
          atomic_load(&(id_instance_array[id_index].k)); /* get fresh copy */
  ...
  if (success) {
    ...
    if (!atomic_compare_exchange_strong(&(objects_array[obj_index].k),
                                        &id_obj_k, mod_id_obj_k)) {

      if (rpt_failures) {

        fprintf(stderr,
                "try_register_id():%d: can't mark target id obj in progress.\n",
                tid);
      }

      /* in progress flag is set in id_instance_array[id_index].k.  Must reset
       * it */
      mod_id_inst_k.in_progress = FALSE;
      mod_id_inst_k.created = id_inst_k.created;
      mod_id_inst_k.discarded = id_inst_k.discarded;
      ...
      if (!atomic_compare_exchange_strong(&(id_instance_array[id_index].k),
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_register_id():%d: can't reset in progress on target id "
                  "inst.\n",
                  tid);
        }
      }
      ...
      id_obj_k =
          atomic_load(&(objects_array[obj_index].k)); /* get fresh copy */
  ...
  if (success) {
    ...
    atomic_store(&(objects_array[obj_index].id_index),
                 id_instance_array[id_index].index);
    atomic_store(&(id_instance_array[id_index].obj_index),
                 objects_array[obj_index].index);
    ...
    if (!atomic_compare_exchange_strong(&(objects_array[obj_index].k),
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_id():%d: Can't update id object for id "
                "registration success on failure.\n",
                tid);
      }
    ...
    if (!atomic_compare_exchange_strong(&(id_instance_array[id_index].k),
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "try_register_id():%d: Can't update id instance for id "
                "registration success or failure.\n",
                tid);
      }
  ...
  if (success) {
    ...
    atomic_fetch_add(&(id_instance_array[id_index].successful_registrations),
                     1ULL);
 ...
 * register_future_id()
 *
 *    Register a future ID.  Note that the creation of the associated real ID,
 *and the update of the object associated with the future ID to point to the
 *real ID are seprate operations.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
...
int register_future_id(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
  ...
  if ((id_type_k.in_progress) || (!id_type_k.created) ||
    ...
    if (rpt_failures) {
      ...
      fprintf(stderr,
              "register_future_id():%d: target id type either in progress, not "
              "created, or discarded on entry.\n",
              tid);
    }
  } else {
  ...
  if (success) {
    if ((id_inst_k.in_progress) || (id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_future_id():%d: target id inst in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    if ((id_obj_k.in_progress) || (id_obj_k.allocated) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_future_id():%d: target id obj in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    ...
    if (!atomic_compare_exchange_strong(&(id_inst_ptr->k), &id_inst_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(
            stderr,
            "register_future_id():%d: can't mark target id inst in progress.\n",
            tid);
      }
    } else {
  ...
  if (success) {
    ...
    if (!atomic_compare_exchange_strong(&(id_obj_ptr->k), &id_obj_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(
            stderr,
            "register_future_id():%d: can't mark target id obj in progress.\n",
            tid);
      }
      ...
      if (!atomic_compare_exchange_strong(&(id_inst_ptr->k), &id_inst_k,
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "register_future_id():%d: can't reset in progress on mark "
                  "target id inst.\n",
                  tid);
        }
      }
  ...
  if (success) {
    ...
    if (id != H5I_INVALID_HID) {
      ...
      if (rpt_failures) {
        ...
        fprintf(
            stderr,
            "register_future__id():%d: Call to H5Iregister_future() failed.\n",
            tid);
      }
    }
    ...
    if (!atomic_compare_exchange_strong(&(id_obj_ptr->k), &id_obj_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_future_id():%d: Can't update id object for id "
                "registration success or failure.\n",
                tid);
      }
    }
    ...
    if (!atomic_compare_exchange_strong(&(id_inst_ptr->k), &id_inst_k,
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "register_future_id():%d: Can't update id instande for id "
                "registration success or failure.\n",
                tid);
      }
    }
 ...
 *
 *    Link the supplied real id object with the supplied future id object.  Once
 *this is done, the next look up of the future ID will cause the future ID
 *object to be replaced with the real ID object.
 *
 *    Note that both the real and future IDS must be registered before this
 *function is callec.
 *
 *                                                              JRM -- 3/8/24
 *
...
int link_real_and_future_ids(id_object_t *future_id_obj_ptr,
  ...
  /* First mark the instance of id_instance_t associated with *real_id_obj_ptr
   * as being the source real ID for a future ID.
   *
   * Note that there isn't much of a problem if we succed in marking the real id
   * instance as being the source real ID for a future ID and then fail to set
   * up the future id instance.
   */
  while ((success) && (!done)) {
    ...
    if (success) {
      ...
      if ((real_id_obj_k.in_progress) || (!real_id_obj_k.allocated) ||
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr, "link_real_and_future_ids(): real id obj in "
                          "progress, not created, or discarded on entry(1).\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if ((real_id_inst_index < 0) ||
          (real_id_inst_index >= NUM_ID_INSTANCES)) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr, "link_real_and_future_ids(): real id instance index "
                          "out of range).\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if ((NULL == real_id_inst_ptr) ||
          (ID_INSTANCE_T__TAG != real_id_inst_ptr->tag)) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "link_real_and_future_ids(): Invalid real_id_inst_ptr.\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if (real_id_inst_k.future_id_src) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr, "link_real_and_future_ids(): future_id_src already "
                          "set in real ID inst.\n");
        }
      }
    }
  ...
  while ((success) && (!done)) {
    ...
    if (success) {
      ...
      if ((future_id_obj_k.in_progress) || (!future_id_obj_k.allocated) ||
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr, "link_real_and_future_ids(): future id obj in "
                          "progress, not created, or discarded on entry.\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if ((real_id_obj_k.in_progress) || (!real_id_obj_k.allocated) ||
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr, "link_real_and_future_ids(): real id obj in "
                          "progress, not created, or discarded on entry(2).\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if (!future_id_obj_k.future) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr, "link_real_and_future_ids(): future id obj not "
                          "marked as future.\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if (real_id_obj_k.future) {
        ...
        if (rpt_failures) {
          ...
          fprintf(
              stderr,
              "link_real_and_future_ids(): real id obj is marked as future.\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if (future_id_obj_k.future_id_discarded) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "link_real_and_future_ids(): future id already discarded.\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if (future_id_obj_k.real_id_defined) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "link_real_and_future_ids(): real id already defined.\n");
        }
      }
    }
    ...
    if (success) {
      ...
      if (future_id_obj_k.real_id_def_in_prog) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr, "link_real_and_future_ids(): link of real and future "
                          "ID alread in progress.\n");
        }
      }
    }
    ...
    if (success) {
      ...
      /* attempt to set the real_id_def_in_progress flag.  IF this fails,
       * re-start the loop and try again.  The objective here is to serialize
       * attempts at setting the the real ID -- thus forcing exactly one attempt
       * to succeed.
       */
      ...
      mod_future_id_obj_k.real_id_defined = future_id_obj_k.real_id_defined;
      mod_future_id_obj_k.future_id_realized =
          future_id_obj_k.future_id_realized;
      mod_future_id_obj_k.future_id_discarded =
          future_id_obj_k.future_id_discarded;
      mod_future_id_obj_k.id = future_id_obj_k.id;

      if (atomic_compare_exchange_strong(
              &(future_id_obj_ptr->k), &future_id_obj_k, mod_future_id_obj_k)) {
        done = TRUE;
  ...
  while ((success) && (!done)) {
    ...
    /* we have successfully set future_id_obj_ptr->k.real_id_def_in_prog to
     * TRUE.  Now set future_id_obj_ptr->real_id and
     * future_id_obj_ptr->real_id_obj_index, and then reset
     * future_id_obj_ptr->k.real_id_def_in_prog and set
     ...
     * atomic_compare_exchange_strong() will fail.  If so, we will just retry.
     * Note that the above setting of the real_id and the real_id_obj_index
     * fields is not an issue, since these fields are not read unless the
     * real_id_defined flag is true.
     */
 ...
 * object_verify()
 *
 *      Call H5Iobject_verify() and verify that it returns the expected pointer.
 *In general, this will be a pointer to and instance of id_object_t as supplied
 *by in the id_obj_ptr parameter.  However, if the call to H5Iobject_verify is
 *expected to fail, id_obj_ptr should be NULL.
 *
 *      If the cs flag is set, the function clears statistics on entry.
 *
 *      If the ds flag is set, the function displays statistics just before
 *exit.
 *
 *      If the rpt_failures flag is set, the function will write an error
 *message to stderr if an error is detected.  If such an error message is
 *generated, the triggering thread (given in the tid field) is reported.
 *
 *      The function returns 0 on success -- that is either:
 *
 *              H5Iobject_verify() returns a non-NULL value, and this value is
 *equal to id_obj_ptr, or
 *
...
int object_verify(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
  ...
  /* If the target ID is neither in progress, not created, discarded, a future
   * ID that hasn't been realized, nor a real ID that has been used to realize a
   * future ID, id_object_ptr must not be NULL.
   *
   * If so, id_obj_ptr->index must equal id_inst_ptr->obj_index, and
   * id_obj_ptr->id_index must equal id_inst_ptr->index.
   *
  ...
  if (!((id_inst_k.in_progress) || (!id_inst_k.created) ||
        ...
        ((id_inst_k.future_id_src) &&
         (-1 == atomic_load(&(id_inst_ptr->obj_index)))))) {
    ...
    if (NULL == id_obj_ptr) {
      ...
      fprintf(
          stderr,
          "object_verify():%d: NULL expected object ptr for non future id.\n",
          tid);
      ...
      fprintf(stderr,
              "object_verify():%d:i id_inst_ptr->index = %d != %d = "
              "id_obj_ptr->id_index.\n",
      ...
      fprintf(stderr,
              "object_verify():%d:i id_obj_ptr->index = %d != %d = "
              "id_inst_ptr->obj_index.\n",
  ...
  if (success) {
    ...
    if ((id_type_k.in_progress) || (!id_type_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "object_verify():%d: id_obj_ptr->index = %d != %d = "
                "id_obj_ptr->id_index.\n",
                tid, id_obj_ptr->index, atomic_load(&(id_inst_ptr->obj_index)));
      }
    } else {
  ...
  /* If id_obj_ptr is not NULL, verify that the id instance has been created and
   * not discarded. Since a NULL id_obj_ptr indicates that the lookup is
   * expected to FAIL, skip this test if id_obj_ptr is NULL.
   */
  if (success) {
    if ((NULL != id_obj_ptr) &&
        ((id_inst_k.in_progress) || (!id_inst_k.created) ||
         (id_inst_k.discarded))) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "object_verify():%d: target id inst either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    } else {
  ...
  if ((success) && (id_obj_ptr)) {
    if ((id_obj_k.in_progress) || (!id_obj_k.allocated) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "object_verify():%d: target id obj either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    ...
    if ((void *)id_obj_ptr != H5Iobject_verify(id, type)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "object_verify():%d: H5Iobject_verify(id = 0x%llx, type = "
                "0x%llx).\n",
                tid, (unsigned long long)id, (unsigned long long)type);
      }
    } else if (id_obj_ptr) {

      /* the call to H5Iobject_verify() has returned id_obj_ptr, and id_obj_ptr
       * is not NULL. verify that id_obj_ptr->index equals
       * id_inst_ptr->obj_index, and id_obj_ptr->id_index equals
       * id_inst_ptr->index.
       */
      ...
      if (id_inst_ptr->index != atomic_load(&(id_obj_ptr->id_index))) {
        ...
        fprintf(stderr,
                "object_verify():%d:f id_inst_ptr->index = %d != %d = "
                "id_obj_ptr->id_index.\n",
                tid, id_inst_ptr->index, atomic_load(&(id_obj_ptr->id_index)));
        ...
        fprintf(stderr,
                "object_verify():%d:f id_obj_ptr->index = %d != %d = "
                "id_inst_ptr->obj_index.\n",
                tid, id_obj_ptr->index, atomic_load(&(id_inst_ptr->obj_index)));
      }
    }
 ...
 * try_object_verify()
 *
 *      Call H5Iobject_verify() on id_instance_array[id_index].k.id, and verify
 *that the pointer returned equals &(objects_array[obj_index]).  Update stats in
 *id_instance_array[id_index] and objects_array[obj_index] as appropriate to
 *refect success or failure.
 *
 *      Note that id_instance_array[id_index].k.id may or may not have been
 ...
 *      If the rpt_failures flag is set, the function will write an error
 *message to stderr if an error is detected.  If such an error message is
 *generated, the triggering thread (given in the tid field) is reported. JRM --
 *4/4/24
 *
 * returns: void
 *
...
int try_object_verify(int id_index, hbool_t cs, hbool_t ds,
  ...
  pre_remove_verify_starts =
      atomic_load(&(id_instance_array[id_index].remove_verify_starts));
  ...
  if (success) {
    ...
    reported_id_obj_ptr =
        (id_object_t *)H5Iobject_verify(pre_id_inst_k.id, id_type_k.type_id);
    ...
    assert((-1 == obj_index) ||
           ((0 <= obj_index) && (obj_index < NUM_ID_OBJECTS)));
    assert((-1 == obj_index) ||
           (ID_OBJECT_T__TAG == objects_array[obj_index].tag));
    ...
    if ((!pre_id_inst_k.created) && (!post_id_inst_k.created)) {
      ...
      /* However, it is possible that the id is in the process of being
       * registerd. If so, the object index is set before the created flag is
       * set.  Thus it is possible that object verify will fail, but
       * id_instance_array[id_index].obj_index will be set.
       *
       * At present, the only possible values for
       * id_instance_array[id_index].obj_index are -1 and id_index.
       */
      ...
      if (success) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_object_verify():%d: pre / post discarded = %d / %d\n",
                  tid, (int)pre_id_inst_k.discarded,
                  (int)post_id_inst_k.discarded);
        }
      ...
      if (((pre_remove_verify_starts > 0) || (post_remove_verify_starts > 0)) &&
          (!success)) {

        /* A call to H5Iremove verify() may have been active about the time the
         * call to H5Iobject_verify() was made.  Since this would explain the
         * failure, log an ambiguous test.
         */
        ambiguous_results++;
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_object_verify(%d):%d failure possibly explained by a "
                  "remove verify in progress.\n",
                  id_index, tid);
        }
      } else {
      ...
      /* the id may or may not have existed when H5Iverify_object() was called
       * -- thus the outcome of the test is ambiguous.
       */
      ambiguous_results++;
    }
  }
 ...
 * get_type()
 *
 *    Verify that the target ID associated with the instance of id_instance_t
 *pointed to by id_inst_ptr exists, and has the type associaed with the instance
 *of id_type_t pointed to by id_type_ptr.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
 ...
 *triggering thread (given in the tid field) is reported.
 *
 *    The function returns 0 on success (i.e. if the target ID exists and is of
 *the indicated type), and 1 if not, or if any error (including non-existance)
 *is detected.
 *
 ***********************************************************************************************/
  ...
  if (success) {
    ...
    if ((id_type_k.in_progress) || (!id_type_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "get_type():%d: target id type either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    } else {
  ...
  if (success) {
    if ((id_inst_k.in_progress) || (!id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "get_type():%d: target id inst either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    } else {
  ...
  if (success) {
    ...
    if (type != H5Iget_type(id)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "get_type():%d: type == 0x%llx != H5Iget_type(id = 0x%llx).\n",
 ...
 *
 *    Before doing so, verify that the supplied instances of id_instance_t and
 *id_object_t indicate that the target ID exists.  Similarly, verify that
 *supplied instance of id_type_t exists.  In passing, also look up the target
 *type and ID needed to call H5Iremove_verify().
 *
 *    Assuming that H5Iremove_verify() is successful, update the instance of
 *id_object_t associated with the ID to mark it as removed.  This is necessary
 *as H5Iremove_verify() doesn't call the free function associaed with the type
 *(and ID).
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 ...
 *triggering thread (given in the tid field) is reported.
 *
 *    The function returns 0 on success (i.e. if the target ID exists and is
 *successfully removed, and 1 if not, or if any error (including non-existance)
 *is detected.
 *
 ***********************************************************************************************/
...
int remove_verify(id_type_t *id_type_ptr, id_instance_t *id_inst_ptr,
  ...
  if (success) {
    ...
    if ((id_type_k.in_progress) || (!id_type_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "remove_verify():%d: target id type either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    } else {
  ...
  if (success) {
    if ((id_inst_k.in_progress) || (!id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "remove_verify():%d: target id inst either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    } else {
  ...
  if (success) {
    if ((id_obj_k.in_progress) || (!id_obj_k.allocated) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "remove_verify():%d: target id obj either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    ...
    if ((void *)id_obj_ptr != H5Iremove_verify(id, type)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "remove_verify():%d: H5Iremove_verify(id = 0x%llx, type = "
                "0x%llx) failed.\n",
                tid, (unsigned long long)id, (unsigned long long)type);
      }
    } else {
      ...
      if (id_obj_k.id != id) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "remove_verify():%d: ID missmatch -- id = 0x%llx != 0x%llx = "
                  "id_obj_k.id.\n",
                  tid, (unsigned long long)id,
                  (unsigned long long)(id_obj_k.id));
        }
      }
    }
  }
  ...
  if (success) {
    ...
    if ((id_inst_k.discarded) || (id_obj_k.discarded)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "remove_verify():%d: ((id_inst_k.discarded) || "
                "(id_obj_k.discarded))\n",
                tid);
      }
    }
  ...
  if (success) { /* must mark *id_inst_ptr as discarded */
    ...
    while (!id_inst_k.discarded) {
      ...
      atomic_compare_exchange_strong(&(id_inst_ptr->k), &id_inst_k,
                                     mod_id_inst_k);
    ...
    if ((inst_retry_cnt > 0) || (obj_retry_cnt > 0)) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "remove_verify():%d: inst_retry_cnt = %d, obj_retry_cnt = %d\n",
 ...
 * try_remove_verify()
 *
 *      Call H5Iremove_verify() on id_instance_array[id_index].k.id, and test to
 *see if the pointer returned equals &(objects_array[obj_index]).  Update stats
 *in id_instance_array[id_index] and objects_array[obj_index] as appropriate to
 *refect success or failure.
 *
 *      Note that id_instance_array[id_index].k.id may or may not have been
 *initialized on entry.
 ...
 *      id_instance_array[id_index] to see whether success or failure is
 *expected, and assert if an unexpected is returned by H5Iremove_verify().  In
 *ambiguous cases, increment the ambiguous results counter, and generate an
 *error message if rpt_failures is TRUE.
 *
 *      If the cs flag is set, the function clears statistics on entry.
 *
 *      If the ds flag is set, the function displays statistics just before
...
int try_remove_verify(int id_index, hbool_t cs, hbool_t ds,
  ...
  if (success) {
    ...
    pre_remove_verify_starts = atomic_fetch_add(
        &(id_instance_array[id_index].remove_verify_starts), 1ULL);
    ...
    if (NULL == reported_id_obj_ptr) {
      ...
      post_remove_verify_starts = atomic_fetch_sub(
          &(id_instance_array[id_index].remove_verify_starts), 1ULL);
    ...
    if ((!pre_id_inst_k.created) && (!post_id_inst_k.created)) {
       ...
       *
       * While the object index will usually be -1, the registration of an ID in
       * the test bed is not an atomic process.  In particular, since the
       * obj_index is set before the id is marked as created, it is possible
       * that it will be set even though the id instance is not listed as being
       * created.
       *
       * Deal wiht this case by allowing the obj_index to be either -1 or equal
       * to to the id_index.  While this is good for now, note that it may
       * change.
       */
      assert(!success);
      assert((-1 == obj_index) || (id_index == obj_index));
       ...
       * -- thus the call must have failed.  However, the object index must be
       * defined (i.e. not -1).  Further the indicated object must be marked as
       * discarded, and must be linked to the id via its id_index field.
       */
      assert(!success);
      assert((0 <= obj_index) && (obj_index < NUM_ID_OBJECTS));
      ...
      if (((0 < pre_remove_verify_starts) || (0 < post_remove_verify_starts)) &&
          (!success)) {

        /* another call to H5Iremove_verify() has been active sometime around
         * the same time as this call.  Thus the failure of this call to
         * H5Iremove_verify() is probably OK.  Since we don't know this sure,
         * log this as an ambiguous result.
         */
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_remove_verify(%d):%d: failure possibly caused by a "
                  "concurrent revmoce verify.\n",
                  id_index, tid);
        }
      } else {

        /* the ID was created and not discarded at the time that
         * H5Iremove_verify() was called -- thus the call must have succeeded.
         *
        ...
        if (SUCCEED != free_func((void *)(&(objects_array[obj_index])), NULL)) {
          ...
          if (rpt_failures) {
            ...
            fprintf(stderr,
                    "try_remove_verify():%d: free_func() failed -- id/obj "
                    "indexes = %d / %d.\n",
       ...
       * afterwards.  Since H5Iremove_verify() does not call the free_func(),
       * this implies that some other call deleted the target ID from the index,
       * and thus that the call to H5Iremove_verify() failed.
       *
  ...
  if (success) {
    ...
    atomic_fetch_add(&(id_instance_array[id_index].successful_remove_verifies),
                     1ULL);

  } else {

    atomic_fetch_add(&(id_instance_array[id_index].failed_remove_verifies),
                     1ULL);
  }
 ...
 * dec_ref()
 *
 *    If the target id instance is listed as existing in the supplied instance
 *of id_instance_t, call H5Idec_ref() to decrement the current ref count for the
 *id.  If the reference count is decremented to zero, mark the supplied id
 *instance as discarded, and verify that the associated id_object_t is marked as
 *discarded.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 ...
 *triggering thread (given in the tid field) is reported.
 *
 *    The function returns 0 on success (i.e. if the target ID exists and
 *H5Idec_ref() returned a non-negative integer, and 1 otherwise, or if any error
  ...
  if (success) {
    ...
    if ((id_type_k.in_progress) || (!id_type_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "dec_ref():%d: target id type either in progress, not created, "
                "or discarded on entry.\n",
                tid);
      }
    }
  ...
  if (success) {
    ...
    if ((id_inst_k.in_progress) || (!id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "dec_ref():%d: target id inst either in progress, not created, "
                "or discarded on entry.\n",
                tid);
      }
    } else {
  ...
  if (success) {
    if ((id_obj_k.in_progress) || (!id_obj_k.allocated) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "dec_ref():%d: target id obj either in progress, not created, "
                "or discarded on entry.\n",
                tid);
      }
    }
    ...
    if (ref_count < 0) {
      ...
      if (!id_obj_k.discarded) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "dec_ref():%d: H5Idec_ref(0x%llx) returns 0, but id object "
                  "not marked as discared.\n",
                  tid, (unsigned long long)id);
        }
      }
      ...
      if (retries > 0) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "dec_ref():%d: %d retries needed  to mark instance as "
                  "discarded after H5Idec_ref(0x%llx) returns 0.\n",
 ...
 * try_dec_ref()
 *
 *      Call H5Ided_ref() on id_instance_array[id_index].k.id, and update stats
 *in id_instance_array[id_index] to reflect success or failure.
 *
 *      Record the relevant elements of the state of id_instance_array[id_index]
 *both before and after the call to H5Idec_ref().  Test to see if this data is
 *consistent with the result of the call to H5Idec_ref(), and trigger an assert
 *if it is not.
 *
 *      Note that it is not always possible to determine the expected result
 *from this data -- if so, test is ambiguous.
 *
 *      Note that id_instance_array[id_index].k.id may or may not have been
 *registered on entry.
 *
...
int try_dec_ref(int id_index, hbool_t cs, hbool_t ds, hbool_t rpt_failures,
  ...
  if (success) {
    ...
    if (ref_count < 0) {
      ...
      /* 0 == ref_count implies that the call to H5Idec_ref() deleted the id
       * from the the index.
       */
    ...
    if (((!pre_id_inst_k.created) && (!post_id_inst_k.created)) ||
      ...
      /* the id was not registered during at the time H5Idec_ref() was called --
       * thus the call cannot have succeeded.
       */
      assert(!success);
      assert(-1 == ref_count);
      ...
      if (((0 < pre_remove_verify_starts) || (0 < post_remove_verify_starts)) &&
        ...
        /* A call to H5Iremove verify() may have been active about the time the
         * call to H5Idec_ref() was made.  Since this could explain the failure,
         * log an ambiguous test.
         */
        ambiguous_results++;
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_dec_ref(%d):%d failure possibly explained by a remove "
                  "verify in progress.\n",
                  id_index, tid);
        }
      } else {
      ...
      /* The id may or may not have been registered when H5Idec_ref() was
 ...
 *
 *    If the target id instance is listed as existing in the supplied instance
 *of id_instance_t, call H5Iinc_ref() to increment the current ref count for the
 *id.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 ...
 *
 *    The function returns 0 on success (i.e. if the target ID exists and
 *H5Iinc_ref() returned a positive integer, and 1 otherwise, or if any error
 *(including non-existance) is detected.
 *
 ***********************************************************************************************/
  ...
  if (success) {
    ...
    if ((id_inst_k.in_progress) || (!id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "inc_ref():%d: target id inst either in progress, not created, "
                "or discarded on entry.\n",
                tid);
      }
    } else {
  ...
  if (success) {
    ...
    if (ref_count <= 0) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "inc_ref():%d: H5Iinc_ref(0x%llx) returned %d -- positive "
                "value expected.\n",
 ...
 * try_inc_ref()
 *
 *      Call H5Iinc_ref() on id_instance_array[id_index].k.id, and update stats
 *in id_instance_array[id_index] to reflect success or failure.
 *
 *      If the target id is defined both before and after the call to
 *H5Iinc_ref(), the function asserts that the call to H5Iinc_ref() is
 *successful.
 *
 *      If the target id is undefined both before and after the call to
 *H5Iinc_ref(), the function asserts that the call to H5Iinc_ref() is
 *unsuccessful.
 *
 *      If the target id is undefined on one side of the call to H5Iinc_ref(),
 *and defined on the other side, the expected result of the call to H5Iinc_ref()
 *is unknown, and thus the result of the test is ambiguous.
 *
 *      Note that id_instance_array[id_index].k.id may or may not have been
 *registered on entry.
 *
    ...
    if ((pre_id_inst_k.created) && (post_id_inst_k.created) &&
      ...
      if (((0 < pre_remove_verify_starts) || (0 < post_remove_verify_starts)) &&
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "try_inc_ref(%d):%d: failure possibly caused by a concurrent "
                  "revoce verify.\n",
                  id_index, tid);
        }
      } else {
 ...
 *
 *    If the target id instance is listed as existing in the supplied instance
 *of id_instance_t, call H5Iget_ref() to obtain the current ref count for the
 *id, and return it.
 *
  ...
  if (success) {
    ...
    if ((id_inst_k.in_progress) || (!id_inst_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "get_ref():%d: target id inst either in progress, not created, "
                "or discarded on entry.\n",
                tid);
      }
    } else {
 ...
 * try_get_ref()
 *
 *      Call H5Iget_ref() on id_instance_array[id_index].k.id, and update stats
 *in id_instance_array[id_index] to reflect success or failure.
 *
 *      If the target id is defined both before and after the call to
 *H5Iget_ref(), the function asserts that the call to H5Iget_ref() is successful
 *and returns a non-negative value (since we are using an external API call, it
 *is possible that the application ref coutn is zero, while the internal ref
 *count is positive).
 *
 *      If the target id is undefined both before and after the call to
 *H5Iget_ref(), the function asserts that the call to H5Iget_ref() returns -1,
 *which indicates that that the target id doesn't exist.
 *
 *      If the target id is undefined on one side of the call to H5Iinc_ref(),
 *and defined on the other side, the expected result of the call to H5Iinc_ref()
 *is unknown, and thus the result of the test is ambiguous.
...
int try_get_ref(int id_index, hbool_t cs, hbool_t ds, hbool_t rpt_failures,
  ...
  if (success) {
    ...
    atomic_fetch_add(&(id_instance_array[id_index].successful_get_ref_cnts),
                     1ULL);
 ...
 * nmembers()
 *
 *    If the target type id is listed as existing in the supplied instance of
 *id_type_t, call H5Inmembers() to obtain the current number of IDs of the
 *supplied type, and then return this value.
 *
 *    If the target id instance is not listed as existing, H5Inmembers() failes,
 *or any other error is detected, return -1.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
  ...
  if (success) {
    ...
    if ((id_type_k.in_progress) || (!id_type_k.created) ||
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "nmembers():%d: target id type either in progress, not "
                "created, or discarded on entry.\n",
                tid);
      }
    } else {
 ...
 * type_exists()
 *
 *    If the target type id is listed as having been created in the supplied
 *instance of id_type_t, call H5Itype_exists(), and return its return value.
 *Otherwise, return FAIL.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 ...
 *
 *    If the target type id is listed as having been created in the supplied
 *instance of id_type_t, call H5Iinc_type_ref().  If this call returns success,
 *return 0.  If it fails, or if any other error is detected (including target
 *type id not created) return 1.
 *
 *    If the cs flag is set, the function clears statistics on entry.
 *
 *    If the ds flag is set, the function displays statistics just before exit.
  ...
  if (success) {
    ...
    if (ref_count == -1) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "inc_type_ref():%d: H5Iinc_type_ref(0x%llx) reports failure.\n",
                tid, (unsigned long long)type);
      }
    }
  }
 ...
 *
 *    If the target type id is listed as having been created in the supplied
 *instance of id_type_t, call H5Idec_type_ref().  If this call returns success,
 *return 0.  If it fails, or if any other error is detected (including target
 *type id not created) return 1.
 *
 *    If the cs flag is set, the function clears statistics on entry.
  ...
  if (success) {
    ...
    if (ref_count == -1) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "dec_type_ref():%d: H5Idec_type_ref(0x%llx) reports failure.\n",
                tid, (unsigned long long)type);
      }
    }
  }
 ...
 * create_types()
 *
 *    Register types_count new types, associate them with entries in
 *types_array[] starting with types_start, and every types_stride entries
 *thereafter until types_count types have been created.  For each effected
 *instance of id_type_t in types_array[], update that structure accordingly.
 *
 ...
 *
 *    If the rpt_failures flag is set, the function will write an error message
 *to stderr if one or more errors are detected.  If such an error message is
 *generated, the triggering thread (given in the tid field) is reported.
 *
 *    The function returns 0 on success, and the number of errors encountered
 *otherwise.
 *
 ***********************************************************************************************/
...
int create_types(int types_start, int types_count, int types_stride, hbool_t cs,
  ...
  for (i = types_start; i < types_start + (types_count * types_stride);
       i += types_stride) {
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "create_types():%d: %d errors reported (types start/count/stride = "
            "%d/%d/%d)\n",
 ...
 * dec_type_refs()
 *
 *    Decrement the reference counts on types_count types, starting with
 *types_start in types_array[], and then every types_stride entries thereafter
 *until the reference counts of types_count types have been decremented.  Do
 *this via calls to dec_type_ref(). Return the number of errors encountered.
 *
...
int dec_type_refs(int types_start, int types_count, int types_stride,
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "dec_type_refs():%d: %d errors reported (types start/count/stride "
            "= %d/%d/%d)\n",
            tid, err_cnt, types_start, types_count, types_stride);
  }
 ...
 * inc_type_refs()
 *
 *    Increment the reference counts on types_count types, starting with
 *types_start in types_array[], and then every types_stride entry thereafter
 *until the reference counts of types_count types have been incremented.  Do
 *this via calls to inc_type_ref(). Return the number of errors encountered.
 *
...
int inc_type_refs(int types_start, int types_count, int types_stride,
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "inc_type_refs():%d: %d errors reported (types start/count/stride "
            "= %d/%d/%d)\n",
            tid, err_cnt, types_start, types_count, types_stride);
  }
...
int destroy_types(int types_start, int types_count, int types_stride,
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "destroy_types():%d: %d errors reported (types start/count/stride "
            "= %d/%d/%d)\n",
            tid, err_cnt, types_start, types_count, types_stride);
  }
 ...
 * register_ids()
 *
 *    Register ids_count new ids, associate them with entries in
 *id_instance_array[] starting with ids_start, and every ids_stride entries
 *thereafter until ids_count ids have been created.  For each effected instance
 *of id_instance_t in id_inst_array[] and effected instance of id_object_t in
 *id_objs_array[], update those structures accordingly.
 *
...
int register_ids(int types_start, int types_count, int types_stride,
  ...
  for (j = ids_start, k = 0; j < ids_start + (ids_count * ids_stride);
    ...
    err_cnt += register_id(&(types_array[i]), &(id_instance_array[j]),
                           &(objects_array[j]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);
  }
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "register_ids():%d: %d errors reported (types st/cnt/str = "
            "%d/%d/%d, ids st/cnt/str = %d/%d/%d)\n",
            tid, err_cnt, types_start, types_count, types_stride, ids_start,
            ids_count, ids_stride);
  }
 ...
 * dec_refs()
 *
 *    Decrement the ref counts of ids_count existing IDs.  If the ref count
 *drops to zero, mark the associated instances of id_object_t as deleted, and
 *verify that the associated instances of id_object_t are marked as deleted as
 *well.
 *
 *    Decrement the ref counts of IDs associated with entries in
 *id_instance_array[] starting with ids_start, and every ids_stride entries
 *thereafter until ids_count ids have been created.
 *
 *    These id creations are implemented via calls to register_type().
 *
 *    The cs and ds flags are simply passed to register_id().
...
int dec_refs(int types_start, int types_count, int types_stride, int ids_start,
  ...
  for (j = ids_start, k = 0; j < ids_start + (ids_count * ids_stride);
    ...
    err_cnt += dec_ref(&(types_array[i]), &(id_instance_array[j]),
                       &(objects_array[j]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);
  }
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "dec_refs():%d: %d errors reported (types st/cnt/str = %d/%d/%d, "
            "ids st/cnt/str = %d/%d/%d)\n",
            tid, err_cnt, types_start, types_count, types_stride, ids_start,
            ids_count, ids_stride);
  }
 ...
 *
 *    Increment the ref counts of IDs associated with entries in
 *id_instance_array[] starting with ids_start, and every ids_stride entries
 *thereafter until ids_count ids have been created.
 *
 *    These id creations are implemented via calls to register_type().
...
int inc_refs(int ids_start, int ids_count, int ids_stride, hbool_t cs,
  ...
  for (j = ids_start; j < ids_start + (ids_count * ids_stride);
       j += ids_stride) {
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "inc_refs():%d: %d errors reported (ids st/cnt/str = %d/%d/%d)\n",
 ...
 *    id_instance_array[] and id_objs_array[] (starting with ids_start, and
 *every ids_stride entries thereafter until ids_count ids have been verified)
 ...
 *generated, the triggering thread (given in the tid field) is reported.
 *
 *    The function returns 0 on success, and the number of errors / failed
 *verifications otherwise.
 *
 *******************************************************************************************/
...
int verify_objects(int types_start, int types_count, int types_stride,
  ...
  for (j = ids_start, k = 0; j < ids_start + (ids_count * ids_stride);
    ...
    err_cnt += object_verify(&(types_array[i]), &(id_instance_array[j]),
                             &(objects_array[j]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);
  }
  ...
  if ((err_cnt > 0) && (rpt_failures)) {
    ...
    fprintf(stderr,
            "verify_objects():%d: %d errors reported (types st/cnt/str = "
            "%d/%d/%d, ids st/cnt/str = %d/%d/%d)\n",
            tid, err_cnt, types_start, types_count, types_stride, ids_start,
            ids_count, ids_stride);
  }
 ...
 *
 *    j) decrement the ref count on one of the IDs, and verify that its ref
count is
 *       is now 1.
 *
 ...
 *
 *    m) increment the ref counts of the new IDs from l) above and verify that
their
 ...
 *
 *    q) Increment the type ref count, decrement it, and then verify that the
type
 *       still exits.
 *
 *    r) If i is even, decrement the ref count the ref count of the type.  If i
is odd
 *       destroy the type.  In either case, verifty that the type no longer
exists.
 *
...
void serial_test_1(void) {
  ...
  for (i = 0; i < NUM_ID_TYPES; i++) {
    ...
    err_cnt += register_id(&(types_array[i]), &(id_instance_array[i]),
                           &(objects_array[i]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);

    err_cnt += object_verify(&(types_array[i]), &(id_instance_array[i]),
                             &(objects_array[i]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);

    err_cnt += get_type(&(types_array[i]), &(id_instance_array[i]), cs, ds,
                        rpt_failures, tid);
    assert(0 == err_cnt);

    err_cnt += remove_verify(&(types_array[i]), &(id_instance_array[i]),
                             &(objects_array[i]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);
    ...
    err_cnt += register_id(&(types_array[i]), &(id_instance_array[k]),
                           &(objects_array[k]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);
    ...
    if (1 !=
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "serial_test_1():%d: nmembers returns %d, 1 expected.\n", tid,
                num_mem);
      }
    }

    err_cnt += register_id(&(types_array[i]), &(id_instance_array[l]),
                           &(objects_array[l]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);

    err_cnt += register_id(&(types_array[i]), &(id_instance_array[m]),
                           &(objects_array[m]), cs, ds, rpt_failures, tid);
    assert(0 == err_cnt);
    ...
    if (3 !=
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "serial_test_1():%d: nmembers returns %d, 3 expected.\n", tid,
                num_mem);
      }
    }
    ...
    case FALSE:
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "serial_test_1():%d: type_exists(&(types_array[%d]), ...) "
                "returned FALSE, TRUE expected.\n",
                tid, i);
      }
      break;
    case FAIL:
   ...
   * This assertion can ignore the initial free list length, since whatever it
   * is on entry, the number of ids created is such that it will be empty by the
  ...
  /* Verify that the increments to the id info and type info reallocatable
   * counts balance. Note that it is possible that this test will fail if
   * increments to the free list entries reallocable fields collide just so.
   * This seems very improbable, but if it happens with any regularity, the test
   * will have to be modified to account for this.
   */
  assert(
      0 ==
 ...
 *
 *      1) Register types as directed by the types_start and types_count
 *parameters
 *
 ...
 *
 *      4) Verify that the types (or indexes) have the expected number of
 *members.
 *
 *      5) Increment the ref count on every other ID, decrementing the ref count
...
void serial_test_2(int types_start, int types_count, int ids_start,
  ...
  for (i = types_start; i < types_start + types_count; i++) {
    ...
    num_mem = nmembers(&(types_array[i]), cs, ds, rpt_failures, tid);
    expected =
        (ids_count / types_count) + (((ids_count % types_count) > i) ? 1 : 0);
    ...
    if (num_mem != expected) {
      ...
      if (rpt_failures) {
        ...
        fprintf(stderr,
                "nmembers(type_array[%d], ...) returns %d, %d expected,\n", i,
                num_mem, expected);
      }
    }
  }
  ...
  if (SERIAL_TEST_2__DISPLAY_FINAL_STATS) {
    ...
    fprintf(stderr,
            "init_id_info_fl_len = %lld, init_type_info_fl_len = %lld\n",
            (unsigned long long)init_id_info_fl_len,
            (unsigned long long)init_type_info_fl_len);
    fprintf(stderr,
            "init_num_id_info_fl_entries_reallocable = %lld, "
            "init_num_type_info_fl_entries_reallocable = %lld\n",
   ...
   * counts balance.
   *
   * As above, we must include the initial free list lengths since the
   * allocations and deallocations balence in this test.
   *
   * Note that it is possible that this test will fail if increments to the free
   * list entries reallocable fields collide just so.  This seems very
   * improbable, but if it happens with any regularity, the test will have to be
 ...
 *
 *       1) Create 64 future IDs and 64 real IDs that will evenutally be
 *associated with the future IDs, and allow them to be realized.  Do not set up
 *the associations just yet.
 *
 *
 *      Verify that verification of future IDs works in the base case:
 *
 *       2) Lookup all the future and real IDs.  Verify that the real ID lookups
 *succeed, and that the future ID lookups all fail.
 *
 *       3) For every eighth future ID (i = 0, 16, 32, 48, ... ), link the
 *associated real ID (j = 1, 17, 33, 47, ...) to the future ID.
 *
 *       4) Verify that lookups of all the real IDs still succeed.
 *
 *       5) Lookup all the future and real IDs.  Verify that every eighth future
 *ID lookup (i = 0, 16, 32, 48, ... ) succeeds, while the rest fail, and that
 *every eighth real ID lookup (j = 1, 17, 33, 47, ...) fails while the rest
 *succeed.
 *
 *
 *      Link some real IDs to future IDs, and then delete the real IDs before
 *the linked future IDs are realized.  This appears to expose a fundamental
 *problem with the current future ID mechanism.  At present, this is papered
 *over via the user provided realize and discard callbacks -- however, in the
 *absence of this fix, data structure corruption is possible.  NEED A FIX FOR
 *THIS.
 *
 *       6) For every eighth + 1 future ID (i = 2, 18, 34, 50, ... ), link the
 *associated real ID (j = 3, 19, 35, 51, ...) to the future ID.
 *
 ...
 *
 *          Verify that every eighth future ID lookup (i = 0, 16, 32, 48, ... )
 *succeeds, while the rest fail
 *
 *          Verify that every eighth (j = 1, 17, 33, 47, ... ) and every eighth
 *+ 1 (j = 3, 19, 35, 51, ...) real ID fails, while the rest succeed.
 *
 *
 *      Verify that source real IDs are deleted properly even if their reference
 *counts are are greater than 1.
 *
 *      11) For every eighth + 1 future ID (i = 2, 18, 34, 50, ... ), link the
 *associated real ID (j = 3, 19, 35, 51, ...) to the future ID.
 *
 ...
 *
 *          Verify that every eighth and every eight +2 future ID lookup (i = 0,
 *16, 32, 48, ... ) succeeds, while the rest fail
 *
 *          Verify that every eighth (j = 1, 17, 33, 47, ... ), every eighth + 1
 *          (j = 3, 19, 35, 51, ...) and every eight + 2 (j = 5, 21, 37, 53,
 *...) real ID fails, while the rest succeed.
 *
 *
 *
 *******************************************************************************************/
...
void serial_test_4(void) {
  ...
  /* create the future IDs and their associated real IDs.  Note that they are
   * not associated yet, so the future IDs can't be realized on search.
   */
  for (i = 0; i < 128; i += 2) {
    j = i + 1;

    err_cnt += register_future_id(&(types_array[0]), &(id_instance_array[i]),
                                  &(objects_array[i]), realize_cb_0,
                                  discard_cb_0, cs, ds, rpt_failures, tid);

    err_cnt += register_id(&(types_array[0]), &(id_instance_array[j]),
                           &(objects_array[j]), cs, ds, rpt_failures, tid);
  }
  ...
  /* verify that lookups of the future IDs fail and lookups of the real IDs
   * succeed */
  for (i = 0; i < 128; i += 2) {
    ...
    err_cnt += object_verify(&(types_array[0]), &(id_instance_array[j]),
                             &(objects_array[j]), cs, ds, rpt_failures, tid);
  }
  ...
  /* For every eighth future ID (i = 0, 16, 32, 48, ... ), link the associated
   * real ID (j = 1, 17, 33, 47, ...) to the future ID.
   */
  for (i = 0; i < 128; i += 2 * 8) {

    j = i + 1;

    err_cnt += link_real_and_future_ids(&(objects_array[i]),
                                        &(objects_array[j]), rpt_failures);
  }
  ...
  /* Lookup all the future and real IDs.  Verify that every eighth future ID
   * lookup (i = 0, 16, 32, 48, ... ) succeeds, while the rest fail, and that
   * every eighth real ID lookup (j = 1, 17, 33, 47, ...) fails while the rest
   * succeed.
   */
  for (i = 0; i < 128; i += 2) {
    ...
    if (0 == i % 16) {
      ...
      err_cnt += object_verify(&(types_array[0]), &(id_instance_array[i]),
                               &(objects_array[j]), cs, ds, FALSE, tid);
      ...
      if (!id_obj_k.discarded) {
        ...
        if (rpt_failures) {
          ...
          fprintf(stderr,
                  "objects_array[%d], not marked as discarded after future id "
                  "realized.\n",
                  i);
        }
      }
      ...
      err_cnt += object_verify(&(types_array[0]), &(id_instance_array[j]),
                               &(objects_array[j]), cs, ds, TRUE, tid);
    }
  }
  ...
  /* For every eighth + 1 future ID (i = 2, 18, 34, 50, ... ), link the
   * associated real ID (j = 3, 19, 35, 51, ...) to the future ID.
   */
  for (i = 2; i < 128; i += 2 * 8) {
  ...
  for (i = 2; i < 128; i += 2 * 8) {
    ...
    err_cnt += dec_ref(&(types_array[0]), &(id_instance_array[j]),
                       &(objects_array[j]), cs, ds, TRUE, tid);
  }
  ...
  if (display_op_stats) {
    ...
    H5I_dump_nz_stats(
        stdout, "decrement (really delete) real ID ref counts 1 -- test b");
  ...
  if (display_op_stats) {
    ...
    H5I_dump_nz_stats(
        stdout, "ID lookups 4 -- verify real IDs -- should fail -- test b");
  ...
  for (i = 0; i < 128; i += 2) {
    ...
    if (0 == i % 16) {
      ...
      /* Attempt to verify the future ID twise -- should fail both times.  Need
       * to do this because the fact that the real ID has been deleted will not
       * become evident until after the realize callback has been called on the
       * first invocation.  Thus a second attempt is necessary to verify that we
       * handle the repeat call to the realize callback gracefully.
       */
      err_cnt += object_verify(&(types_array[0]), &(id_instance_array[i]), NULL,
                               cs, ds, FALSE, tid);
      err_cnt += object_verify(&(types_array[0]), &(id_instance_array[i]), NULL,
  ...
  if (display_op_stats) {
    ...
    H5I_dump_nz_stats(stdout,
                      "Realize future IDs 3 -- should succeed -- test c");
 ...
 *
 *      This test function is intended to implement an initial smoke check in
 *which multiple IDs of multiple types are created, accessed, ref count
 *incremented and/or decremented, and finally discarded -- all without any
 *expected failures.
 *
 *      Note that all types must be defined prior to the invocation of this
 *function.
 *
 *      The test proceeds as follows:
 *
 *      1) register params->ids_count IDs with the specified start and stride in
 *the id_instance_array[], and with types specified by the params->type_start,
 ...
 *      6) Verify all IDs
 *
 *      7) Decrement the ref count on all IDs -- in effect, this deletes every
 *other ID registered in 1).
 *
 *******************************************************************************************/
...
void *mt_test_fcn_1(void *_params) {
  ...
  if (show_progress) {
    ...
    fprintf(stderr, "mt_test_fcn_1:%d: -- point 0 -- entering \n",
            params->thread_id);
    fprintf(stderr,
            "mt_test_fcn_1:%d: params st/cnt/st = %d/%d/%d, id st/cnt/st = "
            "%d/%d/%d, obj st/cnt/st = %d/%d/%d\n",
            params->thread_id, params->types_start, params->types_count,
            params->types_stride, params->ids_start, params->ids_count,
            params->ids_stride, params->objects_start, params->objects_count,
            params->objects_stride);
  }
  ...
  params->err_cnt += inc_type_refs(params->types_start, params->types_count / 2,
                                   params->types_stride, params->cs, params->ds,
  ...
  for (i = 0; i < params->types_count; i++) {
    ...
    params->err_cnt +=
        dec_refs(j, 1, 1, params->ids_start + i,
                 params->ids_count / (params->types_count * 2),
                 params->ids_stride * params->types_count, params->cs,
                 params->ds, params->rpt_failures, params->thread_id);
  }
 ...
 *
 *      This test function is intended to implement a test consisting of a
 *largely random interleaving of various H5I API calls in which types are
 *created, flushed, and destroyed, and IDs are registered, accessed, reference
 *counts incremented and decremented, and eventually deleted.  Since this
 *process is largely random, this means that types and IDs may be referenced
...
void *mt_test_fcn_2(void *_params) {
  ...
  if (show_progress) {
    ...
    fprintf(stderr, "mt_test_fcn_1:%d: -- point 0 -- entering \n",
            params->thread_id);
    fprintf(stderr,
            "mt_test_fcn_2:%d: type st/cnt/st = %d/%d/%d, id st/cnt/st = "
            "%d/%d/%d, obj st/cnt/st = %d/%d/%d\n",
            params->thread_id, params->types_start, params->types_count,
            params->types_stride, params->ids_start, params->ids_count,
            params->ids_stride, params->objects_start, params->objects_count,
  ...
  for (i = 0; i < NUM_ID_TYPES; i++) {
    ...
    if (i < type_index) {
      ...
      try_register_type(type_index, params->cs, params->ds,
                        params->rpt_failures, params->thread_id);
  ...
  H5E_BEGIN_TRY {
    ...
    if (NULL == (id_op_counts = (int *)malloc(((size_t)(NUM_ID_INSTANCES)) *
                                              sizeof(int)))) {
    ...
    while ((proceed) && (ids_completed < params->ids_count)) {
      ...
      target_id_index = params->ids_start +
                        ((rand() % params->ids_count) * params->ids_stride);
      assert(target_id_index >= 0);
      assert(target_id_index < NUM_ID_INSTANCES);

      /* load the target ID's assigned type index.  Note that this type is not
       * guaranteed to have benn registered yet unless it is one of the types
      ...
      /* if id_op_counts[target_id_index] == 0, check to see if the
       * target_id_type_index is one of the type indexes assigned to this
       * thread.  If so, register the target id.  Otherwise, increment its
       * reference count.
       */
      if (0 == id_op_counts[target_id_index]) {
        ...
        if (types_assigned[target_id_type_index]) {
          ...
          /* the type assigned to target_id_type_index has been registered --
           * now register the id */

          assert(0 == atomic_load(&(id_instance_array[target_id_index]
                                        .successful_registrations)));
          assert(0 ==
                 atomic_load(&(
                     id_instance_array[target_id_index].failed_registrations)));

          /* use the same index for the object as the instance */
          try_register_id(target_id_index, target_id_index, params->cs,
                          params->ds, params->rpt_failures, params->thread_id);

          assert(1 == atomic_load(&(id_instance_array[target_id_index]
                                        .successful_registrations)));
          assert(0 ==
                 atomic_load(&(
                     id_instance_array[target_id_index].failed_registrations)));
          ...
          /* the target id may or may not be registerd -- just try to increment
           * its ref count */
    ...
    if (MT_TEST_1__DISPLAY_FINAL_STATS) {
      ...
      fprintf(stderr,
              "\nref cnt incr / decr / get ambig = %d / %d / %d, obj ver / rmv "
              "ver ambig = %d / %d, ambig_cnt = %d\n\n",
              id_incr_ambig, id_decr_ambig, id_get_ref_ambig, id_obj_ver_ambig,
              id_obj_remove_ver_ambig, params->ambig_cnt);
    }
    ...
    for (i = 0; i < NUM_ID_TYPES; i++) {
      ...
      if (types_assigned[i]) {
        ...
        params->ambig_cnt += try_destroy_type(
            i, params->cs, params->ds, params->rpt_failures, params->thread_id);
      }
    }
  }
  H5E_END_TRY
 ...
 *
 *      1) set up the instance of mt_test_params_t necessary for the call to
 *mt_test_fcn_1.
 *
...
void mt_test_fcn_1_serial_test(void) {
  ...
  err_cnt += destroy_types(params.types_start, params.types_count,
                           params.types_stride, params.cs, params.ds,
                           params.rpt_failures, params.thread_id);
 ...
 * mt_test_1()
 *
 *      Initial multi-thread test of H5I.  The objective is to perform an
 *initial smoke check with no failed calls.
 *
 *      1) set up the instances of mt_test_params_t for the specified number of
 *threads.
 *
 *      2) Create the types needed in the call to mt_test_fcn_1().
 *
 *      3) Create the specified number of threads and invoke mt_test_fcn_1 with
 *the appropriate instance of mt_test_params_t.
 *
...
void mt_test_1(int num_threads) {
  ...
  err_cnt += create_types(params[0].types_start, params[0].types_count,
                          params[0].types_stride, cs, ds, rpt_failures, 0);

  for (i = 0; i < num_threads; i++) {

    if (0 != pthread_create(&(threads[i]), NULL, &mt_test_fcn_1,
                            (void *)(&(params[i])))) {
  ...
  err_cnt += destroy_types(params[0].types_start, params[0].types_count,
                           params[0].types_stride, params[0].cs, params[0].ds,
                           params[0].rpt_failures, params[0].thread_id);
...
void mt_test_2(int num_threads) {
  ...
  for (i = 0; i < num_threads; i++) {
    ...
    if (0 != pthread_create(&(threads[i]), NULL, &mt_test_fcn_2,
                            (void *)(&(params[i])))) {
  ...
  for (i = 0; i < NUM_ID_TYPES; i++) {
    ...
    type_successful_registers +=
        atomic_load(&(types_array[i].successful_registers));
    ...
    type_successful_destroys +=
        atomic_load(&(types_array[i].successful_destroys));
  ...
  for (i = 0; i < NUM_ID_INSTANCES; i++) {
    ...
    id_successful_registrations +=
        atomic_load(&(id_instance_array[i].successful_registrations));
    id_failed_registrations +=
        atomic_load(&(id_instance_array[i].failed_registrations));
    id_successful_verifies +=
        atomic_load(&(id_instance_array[i].successful_verifies));
    id_failed_verifies += atomic_load(&(id_instance_array[i].failed_verifies));
    id_successful_inc_refs +=
        atomic_load(&(id_instance_array[i].successful_inc_refs));
    id_failed_inc_refs += atomic_load(&(id_instance_array[i].failed_inc_refs));
    id_successful_dec_refs +=
        atomic_load(&(id_instance_array[i].successful_dec_refs));
    ...
    id_successful_get_ref_cnts +=
        atomic_load(&(id_instance_array[i].successful_get_ref_cnts));
    id_failed_get_ref_cnts +=
        atomic_load(&(id_instance_array[i].failed_get_ref_cnts));
    id_successful_remove_verifies +=
        atomic_load(&(id_instance_array[i].successful_remove_verifies));
    id_failed_remove_verifies +=
        atomic_load(&(id_instance_array[i].failed_remove_verifies));
  }
