...
static int test_dangle_dataset(H5F_close_degree_t degree) {
  ...
  if ((dsid = H5Dcreate2(fid, DSETNAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
...
static int test_dangle_datatype1(H5F_close_degree_t degree) {
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    if (H5Tcommit2(fid, TYPENAME, tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) >=
        0)
      TEST_ERROR;
  }
  H5E_END_TRY
...
static int test_dangle_datatype2(H5F_close_degree_t degree) {
  ...
  if ((sid = H5Screate(H5S_SCALAR)) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(fid, DSETNAME, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Sclose(sid) < 0)
    TEST_ERROR;
...
static int test_dangle_attribute(H5F_close_degree_t degree) {
  ...
  if ((aid = H5Acreate2(dsid, ATTRNAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
