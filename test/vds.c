...
static const char *FILENAME[] = {
    "vds_virt_0", "vds_virt_1", "vds_src_0",  "vds_src_1", "vds%%_src",
...
static htri_t vds_select_equal(hid_t space1, hid_t space2) {
  ...
  case H5S_SEL_POINTS: {
    ...
    if (NULL == (buf1 = (hsize_t *)malloc((size_t)rank1 * (size_t)npoints1 *
                                          sizeof(hsize_t))))
      TEST_ERROR;
    if (NULL == (buf2 = (hsize_t *)malloc((size_t)rank1 * (size_t)npoints1 *
                                          sizeof(hsize_t))))
      TEST_ERROR;

    /* Get and compare point lists */
    if (H5Sget_select_elem_pointlist(space1, (hsize_t)0, (hsize_t)npoints1,
                                     buf1) < 0)
      TEST_ERROR;
    if (H5Sget_select_elem_pointlist(space2, (hsize_t)0, (hsize_t)npoints1,
                                     buf2) < 0)
      TEST_ERROR;
  ...
  case H5S_SEL_HYPERSLABS: {
     ...
     * allocating, to make sure buffers are freed. */
    if (NULL == (buf1 = (hsize_t *)malloc((size_t)2 * (size_t)rank1 *
                                          (size_t)nblocks1 * sizeof(*buf1))))
      TEST_ERROR;
    if (NULL == (buf2 = (hsize_t *)malloc((size_t)2 * (size_t)rank1 *
                                          (size_t)nblocks1 * sizeof(*buf2))))
      TEST_ERROR;

    /* Get and compare block lists */
    if (H5Sget_select_hyper_blocklist(space1, (hsize_t)0, (hsize_t)nblocks1,
                                      buf1) < 0)
      TEST_ERROR;
    if (H5Sget_select_hyper_blocklist(space2, (hsize_t)0, (hsize_t)nblocks1,
                                      buf2) < 0)
      TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int vds_check_mapping(hid_t dcpl, size_t i, hid_t vspace, hid_t srcspace,
...
static int test_api_get_ex_dcpl(test_api_config_t config, hid_t fapl,
                                hid_t dcpl, hid_t *ex_dcpl, hid_t vspace,
  ...
  if (config >= TEST_API_CREATE_DSET) {
    ...
    if ((file = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
      TEST_ERROR;
    if ((dset = H5Dcreate2(file, "vdset", H5T_NATIVE_INT, vspace, H5P_DEFAULT,
                           dcpl, H5P_DEFAULT)) < 0)
      TEST_ERROR;
...
static int test_api(test_api_config_t config, hid_t fapl, H5F_libver_t low) {
  ...
  const char *src_file[4] = {
      "src_file1", "src_file2.", "src_file3..",
      "src_file4..."}; /* Source file names (different lengths) */
  const char *src_dset[4] = {
      "src_dset1....", "src_dset2.....", "src_dset3......",
  ...
  if (H5Pset_virtual(dcpl, vspace[0], src_file[0], src_dset[0], srcspace[0]) <
      0)
    TEST_ERROR;
  ...
  if (vds_check_mapping(ex_dcpl, (size_t)0, vspace[0], srcspace[0], src_file[0],
                        src_dset[0]) < 0)
    TEST_ERROR;
  ...
  block[0] = 2;
  block[1] = 4;
  if (H5Sselect_hyperslab(srcspace[0], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    TEST_ERROR;
  ...
  block[0] = 5;
  block[1] = 6;
  if (H5Sselect_hyperslab(vspace[0], H5S_SELECT_SET, start, NULL, count,
                          block) < 0)
    TEST_ERROR;
  start[0] = 7;
  start[1] = 0;
  block[0] = 1;
  block[1] = 18;
  if (H5Sselect_hyperslab(vspace[0], H5S_SELECT_OR, start, NULL, count, block) <
      0)
    TEST_ERROR;
  ...
  block[0] = 1;
  block[1] = 5;
  if (H5Sselect_hyperslab(srcspace[0], H5S_SELECT_SET, start, NULL, count,
                          block) < 0)
    TEST_ERROR;
  ...
  block[0] = 2;
  block[1] = 4;
  if (H5Sselect_hyperslab(srcspace[1], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    TEST_ERROR;
  ...
  block[0] = 5;
  block[1] = 6;
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_SET, start, NULL, count,
                          block) < 0)
    TEST_ERROR;
  start[0] = 7;
  start[1] = 0;
  block[0] = 1;
  block[1] = 18;
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_OR, start, NULL, count, block) <
      0)
    TEST_ERROR;
  ...
  block[0] = 1;
  block[1] = 5;
  if (H5Sselect_hyperslab(srcspace[3], H5S_SELECT_SET, start, NULL, count,
                          block) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < 4; i++)
    if (H5Pset_virtual(dcpl, vspace[i], src_file[i], src_dset[i], srcspace[i]) <
        0)
      TEST_ERROR;
  ...
  for (i = 0; i < 4; i++)
    if (vds_check_mapping(ex_dcpl, (size_t)i, vspace[i], srcspace[i],
                          src_file[i], src_dset[i]) < 0)
      TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    ret =
        H5Pset_virtual(dcpl, vspace[0], src_file[0], src_dset[0], srcspace[0]);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
  ...
  for (i = 0; i < LIST_DOUBLE_SIZE; i++) {
    ...
    start[0] = (hsize_t)i;
    if (H5Sselect_hyperslab(vspace[i], H5S_SELECT_SET, start, NULL, count,
                            block) < 0)
      TEST_ERROR;
    ...
    /* Add virtual layout mapping */
    if (H5Pset_virtual(dcpl, vspace[i], tmp_filename, tmp_dsetname,
                       srcspace[0]) < 0)
      TEST_ERROR;
  }
  ...
  /* Get examination DCPL */
  if (test_api_get_ex_dcpl(config, fapl, dcpl, &ex_dcpl, vspace[0], filename,
                           (low >= H5F_LIBVER_V112) ? (hsize_t)607
                                                    : (hsize_t)697) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < LIST_DOUBLE_SIZE; i++) {
    ...
    /* Check that the mapping in the DCPL is correct */
    if (vds_check_mapping(ex_dcpl, (size_t)i, vspace[i], srcspace[0],
                          tmp_filename, tmp_dsetname) < 0)
      TEST_ERROR;
  }
 ...
 *        Should be able to access the target source files in tmp directory via
 *the prefix set by H5Pset_virtual_prefix()
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int test_vds_prefix_first(unsigned config, hid_t vds_fapl,
                                 hid_t src_fapl) {
  ...
  if ((srcfilename_map = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) ==
      NULL)
    TEST_ERROR_SUPPRESSED;
  if ((vfilename = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) == NULL)
    TEST_ERROR_SUPPRESSED;
  if ((srcfilenamepct = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) ==
      NULL)
    TEST_ERROR_SUPPRESSED;
  if ((srcfilenamepct_map = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) ==
      NULL)
    TEST_ERROR_SUPPRESSED;
  ...
  h5_fixname_printf(srcfilenamepct_map_orig, src_fapl, srcfilenamepct_map,
                    FILENAME_BUF_SIZE);
  ...
  /* Add virtual layout mapping */
  if (H5Pset_virtual(dcpl, vspace[0],
                     config & TEST_IO_DIFFERENT_FILE ? srcfilename_map : ".",
  ...
  if (config & TEST_IO_DIFFERENT_FILE) {
    ...
    if (HDchdir(TMPDIR) < 0)
      TEST_ERROR_SUPPRESSED;
    if ((srcfile[0] =
             H5Fcreate(srcfilename, H5F_ACC_TRUNC, H5P_DEFAULT, src_fapl)) < 0)
      TEST_ERROR_SUPPRESSED;
    if (HDchdir(buffer) < 0)
      TEST_ERROR_SUPPRESSED;
  } else {
  ...
  if ((srcdset[0] =
           H5Dcreate2(srcfile[0], "src_dset", H5T_NATIVE_INT, srcspace[0],
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;

  /* Create virtual dataset */
  if ((vdset = H5Dcreate2(vfile, "v_dset", H5T_NATIVE_INT, vspace[0],
                          H5P_DEFAULT, dcpl, dapl)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(srcdset[0], H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dread(vdset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rbuf[0]) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(vdset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf[0]) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  memset(rbuf[0], 0, sizeof(rbuf));
  if (H5Dread(srcdset[0], H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
...
static int test_basic_io(unsigned config, hid_t vds_fapl, hid_t src_fapl) {
  ...
  start[0] = 0;
  start[1] = 0;
  if (H5Sselect_hyperslab(vspace[0], H5S_SELECT_SET, start, NULL, dims, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  start[1] = 13;
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_SET, start, NULL, dims, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Pset_virtual(dcpl, vspace[0],
    ...
    TEST_ERROR_SUPPRESSED;
  if (H5Pset_virtual(dcpl, vspace[1],
                     config & TEST_IO_DIFFERENT_FILE ? srcfilename_map : ".",
  ...
  if ((srcdset[0] =
           H5Dcreate2(srcfile[0], "%src_dset1", H5T_NATIVE_INT, srcspace[0],
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[1] =
           H5Dcreate2(srcfile[0], "src_dset2%", H5T_NATIVE_INT, srcspace[0],
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(srcdset[0], H5T_NATIVE_INT, vspace[0], H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[1], H5T_NATIVE_INT, vspace[1], H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  memset(rbuf[0], 0, sizeof(rbuf));
  if (H5Dread(srcdset[0], H5T_NATIVE_INT, vspace[0], H5S_ALL, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dread(srcdset[1], H5T_NATIVE_INT, vspace[1], H5S_ALL, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (config & TEST_IO_DIFFERENT_FILE) {
    ...
    if ((vfile2 = H5Fcreate(vfilename2, H5F_ACC_TRUNC, H5P_DEFAULT, vds_fapl)) <
        0)
      TEST_ERROR_SUPPRESSED;

    /* Copy virtual dataset */
    if (H5Ocopy(vfile, "v_dset", vfile2, "v_dset3", H5P_DEFAULT, H5P_DEFAULT) <
        0)
      TEST_ERROR_SUPPRESSED;
  ...
  if (H5Soffset_simple(vspace[0], offset) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[1], H5T_NATIVE_INT, vspace[0], H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Soffset_simple(vspace[0], offset) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dread(srcdset[1], H5T_NATIVE_INT, vspace[0], H5S_ALL, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Add virtual layout mappings */
  if (H5Pset_virtual(dcpl, vspace[0],
                     config & TEST_IO_DIFFERENT_FILE ? srcfilenamepct_map : ".",
                     "src_dset1", srcspace[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Pset_virtual(dcpl, vspace[1],
                     config & TEST_IO_DIFFERENT_FILE ? srcfilenamepct_map : ".",
  ...
  /* Create source file if requested */
  if (config & TEST_IO_DIFFERENT_FILE) {
    if ((srcfile[0] = H5Fcreate(srcfilenamepct, H5F_ACC_TRUNC, H5P_DEFAULT,
                                src_fapl)) < 0)
      TEST_ERROR_SUPPRESSED;
  } else {
    srcfile[0] = vfile;
  ...
  memset(rbuf[0], 0, sizeof(rbuf));
  if (H5Dread(vdset, H5T_NATIVE_INT, vspace[0], vspace[0], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  for (i = 0; i < (int)(sizeof(buf) / sizeof(buf[0])); i++)
    for (j = 0; j < (int)(sizeof(buf[0]) / sizeof(buf[0][0])); j++)
      if (rbuf[i][j] !=
          (j < (int)(sizeof(buf[0]) / sizeof(buf[0][0]) / 2) ? buf[i][j] : 0))
        TEST_ERROR_SUPPRESSED;

  /* Read second source dataset through virtual dataset */
  memset(rbuf[0], 0, sizeof(rbuf));
  if (H5Dread(vdset, H5T_NATIVE_INT, vspace[1], vspace[1], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  for (i = 0; i < (int)(sizeof(buf) / sizeof(buf[0])); i++)
    for (j = 0; j < (int)(sizeof(buf[0]) / sizeof(buf[0][0])); j++)
      if (rbuf[i][j] !=
          (j < (int)(sizeof(buf[0]) / sizeof(buf[0][0]) / 2) ? 0 : buf[i][j]))
        TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(vdset, H5T_NATIVE_INT, vspace[0], vspace[0], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(vdset, H5T_NATIVE_INT, vspace[1], vspace[1], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  for (i = 0; i < (int)(sizeof(buf) / sizeof(buf[0])); i++)
    for (j = 0; j < (int)(sizeof(buf[0]) / sizeof(buf[0][0])); j++)
      if (rbuf[i][j] !=
          (j < (int)(sizeof(buf[0]) / sizeof(buf[0][0]) / 2)
               ? (buf[i][j] - (int)(sizeof(buf) / sizeof(buf[0][0])))
               : buf[i][j]))
        TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(srcspace[0], H5S_SELECT_SET, start, NULL, count,
    ...
    TEST_ERROR_SUPPRESSED;
  start[1] = 13;
  if (H5Sselect_hyperslab(srcspace[1], H5S_SELECT_SET, start, NULL, count,
                          NULL) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  count[0] = 5;
  count[1] = 26;
  if (H5Sselect_hyperslab(vspace[0], H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  start[0] = 5;
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  if ((srcdset[0] =
    ...
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[1] =
           H5Dcreate2(srcfile[0], "src_dset2", H5T_NATIVE_INT, srcspace[1],
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(srcdset[1], H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read first slice */
  if (H5Dread(vdset, H5T_NATIVE_INT, vspace[0], srcspace[0], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read second slice */
  if (H5Dread(vdset, H5T_NATIVE_INT, vspace[1], srcspace[1], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write data through virtual dataset */
  /* Write first slice */
  if (H5Dwrite(vdset, H5T_NATIVE_INT, vspace[0], srcspace[0], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write second slice */
  if (H5Dwrite(vdset, H5T_NATIVE_INT, vspace[1], srcspace[1], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read first dataset */
  if (H5Dread(srcdset[0], H5T_NATIVE_INT, srcspace[0], srcspace[0], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read second dataset */
  if (H5Dread(srcdset[1], H5T_NATIVE_INT, srcspace[1], srcspace[1], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  block[0] = 1;
  block[1] = 1;
  if (H5Sselect_hyperslab(vspace[0], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    TEST_ERROR_SUPPRESSED;
  start[0] = 1;
  start[1] = 1;
  if (H5Sselect_hyperslab(vspace[0], H5S_SELECT_OR, start, stride, count,
                          block) < 0)
    TEST_ERROR_SUPPRESSED;
  start[0] = 0;
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    TEST_ERROR_SUPPRESSED;
  start[0] = 1;
  start[1] = 0;
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_OR, start, stride, count,
                          block) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write data directly to source datasets */
  /* Write first dataset */
  if (H5Dwrite(srcdset[0], H5T_NATIVE_INT, memspace, srcspace[0], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write second dataset */
  if (H5Dwrite(srcdset[1], H5T_NATIVE_INT, memspace, srcspace[0], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dread(vdset, H5T_NATIVE_INT, memspace, srcspace[0], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dread(vdset, H5T_NATIVE_INT, memspace, srcspace[1], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write data through virtual dataset */
  /* Write first slice */
  if (H5Dwrite(vdset, H5T_NATIVE_INT, memspace, srcspace[0], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write second slice */
  if (H5Dwrite(vdset, H5T_NATIVE_INT, memspace, srcspace[1], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read first dataset */
  if (H5Dread(srcdset[0], H5T_NATIVE_INT, memspace, srcspace[0], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read second dataset */
  if (H5Dread(srcdset[1], H5T_NATIVE_INT, memspace, srcspace[0], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  count[0] = 2;
  count[1] = 16;
  if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;

  /* Write data directly to source dataset */
  if (H5Dwrite(srcdset[0], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read first stripe pattern */
  if (H5Dread(vdset, H5T_NATIVE_INT, memspace, vspace[0], H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read third stripe pattern */
  if (H5Dread(vdset, H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
              rbuf99[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(vdset, H5T_NATIVE_INT, memspace, vspace[0], H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read dataset */
  if (H5Dread(srcdset[0], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  H5E_BEGIN_TRY {
    ret =
        H5Dwrite(vdset, H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT, buf[0]);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(srcdset[1], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  block[0] = 1;
  block[1] = 1;
  if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_SET, start, NULL, dims, NULL) <
    ...
    TEST_ERROR_SUPPRESSED;
  start[0] = 4;
  if (H5Sselect_hyperslab(vspace[2], H5S_SELECT_SET, start, NULL, dims, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Pset_virtual(dcpl, vspace[1],
                     ...
                     "src_dset2", srcspace[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Pset_virtual(dcpl, vspace[2],
                     config & TEST_IO_DIFFERENT_FILE ? srcfilename_map : ".",
                     "src_dset3", srcspace[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if ((srcdset[1] =
    ...
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[2] =
           H5Dcreate2(srcfile[0], "src_dset3", H5T_NATIVE_INT, srcspace[0],
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(srcdset[2], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read data through virtual dataset */
  memset(rbuf[0], 0, sizeof(rbuf));
  if (H5Dread(vdset, H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT, rbuf[0]) <
      0)
    TEST_ERROR_SUPPRESSED;
...
static int test_unlim(unsigned config, hid_t vds_fapl, hid_t src_fapl) {
  ...
  h5_fixname_printf(FILENAME[2], src_fapl, srcfilename_map,
                    sizeof srcfilename_map);
  ...
  if ((srcdset[0] =
           H5Dcreate2(srcfile[0], "src_dset1", H5T_NATIVE_INT, srcspace[0],
                      H5P_DEFAULT, srcdcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[1] =
           H5Dcreate2(srcfile[0], "src_dset2", H5T_NATIVE_INT, srcspace[0],
                      H5P_DEFAULT, srcdcpl, H5P_DEFAULT)) < 0)
  ...
  /* Select hyperslab in memory space */
  start[0] = 0;
  if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, NULL, dims, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dread(vdset, H5T_NATIVE_INT, memspace, memspace, H5P_DEFAULT, rbuf[0]) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(srcspace[1], H5S_SELECT_SET, start, NULL, count,
                          NULL) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Sselect_hyperslab(srcspace[2], H5S_SELECT_SET, start, NULL, count,
                          NULL) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(vspace[1], H5S_SELECT_SET, start, stride, count,
    ...
    TEST_ERROR_SUPPRESSED;
  start[1] = 4;
  if (H5Sselect_hyperslab(vspace[2], H5S_SELECT_SET, start, stride, count,
                          block) < 0)
    TEST_ERROR_SUPPRESSED;
  start[1] = 0;
  ...
  if ((srcdset[1] =
    ...
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[2] =
           H5Dcreate2(srcfile[0], "src_dset3", H5T_NATIVE_INT, srcspace[2],
                      H5P_DEFAULT, srcdcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(filespace, H5S_SELECT_SET, start, NULL, count, NULL) <
    ...
    TEST_ERROR_SUPPRESSED;
  start[1] = 0;
  if (H5Dwrite(srcdset[2], H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Sclose(filespace) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(filespace, H5S_SELECT_SET, start, NULL, count, NULL) <
    ...
    TEST_ERROR_SUPPRESSED;
  start[0] = 0;
  if (H5Dread(vdset, H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
...
static int test_printf(unsigned config, hid_t vds_fapl, hid_t src_fapl) {
  ...
  if ((srcfilename2 = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) == NULL)
    TEST_ERROR_SUPPRESSED;
  if ((srcfilename2_map = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) ==
      NULL)
    TEST_ERROR_SUPPRESSED;
  if ((vfilename = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) == NULL)
    TEST_ERROR_SUPPRESSED;
  if ((printf_srcfilename_map =
           (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) == NULL)
    TEST_ERROR_SUPPRESSED;
  if ((srcfilenamepct = (char *)calloc(FILENAME_BUF_SIZE, sizeof(char))) ==
      NULL)
    TEST_ERROR_SUPPRESSED;
  ...
  h5_fixname_printf(printf_srcfilename_map_orig, src_fapl,
                    printf_srcfilename_map, FILENAME_BUF_SIZE);
  h5_fixname(FILENAME[4], src_fapl, srcfilenamepct, FILENAME_BUF_SIZE);
  h5_fixname_printf(srcfilenamepct_map_orig, src_fapl, srcfilenamepct_map,
                    FILENAME_BUF_SIZE);
  ...
  if ((srcdset[0] =
           H5Dcreate2(srcfile[0], "src_dset0", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[1] =
           H5Dcreate2(srcfile[0], "src_dset1", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write to srcdset[0] */
  if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, NULL, block, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[0], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
  ...
  if ((srcdset[2] =
           H5Dcreate2(srcfile[0], "src_dset2", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  for (start[1] = (hsize_t)0; start[1] < (hsize_t)5; start[1]++)
    for (count[1] = (hsize_t)1; count[1] < (hsize_t)11; count[1]++) {
      ...
      /* Select hyperslab in memory space */
      if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, NULL, count,
                              NULL) < 0)
        TEST_ERROR_SUPPRESSED;

      /* Select hyperslab in file space */
      if (H5Sselect_hyperslab(filespace, H5S_SELECT_SET, start, NULL, count,
                              NULL) < 0)
        TEST_ERROR_SUPPRESSED;
  ...
  /* Select hyperslab in file space */
  if (H5Sselect_hyperslab(filespace, H5S_SELECT_SET, start, NULL, dims, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Dwrite(vdset, H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read srcdset[1] */
  if (H5Dread(srcdset[1], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Read srcdset[2] */
  if (H5Dread(srcdset[2], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
              rbuf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if ((srcdset[1] =
    ...
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[2] =
           H5Dcreate2(srcfile[0], "src_dset3", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[3] =
           H5Dcreate2(srcfile[0], "src_dset6", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[4] =
           H5Dcreate2(srcfile[0], "src_dset10", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[5] =
           H5Dcreate2(srcfile[0], "src_dset15", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write to srcdset[3] */
  if (H5Dwrite(srcdset[3], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write to srcdset[4] */
  if (H5Dwrite(srcdset[4], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Write to srcdset[5] */
  if (H5Dwrite(srcdset[5], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (config & TEST_IO_DIFFERENT_FILE) {
    ...
    /* Add virtual layout mapping */
    if (H5Pset_virtual(dcpl, vspace[0], printf_srcfilename_map, "src_dset",
                       srcspace) < 0)
      TEST_ERROR_SUPPRESSED;

    /* Create virtual file */
    if ((vfile = H5Fcreate(vfilename, H5F_ACC_TRUNC, H5P_DEFAULT, vds_fapl)) <
        0)
      TEST_ERROR_SUPPRESSED;
    ...
    if ((srcfile[0] =
      ...
      TEST_ERROR_SUPPRESSED;
    if ((srcdset[0] =
             H5Dcreate2(srcfile[0], "src_dset", H5T_NATIVE_INT, srcspace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
      TEST_ERROR_SUPPRESSED;
    if ((srcfile[1] =
             H5Fcreate(srcfilename2, H5F_ACC_TRUNC, H5P_DEFAULT, src_fapl)) < 0)
      TEST_ERROR_SUPPRESSED;
    ...
    /* Write to srcdset[0] */
    if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, NULL, block,
                            NULL) < 0)
      TEST_ERROR_SUPPRESSED;
    if (H5Dwrite(srcdset[0], H5T_NATIVE_INT, memspace, H5S_ALL, H5P_DEFAULT,
                 buf[0]) < 0)
    ...
    if ((srcdset[1] =
             H5Dcreate2(srcfile[1], "src_dset", H5T_NATIVE_INT, srcspace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
      TEST_ERROR_SUPPRESSED;
    ...
    /* Add virtual layout mapping */
    if (H5Pset_virtual(dcpl, vspace[0], printf_srcfilename_map,
                       "%%src%%_dset%%%b", srcspace) < 0)
      TEST_ERROR_SUPPRESSED;
    ...
    if ((srcfile[0] =
      ...
      TEST_ERROR_SUPPRESSED;
    if ((srcdset[0] =
             H5Dcreate2(srcfile[0], "%src%_dset%0", H5T_NATIVE_INT, srcspace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
      TEST_ERROR_SUPPRESSED;
    if ((srcfile[1] =
             H5Fcreate(srcfilename2, H5F_ACC_TRUNC, H5P_DEFAULT, src_fapl)) < 0)
    ...
    /* Create 2nd source dataset */
    if ((srcdset[1] =
             H5Dcreate2(srcfile[1], "%src%_dset%1", H5T_NATIVE_INT, srcspace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
      TEST_ERROR_SUPPRESSED;
  ...
  count[0] = 10;
  count[1] = 1;
  if (H5Sselect_hyperslab(srcspace, H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Create 2 source datasets */
  if ((srcdset[0] =
           H5Dcreate2(srcfile[0], "0src_dset_a0%", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[1] =
           H5Dcreate2(srcfile[0], "src_dset_b0%%", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Create 3rd source dataset */
  if ((srcdset[2] =
           H5Dcreate2(srcfile[0], "src_dset_b1%%", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if ((srcdset[3] =
           H5Dcreate2(srcfile[0], "2src_dset_a2%", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Create 2 source datasets */
  if ((srcdset[0] =
           H5Dcreate2(srcfile[0], "src_dset_a0", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  if ((srcdset[1] =
           H5Dcreate2(srcfile[0], "src_dset_b0", H5T_NATIVE_INT, srcspace,
  ...
  if (H5Sselect_hyperslab(srcspace, H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[0], H5T_NATIVE_INT, memspace, srcspace, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_all(srcspace) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[1], H5T_NATIVE_INT, memspace, srcspace, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Create 3rd source dataset */
  if ((srcdset[2] =
           H5Dcreate2(srcfile[0], "src_dset_a1", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(srcspace, H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[2], H5T_NATIVE_INT, memspace, srcspace, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  /* Create 4th source dataset */
  if ((srcdset[3] =
           H5Dcreate2(srcfile[0], "src_dset_a2", H5T_NATIVE_INT, srcspace,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, start, NULL, block, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[3], H5T_NATIVE_INT, memspace, srcspace, H5P_DEFAULT,
               buf[0]) < 0)
    TEST_ERROR_SUPPRESSED;
...
static int test_all(unsigned config, hid_t vds_fapl, hid_t src_fapl) {
  ...
  if ((srcspace[1] = H5Screate_simple(2, dims, block)) < 0)
    TEST_ERROR_SUPPRESSED;
  start[0] = 0;
  if (H5Sselect_hyperslab(srcspace[1], H5S_SELECT_SET, start, NULL, block,
                          NULL) < 0)
    TEST_ERROR_SUPPRESSED;
  if (H5Pset_virtual(dcpl, vspace[1],
                     config & TEST_IO_DIFFERENT_FILE ? srcfilename_map : ".",
  ...
  if ((srcdset[0] =
           H5Dcreate2(srcfile, "src_dset_fixed", H5T_NATIVE_INT, srcspace[0],
                      H5P_DEFAULT, srcdcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;

  /* Create unlimited source_dataset */
  if ((srcdset[1] =
           H5Dcreate2(srcfile, "src_dset_unlim", H5T_NATIVE_INT, srcspace[1],
                      H5P_DEFAULT, srcdcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if ((srcdset[2] =
           H5Dcreate2(srcfile, "src_dset_printf_0", H5T_NATIVE_INT, srcspace[2],
                      H5P_DEFAULT, srcdcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  start[0] = 2;
  start[1] = 0;
  if (H5Sselect_hyperslab(filespace, H5S_SELECT_SET, start, NULL, block, NULL) <
      0)
    TEST_ERROR_SUPPRESSED;
  if (H5Dwrite(srcdset[1], H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT,
               buf[0]) < 0)
  ...
  if ((srcdset[3] =
           H5Dcreate2(srcfile, "src_dset_printf_1", H5T_NATIVE_INT, srcspace[2],
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
  ...
  if ((srcdset[4] =
           H5Dcreate2(srcfile, "src_dset_printf_2", H5T_NATIVE_INT, srcspace[2],
                      H5P_DEFAULT, srcdcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR_SUPPRESSED;
...
static int test_dapl_values(hid_t fapl_id) {
  ...
  count = H5S_UNLIMITED;
  block = 42;
  if (H5Sselect_hyperslab(vds_sid, H5S_SELECT_SET, &start, &stride, &count,
                          &block) < 0)
    FAIL_STACK_ERROR;
  /* map */
  ...
  /* Create the datasets */
  if ((did1 = H5Dcreate2(fid, "dset1", H5T_NATIVE_INT, vds_sid, H5P_DEFAULT,
                         dcpl_id, dapl_id1)) < 0)
    FAIL_STACK_ERROR;
  if ((did2 = H5Dcreate2(fid, "dset2", H5T_NATIVE_INT, vds_sid, H5P_DEFAULT,
                         dcpl_id, dapl_id2)) < 0)
    FAIL_STACK_ERROR;
...
int main(void) {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      HDsnprintf(
          msg, sizeof(msg),
          "Testing virtual dataset I/O with file version bounds: (%s, %s):",
          low_string, high_string);
      HDputs(msg);

      for (test_api_config = (int)TEST_API_BASIC;
           test_api_config < (int)TEST_API_NTESTS; test_api_config++)
      ...
      for (bit_config = 0; bit_config < TEST_IO_NTESTS; bit_config++) {
        ...
        PRINT_CONFIG(
            ...
            bit_config & TEST_IO_FCLOSE_SEMI
                ? "H5F_CLOSE_SEMI"
                : (bit_config & TEST_IO_FCLOSE_STRONG ? "H5F_CLOSE_STRONG"
                                                      : "H5F_CLOSE_WEAK"));
