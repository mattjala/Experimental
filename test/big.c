  ...
  while (overlap != 0 && tries < MAX_TRIES) {
    ...
    for (i = 0; i < current_index; i++) {
      if ((acc >= values_used[i]) && (acc < values_used[i] + WRT_SIZE))
        overlap = 1;
      if ((acc + WRT_SIZE >= values_used[i]) &&
          (acc + WRT_SIZE < values_used[i] + WRT_SIZE))
        overlap = 1;
    }
    tries++;
  }
...
static fsizes_t supports_big(void) {
  ...
  if ((fd = HDopen("y.h5", O_RDWR | O_TRUNC | O_CREAT,
                   H5_POSIX_CREATE_MODE_RW)) < 0)
    goto error;
...
static int enough_room(hid_t fapl) {
  ...
  for (i = 0; i < NELMTS(fd); i++) {
    ...
    if ((fd[i] = HDopen(name, O_RDWR | O_CREAT | O_TRUNC,
                        H5_POSIX_CREATE_MODE_RW)) < 0) {
      goto done;
    }
 ...
 *-------------------------------------------------------------------------
 */
static int writer(char *filename, hid_t fapl, fsizes_t testsize, int wrt_n) {
   ...
   *  will create a set of solid 1GB files; test will crash if quotas are
   * enforced or it will take some time to write a file. We should create a
   * dataset allocating space late and never writing fill values. EIP 4/8/03
   */
  dcpl = H5Pcreate(H5P_DATASET_CREATE);
  H5Pset_alloc_time(dcpl, H5D_ALLOC_TIME_LATE);
  H5Pset_fill_time(dcpl, H5D_FILL_TIME_NEVER);
  if ((d1 = H5Dcreate2(file, "d1", H5T_NATIVE_INT, space1, H5P_DEFAULT, dcpl,
                       H5P_DEFAULT)) < 0 ||
      (d2 = H5Dcreate2(file, "d2", H5T_NATIVE_INT, space2, H5P_DEFAULT, dcpl,
                       H5P_DEFAULT)) < 0) {
    goto error;
  }
  ...
  for (i = 0; i < wrt_n; i++) {
    ...
    if (H5Sselect_hyperslab(space2, H5S_SELECT_SET, hs_start, NULL, hs_size,
                            NULL) < 0)
      goto error;
...
static int reader(char *filename, hid_t fapl) {
  ...
  while (HDfgets(ln, (int)sizeof(ln), script)) {
    ...
    if (H5Sselect_hyperslab(fspace, H5S_SELECT_SET, hs_offset, NULL, hs_size,
                            NULL) < 0)
      FAIL_STACK_ERROR;
...
static int test_sec2(hid_t fapl) {
  ...
  if (testsize == NO_FILE) {
    fprintf(stdout, "Test for sec2 is skipped because file system does not "
                    "support big files.\n");
    goto quit;
  }
...
static int test_stdio(hid_t fapl) {
  ...
  testsize = supports_big();
  if (testsize == NO_FILE) {
    fprintf(stdout, "Test for stdio is skipped because file system does not "
                    "support big files.\n");
    goto quit;
  }
