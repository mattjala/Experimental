 ...
 *-------------------------------------------------------------------------
 */
static hbool_t files_have_same_contents(const char *name1, const char *name2) {
  int fd1 = 0;
  int fd2 = 0;
  ssize_t n1 = 0;
  ssize_t n2 = 0;
...
static int test_non_extendible(hid_t file) {
  ...
  if (H5Pset_external(dcpl, "ext1.data", (off_t)0,
                      (hsize_t)(max_size[0] * sizeof(int))) < 0)
    FAIL_STACK_ERROR;
  if ((space = H5Screate_simple(1, cur_size, max_size)) < 0)
    FAIL_STACK_ERROR;
  if ((dset = H5Dcreate2(file, "dset1", H5T_NATIVE_INT, space, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Dclose(dset) < 0)
    FAIL_STACK_ERROR;
...
static int test_too_small(hid_t file) {
  ...
  if ((dcpl = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    FAIL_STACK_ERROR;
  if (H5Pset_external(dcpl, "ext1.data", (off_t)0,
                      (hsize_t)(max_size[0] * sizeof(int) - 1)) <
      0) /* note -1 */
    FAIL_STACK_ERROR;
  if ((space = H5Screate_simple(1, cur_size, max_size)) < 0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    dset = H5Dcreate2(file, "dset2", H5T_NATIVE_INT, space, H5P_DEFAULT, dcpl,
                      H5P_DEFAULT);
  }
  H5E_END_TRY
  if (dset >= 0)
...
static int test_large_enough_current_eventual(hid_t file) {
  ...
  if ((space = H5Screate_simple(1, cur_size, max_size)) < 0)
    FAIL_STACK_ERROR;
  if ((dset = H5Dcreate2(file, "dset3", H5T_NATIVE_INT, space, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static int test_unlimited(hid_t file) {
  ...
  if ((space = H5Screate_simple(1, cur_size, max_size)) < 0)
    FAIL_STACK_ERROR;
  if ((dset = H5Dcreate2(file, "dset5", H5T_NATIVE_INT, space, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int add_external_files(hid_t dcpl_id, unsigned int n_external_files,
                              off_t offset, hsize_t max_ext_size) {
  ...
  for (i = 0; i < n_external_files; i++) {
    if (HDsnprintf(exname, AEF_EXNAME_MAX_LEN, "ext%d.data", i + 1) >
        AEF_EXNAME_MAX_LEN) {
...
static int test_multiple_files(hid_t file) {
  ...
  if ((dset = H5Dcreate2(file, "dset6", H5T_NATIVE_INT, space, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    dset = H5Dcreate2(file, "dset7", H5T_NATIVE_INT, space, H5P_DEFAULT, dcpl,
                      H5P_DEFAULT);
  }
  H5E_END_TRY
  if (dset >= 0)
...
static int test_add_to_unlimited(void) {
  ...
  if (status >= 0)
    FAIL_PUTS_ERROR(
        "    H5Pset_external() succeeded when it should have failed.");
...
static int test_read_file_set(hid_t fapl) {
  ...
  if ((file = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
    FAIL_STACK_ERROR;
  if ((grp = H5Gcreate2(file, "emit-diagnostics", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(grp) < 0)
    FAIL_STACK_ERROR;
  ...
  for (i = 0; i < N_EXT_FILES; i++) {
    ...
    if (H5Pset_external(dcpl, filename, (off_t)(i * GARBAGE_PER_FILE),
                        (hsize_t)sizeof(part)) < 0)
      FAIL_STACK_ERROR;
  }
  ...
  if ((hs_space = H5Scopy(space)) < 0)
    FAIL_STACK_ERROR;
  if (H5Sselect_hyperslab(hs_space, H5S_SELECT_SET, &hs_start, NULL, &hs_count,
                          NULL) < 0)
    FAIL_STACK_ERROR;
...
static int test_write_file_set(hid_t fapl) {
  ...
  for (i = 0; i < N_EXT_FILES; i++) {
    ...
    if (H5Pset_external(dcpl, filename, (off_t)(i * GARBAGE_PER_FILE), size) <
        0)
      FAIL_STACK_ERROR;
  } /* end for */
  ...
  if ((file_space = H5Scopy(mem_space)) < 0)
    FAIL_STACK_ERROR;
  if ((dset = H5Dcreate2(file, "dset1", H5T_NATIVE_INT, file_space, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static int test_path_absolute(hid_t fapl) {
  ...
  if (NULL == HDgetcwd(cwdpath, sizeof(cwdpath)))
    TEST_ERROR;
  for (i = 0; i < N_EXT_FILES; i++) {
    HDsnprintf(filename, sizeof(filename), "%s%sextern_%zur.raw", cwdpath,
               H5_DIR_SEPS, i + 1);
#if defined(H5_HAVE_WINDOW_PATH)
    /* For windows, test path-absolute case (\dir\file.raw) for the second file
     */
    if (i == 1)
      HDsnprintf(filename, sizeof(filename), "%s%sextern_%zur.raw", cwdpath + 2,
                 H5_DIR_SEPS, i + 1);
#endif
    if (H5Pset_external(dcpl, filename, (off_t)(i * GARBAGE_PER_FILE),
                        (hsize_t)sizeof(part)) < 0)
...
static int test_path_relative_cwd(hid_t fapl) {
  ...
  if ((dcpl = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    FAIL_STACK_ERROR;
  for (i = 0; i < N_EXT_FILES; i++) {
    HDsnprintf(filename, sizeof(filename), "..%sextern_%dr.raw", H5_DIR_SEPS,
               (int)i + 1);
    if (H5Pset_external(dcpl, filename, (off_t)(i * GARBAGE_PER_FILE),
                        (hsize_t)sizeof(part)) < 0)
      FAIL_STACK_ERROR;
  ...
  if (dset3 >= 0)
    FAIL_PUTS_ERROR(
        "reopening the dataset with a different efile_prefix succeeded");
...
static int test_h5d_get_access_plist(hid_t fapl_id) {
  ...
  if ((sid = H5Screate_simple(1, &dims, NULL)) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dcreate2(fid, "dset1", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl_id,
                        dapl_id)) < 0)
    FAIL_STACK_ERROR;
...
int main(void) {
  ...
  if ((fapl_id_new = H5Pcopy(fapl_id_old)) < 0)
    FAIL_STACK_ERROR;
  if (H5Pset_libver_bounds(fapl_id_new, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) <
      0)
    FAIL_STACK_ERROR;
  ...
  for (latest_format = FALSE; latest_format <= TRUE; latest_format++) {
    ...
    if ((gid = H5Gcreate2(fid, "emit-diagnostics", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      FAIL_STACK_ERROR;
    if (H5Gclose(gid) < 0)
      FAIL_STACK_ERROR;
