...
static const unsigned def_type_flags[H5O_SHMESG_MAX_NINDEXES] = {0, 0, 0,
                                                                 0, 0, 0};
static const unsigned def_minsizes[H5O_SHMESG_MAX_NINDEXES] = {250, 250, 250,
                                                               250, 250, 250};
#define DEF_L2B 50
#define DEF_B2L 40
...
static const unsigned test_type_flags[H5O_SHMESG_MAX_NINDEXES] = {
    ...
    0,
    0};
static const unsigned test_minsizes[H5O_SHMESG_MAX_NINDEXES] = {0,   2, 40,
                                                                100, 3, 1000};
#define TEST_L2B 65
#define TEST_B2L 64
...
static const char *ENUM_NAME[] = {
    "enum_member0",  "enum_member1",  "enum_member2",
...
static const int ENUM_VAL[] = {0,    13,  -500,  63, 64,    -64,   65,  2048,
                               1,    2,   -1,    7,  130,   -5000, 630, 640,
                               -640, 650, 20480, 10, -1001, -10};
...
#define LONG_STRING                                                            \
  "00 index.  A long string used for testing. To create new strings, set the " \
  "first two characters to be "                                                \
...
**
****************************************************************/
static void verify_fcpl_values(hid_t fcpl_id, const unsigned nindexes_expected,
                               const unsigned *flags_expected,
                               const unsigned *minsizes_expected, unsigned l2b,
                               unsigned b2l) {
...
static void test_sohm_fcpl(void) {
  ...
  verify_fcpl_values(fcpl_id, DEF_NUM_INDEXES, def_type_flags, def_minsizes,
                     DEF_L2B, DEF_B2L);
  ...
  verify_fcpl_values(fcpl2_id, DEF_NUM_INDEXES, def_type_flags, def_minsizes,
                     DEF_L2B, DEF_B2L);
  ...
  for (x = 0; x < TEST_NUM_INDEXES; ++x) {
    ret = H5Pset_shared_mesg_index(fcpl_id, x, test_type_flags[x],
                                   test_minsizes[x]);
  ...
  verify_fcpl_values(fcpl_id, TEST_NUM_INDEXES, test_type_flags, test_minsizes,
                     TEST_L2B, TEST_B2L);
  ...
  verify_fcpl_values(fcpl2_id, TEST_NUM_INDEXES, test_type_flags, test_minsizes,
                     TEST_L2B, TEST_B2L);
   ...
   * values.
   * Creating a file with uninitialized indexes should work. (TODO: not
   * implemented?)
   */
  ret = H5Pset_shared_mesg_nindexes(fcpl_id, H5O_SHMESG_MAX_NINDEXES);
  CHECK_I(ret, "H5Pset_shared_mesg_nindexes");
  ret = H5Pset_shared_mesg_phase_change(fcpl_id, H5O_SHMESG_MAX_LIST_SIZE,
...
static void test_sohm_fcpl_errors(void) {
  ...
  H5E_BEGIN_TRY {
    ...
    ret = H5Pset_shared_mesg_index(
        fcpl_id, 1, H5O_SHMESG_DTYPE_FLAG | H5O_SHMESG_FILL_FLAG, 15);
    CHECK_I(ret, "H5Pset_shared_mesg_index");
    fid = H5Fcreate(FILENAME, H5F_ACC_TRUNC, fcpl_id, H5P_DEFAULT);
    VERIFY(fid, -1, "H5Fcreate");
    ...
    ret = H5Pset_shared_mesg_phase_change(fcpl_id, H5O_SHMESG_MAX_LIST_SIZE + 1,
                                          0);
    VERIFY(ret, -1, "H5Pset_shared_mesg_phase_change");
    ret = H5Pset_shared_mesg_phase_change(fcpl_id, 10,
                                          H5O_SHMESG_MAX_LIST_SIZE + 10);
    VERIFY(ret, -1, "H5Pset_shared_mesg_phase_change");
    ret = H5Pset_shared_mesg_phase_change(fcpl_id, H5O_SHMESG_MAX_LIST_SIZE,
                                          H5O_SHMESG_MAX_LIST_SIZE + 1);
    VERIFY(ret, -1, "H5Pset_shared_mesg_phase_change");
  }
  H5E_END_TRY
 ...
 *
 * Return:      Success:        datatype ID (should be closed by calling
 *function) Failure:        negative
 *
 *-------------------------------------------------------------------------
 */
static hid_t make_dtype_1(void) {
  ...
  if (H5Tinsert(dtype1_id, "i1", HOFFSET(dtype1_struct, i1), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  ...
  if (H5Tinsert(dtype1_id, "string", HOFFSET(dtype1_struct, str), str_id) < 0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "i2", HOFFSET(dtype1_struct, i2), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "i3", HOFFSET(dtype1_struct, i3), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "i4", HOFFSET(dtype1_struct, i4), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "i5", HOFFSET(dtype1_struct, i5), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "i6", HOFFSET(dtype1_struct, i6), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "i7", HOFFSET(dtype1_struct, i7), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "i8", HOFFSET(dtype1_struct, i8), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(dtype1_id, "f1", HOFFSET(dtype1_struct, f1), H5T_NATIVE_FLOAT) <
      0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static hid_t size1_helper(hid_t file, const char *filename, hid_t fapl_id,
                          hbool_t test_file_closing) {
  dtype1_struct wdata;
  dtype1_struct rdata;
...
#define TSOHM_S1H_VERIFY_DATA(dset_id, dtype_id)                               \
  {                                                                            \
    ...
    if (0 > H5Dread((dset_id), (dtype_id), H5S_ALL, H5S_ALL, H5P_DEFAULT,      \
                    &rdata)) {                                                 \
      H5_FAILED();                                                             \
      AT();                                                                    \
  ...
  dset_id = H5Dcreate2(file, DSETNAME[0], dtype1_id, space_id, H5P_DEFAULT,
  ...
  for (x = 1; x < 4; x++) {
    dset_id = H5Dcreate2(file, DSETNAME[x], dtype1_id, space_id, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT);
    if (0 > dset_id)
      FAIL_STACK_ERROR;
    if (x == 3)
      if (H5Dwrite(dset_id, dtype1_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, &wdata) <
          0)
        TEST_ERROR;
    if (H5Dclose(dset_id) < 0)
      FAIL_STACK_ERROR;
  ...
  for (x = 0; x < SOHM_HELPER_NUM_EX_DSETS; x++) {
    dset_id = H5Dcreate2(file, EXTRA_DSETNAME[x], dtype1_id, space_id,
 ...
 *----------------------------------------------------------------------------
 */
static h5_stat_size_t getsize_testsize1(const char *filename, hid_t fcpl_id,
                                        hid_t fapl_id,
                                        hbool_t test_file_closing,
  ...
  ret = H5Oget_native_info_by_name(fid, DSETNAME[0], ninfo, H5O_NATIVE_INFO_HDR,
                                   H5P_DEFAULT);
...
static void test_sohm_size1(void) {
  ...
  for (use_shared = 0; use_shared < 2; use_shared++) {
    for (use_btree = 0; use_btree < 2; use_btree++) {
      ...
      file_sizes[size_index++] = getsize_testsize1(FILENAME, fcpl_id, fapl_id,
                                                   test_open_close, &ninfo);
...
#define TSOHM_SAH_OP_COUNT 3
  ...
  for (op_index = 0; op_index < TSOHM_SAH_OP_COUNT; op_index++) {
    ...
    group_id = H5Gcreate2(file_id, groupnames[op_index], H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
     ...
     */
    if (op_index == 1) {
      ret = H5Tcommit2(file_id, "datatype", type_id, H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
      CHECK_I(ret, "H5Tcommit2");
    }

    attr_id = H5Acreate2(group_id, "attribute", type_id, space_id, H5P_DEFAULT,
                         H5P_DEFAULT);
 ...
 *-------------------------------------------------------------------------
 */
static int size2_helper(hid_t fcpl_id, int test_file_closing,
  ...
  for (x = 0; x < NUM_DATASETS; ++x) {
    dset_id = H5Dcreate2(file_id, DSETNAME[x], dtype1_id, dspace1_id,
                         H5P_DEFAULT, dcpl1_id, H5P_DEFAULT);
    CHECK_I(dset_id, "H5Dcreate2");

    attr_id = H5Acreate2(dset_id, "attr_name", attr_type_id, attr_space_id,
                         H5P_DEFAULT, H5P_DEFAULT);
    CHECK_I(attr_id, "H5Acreate2");
  ...
  CHECK_I(file_id, "H5Fopen");
  group_id =
      H5Gcreate2(file_id, "group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK_I(group_id, "H5Gcreate2");

  /* Create NUM_DATASETS datasets in the new group */
  for (x = 0; x < NUM_DATASETS; ++x) {
    dset_id = H5Dcreate2(group_id, DSETNAME[x], dtype2_id, dspace2_id,
                         H5P_DEFAULT, dcpl2_id, H5P_DEFAULT);
    CHECK_I(dset_id, "H5Dcreate2");
  ...
  file_id = H5Fopen(FILENAME, H5F_ACC_RDWR, H5P_DEFAULT);
  CHECK_I(file_id, "H5Fopen");
  group_id = H5Gcreate2(file_id, "interleaved group", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT);
  CHECK_I(group_id, "H5Gcreate2");

  /* Create NUM_DATASETS datasets in the new group */
  for (x = 0; x < NUM_DATASETS; x += 2) {
    dset_id = H5Dcreate2(group_id, DSETNAME[x], dtype1_id, dspace1_id,
                         H5P_DEFAULT, dcpl1_id, H5P_DEFAULT);
    CHECK_I(dset_id, "H5Dcreate2");
    ...
    dset_id = H5Dcreate2(group_id, DSETNAME[x + 1], dtype2_id, dspace2_id,
                         H5P_DEFAULT, dcpl2_id, H5P_DEFAULT);
    CHECK_I(dset_id, "H5Dcreate2");
  ...
  for (x = 0; x < NUM_ATTRIBUTES; ++x) {
    ...
    attr_id = H5Acreate2(group_id, attr_name, attr_type_id, attr_space_id,
                         H5P_DEFAULT, H5P_DEFAULT);
    CHECK_I(attr_id, "H5Acreate2");
    ret = H5Awrite(attr_id, attr_type_id, attr_string1);
    CHECK_I(ret, "H5Awrite");
...
static void test_sohm_size2(int close_reopen) {
  ...
  size2_helper_struct share_some_toobig_index, share_tiny_index,
      type_space_index;
  herr_t ret;
  ...
  if (close_reopen == 0)
    ...
    MESSAGE(5, ("Testing that shared messages save space when file is closed "
                "and reopened\n"));
  ...
  CHECK_I(ret, "H5Pset_shared_mesg_nindexes");
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 0, H5O_SHMESG_SDSPACE_FLAG | H5O_SHMESG_DTYPE_FLAG, 20);
  CHECK_I(ret, "H5Pset_shared_mesg_index");
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 1, H5O_SHMESG_FILL_FLAG | H5O_SHMESG_PLINE_FLAG, 20);
  CHECK_I(ret, "H5Pset_shared_mesg_index");
  ...
  /* Edit the same property list (this should work) and don't share all
   * messages.
   */
  ret = H5Pset_shared_mesg_index(fcpl_id, 0, H5O_SHMESG_PLINE_FLAG, 20);
  CHECK_I(ret, "H5Pset_shared_mesg_index");
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 1, H5O_SHMESG_DTYPE_FLAG | H5O_SHMESG_FILL_FLAG, 100000);
  CHECK_I(ret, "H5Pset_shared_mesg_index");
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 2, H5O_SHMESG_ATTR_FLAG | H5O_SHMESG_SDSPACE_FLAG, 20);
  CHECK_I(ret, "H5Pset_shared_mesg_index");
  ...
  ret = H5Pset_shared_mesg_nindexes(fcpl_id, 1);
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 0, H5O_SHMESG_DTYPE_FLAG | H5O_SHMESG_SDSPACE_FLAG, 1);
  ret = H5Pset_shared_mesg_phase_change(fcpl_id, 1000, 900);
  ...
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 0, H5O_SHMESG_DTYPE_FLAG | H5O_SHMESG_SDSPACE_FLAG, 100);
  ret = H5Pset_shared_mesg_phase_change(fcpl_id, 1000, 900);
  ...
  if (list_index_med.empty_size != list_index_big.empty_size)
    VERIFY(list_index_med.empty_size, list_index_big.empty_size,
           "h5_get_file_size");
  if (list_index_med.empty_size != btree_index.empty_size)
    VERIFY(list_index_med.empty_size, btree_index.empty_size,
           "h5_get_file_size");
  if (list_index_med.empty_size != list_index_small.empty_size)
    VERIFY(list_index_med.empty_size, list_index_small.empty_size,
           "h5_get_file_size");
  /* The files with indexes shouldn't be that much bigger than an
   * empty file.
   */
  if (list_index_med.empty_size >
      (h5_stat_size_t)((float)norm_sizes.empty_size * OVERHEAD_ALLOWED))
  ...
  if (btree_index.dsets2 >
      (h5_stat_size_t)((float)list_index_small.dsets2 * OVERHEAD_ALLOWED))
  ...
  if (list_index_small.dsets2 >
      (h5_stat_size_t)((float)btree_index.dsets2 * OVERHEAD_ALLOWED *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  ...
  if (list_index_med.interleaved >= btree_index.interleaved)
    VERIFY(0, 1, "h5_get_file_size");
  if (btree_index.interleaved >
      (h5_stat_size_t)((float)list_index_small.interleaved * OVERHEAD_ALLOWED))
    VERIFY(btree_index.interleaved, list_index_small.interleaved,
           "h5_get_file_size");
   ...
   */
  if (btree_index.attrs1 >
      (h5_stat_size_t)((float)list_index_small.attrs1 * OVERHEAD_ALLOWED))
    VERIFY(btree_index.attrs1, list_index_small.attrs1, "h5_get_file_size");
  if (btree_index.attrs1 >
      (h5_stat_size_t)((float)list_index_med.attrs1 * OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  if (list_index_med.attrs1 >
      (h5_stat_size_t)((float)btree_index.attrs1 * OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  if (list_index_small.attrs1 >
      (h5_stat_size_t)((float)btree_index.attrs1 * OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  ...
  /* Give it some overhead (for checkin to move messages into continuation
   * message) */
  if ((list_index_small.attrs1 - list_index_small.interleaved) >
      (h5_stat_size_t)((float)(btree_index.attrs1 - btree_index.interleaved) *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
   ...
   */
  if (btree_index.attrs2 >
      (h5_stat_size_t)((float)list_index_small.attrs2 * OVERHEAD_ALLOWED))
    VERIFY(btree_index.attrs2, list_index_small.attrs2, "h5_get_file_size");
  if (list_index_small.attrs2 >
      (h5_stat_size_t)((float)btree_index.attrs2 * OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  if (btree_index.attrs2 >
      (h5_stat_size_t)((float)list_index_med.attrs2 * OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  if (list_index_med.attrs2 >
      (h5_stat_size_t)((float)btree_index.attrs2 * OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  ...
  if ((mult_index_med.dsets2 - mult_index_med.dsets1) >
      (h5_stat_size_t)((float)(list_index_med.dsets2 - list_index_med.dsets1) *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  if ((mult_index_btree.dsets2 - mult_index_btree.dsets1) >
      (h5_stat_size_t)((float)(btree_index.dsets2 - btree_index.dsets1) *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");

  if ((mult_index_med.interleaved - mult_index_med.dsets2) >
      (h5_stat_size_t)((float)(list_index_med.interleaved -
                               list_index_med.dsets2) *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  if ((mult_index_btree.interleaved - mult_index_btree.dsets2) >
      (h5_stat_size_t)((float)(btree_index.interleaved - btree_index.dsets2) *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  ...
  if ((mult_index_med.attrs2 - mult_index_med.attrs1) >
      (h5_stat_size_t)((float)(list_index_med.attrs2 - list_index_med.attrs1) *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  if ((mult_index_btree.attrs2 - mult_index_btree.attrs1) >
      (h5_stat_size_t)((float)(btree_index.attrs2 - btree_index.attrs1) *
                       OVERHEAD_ALLOWED))
    VERIFY(0, 1, "h5_get_file_size");
  ...
  if (mult_index_btree.attrs2 >= norm_sizes.attrs2)
    VERIFY(0, 1, "h5_get_file_size");
  if ((h5_stat_size_t)((float)mult_index_med.attrs2 * OVERHEAD_ALLOWED) <
      btree_index.attrs2)
    VERIFY(0, 1, "h5_get_file_size");
  if ((h5_stat_size_t)((float)mult_index_btree.attrs2 * OVERHEAD_ALLOWED) <
      btree_index.attrs2)
    VERIFY(0, 1, "h5_get_file_size");
  ...
  if (share_tiny_index.dsets1 >= type_space_index.dsets1)
    VERIFY(0, 1, "h5_get_file_size");
  if ((h5_stat_size_t)((float)share_tiny_index.dsets1 * OVERHEAD_ALLOWED) <
      type_space_index.dsets1)
    VERIFY(0, 1, "h5_get_file_size");

  if (share_tiny_index.dsets2 >= type_space_index.dsets2)
    VERIFY(0, 1, "h5_get_file_size");
  if ((h5_stat_size_t)((float)share_tiny_index.dsets2 * OVERHEAD_ALLOWED) <
  ...
  if (share_tiny_index.attrs1 >= type_space_index.attrs1)
    VERIFY(0, 1, "h5_get_file_size");
  if ((h5_stat_size_t)((float)share_tiny_index.attrs1 * OVERHEAD_ALLOWED) <
      type_space_index.attrs1)
    VERIFY(0, 1, "h5_get_file_size");

  if (share_tiny_index.attrs2 >= type_space_index.attrs2)
    VERIFY(0, 1, "h5_get_file_size");
  if ((h5_stat_size_t)((float)share_tiny_index.attrs2 * OVERHEAD_ALLOWED) <
 ...
 *-------------------------------------------------------------------------
 */
static void delete_helper_write(hid_t file_id, hid_t *dspace_id, hid_t *dcpl_id,
                                int x) {
  hid_t dset_id = -1;
  hid_t attr_id = -1;
  char wdata;
  ...
  dset_id = H5Dcreate2(file_id, DSETNAME[x], H5T_NATIVE_CHAR, dspace_id[x],
                       H5P_DEFAULT, dcpl_id[x], H5P_DEFAULT);
  CHECK_I(dset_id, "H5Dcreate2");
  wdata = (char)(x + 'a');
  ret = H5Dwrite(dset_id, H5T_NATIVE_CHAR, dspace_id[x], dspace_id[x],
                 H5P_DEFAULT, &wdata);
  CHECK_I(ret, "H5Dwrite");

  attr_id = H5Acreate2(dset_id, "attr_name", H5T_NATIVE_CHAR, dspace_id[x],
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK_I(attr_id, "H5Acreate2");
...
static void delete_helper_read(hid_t file_id, hid_t *dspace_id, int x) {
  ...
  CHECK_I(dset_id, "H5Dopen2");
  rdata = '\0';
  ret = H5Dread(dset_id, H5T_NATIVE_CHAR, dspace_id[x], dspace_id[x],
                H5P_DEFAULT, &rdata);
  CHECK_I(ret, "H5Dread");
...
static void delete_helper(hid_t fcpl_id, hid_t *dspace_id, hid_t *dcpl_id) {
  ...
  if (norm_filesize >
      (h5_stat_size_t)((float)deleted_filesize * OVERHEAD_ALLOWED))
    VERIFY(norm_filesize, deleted_filesize, "h5_get_file_size");
  if (deleted_filesize >
      (h5_stat_size_t)((float)norm_filesize * OVERHEAD_ALLOWED))
...
static void test_sohm_delete(void) {
  ...
  ret = H5Pset_shared_mesg_phase_change(fcpl_id, HALF_DELETE_NUM_MESGS,
                                        HALF_DELETE_NUM_MESGS - 1);
  CHECK_I(ret, "H5Pset_shared_mesg_phase_change");
  ...
  CHECK_I(ret, "H5Pset_shared_mesg_nindexes");
  ret = H5Pset_shared_mesg_index(
      fcpl_id, 0, H5O_SHMESG_SDSPACE_FLAG | H5O_SHMESG_ATTR_FLAG, 16);
  CHECK_I(ret, "H5Pset_shared_mesg_index");
  ...
  /* Set phase change values so that one index converts to a B-tree and one
   * doesn't */
...
static int verify_dset_create_and_delete_does_not_grow_file(hid_t fcpl_id) {
  ...
  dset_id = H5Dcreate2(file_id, "dset", H5T_NATIVE_SHORT, dspace_id,
                       H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK_I(dset_id, "H5Dcreate2");
  ...
  dset_id = H5Dcreate2(file_id, "dset2", H5T_NATIVE_SHORT, dspace_id,
                       H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK_I(dset_id, "H5Dcreate2");
  ret = H5Dclose(dset_id);
  CHECK_I(ret, "H5Dclose");
 ...
 */
static void
verify_dset_create_and_open_through_extlink_with_sohm(hid_t src_fcpl_id,
                                                      hid_t dst_fcpl_id) {
  hid_t src_file_id = -1;
  hid_t dst_file_id = -1;
  hid_t space_id = -1;
  ...
  /* Create files */
  src_file_id =
      H5Fcreate(FILENAME_SRC, H5F_ACC_TRUNC, src_fcpl_id, H5P_DEFAULT);
  CHECK_I(src_file_id, "H5Fcreate");
  dst_file_id =
      H5Fcreate(FILENAME_DST, H5F_ACC_TRUNC, dst_fcpl_id, H5P_DEFAULT);
  CHECK_I(dst_file_id, "H5Fcreate");

  /* Create an external link from the source file to the destination file */
  ret = H5Lcreate_external(FILENAME_DST, "/", src_file_id, "ext_link",
                           H5P_DEFAULT, H5P_DEFAULT);
  ...
  CHECK_I(space_id, "H5Screate_simple");
  dset_id = H5Dcreate2(src_file_id, "ext_link/dataset", H5T_NATIVE_FLOAT,
 ...
 *-------------------------------------------------------------------------
 */
static int verify_dataset_extension(hid_t fcpl_id, hbool_t close_reopen) {
...
#define TSOHM_VDE_CLOSE_REOPEN_FILE_AND_DSETS(n)                               \
  ...
  dset1_id = H5Dcreate2(file_id, "dataset", H5T_NATIVE_LONG, orig_space_id,
  ...
  dset2_id = H5Dcreate2(file_id, "dataset2", H5T_NATIVE_LONG, orig_space_id,
                        H5P_DEFAULT, dcpl_id, H5P_DEFAULT);
  ...
  dset3_id = H5Dcreate2(file_id, "dataset3", H5T_NATIVE_LONG, orig_space_id,
                        H5P_DEFAULT, dcpl_id, H5P_DEFAULT);
...
static void test_sohm_external_dtype(void) {
  ...
  for (x = 0; x < TEST_NUM_INDEXES; ++x) {
    ret =
        H5Pset_shared_mesg_index(fcpl, x, test_type_flags[x], test_minsizes[x]);
    CHECK_I(ret, "H5Pset_shared_mesg_index");
  }
  ...
  dataset1 = H5Dcreate2(file1, "dataset_1", s1_tid, space, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  ...
  dataset2 = H5Dcreate2(file2, "dataset_2", dset1_tid, space, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
