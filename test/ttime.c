...
static void test_time_commit(void) {
  ...
  status = H5Tcommit2(file_id, "Committed D32LE type", tid, H5P_DEFAULT,
  ...
  CHECK(tid, FAIL, "H5Tcopy");
  status = H5Tcommit2(file_id, "Committed D32BE type", tid, H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
  CHECK(status, FAIL, "H5Tcommit2");
  status = H5Tclose(tid);
  CHECK(status, FAIL, "H5Tclose");
  ...
  CHECK(tid, FAIL, "H5Tcopy");
  status = H5Tcommit2(file_id, "Committed D64LE type", tid, H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
  CHECK(status, FAIL, "H5Tcommit2");
  status = H5Tclose(tid);
  CHECK(status, FAIL, "H5Tclose");
  ...
  CHECK(tid, FAIL, "H5Tcopy");
  status = H5Tcommit2(file_id, "Committed D64BE type", tid, H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
  CHECK(status, FAIL, "H5Tcommit2");
  status = H5Tclose(tid);
  CHECK(status, FAIL, "H5Tclose");
...
static void test_time_io(void) {
  ...
  dsid = H5Dcreate2(fid, DATASETNAME, H5T_UNIX_D32LE, sid, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
  ...
  status =
      H5Dwrite(dsid, H5T_UNIX_D32LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &timenow);
  ...
  status =
      H5Dread(dsid, H5T_UNIX_D32LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &timethen);
