...
/* Tests for H5Pget/set_metadata_read_attempts(),
 * H5Fget_metadata_read_retry_info */
...
static int test_file_locking(hid_t in_fapl, hbool_t turn_locking_on,
                             hbool_t env_var_override);
...
/* Tests for multiple opens of files and datasets with H5Drefresh() &
 * H5Fstart_swmr_write(): same process */
static int test_multiple_same(hid_t in_fapl, hbool_t new_format);

/*
 * Tests for H5Pget/set_metadata_read_attemps(),
 * H5Fget_metadata_read_retry_info()
 */
...
static int test_metadata_read_attempts(hid_t in_fapl) {
  ...
  if (H5FDdriver_query(driver_id, &driver_flags) < 0)
    FAIL_STACK_ERROR;
  compat_w_default_vfd =
      (driver_flags & H5FD_FEAT_DEFAULT_VFD_COMPATIBLE) ? TRUE : FALSE;
  ...
  /* Set the # of read attempts to the default for non-SWMR access:
   * H5F_METADATA_READ_ATTEMPTS --should succeed */
  ...
  /* Set the # of read attempts to the default for SWMR access:
   * H5F_SWMR_METADATA_READ_ATEMPTS --should succeed */
  if (H5Pset_metadata_read_attempts(fapl, H5F_SWMR_METADATA_READ_ATTEMPTS) < 0)
    TEST_ERROR;

  /* Retrieve the # of read attempts -- should be
  ...
  /* Retrieve the # of read attempts from file's fapl -- should be
  ...
  /* Retrieve the # of read attempts from file fapl -- should be
   * H5F_METADATA_READ_ATTEMPTS */
  if (H5Pget_metadata_read_attempts(file_fapl, &attempts) < 0)
    FAIL_STACK_ERROR;
  if (attempts != H5F_METADATA_READ_ATTEMPTS)
  ...
  if ((fid2 = H5Fopen(filename, (H5F_ACC_RDONLY | H5F_ACC_SWMR_READ), fapl)) <
      0)
    FAIL_STACK_ERROR;
  ...
  /* Retrieve the # of read attempts from file fapl -- should be
   * H5F_SWMR_METADATA_READ_ATTEMPTS, not 9 */
  if (H5Pget_metadata_read_attempts(file_fapl, &attempts) < 0)
    FAIL_STACK_ERROR;
  if (attempts != H5F_SWMR_METADATA_READ_ATTEMPTS)
    TEST_ERROR;
...
static int test_metadata_read_retry_info(hid_t in_fapl) {
  ...
  hsize_t max_dims_1un[2] = {H5S_UNLIMITED,
                             H5S_UNLIMITED}; /* Dataset maximum dimensions */
  hsize_t max_dims_2un[2] = {500,
                             H5S_UNLIMITED}; /* Dataset maximum dimensions */
  ...
  /* Create a chunked dataset with 1 unlimited dimension: extensible array
  ...
  if (H5Pset_chunk(dcpl, 2, chunk_dims) < 0)
    FAIL_STACK_ERROR;
  if ((did1 = H5Dcreate2(fid, "DSET_1UNLIM", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Create a chunked dataset with 2 unlimited dimension: v2 Btree indexing will
   * be used */
  ...
  if (H5Pset_chunk(dcpl, 2, chunk_dims) < 0)
    FAIL_STACK_ERROR;
  if ((did2 = H5Dcreate2(fid, "DSET_2UNLIM", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Open the file with SWMR access */
  if ((fid = H5Fopen(filename, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, new_fapl)) <
      0)
    FAIL_STACK_ERROR;
  ...
  /*
   * Open a file with SWMR access, default # of read attempts:
   *   --increment retries for metadata item: fixed array data block page
   * (retries[19][0]) H5Freopen the same file:
   *   --increment retries for metadata item: free-space sections
   * (retries[9][1])--
   */
  /* Open the file with SWMR access */
  if ((fid = H5Fopen(filename, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, fapl)) < 0)
 ...
 *      Create a chunked dataset "dataset1" in the file -- should be using
 * latest chunk indexing Should fail to enable SWMR as the file is already in
 * SWMR writing mode Close the file
 *
 *  (B) Opening a file
 ...
 *      Create a chunked dataset "dataset2" in the group--should be using latest
 * chunk indexing--keep it open Should succeed in enabling SWMR Should succeed
 * in reading from multiple opens of "dataset1" Close multiple opens of
 * "dataset1" Close "dataset2" Create "dataset3"--should be using latest chunk
 * indexing Close "dataset3" Close the group and file
 */
static int test_start_swmr_write(hid_t in_fapl, hbool_t new_format) {
  hid_t fid = -1;                        /* File ID */
  ...
  hsize_t max_dim2[2] = {H5S_UNLIMITED,
  ...
  if (new_format) {
    ...
    TESTING("H5Fstart_swmr_write() when creating/opening a file without latest "
            "format");
  } /* end if */
  ...
  /* Should be 100 */
  if (attempts != (unsigned int)(new_format ? H5F_METADATA_READ_ATTEMPTS
                                            : H5F_SWMR_METADATA_READ_ATTEMPTS))
    TEST_ERROR;
  ...
  if ((sid1 = H5Screate_simple(1, dim, max_dim)) < 0)
    FAIL_STACK_ERROR;
  if ((did1 = H5Dcreate2(fid, "dataset1", H5T_NATIVE_INT, sid1, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((sid2 = H5Screate_simple(2, dim2, max_dim2)) < 0)
    FAIL_STACK_ERROR;
  if ((did2 = H5Dcreate2(gid, "dataset2", H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dread(did1_b, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &rdata) <
      0)
    FAIL_STACK_ERROR;
  if (wdata != rdata)
    FAIL_STACK_ERROR;
  ...
  if (H5Dread(did1_a, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &rdata) <
      0)
    FAIL_STACK_ERROR;
  if (wdata != rdata)
    FAIL_STACK_ERROR;
  ...
  if ((sid3 = H5Screate_simple(2, dim2, max_dim2)) < 0)
    FAIL_STACK_ERROR;
  if ((did3 = H5Dcreate2(fid, "dataset3", H5T_NATIVE_INT, sid3, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
 ...
 *
 *  Verify failure conditions in enabling SWMR writing mode via
 H5Fstart_swmr_write():
 ...
 *      If (latest-format):
 *      (2a) Create a file with write + latest format and with opened named
 datatype
 *      --fail to enable SWMR because there are opened datatype
 *      If !(latest-format):
 *      (2b) Create a file with write + non-latest-format
 *              --fail to enable SWMR because superblock version is not at least
 3
 *
 ...
 *      --fail to enable SWMR because the file is not opened with write
 *      (4) Open the file with write + with/without latest format and with
 opened named datatype/attribute
 *      --fail to enable SWMR because there are opened datatype/attribute
 *
 *  (C) When doing multiple opens for a file:
 *      Create a file with (a) write + latest format or (b) SMWR write +
 non-latest-format
 *      Close the file
 ...
 *      --Close the file
 *
 *  (D) (!new_format): When opening a file which is created with write +
 non-latest-format:
...
static int test_err_start_swmr_write(hid_t in_fapl, hbool_t new_format) {
  ...
  fid = H5Fcreate(filename, H5F_ACC_TRUNC | H5F_ACC_SWMR_WRITE, H5P_DEFAULT,
                  fapl);

  /* Should fail to enable SWMR writing when the file is already in SWMR writing
   * mode */
  H5E_BEGIN_TRY { ret = H5Fstart_swmr_write(fid); }
  H5E_END_TRY
  if (ret >= 0)
  ...
  if (new_format) {
    ...
    /* Should fail to enable SWMR writing because the file's superblock version
     * is not at least 3 */
    H5E_BEGIN_TRY { ret = H5Fstart_swmr_write(fid); }
    H5E_END_TRY
    if (ret >= 0)
  ...
  /* Should fail to enable SWMR writing when the file is opened with SWMR read
   * access only */
  H5E_BEGIN_TRY { ret = H5Fstart_swmr_write(fid); }
  H5E_END_TRY
  if (ret >= 0)
  ...
  if ((aid = H5Acreate2(tid, "attr", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Should fail to enable SWMR writing when there are opened named datatype and
   * attribute */
  H5E_BEGIN_TRY { ret = H5Fstart_swmr_write(fid); }
  H5E_END_TRY
  if (ret >= 0)
  ...
  if (!new_format) {
    ...
    /* Should fail to open the file with SWMR write + latest format due to
     * superblock version not at least
     * 3 */
    H5E_BEGIN_TRY {
    ...
    /* Should fail to open the file with SWMR write + non-latest-format due to
     * superblock version not at least 3 */
    H5E_BEGIN_TRY {
    ...
    /* Should fail to enable SWMR writing due to superblock version not at least
     * 3 */
    H5E_BEGIN_TRY { ret = H5Fstart_swmr_write(fid); }
    H5E_END_TRY
    if (ret >= 0)
 ...
 *
 * The "new_format" parameter indicates whether to create the file with latest
 * format or not. To have SWMR support, can use either one of the following in
 * creating a file: (a) Create the file with write + latest format:
 ...
 *      Child: concurrent open of the file with write and SWMR write access
 * (fail)
 */
#if !(defined(H5_HAVE_FORK) && defined(H5_HAVE_WAITPID))

static int test_start_swmr_write_concur(hid_t H5_ATTR_UNUSED in_fapl,
                                        hbool_t new_format) {
  if (new_format) {
...
static int test_start_swmr_write_concur(hid_t in_fapl, hbool_t new_format) {
  ...
  if (H5Pset_chunk(dcpl, 1, chunk_dims) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dcreate2(fid, "dataset", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (childpid == 0) {                      /* Child process */
    ...
    if ((child_fid1 =
             H5Fopen(filename, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, fapl)) < 0)
      exit(EXIT_FAILURE);
    if ((child_fid2 =
             H5Fopen(filename, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, fapl)) < 0)
      exit(EXIT_FAILURE);
    ...
    rdata = 0;
    if (H5Dread(child_did1, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                &rdata) < 0)
      exit(EXIT_FAILURE);
    if (rdata != 88)
      exit(EXIT_FAILURE);
    ...
    rdata = 0;
    if (H5Dread(child_did2, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                &rdata) < 0)
      exit(EXIT_FAILURE);
    if (rdata != 99)
      exit(EXIT_FAILURE);
...
static int test_start_swmr_write_stress_ohdr(hid_t in_fapl) {
  ...
  if (H5Pset_fill_value(dcpl, tid, &fill) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dcreate2(fid, "dataset", tid, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Pset_fill_value(dcpl, tid, &fill) < 0)
    FAIL_STACK_ERROR;
  if ((did2 = H5Dcreate2(fid, "dataset2", tid, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create attribute on original dataset, to push dataspace header message out
   * of header chunk #0 */
  if ((sid = H5Screate(H5S_SCALAR)) < 0)
    FAIL_STACK_ERROR;
  if ((tid = H5Tcopy(H5T_C_S1)) < 0)
 ...
 *
 */
static herr_t dummy_append_flush_cb(hid_t H5_ATTR_UNUSED dataset_id,
                                    hsize_t H5_ATTR_UNUSED *cur_dims,
                                    void H5_ATTR_UNUSED *user_data) {
  return SUCCEED;
}

static herr_t tssw_persist_dapl_verify(
    hid_t did, hid_t vdsid1, hid_t vdsid2, hsize_t boundary,
    H5D_append_cb_t append_func, void *append_func_ud, size_t rdcc_nslots,
    size_t rdcc_nbytes, double rdcc_w0, const char *efile_prefix,
  ...
  if (H5Pget_append_flush(dapl, 1, &boundary_out, &append_func_out,
                          &append_func_ud_out) < 0)
    TEST_ERROR;
  ...
  if (H5Pget_chunk_cache(dapl, &rdcc_nslots_out, &rdcc_nbytes_out,
                         &rdcc_w0_out) < 0)
    TEST_ERROR;
  ...
  if (H5Pget_virtual_prefix(vds_dapl1, virtual_prefix_out,
                            sizeof(virtual_prefix_out)) < 0)
    TEST_ERROR;
...
static int test_start_swmr_write_persist_dapl(hid_t in_fapl) {
  ...
  boundary = 23;
  if (H5Pset_append_flush(dapl1, 1, &boundary, dummy_append_flush_cb,
                          append_func_ud) < 0)
    TEST_ERROR;
  ...
  if ((sid = H5Screate_simple(1, dim, max_dim)) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(fid, "dataset1", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        dapl1)) < 0)
    TEST_ERROR;
  ...
  if (H5Pset_layout(vds_dcpl, H5D_VIRTUAL) < 0)
    TEST_ERROR;
  if ((vdsid1 = H5Dcreate2(fid, "vds1", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                           vds_dcpl, dapl1)) < 0)
    TEST_ERROR;

  /* Create "vds2" */
  if ((vdsid2 = H5Dcreate2(fid, "vds2", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                           vds_dcpl, dapl2)) < 0)
    TEST_ERROR;
  ...
  if (tssw_persist_dapl_verify(did, vdsid1, vdsid2, boundary, append_func,
                               append_func_ud, rdcc_nslots, rdcc_nbytes,
                               rdcc_w0, efile_prefix, virtual_prefix, gap_size,
                               virtual_view) < 0)
    TEST_ERROR;
  ...
  /* Create "dataset2" */
  if ((did = H5Dcreate2(fid, "dataset2", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        dapl1)) < 0)
    TEST_ERROR;

  /* Create "vds3" */
  if ((vdsid1 = H5Dcreate2(fid, "vds3", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                           vds_dcpl, dapl1)) < 0)
    TEST_ERROR;

  /* Create "vds4" */
  if ((vdsid2 = H5Dcreate2(fid, "vds4", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                           vds_dcpl, dapl2)) < 0)
    TEST_ERROR;
...
static int test_object_flush_cb(hid_t in_fapl) {
  ...
  H5F_flush_cb_t
      ret_cb;   /* The callback function set in object flush property */
   ...
   */
  /* Should fail if the callback function is not defined but user data is
   * defined */
  ...
  /* Retrieve object flush property values for the default file access property
   * list */
  ...
  /* Retrieve object flush property values for the non-default file access
   * property list */
  if (H5Pget_object_flush_cb(fapl, &ret_cb, &ret_ct) < 0)
    TEST_ERROR;
  ...
  /* Create a dataset */
  if ((did2 = H5Dcreate2(gid, "dataset2", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static herr_t append_cb(hid_t H5_ATTR_UNUSED dset_id,
                        hsize_t H5_ATTR_UNUSED *cur_dims, void *_udata) {
...
/* The callback function for append flush property */
static herr_t append_cb2(hid_t H5_ATTR_UNUSED dset_id,
                         hsize_t H5_ATTR_UNUSED *cur_dims, void *_udata) {
  unsigned *count = (unsigned *)_udata;
  ++(*count++);
  return 0;
 ...
 *      -- the # of boundary sizes retrieved does not exceed MIN(input ndims,
 * the ndims set)
 */
static int test_append_flush_generic(void) {
  ...
  H5D_append_cb_t
      ret_cb;          /* The callback function set in append flush property */
  unsigned *ret_count; /* The user data set in append flush property */
  herr_t ret;          /* The return value */

  TESTING(
      "H5Fget/set_append_flush() for a generic dataset access property list");
  ...
  if (H5Pget_append_flush(dapl, 2, ret_boundary, &ret_cb, (void **)&ret_count) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Pget_append_flush(dapl, 3, ret_boundary, &ret_cb, (void **)&ret_count) <
      0)
    TEST_ERROR;
 ...
 *     -- the rank set in append flush property is not the same as the dataset's
 * rank
 *     -- boundary (non-zero) is set for a non-extendible dimension
 *      3) To verify the append flush property values retrieved from a
 * non-default access property list:
 *     -- the set callback function, the set user data
 *     -- the # of boundary sizes retrieved does not exceed MIN(input ndims, the
 * ndims set)
 */
static int test_append_flush_dataset_chunked(hid_t in_fapl) {
  hid_t fid = -1;             /* file ID */
  ...
  H5D_append_cb_t
      ret_cb; /* The callback function set in the append flush property */
  ...
  hsize_t maxdims[2] = {
      100, H5S_UNLIMITED};        /* The dataset maximum dimension sizes */
  hsize_t chunk_dims[2] = {5, 2}; /* The chunk dimesion sizes */

  TESTING(
      "H5Fget/set_append_flush() for a chunked dataset's access property list");

  /*
   *  Case (1)--
   *  For a chunked dataset's access property list:
   *  --to verify the append flush property values retrieved from a default
   * access a default access property list is: zero rank, zero boundary, null
   * callback function, null user data
   */
  ...
  /* Retrieve the append flush property values */
  if (H5Pget_append_flush(ddapl, 3, ret_boundary, &ret_cb,
                          (void **)&ret_count) < 0)
    TEST_ERROR;
   ...
   *      --the rank set in append flush property is not the same as the
   * dataset's rank
   *      -- boundary (non-zero) is set for a non-extendible dimension
  ...
  H5E_BEGIN_TRY {
    did2 = H5Dcreate2(fid, "dataset2", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                      dapl);
  }
  H5E_END_TRY
  if (did2 >= 0)
 ...
 *  (1) To verify success in creating the dataset--whatever is set for the
 * append flush property setting (2) To verify that default append flush
 * property values are retrieved for both default or non-default access property
 * list:
 *      -- zero boundary, null callback function, null user data
 */
static int test_append_flush_dataset_fixed(hid_t in_fapl) {
   ...
   *  Case (2)--
   *  For a non-chunked dataset's access property list:
   *  --to verify success in creating and opening the dataset even when append
   * flush property is setup with error conditions:
   *      --the rank set in append flush property is not the same as the
   * dataset's rank
   *      --boundary is set
  ...
  /* Retrieve the append flush property values */
  if (H5Pget_append_flush(ddapl, 1, ret_boundary, &ret_cb,
                          (void **)&ret_count) < 0)
    TEST_ERROR;
 ...
 *      -- should return append flush property values set in dapl1
 *  NOTE:
 *    FOR NOW: return the append flush property values of the create or the very
 * first open LATER ON: should REJECT subsequent dataset open if append flush
...
static int test_append_flush_dataset_multiple(hid_t in_fapl) {
  ...
  hsize_t maxdims[2] = {
      H5S_UNLIMITED, H5S_UNLIMITED}; /* The dataset maximum dimension sizes */
  hsize_t chunk_dims[2] = {5, 2};    /* The chunk dimesion sizes */
   ...
   *      -- should return append flush property values set in dapl1
   */
  if ((did1 = H5Dcreate2(fid, "dataset2", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Dclose(did1) < 0)
    FAIL_STACK_ERROR;
...
static int test_file_lock_same(hid_t in_fapl) {
  ...
  /* Output message about test being performed */
  TESTING(
      "File open with different combinations of flags--single process access");
...
static int test_file_lock_swmr_same(hid_t in_fapl) {
  ...
  /* Output message about test being performed */
  TESTING("File open with different combinations of flags + SWMR flags--single "
          "process access");
  ...
  H5E_BEGIN_TRY {
    fid = H5Fopen(filename,
                  H5F_ACC_RDWR | H5F_ACC_SWMR_WRITE | H5F_ACC_SWMR_READ, fapl);
  }
  H5E_END_TRY
  if (fid >= 0)
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    fid =
        H5Fopen(filename,
                H5F_ACC_RDONLY | H5F_ACC_SWMR_WRITE | H5F_ACC_SWMR_READ, fapl);
  }
  H5E_END_TRY
  if (fid >= 0)
    TEST_ERROR;
...
#endif /* #if !(defined(H5_HAVE_FORK) && defined(H5_HAVE_WAITPID) &&           \
          defined(H5_HAVE_FLOCK)) */
...
static int test_file_lock_swmr_concur(hid_t H5_ATTR_UNUSED in_fapl) {
  /* Output message about test being performed */
  TESTING("File open with different combintations of flags + SWMR "
...
static int test_file_locking(hid_t in_fapl, hbool_t turn_locking_on,
  ...
  if (env_var_override) {
    if (HDsetenv(HDF5_USE_FILE_LOCKING, turn_locking_on ? "FALSE" : "TRUE",
                 TRUE) < 0)
      TEST_ERROR;
...
static int test_different_lock_flags(hid_t in_fapl) {
  ...
  if (H5I_INVALID_HID != fid3)
    FAIL_PUTS_ERROR("Should not have been able to open a file with different "
                    "locking flags");
...
static int test_swmr_vfd_flag(void) {
  ...
  hid_t bad_fapl =
      -1; /* fapl ID of a VFD that does not support SWMR writes (stdio) */
  char filename[NAME_BUF_SIZE]; /* file name */
...
#ifdef OUT
/*
 * This exposes a bug for H5Orefresh while handling opened objects for
 * H5Fstart_swmr_write(). The boolean to skip file truncation test when reading
 * in superblock will fix the problem. Will work to move that to
 * test/flushrefresh.c later.
 */
static int test_bug_refresh(hid_t in_fapl) {
  ...
  if ((gid1 = H5Gcreate2(fid, "group1", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid2 = H5Gcreate2(fid, "group2", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid3 = H5Gcreate2(fid, "group3", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid4 = H5Gcreate2(fid, "group4", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid5 = H5Gcreate2(fid, "group5", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid6 = H5Gcreate2(fid, "group6", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid7 = H5Gcreate2(fid, "group7", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid8 = H5Gcreate2(fid, "group8", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid9 = H5Gcreate2(fid, "group9", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static int test_refresh_concur(hid_t H5_ATTR_UNUSED in_fapl,
                               hbool_t new_format) {
  if (new_format) {
 ...
 *      --result in v3 superblock with latest chunk indexing types
 *
 * Verify that H5Drefresh() and H5Fstart_swmr_write() work properly with
 * multiple opens of files and datasets.
 */
static int test_multiple_same(hid_t in_fapl, hbool_t new_format) {
...
int main(void) {
  ...
  hbool_t file_locking_enabled =
      FALSE; /* Checks if the file system supports locks */
  ...
  /* Tests on H5Pget/set_metadata_read_attempts() and
   * H5Fget_metadata_read_retry_info() */
   ...
   *   H5Fcreate(write, latest format) or  H5Fcreate(SWMR write,
   * non-latest-format)
