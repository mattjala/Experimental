...
static void test_vlstrings_basic(void) {
  ...
  dataset2 = H5Dcreate2(fid1, "Dataset2", tid1, dataspace, H5P_DEFAULT,
  ...
  ret = H5Pset_vlen_mem_manager(xfer_pid, test_vlstr_alloc_custom, &mem_used,
                                test_vlstr_free_custom, &mem_used);
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    if (HDstrlen(wdata[i]) != HDstrlen(rdata[i])) {
      TestErrPrintf("VL data length don't match!, strlen(wdata[%d])=%d, "
                    "strlen(rdata[%d])=%d\n",
                    (int)i, (int)HDstrlen(wdata[i]), (int)i,
...
static void test_vlstrings_special(void) {
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    ...
    if ((wdata[i] == NULL && rdata[i] != NULL) ||
        (rdata[i] == NULL && wdata[i] != NULL)) {
  ...
  dataset =
      H5Dcreate2(fid1, "Dataset4", tid1, sid1, H5P_DEFAULT, dcpl, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
...
static void test_vlstring_type(void) {
  ...
  ret = H5Tcommit2(fid, VLSTR_TYPE, tid_vlstr, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
...
static void test_compact_vlstring(void) {
  ...
  /* Create a dataset */
  dataset =
      H5Dcreate2(fid1, "Dataset5", tid1, sid1, H5P_DEFAULT, plist, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
...
static void test_write_vl_string_attribute(void) {
  ...
  att = H5Acreate2(root, "test_scalar", type, dataspace, H5P_DEFAULT,
                   H5P_DEFAULT);
  ...
  if (HDstrcmp(string_att_check, string_att) != 0)
    TestErrPrintf("VL string attributes don't match!, string_att=%s, "
                  "string_att_check=%s\n",
  ...
  att = H5Acreate2(root, "test_scalar_large", type, dataspace, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(att, FAIL, "H5Acreate2");
...
static void write_scalar_dset(hid_t file, hid_t type, hid_t space, char *name,
                              char *data) {
  hid_t dset;
  herr_t ret;
...
/* Helper routine for test_vl_rewrite() */
static void read_scalar_dset(hid_t file, hid_t type, hid_t space, char *name,
                             char *data) {
  hid_t dset;
  herr_t ret;
  char *data_read;
...
static void test_write_same_element(void) {
  ...
  dataset1 = H5Dcreate2(file1, DATASET, dtype, fspace, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT);
  ...
  /*
   * Open the file.  Select the same points, write values to those point
   * locations.
   */
  ...
  ret =
      H5Sselect_elements(fspace, H5S_SELECT_SET, NUMP, (const hsize_t *)&coord);
