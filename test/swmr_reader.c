...
static int read_records(const char *filename, hbool_t verbose,
                        FILE *verbose_file, unsigned random_seed,
                        unsigned long nseconds, unsigned poll_time,
 ...
 */
static int check_dataset(hid_t fid, hbool_t verbose, FILE *verbose_file,
                         const char *sym_name, symbol_t *record,
                         hid_t rec_sid) {
    ...
    if (record->rec_id != start[1]) {
      ...
      if (verbose) {
        ...
        fprintf(verbose_file,
                "Time = %llu.%llu, Symbol = '%s', # of records = %lld, "
                "record->rec_id = %llu\n",
...
static int read_records(const char *filename, hbool_t verbose,
  ...
  if (ncommon > 0) {
    ...
    if (NULL ==
        (sym_com = (symbol_info_t **)malloc(sizeof(symbol_info_t *) * ncommon)))
      return -1;
  ...
  if (nrandom > 0) {
    ...
    if (NULL == (sym_rand = (symbol_info_t **)malloc(sizeof(symbol_info_t *) *
                                                     nrandom)))
      return -1;
  ...
  if (use_log_vfd) {
    ...
    HDsnprintf(verbose_name, sizeof(verbose_name), "swmr_reader.log.%u",
               random_seed);

    H5Pset_fapl_log(fapl, verbose_name, H5FD_LOG_ALL,
                    (size_t)(512 * 1024 * 1024));
  } /* end if */

  /* Loop over reading records until [at least] the correct # of seconds have
   * passed */
  while (curr_time < (time_t)(start_time + (time_t)nseconds)) {
    ...
    if (ncommon > 0) {
      ...
      for (v = 0; v < ncommon; v++) {
        ...
        if (check_dataset(fid, verbose, verbose_file, sym_com[v]->name, &record,
                          mem_sid) < 0)
          return -1;
        memset(&record, 0, sizeof(record));
      } /* end for */
    ...
    if (nrandom > 0) {
      ...
      for (v = 0; v < nrandom; v++) {
        ...
        if (check_dataset(fid, verbose, verbose_file, sym_rand[v]->name,
                          &record, mem_sid) < 0)
          return -1;
        memset(&record, 0, sizeof(record));
      } /* end for */
...
static void usage(void) {
  ...
  printf("\n");
  printf(
      "Usage: swmr_reader [-q] [-s <# of seconds to sleep between polling>]\n");
  printf("    [-h <# of common symbols to poll>] [-l <# of random symbols to "
         "poll>]\n");
  printf("    [-r <random seed>] <# of seconds to test>\n");
  printf("\n");
  printf("<# of seconds to test> must be specified.\n");
  printf("\n");
  printf("Defaults to verbose (no '-q' given), 1 second between polling ('-s "
         "1'),\n");
  printf("5 common symbols to poll ('-h 5'), 10 random symbols to poll ('-l "
         "10'),\n");
...
int main(int argc, char *argv[]) {
  ...
  if (verbose) {
    ...
    HDsnprintf(verbose_name, sizeof(verbose_name), "swmr_reader.out.%u",
               random_seed);
  ...
  if (read_records(FILENAME, verbose, verbose_file, random_seed,
                   (unsigned long)nseconds, (unsigned)poll_time,
