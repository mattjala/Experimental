...
static herr_t test_atomic_dtype(hid_t file) {
  ...
  if (H5Tconvert(native_type, H5T_NATIVE_INT, (DIM0 * DIM1), icheck2, NULL,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
...
#error "Unknown 'long long' size"
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_COMPOUND_NAME_2, tid, space, H5P_DEFAULT,
...
static herr_t test_compound_dtype(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_COMPOUND_NAME, tid, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_compound_dtype3(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_COMPOUND_NAME_3, tid, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_compound_opaque(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_COMPOUND_NAME_4, tid, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_enum_dtype(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_ENUM_NAME, tid, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tconvert(native_type, tid_m, (DIM0 * DIM1), scheck2, NULL,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static herr_t test_array_dtype(hid_t file) {
  ...
  hid_t dataset = -1, space = -1;
  hid_t dtype = -1, native_type = -1, tid = -1, tid2 = -1, tid3 = -1,
        tid_m = -1;
  int i, j, k, n;
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_ARRAY_NAME, tid, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tconvert(native_type, tid_m, (DIM0 * DIM1), check, NULL, H5P_DEFAULT) <
      0)
    TEST_ERROR;
...
static herr_t test_array_dtype2(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_ARRAY2_NAME, tid, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tconvert(native_type, tid_m, (DIM0 * DIM1), icheck3, NULL,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static herr_t test_vl_dtype(hid_t file) {
  ...
  hid_t dataset = -1, space = -1;
  hid_t dtype = -1, native_type = -1, nat_super_type = -1, tid = -1, tid2 = -1,
        tid_m = -1, tid_m2 = -1;
  size_t i, j, k;
  void **tmp = NULL;
  ...
  /* Create a dataset */
  if ((dataset = H5Dcreate2(file, DSET_VL_NAME, tid, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Also get native base type for this nested VL type.  Should be an integer
   * type. */
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    if (wdata[i].len != rdata[i].len) {
      H5_FAILED();
      printf("    VL data length don't match!, wdata[%d].len=%d, "
             "rdata[%d].len=%d\n",
    ...
    for (t1 = (hvl_t *)wdata[i].p, t2 = (hvl_t *)rdata[i].p, j = 0;
      ...
      if (H5Tconvert(nat_super_type, H5T_NATIVE_UINT, t2->len, tmp, NULL,
                     H5P_DEFAULT) < 0)
        TEST_ERROR;
      ...
      for (k = 0; k < t2->len; k++) {
        if (((unsigned int *)t1->p)[k] != ((unsigned int *)tmp)[k]) {
          ...
          printf("    VL data don't match!, wdata[%u].p=%d, rdata[%u].p=%u\n",
                 (unsigned)i, ((unsigned int *)t1->p)[k], (unsigned)i,
                 ((unsigned int *)tmp)[k]);
          goto error;
        } /* end if */
      }   /* end for */
...
static herr_t test_vlstr_dtype(hid_t file) {
  ...
  /* Create a dataset */
  if ((dataset = H5Dcreate2(file, DSET_VLSTR_NAME, tid1, sid1, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    if (HDstrlen(wdata[i]) != HDstrlen(rdata[i])) {
      H5_FAILED();
      printf("    VL data length don't match!, strlen(wdata[%d])=%d, "
             "strlen(rdata[%d])=%d\n",
...
static herr_t test_str_dtype(hid_t file) {
  const char wdata[SPACE1_DIM1][4] = {"one", "two", "3rd",
                                      "4th"};    /* Information to write */
  ...
  /* Create a dataset */
  if ((dataset = H5Dcreate2(file, DSET_STR_NAME, tid1, sid1, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    if (HDstrlen(wdata[i]) != HDstrlen(rdata[i])) {
      H5_FAILED();
      printf("    data length don't match!, strlen(wdata[%d])=%d, "
             "strlen(rdata[%d])=%d\n",
             (int)i, (int)HDstrlen(wdata[i]), (int)i, (int)HDstrlen(rdata[i]));
      goto error;
    } /* end if */
...
static herr_t test_refer_dtype(hid_t file) {
  ...
  /* Create a group */
  if ((group = H5Gcreate2(file, "Group1", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create a dataset */
  if ((dataset = H5Dcreate2(file, "Dataset3", H5T_STD_REF_OBJ, sid1,
                            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_refer_dtype2(hid_t file) {
  ...
  /* Write selection to disk */
  if (H5Dwrite(dset1, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               &wbuf) < 0)
    TEST_ERROR;
  ...
  /* Read selection from disk */
  if (H5Dread(dset1, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              &rbuf) < 0)
    TEST_ERROR;

  /* Try to open objects */
  if ((dset2 = H5Rdereference2(dset1, H5P_DEFAULT, H5R_DATASET_REGION, &rbuf)) <
      0)
    TEST_ERROR;
...
static herr_t test_opaque_dtype(hid_t file) {
  ...
  if ((space = H5Screate_simple(1, &nelmts, NULL)) < 0)
    TEST_ERROR;
  if ((dset = H5Dcreate2(file, DSET_OPAQUE_NAME, type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_bitfield_dtype(hid_t file) {
  ...
  if ((dset1 = H5Dcreate2(file, DSET1_BITFIELD_NAME, type, space, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((dset2 = H5Dcreate2(file, DSET2_BITFIELD_NAME, type, space, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_ninteger(void) {
  ...
  if ((fid1 = H5Fcreate("tstint1.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* create a dataset */
  if ((did1 = H5Dcreate2(fid1, "dset", H5T_NATIVE_INT, sid1, H5P_DEFAULT, dcpl1,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* create a file using default properties */
  if ((fid2 = H5Fcreate("tstint2.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* create a dataset using the native type */
  if ((did2 = H5Dcreate2(fid2, "dset", nid1, sid1, H5P_DEFAULT, dcpl1,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
