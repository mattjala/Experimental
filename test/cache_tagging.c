...
static unsigned check_group_creation_tags(void) {
  ...
  if ((gid = H5Gcreate2(fid, GROUPNAME, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static unsigned check_multi_group_creation_tags(void) {
  ...
  for (i = 0; i < MULTIGROUPS; i++) {
    ...
    if ((gid = H5Gcreate2(fid, gname, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
        0)
      TEST_ERROR;
    if (H5Gclose(gid) < 0)
      TEST_ERROR;
...
static unsigned check_link_iteration_tags(void) {
  ...
  for (i = 0; i < 500; i++) {
    ...
    if ((did = H5Dcreate2(fid, dsetname, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
...
static unsigned check_dense_attribute_tags(void) {
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DATASETNAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Pclose(dcpl) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < 50; i++) {
    ...
    if ((aid = H5Acreate2(did, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      TEST_ERROR;
  ...
  if ((aid = H5Aopen_by_idx(did, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)4,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static unsigned check_attribute_creation_tags(hid_t fcpl, int type) {
  ...
  if ((aid = H5Acreate2(gid, ATTRNAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static unsigned check_dataset_creation_tags(hid_t fcpl, int type) {
  ...
  /* Create Dataset */
  if ((did = H5Dcreate2(fid, DATASETNAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Pclose(dcpl) < 0)
    TEST_ERROR;
...
static unsigned check_dataset_creation_earlyalloc_tags(hid_t fcpl, int type) {
  ...
  if ((did = H5Dcreate2(fid, DATASETNAME2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Pclose(dcpl) < 0)
    TEST_ERROR;
...
static unsigned check_object_info_tags(void) {
   ...
   */
  if (H5Oget_native_info_by_name(fid, GROUPNAME, &ninfo, H5O_NATIVE_INFO_ALL,
                                 H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static unsigned check_link_getname_tags(void) {
  ...
  H5Lget_name_by_idx(fid, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)1, name,
                     (size_t)500, H5P_DEFAULT);
...
static unsigned check_external_link_creation_tags(void) {
  ...
  if ((gid = H5Gcreate2(fid2, GROUPNAME, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(FILENAME2, GROUPNAMEPATH, fid, LINKNAME, H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static unsigned check_invalid_tag_application(void) {
  ...
  /* Call H5HL_create, an internal function that calls H5AC_insert_entry without
   * setting up a tag */
  ...
  /* Verify the same call to H5HL_create now works as intended, with a tag set
   * up. */
  if (H5HL_create(f, (size_t)1024, &addr) < 0)
    TEST_ERROR;
...
int main(void) {
  ...
  H5Pset_file_space_strategy(fcpl_shmesg_all, H5F_FSPACE_STRATEGY_FSM_AGGR,
                             TRUE, (hsize_t)0);
