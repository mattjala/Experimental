...
/* Files created under 1.6 branch and 1.8 branch--used in
...
static void test_libver_bounds_super_create(hid_t fapl, hid_t fcpl,
                                            htri_t is_swmr, htri_t non_def_fsm);
static void test_libver_bounds_super_open(hid_t fapl, hid_t fcpl,
                                          htri_t is_swmr, htri_t non_def_fsm);
...
static void test_file_create(void) {
  ...
  H5E_BEGIN_TRY { H5Fdelete(FILE1, H5P_DEFAULT); }
  H5E_END_TRY

  /* Try opening a non-existent file */
  H5E_BEGIN_TRY { fid1 = H5Fopen(FILE1, H5F_ACC_RDWR, H5P_DEFAULT); }
  H5E_END_TRY
  ...
  if (h5_using_default_driver(NULL)) {
    ...
    /* Make certain we can create a dataset properly in the file with the
     * userblock */
    {
      ...
      /* Create the dataset. */
      dataset_id = H5Dcreate2(fid2, F2_DSET, H5T_NATIVE_UINT, dataspace_id,
      ...
      ret = H5Dwrite(dataset_id, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     data);
...
**
****************************************************************/
static void test_file_open(const char *env_h5_drvr) {
  ...
  H5E_BEGIN_TRY { fid2 = H5Fopen(FILE2, H5F_ACC_RDWR, fapl_id); }
  H5E_END_TRY
  VERIFY(fid2, FAIL, "H5Fopen");

  /* Check that the intent fails for an invalid ID */
  H5E_BEGIN_TRY { ret = H5Fget_intent(fid1, &intent); }
  H5E_END_TRY
...
static void test_file_reopen(void) {
  ...
  did = H5Dcreate2(fid, REOPEN_DSET, H5T_NATIVE_INT, sid, H5P_DEFAULT,
  ...
  H5E_BEGIN_TRY { H5Fdelete(REOPEN_FILE, H5P_DEFAULT); }
  H5E_END_TRY
...
static void test_file_close(void) {
  ...
  /* should fail */
  H5E_BEGIN_TRY { fid2 = H5Fopen(FILE1, H5F_ACC_RDWR, fapl_id); }
  H5E_END_TRY
  VERIFY(fid2, FAIL, "H5Fopen");
   ...
   * still open. */
  H5E_BEGIN_TRY { ret = H5Fclose(fid1); }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY { ret = H5Idec_ref(fid2); }
  H5E_END_TRY
  ...
  test_obj_count_and_id(fid1, fid2, dataset_id, group_id1, group_id2,
                        group_id3);
...
**
****************************************************************/
static void create_objects(hid_t fid1, hid_t fid2, hid_t *ret_did,
                           hid_t *ret_gid1, hid_t *ret_gid2, hid_t *ret_gid3) {
  ssize_t oid_count;
  herr_t ret;
   ...
   */
  {
    ...
    oid_count = H5Fget_obj_count(fid1, H5F_OBJ_DATASET | H5F_OBJ_GROUP |
    ...
    oid_count = H5Fget_obj_count(fid2, H5F_OBJ_DATASET | H5F_OBJ_GROUP |
                                           H5F_OBJ_DATATYPE | H5F_OBJ_ATTR);
    CHECK(oid_count, FAIL, "H5Fget_obj_count");
    VERIFY(oid_count, OBJ_ID_COUNT_0, "H5Fget_obj_count");
  }
  ...
  /* create a dataset in the first file open */
  {
    ...
    /* Create the dataset. */
    dataset_id = H5Dcreate2(fid1, "/dset", H5T_NATIVE_UINT, dataspace_id,
                            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    CHECK(dataset_id, FAIL, "H5Dcreate2");
...
static void test_get_obj_ids(void) {
  ...
  for (n = 0; n < NDSETS; n++) {
    ...
    dset[n] = H5Dcreate2(fid, dname, H5T_NATIVE_INT, filespace, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT);
    CHECK(dset[n], FAIL, "H5Dcreate2");
  }

  /* The number of opened objects should be NGROUPS + NDSETS + 1.  One is opened
   * file. */
  ...
  /* Call the public function H5F_get_obj_ids to use H5F__get_objects.  User
   * reported having problem here. that the returned size (ret_count) from
   * H5Fget_obj_ids is one greater than the size passed in (oid_list_size) */
  ...
  /* Close all object IDs on the list except the file ID. The first ID is
   * supposed to be file ID according to the library design */
  ...
  /* The number of opened objects should be NGROUPS + 1 + 1.  The first one is
   * opened file. The second one is the dataset ID left open from the previous
   * around of H5Fget_obj_ids */
  oid_count = H5Fget_obj_count(fid, H5F_OBJ_ALL);
  CHECK(oid_count, FAIL, "H5Fget_obj_count");
  ...
  /* Close all object IDs on the list except the file ID. The first ID is still
   * the file ID */
  for (i = 0; i < ret_count; i++) {
    if (fid != oid_list[i]) {
      ret = H5Oclose(oid_list[i]);
  ...
  /* Reopen the file to check whether H5Fget_obj_count and H5Fget_obj_ids still
   * works when the file is closed first */
  ...
  ret_count = H5Fget_obj_ids((hid_t)H5F_OBJ_ALL, H5F_OBJ_ALL, (size_t)oid_count,
                             oid_list);
  CHECK(ret_count, FAIL, "H5Fget_obj_ids");
...
static void test_get_file_id(void) {
  ...
  dataset_id = H5Dcreate2(group_id, DSET_NAME, H5T_NATIVE_INT, dataspace_id,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset_id, FAIL, "H5Dcreate2");
   ...
   */
  attr_id = H5Acreate2(dataset_id, ATTR_NAME, H5T_NATIVE_INT, dataspace_id,
                       H5P_DEFAULT, H5P_DEFAULT);
  ...
  ret = H5Tcommit2(fid, TYPE_NAME, datatype_id, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
...
**
****************************************************************/
static void test_obj_count_and_id(hid_t fid1, hid_t fid2, hid_t did, hid_t gid1,
  ...
  if (oid_count > 0) {
    ...
    if (oid_list != NULL) {
      ...
      ret_count = H5Fget_obj_ids((hid_t)H5F_OBJ_ALL, H5F_OBJ_ALL,
                                 (size_t)oid_count, oid_list);
      CHECK(ret_count, FAIL, "H5Fget_obj_ids");
...
static void test_file_perm(void) {
  ...
  dset = H5Dcreate2(file, F2_DSET, H5T_NATIVE_INT, dspace, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
  ...
  H5E_BEGIN_TRY {
    dset = H5Dcreate2(filero, F2_DSET, H5T_NATIVE_INT, dspace, H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
...
static void test_file_perm2(void) {
  ...
  H5E_BEGIN_TRY {
    group =
        H5Gcreate2(filero, "MY_GROUP", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    attr = H5Acreate2(filero, "MY_ATTR", H5T_NATIVE_INT, dspace, H5P_DEFAULT,
                      H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    ret = H5Tcommit2(filero, "MY_DTYPE", type, H5P_DEFAULT, H5P_DEFAULT,
                     H5P_DEFAULT);
  }
  H5E_END_TRY
...
static void test_file_is_accessible(const char *env_h5_drvr) {
  ...
  char
      non_hdf5_sb_filename[FILENAME_LEN]; /* Name of non-hdf5 superblock file */
  ...
  if (h5_driver_is_default_vfd_compatible(fapl_id,
                                          &driver_is_default_compatible) < 0) {
  ...
  h5_fixname(FILE_IS_ACCESSIBLE_NON_HDF5, fapl_id, non_hdf5_filename,
             sizeof(non_hdf5_filename));
  h5_fixname_superblock(FILE_IS_ACCESSIBLE_NON_HDF5, fapl_id,
                        non_hdf5_sb_filename, sizeof(non_hdf5_sb_filename));
  ...
  if (driver_is_default_compatible) {
    ...
    fd = HDopen(non_hdf5_sb_filename, O_RDWR | O_CREAT | O_TRUNC,
...
static void test_file_ishdf5(const char *env_h5_drvr) {
  hid_t fid = H5I_INVALID_HID;     /* File opened with read-write permission */
  hid_t fcpl_id = H5I_INVALID_HID; /* File creation property list */
  hid_t fapl_id = H5I_INVALID_HID; /* File access property list */
  int fd;                          /* POSIX file descriptor */
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Detection of HDF5 Files (using deprecated H5Fis_hdf5() "
              "call)\n"));
   ...
   */
  h5_fixname(FILE_IS_ACCESSIBLE, fapl_id, filename, sizeof(filename));
  h5_fixname_superblock(FILE_IS_ACCESSIBLE, fapl_id, sb_filename,
                        sizeof(filename));
...
static void test_file_open_dot(void) {
  ...
  H5E_BEGIN_TRY {
    did = H5Dcreate2(fid, ".", H5T_NATIVE_INT, sid, H5P_DEFAULT, H5P_DEFAULT,
                     H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    did = H5Dcreate2(gid, ".", H5T_NATIVE_INT, sid, H5P_DEFAULT, H5P_DEFAULT,
                     H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(did, FAIL, "H5Dcreate2");
...
static void test_file_open_overlap(void) {
  ...
  did1 = H5Dcreate2(gid, DSET1, H5T_NATIVE_INT, sid, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
  ...
  did2 = H5Dcreate2(fid2, DSET2, H5T_NATIVE_INT, sid, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
...
static void test_file_getname(void) {
  ...
  group_id = H5Gcreate2(file_id, TESTA_GROUPNAME, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT);
  CHECK(group_id, FAIL, "H5Gcreate2");
  ...
  dataset_id = H5Dcreate2(file_id, TESTA_DSETNAME, H5T_NATIVE_INT, space_id,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset_id, FAIL, "H5Dcreate2");
  ...
  attr_id = H5Acreate2(dataset_id, TESTA_ATTRNAME, H5T_NATIVE_INT, space_id,
                       H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* Save it on file */
  ret = H5Tcommit2(file_id, TESTA_DTYPENAME, type_id, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");
...
static void test_file_double_group_open(void) {
  ...
  grp1_id =
      H5Gcreate2(file1_id, GRP_NAME, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
static void test_file_double_dataset_open(void) {
  ...
  dset1_id = H5Dcreate2(file1_id, DSET_NAME, H5T_NATIVE_INT, space_id,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
**
*****************************************************************/
static void test_file_double_file_dataset_open(hbool_t new_format) {
  ...
  hsize_t max_dims1[1] = {
      H5S_UNLIMITED}; /* Maximum dimesion sizes for extensible array index */
  hsize_t max_dims2[2] = {
      H5S_UNLIMITED,
      H5S_UNLIMITED}; /* Maximum dimension sizes for v2 B-tree index */
  hsize_t chunks[1] = {2}, chunks2[2] = {4, 5}; /* Chunk dimension sizes */
  hsize_t size;                                 /* File size */
  char filename[FILENAME_LEN];                  /* Filename to use */
  const char *data[] = {"String 1", "String 2", "String 3", "String 4",
                        "String 5"}; /* Input Data */
  ...
  did1 =
      H5Dcreate2(fid1, "dset_fa", tid1, sid1, H5P_DEFAULT, dcpl, H5P_DEFAULT);
  CHECK(did1, FAIL, "H5Dcreate2");
  ...
  did1 =
      H5Dcreate2(fid1, "dset_ea", tid1, sid1, H5P_DEFAULT, dcpl, H5P_DEFAULT);
  CHECK(did1, FAIL, "H5Dcreate2");
  ...
  did2 = H5Dcreate2(fid1, "dset_bt2", H5T_NATIVE_INT, sid2, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
  CHECK(did2, FAIL, "H5Dcreate2");
...
static void test_file_double_datatype_open(void) {
  ...
  ret = H5Tcommit2(file1_id, TYPE_NAME, type1_id, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");
...
**
*****************************************************************/
static void test_userblock_file_size(const char *env_h5_drvr) {
  hid_t file1_id, file2_id;
  ...
  /* Create groups */
  group1_id =
      H5Gcreate2(file1_id, GROUP1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(group1_id, FAIL, "H5Gcreate2");
  group2_id =
      H5Gcreate2(file2_id, GROUP1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* Create datasets */
  dset1_id = H5Dcreate2(file1_id, DSET2, H5T_NATIVE_INT, space_id, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dset1_id, FAIL, "H5Dcreate2");
  dset2_id = H5Dcreate2(file2_id, DSET2, H5T_NATIVE_INT, space_id, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  ...
  VERIFY_TYPE((unsigned long long)filesize2,
              (unsigned long long)(filesize1 + USERBLOCK_SIZE),
...
static void test_rw_noupdate(void) {
  ...
  MESSAGE(
      5,
      ("Testing to verify that nothing is written if nothing is changed.\n"));
...
static int test_userblock_alignment_helper1(hid_t fcpl, hid_t fapl) {
  ...
  if (fid > 0) {
    ...
    sid = H5Screate(H5S_SCALAR);
    CHECK(sid, FAIL, "H5Screate");
    did = H5Dcreate2(gid, "dataset", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                     H5P_DEFAULT, H5P_DEFAULT);
...
**
*****************************************************************/
static int test_userblock_alignment_helper2(hid_t fapl, hbool_t open_rw) {
  hid_t fid;                            /* File ID */
  int curr_num_errs = GetTestNumErrs(); /* Retrieve the current # of errors */
  herr_t ret;                           /* Generic return value */
...
**
*****************************************************************/
static void test_userblock_alignment(const char *env_h5_drvr) {
  hid_t fid;  /* File ID */
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing that non-zero userblocks and object alignment interact "
              "correctly.\n"));
...
**
*****************************************************************/
static void test_userblock_alignment_paged(const char *env_h5_drvr) {
  hid_t fid;  /* File ID */
  hid_t fcpl; /* File creation property list ID */
  hid_t fapl; /* File access property list ID */
  herr_t ret; /* Generic return value */
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing interaction between userblock and alignment (via paged "
              "aggregation and "
              "H5Pset_alignment)\n"));
  ...
  ret = H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, FALSE,
                                   (hsize_t)1);
  ...
  ret = H5Pset_userblock(fcpl, (hsize_t)512);
  CHECK(ret, FAIL, "H5Pset_userblock");
  ret = H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, TRUE,
                                   (hsize_t)1);
  CHECK(ret, FAIL, "H5Pset_file_space_strategy");
  ret = H5Pset_file_space_page_size(fcpl, (hsize_t)512);
  ...
  ret = H5Pset_userblock(fcpl, (hsize_t)512);
  CHECK(ret, FAIL, "H5Pset_userblock");
  ret = H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_NONE, FALSE,
                                   (hsize_t)1);
  CHECK(ret, FAIL, "H5Pset_file_space_strategy");
  ret = H5Pset_file_space_page_size(fcpl, (hsize_t)512);
  CHECK(ret, FAIL, "H5Pset_file_space_page_size");
  ...
  ret = H5Pset_userblock(fcpl, (hsize_t)1024);
  CHECK(ret, FAIL, "H5Pset_userblock");
  ret = H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, FALSE,
                                   (hsize_t)1);
  CHECK(ret, FAIL, "H5Pset_file_space_strategy");
  ret = H5Pset_file_space_page_size(fcpl, (hsize_t)1023);
  CHECK(ret, FAIL, "H5Pset_file_space_page_size");
...
**
****************************************************************/
static void test_filespace_info(const char *env_h5_drvr) {
  hid_t fid;                      /* File IDs    */
  ...
  H5F_fspace_strategy_t
      fs_strategy;      /* File space strategy--iteration variable */
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing file creation public routines: "
              "H5Pget/set_file_space_strategy & "
  ...
  for (new_format = FALSE; new_format <= TRUE; new_format++) {
    ...
    for (fs_persist = FALSE; fs_persist <= TRUE; fs_persist++) {
      ...
      for (fs_threshold = 0; fs_threshold <= TEST_THRESHOLD10; fs_threshold++) {
        ...
        for (fs_strategy = H5F_FSPACE_STRATEGY_FSM_AGGR;
             fs_strategy < H5F_FSPACE_STRATEGY_NTYPES; fs_strategy++) {

          if (!contig_addr_vfd &&
              (fs_strategy == H5F_FSPACE_STRATEGY_PAGE || fs_persist))
            continue;
          ...
          ret = H5Pset_file_space_strategy(fcpl, fs_strategy,
                                           (hbool_t)fs_persist, fs_threshold);
          CHECK(ret, FAIL, "H5Pset_file_space_strategy");
          ...
          /* Retrieve file space information */
          ret =
              H5Pget_file_space_strategy(fcpl, &strategy, &persist, &threshold);
          CHECK(ret, FAIL, "H5Pget_file_space_strategy");
          ...
          /* Retrieve file space information */
          ret = H5Pget_file_space_strategy(fcpl1, &strategy, &persist,
                                           &threshold);
          CHECK(ret, FAIL, "H5Pget_file_space_strategy");
          ...
          /* Retrieve file space information */
          ret = H5Pget_file_space_strategy(fcpl2, &strategy, &persist,
                                           &threshold);
          CHECK(ret, FAIL, "H5Pget_file_space_strategy");
...
**
*****************************************************************/
static int set_multi_split(hid_t fapl, hsize_t pagesize, hbool_t split) {
...
**
*****************************************************************/
static void test_file_freespace(const char *env_h5_drvr) {
  hid_t file;                    /* File opened with read-write permission */
  ...
  if (!split_vfd && !multi_vfd) {
    ...
    for (new_format = FALSE; new_format <= TRUE; new_format++) {
      ...
      for (u = 0; u < 10; u++) {
        ...
        dset = H5Dcreate2(file, name, H5T_STD_U32LE, dspace, H5P_DEFAULT, dcpl,
                          H5P_DEFAULT);
        CHECK(dset, FAIL, "H5Dcreate2");
...
**
*****************************************************************/
static void test_sects_freespace(const char *env_h5_drvr, hbool_t new_format) {
  char filename[FILENAME_LEN]; /* Filename to use */
  hid_t file;                  /* File ID */
  ...
  hssize_t nmeta = 0,
           nraw = 0; /* # of free-space sections for meta/raw/generic data */
  H5F_sect_info_t sect_info[15];     /* Array to hold free-space information */
  H5F_sect_info_t all_sect_info[15]; /* Array to hold free-space information for
                                        all types of data */
  H5F_sect_info_t meta_sect_info[15]; /* Array to hold free-space information
                                         for metadata */
  H5F_sect_info_t
      raw_sect_info[15]; /* Array to hold free-space information for raw data */
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing H5Fget_free_sections()--free-space section info in the "
              "file\n"));
  ...
  if (!split_vfd && !multi_vfd) {
    ...
    if (new_format) {
      ...
      ret = H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, TRUE,
                                       (hsize_t)1);
      CHECK(ret, FAIL, "H5Pget_file_space_strategy");
    }
    ...
    dset = H5Dcreate2(file, "Dataset_large", H5T_STD_U32LE, dspace, H5P_DEFAULT,
                      dcpl, H5P_DEFAULT);
    CHECK(dset, FAIL, "H5Dcreate2");
    ...
    nsects =
        H5Fget_free_sections(file, H5FD_MEM_DEFAULT, (size_t)0, all_sect_info);
    ...
    nsects = H5Fget_free_sections(file, H5FD_MEM_DEFAULT, (size_t)nall,
                                  all_sect_info);
    ...
    nsects = H5Fget_free_sections(file, H5FD_MEM_DEFAULT, (size_t)(nall - 1),
                                  sect_info);
    VERIFY(nsects, nall, "H5Fget_free_sections");
    ...
    if (multi_vfd) {
      ...
      for (type = H5FD_MEM_SUPER; type < H5FD_MEM_NTYPES; type++) {
        ...
        if (ntmp > 0) {
          nsects = H5Fget_free_sections(file, type, (size_t)ntmp,
                                        &meta_sect_info[nmeta]);
      ...
      nsects = H5Fget_free_sections(file, H5FD_MEM_SUPER, (size_t)nmeta,
                                    meta_sect_info);
    ...
    nsects =
        H5Fget_free_sections(file, H5FD_MEM_DRAW, (size_t)nraw, raw_sect_info);
...
static void test_filespace_compatible(void) {
  ...
  for (j = 0; j < NELMTS(OLD_FILENAME); j++) {
    const char *filename =
        H5_get_srcdir_filename(OLD_FILENAME[j]); /* Corrected test file name */
    ...
    VERIFY(strategy, H5F_FSPACE_STRATEGY_FSM_AGGR,
...
**    h5fc_ext1_i.h5:   H5F_FILE_SPACE_ALL, default threshold; has superblock
*extension but no fsinfo message
**    h5fc_ext1_f.h5:   H5F_FILE_SPACE_ALL_PERSIST, default threshold; has
*superblock extension with fsinfo message
**    h5fc_ext2_if.h5:  H5F_FILE_SPACE_ALL, non-default threshold; has
*superblock extension with fsinfo message
**    h5fc_ext2_sf.h5:  H5F_FILE_SPACE_VFD, default threshold; has superblock
*extension with fsinfo message
**    h5fc_ext3_isf.h5: H5F_FILE_SPACE_AGGR_VFD, default threshold; has
*superblock extension with fsinfo message
**    h5fc_ext_none.h5: H5F_FILE_SPACE_ALL, default threshold; without
*superblock extension
**  The above files are copied from release 1.10.0
*tools/h5format_convert/testfiles.
**
****************************************************************/
static void test_filespace_1_10_0_compatible(void) {
  ...
  for (j = 0; j < NELMTS(OLD_1_10_0_FILENAME); j++) {
    ...
    switch (j) {
    case 0:
      VERIFY(strategy, H5F_FILE_SPACE_STRATEGY_DEF,
             "H5Pget_file_space_strategy");
      VERIFY(persist, H5F_FREE_SPACE_PERSIST_DEF, "H5Pget_file_space_strategy");
      VERIFY(threshold, H5F_FREE_SPACE_THRESHOLD_DEF,
             "H5Pget_file_space_strategy");
...
static void test_filespace_round_compatible(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("File space compatibility testing for files from trunk to 1_8 to "
              "trunk\n"));
...
**
****************************************************************/
static void test_libver_bounds_real(H5F_libver_t libver_create,
                                    unsigned oh_vers_create,
                                    H5F_libver_t libver_mod,
   ...
   */
  ret = H5Oget_native_info_by_name(file, "/", &ninfo, H5O_NATIVE_INFO_HDR,
                                   H5P_DEFAULT);
...
static void test_libver_bounds_open(void) {
  ...
  /* Create dataset */
  dset = H5Dcreate2(file, VERBDSNAME, H5T_NATIVE_INT, space, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
  CHECK(dset, FAIL, "H5Dcreate2");
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    ...
    VERIFY(new_low >= H5F_LIBVER_V110, TRUE,
           "Low bound should be upgraded to at least H5F_LIBVER_V110");
 ...
 *
 *              The test creates the destination file with (v18, v18) version
 *bounds. H5Ocopy() should succeed in copying the dataset in the source file to
...
static void test_libver_bounds_copy(void) {
  ...
  MESSAGE(5, ("Testing H5Ocopy a dataset in a 1.8 library file to a 1.10 "
              "library file\n"));

  ret = h5_driver_is_default_vfd_compatible(H5P_DEFAULT,
                                            &driver_is_default_compatible);
  ...
  H5E_BEGIN_TRY { H5Fdelete(DST_FILE, H5P_DEFAULT); }
  H5E_END_TRY
...
**          (5) (v110, v110)
**
**      For each pair of setting in fapl, verify format versions with the
*following
**      six tests:
**          (1) test_libver_bounds_super(fapl): superblock versions
**          (2) test_libver_bounds_obj(fapl): object header versions
**          (3) test_libver_bounds_dataset(fapl): message versions associated
*with dataset
...
static void test_libver_bounds_super(hid_t fapl, const char *env_h5_drvr) {
  ...
  if (h5_using_default_driver(env_h5_drvr)) {
    ...
    fcpl = H5Pcreate(H5P_FILE_CREATE);
    CHECK(fcpl, H5I_INVALID_HID, "H5Pcreate");
    ret = H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, 1,
                                     (hsize_t)1);
...
**
**      For file creation, the bounds setting in fapl, the feature enabled in
*fcpl,
**      and with/without SWMR file access will determine the results for #a to
*#c.
**
**      The first row for the following two tables is the 5 pairs of low/high
*bounds setting
**      in the input fapl. The next three rows list the expected results for #a
*to #c.
**      "-->" indicates "upgrade to"
**
**      The last table lists the expected results in creating the file when
*non-default
...
**
*--------------------------------------------------------------------------------
**                    | (earliest, v18) | (earliest, v110) | (v18, v18) | (v18,
*v110) | (v110, v110) |
**
*|______________________________________________________________________________|
** Superblock version | vers 0, 1, 2    | vers 0, 1, 2     | vers 2     | vers 2
*| vers 3       |
**
*|------------------------------------------------------------------------------|
...
**
*|______________________________________________________________________________|
** Superblock version | --              | vers 3           | --         | vers
*3     | vers 3       |
**
*|------------------------------------------------------------------------------|
** File's low bound   | --              | ->v110           | --         | ->v110
*| no change    |
**
*|------------------------------------------------------------------------------|
** File creation      | fail            | succeed          | fail       |
*succeed     | succeed      |
**
*|______________________________________________________________________________|
**
**                                  Creating a file with write/SWMR-write access
*+ non-default fsinfo
**
*--------------------------------------------------------------------------------
**                    | (earliest, v18) | (earliest, v110) | (v18, v18) | (v18,
...
static void test_libver_bounds_super_create(hid_t fapl, hid_t fcpl,
  ...
  H5E_BEGIN_TRY {
    fid = H5Fcreate(FILE8, H5F_ACC_TRUNC | (is_swmr ? H5F_ACC_SWMR_WRITE : 0),
                    fcpl, fapl);
  }
  H5E_END_TRY
  ...
  if (non_def_fsm && high < H5F_LIBVER_V110)
    ...
    if (high >= H5F_LIBVER_V110) { /* Should succeed */
      ...
      VERIFY(HDF5_SUPERBLOCK_VERSION_3, f->shared->sblock->super_vers,
...
**      Verify the following when opening a file which is created with the input
*fapl, fcpl,
**      and with/without SWMR access:
...
**      (2) Close the file
**      (3) Reopen the file with a new fapl that is set to the 5 pairs of
*low/high bounds
**          in a for loop. For each pair of setting in the new fapl:
**          --Verify the expected results for #a and #b above.
**          --Close the file.
**
**      For file open, the file's superblock version, the low/high bounds
*setting in fapl,
**      and with/without SWMR file access will determine the results for #a and
*#b.
**
**      The first row for the following tables (#A - #B) is the 5 pairs of
*low/high bounds setting
**      in the input fapl. The next two rows list the expected results for #a
*and #b.
**      "-->" indicates "upgrade to"
**
**      The last table (#C) lists the expected results in opening the file when
*non-default
**      free-space info (fsinfo) is enabled in fcpl.
**
...
**
*|------------------------------------------------------------------------------|
** File open        | fail            | succeed          | fail       | succeed
*| succeed      |
**
*|______________________________________________________________________________|
**
...
**
*-------------------------------------------------------------------------------
**                  | (earliest, v18) | (earliest, v10) | (v18, v18) | (v18,
*v110) | (v110, v110) |
**
...
**
**
**                      (C) Opening a file with write/SWMR-write access +
*non-default fsinfo
**
*-------------------------------------------------------------------------------
**                  | (earliest, v18) | (earliest, v10) | (v18, v18) | (v18,
  ...
  if (non_def_fsm && high < H5F_LIBVER_V110) {
    ...
    for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
      for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
        ...
        H5E_BEGIN_TRY {
          fid =
              H5Fopen(FILE8, H5F_ACC_RDWR | (is_swmr ? H5F_ACC_SWMR_WRITE : 0),
                      new_fapl);
        }
        H5E_END_TRY
...
static void test_libver_bounds_obj(hid_t fapl) {
  ...
  ret = H5Oget_native_info_by_name(fid, "/", &ninfo, H5O_NATIVE_INFO_HDR,
                                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_native_info_by_name");
    ...
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      if (fid >= 0) { /* The file open succeeds */
        ...
        if (f->shared->low_bound >= H5F_LIBVER_V18)
          ...
          VERIFY(ginfo.storage_type, H5G_STORAGE_TYPE_SYMBOL_TABLE,
                 "H5Gget_info");

        /* Get object header information */
        ret = H5Oget_native_info_by_name(gid, GRP_NAME, &ninfo,
                                         H5O_NATIVE_INFO_HDR, H5P_DEFAULT);
        CHECK(ret, FAIL, "H5Oget_native_info_by_name");

        /* Verify object header version as indicated by low_bound */
        VERIFY(ninfo.hdr.version, H5O_obj_ver_bounds[f->shared->low_bound],
               "H5O_obj_ver_bounds");
...
static void test_libver_bounds_dataset(hid_t fapl) {
  ...
  hsize_t max_dims2[2] = {H5S_UNLIMITED,
                          H5S_UNLIMITED}; /* Maximum dimension sizes */
  ...
  did = H5Dcreate2(fid, DSETA, H5T_NATIVE_INT, sid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  ...
  if (low == H5F_LIBVER_EARLIEST) {
    ...
    VERIFY(dset->shared->layout.version, H5O_layout_ver_bounds[low],
           "H5O_layout_ver_bounds");
    VERIFY(dset->shared->dcpl_cache.fill.version, H5O_fill_ver_bounds[low],
           "H5O_fill_ver_bounds");
  }

  /* Verify filter pipeline message version */
  VERIFY(dset->shared->dcpl_cache.pline.version, H5O_pline_ver_bounds[low],
  ...
  if (did >= 0) {
    ...
    VERIFY(dset->shared->layout.version, H5O_LAYOUT_VERSION_4,
           "H5O_layout_ver_bounds");
    VERIFY(dset->shared->layout.u.chunk.idx_type, H5D_CHUNK_IDX_FARRAY,
           "chunk_index_type");
  ...
  /* Verify the dataset's layout, fill value and filter pipeline message
   * versions */
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      H5E_BEGIN_TRY { ret = H5Pset_libver_bounds(new_fapl, low, high); }
      ...
      if (fid >= 0) { /* The file open succeeds */
        ...
        /* Create the chunked dataset */
        did = H5Dcreate2(fid, DSETC, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT);
        ...
        if (dset) {
          ...
          if (f->shared->low_bound == H5F_LIBVER_EARLIEST) {
            /* For layout message: the earliest version the library will set is
             * 3 */
            /* For fill value message: the earliest version the library will set
             * is 2 */
            VERIFY(dset->shared->layout.version, H5O_LAYOUT_VERSION_DEFAULT,
                   "H5O_layout_ver_bounds");
            VERIFY(dset->shared->dcpl_cache.fill.version, H5O_FILL_VERSION_2,
            ...
            VERIFY(dset->shared->layout.version,
                   H5O_layout_ver_bounds[f->shared->low_bound],
                   "H5O_layout_ver_bounds");
            VERIFY(dset->shared->dcpl_cache.fill.version,
                   H5O_fill_ver_bounds[f->shared->low_bound],
                   "H5O_fill_ver_bounds");
          }

          /* Verify the filter pipeline message version */
          VERIFY(dset->shared->dcpl_cache.pline.version,
                 H5O_pline_ver_bounds[f->shared->low_bound],
                 "H5O_pline_ver_bounds");

          /* Verify the dataset's chunk indexing type */
          if (dset->shared->layout.version == H5O_LAYOUT_VERSION_LATEST)
            VERIFY(dset->shared->layout.u.chunk.idx_type, H5D_CHUNK_IDX_BT2,
                   "chunk_index_type");
          else
...
static void test_libver_bounds_dataspace(hid_t fapl) {
  ...
  hid_t did_compact = H5I_INVALID_HID,
        did_contig = H5I_INVALID_HID;                      /* Dataset IDs */
  hid_t sid = H5I_INVALID_HID, sid_null = H5I_INVALID_HID; /* Dataspace IDs */
  hid_t sid_compact = H5I_INVALID_HID,
        sid_contig = H5I_INVALID_HID; /* Dataspace IDs */
  hid_t dcpl = H5I_INVALID_HID;       /* Dataset creation property list */
  hid_t dcpl_compact = H5I_INVALID_HID,
        dcpl_contig = H5I_INVALID_HID;     /* Dataset creation property lists */
  ...
  VERIFY(space->extent.version, H5O_sdspace_ver_bounds[low],
  ...
  did_null = H5Dcreate2(fid, DSET_NULL, H5T_NATIVE_INT, sid_null, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* Verify the dataspace version */
  VERIFY(space_null->extent.version, H5O_SDSPACE_VERSION_2,
         "H5O_sdspace_ver_bounds");
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      H5S_t *tmp_space, *tmp_space_compact,
          *tmp_space_contig; /* Internal dataspace pointers */
      ...
      if (fid >= 0) { /* The file open succeeds */
        ...
        /* Create the chunked dataset */
        did = H5Dcreate2(fid, DSETA, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT);
        CHECK(did, H5I_INVALID_HID, "H5Dcreate2");
        ...
        did_compact = H5Dcreate2(fid, DSETB, H5T_NATIVE_INT, sid_compact,
                                 H5P_DEFAULT, dcpl_compact, H5P_DEFAULT);
        ...
        did_contig = H5Dcreate2(fid, DSETC, H5T_NATIVE_INT, sid_contig,
                                H5P_DEFAULT, dcpl_contig, H5P_DEFAULT);
        ...
        if (tmp_space) {
          ...
          VERIFY(tmp_space->extent.version,
                 H5O_sdspace_ver_bounds[f->shared->low_bound],
                 "H5O_sdspace_ver_bounds");
        }
        if (tmp_space_compact) {
          VERIFY(tmp_space_compact->extent.version,
                 H5O_sdspace_ver_bounds[f->shared->low_bound],
                 "H5O_sdspace_ver_bounds");
        }
        if (tmp_space_contig) {
          VERIFY(tmp_space_contig->extent.version,
                 H5O_sdspace_ver_bounds[f->shared->low_bound],
                 "H5O_sdspace_ver_bounds");
        }
...
****************************************************************/
static void test_libver_bounds_datatype(hid_t fapl) {
  hid_t tid = H5I_INVALID_HID, tid_enum = H5I_INVALID_HID,
        tid_array = H5I_INVALID_HID; /* Datatype IDs */
  hid_t tid_compound = H5I_INVALID_HID,
        tid_vlen = H5I_INVALID_HID; /* Datatype IDs */
...
static void test_libver_bounds_datatype_check(hid_t fapl, hid_t tid) {
  ...
  H5T_t *str_dtype =
      NULL;        /* Internal datatype pointer for the string datatype */
  H5F_t *f = NULL; /* Internal file pointer */
  H5F_libver_t low, high; /* Low and high bounds */
  herr_t ret;             /* Return value */
  ...
  CHECK(ret, FAIL, "H5Tset_size");
  ret = H5Tcommit2(fid, "datatype", str_tid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");
  ...
  if (dtype->shared->type == H5T_COMPOUND || dtype->shared->type == H5T_ENUM ||
    ...
    if (dtype->shared->type == H5T_ARRAY && low == H5F_LIBVER_EARLIEST)
      VERIFY(dtype->shared->version, H5O_DTYPE_VERSION_2,
             "H5O_dtype_ver_bounds");
    else
      VERIFY(dtype->shared->version, H5O_dtype_ver_bounds[low],
             "H5O_dtype_ver_bounds");
  } else
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      if (fid >= 0) { /* The file open succeeds */
        ...
        VERIFY(str_dtype->shared->version,
               H5O_dtype_ver_bounds[H5F_LIBVER_EARLIEST],
               "H5O_dtype_ver_bounds");
        ...
        /* Create the chunked dataset */
        did =
            H5Dcreate2(fid, DSETNAME, tid, sid, H5P_DEFAULT, dcpl, H5P_DEFAULT);
        CHECK(did, H5I_INVALID_HID, "H5Dcreate2");
        ...
        if (dtype) {
          ...
          if (dtype->shared->type == H5T_COMPOUND ||
            ...
            if (dtype->shared->type == H5T_ARRAY &&
              ...
              VERIFY(dtype->shared->version,
                     H5O_dtype_ver_bounds[f->shared->low_bound],
                     "H5O_dtype_ver_bounds");
          } else
            VERIFY(dtype->shared->version,
                   H5O_dtype_ver_bounds[H5F_LIBVER_EARLIEST],
...
static void test_libver_bounds_attributes(hid_t fapl) {
  ...
  if (low == H5F_LIBVER_EARLIEST)
    /* The earliest version the library can set for an attribute with committed
     * datatype is 2 */
    VERIFY(attr->shared->version, H5O_ATTR_VERSION_2, "H5O_attr_ver_bounds");
  else
    VERIFY(attr->shared->version, H5O_attr_ver_bounds[low],
           "H5O_attr_ver_bounds");
  ...
  /* Verify attribute version */
  if (low == H5F_LIBVER_EARLIEST)
    /* The earliest version the library can set for an attribute with character
     * encoding is 3 */
  ...
  /* Verify the attribute version */
  if (low == H5F_LIBVER_EARLIEST)
    /* The earliest version the library can set for an attribute with shared
     * datatype is 2 */
    VERIFY(attr->shared->version, H5O_ATTR_VERSION_2, "H5O_attr_ver_bounds");
  else
    VERIFY(attr->shared->version, H5O_attr_ver_bounds[low],
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      if (fid >= 0) { /* The file open succeeds */
        ...
        aid = H5Acreate2(gid, "attr1", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         H5P_DEFAULT);
        CHECK(aid, FAIL, "H5Acreate2");
...
static void test_libver_macros(void) {
  ...
  VERIFY(H5_VERSION_GE(major + 1, minor - 1, release - 1), FALSE,
         "H5_VERSION_GE");
  ...
  VERIFY(H5_VERSION_LE(major + 1, minor - 1, release - 1), TRUE,
         "H5_VERSION_LE");
  ...
  VERIFY(H5_VERSION_LE(major - 1, minor + 1, release + 1), FALSE,
         "H5_VERSION_LE");
...
static void test_incr_filesize(void) {
  ...
  if (driver_flags & H5FD_FEAT_SUPPORTS_SWMR_IO) {
    ...
    for (u = 0; u < 10; u++) {
      HDsnprintf(name, sizeof(name), "Dataset %u", u);
      dset = H5Dcreate2(fid, name, H5T_STD_U32LE, dspace, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT);
      CHECK(dset, FAIL, "H5Dcreate2");
...
****************************************************************/
#ifndef H5_NO_DEPRECATED_SYMBOLS
static void test_deprec(const char *env_h5_drvr) {
  ...
  if (h5_using_default_driver(env_h5_drvr)) {
    ...
    ret = H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, 1,
                                     (hsize_t)0);
    ret = H5Pset_file_space_page_size(fcpl, (hsize_t)512);
    CHECK(ret, FAIL, "H5Pset_file_space_strategy");
...
void test_file(void) {
  ...
  ret = h5_driver_is_default_vfd_compatible(fapl_id,
                                            &driver_is_default_compatible);
  ...
  test_file_is_accessible(
      env_h5_drvr);          /* Test detecting HDF5 files correctly */
  test_file_delete(fapl_id); /* Test H5Fdelete */
  test_file_open_dot();      /* Test opening objects with "." for a name */
  test_file_open_overlap();  /* Test opening files in an overlapping manner */
  test_file_getname();       /* Test basic H5Fget_name() functionality */
  test_file_double_root_open();  /* Test opening root group from two files works
                                    properly */
  test_file_double_group_open(); /* Test opening same group from two files works
                                    properly */
  test_file_double_dataset_open();  /* Test opening same dataset from two files
                                       works properly */
  test_file_double_datatype_open(); /* Test opening same named datatype from two
                                       files works properly */
  test_file_double_file_dataset_open(TRUE);
  test_file_double_file_dataset_open(FALSE);
  test_userblock_file_size(env_h5_drvr); /* Tests that files created with a
                                            userblock have the correct size */
  test_cached_stab_info(); /* Tests that files are created with cached stab info
                              in the superblock */

  if (driver_is_default_compatible) {
    test_rw_noupdate(); /* Test to ensure that RW permissions don't write the
                           file unless dirtied */
  }

  test_userblock_alignment(
      env_h5_drvr); /* Tests that files created with a userblock and alignment
                       interact properly */
  test_userblock_alignment_paged(
      env_h5_drvr); /* Tests files created with a userblock and alignment (via
  ...
  test_sects_freespace(env_h5_drvr,
                       TRUE); /* Test file public routine H5Fget_free_sections()
                                 for new format */
                              /* Skipped testing for multi/split drivers */
  /* Setup for multi/split drivers are there already */
  test_sects_freespace(
      env_h5_drvr, FALSE); /* Test file public routine H5Fget_free_sections() */
                           /* Skipped testing for multi/split drivers */
  ...
  if (driver_is_default_compatible) {
    ...
    test_filespace_round_compatible();  /* Testing file space compatibility for
                                           files from trunk to 1_8 to  trunk */
    test_filespace_1_10_0_compatible(); /* Testing file space compatibility for
                                         * files from release 1.10.0
                                         */
  }
