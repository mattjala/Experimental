...
const char *DSET_NAME[] = {"contig_src_subset", "chunk_src_subset",
                           "contig_dst_subset", "chunk_dst_subset", NULL};
...
static unsigned test_compound(char *filename, hid_t fapl) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, "s1", s1_tid, space, H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 0; i < NX * NY; i++) {
    if (s1[i].a != s2[i].a || s1[i].b != s2[i].b || s1[i].c[0] != s2[i].c[0] ||
        s1[i].c[1] != s2[i].c[1] || s1[i].c[2] != s2[i].c[2] ||
  ...
  for (i = 0; i < NX * NY; i++) {
    if (s1[i].a != s3[i].a || s1[i].b != s3[i].b || s1[i].c[0] != s3[i].c[0] ||
        s1[i].c[1] != s3[i].c[1] || s1[i].c[2] != s3[i].c[2] ||
  ...
  for (i = 0; i < NX * NY; i++) {
    if (s1[i].a != s5[i].a || s1[i].b != s5[i].b || s1[i].c[0] != s5[i].c[0] ||
        s1[i].c[1] != s5[i].c[1] || s1[i].c[2] != s5[i].c[2] ||
  ...
  for (i = 0; i < NX * NY; i++) {
    if (s5[i].pre != 1000 + 4 * i || s5[i].mid1 != 1001 + 4 * i ||
        s5[i].mid2 != 1002 + 4 * i || s5[i].post != 1003 + 4 * i) {
      H5_FAILED();
  ...
  for (i = 0; i < NX * NY; i++) {
    if (s1[i].a != s6[i].a || s1[i].b != s6[i].b || s1[i].c[0] != s6[i].c[0] ||
        s1[i].c[1] != s6[i].c[1] || s1[i].c[2] != s6[i].c[2] ||
  ...
  /* Check that no previous values were clobbered */
  for (i = 0; i < NX * NY; i++) {
    if (s6[i].pre != 1000 + 4 * i || s6[i].mid1 != 1001 + 4 * i ||
        s6[i].mid2 != 1002 + 4 * i || s6[i].post != 1003 + 4 * i) {
      H5_FAILED();
      HDputs("    Memory values were clobbered");
      goto error;
    }
  ...
  /* Compare */
  for (i = 0; i < NX * NY; i++) {
    if (s1[i].a != 8 * i + 0 || s1[i].b != 8 * i + 1 ||
        s1[i].c[0] != 8 * i + 2 || s1[i].c[1] != 8 * i + 3 ||
        s1[i].c[2] != 8 * i + 4 || s1[i].c[3] != 8 * i + 5 ||
        s1[i].d != 8 * i + 6 || s1[i].e != 8 * i + 7) {
      H5_FAILED();
      printf("    i==%u, row=%u, col=%u\n", i, i / NY, i % NY);
      printf("    got: {%7d,%7d,[%7d,%7d,%7d,%7d],%7d,%7d}\n", s1[i].a, s1[i].b,
             s1[i].c[0], s1[i].c[1], s1[i].c[2], s1[i].c[3], s1[i].d, s1[i].e);
      printf("    ans: {%7d,%7d,[%7d,%7d,%7d,%7d],%7d,%7d}\n", 8 * i + 0,
             8 * i + 1, 8 * i + 2, 8 * i + 3, 8 * i + 4, 8 * i + 5, 8 * i + 6,
             8 * i + 7);
      goto error;
    }
  }
  ...
  for (i = 0; i < NX * NY; i++) {
    if (s2[i].a != s1[i].a || s2[i].b != s1[i].b || s2[i].c[0] != s1[i].c[0] ||
        s2[i].c[1] != s1[i].c[1] || s2[i].c[2] != s1[i].c[2] ||
  ...
  if (H5Sselect_hyperslab(s8_f_sid, H5S_SELECT_SET, f_offset, NULL, h_size,
                          NULL) < 0)
    goto error;
  ...
  for (i = 0; i < h_size[0]; i++) {
    for (j = 0; j < h_size[1]; j++) {
      ...
      if (ps8->a != ps1->a || ps8->b != ps1->b || ps8->c[0] != ps1->c[0] ||
          ps8->c[1] != ps1->c[1] || ps8->c[2] != ps1->c[2] ||
          ps8->c[3] != ps1->c[3] || ps8->d != ps1->d || ps8->e != ps1->e) {
        H5_FAILED();
        HDputs("    Incorrect values read from file");
        goto error;
      }
    ...
    for (j = 0; j < NY; j++) {
      ...
      if (i >= f_offset[0] && i < f_offset[0] + h_size[0] && j >= f_offset[1] &&
        ...
        if (ps2->a != ps1->a || ps2->b != ps1->b || ps2->c[0] != ps1->c[0] ||
            ps2->c[1] != ps1->c[1] || ps2->c[2] != ps1->c[2] ||
            ps2->c[3] != ps1->c[3] || ps2->d != ps1->d || ps2->e != ps1->e) {
          H5_FAILED();
          HDputs("    Memory values clobbered");
          goto error;
        }
      } else {
        if (ps2->a != (unsigned)(-1) || ps2->b != (unsigned)(-1) ||
            ps2->c[0] != (unsigned)(-1) || ps2->c[1] != (unsigned)(-1) ||
            ps2->c[2] != (unsigned)(-1) || ps2->c[3] != (unsigned)(-1) ||
            ps2->d != (unsigned)(-1) || ps2->e != (unsigned)(-1)) {
          H5_FAILED();
          HDputs("    Incorrect values read from file");
          goto error;
        }
  ...
  for (i = 0; i < NX; i++) {
    for (j = 0; j < NY; j++) {
      ...
      if (i >= f_offset[0] && i < f_offset[0] + h_size[0] && j >= f_offset[1] &&
          j < f_offset[1] + h_size[1]) {
        if (ps5->pre != (unsigned)(-1) || ps5->a != ps1->a ||
            ps5->b != ps1->b || ps5->mid1 != (unsigned)(-1) ||
            ps5->c[0] != ps1->c[0] || ps5->c[1] != ps1->c[1] ||
            ps5->c[2] != ps1->c[2] || ps5->c[3] != ps1->c[3] ||
            ps5->mid2 != (unsigned)(-1) || ps5->d != ps1->d ||
        ...
        if (ps5->pre != (unsigned)(-1) || ps5->a != (unsigned)(-1) ||
            ps5->b != (unsigned)(-1) || ps5->mid1 != (unsigned)(-1) ||
            ps5->c[0] != (unsigned)(-1) || ps5->c[1] != (unsigned)(-1) ||
            ps5->c[2] != (unsigned)(-1) || ps5->c[3] != (unsigned)(-1) ||
            ps5->mid2 != (unsigned)(-1) || ps5->d != (unsigned)(-1) ||
...
static int compare_data(void *src_data, void *dst_data, hbool_t src_subset) {
  ...
  for (i = 0; i < (int)(NX * NY); i++) {
    ...
    if (s_ptr->a != d_ptr->a || s_ptr->b != d_ptr->b ||
      ...
      H5_FAILED();
      printf("    i=%d\n", i);
      printf("    src={a=%d, b=%d, c=[%d,%d,%d,%d,%d,%d,%d,%d], d=%d, e=%d, "
             "f=%f, g=%f, "
             "h=[%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f], i=%f, j=%f, "
             "k=%f, l=%f, m=%f, n=%f}\n",
             s_ptr->a, s_ptr->b, s_ptr->c[0], s_ptr->c[1], s_ptr->c[2],
             s_ptr->c[3], s_ptr->c[4], s_ptr->c[5], s_ptr->c[6], s_ptr->c[7],
             s_ptr->d, s_ptr->e, (double)s_ptr->f, (double)s_ptr->g,
             (double)s_ptr->h[0], (double)s_ptr->h[1], (double)s_ptr->h[2],
             (double)s_ptr->h[3], (double)s_ptr->h[4], (double)s_ptr->h[5],
             (double)s_ptr->h[6], (double)s_ptr->h[7], (double)s_ptr->h[8],
             (double)s_ptr->h[9], (double)s_ptr->h[10], (double)s_ptr->h[11],
             (double)s_ptr->h[12], (double)s_ptr->h[13], (double)s_ptr->h[14],
             (double)s_ptr->h[15], (double)s_ptr->i, (double)s_ptr->j, s_ptr->k,
             s_ptr->l, s_ptr->m, s_ptr->n);
      printf("    dst={a=%d, b=%d, c=[%d,%d,%d,%d,%d,%d,%d,%d], d=%d, e=%d, "
             "f=%f, g=%f, "
             "h=[%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f], i=%f, j=%f, "
             "k=%f, l=%f, m=%f, n=%f}\n",
             d_ptr->a, d_ptr->b, d_ptr->c[0], d_ptr->c[1], d_ptr->c[2],
             d_ptr->c[3], d_ptr->c[4], d_ptr->c[5], d_ptr->c[6], d_ptr->c[7],
             d_ptr->d, d_ptr->e, (double)d_ptr->f, (double)d_ptr->g,
             (double)d_ptr->h[0], (double)d_ptr->h[1], (double)d_ptr->h[2],
             (double)d_ptr->h[3], (double)d_ptr->h[4], (double)d_ptr->h[5],
             (double)d_ptr->h[6], (double)d_ptr->h[7], (double)d_ptr->h[8],
             (double)d_ptr->h[9], (double)d_ptr->h[10], (double)d_ptr->h[11],
             (double)d_ptr->h[12], (double)d_ptr->h[13], (double)d_ptr->h[14],
             (double)d_ptr->h[15], (double)d_ptr->i, (double)d_ptr->j, d_ptr->k,
             d_ptr->l, d_ptr->m, d_ptr->n);
      goto error;
    }
  }
...
static unsigned test_hdf5_src_subset(char *filename, hid_t fapl) {
  ...
  if ((dataset = H5Dcreate2(file, DSET_NAME[0], src_tid, space, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    goto error;
  ...
  if ((dataset = H5Dcreate2(file, DSET_NAME[1], src_tid, space, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    goto error;
...
static unsigned test_hdf5_dst_subset(char *filename, hid_t fapl) {
  ...
  /* Create contiguous data set */
  if ((dataset = H5Dcreate2(file, DSET_NAME[2], src_tid, space, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    goto error;
  ...
  /* Create chunked data set */
  if ((dataset = H5Dcreate2(file, DSET_NAME[3], src_tid, space, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    goto error;
...
static unsigned test_pack_ooo(void) {
  ...
  for (i = 0; i < PACK_NMEMBS; i++) {
    ...
    if (j < (num_free - 1))
      memmove(&free_order[j], &free_order[j + 1],
              (num_free - j - 1) * sizeof(free_order[0]));
  } /* end for */
  ...
  for (extra_space = 0; extra_space < 2; extra_space++) {
    ...
    if ((cmpd = H5Tcreate(H5T_COMPOUND,
                          (size_t)((4 * PACK_NMEMBS) + extra_space))) < 0)
      PACK_OOO_ERROR
    ...
    for (i = 0; i < PACK_NMEMBS; i++) {
      ...
      if (i == sub_cmpd_order) {
        if (H5Tinsert(cmpd, name, (size_t)(4 * order[i]), sub_cmpd) < 0)
          PACK_OOO_ERROR
      } else if (H5Tinsert(cmpd, name, (size_t)(4 * order[i]), H5T_STD_I32BE) <
                 0)
        PACK_OOO_ERROR
    } /* end for */
    ...
    for (i = 0; i < PACK_NMEMBS; i++) {
      HDsnprintf(name, sizeof(name), "%05d", i);
      if (i == PACK_NMEMBS - 1) {
        if (H5Tinsert(cmpd, name, (size_t)(4 * (PACK_NMEMBS - i - 1)),
                      sub_cmpd) < 0)
          PACK_OOO_ERROR
      } else if (H5Tinsert(cmpd, name, (size_t)(4 * (PACK_NMEMBS - i - 1)),
                           H5T_STD_I32BE) < 0)
        PACK_OOO_ERROR
    } /* end for */
...
static unsigned test_ooo_order(char *filename, hid_t fapl_id) {
  ...
  /* Commit the datatype */
  if (H5Tcommit2(file, "dtype", dtype, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(file, "dtype2", dtype, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
...
int main(int argc, char *argv[]) {
  ...
  HDputs("Testing the optimization of when the source type is a subset of the "
         "dest:");
  h5_fixname(FILENAME[1], fapl_id, fname, sizeof(fname));
  nerrors += test_hdf5_src_subset(fname, fapl_id);

  HDputs("Testing the optimization of when the dest type is a subset of the "
         "source:");
