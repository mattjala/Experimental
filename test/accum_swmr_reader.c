...
/* Filename: this is the same as the define in accum.c used by
 * test_swmr_write_big() */
 ...
 *		        test in accum.c.  The reader reads at address 1024 from
 *the file and verifies that the metadata in the accumulator at address 1024
 *does get written to disk.
 *
...
int main(void) {
  ...
  if (H5F_block_read(f, H5FD_MEM_DEFAULT, (haddr_t)1024, (size_t)1024, rbuf) <
      0)
    FAIL_STACK_ERROR;
