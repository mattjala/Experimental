...
static int is_sparse(void) {
  ...
  if ((fd = HDopen("x.h5", O_RDWR | O_TRUNC | O_CREAT,
 ...
 *-------------------------------------------------------------------------
 */
static hid_t new_object(hid_t f, const char *name, int ndims, hsize_t dims[],
                        hsize_t cdims[]) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(f, name, TEST_DATATYPE, space, H5P_DEFAULT, dcpl,
                            H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_extend(hid_t f, const char *prefix, size_t nx, size_t ny,
                          size_t nz) {
  hid_t dataset; /* Dataset ID */
  ...
  if (!nz) {
    if (!ny) {
      ...
      ndims = 2;
      nz = 1;
      HDsnprintf(dims, sizeof(dims), "%lux%lu", (unsigned long)nx,
                 (unsigned long)ny);
    }
  } else {
    ndims = 3;
    HDsnprintf(dims, sizeof(dims), "%lux%lux%lu", (unsigned long)nx,
  ...
  for (ctr = 0; H5VM_vector_lt_u((unsigned)ndims, max_corner, whole_size);
       ctr++) {
    ...
    if (H5Sselect_hyperslab(fspace, H5S_SELECT_SET, offset, NULL, size, NULL) <
        0)
      TEST_ERROR;

    /* Write to disk */
    if (H5Dwrite(dataset, TEST_DATATYPE, mspace, fspace, H5P_DEFAULT, buf) <
        0) {
      H5_FAILED();
    ...
    if (H5Dread(dataset, TEST_DATATYPE, mspace, fspace, H5P_DEFAULT, check) <
        0) {
      H5_FAILED();
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_sparse(hid_t f, const char *prefix, size_t nblocks,
...
int main(int argc, char *argv[]) {
  ...
  skip_test = 0;
  if (size_of_test & TEST_SMALL) {
    status = test_sparse(file, "sparse", (size_t)100, (size_t)5, (size_t)0,
                         (size_t)0, skip_test);
    nerrors += status < 0 ? 1 : 0;
    status = test_sparse(file, "sparse", (size_t)100, (size_t)3, (size_t)4,
                         (size_t)0, skip_test);
    nerrors += status < 0 ? 1 : 0;
    status = test_sparse(file, "sparse", (size_t)100, (size_t)2, (size_t)3,
                         (size_t)4, skip_test);
    nerrors += status < 0 ? 1 : 0;
  }
  if (size_of_test & TEST_MEDIUM) {
    status = test_sparse(file, "sparse", (size_t)1000, (size_t)30, (size_t)0,
                         (size_t)0, skip_test);
    nerrors += status < 0 ? 1 : 0;
    status = test_sparse(file, "sparse", (size_t)2000, (size_t)7, (size_t)3,
                         (size_t)0, skip_test);
    nerrors += status < 0 ? 1 : 0;
    status = test_sparse(file, "sparse", (size_t)2000, (size_t)4, (size_t)2,
                         (size_t)3, skip_test);
    nerrors += status < 0 ? 1 : 0;
  }
  ...
  if (size_of_test & TEST_LARGE) {
     ...
     */
    status = test_sparse(file, "sparse", (size_t)800, (size_t)50, (size_t)50,
                         (size_t)50, skip_test);
    if (skip_test)
      printf("    The current VFD does not support sparse files on this "
             "platform.\n");
    nerrors += status < 0 ? 1 : 0;
  }
