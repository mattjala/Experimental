...
static const char *FILENAME[] = {"vds_env_virt_0", "vds_env_virt_3",
                                 "vds_env_src_2", "vds_env%%_src2", NULL};
...
static int test_vds_prefix_second(unsigned config, hid_t fapl) {
  ...
  h5_fixname_printf(srcfilenamepct_map_orig, fapl, srcfilenamepct_map,
                    FILENAME_BUF_SIZE);
  ...
  /* Set prefix to a nonexistent directory, will be overwritten by environment
   * variable */
  ...
  if (config & TEST_IO_DIFFERENT_FILE) {
    ...
    if (HDchdir(TMPDIR) < 0)
      TEST_ERROR;
    if ((srcfile[0] =
             H5Fcreate(srcfilename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
      TEST_ERROR;
    if (HDchdir(buffer) < 0)
      TEST_ERROR;
  } else {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      HDsnprintf(msg, sizeof(msg),
                 "Testing virtual dataset with file version bounds: (%s, %s):",
