...
static int read_records(const char *filename, unsigned verbose,
...
static int check_dataset(hid_t fid, unsigned verbose, const char *sym_name,
  ...
  if (snpoints > 0) {
    ...
    if (record->rec_id != start[1] && record->rec_id != (uint64_t)0) {
      ...
      fprintf(
          stderr, "Symbol = '%s', # of records = %lld, record->rec_id = %llx\n",
  ...
  while (curr_time < (time_t)(start_time + (time_t)nseconds)) {
    ...
    if (nrandom > 0) {
      ...
      for (v = 0; v < nrandom; v++) {
        ...
        if (check_dataset(fid, verbose, sym_rand[v]->name, &record, mem_sid) <
            0)
          return -1;
        memset(&record, 0, sizeof(record));
      } /* end for */
...
static void usage(void) {
  ...
  printf("    polling>] [-h <# of common symbols to poll>] [-l <# of random "
         "symbols\n");
...
int main(int argc, char *argv[]) {
  ...
  if (read_records(FILENAME, verbose, (unsigned long)nseconds,
                   (unsigned)poll_time, (unsigned)ncommon,
                   (unsigned)nrandom) < 0) {
