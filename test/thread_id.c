...
static void my_errx(int code, const char *fmt, ...) {
  va_list ap;
...
static void barrier_lock(pthread_barrier_t *barrier) {
  int rc;
...
static void barrier_unlock(pthread_barrier_t *barrier) {
  int rc;
...
static void my_err(int code, const char *fmt, ...) {
  va_list ap;
...
#define threads_failure(_call, _result)                                        \
  do {                                                                         \
    my_errx(EXIT_FAILURE, "%s.%d: " #_call ": %s", __func__, __LINE__,         \
            HDstrerror(_result));                                              \
  } while (false)
...
static void atomic_printf(const char *fmt, ...) {
  char buf[80];
  va_list ap;
  ...
  if (nwritten < nprinted) {
    my_errx(EXIT_FAILURE, "%s.%d: write error or short write", __func__,
            __LINE__);
  }
}
 ...
 */
static void *thread_main(void H5_ATTR_UNUSED *arg) {
  uint64_t ntid, tid;
