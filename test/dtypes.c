...
#define CHECK_NMEMBS(NMEMBS, SRC_ID, DST_ID)                                   \
  do {                                                                         \
    if (H5Tunregister(H5T_PERS_SOFT, NULL, SRC_ID, DST_ID, NULL) < 0)          \
      FAIL_STACK_ERROR;                                                        \
    if (H5Tclose(SRC_ID) < 0 ||                                                \
        ((SRC_ID) != (DST_ID) && H5Tclose(DST_ID) < 0))                        \
      FAIL_STACK_ERROR;                                                        \
...
static const char *FILENAME[] = {"dtypes0", "dtypes1", "dtypes2",  "dtypes3",
                                 "dtypes4", "dtypes5", "dtypes6",  "dtypes7",
                                 "dtypes8", "dtypes9", "dtypes10", NULL};
...
static int test_detect(void) {
  ...
  if ((atom_cmpd_id = H5Tcreate(H5T_COMPOUND, sizeof(struct atomic))) < 0)
    TEST_ERROR;
  if (H5Tinsert(atom_cmpd_id, "i", HOFFSET(struct atomic, i), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(atom_cmpd_id, "f", HOFFSET(struct atomic, f),
                H5T_NATIVE_FLOAT) < 0)
    TEST_ERROR;
  if (H5Tinsert(atom_cmpd_id, "c", HOFFSET(struct atomic, c), H5T_NATIVE_CHAR) <
      0)
    TEST_ERROR;
  if (H5Tinsert(atom_cmpd_id, "d", HOFFSET(struct atomic, d),
                H5T_NATIVE_DOUBLE) < 0)
    TEST_ERROR;
  if (H5Tinsert(atom_cmpd_id, "s", HOFFSET(struct atomic, s),
                H5T_NATIVE_SHORT) < 0)
    TEST_ERROR;
  ...
  if ((cplx_cmpd_id = H5Tcreate(H5T_COMPOUND, sizeof(struct complex))) < 0)
    TEST_ERROR;
  if (H5Tinsert(cplx_cmpd_id, "arr_r", HOFFSET(struct complex, arr_r),
                atom_arr_id) < 0)
    TEST_ERROR;
  if (H5Tinsert(cplx_cmpd_id, "i", HOFFSET(struct complex, i), H5T_NATIVE_INT) <
      0)
    TEST_ERROR;
  if (H5Tinsert(cplx_cmpd_id, "vl_f", HOFFSET(struct complex, vl_f),
                atom_vlf_id) < 0)
    TEST_ERROR;
  if (H5Tinsert(cplx_cmpd_id, "vl_s", HOFFSET(struct complex, vl_s),
                atom_vls_id) < 0)
    TEST_ERROR;
  if (H5Tinsert(cplx_cmpd_id, "c", HOFFSET(struct complex, c),
                H5T_NATIVE_CHAR) < 0)
    TEST_ERROR;
  if (H5Tinsert(cplx_cmpd_id, "s", HOFFSET(struct complex, s),
                H5T_NATIVE_SHORT) < 0)
    TEST_ERROR;
...
static int test_compound_1(void) {
  ...
  /* Add a couple fields */
  if (H5Tinsert(complex_id, "real", HOFFSET(complex_t, re), H5T_NATIVE_DOUBLE) <
      0)
    goto error;
...
static int test_compound_2(void) {
  ...
  if (NULL == (buf = (unsigned char *)malloc(
                   nelmts * MAX(sizeof(struct st), sizeof(struct dt)))))
    goto error;
  ...
  for (i = 0; i < (int)nelmts; i++) {
    ...
    if (s_ptr->a != d_ptr->a || s_ptr->b != d_ptr->b ||
        s_ptr->c[0] != d_ptr->c[0] || s_ptr->c[1] != d_ptr->c[1] ||
        s_ptr->c[2] != d_ptr->c[2] || s_ptr->c[3] != d_ptr->c[3] ||
        s_ptr->d != d_ptr->d || s_ptr->e != d_ptr->e) {
      H5_FAILED();
      printf("    i=%d\n", i);
      printf("    src={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n", s_ptr->a,
             s_ptr->b, s_ptr->c[0], s_ptr->c[1], s_ptr->c[2], s_ptr->c[3],
             s_ptr->d, s_ptr->e);
      printf("    dst={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n", d_ptr->a,
             d_ptr->b, d_ptr->c[0], d_ptr->c[1], d_ptr->c[2], d_ptr->c[3],
             d_ptr->d, d_ptr->e);
      goto error;
    }
  }
...
static int test_compound_3(void) {
  ...
  for (i = 0; i < (int)nelmts; i++) {
    s_ptr = ((struct st *)((void *)orig)) + i;
    d_ptr = ((struct dt *)((void *)buf)) + i;
    if (s_ptr->a != d_ptr->a || s_ptr->c[0] != d_ptr->c[0] ||
        s_ptr->c[1] != d_ptr->c[1] || s_ptr->c[2] != d_ptr->c[2] ||
        s_ptr->c[3] != d_ptr->c[3] || s_ptr->e != d_ptr->e) {
      H5_FAILED();
      printf("    i=%d\n", i);
      printf("    src={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n", s_ptr->a,
             s_ptr->b, s_ptr->c[0], s_ptr->c[1], s_ptr->c[2], s_ptr->c[3],
      ...
      printf("    dst={a=%d, c=[%d,%d,%d,%d], e=%d\n", d_ptr->a, d_ptr->c[0],
             d_ptr->c[1], d_ptr->c[2], d_ptr->c[3], d_ptr->e);
      goto error;
    }
  }
...
static int test_compound_5(void) {
  ...
  /* Check results */
  if (memcmp(src[1].name, dst[1].name, sizeof(src[1].name)) != 0 ||
      src[1].tdim != dst[1].tdim || src[1].coll_ids[0] != dst[1].coll_ids[0] ||
...
static int test_compound_8(void) {
  ...
  if (tsize != (sizeof(char) + sizeof(char) + sizeof(int))) {
    ...
    AT();
    printf(
        "The size of the packed compound datatype is incorrect: tsize = %zu\n",
        tsize);
    goto error;
  } /* end if */
...
static int test_compound_9(void) {
  ...
  if (H5Tinsert(cmpd_tid, "i1", HOFFSET(struct cmpd_struct_w, i1),
                H5T_NATIVE_INT) < 0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tinsert(cmpd_tid, "vl_string", HOFFSET(cmpd_struct_w, str), str_id) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'i1'\n");
  ...
  if (H5Tinsert(cmpd_tid, "i2", HOFFSET(struct cmpd_struct_w, i2),
                H5T_NATIVE_INT) < 0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tcommit2(file, "compound", cmpd_tid, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0) {
    H5_FAILED();
    AT();
  ...
  if ((dset_id = H5Dcreate2(file, "Dataset", dup_tid, space_id, H5P_DEFAULT,
  ...
  if (rdata.i1 != wdata.i1 || rdata.i2 != wdata.i2 ||
      HDstrcmp(rdata.str, wdata.str) != 0) {
    H5_FAILED();
    AT();
...
static int test_compound_10(void) {
  ...
  for (i = 0; i < ARRAY_DIM; i++) {
    ...
    ((char *)(wdata[i].text.p))[len - 2] =
        (char)(((char *)(wdata[i].text.p))[len - 2] + i);
  ...
  if (H5Tinsert(cmpd_tid, "i1", HOFFSET(struct cmpd_struct, i1),
                H5T_NATIVE_INT) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'i1'\n");
  ...
  if (H5Tinsert(cmpd_tid, "vl_string", HOFFSET(cmpd_struct, text), vlstr_id) <
      0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tinsert(cmpd_tid, "i2", HOFFSET(struct cmpd_struct, i2),
                H5T_NATIVE_INT) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'i2'\n");
  ...
  if ((dset_id = H5Dcreate2(file, "Dataset", arr_tid, space_id, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    AT();
    printf("Can't create dataset\n");
...
static int test_compound_11(void) {
  ...
  if (H5Tconvert(big_tid2, little_tid2, (size_t)NTESTELEM, buf, bkg,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Tregister(H5T_PERS_HARD, "opaq_test", opaq_src_tid, opaq_dst_tid,
                  convert_opaque) < 0)
    TEST_ERROR;
  ...
  /* Convert buffer from 'big' to 'little' struct */
  if (H5Tconvert(big_tid, little_tid, (size_t)NTESTELEM, buf, bkg,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Tunregister(H5T_PERS_HARD, "opaq_test", opaq_src_tid, opaq_dst_tid,
                    convert_opaque) < 0)
    TEST_ERROR;
...
static int test_compound_13(void) {
  ...
  if ((attid = H5Acreate2(grpid, COMPOUND13_ATTR_NAME, dtypeid, spaceid,
                          H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static int test_compound_14(void) {
  ...
  if ((cmpd_m1_tid = H5Tcreate(H5T_COMPOUND, sizeof(struct cmpd_struct_1_w))) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't create datatype!\n");
  ...
  if (H5Tinsert(cmpd_m1_tid, "c1", HOFFSET(struct cmpd_struct_1_w, c1),
                H5T_NATIVE_CHAR) < 0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tinsert(cmpd_m1_tid, "c2", HOFFSET(struct cmpd_struct_1_w, c2),
                H5T_NATIVE_CHAR) < 0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tinsert(cmpd_m1_tid, "vl_string", HOFFSET(cmpd_struct_1_w, str),
                str_id) < 0) {
    H5_FAILED();
    AT();
  ...
  if ((cmpd_m2_tid = H5Tcreate(H5T_COMPOUND, sizeof(struct cmpd_struct_2_w))) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't create datatype!\n");
  ...
  if (H5Tinsert(cmpd_m2_tid, "c1", HOFFSET(struct cmpd_struct_2_w, c1),
                H5T_NATIVE_CHAR) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'c1'\n");
  ...
  if (H5Tinsert(cmpd_m2_tid, "c2", HOFFSET(struct cmpd_struct_2_w, c2),
                H5T_NATIVE_CHAR) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'c2'\n");
  ...
  if (H5Tinsert(cmpd_m2_tid, "vl_string", HOFFSET(cmpd_struct_2_w, str),
                str_id) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'vl_string'\n");
  ...
  if (H5Tinsert(cmpd_m2_tid, "l1", HOFFSET(struct cmpd_struct_2_w, l1),
                H5T_NATIVE_LONG) < 0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tinsert(cmpd_m2_tid, "l2", HOFFSET(struct cmpd_struct_2_w, l2),
                H5T_NATIVE_LONG) < 0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tinsert(cmpd_m2_tid, "l3", HOFFSET(struct cmpd_struct_2_w, l3),
                H5T_NATIVE_LONG) < 0) {
    H5_FAILED();
    AT();
  ...
  if (H5Tinsert(cmpd_m2_tid, "l4", HOFFSET(struct cmpd_struct_2_w, l4),
                H5T_NATIVE_LONG) < 0) {
    H5_FAILED();
    AT();
  ...
  if ((cmpd_f2_tid = H5Tcreate(H5T_COMPOUND,
                               8 + 1 + sizeof(hvl_t) + 4 * sizeof(long))) < 0) {
    H5_FAILED();
    AT();
    printf("Can't create datatype!\n");
    goto error;
  ...
  if (H5Tinsert(cmpd_f2_tid, "l1", 8 + 1 + sizeof(hvl_t), H5T_NATIVE_LONG) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'l1'\n");
  ...
  if (H5Tinsert(cmpd_f2_tid, "l2", 8 + 1 + sizeof(hvl_t) + sizeof(long),
                H5T_NATIVE_LONG) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'l2'\n");
  ...
  if (H5Tinsert(cmpd_f2_tid, "l3", 8 + 1 + sizeof(hvl_t) + 2 * sizeof(long),
                H5T_NATIVE_LONG) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'l3'\n");
  ...
  if (H5Tinsert(cmpd_f2_tid, "l4", 8 + 1 + sizeof(hvl_t) + 3 * sizeof(long),
                H5T_NATIVE_LONG) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'l4'\n");
  ...
  if ((dset1_id = H5Dcreate2(file, "Dataset1", cmpd_f1_tid, space_id,
  ...
  if ((dset2_id = H5Dcreate2(file, "Dataset2", cmpd_f2_tid, space_id,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    AT();
    printf("Can't create dataset\n");
  ...
  if (H5Dwrite(dset1_id, cmpd_m1_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, &wdata1) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't write data\n");
  ...
  if (H5Dwrite(dset2_id, cmpd_m2_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, &wdata2) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't write data\n");
  ...
  if (H5Dread(dset1_id, cmpd_m1_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, &rdata1) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't read data\n");
  ...
  if (H5Dread(dset2_id, cmpd_m2_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, &rdata2) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't read data\n");
  ...
  if (rdata1.c1 != wdata1.c1 || rdata1.c2 != wdata1.c2 ||
      HDstrcmp(rdata1.str, wdata1.str) != 0) {
    H5_FAILED();
    AT();
    printf("incorrect read data\n");
    goto error;
  ...
  if (rdata2.c1 != wdata2.c1 || rdata2.c2 != wdata2.c2 ||
      HDstrcmp(rdata2.str, wdata2.str) != 0 || rdata2.l1 != wdata2.l1 ||
      rdata2.l2 != wdata2.l2 || rdata2.l3 != wdata2.l3 ||
      rdata2.l4 != wdata2.l4) {
    H5_FAILED();
    AT();
    printf("incorrect read data\n");
...
static int test_compound_15(void) {
  ...
  if (H5Tinsert(cmpd_f_tid, "i1", HOFFSET(struct cmpd_struct, i1),
                H5T_NATIVE_INT) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'i1'\n");
  ...
  if (H5Tinsert(cmpd_f_tid, "i2", HOFFSET(struct cmpd_struct, i2),
                H5T_NATIVE_INT) < 0) {
    H5_FAILED();
    AT();
    printf("Can't insert field 'i2'\n");
  ...
  if ((dset_id = H5Dcreate2(file, "Dataset", cmpd_f_tid, space_id, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    AT();
    printf("Can't create dataset\n");
  ...
  if (H5Dwrite(dset_id, cmpd_f_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, &wdata1) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't write data\n");
  ...
  if (H5Dwrite(dset_id, cmpd_m_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, &wdata2) <
      0) {
    H5_FAILED();
    AT();
    printf("Can't write data\n");
...
static int test_compound_15_attr(void) {
  ...
  TESTING(
      "compound subset conversion with extra space in source for attribute");
  ...
  if ((attr_id = H5Acreate_by_name(file, ".", "attr_cmpd", cmpd_f_tid, space_id,
                                   H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
...
static int test_compound_16(void) {
  ...
  if ((int_id = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(file, "int", int_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
...
static int test_compound_17(void) {
  ...
  if (H5Tcommit2(file, "arr_int", arr_int, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Tcommit2(file, "arr_ext", arr_ext, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int test_compound_18(void) {
  ...
  hsize_t dim = 1;
  const char *testfile =
      H5_get_srcdir_filename(TESTFILE); /* Corrected test file name */
  char filename[1024];
  ...
  H5E_BEGIN_TRY {
    did = H5Dcreate2(file, "dataset", tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                     H5P_DEFAULT);
  }
  H5E_END_TRY
  if (did > 0) {
  ...
  if (h5_driver_is_default_vfd_compatible(H5P_DEFAULT,
...
static int test_query(void) {
  ...
  if (H5Tcommit2(file, compnd_type, tid1, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0) {
    H5_FAILED();
  ...
  if (H5Tcommit2(file, enum_type, tid2, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0) {
    H5_FAILED();
    printf("Can't commit compound datatype\n");
    goto error;
...
static int test_transient(hid_t fapl) {
  ...
  H5E_BEGIN_TRY {
    ret_id = H5Acreate2(type, "attr1", H5T_NATIVE_INT, space, H5P_DEFAULT,
                        H5P_DEFAULT);
  }
  H5E_END_TRY
  if (ret_id >= 0) {
  ...
  if ((type = H5Tcopy(H5T_NATIVE_INT)) < 0)
    goto error;
  if ((dset = H5Dcreate2(file, "dset1", type, space, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    goto error;
...
static int test_named(hid_t fapl) {
  hid_t file = -1, type = -1, space = -1, dset = -1, t2 = -1, t3 = -1,
        attr1 = -1;
  herr_t status;
  static hsize_t ds_size[2] = {10, 20};
  size_t i, j;
  ...
  H5E_BEGIN_TRY {
    status = H5Tcommit2(file, "test_named_1 (should not exist)", H5T_NATIVE_INT,
  ...
  if ((type = H5Tcopy(H5T_NATIVE_INT)) < 0)
    goto error;
  if (H5Tcommit2(file, "native-int", type, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    goto error;
  ...
  H5E_BEGIN_TRY {
    status = H5Tcommit2(file, "test_named_2 (should not exist)", type,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (status >= 0) {
  ...
  if ((attr1 = H5Acreate2(type, "attr1", H5T_NATIVE_UCHAR, space, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    goto error;
  ...
  if (H5Dclose(dset) < 0)
    goto error;
  if ((dset = H5Dcreate2(file, "dset2", t2, space, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    goto error;
  ...
  if ((dset = H5Dcreate2(file, "dset3", t2, space, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    goto error;
  ...
  if (status) {
    H5_FAILED();
    HDputs("    Datatype from dataset using copied type should not be a named "
           "type!");
    goto error;
  }
  if (H5Tclose(t3) < 0)
  ...
  if ((type = H5Tcopy(H5T_NATIVE_INT)) < 0)
    goto error;
  H5E_BEGIN_TRY {
    status = H5Tcommit2(file, "test_named_3 (should not exist)", type,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (status >= 0) {
...
static int test_conv_enum_1(void) {
  ...
  if (NULL ==
      (buf = (int *)malloc(nelmts * MAX(H5Tget_size(t1), H5Tget_size(t2)))))
    goto error;
  for (u = 0; u < nelmts; u++)
    buf[u] = HDrand() % 26;
...
static int test_conv_enum_2(void) {
  ...
  const char *mname[] = {"RED",  "GREEN",  "BLUE",   "YELLOW",
  ...
  H5Tset_size(
      oddsize,
      (size_t)3); /*reduce to 24 bits, not corresponding to any native size*/
 ...
 *-------------------------------------------------------------------------
 */
static herr_t convert_opaque(hid_t H5_ATTR_UNUSED st, hid_t H5_ATTR_UNUSED dt,
...
static int test_encode(void) {
  ...
  /* Commit enumeration datatype and close it */
  if (H5Tcommit2(file, vlstr_type, tid3, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0) {
    H5_FAILED();
...
static int test_latest(void) {
  ...
  if (H5Tcommit2(file, compnd_type, tid2, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* Get information about datatype on disk */
  if (H5Oget_native_info_by_name(file, compnd_type, &oi, H5O_NATIVE_INFO_HDR,
                                 H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
...
typedef struct {
  unsigned
      num_range_hi; /* Number of H5T_CONV_EXCEPT_RANGE_HI exceptions seen */
  unsigned
      num_range_low; /* Number of H5T_CONV_EXCEPT_RANGE_LOW exceptions seen */
  unsigned
      num_precision; /* Number of H5T_CONV_EXCEPT_PRECISION exceptions seen */
  unsigned
      num_truncate;   /* Number of H5T_CONV_EXCEPT_TRUNCATE exceptions seen */
  unsigned num_other; /* Number of other exceptions seen */
} except_info_t;

static H5T_conv_ret_t
conv_except(H5T_conv_except_t except_type, hid_t H5_ATTR_UNUSED src_id,
            hid_t H5_ATTR_UNUSED dst_id, void H5_ATTR_UNUSED *src_buf,
            void H5_ATTR_UNUSED *dst_buf, void *_user_data) {
...
static int test_int_float_except(void) {
  ...
  float buf_float[CONVERT_SIZE] = {(float)INT_MIN, (float)INT_MAX + 1.0F,
                                   (float)INT_MAX - 127.0F, 4};
  int *intp; /* Pointer to buffer, as integers */
  int buf2[CONVERT_SIZE] = {INT_MIN, INT_MAX, INT_MAX - 72, 0};
  float buf2_float[CONVERT_SIZE] = {(float)INT_MIN, (float)INT_MAX,
                                    (float)INT_MAX - 127.0F, (float)0.0F};
  ...
  if (H5Tconvert(H5T_NATIVE_FLOAT, H5T_NATIVE_INT, (size_t)CONVERT_SIZE, buf,
                 NULL, dxpl) < 0)
    TEST_ERROR;
  ...
  /* Convert buffer */
  memset(&e, 0, sizeof(except_info_t));
  if (H5Tconvert(H5T_NATIVE_INT, H5T_NATIVE_FLOAT, (size_t)CONVERT_SIZE, buf,
                 NULL, dxpl) < 0)
    TEST_ERROR;
  ...
  memset(&e, 0, sizeof(except_info_t));
  if (H5Tconvert(H5T_NATIVE_INT, H5T_NATIVE_FLOAT, (size_t)CONVERT_SIZE, buf2,
                 NULL, dxpl) < 0)
    TEST_ERROR;
  ...
  /* Convert buffer */
  memset(&e, 0, sizeof(except_info_t));
  if (H5Tconvert(H5T_NATIVE_FLOAT, H5T_NATIVE_INT, (size_t)CONVERT_SIZE, buf2,
                 NULL, dxpl) < 0)
    TEST_ERROR;
...
static int test_set_order_compound(hid_t fapl) {
  ...
  hid_t file = -1;
  hid_t cmpd = -1, memb_cmpd = -1, memb_array1 = -1, memb_array2 = -1,
        cmpd_array = -1;
  hid_t vl_id = -1;
  hsize_t dims[2] = {3, 4}; /* Array dimensions */
  char filename[1024];
  ...
  if (H5Tcommit2(file, "compound", cmpd, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
...
static int test_named_indirect_reopen(hid_t fapl) {
  hid_t file = -1, type = -1, reopened_type = -1, strtype = -1, dset = -1,
        space = -1;
  ...
  if (H5Tcommit2(file, "cmpd_type", type, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((dset = H5Dcreate2(file, "cmpd_dset", type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(file, "enum_type", type, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (dt_size != H5Tget_size(type))
    TEST_ERROR;
  ...
  if ((dset = H5Dcreate2(file, "enum_dset", type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(file, "vlen_type", type, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (dt_size != H5Tget_size(type))
    TEST_ERROR;
  ...
  if ((dset = H5Dcreate2(file, "vlen_dset", type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(file, "opaque_type", type, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (dt_size != H5Tget_size(type))
    TEST_ERROR;
  ...
  if ((dset = H5Dcreate2(file, "opaque_dset", type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(file, "array_type", type, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (dt_size != H5Tget_size(type))
    TEST_ERROR;
  ...
  if ((dset = H5Dcreate2(file, "array_dset", type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static void create_del_obj_named_test_file(const char *filename, hid_t fapl,
  ...
  /* Set new format flag.  Note: the case high < low should be caught in the
   * caller */
  use_at_least_v18 = 0;
  if (low >= H5F_LIBVER_V18)
    use_at_least_v18 = 1;

  /* Create a file creation property list (used for the root group's creation
   * property list) */
  ...
  /* Commit datatype */
  status = H5Tcommit2(file, DEL_OBJ_NAMED_NAMED_DTYPE, type, H5P_DEFAULT,
  ...
  /* Create dataset */
  dset = H5Dcreate2(file, DEL_OBJ_NAMED_DATASET, type, space, H5P_DEFAULT, dcpl,
                    H5P_DEFAULT);
  assert(dset > 0);
  ...
  /* Create attribute */
  attr = H5Acreate_by_name(file, DEL_OBJ_NAMED_DATASET, DEL_OBJ_NAMED_ATTRIBUTE,
...
static int test_delete_obj_named(hid_t fapl) {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      if ((attr = H5Aopen_by_name(filea1, DEL_OBJ_NAMED_DATASET,
                                  DEL_OBJ_NAMED_ATTRIBUTE, H5P_DEFAULT,
                                  H5P_DEFAULT)) < 0)
        FAIL_STACK_ERROR;
      if (H5Aclose(attr) < 0)
        FAIL_STACK_ERROR;
      ...
      if (H5Adelete_by_name(filea2, DEL_OBJ_NAMED_DATASET,
                            DEL_OBJ_NAMED_ATTRIBUTE, H5P_DEFAULT) < 0)
        FAIL_STACK_ERROR;
...
static int test_deprec(hid_t fapl) {
  ...
  for (u = 0; u < rank; u++)
    if (rdims[u] != dims[u]) {
      TestErrPrintf("Array dimension information doesn't match!, "
                    "rdims1[%u]=%d, tdims1[%u]=%d\n",
  ...
  for (u = 0; u < rank; u++)
    if (rperm[u] != -2) {
      TestErrPrintf("Array dimension permutation information was modified!, "
                    "rdims1[%u]=%d, tdims1[%u]=%d\n",
                    u, rperm[u], u, perm[u]);
      dim_mismatch = TRUE;
    } /* end if */
  ...
  H5E_BEGIN_TRY {
    status =
        H5Tcommit1(file, "test_named_1 (should not exist)", H5T_NATIVE_INT);
  }
  H5E_END_TRY
  if (status >= 0)
...
static int test_utf_ascii_conv(void) {
  ...
  if ((did = H5Dcreate2(fid, UTF8_DATASET, utf8_vtid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Write the UTF8 string, as UTF8 */
  if ((status = H5Dwrite(did, utf8_vtid, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                         &utf8_w)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((did = H5Dcreate2(fid, ASCII_DATASET, ascii_vtid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Write the ASCII string, as ASCII */
  if ((status = H5Dwrite(did, ascii_vtid, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                         &ascii_w)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create a dataset */
  if ((did = H5Dcreate2(fid, UTF8_DATASET2, utf8_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Write the UTF8 string, as UTF8 */
  if ((status =
           H5Dwrite(did, utf8_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, &utf8_w)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create a dataset */
  if ((did = H5Dcreate2(fid, ASCII_DATASET2, ascii_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Write the ASCII string, as ASCII */
  if ((status = H5Dwrite(did, ascii_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                         &ascii_w)) < 0)
    FAIL_STACK_ERROR;
 ...
 *
 *************************************************************************/
static herr_t verify_version(hid_t dtype, H5F_libver_t low,
                             unsigned *highest_version) {
  hid_t base_dtype = -1;
  hid_t mem_dtype = -1;
  ...
  case H5T_ARRAY: {
    ...
    if (low == H5F_LIBVER_EARLIEST)
      VERIFY(dtypep->shared->version, H5O_DTYPE_VERSION_2,
             "H5O_dtype_ver_bounds");
    else
      VERIFY(dtypep->shared->version, H5O_dtype_ver_bounds[low],
             "H5O_dtype_ver_bounds");
  ...
  case H5T_FLOAT:
  case H5T_INTEGER:
    VERIFY(dtypep->shared->version, H5O_dtype_ver_bounds[H5F_LIBVER_EARLIEST],
           "verify_version");
    break;
  case H5T_NCLASSES:
  case H5T_NO_CLASS:
...
static int test_versionbounds(void) {
  ...
  H5F_libver_t versions[] = {H5F_LIBVER_EARLIEST, H5F_LIBVER_V18,
                             H5F_LIBVER_V110,     H5F_LIBVER_V112,
  ...
  ret = H5Tinsert(simple_cmp_type, "single_int",
                  HOFFSET(simple_cmp_t, single_int), H5T_NATIVE_INT);
  if (ret < 0)
    TEST_ERROR;

  /* Insert array field */
  ret = H5Tinsert(simple_cmp_type, "char_arr", HOFFSET(simple_cmp_t, char_arr),
                  arr_chartype);
  if (ret < 0)
    TEST_ERROR;
  ...
  /* Create the innermost compound datatype that houses inner_arr_type and
   * vlen_floattype */
  ...
  /* Insert integer field */
  ret = H5Tinsert(inner_cmp_type, "inner_arr", HOFFSET(inner_cmp_t, inner_arr),
                  inner_arr_type);
  if (ret < 0)
    TEST_ERROR;

  /* Insert integer field */
  ret = H5Tinsert(inner_cmp_type, "vlen_float",
                  HOFFSET(inner_cmp_t, vlen_float), vlen_floattype);
  if (ret < 0)
    TEST_ERROR;
  ...
  /* Create the outer compound datatype that contains the inner compound
   * datatype and the enum datatype */
  outer_cmp_type = H5Tcreate(H5T_COMPOUND, sizeof(outer_cmp_t));
  if (ret < 0)
    TEST_ERROR;

  /* Insert integer field */
  ret = H5Tinsert(outer_cmp_type, "inner_cmp", HOFFSET(outer_cmp_t, inner_cmp),
                  inner_cmp_type);
  if (ret < 0)
    TEST_ERROR;

  /* Insert enum field */
  ret = H5Tinsert(outer_cmp_type, "enum_color",
                  HOFFSET(outer_cmp_t, enum_color), enum_type);
  if (ret < 0)
    TEST_ERROR;

  /* Create an array datatype containing the outer compound datatype */
  if ((outer_arr_type = H5Tarray_create2(outer_cmp_type, ARRAY_RANK, arr_dim)) <
      0)
    TEST_ERROR;
  ...
  for (low = 0; low < versions_count; low++) {
    ...
    for (high = 0; high < versions_count; high++) {
      ...
      dset = H5Dcreate2(file, VERDSNAME, outer_arr_type, space, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT);
      if (dset < 0)
        TEST_ERROR;
