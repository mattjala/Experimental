...
static int check_dataset(hid_t fid, unsigned verbose,
                         const symbol_info_t *symbol, symbol_t *record,
                         hid_t rec_sid);
static int read_records(const char *filename, unsigned verbose,
                        unsigned long nrecords, unsigned poll_time,
...
static int check_dataset(hid_t fid, unsigned verbose,
  ...
  if (H5Sselect_hyperslab(file_sid, H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    return -1;
  ...
  if (record->rec_id != start[1]) {
    ...
    fprintf(stderr,
            "Symbol = '%s', location = %" PRIuMAX ",%" PRIuMAX
...
static int read_records(const char *filename, unsigned verbose,
  ...
  for (u = 0; u < nrecords; u++) {
    ...
    do {
      ...
      if ((attr_exists =
               H5Aexists_by_name(fid, symbol->name, "seq", H5P_DEFAULT)) < 0)
        return -1;

      if (attr_exists) {
        /* Read sequence number attribute */
        if ((aid = H5Aopen_by_name(fid, symbol->name, "seq", H5P_DEFAULT,
                                   H5P_DEFAULT)) < 0)
          return -1;
      ...
      if (H5Fclose(fid) < 0)
        return -1;
      if ((fid = H5Fopen(filename, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, fapl)) <
          0)
        return -1;
...
static void usage(void) {
  ...
  printf("Usage error!\n");
  printf("\n");
  printf("Usage: swmr_sparse_reader [-q] [-s <# of seconds to wait for "
         "writer>]\n");
  printf("    [-n <# of reads between reopens>] <# of records>\n");
  printf("\n");
  printf("Defaults to verbose (no '-q' given), 1 second wait ('-s 1') and 1 "
         "read\n");
...
int main(int argc, char *argv[]) {
  ...
  if (read_records(FILENAME, verbose, (unsigned long)nrecords,
                   (unsigned)poll_time, (unsigned)reopen_count) < 0) {
