 ...
 *    test_mf_eoa_*()      tests for file meomory that interact with file
 * allocation test_mf_fs_*()       tests for file memory that interact with the
 * free-space manager test_mf_aggr_*()  tests for file memory that interact with
 * the aggregators test_mf_align_*() tests for file memory with alignment
 * setting test_filespace_*() tests for file space management test_page_*()
 * tests for file space paging
 */
...
static unsigned test_mf_align_eoa(const char *env_h5_drvr, hid_t fapl,
                                  hid_t new_fapl);
static unsigned test_mf_align_fs(const char *env_h5_drvr, hid_t fapl,
                                 hid_t new_fapl);
static unsigned test_mf_align_alloc1(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl);
static unsigned test_mf_align_alloc2(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl);
static unsigned test_mf_align_alloc3(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl);
static unsigned test_mf_align_alloc4(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl);
static unsigned test_mf_align_alloc5(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl);
static unsigned test_mf_align_alloc6(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl);
static unsigned test_mf_tmp(const char *env_h5_drvr, hid_t fapl,
                            hbool_t new_format);
static unsigned test_mf_fs_gone(const char *env_h5_drvr, hid_t fapl,
                                hbool_t new_format);
static unsigned test_mf_strat_thres_gone(const char *env_h5_drvr, hid_t fapl,
                                         hbool_t new_format);
static unsigned test_mf_fs_persist(const char *env_h5_drvr, hid_t fapl,
                                   hbool_t new_format);
static unsigned test_mf_strat_thres_persist(const char *env_h5_drvr, hid_t fapl,
                                            hbool_t new_format);
#ifdef PB_OUT
...
static unsigned test_mf_eoa(const char *env_h5_drvr, hid_t fapl) {
   ...
   */
  suitable_vfd = (hbool_t)(HDstrcmp(env_h5_drvr, "split") != 0 &&
...
static unsigned test_mf_eoa_extend(const char *env_h5_drvr, hid_t fapl) {
  ...
  if (suitable_vfd) {
    ...
    /* should succeed */
    was_extended = H5MF_try_extend(
        f, type, (haddr_t)addr, (hsize_t)TBLOCK_SIZE30, (hsize_t)TBLOCK_SIZE50);
  ...
  if (suitable_vfd) {
    ...
    was_extended =
        H5MF_try_extend(f, type, (haddr_t)addr, (hsize_t)(TBLOCK_SIZE30 - 10),
...
static unsigned test_mf_tmp(const char *env_h5_drvr, hid_t fapl,
  ...
  if (!h5_driver_uses_multiple_files(env_h5_drvr, 0)) {
    ...
    /* Create the file to work on */
    if ((file = H5Fcreate(filename, H5F_ACC_TRUNC, fcpl,
                          new_format ? fapl2 : fapl)) < 0)
      FAIL_STACK_ERROR;
    ...
    if (HADDR_UNDEF ==
        (norm_addr = H5MF_alloc(f, H5FD_MEM_DRAW, (hsize_t)(maxaddr / 3))))
      FAIL_STACK_ERROR;
...
static unsigned test_mf_fs_alloc_free(hid_t fapl) {
  ...
  /* Create section A */
  sect_node = H5MF__sect_new(H5MF_FSPACE_SECT_SIMPLE, (haddr_t)TBLOCK_ADDR70,
                             (hsize_t)TBLOCK_SIZE30);

  /* Add section A to free-space manager */
  if (H5MF__add_sect(f, H5FD_MEM_SUPER, f->shared->fs_man[H5FD_MEM_SUPER],
                     sect_node))
    FAIL_STACK_ERROR;
  ...
  if (H5MF__find_sect(f, H5FD_MEM_SUPER, (hsize_t)TBLOCK_SIZE30,
 ...
 *    Succeed: section A adjoins section B (70+30=100 which is section B's
 *address) and requested-size (50) is equal to the size of section B Test 2: Add
 *section A to free-space manager: addr=70, size=30 Allocate a block of size 30
 *from free-space manager Add section B to free-space manager: addr=100, size=50
 *    Try to extend the allocated block by requested-size=60
 *    Fail: section A adjoins section B (70+30=100 which is section B's address)
 *but requested-size (60) > size of section B (50)
 *
 * Test 3:
 *    Add section A to free-space manager: addr=70, size=30
 *    Allocate a block of size 30 from free-space manager
 ...
 *    Succeed: section A adjoins section B (70+30=100 which is section B's
 *address) and requested-size (40) < size of section B (50), therefore, a
 *section of 10 is left in the free-space manager Test 4: Add section A to
 *free-space manager: addr=70, size=20 Allocate a block of size 20 from
 *free-space manager Add section B to free-space manager: addr=100, size=50 Try
 *to extend the allocated block by 50 from the free-space_manager: Fail: section
 *A does not adjoin section B (70+20 != address of section B) even though the
 *requested-size (50) equal to size of section B (50)
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_fs_extend(hid_t fapl) {
  hid_t file = -1;                         /* File ID */
  ...
  /* Create section A */
  sect_node1 = H5MF__sect_new(H5MF_FSPACE_SECT_SIMPLE, (haddr_t)TBLOCK_ADDR70,
                              (hsize_t)TBLOCK_SIZE30);
  ...
  /* Create section B */
  sect_node2 = H5MF__sect_new(H5MF_FSPACE_SECT_SIMPLE, (haddr_t)TBLOCK_ADDR100,
                              (hsize_t)TBLOCK_SIZE50);
  ...
  was_extended =
      H5MF_try_extend(f, H5FD_MEM_SUPER, (haddr_t)TBLOCK_ADDR70,
                      (hsize_t)TBLOCK_SIZE30, (hsize_t)TBLOCK_SIZE50);
  ...
  if (H5MF__find_sect(f, H5FD_MEM_SUPER,
                      (hsize_t)(TBLOCK_SIZE30 + TBLOCK_SIZE50),
                      f->shared->fs_man[H5FD_MEM_SUPER], &tmp) != TRUE)
    TEST_ERROR;
  ...
  was_extended =
      H5MF_try_extend(f, H5FD_MEM_SUPER, (haddr_t)TBLOCK_ADDR70,
                      (hsize_t)TBLOCK_SIZE30, (hsize_t)(TBLOCK_SIZE50 + 10));
  ...
  was_extended =
      H5MF_try_extend(f, H5FD_MEM_SUPER, (haddr_t)TBLOCK_ADDR70,
                      (hsize_t)TBLOCK_SIZE30, (hsize_t)(TBLOCK_SIZE40));
  ...
  /* rest info is same, the extended section returned is merged with the section
   * in free-space */
  ...
  was_extended =
      H5MF_try_extend(f, H5FD_MEM_SUPER, (haddr_t)TBLOCK_ADDR70,
                      (hsize_t)(TBLOCK_SIZE30 - 10), (hsize_t)TBLOCK_SIZE50);
  ...
  if (H5MF__find_sect(f, H5FD_MEM_SUPER, (hsize_t)(TBLOCK_SIZE30 - 10),
                      f->shared->fs_man[H5FD_MEM_SUPER], &tmp) != TRUE)
    TEST_ERROR;
  ...
  if (H5MF__find_sect(f, H5FD_MEM_SUPER, (hsize_t)TBLOCK_SIZE50,
                      f->shared->fs_man[H5FD_MEM_SUPER], &tmp) != TRUE)
    TEST_ERROR;
 ...
 *              section size + remaining size of aggregator is >
 *aggr->alloc_size, section is allowed to absorb an aggregator
 *(allow_sect_absorb is true)
 *
...
static unsigned test_mf_fs_absorb(const char *env_h5_drvr, hid_t fapl) {
  ...
  contig_addr_vfd = (hbool_t)(HDstrcmp(env_h5_drvr, "split") != 0 &&
                              HDstrcmp(env_h5_drvr, "multi") != 0);
  if (contig_addr_vfd) {
    /* Set the filename to use for this test (dependent on fapl) */
    h5_fixname(FILENAME[0], fapl, filename, sizeof(filename));
    ...
    /* Create a section */
    sect_node =
        H5MF__sect_new(H5MF_FSPACE_SECT_SIMPLE, (haddr_t)(ma_addr + ma_size),
                       (hsize_t)TBLOCK_SIZE2048);
  ...
  if (contig_addr_vfd) {
    ...
    sect_node = H5MF__sect_new(H5MF_FSPACE_SECT_SIMPLE, (haddr_t)addr,
                               (hsize_t)TBLOCK_SIZE30);
 ...
 *        request-size is > what is left in meta_aggr and <
 *meta_aggr->alloc_size Result: A block of meta_aggr->alloc_size is allocated
 *from file allocation The first block of 30 is allocated from meta_aggr There
 *is space of 2018 left in meta_aggr
 *
...
static unsigned test_mf_aggr_alloc1(const char *env_h5_drvr, hid_t fapl) {
  ...
  /* Skip test when using VFDs that don't use the metadata aggregator. Also skip
   * test for Direct VFD. */
  suitable_vfd = (hbool_t)(HDstrcmp(env_h5_drvr, "split") != 0 &&
                           HDstrcmp(env_h5_drvr, "multi") != 0 &&
                           HDstrcmp(env_h5_drvr, "direct") != 0);
  if (suitable_vfd) {
    ...
    if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_AGGR, FALSE,
                                   (hsize_t)1) < 0)
      TEST_ERROR;
    ...
    /* Free the two blocks: order matters because of H5F_FSPACE_STRATEGY_AGGR
     * strategy */
 ...
 *    Allocate first block (30) from meta_aggr: (nothing in the aggregator)
 *        request-size is > what is left in meta_aggr and <
 *meta_aggr->alloc_size Result: A block of meta_aggr->alloc_size is obtained via
 *file allocation There is space of 2018 left in meta_aggr
 *
 *    Allocate second block (50) from meta_aggr:
 *        request-size is <= what is left in meta_aggr
 *    Result:
 ...
 *        request-size is > what is left in meta_aggr and is >=
 *meta_aggr->alloc_size meta_aggr is at EOA Result: A block of request-size is
 *extended via file allocation and is merged with meta_aggr The block of 2058 is
 *allocated out of meta_aggr There is space of 1968 left in meta_aggr
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_aggr_alloc2(const char *env_h5_drvr, hid_t fapl) {
  hid_t file = -1;                         /* File ID */
  ...
  if (suitable_vfd) {
    ...
    if (new_file_size !=
        (file_size + TBLOCK_SIZE30 + TBLOCK_SIZE50 + TBLOCK_SIZE2058))
      TEST_ERROR;
    ...
    H5MF_xfree(f, type, addr1,
               (hsize_t)TBLOCK_SIZE30 + TBLOCK_SIZE50 + TBLOCK_SIZE2058);
 ...
 *        request-size is > what is left in meta_aggr and <
 *meta_aggr->alloc_size Result: A block of meta_aggr->alloc_size is obtained via
 *file allocation The first block of 30 is allocated from there There is space
 *of 2018 left in meta_aggr
 *
 *    Allocate second block (50) from meta_aggr:
 *        request-size is <= what is left in meta_aggr
 *    Result:
 ...
 *        request-size is > what is left in other_aggr and is <
 *sdata_aggr->alloc_size Result: A block of sdata_aggr->alloc_size is obtained
 *via file allocation The first block of 30 is allocated from there There is
 *space of 2018 left in sdata_aggr
 *
 *    Allocate the third block (2058) from meta_aggr:
 *        request-size is > what is left in meta_aggr and >=
 *meta_aggr->alloc_size sdata_aggr is at EOA but has not used up more than
 *sdata_aggr->alloc_size Result: A block of request-size is obtained via file
 *allocation The new block's address is returned Nothing is changed in meta_aggr
 *and sdata_aggr
 *
 *    Allocate fourth block (50) from meta_aggr:
 *        request-size is <= what is left in meta_aggr and <
 *meta_aggr->alloc_size Result: The fourth block of 50 is allocated from what is
 *left in meta_aggr There is space of 1968 left in meta_aggr
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_aggr_alloc3(const char *env_h5_drvr, hid_t fapl) {
  hid_t file = -1;                         /* File ID */
  ...
  if (contig_addr_vfd) {
    ...
    if ((addr4 + TBLOCK_SIZE50) != ma_addr)
      TEST_ERROR;
    if (ma_size !=
        (TBLOCK_SIZE2048 - (TBLOCK_SIZE30 + TBLOCK_SIZE50 + TBLOCK_SIZE50)))
      TEST_ERROR;
 ...
 *        request-size is > what is left in meta_aggr and <
 *meta_aggr->alloc_size Result: A block of meta_aggr->alloc_size is obtained via
 *file allocation There is space of 2018 left in meta_aggr The first block of 30
 *is allocated from there
 *
 *    Allocate first block (30) from sdata_aggr: (nothing in sdata_aggr)
 *        request-size is > what is left in sdata_aggr and <
 *sdata_aggr->alloc_size Result: A block of sdata_aggr->alloc_size is obtained
 *via file allocation The first block of 30 is allocated from there
 *
 *    Allocate the second block (2018) from sdata_aggr:
 *        request-size is <= what is left in sdata_aggr and <
 *sdata_aggr->alloc_size request-size is < sdata_aggr->alloc_size Result: The
 *block is allocated from what is left in sdata_aggr (all used up)
 *
 *    Allocate third block (50) from sdata_aggr :
 *        request-size is > what is left in sdata_aggr and <
 *sdata_aggr->alloc_size Result: A block of sdata_aggr->alloc_size is extended
 *via file allocation The third block of 50 is allocated from there There is
 *space of 1998 left in the sdata_aggr
 *
 *    Allocate second block (2058) from meta_aggr:
 *        request-size is > what is left in meta_aggr and >=
 *meta_aggr->alloc_size sdata_aggr is at EOA and has used up more than
 *sdata_aggr->alloc_size Result: unused spaced in sdata_aggr is freed to
 *free-space and is shrunk sdata_aggr is reset to 0 A block of request-size is
 *obtained via file allocation The new block's address is returned The block
 *does not adjoin meta_aggr meta_aggr's info is unchanged
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_aggr_alloc4(const char *env_h5_drvr, hid_t fapl) {
  hid_t file = -1;                         /* File ID */
  ...
  if (contig_addr_vfd) {
    ...
    /* Allocate second block of 2058, which is from file allocation, not from
     * meta_aggr */
 ...
 *        request-size is > what is left in meta_aggr and <
 *meta_aggr->alloc_size Result: A block of meta_aggr->alloc_size is obtained via
 *file allocation The first block of 30 is allocate from there
 *
 *    Allocate second block (50) from meta_aggr:
 *        request-size is < what is left in meta_aggr
 *    Result:
 ...
 *        request-size is > what is left in meta_aggr and is <
 *meta_aggr->alloc_size Result: A block of meta_aggr->alloc_size is extended via
 *file allocation and is absorbed into the meta_aggr The block of 1970 is
 *allocated from there There is space of 2046 left in meta_aggr
 *
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_aggr_alloc5(const char *env_h5_drvr, hid_t fapl) {
 ...
 *        request-size is > what is left in meta_aggr and <
 *meta_aggr->alloc_size Result: A block of meta_aggr->alloc_size is obtained via
 *file allocation The first block of 30 is allocated from there
 *
 *    Allocate second block (50) from meta_aggr:
 *        request-size is <= what is left in meta_aggr
 *    Result:
 ...
 *
 *    Allocate first block (30) from sdata_aggr: (nothing in sdata_aggr)
 *        request-size is > what is left in sdata_aggr and is <
 *sdata_aggr->alloc_size Result: A block of sdata_aggr->alloc_size is obtained
 *via file allocation The first block of 30 is allocated from there There is
 *space of 2018 left in sdata_aggr
 *
 ...
 *        request-size is > what is left in meta_aggr and <
 *meta_aggr->alloc_size sdata_aggr is at EOA but has not used up more than
 *sdata_aggr->alloc_size Result: A block of meta_aggr->alloc_size is obtained
 *via file allocation. The block does not adjoin meta_aggr sdata_aggr is
 *untouched meta_aggr's unused space of [880, 1968] is freed to free-space
 ...
 *        request-size is <= what is left in meta_aggr
 *    Result:
 *        The second block of 50 is allocated from what is left in the
 *aggregator There is space of 1968 left in the meta_aggr
 *
 *    Allocate first block (30) from sdata_aggr: (nothing in sdata_aggr)
 *        request-size is > what is left in sdata_aggr->size and <
 *sdata_aggr->alloc_size Result: A block of sdata_aggr->alloc_size is obtained
 *via file allocation The first block of 30 is allocate from there
 *
 *    Allocate second block (2018) from sdata_aggr:
 *        request-size is <= what is left in sdata_aggr and is <
 *sdata_aggr->alloc_size Result: The second block of 2018 is allocated from what
 *is left in sdata_aggr (all used up)
 *
 *    Allocate third block (50) from sdata_aggr:
 *        request-size is > what is left in sdata_aggr and <
 *sdata_aggr->alloc_size Result: A block of sdata_aggr->alloc_size is extended
 *via file allocation The third block of 50 is allocated from there
 *
 *    Allocate third block (1970) from meta_aggr:
 *        request-size is > what is left in meta_aggr and is <
 *meta_aggr->alloc_size sdata_aggr is at EOA and has used up more than
 *sdata_aggr->alloc_size Result: unused space in sdata_aggr is freed to
 *free-space and is shrunk sdata_aggr is reset to 0 A block of
 *meta_aggr->alloc_size is obtained via file allocation The block does not
 *adjoin meta_aggr meta_aggr's unused space of [880, 1968] is freed to
 *free-space meta_aggr is updated to point to the new block
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_aggr_alloc7(const char *env_h5_drvr, hid_t fapl) {
  hid_t file = -1;             /* File ID */
 ...
 *        Try to extend the block which adjoins the aggregator that is at end of
 *file a. block size < (% * aggr->alloc_size) The block is allocated from the
 *aggregator b. block size > (% * aggr->alloc_size) but block size <
 *aggr->alloc_size The block is extended by aggr->alloc_size and the block is
 *allocated from the aggregator c. block size > (% * aggr->alloc_size) but block
 *size > aggr->alloc_size The block is extended by extended-request and the
 *block is allocated from the aggregator
 *
 *    Test 2: Allocate block A from meta_aggr
 *        Allocate block B from sdata_aggr so that meta_aggr is not at end of
 *file Try to extend a block which adjoins meta_aggr and meta_aggr can fulfill
 *the extended-request H5MF_try_extend() succeeds: the block is extended into
 *the aggregator
 *
 *    Test 3: Allocate block A from meta_aggr
 *        Allocate block B from sdata_aggr so that meta_aggr is not at end of
 *file Try to extend a block which adjoins meta_aggr but meta_aggr cannot
 *fulfill the extended-request H5MF_try_extend() fails
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_aggr_extend(const char *env_h5_drvr, hid_t fapl) {
  hid_t file = -1;             /* File ID */
  ...
  if (contig_addr_vfd) {
    ...
    was_extended = H5MF_try_extend(f, type, (haddr_t)new_addr, (hsize_t)10,
                                   (hsize_t)(TBLOCK_SIZE50));
    ...
    /* Try to extend the block by an amount > (% * aggr->alloc_size) but amount
     * < aggr->alloc_size */
    was_extended = H5MF_try_extend(f, type, (haddr_t)new_addr, (hsize_t)10,
                                   (hsize_t)(TBLOCK_SIZE700));
 ...
 *        H5MF_try_shrink() block B should merge it back to the end of
 *sdata_aggr because sec2 driver is FLMAP_DICHOTOMY by default
 *
...
static unsigned test_mf_aggr_absorb(const char *env_h5_drvr, hid_t fapl) {
  ...
  if (contig_addr_vfd) {
    ...
    H5MF__aggr_query(f, &(f->shared->sdata_aggr), &new_sdata_addr,
                     &new_sdata_size);
    ...
    /* meta_aggr info should be updated because the block is absorbed into the
     * meta_aggr */
    H5MF__aggr_query(f, &(f->shared->meta_aggr), &new_ma_addr, &new_ma_size);
    if (new_ma_addr != ma_addr)
      TEST_ERROR;
...
/*
 *-------------------------------------------------------------------------
 * To verify that a block allocated from file allocation is aligned, can be
 *shrunk and extended
 *
...
static unsigned test_mf_align_eoa(const char *env_h5_drvr, hid_t fapl,
   ...
   */
  suitable_vfd = (hbool_t)(HDstrcmp(env_h5_drvr, "stdio") != 0 &&
                           HDstrcmp(env_h5_drvr, "split") != 0 &&
                           HDstrcmp(env_h5_drvr, "multi") != 0 &&
                           HDstrcmp(env_h5_drvr, "direct") != 0);
  if (suitable_vfd) {
    /* Set the filename to use for this test (dependent on fapl) */
  ...
  if (suitable_vfd) {
    ...
    was_extended =
        H5MF_try_extend(f, type, (haddr_t)addr1, (hsize_t)TBLOCK_SIZE50,
                        (hsize_t)TBLOCK_SIZE30);
 ...
 *
 * Test 1:
 *    Add section A with an aligned address to free-space manager
 *(addr=alignment, size=50) Allocate a block of size=50 The returned space's
 *address should be same as section A's address
 *
 * Test 2:
 *    Add section A to free-space manager (addr=70, size=8000):
 *        section A is mis-aligned but the size is big enough for allocation
 *with alignment Allocate a block of size=600 The returned space should be
 *allocated from section A with an aligned address: address=alignment  size=600
 *    There will be 2 sections in free-space: (alignment = 1024 or alignment =
 *4096) the fragment left from aligning section A: [70, 954] or [70, 4026] the
 *section left after allocating block A: [1624, 416] or [4696, 3374]
 *    H5MF_try_extend() the block of size 600 by 200 should succeed:
 *        the existing fragment left from aligning section A: [70, 954] or [70,
 *4026] the section left after extending block A: [1824, 216] or [4896, 3174]
 *
 * Test 3:
 *    Add section A to free-space manager (addr=70, size=700):
 *        section A is mis-aligned but the size is not big enough for allocation
 *with alignment Allocate a block of size=40 The free-space manager is unable to
 *fulfill the request The block is allocated from file allocation and should be
 *aligned
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_align_fs(const char *env_h5_drvr, hid_t fapl,
  ...
  /* Create section A */
  sect_node = H5MF__sect_new(H5MF_FSPACE_SECT_SIMPLE, (haddr_t)alignment,
                             (hsize_t)TBLOCK_SIZE50);
  ...
  /* try to extend the block */
  was_extended =
      H5MF_try_extend(f, H5FD_MEM_SUPER, (haddr_t)addr, (hsize_t)TBLOCK_SIZE600,
                      (hsize_t)TBLOCK_SIZE200);
  ...
  /* Free the block to free-space manager */
  H5MF_xfree(f, H5FD_MEM_SUPER, addr,
             (hsize_t)(TBLOCK_SIZE600 + TBLOCK_SIZE200));
  ...
  if (suitable_vfd) {
    ...
    /* verify that the allocated block is from file allocation, not section A in
     * free-space */
    if (!(addr >= (haddr_t)file_size))
      TEST_ERROR;
 ...
 *    Result:
 *        An "aggr->alloc_size" block is allocated from file allocation for the
 *aggregator EOA is 3072 The first block of 30 is allocated from the aggregator
 *and should be aligned Fragment from alignment of file allocation is freed to
 *free-space:[800, 224] There is space of 2018 left in meta_aggr
 *
 *    Allocate second block (50) from meta_aggr:
 *        (request-size + fragment size) <= aggr->size
 *    Result:
 *        The second block of 50 is allocated from the aggregator and should be
 *aligned Fragment from alignment of aggregator allocation is freed to
 *free-space:[1054, 994] There is space of 974 left in meta_aggr
 *
 ...
 *        meta_aggr is at EOA
 *    Result:
 *        A block of "meta_aggr->alloc_size" is extended from file allocation
 *for meta_aggr EOA is 5120 The third block of 80 is allocated from the
 *aggregator and should be aligned Fragment from alignment of aggregator
 *allocation is freed to free-space:[2098, 974] There is space of 1968 left in
 *meta_aggr
 *
 ...
 *        meta_aggr is at EOA
 *    Result:
 *        A block of aggr->alloc_size + fragment size - (aggr->alloc_size -
 *request-size)) is extended from file allocation for meta_aggr The third block
 *of 1970 is allocated from the aggregator and should be aligned Fragment from
 *alignment of aggregator allocation is freed to free-space:[3152, 944] There is
 *space of 1968 left in meta_aggr EOA is at 8034
 *
 *
 ...
 *    Result:
 *        A meta_aggr->alloc_size block is allocated from file allocation for
 *the aggregator The first block of 30 is allocated from the aggregator and
 *should be aligned Fragment from alignment of file allocation is freed to
 *free-space:[800, 3296] There is space of 2018 left in meta_aggr EOA is at 6144
 *
 *    Allocate second block (50) from meta_aggr:
 ...
 *        meta_aggr is at EOA
 *    Result:
 *        A block of meta_aggr->alloc_size + (fragment size -
 *(meta_aggr->alloc_size - request-size)) is extended from file allocation for
 *the aggregator The second block of 50 is allocated from the aggregator and
 *should be aligned Fragment from alignment of aggregator allocation is freed to
 *free-space:[4126, 4066] There is space of 2018 left in meta_aggr EOA is at
 *10260
 *
 *    Allocate third block (80) from meta_aggr:
 *        (request-size + fragment size) is > meta_aggr->size
 ...
 *        A block of meta_aggr->alloc_size + (fragment size -
 *(meta_aggr->alloc_size - request-size)) is extended from file allocation for
 *the aggregator The third block of 80 is allocated from the aggregator and
 *should be aligned Fragment from alignment of aggregator allocation is freed to
 *free-space:[8242, 4046] There is space of 2018 left in meta_aggr EOA is at
 *14386
 *
 *    Allocate fourth block (1970) from meta_aggr:
 ...
 *        A block of meta_aggr->alloc_size + (fragment size -
 *(meta_aggr->alloc_size - request-size)) is extended from file allocation for
 *the aggregator The fourth block of 1970 is allocated from the aggregator and
 *should be aligned Fragment from alignment of aggregator allocation is freed to
 *free-space:[12368, 4016] There is space of 2018 left in meta_aggr EOA is at
 *20372
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_align_alloc1(const char *env_h5_drvr, hid_t fapl,
 ...
 *the aggregator The first block of 30 is allocated from the aggregator and
 *should be aligned Fragment from alignment of file allocation is freed to
 *free-space:[800, 224] There is space of 2018 left in meta_aggr EOA is 3072
 *
 *    Allocate second block (50) from meta_aggr:
 *        (request-size+fragment size) <= aggr->size
 *    Result:
 ...
 *        The first block of 30 is allocated from sdata_aggr and should be
 *aligned EOA is 5120
 *
 *    Allocate third block (80) from meta_aggr:
 *        request-size+fragment size is > meta_aggr->size
 *        sdata_aggr is at EOA but has not used up more than
 *sdata_aggr->alloc_size Result: A block of meta_aggr->alloc_size is allocated
 *from file allocation The unused space in meta_aggr is freed to free-space
 *[2098, 974] meta_aggr is updated to point to the new block The third block of
 *80 is allocated from meta_aggr and should be aligned EOA is 7168
 *
 * Alignment = 4096     aggr->alloc_size = 2048
 *
 *    Allocate first block (30) from meta_aggr: (meta_aggr is empty)
 ...
 *the aggregator The first block of 30 is allocated from the aggregator and
 *should be aligned Fragment from alignment of file allocation is freed to
 *free-space:[800, 3296] There is space of 2018 left meta_aggr EOA is at 6144
 *
 *    Allocate second block (50) from meta_aggr:
 *        (request-size + fragment size) > aggr->size
 ...
 *        fragment size > (aggr->alloc_size - request-size)
 *    Result:
 *        A block of (fragment size  + request-size) is extended from file
 *allocation for the aggregator The second block of 50 is allocated from the
 *aggregator and should be aligned Fragment from alignment of aggregator
 *allocation is freed to free-space:[4126, 4066] There is space of 2018 left in
 *meta_aggr EOA is at 10260
 *
 *    Allocate first block (30) from sdata_aggr: (sdata_aggr is empty)
 *        request-size is > sdata_aggr->size and < sdata_aggr->alloc_size
 *        meta_aggr is at EOA and has used up more than meta_aggr->alloc_size
 *    Result:
 *        The remaining space in meta_aggr is freed to free-space [8242, 2018]
 *and shrunk since at EOF meta_aggr is reset to 0 A block of
 *sdata_aggr->alloc_size is obtained via file allocation Fragment from alignment
 *of file allocation is freed to free-space: [8242, 4046] The first block of 30
 *is allocated from sdata_aggr and should be aligned There is space of 2018 left
 *in sdata_aggr EOA is 14336
 *
 *    Allocate third block (80) from meta_aggr:
 *        (request-size + fragment size) is > meta_aggr->size
 *        request-size < meta_aggr->alloc_size
 *        sdata_aggr is at EOA but has not used up more than
 *sdata_aggr->alloc_size Result: A block of meta_aggr->alloc_size is allocated
 *from file allocation for the aggregator Fragment from alignment of file
 *allocation is freed to free-space:[14336, 2048] other_aggr is [12318, 2018]
 *        The third block of 80 is allocated from the aggregator and should be
 *aligned There is space of 1968 left in meta_aggr EOA is at 18432
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_align_alloc2(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl) {
  ...
  if (suitable_vfd) {
     ...
     */
    mis_align = 0;
    if ((alignment == TEST_ALIGN1024) &&
        (tmp = ((ma_addr + ma_size) % alignment)))
      mis_align = alignment - tmp;
    ...
    if ((alignment == TEST_ALIGN1024) && (tmp = (ma_addr % alignment)))
      mis_align = alignment - tmp;
    else if ((alignment == TEST_ALIGN4096) &&
             (tmp = ((sdata_addr + sdata_size) % alignment)))
      mis_align = alignment - tmp;
 ...
 *        request-size is > meta_aggr->size and < meta_aggr->alloc_size
 *    Result:
 *        A block of meta_aggr->alloc_size is allocated from file allocation for
 *the aggregator Fragment from alignment of file allocation is freed to
 *free-space:[800, 224] The first block of 30 is allocated from the aggregator
 *and should be aligned There is space of 2018 left in meta_aggr EOA is 3072
 *
 *    Allocate second block (50) from meta_aggr:
 *        (request-size+fragment size) is <= aggr->size
 *    Result:
 *        The second block of 50 is allocated from the aggregator and should be
 *aligned Fragment from alignment of aggregator allocation is freed to
 *free-space:[1054, 994] There is space of 974 left in the aggregator
 *
 *    Allocate first block (30) from other_aggr: (nothing in other_aggr)
 *        request-size is > what is left in other_aggr->size and <
 *other_aggr->alloc_size Result: A "other_aggr->alloc_size" block is allocated
 *from file allocation for other_aggr The first block of 30 is allocated from
 *other_aggr and should be aligned There is space of 2018 left in
 *other_aggr->size EOA is 5120
 *
 *    Allocate second block (50) from sdata_aggr:
 *        (request-size+fragment size) < sdata_aggr->size
 *    Result:
 *        The second block of 50 is allocated from sdata_aggr and should be
 *aligned Fragment from alignment of aggregator allocation is freed to
 *free-space:[3102, 994] There is space of 974 left in sdata_aggr
 *
 ...
 *    Result:
 *        Another block of sdata_aggr->alloc_size is extended from file
 *allocation for sdata_aggr The third block of 80 is allocated from sdata_aggr
 *and should be aligned Fragment from alignment of aggregator allocation is
 *freed to free-space:[4146, 974] There is space of 1968 left in sdata_aggr EOA
 *is 7168
 *
 *    Allocate third block (1034) from meta_aggregator:
 *        (request-size + alignment) > meta_aggr->size but <
 *meta_aggr->alloc_size sdata_aggr is at EOA and has used up more than
 *sdata_aggr->alloc_size Result: The unused space in sdata_aggr is freed to
 *free-space [5200, 1968] then shrunk sdata_aggr is reset to 0 A block of
 *meta_aggr->alloc_size is allocated from file allocation Fragment from
 *alignment of file allocation is freed to free-space [5200, 944] The unused
 *space in meta_aggr is freed to free-space [2098, 974] The meta_aggr is updated
 *to point to the new space The block of 1034 is allocated from the new block
 *and should be aligned There is space of 1014 left in meta_aggr EOA is 8192
 *
 * Alignment = 4096     aggr->alloc_size = 2048
 *
 *    Allocate first block (30) from meta_aggr: (meta_aggr is empty)
 ...
 *    Result:
 *        A meta_aggr->alloc block is allocated from file allocation for the
 *aggregator The first block of 30 is allocated from the aggregator and should
 *be aligned Fragment from alignment of file allocation is freed to
 *free-space:[800, 3296] There is space of 2018 left in meta_aggr EOA is at 6144
 *
 *    Allocate second block (50) from meta_aggr:
 ...
 *    Result:
 *        A block of aggr->alloc_size + (fragment size - (aggr->alloc_size -
 *request-size)) is extended from file allocation for the aggregator The second
 *block of 50 is allocated from the aggregator and should be aligned Fragment
 *from alignment of aggregator allocation is freed to free-space:[4126, 4066]
 ...
 *    Result:
 *        A block of sdata_aggr->alloc_size + (fragment size -
 *(sdata_aggr->alloc_size - request-size)) is extended from file allocation for
 *the aggregator Fragment from alignment of aggregator allocation is freed to
 *free-space:[12318, 4066] The second block of 50 is allocated from the
 *aggregator and should be aligned There is space of 2018 left in the sdata_aggr
 *        EOA is at 18452
 *
 *    Allocate third block (80) from sdata_aggr:
 *        request-size + fragment size is > sdata_aggr->size
 ...
 *    Result:
 *        A block of sdata_aggr->alloc_size + (fragment size -
 *(sdata_aggr->alloc_size - request-size) is allocated from file allocation for
 *the aggregator Fragment from alignment of aggregator allocation is freed to
 *free-space:[16434, 4046] The third block of 80 is allocated from the
 *aggregator and should be aligned There is space of 2018 left in the sdata_aggr
 *        EOA is at 22578
 *
 *    Allocate third block (1034) from meta_aggregator:
 *        (request-size + fragment size) is > meta_aggr->size but request-size <
 *meta_aggr->alloc_size sdata_aggr is at EOA and has used up more than
 *sdata_aggr->alloc_size Result: The remaining space in sdata_aggr is freed to
 *free-space [20560, 2018] then shrunk sdata_aggr is reset to 0 There is nothing
 *in meta_aggr A block of meta_aggr->alloc_size is allocated from file
 *allocation Fragment from alignment of file allocation is freed to free-space
 *[20560, 4016] EOA is 26624 The meta_aggr is updated to point to the new space
 *        The block of 1034 is allocated from the new block and should be
 *aligned There is space of 1014 left in meta_aggr
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_align_alloc3(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl) {
  ...
  if (suitable_vfd) {
    ...
    /* calculate unused space in meta_aggr that is freed to free-space after
     * block 1034 */
    mis_align = 0;
    if ((alignment == TEST_ALIGN1024) && (tmp = (ma_addr % alignment)))
      mis_align = alignment - tmp;

    /* fragment for unused space in meta_aggr after block 1034 is freed to
     * free-space */
    if (mis_align) {
      state[type].tot_space += mis_align;
      state[type].tot_sect_count += 1;
 ...
 *        Fragment from alignment of file allocation is freed to
 *free-space:[800, 224] The first block of 30 is allocated from meta_aggr and
 *should be aligned There is space of 2018 left in meta_aggr EOA is 3072
 *
 *    Allocate second block (2058) from meta_aggr:
 *        (request-size+fragment) is > meta_aggr->size and request-size is >
 *meta_aggr->alloc_size meta_aggr is at EOA Result: The second block of 2058 +
 *fragment is extended and merged together with meta_aggr The block of 2058 is
 *allocated out of the aggregator Fragment from alignment of aggregator
 *allocation is freed to free-space:[1054, 994] There is space of 2018 (same as
 *before) left in meta_aggr EOA is 6124
 *
 *    Allocate third block (5) from meta_aggr:
 *        request-size+fragment < meta_aggr->size
 *    Result:
 *        A block of 5 is allocated from the aggregator
 *        Fragment from alignment of aggregator allocation is freed to
 *free-space:[4106, 1014] There is space of 999 left in meta_aggr
 *
 * Alignment = 4096     aggr->alloc_size = 2048
 *
 *    Allocate first block (30) from meta_aggr: (meta_aggr is empty)
 ...
 *        A block of meta_aggr->alloc_size is allocated from file allocation
 *        Fragment from alignment of file allocation is freed to
 *free-space:[800, 3296] The first block of 30 is allocated from meta_aggr and
 *should be aligned There is space of 2018 left in meta_aggr EOA is 6144
 *
 *    Allocate second block (2058) from meta_aggr:
 *        (request-size+fragment) is > meta_aggr->size and request-size is >
 *meta_aggr->alloc_size meta_aggr is at EOA Result: The second block of 2058 +
 *fragment is extended and merged together with meta_aggr The block of 2058 is
 *allocated out of the aggregator Fragment from alignment of aggregator
 *allocation is freed to free-space:[4126, 4066] There is space of 2018 (same as
 *before) left in meta_aggr EOA is 12268
 *
 *    Allocate third block (5) from meta_aggr:
 ...
 *        meta_aggr is at EOA
 *    Result:
 *        A block of meta_aggr->alloc_size is extended from file allocation for
 *the aggregator A block of 5 is allocated from the aggregator Fragment from
 *alignment of aggregator allocation is freed to free-space:[10250, 2038] There
 *is space of 2023 left in meta_aggr
 *
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_align_alloc4(const char *env_h5_drvr, hid_t fapl,
 ...
 *        The first block of 30 is allocated from the aggregator and should be
 *aligned There is space of 2018 left in sdata_aggr EOA is 5120
 *
 *    Allocate second block (2058) from meta_aggr:
 *        (request-size + fragment size) > meta_aggr->size and >
 *meta_aggr->alloc_size sdata_aggr is at EOA but has not used up
 *sdata_aggr->alloc_size Result: A block of 2058 is allocated from file
 *allocation EOA is 7178 Nothing is changed in meta_aggr and sdata_aggr
 *
 * Alignment = 4096     aggr->alloc_size = 2048
 *
 *    Allocate first block (30) from meta_aggr: (meta_aggr is empty)
 ...
 *        meta_aggr is at EOA but has not used up more than
 *meta_aggr->alloc_size Result: A block of sdata_aggr->alloc_size is allocated
 *from file allocation Fragment from alignment of file allocation is freed to
 *free-space:[6144, 2048] This fragment adjoins meta_aggr and fulfills "absorb"
 *condition, the remaining space left in meta_aggr is absorbed into the fragment
 *and freed to free-space: [4126, 2018] meta_aggr is reset to 0 The first block
 *of 30 is allocated from the aggregator and should be aligned There is space of
 *2018 left in sdata_aggr EOA is 10240
 *
 *    Allocate second block (2058) from meta_aggr:
 *        request-size + fragment size is > meta_aggr->size
 ...
 *        sdata_aggr is at EOA but has not used up more than
 *sdata_aggr->alloc_size Result: A block of 2058 is allocated from file
 *allocation Fragment from alignment of file allocation is freed to
 *free-space:[10240, 2048] This fragment adjoins sdata_aggr and fulfills
 *"absorb" condition, the remaining space left in sdata_aggr is absorbed into
 *the fragment and freed to free-space: [8222, 2018] sdata_aggr is reset to 0
 *        EOA is 14346
 ...
 *        Fragment from alignment of file allocation is freed to
 *free-space:[800, 224] The first block of 30 is allocated from the aggregator
 *and should be aligned There is space of 2018 left in meta_aggr->size EOA is
 *3072
 *
 *    Allocate first block (30) from sdata_aggr: (sdata_aggr is empty)
 ...
 *        fragment size < (sdata_aggr->alloc_size - request-size)
 *    Result:
 *        Another block of sdata_aggr->alloc_size block is extended from file
 *allocation for sdata_aggr The third block of 80 is allocated from sdata_aggr
 *and should be aligned Fragment from alignment of aggregator allocation is
 *freed to free-space:[4146, 974] There is space of 1968 left in sdata_aggr EOA
 *is 7168
 ...
 *        Fragment from alignment of file allocation is freed to
 *free-space:[5200, 944] EOA is at 8202 meta_aggr is unchanged
 *
 * Alignment = 4096     aggr->alloc_size = 2048
 *
 *    Allocate first block (30) from meta_aggr: (meta_aggr is empty)
 ...
 *        A block of meta_aggr->alloc_size is allocated from file allocation
 *        Fragment from alignment of file allocation is freed to
 *free-space:[800, 3296] The first block of 30 is allocated from the aggregator
 *and should be aligned There is space of 2018 left in meta_aggr EOA is 6144
 *
 *    Allocate first block (30) from sdata_aggr: (sdata_aggr is empty)
 *        request_size > sdata_aggr->size and < sdata_aggr->alloc_size
 *    Result:
 *        A block of sdata_aggr->alloc_size is allocated from file allocation
 *        Fragment from alignment of file allocation is freed to free-space:
 *[6144, 2048] The first block of 30 is allocated from the aggregator and should
 *be aligned There is space of 2018 left in sdata_aggr EOA is 10240
 *
 *    Allocate second block (50) from sdata_aggr:
 *        (request-size+fragment size) is > sdata_aggr->size
 *        request-size < sdata_aggr->alloc_size
 ...
 *    Result:
 *        A block of (fragment size + request-size) is extended from file
 *allocation for the aggregator The second block of 50 is allocated from
 *sdata_aggr and should be aligned Fragment from alignment of aggregator
 *allocation is freed to free-space:[8222, 4066] There is space of 2018 left in
 *sdata_aggr EOA is at 14356
 *
 *    Allocate third block (80) from sdata_aggr:
 *        (request-size+fragment size) is > sdata_aggr->size
 *        request-size < sdata_aggr->alloc_size
 ...
 *allocation for sdata_aggr The third block of 80 is allocated from sdata_aggr
 *and should be aligned Fragment from alignment of aggregator allocation is
 *freed to free-space:[12338, 4046] There is space of 2018 left in sdata_aggr
 *        EOA is 18482
 *
 *    Allocate second block (2058) from meta_aggr:
 *        request-size + fragment size is > meta_aggr->size
 ...
 *    Result:
 *        The remaining space in sdata_aggr is freed to free-space and shrunk:
 *[16464, 2018] sdata_aggr is reset to 0 A block of 2058 is allocated from file
 *allocation Fragment from alignment of file allocation is freed to
 *free-space:[16464, 4016] EOA is at 22538 meta_aggr is unchanged
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_align_alloc6(const char *env_h5_drvr, hid_t fapl,
                                     hid_t new_fapl) {
 ...
 */
static unsigned test_mf_bug1(const char *env_h5_drvr, hid_t fapl) {
  hid_t file = -1;             /* File ID */
  ...
  if (split || multi) {
    if (split) {
      ...
      if (H5Pget_fapl_multi(copied_fapl, NULL, memb_fapl_arr, NULL, NULL,
                            NULL) < 0)
        TEST_ERROR;
      ...
      if (H5Pset_fapl_split(copied_fapl, "-m.h5", memb_fapl_arr[H5FD_MEM_SUPER],
                            "-r.h5", memb_fapl_arr[H5FD_MEM_DRAW]) < 0)
        TEST_ERROR;
    } /* end if */
    else {
      ...
      if (H5Pget_fapl_multi(copied_fapl, memb_map, memb_fapl_arr, memb_name,
                            memb_addr, &relax) < 0)
        TEST_ERROR;
      ...
      if (H5Pset_fapl_multi(copied_fapl, memb_map, memb_fapl_arr,
                            (const char *const *)memb_name, memb_addr,
                            relax) < 0)
        TEST_ERROR;
...
static unsigned test_mf_fs_persist_split(void) {
  ...
  /* Retrieve block #1 from H5FD_MEM_SUPER free-space manager; block #3 still in
   * free-space */
  ...
  /* works for this one because the freeing of sect_addr is to H5FD_MEM_SUPER
   * fs, not against itself */
  ...
  /* There should still be block #5 of H5FD_MEM_BTREE in H5FD_MEM_BTREE
   * free-space manager */
  if (H5Fclose(file) < 0)
    FAIL_STACK_ERROR;
...
static unsigned test_mf_fs_persist_multi(void) {
  ...
  if (H5Pset_fapl_multi(fapl, memb_map, memb_fapl, memb_name, memb_addr, TRUE) <
      0)
    TEST_ERROR;
  ...
  if (H5_addr_defined(f->shared->fs_addr[type])) {
    ...
    if ((node_found =
             H5FS_sect_find(f, f->shared->fs_man[type], (hsize_t)TBLOCK_SIZE1,
    ...
    if ((node_found =
             H5FS_sect_find(f, f->shared->fs_man[type], (hsize_t)TBLOCK_SIZE3,
                            (H5FS_section_info_t **)&node)) < 0)
      FAIL_STACK_ERROR;
    if (node_found)
      TEST_ERROR;
...
/*
 *-------------------------------------------------------------------------
 * Verify that the file's free-space persists where there are free sections in
 *the manager
 *-------------------------------------------------------------------------
 */
static unsigned test_mf_fs_persist(const char *env_h5_drvr, hid_t fapl,
  ...
  H5FS_stat_t fs_stat;         /* Information for free-space manager */
  haddr_t addr1, addr2, addr3, addr4, addr5,
      addr6;        /* File address for H5FD_MEM_SUPER */
  haddr_t tmp_addr; /* Temporary variable for address */
  ...
  if (HDstrcmp(env_h5_drvr, "split") != 0 &&
      HDstrcmp(env_h5_drvr, "multi") != 0) {
    ...
    if (new_format) {
      ...
      if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, TRUE,
                                     (hsize_t)1) < 0)
        TEST_ERROR;
    } else {
    ...
    H5MF__alloc_to_fs_type(f->shared, type, TBLOCK_SIZE6,
                           (H5F_mem_page_t *)&tt);
    ...
    SKIPPED();
    HDputs("    Current VFD doesn't support persisting free-space or paged "
...
static unsigned test_mf_fs_gone(const char *env_h5_drvr, hid_t fapl,
  ...
  if (contig_addr_vfd) {
    ...
    H5MF__alloc_to_fs_type(f->shared, type, TBLOCK_SIZE4,
    ...
    /* Allocate/free space accordingly so that the free-space manager will go
     * away */
    if (new_format) {
...
static unsigned test_mf_strat_thres_persist(const char *env_h5_drvr, hid_t fapl,
  ...
  for (fs_persist = FALSE; fs_persist <= TRUE; fs_persist++) {
    ...
    for (fs_threshold = 0; fs_threshold <= TEST_THRESHOLD10; fs_threshold++) {
      ...
      /* Testing for H5F_FSPACE_STRATEGY_FSM_AGGR and H5F_FSPACE_STRATEGY_PAGE
       * strategies only */
      for (fs_type = H5F_FSPACE_STRATEGY_FSM_AGGR;
           fs_type < H5F_FSPACE_STRATEGY_AGGR; fs_type++) {

        if (!contig_addr_vfd &&
            (fs_persist || fs_type == H5F_FSPACE_STRATEGY_PAGE))
          continue;
        ...
        if (H5Pset_file_space_strategy(fcpl, fs_type, (hbool_t)fs_persist,
                                       fs_threshold) < 0)
          FAIL_STACK_ERROR;
        ...
        if (f->shared->fs_persist) {
          ...
          H5F_sect_info_t
              *sect_info; /* array to hold the free-space information */

          /* Get the # of free-space sections in the file */
          if ((nsects = H5Fget_free_sections(file, H5FD_MEM_DEFAULT, (size_t)0,
                                             NULL)) < 0)
            FAIL_STACK_ERROR;

          /* Verify no free-space sections */
          /* paged aggregation has 1 section for last_small */
          if (fs_threshold > TBLOCK_SIZE5 && nsects &&
              fs_type != H5F_FSPACE_STRATEGY_PAGE)
            TEST_ERROR;

          if (nsects) {
            /* Allocate storage for the free space section information */
            sect_info = (H5F_sect_info_t *)calloc((size_t)nsects,
                                                  sizeof(H5F_sect_info_t));

            H5Fget_free_sections(file, H5FD_MEM_DEFAULT, (size_t)nsects,
                                 sect_info);
...
static unsigned test_mf_strat_thres_gone(const char *env_h5_drvr, hid_t fapl,
  ...
  if (new_format)
    TESTING("File space merge/shrink for section size < threshold with new "
            "library format");
  else
    TESTING("File space merge/shrink for section size < threshold with old "
            "library format");
  ...
  for (fs_persist = FALSE; fs_persist <= TRUE; fs_persist++) {
    ...
    for (fs_type = H5F_FSPACE_STRATEGY_FSM_AGGR;
      ...
      /* Skip for multi/split driver: persisting free-space or paged aggregation
       * strategy */
      if (!contig_addr_vfd &&
          (fs_persist || fs_type == H5F_FSPACE_STRATEGY_PAGE))
        continue;
      ...
      /* Set default file space information */
      if (H5Pset_file_space_strategy(fcpl, fs_type, fs_persist,
                                     (hsize_t)TEST_THRESHOLD3) < 0)
        FAIL_STACK_ERROR;
      ...
      /* For paged aggregation, the section in the page at EOF for small meta fs
       * is not shrunk away */
      ...
      /* For paged aggregation, the sections in the page at EOF for small meta
       * fs are merged but are not shrunk away */
      if (fs_type == H5F_FSPACE_STRATEGY_PAGE) {
      ...
      if (check_stats(f, f->shared->fs_man[tt],
                      (fs_type == H5F_FSPACE_STRATEGY_PAGE) ? &fs_state
                                                            : &fs_state_zero))
        TEST_ERROR;
...
/*
 *-------------------------------------------------------------------------
 * To verify that file space is allocated from the corresponding free-space
 *manager because H5FD_FLMAP_DICHOTOMY is used as the default free-list mapping.
 *
 ...
 *-------------------------------------------------------------------------
 *  set_multi_split():
 *      Internal routine to set up page-aligned address space for multi/split
 *driver when testing paged aggregation.
 *-------------------------------------------------------------------------
 */
static int set_multi_split(hid_t fapl, hsize_t pagesize,
                           hbool_t is_multi_or_split) {
  H5FD_mem_t memb_map[H5FD_MEM_NTYPES];
  hid_t memb_fapl_arr[H5FD_MEM_NTYPES];
  char *memb_name[H5FD_MEM_NTYPES];
  haddr_t memb_addr[H5FD_MEM_NTYPES];
  ...
  if (H5Pget_fapl_multi(fapl, memb_map, memb_fapl_arr, memb_name, memb_addr,
                        &relax) < 0)
    TEST_ERROR;
  ...
  if (is_multi_or_split) {
    ...
    memb_addr[H5FD_MEM_SUPER] =
        ((memb_addr[H5FD_MEM_SUPER] + pagesize - 1) / pagesize) * pagesize;
    memb_addr[H5FD_MEM_DRAW] =
        ((memb_addr[H5FD_MEM_DRAW] + pagesize - 1) / pagesize) * pagesize;
  } else {
    /* Set memb_addr aligned */
    for (mt = H5FD_MEM_DEFAULT; mt < H5FD_MEM_NTYPES; mt++)
  ...
  /* Set multi driver with new FAPLs */
  if (H5Pset_fapl_multi(fapl, memb_map, memb_fapl_arr,
                        (const char *const *)memb_name, memb_addr, relax) < 0)
    TEST_ERROR;
...
static unsigned test_page_alloc_xfree(const char *env_h5_drvr, hid_t fapl) {
  ...
  if (!multi && !split) {
    ...
    for (fs_persist = FALSE; fs_persist <= TRUE; fs_persist++) {
      ...
      if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, fs_persist,
                                     (hsize_t)1) < 0)
        TEST_ERROR;
      ...
      if (!fs_persist) {
        H5MF__alloc_to_fs_type(f->shared, H5FD_MEM_OHDR, TBLOCK_SIZE1034,
                               (H5F_mem_page_t *)&fs_type);

        /* Verify that the freed block with addr2 is found from the small
         * metadata manager */
        if (H5MF__find_sect(f, H5FD_MEM_OHDR, (hsize_t)TBLOCK_SIZE1034,
      ...
      if (!fs_persist) {
        /* Verify that the freed block with saddr1 is found from the small raw
         * data manager */
        if (H5MF__find_sect(f, H5FD_MEM_DRAW, (hsize_t)TBLOCK_SIZE30,
                            f->shared->fs_man[H5F_MEM_PAGE_DRAW],
                            &found_addr) < 0)
          TEST_ERROR;
      ...
      if (!fs_persist) {
        H5MF__alloc_to_fs_type(f->shared, H5FD_MEM_DRAW, TBLOCK_SIZE5000,
                               (H5F_mem_page_t *)&fs_type);

        /* Verify that the freed block with gaddr1 is found from the large data
         * manager */
        if (H5MF__find_sect(f, H5FD_MEM_DRAW, (hsize_t)TBLOCK_SIZE8192,
                            f->shared->fs_man[fs_type], &found_addr) < 0)
          TEST_ERROR;
      ...
      if (fs_persist) {
        ...
        H5MF__alloc_to_fs_type(f->shared, H5FD_MEM_OHDR,
                               f->shared->fs_page_size - 1,
                               (H5F_mem_page_t *)&fs_type);
        if (!H5_addr_defined(f->shared->fs_addr[fs_type]))
          TEST_ERROR;
...
static unsigned test_page_try_shrink(const char *env_h5_drvr, hid_t fapl) {
  ...
  if (contig_addr_vfd) {
    ...
    /* Set the strategy to paged aggregation */
    if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, FALSE,
                                   (hsize_t)1) < 0)
      FAIL_STACK_ERROR;
    ...
    if ((status = H5MF_try_shrink(f, H5FD_MEM_OHDR, addr1,
                                  (hsize_t)TBLOCK_SIZE50)) < 0)
      FAIL_STACK_ERROR;

    /* Couldn't shrink due to the section (remaining space in the page) is in
     * the small metadata free-space manager */
    if (status == TRUE)
      TEST_ERROR;
    ...
    if ((status = H5MF_try_shrink(f, H5FD_MEM_DRAW, saddr1,
                                  (hsize_t)TBLOCK_SIZE50)) < 0)
      FAIL_STACK_ERROR;

    /* Couldn't shrink due to the section (remaining space in the page) is in
     * the small raw data free-space manager */
    if (status == TRUE)
      TEST_ERROR;
    ...
    if ((status = H5MF_try_shrink(f, H5FD_MEM_DRAW, gaddr1,
                                  (hsize_t)TBLOCK_SIZE5000)) < 0)
      FAIL_STACK_ERROR;

    /* Couldn't shrink due to the section (remaining space in the page) is in
     * the large-sized free-space manager */
    if (status == TRUE)
      TEST_ERROR;

    /* Free the block with saddr1--merge to become 1 page, then return to the
     * large manager */
 ...
 *
 * Purpose:     To verify that extending a small block via H5MF_try_extend()
 *works properly when paged aggregation is enabled.
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 *
 *-------------------------------------------------------------------------
 */
static unsigned test_page_small_try_extend(const char *env_h5_drvr,
                                           hid_t fapl) {
  ...
  if (contig_addr_vfd) {
    ...
    was_extended = H5MF_try_extend(f, H5FD_MEM_OHDR, (haddr_t)addr1,
                                   (hsize_t)TBLOCK_SIZE98, (hsize_t)3100);
    /* Should succeed */
    if (was_extended != TRUE)
      TEST_ERROR;

    /* Allocate 2 small metadata blocks with addr2 and addr3--will be on another
     * metadata page */
    ...
    /* Try extending the block with addr1 that will cross to the next page where
     * the freed block with addr2 resides */
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_OHDR, (haddr_t)addr1,
                        (hsize_t)TBLOCK_SIZE3198, (hsize_t)TBLOCK_SIZE100);
    /* Shouldn't succeed--should not cross page boundary */
    if (was_extended == TRUE)
      TEST_ERROR;

    /* Try extending the block with addr1 into the free-space section that is
     * big enough to fulfill the request */
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_OHDR, (haddr_t)addr1,
                        (hsize_t)TBLOCK_SIZE3198, (hsize_t)TBLOCK_SIZE50);
    /* Should succeed */
    if (was_extended != TRUE)
      TEST_ERROR;
    ...
    /* There is a page end threshold of size H5F_FILE_SPACE_PGEND_META_THRES at
     * the end of the block */
    /* The block is right next to the threshold */
    addr1 = H5MF_alloc(f, H5FD_MEM_OHDR, (hsize_t)TBLOCK_SIZE3286);

    /* Try extending the block into the threshold with size >
     * H5F_FILE_SPACE_PGEND_META_THRES */
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_OHDR, (haddr_t)addr1,
                        (hsize_t)TBLOCK_SIZE3286, (hsize_t)TBLOCK_SIZE11);
    /* Shouldn't succeed */
    if (was_extended == TRUE)
      TEST_ERROR;
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_OHDR, (haddr_t)addr1,
                        (hsize_t)TBLOCK_SIZE3286, (hsize_t)TBLOCK_SIZE2);
    /* Should succeed */
    if (was_extended != TRUE)
      TEST_ERROR;

    /* Free the block with addr3--will merge with the remaining sections to
     * become a page and then free the page */
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)saddr1,
                        (hsize_t)TBLOCK_SIZE4086, (hsize_t)TBLOCK_SIZE11);
    /* Shouldn't succeed */
    if (was_extended == TRUE)
      TEST_ERROR;
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)saddr1,
                        (hsize_t)TBLOCK_SIZE4086, (hsize_t)TBLOCK_SIZE10);
    /* Should succeed */
    if (was_extended != TRUE)
      TEST_ERROR;
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)saddr1,
                        (hsize_t)TBLOCK_SIZE4096, (hsize_t)TBLOCK_SIZE10);
    /* Should succeed */
    if (was_extended != TRUE)
      TEST_ERROR;
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)saddr1,
                        (hsize_t)TBLOCK_SIZE4106, (hsize_t)TBLOCK_SIZE5000);
    /* Should not succeed because the mis-aligned fragment in the page is in the
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_page_large_try_extend(const char *env_h5_drvr,
                                           hid_t fapl) {
  ...
  if (contig_addr_vfd) {
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)gaddr1,
                        (hsize_t)TBLOCK_SIZE6000, (hsize_t)TBLOCK_SIZE2192);
    /* Should succeed */
    if (was_extended != TRUE)
      TEST_ERROR;
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)gaddr1,
                        (hsize_t)TBLOCK_SIZE8192, (hsize_t)TBLOCK_SIZE50);
    /* Should not succeed */
    if (was_extended == TRUE)
      TEST_ERROR;
    ...
    /* Try extending the block with gaddr2--there is a free-space section big
     * enough to fulfill the request */
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)gaddr2,
                        (hsize_t)TBLOCK_SIZE8000, (hsize_t)TBLOCK_SIZE100);
    /* Should succeed */
    if (was_extended == FALSE)
      TEST_ERROR;

    /* Try extending the block with gaddr2--there is no free-space section big
     * enough to fulfill the request */
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)gaddr2,
                        (hsize_t)TBLOCK_SIZE8100, (hsize_t)TBLOCK_SIZE100);
    /* Should not succeed */
    if (was_extended == TRUE)
      TEST_ERROR;
    ...
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)gaddr2,
                        (hsize_t)TBLOCK_SIZE8100, (hsize_t)TBLOCK_SIZE90);
    /* Should succeed */
    if (was_extended == FALSE)
      TEST_ERROR;

    /* Try extending the block with gaddr2 */
    /* There is no free-space section big enough to fulfill the request (request
     * is < H5F_FILE_SPACE_PGEND_META_THRES) */
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)gaddr2,
                        (hsize_t)TBLOCK_SIZE8190, (hsize_t)TBLOCK_SIZE5);
    /* Should not succeed */
    if (was_extended == TRUE)
      TEST_ERROR;
    ...
    /* Free the block with gaddr3--will merge with remaining free space to
     * become 2 pages + section (size 2) in previous page */
    H5MF_xfree(f, H5FD_MEM_DRAW, gaddr3, (hsize_t)TBLOCK_SIZE8000);

    /* Try extending the block with gaddr2 crossing page boundary--there is
     * free-space section big enough to fulfill the request */
    was_extended =
        H5MF_try_extend(f, H5FD_MEM_DRAW, (haddr_t)gaddr2,
                        (hsize_t)TBLOCK_SIZE8190, (hsize_t)TBLOCK_SIZE5);
    /* Should succeed */
...
static unsigned test_page_large(const char *env_h5_drvr, hid_t fapl) {
  ...
  TESTING(
      "Paged aggregation for file space: large allocations and de-allocations");
  ...
  if (contig_addr_vfd) {
    ...
    /* Get free-space info */
    if (H5FS_stat_info(f, f->shared->fs_man[H5F_MEM_PAGE_GENERIC], &fs_stat) <
        0)
      FAIL_STACK_ERROR;
    ...
    /* Allocate a large data block with gaddr4--there is a free-space section
...
static unsigned test_page_small(const char *env_h5_drvr, hid_t fapl) {
  ...
  TESTING(
      "Paged aggregation for file space: small allocations and de-allocations");
  ...
  if (!multi && !split) {
    ...
    /* Allocate a small metadata block with addr3--there is no free-space
     * section big enough to fulfill the request */
    ...
    /* Allocate a small metadata block with addr4--there is a free-space section
     * big enough to fulfill the request */
    ...
    /* Allocate a small metadata block with addr5--there is a free-space section
     * big enough to fulfill the request */
    ...
    /* Allocate a small metadata block with addr6--taking up the remaining space
     * in the first page */
    if (family)
    ...
    /* Allocate a small metadata block with addr7--taking up the remaining space
     * in the third page */
    H5MF_alloc(f, H5FD_MEM_OHDR, (hsize_t)TBLOCK_SIZE36);

    /* Allocate 2 small metadata blocks: addr8, addr9--there is no free-space to
     * fulfill the request */
    H5MF_alloc(f, H5FD_MEM_OHDR, (hsize_t)TBLOCK_SIZE50);
    addr9 = H5MF_alloc(f, H5FD_MEM_OHDR, (hsize_t)TBLOCK_SIZE80);

    /* Free the block with saddr1 and saddr2--merge with remaining section to
     * become a page which will be returned to the large manager */
    H5MF_xfree(f, H5FD_MEM_DRAW, saddr1, (hsize_t)TBLOCK_SIZE30);
    H5MF_xfree(f, H5FD_MEM_DRAW, saddr2, (hsize_t)TBLOCK_SIZE50);

    /* Verify that the large manager does contain a section with file space page
    ...
    /* Allocate a small metadata block with addr10--there is a free-space
     * section big enough to fulfill the request */
    ...
    /* The current free-space section is unable to fulfill the request; obtain a
     * page from the large manager */
...
static unsigned test_page_alignment(const char *env_h5_drvr, hid_t fapl) {
  ...
  TESTING("Paged aggregation and H5Pset_alignment: verify proper alignment is "
          "used");
  ...
  if (!multi && !split) {
     ...
     */
    if (H5Pset_libver_bounds(fapl_new, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) <
        0)
      FAIL_STACK_ERROR;
    ...
    if (split || multi) {
      ...
      if (split) {
        /* Set split driver with new FAPLs */
        if (H5Pset_fapl_split(fapl_new, "-m.h5", memb_fapl, "-r.h5",
                              memb_fapl) < 0)
          TEST_ERROR;

        /* Get current multi settings */
        if (H5Pget_fapl_multi(fapl_new, memb_map, memb_fapl_arr, memb_name,
                              memb_addr, &relax) < 0)
          TEST_ERROR;

        /* Set memb_addr aligned */
        memb_addr[H5FD_MEM_SUPER] =
            ((memb_addr[H5FD_MEM_SUPER] + TBLOCK_SIZE4096 - 1) /
             TBLOCK_SIZE4096) *
            TBLOCK_SIZE4096;
        memb_addr[H5FD_MEM_DRAW] =
            ((memb_addr[H5FD_MEM_DRAW] + TBLOCK_SIZE4096 - 1) /
             TBLOCK_SIZE4096) *
            TBLOCK_SIZE4096;

        /* Set split driver with new FAPLs */
        if (H5Pset_fapl_multi(fapl_new, memb_map, memb_fapl_arr,
                              (const char *const *)memb_name, memb_addr,
                              relax) < 0)
          TEST_ERROR;
      } else {
        /* Get current multi settings */
        if (H5Pget_fapl_multi(fapl_new, memb_map, NULL, memb_name, memb_addr,
                              &relax) < 0)
          TEST_ERROR;
        ...
        for (mt = H5FD_MEM_DEFAULT; mt < H5FD_MEM_NTYPES; mt++) {
          ...
          memb_addr[mt] =
              ((memb_addr[mt] + TBLOCK_SIZE4096 - 1) / TBLOCK_SIZE4096) *
              TBLOCK_SIZE4096;
        }
    ...
    /* Should be on the second page and page aligned on 4096 (default file space
     * page size) */
    if (saddr1 % TBLOCK_SIZE4096)
      TEST_ERROR;
    ...
    if ((addr1 + TBLOCK_SIZE30) % TEST_ALIGN16)
      if (addr2 !=
          (((addr1 + TBLOCK_SIZE30) / TEST_ALIGN16) + 1) * TEST_ALIGN16)
        TEST_ERROR;
    ...
    if (!multi && !split) {
      ...
      if ((addr2 + TBLOCK_SIZE50) % TEST_ALIGN16)
        if (saddr1 !=
            (((addr2 + TBLOCK_SIZE50) / TEST_ALIGN16) + 1) * TEST_ALIGN16)
          TEST_ERROR;
    }
    ...
    if ((saddr1 + TBLOCK_SIZE80) % TEST_ALIGN16)
      if (saddr2 !=
          (((saddr1 + TBLOCK_SIZE80) / TEST_ALIGN16) + 1) * TEST_ALIGN16)
        TEST_ERROR;
    ...
    /* Set strategy to H5F_FSPACE_STRATEGY_AGGR but meta/raw data block is 0 as
     * set in fapl_new */
    if (H5Pset_file_space_strategy(fcpl2, H5F_FSPACE_STRATEGY_AGGR, FALSE,
                                   (hsize_t)1) < 0)
      TEST_ERROR;

    /* fapl_new has latest format, H5Pset_alignment set, and disable meta/raw
     * block */
    ...
    if ((saddr1 + TBLOCK_SIZE30) % TEST_ALIGN16)
      if (saddr2 !=
          (((saddr1 + TBLOCK_SIZE30) / TEST_ALIGN16) + 1) * TEST_ALIGN16)
        TEST_ERROR;
    ...
    if ((saddr2 + TBLOCK_SIZE50) % TEST_ALIGN16)
      if (gaddr1 !=
          (((saddr2 + TBLOCK_SIZE50) / TEST_ALIGN16) + 1) * TEST_ALIGN16)
        TEST_ERROR;

    /* There is no free-space manager involved for H5F_FSPACE_STRATEGY_AGGR
     * strategy */
...
int main(void) {
  ...
  hid_t new_fapl =
      -1; /* File access property list for alignment & aggr setting */
  ...
  /* Temporary: modify to skip testing for multi/split driver:
       fail file create when persisting free-space or using paged aggregation
     strategy */
   ...
   */
  /* Temporary: The following 7 tests are modified to skip testing for
     multi/split driver: fail file create when persisting free-space or using
     paged aggregation strategy */
  ...
  nerrors +=
      test_page_alloc_xfree(env_h5_drvr, fapl); /* can handle multi/split */
  nerrors +=
      test_page_alignment(env_h5_drvr, fapl); /* can handle multi/split */
