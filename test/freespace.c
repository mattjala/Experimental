...
static herr_t TEST_sect_can_merge(const H5FS_section_info_t *,
                                  const H5FS_section_info_t *,
                                  void H5_ATTR_UNUSED *);
static herr_t TEST_sect_merging(H5FS_section_info_t **, H5FS_section_info_t *,
                                void H5_ATTR_UNUSED *);
...
const H5FS_section_class_t *test_classes[] = {TEST_FSPACE_SECT_CLS,
                                              TEST_FSPACE_SECT_CLS_NEW,
                                              TEST_FSPACE_SECT_CLS_NOINIT};

static void init_cparam(H5FS_create_t *);
static void init_sect_node(TEST_free_section_t *, haddr_t, hsize_t, unsigned,
...
TEST_sect_can_merge(const H5FS_section_info_t *_sect1,
  ...
  ret_value = H5_addr_eq(sect1->sect_info.addr + sect1->sect_info.size,
 ...
 */
static herr_t TEST_sect_merging(H5FS_section_info_t **_sect1,
 ...
 */
static herr_t TEST_sect_can_shrink(const H5FS_section_info_t *_sect,
                                   void *_udata) {
...
static haddr_t TEST_get_eoa(void) { return g_eoa; }

/* supporting routine for shrinking */
static void TEST_set_eoa(haddr_t val) { g_eoa = val; }
 ...
 */
static void init_sect_node(TEST_free_section_t *sect_node, haddr_t addr,
                           hsize_t size, unsigned type,
 ...
 */
static int check_stats(const H5F_t *f, const H5FS_t *frsp,
...
static unsigned test_fs_create(hid_t fapl) {
  ...
  if (NULL == (frsp = H5FS_create(f, &fs_addr, &cparam, nclasses, test_classes,
                                  &init_flags, (hsize_t)FSPACE_THRHD_DEF,
  ...
  if (NULL ==
      (frsp = H5FS_open(f, fs_addr, nclasses, test_classes, NULL,
 ...
 *
 * Test 3:
 *	Add section A via H5FS_sect_add() to allow shrinking with
 *H5FS_ADD_RETURNED_SPACE Set EOF to be the ending address of section A Result:
 *H5FS_sect_add() will shrink section A
 *
 * Test 4:
 *	Add section A via H5FS_sect_add() to allow shrinking with
 *H5FS_ADD_DESERIALIZING Set EOF to be the ending address of section A Result:
 *H5FS_sect_add() will not shrink section A
 *
 */
static unsigned test_fs_sect_add(hid_t fapl) {
  hid_t file = -1;               /* File ID */
  ...
  if (NULL ==
      (sect_node = (TEST_free_section_t *)malloc(sizeof(TEST_free_section_t))))
    FAIL_STACK_ERROR;

  init_sect_node(sect_node, (haddr_t)TEST_SECT_ADDR80,
                 (hsize_t)TEST_SECT_SIZE20, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  TEST_set_eoa(
      (haddr_t)TEST_SECT_ADDR150); /* set end of file address for shrinking */
   ...
   * Add section A
   */
  init_sect_node(sect_node, (haddr_t)TEST_SECT_ADDR100,
                 (hsize_t)TEST_SECT_SIZE50, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
 ...
 *
 * Test 2: Add a section and find the section whose size is equal to the
 *requested-size Set up: Add section A whose size is less than requested-size
 *		Add section B whose size is the same as requested-size with
 *addr=b Add section C whose size is the same as requested-size with addr=c > b
 ...
 *
 * Test 4: Add a section but the section is not able to fulfill the
 *requested-size Set up:	Add section A whose size is less than
 *requested-size
 *
 */
static unsigned test_fs_sect_find(hid_t fapl) {
  ...
  TEST_free_section_t *sect_node1 = NULL, *sect_node2, *sect_node3 = NULL,
                      *sect_node4 = NULL;
  ...
  if ((node_found = H5FS_sect_find(f, frsp, (hsize_t)TEST_SECT_SIZE30,
   ...
   */
  if (NULL ==
      (sect_node1 = (TEST_free_section_t *)malloc(sizeof(TEST_free_section_t))))
    FAIL_STACK_ERROR;

  init_sect_node(sect_node1, (haddr_t)TEST_SECT_ADDR60,
                 (hsize_t)TEST_SECT_SIZE30, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
   ...
   */
  if (NULL ==
      (sect_node3 = (TEST_free_section_t *)malloc(sizeof(TEST_free_section_t))))
    FAIL_STACK_ERROR;

  init_sect_node(sect_node3, (haddr_t)(TEST_SECT_ADDR200),
                 (hsize_t)TEST_SECT_SIZE50, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
   ...
   */
  if (NULL ==
      (sect_node2 = (TEST_free_section_t *)malloc(sizeof(TEST_free_section_t))))
    FAIL_STACK_ERROR;

  init_sect_node(sect_node2, (haddr_t)TEST_SECT_ADDR100,
                 (hsize_t)TEST_SECT_SIZE50, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
   ...
   */
  if (NULL ==
      (sect_node4 = (TEST_free_section_t *)malloc(sizeof(TEST_free_section_t))))
    FAIL_STACK_ERROR;

  init_sect_node(sect_node4, (haddr_t)TEST_SECT_ADDR300,
                 (hsize_t)TEST_SECT_SIZE80, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  if ((node_found = H5FS_sect_find(f, frsp, (hsize_t)TEST_SECT_SIZE50,
                                   (H5FS_section_info_t **)&node)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((node->sect_info.addr != TEST_SECT_ADDR100) ||
      (node->sect_info.size != TEST_SECT_SIZE50))
    TEST_ERROR;
  ...
  TESTING(
      "H5FS_sect_find() a section greater than requested-size from free-space");
  ...
  init_sect_node(sect_node2, (haddr_t)TEST_SECT_ADDR200,
                 (hsize_t)TEST_SECT_SIZE80, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  if (!node_found)
    TEST_ERROR;
  if ((node->sect_info.addr != TEST_SECT_ADDR200) ||
      (node->sect_info.size < TEST_SECT_SIZE50))
    TEST_ERROR;
 ...
 *	H5FS_ADD_RETURNED_SPACE is passed to H5FS_sect_add()
 *
 *   Add sections A & B that can be merged together but cannot do so because
 *H5FS_CLS_SEPAR_OBJ flag is set
 *
 * Test 3:
 *   Set up:
 *	H5FS_CLS_SEPAR_OBJ (cls->flags) is not set
 ...
 *	section A cannot be merged with the merged section of B & C because of
 *different section class type section D cannot be merged with the merged
 *section of B & C because of different section class type
 */
static unsigned test_fs_sect_merge(hid_t fapl) {
  hid_t file = -1;               /* File ID */
  char filename[FILENAME_LEN];   /* Filename to use */
  ...
  TEST_free_section_t *sect_node1 = NULL, *sect_node2 = NULL,
                      *sect_node3 = NULL, *sect_node4 = NULL;
  unsigned init_flags = 0;
  htri_t node_found = FALSE;
  TEST_free_section_t *node;
  ...
  init_sect_node(sect_node2, (haddr_t)TEST_SECT_ADDR70,
                 (hsize_t)TEST_SECT_SIZE30, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  init_sect_node(sect_node3, (haddr_t)TEST_SECT_ADDR60,
                 (hsize_t)TEST_SECT_SIZE10, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  init_sect_node(sect_node4, (haddr_t)TEST_SECT_ADDR150,
                 (hsize_t)TEST_SECT_SIZE80, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  if ((node_found =
           H5FS_sect_find(f, frsp,
                          (hsize_t)(TEST_SECT_SIZE10 + TEST_SECT_SIZE30 +
                                    TEST_SECT_SIZE50 + TEST_SECT_SIZE80),
                          (H5FS_section_info_t **)&node)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((node->sect_info.addr != TEST_SECT_ADDR60) ||
      (node->sect_info.size != (TEST_SECT_SIZE10 + TEST_SECT_SIZE30 +
                                TEST_SECT_SIZE50 + TEST_SECT_SIZE80)))
    TEST_ERROR;
  ...
  init_sect_node(sect_node1, (haddr_t)TEST_SECT_ADDR70,
                 (hsize_t)TEST_SECT_SIZE30, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  if ((node_found = H5FS_sect_find(
           f, frsp, (hsize_t)(TEST_SECT_SIZE30 + TEST_SECT_SIZE50),
           (H5FS_section_info_t **)&node)) < 0)
    FAIL_STACK_ERROR;
  ...
  init_sect_node(sect_node2, (haddr_t)TEST_SECT_ADDR70,
                 (hsize_t)TEST_SECT_SIZE30, TEST_FSPACE_SECT_TYPE_NEW,
                 H5FS_SECT_LIVE);
  ...
  /* sections A & B are not merged because H5FS_CLS_MERGE_SYM is set & section
  ...
  init_sect_node(sect_node3, (haddr_t)TEST_SECT_ADDR100,
                 (hsize_t)TEST_SECT_SIZE50, TEST_FSPACE_SECT_TYPE_NEW,
                 H5FS_SECT_LIVE);
  ...
  /* sections B & C are merged because H5FS_CLS_MERGE_SYM is set & section class
   * type is the same */
  state.tot_space += TEST_SECT_SIZE50;
  ...
  if ((node_found = H5FS_sect_find(f, frsp, (hsize_t)(TEST_SECT_SIZE10),
                                   (H5FS_section_info_t **)&node)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((node->sect_info.addr != TEST_SECT_ADDR60) ||
      (node->sect_info.size != TEST_SECT_SIZE10))
    TEST_ERROR;
  ...
  if ((node_found = H5FS_sect_find(f, frsp, (hsize_t)(TEST_SECT_SIZE80),
                                   (H5FS_section_info_t **)&node)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((node->sect_info.addr != TEST_SECT_ADDR150) ||
      (node->sect_info.size != TEST_SECT_SIZE80))
    TEST_ERROR;
 ...
 *	  H5FS_CLS_SEPAR_OBJ (cls->flags) is not set when creating free-space
 *manager Add section A to allow shrinking but is not shrunk because its section
 *class type TEST_FSPACE_SECT_TYPE_NEW does not define "can_shrink"
 *	  Result:section A is not shrunk and section A is still in free-space
 *
 *	  Re-add section A to allow shrinking and with section class type
 *TEST_FSPACE_SECT_TYPE that defines "can_shrink" Result:section A is shrunk and
 *there is nothing in free-space
 *
 *	Test 2:
 ...
 *
 *	  Add section A to allow shrinking but is not shrunk because it is not
 *at EOF, and section A is not on the merge list due to H5FS_CLS_SEPAR_OBJ Add
 *section B to allow shrinking and whose ending address is the same as eof.
 *		 Section B is not merged with section A because of
 *H5FS_CLS_SEPAR_OBJ but it is shrunk Result: section A is still in free-space
 *
 *	Test 3:
 *	  Set EOF to be greater than the ending address of section A
 *	  H5FS_CLS_SEPAR_OBJ (cls->flags) is not set when creating free-space
 ...
 *
 *	  Add section A to allow shrinking but is not shrunk because it is not
 *at EOF, and section A is on the merge list Add section B to allow shrinking
 *and whose ending address is the same as eof. Section B is merged with section
 *A and then shrunk. Result: free-space should be empty
 */
static unsigned test_fs_sect_shrink(hid_t fapl) {
  hid_t file = -1;               /* File ID */
  char filename[FILENAME_LEN];   /* Filename to use */
  ...
  /*
   * Add section A that allow shrinking but its section class type does not
   * define "can_shrink"
   */
  if (NULL ==
      (sect_node1 = (TEST_free_section_t *)malloc(sizeof(TEST_free_section_t))))
  ...
  /*
   * Re-add section A that allow shrinking and its section class type defines
   * "can_shrink"
   */
  if (NULL ==
      (sect_node1 = (TEST_free_section_t *)malloc(sizeof(TEST_free_section_t))))
  ...
  init_sect_node(sect_node1, (haddr_t)TEST_SECT_ADDR80,
                 (hsize_t)TEST_SECT_SIZE20, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  /* section A should still be there in free-space */
  if ((node_found = H5FS_sect_find(f, frsp, (hsize_t)(TEST_SECT_SIZE20),
                                   (H5FS_section_info_t **)&node)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((node->sect_info.addr != TEST_SECT_ADDR80) ||
      (node->sect_info.size != TEST_SECT_SIZE20))
    TEST_ERROR;
 ...
 *
 * Test 1:
 *	Add section A with TEST_FSPACE_SECT_TYPE class type with
 *H5FS_CLS_GHOST_OBJ setting Add section B with TEST_FSPACE_SECT_TYPE_NONE class
 *type without H5FS_CLS_GHOST_OBJ setting Change section A's class to the second
 *section's class Result: serial_sect_count is incremented by 1;
 *ghost_sect_count is decremented by 1
 *
 * Test 2:
 *	Add section A with TEST_FSPACE_SECT_TYPE class type with
 *H5FS_CLS_SEPAR_OBJ setting Add section B with TEST_FSPACE_SECT_TYPE_NONE class
 *type without H5FS_CLS_SEPAR_OBJ setting Add section C with
 *TEST_FSPACE_SECT_TYPE_NONE class type without H5FS_CLS_SEPAR_OBJ setting
 *	Sections B & C are on the merge list
 * 	Change section class of B and C to A's section class
 *	Result: the merge list should be null and the class of sections B & C
 *should be changed
 */
static unsigned test_fs_sect_change_class(hid_t fapl) {
  hid_t file = -1;               /* File ID */
  ...
  init_sect_node(sect_node2, (haddr_t)TEST_SECT_ADDR100,
                 (hsize_t)TEST_SECT_SIZE50, TEST_FSPACE_SECT_TYPE_NONE,
                 H5FS_SECT_LIVE);
  ...
  if (H5FS_sect_find(f, frsp, (hsize_t)TEST_SECT_SIZE30,
                     (H5FS_section_info_t **)&node) < 0)
    FAIL_STACK_ERROR;
  ...
  init_sect_node(sect_node3, (haddr_t)TEST_SECT_ADDR200,
                 (hsize_t)TEST_SECT_SIZE80, TEST_FSPACE_SECT_TYPE_NONE,
                 H5FS_SECT_LIVE);
  ...
  if (H5FS_sect_find(f, frsp, (hsize_t)TEST_SECT_SIZE50,
                     (H5FS_section_info_t **)&node) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5FS_sect_find(f, frsp, (hsize_t)TEST_SECT_SIZE80,
                     (H5FS_section_info_t **)&node) < 0)
    FAIL_STACK_ERROR;
 ...
 *
 * Test 1: Try to extend the block by requested-size, which is equal to section
 *B's size Add section A (addr=70, size=5) Add section B (addr=100, size=40) Try
 *to extend the block (addr=80, size=20) by 40, which is the same as section B's
 *size Result: succeed in extending the block
 *
 * Test 2: Try to extend the block by requested-size, which is greater than
 *section B's size Add section A (addr=70, size=5) Add section B (addr=100,
 *size=40) Try to extend the block (addr=80, size=20) by 50, which is greater
 *than section B's size Result: fail in extending the block
 *
 * Test 3: Try to extend the block by requested-size, which is less than section
 *B's size Add section A (addr=70, size=5) Add section B (addr=100, size=40) Try
 *to extend the block (addr=80, size=20) by 30, which is less than section B's
 *size Result: succeed in extending the block and a section of size=10 is left
 *in free-space
 *
 ...
 *			the block does not adjoin section B (80+15 != addr of
 *section B (80)) even though the requested-size is equal to section B's size
 *
 */
static unsigned test_fs_sect_extend(hid_t fapl) {
  hid_t file = -1;               /* File ID */
  ...
  init_sect_node(sect_node1, (haddr_t)TEST_SECT_ADDR70,
                 (hsize_t)TEST_SECT_SIZE5, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  init_sect_node(sect_node2, (haddr_t)TEST_SECT_ADDR100,
                 (hsize_t)TEST_SECT_SIZE40, TEST_FSPACE_SECT_TYPE,
                 H5FS_SECT_LIVE);
  ...
  if ((status = H5FS_sect_try_extend(f, frsp, (haddr_t)TEST_SECT_SIZE80,
                                     (hsize_t)TEST_SECT_SIZE20,
  ...
  if ((status = H5FS_sect_try_extend(f, frsp, (haddr_t)TEST_SECT_ADDR80,
                                     (hsize_t)TEST_SECT_SIZE20,
  ...
  /* Succeed in extending the block: total free space is decreased but other
...
static unsigned test_fs_sect_iterate(hid_t fapl) {
  ...
  for (i = 1; i <= NUM_SECTIONS; i++) {
    if (NULL == (sect_node = (TEST_free_section_t *)malloc(
                     sizeof(TEST_free_section_t))))
      FAIL_STACK_ERROR;

    sect_size = (unsigned)((i - 1) % 9) + 1;
    init_sect_node(sect_node, (haddr_t)i * 10, (hsize_t)sect_size,
                   TEST_FSPACE_SECT_TYPE, H5FS_SECT_LIVE);
