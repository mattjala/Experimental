...
int main(int argc, char **argv) {
  ...
  if ((fid = H5Fcreate("HDFFV-10992.h5", H5F_ACC_TRUNC, H5P_DEFAULT,
                       H5P_DEFAULT)) < 0) {
  ...
  if ((did = H5Dcreate2(fid, "dset", H5T_STD_REF, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0) {
  ...
  if (H5Dwrite(did, H5T_STD_REF, H5S_ALL, H5S_ALL, H5P_DEFAULT, &write_ref) <
      0) {
  ...
  for (i = 0; i < 500; i++) {
    ...
    if (H5Dread(did, H5T_STD_REF, H5S_ALL, H5S_ALL, H5P_DEFAULT, &read_ref) <
        0) {
