 ...
 *		calling _exit(EXIT_SUCCESS) since this doesn't flush HDF5 caches
 *but still exits with success.
 */
#include "h5test.h"
...
static hid_t create_file(const char *filename, hid_t fapl_id, hbool_t swmr) {
  ...
  /* Create some groups */
  if ((top_gid = H5Gcreate2(fid, "top_group", H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    STACK_ERROR;
  for (i = 0; i < NGROUPS; i++) {
    HDsnprintf(group_name, sizeof(group_name), "group%02d", i);
    if ((gid = H5Gcreate2(top_gid, group_name, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      STACK_ERROR;
    if (H5Gclose(gid) < 0)
      STACK_ERROR;
...
static herr_t add_dset_to_file(hid_t fid, const char *dset_name) {
  ...
  if ((sid = H5Screate_simple(NDIMS, dims, NULL)) < 0)
    STACK_ERROR;
  if ((did = H5Dcreate2(fid, dset_name, H5T_NATIVE_FLOAT, sid, H5P_DEFAULT,
