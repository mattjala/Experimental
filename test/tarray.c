...
static void test_array_atomic_1d(void) {
  ...
  for (i = 0; i < ndims; i++)
    if (rdims1[i] != tdims1[i]) {
      TestErrPrintf(
          "Array dimension information doesn't match!, rdims1[%d]=%" PRIuHSIZE
          ", tdims1[%d]=%" PRIuHSIZE "\n",
    ...
    for (j = 0; j < ARRAY1_DIM1; j++)
      if (wdata[i][j] != rdata[i][j]) {
        TestErrPrintf("Array data information doesn't match!, "
                      "wdata[%d][%d]=%d, rdata[%d][%d]=%d\n",
                      (int)i, (int)j, (int)wdata[i][j], (int)i, (int)j,
                      (int)rdata[i][j]);
        continue;
      } /* end if */
...
static void test_array_atomic_3d(void) {
  ...
  int rdata[SPACE1_DIM1][ARRAY2_DIM1][ARRAY2_DIM2]
           [ARRAY2_DIM3]; /* Information read in */
        ...
        for (l = 0; l < ARRAY2_DIM3; l++)
          if (wdata[i][j][k][l] != rdata[i][j][k][l]) {
            TestErrPrintf("Array data information doesn't match!, "
                          ...
                          (int)wdata[i][j][k][l], (int)i, (int)j, (int)k,
                          (int)l, (int)rdata[i][j][k][l]);
            continue;
          } /* end if */
...
static void test_array_array_atomic(void) {
  ...
  int rdata[SPACE1_DIM1][ARRAY1_DIM1][ARRAY3_DIM1]
           [ARRAY3_DIM2]; /* Information read in */
  hid_t fid;              /* HDF5 File IDs                */
  hid_t dataset;          /* Dataset ID                   */
  hid_t sid;              /* Dataspace ID                 */
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing 1-D Array 2-D Arrays of Atomic Datatypes Functionality\n"));
...
static void test_array_compound_atomic(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing 1-D Array of Compound Atomic Datatypes Functionality\n"));
  ...
  for (i = 0; i < SPACE1_DIM1; i++)
    for (j = 0; j < ARRAY1_DIM1; j++) {
      if (wdata[i][j].i != rdata[i][j].i) {
        TestErrPrintf("Array data information doesn't match!, "
                      "wdata[%d][%d].i=%d, rdata[%d][%d].i=%d\n",
                      (int)i, (int)j, (int)wdata[i][j].i, (int)i, (int)j,
                      (int)rdata[i][j].i);
        continue;
      } /* end if */
      if (!H5_FLT_ABS_EQUAL(wdata[i][j].f, rdata[i][j].f)) {
        TestErrPrintf("Array data information doesn't match!, "
                      ...
                      (int)i, (int)j, (double)wdata[i][j].f, (int)i, (int)j,
                      (double)rdata[i][j].f);
        continue;
      } /* end if */
    }   /* end for */
...
static void test_array_compound_array(void) {
  ...
  for (i = 0; i < ndims; i++)
    if (rdims1[i] != tdims1[i]) {
      TestErrPrintf("Nested array dimension information doesn't match!, "
                    "rdims1[%d]=%" PRIuHSIZE ", tdims1[%d]=%" PRIuHSIZE "\n",
                    i, rdims1[i], i, tdims1[i]);
      continue;
    } /* end if */
      ...
      for (k = 0; k < ARRAY1_DIM1; k++)
        if (!H5_FLT_ABS_EQUAL(wdata[i][j].f[k], rdata[i][j].f[k])) {
          TestErrPrintf(
              ...
              "rdata[%d][%d].f[%d]=%f\n",
              (int)i, (int)j, (int)k, (double)wdata[i][j].f[k], (int)i, (int)j,
              (int)k, (double)rdata[i][j].f[k]);
          continue;
        } /* end if */
    }     /* end for */
  }       /* end for */
...
static void test_array_vlen_atomic(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing 1-D Array of Atomic Variable-Length Datatypes "
              "Functionality\n"));
  ...
  for (i = 0; i < SPACE1_DIM1; i++)
    for (j = 0; j < ARRAY1_DIM1; j++) {
      ...
      for (k = 0; k < (i + j + 1); k++)
        ((unsigned int *)wdata[i][j].p)[k] =
            (unsigned int)(i * 100 + j * 10 + k);
    } /* end for */
  ...
  ret = H5Pset_vlen_mem_manager(xfer_pid, test_array_alloc_custom, &mem_used,
                                test_array_free_custom, &mem_used);
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    for (j = 0; j < ARRAY1_DIM1; j++) {
      if (wdata[i][j].len != rdata[i][j].len) {
        TestErrPrintf("VL data length don't match!, wdata[%d][%d].len=%d, "
                      "rdata[%d][%d].len=%d\n",
                      (int)i, (int)j, (int)wdata[i][j].len, (int)i, (int)j,
                      (int)rdata[i][j].len);
        continue;
      } /* end if */
      for (k = 0; k < (int)rdata[i][j].len; k++) {
        if (((unsigned int *)wdata[i][j].p)[k] !=
            ((unsigned int *)rdata[i][j].p)[k]) {
          TestErrPrintf("VL data values don't match!, wdata[%d][%d].p[%d]=%d, "
                        "rdata[%d][%d].p[%d]=%d\n",
                        (int)i, (int)j, (int)k,
                        (int)((unsigned int *)wdata[i][j].p)[k], (int)i, (int)j,
...
static void test_array_vlen_array(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing 1-D Array of 1-D Array Variable-Length Datatypes "
              "Functionality\n"));
   ...
   */
  VERIFY(size, 64 * (sizeof(unsigned int) * ARRAY1_DIM1),
...
static void test_array_bkg(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Partial I/O of Array Fields in Compound Datatype "
              "Functionality\n"));
  ...
  /* Create the dataset */
  /* ------------------ */
  dataset = H5Dcreate2(fid, FIELDNAME, type, space, H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  for (i = 0; i < LENGTH; i++) {
    for (j = 0; j < ALEN; j++) {
      if (cf[i].a[j] != cfr[i].a[j]) {
        TestErrPrintf(
            "Field a data doesn't match, cf[%d].a[%d]=%d, cfr[%d].a[%d]=%d\n",
            (int)i, (int)j, (int)cf[i].a[j], (int)i, (int)j, (int)cfr[i].a[j]);
        continue;
      } /* end if */
      if (!H5_FLT_ABS_EQUAL(cf[i].b[j], cfr[i].b[j])) {
        TestErrPrintf(
            "Field b data doesn't match, cf[%d].b[%d]=%f, cfr[%d].b[%d]=%f\n",
            (int)i, (int)j, (double)cf[i].b[j], (int)i, (int)j,
            (double)cfr[i].b[j]);
        continue;
      } /* end if */
      if (!H5_DBL_ABS_EQUAL(cf[i].c[j], cfr[i].c[j])) {
        TestErrPrintf(
            "Field c data doesn't match, cf[%d].b[%d]=%f, cfr[%d].b[%d]=%f\n",
            (int)i, (int)j, (double)cf[i].c[j], (int)i, (int)j,
            (double)cfr[i].c[j]);
        continue;
      } /* end if */
    }   /* end for */
  ...
  for (i = 0; i < LENGTH; i++)
    for (j = 0; j < ALEN; j++)
      if (!H5_FLT_ABS_EQUAL(fld[i].b[j], fldr[i].b[j])) {
        TestErrPrintf(
            "Field data doesn't match, fld[%d].b[%d]=%f, fldr[%d].b[%d]=%f\n",
            (int)i, (int)j, (double)fld[i].b[j], (int)i, (int)j,
            (double)fldr[i].b[j]);
        continue;
      } /* end if */
...
void test_array(void) {
  ...
  test_array_array_atomic(); /* Test 1-D array of 2-D arrays of atomic datatypes
                              */
  test_array_compound_atomic(); /* Test 1-D array of compound datatypes (with no
                                   array fields)          */
  test_array_compound_array();  /* Test 1-D array of compound datatypes (with
                                   array fields)             */
  ...
  test_array_bkg(); /* Read compound datatype with array fields and background
                       fields read  */
