...
static void test_misc1(void) {
  ...
  dataset = H5Dcreate2(file, MISC1_DSET_NAME, H5T_NATIVE_INT, dataspace,
  ...
  ret =
      H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &i_check);
...
static void test_misc2_write_attribute(void) {
  ...
  att1 = H5Acreate2(root1, MISC2_ATT_NAME_1, type, dataspace, H5P_DEFAULT,
                    H5P_DEFAULT);
  ...
  att2 = H5Acreate2(root2, MISC2_ATT_NAME_2, type, dataspace, H5P_DEFAULT,
                    H5P_DEFAULT);
...
static void test_misc2_read_attribute(const char *filename,
                                      const char *att_name) {
  hid_t file, root, att;
  hid_t type;
  hid_t space;
...
static void test_misc3(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing reading from chunked dataset with non-zero fill-value\n"));
  ...
  /* Create the dataset */
  dataset = H5Dcreate2(file, MISC3_DSET_NAME, H5T_NATIVE_INT, dataspace,
...
static void test_misc4(void) {
  ...
  group1 =
      H5Gcreate2(file1, MISC4_GROUP_1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(group1, FAIL, "H5Gcreate2");

  /* Create the second group */
  group2 =
      H5Gcreate2(file1, MISC4_GROUP_2, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* Create the first group */
  group3 =
      H5Gcreate2(file2, MISC4_GROUP_1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(group3, FAIL, "H5Gcreate2");

  /* Get the stat information for each group */
  ret = H5Oget_info_by_name3(file1, MISC4_GROUP_1, &oinfo1, H5O_INFO_BASIC,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name3");
  ret = H5Oget_info_by_name3(file1, MISC4_GROUP_2, &oinfo2, H5O_INFO_BASIC,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name3");
  ret = H5Oget_info_by_name3(file2, MISC4_GROUP_1, &oinfo3, H5O_INFO_BASIC,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name3");
...
static misc5_struct3_hndl *create_struct3(void) {
  ...
  ret = H5Tinsert(str3hndl->st3h_base, "st3_el1",
                  HOFFSET(misc5_struct3, st3_el1), H5T_NATIVE_INT);
  CHECK(ret, FAIL, "H5Tinsert");
...
static void set_struct3(misc5_struct3 *buf) { buf->st3_el1 = MISC5_DBGELVAL3; }
...
static misc5_struct2_hndl *create_struct2(void) {
  ...
  ret = H5Tinsert(str2hndl->st2h_base, "st2_el1",
                  HOFFSET(misc5_struct2, st2_el1), H5T_NATIVE_INT);
  CHECK(ret, FAIL, "H5Tinsert");
  ...
  ret =
      H5Tinsert(str2hndl->st2h_base, "st2_el2", HOFFSET(misc5_struct2, st2_el2),
...
static void clear_struct2(misc5_struct2 *buf) { free(buf->st2_el2.p); }
...
static misc5_struct1_hndl *create_struct1(void) {
  ...
  ret = H5Tinsert(str1hndl->st1h_base, "st1_el1",
                  HOFFSET(misc5_struct1, st1_el1), H5T_NATIVE_INT);
  CHECK(ret, FAIL, "H5Tinsert");
  ...
  ret =
      H5Tinsert(str1hndl->st1h_base, "st1_el2", HOFFSET(misc5_struct1, st1_el2),
...
static void test_misc5(void) {
  ...
  /* Create the dataset */
  dataset_id = H5Dcreate2(loc_id, MISC5_DSETNAME, str1hndl->st1h_id, space_id,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  /* Write the data out */
  ret = H5Dwrite(dataset_id, str1hndl->st1h_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 &buf);
  CHECK(ret, FAIL, "H5Dwrite");
  ...
  for (i = 0; i < (buf.len); i++) {
    ...
    for (j = 0; j < (((misc5_struct1 *)(buf.p))[i].st1_el2.len); j++) {
      ...
      VERIFY(((misc5_struct2 *)(((misc5_struct1 *)(buf.p))[i].st1_el2.p))[j]
                 .st2_el1,
             MISC5_DBGELVAL2, "H5Dread");
      for (k = 0;
           k < (((misc5_struct2 *)(((misc5_struct1 *)(buf.p))[i].st1_el2.p))[j]
                    .st2_el2.len);
           k++) {
        /* printf("      [%d]=%d\n",k, ((misc5_struct3 *)(((misc5_struct2 *)
         * (((misc5_struct1
         * *)(buf.p))[i].  st1_el2.p))[j].st2_el2.p))[k].st3_el1); */
        VERIFY(((misc5_struct3 *)(((
                   misc5_struct2 *)(((misc5_struct1 *)(buf.p))[i].st1_el2.p))[j]
                                      .st2_el2.p))[k]
                   .st3_el1,
...
static void test_misc6(void) {
  ...
  dataset_id = H5Dcreate2(loc_id, MISC6_DSETNAME1, H5T_NATIVE_INT, space_id,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset_id, FAIL, "H5Dcreate2");
  ...
  dataset_id = H5Dcreate2(loc_id, MISC6_DSETNAME2, H5T_NATIVE_INT, space_id,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset_id, FAIL, "H5Dcreate2");
  ...
  for (u = 0; u < MISC6_NUMATTR; u++) {
    ...
    attr_id = H5Acreate2(dataset_id, attr_name, H5T_NATIVE_INT, space_id,
...
static void test_misc7(void) {
  ...
  ret = H5Tcommit2(fid, MISC7_TYPENAME1, tid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  VERIFY(ret, FAIL, "H5Tcommit2");

  /* Attempt to use empty compound datatype to create dataset */
  did = H5Dcreate2(fid, MISC7_DSETNAME1, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  ...
  ret = H5Tcommit2(fid, MISC7_TYPENAME2, tid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  VERIFY(ret, FAIL, "H5Tcommit2");

  /* Attempt to use empty enum datatype to create dataset */
  did = H5Dcreate2(fid, MISC7_DSETNAME2, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
...
static void test_misc8(void) {
  ...
  did = H5Dcreate2(fid, MISC8_DSETNAME1, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  VERIFY(storage_size,
         (hsize_t)(MISC8_DIM0 * MISC8_DIM1 * H5Tget_size(H5T_NATIVE_INT)),
         "H5Dget_storage_size");
  ...
  /* Should fail */
  H5E_BEGIN_TRY {
    did = H5Dcreate2(fid, MISC8_DSETNAME4, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                     dcpl, H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(did, FAIL, "H5Dcreate2");
  ...
  /* Create a contiguous dataset, with space allocation early */
  did = H5Dcreate2(fid, MISC8_DSETNAME4, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  did = H5Dcreate2(fid, MISC8_DSETNAME5, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  did = H5Dcreate2(fid, MISC8_DSETNAME7, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  storage_size = H5Dget_storage_size(did);
  VERIFY(storage_size,
         (hsize_t)(4 * MISC8_CHUNK_DIM0 * MISC8_CHUNK_DIM1 *
    ...
    for (v = 0; v < MISC8_DIM1; v++, tdata++, tdata2++)
      if (*tdata != *tdata2)
        TestErrPrintf("Error on line %d: u=%u, v=%d, *tdata=%d, *tdata2=%d\n",
                      __LINE__, (unsigned)u, (unsigned)v, (int)*tdata,
                      (int)*tdata2);
#endif /* VERIFY_DATA */
  ...
  /* Create a chunked dataset, with space allocation early */
  did = H5Dcreate2(fid, MISC8_DSETNAME8, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  /* Create a chunked dataset, with space allocation incremental */
  did = H5Dcreate2(fid, MISC8_DSETNAME10, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   dcpl, H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
...
#ifdef H5_HAVE_FILTER_DEFLATE
  if (storage_size >=
      (4 * MISC8_CHUNK_DIM0 * MISC8_CHUNK_DIM1 * H5Tget_size(H5T_NATIVE_INT)))
    TestErrPrintf("Error on line %d: data wasn't compressed! storage_size=%u\n",
                  __LINE__, (unsigned)storage_size);
#else  /* Compression is not configured */
  if (storage_size !=
...
static void test_misc9(void) {
  ...
  H5E_BEGIN_TRY { fid = H5Fopen(MISC9_FILE, H5F_ACC_RDWR, fapl); }
  H5E_END_TRY
...
static void test_misc10(void) {
  ...
  const char *testfile =
      H5_get_srcdir_filename(MISC10_FILE_OLD); /* Corrected test file name */
  ...
  dataset_new = H5Dcreate2(file_new, MISC10_DSETNAME, type, space, H5P_DEFAULT,
                           dcpl, H5P_DEFAULT);
...
static void test_misc11(void) {
  ...
  ret =
      H5Pset_sizes(fcpl, (size_t)MISC11_SIZEOF_OFF, (size_t)MISC11_SIZEOF_LEN);
  CHECK(ret, FAIL, "H5Pset_sizes");

  /* This should fail as (32770*2) will exceed ^16 - 2 bytes for storing btree
   * entries */
...
static void test_misc12(void) {
  ...
  char
      *rdata[MISC12_SPACE1_DIM1 + MISC12_APPEND_SIZE]; /* Information read in */
  hid_t fid1;
  hid_t dataset;
  ...
  /* Create a dataset */
  dataset = H5Dcreate2(fid1, MISC12_DSET_NAME, tid1, sid1, H5P_DEFAULT, cparms,
                       H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  for (; i < (MISC12_SPACE1_DIM1 + MISC12_APPEND_SIZE); i++)
    if (HDstrcmp(wdata1[i - MISC12_SPACE1_DIM1], rdata[i]) != 0)
      TestErrPrintf("Error on line %d: wdata1[%d]=%s, rdata[%d]=%s\n", __LINE__,
                    i - MISC12_SPACE1_DIM1, wdata1[i - MISC12_SPACE1_DIM1], i,
                    rdata[i]);
...
static hbool_t misc13_verify_data_match(const unsigned *original_data,
                                        const unsigned *read_data) {
  unsigned u;
...
static void misc13_create_dataset(hid_t loc_id, const char *name, hid_t dcpl,
                                  const unsigned *data) {
...
static void misc13_verify_dataset(hid_t loc_id, const char *name,
                                  const unsigned *data) {
  ...
  /* Read the data */
  ret =
      H5Dread(dsid, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, read_data);
  CHECK(ret, FAIL, "H5Dread");
...
static void misc13_create_hdf_file(const char *name, const unsigned *data) {
  ...
  ret = H5Tcommit2(fid, MISC13_DTYPE_NAME, tid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");
  ...
  gid1 = H5Gcreate2(fid, MISC13_GROUP1_NAME, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(gid1, FAIL, "H5Gcreate2");

  /* Create another group in the new group */
  gid2 = H5Gcreate2(gid1, MISC13_GROUP2_NAME, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
  ...
  ret = H5Tcommit2(gid1, MISC13_DTYPE_NAME, tid, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");
...
static void misc13_insert_user_block(const char *old_name, const char *new_name,
  ...
  while ((read_in = fread(copy_buf, (size_t)1, (size_t)MISC13_COPY_BUF_SIZE,
                          old_fp)) > 0) {
...
static void misc13_verify_file(const char *name, const unsigned *data,
...
static void test_misc13(void) {
  ...
  hbool_t check_for_new_dataset; /* Whether to check for the
                                    post-userblock-creation dataset */
  ...
  misc13_verify_file(MISC13_FILE_1, data, userblock_size,
                     check_for_new_dataset);

  /* Create a new file by inserting a user block in front of the first file */
  misc13_insert_user_block(MISC13_FILE_1, MISC13_FILE_2, "Test String",
                           (size_t)MISC13_USERBLOCK_SIZE);

  /* Verify file contents are still correct */
  userblock_size = MISC13_USERBLOCK_SIZE;
  check_for_new_dataset = FALSE;
  misc13_verify_file(MISC13_FILE_2, data, userblock_size,
                     check_for_new_dataset);
...
static void test_misc14(void) {
  ...
  Dataset1 = H5Dcreate2(file_id, MISC14_DSET1_NAME, H5T_NATIVE_DOUBLE,
                        DataSpace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(Dataset1, FAIL, "H5Dcreate2");

  ret = H5Dwrite(Dataset1, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 &data1);
  CHECK(ret, FAIL, "H5Dwrite");

  /* Create second dataset (to be unlinked).  */
  Dataset2 = H5Dcreate2(file_id, MISC14_DSET2_NAME, H5T_NATIVE_DOUBLE,
                        DataSpace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(Dataset2, FAIL, "H5Dcreate2");

  ret = H5Dwrite(Dataset2, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 &data2);
  CHECK(ret, FAIL, "H5Dwrite");

  /* Check data from first dataset */
  ret = H5Dread(Dataset1, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                &rdata);
  CHECK(ret, FAIL, "H5Dread");
  ...
  ret = H5Dread(Dataset2, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                &rdata);
  CHECK(ret, FAIL, "H5Dread");
  ...
  Dataset3 = H5Dcreate2(file_id, MISC14_DSET3_NAME, H5T_NATIVE_DOUBLE,
                        DataSpace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(Dataset2, FAIL, "H5Dcreate2");

  ret = H5Dwrite(Dataset3, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 &data3);
  CHECK(ret, FAIL, "H5Dwrite");
  ...
  ret = H5Dread(Dataset3, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                &rdata);
  CHECK(ret, FAIL, "H5Dread");
...
static void test_misc16(void) {
  ...
  /* Create a dataset */
  dataset = H5Dcreate2(file, MISC16_DSET_NAME, tid, sid, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  for (i = 0; i < MISC16_SPACE_DIM; i++) {
    if (HDstrlen(wdata[i]) != HDstrlen(rdata[i])) {
      TestErrPrintf("Line %u: VL data length don't match!, "
                    "strlen(wdata[%d])=%d, strlen(rdata[%d])=%d\n",
                    (unsigned)__LINE__, (int)i, (int)HDstrlen(wdata[i]), (int)i,
                    (int)HDstrlen(rdata[i]));
      continue;
    } /* end if */
    if (HDstrcmp(wdata[i], rdata[i]) != 0) {
      TestErrPrintf(
          "Line %u: VL data values don't match!, wdata[%d]=%s, rdata[%d]=%s\n",
...
static void test_misc17(void) {
  ...
  /* Create a dataset */
  dataset = H5Dcreate2(file, MISC17_DSET_NAME, tid, sid, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
...
static void test_misc18(void) {
  ...
  did1 = H5Dcreate2(fid, MISC18_DSET1_NAME, H5T_STD_U32LE, sid, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
  CHECK(did1, FAIL, "H5Dcreate2");

  /* Get object information */
  ret = H5Oget_info_by_name3(fid, MISC18_DSET1_NAME, &oinfo, H5O_INFO_NUM_ATTRS,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name");
  VERIFY(oinfo.num_attrs, 0, "H5Oget_info_by_name");
#ifndef H5_NO_DEPRECATED_SYMBOLS
  ret = H5Oget_info_by_name2(fid, MISC18_DSET1_NAME, &old_oinfo,
                             H5O_INFO_HDR | H5O_INFO_NUM_ATTRS, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name");
...
#endif /* H5_NO_DEPRECATED_SYMBOLS */
  ret = H5Oget_native_info_by_name(fid, MISC18_DSET1_NAME, &ninfo,
  ...
  /* Create second dataset */
  did2 = H5Dcreate2(fid, MISC18_DSET2_NAME, H5T_STD_U32LE, sid, H5P_DEFAULT,
                    H5P_DEFAULT, H5P_DEFAULT);
  CHECK(did2, FAIL, "H5Dcreate2");

  /* Get object information */
  ret = H5Oget_info_by_name3(fid, MISC18_DSET2_NAME, &oinfo, H5O_INFO_NUM_ATTRS,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name");
  VERIFY(oinfo.num_attrs, 0, "H5Oget_info_by_name");
#ifndef H5_NO_DEPRECATED_SYMBOLS
  ret = H5Oget_info_by_name2(fid, MISC18_DSET2_NAME, &old_oinfo,
                             H5O_INFO_HDR | H5O_INFO_NUM_ATTRS, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name");
  VERIFY(old_oinfo.hdr.nmesgs, 6, "H5Oget_info_by_name");
  VERIFY(old_oinfo.hdr.nchunks, 1, "H5Oget_info_by_name");
  ...
  VERIFY(old_oinfo.num_attrs, 0, "H5Oget_info_by_name");
#endif /* H5_NO_DEPRECATED_SYMBOLS */
  ret = H5Oget_native_info_by_name(fid, MISC18_DSET2_NAME, &ninfo,
                                   H5O_NATIVE_INFO_HDR, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_native_info_by_name");
  VERIFY(ninfo.hdr.nmesgs, 6, "H5Oget_native_info_by_name");
  VERIFY(ninfo.hdr.nchunks, 1, "H5Oget_native_info_by_name");
  ...
  /* Loop creating attributes on each dataset, flushing them to the file each
   * time */
  for (u = 0; u < 10; u++) {
    ...
    aid = H5Acreate2(did1, attr_name, H5T_STD_U32LE, sid, H5P_DEFAULT,
                     H5P_DEFAULT);
    ...
    aid = H5Acreate2(did2, attr_name, H5T_STD_U32LE, sid, H5P_DEFAULT,
                     H5P_DEFAULT);
    CHECK(aid, FAIL, "H5Acreate2");
...
static void test_misc19(void) {
  ...
  pcid = H5Pcreate_class(H5P_DATASET_CREATE, "foo", NULL, NULL, NULL, NULL,
                         NULL, NULL);
  ...
  /* Create a dataset */
  did = H5Dcreate2(fid, MISC19_DSET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  /* Create an attribute */
  aid = H5Acreate2(gid, MISC19_ATTR_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  ...
  /* Create a group */
  gid =
      H5Gcreate2(fid, MISC19_GROUP_NAME, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
static void test_misc20(void) {
  ...
  hsize_t big_dims[MISC20_SPACE_RANK] = {
      MISC20_SPACE_DIM0, MISC20_SPACE_DIM1}; /* Large dimensions */
  hsize_t small_dims[MISC20_SPACE_RANK] = {
      MISC20_SPACE2_DIM0, MISC20_SPACE2_DIM1}; /* Small dimensions */
  unsigned version;    /* Version of storage layout info */
  hsize_t contig_size; /* Size of contiguous storage size from layout into */
  const char *testfile =
      H5_get_srcdir_filename(MISC20_FILE_OLD); /* Corrected test file name */
  hbool_t driver_is_default_compatible;
  herr_t ret; /* Generic return value */
  ...
  did = H5Dcreate2(fid, MISC20_DSET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   dcpl, H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  /* Create dataset with big dataspace */
  did = H5Dcreate2(fid, MISC20_DSET2_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   dcpl, H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ...
  VERIFY(contig_size,
         (MISC20_SPACE_DIM0 * MISC20_SPACE_DIM1 * H5Tget_size(H5T_NATIVE_INT)),
  ...
  VERIFY(
      contig_size,
      (MISC20_SPACE2_DIM0 * MISC20_SPACE2_DIM1 * H5Tget_size(H5T_NATIVE_INT)),
      "H5D__layout_contig_size_test");
  ...
  /* Verify that the storage size is computed correctly for older versions of
   * layout info */
  ...
  CHECK(ret, FAIL, "H5D__layout_contig_size_test");
  VERIFY(contig_size,
         (MISC20_SPACE_DIM0 * MISC20_SPACE_DIM1 * H5Tget_size(H5T_STD_I32LE)),
         "H5D__layout_contig_size_test");
...
static void test_misc21(void) {
  ...
  /* Create the dataset */
  dsid = H5Dcreate2(fid, MISC21_DSET_NAME, H5T_NATIVE_UINT8, sid, H5P_DEFAULT,
                    dcpl, H5P_DEFAULT);
  CHECK(dsid, FAIL, "H5Dcreate2");
    ...
    for (j = 0; j < 4; j++) {
      ...
      for (k = 0; k < 5; k++) {
        ...
        MESSAGE(5, ("  Testing datatypes size=%zu precision=%u offset=%d\n",
                    H5Tget_size(idts[i]), (unsigned)prec[j],
                    (unsigned)offsets[k]));
        ...
        /* Create the dataset */
        dsid = H5Dcreate2(fid, MISC22_DSET_NAME, dtype, sid, H5P_DEFAULT, dcpl,
                          H5P_DEFAULT);
        CHECK(dsid, FAIL, "H5Dcreate2");
        ...
        ret = H5Pget_filter_by_id2(dcpl2, H5Z_FILTER_SZIP, &flags, &cd_nelmts,
                                   cd_values, 0, NULL, NULL);
        CHECK(ret, FAIL, "H5Pget_filter_by_id2");

        VERIFY(cd_values[2], (unsigned)correct,
               "SZIP filter returned value for precision");
...
static void test_misc23(void) {
  hsize_t dims[] = {10};
  hid_t file_id = 0, group_id = 0, type_id = 0, space_id = 0, tmp_id = 0,
        create_id = H5P_DEFAULT, access_id = H5P_DEFAULT;
  ...
  tmp_id =
      H5Gcreate2(file_id, "/A/B02/C02/grp", create_id, H5P_DEFAULT, access_id);
  CHECK(tmp_id, FAIL, "H5Gcreate2");
  ...
  tmp_id =
      H5Gcreate2(group_id, "/A/B04/grp/", create_id, H5P_DEFAULT, access_id);
  CHECK(tmp_id, FAIL, "H5Gcreate2");
  ...
  tmp_id =
      H5Gcreate2(file_id, "/A/B05/C05/A", create_id, H5P_DEFAULT, access_id);
  CHECK(tmp_id, FAIL, "H5Gcreate2");
  ...
  tmp_id = H5Dcreate2(file_id, "/A/B06/dset", type_id, space_id, create_id,
                      H5P_DEFAULT, H5P_DEFAULT);
  ...
  tmp_id = H5Dcreate2(file_id, "/A/B07/B07/dset", type_id, space_id, create_id,
                      H5P_DEFAULT, H5P_DEFAULT);
  CHECK(tmp_id, FAIL, "H5Dcreate2");
  ...
  tmp_id = H5Dcreate2(group_id, "B08/dset", type_id, space_id, create_id,
                      H5P_DEFAULT, H5P_DEFAULT);
  CHECK(tmp_id, FAIL, "H5Dcreate2");
  ...
  tmp_id = H5Dcreate2(group_id, "/A/B09/dset", type_id, space_id, create_id,
                      H5P_DEFAULT, H5P_DEFAULT);
  CHECK(tmp_id, FAIL, "H5Dcreate2");
  ...
  tmp_id = H5Dcreate2(file_id, "/A/B10/C10/A/dset", type_id, space_id,
                      create_id, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(tmp_id, FAIL, "H5Dcreate2");
  ...
  status = H5Tcommit2(file_id, "/A/B11/dtype", tmp_id, create_id, H5P_DEFAULT,
                      access_id);
  ...
  status = H5Tcommit2(file_id, "/A/B12/C12/dtype", tmp_id, create_id,
  ...
  status = H5Tcommit2(group_id, "B13/C12/dtype", tmp_id, create_id, H5P_DEFAULT,
                      access_id);
  CHECK(status, FAIL, "H5Tcommit2");
  ...
  status = H5Tcommit2(group_id, "/A/B14/dtype", tmp_id, create_id, H5P_DEFAULT,
                      access_id);
  CHECK(status, FAIL, "H5Tcommit2");
  ...
  status = H5Tcommit2(file_id, "/A/B15/C15/A/dtype", tmp_id, create_id,
                      H5P_DEFAULT, access_id);
  CHECK(status, FAIL, "H5Tcommit2");
  ...
  status = H5Lcopy(file_id, "/A/B01/grp", file_id, "/A/B16/grp", create_id,
                   access_id);
  ...
  status = H5Lmove(file_id, "/A/B16/grp", file_id, "/A/B17/grp", create_id,
                   access_id);
  ...
  status = H5Lcreate_hard(file_id, "/A/B01/grp", file_id, "/A/B18/grp",
                          create_id, access_id);
  ...
  status =
      H5Lcreate_soft("/A/B01/grp", file_id, "/A/B19/grp", create_id, access_id);
  ...
  status = H5Lcreate_external("fake_filename", "fake_path", file_id,
  ...
  status = H5Lcreate_ud(file_id, "/A/B21/grp", H5L_TYPE_EXTERNAL, "file\0obj",
                        (size_t)9, create_id, access_id);
...
static void test_misc24(void) {
  hid_t file_id = 0, group_id = 0, type_id = 0, space_id = 0, dset_id = 0,
        tmp_id = 0;
  herr_t ret; /* Generic return value */
  ...
  group_id = H5Gcreate2(file_id, MISC24_GROUP_NAME, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT);
  CHECK(group_id, FAIL, "H5Gcreate2");

  dset_id = H5Dcreate2(file_id, MISC24_DATASET_NAME, H5T_NATIVE_INT, space_id,
                       H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  ret = H5Tcommit2(file_id, MISC24_DATATYPE_NAME, type_id, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");

  /* Create soft links to the objects created */
  ret = H5Lcreate_soft(MISC24_GROUP_NAME, file_id, MISC24_GROUP_LINK,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Lcreate_soft");

  ret = H5Lcreate_soft(MISC24_DATASET_NAME, file_id, MISC24_DATASET_LINK,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Lcreate_soft");

  ret = H5Lcreate_soft(MISC24_DATATYPE_NAME, file_id, MISC24_DATATYPE_LINK,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Lcreate_soft");
  ...
  /* Attempt to open each kind of object with wrong API, including using soft
   * links */
...
static void test_misc25a(void) {
  ...
  /* Create top group */
  gid = H5Gcreate2(fid, MISC25A_GROUP0_NAME, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(gid, FAIL, "H5Gcreate2");
  ...
  /* Create first group */
  gid = H5Gcreate2(fid, MISC25A_GROUP1_NAME, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(gid, FAIL, "H5Gcreate2");
  ...
  /* Create second group */
  gid2 = H5Gcreate2(fid, MISC25A_GROUP2_NAME, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(gid2, FAIL, "H5Gcreate2");
  ...
  /* Create third group */
  gid3 = H5Gcreate2(fid, MISC25A_GROUP3_NAME, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
...
static void test_misc25b(void) {
  ...
  hid_t gid; /* Group ID */
  const char *testfile =
      H5_get_srcdir_filename(MISC25B_FILE); /* Corrected test file name */
  hbool_t driver_is_default_compatible;
  herr_t ret; /* Generic return value */
...
static void test_misc25c(void) {
  ...
  ret = H5Pset_link_creation_order(
      gcpl, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED));
  ...
  /* Create the dataset */
  did = H5Dcreate2(gid, MISC25C_DSETNAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");

  /* Create an extra group */
  gid2 =
      H5Gcreate2(fid, MISC25C_GRPNAME, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(gid2, FAIL, "H5Gcreate2");
  ...
  aid = H5Acreate2(gid, MISC25C_ATTRNAME, H5T_NATIVE_CHAR, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  ...
  gid2 =
      H5Gcreate2(fid, MISC25C_GRPNAME2, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(gid2, FAIL, "H5Gcreate2");
  ...
  aid = H5Acreate2(gid, MISC25C_ATTRNAME2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  ...
  /* Rename the dataset */
  ret = H5Lmove(gid, MISC25C_DSETNAME, H5L_SAME_LOC, MISC25C_DSETNAME2,
                H5P_DEFAULT, H5P_DEFAULT);
...
static void test_misc26(void) {
  ...
  did = H5Dcreate2(fid, "dataset1", H5T_NATIVE_FLOAT, sid, H5P_DEFAULT, dcpl1,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ret = H5Dclose(did);
  CHECK_I(ret, "H5Dclose");

  did = H5Dcreate2(fid, "dataset2", H5T_NATIVE_FLOAT, sid, H5P_DEFAULT, dcpl2,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ret = H5Dclose(did);
  CHECK_I(ret, "H5Dclose");

  did = H5Dcreate2(fid, "dataset3", H5T_NATIVE_FLOAT, sid, H5P_DEFAULT, dcpl3,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
  ret = H5Dclose(did);
  CHECK_I(ret, "H5Dclose");
...
static void test_misc27(void) {
  ...
  hid_t gid; /* Group ID */
  const char *testfile =
      H5_get_srcdir_filename(MISC27_FILE); /* Corrected test file name */
  hbool_t driver_is_default_compatible;
  herr_t ret; /* Generic return value */
...
static void test_misc28(void) {
  ...
  did = H5Dcreate2(fid, "dataset", H5T_NATIVE_CHAR, sidf, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  CHECK(did, FAIL, "H5Dcreate2");
...
static int test_misc30_get_info_cb(hid_t loc_id, const char *name,
                                   const H5L_info2_t H5_ATTR_UNUSED *info,
                                   void H5_ATTR_UNUSED *op_data) {
  H5O_info2_t object_info;

  return H5Oget_info_by_name3(loc_id, name, &object_info, H5O_INFO_BASIC,
                              H5P_DEFAULT);
}

static int test_misc30_get_info(hid_t loc_id) {
  return H5Literate2(loc_id, H5_INDEX_NAME, H5_ITER_INC, NULL,
                     test_misc30_get_info_cb, NULL);
}
...
static void test_misc31(void) {
  ...
  space_id = H5Screate(H5S_SCALAR);
  CHECK(space_id, FAIL, "H5Screate");
  dset_id = H5Dcreate1(file_id, MISC31_DSETNAME, H5T_NATIVE_INT, space_id,
                       H5P_DEFAULT);
  ...
  space_id = H5Screate(H5S_SCALAR);
  CHECK(space_id, FAIL, "H5Screate");
  attr_id = H5Acreate1(dset_id, MISC31_ATTRNAME1, H5T_NATIVE_INT, space_id,
                       H5P_DEFAULT);
  ...
  space_id = H5Screate(H5S_SCALAR);
  CHECK(space_id, FAIL, "H5Screate");
  attr_id = H5Acreate1(dset_id, MISC31_ATTRNAME2, H5T_NATIVE_INT, space_id,
                       H5P_DEFAULT);
  CHECK(attr_id, FAIL, "H5Acreate1");
  ...
  ret = H5Pregister1(H5P_OBJECT_CREATE, MISC31_PROPNAME, 0, NULL, NULL, NULL,
                     NULL, NULL, NULL, NULL);
...
static void test_misc33(void) {
  hid_t fid = -1; /* File ID */
  const char *testfile =
      H5_get_srcdir_filename(MISC33_FILE); /* Corrected test file name */
  ...
  /* Case (1) */
  H5E_BEGIN_TRY {
    ret = H5Oget_info_by_name3(fid, "/soft_two", &oinfo, H5O_INFO_BASIC,
                               H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  /* Case (2) */
  H5E_BEGIN_TRY {
    ret = H5Oget_info_by_name3(fid, "/dsetA", &oinfo, H5O_INFO_BASIC,
                               H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(ret, FAIL, "H5Oget_info_by_name3");
  ...
  /* Case (3) */
  H5E_BEGIN_TRY {
    ret = H5Oget_info_by_name3(fid, "/soft_one", &oinfo, H5O_INFO_BASIC,
                               H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(ret, FAIL, "H5Oget_info_by_name3");
...
static void test_misc35(void) {
  ...
  hsize_t dims[] = {MISC35_SPACE_DIM1, MISC35_SPACE_DIM2,
                    MISC35_SPACE_DIM3}; /* Dataspace dims */
  hsize_t coord[MISC35_NPOINTS]
               [MISC35_SPACE_RANK] = /* Coordinates for point selection */
  ...
  ret = H5Sselect_elements(sid, H5S_SELECT_SET, (size_t)MISC35_NPOINTS,
                           (const hsize_t *)coord);
  ...
  ret = H5get_free_list_sizes(&reg_size_start, &arr_size_start, &blk_size_start,
                              &fac_size_start);
  ...
  ret = H5get_free_list_sizes(&reg_size_final, &arr_size_final, &blk_size_final,
                              &fac_size_final);
  CHECK(ret, FAIL, "H5get_free_list_sizes");
...
void test_misc(void) {
  ...
  test_misc4(); /* Test retrieving the fileno for various objects with
                   H5Oget_info() */
  test_misc5(); /* Test several level deep nested compound & VL datatypes */
  ...
  test_misc10(); /* Test for using dataset creation property lists from old
                    files */

  if (default_driver) {
    test_misc11(); /* Test for all properties of a file creation property list
                      being stored */
  }
  ...
  if (default_driver) {
    test_misc13(); /* Test that a user block can be insert in front of file
                      contents */
  }

  test_misc14(); /* Test that deleted dataset's data is removed from sieve
                    buffer correctly */
  test_misc15(); /* Test that checking a file's access property list more than
                    once works */
  ...
  test_misc19(); /* Test incrementing & decrementing ref count on IDs */
  test_misc20(); /* Test problems with truncated dimensions in version 2 of
                    storage layout message */

#ifdef H5_HAVE_FILTER_SZIP
  test_misc21();  /* Test that "late" allocation time is treated the same as
                     "incremental", for chunked  datasets w/a filters */
  test_misc22();  /* check szip bits per pixel */
#endif            /* H5_HAVE_FILTER_SZIP */
  test_misc23();  /* Test intermediate group creation */
  test_misc24();  /* Test inappropriate API opens of objects */
  test_misc25a(); /* Exercise null object header message merge bug */
  test_misc25b(); /* Exercise null object header message merge bug on existing
                     file */
  test_misc25c(); /* Exercise another null object header message merge bug */
  test_misc26();  /* Test closing property lists with long filter pipelines */
  test_misc27();  /* Test opening file with object that has bad # of object
                     header messages */
  ...
  test_misc29();  /* Test that speculative metadata reads are handled correctly
                   */
  test_misc30();  /* Exercise local heap loading bug where free lists were
                     getting dropped */

  if (default_driver) {
    test_misc31(); /* Test Reentering library through deprecated routines after
                      H5close() */
  }

  test_misc32(); /* Test filter memory allocation functions */
  test_misc33(); /* Test to verify that H5HL_offset_into() returns error if
                    offset exceeds heap block */
