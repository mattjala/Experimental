...
void usage(const char *prog) {
  ...
  fprintf(stderr, "     -l w|r        launch writer or reader only. [default: "
                  "launch both]\n");
...
int parse_option(int argc, char *const argv[]) {
  ...
  while (1) {
    ...
    case 'n': /* number of planes to write/read */
      if ((nlinkedblock_g = atoi(optarg)) < 2) {
        fprintf(stderr,
                "bad number of linked blocks %s, must be greater than 1.\n",
                optarg);
        usage(progname_g);
        Hgoto_error(-1);
      }
  ...
  if (part_size_g < blocksize_g) {
    fprintf(stderr,
            "Blocksize %d should not be bigger than partition size %d\n",
...
int create_wo_file(void) {
  ...
  if ((write_fd_g = HDopen(DATAFILE, O_RDWR | O_TRUNC | O_CREAT,
  ...
  if ((bytes_wrote = HDwrite(write_fd_g, &blkaddr, (size_t)SIZE_BLKADDR)) !=
      SIZE_BLKADDR) {
...
int write_wo_file(void) {
  ...
  for (i = 1; i < nlinkedblock_g; i++) {
    ...
    if ((bytes_wrote = HDwrite(write_fd_g, buffer, (size_t)blocksize_g)) !=
        blocksize_g) {
  ...
  if ((bytes_wrote =
           HDwrite(write_fd_g, &blkaddr_old, (size_t)sizeof(blkaddr_old))) !=
      sizeof(blkaddr_old)) {
    printf("blkaddr write failed in partition %d\n", 0);
    return -1;
  }
...
int read_wo_file(void) {
  ...
  /* keep reading the initial block address until it is non-zero before
   * proceeding. */
  while (blkaddr == 0) {
    HDlseek(read_fd, (HDoff_t)0, SEEK_SET);
    if ((bytes_read = HDread(read_fd, &blkaddr, (size_t)sizeof(blkaddr))) !=
        sizeof(blkaddr)) {
  ...
  while (blkaddr != 0) {
    ...
    if ((bytes_read = HDread(read_fd, buffer, (size_t)blocksize_g)) !=
        blocksize_g) {
      printf("blkaddr read failed in partition %d\n", 0);
      return -1;
    }
