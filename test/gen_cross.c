 ...
 *  the file to be read by cross_read.c, to test reading data from
 * cross-systems. Run it on an OpenVMS, a little-endian, and a big-endian
 * machine.  Change the output file names to vms_data.h5, le_data.h5, and
 * be_data.h5, and put them under hdf5/test/ directory.
 *
...
int create_normal_dset(hid_t fid, hid_t fsid, hid_t msid) {
   ...
   */
  if ((dataset = H5Dcreate2(fid, DATASETNAME, H5T_IEEE_F64LE, fsid, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
int create_scale_offset_dsets_short(hid_t fid, hid_t fsid, hid_t msid) {
   ...
   * endian datatype and default dataset creation properties.
   */
  if ((dataset = H5Dcreate2(fid, DATASETNAME8, H5T_STD_I16LE, fsid, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Now create a dataset with a big-endian type */
  if ((dataset = H5Dcreate2(fid, DATASETNAME9, H5T_STD_I16BE, fsid, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(dataset, H5T_NATIVE_SHORT, msid, fsid, H5P_DEFAULT, data) < 0)
    TEST_ERROR;
...
int create_deflate_dsets_float(hid_t fid, hid_t fsid, hid_t msid) {
  ...
  const char *not_supported =
      "Deflate filter is not enabled. Can't create the dataset.";
...
int create_nbit_dsets_float(hid_t fid, hid_t fsid, hid_t msid) {
   ...
   */
  if ((dataset = H5Dcreate2(fid, DATASETNAME22, datatype, fsid, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Tset_order(datatype, H5T_ORDER_BE) < 0)
    TEST_ERROR;
  if ((dataset = H5Dcreate2(fid, DATASETNAME23, datatype, fsid, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(dataset, H5T_NATIVE_FLOAT, msid, fsid, H5P_DEFAULT, data) < 0)
    TEST_ERROR;
