...
static const char *FILENAME[] = {"unregister_filter_1", "unregister_filter_2",
                                 NULL};
...
static herr_t test_unregister_filters(hid_t fapl_id) {
  ...
  const hsize_t chunk_dims[2] = {FILTER_CHUNK_DIM1,
  ...
  if ((gcpl_id = H5Pcreate(H5P_GROUP_CREATE)) < 0)
    goto error;
  if (H5Pset_filter(gcpl_id, H5Z_FILTER_DUMMY, H5Z_FLAG_MANDATORY, (size_t)0,
                    NULL) < 0)
    goto error;

  /* Create a group using this filter */
  if ((gid = H5Gcreate2(fid1, GROUP_NAME, H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) <
      0)
    goto error;
  ...
  for (i = 0; i < GROUP_ITERATION; i++) {
    ...
    if ((gid_loop = H5Gcreate2(gid, group_name, H5P_DEFAULT, H5P_DEFAULT,
                               H5P_DEFAULT)) < 0)
      goto error;
  ...
  if ((did = H5Dcreate2(fid1, DSET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
  ...
  if ((did = H5Dcreate2(fid2, DSET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl_id, H5P_DEFAULT)) < 0)
    goto error;
