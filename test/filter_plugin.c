...
/* Global size arrays */
static const hsize_t sizes_g[2] = {DSET_DIM1,
                                   DSET_DIM2}; /* Dataset dimensions   */
static const hsize_t hs_sizes_g[2] = {HYPERSLAB_SIZE1,
                                      HYPERSLAB_SIZE2}; /* Hyperslab sizes */
static const hsize_t hs_offsets_g[2] = {
    HYPERSLAB_OFFSET1, HYPERSLAB_OFFSET2}; /* Hyperslab offsets    */
static const hsize_t chunk_sizes_g[2] = {CHUNK_DIM1,
                                         CHUNK_DIM2}; /* Chunk dimensions     */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t allocate_and_init_2D_array(int ***arr, const hsize_t *sizes,
                                         int **initial_values) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t compare_2D_arrays(int **dset1, int **dset2, const hsize_t *sizes,
                                /*OUT*/ hbool_t *are_same) {
...
static herr_t ensure_filter_works(hid_t fid, const char *name, hid_t dcpl_id) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, name, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl_id,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Sselect_hyperslab(sid, H5S_SELECT_SET, hs_offsets_g, NULL, hs_sizes_g,
                          NULL) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:   Tests creating datasets and writing data with dynamically loaded
 *filters
 *
 * Return:    SUCCEED/FAIL
 *
...
static herr_t test_dataset_write_with_filters(hid_t fid) {
   ...
   */
  filter1_data = 9;
  if (H5Pset_filter(dcpl_id, FILTER1_ID, H5Z_FLAG_MANDATORY, (size_t)1,
                    &filter1_data) < 0)
    TEST_ERROR;
  ...
  /* Unregister the dynamic filter for testing purpose. The next time when this
   * test is run for the new file format, the library's H5PL code has to search
   * in the table of loaded plugin libraries for this filter.
   */
  ...
  if (H5get_libversion(&libver_values[1], &libver_values[2],
                       &libver_values[3]) < 0)
    TEST_ERROR;
  if (H5Pset_filter(dcpl_id, FILTER3_ID, H5Z_FLAG_MANDATORY, (size_t)4,
                    libver_values) < 0)
    TEST_ERROR;
...
static herr_t test_creating_groups_using_plugins(hid_t fid) {
  ...
  if (H5Pset_filter(gcpl_id, FILTER4_ID, H5Z_FLAG_MANDATORY, (size_t)0, NULL) <
      0)
    TEST_ERROR;

  /* Create a group using this filter */
  if ((gid = H5Gcreate2(fid, TOP_LEVEL_GROUP_NAME, H5P_DEFAULT, gcpl_id,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < N_SUBGROUPS; i++) {
    ...
    if ((sub_gid = H5Gcreate2(gid, subgroup_name, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
      TEST_ERROR;
...
static herr_t test_path_api_calls(void) {
  ...
  /* Try to remove a path where the index is beyond the table capacity (SHOULD
   * FAIL) */
...
static herr_t test_filter_numbers(void) {
  ...
  H5E_BEGIN_TRY {
    id = H5Z_FILTER_MAX + 1;
    status = H5Pget_filter_by_id2(dcpl_id, id, &flags, &nelmts, NULL, 0, NULL,
                                  &filter_config);
  }
  H5E_END_TRY
...
static herr_t disable_chunk_cache(hid_t fapl_id) {
  ...
  if (H5Pget_cache(fapl_id, &mdc_nelmts, &rdcc_nelmts, &rdcc_nbytes, &rdcc_w0) <
      0)
    TEST_ERROR;
  rdcc_nbytes = 0;
...
int main(void) {
  ...
  for (new_format = FALSE; new_format <= TRUE; new_format++) {
    ...
    /* Turn off the chunk cache, so all the chunks are immediately written to
     * disk */
    ...
    if (new_format) {
      ...
      if ((new_ff_fapl_id = H5Pcopy(old_ff_fapl_id)) < 0)
        TEST_ERROR;
      if (H5Pset_libver_bounds(new_ff_fapl_id, H5F_LIBVER_LATEST,
                               H5F_LIBVER_LATEST) < 0)
        TEST_ERROR;
