...
#define MDSET_ALL_FLAGS                                                        \
  (MDSET_FLAG_CHUNK | MDSET_FLAG_MLAYOUT | MDSET_FLAG_SHAPESAME |              \
   MDSET_FLAG_MDSET | MDSET_FLAG_TCONV | MDSET_FLAG_FILTER)
...
static int test_mdset_location(hid_t fapl_id) {
  ...
  /* Create files */
  if ((file_id1 = H5Fcreate(filename1, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_id)) <
      0)
    TEST_ERROR;
  if ((file_id2 = H5Fcreate(filename2, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_id)) <
      0)
    TEST_ERROR;
  ...
  if ((dset_ids[0] = H5Dcreate2(file_id1, dset_name[0], H5T_NATIVE_UINT,
                                file_space_ids[0], H5P_DEFAULT, H5P_DEFAULT,
                                H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((dset_ids[1] = H5Dcreate2(file_id2, dset_name[1], H5T_NATIVE_UINT,
                                file_space_ids[1], H5P_DEFAULT, H5P_DEFAULT,
                                H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    ret = H5Dwrite_multi(2, dset_ids, mem_type_ids, mem_space_ids,
                         file_space_ids, H5P_DEFAULT, wbufs);
  }
  H5E_END_TRY

  if (ret >= 0) {
    fprintf(stderr,
            "H5Dmulti_write with datasets in multiple files should fail.\n");
    TEST_ERROR;
  }
  ...
  H5E_BEGIN_TRY {
    ret = H5Dread_multi(2, dset_ids, mem_type_ids, mem_space_ids,
                        file_space_ids, H5P_DEFAULT, rbufs);
  }
  H5E_END_TRY

  if (ret >= 0) {
    fprintf(stderr,
            "H5Dmulti_read with datasets in multiple files should fail.\n");
    TEST_ERROR;
  }
...
static int test_mdset(size_t niter, unsigned flags, hid_t fapl_id) {
  ...
  if ((mem_space_ids[0] = H5Screate_simple(
           (flags & MDSET_FLAG_SHAPESAME) ? 2 : 3, dset_dims[0], NULL)) < 0)
    TEST_ERROR;
  ...
  /* If this is a multi layout run, set up different filters and layouts.
   * Chunked and virtual datasets will be set every iteration (with different
   * dims), and contiguous is the default, so no need to set either of those. */
  ...
  for (i = 0; i < niter; i++) {
    ...
    ndsets = (flags & MDSET_FLAG_MLAYOUT) ? 6
             : (flags & MDSET_FLAG_MDSET)
                 ? (size_t)((size_t)HDrandom() % max_dsets) + 1
                 : 1;

    /* Create file */
    if ((file_id = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_id)) <
        0)
      TEST_ERROR;
    ...
    for (j = 0; j < ndsets; j++) {
      ...
      hbool_t use_chunk = (flags & MDSET_FLAG_CHUNK) ||
                          ((flags & MDSET_FLAG_MLAYOUT) && (j == 1 || j == 2));

      /* Generate file dataspace */
      dset_dims[j][0] = (hsize_t)((HDrandom() % MAX_DSET_X) + 1);
      dset_dims[j][1] = (hsize_t)((HDrandom() % MAX_DSET_Y) + 1);
      if ((file_space_ids[j] = H5Screate_simple(
               2, dset_dims[j], use_chunk ? max_dims : NULL)) < 0)
        TEST_ERROR;

      /* Generate chunk if called for by configuration (multi layout uses
       * chunked for datasets 1 and 2) */
      if (use_chunk) {
        ...
        if (H5Pset_virtual(dcpl_id[j], file_space_ids[j], ".", SOURCE_DS_NAME,
                           file_space_ids[j]) < 0)
          TEST_ERROR;
      }

      /* Create dataset */
      /* If MDSET_FLAG_TCONV is set, use a different datatype with 50%
       * probability, so
       * some datasets require type conversion and others do not */
      if ((dset_ids[j] = H5Dcreate2(
               file_id, dset_name[j],
      ...
      /* Create virtual source dataset if necessary.  Use dcpl_id[0] for a
       * contiguous dataset
       */
    ...
    for (j = 0; j < max_dsets; j++)
      for (k = 0; k < MAX_DSET_X; k++)
        for (l = 0; l < MAX_DSET_Y; l++)
          wbufi[j][k][l] =
              (unsigned)((j * MAX_DSET_X * MAX_DSET_Y) + (k * MAX_DSET_Y) + l);
    ...
    for (j = 0; j < OPS_PER_FILE; j++) {
      /* Decide whether to read or write.  Can't read on the first iteration
       * with external
       * layout because the write is needed to create the external file. */
      do_read = (j == 0 && flags & MDSET_FLAG_MLAYOUT)
                    ? FALSE
                    : (hbool_t)(HDrandom() % 2);
      ...
      for (k = 0; k < ndsets; k++) {
        ...
        if (sel_type == 0) {
          /* Hyperslab */
          size_t nhs =
              (size_t)((HDrandom() % MAX_HS) + 1); /* Number of hyperslabs */
          size_t max_hs_x =
              (MAX_HS_X <= dset_dims[k][0])
                  ? MAX_HS_X
                  : dset_dims[k][0]; /* Determine maximum hyperslab size in X */
          size_t max_hs_y =
              (MAX_HS_Y <= dset_dims[k][1])
                  ? MAX_HS_Y
          ...
          for (l = 0; l < nhs; l++) {
            ...
            /* Select hyperslab */
            if (H5Sselect_hyperslab(mem_space_ids[k], H5S_SELECT_OR, start,
                                    NULL, count, NULL) < 0)
              TEST_ERROR;
            if (H5Sselect_hyperslab(file_space_ids[k], H5S_SELECT_OR, start,
                                    NULL, count, NULL) < 0)
              TEST_ERROR;
          ...
          /* Point selection */
          size_t npoints = (size_t)(((size_t)HDrandom() % MAX_POINTS) +
                                    1); /* Number of points */
          ...
          for (l = 0; l < npoints; l++) {
            ...
            points[(2 * l) + 1] =
                (unsigned)((hsize_t)HDrandom() % dset_dims[k][1]);
          } /* end for */
          ...
          count[0] = dset_dims[k][0];
          count[1] = dset_dims[k][1];
          if (H5Sselect_hyperslab(mem_space_ids[k], H5S_SELECT_SET, start, NULL,
                                  count, NULL) < 0)
            TEST_ERROR;
      ...
      if (do_read) {
        if (flags & MDSET_FLAG_MDSET) {
          ...
          /* Read datasets */
          if (H5Dread_multi(ndsets, dset_ids, mem_type_ids, mem_space_ids,
                            file_space_ids, H5P_DEFAULT, rbufs) < 0)
            TEST_ERROR;
        } /* end if */
        else
          /* Read */
        ...
        if (flags & MDSET_FLAG_MDSET) {
          ...
          /* Write datasets */
          if (H5Dwrite_multi(ndsets, dset_ids, mem_type_ids, mem_space_ids,
                             file_space_ids, H5P_DEFAULT, wbufs) < 0)
            TEST_ERROR;
        } /* end if */
        else
          /* Write */
    ...
    /* Cleanup external file.  Need to do this because otherwise there is
     * garbage when the
