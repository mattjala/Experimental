...
static int test_properties(void) {
   ...
   *
   * Note: this image will not contain a valid HDF5 file, as it complicates
   * testing property list functions. In the file driver tests further down,
  ...
  /* Check that sizes are the same, and that the buffers are identical but
   * separate */
  ...
  VERIFY(temp2 != temp,
         "Retrieved buffer is the same as previously retrieved buffer");
 ...
 ******************************************************************************
 */
static void *memcpy_cb(void *dest, const void *src, size_t size,
                       H5FD_file_image_op_t op, void *udata) {
  udata_t *u = (udata_t *)udata;
 ...
 * Function:    realloc_cb
 *
 * Purpose:     This function allows calls to the realloc callback to be
 *tracked.
 *
 ...
 ******************************************************************************
 */
static void *realloc_cb(void *ptr, size_t size, H5FD_file_image_op_t op,
                        void *udata) {
  udata_t *u = (udata_t *)udata;
 ...
 *
 * Purpose:     This function allows calls to the udata_copy callback to be
 *tracked. No copying actually takes place; it is easier to deal with only one
 ...
 *
 * Purpose:     This function allows calls to the udata_free callback to be
 *tracked.
 *
 *              Note: this callback doesn't actually do anything. Since the
 *              udata_copy callback doesn't copy, only one instance of the udata
 *              is kept alive and such it must be freed explicitly at the end of
 *the tests.
 *
 ******************************************************************************
 */
 ...
 *
 * Purpose:     Resets the udata to default values. This facilitates storing
 *only the results of a single operation in the udata.
 *
 ******************************************************************************
 */
static void reset_udata(udata_t *u) {
  u->used_callbacks = 0;
  u->malloc_src = u->memcpy_src = u->realloc_src = u->free_src =
 ...
 *
 * Purpose:     Tests that callbacks are called properly in property list
 *functions.
 *
 ******************************************************************************
 */
static int test_callbacks(void) {
  H5FD_file_image_callbacks_t real_callbacks = {
      &malloc_cb,     &memcpy_cb,     &realloc_cb, &free_cb,
      &udata_copy_cb, &udata_free_cb, NULL};
  H5FD_file_image_callbacks_t null_callbacks = {NULL, NULL, NULL, NULL,
                                                NULL, NULL, NULL};
  ...
  /* Verify values */
  VERIFY(callbacks.image_malloc == &malloc_cb,
         "malloc callback was not set or retrieved properly");
  VERIFY(callbacks.image_memcpy == &memcpy_cb,
         "memcpy callback was not set or retrieved properly");
  VERIFY(callbacks.image_realloc == &realloc_cb,
         "realloc callback was not set or retrieved properly");
  VERIFY(callbacks.image_free == &free_cb,
         "free callback was not set or retrieved properly");
  VERIFY(callbacks.udata_copy == &udata_copy_cb,
         "udata copy callback was not set or retrieved properly");
  VERIFY(callbacks.udata_free == &udata_free_cb,
         "udata free callback was not set or retrieved properly");
   ...
   */
  VERIFY(udata->used_callbacks == (UDATA_COPY),
         "Copying a fapl with no image used incorrect callbacks");
  ...
  VERIFY(udata->used_callbacks == (UDATA_FREE),
         "Closing a fapl with no image used incorrect callbacks");
  ...
  /* Verify that the property's delete callback was called using the correct
   * image callbacks */
  VERIFY(
      udata->used_callbacks == (UDATA_FREE),
  ...
  VERIFY(udata->used_callbacks == 0,
         "attempting to retrieve the image from a fapl without an image has an "
         "unexpected callback");
  ...
  VERIFY(udata->malloc_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_COPY,
         "malloc callback has wrong source");
  VERIFY(udata->memcpy_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_COPY,
         "memcpy callback has wrong source");
  ...
  VERIFY(udata->free_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_CLOSE,
         "free callback has wrong source");
   ...
   * image callbacks */
  VERIFY(
      udata->used_callbacks == (FREE | UDATA_FREE),
  ...
  /* Verify that the correct callbacks were used */
  VERIFY(udata->used_callbacks == (MALLOC | MEMCPY),
         "attempting to retrieve the image from a fapl with an image has an "
         "unexpected callback");
  VERIFY(udata->malloc_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_GET,
         "malloc callback has wrong source");
  VERIFY(udata->memcpy_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_GET,
         "memcpy callback has wrong source");
  ...
  VERIFY(udata->malloc_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_SET,
         "malloc callback has wrong source");
  VERIFY(udata->memcpy_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_SET,
         "memcpy callback has wrong source");
  VERIFY(udata->free_src == H5FD_FILE_IMAGE_OP_PROPERTY_LIST_SET,
         "freec callback has wrong source");
...
static int test_core(void) {
  ...
  VERIFY((udata->used_callbacks == MALLOC) ||
             (udata->used_callbacks == (MALLOC | UDATA_COPY | UDATA_FREE)),
  ...
  VERIFY(udata->used_callbacks == FREE,
         "Closing a core file used the wrong callbacks");
  ...
  reset_udata(udata);
  dset = H5Dcreate2(file, DSET_NAME, H5T_NATIVE_INT, space, H5P_DEFAULT,
  ...
  VERIFY(udata->used_callbacks == (REALLOC),
         "core write used the wrong callbacks");
 ...
 *      'member_file_name' in the code below, but early (4.4.7, at least) gcc
 * only allows diagnostic pragmas to be toggled outside of functions.
 */
H5_GCC_CLANG_DIAG_OFF("format-nonliteral")
static int test_get_file_image(const char *test_banner, const int file_name_num,
                               hid_t fapl, hbool_t user) {
  ...
  /* Create a dataset */
  dset_id = H5Dcreate2(file_id, "dset 0", H5T_NATIVE_INT, space_id, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
...
static int test_get_file_image_error_rejection(void) {
  ...
  H5E_BEGIN_TRY {
    bytes_read =
        H5Fget_file_image(file_id, image_ptr, (size_t)(image_size - 1));
  }
  H5E_END_TRY
  ...
  err = H5Pset_fapl_multi(fapl_id, memb_map, memb_fapl, memb_name, memb_addr,
                          FALSE);
...
int main(void) {
  ...
  for (user = FALSE; user <= TRUE; user++) {
    ...
    if (H5Pset_fapl_sec2(fapl) < 0)
      ...
      errors += test_get_file_image("H5Fget_file_image() with sec2 driver", 0,
                                    fapl, user);
    ...
    if (H5Pset_fapl_stdio(fapl) < 0)
      ...
      errors += test_get_file_image("H5Fget_file_image() with stdio driver", 1,
                                    fapl, user);
    ...
    if (H5Pset_fapl_core(fapl, (size_t)(64 * 1024), TRUE) < 0)
      ...
      errors += test_get_file_image("H5Fget_file_image() with core driver", 2,
                                    fapl, user);
