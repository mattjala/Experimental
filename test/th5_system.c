...
static void test_h5_dirname(void) {
  ...
  dirname = NULL;
  H5E_BEGIN_TRY { ret = H5_dirname(NULL, &dirname); }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY { ret = H5_dirname(path, NULL); }
  H5E_END_TRY
  ...
  *path = '\0';
  ...
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, ".",
             "comparing H5_dirname with non-separated path to \".\"");
  H5MM_free(dirname);
  ...
  VERIFY_STR(dirname, H5_DIR_SEPS,
             "comparing H5_dirname with file separator path to file separator");
  H5MM_free(dirname);
  ...
  VERIFY_STR(
      dirname, H5_DIR_SEPS,
      "comparing H5_dirname with leading separator path to file separator");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "%s%s%s%stestdir", H5_DIR_SEPS,
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(
      dirname, H5_DIR_SEPS,
      "comparing H5_dirname with leading separators path to file separator");
  H5MM_free(dirname);
  ...
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, ".",
             "comparing H5_dirname with trailing separator path to \".\"");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "testdir%s%s%s%s", H5_DIR_SEPS,
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, ".",
             "comparing H5_dirname with trailing separators path to \".\"");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "%stestdir%s", H5_DIR_SEPS,
             H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, H5_DIR_SEPS,
             "comparing H5_dirname with leading and trailing separator path to "
             "file separator");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "%s%s%s%stestdir%s", H5_DIR_SEPS,
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, H5_DIR_SEPS,
             "comparing H5_dirname with leading separators and a trailing "
             "separator path to file separator");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "%stestdir%s%s%s%s", H5_DIR_SEPS,
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, H5_DIR_SEPS,
             "comparing H5_dirname with leading separator and trailing "
             "separators path to file separator");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "%s%s%s%stestdir%s%s%s%s",
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS,
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, H5_DIR_SEPS,
             "comparing H5_dirname with leading and trailing separators path "
             "to file separator");
  H5MM_free(dirname);
  ...
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, "topdir",
             "comparing H5_dirname with normal path to proper dirname");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "%stopdir%sunderdir", H5_DIR_SEPS,
             H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, H5_DIR_SEPS "topdir",
             "comparing H5_dirname with normal path to proper dirname");
  H5MM_free(dirname);
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "%stopdir%sunderdir%s", H5_DIR_SEPS,
             H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
  VERIFY_STR(dirname, H5_DIR_SEPS "topdir",
             "comparing H5_dirname with normal path to proper dirname");
   ...
   */
  dirname = NULL;
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX,
             "%stopdir%sunderdir%s%s%sfinaldir%s", H5_DIR_SEPS, H5_DIR_SEPS,
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_dirname(path, &dirname);
...
static void test_h5_basename(void) {
  ...
  basename = NULL;
  H5E_BEGIN_TRY { ret = H5_basename(NULL, &basename); }
  H5E_END_TRY
  ...
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "topdir%sunderdir%sfinaldir",
             H5_DIR_SEPS, H5_DIR_SEPS);
  H5E_BEGIN_TRY { ret = H5_basename(path, NULL); }
  H5E_END_TRY
  ...
  *path = '\0';
  ...
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "testdirname");
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdirname",
             "comparing H5_basename with non-separated path to same path");
  H5MM_free(basename);
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(
      basename, H5_DIR_SEPS,
      "comparing H5_basename with file separator path to file separator");
  H5MM_free(basename);
  ...
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with leading separator path to filename "
             "component");
  H5MM_free(basename);
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with leading separators path to filename "
             "component");
  H5MM_free(basename);
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with trailing separator path to filename "
             "component");
  H5MM_free(basename);
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with trailing separators path to filename "
             "component");
  H5MM_free(basename);
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with leading and trailing separator path "
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with leading separators and a trailing "
             "separator path to filename component");
  H5MM_free(basename);
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with leading separator and trailing "
             "separators path to filename component");
  H5MM_free(basename);
  ...
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "testdir",
             "comparing H5_basename with leading and trailing separators path "
             "to filename component");
  H5MM_free(basename);
  ...
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX, "topdir%sunderdir", H5_DIR_SEPS);
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "underdir",
             "comparing H5_basename with normal path to proper basename");
  H5MM_free(basename);
  ...
  HDsnprintf(path, H5_SYSTEM_TEST_PATH_MAX,
             ...
             H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS, H5_DIR_SEPS);
  ret = H5_basename(path, &basename);
  VERIFY_STR(basename, "finaldir",
             "comparing H5_basename with contrived path to proper basename");
  H5MM_free(basename);
...
static void test_h5_strcasestr(void) {
  ...
  if (str)
    VERIFY_STR(
        str, haystack,
        "comparing H5_strcasestr to original string for empty substring");
  ...
  if (str)
    VERIFY_STR(str, "string",
               "comparing H5_strcasestr for substring of same case");
  ...
  if (str)
    VERIFY_STR(str, "string",
               "comparing H5_strcasestr for substring of different case");
...
static void test_HDstrcasestr(void) {
  ...
  if (str)
    VERIFY_STR(str, haystack,
               "comparing HDstrcasestr to original string for empty substring");
  ...
  if (str)
    VERIFY_STR(str, "string",
               "comparing HDstrcasestr for substring of same case");
  ...
  if (str)
    VERIFY_STR(str, "string",
               "comparing HDstrcasestr for substring of different case");
...
static void test_h5_strndup(void) {
  ...
  H5E_BEGIN_TRY { str = H5_strndup(NULL, 20); }
  H5E_END_TRY
  ...
  if (str)
    VERIFY_STR(str, "mytest",
               "comparing H5_strndup for partial copy to partial string");
  str = H5MM_xfree(str);
  ...
  if (str)
    VERIFY_STR(str, teststr,
               "comparing H5_strndup for identical copy to original string");
  str = H5MM_xfree(str);
  ...
  if (str)
    VERIFY_STR(str, teststr,
               "comparing H5_strndup with larger 'n' value to original string");
  str = H5MM_xfree(str);
...
void cleanup_h5_system(void) { /* Nothing to cleanup yet */ }
