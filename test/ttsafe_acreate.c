...
void tts_acreate(void) {
  ...
  dataset = H5Dcreate2(file, DATASETNAME, datatype, dataspace, H5P_DEFAULT,
...
void *tts_acreate_thread(void *client_data) {
  ...
  attribute =
      H5Acreate2(attrib_data->dataset, attribute_name, attrib_data->datatype,
...
void cleanup_acreate(void) { HDunlink(FILENAME); }
