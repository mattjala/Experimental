...
static int test_named(hid_t file) {
  ...
  if ((gid = H5Gcreate2(file, "test_named", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static int test_conv(hid_t file) {
  ...
  c_e1 data1[] = {E1_RED,   E1_GREEN, E1_BLUE,  E1_GREEN, E1_WHITE,
  ...
  if ((gid = H5Gcreate2(file, "test_conv", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid, "color_table1", tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < ds_size; i++)
    if ((int)data1[i] != (int)data_short[i]) {
      ...
      printf("    2. data1[%zu]=%d, data_short[%zu]=%d (should be same)\n", i,
             (int)data1[i], i, (int)data_short[i]);
      goto error;
    }

  /* Test converting the data to floating number. Read enum data back as
   * floating number */
  ...
  for (i = 0; i < ds_size; i++)
    if ((int)data1[i] != (int)data_double[i]) {
      ...
      printf("    3. data1[%zu]=%d, data_double[%zu]=%d (should be same)\n", i,
             (int)data1[i], i, (int)data_double[i]);
      goto error;
    }
  ...
  if ((did = H5Dcreate2(gid, "color_table2", H5T_NATIVE_INT, sid, H5P_DEFAULT,
  ...
  if ((did = H5Dcreate2(gid, "color_table3", H5T_NATIVE_DOUBLE, sid,
...
static int test_tr1(hid_t file) {
  ...
  if ((gid = H5Gcreate2(file, "test_tr1", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((sid = H5Screate_simple(1, &ds_size, NULL)) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(gid, "color_table", f_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static int test_tr2(hid_t file) {
  ...
  if ((gid = H5Gcreate2(file, "test_tr2", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static int test_value_dsnt_exist(void) {
  ...
  H5E_BEGIN_TRY { ret = H5Tenum_valueof(tid, "SAX", &val); }
  H5E_END_TRY
  if (ret >= 0)
    FAIL_PUTS_ERROR("H5Tenum_valueof should not pass with a non-existing name");

  val = 3;
  H5E_BEGIN_TRY { ret = H5Tenum_nameof(tid, &val, name, size); }
  H5E_END_TRY
  if (ret >= 0)
  ...
  for (int i = 0; i < N_BAD_VALUES; i++) {
    H5E_BEGIN_TRY { ret = H5Tenum_nameof(tid, &BAD_VALUES[i], name, size); }
    H5E_END_TRY
    if (ret >= 0) {
      H5_FAILED();
  ...
  for (int i = 0; i < N_BAD_NAMES; i++) {
    H5E_BEGIN_TRY { ret = H5Tenum_valueof(tid, BAD_NAMES[i], &val); }
    H5E_END_TRY
    if (ret >= 0) {
      H5_FAILED();
