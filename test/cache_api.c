...
static hbool_t mdc_api_call_smoke_check(int express_test, unsigned paged,
                                        hid_t fcpl_id);
...
static hbool_t check_fapl_mdc_api_calls(unsigned paged, hid_t fcpl_id) {
  ...
  if (pass) {
    ...
    if (!resize_configs_are_equal(&mod_auto_size_ctl, &cache_ptr->resize_ctl,
                                  TRUE)) {
...
static hbool_t check_file_mdc_api_calls(unsigned paged, hid_t fcpl_id) {
  ...
  if (pass) {
    ...
    if (H5Fget_mdc_size(file_id, &max_size, &min_clean_size, &cur_size,
                        &cur_num_entries) < 0) {
      ...
      failure_mssg = "H5Fget_mdc_size() failed 1.\n";
               ...
               (min_clean_size !=
                (size_t)((double)max_size * mod_config_4.min_clean_fraction))) {
...
static hbool_t mdc_api_call_smoke_check(int express_test, unsigned paged,
  ...
  while ((pass) && (n < NUM_RANDOM_ACCESSES)) {
    ...
    a_size[0] = CHUNK_SIZE; /*size of hyperslab*/
    a_size[1] = CHUNK_SIZE;
    status = H5Sselect_hyperslab(filespace_ids[m], H5S_SELECT_SET, offset, NULL,
                                 a_size, NULL);
...
static H5AC_cache_config_t *init_invalid_configs(void) {
  ...
  /* Allocate memory */
  if (NULL == (configs = (H5AC_cache_config_t *)calloc(
...
static hbool_t check_file_mdc_api_errs(unsigned paged, hid_t fcpl_id) {
  ...
  if (pass) {
    ...
    H5E_BEGIN_TRY {
      result = H5Fget_mdc_size((hid_t)-1, &max_size, &min_clean_size, &cur_size,
                               &cur_num_entries);
    }
    H5E_END_TRY
...
int main(void) {
  ...
  if (H5Pset_file_space_strategy(fcpl2_id, H5F_FSPACE_STRATEGY_PAGE, 1,
                                 (hsize_t)1) < 0) {
  ...
  /* The "my_fcpl" passed to each test has the paged or non-paged strategy set
   * up accordingly */
