...
#define ATTR1_RANK 1
#define ATTR1_DIM1 3
static int attr_data1[ATTR1_DIM1] = {512, -234,
                                     98123}; /* Test data for 1st attribute */
...
#define ATTR2_DIM1 2
#define ATTR2_DIM2 2
static int attr_data2[ATTR2_DIM1][ATTR2_DIM2] = {
    {7614, -416}, {197814, -3}}; /* Test data for 2nd attribute */
...
static void test_attr_basic_write(hid_t fapl) {
  ...
  /* Try to create an attribute on the file (should create an attribute on root
   * group) */
  attr = H5Acreate2(fid1, ATTR1_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  ...
  attr = H5Acreate2(dataset, ATTR1_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");

  /* Try to create the same attribute again (should fail) */
  H5E_BEGIN_TRY {
    ret_id = H5Acreate2(dataset, ATTR1_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                        H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  attr2 = H5Acreate2(dataset, ATTR1A_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                     H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");
  ...
  for (i = 0; i < ATTR1_DIM1; i++)
    if (attr_data1[i] != read_data1[i])
      TestErrPrintf("%d: attribute data different: attr_data1[%d]=%d, "
                    "read_data1[%d]=%d\n",
  ...
  for (i = 0; i < ATTR1_DIM1; i++)
    if (attr_data1a[i] != read_data1[i])
      TestErrPrintf("%d: attribute data different: attr_data1a[%d]=%d, "
                    "read_data1[%d]=%d\n",
  ...
  attr = H5Acreate2(group, ATTR2_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");

  /* Check storage size for attribute */
  attr_size = H5Aget_storage_size(attr);
  VERIFY(attr_size, (ATTR2_DIM1 * ATTR2_DIM2 * sizeof(int)),
         "H5Aget_storage_size");

  /* Try to create the same attribute again (should fail) */
  H5E_BEGIN_TRY {
    ret_id = H5Acreate2(group, ATTR2_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                        H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(ret_id, FAIL, "H5Acreate2");
...
static void test_attr_basic_read(hid_t fapl) {
  ...
  int read_data1[ATTR1_DIM1] = {0}; /* Buffer for reading 1st attribute */
  int read_data2[ATTR2_DIM1][ATTR2_DIM2] = {
      {0}};   /* Buffer for reading 2nd attribute */
  ...
  for (i = 0; i < ATTR2_DIM1; i++)
    for (j = 0; j < ATTR2_DIM2; j++)
      if (attr_data2[i][j] != read_data2[i][j])
        TestErrPrintf("%d: attribute data different: attr_data2[%d][%d]=%d, "
                      "read_data2[%d][%d]=%d\n",
...
static void test_attr_flush(hid_t fapl) {
  ...
  set = H5Dcreate2(fil, DSET1_NAME, H5T_NATIVE_DOUBLE, spc, H5P_DEFAULT, dcpl_g,
                   H5P_DEFAULT);
  CHECK(set, FAIL, "H5Dcreate2");

  att = H5Acreate2(set, ATTR1_NAME, H5T_NATIVE_DOUBLE, spc, H5P_DEFAULT,
                   H5P_DEFAULT);
...
static void test_attr_plist(hid_t fapl) {
  ...
  /* Create a dataset */
  did = H5Dcreate2(fid, DSET1_NAME, H5T_NATIVE_UCHAR, dsid, H5P_DEFAULT, dcpl_g,
                   H5P_DEFAULT);
  ...
  /* Re-open the second attribute and ensure that its character encoding is
   * correct */
...
static void test_attr_compound_write(hid_t fapl) {
  ...
  H5E_BEGIN_TRY {
    ret_id =
        H5Acreate2(dataset, ATTR4_NAME, tid1, sid2, H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(ret_id, FAIL, "H5Acreate2");
...
static void test_attr_compound_read(hid_t fapl) {
  ...
  struct attr4_struct
      read_data4[ATTR4_DIM1][ATTR4_DIM2]; /* Buffer for reading 4th attribute */
  ...
  attr = H5Aopen_by_idx(dataset, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                        (hsize_t)0, H5P_DEFAULT, H5P_DEFAULT);
  ...
  for (i = 0; i < fields; i++) {
    ...
    if (!(HDstrcmp(fieldname, ATTR4_FIELDNAME1) != 0 ||
          HDstrcmp(fieldname, ATTR4_FIELDNAME2) != 0 ||
  ...
  VERIFY_TYPE(order, H5Tget_order(H5T_NATIVE_INT), H5T_order_t, "%d",
              "H5Tget_order");
  ...
  order = H5Tget_order(field);
  VERIFY_TYPE(order, H5Tget_order(H5T_NATIVE_DOUBLE), H5T_order_t, "%d",
              "H5Tget_order");
  size = H5Tget_size(field);
  ...
  VERIFY(t_class, H5T_INTEGER, "H5Tget_class");
  order = H5Tget_order(field);
  VERIFY_TYPE(order, H5Tget_order(H5T_NATIVE_SCHAR), H5T_order_t, "%d",
              "H5Tget_order");
  size = H5Tget_size(field);
  ...
  for (i = 0; i < ATTR4_DIM1; i++)
    for (j = 0; j < ATTR4_DIM2; j++)
      if (memcmp(&attr_data4[i][j], &read_data4[i][j],
                 sizeof(struct attr4_struct)) != 0) {
        printf("%d: attribute data different: attr_data4[%d][%d].i=%d, "
               "read_data4[%d][%d].i=%d\n",
               __LINE__, i, j, attr_data4[i][j].i, i, j, read_data4[i][j].i);
        printf("%d: attribute data different: attr_data4[%d][%d].d=%f, "
               "read_data4[%d][%d].d=%f\n",
               __LINE__, i, j, attr_data4[i][j].d, i, j, read_data4[i][j].d);
        TestErrPrintf("%d: attribute data different: attr_data4[%d][%d].c=%c, "
                      "read_data4[%d][%d].c=%c\n",
                      __LINE__, i, j, attr_data4[i][j].c, i, j,
                      read_data4[i][j].c);
      } /* end if */
...
static void test_attr_scalar_write(hid_t fapl) {
  ...
  /* Create an attribute for the dataset */
  attr = H5Acreate2(dataset, ATTR5_NAME, H5T_NATIVE_FLOAT, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");

  /* Try to create the same attribute again (should fail) */
  H5E_BEGIN_TRY {
    ret_id = H5Acreate2(dataset, ATTR5_NAME, H5T_NATIVE_FLOAT, sid2,
...
static void test_attr_scalar_read(hid_t fapl) {
  ...
  if (!H5_FLT_ABS_EQUAL(rdata, attr_data5))
    printf("*** UNEXPECTED VALUE from %s should be %f, but is %f at line %4d "
           "in %s\n",
           "H5Aread", (double)attr_data5, (double)rdata, (int)__LINE__,
           __FILE__);
...
static void test_attr_mult_write(hid_t fapl) {
  ...
  attr = H5Acreate2(dataset, ATTR2_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");

  /* Try to create the same attribute again (should fail) */
  H5E_BEGIN_TRY {
    ret_id = H5Acreate2(dataset, ATTR2_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                        H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(ret_id, FAIL, "H5Acreate2");
  ...
  attr = H5Acreate2(dataset, ATTR3_NAME, H5T_NATIVE_DOUBLE, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");

  /* Try to create the same attribute again (should fail) */
  H5E_BEGIN_TRY {
    ret_id = H5Acreate2(dataset, ATTR3_NAME, H5T_NATIVE_DOUBLE, sid2,
                        H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(ret_id, FAIL, "H5Acreate2");
...
static void test_attr_mult_read(hid_t fapl) {
  ...
  int read_data2[ATTR2_DIM1][ATTR2_DIM2] = {
      {0}}; /* Buffer for reading 2nd attribute */
  double read_data3[ATTR3_DIM1][ATTR3_DIM2][ATTR3_DIM3] = {
      {{0}}};        /* Buffer for reading 3rd attribute */
  ssize_t name_len;  /* Length of attribute name */
  H5O_info2_t oinfo; /* Object info */
  ...
  temp_name[HDstrlen(ATTR1_NAME) - 1] =
      '\0'; /* truncate it to match the one retrieved */
  ...
  attr = H5Aopen_by_idx(dataset, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                        (hsize_t)1, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Aopen_by_idx");
  ...
  temp_name[HDstrlen(ATTR2_NAME) - 1] =
      '\0'; /* truncate it to match the one retrieved */
  if (HDstrcmp(attr_name, temp_name) != 0)
    TestErrPrintf("attribute name different: attr_name=%s, should be %s\n",
                  attr_name, temp_name);
  ...
  /* Open 2nd attribute for the dataset */
  attr = H5Aopen_by_idx(dataset, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                        (hsize_t)2, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Aopen_by_idx");
      ...
      for (k = 0; k < ATTR3_DIM3; k++)
        if (!H5_DBL_ABS_EQUAL(attr_data3[i][j][k], read_data3[i][j][k]))
          TestErrPrintf(
              ...
              "read_data3[%d][%d][%d]=%f\n",
              __LINE__, i, j, k, attr_data3[i][j][k], i, j, k,
              read_data3[i][j][k]);
  ...
  temp_name[HDstrlen(ATTR3_NAME) - 1] =
      '\0'; /* truncate it to match the one retrieved */
  if (HDstrcmp(attr_name, temp_name) != 0)
    TestErrPrintf("attribute name different: attr_name=%s, should be %s\n",
                  attr_name, temp_name);
...
**
****************************************************************/
static herr_t attr_op1(hid_t H5_ATTR_UNUSED loc_id, const char *name,
                       const H5A_info_t H5_ATTR_UNUSED *ainfo, void *op_data) {
...
static void test_attr_iterate(hid_t fapl) {
  ...
  dataset = H5Dcreate2(file, DSET2_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                       dcpl_g, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  count = 0;
  ret =
      H5Aiterate2(dataset, H5_INDEX_NAME, H5_ITER_INC, NULL, attr_op1, &count);
...
static void test_attr_delete(hid_t fapl) {
  ...
  H5E_BEGIN_TRY { ret = H5Adelete(dataset, "Bogus"); }
  H5E_END_TRY
...
static void test_attr_dtype_shared(hid_t fapl) {
  ...
  ret = H5Tcommit2(file_id, TYPE1_NAME, type_id, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");

  /* Check reference count on named datatype */
  ret = H5Oget_info_by_name3(file_id, TYPE1_NAME, &oinfo, H5O_INFO_BASIC,
                             H5P_DEFAULT);
  ...
  /* Create dataset */
  dset_id = H5Dcreate2(file_id, DSET1_NAME, type_id, space_id, H5P_DEFAULT,
                       dcpl_g, H5P_DEFAULT);
  ...
  attr_id = H5Acreate2(dset_id, ATTR1_NAME, type_id, space_id, H5P_DEFAULT,
                       H5P_DEFAULT);
...
static void test_attr_duplicate_ids(hid_t fapl) {
  ...
  hsize_t dims2[] = {ATTR1_DIM1};
  int read_data1[ATTR1_DIM1] = {0}; /* Buffer for reading 1st attribute */
  int rewrite_data[ATTR1_DIM1] = {1234, -423,
                                  9907256}; /* Test data for rewrite */
  int i;
  herr_t ret; /* Generic return value        */
  ...
  /*-----------------------------------------------------------------------------------
   *        Reopen the file and verify the data.  Also rewrite the data and
   * verify it.
   */
  ...
  ret = H5Lcreate_hard(gid1, GROUP1_NAME, H5L_SAME_LOC, GROUP2_NAME,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Lcreate_hard");

  /* Try to create an attribute on the group */
  attr = H5Acreate2(gid1, ATTR2_NAME, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");
...
static int test_attr_dense_verify(hid_t loc_id, unsigned max_attr) {
  ...
  for (u = 0; u < max_attr; u++) {
    ...
    /* Open attribute */
    attr = H5Aopen_by_idx(loc_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                          (hsize_t)u, H5P_DEFAULT, H5P_DEFAULT);
    CHECK(attr, FAIL, "H5Aopen_by_idx");
    ...
    if (HDstrcmp(check_name, attrname) != 0)
      TestErrPrintf(
          "attribute name different: attrname = '%s', should be '%s'\n",
          check_name, attrname);
...
static void test_attr_dense_create(hid_t fcpl, hid_t fapl) {
  ...
  /* Create a dataset */
  dataset = H5Dcreate2(fid, DSET1_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                       dcpl, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  for (u = 0; u < max_compact; u++) {
    /* Create attribute */
    HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
    attr = H5Acreate2(dataset, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                      H5P_DEFAULT);
    CHECK(attr, FAIL, "H5Acreate2");
...
static void test_attr_dense_delete(hid_t fcpl, hid_t fapl) {
  ...
  if (use_min_dset_oh) { /* using minimized dataset headers */
    ...
    ret =
        H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, TRUE, 1);
...
static void test_attr_dense_rename(hid_t fcpl, hid_t fapl) {
  ...
  for (use_corder = FALSE; use_corder <= TRUE; use_corder++) {
    ...
    for (u = 0; u < (max_compact * 2); u++) {
      ...
      /* Rename attribute */
      ret = H5Arename_by_name(fid, DSET1_NAME, attrname, new_attrname,
                              H5P_DEFAULT);
...
static void test_attr_dense_limits(hid_t fcpl, hid_t fapl) {
  ...
  unsigned max_compact,
      rmax_compact; /* Maximum # of attributes to store compactly */
  unsigned min_dense,
      rmin_dense;  /* Minimum # of attributes to store "densely" */
  htri_t is_dense; /* Are attributes stored densely? */
  unsigned u;      /* Local index variable */
  h5_stat_size_t empty_filesize; /* Size of empty file */
  h5_stat_size_t filesize;       /* Size of file after modifications */
...
static void test_attr_dense_dup_ids(hid_t fcpl, hid_t fapl) {
  ...
  /* Create attribute */
  HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
  attr = H5Acreate2(dataset, attrname, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");
  ...
  /*-----------------------------------------------------------------------------------
   *        Open the attribute by index.  Verify the data is shared when the
   * attribute is opened twice.
   */
  /* Open file */
  fid = H5Fopen(FILENAME, H5F_ACC_RDWR, fapl);
  CHECK(fid, FAIL, "H5Fopen");
  ...
  if (read_scalar != scalar_data)
    TestErrPrintf(
        "%d: attribute data different: read_scalar=%d, scalar_data=%d\n",
   ...
   *        attribute storage should switch from dense to compact.  Then open
   * the same attribute for the second time and verify that the attribute data
   *        is shared.
   */
  /* Open file */
  fid = H5Fopen(FILENAME, H5F_ACC_RDWR, fapl);
   ...
   *        attribute storage should switch from compact to dense.  Then open
   * the same attribute for the second time and verify that the attribute data
   *        is shared.
   */
  /* Open file */
  ...
  for (u = min_dense - 1; u <= max_compact; u++) {
    /* Create attribute */
    HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
    add_attr = H5Acreate2(dataset, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                          H5P_DEFAULT);
  ...
  for (u = 0; u < max_compact; u++) {
    /* Create attribute */
    HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
    attr = H5Acreate2(gid1, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                      H5P_DEFAULT);
    CHECK(attr, FAIL, "H5Acreate2");
...
static void test_attr_big(hid_t fcpl, hid_t fapl) {
  ...
  hsize_t dims[ATTR6_RANK] = {ATTR6_DIM1, ATTR6_DIM2,
                              ATTR6_DIM3}; /* Attribute dimensions */
  hid_t attr;                              /* Attribute ID            */
  hid_t dcpl;                   /* Dataset creation property list ID */
  char attrname[NAME_BUF_SIZE]; /* Name of attribute */
  unsigned max_compact;         /* Maximum # of attributes to store compactly */
  ...
  /* Retrieve # of shared message indices (ie. whether attributes are shared or
   * not) */
  ...
  u = 2;
  HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
  attr = H5Acreate2(dataset, attrname, H5T_NATIVE_UINT, big_sid, H5P_DEFAULT,
                    H5P_DEFAULT);
  if (low == H5F_LIBVER_LATEST) {
    ...
    VERIFY(is_empty, FALSE, "H5O__is_attr_empty_test");
    is_dense = H5O__is_attr_dense_test(dataset);
    VERIFY(is_dense, (nshared_indices ? FALSE : TRUE),
    ...
    /* Delete second "small" attribute, attributes should still be stored
     * densely */
...
static void test_attr_null_space(hid_t fcpl, hid_t fapl) {
  ...
  attr = H5Acreate2(dataset, attrname, H5T_NATIVE_UINT, null_sid, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");
...
static void test_attr_deprec(hid_t fcpl, hid_t fapl) {
  ...
  H5E_BEGIN_TRY { ret = H5Aget_num_attrs((hid_t)-1); }
  H5E_END_TRY
...
**
****************************************************************/
static void test_attr_many(hbool_t new_format, hid_t fcpl, hid_t fapl) {
  hid_t fid;                    /* HDF5 File ID            */
  hid_t gid;                    /* Group ID            */
  hid_t sid;                    /* Dataspace ID            */
  ...
  char attrname[NAME_BUF_SIZE]; /* Name of attribute */
  unsigned nattr =
      (new_format ? NATTR_MANY_NEW : NATTR_MANY_OLD); /* Number of attributes */
  ...
  for (u = 0; u < nattr; u++) {
    ...
    aid = H5Acreate2(gid, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                     H5P_DEFAULT);
...
static void test_attr_corder_create_basic(hid_t fcpl, hid_t fapl) {
  ...
  /* Setting invalid combination of a attribute order creation order indexing on
   * should fail */
  H5E_BEGIN_TRY {
  ...
  ret = H5Pset_attr_creation_order(
      dcpl, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED));
  CHECK(ret, FAIL, "H5Pset_attr_creation_order");
  ret = H5Pget_attr_creation_order(dcpl, &crt_order_flags);
  CHECK(ret, FAIL, "H5Pget_attr_creation_order");
  VERIFY(crt_order_flags, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED),
...
**      Tests compact attribute storage on objects with attribute creation order
*info
**
****************************************************************/
static void test_attr_corder_create_compact(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing Compact Storage of Attributes with Creation Order Info\n"));
  ...
  dset2 = H5Dcreate2(fid, DSET2_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT, dcpl,
                     H5P_DEFAULT);
  CHECK(dset2, FAIL, "H5Dcreate2");
  dset3 = H5Dcreate2(fid, DSET3_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT, dcpl,
                     H5P_DEFAULT);
  ...
  for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
    ...
    for (u = 0; u < max_compact; u++) {
      /* Create attribute */
      HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
      attr = H5Acreate2(my_dataset, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                        H5P_DEFAULT);
      CHECK(attr, FAIL, "H5Acreate2");
  ...
  for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
    ...
    /* (the name index is used, but the creation order value is in the same
     * order) */
    for (u = 0; u < max_compact; u++) {
...
**      Tests dense attribute storage on objects with attribute creation order
*info
**
****************************************************************/
static void test_attr_corder_create_dense(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing Dense Storage of Attributes with Creation Order Info\n"));
...
static void test_attr_corder_create_reopen(hid_t fcpl, hid_t fapl) {
  ...
  ret = H5Pset_attr_creation_order(gcpl_id, H5P_CRT_ORDER_TRACKED |
                                                H5P_CRT_ORDER_INDEXED);
  CHECK(ret, FAIL, "H5Pset_attr_creation_order");
  ...
  aid = H5Acreate2(gid, "attr-003", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  buf = 3;
  ...
  aid = H5Acreate2(gid, "attr-004", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  buf = 4;
  ret = H5Awrite(aid, H5T_NATIVE_INT, &buf);
  ...
  aid = H5Acreate2(gid, "attr-008", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  buf = 8;
  ret = H5Awrite(aid, H5T_NATIVE_INT, &buf);
  ...
  aid = H5Acreate2(gid, "attr-006", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  buf = 6;
  ret = H5Awrite(aid, H5T_NATIVE_INT, &buf);
...
**      Tests attribute storage transitions on objects with attribute creation
*order info
**
****************************************************************/
static void test_attr_corder_transition(hid_t fcpl, hid_t fapl) {
  ...
  MESSAGE(
      5,
      ("Testing Storage Transitions of Attributes with Creation Order Info\n"));
  ...
  for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
    ...
    /* Delete several attributes from object, until attribute storage resumes
     * compact form */
...
**      Tests deleting object w/dense attribute storage on objects with
*attribute creation order info
**
****************************************************************/
static void test_attr_corder_delete(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Deleting Object w/Dense Attribute Storage and Creation "
              "Order Info\n"));
  ...
  for (reopen_file = FALSE; reopen_file <= TRUE; reopen_file++) {
    ...
    /* Create datasets */
    dset1 = H5Dcreate2(fid, DSET1_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                       dcpl, H5P_DEFAULT);
    CHECK(dset1, FAIL, "H5Dcreate2");
    dset2 = H5Dcreate2(fid, DSET2_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                       dcpl, H5P_DEFAULT);
    CHECK(dset2, FAIL, "H5Dcreate2");
    dset3 = H5Dcreate2(fid, DSET3_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                       dcpl, H5P_DEFAULT);
    CHECK(dset3, FAIL, "H5Dcreate2");
    ...
    for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
      ...
      for (u = 0; u < max_compact * 2; u++) {
        /* Create attribute */
        HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
        attr = H5Acreate2(my_dataset, attrname, H5T_NATIVE_UINT, sid,
                          H5P_DEFAULT, H5P_DEFAULT);
        CHECK(attr, FAIL, "H5Acreate2");
 ...
 *-------------------------------------------------------------------------
 */
static int attr_info_by_idx_check(hid_t obj_id, const char *attrname, hsize_t n,
                                  hbool_t use_index) {
  ...
  ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
  ...
  memset(&ainfo, 0, sizeof(ainfo));
  ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, n,
                           &ainfo, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Aget_info_by_idx");
  ...
  ret = (herr_t)H5Aget_name_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                                   n, tmpname, (size_t)NAME_BUF_SIZE,
                                   H5P_DEFAULT);
  ...
  if (use_index) {
    ...
    memset(&ainfo, 0, sizeof(ainfo));
    ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE,
                             (hsize_t)0, &ainfo, H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Aget_info_by_idx");
    VERIFY(ainfo.corder, 0, "H5Aget_info_by_idx");
    ...
    memset(&ainfo, 0, sizeof(ainfo));
    ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, n,
                             &ainfo, H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Aget_info_by_idx");
    VERIFY(ainfo.corder, n, "H5Aget_info_by_idx");
  ...
  memset(&ainfo, 0, sizeof(ainfo));
  ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC, n,
                           &ainfo, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Aget_info_by_idx");
  VERIFY(ainfo.corder, 0, "H5Aget_info_by_idx");
  ...
  /* Verify the information for new attribute, in increasing creation order */
  memset(&ainfo, 0, sizeof(ainfo));
  ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                           (hsize_t)0, &ainfo, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Aget_info_by_idx");
  VERIFY(ainfo.corder, n, "H5Aget_info_by_idx");
  ...
  /* Verify the name for new link, in increasing creation order */
  memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
  ret = (herr_t)H5Aget_name_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC,
  ...
  memset(&ainfo, 0, sizeof(ainfo));
  ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)0,
                           &ainfo, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Aget_info_by_idx");
  VERIFY(ainfo.corder, 0, "H5Aget_info_by_idx");
  ...
  memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
  ret = (herr_t)H5Aget_name_by_idx(obj_id, ".", H5_INDEX_NAME, H5_ITER_INC, n,
  ...
  /* Verify the information for new attribute, in increasing name order */
  memset(&ainfo, 0, sizeof(ainfo));
  ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)0,
                           &ainfo, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Aget_info_by_idx");
  VERIFY(ainfo.corder, n, "H5Aget_info_by_idx");
  ...
  /* Verify the name for new link, in increasing name order */
  memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
  ret = (herr_t)H5Aget_name_by_idx(obj_id, ".", H5_INDEX_NAME, H5_ITER_DEC,
                                   (hsize_t)0, tmpname, (size_t)NAME_BUF_SIZE,
                                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Aget_name_by_idx");
  if (HDstrcmp(attrname, tmpname) != 0)
...
**
****************************************************************/
static void test_attr_info_by_idx(hbool_t new_format, hid_t fcpl, hid_t fapl) {
  hid_t fid;                          /* HDF5 File ID            */
  hid_t dset1, dset2, dset3;          /* Dataset IDs            */
  hid_t my_dataset = H5I_INVALID_HID; /* Current dataset ID        */
  hid_t sid;                          /* Dataspace ID            */
  ...
  for (use_index = FALSE; use_index <= TRUE; use_index++) {
    /* Output message about test being performed */
    if (use_index)
      MESSAGE(5, ("Testing Querying Attribute Info By Index w/Creation Order "
                  "Index\n"));
    else
      MESSAGE(5, ("Testing Querying Attribute Info By Index w/o Creation Order "
                  "Index\n"));
    ...
    for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
      ...
      H5E_BEGIN_TRY {
        ret = H5Aget_info_by_idx(my_dataset, ".", H5_INDEX_CRT_ORDER,
                                 H5_ITER_INC, (hsize_t)0, &ainfo, H5P_DEFAULT);
      }
      H5E_END_TRY
      VERIFY(ret, FAIL, "H5Aget_info_by_idx");
      H5E_BEGIN_TRY {
        ret = (herr_t)H5Aget_name_by_idx(my_dataset, ".", H5_INDEX_CRT_ORDER,
      ...
      for (u = 0; u < max_compact; u++) {
        ...
        ret =
            attr_info_by_idx_check(my_dataset, attrname, (hsize_t)u, use_index);
      ...
      H5E_BEGIN_TRY {
        ret = H5Aget_info_by_idx(my_dataset, ".", H5_INDEX_CRT_ORDER,
                                 H5_ITER_INC, (hsize_t)u, &ainfo, H5P_DEFAULT);
      }
      H5E_END_TRY
      VERIFY(ret, FAIL, "H5Aget_info_by_idx");
      H5E_BEGIN_TRY {
        ret = H5Aget_info_by_idx(my_dataset, ".", H5_INDEX_CRT_ORDER,
                                 H5_ITER_DEC, (hsize_t)u, &ainfo, H5P_DEFAULT);
      }
      H5E_END_TRY
      VERIFY(ret, FAIL, "H5Aget_info_by_idx");
      H5E_BEGIN_TRY {
      ...
      for (; u < (max_compact * 2); u++) {
        ...
        /* Verify state of object */
        is_dense = H5O__is_attr_dense_test(my_dataset);
        VERIFY(is_dense, (new_format ? TRUE : FALSE),
               "H5O__is_attr_dense_test");
...
static void test_attr_info_null_info_pointer(hid_t fcpl, hid_t fapl) {
  ...
  /* Create attribute */
  attr = H5Acreate2(fid, GET_INFO_NULL_POINTER_ATTR_NAME, H5T_NATIVE_UINT, sid,
                    H5P_DEFAULT, H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");
  ...
  H5E_BEGIN_TRY {
    err_ret = H5Aget_info_by_name(fid, ".", GET_INFO_NULL_POINTER_ATTR_NAME,
                                  NULL, H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    err_ret = H5Aget_info_by_idx(fid, ".", H5_INDEX_NAME, H5_ITER_INC, 0, NULL,
                                 H5P_DEFAULT);
  }
  H5E_END_TRY
...
static void test_attr_rename_invalid_name(hid_t fcpl, hid_t fapl) {
  ...
  /* Create attribute */
  attr = H5Acreate2(fid, INVALID_RENAME_TEST_ATTR_NAME, H5T_NATIVE_UINT, sid,
                    H5P_DEFAULT, H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");
  ...
  H5E_BEGIN_TRY {
    err_ret = H5Arename_by_name(fid, ".", NULL,
                                INVALID_RENAME_TEST_NEW_ATTR_NAME, H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    err_ret = H5Arename_by_name(fid, ".", "", INVALID_RENAME_TEST_NEW_ATTR_NAME,
                                H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    err_ret = H5Arename_by_name(fid, ".", INVALID_RENAME_TEST_ATTR_NAME, NULL,
                                H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    err_ret = H5Arename_by_name(fid, ".", INVALID_RENAME_TEST_ATTR_NAME, "",
                                H5P_DEFAULT);
  }
  H5E_END_TRY
...
static void test_attr_get_name_invalid_buf(hid_t fcpl, hid_t fapl) {
  ...
  /* Create attribute */
  attr = H5Acreate2(fid, GET_NAME_INVALID_BUF_TEST_ATTR_NAME, H5T_NATIVE_UINT,
                    sid, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(attr, FAIL, "H5Acreate2");
  ...
  H5E_BEGIN_TRY {
    err_ret = H5Aget_name_by_idx(fid, ".", H5_INDEX_NAME, H5_ITER_INC, 0, NULL,
                                 1, H5P_DEFAULT);
  }
  H5E_END_TRY
...
**
****************************************************************/
static void test_attr_delete_by_idx(hbool_t new_format, hid_t fcpl,
                                    hid_t fapl) {
  hid_t fid;                          /* HDF5 File ID            */
  hid_t dset1, dset2, dset3;          /* Dataset IDs            */
  hid_t my_dataset = H5I_INVALID_HID; /* Current dataset ID        */
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              MESSAGE(5, ("Testing Deleting Attribute By Creation Order Index "
                          "in Increasing "
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              MESSAGE(5, ("Testing Deleting Attribute By Name Index in "
                          "Increasing Order "
        ...
        for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
          ...
          H5E_BEGIN_TRY {
            ret = H5Adelete_by_idx(my_dataset, ".", idx_type, order, (hsize_t)0,
                                   H5P_DEFAULT);
          }
          H5E_END_TRY
          ...
          for (u = 0; u < max_compact; u++) {
            ...
            /* Verify information for new attribute */
            ret = attr_info_by_idx_check(my_dataset, attrname, (hsize_t)u,
                                         use_index);
            CHECK(ret, FAIL, "attr_info_by_idx_check");
          } /* end for */
          ...
          H5E_BEGIN_TRY {
            ret = H5Adelete_by_idx(my_dataset, ".", idx_type, order, (hsize_t)u,
                                   H5P_DEFAULT);
          }
          H5E_END_TRY
          VERIFY(ret, FAIL, "H5Adelete_by_idx");
        ...
        for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
          ...
          for (u = 0; u < (max_compact - 1); u++) {
            ...
            /* Verify the attribute information for first attribute in
             * appropriate order */
            memset(&ainfo, 0, sizeof(ainfo));
            ret = H5Aget_info_by_idx(my_dataset, ".", idx_type, order,
                                     (hsize_t)0, &ainfo, H5P_DEFAULT);
            if (new_format) {
              if (order == H5_ITER_INC) {
                ...
                VERIFY(ainfo.corder, (max_compact - (u + 2)),
                       "H5Aget_info_by_idx");
              } /* end else */
            }   /* end if */
            ...
            ret = (herr_t)H5Aget_name_by_idx(
                my_dataset, ".", idx_type, order, (hsize_t)0, tmpname,
                (size_t)NAME_BUF_SIZE, H5P_DEFAULT);
            if (order == H5_ITER_INC)
              HDsnprintf(attrname, sizeof(attrname), "attr %02u", (u + 1));
            else
        ...
        for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
          ...
          if (new_format) {
            /* Retrieve & verify # of records in the name & creation order
             * indices */
            ret = H5O__attr_dense_info_test(my_dataset, &name_count,
                                            &corder_count);
            CHECK(ret, FAIL, "H5O__attr_dense_info_test");
            if (use_index)
              VERIFY(name_count, corder_count, "H5O__attr_dense_info_test");
          ...
          for (u = 0; u < ((max_compact * 2) - 1); u++) {
            ...
            if (new_format) {
              if (order == H5_ITER_INC) {
                ...
                VERIFY(ainfo.corder, ((max_compact * 2) - (u + 2)),
                       "H5Aget_info_by_idx");
              } /* end else */
            }   /* end if */
        ...
        for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
          ...
          /* Delete every other attribute from dense storage, in appropriate
           * order */
          for (u = 0; u < max_compact; u++) {
            /* Delete attribute */
            ret = H5Adelete_by_idx(my_dataset, ".", idx_type, order, (hsize_t)u,
            ...
            memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
            ret = (herr_t)H5Aget_name_by_idx(
                my_dataset, ".", idx_type, order, (hsize_t)u, tmpname,
                (size_t)NAME_BUF_SIZE, H5P_DEFAULT);
            if (order == H5_ITER_INC)
              HDsnprintf(attrname, sizeof(attrname), "attr %02u",
                         ((u * 2) + 1));
        ...
        for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
          ...
          /* Delete remaining attributes from dense storage, in appropriate
           * order */
          for (u = 0; u < (max_compact - 1); u++) {
            /* Delete attribute */
            ret = H5Adelete_by_idx(my_dataset, ".", idx_type, order, (hsize_t)0,
...
**
****************************************************************/
static herr_t attr_iterate2_cb(hid_t loc_id, const char *attr_name,
                               const H5A_info_t *info, void *_op_data) {
  ...
  /* Check more things for revised attribute iteration (vs. older attribute
   * iteration) */
  if (info) {
...
**
****************************************************************/
static herr_t attr_iterate1_cb(hid_t loc_id, const char *attr_name,
                               void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int attr_iterate2_fail_cb(hid_t H5_ATTR_UNUSED group_id,
                                 const char H5_ATTR_UNUSED *attr_name,
                                 const H5A_info_t H5_ATTR_UNUSED *info,
 ...
 *-------------------------------------------------------------------------
 */
static int attr_iterate_check(hid_t fid, const char *dsetname, hid_t obj_id,
  ...
  ret =
      H5Aiterate2(obj_id, idx_type, order, &skip, attr_iterate2_cb, iter_info);
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_attrs - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  ret = H5Aiterate_by_name(fid, dsetname, idx_type, order, &skip,
                           attr_iterate2_cb, iter_info, H5P_DEFAULT);
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_attrs - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  ret = H5Aiterate_by_name(obj_id, ".", idx_type, order, &skip,
                           attr_iterate2_cb, iter_info, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Aiterate_by_name");
  ...
  if (order == H5_ITER_INC) {
    for (v = 0; v < (max_attrs / 2); v++)
      VERIFY(iter_info->visited[v + (max_attrs / 2)], TRUE,
             "H5Aiterate_by_name");
  } /* end if */
  else if (order == H5_ITER_DEC) {
    for (v = 0; v < (max_attrs / 2); v++)
  ...
  iter_info->ncalled = 0;
  iter_info->curr =
      order != H5_ITER_DEC ? (unsigned)oskip : ((max_attrs - 1) - oskip);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  ret = H5Aiterate1(obj_id, &oskip, attr_iterate1_cb, iter_info);
  CHECK(ret, FAIL, "H5Aiterate1");
...
#ifndef H5_NO_DEPRECATED_SYMBOLS
  /* Iterate over attributes on object, stopping in the middle, with
   * H5Aiterate1() */
  iter_info->nskipped = oskip = 0;
  iter_info->order = order;
  iter_info->stop = 3;
  ...
  skip = 0;
  H5E_BEGIN_TRY {
    ret = H5Aiterate2(obj_id, idx_type, order, &skip, attr_iterate2_fail_cb,
                      NULL);
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    ret = H5Aiterate_by_name(fid, dsetname, idx_type, order, &skip,
                             attr_iterate2_fail_cb, NULL, H5P_DEFAULT);
  }
  H5E_END_TRY
...
**
****************************************************************/
static void test_attr_iterate2(hbool_t new_format, hid_t fcpl, hid_t fapl) {
  hid_t fid;                          /* HDF5 File ID            */
  hid_t dset1, dset2, dset3;          /* Dataset IDs            */
  hid_t my_dataset = H5I_INVALID_HID; /* Current dataset ID        */
  hid_t sid;                          /* Dataspace ID            */
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_DEC; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              MESSAGE(5, ("Testing Iterating over Attributes By Creation Order "
                          "Index in "
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              MESSAGE(5, ("Testing Iterating over Attributes By Name Index in "
                          "Increasing Order "
                          "w/Creation Order Index\n"));
            else
              MESSAGE(5, ("Testing Iterating over Attributes By Name Index in "
        ...
        for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
          ...
          ret = H5Aiterate_by_name(fid, dsetname, idx_type, order, NULL,
                                   attr_iterate2_cb, NULL, H5P_DEFAULT);
          CHECK(ret, FAIL, "H5Aiterate_by_name");

          ret = H5Aiterate_by_name(my_dataset, ".", idx_type, order, NULL,
                                   attr_iterate2_cb, NULL, H5P_DEFAULT);
          CHECK(ret, FAIL, "H5Aiterate_by_name");
          ...
          idx = u;
          H5E_BEGIN_TRY {
            ret = H5Aiterate2(my_dataset, idx_type, order, &idx,
                              attr_iterate2_cb, NULL);
          }
          H5E_END_TRY
          VERIFY(ret, FAIL, "H5Aiterate2");
          ...
          idx = u;
          H5E_BEGIN_TRY {
            ret = H5Aiterate_by_name(fid, dsetname, idx_type, order, &idx,
                                     attr_iterate2_cb, NULL, H5P_DEFAULT);
          }
          H5E_END_TRY
          VERIFY(ret, FAIL, "H5Aiterate_by_name");
          ...
          idx = u;
          H5E_BEGIN_TRY {
            ret = H5Aiterate_by_name(my_dataset, ".", idx_type, order, &idx,
                                     attr_iterate2_cb, NULL, H5P_DEFAULT);
          }
          H5E_END_TRY
          VERIFY(ret, FAIL, "H5Aiterate_by_name");
          ...
          ret = attr_iterate_check(fid, dsetname, my_dataset, idx_type, order,
                                   u, &iter_info);
 ...
 *-------------------------------------------------------------------------
 */
static int attr_open_by_idx_check(hid_t obj_id, H5_index_t idx_type,
                                  H5_iter_order_t order, unsigned max_attrs) {
  ...
  for (u = 0; u < max_attrs; u++) {
    /* Open the attribute */
    attr_id = H5Aopen_by_idx(obj_id, ".", idx_type, order, (hsize_t)u,
                             H5P_DEFAULT, H5P_DEFAULT);
...
**
****************************************************************/
static void test_attr_open_by_idx(hbool_t new_format, hid_t fcpl, hid_t fapl) {
  hid_t fid;                          /* HDF5 File ID            */
  hid_t dset1, dset2, dset3;          /* Dataset IDs            */
  hid_t my_dataset = H5I_INVALID_HID; /* Current dataset ID        */
  hid_t sid;                          /* Dataspace ID            */
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_DEC; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              MESSAGE(5, ("Testing Opening Attributes By Creation Order Index "
                          "in Increasing "
                          "Order w/Creation Order Index\n"));
            else
              MESSAGE(5, ("Testing Opening Attributes By Creation Order Index "
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              MESSAGE(5, ("Testing Opening Attributes By Name Index in "
                          "Increasing Order "
                          "w/Creation Order Index\n"));
            else
              MESSAGE(5, ("Testing Opening Attributes By Name Index in "
        ...
        for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
          ...
          H5E_BEGIN_TRY {
            ret_id = H5Aopen_by_idx(my_dataset, ".", idx_type, order,
                                    (hsize_t)0, H5P_DEFAULT, H5P_DEFAULT);
          }
          H5E_END_TRY
 ...
 *-------------------------------------------------------------------------
 */
static int attr_open_check(hid_t fid, const char *dsetname, hid_t obj_id,
                           unsigned max_attrs) {
  hid_t attr_id;                /* ID of attribute to test */
  H5A_info_t ainfo;             /* Attribute info */
  char attrname[NAME_BUF_SIZE]; /* Name of attribute */
  ...
  for (u = 0; u < max_attrs; u++) {
    ...
    /* Open the attribute */
    attr_id =
        H5Aopen_by_name(fid, dsetname, attrname, H5P_DEFAULT, H5P_DEFAULT);
    CHECK(attr_id, FAIL, "H5Aopen_by_name");
...
**
****************************************************************/
static void test_attr_open_by_name(hbool_t new_format, hid_t fcpl, hid_t fapl) {
  hid_t fid;                          /* HDF5 File ID            */
  hid_t dset1, dset2, dset3;          /* Dataset IDs            */
  hid_t my_dataset = H5I_INVALID_HID; /* Current dataset ID        */
  hid_t sid;                          /* Dataspace ID            */
  ...
  for (use_index = FALSE; use_index <= TRUE; use_index++) {
    ...
    if (use_index)
      MESSAGE(5,
              ("Testing Opening Attributes By Name w/Creation Order Index\n"));
    else
      MESSAGE(
          5, ("Testing Opening Attributes By Name w/o Creation Order Index\n"));
    ...
    for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
      ...
      /* Check for opening a non-existent attribute on an object with no
       * attributes */
      H5E_BEGIN_TRY { ret_id = H5Aopen(my_dataset, "foo", H5P_DEFAULT); }
      H5E_END_TRY
      VERIFY(ret_id, FAIL, "H5Aopen");

      H5E_BEGIN_TRY {
        ret_id =
            H5Aopen_by_name(my_dataset, ".", "foo", H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      H5E_BEGIN_TRY {
        ret_id =
            H5Aopen_by_name(fid, dsetname, "foo", H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
      VERIFY(ret_id, FAIL, "H5Aopen_by_name");
      ...
      /* Check for opening a non-existent attribute on an object with compact
       * attribute storage */
      H5E_BEGIN_TRY { ret_id = H5Aopen(my_dataset, "foo", H5P_DEFAULT); }
      H5E_END_TRY
      VERIFY(ret_id, FAIL, "H5Aopen");
    ...
    for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
      ...
      /* Check for opening a non-existent attribute on an object with dense
       * attribute storage */
      H5E_BEGIN_TRY { ret_id = H5Aopen(my_dataset, "foo", H5P_DEFAULT); }
      H5E_END_TRY
      VERIFY(ret_id, FAIL, "H5Aopen");
...
**
****************************************************************/
static void test_attr_create_by_name(hbool_t new_format, hid_t fcpl,
                                     hid_t fapl) {
  hid_t fid;                          /* HDF5 File ID            */
  hid_t dset1, dset2, dset3;          /* Dataset IDs            */
  hid_t my_dataset = H5I_INVALID_HID; /* Current dataset ID        */
  ...
  for (use_index = FALSE; use_index <= TRUE; use_index++) {
    ...
    for (curr_dset = 0; curr_dset < NUM_DSETS; curr_dset++) {
      ...
      for (u = 0; u < max_compact; u++) {
        /* Create attribute */
        HDsnprintf(attrname, sizeof(attrname), "attr %02u", u);
        attr = H5Acreate_by_name(fid, dsetname, attrname, H5T_NATIVE_UINT, sid,
...
**      Tests writing mix of shared & un-shared attributes in "compact" &
*"dense" storage
**
****************************************************************/
static void test_attr_shared_write(hid_t fcpl, hid_t fapl) {
  ...
  hsize_t big_dims[] = {SPACE1_DIM1, SPACE1_DIM2,
                        SPACE1_DIM3}; /* Dimensions for "big" attribute */
  hid_t attr;                         /* Attribute ID            */
  hid_t dcpl;                         /* Dataset creation property list ID */
  char attrname[NAME_BUF_SIZE];       /* Name of attribute */
  unsigned max_compact;    /* Maximum # of attributes to store compactly */
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Writing Shared & Unshared Attributes in Compact & Dense "
              "Storage\n"));

  /* Allocate & initialize "big" attribute data */
  big_value = (unsigned *)malloc(
      (size_t)(SPACE1_DIM1 * SPACE1_DIM2 * SPACE1_DIM3) * sizeof(unsigned));
  CHECK_PTR(big_value, "malloc");
  memset(big_value, 1,
         sizeof(unsigned) * (size_t)(SPACE1_DIM1 * SPACE1_DIM2 * SPACE1_DIM3));
  ...
  for (test_shared = 0; test_shared < 3; test_shared++) {
    ...
    if (test_shared == 0) {
      ...
      ret = H5Pset_shared_mesg_index(my_fcpl, (unsigned)0, H5O_SHMESG_ATTR_FLAG,
                                     (unsigned)500);
      ...
      ret = H5Pset_shared_mesg_index(my_fcpl, (unsigned)1,
                                     H5O_SHMESG_DTYPE_FLAG, (unsigned)1);
      CHECK_I(ret, "H5Pset_shared_mesg_index");
      ret = H5Pset_shared_mesg_index(my_fcpl, (unsigned)2,
                                     H5O_SHMESG_SDSPACE_FLAG, (unsigned)1);
      CHECK_I(ret, "H5Pset_shared_mesg_index");
    } /* end else */
    ...
    /* Commit datatype to file */
    if (test_shared == 2) {
      ret = H5Tcommit2(fid, TYPE1_NAME, attr_tid, H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
      CHECK(ret, FAIL, "H5Tcommit2");
    } /* end if */
    ...
    dataset = H5Dcreate2(fid, DSET1_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT);
    CHECK(dataset, FAIL, "H5Dcreate2");
    dataset2 = H5Dcreate2(fid, DSET2_NAME, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
                          dcpl, H5P_DEFAULT);
    ...
    for (u = 0; u < max_compact * 2; u++) {
      ...
      if (u % 2) {
        ...
        attr = H5Acreate2(dataset, attrname, attr_tid, sid, H5P_DEFAULT,
                          H5P_DEFAULT);
        CHECK(attr, FAIL, "H5Acreate2");
        ...
        attr = H5Acreate2(dataset, attrname, attr_tid, big_sid, H5P_DEFAULT,
                          H5P_DEFAULT);
        CHECK(attr, FAIL, "H5Acreate2");
      ...
      if (u % 2) {
        ...
        attr = H5Acreate2(dataset2, attrname, attr_tid, sid, H5P_DEFAULT,
                          H5P_DEFAULT);
        CHECK(attr, FAIL, "H5Acreate2");
        ...
        attr = H5Acreate2(dataset2, attrname, attr_tid, big_sid, H5P_DEFAULT,
                          H5P_DEFAULT);
        CHECK(attr, FAIL, "H5Acreate2");
...
static void test_attr_shared_rename(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Renaming Shared & Unshared Attributes in Compact & "
              "Dense Storage\n"));
  ...
  for (test_shared = 0; test_shared < 3; test_shared++) {
    ...
    for (u = 0; u < max_compact * 2; u++) {
      ...
      ret =
          H5Arename_by_name(fid, DSET2_NAME, attrname, attrname2, H5P_DEFAULT);
      CHECK(ret, FAIL, "H5Arename_by_name");
      ...
      ret =
          H5Arename_by_name(fid, DSET2_NAME, attrname2, attrname, H5P_DEFAULT);
      CHECK(ret, FAIL, "H5Arename_by_name");
...
static void test_attr_shared_delete(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Deleting Shared & Unshared Attributes in Compact & "
              "Dense Storage\n"));
...
**      Tests unlinking object with  shared attributes in "compact" & "dense"
*storage
**
****************************************************************/
static void test_attr_shared_unlink(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Unlinking Object with Shared Attributes in Compact & "
              "Dense Storage\n"));
...
static void test_attr_bug1(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing Allocating and De-allocating Attributes in Unusual Way\n"));
  ...
  aid = H5Acreate2(gid, ATTR7_NAME, H5T_NATIVE_DOUBLE, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  ...
  /* Re-open file and create another group, then another attribute on first
   * group */
  fid = H5Fopen(FILENAME, H5F_ACC_RDWR, fapl);
  CHECK(fid, FAIL, "H5Fopen");
  ...
  aid = H5Acreate2(gid, ATTR8_NAME, H5T_NATIVE_DOUBLE, sid, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
...
static void test_attr_bug4(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing that attributes can always be added to named datatypes\n"));
...
static void test_attr_bug5(hid_t fcpl, hid_t fapl) {
  ...
  MESSAGE(
      5,
      ("Testing Opening an Attribute Through Multiple Files Concurrently\n"));
  ...
  CHECK(tid1, FAIL, "H5Tcopy");
  ret = H5Tcommit2(fid1, BUG3_DT_NAME, tid1, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");

  /* Create dataset */
  did1 = H5Dcreate2(fid1, BUG3_DSET_NAME, tid1, sid, H5P_DEFAULT, dcpl_g,
                    H5P_DEFAULT);
...
static void test_attr_bug6(hid_t fcpl, hid_t fapl) {
  ...
  aid1 = H5Acreate2(gid, ATTR1_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                    H5P_DEFAULT);
  CHECK(aid1, FAIL, "H5Acreate2");
...
static void test_attr_bug7(hid_t fcpl, hid_t fapl) {
  ...
  CHECK(sid, FAIL, "H5Screate_simple");
  aid =
      H5Acreate2(tid, ATTR1_NAME, H5T_STD_I8LE, sid, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  ...
  if (ainfo.data_size != dims_s)
    TestErrPrintf(
        "attribute data size different: data_size=%llu, should be %llu\n",
   ...
   */
  aid =
      H5Acreate2(tid, ATTR2_NAME, H5T_STD_I8LE, sid, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
  ...
  aid =
      H5Acreate2(tid, ATTR3_NAME, H5T_STD_I8LE, sid, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
...
static void test_attr_bug8(hid_t fcpl, hid_t fapl) {
  ...
  MESSAGE(
      5,
      ("Testing attribute expanding object header with undecoded messages\n"));
  ...
  CHECK(sid, FAIL, "H5Screate_simple");
  aid =
      H5Acreate2(gid, ATTR1_NAME, H5T_STD_I8LE, sid, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
...
static void test_attr_bug9(hid_t fcpl, hid_t fapl) {
  ...
  for (create_link = 0; create_link < 2; create_link++) {
    ...
    for (i = 0; i < max_compact + 1; i++) {
      ...
      aid = H5Acreate2(gid, aname, H5T_NATIVE_CHAR, sid, H5P_DEFAULT,
                       H5P_DEFAULT);
      CHECK(aid, FAIL, "H5Acreate2");
...
static void test_attr_bug10(hid_t fcpl, hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing that vlen attributes can be written to after a second "
...
static void test_attr_delete_last_dense(hid_t fcpl, hid_t fapl) {
  ...
  aid =
      H5Acreate2(gid, ATTRNAME, H5T_IEEE_F64LE, sid, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(aid, FAIL, "H5Acreate2");
...
void test_attr(void) {
  ...
  unsigned
      minimize_dset_oh; /* Whether to use minimized dataset object headers */
  herr_t ret;           /* Generic return value */
  ...
  CHECK_I(ret, "H5Pset_shared_mesg_nindexes");
  ret = H5Pset_shared_mesg_index(fcpl2, (unsigned)0, H5O_SHMESG_ATTR_FLAG,
                                 (unsigned)1);
  CHECK_I(ret, "H5Pset_shared_mesg_index");
  ...
  for (minimize_dset_oh = 0; minimize_dset_oh <= 1; minimize_dset_oh++) {
    ...
    for (new_format = FALSE; new_format <= TRUE; new_format++) {
      ...
      test_attr_flush(
          my_fapl); /* Test H5A I/O in the presence of H5Fflush calls */
      test_attr_plist(my_fapl); /* Test attribute property lists */

      /* These next two tests use the same file information */
      test_attr_compound_write(
          my_fapl); /* Test complex datatype H5A writing code */
      test_attr_compound_read(
          my_fapl); /* Test complex datatype H5A reading code */

      /* These next two tests use the same file information */
      test_attr_scalar_write(
          my_fapl); /* Test scalar dataspace H5A writing code */
      test_attr_scalar_read(
          my_fapl); /* Test scalar dataspace H5A reading code */

      /* These next four tests use the same file information */
      test_attr_mult_write(
          my_fapl); /* Test H5A writing code for multiple attributes */
      test_attr_mult_read(
          my_fapl); /* Test H5A reading code for multiple attributes */
      ...
      test_attr_dtype_shared(
          my_fapl); /* Test using shared datatypes in attributes */
      ...
      for (use_shared = FALSE; use_shared <= TRUE; use_shared++) {
        ...
        test_attr_null_space(
            my_fcpl, my_fapl); /* Test storing attribute with NULL dataspace */
        test_attr_deprec(fcpl, my_fapl); /* Test deprecated API routines */
        test_attr_many(new_format, my_fcpl,
                       my_fapl); /* Test storing lots of attributes */
        test_attr_info_null_info_pointer(
            my_fcpl, my_fapl); /* Test passing a NULL attribute info pointer to
                                  H5Aget_info(_by_name/_by_idx) */
        test_attr_rename_invalid_name(
            my_fcpl, my_fapl); /* Test passing a NULL or empty attribute name to
                                  H5Arename(_by_name) */
        test_attr_get_name_invalid_buf(
            my_fcpl,
            my_fapl); /* Test passing NULL buffer to H5Aget_name(_by_idx) */
        ...
        test_attr_delete_by_idx(new_format, my_fcpl,
                                my_fapl); /* Test deleting attribute by index */
        test_attr_iterate2(
            new_format, my_fcpl,
            my_fapl); /* Test iterating over attributes by index */
        test_attr_open_by_idx(new_format, my_fcpl,
                              my_fapl); /* Test opening attributes by index */
        test_attr_open_by_name(new_format, my_fcpl,
                               my_fapl); /* Test opening attributes by name */
        test_attr_create_by_name(
            new_format, my_fcpl,
            my_fapl); /* Test creating attributes by name */
        ...
        test_attr_bug3(my_fcpl,
                       my_fapl); /* Test "self referential" attributes */
        test_attr_bug4(my_fcpl,
                       my_fapl); /* Test attributes on named datatypes */
        test_attr_bug5(my_fcpl, my_fapl); /* Test opening/closing attributes
                                             through different file handles */
        ...
        test_attr_bug8(my_fcpl, my_fapl); /* Test attribute expanding object
                                             header with undecoded messages */
        test_attr_bug9(
            my_fcpl,
            my_fapl); /* Test large attributes converting to dense storage */
        test_attr_bug10(my_fcpl,
                        my_fapl); /* Test writing an attribute after opening and
                                     closing through a different file handle */

        /* tests specific to the "new format" */
        if (new_format == TRUE) {
          /* General attribute tests */
          test_attr_dense_create(
              my_fcpl, my_fapl); /* Test dense attribute storage creation */
          test_attr_dense_open(
              my_fcpl, my_fapl); /* Test opening attributes in dense storage */
          test_attr_dense_delete(
              my_fcpl, my_fapl); /* Test deleting attributes in dense storage */
          test_attr_dense_rename(
              my_fcpl, my_fapl); /* Test renaming attributes in dense storage */
          test_attr_dense_unlink(my_fcpl,
                                 my_fapl); /* Test unlinking object with
                                              attributes in dense storage */
          test_attr_dense_limits(
              my_fcpl, my_fapl); /* Test dense attribute storage limits */
          test_attr_dense_dup_ids(
              my_fcpl,
          ...
          test_attr_corder_create_basic(
              my_fcpl, my_fapl); /* Test creating an object w/attribute creation
                                    order info */
          test_attr_corder_create_compact(
              my_fcpl, my_fapl); /* Test compact attribute storage on an object
                                    w/attribute creation order info */
          test_attr_corder_create_dense(
              my_fcpl, my_fapl); /* Test dense attribute storage on an object
                                    w/attribute creation order info */
          test_attr_corder_create_reopen(
              my_fcpl, my_fapl); /* Test creating attributes w/reopening file
                                    from using new format to using old format */
          test_attr_corder_transition(
              my_fcpl, my_fapl); /* Test attribute storage transitions on an
                                    object w/attribute creation order info */
          test_attr_corder_delete(
              my_fcpl, my_fapl); /* Test deleting object using dense storage
                                    w/attribute creation order info */

          /* More complex tests with exclusively both "new format" and "shared"
           * attributes */
          if (use_shared == TRUE) {
            test_attr_shared_write(
                my_fcpl, my_fapl); /* Test writing to shared attributes in
                                      compact & dense storage */
            test_attr_shared_rename(my_fcpl,
                                    my_fapl); /* Test renaming shared attributes
                                                 in compact & dense storage */
            test_attr_shared_delete(my_fcpl,
                                    my_fapl); /* Test deleting shared attributes
                                                 in compact & dense storage */
            test_attr_shared_unlink(
                my_fcpl, my_fapl); /* Test unlinking object with shared
           ...
           * and in fact fails if used with the old format due to the
           * attributes being larger than 64K */
          test_attr_bug7(my_fcpl, my_fapl); /* Test creating and deleting large
                                               attributes in ohdr chunk 0 */
