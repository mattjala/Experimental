...
#define SMALL_MAN_MAX_DIRECT_SIZE                                              \
  (64 * 1024)                  /* Managed obj. max. direct block size */
#define SMALL_MAN_MAX_INDEX 32 /* Managed obj. # of bits for total heap size   \
                                */
#define SMALL_MAN_START_ROOT_ROWS                                              \
  1                    /* Managed obj. starting # of root indirect block rows */
#define SMALL_ID_LEN 0 /* "Default" heap ID length */
#define SMALL_STAND_SIZE                                                       \
  (SMALL_MAN_MAX_DIRECT_SIZE -                                                 \
   SMALL_DBLOCK_OVERHEAD) /* Standalone obj. min. size */
...
#define LARGE_MAN_MAX_DIRECT_SIZE                                              \
  (1024 * 1024)                /* Managed obj. max. direct block size */
#define LARGE_MAN_MAX_INDEX 64 /* Managed obj. # of bits for total heap size   \
                                */
#define LARGE_MAN_START_ROOT_ROWS                                              \
  1                    /* Managed obj. starting # of root indirect block rows */
#define LARGE_ID_LEN 0 /* "Default" heap ID length */
#define LARGE_STAND_SIZE                                                       \
  (LARGE_MAN_MAX_DIRECT_SIZE -                                                 \
...
/* The number of settings for testing: page buffering, file space strategy and
...
typedef enum {
  ...
  FHEAP_TEST_FILL_N /* The number of different ways to test filling heap blocks,
                       must be last */
} fheap_test_fill_t;
...
typedef enum {
  ...
  FHEAP_TEST_COMP_N /* The number of different ways to test compressing heap
                       blocks, must be last */
} fheap_test_comp_t;
...
typedef struct fheap_test_param_t {
  fheap_test_type_t
      reopen_heap; /* Whether to re-open the heap during the test */
  fheap_test_del_dir_t
      del_dir; /* Whether to delete objects forward or reverse */
  fheap_test_del_drain_t drain_half; /* Whether to drain half of the objects &
                                        refill, when deleting objects */
 ...
 *-------------------------------------------------------------------------
 */
static int add_obj(H5HF_t *fh, size_t obj_off, size_t obj_size,
                   fheap_heap_state_t *state, fheap_heap_ids_t *keep_ids) {
  ...
  if (state) {
    ...
    hbool_t
        tiny_len_extended; /* Do tiny objects use two bytes for the length? */
  ...
  if (keep_ids) {
    ...
    if (keep_ids->num_ids + 1 > keep_ids->alloc_ids) {
      ...
      if (NULL == (tmp_ids = (unsigned char *)H5MM_realloc(
                       keep_ids->ids, id_len * keep_ids->alloc_ids)))
        TEST_ERROR;
      keep_ids->ids = tmp_ids;
      if (NULL == (tmp_lens = (size_t *)H5MM_realloc(
                       keep_ids->lens, sizeof(size_t) * keep_ids->alloc_ids)))
        TEST_ERROR;
      keep_ids->lens = tmp_lens;
      if (NULL == (tmp_offs = (size_t *)H5MM_realloc(
                       keep_ids->offs, sizeof(size_t) * keep_ids->alloc_ids)))
        TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int begin_test(fheap_test_param_t *tparam, const char *base_desc,
                      fheap_heap_ids_t *keep_ids, size_t *fill_size) {
  char *test_desc; /* Test description */
  const char *del_str = get_del_string(tparam);

  /*
   * Test filling & removing all (small) objects from root direct block of
   * absolute heap
   */
 ...
 *-------------------------------------------------------------------------
 */
static int reopen_file(hid_t *file, H5F_t **f, const char *filename, hid_t fapl,
                       H5HF_t **fh, haddr_t fh_addr,
 ...
 *-------------------------------------------------------------------------
 */
static int open_heap(char *filename, hid_t fapl, const H5HF_create_t *cparam,
                     const fheap_test_param_t *tparam, hid_t *file, H5F_t **f,
                     H5HF_t **fh, haddr_t *fh_addr, fheap_heap_state_t *state,
 ...
 *-------------------------------------------------------------------------
 */
static int reopen_heap(H5F_t *f, H5HF_t **fh, haddr_t fh_addr,
                       const fheap_test_param_t *tparam) {
  /* Check for closing & re-opening the heap */
  if (tparam->reopen_heap) {
    /* Close (empty) heap */
 ...
 *-------------------------------------------------------------------------
 */
static int close_heap(char *filename, hid_t fapl, fheap_test_param_t *tparam,
 ...
 *-------------------------------------------------------------------------
 */
static int del_objs_half_refill(H5F_t *f, H5HF_t **fh,
  ...
  for (u = 0; u < half_nobjs; u++) {
    ...
    if (H5HF_insert(*fh, keep_ids->lens[obj_idx], wobj,
                    &keep_ids->ids[id_len * obj_idx]) < 0)
      FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int fill_heap(H5HF_t *fh, unsigned block_row, size_t obj_size,
                     fheap_heap_state_t *state, fheap_heap_ids_t *keep_ids) {
  unsigned char *wobj;        /* Buffer for object to insert */
  ...
  /* Loop over inserting objects into the root direct block, until there's no
   * more space */
  while (data_size >= obj_size) {
    ...
    if (num_ids > shared_alloc_ids_g) {
      ...
      if (NULL == (shared_ids_g = (unsigned char *)H5MM_realloc(
                       shared_ids_g, id_len * shared_alloc_ids_g)))
        TEST_ERROR;
      if (NULL == (shared_lens_g = (size_t *)H5MM_realloc(
                       shared_lens_g, sizeof(size_t) * shared_alloc_ids_g)))
        TEST_ERROR;
      if (NULL == (shared_offs_g = (size_t *)H5MM_realloc(
                       shared_offs_g, sizeof(size_t) * shared_alloc_ids_g)))
        TEST_ERROR;
  ...
  if (keep_ids) {
    ...
    if (keep_ids->num_ids + num_ids > keep_ids->alloc_ids) {
      keep_ids->alloc_ids = MAX(1024, (keep_ids->alloc_ids * 2));
      if (NULL == (keep_ids->ids = (unsigned char *)H5MM_realloc(
                       keep_ids->ids, id_len * keep_ids->alloc_ids)))
        TEST_ERROR;
      if (NULL == (keep_ids->lens = (size_t *)H5MM_realloc(
                       keep_ids->lens, sizeof(size_t) * keep_ids->alloc_ids)))
        TEST_ERROR;
      if (NULL == (keep_ids->offs = (size_t *)H5MM_realloc(
                       keep_ids->offs, sizeof(size_t) * keep_ids->alloc_ids)))
        TEST_ERROR;
    } /* end if */

    /* Append the IDs onto the array */
    memcpy(&keep_ids->ids[keep_ids->num_ids * id_len], shared_ids_g,
           (num_ids * id_len));
    memcpy(&keep_ids->lens[keep_ids->num_ids], shared_lens_g,
           (num_ids * sizeof(size_t)));
    memcpy(&keep_ids->offs[keep_ids->num_ids], shared_offs_g,
           (num_ids * sizeof(size_t)));
 ...
 *-------------------------------------------------------------------------
 */
static int fill_root_row(H5HF_t *fh, unsigned row, size_t obj_size,
                         fheap_heap_state_t *state,
                         fheap_heap_ids_t *keep_ids) {
  hsize_t
      first_free_space;   /* Size of free space in heap after the first block */
 ...
 *
 * Purpose:   Fill up part of a row of direct blocks in an non-root indirect
 *block
 *
 * Return:    Success:    0
 *
 ...
 *-------------------------------------------------------------------------
 */
static int fill_partial_row(H5HF_t *fh, unsigned row, unsigned width,
                            size_t obj_size, fheap_heap_state_t *state,
                            fheap_heap_ids_t *keep_ids) {
 ...
 *-------------------------------------------------------------------------
 */
static int fill_row(H5HF_t *fh, unsigned row, size_t obj_size,
                    fheap_heap_state_t *state, fheap_heap_ids_t *keep_ids) {
  /* Sanity check */
  assert(fh);
  assert(state);
 ...
 *-------------------------------------------------------------------------
 */
static int fill_root_direct(H5HF_t *fh, size_t obj_size,
                            fheap_heap_state_t *state,
                            fheap_heap_ids_t *keep_ids) {
 ...
 *-------------------------------------------------------------------------
 */
static int fill_2nd_indirect(H5HF_t *fh, unsigned pos, size_t obj_size,
                             fheap_heap_state_t *state,
                             fheap_heap_ids_t *keep_ids) {
  unsigned max_dblock_rows; /* Max. # of direct block rows in indirect block */
 ...
 *-------------------------------------------------------------------------
 */
static int fill_all_direct(H5HF_t *fh, size_t obj_size,
                           fheap_heap_state_t *state,
                           fheap_heap_ids_t *keep_ids) {
  unsigned max_dblock_rows; /* Max. # of direct block rows in indirect block */
  unsigned row;             /* Row being created */
 ...
 *-------------------------------------------------------------------------
 */
static int fill_2nd_indirect_row(H5HF_t *fh, unsigned pos, size_t obj_size,
                                 fheap_heap_state_t *state,
                                 fheap_heap_ids_t *keep_ids) {
  unsigned width; /* Width of heap's doubling table */
  unsigned u;     /* Local index variable */
 ...
 *
 * Purpose:   Insert (small) objects to fill up the free space in all direct
 *              heap blocks in all rows of second-level indirect blocks (which
 ...
 *-------------------------------------------------------------------------
 */
static int fill_all_2nd_indirect_rows(H5HF_t *fh, size_t obj_size,
                                      fheap_heap_state_t *state,
                                      fheap_heap_ids_t *keep_ids) {
  unsigned width; /* Width of heap's doubling table */
  unsigned u;     /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
static int fill_3rd_indirect(H5HF_t *fh, unsigned pos, size_t obj_size,
                             fheap_heap_state_t *state,
                             fheap_heap_ids_t *keep_ids) {
  unsigned u; /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
static int fill_3rd_indirect_row(H5HF_t *fh, unsigned pos, size_t obj_size,
                                 fheap_heap_state_t *state,
                                 fheap_heap_ids_t *keep_ids) {
  unsigned width; /* Width of heap's doubling table */
  unsigned u;     /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
static int fill_all_3rd_indirect_rows(H5HF_t *fh, size_t obj_size,
                                      fheap_heap_state_t *state,
                                      fheap_heap_ids_t *keep_ids) {
  unsigned width; /* Width of heap's doubling table */
  unsigned u;     /* Local index variable */
 ...
 *-------------------------------------------------------------------------
 */
static int fill_4th_indirect_row(H5HF_t *fh, unsigned pos, size_t obj_size,
                                 fheap_heap_state_t *state,
                                 fheap_heap_ids_t *keep_ids) {
  unsigned width; /* Width of heap's doubling table */
  ...
  for (u = 0; u < width; u++) {
    ...
    /* Fill all rows of 2nd level deep indirect blocks in fourth level indirect
     * block */
 ...
 *-------------------------------------------------------------------------
 */
static int fill_all_4th_indirect_rows(H5HF_t *fh, size_t obj_size,
                                      fheap_heap_state_t *state,
                                      fheap_heap_ids_t *keep_ids) {
  unsigned width; /* Width of heap's doubling table */
  unsigned u;     /* Local index variable */
  ...
  for (u = 0; u < (H5VM_log2_of2(width) + 1); u++) {
    ...
    /* (Note: this is tied to the particular doubling table/heap creation
     * parameters) */
    if (u == 0) {
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_create(hid_t fapl, H5HF_create_t *cparam,
                            fheap_test_param_t *tparam) {
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_reopen(hid_t fapl, H5HF_create_t *cparam,
                            fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_open_twice(hid_t fapl, H5HF_create_t *cparam,
                                fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_delete_open(hid_t fapl, H5HF_create_t *cparam,
                                 fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
...
static unsigned test_id_limits(hid_t fapl, H5HF_create_t *cparam, hid_t fcpl) {
  ...
  /* Set the heap ID length to the size needed for directly accessing 'huge'
   * objects */
  ...
  deflate_level = 6;
  if (H5Z_append(&tmp_cparam.pline, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL,
                 (size_t)1, &deflate_level) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_filtered_create(hid_t fapl, H5HF_create_t *cparam,
                                     hid_t fcpl) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
...
static unsigned test_size(hid_t fapl, H5HF_create_t *cparam, hid_t fcpl) {
  ...
  hsize_t
      one_heap_size; /* Total size of heap on disk after inserting one object */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_insert_weird(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                    /* File ID */
  char filename[FHEAP_FILENAME_LEN];  /* Filename to use */
  H5F_t *f = NULL;                    /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_insert_first(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_insert_second(hid_t fapl, H5HF_create_t *cparam,
                                       fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_insert_root_mult(hid_t fapl, H5HF_create_t *cparam,
                                          fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_insert_force_indirect(hid_t fapl,
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_insert_fill_second(hid_t fapl, H5HF_create_t *cparam,
                                            fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Fill the second direct block heap up (also creates initial root indirect
   * block) */
  state.man_size = cparam->managed.width * DBLOCK_SIZE(fh, 0);
  state.man_alloc_size += DBLOCK_SIZE(fh, 0);
  state.man_free_space = (cparam->managed.width - 1) * DBLOCK_FREE(fh, 0);
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_insert_third_direct(hid_t fapl, H5HF_create_t *cparam,
                                             fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_first_row(hid_t fapl, H5HF_create_t *cparam,
                                        fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /*
   * Test inserting mult. (small) objects to fill first row in root indirect
   * block
   */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_start_second_row(hid_t fapl, H5HF_create_t *cparam,
                                          fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /*
   * Test inserting mult. (small) objects to start second row in root indirect
   * block
   */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_second_row(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_start_third_row(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /*
   * Test inserting mult. (small) objects to start third row in root indirect
   * block
   */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_fourth_row(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /*
   * Test inserting mult. (small) objects to fill four rows in root indirect
   * block
   */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_all_root_direct(hid_t fapl, H5HF_create_t *cparam,
                                              fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /*
   * Test inserting mult. (small) objects to fill all direct  rows in root
   * indirect block
   */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_first_recursive_indirect(hid_t fapl,
                                                  H5HF_create_t *cparam,
                                                  fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  /*
   * Test inserting mult. (small) objects to force creation of first recursive
   * indirect block
   */
 ...
 */
static unsigned
test_man_second_direct_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                          fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   */
  TESTING("inserting objects to create second direct block in first recursive "
          "indirect block");
 ...
 */
static unsigned
test_man_fill_first_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                       fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   */
  TESTING("inserting objects to fill all direct blocks in first recursive "
          "indirect block");
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_second_recursive_indirect(hid_t fapl,
                                                   H5HF_create_t *cparam,
                                                   fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
 ...
 */
static unsigned
test_man_fill_second_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                        fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   *  blocks in first recursive indirect block
   */
  TESTING("inserting objects to fill all direct blocks in second recursive "
          "indirect block");
 ...
 */
static unsigned
test_man_fill_recursive_indirect_row(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   *  blocks in first recursive indirect block
   */
  TESTING("inserting objects to fill all direct blocks in first row of "
          "recursive indirect block");
 ...
 */
static unsigned
test_man_start_2nd_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 */
static unsigned
test_man_recursive_indirect_two_deep(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   */
  TESTING(
      "inserting objects to fill recursive indirect blocks two levels deep");
 ...
 */
static unsigned
test_man_start_3rd_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   */
  TESTING(
      "inserting objects to start recursive indirect blocks three levels deep");
 ...
 */
static unsigned
test_man_fill_first_3rd_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                           fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   *  blocks in first recursive indirect block
   */
  TESTING("inserting objects to fill first indirect block of recursive "
 ...
 */
static unsigned
test_man_fill_3rd_recursive_indirect_row(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   *  blocks in first recursive indirect block
   */
  TESTING("inserting objects to fill row of indirect blocks in recursive "
          "indirect blocks three levels deep");
 ...
 */
static unsigned
test_man_fill_all_3rd_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 */
static unsigned
test_man_start_4th_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   *  blocks in first recursive indirect block
   */
  TESTING("inserting objects to start first direct block in recursive indirect "
          "blocks four levels deep");
 ...
 */
static unsigned
test_man_fill_first_4th_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                           fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   *  blocks in first recursive indirect block
   */
  TESTING("inserting objects to fill first (3rd level) indirect block in "
          "recursive indirect block four "
          "levels deep");
  ...
  /* Fill first row of 3rd level deep indirect blocks in fourth level indirect
   * block */
  if (fill_3rd_indirect_row(fh, 1, fill_size, &state, NULL))
    FAIL_STACK_ERROR;
 ...
 */
static unsigned
test_man_fill_4th_recursive_indirect_row(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   *  blocks in first recursive indirect block
   */
  TESTING("inserting objects to fill first row of recursive indirect blocks "
          "four levels deep");
 ...
 */
static unsigned
test_man_fill_all_4th_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
   ...
   */
  TESTING("inserting objects to fill all rows of recursive indirect blocks "
          "four levels deep");
 ...
 */
static unsigned
test_man_start_5th_recursive_indirect(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_bogus(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                    /* File ID */
  char filename[FHEAP_FILENAME_LEN];  /* Filename to use */
  H5F_t *f = NULL;                    /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_one(hid_t fapl, H5HF_create_t *cparam,
                                    fheap_test_param_t *tparam) {
  hid_t file = -1;                    /* File ID */
  char filename[FHEAP_FILENAME_LEN];  /* Filename to use */
  H5F_t *f = NULL;                    /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_two(hid_t fapl, H5HF_create_t *cparam,
                                    fheap_test_param_t *tparam) {
  hid_t file = -1;                     /* File ID */
  char filename[FHEAP_FILENAME_LEN];   /* Filename to use */
  H5F_t *f = NULL;                     /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_one_larger(hid_t fapl, H5HF_create_t *cparam,
                                           fheap_test_param_t *tparam) {
  hid_t file = -1;                    /* File ID */
  char filename[FHEAP_FILENAME_LEN];  /* Filename to use */
  H5F_t *f = NULL;                    /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_two_larger(hid_t fapl, H5HF_create_t *cparam,
                                           fheap_test_param_t *tparam) {
  hid_t file = -1;                     /* File ID */
  char filename[FHEAP_FILENAME_LEN];   /* Filename to use */
  H5F_t *f = NULL;                     /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_three_larger(hid_t fapl, H5HF_create_t *cparam,
                                             fheap_test_param_t *tparam) {
  hid_t file = -1;                     /* File ID */
  char filename[FHEAP_FILENAME_LEN];   /* Filename to use */
  H5F_t *f = NULL;                     /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_incr_insert_remove(hid_t fapl, H5HF_create_t *cparam,
                                            fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Set up data array */
  if (NULL == (heap_id_data = (unsigned char *)calloc(100 * MAX_HEAP_ID_LEN,
  ...
  for (i = 0; i < 100; i++) {
    for (j = 0; j < i; j++) {
      ...
      HDsnprintf(obj2.b, sizeof(obj2.b), "%s%2d", "ABCDEFGHIJKLMNOPQRSTUVWXYZ",
                 j);
    ...
    HDsnprintf(obj1.b, sizeof(obj1.b), "%s%2d", "ABCDEFGHIJKLMNOPQRSTUVWXYZ",
               i);
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_root_direct(hid_t fapl, H5HF_create_t *cparam,
                                            fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  const char *base_desc =
      "removing all objects from root direct block of absolute heap";
  ...
  if (open_heap(filename, fapl, cparam, tparam, &file, &f, &fh, &fh_addr,
                &state, &empty_size) < 0)
    TEST_ERROR;
  ...
  if (close_heap(filename, fapl, tparam, file, f, &fh, fh_addr, &state,
                 &keep_ids, empty_size) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_two_direct(hid_t fapl, H5HF_create_t *cparam,
                                           fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "removing all objects from two direct blocks of absolute heap";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_first_row(hid_t fapl, H5HF_create_t *cparam,
                                          fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "removing all objects from first row of direct blocks of absolute heap";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_first_two_rows(hid_t fapl,
                                               H5HF_create_t *cparam,
                                               fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  fheap_heap_state_t state; /* State of fractal heap */
  /* Test description */
  const char *base_desc = "removing all objects from first two rows of direct "
                          "blocks of absolute heap";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_first_four_rows(hid_t fapl,
                                                H5HF_create_t *cparam,
                                                fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  fheap_heap_state_t state; /* State of fractal heap */
  /* Test description */
  const char *base_desc = "removing all objects from first four rows of direct "
                          "blocks of absolute heap";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_all_root_direct(hid_t fapl,
                                                H5HF_create_t *cparam,
                                                fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  fheap_heap_state_t state; /* State of fractal heap */
  /* Test description */
  const char *base_desc = "removing all objects from all direct blocks of root "
                          "group in absolute heap";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_2nd_indirect(hid_t fapl, H5HF_create_t *cparam,
                                             fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "removing all objects from 2nd level indirect blocks of absolute heap";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_remove_3rd_indirect(hid_t fapl, H5HF_create_t *cparam,
                                             fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "removing all objects from 3rd level indirect blocks of absolute heap";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_skip_start_block(hid_t fapl, H5HF_create_t *cparam,
                                          fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  fheap_heap_state_t state;          /* State of fractal heap */
  /* Test description */
  const char *base_desc = "inserting object that is too large for starting "
                          "block, then remove all objects";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_skip_start_block_add_back(hid_t fapl,
                                                   H5HF_create_t *cparam,
                                                   fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  fheap_heap_state_t state;          /* State of fractal heap */
  /* Test description */
  const char *base_desc = "skipping starting block, then adding object back to "
                          "first block, then remove all objects";
 ...
 */
static unsigned
test_man_skip_start_block_add_skipped(hid_t fapl, H5HF_create_t *cparam,
                                      fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  fheap_heap_state_t state; /* State of fractal heap */
  /* Test description */
  const char *base_desc = "skipping starting block, then adding objects to "
  ...
  /* Insert another object, which should extend direct blocks, instead of
   * backfill */
  state.man_alloc_size += DBLOCK_SIZE(fh, 2);
  if (add_obj(fh, (size_t)20, (size_t)SMALL_OBJ_SIZE2, &state, &keep_ids))
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_skip_2nd_block(hid_t fapl, H5HF_create_t *cparam,
                                        fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  fheap_heap_state_t state;          /* State of fractal heap */
  /* Test description */
  const char *base_desc = "insert object to initial block, then add object too "
                          "large for starting direct "
 ...
 */
static unsigned
test_man_skip_2nd_block_add_skipped(hid_t fapl, H5HF_create_t *cparam,
                                    fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "insert object to initial block, then add object too large for starting "
      "direct "
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_one_partial_skip_2nd_block_add_skipped(
  ...
  unsigned u;               /* Local index variable */
  /* Test description */
  const char *base_desc = "skipping blocks with indirect root, then backfill "
                          "and extend, then remove all objects";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_row_skip_add_skipped(hid_t fapl,
                                                   H5HF_create_t *cparam,
                                                   fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  fheap_heap_state_t state; /* State of fractal heap */
  /* Test description */
  const char *base_desc = "filling first row, then skipping rows, then "
                          "backfill and extend, then remove all objects";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_skip_direct_skip_indirect_two_rows_add_skipped(
    hid_t fapl, H5HF_create_t *cparam, fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  haddr_t fh_addr;                   /* Address of fractal heap */
  fheap_heap_ids_t keep_ids;         /* Structure to retain heap IDs */
  unsigned num_direct_rows;  /* Number of rows (of direct blocks) in root
                                indirect block */
  ...
  /* Test description */
  const char *base_desc =
      "skipping direct blocks to last row and skipping two rows of root "
      "indirect "
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_direct_skip_indirect_start_block_add_skipped(
    hid_t fapl, H5HF_create_t *cparam, fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  fheap_heap_state_t state; /* State of fractal heap */
  /* Test description */
  const char *base_desc = "filling direct blocks and skipping blocks in "
                          "non-root indirect block, then "
                          "backfill and extend, then remove all objects";
  ...
  /* Fill direct block heaps with 2 * initial block size in nested indirect
   * block */
  if (fill_row(fh, 2, fill_size, &state, &keep_ids))
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_direct_skip_2nd_indirect_start_block_add_skipped(
    hid_t fapl, H5HF_create_t *cparam, fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  haddr_t fh_addr;                   /* Address of fractal heap */
  fheap_heap_ids_t keep_ids;         /* Structure to retain heap IDs */
  unsigned num_first_indirect_rows; /* Number of rows (of direct blocks) in each
                                       of the */
  ...
  unsigned u;               /* Local index variable */
  /* Test description */
  const char *base_desc = "filling direct blocks and skipping row of non-root "
                          "indirect blocks, then "
                          "backfill and extend, then remove all objects";
  ...
  /* Fill all rows of direct blocks that are smaller than large object's block
   * size */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_2nd_direct_less_one_wrap_start_block_add_skipped(
    hid_t fapl, H5HF_create_t *cparam, fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  const char *base_desc = "filling direct blocks, filling 2nd level indirect "
                          "blocks, except last one, and insert object too "
                          "large for 2nd level indirect blocks, then backfill "
                          "and extend, then remove all objects";
...
test_man_fill_direct_skip_2nd_indirect_skip_2nd_block_add_skipped(
  ...
  /* Test description */
  const char *base_desc =
      "filling direct blocks and skipping row of non-root indirect blocks, "
      "then skip "
  ...
  /* Finish off blocks in row of medium block size (just to make row filling
   * easier below) */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_direct_skip_indirect_two_rows_add_skipped(
    hid_t fapl, H5HF_create_t *cparam, fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  unsigned num_first_indirect_rows; /* Number of rows (of direct blocks) in each
                                       of the first indirect blocks */
  unsigned max_dblock_rows;  /* Max. # of rows (of direct blocks) in the root
                                indirect block */
  h5_stat_size_t empty_size; /* Size of a file with an empty heap */
  size_t obj_size;           /* Size of object */
  size_t fill_size;          /* Size of objects for "bulk" filled blocks */
  ...
  unsigned u, v;             /* Local index variables */
  /* Test description */
  const char *base_desc = "filling direct blocks and skipping two rows of root "
                          "indirect block, then "
                          "backfill and extend, then remove all objects";
  ...
  /* Fill rows in second row of skipped 2nd level indirect blocks (and used 2nd
   * level block) */
...
test_man_fill_direct_skip_indirect_two_rows_skip_indirect_row_add_skipped(
  ...
  /* Test description */
  const char *base_desc =
      "filling direct blocks and skipping two rows of root indirect block, "
      "skip one row of root indirect "
      "block, then backfill and extend, then remove all objects";
  ...
  if (fill_partial_row(fh, num_first_indirect_rows, cparam->managed.width - 1,
                       fill_size, &state, &keep_ids))
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_fill_2nd_direct_skip_start_block_add_skipped(
    hid_t fapl, H5HF_create_t *cparam, fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc = "filling direct blocks, filling 2nd level indirect "
                          "blocks, and skip first rows of direct blocks of "
                          "3rd level indirect block, then backfill and extend, "
...
test_man_fill_2nd_direct_skip_2nd_indirect_start_block_add_skipped(
  ...
  fheap_heap_state_t state; /* State of fractal heap */
                            /* Test description */
  const char *base_desc =
      "filling direct blocks, filling 2nd level indirect blocks, filling 3rd "
      "level indirect block's direct "
      "blocks, and skip first rows of direct blocks of 3rd level indirect "
      "block's 2nd level indirect "
      "block, then backfill and extend, then remove all objects";
...
test_man_fill_2nd_direct_fill_direct_skip_3rd_indirect_start_block_add_skipped(
  ...
  const char *base_desc =
      "filling direct blocks, filling 2nd level indirect blocks, filling 3rd "
      "level "
      "indirect block's direct blocks, and skip first row of indirect blocks "
      "of 3rd "
...
test_man_fill_2nd_direct_fill_direct_skip2_3rd_indirect_start_block_add_skipped(
  ...
  /* Test description */
  const char *base_desc = "filling direct blocks, filling 2nd level indirect "
                          "blocks, filling 3rd level "
                          "indirect block's direct blocks, and skip first two "
                          "rows of indirect blocks of "
                          "3rd level indirect block, then backfill and extend, "
                          "then remove all objects";
...
test_man_fill_3rd_direct_less_one_fill_direct_wrap_start_block_add_skipped(
  ...
  /* Test description */
  const char *base_desc = "filling direct blocks, filling 2nd level indirect "
                          "blocks, filling first row of 3rd level indirect "
                          "blocks, except last one, fill all direct blocks in "
                          "last 3rd level indirect block, and insert object "
                          "too large for it's 2nd level indirect blocks, then "
                          "backfill and extend, then remove all objects";
...
test_man_fill_1st_row_3rd_direct_fill_2nd_direct_less_one_wrap_start_block_add_skipped(
  ...
  const char *base_desc = "filling direct blocks, filling 2nd level indirect "
                          "blocks, filling first row of 3rd level indirect "
                          "blocks, fill all direct blocks in next 3rd level "
                          "indirect block, fill all 1st row of 2nd level "
                          "indirect blocks, except last one, and insert object "
                          "too large for 2nd level indirect block, then "
                          "backfill and extend, then remove all objects";
...
test_man_fill_3rd_direct_fill_direct_skip_start_block_add_skipped(
  ...
  const char *base_desc =
      "filling direct blocks, filling 2nd level indirect blocks, filling 3rd "
      "level indirect blocks, fill "
      "4th level indirect block's direct blocks, and skip first row of 2nd "
      "indirect blocks of 4th level "
...
test_man_fill_3rd_direct_fill_2nd_direct_fill_direct_skip_3rd_indirect_start_block_add_skipped(
  ...
  const char *base_desc =
      "filling direct blocks, filling 2nd level indirect blocks, filling 3rd "
      "level indirect blocks, fill "
      "4th level indirect block's direct, 2nd level indirect blocks and 3rd "
      "level direct block, and skip "
      "first row of 2nd indirect blocks of 4th level indirect block's 3rd "
      "level indirect block, then "
      "backfill and extend, then remove all objects";
  ...
  /* Fill all direct block rows in fourth level indirect block's 3rd level
   * indirect block */
  if (fill_all_direct(fh, fill_size, &state, &keep_ids))
    TEST_ERROR;
...
test_man_fill_3rd_direct_fill_2nd_direct_fill_direct_skip_3rd_indirect_two_rows_start_block_add_skipped(
  ...
  /* Test description */
  const char *base_desc = "filling direct blocks, filling 2nd level indirect "
                          "blocks, filling 3rd level indirect blocks, fill "
                          "first row of 4th level indirect blocks, fill 2nd "
                          "row 4th level indirect block's direct, 2nd level "
                          "indirect blocks, first row of 3rd level indirect "
                          "blocks, 3rd level direct block in 2nd row, and "
                          "skip first row of 2nd indirect blocks of 4th level "
                          "indirect block's 3rd level indirect block, then "
                          "backfill and extend, then remove all objects";
  ...
  /* Fill all rows of 2nd level indirect blocks in 2nd row 4th level indirect
   * block */
  if (fill_all_2nd_indirect_rows(fh, fill_size, &state, &keep_ids))
    TEST_ERROR;
  ...
  /* Fill first row of 3rd level indirect blocks in 2nd row 4th level indirect
   * block */
  if (fill_3rd_indirect_row(fh, 1, fill_size, &state, &keep_ids))
    TEST_ERROR;
  ...
  /* Fill all direct block rows in 4th level indirect block's 2nd row of 3rd
...
test_man_fill_3rd_direct_fill_2nd_direct_fill_direct_skip_3rd_indirect_wrap_start_block_add_skipped(
  ...
  const char *base_desc = "filling direct blocks, filling 2nd level indirect "
                          "blocks, filling 3rd level indirect blocks, fill "
                          "first row of 3rd level indirect blocks in 4th level "
                          "indirect block except last 3rd level block, "
                          "fill direct blocks in 3rd level block, and skip row "
                          "of 2nd indirect blocks of 4th level indirect "
                          "block's 3rd level indirect block, then backfill and "
                          "extend, then remove all objects";
  ...
  /* Fill first row (except one) of 3rd level indirect blocks in 4th level
   * indirect block */
  for (u = 0; u < cparam->managed.width - 1; u++) {
  ...
  /* Fill all direct block rows in 4th level indirect block's last 3rd level
   * indirect block */
  if (fill_all_direct(fh, fill_size, &state, &keep_ids))
    TEST_ERROR;
...
test_man_fill_4th_direct_less_one_fill_2nd_direct_fill_direct_skip_3rd_indirect_wrap_start_block_add_skipped(
  ...
  const char *base_desc =
      "filling direct blocks, filling 2nd level indirect blocks, filling 3rd "
      "level indirect blocks, fill "
      "first row of 4th level indirect blocks, except last one, fill first row "
      "of 3rd level indirect "
      "blocks in last 4th level indirect block except last 3rd level block, "
      "fill direct blocks in 3rd "
      "level block, and skip row of 2nd indirect blocks of 4th level indirect "
      "block's 3rd level indirect "
      "block, then backfill and extend, then remove all objects";
  ...
  /* Fill row (except one) of 3rd level indirect blocks in 4th level indirect
   * block */
  for (u = 0; u < cparam->managed.width - 1; u++) {
    /* Fill all direct block rows in 3rd level indirect block */
    if (fill_all_direct(fh, fill_size, &state, &keep_ids))
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_frag_simple(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  unsigned u;                        /* Local index variables */
  /* Test description */
  const char *base_desc = "fragmenting small blocks, then backfill and extend, "
                          "then remove all objects";
  ...
  /* Go back and fill in direct blocks of initial block size (which have large
   * free space in them) */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_frag_direct(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  unsigned u, v;                     /* Local index variables */
  /* Test description */
  const char *base_desc = "fragmenting direct blocks, then backfill and "
                          "extend, then remove all objects";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_frag_2nd_direct(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "fill root direct blocks, then fragment 2nd level indirect block's "
      "direct "
      "blocks, then backfill and extend, then remove all objects";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_man_frag_3rd_direct(hid_t fapl, H5HF_create_t *cparam,
                                         fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "fill root direct blocks and 2nd level indirect blocks, then fragment "
      "3rd level indirect block's "
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_huge_insert_one(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_huge_insert_two(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_huge_insert_three(hid_t fapl, H5HF_create_t *cparam,
                                       fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_huge_insert_mix(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_filtered_huge(hid_t fapl, H5HF_create_t *cparam,
                                   fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  hbool_t huge_ids_direct;           /* Are 'huge' objects directly accessed? */
  hbool_t pline_init =
      FALSE; /* Whether the I/O pipeline has been initialized */
  /* Test description */
  const char *base_desc =
      "insert 'huge' object into heap with I/O filters, then remove";
  ...
  /* Adjust actual ID length, if asking for IDs that can directly access 'huge'
   * objects */
  if (cparam->id_len == 1) {
    old_actual_id_len = tparam->actual_id_len;
    tparam->actual_id_len =
        29; /* 1 + 8 (file address size) + 8 (file length size) + 4 (filter mask
               length) + 8 (object length size) */
  }         /* end if */

  /* Perform common file & heap open operations */
  if (open_heap(filename, fapl, &tmp_cparam, tparam, &file, &f, &fh, &fh_addr,
                &state, &empty_size) < 0)
    TEST_ERROR;
  ...
  /* Reset actual ID length, if asking for IDs that can directly access 'huge'
   * objects */
  if (cparam->id_len == 1)
  ...
  H5O_msg_reset(
      H5O_PLINE_ID,
      &tmp_cparam.pline); /* Release the I/O pipeline filter information */
  H5MM_xfree(heap_id);
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_tiny_insert_one(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_tiny_insert_two(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_tiny_insert_mix(hid_t fapl, H5HF_create_t *cparam,
                                     fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "insert mix of normal, huge & tiny objects, then remove";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_filtered_man_root_direct(hid_t fapl, H5HF_create_t *cparam,
                                              fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  ...
  /* Test description */
  const char *base_desc =
      "insert one 'managed' object into heap with I/O filters, then remove";
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_filtered_man_root_indirect(hid_t fapl,
                                                H5HF_create_t *cparam,
                                                fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  /* Test description */
  const char *base_desc =
      "insert two 'managed' objects into heap with I/O filters, then remove";
  ...
  state.man_free_space =
      (cparam->managed.width * DBLOCK_FREE(fh, 0)) - (obj_size * 2);
  state.man_nobjs += 2;
  if (check_stats(fh, &state))
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_random(hsize_t size_limit, hid_t fapl,
                            H5HF_create_t *cparam, fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
  H5HF_t *fh = NULL;                 /* Fractal heap wrapper */
  ...
  if (tparam->del_dir == FHEAP_DEL_HEAP) {
    if (tparam->comp == FHEAP_TEST_COMPRESS)
      TESTING("inserting random-sized objects in heap with compressed blocks, "
              "then remove all objects "
              "(all - deleting heap)");
    else
...
fprintf(stderr, "Random # seed was: %lu\n", seed);
  ...
  while (total_obj_added < size_limit) {
    /* Choose a random size of object (from 1 up to above standalone block size
     * limit) */
  ...
  for (u = 0; u < keep_ids.num_ids; u++) {
    ...
    if (pos > 0) {
      ...
      memcpy(&keep_ids.ids[u * id_len], &keep_ids.ids[(u + pos) * id_len],
             id_len);
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_random_pow2(hsize_t size_limit, hid_t fapl,
                                 H5HF_create_t *cparam,
                                 fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  ...
  if (tparam->del_dir == FHEAP_DEL_HEAP) {
    if (tparam->comp == FHEAP_TEST_COMPRESS)
      TESTING("inserting random-sized objects with power of 2 distribution in "
              "heap with compressed "
              "blocks, then remove all objects (all - deleting heap)");
    else
      TESTING("inserting random-sized objects with power of 2 distribution, "
              "then remove all objects "
              "(all - deleting heap)");
  } /* end if */
  else {
...
fprintf(stderr, "Random # seed was: %lu\n", seed);
  ...
  while (total_obj_added < size_limit) {
    size_t size_range =
        (tmp_cparam.managed.start_block_size / 8); /* Object size range */
...
static unsigned test_write(hid_t fapl, H5HF_create_t *cparam,
                           fheap_test_param_t *tparam) {
  hid_t file = -1;                         /* File ID */
  char filename[FHEAP_FILENAME_LEN];       /* Filename to use */
  H5F_t *f = NULL;                         /* Internal file object pointer */
  ...
  unsigned char *rewrite_obj =
      NULL;                  /* Pointer to re-write buffer for objects */
  fheap_heap_ids_t keep_ids; /* Structure to retain heap IDs */
  h5_stat_size_t empty_size; /* Size of a file with an empty heap */
  size_t obj_size;           /* Size of object */
  size_t obj_loc;            /* Location of object in buffer */
  ...
  if (tparam->comp == FHEAP_TEST_COMPRESS) {
    ...
    if (H5Zregister(&filter_class) < 0)
      TEST_ERROR;
    if (H5Z_append(&tmp_cparam.pline, H5Z_FILTER_RESERVED + 43, 0, (size_t)0,
                   NULL) < 0)
      FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_bug1(hid_t fapl, H5HF_create_t *cparam,
                          fheap_test_param_t *tparam) {
  hid_t file = -1;                   /* File ID */
  char filename[FHEAP_FILENAME_LEN]; /* Filename to use */
  H5F_t *f = NULL;                   /* Internal file object pointer */
...
int main(void) {
  ...
  hid_t fcpl = -1,
        def_fcpl = -1;         /* File creation property list for data files */
  fheap_test_type_t curr_test; /* Current test being worked on */
  unsigned u, v;               /* Local index variable */
  unsigned nerrors = 0;        /* Cumulative error count */
  unsigned num_pb_fs = 1;  /* The number of settings to test for page buffering
                              and file space handling */
  ...
  for (v = 0; v < num_pb_fs; v++) {
    /* Skip test when:
       ...
       because the library will fail file creation (temporary) for the above
       conditions */
    ...
    switch (v) {
    case 0:
      if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, FALSE,
                                     (hsize_t)1) < 0)
        TEST_ERROR;
      fapl = def_fapl;
      break;
    case 1:
      if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, TRUE,
        ...
        TEST_ERROR;
      fapl = def_fapl;
      /* This is a fix for the daily test failure from the checkin for libver
       * bounds. */
      /*
       * Many tests failed the file size check when comparing (a) and (b) as
       * below:
       ...
       *   When the file is initially created with persisting free-space and
       * with (earliest, latest) libver bounds, the file will have version 2
       * superblock due to non-default free-space handling.  As the low bound is
       * earliest, the library uses version 1 object header when creating the
       * superblock extension message. When the file is reopened with the same
       * libver bounds, the file's low bound is upgraded to v18 because the file
       * has version 2 superblock. When the library creates the superblock
       * extension message on file close, the library uses version 2 object
       * header for the superblock extension message since the low bound is v18.
       ...
       *  The fix:
       *    Set libver bounds in fapl to (v18, latest) so that the file created
       * in the test routines will have low bound set to v18.  This will cause
       * the library to use version 2 object header for the superblock extension
       *    message.
       */
    ...
    for (curr_test = FHEAP_TEST_NORMAL; curr_test < FHEAP_TEST_NTESTS;
...

      {
        ...
        for (fill = FHEAP_TEST_FILL_LARGE; fill < FHEAP_TEST_FILL_N; fill++) {
          ...
          nerrors +=
              test_man_insert_force_indirect(fapl, &small_cparam, &tparam);
          ...
          nerrors +=
              test_man_fill_all_root_direct(fapl, &small_cparam, &tparam);
          nerrors +=
              test_man_first_recursive_indirect(fapl, &small_cparam, &tparam);
          nerrors += test_man_second_direct_recursive_indirect(
              fapl, &small_cparam, &tparam);
          nerrors += test_man_fill_first_recursive_indirect(fapl, &small_cparam,
                                                            &tparam);
          nerrors +=
              test_man_second_recursive_indirect(fapl, &small_cparam, &tparam);
          nerrors += test_man_fill_second_recursive_indirect(
              fapl, &small_cparam, &tparam);
          nerrors += test_man_fill_recursive_indirect_row(fapl, &small_cparam,
                                                          &tparam);
          nerrors += test_man_start_2nd_recursive_indirect(fapl, &small_cparam,
                                                           &tparam);
          nerrors += test_man_recursive_indirect_two_deep(fapl, &small_cparam,
                                                          &tparam);
          nerrors += test_man_start_3rd_recursive_indirect(fapl, &small_cparam,
                                                           &tparam);
          nerrors += test_man_fill_first_3rd_recursive_indirect(
              fapl, &small_cparam, &tparam);
          nerrors += test_man_fill_3rd_recursive_indirect_row(
              fapl, &small_cparam, &tparam);
          nerrors += test_man_fill_all_3rd_recursive_indirect(
              fapl, &small_cparam, &tparam);
          nerrors += test_man_start_4th_recursive_indirect(fapl, &small_cparam,
                                                           &tparam);
          nerrors += test_man_fill_first_4th_recursive_indirect(
              fapl, &small_cparam, &tparam);
          nerrors += test_man_fill_4th_recursive_indirect_row(
              fapl, &small_cparam, &tparam);
          nerrors += test_man_fill_all_4th_recursive_indirect(
              fapl, &small_cparam, &tparam);
#endif /* ALL_INSERT_TESTS */
          /* If this test fails, uncomment the tests above, which build up to
           * this level of complexity gradually. -QAK
           */
          if (ExpressMode > 1)
            printf("***Express test mode on.  "
                   "test_man_start_5th_recursive_indirect is skipped\n");
          else
...

          {
            ...
            for (del_dir = FHEAP_DEL_FORWARD; del_dir < FHEAP_DEL_NDIRS;
              ...
              for (drain_half = FHEAP_DEL_DRAIN_ALL;
                   drain_half < FHEAP_DEL_DRAIN_N; drain_half++) {
                tparam.drain_half = drain_half;
                /* Don't need to test deletion directions when deleting entire
                 * heap */
                if (tparam.del_dir == FHEAP_DEL_HEAP &&
                ...
                nerrors +=
                    test_man_remove_root_direct(fapl, &small_cparam, &tparam);
                nerrors +=
                    test_man_remove_two_direct(fapl, &small_cparam, &tparam);
                nerrors +=
                    test_man_remove_first_row(fapl, &small_cparam, &tparam);
                nerrors += test_man_remove_first_two_rows(fapl, &small_cparam,
                                                          &tparam);
                ...
                nerrors +=
                    test_man_skip_start_block(fapl, &small_cparam, &tparam);
                nerrors += test_man_skip_start_block_add_back(
                    fapl, &small_cparam, &tparam);
                nerrors += test_man_skip_start_block_add_skipped(
                    fapl, &small_cparam, &tparam);
                nerrors +=
                    test_man_skip_2nd_block(fapl, &small_cparam, &tparam);
                nerrors += test_man_skip_2nd_block_add_skipped(
                    fapl, &small_cparam, &tparam);
                nerrors += test_man_fill_one_partial_skip_2nd_block_add_skipped(
                    fapl, &small_cparam, &tparam);
                nerrors += test_man_fill_row_skip_add_skipped(
                    fapl, &small_cparam, &tparam);
                nerrors +=
                    test_man_skip_direct_skip_indirect_two_rows_add_skipped(
                        fapl, &small_cparam, &tparam);
                ...
                nerrors +=
                    test_man_frag_2nd_direct(fapl, &small_cparam, &tparam);
                nerrors +=
                    test_man_frag_3rd_direct(fapl, &small_cparam, &tparam);
              } /* end for */
            }   /* end for */
       ...
       */
      {
        ...
        for (id_len = 0; id_len < 3; id_len++) {
          ...
          case 1:
            ...
            /* Adjust actual length of heap IDs for directly storing 'huge'
             * object's file offset & length in heap ID */
            tparam.actual_id_len =
                17; /* 1 + 8 (file address size) + 8 (file length size) */
            break;

          /* Use "direct" storage for 'huge' objects and larger IDs for 'tiny'
           * objects */
          case 2:
            small_cparam.id_len = 37;
            HDputs("Using 'direct' heap ID format for 'huge' objects and "
                   "larger IDs for "
                   "'tiny' objects");
      ...
      /* Try several different methods of deleting objects */
      {
        ...
        for (del_dir = FHEAP_DEL_FORWARD; del_dir < FHEAP_DEL_NDIRS;
          ...
          /* Controlled tests */
          /* XXX: Re-enable file size checks in these tests, after the file has
           * persistent free space tracking working */
          nerrors +=
              test_filtered_man_root_direct(fapl, &small_cparam, &tparam);
          nerrors +=
              test_filtered_man_root_indirect(fapl, &small_cparam, &tparam);
      ...
      if (ExpressMode > 1)
        ...
        /* XXX: Try to speed things up enough that these tests don't have to be
         * reduced when re-opening */
        tparam.del_dir = FHEAP_DEL_FORWARD;
        nerrors += test_random((curr_test == FHEAP_TEST_NORMAL
                                    ? (hsize_t)(100 * 1000 * 1000)
                                    : (hsize_t)(50 * 1000 * 1000)),
                               fapl, &small_cparam, &tparam);
        nerrors += test_random_pow2((curr_test == FHEAP_TEST_NORMAL
                                         ? (hsize_t)(100 * 1000 * 1000)
