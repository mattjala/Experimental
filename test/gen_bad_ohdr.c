...
int main(void) {
  ...
  if (H5Pinsert2(gcpl, H5O_BAD_MESG_COUNT_NAME, H5O_BAD_MESG_COUNT_SIZE,
                 &store_bad_mesg_count, NULL, NULL, NULL, NULL, NULL, NULL) < 0)
    goto error;
  ...
  if ((aid = H5Acreate2(gid, ATTRNAME1, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto error;
  if (H5Aclose(aid) < 0)
    goto error;
  ...
  if ((aid = H5Acreate2(gid, ATTRNAME2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto error;
  if (H5Aclose(aid) < 0)
    goto error;
