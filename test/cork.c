...
static unsigned verify_old_dset_cork(void) {
  ...
  hid_t did = H5I_INVALID_HID, did2 = H5I_INVALID_HID,
        did3 = H5I_INVALID_HID; /* Dataset IDs */
  hid_t dcpl = H5I_INVALID_HID, dcpl2 = H5I_INVALID_HID,
        dcpl3 = H5I_INVALID_HID; /* Dataset creation property lists */
  hid_t sid = H5I_INVALID_HID, sid2 = H5I_INVALID_HID,
        sid3 = H5I_INVALID_HID; /* Dataspace IDs */
  hsize_t dims[2] = {100, 20};  /* Dataset dimension sizes */
  hsize_t max_dims[2] = {100,
  ...
  if ((sid = H5Screate_simple(2, dims, max_dims)) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(fid, DSET_BT1, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf_data) <
      0)
    TEST_ERROR;
  ...
  if (H5Pset_alloc_time(dcpl2, H5D_ALLOC_TIME_EARLY) < 0)
    FAIL_STACK_ERROR;
  if ((did2 = H5Dcreate2(fid, DSET_COMPACT, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                         dcpl2, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Pset_layout(dcpl3, H5D_CONTIGUOUS) < 0)
    FAIL_STACK_ERROR;
  if ((did3 = H5Dcreate2(fid, DSET_CONTIG, H5T_NATIVE_INT, sid3, H5P_DEFAULT,
                         dcpl3, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static unsigned verify_obj_dset_cork(hbool_t swmr) {
  ...
  /* Create dataset: DSET */
  if ((did = H5Dcreate2(fid, DSET, H5T_NATIVE_UCHAR, sid, H5P_DEFAULT,
  ...
  if ((sid2 = H5Screate_simple(1, dim, NULL)) < 0)
    TEST_ERROR;
  if ((did2 = H5Dcreate2(fid, DSET_NONE, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                         dcpl2, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < 8; i++) {
    ...
    if ((aid = H5Acreate2(did2, attrname, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      TEST_ERROR;
  ...
  if ((aid = H5Aopen_by_idx(oid, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)4,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static unsigned verify_dset_cork(hbool_t swmr, hbool_t new_format) {
  ...
  if ((sid = H5Screate_simple(2, dims, max_dims)) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(fid, DSET_EA, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((sid2 = H5Screate_simple(2, dims, NULL)) < 0)
    TEST_ERROR;
  if ((did2 = H5Dcreate2(fid, DSET_FA, H5T_NATIVE_INT, sid2, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((sid3 = H5Screate_simple(2, dims, max_dims)) < 0)
    TEST_ERROR;
  if ((did3 = H5Dcreate2(fid, DSET_BT2, H5T_NATIVE_INT, sid3, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did2 = H5Dopen2(fid, DSET_FA, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did2, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf_data) <
      0)
    TEST_ERROR;
  ...
  if ((did3 = H5Dopen2(fid, DSET_BT2, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did3, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf_data) <
      0)
    TEST_ERROR;
...
static unsigned verify_group_cork(hbool_t swmr) {
  ...
  hid_t fid = H5I_INVALID_HID;  /* File ID */
  hid_t fapl = H5I_INVALID_HID; /* File access property list */
  hid_t gid = H5I_INVALID_HID, gid2 = H5I_INVALID_HID,
        gid3 = H5I_INVALID_HID;      /* Group IDs */
  H5O_info2_t oinfo, oinfo2, oinfo3; /* Object metadata information */
  hid_t aid = H5I_INVALID_HID;       /* Attribute ID */
  ...
  if ((gid2 = H5Gcreate2(gid, GRP2, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((gid3 = H5Gcreate2(gid2, GRP3, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  for (i = 0; i < 8; i++) {
    HDsnprintf(attrname, sizeof(attrname), "attr %d", i);
    if ((aid = H5Acreate2(gid3, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      TEST_ERROR;
...
static unsigned verify_named_cork(hbool_t swmr) {
  ...
  hid_t fid = H5I_INVALID_HID;  /* File ID */
  hid_t fapl = H5I_INVALID_HID; /* File access property list */
  hid_t tid = H5I_INVALID_HID, tid2 = H5I_INVALID_HID,
        tid3 = H5I_INVALID_HID;                        /* Datatype IDs */
  ...
  for (i = 0; i < 8; i++) {
    HDsnprintf(attrname, sizeof(attrname), "attr %d", i);
    if ((aid = H5Acreate2(tid3, attrname, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      TEST_ERROR;
    if (H5Awrite(aid, H5T_NATIVE_UINT, &i) < 0)
      TEST_ERROR;
...
static unsigned verify_multiple_cork(hbool_t swmr) {
  ...
  if ((sid = H5Screate_simple(1, dim, NULL)) < 0)
    TEST_ERROR;
  if ((did1 = H5Dcreate2(fid1, DSET, tid1, sid, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create attribute on root group */
  if ((aidg1 =
           H5Acreate2(gid1, GRP_ATTR, tid1, sid, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create attribute on dataset */
  if ((aidd1 = H5Acreate2(did1, DSET_ATTR, tid1, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create attribute on datatype */
  if ((aidt1 = H5Acreate2(tid1, DT_ATTR, tid1, sid, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
 ...
 *H5Odisable_mdc_flushes/H5Oenable_mdc_flushes/H5Oare_mdc_flushes_disabled
 *public routines are working as specified.
 *
 * Return:      0 on Success, 1 on Failure
 *
 *-------------------------------------------------------------------------
...
static unsigned test_objs_cork(hbool_t swmr, hbool_t new_format) {
  ...
  if (new_format) {
    if (swmr) {
      TESTING("H5Odisable_mdc_flushes/H5Oenable_mdc_flushes/"
              "H5Oare_mdc_flushes_disabled (new library "
              "format) (SWMR)");
    } /* end if */
    else {
      TESTING("H5Odisable_mdc_flushes/H5Oenable_mdc_flushes/"
    ...
    if (swmr) {
      TESTING("H5Odisable_mdc_flushes/H5Oenable_mdc_flushes/"
              "H5Oare_mdc_flushes_disabled (old library "
              "format) (SWMR)");
    } /* end if */
    else {
      TESTING("H5Odisable_mdc_flushes/H5Oenable_mdc_flushes/"
  ...
  if (H5Tcommit2(fid, "group/datatype", tid, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     This function verifies
 *H5Odisable_mdc_flushes/H5Oenable_mdc_flushes/H5Oare_mdc_flushes_disabled are
...
static unsigned test_dset_cork(hbool_t swmr, hbool_t new_format) {
  ...
  if (new_format) {
    if (swmr) {
      TESTING("H5Odisable_mdc_flushes/H5Oenable_mdc_flushes/"
              "H5Oare_mdc_flushes_disabled on datasets "
  ...
  if ((tid1 = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(gid, "datatype", tid1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  ...
  if ((did1 = H5Dcreate2(gid, "dataset", tid1, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
