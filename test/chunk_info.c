...
static int verify_idx_nchunks(hid_t dset, hid_t dspace,
                              H5D_chunk_index_t exp_idx_type,
                              hsize_t exp_num_chunks);
static int verify_get_chunk_info(hid_t dset, hid_t dspace, hsize_t chk_index,
                                 hsize_t exp_chk_size,
                                 const hsize_t *exp_offset,
                                 unsigned exp_flt_msk);
static int verify_get_chunk_info_by_coord(hid_t dset, hsize_t *offset,
                                          hsize_t exp_chk_size,
                                          unsigned exp_flt_msk);
...
static int verify_get_chunk_info(hid_t dset, hid_t dspace, hsize_t chk_index,
  ...
  if (H5Dget_chunk_info(dset, dspace, chk_index, out_offset, &read_flt_msk,
                        &addr, &size) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int verify_selected_chunks(hid_t dset, hid_t plist, const hsize_t *start,
                                  const hsize_t *end) {
  ...
  for (ii = start[0]; ii < end[0]; ii++)
    for (jj = start[1]; jj < end[1]; jj++, chk_index++) {
      ...
      if (memcmp(expected_buf[chk_index], read_buf, CHUNK_NX * CHUNK_NY) != 0) {
        fprintf(stderr,
                "Read chunk differs from written chunk at offset (%" PRIuHSIZE
                ",%" PRIuHSIZE ")\n",
                offset[0], offset[1]);
        return FAIL;
      }
 ...
 *-------------------------------------------------------------------------
 */
static int write_selected_chunks(hid_t dset, hid_t plist, const hsize_t *start,
                                 const hsize_t *end, unsigned flt_msk) {
  ...
  for (ii = start[0]; ii < end[0]; ii++)
    for (jj = start[1]; jj < end[1]; jj++, chk_index++) {
      offset[0] = ii * CHUNK_NX;
      offset[1] = jj * CHUNK_NY;
      if (H5Dwrite_chunk(dset, plist, flt_msk, offset, CHK_SIZE,
                         (void *)direct_buf[chk_index]) < 0)
        TEST_ERROR;
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_get_chunk_info_highest_v18(hid_t fapl) {
  ...
  dset = H5Dcreate2(chunkfile, SIMPLE_CHUNKED_DSET_NAME, H5T_NATIVE_INT, dspace,
  ...
  for (ii = START_CHK_X; ii < END_CHK_X; ii++)
    for (jj = START_CHK_Y; jj < END_CHK_Y; jj++, n++) {
      offset[0] = ii * CHUNK_NX;
      offset[1] = jj * CHUNK_NY;
      ret = H5Dwrite_chunk(dset, H5P_DEFAULT, flt_msk, offset, chunk_size,
                           (void *)inbuf);
      if (ret < 0)
        TEST_ERROR;
    }
  ...
  offset[0] = 6;
  offset[1] = 12;
  if (verify_get_chunk_info(dset, H5S_ALL, NUM_CHUNKS_WRITTEN - 1, chunk_size,
                            offset, flt_msk) == FAIL)
  ...
  H5E_BEGIN_TRY {
    ret = H5Dget_chunk_info(dset, H5S_ALL, chk_index, out_offset, &read_flt_msk,
                            &addr, &size);
  }
  H5E_END_TRY
  if (ret != FAIL)
  ...
  if (verify_empty_chunk_info(dset, offset) == FAIL)
    FAIL_PUTS_ERROR(
        "Verification of H5Dget_chunk_info_by_coord on empty chunk failed\n");
  ...
  for (ii = START_CHK_X; ii < END_CHK_X; ii++)
    for (jj = START_CHK_Y; jj < END_CHK_Y; jj++, chk_index++) {
      ...
      if (verify_get_chunk_info(dset, dspace, chk_index, chunk_size, offset,
                                flt_msk) == FAIL)
        FAIL_PUTS_ERROR("Verification of H5Dget_chunk_info failed\n");

      /* Use the same offset to pass into the next ...by_coord function */
      if (verify_get_chunk_info_by_coord(dset, offset, chunk_size, flt_msk) ==
          FAIL)
  ...
  dset = H5Dcreate2(chunkfile, EMPTY_DSET_NAME, H5T_NATIVE_INT, dspace,
                    H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  if (H5Dclose(dset) < 0)
  ...
  chk_index = OUTOFRANGE_CHK_INDEX;
  H5E_BEGIN_TRY {
    ret = H5Dget_chunk_info(dset, dspace, chk_index, out_offset, &read_flt_msk,
                            &addr, &size);
  }
  H5E_END_TRY
  if (ret != FAIL)
  ...
  /* Create an empty dataset and close it */
  dset = H5Dcreate2(chunkfile, EMPTY_EARLY_ALLOC_DSET_NAME, H5T_NATIVE_INT,
                    dspace, H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  if (H5Dclose(dset) < 0)
    TEST_ERROR;
  ...
  /* Reopen the empty dataset to verify the chunk query functions on it */
  if ((dset = H5Dopen2(chunkfile, EMPTY_EARLY_ALLOC_DSET_NAME, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_chunk_info_single_chunk(const char *filename, hid_t fapl) {
  hid_t chunkfile = H5I_INVALID_HID; /* File ID */
  hid_t dspace = H5I_INVALID_HID;    /* Dataspace ID */
  hid_t dset = H5I_INVALID_HID;      /* Dataset ID */
  hid_t cparms = H5I_INVALID_HID;    /* Creation plist */
  ...
  /* Create a new dataset using cparms creation properties */
  dset = H5Dcreate2(chunkfile, SINGLE_CHUNK_DSET_NAME, H5T_NATIVE_INT, dspace,
                    H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  ...
  /* Write the chunk */
  if (H5Dwrite(dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data_buf) <
      0)
    TEST_ERROR;
  ...
  if (verify_get_chunk_info(dset, H5S_ALL, 0, SINGLE_CHK_SIZE, offset,
                            flt_msk) == FAIL)
    FAIL_PUTS_ERROR("Verification H5Dget_chunk_info failed\n");

  /* Get and verify info of the chunk at logical coordinates (0,0) */
  if (verify_get_chunk_info_by_coord(dset, offset, SINGLE_CHK_SIZE, flt_msk) ==
      FAIL)
    FAIL_PUTS_ERROR("Verification of H5Dget_chunk_info_by_coord failed\n");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_chunk_info_implicit(char *filename, hid_t fapl) {
  hid_t chunkfile = H5I_INVALID_HID;             /* File ID */
  hid_t dspace = H5I_INVALID_HID;                /* Dataspace ID */
  hid_t dset = H5I_INVALID_HID;                  /* Dataset ID */
  hid_t cparms = H5I_INVALID_HID;                /* Creation plist */
  ...
  /* Create a new dataset using cparms creation properties */
  dset = H5Dcreate2(chunkfile, IMPLICIT_INDEX_DSET_NAME, H5T_NATIVE_INT, dspace,
                    H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  ...
  for (ii = 0; ii < NX / CHUNK_NX; ii++)
    for (jj = 0; jj < NY / CHUNK_NY; jj++, chk_index++) {
      ...
      if (verify_get_chunk_info(dset, H5S_ALL, chk_index, CHK_SIZE, offset,
                                flt_msk) == FAIL)
        FAIL_PUTS_ERROR("Verification of H5Dget_chunk_info failed\n");
      ...
      if (verify_get_chunk_info_by_coord(dset, offset, CHK_SIZE, flt_msk) ==
          FAIL)
        FAIL_PUTS_ERROR("Verification of H5Dget_chunk_info_by_coord failed\n");
    }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_chunk_info_fixed_array(const char *filename, hid_t fapl) {
  hid_t chunkfile = H5I_INVALID_HID;            /* File ID */
  hid_t dspace = H5I_INVALID_HID;               /* Dataspace ID */
  hid_t dset = H5I_INVALID_HID;                 /* Dataset ID */
  hid_t cparms = H5I_INVALID_HID;               /* Creation plist */
  ...
  /* Create a new dataset using cparms creation properties */
  dset = H5Dcreate2(chunkfile, FIXED_ARR_INDEX_DSET_NAME, H5T_NATIVE_INT,
                    dspace, H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  ...
  /* Verify chunk indexing scheme and number of chunks */
  if (verify_idx_nchunks(dset, dspace, H5D_CHUNK_IDX_FARRAY,
                         NO_CHUNK_WRITTEN) == FAIL)
    FAIL_PUTS_ERROR("Verification and write failed\n");
  ...
  for (ii = START_CHK_X; ii < END_CHK_X; ii++)
    for (jj = START_CHK_Y; jj < END_CHK_Y; jj++, chk_index++) {
      offset[0] = ii * CHUNK_NX;
      offset[1] = jj * CHUNK_NY;
      if (verify_get_chunk_info(dset, dspace, chk_index, CHK_SIZE, offset,
                                flt_msk) == FAIL)
        FAIL_PUTS_ERROR("Verification of H5Dget_chunk_info failed\n");
    }

  /* Attempt to get info using an out-of-range index, chk_index is now >
   * NUM_CHUNKS_WRITTEN.  should fail */
  H5E_BEGIN_TRY {
    ret = H5Dget_chunk_info(dset, dspace, chk_index, out_offset, &read_flt_msk,
                            &addr, &size);
  }
  ...
  if (ret != FAIL)
    FAIL_PUTS_ERROR(
        "    Attempted to get info of a chunk using an out-of-range index.");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_chunk_info_extensible_array(const char *filename,
                                               hid_t fapl) {
  hid_t chunkfile = H5I_INVALID_HID;            /* File ID */
  hid_t dspace = H5I_INVALID_HID;               /* Dataspace ID */
  hid_t dset = H5I_INVALID_HID;                 /* Dataset ID */
  ...
  /* Create a new dataset using cparms creation properties */
  dset = H5Dcreate2(chunkfile, EXT_ARR_INDEX_DSET_NAME, H5T_NATIVE_INT, dspace,
                    H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  ...
  /* Verify chunk indexing scheme and number of chunks */
  if (verify_idx_nchunks(dset, dspace, H5D_CHUNK_IDX_EARRAY,
                         NO_CHUNK_WRITTEN) == FAIL)
    FAIL_PUTS_ERROR("Verification and write failed\n");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_chunk_info_version2_btrees(const char *filename,
                                              hid_t fapl) {
  hid_t chunkfile = H5I_INVALID_HID;                   /* File ID */
  hid_t dspace = H5I_INVALID_HID;                      /* Dataspace ID */
  hid_t dset = H5I_INVALID_HID;                        /* Dataset ID */
  ...
  /* Create a new dataset using cparms creation properties */
  dset = H5Dcreate2(chunkfile, V2_BTREE_INDEX_DSET_NAME, H5T_NATIVE_INT, dspace,
                    H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  ...
  /* Verify chunk indexing scheme and number of chunks */
  if (verify_idx_nchunks(dset, dspace, H5D_CHUNK_IDX_BT2, NO_CHUNK_WRITTEN) ==
      FAIL)
    FAIL_PUTS_ERROR("Verification and write failed\n");
...
static int iter_cb(const hsize_t *offset, unsigned filter_mask, haddr_t addr,
...
static int iter_cb_stop(const hsize_t H5_ATTR_UNUSED *offset,
                        unsigned H5_ATTR_UNUSED filter_mask,
...
static int iter_cb_fail(const hsize_t H5_ATTR_UNUSED *offset,
                        unsigned H5_ATTR_UNUSED filter_mask,
                        haddr_t H5_ATTR_UNUSED addr,
                        hsize_t H5_ATTR_UNUSED size, void *op_data) {
  chunk_iter_info_t **chunk_info = (chunk_iter_info_t **)op_data;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_basic_query(hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];             /* File name */
  ...
  /* Create a new dataset using cparms creation properties */
  dset = H5Dcreate2(basicfile, SIMPLE_CHUNKED_DSET_NAME, H5T_NATIVE_INT, dspace,
                    H5P_DEFAULT, cparms, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  ...
  offset[0] = CHUNK_NX;
  offset[1] = CHUNK_NY;
  if (H5Dwrite_chunk(dset, H5P_DEFAULT, flt_msk, offset, CHK_SIZE, direct_buf) <
      0)
    TEST_ERROR;
  ...
  /* Get and verify info of the first and only chunk */
  if (verify_get_chunk_info(dset, H5S_ALL, 0, CHK_SIZE, offset, flt_msk) ==
      FAIL)
    FAIL_PUTS_ERROR("Verification H5Dget_chunk_info failed\n");
  ...
  offset[0] = CHUNK_NX;
  offset[1] = CHUNK_NY;
  if (verify_get_chunk_info(dset, H5S_ALL, 1, CHK_SIZE, offset, flt_msk) ==
      FAIL)
    FAIL_PUTS_ERROR("Verification H5Dget_chunk_info failed\n");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_failed_attempts(const char *filename, hid_t fapl) {
  hid_t chunkfile = H5I_INVALID_HID; /* File ID */
  hid_t dspace = H5I_INVALID_HID;    /* Dataspace ID */
  hid_t dset = H5I_INVALID_HID;      /* Dataset ID */
  hsize_t dims[2] = {NX, NY};        /* Dataset dimensions */
  ...
  dset = H5Dcreate2(chunkfile, CONTIGUOUS_DSET_NAME, H5T_NATIVE_INT, dspace,
  ...
  if (ret != FAIL)
    FAIL_PUTS_ERROR(
        "    Attempt a chunk query function on a contiguous dataset.");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_get_chunk_info_v110(hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];  /* File name */
  hid_t chunkfile = H5I_INVALID_HID; /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_flt_msk_with_skip_compress(hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];    /* File name */
  ...
  /* Create a new dataset using cparms creation properties. */
  if ((dset = H5Dcreate2(filter_file, SKIP_FILTER_DSET_NAME, H5T_NATIVE_INT,
                         dspace, H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  block[0] = CHUNK_NX;
  block[1] = CHUNK_NY;
  if (H5Sselect_hyperslab(dspace, H5S_SELECT_SET, start, stride, count, block) <
      0)
    TEST_ERROR;

  /* Read the chunk back */
  if (H5Dread(dset, H5T_NATIVE_INT, mem_space, dspace, H5P_DEFAULT,
              check_chunk) < 0)
    TEST_ERROR;
  ...
  if (H5Dread_chunk(dset, H5P_DEFAULT, offset, &read_flt_msk, read_direct_buf) <
      0)
    TEST_ERROR;
