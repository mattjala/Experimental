...
static int test_one_dataset_io(void) {
  ...
  if ((file_id = H5Fcreate_async(ASYNC_API_TEST_FILE, H5F_ACC_TRUNC,
  ...
  BEGIN_MULTIPART {
    ...
    PART_BEGIN(single_dset_fclose) {
      ...
      if ((file_id = H5Fopen_async(ASYNC_API_TEST_FILE, H5F_ACC_RDONLY,
                                   H5P_DEFAULT, es_id)) < 0)
        PART_TEST_ERROR_API(single_dset_fclose);
...
static int test_multi_dataset_io(void) {
  hid_t file_id = H5I_INVALID_HID;
  hid_t dset_id[5] = {H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID,
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(multi_dset_open) {
      ...
      for (i = 0; i < 5; i++) {
        /* Set dataset name */
        if ((chars_written = snprintf(dset_name, ASYNC_API_TEST_FILENAME_SIZE,
                                      "dset%d", i)) < 0 ||
        ...
        /* Write the dataset asynchronously */
        if (H5Dwrite_async(dset_id[i], H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                           H5P_DEFAULT, wbuf[i], es_id) < 0)
          PART_TEST_ERROR_API(multi_dset_open);
      } /* end for */
      ...
      for (i = 0; i < 5; i++) {
        /* Read the dataset asynchronously */
        if (H5Dread_async(dset_id[i], H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                          H5P_DEFAULT, rbuf[i], es_id) < 0)
          PART_TEST_ERROR_API(multi_dset_open);
      } /* end for */
    ...
    PART_BEGIN(multi_dset_close) {
      ...
      for (i = 0; i < 5; i++) {
        ...
        /* Write the dataset asynchronously */
        if (H5Dwrite_async(dset_id[0], H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                           H5P_DEFAULT, wbuf[i], es_id) < 0)
          PART_TEST_ERROR_API(multi_dset_close);
      ...
      for (i = 0; i < 5; i++) {
        ...
        /* Read the dataset asynchronously */
        if (H5Dread_async(dset_id[0], H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                          H5P_DEFAULT, rbuf[i], es_id) < 0)
          PART_TEST_ERROR_API(multi_dset_close);
 ...
 */
static int test_multi_file_dataset_io(void) {
  hid_t file_id[5] = {H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID,
                      H5I_INVALID_HID, H5I_INVALID_HID};
  hid_t dset_id[5] = {H5I_INVALID_HID, H5I_INVALID_HID, H5I_INVALID_HID,
                      H5I_INVALID_HID, H5I_INVALID_HID};
  hid_t space_id = H5I_INVALID_HID;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(multi_file_dset_open) {
      ...
      for (i = 0; i < 5; i++) {
        /* Set file name */
        if ((chars_written = snprintf(file_name, ASYNC_API_TEST_FILENAME_SIZE,
                                      ASYNC_API_TEST_FILE_PRINTF, i)) < 0 ||
            chars_written >= ASYNC_API_TEST_FILENAME_SIZE)
    ...
    PART_BEGIN(multi_file_dset_fclose) {
      ...
      for (i = 0; i < 5; i++) {
        ...
        if ((file_id[0] = H5Fopen_async(file_name, H5F_ACC_RDWR, H5P_DEFAULT,
                                        es_id)) < 0)
          PART_TEST_ERROR_API(multi_file_dset_fclose);
      ...
      for (i = 0; i < 5; i++) {
        ...
        /* Open the file asynchronously */
        if ((file_id[0] = H5Fopen_async(file_name, H5F_ACC_RDONLY, H5P_DEFAULT,
                                        es_id)) < 0)
          PART_TEST_ERROR_API(multi_file_dset_fclose);
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(multi_file_grp_dset_no_kick) {
      ...
      for (i = 0; i < 5; i++) {
        ...
        /* Write the dataset asynchronously */
        if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                           H5P_DEFAULT, wbuf[i], es_id) < 0)
          PART_TEST_ERROR_API(multi_file_grp_dset_no_kick);
      ...
      for (i = 0; i < 5; i++) {
        ...
        /* Open the file asynchronously */
        if ((file_id = H5Fopen_async(file_name, H5F_ACC_RDONLY, H5P_DEFAULT,
                                     es_id)) < 0)
          PART_TEST_ERROR_API(multi_file_grp_dset_no_kick);
        ...
        /* Read the dataset asynchronously */
        if (H5Dread_async(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                          H5P_DEFAULT, rbuf[i], es_id) < 0)
          PART_TEST_ERROR_API(multi_file_grp_dset_no_kick);
...
static int test_set_extent(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    SKIPPED_API();
    printf("    API functions for basic file, dataset, dataset more, or flush "
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (i) {
      ...
      start[0] = (hsize_t)i;
      if (H5Sselect_hyperslab(fspace_id[i], H5S_SELECT_SET, start, NULL, count,
                              NULL) < 0)
        TEST_ERROR;
    } /* end if */
    ...
    if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, mspace_id, fspace_id[i],
                       H5P_DEFAULT, wbuf[i], es_id) < 0)
      TEST_ERROR;
  } /* end for */
...
static int test_attribute_exists(void) {
  ...
  if ((file_id = H5Fopen_async(ASYNC_API_TEST_FILE, H5F_ACC_RDWR, H5P_DEFAULT,
                               es_id)) < 0)
    TEST_ERROR;
...
static int test_group(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_FLUSH_REFRESH)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, or group more aren't "
           "supported "
           "with this connector\n");
    return 0;
  }
