...
static int test_search_func(void H5_ATTR_UNUSED *ptr1, hid_t H5_ATTR_UNUSED id,
                            void H5_ATTR_UNUSED *ptr2) {
  return 0;
}
...
static int test_is_valid(void) {
  ...
  if (tri_ret != TRUE)
    goto out;
#if 0 /* Cannot call internal APIs and cannot call public H5Inmembers on       \
         library types */
 ...
 */
/* IDs in sequential order until H5I_MAX_NUM_TYPES IDs have been given out, at
 * which */
/* point it will search for type IDs that were allocated but have since been */
/* deleted. */
/* This test will allocate IDs up to H5I_MAX_NUM_TYPES, ensure that IDs wrap
 * around */
...
#define RCT_NITER                                                              \
  50 /* Number of times we cycle through object creation and deletion */
...
static int test_remove_clear_type(void) {
  ...
  rct_obj_t *objects =
      NULL; /* Convenience pointer to objects stored in master list */
  size_t list_size;
  long i, j;
  herr_t ret; /* return value */
  ...
  for (i = 0; i < RCT_NITER; i++) {
    ...
    obj_list.count = obj_list.remaining =
        RCT_MIN_NOBJS +
        (HDrandom() % (long)(RCT_MAX_NOBJS - RCT_MIN_NOBJS + 1));
    ...
    VERIFY(
        nmembers, found,
        "The number of members remaining in the type did not match our count");
    ...
    if (ret == FAIL)
      goto error;
    VERIFY(nmembers, 0,
           "The number of members remaining in the type was not zero");
    if (nmembers != 0)
      goto error;
  }
...
static int test_future_ids(void) {
  ...
  H5E_BEGIN_TRY {
    future_id =
        H5Iregister_future(obj_type, &fake_future_obj, realize_future_cb, NULL);
  }
  H5E_END_TRY
  VERIFY(future_id, H5I_INVALID_HID, "H5Iregister_future");
  if (H5I_INVALID_HID != future_id)
  ...
  H5E_BEGIN_TRY {
    future_id =
        H5Iregister_future(obj_type, &fake_future_obj, NULL, discard_future_cb);
  }
  H5E_END_TRY
  VERIFY(future_id, H5I_INVALID_HID, "H5Iregister_future");
  if (H5I_INVALID_HID != future_id)
  ...
  H5E_BEGIN_TRY {
    future_id = H5Iregister_future(H5I_BADID, &fake_future_obj,
                                   realize_future_cb, discard_future_cb);
  }
  H5E_END_TRY
  VERIFY(future_id, H5I_INVALID_HID, "H5Iregister_future");
  if (H5I_INVALID_HID != future_id)
  ...
  future_id = H5Iregister_future(obj_type, future_obj, realize_future_cb,
                                 discard_future_cb);
   ...
   */
  future_id = H5Iregister_future(obj_type, NULL, realize_future_generate_cb,
                                 discard_future_generate_cb);
  CHECK(future_id, H5I_INVALID_HID, "H5Iregister_future");
  if (H5I_INVALID_HID == future_id)
    goto error;
  ...
  future_id = H5Iregister_future(H5I_DATASPACE, future_obj, realize_future_cb,
                                 discard_future_cb);
  CHECK(future_id, H5I_INVALID_HID, "H5Iregister_future");
  if (H5I_INVALID_HID == future_id)
    goto error;
  ...
  future_id = H5Iregister_future(H5I_DATATYPE, future_obj, realize_future_cb,
                                 discard_future_cb);
  CHECK(future_id, H5I_INVALID_HID, "H5Iregister_future");
  if (H5I_INVALID_HID == future_id)
    goto error;
  ...
  future_id = H5Iregister_future(H5I_GENPROP_LST, future_obj, realize_future_cb,
                                 discard_future_cb);
  CHECK(future_id, H5I_INVALID_HID, "H5Iregister_future");
  if (H5I_INVALID_HID == future_id)
    goto error;
