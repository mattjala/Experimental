...
#define ATTR1_RANK 1
#define ATTR1_DIM1 3
int attr_data1[ATTR1_DIM1] = {512, -234,
...
#define ATTR2_DIM1 2
#define ATTR2_DIM2 2
int attr_data2[ATTR2_DIM1][ATTR2_DIM2] = {
...
static void test_attr_basic_write(hid_t fapl) {
  ...
  if (vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC) {
    ...
    H5E_BEGIN_TRY {
      ret_id = H5Acreate2(dataset, ATTR1_NAME, H5T_NATIVE_INT, sid2,
...
static void test_attr_flush(hid_t fapl) {
  ...
  if ((vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC) &&
      (vol_cap_flags_g & H5VL_CAP_FLAG_FILL_VALUES) &&
    ...
    printf("** SKIPPED attribute pre-read due to fill values not being "
           "supported **\n");
  }
...
static void test_attr_mult_write(hid_t fapl) {
  ...
  if (vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC) {
    /* Try to create the same attribute again (should fail) */
    H5E_BEGIN_TRY {
      ret_id = H5Acreate2(dataset, ATTR2_NAME, H5T_NATIVE_INT, sid2,
                          H5P_DEFAULT, H5P_DEFAULT);
    }
    H5E_END_TRY
    VERIFY(ret_id, FAIL, "H5Acreate2");
...
static int attr_info_by_idx_check(hid_t obj_id, const char *attrname, hsize_t n,
  ...
  if (use_index) {
    ...
    memset(&ainfo, 0, sizeof(ainfo));
    ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER,
                             H5_ITER_INC /* H5_ITER_NATIVE */, (hsize_t)0,
                             &ainfo, H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Aget_info_by_idx");
    VERIFY(ainfo.corder, 0, "H5Aget_info_by_idx");
    ...
    memset(&ainfo, 0, sizeof(ainfo));
    ret = H5Aget_info_by_idx(obj_id, ".", H5_INDEX_CRT_ORDER,
                             H5_ITER_INC /* H5_ITER_NATIVE */, n, &ainfo,
                             H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Aget_info_by_idx");
    VERIFY(ainfo.corder, n, "H5Aget_info_by_idx");
    ...
    ret = (herr_t)H5Aget_name_by_idx(
        obj_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC /* H5_ITER_NATIVE */, n,
...
static void test_attr_info_by_idx(hbool_t new_format, hid_t fcpl, hid_t fapl) {
  ...
  for (use_index = FALSE; use_index <= TRUE; use_index++) {
    ...
    if (use_index)
      MESSAGE(
          5,
          ("Testing Querying Attribute Info By Index w/Creation Order Index\n"))
    else
...
static void test_attr_open_by_name(hbool_t new_format, hid_t fcpl, hid_t fapl) {
  ...
  for (use_index = FALSE; use_index <= TRUE; use_index++) {
    ...
    if (use_index)
      ...
      MESSAGE(5,
              ("Testing Opening Attributes By Name w/o Creation Order Index\n"))
...
static void test_attr_create_by_name(hbool_t new_format, hid_t fcpl,
  ...
  for (use_index = FALSE; use_index <= TRUE; use_index++) {
    ...
    if (use_index)
      ...
      MESSAGE(
          5, ("Testing Creating Attributes By Name w/o Creation Order Index\n"))
...
**
****************************************************************/
#if 0 /* Native VOL connector only supports large attributes with latest       \
         format */
static void
