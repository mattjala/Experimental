...
static void test_iter_group(hid_t fapl, hbool_t new_format) {
  ...
  if ((vol_cap_flags_g & H5VL_CAP_FLAG_ITERATE) &&
    ...
    for (i = 0; i < (int)ginfo.nlinks; i++) {
      ...
      ret =
          H5Oget_info_by_idx3(root_group, ".", H5_INDEX_NAME, H5_ITER_INC,
    ...
    for (i = 0; i < (int)ginfo.nlinks; i++) {
      ...
      ret = (herr_t)H5Lget_name_by_idx(file, ".", H5_INDEX_NAME, H5_ITER_INC,
                                       (hsize_t)i, dataset_name,
                                       (size_t)NAMELEN, H5P_DEFAULT);
      CHECK(ret, FAIL, "H5Lget_name_by_idx");

      ret =
          H5Oget_info_by_idx3(file, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)i,
    ...
    H5E_BEGIN_TRY {
      ret =
          H5Literate2(file, H5_INDEX_NAME, H5_ITER_INC, &idx, liter_cb, &info);
    }
    H5E_END_TRY
    ...
    while ((ret = H5Literate2(file, H5_INDEX_NAME, H5_ITER_INC, &idx, liter_cb,
      ...
      if (HDstrncmp(info.name, lnames[(size_t)(idx - 1)], NAMELEN) != 0)
        TestErrPrintf("Group iteration function didn't return name correctly "
                      "for link - lnames[%u] = '%s'!\n",
...
static void test_iter_attr(hid_t fapl, hbool_t new_format) {
  ...
  if ((vol_cap_flags_g & H5VL_CAP_FLAG_ITERATE) &&
    ...
    dataset = H5Dcreate2(file, "Dataset", H5T_NATIVE_INT, filespace,
    ...
    idx = NATTR;
    H5E_BEGIN_TRY {
      ret = H5Aiterate2(dataset, H5_INDEX_NAME, H5_ITER_INC, &idx, aiter_cb,
                        &info);
    }
    H5E_END_TRY
    ...
    i = 0;
    idx = 0;
    while ((ret = H5Aiterate2(dataset, H5_INDEX_NAME, H5_ITER_INC, &idx,
                              aiter_cb, &info)) > 0) {
      /* Verify return value from iterator gets propagated correctly */
...
static void test_iter_group_large(hid_t fapl) {
  ...
  if ((vol_cap_flags_g & H5VL_CAP_FLAG_ITERATE) &&
    ...
    ret = H5Tcommit2(file, "Datatype1", tid, H5P_DEFAULT, H5P_DEFAULT,
                     H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Tcommit2");
    ...
    curr_name = &names[0];
    ret = H5Literate2(file, H5_INDEX_NAME, H5_ITER_INC, NULL, liter_cb2,
                      curr_name);
