...
static int test_dataset_io_point_selections(void) {
  ...
  for (do_chunk = FALSE;; do_chunk = TRUE) {
    if (do_chunk) {
      TESTING("point selection I/O with all selection in memory and points in "
              "file with chunking");
    ...
    for (i = 0; i < DATASET_IO_POINT_DIM_0; i++)
      for (j = 0; j < DATASET_IO_POINT_DIM_1; j++)
        if (buf_all[i][j] != file_state[i][j])
          FAIL_PUTS_ERROR("Incorrect data found after writing from all memory "
                          "buffer to points");
    ...
    for (i = 0; i < DATASET_IO_POINT_DIM_0; i++)
      for (j = 0; j < DATASET_IO_POINT_DIM_1; j++)
        if (buf_all[i][j] != erbuf[i][j])
          FAIL_PUTS_ERROR("Incorrect data found read from points in file to "
                          "points in memory");
    ...
    for (i = 0; i < DATASET_IO_POINT_DIM_0; i++)
      for (j = 0; j < DATASET_IO_POINT_DIM_1; j++)
        if (buf_all[i][j] != file_state[i][j])
          FAIL_PUTS_ERROR("Incorrect data found after writing from points in "
                          "memory to points in dataset");

    PASSED();

    if (do_chunk)
      TESTING("point selection I/O with points in memory and file (different "
              "shape) with chunking");
    else
      TESTING("point selection I/O with points in memory and file (different "
              "shape)");
    ...
    for (i = 0; i < DATASET_IO_POINT_DIM_0; i++)
      for (j = 0; j < DATASET_IO_POINT_DIM_1; j++)
        if (buf_all[i][j] != erbuf[i][j])
          FAIL_PUTS_ERROR("Incorrect data found after reading from points in "
                          "file to points in memory");
    ...
    for (i = 0; i < DATASET_IO_POINT_DIM_0; i++)
      for (j = 0; j < DATASET_IO_POINT_DIM_1; j++)
        if (buf_all[i][j] != file_state[i][j])
          FAIL_PUTS_ERROR("Incorrect data found after writing from hyperslab "
    ...
    for (i = 0; i < DATASET_IO_POINT_DIM_0; i++)
      for (j = 0; j < DATASET_IO_POINT_DIM_1; j++)
        if (buf_all[i][j] != erbuf[i][j])
          FAIL_PUTS_ERROR("Incorrect data found after reading from hyperslab "
    ...
    for (i = 0; i < DATASET_IO_POINT_DIM_0; i++)
      for (j = 0; j < DATASET_IO_POINT_DIM_1; j++)
        if (buf_all[i][j] != file_state[i][j])
          FAIL_PUTS_ERROR("Incorrect data found after writing from points in "
                          "memory to hyperslab in dataset");
...
static int test_dataset_compound_partial_io(void) {
  ...
  BEGIN_MULTIPART {
    ...
    PART_BEGIN(write_b_read_full) {
      TESTING_2("H5Dwrite with compound member b then H5Dread with all "
                "compound members");
...
static int test_write_multi_chunk_dataset_diff_shape_read(void) {
  ...
  TESTING("write to dataset with multiple chunks using differently shaped "
          "dataspaces");
...
static int test_overwrite_multi_chunk_dataset_same_shape_read(void) {
  ...
  TESTING("several overwrites to dataset with multiple chunks using same "
          "shaped dataspaces");
...
static int test_overwrite_multi_chunk_dataset_diff_shape_read(void) {
  ...
  TESTING("several overwrites to dataset with multiple chunks using "
