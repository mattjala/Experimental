...
static void test_h5s_zero_dim(void) {
  ...
  for (alloc_time = H5D_ALLOC_TIME_EARLY; alloc_time <= H5D_ALLOC_TIME_INCR;
    ...
    if (vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) {
      /* Select a hyperslab beyond its current dimension sizes, then try to
       * write the data.  It should fail. */
      ret = H5Sselect_hyperslab(sid1, H5S_SELECT_SET, start, NULL, count, NULL);
      CHECK(ret, FAIL, "H5Sselect_hyperslab");

      H5E_BEGIN_TRY {
        ret =
            H5Dwrite(dset1, H5T_NATIVE_INT, H5S_ALL, sid1, H5P_DEFAULT, wdata);
      }
      H5E_END_TRY
      VERIFY(ret, FAIL, "H5Dwrite");
    }
    ...
    if ((vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) &&
      ...
      /* Now extend the first dimension size of the dataset to SPACE1_DIM1*3
       * past the maximal size. It is supposed to fail. */
...
void test_h5s(void) {
  ...
  test_h5s_encode_regular_hyper(low,
                                high); /* Test encoding regular hyperslabs */
  test_h5s_encode_irregular_hyper(
