...
static herr_t object_copy_attribute_iter_callback(hid_t location_id,
                                                  const char *attr_name,
                                                  const H5A_info_t *ainfo,
                                                  void *op_data);
static herr_t object_copy_soft_link_non_expand_callback(hid_t group,
                                                        const char *name,
                                                        const H5L_info2_t *info,
                                                        void *op_data);
static herr_t object_copy_soft_link_expand_callback(hid_t group,
                                                    const char *name,
                                                    const H5L_info2_t *info,
                                                    void *op_data);
static herr_t object_visit_callback(hid_t o_id, const char *name,
                                    const H5O_info2_t *object_info,
                                    void *op_data);
static herr_t object_visit_dset_callback(hid_t o_id, const char *name,
                                         const H5O_info2_t *object_info,
                                         void *op_data);
static herr_t object_visit_dtype_callback(hid_t o_id, const char *name,
                                          const H5O_info2_t *object_info,
                                          void *op_data);
static herr_t object_visit_soft_link_callback(hid_t o_id, const char *name,
                                              const H5O_info2_t *object_info,
                                              void *op_data);
static herr_t object_visit_noop_callback(hid_t o_id, const char *name,
                                         const H5O_info2_t *object_info,
                                         void *op_data);
...
static int test_open_object(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
    ...
    SKIPPED_API();
    printf("    API functions for basic file, group, object, dataset, or "
           "stored datatype aren't "
  ...
  if ((container_group =
           H5Gopen2(file_id, OBJECT_TEST_GROUP_NAME, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  if ((fspace_id = generate_random_dataspace(OBJECT_OPEN_TEST_SPACE_RANK, NULL,
                                             NULL, FALSE)) < 0)
    TEST_ERROR;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oopen_group) {
      ...
      H5E_BEGIN_TRY { H5Gclose(group_id2); }
      H5E_END_TRY

      if ((group_id2 =
               H5Oopen(group_id, OBJECT_OPEN_TEST_GRP_NAME, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Oopen_dset) {
      ...
      if ((dset_id = H5Oopen(group_id, OBJECT_OPEN_TEST_DSET_NAME,
                             H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Oopen_dtype) {
      ...
      if (H5Tcommit2(group_id, OBJECT_OPEN_TEST_TYPE_NAME, type_id, H5P_DEFAULT,
      ...
      H5E_BEGIN_TRY { H5Tclose(type_id); }
      H5E_END_TRY

      if ((type_id = H5Oopen(group_id, OBJECT_OPEN_TEST_TYPE_NAME,
                             H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Oopen_by_idx_group) {
      ...
      if ((group_id2 = H5Oopen_by_idx(
               container_group, OBJECT_OPEN_TEST_GROUP_NAME, H5_INDEX_NAME,
    ...
    PART_BEGIN(H5Oopen_by_idx_dset) {
      ...
      if ((dset_id = H5Oopen_by_idx(container_group,
                                    OBJECT_OPEN_TEST_GROUP_NAME, H5_INDEX_NAME,
    ...
    PART_BEGIN(H5Oopen_by_idx_dtype) {
      ...
      if ((type_id = H5Oopen_by_idx(container_group,
                                    OBJECT_OPEN_TEST_GROUP_NAME, H5_INDEX_NAME,
                                    H5_ITER_INC, 2, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf(
            "    couldn't open committed datatype '%s' with H5Oopen_by_idx\n",
            OBJECT_OPEN_TEST_TYPE_NAME);
...
static int test_open_object_invalid_params(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_OBJECT_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, or object aren't "
           "supported with "
           "this connector\n");
    return 0;
  }
  ...
  if (vol_cap_flags_g & H5VL_CAP_FLAG_CREATION_ORDER) {
    if (H5Pset_link_creation_order(gcpl_id, H5P_CRT_ORDER_TRACKED |
      ...
      H5_FAILED_API();
      printf("    couldn't enable link creation order tracking and indexing on "
             "GCPL\n");
      goto error;
    }
  }
  ...
  if ((group_id2 =
           H5Gcreate2(group_id, OBJECT_OPEN_INVALID_PARAMS_TEST_GRP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oopen_invalid_loc_id) {
      ...
      H5E_BEGIN_TRY {
        group_id2 =
            H5Oopen(H5I_INVALID_HID, OBJECT_OPEN_INVALID_PARAMS_TEST_GRP_NAME,
                    H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Oopen_invalid_obj_name) {
      ...
      H5E_BEGIN_TRY { group_id2 = H5Oopen(group_id, NULL, H5P_DEFAULT); }
      H5E_END_TRY
      ...
      H5E_BEGIN_TRY { group_id2 = H5Oopen(group_id, "", H5P_DEFAULT); }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Oopen_invalid_lapl) {
      ...
      H5E_BEGIN_TRY {
        group_id2 = H5Oopen(group_id, OBJECT_OPEN_INVALID_PARAMS_TEST_GRP_NAME,
                            H5I_INVALID_HID);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Oopen_by_idx_invalid_loc_id) {
      ...
      H5E_BEGIN_TRY {
        group_id2 = H5Oopen_by_idx(H5I_INVALID_HID,
                                   OBJECT_OPEN_INVALID_PARAMS_TEST_GROUP_NAME,
    ...
    PART_BEGIN(H5Oopen_by_idx_invalid_grp_name) {
      ...
      H5E_BEGIN_TRY {
        group_id2 = H5Oopen_by_idx(container_group, NULL, H5_INDEX_NAME,
      ...
      H5E_BEGIN_TRY {
        group_id2 = H5Oopen_by_idx(container_group, "", H5_INDEX_NAME,
                                   H5_ITER_INC, 0, H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      if (group_id2 >= 0) {
        H5_FAILED_API();
        printf(
            "    H5Oopen_by_idx succeeded with an invalid group name of ''!\n");
        H5Gclose(group_id2);
        PART_ERROR_API(H5Oopen_by_idx_invalid_grp_name);
      }
    ...
    PART_BEGIN(H5Oopen_by_idx_invalid_index_type) {
      ...
      H5E_BEGIN_TRY {
        group_id2 = H5Oopen_by_idx(
            container_group, OBJECT_OPEN_INVALID_PARAMS_TEST_GROUP_NAME,
      ...
      if (group_id2 >= 0) {
        H5_FAILED_API();
        printf("    H5Oopen_by_idx succeeded with invalid index type "
               "H5_INDEX_UNKNOWN!\n");
        H5Gclose(group_id2);
      ...
      H5E_BEGIN_TRY {
        group_id2 = H5Oopen_by_idx(container_group,
                                   OBJECT_OPEN_INVALID_PARAMS_TEST_GROUP_NAME,
    ...
    PART_BEGIN(H5Oopen_by_idx_invalid_iter_order) {
      ...
      if (group_id2 >= 0) {
        H5_FAILED_API();
        printf("    H5Oopen_by_idx succeeded with an invalid iteration "
               "ordering H5_ITER_UNKNOWN!\n");
        H5Gclose(group_id2);
    ...
    PART_BEGIN(H5Oopen_by_token_invalid_token) {
      ...
      H5E_BEGIN_TRY { group_id2 = H5Oopen_by_token(file_id, H5O_TOKEN_UNDEF); }
      H5E_END_TRY
...
static int test_object_exists(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
    ...
    SKIPPED_API();
    printf("    API functions for basic file, group, object, dataset, stored "
           "datatype or soft link "
  ...
  if ((fspace_id = generate_random_dataspace(OBJECT_EXISTS_TEST_DSET_SPACE_RANK,
                                             NULL, NULL, FALSE)) < 0)
    TEST_ERROR;
   ...
   *       H5Oexists_by_name for a soft link may fail if the link doesn't
   * resolve.
   */
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oexists_by_name_group) {
      ...
      if ((object_exists = H5Oexists_by_name(
               group_id, OBJECT_EXISTS_TEST_GRP_NAME, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Oexists_by_name_dset) {
      ...
      if ((dset_id = H5Dcreate2(group_id, OBJECT_EXISTS_TEST_DSET_NAME,
                                dset_dtype, fspace_id, H5P_DEFAULT, H5P_DEFAULT,
                                H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create dataset '%s'\n",
      ...
      if ((object_exists = H5Oexists_by_name(
               group_id, OBJECT_EXISTS_TEST_DSET_NAME, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if object '%s' exists\n",
               OBJECT_EXISTS_TEST_DSET_NAME);
        PART_ERROR_API(H5Oexists_by_name_dset);
    ...
    PART_BEGIN(H5Oexists_by_name_dtype) {
      ...
      if ((object_exists = H5Oexists_by_name(
               group_id, OBJECT_EXISTS_TEST_TYPE_NAME, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if object '%s' exists\n",
               OBJECT_EXISTS_TEST_TYPE_NAME);
        PART_ERROR_API(H5Oexists_by_name_dtype);
    ...
    PART_BEGIN(H5Oexists_by_name_soft_link) {
      ...
      if (H5Lcreate_soft("/" OBJECT_TEST_GROUP_NAME
                         "/" OBJECT_EXISTS_TEST_SUBGROUP_NAME,
                         group_id, OBJECT_EXISTS_TEST_SOFT_LINK_NAME,
                         H5P_DEFAULT, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if ((object_exists = H5Oexists_by_name(
               group_id, OBJECT_EXISTS_TEST_SOFT_LINK_NAME, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if object '%s' exists\n",
               OBJECT_EXISTS_TEST_SOFT_LINK_NAME);
        PART_ERROR_API(H5Oexists_by_name_soft_link);
    ...
    PART_BEGIN(H5Oexists_by_name_dangling_soft_link) {
      ...
      if (H5Lcreate_soft("/" OBJECT_TEST_GROUP_NAME
                         "/" OBJECT_EXISTS_TEST_SUBGROUP_NAME
                         "/non_existent_object",
                         group_id, OBJECT_EXISTS_TEST_DANGLING_LINK_NAME,
                         H5P_DEFAULT, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    couldn't create soft link '%s'\n",
      ...
      if ((object_exists = H5Oexists_by_name(
               group_id, OBJECT_EXISTS_TEST_DANGLING_LINK_NAME, H5P_DEFAULT)) <
          0) {
        H5_FAILED_API();
        printf("    couldn't determine if object '%s' exists\n",
      ...
      if (object_exists) {
        H5_FAILED_API();
        printf("    object pointed to by dangling soft link should not have "
               "existed!\n");
        PART_ERROR_API(H5Oexists_by_name_dangling_soft_link);
      }
...
static int test_object_exists_invalid_params(void) {
  ...
  if ((group_id2 =
           H5Gcreate2(group_id, OBJECT_EXISTS_INVALID_PARAMS_TEST_GRP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oexists_by_name_invalid_loc_id) {
      ...
      H5E_BEGIN_TRY {
        object_exists = H5Oexists_by_name(
            H5I_INVALID_HID, OBJECT_EXISTS_INVALID_PARAMS_TEST_GRP_NAME,
            H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      if (object_exists >= 0) {
        H5_FAILED_API();
        printf(
            "    H5Oexists_by_name succeeded with an invalid location ID!\n");
    ...
    PART_BEGIN(H5Oexists_by_name_invalid_lapl) {
      ...
      H5E_BEGIN_TRY {
        object_exists = H5Oexists_by_name(
            group_id, OBJECT_EXISTS_INVALID_PARAMS_TEST_GRP_NAME,
            H5I_INVALID_HID);
      }
      H5E_END_TRY
...
static int test_link_object(void) {
  ...
  if ((fspace_id = generate_random_dataspace(OBJECT_LINK_TEST_SPACE_RANK, NULL,
                                             NULL, FALSE)) < 0)
    TEST_ERROR;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Olink_group) {
      ...
      if ((group_id2 = H5Gcreate_anon(group_id, H5P_DEFAULT, H5P_DEFAULT)) <
          0) {
        H5_FAILED_API();
      ...
      if (H5Olink(group_id2, group_id, OBJECT_LINK_TEST_GROUP_NAME2,
                  H5P_DEFAULT, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Olink_dataset) {
      ...
      if ((dset_id = H5Dcreate_anon(group_id, dset_dtype, fspace_id,
                                    H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
      ...
      if (H5Olink(dset_id, group_id, OBJECT_LINK_TEST_DSET_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Olink_datatype) {
      ...
      if (H5Olink(dset_dtype, group_id, OBJECT_LINK_TEST_DTYPE_NAME,
                  H5P_DEFAULT, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
...
static int test_link_object_invalid_params(void) {
  ...
  if ((group_id = H5Gcreate2(container_group,
                             OBJECT_LINK_INVALID_PARAMS_TEST_GROUP_NAME,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container sub-group '%s'\n",
           OBJECT_LINK_TEST_GROUP_NAME);
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Olink_invalid_object_id) {
      ...
      H5E_BEGIN_TRY {
        status =
            H5Olink(H5I_INVALID_HID, group_id, OBJECT_LINK_TEST_GROUP_NAME2,
    ...
    PART_BEGIN(H5Olink_invalid_location) {
      ...
      H5E_BEGIN_TRY {
        status =
            H5Olink(group_id2, H5I_INVALID_HID, OBJECT_LINK_TEST_GROUP_NAME2,
                    H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Olink_invalid_lcpl) {
      ...
      H5E_BEGIN_TRY {
        status = H5Olink(group_id2, group_id, OBJECT_LINK_TEST_GROUP_NAME2,
...
static int test_incr_decr_object_refcount(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_STORED_DATATYPES)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, dataset, stored datatype, "
           "or object  "
           "aren't supported with this connector\n");
    return 0;
  }
  ...
  if ((group_id =
           H5Gcreate2(container_group, OBJECT_REF_COUNT_TEST_SUBGROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container subgroup '%s'\n",
  ...
  if ((fspace_id = generate_random_dataspace(
           OBJECT_REF_COUNT_TEST_DSET_SPACE_RANK, NULL, NULL, FALSE)) < 0)
    TEST_ERROR;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oincr_decr_refcount_group) {
      ...
      if (H5Oget_info_by_name3(group_id, OBJECT_REF_COUNT_TEST_GRP_NAME, &oinfo,
    ...
    PART_BEGIN(H5Oincr_decr_refcount_dset) {
      ...
      if ((dset_id = H5Dcreate2(group_id, OBJECT_REF_COUNT_TEST_DSET_NAME,
                                dset_dtype, fspace_id, H5P_DEFAULT, H5P_DEFAULT,
                                H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create dataset '%s'\n",
      ...
      /* Verify that reference count is 2 now */
      if (H5Oget_info_by_name3(group_id, OBJECT_REF_COUNT_TEST_DSET_NAME,
                               &oinfo, H5O_INFO_BASIC, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Oincr / decr_refcount_dtype) {
      TESTING_2_API(
          "H5Oincr_refcount/H5Odecr_refcount on a committed datatype");
      ...
      if (H5Oincr_refcount(dset_dtype) < 0) {
        H5_FAILED_API();
        printf(
            "    couldn't increment reference count for the datatype '%s' \n",
            OBJECT_REF_COUNT_TEST_TYPE_NAME);
        PART_ERROR_API(H5Oincr_decr_refcount_dtype);
      }

      /* Verify that reference count is 2 now */
      if (H5Oget_info_by_name3(group_id, OBJECT_REF_COUNT_TEST_TYPE_NAME,
                               &oinfo, H5O_INFO_BASIC, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if (H5Odecr_refcount(dset_dtype) < 0) {
        H5_FAILED_API();
        printf(
            "    couldn't decrement reference count for the datatype '%s' \n",
            OBJECT_REF_COUNT_TEST_TYPE_NAME);
        PART_ERROR_API(H5Oincr_decr_refcount_dtype);
      }
...
static int test_incr_decr_object_refcount_invalid_params(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_OBJECT_MORE)) {
    SKIPPED_API();
    printf("    API functions for more object aren't supported with this "
           "connector\n");
    return 0;
  }
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oincr_refcount_invalid_param) {
      ...
      H5E_BEGIN_TRY { status = H5Oincr_refcount(H5I_INVALID_HID); }
      H5E_END_TRY

      if (status >= 0) {
        H5_FAILED_API();
        printf(
            "    incremented the reference count for an invalid object ID\n");
    ...
    PART_BEGIN(H5Odecr_refcount_invalid_param) {
      ...
      H5E_BEGIN_TRY { status = H5Odecr_refcount(H5I_INVALID_HID); }
      H5E_END_TRY

      if (status >= 0) {
        H5_FAILED_API();
        printf(
            "    decremented the reference count for an invalid object ID\n");
...
static int test_object_copy_basic(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_STORED_DATATYPES)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, object, link, dataset, "
           "attribute, iterate, or "
  ...
  if ((group_id =
           H5Gcreate2(container_group, OBJECT_COPY_BASIC_TEST_SUBGROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container subgroup '%s'\n",
  ...
  if ((space_id = generate_random_dataspace(OBJECT_COPY_BASIC_TEST_SPACE_RANK,
                                            NULL, NULL, FALSE)) < 0)
    TEST_ERROR;
  if ((attr_space_id = generate_random_dataspace(
           OBJECT_COPY_BASIC_TEST_SPACE_RANK, NULL, NULL, TRUE)) < 0)
    TEST_ERROR;

  if ((dset_dtype = generate_random_datatype(H5T_NO_CLASS, FALSE)) < 0)
    TEST_ERROR;

  /* Create the test group object, along with its nested members and the
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_BASIC_TEST_NUM_NESTED_OBJS; i++) {
    ...
    if ((tmp_group_id = H5Gcreate2(group_id2, grp_name, H5P_DEFAULT,
                                   H5P_DEFAULT, H5P_DEFAULT)) < 0) {
      H5_FAILED_API();
    ...
    if (i == (OBJECT_COPY_BASIC_TEST_NUM_NESTED_OBJS - 1)) {
      if (H5Gclose(H5Gcreate2(tmp_group_id,
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_BASIC_TEST_NUM_ATTRS; i++) {
    ...
    if ((tmp_attr_id = H5Acreate2(group_id2, attr_name, H5T_NATIVE_INT,
   ...
   */
  if ((dset_id =
           H5Dcreate2(group_id, OBJECT_COPY_BASIC_TEST_DSET_NAME, dset_dtype,
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_BASIC_TEST_NUM_ATTRS; i++) {
    ...
    if ((tmp_attr_id = H5Acreate2(dset_id, attr_name, H5T_NATIVE_INT,
                                  attr_space_id, H5P_DEFAULT, H5P_DEFAULT)) <
        0) {
      H5_FAILED_API();
  ...
  /* Create the test committed datatype object, along with the attributes
   * attached to it. */
  if ((dtype_id = generate_random_datatype(H5T_NO_CLASS, FALSE)) < 0) {
    H5_FAILED_API();
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_BASIC_TEST_NUM_ATTRS; i++) {
    ...
    if ((tmp_attr_id = H5Acreate2(dtype_id, attr_name, H5T_NATIVE_INT,
                                  attr_space_id, H5P_DEFAULT, H5P_DEFAULT)) <
        0) {
      H5_FAILED_API();
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ocopy_group) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_BASIC_TEST_GROUP_NAME, group_id,
                  OBJECT_COPY_BASIC_TEST_NEW_GROUP_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if ((object_link_exists =
               H5Lexists(group_id, OBJECT_COPY_BASIC_TEST_NEW_GROUP_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
      ...
      /* Ensure that the new group has all the members of the copied group, and
       * all its attributes */
      if ((tmp_group_id =
               H5Gopen2(group_id, OBJECT_COPY_BASIC_TEST_NEW_GROUP_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
      ...
      if (group_info.nlinks != OBJECT_COPY_BASIC_TEST_NUM_NESTED_OBJS) {
        H5_FAILED_API();
        printf("    copied group contained %d members instead of %d members "
               "after a deep copy!\n",
      ...
      if (object_info.num_attrs == 0) {
        H5_FAILED_API();
        printf("    copied group didn't contain any attributes after copy "
               "operation!\n");
        PART_ERROR_API(H5Ocopy_group);
      }
      ...
      if (i != OBJECT_COPY_BASIC_TEST_NUM_ATTRS) {
        H5_FAILED_API();
        printf("    number of attributes on copied group (%llu) didn't match "
               "expected number (%llu)!\n",
       ...
       */
      {
        ...
        if (group_info.nlinks != 1) {
          H5_FAILED_API();
          printf("    copied group's immediate members didn't contain nested "
                 "members after a "
                 "deep copy!\n");
          PART_ERROR_API(H5Ocopy_group);
        }
    ...
    if (tmp_group_id >= 0) {
      H5E_BEGIN_TRY { H5Gclose(tmp_group_id); }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ocopy_dset) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_BASIC_TEST_DSET_NAME, group_id,
                  OBJECT_COPY_BASIC_TEST_NEW_DSET_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if ((object_link_exists =
               H5Lexists(group_id, OBJECT_COPY_BASIC_TEST_NEW_DSET_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
      ...
      /* Ensure that the new dataset has all of the attributes of the copied
       * dataset */
      if ((tmp_dset_id =
               H5Dopen2(group_id, OBJECT_COPY_BASIC_TEST_NEW_DSET_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
      ...
      if (object_info.num_attrs == 0) {
        H5_FAILED_API();
        printf("    copied dataset didn't contain any attributes after copy "
               "operation!\n");
        PART_ERROR_API(H5Ocopy_dset);
      }
      ...
      if (i != OBJECT_COPY_BASIC_TEST_NUM_ATTRS) {
        H5_FAILED_API();
        printf("    number of attributes on copied dataset (%llu) didn't match "
               "expected number "
               "(%llu)!\n",
               (unsigned long long)i,
               (unsigned long long)OBJECT_COPY_BASIC_TEST_NUM_ATTRS);
    ...
    if (tmp_dset_id >= 0) {
      H5E_BEGIN_TRY { H5Dclose(tmp_dset_id); }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ocopy_dtype) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_BASIC_TEST_DTYPE_NAME, group_id,
                  OBJECT_COPY_BASIC_TEST_NEW_DTYPE_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if ((object_link_exists =
               H5Lexists(group_id, OBJECT_COPY_BASIC_TEST_NEW_DTYPE_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf(
            "    couldn't determine if link '%s' to copied datatype exists\n",
            OBJECT_COPY_BASIC_TEST_NEW_DTYPE_NAME);
        PART_ERROR_API(H5Ocopy_dtype);
      }
      ...
      /* Ensure that the new committed datatype has all the attributes of the
       * copied datatype */
      if ((tmp_dtype_id =
               H5Topen2(group_id, OBJECT_COPY_BASIC_TEST_NEW_DTYPE_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
      ...
      if (object_info.num_attrs == 0) {
        H5_FAILED_API();
        printf("    copied committed datatype didn't contain any attributes "
      ...
      if (i != OBJECT_COPY_BASIC_TEST_NUM_ATTRS) {
        H5_FAILED_API();
        printf("    number of attributes on copied datatype (%llu) didn't "
    ...
    if (tmp_dtype_id >= 0) {
      H5E_BEGIN_TRY { H5Tclose(tmp_dtype_id); }
      H5E_END_TRY
...
static int test_object_copy_already_existing(void) {
  ...
  if ((space_id = generate_random_dataspace(
           OBJECT_COPY_ALREADY_EXISTING_TEST_SPACE_RANK, NULL, NULL, FALSE)) <
      0)
    TEST_ERROR;
  if ((dset_dtype = generate_random_datatype(H5T_NO_CLASS, FALSE)) < 0)
    TEST_ERROR;
  ...
  if ((group_id2 =
           H5Gcreate2(group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_GROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  if ((dset_id = H5Dcreate2(
           group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_DSET_NAME, dset_dtype,
           space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create dataset '%s'\n",
  ...
  if (H5Tcommit2(group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_DTYPE_NAME,
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ocopy_already_existing_group) {
      ...
      H5E_BEGIN_TRY {
        err_ret =
            H5Ocopy(group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_GROUP_NAME,
                    group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_GROUP_NAME,
                    H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    group copy succeeded in location where group already "
               "exists!\n");
    ...
    PART_BEGIN(H5Ocopy_already_existing_dset) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ocopy(group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_DSET_NAME,
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    dataset copy succeeded in location where dataset already "
               "exists!\n");
    ...
    PART_BEGIN(H5Ocopy_already_existing_dtype) {
      TESTING_2_API("H5Ocopy committed datatype to location where committed "
                    "datatype already exists");

      H5E_BEGIN_TRY {
        err_ret =
            H5Ocopy(group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_DTYPE_NAME,
                    group_id, OBJECT_COPY_ALREADY_EXISTING_TEST_DTYPE_NAME,
                    H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    committed datatype copy succeeded in location where "
               "committed datatype already "
               "exists!\n");
...
static int test_object_copy_shallow_group_copy(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_LINK_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, object, or link aren't "
           "supported with this "
           "connector\n");
    return 0;
  }
  ...
  if ((group_id =
           H5Gcreate2(container_group, OBJECT_COPY_SHALLOW_TEST_SUBGROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container subgroup '%s'\n",
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_SHALLOW_TEST_NUM_NESTED_OBJS; i++) {
    ...
    if (i == (OBJECT_COPY_SHALLOW_TEST_NUM_NESTED_OBJS - 1)) {
      if (H5Gclose(H5Gcreate2(tmp_group_id,
                              OBJECT_COPY_SHALLOW_TEST_DEEP_NESTED_GROUP_NAME,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create nested group '%s' under group '%s'\n",
  ...
  if (H5Ocopy(group_id, OBJECT_COPY_SHALLOW_TEST_GROUP_NAME, group_id,
              OBJECT_COPY_SHALLOW_TEST_NEW_GROUP_NAME, ocpypl_id,
              H5P_DEFAULT) < 0) {
    H5_FAILED_API();
    printf("    failed to copy group '%s' to '%s'\n",
           OBJECT_COPY_SHALLOW_TEST_GROUP_NAME,
  ...
  if ((object_link_exists =
           H5Lexists(group_id, OBJECT_COPY_SHALLOW_TEST_NEW_GROUP_NAME,
                     H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't determine if link '%s' to copied group exists\n",
           OBJECT_COPY_SHALLOW_TEST_NEW_GROUP_NAME);
  ...
  /*
   * Ensure that the new group has only the immediate members of the copied
   * group.
   */
  if ((tmp_group_id =
           H5Gopen2(group_id, OBJECT_COPY_SHALLOW_TEST_NEW_GROUP_NAME,
                    H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    failed to open group copy '%s'\n",
           OBJECT_COPY_SHALLOW_TEST_NEW_GROUP_NAME);
  ...
  if (group_info.nlinks != OBJECT_COPY_SHALLOW_TEST_NUM_NESTED_OBJS) {
    H5_FAILED_API();
    printf("    copied group contained %d members instead of %d members after "
           "a shallow copy!\n",
   ...
   */
  {
    ...
    if (group_info.nlinks != 0) {
      H5_FAILED_API();
      printf("    copied group's immediate members contained nested members "
...
static int test_object_copy_no_attributes(void) {
  ...
  if ((group_id =
           H5Gcreate2(container_group, OBJECT_COPY_NO_ATTRS_TEST_SUBGROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container subgroup '%s'\n",
  ...
  if ((space_id = generate_random_dataspace(
           OBJECT_COPY_NO_ATTRS_TEST_SPACE_RANK, NULL, NULL, FALSE)) < 0)
    TEST_ERROR;
  if ((attr_space_id = generate_random_dataspace(
           OBJECT_COPY_NO_ATTRS_TEST_SPACE_RANK, NULL, NULL, TRUE)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_NO_ATTRS_TEST_NUM_ATTRS; i++) {
    ...
    if ((attr_id = H5Acreate2(group_id2, attr_name, H5T_NATIVE_INT,
   ...
   */
  if ((dset_id =
           H5Dcreate2(group_id, OBJECT_COPY_NO_ATTRS_TEST_DSET_NAME, dset_dtype,
                      space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create dataset '%s'\n",
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_NO_ATTRS_TEST_NUM_ATTRS; i++) {
    ...
    if ((attr_id = H5Acreate2(dset_id, attr_name, H5T_NATIVE_INT, attr_space_id,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0) {
      H5_FAILED_API();
      printf("    couldn't create attribute '%s' on dataset '%s'\n", attr_name,
             OBJECT_COPY_NO_ATTRS_TEST_DSET_NAME);
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_NO_ATTRS_TEST_NUM_ATTRS; i++) {
    ...
    if ((attr_id = H5Acreate2(dtype_id, attr_name, H5T_NATIVE_INT,
                              attr_space_id, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
      H5_FAILED_API();
      printf("    couldn't create attribute '%s' on committed datatype '%s'\n",
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ocopy_group_no_attributes) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_NO_ATTRS_TEST_GROUP_NAME, group_id,
                  OBJECT_COPY_NO_ATTRS_TEST_NEW_GROUP_NAME, ocpypl_id,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    failed to copy group '%s' to '%s'\n",
               OBJECT_COPY_NO_ATTRS_TEST_GROUP_NAME,
      ...
      if ((object_link_exists =
               H5Lexists(group_id, OBJECT_COPY_NO_ATTRS_TEST_NEW_GROUP_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if link '%s' to copied group exists\n",
               OBJECT_COPY_NO_ATTRS_TEST_NEW_GROUP_NAME);
      ...
      if ((tmp_group_id =
               H5Gopen2(group_id, OBJECT_COPY_NO_ATTRS_TEST_NEW_GROUP_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    failed to open group copy '%s'\n",
               OBJECT_COPY_NO_ATTRS_TEST_NEW_GROUP_NAME);
      ...
      if (object_info.num_attrs != 0) {
        H5_FAILED_API();
        printf("    copied group contained attributes after a non-attribute "
               "copy!\n");
        PART_ERROR_API(H5Ocopy_group_no_attributes);
      }
    ...
    if (ocpypl_id >= 0) {
      H5E_BEGIN_TRY { H5Pclose(ocpypl_id); }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ocopy_dset_no_attributes) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_NO_ATTRS_TEST_DSET_NAME, group_id,
                  OBJECT_COPY_NO_ATTRS_TEST_NEW_DSET_NAME, ocpypl_id,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    failed to copy dataset '%s' to '%s'\n",
               OBJECT_COPY_NO_ATTRS_TEST_DSET_NAME,
      ...
      if ((object_link_exists =
               H5Lexists(group_id, OBJECT_COPY_NO_ATTRS_TEST_NEW_DSET_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if link '%s' to copied dataset exists\n",
               OBJECT_COPY_NO_ATTRS_TEST_NEW_DSET_NAME);
      ...
      if ((tmp_dset_id =
               H5Dopen2(group_id, OBJECT_COPY_NO_ATTRS_TEST_NEW_DSET_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    failed to open dataset copy '%s'\n",
               OBJECT_COPY_NO_ATTRS_TEST_NEW_DSET_NAME);
      ...
      if (object_info.num_attrs != 0) {
        H5_FAILED_API();
        printf("    copied dataset contained attributes after a non-attribute "
               "copy!\n");
        PART_ERROR_API(H5Ocopy_dset_no_attributes);
      }
    ...
    PART_BEGIN(H5Ocopy_dtype_no_attributes) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_NO_ATTRS_TEST_DTYPE_NAME, group_id,
                  OBJECT_COPY_NO_ATTRS_TEST_NEW_DTYPE_NAME, ocpypl_id,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    failed to copy datatype '%s' to '%s'\n",
               OBJECT_COPY_NO_ATTRS_TEST_DTYPE_NAME,
      ...
      if ((object_link_exists =
               H5Lexists(group_id, OBJECT_COPY_NO_ATTRS_TEST_NEW_DTYPE_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf(
            "    couldn't determine if link '%s' to copied datatype exists\n",
      ...
      if ((tmp_dtype_id =
               H5Topen2(group_id, OBJECT_COPY_NO_ATTRS_TEST_NEW_DTYPE_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    failed to open dataset copy '%s'\n",
               OBJECT_COPY_NO_ATTRS_TEST_NEW_DTYPE_NAME);
      ...
      if (object_info.num_attrs != 0) {
        H5_FAILED_API();
        printf("    copied committed datatype contained attributes after a "
               "non-attribute copy!\n");
        PART_ERROR_API(H5Ocopy_dtype_no_attributes);
      }
...
static int test_object_copy_by_soft_link(void) {
  ...
  if ((group_id =
           H5Gcreate2(container_group, OBJECT_COPY_SOFT_LINK_TEST_SUBGROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container subgroup '%s'\n",
  ...
  if ((attr_space_id = generate_random_dataspace(
           OBJECT_COPY_SOFT_LINK_TEST_SPACE_RANK, NULL, NULL, TRUE)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_SOFT_LINK_TEST_NUM_NESTED_OBJS; i++) {
    ...
    if (i == (OBJECT_COPY_SOFT_LINK_TEST_NUM_NESTED_OBJS - 1)) {
      if (H5Gclose(H5Gcreate2(tmp_group_id,
                              OBJECT_COPY_SOFT_LINK_TEST_DEEP_NESTED_GROUP_NAME,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create nested group '%s' under group '%s'\n",
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ocopy_through_soft_link) {
      ...
      if (H5Lcreate_soft("/" OBJECT_TEST_GROUP_NAME
                         "/" OBJECT_COPY_SOFT_LINK_TEST_SUBGROUP_NAME
                         "/" OBJECT_COPY_SOFT_LINK_TEST_GROUP_NAME,
                         group_id, OBJECT_COPY_SOFT_LINK_TEST_SOFT_LINK_NAME,
                         H5P_DEFAULT, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_SOFT_LINK_TEST_SOFT_LINK_NAME, group_id,
                  OBJECT_COPY_SOFT_LINK_TEST_NEW_GROUP_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    failed to copy group '%s' to '%s'\n",
               OBJECT_COPY_SOFT_LINK_TEST_GROUP_NAME,
      ...
      if ((object_link_exists =
               H5Lexists(group_id, OBJECT_COPY_SOFT_LINK_TEST_NEW_GROUP_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if link '%s' to copied group exists\n",
               OBJECT_COPY_SOFT_LINK_TEST_NEW_GROUP_NAME);
      ...
      if (H5Lget_info2(group_id, OBJECT_COPY_SOFT_LINK_TEST_NEW_GROUP_NAME,
      ...
      if (link_info.type != H5L_TYPE_HARD) {
        H5_FAILED_API();
        printf("    after group copy through soft link, group's new link type "
               "wasn't H5L_TYPE_HARD!\n");
        PART_ERROR_API(H5Ocopy_through_soft_link);
      }
       ...
       */
      if ((tmp_group_id =
               H5Gopen2(group_id, OBJECT_COPY_SOFT_LINK_TEST_NEW_GROUP_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    failed to open group copy '%s'\n",
               OBJECT_COPY_SOFT_LINK_TEST_NEW_GROUP_NAME);
    ...
    PART_BEGIN(H5Ocopy_through_dangling_soft_link) {
      ...
      H5E_BEGIN_TRY {
        err_ret =
            H5Ocopy(group_id, OBJECT_COPY_SOFT_LINK_TEST_DANGLING_LINK_NAME,
                    group_id, OBJECT_COPY_SOFT_LINK_TEST_DANGLING_LINK_NAME "2",
                    H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    copied non-existent object through use of a dangling soft "
               "link!\n");
        PART_ERROR_API(H5Ocopy_through_dangling_soft_link);
      }
...
static int test_object_copy_group_with_soft_links(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_SOFT_LINKS)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, object, link, iterate, or "
           "soft link aren't "
           "supported with "
           "this connector\n");
    return 0;
  ...
  /* Create the test group object. */
  if ((group_id2 = H5Gcreate2(group_id,
  ...
  /* Create several groups at the root level and add soft links pointing to them
   * inside the test group object.
   */
  for (i = 0;
       i < (size_t)OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_NUM_NESTED_OBJS;
       i++) {
    ...
    snprintf(grp_name, OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_BUF_SIZE, "grp%d",
             (int)i);
    snprintf(lnk_name, OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_BUF_SIZE,
             "link%d", (int)i);
    snprintf(lnk_target, 2 * OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_BUF_SIZE,
             "/" OBJECT_TEST_GROUP_NAME
             "/" OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_SUBGROUP_NAME "/%s",
             grp_name);

    if ((tmp_group_id = H5Gcreate2(group_id, grp_name, H5P_DEFAULT, H5P_DEFAULT,
                                   H5P_DEFAULT)) < 0) {
      H5_FAILED_API();
      printf("    couldn't create group '%s' under group '%s'\n", grp_name,
             OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_SUBGROUP_NAME);
    ...
    if (H5Lcreate_soft(lnk_target, group_id2, lnk_name, H5P_DEFAULT,
                       H5P_DEFAULT) < 0) {
      H5_FAILED_API();
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ocopy_dont_expand_soft_links) {
      TESTING_2_API(
          "H5Ocopy on group with soft links (soft links not expanded)");

      if (H5Ocopy(group_id, OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_GROUP_NAME,
                  group_id,
      ...
      if (i != OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_NUM_NESTED_OBJS) {
        H5_FAILED_API();
        printf("    number of links in copied group (%llu) didn't match "
               "expected number (%llu)!\n",
               (unsigned long long)i,
               (unsigned long long)
                   OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_NUM_NESTED_OBJS);
    ...
    if (ocpypl_id >= 0) {
      H5E_BEGIN_TRY { H5Gclose(ocpypl_id); }
      H5E_END_TRY
      ocpypl_id = H5I_INVALID_HID;
    }
    if (tmp_group_id >= 0) {
...
static int test_object_copy_between_files(void) {
   ...
   */
  if (prefix_filename(test_path_prefix,
  ...
  if ((file_id2 = H5Fcreate(obj_copy_filename, H5F_ACC_TRUNC, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  if ((space_id = generate_random_dataspace(
           OBJECT_COPY_BETWEEN_FILES_TEST_SPACE_RANK, NULL, NULL, FALSE)) < 0)
    TEST_ERROR;
  if ((attr_space_id = generate_random_dataspace(
           OBJECT_COPY_BETWEEN_FILES_TEST_SPACE_RANK, NULL, NULL, TRUE)) < 0)
    TEST_ERROR;
   ...
   * attributes attached to it. */
  if ((group_id2 =
           H5Gcreate2(group_id, OBJECT_COPY_BETWEEN_FILES_TEST_GROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_BETWEEN_FILES_TEST_NUM_NESTED_OBJS; i++) {
    ...
    snprintf(grp_name, OBJECT_COPY_BETWEEN_FILES_TEST_BUF_SIZE, "grp%d",
             (int)i);
    ...
    if (i == (OBJECT_COPY_BETWEEN_FILES_TEST_NUM_NESTED_OBJS - 1)) {
      if (H5Gclose(
              H5Gcreate2(tmp_group_id,
                         OBJECT_COPY_BETWEEN_FILES_TEST_DEEP_NESTED_GROUP_NAME,
                         H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create nested group '%s' under group '%s'\n",
  ...
  for (i = 0; i < (size_t)OBJECT_COPY_BETWEEN_FILES_TEST_NUM_ATTRS; i++) {
    ...
    snprintf(attr_name, OBJECT_COPY_BETWEEN_FILES_TEST_BUF_SIZE, "attr%d",
             (int)i);
  ...
  /* Create the test dataset object, along with the attributes attached to it.
   */
  if ((dset_id = H5Dcreate2(group_id, OBJECT_COPY_BETWEEN_FILES_TEST_DSET_NAME,
                            dset_dtype, space_id, H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create dataset '%s'\n",
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ocopy_group_between_files) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_BETWEEN_FILES_TEST_GROUP_NAME, file_id2,
                  OBJECT_COPY_BETWEEN_FILES_TEST_NEW_GROUP_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if ((object_link_exists = H5Lexists(
               file_id2, OBJECT_COPY_BETWEEN_FILES_TEST_NEW_GROUP_NAME,
               H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if link '%s' to copied group exists\n",
               OBJECT_COPY_BETWEEN_FILES_TEST_NEW_GROUP_NAME);
      ...
      if (!object_link_exists) {
        H5_FAILED_API();
        printf(
            "    link '%s' to copied group in second file '%s' didn't exist!\n",
       ...
       * all its attributes */
      if ((tmp_group_id =
               H5Gopen2(file_id2, OBJECT_COPY_BETWEEN_FILES_TEST_NEW_GROUP_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    failed to open group copy '%s'\n",
               OBJECT_COPY_BETWEEN_FILES_TEST_NEW_GROUP_NAME);
    ...
    PART_BEGIN(H5Ocopy_dset_between_files) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_BETWEEN_FILES_TEST_DSET_NAME, file_id2,
                  OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DSET_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
      ...
      if ((object_link_exists =
               H5Lexists(file_id2, OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DSET_NAME,
                         H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if link '%s' to copied dataset exists\n",
               OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DSET_NAME);
      ...
      if (!object_link_exists) {
        H5_FAILED_API();
        printf("    link '%s' to copied dataset in second file '%s' didn't "
               "exist!\n",
               OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DSET_NAME, obj_copy_filename);
        PART_ERROR_API(H5Ocopy_dset_between_files);
      }

      /* Ensure that the new dataset has all the attributes of the copied
       * dataset */
      if ((tmp_dset_id =
               H5Dopen2(file_id2, OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DSET_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    failed to open dataset copy '%s'\n",
               OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DSET_NAME);
    ...
    PART_BEGIN(H5Ocopy_dtype_between_files) {
      ...
      if (H5Ocopy(group_id, OBJECT_COPY_BETWEEN_FILES_TEST_DTYPE_NAME, file_id2,
                  OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DTYPE_NAME, H5P_DEFAULT,
                  H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf(
            "    failed to copy committed datatype '%s' to second file '%s'\n",
            OBJECT_COPY_BETWEEN_FILES_TEST_DTYPE_NAME,
      ...
      if ((object_link_exists = H5Lexists(
               ...
               H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if link '%s' to copied committed "
               "datatype exists\n",
               OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DTYPE_NAME);
        PART_ERROR_API(H5Ocopy_dtype_between_files);
      }
      ...
      if (!object_link_exists) {
        H5_FAILED_API();
        printf("    link '%s' to copied committed datatype in second file '%s' "
               "didn't exist!\n",
               OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DTYPE_NAME,
               obj_copy_filename);
        PART_ERROR_API(H5Ocopy_dtype_between_files);
      ...
      /* Ensure that the new committed datatype has all the attributes of the
       * copied committed datatype
       */
      if ((tmp_dtype_id =
               H5Topen2(file_id2, OBJECT_COPY_BETWEEN_FILES_TEST_NEW_DTYPE_NAME,
                        H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
...
static int test_object_copy_invalid_params(void) {
  ...
  if ((group_id2 =
           H5Gcreate2(group_id, OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ocopy_invalid_src_loc_id) {
      ...
      H5E_BEGIN_TRY {
        err_ret =
            H5Ocopy(H5I_INVALID_HID, OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME,
                    group_id, OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME2,
                    H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ocopy_invalid_src_obj_name) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ocopy(group_id, NULL, group_id,
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ocopy(group_id, "", group_id,
                          OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME2,
                          H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ocopy_invalid_dst_loc_id) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ocopy(group_id, OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME,
                          H5I_INVALID_HID,
                          OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME2,
                          H5P_DEFAULT, H5P_DEFAULT);
      }
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf(
            "    H5Ocopy succeeded with an invalid destination location ID!\n");
    ...
    PART_BEGIN(H5Ocopy_invalid_dst_obj_name) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ocopy(group_id, OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME,
                          group_id, NULL, H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ocopy(group_id, OBJECT_COPY_INVALID_PARAMS_TEST_GROUP_NAME,
                          group_id, "", H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY

      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    H5Ocopy succeeded with an invalid destination object name "
               "of ''!\n");
        PART_ERROR_API(H5Ocopy_invalid_dst_obj_name);
      }
...
static int test_object_visit(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, object, dataset, "
           "attribute, stored datatype, or "
  ...
  if ((fspace_id = generate_random_dataspace(OBJECT_VISIT_TEST_SPACE_RANK, NULL,
                                             NULL, FALSE)) < 0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(group_id, OBJECT_VISIT_TEST_TYPE_NAME, type_id, H5P_DEFAULT,
                 H5P_DEFAULT, H5P_DEFAULT) < 0) {
    H5_FAILED_API();
    printf("    couldn't commit datatype '%s'\n", OBJECT_VISIT_TEST_TYPE_NAME);
    goto error;
  ...
  BEGIN_MULTIPART {
    ...
    PART_BEGIN(H5Ovisit_obj_name_increasing) {
      ...
      if (H5Ovisit3(group_id, H5_INDEX_NAME, H5_ITER_INC, object_visit_callback,
                    &i, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Ovisit_obj_name_decreasing) {
      ...
      if (H5Ovisit3(group_id, H5_INDEX_NAME, H5_ITER_DEC, object_visit_callback,
                    &i, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Ovisit_create_order_increasing) {
      ...
      if (H5Ovisit3(group_id, H5_INDEX_CRT_ORDER, H5_ITER_INC,
                    object_visit_callback, &i, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Ovisit_create_order_decreasing) {
      ...
      if (H5Ovisit3(group_id, H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                    object_visit_callback, &i, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Ovisit_dset) {
      ...
      if (H5Ovisit3(dset_id, H5_INDEX_NAME, H5_ITER_INC,
                    object_visit_dset_callback, NULL, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Ovisit_dtype) {
      ...
      if (H5Ovisit3(type_id, H5_INDEX_NAME, H5_ITER_INC,
                    object_visit_dtype_callback, NULL, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit failed\n");
    ...
    PART_BEGIN(H5Ovisit_by_name_obj_name_increasing) {
      ...
      if (H5Ovisit_by_name3(group_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                            object_visit_callback, &i, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf(
            "    H5Ovisit_by_name by object name in increasing order failed\n");
      ...
      /* Reset the special counter and repeat the test using an indirect object
       * name. */
      i = 0;

      if (H5Ovisit_by_name3(container_group, OBJECT_VISIT_TEST_SUBGROUP_NAME,
                            H5_INDEX_NAME, H5_ITER_INC, object_visit_callback,
    ...
    PART_BEGIN(H5Ovisit_by_name_obj_name_decreasing) {
      ...
      /* First, test visiting using "." for the object name */
      if (H5Ovisit_by_name3(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC,
                            object_visit_callback, &i, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf(
            "    H5Ovisit_by_name by object name in decreasing order failed\n");
      ...
      if (H5Ovisit_by_name3(container_group, OBJECT_VISIT_TEST_SUBGROUP_NAME,
                            H5_INDEX_NAME, H5_ITER_DEC, object_visit_callback,
                            &i, H5O_INFO_ALL, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf(
            "    H5Ovisit_by_name by object name in decreasing order failed\n");
    ...
    PART_BEGIN(H5Ovisit_by_name_create_order_increasing) {
      ...
      if (H5Ovisit_by_name3(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                            ...
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit_by_name by creation order in increasing order "
               "failed\n");
    ...
    PART_BEGIN(H5Ovisit_by_name_create_order_decreasing) {
      ...
      /* First, test visiting using "." for the object name */
      if (H5Ovisit_by_name3(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                            object_visit_callback, &i, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit_by_name by creation order in decreasing order "
               "failed\n");
      ...
      if (H5Ovisit_by_name3(container_group, OBJECT_VISIT_TEST_SUBGROUP_NAME,
                            H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                            object_visit_callback, &i, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit_by_name by creation order in decreasing order "
    ...
    PART_BEGIN(H5Ovisit_by_name_dset) {
      ...
      if (H5Ovisit_by_name3(group_id, OBJECT_VISIT_TEST_DSET_NAME,
                            H5_INDEX_NAME, H5_ITER_INC,
                            object_visit_dset_callback, NULL, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Ovisit_by_name_dtype) {
      ...
      if (H5Ovisit_by_name3(group_id, OBJECT_VISIT_TEST_TYPE_NAME,
                            H5_INDEX_NAME, H5_ITER_INC,
                            object_visit_dtype_callback, NULL, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit_by_name failed\n");
...
static int test_object_visit_soft_link(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_SOFT_LINKS)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, object, soft link, or "
           "iterate "
           "aren't supported with this connector\n");
    return 0;
  }
  ...
  if ((subgroup_id =
           H5Gcreate2(group_id, OBJECT_VISIT_SOFT_LINK_TEST_GROUP_NAME1,
                      H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  if ((subgroup_id2 =
           H5Gcreate2(group_id, OBJECT_VISIT_SOFT_LINK_TEST_GROUP_NAME2,
                      H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  if ((linked_group_id =
           H5Gcreate2(subgroup_id2, OBJECT_VISIT_SOFT_LINK_TEST_OBJ_NAME1,
                      H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  if ((linked_group_id =
           H5Gcreate2(subgroup_id2, OBJECT_VISIT_SOFT_LINK_TEST_OBJ_NAME2,
                      H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  if ((linked_group_id =
           H5Gcreate2(subgroup_id2, OBJECT_VISIT_SOFT_LINK_TEST_OBJ_NAME3,
                      H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  /* Create soft links under subgroup 1 to point to the previously-created
   * objects */
  if (H5Lcreate_soft("/" OBJECT_TEST_GROUP_NAME
                     "/" OBJECT_VISIT_SOFT_LINK_TEST_SUBGROUP_NAME
                     "/" OBJECT_VISIT_SOFT_LINK_TEST_GROUP_NAME2
                     "/" OBJECT_VISIT_SOFT_LINK_TEST_OBJ_NAME1,
                     subgroup_id, OBJECT_VISIT_SOFT_LINK_TEST_LINK_NAME1,
                     H5P_DEFAULT, H5P_DEFAULT) < 0) {
    H5_FAILED_API();
    printf("    couldn't create soft link '%s'\n",
  ...
  if (H5Lcreate_soft("/" OBJECT_TEST_GROUP_NAME
                     ...
                     "/" OBJECT_VISIT_SOFT_LINK_TEST_GROUP_NAME2
                     "/" OBJECT_VISIT_SOFT_LINK_TEST_OBJ_NAME2,
                     subgroup_id, OBJECT_VISIT_SOFT_LINK_TEST_LINK_NAME2,
                     H5P_DEFAULT, H5P_DEFAULT) < 0) {
    H5_FAILED_API();
    printf("    couldn't create soft link '%s'\n",
  ...
  if (H5Lcreate_soft("/" OBJECT_TEST_GROUP_NAME
                     ...
                     "/" OBJECT_VISIT_SOFT_LINK_TEST_GROUP_NAME2
                     "/" OBJECT_VISIT_SOFT_LINK_TEST_OBJ_NAME3,
                     subgroup_id, OBJECT_VISIT_SOFT_LINK_TEST_LINK_NAME3,
                     H5P_DEFAULT, H5P_DEFAULT) < 0) {
    H5_FAILED_API();
    printf("    couldn't create soft link '%s'\n",
  ...
  BEGIN_MULTIPART {
    ...
    PART_BEGIN(H5Ovisit_obj_name_increasing) {
      ...
      if (H5Ovisit3(subgroup_id, H5_INDEX_NAME, H5_ITER_INC,
                    object_visit_soft_link_callback, &i, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit by object name in increasing order failed\n");
        PART_ERROR_API(H5Ovisit_obj_name_increasing);
      }
    ...
    PART_BEGIN(H5Ovisit_obj_name_decreasing) {
      ...
      if (H5Ovisit3(subgroup_id, H5_INDEX_NAME, H5_ITER_DEC,
                    object_visit_soft_link_callback, &i, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit by object name in decreasing order failed\n");
        PART_ERROR_API(H5Ovisit_obj_name_decreasing);
    ...
    PART_BEGIN(H5Ovisit_create_order_decreasing) {
      ...
      if (H5Ovisit3(subgroup_id, H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                    object_visit_soft_link_callback, &i, H5O_INFO_ALL) < 0) {
        H5_FAILED_API();
        printf("    H5Ovisit by creation order in decreasing order failed\n");
        PART_ERROR_API(H5Ovisit_create_order_decreasing);
    ...
    PART_BEGIN(H5Ovisit_by_name_obj_name_increasing) {
      ...
      if (H5Ovisit_by_name3(subgroup_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                            object_visit_soft_link_callback, &i, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf(
            "    H5Ovisit_by_name by object name in increasing order failed\n");
      ...
      if (H5Ovisit_by_name3(group_id, OBJECT_VISIT_SOFT_LINK_TEST_GROUP_NAME1,
                            H5_INDEX_NAME, H5_ITER_INC,
                            object_visit_soft_link_callback, &i, H5O_INFO_ALL,
                            H5P_DEFAULT) < 0) {
        H5_FAILED_API();
...
static int test_object_visit_invalid_params(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_ITERATE)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, or iterate aren't "
           "supported with this connector\n");
    return 0;
  }
  ...
  if ((group_id2 =
           H5Gcreate2(group_id, OBJECT_VISIT_INVALID_PARAMS_TEST_GROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create group '%s'\n",
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Ovisit_invalid_obj_id) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ovisit3(H5I_INVALID_HID, H5_INDEX_NAME, H5_ITER_INC,
                            object_visit_noop_callback, NULL, H5O_INFO_ALL);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ovisit_invalid_index_type) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ovisit3(group_id, H5_INDEX_UNKNOWN, H5_ITER_INC,
                            object_visit_noop_callback, NULL, H5O_INFO_ALL);
      }
      H5E_END_TRY
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    H5Ovisit succeeded with invalid index type "
               "H5_INDEX_UNKNOWN!\n");
        PART_ERROR_API(H5Ovisit_invalid_index_type);
      }

      H5E_BEGIN_TRY {
        err_ret = H5Ovisit3(group_id, H5_INDEX_N, H5_ITER_INC,
                            object_visit_noop_callback, NULL, H5O_INFO_ALL);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ovisit_invalid_iter_order) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ovisit3(group_id, H5_INDEX_NAME, H5_ITER_UNKNOWN,
                            object_visit_noop_callback, NULL, H5O_INFO_ALL);
      }
      H5E_END_TRY
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    H5Ovisit succeeded with invalid iteration ordering "
               "H5_ITER_UNKNOWN!\n");
        PART_ERROR_API(H5Ovisit_invalid_iter_order);
      }

      H5E_BEGIN_TRY {
        err_ret = H5Ovisit3(group_id, H5_INDEX_NAME, H5_ITER_N,
                            object_visit_noop_callback, NULL, H5O_INFO_ALL);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ovisit_by_name_invalid_loc_id) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ovisit_by_name3(H5I_INVALID_HID, ".", H5_INDEX_NAME,
                                    H5_ITER_N, object_visit_noop_callback, NULL,
    ...
    PART_BEGIN(H5Ovisit_by_name_invalid_obj_name) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ovisit_by_name3(group_id, NULL, H5_INDEX_NAME, H5_ITER_N,
                                    object_visit_noop_callback, NULL,
                                    H5O_INFO_ALL, H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ovisit_by_name_invalid_index_type) {
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    H5Ovisit_by_name succeeded with invalid index type "
               "H5_INDEX_UNKNOWN!\n");
        PART_ERROR_API(H5Ovisit_by_name_invalid_index_type);
      }

      H5E_BEGIN_TRY {
        err_ret = H5Ovisit_by_name3(group_id, ".", H5_INDEX_N, H5_ITER_N,
                                    object_visit_noop_callback, NULL,
                                    H5O_INFO_ALL, H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Ovisit_by_name_invalid_iter_order) {
      ...
      H5E_BEGIN_TRY {
        err_ret = H5Ovisit_by_name3(group_id, ".", H5_INDEX_NAME,
                                    H5_ITER_UNKNOWN, object_visit_noop_callback,
      ...
      if (err_ret >= 0) {
        H5_FAILED_API();
        printf("    H5Ovisit_by_name succeeded with invalid iteration ordering "
               "H5_ITER_UNKNOWN!\n");
...
static int test_close_object(void) {
  ...
  if ((fspace_id = generate_random_dataspace(OBJECT_CLOSE_TEST_SPACE_RANK, NULL,
                                             NULL, FALSE)) < 0)
    TEST_ERROR;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oclose_group) {
      ...
      if ((group_id2 = H5Oopen(group_id, OBJECT_CLOSE_TEST_GRP_NAME,
                               H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't open group '%s' with H5Oopen\n",
               OBJECT_CLOSE_TEST_GRP_NAME);
    ...
    PART_BEGIN(H5Oclose_dset) {
      ...
      if ((dset_id = H5Dcreate2(group_id, OBJECT_CLOSE_TEST_DSET_NAME,
                                dset_dtype, fspace_id, H5P_DEFAULT, H5P_DEFAULT,
                                H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create dataset '%s'\n",
      ...
      if ((dset_id = H5Oopen(group_id, OBJECT_CLOSE_TEST_DSET_NAME,
                             H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't open dataset '%s' with H5Oopen\n",
               OBJECT_CLOSE_TEST_DSET_NAME);
    ...
    PART_BEGIN(H5Oclose_dtype) {
      ...
      H5E_BEGIN_TRY { H5Tclose(dtype_id); }
      H5E_END_TRY

      if ((dtype_id = H5Oopen(group_id, OBJECT_CLOSE_TEST_TYPE_NAME,
                              H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't open datatype '%s' with H5Oopen\n",
               OBJECT_CLOSE_TEST_TYPE_NAME);
...
static int test_close_object_invalid_params(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      !(vol_cap_flags_g & H5VL_CAP_FLAG_OBJECT_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic file or object aren't supported with "
           "this connector\n");
    return 0;
  }
  ...
  H5E_BEGIN_TRY { err_ret = H5Oclose(H5I_INVALID_HID); }
  H5E_END_TRY
...
static int test_close_invalid_objects(void) {
  ...
  if ((group_id =
           H5Gcreate2(container_group, OBJECT_CLOSE_INVALID_TEST_GROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container sub-group '%s'\n",
           OBJECT_OPEN_TEST_GROUP_NAME);
  ...
  if ((attr_space_id = generate_random_dataspace(
           OBJECT_CLOSE_INVALID_TEST_SPACE_RANK, NULL, NULL, TRUE)) < 0)
    TEST_ERROR;
  ...
  if ((attr_id =
           H5Acreate2(group_id, OBJECT_CLOSE_INVALID_TEST_ATTRIBUTE_NAME,
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Oclose_file) {
      ...
      H5E_BEGIN_TRY { status = H5Oclose(file_id); }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Oclose_plist) {
      ...
      H5E_BEGIN_TRY { status = H5Oclose(fapl_id); }
      H5E_END_TRY

      if (status >= 0) {
        H5_FAILED_API();
        printf(
            "    H5Oclose succeeded with an invalid object (property list)!\n");
    ...
    PART_BEGIN(H5Oclose_dspace) {
      ...
      H5E_BEGIN_TRY { status = H5Oclose(attr_space_id); }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Oclose_attribute) {
      ...
      H5E_BEGIN_TRY { status = H5Oclose(attr_id); }
      H5E_END_TRY
...
static herr_t object_copy_attribute_iter_callback(hid_t location_id,
  ...
  if ((types_equal = H5Tequal(attr_type, H5T_NATIVE_INT)) < 0) {
    printf("    failed to determine if attribute's datatype matched what is "
           "expected\n");
    ret_value = H5_ITER_ERROR;
    goto done;
  }
...
static herr_t object_copy_soft_link_non_expand_callback(hid_t group,
  ...
  if (H5L_TYPE_SOFT != info->type) {
    printf(
        "    link type was not H5L_TYPE_SOFT; link must have been expanded!\n");
    ret_value = H5_ITER_ERROR;
    goto done;
  }
  ...
  if (H5Lget_val(group, name, link_val_buf, info->u.val_size, H5P_DEFAULT) <
      0) {
  ...
  snprintf(expected_link_val, OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_BUF_SIZE,
           "/" OBJECT_TEST_GROUP_NAME
           "/" OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_SUBGROUP_NAME "/grp%d",
           (int)(*counter));

  if (strncmp(link_val_buf, expected_link_val,
...
static herr_t object_copy_soft_link_expand_callback(hid_t group,
  ...
  if (H5L_TYPE_HARD != info->type) {
    printf("    link type was not H5L_TYPE_HARD; link must not have been "
           "expanded!\n");
    ret_value = H5_ITER_ERROR;
    goto done;
  }

  /* Ensure that the link's name still follows the 'link1', 'link2', etc.
   * pattern */
  snprintf(expected_link_name, OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_BUF_SIZE,
           "link%d", (int)(*counter));

  if (strncmp(name, expected_link_name,
              OBJECT_COPY_GROUP_WITH_SOFT_LINKS_TEST_BUF_SIZE)) {
...
/*
 * H5Ovisit callback to simply iterate recursively through all of the objects in
 * a group and check to make sure their names match what is expected.
 */
static herr_t object_visit_callback(hid_t o_id, const char *name,
                                    const H5O_info2_t *object_info,
                                    void *op_data) {
  ...
  if (!HDstrncmp(name, ".", strlen(".") + 1) &&
      (counter_val == 0 || counter_val == 4 || counter_val == 8 ||
       counter_val == 12)) {
    if (H5O_TYPE_GROUP == object_info->type)
      goto done;
    else
      printf("    type for object '%s' was not H5O_TYPE_GROUP\n", name);
  } else if (!HDstrncmp(name, OBJECT_VISIT_TEST_GROUP_NAME,
                        strlen(OBJECT_VISIT_TEST_GROUP_NAME) + 1) &&
             (counter_val == 2 || counter_val == 6 || counter_val == 9 ||
              counter_val == 15)) {
    if (H5O_TYPE_GROUP == object_info->type)
      goto done;
    else
      printf("    type for object '%s' was not H5O_TYPE_GROUP\n", name);
  } else if (!HDstrncmp(name, OBJECT_VISIT_TEST_DSET_NAME,
                        strlen(OBJECT_VISIT_TEST_DSET_NAME) + 1) &&
             (counter_val == 1 || counter_val == 7 || counter_val == 10 ||
              counter_val == 14)) {
    if (H5O_TYPE_DATASET == object_info->type)
      goto done;
    else
      printf("    type for object '%s' was not H5O_TYPE_DATASET\n", name);
  } else if (!HDstrncmp(name, OBJECT_VISIT_TEST_TYPE_NAME,
                        strlen(OBJECT_VISIT_TEST_TYPE_NAME) + 1) &&
             (counter_val == 3 || counter_val == 5 || counter_val == 11 ||
              counter_val == 13)) {
    ...
    printf("    object '%s' didn't match known names or came in an incorrect "
           "order\n",
           name);
...
static void cleanup_files(void) {
  ...
  prefix_filename(test_path_prefix, OBJECT_COPY_BETWEEN_FILES_TEST_FILE_NAME,
                  &obj_copy_filename);
