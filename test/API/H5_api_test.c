...
/******************************************************************************/
static void H5_api_test_run(void) {
 ...
 *         A result summary of the tests run can be found in
 * threat_info_t->result
 */
void *run_h5_API_tests_thread(void *thread_info) {
...
#ifdef H5_HAVE_MULTITHREAD
  if (MAX_THREAD_ID_LEN + strlen(test_path_prefix) + strlen(TEST_FILE_NAME) >=
  ...
  if (NULL == (tinfo->H5_api_test_filename =
                   (char *)calloc(1, H5_API_TEST_FILENAME_MAX_LENGTH))) {
    fprintf(stderr, "Unable to allocate memory for test file name\n");
    tinfo->result = API_TEST_ERROR;
    goto done;
  }

  if ((chars_written = HDsnprintf(tinfo->H5_api_test_filename,
                                  H5_API_TEST_FILENAME_MAX_LENGTH, "%zu%s%s",
                                  tinfo->thread_idx, test_path_prefix,
                                  TEST_FILE_NAME)) < 0) {
...
#else
  if ((chars_written = HDsnprintf(
           H5_api_test_filename, H5_API_TEST_FILENAME_MAX_LENGTH, "%zu%s%s",
           tinfo->thread_idx, test_path_prefix, TEST_FILE_NAME)) < 0) {
    fprintf(stderr, "Error while creating test file name\n");
    tinfo->result = API_TEST_ERROR;
    goto done;
  }
  ...
  if (0 != HDstrcmp(tinfo->vol_connector_name, "native")) {
    ...
    if (!is_registered) {
      fprintf(stderr,
              "Specified VOL connector '%s' wasn't correctly registered!\n",
              tinfo->vol_connector_name);
      tinfo->result = API_TEST_ERROR;
      goto done;
    } else {
      ...
      if (H5Pget_vol_id(fapl_id, &default_con_id) < 0) {
        fprintf(stderr,
                "Couldn't retrieve ID of VOL connector set on default FAPL\n");
        tinfo->result = API_TEST_ERROR;
        goto done;
      }

      if ((registered_con_id =
               H5VLget_connector_id_by_name(tinfo->vol_connector_name)) < 0) {
      ...
      if (default_con_id != registered_con_id) {
        fprintf(stderr, "VOL connector set on default FAPL didn't match "
                        "specified VOL connector\n");
        tinfo->result = API_TEST_ERROR;
        goto done;
      }
    }
...
#define MAX_THREADS 1
   ...
   * running these tests. Even though these are meant to be serial tests, they
   * will likely be run using mpirun (or similar) and we cannot necessarily
   * expect HDF5 or an HDF5 VOL connector to call MPI_Init.
   */
  MPI_Init(&argc, &argv);
#endif
  ...
  for (size_t nthreads = 1; nthreads <= MAX_THREADS; nthreads++) {
    ...
    for (size_t i = 0; i < nthreads; i++) {
      ...
      if (((thread_info_t *)(retval))->result == API_TEST_ERROR) {
        fprintf(stderr,
                "An internal error occurred during API tests in thread %zu\n",
    ...
    for (size_t i = 0; i < nthreads; i++) {
      ...
      printf(
          "[T%zu] %zu/%zu (%.2f%%) API tests passed with VOL connector '%s'\n",
          tinfo[i].thread_idx, tinfo[i].n_tests_passed_g,
          tinfo[i].n_tests_run_g,
          ((double)tinfo[i].n_tests_passed_g / (double)tinfo[i].n_tests_run_g *
           100.0),
          tinfo[i].vol_connector_name);
      printf("[T%zu] %zu/%zu (%.2f%%) API tests did not pass with VOL "
             "connector '%s'\n",
             tinfo[i].thread_idx, tinfo[i].n_tests_failed_g,
             tinfo[i].n_tests_run_g,
             ((double)tinfo[i].n_tests_failed_g /
              (double)tinfo[i].n_tests_run_g * 100.0),
             tinfo[i].vol_connector_name);
      printf("[T%zu] %zu/%zu (%.2f%%) API tests were skipped with VOL "
             "connector '%s'\n\n",
             tinfo[i].thread_idx, tinfo[i].n_tests_skipped_g,
             tinfo[i].n_tests_run_g,
             ((double)tinfo[i].n_tests_skipped_g /
              (double)tinfo[i].n_tests_run_g * 100.0),
             tinfo[i].vol_connector_name);
    }

    printf("[All threads] %zu/%zu (%.2f%%) API tests passed\n", n_tests_passed,
           n_tests_run, ((double)n_tests_passed / (double)n_tests_run * 100.0));
    printf("[All threads] %zu/%zu (%.2f%%) API tests did not pass\n",
           n_tests_failed, n_tests_run,
           ((double)n_tests_failed / (double)n_tests_run * 100.0));
    printf("[All threads] %zu/%zu (%.2f%%) API tests were skipped\n",
           n_tests_skipped, n_tests_run,
           ((double)n_tests_skipped / (double)n_tests_run * 100.0));
