...
static int test_open_link_without_leading_slash(void) {
  ...
  if ((container_group =
           H5Gopen2(file_id, MISCELLANEOUS_TEST_GROUP_NAME, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  if ((space_id = generate_random_dataspace(
           OPEN_LINK_WITHOUT_SLASH_DSET_SPACE_RANK, NULL, NULL, FALSE)) < 0)
    TEST_ERROR;

  if ((dset_dtype = generate_random_datatype(H5T_NO_CLASS, FALSE)) < 0)
    TEST_ERROR;

  if ((dset_id = H5Dcreate2(container_group, OPEN_LINK_WITHOUT_SLASH_DSET_NAME,
  ...
  if ((dset_id = H5Dopen2(group_id,
                          ...
                          "/" OPEN_LINK_WITHOUT_SLASH_DSET_NAME,
                          H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
...
static int test_object_creation_by_absolute_path(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    SKIPPED_API();
    printf("    API functions for basic file, group, dataset, link, or stored "
           "datatype aren't "
           "supported with this connector\n");
    return 0;
  }
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Gcreate_using_absolute_path) {
      ...
      /* Try to create a group under the container group by using an absolute
       * pathname */
      if ((sub_group_id = H5Gcreate2(
               file_id,
               "/" MISCELLANEOUS_TEST_GROUP_NAME
               "/" OBJECT_CREATE_BY_ABSOLUTE_PATH_TEST_CONTAINER_GROUP_NAME
    ...
    PART_BEGIN(H5Dcreate_using_absolute_path) {
      ...
      /* Try to create a dataset nested at the end of this group chain by using
       * an absolute pathname */
      if ((fspace_id = generate_random_dataspace(
               OBJECT_CREATE_BY_ABSOLUTE_PATH_TEST_DSET_SPACE_RANK, NULL, NULL,
               FALSE)) < 0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Tcommit_using_absolute_path) {
      ...
      /* Try to create a committed datatype in the same fashion as the preceding
       * dataset */
...
static int test_absolute_vs_relative_path(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_LINK_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic file, group, dataset, or link aren't "
           "supported with this "
           "connector\n");
    return 0;
  }
  ...
  /* Start by creating a group to be used during some of the dataset creation
   * operations */
  if ((group_id = H5Gcreate2(
           container_group, ABSOLUTE_VS_RELATIVE_PATH_TEST_CONTAINER_GROUP_NAME,
           H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create container group\n");
    goto error;
  ...
  if ((fspace_id = generate_random_dataspace(
           ABSOLUTE_VS_RELATIVE_PATH_TEST_DSET_SPACE_RANK, NULL, NULL, FALSE)) <
      0)
    TEST_ERROR;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Dcreate_absolute_from_root) {
      ...
      /* Create a dataset by absolute path in the form "/group/dataset" starting
       * from the root group */
      if ((dset_id1 = H5Dcreate2(
               file_id,
               "/" MISCELLANEOUS_TEST_GROUP_NAME
               "/" ABSOLUTE_VS_RELATIVE_PATH_TEST_CONTAINER_GROUP_NAME
               "/" ABSOLUTE_VS_RELATIVE_PATH_TEST_DSET1_NAME,
               dset_dtype1, fspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
          0) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Dcreate_absolute_from_nonroot) {
      ...
      if ((dset_id4 = H5Dcreate2(
               ...
               dset_dtype4, fspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
          0) {
        H5_FAILED_API();
        printf("    couldn't create dataset by absolute path from container "
               "group\n");
    ...
    PART_BEGIN(H5Dcreate_relative_from_nonroot) {
      ...
      /* Create a dataset by relative path in the form "dataset" starting from
       * the test container group
       */
      if ((dset_id5 = H5Dcreate2(
               group_id, ABSOLUTE_VS_RELATIVE_PATH_TEST_DSET5_NAME, dset_dtype5,
               fspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create dataset by relative path from container "
               "group\n");
        PART_ERROR_API(H5Dcreate_relative_from_nonroot);
      }

      /* Create a dataset by relative path in the form "group/dataset" starting
       * from the top-level container group */
      if ((dset_id2 =
               H5Dcreate2(container_group,
                          ABSOLUTE_VS_RELATIVE_PATH_TEST_CONTAINER_GROUP_NAME
                          "/" ABSOLUTE_VS_RELATIVE_PATH_TEST_DSET2_NAME,
                          dset_dtype2, fspace_id, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create dataset by relative path from container "
               "group\n");
    ...
    PART_BEGIN(H5Dcreate_relative_leading_dot_root) {
      TESTING_2_API(
          "dataset creation by path with leading '.' from root group");

      /* Create a dataset by relative path in the form "./group/dataset"
      ...
      if ((dset_id3 = H5Dcreate2(
               ...
               dset_dtype3, fspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
          0) {
        H5_FAILED_API();
        printf("    couldn't create dataset by relative path from root with "
               "leading '.'\n");
    ...
    PART_BEGIN(H5Dcreate_relative_leading_dot_nonroot) {
      TESTING_2_API(
          "dataset creation by path with leading '.' from non-root group");

      /* Create a dataset by relative path in the form "./dataset" starting from
       * the container group */
      if ((dset_id6 = H5Dcreate2(group_id,
                                 "./" ABSOLUTE_VS_RELATIVE_PATH_TEST_DSET6_NAME,
                                 dset_dtype6, fspace_id, H5P_DEFAULT,
...
static int test_dot_for_object_name(void) {
  ...
  if ((subgroup_id =
           H5Gcreate2(container_group, DOT_AS_OBJECT_NAME_TEST_SUBGROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Gcreate_dot_as_name) {
      ...
      H5E_BEGIN_TRY {
        group_id =
            H5Gcreate2(subgroup_id, ".", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Dcreate_dot_as_name) {
      ...
      H5E_BEGIN_TRY {
        dset_id = H5Dcreate2(subgroup_id, ".", H5T_NATIVE_INT, dspace_id,
    ...
    PART_BEGIN(H5Tcommit_dot_as_name) {
      ...
      H5E_BEGIN_TRY {
        ret = H5Tcommit2(subgroup_id, ".", dtype_id, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT);
      }
      H5E_END_TRY
...
static int test_symbols_in_compound_field_name(void) {
  ...
  char member_names[COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_NUM_SUBTYPES]
                   [256];
  ...
  if ((group_id =
           H5Gcreate2(container_group,
                      COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_SUBGROUP_NAME,
                      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  for (i = 0, total_type_size = 0;
       i < COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_NUM_SUBTYPES; i++) {
  ...
  for (i = 0, next_offset = 0;
       i < COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_NUM_SUBTYPES; i++) {
    if (H5Tinsert(compound_type, member_names[i], next_offset, type_pool[i]) <
        0) {
      H5_FAILED_API();
  ...
  if ((fspace_id = generate_random_dataspace(
           COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_DSET_RANK, NULL, NULL,
           FALSE)) < 0)
    TEST_ERROR;

  if ((dset_id = H5Dcreate2(
           group_id, COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_DSET_NAME,
           compound_type, fspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0) {
    H5_FAILED_API();
    printf("    couldn't create dataset\n");
    goto error;
  ...
  if ((dset_id = H5Dopen2(group_id,
                          COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_DSET_NAME,
                          H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
...
error:
  H5E_BEGIN_TRY {
    for (i = 0; i < COMPOUND_WITH_SYMBOLS_IN_MEMBER_NAMES_TEST_NUM_SUBTYPES;
         i++)
