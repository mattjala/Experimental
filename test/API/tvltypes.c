...
void *test_vltypes_alloc_custom(size_t size, void *mem_used) {
  ...
  const size_t extra =
      MAX(sizeof(void *), sizeof(size_t)); /* Extra space needed */
...
void test_vltypes_free_custom(void *_mem, void *mem_used) {
  if (_mem) {
     ...
     */
    unsigned char *mem =
        ((unsigned char *)_mem) - extra; /* Pointer to actual block allocated */
...
static void test_vltypes_dataset_create(void) {
  ...
  H5E_BEGIN_TRY {
    dataset = H5Dcreate2(fid1, "Dataset1", tid1, sid1, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT);
  }
  H5E_END_TRY
...
static void test_vltypes_vlen_atomic(void) {
  ...
  dataset = H5Dcreate2(fid1, "Dataset2", tid1, sid1, H5P_DEFAULT, dcpl_pid,
                       H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  ret = H5Pset_vlen_mem_manager(xfer_pid, test_vltypes_alloc_custom, &mem_used,
                                test_vltypes_free_custom, &mem_used);
  ...
  VERIFY(size, ((SPACE1_DIM1 * (SPACE1_DIM1 + 1)) / 2) * sizeof(unsigned int),
  ...
  /* 10 elements allocated = 1 + 2 + 3 + 4 elements for each array position */
  VERIFY(mem_used,
         ((SPACE1_DIM1 * (SPACE1_DIM1 + 1)) / 2) * sizeof(unsigned int),
         "H5Dread");

  /* Compare data read in */
  for (i = 0; i < SPACE1_DIM1; i++) {
    if (wdata[i].len != rdata[i].len) {
      TestErrPrintf("%d: VL data lengths don't match!, wdata[%d].len=%d, "
                    "rdata[%d].len=%d\n",
                    __LINE__, (int)i, (int)wdata[i].len, (int)i,
                    (int)rdata[i].len);
      continue;
    } /* end if */
    for (j = 0; j < rdata[i].len; j++) {
      if (((unsigned int *)wdata[i].p)[j] != ((unsigned int *)rdata[i].p)[j]) {
        TestErrPrintf("VL data values don't match!, wdata[%d].p[%d]=%d, "
                      "rdata[%d].p[%d]=%d\n",
                      (int)i, (int)j, (int)((unsigned int *)wdata[i].p)[j],
                      (int)i, (int)j, (int)((unsigned int *)rdata[i].p)[j]);
        continue;
      } /* end if */
    }   /* end for */
  }     /* end for */
...
static void rewrite_vltypes_vlen_atomic(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Check Memory Leak for Basic Atomic VL Datatype Functionality\n"));
...
static void test_vltypes_vlen_compound(void) {
  ...
  /* 10 elements allocated = 1 + 2 + 3 + 4 elements for each array position */
  VERIFY(size, ((SPACE1_DIM1 * (SPACE1_DIM1 + 1)) / 2) * sizeof(s1),
         "H5Dvlen_get_buf_size");
  ...
  /* Make certain the correct amount of memory has been used */
  /* 10 elements allocated = 1 + 2 + 3 + 4 elements for each array position */
  VERIFY(mem_used, ((SPACE1_DIM1 * (SPACE1_DIM1 + 1)) / 2) * sizeof(s1),
         "H5Dread");

  /* Compare data read in */
  for (i = 0; i < SPACE1_DIM1; i++) {
    if (wdata[i].len != rdata[i].len) {
      TestErrPrintf("%d: VL data length don't match!, wdata[%d].len=%d, "
                    "rdata[%d].len=%d\n",
                    __LINE__, (int)i, (int)wdata[i].len, (int)i,
                    (int)rdata[i].len);
      continue;
    ...
    for (j = 0; j < rdata[i].len; j++) {
      if (((s1 *)wdata[i].p)[j].i != ((s1 *)rdata[i].p)[j].i) {
        TestErrPrintf("VL data values don't match!, wdata[%d].p[%d].i=%d, "
                      "rdata[%d].p[%d].i=%d\n",
                      (int)i, (int)j, (int)((s1 *)wdata[i].p)[j].i, (int)i,
                      (int)j, (int)((s1 *)rdata[i].p)[j].i);
        continue;
      } /* end if */
      if (!H5_FLT_ABS_EQUAL(((s1 *)wdata[i].p)[j].f, ((s1 *)rdata[i].p)[j].f)) {
        TestErrPrintf("VL data values don't match!, wdata[%d].p[%d].f=%f, "
                      "rdata[%d].p[%d].f=%f\n",
                      (int)i, (int)j, (double)((s1 *)wdata[i].p)[j].f, (int)i,
                      (int)j, (double)((s1 *)rdata[i].p)[j].f);
        continue;
      } /* end if */
    }   /* end for */
  }     /* end for */
...
static void rewrite_vltypes_vlen_compound(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Check Memory Leak for Basic Compound VL Datatype Functionality\n"));
...
static void test_vltypes_compound_vlen_vlen(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Compound Datatypes with VL Atomic Datatype Component "
              "Functionality\n"));
  ...
  for (i = 0; i < SPACE3_DIM1; i++) {
    if (wdata[i].i != rdata[i].i) {
      TestErrPrintf(
          "Integer components don't match!, wdata[%d].i=%d, rdata[%d].i=%d\n",
          (int)i, (int)wdata[i].i, (int)i, (int)rdata[i].i);
      continue;
    } /* end if */
    if (!H5_FLT_ABS_EQUAL(wdata[i].f, rdata[i].f)) {
      TestErrPrintf(
          "Float components don't match!, wdata[%d].f=%f, rdata[%d].f=%f\n",
          (int)i, (double)wdata[i].f, (int)i, (double)rdata[i].f);
      continue;
    } /* end if */

    if (wdata[i].v.len != rdata[i].v.len) {
      TestErrPrintf("%d: VL data length don't match!, wdata[%d].v.len=%d, "
                    "rdata[%d].v.len=%d\n",
                    __LINE__, (int)i, (int)wdata[i].v.len, (int)i,
                    (int)rdata[i].v.len);
      continue;
    } /* end if */

    for (t1 = (hvl_t *)(wdata[i].v.p), t2 = (hvl_t *)(rdata[i].v.p), j = 0;
         j < rdata[i].v.len; j++, t1++, t2++) {
      if (t1->len != t2->len) {
        TestErrPrintf("%d: VL data length don't match!, i=%d, j=%d, "
                      "t1->len=%d, t2->len=%d\n",
      ...
      for (k = 0; k < t2->len; k++) {
        if (((unsigned int *)t1->p)[k] != ((unsigned int *)t2->p)[k]) {
          TestErrPrintf(
              "VL data values don't match!, t1->p[%d]=%d, t2->p[%d]=%d\n",
              (int)k, (int)((unsigned int *)t1->p)[k], (int)k,
...
static void test_vltypes_compound_vlstr(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing VL Datatype of Compound Datatype with VL String "
              "Functionality\n"));
  ...
  /* Create a dataset */
  dataset = H5Dcreate2(fid1, "Dataset1", tid2, sid1, H5P_DEFAULT, cparms,
                       H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  ret =
      H5Sselect_hyperslab(filespace, H5S_SELECT_SET, offset, NULL, dims1, NULL);
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    ...
    for (t1 = (s2 *)(wdata[i].v.p), t2 = (s2 *)(rdata[i].v.p), j = 0;
         j < rdata[i].v.len; j++, t1++, t2++) {
      if (HDstrcmp(t1->string, t2->string) != 0) {
        TestErrPrintf(
            "VL data values don't match!, t1->string=%s, t2->string=%s\n",
            t1->string, t2->string);
        continue;
      } /* end if */
      if (t1->color != t2->color) {
        TestErrPrintf(
            "VL data values don't match!, t1->color=%d, t2->color=%d\n",
            t1->color, t2->color);
        continue;
      } /* end if */
    }   /* end for */
  ...
  ret = H5Sselect_hyperslab(filespace2, H5S_SELECT_SET, offset, NULL, dims1,
                            NULL);
  CHECK(ret, FAIL, "H5Sselect_hyperslab");
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    if (wdata2[i].v.len != rdata2[i].v.len) {
      TestErrPrintf("%d: VL data length don't match!, wdata2[%d].v.len=%d, "
                    "rdata2[%d].v.len=%d\n",
                    __LINE__, (int)i, (int)wdata2[i].v.len, (int)i,
                    (int)rdata2[i].v.len);
      continue;
    } /* end if */

    for (t1 = (s2 *)(wdata2[i].v.p), t2 = (s2 *)(rdata2[i].v.p), j = 0;
         j < rdata2[i].v.len; j++, t1++, t2++) {
      if (HDstrcmp(t1->string, t2->string) != 0) {
        TestErrPrintf(
            "VL data values don't match!, t1->string=%s, t2->string=%s\n",
            t1->string, t2->string);
...
static void test_vltypes_compound_vlen_atomic(void) {
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    ...
    for (j = 0; j < rdata[i].v.len; j++) {
      if (((unsigned int *)wdata[i].v.p)[j] !=
          ((unsigned int *)rdata[i].v.p)[j]) {
        TestErrPrintf("VL data values don't match!, wdata[%d].v.p[%d]=%d, "
                      "rdata[%d].v.p[%d]=%d\n",
                      (int)i, (int)j, (int)((unsigned int *)wdata[i].v.p)[j],
                      (int)i, (int)j, (int)((unsigned int *)rdata[i].v.p)[j]);
        continue;
      } /* end if */
    }   /* end for */
  }     /* end for */
  ...
  dataset = H5Dcreate2(fid1, "Dataset2", tid2, sid1, H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  /* Check data read in */
  for (i = 0; i < SPACE1_DIM1; i++)
    if (rdata[i].i != 0 || !H5_FLT_ABS_EQUAL(rdata[i].f, 0.0F) ||
        rdata[i].v.len != 0 || rdata[i].v.p != NULL)
      TestErrPrintf("VL doesn't match!, rdata[%d].i=%d, rdata[%d].f=%f, "
                    "rdata[%d].v.len=%u, rdata[%d].v.p=%p\n",
                    (int)i, rdata[i].i, (int)i, (double)rdata[i].f, (int)i,
                    (unsigned)rdata[i].v.len, (int)i, rdata[i].v.p);
...
static void rewrite_vltypes_compound_vlen_atomic(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Checking memory leak for compound datatype with VL Atomic "
              "Datatype Component Functionality\n"));
...
static void test_vltypes_vlen_vlen_atomic(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing VL Datatypes with VL Atomic Datatype Component "
              "Functionality\n"));
  ...
  for (i = 0; i < SPACE1_DIM1; i++) {
    ...
    for (t1 = (hvl_t *)wdata[i].p, t2 = (hvl_t *)(rdata[i].p), j = 0;
         j < rdata[i].len; j++, t1++, t2++) {
      if (t1->len != t2->len) {
        TestErrPrintf("%d: VL data length don't match!, i=%d, j=%d, "
                      "t1->len=%d, t2->len=%d\n",
                      __LINE__, (int)i, (int)j, (int)t1->len, (int)t2->len);
...
static void rewrite_longer_vltypes_vlen_vlen_atomic(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Check memory leak for VL Datatypes with VL Atomic Datatype "
  ...
  /* 18 hvl_t elements allocated = 3 + 4 + 5 + 6 elements for each array
   * position */
  /* 52 unsigned int elements allocated = 6 + 10 + 15 + 21 elements */
  /*VERIFY(size, 18 * sizeof(hvl_t) + 52 * sizeof(unsigned int),
   * "H5Dvlen_get_buf_size");*/
...
static void test_vltypes_fill_value(void) {
  ...
  hid_t
      small_select_dspace_id; /* Dataspace ID for selection in small datasets */
  hid_t
      large_select_dspace_id; /* Dataspace ID for selection in large datasets */
  hid_t dset_dspace_id = -1;  /* Dataspace ID for a particular dataset */
  hid_t dset_select_dspace_id =
      -1; /* Dataspace ID for selection in a particular dataset */
  ...
  hsize_t select_offset[] = {
      0}; /* Offset of non-contiguous element selection */
  hsize_t select_stride[] = {
      2}; /* Stride size of non-contiguous element selection */
  hsize_t small_select_count[] = {
      SPACE4_DIM_SMALL /
      2}; /* Count of small non-contiguous element selection */
  hsize_t large_select_count[] = {
      SPACE4_DIM_LARGE /
      2}; /* Count of large non-contiguous element selection */
  hsize_t select_block[] = {
      1}; /* Block size of non-contiguous element selection */
  ...
  const dtype1_struct wdata = {3, 4,   "",    NULL, "\0",    "foo", "two",
                               6, 8.0, 200.0, 2.0,  "solid", "yard"};
  ...
  ret = H5Tinsert(dtype1_id, "guid", HOFFSET(struct dtype1_struct, gui),
                  H5T_NATIVE_UINT);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "pguid", HOFFSET(struct dtype1_struct, pgui),
                  H5T_NATIVE_UINT);
  CHECK(ret, FAIL, "H5Tinsert");
  ...
  ret = H5Tinsert(dtype1_id, "str_name", HOFFSET(dtype1_struct, str_name),
                  str_id);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "str_desc", HOFFSET(dtype1_struct, str_desc),
                  str_id);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "str_orig", HOFFSET(dtype1_struct, str_orig),
                  str_id);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "str_stat", HOFFSET(dtype1_struct, str_stat),
                  str_id);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "ver", HOFFSET(struct dtype1_struct, ver),
                  H5T_NATIVE_UINT);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "val", HOFFSET(struct dtype1_struct, val),
                  H5T_NATIVE_DOUBLE);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "ma", HOFFSET(struct dtype1_struct, ma),
                  H5T_NATIVE_DOUBLE);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "mi", HOFFSET(struct dtype1_struct, mi),
                  H5T_NATIVE_DOUBLE);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "str_form", HOFFSET(dtype1_struct, str_form),
                  str_id);
  CHECK(ret, FAIL, "H5Tinsert");

  ret = H5Tinsert(dtype1_id, "str_unit", HOFFSET(dtype1_struct, str_unit),
                  str_id);
  CHECK(ret, FAIL, "H5Tinsert");
  ...
  ret =
      H5Sselect_hyperslab(small_select_dspace_id, H5S_SELECT_SET, select_offset,
                          select_stride, small_select_count, select_block);
  CHECK(ret, FAIL, "H5Sselect_hyperslab");
  ...
  ret =
      H5Sselect_hyperslab(large_select_dspace_id, H5S_SELECT_SET, select_offset,
                          select_stride, large_select_count, select_block);
  CHECK(ret, FAIL, "H5Sselect_hyperslab");
  ...
  for (layout = H5D_COMPACT; layout <= H5D_CHUNKED; layout++) {
    unsigned compress_loop; /* # of times to run loop, for testing compressed
                               chunked dataset */
    ...
    for (test_loop = 0; test_loop < compress_loop; test_loop++) {
      ...
      dset_id = H5Dcreate2(file_id, dset_name1, dtype1_id, dset_dspace_id,
                           H5P_DEFAULT, tmp_dcpl_id, H5P_DEFAULT);
      ...
      dset_id = H5Dcreate2(file_id, dset_name2, dtype1_id, dset_dspace_id,
                           H5P_DEFAULT, tmp_dcpl_id, H5P_DEFAULT);
      CHECK(dset_id, FAIL, "H5Dcreate2");
  ...
  for (layout = H5D_COMPACT; layout <= H5D_CHUNKED; layout++) {
    ...
    for (test_loop = 0; test_loop < compress_loop; test_loop++) {
      ...
      ret = H5Dread(dset_id, dtype1_id, dset_dspace_id, dset_dspace_id,
                    xfer_pid, rbuf);
      CHECK(ret, FAIL, "H5Dread");

      /* Compare data read in */
      for (i = 0; i < dset_elmts; i++) {
        if (HDstrcmp(rbuf[i].str_id, "foobar") != 0 ||
            HDstrcmp(rbuf[i].str_name, "") != 0 || rbuf[i].str_desc ||
            HDstrcmp(rbuf[i].str_orig, "\0") != 0 ||
      ...
      for (i = 0; i < dset_elmts; i++) {
        if ((i % 2) == select_offset[0]) {
          ...
          if (rbuf[i].str_id || rbuf[i].str_name || rbuf[i].str_desc ||
              rbuf[i].str_orig || rbuf[i].str_stat || rbuf[i].str_form ||
              rbuf[i].str_unit) {
            TestErrPrintf("%d: VL data doesn't match!, index(i) = %d\n",
                          __LINE__, (int)i);
            continue;
  ...
  for (layout = H5D_COMPACT; layout <= H5D_CHUNKED; layout++) {
    ...
    for (test_loop = 0; test_loop < compress_loop; test_loop++) {
      ...
      ret = H5Dwrite(dset_id, dtype1_id, scalar_dspace_id, single_dspace_id,
                     xfer_pid, &wdata);
      CHECK(ret, FAIL, "H5Dwrite");
...
void test_vltypes(void) {
  ...
  test_vltypes_compound_vlen_atomic(); /* Test compound datatypes with VL atomic
                                          components */
  ...
  test_vltypes_vlen_vlen_atomic(); /* Test VL datatype with VL atomic components
                                    */
  rewrite_longer_vltypes_vlen_vlen_atomic();  /*overwrite with VL data of longer
                                                 sequence*/
  rewrite_shorter_vltypes_vlen_vlen_atomic(); /*overwrite with VL data of
                                                 shorted sequence*/
  test_vltypes_compound_vlen_vlen(); /* Test compound datatypes with VL atomic
                                        components */
