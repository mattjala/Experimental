...
#define CHECK_PTR_NULL(ret, where)                                             \
  {                                                                            \
    ...
    if (ret) {                                                                 \
      TestErrPrintf(                                                           \
          "*** UNEXPECTED RETURN from %s is not NULL line %4d in %s\n",        \
          (where), (int)__LINE__, __FILE__);                                   \
      H5Eprint2(H5E_DEFAULT, stdout);                                          \
    }                                                                          \
  }
...
#define CHECK_PTR_EQ(ret, val, where)                                          \
  {                                                                            \
    ...
    if (ret != val) {                                                          \
      TestErrPrintf("*** UNEXPECTED RETURN from %s: returned value of %p is "  \
                    "not equal to %p line %4d in %s\n",                        \
                    (where), (const void *)(ret), (const void *)(val),         \
...
#define VERIFY(_x, _val, where)                                                \
  do {                                                                         \
    ...
    if ((__x) != (__val)) {                                                    \
      TestErrPrintf("*** UNEXPECTED VALUE from %s should be %ld, but is %ld "  \
                    "at line %4d "                                             \
                    "in %s\n",                                                 \
...
#define VERIFY_TYPE(_x, _val, _type, _format, where)                           \
  do {                                                                         \
    ...
    if ((__x) != (__val)) {                                                    \
      TestErrPrintf("*** UNEXPECTED VALUE from %s should be " _format          \
                    ", but is " _format " at line %4d "                        \
                    "in %s\n",                                                 \
                    (where), __val, __x, (int)__LINE__, __FILE__);             \
      H5Eprint2(H5E_DEFAULT, stdout);                                          \
    }                                                                          \
...
char *h5_fixname(const char *base_name, hid_t fapl, char *fullname,
                 size_t size);
char *h5_fixname_superblock(const char *base_name, hid_t fapl, char *fullname,
                            size_t size);
