...
typedef hid_t (*generate_datatype_func)(H5T_class_t parent_class,
                                        hbool_t is_compact, size_t depth);
static hid_t generate_random_datatype_internal(H5T_class_t parent_class,
                                               hbool_t is_compact,
                                               size_t depth);
static hid_t generate_random_datatype_integer(H5T_class_t parent_class,
                                              hbool_t is_compact, size_t depth);
static hid_t generate_random_datatype_float(H5T_class_t parent_class,
                                            hbool_t is_compact, size_t depth);
static hid_t generate_random_datatype_string(H5T_class_t parent_class,
                                             hbool_t is_compact, size_t depth);
static hid_t generate_random_datatype_compound(H5T_class_t parent_class,
                                               hbool_t is_compact,
                                               size_t depth);
static hid_t generate_random_datatype_reference(H5T_class_t parent_class,
                                                hbool_t is_compact,
                                                size_t depth);
static hid_t generate_random_datatype_enum(H5T_class_t parent_class,
                                           hbool_t is_compact, size_t depth);
static hid_t generate_random_datatype_array(H5T_class_t parent_class,
                                            hbool_t is_compact, size_t depth);
...
roll_datatype:
  ...
  case H5T_OPAQUE:
    ...
    break;
  case H5T_COMPOUND:
    /* Currently only allows arrays of integer, float or string. Pick another
     * type if we are creating an array of something other than these. Also
     * don't allow recursion to go too deep. Pick another type that doesn't
     * recursively call this function. */
    if ((H5T_ARRAY == parent_class) ||
        ((depth + 1) > TYPE_GEN_RECURSION_MAX_DEPTH))
      goto roll_datatype;
  ...
  case H5T_REFERENCE:
    ...
    /* Currently only allows arrays of integer, float or string. Pick another
     * type if we are creating an array of something other than these. */
    if (H5T_ARRAY == parent_class)
      goto roll_datatype;
...
static hid_t
generate_random_datatype_integer(H5T_class_t H5_ATTR_UNUSED parent_class,
                                 hbool_t H5_ATTR_UNUSED is_compact,
  ...
  default:
    printf(
        "    invalid value for predefined integer type; should not happen\n");
    goto done;
  }
...
static hid_t
generate_random_datatype_float(H5T_class_t H5_ATTR_UNUSED parent_class,
                               hbool_t H5_ATTR_UNUSED is_compact,
                               H5_ATTR_UNUSED size_t depth) {
  hid_t type_to_copy = H5I_INVALID_HID;
  hid_t datatype = H5I_INVALID_HID;
...
static hid_t
generate_random_datatype_string(H5T_class_t H5_ATTR_UNUSED parent_class,
                                hbool_t H5_ATTR_UNUSED is_compact,
                                H5_ATTR_UNUSED size_t depth) {
  hid_t datatype = H5I_INVALID_HID;
  hid_t ret_value = H5I_INVALID_HID;
   ...
   */
    ...
    if (0 == (rand() % 2)) {
#endif
  if ((datatype = H5Tcreate(H5T_STRING,
                            (size_t)(rand() % STRING_TYPE_MAX_SIZE) + 1)) < 0) {
...
static hid_t
generate_random_datatype_compound(H5T_class_t H5_ATTR_UNUSED parent_class,
                                  hbool_t is_compact, size_t depth) {
  ...
  for (size_t i = 0; i < num_members; i++) {
    ...
    if ((compound_members[i] = generate_random_datatype_internal(
             H5T_COMPOUND, is_compact, depth)) < 0) {
    ...
    if (H5Tinsert(datatype, member_name, next_offset, compound_members[i]) <
        0) {
...
static hid_t
generate_random_datatype_reference(H5T_class_t H5_ATTR_UNUSED parent_class,
                                   hbool_t H5_ATTR_UNUSED is_compact,
                                   H5_ATTR_UNUSED size_t depth) {
  hid_t datatype = H5I_INVALID_HID;
  hid_t ret_value = H5I_INVALID_HID;
...
static hid_t
generate_random_datatype_enum(H5T_class_t H5_ATTR_UNUSED parent_class,
                              hbool_t H5_ATTR_UNUSED is_compact,
                              H5_ATTR_UNUSED size_t depth) {
  size_t num_members = 0;
  hid_t datatype = H5I_INVALID_HID;
...
static hid_t
generate_random_datatype_array(H5T_class_t H5_ATTR_UNUSED parent_class,
                               hbool_t is_compact, size_t depth) {
  unsigned ndims = 0;
  ...
  if ((base_datatype = generate_random_datatype_internal(H5T_ARRAY, is_compact,
                                                         depth)) < 0) {
...
hid_t generate_random_dataspace(int rank, const hsize_t *max_dims,
  ...
  if (is_compact && (rank > COMPACT_SPACE_MAX_DIMS)) {
    printf("    current rank of compact dataspace (%lld) exceeds maximum "
           "dimensionality (%lld)\n",
...
int create_test_container(char *filename, uint64_t vol_cap_flags) {
  ...
  if (vol_cap_flags & H5VL_CAP_FLAG_GROUP_BASIC) {
     ...
     */
    if ((group_id = H5Gcreate2(file_id, GROUP_TEST_GROUP_NAME, H5P_DEFAULT,
                               H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
      printf("    created container group for Group tests\n");
      H5Gclose(group_id);
    }

    if ((group_id = H5Gcreate2(file_id, ATTRIBUTE_TEST_GROUP_NAME, H5P_DEFAULT,
                               H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
      printf("    created container group for Attribute tests\n");
      H5Gclose(group_id);
    }

    if ((group_id = H5Gcreate2(file_id, DATASET_TEST_GROUP_NAME, H5P_DEFAULT,
                               H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
      printf("    created container group for Dataset tests\n");
      H5Gclose(group_id);
    }

    if ((group_id = H5Gcreate2(file_id, DATATYPE_TEST_GROUP_NAME, H5P_DEFAULT,
                               H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
      printf("    created container group for Datatype tests\n");
      H5Gclose(group_id);
    }

    if ((group_id = H5Gcreate2(file_id, LINK_TEST_GROUP_NAME, H5P_DEFAULT,
                               H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
      printf("    created container group for Link tests\n");
      H5Gclose(group_id);
    }

    if ((group_id = H5Gcreate2(file_id, OBJECT_TEST_GROUP_NAME, H5P_DEFAULT,
                               H5P_DEFAULT, H5P_DEFAULT)) >= 0) {
...
herr_t prefix_filename(const char *prefix, const char *filename,
...
#ifdef H5_HAVE_MULTITHREAD
  if ((tinfo = (thread_info_t *)pthread_getspecific(thread_info_key_g)) ==
      NULL) {
  ...
  if ((chars_written =
           HDsnprintf(out_buf, H5_API_TEST_FILENAME_MAX_LENGTH, "%zu%s%s",
                      tinfo->thread_idx, prefix, filename)) < 0) {
    printf("    couldn't prefix filename\n");
    ret_value = FAIL;
    goto done;
  }
#else
  if ((chars_written = HDsnprintf(out_buf, H5_API_TEST_FILENAME_MAX_LENGTH,
