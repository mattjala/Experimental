...
static int test_get_file_obj_count(void) {
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fget_obj_count_files) {
      ...
      if (obj_count != 2) {
        H5_FAILED();
        printf("    number of open files (%ld) did not match expected number "
               "(2)\n",
               obj_count);
        PART_ERROR(H5Fget_obj_count_files);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_types) {
      ...
      if (obj_count != 1) {
        H5_FAILED();
        printf("    number of open named datatypes (%ld) did not match "
               "expected number (1)\n",
               obj_count);
        PART_ERROR(H5Fget_obj_count_types);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_attrs) {
      ...
      if (obj_count != 1) {
        H5_FAILED();
        printf("    number of open attributes (%ld) did not match expected "
               "number (1)\n",
               obj_count);
        PART_ERROR(H5Fget_obj_count_attrs);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_dsets) {
      ...
      if (obj_count != 1) {
        H5_FAILED();
        printf("    number of open datasets (%ld) did not match expected "
               "number (1)\n",
               obj_count);
        PART_ERROR(H5Fget_obj_count_dsets);
      }
