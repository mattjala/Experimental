...
static int test_create_file(void) {
  ...
  if (prefix_filename(test_path_prefix, FILE_CREATE_TEST_FILENAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
...
static int test_create_file_invalid_params(void) {
  ...
  if (prefix_filename(test_path_prefix, FILE_CREATE_INVALID_PARAMS_FILE_NAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  BEGIN_MULTIPART {
    ...
    PART_BEGIN(H5Fcreate_invalid_flags) {
      ...
      H5E_BEGIN_TRY {
        file_id = H5Fcreate(prefixed_filename, H5F_ACC_RDWR, H5P_DEFAULT,
                            H5P_DEFAULT);
      }
      H5E_END_TRY
      ...
      H5E_BEGIN_TRY {
        file_id = H5Fcreate(prefixed_filename, H5F_ACC_CREAT, H5P_DEFAULT,
                            H5P_DEFAULT);
      }
      H5E_END_TRY
...
static int test_create_file_excl(void) {
  ...
  if (prefix_filename(test_path_prefix, FILE_CREATE_EXCL_FILE_NAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_EXCL, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  H5E_BEGIN_TRY {
    file_id =
        H5Fcreate(prefixed_filename, H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  ...
  /* Test creating with H5F_ACC_TRUNC. This will truncate the existing file on
   * disk. */
  if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
...
static int test_open_file(void) {
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fopen_rdonly) {
      ...
      if ((file_id = H5Fopen(H5_API_TEST_FILENAME, H5F_ACC_RDONLY,
                             H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
...
static int test_open_file_invalid_params(void) {
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fopen_invalid_name) {
      ...
      H5E_BEGIN_TRY { file_id = H5Fopen(NULL, H5F_ACC_RDWR, H5P_DEFAULT); }
      H5E_END_TRY
      ...
      H5E_BEGIN_TRY { file_id = H5Fopen("", H5F_ACC_RDWR, H5P_DEFAULT); }
      H5E_END_TRY
...
static int test_open_nonexistent_file(void) {
  ...
  if (prefix_filename(test_path_prefix, NONEXISTENT_FILENAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  /* XXX: Make sure to first delete the file so we know for sure it doesn't
   * exist */
...
static int test_file_permission(void) {
  ...
  if (prefix_filename(test_path_prefix, FILE_PERMISSION_TEST_FILENAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  if ((dset_id =
           H5Dcreate2(file_id, FILE_PERMISSION_TEST_DSET_NAME, H5T_STD_U32LE,
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Gcreate_rdonly_file) {
      ...
      H5E_BEGIN_TRY {
        group_id = H5Gcreate2(file_id, FILE_PERMISSION_TEST_GRP_NAME,
    ...
    PART_BEGIN(H5Dcreate_rdonly_file) {
      ...
      H5E_BEGIN_TRY {
        dset_id =
            H5Dcreate2(file_id, FILE_PERMISSION_TEST_DSET2_NAME, H5T_STD_U32LE,
      ...
      H5E_BEGIN_TRY {
        dset_id = H5Dcreate_anon(file_id, H5T_STD_U32LE, dspace_id, H5P_DEFAULT,
                                 H5P_DEFAULT);
      }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Acreate_rdonly_file) {
      ...
      H5E_BEGIN_TRY {
        attr_id =
            H5Acreate2(file_id, FILE_PERMISSION_TEST_ATTR_NAME, H5T_NATIVE_INT,
    ...
    PART_BEGIN(H5Tcommit_rdonly_file) {
      ...
      H5E_BEGIN_TRY {
        h5_ret = H5Tcommit2(file_id, FILE_PERMISSION_TEST_NAMED_DTYPE, dtype_id,
                            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
...
static int test_close_file_invalid_id(void) {
  ...
  H5E_BEGIN_TRY { err_ret = H5Fclose(H5I_INVALID_HID); }
  H5E_END_TRY
...
static int test_flush_file(void) {
  ...
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
    ...
    SKIPPED_API();
    printf("    API functions for basic file, dataset, or file flush aren't "
           "supported with this "
           "connector\n");
    return 0;
  }
  ...
  if (prefix_filename(test_path_prefix, FILE_FLUSH_TEST_FILENAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  for (u = 0; u < 10; u++) {
    ...
    if ((dset_id = H5Dcreate2(file_id, dset_name, H5T_STD_U32LE, dspace_id,
...
static int test_file_is_accessible(void) {
  ...
  if (prefix_filename(test_path_prefix, fake_filename, &prefixed_filename) <
      0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fis_accessible_valid_file) {
      ...
      if ((is_accessible =
               H5Fis_accessible(H5_API_TEST_FILENAME, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't determine if file '%s' is accessible with default "
               "FAPL\n",
               H5_API_TEST_FILENAME);
    ...
    PART_BEGIN(H5Fis_accessible_invalid_file) {
      ...
      if (is_accessible > 0) {
        H5_FAILED_API();
        printf("    non-existent file '%s' was accessible with default FAPL: "
               "is_accessible=%d!\n",
...
static int test_file_property_lists(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
    ...
    SKIPPED_API();
    printf("    API functions for basic or more file or get property list "
  ...
  if (prefix_filename(test_path_prefix, FILE_PROPERTY_LIST_TEST_FNAME1,
                      &prefixed_filename1) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  }
  if (prefix_filename(test_path_prefix, FILE_PROPERTY_LIST_TEST_FNAME2,
  ...
  if ((file_id1 = H5Fcreate(prefixed_filename1, H5F_ACC_TRUNC, fcpl_id1,
                            H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  if ((file_id2 = H5Fcreate(prefixed_filename2, H5F_ACC_TRUNC, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create file\n");
    goto error;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fget_create_plist) {
      ...
      if (prop_val != FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL) {
        H5_FAILED_API();
        printf("    retrieved test property value '%llu' did not match "
               "expected value '%llu'\n",
      ...
      if (prop_val == FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL) {
        printf("    retrieved test property value '%llu' matched control value "
               "'%llu' when it "
               "shouldn't have\n",
               (long long unsigned)prop_val,
               (long long unsigned)FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL);
    ...
    PART_BEGIN(H5Fget_access_plist) {
      ...
      /* Due to the nature of needing to supply a FAPL with the VOL connector
       * having been set on it to the H5Fcreate() call, we cannot exactly test
       * using H5P_DEFAULT as the FAPL for one of the create calls in this test.
       * However, the use of H5Fget_access_plist() will still be used to check
       * that the FAPL is correct after both creating and opening a file.
       */
    ...
    /* Now see if we can still retrieve copies of the property lists upon
     * opening (instead of creating) a file. If they were reconstructed properly
     * upon file open, the creation property lists should also have the same
     * test values as set before.
     */
    if (fcpl_id1 >= 0) {
      H5E_BEGIN_TRY { H5Pclose(fcpl_id1); }
      H5E_END_TRY
      fcpl_id1 = H5I_INVALID_HID;
    }
    if (fcpl_id2 >= 0) {
      H5E_BEGIN_TRY { H5Pclose(fcpl_id2); }
      H5E_END_TRY
      fcpl_id2 = H5I_INVALID_HID;
    }
    if (fapl_id1 >= 0) {
      H5E_BEGIN_TRY { H5Pclose(fapl_id1); }
      H5E_END_TRY
      fapl_id1 = H5I_INVALID_HID;
    }
    if (fapl_id2 >= 0) {
      H5E_BEGIN_TRY { H5Pclose(fapl_id2); }
      H5E_END_TRY
      fapl_id2 = H5I_INVALID_HID;
    }
    if (file_id1 >= 0) {
      H5E_BEGIN_TRY { H5Fclose(file_id1); }
      H5E_END_TRY
      file_id1 = H5I_INVALID_HID;
    }
    if (file_id2 >= 0) {
      H5E_BEGIN_TRY { H5Fclose(file_id2); }
      H5E_END_TRY
    ...
    PART_BEGIN(H5Fget_create_plist_reopened) {
      ...
      if ((file_id1 = H5Fopen(prefixed_filename1, H5F_ACC_RDWR, H5P_DEFAULT)) <
          0) {
        H5_FAILED_API();
        printf("    couldn't open file\n");
      ...
      if ((file_id2 = H5Fopen(prefixed_filename2, H5F_ACC_RDWR, H5P_DEFAULT)) <
          0) {
        H5_FAILED_API();
        printf("    couldn't open file\n");
        PART_ERROR_API(H5Fget_create_plist_reopened);
...
static int test_get_file_intent(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_MORE)) {
    SKIPPED_API();
    printf("    API functions for basic or more file aren't supported with "
           "this connector\n");
    return 0;
  }
  ...
  if (prefix_filename(test_path_prefix, FILE_INTENT_TEST_FILENAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
...
static int test_get_file_obj_count(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic or more file,  basic dataset, group, "
           "stored datatypes, or attribute "
  ...
  if (prefix_filename(test_path_prefix, GET_OBJ_COUNT_TEST_FILENAME1,
                      &prefixed_filename1) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  }
  if (prefix_filename(test_path_prefix, GET_OBJ_COUNT_TEST_FILENAME2,
                      &prefixed_filename2) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  if ((file_id = H5Fcreate(prefixed_filename1, H5F_ACC_TRUNC, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create file '%s'\n", prefixed_filename1);
    goto error;
  ...
  if ((group_id = H5Gcreate2(file_id, GET_OBJ_COUNT_TEST_GRP_NAME, H5P_DEFAULT,
  ...
  if (H5Tcommit2(file_id2, GET_OBJ_COUNT_TEST_NAMED_DTYPE, named_dtype_id,
                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) < 0) {
    H5_FAILED_API();
  ...
  if ((attr_id =
           H5Acreate2(file_id2, GET_OBJ_COUNT_TEST_ATTR_NAME, H5T_NATIVE_INT,
                      dspace_id, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  if ((dset_id =
           H5Dcreate2(file_id2, GET_OBJ_COUNT_TEST_DSET_NAME, H5T_NATIVE_INT,
                      dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fget_obj_count_files) {
      ...
      if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_FILE)) <
          0) {
        H5_FAILED_API();
      ...
      if (check_open_obj_count(obj_count, 2) < 0) {
        H5_FAILED_API();
        printf("    number of open files (%ld) did not match %s expected "
               "number (2)\n",
    ...
    PART_BEGIN(H5Fget_obj_count_grps_single_file) {
      ...
      if (obj_count != 1) {
        H5_FAILED_API();
        printf("    number of open groups (%ld) did not match expected number "
               "(1)\n",
               obj_count);
        PART_ERROR_API(H5Fget_obj_count_grps_single_file);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_grps) {
      ...
      if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_GROUP)) <
          0) {
        H5_FAILED_API();
      ...
      if (check_open_obj_count(obj_count, 1) < 0) {
        H5_FAILED_API();
        printf("    number of open groups (%ld) did not match %s expected "
               "number (1)\n",
               obj_count, active_thread_ct > 1 ? "or exceed" : "");
        PART_ERROR_API(H5Fget_obj_count_grps);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_types) {
      ...
      if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_DATATYPE)) <
          0) {
        H5_FAILED_API();
      ...
      if (check_open_obj_count(obj_count, 1) < 0) {
        H5_FAILED_API();
        printf("    number of open named datatypes (%ld) did not match %s "
               "expected number (1)\n",
               obj_count, active_thread_ct > 1 ? "or exceed" : "");
        PART_ERROR_API(H5Fget_obj_count_types);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_attrs) {
      ...
      if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_ATTR)) <
          0) {
        H5_FAILED_API();
      ...
      if (check_open_obj_count(obj_count, 1) < 0) {
        H5_FAILED_API();
        printf("    number of open attributes (%ld) did not match %s expected "
               "number (1)\n",
               obj_count, active_thread_ct > 1 ? "or exceed" : "");
        PART_ERROR_API(H5Fget_obj_count_attrs);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_dsets) {
      ...
      if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_DATASET)) <
          0) {
        H5_FAILED_API();
      ...
      if (check_open_obj_count(obj_count, 1) < 0) {
        H5_FAILED_API();
        printf("    number of open datasets (%ld) did not match %s expected "
               "number (1)\n",
               obj_count, active_thread_ct > 1 ? "or exceed" : "");
        PART_ERROR_API(H5Fget_obj_count_dsets);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_all_single_file) {
      ...
      if (obj_count != 2) {
        H5_FAILED_API();
        printf("    number of open objects (%ld) did not match expected number "
               "(2)\n",
               obj_count);
        PART_ERROR_API(H5Fget_obj_count_all_single_file);
      }
    ...
    PART_BEGIN(H5Fget_obj_count_all) {
      ...
      if (check_open_obj_count(obj_count, 6) < 0) {
        H5_FAILED_API();
        printf("    number of open objects (%ld) did not match %s expected "
               "number (6)\n",
               obj_count, active_thread_ct > 1 ? "or exceed" : "");
        PART_ERROR_API(H5Fget_obj_count_all);
      }
    ...
    PART_BEGIN(H5Fget_obj_ids_singular_grp) {
      ...
      if (H5Fget_obj_ids(file_id, H5F_OBJ_GROUP, (size_t)obj_count,
                         &object_id) < 0) {
        H5_FAILED_API();
      ...
      if (object_id != group_id) {
        H5_FAILED_API();
        printf("    opened object ID (%ld) did not match only currently open "
               "group ID (%ld)\n",
               object_id, group_id);
        PART_ERROR_API(H5Fget_obj_ids_singular_grp);
      }
...
static int test_file_open_overlap(void) {
  ...
  if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) ||
      ...
      !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic or more file, dataset, or group aren't "
           "supported with this "
           "connector\n");
    return 0;
  }

  if (prefix_filename(test_path_prefix, OVERLAPPING_FILENAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  if ((group_id = H5Gcreate2(file_id, OVERLAPPING_OPEN_TEST_GRP_NAME,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  if ((dset_id =
           H5Dcreate2(group_id, OVERLAPPING_OPEN_TEST_DSET_NAME, H5T_NATIVE_INT,
                      dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create the dataset '%s'\n",
  ...
  /* Get the number of objects opened in the first file: 3 == file + dataset +
   * group */
  if ((obj_count = H5Fget_obj_count(file_id, H5F_OBJ_LOCAL | H5F_OBJ_ALL)) <
      0) {
    H5_FAILED_API();
  ...
  if (obj_count != 3) {
    H5_FAILED_API();
    printf("    number of objects opened in file (%ld) did not match expected "
           "number (3)\n",
           obj_count);
    goto error;
  }
  ...
  if ((dset_id =
           H5Dcreate2(file_id2, OVERLAPPING_OPEN_TEST_DSET_NAME, H5T_NATIVE_INT,
                      dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
    printf("    couldn't create the dataset '%s'\n",
           OVERLAPPING_OPEN_TEST_DSET_NAME);
  ...
  if (obj_count != 2) {
    H5_FAILED_API();
    printf("    number of objects opened in the file (%ld) did not match "
           "expected number (2)\n",
           obj_count);
    goto error;
  }
...
static int test_file_mounts(void) {
  ...
  /* Make sure the connector supports the API functions being tested */
  if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) ||
      !(vol_cap_flags_g & H5VL_CAP_FLAG_MOUNT) ||
      !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC)) {
    SKIPPED_API();
    printf("    API functions for basic file,  file mount, or basic group "
           "aren't supported with this "
           "connector\n");
    return 0;
  }

  if (prefix_filename(test_path_prefix, FILE_MOUNT_TEST_FILENAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  if ((group_id = H5Gcreate2(file_id, FILE_MOUNT_TEST_GRP_NAME, H5P_DEFAULT,
                             H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5_FAILED_API();
  ...
  if ((child_fid = H5Fopen(H5_API_TEST_FILENAME, H5F_ACC_RDWR, H5P_DEFAULT)) <
      0) {
    H5_FAILED_API();
    printf("    couldn't open file '%s'\n", H5_API_TEST_FILENAME);
    goto error;
...
static int test_get_file_name(void) {
  ...
  if (prefix_filename(test_path_prefix, GET_FILE_NAME_TEST_FNAME,
                      &prefixed_filename) < 0) {
    H5_FAILED_API();
    printf("    couldn't prefix filename\n");
    goto error;
  ...
  BEGIN_MULTIPART {
    PART_BEGIN(H5Fget_name_file_id) {
      ...
      if (H5Fget_name(file_id, file_name_buf, (size_t)file_name_buf_len + 1) <
          0) {
        H5_FAILED_API();
      ...
      if (HDstrncmp(file_name_buf, prefixed_filename,
                    (size_t)file_name_buf_len)) {
        H5_FAILED_API();
    ...
    PART_BEGIN(H5Fget_name_grp_id) {
      ...
      /* Attempt to retrieve the name of the file from an object that isn't the
       * root group */
      memset(file_name_buf, 0, (size_t)file_name_buf_len);

      if ((group_id = H5Gcreate2(file_id, GET_FILE_NAME_TEST_GRP_NAME,
                                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
      ...
      if (H5Fget_name(group_id, file_name_buf, (size_t)file_name_buf_len + 1) <
          0) {
        H5_FAILED_API();
        printf("    couldn't get file name %s\n", prefixed_filename);
        PART_ERROR_API(H5Fget_name_grp_id);
      ...
      if (group_id >= 0) {
        H5E_BEGIN_TRY { H5Gclose(group_id); }
        H5E_END_TRY
    ...
    PART_BEGIN(H5Fget_name_dset_id) {
      ...
      if ((dset_id = H5Dcreate2(file_id, GET_FILE_NAME_TEST_DSET_NAME,
      ...
      if (H5Fget_name(dset_id, file_name_buf, (size_t)file_name_buf_len + 1) <
          0) {
        H5_FAILED_API();
        printf("    couldn't get file name %s\n", prefixed_filename);
        PART_ERROR_API(H5Fget_name_dset_id);
      ...
      if (dspace_id >= 0) {
        H5E_BEGIN_TRY { H5Sclose(dspace_id); }
        H5E_END_TRY
    ...
    PART_BEGIN(H5Fget_name_attr_id) {
      ...
      if ((attr_id =
               H5Acreate2(file_id, GET_FILE_NAME_TEST_ATTR_NAME, H5T_NATIVE_INT,
                          dspace_id, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED_API();
        printf("    couldn't create the attribute '%s'\n",
      ...
      if (H5Fget_name(attr_id, file_name_buf, (size_t)file_name_buf_len + 1) <
          0) {
        H5_FAILED_API();
        printf("    couldn't get file name %s\n", prefixed_filename);
        PART_ERROR_API(H5Fget_name_attr_id);
    ...
    PART_BEGIN(H5Fget_name_dtype_id) {
      ...
      if (H5Tcommit2(file_id, GET_FILE_NAME_TEST_NAMED_DTYPE, named_dtype_id,
                     H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) < 0) {
        H5_FAILED_API();
        printf("    couldn't commit a named datatype\n");
        PART_ERROR_API(H5Fget_name_dtype_id);
      ...
      if (H5Fget_name(named_dtype_id, file_name_buf,
                      (size_t)file_name_buf_len + 1) < 0) {
        H5_FAILED_API();
        printf("    couldn't get file name %s\n", prefixed_filename);
        PART_ERROR_API(H5Fget_name_dtype_id);
      }
      ...
      if (named_dtype_id >= 0) {
        H5E_BEGIN_TRY { H5Tclose(named_dtype_id); }
        H5E_END_TRY
    ...
    PART_BEGIN(H5Fget_name_dspace_id) {
      ...
      H5E_BEGIN_TRY {
        name_len = H5Fget_name(dspace_id, file_name_buf,
                               (size_t)file_name_buf_len + 1);
      }
      H5E_END_TRY
      ...
      if (name_len >= 0) {
        H5_FAILED_API();
        printf(
            "    retrieved file name using H5Fget_name on a dataspace ID!\n");
        PART_ERROR_API(H5Fget_name_dspace_id);
      }
...
/*
 * Helper to check that the number of recorded open objects in the entire
 * library is as expected. This is only necessary when H5Fget_obj_count()'s
 * first argument is H5F_OBJ_ALL.
 *
 * If the API tests are being run in a single thread, this is a wrapper around a
 * direct comparison.
 *
 * If the API tests are being run in multiple threads concurrently, then the
 * number of open objects could be any value at least as large as the number of
 * objects opened by this thread
 * - the best we can do is verify it's not definitely wrong.
 */
herr_t check_open_obj_count(ssize_t obj_count, int expected) {
  herr_t ret_value = SUCCEED;
  /* If multiple threads are concurrently executing tests, then more objects
   * than expected may be open in the library */
  if (active_thread_ct > 1 && obj_count < expected) {
    ret_value = FAIL;
  } else if (active_thread_ct == 1 &&
