 ...
 *                      "/dsetA": replace name offset into private heap "72" by
 *bad offset (3) HDlseek(fd, (HDoff_t)1616, SEEK_SET); /soft_one: replace link
 *value offset in the scratch pad "32" by bad offset (C) Write the bad offset
 *value to the file for (1), (2) and (3): write(fd, &val, sizeof(val));
 *
 *              Note: if the groups/datasets/symbolic links are changed in the
 *file, the above locations need to be adjusted accordingly.
 *
...
int main(void) {
  ...
  if ((gid1 = H5Gcreate2(fid, GRP1, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((gid2 = H5Gcreate2(gid1, GRP2, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Lcreate_soft("/group1", fid, SOFT1, H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Lcreate_soft("/group1/group2", fid, SOFT2, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
  ...
  if ((sid = H5Screate(H5S_SCALAR)) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dcreate2(fid, DSET, H5T_NATIVE_INT, sid, H5P_DEFAULT,
  ...
  /* Position the file for /dsetA: replace name offset into private heap "72" by
   * bad offset */
  ...
  /* Position the file for /soft_one: replace link value offset in the scratch
   * pad "32" by bad offset */
