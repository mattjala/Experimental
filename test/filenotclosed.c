...
/*
 * Purpose:	Test to verify that the assertion/abort failure is fixed when
 * the application does not close the file. (See HDFFV-10160)
 */
 ...
 * Function:	main
 *
 * Purpose:	Test to verify the following problem described in HDFFV-10160 is
 *fixed: "a.out: H5Fint.c:1679: H5F_close: Assertion `f->file_id > 0' failed."
 *
...
int main(void) {
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, DATASET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
