...
static hsize_t DIMS[N_SOURCES][RANK] = {
    {0, SM_HEIGHT, WIDTH}, {0, LG_HEIGHT, WIDTH}, {0, SM_HEIGHT, WIDTH},
...
static hsize_t MAX_DIMS[N_SOURCES][RANK] = {
    {N_MAX_PLANES, SM_HEIGHT, WIDTH}, {N_MAX_PLANES, LG_HEIGHT, WIDTH},
    {N_MAX_PLANES, SM_HEIGHT, WIDTH}, {N_MAX_PLANES, LG_HEIGHT, WIDTH},
    {N_MAX_PLANES, SM_HEIGHT, WIDTH}, {N_MAX_PLANES, LG_HEIGHT, WIDTH}};
...
int main(void) {
  ...
  for (i = 0; i < N_SOURCES; i++) {
    ...
    if ((fid = H5Fcreate(FILE_NAMES[i], H5F_ACC_TRUNC, H5P_DEFAULT, faplid)) <
        0)
      TEST_ERROR;
    if ((src_sid = H5Screate_simple(RANK, DIMS[i], MAX_DIMS[i])) < 0)
      TEST_ERROR;
    if ((did = H5Dcreate2(fid, SOURCE_DSET_NAME, SOURCE_DATATYPE, src_sid,
                          H5P_DEFAULT, src_dcplid, H5P_DEFAULT)) < 0)
      TEST_ERROR;

    /* set up hyperslabs for source and destination datasets */
    start[1] = 0;
    if (H5Sselect_hyperslab(src_sid, H5S_SELECT_SET, start, NULL, MAX_DIMS[i],
                            NULL) < 0)
      TEST_ERROR;
    start[1] = (hsize_t)map_start;
    if (H5Sselect_hyperslab(vds_sid, H5S_SELECT_SET, start, NULL, MAX_DIMS[i],
                            NULL) < 0)
      TEST_ERROR;
    ...
    /* Add VDS mapping */
    if (H5Pset_virtual(vds_dcplid, vds_sid, FILE_NAMES[i], SOURCE_DSET_PATH,
                       src_sid) < 0)
      TEST_ERROR;
  ...
  /* dataset */
  if ((did = H5Dcreate2(fid, VDS_DSET_NAME, VDS_DATATYPE, vds_sid, H5P_DEFAULT,
                        vds_dcplid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
