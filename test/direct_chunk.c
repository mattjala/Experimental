...
static int test_direct_chunk_write(hid_t file) {
   ...
   * creation properties.
   */
  if ((dataset = H5Dcreate2(file, DATASETNAME1, H5T_NATIVE_INT, dataspace,
   ...
   */
  if ((status =
           H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl, data)) < 0)
    goto error;
  ...
  for (i = 0; i < NX / CHUNK_NX; i++) {
    for (j = 0; j < NY / CHUNK_NY; j++) {
      status = H5Dwrite_chunk(dataset, dxpl, filter_mask, offset, z_dst_nbytes,
                              outbuf);
      offset[1] += CHUNK_NY;
    }
    offset[0] += CHUNK_NX;
  ...
  block[0] = CHUNK_NX;
  block[1] = CHUNK_NY;
  if ((status = H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, start, stride,
                                    count, block)) < 0)
    goto error;

  /* Read the chunk back */
  if ((status = H5Dread(dataset, H5T_NATIVE_INT, mem_space, dataspace,
                        H5P_DEFAULT, check_chunk)) < 0)
    goto error;
...
static int test_direct_chunk_overwrite_data(hid_t fid) {
  ...
  hsize_t dset_max_dims[] = {H5S_UNLIMITED, OVERWRITE_CHUNK_NY,
                             OVERWRITE_CHUNK_2NX};
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DATASETNAME7, tid, sid, H5P_DEFAULT, dcpl_id,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dwrite_chunk(did, H5P_DEFAULT, filter_mask, offset, buf_size,
                     data_buf) < 0)
    FAIL_STACK_ERROR;
...
static int test_skip_compress_write1(hid_t file) {
   ...
   * creation properties.
   */
  if ((dataset = H5Dcreate2(file, DATASETNAME2, H5T_NATIVE_INT, dataspace,
                            H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    goto error;
  ...
  /* write the uncompressed chunk data repeatedly to dataset, using the direct
   * writing function. Indicate skipping the compression filter.     */
  offset[0] = CHUNK_NX;
  offset[1] = CHUNK_NY;

  filter_mask = 0x00000001;

  if ((status = H5Dwrite_chunk(dataset, dxpl, filter_mask, offset, buf_size,
                               direct_buf)) < 0)
    goto error;
  ...
  if ((status = H5Dread_chunk(dataset, H5P_DEFAULT, offset, &read_filter_mask,
                              read_direct_buf)) < 0)
    goto error;
 ...
 *-------------------------------------------------------------------------
 */
static size_t filter_bogus1(unsigned int flags, size_t H5_ATTR_UNUSED cd_nelmts,
 ...
 *-------------------------------------------------------------------------
 */
static size_t filter_bogus2(unsigned int flags, size_t H5_ATTR_UNUSED cd_nelmts,
                            const unsigned int H5_ATTR_UNUSED *cd_values,
                            size_t nbytes, size_t *buf_size, void **buf) {
  int *int_ptr = (int *)*buf;  /* Pointer to the data values */
  size_t buf_left = *buf_size; /* Amount of data buffer left to process */
...
static int test_skip_compress_write2(hid_t file) {
   ...
   * creation properties.
   */
  if ((dataset = H5Dcreate2(file, DATASETNAME3, H5T_NATIVE_INT, dataspace,
                            H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    goto error;
  ...
  /* Initialize data for one chunk. Apply operations of two bogus filters to the
   * chunk */
  for (i = n = 0; i < CHUNK_NX; i++)
    for (j = 0; j < CHUNK_NY; j++) {
  ...
  /* write the uncompressed chunk data repeatedly to dataset, using the direct
   * writing function. Indicate skipping the compression filter but keep the
   * other two bogus filters */
  offset[0] = CHUNK_NX;
  offset[1] = CHUNK_NY;
...
static int test_data_conv(hid_t file) {
  ...
  if ((status =
           H5Dread_chunk(dataset, dxpl, offset, &filter_mask, read_chunk)) < 0)
    goto error;
    ...
    for (j = 0; j < CHUNK_NY; j++) {
      if ((direct_buf[i][j]).a != (read_chunk[i][j]).a ||
        ...
        printf("    src={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n",
               (direct_buf[i][j]).a, (direct_buf[i][j]).b,
               (direct_buf[i][j]).c[0], (direct_buf[i][j]).c[1],
               (direct_buf[i][j]).c[2], (direct_buf[i][j]).c[3],
               (direct_buf[i][j]).d, (direct_buf[i][j]).e);
        printf("    dst={a=%d, b=%d, c=[%d,%d,%d,%d], d=%d, e=%d\n",
               (read_chunk[i][j]).a, (read_chunk[i][j]).b,
               (read_chunk[i][j]).c[0], (read_chunk[i][j]).c[1],
               (read_chunk[i][j]).c[2], (read_chunk[i][j]).c[3],
               (read_chunk[i][j]).d, (read_chunk[i][j]).e);
  ...
  if ((status = H5Dread(dataset, dt, mem_space, dataspace, H5P_DEFAULT,
                        check_chunk)) < 0)
    goto error;
    ...
    for (j = 0; j < CHUNK_NY; j++) {
      if ((direct_buf[i][j]).a != (check_chunk[i][j]).a ||
        ...
        printf("    dst={a=%d, c=[%d,%d,%d,%d], e=%d\n", (check_chunk[i][j]).a,
               (check_chunk[i][j]).c[0], (check_chunk[i][j]).c[1],
               (check_chunk[i][j]).c[2], (check_chunk[i][j]).c[3],
               (check_chunk[i][j]).e);
...
static int test_invalid_parameters(hid_t file) {
  ...
  /*
   * Create a new contiguous dataset to verify H5Dwrite_chunk/H5Dread_chunk
   * doesn't work
   */
  if ((dataset = H5Dcreate2(file, DATASETNAME5, H5T_NATIVE_INT, dataspace,
                            H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    goto error;
  ...
  H5E_BEGIN_TRY {
    if ((status = H5Dget_chunk_storage_size(dataset, offset, &chunk_nbytes)) !=
        FAIL)
      goto error;
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    if ((status = H5Dread_chunk(dataset, dxpl, offset, &filter_mask,
                                direct_buf)) != FAIL)
      goto error;
  }
  H5E_END_TRY
   ...
   * creation properties.
   */
  if ((dataset = H5Dcreate2(file, DATASETNAME6, H5T_NATIVE_INT, dataspace,
                            H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    goto error;

  /* Check invalid dataset ID for H5Dwrite_chunk and H5Dread_chunk */
  H5E_BEGIN_TRY {
    if ((status = H5Dwrite_chunk((hid_t)-1, dxpl, filter_mask, offset, buf_size,
                                 direct_buf)) != FAIL)
      goto error;
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    if ((status = H5Dread_chunk((hid_t)-1, dxpl, offset, &filter_mask,
                                direct_buf)) != FAIL)
      goto error;
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    if ((status = H5Dwrite_chunk(dataset, (hid_t)-1, filter_mask, offset,
                                 buf_size, direct_buf)) != FAIL)
      goto error;
  }
  H5E_END_TRY

  H5E_BEGIN_TRY {
    if ((status = H5Dread_chunk(dataset, (hid_t)-1, offset, &filter_mask,
                                direct_buf)) != FAIL)
      goto error;
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    if ((status = H5Dwrite_chunk(dataset, dxpl, filter_mask, NULL, buf_size,
                                 direct_buf)) != FAIL)
      goto error;
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    if ((status = H5Dread_chunk(dataset, dxpl, NULL, &filter_mask,
                                direct_buf)) != FAIL)
      goto error;
  }
  H5E_END_TRY

  /* Check when OFFSET is out of dataset range for H5Dwrite_chunk and
   * H5Dread_chunk */
  offset[0] = NX + 1;
  offset[1] = NY;
  H5E_BEGIN_TRY {
  ...
  /* Check when OFFSET is not on chunk boundary for H5Dwrite_chunk and
   * H5Dread_chunk */
  offset[0] = CHUNK_NX;
  offset[1] = CHUNK_NY + 1;
  H5E_BEGIN_TRY {
  ...
  H5E_BEGIN_TRY {
    if ((status = H5Dread_chunk(dataset, dxpl, offset, &filter_mask, NULL)) !=
        FAIL)
      goto error;
  }
  H5E_END_TRY
...
static int test_direct_chunk_read_no_cache(hid_t file) {
  ...
  int direct_buf[CHUNK_NX][CHUNK_NY];  /* chunk read with H5Dread and manually
                                          decompressed */
  ...
  if ((status = H5Pset_chunk_cache(dapl, 0, H5D_CHUNK_CACHE_NBYTES_DEFAULT,
                                   H5D_CHUNK_CACHE_W0_DEFAULT)) < 0)
    goto error;

  /* Create a new dataset within the file using cparms creation properties. */
  if ((dataset = H5Dcreate2(file, DATASETNAME8, H5T_NATIVE_INT, dataspace,
                            H5P_DEFAULT, cparms, dapl)) < 0)
    goto error;
  ...
  /* For each chunk in the dataset, compare the result of H5Dread and
   * H5Dread_chunk. */
  for (i = 0; i < NX / CHUNK_NX; i++) {
    for (j = 0; j < NY / CHUNK_NY; j++) {
      /* Select hyperslab for one chunk in the file */
      ...
      if ((status = H5Sselect_hyperslab(dataspace, H5S_SELECT_SET, start,
      ...
      if ((status =
               H5Dread_chunk(dataset, dxpl, offset, &filter_mask, outbuf)) < 0)
        goto error;
...
#ifdef H5_HAVE_FILTER_DEFLATE
static int test_direct_chunk_read_cache(hid_t file, hbool_t flush) {
  hid_t dataspace = -1, dataset = -1;
  hid_t mem_space = -1;
  hid_t cparms = -1, dxpl = -1;
  hsize_t dims[2] = {NX, NY};
  ...
  if (flush) {
    ...
    TESTING(
        "basic functionality of H5Dread_chunk (does not flush chunk cache)");
  }
  ...
  /* Create a new dataset within the file using cparms creation properties. */
  if ((dataset = H5Dcreate2(file, flush ? DATASETNAME9 : DATASETNAME10,
                            H5T_NATIVE_INT, dataspace, H5P_DEFAULT, cparms,
                            H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 0; i < NX / CHUNK_NX; i++) {
    for (j = 0; j < NY / CHUNK_NY; j++) {
      ...
      /* Query chunk storage size */
      if ((status =
               H5Dget_chunk_storage_size(dataset, offset, &read_buf_size)) < 0)
        goto error;
      if (read_buf_size == 0)
        goto error;
...
static int test_read_unfiltered_dset(hid_t file) {
  ...
  /* Create a new dataset within the file using cparms creation properties. */
  if ((dataset = H5Dcreate2(file, DATASETNAME12, H5T_NATIVE_INT, dataspace,
                            H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    goto error;
 ...
 *              offsets to chunks that have not been written to the dataset and
 *are not allocated in the chunk storage on disk.
 *
 * Return:      Success:        0
 *              Failure:        1
 *
...
static int test_read_unallocated_chunk(hid_t file) {
  ...
  /* Create a new dataset within the file using cparms creation properties. */
  if ((dataset = H5Dcreate2(file, DATASETNAME11, H5T_NATIVE_INT, dataspace,
                            H5P_DEFAULT, cparms, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dwrite_chunk(dataset, dxpl, filter_mask, offset, chunk_nbytes,
                     direct_buf) < 0)
    FAIL_STACK_ERROR;
  ...
  for (i = 1; i < NX / CHUNK_NX; i++) {
    for (j = 0; j < NY / CHUNK_NY; j++) {
      ...
      H5E_BEGIN_TRY {
        status =
            H5Dread_chunk(dataset, dxpl, offset, &filter_mask, &direct_buf);
      }
      H5E_END_TRY
      ...
      H5E_BEGIN_TRY {
        status =
            H5Dget_chunk_storage_size(dataset, offset, &direct_chunk_nbytes);
      }
      H5E_END_TRY
...
static int test_single_chunk(unsigned config) {
  ...
  if (config & CONFIG_DIRECT_WRITE) {
    ...
    if (H5Dwrite_chunk(did, H5P_DEFAULT, 0, offset, CHUNK0 * CHUNK1 * 4,
                       (void *)wdata) < 0)
      FAIL_STACK_ERROR;
  } /* end if */
  else
