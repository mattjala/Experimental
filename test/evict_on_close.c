 ...
 *-------------------------------------------------------------------------
 */
static H5_ATTR_PURE hbool_t verify_tag_not_in_cache(const H5F_t *f,
                                                    haddr_t tag) {
...
static hid_t generate_eoc_test_file(hid_t fapl_id) {
  ...
  if ((fapl_copy_id = H5Pcopy(fapl_id)) < 0)
    TEST_ERROR;
  if (H5Pset_libver_bounds(fapl_copy_id, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) <
      0)
    TEST_ERROR;
   ...
   */
  if ((gid1 = H5Gcreate2(fid, GROUP_OLD_STYLE_NAME, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < NSUBGROUPS; i++) {
    ...
    if (HDsnprintf(subgroup_name, (size_t)(SUBGROUP_NAME_SIZE - 1), "%d", i) <
        0)
      TEST_ERROR;

    if ((gid2 = H5Gcreate2(gid1, subgroup_name, H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
      TEST_ERROR;
    if (H5Gclose(gid2) < 0)
      TEST_ERROR;
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DSET_BTREE_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
   ...
   * multiple cache entries.
   */
  if ((gid1 = H5Gcreate2(fid, GROUP_NEW_STYLE_NAME, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DSET_EARRAY_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl_id, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DSET_BT2_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl_id, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DSET_FARRAY_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl_id, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DSET_SINGLE_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl_id, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DSET_CONTIGUOUS_NAME, H5T_NATIVE_INT, sid,
  ...
  /* Create dataset */
  if ((did = H5Dcreate2(fid, DSET_COMPACT_NAME, H5T_NATIVE_INT, sid,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t check_evict_on_close_parallel_fail(void) {
  hid_t fapl_id = -1;
  hbool_t evict_on_close;
  herr_t status;
...
int main(void) {
  ...
  printf("All evict-on-close tests passed.\n");
  printf("Note that EoC is not supported under parallel so most tests are "
         "skipped.\n");
