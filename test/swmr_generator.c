...
static int gen_skeleton(const char *filename, hbool_t verbose,
                        hbool_t swmr_write, int comp_level,
...
static int gen_skeleton(const char *filename, hbool_t verbose,
  ...
  /* Can create a file for SWMR support with: (a) (write+latest-format) or (b)
   * (SWMR write+non-latest-format) */
  if (!swmr_write) {
  ...
  if ((fid = H5Fcreate(filename,
                       H5F_ACC_TRUNC | (swmr_write ? H5F_ACC_SWMR_WRITE : 0),
                       fcpl, fapl)) < 0)
    return -1;
  ...
  for (u = 0; u < NLEVELS; u++)
    for (v = 0; v < symbol_count[u]; v++) {
      ...
      hbool_t move_dataspace_message =
          FALSE; /* Whether to move the dataspace message out of object header
                    chunk #0 */

      generate_name(name_buf, sizeof(name_buf), u, v);
      if ((dsid = H5Dcreate2(fid, name_buf, tid, sid, H5P_DEFAULT, dcpl,
                             H5P_DEFAULT)) < 0)
        return -1;
...
static void usage(void) {
  ...
  printf("\n");
  printf(
      "NOTE: The random seed option is only used by the sparse test.  Other\n");
  ...
  printf("\n");
  printf("Defaults to verbose (no '-q' given), no SWMR_WRITE mode (no '-s' "
         "given) no\n");
  printf("compression ('-c -1'), v1 b-tree indexing (-i b1), and will generate "
         "a random\n");
...
int main(int argc, char *argv[]) {
  ...
  hbool_t swmr_write =
      FALSE; /* Whether to create file with SWMR_WRITE access */
  ...
  fprintf(stderr,
          "Using generator random seed (used in sparse test only): %u\n",
          random_seed);
  ...
  if (gen_skeleton(FILENAME, verbose, swmr_write, comp_level, index_type,
                   random_seed) < 0) {
