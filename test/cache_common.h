...
#define NANO_BASE_ADDR                                                         \
  (haddr_t)(PICO_BASE_ADDR + (PICO_ENTRY_SIZE * NUM_PICO_ENTRIES))
#define MICRO_BASE_ADDR                                                        \
  (haddr_t)(NANO_BASE_ADDR + (NANO_ENTRY_SIZE * NUM_NANO_ENTRIES))
#define TINY_BASE_ADDR                                                         \
  (haddr_t)(MICRO_BASE_ADDR + (MICRO_ENTRY_SIZE * NUM_MICRO_ENTRIES))
#define SMALL_BASE_ADDR                                                        \
  (haddr_t)(TINY_BASE_ADDR + (TINY_ENTRY_SIZE * NUM_TINY_ENTRIES))
#define MEDIUM_BASE_ADDR                                                       \
  (haddr_t)(SMALL_BASE_ADDR + (SMALL_ENTRY_SIZE * NUM_SMALL_ENTRIES))
#define LARGE_BASE_ADDR                                                        \
  (haddr_t)(MEDIUM_BASE_ADDR + (MEDIUM_ENTRY_SIZE * NUM_MEDIUM_ENTRIES))
#define HUGE_BASE_ADDR                                                         \
  (haddr_t)(LARGE_BASE_ADDR + (LARGE_ENTRY_SIZE * NUM_LARGE_ENTRIES))
#define MONSTER_BASE_ADDR                                                      \
  (haddr_t)(HUGE_BASE_ADDR + (HUGE_ENTRY_SIZE * NUM_HUGE_ENTRIES))
#define VARIABLE_BASE_ADDR                                                     \
  (haddr_t)(MONSTER_BASE_ADDR + (MONSTER_ENTRY_SIZE * NUM_MONSTER_ENTRIES))
#define NOTIFY_BASE_ADDR                                                       \
  (haddr_t)(VARIABLE_BASE_ADDR + (VARIABLE_ENTRY_SIZE * NUM_VARIABLE_ENTRIES))

#define PICO_ALT_BASE_ADDR                                                     \
  (haddr_t)(NOTIFY_BASE_ADDR + (NOTIFY_ENTRY_SIZE * NUM_NOTIFY_ENTRIES))
#define NANO_ALT_BASE_ADDR                                                     \
  (haddr_t)(PICO_ALT_BASE_ADDR + (PICO_ENTRY_SIZE * NUM_PICO_ENTRIES))
#define MICRO_ALT_BASE_ADDR                                                    \
  (haddr_t)(NANO_ALT_BASE_ADDR + (NANO_ENTRY_SIZE * NUM_NANO_ENTRIES))
#define TINY_ALT_BASE_ADDR                                                     \
  (haddr_t)(MICRO_ALT_BASE_ADDR + (MICRO_ENTRY_SIZE * NUM_MICRO_ENTRIES))
#define SMALL_ALT_BASE_ADDR                                                    \
  (haddr_t)(TINY_ALT_BASE_ADDR + (TINY_ENTRY_SIZE * NUM_TINY_ENTRIES))
#define MEDIUM_ALT_BASE_ADDR                                                   \
  (haddr_t)(SMALL_ALT_BASE_ADDR + (SMALL_ENTRY_SIZE * NUM_SMALL_ENTRIES))
#define LARGE_ALT_BASE_ADDR                                                    \
  (haddr_t)(MEDIUM_ALT_BASE_ADDR + (MEDIUM_ENTRY_SIZE * NUM_MEDIUM_ENTRIES))
#define HUGE_ALT_BASE_ADDR                                                     \
  (haddr_t)(LARGE_ALT_BASE_ADDR + (LARGE_ENTRY_SIZE * NUM_LARGE_ENTRIES))
#define MONSTER_ALT_BASE_ADDR                                                  \
  (haddr_t)(HUGE_ALT_BASE_ADDR + (HUGE_ENTRY_SIZE * NUM_HUGE_ENTRIES))
#define VARIABLE_ALT_BASE_ADDR                                                 \
  (haddr_t)(MONSTER_ALT_BASE_ADDR + (MONSTER_ENTRY_SIZE * NUM_MONSTER_ENTRIES))
#define NOTIFY_ALT_BASE_ADDR                                                   \
  (haddr_t)(VARIABLE_ALT_BASE_ADDR +                                           \
            (VARIABLE_ENTRY_SIZE * NUM_VARIABLE_ENTRIES))
#define MAX_ADDR                                                               \
  (haddr_t)(NOTIFY_ALT_BASE_ADDR + (NOTIFY_ENTRY_SIZE * NUM_NOTIFY_ENTRIES))
...
typedef struct test_entry_t {
  ...
  struct flush_op flush_ops[MAX_FLUSH_OPS]; /* Array of instances
                                             ...
                                             */
  hbool_t
      flush_op_self_resize_in_progress; /* Boolean flag
  ...
  hbool_t expunged;                     /* entry has been expunged since the
                                         * last time it was reset.
                                         */
  int flush_dep_par_type[MAX_FLUSH_DEP_PARS]; /* Entry types of flush dependency
                                                 parents */
  int flush_dep_par_idx[MAX_FLUSH_DEP_PARS];  /* Indices of flush dependency
                                                 parents */
  ...
  unsigned notify_after_insert_count; /* Count of times that entry was inserted
                                         in cache */
  unsigned notify_before_evict_count; /* Count of times that entry was removed
                                         in cache */
  size_t
      actual_len; /* Simulate the entry's actual size for a speculative load */
...
#define CACHE_CONFIGS_EQUAL(a, b, cmp_set_init, cmp_init_size)                 \
  (((a).version == (b).version) &&                                             \
   ...
   ((a).max_size == (b).max_size) && ((a).min_size == (b).min_size) &&         \
...
H5TEST_DLL void add_flush_op(int target_type, int target_idx, int op_code,
                             int type, int idx, hbool_t flag, size_t size,
                             unsigned *order);

H5TEST_DLL void addr_to_type_and_index(haddr_t addr, int32_t *type_ptr,
                                       int32_t *index_ptr);

H5TEST_DLL void dirty_entry(H5F_t *file_ptr, int32_t type, int32_t idx,
                            hbool_t dirty_pin);

H5TEST_DLL void expunge_entry(H5F_t *file_ptr, int32_t type, int32_t idx);

H5TEST_DLL void insert_entry(H5F_t *file_ptr, int32_t type, int32_t idx,
                             unsigned int flags);

H5TEST_DLL void mark_entry_dirty(int32_t type, int32_t idx);

H5TEST_DLL void move_entry(H5C_t *cache_ptr, int32_t type, int32_t idx,
                           hbool_t main_addr);
...
H5TEST_DLL void resize_entry(H5F_t *file_ptr, int32_t type, int32_t idx,
                             size_t new_size, hbool_t in_cache);

H5TEST_DLL void row_major_scan_forward(
    H5F_t *file_ptr, int32_t max_index, int32_t lag, hbool_t verbose,
    hbool_t reset_stats, hbool_t display_stats, hbool_t display_detailed_stats,
...
H5TEST_DLL void hl_row_major_scan_forward(H5F_t *file_ptr, int32_t max_index,
                                          hbool_t verbose, hbool_t reset_stats,
                                          hbool_t display_stats,
                                          hbool_t display_detailed_stats,
                                          hbool_t do_inserts);

H5TEST_DLL void row_major_scan_backward(
    H5F_t *file_ptr, int32_t max_index, int32_t lag, hbool_t verbose,
    hbool_t reset_stats, hbool_t display_stats, hbool_t display_detailed_stats,
    hbool_t do_inserts, hbool_t do_moves, hbool_t move_to_main_addr,
    hbool_t do_destroys, hbool_t do_mult_ro_protects, int dirty_destroys,
...
H5TEST_DLL void hl_row_major_scan_backward(H5F_t *file_ptr, int32_t max_index,
                                           hbool_t verbose, hbool_t reset_stats,
                                           hbool_t display_stats,
                                           hbool_t display_detailed_stats,
                                           hbool_t do_inserts);
...
H5TEST_DLL void
col_major_scan_forward(H5F_t *file_ptr, int32_t max_index, int32_t lag,
                       hbool_t verbose, hbool_t reset_stats,
                       hbool_t display_stats, hbool_t display_detailed_stats,
                       hbool_t do_inserts, int dirty_unprotects);

H5TEST_DLL void hl_col_major_scan_forward(H5F_t *file_ptr, int32_t max_index,
                                          hbool_t verbose, hbool_t reset_stats,
                                          hbool_t display_stats,
                                          hbool_t display_detailed_stats,
                                          hbool_t do_inserts,
...
H5TEST_DLL void
col_major_scan_backward(H5F_t *file_ptr, int32_t max_index, int32_t lag,
                        hbool_t verbose, hbool_t reset_stats,
                        hbool_t display_stats, hbool_t display_detailed_stats,
                        hbool_t do_inserts, int dirty_unprotects);

H5TEST_DLL void hl_col_major_scan_backward(H5F_t *file_ptr, int32_t max_index,
                                           hbool_t verbose, hbool_t reset_stats,
                                           hbool_t display_stats,
                                           hbool_t display_detailed_stats,
                                           hbool_t do_inserts,
...
H5TEST_DLL void flush_cache(H5F_t *file_ptr, hbool_t destroy_entries,
                            hbool_t dump_stats, hbool_t dump_detailed_stats);

H5TEST_DLL void unpin_entry(int32_t type, int32_t idx);

H5TEST_DLL void unprotect_entry(H5F_t *file_ptr, int32_t type, int32_t idx,
                                unsigned int flags);
...
H5TEST_DLL void create_flush_dependency(int32_t parent_type, int32_t parent_idx,
                                        int32_t child_type, int32_t child_idx);

H5TEST_DLL void destroy_flush_dependency(int32_t parent_type,
                                         int32_t parent_idx, int32_t child_type,
                                         int32_t child_idx);
...
H5TEST_DLL void
check_and_validate_cache_size(hid_t file_id, size_t *max_size_ptr,
                              size_t *min_clean_size_ptr, size_t *cur_size_ptr,
                              int32_t *cur_num_entries_ptr, hbool_t dump_data);
