 ...
 *----------------------------------------------------------------------------
 */
static inline void jserr_long(long expected, long actual, const char *reason) {
  if (reason != NULL) {
    printf("%s\n", reason);
  } else {
    printf("  ! Expected %ld\n  ! Actual   %ld\n", expected, actual);
 ...
 *----------------------------------------------------------------------------
 */
static inline void jserr_str(const char *expected, const char *actual,
                             const char *reason) {
  if (reason != NULL) {
    printf("%s\n", reason);
  } else {
 ...
 *----------------------------------------------------------------------------
 */
static herr_t test_macro_format_credential(void) {
  /************************
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_aws_canonical_request(void) {
  /*************************
  ...
  for (i = 0; i < n_cases; i++) {
    ...
    for (j = 0; j < C->listsize; j++) {
      FAIL_IF(FAIL == H5FD_s3comms_hrb_node_set(&node, C->list[j].name,
                                                C->list[j].value));
    }
     ...
     */
    JSVERIFY(SUCCEED,
             H5FD_s3comms_aws_canonical_request(cr_dest, 512, sh_dest, 64, hrb),
   ...
   */
  JSVERIFY(FAIL,
           H5FD_s3comms_aws_canonical_request(cr_dest, 20, sh_dest, 20, NULL),
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_bytes_to_hex(void) {
  /*************************
   * test-local structures *
   *************************/
  ...
  for (i = 0; i < n_cases; i++) {
    ...
    JSVERIFY(SUCCEED,
             H5FD_s3comms_bytes_to_hex(out, cases[i].in, cases[i].size,
                                       cases[i].lower),
             NULL)
  ...
  JSVERIFY(
      FAIL,
      H5FD_s3comms_bytes_to_hex(NULL, (const unsigned char *)"nada", 5, FALSE),
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_hrb_init_request(void) {
  /*********************
   * test-local macros *
   *********************/
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_hrb_node_set(void) {
  /*************************
   * test-local structures *
   *************************/
  ...
  for (test_i = 0; test_i < testcases_count; test_i++) {
    ...
    JSVERIFY(
        test->returned,
        H5FD_s3comms_hrb_node_set(&list, test->delta.name, test->delta.value),
        test->message)
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_HMAC_SHA256(void) {
  ...
  struct testcase cases[] = {
      {
          ...
          HDstrlen("AWS4-HMAC-SHA256\n20130524T000000Z\n20130524/us-east-1/s3/"
                   "aws4_"
                   "request\n7344ae5b7ee6c3e7e6b0fe0640412a37625d1fbfff95c48bbb"
                   "2dc43964946972"),
  ...
  for (i = 0; i < n_cases; i++) {
    ...
    JSVERIFY(cases[i].ret,
             H5FD_s3comms_HMAC_SHA256(cases[i].key, cases[i].key_len,
                                      cases[i].msg, cases[i].msg_len, dest),
             cases[i].msg);
 ...
 *----------------------------------------------------------------------------
 */
static herr_t test_nlowercase(void) {
  /*************************
   * test-local structures *
   *************************/
  ...
  for (i = 0; i < n_cases; i++) {
    ...
    JSVERIFY(SUCCEED, H5FD_s3comms_nlowercase(dest, cases[i].in, cases[i].len),
             cases[i].in)
    if (cases[i].len > 0) {
  ...
  JSVERIFY(FAIL, H5FD_s3comms_nlowercase(NULL, cases[0].in, cases[0].len),
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_parse_url(void) {
  /*********************
   * test-local macros *
   *********************/
  ...
  for (i = 0; i < ncases; i++) {
    ...
    JSVERIFY(cases[i].exp_ret, H5FD_s3comms_parse_url(cases[i].url, &purl),
             cases[i].msg)
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_percent_encode_char(void) {
  /*************************
   * test-local structures *
   *************************/
  ...
  for (i = 0; i < n_cases; i++) {
    JSVERIFY(SUCCEED,
             H5FD_s3comms_percent_encode_char(
                 dest, (const unsigned char)cases[i].c, &dest_len),
             NULL)
  ...
  JSVERIFY(FAIL,
           H5FD_s3comms_percent_encode_char(NULL, (const unsigned char)'^',
                                            &dest_len),
           NULL)
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_s3r_get_filesize(void) {
  ...
  FAIL_IF(S3_TEST_MAX_URL_SIZE < HDsnprintf(url_raven, S3_TEST_MAX_URL_SIZE,
                                            "%s/%s", s3_test_bucket_url,
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_s3r_open(void) {
  ...
  FAIL_IF(S3_TEST_MAX_URL_SIZE <
          HDsnprintf(url_shakespeare, S3_TEST_MAX_URL_SIZE, "%s/%s",
                     s3_test_bucket_url, S3_TEST_RESOURCE_TEXT_RESTRICTED));

  FAIL_IF(S3_TEST_MAX_URL_SIZE < HDsnprintf(url_missing, S3_TEST_MAX_URL_SIZE,
                                            "%s/%s", s3_test_bucket_url,
  ...
  FAIL_IF(S3_TEST_MAX_URL_SIZE <
          HDsnprintf(url_raven_badport, S3_TEST_MAX_URL_SIZE, "%s://%s:%s/%s",
   ...
   */
  FAIL_IF(FAIL == H5FD_s3comms_signing_key(
                      signing_key, (const char *)s3_test_aws_secret_access_key,
   ...
   */
  handle = H5FD_s3comms_s3r_open(url_missing, (const char *)s3_test_aws_region,
  ...
  printf(
      "Opening on inactive port may hang for a minute; waiting for timeout\n");
   ...
   */
  handle = H5FD_s3comms_s3r_open(url_raven, (const char *)s3_test_aws_region,
                                 (const char *)s3_test_aws_access_key_id,
                                 (const unsigned char *)signing_key,
                                 (const char *)s3_test_aws_security_token);
  FAIL_IF(handle == NULL);
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_s3r_read(void) {
...
#define S3COMMS_TEST_BUFFER_SIZE 256
  ...
  JSVERIFY(SUCCEED,
           H5FD_s3comms_s3r_read(handle, (haddr_t)0, (size_t)118, buffer), NULL)
  JSVERIFY_STR(
  ...
  JSVERIFY(SUCCEED,
           H5FD_s3comms_s3r_read(handle, (haddr_t)2540, (size_t)54, buffer),
           NULL)
  JSVERIFY_STR("the grave and stern decorum of the countenance it wore", buffer,
               NULL)
  ...
  JSVERIFY(SUCCEED,
           H5FD_s3comms_s3r_read(handle, (haddr_t)2540, (size_t)1, buffer),
           NULL)
  ...
  JSVERIFY(SUCCEED,
           H5FD_s3comms_s3r_read(handle, (haddr_t)6370, (size_t)0, buffer),
           NULL)
  JSVERIFY(0,
           HDstrncmp(buffer,
  ...
  JSVERIFY(FAIL,
           H5FD_s3comms_s3r_read(handle, (haddr_t)6400,
  ...
  JSVERIFY(FAIL,
           H5FD_s3comms_s3r_read(handle, (haddr_t)6464, (size_t)0, buffer),
           NULL)
  JSVERIFY(0, HDstrcmp("", buffer), NULL)
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_signing_key(void) {
  /*************************
   * test-local structures *
   *************************/
  ...
  for (i = 0; i < ncases; i++) {
    ...
    JSVERIFY(SUCCEED,
             H5FD_s3comms_signing_key(key, cases[i].secret_key, cases[i].region,
                                      cases[i].when),
             NULL)

    JSVERIFY(0,
             HDstrncmp((const char *)cases[i].exp, (const char *)key,
                       SHA256_DIGEST_LENGTH),
  ...
  JSVERIFY(FAIL,
           H5FD_s3comms_signing_key(NULL, cases[0].secret_key, cases[0].region,
                                    cases[0].when),
           "destination cannot be NULL")

  JSVERIFY(FAIL,
           H5FD_s3comms_signing_key(key, NULL, cases[0].region, cases[0].when),
           "secret key cannot be NULL")

  JSVERIFY(
      FAIL,
      H5FD_s3comms_signing_key(key, cases[0].secret_key, NULL, cases[0].when),
      "aws region cannot be NULL")

  JSVERIFY(
      FAIL,
      H5FD_s3comms_signing_key(key, cases[0].secret_key, cases[0].region, NULL),
 ...
 *---------------------------------------------------------------------------
 */
static herr_t test_tostringtosign(void) {
  /************************
   * test-local variables *
   ************************/
  ...
  JSVERIFY_STR("AWS4-HMAC-SHA256\n20130524T000000Z\n20130524/us-east-1/s3/"
               "aws4_"
               "request\n7344ae5b7ee6c3e7e6b0fe0640412a37625d1fbfff95c48bbb2dc4"
               "3964946972",
               s2s, NULL)
  ...
  JSVERIFY(FAIL, H5FD_s3comms_tostringtosign(s2s, canonreq, NULL, region),
           "time string cannot be NULL")

  JSVERIFY(FAIL, H5FD_s3comms_tostringtosign(s2s, canonreq, iso8601now, NULL),
           "aws region cannot be NULL")
...
static herr_t test_trim(void) {
  ...
  for (i = 0; i < n_cases; i++) {
    ...
    JSVERIFY(SUCCEED, H5FD_s3comms_trim(dest, str, cases[i].in_len, &dest_len),
             NULL)
 ...
 *----------------------------------------------------------------------------
 */
static herr_t test_uriencode(void) {
  /*************************
   * test-local structures *
   *************************/
  ...
  for (i = 0; i < ncases; i++) {
    ...
    JSVERIFY(SUCCEED,
             H5FD_s3comms_uriencode(dest, cases[i].str, str_len,
                                    cases[i].encode_slash, &dest_written),
             NULL);
    JSVERIFY(HDstrlen(cases[i].expected), dest_written, NULL)
    JSVERIFY(0, HDstrncmp(dest, cases[i].expected, dest_written),
             cases[i].expected);
...
int main(void) {
  ...
  if (SUCCEED == H5FD_s3comms_load_aws_profile(
                     S3_TEST_PROFILE_NAME, s3_test_aws_access_key_id,
