...
static unsigned open_file(char *filename, hid_t fapl, hsize_t page_size,
...
static unsigned create_file(char *filename, hid_t fcpl, hid_t fapl) {
  ...
  if ((grp_id = H5Gcreate2(file_id, "GROUP", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  for (i = 0; i < NUM_DSETS; i++) {
    ...
    if (H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
        0)
      FAIL_STACK_ERROR;
    if (H5Dclose(dset_id) < 0)
      FAIL_STACK_ERROR;
    ...
    if ((dset_id = H5Dopen2(grp_id, dset_name, H5P_DEFAULT)) < 0)
      FAIL_STACK_ERROR;
    if (H5Dread(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
        0)
      FAIL_STACK_ERROR;
    if (H5Dclose(dset_id) < 0)
      FAIL_STACK_ERROR;
 ...
 *
 */
static unsigned set_multi_split(const char *env_h5_drvr, hid_t fapl,
                                hsize_t pagesize) {
  hbool_t split = FALSE;
  hbool_t multi = FALSE;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_args(hid_t orig_fapl, const char *env_h5_drvr) {
...
static unsigned test_raw_data_handling(hid_t orig_fapl,
  ...
  if (HADDR_UNDEF ==
      (addr = H5MF_alloc(f, H5FD_MEM_DRAW, sizeof(int) * (size_t)num_elements)))
    FAIL_STACK_ERROR;
  ...
  /* read elements 800 - 1200, this should not affect the PB, and should read
   * -1s */
  ...
  if (H5FD_read(f->shared->lf, H5FD_MEM_DRAW, addr, sizeof(int) * 800, data) <
      0)
    FAIL_STACK_ERROR;
  i = 0;
  while (i < 800) {
...
static unsigned test_lru_processing(hid_t orig_fapl, const char *env_h5_drvr) {
  ...
  if (NULL == H5SL_search(f->shared->page_buf->slist_ptr, &(search_addr)))
    FAIL_STACK_ERROR;
  /* read elements 175 - 225, this should move 100 to the top, evict 600 and
   * bring in 200 */
...
static unsigned test_min_threshold(hid_t orig_fapl, const char *env_h5_drvr) {
  ...
  if (HADDR_UNDEF ==
      (meta_addr =
           H5MF_alloc(f, H5FD_MEM_SUPER, sizeof(int) * (size_t)num_elements)))
    FAIL_STACK_ERROR;

  if (HADDR_UNDEF ==
      (raw_addr =
           H5MF_alloc(f, H5FD_MEM_DRAW, sizeof(int) * (size_t)num_elements)))
    FAIL_STACK_ERROR;
  ...
  if (H5F_block_write(f, H5FD_MEM_SUPER, meta_addr, sizeof(int) * 100, data) <
      0)
    FAIL_STACK_ERROR;
  ...
  printf("\tMinimum metadata threshold = 40%%, Minimum rawdata threshold = "
         "40%%\n");
  page_count = 0;
...
static unsigned test_stats_collection(hid_t orig_fapl,
...

  {
    ...
    if (H5Fget_page_buffering_stats(file_id, accesses, hits, misses, evictions,
                                    bypasses) < 0)
      FAIL_STACK_ERROR;
...
int main(void) {
  ...
  if ((0 == HDstrcmp(env_h5_drvr, "multi")) ||
    ...
    SKIPPED();
    HDputs("Skip page buffering test because paged aggregation is disabled for "
           "multi/split drivers");
    exit(EXIT_SUCCESS);
  } /* end if */
