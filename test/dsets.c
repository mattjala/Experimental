...
#define BOGUS2_PARAM_2 35 /* (No particular meaning, just for checking value)  \
                           */
#define BOGUS2_ALL_NPARMS                                                      \
  4 /* Total number of parameter = permanent + "local" parameters */
...
#define NOENCODER_SZIP_SHUFF_FLETCH_DATASET                                    \
  "noencoder_szip_shuffle_fletcher_dset.h5"
#endif /* H5_HAVE_FILTER_SZIP */
 ...
 *-------------------------------------------------------------------------
 */
static size_t filter_count(unsigned int flags, size_t H5_ATTR_UNUSED cd_nelmts,
                           const unsigned int H5_ATTR_UNUSED *cd_values,
                           size_t nbytes, size_t H5_ATTR_UNUSED *buf_size,
...
static herr_t test_create(hid_t file) {
  ...
  status = H5Oset_comment_by_name(file, DSET_DEFAULT_NAME, "This is a dataset",
                                  H5P_DEFAULT);
  if (status < 0)
    goto error;
  ...
  dataset = H5Dcreate2(file, DSET_COMPACT_NAME, H5T_NATIVE_DOUBLE, small_space,
                       H5P_DEFAULT, create_parms, H5P_DEFAULT);
  if (dataset < 0)
    goto error;
  H5Pclose(create_parms);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_simple_io(const char *env_h5_drvr, hid_t fapl) {
  ...
  /* Can't run this test with multi-file VFDs because of HDopen/read/seek the
   * file directly */
  if (HDstrcmp(env_h5_drvr, "split") != 0 &&
    ...
    /* Set up data array */
    if (NULL ==
        (rdata_bytes = (int *)calloc(DSET_DIM1 * DSET_DIM2, sizeof(int))))
      TEST_ERROR;
    ...
    if (H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, xfer, points_data) <
        0)
      goto error;
    ...
    if (H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, xfer, check_data) <
        0)
      goto error;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_userblock_offset(const char *env_h5_drvr, hid_t fapl,
                                    hbool_t new_format) {
  char filename[FILENAME_BUF_SIZE];
  ...
  if (HDstrcmp(env_h5_drvr, "split") != 0 &&
    ...
    /* Create the dataset */
    if ((dataset = H5Dcreate2(file, DSET_USERBLOCK_IO_NAME, H5T_NATIVE_INT,
...
static herr_t test_compact_io(hid_t fapl) {
  ...
  if ((dataset = H5Dcreate2(file, DSET_COMPACT_IO_NAME, H5T_NATIVE_INT, space,
  ...
  if (H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wbuf) <
      0)
    TEST_ERROR;
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      if ((verfile =
               H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, new_fapl)) < 0)
        TEST_ERROR;

      /* Create the compact dataset */
      dataset = H5Dcreate2(verfile, DSET_DEFAULT_NAME, H5T_NATIVE_INT, space,
      ...
      if (fp->shared->low_bound == H5F_LIBVER_EARLIEST) {
        VERIFY(dsetp->shared->layout.version, H5O_LAYOUT_VERSION_DEFAULT,
               "layout_ver_bounds");
        VERIFY(dsetp->shared->dcpl_cache.fill.version, H5O_FILL_VERSION_2,
               "fill_ver_bounds");
      } else {
        VERIFY(dsetp->shared->layout.version,
               H5O_layout_ver_bounds[fp->shared->low_bound],
               "layout_ver_bounds");
        VERIFY(dsetp->shared->dcpl_cache.fill.version,
               H5O_fill_ver_bounds[fp->shared->low_bound], "fill_ver_bounds");
      }
...
static herr_t test_max_compact(hid_t fapl) {
  ...
  /* Create and write to a compact dataset */
  if ((dataset = H5Dcreate2(file, DSET_COMPACT_MAX_NAME, H5T_NATIVE_INT, space,
                            H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create and write to a compact dataset */
  H5E_BEGIN_TRY {
    H5Dcreate2(file, DSET_COMPACT_MAX2_NAME, H5T_NATIVE_INT, space, H5P_DEFAULT,
               plist, H5P_DEFAULT);
  }
  H5E_END_TRY
...
static herr_t test_layout_extend(hid_t fapl) {
  ...
  hid_t dcpl_compact = -1, dcpl_contig = -1,
        dcpl_chunked = -1;              /* Dataset creation property list id */
  hid_t did_fixed = -1, did_unlim = -1; /* Dataset id */
  hsize_t cur_size[1] = {10};           /* Current size of dataspace */
  hsize_t max_unlim[1] = {
      H5S_UNLIMITED};          /* Maximum size of dataspace (unlimited) */
  ...
  H5E_BEGIN_TRY {
    if (H5Dcreate2(fid, "compact", H5T_NATIVE_INT, sid_fix, H5P_DEFAULT,
                   dcpl_compact, H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
   ...
   */
  H5E_BEGIN_TRY {
    if (H5Dcreate2(fid, "compact", H5T_NATIVE_INT, sid_unlim, H5P_DEFAULT,
                   dcpl_compact, H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  /* Create dataset with extendible dataspace (fixed max_dims) should fail */
  H5E_BEGIN_TRY {
    if (H5Dcreate2(fid, "contig", H5T_NATIVE_INT, sid_fix, H5P_DEFAULT,
                   dcpl_contig, H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  /* Create dataset with extendible dataspace (unlimited max_dims) should fail*/
  H5E_BEGIN_TRY {
    if (H5Dcreate2(fid, "contig", H5T_NATIVE_INT, sid_unlim, H5P_DEFAULT,
                   dcpl_contig, H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  if ((did_fixed = H5Dcreate2(fid, "chunked_fixed", H5T_NATIVE_INT, sid_fix,
                              H5P_DEFAULT, dcpl_chunked, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Create dataset with extendible dataspace (unlimited max_dims) should
   * succeed */
  if ((did_unlim = H5Dcreate2(fid, "chunked_unlim", H5T_NATIVE_INT, sid_unlim,
                              H5P_DEFAULT, dcpl_chunked, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static herr_t test_conv_buffer(hid_t fid) {
  ...
  hid_t arr_type1 = H5I_INVALID_HID, arr_type2 = H5I_INVALID_HID,
        arr_type3 = H5I_INVALID_HID, arr_type4 = H5I_INVALID_HID,
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(fid, DSET_CONV_BUF_NAME, ctype1, space, H5P_DEFAULT,
 ...
 *-------------------------------------------------------------------------
 */
static htri_t can_apply_bogus(hid_t H5_ATTR_UNUSED dcpl_id, hid_t type_id,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t set_local_bogus2(hid_t dcpl_id, hid_t type_id,
                               hid_t H5_ATTR_UNUSED space_id) {
  ...
  if (H5Pget_filter_by_id2(dcpl_id, H5Z_FILTER_SET_LOCAL_TEST, &flags,
  ...
  if (H5Pmodify_filter(dcpl_id, H5Z_FILTER_SET_LOCAL_TEST, flags,
                       (size_t)BOGUS2_ALL_NPARMS, cd_values) < 0)
    return (FAIL);
...
static size_t filter_corrupt(unsigned int flags, size_t cd_nelmts,
  ...
  if (flags &
      H5Z_FLAG_REVERSE) { /* Verify data is actually corrupted during read */
    dst += offset;
 ...
 *-------------------------------------------------------------------------
 */
static H5Z_cb_return_t filter_cb_cont(H5Z_filter_t filter,
                                      void H5_ATTR_UNUSED *buf,
                                      size_t H5_ATTR_UNUSED buf_size,
 ...
 *-------------------------------------------------------------------------
 */
static H5Z_cb_return_t filter_cb_fail(H5Z_filter_t filter,
                                      void H5_ATTR_UNUSED *buf,
                                      size_t H5_ATTR_UNUSED buf_size,
                                      void H5_ATTR_UNUSED *op_data) {
  if (H5Z_FILTER_FLETCHER32 == filter)
...
static herr_t test_filter_internal(hid_t fid, const char *name, hid_t dcpl,
                                   int if_fletcher32, int corrupted,
                                   hsize_t *dset_size) {
  ...
  const hsize_t hs_offset[2] = {FILTER_HS_OFFSET1,
                                FILTER_HS_OFFSET2}; /* Hyperslab offset */
  const hsize_t hs_size[2] = {FILTER_HS_SIZE1,
                              FILTER_HS_SIZE2}; /* Hyperslab size */
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(fid, name, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                            H5P_DEFAULT)) < 0)
    goto error;
  ...
  if (H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, write_dxpl,
               points_data) < 0)
    TEST_ERROR;
  ...
  if (corrupted) {
    ...
    H5E_BEGIN_TRY {
      status = H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, write_dxpl,
                       check_data);
    }
    H5E_END_TRY
    if (status >= 0)
    ...
    if (H5Pset_filter_callback(dxpl, filter_cb_cont, NULL) < 0)
      TEST_ERROR;
    if (H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl, check_data) <
        0)
      TEST_ERROR;
  ...
  for (i = 0; i < (size_t)hs_size[0]; i++) {
    for (j = 0; j < (size_t)hs_size[1]; j++) {
      points[(size_t)hs_offset[0] + i][(size_t)hs_offset[1] + j] =
          (int)HDrandom();
    }
  }
  if (H5Sselect_hyperslab(sid, H5S_SELECT_SET, hs_offset, NULL, hs_size, NULL) <
      0)
    TEST_ERROR;
  /* (Use the "read" DXPL because partial I/O on corrupted data test needs to
...
static herr_t test_filter_noencoder(const char *dset_name) {
  ...
  err = H5Dread(dset_id, H5T_NATIVE_INT, space_id, space_id, H5P_DEFAULT,
                read_buf);
  if (err < 0)
    goto error;
  ...
  H5E_BEGIN_TRY {
    test_dset_id = H5Dcreate2(file_id, NOENCODER_TEST_DATASET, H5T_NATIVE_INT,
   ...
   */
  H5E_BEGIN_TRY {
    err = H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   test_ints);
  }
  H5E_END_TRY
...
static herr_t test_filters(hid_t file) {
  ...
  const hsize_t chunk_size[2] = {FILTER_CHUNK_DIM1,
                                 FILTER_CHUNK_DIM2}; /* Chunk dimensions */
  ...
  if (test_filter_internal(file, DSET_BOGUS_NAME, dc, DISABLE_FLETCHER32,
                           DATA_NOT_CORRUPTED, &null_size) < 0)
    goto error;
  ...
  if (test_filter_internal(file, DSET_FLETCHER32_NAME, dc, ENABLE_FLETCHER32,
                           DATA_NOT_CORRUPTED, &fletcher32_size) < 0)
    goto error;
  ...
  if (test_filter_internal(file, DSET_FLETCHER32_NAME_2, dc, DISABLE_FLETCHER32,
                           DATA_NOT_CORRUPTED, &fletcher32_size) < 0)
    goto error;
  if (fletcher32_size <= null_size) {
    H5_FAILED();
  ...
  if (H5Pset_filter(dc, H5Z_FILTER_CORRUPT, 0, (size_t)3, data_corrupt) < 0)
    goto error;
  if (test_filter_internal(file, DSET_FLETCHER32_NAME_3, dc, DISABLE_FLETCHER32,
                           DATA_CORRUPTED, &fletcher32_size) < 0)
    goto error;
  if (fletcher32_size <= null_size) {
    H5_FAILED();
    HDputs("    Size after checksumming is incorrect.");
  ...
  if (test_filter_internal(file, DSET_DEFLATE_NAME, dc, DISABLE_FLETCHER32,
                           DATA_NOT_CORRUPTED, &deflate_size) < 0)
    goto error;
  /* Clean up objects used for this test */
  if (H5Pclose(dc) < 0)
    goto error;
  ...
  if (h5_szip_can_encode() == 1) {
    ...
    if (H5Pset_szip(dc, szip_options_mask, szip_pixels_per_block) < 0)
      goto error;
    if (test_filter_internal(file, DSET_SZIP_NAME, dc, DISABLE_FLETCHER32,
                             DATA_NOT_CORRUPTED, &szip_size) < 0)
      goto error;
    if (H5Pclose(dc) < 0)
      goto error;
  } else {
  ...
  if (test_filter_internal(file, DSET_SHUFFLE_NAME, dc, DISABLE_FLETCHER32,
                           DATA_NOT_CORRUPTED, &shuffle_size) < 0)
    goto error;
  ...
  if (test_filter_internal(file, DSET_SHUF_DEF_FLET_NAME, dc, ENABLE_FLETCHER32,
                           DATA_NOT_CORRUPTED, &combo_size) < 0)
    goto error;
  ...
  if (test_filter_internal(file, DSET_SHUF_DEF_FLET_NAME_2, dc,
                           ENABLE_FLETCHER32, DATA_NOT_CORRUPTED,
                           &combo_size) < 0)
    goto error;
  ...
  if (h5_szip_can_encode() == 1) {
    ...
    if (H5Pset_szip(dc, szip_options_mask, szip_pixels_per_block) < 0)
      goto error;
    if (test_filter_internal(file, DSET_SHUF_SZIP_FLET_NAME, dc,
                             ENABLE_FLETCHER32, DATA_NOT_CORRUPTED,
                             &combo_size) < 0)
      goto error;
  } else {
  ...
  if (h5_szip_can_encode() == 1) {
    ...
    if (test_filter_internal(file, DSET_SHUF_SZIP_FLET_NAME_2, dc,
                             ENABLE_FLETCHER32, DATA_NOT_CORRUPTED,
                             &combo_size) < 0)
      goto error;
...
static herr_t test_missing_filter(hid_t file) {
  ...
  herr_t ret;                                     /* Generic return value */
  const char *testfile =
      H5_get_srcdir_filename(FILE_DEFLATE_NAME); /* Corrected test file name */
  ...
  /* Create new dataset */
  if ((dsid = H5Dcreate2(file, DSET_MISSING_NAME, H5T_NATIVE_INT, sid,
  ...
  /* Write data */
  if (H5Dwrite(dsid, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               points_data) < 0) {
    H5_FAILED();
  ...
  /* Read data */
  if (H5Dread(dsid, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, check_data) <
      0) {
    H5_FAILED();
...
static herr_t test_onebyte_shuffle(hid_t file) {
  ...
  if ((dataset = H5Dcreate2(file, DSET_ONEBYTE_SHUF_NAME, H5T_NATIVE_UCHAR,
                            space, H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;
...
static herr_t test_nbit_int(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_INT_NAME, datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 0; i < (size_t)size[0]; i++)
    for (j = 0; j < (size_t)size[1]; j++) {
      ...
      orig_data[i][j] =
          (int)(((long long)HDrandom() % (long long)power) << offset);
  ...
  /* Read the dataset back */
  if (H5Dread(dataset, mem_datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, new_data) <
      0)
    goto error;
  ...
  for (i = 0; i < (size_t)size[0]; i++) {
    for (j = 0; j < (size_t)size[1]; j++) {
      if (((unsigned)new_data[i][j] & mask) !=
          ((unsigned)orig_data[i][j] & mask)) {
        H5_FAILED();
        printf("    Read different values than written.\n");
        printf("    At index %lu,%lu\n", (unsigned long)i, (unsigned long)j);
        goto error;
...
static herr_t test_nbit_float(hid_t file) {
  ...
  /* orig_data[] are initialized to be within the range that can be represented
   * by dataset datatype (no precision loss during datatype conversion)
   */
  float orig_data[2][5] = {
      {188384.0F, 19.103516F, -1.0831790e9F, -84.242188F, 5.2045898F},
  ...
  if (H5Tset_fields(datatype, (size_t)26, (size_t)20, (size_t)6, (size_t)7,
                    (size_t)13) < 0)
    FAIL_STACK_ERROR;
  offset = 7;
  if (H5Tset_offset(datatype, offset) < 0)
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_FLOAT_NAME, datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  PASSED();
...
static herr_t test_nbit_double(hid_t file) {
  ...
  double orig_data[2][5] = {
      {(double)1.6081706885101836e+60L, -255.32099170994480,
  ...
  if (H5Tset_fields(datatype, (size_t)55, (size_t)46, (size_t)9, (size_t)5,
                    (size_t)41) < 0)
    FAIL_STACK_ERROR;
  offset = 5;
  if (H5Tset_offset(datatype, offset) < 0)
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_DOUBLE_NAME, datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static herr_t test_nbit_array(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_ARRAY_NAME, array_datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;
    ...
    for (j = 0; j < (size_t)size[1]; j++)
      for (m = 0; m < (size_t)adims[0]; m++)
        for (n = 0; n < (size_t)adims[1]; n++) {
          ...
          orig_data[i][j][m][n] =
              (unsigned int)(((long long)HDrandom() % (long long)power)
                             << offset);
        } /* end for */
  PASSED();
        ...
        for (n = 0; n < (size_t)adims[1]; n++) {
          if (new_data[i][j][m][n] != orig_data[i][j][m][n]) {
            ...
            printf("    At index %lu,%lu,%lu,%lu\n", (unsigned long)i,
                   (unsigned long)j, (unsigned long)m, (unsigned long)n);
            goto error;
          }
        }
...
static herr_t test_nbit_compound(hid_t file) {
  ...
  if (H5Tset_fields(f_tid, (size_t)26, (size_t)20, (size_t)6, (size_t)7,
                    (size_t)13) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_COMPOUND_NAME, cmpd_tid, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Read the dataset back */
  if (H5Dread(dataset, mem_cmpd_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, new_data) <
      0)
    FAIL_STACK_ERROR;
   ...
   * Use mask for checking the significant bits, ignoring the padding bits
   */
  i_mask = ~((unsigned)~0 << (precision[0] + offset[0])) &
           ((unsigned)~0 << offset[0]);
  c_mask = ~((unsigned)~0 << (precision[1] + offset[1])) &
           ((unsigned)~0 << offset[1]);
  s_mask = ~((unsigned)~0 << (precision[2] + offset[2])) &
           ((unsigned)~0 << offset[2]);
  for (i = 0; i < size[0]; i++) {
    for (j = 0; j < size[1]; j++) {
      if (((unsigned)new_data[i][j].i & i_mask) !=
              ((unsigned)orig_data[i][j].i & i_mask) ||
          ((unsigned)new_data[i][j].c & c_mask) !=
              ((unsigned)orig_data[i][j].c & c_mask) ||
          ((unsigned)new_data[i][j].s & s_mask) !=
              ((unsigned)orig_data[i][j].s & s_mask) ||
          (!HDisnan(orig_data[i][j].f) &&
...
static herr_t test_nbit_compound_2(hid_t file) {
  ...
  if ((array_cmplx_tid = H5Tarray_create2(cmpd_tid1, 2, array_dims)) < 0)
    FAIL_STACK_ERROR;
  if ((mem_array_cmplx_tid = H5Tarray_create2(mem_cmpd_tid1, 2, array_dims)) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Tinsert(mem_cmpd_tid2, "b", HOFFSET(complex, b), array_tid) < 0)
    FAIL_STACK_ERROR;
  if (H5Tinsert(mem_cmpd_tid2, "d", HOFFSET(complex, d), mem_array_cmplx_tid) <
      0)
    FAIL_STACK_ERROR;
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_COMPOUND_NAME_2, cmpd_tid2, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
    ...
    for (j = 0; j < (size_t)size[1]; j++) {
      ...
      for (m = 0; m < (size_t)array_dims[0]; m++)
        for (n = 0; n < (size_t)array_dims[1]; n++) {
          ...
          orig_data[i][j].b[m][n] =
              (char)(((long long)HDrandom() % (long long)power) << offset[4]);
        } /* end for */
  ...
  /* Read the dataset back */
  if (H5Dread(dataset, mem_cmpd_tid2, H5S_ALL, H5S_ALL, H5P_DEFAULT, new_data) <
      0)
    FAIL_STACK_ERROR;
   ...
   */
  /* The original code
   *   i_mask = ~((unsigned)~0 << (precision[0] + offset[0])) & ((unsigned)~0 <<
   * offset[0]); left shift a 32-bit integer for 32-bit.  The result is
   * undefined by C language.  A user discovered it using clang compiler with
   * -fcatch-undefined-behavior option (see Issue 7674 in Jira).  So I changed
   * it in a funny way as below to avoid it. SLU - 2011/8/11
   */
  ...
  s_mask = ~((unsigned)~0 << (precision[2] + offset[2])) &
           ((unsigned)~0 << offset[2]);
  b_mask = ~((unsigned)~0 << (precision[4] + offset[4])) &
           ((unsigned)~0 << offset[4]);
  for (i = 0; i < (size_t)size[0]; i++) {
    for (j = 0; j < (size_t)size[1]; j++) {
      b_failed = 0;
        ...
        for (n = 0; n < (size_t)array_dims[1]; n++)
          if (((unsigned)new_data[i][j].d[m][n].i & i_mask) !=
              ...
              (!HDisnan(new_data[i][j].d[m][n].f) &&
               !H5_FLT_ABS_EQUAL(new_data[i][j].d[m][n].f,
                                 new_data[i][j].d[m][n].f))) {
            d_failed = 1;
            goto out;
          }

    out:
      if (((unsigned)new_data[i][j].a.i & i_mask) !=
              ((unsigned)orig_data[i][j].a.i & i_mask) ||
          ((unsigned)new_data[i][j].a.c & c_mask) !=
              ((unsigned)orig_data[i][j].a.c & c_mask) ||
          ((unsigned)new_data[i][j].a.s & s_mask) !=
              ((unsigned)orig_data[i][j].a.s & s_mask) ||
          (!HDisnan(new_data[i][j].a.f) &&
...
static herr_t test_nbit_compound_3(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_COMPOUND_NAME_3, cmpd_tid, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;

  /* Create the dataset object reference points to */
  if ((obj_ref_dataset = H5Dcreate2(file, "nbit_obj_ref", H5T_NATIVE_INT, space,
                                    H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 0; i < (size_t)size[0]; i++) {
    ...
    if (H5Rcreate(&orig_data[i].r, file, "nbit_obj_ref", H5R_OBJECT,
                  (hid_t)-1) < 0)
      goto error;
  ...
  for (i = 0; i < (size_t)size[0]; i++) {
    if (new_data[i].i != orig_data[i].i ||
        HDstrcmp(new_data[i].str, orig_data[i].str) != 0 ||
    ...
    for (k = 0; k < i + 1; k++)
      if (((unsigned int *)orig_data[i].v.p)[k] !=
          ((unsigned int *)new_data[i].v.p)[k]) {
        H5_FAILED();
        printf("    Read different values than written.\n");
        printf("    At index %lu\n", (unsigned long)i);
        goto error;
...
static herr_t test_nbit_int_size(hid_t file) {
   ...
   */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_INT_SIZE_NAME, datatype, dataspace,
  ...
  /*
   * The size of the dataset after compression should around 2 * DSET_DIM1 *
   * DSET_DIM2
   */
  if ((dset_size = H5Dget_storage_size(dataset)) <
          DSET_DIM1 * DSET_DIM2 * (precision / 8) ||
...
static herr_t test_nbit_flt_size(hid_t file) {
  ...
  /* Set up data array */
  if (NULL ==
      (orig_data = (float *)calloc(DSET_DIM1 * DSET_DIM2, sizeof(float))))
    TEST_ERROR;
   ...
   * Create a new dataset within the file.
   */
  if ((dataset = H5Dcreate2(file, DSET_NBIT_FLT_SIZE_NAME, datatype, dataspace,
                            H5P_DEFAULT, dset_create_props, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    line %d: H5dwrite failed\n", __LINE__);
    goto error;
...
static herr_t test_scaleoffset_int(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_SCALEOFFSET_INT_NAME, datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;
...
static herr_t test_scaleoffset_int_2(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_SCALEOFFSET_INT_NAME_2, datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;
  ...
  block[0] = 1;
  block[1] = 5;
  if (H5Sselect_hyperslab(mspace, H5S_SELECT_SET, start, stride, count, block) <
      0)
    goto error;
  ...
  /* only data in the hyperslab will be written, other value should be fill
   * value */
  ...
  /* Read the dataset back */
  if (H5Dread(dataset, H5T_NATIVE_INT, mspace, mspace, H5P_DEFAULT, new_data) <
      0)
    goto error;
...
static herr_t test_scaleoffset_float(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_SCALEOFFSET_FLOAT_NAME, datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 0; i < (size_t)size[0]; i++) {
    for (j = 0; j < (size_t)size[1]; j++) {
      if (HDfabs((double)(new_data[i][j] - orig_data[i][j])) >
          HDpow(10.0, -3.0)) {
        H5_FAILED();
        printf("    Read different values than written.\n");
        printf("    At index %lu,%lu\n", (unsigned long)i, (unsigned long)j);
...
static herr_t test_scaleoffset_float_2(hid_t file) {
  ...
  /* Check that the values read are the same as the values written */
  for (j = 0; j < (size_t)size[1]; j++) {
    if (HDfabs((double)(new_data[0][j] - orig_data[0][j])) >
        HDpow(10.0, -3.0)) {
      H5_FAILED();
      printf("    Read different values than written.\n");
      printf("    At index %lu,%lu\n", (unsigned long)0, (unsigned long)j);
...
static herr_t test_scaleoffset_double(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_SCALEOFFSET_DOUBLE_NAME, datatype, space,
                            H5P_DEFAULT, dc, H5P_DEFAULT)) < 0)
    goto error;
...
static herr_t test_multiopen(hid_t file) {
  ...
  if ((space = H5Screate_simple(1, cur_size, max_size)) < 0)
    goto error;
  if ((dset1 = H5Dcreate2(file, "multiopen", H5T_NATIVE_INT, space, H5P_DEFAULT,
                          dcpl, H5P_DEFAULT)) < 0)
    goto error;
...
static herr_t test_types(hid_t file) {
  ...
  if ((grp = H5Gcreate2(file, "typetests", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto error;
  ...
  if ((type = H5Tcopy(H5T_STD_B8LE)) < 0 ||
      ...
      (dset = H5Dcreate2(grp, "bitfield_1", type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto error;
  ...
  if ((type = H5Tcopy(H5T_STD_B16LE)) < 0 ||
      (space = H5Screate_simple(1, &nelmts, NULL)) < 0 ||
      (dset = H5Dcreate2(grp, "bitfield_2", type, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    goto error;
  for (i = 0; i < sizeof buf; i++)
    buf[i] = (unsigned char)(0xff ^ i);
  ...
  if ((type = H5Tcreate(H5T_OPAQUE, (size_t)1)) < 0 ||
      ...
      (space = H5Screate_simple(1, &nelmts, NULL)) < 0 ||
      (dset = H5Dcreate2(grp, "opaque_1", type, space, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    goto error;
  for (i = 0; i < sizeof buf; i++)
    buf[i] = (unsigned char)(0xff ^ i);
  ...
  if ((type = H5Tcreate(H5T_OPAQUE, (size_t)4)) < 0 ||
      ...
      (space = H5Screate_simple(1, &nelmts, NULL)) < 0 ||
      (dset = H5Dcreate2(grp, "opaque_2", type, space, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    goto error;
  for (i = 0; i < sizeof buf; i++)
    buf[i] = (unsigned char)(0xff ^ i);
...
static herr_t test_can_apply(hid_t file) {
  ...
  H5E_BEGIN_TRY {
    dsid = H5Dcreate2(file, DSET_CAN_APPLY_NAME, H5T_NATIVE_DOUBLE, sid,
  ...
  H5E_BEGIN_TRY {
    dsid = H5Dcreate2(file, DSET_CAN_APPLY_NAME, H5T_NATIVE_FLOAT, sid,
                      H5P_DEFAULT, dcpl, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (dsid >= 0) {
  ...
  /* Create new dataset */
  if ((dsid = H5Dcreate2(file, DSET_CAN_APPLY_NAME, H5T_NATIVE_INT, sid,
                         H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    Line %d: Can't create dataset\n", __LINE__);
    goto error;
...
static herr_t test_can_apply2(hid_t file) {
  ...
  if (H5Pset_filter(dcpl, H5Z_FILTER_CAN_APPLY_TEST2, H5Z_FLAG_OPTIONAL,
                    (size_t)0, NULL) < 0) {
    H5_FAILED();
    printf("    Line %d: Can't set bogus filter\n", __LINE__);
    goto error;
  ...
  /* Create new dataset */
  if ((dsid = H5Dcreate2(file, DSET_CAN_APPLY_NAME2, H5T_NATIVE_DOUBLE, sid,
                         H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    Line %d: Can't create dataset\n", __LINE__);
    goto error;
...
static herr_t test_optional_filters(hid_t file) {
  ...
  /* The filter is optional. */
  if (H5Pset_filter(dcplid, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, cd_nelmts,
                    cd_values) < 0)
    TEST_ERROR;

  /* Create dataset with optional filter */
  if ((dsid = H5Dcreate2(file, DSET_OPTIONAL_SCALAR, strtid, sid, H5P_DEFAULT,
                         dcplid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Create dataset with optional filter */
  if ((dsid = H5Dcreate2(file, DSET_OPTIONAL_VLEN, vlentid, sid, H5P_DEFAULT,
                         dcplid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_can_apply_szip(hid_t
  ...
  if (h5_szip_can_encode() == 1) {
    ...
    H5E_BEGIN_TRY {
      dsid = H5Dcreate2(file, DSET_CAN_APPLY_SZIP_NAME, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl, H5P_DEFAULT);
    }
    H5E_END_TRY
    if (dsid <= 0) {
    ...
    /* Create new dataset */
    /* (Should fail because the 'can apply' filter should indicate inappropriate
     * combination) */
    H5E_BEGIN_TRY {
      dsid = H5Dcreate2(file, DSET_CAN_APPLY_SZIP_NAME, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl, H5P_DEFAULT);
...
static herr_t test_set_local(hid_t fapl) {
  ...
  const hsize_t chunk_dims[2] = {2, 25};          /* Chunk dimensions */
  hsize_t dset_size;                              /* Dataset size */
  unsigned cd_values[2] = {BOGUS2_PARAM_1,
                           BOGUS2_PARAM_2}; /* Parameters for Bogus2 filter */
  size_t i, j;                              /* Local index variables */
  ...
  if (H5Pset_filter(dcpl, H5Z_FILTER_SET_LOCAL_TEST, 0,
                    (size_t)BOGUS2_PERM_NPARMS, cd_values) < 0) {
    H5_FAILED();
  ...
  /* Create new dataset */
  if ((dsid = H5Dcreate2(file, DSET_SET_LOCAL_NAME, H5T_NATIVE_INT, sid,
                         H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    Line %d: Can't create dataset\n", __LINE__);
    goto error;
  ...
  if ((dsid = H5Dcreate2(file, DSET_SET_LOCAL_NAME_2, H5T_NATIVE_DOUBLE, sid,
                         H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
    H5_FAILED();
    printf("    Line %d: Can't create dataset\n", __LINE__);
    goto error;
  ...
  /* Write data */
  if (H5Dwrite(dsid, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               points_dbl_data) < 0) {
    H5_FAILED();
    printf("    Line %d: Error writing dataset data\n", __LINE__);
    goto error;
  ...
  /* Read data */
  if (H5Dread(dsid, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              check_dbl_data) < 0) {
    H5_FAILED();
    printf("    Line %d: Error reading dataset data\n", __LINE__);
    goto error;
...
static herr_t test_compare_dcpl(hid_t file) {
  ...
  hid_t dcpl1 = (-1),
        dcpl2 = (-1); /* Dataspace creation property list IDs from datasets */
  const hsize_t dims[2] = {500, 4096};       /* Dataspace dimensions */
  const hsize_t chunk_dims[2] = {250, 2048}; /* Chunk dimensions */
  ...
  if ((dsid = H5Dcreate2(file, DSET_COMPARE_DCPL_NAME, H5T_NATIVE_INT, sid,
                         H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((dsid = H5Dcreate2(file, DSET_COMPARE_DCPL_NAME_2, H5T_NATIVE_INT, sid,
                         H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_copy_dcpl(hid_t file, hid_t fapl) {
  ...
  hid_t dcpl1_copy = (-1),
        dcpl2_copy = (-1); /* Copies of creation property list IDs */
  const hsize_t dims[2] = {500, 4096};       /* Dataspace dimensions */
  const hsize_t chunk_dims[2] = {250, 2048}; /* Chunk dimensions */
  char filename[FILENAME_BUF_SIZE];
  hid_t new_file = (-1);
  ...
  if ((dsid1 = H5Dcreate2(file, DSET_COPY_DCPL_NAME_1, H5T_NATIVE_INT, sid,
                          H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Change the DCPL for contiguous layout with external storage.  The size of
   * the reserved space in the external file is the size of the dataset -
   * 500*4096*sizeof(int). There's no need to clean up the external file since
   * the library doesn't create it until the data is written to it. */
  if (H5Pset_layout(dcpl, H5D_CONTIGUOUS) < 0)
    TEST_ERROR;
  if (H5Premove_filter(dcpl, H5Z_FILTER_FLETCHER32) < 0)
    TEST_ERROR;
  if (H5Pset_external(dcpl, COPY_DCPL_EXTFILE_NAME, (off_t)0,
                      (hsize_t)(500 * 4096 * sizeof(int))) < 0)
    TEST_ERROR;

  /* Create second dataset of contiguous layout with external storage */
  if ((dsid2 = H5Dcreate2(file, DSET_COPY_DCPL_NAME_2, H5T_NATIVE_INT, sid,
                          H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Create a second file and create 2 datasets with the copies of the DCPLs in
   * the first file.  Test whether the copies of DCPLs work. */
  h5_fixname(FILENAME[13], fapl, filename, sizeof filename);
  if ((new_file =
           H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  if ((new_dsid1 = H5Dcreate2(new_file, DSET_COPY_DCPL_NAME_1, H5T_NATIVE_INT,
                              sid, H5P_DEFAULT, dcpl1_copy, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  if ((new_dsid2 = H5Dcreate2(new_file, DSET_COPY_DCPL_NAME_2, H5T_NATIVE_INT,
                              sid, H5P_DEFAULT, dcpl2_copy, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_filter_delete(hid_t file) {
  ...
  /* create a dataset */
  if ((dsid = H5Dcreate2(file, "dsetdel", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    goto error;
  ...
  for (i = 0; i < nfilters; i++) {
    filtn = H5Pget_filter2(dcpl1, (unsigned)i, NULL, NULL, NULL, (size_t)0,
                           NULL, NULL);
  ...
  H5E_BEGIN_TRY {
    ret = H5Pget_filter_by_id2(dcpl1, H5Z_FILTER_DEFLATE, &flags, NULL, NULL,
                               (size_t)0, NULL, NULL);
  }
  H5E_END_TRY
  if (ret >= 0) {
...
static herr_t test_filters_endianess(void) {
  ...
  hid_t sid = -1;  /* dataspace ID */
  hid_t dcpl = -1; /* dataset creation property list ID */
  const char *data_file = H5_get_srcdir_filename(
      "test_filters_le.h5"); /* Corrected test file name */
  ...
  data_file = H5_get_srcdir_filename(
      "test_filters_be.h5"); /* Corrected test file name */
...
static herr_t test_zero_dims(hid_t file) {
  ...
  H5E_BEGIN_TRY {
    d = H5Dcreate2(file, ZERODIM_DATASET, H5T_NATIVE_INT, s, H5P_DEFAULT, dcpl,
                   H5P_DEFAULT);
  }
  H5E_END_TRY
  if (d > 0) {
  ...
  if (H5Pset_chunk(dcpl, 1, &csize) < 0)
    FAIL_STACK_ERROR;
  if ((d = H5Dcreate2(file, ZERODIM_DATASET, H5T_NATIVE_INT, s, H5P_DEFAULT,
                      dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dwrite(d, H5T_NATIVE_INT, s, s, H5P_DEFAULT, NULL) < 0)
    FAIL_STACK_ERROR;
  if (H5Dwrite(d, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, (void *)911) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dread(d, H5T_NATIVE_INT, s, s, H5P_DEFAULT, NULL) < 0)
    FAIL_STACK_ERROR;
  if (H5Dread(d, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, (void *)911) <
      0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    d2 = H5Dcreate2(file, ZERODIM_DATASET2, H5T_NATIVE_INT, s2, H5P_DEFAULT,
                    dcpl2, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (d2 > 0) {
  ...
  /* Create the dataset */
  if ((d2 = H5Dcreate2(file, ZERODIM_DATASET2, H5T_NATIVE_INT, s2, H5P_DEFAULT,
                       dcpl2, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static herr_t test_missing_chunk(hid_t file) {
  ...
  hsize_t hs_start[1], hs_stride[1], hs_count[1],
      hs_block[1]; /* Hyperslab setting */
  hsize_t hs_start2[2], hs_stride2[2], hs_count2[2],
      hs_block2[2]; /* Hyperslab setting */
  ...
  if (NULL == (wdata2_bytes = (int *)calloc(
                   MISSING_CHUNK_DIM * MISSING_CHUNK_DIM, sizeof(int))))
    TEST_ERROR;
  if (NULL ==
      (wdata2 = (int **)calloc(MISSING_CHUNK_DIM, sizeof(wdata2_bytes))))
    TEST_ERROR;
  for (i = 0; i < MISSING_CHUNK_DIM; i++)
    wdata2[i] = wdata2_bytes + (i * MISSING_CHUNK_DIM);

  if (NULL == (rdata2_bytes = (int *)calloc(
                   MISSING_CHUNK_DIM * MISSING_CHUNK_DIM, sizeof(int))))
    TEST_ERROR;
  if (NULL ==
      (rdata2 = (int **)calloc(MISSING_CHUNK_DIM, sizeof(rdata2_bytes))))
    TEST_ERROR;
  for (i = 0; i < MISSING_CHUNK_DIM; i++)
  ...
  if ((d = H5Dcreate2(file, MISSING_CHUNK_DATASET, H5T_NATIVE_INT, s,
                      H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((did2 = H5Dcreate2(file, MISSING_CHUNK_DATASET2, H5T_NATIVE_INT, sid2,
                         H5P_DEFAULT, dcpl2, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Sselect_hyperslab(sid2, H5S_SELECT_SET, hs_start2, hs_stride2,
                          hs_count2, hs_block2) < 0)
    TEST_ERROR;
  ...
  if (H5Dread(d, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata) < 0)
    TEST_ERROR;
  if (H5Dread(did2, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              rdata2_bytes) < 0)
    TEST_ERROR;
    ...
    for (j = 0; j < MISSING_CHUNK_DIM; j++) {
      ...
      if ((i % 10) >= 5 || (j % 10) >= 5) {
        ...
        if (rdata2[i][j] != wdata2[i][j]) {
          printf("    Line %d: Incorrect value, wdata2[%u][%u] = %d, "
                 "rdata2[%u][%u] = %d\n",
                 __LINE__, (unsigned)i, (unsigned)j, wdata2[i][j], (unsigned)i,
 ...
 */
static void make_random_offset_and_increment(long nelts, long *offsetp,
                                             long *incp) {
  long inc;
  long maxinc;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_random_chunks_real(const char *testname, hbool_t early_alloc,
                                      hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];
  hid_t s = -1, m = -1, d = -1, dcpl = -1, file = -1;
  int wbuf[NPOINTS], rbuf[NPOINTS], check2[20][20];
  hsize_t coord[NPOINTS][2];
  const hsize_t dsize[2] = {100, 100}, dmax[2] = {H5S_UNLIMITED, H5S_UNLIMITED},
                csize[2] = {10, 10}, nsize[2] = {200, 200};
  ...
  /* Set early allocation time for one dataset; the other dataset is using
   * default alloc time */
  if (early_alloc)
  ...
  /* Create dataset */
  if ((d = H5Dcreate2(file, dname, H5T_NATIVE_INT, s, H5P_DEFAULT, dcpl,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Sselect_elements(s, H5S_SELECT_SET, (size_t)NPOINTS,
  ...
  for (i = 0; i < NPOINTS; i++)
    if (rbuf[i] != wbuf[i]) {
      ...
      printf("             coord[%u] = {%lu, %lu}\n", (unsigned)i,
             (unsigned long)coord[i][0], (unsigned long)coord[i][1]);
      TEST_ERROR;
    } /* end if */
 ...
 * Purpose: Tests that write/read on randomly selected chunks
 *          First file:
 *              One dataset has fixed dimensions without max. dims &
 *H5D_ALLOC_TIME_EARLY One dataset has fixed dimensions without max. dims &
 *default alloc time Second file: One extendible dataset with unlimited max.
 *dims & H5D_ALLOC_TIME_EARLY One extendible dataset with unlimited max. dims &
 *default alloc time third file: one extendible dataset with fixed max. dims &
 *H5D_ALLOC_TIME_EARLY one extendible dataset with fixed max. dims & default
 *alloc time
 *
 *          All the datasets in second & third files are extended before
...
static herr_t test_random_chunks(hid_t fapl) {
  ...
  nerrors += test_random_chunks_real(
      "Write/read on randomly selected chunks w/non-implicit index", FALSE,
      fapl);
  nerrors += test_random_chunks_real(
      "Write/read on randomly selected chunks w/implicit index", TRUE, fapl);
...
static htri_t can_apply_deprec(hid_t H5_ATTR_UNUSED dcpl_id,
                               hid_t H5_ATTR_UNUSED type_id,
                               hid_t H5_ATTR_UNUSED space_id) {
  return 1;
}

static herr_t set_local_deprec(hid_t H5_ATTR_UNUSED dcpl_id,
                               hid_t H5_ATTR_UNUSED type_id,
                               hid_t H5_ATTR_UNUSED space_id) {
  return (SUCCEED);
}
...
static herr_t test_deprec(hid_t file) {
   ...
   * not sure what they are, so we won't check.
   */
  if ((dataset = H5Dcreate1(file, DSET_DEPREC_NAME, H5T_NATIVE_DOUBLE, space,
                            H5P_DEFAULT)) < 0)
    goto error;
...
#if defined H5_HAVE_FILTER_DEFLATE
  {
    ...
    filtn = H5Pget_filter1(create_parms, (unsigned)0, NULL, &cd_nelmts,
                           &cd_value, (size_t)0, NULL);
    if (H5Z_FILTER_DEFLATE != filtn)
      goto error;
    if (1 != cd_nelmts)
      goto error;
    ...
    /* Check for the deflate filter */
    if (H5Pget_filter_by_id1(create_parms, H5Z_FILTER_DEFLATE, NULL, &cd_nelmts,
                             &cd_value, (size_t)0, NULL) < 0)
      goto error;
    if (1 != cd_nelmts)
      goto error;
    if (6 != cd_value)
  ...
  assert(status >= 0);
  H5E_BEGIN_TRY {
    dataset = H5Dcreate1(file, DSET_DEPREC_NAME_CHUNKED, H5T_NATIVE_DOUBLE,
                         space, create_parms);
  }
  H5E_END_TRY
  if (dataset >= 0) {
  ...
  if ((dataset = H5Dcreate1(file, DSET_DEPREC_NAME_CHUNKED, H5T_NATIVE_DOUBLE,
                            space, create_parms)) < 0)
    goto error;
  H5Pclose(create_parms);
  ...
  if ((dataset = H5Dcreate1(file, DSET_DEPREC_NAME_COMPACT, H5T_NATIVE_DOUBLE,
                            small_space, create_parms)) < 0)
    goto error;
  H5Pclose(create_parms);
  if (H5Dclose(dataset) < 0)
    goto error;
  ...
  HDputs("");
  if (test_filter_internal(file, DSET_DEPREC_NAME_FILTER, dcpl,
                           DISABLE_FLETCHER32, DATA_NOT_CORRUPTED,
                           &deprec_size) < 0)
    goto error;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_huge_chunks(hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];
  hid_t fid = -1;                 /* File ID */
  hid_t dcpl = -1;                /* Dataset creation property list ID */
  hid_t sid = -1;                 /* Dataspace ID */
  ...
  H5E_BEGIN_TRY {
    dsid = H5Dcreate2(fid, HUGE_DATASET, H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                      H5P_DEFAULT);
  }
  H5E_END_TRY
  if (dsid >= 0)
    FAIL_PUTS_ERROR(
        "    1-D Dataset with too large of chunk dimensions created.");
  ...
  /* Try to create dataset */
  H5E_BEGIN_TRY {
    dsid = H5Dcreate2(fid, HUGE_DATASET2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                      dcpl, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (dsid >= 0)
    FAIL_PUTS_ERROR(
        "    n-D Dataset with too large of chunk dimensions created.");
...
static herr_t test_chunk_cache(hid_t fapl) {
  ...
  if (H5Pget_chunk_cache(dapl1, &nslots_4, &nbytes_4, &w0_4) < 0)
    FAIL_STACK_ERROR;
  if ((nslots_1 != nslots_4) || (nbytes_1 != nbytes_4) ||
      !H5_DBL_ABS_EQUAL(w0_1, w0_4))
    FAIL_PUTS_ERROR(
        "    Cache values from default dapl do not match those from fapl.");
  ...
  /* Retrieve dapl from dataset, verify cache values are the same as on
   * fapl_local */
  if ((dapl2 = H5Dget_access_plist(dsid)) < 0)
    FAIL_STACK_ERROR;
  if (H5Pget_chunk_cache(dapl2, &nslots_4, &nbytes_4, &w0_4) < 0)
    FAIL_STACK_ERROR;
  if ((nslots_2 != nslots_4) || (nbytes_2 != nbytes_4) ||
      !H5_DBL_ABS_EQUAL(w0_2, w0_4))
    FAIL_PUTS_ERROR(
        "    Cache values from retrieved dapl do not match those from fapl.");
  if (H5Pclose(dapl2) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Pget_chunk_cache(dapl2, &nslots_4, &nbytes_4, &w0_4) < 0)
    FAIL_STACK_ERROR;
  if ((nslots_3 != nslots_4) || (nbytes_2 != nbytes_4) ||
      !H5_DBL_ABS_EQUAL(w0_3, w0_4))
    FAIL_PUTS_ERROR(
        "    Cache values from retrieved dapl do not match those from dapl1.");
  if (H5Pclose(dapl2) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dclose(dsid) < 0)
    FAIL_STACK_ERROR;
  if ((dsid = H5Dcreate2(fid, "dset2", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((dapl2 = H5Dget_access_plist(dsid)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((nslots_2 != nslots_4) || (nbytes_2 != nbytes_4) ||
      !H5_DBL_ABS_EQUAL(w0_2, w0_4))
    FAIL_PUTS_ERROR(
        "    Cache values from retrieved dapl do not match those from dapl2.");
  ...
  if ((nslots_3 != nslots_4) || (nbytes_2 != nbytes_4) ||
      !H5_DBL_ABS_EQUAL(w0_3, w0_4))
    FAIL_PUTS_ERROR(
        "    Cache values from retrieved dapl do not match those expected.");
  if (H5Pclose(dapl2) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Pget_chunk_cache(dapl2, &nslots_4, &nbytes_4, &w0_4) < 0)
    FAIL_STACK_ERROR;
  if ((nslots_3 != nslots_4) || (nbytes_3 != nbytes_4) ||
      !H5_DBL_ABS_EQUAL(w0_3, w0_4))
    FAIL_PUTS_ERROR(
        "    Cache values from retrieved dapl do not match those from fapl.");
  if (H5Pclose(dapl2) < 0)
  ...
  if (H5Pget_chunk_cache(dapl2, &nslots_4, &nbytes_4, &w0_4) < 0)
    FAIL_STACK_ERROR;
  if ((nslots_3 != nslots_4) || (nbytes_1 != nbytes_4) ||
      !H5_DBL_ABS_EQUAL(w0_3, w0_4))
    FAIL_PUTS_ERROR(
        "    Cache values from dapl2 do not match those from dapl1.");
...
static herr_t test_big_chunks_bypass_cache(hid_t fapl) {
  ...
  hsize_t t_count[2], t_stride[2], t_offset[2],
      t_block[2]; /* Setting for hyperslab (2-D) */
  ...
  /* Set up data arrays */
  if (NULL ==
      (t_wdata_bytes = (int *)calloc(
           (BYPASS_CHUNK_DIM / 2) * (BYPASS_CHUNK_DIM / 2), sizeof(int))))
    TEST_ERROR;
  if (NULL ==
      (t_wdata = (int **)calloc((BYPASS_CHUNK_DIM / 2), sizeof(t_wdata_bytes))))
    TEST_ERROR;
  for (i = 0; i < (BYPASS_CHUNK_DIM / 2); i++)
    t_wdata[i] = t_wdata_bytes + (i * (BYPASS_CHUNK_DIM / 2));

  if (NULL ==
      (t_rdata1_bytes = (int *)calloc(BYPASS_DIM * BYPASS_DIM, sizeof(int))))
    TEST_ERROR;
  ...
  if (NULL ==
      (t_rdata2_bytes = (int *)calloc(
           (BYPASS_CHUNK_DIM / 2) * (BYPASS_CHUNK_DIM / 2), sizeof(int))))
    TEST_ERROR;
  if (NULL == (t_rdata2 = (int **)calloc((BYPASS_CHUNK_DIM / 2),
                                         sizeof(t_rdata2_bytes))))
    TEST_ERROR;
  for (i = 0; i < (BYPASS_CHUNK_DIM / 2); i++)
  ...
  if ((dsid = H5Dcreate2(fid, BYPASS_DATASET1, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Create the first 2-D dataset */
  if ((t_dsid = H5Dcreate2(fid, T_BYPASS_DATASET1, H5T_NATIVE_INT, t_sid,
                           H5P_DEFAULT, t_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Chunk index type expected depends on whether we are using the latest
  ...
  if (H5Sselect_hyperslab(sid, H5S_SELECT_SET, &offset, &stride, &count,
                          &block) < 0)
    FAIL_STACK_ERROR;

  if (H5Sselect_hyperslab(t_sid, H5S_SELECT_SET, t_offset, t_stride, t_count,
                          t_block) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dwrite(dsid, H5T_NATIVE_INT, H5S_ALL, sid, H5P_DEFAULT, wdata) < 0)
    FAIL_STACK_ERROR;
  if (H5Dwrite(t_dsid, H5T_NATIVE_INT, mid, t_sid, H5P_DEFAULT, t_wdata_bytes) <
      0)
    FAIL_STACK_ERROR;
  ...
  /* Reads both 2 chunks.  Reading the second chunk should bypass the cache
   * because the chunk is bigger than the cache size and it isn't allocated on
   * disk. */
  if (H5Dread(dsid, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata1) < 0)
    FAIL_STACK_ERROR;
  if (H5Dread(t_dsid, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              t_rdata1_bytes) < 0)
    FAIL_STACK_ERROR;
  ...
  for (i = 0; i < BYPASS_CHUNK_DIM / 2; i++)
    for (j = 0; j < BYPASS_CHUNK_DIM / 2; j++)
      if (t_rdata1[i][j] != j) {
        printf("    Read different values than written in the 1st chunk.\n");
        printf("    At line %d and index (%d, %d), t_rdata1 = %d. It should be "
               "%d.\n",
  ...
  if ((dsid = H5Dcreate2(fid, BYPASS_DATASET2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((t_dsid = H5Dcreate2(fid, T_BYPASS_DATASET2, H5T_NATIVE_INT, t_sid,
                           H5P_DEFAULT, t_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dread(dsid, H5T_NATIVE_INT, H5S_ALL, sid, H5P_DEFAULT, rdata2) < 0)
    FAIL_STACK_ERROR;
  if (H5Dread(t_dsid, H5T_NATIVE_INT, mid, t_sid, H5P_DEFAULT, t_rdata2_bytes) <
      0)
    FAIL_STACK_ERROR;
  ...
  for (i = 0; i < BYPASS_CHUNK_DIM / 2; i++)
    for (j = 0; j < BYPASS_CHUNK_DIM / 2; j++)
      if (t_rdata2[i][j] != j) {
        printf("    Read different values than written in the chunk.\n");
        printf("    At line %d and index (%d, %d), t_rdata2 = %d. It should be "
               "%d.\n",
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_chunk_fast(const char *env_h5_driver, hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];
  hid_t fid = -1;                     /* File ID */
  ...
  /* Turn on the chunk cache again */
  {
    ...
    if (H5Pget_cache(my_fapl, &mdc_nelmts, &rdcc_nelmts, &rdcc_nbytes,
                     &rdcc_w0) < 0)
      FAIL_STACK_ERROR;
    rdcc_nbytes = 1048576;
    if (H5Pset_cache(my_fapl, mdc_nelmts, rdcc_nelmts, rdcc_nbytes, rdcc_w0) <
        0)
      FAIL_STACK_ERROR;
  } /* end block */
      ...
      for (alloc_time = H5D_ALLOC_TIME_EARLY; alloc_time <= H5D_ALLOC_TIME_INCR;
        ...
        for (ndims = 1; ndims < (EARRAY_MAX_RANK + 1); ndims++) {
          ...
          for (unlim_dim = 0; unlim_dim < ndims; unlim_dim++) {
            H5D_chunk_index_t idx_type; /* Dataset chunk index type */
            hsize_t dim[EARRAY_MAX_RANK],
                max_dim[EARRAY_MAX_RANK]; /* Dataset dimensions */
            hsize_t
                swizzled_dim[EARRAY_MAX_RANK]; /* Dimensions, with unlimited
                                                  dimension moved to rank 0 */
            hsize_t down[EARRAY_MAX_RANK]; /* 'down' sizes, for computing array
                                              index */
            ...
            /* Create file */
            if ((fid = H5Fcreate(
                     filename, H5F_ACC_TRUNC | (swmr ? H5F_ACC_SWMR_WRITE : 0),
            ...
            if ((dsid = H5Dcreate2(fid, "dset", H5T_NATIVE_UINT, sid,
                                   H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
              FAIL_STACK_ERROR;
            ...
            /* Chunk index type expected depends on whether we are using the
            ...
            for (u = 0; u < npoints; u++) {
              ...
              if (H5Sselect_hyperslab(sid, H5S_SELECT_SET, hs_offset, NULL,
                                      hs_size, NULL) < 0)
                FAIL_STACK_ERROR;

              /* Read (unwritten) element from dataset */
              read_elem = 1;
              if (H5Dread(dsid, H5T_NATIVE_UINT, scalar_sid, sid, H5P_DEFAULT,
                          &read_elem) < 0)
                FAIL_STACK_ERROR;
            ...
            while (dim[unlim_dim] < EARRAY_MAX_EXTEND) {
              hssize_t
                  snew_npoints; /* # of points in dataspace extent (signed) */
              ...
              for (u = npoints; u < new_npoints; u++) {
                ...
                /* Un-swizzle hyperslab offset in same way as swizzled
                 * dimensions */
                H5VM_unswizzle_coords(hsize_t, hs_offset, unlim_dim);
                ...
                /* Write element to dataset */
                write_elem = (unsigned)u;
                if (H5Dwrite(dsid, H5T_NATIVE_UINT, scalar_sid, sid,
                             H5P_DEFAULT, &write_elem) < 0)
                  FAIL_STACK_ERROR;
            ...
            if ((fid = H5Fopen(filename,
                               H5F_ACC_RDONLY | (swmr ? H5F_ACC_SWMR_READ : 0),
                               my_fapl)) < 0)
              FAIL_STACK_ERROR;
            ...
            /* Chunk index tyepe expected depends on whether we are using the
             * latest version of the format */
            if (low == H5F_LIBVER_LATEST || swmr) {
              /* Verify index type */
              if (idx_type != H5D_CHUNK_IDX_EARRAY)
 ...
 *-------------------------------------------------------------------------
 */
static size_t filter_expand(unsigned int flags, size_t H5_ATTR_UNUSED cd_nelmts,
                            const unsigned int H5_ATTR_UNUSED *cd_values,
...
static herr_t test_chunk_expand(hid_t fapl) {
  ...
  hsize_t dim, max_dim, chunk_dim; /* Dataset and chunk dimensions */
  hsize_t dim2[2], max_dim2[2],
      chunk_dim2[2];                     /* Dataset and chunk dimensions */
  ...
  if (sizeof(size_t) <= 4 && low != H5F_LIBVER_LATEST) {
    ...
    for (alloc_time = H5D_ALLOC_TIME_EARLY; alloc_time <= H5D_ALLOC_TIME_INCR;
      ...
      if (H5D_ALLOC_TIME_EARLY == alloc_time) {
        ...
        H5E_BEGIN_TRY {
          dsid = H5Dcreate2(fid, "dset", H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                            dcpl, H5P_DEFAULT);
        }
        H5E_END_TRY
        if (dsid >= 0)
          FAIL_PUTS_ERROR(
              "should fail to create dataset when allocation time is early");

        H5E_BEGIN_TRY {
          dsid2 = H5Dcreate2(fid, "dset2", H5T_NATIVE_UINT, sid2, H5P_DEFAULT,
                             dcpl2, H5P_DEFAULT);
        }
        H5E_END_TRY
        if (dsid2 >= 0)
        ...
        if ((dsid = H5Dcreate2(fid, "dset", H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                               dcpl, H5P_DEFAULT)) < 0)
          FAIL_STACK_ERROR;

        if ((dsid2 = H5Dcreate2(fid, "dset2", H5T_NATIVE_UINT, sid2,
                                H5P_DEFAULT, dcpl2, H5P_DEFAULT)) < 0)
          FAIL_STACK_ERROR;
        ...
        for (u = 0; u < 100; u++) {
          ...
          /* Select a single element in the 2-D dataset; NOT every element is
           * selected */
          if (H5Sselect_hyperslab(sid2, H5S_SELECT_SET, hs_offset2, NULL,
                                  hs_size2, NULL) < 0)
            FAIL_STACK_ERROR;
          ...
          if (H5Dread(dsid, H5T_NATIVE_UINT, scalar_sid, sid, H5P_DEFAULT,
                      &read_elem) < 0)
            FAIL_STACK_ERROR;
          if (H5Dread(dsid2, H5T_NATIVE_UINT, scalar_sid, sid2, H5P_DEFAULT,
                      &read_elem2) < 0)
            FAIL_STACK_ERROR;
          ...
          if (H5Dwrite(dsid, H5T_NATIVE_UINT, scalar_sid, sid, H5P_DEFAULT,
                       &write_elem) < 0)
            FAIL_STACK_ERROR;
          if (H5Dwrite(dsid2, H5T_NATIVE_UINT, scalar_sid, sid2, H5P_DEFAULT,
                       &write_elem2) < 0)
            FAIL_STACK_ERROR;
          ...
          write_elem = write_elem2 = u;
          H5E_BEGIN_TRY {
            status = H5Dwrite(dsid, H5T_NATIVE_UINT, scalar_sid, sid,
                              H5P_DEFAULT, &write_elem);
          }
          H5E_END_TRY
          if (status >= 0)
            FAIL_PUTS_ERROR("should fail to write to dataset when allocation "
                            "time is not early");

          H5E_BEGIN_TRY {
            status = H5Dwrite(dsid2, H5T_NATIVE_UINT, scalar_sid, sid2,
                              H5P_DEFAULT, &write_elem2);
          }
          H5E_END_TRY
          if (status >= 0)
            FAIL_PUTS_ERROR("should fail to write to dataset when allocation "
 ...
 *         (Note that the third dataset is created with bigger size for curr &
 *max. dims so that Fixed Array Indexing with paging is exercised)
 *
 *      Repeat the following test with/without compression filter
 *              Repeat the following test with
 *H5D_ALLOC_TIME_EARLY/H5D_ALLOC_TIME_LATE/H5D_ALLOC_TIME_INCR For the old
 *format, verify that v1 btree indexing type is used for all 3 datasets with all
 *settings For the new format: Verify that Implicit Index type is used for #1,
 *#2, #3 datasets when ALLOC_TIME_EARLY and compression are true Verify Fixed
 *Array indexing type is used for #1, #2, #3 datasets with all other settings
 *
 * Return:      Success: 0
 *              Failure: -1
 *
...
static herr_t test_fixed_array(hid_t fapl) {
  ...
  hsize_t msize_big[1] = {
      POINTS_BIG}; /* Size of memory space for big dataset */
  ...
  if (NULL ==
      (coord_big_bytes = (hsize_t *)calloc(POINTS_BIG * 2, sizeof(hsize_t))))
    TEST_ERROR;
  if (NULL ==
      (coord_big = (hsize_t **)calloc(POINTS_BIG, sizeof(coord_big_bytes))))
    TEST_ERROR;
  ...
  for (compress = FALSE; compress <= TRUE; compress++) {
    ...
    for (alloc_time = H5D_ALLOC_TIME_EARLY; alloc_time <= H5D_ALLOC_TIME_INCR;
      ...
      if ((sid_max = H5Screate_simple(2, dim2, dim2_max)) < 0)
        FAIL_STACK_ERROR;
      dsid_max = H5Dcreate2(fid, DSET_FIXED_MAX, H5T_NATIVE_INT, sid_max,
                            H5P_DEFAULT, dcpl, H5P_DEFAULT);
      if (dsid_max < 0)
        FAIL_PUTS_ERROR(
            "    Creating Chunked Dataset with maximum dimensions.");

      /* Get the chunk index type */
      if (H5D__layout_idx_type_test(dsid_max, &idx_type) < 0)
        FAIL_STACK_ERROR;

      /* Chunk index type depends on whether we are using the latest version of
       * the format */
      if (low == H5F_LIBVER_LATEST) {
      ...
      /* Select the random points for writing */
      if (H5Sselect_elements(sid_max, H5S_SELECT_SET, POINTS,
                             (const hsize_t *)coord_bytes) < 0)
        TEST_ERROR;

      /* Write into dataset */
      if (H5Dwrite(dsid_max, H5T_NATIVE_INT, mem_id, sid_max, H5P_DEFAULT,
                   wbuf) < 0)
        TEST_ERROR;
      ...
      if ((sid = H5Screate_simple(2, dim2, NULL)) < 0)
        FAIL_STACK_ERROR;
      dsid = H5Dcreate2(fid, DSET_FIXED_NOMAX, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT);
      if (dsid < 0)
      ...
      /* Select the random points for writing */
      if (H5Sselect_elements(sid, H5S_SELECT_SET, POINTS,
                             (const hsize_t *)coord_bytes) < 0)
        TEST_ERROR;
      ...
      /* Create the third dataset with bigger size and both curr & max
       * dimensions are the same */
      if ((sid_big = H5Screate_simple(2, dim2_big, dim2_big)) < 0)
        FAIL_STACK_ERROR;
      dsid_big = H5Dcreate2(fid, DSET_FIXED_BIG, H5T_NATIVE_INT, sid_big,
                            H5P_DEFAULT, dcpl, H5P_DEFAULT);
      if (dsid_big < 0)
      ...
      for (i = 0; i < POINTS_BIG; i++) {
        ...
        wbuf_big[i] = chunks_big[chunk_row][chunk_col] =
      ...
      /* Select the random points for writing */
      if (H5Sselect_elements(sid_big, H5S_SELECT_SET, POINTS_BIG,
                             (const hsize_t *)coord_big_bytes) < 0)
        TEST_ERROR;

      /* Write into dataset */
      if (H5Dwrite(dsid_big, H5T_NATIVE_INT, big_mem_id, sid_big, H5P_DEFAULT,
                   wbuf_big) < 0)
        TEST_ERROR;
      ...
      if (H5Sselect_elements(sid_big, H5S_SELECT_SET, POINTS_BIG,
        ...
        TEST_ERROR;
      /* Read from dataset */
      if (H5Dread(dsid_big, H5T_NATIVE_INT, big_mem_id, sid_big, H5P_DEFAULT,
                  rbuf_big) < 0)
        TEST_ERROR;
      ...
      for (i = 0; i < POINTS_BIG; i++)
        if (rbuf_big[i] != wbuf_big[i]) {
          printf("    Line %d: Incorrect value, wbuf_bif[%u]=%d, "
                 "rbuf_big[%u]=%d\n",
 ...
 *              Repeat the following test with
 *H5D_ALLOC_TIME_EARLY/H5D_ALLOC_TIME_LATE/H5D_ALLOC_TIME_INCR For the old
 *format, verify that v1 btree indexing type is used for all datasets with all
 *settings For the new format: Verify that Single Chunk indexing type is used
 *for all datasets with all settings
 *
 * Return:      Success: 0
 *              Failure: -1
 *
...
static herr_t test_single_chunk(hid_t fapl) {
  ...
  hid_t sid = -1,
        sid_max = -1; /* Dataspace ID for dataset with fixed dimensions */
  hid_t did = -1,
        did_max = -1; /* Dataset ID for dataset with fixed dimensions */
  ...
  if (NULL == (rbuf = (int *)malloc(sizeof(int) * (DSET_DIM1 * DSET_DIM2))))
    TEST_ERROR;
  if (NULL ==
      (t_wbuf = (int *)malloc(sizeof(int) * (DSET_TMP_DIM1 * DSET_TMP_DIM2))))
    TEST_ERROR;
  if (NULL ==
      (t_rbuf = (int *)malloc(sizeof(int) * (DSET_TMP_DIM1 * DSET_TMP_DIM2))))
    TEST_ERROR;
  ...
  for (compress = FALSE; compress <= TRUE; compress++) {
    ...
    for (alloc_time = H5D_ALLOC_TIME_EARLY; alloc_time <= H5D_ALLOC_TIME_INCR;
      ...
      if ((sid_max = H5Screate_simple(2, dim2, dim2)) < 0)
        FAIL_STACK_ERROR;
      did_max = H5Dcreate2(fid, DSET_SINGLE_MAX, H5T_NATIVE_INT, sid_max,
                           H5P_DEFAULT, dcpl, H5P_DEFAULT);
      if (did_max < 0)
        FAIL_PUTS_ERROR(
            "    Creating Chunked Dataset with maximum dimensions.");
      ...
      /* Write into dataset */
      if (H5Dwrite(did_max, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                   wbuf) < 0)
        TEST_ERROR;
      ...
      if ((sid = H5Screate_simple(2, t_dim2, NULL)) < 0)
        FAIL_STACK_ERROR;
      did = H5Dcreate2(fid, DSET_SINGLE_NOMAX, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                       t_dcpl, H5P_DEFAULT);
      if (did < 0)
        FAIL_PUTS_ERROR("    Creating Chunked Dataset.");
      ...
      /* Read from dataset */
      if (H5Dread(did_max, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                  rbuf) < 0)
        TEST_ERROR;
      ...
      /* Read from dataset */
      if (H5Dread(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, t_rbuf) <
          0)
        TEST_ERROR;
...
static herr_t test_zero_dim_dset(hid_t fapl) {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      /* Create chunked dataset */
      if ((dsid = H5Dcreate2(fid, "dset", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                             dcpl, H5P_DEFAULT)) < 0)
        FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_swmr_non_latest(const char *env_h5_driver, hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];
  hid_t fid = -1;                       /* File ID */
  ...
  TESTING("File created with write+latest-format/SWMR-write+non-latest-format: "
          "dataset with latest chunk "
          "index");
  ...
  if (low == H5F_LIBVER_LATEST) {
    ...
    if ((fid = H5Fcreate(filename, H5F_ACC_TRUNC | H5F_ACC_SWMR_WRITE,
                         H5P_DEFAULT, fapl)) < 0)
      FAIL_STACK_ERROR;
  } /* end else */
  ...
  if ((did = H5Dcreate2(fid, DSET_CHUNKED_NAME, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create a chunked dataset in the group: this will use v2 B-tree chunk
   * indexing */
  if ((dcpl = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((did = H5Dcreate2(gid, DSET_CHUNKED_NAME, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_earray_hdr_fd(const char *env_h5_driver, hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];
  hid_t fid = -1;
  hid_t sid = -1;
  hid_t did = -1;
  ...
  TESTING("Extensible array chunk index header flush dependencies handled "
          "correctly");
  ...
  if (H5Pset_chunk(dcpl, 1, chunk) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dcreate2(fid, DSET_EARRAY_HDR_FD, tid, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Oget_info_by_name3(fid, DSET_EARRAY_HDR_FD, &info, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Oget_info_by_name3(fid, DSET_EARRAY_HDR_FD, &info, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_farray_hdr_fd(const char *env_h5_driver, hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];
  hid_t fid = -1;
  hid_t sid = -1;
  hid_t did = -1;
  ...
  TESTING(
      "Fixed array chunk index header flush dependencies handled correctly");
  ...
  if (H5Pset_chunk(dcpl, 1, chunk) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dcreate2(fid, DSET_FARRAY_HDR_FD, tid, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* The second call triggered a bug in the library (JIRA issue: SWMR-95) */
  if (H5Oget_info_by_name3(fid, DSET_FARRAY_HDR_FD, &info, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Oget_info_by_name3(fid, DSET_FARRAY_HDR_FD, &info, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_bt2_hdr_fd(const char *env_h5_driver, hid_t fapl) {
  char filename[FILENAME_BUF_SIZE];
  hid_t fid = -1;
  hid_t sid = -1;
  hid_t did = -1;
  ...
  TESTING("Version 2 B-tree chunk index header flush dependencies handled "
          "correctly");
  ...
  if (H5Pset_chunk(dcpl, 2, chunk) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dcreate2(fid, DSET_BT2_HDR_FD, tid, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* The second call triggered a bug in the library (JIRA issue: SWMR-95) */
  if (H5Oget_info_by_name3(fid, DSET_BT2_HDR_FD, &info, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Oget_info_by_name3(fid, DSET_BT2_HDR_FD, &info, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
...
static herr_t test_storage_size(hid_t fapl) {
  ...
  if (0 == (ssize = H5Dget_storage_size(dsid)))
    FAIL_STACK_ERROR;
  if ((sizeof(int) * 6 * STORAGE_SIZE_CHUNK_DIM1 * STORAGE_SIZE_CHUNK_DIM2) !=
      ssize)
    TEST_ERROR;
  ...
  if ((dsid = H5Dcreate2(fid, "dset2a", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl2,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (0 == (ssize = H5Dget_storage_size(dsid)))
    FAIL_STACK_ERROR;
  if ((sizeof(int) * 15 * STORAGE_SIZE_CHUNK_DIM1 * STORAGE_SIZE_CHUNK_DIM2) !=
      ssize)
    TEST_ERROR;
  ...
  /* Create chunked dataset */
  if ((dsid = H5Dcreate2(fid, "dset3", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create chunked dataset, w/early allocation */
  if ((dsid = H5Dcreate2(fid, "dset3a", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl2,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create chunked dataset */
  if ((dsid = H5Dcreate2(fid, "dset4", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create chunked dataset, w/early allocation */
  if ((dsid = H5Dcreate2(fid, "dset4a", H5T_NATIVE_INT, sid, H5P_DEFAULT, dcpl2,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static herr_t test_power2up(hid_t fapl) {
  ...
  /* Create chunked dataset */
  if ((did = H5Dcreate2(fid, "dset", H5T_NATIVE_INT64, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t scatter_cb(void **src_buf /*out*/,
...
static herr_t test_scatter(void) {
  ...
  for (src_buf_size = 1; src_buf_size <= 9; src_buf_size++) {
    ...
    /* Scatter data */
    if (H5Dscatter((H5D_scatter_func_t)scatter_cb, &scatter_info,
                   H5T_NATIVE_INT, sid, dst_buf) < 0)
      TEST_ERROR;
   ...
   */
  /* Select hyperslabs */
  if (H5Sselect_elements(sid, H5S_SELECT_SET, sizeof(point) / sizeof(point[0]),
                         (hsize_t *)point) < 0)
    TEST_ERROR;
...
typedef struct gather_info_t {
  ...
  hbool_t last_call;   /* Whether this should be the last time the callback is
                          called */
} gather_info_t;

static herr_t gather_cb(const void *dst_buf, size_t dst_buf_bytes_used,
                        void *_gather_info) {
...
static herr_t test_gather(void) {
  ...
  for (i = 0; i < (int)(sizeof(src_buf) / sizeof(src_buf[0])); i++)
    for (j = 0; j < (int)(sizeof(src_buf[0]) / sizeof(src_buf[0][0])); j++)
      for (k = 0; k < (int)(sizeof(src_buf[0][0]) / sizeof(src_buf[0][0][0]));
           k++)
        src_buf[i][j][k] =
            1 + k +
  ...
  for (dst_buf_size = 1; dst_buf_size <= 9; dst_buf_size++) {
    ...
    /* Gather data */
    if (H5Dgather(sid, src_buf, H5T_NATIVE_INT,
                  dst_buf_size * sizeof(dst_buf[0]), dst_buf, gather_cb,
                  &gather_info) < 0)
      TEST_ERROR;
  ...
  for (dst_buf_size = 8; dst_buf_size <= 9; dst_buf_size++) {
    ...
    /* Gather data */
    if (H5Dgather(sid, src_buf, H5T_NATIVE_INT,
                  dst_buf_size * sizeof(dst_buf[0]), dst_buf, NULL, NULL) < 0)
      TEST_ERROR;
  ...
  /* Gather data */
  if (H5Dgather(sid, src_buf, H5T_NATIVE_INT,
                dst_buf_size * sizeof(dst_buf[0]) - 1, dst_buf, gather_cb,
                &gather_info) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t scatter_error_cb_fail(void **src_buf /*out*/,
                                    size_t *src_buf_bytes_used /*out*/,
                                    void *_scatter_info) {
  scatter_info_t *scatter_info = (scatter_info_t *)_scatter_info;
  size_t nelmts; /* Number of elements to return in src_buf */
...
static herr_t scatter_error_cb_null(void **src_buf /*out*/,
                                    size_t *src_buf_bytes_used /*out*/,
                                    void *_scatter_info) {
  scatter_info_t *scatter_info = (scatter_info_t *)_scatter_info;
  size_t nelmts; /* Number of elements to return in src_buf */
...
static herr_t scatter_error_cb_unalign(void **src_buf /*out*/,
                                       size_t *src_buf_bytes_used /*out*/,
...
static herr_t test_scatter_error(void) {
  ...
  scatter_info.size = 6;
  if (H5Dscatter((H5D_scatter_func_t)scatter_cb, &scatter_info, H5T_NATIVE_INT,
                 sid, dst_buf) < 0)
    TEST_ERROR;
  ...
  scatter_info.size = 6;
  H5E_BEGIN_TRY {
    ret = H5Dscatter((H5D_scatter_func_t)scatter_cb, &scatter_info, sid, sid,
                     dst_buf);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  scatter_info.size = 6;
  H5E_BEGIN_TRY {
    ret = H5Dscatter((H5D_scatter_func_t)scatter_cb, &scatter_info,
                     H5T_NATIVE_INT, H5T_NATIVE_INT, dst_buf);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
  ...
  scatter_info.size = 6;
  H5E_BEGIN_TRY {
    ret = H5Dscatter((H5D_scatter_func_t)scatter_error_cb_fail, &scatter_info,
                     H5T_NATIVE_INT, sid, dst_buf);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  scatter_info.size = 6;
  H5E_BEGIN_TRY {
    ret = H5Dscatter((H5D_scatter_func_t)scatter_error_cb_null, &scatter_info,
                     H5T_NATIVE_INT, sid, dst_buf);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  cb_unalign_nbytes = 0;
  H5E_BEGIN_TRY {
    ret = H5Dscatter((H5D_scatter_func_t)scatter_error_cb_unalign,
                     &cb_unalign_nbytes, H5T_NATIVE_INT, sid, dst_buf);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t gather_error_cb_fail(const void H5_ATTR_UNUSED *dst_buf,
                                   size_t H5_ATTR_UNUSED dst_buf_bytes_used,
                                   void H5_ATTR_UNUSED *op_data) {
  return FAIL;
...
static herr_t test_gather_error(void) {
  ...
  gather_info.last_call = FALSE;
  if (H5Dgather(sid, src_buf, H5T_NATIVE_INT, 6 * sizeof(dst_buf[0]), dst_buf,
                gather_cb, &gather_info) < 0)
    TEST_ERROR;
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(H5T_NATIVE_INT, src_buf, H5T_NATIVE_INT,
                    6 * sizeof(dst_buf[0]), dst_buf, gather_cb, &gather_info);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(sid, NULL, H5T_NATIVE_INT, 6 * sizeof(dst_buf[0]), dst_buf,
                    gather_cb, &gather_info);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(sid, src_buf, sid, 6 * sizeof(dst_buf[0]), dst_buf,
                    gather_cb, &gather_info);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(sid, src_buf, H5T_NATIVE_INT, 0, dst_buf, gather_cb,
                    &gather_info);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(sid, src_buf, H5T_NATIVE_INT, 1, dst_buf, gather_cb,
                    &gather_info);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(sid, src_buf, H5T_NATIVE_INT, 6 * sizeof(dst_buf[0]), NULL,
                    gather_cb, &gather_info);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(sid, src_buf, H5T_NATIVE_INT, 5 * sizeof(dst_buf[0]),
                    dst_buf, NULL, &gather_info);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
  ...
  gather_info.last_call = FALSE;
  H5E_BEGIN_TRY {
    ret = H5Dgather(sid, src_buf, H5T_NATIVE_INT, 6 * sizeof(dst_buf[0]),
                    dst_buf, gather_error_cb_fail, NULL);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
...
static herr_t dls_01_setup_file(hid_t fid) {
  ...
  did =
      H5Dcreate2(fid, DLS_01_DATASET, tid, sid, H5P_DEFAULT, dcpl, H5P_DEFAULT);
  if (did <= 0)
    TEST_ERROR;
...
static herr_t dls_01_main(void) {
  ...
  int status = 0;
  hid_t fapl = 0, fid = 0;
  const char *strings[DLS_01_DIMS] = {"String 1", "Test string 2",
...
static herr_t test_compact_open_close_dirty(hid_t fapl) {
  ...
  if ((did = H5Dcreate2(fid, DSET_COMPACT_MAX_NAME, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_versionbounds(void) {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      if ((srcfile = H5Fcreate(srcfilename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) <
          0)
        TEST_ERROR;
      if ((srcdset = H5Dcreate2(srcfile, SRC_DSET, H5T_NATIVE_INT, srcspace,
                                H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        TEST_ERROR;
      ...
      H5E_BEGIN_TRY {
        vdset = H5Dcreate2(vfile, V_DSET, H5T_NATIVE_INT, vspace, H5P_DEFAULT,
                           dcpl, H5P_DEFAULT);
      }
      H5E_END_TRY
...
static herr_t test_object_header_minimization_dcpl(void) {
  ...
  if (NULL ==
      h5_fixname(OHMIN_FILENAME_A, H5P_DEFAULT, filename, sizeof(filename)))
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY { ret = H5Pget_dset_no_attrs_hint(H5I_INVALID_HID, &minimize); }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;

  /* Invalid DCPL ID should fail */
  H5E_BEGIN_TRY { ret = H5Pset_dset_no_attrs_hint(H5I_INVALID_HID, FALSE); }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;

  /* Invalid DCPL ID should fail */
  H5E_BEGIN_TRY { ret = H5Pset_dset_no_attrs_hint(H5I_INVALID_HID, TRUE); }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;

  /* NULL out pointer should fail */
  H5E_BEGIN_TRY { ret = H5Pget_dset_no_attrs_hint(dcpl_id, NULL); }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;
...
static herr_t test_h5s_block(void) {
  ...
  if ((file_space_id = H5Screate_simple(1, dims, NULL)) < 0)
    FAIL_STACK_ERROR;
  if ((dset_id = H5Dcreate2(file_id, "dset", H5T_NATIVE_INT, file_space_id,
                            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    ret =
        H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_BLOCK, H5P_DEFAULT, buf);
  }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;
  ...
  if (H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_BLOCK, H5S_ALL, H5P_DEFAULT, buf) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Dread(dset_id, H5T_NATIVE_INT, H5S_BLOCK, H5S_ALL, H5P_DEFAULT, buf) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Sselect_hyperslab(file_space_id, H5S_SELECT_SET, &start, NULL, &count,
                          NULL) < 0)
    FAIL_STACK_ERROR;
  if (H5Dread(dset_id, H5T_NATIVE_INT, H5S_BLOCK, file_space_id, H5P_DEFAULT,
              buf) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Verify that reading 0 elements is handled correctly and doesn't modify
   * buffer */
...
static herr_t test_h5s_plist(void) {
  ...
  hid_t dxpl_id_copy =
      H5I_INVALID_HID; /* Copy of dataset xfer property list ID */
  hsize_t start = 2;   /* Starting offset of hyperslab selection */
  ...
  H5E_BEGIN_TRY {
    /* Bad rank */
    ret = H5Pset_dataset_io_hyperslab_selection(
        dxpl_id, 0, H5S_SELECT_SET, &start, &stride, &count, &block);
  }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    ...
    ret = H5Pset_dataset_io_hyperslab_selection(
        dxpl_id, 1, H5S_SELECT_NOOP, &start, &stride, &count, &block);
  }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    /* Bad start pointer */
    ret = H5Pset_dataset_io_hyperslab_selection(dxpl_id, 1, H5S_SELECT_SET,
                                                NULL, &stride, &count, &block);
  }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    ...
    stride = 0;
    ret = H5Pset_dataset_io_hyperslab_selection(
        dxpl_id, 1, H5S_SELECT_SET, &start, &stride, &count, &block);
    stride = 1;
  }
  H5E_END_TRY
  if (ret == SUCCEED)
  ...
  H5E_BEGIN_TRY {
    ...
    ret =
        H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_PLIST, H5S_ALL, H5P_DEFAULT, buf);
  }
  H5E_END_TRY
  if (ret == SUCCEED)
    TEST_ERROR;
  ...
  if (H5Pset_dataset_io_hyperslab_selection(dxpl_id, 1, H5S_SELECT_SET, &start,
  ...
  /* Read a hyperslab from the file to the first 10 elements of the buffer */
  if (H5Dread(dset_id, H5T_NATIVE_INT, H5S_BLOCK, H5S_PLIST, dxpl_id_copy,
              buf) < 0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    ret = H5Pset_dataset_io_hyperslab_selection(dxpl_id_copy, H5S_MAX_RANK + 1,
                                                H5S_SELECT_OR, &start, &stride,
                                                &count, &block);
  }
  H5E_END_TRY
  if (ret == SUCCEED)
  ...
  if (H5Pset_dataset_io_hyperslab_selection(
          dxpl_id_copy, 1, H5S_SELECT_SET, &start, &stride, &count, &block) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Pset_dataset_io_hyperslab_selection(dxpl_id, 1, H5S_SELECT_OR, &start2,
                                            &stride, &count2, &block) < 0)
    FAIL_STACK_ERROR;

  /* Read a disjoint hyperslab from the file to the first 10 elements of the
   * buffer */
  if (H5Dread(dset_id, H5T_NATIVE_INT, H5S_BLOCK, H5S_PLIST, dxpl_id, buf) < 0)
    FAIL_STACK_ERROR;
...
static herr_t test_0sized_dset_metadata_alloc(hid_t fapl_id) {
  ...
  for (new_format = FALSE; new_format <= TRUE; new_format++) {
     ...
     * objects in the file */
    if (new_format)
      if (H5Pset_libver_bounds(fapl_id_copy, H5F_LIBVER_LATEST,
                               H5F_LIBVER_LATEST) < 0)
        FAIL_STACK_ERROR;

    /* Create test file */
    if ((file_id =
             H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_id_copy)) < 0)
      FAIL_STACK_ERROR;
    ...
    for (layout = H5D_COMPACT; layout <= H5D_CHUNKED; layout++) {
      for (alloc_time = H5D_ALLOC_TIME_EARLY; alloc_time <= H5D_ALLOC_TIME_INCR;
        ...
        const hsize_t max_dims[1] = {
            H5S_UNLIMITED};                  /* Maximum dataset dimensions */
        ...
        HDsnprintf(dset_name, sizeof(dset_name), "/Dataset-%u-%u",
                   (unsigned)alloc_time, (unsigned)layout);
        ...
        if ((dset_space_id = H5Screate_simple(
                 1, dims, (H5D_CHUNKED == layout ? max_dims : NULL))) < 0)
          FAIL_STACK_ERROR;

        /* Create the dataset with the appropriate parameters */
        if ((dset_id =
                 H5Dcreate2(file_id, dset_name, H5T_NATIVE_INT, dset_space_id,
                            H5P_DEFAULT, dcpl_id_copy, H5P_DEFAULT)) < 0)
          FAIL_STACK_ERROR;
        ...
        if (H5Oget_native_info(dset_id, &nat_info, H5O_NATIVE_INFO_META_SIZE) <
            0)
          FAIL_STACK_ERROR;
        ...
        if (H5D_CHUNKED == layout) {
          ...
          const hsize_t mem_dims[1] = {
              1};                  /* Memory buffer dataset dimensions */
          ...
          /* Write the data to the dataset */
          if (H5Dwrite(dset_id, H5T_NATIVE_INT, buf_space_id, dset_space_id,
                       H5P_DEFAULT, &val) < 0)
            FAIL_STACK_ERROR;
          ...
          /* Retrieve & verify the dataset's index info */
          memset(&nat_info, 0, sizeof(nat_info));
          if (H5Oget_native_info(dset_id, &nat_info,
                                 H5O_NATIVE_INFO_META_SIZE) < 0)
            FAIL_STACK_ERROR;
          if (0 == nat_info.meta_size.obj.index_size)
...
int main(void) {
  ...
  contig_addr_vfd = (hbool_t)(HDstrcmp(envval, "split") != 0 &&
                              HDstrcmp(envval, "multi") != 0);
  ...
  /* points_dbl */
  if (NULL == (points_dbl_data =
                   (double *)calloc(DSET_DIM1 * DSET_DIM2, sizeof(double))))
    TEST_ERROR;
  if (NULL ==
      (points_dbl = (double **)calloc(DSET_DIM1, sizeof(points_dbl_data))))
    TEST_ERROR;
  for (i = 0; i < DSET_DIM1; i++)
    points_dbl[i] = points_dbl_data + (i * DSET_DIM2);

  /* check_dbl */
  if (NULL == (check_dbl_data =
                   (double *)calloc(DSET_DIM1 * DSET_DIM2, sizeof(double))))
    TEST_ERROR;
  if (NULL ==
      (check_dbl = (double **)calloc(DSET_DIM1, sizeof(check_dbl_data))))
    TEST_ERROR;
  for (i = 0; i < DSET_DIM1; i++)
  ...
  if (H5Pset_file_space_strategy(fcpl2, H5F_FSPACE_STRATEGY_PAGE, TRUE,
                                 (hsize_t)1) < 0)
    TEST_ERROR;
  ...
  for (paged = FALSE; paged <= TRUE; paged++) {
    ...
    for (minimized_ohdr = FALSE; minimized_ohdr <= TRUE; minimized_ohdr++) {
      ...
      for (new_format = FALSE; new_format <= TRUE; new_format++) {
        ...
        if ((grp = H5Gcreate2(file, "emit diagnostics", H5P_DEFAULT,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0)
          goto error;
        ...
        nerrors +=
            (test_userblock_offset(envval, my_fapl, new_format) < 0 ? 1 : 0);
