...
static void usage(void) {
  ...
  printf("Usage error!\n");
  printf("\n");
  printf("Usage: swmr_remove_writer [-q] [-o] [-f <# of shrinks between "
         "flushing\n");
  ...
  printf("Defaults to verbose (no '-q' given), latest format when opening file "
         "(no '-o' given),\n");
  printf("flushing every 1000 shrinks ('-f 1000'), and will generate a random "
         "seed (no -r given).\n");
  printf("\n");
  exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
  ...
  if (remove_records(fid, verbose, (unsigned long)nshrinks,
