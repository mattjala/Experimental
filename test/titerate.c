...
static int find_err_msg_cb(unsigned n, const H5E_error2_t *err_desc,
                           void *_client_data);
...
**
****************************************************************/
static herr_t liter_cb(hid_t H5_ATTR_UNUSED group, const char *name,
...
static void test_iter_group(hid_t fapl, hbool_t new_format) {
  ...
  for (i = 0; i < NDATASETS; i++) {
    ...
    dataset = H5Dcreate2(file, name, datatype, filespace, H5P_DEFAULT,
  ...
  ret = H5Tcommit2(file, "dtype", datatype, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  ...
  for (i = 0; i < (int)ginfo.nlinks; i++) {
    ...
    ret = (herr_t)H5Lget_name_by_idx(root_group, ".", H5_INDEX_NAME,
                                     H5_ITER_INC, (hsize_t)i, dataset_name,
                                     (size_t)NAMELEN, H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Lget_name_by_idx");

    //! [H5Oget_info_by_idx3_snip]

    ret = H5Oget_info_by_idx3(root_group, ".", H5_INDEX_NAME, H5_ITER_INC,
                              (hsize_t)i, &oinfo, H5O_INFO_BASIC, H5P_DEFAULT);
  ...
  H5E_BEGIN_TRY {
    ret = (herr_t)H5Lget_name_by_idx(
        root_group, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)(NDATASETS + 3),
  ...
  /* These two functions, H5Oget_info_by_idx and H5Lget_name_by_idx, actually
   * iterate through B-tree for group members in internal library design.
   *  (Same as test above, but with the file ID instead of opening the root
   * group)
   */
  ...
  for (i = 0; i < (int)ginfo.nlinks; i++) {
    ...
    ret = (herr_t)H5Lget_name_by_idx(file, ".", H5_INDEX_NAME, H5_ITER_INC,
                                     (hsize_t)i, dataset_name, (size_t)NAMELEN,
                                     H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Lget_name_by_idx");

    ret = H5Oget_info_by_idx3(file, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)i,
  ...
  H5E_BEGIN_TRY {
    ret = (herr_t)H5Lget_name_by_idx(file, ".", H5_INDEX_NAME, H5_ITER_INC,
                                     (hsize_t)(NDATASETS + 3), dataset_name,
                                     (size_t)NAMELEN, H5P_DEFAULT);
  }
  H5E_END_TRY
  VERIFY(ret, FAIL, "H5Lget_name_by_idx");
  ...
  info.command = RET_ZERO;
  idx = 0;
  if ((ret = H5Literate2(file, H5_INDEX_NAME, H5_ITER_INC, &idx, liter_cb,
                         &info)) > 0)
  ...
  i = 0;
  idx = 0;
  while ((ret = H5Literate2(file, H5_INDEX_NAME, H5_ITER_INC, &idx, liter_cb,
                            &info)) > 0) {
    ...
    if (HDstrcmp(info.name, lnames[(size_t)(idx - 1)]) != 0)
      TestErrPrintf("Group iteration function didn't return name correctly for "
                    "link - lnames[%u] = '%s'!\n",
  ...
  if (i != (NDATASETS + 2))
    TestErrPrintf("%u: Group iteration function didn't perform multiple "
...
**
****************************************************************/
herr_t aiter_cb(hid_t H5_ATTR_UNUSED group, const char *name,
...
static void test_iter_attr(hid_t fapl, hbool_t new_format) {
  ...
  dataset = H5Dcreate2(file, "Dataset", H5T_NATIVE_INT, filespace, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");

  for (i = 0; i < NATTR; i++) {
    HDsnprintf(name, sizeof(name), "Attribute %02d", i);
    attribute = H5Acreate2(dataset, name, H5T_NATIVE_INT, filespace,
                           H5P_DEFAULT, H5P_DEFAULT);
  ...
  H5E_BEGIN_TRY {
    ret =
        H5Aiterate2(dataset, H5_INDEX_NAME, H5_ITER_INC, &idx, aiter_cb, &info);
  }
  H5E_END_TRY
  ...
  info.command = RET_ZERO;
  idx = 0;
  if ((ret = H5Aiterate2(dataset, H5_INDEX_NAME, H5_ITER_INC, &idx, aiter_cb,
                         &info)) > 0)
    TestErrPrintf(
        "Attribute iteration function didn't return zero correctly!\n");
  ...
  i = 0;
  idx = 0;
  while ((ret = H5Aiterate2(dataset, H5_INDEX_NAME, H5_ITER_INC, &idx, aiter_cb,
                            &info)) > 0) {
    /* Verify return value from iterator gets propagated correctly */
    ...
    if (!new_format) {
      ...
      if (HDstrcmp(info.name, anames[(size_t)idx - 1]) != 0)
        TestErrPrintf("%u: Attribute iteration function didn't set names "
                      "correctly, info.name = "
                      "'%s', anames[%u] = '%s'!\n",
                      __LINE__, info.name, (unsigned)(idx - 1),
                      anames[(size_t)idx - 1]);
    } /* end if */
  }   /* end while */
  VERIFY(ret, -1, "H5Aiterate2");
  if (i != 50 || idx != 50)
    TestErrPrintf("%u: Attribute iteration function didn't perform multiple "
                  "iterations correctly!\n",
                  __LINE__);
...
**
****************************************************************/
static herr_t liter_cb2(hid_t loc_id, const char *name,
                        const H5L_info2_t H5_ATTR_UNUSED *link_info,
                        void *opdata) {
...
static void test_iter_group_large(hid_t fapl) {
  ...
  /* Create a dataset  */
  dataset = H5Dcreate2(file, "Dataset1", H5T_STD_U32LE, sid, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  ret =
      H5Tcommit2(file, "Datatype1", tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");
  ...
  /* Need to sort the names in the root group, cause that's what the library
   * does */
  ...
  curr_name = &names[0];
  ret =
      H5Literate2(file, H5_INDEX_NAME, H5_ITER_INC, NULL, liter_cb2, curr_name);
  CHECK(ret, FAIL, "H5Literate2");
  for (i = 1; i < 100; i++) {
    hsize_t idx = (hsize_t)i;

    curr_name = &names[i];
    ret = H5Literate2(file, H5_INDEX_NAME, H5_ITER_INC, &idx, liter_cb2,
                      curr_name);
    CHECK(ret, FAIL, "H5Literate2");
  } /* end for */
...
static void test_grp_memb_funcs(hid_t fapl) {
  ...
  for (i = 0; i < (int)ginfo.nlinks; i++) {
    ...
    name_len =
        H5Lget_name_by_idx(root_group, ".", H5_INDEX_NAME, H5_ITER_INC,
                           (hsize_t)i, NULL, (size_t)NAMELEN, H5P_DEFAULT);
    CHECK(name_len, FAIL, "H5Lget_name_by_idx");

    ret = (herr_t)H5Lget_name_by_idx(root_group, ".", H5_INDEX_NAME,
                                     H5_ITER_INC, (hsize_t)i, dataset_name,
                                     (size_t)(name_len + 1), H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Lget_name_by_idx");
...
static void test_links(hid_t fapl) {
  ...
  ret = H5Lcreate_hard(gid, "/g1", H5L_SAME_LOC, "hardlink", H5P_DEFAULT,
                       H5P_DEFAULT);
  ...
  for (i = 0; i < ginfo.nlinks; i++) {
    ...
    /* Get link name */
    name_len = H5Lget_name_by_idx(gid, ".", H5_INDEX_NAME, H5_ITER_INC, i,
                                  obj_name, (size_t)NAMELEN, H5P_DEFAULT);
    CHECK(name_len, FAIL, "H5Lget_name_by_idx");

    /* Get link type */
    ret = H5Lget_info_by_idx2(gid, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)i,
                              &linfo, H5P_DEFAULT);
    ...
    if (linfo.type == H5L_TYPE_HARD) {
      ret =
          H5Oget_info_by_idx3(gid, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)i,
                              &oinfo, H5O_INFO_BASIC, H5P_DEFAULT);
      CHECK(ret, FAIL, "H5Oget_info_by_idx3");
    } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static int find_err_msg_cb(unsigned H5_ATTR_UNUSED n,
                           const H5E_error2_t *err_desc, void *_client_data) {
  ...
  if (err_desc->desc != NULL &&
      HDstrcmp(err_desc->desc, searched_err->message) == 0) {
...
static void test_corrupted_attnamelen(void) {
  ...
  const char *testfile = H5_get_srcdir_filename(
      CORRUPTED_ATNAMELEN_FILE); /* Corrected test file name */
  ...
  if (err_status == -1) {
    ...
    CHECK(err_caught.found, FALSE,
          "test_corrupted_attnamelen: Expected error not found");
  }
...
static void test_links_deprec(hid_t fapl) {
  ...
  /* Output message about test being performed */
  MESSAGE(5, ("Testing Soft and Hard Link Iteration Functionality Using "
              "Deprecated Routines\n"));
...
void test_iterate(void) {
  ...
  for (new_format = FALSE; new_format <= TRUE; new_format++) {
    test_iter_group(new_format ? fapl2 : fapl,
                    new_format); /* Test group iteration */
    test_iter_group_large(
        new_format ? fapl2
                   : fapl); /* Test group iteration for large # of objects */
    test_iter_attr(new_format ? fapl2 : fapl,
                   new_format); /* Test attribute iteration */
    test_grp_memb_funcs(
        new_format ? fapl2
                   : fapl); /* Test group member information functions */
    test_links(new_format ? fapl2
                          : fapl); /* Test soft and hard link iteration */
#ifndef H5_NO_DEPRECATED_SYMBOLS
    test_links_deprec(
        new_format ? fapl2 : fapl); /* Test soft and hard link iteration */
#endif
  } /* end for */
