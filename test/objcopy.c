...
static const char *FILENAME[] = {
    "objcopy_src",  "objcopy_dst",  "objcopy_ext", "objcopy_src2",
...
#define CHUNK_SIZE_1                                                           \
  5 /* Not an even fraction of dimension sizes, so we test copying partial     \
       chunks */
...
static int compare_data(hid_t parent1, hid_t parent2, hid_t pid, hid_t tid,
                        size_t nelmts, const void *buf1, const void *buf2,
                        hid_t obj_owner);
static int compare_datasets(hid_t did, hid_t did2, hid_t pid, const void *wbuf);
static int compare_groups(hid_t gid, hid_t gid2, hid_t pid, int depth,
                          unsigned copy_flags);
static int compare_idx_type(hid_t fapl, hid_t did, H5D_chunk_index_t new_type,
                            H5D_chunk_index_t old_type);

static int test_copy_attribute_compound_vlstr(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl);
...
static void token_insert(H5O_info2_t *oi) {
  ...
  if (idtab_g.nobjs >= idtab_g.nalloc) {
    ...
    idtab_g.obj = (H5O_token_t *)realloc(
        idtab_g.obj, idtab_g.nalloc * sizeof(idtab_g.obj[0]));
  } /* end if */
 ...
 *-------------------------------------------------------------------------
 */
static H5_ATTR_PURE hbool_t token_lookup(hid_t loc_id, H5O_info2_t *oi) {
  size_t n;
  int token_cmp;
...
static herr_t attach_ref_attr(hid_t file_id, hid_t loc_id) {
  ...
  if ((sid_ref = H5Screate_simple(1, dims_ref, NULL)) < 0)
    TEST_ERROR;
  if ((did1 = H5Dcreate2(file_id, dsetname1, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did1, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data1) < 0)
    TEST_ERROR;
  if ((did2 = H5Dcreate2(file_id, dsetname2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Rcreate(&ref[1], file_id, dsetname2, H5R_OBJECT, (hid_t)-1) < 0)
    TEST_ERROR;
  if ((aid = H5Acreate2(loc_id, "obj_ref_attr", H5T_STD_REF_OBJ, sid_ref,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t attach_reg_ref_attr(hid_t file_id, hid_t loc_id) {
  ...
  int data[2][9] = {{1, 1, 2, 3, 3, 4, 5, 5, 999},
                    {1, 2, 2, 3, 4, 4, 5, 6, 999}};
  hsize_t start[2] = {0, 3};
  hsize_t count[2] = {2, 3};
  ...
  if ((spacer_id = H5Screate_simple(rankr, dimsr, NULL)) < 0)
    TEST_ERROR;
  if ((dsetv_id = H5Dcreate2(file_id, dsetnamev, H5T_NATIVE_INT, space_id,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(dsetv_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
      0)
    TEST_ERROR;

  /* create reg_ref of block selection */
  if (H5Sselect_hyperslab(space_id, H5S_SELECT_SET, start, NULL, count, NULL) <
      0)
    TEST_ERROR;
  ...
  if (H5Sselect_none(space_id) < 0)
    TEST_ERROR;
  if (H5Sselect_elements(space_id, H5S_SELECT_SET, num_points,
  ...
  if ((aid = H5Acreate2(loc_id, "reg_ref_attr", H5T_STD_REF_DSETREG, spacer_id,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  if ((aid = H5Acreate2(dsetv_id, "reg_ref_attr", H5T_STD_REF_DSETREG,
                        spacer_id, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Awrite(aid, H5T_STD_REF_DSETREG, ref) < 0)
    TEST_ERROR;
...
static herr_t create_reg_ref_dataset(hid_t file_id, hid_t loc_id) {
  ...
  if (H5Dwrite(dsetv_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
      0)
    TEST_ERROR;
  if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer, H5T_STD_REF_DSETREG, spacer_id,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Rcreate(&ref[1], file_id, dsetnamev, H5R_DATASET_REGION, space_id) < 0)
    TEST_ERROR;
  if (H5Dwrite(dsetr_id, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               ref) < 0)
    TEST_ERROR;
  ...
  if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer1, H5T_STD_REF_DSETREG, spacer_id,
                             H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Pclose(pid) < 0)
    TEST_ERROR;
  if (H5Dwrite(dsetr_id, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
  ...
  if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer2, H5T_STD_REF_DSETREG, spacer_id,
                             H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Pclose(pid) < 0)
    TEST_ERROR;
...
static int test_copy_attach_attribute_vl(hid_t loc_id) {
  ...
  if ((aid = H5Acreate2(loc_id, "vlen attribute", tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto done;
...
static int test_copy_attach_attributes(hid_t loc_id, hid_t type_id) {
  ...
  for (u = 0; u < num_attributes_g; u++) {
    ...
    if ((aid = H5Acreate2(loc_id, attr_name, type_id, sid, acpl, H5P_DEFAULT)) <
        0)
      goto done;
 ...
 *
 * Purpose:     Attach NUM_ATTRIBUTES attributes to a pair of objects to be
 *copied
 *
 * Return:    Non-negative on success/Negative on failure
 *
 *-------------------------------------------------------------------------
 */
static int test_copy_attach_paired_attributes(hid_t loc_id, hid_t loc_id2,
                                              hid_t type_id) {
  hid_t aid = -1, sid = -1;
  char attr_name[ATTR_NAME_LEN];
  int attr_data[2];
  ...
  for (u = 0; u < num_attributes_g; u++) {
    ...
    if ((aid = H5Acreate2(loc_id, attr_name, type_id, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto done;
    if (H5Awrite(aid, H5T_NATIVE_INT, attr_data) < 0)
      goto done;
    ...
    if ((aid = H5Acreate2(loc_id2, attr_name, type_id, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      goto done;
    if (H5Awrite(aid, H5T_NATIVE_INT, attr_data) < 0)
      goto done;
 ...
 *-------------------------------------------------------------------------
 */
static int compare_attribute(hid_t aid, hid_t aid2, hid_t pid, const void *wbuf,
                             hid_t obj_owner) {
  ...
  if (wbuf) {
    if (!compare_data(aid, (hid_t)0, pid, tid, (size_t)nelmts, wbuf, rbuf,
                      obj_owner))
      TEST_ERROR;
    if (!compare_data(aid2, (hid_t)0, pid, tid2, (size_t)nelmts, wbuf, rbuf2,
                      obj_owner))
      TEST_ERROR;
  } /* end if */
  /* Don't have written data, just compare data between the two attributes */
  else if (!compare_data(aid, aid2, pid, tid, (size_t)nelmts, rbuf, rbuf2,
                         obj_owner))
    TEST_ERROR;
 ...
 *
 * Purpose:     Compare "standard" attributes on two objects to check that they
 *are equal
 *
...
static int compare_std_attributes(hid_t oid, hid_t oid2, hid_t pid) {
  ...
  if (cpy_flags & H5O_COPY_WITHOUT_ATTR_FLAG) {
    ...
    for (i = 0; i < (unsigned)oinfo1.num_attrs; i++) {
      if ((aid = H5Aopen_by_idx(oid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                                (hsize_t)i, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        TEST_ERROR;
...
static int compare_data(hid_t parent1, hid_t parent2, hid_t pid, hid_t tid,
  ...
  if ((H5Tget_class(tid) == H5T_COMPOUND) &&
    ...
    for (u = 0; u < nelmts; u++, vl_buf1++, vl_buf2++) {
      ...
      /* Check vlen data */
      if (!compare_data(parent1, parent2, pid, base_tid, vl_buf1->len,
                        vl_buf1->p, vl_buf2->p, obj_owner))
        TEST_ERROR;
    } /* end for */
    ...
    if (H5Tequal(tid, H5T_STD_REF_OBJ) > 0) {
      const hobj_ref_t *ref_buf1,
          *ref_buf2; /* Aliases for buffers to compare */
      ...
      for (u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
        ...
        if ((obj1_id = H5Rdereference2(parent1, H5P_DEFAULT, H5R_OBJECT,
                                       ref_buf1)) < 0)
          TEST_ERROR;
        if ((obj2_id = H5Rdereference2(parent2, H5P_DEFAULT, H5R_OBJECT,
                                       ref_buf2)) < 0)
          TEST_ERROR;
        ...
        if (obj_owner > 0) {
          ...
          if (H5Oget_info3(obj1_id, &oinfo2, H5O_INFO_BASIC) < 0)
            TEST_ERROR;
          if (H5Otoken_cmp(obj1_id, &oinfo1.token, &oinfo2.token, &token_cmp) <
              0)
            TEST_ERROR;
          if (!token_cmp) {
      ...
      for (u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
        ...
        /* Check for types of objects handled */
        if (H5Rget_obj_type2(parent1, H5R_DATASET_REGION, ref_buf1,
                             &obj1_type) < 0)
          TEST_ERROR;
        if (H5Rget_obj_type2(parent2, H5R_DATASET_REGION, ref_buf2,
                             &obj2_type) < 0)
          TEST_ERROR;
        if (obj1_type != obj2_type)
          TEST_ERROR;
        ...
        /* Open referenced objects */
        if ((obj1_id = H5Rdereference2(parent1, H5P_DEFAULT, H5R_DATASET_REGION,
                                       ref_buf1)) < 0)
          TEST_ERROR;
        if ((obj2_id = H5Rdereference2(parent2, H5P_DEFAULT, H5R_DATASET_REGION,
                                       ref_buf2)) < 0)
          TEST_ERROR;
        ...
        if ((obj1_sid = H5Rget_region(parent1, H5R_DATASET_REGION, ref_buf1)) <
            0)
          TEST_ERROR;
        if ((obj2_sid = H5Rget_region(parent2, H5R_DATASET_REGION, ref_buf2)) <
            0)
          TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int compare_datasets(hid_t did, hid_t did2, hid_t pid,
                            const void *wbuf) {
  hid_t sid = -1, sid2 = -1;        /* Dataspace IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype IDs */
  ...
  if (!(nfilters > 0 && (H5Tdetect_class(tid, H5T_VLEN) ||
                         (H5Tdetect_class(tid, H5T_REFERENCE) &&
                          H5Tequal(tid, H5T_STD_REF_DSETREG))))) {
    hsize_t storage_size =
        H5Dget_storage_size(did); /* Dataset's raw data storage size */
    hsize_t storage_size2 =
        H5Dget_storage_size(did2); /* 2nd Dataset's raw data storage size */
  ...
  if (wbuf) {
    if (!compare_data(did, (hid_t)0, pid, tid, (size_t)nelmts, wbuf, rbuf, did))
      TEST_ERROR;
    if (!compare_data(did2, (hid_t)0, pid, tid2, (size_t)nelmts, wbuf, rbuf2,
                      did2))
      TEST_ERROR;
  } /* end if */
...
static int compare_groups(hid_t gid, hid_t gid2, hid_t pid, int depth,
  ...
  if (ginfo2.nlinks > 0) {
    ...
    for (idx = 0; idx < ginfo.nlinks; idx++) {
      ...
      if (H5Lget_name_by_idx(gid, ".", H5_INDEX_NAME, H5_ITER_INC, idx, objname,
                             (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (H5Lget_name_by_idx(gid2, ".", H5_INDEX_NAME, H5_ITER_INC, idx,
                             objname2, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
        TEST_ERROR;
      ...
      if (linfo.type == H5L_TYPE_HARD) {
        ...
        if (H5Oget_info_by_name3(gid, objname, &oinfo, H5O_INFO_BASIC,
                                 H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5Oget_info_by_name3(gid2, objname2, &oinfo2, H5O_INFO_BASIC,
                                 H5P_DEFAULT) < 0)
          TEST_ERROR;

        /* Get native object info */
        if (H5Oget_native_info_by_name(gid, objname, &ninfo,
                                       H5O_NATIVE_INFO_HDR, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5Oget_native_info_by_name(gid2, objname2, &ninfo2,
                                       H5O_NATIVE_INFO_HDR, H5P_DEFAULT) < 0)
          TEST_ERROR;
        ...
        if (linfo.type == H5L_TYPE_SOFT ||
          ...
          if (H5Lget_val(gid, objname, linkval, (size_t)NAME_BUF_SIZE,
                         H5P_DEFAULT) < 0)
            TEST_ERROR;
          if (H5Lget_val(gid2, objname2, linkval2, (size_t)NAME_BUF_SIZE,
                         H5P_DEFAULT) < 0)
            TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_named_datatype(hid_t fcpl_src, hid_t fcpl_dst,
                                    hid_t src_fapl, hid_t dst_fapl) {
  ...
  /* create source file */
  if ((fid_src = H5Fcreate(src_filename, H5F_ACC_TRUNC, fcpl_src, src_fapl)) <
      0)
    TEST_ERROR;
  ...
  if ((H5Tcommit2(fid_src, NAME_DATATYPE_SIMPLE, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((fid_dst = H5Fcreate(dst_filename, H5F_ACC_TRUNC, fcpl_dst, dst_fapl)) <
      0)
    TEST_ERROR;

  /* Create an uncopied object in destination file so that tokens in source and
   * destination files aren't the same */
  if (H5Gclose(H5Gcreate2(fid_dst, NAME_GROUP_UNCOPIED, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* copy the datatype from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATATYPE_SIMPLE, fid_dst, NAME_DATATYPE_SIMPLE,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_named_datatype_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                       hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype IDs */
  char src_filename[NAME_BUF_SIZE];
  ...
  /* create named datatype */
  if ((H5Tcommit2(fid_src, NAME_DATATYPE_VL, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the datatype from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATATYPE_VL, fid_dst, NAME_DATATYPE_VL, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Create named vlen of vlen datatype in SRC file and copy it to
 *DST file
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_named_datatype_vl_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                          hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype IDs */
  char src_filename[NAME_BUF_SIZE];
  ...
  /* create named datatype */
  if ((H5Tcommit2(fid_src, NAME_DATATYPE_VL_VL, tid2, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the datatype from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATATYPE_VL_VL, fid_dst, NAME_DATATYPE_VL_VL,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_named_datatype_attr_self(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype IDs */
  ...
  /* create attribute */
  if ((aid = H5Acreate2(tid, "attr_self", tid, sid, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  if (!same_type)
    FAIL_PUTS_ERROR(
        "destination attribute does not use the same committed datatype");
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_simple(hid_t fcpl_src, hid_t fcpl_dst,
                                    hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_SIMPLE, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_dst, NAME_DATASET_SIMPLE,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int test_copy_dataset_versionbounds(hid_t fcpl_src, hid_t fapl_src) {
  ...
  did_src = H5Dcreate2(fid_src, NAME_DATASET_SIMPLE, H5T_NATIVE_INT, sid,
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      H5E_BEGIN_TRY {
        ret = H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_dst,
                      NAME_DATASET_SIMPLE, H5P_DEFAULT, H5P_DEFAULT);
      }
      H5E_END_TRY
...
static int test_copy_dataset_simple_samefile(hid_t fcpl, hid_t fapl) {
  ...
  /* create 2D int dataset at SRC file */
  if ((did = H5Dcreate2(fid, NAME_DATASET_SIMPLE, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid, NAME_DATASET_SIMPLE, fid, NAME_DATASET_SIMPLE2, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *
 *              Note: The parameter "test_open" is added to test for H5Ocopy
 *when the dataset is open in the file (HDFFV-7853).
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 *
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_simple_empty(hid_t fcpl_src, hid_t fcpl_dst,
                                          hid_t src_fapl, hid_t dst_fapl,
                                          hbool_t test_open) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compound(hid_t fcpl_src, hid_t fcpl_dst,
                                      hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_COMPOUND, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_COMPOUND, fid_dst, NAME_DATASET_COMPOUND,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_chunked(hid_t fcpl_src, hid_t fcpl_dst,
                                     hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED, H5T_NATIVE_FLOAT, sid,
                        H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED_SINGLE, H5T_NATIVE_FLOAT,
                        sid, H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED2, H5T_NATIVE_FLOAT, sid,
                        H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED3, H5T_NATIVE_FLOAT, sid,
                        H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED2_SINGLE, H5T_NATIVE_FLOAT,
                        sid, H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED3_SINGLE, H5T_NATIVE_FLOAT,
                        sid, H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /*
   * Create 2-D dataset: chunked, non-filterd, with data, dims=max dims=chunk
   * dims, H5D_ALLOC_TIME_LATE
   */
  /* create 2-D dataspace */
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED4_SINGLE, H5T_NATIVE_FLOAT,
                        sid, H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED, fid_dst, NAME_DATASET_CHUNKED,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED2, fid_dst, NAME_DATASET_CHUNKED2,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED3, fid_dst, NAME_DATASET_CHUNKED3,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;

  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED_SINGLE, fid_dst,
              NAME_DATASET_CHUNKED_SINGLE, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED2_SINGLE, fid_dst,
              NAME_DATASET_CHUNKED2_SINGLE, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED3_SINGLE, fid_dst,
              NAME_DATASET_CHUNKED3_SINGLE, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED4_SINGLE, fid_dst,
              NAME_DATASET_CHUNKED4_SINGLE, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (compare_idx_type(src_fapl, did2, H5D_CHUNK_IDX_EARRAY,
                       H5D_CHUNK_IDX_BTREE) != TRUE)
    TEST_ERROR;
  ...
  /* Check if the array index type is correct */
  if (compare_idx_type(src_fapl, did2, H5D_CHUNK_IDX_SINGLE,
                       H5D_CHUNK_IDX_BTREE) != TRUE)
    TEST_ERROR;
  ...
  /* Check if the array index type is correct */
  if (compare_idx_type(src_fapl, did2, H5D_CHUNK_IDX_FARRAY,
                       H5D_CHUNK_IDX_BTREE) != TRUE)
    TEST_ERROR;
  ...
  /* Check if the array index type is correct */
  if (compare_idx_type(src_fapl, did2, H5D_CHUNK_IDX_NONE,
                       H5D_CHUNK_IDX_BTREE) != TRUE)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_chunked_empty(hid_t fcpl_src, hid_t fcpl_dst,
                                           hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t pid = -1;                   /* Dataset creation property list ID */
 ...
 *
 * Purpose:     Create a chunked dataset with unlimited dimensions and
 *un-written chunks in SRC file and copy it to DST file
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 *
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_chunked_sparse(hid_t fcpl_src, hid_t fcpl_dst,
                                            hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t pid = -1;                   /* Dataset creation property list ID */
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED4, H5T_NATIVE_FLOAT, sid,
                        H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED3, fid_dst, NAME_DATASET_CHUNKED3,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED4, fid_dst, NAME_DATASET_CHUNKED4,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Check if the array index type is correct */
  if (compare_idx_type(src_fapl, did2, H5D_CHUNK_IDX_BT2,
                       H5D_CHUNK_IDX_BTREE) != TRUE)
    TEST_ERROR;
 ...
 *
 * Purpose:     Create a compressed, chunked dataset in SRC file and copy it to
 *DST file
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compressed(hid_t fcpl_src, hid_t fcpl_dst,
                                        hid_t src_fapl, hid_t dst_fapl) {
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_no_edge_filt(hid_t fcpl_src, hid_t fcpl_dst,
                                          hid_t src_fapl, hid_t dst_fapl,
                                          hbool_t test_open) {
#ifdef H5_HAVE_FILTER_DEFLATE
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  ...
  /* H5Pset_chunk_opts() will set layout version to 4 which will use latest
   * indexing available */
  if (compare_idx_type(src_fapl, did2, H5D_CHUNK_IDX_FARRAY,
                       H5D_CHUNK_IDX_FARRAY) != TRUE)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compact(hid_t fcpl_src, hid_t fcpl_dst,
                                     hid_t src_fapl, hid_t dst_fapl,
                                     hbool_t test_open) {
  hid_t fid_src = -1, fid_dst = -1;  /* File IDs */
  hid_t sid = -1;                    /* Dataspace ID */
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_COMPACT, H5T_NATIVE_FLOAT, sid,
                        H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_COMPACT, fid_dst, NAME_DATASET_COMPACT,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_external(hid_t fcpl_src, hid_t fcpl_dst,
                                      hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t pid = -1;                   /* Dataset creation property list ID */
  ...
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_EXTERNAL, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_EXTERNAL, fid_dst, NAME_DATASET_EXTERNAL,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_named_dtype(hid_t fcpl_src, hid_t fcpl_dst,
                                         hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_NAMED_DTYPE, tid, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_NAMED_DTYPE, fid_dst,
              NAME_DATASET_NAMED_DTYPE, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_named_dtype_hier(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  if ((gid = H5Gcreate2(fid_src, NAME_GROUP_TOP, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(gid, NAME_DATATYPE_SIMPLE, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create first dataset at SRC file */
  if ((did = H5Dcreate2(gid, NAME_DATASET_NAMED_DTYPE, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid, NAME_DATASET_NAMED_DTYPE2, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_GROUP_TOP, fid_dst, NAME_GROUP_TOP, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_named_dtype_hier_outside(hid_t fcpl_src,
                                                      hid_t fcpl_dst,
                                                      hid_t src_fapl,
                                                      hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_multi_ohdr_chunks(hid_t fcpl_src, hid_t fcpl_dst,
                                               hid_t src_fapl, hid_t dst_fapl,
                                               hbool_t test_open) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create first dataset at SRC file */
  if ((did = H5Dcreate2(gid, NAME_DATASET_MULTI_OHDR, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create second dataset at SRC file */
  if ((did2 = H5Dcreate2(gid, NAME_DATASET_MULTI_OHDR2, H5T_NATIVE_INT, sid,
                         H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_attr_named_dtype(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create first dataset at SRC file */
  if ((did = H5Dcreate2(gid, NAME_DATASET_MULTI_OHDR, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create second dataset at SRC file */
  if ((did2 = H5Dcreate2(gid, NAME_DATASET_MULTI_OHDR2, tid, sid, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_contig_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                       hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_VL, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_VL, fid_dst, NAME_DATASET_VL, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_chunked_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                        hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_VL, tid, sid, H5P_DEFAULT, pid,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_VL2, tid, sid, H5P_DEFAULT, pid,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_DATASET_VL, fid_dst, NAME_DATASET_VL, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_VL2, fid_dst, NAME_DATASET_VL2, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compact_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                        hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_attribute_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                  hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t did = -1, did2 = -1;        /* Dataset IDs */
  ...
  if ((aid = H5Aopen_by_idx(did, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                            (hsize_t)0, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((aid2 = H5Aopen_by_idx(did2, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                             (hsize_t)0, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Attach a compound datatype with a variable length string to the
 *object
 *
 * Return:      Non-negative on success/Negative on failure
 *
...
static int attach_attribute_compound_vlstr(hid_t loc_id) {
  ...
  if ((aid = H5Acreate2(loc_id, "attr_cmpd_vlstr", cmpd_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto done;
...
static int compare_attribute_compound_vlstr(hid_t loc, hid_t loc2) {
  ...
  if ((aid = H5Aopen_by_idx(loc, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                            (hsize_t)0, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((aid2 = H5Aopen_by_idx(loc2, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                             (hsize_t)0, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static int test_copy_attribute_compound_vlstr(hid_t fcpl_src, hid_t fcpl_dst,
  ...
  TESTING("H5Ocopy(): attribute with compound datatype consisting of variable "
          "length string");
  ...
  /* create a group */
  if ((gid = H5Gcreate2(fid_src, NAME_GROUP_EMPTY, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Copy the group */
  if (H5Ocopy(fid_src, NAME_GROUP_EMPTY, fid_dst, NAME_GROUP_EMPTY, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compressed_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                           hid_t src_fapl, hid_t dst_fapl) {
#ifdef H5_HAVE_FILTER_DEFLATE
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED, tid, sid, H5P_DEFAULT,
                        pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_group_empty(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                                 hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t gid = -1, gid2 = -1;        /* Group IDs */
  char src_filename[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_root_group(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                                hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t did = -1;                   /* Dataset ID */
  ...
  if ((did = H5Dcreate2(fid_src, NAME_GROUP_DATASET, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
    TEST_ERROR;
  ...
  /* create a sub-group */
  if ((gid_sub = H5Gcreate2(fid_src, NAME_GROUP_SUB, H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((gid_sub = H5Gcreate2(fid_src, NAME_GROUP_SUB_2, H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid_sub) < 0)
    TEST_ERROR;
  ...
  /* copy the group from SRC to DST */
  if (H5Ocopy(fid_src, "/", fid_dst, "/root_from_src", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_group(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                           hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t did = -1;                   /* Dataset ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_group_deep(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                                hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t did = -1;                   /* Dataset ID */
  ...
  for (i = 0; i < NUM_SUB_GROUPS; i++) {
    ...
    if ((gid_sub = H5Gcreate2(gid, objname, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
      TEST_ERROR;

    for (j = 0; j < NUM_SUB_GROUPS; j++) {
      HDsnprintf(objname, sizeof(objname), "Group #%d", j);
      if ((gid_sub2 = H5Gcreate2(gid_sub, objname, H5P_DEFAULT, H5P_DEFAULT,
                                 H5P_DEFAULT)) < 0)
        TEST_ERROR;
      ...
      for (k = 0; k < NUM_DATASETS; k++) {
        ...
        /* add a dataset to the group */
        if ((did = H5Dcreate2(gid_sub2, objname, H5T_NATIVE_INT, sid,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
          TEST_ERROR;
        if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) <
            0)
          TEST_ERROR;
        if (H5Dclose(did) < 0)
          TEST_ERROR;
 ...
 *
 * Purpose:     Create a group hier. with loops in SRC file and copy it to DST
 *file
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_group_loop(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                                hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1;  /* File IDs */
  hid_t gid = -1, gid2 = -1;         /* Group IDs */
  ...
  /* create sub-groups */
  if ((gid_sub = H5Gcreate2(gid, NAME_GROUP_SUB, H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
    TEST_ERROR;

  if ((gid_sub2 = H5Gcreate2(gid, NAME_GROUP_SUB_SUB, H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create link to top group */
  if (H5Lcreate_hard(gid, ".", gid_sub2, NAME_GROUP_LOOP, H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_group_wide_loop(hid_t fcpl_src, hid_t fcpl_dst,
                                     hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1;  /* File IDs */
  hid_t gid = -1, gid2 = -1;         /* Group IDs */
  hid_t gid_sub = -1, gid_sub2 = -1; /* Sub-group IDs */
  ...
  for (u = 0; u < NUM_WIDE_LOOP_GROUPS; u++) {
    ...
    for (v = 0; v < NUM_WIDE_LOOP_GROUPS; v++) {
      ...
      if (H5Lcreate_hard(gid_sub, ".", gid_sub2, NAME_GROUP_LOOP2, H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
        FAIL_STACK_ERROR;

      /* Create link to self :-) */
      if (H5Lcreate_hard(gid_sub2, ".", gid_sub2, NAME_GROUP_LOOP3, H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
        FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_group_links(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                                 hid_t dst_fapl) {
  ...
  if ((fid_ext = H5Fcreate(ext_filename, H5F_ACC_TRUNC, fcpl_src, src_fapl)) <
      0)
    TEST_ERROR;

  /* create groups at the SRC file.  Group 2 will hold dangling links. */
  if ((gid = H5Gcreate2(fid_src, NAME_GROUP_LINK, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((gid2 = H5Gcreate2(fid_src, NAME_GROUP_LINK2, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* add a dataset to the group */
  if ((did = H5Dcreate2(fid_src, NAME_LINK_DATASET, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(fid_ext, NAME_DATASET_SIMPLE, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_hard(fid_src, NAME_LINK_DATASET, H5L_SAME_LOC, NAME_LINK_HARD,
                     H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* make a soft link to the dataset */
  if (H5Lcreate_soft(NAME_LINK_DATASET, fid_src, NAME_LINK_SOFT, H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* make an external link to the external dataset */
  if (H5Lcreate_external(ext_filename, NAME_DATASET_SIMPLE, fid_src,
                         NAME_LINK_EXTERN, H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* make a dangling soft link */
  if (H5Lcreate_soft("nowhere", fid_src, NAME_LINK_SOFT_DANGLE2, H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* make a dangling external link */
  if (H5Lcreate_external("no_file", "no_object", fid_src,
                         NAME_LINK_EXTERN_DANGLE2, H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  for (expand_soft = 0; expand_soft <= 1; expand_soft++) {
    for (expand_ext = 0; expand_ext <= 1; expand_ext++) {
      ...
      /* create destination file */
      if ((fid_dst =
               H5Fcreate(dst_filename, H5F_ACC_TRUNC, fcpl_dst, dst_fapl)) < 0)
        TEST_ERROR;

      /* Create an uncopied object in destination file so that tokens in source
       * and destination files aren't the same */
      if (H5Gclose(H5Gcreate2(fid_dst, NAME_GROUP_UNCOPIED, H5P_DEFAULT,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0)
        TEST_ERROR;

      /* copy the group from SRC to DST */
      if (H5Ocopy(fid_src, NAME_GROUP_LINK, fid_dst, NAME_GROUP_LINK, plid,
                  H5P_DEFAULT) < 0)
        TEST_ERROR;
      ...
      if (expand_soft) {
        ...
        if (H5Ldelete(fid_dst, NAME_LINK_SOFT, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5Lcreate_soft(NAME_LINK_DATASET, fid_dst, NAME_LINK_SOFT,
                           H5P_DEFAULT, H5P_DEFAULT) < 0)
          TEST_ERROR;
      ...
      if (expand_ext) {
        ...
        if (H5Ldelete(fid_dst, NAME_LINK_EXTERN, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5Lcreate_external(ext_filename, NAME_DATASET_SIMPLE, fid_dst,
                               NAME_LINK_EXTERN, H5P_DEFAULT, H5P_DEFAULT) < 0)
          TEST_ERROR;
       ...
       * to hard links. */
      if (H5Ocopy(fid_src, NAME_GROUP_LINK2, fid_dst, NAME_GROUP_LINK2, plid,
                  H5P_DEFAULT) < 0)
        TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_soft_link(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_LINK_SOFT, fid_dst, NAME_DATASET_SIMPLE,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_ext_link(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                              hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1, fid_ext = -1; /* File IDs */
  hid_t sid = -1;                                 /* Dataspace ID */
  hid_t did = -1, did2 = -1;                      /* Dataset IDs */
  ...
  if ((gid = H5Gcreate2(fid_ext, NAME_GROUP_LINK, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create an external link to the dataset in the source file */
  if (H5Lcreate_external(src_filename, NAME_LINK_DATASET, fid_ext,
                         NAME_LINK_EXTERN, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_ext, NAME_LINK_EXTERN, fid_dst, NAME_DATASET_SIMPLE,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_exist(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                           hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  H5E_BEGIN_TRY {
    ret = H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_dst, NAME_DATASET_SIMPLE,
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_path(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                          hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t did = -1, did2 = -1;        /* Dataset IDs */
  ...
  /* copy the dataset from SRC to DST (should fail - intermediate groups not
   * there) */
  H5E_BEGIN_TRY {
    ret = H5Ocopy(fid_src, NAME_DATASET_SUB_SUB, fid_dst, NAME_DATASET_SUB_SUB,
                  H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  if ((gid = H5Gcreate2(fid_dst, NAME_GROUP_TOP, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;

  if ((gid = H5Gcreate2(fid_dst, NAME_GROUP_SUB, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;

  if ((gid = H5Gcreate2(fid_dst, NAME_GROUP_SUB_SUB, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_dst, NAME_DATASET_SUB_SUB,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int test_copy_same_file_named_datatype(hid_t fcpl_src, hid_t fapl) {
  ...
  /* create named datatype */
  if ((H5Tcommit2(fid, NAME_DATATYPE_SIMPLE, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* copy the datatype from SRC to DST */
  if (H5Ocopy(fid, NAME_DATATYPE_SIMPLE, fid, NAME_DATATYPE_SIMPLE2,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int test_copy_old_layout(hid_t fcpl_dst, hid_t fapl, hbool_t test_open) {
  ...
  const char *src_filename =
      H5_get_srcdir_filename(FILE_OLD_LAYOUT); /* Corrected test file name */
  char dst_filename[NAME_BUF_SIZE];
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_OLD_FORMAT, fid_dst, NAME_DATASET_SIMPLE,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compact_named_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_contig_named_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                             hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid_copy = -1;    /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_chunked_named_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl,
                                              hbool_t test_open) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid_copy = -1;    /* Datatype ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compressed_named_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                                 hid_t src_fapl,
                                                 hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid_copy = -1;    /* Datatype ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compact_vl_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                           hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_VL_VL, tid2, sid, H5P_DEFAULT,
                        pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_VL_VL, fid_dst, NAME_DATASET_VL_VL,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_contig_vl_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                          hid_t src_fapl, hid_t dst_fapl,
                                          hbool_t test_open) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype ID */
  ...
  if (test_open) {
    TESTING(
        "H5Ocopy(): contiguous and opened dataset with nested VLEN datatype");
  } else {
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_chunked_vl_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                           hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype ID */
  hid_t sid = -1;                   /* Dataspace ID */
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_VL_VL2, tid2, sid, H5P_DEFAULT,
                        pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_DATASET_VL_VL, fid_dst, NAME_DATASET_VL_VL,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_VL_VL2, fid_dst, NAME_DATASET_VL_VL2,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compressed_vl_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl,
                                              hbool_t test_open) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype ID */
  ...
  if (test_open) {
    TESTING(
        "H5Ocopy(): compressed and opened dataset with nested VLEN datatype");
  } else {
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_contig_cmpd_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                            hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_CMPD_VL, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy the dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_CMPD_VL, fid_dst, NAME_DATASET_CMPD_VL,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_chunked_cmpd_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                             hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype IDs */
  hid_t sid = -1;                   /* Dataspace ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_compact_cmpd_vl(hid_t fcpl_src, hid_t fcpl_dst,
                                             hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1, tid2 = -1;        /* Datatype IDs */
  hid_t sid = -1;                   /* Dataspace ID */
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_null_ref(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                              hid_t dst_fapl) {
  ...
  char zeros[MAX(sizeof(obj_buf),
                 sizeof(reg_buf))]; /* Array of zeros, for memcmp */
  char src_filename[NAME_BUF_SIZE];
  ...
  if ((did2 = H5Dcreate2(fid1, "reg_ref_dset", H5T_STD_REF_DSETREG, sid,
                         H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *              references are handled correctly.
 *
 *              Note: this is copied from test_copy_null_ref() with
 *modifications to test for opened datasets in the files.
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 *
 *-------------------------------------------------------------------------
 */
static int test_copy_null_ref_open(hid_t fcpl_src, hid_t fcpl_dst,
                                   hid_t src_fapl, hid_t dst_fapl) {
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_attr_crt_order(hid_t fcpl_src, hid_t fcpl_dst,
                                    hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid1 = -1, fid2 = -1; /* File IDs */
  ...
  if (H5Pset_attr_creation_order(gcplid, H5P_CRT_ORDER_TRACKED) < 0)
    TEST_ERROR;
  if ((gid1 = H5Gcreate2(fid1, NAME_GROUP_TOP, H5P_DEFAULT, gcplid,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_committed_datatype_merge(hid_t fcpl_src, hid_t fcpl_dst,
                                              hid_t src_fapl, hid_t dst_fapl,
                                              hbool_t reopen) {
  ...
  /* create source files */
  if ((fid_src1 = H5Fcreate(src1_filename, H5F_ACC_TRUNC, fcpl_src, src_fapl)) <
      0)
    TEST_ERROR;
  if ((fid_src2 = H5Fcreate(src2_filename, H5F_ACC_TRUNC, fcpl_src, src_fapl)) <
      0)
    TEST_ERROR;
  ...
  /* committed datatype */
  if ((H5Tcommit2(fid_src1, NAME_DATATYPE_SIMPLE, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src1, NAME_DATASET_SIMPLE, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* committed datatype */
  if ((H5Tcommit2(fid_src2, NAME_DATATYPE_SIMPLE, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src2, NAME_DATASET_SIMPLE, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy SRC1 to DST */
  if (H5Ocopy(fid_src1, "/", fid_dst, NAME_GROUP_TOP, ocpypl_id, H5P_DEFAULT) <
      0)
    TEST_ERROR;

  /* copy SRC2 to DST */
  if (H5Ocopy(fid_src2, "/", fid_dst, NAME_GROUP_TOP2, ocpypl_id, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid_dst, NAME_GROUP_TOP "/" NAME_DATATYPE_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_TOP "/" NAME_DATASET_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_TOP2 "/" NAME_DATASET_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  /*
   * Now copy only the datasets to the destination file, and verify the
   * committed datatypes are merged
   */
  ...
  /* copy SRC1 to DST */
  if (H5Ocopy(fid_src1, NAME_DATASET_SIMPLE, fid_dst, NAME_DATASET_SIMPLE,
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* copy SRC2 to DST */
  if (H5Ocopy(fid_src2, NAME_DATASET_SIMPLE, fid_dst, NAME_DATASET_SIMPLE2,
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_committed_datatype_merge_same_file(hid_t fcpl, hid_t fapl,
                                                        hbool_t reopen) {
  hid_t fid = -1;        /* File ID */
  hid_t tid = -1;        /* Datatype ID */
  hid_t sid = -1;        /* Dataspace ID */
   ...
   */
  /* Create group */
  if ((gid = H5Gcreate2(fid, NAME_GROUP_TOP, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* committed datatype */
  if ((H5Tcommit2(fid, NAME_GROUP_TOP "/" NAME_DATATYPE_SIMPLE, tid,
                  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create dataset */
  if ((did = H5Dcreate2(fid, NAME_GROUP_TOP "/" NAME_DATASET_SIMPLE, tid, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
   ...
   */
  /* Create group */
  if ((gid = H5Gcreate2(fid, NAME_GROUP_TOP2, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* committed datatype */
  if ((H5Tcommit2(fid, NAME_GROUP_TOP2 "/" NAME_DATATYPE_SIMPLE, tid,
                  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create dataset */
  if ((did = H5Dcreate2(fid, NAME_GROUP_TOP2 "/" NAME_DATASET_SIMPLE, tid, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((gid = H5Gcreate2(fid, NAME_GROUP_TOP3, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  ...
  /* copy group 1 to DST */
  if (H5Ocopy(fid, NAME_GROUP_TOP, fid, NAME_GROUP_TOP3 "/" NAME_GROUP_TOP,
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* copy group 2 to DST */
  if (H5Ocopy(fid, NAME_GROUP_TOP2, fid, NAME_GROUP_TOP3 "/" NAME_GROUP_TOP2,
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid, NAME_GROUP_TOP "/" NAME_DATATYPE_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid, NAME_GROUP_TOP "/" NAME_DATASET_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(
           fid, NAME_GROUP_TOP3 "/" NAME_GROUP_TOP "/" NAME_DATATYPE_SIMPLE,
           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(
           fid, NAME_GROUP_TOP3 "/" NAME_GROUP_TOP "/" NAME_DATASET_SIMPLE,
           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid, NAME_GROUP_TOP2 "/" NAME_DATATYPE_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid, NAME_GROUP_TOP2 "/" NAME_DATASET_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(
           fid, NAME_GROUP_TOP3 "/" NAME_GROUP_TOP2 "/" NAME_DATATYPE_SIMPLE,
           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(
           fid, NAME_GROUP_TOP3 "/" NAME_GROUP_TOP2 "/" NAME_DATASET_SIMPLE,
           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  /*
   * Now copy only the datasets to the destination group, and verify the
   * committed datatypes are merged as expected
   */
  /* Create destination group */
  if ((gid = H5Gcreate2(fid, NAME_GROUP_TOP4, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  ...
  /* copy SRC1 to DST */
  if (H5Ocopy(fid, NAME_GROUP_TOP "/" NAME_DATASET_SIMPLE, fid,
              NAME_GROUP_TOP4 "/" NAME_DATASET_SIMPLE, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* copy SRC2 to DST */
  if (H5Ocopy(fid, NAME_GROUP_TOP2 "/" NAME_DATASET_SIMPLE, fid,
              NAME_GROUP_TOP4 "/" NAME_DATASET_SIMPLE2, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Open group 1 dset dtype, check token */
  if ((did = H5Dopen2(fid, NAME_GROUP_TOP4 "/" NAME_DATASET_SIMPLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  /* Open group 2 dset dtype, check token */
  if ((did = H5Dopen2(fid, NAME_GROUP_TOP4 "/" NAME_DATASET_SIMPLE2,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_committed_dt_merge_sugg(hid_t fcpl_src, hid_t fcpl_dst,
                                             hid_t src_fapl, hid_t dst_fapl,
                                             hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  if (reopen)
    TESTING(
        "H5Ocopy(): merging committed datatypes with suggestions and reopen");
  else
  ...
  /* create dataset at SRC file */
  if ((did = H5Dcreate2(fid_src, NAME_DATASET_SIMPLE, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((H5Tcommit2(fid_dst, "/a", tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  if ((H5Tcommit2(fid_dst, "/b", tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  /* copy SRC dset to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_dst, NAME_DATASET_SIMPLE2,
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_committed_dt_merge_attr(hid_t fcpl_src, hid_t fcpl_dst,
                                             hid_t src_fapl, hid_t dst_fapl,
                                             hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  if (reopen)
    TESTING(
        "H5Ocopy(): merging committed datatypes with attributes and reopen");
  else
  ...
  if ((aid = H5Aopen_by_name(fid_dst, NAME_GROUP_TOP, "attr", H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Aget_type(aid)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_cdt_hier_merge(hid_t fcpl_src, hid_t fcpl_dst,
                                    hid_t src_fapl, hid_t dst_fapl,
                                    hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1;           /* File IDs */
  hid_t tid = -1;                             /* Datatype ID */
  ...
  if ((f_tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_src, ROOT_NDT_INT, f_tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((g_tid = H5Tcopy(H5T_NATIVE_SHORT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(gid, GROUP_NDT_SHORT, g_tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid, SRC_DSET1, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid, SRC_NDT_DSET2, g_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid, SRC_NDT_DSET3, f_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((gid = H5Gcreate2(fid_dst, NAME_GROUP_UNCOPIED, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(gid, DST_ATTR_ANON_SHORT, anon_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(gid, DST_ATTR_ANON_INT, anon_tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
   ...
   */
  if (H5Ocopy(fid_src, "/", fid_dst, SRC_ROOT_GROUP, ocpypl_id, H5P_DEFAULT) <
      0)
    TEST_ERROR;

  /* get token of committed datatype at root group */
  if ((tid = H5Topen2(fid_dst, SRC_ROOT_GROUP "/" ROOT_NDT_INT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((tid =
           H5Topen2(fid_dst, SRC_ROOT_GROUP NAME_GROUP_TOP "/" GROUP_NDT_SHORT,
                    H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid_dst, SRC_ROOT_GROUP NAME_GROUP_TOP "/" SRC_DSET1,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid_dst, SRC_ROOT_GROUP NAME_GROUP_TOP "/" SRC_NDT_DSET2,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid_dst, SRC_ROOT_GROUP NAME_GROUP_TOP "/" SRC_NDT_DSET3,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
   ...
   */
  if (H5Ocopy(fid_src, NAME_GROUP_TOP, fid_dst, NAME_GROUP_TOP, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* get token of committed datatype at /g0 */
  if ((tid = H5Topen2(fid_dst, NAME_GROUP_TOP "/" GROUP_NDT_SHORT,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  /* check token of datatype for second dataset */
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_TOP "/" SRC_NDT_DSET2, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  /* check token of datatype for third dataset */
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_TOP "/" SRC_NDT_DSET3, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
   ...
   */
  if (H5Ocopy(fid_src, NAME_GROUP_TOP "/" SRC_DSET1, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_DSET1, ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_GROUP_TOP "/" SRC_NDT_DSET2, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET2, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_GROUP_TOP "/" SRC_NDT_DSET3, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET3, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Open attribute with anon ndt (short), get token */
  if ((aid = H5Aopen_by_name(fid_dst, NAME_GROUP_UNCOPIED, DST_ATTR_ANON_SHORT,
                             H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Aget_type(aid)) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Aopen_by_name(fid_dst, NAME_GROUP_UNCOPIED, DST_ATTR_ANON_INT,
                             H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Aget_type(aid)) < 0)
    TEST_ERROR;
  ...
  /* verify the datatype of first dataset is not committed */
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_UNCOPIED "/" SRC_DSET1,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  /* check token of datatype for second dataset */
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET2,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  /* check token of datatype for third dataset */
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET3,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
 ...
 *            Create committed datatype (double), with attribute of anon ndt
 *short DST file: Create committed datatype (int) Create committed datatype
 *(float), with attribute of native int Create committed datatype (double), with
 *attribute of anon ndt short
 *
 *        Copy / at SRC to DST
 *        Verify that committed datatypes are copied and merged correctly
 *
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_cdt_merge_cdt(hid_t fcpl_src, hid_t fcpl_dst,
                                   hid_t src_fapl, hid_t dst_fapl,
                                   hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  ...
  if ((tid1 = H5Tcopy(H5T_NATIVE_SHORT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_src, SRC_NDT_SHORT, tid1, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid2 = H5Tcopy(H5T_NATIVE_FLOAT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_src, SRC_NDT_FLOAT, tid2, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid3 = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_src, SRC_NDT_INT, tid3, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create an attribute of committed datatype (int); attach to committed
   * datatype (int) */
  if ((aid = H5Acreate2(tid3, SRC_ATTR, tid3, sid, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((tid4 = H5Tcopy(H5T_NATIVE_DOUBLE)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_src, SRC_NDT_DOUBLE, tid4, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create an attribute of anon ndt (short); attach to committed datatype
   * (double) */
  if ((aid = H5Acreate2(tid4, SRC_ATTR, tid5, sid, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((tid1 = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_dst, DST_NDT_INT, tid1, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid2 = H5Tcopy(H5T_NATIVE_FLOAT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_dst, DST_NDT_FLOAT, tid2, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create an attribute of native integer; attach to committed datatype (float)
   */
  if ((aid = H5Acreate2(tid2, DST_ATTR, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((tid3 = H5Tcopy(H5T_NATIVE_DOUBLE)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_dst, DST_NDT_DOUBLE, tid3, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(tid3, DST_ATTR, tid4, sid, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid_dst, "/" SRC_ROOT_GROUP "/" SRC_NDT_DOUBLE,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid_dst, "/" SRC_ROOT_GROUP "/" SRC_NDT_FLOAT,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid_dst, "/" SRC_ROOT_GROUP "/" SRC_NDT_INT,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid_dst, "/" SRC_ROOT_GROUP "/" SRC_NDT_SHORT,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
   ...
   * /src_root/src_ndt_double/dst_attr */
  if ((aid = H5Aopen_by_name(fid_dst, "/" SRC_ROOT_GROUP "/" SRC_NDT_DOUBLE,
                             DST_ATTR, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Aget_type(aid)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
 ...
 *
 * Purpose:     Tests the suggested searching paths feature
 *(H5Padd_merge_committed_dtype_path) is correctly applied in merging the
 *committed datatypes.
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_cdt_merge_suggs(hid_t fcpl_src, hid_t fcpl_dst,
                                     hid_t src_fapl, hid_t dst_fapl,
                                     hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_src, SRC_NDT_INT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_dst, DST_NDT_INT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, SRC_NDT_INT, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_NDT_INT, ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid_dst, NAME_GROUP_UNCOPIED "/" SRC_NDT_INT,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
  ...
  if (H5Padd_merge_committed_dtype_path(ocpypl_id, NAME_GROUP_UNCOPIED
                                        "/" SRC_NDT_INT) < 0)
    TEST_ERROR;

  /* copy "/src_ndt_int" from SRC file to "/src_ndt_int" at DST file */
  if (H5Ocopy(fid_src, SRC_NDT_INT, fid_dst, SRC_NDT_INT, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* copy with merging and search suggestion: "/dst_ndt_int,
   * /uncopied/src_ndt_int" */
  if (H5Ocopy(fid_src, SRC_NDT_INT, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_NDT_INT2, ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Topen2(fid_dst, NAME_GROUP_UNCOPIED "/" SRC_NDT_INT2,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Oget_info3(tid, &oinfo, H5O_INFO_BASIC) < 0)
    TEST_ERROR;
 ...
 * Purpose:     Tests the suggested searching paths feature
 *(H5Padd_merge_committed_dtype_path) is correctly applied in merging the
 *committed datatypes of datasets.
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 *
 *-------------------------------------------------------------------------
 */
static int test_copy_cdt_merge_dset_suggs(hid_t fcpl_src, hid_t fcpl_dst,
                                          hid_t src_fapl, hid_t dst_fapl,
                                          hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  if (reopen)
    TESTING("H5Ocopy(): merging committed datatypes of datasets with "
            "suggestions and reopen");
  else
    TESTING(
        "H5Ocopy(): merging committed datatypes of datasets with suggestions");
  ...
  /* create dataset */
  if ((did = H5Dcreate2(fid_src, SRC_NDT_DSET, tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
   ...
   */
  if (H5Ocopy(fid_src, SRC_NDT_DSET, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET, ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((tid = H5Dget_type(did)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, SRC_NDT_DSET, fid_dst, SRC_NDT_DSET, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Open committed dtype dataset "/uncopied/src_ndt_dset", get its datatype
   * token */
  if ((did = H5Dopen2(fid_dst, NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET,
                      H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* use default ocpypl_id -- without merging and suggestion */
  if (H5Ocopy(fid_src, SRC_NDT_DSET, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET, H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, SRC_NDT_DSET, fid_dst, SRC_NDT_DSET2, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* use merging and suggested search paths: "/src_ndt_dset,
   * /uncopied/src_ndt_dset" */
  if (H5Ocopy(fid_src, SRC_NDT_DSET, fid_dst,
              NAME_GROUP_UNCOPIED "/" SRC_NDT_DSET2, ocpypl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Tests the merging committed datatype + search suggestion
 *feature.
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_cdt_merge_all_suggs(hid_t fcpl_src, hid_t fcpl_dst,
                                         hid_t src_fapl, hid_t dst_fapl,
                                         hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t gid = -1;                   /* Group ID */
  ...
  if (reopen)
    ...
    TESTING(
        "H5Ocopy(): merging different committed datatypes with suggestions");
  ...
  /* create a group */
  if ((gid = H5Gcreate2(fid_src, SRC_GRP, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_SHORT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(gid, SRC_NDT_SHORT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create first dataset in group */
  if ((did = H5Dcreate2(gid, SRC_NDT_DSET, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(gid, SRC_NDT_INT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create second dataset in group */
  if ((did = H5Dcreate2(gid, SRC_NDT_DSET2, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid, SRC_DSET, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* get datatype */
  if ((tid_short =
           H5Topen2(fid_src, "/" SRC_GRP "/" SRC_NDT_SHORT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create an attribute attached to the dataset */
  if ((aid = H5Acreate2(did, SRC_ATTR, tid_short, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_SHORT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_dst, DST_NDT_SHORT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Tclose(tid) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_FLOAT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_dst, DST_NDT_FLOAT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create an attribute attached to committed datatype */
  if ((aid = H5Acreate2(tid, DST_ATTR, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  /* create a group */
  if ((gid = H5Gcreate2(fid_dst, DST_GRP, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(gid, DST_NDT_INT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(gid, DST_ATTR2, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* create a group */
  if ((gid = H5Gcreate2(fid_dst, DST_GRP2, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_SHORT)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(gid, DST_NDT_SHORT, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_DOUBLE)) < 0)
    TEST_ERROR;
  if ((H5Tcommit2(fid_dst, DST_NDT_DOUBLE, tid, H5P_DEFAULT, H5P_DEFAULT,
                  H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* get datatype */
  if ((tid_short =
           H5Topen2(fid_dst, "/" DST_GRP2 "/" DST_NDT_SHORT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* create an attribute attached to committed datatype */
  if ((aid = H5Acreate2(tid, DST_ATTR, tid_short, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, "/" SRC_GRP "/" SRC_NDT_DSET2, fid_dst, "A_src_dset2",
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Aopen_by_name(fid_dst, DST_GRP, DST_ATTR, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* copy "/src_grp/src_ndt_dset2" from SRC file to DST file */
  if (H5Ocopy(fid_src, "/" SRC_GRP "/" SRC_NDT_DSET2, fid_dst, "B_src_dset2",
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* get datatype for attribute attached to the group */
  if ((aid = H5Aopen_by_name(fid_dst, DST_GRP2, DST_ATTR, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((exp_tid = H5Aget_type(aid)) < 0)
    TEST_ERROR;
  ...
  if (H5Padd_merge_committed_dtype_path(ocpypl_id,
                                        "/" DST_GRP "/" DST_NDT_INT) < 0)
    TEST_ERROR;

  /* copy "/src_grp/src_ndt_dset2" from SRC file to DST file */
  if (H5Ocopy(fid_src, "/" SRC_GRP "/" SRC_NDT_DSET2, fid_dst, "C_src_dset2",
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((exp_tid = H5Topen2(fid_dst, "/" DST_GRP "/" DST_NDT_INT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  /* copy "/src_grp/src_ndt_dset2" from SRC file to DST file */
  if (H5Ocopy(fid_src, "/" SRC_GRP "/" SRC_NDT_DSET2, fid_dst, "D_src_dset2",
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, "/" SRC_GRP "/" SRC_NDT_DSET, fid_dst, "A_src_dset",
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Aopen_by_name(fid_dst, DST_NDT_DOUBLE, DST_ATTR, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((exp_tid = H5Aget_type(aid)) < 0)
    TEST_ERROR;
  ...
  /* copy "/src_grp/src_ndt_dset" from SRC file to DST file */
  if (H5Ocopy(fid_src, "/" SRC_GRP "/" SRC_NDT_DSET, fid_dst, "B_src_dset",
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int test_copy_set_mcdt_search_cb(hid_t fcpl_src, hid_t fcpl_dst,
                                        hid_t src_fapl, hid_t dst_fapl,
                                        hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  /*
   * Stop the search, default action is to create an anonymous committed
   * datatype
   */
  ...
  /* copy SRC dset to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_dst, NAME_DATASET_SIMPLE3,
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /*
   * Stop the search, default action is to create an anonymous committed
   * datatype. Disable suggestion list.
   */
  cb_udata.search_action = H5O_MCDT_SEARCH_STOP;
  cb_udata.called = 0;
...
static int test_copy_set_get_mcdt_search_cb(hid_t fcpl_src, hid_t fcpl_dst,
                                            hid_t src_fapl, hid_t dst_fapl,
                                            hbool_t reopen) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  H5O_mcdt_search_ret_t *mcdt_udata_p =
      NULL; /* Pointer to user data for callback */
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_copy_iterate_cb(hid_t loc_id, const char *name,
                                   const H5L_info2_t H5_ATTR_UNUSED *link_info,
                                   void *op_data) {
...
static int test_copy_cdt_v1_header_bug(hid_t fcpl_src, hid_t src_fapl) {
  ...
  if ((file_id = H5Fcreate(src_filename, H5F_ACC_TRUNC, fcpl_src, src_fapl)) <
      0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(file_id, "committed_str_type", type_id, H5P_DEFAULT,
                 H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(file_id, "committed_str_type", file_id, "committed_str_type2",
              ocpypl_id, H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int test_copy_iterate(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                             hid_t dst_fapl) {
  hid_t fid1 = -1, fid2 = -1; /* File IDs */
  hid_t gid = -1;             /* Group ID */
  int i;
  ...
  if ((gid = H5Gopen2(fid1, "/", H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Literate2(gid, H5_INDEX_NAME, H5_ITER_INC, NULL, test_copy_iterate_cb,
                  &fid2) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy_option(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl,
                            hid_t dst_fapl, unsigned flag,
                            hbool_t crt_intermediate_grp,
                            const char *test_desciption) {
  hid_t fid_src = -1, fid_dst = -1, fid_ext = -1; /* File IDs */
  hid_t sid = -1;                                 /* Dataspace ID */
  hid_t did = -1;                                 /* Dataset ID */
  ...
  if ((did = H5Dcreate2(gid, NAME_DATASET_SIMPLE, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid_sub, NAME_DATASET_SIMPLE, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
    TEST_ERROR;
  ...
  if ((gid_sub_sub = H5Gcreate2(gid_sub, NAME_GROUP_SUB_SUB2, H5P_DEFAULT,
                                H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* add a dataset to the sub sub group */
  if ((did = H5Dcreate2(gid_sub_sub, NAME_DATASET_SIMPLE, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
    TEST_ERROR;
  ...
  if ((flag & H5O_COPY_EXPAND_SOFT_LINK_FLAG) > 0) {
    ...
    if ((gid = H5Gcreate2(fid_src, NAME_GROUP_LINK, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      FAIL_STACK_ERROR;
    if (H5Lcreate_soft(NAME_DATASET_SUB_SUB, fid_src, NAME_LINK_SOFT,
                       H5P_DEFAULT, H5P_DEFAULT) < 0)
      FAIL_STACK_ERROR;
    if (H5Lcreate_soft("nowhere", fid_src, NAME_LINK_SOFT_DANGLE, H5P_DEFAULT,
                       H5P_DEFAULT) < 0)
      FAIL_STACK_ERROR;
    if (H5Gclose(gid) < 0)
      FAIL_STACK_ERROR;
    ...
    if ((gid = H5Gcreate2(fid_src, NAME_GROUP_LINK2, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      FAIL_STACK_ERROR;
    if (H5Lcreate_hard(fid_src, NAME_DATASET_SUB_SUB, H5L_SAME_LOC,
                       NAME_LINK_SOFT2, H5P_DEFAULT, H5P_DEFAULT) < 0)
      FAIL_STACK_ERROR;
    if (H5Lcreate_soft("nowhere", fid_src, NAME_LINK_SOFT_DANGLE2, H5P_DEFAULT,
                       H5P_DEFAULT) < 0)
      FAIL_STACK_ERROR;
  ...
  if ((flag & H5O_COPY_EXPAND_EXT_LINK_FLAG) > 0) {
    ...
    if (H5Lcreate_external(ext_filename, NAME_DATASET_SIMPLE, fid_src,
      ...
      TEST_ERROR;
    if (H5Lcreate_external("no_file", "no_object", fid_src,
                           NAME_LINK_EXTERN_DANGLE, H5P_DEFAULT,
                           H5P_DEFAULT) < 0)
      TEST_ERROR;
    if (H5Gclose(gid) < 0)
      TEST_ERROR;
    ...
    if (!(flag & H5O_COPY_EXPAND_SOFT_LINK_FLAG)) {
      ...
      TEST_ERROR;
    if ((did = H5Dcreate2(fid_src, NAME_LINK_EXTERN2, H5T_NATIVE_INT, sid,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
      TEST_ERROR;
    if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf) < 0)
      TEST_ERROR;
  ...
  if ((flag & H5O_COPY_EXPAND_REFERENCE_FLAG) > 0) {
    if ((gid_ref = H5Gcreate2(fid_src, NAME_GROUP_REF, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
      TEST_ERROR;
  ...
  if (crt_intermediate_grp) {
    ...
    if (H5Ocopy(fid_src, NAME_GROUP_TOP, fid_dst, "/new_g0/new_g00", pid,
                lcpl_id) < 0)
      TEST_ERROR;
    ...
    if (H5Ocopy(fid_src, NAME_GROUP_LINK, fid_dst, NAME_GROUP_LINK, pid,
                H5P_DEFAULT) < 0)
      TEST_ERROR;
    ...
    if (H5Ocopy(fid_src, NAME_GROUP_TOP, fid_dst, NAME_GROUP_TOP, pid,
                H5P_DEFAULT) < 0)
      TEST_ERROR;
    ...
    if (H5Ocopy(fid_src, NAME_GROUP_REF, fid_dst, NAME_GROUP_REF, pid,
                H5P_DEFAULT) < 0)
      TEST_ERROR;
 ...
 *
 * Purpose:     To ensure that H5Ocopy() copies data of opened dataset
 *correctly. This is for bug fix HDFFV-7853.
 *
 *        Test Case 1:
 ...
 *        Copy the opened dataset (with the opened committed datatype) to
 *another location in the same file Copy the opened dataset (with the opened
 *committed datatype) to DST file Close the dataset and the committed datatype
 *
 *        Test Case 4:
 *        Create a group with attributes, create a dataset in the group
 *        Copy the opened group (together with the opened dataset) to another
 *location in the same file Copy the opened group (together with the opened
 *dataset) to DST file Close the group and the dataset
 *
 * Return:      Success:        0
 *              Failure:        number of errors
 *
 *-------------------------------------------------------------------------
 */
static int test_copy_dataset_open(hid_t fcpl_src, hid_t fcpl_dst,
                                  hid_t src_fapl, hid_t dst_fapl) {
  hid_t fid_src = -1, fid_dst = -1; /* File IDs */
  hid_t sid = -1;                   /* Dataspace ID */
  hid_t tid = -1;                   /* Datatype ID */
  ...
  hsize_t chunk_dim2d[2] = {CHUNK_SIZE_1,
                            CHUNK_SIZE_2}; /* Chunk dimension sizes */
  int buf[DIM_SIZE_1][DIM_SIZE_2];         /* Buffer for writing data */
  ...
  if ((did2 = H5Dcreate2(fid_src, NAME_DATASET_CHUNKED, H5T_NATIVE_INT, sid,
                         H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_src, NAME_DATASET_SIMPLE2,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED, fid_src, NAME_DATASET_CHUNKED2,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
   ...
   */
  /* copy the opened dataset to another location in SRC file */
  if (H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_src, "NEW_DATASET_SIMPLE",
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED, fid_src, "NEW_DATASET_CHUNKED",
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
   ...
   */
  /* copy the opened dataset from SRC to DST */
  if (H5Ocopy(fid_src, NAME_DATASET_SIMPLE, fid_dst, "NEW_DATASET_SIMPLE",
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Ocopy(fid_src, NAME_DATASET_CHUNKED, fid_dst, "NEW_DATASET_CHUNKED",
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
   ...
   * Copy within the same file
   */
  /* copy the opened dataset (with the opened committed datatype) to another
   * location in SRC file */
  if (H5Ocopy(fid_src, NAME_DATASET_NAMED_DTYPE, fid_src,
              NAME_DATASET_NAMED_DTYPE2, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
   ...
   * Copy to another file
   */
  /* copy the opened dataset (with the opened committed datatype) from SRC to
   * DST */
  if (H5Ocopy(fid_src, NAME_DATASET_NAMED_DTYPE, fid_dst,
              NAME_DATASET_NAMED_DTYPE2, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
   ...
   * Copy within the same file
   */
  /* copy the opened group (together with opened dataset) to another location in
   * SRC file */
  if (H5Ocopy(fid_src, NAME_GROUP_TOP, fid_src, "COPIED_GROUP", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Ocopy(fid_src, NAME_GROUP_TOP, fid_dst, "COPIED_GROUP", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
...
int main(void) {
  ...
  /* Set the "use the latest version of the format" bounds for creating objects
   * in the file */
  ...
  for (configuration = 0; configuration <= MAX_CONFIGURATION; configuration++) {
    ...
    nerrors += test_copy_dataset_simple_empty(fcpl_src, fcpl_dst, src_fapl,
                                              dst_fapl, FALSE);
    nerrors += test_copy_dataset_simple_empty(fcpl_src, fcpl_dst, src_fapl,
                                              dst_fapl, TRUE);

    nerrors +=
        test_copy_dataset_compound(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
    nerrors +=
        test_copy_dataset_chunked(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
    nerrors +=
        test_copy_dataset_chunked_empty(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
    nerrors += test_copy_dataset_chunked_sparse(fcpl_src, fcpl_dst, src_fapl,
                                                dst_fapl);
    nerrors +=
        test_copy_dataset_compressed(fcpl_src, fcpl_dst, src_fapl, dst_fapl);

    /* Test with dataset opened in the file or not */
    nerrors += test_copy_dataset_no_edge_filt(fcpl_src, fcpl_dst, src_fapl,
                                              dst_fapl, FALSE);
    nerrors += test_copy_dataset_no_edge_filt(fcpl_src, fcpl_dst, src_fapl,
                                              dst_fapl, TRUE);
    ...
    nerrors += test_copy_dataset_compact(fcpl_src, fcpl_dst, src_fapl, dst_fapl,
                                         FALSE);
    nerrors +=
        test_copy_dataset_compact(fcpl_src, fcpl_dst, src_fapl, dst_fapl, TRUE);

    nerrors += test_copy_dataset_multi_ohdr_chunks(fcpl_src, fcpl_dst, src_fapl,
                                                   dst_fapl, FALSE);
    nerrors += test_copy_dataset_multi_ohdr_chunks(fcpl_src, fcpl_dst, src_fapl,
                                                   dst_fapl, TRUE);

    nerrors += test_copy_dataset_attr_named_dtype(fcpl_src, fcpl_dst, src_fapl,
                                                  dst_fapl);
    ...
    nerrors +=
        test_copy_group_wide_loop(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
    nerrors += test_copy_group_links(fcpl_src, fcpl_dst, src_fapl, dst_fapl);

    /* Test with dataset opened in the file or not */
    nerrors +=
        test_copy_soft_link(fcpl_src, fcpl_dst, src_fapl, dst_fapl, FALSE);
    nerrors +=
        test_copy_soft_link(fcpl_src, fcpl_dst, src_fapl, dst_fapl, TRUE);
    ...
    nerrors += test_copy_named_datatype_attr_self(fcpl_src, fcpl_dst, src_fapl,
                                                  dst_fapl);
    ...
    nerrors += test_copy_option(fcpl_src, fcpl_dst, src_fapl, dst_fapl,
                                H5O_COPY_SHALLOW_HIERARCHY_FLAG, FALSE,
                                "H5Ocopy(): shallow group copy");
    nerrors += test_copy_option(fcpl_src, fcpl_dst, src_fapl, dst_fapl,
                                H5O_COPY_EXPAND_REFERENCE_FLAG, FALSE,
                                "H5Ocopy(): expand object reference");
    nerrors += test_copy_option(fcpl_src, fcpl_dst, src_fapl, dst_fapl,
                                H5O_COPY_PRESERVE_NULL_FLAG, FALSE,
    ...
    if (configuration < CONFIG_DENSE) {
      ...
      nerrors +=
          test_copy_named_datatype(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors +=
          test_copy_named_datatype_vl(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors += test_copy_named_datatype_vl_vl(fcpl_src, fcpl_dst, src_fapl,
                                                dst_fapl);

      /* Loop over reopening the file */
      for (reopen = FALSE; reopen <= TRUE; reopen++) {
        nerrors += test_copy_committed_datatype_merge(
            fcpl_src, fcpl_dst, src_fapl, dst_fapl, reopen);

        if (same_file)
          nerrors += test_copy_committed_datatype_merge_same_file(
              fcpl_src, src_fapl, reopen);

        nerrors += test_copy_committed_dt_merge_sugg(
            fcpl_src, fcpl_dst, src_fapl, dst_fapl, reopen);
        nerrors += test_copy_committed_dt_merge_attr(
            fcpl_src, fcpl_dst, src_fapl, dst_fapl, reopen);

        /* tests added for merging committed datatypes + suggestions + callback
         */
        nerrors += test_copy_cdt_hier_merge(fcpl_src, fcpl_dst, src_fapl,
                                            dst_fapl, reopen);
        nerrors += test_copy_cdt_merge_cdt(fcpl_src, fcpl_dst, src_fapl,
                                           dst_fapl, reopen);
        nerrors += test_copy_cdt_merge_suggs(fcpl_src, fcpl_dst, src_fapl,
                                             dst_fapl, reopen);
        nerrors += test_copy_cdt_merge_dset_suggs(fcpl_src, fcpl_dst, src_fapl,
                                                  dst_fapl, reopen);
        nerrors += test_copy_cdt_merge_all_suggs(fcpl_src, fcpl_dst, src_fapl,
                                                 dst_fapl, reopen);
        nerrors += test_copy_set_mcdt_search_cb(fcpl_src, fcpl_dst, src_fapl,
                                                dst_fapl, reopen);
        nerrors += test_copy_set_get_mcdt_search_cb(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl, reopen);
      } /* end for */

      nerrors +=
          test_copy_dataset_external(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors +=
          test_copy_dataset_named_dtype(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors += test_copy_dataset_named_dtype_hier(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl);
      nerrors += test_copy_dataset_named_dtype_hier_outside(fcpl_src, fcpl_dst,
                                                            src_fapl, dst_fapl);

      nerrors +=
          test_copy_dataset_contig_vl(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors +=
          test_copy_dataset_chunked_vl(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors +=
          test_copy_dataset_compact_vl(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors += test_copy_dataset_compressed_vl(fcpl_src, fcpl_dst, src_fapl,
                                                 dst_fapl);
      nerrors += test_copy_attribute_vl(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
      nerrors += test_copy_attribute_compound_vlstr(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl);
      nerrors += test_copy_dataset_compact_named_vl(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl);
      nerrors += test_copy_dataset_contig_named_vl(fcpl_src, fcpl_dst, src_fapl,
                                                   dst_fapl);

      /* Test with dataset opened in the file or not */
      nerrors += test_copy_dataset_chunked_named_vl(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl, FALSE);
      nerrors += test_copy_dataset_chunked_named_vl(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl, TRUE);

      nerrors += test_copy_dataset_compressed_named_vl(fcpl_src, fcpl_dst,
                                                       src_fapl, dst_fapl);
      nerrors += test_copy_dataset_compact_vl_vl(fcpl_src, fcpl_dst, src_fapl,
                                                 dst_fapl);

      /* Test with dataset opened in the file or not */
      nerrors += test_copy_dataset_contig_vl_vl(fcpl_src, fcpl_dst, src_fapl,
                                                dst_fapl, FALSE);
      nerrors += test_copy_dataset_contig_vl_vl(fcpl_src, fcpl_dst, src_fapl,
                                                dst_fapl, TRUE);

      nerrors += test_copy_dataset_chunked_vl_vl(fcpl_src, fcpl_dst, src_fapl,
                                                 dst_fapl);

      /* Test with dataset opened in the file or not */
      nerrors += test_copy_dataset_compressed_vl_vl(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl, FALSE);
      nerrors += test_copy_dataset_compressed_vl_vl(fcpl_src, fcpl_dst,
                                                    src_fapl, dst_fapl, TRUE);

      nerrors += test_copy_dataset_contig_cmpd_vl(fcpl_src, fcpl_dst, src_fapl,
                                                  dst_fapl);
      nerrors += test_copy_dataset_chunked_cmpd_vl(fcpl_src, fcpl_dst, src_fapl,
                                                   dst_fapl);
      nerrors += test_copy_dataset_compact_cmpd_vl(fcpl_src, fcpl_dst, src_fapl,
                                                   dst_fapl);
      ...
      nerrors +=
          test_copy_null_ref_open(fcpl_src, fcpl_dst, src_fapl, dst_fapl);
