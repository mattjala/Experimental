 ...
 *    The remaining message(s) in the last chunk should be moved forward into
 * the continuation message The process will repeat when the continuation
 * message is big enough to hold all the messages in the last chunk. Result: the
 * number of chunks should be reduced
 */
static herr_t test_cont(char *filename, hid_t fapl) {
  ...
  if (H5O_create(f, (size_t)H5O_MIN_SIZE, (size_t)0, H5P_GROUP_CREATE_DEFAULT,
                 &oh_locA /*out*/) < 0)
    FAIL_STACK_ERROR;
...
static herr_t test_ohdr_cache(char *filename, hid_t fapl) {
  ...
  if (H5O_create(f, (size_t)2048, (size_t)1, H5P_GROUP_CREATE_DEFAULT,
                 &oh_loc /*out*/) < 0)
    FAIL_STACK_ERROR;
 ...
 *  When the object header is read in H5O_load() of H5Ocache.c, the library
 * initially reads H5O_SPEC_READ_SIZE (512, currently)  bytes for decoding, then
 * reads the remaining bytes later if the object header is greater than
 * H5O_SPEC_READ_SIZE bytes.  For SWMR access, the read should be done all at
 * one time.
 */
static herr_t test_ohdr_swmr(hbool_t new_format) {
  hid_t fid = -1;             /* File ID */
  ...
  if (new_format) {
    TESTING("exercise the coding for the re-read of the object header for SWMR "
            "access: latest-format");
  } else {
    TESTING("exercise the coding for the re-read of the object header for SWMR "
            "access: non-latest-format");
  } /* end if */
  ...
  /* Create the file with/without latest format: ensure version 2 object header
   * for SWMR */
  if (new_format) {
    ...
    if ((fid = H5Fcreate(FILE_OHDR_SWMR, H5F_ACC_TRUNC | H5F_ACC_SWMR_WRITE,
                         H5P_DEFAULT, fapl)) < 0)
      TEST_ERROR;
  } /* end else */
  ...
  if ((did = H5Dcreate2(fid, DSET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT, plist,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((fid = H5Fopen(FILE_OHDR_SWMR, H5F_ACC_RDWR | H5F_ACC_SWMR_WRITE, fapl)) <
      0)
    FAIL_STACK_ERROR;
  ...
  /* The size of object header should be greater than the speculative read size
   * of H5O_SPEC_READ_SIZE */
  /* This will exercise the coding for the re-read of the object header for SWMR
   * access */
 ...
 *   --objects that have unknown header messages with H5O_BOGUS_INVALID_ID in
 * "/group"
 *
 *   The test also uses the test file FILENAME[0] (ohdr.h5): the parameter
 * "filename"
 */
static herr_t test_unknown(unsigned bogus_id, char *filename, hid_t fapl) {
  ...
  if (bogus_id == H5O_BOGUS_INVALID_ID) {
    ...
    if ((gid = H5Gcreate2(fid, "group", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      FAIL_STACK_ERROR;
    loc = gid;
  ...
  TESTING("object in r/o file with unknown header message & 'fail if unknown "
          "and open for write' flag set");

  /* Open the dataset with the unknown header message, and "fail if unknown and
   * open for write" flag */
  ...
  /* Attempt to open the dataset with the unknown header message, and "fail if
   * unknown always" flag */
  ...
  /* Copy object with "mark if unknown" flag on message into file (FILENAME[0])
   * that can be modified */
  if (H5Ocopy(loc_bogus, "Dataset4", loc, "Dataset4", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((aid = H5Acreate2(did, "Attr", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  TESTING("object in r/w file with unknown header message & 'fail if unknown "
          "and open for write' flag set");
  ...
  /* Attempt to open the dataset with the unknown header message, and "fail if
   * unknown and open for write" flag */
...
static herr_t test_minimized_dset_ohdr_attribute_addition(hid_t fapl_id) {
  ...
  TESTING(
      "adding attributes to datasets created with H5Pset_dset_no_attrs_hint()");

  /* Create the test file */
  if (NULL == h5_fixname(FILENAME[1], fapl_id, filename, sizeof(filename)))
    TEST_ERROR;
  if ((fid = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_id)) ==
      H5I_INVALID_HID)
    TEST_ERROR;
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, "H5Pset_dset_no_attrs_hint", H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) == H5I_INVALID_HID)
    TEST_ERROR;
  ...
  if ((sid = H5Screate_simple(1, dims, NULL)) == H5I_INVALID_HID)
    TEST_ERROR;
  if ((aid = H5Acreate2(did, "string_attr", H5T_NATIVE_CHAR, sid, H5P_DEFAULT,
  ...
  for (i = 0; i < N_ATTRS; i++) {
    ...
    if ((aid = H5Acreate2(did, attr_name, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                          H5P_DEFAULT)) == H5I_INVALID_HID)
      TEST_ERROR;
...
static herr_t test_minimized_dset_ohdr_size_comparisons(hid_t fapl_id) {
  ...
  for (compact = 0; compact < 2; compact++) { /* 0 or 1 */
    ...
    dset_f_x_id = H5Dcreate2(file_f_id, "default", int_type_id, dspace_id,
                             H5P_DEFAULT, dcpl_default, H5P_DEFAULT);
    if (dset_f_x_id < 0)
      TEST_ERROR;

    dset_f_N_id = H5Dcreate2(file_f_id, "dsetNOT", int_type_id, dspace_id,
                             H5P_DEFAULT, dcpl_dontmin, H5P_DEFAULT);
    if (dset_f_N_id < 0)
      TEST_ERROR;

    dset_f_Y_id = H5Dcreate2(file_f_id, "dsetMIN", int_type_id, dspace_id,
                             H5P_DEFAULT, dcpl_minimize, H5P_DEFAULT);
    if (dset_f_x_id < 0)
      TEST_ERROR;
...
static herr_t test_minimized_dset_ohdr_with_filter(hid_t fapl_id) {
  ...
  if (ret < 0)
    TEST_ERROR;
  ret = H5Pset_filter(dcpl_xZ_id, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, 0,
                      filter_values);
  if (ret < 0)
    TEST_ERROR;
  ...
  if (ret < 0)
    TEST_ERROR;
  ret = H5Pset_filter(dcpl_mZ_id, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, 0,
                      filter_values);
  if (ret < 0)
    TEST_ERROR;
  ...
  dset_xx_id = H5Dcreate2(file_id, "xx", dtype_id, dspace_id, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  if (dset_xx_id < 0)
    TEST_ERROR;

  dset_mx_id = H5Dcreate2(file_id, "Mx", dtype_id, dspace_id, H5P_DEFAULT,
                          dcpl_mx_id, H5P_DEFAULT);
  if (dset_mx_id < 0)
    TEST_ERROR;

  dset_xZ_id = H5Dcreate2(file_id, "xZ", dtype_id, dspace_id, H5P_DEFAULT,
                          dcpl_xZ_id, H5P_DEFAULT);
  if (dset_xZ_id < 0)
    TEST_ERROR;

  dset_mZ_id = H5Dcreate2(file_id, "MZ", dtype_id, dspace_id, H5P_DEFAULT,
                          dcpl_mZ_id, H5P_DEFAULT);
  if (dset_mZ_id < 0)
    TEST_ERROR;
...
static herr_t test_minimized_dset_ohdr_modification_times(hid_t _fapl_id) {
  ...
  for (i = 0; i < n_cases; i++) {
    ...
    dset_xT_id = H5Dcreate2(file_id, "xT", dtype_id, dspace_id, H5P_DEFAULT,
                            dcpl_xT_id, H5P_DEFAULT);
    if (dset_xT_id < 0)
      TEST_ERROR;
    dset_mT_id = H5Dcreate2(file_id, "mT", dtype_id, dspace_id, H5P_DEFAULT,
                            dcpl_mT_id, H5P_DEFAULT);
    if (dset_mT_id < 0)
      TEST_ERROR;

    dset_mN_id = H5Dcreate2(file_id, "mN", dtype_id, dspace_id, H5P_DEFAULT,
                            dcpl_mN_id, H5P_DEFAULT);
    if (dset_mN_id < 0)
      TEST_ERROR;
...
test_minimized_dset_ohdr_fillvalue_backwards_compatability(hid_t _fapl_id) {
  ...
  TESTING("minimized dset object headers with fill values and different libver "
          "support");
  ...
  dset_0_id = H5Dcreate2(file_id, "fullrange", dtype_id, dspace_id, H5P_DEFAULT,
                         dcpl_id, H5P_DEFAULT);
  if (dset_0_id < 0)
    TEST_ERROR;
  ...
  dset_1_id = H5Dcreate2(file_id, "upperrange", dtype_id, dspace_id,
...
int main(void) {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      HDsnprintf(msg, sizeof(msg), "Using file format version: (%s, %s)",
       ...
       * (using default group creation property list only because it's
       * convenient)
       */
      TESTING("object header creation");
      memset(&oh_loc, 0, sizeof(oh_loc));
      if (H5O_create(f, (size_t)64, (size_t)0, H5P_GROUP_CREATE_DEFAULT,
                     &oh_loc /*out*/) < 0)
        FAIL_STACK_ERROR;
      PASSED();
      ...
      /* Make certain that chunk #0 in the object header can be encoded with a
       * 1-byte size */
      ...
      /* Make certain that chunk #0 in the object header will be encoded with a
       * 2-byte size */
      if (H5O_get_hdr_info(&oh_loc, &hdr_info) < 0)
        FAIL_STACK_ERROR;
      if (hdr_info.space.total < 256)
       ...
       *  between 1-byte chunk #0 size encoding and 2-byte chunk #0 size
       * encoding works correctly - QAK)
       */
      ...
      for (i = 0; i < 10; i++) {
        ...
        if (H5AC_secure_from_file_flush(f) < 0)
          FAIL_STACK_ERROR;
        if (H5AC_expunge_entry(f, H5AC_OHDR, oh_loc.addr, H5AC__NO_FLAGS_SET) <
            0)
          FAIL_STACK_ERROR;
      } /* end for */
      PASSED();
      ...
      time_new = 22222222;
      if (H5O_msg_create(&oh_loc, H5O_MTIME_NEW_ID, H5O_MSG_FLAG_CONSTANT, 0,
                         &time_new) < 0)
        FAIL_STACK_ERROR;
      if (H5AC_prep_for_file_flush(f) < 0)
        FAIL_STACK_ERROR;
      ...
      if (single_file_vfd) {
        ...
        SKIPPED();
        HDputs("    Unknown header message test not supported with the current "
               "VFD.");
      } /* end else */
