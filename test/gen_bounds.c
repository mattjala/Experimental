...
static herr_t gen_earliest_latest(void) {
  ...
  dset = H5Dcreate2(fid, "DS_chunked_layout_3", H5T_NATIVE_FLOAT, space,
  ...
  /* Create and write the dataset */
  dset = H5Dcreate2(fid, "DS_chunked_layout_4", H5T_NATIVE_FLOAT, space,
                    H5P_DEFAULT, dcpl, H5P_DEFAULT);
  if (dset < 0)
    TEST_ERROR;
  ret = H5Dwrite(dset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf);
 ...
 *
 ***********************************************************************/
static herr_t gen_sel_files(const char *filename, H5F_libver_t low_bound,
  ...
  if ((did = H5Dcreate2(fid, DATASET, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
  ...
  if (H5Sselect_hyperslab(sid, H5S_SELECT_SET, &ref_start, &ref_stride,
                          &ref_count, &ref_block) < 0)
    TEST_ERROR;
  ...
  if (high_bound >= H5F_LIBVER_V110) {
    ...
    if ((ref_did = H5Dcreate2(fid, SEL_EX_REG_DSET, H5T_STD_REF_DSETREG,
                              ref_sid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
        0)
      TEST_ERROR;

    /* Write to the reference dataset */
    if (H5Dwrite(ref_did, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 ref_wbuf) < 0)
      TEST_ERROR;
  ...
  ref_block = 2;
  ref_stride = POWER32;
  if (H5Sselect_hyperslab(sid, H5S_SELECT_OR, &ref_start, &ref_stride,
                          &ref_count, &ref_block) < 0)
    TEST_ERROR;
  ...
  if (high_bound >= H5F_LIBVER_V112) {
    ...
    /* Create the reference dataset */
    if ((ref_did = H5Dcreate2(fid, SEL_EX_IRR_DSET, H5T_STD_REF_DSETREG,
                              ref_sid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
        0)
      TEST_ERROR;
  ...
  if (high_bound >= H5F_LIBVER_V112) {
    ...
    /* Create the reference dataset */
    if ((ref_did = H5Dcreate2(fid, SEL_EX_PT_DSET, H5T_STD_REF_DSETREG, ref_sid,
 ...
 *
 ***********************************************************************/
static herr_t gen_ref_files(const char *filename, H5F_libver_t low_bound,
                            H5F_libver_t high_bound) {
  hid_t fid = H5I_INVALID_HID;     /* File ID */
  ...
  hobj_ref_t
      old_ref_obj_wbuf[1]; /* Buffer for storing the old reference object */
  hdset_reg_ref_t old_ref_reg_wbuf[1]; /* Buffer for storing the old dataset
                                          region reference */
  ...
  if ((aid = H5Acreate2(fid, ATTR, H5T_NATIVE_UINT, asid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(gid, DATASET, H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (high_bound >= H5F_LIBVER_V112) {
    ...
    ref_did = H5Dcreate2(fid, REVISED_REFS_DSET, H5T_STD_REF, ref_sid,
    ...
    if (H5Rcreate_region(fid, "/Group/Dataset", sid, H5P_DEFAULT,
                         &rev_ref_wbuf[2]) < 0)
      TEST_ERROR;

    /* Write to the reference dataset */
    if (H5Dwrite(ref_did, H5T_STD_REF, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 rev_ref_wbuf) < 0)
      TEST_ERROR;
  ...
  if ((ref_did = H5Dcreate2(fid, OLD_REF_OBJ_DSET, H5T_STD_REF_OBJ, ref_sid,
                            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Write to the reference dataset */
  if (H5Dwrite(ref_did, H5T_STD_REF_OBJ, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               old_ref_obj_wbuf) < 0)
    TEST_ERROR;
  ...
  if ((ref_did = H5Dcreate2(fid, OLD_REF_REG_DSET, H5T_STD_REF_DSETREG, ref_sid,
                            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create dataset region reference */
  if (H5Rcreate(&old_ref_reg_wbuf[0], fid, "/Group/Dataset", H5R_DATASET_REGION,
                sid) < 0)
    TEST_ERROR;
...
int main(void) {
  ...
  if (gen_ref_files(FILENAME_REF_E_L, H5F_LIBVER_EARLIEST, H5F_LIBVER_LATEST) <
      0)
    TEST_ERROR;
  ...
  if (gen_ref_files(FILENAME_REF_V112_V112, H5F_LIBVER_V112, H5F_LIBVER_V112) <
      0)
    TEST_ERROR;

  /* Generate bounds_ref_v110_v110.h5 */
  if (gen_ref_files(FILENAME_REF_V110_V110, H5F_LIBVER_V110, H5F_LIBVER_V110) <
      0)
    TEST_ERROR;
  ...
  if (gen_sel_files(FILENAME_SEL_E_L, H5F_LIBVER_EARLIEST, H5F_LIBVER_LATEST) <
      0)
    TEST_ERROR;
  ...
  if (gen_sel_files(FILENAME_SEL_V112_V112, H5F_LIBVER_V112, H5F_LIBVER_V112) <
      0)
    TEST_ERROR;

  /* Generate bounds_sel_v110_v110.h5 */
  if (gen_sel_files(FILENAME_SEL_V110_V110, H5F_LIBVER_V110, H5F_LIBVER_V110) <
      0)
    TEST_ERROR;
