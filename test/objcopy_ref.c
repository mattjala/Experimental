 ...
 *-------------------------------------------------------------------------
 */
static H5_ATTR_PURE hbool_t token_lookup(hid_t loc_id, H5O_info2_t *oinfo) {
  size_t n;
  int token_cmp;
...
static herr_t attach_ref_attr(hid_t file_id, hid_t loc_id) {
  ...
  if (H5Rcreate_object(file_id, dsetname2, H5P_DEFAULT, &ref[1]) < 0)
    TEST_ERROR;
  if ((aid = H5Acreate2(loc_id, "obj_ref_attr", H5T_STD_REF, sid_ref,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t attach_reg_ref_attr(hid_t file_id, hid_t loc_id) {
  ...
  if ((aid = H5Acreate2(loc_id, "reg_ref_attr", H5T_STD_REF, spacer_id,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Awrite(aid, H5T_STD_REF, ref) < 0)
    TEST_ERROR;
  ...
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  if ((aid = H5Acreate2(dsetv_id, "reg_ref_attr", H5T_STD_REF, spacer_id,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Awrite(aid, H5T_STD_REF, ref) < 0)
    TEST_ERROR;
...
static herr_t create_reg_ref_dataset(hid_t file_id, hid_t loc_id) {
  ...
  if (H5Dwrite(dsetv_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
      0)
    TEST_ERROR;
  if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer, H5T_STD_REF, spacer_id,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer1, H5T_STD_REF, spacer_id,
  ...
  if ((dsetr_id = H5Dcreate2(loc_id, dsetnamer2, H5T_STD_REF, spacer_id,
                             H5P_DEFAULT, pid, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Pclose(pid) < 0)
    TEST_ERROR;
...
static int compare_attribute(hid_t aid, hid_t aid2, hid_t pid, const void *wbuf,
  ...
  if (H5Tdetect_class(tid, H5T_VLEN) == TRUE ||
      H5Tdetect_class(tid, H5T_REFERENCE) == TRUE)
    if (H5Treclaim(tid, sid, H5P_DEFAULT, rbuf) < 0)
      TEST_ERROR;
  if (H5Tdetect_class(tid2, H5T_VLEN) == TRUE ||
      H5Tdetect_class(tid2, H5T_REFERENCE) == TRUE)
    ...
    if (H5Tequal(tid, H5T_STD_REF) > 0) {
      ...
      for (u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
        ...
        if (H5Rget_type(ref_buf1) == H5R_DATASET_REGION2) {
          ...
          if ((obj1_sid = H5Ropen_region(ref_buf1, H5P_DEFAULT, H5P_DEFAULT)) <
              0)
            TEST_ERROR;
          if ((obj2_sid = H5Ropen_region(ref_buf2, H5P_DEFAULT, H5P_DEFAULT)) <
              0)
            TEST_ERROR;
...
int main(void) {
  ...
  for (configuration = 0; configuration <= MAX_CONFIGURATION; configuration++) {
    ...
    /* TODO Region references currently do not support copy from new format to
     * old format (this may be fixed once H5Sencode/decode and H5CXis fixed) */
    if ((configuration & CONFIG_SRC_NEW_FORMAT) &&
        !(configuration & CONFIG_DST_NEW_FORMAT))
      continue;
