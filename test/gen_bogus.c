...
static int generate_datasets(hid_t loc_id, unsigned bogus_id) {
  ...
  if (H5Pinsert2(dcpl, H5O_BOGUS_MSG_FLAGS_NAME, H5O_BOGUS_MSG_FLAGS_SIZE,
                 &bogus_flags, NULL, NULL, NULL, NULL, NULL, NULL) < 0)
    goto error;

  /* Add property for bogus message ID */
  if (H5Pinsert2(dcpl, H5O_BOGUS_MSG_ID_NAME, H5O_BOGUS_MSG_ID_SIZE, &bogus_id,
                 NULL, NULL, NULL, NULL, NULL, NULL) < 0)
    goto error;

  /* Create dataset with "bogus" message, but no message flags */
  if ((did = H5Dcreate2(loc_id, "Dataset1", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    goto error;
  if (H5Dclose(did) < 0)
    goto error;
  ...
  if ((did = H5Dcreate2(loc_id, "Dataset2", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    goto error;
  if (H5Dclose(did) < 0)
    goto error;
  ...
  if ((did = H5Dcreate2(loc_id, "Dataset3", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    goto error;
  if (H5Dclose(did) < 0)
    goto error;
  ...
  if ((did = H5Dcreate2(loc_id, "Dataset4", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    goto error;
  if (H5Dclose(did) < 0)
    goto error;
  ...
  if ((did = H5Dcreate2(loc_id, "Dataset5", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    goto error;
  if (H5Dclose(did) < 0)
    goto error;
