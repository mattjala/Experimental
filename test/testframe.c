...
void AddTest(const char *TheName, void (*TheCall)(void), void (*Cleanup)(void),
  ...
  if (Index >= TestAlloc) {
    ...
    /* Reallocate array */
    if (NULL == (newTest = (TestStruct *)realloc(
                     Test, newAlloc * sizeof(TestStruct)))) {
      printf("Out of memory for tests, Index = %u, TestAlloc = %u, newAlloc = "
             "%u\n",
...
void TestUsage(void) {
  ...
  print_func(
      "cleanoff  does not delete *.hdf files after execution of tests\n");
...
void TestParseCmdLine(int argc, char *argv[]) {
  ...
  while ((void)argv++, --argc > 0) {
    if ((HDstrcmp(*argv, "-verbose") == 0) || (HDstrcmp(*argv, "-v") == 0)) {
    ...
    } else if ((HDstrcmp(*argv, "-summary") == 0) ||
               (HDstrcmp(*argv, "-s") == 0))
      Summary = 1;
      ...
      TestUsage();
      exit(EXIT_SUCCESS);
    } else if ((HDstrcmp(*argv, "-cleanoff") == 0) ||
               (HDstrcmp(*argv, "-c") == 0))
      SetTestNoCleanup();
    else {
...
void PerformTests(void) {
  ...
  for (Loop = 0; Loop < Index; Loop++)
    if (Test[Loop].SkipFlag) {
      ...
      MESSAGE(5, ("===============================================\n"));
      MESSAGE(
          5, ("There were %d errors detected.\n\n", (int)Test[Loop].NumErrors));
    }
 ...
 */
H5_ATTR_PURE int GetTestVerbosity(void) { return (TestVerbosity); }
 ...
 */
H5_ATTR_PURE int GetTestSummary(void) { return (Summary); }

/*
 * Retrieve Cleanup request value.
 *     0 means no Cleanup, 1 means yes.
 */
H5_ATTR_PURE int GetTestCleanup(void) { return (CleanUp); }
 ...
 */
H5_ATTR_PURE int GetTestNumErrs(void) { return (num_errs); }

/*
 * Increment the number of testing errors
 */
void IncTestNumErrs(void) { num_errs++; }

/*
 * Retrieve the current Test Parameters pointer.
 */
H5_ATTR_PURE const void *GetTestParameters(void) { return (Test_parameters); }
...
/* Enable a test timer that will kill long-running tests, the time is
...
void TestAlarmOn(void) {
  ...
  unsigned long alarm_sec =
      H5_ALARM_SEC; /* Number of seconds before alarm goes off */
