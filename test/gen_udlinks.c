...
int main(void) {
  ...
  if ((gid = H5Gcreate2(fid2, "group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    goto error;
  if ((H5Gclose(gid)) < 0)
    goto error;
  if ((gid = H5Gcreate2(fid2, "group/subgroup", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto error;
  if ((H5Gclose(gid)) < 0)
    goto error;

  /* Create an external link in the first file pointing to the group in the
   * second file */
  if (H5Lcreate_external(filename2, "group", fid1, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    goto error;
