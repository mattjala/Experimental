...
static herr_t pico_get_initial_load_size(void *udata_ptr,
                                         size_t *image_len_ptr);
static herr_t nano_get_initial_load_size(void *udata_ptr,
                                         size_t *image_len_ptr);
static herr_t micro_get_initial_load_size(void *udata_ptr,
                                          size_t *image_len_ptr);
static herr_t tiny_get_initial_load_size(void *udata_ptr,
                                         size_t *image_len_ptr);
static herr_t small_get_initial_load_size(void *udata_ptr,
                                          size_t *image_len_ptr);
static herr_t medium_get_initial_load_size(void *udata_ptr,
                                           size_t *image_len_ptr);
static herr_t large_get_initial_load_size(void *udata_ptr,
                                          size_t *image_len_ptr);
static herr_t huge_get_initial_load_size(void *udata_ptr,
                                         size_t *image_len_ptr);
static herr_t monster_get_initial_load_size(void *udata_ptr,
                                            size_t *image_len_ptr);
static herr_t variable_get_initial_load_size(void *udata_ptr,
                                             size_t *image_len_ptr);
static herr_t notify_get_initial_load_size(void *udata_ptr,
                                           size_t *image_len_ptr);

static herr_t variable_get_final_load_size(const void *image, size_t image_len,
                                           void *udata, size_t *actual_len);

static htri_t variable_verify_chksum(const void *image_ptr, size_t len,
                                     void *udata_ptr);

static void *pico_deserialize(const void *image_ptr, size_t len,
                              void *udata_ptr, hbool_t *dirty_ptr);
static void *nano_deserialize(const void *image_ptr, size_t len,
                              void *udata_ptr, hbool_t *dirty_ptr);
static void *micro_deserialize(const void *image_ptr, size_t len,
                               void *udata_ptr, hbool_t *dirty_ptr);
static void *tiny_deserialize(const void *image_ptr, size_t len,
                              void *udata_ptr, hbool_t *dirty_ptr);
static void *small_deserialize(const void *image_ptr, size_t len,
                               void *udata_ptr, hbool_t *dirty_ptr);
static void *medium_deserialize(const void *image_ptr, size_t len,
                                void *udata_ptr, hbool_t *dirty_ptr);
static void *large_deserialize(const void *image_ptr, size_t len,
                               void *udata_ptr, hbool_t *dirty_ptr);
static void *huge_deserialize(const void *image_ptr, size_t len,
                              void *udata_ptr, hbool_t *dirty_ptr);
static void *monster_deserialize(const void *image_ptr, size_t len,
                                 void *udata_ptr, hbool_t *dirty_ptr);
static void *variable_deserialize(const void *image_ptr, size_t len,
                                  void *udata_ptr, hbool_t *dirty_ptr);
static void *notify_deserialize(const void *image_ptr, size_t len,
                                void *udata_ptr, hbool_t *dirty_ptr);
...
static herr_t pico_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                 size_t len, haddr_t *new_addr_ptr,
                                 size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t nano_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                 size_t len, haddr_t *new_addr_ptr,
                                 size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t micro_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                  size_t len, haddr_t *new_addr_ptr,
                                  size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t tiny_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                 size_t len, haddr_t *new_addr_ptr,
                                 size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t small_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                  size_t len, haddr_t *new_addr_ptr,
                                  size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t medium_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                   size_t len, haddr_t *new_addr_ptr,
                                   size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t large_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                  size_t len, haddr_t *new_addr_ptr,
                                  size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t huge_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                 size_t len, haddr_t *new_addr_ptr,
                                 size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t monster_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                    size_t len, haddr_t *new_addr_ptr,
                                    size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t variable_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                     size_t len, haddr_t *new_addr_ptr,
                                     size_t *new_len_ptr, unsigned *flags_ptr);
static herr_t notify_pre_serialize(H5F_t *f, void *thing, haddr_t addr,
                                   size_t len, haddr_t *new_addr_ptr,
                                   size_t *new_len_ptr, unsigned *flags_ptr);

static herr_t pico_serialize(const H5F_t *f, void *image_ptr, size_t len,
                             void *thing);
static herr_t nano_serialize(const H5F_t *f, void *image_ptr, size_t len,
                             void *thing);
static herr_t micro_serialize(const H5F_t *f, void *image_ptr, size_t len,
                              void *thing);
static herr_t tiny_serialize(const H5F_t *f, void *image_ptr, size_t len,
                             void *thing);
static herr_t small_serialize(const H5F_t *f, void *image_ptr, size_t len,
                              void *thing);
static herr_t medium_serialize(const H5F_t *f, void *image_ptr, size_t len,
                               void *thing);
static herr_t large_serialize(const H5F_t *f, void *image_ptr, size_t len,
                              void *thing);
static herr_t huge_serialize(const H5F_t *f, void *image_ptr, size_t len,
                             void *thing);
static herr_t monster_serialize(const H5F_t *f, void *image_ptr, size_t len,
                                void *thing);
static herr_t variable_serialize(const H5F_t *f, void *image_ptr, size_t len,
                                 void *thing);
static herr_t notify_serialize(const H5F_t *f, void *image_ptr, size_t len,
                               void *thing);
...
static herr_t image_len(const void *thing, size_t *image_len_ptr,
                        int32_t entry_type);
static herr_t pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
...
const int32_t max_indices[NUMBER_OF_ENTRY_TYPES] = {
    NUM_PICO_ENTRIES - 1,     NUM_NANO_ENTRIES - 1,  NUM_MICRO_ENTRIES - 1,
    NUM_TINY_ENTRIES - 1,     NUM_SMALL_ENTRIES - 1, NUM_MEDIUM_ENTRIES - 1,
    NUM_LARGE_ENTRIES - 1,    NUM_HUGE_ENTRIES - 1,  NUM_MONSTER_ENTRIES - 1,
    NUM_VARIABLE_ENTRIES - 1, NUM_NOTIFY_ENTRIES - 1};

const size_t entry_sizes[NUMBER_OF_ENTRY_TYPES] = {
    PICO_ENTRY_SIZE,    NANO_ENTRY_SIZE,     MICRO_ENTRY_SIZE, TINY_ENTRY_SIZE,
    SMALL_ENTRY_SIZE,   MEDIUM_ENTRY_SIZE,   LARGE_ENTRY_SIZE, HUGE_ENTRY_SIZE,
    MONSTER_ENTRY_SIZE, VARIABLE_ENTRY_SIZE, NOTIFY_ENTRY_SIZE};

const haddr_t base_addrs[NUMBER_OF_ENTRY_TYPES] = {
    PICO_BASE_ADDR,    NANO_BASE_ADDR,     MICRO_BASE_ADDR, TINY_BASE_ADDR,
    SMALL_BASE_ADDR,   MEDIUM_BASE_ADDR,   LARGE_BASE_ADDR, HUGE_BASE_ADDR,
    MONSTER_BASE_ADDR, VARIABLE_BASE_ADDR, NOTIFY_BASE_ADDR};

const haddr_t alt_base_addrs[NUMBER_OF_ENTRY_TYPES] = {
    PICO_ALT_BASE_ADDR,     NANO_ALT_BASE_ADDR,  MICRO_ALT_BASE_ADDR,
    TINY_ALT_BASE_ADDR,     SMALL_ALT_BASE_ADDR, MEDIUM_ALT_BASE_ADDR,
    LARGE_ALT_BASE_ADDR,    HUGE_ALT_BASE_ADDR,  MONSTER_ALT_BASE_ADDR,
    VARIABLE_ALT_BASE_ADDR, NOTIFY_ALT_BASE_ADDR};
...
const H5C_class_t *types[NUMBER_OF_ENTRY_TYPES] = {
    pico_class,    nano_class,     micro_class, tiny_class,
 ...
 *-------------------------------------------------------------------------
 */
void addr_to_type_and_index(haddr_t addr, int32_t *type_ptr,
                            int32_t *index_ptr) {
  int i;
  int32_t type;
  int32_t idx;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t get_initial_load_size(void *udata, size_t *image_length,
...
static herr_t variable_get_initial_load_size(void *udata,
                                             size_t *image_length) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t get_final_load_size(const void H5_ATTR_UNUSED *image,
                                  size_t H5_ATTR_UNUSED image_len, void *udata,
                                  size_t *actual_len,
                                  int32_t H5_ATTR_NDEBUG_UNUSED entry_type) {
  test_entry_t *entry;
  test_entry_t *base_addr;
...
static herr_t variable_get_final_load_size(const void *image, size_t image_len,
                                           void *udata, size_t *actual_len) {
  return get_final_load_size(image, image_len, udata, actual_len,
                             VARIABLE_ENTRY_TYPE);
}
...
static htri_t verify_chksum(const void H5_ATTR_UNUSED *image,
                            size_t H5_ATTR_UNUSED len, void *udata,
                            int32_t H5_ATTR_NDEBUG_UNUSED entry_type) {
  test_entry_t *entry;
  test_entry_t *base_addr;
  haddr_t addr = *(const haddr_t *)udata;
...
static htri_t variable_verify_chksum(const void *image, size_t len,
                                     void *udata) {
 ...
 *-------------------------------------------------------------------------
 */
static void *deserialize(const void *image, size_t H5_ATTR_NDEBUG_UNUSED len,
...
void *pico_deserialize(const void *image, size_t len, void *udata,
                       hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, PICO_ENTRY_TYPE);
}

void *nano_deserialize(const void *image, size_t len, void *udata,
                       hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, NANO_ENTRY_TYPE);
}

void *micro_deserialize(const void *image, size_t len, void *udata,
                        hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, MICRO_ENTRY_TYPE);
}

void *tiny_deserialize(const void *image, size_t len, void *udata,
                       hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, TINY_ENTRY_TYPE);
}

void *small_deserialize(const void *image, size_t len, void *udata,
                        hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, SMALL_ENTRY_TYPE);
}

void *medium_deserialize(const void *image, size_t len, void *udata,
                         hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, MEDIUM_ENTRY_TYPE);
}

void *large_deserialize(const void *image, size_t len, void *udata,
                        hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, LARGE_ENTRY_TYPE);
}

void *huge_deserialize(const void *image, size_t len, void *udata,
                       hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, HUGE_ENTRY_TYPE);
}

void *monster_deserialize(const void *image, size_t len, void *udata,
                          hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, MONSTER_ENTRY_TYPE);
}

void *variable_deserialize(const void *image, size_t len, void *udata,
                           hbool_t *dirty) {
  return deserialize(image, len, udata, dirty, VARIABLE_ENTRY_TYPE);
}

void *notify_deserialize(const void *image, size_t len, void *udata,
                         hbool_t *dirty) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t image_len(const void *thing, size_t *image_length,
                 int32_t H5_ATTR_NDEBUG_UNUSED entry_type) {
 ...
 *-------------------------------------------------------------------------
 */
herr_t pre_serialize(H5F_t H5_ATTR_NDEBUG_UNUSED *f, void *thing,
                     haddr_t H5_ATTR_NDEBUG_UNUSED addr,
                     size_t H5_ATTR_NDEBUG_UNUSED len, haddr_t *new_addr_ptr,
                     size_t *new_len_ptr, unsigned *flags_ptr) {
  test_entry_t *entry;
  int32_t i;
  ...
  *flags_ptr = H5C__SERIALIZE_NO_FLAGS_SET;
  ...
  if (entry->num_flush_ops > 0) {
    for (i = 0; i < entry->num_flush_ops; i++) {
      ...
      execute_flush_op(entry->file_ptr, entry, &((entry->flush_ops)[i]),
                       flags_ptr);
    } /* end for */
...
herr_t pico_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                          haddr_t *new_addr_ptr, size_t *new_len_ptr,
                          unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
}

herr_t nano_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                          haddr_t *new_addr_ptr, size_t *new_len_ptr,
                          unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t micro_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                           haddr_t *new_addr_ptr, size_t *new_len_ptr,
                           unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t tiny_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                          haddr_t *new_addr_ptr, size_t *new_len_ptr,
                          unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t small_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                           haddr_t *new_addr_ptr, size_t *new_len_ptr,
                           unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t medium_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                            haddr_t *new_addr_ptr, size_t *new_len_ptr,
                            unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t large_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                           haddr_t *new_addr_ptr, size_t *new_len_ptr,
                           unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t huge_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                          haddr_t *new_addr_ptr, size_t *new_len_ptr,
                          unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t monster_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                             haddr_t *new_addr_ptr, size_t *new_len_ptr,
                             unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t variable_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                              haddr_t *new_addr_ptr, size_t *new_len_ptr,
                              unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
...
herr_t notify_pre_serialize(H5F_t *f, void *thing, haddr_t addr, size_t len,
                            haddr_t *new_addr_ptr, size_t *new_len_ptr,
                            unsigned *flags_ptr) {
  return pre_serialize(f, thing, addr, len, new_addr_ptr, new_len_ptr,
                       flags_ptr);
 ...
 *-------------------------------------------------------------------------
 */
herr_t serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr, size_t len,
                 void *thing) {
  test_entry_t *entry;
  int32_t type;
...
herr_t pico_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                      size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t nano_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                      size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t micro_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                       size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t tiny_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                      size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t small_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                       size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t medium_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                        size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t large_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                       size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t huge_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                      size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t monster_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                         size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t variable_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                          size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}

herr_t notify_serialize(const H5F_t H5_ATTR_UNUSED *f, void *image_ptr,
                        size_t len, void *thing) {
  return serialize(f, image_ptr, len, thing);
}
...
static herr_t notify(H5C_notify_action_t action, void *thing,
  ...
  assert(entry == &(entries[entry->type][entry->index]));
  assert(entry == entry->self);
  if (!(action == H5C_NOTIFY_ACTION_ENTRY_DIRTIED &&
        entry->action == TEST_ENTRY_ACTION_MOVE))
  ...
  case H5C_NOTIFY_ACTION_BEFORE_EVICT: /* Entry is about to be evicted from
                                          cache */
...
herr_t free_icr(test_entry_t *entry, int32_t H5_ATTR_NDEBUG_UNUSED entry_type) {
  ...
  assert((entry->header.destroy_in_progress) ||
...
void add_flush_op(int target_type, int target_idx, int op_code, int type,
                  int idx, hbool_t flag, size_t new_size, unsigned *order_ptr) {
  int i;
...
void create_pinned_entry_dependency(H5F_t *file_ptr, int pinning_type,
                                    int pinning_idx, int pinned_type,
                                    int pinned_idx) {
...
void dirty_entry(H5F_t *file_ptr, int32_t type, int32_t idx,
                 hbool_t dirty_pin) {
  test_entry_t *base_addr;
...
void execute_flush_op(H5F_t *file_ptr, struct test_entry_t *entry_ptr,
  ...
  if (pass) {
    ...
    case FLUSH_OP__DIRTY:
      assert((entry_ptr->type != op_ptr->type) ||
    ...
    case FLUSH_OP__RESIZE:
      if ((entry_ptr->type == op_ptr->type) &&
        ...
        resize_entry(file_ptr, op_ptr->type, op_ptr->idx, op_ptr->size,
                     op_ptr->flag);
      }
      break;
    ...
    case FLUSH_OP__DEST_FLUSH_DEP:
      assert((entry_ptr->type != op_ptr->type) ||
             (entry_ptr->index != op_ptr->idx));
      destroy_flush_dependency(op_ptr->type, op_ptr->idx, entry_ptr->type,
                               entry_ptr->index);
      break;
...
{
  /* pico entries */
  if (NULL == (pico_entries = (test_entry_t *)calloc(NUM_PICO_ENTRIES,
                                                     sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_pico_entries = (test_entry_t *)calloc(
                   NUM_PICO_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* nano entries */
  if (NULL == (nano_entries = (test_entry_t *)calloc(NUM_NANO_ENTRIES,
                                                     sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_nano_entries = (test_entry_t *)calloc(
                   NUM_NANO_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* micro entries */
  if (NULL == (micro_entries = (test_entry_t *)calloc(NUM_MICRO_ENTRIES,
                                                      sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_micro_entries = (test_entry_t *)calloc(
                   NUM_MICRO_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* tiny entries */
  if (NULL == (tiny_entries = (test_entry_t *)calloc(NUM_TINY_ENTRIES,
                                                     sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_tiny_entries = (test_entry_t *)calloc(
                   NUM_TINY_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* small entries */
  if (NULL == (small_entries = (test_entry_t *)calloc(NUM_SMALL_ENTRIES,
                                                      sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_small_entries = (test_entry_t *)calloc(
                   NUM_SMALL_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* medium entries */
  if (NULL == (medium_entries = (test_entry_t *)calloc(NUM_MEDIUM_ENTRIES,
                                                       sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_medium_entries = (test_entry_t *)calloc(
                   NUM_MEDIUM_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* large entries */
  if (NULL == (large_entries = (test_entry_t *)calloc(NUM_LARGE_ENTRIES,
                                                      sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_large_entries = (test_entry_t *)calloc(
                   NUM_LARGE_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* huge entries */
  if (NULL == (huge_entries = (test_entry_t *)calloc(NUM_HUGE_ENTRIES,
                                                     sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_huge_entries = (test_entry_t *)calloc(
                   NUM_HUGE_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* monster entries */
  if (NULL == (monster_entries = (test_entry_t *)calloc(NUM_MONSTER_ENTRIES,
                                                        sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_monster_entries = (test_entry_t *)calloc(
                   NUM_MONSTER_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* variable entries */
  if (NULL == (variable_entries = (test_entry_t *)calloc(NUM_VARIABLE_ENTRIES,
                                                         sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_variable_entries = (test_entry_t *)calloc(
                   NUM_VARIABLE_ENTRIES, sizeof(test_entry_t))))
    goto error;

  /* notify entries */
  if (NULL == (notify_entries = (test_entry_t *)calloc(NUM_NOTIFY_ENTRIES,
                                                       sizeof(test_entry_t))))
    goto error;
  if (NULL == (orig_notify_entries = (test_entry_t *)calloc(
                   NUM_NOTIFY_ENTRIES, sizeof(test_entry_t))))
    goto error;
...

{
  ...
  if (!orig_entry_arrays_init) {
    ...
    for (i = 0; i < NUMBER_OF_ENTRY_TYPES; i++) {
      ...
      memcpy(orig_base_addr, base_addr,
             (size_t)(max_index + 1) * sizeof(*base_addr));
    } /* end for */
    ...
    for (i = 0; i < NUMBER_OF_ENTRY_TYPES; i++) {
      ...
      /* Make copy of entries in base_addr for later */
      memcpy(base_addr, orig_base_addr,
             (size_t)(max_index + 1) * sizeof(*base_addr));
    } /* end for */
  }   /* end else */
...
void resize_entry(H5F_t *file_ptr, int32_t type, int32_t idx, size_t new_size,
                  hbool_t in_cache) {
  test_entry_t *base_addr;
  test_entry_t *entry_ptr;
  herr_t result;
  ...
  while ((pass) && (i < num_entries)) {
    ...
    if ((!expected[i].in_cache) &&
      ...
      HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                 "%d: Contradictory data in expected[%d].\n", tag, i);
      failure_mssg = tmp_msg_buf;
    }

    if ((!expected[i].in_cache) && (expected[i].is_dirty) &&
        (!entry_ptr->expunged)) {
    ...
    if (pass) {
      ...
      if (in_cache != expected[i].in_cache) {
        ...
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   ...
                   (int)in_cache, (int)expected[i].in_cache);
        failure_mssg = tmp_msg_buf;
      }
    }
    ...
    if (pass) {
      ...
      if (entry_ptr->size != expected[i].size) {
        ...
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   ...
                   (int)expected[i].entry_type, (int)expected[i].entry_index,
                   (long)(entry_ptr->size), (long)expected[i].size);
        failure_mssg = tmp_msg_buf;
      }
    }
    ...
    if ((pass) && (in_cache)) {
      ...
      if (entry_ptr->header.size != expected[i].size) {
        ...
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   ...
                   (int)expected[i].entry_index, (long)(entry_ptr->header.size),
    ...
    if (pass) {
      ...
      if (entry_ptr->at_main_addr != expected[i].at_main_addr) {
        ...
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   ...
                   tag, (int)expected[i].entry_type,
                   (int)expected[i].entry_index, (int)(entry_ptr->at_main_addr),
    ...
    if ((pass) && (in_cache)) {
      ...
      if (entry_ptr->header.is_dirty != expected[i].is_dirty) {
        ...
        pass = FALSE;
        HDsnprintf(
            tmp_msg_buf, sizeof(tmp_msg_buf),
    ...
    if (pass) {
      ...
      if (entry_ptr->is_protected != expected[i].is_protected) {
        ...
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   ...
                   tag, (int)expected[i].entry_type,
                   (int)expected[i].entry_index, (int)(entry_ptr->is_protected),
    ...
    if (pass) {
      ...
      if (entry_ptr->is_pinned != expected[i].is_pinned) {
        ...
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   ...
                   tag, (int)expected[i].entry_type,
                   (int)expected[i].entry_index, (int)(entry_ptr->is_pinned),
    ...
    if (pass) {
      ...
      if (entry_ptr->is_corked != expected[i].is_corked) {
        ...
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   ...
                   tag, (int)expected[i].entry_type,
                   (int)expected[i].entry_index, (int)(entry_ptr->is_corked),
    ...
    if (pass) {
      ...
      if ((entry_ptr->deserialized != expected[i].deserialized) ||
        ...
        pass = FALSE;
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   "%d entry (%d,%d) deserialized = %d(%d), serialized = "
                   "%d(%d), dest = %d(%d)\n",
                   tag, (int)expected[i].entry_type,
                   (int)expected[i].entry_index, (int)(entry_ptr->deserialized),
    ...
    if (pass) {
      if (entry_ptr->flush_dep_npar != expected[i].flush_dep_npar) {
        ...
        HDsnprintf(
            ...
            entry_ptr->flush_dep_npar, expected[i].flush_dep_npar);
        failure_mssg = tmp_msg_buf;
      } /* end if */
    }   /* end if */
    if ((pass) && (in_cache)) {
      if (entry_ptr->header.flush_dep_nparents != expected[i].flush_dep_npar) {
        pass = FALSE;
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   "%d entry (%d, %d) header flush_dep_nparents "
                   "actual/expected = %u/%u.\n",
    ...
    if (pass) {
      for (u = 0; u < entry_ptr->flush_dep_npar; u++) {
        if (entry_ptr->flush_dep_par_type[u] !=
            expected[i].flush_dep_par_type[u]) {
          pass = FALSE;
          HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                     "%d entry (%d, %d) flush_dep_par_type[%u] actual/expected "
                     "= %d/%d.\n",
                     tag, expected[i].entry_type, expected[i].entry_index, u,
    ...
    if (pass) {
      for (u = 0; u < entry_ptr->flush_dep_npar; u++) {
        if (entry_ptr->flush_dep_par_idx[u] !=
            expected[i].flush_dep_par_idx[u]) {
          pass = FALSE;
          HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                     "%d entry (%d, %d) flush_dep_par_idx[%u] actual/expected "
                     "= %d/%d.\n",
                     tag, expected[i].entry_type, expected[i].entry_index, u,
    ...
    if (pass) {
      if (entry_ptr->flush_dep_nchd != expected[i].flush_dep_nchd) {
        ...
        HDsnprintf(
            ...
            expected[i].entry_type, expected[i].entry_index,
            entry_ptr->flush_dep_nchd, expected[i].flush_dep_nchd);
        failure_mssg = tmp_msg_buf;
      } /* end if */
    }   /* end if */
    if ((pass) && (in_cache)) {
      if (entry_ptr->header.flush_dep_nchildren != expected[i].flush_dep_nchd) {
        pass = FALSE;
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   "%d entry (%d, %d) header flush_dep_nchildren "
                   "actual/expected = %u/%u.\n",
                   tag, expected[i].entry_type, expected[i].entry_index,
    ...
    if (pass) {
      if (entry_ptr->flush_dep_ndirty_chd != expected[i].flush_dep_ndirty_chd) {
        ...
        HDsnprintf(
            ...
            tag, expected[i].entry_type, expected[i].entry_index,
            entry_ptr->flush_dep_ndirty_chd, expected[i].flush_dep_ndirty_chd);
        failure_mssg = tmp_msg_buf;
      } /* end if */
    }   /* end if */
    if ((pass) && (in_cache)) {
      if (entry_ptr->header.flush_dep_ndirty_children !=
          expected[i].flush_dep_ndirty_chd) {
        pass = FALSE;
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   "%d entry (%d, %d) header flush_dep_ndirty_children "
                   "actual/expected = %u/%u.\n",
                   tag, expected[i].entry_type, expected[i].entry_index,
    ...
    if (pass) {
      if (expected[i].flush_order >= 0 &&
          entry_ptr->flush_order != (unsigned)expected[i].flush_order) {
        pass = FALSE;
        HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf),
                   "%d entry (%d, %d) flush_order actual/expected = %u/%d.\n",
                   tag, expected[i].entry_type, expected[i].entry_index,
                   entry_ptr->flush_order, expected[i].flush_order);
        failure_mssg = tmp_msg_buf;
      } /* end if */
    }   /* end if */
...
void expunge_entry(H5F_t *file_ptr, int32_t type, int32_t idx) {
  ...
  if (pass) {
    ...
    result = H5C_expunge_entry(file_ptr, types[type], entry_ptr->addr,
                               H5C__NO_FLAGS_SET);
...
void flush_cache(H5F_t *file_ptr, hbool_t destroy_entries, hbool_t dump_stats,
    ...
    if ((pass) && (destroy_entries) &&
      ...
      if (verbose) {
        ...
        fprintf(stdout, "%s: unexpected il/is/cis/dis = %lld/%lld/%lld/%lld.\n",
                __func__, (long long)(cache_ptr->index_len),
...
void insert_entry(H5F_t *file_ptr, int32_t type, int32_t idx,
                  unsigned int flags) {
  H5C_t *cache_ptr;
  herr_t result;
  ...
  if (pass) {
    ...
    result = H5C_insert_entry(file_ptr, types[type], entry_ptr->addr,
...
void move_entry(H5C_t *cache_ptr, int32_t type, int32_t idx,
                hbool_t main_addr) {
  herr_t result = 0;
  ...
  if (pass) {
    ...
    if (!done) {
      ...
      if ((result < 0) || ((!(entry_ptr->header.destroy_in_progress)) &&
...
void protect_entry(H5F_t *file_ptr, int32_t type, int32_t idx) {
  ...
  if (pass) {
    ...
    cache_entry_ptr =
        (H5C_cache_entry_t *)H5C_protect(file_ptr, types[type], entry_ptr->addr,
...
void unprotect_entry(H5F_t *file_ptr, int32_t type, int32_t idx,
                     unsigned int flags) {
  herr_t result;
  ...
  if (pass) {
    ...
    if ((result < 0) ||
      ...
      if (pin_flag_set) {
        ...
        assert(entry_ptr->header.is_pinned ==
               entry_ptr->header.pinned_from_cache);
        entry_ptr->pinned_from_client = FALSE;
        entry_ptr->is_pinned = entry_ptr->pinned_from_cache;
      }
    }
 ...
 *-------------------------------------------------------------------------
 */
void row_major_scan_forward(H5F_t *file_ptr, int32_t max_index, int32_t lag,
                            hbool_t verbose, hbool_t reset_stats,
                            hbool_t display_stats,
                            hbool_t display_detailed_stats, hbool_t do_inserts,
                            hbool_t do_moves, hbool_t move_to_main_addr,
  ...
  while (pass && type < NUMBER_OF_ENTRY_TYPES) {
    ...
    while (pass && idx <= (local_max_index + lag)) {
      ...
      if (pass && do_inserts && (tmp_idx >= 0) &&
          (tmp_idx <= local_max_index) && ((tmp_idx % 2) == 0) &&
      ...
      if (pass && do_moves && (tmp_idx >= 0) && (tmp_idx <= local_max_index) &&
        ...
        if (verbose)
          fprintf(stdout, "4(r, %d, %d, %d) ", type, tmp_idx,
...
void hl_row_major_scan_forward(H5F_t *file_ptr, int32_t max_index,
                               hbool_t verbose, hbool_t reset_stats,
                               hbool_t display_stats,
  ...
  while ((pass) && (type < NUMBER_OF_ENTRY_TYPES)) {
    ...
    while ((pass) && (idx <= (local_max_index + lag))) {
      if ((pass) && (do_inserts) && ((idx + lag) >= 0) &&
          ((idx + lag) <= max_indices[type]) && (((idx + lag) % 2) == 0) &&
...
void row_major_scan_backward(H5F_t *file_ptr, int32_t max_index, int32_t lag,
                             hbool_t verbose, hbool_t reset_stats,
                             hbool_t display_stats,
                             hbool_t display_detailed_stats, hbool_t do_inserts,
                             hbool_t do_moves, hbool_t move_to_main_addr,
  ...
  while ((pass) && (type >= 0)) {
    ...
    while ((pass) && (idx >= -lag)) {
      ...
      tmp_idx = idx - lag;
      if ((pass) && (do_inserts) && (tmp_idx >= 0) &&
          (tmp_idx <= local_max_index) && ((tmp_idx % 2) == 1) &&
          (!entry_in_cache(cache_ptr, type, tmp_idx))) {
      ...
      if ((pass) && (do_moves) && (tmp_idx >= 0) &&
          (tmp_idx <= local_max_index) && ((tmp_idx % 3) == 0)) {

        if (verbose)
          fprintf(stdout, "(r, %d, %d, %d) ", type, tmp_idx,
                  (int)move_to_main_addr);
...
void hl_row_major_scan_backward(H5F_t *file_ptr, int32_t max_index,
                                hbool_t verbose, hbool_t reset_stats,
                                hbool_t display_stats,
                                hbool_t display_detailed_stats,
                                hbool_t do_inserts) {
  ...
  while ((pass) && (type >= 0)) {
    ...
    while ((pass) && (idx >= -lag)) {
      if ((pass) && (do_inserts) && ((idx + lag) >= 0) &&
          ((idx + lag) <= local_max_index) && (((idx + lag) % 2) == 0) &&
          (!entry_in_cache(cache_ptr, type, (idx + lag)))) {
...
void col_major_scan_forward(H5F_t *file_ptr, int32_t max_index, int32_t lag,
                            hbool_t verbose, hbool_t reset_stats,
                            hbool_t display_stats,
                            hbool_t display_detailed_stats, hbool_t do_inserts,
  ...
  while ((pass) && ((idx - lag) <= MAX_ENTRIES)) {
    ...
    while ((pass) && (type < NUMBER_OF_ENTRY_TYPES)) {
      if ((pass) && (do_inserts) && ((idx + lag) >= 0) &&
          ((idx + lag) <= local_max_index[type]) && (((idx + lag) % 3) == 0) &&
          (!entry_in_cache(cache_ptr, type, (idx + lag)))) {
...
void hl_col_major_scan_forward(H5F_t *file_ptr, int32_t max_index,
                               hbool_t verbose, hbool_t reset_stats,
                               hbool_t display_stats,
                               hbool_t display_detailed_stats,
    ...
    while ((pass) && (i >= 0) && (i >= (idx - lag))) {
      ...
      while ((pass) && (type < NUMBER_OF_ENTRY_TYPES)) {
        if ((pass) && (do_inserts) && (i == idx) && (i <= local_max_index) &&
            ((i % 3) == 0) && (!entry_in_cache(cache_ptr, type, i))) {
...
void col_major_scan_backward(H5F_t *file_ptr, int32_t max_index, int32_t lag,
                             hbool_t verbose, hbool_t reset_stats,
                             hbool_t display_stats,
                             hbool_t display_detailed_stats, hbool_t do_inserts,
                             int dirty_unprotects) {
...
void hl_col_major_scan_backward(H5F_t *file_ptr, int32_t max_index,
                                hbool_t verbose, hbool_t reset_stats,
                                hbool_t display_stats,
                                hbool_t display_detailed_stats,
                                hbool_t do_inserts, int dirty_unprotects) {
...
void create_flush_dependency(int32_t par_type, int32_t par_idx,
                             int32_t chd_type, int32_t chd_idx) {
  ...
  if (pass) {
    ...
    if (chd_entry_ptr->is_dirty || chd_entry_ptr->flush_dep_ndirty_chd > 0) {
      assert(par_entry_ptr->flush_dep_ndirty_chd <
             par_entry_ptr->flush_dep_nchd);
...
void destroy_flush_dependency(int32_t par_type, int32_t par_idx,
                              int32_t chd_type, int32_t chd_idx) {
  assert((0 <= par_type) && (par_type < NUMBER_OF_ENTRY_TYPES));
  assert((0 <= par_idx) && (par_idx <= max_indices[par_type]));
  assert((0 <= chd_type) && (chd_type < NUMBER_OF_ENTRY_TYPES));
...
void check_and_validate_cache_hit_rate(hid_t file_id, double *hit_rate_ptr,
                                       hbool_t dump_data, int64_t min_accesses,
                                       double min_hit_rate) {
  herr_t result;
...
void check_and_validate_cache_size(hid_t file_id, size_t *max_size_ptr,
  ...
  if (pass) {
    ...
    result = H5Fget_mdc_size(file_id, &max_size, &min_clean_size, &cur_size,
                             &cur_num_entries);
...
H5_ATTR_PURE hbool_t resize_configs_are_equal(const H5C_auto_size_ctl_t *a,
...
void validate_mdc_config(hid_t file_id, H5AC_cache_config_t *ext_config_ptr,
                         hbool_t compare_init, int test_num) {
  H5F_t *file_ptr = NULL;
  H5C_t *cache_ptr = NULL;
  ...
  if (pass) {
    ...
    if (file_ptr == NULL) {
      ...
      pass = FALSE;
      HDsnprintf(tmp_msg_buf, sizeof(tmp_msg_buf), "Can't get file_ptr #%d.",
                 test_num);
      failure_mssg = tmp_msg_buf;
    } else {
