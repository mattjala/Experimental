...
herr_t fake_wait_request_wait(void *req, uint64_t timeout,
...
herr_t fake_wait_request_wait(void H5_ATTR_UNUSED *req,
                              uint64_t H5_ATTR_UNUSED timeout,
                              H5VL_request_status_t *status) {
...
static int test_es_get_requests(void) {
  ...
  if (H5ESget_requests(es_id, H5_ITER_NATIVE, connector_ids, NULL, 2, &count) <
      0)
    TEST_ERROR;
  if (count != 0)
    TEST_ERROR;
  ...
  requests[0] = NULL;
  requests[1] = NULL;
  if (H5ESget_requests(es_id, H5_ITER_NATIVE, connector_ids, requests, 2,
                       &count) < 0)
    TEST_ERROR;
  if (count != 0)
    TEST_ERROR;
  ...
  requests[0] = NULL;
  requests[1] = NULL;
  if (H5ESget_requests(es_id, H5_ITER_INC, connector_ids, requests, 2, &count) <
      0)
    TEST_ERROR;
  if (count != 2)
    TEST_ERROR;
  ...
  requests[0] = NULL;
  requests[1] = NULL;
  if (H5ESget_requests(es_id, H5_ITER_DEC, connector_ids, requests, 2, &count) <
      0)
    TEST_ERROR;
  if (count != 2)
    TEST_ERROR;
  ...
  requests[0] = NULL;
  requests[1] = NULL;
  if (H5ESget_requests(es_id, H5_ITER_INC, connector_ids, requests, 1, &count) <
      0)
    TEST_ERROR;
  if (connector_ids[0] != connector_ids_g[0])
    TEST_ERROR;
  ...
  requests[0] = NULL;
  requests[1] = NULL;
  if (H5ESget_requests(es_id, H5_ITER_DEC, connector_ids, requests, 1, &count) <
      0)
    TEST_ERROR;
  if (connector_ids[0] != connector_ids_g[1])
    TEST_ERROR;
...
int main(void) {
  ...
  for (i = 0; i < EVENT_SET_NUM_CONNECTOR_IDS; i++)
    if ((connector_ids_g[i] =
             H5VLregister_connector(&fake_wait_vol_g, H5P_DEFAULT)) < 0)
      TEST_ERROR;
