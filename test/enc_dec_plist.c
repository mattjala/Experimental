...
static int test_encode_decode(hid_t orig_pl, H5F_libver_t low,
                              H5F_libver_t high) {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      HDsnprintf(msg, sizeof(msg),
                 "Testing ENCODE/DECODE with file version bounds: (%s, %s):",
      ...
      if ((H5Pset_attr_creation_order(
              ocpl, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED))) < 0)
        FAIL_STACK_ERROR;
      ...
      if ((H5Pset_link_creation_order(
              gcpl, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED))) < 0)
        FAIL_STACK_ERROR;
      ...
      if ((H5Pset_small_data_block_size(fapl, 2048)) < 0)
        FAIL_STACK_ERROR;
      if ((H5Pset_libver_bounds(fapl, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST)) <
          0)
        FAIL_STACK_ERROR;
