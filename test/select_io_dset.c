 ...
 */
static herr_t test_no_type_conv(hid_t fid, unsigned chunked, unsigned dtrans,
                                unsigned mwbuf) {
  int i;
 ...
 */
static herr_t test_no_size_change_no_bkg(hid_t fid, unsigned chunked,
                                         unsigned mwbuf) {
  int i;
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
 ...
 *
 */
static herr_t test_larger_mem_type_no_bkg(hid_t fid, unsigned chunked,
 ...
 */
static herr_t test_smaller_mem_type_no_bkg(hid_t fid, unsigned chunked,
                                           unsigned dtrans, unsigned mwbuf) {
  int i;
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
 ...
 */
static herr_t test_multi_dsets_no_bkg(hid_t fid, unsigned chunked,
                                      unsigned dtrans, unsigned mwbuf) {
  size_t ndsets;
  int i, j;
  hid_t dcpl = H5I_INVALID_HID;
 ...
 *   (c) Update all fields of the compound type in memory read buffer with new
 * unique values Randomized <mm> dataset:
 ...
 *      --Verify values read
 */
static herr_t test_multi_dsets_cmpd_with_bkg(hid_t fid, unsigned chunked,
                                             unsigned mwbuf) {
  size_t ndsets;
  int i, j, mm;
  hid_t dcpl = H5I_INVALID_HID;
 ...
 */
static herr_t test_multi_dsets_size_change_no_bkg(hid_t fid, unsigned chunked,
                                                  unsigned mwbuf) {
  size_t ndsets;
  int i, j;
  hid_t dcpl = H5I_INVALID_HID;
 ...
 *
 */
static herr_t test_multi_dsets_all(int niter, hid_t fid, unsigned chunked,
                                   unsigned mwbuf) {
  size_t ndsets;
  int i, j, mm;
  int s, n;
 ...
 */
static herr_t test_set_get_select_io_mode(hid_t fid) {
  hid_t did = H5I_INVALID_HID;
  hid_t sid = H5I_INVALID_HID;
  hid_t dcpl = H5I_INVALID_HID;
  hid_t dxpl = H5I_INVALID_HID;
  ...
  if ((did = H5Dcreate2(fid, "test_chk_dset", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
 ...
 */
static herr_t test_no_selection_io_cause_mode(const char *filename, hid_t fapl,
                                              uint32_t test_mode) {
  hid_t dcpl = H5I_INVALID_HID;
  hid_t dxpl = H5I_INVALID_HID;
  ...
  if (test_mode & TEST_PAGE_BUFFER) {
    ...
    /* The split and multi driver are not compatible with page buffering.  No
     * message since the other cases aren't skipped. */
  ...
  if (test_mode & TEST_PAGE_BUFFER) {
    ...
    if (H5Pset_page_buffer_size(fapl, 0, 0, 0) < 0)
      FAIL_STACK_ERROR;
    if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_FSM_AGGR, 0,
                                   (hsize_t)1) < 0)
      FAIL_STACK_ERROR;
  }
  ...
  if (test_mode & TEST_DATATYPE_CONVERSION) {
    ...
    if (test_mode & TEST_TCONV_BUF_TOO_SMALL) {
      ...
      /* If we're using in-place type conversion sel io will succeed and only
       * switch to scalar at the VFL */
  ...
  if (test_mode & TEST_NO_VECTOR_OR_SELECTION_IO_CB ||
      test_mode & TEST_DATATYPE_CONVERSION || test_mode & TEST_PAGE_BUFFER) {
...
static herr_t test_get_no_selection_io_cause(const char *filename, hid_t fapl) {
  ...
  errs += test_no_selection_io_cause_mode(
      filename, fapl, TEST_NOT_CONTIGUOUS_OR_CHUNKED_DATASET);
  errs += test_no_selection_io_cause_mode(filename, fapl,
                                          TEST_CONTIGUOUS_SIEVE_BUFFER);
  ...
  errs += test_no_selection_io_cause_mode(filename, fapl,
                                          TEST_NO_VECTOR_OR_SELECTION_IO_CB);
  errs +=
      ...
      for (mwbuf = FALSE; mwbuf <= TRUE; mwbuf++) {
        ...
        for (test_select_config = (int)TEST_NO_TYPE_CONV;
          ...
          case TEST_NO_TYPE_CONV: /* case 1 */
            ...
            nerrors +=
                (test_no_type_conv(fid, chunked, dtrans, mwbuf) < 0 ? 1 : 0);
          ...
          case TEST_NO_SIZE_CHANGE_NO_BKG: /* case 2 */
            ...
            if (dtrans)
              ...
              nerrors +=
                  (test_no_size_change_no_bkg(fid, chunked, mwbuf) < 0 ? 1 : 0);
          ...
          case TEST_LARGER_MEM_NO_BKG: /* case 3 */
            ...
            nerrors +=
                (test_larger_mem_type_no_bkg(fid, chunked, dtrans, mwbuf) < 0
                     ? 1
                     : 0);
          ...
          case TEST_SMALLER_MEM_NO_BKG: /* case 4 */
            ...
            nerrors +=
                (test_smaller_mem_type_no_bkg(fid, chunked, dtrans, mwbuf) < 0
                     ? 1
                     : 0);
