...
#define MAX_DBLOCK_PAGE_NELMTS_BITS                                            \
...
typedef enum {
  ...
  EARRAY_DIR_CYCLIC,  /* Insert every n'th object cyclicly: 0, n, 2n, 3n, ...,
                         nobjs/n, 1+nobjs/n,  1+n+nobjs/n, 1+2n+nobjs/n, ...,
                         nobjs */
  EARRAY_DIR_REVERSE, /* Insert objects from nobjs -> 0 */
  EARRAY_DIR_INWARD, /* Insert objects from outside to in: 0, nobjs, 1, nobjs-1,
                        2, nobjs-2, ..., nobjs/2 */
  EARRAY_DIR_OUTWARD, /* Insert objects from inside to out: nobjs/2,
                       * (nobjs/2)-1, (nobjs/2)+1, ..., 0, nobjs
                       */
...
typedef enum {
  ...
  EARRAY_TEST_COMP_N /* The number of different ways to test compressing array
                        blocks, must be last */
} earray_test_comp_t;
...
typedef struct earray_state_t {
  ...
  hsize_t max_idx_set; /* Highest element index stored (+1 - i.e. if element 0
                          has been set, this value with be '1', if no elements
                          have been stored, this value will be '0') */
...
typedef struct earray_iter_t {
  ...
  int (*state)(void *in_eiter, const H5EA_create_t *cparam,
               const earray_test_param_t *tparam, earray_state_t *state,
...
struct earray_test_param_t {
  earray_test_type_t
      reopen_array;           /* Whether to re-open the array during the test */
...
typedef struct earray_flush_depend_ctx_t {
  ...
  hbool_t
      idx0_obj; /* Flag to indicate that index 0's object has been flushed */
  hbool_t
      idx0_elem; /* Flag to indicate that index 0's element has been flushed */
  hbool_t
      idx1_obj; /* Flag to indicate that index 1's object has been flushed */
  hbool_t
      idx1_elem; /* Flag to indicate that index 1's element has been flushed */
  hbool_t idx10000_obj;  /* Flag to indicate that index 10000's object has been
                            flushed */
  hbool_t idx10000_elem; /* Flag to indicate that index 10000's element has been
                            flushed */
...
typedef struct earray_test_t {
  ...
  earray_flush_depend_ctx_t
      *fd_info; /* Context information for flush depend test */
} earray_test_t;
 ...
 *-------------------------------------------------------------------------
 */
static int init_tparam(earray_test_param_t *tparam,
  ...
  tparam->nsblks =
      1 + (cparam->max_nelmts_bits - H5VM_log2_of2(cparam->data_blk_min_elmts));

  /* Allocate information for each super block */
  tparam->sblk_info =
      (H5EA_sblk_info_t *)malloc(sizeof(H5EA_sblk_info_t) * tparam->nsblks);
  ...
  for (u = 0; u < tparam->nsblks; u++) {
    ...
    tparam->sblk_info[u].dblk_nelmts =
        (size_t)H5_EXP2((u + 1) / 2) * cparam->data_blk_min_elmts;
    ...
    start_idx += (hsize_t)tparam->sblk_info[u].ndblks *
                 (hsize_t)tparam->sblk_info[u].dblk_nelmts;
...
/* Don't compare this currently, it's very hard to compute */
  ...
  if (earray_stats.stored.data_blk_size != state->data_blk_size) {
    fprintf(stdout,
            "earray_stats.stored.data_blk_size = %" PRIuHSIZE
            ", state->data_blk_size = %" PRIuHSIZE "\n",
  ...
  if (earray_stats.stored.nsuper_blks != state->nsuper_blks) {
    fprintf(stdout,
            "earray_stats.stored.nsuper_blks = %" PRIuHSIZE
            ", state->nsuper_blks = %" PRIuHSIZE "\n",
...
/* Don't compare this currently, it's very hard to compute */
  ...
  if (earray_stats.stored.super_blk_size != state->super_blk_size) {
    fprintf(stdout,
            "earray_stats.stored.super_blk_size = %" PRIuHSIZE
            ", state->super_blk_size = %" PRIuHSIZE "\n",
 ...
 *-------------------------------------------------------------------------
 */
static int reopen_file(hid_t *file, H5F_t **f, hid_t fapl, H5EA_t **ea,
                       haddr_t ea_addr, const earray_test_param_t *tparam) {
 ...
 *-------------------------------------------------------------------------
 */
static int create_array(H5F_t *f, const H5EA_create_t *cparam, H5EA_t **ea,
                        haddr_t *ea_addr, H5EA__ctx_cb_t *cb) {
 ...
 *-------------------------------------------------------------------------
 */
static int finish(hid_t file, hid_t fapl, H5F_t *f, H5EA_t *ea,
                  haddr_t ea_addr) {
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_create(hid_t fapl, H5EA_create_t *cparam,
                            earray_test_param_t H5_ATTR_UNUSED *tparam) {
...
#ifndef NDEBUG
  {
    ...
    memcpy(&test_cparam, cparam, sizeof(test_cparam));
    test_cparam.max_dblk_page_nelmts_bits =
        4; /* corresponds to 16 elements in data block page, which is
    ...
    memcpy(&test_cparam, cparam, sizeof(test_cparam));
    test_cparam.max_dblk_page_nelmts_bits =
        (uint8_t)(test_cparam.max_nelmts_bits + 1);
    H5E_BEGIN_TRY { ea = H5EA_create(f, &test_cparam, NULL); }
    H5E_END_TRY
    if (ea) {
      /* Close opened extensible array */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_reopen(hid_t fapl, H5EA_create_t *cparam,
                            earray_test_param_t *tparam) {
  hid_t file = -1;               /* File ID */
  H5F_t *f = NULL;               /* Internal file object pointer */
  H5EA_t *ea = NULL;             /* Extensible array wrapper */
  haddr_t ea_addr = HADDR_UNDEF; /* Array address in file */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_open_twice(hid_t fapl, H5EA_create_t *cparam,
                                earray_test_param_t *tparam) {
  hid_t file = -1;               /* File ID */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_open_twice_diff(hid_t fapl, H5EA_create_t *cparam,
                                     earray_test_param_t *tparam) {
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_delete_open(hid_t fapl, H5EA_create_t *cparam,
                                 earray_test_param_t *tparam) {
  hid_t file = -1;               /* File ID */
  H5F_t *f = NULL;               /* Internal file object pointer */
  H5EA_t *ea = NULL;             /* Extensible array wrapper */
 ...
 *-------------------------------------------------------------------------
 */
static void *eiter_fw_init(const H5EA_create_t H5_ATTR_UNUSED *cparam,
                           const earray_test_param_t H5_ATTR_UNUSED *tparam,
 ...
 *-------------------------------------------------------------------------
 */
static int eiter_fw_state(void *in_eiter, const H5EA_create_t *cparam,
  ...
  if (idx < cparam->idx_blk_elmts) {
    ...
    sblk_idx = H5VM_log2_gen((
        uint64_t)(((idx - cparam->idx_blk_elmts) / cparam->data_blk_min_elmts) +
                  1));
...
typedef struct eiter_rv_t {
  ...
  hsize_t
      max_sblk_idx; /* Which superblock does the max. array location fall in? */
  hsize_t max_nelmts;     /* Max. # of elements for array */
  hsize_t max_ndata_blks; /* Max. # of data blocks for array */
  hsize_t idx_blk_nsblks; /* Number of superblocks directly pointed to in the
                             index block */
} eiter_rv_t;
 ...
 *-------------------------------------------------------------------------
 */
static void *eiter_rv_init(const H5EA_create_t *cparam,
                           const earray_test_param_t *tparam, hsize_t cnt) {
  ...
  if (cnt > cparam->idx_blk_elmts) {
    eiter->max_sblk_idx =
        H5VM_log2_gen((uint64_t)(((eiter->max - cparam->idx_blk_elmts) /
                                  cparam->data_blk_min_elmts) +
                                 1));
    eiter->max_nelmts =
        EA_NELMTS(cparam, tparam, eiter->max, eiter->max_sblk_idx);
    eiter->max_ndata_blks =
        EA_NDATA_BLKS(cparam, tparam, eiter->max, eiter->max_sblk_idx);
 ...
 *-------------------------------------------------------------------------
 */
static int eiter_rv_state(void *in_eiter, const H5EA_create_t *cparam,
                          const earray_test_param_t *tparam,
                          earray_state_t *state, hsize_t idx) {
  eiter_rv_t *eiter = (eiter_rv_t *)in_eiter;
  ...
  if (eiter->max < cparam->idx_blk_elmts) {
    ...
    hsize_t loc_idx =
        0; /* Local index, for computing an offset in next lower data block */
    unsigned idx_sblk_idx; /* Which superblock does this index fall in? */
    unsigned loc_sblk_idx =
        0; /* Which superblock does the local index fall in? */
    ...
    if (idx < cparam->idx_blk_elmts + cparam->data_blk_min_elmts)
      ...
      idx_sblk_idx = H5VM_log2_gen((uint64_t)(((idx - cparam->idx_blk_elmts) /
                                               cparam->data_blk_min_elmts) +
                                              1));
      tmp_idx = idx - (cparam->idx_blk_elmts +
                       tparam->sblk_info[idx_sblk_idx].start_idx);
      dblk_idx = tmp_idx / tparam->sblk_info[idx_sblk_idx].dblk_nelmts;
      if (dblk_idx > 0)
        loc_idx = idx - tparam->sblk_info[idx_sblk_idx].dblk_nelmts;
      else
        loc_idx = cparam->idx_blk_elmts +
                  tparam->sblk_info[idx_sblk_idx].start_idx - 1;
      loc_sblk_idx =
          H5VM_log2_gen((uint64_t)(((loc_idx - cparam->idx_blk_elmts) /
                                    cparam->data_blk_min_elmts) +
                                   1));
    } /* end else */
    ...
    /* Check if we have any super blocks yet */
    if (tparam->sblk_info[eiter->max_sblk_idx].ndblks >=
 ...
 *-------------------------------------------------------------------------
 */
static void *eiter_rnd_init(const H5EA_create_t H5_ATTR_UNUSED *cparam,
                            const earray_test_param_t H5_ATTR_UNUSED *tparam,
                            hsize_t cnt) {
 ...
 *-------------------------------------------------------------------------
 */
static void *eiter_rnd2_init(const H5EA_create_t H5_ATTR_UNUSED *cparam,
                             const earray_test_param_t H5_ATTR_UNUSED *tparam,
                             hsize_t cnt) {
  eiter_rnd_t *eiter; /* Random element iteration object */
  size_t u;           /* Local index variable */
  ...
  if (cnt > 1) {
    ...
    hsize_t sparse_cnt =
        (hsize_t)(cnt * EA_RND2_SCALE); /* Sparse range to choose from */
 ...
 *-------------------------------------------------------------------------
 */
static void *eiter_cyc_init(const H5EA_create_t H5_ATTR_UNUSED *cparam,
                            const earray_test_param_t H5_ATTR_UNUSED *tparam,
                            hsize_t cnt) {
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_set_elmts(hid_t fapl, H5EA_create_t *cparam,
                               earray_test_param_t *tparam, hsize_t nelmts,
                               const char *test_str) {
  hid_t file = -1;        /* File ID */
  H5F_t *f = NULL;        /* Internal file object pointer */
  H5EA_t *ea = NULL;      /* Extensible array wrapper */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_skip_elmts(hid_t fapl, H5EA_create_t *cparam,
                                earray_test_param_t *tparam, hsize_t skip_elmts,
                                const char *test_str) {
  hid_t file = -1;               /* File ID */
  H5F_t *f = NULL;               /* Internal file object pointer */
  H5EA_t *ea = NULL;             /* Extensible array wrapper */
...
int main(void) {
  ...
  for (curr_test = EARRAY_TEST_NORMAL; curr_test < EARRAY_TEST_NTESTS;
       curr_test++) {
    ...
    for (curr_iter = EARRAY_ITER_FW; curr_iter < EARRAY_ITER_NITERS;
         curr_iter++) {
      ...
      nerrors += test_set_elmts(fapl, &cparam, &tparam, (hsize_t)1,
                                "setting first element of array");
      nerrors +=
          test_set_elmts(fapl, &cparam, &tparam, (hsize_t)cparam.idx_blk_elmts,
      ...
      for (sblk = 0; sblk < 9; sblk++) {
        for (dblk = 0; dblk < tparam.sblk_info[sblk].ndblks; dblk++) {
          ...
          HDsnprintf(test_str, sizeof(test_str),
                     "setting all elements of array's data block #%llu",
                     (unsigned long long)ndblks);
          nerrors += test_set_elmts(fapl, &cparam, &tparam, nelmts, test_str);
    ...
    nerrors += test_skip_elmts(fapl, &cparam, &tparam, (hsize_t)1,
                               "skipping 1st element");
    nerrors +=
        test_skip_elmts(fapl, &cparam, &tparam, (hsize_t)cparam.idx_blk_elmts,
                        "skipping index block elements");
    nerrors += test_skip_elmts(
        fapl, &cparam, &tparam,
