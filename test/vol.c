...
static herr_t reg_opt_op_optional(void *obj, H5VL_optional_args_t *args,
                                  hid_t dxpl_id, void **req);
static herr_t reg_opt_link_optional(void *obj,
                                    const H5VL_loc_params_t *loc_params,
                                    H5VL_optional_args_t *args, hid_t dxpl_id,
                                    void **req);
static herr_t reg_opt_datatype_get(void *obj, H5VL_datatype_get_args_t *args,
                                   hid_t dxpl_id, void **req);
 ...
 *-------------------------------------------------------------------------
 */
static herr_t reg_opt_op_optional_verify(void *obj,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t reg_opt_datatype_get(void H5_ATTR_UNUSED *obj,
  ...
  if (H5VL_DATATYPE_GET_BINARY_SIZE == args->op_type) {
    ...
    if (H5Tencode(H5T_NATIVE_INT, args->args.get_binary.buf,
                  &args->args.get_binary.buf_size) < 0)
      ret_value = FAIL;
  } /* end if */
  else
    ret_value = FAIL;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t fake_get_cap_flags(const void H5_ATTR_UNUSED *info,
                                 uint64_t *cap_flags) {
 ...
 *-------------------------------------------------------------------------
 */
static herr_t fake_async_get_cap_flags(const void H5_ATTR_UNUSED *info,
                                       uint64_t *cap_flags) {
...
static herr_t test_vol_registration(void) {
  ...
  /* The test/fake VOL connector should not be registered at the start of the
   * test */
  ...
  if ((is_registered = H5VLis_connector_registered_by_value(FAKE_VOL_VALUE)) <
      0)
    TEST_ERROR;
  if (is_registered > 0)
    FAIL_PUTS_ERROR("VOL connector is inappropriately registered");
  ...
  if (H5I_INVALID_HID != vol_id)
    FAIL_PUTS_ERROR("should not be able to register a connector with an "
                    "incorrect property list");
  ...
  if (H5I_INVALID_HID ==
      (native_id = H5VLget_connector_id_by_name(H5VL_NATIVE_NAME)))
    TEST_ERROR;
  H5E_BEGIN_TRY { ret = H5VLunregister_connector(native_id); }
  H5E_END_TRY
  if (FAIL != ret)
    FAIL_PUTS_ERROR(
        "should not be able to unregister the native VOL connector");
...
static herr_t test_native_vol_init(void) {
  ...
  if ((is_registered = H5VLis_connector_registered_by_name(H5VL_NATIVE_NAME)) <
      0)
    TEST_ERROR;
  if (0 == is_registered)
  ...
  if ((is_registered =
           H5VLis_connector_registered_by_value(H5VL_NATIVE_VALUE)) < 0)
    TEST_ERROR;
  if (0 == is_registered)
    FAIL_PUTS_ERROR("native VOL connector is un-registered");
 ...
 *-------------------------------------------------------------------------
 */
static herr_t test_basic_file_operation(const char *env_h5_drvr) {
  ...
  if ((obj_count = H5Fget_obj_ids(fid, H5F_OBJ_ALL, 2, obj_id_list)) < 0)
    TEST_ERROR;
  if ((obj_count = H5Fget_obj_ids((hid_t)H5F_OBJ_ALL, H5F_OBJ_DATASET, 2,
                                  obj_id_list)) < 0)
    TEST_ERROR;

  /* Can't compare VFD properties for several VFDs */
  if ((hbool_t)(HDstrcmp(env_h5_drvr, "split") != 0 &&
                HDstrcmp(env_h5_drvr, "multi") != 0 &&
                HDstrcmp(env_h5_drvr, "family") != 0 &&
                HDstrcmp(env_h5_drvr, "direct") != 0 &&
                HDstrcmp(env_h5_drvr, "core") != 0 &&
                HDstrcmp(env_h5_drvr, "core_paged") != 0 &&
                HDstrcmp(env_h5_drvr, "mpio") != 0 &&
                HDstrcmp(env_h5_drvr, "splitter") != 0)) {
    /* H5Fget_access_plist */
...
static herr_t test_basic_group_operation(void) {
  ...
  /* H5Gcreate */
  if ((gid = H5Gcreate2(fid, NATIVE_VOL_TEST_GROUP_NAME, H5P_DEFAULT,
  ...
  /* H5Gget_info_by_name */
  if (H5Gget_info_by_name(fid, NATIVE_VOL_TEST_GROUP_NAME, &info, H5P_DEFAULT) <
      0)
    TEST_ERROR;

  /* H5Gget_info_by_idx */
  if (H5Gget_info_by_idx(fid, "/", H5_INDEX_NAME, H5_ITER_NATIVE, 0, &info,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* H5Gflush - skip for parallel file drivers as flush calls cause assertions
   * in the library */
 ...
 *
 * Purpose:     Uses the native VOL connector to test basic VOL dataset
 *operations
 *
 * Return:      SUCCEED/FAIL
 *
...
static herr_t test_basic_dataset_operation(void) {
  ...
  if (H5Pset_chunk(dcpl_id, 1, &curr_dims) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(fid, NATIVE_VOL_TEST_DATASET_NAME, H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* H5Dcreate_anon */
  if ((did_a = H5Dcreate_anon(fid, H5T_NATIVE_INT, sid, dcpl_id, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  /* H5Dflush - skip for parallel file drivers as flush calls cause assertions
   * in the library */
  if (h5_using_parallel_driver(fapl_id, &driver_is_parallel) < 0)
    TEST_ERROR;
  if (!driver_is_parallel)
 ...
 *
 * Purpose:     Uses the native VOL connector to test basic VOL attribute
 *operations
 *
 * Return:      SUCCEED/FAIL
 *
...
static herr_t test_basic_attribute_operation(void) {
  ...
  /* H5Acreate */
  if ((aid = H5Acreate2(fid, NATIVE_VOL_TEST_ATTRIBUTE_NAME, H5T_NATIVE_INT,
                        sid, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* H5Acreate_by_name */
  if ((aid_name = H5Acreate_by_name(
           fid, NATIVE_VOL_TEST_GROUP_NAME, NATIVE_VOL_TEST_ATTRIBUTE_NAME,
  ...
  /* H5Adelete_by_name */
  if (H5Adelete_by_name(fid, NATIVE_VOL_TEST_GROUP_NAME,
                        NATIVE_VOL_TEST_ATTRIBUTE_NAME, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Uses the native VOL connector to test basic VOL object
 *operations
 *
 * Return:      SUCCEED/FAIL
 *
...
static herr_t test_basic_object_operation(void) {
  ...
  /* H5Oget_info_by_name */
  if (H5Oget_info_by_name3(fid, NATIVE_VOL_TEST_GROUP_NAME, &object_info,
...
static herr_t test_basic_link_operation(void) {
  ...
  /* H5Lcreate_hard */
  if (H5Lcreate_hard(fid, "/", gid, NATIVE_VOL_TEST_HARD_LINK_NAME, H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* H5Lcreate_soft (to itself) */
  if (H5Lcreate_soft("/", fid, NATIVE_VOL_TEST_SOFT_LINK_NAME, H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* H5Lcopy */
  if (H5Lcopy(gid, NATIVE_VOL_TEST_HARD_LINK_NAME, fid,
              NATIVE_VOL_TEST_COPY_LINK_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* H5Lmove */
  if (H5Lmove(fid, NATIVE_VOL_TEST_COPY_LINK_NAME, gid,
              NATIVE_VOL_TEST_MOVE_LINK_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Uses the native VOL connector to test basic VOL datatype
 *operations
 *
 * Return:      SUCCEED/FAIL
 *
...
static herr_t test_basic_datatype_operation(void) {
  ...
  /* H5Tcommit */
  if (H5Tcommit2(fid, NATIVE_VOL_TEST_DATATYPE_NAME, tid, H5P_DEFAULT,
                 H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* H5Tflush - skip for parallel file drivers as flush calls cause assertions
   * in the library */
  if (h5_using_parallel_driver(fapl_id, &driver_is_parallel) < 0)
    TEST_ERROR;
  if (!driver_is_parallel)
...
typedef herr_t (*reg_opt_obj_oper_t)(const char *app_file, const char *app_func,
 ...
 *-------------------------------------------------------------------------
 */
static herr_t exercise_reg_opt_oper(hid_t fake_vol_id, hid_t reg_opt_vol_id,
                                    H5VL_subclass_t subcls,
                                    const char *subcls_name, H5I_type_t id_type,
  ...
  if (H5I_INVALID_HID == (obj_id = H5VL_register_using_vol_id(
                              id_type, &fake_obj, fake_vol_id, TRUE)))
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    if (H5VL_SUBCLS_LINK == subcls || H5VL_SUBCLS_OBJECT == subcls)
      ret = (*reg_opt_op.link_op)(__FILE__, __func__, __LINE__, obj_id, ".",
                                  H5P_DEFAULT, &vol_cb_args, H5P_DEFAULT,
                                  H5ES_NONE);
    else
      ret = (*reg_opt_op.obj_op)(__FILE__, __func__, __LINE__, obj_id,
                                 &vol_cb_args, H5P_DEFAULT, H5ES_NONE);
  }
  H5E_END_TRY
  if (FAIL != ret)
    FAIL_PUTS_ERROR("should not be able to perform an optional operation with "
                    "a NULL callback");
  if ((-1) != fake_obj)
  ...
  if (H5I_INVALID_HID == (obj_id = H5VL_register_using_vol_id(
                              id_type, &fake_obj, reg_opt_vol_id, TRUE)))
    TEST_ERROR;
...
static herr_t test_register_opt_operation(void) {
  ...
  struct {
    ...
    reg_opt_oper_t reg_opt_op;
      ...
      {H5VL_SUBCLS_ATTR, "attr", H5I_ATTR, {.obj_op = H5VLattr_optional_op}},
      {H5VL_SUBCLS_DATASET,
       "dataset",
       H5I_DATASET,
       {.obj_op = H5VLdataset_optional_op}},
      {H5VL_SUBCLS_DATATYPE,
       "datatype",
       H5I_DATATYPE,
       {.obj_op = H5VLdatatype_optional_op}},
      {H5VL_SUBCLS_FILE, "file", H5I_FILE, {.obj_op = H5VLfile_optional_op}},
      {H5VL_SUBCLS_GROUP,
       "group",
       H5I_GROUP,
       {.obj_op = H5VLgroup_optional_op}},
      {H5VL_SUBCLS_LINK, "link", H5I_GROUP, {.link_op = H5VLlink_optional_op}},
      {H5VL_SUBCLS_OBJECT,
       "object",
       H5I_GROUP,
       {.link_op = H5VLobject_optional_op}}};
  int op_val = -1;
  unsigned u;
  herr_t ret = SUCCEED;
  ...
  if ((fake_vol_id = H5VLregister_connector(&fake_vol_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((reg_opt_vol_id = H5VLregister_connector(&reg_opt_vol_g, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  H5E_END_TRY
  if (FAIL != ret)
    FAIL_PUTS_ERROR("should not be able to register an optional operation for "
                    "the 'NONE' VOL subclass");
  if ((-1) != op_val)
  ...
  if (FAIL != ret)
    FAIL_PUTS_ERROR("should not be able to register an optional operation for "
                    "the 'TOKEN' VOL subclass");
  if ((-1) != op_val)
    FAIL_PUTS_ERROR("'op_val' changed during failed operation?");

  /* Test registering valid optional VOL subclass operation with NULL op_val
   * ptr*/
  H5E_BEGIN_TRY {
  ...
  H5E_END_TRY
  if (FAIL != ret)
    FAIL_PUTS_ERROR("should not be able to register an optional operation with "
                    "a NULL 'op_val'");
  ...
  if (FAIL != ret)
    FAIL_PUTS_ERROR(
        "should not be able to find a non-existent optional operation");
  ...
  if (FAIL != ret)
    FAIL_PUTS_ERROR(
        "should not be able to unregister a non-existent optional operation");

  /* Optional operations on requests are supported (but difficult to test
   * further) */
  ...
  for (u = 0; u < NELMTS(test_params); u++)
    ...
    if (exercise_reg_opt_oper(fake_vol_id, reg_opt_vol_id,
                              test_params[u].subcls, test_params[u].subcls_name,
...
static herr_t test_async_vol_props(void) {
  ...
  /* Override possible environment variable & re-initialize default VOL
   * connector */
  ...
  /* Test query w/default VOL, which should indicate no async, since native
  ...
  /* Set environment variable to use 'fake async' connector & re-init default
   * connector */
  ...
  /* Stack the [internal] passthrough VOL connector on top of the fake async
   * connector */
...
static herr_t test_get_vol_name(void) {
  ...
  if (HDstrcmp(conn_env_str, "native") &&
      HDstrcmp(conn_env_str, "pass_through")) {
    SKIPPED();
  ...
  /* When comparing the pass_through connector, ignore the rest information
...
static herr_t test_wrap_register(void) {
  ...
  if ((group_id = H5Gcreate2(file_id, "test", H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Try to register a second ID for the group.  It should fail because this
   * routine is mainly targeted toward wrapping objects for iteration routine
   * callbacks, not for application use
   */
  H5E_BEGIN_TRY { wrap_id = H5VLwrap_register(vol_obj, H5I_GROUP); }
  H5E_END_TRY

  if (H5I_INVALID_HID != wrap_id)
    FAIL_PUTS_ERROR(
        "should not be able to call H5VLwrap_register in an application");
...
static herr_t test_query_optional(void) {
  ...
  if ((group_id = H5Gcreate2(file_id, "test_group", H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Test H5VLquery_optional with a group */
  if (H5VLquery_optional(group_id, H5VL_SUBCLS_OBJECT,
                         H5VL_NATIVE_OBJECT_GET_COMMENT, &supported) < 0)
    TEST_ERROR;
  ...
  if (H5Tcommit2(file_id, "test_dtype", dtype_id, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Test H5VLquery_optional with a committed datatype where the assertion
   * failure happened in the past */
  if (H5VLquery_optional(dtype_id, H5VL_SUBCLS_OBJECT,
                         H5VL_NATIVE_OBJECT_GET_COMMENT, &supported) < 0)
    TEST_ERROR;
