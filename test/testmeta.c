...
int main(void) {
  ...
  /* Create dataset */
  dataset_id = H5Dcreate2(file_id, "/NumDataObj", H5T_NATIVE_UINT, dataspace_id,
  ...
  for (i = 0; i < NEXTARRAYS; i++) {
    ...
    dataset_id = H5Dcreate2(file_id, name, H5T_NATIVE_FLOAT, dataspace_id,
  ...
  group_id =
      H5Gcreate2(file_id, "/DataArray", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  H5Gclose(group_id);
  ...
  for (j = 0; j < NDATAOBJECTS; j++) {
    ...
    for (i = 0; i < NDATAARRAYS; i++) {
      ...
      dataset_id = H5Dcreate2(group_id, name, H5T_NATIVE_FLOAT, dataspace_id,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
      if (dataset_id < 0) {
    ...
    for (i = 0; i < NEXTARRAYS; i++) {
      ...
      start[0] = 0;
      status = H5Sselect_hyperslab(memspace_id, H5S_SELECT_SET, start, stride,
                                   count, NULL);
      start[0] = (hsize_t)j;
      status = H5Sselect_hyperslab(dataspace_id, H5S_SELECT_SET, start, stride,
                                   count, NULL);
      status = H5Dwrite(dataset_id, type_id, memspace_id, dataspace_id,
                        H5P_DEFAULT, &floatval);
      if (status < 0) {
        fprintf(stderr, "Failed to write DataArray dataset.\n");
        status = H5Fclose(file_id);
        return -1;
