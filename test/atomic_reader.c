 ...
 *              c) atomic_data--the name of the data file used by writer and
 *reader
 *
 *-------------------------------------------------------------------------
 */
...
static void usage(void) {
  printf("\n");
  printf("Usage error!\n");
  printf("Usage: atomic_reader -n <number of integers to read> -i <number of "
         "iterations for reader>\n");
  printf(
      "       Note**The number of integers for option n has to be positive\n");
  printf(
      "       Note**The number of integers for option i has to be positive\n");
  printf("\n");
} /* usage() */
 ...
 *
 * Purpose:     To verify that the data read is the pattern expected.
 *              (1) Make sure the file opens successfully and the # of bytes
 *read is as expected (2) Iterate the reader with i iterations (3) Read and
 *verify n integers for each iteration (4) On verification error, re-read the
 *data at most READ_TRIES times to see if correct data can be obtained (5) Print
 *out statistics for the number of re-retries for each iteration
 *
 * Note:
 *      (a) The # of integers (via -n option) used by the writer and reader
 *should be the same. (b) The data file used by the writer and reader should be
 *the same.
 *
 * Future enhancement:
 *      1) Provide default values for n and i and allow user to run with either
 *0 or 1 option 2) Use HDF library HD<system calls> instead of the system calls
  ...
  for (i = 1; i <= iterations; i++) {
    ...
    while (opens--) {
      ...
      if ((fd = open(FILENAME, O_RDONLY, 0644)) < 0) {
        ...
        printf("READER: error from fstat or file size of %u is "
               "incorrect--retry open again\n",
               (unsigned int)sinfo.st_size);
        if (close(fd) < 0) {
    ...
    if (fd < 0) {
      printf("READER: *****open failure/incorrect file size for all %u tries, "
             "continue next "
             "iteration*****\n\n",
             OPEN_TRIES);
      continue;
