...
static void test_reference_params(void) {
  ...
  wbuf = (hobj_ref_t *)malloc(MAX(sizeof(unsigned), sizeof(hobj_ref_t)) *
                              SPACE1_DIM1);
  CHECK_PTR(wbuf, "malloc");
  rbuf = (hobj_ref_t *)malloc(MAX(sizeof(unsigned), sizeof(hobj_ref_t)) *
                              SPACE1_DIM1);
  CHECK_PTR(rbuf, "malloc");
  tbuf = (hobj_ref_t *)malloc(MAX(sizeof(unsigned), sizeof(hobj_ref_t)) *
                              SPACE1_DIM1);
  ...
  ret =
      H5Rcreate(&wbuf[0], (hid_t)-1, "/Group1/Dataset1", H5R_OBJECT, (hid_t)-1);
  ...
  ret = H5Rcreate(&wbuf[0], fid1, "/Group1/Dataset1", H5R_DATASET_REGION,
                  (hid_t)-1);
...
static void test_reference_region(H5F_libver_t libver_low,
  ...
  dset_NA = H5Dcreate2(fid1, "DS_NA", H5T_STD_REF_DSETREG, space_NA,
  ...
  /* Read the data */
  ret = H5Dread(dset_NA, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                rdata_NA);
  CHECK(ret, FAIL, "H5Dread");
  ...
  H5E_BEGIN_TRY {
    dset2 =
        H5Rdereference2(dset_NA, H5P_DEFAULT, H5R_DATASET_REGION, &rdata_NA[0]);
  }
  H5E_END_TRY
  ...
  /* Read selection from disk */
  ret =
      H5Dread(dset1, H5T_STD_REF_DSETREG, H5S_ALL, H5S_ALL, H5P_DEFAULT, rbuf);
  CHECK(ret, FAIL, "H5Dread");
  ...
  coords = (hsize_t *)malloc(
      (size_t)ret * SPACE2_RANK *
...
static void test_reference_obj_deleted(void) {
  ...
  /* Create a dataset */
  dataset = H5Dcreate2(fid1, "Dataset2", H5T_STD_REF_OBJ, sid1, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
  ...
  /* Write selection to disk */
  ret =
      H5Dwrite(dataset, H5T_STD_REF_OBJ, H5S_ALL, H5S_ALL, H5P_DEFAULT, &oref);
  CHECK(ret, FAIL, "H5Dwrite");
...
static void test_reference_group(void) {
  ...
  /* Create dataset */
  did = H5Dcreate2(fid, DSETNAME, H5T_STD_REF_OBJ, sid, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
...
static void test_reference_compat(void) {
  ...
  /* Create reference to dataset */
  ret =
      H5Rcreate(&wbuf_obj[0], fid1, "/Group1/Dataset1", H5R_OBJECT, (hid_t)-1);
  CHECK(ret, FAIL, "H5Rcreate");

  /* Create reference to dataset */
  ret =
      H5Rcreate(&wbuf_obj[1], fid1, "/Group1/Dataset2", H5R_OBJECT, (hid_t)-1);
  CHECK(ret, FAIL, "H5Rcreate");
  ...
  /* Create reference to named datatype */
  ret =
      H5Rcreate(&wbuf_obj[3], fid1, "/Group1/Datatype1", H5R_OBJECT, (hid_t)-1);
  CHECK(ret, FAIL, "H5Rcreate");
  ...
  dataset = H5Dcreate2(fid1, "Dataset4", H5T_STD_REF_DSETREG, sid1, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  CHECK(dataset, FAIL, "H5Dcreate2");
