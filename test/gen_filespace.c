...
const char *FILENAMES[] = {
    "fsm_aggr_nopersist.h5", /* H5F_FSPACE_STRATEGY_FSM_AGGR + not persisting
                                free-space */
    "fsm_aggr_persist.h5",   /* H5F_FSPACE_STRATEGY_FSM_AGGR + persisting
                                free-space */
    "paged_nopersist.h5",    /* H5F_FSPACE_STRATEGY_PAGE + not persisting
                                free-space */
...
int main(void) {
  ...
  for (fs_strategy = H5F_FSPACE_STRATEGY_FSM_AGGR;
    ...
    for (fs_persist = FALSE; fs_persist <= TRUE; fs_persist++) {
      ...
      if (H5Pset_file_space_strategy(fcpl, fs_strategy, fs_persist,
                                     (hsize_t)1) < 0)
        goto error;
      ...
      /* Write the dataset */
      if (H5Dwrite(did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
          0)
        goto error;
