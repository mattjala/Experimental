...
void tts_error(void) {
  ...
  if (vol_id == H5VL_NATIVE) {
    ...
    if (error_flag_g) {
      TestErrPrintf("At least one thread reported a value that was different "
                    "from the expected value\n");
      printf("(Update the expected_g[] array in tts_error for this test if the "
             "error stack changed!)\n");
    }
...
static void *tts_error_thread(void H5_ATTR_UNUSED *arg) {
  ...
  dataset = H5Dcreate2(error_file_g, DATASETNAME, datatype, dataspace,
...
static herr_t error_callback(hid_t H5_ATTR_UNUSED estack_id,
                             void *client_data) {
  H5TS_mutex_lock_simple(&error_mutex_g);
  error_count_g++;
  H5TS_mutex_unlock_simple(&error_mutex_g);
  return H5Ewalk2(H5E_DEFAULT, H5E_WALK_DOWNWARD, walk_error_callback,
                  client_data);
}

static herr_t walk_error_callback(unsigned n, const H5E_error2_t *err_desc,
                                  void H5_ATTR_UNUSED *client_data) {
  ...
  if (err_desc) {
    ...
    if (n <= EXPECTED_ERROR_DEPTH && maj_num == expected_g[n].maj_num &&
        min_num == expected_g[n].min_num)
      return SUCCEED;
  }
  ...
  fprintf(stderr,
          ...
          err_desc->maj_num, err_desc->min_num, err_desc->line,
          err_desc->func_name, err_desc->file_name, err_desc->desc);
...
void cleanup_error(void) { HDunlink(FILENAME); }
