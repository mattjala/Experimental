...
static void test_h5o_open(void) {
  ...
  ret = H5Tcommit2(fid, "group/datatype", dtype, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT);
  ...
  /* Create the dataset. */
  dset = H5Dcreate2(fid, "dataset", H5T_NATIVE_INT, dspace, H5P_DEFAULT,
  ...
  VERIFY(ginfo.nlinks, 1,
         "H5Gget_info"); /* There should be one object, the datatype */
...
static void test_h5o_open_by_addr(void) {
  ...
  /* For instance, an objectno smaller than the end of the file's superblock
...
static void test_h5o_refcount(void) {
  ...
  /* Output message about test being performed */
  MESSAGE(5,
          ("Testing retrieval of object reference count with H5Oget_info\n"));
  ...
  CHECK(dtype, FAIL, "H5Tcopy");
  ret =
      H5Tcommit2(fid, "datatype", dtype, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Tcommit2");
  ...
  /* Get ref counts for each object.  They should all be 1, since each object
   * has a hard link. */
  ...
  ret = H5Oget_info_by_name3(fid, "datatype", &oinfo, H5O_INFO_BASIC,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name3");
  VERIFY(oinfo.rc, 1, "reference count in H5Oget_info_by_name3");
  ret =
      H5Oget_info_by_name3(fid, "dataset", &oinfo, H5O_INFO_BASIC, H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name3");
  VERIFY(oinfo.rc, 1, "reference count in H5Oget_info_by_name3");
  ...
  /* Increment the reference counts and then close the file to make sure the
   * increment is permanent */
  ret = H5Oincr_refcount(grp);
  CHECK(ret, FAIL, "H5Oincr_refcount");
  ret = H5Oincr_refcount(dtype);
  CHECK(ret, FAIL, "H5Oincr_refcount");
  ...
  /* Re-open the file and check that the reference counts were really
   * incremented */
...
static void test_h5o_plist(void) {
  ...
  /* Set non-default attribute phase change values on each creation property
   * list */
  ...
  /* Retrieve attribute phase change values on each creation property list and
   * verify */
...
static void test_h5o_link(void) {
  ...
  for (low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
    for (high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
      ...
      dset_id =
          H5Dcreate_anon(file_id, type_id, space_id, H5P_DEFAULT, H5P_DEFAULT);
      CHECK(dset_id, FAIL, "H5Dcreate_anon");

      /* Verify that we can write to and read from the dataset */

      /* Write the data to the dataset */
      ret = H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     wdata);
      CHECK(ret, FAIL, "H5Dwrite");

      /* Read the data back */
      ret = H5Dread(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    rdata);
      ...
      ret = H5Olink(dset_id, group_id, "inter_group/dataset", lcpl_id,
                    H5P_DEFAULT);
      CHECK(ret, FAIL, "H5Olink");
...
static void test_h5o_comment(void) {
  ...
  attr_id = H5Acreate2(fid, "file attribute", H5T_NATIVE_INT, attr_space,
                       H5P_DEFAULT, H5P_DEFAULT);
...
static void test_h5o_comment_by_name(void) {
  ...
  len = H5Oget_comment_by_name(fid, ".", check_comment, (size_t)comment_len + 1,
                               H5P_DEFAULT);
  ...
  /* Getting the comment on the group and verify it */
  comment_len =
      H5Oget_comment_by_name(fid, "group", NULL, (size_t)0, H5P_DEFAULT);
  CHECK(comment_len, FAIL, "H5Oget_comment_by_name");

  len = H5Oget_comment_by_name(fid, "group", check_comment,
  ...
  /* Getting the comment on the datatype and verify it */
  comment_len =
      H5Oget_comment_by_name(grp, "datatype", NULL, (size_t)0, H5P_DEFAULT);
  CHECK(comment_len, FAIL, "H5Oget_comment_by_name");

  len = H5Oget_comment_by_name(grp, "datatype", check_comment,
                               (size_t)comment_len + 1, H5P_DEFAULT);
  CHECK(len, FAIL, "H5Oget_comment");
  ...
  /* Getting the comment on the dataset and verify it */
  comment_len =
      H5Oget_comment_by_name(fid, "dataset", NULL, (size_t)0, H5P_DEFAULT);
  CHECK(comment_len, FAIL, "H5Oget_comment_by_name");

  len = H5Oget_comment_by_name(fid, "dataset", check_comment,
                               (size_t)comment_len + 1, H5P_DEFAULT);
  CHECK(len, FAIL, "H5Oget_comment_by_name");
...
static void test_h5o_getinfo_same_file(void) {
  ...
  ret = H5Oget_info_by_name3(fid1, "group1", &oinfo1, H5O_INFO_BASIC,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name3");
  ret = H5Oget_info_by_name3(fid1, "group2", &oinfo2, H5O_INFO_BASIC,
                             H5P_DEFAULT);
  CHECK(ret, FAIL, "H5Oget_info_by_name3");
...
**
****************************************************************/
static int visit_obj_cb(hid_t group_id, const char *name,
                        const H5O_info1_t *oinfo1,
  ...
  if (!(HDstrcmp(name, "group1"))) {
    H5Oget_info_by_name2(group_id, name, &oinfo2, H5O_INFO_NUM_ATTRS,
                         H5P_DEFAULT);
    ...
    VERIFY(oinfo1->hdr.nmesgs, oinfo2.hdr.nmesgs,
           "obj info from H5Ovisit1/H5Oget_info2");
    VERIFY(oinfo1->hdr.nchunks, oinfo2.hdr.nchunks,
           "obj info from H5Ovisit1/H5Oget_info2");
  } else if (!(HDstrcmp(name, "."))) {
    H5Oget_info_by_name2(group_id, name, &oinfo2, H5O_INFO_META_SIZE,
                         H5P_DEFAULT);
...
static void test_h5o_getinfo_visit(void) {
  ...
  for (j = 0; j < 10; j++) {
    ...
    aid = H5Acreate2(gid1, attrname, H5T_NATIVE_INT, sid, H5P_DEFAULT,
                     H5P_DEFAULT);
...
#ifndef H5_NO_DEPRECATED_SYMBOLS
  test_h5o_open_by_addr_deprec(); /* Test opening objects by address with
                                     H5Lget_info1 */
