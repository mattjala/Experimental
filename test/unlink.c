...
static const char *FILENAME[] = {
    "unlink",          "new_move_a",     "new_move_b",    "lunlink",
...
static int test_one(hid_t file) {
  ...
  /* Create a test group */
  if ((work = H5Gcreate2(file, "/test_one", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Delete by absolute name */
  TESTING("unlink by absolute name");
  if ((grp = H5Gcreate2(work, "foo", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    FAIL_STACK_ERROR;
  if (H5Gclose(grp) < 0)
    FAIL_STACK_ERROR;
...
static int test_many(hid_t file) {
  ...
  /* Create a test group */
  if ((work = H5Gcreate2(file, "/test_many", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    goto error;
  if ((grp = H5Gcreate2(work, "/test_many_foo", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    goto error;
  if (H5Gclose(grp) < 0)
    goto error;
  ...
  for (i = 0; i < how_many; i++) {
    ...
    if (H5Lcreate_hard(work, "/test_many_foo", H5L_SAME_LOC, name, H5P_DEFAULT,
                       H5P_DEFAULT) < 0)
      FAIL_STACK_ERROR;
  } /* end for */
  for (i = 0; i < how_many; i++) {
...
static int test_symlink(hid_t file) {
  ...
  if ((work = H5Gcreate2(file, "/test_symlink", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
...
static int test_rename(hid_t file) {
  ...
  if ((work = H5Gcreate2(file, "/test_rename", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((foo = H5Gcreate2(work, "foo", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    FAIL_STACK_ERROR;
  if (H5Lmove(work, "foo", H5L_SAME_LOC, "bar", H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if ((inner =
           H5Gcreate2(foo, "inner", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(inner) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(foo) < 0)
  ...
  if (H5Lcreate_soft("link_value", work, "link_one", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
  if (H5Lmove(work, "link_one", H5L_SAME_LOC, "link_two", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
...
static int check_new_move(hid_t fapl) {
  ...
  /* Get hard link info */
  if (H5Oget_info_by_name3(file, "/group2/group_new_name", &oi_hard1,
                           H5O_INFO_BASIC, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Oget_info_by_name3(file, "/group1/hard", &oi_hard2, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if (token_cmp)
    FAIL_PUTS_ERROR("    Hard link test failed.  Link seems not to point to "
                    "the expected file location.");

  /* Check soft links */
  if (H5Lget_val(file, "group2/soft", linkval, sizeof(linkval), H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
...
static int test_filespace(hid_t fapl) {
  hid_t fapl_nocache; /* File access property list with raw data cache turned
                         off */
  hid_t contig_dcpl; /* Dataset creation property list for contiguous dataset */
  hid_t early_chunk_dcpl; /* Dataset creation property list for chunked dataset
                             & early allocation */
  hid_t late_chunk_dcpl; /* Dataset creation property list for chunked dataset &
                            late allocation */
  hid_t comp_dcpl;     /* Dataset creation property list for compressed, chunked
                          dataset */
  ...
  hsize_t dims[FILESPACE_NDIMS] = {FILESPACE_DIM0, FILESPACE_DIM1,
                                   FILESPACE_DIM2}; /* Dataset dimensions */
  hsize_t chunk_dims[FILESPACE_NDIMS] = {
      FILESPACE_CHUNK0, FILESPACE_CHUNK1,
      FILESPACE_CHUNK2}; /* Chunk dimensions */
  hsize_t attr_dims[FILESPACE_ATTR_NDIMS] = {
      FILESPACE_ATTR_DIM0, FILESPACE_ATTR_DIM1}; /* Attribute dimensions */
  ...
  if (H5Pget_cache(fapl_nocache, &mdc_nelmts, &rdcc_nelmts, &rdcc_nbytes,
                   &rdcc_w0) < 0)
    TEST_ERROR;
  ...
  rdcc_nelmts = 0;
  rdcc_nbytes = 0;
  if (H5Pset_cache(fapl_nocache, mdc_nelmts, rdcc_nelmts, rdcc_nbytes,
                   rdcc_w0) < 0)
    TEST_ERROR;
  ...
  /* Create dataset creation property list for chunked storage & early
   * allocation */
  ...
  /* Create dataset creation property list for compressed, chunked storage &
   * early allocation */
  ...
  if (NULL == (data = (int *)malloc(sizeof(int) * FILESPACE_DIM0 *
                                    FILESPACE_DIM1 * FILESPACE_DIM2)))
    TEST_ERROR;

  /* Create single dataset (with contiguous storage & late allocation), remove
   * it & verify file size */
  ...
  if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space,
  ...
  /* Create single dataset (with contiguous storage & early allocation), remove
   * it & verify file size */
  ...
  /* Create a single dataset to remove */
  if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space,
                            H5P_DEFAULT, contig_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Dclose(dataset) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create single dataset (with chunked storage & late allocation), remove it &
   * verify file size */
  ...
  /* Create a single dataset to remove */
  if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space,
                            H5P_DEFAULT, late_chunk_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Dclose(dataset) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create single dataset (with chunked storage & early allocation), remove it
   * & verify file size */
  ...
  /* Create a single dataset to remove */
  if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space,
                            H5P_DEFAULT, early_chunk_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Dclose(dataset) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create single dataset (with compressed storage & early allocation), remove
   * it & verify file size */
  ...
  /* Create a single dataset to remove */
  if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space,
                            H5P_DEFAULT, comp_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Dclose(dataset) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create single dataset (with compressed storage & early allocation),
   * re-write it a bunch of times (which should re-allocate blocks many times)
   * and remove it & verify file size.
   */
  TESTING("    re-writing compressed, chunked dataset");

  /* Create file (using FAPL with disabled raw data cache) */
  if ((file = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_nocache)) <
      0)
    FAIL_STACK_ERROR;
  ...
  for (u = 0; u < FILESPACE_REWRITE; u++) {
    ...
    for (v = 0, tmp_data = data;
         v < (FILESPACE_DIM0 * FILESPACE_DIM1 * FILESPACE_DIM2); v++)
      *tmp_data++ = (int)(v * u);

    /* Write the buffer to the dataset */
    if (H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
        0)
      FAIL_STACK_ERROR;
  ...
  /* Create a single dataset to remove */
  if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space,
                            H5P_DEFAULT, compact_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Dclose(dataset) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space,
                            H5P_DEFAULT, contig_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((dataset2 = H5Dcreate2(file, DATASET2NAME, H5T_NATIVE_INT, space,
                             H5P_DEFAULT, contig_dcpl, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Create a dataspace for the attributes */
  if ((attr_space = H5Screate_simple(FILESPACE_ATTR_NDIMS, attr_dims, NULL)) <
      0)
    FAIL_STACK_ERROR;
  ...
  for (u = 0; u < FILESPACE_NATTR; u++) {
    ...
    if ((attr = H5Acreate2(dataset, objname, H5T_NATIVE_INT, attr_space,
    ...
    if ((attr = H5Acreate2(dataset2, objname, H5T_NATIVE_INT, attr_space,
                           H5P_DEFAULT, H5P_DEFAULT)) < 0)
      FAIL_STACK_ERROR;
  ...
  if (H5Tcommit2(file, TYPENAME, type, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
  if (H5Tclose(type) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((group = H5Gcreate2(file, GROUPNAME, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(group) < 0)
    FAIL_STACK_ERROR;
  ...
  for (u = 0; u < UNLINK_NGROUPS; u++) {
    ...
    if ((group = H5Gcreate2(file, objname, H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT)) < 0)
      FAIL_STACK_ERROR;
    if (H5Gclose(group) < 0)
      FAIL_STACK_ERROR;
  ...
  /* (Remove them in reverse order just to make file size calculation easier
   * -QAK) */
  ...
  if ((group = H5Gcreate2(file, GROUPNAME, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((group2 = H5Gcreate2(group, GROUP2NAME, H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  for (u = 0; u < FILESPACE_TOP_GROUPS; u++) {
    ...
    for (v = 0; v < FILESPACE_NESTED_GROUPS; v++) {
      ...
      if ((group2 = H5Gcreate2(group, objname, H5P_DEFAULT, H5P_DEFAULT,
                               H5P_DEFAULT)) < 0)
        FAIL_STACK_ERROR;
      ...
      for (w = 0; w < FILESPACE_NDATASETS; w++) {
        ...
        if ((dataset = H5Dcreate2(group2, objname, H5T_NATIVE_INT, space,
                                  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
          FAIL_STACK_ERROR;
        if (H5Dclose(dataset) < 0)
          FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    dataset = H5Dcreate2(file, DATASETNAME, H5T_NATIVE_INT, space, H5P_DEFAULT,
  ...
  /* Create named datatype and duplicate named datatype, remove original &
   * verify file size */
  ...
  H5E_BEGIN_TRY {
    status =
        H5Tcommit2(file, TYPENAME, type, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (status >= 0)
    TEST_ERROR;
  ...
  if ((attr = H5Acreate2(dataset, ATTRNAME, H5T_NATIVE_INT, attr_space,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    attr = H5Acreate2(dataset, ATTRNAME, H5T_NATIVE_INT, attr_space,
                      H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (attr >= 0) {
...
static int test_link_slashes(hid_t fapl) {
  ...
  if ((gid = H5Gcreate2(fid, SLASHES_GROUP_NAME, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Create a nested group in the root group */
  if ((gid2 = H5Gcreate2(gid, SLASHES_GROUP_NAME, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Lcreate_hard(gid, SLASHES_GROUP_NAME, H5L_SAME_LOC,
                     SLASHES_HARDLINK_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* Create a soft link with a relative path to the nested group */
  if (H5Lcreate_soft(SLASHES_GROUP_NAME, gid, SLASHES_SOFTLINK_NAME,
                     H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* Create a soft link with the full path to the nested group */
  if (H5Lcreate_soft("////" SLASHES_GROUP_NAME "" SLASHES_GROUP_NAME, gid,
                     SLASHES_SOFTLINK2_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* Create a soft link to the root group */
  if (H5Lcreate_soft("////", gid, SLASHES_ROOTLINK_NAME, H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Lcreate_hard(fid, SLASHES_GROUP_NAME, H5L_SAME_LOC,
                     SLASHES_HARDLINK_NAME, H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
...
static int test_unlink_rightleaf(hid_t fid) {
  ...
  for (n = 0; n < ngroups; n++) {
    ...
    if ((gids[n] = H5Gcreate2(rootid, name, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
      TEST_ERROR;
  } /* end for */
...
static int test_unlink_middlenode(hid_t fid) {
  ...
  /* Unlink specific objects to trigger deletion of all leafs in "interior"
   * non-leaf node */
...
static int test_resurrect_dataset(hid_t fapl) {
  ...
  if ((s = H5Screate(H5S_SCALAR)) < 0)
    FAIL_STACK_ERROR;
  if ((d = H5Dcreate2(f, DATASETNAME, H5T_NATIVE_INT, s, H5P_DEFAULT,
                      H5P_DEFAULT, H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Sclose(s) < 0)
    FAIL_STACK_ERROR;
...
static int test_resurrect_group(hid_t fapl) {
  ...
  if (H5Lcreate_hard(group, ".", file, GROUP2NAME, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
...
static int test_unlink_chunked_dataset(hid_t fapl) {
  ...
  hsize_t dims[FILESPACE_NDIMS] = {FILESPACE_DIM0, FILESPACE_DIM1,
                                   FILESPACE_DIM2};
  hsize_t max_dims[FILESPACE_NDIMS] = {H5S_UNLIMITED, H5S_UNLIMITED,
                                       H5S_UNLIMITED};
  ...
  /* Create the dataset */
  if ((dset_id = H5Dcreate2(file_id, DATASETNAME, H5T_NATIVE_INT, space_id,
 ...
 *
 * Purpose:     Test deleting a compact group which still has valid objects in
 *it
 *
 * Return:      Success:        0
 *              Failure:        number of errors
...
static int test_full_group_compact(hid_t fapl) {
  ...
  if ((gid = H5Gcreate2(file_id, "/keep", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((gid = H5Gcreate2(file_id, "/delete", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;

  /* Create external link (doesn't matter if it dangles) */
  if (H5Lcreate_external("foo.h5", "/dst", gid, "external", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  for (u = 0; u < FULL_GROUP_NUM_KEEP; u++) {
    ...
    if (H5Lcreate_hard(file_id, objname, gid, objname2, H5P_DEFAULT,
                       H5P_DEFAULT) < 0)
      FAIL_STACK_ERROR;
  } /* end for */
  ...
  for (u = 0; u < FULL_GROUP_NUM_KEEP; u++) {
    HDsnprintf(objname, sizeof(objname), "/keep/keep %u\n", u);
    if (H5Oget_info_by_name3(file_id, objname, &oi, H5O_INFO_BASIC,
                             H5P_DEFAULT) < 0)
      FAIL_STACK_ERROR;
    if (oi.rc != 2)
      TEST_ERROR;
...
static int test_full_group_dense(hid_t fapl) {
  ...
  if ((gcpl = H5Pcreate(H5P_GROUP_CREATE)) < 0)
    FAIL_STACK_ERROR;
  if (H5Pset_est_link_info(gcpl, FULL_GROUP_EST_NUM_ENTRIES,
                           FULL_GROUP_EST_ENTRY_LEN) < 0)
    FAIL_STACK_ERROR;
   ...
   */
  if ((gid = H5Gcreate2(file_id, "/delete", H5P_DEFAULT, gcpl, H5P_DEFAULT)) <
      0)
    FAIL_STACK_ERROR;
...
int main(void) {
  ...
  for (new_format = FALSE; new_format <= TRUE; new_format++) {
    ...
    nerrors += test_create_unlink("create and unlink large number of objects",
                                  my_fapl);

    {
      hid_t fapl_small_mdc;

      /* Make copy of regular fapl, to turn down the elements in the metadata
       * cache */
      if ((fapl_small_mdc = H5Pcopy(my_fapl)) < 0)
        goto error;

      /* Get FAPL cache settings */
      if (H5Pget_cache(fapl_small_mdc, &mdc_nelmts, &rdcc_nelmts, &rdcc_nbytes,
                       &rdcc_w0) < 0)
        printf("H5Pget_cache failed\n");

      /* Change FAPL cache settings */
      mdc_nelmts = 1;
      if (H5Pset_cache(fapl_small_mdc, mdc_nelmts, rdcc_nelmts, rdcc_nbytes,
                       rdcc_w0) < 0)
        printf("H5Pset_cache failed\n");

      /* Test creating & unlinking lots of objects with a 1-element metadata
       * cache FAPL */
      nerrors += test_create_unlink(
          "create and unlink large number of objects with small cache",
          fapl_small_mdc);
