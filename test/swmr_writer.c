...
static hid_t open_skeleton(const char *filename, hbool_t verbose,
...
static hid_t open_skeleton(const char *filename, hbool_t verbose,
  ...
  if (use_log_vfd) {
    ...
    HDsnprintf(verbose_name, sizeof(verbose_name), "swmr_writer.log.%u",
               random_seed);
  ...
  for (u = 0; u < NLEVELS; u++)
    for (v = 0; v < symbol_count[u]; v++) {
      if ((symbol_info[u][v].dsid =
               H5Dopen2(fid, symbol_info[u][v].name, H5P_DEFAULT)) < 0)
        return -1;
...
static void usage(void) {
  ...
  printf("Usage error!\n");
  printf("\n");
  printf("Usage: swmr_writer [-q] [-o] [-f <# of records to write between "
         "flushing\n");
  ...
  printf("\n");
  printf("Defaults to verbose (no '-q' given), latest format when opening file "
         "(no '-o' given),\n");
  printf("flushing every 10000 records ('-f 10000'), and will generate a "
         "random seed (no -r given).\n");
  printf("\n");
  exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
  ...
  /* Open file skeleton */
  if ((fid = open_skeleton(FILENAME, verbose, verbose_file, random_seed, old)) <
      0) {
