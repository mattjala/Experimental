...
static herr_t UD_hard_delete(const char *link_name, hid_t file,
                             const void *udata, size_t udata_size);
...
static herr_t UD_hard_create_deprec(const char *link_name, hid_t loc_group,
                                    const void *udata, size_t udata_size,
                                    hid_t lcpl_id);
static hid_t UD_hard_traverse_deprec(const char *link_name, hid_t cur_group,
                                     const void *udata, size_t udata_size,
                                     hid_t lapl_id, hid_t dxpl_id);
static herr_t UD_hard_delete_deprec(const char *link_name, hid_t file,
                                    const void *udata, size_t udata_size);
...
static hid_t UD_rereg_traverse(const char *link_name, hid_t cur_group,
                               const void *udata, size_t udata_size,
                               hid_t lapl_id, hid_t dxpl_id);
...
static herr_t UD_cb_create(const char *link_name, hid_t loc_group,
                           const void *udata, size_t udata_size, hid_t lcpl_id);
static herr_t UD_cb_move(const char *new_name, hid_t new_loc, const void *udata,
                         size_t udata_size);
static hid_t UD_cb_traverse(const char *link_name, hid_t cur_group,
                            const void *udata, size_t udata_size, hid_t lapl_id,
                            hid_t dxpl_id);
static herr_t UD_cb_delete(const char *link_name, hid_t file, const void *udata,
                           size_t udata_size);
static ssize_t UD_cb_query(const char *link_name, const void *udata,
                           size_t udata_size, void *buf, size_t buf_size);
 ...
 *-------------------------------------------------------------------------
 */
static void fix_ext_filename(char *path_name, char *cwd,
 ...
 *-------------------------------------------------------------------------
 */
static int mklinks(hid_t fapl, hbool_t new_format) {
  ...
  /* Create a group */
  if ((grp = H5Gcreate2(file, "grp1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Gclose(grp) < 0)
    TEST_ERROR;
  ...
  /* Create a dataset */
  if ((d1 = H5Dcreate2(file, "d1", H5T_NATIVE_INT, scalar, H5P_DEFAULT, dcpl_g,
                       H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dclose(d1) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_soft("foobar", file, "grp1/dangle", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;

  /* Create a recursive symbolic link */
  if (H5Lcreate_soft("/grp1/recursive", file, "/grp1/recursive", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int new_links(hid_t fapl, hbool_t new_format) {
  ...
  if ((grp1_a = H5Gcreate2(file_a, "grp1", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((grp2_a = H5Gcreate2(file_a, "grp2", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((grp1_b = H5Gcreate2(file_b, "grp1", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((grp2_b = H5Gcreate2(file_b, "grp2", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create datasets */
  if ((dset1 = H5Dcreate2(file_a, "dataset1", H5T_NATIVE_INT, scalar,
                          H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((dset2 = H5Dcreate2(grp1_a, "dataset2", H5T_NATIVE_INT, scalar,
                          H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    if (H5Lcreate_hard(H5L_SAME_LOC, "dataset1", H5L_SAME_LOC, "hard",
                       H5P_DEFAULT, H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  H5E_BEGIN_TRY {
    if (H5Lcreate_soft("dataset1", H5L_SAME_LOC, "soft", H5P_DEFAULT,
                       H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  H5E_BEGIN_TRY {
    if (H5Lcreate_hard(file_a, "dataset1", file_b, "hard", H5P_DEFAULT,
                       H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  if (H5Lcreate_hard(grp1_a, "dataset2", grp2_a, "hard2", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int cklinks(hid_t fapl, hbool_t new_format) {
  hid_t file;
  ...
  /* Hard link */
  if (H5Oget_info_by_name3(file, "d1", &oinfo1, H5O_INFO_BASIC, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
  if (H5Oget_info_by_name3(file, "grp1/hard", &oinfo2, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if (status >= 0) {
    H5_FAILED();
    HDputs("    H5Lexists() should have failed for a path with missing "
           "components.");
    TEST_ERROR;
  } /* end if */
  H5E_BEGIN_TRY { status = H5Lexists(file, "/no_grp1/hard", H5P_DEFAULT); }
  ...
  /* Symbolic link */
  if (H5Oget_info_by_name3(file, "grp1/soft", &oinfo2, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5O_TYPE_DATASET != oinfo2.type) {
    H5_FAILED();
  ...
  /* Dangling link */
  H5E_BEGIN_TRY {
    status = H5Oget_info_by_name3(file, "grp1/dangle", &oinfo2, H5O_INFO_BASIC,
                                  H5P_DEFAULT);
  }
  H5E_END_TRY
  if (status >= 0) {
  ...
  if (H5Lget_val(file, "grp1/dangle", linkval, sizeof linkval, H5P_DEFAULT) <
      0) {
    H5_FAILED();
  ...
  /* Recursive link */
  H5E_BEGIN_TRY {
    status = H5Oget_info_by_name3(file, "grp1/recursive", &oinfo2,
  ...
  if (status >= 0) {
    H5_FAILED();
    HDputs(
        "    H5Oget_info_by_name() should have failed for a recursive link.");
    TEST_ERROR;
  } /* end if */
  ...
  if (H5Lget_val(file, "grp1/recursive", linkval, sizeof linkval, H5P_DEFAULT) <
      0) {
    H5_FAILED();
    printf("    %d: Can't retrieve link value\n", __LINE__);
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int ck_new_links(hid_t fapl, hbool_t new_format) {
  hid_t file;
  ...
  /* Get hard link info */
  if (H5Oget_info_by_name3(file, "/grp1/dataset2", &oi_dset, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file, "/grp1/hard1", &oi_hard1, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file, "/grp2/hard2", &oi_hard2, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int long_links(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;       /* File ID */
  ...
  if ((gid = H5Gcreate2(fid, "grp1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  if ((gid2 = H5Gcreate2(gid, objname, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int toomany(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  ...
  /* Create group with short name in file (used as target for hard links) */
  if ((gid = H5Gcreate2(fid, "final", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  if (H5Lcreate_hard(fid, "hard9", fid, "hard10", H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard10", fid, "hard11", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard11", fid, "hard12", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard12", fid, "hard13", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard13", fid, "hard14", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard14", fid, "hard15", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard15", fid, "hard16", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard16", fid, "hard17", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard17", fid, "hard18", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard18", fid, "hard19", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard19", fid, "hard20", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "hard20", fid, "hard21", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  ...
  if ((gid2 = H5Gcreate2(gid, "new_hard", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((gid2 = H5Gcreate2(gid, "new_soft", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_lcpl(hid_t fapl, hbool_t new_format) {
  hid_t file_id = -1;
  hid_t group_id = -1;
  hid_t space_id = -1;
  hid_t dset_id = -1;
  ...
  if ((group_id = H5Gcreate2(file_id, "/group", H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((type_id = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(file_id, "/type", type_id, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((dset_id = H5Dcreate2(file_id, "/dataset", H5T_NATIVE_INT, space_id,
                            H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((group_id = H5Gcreate2(file_id, "/group2", lcpl_id, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(group_id) < 0)
    TEST_ERROR;
  ...
  if ((type_id = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(file_id, "/type2", type_id, lcpl_id, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Tclose(type_id) < 0)
    TEST_ERROR;
  ...
  if ((dset_id = H5Dcreate2(file_id, "/dataset2", H5T_NATIVE_INT, space_id,
                            lcpl_id, dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dclose(dset_id) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_hard(file_id, "/dataset2", file_id, "/dataset2_link", lcpl_id,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Pset_char_encoding(lcpl_id, H5T_CSET_UTF8) < 0)
    TEST_ERROR;
  if (H5Lcreate_soft("dataset2", file_id, "slink_to_dset2", lcpl_id,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Pset_char_encoding(lcpl_id, H5T_CSET_ASCII) < 0)
    TEST_ERROR;
  if (H5Lmove(file_id, "slink_to_dset2", file_id, "moved_slink", lcpl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Pset_char_encoding(lcpl_id, H5T_CSET_UTF8) < 0)
    TEST_ERROR;
  if (H5Lcopy(file_id, "moved_slink", file_id, "copied_slink", lcpl_id,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* H5Lcreate_external */
  if (H5Lcreate_external("filename", "path", file_id, "extlink", lcpl_id,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_move(hid_t fapl, hbool_t new_format) {
  ...
  if ((grp_1 = H5Gcreate2(file_a, "group1", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((grp_2 = H5Gcreate2(file_a, "group2", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((grp_move = H5Gcreate2(grp_1, "group_move", H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create hard, soft and external links. */
  if (H5Lcreate_hard(grp_1, "group_move", H5L_SAME_LOC, "hard", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_soft("/group1/group_move", grp_2, "soft", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external("filename", "pathname", grp_2, "ext", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    if (H5Lmove(H5L_SAME_LOC, "group_move", H5L_SAME_LOC, "group_new_name",
                H5P_DEFAULT, H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  if (H5Lmove(grp_2, "soft", file_b, "soft_new_name", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lmove(grp_2, "ext", file_b, "ext_new_name", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  ...
  if (H5Lmove(grp_1, "group_move", grp_2, "group_new_name", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lmove(grp_2, "group_new_name", H5L_SAME_LOC, "group_newer_name",
  ...
  if (H5Lmove(grp_2, "group_newer_name", grp_1, "group_newer_name", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lmove(file_a, "/group1/group_newer_name", grp_2,
              "/group2/group_newest_name", H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_copy(hid_t fapl, hbool_t new_format) {
  hid_t file_a = -1, file_b = -1;
  hid_t grp_1 = -1, grp_2 = -1, grp_move = -1, moved_grp = -1;
  char filename[1024];
  ...
  if ((grp_2 = H5Gcreate2(file_a, "group2", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if ((grp_move = H5Gcreate2(grp_1, "group_copy", H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create hard, soft and external links. */
  if (H5Lcreate_hard(grp_1, "group_copy", H5L_SAME_LOC, "hard", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_soft("/group1/group_copy", grp_2, "soft", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external("filename", "pathname", grp_2, "ext", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
   ...
   * H5L_SAME_LOC.  Should fail. */
  H5E_BEGIN_TRY {
    if (H5Lcopy(H5L_SAME_LOC, "group_copy", H5L_SAME_LOC, "group_new_name",
                H5P_DEFAULT, H5P_DEFAULT) != FAIL)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  if (H5Lcopy(grp_2, "soft", file_b, "soft_new_name", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lexists(file_b, "soft_new_name", H5P_DEFAULT) != TRUE)
    TEST_ERROR;
  ...
  if (H5Lcopy(grp_2, "ext", file_b, "ext_new_name", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Lexists(file_b, "ext_new_name", H5P_DEFAULT) != TRUE)
    TEST_ERROR;
  ...
  /* Move a group across groups in the same file while renaming it. */
  if (H5Lcopy(grp_1, "group_copy", grp_2, "group_new_name", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
   ...
   */
  if (H5Lcopy(grp_2, "group_new_name", H5L_SAME_LOC, "group_newer_name",
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lcopy(grp_2, "group_newer_name", grp_1, "group_newer_name", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lcopy(file_a, "/group1/group_newer_name", grp_2,
              "/group2/group_newest_name", H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int test_move_preserves(hid_t fapl_id, hbool_t new_format) {
  hid_t file_id = -1;
  hid_t group_id = -1;
  ...
  if ((group_id =
           H5Gcreate2(file_id, "group", lcpl_id, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(group_id) < 0)
    TEST_ERROR;
  ...
  if (H5Lget_info2(file_id, "group", &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file_id, "group", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  old_cset = linfo.cset;
  ...
  /* If this test happens too quickly, the times will all be the same.  Make
  ...
  /* Get the link's character set & modification time .  They should be
   * unchanged */
  if (H5Lget_info2(file_id, "group", &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file_id, "group", &oinfo, H5O_INFO_TIME,
  ...
  /* Create a new link to the group.  It should have a different creation order
   * value but the same modification time */
  if (H5Lcreate_hard(file_id, "group", file_id, "group2", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file_id, "group2", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
   ...
   * should not change.
   */
  if (H5Lcopy(file_id, "group", file_id, "group_copied", lcpl_id, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file_id, "group_copied", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
  ...
  if (H5Lmove(file_id, "group_copied", file_id, "group_copied2", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file_id, "group_copied2", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
   ...
   *  change. */
  if (H5Lmove(file_id, "group", file_id, "group_moved", lcpl_id, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file_id, "group_moved", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
  ...
  if (H5Lmove(file_id, "group_moved", file_id, "group_moved_again", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(file_id, "group_moved_again", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
 ...
 */
#ifndef H5_NO_DEPRECATED_SYMBOLS
static int test_deprec(hid_t fapl, hbool_t new_format) {
  hid_t file_id = -1;
  hid_t group1_id = -1;
  hid_t group2_id = -1;
  ...
  if ((group1_id = H5Gcreate2(file_id, "group1", H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((group2_id = H5Gcreate2(file_id, "group2", H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Glink(file_id, H5G_LINK_HARD, "group2", "group1/link_to_group2") < 0)
    FAIL_STACK_ERROR;
  if (H5Glink2(file_id, "group1", H5G_LINK_HARD, group2_id, "link_to_group1") <
      0)
    FAIL_STACK_ERROR;
  if (H5Glink2(file_id, "link_to_group1", H5G_LINK_SOFT, H5G_SAME_LOC,
               "group2/soft_link_to_group1") < 0)
    FAIL_STACK_ERROR;
  if (H5Glink2(file_id, "dangle", H5G_LINK_SOFT, H5G_SAME_LOC,
               "group2/dangle_soft_link") < 0)
    FAIL_STACK_ERROR;
  ...
  if (HDstrcmp(tmpstr, "link_to_group2") != 0)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    if (H5Gget_objname_by_idx(group1_id, (hsize_t)1, tmpstr, sizeof(tmpstr)) >=
        0)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  /* Test the soft link */
  if (H5Gget_objinfo(file_id, "/group2/soft_link_to_group1", FALSE, &sb_soft1) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Gget_linkval(group2_id, "soft_link_to_group1", sb_soft1.linklen,
                     tmpstr) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Gget_linkval(group2_id, "dangle_soft_link", sb_soft2.linklen, tmpstr) <
      0)
    FAIL_STACK_ERROR;
  ...
  if ((group1_id = H5Gopen2(file_id, "moved_group1", H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((group2_id =
           H5Gopen2(file_id, "moved_group1/moved_group2", H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int cklinks_deprec(hid_t fapl, hbool_t new_format) {
  hid_t file;
  ...
  /* Hard link */
  if (H5Oget_info_by_name2(file, "d1", &oinfo1, H5O_INFO_BASIC, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
  if (H5Oget_info_by_name2(file, "grp1/hard", &oinfo2, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5O_TYPE_DATASET != oinfo2.type) {
    H5_FAILED();
  ...
  if (H5Lexists(file, "/grp1/hard", H5P_DEFAULT) != TRUE)
    FAIL_STACK_ERROR;
  H5E_BEGIN_TRY { exists = H5Lexists(file, "no_grp1/hard", H5P_DEFAULT); }
  H5E_END_TRY
  if (exists >= 0) {
    H5_FAILED();
    HDputs("    H5Lexists() should have failed for a path with missing "
  ...
  /* Symbolic link */
  if (H5Oget_info_by_name2(file, "grp1/soft", &oinfo2, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5O_TYPE_DATASET != oinfo2.type) {
    H5_FAILED();
  ...
  /* Dangling link */
  H5E_BEGIN_TRY {
    status = H5Oget_info_by_name2(file, "grp1/dangle", &oinfo2, H5O_INFO_BASIC,
                                  H5P_DEFAULT);
  }
  H5E_END_TRY
  if (status >= 0) {
  ...
  /* Recursive link */
  H5E_BEGIN_TRY {
    status = H5Oget_info_by_name2(file, "grp1/recursive", &oinfo2,
                                  H5O_INFO_BASIC, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (status >= 0) {
 ...
 *-------------------------------------------------------------------------
 */
static int test_lcpl_deprec(hid_t fapl, hbool_t new_format) {
  hid_t file_id = -1;
  hid_t group_id = -1;
  hid_t space_id = -1;
  hid_t dset_id = -1;
  ...
  if (new_format)
    TESTING("link creation property lists using deprecated routines (w/new "
            "group format)");
  else
 ...
 *-------------------------------------------------------------------------
 */
static int test_move_preserves_deprec(hid_t fapl_id, hbool_t new_format) {
  hid_t file_id = -1;
  hid_t group_id = -1;
  hid_t fcpl_id = -1; /* Group creation property list ID */
  hid_t lcpl_id = -1;
  ...
  if (new_format)
    TESTING("moving and copying links using deprecated routines preserves "
            "their properties (w/new group "
            "format)");
  else
    TESTING("moving and copying links using deprecated routines preserves "
            "their properties");
  ...
  if (H5Lget_info1(file_id, "group", &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(file_id, "group", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  old_cset = linfo.cset;
  if (old_cset != H5T_CSET_UTF8)
  ...
  if (H5Lcreate_hard(file_id, "group", file_id, "group2", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(file_id, "group2", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
  ...
  if (H5Lcopy(file_id, "group", file_id, "group_copied", lcpl_id, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(file_id, "group_copied", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
  ...
  if (H5Lmove(file_id, "group_copied", file_id, "group_copied2", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(file_id, "group_copied2", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
  ...
  if (H5Lmove(file_id, "group", file_id, "group_moved", lcpl_id, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(file_id, "group_moved", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
  ...
  if (H5Lmove(file_id, "group_moved", file_id, "group_moved_again", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(file_id, "group_moved_again", &oinfo, H5O_INFO_TIME,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (old_modification_time != oinfo.mtime)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_root_deprec(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  hid_t gid = -1, gid2 = -1;   /* Group IDs */
  ...
  if (new_format)
    TESTING(
        "external link to root using deprecated routines (w/new group format)");
  else
  ...
  if (H5Lcreate_external(filename1, "/", fid, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link to object in first file */
  /* (add a few extra '/'s to make certain library normalizes external link
   * object names) */
  if (H5Lcreate_external(filename1, "///", fid, "ext_link2", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((gid2 = H5Gcreate2(gid, "new_group", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
   ...
   * directly
   */
  if ((gid = H5Gcreate2(fid, "ext_link/newer_group", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    gid = H5Gcreate2(fid, "ext_link/readonly_group", H5P_DEFAULT, H5P_DEFAULT,
                     H5P_DEFAULT);
  }
  H5E_END_TRY
  if (gid >= 0)
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_query_deprec(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;          /* File ID */
  ...
  if (new_format)
    TESTING("query aspects of external link using deprecated routines (w/new "
            "group format)");
  else
  ...
  /* (add a few extra '/'s to make certain library normalizes external link
   * object names) */
  if (H5Lcreate_external(filename2, "///dst//", fid, "src", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Get information for external link.  It should be two strings right after
   * each other */
  if (H5Lget_val(fid, "src", query_buf, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Extract the file and object names from the buffer */
  if (H5Lunpack_elink_val(query_buf, li.u.val_size, NULL, &file_name,
                          &object_name) < 0)
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    if (H5Lunpack_elink_val(query_buf, li.u.val_size - 1, NULL, NULL, NULL) >=
        0)
      TEST_ERROR;
  }
  H5E_END_TRY
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_closing_deprec(hid_t fapl, hbool_t new_format) {
  ...
  if (new_format)
    TESTING(
        "that external files are closed during traversal (w/new group format)");
  else
  ...
  /* Create a dataspace and a datatype so we can create/commit a
   * dataset/datatype in the files */
  dims[0] = 2;
  dims[1] = 2;
  ...
  if (H5Lcreate_external(filename2, "/", fid1, "elink", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename3, "/", fid2, "elink", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename4, "/", fid3, "elink", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((gid = H5Gcreate2(fid1, "elink/elink/elink/group1", H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(fid1, "elink/elink/elink/type1", tid, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(fid1, "elink/elink/elink/dataset1", tid2, sid,
                        H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Lget_info1(fid1, "elink/elink/elink", &li, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(fid1, "elink/elink/elink/type1", &oi, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(fid1, "elink/elink/elink", &oi, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Test move */
  if (H5Lmove(fid1, "elink/elink/elink/group1", fid1,
              "elink/elink/elink/group1_moved", H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Open file 4 so we can do some fancy things */
  if ((fid4 = H5Fopen(filename4, H5F_ACC_RDWR, fapl)) < 0)
    FAIL_STACK_ERROR;
  if (H5Lmove(fid1, "elink/elink/elink/type1", fid4, "type1_moved", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Lmove(fid4, "dataset1", fid1, "elink/elink/elink/dataset1_moved",
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Lcopy(fid1, "elink/elink/elink", fid1, "elink/elink/elink_copied",
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Lcopy(fid1, "elink/elink/elink", fid1,
              "elink/elink/elink/elink_copied2", H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;

  /* Test H5Gset and get comment */
  if (H5Oset_comment_by_name(fid1, "elink/elink/elink/group1_moved", "comment",
                             H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Oget_comment_by_name(fid1, "elink/elink/elink/group1_moved", buf,
                             sizeof(buf), H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((tid = H5Topen2(fid1, "elink/elink/elink/type1_moved", H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if ((did = H5Dopen2(fid1, "elink/elink/elink/dataset1_moved", H5P_DEFAULT)) <
      0)
    FAIL_STACK_ERROR;
  /* Close objects */
  if (H5Gclose(gid) < 0)
  ...
  /* Test H5Oopen */
  if ((did = H5Oopen(fid1, "elink/elink/elink/dataset1_moved", H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Dclose(did) < 0)
    TEST_ERROR;
  ...
  /* Test H5Fmount */
  if ((gid = H5Gcreate2(fid1, "elink/elink/elink/mnt", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  ...
  /* Test H5Rcreate */
  if (H5Rcreate(&obj_ref, fid1, "elink/elink/elink/type1_moved", H5R_OBJECT,
                (hid_t)(-1)) < 0)
    TEST_ERROR;
   ...
   * Now test that we can't confuse HDF5 by giving unusual paths with external
   * links
   */
  /* Create an external link that points to another external link */
  if ((fid2 = H5Fopen(filename2, H5F_ACC_RDWR, fapl)) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename3, "/elink", fid2, "elink2", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Fclose(fid2) < 0)
    TEST_ERROR;
   ...
   */
  if ((gid = H5Gcreate2(fid1, "elink/elink2/group2", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
   ...
   */
  if ((gid = H5Gcreate2(fid1, "elink/elink2/group2/group3", H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  if ((gid = H5Gcreate2(fid1, "elink/elink2/group2/group3/group4", H5P_DEFAULT,
                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name2(fid1, "elink/elink2/group2/group3/group4", &oi,
                           H5O_INFO_BASIC, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Add a few regular groups and a soft link in file2 using intermediate group
   * creation */
  if ((lcpl_id = H5Pcreate(H5P_LINK_CREATE)) < 0)
    TEST_ERROR;
   ...
   * points to file 4.
   */
  if ((gid = H5Gcreate2(fid1, "elink/file2group1/file2group2/slink/group3",
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  if (H5Lget_info1(fid1, "elink/file2group1/file2group2/slink/group3", &li,
                   H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Some simpler tests */
  if ((gid = H5Gcreate2(fid1, "elink/file2group3", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
...
static herr_t UD_hard_create_deprec(const char H5_ATTR_UNUSED *link_name,
                                    hid_t loc_group, const void *udata,
                                    size_t udata_size,
...
static hid_t UD_hard_traverse_deprec(const char H5_ATTR_UNUSED *link_name,
                                     hid_t cur_group, const void *udata,
                                     size_t udata_size,
  ...
  ret_value = H5Oopen_by_addr(
      cur_group, addr); /* If this fails, our return value will be negative. */

  return ret_value;
} /* end UD_hard_traverse() */

/* UD_hard_delete decrements the object's reference count */
static herr_t UD_hard_delete_deprec(const char H5_ATTR_UNUSED *link_name,
                                    hid_t file, const void *udata,
                                    size_t udata_size) {
  haddr_t addr;
  hid_t target_obj = -1;
  herr_t ret_value = 0;
...
static int ud_hard_links_deprec(hid_t fapl) {
  ...
  TESTING(
      "user-defined hard link using deprecated routines (w/new group format)");
  ...
  if ((gid = H5Gcreate2(fid, "group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  if (H5Lcreate_ud(fid, "ud_link", (H5L_type_t)UD_HARD_TYPE, &(li.u.address),
                   (size_t)sizeof(haddr_t), H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int ud_link_reregister_deprec(hid_t fapl) {
  ...
  TESTING("registering a new class for existing UD links using deprecated "
          "routines (w/new group format)");
  ...
  /* Point a UD defined hard link to a group in the same way as the previous
   * test */
  if ((gid = H5Gcreate2(fid, "group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  ...
  if ((gid = H5Gcreate2(fid, REREG_TARGET_NAME, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
...
static int ud_callbacks_deprec(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;                       /* File ID */
  hid_t gid = -1;                       /* Group ID */
  ...
  if (new_format)
    TESTING("user-defined link callbacks using deprecated routines (w/new "
            "group format)");
  else
  ...
  if ((gid = H5Gcreate2(fid, UD_CB_TARGET, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create a user-defined link to the group.  These UD links behave like soft
   * links. */
  if (H5Lcreate_ud(fid, UD_CB_LINK_NAME, (H5L_type_t)UD_CB_TYPE, ud_target_name,
  ...
  if (H5Lget_val(fid, UD_CB_LINK_NAME, query_buf, (size_t)NAME_BUF_SIZE,
                 H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Move the link */
  if (H5Lmove(fid, UD_CB_LINK_NAME, H5L_SAME_LOC, NEW_UD_CB_LINK_NAME,
              H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Pset_char_encoding(lcpl, H5T_CSET_UTF8) < 0)
    FAIL_STACK_ERROR;
  if (H5Lcreate_ud(fid, UD_CB_LINK_NAME, (H5L_type_t)UD_CB_TYPE, ud_target_name,
                   (size_t)UD_CB_TARGET_LEN, lcpl, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Pclose(lcpl) < 0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    if (H5Lcreate_ud(fid, NEW_UD_CB_LINK_NAME, (H5L_type_t)UD_CB_TYPE,
                     ud_target_name, (size_t)UD_CB_TARGET_LEN, H5P_DEFAULT,
                     H5P_DEFAULT) >= 0)
      FAIL_STACK_ERROR;
    if (H5Lmove(fid, UD_CB_LINK_NAME, H5L_SAME_LOC, NEW_UD_CB_LINK_NAME,
                H5P_DEFAULT, H5P_DEFAULT) >= 0)
 ...
 *-------------------------------------------------------------------------
 */
static int lapl_nlinks_deprec(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;                        /* File ID */
  hid_t gid = -1, gid2 = -1;             /* Group IDs */
  ...
  if (new_format)
    TESTING("adjusting nlinks with LAPL using deprecated routines (w/new group "
            "format)");
  else
  ...
  if (H5Lcopy(fid, "soft17", fid, "soft17/newer_soft", H5P_DEFAULT, plist) < 0)
    TEST_ERROR;
  if (H5Lmove(fid, "soft17/newer_soft", fid, "soft17/newest_soft", H5P_DEFAULT,
              plist) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_hard(fid, "soft17", fid, "soft17/link2_to_group", H5P_DEFAULT,
                     plist) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external("filename", "path", fid, "soft17/extlink", H5P_DEFAULT,
                         plist) < 0)
    TEST_ERROR;
  if (H5Lregister(UD_rereg_class) < 0)
    TEST_ERROR;
  if (H5Lcreate_ud(fid, "soft17/udlink", (H5L_type_t)UD_HARD_TYPE, NULL,
                   (size_t)0, H5P_DEFAULT, plist) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(gid, "datatype", tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if (H5Tclose(tid) < 0)
    TEST_ERROR;
  ...
  if ((sid = H5Screate_simple(2, dims, NULL)) < 0)
    TEST_ERROR;
  if ((did = H5Dcreate2(gid, "dataset", H5T_NATIVE_INT, sid, H5P_DEFAULT,
 ...
 *-------------------------------------------------------------------------
 */
static int linkinfo_deprec(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;           /* File ID */
  hid_t gid = -1;           /* Group ID */
  hid_t tid = -1;           /* Type ID */
  ...
  if (new_format)
    TESTING("link type field in H5Lget_info using deprecated routines (w/new "
            "group format)");
  else
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(fid, "datatype", tid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  if ((gid = H5Gcreate2(fid, "group", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
  ...
  if (H5Lcreate_ud(fid, "ud_link", (H5L_type_t)UD_PLIST_TYPE, NULL, (size_t)0,
                   H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external("file_name", "obj_path", fid, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int corder_create_compact_deprec(hid_t fapl) {
  ...
  TESTING("creating compact group with creation order indexing using "
          "deprecated routines");
  ...
  if (H5Pset_link_creation_order(
          gcpl_id, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED)) < 0)
    TEST_ERROR;

  /* Create group with creation order indexing & tracking on */
  if ((group_id = H5Gcreate2(file_id, CORDER_GROUP_NAME, H5P_DEFAULT, gcpl_id,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (u = 0; u < max_compact; u++) {
    ...
    if ((group_id2 = H5Gcreate2(group_id, objname, H5P_DEFAULT, H5P_DEFAULT,
                                H5P_DEFAULT)) < 0)
      TEST_ERROR;
...
static int corder_create_dense_deprec(hid_t fapl) {
  ...
  TESTING("creating dense group with creation order indexing using deprecated "
          "routines");
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx_check_deprec(hid_t group_id, const char *linkname,
                                         hsize_t n, hbool_t hard_link,
                                         hbool_t use_index) {
  ...
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                          (hsize_t)0, &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, n,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (!hard_link) {
    ...
    if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, n,
                          tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
      TEST_ERROR;
  ...
  if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, n,
                         tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
    TEST_ERROR;
   ...
   */
  if (use_index) {
    /* Verify the link information for first link, in native creation order
     * (which is increasing) */
    memset(&linfo, 0, sizeof(linfo));
    if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE,
                            (hsize_t)0, &linfo, H5P_DEFAULT) < 0)
      TEST_ERROR;
    if (linfo.corder != 0)
      TEST_ERROR;

    /* Verify the link information for new link, in native creation order (which
     * is increasing) */
    memset(&linfo, 0, sizeof(linfo));
    if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE,
                            n, &linfo, H5P_DEFAULT) < 0)
      TEST_ERROR;
    if (linfo.corder != (int64_t)n)
      TEST_ERROR;

    /* Verify value for new soft link, in native creation order (which is
     * increasing) */
    if (!hard_link) {
      memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
      if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE,
                            n, tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (HDstrcmp(valname, tmpval) != 0)
        TEST_ERROR;
    } /* end if */

    /* Verify the name for new link, in native creation order (which is
     * increasing) */
    memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
    if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE, n,
                           tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
      TEST_ERROR;
    if (HDstrcmp(linkname, tmpname) != 0)
      TEST_ERROR;
  ...
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC, n,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                          (hsize_t)0, &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != (int64_t)n)
    TEST_ERROR;
  ...
  if (!hard_link) {
    memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
    if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                          (hsize_t)0, tmpval, (size_t)NAME_BUF_SIZE,
                          H5P_DEFAULT) < 0)
      TEST_ERROR;
    if (HDstrcmp(valname, tmpval) != 0)
      TEST_ERROR;
  ...
  memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
  if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                         (hsize_t)0, tmpname, (size_t)NAME_BUF_SIZE,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (HDstrcmp(linkname, tmpname) != 0)
    TEST_ERROR;
   ...
   */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)0,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, n, &linfo,
                          H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != (int64_t)n)
    TEST_ERROR;
  ...
  if (!hard_link) {
    memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
    if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, n, tmpval,
  ...
  memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
  if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, n, tmpname,
                         (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (HDstrcmp(linkname, tmpname) != 0)
    TEST_ERROR;
   ...
   */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, n, &linfo,
                          H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)0,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != (int64_t)n)
    TEST_ERROR;
  ...
  if (!hard_link) {
    memset(tmpval, 0, (size_t)NAME_BUF_SIZE);
    if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)0,
                          tmpval, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
      TEST_ERROR;
    if (HDstrcmp(valname, tmpval) != 0)
      TEST_ERROR;
  ...
  memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
  if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)0,
                         tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (HDstrcmp(linkname, tmpname) != 0)
    TEST_ERROR;
...
static int link_info_by_idx_deprec(hid_t fapl) {
  ...
  for (hard_link = FALSE; hard_link <= TRUE; hard_link++) {
    ...
    for (use_index = FALSE; use_index <= TRUE; use_index++) {
      if (hard_link) {
        if (use_index)
          TESTING("querying info by index w/creation order index, using hard "
                  "links and deprecated "
                  "routines");
        else
          TESTING("querying info by index w/o creation order index, using hard "
                  "links and "
                  "deprecated routines");
      } /* end if */
      else {
        if (use_index)
          TESTING("querying info by index w/creation order index, using soft "
                  "links and deprecated "
                  "routines");
        else
          TESTING("querying info by index w/o creation order index, using soft "
                  "links and "
                  "deprecated routines");
      } /* end else */
      ...
      /* Set creation order tracking & indexing on group */
      if (H5Pset_link_creation_order(
              gcpl_id, (H5P_CRT_ORDER_TRACKED |
                        (use_index ? H5P_CRT_ORDER_INDEXED : (unsigned)0))) < 0)
        TEST_ERROR;

      /* Create group with creation order indexing & tracking on */
      if ((group_id = H5Gcreate2(file_id, CORDER_GROUP_NAME, H5P_DEFAULT,
      ...
      H5E_BEGIN_TRY {
        ret = H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER,
                                  H5_ITER_INC, (hsize_t)0, &linfo, H5P_DEFAULT);
      }
      H5E_END_TRY
      if (ret >= 0)
        TEST_ERROR;
      H5E_BEGIN_TRY {
        name_len = H5Lget_name_by_idx(group_id, ".", H5_INDEX_CRT_ORDER,
                                      H5_ITER_INC, (hsize_t)0, tmpname,
      ...
      for (u = 0; u < max_compact; u++) {
        ...
        if (hard_link) {
          ...
          if ((group_id2 = H5Gcreate2(group_id, objname, H5P_DEFAULT,
                                      H5P_DEFAULT, H5P_DEFAULT)) < 0)
            TEST_ERROR;
          if (H5Gclose(group_id2) < 0)
            TEST_ERROR;
          ...
          /* Create soft link */
          if (H5Lcreate_soft(valname, group_id, objname, H5P_DEFAULT,
                             H5P_DEFAULT) < 0)
            TEST_ERROR;
        } /* end else */

        /* Verify link information for new link */
        if (link_info_by_idx_check_deprec(group_id, objname, (hsize_t)u,
                                          hard_link, use_index) < 0)
          TEST_ERROR;
      } /* end for */
      ...
      H5E_BEGIN_TRY {
        ret = H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER,
                                  H5_ITER_INC, (hsize_t)u, &linfo, H5P_DEFAULT);
      }
      H5E_END_TRY
      if (ret >= 0)
        TEST_ERROR;
      H5E_BEGIN_TRY {
        ret = H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER,
                                  H5_ITER_DEC, (hsize_t)u, &linfo, H5P_DEFAULT);
      }
      H5E_END_TRY
      if (ret >= 0)
        TEST_ERROR;
...
static int link_info_by_idx_old_deprec(hid_t fapl) {
  ...
  for (hard_link = FALSE; hard_link <= TRUE; hard_link++) {
    if (hard_link)
      TESTING("querying info by index in old-style group, using hard links and "
              "deprecated routines");
    else
      TESTING("querying info by index in old-style group, using soft links and "
              "deprecated routines");
    ...
    for (u = 0; u < CORDER_NLINKS; u++) {
      ...
      if (hard_link) {
        if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                                (hsize_t)u, &linfo, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5_addr_ne(linfo.u.address, objno[u]))
          TEST_ERROR;
      } /* end if */
      else {
        if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                              (hsize_t)u, tmpval, (size_t)NAME_BUF_SIZE,
                              H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (HDstrcmp(valname, tmpval) != 0)
          TEST_ERROR;
      ...
      if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                             (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE,
                             H5P_DEFAULT) < 0)
        TEST_ERROR;
      ...
      if (hard_link) {
        if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE,
                                (hsize_t)u, &linfo, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5_addr_ne(linfo.u.address, objno[u]))
          TEST_ERROR;
        ...
        if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE,
                              (hsize_t)u, tmpval, (size_t)NAME_BUF_SIZE,
                              H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (HDstrcmp(valname, tmpval) != 0)
      ...
      if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE,
                             (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE,
                             H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (HDstrcmp(objname, tmpname) != 0)
      ...
      if (hard_link) {
        if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC,
                                (hsize_t)u, &linfo, H5P_DEFAULT) < 0)
          TEST_ERROR;
        ...
        if (H5Lget_val_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC,
                              (hsize_t)u, tmpval, (size_t)NAME_BUF_SIZE,
                              H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (HDstrcmp(valname, tmpval) != 0)
      ...
      if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC,
                             (hsize_t)u, tmpname, (size_t)NAME_BUF_SIZE,
                             H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (HDstrcmp(objname, tmpname) != 0)
    ...
    H5E_BEGIN_TRY {
      ret = H5Lget_info_by_idx1(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                                (hsize_t)u, &linfo, H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
      TEST_ERROR;
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("deleting links by creation order index in increasing "
                      "order w/creation "
            ...
            if (use_index)
              TESTING("deleting links by creation order index in decreasing "
                      "order w/creation "
                      "order index using deprecated routines");
            else
              TESTING("deleting links by creation order index in decreasing "
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("deleting links by name index in increasing order "
                      "w/creation order index "
                      "using deprecated routines");
            else
              TESTING("deleting links by name index in increasing order w/o "
                      "creation order "
                      "index using deprecated routines");
          } /* end if */
          else {
            if (use_index)
              TESTING("deleting links by name index in decreasing order "
                      "w/creation order index "
                      "using deprecated routines");
            else
              TESTING("deleting links by name index in decreasing order w/o "
                      "creation order "
                      "index using deprecated routines");
          } /* end else */
        }   /* end else */
        ...
        /* Create file */
        h5_fixname(FILENAME[0], fapl, filename, sizeof filename);
        if ((file_id = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) <
            0)
          TEST_ERROR;
        ...
        H5E_BEGIN_TRY {
          ret = H5Ldelete_by_idx(group_id, ".", idx_type, order, (hsize_t)0,
                                 H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
        ...
        H5E_BEGIN_TRY {
          ret = H5Ldelete_by_idx(group_id, "None", idx_type, order, (hsize_t)0,
                                 H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
        ...
        for (u = 0; u < max_compact; u++) {
          ...
          /* Verify link information for new link */
          if (link_info_by_idx_check_deprec(group_id, objname, (hsize_t)u, TRUE,
                                            use_index) < 0)
            TEST_ERROR;
        } /* end for */
        ...
        H5E_BEGIN_TRY {
          ret = H5Ldelete_by_idx(group_id, ".", idx_type, order, (hsize_t)u,
                                 H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
        ...
        for (u = 0; u < (max_compact - 1); u++) {
          ...
          if (H5Ldelete_by_idx(group_id, ".", idx_type, order, (hsize_t)0,
                               H5P_DEFAULT) < 0)
            TEST_ERROR;

          /* Verify the link information for first link in appropriate order */
          memset(&linfo, 0, sizeof(linfo));
          if (H5Lget_info_by_idx1(group_id, ".", idx_type, order, (hsize_t)0,
                                  &linfo, H5P_DEFAULT) < 0)
            TEST_ERROR;
          if (order == H5_ITER_INC) {
            if (linfo.corder != (u + 1))
          ...
          memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
          if (H5Lget_name_by_idx(group_id, ".", idx_type, order, (hsize_t)0,
                                 tmpname, (size_t)NAME_BUF_SIZE,
                                 H5P_DEFAULT) < 0)
            TEST_ERROR;
          if (order == H5_ITER_INC)
        ...
        for (u = 0; u < max_compact; u++) {
          /* Delete link */
          if (H5Ldelete_by_idx(group_id, ".", idx_type, order, (hsize_t)u,
                               H5P_DEFAULT) < 0)
            TEST_ERROR;
           ...
           */
          memset(&linfo, 0, sizeof(linfo));
          if (H5Lget_info_by_idx1(group_id, ".", idx_type, order, (hsize_t)u,
                                  &linfo, H5P_DEFAULT) < 0)
            TEST_ERROR;
          if (order == H5_ITER_INC) {
          ...
          memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
          if (H5Lget_name_by_idx(group_id, ".", idx_type, order, (hsize_t)u,
                                 tmpname, (size_t)NAME_BUF_SIZE,
                                 H5P_DEFAULT) < 0)
            TEST_ERROR;
          if (order == H5_ITER_INC)
...
static int delete_by_idx_old_deprec(hid_t fapl) {
  ...
  for (order = H5_ITER_INC; order <= H5_ITER_DEC; order++) {
    ...
    H5E_BEGIN_TRY {
      ret = H5Ldelete_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
                             H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    H5E_BEGIN_TRY {
      ret = H5Ldelete_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, order,
                             (hsize_t)0, H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
      TEST_ERROR;
    ...
    H5E_BEGIN_TRY {
      ret = H5Ldelete_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                             H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    for (u = 0; u < (CORDER_NLINKS - 1); u++) {
      ...
      /* Delete first link in appropriate order */
      if (H5Ldelete_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
                           H5P_DEFAULT) < 0)
        TEST_ERROR;

      /* Verify the link information for first link in appropriate order */
      memset(&linfo, 0, sizeof(linfo));
      if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
                              &linfo, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (order == H5_ITER_INC) {
      ...
      /* Verify the name for first link in appropriate order */
      memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
      if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
                             tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (order == H5_ITER_INC)
        HDsnprintf(objname, sizeof(objname), "filler %02u", (u + 1));
    ...
    for (u = 0; u < (CORDER_NLINKS / 2); u++) {
      unsigned dec_u =
          CORDER_NLINKS - ((u * 2) + 2); /* Decreasing mapped index */

      /* Delete link */
      if (H5Ldelete_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                           H5P_DEFAULT) < 0)
        TEST_ERROR;

      /* Verify the link information for current link in appropriate order */
      memset(&linfo, 0, sizeof(linfo));
      if (H5Lget_info_by_idx1(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                              &linfo, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (order == H5_ITER_INC) {
      ...
      /* Verify the name for current link in appropriate order */
      memset(tmpname, 0, (size_t)NAME_BUF_SIZE);
      if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                             tmpname, (size_t)NAME_BUF_SIZE, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (order == H5_ITER_INC)
        HDsnprintf(objname, sizeof(objname), "filler %02u", ((u * 2) + 1));
    ...
    for (u = 0; u < ((CORDER_NLINKS / 2) - 1); u++) {
      unsigned dec_u =
          CORDER_NLINKS - ((u * 2) + 4); /* Decreasing mapped index */
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_deprec_cb(hid_t group_id, const char *link_name,
                                  const H5L_info1_t *info, void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_deprec_cb(hid_t group_id, const char *link_name,
                                   void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_check_deprec(hid_t group_id, H5_index_t idx_type,
                                     H5_iter_order_t order, unsigned max_links,
  ...
  if (H5Literate1(group_id, idx_type, order, &skip, link_iterate_deprec_cb,
                  iter_info) < 0)
    TEST_ERROR;
  ...
  iter_info->ncalled = 0;
  iter_info->curr =
      (int64_t)(order != H5_ITER_DEC ? skip : ((max_links - 1) - skip));
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if (H5Literate1(group_id, idx_type, order, &skip, link_iterate_deprec_cb,
                  iter_info) < 0)
    TEST_ERROR;
  ...
  iter_info->stop = -1;
  iter_info->ncalled = 0;
  iter_info->curr = order != H5_ITER_DEC ? (unsigned)gskip
                                         : ((max_links - 1) - (unsigned)gskip);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if (H5Giterate(group_id, ".", &gskip, group_iterate_deprec_cb, iter_info) < 0)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Literate1(group_id, idx_type, order, &skip,
                         link_iterate_deprec_cb, iter_info)) < 0)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Giterate(group_id, ".", &gskip, group_iterate_deprec_cb,
                        iter_info)) < 0)
    TEST_ERROR;
  if (ret != CORDER_ITER_STOP)
    TEST_ERROR;
  ...
  skip = 0;
  H5E_BEGIN_TRY {
    ret = H5Literate1(group_id, idx_type, order, &skip,
                      link_iterate_fail_deprec_cb, NULL);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
 ...
 *
 * Purpose:     Create a group with creation order indices and test iterating
 *over links by index.
 *
 * Return:      Success:        0
 *              Failure:        -1
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("iterating over links by creation order index in "
                      "increasing order "
                      "w/creation order index using deprecated routines");
            else
              TESTING("iterating over links by creation order index in "
                      "increasing order w/o "
            ...
            if (use_index)
              ...
              TESTING("iterating over links by creation order index in "
                      "decreasing order w/o "
                      "creation order index using deprecated routines");
          } /* end else */
          else {
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("iterating over links by creation order index in native "
                      "order w/creation "
                      "order index using deprecated routines");
            else
              TESTING("iterating over links by creation order index in native "
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("iterating over links by name index in increasing order "
                      "w/creation order "
                      "index using deprecated routines");
            else
              TESTING("iterating over links by name index in increasing order "
            ...
            if (use_index)
              TESTING("iterating over links by name index in decreasing order "
                      "w/creation order "
                      "index using deprecated routines");
            else
              TESTING("iterating over links by name index in decreasing order "
            ...
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("iterating over links by name index in native order "
                      "w/creation order "
                      "index using deprecated routines");
            else
              TESTING("iterating over links by name index in native order w/o "
                      "creation order "
                      "index using deprecated routines");
          } /* end else */
        }   /* end else */
        ...
        /* (should be OK) */
        if (H5Literate1(group_id, idx_type, order, NULL, link_iterate_deprec_cb,
                        NULL) < 0)
          TEST_ERROR;
        ...
        if (link_iterate_check_deprec(group_id, idx_type, order, u,
                                      &iter_info) < 0)
          TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_deprec_cb(hid_t group_id, const char *link_name,
                                      const H5L_info1_t *info, void *_op_data) {
  link_iter_info_t *op_data = (link_iter_info_t *)_op_data; /* User data */
  char objname[NAME_BUF_SIZE];                              /* Object name */
  H5L_info1_t my_info; /* Local link info */
  ...
  /* Verify name of link */
  HDsnprintf(objname, sizeof(objname), "filler %02u",
             (info ? (unsigned)op_data->curr
                   : (unsigned)((op_data->ncalled - 1) + op_data->nskipped)));
  if (HDstrcmp(link_name, objname) != 0)
    return H5_ITER_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_old_deprec_cb(hid_t group_id, const char *link_name,
                                       void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_check_deprec(hid_t group_id, H5_iter_order_t order,
                                         unsigned max_links,
                                         link_iter_info_t *iter_info) {
  unsigned v;   /* Local index variable */
  hsize_t skip; /* # of links to skip in group */
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if (H5Literate1(group_id, H5_INDEX_NAME, order, &skip,
                  link_iterate_old_deprec_cb, iter_info) < 0)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if (H5Giterate(group_id, ".", &gskip, group_iterate_old_deprec_cb,
                 iter_info) < 0)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Literate1(group_id, H5_INDEX_NAME, order, &skip,
                         link_iterate_old_deprec_cb, iter_info)) < 0)
    TEST_ERROR;
  if (ret != CORDER_ITER_STOP)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Giterate(group_id, ".", &gskip, group_iterate_old_deprec_cb,
                        iter_info)) < 0)
    TEST_ERROR;
  if (ret != CORDER_ITER_STOP)
    TEST_ERROR;
  ...
  skip = 0;
  H5E_BEGIN_TRY {
    ret = H5Literate1(group_id, H5_INDEX_NAME, order, &skip,
                      link_iterate_fail_deprec_cb, NULL);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  skip = 0;
  H5E_BEGIN_TRY {
    ret = H5Literate1((hid_t)(-1), H5_INDEX_NAME, order, &skip,
                      link_iterate_fail_deprec_cb, NULL);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  H5E_BEGIN_TRY {
    ret = H5Giterate((hid_t)(-1), ".", &gskip, group_iterate_old_deprec_cb,
                     iter_info);
  }
  H5E_END_TRY
  if (ret >= 0)
 ...
 *
 * Purpose:     Create a "old-style" group and test iterating over links by
 *index.
 *
 * Return:      Success:        0
 *              Failure:        -1
...
static int link_iterate_old_deprec(hid_t fapl) {
  ...
  for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
    ...
    /* Check for iteration on empty group */
    /* (should be OK) */
    if (H5Literate1(group_id, H5_INDEX_NAME, order, NULL,
                    link_iterate_old_deprec_cb, NULL) < 0)
      TEST_ERROR;
    ...
    H5E_BEGIN_TRY {
      ret = H5Literate1(group_id, H5_INDEX_NAME, order, &skip,
                        link_iterate_old_deprec_cb, NULL);
    }
    H5E_END_TRY
    if (ret >= 0)
      TEST_ERROR;
    ...
    skip = (hsize_t)0;
    H5E_BEGIN_TRY {
      ret = H5Literate1(group_id, H5_INDEX_CRT_ORDER, order, &skip,
                        link_iterate_old_deprec_cb, NULL);
    }
    H5E_END_TRY
    if (ret >= 0)
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_root(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  hid_t gid = -1, gid2 = -1;   /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_path(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  hid_t gid = -1, gid2 = -1;   /* Group IDs */
  char objname[NAME_BUF_SIZE]; /* Object name */
  char filename1[NAME_BUF_SIZE];
  ...
  if ((gid = H5Gcreate2(fid, "A/B/C", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  ...
  /* Create external link to object in first file */
  if (H5Lcreate_external(filename1, "/A/B/C", fid, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_mult(hid_t fapl, hbool_t new_format) {
  ...
  /* Create external link to object in first file */
  if (H5Lcreate_external(filename1, "/A/B/C", gid, "F", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(filename2, "/D/E/F", gid, "I", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link to object in first file */
  if (H5Lcreate_external(filename3, "/G/H/I", fid, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* The intermediate files should not stay open. Replace one of them with a new
   * file. */
  if ((fid2 = H5Fcreate(filename2, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
    TEST_ERROR;
  if (H5Fclose(fid2) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_self(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  hid_t gid = -1, gid2 = -1;   /* Group IDs */
  ...
  if (H5Lcreate_external(filename1, "/X", fid, "A/B/C", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(filename1, "/A", fid, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(filename3, "/", fid, "/X/Y/Z", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Re-open file2 and traverse through file1 (with its recursive extlink) to
   * file3 */
  if ((fid = H5Fopen(filename2, H5F_ACC_RDWR, fapl)) < 0)
    TEST_ERROR;
  ...
  /* Create object through external link */
  if ((gid2 = H5Gcreate2(gid, "newer_group", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_pingpong(hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1);              /* File ID */
  hid_t gid = (-1), gid2 = (-1); /* Group IDs */
  char objname[NAME_BUF_SIZE];   /* Object name */
  char filename1[NAME_BUF_SIZE],
  ...
  if (H5Lcreate_external(filename2, "/link2", fid, "link1", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/link4", fid, "link3", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/link6", fid, "link5", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external links for chain */
  if (H5Lcreate_external(filename1, "/link3", fid, "link2", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/link5", fid, "link4", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/final", fid, "link6", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_toomany(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  hid_t gid = -1, gid2 = -1;   /* Group IDs */
  char objname[NAME_BUF_SIZE]; /* Object name */
  char filename1[NAME_BUF_SIZE],
  ...
  if (H5Lcreate_external(filename2, "/link6", fid, "link5", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/link8", fid, "link7", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/link10", fid, "link9", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/link12", fid, "link11", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/link14", fid, "link13", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/link16", fid, "link15", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/final", fid, "link17", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(filename1, "/link5", fid, "link4", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/link7", fid, "link6", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/link9", fid, "link8", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/link11", fid, "link10", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/link13", fid, "link12", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/link15", fid, "link14", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/link17", fid, "link16", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (gid >= 0) {
    H5_FAILED();
    printf("%d:    Should have failed for sequence of too many nested links.",
           __LINE__);
    goto error;
  }
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_dangling(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;    /* File ID */
  hid_t gid = -1;    /* Group IDs */
  ...
  if (H5Lcreate_external("missing", "/missing", fid, "no_file", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/missing", fid, "no_object", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  H5E_BEGIN_TRY {
    status = H5Lget_name_by_idx(rid, "no_file", H5_INDEX_NAME, H5_ITER_INC, 0,
                                NULL, 0, H5P_DEFAULT);
  }
  H5E_END_TRY
  if (status >= 0) {
    H5_FAILED();
    HDputs("    Retrieving name of object by index through dangling file "
           "external link should have failed.");
  } /* end if */
 ...
 *              4. Set up external link prefix via H5Pset_elink_prefix() to be
 *"tmp_links" Should be able to access the target file in tmp_links directory
 *via the prefix set by H5Pset_elink_prefix()
 *
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_prefix(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  hid_t gapl_id = -1;
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
  ...
  if (H5Lcreate_external(filename2, "/A", fid, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* should be able to find the target file from pathnames set via
   * H5Pset_elink_prefix() */
  if (gid < 0) {
    H5_FAILED();
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_abs_mainpath(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];

  if (new_format)
    TESTING(
        "external links via main file's absolute path (w/new group format)");
  else
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_rel_mainpath(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE];
  ...
  /* should be able to find the target file from the main file's relative
   * pathname */
  if (gid < 0) {
    H5_FAILED();
 ...
 *              2. main file: Linux:"/CWD/tmp_links/extlinks0"; Window: "<cur
 *drive>:/CWD/tmp_links/extlinks0"
 *              3. target file: "extlinks5"
 *                 Should be able to access the target file in the current
 *working directory
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_cwd(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_abstar(hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1); /* File ID */
  hid_t gid = (-1); /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
 ...
 *
 * Purpose:     1. target link: Linux: "/CWD/tmp_links/extlinks7"; Windows:
 *"<cur drive>:/CWD/tmp_links/extlinks7"
 *              2. main file: "extlinks0"
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_abstar_cur(hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1); /* File ID */
  hid_t gid = (-1); /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
  ...
  /* should be able to find the target file from main file's current working
   * directory */
  if (gid < 0) {
    H5_FAILED();
    HDputs("    Should have found the file in current working directory.");
 ...
 *                 Should be able to access the target file via the main file's
 *CWD+ target's relative path
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_reltar(hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1); /* File ID */
  hid_t gid = (-1); /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE];
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_chdir(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE];
   ...
   * main file's current working directory + pathname of external linked
   * targetfile
   */
  if (gid < 0) {
    H5_FAILED();
 ...
 *
 * Purpose:     To verify that the external linked target file with physical
 *layout different from the parent can be successfully opened.
 *
 *        1. target link: "extlinks16"
 *        2. target file: "extlinks16"
 *        3. main file: Linux:"/CWD/tmp_links/extlinks0"; Window: "<cur
 *drive>:/CWD/tmp_links/extlinks0"
 *        4. Create target file A to be a "family" file: extlinks16A
 *        4. Create target file B to be a "multi" file: extlinks16B
 *        5. Create external link from main file to target file A:
 *ext_linkA->extlinks16A:/A
 *        5. Create external link from main file to target file B:
 *ext_linkB->extlinks16B:/B
 ...
 *-------------------------------------------------------------------------
 */
static int external_set_elink_fapl1(hid_t fapl, hbool_t new_format) {
  hid_t fid = (-1);
  hid_t fidA = (-1), fidB = (-1);
  hid_t gidA = (-1), gidB = (-1);
  hid_t oidA = (-1), oidB = (-1);
  char filename1[NAME_BUF_SIZE], filename2A[NAME_BUF_SIZE],
      filename2B[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
  ...
  if (H5Pset_fapl_multi(multi_fapl, memb_map, memb_fapl, memb_name, memb_addr,
                        TRUE) < 0)
    TEST_ERROR;
  ...
  if ((fidB = H5Fcreate(filename2B, H5F_ACC_TRUNC, H5P_DEFAULT, multi_fapl)) <
      0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(filename2A, "/A", fid, "ext_linkA", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  /* Create external link to target file B:/B */
  if (H5Lcreate_external(filename2B, "/B", fid, "ext_linkB", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* should succeed in opening the target object A in the current working
   * directory */
  if (oidA < 0) {
    H5_FAILED();
  ...
  /* should succeed in opening the target object B in the current working
   * directory */
  if (oidB < 0) {
    H5_FAILED();
    HDputs("    Should succeed in opening multi target file B in current "
           "working directory");
    goto error;
  }
 ...
 *
 * Purpose:     To verify that processing done to the external linked target
 *object is correctly handled when the parent and target files have the same
 ...
 *         5. Create external link from main file to target
 *file:ext_link->target file:/A/Dataset
 *         6. Set the file access property list of the link access to use "core"
 *file without backing store
 *         7. Should succeed in opening the target dataset: ext_link
 *         8. Write data to the target dataset
 *         9. On closing, the file size of target should be the same as before
 *since it is opened without backing store.
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_set_elink_fapl2(hid_t fapl, hbool_t new_format) {
  ...
  if ((dset = H5Dcreate2(gid, "Dataset", H5T_NATIVE_INT, space, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(filename2, "/A/Dataset", fid, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *            9. H5Pclose() fapl should fail since closing in step #8 should
 *also close its fapl
 ...
 *           13. H5Pclose() fapl set in the copied link access should fail since
 *the removal in #12 should also close its fapl
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_set_elink_fapl3(hbool_t new_format) {
 ...
 *-------------------------------------------------------------------------
 */
static int external_set_elink_acc_flags(const char *env_h5_drvr, hid_t fapl,
                                        hbool_t new_format) {
  ...
  if ((file2 = H5Fcreate(filename2, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/", file1, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Attempt to create a group through the external link using gapl (should
   * fail) */
  H5E_BEGIN_TRY {
    group =
        H5Gcreate2(file1, "/ext_link/group", H5P_DEFAULT, H5P_DEFAULT, gapl);
  }
  H5E_END_TRY
  if (group != FAIL)
    TEST_ERROR;
  ...
  if ((group = H5Gcreate2(file1, "/ext_link/group", H5P_DEFAULT, H5P_DEFAULT,
                          gapl)) < 0)
    TEST_ERROR;
   ...
   * fail) */
  H5E_BEGIN_TRY {
    subgroup = H5Gcreate2(file1, "/ext_link/group/subgroup", H5P_DEFAULT,
                          H5P_DEFAULT, gapl);
  }
  H5E_END_TRY
  if (subgroup != FAIL)
  ...
  if (H5FD__supports_swmr_test(env_h5_drvr)) {
    ...
    if (new_format) {
      if ((file1 =
               H5Fopen(filename1, H5F_ACC_RDWR | H5F_ACC_SWMR_WRITE, fapl)) < 0)
        FAIL_STACK_ERROR;
    ...
    if ((file1 = H5Fopen(filename1, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, fapl)) <
        0)
      FAIL_STACK_ERROR;
  ...
  if (file1 != FAIL)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    file1 = H5Fcreate(filename1, H5F_ACC_TRUNC | H5F_ACC_DEFAULT, H5P_DEFAULT,
                      fapl);
  }
  H5E_END_TRY
  if (file1 != FAIL)
...
static herr_t external_set_elink_cb_cb(const char *parent_file,
                                       ...
                                       const char *target_obj, unsigned *flags,
...
/* Main test function */
static int external_set_elink_cb(hid_t fapl, hbool_t new_format) {
  hid_t file1 = -1, file2 = -1, group = -1, gapl = -1, fam_fapl = -1,
        ret_fapl = -1, base_driver;
  ...
  /* Core file driver has issues when used as the member file driver for a
   * family file */
  /* Family file driver cannot be used with family or multi drivers for member
   * files */
  /* Also disable parallel member drivers, because
     H5F_HAS_FEATURE(H5FD_FEAT_HAS_MPI) would report FALSE, causing problems */
  base_driver = H5Pget_driver(fapl);
  op_data.base_fapl =
      (base_driver == H5FD_FAMILY || base_driver == H5FD_MULTI ||
       base_driver == H5FD_MPIO || base_driver == H5FD_CORE ||
       base_driver == H5FD_DIRECT || base_driver == H5FD_SUBFILING)
          ? H5P_DEFAULT
          : fapl;
  ...
  if ((file2 = H5Fcreate(filename2, H5F_ACC_TRUNC, H5P_DEFAULT, fam_fapl)) < 0)
    TEST_ERROR;
  if ((group = H5Gcreate2(file1, "group1", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Lcreate_external(filename2, "/", group, "ext_link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((group = H5Gcreate2(file1, "/group1/ext_link/group2", H5P_DEFAULT,
                          H5P_DEFAULT, gapl)) < 0)
    TEST_ERROR;
  ...
  /* Modify the user data structure to cause the callback to return invalid
   * flags */
  op_data.code = 2;
...
static int external_reset_register(void) {
  ...
  if (H5Lcreate_external("some_file", "some_obj", file, "ext_link1",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((file = H5Fopen(filename, H5F_ACC_RDWR, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Lcreate_external("another_file", "another_obj", file, "ext_link2",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Fclose(file) < 0)
    TEST_ERROR;
 ...
 *            Should be able to find the target file via main file's current
 *drive/rel path
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_win1(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
  ...
  if (new_format)
    TESTING("external links via main file's current drive/rel "
            "path(windows)(w/new group format)");
  else
  ...
  HDstrcpy(tmpname, &cwdpath[2]); /* stripped the drive letter to make it rel
                                     drive but absolute path */
  HDstrcat(tmpname, "/");
 ...
 *              Should be able to access the target file directly (rel drive/abs
 *path)
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_win2(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
  ...
  if (new_format)
    TESTING("external links via target's rel drive/abs path "
            "directly(windows)(w/new group format)");
  else
  ...
  HDstrcpy(tmpname, &cwdpath[2]); /* stripped the drive letter to make it
                                     relative drive but absolute path */
  HDstrcat(tmpname, "/");
 ...
 *              Should be able to access the target file directly (abs drive/rel
 *path)
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_win3(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  int drive = 0;
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE];

  if (new_format)
    TESTING("external links via target's abs drive/rel path directly "
            "(windows)(w/new group format)");
  else
    TESTING(
        "external links via target's abs drive/rel path directly (windows)");
  ...
  drive = HDgetdrive();
  HDsnprintf(tmpname, sizeof(tmpname), "%c:%s", (drive + 'A' - 1),
             FILENAME[32]);
  h5_fixname(tmpname, fapl, filename2, sizeof filename2);
 ...
 *           Should be able to access the target file via main file's abs
 *drive/rel path
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_win4(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  int drive = 0;
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
  ...
  if (new_format)
    TESTING("external links via main file's abs drive/rel path (windows)(w/new "
            "group format)");
  else
  ...
  /* should be able to find the target file via main file's absolute
   * drive/relative path */
  if (gid < 0) {
    H5_FAILED();
    HDputs("    Should have found the file in CWD.");
 ...
 *              Should be able to access the target file via main file's
 *relative drive/absolute path
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_win5(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  int drive = 0;
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
  ...
  if (new_format)
    TESTING("external links via main file's rel drive/abs path (windows)(w/new "
            "group format)");
  else
 ...
 *              Should be able to access the target file via target's current
 *drive/rel path
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_win6(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  int drive = 0;
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
  ...
  if (new_format)
    TESTING("external links via target's current drive/rel path "
            "(windows)(w/new group format)");
  else
  ...
  /* should be able to find the target file via target file's rel path in
   * current drive */
  if (gid < 0) {
    H5_FAILED();
    HDputs("    Should have found the file in tmp_links.");
 ...
 *         Should be able to find the target file via main file's local
 *host/main drive/rel path
 *
 * Return:      Success:        0
 *              Failure:        -1
 *-------------------------------------------------------------------------
 */
static int external_link_win7(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
  ...
  if (new_format)
    TESTING("external links via main file's UNC local host/main drive/rel "
            "path(windows)(w/new group format)");
  else
    TESTING("external links via main file's UNC local host/main drive/rel "
            "path(windows)");
  ...
  /* should be able to find the target file via main file's local host/main
   * drive*/
  if (gid < 0) {
    H5_FAILED();
 ...
 *      2. main file: "extlinks0"
 *      3. target file: "extlinks10"
 *         Should be able to access the target file directly (abs drive/abs
 *path)
 *
 * Return:      Success:        0
 *              Failure:        -1
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win8(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  int drive = 0;
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
  ...
  if (new_format)
    TESTING("external links via target's Long UNC abs drive/abs path directly "
            "(windows)(w/new group format)");
  else
    TESTING("external links via target's Long UNC abs drive/abs path directly "
            "(windows)");
  ...
  drive = HDgetdrive();
  HDsnprintf(tmpname, sizeof(tmpname), "\\\\?\\%c:%s\\%s", (drive + 'A' - 1),
             &cwdpath[2], FILENAME[30]);
  h5_fixname(tmpname, fapl, filename2, sizeof filename2);
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_win9(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE], filename2[NAME_BUF_SIZE],
      filename3[NAME_BUF_SIZE], tmpname[NAME_BUF_SIZE], cwdpath[NAME_BUF_SIZE];
  ...
  if (new_format)
    TESTING("external links via main file's Long UNC local host/main drive/rel "
            "path(windows)(w/new group "
            "format)");
  else
    TESTING("external links via main file's Long UNC local host/main drive/rel "
            "path(windows)");
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_recursive(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;                /* File ID */
  hid_t gid = -1;                /* Group IDs */
  ...
  if (H5Lcreate_external(filename1, "/recursive", fid, "recursive", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_query(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;          /* File ID */
  hid_t gid = -1;          /* Group IDs */
  const char *file_name;   /* Name of the file the external link points to */
  const char *object_name; /* Name of the object the external link points to */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_unlink_compact(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;            /* File ID */
  hid_t gid = -1, gid2 = -1; /* Group IDs */
  char filename1[NAME_BUF_SIZE],
      filename2[NAME_BUF_SIZE]; /* Names of files to externally link across */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_unlink_dense(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  ...
  /* Create external link */
  /* (This also covers the case of having an external link in a compact group
   * that's converted to a dense group) */
  if (H5Lcreate_external(filename2, "/dst", gid, "src", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  for (u = 0; u < max_compact; u++) {
    HDsnprintf(objname, sizeof(objname), "filler %u", u);
    if ((gid2 = H5Gcreate2(gid, objname, H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
      TEST_ERROR;
    if (H5Gclose(gid2) < 0)
      TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_move(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  hid_t gid = -1, gid2 = -1;   /* Group IDs */
  char objname[NAME_BUF_SIZE]; /* Object name */
  char filename1[NAME_BUF_SIZE],
  ...
  if ((gid = H5Gcreate2(fid, "group2", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    FAIL_STACK_ERROR;
  ...
  /* Create object in external file */
  if ((gid2 = H5Gcreate2(gid, "new_group2", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Lmove(fid, "/group2/src3", H5L_SAME_LOC, "/src", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create object in external file */
  if ((gid2 = H5Gcreate2(gid, "new_group3", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_ride(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  hid_t gcpl = -1;             /* Group creation property list ID */
  hid_t gid = -1, gid2 = -1;   /* Group IDs */
  char objname[NAME_BUF_SIZE]; /* Object name */
  ...
  /* Remove enough objects to convert group containing external link back into
   * compact form */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_closing(hid_t fapl, hbool_t new_format) {
  hid_t fid1 = (-1), fid2 = (-1), fid3 = (-1), fid4 = (-1);
  hid_t gid = (-1), tid = (-1), tid2 = (-1), sid = (-1), did = (-1);
  hid_t lcpl_id = (-1);
  hsize_t dims[2];
  ...
  if (H5Lget_info2(fid1, "elink/elink/elink", &li, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(fid1, "elink/elink/elink/type1", &oi, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(fid1, "elink/elink/elink", &oi, H5O_INFO_BASIC,
                           H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  if (H5Oget_info_by_name3(fid1, "elink/elink2/group2/group3/group4", &oi,
                           H5O_INFO_BASIC, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  if (H5Lget_info2(fid1, "elink/file2group1/file2group2/slink/group3", &li,
                   H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_endian(hbool_t new_format) {
  hid_t fid = -1;                        /* File ID */
  hid_t gid = -1, gid2 = -1;             /* Group IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_strong(hid_t fapl, hbool_t new_format) {
  ...
  if (new_format)
    TESTING("that external files work with strong file close degree (w/new "
            "group format)");
  else
  ...
  if (H5Gclose(gid1) < 0)
    TEST_ERROR;
  if ((gid1 = H5Gcreate2(fid1, "A/B", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Gclose(gid1) < 0)
    TEST_ERROR;
  if ((gid1 = H5Gcreate2(fid1, "A/B/C", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid1) < 0)
    TEST_ERROR;
  ...
  if ((fid2 = H5Fcreate(filename2, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
    TEST_ERROR;
  if ((gid2 = H5Gcreate2(fid2, "/W", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Gclose(gid2) < 0)
    TEST_ERROR;
  if ((gid2 = H5Gcreate2(fid2, "/W/X", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Lcreate_external(filename1, "/A/B/C", gid2, "DLINK", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Gclose(gid2) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_symlink(const char *env_h5_drvr, hid_t fapl,
                            hbool_t new_format) {
  ...
  char *tmpname = NULL;
  char *cwdpath = NULL;
  hbool_t have_posix_compat_vfd; /* Whether VFD used is compatible w/POSIX I/O
                                    calls */
  ...
  /* set up names for file #2 in temporary directory #2:
   * "tmp2_links/extlinks21B" */
  ...
  /* Create symbolic link #1 in temporary directory #1 to file #2 in temporary
   * directory #2 */
  ...
  /* Create symbolic link #2 in temporary directory #2 to file #4 in temporary
   * directory #1 */
  ...
  /* Create external link to file & object in temporary directory #2, using
   * symlink #1 name */
  if (H5Lcreate_external(SYMLINK1, "group2", file1, "extlink2-sym", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((group2 = H5Gcreate2(file2, "group2", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create external link to file #3 & object in temporary directory #2 */
  if (H5Lcreate_external(filename3b, "group3", group2, "extlink3", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((group3 = H5Gcreate2(file3, "group3", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create external link to file & object in temporary directory #1, using
   * symlink #2 name */
  if (H5Lcreate_external(SYMLINK2, "group4", group3, "extlink4-sym",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((group4 = H5Gcreate2(file4, "group4", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create external link to file #5 & object in temporary directory #1 */
  if (H5Lcreate_external(filename5b, "group5", group4, "extlink5", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((group5 = H5Gcreate2(file5, "group5", H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Open group in file #4, through external link w/symlink to external link
   * w/symlink */
  if ((group4 = H5Gopen2(file1, "extlink2-sym/extlink3/extlink4-sym",
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(group4) < 0)
    TEST_ERROR;

  /* Open group in file #5, through external link w/symlink to external link
   * w/symlink to external link */
  if ((group5 = H5Gopen2(file1, "extlink2-sym/extlink3/extlink4-sym/extlink5",
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(group5) < 0)
    TEST_ERROR;
...
error:
  ...
  SKIPPED();
  HDputs("    Current file system or operating system doesn't support symbolic "
         "links");
 ...
 *-------------------------------------------------------------------------
 */
static int external_copy_invalid_object(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;      /* File ID */
  hid_t gid = -1;      /* Group ID */
  ...
  if ((fid = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
    TEST_ERROR;
  if ((gid = H5Gcreate2(fid, "group1", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
   ...
   * object name */
  if (H5Lcreate_external(filename, "no_object", fid, "/group1/link",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_dont_fail_to_source(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group ID */
  ...
  if (new_format)
    TESTING("that invalid external links don't open the source file (w/new "
            "group format)");
  else
  ...
  if (H5Lcreate_external("no_file", "/group", fid, "link", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_file_cache(hid_t fapl, hbool_t new_format) {
 ...
 *-------------------------------------------------------------------------
 */
static int external_open_twice(hid_t fapl, hbool_t new_format) {
  hid_t fid1 = -1;  /* File ID */
  hid_t fid2 = -1;  /* File ID */
  ...
  /* Create target group */
  if ((oid1 = H5Gcreate2(fid2, "group", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(oid1) < 0)
    TEST_ERROR;
 ...
 *              (A) Attach an attribute to an externally linked group in the
 *target file. The attribute's datatype is a committed datatype to the root
 *group in the main file. (B) Create a dataset to an externally group in the
 *target file. The dataset's datatype is a committed datatype to the root group
 *              in the main file.
 *
 * Return:      Success:        0
 *              Failure:        -1
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_with_committed_datatype(hid_t fapl,
                                                 hbool_t new_format) {
  ...
  if (new_format)
    TESTING("attach committed datatype to external group's "
            "attribute/dataset(w/new group format)");
  else
  ...
  if (H5Lcreate_external(filename2, "target_group", fid1, "link_to_2",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((gid2 = H5Gcreate2(fid2, "target_group", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5Tcommit2(fid1, "myDatatype", tid, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((aid = H5Acreate2(gid1, "myAttribute", tid, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  ...
  if ((did = H5Dcreate2(gid1, "myDataset", tid, sid2, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int external_link_public_macros(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;              /* File ID */
  H5L_info2_t linfo;           /* Link information */
  char objname[NAME_BUF_SIZE]; /* Object name */
  char filename1[NAME_BUF_SIZE];
...
/* Callback functions for UD hard links. */
/* UD_hard_create increments the object's reference count */
static herr_t UD_hard_create(const char H5_ATTR_UNUSED *link_name,
                             hid_t loc_group, const void *udata,
...
static hid_t UD_hard_traverse(const char H5_ATTR_UNUSED *link_name,
                              hid_t cur_group, const void *udata,
                              size_t udata_size, hid_t H5_ATTR_UNUSED lapl_id,
                              hid_t H5_ATTR_UNUSED dxpl_id) {
  H5O_token_t token;
  hid_t ret_value = -1;
...
/* UD_hard_delete decrements the object's reference count */
static herr_t UD_hard_delete(const char H5_ATTR_UNUSED *link_name, hid_t file,
                             const void *udata, size_t udata_size) {
  H5O_token_t token;
  hid_t target_obj = -1;
  herr_t ret_value = 0;
...
static herr_t UD_cb_create(const char *link_name, hid_t loc_group,
  ...
  if (HDstrcmp(link_name, UD_CB_LINK_NAME) != 0 &&
      HDstrcmp(link_name, NEW_UD_CB_LINK_NAME) != 0)
    TEST_ERROR;
...
static int ud_callbacks(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;                       /* File ID */
  hid_t gid = -1;                       /* Group ID */
  hid_t lcpl = -1;                      /* Link Creation PL */
  H5L_info2_t li;                       /* Link information */
...
static int lapl_udata(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;            /* File ID */
  hid_t gid = -1, gid2 = -1; /* Group IDs */
  ...
  if ((gid = H5Gcreate2(fid, "group_a", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  if ((gid = H5Gcreate2(fid, "group_b", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid) < 0)
    TEST_ERROR;
  ...
  if (H5Pinsert2(plist_id, DEST_PROP_NAME, (size_t)NAME_BUF_SIZE, group_a_name,
  ...
  if ((gid = H5Oopen(fid, "ud_link", plist_id)) < 0)
    TEST_ERROR;
  if ((gid2 = H5Gcreate2(gid, "subgroup_a", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid2) < 0)
    TEST_ERROR;
  ...
  if ((gid = H5Oopen(fid, "ud_link", plist_id)) < 0)
    TEST_ERROR;
  if ((gid2 = H5Gcreate2(gid, "subgroup_b", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Gclose(gid2) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static herr_t UD_cbsucc_create(const char H5_ATTR_UNUSED *link_name,
                               hid_t H5_ATTR_UNUSED loc_group,
                               const void *udata, size_t udata_size,
                               hid_t H5_ATTR_UNUSED lcpl_id) {
...
static herr_t UD_cbfail_move(const char H5_ATTR_UNUSED *new_name,
                             hid_t H5_ATTR_UNUSED new_loc,
                             const void H5_ATTR_UNUSED *udata,
...
static herr_t UD_cbsucc_move(const char H5_ATTR_UNUSED *new_name,
                             hid_t H5_ATTR_UNUSED new_loc,
                             const void H5_ATTR_UNUSED *udata,
                             size_t H5_ATTR_UNUSED udata_size) {
...
/* Callback for when the link is deleted.  Also called during move */
static herr_t UD_cbsucc_delete(const char H5_ATTR_UNUSED *link_name,
                               hid_t H5_ATTR_UNUSED file,
                               const void H5_ATTR_UNUSED *udata,
                               size_t H5_ATTR_UNUSED udata_size) {
...
/* Callback for when the link is deleted.  Also called during move */
static herr_t UD_cbfail_delete(const char H5_ATTR_UNUSED *link_name,
                               hid_t H5_ATTR_UNUSED file,
                               const void H5_ATTR_UNUSED *udata,
                               size_t H5_ATTR_UNUSED udata_size) {
  /* This traversal function will always fail. */
...
/* Callback for when the link is queried */
static ssize_t UD_cbsucc_query(const char H5_ATTR_UNUSED *link_name,
                               const void H5_ATTR_UNUSED *udata,
                               size_t H5_ATTR_UNUSED udata_size, void *buf,
                               size_t buf_size) {
  /* This traversal function will return a buffer size,
   * but will fail when a buffer is passed in ("writing to the buffer"
   * fails
...
static int ud_link_errors(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1; /* File ID */
  hid_t gid = -1; /* Group IDs */
  ...
  H5E_BEGIN_TRY {
    if (H5Lcreate_ud(fid, "/ud_link", H5L_TYPE_HARD, NULL, (size_t)0,
                     H5P_DEFAULT, H5P_DEFAULT) >= 0)
      TEST_ERROR;
    if (H5Lcreate_ud(fid, "/ud_link", H5L_TYPE_SOFT, "str", (size_t)4,
                     H5P_DEFAULT, H5P_DEFAULT) >= 0)
      TEST_ERROR;
  }
  H5E_END_TRY

  /* Try to create a link with H5Lcreate_ud that has a NULL udata pointer, but a
   * non-zero udata_size value
   */
  H5E_BEGIN_TRY {
    if (H5Lcreate_ud(fid, "/ud_link", (H5L_type_t)UD_CBFAIL_TYPE, NULL, 1,
                     H5P_DEFAULT, H5P_DEFAULT) >= 0)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  if (H5Lcreate_ud(fid, "/ud_link", (H5L_type_t)UD_CBFAIL_TYPE, &group_name,
                   HDstrlen(group_name) + 1, H5P_DEFAULT, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  H5E_BEGIN_TRY {
    ...
    if (H5Lcreate_ud(fid, "fail", (H5L_type_t)UD_CBFAIL_TYPE, NULL, (size_t)0,
                     H5P_DEFAULT, H5P_DEFAULT) >= 0)
      TEST_ERROR;

    /* The move and copy callbacks will fail */
    if (H5Lmove(fid, "ud_link", H5L_SAME_LOC, "move_fail", H5P_DEFAULT,
                H5P_DEFAULT) >= 0)
      TEST_ERROR;
    if (H5Lcopy(fid, "ud_link", fid, "copy_fail", H5P_DEFAULT, H5P_DEFAULT) >=
        0)
      TEST_ERROR;
  ...
  /* The query callback will succeed when we only want to get the size of the
   * buffer... */
  if (H5Lget_info2(fid, "ud_link", &li, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (li.u.val_size != 0)
  ...
  H5E_BEGIN_TRY {
    if (H5Lget_val(fid, "ud_link", query_buf, (size_t)NAME_BUF_SIZE,
                   H5P_DEFAULT) >= 0)
      TEST_ERROR;
  }
  H5E_END_TRY
  ...
  if (H5Lmove(fid, "copy_succ", H5L_SAME_LOC, "move_succ", H5P_DEFAULT,
              H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int lapl_nlinks(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;                        /* File ID */
  hid_t gid = -1, gid2 = -1;             /* Group IDs */
  hid_t plist = -1;                      /* lapl ID */
  hid_t tid = -1, sid = -1, did = -1;    /* Other IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int linkinfo(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1;           /* File ID */
  hid_t gid = -1;           /* Group ID */
  hid_t tid = -1;           /* Type ID */
  hid_t sid = -1, did = -1; /* Dataspace and dataset IDs */
 ...
 *-------------------------------------------------------------------------
 */
static int check_all_closed(hid_t fapl, hbool_t new_format, int stopat) {
  hid_t fid = -1;
  char filename[NAME_BUF_SIZE];
  int x;
...
static hid_t build_visit_file(hid_t fapl) {
  ...
  char filename[NAME_BUF_SIZE];
  const char *pathname =
      H5_get_srcdir_filename(LINKED_FILE); /* Corrected test file name */
  ...
  /* Create group */
  if ((gid = H5Gcreate2(fid, "/Group1", H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create nested group */
  if ((gid2 = H5Gcreate2(gid, "Group2", H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_soft("/Group1", fid, "/soft_one", H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_soft("/Group1/Group2", fid, "/soft_two", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Create dangling soft link */
  if (H5Lcreate_soft("nowhere", fid, "/soft_dangle", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    TEST_ERROR;
  ...
  if (H5Lcreate_hard(fid, "/", fid, "hard_zero", H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "/Group1", fid, "hard_one", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "/Group1/Group2", fid, "hard_two", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Create loops w/hard links */
  if (H5Lcreate_hard(fid, "/Group1", fid, "/Group1/hard_one", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Lcreate_hard(fid, "/", fid, "/Group1/Group2/hard_zero", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Create external link to existing file */
  if (H5Lcreate_external(pathname, "/group", fid, "/ext_one", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Create dangling external link to non-existent file */
  if (H5Lcreate_external("foo.h5", "/group", fid, "/ext_dangle", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if ((did = H5Dcreate2(fid, "/Dataset_zero", H5T_NATIVE_INT, sid, H5P_DEFAULT,
                        dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dclose(did) < 0)
    TEST_ERROR;

  if ((did = H5Dcreate2(fid, "/Group1/Dataset_one", H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dclose(did) < 0)
    TEST_ERROR;

  if ((did = H5Dcreate2(fid, "/Group1/Group2/Dataset_two", H5T_NATIVE_INT, sid,
                        H5P_DEFAULT, dcpl_g, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Dclose(did) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(fid, "/Type_zero", tid, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Tclose(tid) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(fid, "/Group1/Type_one", tid, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Tclose(tid) < 0)
    TEST_ERROR;
  ...
  if ((tid = H5Tcopy(H5T_NATIVE_INT)) < 0)
    TEST_ERROR;
  if (H5Tcommit2(fid, "/Group1/Group2/Type_two", tid, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (H5Tclose(tid) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int visit_link_cb(hid_t H5_ATTR_UNUSED group_id, const char *name,
                         const H5L_info2_t *linfo, void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int link_visit(hid_t fapl, hbool_t new_format) {
 ...
 *-------------------------------------------------------------------------
 */
static int link_visit_by_name(hid_t fapl, hbool_t new_format) {
  lvisit_ud_t udata; /* User-data for visiting */
  hid_t fid = -1;
  hid_t gid = -1; /* Group ID */
  ...
  udata.idx = 0;
  udata.info = lvisit0;
  if (H5Lvisit_by_name2(fid, "/", H5_INDEX_NAME, H5_ITER_INC, visit_link_cb,
                        &udata, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  udata.idx = 0;
  udata.info = lvisit0;
  if (H5Lvisit_by_name2(gid, ".", H5_INDEX_NAME, H5_ITER_INC, visit_link_cb,
                        &udata, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(gid) < 0)
    FAIL_STACK_ERROR;
  ...
  udata.idx = 0;
  udata.info = lvisit1;
  if (H5Lvisit_by_name2(fid, "/Group1", H5_INDEX_NAME, H5_ITER_INC,
                        visit_link_cb, &udata, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  udata.idx = 0;
  udata.info = lvisit2;
  if (H5Lvisit_by_name2(fid, "/Group1/Group2", H5_INDEX_NAME, H5_ITER_INC,
                        visit_link_cb, &udata, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int visit_obj_cb(hid_t H5_ATTR_UNUSED group_id, const char *name,
 ...
 *-------------------------------------------------------------------------
 */
static int obj_visit(hid_t fapl, hbool_t new_format) {
  ...
  if (H5Ovisit3(fid, H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata,
                H5O_INFO_BASIC) < 0)
    FAIL_STACK_ERROR;
  ...
  udata.idx = 0;
  udata.info = new_format ? ovisit0_new : ovisit0_old;
  if (H5Ovisit3(gid, H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb, &udata,
                H5O_INFO_BASIC) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(gid) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int obj_visit_by_name(hid_t fapl, hbool_t new_format) {
  ovisit_ud_t udata; /* User-data for visiting */
  hid_t fid = -1;
  hid_t gid = -1; /* Group ID */
  ...
  udata.idx = 0;
  udata.info = new_format ? ovisit0_new : ovisit0_old;
  if (H5Ovisit_by_name3(fid, "/", H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb,
                        &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  udata.idx = 0;
  udata.info = new_format ? ovisit0_new : ovisit0_old;
  if (H5Ovisit_by_name3(gid, ".", H5_INDEX_NAME, H5_ITER_INC, visit_obj_cb,
                        &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  if (H5Gclose(gid) < 0)
    FAIL_STACK_ERROR;
  ...
  udata.idx = 0;
  udata.info = new_format ? ovisit1_new : ovisit1_old;
  if (H5Ovisit_by_name3(fid, "/Group1", H5_INDEX_NAME, H5_ITER_INC,
                        visit_obj_cb, &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  udata.idx = 0;
  udata.info = new_format ? ovisit2_new : ovisit2_old;
  if (H5Ovisit_by_name3(fid, "/Group1/Group2", H5_INDEX_NAME, H5_ITER_INC,
                        visit_obj_cb, &udata, H5O_INFO_BASIC, H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int visit_obj_stop_cb(hid_t H5_ATTR_UNUSED group_id,
                             const char H5_ATTR_UNUSED *name,
                             const H5O_info2_t H5_ATTR_UNUSED *oinfo,
                             void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int obj_visit_stop(hid_t fapl, hbool_t new_format) {
   ...
   */
  nvisited = 0;
  if ((ret = H5Ovisit3(fid, H5_INDEX_NAME, H5_ITER_INC, visit_obj_stop_cb,
                       &nvisited, H5O_INFO_BASIC)) < 0)
    FAIL_STACK_ERROR;
  if (ret != H5_ITER_STOP)
    TEST_ERROR;
  if (nvisited != 1)
  ...
  nvisited = 0;
  if ((ret = H5Ovisit_by_name3(fid, "/", H5_INDEX_NAME, H5_ITER_INC,
                               visit_obj_stop_cb, &nvisited, H5O_INFO_BASIC,
                               H5P_DEFAULT)) < 0)
    FAIL_STACK_ERROR;
  if (ret != H5_ITER_STOP)
    TEST_ERROR;
...
static htri_t link_filter_can_apply(hid_t dcpl_id, hid_t type_id,
                                    hid_t space_id) {
...
static herr_t link_filter_set_local(hid_t dcpl_id, hid_t type_id,
                                    hid_t space_id) {
  if (dcpl_id >= 0 || type_id >= 0 || space_id >= 0)
    return -1;
...
static int link_filters(hid_t fapl, hbool_t new_format) {
  hid_t fid = -1, fcpl = -1;
  ...
  if (H5Zregister(&filter_class) < 0)
    TEST_ERROR;
  if (H5Pset_filter(gcpl1, H5Z_FILTER_RESERVED + 42, 0, (size_t)1, &cd_value) <
      0)
    TEST_ERROR;
  nfilters++;
  ...
  cd_value++;
  if (H5Pmodify_filter(gcpl2, H5Z_FILTER_RESERVED + 42, 0, (size_t)1,
                       &cd_value) < 0)
    TEST_ERROR;
  if (H5Pget_filter_by_id2(gcpl2, H5Z_FILTER_RESERVED + 42, &flags_out,
                           &cd_nelmts, &cd_value_out, (size_t)24, name_out,
  ...
  if (H5Premove_filter(gcpl2, H5Z_FILTER_RESERVED + 42) < 0)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    status = H5Pget_filter_by_id2(gcpl2, H5Z_FILTER_RESERVED + 42, &flags_out,
                                  &cd_nelmts, &cd_value_out, (size_t)24,
                                  name_out, &filter_config_out);
  }
  H5E_END_TRY
  if (status >= 0)
    TEST_ERROR;
  ...
  if ((gid1 = H5Gcreate2(fid, "group1/group2/group3/group4", lcpl, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Open intermediate groups that were created and verify that they have
   * filters */
 ...
 *-------------------------------------------------------------------------
 */
static int obj_exists(hid_t fapl, hbool_t new_format) {
  ...
  /* Hard links */
  /* Verify that H5Oexists_by_name() returns false for non-existent link in root
   * group */
  ...
  /* Verify that H5Oexists_by_name() returns false for non-existent object in
   * non-root group */
  ...
  /* Verify that H5Oexists_by_name() returns FALSE for dangling soft-link in
   * root group */
  ...
  /* Verify that H5Oexists_by_name() returns TRUE for soft-link in root group
   * that points to object */
  if (TRUE != H5Oexists_by_name(fid, "soft2", H5P_DEFAULT))
    TEST_ERROR;

  /* Create dangling soft-link in non-root group */
  if (H5Lcreate_soft("dangle", fid, "group/soft1", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5Lcreate_soft("/group", fid, "group/soft2", H5P_DEFAULT, H5P_DEFAULT) <
      0)
    FAIL_STACK_ERROR;

  /* Verify that H5Oexists_by_name() returns TRUE for soft-link in non-root
   * group that points to object */
  if (TRUE != H5Oexists_by_name(fid, "group/soft2", H5P_DEFAULT))
    TEST_ERROR;

  /* External links */
  /* Create dangling (file doesn't exist) external link in root group */
  if (H5Lcreate_external("nofile", "dangle", fid, "external1", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Verify that H5Oexists_by_name() returns FALSE for dangling (file doesn't
   * exist) external link in root group */
  if (FALSE != H5Oexists_by_name(fid, "external1", H5P_DEFAULT))
    TEST_ERROR;

  /* Create dangling (object doesn't exist) external link in root group */
  if (H5Lcreate_external(filename, "dangle", fid, "external2", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Verify that H5Oexists_by_name() returns FALSE for dangling (object doesn't
   * exist) external link in root group */
  if (FALSE != H5Oexists_by_name(fid, "external2", H5P_DEFAULT))
    TEST_ERROR;

  /* Create external link in root group that points to object */
  if (H5Lcreate_external(filename, "group", fid, "external3", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Verify that H5Oexists_by_name() returns TRUE for external link in root
   * group that points to object */
  if (TRUE != H5Oexists_by_name(fid, "external3", H5P_DEFAULT))
    TEST_ERROR;

  /* Create dangling (file doesn't exist) external link in non-root group */
  if (H5Lcreate_external("nofile", "dangle", fid, "group/external1",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Verify that H5Oexists_by_name() returns FALSE for dangling (file doesn't
   * exist) external link in non-root group */
  ...
  if (H5Lcreate_external(filename, "dangle", fid, "group/external2",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  if (H5Lcreate_external(filename, "group", fid, "group/external3", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Verify that H5Oexists_by_name() returns TRUE for external link in non-root
   * group that points to object
   */
  ...
  /* Soft->External links */
  /* Create soft-link in root group that points to dangling (file doesn't exist)
   * external link */
  if (H5Lcreate_soft("external1", fid, "soft-elink1", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in root group that points to dangling (object doesn't
   * exist) external link */
  if (H5Lcreate_soft("external2", fid, "soft-elink2", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in root group that points to external link that points to
   * object */
  if (H5Lcreate_soft("external3", fid, "soft-elink3", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in root group that points to dangling (file doesn't exist)
   * external link in non-root group */
  if (H5Lcreate_soft("group/external1", fid, "soft-elink4", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in root group that points to dangling (object doesn't
   * exist) external link in non-root group */
  if (H5Lcreate_soft("group/external2", fid, "soft-elink5", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in root group that points to external link in non-root
   * group that points to object */
  if (H5Lcreate_soft("group/external3", fid, "soft-elink6", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in non-root group that points to dangling (file doesn't
   * exist) external link */
  if (H5Lcreate_soft("/external1", fid, "group/soft-elink1", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in non-root group that points to dangling (object doesn't
   * exist) external link */
  if (H5Lcreate_soft("/external2", fid, "group/soft-elink2", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in non-root group that points to external link that points
   * to object */
  if (H5Lcreate_soft("/external3", fid, "group/soft-elink3", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in non-root group that points to dangling (file doesn't
   * exist) external link in non-root group */
  if (H5Lcreate_soft("/group/external1", fid, "group/soft-elink4", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in non-root group that points to dangling (object doesn't
   * exist) external link in non-root group */
  if (H5Lcreate_soft("/group/external2", fid, "group/soft-elink5", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* Create soft-link in non-root group that points to external link in non-root
   * group that points to object
   */
  if (H5Lcreate_soft("/group/external3", fid, "group/soft-elink6", H5P_DEFAULT,
                     H5P_DEFAULT) < 0)
    FAIL_STACK_ERROR;
  ...
  /* External->Soft links */
  /* Create external link in root group that points to dangling soft link in
   * root group */
  if (H5Lcreate_external(filename, "soft1", fid, "elink-soft1", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link in root group that points to soft link in root group
   * that points to object */
  if (H5Lcreate_external(filename, "soft2", fid, "elink-soft2", H5P_DEFAULT,
                         H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link in root group that points to dangling soft link in
   * non-root group */
  if (H5Lcreate_external(filename, "group/soft1", fid, "elink-soft3",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link in root group that points to soft link in root group
   * that points to object */
  if (H5Lcreate_external(filename, "group/soft2", fid, "elink-soft4",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link in non-root group that points to dangling soft link in
   * root group */
  if (H5Lcreate_external(filename, "soft1", fid, "group/elink-soft1",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link in non-root group that points to soft link in root
   * group that points to object */
  if (H5Lcreate_external(filename, "soft2", fid, "group/elink-soft2",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link in non-root group that points to dangling soft link in
   * non-root group */
  if (H5Lcreate_external(filename, "group/soft1", fid, "group/elink-soft3",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
  ...
  /* Create external link in non-root group that points to soft link in non-root
   * group that points to object
   */
  if (H5Lcreate_external(filename, "group/soft2", fid, "group/elink-soft4",
                         H5P_DEFAULT, H5P_DEFAULT) < 0)
    TEST_ERROR;
...
static int corder_create_empty(hid_t fapl) {
  ...
  /* Setting invalid combination of a group order creation order indexing on
   * should fail */
  H5E_BEGIN_TRY {
  ...
  if (ret > 0) {
    H5_FAILED();
    HDputs("    H5Pset_link_create_order() should have failed for a creation "
           "order index with no tracking.");
    TEST_ERROR;
  } /* end if */
...
static int corder_delete(hid_t fapl) {
  ...
  for (reopen_file = FALSE; reopen_file <= TRUE; reopen_file++) {
    ...
    /* Creating file with latest format will enable paged aggregation with
     * persistent fs */
    if ((file_id = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
      FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int link_info_by_idx_check(hid_t group_id, const char *linkname,
                                  hsize_t n, hbool_t hard_link,
                                  hbool_t use_index) {
  char tmpname[NAME_BUF_SIZE]; /* Temporary link name */
  char valname[NAME_BUF_SIZE]; /* Link value name */
  ...
  /* Verify the link information for first link, in increasing creation order */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                          (hsize_t)0, &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  /* Verify the link information for new link, in increasing creation order */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, n,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != (int64_t)n)
    TEST_ERROR;
  ...
  if (use_index) {
     ...
     * (which is increasing) */
    memset(&linfo, 0, sizeof(linfo));
    if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_NATIVE,
                            (hsize_t)0, &linfo, H5P_DEFAULT) < 0)
      TEST_ERROR;
    if (linfo.corder != 0)
      TEST_ERROR;
  ...
  /* Verify the link information for first link, in decreasing creation order */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC, n,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  /* Verify the link information for new link, in decreasing creation order */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_DEC,
                          (hsize_t)0, &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != (int64_t)n)
    TEST_ERROR;
   ...
   */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, (hsize_t)0,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  /* Verify the link information for new link, in increasing link name order */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, n, &linfo,
                          H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != (int64_t)n)
    TEST_ERROR;
   ...
   */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, n, &linfo,
                          H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != 0)
    TEST_ERROR;
  ...
  /* Verify the link information for new link, in decreasing link name order */
  memset(&linfo, 0, sizeof(linfo));
  if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC, (hsize_t)0,
                          &linfo, H5P_DEFAULT) < 0)
    TEST_ERROR;
  if (linfo.corder != (int64_t)n)
    TEST_ERROR;
...
static int link_info_by_idx(hid_t fapl) {
  ...
  for (hard_link = FALSE; hard_link <= TRUE; hard_link++) {
    ...
    for (use_index = FALSE; use_index <= TRUE; use_index++) {
      ...
      /* Check for query on empty group */
      H5E_BEGIN_TRY {
        ret = H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER,
                                  H5_ITER_INC, (hsize_t)0, &linfo, H5P_DEFAULT);
      }
      H5E_END_TRY
      if (ret >= 0)
      ...
      for (u = 0; u < max_compact; u++) {
        ...
        /* Verify link information for new link */
        if (link_info_by_idx_check(group_id, objname, (hsize_t)u, hard_link,
                                   use_index) < 0)
          TEST_ERROR;
      } /* end for */
...
static int link_info_by_idx_old(hid_t fapl) {
  ...
  char filename[NAME_BUF_SIZE]; /* File name */
  H5O_token_t
      objtoken[CORDER_NLINKS]; /* Tokens (Addresses) of the objects created */
  ...
  for (hard_link = FALSE; hard_link <= TRUE; hard_link++) {
    ...
    for (u = 0; u < CORDER_NLINKS; u++) {
      ...
      /* Verify link information (in increasing order) */
      if (hard_link) {
        if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                                (hsize_t)u, &linfo, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5Otoken_cmp(group_id, &linfo.u.token, &objtoken[u], &token_cmp) <
            0)
          TEST_ERROR;
        if (token_cmp)
          TEST_ERROR;
      ...
      /* Verify link information (in native order - native is increasing) */
      if (hard_link) {
        if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_NATIVE,
                                (hsize_t)u, &linfo, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5Otoken_cmp(group_id, &linfo.u.token, &objtoken[u], &token_cmp) <
            0)
      ...
      /* Verify link information (in decreasing order) */
      if (hard_link) {
        if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, H5_ITER_DEC,
                                (hsize_t)u, &linfo, H5P_DEFAULT) < 0)
          TEST_ERROR;
        if (H5Otoken_cmp(group_id, &linfo.u.token, &objtoken[dec_u],
                         &token_cmp) < 0)
          TEST_ERROR;
        if (token_cmp)
          TEST_ERROR;
    ...
    /* Check for creation order index queries */
    H5E_BEGIN_TRY {
      ret = H5Lget_info_by_idx2(group_id, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                                (hsize_t)u, &linfo, H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_DEC; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        for (u = 0; u < max_compact; u++) {
          ...
          /* Verify link information for new link */
          if (link_info_by_idx_check(group_id, objname, (hsize_t)u, TRUE,
                                     use_index) < 0)
            TEST_ERROR;
        } /* end for */
        ...
        for (u = 0; u < (max_compact - 1); u++) {
          ...
          /* Verify the link information for first link in appropriate order */
          memset(&linfo, 0, sizeof(linfo));
          if (H5Lget_info_by_idx2(group_id, ".", idx_type, order, (hsize_t)0,
                                  &linfo, H5P_DEFAULT) < 0)
            TEST_ERROR;
          if (order == H5_ITER_INC) {
            if (linfo.corder != (u + 1))
        ...
        for (u = 0; u < max_compact; u++) {
           ...
           */
          memset(&linfo, 0, sizeof(linfo));
          if (H5Lget_info_by_idx2(group_id, ".", idx_type, order, (hsize_t)u,
                                  &linfo, H5P_DEFAULT) < 0)
            TEST_ERROR;
          if (order == H5_ITER_INC) {
            if (linfo.corder != ((u * 2) + 1))
...
static int delete_by_idx_old(hid_t fapl) {
  ...
  for (order = H5_ITER_INC; order <= H5_ITER_DEC; order++) {
    ...
    for (u = 0; u < (CORDER_NLINKS - 1); u++) {
      ...
      /* Verify the link information for first link in appropriate order */
      memset(&linfo, 0, sizeof(linfo));
      if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
                              &linfo, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (order == H5_ITER_INC) {
        if (H5Otoken_cmp(group_id, &linfo.u.token, &objtoken[u + 1],
                         &token_cmp) < 0)
          TEST_ERROR;
        if (token_cmp)
          TEST_ERROR;
    ...
    for (u = 0; u < (CORDER_NLINKS / 2); u++) {
      ...
      /* Verify the link information for current link in appropriate order */
      memset(&linfo, 0, sizeof(linfo));
      if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                              &linfo, H5P_DEFAULT) < 0)
        TEST_ERROR;
      if (order == H5_ITER_INC) {
        if (H5Otoken_cmp(group_id, &linfo.u.token, &objtoken[(u * 2) + 1],
                         &token_cmp) < 0)
          TEST_ERROR;
        if (token_cmp)
          TEST_ERROR;
    ...
    for (u = 0; u < ((CORDER_NLINKS / 2) - 1); u++) {
      ...
      if (H5Lget_info_by_idx2(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
        ...
        TEST_ERROR;
      if (order == H5_ITER_INC) {
        if (H5Otoken_cmp(group_id, &linfo.u.token, &objtoken[(u * 2) + 3],
                         &token_cmp) < 0)
          TEST_ERROR;
        if (token_cmp)
          TEST_ERROR;
...
static int link_iterate_cb(hid_t group_id, const char *link_name,
  ...
  if (info) {
    ...
    if (info->cset != my_info.cset)
      return H5_ITER_ERROR;
    if (H5Otoken_cmp(group_id, &info->u.token, &my_info.u.token, &token_cmp) <
        0)
      return H5_ITER_ERROR;
    if (token_cmp)
      return H5_ITER_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_cb(hid_t group_id, const char *link_name,
                            void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_fail_cb(hid_t H5_ATTR_UNUSED group_id,
                                const char H5_ATTR_UNUSED *link_name,
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_check(hid_t group_id, H5_index_t idx_type,
                              H5_iter_order_t order, unsigned max_links,
                              link_iter_info_t *iter_info) {
  unsigned v;   /* Local index variable */
  hsize_t skip; /* # of links to skip in group */
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if (H5Literate2(group_id, idx_type, order, &skip, link_iterate_cb,
                  iter_info) < 0)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Literate2(group_id, idx_type, order, &skip, link_iterate_cb,
                         iter_info)) < 0)
    TEST_ERROR;
  if (ret != CORDER_ITER_STOP)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Giterate(group_id, ".", &gskip, group_iterate_cb, iter_info)) <
      0)
    TEST_ERROR;
  if (ret != CORDER_ITER_STOP)
    TEST_ERROR;
  ...
  skip = 0;
  H5E_BEGIN_TRY {
    ret = H5Literate2(group_id, idx_type, order, &skip, link_iterate_fail_cb,
                      NULL);
  }
  H5E_END_TRY
  if (ret >= 0)
  ...
  for (idx_type = H5_INDEX_NAME; idx_type <= H5_INDEX_CRT_ORDER; idx_type++) {
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        /* Check for iteration on empty group */
        /* (should be OK) */
        if (H5Literate2(group_id, idx_type, order, NULL, link_iterate_cb,
                        NULL) < 0)
          TEST_ERROR;
        ...
        skip = (hsize_t)u;
        H5E_BEGIN_TRY {
          ret = H5Literate2(group_id, idx_type, order, &skip, link_iterate_cb,
                            NULL);
        }
        H5E_END_TRY
        if (ret >= 0)
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_cb(hid_t group_id, const char *link_name,
                               const H5L_info2_t *info, void *_op_data) {
  link_iter_info_t *op_data = (link_iter_info_t *)_op_data; /* User data */
  char objname[NAME_BUF_SIZE];                              /* Object name */
  H5L_info2_t my_info; /* Local link info */
 ...
 *-------------------------------------------------------------------------
 */
static int group_iterate_old_cb(hid_t group_id, const char *link_name,
                                void *_op_data) {
 ...
 *-------------------------------------------------------------------------
 */
static int link_iterate_old_check(hid_t group_id, H5_iter_order_t order,
                                  unsigned max_links,
                                  link_iter_info_t *iter_info) {
  unsigned v;   /* Local index variable */
  hsize_t skip; /* # of links to skip in group */
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if (H5Literate2(group_id, H5_INDEX_NAME, order, &skip, link_iterate_old_cb,
                  iter_info) < 0)
    TEST_ERROR;
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Literate2(group_id, H5_INDEX_NAME, order, &skip,
                         link_iterate_old_cb, iter_info)) < 0)
    TEST_ERROR;
  if (ret != CORDER_ITER_STOP)
    TEST_ERROR;
  if (iter_info->ncalled != 3)
  ...
  iter_info->curr = order != H5_ITER_DEC ? 0 : (max_links - 1);
  memset(iter_info->visited, 0, sizeof(hbool_t) * iter_info->max_visit);
  if ((ret = H5Giterate(group_id, ".", &gskip, group_iterate_old_cb,
                        iter_info)) < 0)
    TEST_ERROR;
  if (ret != CORDER_ITER_STOP)
    TEST_ERROR;
  ...
  skip = 0;
  H5E_BEGIN_TRY {
    ret = H5Literate2(group_id, H5_INDEX_NAME, order, &skip,
                      link_iterate_fail_cb, NULL);
  }
  H5E_END_TRY
  if (ret >= 0)
    TEST_ERROR;
  ...
  skip = 0;
  H5E_BEGIN_TRY {
    ret = H5Literate2((hid_t)(-1), H5_INDEX_NAME, order, &skip,
                      link_iterate_fail_cb, NULL);
  }
  H5E_END_TRY
  if (ret >= 0)
...
static int link_iterate_old(hid_t fapl) {
  ...
  for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
    ...
    /* Check for iteration on empty group */
    /* (should be OK) */
    if (H5Literate2(group_id, H5_INDEX_NAME, order, NULL, link_iterate_old_cb,
                    NULL) < 0)
      TEST_ERROR;
    ...
    skip = (hsize_t)0;
    H5E_BEGIN_TRY {
      ret = H5Literate2(group_id, H5_INDEX_CRT_ORDER, order, &skip,
                        link_iterate_old_cb, NULL);
    }
    H5E_END_TRY
    if (ret >= 0)
 ...
 *-------------------------------------------------------------------------
 */
static int open_by_idx_check(hid_t main_group_id, hid_t soft_group_id,
                             hid_t mount_file_id, H5_index_t idx_type,
                             H5_iter_order_t order, unsigned max_links,
                             H5O_token_t *objno) {
  ...
  H5O_info2_t oi;              /* Buffer for querying object's info */
  H5O_token_t
      mnt_root_token; /* Token (address) of root group in file to mount */
  ...
  for (v = 0; v < 2; v++) {
    ...
    /* Open each object in main group by index and check that it's the correct
     * one */
    for (u = 0; u < max_links; u++) {
      /* Open the object */
      if ((obj_id = H5Oopen_by_idx(group_id, ".", idx_type, order, (hsize_t)u,
                                   H5P_DEFAULT)) < 0)
        TEST_ERROR;
  ...
  mnt_idx = 2;
  HDsnprintf(mntname, sizeof(mntname), "/%s/filler %02u", CORDER_GROUP_NAME,
             mnt_idx);
  if (H5Fmount(main_group_id, mntname, mount_file_id, H5P_DEFAULT) < 0)
    TEST_ERROR;

  /* Open the object that the file is mounted on */
  if ((obj_id = H5Oopen_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC,
                               (hsize_t)mnt_idx, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  /* Check that the object is the root of the mounted file and not in the
   * previous file */
...
static int open_by_idx(hid_t fapl) {
  ...
  if (NULL ==
      (objno = (H5O_token_t *)malloc(sizeof(H5O_token_t) * (max_compact * 2))))
    TEST_ERROR;

  /* Create file to mount */
  h5_fixname(FILENAME[1], fapl, filename, sizeof filename);
  if ((mount_file_id = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) <
      0)
    TEST_ERROR;
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("open object by creation order index in increasing order "
                      "w/creation "
                      "order index");
            else
              TESTING("open object by creation order index in increasing order "
            ...
            if (use_index)
              TESTING("open object by creation order index in decreasing order "
                      "w/creation "
                      "order index");
            else
              TESTING("open object by creation order index in decreasing order "
            ...
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("open object by creation order index in native order "
                      "w/creation order index");
            else
              TESTING("open object by creation order index in native order w/o "
                      "creation order "
                      "index");
          } /* end else */
        }   /* end if */
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("open object by name index in increasing order "
                      "w/creation order index");
            else
              TESTING("open object by name index in increasing order w/o "
                      "creation order index");
          } /* end if */
          else if (order == H5_ITER_DEC) {
            if (use_index)
              TESTING("open object by name index in decreasing order "
                      "w/creation order index");
            else
              TESTING("open object by name index in decreasing order w/o "
                      "creation order index");
          } /* end else */
          else {
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("open object by name index in native order w/creation "
                      "order index");
            else
              TESTING("open object by name index in native order w/o creation "
                      "order index");
          } /* end else */
        }   /* end else */
        ...
        if ((soft_group_id = H5Gcreate2(file_id, CORDER_SOFT_GROUP_NAME,
                                        H5P_DEFAULT, gcpl_id, H5P_DEFAULT)) < 0)
          TEST_ERROR;

        /* Try to open on object in an empty group */
        H5E_BEGIN_TRY {
          ret = H5Oopen_by_idx(group_id, ".", idx_type, order, (hsize_t)0,
                               H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
        ...
        for (u = 0; u < max_compact; u++) {
          ...
          HDsnprintf(valname, sizeof(valname), "/%s/%s", CORDER_GROUP_NAME,
                     objname);
          if (H5Lcreate_soft(valname, soft_group_id, objname, H5P_DEFAULT,
                             H5P_DEFAULT) < 0)
            TEST_ERROR;
        }
        ...
        H5E_BEGIN_TRY {
          ret = H5Oopen_by_idx(group_id, ".", idx_type, order, (hsize_t)u,
                               H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
        ...
        if (open_by_idx_check(group_id, soft_group_id, mount_file_id, idx_type,
                              order, u, objno) < 0)
          TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int open_by_idx_check_old(hid_t main_group_id, hid_t soft_group_id,
                                 hid_t mount_file_id, H5_index_t idx_type,
                                 H5_iter_order_t order, unsigned max_links,
                                 H5O_token_t *objno) {
  char mntname[NAME_BUF_SIZE]; /* Link value */
...
static int open_by_idx_old(hid_t fapl) {
  ...
  for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
    ...
    if (order == H5_ITER_INC) {
      TESTING(
          "open object by name index in increasing order in old-style group");
    } /* end if */
    else if (order == H5_ITER_DEC) {
      TESTING(
          "open object by name index in decreasing order in old-style group");
    } /* end else */
    else {
      assert(order == H5_ITER_NATIVE);
    ...
    /* Try to open on object in an empty group */
    H5E_BEGIN_TRY {
      ret = H5Oopen_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
                           H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    H5E_BEGIN_TRY {
      ret = H5Oopen_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                           H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    H5E_BEGIN_TRY {
      ret = H5Oopen_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, order,
                           (hsize_t)(u - 1), H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
      TEST_ERROR;
    ...
    /* Verify opening objects by index */
    if (open_by_idx_check_old(group_id, soft_group_id, mount_file_id,
                              H5_INDEX_NAME, order, u, objno) < 0)
      TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int object_info_check(hid_t main_group_id, hid_t soft_group_id,
  ...
  for (v = 0; v < 2; v++) {
    ...
    for (u = 0; u < max_links; u++) {
      ...
      if (H5Oget_info_by_name3(group_id, objname, &oinfo,
      ...
      if (H5Oget_info_by_idx3(group_id, ".", idx_type, order, (hsize_t)u,
                              &oinfo, H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS,
                              H5P_DEFAULT) < 0)
        TEST_ERROR;
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("query object info by creation order index in increasing "
                      "order "
                      "w/creation order index");
            else
              TESTING("query object info by creation order index in increasing "
            ...
            if (use_index)
              TESTING("query object info by creation order index in decreasing "
                      "order "
                      "w/creation order index");
            else
              TESTING("query object info by creation order index in decreasing "
            ...
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("query object info by creation order index in native "
                      "order w/creation "
                      "order index");
            else
              TESTING("query object info by creation order index in native "
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("query object info by name index in increasing order "
                      "w/creation order index");
            else
              TESTING("query object info by name index in increasing order w/o "
                      "creation order "
                      "index");
          } /* end if */
          else if (order == H5_ITER_DEC) {
            if (use_index)
              TESTING("query object info by name index in decreasing order "
                      "w/creation order index");
            else
              TESTING("query object info by name index in decreasing order w/o "
                      "creation order "
                      "index");
          } /* end else */
          else {
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("query object info by name index in native order "
                      "w/creation order index");
            else
              TESTING("query object info by name index in native order w/o "
                      "creation order index");
          } /* end else */
        }   /* end else */
        ...
        H5E_BEGIN_TRY {
          ret = H5Oget_info_by_idx3(group_id, ".", H5_INDEX_NAME, order,
                                    (hsize_t)0, &oinfo, H5O_INFO_BASIC,
                                    H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
        ...
        for (u = 0; u < max_compact; u++) {
          ...
          for (v = 0; v < u; v++) {
            ...
            /* Create attribute */
            if ((attr_id = H5Acreate2(group_id2, attrname, H5T_NATIVE_INT,
        ...
        H5E_BEGIN_TRY {
          ret = H5Oget_info_by_idx3(group_id, ".", H5_INDEX_NAME, order,
                                    (hsize_t)u, &oinfo, H5O_INFO_BASIC,
                                    H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
        ...
        if (object_info_check(group_id, soft_group_id, idx_type, order, u,
                              objno) < 0)
          TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int object_info_check_old(hid_t main_group_id, hid_t soft_group_id,
                                 H5_index_t idx_type, H5_iter_order_t order,
                                 unsigned max_links, H5O_token_t *objno) {
  char objname[NAME_BUF_SIZE]; /* Object name */
  hid_t group_id = -1;         /* ID of group to test */
...
static int object_info_old(hid_t fapl) {
  ...
  for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
    ...
    if (order == H5_ITER_INC) {
      ...
      assert(order == H5_ITER_NATIVE);
      TESTING(
          "query object info by name index in native order in old-style group");
    } /* end else */
    ...
    /* Check for out of bound query by index on empty group */
    H5E_BEGIN_TRY {
      ret = H5Oget_info_by_idx3(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
    ...
    for (u = 0; u < CORDER_NLINKS; u++) {
      ...
      for (v = 0; v < u; v++) {
        ...
        /* Create attribute */
        if ((attr_id = H5Acreate2(group_id2, attrname, H5T_NATIVE_INT, space_id,
                                  H5P_DEFAULT, H5P_DEFAULT)) < 0)
          TEST_ERROR;
    ...
    /* Check for out of bound query by index */
    H5E_BEGIN_TRY {
      ret = H5Oget_info_by_idx3(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                                &oinfo, H5O_INFO_BASIC, H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    H5E_BEGIN_TRY {
      ret = H5Oget_info_by_idx3(group_id, ".", H5_INDEX_CRT_ORDER, order,
                                (hsize_t)(u - 1), &oinfo, H5O_INFO_BASIC,
                                H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    /* Verify querying objects by name */
    if (object_info_check_old(group_id, soft_group_id, H5_INDEX_NAME, order, u,
                              objno) < 0)
      TEST_ERROR;
    ...
    for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
      ...
      for (use_index = FALSE; use_index <= TRUE; use_index++) {
        ...
        if (idx_type == H5_INDEX_CRT_ORDER) {
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("query group info by creation order index in increasing "
                      "order w/creation "
                      "order index");
            else
              TESTING("query group info by creation order index in increasing "
            ...
            if (use_index)
              TESTING("query group info by creation order index in decreasing "
                      "order w/creation "
                      "order index");
            else
              TESTING("query group info by creation order index in decreasing "
            ...
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("query group info by creation order index in native "
                      "order w/creation "
                      "order index");
            else
              TESTING("query group info by creation order index in native "
          ...
          if (order == H5_ITER_INC) {
            if (use_index)
              TESTING("query group info by name index in increasing order "
                      "w/creation order index");
            else
              TESTING("query group info by name index in increasing order w/o "
                      "creation order "
                      "index");
          } /* end if */
          else if (order == H5_ITER_DEC) {
            if (use_index)
              TESTING("query group info by name index in decreasing order "
                      "w/creation order index");
            else
              TESTING("query group info by name index in decreasing order w/o "
                      "creation order "
                      "index");
          } /* end else */
          else {
            assert(order == H5_ITER_NATIVE);
            if (use_index)
              TESTING("query group info by name index in native order "
                      "w/creation order index");
            else
              TESTING("query group info by name index in native order w/o "
                      "creation order index");
          } /* end else */
        }   /* end else */
        ...
        /* Check for out of bound query by index on empty group */
        H5E_BEGIN_TRY {
          ret = H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order,
                                   (hsize_t)0, &grp_info, H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
          TEST_ERROR;
        ...
        for (u = 0; u < max_compact; u++) {
          ...
          /* Create hard link, with group object */
          if ((group_id2 = H5Gcreate2(group_id, objname, H5P_DEFAULT, gcpl_id,
                                      H5P_DEFAULT)) < 0)
            TEST_ERROR;
          ...
          for (v = 0; v <= u; v++) {
            ...
            /* Create hard link, with group object */
            if ((group_id3 = H5Gcreate2(group_id2, objname2, H5P_DEFAULT,
                                        H5P_DEFAULT, H5P_DEFAULT)) < 0)
              TEST_ERROR;
          ...
          if (order != H5_ITER_NATIVE) {
            if (order == H5_ITER_INC) {
              if (H5Gget_info_by_idx(group_id, ".", idx_type, order, (hsize_t)u,
                                     &grp_info, H5P_DEFAULT) < 0)
                TEST_ERROR;
            } /* end if */
            else {
              if (H5Gget_info_by_idx(group_id, ".", idx_type, order, (hsize_t)0,
                                     &grp_info, H5P_DEFAULT) < 0)
                TEST_ERROR;
            } /* end else */
          ...
          if (H5Gget_info_by_name(file_id, CORDER_GROUP_NAME, &grp_info,
                                  H5P_DEFAULT) < 0)
            TEST_ERROR;
        ...
        H5E_BEGIN_TRY {
          ret = H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order,
                                   (hsize_t)u, &grp_info, H5P_DEFAULT);
        }
        H5E_END_TRY
        if (ret >= 0)
          TEST_ERROR;
...
static int group_info_old(hid_t fapl) {
  ...
  for (order = H5_ITER_INC; order <= H5_ITER_NATIVE; order++) {
    if (order == H5_ITER_INC) {
      ...
      assert(order == H5_ITER_NATIVE);
      TESTING(
          "query group info by name index in native order in old-style group");
    } /* end else */
    ...
    /* Check for out of bound query by index on empty group */
    H5E_BEGIN_TRY {
      ret = H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)0,
    ...
    for (u = 0; u < CORDER_NLINKS; u++) {
      ...
      if (order != H5_ITER_NATIVE) {
        if (order == H5_ITER_INC) {
          if (H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order,
                                 (hsize_t)u, &grp_info, H5P_DEFAULT) < 0)
            TEST_ERROR;
        } /* end if */
        else {
          if (H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order,
                                 (hsize_t)0, &grp_info, H5P_DEFAULT) < 0)
            TEST_ERROR;
        } /* end else */
    ...
    /* Check for out of bound query by index */
    H5E_BEGIN_TRY {
      ret = H5Gget_info_by_idx(group_id, ".", H5_INDEX_NAME, order, (hsize_t)u,
                               &grp_info, H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
    ...
    H5E_BEGIN_TRY {
      ret = H5Gget_info_by_idx(group_id, ".", H5_INDEX_CRT_ORDER, order,
                               (hsize_t)0, &grp_info, H5P_DEFAULT);
    }
    H5E_END_TRY
    if (ret >= 0)
...
static int timestamps(hid_t fapl) {
  ...
  if ((group_id = H5Gcreate2(file_id, TIMESTAMP_GROUP_1, H5P_DEFAULT, gcpl_id,
                             H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((group_id2 = H5Gcreate2(file_id, TIMESTAMP_GROUP_2, H5P_DEFAULT,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
int main(void) {
  ...
  if (h5_driver_is_default_vfd_compatible(fapl, &driver_is_default_compatible) <
      0)
    TEST_ERROR;
  ...
  for (minimize_dset_oh = 0; minimize_dset_oh <= 1; minimize_dset_oh++) {
    ...
    for (new_format = FALSE; new_format <= TRUE; new_format++) {
      ...
      /* Skip external link tests for splitter VFD, which has external
       * link-related bugs */
      if (HDstrcmp(env_h5_drvr, "splitter")) {
         ...
         */
        nerrors +=
            external_set_elink_acc_flags(env_h5_drvr, my_fapl, new_format) < 0
                ? 1
                : 0;
        ...
        for (efc = FALSE; efc <= TRUE; efc++) {
...
#ifndef H5_NO_DEPRECATED_SYMBOLS
          nerrors +=
              external_link_query_deprec(my_fapl, new_format) < 0 ? 1 : 0;
#endif /* H5_NO_DEPRECATED_SYMBOLS */
          nerrors +=
              external_link_unlink_compact(my_fapl, new_format) < 0 ? 1 : 0;
          nerrors +=
              external_link_unlink_dense(my_fapl, new_format) < 0 ? 1 : 0;
...
#ifndef H5_NO_DEPRECATED_SYMBOLS
          nerrors +=
              external_link_closing_deprec(my_fapl, new_format) < 0 ? 1 : 0;
#endif /* H5_NO_DEPRECATED_SYMBOLS */
          ...
          nerrors +=
              external_link_abs_mainpath(my_fapl, new_format) < 0 ? 1 : 0;
          nerrors +=
              external_link_rel_mainpath(my_fapl, new_format) < 0 ? 1 : 0;
          ...
          if (driver_is_default_compatible) {
            nerrors +=
                external_set_elink_fapl1(my_fapl, new_format) < 0 ? 1 : 0;
          }
...
#endif
          nerrors +=
              external_symlink(env_h5_drvr, my_fapl, new_format) < 0 ? 1 : 0;
          nerrors +=
              external_copy_invalid_object(my_fapl, new_format) < 0 ? 1 : 0;
          nerrors +=
              external_dont_fail_to_source(my_fapl, new_format) < 0 ? 1 : 0;
          nerrors += external_open_twice(my_fapl, new_format) < 0 ? 1 : 0;
          nerrors +=
              external_link_with_committed_datatype(my_fapl, new_format) < 0
                  ? 1
                  : 0;
          nerrors +=
              external_link_public_macros(my_fapl, new_format) < 0 ? 1 : 0;
      ...
      if (new_format == TRUE) {
        nerrors +=
            ud_hard_links(fapl2) < 0 ? 1 : 0; /* requires new format groups */
#ifndef H5_NO_DEPRECATED_SYMBOLS
        nerrors += ud_hard_links_deprec(fapl2) < 0
                       ? 1
                       : 0; /* requires new format groups */
#endif                      /* H5_NO_DEPRECATED_SYMBOLS */
        nerrors += ud_link_reregister(fapl2) < 0
                       ? 1
                       : 0; /* requires new format groups */
#ifndef H5_NO_DEPRECATED_SYMBOLS
        nerrors += ud_link_reregister_deprec(fapl2) < 0
                       ? 1
                       : 0; /* requires new format groups */
#endif                      /* H5_NO_DEPRECATED_SYMBOLS */
      }                     /* end if */
    ...
    /* XXX: when creation order indexing is fully working, go back and add
     * checks to these tests to make certain that the creation order values are
     *      correct.
     */
