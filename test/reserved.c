...
static herr_t rsrv_heap(void) {
  ...
  for (i = 0; i < 200; i++) {
    ...
    H5E_BEGIN_TRY {
      dataset_id = H5Dcreate2(file_id, dset_name, H5T_NATIVE_INT, dataspace_id,
    ...
    H5E_BEGIN_TRY {
      H5Dwrite(dataset_id, H5T_NATIVE_INT, dataspace_id, dataspace_id,
               H5P_DEFAULT, &i);
    }
    H5E_END_TRY
   ...
   * enough address space to hold 200 datasets (or this test needs to be
   * updated!).
   */
  if (i == 200)
    TEST_ERROR;
...
static herr_t rsrv_ohdr(void) {
  ...
  for (i = 0; i < 2000; i++) {
    ...
    H5E_BEGIN_TRY {
      ...
      attr_id = H5Acreate2(dataset_id, attrname, H5T_STD_I32BE, aid,
  ...
  /* The loop should have broken before completing--the file should not have had
   * enough address space to hold 2000 attributes (or this test needs to be
updated
!).
   */
  if (i == 2000)
...
static herr_t rsrv_vlen(void) {
  ...
  /* Create the dataset. */
  dataset_id = H5Dcreate2(file_id, "/dset", type_id, dataspace_id, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  if (dataset_id < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < 2000; i++) {
    ...
    H5E_BEGIN_TRY
    status = H5Dwrite(dataset_id, type_id, mem_space_id, dataspace_id,
...
int main(void) {
  ...
  if (HDstrcmp(envval, "core") && HDstrcmp(envval, "split") &&
      HDstrcmp(envval, "multi") && HDstrcmp(envval, "family")) {
    num_errs += rsrv_ohdr();
    num_errs += rsrv_heap();
    num_errs += rsrv_vlen();
    ...
    HDputs("All address space reservation tests skipped - Incompatible with "
           "current Virtual File Driver");
  }
#endif /* BROKEN */
