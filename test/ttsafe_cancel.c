...
void *tts_cancel_thread(void H5_ATTR_UNUSED *arg) {
  ...
  dataset = H5Dcreate2(cancel_file, DATASETNAME, datatype, dataspace,
  ...
  status = H5Diterate(&buffer, H5T_NATIVE_INT, dataspace, tts_cancel_callback,
                      &dataset);
...
herr_t tts_cancel_callback(void *elem, hid_t H5_ATTR_UNUSED type_id,
...
void cleanup_cancel(void) { HDunlink(FILENAME); }
