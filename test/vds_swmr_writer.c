...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    fprintf(stderr,
            "ERROR: Must pass the source file number on the command line.\n");
    return EXIT_FAILURE;
  }
  ...
  if (H5Pset_libver_bounds(faplid, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) < 0)
    TEST_ERROR;
  if ((fid = H5Fopen(FILE_NAMES[file_number], H5F_ACC_RDWR | H5F_ACC_SWMR_WRITE,
                     faplid)) < 0)
    TEST_ERROR;
