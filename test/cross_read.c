 ...
 *              little-endian machines can be read on the machine running this
 * test.
 */
...
static int check_data_i(const char *dsetname, hid_t fid) {
   ...
   * memory and display.
   */
  if (H5Dread(did, H5T_NATIVE_LLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, data_out) <
      0)
    FAIL_STACK_ERROR;
      ...
      if (data_out[i][j] != data_in[i][j])
        if (!nerrors++) {
          ...
          printf("element [%d][%d] is %lld but should have been %lld\n", (int)i,
                 (int)j, data_out[i][j], data_in[i][j]);
        } /* end if */
...
static int check_data_f(const char *dsetname, hid_t fid) {
   ...
   * memory and display.
   */
  if (H5Dread(did, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data_out) <
      0)
    FAIL_STACK_ERROR;
...
static int check_file(char *filename) {
  const char *pathname =
      H5_get_srcdir_filename(filename); /* Corrected test file name     */
  hid_t fid = -1;                       /* file ID                      */
  int nerrors = 0;                      /* # of datasets with errors    */
#if !defined(H5_HAVE_FILTER_DEFLATE) || !defined(H5_HAVE_FILTER_SZIP)
  const char *not_supported =
      "    filter is not enabled."; /* no filter message            */
#endif
