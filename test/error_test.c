...
#ifdef H5_USE_16_API
int main(void) {
  fprintf(stderr, "Test skipped because backward compatibility with v1.6 is "
                  "configured in\n");
  return 0;
}
#else /* H5_USE_16_API */
...
static herr_t test_error(hid_t file) {
  ...
  /* Create the dataset */
  if ((dataset = H5Dcreate2(file, DSET_NAME, H5T_STD_I32BE, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT)) < 0) {
    H5Epush(H5E_DEFAULT, __FILE__, FUNC_test_error, __LINE__, ERR_CLS,
            ERR_MAJ_IO, ERR_MIN_CREATE, "H5Dcreate2 failed");
    goto error;
  }
  ...
  if (H5Dwrite(FAKE_ID, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               ipoints2) >= 0) {
    H5Epush(H5E_DEFAULT, __FILE__, FUNC_test_error, __LINE__, ERR_CLS,
            ERR_MAJ_IO, ERR_MIN_WRITE, "H5Dwrite shouldn't succeed");
    goto error;
  }
  ...
  if (H5Dwrite(FAKE_ID, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               ipoints2) < 0) {
    H5Epush(H5E_DEFAULT, __FILE__, FUNC_test_error, __LINE__, ERR_CLS,
            ERR_MAJ_IO, ERR_MIN_WRITE, "H5Dwrite failed as supposed to");
...
static herr_t init_error(void) {
  ...
  if ((ERR_MIN_SUBROUTINE =
           H5Ecreate_msg(ERR_CLS, H5E_MINOR, ERR_MIN_SUBROUTINE_MSG)) < 0)
    TEST_ERROR;
  if ((ERR_MIN_ERRSTACK =
           H5Ecreate_msg(ERR_CLS, H5E_MINOR, ERR_MIN_ERRSTACK_MSG)) < 0)
    TEST_ERROR;
  if ((ERR_MIN_CREATE = H5Ecreate_msg(ERR_CLS, H5E_MINOR, ERR_MIN_CREATE_MSG)) <
      0)
    TEST_ERROR;
  if ((ERR_MIN_WRITE = H5Ecreate_msg(ERR_CLS, H5E_MINOR, ERR_MIN_WRITE_MSG)) <
      0)
    TEST_ERROR;
  if ((ERR_MIN_GETNUM = H5Ecreate_msg(ERR_CLS, H5E_MINOR, ERR_MIN_GETNUM_MSG)) <
      0)
    TEST_ERROR;

  if (msg_size !=
      H5Eget_msg(ERR_MIN_SUBROUTINE, &msg_type, msg, (size_t)msg_size) + 1)
    TEST_ERROR;
...
static herr_t error_stack(void) {
  ...
  if ((err_num = H5Eget_num(ERR_STACK)) == 0) {
    H5Epush(ERR_STACK, __FILE__, FUNC_error_stack, __LINE__, ERR_CLS,
            ERR_MAJ_API, ERR_MIN_GETNUM, "Get number test failed, returned %d",
            (int)err_num);
    goto error;
  }
 ...
 *-------------------------------------------------------------------------
 */
static herr_t long_desc_cb(unsigned H5_ATTR_UNUSED n,
                           const H5E_error2_t *err_desc, void *client_data) {
...
static herr_t test_long_desc(void) {
  ...
  if (H5Epush(H5E_DEFAULT, __FILE__, test_FUNC, __LINE__, ERR_CLS, ERR_MAJ_TEST,
              ERR_MIN_SUBROUTINE, format, long_desc) < 0)
    TEST_ERROR;

  /* Create the string that should be in the description. Must use HDsnprintf
   * here because snprintf is _snprintf on Windows
   */
...
static herr_t custom_print_cb(unsigned n, const H5E_error2_t *err_desc,
  ...
  fprintf(stream, "%*serror #%03d: %s in %s(): line %u\n", indent, "", n,
          err_desc->file_name, err_desc->func_name, err_desc->line);
...
static herr_t test_create(void) {
  ...
  /* Push an error with a long description */
  if (H5Epush(estack_id, __FILE__, err_func, __LINE__, ERR_CLS, ERR_MAJ_TEST,
              ERR_MIN_SUBROUTINE, "%s", err_msg) < 0)
    TEST_ERROR;
...
static herr_t test_copy(void) {
  ...
  /* Push an error with a long description */
  if (H5Epush(H5E_DEFAULT, __FILE__, err_func, __LINE__, ERR_CLS, ERR_MAJ_TEST,
              ERR_MIN_SUBROUTINE, "%s", err_msg) < 0)
    TEST_ERROR;
...
static herr_t test_append(void) {
  ...
  const char *err_msg1 =
      "Error message #1"; /* Error message #1 for pushing error */
  const char *err_msg2 =
      "Error message #2"; /* Error message #2 for pushing error */
  ssize_t err_num;        /* Number of errors on stack */
  ...
  /* Push an error */
  if (H5Epush(H5E_DEFAULT, __FILE__, err_func, __LINE__, ERR_CLS, ERR_MAJ_TEST,
              ERR_MIN_SUBROUTINE, "%s", err_msg1) < 0)
    TEST_ERROR;
  ...
  if (H5Epush(estack_id2, __FILE__, err_func, __LINE__, ERR_CLS, ERR_MAJ_IO,
              ERR_MIN_CREATE, "%s", err_msg2) < 0)
    TEST_ERROR;
...
static herr_t test_filter_error(const char *fname, hid_t fapl) {
  const char *pathname =
      H5_get_srcdir_filename(fname); /* Corrected test file name */
  hid_t file = -1;
  hid_t dataset = -1;
  int buf[20];

  fprintf(stderr, "\nTesting error message during data reading when filter "
                  "isn't registered\n");
...
int main(void) {
  ...
  fprintf(stderr, "   This program tests the Error API.  There're supposed to "
  ...
  if (error_stack() < 0) {
    ...
    if (H5Epush(ERR_STACK, __FILE__, FUNC_main, __LINE__, ERR_CLS, ERR_MAJ_TEST,
                ERR_MIN_ERRSTACK, "Error stack test failed") < 0)
      TEST_ERROR;
    ...
    H5Epush(ERR_STACK, __FILE__, FUNC_main, __LINE__, ERR_CLS2, ERR_MAJ_TEST,
            ERR_MIN_ERRSTACK, "Error stack test failed");
  ...
  if (test_error(file) < 0) {
    H5Epush(H5E_DEFAULT, __FILE__, FUNC_main, __LINE__, ERR_CLS, ERR_MAJ_TEST,
            ERR_MIN_SUBROUTINE, "Error test failed, %s", "it's wrong");
    estack_id = H5Eget_current_stack();
