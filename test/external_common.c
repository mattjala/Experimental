...
herr_t reset_raw_data_files(hbool_t is_env) {
  ...
  for (i = 0; i < N_EXT_FILES; i++) {
    ...
    /* Open file */
    if (is_env)
      HDsnprintf(filename, sizeof(filename), "extern_env_%lur.raw",
                 (unsigned long)i + 1);
    else
      HDsnprintf(filename, sizeof(filename), "extern_%lur.raw",
                 (unsigned long)i + 1);
  ...
  for (i = 0; i < N_EXT_FILES; i++) {
    ...
    /* Open file */
    if (is_env)
      HDsnprintf(filename, sizeof(filename), "extern_env_%luw.raw",
                 (unsigned long)i + 1);
    else
      HDsnprintf(filename, sizeof(filename), "extern_%luw.raw",
                 (unsigned long)i + 1);
    if ((fd = HDopen(filename, O_RDWR | O_CREAT | O_TRUNC,
                     H5_POSIX_CREATE_MODE_RW)) < 0)
      goto error;
