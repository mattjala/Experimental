...
int main(void) {
  printf("Test skipped because backward compatibility with v1.6 is NOT "
         "configured in\n");
  return 0;
}
...
static herr_t user_print2(hid_t err_stack, FILE *stream) {
  ...
  fprintf(stderr,
          "\n********* Print error stack in customized way *********\n");
  if (H5Ewalk2(err_stack, H5E_WALK_UPWARD, (H5E_walk2_t)custom_print_cb2,
               stream) < 0)
    TEST_ERROR;
...
static herr_t test_error_compat(void) {
   ...
   * doesn't exist. */
  did = H5Dcreate2(H5I_INVALID_HID, DSET_NAME, H5T_STD_I32BE, sid, H5P_DEFAULT,
  ...
  /* This call should fail because the test mixes H5Eget_auto2 with
   * H5Eset_auto1. Once the H5Eset_auto1 is called with a user-defined printing
   * function, a call to H5Eget_auto2 will fail. But keep in mind the printing
   * function is user_print1. */
  ...
  /* This call should work because the H5Eset_auto2 above set the default
   * printing function to H5Eprint2.  It simply returns user_print1. */
  if ((ret = H5Eget_auto1(&old_func1, &old_data)) < 0)
    TEST_ERROR;
  if (old_data != NULL)
    TEST_ERROR;
  ...
  /* This function changes the new-style printing function back to the default
   * H5Eprint1. */
  if (H5Eset_auto1((H5E_auto1_t)H5Eprint1, NULL) < 0)
    TEST_ERROR;

  /* This call should work because the H5Eset_auto1 above restored the default
   * printing function H5Eprint1.  It simply returns H5Eprint2. */
  if ((ret = H5Eget_auto2(H5E_DEFAULT, &old_func2, &old_data)) < 0)
    TEST_ERROR;
  if (old_data != NULL)
    TEST_ERROR;
...
static herr_t test_h5epush1(hid_t file) {
  ...
  /* Test H5E_BEGIN_TRY */
  H5E_BEGIN_TRY {
    did = H5Dcreate2(H5I_INVALID_HID, DSET_NAME, H5T_STD_I32BE, sid,
                     H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }
  H5E_END_TRY

  /* Create the dataset */
  if ((did = H5Dcreate2(file, DSET_NAME, H5T_STD_I32BE, sid, H5P_DEFAULT,
  ...
  if (H5Dwrite(H5I_INVALID_HID, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               ipoints2) < 0)
...
int main(void) {
  ...
  fprintf(stderr, "   This program tests the Error API compatible with HDF5 "
                  "v1.6.  There are supposed to "
