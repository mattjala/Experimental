 ...
 *with the library on the trunk as of when this file is checked in.
 */
...
int main(void) {
  ...
  /* Create dataset #1 */
  did = H5Dcreate2(fid, "dset1", H5T_NATIVE_UINT, space, H5P_DEFAULT,
  ...
  /* Create dataset #2 */
  did = H5Dcreate2(fid, "dset2", H5T_NATIVE_UINT, space, H5P_DEFAULT,
                   H5P_DEFAULT, H5P_DEFAULT);
  assert(did > 0);
  ret = H5Dwrite(did, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
  assert(ret >= 0);
