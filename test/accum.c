...
#define accum_write(a, s, b)                                                   \
  H5F_block_write(f, H5FD_MEM_DEFAULT, (haddr_t)(a), (size_t)(s), (b))
#define accum_read(a, s, b)                                                    \
  H5F_block_read(f, H5FD_MEM_DEFAULT, (haddr_t)(a), (size_t)(s), (b))
#define accum_free(f, a, s)                                                    \
  H5F__accum_free(f->shared, H5FD_MEM_DEFAULT, (haddr_t)(a), (hsize_t)(s))
...
int main(void) {
  ...
  if (H5FD_set_eoa(f->shared->lf, H5FD_MEM_DEFAULT,
                   (haddr_t)(1024 * 1024 * 10)) < 0)
    FAIL_STACK_ERROR;
 ...
 *
 * Purpose:     Simple test to write to then read from before metadata
 *accumulator.
 *
 * Return:      Success: SUCCEED
 *              Failure: FAIL
 ...
 *
 * Purpose:     Simple test to write to then read from after metadata
 *accumulator.
 *
 * Return:      Success: SUCCEED
 *              Failure: FAIL
...
unsigned test_big(H5F_t *f) {
  ...
  /* Write entire segment to from file */
  /* (With write buffer #2) */
  /* (Partially overwriting dirty region, and not invalidating entire
   * accumulator) */
  if (accum_write(512, BIG_BUF_SIZE, wbuf2) < 0)
    FAIL_STACK_ERROR;
...
unsigned test_swmr_write_big(hbool_t newest_format) {
  ...
  /* Both cases will result in v3 superblock and version 2 object header for
   * SWMR */
  ...
  /* We'll be writing lots of garbage data, so extend the
      file a ways. 10MB should do. */
  if (H5FD_set_eoa(rf->shared->lf, H5FD_MEM_DEFAULT,
                   (haddr_t)(1024 * 1024 * 10)) < 0)
    FAIL_STACK_ERROR;
  ...
  if (H5F_block_write(rf, H5FD_MEM_DEFAULT, (haddr_t)1024, (size_t)1024, wbuf) <
      0)
    FAIL_STACK_ERROR;
  /* Read the data */
  if (H5F_block_read(rf, H5FD_MEM_DEFAULT, (haddr_t)1024, (size_t)1024, rbuf) <
      0)
    FAIL_STACK_ERROR;
  ...
  if (H5F_block_write(rf, H5FD_MEM_DEFAULT, (haddr_t)2048, (size_t)BIG_BUF_SIZE,
                      wbuf2) < 0)
    FAIL_STACK_ERROR;
  /* Read the data */
  if (H5F_block_read(rf, H5FD_MEM_DEFAULT, (haddr_t)2048, (size_t)BIG_BUF_SIZE,
                     rbuf) < 0)
    FAIL_STACK_ERROR;
  /* Verify the data read is correct */
  if (memcmp(wbuf2, rbuf, (size_t)BIG_BUF_SIZE) != 0)
...
#if defined(H5_HAVE_WIN32_API)
  {
    ...
    if (0 == CreateProcess(NULL, SWMR_READER, NULL, NULL, FALSE, 0, NULL, NULL,
                           &si, &pi)) {
...
#else  /* defined(H5_HAVE_WIN32_API) */
  {
    ...
    /* Fork child process to verify that the data at [1024, 2014] does get
     * written to disk */
    if ((pid = fork()) < 0) {
      HDperror("fork");
      FAIL_STACK_ERROR;
