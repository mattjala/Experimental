 ...
 *-------------------------------------------------------------------------
 */
static int create_btree(H5F_t *f, const H5B2_create_t *cparam, H5B2_t **bt2,
                        haddr_t *bt2_addr) {
 ...
 *-------------------------------------------------------------------------
 */
static int reopen_btree(H5F_t *f, H5B2_t **bt2, haddr_t bt2_addr,
 ...
 *-------------------------------------------------------------------------
 */
static int check_node_info(H5B2_t *bt2, hsize_t record,
 ...
 *-------------------------------------------------------------------------
 */
static int modify_cb(void *_record, void *_op_data, hbool_t *changed) {
 ...
 *-------------------------------------------------------------------------
 */
static int modify_rec_cb(void *_record, void *_op_data, hbool_t *changed) {
 ...
 *-------------------------------------------------------------------------
 */
static int no_modify_cb(void H5_ATTR_UNUSED *_record,
                        void H5_ATTR_UNUSED *_op_data, hbool_t *changed) {
  *changed = FALSE;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_insert_basic(hid_t fapl, const H5B2_create_t *cparam,
                                  const bt2_test_param_t *tparam) {
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_insert_split_root(hid_t fapl, const H5B2_create_t *cparam,
                                       const bt2_test_param_t *tparam) {
  hid_t file = -1;      /* File ID */
  H5F_t *f = NULL;      /* Internal file object pointer */
  H5B2_t *bt2 = NULL;   /* v2 B-tree wrapper */
  ...
  idx = 0;
  H5E_BEGIN_TRY {
    ret = H5B2_index(bt2, H5_ITER_INC, (hsize_t)(INSERT_SPLIT_ROOT_NREC + 2),
                     find_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
...
test_insert_level1_2leaf_redistrib(hid_t fapl, const H5B2_create_t *cparam,
  ...
  bt2_stat.depth = 1;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC + (INSERT_SPLIT_ROOT_NREC / 2) + 1);
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = (hsize_t)((INSERT_SPLIT_ROOT_NREC / 2) +
                     (INSERT_SPLIT_ROOT_NREC / 4) + 1);
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_insert_level1_side_split(hid_t fapl,
   ...
   */
  TESTING(
      "B-tree insert: split side leaf into 2 leaves in level 1 B-tree (l->r)");
   ...
   */
  TESTING(
      "B-tree insert: split side leaf into 2 leaves in level 1 B-tree (r->l)");
...
test_insert_level1_3leaf_redistrib(hid_t fapl, const H5B2_create_t *cparam,
  ...
  bt2_stat.depth = 1;
  bt2_stat.nrecords =
      (2 * INSERT_SPLIT_ROOT_NREC) + (INSERT_SPLIT_ROOT_NREC / 2) + 1;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = 52;
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_insert_make_level2(hid_t fapl, const H5B2_create_t *cparam,
                                        const bt2_test_param_t *tparam) {
  hid_t file = -1;      /* File ID */
  H5F_t *f = NULL;      /* Internal file object pointer */
  H5B2_t *bt2 = NULL;   /* v2 B-tree wrapper */
  ...
  idx = 0;
  H5E_BEGIN_TRY {
    ret = H5B2_index(bt2, H5_ITER_INC, (hsize_t)(INSERT_SPLIT_ROOT_NREC * 30),
                     find_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
...
test_insert_level2_leaf_redistrib(hid_t fapl, const H5B2_create_t *cparam,
  ...
  for (; u < ((INSERT_SPLIT_ROOT_NREC * 29) + (INSERT_SPLIT_ROOT_NREC / 2));
       u++) {
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 29) + (INSERT_SPLIT_ROOT_NREC / 2);
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = 1008;
  if (check_node_depth(bt2, &record, (unsigned)2) < 0)
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 29) + (INSERT_SPLIT_ROOT_NREC / 2) + 1;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = 1008;
  if (check_node_depth(bt2, &record, (unsigned)2) < 0)
  ...
  if (check_node_depth(bt2, &record, (unsigned)2) < 0) /* Record in root node */
    TEST_ERROR;
  record = 535;
  if (check_node_depth(bt2, &record, (unsigned)1) <
  ...
  if (check_node_depth(bt2, &record, (unsigned)1) <
    ...
    TEST_ERROR;
  record = 568;
  if (check_node_depth(bt2, &record, (unsigned)0) <
      0) /* Record in leaf node just after insertion point */
    TEST_ERROR;

  /* Add more records to middle leaf, to force a split and a 3 node
   * redistribution on middle leaf */
  for (u = 0; u < (INSERT_SPLIT_ROOT_NREC / 2) + 1; u++) {
    record =
        u + (INSERT_SPLIT_ROOT_NREC * 8) + (INSERT_SPLIT_ROOT_NREC / 2) + 1;
    if (H5B2_insert(bt2, &record) < 0)
      FAIL_STACK_ERROR;
  } /* end for */
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 30) + (INSERT_SPLIT_ROOT_NREC / 2) + 2;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = 1008;
  if (check_node_depth(bt2, &record, (unsigned)2) < 0) /* Record in root node */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_insert_level2_leaf_split(hid_t fapl,
                                              const H5B2_create_t *cparam,
                                              const bt2_test_param_t *tparam) {
  hid_t file = -1;      /* File ID */
  H5F_t *f = NULL;      /* Internal file object pointer */
  ...
  for (u = 0; u < ((INSERT_SPLIT_ROOT_NREC / 2) + 1); u++) {
    record =
        u + (INSERT_SPLIT_ROOT_NREC * 29) + (INSERT_SPLIT_ROOT_NREC / 2) + 2;
    if (H5B2_insert(bt2, &record) < 0)
      FAIL_STACK_ERROR;
  } /* end for */
  ...
  /* Add another record to left-most leaf, to force a 1->2 node split on left
   * leaf */
  record = 0;
  if (H5B2_insert(bt2, &record) < 0)
    FAIL_STACK_ERROR;
...
test_insert_level2_2internal_split(hid_t fapl, const H5B2_create_t *cparam,
   ...
   */
  TESTING(
      "B-tree insert: split side internal node to 2 in level 2 B-tree (r->l)");
  ...
  for (u = 0; u < (INSERT_SPLIT_ROOT_NREC * 59); u++) {
    record =
        u + (INSERT_SPLIT_ROOT_NREC * 14) - (INSERT_SPLIT_ROOT_NREC / 4) + 3;
    if (H5B2_insert(bt2, &record) < 0)
      FAIL_STACK_ERROR;
  } /* end for */
  ...
  TESTING(
      "B-tree insert: split side internal node to 2 in level 2 B-tree (l->2)");
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 73) - (INSERT_SPLIT_ROOT_NREC / 4) + 4;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
...
test_insert_level2_3internal_redistrib(hid_t fapl, const H5B2_create_t *cparam,
  ...
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  record =
      3199; /* Record to right of insertion point in middle internal node */
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  ...
  for (u = 0; u < ((INSERT_SPLIT_ROOT_NREC * 13) +
                   ((3 * INSERT_SPLIT_ROOT_NREC) / 4) + 2);
       u++) {
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 72) + ((3 * INSERT_SPLIT_ROOT_NREC) / 4) + 3;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = 1889; /* Left record in root node */
  if (check_node_depth(bt2, &record, (unsigned)2) < 0)
  ...
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  record =
      3137; /* Record to right of insertion point in middle internal node */
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 72) + ((3 * INSERT_SPLIT_ROOT_NREC) / 4) + 4;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
...
#ifdef NONE
  record =
      2862; /* Record to left of insertion point in right internal node (now) */
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
#endif           /* NONE */
  record = 3137; /* Record to right of insertion point in right internal node
                    (now) */
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  record = 3135; /* Record just above insertion point in leaf node */
...
test_insert_level2_3internal_split(hid_t fapl, const H5B2_create_t *cparam,
  ...
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  record =
      2884; /* Record to right of insertion point in middle internal node */
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 87) + ((3 * INSERT_SPLIT_ROOT_NREC) / 4) + 2;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = 1889; /* Left record in root node */
  if (check_node_depth(bt2, &record, (unsigned)2) < 0)
  ...
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  record =
      2822; /* Record to right of insertion point in middle internal node */
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  ...
  bt2_stat.depth = 2;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC * 87) + ((3 * INSERT_SPLIT_ROOT_NREC) / 4) + 3;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record = 1889; /* Left record in root node */
  if (check_node_depth(bt2, &record, (unsigned)2) < 0)
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_insert_lots(hid_t fapl, const H5B2_create_t *cparam,
                                 const bt2_test_param_t *tparam) {
  hid_t file = -1;      /* File ID */
  char filename[1024];  /* Filename to use */
  H5F_t *f = NULL;      /* Internal file object pointer */
...
fprintf(stderr,"curr_time=%lu\n",(unsigned long)curr_time);
  ...
  /* Attempt to index non-existent record in level-4 B-tree, in increasing &
   * decreasing order */
  H5E_BEGIN_TRY {
    ret =
        H5B2_index(bt2, H5_ITER_INC, (hsize_t)(INSERT_MANY * 3), find_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
  if (ret != FAIL)
  ...
  H5E_BEGIN_TRY {
    ret =
        H5B2_index(bt2, H5_ITER_DEC, (hsize_t)(INSERT_MANY * 3), find_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
  if (ret != FAIL)
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_update_basic(hid_t fapl, const H5B2_create_t *cparam,
                                  const bt2_test_param_t *tparam) {
  hid_t file = -1;        /* File ID */
  H5F_t *f = NULL;        /* Internal file object pointer */
  H5B2_t *bt2 = NULL;     /* v2 B-tree wrapper */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_update_split_root(hid_t fapl, const H5B2_create_t *cparam,
                                       const bt2_test_param_t *tparam) {
  hid_t file = -1;        /* File ID */
  H5F_t *f = NULL;        /* Internal file object pointer */
  H5B2_t *bt2 = NULL;     /* v2 B-tree wrapper */
  ...
  H5E_BEGIN_TRY {
    ret =
        H5B2_index(bt2, H5_ITER_INC, (hsize_t)(INSERT_SPLIT_ROOT_NREC_REC + 2),
                   index_rec_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
...
test_update_level1_2leaf_redistrib(hid_t fapl, const H5B2_create_t *cparam,
  ...
  bt2_stat.depth = 1;
  bt2_stat.nrecords =
      (INSERT_SPLIT_ROOT_NREC_REC + (INSERT_SPLIT_ROOT_NREC_REC / 2) + 1);
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record.key =
      (INSERT_SPLIT_ROOT_NREC_REC / 2) + (INSERT_SPLIT_ROOT_NREC_REC / 4);
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
  ...
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record.key =
      (INSERT_SPLIT_ROOT_NREC_REC / 2) + (INSERT_SPLIT_ROOT_NREC_REC / 4) - 1;
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_update_level1_side_split(hid_t fapl,
                                              const H5B2_create_t *cparam,
                                              const bt2_test_param_t *tparam) {
  hid_t file = -1;        /* File ID */
  H5F_t *f = NULL;        /* Internal file object pointer */
   ...
   */
  TESTING(
      "B-tree update: split side leaf into 2 leaves in level 1 B-tree (l->r)");
   ...
   */
  TESTING(
      "B-tree update: split side leaf into 2 leaves in level 1 B-tree (r->l)");
...
test_update_level1_3leaf_redistrib(hid_t fapl, const H5B2_create_t *cparam,
  ...
  for (u = 0; u < INSERT_SPLIT_ROOT_NREC_REC; u++) {
    record.key =
        u + (INSERT_SPLIT_ROOT_NREC_REC + (INSERT_SPLIT_ROOT_NREC_REC / 2) + 1);
  ...
  bt2_stat.depth = 1;
  bt2_stat.nrecords =
      (2 * INSERT_SPLIT_ROOT_NREC_REC) + (INSERT_SPLIT_ROOT_NREC_REC / 2) + 1;
  if (check_stats(bt2, &bt2_stat) < 0)
    TEST_ERROR;
  record.key = 52;
  if (check_node_depth(bt2, &record, (unsigned)1) < 0)
  ...
  /* Make certain that the index is correct */
  if (idx.key !=
      ((INSERT_SPLIT_ROOT_NREC_REC * 2) + (INSERT_SPLIT_ROOT_NREC_REC / 2) + 1))
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_update_make_level2(hid_t fapl, const H5B2_create_t *cparam,
                                        const bt2_test_param_t *tparam) {
  hid_t file = -1;        /* File ID */
  H5F_t *f = NULL;        /* Internal file object pointer */
  H5B2_t *bt2 = NULL;     /* v2 B-tree wrapper */
  ...
  H5E_BEGIN_TRY {
    ret =
        H5B2_index(bt2, H5_ITER_INC, (hsize_t)(INSERT_SPLIT_ROOT_NREC_REC * 42),
                   index_rec_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
  ...
  for (; u < (INSERT_SPLIT_ROOT_NREC_REC * 41); u++) {
    record.key = ((INSERT_SPLIT_ROOT_NREC_REC * 41) + 1) -
                 (u + 2); /* Leave a gap for later insertion */
    record.val = record.key * 2;
    if (H5B2_update(bt2, &record, no_modify_cb, NULL) < 0)
      FAIL_STACK_ERROR;
  } /* end for */
  ...
  /* Add some extra records to middle leaf */
  record.key =
      ((INSERT_SPLIT_ROOT_NREC_REC * 41) - (INSERT_SPLIT_ROOT_NREC_REC * 9));
  record.val = record.key * 2;
  if (H5B2_update(bt2, &record, no_modify_cb, NULL) < 0)
    FAIL_STACK_ERROR;
  record.key =
  ...
  for (; u < (INSERT_SPLIT_ROOT_NREC_REC * 41); u++) {
    record.key = u - ((INSERT_SPLIT_ROOT_NREC_REC * 9) -
                      2); /* Leave a gap for later insertion */
    record.val = record.key * 2;
    if (H5B2_update(bt2, &record, no_modify_cb, NULL) < 0)
      FAIL_STACK_ERROR;
  } /* end for */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_update_lots(hid_t fapl, const H5B2_create_t *cparam,
                                 const bt2_test_param_t *tparam) {
  hid_t file = -1;    /* File ID */
  H5F_t *f = NULL;    /* Internal file object pointer */
  H5B2_t *bt2 = NULL; /* v2 B-tree wrapper */
...
fprintf(stderr, "curr_time = %lu\n", (unsigned long)curr_time);
  ...
  /* Allocate space for the records */
  if (NULL == (records = (H5B2_test_rec_t *)malloc(sizeof(H5B2_test_rec_t) *
                                                   INSERT_MANY_REC)))
    TEST_ERROR;
   ...
   * decreasing order */
  H5E_BEGIN_TRY {
    ret = H5B2_index(bt2, H5_ITER_INC, (hsize_t)(INSERT_MANY_REC * 3),
                     find_rec_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
  if (ret != FAIL)
    TEST_ERROR;
  H5E_BEGIN_TRY {
    ret = H5B2_index(bt2, H5_ITER_DEC, (hsize_t)(INSERT_MANY_REC * 3),
                     find_rec_cb, NULL);
  }
  H5E_END_TRY
  /* Should fail */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_basic(hid_t fapl, const H5B2_create_t *cparam,
                                  const bt2_test_param_t *tparam) {
  hid_t file = -1;    /* File ID */
  H5F_t *f = NULL;    /* Internal file object pointer */
  H5B2_t *bt2 = NULL; /* v2 B-tree wrapper */
  ...
  /* Attempt to remove a non-existent record from a level-0 B-tree with mult.
   * record */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_noredistrib(hid_t fapl,
                                               const H5B2_create_t *cparam,
                                               const bt2_test_param_t *tparam) {
  hid_t file = -1;    /* File ID */
  H5F_t *f = NULL;    /* Internal file object pointer */
  ...
  /* Attempt to remove a record from right leaf of a level-1 B-tree with
   * noredistribution */
  ...
  /* Attempt to remove a record from left leaf of a level-1 B-tree with
   * noredistribution */
  ...
  /* Attempt to remove a record from middle leaf of a level-1 B-tree with
   * noredistribution */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_redistrib(hid_t fapl,
                                             const H5B2_create_t *cparam,
                                             const bt2_test_param_t *tparam) {
  hid_t file = -1;    /* File ID */
  H5F_t *f = NULL;    /* Internal file object pointer */
  ...
  /* Attempt to remove enough records from right leaf of a level-1 B-tree to
  ...
  /* Attempt to remove enough records from left leaf of a level-1 B-tree to
   * force redistribution */
  ...
  /* Attempt to remove enough records from middle leaf of a level-1 B-tree to
   * force redistribution */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_2leaf_merge(hid_t fapl,
                                               const H5B2_create_t *cparam,
                                               const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_3leaf_merge(hid_t fapl,
                                               const H5B2_create_t *cparam,
                                               const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_promote(hid_t fapl,
                                           const H5B2_create_t *cparam,
                                           const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  ...
  /* Attempt to remove record from root node of a level-1 B-tree to force
   * promotion from right leaf */
  record = 220;
  rrecord = HSIZET_MAX;
  if (H5B2_remove(bt2, &record, remove_cb, &rrecord) < 0)
    FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_promote_2leaf_redistrib(
    hid_t fapl, const H5B2_create_t *cparam, const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  H5B2_t *bt2 = NULL;          /* v2 B-tree wrapper */
  haddr_t bt2_addr;            /* Address of B-tree created */
  ...
  TESTING(
      "B-tree remove: promote from leaf of level-1 B-tree w/2 node redistrib");
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_promote_3leaf_redistrib(
    hid_t fapl, const H5B2_create_t *cparam, const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  H5B2_t *bt2 = NULL;          /* v2 B-tree wrapper */
  ...
  TESTING(
      "B-tree remove: promote from leaf of level-1 B-tree w/3 node redistrib");
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level1_collapse(hid_t fapl,
                                            const H5B2_create_t *cparam,
                                            const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level2_promote(hid_t fapl,
                                           const H5B2_create_t *cparam,
                                           const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  ...
  /* Attempt to remove record from right internal node of a level-2 B-tree to
   * force promotion */
  record = 2960;
  rrecord = HSIZET_MAX;
  if (H5B2_remove(bt2, &record, remove_cb, &rrecord) < 0)
  ...
  /* Attempt to remove record from left internal node of a level-2 B-tree to
   * force promotion */
  ...
  /* Attempt to remove record from middle internal node of a level-2 B-tree to
   * force promotion */
  ...
  /* Attempt to remove record from root node of a level-2 B-tree to force
   * promotion */
...
static unsigned test_remove_level2_promote_2internal_redistrib(
  ...
  /* Attempt to remove record from right internal node of a level-2 B-tree to
   * force promotion w/redistribution */
  for (u = 0; u < 8; u++) {
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level2_promote_3internal_redistrib(
    hid_t fapl, const H5B2_create_t *cparam, const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  H5B2_t *bt2 = NULL;          /* v2 B-tree wrapper */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level2_promote_2internal_merge(
    hid_t fapl, const H5B2_create_t *cparam, const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  H5B2_t *bt2 = NULL;          /* v2 B-tree wrapper */
  ...
  TESTING(
      "B-tree remove: promote from right internal of level-2 B-tree w/merge");
...
static unsigned test_remove_level2_promote_3internal_merge(
  ...
  TESTING(
      "B-tree remove: promote from middle internal of level-2 B-tree w/merge");
...
test_remove_level2_2internal_merge_left(hid_t fapl, const H5B2_create_t *cparam,
  ...
  TESTING(
      "B-tree remove: merge 2 internal nodes to 1 in level-2 B-tree (l->r)");
  ...
  /* Attempt to remove records from a level-2 B-tree to force 2 internal nodes
   * to merge */
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_level2_2internal_merge_right(
    hid_t fapl, const H5B2_create_t *cparam, const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  H5B2_t *bt2 = NULL;          /* v2 B-tree wrapper */
  ...
  TESTING(
      "B-tree remove: merge 2 internal nodes to 1 in level-2 B-tree (r->l)");
 ...
 *-------------------------------------------------------------------------
 */
static unsigned gen_l4_btree2(const char *filename, hid_t fapl,
                              const H5B2_create_t *cparam, haddr_t *bt2_addr,
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_remove_lots(const char *env_h5_drvr, hid_t fapl,
                                 const H5B2_create_t *cparam) {
  hid_t file = -1;        /* File ID */
  char filename[1024];    /* Filename to use */
  H5F_t *f = NULL;        /* Internal file object pointer */
...
fprintf(stderr, "curr_time = %lu\n", (unsigned long)curr_time);
  ...
  single_file_vfd = !h5_driver_uses_multiple_files(
      env_h5_drvr, H5_EXCLUDE_NON_MULTIPART_DRIVERS);
  if (single_file_vfd) {
  ...
  if (single_file_vfd) {
    ...
    /* Open the file just created */
    if ((fd = HDopen(filename, O_RDWR | O_CREAT | O_TRUNC,
  ...
  for (u = 0; u < INSERT_MANY; u++) {
    ...
    if (H5B2_remove_by_idx(bt2, H5_ITER_INC, (hsize_t)rem_idx, remove_cb,
                           &rrecord) < 0)
      FAIL_STACK_ERROR;
  ...
  for (u = 0; u < INSERT_MANY; u++) {
    /* Remove first record */
    rrecord = HSIZET_MAX;
    if (H5B2_remove_by_idx(bt2, H5_ITER_INC, (hsize_t)0, remove_cb, &rrecord) <
        0)
      FAIL_STACK_ERROR;
  ...
  for (u = 0; u < INSERT_MANY; u++) {
    /* Remove last record */
    rrecord = HSIZET_MAX;
    if (H5B2_remove_by_idx(bt2, H5_ITER_DEC, (hsize_t)0, remove_cb, &rrecord) <
        0)
      FAIL_STACK_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_find_neighbor(hid_t fapl, const H5B2_create_t *cparam,
                                   const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  H5B2_t *bt2 = NULL;          /* v2 B-tree wrapper */
  ...
  H5E_BEGIN_TRY {
    ret =
        H5B2_neighbor(bt2, H5B2_COMPARE_GREATER, &search, neighbor_cb, &record);
  }
  H5E_END_TRY
  /* Should fail */
  if (ret != FAIL)
  ...
  search = 0;
  if (H5B2_neighbor(bt2, H5B2_COMPARE_GREATER, &search, neighbor_cb, &record) <
      0)
    FAIL_STACK_ERROR;
  if (record != 2)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static unsigned test_modify(hid_t fapl, const H5B2_create_t *cparam,
                            const bt2_test_param_t *tparam) {
  hid_t file = -1;             /* File ID */
  H5F_t *f = NULL;             /* Internal file object pointer */
  H5B2_t *bt2 = NULL;          /* v2 B-tree wrapper */
...
int main(void) {
  ...
  for (reopen = FALSE; reopen <= TRUE; reopen++) {
    ...
    /* Querying the number of records routine also tested in these routines as
     * well */
    ...
    nerrors +=
        test_remove_level1_promote_2leaf_redistrib(fapl, &cparam, &tparam);
    nerrors +=
        test_remove_level1_promote_3leaf_redistrib(fapl, &cparam, &tparam);
    ...
    nerrors +=
        test_remove_level2_promote_2internal_redistrib(fapl, &cparam, &tparam);
    nerrors +=
        test_remove_level2_promote_3internal_redistrib(fapl, &cparam, &tparam);
    nerrors +=
        test_remove_level2_promote_2internal_merge(fapl, &cparam, &tparam);
    nerrors +=
        test_remove_level2_promote_3internal_merge(fapl, &cparam, &tparam);
