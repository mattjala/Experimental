...
/* Initialize source buffer of integer for integer->integer and
 * integer->floating-point conversion test. This algorithm is mainly to avoid
 * any casting and comparison between source and destination types
 * for compiler, because we're testing conversions. */
#define INIT_INTEGER(TYPE, SRC_MAX, SRC_MIN, SRC_SIZE, DST_SIZE, SRC_PREC,     \
                     BUF, SAVED, NELMTS)                                       \
  do {                                                                         \
    ...
    saved_p = SAVED;                                                           \
                                                                               \
    /*positive values, ascending order. VALUE1 starts from 00000001, to        \
     * 00000010, until 10000000*/                                              \
    /*VALUE2 ascends from 00000000, to 00000011, 00000111,...,  until          \
     * 11111111.*/                                                             \
    for (n = 0; n < SRC_PREC; n++) {                                           \
      ...
      } else if (n == SRC_PREC - 2) { /*to avoid overflow of negative values   \
                                         for signed integer*/                  \
        value1 = (TYPE)(value1 << 1);                                          \
    ...
    /* VALUE2 descends from 11111111 to 11111110, 11111100, ..., until         \
     * 10000000. */                                                            \
...
/* Change a buffer's byte order from big endian to little endian.  It's mainly
 * for library's bit operations which handle only little endian order.
 */
 ...
 */
#define INIT_FP_NORM(TYPE, SRC_MAX, SRC_MIN, SRC_MAX_10_EXP, SRC_MIN_10_EXP,   \
  ...
  do {                                                                         \
    ...
    TYPE multiply;                                                             \
                                                                               \
    /*Determine the number of normalized values and increment pace.  The       \
     *values start from minimal normalized value and are multiplied by                                \
     *MULTIPLY each step until reach to maximal normalized value.*/                                                           \
    ...
    /*Reduce the number of values by 2^factor. MULTIPLY=10^(2^factor). Using   \
     *this algorithm instead of arithmetic operation to avoid any conversion*/                     \
    num_norm >>= factor;                                                       \
                                                                               \
 ...
 * It's for conversion test of floating-point as the source.
 */
#define INIT_FP_DENORM(TYPE, SRC_MANT_DIG, SRC_SIZE, SRC_PREC, SRC_ORDR,       \
  ...
  do {                                                                         \
    ...
    saved_p = SAVED;                                                           \
                                                                               \
    /*Denormalized values. Exponent is 0. Let mantissa starts from 00000001,   \
     *00000011, 00000111,..., until 11111111.*/                                               \
    ...
    for (n = 0; n < SRC_MANT_DIG - 1; n++) {                                   \
      ...
      CHANGE_ORDER(tmp1, SRC_ORDR,                                             \
                   SRC_SIZE); /*change back the order for bit operation*/      \
      buf_p += SRC_SIZE;                                                       \
      saved_p += SRC_SIZE;                                                     \
                                                                               \
      /*negative values*/                                                      \
...
/* Allocate buffer and initialize it with floating-point special values, +/-0,
 * +/-infinity,
  ...
  do {                                                                         \
    ...
    for (n = 0; n < 2; n++) {                                                  \
      if (n == 1) {                                                            \
        ...
        CHANGE_ORDER(value, SRC_ORDR,                                          \
                     SRC_SIZE); /*change order for big endian*/                \
        memcpy(buf_p, value, SRC_SIZE * sizeof(unsigned char));                \
        CHANGE_ORDER(value, SRC_ORDR,                                          \
                     SRC_SIZE); /*change back the order for bit operation*/    \
        buf_p += SRC_SIZE;                                                     \
                                                                               ...
                                                                               \
      /* +/-infinity */                                                        \
      H5T__bit_set(value, (size_t)(SRC_MANT_DIG - 1), SRC_PREC - SRC_MANT_DIG, \
                   TRUE);                                                      \
...
static hbool_t overflows(unsigned char *origin_bits, hid_t src_id,
                         size_t dst_num_bits);
static int my_isnan(dtype_t type, void *val);
static int my_isinf(int endian, const unsigned char *val, size_t size,
...
static H5T_conv_ret_t except_func(H5T_conv_except_t except_type,
                                  ...
                                  void H5_ATTR_UNUSED *src_buf, void *dst_buf,
                                  void *user_data) {
...
static int test_particular_fp_integer(void) {
  ...
  saved_buf1 =
      (unsigned char *)calloc((size_t)1, (size_t)MAX(src_size1, dst_size1));

  memcpy(buf1, &src_d, src_size1);
  memcpy(saved_buf1, &src_d, src_size1);

  /* Register exception handling function and signal the destination is "signed
   * char". */
  flag = 1;
  ...
  /* Do conversion */
  if (H5Tconvert(H5T_NATIVE_DOUBLE, H5T_NATIVE_SCHAR, (size_t)1, buf1, NULL,
                 dxpl_id) < 0) {
    H5_FAILED();
  ...
  saved_buf2 =
      (unsigned char *)calloc((size_t)1, (size_t)MAX(src_size2, dst_size2));
  ...
  /* Do conversion */
  if (H5Tconvert(H5T_NATIVE_FLOAT, H5T_NATIVE_INT, (size_t)1, buf2, NULL,
                 dxpl_id) < 0) {
    H5_FAILED();
    printf("Can't convert data\n");
    goto error;
...
static int test_derived_flt(void) {
   ...
   *-----------------------------------------------------------------------*/
  if (H5Tset_fields(tid1, (size_t)44, (size_t)34, (size_t)10, (size_t)3,
                    (size_t)31) < 0) {
    H5_FAILED();
  ...
  if (H5Tcommit2(file, "new float type 1", tid1, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0) {
    H5_FAILED();
  ...
  for (i = 0; i < nelmts; i++) {
    ...
    if (0 == fails_this_test++) {
      HDsnprintf(str, sizeof(str),
                 "\nTesting random sw derived floating-point -> derived "
   ...
   *--------------------------------------------------------------------------*/
  if (H5Tset_fields(tid2, (size_t)23, (size_t)16, (size_t)7, (size_t)0,
                    (size_t)16) < 0) {
    H5_FAILED();
    printf("Can't set fields\n");
    goto error;
  ...
  if (H5Tcommit2(file, "new float type 2", tid2, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0) {
    H5_FAILED();
    printf("Can't set inpad\n");
    goto error;
  ...
  for (i = 0; i < nelmts; i++) {
     ...
     */
    if ((buf[i * src_size + 2] == 0x7f &&
         saved_buf[i * src_size + 2] == 0x7f) ||
...
static int test_derived_integer(void) {
  ...
  if (H5Tcommit2(file, "new integer type 1", tid1, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0) {
    H5_FAILED();
  ...
  if (H5Tcommit2(file, "new integer type 2", tid2, H5P_DEFAULT, H5P_DEFAULT,
                 H5P_DEFAULT) < 0) {
    H5_FAILED();
    printf("Can't commit data type\n");
    goto error;
...
static int test_conv_int_1(const char *name, hid_t src, hid_t dst) {
  ...
  if (OTHER == src_type || OTHER == dst_type) {
    HDsnprintf(str, sizeof(str), "Testing %s %s -> %s conversions", name,
  ...
  /* Allocate and initialize the source buffer through macro INIT_INTEGER.  The
   * BUF will be used for the conversion while the SAVED buffer will be used for
  ...
  if (src_type == INT_SCHAR) {
    ...
    INIT_INTEGER(unsigned char, UCHAR_MAX, 0, src_size, dst_size, src_nbits,
                 buf, saved, nelmts);
  } else if (src_type == INT_SHORT) {
    INIT_INTEGER(short, SHRT_MAX, SHRT_MIN, src_size, dst_size, src_nbits, buf,
                 saved, nelmts);
  } else if (src_type == INT_USHORT) {
    INIT_INTEGER(unsigned short, USHRT_MAX, 0, src_size, dst_size, src_nbits,
                 buf, saved, nelmts);
  } else if (src_type == INT_INT) {
    INIT_INTEGER(int, INT_MAX, INT_MIN, src_size, dst_size, src_nbits, buf,
                 saved, nelmts);
  } else if (src_type == INT_UINT) {
    INIT_INTEGER(unsigned int, UINT_MAX, 0, src_size, dst_size, src_nbits, buf,
                 saved, nelmts);
  } else if (src_type == INT_LONG) {
    INIT_INTEGER(long, LONG_MAX, LONG_MIN, src_size, dst_size, src_nbits, buf,
                 saved, nelmts);
  } else if (src_type == INT_ULONG) {
    INIT_INTEGER(unsigned long, ULONG_MAX, 0, src_size, dst_size, src_nbits,
                 buf, saved, nelmts);
  } else if (src_type == INT_LLONG) {
    INIT_INTEGER(long long, LLONG_MAX, LLONG_MIN, src_size, dst_size, src_nbits,
                 buf, saved, nelmts);
  ...
  for (j = 0; j < nelmts; j++) {
    if (INT_SCHAR == dst_type) {
      ...
      case INT_SCHAR:
        ...
        break;
      case INT_UCHAR:
        memcpy(aligned, saved + j * sizeof(unsigned char),
      ...
      case INT_SHORT:
        ...
        break;
      case INT_USHORT:
        memcpy(aligned, saved + j * sizeof(unsigned short),
      ...
      case INT_LONG:
        ...
        break;
      case INT_ULONG:
        memcpy(aligned, saved + j * sizeof(unsigned long),
      ...
      case INT_LLONG:
        ...
        break;
      case INT_ULLONG:
        memcpy(aligned, saved + j * sizeof(unsigned long long),
    ...
    for (k = 0; k < src_size; k++)
      src_bits[src_size - (k + 1)] =
          saved[j * src_size + ENDIAN(src_size, k, endian)];

    for (k = 0; k < dst_size; k++)
      dst_bits[dst_size - (k + 1)] =
          buf[j * dst_size + ENDIAN(dst_size, k, endian)];
    ...
    if (H5T_SGN_2 == src_sign && H5T_SGN_2 == dst_sign) {
      if (src_nbits > dst_nbits) {
        if (0 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1) &&
          ...
          if (0 == H5T__bit_get_d(dst_bits, dst_nbits - 1, (size_t)1) &&
              H5T__bit_find(dst_bits, (size_t)0, dst_nbits - 1, H5T_BIT_LSB,
                            0) < 0)
            continue; /*no error*/
        } else if (1 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1) &&
                   H5T__bit_find(src_bits, (size_t)0, src_nbits - 1,
                                 H5T_BIT_MSB, 0) +
                           1 >=
                       (ssize_t)dst_nbits) {
          /*
        ...
        if (0 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1)) {
          ...
          if (0 == H5T__bit_get_d(dst_bits, src_nbits - 1, (size_t)1) &&
              H5T__bit_find(dst_bits, src_nbits, dst_nbits - src_nbits,
                            H5T_BIT_LSB, 1) < 0)
            continue; /*no error*/
        } else {
          /*
      ...
      if (src_nbits >= dst_nbits &&
         ...
         */
        if (0 == H5T__bit_get_d(dst_bits, dst_nbits - 1, (size_t)1) &&
            H5T__bit_find(dst_bits, (size_t)0, dst_nbits - 1, H5T_BIT_LSB, 0) <
                0)
          continue; /*no error*/
      }
    } else {
      if (src_nbits > dst_nbits &&
          H5T__bit_find(src_bits, dst_nbits, src_nbits - dst_nbits, H5T_BIT_LSB,
                        1) >= 0) {
        /*
    ...
    if (++fails_all_tests >= max_fails) {
      HDputs("    maximum failures reached, aborting test...");
      HDputs("    (dst is library's conversion output. ans is compiler's "
             "conversion output.)");
      goto done;
    }
  }
...
static int test_conv_int_2(void) {
  ...
  for (i = 1; i <= TMP_BUF_DIM1; i++) {
    for (j = 1; j <= TMP_BUF_DIM1; j++) {
       ...
       */
      H5Tconvert(src_type, dst_type, (size_t)TMP_BUF_DIM2, buf, NULL,
                 H5P_DEFAULT);
      H5Tclose(src_type);
      H5Tclose(dst_type);
    }
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_flt_1(const char *name, int run_test, hid_t src,
                           hid_t dst) {
  dtype_t src_type, dst_type;       /*data types        */
  size_t nelmts = 0;                /*num values per test    */
  const size_t max_fails = 8;       /*max number of failures*/
  ...
  if ((child_pid = fork()) < 0) {
    ...
    if (WIFEXITED(status) && 255 == WEXITSTATUS(status)) {
      ...
      HDsnprintf(str, sizeof(str), "   Child caught signal %d.",
                 WTERMSIG(status));
      HDputs(str);
  ...
  if (OTHER == src_type || OTHER == dst_type) {
    if (!HDstrcmp(name, "noop"))
      ...
      HDsnprintf(str, sizeof(str),
                 "Testing %s denormalized %s -> %s conversions", name,
                 src_type_name, dst_type_name);
  ...
  /* Allocate and initialize the source buffer through macro INIT_FP_NORM or
   * INIT_FP_SPECIAL. The BUF will be used for the conversion while the SAVED
   * buffer will be used for the comparison later.  INIT_FP_NORM will fill in
   * the buffer with regular values like normalized and denormalized values;
   * INIT_FP_SPECIAL will fill with special values like infinity, NaN.
   */
  switch (run_test) {
  case TEST_NOOP:
  case TEST_NORMAL:
    if (src_type == FLT_FLOAT) {
      ...
      INIT_FP_NORM(long double, LDBL_MAX, LDBL_MIN, LDBL_MAX_10_EXP,
                   LDBL_MIN_10_EXP, src_size, dst_size, buf, saved, nelmts);
#endif
    } else
      goto error;
  ...
  case TEST_DENORM:
    if (src_type == FLT_FLOAT) {
      INIT_FP_DENORM(float, FLT_MANT_DIG, src_size, src_nbits, sendian,
                     dst_size, buf, saved, nelmts);
    } else if (src_type == FLT_DOUBLE) {
      INIT_FP_DENORM(double, DBL_MANT_DIG, src_size, src_nbits, sendian,
                     dst_size, buf, saved, nelmts);
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
    } else if (src_type == FLT_LDOUBLE) {
      INIT_FP_DENORM(long double, LDBL_MANT_DIG, src_size, src_nbits, sendian,
                     dst_size, buf, saved, nelmts);
#endif
    } else
      goto error;
  ...
  case TEST_SPECIAL:
    if (src_type == FLT_FLOAT) {
      INIT_FP_SPECIAL(src_size, src_nbits, sendian, FLT_MANT_DIG, dst_size, buf,
                      saved, nelmts);
    } else if (src_type == FLT_DOUBLE) {
      INIT_FP_SPECIAL(src_size, src_nbits, sendian, DBL_MANT_DIG, dst_size, buf,
                      saved, nelmts);
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
    } else if (src_type == FLT_LDOUBLE) {
  ...
  for (j = 0; j < nelmts; j++) {
     ...
     */
    if (FLT_FLOAT == dst_type && my_isnan(dst_type, buf + j * sizeof(float)) &&
     ...
     */
    {
      ...
      if (FLT_FLOAT == dst_type) {
        ...
        if (overflow && my_isinf(dendian, buf + j * sizeof(float), dst_size,
        ...
        if (underflow && HDfabs(x) <= DBL_MIN && HDfabs(hw_d) <= DBL_MIN)
          continue; /* all underflowed, no error */
        if (overflow && my_isinf(dendian, buf + j * sizeof(double), dst_size,
                                 dst_mpos, dst_msize, dst_epos, dst_esize))
          continue; /* all overflowed, no error */
      ...
      if (check_expo[0] < (-(int)dst_ebias) ||
          check_expo[1] < (-(int)dst_ebias)) {
        int expo_diff = check_expo[0] - check_expo[1];
        int valid_bits = (int)((dst_ebias + dst_msize) +
                               (size_t)MIN(check_expo[0], check_expo[1])) -
                         1;
        double epsilon = 1.0;
 ...
 *-------------------------------------------------------------------------
 */
static int test_conv_int_fp(const char *name, int run_test, hid_t src,
                            hid_t dst) {
  hid_t dxpl_id;      /*dataset transfer property list*/
  int fill_value = 9; /*fill value for conversion exception*/
  H5T_conv_except_func_t
      op;          /*returned callback function for conversion exception*/
  ...
  if ((INT_SCHAR == src_type || INT_UCHAR == src_type ||
       INT_SHORT == src_type || INT_USHORT == src_type || INT_INT == src_type ||
       INT_UINT == src_type || INT_LONG == src_type || INT_ULONG == src_type ||
  ...
  if ((FLT_FLOAT == src_type || FLT_DOUBLE == src_type ||
      ...
      (INT_SCHAR != dst_type && INT_UCHAR != dst_type &&
       INT_SHORT != dst_type && INT_USHORT != dst_type && INT_INT != dst_type &&
       INT_UINT != dst_type && INT_LONG != dst_type && INT_ULONG != dst_type &&
  ...
  /* Allocate and initialize the source buffer through macro INIT_INTEGER if the
   * source is integer, INIT_FP_NORM if floating-point.  The BUF will be used
   * for the conversion while the SAVED buffer will be used for the comparison
   * later.
   */
  if (src_type == INT_SCHAR) {
    INIT_INTEGER(signed char, SCHAR_MAX, SCHAR_MIN, src_size, dst_size,
  ...
  for (j = 0; j < nelmts; j++) {
    ...
    if ((FLT_FLOAT == src_type || FLT_DOUBLE == src_type) &&
      ...
      for (k = 0; k < src_size; k++)
        src_bits[src_size - (k + 1)] =
            saved[j * src_size + ENDIAN(src_size, k, sendian)];
    }

    for (k = 0; k < dst_size; k++)
      dst_bits[dst_size - (k + 1)] =
          buf[j * dst_size + ENDIAN(dst_size, k, dendian)];
    ...
    if ((FLT_FLOAT == src_type || FLT_DOUBLE == src_type ||
         FLT_LDOUBLE == src_type) &&
        (INT_SCHAR == dst_type || INT_SHORT == dst_type ||
         INT_INT == dst_type || INT_LONG == dst_type ||
         INT_LLONG == dst_type)) {
      if (0 == H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1) &&
    ...
    if ((FLT_FLOAT == src_type || FLT_DOUBLE == src_type ||
         FLT_LDOUBLE == src_type) &&
        (INT_UCHAR == dst_type || INT_USHORT == dst_type ||
         INT_UINT == dst_type || INT_ULONG == dst_type ||
         INT_ULLONG == dst_type)) {
      if (H5T__bit_get_d(src_bits, src_nbits - 1, (size_t)1)) {
        /*
         * The source is negative so the result should be zero.
    ...
    case INT_LLONG:
      ...
      break;
    case INT_ULLONG:
      memcpy(aligned, buf + j * sizeof(unsigned long long),
             sizeof(unsigned long long));
      fprintf(stdout, " %29llu\n", *((unsigned long long *)aligned));
      break;
    case FLT_FLOAT:
   ...
   * if it is denormalized or special floating values, treat the failure as
   * warning.*/
  if (run_test == TEST_NORMAL)
    return (int)fails_all_tests;
  else if (run_test == TEST_DENORM || run_test == TEST_SPECIAL)
...
static hbool_t overflows(unsigned char *origin_bits, hid_t src_id,
  ...
  } else if (H5T_NORM_NONE == norm &&
             H5T__bit_find(bits, mpos, mant_digits - 1, H5T_BIT_LSB, TRUE) <
                 0 &&
  ...
  /* shift significand */
  H5T__bit_shift(mant_bits, (ssize_t)(expt - expt_digits), (size_t)0,
                 (size_t)(32 * 8));
...
static int run_fp_tests(const char *name) {
  ...
  if (!HDstrcmp(name, "noop")) {
    nerrors +=
        test_conv_flt_1("noop", TEST_NOOP, H5T_NATIVE_FLOAT, H5T_NATIVE_FLOAT);
    nerrors += test_conv_flt_1("noop", TEST_NOOP, H5T_NATIVE_DOUBLE,
                               H5T_NATIVE_DOUBLE);
    nerrors += test_conv_flt_1("noop", TEST_NOOP, H5T_NATIVE_LDOUBLE,
                               H5T_NATIVE_LDOUBLE);
    goto done;
  }
  ...
  nerrors +=
      test_conv_flt_1(name, TEST_NORMAL, H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE);
  nerrors +=
      test_conv_flt_1(name, TEST_NORMAL, H5T_NATIVE_DOUBLE, H5T_NATIVE_FLOAT);
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
  nerrors +=
      test_conv_flt_1(name, TEST_NORMAL, H5T_NATIVE_FLOAT, H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_flt_1(name, TEST_NORMAL, H5T_NATIVE_DOUBLE, H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_flt_1(name, TEST_NORMAL, H5T_NATIVE_LDOUBLE, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_flt_1(name, TEST_NORMAL, H5T_NATIVE_LDOUBLE, H5T_NATIVE_DOUBLE);
#endif

  /*Test denormalized values.  TEST_DENORM indicates denormalized values.*/
  nerrors +=
      test_conv_flt_1(name, TEST_DENORM, H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE);
  nerrors +=
      test_conv_flt_1(name, TEST_DENORM, H5T_NATIVE_DOUBLE, H5T_NATIVE_FLOAT);
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
  nerrors +=
      test_conv_flt_1(name, TEST_DENORM, H5T_NATIVE_FLOAT, H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_flt_1(name, TEST_DENORM, H5T_NATIVE_DOUBLE, H5T_NATIVE_LDOUBLE);
#ifndef H5_DISABLE_SOME_LDOUBLE_CONV
  nerrors +=
      test_conv_flt_1(name, TEST_DENORM, H5T_NATIVE_LDOUBLE, H5T_NATIVE_FLOAT);
#else
  {
    char str[256]; /*string        */
    ...
    SKIPPED();
    HDputs(
        "    Test skipped due to the conversion problem on IBM ppc64le cpu.");
  }
#endif

  nerrors +=
      test_conv_flt_1(name, TEST_DENORM, H5T_NATIVE_LDOUBLE, H5T_NATIVE_DOUBLE);
#endif

  /*Test special values, +/-0, +/-infinity, +/-QNaN, +/-SNaN.*/
  nerrors +=
      test_conv_flt_1(name, TEST_SPECIAL, H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE);
  nerrors +=
      test_conv_flt_1(name, TEST_SPECIAL, H5T_NATIVE_DOUBLE, H5T_NATIVE_FLOAT);
#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
  nerrors +=
      test_conv_flt_1(name, TEST_SPECIAL, H5T_NATIVE_FLOAT, H5T_NATIVE_LDOUBLE);
  nerrors += test_conv_flt_1(name, TEST_SPECIAL, H5T_NATIVE_DOUBLE,
                             H5T_NATIVE_LDOUBLE);
#ifndef H5_DISABLE_SOME_LDOUBLE_CONV
  nerrors +=
      test_conv_flt_1(name, TEST_SPECIAL, H5T_NATIVE_LDOUBLE, H5T_NATIVE_FLOAT);
  nerrors += test_conv_flt_1(name, TEST_SPECIAL, H5T_NATIVE_LDOUBLE,
                             H5T_NATIVE_DOUBLE);
#else
  {
    char str[256]; /*string        */
...
static int run_int_fp_conv(const char *name) {
  ...
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_SCHAR, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_SCHAR, H5T_NATIVE_DOUBLE);

  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_UCHAR, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_UCHAR, H5T_NATIVE_DOUBLE);

  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_SHORT, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_SHORT, H5T_NATIVE_DOUBLE);

  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_USHORT, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_USHORT, H5T_NATIVE_DOUBLE);

  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_INT, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_INT, H5T_NATIVE_DOUBLE);

  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_UINT, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_UINT, H5T_NATIVE_DOUBLE);

#if H5_SIZEOF_LONG != H5_SIZEOF_INT
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_LONG, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_LONG, H5T_NATIVE_DOUBLE);

  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_ULONG, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_ULONG, H5T_NATIVE_DOUBLE);
#endif

#if H5_SIZEOF_LONG_LONG != H5_SIZEOF_LONG
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_LLONG, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_LLONG, H5T_NATIVE_DOUBLE);

  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_ULLONG, H5T_NATIVE_FLOAT);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_ULLONG, H5T_NATIVE_DOUBLE);
#endif

#if H5_SIZEOF_LONG_DOUBLE != H5_SIZEOF_DOUBLE
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_SCHAR, H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_UCHAR, H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_SHORT, H5T_NATIVE_LDOUBLE);
  nerrors += test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_USHORT,
                              H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_INT, H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_UINT, H5T_NATIVE_LDOUBLE);
#if H5_SIZEOF_LONG != H5_SIZEOF_INT
#if !defined(H5_LONG_TO_LDOUBLE_SPECIAL) &&                                    \
    !defined(H5_DISABLE_SOME_LDOUBLE_CONV)
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_LONG, H5T_NATIVE_LDOUBLE);
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_ULONG, H5T_NATIVE_LDOUBLE);
#else
  {
    char str[256]; /*string        */
    ...
    printf("%-70s", str);
    SKIPPED();
    HDputs("    Test skipped due to the special algorithm of hardware "
           "conversion.");
  }
#endif
  ...
  nerrors +=
      test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_LLONG, H5T_NATIVE_LDOUBLE);
...
#if H5_LLONG_TO_LDOUBLE_CORRECT
  nerrors += test_conv_int_fp(name, TEST_NORMAL, H5T_NATIVE_ULLONG,
                              H5T_NATIVE_LDOUBLE);
#else  /* H5_LLONG_TO_LDOUBLE_CORRECT */
  {
    char str[256]; /*hello string        */
...
static int run_fp_int_conv(const char *name) {
  ...
  for (test_values = TEST_NORMAL; test_values <= TEST_SPECIAL; test_values++) {
    ...
    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT, H5T_NATIVE_SCHAR);
    nerrors += test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE,
                                H5T_NATIVE_SCHAR);

    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT, H5T_NATIVE_UCHAR);
    nerrors += test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE,
                                H5T_NATIVE_UCHAR);

    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT, H5T_NATIVE_SHORT);
    nerrors += test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE,
                                H5T_NATIVE_SHORT);

    nerrors += test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT,
                                H5T_NATIVE_USHORT);
    nerrors += test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE,
                                H5T_NATIVE_USHORT);

    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT, H5T_NATIVE_INT);
    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE, H5T_NATIVE_INT);

    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT, H5T_NATIVE_UINT);
    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE, H5T_NATIVE_UINT);

#if H5_SIZEOF_LONG != H5_SIZEOF_INT
    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT, H5T_NATIVE_LONG);
    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE, H5T_NATIVE_LONG);

    nerrors +=
        test_conv_int_fp(name, test_values, H5T_NATIVE_FLOAT, H5T_NATIVE_ULONG);
    nerrors += test_conv_int_fp(name, test_values, H5T_NATIVE_DOUBLE,
                                H5T_NATIVE_ULONG);
#endif
