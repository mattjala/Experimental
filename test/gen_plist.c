...
static int encode_plist(hid_t plist_id, int little_endian, int word_length,
...
int main(void) {
  ...
  H5AC_cache_image_config_t my_cache_image_config = {
      H5AC__CURR_CACHE_IMAGE_CONFIG_VERSION, TRUE, FALSE, -1};
  ...
  /* Explicitly initialize the library, since we are including the private
   * header file */
  H5open();
  ...
  if ((ret = encode_plist(dcpl1, little_endian, word_length,
                          "testfiles/plist_files/def_dcpl_")) < 0)
    assert(ret > 0);
  ...
  ret = H5Tconvert(H5T_NATIVE_INT, H5T_STD_I32BE, (size_t)1, &fill, NULL,
                   H5P_DEFAULT);
  assert(ret >= 0);
  ...
  if ((ret = encode_plist(dapl1, little_endian, word_length,
                          "testfiles/plist_files/def_dapl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(dxpl1, little_endian, word_length,
                          "testfiles/plist_files/def_dxpl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = H5Pset_dxpl_mpio(dxpl1, H5FD_MPIO_COLLECTIVE)) < 0)
    assert(ret > 0);
  if ((ret = H5Pset_dxpl_mpio_collective_opt(dxpl1, H5FD_MPIO_INDIVIDUAL_IO)) <
      0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(gcpl1, little_endian, word_length,
                          "testfiles/plist_files/def_gcpl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = H5Pset_link_creation_order(
           gcpl1, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED))) < 0)
    assert(ret > 0);

  if ((ret = encode_plist(gcpl1, little_endian, word_length,
                          "testfiles/plist_files/gcpl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(lcpl1, little_endian, word_length,
                          "testfiles/plist_files/def_lcpl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(ocpypl1, little_endian, word_length,
                          "testfiles/plist_files/def_ocpypl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(ocpypl1, little_endian, word_length,
                          "testfiles/plist_files/ocpypl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(ocpl1, little_endian, word_length,
                          "testfiles/plist_files/def_ocpl_")) < 0)
    assert(ret > 0);

  if ((ret = H5Pset_attr_creation_order(
           ocpl1, (H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED))) < 0)
    assert(ret > 0);

  if ((ret = H5Pset_attr_phase_change(ocpl1, 110, 105)) < 0)
    assert(ret > 0);

  if ((ret = H5Pset_filter(ocpl1, H5Z_FILTER_FLETCHER32, 0, (size_t)0, NULL)) <
      0)
    assert(ret > 0);

  if ((ret = encode_plist(ocpl1, little_endian, word_length,
                          "testfiles/plist_files/ocpl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(lapl1, little_endian, word_length,
                          "testfiles/plist_files/def_lapl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(fapl1, little_endian, word_length,
                          "testfiles/plist_files/def_fapl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret = H5Pset_small_data_block_size(fapl1, 2048)) < 0)
    assert(ret > 0);
  if ((ret = H5Pset_libver_bounds(fapl1, H5F_LIBVER_LATEST,
                                  H5F_LIBVER_LATEST)) < 0)
    assert(ret > 0);
  ...
  if ((ret = encode_plist(fcpl1, little_endian, word_length,
                          "testfiles/plist_files/def_fcpl_")) < 0)
    assert(ret > 0);
  ...
  if ((ret =
           H5Pset_shared_mesg_index(fcpl1, 1, H5O_SHMESG_SDSPACE_FLAG, 32) < 0))
    assert(ret > 0);
  ...
  if ((ret = H5Pset_file_space_strategy(fcpl1, H5F_FSPACE_STRATEGY_PAGE, TRUE,
                                        (hsize_t)1)) < 0)
    assert(ret > 0);
  ...
  ret = encode_plist(strcpl1, little_endian, word_length,
                     "testfiles/plist_files/def_strcpl_");
  assert(ret > 0);
  ...
  ret = encode_plist(acpl1, little_endian, word_length,
                     "testfiles/plist_files/def_acpl_");
  assert(ret > 0);
...
static int encode_plist(hid_t plist_id, int little_endian, int word_length,
  ...
  /* Generate filename */
  if ((ret = HDsnprintf(filename, sizeof(filename), "%s%d%s", filename_prefix,
                        word_length, little_endian ? "le" : "be")) < 0)
    assert(ret > 0);
