...
static int
splitter_create_single_file_at(const char *filename, hid_t fapl_id,
...
static herr_t run_ctl_test(uint64_t op_code, uint64_t flags,
                           ctl_test_opc_type opc_type, hid_t fapl_id);
static H5FD_t *H5FD__ctl_test_vfd_open(const char *name, unsigned flags,
...
static herr_t H5FD__ctl_test_vfd_set_eoa(H5FD_t *_file, H5FD_mem_t type,
                                         haddr_t addr);
static haddr_t H5FD__ctl_test_vfd_get_eof(const H5FD_t *_file, H5FD_mem_t type);
static herr_t H5FD__ctl_test_vfd_read(H5FD_t *_file, H5FD_mem_t type,
                                      hid_t dxpl_id, haddr_t addr, size_t size,
                                      void *buf);
static herr_t H5FD__ctl_test_vfd_write(H5FD_t *_file, H5FD_mem_t type,
                                       hid_t dxpl_id, haddr_t addr, size_t size,
                                       const void *buf);
static herr_t H5FD__ctl_test_vfd_ctl(H5FD_t *_file, uint64_t op_code,
...
static herr_t test_sec2(void) {
  ...
  if (driver_flags !=
      (H5FD_FEAT_AGGREGATE_METADATA | H5FD_FEAT_ACCUMULATE_METADATA |
       H5FD_FEAT_DATA_SIEVE | H5FD_FEAT_AGGREGATE_SMALLDATA |
       H5FD_FEAT_POSIX_COMPAT_HANDLE | H5FD_FEAT_SUPPORTS_SWMR_IO |
  ...
  if (os_file_handle == NULL)
    FAIL_PUTS_ERROR(
        "NULL os-specific vfd/file handle was returned from H5Fget_vfd_handle");
...
static herr_t test_core(void) {
   ...
   */
  if (H5Pget_core_write_tracking(fapl_id, &use_write_tracking,
                                 &write_tracking_page_size) < 0)
    TEST_ERROR;
  ...
  if (TRUE != use_write_tracking)
    FAIL_PUTS_ERROR("write tracking flag incorrect in fapl obtained from "
                    "H5Fget_access_plist");
  if (CORE_PAGE_SIZE != write_tracking_page_size)
    FAIL_PUTS_ERROR("write tracking page size incorrect in fapl obtained from "
                    "H5Fget_access_plist");
  ...
  /* Create the dataset */
  if ((did = H5Dcreate2(fid, CORE_DSET_NAME, H5T_NATIVE_INT, sid, H5P_DEFAULT,
  ...
  if (status > 0)
    FAIL_PUTS_ERROR(
        "core VFD dataset created in file when backing store disabled");
  ...
  /* Try to delete the file with the backing store off (shouldn't delete
   * anything) */
  if (H5Pset_fapl_core(fapl_id, (size_t)CORE_INCREMENT, FALSE) < 0)
    TEST_ERROR;
...
static herr_t test_direct(void) {
  ...
  /* Allocate aligned memory for data set 1. For data set 1, everything is
   * aligned including memory address, size of data, and file address. */
  if (0 != posix_memalign(&proto_points, (size_t)FBSIZE,
                          (size_t)(DSET1_DIM1 * DSET1_DIM2 * sizeof(int))))
    TEST_ERROR;
  points = proto_points;
  if (0 != posix_memalign(&proto_check, (size_t)FBSIZE,
                          (size_t)(DSET1_DIM1 * DSET1_DIM2 * sizeof(int))))
    TEST_ERROR;
  check = proto_check;
  ...
  /* Create the dset1 */
  if ((dset1 = H5Dcreate2(file, DSET1_NAME, H5T_NATIVE_INT, space1, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Write the data to the dset1 */
  if (H5Dwrite(dset1, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, points) <
      0)
    TEST_ERROR;
  ...
  /* Create the data space2. For data set 2, memory address and data size are
   * not aligned. */
  dims2[0] = DSET2_DIM;
  ...
  /* Create the dset2 */
  if ((dset2 = H5Dcreate2(file, DSET2_NAME, H5T_NATIVE_INT, space2, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Write the data to the dset1 */
  if (H5Dwrite(dset2, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wdata2) <
      0)
    TEST_ERROR;
...
static herr_t test_family(void) {
  ...
  /* Set up data array */
  if (NULL ==
      (buf_data = (int *)calloc(FAMILY_NUMBER * FAMILY_SIZE, sizeof(int))))
    TEST_ERROR;
  ...
  if ((dset = H5Dcreate2(file, dname, H5T_NATIVE_INT, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if (H5Dwrite(dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, buf_data) <
      0)
    TEST_ERROR;
...
/*
 *      This pragma only needs to surround the snprintf() calls with
 *      'newname_individual', etc. in the code below, but early (4.4.7, at
 * least) gcc only allows diagnostic pragmas to be toggled outside of functions.
 */
H5_GCC_CLANG_DIAG_OFF("format-nonliteral")
static herr_t test_family_compat(void) {
  ...
  /* The following code makes the copies of the family files in the source
   * directory. Since we're going to open the files with write mode, this
   * protects the original files.
   */
  HDsnprintf(newname_individual, sizeof(newname_individual), newname, counter);
  HDsnprintf(pathname_individual, sizeof(pathname_individual), pathname,
             counter);

  while (h5_make_local_copy(pathname_individual, newname_individual) >= 0) {
    counter++;
    HDsnprintf(newname_individual, sizeof(newname_individual), newname,
               counter);
    HDsnprintf(pathname_individual, sizeof(pathname_individual), pathname,
               counter);
  } /* end while */
...
static herr_t test_family_member_fapl(void) {
  ...
  if ((file = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, fapl_id)) ==
      H5I_INVALID_HID)
    TEST_ERROR;
...
static herr_t test_multi(void) {
  ...
  if (driver_flags != (H5FD_FEAT_DATA_SIEVE | H5FD_FEAT_AGGREGATE_SMALLDATA |
                       H5FD_FEAT_USE_ALLOC_SIZE | H5FD_FEAT_PAGED_AGGR))
    TEST_ERROR;
  ...
  if ((attr = H5Acreate2(root, "Metadata", atype, aspace, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_multi_compat(void) {
   ...
   */
  HDsnprintf(filename_s, sizeof(filename_s), "%s-%c.h5", MULTI_COMPAT_BASENAME,
             's');
  HDsnprintf(newname_s, sizeof(newname_s), "%s-%c.h5", FILENAME[9], 's');
  h5_make_local_copy(filename_s, newname_s);

  HDsnprintf(filename_r, sizeof(filename_r), "%s-%c.h5", MULTI_COMPAT_BASENAME,
             'r');
  ...
  if ((dset = H5Dcreate2(file, DSET3_NAME, H5T_NATIVE_INT, space, H5P_DEFAULT,
                         H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_log(void) {
  ...
  H5E_BEGIN_TRY { ret = H5Pset_fapl_log(H5I_INVALID_HID, LOG_FILENAME, 0, 0); }
  H5E_END_TRY
  if (SUCCEED == ret)
    TEST_ERROR;
 ...
 *-------------------------------------------------------------------------
 */
static int compare_splitter_config_info(hid_t fapl_id,
                                        H5FD_splitter_vfd_config_t *info) {
  int ret_value = 0;
...
static int run_splitter_test(const struct splitter_dataset_def *data,
  ...
  if (H5Dwrite(dset_id, data->mem_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               data->buf) < 0) {
...
static int splitter_RO_test(const struct splitter_dataset_def *data,
  ...
  if (file_id >= 0) {
    SPLITTER_TEST_FAULT(
        "R/O open on nonexistent files unexpectedly successful\n");
  }
  ...
  if (splitter_create_single_file_at(vfd_config->wo_path,
                                     vfd_config->wo_fapl_id, data) < 0) {
  ...
  if (file_id >= 0) {
    SPLITTER_TEST_FAULT(
        "R/O open with extant W/O file unexpectedly successful\n");
  }
  ...
  if (splitter_create_single_file_at(filename_rw, vfd_config->rw_fapl_id,
                                     data) < 0) {
  ...
  if (file_id >= 0) {
    SPLITTER_TEST_FAULT(
        "R/O open with extant R/W file unexpectedly successful\n");
  }
...
static int splitter_prepare_file_paths(H5FD_splitter_vfd_config_t *vfd_config,
   ...
   */
  h5_fixname(FILENAME[11], vfd_config->rw_fapl_id, filename_rw_out,
             H5FD_SPLITTER_PATH_MAX);
  h5_fixname(FILENAME[12], vfd_config->wo_fapl_id, vfd_config->wo_path,
             H5FD_SPLITTER_PATH_MAX);
  h5_fixname_no_suffix(FILENAME[13], vfd_config->wo_fapl_id,
                       vfd_config->log_file_path, H5FD_SPLITTER_PATH_MAX);
 ...
 *              and write data as defined in `data` in a pre-determined location
 *in the file.
 *
...
splitter_compare_expected_data(hid_t file_id,
  ...
  if (sizeof((void *)buf) != sizeof(data->buf)) {
    SPLITTER_TEST_FAULT(
        "invariant size of expected data does not match that received!\n");
  }
  ...
  if (H5Dread(dset_id, data->mem_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              (void *)buf) < 0) {
...
static int splitter_tentative_open_test(hid_t child_fapl_id) {
  ...
  if (file_id != H5I_INVALID_HID) {
    SPLITTER_TEST_FAULT(
        "open with both nonexistent files unexpectedly succeeded\n");
  }
  ...
  if (file_id != H5I_INVALID_HID) {
    SPLITTER_TEST_FAULT(
        "open with nonexistent R/W file unexpectedly succeeded\n");
  }
  if (file_exists(filename_rw, child_fapl_id)) {
    SPLITTER_TEST_FAULT("R/W file unexpectedly created\n");
  }
...
static herr_t test_splitter(void) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (run_splitter_test(&data, ignore_wo_errors, provide_logfile_path,
                            child_fapl_ids) < 0) {
        TEST_ERROR;
      }
 ...
 */
static H5FD_t *H5FD__ctl_test_vfd_open(const char H5_ATTR_UNUSED *name,
...
static haddr_t H5FD__ctl_test_vfd_get_eoa(const H5FD_t H5_ATTR_UNUSED *file,
                                          H5FD_mem_t H5_ATTR_UNUSED type) {
  return HADDR_UNDEF;
}
static herr_t H5FD__ctl_test_vfd_set_eoa(H5FD_t H5_ATTR_UNUSED *_file,
                                         H5FD_mem_t H5_ATTR_UNUSED type,
                                         haddr_t H5_ATTR_UNUSED addr) {
  return FAIL;
}
static haddr_t H5FD__ctl_test_vfd_get_eof(const H5FD_t H5_ATTR_UNUSED *file,
                                          H5FD_mem_t H5_ATTR_UNUSED type) {
  return HADDR_UNDEF;
}
...
static herr_t H5FD__ctl_test_vfd_ctl(H5FD_t H5_ATTR_UNUSED *_file,
                                     uint64_t op_code, uint64_t flags,
...
static herr_t run_ctl_test(uint64_t op_code, uint64_t flags,
  ...
  is_passthrough_vfd =
      ((driver_id == H5FD_SPLITTER) || (driver_id == H5FD_MULTI));
  ...
  if (NULL ==
      (file_drv_ptr = H5FDopen(filename, H5F_ACC_RDWR, fapl_id, HADDR_UNDEF)))
  ...
  if (is_passthrough_vfd) {
     ...
     */
    expect_fail = expect_fail || ((CTL_OPC_KNOWN_TERMINAL == opc_type) &&
                                  !routing_flag_set && fail_if_unknown);
  }
...
error:
  H5E_BEGIN_TRY { H5FDclose(file_drv_ptr); }
  H5E_END_TRY
...
static herr_t test_ctl(void) {
  ...
  TESTING_2("known op code to terminal VFD (without fail on unknown flag/route "
            "to terminal VFD)");
  ...
  TESTING_2("known op code to terminal VFD (with fail on unknown flag/route to "
            "terminal VFD)");
  ...
  TESTING_2("unknown op code to terminal VFD (without fail on unknown "
            "flag/route to terminal VFD)");
  ...
  TESTING_2("unknown op code to terminal VFD (with fail on unknown flag/route "
            "to terminal VFD)");
  ...
  h5_fixname(FILENAME[15], splitter_config->wo_fapl_id,
             splitter_config->wo_path, H5FD_SPLITTER_PATH_MAX);
  ...
  TESTING_2("known op code through passthrough VFD to terminal VFD (without "
            "fail on unknown flag/no "
            "routing flag)");
  ...
  TESTING_2("known op code through passthrough VFD to terminal VFD (with fail "
            "on unknown flag/no routing flag)");
  ...
  TESTING_2(
      "unknown op code to passthrough VFD (without fail on unknown flag)");
  ...
  TESTING_2("unknown op code to passthrough VFD (without fail on unknown "
            "flag/route to terminal VFD)");
  ...
  TESTING_2("unknown op code to passthrough VFD (with fail on unknown "
            "flag/route to terminal VFD)");
...
static hbool_t test_vector_io__setup_v(uint32_t count, H5FD_mem_t types[],
...
static hbool_t test_vector_io__setup_fixed_size_v(
...
static hbool_t test_vector_io__read_v_indiv(H5FD_t *lf, uint32_t count,
                                            H5FD_mem_t types[], haddr_t addrs[],
                                            size_t sizes[], void *read_bufs[]) {
...
static hbool_t test_vector_io__write_v_indiv(H5FD_t *lf, uint32_t count,
                                             H5FD_mem_t types[],
                                             haddr_t addrs[], size_t sizes[],
                                             const void *write_bufs[]) {
  hbool_t size_fixed = FALSE;
...
static hbool_t test_vector_io__verify_v(uint32_t count, H5FD_mem_t types[],
                                        size_t sizes[],
                                        const void *write_bufs[],
      ...
      if (w_buf[j] != r_buf[j]) {
        ...
        if (verbose) {
          ...
          fprintf(stdout,
                  ...
                  (long long unsigned)size, w_buf[j], r_buf[j],
                  mem_type_names[type]);
        }
      }
      j++;
...
static void test_vector_io__dump_test_vectors(
    uint32_t count, H5FD_mem_t types[], haddr_t addrs[], size_t sizes[],
...
static herr_t test_vector_io(const char *vfd_name) {
  ...
  HDsnprintf(test_title, sizeof(test_title), "vector I/O with %s VFD",
             vfd_name);
   ...
   */
  if (!(test_vector_io__setup_v(count, types_0, addrs_0, sizes_0, write_bufs_0,
                                read_bufs_0, 'a') &&
        test_vector_io__setup_v(count, types_1, addrs_1, sizes_1, write_bufs_1,
                                read_bufs_1, 'e') &&
        test_vector_io__setup_v(count, types_2, addrs_2, sizes_2, write_bufs_2,
                                read_bufs_2, 'A')))
    TEST_ERROR;

  if (!(test_vector_io__setup_fixed_size_v(count, f_types_0, f_addrs_0,
                                           f_sizes_0, f_write_bufs_0,
                                           f_read_bufs_0, 'b') &&
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((addrs_0[i] == HADDR_UNDEF) || (addrs_1[i] == HADDR_UNDEF) ||
    ...
    if ((f_addrs_0[i] == HADDR_UNDEF) || (f_addrs_1[i] == HADDR_UNDEF) ||
  ...
  if (H5FDwrite_vector(lf, H5P_DEFAULT, count - 3, &(types_0[3]), &(addrs_0[3]),
                       &(sizes_0[3]), &(write_bufs_0[3])) < 0)
    TEST_ERROR;

  if (H5FDread_vector(lf, H5P_DEFAULT, count - 3, &(types_0[3]), &(addrs_0[3]),
                      &(sizes_0[3]), &(read_bufs_0[3])) < 0)
    TEST_ERROR;

  /* for fixed size / type vector, just write and read as single operations */
  if (H5FDwrite_vector(lf, H5P_DEFAULT, count, f_types_0, f_addrs_0, f_sizes_0,
                       f_write_bufs_0) < 0)
    TEST_ERROR;

  if (H5FDread_vector(lf, H5P_DEFAULT, count, f_types_0, f_addrs_0, f_sizes_0,
                      f_read_bufs_0) < 0)
    TEST_ERROR;

  /* verify that the expected data is read */
  if (!test_vector_io__verify_v(count, types_0, sizes_0, write_bufs_0,
                                read_bufs_0, "zero"))
    TEST_ERROR;

  if (!test_vector_io__verify_v(count, f_types_0, f_sizes_0, f_write_bufs_0,
                                f_read_bufs_0, "fixed zero"))
    TEST_ERROR;
  ...
  if (H5FDread_vector(lf, H5P_DEFAULT, count - 3, &(types_1[3]), &(addrs_1[3]),
                      &(sizes_1[3]), &(read_bufs_1[3])) < 0)
    TEST_ERROR;

  /* for fixed size, write individually, and the read back in a single call */
  if (!test_vector_io__write_v_indiv(lf, count, f_types_1, f_addrs_1, f_sizes_1,
                                     f_write_bufs_1))
    TEST_ERROR;

  if (H5FDread_vector(lf, H5P_DEFAULT, count, f_types_1, f_addrs_1, f_sizes_1,
                      f_read_bufs_1) < 0)
    TEST_ERROR;

  /* verify that the expected data is read */
  if (!test_vector_io__verify_v(count, types_1, sizes_1, write_bufs_1,
                                read_bufs_1, "one"))
    TEST_ERROR;

  if (!test_vector_io__verify_v(count, f_types_1, f_sizes_1, f_write_bufs_1,
                                f_read_bufs_1, "fixed one"))
    TEST_ERROR;
  ...
  if (H5FDwrite_vector(lf, H5P_DEFAULT, count - 3, &(types_2[3]), &(addrs_2[3]),
                       &(sizes_2[3]), &(write_bufs_2[3])) < 0)
    TEST_ERROR;

  if (!test_vector_io__read_v_indiv(lf, count, types_2, addrs_2, sizes_2,
                                    read_bufs_2))
    TEST_ERROR;

  /* for fixed size, write as a single vector, read back individually */
  if (H5FDwrite_vector(lf, H5P_DEFAULT, count, f_types_2, f_addrs_2, f_sizes_2,
                       f_write_bufs_2) < 0)
    TEST_ERROR;

  if (!test_vector_io__read_v_indiv(lf, count, f_types_2, f_addrs_2, f_sizes_2,
                                    f_read_bufs_2))
    TEST_ERROR;

  /* verify that the expected data is read */
  if (!test_vector_io__verify_v(count, types_2, sizes_2, write_bufs_2,
                                read_bufs_2, "two"))
    TEST_ERROR;

  if (!test_vector_io__verify_v(count, f_types_2, f_sizes_2, f_write_bufs_2,
                                f_read_bufs_2, "fixed two"))
    TEST_ERROR;
  ...
  if (H5FDread_vector(lf, H5P_DEFAULT, count, types_0, addrs_0, sizes_0,
                      read_bufs_0) < 0)
    TEST_ERROR;

  if (H5FDread_vector(lf, H5P_DEFAULT, count, types_1, addrs_1, sizes_1,
                      read_bufs_1) < 0)
    TEST_ERROR;

  if (H5FDread_vector(lf, H5P_DEFAULT, count, types_2, addrs_2, sizes_2,
                      read_bufs_2) < 0)
    TEST_ERROR;
  ...
  if (H5FDread_vector(lf, H5P_DEFAULT, count, f_types_2, f_addrs_2, f_sizes_2,
                      f_read_bufs_2) < 0)
    TEST_ERROR;
...
static herr_t test_selection_io_write(H5FD_t *lf, H5FD_mem_t type,
                                      uint32_t count, hid_t mem_spaces[],
                                      hid_t file_spaces[], haddr_t offsets[],
                                      size_t element_sizes[], int *wbufs[]) {
  ...
  /* Issue write call */
  if (H5FDwrite_selection(lf, type, H5P_DEFAULT, count, mem_spaces, file_spaces,
...
static herr_t test_selection_io_read_verify(
  ...
  /* Issue read call */
  if (H5FDread_selection(lf, type, H5P_DEFAULT, count, mem_spaces, file_spaces,
...
static herr_t test_selection_io(const char *vfd_name) {
  ...
  size_t element_sizes[2] = {sizeof(int),
                             sizeof(int)}; /* element sizes vector */
  ...
  HDsnprintf(test_title, sizeof(test_title), "selection I/O with %s VFD",
             vfd_name);
  ...
  for (type = 1; type < H5FD_MEM_NTYPES; type++) {
    ...
    addrs[0] = H5FDalloc(lf, type, H5P_DEFAULT,
                         (hsize_t)(sizeof(int) * SEL_IO_DIM0 * SEL_IO_DIM1));
     ...
     */
    /* Issue write call */
    if (test_selection_io_write(lf, type, 1, &mem_spaces[0], &file_spaces[0],
                                &addrs[0], element_sizes,
                                (int **)&wbufs[0]) < 0)
      TEST_ERROR;
    ...
    /* Read and verify */
    if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[0],
     ...
     */
    /* Issue write call */
    if (test_selection_io_write(lf, type, 1, &mem_spaces[1], &file_spaces[1],
                                &addrs[1], element_sizes,
                                (int **)&wbufs[1]) < 0)
      TEST_ERROR;
    ...
    /* Read and verify */
    if (test_selection_io_read_verify(lf, type, 1, &mem_spaces[1],
    ...
    /* Issue write call */
    if (test_selection_io_write(lf, type, 1, &mem_spaces[0], &file_spaces[1],
                                &addrs[1], element_sizes,
                                (int **)&wbufs[0]) < 0)
      TEST_ERROR;
    ...
    /* Issue write call */
    if (test_selection_io_write(lf, type, 1, &mem_spaces[1], &file_spaces[0],
                                &addrs[0], element_sizes,
                                (int **)&wbufs[1]) < 0)
      TEST_ERROR;
    ...
    for (shorten_element_sizes = 0; shorten_element_sizes <= 1;
      ...
      count[0] = (SEL_IO_DIM0 * SEL_IO_DIM1) / 2;
      block[0] = 1;
      if (H5Sselect_hyperslab(mem_spaces[0], H5S_SELECT_SET, start, stride,
                              count, block) < 0)
        TEST_ERROR;
      ...
      block[0] = 1;
      block[1] = 1;
      if (H5Sselect_hyperslab(mem_spaces[1], H5S_SELECT_SET, start, stride,
                              count, block) < 0)
        TEST_ERROR;
      ...
      /* Issue write call */
      if (test_selection_io_write(lf, type, 2, mem_spaces, file_spaces, addrs,
                                  element_sizes, (int **)wbufs) < 0)
        TEST_ERROR;
      ...
      /* Read and verify */
      if (test_selection_io_read_verify(lf, type, 2, mem_spaces, file_spaces,
                                        addrs, element_sizes, 2, (int **)erbufs,
                                        FALSE) < 0)
        TEST_ERROR;
      ...
      /* Read entire file buffer and verify */
      if (test_selection_io_read_verify(lf, type, 2, mem_spaces, file_spaces,
                                        addrs, element_sizes, 2, (int **)fbufs,
                                        FALSE) < 0)
        TEST_ERROR;

      /*
       * Test 12: Strided <> Strided 2D I/O, 2 different selections in the same
       * memory buffer
       */
