...
static void usage(const char *prog) {
  ...
  fprintf(stderr,
          "     -n N        Number of planes to write. [default: 1000]\n");
  fprintf(
      stderr,
      "     -s N        Use SWMR mode (0: no, non-0: yes) default is yes\n");
...
static void show_parameters(void) {
  ...
  printf("chunk dims=(%llu, %llu, %llu)\n", (unsigned long long)chunkdims_g[0],
         (unsigned long long)chunkdims_g[1],
         (unsigned long long)chunkdims_g[2]);
  printf("dataset max dims=(%llu, %llu, %llu)\n",
         (unsigned long long)max_dims_g[0], (unsigned long long)max_dims_g[1],
         (unsigned long long)max_dims_g[2]);
...
static int create_file(void) {
  ...
  if ((dsid = H5Dcreate2(fid, progname_g, UC_DATATYPE, sid, H5P_DEFAULT, dcpl,
                         H5P_DEFAULT)) < 0)
    return -1;
...
static int write_file(void) {
  ...
  if (use_swmr_g)
    if (H5Pset_libver_bounds(fapl, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) < 0)
      return -1;
  if ((fid = H5Fopen(name, H5F_ACC_RDWR | (use_swmr_g ? H5F_ACC_SWMR_WRITE : 0),
                     fapl)) < 0) {
  ...
  if (chunk_dims[0] != chunkdims_g[0] || chunk_dims[1] != cz ||
      chunk_dims[2] != cz) {
    fprintf(
  ...
  /* Closing the dataset later will enable mdc flushes automatically if this is
   * not done */
  if (disabled)
