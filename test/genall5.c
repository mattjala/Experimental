  ...
  while ((pass) && (u < nlinks)) {
    ...
    if (0 == (u % 3)) {
      ...
      if (H5L_TYPE_SOFT != lnk_info.type) {
        ...
        pass = FALSE;
        failure_mssg =
            "vrfy_ns_grp_c: (HDstrlen(group_name) + 1) != lnk_info.u.val_size";
      }
      ...
      ret =
          H5Lget_val(gid, linkname, slinkval, lnk_info.u.val_size, H5P_DEFAULT);
      if (ret < 0) {
        pass = FALSE;
      ...
      if (ret < 0) {
        ...
        if (H5Otoken_cmp(fid, &root_oinfo.token, &lnk_info.u.token,
                         &token_cmp) < 0) {
          pass = FALSE;
      ...
      ret =
          H5Lget_val(gid, linkname, elinkval, lnk_info.u.val_size, H5P_DEFAULT);
      if (ret < 0) {
        pass = FALSE;
        failure_mssg = "vrfy_ns_grp_c: H5Lget_val() failed.";
      }
      ...
      ret = H5Lunpack_elink_val(elinkval, lnk_info.u.val_size, NULL, &file,
                                &path);
      if (ret < 0) {
        pass = FALSE;
...
void vrfy_os_grp_0(hid_t fid, const char *group_name) {
  ...
  if (pass) {
    ...
    if (ret < 0) {
      ...
      pass = FALSE;
      failure_mssg = "vrfy_os_grp_0: H5G_STORAGE_TYPE_SYMBOL_TABLE != "
                     "grp_info.storage_type";
    } else if (0 != grp_info.nlinks) {
      pass = FALSE;
...
void os_grp_n(hid_t fid, const char *group_name, int proc_num,
              unsigned nlinks) {
  hid_t gid = -1;
  unsigned u;
  hid_t fapl = -1;
...
void vrfy_os_grp_n(hid_t fid, const char *group_name, int proc_num,
  ...
  if (pass) {
    ...
    if (ret < 0) {
      ...
      pass = FALSE;
      failure_mssg = "vrfy_os_grp_n: H5G_STORAGE_TYPE_SYMBOL_TABLE != "
                     "grp_info.storage_type";
    } else if (nlinks != grp_info.nlinks) {
      pass = FALSE;
  ...
  while ((pass) && (u < nlinks)) {
    ...
    if (0 == (u % 2)) {
      ...
      if (H5L_TYPE_SOFT != lnk_info.type) {
        ...
        pass = FALSE;
        failure_mssg =
            "vrfy_os_grp_n: (HDstrlen(group_name) + 1) != lnk_info.u.val_size";
      }
      assert(H5L_TYPE_SOFT == lnk_info.type);
      assert((HDstrlen(group_name) + 1) == lnk_info.u.val_size);
...
void vrfy_ds_ctg_i(hid_t fid, const char *dset_name, hbool_t write_data) {
  ...
  if (pass) {
    ...
    if (ret < 0) {
      ...
      pass = FALSE;
      failure_mssg = "vrfy_ds_ctg_i: write_data && allocation != "
...
void vrfy_ds_chk_i(hid_t fid, const char *dset_name, hbool_t write_data) {
  ...
  if (pass) {
    ...
    if (ret < 0) {
      ...
      pass = FALSE;
      failure_mssg = "vrfy_ds_chk_i: write_data && allocation != "
                     "H5D_SPACE_STATUS_ALLOCATED";
    } else if (!write_data && (allocation != H5D_SPACE_STATUS_NOT_ALLOCATED)) {
      pass = FALSE;
      failure_mssg = "vrfy_ds_chk_i: !write_data && allocation != "
...
void ds_ctg_v(hid_t fid, const char *dset_name, hbool_t write_data) {
  ...
  if (pass) {
    dsid = H5Dcreate2(fid, dset_name, tid, sid, H5P_DEFAULT, H5P_DEFAULT,
                      H5P_DEFAULT);
...
void vrfy_ds_ctg_v(hid_t fid, const char *dset_name, hbool_t write_data) {
  ...
  if (pass) {
    ...
    if (ret < 0) {
      ...
      pass = FALSE;
      failure_mssg = "vrfy_ds_ctg_v: write_data && allocation != "
                     "H5D_SPACE_STATUS_ALLOCATED";
    } else if (!write_data && (allocation != H5D_SPACE_STATUS_NOT_ALLOCATED)) {
      pass = FALSE;
      failure_mssg = "vrfy_ds_ctg_v: !write_data && allocation != "
