  ...
  for (u = 0; u < nrecords; u++) {
    ...
    if (symbol->nrecords == 0) {
      ...
      if ((file_sid = H5Screate(H5S_SCALAR)) < 0)
        return -1;
      if ((aid = H5Acreate2(symbol->dsid, "seq", H5T_NATIVE_ULONG, file_sid,
...
static void usage(void) {
  ...
  printf("\n");
  printf(
      "Defaults to verbose (no '-q' given) and flushing every 1000 records\n");
  printf("('-f 1000')\n");
  printf("\n");
  exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
  ...
  if (add_records(fid, verbose, (unsigned long)nrecords,
