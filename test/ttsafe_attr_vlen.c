 ...
 *
 *          The cause of the problem in this jira issue is due to the file
 *pointer that is set in the variable length string datatype for the attribute.
...
void *tts_attr_vlen_thread(void H5_ATTR_UNUSED *client_data) {
  hid_t fid = H5I_INVALID_HID;     /* File ID */
  hid_t gid = H5I_INVALID_HID;     /* Group ID */
...
void cleanup_attr_vlen(void) { HDunlink(FILENAME); }
