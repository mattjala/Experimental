...
static herr_t populate_filepath(const char *dirname, const char *_basename,
 ...
 * ----------------------------------------------------------------------------
 */
static herr_t populate_filepath(const char *dirname, const char *basename,
                                hid_t fapl_id, char *path_out,
                                hbool_t h5suffix) {
  char *path = NULL;

  if ((basename == NULL) || (*basename == 0) || (dirname == NULL) ||
  ...
  if (HDsnprintf(
          path, H5FD_SPLITTER_PATH_MAX, "%s%s%s", dirname,
  ...
  if (h5suffix == TRUE) {
    ...
    if (h5_fixname_no_suffix(path, fapl_id, path_out, H5FD_SPLITTER_PATH_MAX) ==
        NULL)
      TEST_ERROR;
  }
 ...
 * ---------------------------------------------------------------------------
 */
static herr_t build_paths(const char *basename,
  ...
  if (populate_filepath(MIRROR_RW_DIR, basename, splitter_config->rw_fapl_id,
                        names->rw, TRUE) < 0)
    TEST_ERROR;

  if (populate_filepath(MIRROR_WO_DIR, basename, splitter_config->wo_fapl_id,
                        names->wo, TRUE) < 0)
    TEST_ERROR;

  if (basename == NULL || *basename == 0)
    TEST_ERROR;
  if (HDsnprintf(baselogname, H5FD_SPLITTER_PATH_MAX, "%s_err.log", basename) >
      H5FD_SPLITTER_PATH_MAX)
    TEST_ERROR;

  if (populate_filepath(MIRROR_WO_DIR, baselogname, splitter_config->wo_fapl_id,
                        names->log, FALSE) < 0)
    TEST_ERROR;
...
static int test_encdec_open(H5FD_mirror_xmit_t xmit_mock) {
  ...
  /* Allocate memory */
  if (NULL ==
      (buf = malloc((H5FD_MIRROR_XMIT_OPEN_SIZE + 8) * sizeof(unsigned char))))
    FAIL_PUTS_ERROR("Unable to allocate memory for buf");
  if (NULL == (expected = malloc((H5FD_MIRROR_XMIT_OPEN_SIZE + 8) *
  ...
  if (HDstrncmp(xmit_out->filename, xmit_in->filename,
                H5FD_MIRROR_XMIT_FILEPATH_MAX) != 0) {
...
static int test_encdec_reply(H5FD_mirror_xmit_t xmit_mock) {
  ...
  if (HDstrncmp(xmit_out.message, xmit_in.message,
                H5FD_MIRROR_STATUS_MESSAGE_MAX) != 0) {
 ...
 * ---------------------------------------------------------------------------
 */
static hid_t create_mirroring_split_fapl(const char *basename,
                                         struct mirrortest_filenames *names,
                                         const struct mt_opts *opts) {
  ...
  if (HDstrncpy(mirror_conf.remote_ip, opts->ip, H5FD_MIRROR_MAX_IP_LEN) ==
      NULL)
    TEST_ERROR;
...
static herr_t _create_chunking_ids(hid_t file_id, unsigned min_dset,
  ...
  for (unsigned m = min_dset; m <= max_dset; m++) {
    ...
    if ((dataset_ids[m] =
             H5Dcreate2(file_id, dset_name, H5T_STD_I32BE, dataspace_ids[m],
...
static int test_basic_dataset_write(const struct mt_opts *opts) {
  ...
  if ((dspace_id = H5Screate_simple(2, dims, NULL)) < 0)
    TEST_ERROR;
  if ((dset_id = H5Dcreate2(file_id, "dataset", dtype_id, dspace_id,
...
static int test_on_disk_zoo(const struct mt_opts *opts) {
  ...
  if ((grp_id = H5Gcreate2(file_id, grp_name, H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  for (i = 0; i < max_loops; i++) {
    ...
    if (i < (max_loops - max_at_one_time)) {
      if (HDsnprintf(namebuf, DSET_NAME_LEN, "/dset%02d", i) > DSET_NAME_LEN)
        TEST_ERROR;
      if ((dset_id = H5Dcreate2(file_id, namebuf, H5T_STD_U32LE, dspace_id,
                                H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        TEST_ERROR;
...
static int test_concurrent_access(const struct mt_opts *opts) {
  ...
  for (i = 0; i < CONCURRENT_COUNT; i++) {
    ...
    if ((fapl_id = create_mirroring_split_fapl(name, &bundle[i].names, opts)) <
        0)
      TEST_ERROR;
  ...
  for (i = 0; i < CONCURRENT_COUNT; i++) {
    ...
    if ((file_id = H5Fcreate(bundle[i].names.rw, H5F_ACC_TRUNC, H5P_DEFAULT,
                             bundle[i].fapl_id)) < 0)
      TEST_ERROR;
...
static int confirm_server(struct mt_opts *opts) {
  ...
  while (1) {
    if (connect(live_socket, (struct sockaddr *)&target_addr,
