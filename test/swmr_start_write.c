 ...
 *
 * Purpose:     This program enables SWMR writing mode via
 *H5Fstart_swmr_write(). It writes data to a randomly selected subset of the
 *datasets in the SWMR test file; and it is intended to run concurrently with
 *the swmr_reader program.
 *
...
static hid_t create_file(const char *filename, hbool_t verbose,
  ...
  if ((sid = H5Screate(H5S_SCALAR)) < 0)
    return -1;
  if ((aid = H5Acreate2(fid, "seed", H5T_NATIVE_UINT, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    return -1;
 ...
 *
 * Purpose:     Create datasets (and keep them opened) which will be used for
  ...
  for (u = 0; u < NLEVELS; u++)
    for (v = 0; v < symbol_count[u]; v++) {
      ...
      if ((symbol_info[u][v].dsid =
               H5Dcreate2(fid, symbol_info[u][v].name, tid, sid, H5P_DEFAULT,
                          dcpl, H5P_DEFAULT)) < 0)
        return -1;
 ...
 *              flush_count: # of records to write before flushing the file to
 *disk
 *
 * Return:      Success: 0
 *              Failure: -1
...
static int add_records(hid_t fid, hbool_t verbose, FILE *verbose_file,
  ...
  for (u = 0; u < nrecords; u++) {
    ...
    if (H5Sselect_hyperslab(file_sid, H5S_SELECT_SET, start, NULL, count,
                            NULL) < 0)
      return -1;

    /* Write record to the dataset */
    if (H5Dwrite(symbol->dsid, tid, mem_sid, file_sid, H5P_DEFAULT, &record) <
        0)
      return -1;
...
static void usage(void) {
  ...
  printf("Usage error!\n");
  printf("\n");
  printf("Usage: swmr_start_write [-f <# of records to write between flushing "
         "file contents>]\n");
  printf("    [-i <index type>] [-c <deflate compression level>]\n");
  printf("    [-r <random seed>] [-q] <# of records>\n");
  printf("\n");
  printf(
      "<# of records to write between flushing file contents> should be 0\n");
  printf("(for no flushing) or between 1 and (<# of records> - 1).\n");
  printf("\n");
  printf("<index type> should be b2 or ea\n");
  printf("\n");
  printf(
      "<deflate compression level> should be -1 (for no compression) or 0-9\n");
  printf("\n");
  ...
  printf("will generate a random seed (no -r given), and verbose (no '-q' "
         "given)\n");
  printf("\n");
  exit(EXIT_FAILURE);
} /* usage() */
 ...
 *	1) create_file(), create_datasets(), H5Fstart_swmr_write(),
 *add_records(), H5Fclose(). 2) create_file(), create_close_datasets(),
 *open_datasets(), H5Fstart_swmr_write(), add_records(), H5Fclose(). 3)
 *create_file(), create_close_datasets(), H5Fclose(), open_file(),
 *open_dataset(), H5Fstart_swmr_write(), add_records(), H5Fclose().
 */
int main(int argc, char *argv[]) {
  ...
  hbool_t use_seed =
      FALSE; /* Set to TRUE if a seed was set on the command line */
  ...
  if (argc > 1) {
    ...
    while (u < (unsigned)argc) {
      if (argv[u][0] == '-') {
        ...
        case 'i':
          index_type = argv[u + 1];
          if (HDstrcmp(index_type, "ea") != 0 &&
              HDstrcmp(index_type, "b2") != 0)
            usage();
          u += 2;
          break;
  ...
  if (verbose) {
    ...
    HDsnprintf(verbose_name, sizeof(verbose_name), "swmr_writer.out.%u",
               random_seed);
  ...
  if (add_records(fid, verbose, verbose_file, (unsigned long)nrecords,
