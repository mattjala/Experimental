...
herr_t run_flush_verification_process(const char *obj_pathname,
                                      const char *expected);
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    ...
    if (envval == NULL || H5FD__supports_swmr_test(envval)) {
      ...
      fprintf(stdout, "Skipping all flush/refresh tests (only run with "
                      "SWMR-enabled file drivers).\n");
...
herr_t test_flush(void) {
  ...
  /* Variables */
  hid_t fid, gid, gid2, gid3, sid, tid1, tid2, tid3, did, did2, did3, rid, fapl,
      status = 0;
  hsize_t dims[2] = {3, 5};
  ...
  /* Create Dataset1 */
  if ((did = H5Dcreate2(fid, "Dataset1", tid1, sid, H5P_DEFAULT, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create Dataset2 */
  if ((did2 = H5Dcreate2(gid, "Dataset2", tid3, sid, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create Dataset3 */
  if ((did3 = H5Dcreate2(gid3, "Dataset3", tid2, sid, H5P_DEFAULT, H5P_DEFAULT,
                         H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create CommittedDatatype1 */
  if ((status = H5Tcommit2(fid, "CommittedDatatype1", tid1, H5P_DEFAULT,
                           H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create CommittedDatatype2 */
  if ((status = H5Tcommit2(gid2, "CommittedDatatype2", tid2, H5P_DEFAULT,
                           H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;

  /* Create CommittedDatatype3 */
  if ((status = H5Tcommit2(gid3, "CommittedDatatype3", tid3, H5P_DEFAULT,
                           H5P_DEFAULT, H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
herr_t test_refresh(void) {
  ...
  /* Create Dataset1 */
  if ((did = H5Dcreate2(fid, "Dataset1", tid1, sid, H5P_DEFAULT, dcpl,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(did, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(did2, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(gid, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(gid2, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(tid1, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(tid2, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(did3, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(gid3, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
  ...
  if ((aid = H5Acreate2(tid3, "Attribute", tid1, sid, H5P_DEFAULT,
                        H5P_DEFAULT)) < 0)
    TEST_ERROR;
  if (H5Aclose(aid) < 0)
    TEST_ERROR;
...
herr_t run_flush_verification_process(const char *obj_pathname,
  ...
  /* Send Signal to SCRIPT indicating that it should kick off a verification
   * process. */
  h5_send_message(SIGNAL_TO_SCRIPT, obj_pathname, expected);

  /* Wait for Signal from SCRIPT indicating that verification process has
   * completed. */
...
herr_t flush_verification(const char *obj_pathname, const char *expected) {
  ...
  if (HDstrcmp(expected, FLUSHED) == 0) {
    ...
    if ((oid > 0) || (status > 0)) {
      fprintf(
          stderr,
          "Error! %s not expected to be flushed, but it was found on disk!\n",
          obj_pathname);
      PROCESS_ERROR;
    } /* end if */
  } else {
    fprintf(stderr,
            "Error! Bad verification parameters. %s is an invalid expected "
            "outcome.\n",
            expected);
    PROCESS_ERROR;
  } /* end if */
...
herr_t refresh_verification(const char *obj_pathname) {
  ...
  /* Get Object info */
  if ((status = H5Oget_info3(oid, &flushed_oinfo,
                             H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS)) < 0)
    PROCESS_ERROR;
  if ((status = H5Oget_native_info(oid, &flushed_ninfo, H5O_NATIVE_INFO_HDR)) <
      0)
    PROCESS_ERROR;
  ...
  if ((status = H5Oget_info3(oid, &refreshed_oinfo,
                             H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS)) < 0)
    PROCESS_ERROR;
  if ((status =
           H5Oget_native_info(oid, &refreshed_ninfo, H5O_NATIVE_INFO_HDR)) < 0)
    PROCESS_ERROR;
  ...
  if (H5Otoken_cmp(oid, &flushed_oinfo.token, &refreshed_oinfo.token,
                   &token_cmp) < 0)
    PROCESS_ERROR;
  if (token_cmp)
    PROCESS_ERROR;
  ...
  do {
    ...
    if ((HDstrcmp(obj_pathname, D1) == 0) ||
        (HDstrcmp(obj_pathname, D2) == 0)) {
      if (H5Drefresh(oid) < 0)
        PROCESS_ERROR;
    } /* end if */
    else if ((HDstrcmp(obj_pathname, G1) == 0) ||
             (HDstrcmp(obj_pathname, G2) == 0)) {
      if (H5Grefresh(oid) < 0)
        PROCESS_ERROR;
    } /* end if */
    else if ((HDstrcmp(obj_pathname, T1) == 0) ||
             (HDstrcmp(obj_pathname, T2) == 0)) {
      if (H5Trefresh(oid) < 0)
        PROCESS_ERROR;
    } /* end if */
    else if ((HDstrcmp(obj_pathname, D3) == 0) ||
             (HDstrcmp(obj_pathname, G3) == 0) ||
    ...
    /* Get object info. This should now accurately reflect the refreshed object
     * on disk. */
    if ((status = H5Oget_info3(oid, &refreshed_oinfo,
                               H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS)) < 0)
      PROCESS_ERROR;
    if ((status = H5Oget_native_info(oid, &refreshed_ninfo,
                                     H5O_NATIVE_INFO_HDR)) < 0)
      PROCESS_ERROR;
    if (H5Otoken_cmp(oid, &flushed_oinfo.token, &refreshed_oinfo.token,
                     &token_cmp) < 0)
      PROCESS_ERROR;
