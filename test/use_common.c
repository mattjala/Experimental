...
void usage(const char *prog) {
  ...
  fprintf(stderr,
          "     -f FN                 Test file name [default: %s.h5]\n", prog);
  fprintf(stderr, "     -i N, --iteration=N   Number of iterations to repeat "
                  "the whole thing. [default: 1]\n");
  fprintf(stderr, "     -l w|r                launch writer or reader only. "
                  "[default: launch both]\n");
  fprintf(stderr, "     -n N, --nplanes=N     Number of planes to write/read. "
                  "[default: 1000]\n");
  fprintf(stderr, "     -s N, --swmr=N        Use SWMR mode (0: no, non-0: "
                  "yes) default is yes\n");
...
int parse_option(int argc, char *const argv[], options_t *opts) {
  ...
  while (1) {
    ...
    case 'i': /* iterations */
      if ((opts->iterations = atoi(optarg)) <= 0) {
        fprintf(stderr,
                "bad iterations number %s, must be a positive integer\n",
                optarg);
        usage(opts->progname);
        Hgoto_error(-1);
      }
    ...
    case 'y': /* Number of planes per chunk */
      if ((opts->chunkplanes = strtoul(optarg, NULL, 0)) <= 0) {
        fprintf(
            stderr,
            "bad number of planes per chunk %s, must be a positive integer\n",
            optarg);
        usage(opts->progname);
        Hgoto_error(-1);
      }
  ...
  if (!opts->filename) {
    ...
    if ((opts->filename = (char *)malloc(HDstrlen(opts->progname) + 4)) ==
        NULL) {
...
int create_uc_file(options_t *opts) {
  ...
  if ((fid = H5Fcreate(opts->filename, H5F_ACC_TRUNC, H5P_DEFAULT,
                       opts->fapl_id)) < 0)
    return -1;
  ...
  if ((dsid = H5Dcreate2(fid, opts->progname, UC_DATATYPE, sid, H5P_DEFAULT,
                         dcpl, H5P_DEFAULT)) < 0)
    return -1;
...
int write_uc_file(hbool_t tosend, hid_t file_id, options_t *opts) {
  ...
  if (chunk_dims[0] != opts->chunkdims[0] || chunk_dims[1] != cz ||
      chunk_dims[2] != cz) {
    fprintf(
        stderr, "chunk size is not as expected. Got dims=(%llu,%llu,%llu)\n",
  ...
  if ((buffer = (UC_CTYPE *)malloc((size_t)memdims[1] * (size_t)memdims[2] *
                                   sizeof(UC_CTYPE))) == NULL) {
    fprintf(stderr, "malloc: failed\n");
    return -1;
  }
  ...
  /* verify that file space dims are as expected and are consistent with memory
   * space dims */
  ...
  for (i = 0; i < opts->nplanes; i++) {
    ...
    if (H5Sselect_hyperslab(f_sid, H5S_SELECT_SET, start, NULL, count, NULL) <
        0) {
...
int read_uc_file(hbool_t towait, options_t *opts) {
  ...
  /* Before reading, wait for the message that H5Fopen is complete--file lock is
   * released */
  ...
  if ((fid = H5Fopen(opts->filename,
                     H5F_ACC_RDONLY | (opts->use_swmr ? H5F_ACC_SWMR_READ : 0),
                     opts->fapl_id)) < 0) {
          ...
          if ((hsize_t)*bufptr++ != nplane) {
            if (++nerrs < ErrorReportMax) {
              fprintf(
                  ...
                  (unsigned long long)k, (unsigned long long)nplane,
                  (int)*(bufptr - 1));
