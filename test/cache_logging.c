...
static herr_t test_logging_api(void) {
  ...
  if (H5Pset_mdc_log_options(fapl, is_enabled, LOG_LOCATION, start_on_access) <
      0)
    TEST_ERROR;
  ...
  if ((H5Fget_mdc_logging_status(fid, &is_enabled, &is_currently_logging) <
       0) ||
      (is_enabled != TRUE) || (is_currently_logging != FALSE))
    TEST_ERROR;
  ...
  if ((H5Fget_mdc_logging_status(fid, &is_enabled, &is_currently_logging) <
       0) ||
      (is_enabled != TRUE) || (is_currently_logging != TRUE))
    TEST_ERROR;
  ...
  for (i = 0; i < N_GROUPS; i++) {
    ...
    if ((gid = H5Gcreate2(fid, group_name, H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT)) < 0)
      TEST_ERROR;
    if (H5Gclose(gid) < 0)
      TEST_ERROR;
