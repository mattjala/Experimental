 ...
 *		This program is used to create the test file `tarrnew.h5' which
 *has a datatypes stored in the newer (version 2) style in the object headers.
 *		To build the test file, this program MUST be compiled and linked
 *with the hdf5-1.3+ series of libraries and the generated test file must be put
 *into the 'test' directory in the 1.2.x branch of the library. The test file
 *should be generated on a little-endian machine with 16-bit shorts, 32-bit
 *floats, 32-bit ints and 64-bit doubles.
 */
#include "hdf5.h"
...
int main(void) {
  ...
  dset = H5Dcreate2(file, "Dataset1", type, space, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
  if (dset < 0)
    printf("dset<0!\n");
  H5Dclose(dset);
  ...
  dset = H5Dcreate2(file, "Dataset2", type, space, H5P_DEFAULT, H5P_DEFAULT,
                    H5P_DEFAULT);
  if (dset < 0)
    printf("dset<0!\n");
  H5Dclose(dset);
