 ...
 *		This program is used to create the test file `tarrold.h5' which
 *has a datatypes stored in the older (version 1) style in the object headers.
 *		To build the test file, this program MUST be compiled and linked
 *with the hdf5-1.2.x series of libraries and the generated test file must be
...
int main(void) {
  ...
  ret = H5Tinsert_array(type, "f", HOFFSET(s3_t, f), ARRAY1_RANK, tdims1, NULL,
                        H5T_NATIVE_FLOAT);
  if (ret < 0)
    printf("field 3 insert<0!\n");

  /* Insert long array field */
  ret = H5Tinsert_array(type, "l", HOFFSET(s3_t, l), ARRAY1_RANK, tdims1, NULL,
                        H5T_NATIVE_LONG);
  if (ret < 0)
    printf("field 3 insert<0!\n");
