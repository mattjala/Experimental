...
int_f fill_h5o_info_t_f(H5O_info2_t Oinfo, H5O_info_t_f *object_info) {
  ...
  object_info->atime[7] =
      -32767; /* millisecond is not available, assign it -HUGE(0) */
 ...
 *  new_loc_id       - File or group identifier specifying location at which
 * object is to be linked. name             - Name of link to be created,
 * relative to new_loc_id. namelen          - Length of buffer for link to be
 * created. lcpl_id          - Link creation property list identifier. lapl_id
 * - Link access property list identifier. RETURNS 0 on success, -1 on failure
 * SOURCE
 */
int_f h5olink_c(hid_t_f *object_id, hid_t_f *new_loc_id, _fcd name,
 ...
 * SOURCE
 */
int_f h5oopen_by_token_c(hid_t_f *loc_id, H5O_token_t *token, hid_t_f *obj_id)
/******/
{
  int_f ret_value = 0; /* Return value */
 ...
 * INPUTS
 *  loc_id       - File or group identifier specifying location of group in
 * which object is located. name         - Name of group, relative to loc_id.
 *  namelen      - Name length.
 ...
 * SOURCE
 */
int_f h5oget_info_by_name_c(hid_t_f *loc_id, char *name, hid_t_f *lapl_id,
...
/******/
{
  ...
  if ((hid_t)*es_id != -1) {
    if (H5Oget_info_by_name3((hid_t)*loc_id, name, &Oinfo, (unsigned)*fields,
                             (hid_t)*lapl_id) < 0)
      HGOTO_DONE(FAIL);
  } else {
    if (H5Oget_info_by_name_async_wrap(
            file, func, (unsigned)*line, (hid_t)*loc_id, name, &Oinfo,
 ...
 * SOURCE
 */
int_f h5oget_info_by_idx_c(hid_t_f *loc_id, _fcd group_name, size_t_f *namelen,
...
/******/
{
   ...
   */
  if (H5Oget_info_by_idx3((hid_t)*loc_id, c_group_name, c_index_field, c_order,
                          (hsize_t)*n, &Oinfo, (unsigned)*fields,
                          (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5oget_info_c(hid_t_f *object_id, H5O_info_t_f *object_info,
                    int_f *fields)
/******/
{
  int_f ret_value = 0; /* Return value */
 ...
 * SOURCE
 */
int_f h5odecr_refcount_c(hid_t_f *object_id)
/******/
{
  int_f ret_value = 0; /* Return value */
 ...
 * SOURCE
 */
int_f h5oexists_by_name_c(hid_t_f *loc_id, _fcd name, size_t_f *namelen,
                          hid_t_f *lapl_id)
/******/
{
  char *c_name = NULL; /* Buffer to hold C string */
   ...
   */
  if ((ret_value = (int_f)H5Oexists_by_name((hid_t)*loc_id, c_name,
                                            (hid_t)*lapl_id)) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5oincr_refcount_c(hid_t_f *object_id)
/******/
{
  int_f ret_value = 0; /* Return value */
 ...
 * SOURCE
 */
int_f h5oset_comment_c(hid_t_f *object_id, _fcd comment, size_t_f *commentlen)
/******/
{
 ...
 * SOURCE
 */
int_f h5oset_comment_by_name_c(hid_t_f *object_id, _fcd name, size_t_f *namelen,
                               _fcd comment, size_t_f *commentlen,
                               hid_t_f *lapl_id)
/******/
{
  char *c_comment = NULL; /* Buffer to hold C string */
   ...
   */
  if ((hid_t_f)H5Oset_comment_by_name((hid_t)*object_id, c_name, c_comment,
                                      (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5oget_comment_c(hid_t_f *object_id, _fcd comment, size_t_f *commentsize,
                       hssize_t_f *bufsize)
/******/
{
  char *c_comment = NULL; /* Buffer to hold C string */
 ...
 * SOURCE
 */
int_f h5oget_comment_by_name_c(hid_t_f *loc_id, _fcd name, size_t_f *name_size,
                               _fcd comment, size_t_f *commentsize,
...
/******/
{
  ...
  if ((c_bufsize =
           H5Oget_comment_by_name((hid_t)*loc_id, c_name, c_comment,
                                  (size_t)*commentsize, (hid_t)*lapl_id)) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5otoken_cmp_c(hid_t_f *loc_id, H5O_token_t *token1, H5O_token_t *token2,
                     int_f *cmp_value_f)
/******/
{
