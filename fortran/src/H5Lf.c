...
int_f h5lcopy_c(hid_t_f *src_loc_id, _fcd src_name, size_t_f *src_namelen,
                hid_t_f *dest_loc_id, _fcd dest_name, size_t_f *dest_namelen,
...
/******/
{
   ...
   */
  if (H5Lcopy((hid_t)*src_loc_id, c_src_name, (hid_t)*dest_loc_id, c_dest_name,
 ...
 * INPUTS
 *
 *  file_name - Name of the file containing the target object. Neither the file
 * nor the target object is required to exist. May be the file the link is being
 * created in. obj_name - Path within the target file to the target object.
...
int_f h5lcreate_external_c(_fcd file_name, size_t_f *file_namelen,
...
/******/
{
   ...
   * Call H5Lcopy function.
   */
  if (H5Lcreate_external(c_file_name, c_obj_name, (hid_t)*link_loc_id,
                         c_link_name, (hid_t)*lcpl_id, (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 *  token - If the link is a hard link, token specifies the token for the object
 *that the link points to val_size - If the link is a symbolic link, val_size
 ...
 * SOURCE
 */
int_f h5lget_info_c(hid_t_f *link_loc_id, _fcd link_name,
                    size_t_f *link_namelen, int_f *cset, int_f *corder,
                    int_f *corder_valid, int_f *link_type, H5O_token_t *token,
...
/******/
{
   ...
   */
  if (H5Lget_info2((hid_t)*link_loc_id, c_link_name, &link_buff,
                   (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 * OUTPUTS
 *
 *  corder_valid - Indicates whether the creation order data is valid for this
 *attribute corder - Is a positive integer containing the creation order of the
 *attribute cset - Indicates the character set used for the attribute's name
 *  data_size - indicates the size, in the number of characters, of the
 *attribute
 *
 * RETURNS
 *  0 on success, -1 on failure
 * SOURCE
 */
int_f h5lget_info_by_idx_c(hid_t_f *loc_id, _fcd group_name,
                           size_t_f *group_namelen, int_f *index_field,
                           int_f *order, hsize_t_f *n, int_f *link_type,
                           int_f *corder_valid, int_f *corder, int_f *cset,
...
/******/
{
   ...
   */
  if (H5Lget_info_by_idx2((hid_t)*loc_id, c_group_name, c_index_field, c_order,
                          (hsize_t)*n, &link_buff, (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5lis_registered_c(int_f *link_cls_id)
/******/
{
 ...
 * SOURCE
 */
int_f h5lmove_c(hid_t_f *src_loc_id, _fcd src_name, size_t_f *src_namelen,
                hid_t_f *dest_loc_id, _fcd dest_name, size_t_f *dest_namelen,
                hid_t_f *lcpl_id, hid_t_f *lapl_id)
/******/
{
   ...
   */
  if (H5Lmove((hid_t)*src_loc_id, c_src_name, (hid_t)*dest_loc_id, c_dest_name,
              (hid_t)*lcpl_id, (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5lget_name_by_idx_c(hid_t_f *loc_id, _fcd group_name,
                           size_t_f *group_namelen, int_f *index_field,
...
/******/
{
  ...
  if ((c_size_link = H5Lget_name_by_idx((hid_t)*loc_id, c_group_name,
...
/*  * */
/*  *		link_loc_id - File or group identifier. */
/*  *                link_name - Name of the link for which valrmation is being
 * sought */
/*  *             link_namelen - Name length */
/*  *                     size - Maximum number of characters of link value to
 * be returned. */
/*  *                  lapl_id - Link access property list */
/*  * OUTPUTS */
/*  * */
...
/*      * Convert FORTRAN name to C name */
/*      *\/ */
/*     if((c_link_name = HD5f2cstring(link_name, (size_t)*link_namelen)) ==
 * NULL) */
/*       HGOTO_DONE(FAIL); */
/*     /\* */
 ...
 */
/*     HGOTO_DONE(FAIL); */
/*   if((c_create_func = HD5f2cstring(c_create_func,
 * (size_t)*c_create_func_len)) == NULL) */
/*     HGOTO_DONE(FAIL); */
/*   if((c_move_func = HD5f2cstring(c_move_func, (size_t)*sc_move_func_len)) ==
 * NULL) */
/*     HGOTO_DONE(FAIL); */
/*   if((c_copy_func = HD5f2cstring(c_copy_func, (size_t)*c_copy_func_len)) ==
 * NULL) */
/*     HGOTO_DONE(FAIL); */
/*   if((c_trav_func = HD5f2cstring(c_trav_func, (size_t)*sc_trav_func_len)) ==
 * NULL) */
/*     HGOTO_DONE(FAIL); */
/*   if((c_del_func = HD5f2cstring(c_del_func, (size_t)*c_del_func_len)) ==
 * NULL) */
/*     HGOTO_DONE(FAIL); */
 ...
 * SOURCE
 */
int_f h5lget_val_c(hid_t_f *link_loc_id, _fcd link_name, size_t_f *link_namelen,
                   size_t_f *size, void *linkval_buff, hid_t_f *lapl_id)
/******/
{
  char *c_link_name = NULL; /* Buffer to hold C string */
  int_f ret_value = 0;      /* Return value */
   ...
   * Call H5Lget_val
   */
  if (H5Lget_val((hid_t)*link_loc_id, c_link_name, &linkval_buff, (size_t)*size,
                 (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 *  op - Callback function passing data regarding the link to the calling
 * application op_data - User-defined pointer to data required by the
 * application for its processing of the link lapl_id - List access property
 * list identifier
 *
 * OUTPUTS
 *
 ...
 * SOURCE
 */
int_f h5literate_by_name_c(hid_t_f *loc_id, _fcd name, size_t_f *namelen,
                           int_f *index_type, int_f *order, hsize_t_f *idx,
                           H5L_iterate2_t op, void *op_data, hid_t_f *lapl_id)
/******/
{
  ...
  func_ret_value = H5Literate_by_name2(
      (hid_t)*loc_id, c_name, (H5_index_t)*index_type, (H5_iter_order_t)*order,
      &idx_c, op, op_data, (hid_t)*lapl_id);
