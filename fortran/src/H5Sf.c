...
int_f h5screate_simple_c(int_f *rank, hsize_t_f *dims, hsize_t_f *maxdims,
                         hid_t_f *space_id)
/******/
{
...
int_f h5sclose_c(hid_t_f *space_id)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5screate_c(int_f *classtype, hid_t_f *space_id)
/******/
{
...
int_f h5scopy_c(hid_t_f *space_id, hid_t_f *new_space_id)
/******/
{
  int ret_value = 0;
...
int_f h5sget_select_hyper_nblocks_c(hid_t_f *space_id, hssize_t_f *num_blocks)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sget_select_elem_npoints_c(hid_t_f *space_id, hssize_t_f *num_points)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sget_select_hyper_blocklist_c(hid_t_f *space_id, hsize_t_f *startblock,
...
/******/
{
  ...
  c_buf = (hsize_t *)malloc(sizeof(hsize_t) *
                            (size_t)(c_num_blocks * 2 * (hsize_t)rank));
  if (!c_buf)
    return ret_value;
...
int_f h5sget_select_bounds_c(hid_t_f *space_id, hsize_t_f *start,
                             hsize_t_f *end)
/******/
{
  hid_t c_space_id;
...
int_f h5sget_select_elem_pointlist_c(hid_t_f *space_id, hsize_t_f *startpoint,
...
/******/
{
  ...
  c_buf = (hsize_t *)malloc(sizeof(hsize_t) *
                            (size_t)(c_num_points * (hsize_t)rank));
  if (!c_buf)
    return ret_value;
...
int_f h5sselect_all_c(hid_t_f *space_id)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sselect_none_c(hid_t_f *space_id)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sselect_valid_c(hid_t_f *space_id, int_f *flag)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sget_simple_extent_npoints_c(hid_t_f *space_id, hsize_t_f *npoints)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sget_select_npoints_c(hid_t_f *space_id, hssize_t_f *npoints)
/******/
{
  int ret_value = 0;
  hssize_t c_npoints;
...
int_f h5sget_simple_extent_ndims_c(hid_t_f *space_id, int_f *ndims)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sget_simple_extent_type_c(hid_t_f *space_id, int_f *classtype)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5soffset_simple_c(hid_t_f *space_id, hssize_t_f *offset)
/******/
{
  hid_t c_space_id;
  int rank;
...
int_f h5sset_extent_simple_c(hid_t_f *space_id, int_f *rank,
                             hsize_t_f *current_size, hsize_t_f *maximum_size)
/******/
{
  ...
  if (H5Sset_extent_simple((hid_t)*space_id, (int)*rank, c_current_size,
                           c_maximum_size) < 0)
    HGOTO_DONE(FAIL);
...
int_f h5sget_simple_extent_dims_c(hid_t_f *space_id, hsize_t_f *dims,
                                  hsize_t_f *maxdims)
/******/
{
  hid_t c_space_id;
...
int_f h5sis_simple_c(hid_t_f *space_id, int_f *flag)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sextent_copy_c(hid_t_f *dest_space_id, hid_t_f *source_space_id)
/******/
{
  int ret_value = 0;
...
int_f h5sset_extent_none_c(hid_t_f *space_id)
/******/
{
  int ret_value = 0;
  hid_t c_space_id;
...
int_f h5sselect_hyperslab_c(hid_t_f *space_id, int_f *op, hsize_t_f *start,
                            hsize_t_f *count, hsize_t_f *stride,
                            hsize_t_f *block)
/******/
{
  hsize_t c_start[H5S_MAX_RANK];
  ...
  if (H5Sselect_hyperslab((hid_t)*space_id, (H5S_seloper_t)*op, c_start,
                          c_stride, c_count, c_block) < 0)
    HGOTO_DONE(FAIL);
...
int_f h5scombine_hyperslab_c(hid_t_f *space_id, int_f *op, hsize_t_f *start,
                             hsize_t_f *count, hsize_t_f *stride,
...
/******/
{
  ...
  c_space_id = (hid_t)*space_id;
  c_hyper_id = H5Scombine_hyperslab(c_space_id, c_op, c_start, c_stride,
                                    c_count, c_block);
  if (c_hyper_id < 0)
    goto DONE;
...
int_f h5scombine_select_c(hid_t_f *space1_id, int_f *op, hid_t_f *space2_id,
                          hid_t_f *ds_id)
/******/
{
  int ret_value = -1;
...
int_f h5smodify_select_c(hid_t_f *space1_id, int_f *op, hid_t_f *space2_id)
/******/
{
  int ret_value = -1;
  hid_t c_space1_id;
 ...
 * SOURCE
 */
int_f h5sget_select_type_c(hid_t_f *space_id, int_f *type)
/******/
{
  int ret_value = -1;
  hid_t c_space_id;
...
int_f h5sselect_elements_c(hid_t_f *space_id, int_f *op, size_t_f *nelements,
                           hsize_t_f *coord)
/******/
{
  int ret_value = -1;
  ...
  c_coord =
      (hsize_t *)malloc(sizeof(hsize_t) * (size_t)rank * ((size_t)*nelements));
  if (!c_coord)
    return ret_value;
  for (i = 0; i < (size_t)*nelements; i++) {
    for (j = 0; j < rank; j++) {
      c_coord[(size_t)j + i * (size_t)rank] =
          (hsize_t)coord[(size_t)j + i * (size_t)rank];
    }
  }
...
int_f h5sdecode_c(_fcd buf, hid_t_f *obj_id)
/******/
{
  int ret_value = -1;
...
int_f h5sencode_c(_fcd buf, hid_t_f *obj_id, size_t_f *nalloc, hid_t_f *fapl_id)
/******/
{
  int ret_value = -1;
  unsigned char *c_buf = NULL; /* Buffer to hold C string */
...
int_f h5sextent_equal_c(hid_t_f *space1_id, hid_t_f *space2_id,
                        hid_t_f *c_equal)
/******/
{
  int ret_value = -1;
  ...
  if ((*c_equal =
           (hid_t_f)H5Sextent_equal((hid_t)*space1_id, (hid_t)*space2_id)) < 0)
    return ret_value;
