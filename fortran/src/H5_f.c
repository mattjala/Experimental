 ...
 * SOURCE
 */
int_f h5init_types_c(hid_t_f *types, hid_t_f *floatingtypes,
 ...
 * SOURCE
 */
int_f h5close_types_c(hid_t_f *types, int_f *lentypes, hid_t_f *floatingtypes,
                      int_f *floatinglen, hid_t_f *integertypes,
                      int_f *integerlen)
/******/
{
  int ret_value = -1;
 ...
 * SOURCE
 */
int_f h5init_flags_c(int_f *h5d_flags, size_t_f *h5d_size_flags,
                     int_f *h5e_flags, hid_t_f *h5e_hid_flags,
                     H5ES_status_t *h5es_flags, hid_t_f *h5es_hid_flags,
                     int_f *h5f_flags, int_f *h5fd_flags,
                     hid_t_f *h5fd_hid_flags, int_f *h5g_flags,
                     int_f *h5i_flags, int_f *h5l_flags, int_f *h5o_flags,
                     hid_t_f *h5p_flags, int_f *h5p_flags_int, int_f *h5r_flags,
                     int_f *h5s_flags, hid_t_f *h5s_hid_flags,
                     hsize_t_f *h5s_hsize_flags, int_f *h5t_flags,
                     int_f *h5vl_flags, uint64_t *h5vl_int64_flags,
...
/******/
{
  ...
  h5l_flags[4] = (int_f)
      H5L_SAME_LOC; /* Macro to indicate operation occurs on same location */
  h5l_flags[5] = (int_f)
      H5L_LINK_CLASS_T_VERS; /* Current version of the H5L_class_t struct */
  ...
  h5o_flags[0] =
      (int_f)H5O_COPY_SHALLOW_HIERARCHY_FLAG; /* Copy only immediate members */
  h5o_flags[1] = (int_f)
      H5O_COPY_EXPAND_SOFT_LINK_FLAG; /* Expand soft links into new objects */
  h5o_flags[2] = (int_f)H5O_COPY_EXPAND_EXT_LINK_FLAG; /* Expand external links
                                                          into new objects */
  h5o_flags[3] =
      (int_f)H5O_COPY_EXPAND_REFERENCE_FLAG; /* Copy objects that are pointed by
                                                references */
  h5o_flags[4] = (int_f)
      H5O_COPY_WITHOUT_ATTR_FLAG; /* Copy object without copying attributes */
  h5o_flags[5] =
      (int_f)H5O_COPY_PRESERVE_NULL_FLAG; /* Copy NULL messages (empty space) */
  h5o_flags[6] = (int_f)
      H5O_COPY_ALL; /* All object copying flags (for internal checking) */
  ...
  h5o_flags[8] =
      (int_f)H5O_SHMESG_SDSPACE_FLAG;           /* Simple Dataspace Message.  */
  ...
  h5o_flags[14] =
      (int_f)H5O_HDR_CHUNK0_SIZE; /* 2-bit field indicating # of bytes to store
                                     the size of chunk 0's data */
  h5o_flags[15] = (int_f)
      H5O_HDR_ATTR_CRT_ORDER_TRACKED; /* Attribute creation order is tracked */
  h5o_flags[16] = (int_f)
      H5O_HDR_ATTR_CRT_ORDER_INDEXED; /* Attribute creation order has index */
  h5o_flags[17] =
      (int_f)H5O_HDR_ATTR_STORE_PHASE_CHANGE; /* Non-default attribute storage
                                                 phase change values stored */
  h5o_flags[18] =
      (int_f)H5O_HDR_STORE_TIMES; /* Store access, modification, change & birth
                                     times for object */
  ...
  h5o_flags[25] =
      (int_f)H5O_TYPE_NAMED_DATATYPE;     /* Object is a named data type */
  ...
  h5o_flags[28] =
      (int_f)H5O_INFO_BASIC; /* Fill in the fileno, addr, type, and rc fields */
  h5o_flags[29] = (int_f)
   ...
   * These flags determine which fields will be filled in in the
   * H5O_native_info_t struct.
   */
  h5o_flags[31] = (int_f)
      H5O_NATIVE_INFO_ALL; /* (H5O_NATIVE_INFO_HDR|H5O_NATIVE_INFO_META_SIZE) */
  h5o_flags[32] = (int_f)H5O_NATIVE_INFO_HDR; /* Fill in the hdr field */
  h5o_flags[33] =
      (int_f)H5O_NATIVE_INFO_META_SIZE; /* Fill in the meta_size field */
                                        /*
                                         *  H5P flags
                                         */
  ...
  h5_generic_flags[0] =
      (int_f)H5_INDEX_UNKNOWN; /* Unknown index type			*/
  h5_generic_flags[1] =
      (int_f)H5_INDEX_NAME; /* Index on names 			*/
  h5_generic_flags[2] =
      (int_f)H5_INDEX_CRT_ORDER;           /* Index on creation order 		*/
  ...
  h5_generic_flags[7] =
      (int_f)H5_ITER_NATIVE; /* No particular order, whatever is fastest */
...
int_f h5init1_flags_c(int_f *h5lib_flags)
/******/
{
  int ret_value = -1;
 ...
 *
 */
int_f h5get_libversion_c(int_f *majnum, int_f *minnum, int_f *relnum)
/******/
{
 ...
 * SOURCE
 */
int_f h5check_version_c(int_f *majnum, int_f *minnum, int_f *relnum)
/******/
{
  int ret_value = -1;
  unsigned c_majnum, c_minnum, c_relnum;
 ...
 * SOURCE
 */
int_f h5garbage_collect_c(void)
/******/
{
  int ret_value = -1;
