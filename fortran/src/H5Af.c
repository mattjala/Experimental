 ...
 * SOURCE
 */
int_f h5adelete_c(hid_t_f *obj_id, _fcd name, size_t_f *namelen)
/******/
{
 ...
 * SOURCE
 */
int_f h5aget_num_attrs_c(hid_t_f *obj_id, int_f *attr_num)
/******/
{
 ...
 * SOURCE
 */
int_f h5aget_name_c(hid_t_f *attr_id, size_t_f *bufsize, _fcd buf)
/******/
{
  size_t c_bufsize;
 ...
 * SOURCE
 */
int_f h5adelete_by_name_c(hid_t_f *loc_id, _fcd obj_name, size_t_f *obj_namelen,
                          _fcd attr_name, size_t_f *attr_namelen,
                          hid_t_f *lapl_id)
/******/
{
   ...
   */
  if (H5Adelete_by_name((hid_t)*loc_id, c_obj_name, c_attr_name,
                        (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5adelete_by_idx_c(hid_t_f *loc_id, _fcd obj_name, size_t_f *obj_namelen,
                         int_f *idx_type, int_f *order, hsize_t_f *n,
                         hid_t_f *lapl_id)
/******/
{
  char *c_obj_name = NULL; /* Buffer to hold C string */
   ...
   * Call H5Adelete_by_name function.
   */
  if (H5Adelete_by_idx((hid_t)*loc_id, c_obj_name, (H5_index_t)*idx_type,
 ...
 *                          H5_ITER_NATIVE   - No particular order, whatever is
 * fastest H5_ITER_N	     - Number of iteration orders
 *
 *  n  - Attribute's position in index
 *  attr_id  - Attribute identifier
 *  size  - Buffer size ! *TEST* check for 0 value *CHECK* should this return
 * the correct value
 *
 *  lapl_id   - Link access property list
 *  hdferr   - Error code:
 ...
 * SOURCE
 */
int_f h5aget_name_by_idx_c(hid_t_f *loc_id, _fcd obj_name,
                           size_t_f *obj_namelen, int_f *idx_type, int_f *order,
                           hsize_t_f *n, _fcd name, size_t_f *size,
                           hid_t_f *lapl_id)
/******/
{
  char *c_obj_name = NULL; /* Buffer to hold C string */
   ...
   */
  c_size = H5Aget_name_by_idx((hid_t)*loc_id, c_obj_name, (H5_index_t)*idx_type,
                              (H5_iter_order_t)*order, (hsize_t)*n, c_buf,
                              c_buf_size, (hid_t)*lapl_id);
  if (c_size < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5aget_info_c(hid_t_f *loc_id, int_f *corder_valid, int_f *corder,
 ...
 * SOURCE
 */
int_f h5aget_info_by_idx_c(hid_t_f *loc_id, _fcd obj_name,
                           size_t_f *obj_namelen, int_f *idx_type, int_f *order,
                           hsize_t_f *n, hid_t_f *lapl_id, int_f *corder_valid,
...
/******/
{
   ...
   */
  if (H5Aget_info_by_idx((hid_t)*loc_id, c_obj_name, (H5_index_t)*idx_type,
 ...
 * SOURCE
 */
int_f h5aget_info_by_name_c(hid_t_f *loc_id, _fcd obj_name,
...
/******/
{
   ...
   */
  if (H5Aget_info_by_name((hid_t)*loc_id, c_obj_name, c_attr_name, &ainfo,
                          (hid_t)*lapl_id) < 0)
    HGOTO_DONE(FAIL);
 ...
 * SOURCE
 */
int_f h5awrite_f_c(hid_t_f *attr_id, hid_t_f *mem_type_id, void *buf)
/******/
{
  int ret_value = -1;
  herr_t ret;
 ...
 * SOURCE
 */
int_f h5aread_f_c(hid_t_f *attr_id, hid_t_f *mem_type_id, void *buf)
/******/
{
  int ret_value = -1;
  herr_t ret;
