...
void writeToFiles(const char *c_typedef, const char *fortran_type,
                  const char *c_type, int kind);
void writeToCFileOnly(const char *c_typedef, const char *fortran_type,
                      const char *c_type, int size);
void writeToFilesChr(const char *c_typedef, const char *fortran_type,
                     const char *c_type, int size, const char *kind);

static void initCfile(void) {
  fprintf(
      c_header,
      "/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *\n\
 * Copyright by The HDF Group.                                               *\n\
...
static void endCfile(void) {
  fprintf(c_header, "\n#endif /* _H5f90i_gen_H */\n");
}
static void endFfile(void) {
  fprintf(fort_header, "\n        INTEGER(SIZE_T), PARAMETER :: "
...
int main(void) {
  ...
  fprintf(fort_header,
          "        INTEGER, PARAMETER :: H5_FORTRAN_NUM_INTEGER_KINDS = %d\n",
  ...
  for (i = 0; i < H5_FORTRAN_NUM_REAL_KINDS; i++) {
    ...
    /* Don't select a higher precision than Fortran can support */
             ...
             found_long_double == 1 && H5_PAC_FC_MAX_REAL_PRECISION > 28) {
    ...
    /* Don't select a higher precision than Fortran can support */
      ...
      printf("\n                      **** HDF5 WARNING ****\n");
      printf("Fortran REAL(KIND=%d) is %d Bytes, but no corresponding C float "
             "type exists of that size\n",
             RealKinds[i], RealKinds_SizeOf[i]);
      printf("     !!! Fortran interfaces will not be generated for "
             "REAL(KIND=%d) !!!\n\n",
             RealKinds[i]);
  ...
  /* int */
  writeToFiles("int", "Fortran_INTEGER", "int_f",
               H5_FORTRAN_NATIVE_INTEGER_KIND);
  ...
  fprintf(fort_header,
          "        INTEGER, DIMENSION(1:%d), PARAMETER :: "
          "Fortran_INTEGER_AVAIL_KINDS = (/",
          FORTRAN_NUM_INTEGER_KINDS);
  ...
  if (H5_FORTRAN_NATIVE_REAL_SIZEOF == sizeof(long double)) {
    writeToFilesChr("float", "Fortran_REAL", "real_f",
                    H5_FORTRAN_NATIVE_REAL_KIND, "C_LONG_DOUBLE");
  } else
#endif
      if (H5_FORTRAN_NATIVE_REAL_SIZEOF == sizeof(double)) {
    writeToFilesChr("float", "Fortran_REAL", "real_f",
                    H5_FORTRAN_NATIVE_REAL_KIND, "C_DOUBLE");
  } else if (H5_FORTRAN_NATIVE_REAL_SIZEOF == sizeof(float))
    writeToFilesChr("float", "Fortran_REAL", "real_f",
                    H5_FORTRAN_NATIVE_REAL_KIND, "C_FLOAT");
  else {
  ...
  if (H5_FORTRAN_NATIVE_DOUBLE_SIZEOF == sizeof(long double)) {
    writeToFilesChr("float", "Fortran_DOUBLE", "double_f",
                    H5_FORTRAN_NATIVE_DOUBLE_KIND, "C_LONG_DOUBLE");
  } else
#endif
      if (H5_FORTRAN_NATIVE_DOUBLE_SIZEOF == sizeof(double)) {
    writeToFilesChr("float", "Fortran_DOUBLE", "double_f",
                    H5_FORTRAN_NATIVE_DOUBLE_KIND, "C_DOUBLE");
  } else if (H5_FORTRAN_NATIVE_DOUBLE_SIZEOF == sizeof(float))
    writeToFilesChr("float", "Fortran_DOUBLE", "double_f",
                    H5_FORTRAN_NATIVE_DOUBLE_KIND, "C_FLOAT");
#ifdef H5_HAVE_FLOAT128
  /* Don't select a higher precision than Fortran can support */
  else if (sizeof(__float128) == H5_FORTRAN_NATIVE_DOUBLE_SIZEOF &&
           H5_PAC_FC_MAX_REAL_PRECISION > 28) {
    writeToFilesChr("float", "Fortran_DOUBLE", "double_f",
                    H5_FORTRAN_NATIVE_DOUBLE_KIND, "Fortran_REAL_C_FLOAT128");
  }
#else
  else if (sizeof(long double) == H5_FORTRAN_NATIVE_DOUBLE_SIZEOF &&
           H5_PAC_FC_MAX_REAL_PRECISION > 28) {
    writeToFilesChr("float", "Fortran_DOUBLE", "double_f",
                    H5_FORTRAN_NATIVE_DOUBLE_KIND, "Fortran_REAL_C_FLOAT128");
  }
  ...
  fprintf(fort_header,
          "        INTEGER, PARAMETER :: H5R_DSET_REG_REF_BUF_SIZE_F = %u\n",
          H5_SIZEOF_HADDR_T + 4);
  ...
  fprintf(fort_header,
          "        INTEGER, PARAMETER :: H5O_MAX_TOKEN_SIZE_F = %u\n",
          H5O_MAX_TOKEN_SIZE);
