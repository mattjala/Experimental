...
static struct mirror_session *session_init(struct mirror_writer_opts *opts) {
  ...
  session->loginfo =
      mirror_log_init(opts->logpath, "W- ", MIRROR_LOG_DEFAULT_VERBOSITY);
...
static int do_open(struct mirror_session *session,
  ...
  mirror_log(session->loginfo, V_INFO,
             "to open file %s (flags %d) (maxaddr %d)", xmit_open->filename,
             _flags, _maxaddr);
 ...
 * ---------------------------------------------------------------------------
 */
static int do_set_eoa(struct mirror_session *session,
                      const unsigned char *xmit_buf) {
  size_t decode_ret = 0;
...
static int do_write(struct mirror_session *session,
  ...
  do {
    if ((nbytes_in_packet =
             HDread(session->sockfd, buf, H5FD_MIRROR_DATA_BUFFER_MAX)) < 0) {
    ...
    if (HEXDUMP_WRITEDATA) {
      ...
      mirror_log_bytes(session->loginfo, V_ALL, (size_t)nbytes_in_packet,
                       (const unsigned char *)buf);
      mirror_log(session->loginfo, V_ALL, "```");
    }

    mirror_log(session->loginfo, V_INFO, "writing %zd bytes at %zu",
               nbytes_in_packet, (addr + sum_bytes_written));

    if (H5FDwrite(session->file, type, H5P_DEFAULT, (addr + sum_bytes_written),
...
static int receive_communique(struct mirror_session *session,
  ...
  assert((session != NULL) && (session->magic == MW_SESSION_MAGIC) &&
         (comm != NULL) && (comm->magic == MW_SOCK_COMM_MAGIC) &&
         (comm->xmit_recd != NULL) && (comm->raw != NULL) &&
  ...
  if ((read_ret = HDread(session->sockfd, comm->raw,
                         H5FD_MIRROR_XMIT_BUFFER_MAX)) < 0) {
  ...
  decode_ret =
      H5FD_mirror_xmit_decode_header(X, (const unsigned char *)comm->raw);
  ...
  if (session->xmit_count != X->xmit_count) {
    mirror_log(session->loginfo, V_ERR, "xmit_count mismatch exp:%d recd:%d",
