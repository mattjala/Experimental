...
static void run_command(int argc, char **argv, char *cmdline,
                        const char *fname);
static void add_executable(int argc, char **argv, char *cmdstring, int *f_index,
                           int f_count);
...
static void create_default_separators(struct distribute_option *option,
  ...
  MPI_Allreduce(&local_max_file_size, global_max_file_size, 1, MPI_UINT64_T,
  ...
  uint64_t max_magnitude_bin =
      (uint64_t)((ceil(log2((double)(*global_max_file_size)) / 10)) * 10);
...
static int h5dwalk_map_fn(mfu_flist flist __attribute__((unused)), uint64_t idx,
                          int ranks, void *args __attribute__((unused))) {
...
static int print_flist_distribution(int file_histogram,
  ...
  uint64_t *disttotal =
      (uint64_t *)MFU_MALLOC((separators + 1) * sizeof(uint64_t));
  MPI_Allreduce(dist, disttotal, (int)(separators + 1), MPI_UINT64_T, MPI_SUM,
                MPI_COMM_WORLD);
  ...
  if (rank == 0) {
    ...
    for (size_t i = 0; i <= separators; i++) {
      ...
      if (i == 0) {
        ...
        mfu_format_bytes((uint64_t)option->separators[i - 1], &size_tmp,
                         &size_units);
      ...
      if (file_histogram) {
        mfu_format_bytes((uint64_t)option->separators[i], &size_tmp,
                         &size_units);
        number = disttotal[i];
        mfu_format_bytes((uint64_t)option->separators[i], &size_tmp,
                         &size_units);
...
/* * Search the right position to insert the separator * If the separator exists
 * already, return failure * Otherwise, locate the right position, and move the
 * array forward to save the separator.
 */
...
static void usage(void) {
  ...
  PRINTVALSTREAM(rawoutstream, "  -E, --error  <file>     - write processed "
                               "errors to file in text format\n");
  PRINTVALSTREAM(rawoutstream,
                 "  -l, --log_text <dir>    - write individual tool outputs to "
                 "a file. Logs can be written to an "
...
static void print_file(mfu_flist flist, uint64_t idx) {
  ...
  if (mfu_flist_have_detail(flist)) {
    ...
    size_t access_rc =
        strftime(access_s, sizeof(access_s) - 1, "%FT%T", localtime(&access_t));
    size_t modify_rc = strftime(modify_s, sizeof(modify_s) - 1,
                                "%b %e %Y %H:%M", localtime(&modify_t));
    size_t create_rc =
        strftime(create_s, sizeof(create_s) - 1, "%FT%T", localtime(&create_t));
...
/* print information about a file given the index and rank (used in print_files)
 */
static size_t print_file_text(mfu_flist flist, uint64_t idx, char *buffer,
                              size_t bufsize) {
  size_t numbytes = 0;
  ...
  if (mfu_flist_have_detail(flist)) {
    ...
    numbytes = (size_t)snprintf(buffer, bufsize, "%s %s %s %7.3f %3s %s %s\n",
                                mode_format, username, groupname, size_tmp,
                                size_units, modify_s, file);
  } else {
    /* get type */
    mfu_filetype type = mfu_flist_file_get_type(flist, idx);
    char *type_str = type_str_unknown;
    ...
    numbytes =
        (size_t)snprintf(buffer, bufsize, "Type=%s File=%s\n", type_str, file);
  }
...
static void dh5tool_flist_write_text(const char *name, mfu_flist bflist) {
  ...
  int mpirc =
      MPI_File_open(MPI_COMM_WORLD, (const char *)name, amode, info, &fh);
  if (mpirc != MPI_SUCCESS) {
    MPI_Error_string(mpirc, mpierrstr, &mpierrlen);
    MFU_ABORT(1, "Failed to open file for writing: `%s' rc=%d %s", name, mpirc,
              mpierrstr);
  }
  ...
  if (mpirc != MPI_SUCCESS) {
    MPI_Error_string(mpirc, mpierrstr, &mpierrlen);
    MFU_ABORT(1, "Failed to truncate file: `%s' rc=%d %s", name, mpirc,
              mpierrstr);
  }
  ...
  if (mpirc != MPI_SUCCESS) {
    MPI_Error_string(mpirc, mpierrstr, &mpierrlen);
    MFU_ABORT(1, "Failed to set view on file: `%s' rc=%d %s", name, mpirc,
              mpierrstr);
  }
  ...
  while (all_iters > 0) {
    ...
    mpirc = MPI_File_write_at_all(fh, write_offset, ptr, write_count, MPI_BYTE,
                                  &status);
    if (mpirc != MPI_SUCCESS) {
      MPI_Error_string(mpirc, mpierrstr, &mpierrlen);
      MFU_ABORT(1, "Failed to write to file: `%s' rc=%d %s", name, mpirc,
                mpierrstr);
    }
...
static void filter_hdf_files(mfu_flist *pflist, char *regex_exp, int exclude,
                             int name) {
  mfu_flist flist = *pflist;
...
static int fill_file_list(mfu_flist new_flist, const char *config_filename,
                          int myrank, int size) {
  int index = 0;
  ...
  while (HDfgets(linebuf, sizeof(linebuf), config) != NULL) {
    ...
    if (HDstat(linebuf, &statbuf) == 0) {
      if (myrank == (index % size)) {
        mfu_flist_insert_stat((flist_t *)new_flist, linebuf, O_RDONLY,
                              &statbuf);
      }
      index++;
    }
...
static int count_dirpaths(int argc, int startcnt, const char *argv[],
                          int **index_out) {
  int k;
  int path_cnt = 0;
...
static char **copy_args(int argc, const char *argv[], int *mfu_argc,
                        int *copy_len) {
...
static void run_command(int argc __attribute__((unused)), char **argv,
                        char *cmdline, const char *fname) {
  char filepath[1024];
  ...
  if (log_output_in_single_file || use_stdout) {
        ...
        if ((nbytes = (size_t)read(pipefd[0], &buf[offset], remaining)) > 0) {
          ...
          if (remaining == 0) {
            ...
            printf("[%d] Allocate-2 a new read buffer:: buft_count=%d\n",
                   sg_mpi_rank, buft_count);
#endif
            bufs[buft_count++] = thisbuft =
                (buf_t *)MFU_CALLOC(1, sizeof(buf_t));
            assert(thisbuft != NULL);
            thisbuft->buf = MFU_MALLOC(BUFT_SIZE);
            thisbuft->bufsize = BUFT_SIZE;
            thisbuft->dt = MPI_CHAR;
    ...
    if (txtlog == NULL) {
      if ((log_instance > 0) || processing_inputfile) {
        ...
        HDsnprintf(logpath, sizeof(logpath), "%s/%s_%s.log",
                   HDgetcwd(current_dir, sizeof(current_dir)), logbase,
                   thisapp);
      }
    } else {
      ...
      if ((log_instance > 0) || processing_inputfile) {
        if (processing_inputfile)
          log_instance = current_input_index;
        if (txtlog[log_len - 1] == '/')
          HDsnprintf(logpath, sizeof(logpath), "%s%s_%s.log_%d", txtlog,
                     logbase, thisapp, log_instance);
        else
          HDsnprintf(logpath, sizeof(logpath), "%s/%s_%s.log_%d", txtlog,
                     logbase, thisapp, log_instance);
      } else {
        if (txtlog[log_len - 1] == '/')
          HDsnprintf(logpath, sizeof(logpath), "%s%s_%s.log", txtlog, logbase,
                     thisapp);
        else
          HDsnprintf(logpath, sizeof(logpath), "%s/%s_%s.log", txtlog, logbase,
                     thisapp);
    ...
    if (log_errors_in_file) {
      /* We co-locate the error logs in the same directories as the regular log
       * files. The easiest way to do this is to simply replace the .log with
       * .err in a copy of the logpath variable.
       */
...
int MFU_PRED_PRINT(mfu_flist flist, uint64_t idx,
...
static void add_executable(int argc, char **argv, char *cmdstring, int *f_index,
                           int f_count __attribute__((unused))) {
  char cmdline[2048];
  HDsnprintf(cmdline, sizeof(cmdline), "\n---------\nCommand: %s\n", cmdstring);
  argv[argc] = NULL;
...
int main(int argc, char *argv[]) {
  ...
  if (use_config_file > 0) {
    ...
    paths =
        (mfu_param_path *)MFU_MALLOC((size_t)numpaths * sizeof(mfu_param_path));
    mfu_src_file = mfu_file_new();

    /* process each path */
    mfu_param_path_set_all((uint64_t)numpaths, (const char **)argpaths, paths,
                           mfu_src_file, true);
